...
In table \ref{tab.n200} we report simulation results under the same data generating processes than in the main article but with $T=p=200$ instead.

\begin{table}[h!]\setlength\extrarowheight{-0pt}
	\begin{center}
		\begin{tabular}{|l|ccc|ccc|}
			\hline \multicolumn{7}{|c|}{$T=p=200$}\\
			\hline 
...
0.4&	0.9995&	0.9990 &	0.9940&	0.9850	&0.9685	&0.9020\\
...
0.4&	1.0000&	0.9995&	0.9930&	0.9900&	0.97650&	0.9035\\
...
0.4 &	1.0000&	0.9990&	0.9920&	0.9905&	0.9765&	0.9050\\
		...
		\end{tabular}
	\end{center}
	\caption{Rejection probabilities with $T=p=200$ for the three designs.}
	\label{tab.n200}
\end{table}
...
where $$\widehat{W}(\mu,e) =\left(\widehat{W}_1(\mu,e), \dots, \widehat{W}_p(\mu,e)\right)^\top,\ \text{with } \widehat{W}_j(\mu,e)= \frac{1}{\sqrt{T}}\sum_{t=1}^T\widehat{u}_{tj} \widehat{\epsilon}_{\frac{2}{\sqrt{T}}\mu,t} e_t .$$
...
\mathcal{S}_T^{(5)}&= \left\{\left\|\frac{U^\top \mathcal{E}}{T}\right\|_\infty\le s_T^{(5)}\right\},
 \end{align*}
 where $\widetilde{\varepsilon}_t$ denotes the $t^{th}$ element of  $\left(I_T-\widehat{P}\right)\mathcal{E}$ and $\widetilde{f}_t$ is the $K\times 1$ vector corresponding to the $t^{th}$ row of $\left(I_T-\widehat{P}\right)F$ and we recall that $\varphi^*=\gamma^*-B^\top \beta^*$.
...
\end{Lemma}
\begin{Proof} The result is a direct consequence of Lemma \ref{lm.highdimclt} applied to $Z_t = u_t\varepsilon_t$ (and the constant $\bar C$ used in the definition of $s_T^{(9)}$ is introduced in Lemma \ref{lm.highdimclt}). Condition \eqref{lhdi} of Lemma \ref{lm.highdimclt} is satisfied with $\zeta_1=\theta_1/2$ by Lemma \ref{lm.2exp}. Assumption \ref{as.mixing} implies that condition \eqref{lhdii} holds with $\zeta_2=\theta_2$ Condition \eqref{lhdiii} holds with $\zeta=\theta=(2\theta_1^{-1}+\theta_2^{-1})^{-1}$, since, by Assumption \ref{as.mixing}, $2\theta_1^{-1}+\theta_2^{-1}>1$. Concerning condition \eqref{lhdiv}, note that, by Assumption \ref{as.tail} \eqref{taili} and \eqref{tailiv}, we have 
\begin{align*}\E\left[\left(\frac{1}{\sqrt{T}} \sum_{t=1}^T u_t\varepsilon_t\right)\left(\frac{1}{\sqrt{T}} \sum_{t=1}^T u_t\varepsilon_t\right) ^\top \right]&=\frac{1}{T} \sum_{t=1}^T\sum_{s=1}^T\E\left[ u_t\varepsilon_tu_{s}^\top\varepsilon_{s} \right]\\
&=\frac{1}{T} \sum_{t=1}^T\sum_{s=1}^T\E\left[ u_tu_{s}^\top \right]\E\left[\varepsilon_t\varepsilon_{s} \right]\\
&= \E\left[ u_tu_t^\top\right]\E\left[\varepsilon_t^2 \right]=\Sigma\E\left[\varepsilon_t^2\right].\end{align*}
This implies that $$\sigma_{p}\left(\E\left[\left(\frac{1}{\sqrt{T}} \sum_{t=1}^T u_t\varepsilon_t\right)\left(\frac{1}{\sqrt{T}} \sum_{t=1}^T u_t\varepsilon_t\right) ^\top \right]\right)= \sigma_{p}\left(\Sigma\E\left[\varepsilon_t^2\right]\right)\ge \kappa_1\E\left[\varepsilon_t^2\right]=\sigma_*^2>0,$$
...
Remark that, by Assumption \ref{as.tail} \eqref{tailii} and \eqref{tailiv}, $$\kappa_2 \E\left[\varepsilon_t^2\right]> \E\left[u_{tj}u_{tj}^\top\right]\E\left[\varepsilon_t^2\right]\ge \kappa_1 \E\left[\varepsilon_t^2\right]>0$$
for all $j\in[p]$.
...
\begin{Lemma} \label{lm.seq} Under Assumption \ref{as.Rates}, we have \begin{enumerate}[\textup{(}i\textup{)}]  \item\label{lsi} $s_T^{(12)}\sqrt{1\vee \log\left(2p/s_T^{(12)}\right)}\to 0$;  \item\label{lsii} $2T^{-1/2}s_T^{(11)}+s_T^{(2)}+s_T^{(5)}=O\left(\sqrt{\log(T\vee p)/(T\wedge p)}\right)$;  \item\label{lsiii} $\sqrt{T}s_T^{(2)}\sqrt{1\vee \log\left(\frac{2p}{\sqrt{T}s_T^{(2)}}\right)}\to 0$;  \item\label{lsiv} $s_T^{(8)}\to 0$;\item\label{lsv} $s_T^{(9)}\to 0$; \item\label{lsvi}$s_T^{(13)}\sqrt{1\vee \log\left(2p/s_T^{(13)}\right)}\to 0.$
\end{enumerate}
\end{Lemma}
\begin{Proof}
...
We also have \begin{equation}\label{seq3} \left(s_T^{(6)}\right)^2s_T^{(11)}=O\left(\left(\frac{\log(T\vee p)^{6}}{T}\|\beta^*\|_1\right)^{1/2}\right),\end{equation}
 ...
 \begin{equation}\label{seq4}s_T^{(2)}=O\left(\frac{\log(T\vee p)^{3/2}}{T\wedge p}\right)(\|\varphi^*\|_2\vee1),\end{equation}
  so that \begin{equation}\label{seq5}s_T^{(6)}s_T^{(2)}=o\left(s_T^{(2)}\right),\end{equation}
  since $s_T^{(6)}=o(1)$ by \eqref{seq1} and Assumption \ref{as.Rates} \eqref{rateii}. Moreover, it holds that $$s_T^{(4)}=O\left(\frac{\log(T\vee p)^{\frac32+\frac2{\theta_1}}}{T\wedge p}\left(\|\varphi^*\|_2^2\vee 1\right)\right)$$
 and, therefore, \begin{equation}\label{seq6} s_T^{(7)}=O\left(\sqrt{\frac{\log(T\vee p)^{\frac52+\frac2{\theta_1}}}{T(T\wedge p)}}\right)\left(\|\varphi^*\|_2\vee 1\right).\end{equation} Recall that
...
\begin{align}\notag  s_T^{(12)}&=O\left(\left( \sqrt{\frac{\log(T\vee p)^{6}}{T}}\|\beta^*\|_1\right)^{1/2}+ \left(\frac{\log(T\vee p)^{3/2}\sqrt{T}}{(T\wedge p)}+\sqrt{\frac{\log(T\vee p)^{\frac52+\frac2{\theta_1}}}{T(T\wedge p)}}\right)(\|\varphi^*\|_2\vee1)\right)\\
\label{seq7}&= O\left(\left( \sqrt{\frac{\log(T\vee p)^{6}}{T}}\|\beta^*\|_1\right)^{1/2}+ \frac{\log(T\vee p)^{3/2}\sqrt{T}}{(T\wedge p)}\left(\sqrt{\frac{\log(T\vee p)^{\frac2{\theta_1}}}{T}}+1\right)\left(\|\varphi^*\|_2\vee1\right)\right).\end{align}
Additionally, we have $(T(T\wedge p))^{-1/2}=o\left(s_T^{(7)}\right)=O\left(s_T^{(12)}\right)$ so that $\log\left(2p/s_T^{(12)}\right)=O\left(\log(2p)+\log\left(\sqrt{T(T\wedge p)}\right)\right)=O(\log(T\vee p))$. This and \eqref{seq7} imply
\begin{align*}&s_T^{(12)}\sqrt{1\vee \log\left(2p/s_T^{(12)}\right)}\\
&=O\left(\left(\sqrt{\frac{\log(T\vee p)^{8}}{T}}\|\beta^*\|_1\right)^{1/2}+ \frac{\log(T\vee p)^{2}\sqrt{T}}{(T\wedge p)}\left(\sqrt{\frac{\log(T\vee p)^{\frac2{\theta_1}}}{T}}+1\right)\left(\|\varphi^*\|_2\vee1\right)\right)=o(1),\end{align*}
...
\noindent \textit{Proof of \eqref{lsiii}.} We have $(T\wedge p)^{-1}=o\left(\sqrt{T}s_T^{(2)}\right)$, hence $\left(\frac{2p}{\sqrt{T}s_T^{(2)}}\right)=O(\log(T\vee p))$, so that 
$$\sqrt{T}s_T^{(2)}\sqrt{1\vee \left(\frac{2p}{\sqrt{T}s_T^{(2)}}\right)}=O\left(\frac{\log(T\vee p)^{5/2}\sqrt{T}}{T\wedge p}\right)(\|\varphi^*\|_2\vee1) =o(1),$$
...
Next, by Assumptions \ref{as.tail} \eqref{taili}, \eqref{tailii} and \ref{as.moments} \eqref{f4i}, we have
\begin{align}
\notag  &\max_{j\in[p],k\in[K]}\left| \sum_{t=1}^T\E\left[u_{tj}\left(\sum_{\ell=1}^pu_{t\ell}b_{\ell k}\right)\right]\right|      \\
 \notag & \le T \max_{j\in[p],k\in[K]}\sum_{\ell=1}^p\left|\E\left[u_{tj}u_{t\ell}b_{\ell k}\right]\right|\\
 \notag &= T \max_{j\in[p],k\in[K]}\sum_{\ell=1}^p\left|\E\left[u_{tj}u_{t\ell}\right]\right|\left| \E\left[b_{\ell k}\right]\right|\\
 \label{2407soir2}&\le  TM \max_{j\in[p]}\sum_{\ell=1}^p\left|\E\left[u_{tj}u_{t\ell}\right]\right|=TM \max_{j\in[p]}\sum_{\ell=1}^p|\Sigma_{j\ell}| <TM\kappa_2.
\end{align}
