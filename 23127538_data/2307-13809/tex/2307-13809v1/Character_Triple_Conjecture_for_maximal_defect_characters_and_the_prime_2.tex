...
\\
\date{}
Large part of this work was carried out during a stay of the author at the Mathematisches Forschungsinstitut Oberwolfach funded by a Leibniz Fellowship.}}
...
%IMPORTANT!!!!
%\end{center}
%The reduction for the inductive Galois--Alperin--McKay Conjecture does not work because of issues with the reduction to $\O_p(G)=1$. Here, we want to use the inductive Galois--Alperin--McKay Conjecture for quasi-simple groups (i.e. the inductive Galois--Alperin--McKay condition) to deduce the Character Triple Conjecture for maximal defect characters (which theoretically is equivalent to the the inductive Galois--Alperin--McKay Conjecture, although the implication would still require to reduce to the case $\O_p(G)=1$). 
%\begin{center}
%HOWEVER!!!
%\end{center}
%the reduction for the CTC to the case $\O_p(G)=1$ is automatic!!!!! Why do we care about CTC and not only Dade (because CTC is the ultimate conjecture!!!). If we now assume, as we may, that $\O_p(G)$ then we can obtain the iGAM for $G$ by proceeding as in the paper that I wrote on the iGAM (project with Noelia).

%\textbf{PROBLEM?} The inductive hypothesis will give you GALOIS-CTC for maximal defect but not iGAM. Nevertheless, the Galois-CTC for maximal defect implies the iGAM (and observe that the proof of Theorem \ref{thm:CTC implies iAM} works for maximal defect characters also without the reduction of iGAM to $\O_p(G)=1$, in fact we can state the max-defect CTC for all non-trivial defect groups as this yields $d>0$). This argument sounds cyclical, but there might be a double induction argument that works.

%\textbf{SOLUTION?} Try to reduce by induction to the case where iGAM holds for every involved quasi-simple group and that both iGAM and Galois-CTC holds for groups $H$ with $|H:\z(H)|<|G:\z(G)|$. Observe that the proof of Theorem \ref{thm:CTC implies iAM} if we consider just the Galois-CTC-max (with all non-trivial defect groups allowed, remember this yields $d>0$) shows that if Galois-CTC holds for such an $H$, then iGAM holds for the same $H$ (i.e. the argument is not inductive anymore but works group by group).

%\textbf{ANOTHER PROBLEM?} Now we still want to apply the argument in the iGAM reduction (with Noelia) however we need the result there for the DGN. The proof given in \cite{Nav-Spa14I} will not generalize to the Galois setting since it uses central extension to reduce to the case where characters extends. However we already know that the result holds for $\mathcal{H}=1$. Then another approach would be to deduce the Galois version of the DGN with character triples isomorphisms, say Galois iDGN, from the non-Galois iDGN that holds by \cite{Nav-Spa14I}.

%\textbf{ANOTHER PROBLEM?} This is not necessarily a problem but more a hint that this is to much to ask. If we manage to show that iGAM for quasi-simple implies Galois-CTC-max, since we can also show that Galois-CTC-max implies iGAM, we deduce that we can prove iGAM reduces to quasi-simple. This is what I wanted to show initially with Noelia.

%ASK LUCAS how far is iGAM for $p=2$ (we could do this together) with something like this we can apply for research in pairs at the IAS maybe with Mandi as well


%IN ANY CASE WE CAN PROVE THE (NONGALOIS-)CTC for maximal defect and the prim $2$.
%}}
...
The arguments used to prove the above results can be easily adapted to obtain analogous block-free statements. In particular, using recent result of Sp\"ath \cite{Spa23}, we are able to show that the block-free form of the Character Triple Conjecture holds at the prime $3$ for characters of degree coprime to $3$ (see Theorem \ref{thm:CTC block-free p=3}) and hence for every finite group with abelian Sylow $3$-subgroups (see Corollary \ref{cor:CTC block-free, abelian Sylow 3}). This will follow from a reduction of the block-free form of the Character Triple Conjecture to the verification of the inductive McKay condition for (the universal covering group of) non-abelian finite simple groups (see Theorem \ref{thm:CTC non-blockwise for maximal defect follows from iMcK}).
...
\[\Omega:\C^d(G,\O_p(G))_+/G\to\C^d(B,\O_p(G))_-/G\]
such that
\[\left(A_{\sigma,\vartheta},G_\sigma,\vartheta\right)\iso{G}\left(A_\rho,G_\rho,\chi\right)\]
...
\begin{rmk}
In some of the arguments given in Section \ref{sec:iAM implies CTC} will be useful to consider normal $p$-chains. A $p$-chain $\sigma$ is said to be normal if each term $D_i$ is normal in the largest term $D(\sigma)$. Proceeding as in the proof of \cite[Proposition 6.10]{Spa17}, and following previous ideas introduced by Kn\"orr and Robinson (see \cite[Proposition 3.3]{Kno-Rob89}), it follows that when dealing with Conjecture \ref{conj:CTC} it is no loss of generality to assume that each $p$-chain considered in the definition of $\C^d(B,\O_p(G))_\pm$ is normal. For these reasons, we will keep using normal $p$-chains throughout the rest of the paper without further reference. This approach was also used in \cite{Ros22} without any comment.
\end{rmk}

We recall that Conjecture \ref{conj:CTC} implies Dade's Extended Projective Conjecture \cite[4.10]{Dad97} according to \cite[Proposition 6.4]{Spa17} and, as mentioned already in the introduction, that it should be understood as an analogue of the final Dade's Inductive Conjecture \cite[5.8]{Dad97}. In fact, it was announced long ago that the latter would reduce to quasi-simple groups although a proof of this result has not yet appeared. Nevertheless, it was shown in \cite[Theorem 1.3]{Spa17} that the if the Character Triple Conjecture holds for quasi-simple groups then the weaker Dade's Projective Conjecture holds for every finite group. A final reduction of the Character Triple Conjecture to quasi-simple group is currently in preparation \cite{Ros-Reduction_CTC}. Regarding the state of the art of the Character Triple Conjecture, we refer the reader to \cite[Section 9]{Spa17} for the case of sporadic groups, special linear groups of degree $2$, and blocks with cyclic defect, to \cite{Ros22} for the case of $p$-solvable groups, and to the series of papers \cite{Ros-Generalized_HC_theory_for_Dade}, \cite{Ros-Clifford_automorphisms_HC}, \cite{Ros-Unip}, and \cite{Ros-Homotopy} for the case of finite simple groups of Lie type in non-defining characteristic.
...
If Character Triple Conjecture holds for maximal defect characters at the prime $p$, then the inductive Alperin--McKay condition (in the generality considered in Conjecture \ref{conj:iAM}) holds at the prime $p$.
\end{theo}
...
&=|\C^d_1(B,\O_p(G))/G|
\end{align*}
and therefore there exists some element $(\sigma_0,\vartheta_0)$ of $\C^d_0(B,\O_p(G))$ whose $G$-orbit is mapped via $\Omega$ outside the set $\C^d_1(B,\O_p(G))/G$. Now, we proceed as follows: noticing that $\Omega(\overline{(\sigma_0,\vartheta_0)})$ belongs to $\mathcal{J}_-^d/G$, we can apply the the inverse of the bijection $\Pi$ and define
...
While we can obtain the block-free form of the Character Triple Conjecture for the prime $2$ and maximal defect characters as a consequence of Theorem \ref{thm:Main, CTC/DPC-max for p=2}, the above result can be used to handle the prime $3$ thanks to the results recently obtained in \cite{Spa23}.

\begin{theo}
\label{thm:CTC block-free p=3}
Let $G$ be a finite group and write $|G|_3=3^d$. Then, the block-free form of the Character Triple Conjecture \cite[Conjecture 3.5.5]{Ros-Thesis} holds for $G$ at the prime $3$ and with respect to the defect $d$.
\end{theo}

\begin{proof}
By Theorem \ref{thm:CTC non-blockwise for maximal defect follows from iMcK} it suffices to verify the inductive McKay condition for finite simple groups with respect to the prime $3$. This has been verified for Suzuki and Ree groups \cite[Section 16-17]{Isa-Mal-Nav07}, alternating groups \cite[Theorem 3.1]{Mal08II}, groups of Lie type with exceptional Schur multiplier \cite[Theorem 4.1]{Mal08II}, Sporadic groups \cite{Mal08II}, groups of Lie type in defining characteristic \cite[Theorem 1.1]{Spa12}, and groups of Lie type in non-defining characteristic unless of type ${\bf D}$ \cite{Cab-Spa13}, \cite{Cab-Spa17I}, \cite{Cab-Spa17II}, \cite{Cab-Spa19}. The remaining case of groups of Lie type ${\bf D}$ in non-defining characteristic follows from \cite[Theorem 3.1]{Mal-Spa16} and \cite[Theorem A]{Spa23} by applying \cite[Theorem 2.4]{Cab-Spa19}.
\end{proof}

The above argument used to prove Corollary \ref{cor:Main CTC for abelian 2-blocks}, can be used to obtain the block-free form of the Character Triple Conjecture for the prime $3$ and every finite group $G$ with abelian Sylow $3$-subgroup. 

\begin{cor}
\label{cor:CTC block-free, abelian Sylow 3}
The block-free version of the Character Triple Conjecture holds at the prime $3$ for every finite group with abelian Sylow $3$-subgroups.
\end{cor}
