...
Let $A$ be a discrete abelian group. When $G$ is a finite group, there is a well-known natural correspondence between central extensions of $G$ by $A$ and the quadratic group cohomology $H^2(G,A)$.  
From this, one can systematically find cohomological criteria for lifting problems. In this paper we explain how this correspondence can be extended to the topological group setting, when $G$ is topologically a CW-complex, for instance when $G$ is a Lie group.
 
Let $\mb E(G,A)$ be the set of (equivalence classes of) central extensions of $G$ by $A$, and let $BG$ be a classifying space for $G$. Using a construction of Milgram-Steenrod, we define a natural map 
\beq
\alpha_G: \mb E(G,A) \to H^2(BG,A).
\eeq
When $G$ is discrete, $\alpha_G$ agrees with the classical bijection.
For $G$ connected, we prove $\alpha_G$ is bijective. More generally there is an exact sequence
 \beq
1 \to G^\circ \to G \to \pi_0(G) \to 1,
\eeq
where $G^\circ$ is the identity component, and $\pi_0(G)$ is the component group of $G$.
...
\end{equation}	
We demonstrate that the rows are (left) exact, and the injectivity of  $\alpha_G$ follows.  Consequentially we deduce the following lifting criterion.
...
One expects $\alpha_G$ to be bijective in general. We manage to prove that $\alpha_G$ is a bijection, when $G$ is the semidirect product of a discrete group and a connected group. This includes orthogonal groups.


For the orthogonal group $G=\Or(V)$ of a quadratic space, we describe its four inequivalent double covers, notably $\Pin^{\pm}(V)$ and $\widetilde \Or(V)$, and then describe $\alpha_G$ for each. 
...
\end{enumerate}
The set of lifts of $\pi$ is acted on simply transitively by the group of continuous linear characters $\chi: G \to \{ \pm 1\}$. 
\end{thm}
...
% \subsection{Earlier works}
 ...
 The equivalence of $\mb E(G,A)$ and $H^2(BG,A)$ was observed in the '70s. A proof is given in \cite[Theorem 4]{Wigner} by comparing two spectral sequences in simplicial sheaf cohomology. We have nonetheless written this manuscript because we feel that for a result of this importance, it is useful to have alternate proofs using different means. When $G$ is connected, our correspondence essentially agrees with the obstruction class construction in \cite{NWW}, according to Remark 7.17 in that paper and Proposition \ref{alpha.beta} below. % Also,  our definition of $\alpha_G$ was taken from \cite[Section IV.1]{Milgram}.
 
  
 
  
 The layout of the paper is as follows.  After reviewing definitions for topological groups in Section \ref{notprem}, we examine the category of their central extensions in Section \ref{exs.section}. There we establish the  left exactness of the upper row of  \eqref{intro.diag}. An example is given to show it is not right exact in general, however we show it is right exact when  $G$ is the semidirect product of a discrete group and a connected group. The theory of (principal) $G$-bundles is reviewed in Section \ref{G.bund.sec}. In Section \ref{class.space.section} we review the theory of classifying spaces, and in particular  the Milgram-Steenrod model and its functorial properties. %It is essential to note that this construction takes central extensions to principal bundles.
...
\textbf{Acknowledgements.}  The first author was supported by a postdoctoral fellowship from NBHM (National Board of Higher Mathematics), India.  Both authors would like to publicly thank Karthik Vasisht for helpful discussions.
 \section{Notation and Preliminaries} \label{notprem}
 
 \subsection{Groups}
 If $G$ is a group, we write $1_G \in G$ for the identity, $G^{\ab}$ for the abelianization of $G$. When $A$ is an abelian group, we may use additive notation, so the identity would be $0_A$.
If $x,g \in G$, put
 \beq
 \Int(x)(g)=x^{-1}gx.
 \eeq
 ...
 For us, topological groups will always be Hausdorff. For a topological group $G$, denote by $G^\circ$ the path component of $G$ containing the identity. It is a normal subgroup of $G$; write $\pi_0(G)=G^\circ \bks G$ for the quotient.
 
 Write $\Aut(G)$ for the (continuous) automorphisms of $G$.
...
Unfortunately, when $X$ and $Y$ are CG spaces, then $X \times Y$ with the  product topology may not be CG. 
 There is a standard refinement of the product topology, sometimes called the \emph{CG-topology} on $X \times Y$, which is used to remedy this. We will write this as ``$X \times_k Y$''. When either $X$ or $Y$ is locally compact, then the topologies coincide, i.e., $X \times_k Y=X \times Y$. A standard reference for this is \cite[Chapter VII, Section 8]{Maclane}.   The continuity condition for a \emph{right $G$-action} on a space $X$ will be
 that it defines a continuous map
 \beq
 X \times_k G \to X.
 \eeq 
 On the other hand, when $G$ is a topological group,   the multiplication map is continuous when $G \times G$ is given the product topology.
 
  

  
 
 \section{Extensions} \label{exs.section}
 
 In this section we study the category of central extensions of topological groups by discrete groups. Our main concern is the exactness of the top sequence in \eqref{intro.diag}, which we observe is not a short exact sequence in general (Proposition \ref{weil.prop}).
 
 \subsection{Definition}
Let $G$ be a topological group and $A$ a discrete abelian group.
...
\end{equation}
of continuous homomorphisms of topological groups.

 
Another central extension
\beq
...
%[$\bigstar$ It's a pointed set]
 
 
 
 \subsection{Functorial Properties}
 
 A  morphism $\varphi: G' \to G$ gives rise to a ``pullback'' morphism 
 \beq
 \varphi^*:\mb E(G,A) \to \mb E(G',A)
 \eeq
 as follows:
Given $\ms E=(E,i,p) \in {\bf E}(G,A)$, set
 ...
 \end{equation}
  ...
  $\psi_* \ms E=(E',i',p')$ is a central extension of $G$ by $A'$, and this gives a homomorphism 
  \beq
  \psi_*: \mb E(G,A) \to \mb E(G,A').
  \eeq
...
\begin{proof} Since $\theta^*(\ms E) \cong (E,i,\theta \circ p)$, an equivalence between $\ms E$ and $\theta^*(\ms E)$ amounts to the existence of such a $\tilde \theta$.
\end{proof}
...
Define $i_3:A \to E_3$ by 
\beq
i_3(a)=(a,1) \mod \triangledown(A),
\eeq
and $p_3:E_3 \to G$ by 
\beq
p_3(e_1,e_2)=p_1(e_1).
...
Then
\beq
\ms E_3=(E_3,i_3,p_3) \in {\bf E}(G,A),
\eeq
and is called the Baer sum 
\beq
\ms E_3=\ms E_1 + \ms E_2
\eeq
...
\eeq
 ...
 \subsection{Relation to Lifting problem} \label{rel.lift.prob}
...
The group $\Hom_c(G',A)$ of continuous homomorphisms from $G'$ to $A$ acts simply transitively on the set of lifts $\hat \varphi$ of $\varphi$ by the prescription
\beq
(\chi \odot \hat \varphi)(g)=\chi(g) \hat \varphi(g).
\eeq
 Therefore, when $\varphi$ lifts to $E$, the set of lifts is in bijection with $\Hom_c(G',A)$.
...
\eeq
  ...
  \begin{prop} \label{extexact} Let $A$ be a discrete group, and $G$ as above. The following sequence is exact.  
  \beq
...
\eeq
 lies in $A$. Since $G^\circ$ is connected, it must be trivial. So the map 
 \beq
  \tilde \qoppa  \circ s': G \to E
 \eeq
  descends to a map $s: \pi_0 \to E$ which is a splitting of $p$. \newline (Since $p \circ \tilde \qoppa \circ s'=\qoppa$.) Hence $\qoppa^*$ is injective.
 ...
 Write $\tilde \iota: E' \to E$ for the projection, and let $E^0$ be the image of $G^\circ$ under $\tilde \iota \circ s$. 	Also put
 \beq
 E_0=E/E^0,
 \eeq
 and note that $p$ descends to $p_0: E_0 \to \pi_0$. Also let $i_0=\tilde \qoppa \circ i: A \to E_0$. Then $\ms E_0=(E_0,p_0,i_0) \in \mb E(\pi_0,A)$ and $\qoppa^* \ms E_0=\ms E$.
 \beq
...
\begin{proof} Let $\Sq: \C^\times \to \C^\times$ be the squaring map. Since $\Sq$ commutes with complex conjugation, this determines an extension  $\ms E_\bullet \in \mb E(\C^\times,\Z/2\Z)^\Gamma$ by Lemma \ref{liftingtheta}.
...
\begin{equation} \label{quat.comm}
...
We may form the semidirect product $G= H \rtimes \Gamma$.
 ...
 \begin{prop}\label{surj}  With the above notation, the restriction map
  ...
  \begin{proof} 
   ...
   \eeq
    Since $E_\bullet$ is connected, $f$ is constant, and we deduce that $\tilde{\theta_1} = \tilde{\theta_2}$. The prescription $\theta \mapsto \tilde \theta$ is therefore a well-defined map
   \beq
   \Gamma \to \Aut(E_\bullet),
   \eeq
necessarily a homomorphism.  This corresponds to an action of $\Gamma$   on $E_\bullet$, and so we may form the  semidirect product
 \beq
 E=E_\bullet \rtimes \Gamma.
 \eeq
Clearly this gives an extension of $G$ by $A$  whose restriction to $H$ is  $E_\bullet$.
...
\eeq
 is a $\Gamma$-invariant $A_1$-extension of $G$. Moreover
 \beq
 (E_\bullet^\circ)_{A} \cong E_\bullet.
 \eeq
...
\eeq 
 commutes. Since $E_\bullet^\circ$ is connected, we have a lift $\ms E_1 \in \mb E(G,A_1)$ of $\ms E_\bullet'$, and this maps down to the required lift $\ms E$ of $\ms E_\bullet$.
...
\begin{remark} Requiring that $X$ be CG and $B$ be regular sidesteps a technical issue. Since $B$ is regular, we may assume that $U$ is a \emph{regular} open set. This implies that the preimage $p^{-1}(U)$ is CG by \cite[Lemma A.1]{Uribe}, since $X$ is CG. Therefore if $h_U$ is a homeomorphism when we give $U \times G$ the product topology, it must be that $U \times G$ is CG, so $U \times G=U \times_k G$.  \end{remark}
 ...
 A \emph{morphism} of $G$-bundles over $B$ is a map $f: X \to X'$ with $f(x \cdot g)=f(x) \cdot g$. Write $\Bun_B(G)$ for the category of $G$-bundles with base space $B$. 
...
\begin{lemma} \cite[Corollaire, page 94]{Bou.AT} \label{bou.pullback}  Let $(X,p)$ be a $G$-bundle over a space $B$ and $(X',p')$ be a $G$-bundle over a space $B'$. Suppose that $f: B' \to B$ and $f': X' \to X$ are continuous with
\beq
p \circ f'=f \circ p',
\eeq
and moreover that $f'$ is $G$-equivariant. Then 
\beq
	\xymatrix{
...
\begin{defn} \label{induced.bundle} Let $(X,p)$ be a $G$-bundle over $B$, let $G'$ be a CG topological group, and $\psi: G \to G'$ a homomorphism. Write $X[G']$ for the quotient of $X \times_k G'$ by the equivalence relation 
\beq
(x,g') \sim (x \cdot g,\psi(g)^{-1}g'),
\eeq
...
\end{defn} %[$\bigstar$ what is the action on X[G'] of G'? Is it right action or left action? Is it well defined? Are G and G' abelian? ]

Write $f: X \to X[G']$ for the map over $B$, taking $x$ to the class of $(x,1_{G'})$, and satisfies 
...
If we view $X_3'$ as an $A$-space under the action
\beq
(x_1,x_2) \cdot a=(x_1,x_2 \cdot a),
\eeq
then $(X_3',p_3')$ is an $A$-bundle.
	
We may also view $X_3'$ as an $A$-space under the anti-diagonal action 
\beq
(x_1,x_2) \cdot a=(x_1 \cdot a,x_2 \cdot a^{-1}).
\eeq
 Let $X_3$ be the quotient of $X_3'$ by the anti-diagonal $A$-action. Then $X_3$ itself has an $A$-action by $(x_1,x_2) \cdot a=(x_1 \cdot a,x_2)$.
 
 If we define $p_3:X_3 \to B$ by 
\beq
p_3(x_1,x_2)=p_1(x_1),
\eeq
 then $(X_3,p_3) \in \Bun_B(A)$. (A common trivialization of $X_1$ and $X_2$ over $B$ will trivialize $X_3$ as well.) We call $(X_3,p_3)$ the \emph{Baer sum} of the two bundles.    
...
\eeq
When $X$ is path-connected, the last map is surjective.

 
\begin{comment}
If we instead end with
\beq
\cdots \to \pi_1(G,e_0) \to  \pi_1(X,e_0) \to \pi_1(B,b_0),
\eeq
then this is a long exact sequence of groups. \end{comment}
 \end{thm}
 
 \begin{proof}
 See \cite[Section 17.11]{Steenrod.FB}.
 \end{proof}
...
When $D$ is a discrete group,  a $D$-bundle is necessarily a covering space, often simply called a \emph{$D$-cover}. Another approach to $D$-covers is through even actions, as follows.
...
The above prescription defines a homomorphism
\beq
\partial_p: \pi_1(B,b) \to D.
\eeq

If $B$ is nice and path-connected, then the correspondence $(X,p) \leadsto \partial_p$ gives a natural bijection
\beq
...
gives a bijection 
 \beq
[B,B_u] \overset{\sim}{\to} \Bun_B(G)/\text{ isom}.
\eeq
...
\end{itemize}
  ...
  \beq
  F(G \times I) \twoheadrightarrow EG,
  \eeq
  where $F(G \times I)$ is the free group on the set $G \times I$.
  
  From this surjection, we can identify the group $EG$   with the quotient of $F(G \times I)$ by the following relations
 \begin{itemize}
 ...
 \end{itemize}
With this notation, the map $i: G \to EG$ is the homomorphism defined by 
 \beq
 i(g)=(g,1). 
 \eeq


Write $BG$ for the quotient $EG/i(G)$. Let $e_{BG} \in BG$ be the image of $1_{EG}$ under the quotient.
 When $G$ is abelian, then $EG$ is abelian, and so $BG$ is a CW-group.
 
 
 
When $\varphi: G \to G'$ is a homomorphism of topological groups, the map
\beq
G \times I \to G' \times I
\eeq
defined by
\beq
(g,t) \mapsto (\varphi(g),t)
\eeq
induces a topological group homomorphism
\beq
E\varphi: EG \to EG'
\eeq
with the property that
\beq
E\varphi(x \cdot g)=E\varphi(x) \cdot \varphi(g).
\eeq
 
 Hence it descends to a continuous map
 \beq
 B\varphi: BG \to BG'.
 \eeq
  
  
  \begin{prop}  \label{fibration.normal}  \cite[Theorem 7.7]{Piccinini} Let $A$ be a discrete abelian group and $G$ a CW-group. If $\mathscr E=(E,i,p) \in  {\bf E}(G,A)$, then the induced map
  \beq
 Bp: BE \to BG
  \eeq
  is a $BA$-bundle. 
...
In this section we give a weaker version of Proposition \ref{fibration.normal}, which is also asserted in \cite[page 48]{Milgram}, and \cite[Theorem 2.4.12]{Benson.II}. 
...
Define $B_GN=EG/N$. The group $G$ acts diagonally on the product $EG \times E\ol G$; let $B_uG$ be the quotient by this action. The second projection defines a map
\beq
p: B_uG \to B \ol G.
\eeq
...
\eeq
 where the right hand side is understood as a $G$-space under 
 \beq
 (x,u, \ol h) \cdot g=(x \cdot g,u,\ol {hg}).
 \eeq
The map $(x, \ol g) \mapsto x \cdot g$ induces a homeomorphism
 \beq
...
i_G: B_GN \to B_uG
\eeq
induced from the inclusion $EG \to EG \times E \ol G$ into the first factor.
...
\cdots \to \pi_{i+1}(B \ol G) \to \pi_i(B_GN) \to \pi_i(B_uG) \to \pi_i(B \ol G) \to \cdots
 \eeq
 attached to the fibre bundle $p$ of Lemma \ref{fibre.buns}, for instance by  \cite[Theorem 4.41 and Proposition 4.48]{hatcher}. Define the map  $\pi_{i+1}(B \ol G) \to \pi_i(BN)$ by composing the connecting map with the inverse map to
 \beq
 \pi_i(BN) \overset{\sim}{\to} \pi_i(B_GN).
 \eeq
 Clearly this gives the desired LES.
 \end{proof}
...
Suppose $G,G'$ are CW-groups, $\ms E \in {\bf E}(G,A)$ and    $\ms E' \in {\bf E}(G',A)$, and 
\beq
f: E \to E'
\eeq
is a homomorphism with $f \circ i=i' $. Then one has a commutative diagram
 \beq 
	\xymatrix{
...
Consider the function
 ...
 Let $E'$ be as in \eqref{e.prime}. From Diagram \eqref{diag.ek}, we obtain a map $f': BE \to BE'$ over $BG$, satisfying
 \beq
 f'(ax)=B \psi(a) f'(x)
 \eeq
...
\section{Homotopy Theory of Classifying Spaces} \label{long.ex.seq.section}
 In this section let $G$ be a CW-group and $A$ a discrete abelian group. Applying Theorem \ref{homotopy.fibration} to the   $G$-bundle $EG \to BG$ gives a  long exact sequence
\begin{equation} \label{LES}
...
Since $EG$ is contractible, this gives isomorphisms 
...
is an isomorphism. By Hurewicz's theorem, this gives rise to natural isomorphisms
 ...
 \partial_1: H_1(BG) \overset{\sim}{\to}  (G^\circ \bks G)^{\ab},
  \end{equation}
 and
\begin{equation}  \label{since.A.disc}  
...
When $G$ is discrete, the singular homology $H^*(BG,A)$ can be identified with the group cohomology $H^*(G,A)$.
\cite[Remark 3.6, Page 58]{Milgram}
...
\end{prop}
%\begin{proof} 
%Since both these maps are natural  isomorphisms, so too is $m^1$.
%\end{proof} 
...
The Hurewicz map
\beq
 h_2: \pi_2(BG) \to H_2(BG)
 \eeq
  dualizes to a homomorphism
\beq
...
The map $m^2: H^2(BG,A) \to \Hom(\pi_1(G),A)$ defined by $m^2=(\partial_2^*)^{-1} \circ h^2 \circ q^2$ is the composition of natural maps and hence is natural itself.
 ...
 \begin{thm}\label{Gconnect.equiv} Let $G$ be a connected CW-group, and $A$ a discrete abelian group. The above maps give  bijections of pointed sets
\beq
...
\begin{comment}
The composition is hence natural in the sense that when $\varphi: G' \to G$ is a homomorphism of connected CW-groups, the following diagram commutes:
 
 \begin{equation} \label{H2ext}
\xymatrix{ 
		 H^2(BG,A) \ar[r]^{\: \: \: \: \: \: \: \sim} \ar[d] & \mb E(G,A) \ar[d]  \\
		 H^2(BG',A) \ar[r]^{\: \: \: \: \: \: \: \sim}& \mb E(G',A),  \\
	}
\end{equation}

 where the right vertical map is given by the pullback extension.
\end{comment}
...
\end{equation}
where $\partial_{\ms E}$ is defined by \eqref{del.E.defn}.
 ...
 Let $A$ be a discrete abelian group. When a path-connected topological space $K$ has $\pi_2(K) \cong A$ as its only nontrivial homotopy group, it is called a $K(A,2)$-space.   
 Fixing an isomorphism $\zeta: \pi_2(K)  \overset{\sim}{\to} A$, and composing with the Hurewicz map gives an isomorphism
\beq
h_2 \circ \zeta^{-1}: A \overset{\sim}{\to} H_2(K).
\eeq
The inverse of this map, we can write as 
\beq
\zeta \circ h_2^{-1} \in \Hom_{\Z}(H_2(K),A).
\eeq
The UCT gives an isomorphism
\beq
...
%\begin{lemma}\label{m2.f.iota}    We have
...
\begin{lemma} \label{roti.sabzi} We have
\beq
\psi_*(\iota_A)=(B^2 \psi)^*(\iota_{A'}).
\eeq
\end{lemma} 
 ...
 we see that
 \beq
 \psi_* \circ \zeta_A= \zeta_{A'} \circ B^2 \psi_*.
 \eeq
 The lemma then follows from the commutativity of the following diagram.
  \beq
	\xymatrix{
...
$$\mu^*(\iota_A)= \pr_1^*(\iota_A) + \pr_2^*(\iota_A). $$ 
...
At this point we make a compatibility check between certain connecting homomorphisms arising from the various classifying spaces associated to an extension. We show a certain square \emph{anticommutes}, by means of an argument with loops.
...
Write $\gm * \gm'$ for the usual concatenation of paths.
 
 
 Given a loop $\gm$ in $B$ with $\gm(0)=\gm(1)=b$, and $s \in (0,1]$, define a loop $\gm^{\leq s}$ in $B$ by the formula
  \beq
...
\begin{equation}  \label{del.E.defn}
...
BE \to BG,
\eeq
and in particular a homomorphism
\begin{equation} \label{del.BE.defn}
...
\begin{prop}\label{anticommute}
	...
	\eeq
	anticommutes, meaning that
	\beq
	\partial_1 \circ \partial_{BE}=- \partial_E \circ \partial_2.
	\eeq
\end{prop}
...
\begin{proof}
 ...
 It is enough to prove that the path
	\beq
	\la= \ol{ \delta_E} * \gm_{EA}
	\eeq
	in $EE$ is a loop, since its endpoints are $\la(0)=  \delta_E(1)$ and $\la(1)=\gm_{EA}(1)$.
	
	Define	$B'=EG \times_{BG} BE$, we have a map
\beq
...
Moreover the fibres of $q$ are the $A$-orbits of $EE$. It follows that $q$ is an $A$-cover.
	...
	\end{equation}
	 ...
	 For each $s \in [\half,1]$, the formula
		...
		\eeq
	defines a homotopy in $B'$ between $H_\half=q_*(\la)$ and $H_1=\gm_{G} \times \gm_{BA}$.
	
	 
	
	Next, for $s \in I$, the function $K_s:\partial I^2  \to B'$ defined by 
	\beq	
	...
	Therefore $q_*(\lambda)$ is nullhomotopic in $B'$. Since $q$ is a cover, $\la$ is a loop by \cite[Corollary 3.6, page 141]{Bredon}.      
...
In this section we show the left exactness of the bottom row of \eqref{intro.diag}.  A more sophisticated approach would be using \cite[Theorem 5.9, page 147]{McCleary}.
...
\begin{proof}
Since $\qoppa \circ \iota$ is trivial, the induced map from $BG^\circ$ to $B \pi_0$ is constant, hence induces the zero map on cohomology. Next we show that the image of $B\iota_*$ is identical to the image of the Hurewicz map
\beq
h: \pi_2(BG) \to H_2(BG).
\eeq
...
Since $BG^\circ$ is simply connected,  we may apply the Hurewicz theorem \cite[Corollary 10.8, page 478]{Bredon} to deduce that $h^\circ: \pi_2(BG^\circ) \to H_2(BG^\circ)$ is an isomorphism.
...
\eeq 
 we deduce that
\begin{equation} \label{mahavir}
B\iota_*(H_2(BG^\circ)) = h(\pi_2(BG)).
\end{equation}
 
By Corollary \ref{isom.pi.bs} the induced map 
$\pi_1(BG) \to \pi_1(B \pi_0)$ is an isomorphism. Moreover, $\pi_k(B \pi_0)=0$ for $k \geq 2$. 

The proposition is then a consequence of the following lemma.

\end{proof}

\begin{lemma} Let $f:X \to Y$ be a continuous map between path-connected  spaces. Suppose that $f$ induces an isomorphism of fundamental groups, % map $\pi_1(X) \to \pi_1(Y)$ is an isomorphism, 
and that
\beq
\pi_2(Y)=\pi_3(Y)=0.
\eeq
...
f_*: H_2(X) \to H_2(Y)
\eeq
is surjective, and its kernel is  the image of the Hurewicz map 
\beq
h: \pi_2(X) \to H_2(X).
\eeq
\end{lemma}
...
\eeq
Furthermore, since $\pi_2(M_f)=\pi_3(M_f)=0$, the connecting map $\pi_3(M_f,X) \to \pi_2(X)$ is an isomorphism.

The conclusion then follows from the diagram:
...
%(Compare \cite[Exercise 23, Page 390]{hatcher}.) 
...
Note that $$B\iota^*(H^2(BG,A)) \subset H^2(BG^\circ, A)^{\pi_0}$$ by \cite[Lemma 3.1 Page 55]{Milgram}. 
...
\end{proof} 
 ...
 %In this section we apply an idea from \cite[Section IV.1]{Milgram} to associate a 
  
  Let $B$ be a topological space and $A$ a discrete abelian group. A $BA$-bundle $(X,p)$ over $B$  corresponds to a map $f_p: B \to BBA$, unique up to homotopy, with $(X,p)$ the pullback of $EBA \to BBA$ under $f_p$.
  
  Recall from Section \ref{ka2} that $B(BA)$ is a $K(A,2)$, and  a ``fundamental class'' $\iota \in H^2(BBA,A)$ was defined. 
Then we may set
\beq
\kappa(p)=f_{p}^*(\iota).
\eeq
...
\eeq
 ...
 \begin{prop}\label{alphabunnatural} The map $\kappa_{A,B}$ is additive, and natural in both $A$ and $B$.
 \end{prop}
  \begin{proof}
Let $(X_1,p_1), (X_2,p_2) \in \Bun_B(BA)$, and write $(X_3, p_3)$ for their Baer sum.
 
 Let $f_k: B \to BBA$ be classifying maps for $(X_k,p_k)$, with $k=1,2$. Write 
 \beq
 \tilde f_k: X_k \to EBA
 \eeq
 for the overmaps. Since $EBA$ is an abelian group, we may define a map
 \beq
 \tilde f_3: X_1 \times_{B} X_2 \to EBA
 \eeq
 by 
 \beq
 ...
 Being $BA$-equivariant, $\tilde f_3$ descends to a map $f_3:B \to BBA$ is defined. 
 Now
 \beq
 f_3=f_1+f_2,
 \eeq
and  $f_3$ is a classifying map for $X_3$.
 Thus
 \beq
 \begin{split}
 \kappa(p_3) &= f_3^*(\iota) \\ 
 ...
 &= \kappa(p_1) + \kappa(p_2) ,
  \end{split}
  \eeq
  as required. This shows additivity.
...
\eeq
From this we deduce that $\kappa(p')=\varphi^*(\kappa(p))$, in other words $\kappa$ is natural in $B$.
...
making the following diagram commute:
 ...
 \end{proof}
  ...
  Let $A$ be a discrete abelian group, and $G$ a CW-group. Write $\alpha=\alpha_{G,A}$ for the 
  composition
  \beq
  \mb E(G,A) \overset{\ms B}{\to} \Bun_{BG}(BA) \overset{\kappa}{\to} H^2(BG,A).
  \eeq
  
  By Proposition \ref{alphabunnatural}  and the results of Section \ref{rain.car}, we deduce:
  \begin{thm}
  The map $\alpha_{G,A}$ is a homomorphism of abelian groups, natural in $G$ and $A$.
  \end{thm}
    We also note here the situation for discrete groups.
    \begin{prop} \cite[Lemma 1.12, page 120]{Milgram} \label{adem.milg.alpha.disc}
 When $G$ is discrete, $\alpha_G$ agrees with the usual bijection between $\mb E(G,A)$ and group cohomology $H^2(G,A)$.
 \end{prop}
 
 Recall from Definition  \ref{beta.defn} the isomorphism $\beta_G:  \mb E(G,A) \to H^2(BG,A)$, valid for connected CW-groups $G$.
...
Moreover the extension gives a map which we call $\pi_2(f_{\ms E}):\pi_2(BG) \to   \pi_2(K)= A$. We have a morphism of $BA$-bundles:
...
Recall from \eqref{characterized} that $\beta_G$ is characterized by
\beq
m^2(\beta_G(\ms E))=\partial_{\ms E}.
\eeq
...
\eeq	
 ...
 The map $\alpha_{\pi_0}$ is a bijection by Proposition \ref{adem.milg.alpha.disc}. The map $\alpha_{G^\circ}$ is a bijection by Theorem \ref{Gconnect.equiv} and Proposition \ref{alpha.beta}. The injectivity of $\alpha_G$ then follows by a diagram chase.
 
 Next, suppose  $G = G^\circ \rtimes \pi_0$. Then $\iota^*$ is surjective by Proposition  \ref{surj}. From the Five Lemma we deduce that $\alpha_G$ is an isomorphism. 
\end{proof}
...
\begin{thm} \label{lifting.and.coh} Let $\varphi:G' \to G$ be a homomorphism between   CW-groups, $A$ a discrete abelian group, and $\ms E=(E,i,p) \in {\bf E}(G,A)$. Then $\varphi$ lifts to $E$ iff 
\beq
...
\beq
\alpha_{\Gamma_2}(\ms E_2)
\eeq
explicitly with $2$-cocyles. Let us give details for one example, the others being similar. 
...
\begin{cor} Let $G$ be a CW-group. An orthogonal representation $\pi: G \to \Or(V)$ lifts to
\begin{enumerate}
\item $\widetilde \Or(V)$ iff $w_1(\pi)^2=0$.
\item $\Pin^+(V)$ iff $w_2(\pi)=0$.
\item $\Pin^-(V)$ iff $w_2(\pi)+w_1^2(\pi)=0$.
