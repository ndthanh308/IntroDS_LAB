...
\author{Souvik Kundu\thanks{  Authors have equal contribution.} , Sharath Sridhar Nittur$^{*}$,  Maciej Szankin, Sairam Sundaresan\\
...
\end{tabular}
\caption{Performance of Sensi-BERT and other methods on various datasets. ``FD" and ``CPT" indicates fine-tuning distillation, and complex pre-training, respectively. Here, the subscript value of Sensi-BERT represents the budget. * = Representative results utilizing all three.}
\vspace{-5mm}
...
\begin{tabular}{cll}
\hline
\textbf{Density} & \multicolumn{1}{c}{\textbf{1+3}} & \multicolumn{1}{c}{\textbf{1+6}} \\ \hline
\textbf{0.9}     & 90.90\%                          & 91.17\%                          \\
\textbf{0.7}     & 90.90\%                          & 91.16\%                          \\
\textbf{0.5}     & 90.70\%                          & 91.09\%                          \\
\textbf{0.4}     & 90.43\%                          & 90.92\%                          \\
\textbf{0.3}     & 89.94\%                          & 90.51\%                          \\
\textbf{0.2}     & 89.07\%                          & 89.78\%                          \\
\textbf{0.1}     & 86.48\%                          & 87.60\%                          \\ \hline
\end{tabular}
\label{tab:density_ablation}
\caption{Comparison of achieved accuracies for various densities when fine-tuned on QQP dataset for 3 epochs and 6 epochs. For both tests the same optimized model was used. While fine-tuning for more epochs yielded better results (RMSE 0.62\%), it also took twice the computations to complete.},
\end{table}    
\end{comment}
...
As Fig. \ref{fig:sensi_vs_mp} shows that Sensi-BERT yields significant better accuracy by up to $9.5\%$ (\texttt{QNLI} at $B=0.2$) compared to MP, clearly highlighting the importance of the sensitivity analysis step. However, for higher $B$, the model may still remain over-parameterized highlighting the reduced importance of sensitivity driven model trimming.
 \begin{comment}
 We carried out additional ablation study experiments, varying the number of epochs used for fine-tuning. The results, presented in Table  \ref{tab:density_ablation}, demonstrate a moderate improvement in model's accuracy with more fine-tuning epochs, with RMSE of 0.62\%. However, it's important to note that achieving these results require twice the computational time, and as such it should be taken under consideration as a trade-off between required accuracy and time budget.
...
\begin{table}[h]
    ...
    \begin{tabular}{cllll}
    \hline
    \multicolumn{1}{l}{\textbf{Density}} & \textbf{SST-2} & \textbf{QQP} & \textbf{MNLI} & \textbf{QNLI} \\ \hline
    \textbf{0.9}                         & 91.86\%        & 90.90\%      & 84.44\%       & 91.31\%       \\
    \textbf{0.7}                         & 92.09\%        & 90.90\%      & 84.26\%       & 91.31\%       \\
    \textbf{0.5}                         & 91.00\%        & 90.70\%      & 83.65\%       & 90.58\%       \\
    \textbf{0.4}                         & 91.86\%        & 90.43\%      & 82.91\%       & 90.32\%       \\
    \textbf{0.3}                         & 89.79\%        & 89.94\%      & 82.26\%       & 88.61\%       \\
    \textbf{0.2}                         & 89.45\%        & 89.07\%      & 80.20\%       & 85.99\%       \\
    \textbf{0.1}                         & 86.07\%        & 86.48\%      & 76.45\%       & 84.48\%       \\ \hline
    \end{tabular}
    \caption{BERT base model (uncased)}
    \end{subtable}
    ...
    \hline
    \multicolumn{1}{l}{\textbf{Density}} & \textbf{SST-2} & \textbf{QQP} & \textbf{MNLI} & \textbf{QNLI} \\ \hline
    \textbf{0.9}                         & 92.20\%        & 90.56\%      & 83.55\%       & 89.90\%       \\
    \textbf{0.7}                         & 91.17\%        & 90.52\%      & 83.74\%       & 89.35\%       \\
    \textbf{0.5}                         & 91.40\%        & 90.25\%      & 82.98\%       & 88.19\%       \\
    \textbf{0.4}                         & 90.31\%        & 90.04\%      & 82.47\%       & 86.89\%       \\
    \textbf{0.3}                         & 90.37\%        & 89.59\%      & 81.76\%       & 85.92\%       \\
    \textbf{0.2}                         & 89.91\%        & 88.82\%      & 79.74\%       & 84.40\%       \\
    \textbf{0.1}                         & 86.64\%        & 85.69\%      & 75.53\%       & 83.71\%       \\ \hline
    \end{tabular}
    \caption{BERT base model (cased)}
    \end{subtable}
    \caption{Evaluated accuracies for various densities when fine-tuned for 3 epochs on SST-2, QQP, MNLI and QNLI datasets.}
\end{table}
\end{comment}
