...
\usepackage{cancel}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title[Self-regulated biological transportation structures in 1D]{Self-regulated biological transportation structures with general entropy dissipations, part I: the 1D case}
%\date{July 2023}
...
\centerline{
     {\large Clarissa Astuto}\footnote{Mathematical and Computer Sciences and Engineering Division,
         King Abdullah University of Science and Technology,
         Thuwal 23955-6900, Kingdom of Saudi Arabia;
         {\it clarissa.astuto@kaust.edu.sa}} \qquad
     {\large Jan Haskovec}\footnote{Mathematical and Computer Sciences
            and Engineering Division,
         King Abdullah University of Science and Technology,
         Thuwal 23955-6900, Kingdom of Saudi Arabia;
         {\it jan.haskovec@kaust.edu.sa}}\qquad
     {\large Peter Markowich}\footnote{Mathematical and Computer Sciences and Engineering Division,
         King Abdullah University of Science and Technology,
         Thuwal 23955-6900, Kingdom of Saudi Arabia;
         {\it peter.markowich@kaust.edu.sa}, and
         Faculty of Mathematics, University of Vienna,
        Oskar-Morgenstern-Platz 1, 1090 Vienna;
         {\it peter.markowich@univie.ac.at}}\qquad
         {\large Simone Portaro}\footnote{Mathematical and Computer Sciences
            and Engineering Division,
         King Abdullah University of Science and Technology,
         Thuwal 23955-6900, Kingdom of Saudi Arabia;
         {\it simone.portaro@kaust.edu.sa}}
     }
%\begin{big}
\medskip \medskip
% riga bianca da mettere
%\end{big}
...
In Fig.~\ref{fig_exponential_Din1} we show the solution ${\bf D}$ (left part of each plot, in blue), together with ${\bf \Phi}''(u)$ (right part of each plot, in orange) with two different initial conditions:  ${\bf D}(t = 0) = 1$ in (a) and ${\bf D}(t = 0) = 0.1$ in (b) and (c) panels. The expression for the second derivative of the entropy function is $\Phi''(u) = \exp(-u)$, while $ m = -1.98$ and $q = 1$ in the expression \eqref{eq_S_deltas}. In these plots we show ${\bf D}$ and ${\bf \Phi}''(u)$ in different moments, to show how they evolve in time. It is interesting to see that after finite time the solution ${\bf D}$ touches zero if the initial condition is small enough. We show ${\bf \Phi}''(u)$ in the same plot because, analyzing equation \eqref{eq:Dt_2}, it is evident that ${\bf \Phi}''(u)$ acts as a weight, dependent on the solution itself ${\bf D}$, for the integral involving $S(x)$. To induce a change in sign in ${\bf D}$, it is necessary for the integral to become negative at a rate that is sufficient to drive the solution towards zero. Consequently, we select $S(x)$ to ensure that ${\bf \Phi}''(u)$ becomes significantly big when $S(x)$ is negative.
...
\newblock Discrete and Continuous Dynamical Systems, Vol. 43(3 \& 4), pp. 1499--1515, 2023.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Existence of solutions of the multi-dimensional problem in H\"older space}

The goal of this section is to show existence of a solution of the multi-dimensional system (\ref{eq:conservation_law}--\ref{eq:sigma_law}) with initial condition \eqref{eq:initial_condition}.
We shall work in the space of H\"older-continuous functions.

\begin{theorem}
    Let $S \in C^{0,\alpha}(\Bar{\Omega})$ for some $\alpha \in (0, 1)$ and $\Phi \in C^4(\R)$.
    Then the problem (\ref{eq:conservation_law}--\ref{eq:sigma_law}) subject to initial condition \eqref{eq:initial_condition} admits a local in time solution $D \in X_T$ for some $T>0$, where the set $X_T$ is defined as
    \begin{align*}
        X_T := \left\{  D \in C\left( [0, T]; C^{1, \alpha} \left(\Bar{\Omega}\right)\right); \;  D \ge \frac{\gamma}{2} > 0, \| D \|_{C( [0, T]; C^{1, \alpha} (\Bar{\Omega}) )} \le R \, \mbox{ for some } \, R > 0  \right\}.
    \end{align*}
\end{theorem}

\begin{proof}
    We shall employ the Banach fixed point theorem.
    For technical simplicity of the proof, we shall
    assume that $D$ is a scalar quantity; however, the below calculations also hold in the tensor case with obvious modifications.
    \cJH{JH: Don't really like this. Would the proof be that much more complicated if we worked with tensor-valued $D$?}
    
    For $T>0$ to be specified later, we construct the operator $A : X_T \rightarrow X_T$ that maps $\Bar{D} \in X$ to $A(\Bar{D}) =: D$, whose fixed points are solutions of (\ref{eq:conservation_law}--\ref{eq:sigma_law}). Our goal is to demonstrate that $A$ is a contraction on $X_T$ and that $\mathrm{Im}(A) \subset X_T$. Fix $\Bar{D} \in X$, then from \eqref{eq:conservation_law} we have 
    \begin{align*}
        - \Bar{D} \Delta u - \nabla \Bar{D} \cdot \nabla u = S,
    \end{align*}
    which by the Schauder theory \cite{gilbarg1977} admits a solution $u(t) \in C^{2, \alpha}(\Bar{\Omega})$ and there exists a constant $C(n, \alpha, \gamma, R) \equiv C_{\gamma, R} > 0$ such that
    \begin{align}
        \label{eq:holder_u}
        \| u(t) \|_{2, \alpha} \le C_{\gamma, R} \| S \|_{0, \alpha},
    \end{align}
    where $\| \cdot \|_{C^{k,\alpha}(\Bar{\Omega})} = \| \cdot \|_{k,\alpha}$. For the sake of simplicity we denote possibly different constants which depend on $\gamma, R$ by the same symbol $C_{\gamma, R}$. Consequently, $ \Phi'''(u(t)) \Bar{D}(t) | \nabla u(t) |^2 \in C^{0, \alpha}( \Bar{\Omega})$ and there exists $\sigma (t) \in C^{2. \alpha}(\Bar{\Omega})$ satisfying
    \begin{align}
        %\label{eq:holder_sigma}
        \| \sigma(t) \|_{2, \alpha} &\le C_{\gamma, R} \| \Phi''' (u) \|_{0, \alpha} \| \nabla u (t) \|_{0, \alpha}^2 \notag
    \end{align}
    where we used the Banach algebra structure of Hölder spaces. It is straightforward to show that $\| \Phi'''(u) \|_{0, \alpha} \le \| \Phi^\textrm{\romannumeral 4} \|_{L^{\infty}(\mathrm{Im}(u))} \| u \|_{0, \alpha}$ where $\| \Phi^\textrm{\romannumeral 4} \|_{L^{\infty}(\mathrm{Im}(u))}$ just depends on $\gamma, R$ so it will be successively incorporated in the constant $C_{\gamma, R}$. Therefore, using \eqref{eq:holder_u} we rewrite the Hölder estimate for $\sigma$
    \begin{align}
        \label{eq:holder_sigma}
        \| \sigma (t) \|_{2, \alpha} \le C_{\gamma, R} \| S \|_{0,\alpha}^3.
    \end{align}
    In the course of the computations we shall omit the temporal dependence when there is no risk of confusion. Next, let $\Bar{D_1}, \Bar{D_2} \in X$, $u_{1, 2} := u[\Bar{D_{1, 2}}]$ and $\sigma_{1, 2} := \sigma[\Bar{D_{1, 2}}]$ be the solutions of \eqref{eq:conservation_law}, \eqref{eq:sigma_law}, respectively. Then, integrating \eqref{eq:GF} on $t \in [0, T]$ and subtracting we get
    \begin{align}
        \label{eq:contraction_computation_1}
        \| D_1 - D_2 \|_{1, \alpha} &\le \bigg{\|} \int_o^t \bigg[ \left( \Phi''(u_1) | \nabla u_1 |^2 - \Phi''(u_2) | \nabla u_2 |^2  \right) + \left( \nabla u_1 \cdot \nabla \sigma_1 - \nabla u_2 \cdot \nabla \sigma_2 \right) \bigg] \d s \bigg{\|}_{1, \alpha} \notag \\
        &\le \int_0^t \left( J_1 (s) + J_2(s) \right) \d s,
    \end{align}
    where
    \begin{align}
        J_1 (s) &:= \| \Phi''(u_1) | \nabla u_1 |^2 - \Phi''(u_2) | \nabla u_2 |^2 \|_{1, \alpha}, \notag \\
        J_2(s) &:= \| \nabla u_1 \cdot \nabla \sigma_1 - \nabla u_2 \cdot \nabla \sigma_2 \|_{1, \alpha}. \notag
    \end{align}
    First, using \eqref{eq:holder_u}, the local Lipschitz continuity of $\Phi''$ and the continuous embedding of $C^{2, \alpha}(\Bar{\Omega})$ into $C^{1, \alpha}(\Bar{\Omega})$, we estimate $J_1(s)$
    \begin{align}
    \label{eq:J1_estimate1}
        J_1 (s) &\le \big{\|} \left( \Phi''(u_1) - \Phi''(u_2) \right) | \nabla u_1 |^2 \big{\|}_{1, \alpha} + \big{\|} \Phi''(u_2) \left( | \nabla u_1 |^2 - | \nabla u_2 |^2 \right) \big{\|}_{1, \alpha} \notag \\
        &\le C_{\gamma, R} \| S \|_{0, \alpha}^2 \| u_1 - u_2 \|_{1, \alpha} + \| \Phi''(u_2) \|_{1, \alpha} \| \nabla u_1 + \nabla u_2 \|_{1, \alpha} \| \nabla u_1 - \nabla u_2 \|_{1, \alpha} \notag \\
        &\le C_{\gamma, R} \| S \|_{0, \alpha}^2 \| u_1 - u_2 \|_{2, \alpha}.
    \end{align}
    To estimate $u_1 - u_2$ we consider the elliptic equation \eqref{eq:conservation_law} for both $u_1, u_2$ and subtract the two, obtaining
    \begin{align*}
        - \diver \big[ \Bar{D_1} (\nabla u_1 - \nabla u_2) \big] = \diver \big[ (\Bar{D_1} - \Bar{D_2}) \nabla u_2 \big] \in C\left([0,T]; C^{0, \alpha}(\Bar{\Omega})\right).
    \end{align*}
    Employing Schauder theory, estimate \eqref{eq:holder_u} and the continuous embedding of $C^{1, \alpha}(\Bar{\Omega})$ into $C^{0, \alpha}(\Bar{\Omega})$, we can write
    \begin{align}
        \label{eq:holder_u1-u2}
        \| u_1 - u_2 \|_{2, \alpha} \le C_{\gamma, R} \| S \|_{0, \alpha} \| \Bar{D_1} - \Bar{D_2} \|_{1, \alpha},
    \end{align}
    which substituted into \eqref{eq:J1_estimate1} yields
    \begin{align}
        \label{eq:J1_estimate2}
        J_1(s) \le C_{\gamma, R} \| S \|_{0, \alpha}^3 \| \Bar{D_1} - \Bar{D_2} \|_{1, \alpha}.
    \end{align}
    Using \eqref{eq:holder_u}, \eqref{eq:holder_sigma} and \eqref{eq:holder_u1-u2} we obtain the following estimate for $J_2(s)$
    \begin{align}
        \label{eq:J2_estimate}
        J_2 (s) &\le \big{\|} \nabla u_1 \cdot \left( \nabla \sigma_1 - \nabla \sigma_2 \right) \big{\|}_{1, \alpha} + \big{\|} \nabla \sigma_2 \cdot \left( \nabla u_1 - \nabla u_2 \right) \big{\|}_{1, \alpha} \notag \\
        &\le C_{\gamma, R} \| S \|_{0, \alpha} \left( \| \sigma_1 - \sigma_2 \|_{2, \alpha} + \| S \|_{0, \alpha}^3 \| \Bar{D_1} - \Bar{D_2} \|_{1, \alpha} \right).
    \end{align}
    An estimate of $\sigma_1 - \sigma_2$ is obtained by considering the elliptic equation \eqref{eq:sigma_law} for $\sigma_1, \sigma_2$ and subtracting the two equations to get
    \begin{align*}
        - \diver \big[ \Bar{D_1} \nabla \sigma_1 - \Bar{D_2} \nabla \sigma_2 \big] = \Phi'''(u_1) \Bar{D_1} | \nabla u_1 |^2 - \Phi'''(u_2) \Bar{D_2} | \nabla u_2 |^2,
    \end{align*}
    which is recast, after some algebraic manipulation, into
    \begin{align*}
        - \diver \big[ \Bar{D_1} \left( \nabla \sigma_1 - \nabla \sigma_2 \right) \big] = &\diver \big[ \left( \Bar{D_2} - \Bar{D_1} \right) \grad \sigma_2 \big] + \big[ \Phi'''(u_1) - \Phi'''(u_2) \big] \Bar{D_1} | \grad u_1 |^2 \\
        &+ \Phi'''(u_2) \big[ \left( \Bar{D_1} - \Bar{D_2} \right) | \grad u_1 |^2 + \Bar{D_2} \left( | \grad u_1 | + | \grad u_2 | \right) \left( | \grad u_1 | - | \grad u_2 | \right)\big].
    \end{align*}
    The right-hand side of the latter belongs to $C\left([0,T]; C^{0, \alpha}(\Bar{\Omega})\right)$. Therefore, the Schauder theory gives the desired estimate
    \begin{align}
        \label{eq:holder_sigma1-2}
        \| \sigma_1 - \sigma_2 \|_{2, \alpha} \le C_{\gamma, R} \| S \|_{0, \alpha}^3 \| \Bar{D_1} - \Bar{D_2} \|_{1, \alpha} (s).
    \end{align}
    Applying \eqref{eq:holder_sigma1-2} to \eqref{eq:J2_estimate} we get
    \begin{align}
        \label{eq:J2_estimate2}
        J_2 (s) \le C_{\gamma, R} \| S \|_{0, \alpha}^4 \| \Bar{D_1} - \Bar{D_2} \|_{1, \alpha}.
    \end{align}
    Substitution of \eqref{eq:J1_estimate2} and \eqref{eq:J2_estimate2} into \eqref{eq:contraction_computation_1} yields
    \begin{align*}
        \| D_1 - D_2 \|_{1, \alpha} \le C_{\gamma, R} \| S \|_{0, \alpha}^3 \left( \| S \|_{0, \alpha} + 1 \right) \int_0^t \| \Bar{D_1} (s) - \Bar{D_2} (s) \|_{1, \alpha} \d s.
    \end{align*}
    Subsequently, taking the maximum on $[0, T]$ of the latter expression we get
    \begin{align*}
        \| D_1 - D_2 \|_{X} \le C_{\gamma, R} \| S \|_{0, \alpha}^3 \left( \| S \|_{0, \alpha} + 1 \right) T \| \Bar{D_1} - \Bar{D_2} \|_{X},
    \end{align*}
    and choosing $T$ such that
    \begin{align*}
        T < \frac{1}{C_{\gamma, R} \| S \|_{0, \alpha}^3 \left( \| S \|_{0, \alpha} + 1 \right)}
    \end{align*}
    we obtain a contraction mapping $A$.
    
    To complete the proof, it remains to show that $A(\Bar{D}) =: D \in X$ for every $\Bar{D} \in X$.
    Integrating \eqref{eq:GF} on $[0, t]$ for some $t \in (0, T)$ and using \eqref{eq:holder_u}, \eqref{eq:holder_sigma} we obtain the following bound from below for $D$
    \begin{align*}
        D &\ge \gamma - \int_0^t \big{|} \Phi''(u) | \grad u |^2 + \grad u \cdot \grad \sigma \big{|} \d s \\
        &\ge \gamma - \int_0^t \| \Phi''(u) \|_{0, \alpha} \| \grad u \|^2_{0, \alpha} + \| \grad u \|_{0, \alpha} \| \grad \sigma \|_{0, \alpha} \d s \\
        &\ge \gamma - C_{\gamma, R} \| S \|_{0, \alpha}^3 \left( \| S \|_{0, \alpha} + 1 \right) T.
    \end{align*}
    Designing $T$ such that
    \begin{align*}
        T \le \frac{\gamma}{2 C_{\gamma, R} \| S \|_{0, \alpha}^3 \left( \| S \|_{0, \alpha} + 1 \right) },
    \end{align*}
    we ensure $D \ge \frac{\gamma}{2}$, so that $\textrm{Im}(A) \subset X$.
    %Finally, existence and uniqueness follow from Banach fixed point theorem.
\end{proof}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Model}
This paper seeks to address these gaps by providing a more detailed analysis of a transportation network model building upon the work \cite{portaro2023}. Overall, it aims to provide a more comprehensive understanding of the model in relation to different data and various behavior of the entropy generating function. Specifically, we will focus our study on examining the well-posedness of a gradient flow system. This system is derived from the minimization of a broad class of entropy dissipations
\(  
    \label{eq:energy_functional1}
    E[D] = \int_{\Omega} \Phi''(u) \grad u \cdot D \grad u \dx,
\)
with respect to the transportation network's symmetric and positive-definite diffusivity tensor $D = D(x)$. Here, $\Omega \subset \R^d$, $d \ge 1$, with smooth boundary, $\Phi : \R \rightarrow \R$ is the convex entropy generating function, $ u = u(x)$ commonly denotes the concentration of chemical species, ions or nutrients.
\clarissa{Let $\Omega\subset \mathbb{R}^d, \, d\geq 1$ be the domain with smooth boundary, $\Phi : \mathbb{R} \to \mathbb R$ the convex entropy generating function, and $ u = u(x)$ the concentration of chemical species, ions or nutrients.} The above mentioned optimization process is coupled to a conservation equation,
\(
    \label{eq:conservation_law}
    - \diver ( D \grad u) = S \quad \mathrm{in} \, \, \Omega,
\)
where $S = S(x)$ represents the given distribution of sources and sinks of the system, and $u$ is subject to a Dirichlet boundary condition
\(
    \label{eq:boundary_u}
    u = c \quad \mathrm{on} \, \,  \partial\Omega. \notag
\)
Here $c$ is a constant representing the equilibrium of the system. Indeed, we assume $\Phi$ to have a critical point at $c$, i.e.,
\(
    \label{eq:equilibrium_state}
    \Phi'(c) = 0. 
\)
Computing the formal $L^2$-gradient flow of the energy functional \eqref{eq:energy_functional1} constrained by \eqref{eq:conservation_law} (see Lemma 1 of \cite{portaro2023}) we obtain the following system
\(
    \label{eq:GF}
    \part{D}{t} = \Phi''(u) \nabla u \otimes \nabla u + \frac{\nabla\sigma\otimes\nabla u + \nabla u\otimes\nabla\sigma}{2} \qquad \mbox{in } (0, \infty) \times \Omega,
\)
where $t$ is the time-like variable induced by the gradient flow and $\sigma = \sigma (x, t)$ is the solution of the boundary value problem
\(
    \label{eq:sigma_law}
    - \diver (D \nabla \sigma) = \Phi'''(u) \nabla u \cdot D \nabla u \qquad \mathrm{in} \, \, (0, \infty) \times \Omega,
\)
subject to $\sigma = 0$ on $\partial \Omega$.
