...
\subsection{\uppercase{Differential Evolution}}
...
\begin{center}
    ...
    where $f(s) \neq -\infty$
\end{center}
Its versatility comes from its unique implementation that does not require the gradient of the function. DE obtains a minimum solution by initializing a set of candidate solutions and iteratively improving each solution by applying various genetic operators.

\subsubsection{Initialization}
Suppose $f$ has $D$ parameters. An $N$-sized candidate solution population is initialized, with each candidate solution modeled as $x_i$, a $D$-parameter vector \cite{Ahmed_Isa_Lim_Ang_2021}.

\begin{center}
$x_{i, G} = [x_{1, i, G}, x_{2, i, G} ... x_{D, i, G}]$ where $i = 1,2...N$ \newline and $G$ is the generation number
\end{center}

Each index $x_{j, i, G}$ with $j = 1, 2...D$ represents a parameter to be manipulated approximate a solution to the objective function. During the initialization of the first generation, each parameter for all candidate solutions is set randomly from bounds $[x_{j}^{L}, x_{j}^{U}]$.
\begin{center}
...
A mutation is a stochastic change that expands the candidate solution search space. Mutations are used in DE to prevent the algorithm from converging upon a local optimum. In the original mutation scheme devised by Storn, a mutant vector $v_{i}$ is created from randomly sampling three candidate solution vectors $v_{r_{1}}$, $v_{r_{2}}$, $v_{r_{3}}$ such that $r_{1}, r_{2}, r_{3}$ and $i$ are distinct. The mutant vector is obtained by adding the weighted difference of two of the vectors to the third.
...
Mutation, crossover, and selection are cycled until either the
maximum number of generations is attained or the candidate solutions meet a predefined accuracy threshold.
...
% Figure environment removed
\par
The multi-headed attention is primarily used for the model to simultaneously operate different parts of the input sequence, improving the performance. The normalization layer is then passed on top of the attention layer to make the model robust. This avoids the scenario of the model relying too much on specific features, which reduces over-fitting.
\par
The best-optimized set of hyperparameters obtained from each metaheuristic algorithm is then applied to each Transformer-based Neural Network and subsequently tested on the test dataset to generate the results. 
\vspace{0.25cm}

Each model's best set of batch size epoch and learning rate are summarized in Table I below:
\begin{table}[h!]
    \caption{Best set of hyperparameters for each metaheuristic algorithms applied, in the form (batch size, epoch, learning rate)}
    \label{table:1}
    \centering
    \begin{tabular}{| >{\centering}p{3cm} | >{\centering}p{4cm} | } 
      \hline
      \multicolumn{2}{|c|}{\uppercase{best set of hyperparameters for }} \tabularnewline
      \multicolumn{2}{|c|}{\uppercase{Transformer }}\tabularnewline
      \hline
      \uppercase{Metaheuristics} & \uppercase{Hyperparameters} \tabularnewline 
      \hline
      Genetic Algorithm & (80,844,0.0001) \tabularnewline
      \hline
      Particle Swarm & (173,35,0.3109) \tabularnewline
      \hline
      Differential Evolution & (24,1000,0.1) \tabularnewline
      \hline 
    \end{tabular}
\end{table}
...
The dataset \textit{Load Dataset} \footnote{Dataset: \url{https://github.com/AnuvabSen1/Ontario-Power-Dataset}} was created for this project using data scraped from the official website of Government of Canada\cite{Canada_2023}. This data contains meteorological data from Ottawa, Ontario from date 1\textsuperscript{st} January 2017 to 4\textsuperscript{th} July 2023. The data set contains the properties: date, time (in 24 hours), year, quarter, month, week of the year, day of the year, state holiday, an hour of the day, day of the week, day type, temperature (in $^\circ$ Celcius), dew point temperature (in $^\circ$ Celcius), relative humidity (in $\%$), wind speed (in kilometers/hours), visibility (in kilometers) and precipitation amounts (in millimeters). It also has two derived features humidity index and wind chill. The compiled data set contains 19 variables layered in 96,432 rows, where each row represents an hour.
\subsection{Preprocessing}
During preprocessing, we managed missing data from the compiled data set. The precipitation column was missing significant information and was thus removed from the analysis. The date, time,  For temperature, $0.03 \%$ of data was missing. Predictions were forecast up to 24 hours into the future using 3 hours of past data. The data were standardized using the StandardScaler function of sklearn.preprocessing library \cite{pedregosa2011scikit}. The dataset is split into the training dataset $D_{train}$, validation dataset $D_{val}$, and testing dataset $D_{test}$. January 1st, 2017 to December 31st, 2020 comprises the training dataset; wherein 25 \% of the training data makes up the validation dataset; and the remaining data till July 14th, 2023 constitutes the testing dataset.
...
We obtained the mean absolute percentage error (MAPE) using the proposed approach to implement the differential evolution-based hyperparameter tuning of the Transformer-based deep Neural Network offered in the preceding section. This MAPE was compared to the MAPEs generated from the proposed approach with the genetic algorithm and particle swarm optimization-based hyperparameter tuning of the custom architecture. The codes used in this paper are linked below\footnote{Code: \url{https://github.com/AnuvabSen1/Meta-Transformer}}.
\par
...
\par
\vspace{0.25cm}
Mean Absolute Percentage Error (MAPE) is used to gauge the accuracy of the entire model. It provides a measure of the average percentage difference between predicted values and the actual values. Table II below provides us with a comparison of MAPE among various metaheuristic optimization algorithms used here.
\begin{table}[h!]
    \caption{Comparison of Metaheuristics and MAPE}
    \label{table:2}
    \centering
    \begin{tabular}{| >{\centering}p{2.8cm} | >{\centering}p{4cm} | } 
      \hline
      \multicolumn{2}{|c|}{\uppercase{best set of hyperparameters for }} \tabularnewline
      \multicolumn{2}{|c|}{\uppercase{Gated Recurrent Model }}\tabularnewline
      \hline
      \uppercase{Metaheuristics} & \uppercase{mape} \tabularnewline 
      \hline
      Manual Selection & 2.07 \tabularnewline
      \hline
      Genetic Algorithm & 1.31 \tabularnewline
      \hline
      Particle Swarm & 1.28 \tabularnewline
      \hline 
      Differential Evolution & 1.11 \tabularnewline
      \hline
    \end{tabular}
\end{table}
\par
...
The second graph plots the hourly demand variation for 24 hours starting from the N \textsuperscript{th} hour as shown in Figure 5.
\par
...
\bibitem{BACANIN2023119122} Bacanin, N., Jovanovic, L., Zivkovic, M., Kandasamy, V., Antonijevic, M., Deveci, M. \& Strumberger, I. Multivariate energy forecasting via metaheuristic tuned long-short term memory and gated recurrent unit neural networks. {\em Information Sciences}. \textbf{642}, pp. 119122 (2023), \url{https://www.sciencedirect.com/science/article/pii/S0020025523007077}

\bibitem{El-kenawy_Mirjalili_Khodadadi_Abdelhamid_Eid_El-Said_Ibrahim_2023} El-kenawy, E., Mirjalili, S., Khodadadi, N., Abdelhamid, A., Eid, M., El-Said, M. \& Ibrahim, A. Feature selection in Wind Speed Forecasting Systems based on meta-heuristic optimization. {\em PLOS ONE}. \textbf{18} (2023)

\bibitem{Manikandan_Nagaraj_2022} Manikandan, S. \& Nagaraj, B. Hyperparameter tuned bidirectional gated recurrent neural network for Load forecasting. {\em Intelligent Automation \& Soft Computing}. \textbf{33}, 761-775 (2022)
