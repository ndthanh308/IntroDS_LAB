...
We  find
the  cases, when the nonlinear equation, describing  the weak shocks  and kinks can be integrated in terms of elementary functions.
...
\subsubsection{The shocks speed vs. the kinks speed}
...
From the inequalities (\ref{30}) follows that a shock  can not split into two shocks moving in the same direction.  Actually we can make even stronger statement: two shocks moving in the same direction will merge. In fact, let there is the first shock   $\varphi_2\leftarrow\varphi_0$ and the second shock $\varphi_0leftarrow\varphi_1$ ahead of it. Because of inequalities (\ref{30}) the speed of the first shock is larger,  and the speed of the second shock is smaller that $\overline{u}(\varphi_0)$. The statement is proved.
...
\subsection{Elementary solutions for weak  shocks}
\label{weak}

For weak  wave, characterized by the condition
$\varphi_1-\varphi_2\ll 1$ (for the sake of definiteness we have chosen $\varphi_1>0$),  the r.h.s. of  (\ref{system4})
...
As the result,  (\ref{system4}) can be simplified to the damped Helmholtz-Duffing (dHD) equation
\begin{eqnarray}
\label{system444}
...
\end{eqnarray}
(compare with Eq. (39) from Ref. \cite{kogan2}).

 If the assumption which lead to  Eq. (\ref{system444}) is strengthened  to $\varphi_1-\varphi_2\ll\tan\overline{\varphi}$,  there also exists  another elementary solution of Eq. (\ref{system444}).
It is shown in  Appendix \ref{ana}, that under the condition
\begin{eqnarray}
\label{g3}
\gamma=5\sqrt{\frac{(\varphi_1-\varphi_2)\sin\overline{\varphi}}{12}},
\end{eqnarray}
the  solution of   of (\ref{system444})  satisfying the   boundary conditions (\ref{12}) is
\begin{eqnarray}
\label{fi8}
\varphi(\tilde{\tau})=\varphi_2+\frac{\varphi_1-\varphi_2}
{\left(e^{\gamma\tilde{\tau}/5}+1\right)^2}.
\end{eqnarray}

 Looking at Eq. (\ref{system444})  we realise that  for $\varphi_1-\varphi_2\ll\tan\overline{\varphi}$, the condition  (\ref{g})
corresponds to strong damping and the condition (\ref{g3})  -- to critical damping
(see Appendix \ref{ana}).
...
We found the  cases when nonlinear equation describing weak shocks (kinks) can be integrated in terms of elementary functions.
...
-\frac{1}{ a^2}\left(\varphi_{n-1}-2\varphi_n+\varphi_{n+1}\right)=I/I_c,
\nonumber\\
\end{eqnarray}
...
However, because of the similarity of the terms,  the kinks and the solitons obtained in in the framework of these two considerations are qualitatively very similar.
...
\section{Elementary  particular solutions of the generalized dHD equation}
\label{ana}
...
\end{eqnarray}
where $n$ is an integer, $\gamma$, $\alpha$,  $p_1,p_3$  are  positive constants ($p=0$ is the  stable fixed point and $p_1,p_3$  are  unstable fixed points.)
...
\begin{eqnarray}
\label{5.15}
n^2m^2w_{zz}+n\left[(n+1)m^2-\alpha p_1p_3\right]\frac{w_z}{z}\nonumber\\
...
Consider the particular case $p_3=p_1$.
...
\end{eqnarray}
Note that
the condition of compatibility of (\ref{meme}) and (\ref{mumu}) (the latter with $p_3=p_1$) is
\begin{eqnarray}
\label{cond}
...
\end{eqnarray}
can be integrated by quadrature when the coefficients of the equation satisfy Eq.  (\ref{cond}). Note that we could have decreased the number of independent parameters in (\ref{44d}) by introducing the new dependent variable $\overline{p}=p/p_1^{1/n}$, after which the equation would take the form
\begin{eqnarray}
\overline{p}_{\tau\tau}+\gamma \overline{p}_{\tau}=\alpha p_1^2 \overline{p}\left(\overline{p}^{2n}-1\right).
\end{eqnarray}
...
\end{eqnarray}
Hence the solution of (\ref{5.20})
should satisfy the boundary condition
\begin{eqnarray}
\label{conditor}
\lim_{z\to \infty}z^{1/n}w(z)=p_1,
\end{eqnarray}
...
\begin{eqnarray}
\label{44b}
\frac{(n+1)m^2}{(1+z)^2}- \frac{(n+1)m^2-\alpha p_1p_3}
{z(1+z)}\nonumber\\
...
\end{eqnarray}
 ...
 A few remarks concerning the obtained solution  would be we believe relevant. First, because of the symmetry of (\ref{44}) we could have obtained Eqs. (\ref{fii2}) and (\ref{g2}) with $p_1$ and $p_3$ interchanged and $p$ changed to $-p$.  The choice between the two particular solutions was made by imposing the boundary conditions (\ref{condi}) (the first one). Second, if we want that $p(\tau)$ exists and remains finite for $\tau\in(-\infty,+\infty)$ and want to impose the boundary conditions  at $\tau=\pm\infty$, Eq. (\ref{condi}) is the only possible choice (apart from changing $p_1$ to $p_3$).  The trajectory should start in the unstable fixed point and end  in the stable fixed point.

\subsubsection{The strongly asymmetric case}

For $p_3 \gg  p_1$ there exists additional elementary solution of (\ref{44}). In this case the equation can be approximated as
 \begin{eqnarray}
\label{44c}
p_{\tau\tau}+\gamma p_{\tau}=\alpha p_3 p\left(p^n-p_1\right).
\end{eqnarray}
Comparing (\ref{44c}) with (\ref{44d}) (and keeping in mind the elementary solution of the latter (\ref{fii2})) we obtain    the  elementary  solution of (\ref{44c}) in the form
\begin{eqnarray}
\label{fon}
p(\tau)=\frac{p_1^{1/n}}
{\left\{\exp\left[\sqrt{\frac{\alpha p_3p_1}{2(n+2)}} n\tau\right]+1\right\}^{2/n}}.
\end{eqnarray}
where $\gamma$  and $\alpha$ are connected  by the equation (see (\ref{cond}))
\begin{eqnarray}
\label{g22}
\gamma=\sqrt{\frac{\alpha p_3p_1}{2(n+2)}}(n+4).
\end{eqnarray}

Looking at Eq. (\ref{44})  we realise that  for $p_3\gg p_1$, the condition  (\ref{g2})
corresponds to strong damping, when the second derivative term  in the l.h.s.
of the equation is small in comparison with the first derivative term,
and the condition (\ref{g22})  -- to critical damping, when both terms
 are of the same order of magnitude.

\subsubsection{Abel equation}

The  obtained elementary solutions  become more transparent    after we notice that  introducing new dependent variable
$y\equiv p_{\tau}$ and considering $p$ as the independent variable, we may write down (\ref{44})  as Abel equation of the second kind
\begin{eqnarray}
\label{ab}
yy_{p}+\gamma y=\alpha p\left(p^n-p_1\right)\left(p^n+p_3\right).
\end{eqnarray}

For $\gamma$ and $\alpha$ connected by the formula (\ref{g2}), Eq. (\ref{ab})  obviously has {\color{red} the}  particular solution
\begin{eqnarray}
\label{ab2}
p_{\tau}= \sqrt{\frac{\alpha}{n+1}}p\left(p^n-p_1\right)
\end{eqnarray}
 (we substituted $p_{\tau}$ for $y$ in the l.h.s. of the equation).
Integrating thus obtained differential equation
we obtain (\ref{fii2}).

Approximating (\ref{ab}) for $p_3\gg p_1$ as
\begin{eqnarray}
\label{ab5}
yy_{p}+\gamma y=\alpha p_3p\left(p^n-p_1\right)
\end{eqnarray}
we notice that for $\gamma$ and $\alpha$ connected by the formula (\ref{g22}), Eq. (\ref{ab5})   has {\color{red} the} particular solution
\begin{eqnarray}
\label{ab22}
p_{\tau}= \sqrt{\frac{2\alpha p_3}{n+2}}p\left(p^{n/2}-p_1^{1/2}\right)
\end{eqnarray}
 (we substituted $p_{\tau}$ for $y$ in the l.h.s. of the equation).
Integrating thus obtained differential equation
we obtain (\ref{fon}).

It is curious that Eq. (\ref{ab}) doesn't help much in obtaining the general solution of (\ref{44}) which, on the other hand,  can easily be obtained by integrating (\ref{5.20}). Actually, it is the other way round -- this general solution can be used to obtain parametric representation of the general solution of Abel equation (under the conditions
$p_3=p_1$  and  (\ref{cond}) \cite{polyanin}.
