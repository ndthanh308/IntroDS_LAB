...
analyse the interaction between the "sound" (small amplitude small wave vector harmonic wave) and the shock wave. Second  we would like to establish the relation between the shock waves and the kinks. And third, we would like to additionally study the weak shock waves, and, in particular, to look for the cases when the nonlinear  equation, describing weak travelling waves in the JTL, can be integrated analytically.
...
We  also analytically integrate the wave equation  describing weak shocks in the lossy JTL for the specific value of the losses parameter.
We conclude in Section \ref{concl}. In the Appendix \ref{real} we present a physically appealing model of the JTL, composed of superconducting grains. In the Appendix \ref{dif} we explain the condition for the applicability of the continuum approximation used in the paper.
...
\subsection{Weak shock waves}
\label{weak}

For
weak  wave, characterized by the condition
$|\varphi_1-\varphi_2|\ll 1$,  the r.h.s. of (\ref{system26})
...
\begin{eqnarray}
\label{zz}
\tilde{\tau}\to z=e^{\beta\tilde{\tau}},
\end{eqnarray}
where  the parameter $\beta$ will be chosen later. After the change of variable, (\ref{system444}) takes the form
\begin{eqnarray}
\label{stem44}
\beta^2z^2\varphi_{zz}+\beta(\beta+\gamma)z\varphi_z
=\alpha(\varphi-\varphi_1)(\varphi-\varphi_2)(\varphi-\varphi_3).\nonumber\\
\end{eqnarray}

We are looking in this Subsection for analytical solutions of  (\ref{system444}).
After the change of variable, one such  solution   \cite{montroll}  (existing for the appropriate relation between $\alpha$ and $\gamma$  which will be determined immediately)  can be found by inspection:
\begin{eqnarray}
\label{system445}
\varphi(z)=\varphi_2+\frac{\varphi_1-\varphi_2}{z+1}.
\end{eqnarray}
Actually, there are similar solutions for other two pares of indices,
but in the last moment we have recalled the boundary conditions
(\ref{12}), which after the change of the independent variable  turn into
\begin{eqnarray}
\label{bou4}
\lim_{z\to 0}\varphi(z)=\varphi_1,  \hskip 1cm
\lim_{z\to \infty}\varphi=\varphi_2.
\end{eqnarray}
Chosen by us solution, in distinction from other two,  satisfies the boundary conditions (\ref{bou4}).

Substituting (\ref{system445}) into (\ref{stem44})
we obtain
\begin{eqnarray}
\label{system446}
\frac{2\beta^2z^2}{(z+1)^3}- \frac{\beta(\beta+\gamma)z}{(z+1)^2}\nonumber\\
=-\alpha\left(\varphi_1-\varphi_2\right)
\frac{z[(\varphi_2-\varphi_3)z+\varphi_1-\varphi_3]}{(z+1)^3}.
\end{eqnarray}
We realize that equation (\ref{system446}) turns into identity, provided $\beta$ and $\gamma$ satisfy the relations
\begin{subequations}
%\label{5.11}
\begin{alignat}{4}
\beta(\beta+\gamma)&=\alpha(\varphi_1-\varphi_2)(\varphi_1-\varphi_3), \label{5.11b}\\
\beta(\beta-\gamma)&=\alpha(\varphi_1-\varphi_2)(\varphi_3-\varphi_2).\label{5.11a}
\end{alignat}
\end{subequations}
Solving (\ref{5.11b}), (\ref{5.11a}) we obtain
\begin{subequations}
%\label{5.11}
\begin{alignat}{4}
\beta&=\sqrt{\frac{\alpha}{2}}(\varphi_1-\varphi_2)
=\sqrt{\frac{\cos\overline{\varphi}}{12}}(\varphi_1-\varphi_2)\label{g60}\\
\gamma&=\sqrt{\frac{\alpha}{2}}(\varphi_1+\varphi_2-2\varphi_3)
=\sqrt{3\cos\overline{\varphi}}\tan\overline{\varphi}.\label{g}
\end{alignat}
\end{subequations}
So finally, if $\gamma$ satisfies the condition (\ref{g}), the solution of (\ref{system444}) with the boundary conditions (\ref{12}) is
\begin{eqnarray}
\label{fi}
\varphi(\tilde{\tau})=\varphi_2+\frac{\varphi_1-\varphi_2}
{e^{\beta\tilde{\tau}}+1}.
\end{eqnarray}
Equations (\ref{g}) and (\ref{fi}) are applicable  both to the weak shocks and to the weak kinks. In particular, for $\varphi_2=-\varphi_1$ the equations give $\gamma=0$ and
\begin{eqnarray}
\varphi(\tilde{\tau})=-\varphi_1
\tanh\left(\frac{\varphi_1\tau}{2\sqrt{3}}\right).
\end{eqnarray}

Let us return to Eq. (\ref{system444})  and strengthen the assumption which lead to the latter to $|\varphi_1-\varphi_2|\ll|\overline{\varphi}|$.
...
and Eq. (\ref{stem44}) takes the form
\begin{eqnarray}
\label{sem44}
\beta^2z^2\varphi_{zz}+\beta(\beta+\gamma)z\varphi_z
=\alpha'(\varphi-\varphi_1)(\varphi-\varphi_2).
...
\begin{eqnarray}
\label{sem445}
\varphi(z)=\varphi_2+\frac{\varphi_1-\varphi_2}{(z+1)^2}.
\end{eqnarray}
Substituting (\ref{sem445}) into (\ref{sem44})
we obtain
\begin{eqnarray}
\label{s446}
\frac{6\beta^2z^2}{(z+1)^4}- \frac{2\beta(\beta+\gamma)z}{(z+1)^2}
=\alpha'\left(\varphi_1-\varphi_2\right)
\frac{z^2+2z}{(z+1)^4}.
\end{eqnarray}
We realize that equation (\ref{s446}) turns into identity, provided $\beta$ and $\gamma$ satisfy the relations
\begin{subequations}
%\label{5.11}
\begin{alignat}{4}
\beta(\beta+\gamma)&=\alpha'(\varphi_1-\varphi_2), \label{5.11ba}\\
2\beta(2\beta-\gamma)&=-\alpha'(\varphi_1-\varphi_2).\label{5.11aa}
\end{alignat}
\end{subequations}
Solving (\ref{5.11ba}), (\ref{5.11aa}) we obtain
\begin{subequations}
%\label{5.11}
\begin{alignat}{4}
\beta&=\sqrt{\frac{\alpha'(\varphi_1-\varphi_2)}{6}}
=\sqrt{\frac{\sin\overline{\varphi}(\varphi_1-\varphi_2) }{12}} \label{g6}\\
\gamma&=5\sqrt{\frac{\alpha'(\varphi_1-\varphi_2)}{6}}
=5\sqrt{\frac{\sin\overline{\varphi}(\varphi_1-\varphi_2)}{12}}.\label{g3}
\end{alignat}
\end{subequations}
So finally, if $\gamma$ satisfies the condition (\ref{g3}), the solution of (\ref{system444b}) with the boundary conditions (\ref{12})  is
\begin{eqnarray}
\label{fi8}
\varphi(\tilde{\tau})=\varphi_2+\frac{\varphi_1-\varphi_2}
{\left(e^{\beta\tilde{\tau}}+1\right)^2}.
\end{eqnarray}
Pay attention that though (\ref{system444b}) is an approximation to (\ref{system444}), it can be integrated analytically for totally different value of $\gamma$ (and hence the analytic solutions (\ref{fi8}) and (\ref{fi}) are totally different).
...
We found the particular cases when nonlinear equation describing weak travelling waves in the lossy JTL can be integrated analytically.
...
We are grateful to  A. Abanov, M. Goldstein and Jesus Cuevas Maraver for the discussion.
\end{acknowledgments}
