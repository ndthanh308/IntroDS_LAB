...
\begin{frontmatter}
\title{Long-time asymptotics of the Hunter-Saxton equation}
...
\begin{abstract}
%\baselineskip=17pt
We study  the long-time asymptotic behavior for the solution to the weighed Sobolev initial value problem of the Hunter-Saxton equation
\begin{equation*}
\begin{aligned}
&u_{txx}-2\omega u_x+2u_xu_{xx}+uu_{xxx}=0,\quad x\in \mathbb{R},\ t>0,\\
&u(x,0)=u_0(x)\in H^{4,4}(\mathbb{R}),\quad x\in \mathbb{R},
\end{aligned}
\end{equation*}
where $\omega>0$ is a constant. Based on the inverse scattering transform,   the solution of this initial value problem is expressed in term of   the solution of  an  associated Riemann-Hilbert(RH) problem.
Further using the $\bar{\partial}$ steepest descent method, we obtain the long-time asymptotic approximations of the solution $u(x,t)$ in different space-time region. As for $\xi=\frac{y}{t}>0$,
 the solution $u(x,t)$ decays as the speed of $\mathcal{O}(t^{-1/2})$; While  for $\xi=\frac{y}{t}<0,$
 the solution is   approximated by the solution of  a parabolic cylinder model RH problem with an residual error order as $\mathcal{O}(t^{-1+\frac{1}{2p}}),\  p>2$.
\end{abstract}

\begin{keyword}
 Hunter-Saxton equation, Riemann-Hilbert problem, $\bar{\partial}$ steepest descent method, long-time asymptotics.\\[6pt]

	\textit{Mathematics Subject Classification:} 35Q51; 35Q15; 37K15; 35C20.
\end{keyword}
...
Consider the initial value problem for the Hunter-Saxton (HS) equation \cite{Lnls}
\begin{eqnarray}
&&u_{txx}-2\omega u_x+2u_xu_{xx}+uu_{xxx}=0,\quad x\in \mathbb{R},\ t>0,\label{mhs}\\
&&u(x,0)=u_0(x),  \label{mhs0}
\end{eqnarray}
where $\omega>0$ and $u=u(x,t)$ is a real-valued function. The HS equation \eqref{mhs} with  $\omega=0$  was   derived by Hunter and Saxton \cite{HS}.
%The HS equation \eqref{mhs} is integrable with the  Lax pair
%\begin{equation}\label{lax1}
%\begin{aligned}
%&\phi_{xx}=\lambda ( -u_{xx}+\omega)\phi,  \\
%&\phi_t=(\frac{1}{2\lambda}-u)\phi_x+\frac{1}{2}u_x\phi.
%\end{aligned}
%\end{equation}
The HS equation   is discovered to be a model for short capillary waves propagating under the action of gravity \cite{BKMP,FMN}.
Due to  these physical backgrounds,  the HS equation  has been widely concerned and its many properties  and  exact  solutions  have been found. For instances,
Lenells et al. established     Liouville transformation  between the hierarchies of \eqref{mhs} and the Korteweg-de Vries (KdV) hierarchies   \cite{Lens2008}.
...
Lenells provides a rigorous foundation for the geometric interpretation of the HS equation, and yields explicit expressions for the spatially periodic solutions of the related initial value problem \cite{Lenells2008}.
Holden et.al developed  an existence theory for the initial value problem to the stochastic HS equation and proved  several properties of the blow-up of its solutions \cite{Holden}.
Monvel et.al describe the long-time asymptotics for the HS equation with Schwartz initial data \cite{Monvelmain}. The local existence of strong solutions of the periodic Hunter-Saxton equation has been proved by Yin \cite{Yin}.
For initial data in Sobolev spaces $H^s(T)$, $s > 3/2$,  Ti\v{g}lay proved  that the periodic initial value problem for the modified HS equation is locally
well-posed \cite{Tiglay}.

%The inverse scattering transform (IST) procedure was first discovered by Gardner, Green, Kruskal and Miura \cite{GGKM}. As the development of the modern version of IST, the method of IST and Riemann-Hilbert (RH) problem are gradually developed to used to solve the initial value problems and the long time behavior of integrable systems \cite{ZS,ZS2,Mana,ZMana,Schu,Bikb,Bikb2}.
...
The aim of our paper is to extend the asymptotic results in \cite{Monvelmain} to solutions with initial data
in lower regularity weighted Sobolev space $H^{4,4}(\mathbb{R})$
 defined as
\begin{equation}
H^{4,4}(\mathbb{R})=\{f(x)\in L^2(\mathbb{R})|(1+|x|^4)\partial^jf(x)\in L^2(\mathbb{R}),\ for\ j=1,\cdots,4\}.
\end{equation}
 The key tool to obtain
 the long-time asymptotic behavior for the initial value problem \eqref{mhs}-\eqref{mhs0} is  the $\bar{\partial}$-steepest descent method,
which,  as a $\bar{\partial}$-generalization of the steepest descent method,  was first proposed by by McLaughlin and Miller \cite{dbar1,dbar2}.
In recent years, $\bar{\partial}$-steepest descent method shows some advantages and has been successfully used to study long-time asymptotic analysis, soliton resolution and
asymptotic stability of N-soliton solutions to integrable systems in a weighted Sobolev space \cite{dbar1,dbar2,dbar3,dbar4,dbar5,dbar6,dbar7,dbar8,dbar9,dbar10,dbar11,HG,dbar12,dbar13,dbar14,dbar15,dbar16}.

 To applying the $\bar{\partial}$-steepest descent method, we construct a new matrix RH problem connected with the initial value problem \eqref{mhs}-\eqref{mhs0} and derive a different form of reconstruction formula
 from that in \cite{Monvelmain}.
...
\end{equation*}
To find the solutions of the related RH problem, we need a new scale $(y,t)$ as
\begin{equation*}
y=y(x,t)=x-\int_x^\infty(\sqrt{m(\tilde{x},t)+1}-1)\mathrm{d}\tilde{x}.
\end{equation*}
Our main result is stated  as follows

\begin{theorem}\label{main}
Let $u_0\in H^{4,4}(\mathbb{R})$ and satisfy  \begin{equation}
-u_{0xx}(x)+1\geq\epsilon_0>0,\ for\ all\ x\in \mathbb{R},
\end{equation}
where $\epsilon_0$ is a constant. Then the solution  to the initial value problem \eqref{mhs}-\eqref{mhs0}
...
\end{equation}
  where $p>2$, $\hat{f}, f_{11}$ and $\delta_1$ are given in Section 3.
\end{itemize}
\end{theorem}

This paper is arranged as follows.
In Section 2, we first make the direct scatting  transform and  establish a scattering map
 between the initial data and the reflection coefficient.  Further by the inverse scattering transform,
  we construct a  RH problem for the initial value problem \eqref{mhs}-\eqref{mhs0}.
In Section 3, we deal with the RH problem as $\xi <0$ with two stationary points. The original RH problem is deformed into a mixed $\bar{\partial}-$RH problem by several transformations,
which can be decomposed    into a pure RH problem and a $\bar{\partial}-$problem. They
 are further approximated   by the parabolic cylinder  model RH problem with the error computed by a small-norm RH problem.
 Summering  above  results,  we  obtain    the long-time asymptotic result for the solution $u(x,t)$ for the HS equation \eqref{mhs} via the reconstruction formula.
In section 4, in a similar  way as in Section,  we discuss the RH problem as $\xi>0$ and
give long-time asymptotics for $u(x,t)$  without the detailed proof.


\section{The Riemann-Hilbert problem}
In this section, we first introduce the matrix Lax pair for the HS equation and analyze the properties of the Jost solutions. Further  we establish
scattering map  between the initial value $u_0(x)$
and the reflection coefficient $r(k)$.  A RH problem associated with  the initial value problem \eqref{mhs}-\eqref{mhs0} is   constructed.
\subsection{The Lax pair and Jost solutions}
The HS equation \eqref{mhs} can be regarded as the compatibility condition of the following Lax pair
\begin{eqnarray}
&&\tilde{\Phi}_x+\mathrm{i} k \sqrt{m+1} \sigma_3 \tilde{\Phi}=U \tilde{\Phi},\  m:=-u_{xx},\label{lax2-1} \\
&&\tilde{\Phi}_t-\mathrm{i} k\left\{\frac{1}{2 k^2}+u \sqrt{m+1}\right\} \sigma_3 \tilde{\Phi}=V \tilde{\Phi},\label{lax2-2}
\end{eqnarray}
where $\tilde{\Phi} \equiv \tilde{\Phi}(x, t, k)$ is a matrix function, and
\begin{eqnarray}
...
Let
\begin{equation}
p(x,t,k)=x-\int_x^\infty(\sqrt{m(\tilde{x},t)+1}-1)\mathrm{d}\tilde{x}-\frac{t}{2k^2}
\end{equation}
with
\begin{equation*}
p_x=\sqrt{m+1},\quad p_t=-\frac{1}{2k^2}-u\sqrt{m+1}.
\end{equation*}
Then the Jost soulutions of Lax pair \eqref{lax2-1}-\eqref{lax2-2} has the following asymptotic condition
\begin{equation*}
\tilde{\Phi}\sim e^{-ikp(x,t,k)\sigma_3},\quad x\to \pm\infty.
\end{equation*}
...
\end{eqnarray}
which allow $\Phi$ to be written as the total differential
\begin{equation}\label{ingrt}
\mathrm{d}(e^{ikp(x,t,k)\hat{\sigma}_3}\Phi)=e^{ikp(x,t,k)\hat{\sigma}_3}[(U\mathrm{d} x+V\mathrm{d} t)\Phi].
\end{equation}
In \eqref{ingrt}, given a fixed $t>0$, integrating from $\pm\infty$ to $x$ we get
...
\end{equation}
Consider another matrix Lax pair which is also inferred by the Lax pair \eqref{lax1}
\begin{eqnarray}
...
\end{eqnarray}
Substituting the Cauchy expansion of $\Phi_0$ at $x=0$ into this Lax pair, we deduce the asymptotic result
\begin{equation}\label{phi0}
\Phi_0=I+\frac{ik}{2}u_x(i\sigma_2+\sigma_3)-(ik)^2u\sigma_1+O(k^3),\quad k\to 0.
\end{equation}
Considering the linear dependence of $\tilde{\Phi}$ and $\tilde{\Phi}_0$, together with
...
and
\begin{equation*}
C_+(k)\equiv I,\quad C_-(k)=e^{ikc\sigma_3},\quad c=\int_{-\infty}^{+\infty}(\sqrt{m+1}-1)\mathrm{d}x.
\end{equation*}
...
Considering the linear dependence of $\tilde{\Phi}_{\pm}$, we have
...
\end{equation}
Considering the asymptotic formula \eqref{asymphi}, we find $|S(k)|\equiv1$ and for $k\in\mathbb{R}$,
\begin{equation*}
...
\end{equation}
Besides, $a(k)$ has no zero in $\mathbb{C}$, which is found from the structure of relating Dirac equation.
...
|r(k)|^2=1-|a(k)|^{-2}<1,
\end{equation*}
and it has been proved that for $u_0(x)\in H^{4,4}(\mathbb{R})$, $1-|r(k)|^2>c_0>0$, where $c_0$ is a constant \cite{Monvelmain}. Based on the symmetry \eqref{symt}, we can also derive that $|r(k)|=|r(-k)|$.

By the integral expression \eqref{phipm} of $\Phi_{\pm}$ with the method of iteration and the Neumann series, it has been proved that
$
...
In this part, we discuss the relation between the initial data $u_0(x)$ and the reflection coefficient $r(k)$.  Now we give the main conclusion.
\begin{proposition}\label{rk1}
Suppose the initial data $u_0(x)\in H^{4,4}(\mathbb{R})$ satisfying
$-u_{0xx}(x)+1\geq\epsilon_0>0$, then the map $u_0(x)\to r(k)$ is Lipschitz continuous from $H^{4,4}(\mathbb{R})$ to $H^4(\mathbb{R})\cap H^{1,1}(\mathbb{R})$.
\end{proposition}
...
\end{equation*}
are Lipschitz continuous from $H^{4,4}(\mathbb{R})$ to $H^{4}(\mathbb{R})$.
\end{proposition}
...
\end{equation}
where $T$ is a integral operator defined by
\begin{equation}
...
For $u_0(x)\in H^{4,4}(\mathbb{R})$, the following estimates hold:
...
\end{equation}
$(K)_{kk}$,\ $(K)_{kkk}$ and $(K)_{kkkk}$ have the totally same form  with $2i(h(x)-h(y))$ replaced by $(2i(h(x)-h(y)))^2$,\ $(2i(h(x)-h(y)))^3$ and $(2i(h(x)-h(y)))^4$. We have following estimates about these operators:
\begin{lemma}\label{tk}
For $u_0(x)\in H^{4,4}(\mathbb{R})$, the following operator bounds hold uniformly, and the operators are Lipschitz continuous of $u_0(x)$.
\begin{equation*}
...
\end{align*}
\end{lemma}
The method to prove this lemma is the same as Lemma \ref{lem1}.
...
For each $k\in\mathbb{R}$ and $u_0(x)\in H^{4,4}(\mathbb{R})$, $(I-T)^{-1}$ exists as a bounded operator from $C^0(\mathbb{R}^+)$ to itself. What's more, $\hat{L}:=(I-T)^{-1}-I$ is an integral operator with continuous integral kernel $L(x,y,k)$, which satisfies
...
\end{proof}
At the end of this part, we give a remark, which is based on Proposition \ref{rk1} and is proved by simple analysis.
\begin{remark}\label{rmk1}
For $r(k)\in H^4(\mathbb{R})$, by the Sobolev  embedding theorem, $r(k)\in C^{3}(\mathbb{R})$.
\end{remark}
...
\begin{equation}
M(k)=I+\mathcal{O}(k^{-1}),\quad z\to \infty.
\end{equation}
           \end{itemize}
\end{rhp}
Denoting $M(k)=(M_{lj}(k)),\ M_j(k)=M_{1j}(k)+M_{2j}(k)$,
from the expansion of $M(k)$ at $k=0$, we find
%\begin{eqnarray}
%&&M_1(k)\triangleq M_1(0)+M_{1,1}k+M_{1,2}k^2+\mathcal{O}(k^3)=q\left(1-ikN+(ik)^2[\frac{1}{2}N^2-u]+\mathcal{C}(k^3)\right),\qquad\qquad\nonumber\\
%&&M_2(k)\triangleq M_2(0)+M_{2,1}k+M_{2,2}k^2+\mathcal{O}(k^3)=q\left(1+ikN+(ik)^2[\frac{1}{2}N^2-u]+\mathcal{C}(k^3)\right),\nonumber
%\end{eqnarray}
%which implies that the coefficient of $k$ for $M_1(k)M_2(k)$ equals $0$, and
\begin{equation}\label{restr1}
\begin{aligned}
M_1(k)M_2(k)&=(M_1(0)+M_2(0))+\left(M_{1,1}M_{2,1}+M_{1,2}+M_{2,2}\right)k^2+\mathcal{O}(k^3)\\
&=\sqrt{m+1}(1+2k^2u+\mathcal{O}(k^3)).
\end{aligned}
\end{equation}
...
\end{equation}
Under above coordinate transformation, $\widehat{M}(k)$ obeys the jump condition
\begin{equation}
...
\begin{enumerate}
    \item Region I: For $\xi<0$, $\{k|\mathrm{Im}\theta=0\}=\mathbb{R}\cup\{k_1, k_2\}$, where $k_j$ are the two stationary points:
      \begin{equation}
      k_1=-\sqrt{-\frac{1}{2\xi}},\quad k_2=\sqrt{-\frac{1}{2\xi}}.
      \end{equation}
      For convenience, we denote $\rho_0=|k_j|=\sqrt{-\frac{1}{2\xi}}$;
      \item Region II: For $\xi>0$, $\mathrm{Im}\theta=0$ as $\mathrm{Im}k=0$ and $\pm\mathrm{Im}\theta>0$ for $\pm\mathrm{Im}k>0$, thus there is no stationary point;
\end{enumerate}

In following sections, we discuss the asymptotic behavior for the two regions respectively.
\section{The asymptotic analysis for $\xi<0$ }
In this section, we discuss the case with two stationary points.
\subsection{Triangular decomposition}
First of all, based on the decay regions as shown in Figure \ref{sign1}, we need to decompose the jump matrix $\widehat{V}(k)$ into upper and lower triangular matrix with help of such a scalar function $\delta(k)$ satisfying the RH problem as follows.
...
\delta(k)=(k-k_j)^{i\nu(k_j)}e^{i\beta_j(k)}.
\end{equation}
For $r(k)\in H^1(\mathbb{R})$, it is proved that
...
\end{equation}
which is a solution of following Riemann-Hilbert problem.
\begin{rhp}Find a $2\times 2$ matrix-valued function $M^{(1)}(k)$ with following properties:
\begin{itemize}
...
After the triangular decomposition, we make a continuous extension to deal with the jump condition.
% Figure environment removed
...
\end{eqnarray}
where $\Sigma_{jl}$ are shown in Figure \ref{lin1} and
\[
...
\end{proposition}
\begin{proof}
Considering that the functions $R_{jl},\ j=1,2,\ l=1,4$ have the same construction, we take $R_{11}$ for example.

Denote $k=k_1+\rho e^{i\alpha}$, for $k\in\Omega_{11}$, $\rho=|k-k_1|\in[0,\rho_0]$, $\alpha\in[0,\pi/4],$ and the $\bar{\partial}-$derivative
...
\end{equation*}
and $\frac{2|k|^2\xi+1}{2|k|^2}$ has nonzero boundary on $\Sigma_{jl}\setminus U_j$. We take the case as $k\in\Sigma_{11}\setminus U_1$ for example: denote $k=k_1+l e^{\frac{i\pi}{4}},\ l\in(\frac{\rho_0}{4},+\infty)$. Thus, for $1\leq p<+\infty,$
\begin{equation*}
\begin{aligned}
...
As for $k\in \Sigma_{01},$ $|V^{(2)}(k)-I|=|R_{11}e^{2it\theta}|$, \eqref{l0} is deduced by the same way.
\end{proof}
\
...
\subsection{A local RH solvable model near stationary points}
% Figure environment removed
To solve the RH problem $M^R(k)$, we use the PC mode to estimate the behavior near stationary points, which will be defined as a new local RH problem $M^{lo}(k)\equiv M^{lo}(y,t,k)$. And then we find the error function between $M^R(k)$ and $M^{lo}(k)$ to deduce the asymptotic representation of $M^R(k)$.
...
For the function $\omega_{jl}(k),\ j=1,2,\ l=1,2,3,4$, we have following estimation:
\begin{equation}
...
\end{equation*}
\end{corollary}
Therefore, by the proof in \cite{DZ1} and above results, we finally find that the contributions of every crosses $\Sigma^{lo}_j,\ j=1,2$ are separated out.
...
\subsection{Asymptotic behavior for the error function}
% Figure environment removed
...
\end{equation}
Therefore, based on the Beals-Coifman theorem, by the same method as the construction of $M^R(k)$ in Section 3, the solution of the RH problem \ref{rhpe} is represented by
\begin{equation}
...
\end{equation*}
The estimate of $I_2$ is just the same as $I_1$ because that $r'(k)\in L^2(\mathbb{R})$, so we get
$
I_2\lesssim t^{-1/2}.
$
...
\end{aligned}
\end{equation}
Summing up all above, we proved \eqref{m30}. The estimate \eqref{m31} is deduced similarly, so here we omit it.
...
\subsection{Asymptotic approximation for the solution $u(x,t)$}
Finally, we construct the long-time asymptotic approximation for the solution $u(x,t)$ of the HS equation \eqref{HS}. Inverting the transformations \eqref{trs1}, \eqref{trs2}, \eqref{trs3} and \eqref{trs4}, we have
\begin{equation}
...
\begin{proposition}
Define functions $R_j\equiv R_j(k):\bar{\Omega}_j\rightarrow\mathbb{C}, j=1,2,3,4$ with following boundary values:
\begin{eqnarray}
...
Now we give the proof of \eqref{r14}, we define $R_1$ by
\begin{equation}
...
\begin{rhp}Find a $2\times 2$ matrix-valued function $M^{(1)}(k)$ admitting following properties:
\begin{itemize}
...
\begin{dbar}Find a $2\times 2$ matrix-valued function $M^{(2)}(k)$ with following properties:
\begin{itemize}
...
\end{itemize}
\end{dbar}
\subsection{Asymptotic for the RH problem $M^R(k)$}
...
\end{proof}
\subsection{Asymptotic analysis of the pure $\bar{\partial}-$Problem}
...
As $k \rightarrow 0$, $M^{(2)}(k)$ has following asymptotic expansion:
\begin{equation}
...
\end{proof}
\subsection{Asymptotic approximation for $u(x,t)$}
...
\end{aligned}
\end{equation}
Therefore, we give the asymptotic behavior of the potential $u(x,t)$ by the reconstruction formula \eqref{rstr-u}:
\begin{equation}
...
\bibitem{HS}    K. H. John, S. Ralph,  Dynamics of director fields. SIAM J. Appl. Math.,
51(1991), 1498-1521.
...
\bibitem{dbar6}S. Cuccagna, R. Jenkins, On asymptotic stability of N-solitons of the defocusing nonlinear Schr\"{o}dinger equation, Comm. Math. Phys, 343(2016),
921-969.
...
\bibitem{dbar13}   Y. L.  Yang, E. G. Fan, Soliton resolution and large time behavior of solutions to the Cauchy problem for the Novikov equation with a nonzero background,   426(2023),
 109088.

\bibitem{dbar14}   F Wang, W. X. Ma, A $\bar{\partial}$-steepest descent method for oscillatory Riemann-Hilbert problems, 32(2022).
