...
To numerically model the cascaded processes suggested in Fig. \ref{fig:sketch}, we perform numerical simulations with multi-parameter MC method developed in our previous works \cite{Zhao2022,Lu2022}. This approach permits the incorporation of beam effects of colliding particles with arbitrary energy and polarization distribution, and the angle and polarization of final-state particles are also resolved by MC sampling of polarized cross sections, as illustrated in details. Briefly, our developed MC simulation method pairs colliding photons and electrons or positrons into a three-dimensional cell to calculate a series of single Compton scattering or BW process. The energy and momenta of the paired particles in the laboratory frame are transformed to the c.m. frame to calculate the cross section. The scattering angle $\cos{\theta}$ and azimuthal angle $\psi$ of the final-state particles are determined by MC sampling from the differential cross section, and the determined c.m. energy and momenta can be transformed back to the laboratory frame to obtain the scattered photon or lepton. In the laboratory frame, we denote $\varepsilon_\gamma$ and $\varepsilon_e$ as the energy of the photon and the kinetic energy of the electron from ICS, respectively, and $\varepsilon_{pair}$ as the energy of the electron (positron) in the BW process.
...
The polarized cross section of Compton scattering is formulated in the center of mass (c.m.) frame as (relativistic units with $\hbar=c=1$ are used throughout) \cite{Berestetskii1982}
\begin{eqnarray}
\frac{d\sigma_{\rm{c}}}{dtd\psi}=\frac{r_e^2}{32\pi^2}\frac{|M_{fi}|^2}{(s-1)^2},\label{dsigCompton}
\end{eqnarray}
...
\end{eqnarray}
\end{subequations}
In c. m. frame, $s=(\omega+\epsilon)^2$ with $\omega$ of photon energy and $\epsilon=\sqrt{\omega^2+1}$ of electron energy. The density matrixes can be expanded on the defined unit 4-vectors base, and by calculating the trace of matrix product, $|M_{fi}|^2$ can be expressed explicitly with Stokes parameters $\bm{\xi}$ and $\bm{\xi}'$ of initial and final photons, and mean spin vectors $\bm{\zeta}$ and $\bm{\zeta}'$  of initial and final electrons.
...
% \nonumber % Remove numbering (before each equation)
  \frac{d\sigma_{\rm{c}}}{dyd\psi} = \frac{r_e^2}{4x^2}\left[F_{\gamma e}+\sum_{i=1}^{3}(G_i^\gamma\xi'_i+G_i^e\zeta'_i)+\sum_{i,j=1}^{3}H_{i,j}\zeta'_i\xi'_j\right]
\end{align}
where $\zeta'_i$ and $\xi'_i$ are components of $\bm{\zeta}'$ and $\bm{\xi}'$.
After integration over $y$ (between $x/(x+1)$ and $x$) and $\psi$, one obtains the total cross section of Compton scattering
...
\end{subequations}
% Figure environment removed
Since $\sigma_{\rm{c}}$ depends on the products of $\xi_2\zeta_2$ and $\xi_2\zeta_3$ with circular polarization ($\xi_2$) of photon, and both the LSP ($\zeta_2$) and TSP $\zeta_3$ of electron, the non-polarized $\sigma_{\rm{c}}$ can be decomposed into four scenarios, as shown in  Fig. \ref{fig:sigtot}. See that the initial polarization slightly modify the amplitude of $\sigma_{\rm{c}}$ at the down ramp.


%\end{widetext}
By summation over final electron spin, the differential cross section is
\begin{eqnarray}\label{dsigsum}
\frac{d\bar{\sigma}_{\gamma e}}{dyd\psi}=\frac{r_e^2}{4x^4y^2}F_{\rm{c}},
\end{eqnarray}
...
F_1=&16 (x-y) (xy-x+y) \sin{(2\psi)}\\
F_{22}=&4 (x-y) \left(x^2y-2x^2+x y^2+2 y^2\right)\\
F_{23}=&8y(x-y)\sqrt{x-y}\sqrt{x y-x+y}\\
F_3=&16\left(x^2 y-x^2-x y^2+2 x y-y^2\right)\cos{(2\psi)}
	\end{align}
\end{subequations}
...
\end{subequations}
\end{widetext}
In c.m. frame, the normalized invariants $x$ and $y$ are expressed as $x=2\omega(\omega+\epsilon)$ and $y=2\omega(\epsilon+\omega\cos{\theta})$ with scattering angle of $\cos{\theta}$ between initial and scattered photons.
...
\end{align}
\end{subequations}
here, $(\xi_1^{(1)},\xi_2^{(1)},\xi_3^{(1)})$ and $(\xi_1^{(2)},\xi_2^{(2)},\xi_3^{(2)})$ represent the Stokes parameters of the two colliding photons denoted as '(1)' and '(2)' respectively. In c. m. frame, the Lorentz invariants $v$ and $w$ are expressed as $v=2\omega^2-2\omega\sqrt{\omega^2-1}\cos{\theta}$ and $w=2\omega^2+2\omega\sqrt{\omega^2-1}\cos{\theta}$, with $\omega$ the c.m. energy of initial photon and $\cos{\theta}$ the scattering angle between initial photon and final electron.
