...
\begin{enumerate}
\item[(i)] For every $n$-vertex graph $G$ of maximum degree at most $\Delta$ and treewidth at most $t$ there exists a tree $T$ such that $G \subseteq T \boxtimes K_s$, where $n \geq s=\Theta_\Delta(t \log n)$, $v(T) = O_{\Delta}\big(\frac{n}{s}\big)$ and $\Delta(T)\le d$. 
