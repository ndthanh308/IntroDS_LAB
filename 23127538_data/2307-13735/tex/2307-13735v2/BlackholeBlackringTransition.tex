...
Corrections to the Bekenstein-Hawking entropy of a blackhole in 4d consist of various contributions. The first set of corrections come from the Wald entropy \cite{Wald:1993nt} of higher derivative terms in the Wilsonian effective action of massless fields---present due to integrating out other massive degrees of freedom. In the context of 4d $\mathcal{N}=2$ compactifications,
...
In the IIA language, they come from a D2 brane wrapping the $\S^2$ that shrinks as we approach the conifold point. At this point and until the new massless degrees of freedom are taken into account, the string background is expected to be singular. When the new massless degrees of freedom are taken into account, we go through a geometric transition to a different Calabi-Yau.
...
In this paper, we will be interested in the D6, D4-D2-D0 multi-centered blackhole with one unit of D6 charge. 
...
\begin{equation}\label{BR sign}
	...
	\end{aligned}
\end{equation}
In this work, we will talk about the bound state only in this domain. For the charges in this domain, the 4d solution when uplifted to 5d is certainly healthy. We propose this is the physical domain of the uplifted blackring solution (as far as its contribution to the GV formula is concerned). Keeping in mind the identification $q_0=2 j_{\L}$, we need to take into account the zero point shift in entropy as follows \cite{Cyrier:2004hj}
...
In this subsection, we observe that the GV invariants counting 5d BPS states with angular momenta beyond the 5d blackhole extremality bound give rise to 4d BPS states that are stable in the large $B+iJ$ limit but become unbound in a chamber of the K\"ahler moduli space for a `reduced' central charge function corresponding to what is known as a (very) weak stability condition.  
