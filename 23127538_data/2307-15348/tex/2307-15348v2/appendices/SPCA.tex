...
whose solution is $\hat {\boldsymbol{\mu}} = \bar {\boldsymbol{x}}$.
...
\end{equation}
Therefore ${\mathit{\Sigma}}^{-1} {\mathit{S}} - {\mathit{S}} {\mathit{\Sigma}}^{-1} = 0$. Hence, ${\mathit{S}}$ and  ${\mathit{\Sigma}}^{-1}$ are two symmetric matrices that commute, so they must be simultaneously diagonalisable in an orthonormal basis.
...
We now need to find the permutation $\hat \psi \in S_p$ that minimises $g$.
...
\noindent As 
$\frac {\partial h} {\partial \ell_k} = \frac{\gamma_k}{\ell_k} - \ell_k^{-2} \lrp{\sum_{j \in \phi_{\boldsymbol{\gamma}}^{-1} \lrs{k}} \lambda_j}$, we get that {$\hat {\ell}_k = \frac 1 {\gamma_k} \lrp{\sum_{j \in \phi_{\boldsymbol{\gamma}}^{-1} \lrs{k}} \lambda_j}$}.