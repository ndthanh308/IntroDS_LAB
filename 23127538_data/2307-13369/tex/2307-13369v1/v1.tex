...
\providecommand{\geomoptions}{hmargin=3.2cm,vmargin=3.7cm}
...
\newcommand{\powser}[2]{#1[\hspace{-0.1em}[#2]\hspace{-0.1em}]}
	\newcommand{\powrat}[2]{#1(\hspace{-0.1em}(#2)\hspace{-0.1em})}
...
In Section~\ref{s:twist} we demonstrate that similar techniques can be used to prove a related fact, namely that Muller--Speyer's left twist map \cite{MulSpe-Twist} is a quasi-cluster equivalence.
...
I also thank Darius Dramburg, Franco Rota and David Speyer for useful conversations about some of the geometric arguments needed in Section~\ref{s:connected}.
...
Thus it follows from a result of Muller--Speyer \cite[Prop.~7.13]{MulSpe-Twist} that when $j\in F_0$, the Plücker coordinate $\varPlueck{\srclab{j}}|_{\openposvar_\posit}$ is a Laurent monomial in the Plücker coordinates $\Plueck{I}$ for $I\in\tgtneck_D$, and hence does not vanish on $\openposvar_\posit$.
While Muller--Speyer's more general result is stated with an automorphism (the square of the right twist) applied to $\Plueck{I}$, it follows from \cite[Thm.~12.2]{CKP} (see also Theorem~\ref{t:MS=CC} below) that the left twist inverts $\varPlueck{\srclab{j}}|_{\openposvar_\posit}$ when $j\in F_0$, and hence its square fixes this Plucker coordinate.
Since the right twist is inverse to the left twist by \cite[Thm.~6.7]{MulSpe-Twist}, its square also fixes $\varPlueck{\srclab{j}}|_{\openposvar_\posit}$.
...
\item\label{p:op-MS} If $j$ is an alternating region of $D$ and $D^\op$, then $\MSsrc{j}(D^\op)=\MStgt{j}(D)$ and $\MStgt{j}(D^\op)=\MSsrc{j}(D)$.
...
This fact, which is well-known to experts, follows by considering the local strand configuration at the face $j$, and how the labels change when passing through one of the crossings around this face.
...
\end{equation}
Since $\GLtgt_1$ quasi-coincides with $\GLsrc_1$, there is a non-frozen cluster variable $x'\in\cvars_1$ and Laurent monomials $p\in\cmons_1$ and $q\in\cmons_2$ in frozen variables such that $\GLtgt(x|_1)=\GLsrc(x'p|_1)$ and $\GLtgt(x_*|_2)=\GLsrc(q|_2)$.
...
Now we calculate
\begin{align*}
\delta^{\src}\Big(\frac{x'pq^{\deg(x)}}{x_*^{\deg(x)}}\Big)&=\frac{\epsilon\big(k_2\deg_+^{\src}(x)+k_2\deg(x)\deg_+^{\src}(x_*)\big) (x'p|_1\otimes (x_*|_2)^{\deg(x)})(x_*|_1\otimes q|_2)^{\deg(x)}}{\epsilon\big(k_2\deg(x)\deg_+^{\src}(x_*)\big) (x_*|_1\otimes x_*|_2)^{\deg(x)}}\\
&=\epsilon\big( k_2\deg_+^{\src}(x)\big)x'p|_1\otimes (q|_2)^{\deg(x)},
\end{align*}
Thus by Proposition~\ref{p:clust-Segre} and \eqref{eq:GLtgt-val2} we have
\begin{align*}
\partial\GLsrc\Big(\frac{x'pq^{\deg(x)}}{x_*^{\deg(x)}}\Big)
&=(\GLsrc_1\Segprod\GLsrc_2)\delta^{\src}\Big(\frac{x'pq^{\deg(x)}}{x_*^{\deg(x)}}\Big)\\
...
Now since $\GLtgt_1$ quasi-coincides with $\GLsrc_1$, there is a seed $s_1$ for $\clustalg{D_1}$ such that $\yhat_{j,1}^{\tgt}=\yhat_{j',1}^{\src}(s_1)$ for all mutable $j\in Q_0^1$.
Here $j\mapsto j'$ is a bijection between the mutable vertices of $Q_0^1$ and the mutable vertices of the quiver $Q(s_1)$ of $s_1$.
Similarly, there is a seed $s_2$ for $\clustalg{D_2}$ such that $\yhat_{j,2}^{\tgt}=\yhat_{j',2}^{\src}(s_2)$ for all $j\in Q_0^2$.
...
&=\epsilon\big(k_2\deg_+(\yhat_{j'}^{\src})\big)\yhat_{j'}^{\src}|_1(s_1)\otimes \varPlueck{\tgtlab{*}|_2}^{\deg(\yhat_{j'}^{\src}(s_1))}\\
...
In this subsection we recall and slightly extend a result from \cite{CKP} explaining how to categorify Muller--Speyer's left twist automorphism of $\openposvarcone_\posit$ \cite[\S6.1,Thm.~6.7]{MulSpe-Twist}, denoted by $\ltwist$; we omit the definition, since we will not need it.
We also write $\ltwist\colon\CC[\openposvarcone_\posit]\to\CC[\openposvarcone_\posit]$ for the induced map on functions.
...
&0&0&0
\end{tikzcd}\qquad\begin{tikzcd}[column sep=15pt]
&0\arrow{d}&0\arrow{d}&0\arrow{d}\\
...
\begin{prop}
\label{p:der-eq}
The derived categories $\bdcat(\gproj\CM(B))$, $\bdcat(\ginj\CM(B))$ and $\bdcat(\CM(B))$ are tautologically equivalent, in the sense that the natural functor from each category to $\bdcat(\fpmod B)$ is an equivalence.
\end{prop}
\begin{proof}
...
The upshot of the previous two results is that we have a triangle equivalence $\stab{\varphi}\colon\stabginj\CM(B)\isoto\stabgproj\CM(B)$ which lifts to a functor $\varphi$ (indeed, to the tautological equivalence) between the derived categories $\bdcat(\ginj\CM(B))=\bdcat(\fpmod B)=\bdcat(\gproj\CM(B))$.
...
Via much the same approach as used for Theorem~\ref{t:main-thm} we may show that $\ltwist\colon\CC[\openposvarcone_\posit]\to\CC[\openposvarcone_\posit]$ is a quasi-cluster morphism from the target-labelled cluster structure to the source-labelled cluster structure.
...
\begin{thm}
The twist automorphism $\ltwist\colon\CC[\openposvarcone_\posit]\to\CC[\openposvarcone_\posit]$, considered as a map of cluster algebras in which the domain carries the target-labelled cluster structure and the codomain the source-labelled cluster structure, is a quasi-cluster morphism.
\end{thm}
\begin{proof}
The statement reduces to the connected case by analogous arguments to those in Section~\ref{s:connected}, which we leave to the reader.
