...
Recall the notation $\mathcal{T}^k$ as the set of all rooted binary subtrees in the underlying \emph{complete binary tree} from Section~\ref{sec:TS-PCA-intro}. We write $\bm{v} \in \mathcal{T}^k$ if the support set of $\bm{v}$ satisfies $\mathsf{supp}(\bm{v}) \in \mathcal{T}^k$.
...
\end{align} 
denote the estimator obtained from exhaustive search. Define
\begin{align*}
    T^{*} := \argmax_F \rho(\bm{W}, {F}) \quad \text{s.t.}\quad F = \text{conv}(L_{m_1} \cup L_{m_2} \cup L_{m_3}), ~ \text{for all} ~ L_{m_1} \neq L_{m_2} \neq L_{m_3} \in \mathcal{T}^k.
\end{align*}

\begin{corollary}\label{coro:TS-PCA-fund-limits}
Let $\mathcal{T}^k$ be defined as in Section~\ref{sec:TS-PCA-intro}. There is a pair of positive constants $(c, C)$ such that the following is true. \\
...
%and any fixed $\tau \in (0,1)$, we have that with probability at least $1 - \tau$,
\begin{subequations}
\begin{align*}
...
%    \min\bigg\{1, \sqrt{\frac{1 + \lambda}{8\lambda^2}} \sqrt{\frac{k / \log(kd)}{n}} \bigg\}.
\end{align*}
\end{subequations}
...
we have $|\mathcal{T}^k| \leq (2e)^k / (k + 1)$  \citep{baraniuk2010model}, and taking logarithms results in a logarithmic factor gain over vanilla sparse PCA. Corollary~\ref{coro:TS-PCA-fund-limits}(b) provides a minimax lower bound of $\Omega(\sqrt{k / (n \log k)})$ for tree-sparse PCA, which has a logarithm gap $\sqrt{1/\log k}$ compared with the upper bound in Corollary~\ref{coro:TS-PCA-fund-limits}(a). This gap is due to the challenge of obtaining a tight upper bound for the term $\max_{\bm{z} \in \mathcal{Z}_{*}}|\mathcal{N}_H(\bm{z}; k/2)|$ (see Theorem~\ref{thm:fund-limits}) in the tree-sparse case, and we believe that a tighter bound would close such a gap.
...
We first provide the exact projection oracle.
...
\begin{corollary} \label{coro:TS-PCA-PPM}
 Suppose in Algorithm~\ref{alg:PPM} that the initialization $\bm{v}_{0} \in \mathcal{T}^{k} \cap \mathcal{S}^{d-1}$ satisfies $\langle \bm{v}_0,\bm{v}_* \rangle \geq 1/2$. There are universal positive constants $C_1,C_2, C_3$ and $c$ such that for $\lambda \geq C_1$, $n\geq C_2k$ and all $t \geq 1$, the iterate $\bm{v}_t$ from Algorithm~\ref{alg:PPM-project} satisfies
 \begin{align*}
 	\| \bm{v}_{t} - \bm{v}_{*} \|_2 \leq \frac{1}{2^t} \cdot \| \bm{v}_{0} - \bm{v}_{*} \|_2 + C_3 \sqrt{\frac{k}{n}}
 \end{align*} 
% $\{\bm{v}_{t}\}_{t=0}^{T}$ in Algorithm~\ref{alg:PPM-project} satisfy
%\begin{align*}
%    \| \bm{v}_{t+1} - \bm{v}_{*} \|_2 \leq \frac{1}{2} \cdot \| \bm{v}_{t} - \bm{v}_{*} \|_2 + C_3 \sqrt{\frac{k}{n}}
%\end{align*}
...
Assume $k^2 \leq d / e$. There exist universal positive constants $(C, C')$ such that if  $n\geq \max\{C\log d,k^{2}\}$ and
\begin{align*}
    n \geq \frac{C'\max\{\lambda^2,1\} k^2}{\lambda^2} \log(d/k^2), 
\end{align*}
then Algorithm~\ref{alg:initialization} returns an initial vector $\bm{v}_0 \in \mathcal{S}^{d - 1} \cap \mathcal{T}^k$ %obtained from Algorithm~\ref{alg:initialization} 
satisfying $\langle \bm{v}_0, \bm{v}_* \rangle \geq 1/2$ with probability $1 - C'\exp(- \min\{\sqrt{d}, n\}/C')$. %$ for some positive constant $C'$}.
\end{corollary} 
...
%\gwcomment{Should we keep the following discussion after this corollary? I think we should point the gap between this initialization method and the minimax lower bound, which motivates the SDP hardness.}
...
%\apcomment{I have not yet gone through this subsection on SDP hardness.}
%
%We use the following Assumption~\ref{assump:discrete-entries} to simplify the proof of SDP hardness. \apcomment{Explain what your logic is before diving in. You will prove an SDP lower bound. The assumption does not simplify the proof; it gives the algorithm/learner more information, and the SDP cannot estimate in spite of this additional information.} 

%\gwcomment{Add the logic before diving in. (1) solve tree sparse PCA under side information (TS-side), (2) TS-side a subclass (special case) of tree sparse PCA, (3) SDP-relaxation cannot solve TS-side $\Rightarrow$ SDP-relaxation cannot solve tree sparse PCA.}


%Based on previous discussion, there is a gap between the minimax lower bound (Corollary~\ref{coro:TS-PCA-fund-limits}) and the sample size required for initialization method (Corollary~\ref{coro:initial-TS-PCA}). 
%Corollary~\ref{coro:initial-TS-PCA} requires $\Omega(k^{2})$ samples for a good initialization so that the projected power method can guarantee geometric convergence. Such sample size seems to be loose comparing with the minimax lower bound in Corollary~\ref{coro:TS-PCA-fund-limits}. 
...
%
% given the following side information (TS-side):  Since TS-side is a subclass of the proposed tree sparse PCA, any hardness lower bound for TS-side also gives a hardness lower bound for the tree sparse PCA. \\
%
%\noindent Here, we use
\begin{align}\label{re-opt-tree-sparse}
%\widehat{\bm{v}}_{\mathsf{TS-side}} := 
\max_{\bm{v}} \;\; \bm{v}^{\top} \widehat{\bm{\Sigma}} \bm{v} \quad \text{s.t.}\quad \|\bm{v}\|_2^2 = 1,\;\|\bm{v}\|_0 = k\text{ and }\bm{v}(i)^2 \leq \bm{v}(\lfloor i / 2 \rfloor)^2,\;\text{ for all } 2\leq i \leq d.
\end{align} 
...
\begin{split}
    ...
    \\  \text{s.t.} \quad &\sum_{i=1}^{d} \bm{M}_{ii}^{2} = 1,\; \sum_{i=1}^{d}\sum_{j=1}^{d} |\bm{M}_{ij}| \leq k,\; \bm{M} \succeq \bm{0}_{d \times d} \quad \text{and}
...
%Furthermore, these sample complexity bounds are matched with computational 
%We then claim the following proposition for the SDP-relaxation~\eqref{SDP-tree-sparse-PCA}. 


\iffalse
\gwcomment{Here is the original logic: 
\begin{itemize}
	\item There is a gap between minimax lower bound and initialization method on sample size $n$.
	\item Initialization method implies that we need $n = O(k^2)$ samples to run projected power method for local convergence. (which seems not tight, comparing with the minimax lower bound)
	\item Illustrate that $n = O(k^2)$ samples is necessary in SDP hardness. (As a special case of low-degree polynoimal)
		\begin{itemize}
			\item Show SDP hardness under a special case of tree-sparse PCA. (By assuming Assumption 3)
		\end{itemize}
\end{itemize}
Although the additional assumption (Assumption 3) does not simplify the proof (compare with the proof in Ma's paper), but it is required to simplify the constraints used in (17) and (18), which keeps the same format as the SDP presented in Ma's paper. Actually, without Assumption 3, for general tree-sparse PCA, its mixed-integer programming is  
\begin{align*}
\widehat{\bm{v}}_{\mathsf{TS}}' := \argmax_{\bm{v}, \bm{z}} & ~~ \bm{v}^{\top} \widehat{\bm{\Sigma}} \bm{v} \\
\text{s.t.} & ~~ \|\bm{v}\|_2^2 = 1, - \bm{z}_i \leq \bm{v}_i \leq \bm{z}_i ~ \forall ~ i \in [d] \\
& ~ \sum_{i = 1}^n \bm{z}_i = k, ~ \bm{z}_i \leq \bm{z}_{\lfloor i / 2 \rfloor}, ~ \bm{z}_i \in \{0,1\} ~ \forall ~ 2 \leq i \leq d,
\end{align*}
and its SDP relaxation becomes more complicated
\begin{align*}
	\mathsf{SDP}(\widehat{\bm{\Sigma}}) = \max_{\bm{M}, \bm{Z} \in \mathbb{R}^{d \times d}} &\sum_{i=1}^{d} \sum_{j=1}^{d} \widehat{\bm{\Sigma}}_{ij} \bm{M}_{ij} 
    \\  \text{s.t.} \quad &\sum_{i=1}^{d} \bm{M}_{ii}^{2} = 1,\; \sum_{i=1}^{d}\sum_{j=1}^{d} |\bm{M}_{ij}| \leq k,\; \bm{M} \succeq \bm{0}_{d \times d}, \; |\bm{M}_{ij}| \leq \bm{Z}_{ij}  \quad \text{and}
    \\ &  \sum_{i=1}^{d}\sum_{j=1}^{d} \bm{Z}_{ij} = k^2, \; \sum_{i = 1}^d \bm{Z}_{ii} = k, \; \bm{Z} \succeq \bm{0}_{d \times d} \quad \text{and} \\
    & \bm{Z}_{ii} \leq \bm{Z}_{\lfloor i / 2 \rfloor\lfloor i / 2 \rfloor} \; \forall \; 2\leq i\leq d, \; \bm{Z}_{ij} \in [0, 1] \; \forall \; i,j \in [d] 
\end{align*}
}
\fi
...
\textbf{Discrete Entries.} Let $k$ denotes the number of non-zero entries of $\bm{v}_*$. We assume that $\bm{v}_*$ has discrete entries, namely that $\bm{v}_*(i) \in \big\{0, \pm k^{-1/2} \big\}$ for all $i \in [d]$.
\end{assumption} 
Under Assumption~\ref{assump:discrete-entries}, we obtain that
\begin{align*}
    \bm{v}_* \in \bigg\{ \bm{v} \in \mathcal{S}^{d-1}\; | \;  \|\bm{v}\|_{0} = k \text{ and } \bm{v}(i)^2 \leq \bm{v}(\lfloor i / 2 \rfloor)^2, \; \forall\; 2\leq i \leq d \bigg\} \subseteq \mathcal{T}^{k} \cap \mathcal{S}^{d-1},
\end{align*}
where the last step follows since the constraints $\bm{v}(i)^2 \leq \bm{v}(\lfloor i / 2 \rfloor)^2, \; \forall i \in \{2, \ldots, d\}$ ensures that $\mathsf{supp}(\bm{v}) \in \mathcal{T}^{k}$. Consequently, instead of considering the optimization problem~\eqref{eq:TS-PCA-est}, one can modify the feasible set and consider
\begin{align}\label{re-opt-tree-sparse}
\widehat{\bm{v}}_{\mathsf{TS}}' := \argmax_{\bm{v}} \bm{v}^{\top} \widehat{\bm{\Sigma}} \bm{v} \quad \text{s.t.}\quad \|\bm{v}\|_2^2 = 1,\;\|\bm{v}\|_0 = k\text{ and }\bm{v}(i)^2 \leq \bm{v}(\lfloor i / 2 \rfloor)^2,\;\forall 2\leq i \leq d.
\end{align}
In Corrollary~\ref{coro:initial-TS-PCA}, we require $C'k^{2}\log(d/k)$ (for $\lambda \geq 1$) number of samples to obtain a good initialization. In this part, we show that $O(k^{2})$ samples
required for the initialization method is almost “tight” in some sense. In particular, we consider the SDP relaxation of the optimization problem~\eqref{re-opt-tree-sparse}
\begin{align}\label{SDP-tree-sparse-PCA}
\begin{split}
    \mathsf{SDP}(\widehat{\bm{\Sigma}}) = \max_{\bm{M} \in \mathbb{R}^{d \times d}} &\sum_{i=1}^{d} \sum_{j=1}^{d} \widehat{\bm{\Sigma}}_{ij} \bm{M}_{ij} 
    \\  \text{s.t.} \quad &\sum_{i=1}^{d} \bm{M}_{ii}^{2} = 1,\; \sum_{i=1}^{d}\sum_{j=1}^{d} |\bm{M}_{ij}| \leq k,\; \bm{M} \succeq \bm{0}_{d \times d} \quad \text{and}
...
\begin{proposition}\label{prop:TS-SDP-hard}
    ...
    There are universal positive constants $c,\;c_1,\;C$ and $C_1$ such that for 
    \[
        c_1d \leq n \leq C_1 d,\quad n \leq ck^{2} \quad \text{and} \quad 1 \leq \lambda \leq \frac{d}{Cn}, 
    \]
    the optimal solution $\bm{M}_*$ of the SDP relaxation~\eqref{SDP-tree-sparse-PCA} satisfies
    \[
        \big\| \bm{M}_* - \bm{v}_{*}\bm{v}_{*}^{\top}\big\|_{2} \geq \frac{1}{5}
    \]
    with probability at least $1- \tilde{c} d^{-\tilde{c}}$ for some constant $\tilde{c} \geq 1$.
\end{proposition}
%\gwcomment{Still working on...Partially done, need to be checked by Mengqi.}\\
%\gwcomment{ The original paper has prob bound $1 - d^{-10}$. \\
%In this proposition, the pseudo-randomness for $\bm{X}$ holds with probability $1 - c d^{-c}$. The claim (41) holds with prob $1 - c \exp(- cn)$ where $n \asymp d$ in this proposition. The claim (43) holds with prob $1 - c (dn)^{- c}$. Combine all these error bound together, we have $1 - \tilde{c} d^{- \tilde{c}}$.} \apcomment{Is all this resolved?} \gwcomment{Already resolved, $1- \tilde{c} d^{-\tilde{c}}$ is the probability bound.}
%The proof of Proposition~\ref{prop:TS-SDP-hard} is in Section~\ref{proof-prop-TS-SDP-hard}. 
