...
\end{align}  
to denote the corresponding estimate from exhaustive search. Similarly to $F^*$, define
\begin{align*}
    P^{*} := \argmax_F \rho(\bm{W}, {F}) \quad \text{s.t.}\quad F = \text{conv}(L_{m_1} \cup L_{m_2} \cup L_{m_3}), ~ \text{for all} ~ L_{m_1} \neq L_{m_2} \neq L_{m_3} \in \mathcal{P}^k.
\end{align*}
...
Let $\mathcal{P}^k$ be defined as in Section~\ref{sec:PS-PCA-intro}. There is a pair of positive constants $(c, C)$ such that the following is true.
...
\noindent (a) Without loss of generality, assume $\langle \bm{v}_*, \widehat{\bm{v}}_{\mathsf{PS}} \rangle \geq 0$. Then for any $c > 0$ and $\bm{v}_* \in \mathcal{S}^{d - 1} \cap \mathcal{P}^k$, we have
\begin{subequations}
\begin{align*}
...
Corollary~\ref{coro:PS-PCA-fund-limits} is proved in Section~\ref{proof-coro-PS-PCA-fund-limits} based on Theorem~\ref{thm:fund-limits}. In particular, Corollary~\ref{coro:PS-PCA-fund-limits}(a) gives an upper bound on the estimation error of $\widehat{\bm{v}}_{\textsf{PS}}$ by showing that the statistical noise term $\rho(\bm{W}, P^*)$ is of the order $(\lambda + 1) \sqrt{k \cdot ( \ln d - \ln k)/n}$. The minimax lower bound obtained in Corollary~\ref{coro:PS-PCA-fund-limits}(b) is of the same order as the minimax lower bound given in [Theorem 1, \cite{asteris2015stay}] with the outer degree parameter $|\Gamma_{\text{out}}(v)| = (d - 2) / k$. 
...
\subsubsection{Local convergence and initialization} 

Note that both the projected power method and initialization algorithm require an exact projection oracle.


\noindent \textbf{Exact projection oracle:} We build the exact projection oracle for path-sparse PCA $\Pi_{\mathcal{P}^k}$ by picking the component with the largest absolute value in each partition (layer) for a given $(d,k)$-layered graph $G$. The formal procedure is given in Algorithm~\ref{alg:projection-PSPCA}, and has running time $O(d)$.  
...
\State Compute path sparsity vector $\bm{v}^{\tt PS}$ as follows: for its sub-vector $\bm{v}^{\tt PS}_{S_{\ell}}$, set
	\begin{align*}
		...
		\begin{array}{lll}
			~ [\bm{v}_{S_{\ell}}]_i & \text{ if component $i$ has the largest absolute value,} \\
...
\begin{corollary} \label{coro:PS-PCA-PPM}
 Suppose the initiatialization $\bm{v}_0$ in Algorithm~\ref{alg:PPM} satisfies $\bm{v}_{0} \in \mathcal{P}^{k} \cap \mathcal{S}^{d-1}$ and $\langle \bm{v}_0,\bm{v}_* \rangle \geq 1/2$. There are universal positive constants $C_1,C_2,C_3$ and $c$ such that for $\lambda \geq C_1$, $n\geq C_2k\ln(d)$,  and all $t \geq 1$, the iterate $\bm{v}_t$ from Algorithm~\ref{alg:PPM} satisfies
...
%
To obtain the initialization, note that the set $\mathcal{P}^k$ satisfies Assumption~\ref{assump:M-set-initial}. This leads us to the following corollary of Theorem~\ref{thm:initialization-method}.
...
Assume $k^2 \leq d / e$. There exists universal positive constants $(C, C')$ such that if $n\geq \max\{C\log d,k^{2}\}$ and 
\begin{align*}
    n \geq \frac{C'\max\{\lambda^2,1\} k^2}{\lambda^2} \log(d/k^2), 
\end{align*}
then the initial vector $\bm{v}_0 \in \mathcal{S}^{d - 1} \cap \mathcal{P}^k$ obtained from Algorithm~\ref{alg:initialization} satisfies $\langle \bm{v}_0, \bm{v}_* \rangle \geq 7/8$ with probability $1 - C'\exp(- \min\{\sqrt{d}, n\}/C')$. 
...
The proof of Corollary~\ref{coro:initial-PS-PCA} exactly follows the proof of Theorem~\ref{thm:initialization-method} by setting $c_0$ presented in Theorem~\ref{thm:initialization-method} as $7/8$. Thus, we omit the proof of Corollary~\ref{coro:initial-PS-PCA} in appendix. In words, Corollary~\ref{coro:initial-PS-PCA} provides an initialization method whose outputs can be used for the general projected power method (Algorithm~\ref{alg:PPM}) for path-sparse PCA when the number of samples\footnote{The constant $7/8$ in $\langle \bm{v}_0, \bm{v}_* \rangle \geq 7/8$ can be replaced by any positive constant within $(0, 1)$  provided it ensures the good region condition $\langle \bm{v}_0, \bm{v}_* \rangle > t_2(\lambda)$.} $n \geq O(k^2 \log(d/k))$. Notice the gap in sample complexity with what we obtained in Corollary~\ref{coro:initial-PS-PCA}. In the next Section~\ref{sec:examples-average-hard}, we show no randomized polynomial-time algorithm solving path-sparse PCA with $n \ll k^2$ assuming the average-case hardness of the secret-leakage planted clique problem. 
...
This section focuses on the average-case hardness of the path sparse PCA, which is obtained via a reduction from the $K$-partite planted clique (PC) detection problem, which is in turn conjectured to be hard. We require some definitions to make this precise.
...
%\gwcomment{New environment: Conjecture.}
%\gwcomment{Stating a formal conjecture like: (a) Conjecture 2.1 in 2018's paper; (b) Conjecture 1 in https://arxiv.org/pdf/2009.02609.pdf }
%
%\gwcomment{
%\begin{itemize}
%	\item Assume detection problem K-PC is hard (formal). 
%	\item Estimating $\bm{v}_*$ is hard (formal).
%		\begin{itemize}
%			\item State an estimation lower bound in Proposition (K-PC). 
%			\item (a) (assume a recovery for $\bm{v}_*$, assume K-PC is hard) (generate a detection algo using recovery) reduce estimation to detection $\bm{X} \sim K-PC_{\bm{v}}(n, k, 1/2)$ for all $\bm{v} \in \mathcal{P}^k \cap \mathcal{S}^k$
%			\item (b) estimating path-sparse PCA transfer to detection K-PC. 
%			\item (c) take the detector to PCA then detector of K-PC 
%		\end{itemize}   
%\end{itemize}
%}
...
\begin{align*}
	\liminf_{N \rightarrow \infty} \left( \mathbb{P}_{H_0}[\mathcal{A}_N(G) = 1] + \mathbb{P}_{H_1}[\mathcal{A}_N(G) = 0] \right) \geq 1. 
\end{align*}
\end{conjecture}

\begin{definition}\label{def:qual-est}
\textbf{Qualified Estimator$(\epsilon)$.} 
...
A qualified estimator $\widehat{\bm{v}}(n, d_n, k_n, \lambda_n, \epsilon)$ for path-sparse PCA is a sequence of functions $\EST_n: (\mathbb{R}^{d_n})^n \to \mathbb{R}^{d_n}$ mapping $\{\bm{x}_i\}_{i = 1}^n \mapsto \widehat{\bm{v}}$ such that if the set of samples $\{\bm{x}_i\}_{i = 1}^n$ are drawn i.i.d. from $\mathcal{D}(\lambda_n, \bm{v}_*)$ for some $\bm{v}_* \in \mathcal{S}^{d_n - 1} \cap \mathcal{P}^{k_n}$ then 
\begin{align*}
%{\color{red}\text{Should be} \limsup ?} & ~ \liminf_{n \to \infty} \Pr \left\{ \| \widehat{\bm{v}} - \bm{v}_* \|_2 \geq \frac{1}{4} \right\} \leq \frac{1}{2} - \epsilon \\
%& ~ {\color{orange}
\liminf_{n \to \infty} \Pr \left\{ \| \widehat{\bm{v}} - \bm{v}_* \|_2 < \frac{1}{4} \right\} \geq \frac{1}{2} + \epsilon
\end{align*}
for some fixed $0 < \epsilon < 1/2$. 
\end{definition}
...
There is a universal constant $c > 0$ such that the following holds. Let $\beta \in [1/2, ~ 1)$ and $0 < \epsilon < 1/2$ be fixed. Suppose the sequence of parameters $\{(k_n, d_n, \lambda_n, \tau_n)\}_{n \in \mathbb{N}}$ is in the regime
...
\end{align}
where $\lambda_n$ is known and $\tau_n$ is an arbitrarily slowly growing function of $n$. If the K-Partite PC hardness conjecture (Conjecture~\ref{conj:KPC-hardness}) holds, then there is no qualified estimator $\widehat{\bm{v}}(N_n, d_n, k_n, \lambda_n, \epsilon)$ running in time polynomial in $n$ when the sample size satisfies
\begin{align*}
	n \leq c \left( \frac{k_n^2}{2 \tau_n \log k_n} \right). 
\end{align*} 
%with a known constant $\lambda = \Theta(1)$\footnote{Here we set $k_n := \lceil c n^{1/2} \tau_n^{1/2} (\log n)^{1/2} \rceil$ which satisfies the regime $k_n = \lceil n^{\beta} \rceil$ with $\beta \in [1/2, 1)$. Moreover, since $\lambda := \left\lceil \frac{c^2 (\log 2)^2}{24 + \frac{8\log 2}{\log n}} \right\rceil$ under this setting, by letting $c \geq 12/\log 2$, we have the eigengap $\lambda \geq 5$, which could be further used to ensure the good region condition as required in Corollary~\ref{coro:initial-PS-PCA}.}. 
\end{proposition}
%
%\apcomment{Discuss in words what the implications are.}
The proof of Proposition~\ref{prop:reduction-PathPCA} is given in Section~\ref{app:reduction-PathPCA}.
%which is based on an existing average-case reduction method proposed in \cite{brennan2018reducibility}. 
%
In words, Proposition~\ref{prop:reduction-PathPCA} provides an average-case hardness lower bound on the sample size required to compute a nontrivial solution for path sparse PCA when the eigengap satisfies $\lambda = \Theta(1)$, i.e., it shows that $n = \widetilde{\Omega} \left(k^2\right)$ is necessary. 
