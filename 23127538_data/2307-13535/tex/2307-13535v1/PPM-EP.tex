...
Given any vector $\bm{v} \in \mathbb{R}^d$, let
\begin{align}
...
\For{$t = 0, 1, \ldots, T - 1$}
\State Compute $\tilde{\bm{v}}_{t + 1} = \widehat{\bm{\Sigma}} \bm{v}_t / \|\widehat{\bm{\Sigma}} \bm{v}_t\|_2$. \label{alg:PPM-compute}
...
\State Normalize to unit sphere $\bm{v}_{t + 1} = \bm{v}_{t + 1}^{\mathcal{M}} / \|\bm{v}_{t + 1}^{\mathcal{M}}\|_2 \in \mathcal{M} \cap \mathcal{S}^{d - 1}$. 
\EndFor
\end{algorithmic}
...
\begin{align*}
    F^{*} := \argmax_F \rho(\bm{W}, {F}) \quad \text{s.t.}\quad F = \text{conv}(L_{m_1} \cup L_{m_2} \cup L_{m_3}), ~ \text{for all} ~ m_1 \neq m_2 \neq m_3 \in [M].
\end{align*} 
...
\begin{align*}
    \mathbb{G}(\lambda) = \bigg\{ \bm{v} \in \mathcal{M} \cap \mathcal{S}^{d - 1}: \langle \bm{v}, \bm{v}_{*} \rangle \geq t_1(\lambda) := \frac{4}{\lambda + 1 - \rho(\bm{W}, F^{*})} + \frac{5\rho(\bm{W}, F^{*})}{\lambda - 2\rho(\bm{W}, F^{*})}\bigg\}.
\end{align*}
\end{definition}
...
\begin{theorem} \label{thm:convergence}
Suppose the eigengap satisfies $\lambda > 2 \rho(\bm{W}, F^{*})$, and condition~\eqref{eigen-gap-condition} holds. Suppose in Algorithm~\ref{alg:PPM-project} the initialization satisfies $\bm{v}_{0} \in \mathbb{G}(\lambda)$. Then for all $t\geq 1$, the iterate $\bm{v}_{t}$ satisfies
...
The proof of Theorem~\ref{thm:convergence} can be found in Section~\ref{app:PPM-EM}. Once (a) an exact projection oracle $\Pi_{\mathcal{M}}$ is accessible; and (b) an initial vector $\bm{v}_0$ is in the good region $\mathbb{G}(\lambda)$, Theorem~\ref{thm:convergence} ensures a deterministic convergence result. While the result parallels that of~\cite{yuan2013truncated} for vanilla sparse PCA, the key additional technique that we use to control the error accumulated at each iteration is based on an ``equivalent replacement'' step; see Section~\ref{proof-lemma-equivalence-of-v} for details. 
...
we present a computationally efficient initialization method given our structural assumption and matching evidence of computational hardness for two prototypical examples. %Let us now turn to proving these results. 
...
Recall that Theorem~\ref{thm:convergence} requires an initialization $\bm{v}_0$ in the good region $\mathbb{G}(\lambda)$. In this subsection, we provide such an initialization method (see Algorithm~\ref{alg:initialization}) that works when given a projection oracle, provided the following assumption holds true. 
...
\begin{assumption}\label{assump:M-set-initial}
The set $\mathcal{M}$ in Definition~\ref{cond:linear-structure} satisfies
\[
...
Assumption~\ref{assump:M-set-initial} is not guaranteed under general union-of-linear structure. However, it does include many typical examples. For instance, the sets $\mathcal{T}^k$ and $\mathcal{P}^k$ for tree-sparse or path-sparse PCA, respectively, satisfy Assumption~\ref{assump:M-set-initial} in addition to union-of-linear structure. Moreover, if the orthonormal matrix $\bm{\Phi}$ is known, one can reformulate the problem to be estimating the structured-sparse vector $\bm{\Phi}^{\top} \bm{v}_{*}$ from observations $\{\bm{\Phi}^{\top} \bm{x}_i\}_{i = 1}^n$ (see Remark~\ref{rem:eq-structured}).
...
\begin{align*}
    ...
    \begin{array}{llll}
        \widehat{\bm{\Sigma}}_{ij} - [\bm{I}_d]_{ij} - \tau/\sqrt{n} & \text{if } ~ \widehat{\bm{\Sigma}}_{ij} - [\bm{I}_d]_{ij} \geq \tau/\sqrt{n}, \\
        0 & \text{if } ~ - \tau / \sqrt{n} < \widehat{\bm{\Sigma}}_{ij} - [\bm{I}_d]_{ij} < \tau / \sqrt{n}, \\
        \widehat{\bm{\Sigma}}_{ij} - [\bm{I}_d]_{ij} + \tau/\sqrt{n} & \text{if } ~ \widehat{\bm{\Sigma}}_{ij} - [\bm{I}_d]_{ij} \leq - \tau/\sqrt{n}, 
    \end{array}
    \right. ~~ .
\end{align*}
...
Suppose Assumption~\ref{assump:M-set-initial} holds and $k^2 \leq d/e$. There exists positive constants $C_1,C_2,C_3$ and $C$ such that the following happens. Suppose $n\geq \max\{C\log d,k^{2}\}$ and let $\tau_{*} := C_1 \max\{\lambda, 1\} \sqrt{\log (d/k^2)}$. Set the thresholding level according to 
...
\end{align*}
then the initial vector $\bm{v}_0 \in \mathcal{S}^{d - 1} \cap \mathcal{M}$ obtained from Algorithm~\ref{alg:initialization} satisfies the $\langle \bm{v}_0, \bm{v}_* \rangle \geq c_0$ with probability $1 - C'\exp(- \min\{\sqrt{d}, n\}/C')$ for some positive constant $C'$.
...
The proof of Theorem~\ref{thm:initialization-method}, which builds on existing results in \cite{deshpande2016sparse}, can be found in Appendix~\ref{app:initialization}. Let us now show that the output of this algorithm serves as a valid initialization for the projected power method, since this is not immediate given that the event $\mathcal{E}_1 = \{\langle \bm{v}_0, \bm{v}_{*} \rangle \geq c_0\}$  depends on the sample $\{\bm{x}_i\}_{i = 1}^n$.
%
%
%Observe that for any $0 < c_0 < 1$, the event $\{\langle \bm{v}_0, \bm{v}_{*} \rangle \geq c_0\}$  depends on the sample $\{\bm{x}_i\}_{i = 1}^n$. 
Recall the quantities $t_1(\lambda)$ and $t_2(\lambda)$ from Definition~\ref{good-region} and Eq.~\eqref{eigen-gap-condition}, respectively. 
In Theorem~\ref{thm:initialization-method}, set $c_0 := t_2(\lambda) > t_1(\lambda)$. Suppose $\lambda \geq 5$ for convenience. Then we have that the event $\mathcal{E}_2 = \{ t_1(\lambda) < t_2(\lambda) = c_0 < 7/8\} \subseteq \{\rho(\bm{W}, F^*) < 9/400\}$ occurs with probability at least $1 - C' \exp(- \min\{\sqrt{d}, n\} / C')$.
