...
\begin{abstract} We consider the Kato and the Dynkin class and their local counterparts on a smooth Riemannian manifold as Fr\'{e}chet spaces. Based on recent results by Carron, Mondello and Tewodrose we show that for a Riemannian manifold $(X,g)$ of dimension $m\geq 2$ with spectral negative part $\sigma^-_g$ of the Ricci curvature in $L^q_{\phi_g}(X,g)+L^\infty(X,g)$ for some $q>m/2$, the function $\sigma^-_g$ is in the Kato class of $(X,g)$ if and only if $(X,g)$ satisfies a Gaussian upper heat kernel bound for small times and is locally volume doubling. Here $L^q_{\phi_g}(X,g)$ is the $L^q$-space which is weighted with the inverse volume function.\\
...
\begin{itemize}
    ...
    \item Assume $m\geq 2$. We show that if $\sigma^g_-\in \mathcal{K}(X,g)$, then one has a continuous inclusion $L^q_{\phi_g}(X,g)+L^\infty(X;g)\subset  \mathcal{K}(X,g)$ for all $q\in (m/2,\infty)$, where $L^q_{\phi_g}(X,g)$ stands for the $L^q$-space w.r.t. the measure $\phi_g(x)\Id\mu_g(x)$, with $1/\phi_g(x)$ the volume of the ball with radius $1$ around $x$. Moreover, we show that if $\sigma^g_-\in L^q_{\phi_g}(X,g)+L^\infty(X;g)$ for some $q\in (m/2,\infty)$, then one has $\sigma^g_-\in \mathcal{K}(X,g)$ if and only if $p_g(t,x,y)$ satisfies upper and lower Gaussian estimates for small times and $(X,g)$ is volume doubling for small radii. These global results make a heavy use of the recently established results from \cite{carron3}.
...
$$
with $H_g\geq 0$ the Friedrichs realization of the Laplace-Beltrami operator $-\Delta_g$ in $L^2(X,g)$ and $e^{-t H_g}$ is defined via spectral calculus. Recall that $H_g$ is the self-adjoint nonnegative operator induced by the closure of the symmetric and nonnegative bilinear form in $L^2(X,g)$ defined by 
$$
C_c^\infty(X)\times C_c^\infty(X)\ni (\psi_1,\psi_2)\longmapsto \int_X (-\Delta_g\psi_1) \cdot\psi_2 \Id\mu_g\in \IR.
$$ 

We remark that the strict positivity of $p_g$ follows \cite{gri} from the connectedness of $X$.\vspace{1mm}
...
Recall that $(X,g)$ is said to satisfy

\begin{itemize}
\item the \emph{local-in-time Gaussian heat kernel upper estimate}, if there exist constants $\alpha, \beta,\gamma>0$ such that for all $t>0$, $x,y\in X$, 
\begin{align*}
\bf{(GUE_\loc)}\quad \quad p_g(t,x,y)\leq 		\alpha \mu_g(x,\sqrt{t})^{-1}e^{-\beta\frac{\varrho_g(x,y)^2}{s}} e^{\gamma t},
\end{align*}
\item the \emph{local-in-time Gaussian heat kernel lower estimate}, if there exist constants $\alpha', \beta',\gamma'>0$ such that for all $t>0$, $x,y\in X$, 
\begin{align*}
\bf{(GLE_\loc)}\quad \quad p_g(t,x,y)\geq 		\alpha' \mu_g(x,\sqrt{t})^{-1}e^{-\beta'\frac{\varrho_g(x,y)^2}{s}} e^{-\gamma' t},
\end{align*}
\item \emph{locally volume doubling}, if there exists a constant $a>0$ such that for all $0<r<R$,
\begin{align*}
\bf{(VB_\loc)}\quad \quad \mu_g(x,R)\leq a e^{aR}\left(\frac{R}{r}\right)^m\mu(x,r). 
\end{align*}
\end{itemize}
Obviously, the same notions make sense for Dirichlet metric measure spaces (that is, pairs given by a metric measure and a Dirichlet form thereon) whose heat semigroup has a continuous integral kernel.\vspace{1mm}


As a consequence of the recently established results from \cite{carron3} we would like to highlight:
...
\end{align*}
\emph{a)} If $\sigma_g^-	\in  \mathcal{K}(X,g)$, then $(X,g)$ satisfies $\bf{(GUE_\loc)}$, $\bf{(GLE_\loc)}$ and $\bf{(VB_\loc)}$, and for all $q\in (m/2,\infty)$ there is a continuous inclusion $L^q_{\phi_g}(X,g)+L^\infty(X,g)\subset \mathcal{K}(X,g)$.\vspace{1mm}

\emph{b)} Assume there exists $q\in (m/2,\infty)$ such that $\sigma_g^-\in L^q_{\phi_g}(X,g)+L^\infty(X,g)$. Then the following statements are equivalent:
\begin{itemize}
	\item $\sigma_g^-\in \mathcal{K}(X,g)$.
	\item $(X,g)$ satisfies the local Gaussian heat kernel upper estimate and is locally volume doubling.
\end{itemize}

\end{theorem}	


\begin{proof} a) By Theorem A in \cite{carron3} the metric measure space $(X,\varrho_g,\mu_g)$ is quasi-isometric to a finite dimensional (smooth) RCD space. On the other hand, finite dimensional RCD spaces satisfy $\bf{(GUE_\loc)}$, $\bf{(GLE_\loc)}$ and $\bf{(VB_\loc)}$ (cf. \cite{jiang}). Moreover, $\bf{(GUE_\loc)}$ together with $\bf{(VB_\loc)}$, as well as $\bf{(GUE_\loc)}$ together with $\bf{(GLE_\loc)}$ and $\bf{(VB_\loc)}$ are stable under quasi-isometry of Dirichlet metric measure spaces. It follows that $(X,g)$ satisfies $\bf{(GUE_\loc)}$, $\bf{(GLE_\loc)}$ and $\bf{(VB_\loc)}$. To see the continuous inclusion $L^q_{\phi_g}(X,g)+L^\infty(X,g)\subset \mathcal{K}(X,g)$, note that by  $\bf{(VB_\loc)}$ for all $0<t<1$ one has
$$
 \mu(x,\sqrt{t})^{-1}\leq a e^{a}t^{-m/2}\mu_g(x,1)^{-1},
$$	
so that $\bf{(GUE_\loc)}$ implies
$$
p_g(t,x,y)\leq \alpha a e^{a} t^{-m/2}\mu_g(x,1)^{-1}e^{-\beta\frac{\varrho_g(x,y)^2}{s}} \leq \alpha a e^{a} t^{-m/2}\mu_g(x,1)^{-1},
$$
so that the asserted inclusion follows from Proposition \ref{einb} in combination with \eqref{inc}.\vspace{1mm}
...
\emph{Acknowledgements.} The authors would like to thank J\"urgen Voigt for very helpful discussions. The first named author has been financially supported by the DFG project SPP 2026: Geometry at Infinity. 
