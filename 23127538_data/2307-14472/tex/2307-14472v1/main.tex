...
We will focus on the R\'enyi entropy here. Its exponential, the diversity or Hill number \cite{hn} of order $\lambda$, denoted by $D_{\lambda} = \exp( H_{\lambda}) $
\begin{equation}
D_{\lambda} (\bar{p}) = \left( \sum_{\sigma} p_{\sigma}^{\lambda} \right)^{\frac{1}{1-\lambda}}
\end{equation}
where we have defined an abundance vector $\bar{p} = ( p_1, p_2, p_3, \ldots, p_N )$,
...
\begin{equation}
D_{\lambda}^Z (\bar{p}) = \left( \sum_{\sigma} p_{\sigma} \left( \frac{1}{( Z \cdot \bar{p})_{\sigma}} \right)^{1-\lambda} \right)^{\frac{1}{1-\lambda}}
\end{equation}
where
\begin{equation}
...
In \cite{maac} it was found using a phenomenological calculation based on cluster size estimation in spin models \cite{adal} that $D_0$ was maximized for  $\beta=2$, which the is the value given by  Zipf's law \cite{gkz,mejn}. This, and the predicted scaling with $S$, agreed well with numerical simulations.
...
convention that extensive quantities will be denoted
...
Here we quote the result which is 
expressed in terms of a generating function:
\begin{equation}
   K_\beta(\alpha) = \log {\rm Li}_\beta(e^{-\alpha}) 
   \label{kalpha}
\end{equation}
...
while $r \in (0,r_{\rm c})$ in the condensed phase. 
...
for the two cases mentioned above. The R\'enyi entropy density $h_\lambda(\beta,r)$ inherits its singularities from $\phi(\beta,r)$. The primary singularity is at $r_{\rm c}(\beta)$ (\ref{rc}), but
$h_\lambda(\beta,r)$ may also have a secondary singularity 
coming from $\phi(\lambda \beta,r)$ in (\ref{hphi}) which is located at 
\begin{equation}
    r_{{\rm c},\lambda}(\beta) = \frac{\zeta(\lambda \beta)}{
    \zeta(\lambda \beta-1)} .
    \label{rcl}
\end{equation}
In this respect, the R\'enyi entropy density for the zeta-urn model is behaving (unsurprisingly, given (\ref{Esig},\ref{Zsig},\ref{ps})) as an equilibrium statistical mechanical system, with singularities at two different $\beta$ values. It was found in \cite{wbe} that this was not the case for the totally asymmetric exclusion process (TASEP), where $H_2$ was calculated by combinatorial means and found to possess no secondary singularities. It was suggested there that secondary singularities would generically be absent in such non-equilibrium systems, since they were a consequence of the relations (\ref{HZ},\ref{HF}), which are peculiar to equilibrium systems. Although the distribution of balls in a zeta-urn model can be considered as arising as a non-equilibrium steady state in a zero range process (ZRP) with suitable jumping rates for the balls \cite{zrp}, we are treating it as a purely equilibrium model here.
...
% Figure environment removed
The primary singularities of $h_\lambda(\beta,r)$ at $r_{\rm c}$ (\ref{rc}) are directly related to the singularities of $\phi(\beta,r)$.
To determine the type of singularity of $\phi(\beta,r)$ at
$r=r_{\rm c}$ it is convenient to take the partial derivative of $\phi(\beta,r)$ with respect to $r$ because the corresponding 
...
$\mu_{\rm c}(\beta)=\log \zeta(\beta)$), but will not displayed in the analysis below, because we want to concentrate on the dependence 
...
\begin{equation} 
    \partial^2_{rr} \phi(\beta,r) = -E (r-r_{\rm c})^{\frac{3-\beta}{\beta-2}} + 
     o\left((r-r_{\rm c})^{\frac{3-\beta}{\beta-2}}\right) 
\end{equation}
with $E=(\beta-2)D$ being a positive function of $\beta \in (2,3)$. 
The second derivative $\partial^2_{rr} \phi(\beta,r)$ is related to
...
\begin{equation}
    \partial_{rr}^2 \phi(\beta,r) = -d_1 + D + D \log (r-r_{\rm c}) + 
...
and hence 
\begin{equation}
\lim_{r\rightarrow r_{\rm c}^+} \partial^2_{rr} \phi(\beta,r) = 
-\frac{a_1}{b_1}<0 .
\end{equation}
On the other hand 
\begin{equation}
\lim_{r\rightarrow r_{\rm c}^-} \partial^2_{rr} \phi(\beta,r) = 0
\end{equation}
...
To summarize, the second derivative $\partial^2_{rr} \phi(\beta,r)$ 
has a power-law singularity with the exponent $x$ (\ref{x}) for $\beta \in (2,3)$, a logarithmic singularity (\ref{logs}) for $\beta=3$
and a finite discontinuity for $\beta>3$ at the critical point. 
For $\beta>3$ it also contains a singular term $\sim (r-r_{\rm c})^{\beta-3}$.

The singularity of $\partial^2_{rr} \phi(\beta,r)$ at $r_{\rm c}$ is inherited by the R\'{e}nyi entropy density (\ref{hphi})
\begin{equation}
    \partial_{rr}^2 h_\lambda(\beta,r) = 
    \frac{\partial_{rr}^2 \phi(\lambda \beta,r) - \lambda
    \partial_{rr}^2 \phi(\beta,r)}{1-\lambda} .
    \label{hphi2}
\end{equation}
This is the primary singularity. 
Additionally, the R\'{e}nyi entropy can acquire 
a secondary singularity at $r_{{\rm c},\lambda}$ (\ref{rcl}) 
when $\lambda \beta>2$. The singularity type is the same as
for the primary singularity, except that it appears at 
...
\begin{equation}
    \partial^2_{rr} h_1(\beta,r) \sim 
    (r-r_{\rm c})^{\frac{3-\beta}{\beta-2}} \log(r-r_{\rm c}) .
\end{equation}
...
in other statistical mechanical models with Gibbs weights and phase transitions. 
...
\bibitem{balls2} P. Bialas, Z. Burda, and D. Johnston, On Random Allocation Models in the Thermodynamic Limit.
