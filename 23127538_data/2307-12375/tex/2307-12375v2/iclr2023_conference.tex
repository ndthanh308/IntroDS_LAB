...
However, there is currently no consensus about \emph{why} ICL improves predictions.
...
This is related to recent experiments by \citet{wei2023larger}, which we discuss at the end of this section.
...
\looseness=-1
\textbf{Experiments of \citet{wei2023larger}.}
Recently, and independently from us, \citet{wei2023larger} have also studied ICL with flipped and arbitrary labels.
In broad strokes, while they focus on the effects of model scale and instruction-finetuning, we study if LLMs update pre-training preference consistently with expectations about conventional learning algorithms.
Our experiments with flipped and arbitrary labels here and in \S\ref{sec:extended_results} corroborate many of their main findings.
However, similar to our discussion of \citet{min2022rethinking}, we again find that probabilistic metrics can reveal changes in predictions not evident from accuracies.
In particular, we find once more that predictions of smaller models do react and that, for larger models, there is usually a significant difference between ICL predictions for flipped/arbitrary labels and the default in-context labels.
Most relevant to NH2 here, based on our experiments, we would not claim that `large models can override prior knowledge from pretraining [\dots] in-context', given the performance gap, particularly prominent for probabilistic metrics, that we observe, e.g.~in \cref{fig:flipped}.
We continue our discussion of \cite{wei2023larger} in \S\ref{sec:wei_discussion}.
...
\section{Discussion of \citet{wei2023larger}}
\label{sec:wei_discussion}
We here discuss some of the main conclusions of \citet{wei2023larger} in the context of our experiments with flipped and arbitrary labels in \S\ref{sec:flipped_arbitrary} and beyond.
Note that, while we study open source models from the Falcon and LLaMa families, \citet{wei2023larger} study closed source PaLM \citep{chowdhery2022palm}, Flan-PaLM \citep{chung2022scaling} and GPT-3-based models \citep{brown2020language} across a different selection of NLP tasks.

Our experiments with flipped and arbitrary labels in \S\ref{sec:flipped_arbitrary} and \S\ref{sec:extended_results} corroborate their results that `large models can flip their predictions to follow flipped exemplars' and that `instruction-tuned models are more reluctant to follow flipped labels' (cf., e.g., Falcon models in \cref{fig:flipped_app_sst2} and following).
However, our investigation reveals that their conclusion that `small models do not change their predictions when seeing flipped labels' usually does not hold: probabilistic metrics show that predicted probabilities often change in reaction to randomized (\S\ref{sec:random}) or consistently flipped labels even if the accuracy stays unchanged (at least for tasks where performance is better than random on the default labels).
Further, in contrast to \citet{wei2023larger}, we find that smaller models, in particular from the LLaMa family, can in fact learn input–label mappings, although they are typically outperformed by larger models (cf.~\S\ref{sec:author_id} or \cref{fig:flipped_app_sst2} and following).
Lastly, and most relevant to NH2 in \S\ref{sec:flipped_arbitrary}, based on our experiments, we would not make the claim that `large models can override prior knowledge from pretraining [\dots] in-context', given the plateauing behavior and consistent performance gap---particularly noticeable in probabilistic metrics---that we observe in \cref{fig:flipped}.
