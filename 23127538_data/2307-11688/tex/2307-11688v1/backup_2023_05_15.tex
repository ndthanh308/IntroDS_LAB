...
However, numerous domain-specific reviews have highlighted the need for a precise mathematical foundation for key concepts in the field---including the term ``\textit{explanation}'' which still lacks a precise definition. These reviews also call for a sound and unifying formalism for XAI, which can help researchers navigate a rapidly growing body of knowledge and avoid the emergence of ill-posed questions. 
To address this gap, this paper proposes the first formal and unifying theory of explainable AI. Using the formal and abstract language of category theory, we provide the first formal definitions for all essential terms in XAI. Based on these definitions, we propose a taxonomy of XAI methods, demonstrating how the introduced theory can be used to categorize the main classes of XAI systems currently studied in literature.
...
However, several surveys grumble that key fundamental notions of
% at the very core of the idea of 
explainable AI still lack a formal definition, and that the field as a whole is missing a unifying and sound formalism~\citep{adadi2018peeking,palacio2021xai}.
% , despite a few recent review papers~\citep{Das2020OpportunitiesAC,palacio2021xai}. 
The very notion
% of this gap is that the very 
of ``\textit{explanation}'' represents a pivotal example as it still lacks a proper mathematical formalization, as demonstrated by the ``definitions'' in the current literature:
% {\small
% \begin{displayquote}
...
We argue that the absence of a rigorous formalization of key explainable AI notions may severely undermine progress in the field by leading to ill-posed questions, annoying clutter in taxonomies, and narrow perspectives on future research directions. In contrast, a mathematical formalization would provide a solid foundation for XAI notions and taxonomies, leading to a more organized and efficient research field. This way, researchers could easily identify knowledge gaps and promising research directions---including the exploration of the (currently overlooked) theoretical side of explainable AI. 

Category theory is a process-oriented mathematical formalism and, for this reason, it is widely used for formal specifications of programs in theoretical computer science~\citep{Abramsky2004ACS, Selinger2001ControlCA, Swan2022, Turi1997TowardsAM,stein2021}.
We propose a categorical foundation for XAI concepts, following a growing trend to properly formalize AI notions~\citep{katsumata19,aguinaldo2021graphical,shiebler2021category,cruttwell2022categorical,ong2022learnable}. 

\textbf{Contributions.} We propose the first-ever mathematical formalization of fundamental XAI notions. 
...
We introduce the minimal notions of category theory required for our formalization of XAI notions.
Free monoidal categories and string diagrams represent the backbone structure of ``explaining agents'' (\Cref{sec:moncat}); Cartesian streams form a category that models dynamic processes such as ``learning'' (\Cref{sec:cartstream}); signatures and institutions formally define what an ``explanation'' is (\Cref{sec:inst}). 
...
\subsection{Free Monoidal Categories and String Diagrams}
\label{sec:moncat}
...
Examples are the category $\cat{Set}$, whose objects are \emph{sets} and morphisms are \emph{functions}, and the category $\cat{Vec}$, whose objects are \emph{vector spaces} and morphisms are \emph{linear maps}.
Different categories can also be connected using a special operators called \emph{functors} i.e., mappings of objects and morphisms from one category to another (preserving compositions and identity morphisms).  
For instance, there is a functor $U$ from $\cat{Vec}$ to $\cat{Set}$ that forgets the vector space structure. 
It associates to each vector space $\B V\in\cat{Vet}^o$ the set $U(\B V)=V\in\cat{Set}^o$, and to each linear mapping $f\in\hom(\B V,\B W)$ in $\cat{Vet}$ the function $U(f) = f \in\hom(V,W)$ in $\cat{Set}$.
...
A common process in machine learning involves the update of a function parameters based on the \emph{feedback} of an error function. To model this process we can use \emph{feedback monoidal categories}.
...
A \emph{feedback monoidal category} is a symmetric (cf. Appendix~\ref{app:symm-mon-cat}) monoidal category \(\cat{C}\) endowed with a functor $F:\cat{C} \rightarrow \cat{C}$, and an operation \(\fbk[S] \colon \hom (X \times F(S), Y \times S) \to \hom (X,Y)\) for all \(X,Y,S\) in \(\cat{C}^o\), which satisfies a set of axioms (cf.  \Cref{app:feedback-cat}).
\end{definition}
...
In learning processes, optimizing the error function often involves a sequence of feedback iterations. 
Following~\citep{katsumata19}, we use \emph{Cartesian streams} (cf. \Cref{app:streams}) to model this kind of processes.
These form a feedback monoidal category that is well-suited for handling (possibly infinite) streams of objects and morphisms.
\begin{definition}[\citep{katsumata19,uustalu2008comonadic}] 
Let $\cat{C}$ be a Cartesian (cf. Appendix~\ref{app:symm-mon-cat}) category. We call $\Stream{\cat{C}}$ the category of \emph{cartesian streams} over $\cat{C}$, whose objects \(\stream{X} = (X_0, X_1, \dots)\) are countable lists of objects in $\cat{C}$, and given \(\stream{X},\stream{Y}\in \Stream{\cat{C}}^o\), the set of \emph{morphisms} $\hom(\stream{X}, \stream{Y})$ is the set of all \(\stream{f} \colon \stream{X} \to \stream{Y}\), where \(\stream{f}\) is a family of morphisms in $\cat{C}$, \(f_n \colon X_n \times \cdots \times X_0 \to Y_n\), 
...
\item \(\tilde{f}_{n+1} \defn (\id{X_{n+1}} \times \cp_{X_n \times \cdots \times X_0}) \dcomp (f_{n+1} \times \tilde{f}_n)\)
\end{itemize}
...
form a feedback monoidal category.
\end{theorem}
...
%\end{definition}
Institutions can model a broad variety of formal languages, like logic languages and equational theories. However, we will show how institutions can also describe almost any form of ``explanation'' used in the literature.
% In the remaining of this section we provide few examples.
\begin{example}
    First-Order Logic (FOL) is an institution where signatures are represented by sets of relations\footnote{We note that both constant and function symbols can be seen as special cases of relations.} as objects and arity-preserving functions as morphisms. Sentences and models are defined as usual by FOL formulas and standard FOL structures. 
\end{example}
...
%%%%%%%%old FOL
% A simple example of institution is given by First-Order Logic (FOL). The category of signatures in FOL consists in sets of relations with their relative arities as objects, and arity preserving functions which intuitively are vocabulary translators as morphisms. 
%     Given any signature, sentences and models are defined as usual by FOL formulas and standard FOL structures. 
%%%%%%%
...
In this section we use feedback monoidal categories and institution theory to formalize fundamental (X)AI notions.
...
To this end, we first introduce the definition of ``explaining learning agent'' (Section~\ref{sec:learning-agent}) as a morphism in free feedback monoidal categories.
...
% generated by a \emph{model}, an \emph{optimiser} and an \emph{explainer} morphisms
Then we describe a functor instantiating this abstract concept in the concrete feedback monoidal category of \(\Stream{\Set}\) (Section~\ref{sec:interpretation}). 
...
In this section, we formalize the abstract concept of an ``explaining learning agent'', drawing inspiration from ~\citet{cruttwell2022categorical, katsumata19, wilson2021reverse}.
At a high level, a learning process can be characterized as an \emph{iterative process with feedback}. This process involves a function (known as the \emph{model}) which updates its internal states (e.g., a set of \emph{parameters}) guided by some feedback from the environment (often represented by an \emph{optimizer}).
...
\begin{itemize}
    ...
    \item the \textit{explainer} morphism $\eta: X \times P \rightarrow Y \times E$ which produces predictions in $Y$ and explanations in $E$;
    \item the \textit{optimizer} morphism \(\nabla_Y \colon Y \times Y \times P \to P\), which produces updated parameters in \(P\) given supervisions in \(Y\), model predictions in \(Y\), and parameters in \(P\).   %through its explanations in $E$.
...
\begin{definition}
We call $\mathsf{XLearn}$ the free feedback Cartesian category generated by the objects $X,Y,P$ and $E$, and by the morphisms $\eta: X \times P \rightarrow Y\times E$ and $\nabla_{Y}: Y \times Y \times P \rightarrow P$.
% % Figure environment removed
\end{definition}
% \begin{wrapfigure}[3]{r}{0.45\textwidth}
% \vspace{-0.2cm}
...
% \begin{remark}
The introduction of these morphisms allows us to establish a formal definition for an abstract learning agent that produces explanations\footnote{It is worth noting the object symbols used here solely represent the type of an object (e.g., a specific vector space). Consequently, objects represented by the same symbol may still have a different content.}.
% This is also what happen in classic machine learning, e.g. when an optimizer tries to match the predictions of a neural model and the labeled samples belonging being both represented as vectors in $\mathbb{R}^n$.  
% \end{remark}
 %\begin{definition}
 % [Abstract explainer and optimizer]
...
\begin{definition}
% [Translators] 
We call \emph{agent translator} a functor \textbf{$\mc{T}$} from $\mathsf{XLearn}$ to \(\Stream{\Set}\).
\end{definition}
Among the diverse range of translators, we distinguish two significant classes: those that instantiate learning agents (\Cref{def:concrete-la}) and those that instantiate explainable learning agents (\Cref{def:xla}). Intuitively, a concrete learning agent is an instance of an abstract learning agent that does not provide any explanation while a concrete explaining learning agent does output an (non-empty) explanation.
...
Let $\mc{T}$ be an agent translator such that $\mc{T}(E) = \{*\}^{\mathbb{N}}$. % the one element set is the monoidal unit for \times (using the empty set would make everything collapse because \emptyset \times X = \emptyset)
We call \textit{concrete learning agent}, or simply \textit{learning agent (LA)}, the image $\mc{T}(\alpha)$ being $\alpha$ the abstract learning agent.
...
\end{definition}
We recall that the singleton set, $\{*\}$, is the neutral element of the Cartesian monoidal product, which conveys the absence of explanations. In this case, the explainer will be simply called \textit{model}.
% and we will remove the explicit dependence on $\{*\}$ in the output space.
To instantiate explainable learning agents instead, we introduce two distinct types of translators: the semantic and the syntactic translator. This choice is motivated by the foundational elements of institution theory, namely sentences and models: Sentences correspond to well-formed \emph{syntactic} expressions, while models capture the \emph{semantic} interpretations of these sentences~\citep{tarski1944semantic,goguen1992institutions}. We will collectively refer to concrete instances of both syntactic and semantic explaining learning agents as  ``explaining learning agents (XLA)''.
\begin{definition}
\label{def:xla}
% [Concrete learning agent]
Given an agent translator $\mc{T}$ and an institution $I$, we call \textit{concrete syntactic} (\textit{semantic}) \textit{explaining learning agent} the image $\mc{T}(\alpha)$, being $\alpha$ the abstract learning agent and
% of the abstract learning agent ${\displaystyle \alpha = \fbk[P]\left((\id{Y}\times\id{X}\times\nu_P);(\id{Y}\times g \times \id{P});\nabla_Y\right)}$,
$\mc{T}(E) = Sen(\Sigma)^{\mathbb{N}}$ ($\mc{T}(E) = Mod(\Sigma)^{\mathbb{N}}$) for some signature $\Sigma$ of $I$.
\end{definition}
...
The high degree of generality in this formalization enables the definition of (almost) any real-world learning setting and learning agent. Indeed, by utilizing Cartesian streams as the co-domain of translators, we can effectively model a wide range of learning use cases, including (but not limited to) those involving iterative feedback. This enables us to capture the essence of some of the most popular learning algorithms such as decision trees or deep neural networks within our formal framework.
...
% %%%
To simplify the notation, in the remaining of the paper we use of the following shortcuts: $\mathcal{X} = \mc{T}(X)$, $\mathcal{Y} = \mc{T}(Y)$, $\mathcal{P} = \mc{T}(P)$, $\mathcal{E} = \mc{T}(E)$, $\hat{\eta}=\mc{T}(\eta)$  and $\hat{\nabla}_{\mathcal{Y}}=\mc{T}(\nabla_Y)$.  
%%%
...
\label{ex:mlp}
In our setting, a classic multi-layer perceptron (MLP,~\citep{rumelhart1985learning}) classfier with Adam optimizer can be seen as an instance of an abstract explaining learning agent whose translator functor is defined as follows (\Cref{fig:LA}):
...
\end{example}
% Figure environment removed
In this example we successfully model an MLP by constraining the components of the morphism $\eta$ to have constant values $\hat{\eta}_i = \text{MLP}$ (independent of the first $i-1$ inputs). However, by removing this constraint, we can instantiate a broader class of learning agents including e.g., recurrent neural networks~\citep{hopfield1982neural,hochreiter1997long} and transformers~\citep{vaswani2017attention}. In these scenarios, the neural functions become dependent on all previous inputs, effectively capturing the input stream. Additionally, we can also model learning settings where the model's architecture changes over time, as in neural architecture search~\citep{elsken2019neural}.
\begin{example}
\label{ex:nas}
% [Neural Architecture Search]
A classical Neural Architecture Search \citep{elsken2019neural} with Adam optimizer can be seen as an instance of an abstract explaining learning agent whose translator functor is defined as follows:
    $\mc{X}= (\mathbb{R}^n)^{\mathbb{N}}$, $\mc{Y}=([0,1]^m)^{\mathbb{N}}$, $\hat{\eta}_i = \text{MLP}_i$ being a different neural networks for every step, $\mc{P}$ the space of the MLP parameters, e.g. $\mc{P}=(\mathbb{R}^p)^{\mathbb{N}}$, $\hat{\nabla}_{\mc{Y}_i}$ being the Adam optimizer, and $\mc{E} = \{*\}^{\mathbb{N}}$.
\end{example}
...
We can now use our theoretical framework to provide the first formal definition of the term ``explanation'', which embodies the very essence and purpose of explainable AI.
...
% [Explanation]
    ...
    an \emph{explanation} $\mc{E} = \mc{T}(E)$ in a language $\Sigma$ is a set of $\Sigma$-sentences (\emph{syntactic} explanation) or a model of a set of $\Sigma$-sentences (i.e. a $\Sigma$-theory) (\emph{semantic} explanation). 
\end{definition}
...
\label{ex:logsig}
    ...
    $\varepsilon = x_{\textit{flies}}\wedge\neg x_{\textit{animal}}\rightarrow x_{\textit{plane}}$ while a model of $\varepsilon$ e.g., % Figure removed, is a possible semantic explanation. 
...
\section{Categorical Taxonomy of Explainable AI}
\label{sec:tax}
In this section, we aim to develop a theory-grounded taxonomy of XAI methods that goes beyond current taxonomies and catalogues existing approaches in a systematic and rigorous manner. We recognize the importance of such a taxonomy due to the ongoing debates and challenges faced by current taxonomies in comprehensively encompassing all existing XAI methods. Indeed, existing taxonomies in the XAI literature have only provided subjective viewpoints on the field, categorizing methods based on controversial criteria such as: the scope/methodology/usage of explanations~\citep{Das2020OpportunitiesAC}; the how/what/why of explanations~\citep{palacio2021xai}; the relationship between explainers and systems being explained, e.g., intrinsic/post-hoc, model-specific/agnostic, local/global~\citep{molnar2020interpretable}; or the specific data types, such as images~\citep{kulkarni2022explainable}, graphs~\citep{li2022survey}, natural language~\citep{danilevsky2020survey}, and tabular data~\citep{di2022explainable}. However, these taxonomies lack a solid and grounded motivation and rely primarily on subjective preferences. As a result, they are unable to draw truly universal conclusions and provide a comprehensive understanding of the field. On the contrary, our taxonomy aims to fill this gap by providing a comprehensive classification of XAI methods that is grounded in our theoretical framework. In the following, we show that our framework can effectively describe existing macro-classes of XAI methods (\Cref{sec:xai-structures}). Furthermore, we demonstrate that our framework offers valuable insights into overlooked aspects of explainable techniques, specifically the distinction between semantic and syntactic explanations (\Cref{sec:xai-semantics}).
...
Here we show how the proposed theory can serve as a sound formalism for diverse XAI approaches. As an example, we use our categorical framework to explicitly describe the following macro-categories of XAI methods~\citep{molnar2020interpretable,Das2020OpportunitiesAC,palacio2021xai}: intrinsic and post-hoc methods, model-agnostic and model-specific methods, forward-based and backward-based methods, and even more recent approaches such as concept bottleneck models. 
...
% \begin{definition}
 ...
 An intrinsic explainer is a XLA explainer $\hat{\eta}$ whose input objects are parameters $\mc{P}$ and a set of entries of a database $\mathcal{X}$:
...
Common intrinsic explainers are logic/rule-based~\citep{breiman1984classification,friedman2008predictive,schmidt2009distilling,yang2017scalable,manhaeve2018deepproblog,ciravegna2023logic,barbiero2023interpretable}, linear~\citep{verhulst1845resherches,nelder1972generalized,santosa1986linear,tibshirani1996regression,doshi2015graph,hastie2017generalized}, and prototype-based~\citep{kaufmann1987clustering,fix1989discriminatory} approaches; while well-known post-hoc explainers include saliency maps~\citep{simonyan2013deep,selvaraju2017grad}, surrogate models~\citep{ribeiro2016should,ribeiro2018anchors,lundberg2017unified}, and some concept-based approaches~\citep{kim2016examples,ghorbani2019interpretation,ghorbani2019towards,espinosa2022concept}.
...
\paragraph{Model-Agnostic vs Model-Specific}
...
Given an LA model $\hat{\mu}: \mathcal{X} \times \mathcal{P} \rightarrow \mathcal{Y}$, a model-agnostic explainer is an XLA explainer $\hat{\eta}:\mc{X}'\times\mc{P}'\to\mc{Y}'\times\mc{E}$, such that $\mc{X}' = \mathcal{X} \times \mathcal{P}$.
...
Typical examples of model-agnostic explainers include surrogate models~\citep{ribeiro2016should,ribeiro2018anchors,lundberg2017unified} and some concept-based approaches~\citep{kim2016examples,ghorbani2019interpretation,ghorbani2019towards}. Among renowned model-specific explainers instead we can include all model-intrinsic explainers~\citep{molnar2020interpretable} and some post-hoc explainers such as saliency maps~\citep{simonyan2013deep,selvaraju2017grad} as they can only explain gradient-based systems.
...
Another relevant difference between XAI methods for gradient-based models is based on whether the explainer relies on the parameters~\citep{zeiler2014visualizing,zintgraf2017visualizing,petsiuk2018rise} or the gradient of the loss on the parameters~\citep{simonyan2013deep,selvaraju2017grad}.
...
\textbf{Concept bottleneck models} Concept bottleneck models~\citep{koh2020concept} are recent XAI architectures which first predict a set of human-understandable objects called concepts and then use these concepts to learn downstream classification tasks. Our framework allows to formally define even these advanced XAI structures as follows: A concept bottleneck model is system composed of two learning agents: a concept encoder $\hat{\mu}: \mathcal{X} \times \mathcal{P} \rightarrow \mathcal{Y}$ and a task predictor $\hat{\eta}:\mc{X}'\times\mc{P}'\to\mc{Y}'\times\mc{E}$, such that $\mc{X}' = \mathcal{Y}$, where $\mathcal{Y}$ is a set of human-understandable symbols:
\begin{center}% Figure removed\end{center}

Overall these examples demonstrate the flexibility and expressive power of our categorical framework to encompass existing XAI approaches.
...
\subsection{Syntactic vs Semantic Explanations}
\label{sec:xai-semantics}
Common XAI taxonomies often overlook the distinction between syntactic and semantic approaches. However, our \Cref{def:xla} provides a natural distinction opening to a novel perspective to understand XAI methods. On the one hand, syntactic approaches work on the structure of symbolic languages and operate on $\Sigma$-sentences. Notable examples of syntactic approaches are proof systems such as natural deduction~\citep{prawitz2006natural} and sequent calculi~\citep{takeuti2013proof} which were designed to operate on formal languages such as first-order logic.  On the other hand, semantics approaches provide explanations related to the meaning or interpretation of a language. Most XAI techniques actually fall into this class of methods as semantic explanations provide an immediate connection with a concrete problem at hand~\citep{simonyan2013deep,ribeiro2016should,lundberg2017unified,karasmanoglou2022heatmap}. However, this strong connection with a specific setting often makes semantic explanations restricted and brittle, thus hindering human understanding~\citep{ghorbani2019interpretation,rudin2019stop}. For this reason, a promising but often overlooked research direction consists in lifting semantic explanations into a symbolic language in order to provide a syntactic explanation~\citep{breiman1984classification,letham2015interpretable,guidotti2018local,ribeiro2018anchors,costa2018automatic,ciravegna2023logic}.
This trend suggests that symbolic languages such as FOL or natural language are preferable for conveying meaningful messages as explanations. 
By recognizing the importance of this distinction, our definition can provide a more comprehensive framework for understanding and evaluating XAI methods.



\begin{example}
    Saliency maps are classic examples of semantics XLAs. To formally describe this case study  we first need to define its institution $I_{SM}$. 
    $I_{SM}$ 
    can be defined as a fragment of FOL, but with all the signatures' objects consisting of a single predicate. 
    % (e.g. binary in case of 2D-images). 
    Intuitively, in a classification task this relation represents the saliency degree of each pixel in a certain picture for the class prediction. Sentences and models are defined as in FOL by using the provided signature. For instance, the model shown in Figure \ref{fig:salmap} is a semantic explanation for the task of image recognition \fg{aggiungi dettagli a seconda della figura messa}. However, a syntactic explanation describing the semantics model can be easily expressed in $I_{SM}$. Let assume $\Sigma=\{S\},p_1,p_2,\ldots\}$ with $S$ the saliency predicate per pixel, and $p_i$ the constant for the $i$-th pixel. Assuming to collect the highlight pixels in the figure in the set $\textit{RelPix}$, the sentence expressing the saliency map will be:
$\displaystyle\bigwedge_{p\in\textit{RelPix}}S(p)$ .
\begin{wrapfigure}
   [3]{r}{0.25\textwidth}
\vspace{-0.4cm}
% Figure removed    
...
This paper presents the first formalization of key explainable AI notions and the first theory-grounded taxonomy of the field. As with any mathematical definition, our approach represents one possible formalization of the foundational notions of XAI, facing the challenging task of formalizing vague concepts such as ``explanation'', acknowledging the ongoing debate over their meaning, not only within the AI community but also in philosophy, epistemology, and psychology. Nonetheless, as described in Section \ref{sec:framework}, our definitions offer a robust formalization of XAI notions while maintaining sufficient generality to encompass most of the relevant instantiations of AI concepts, and using the well-enstablished mathematical framework of category theory \citep{Abramsky2004ACS, aguinaldo2021graphical, cruttwell2022categorical, ong2022learnable, Selinger2001ControlCA, shiebler2021category, Swan2022, Turi1997TowardsAM}. This formalization aims to serve as a stepping stone for XAI researchers to conduct further rigorous studies on these notions.
...
This work presents the first formal theory of explainable AI. In particular, we formalized key notions that were still lacking a rigorous definition. We then use our definitions to propose the first theory-grounded taxonomy of the XAI literature. 
% \todo{add limitations here} \mike{no need of limitation again you already mention above} 
Through this work, we provide a first answer to the pressing need for a more sound foundation and formalism in explainable AI as advocated by the current literature~\citep{adadi2018peeking,palacio2021xai}. While our taxonomy provides guidance to navigate the field, our formalism strengthens the reputation of explainable AI encouraging a safe and ethical deployment of AI technologies.
We think that this work may contribute in paving the way for new research lines in XAI, e.g. quantitatively investigating the mutual understanding of two agents using different signatures, or even trying to agree on different explanations. 
...
The monoidal unit is an object \(\monoidalunit \in \cat{C}^o\), which represents the ``absence of inputs or outputs'' and needs to satisfy \(X \times \monoidalunit \iso X \iso \monoidalunit \times X\), for each \(X\in \cat{C}^o\).
...
It is customary to indicate with \(\times\) the monoidal product given by the cartesian structure and with \(1\) the corresponding monoidal unit.
Cartesian categories are equipped, for every object \(X\), with morphisms \(\cp_X \colon X \to X \times X\) and \(\discard_X \colon X \to 1\):
