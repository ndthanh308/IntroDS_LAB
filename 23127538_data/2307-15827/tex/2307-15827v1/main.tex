...
For any non-degenerate,  finitely supported probability measure $\mu$ on the lamplighter group $G = L \wr \Z$  with lamps in a non-trivial finite group $L$, $h_\alpha(\mu)>0$ for all $\alpha\in (0,1)$.
\end{maintheorem}
When the $\mu$-random walk has positive drift in the second coordinate (the location of the lamplighter) it is known that $h_1(\mu)>0$ and hence, by the monotonicity of R\'enyi entropies, the claim follows. For symmetric $\mu$,  the proof of Theorem~\ref{thm:main-ll-positivity} involves ``tilting'' $\mu$ along the second coordinate, resulting in a random walk that has positive drift, and then relating its R\'enyi entropies to those of the original, untilted walk.
...
Towards proving that $h_\alpha(\mu)$ is strictly decreasing on $[0,1]$ whenever it is larger than $h_1(\mu)$ we shall prove another convexity property about R\'enyi entropies in the range $\alpha \in (0,1)$. A similar version for $\alpha\in (1,\infty)$ has been observed in \cite{BuryakMishura2021convex}.
Let $\nu $ be a finitely supported measure on a set $\Omega$. For $p<0$, we denote $\ell^p$ norms by $\left\Vert f\right\Vert
...
Now let $f \colon G \to \R$ be given by $f(g) = \mu^{(n)}(g)$ and set $\nu =\mu ^{(n)}$. Then
\[
\log \left\Vert f\right\Vert _{\frac{1}{p}}=p\log \sum_{g}\mu ^{(n)}(g)^{1+\frac{1}{p}}.
\]%
Thus, 
\[
H_{1+\frac{1}{\beta }}(\mu ^{(n)})=-\beta \log \sum_{g}\mu ^{(n)}(g)^{1+\frac{%
...
For the second term, we note that $\pi ^{\ast }\mu $ is a symmetric random
walk on $\mathbb{Z}$. By the Hoeffding's bound, there exists a constant $c>0$
only depending on $\pi ^{\ast }\mu $ such that 
\begin{equation*}
\sum_{\pi (g)=k}\mu^{(n)}(g)\leq \sum_{\pi (g)\geq k}\mu^{(n)}(g)\leq \exp (-c%
\frac{k^{2}}{n})  
\end{equation*}
...
&\sim \frac{1}{cA-2\alpha t}\exp (An(-cA+2\alpha t))
\end{align*}
