...
\label{sec:bip-39}
BIP-39 \cite{bip39} presents a standardized approach for human-readable mnemonic recovery phrases based on cryptographic entropy, checksums to enforce validity, and carefully constructed word lists. Password salting and iteration with PBKDF2 increase brute force resistance compared to raw entropy. Risks from poor user practices remain, as mnemonic extension attacks are still possible. It successfully balances memorability, transcribability, and randomness to achieve user-friendly, secure seed backup phrases.
...
\subsubsection{\ac{mpc} Wallets}
\label{sec:mpc-wallets}
\ac{mpc} wallets \cite{canetti2020uc} are a cutting-edge approach to enhance the security of cryptocurrency wallets, particularly for private key management. \ac{mpc} leverages cryptographic techniques to allow multiple parties to collaboratively compute a function or operation without revealing their individual inputs. In the case of cryptocurrency wallets, \ac{mpc} can be used to secure the private keys in a distributed manner, providing improved protection against single points of failure. 
