...
\usepackage{babel}
\begin{document}
\title{Correlation function of fundamental fermion in holographic QCD}
\maketitle
\begin{center}
...
\begin{abstract}
By using the gauge-gravity duality, we study the correlation function
of fundamental fermion in the $\mathrm{D}_{p}/\mathrm{D}_{p+4}$ model
as top-down approach of holographic QCD for $p=4,3$. The bulk spinor,
as the source of the chiral quark in QCD, is identified to the worldvolume
fermion on the flavor $\mathrm{D}_{p+4}$-branes and the standard
form of its action can be therefore obtained by the T-duality rules
in string theory. Keeping this in hand, we afterwards generalize the
prescription for two-point correlation function in AdS/CFT dictionary
into general D-brane backgrounds and apply it to the case of $p=4,3$
i.e. the D4/D8 and D3/D7 approach respectively. Resultantly, our numerical
calculation with the bubble background always displays discrete peaks
in the correlation functions which imply the bound states as the confinement
...
obtained by the hard thermal loop approximation in the hot medium.
In this sense, we conclude remarkably that our top-down approach in
this work could reveal the fundamental properties of QCD both in the
confined and deconfined phase.
\end{abstract}
\newpage{}
...
model \cite{key-20,key-21,key-22}) and D3/D7 \cite{key-14} model
...
fundamental fermion in $\mathrm{D}_{p}/\mathrm{D}_{p+4}$ model (for
...
as colors and $N_{f}$ probe $\mathrm{D}_{p+4}$-brane as flavors
...
type II supergravity. Accordingly, we identify the bulk spinor, as
the source of the fundamental fermion, to the worldvolume fermionic
field on the probe $\mathrm{D}_{p+4}$-branes, thus its action can
be obtained by the T-duality rules in string theory \cite{key-34,key-35}
which reveals the bulk fermion is not minimal coupled. Keeping this
in hand, we further generalize the prescription for two-point correlation
function in AdS/CFT dictionary into general D-brane backgrounds with
respect to the fundamental fermion in the dual theory. Afterwards,
we apply our method to the D4/D8 and D3/D7 models as tests in order
to evaluate the two-point retarded Green functions variously. 
...
Green functions in our top-down approaches is very different from
it in the bottom-up approaches or it with minimal coupled fermion
\cite{key-30,key-31,key-32,key-33}, while the deconfined Green functions,
obtained from the black brane background, behave similarly. Thus it
illustrates seemingly that the property of QCD confinement is less
clear in the previous works with bottom-up models or with minimal
coupled fermion. Finally, we discuss how to interpret the fermionic
bound states in confined phase as baryon states with the framework
of $\mathrm{D}_{p}/\mathrm{D}_{p+4}$ model which may support that
the open strings on the D-brane behaves somehow as baryons \cite{key-37}.
...
in order to evaluate the retarded Green functions for fundamental
fermions then analyze the numerical results. Summary and discussion
...
by the $\mathrm{D}_{p}$-branes. We note that in the near-horizon
limit, the harmonic function $H_{p}\left(r\right)$ becomes $H_{p}\left(r\right)\rightarrow r_{p}^{7-p}/r^{7-p}$
.
...
f\left(r\right) & =1-\frac{r_{KK}^{7-p}}{r^{7-p}},H_{p}\left(r\right)=1+\frac{r_{KK}^{7-p}}{r^{7-p}},\ \ \mu,\nu=0,1...p-1.\label{eq:4}
\end{align}
...
As QCD also includes fundamental fermions as flavors, the existing
$N_{f}$ coincident $\mathrm{D}_{p+4}$-branes in this holographic
model can just play the role of flavor. The D-brane configuration
of the $\mathrm{D}_{p}/\mathrm{D}_{p+4}$ system is given in Table
\ref{tab:1} for the D3/D7 and D4/D8 approach in this manuscript.
\begin{table}
\begin{centering}
...
to be the holographic QCD.

\section{The holographic correspondence with fundamental fermion}

As our goal is to work out the two-point correlation function of fundamental
fermions through gauge-gravity duality, in this section, let us attempt
to found the associated holographic correspondence in the top-down
approach first, then discuss how to generalize the prescription in
AdS/CFT dictionary into a non-conformal background to compute the
Green function.
...
to the fundamental fermion (quark) in the dual field theory. Then
the two-point retarded Green function $G_{R}$ for the spinor $\chi$
can be evaluated by following \cite{key-10,key-13,key-14} as,
...
$\chi$ in (\ref{eq:6}) (\ref{eq:7}) is identified to the fundamental
...
remains to be supersymmetric \cite{key-42} since there is not any
mechanism to break down the supersymmetry on the $\mathrm{D}_{p+4}$-branes
in principle while the $\mathrm{D}_{p}$-brane is not supersymmetric
due to the boundary condition on its compactified direction. Therefore
the fermionic field $\psi$ in the worldvolume of $\mathrm{D}_{p+4}$-branes
must exist in the low-energy theory, and in our $\mathrm{D}_{p}/\mathrm{D}_{p+4}$
model, it is seemingly to be the suitable candidate uniquely as the
source of the fundamental fermion $\chi$ living in the boundary which
may also be the only consistent interpretation in this system. However
in the black brane solution (\ref{eq:2}), the worldvolume fermion
on the $\mathrm{D}_{p+4}$-branes may acquire thermal mass due to
the contribution from loop diagrams. This means the supersymmetry
may break down so that the supersymmetric fermion could decouple to
the low-energy theory. Nonetheless, it would be interesting to investigate
the fermionic correlation in the black brane background (as the deconfined
phase of QCD) as a comparison to several bottom-up approaches with
deconfined fermions \cite{key-30,key-31,key-32,key-33}. Therefore
we will identify the bulk field $\psi$ to the supersymmetric fermion
on the $\mathrm{D}_{p+4}$-brane in order to continue our discussion. 

\subsection{Action for the bulk spinor}

In the last section, since the bulk field dual to the fundamental
fermion is identified to the worldvolume fermion on the $N_{f}$ coincident
...
\begin{enumerate}
\item Simplify the T-dualitized action (\ref{eq:11}) with respect to the
background fields given in (\ref{eq:2}) (\ref{eq:4})\footnote{When we recall the background (\ref{eq:2}) (\ref{eq:4}), it means
we will use their near-horizon version for holography.} in order to obtain an effective $D+1$ dimensional action involving
the bulk spinor $\psi\left(x,r\right)$, for a $D$ dimensional dual
theory, as 
\begin{align}
S_{D+1} & \propto i\int d^{D}xdr\bar{\psi}\gamma^{r}\partial_{r}\psi+...\nonumber \\
 & =i\int d^{D}x\left(\bar{\psi}\gamma^{r}\psi\right)\big|_{r=r_{KK,H}}^{r=\infty}-i\int d^{4}xdr\partial_{r}\bar{\psi}\gamma^{r}\psi+...\label{eq:16}
\end{align}
where we define
\begin{equation}
\Pi=-i\bar{\psi}\gamma^{r}.
\end{equation}
\item Use the equation of motion for $\psi\left(x,r\right)$ and the Fourier
mode as the ansatz for the solution $\psi\left(x,r\right)$ as,
\begin{equation}
\psi\left(x,r\right)=e^{ik\cdot x}\beta\left(\omega,\vec{k},r\right),k_{\mu}=\left(-\omega,\vec{k}\right),
\end{equation}
to derive the equation for $\xi$ which is given by 
\begin{equation}
\bar{\Pi}=\xi\psi.\label{eq:19}
\end{equation}
\item Solve the the equation for $\xi$ with incoming wave boundary condition
and the two-point retarded Green function $G_{R}$ is given by extracting
the finite part of $\xi$.
\end{enumerate}
The above steps are based on the prescription in AdS/CFT dictionary
for two-point correlation function \cite{key-10,key-14}. We notice
that in the AdS/CFT correspondence, the onshell action (\ref{eq:16})
...
given in (\ref{eq:19}). Therefore in the following sections, we will
test our prescription in the top-down D4/D8 and D3/D7 approaches in
holography.
...
to calculate the two-point Green function and analyze the results
numerically in the 1+3 dimensional QCD.
...
limit. In the language of QCD, the parameters presented in (\ref{eq:20})
can be expressed as,
...
e^{\phi} & =\left(\frac{r}{R}\right)^{3/4},\ F_{4}=dC_{3}=3R^{3}g_{s}^{-1}\epsilon_{4},\ f_{T}\left(r\right)=1-\frac{r_{H}^{3}}{r^{3}},i,j=1,2,3.\label{eq:22}
\end{align}
...
S_{f}^{\mathrm{D8}} & =\frac{iT_{8}}{2}\int d^{9}xe^{-\phi}\sqrt{-g}\bar{\Psi}P_{-}\big[\Gamma^{\alpha}\nabla_{\alpha}+\frac{1}{8\cdot4!}e^{\phi}F_{KLNP}\left(\Gamma^{\alpha}\Gamma^{KLNP}\Gamma_{\alpha}-\Gamma^{KLNP}\right)\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ \ -\frac{1}{2}\Gamma^{M}\partial_{M}\phi\big]\Psi.\label{eq:24}
\end{align}
...
for the dilaton $\phi$ and the R-R form $F_{4}$ presented in (\ref{eq:20})
or (\ref{eq:22}), the action (\ref{eq:24}), after some algebraic
...
S_{f,c}^{\mathrm{D8}}= & \frac{i\mathcal{T}_{c}}{\left(2\pi\alpha^{\prime}\right)^{2}\Omega_{4}}\int d^{4}xdZd\Omega_{4}\bar{\Psi}P_{-}\big(\frac{2}{3}M_{KK}K^{\frac{7}{12}}\gamma^{m}\nabla_{m}^{S^{4}}+K^{\frac{5}{12}}\gamma^{\mu}\partial_{\mu}+M_{KK}K^{\frac{13}{12}}\gamma^{Z}\partial_{Z},\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ \ +\frac{13}{12}M_{KK}ZK^{\frac{1}{12}}\gamma^{4}\big)\Psi,\label{eq:25}
\end{align}
with,
...
\end{equation}
In the deconfined background, the action (\ref{eq:24}) becomes,
...
S_{f,d}^{\mathrm{D8}}= & \frac{i\mathcal{T}_{d}}{\left(2\pi\alpha^{\prime}\right)^{2}\Omega_{4}}\int d^{4}xdZd\Omega_{4}\bar{\Psi}P_{-}\bigg[\frac{4}{3}\pi TZK^{1/12}\gamma^{m}\nabla_{m}^{S^{4}}+K^{5/12}\gamma^{0}\partial_{0}+ZK^{-1/12}\gamma^{i}\partial_{i}\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ \ +2\pi TZK^{7/12}\gamma^{Z}\partial_{Z}+\pi T\left(\frac{13}{6}Z^{2}K^{-5/12}+K^{7/12}\right)\gamma^{Z}\bigg]\Psi,\label{eq:27}
\end{align}
where 
...
fermionic action (\ref{eq:25}) (\ref{eq:27}) to a 5d form as most
...
\begin{equation}
\psi\left(x,Z\right)\otimes\varphi\left(S^{4}\right)\otimes\beta,\label{eq:32}
\end{equation}
...
\tilde{\boldsymbol{\gamma}} & =i\boldsymbol{\gamma}^{6}\boldsymbol{\gamma}^{7}\boldsymbol{\gamma}^{8}\boldsymbol{\gamma}^{9},\label{eq:33}
\end{align}
...
\begin{equation}
\sigma_{3}\beta_{\pm}=\beta_{\pm},\sigma_{1}\beta_{\pm}=\beta_{\mp},\sigma_{2}\beta_{\pm}=\pm i\beta_{\mp},\label{eq:34}
\end{equation}
...
\begin{equation}
\boldsymbol{\gamma}^{m}\nabla_{m}^{S^{4}}\varphi^{\pm l,s}=i\Lambda_{l}^{\pm}\varphi^{\pm l,s};\Lambda_{l}^{\pm}=\pm\left(2+l\right),l=0,1...\label{eq:35}
\end{equation}
Here $s,l$ represents the angular quantum numbers of the spherical
harmonic function.

Put (\ref{eq:32}) into (\ref{eq:25}) (\ref{eq:27}) with the decomposition
(\ref{eq:33}) - (\ref{eq:35}) for $\varphi$ and $\beta$, we could
...
S_{f,c}^{\mathrm{D8}}= & \frac{i\mathcal{T}_{c}}{\left(2\pi\alpha^{\prime}\right)^{2}}\int d^{4}xdZ\bar{\psi}\big(-\frac{2}{3}M_{KK}\Lambda_{l}K^{\frac{7}{12}}+K^{\frac{5}{12}}\boldsymbol{\gamma}^{\mu}\partial_{\mu}+M_{KK}K^{\frac{13}{12}}\boldsymbol{\gamma}\partial_{Z}\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ +\frac{13}{12}M_{KK}ZK^{\frac{1}{12}}\boldsymbol{\gamma}\big)\psi,\label{eq:36}
\end{align}
and
...
S_{f,d}^{\mathrm{D8}}= & \frac{i\mathcal{T}_{d}}{\left(2\pi\alpha^{\prime}\right)^{2}}\int d^{4}xdZ\bar{\psi}\bigg[-\frac{4}{3}\pi TZK^{1/12}\Lambda_{l}+K^{5/12}\boldsymbol{\gamma}^{0}\partial_{0}+ZK^{-1/12}\boldsymbol{\gamma}^{i}\partial_{i}\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ \ +2\pi TZK^{7/12}\boldsymbol{\gamma}\partial_{Z}+\pi T\left(\frac{13}{6}Z^{2}K^{-5/12}+K^{7/12}\right)\boldsymbol{\gamma}\bigg]\psi,\label{eq:37}
\end{align}
...
Section 3.3, the action (\ref{eq:36}) can be further simplified by
rescaling $\psi\rightarrow2\pi\alpha^{\prime}K^{-13/24}\psi$ as,
\begin{equation}
S_{f,c}^{\mathrm{D8}}=i\mathcal{T}_{c}\int d^{4}xdZ\bar{\psi}\left(-\frac{2}{3}M_{KK}\Lambda_{l}K^{-1/2}+K^{-2/3}\boldsymbol{\gamma}^{\mu}\partial_{\mu}+M_{KK}\boldsymbol{\gamma}\partial_{Z}\right)\psi,\label{eq:38}
\end{equation}
while the action (\ref{eq:37}) is rescaled by $\psi\rightarrow2\pi\alpha^{\prime}K^{-7/24}Z^{-1/2}\psi$
as,
...
S_{f,d}^{\mathrm{D8}} & =i\mathcal{T}_{d}\int d^{4}xdZ\bar{\psi}\bigg(-\frac{4}{3}\pi TK^{-1/2}\Lambda_{l}+Z^{-1}K^{-1/6}\boldsymbol{\gamma}^{0}\partial_{0}+K^{-2/3}\boldsymbol{\gamma}^{i}\partial_{i}\nonumber \\
 & \ \ \ \ \ \ \ \ \ \ \ \ \ +2\pi T\boldsymbol{\gamma}\partial_{Z}+\pi TZK^{-1}\boldsymbol{\gamma}\bigg)\psi.\label{eq:39}
\end{align}
And we will use these 5d fermionic actions (\ref{eq:38}) (\ref{eq:39})
to evaluate the two-point Green function.

\subsection{Use the prescription}

Let us attempt to use the prescription in Section 3.3 to evaluate
the two-point Green function with confined (\ref{eq:4}) and deconfined
(\ref{eq:2}) geometry by using action (\ref{eq:38}) (\ref{eq:39})
respectively.

\subsubsection*{Confined phase}

First the equation of motion associated to action (\ref{eq:38}) is
derived as,

\begin{equation}
\left(-\frac{2}{3}M_{KK}\Lambda_{l}K^{-1/2}+K^{-2/3}\boldsymbol{\gamma}^{\mu}\partial_{\mu}+M_{KK}\boldsymbol{\gamma}\partial_{Z}\right)\psi=0.\label{eq:40}
\end{equation}
...
\psi_{R}\\
\psi_{L}
\end{array}\right),\ k\cdot x=k_{\mu}x^{\mu}=-\omega t+\vec{k}\cdot\vec{x},\label{eq:41}
\end{equation}
the equation (\ref{eq:40}) becomes

\begin{align}
-\frac{2}{3}M_{KK}\Lambda_{l}K^{-1/2}\psi_{R}-K^{-2/3}\left(\sigma\cdot k\right)\psi_{L}+M_{KK}\partial_{Z}\psi_{R} & =0,\nonumber \\
-\frac{2}{3}M_{KK}\Lambda_{l}K^{-1/2}\psi_{L}-K^{-2/3}\left(\bar{\sigma}\cdot k\right)\psi_{R}-M_{KK}\partial_{Z}\psi_{L} & =0,\label{eq:42}
\end{align}
...
Solving (\ref{eq:42}), we can further obtain the decoupled equations
for $\psi_{R,L}$ as,

\begin{align}
\partial_{Z}^{2}\psi_{L}+\frac{2K^{\prime}}{3K}\partial_{Z}\psi_{L}+\left(\frac{\lambda K^{\prime}}{9K^{3/2}}-\frac{4}{9}\frac{\Lambda_{l}^{2}}{K}-\frac{k^{2}}{M_{KK}^{2}}\frac{1}{K^{4/3}}\right)\psi_{L} & =0,\nonumber \\
\partial_{Z}^{2}\psi_{R}+\frac{2K^{\prime}}{3K}\partial_{Z}\psi_{R}-\left(\frac{\lambda K^{\prime}}{9K^{3/2}}+\frac{4}{9}\frac{\Lambda_{l}^{2}}{K}+\frac{k^{2}}{M_{KK}^{2}}\frac{1}{K^{4/3}}\right)\psi_{R} & =0,\label{eq:44}
\end{align}
...
\begin{align}
\psi_{R} & =A\left(k,\Lambda_{l}\right)Z^{\frac{2}{3}\Lambda_{l}}+B\left(k,\Lambda_{l}\right)Z^{-\frac{1}{3}-\frac{2}{3}\Lambda_{l}},\nonumber \\
\psi_{L} & =C\left(k,\Lambda_{l}\right)Z^{-\frac{1}{3}+\frac{2}{3}\Lambda_{l}}+D\left(k,\Lambda_{l}\right)Z^{-\frac{2}{3}\Lambda_{l}},\label{eq:45}
\end{align}
where $A,B,C,D$ are constant spinors and furthermore one can find
the following relations as,

\begin{equation}
C\left(k,\Lambda_{l}\right)=\frac{3}{\left(1-4\lambda\right)M_{KK}}\left(\bar{\sigma}\cdot k\right)A\left(k,\Lambda_{l}\right),\ B\left(k,\Lambda_{l}\right)=\frac{3}{\left(1+4\lambda\right)M_{KK}}\left(\sigma\cdot k\right)D\left(k,\Lambda_{l}\right),
\end{equation}
once the analytical solution (\ref{eq:45}) is plugged back into (\ref{eq:42}).
Besides, we notice that the solution at $Z\rightarrow-\infty$ is
equivalent to interchange $\psi_{R,L}$ in the solution (\ref{eq:45}).
$\Lambda_{l}$ is the eigenvalue of the $S^{4}$ part and it satisfies
$\Lambda_{l}\geq2$ according to (\ref{eq:35}). So we obtain the
...
part in (\ref{eq:45}) as \cite{key-13,key-14},

\begin{equation}
\psi_{0}=\lim_{Z\rightarrow+\infty}Z^{-\frac{2}{3}\Lambda_{l}}\psi=A\left(k,\Lambda_{l}\right).
\end{equation}
Next, the conjugate momentum $\Pi_{0}$ associated to $\psi_{0}$
can be evaluated by using (\ref{eq:9}). To this goal, let us take
a look at the action (\ref{eq:38}), 

\begin{align}
S_{f,c}^{\mathrm{D8}}= & \ i\mathcal{T}_{c}\int d^{4}xdZ\bar{\psi}\left(-\frac{2}{3}M_{KK}\lambda K^{-1/2}+K^{-2/3}\boldsymbol{\gamma}^{\mu}\partial_{\mu}+M_{KK}\boldsymbol{\gamma}\partial_{Z}\right)\psi,\nonumber \\
= & \ i\mathcal{T}_{c}\int d^{4}x\left(\bar{\psi}M_{KK}\boldsymbol{\gamma}\psi\right)|_{-\infty}^{+\infty}\nonumber \\
 & -i\mathcal{T}_{c}\int d^{4}xdZ\left(\partial_{Z}\bar{\psi}M_{KK}\boldsymbol{\gamma}+\frac{2}{3}M_{KK}\lambda K^{-1/2}\bar{\psi}-K^{-2/3}\partial_{\mu}\bar{\psi}\boldsymbol{\gamma}^{\mu}\right)\psi,\nonumber \\
\equiv & \int d^{4}x\left(\Pi\psi\right)|_{-\infty}^{+\infty}=\int d^{4}x\left(\Pi_{R}\psi_{R}+\Pi_{L}\psi_{L}\right)|_{-\infty}^{+\infty},\label{eq:49}
\end{align}
where
...
\end{equation}
Notice that the term in the third line of (\ref{eq:49}) vanishes
since it is nothing but the conjugate equation to (\ref{eq:40}).
So once we impose the solution (\ref{eq:45}) to (\ref{eq:49}), it
implies the action (\ref{eq:49}) includes divergence at boundary
$Z\rightarrow\infty$ as\footnote{Since the solution at $Z\rightarrow-\infty$ can be obtained by interchanging
the roles of $\psi_{R,L}$ in (\ref{eq:45}), the action (\ref{eq:49})
has same asymptotic behavior at $Z\rightarrow-\infty$ on the anti
D8-branes. },

\begin{align}
S_{f,c}^{\mathrm{D8}} & \supseteq-\mathcal{T}_{c}M_{KK}\int d^{4}x\left[C\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)Z^{-\frac{1}{3}+\frac{4}{3}\Lambda_{l}}+D\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)+h.c\right]|_{Z\rightarrow+\infty}+...\label{eq:51}
\end{align}
Therefore we can see, it requests for a holographic boundary counterterm
$S_{ct}$ as,

\begin{equation}
S_{ct}=\mathcal{T}_{c}M_{KK}\int d^{4}x\left[C\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)Z^{-\frac{1}{3}+\frac{4}{3}\Lambda_{l}}+h.c\right]|_{Z\rightarrow+\infty},
\end{equation}
to remove the divergence in (\ref{eq:49}). We note that action (\ref{eq:51})
...
\begin{equation}
\Pi_{0}=-\frac{\delta S^{ren}}{\delta\psi_{0}}=\mathcal{T}_{c}M_{KK}D\left(k,\Lambda_{l}\right)^{\dagger},\ S^{ren}=S_{f,c}^{\mathrm{D8}}+S_{ct},
\end{equation}
which implies the Green function $G_{R}\left(k,\Lambda_{l}\right)$
is obtained by

\begin{equation}
\mathcal{T}_{c}M_{KK}D\left(k,\Lambda_{l}\right)=G_{R}\left(k,\Lambda_{l}\right)A\left(k,\Lambda_{l}\right),
\end{equation}
...
\begin{equation}
\xi_{1}=\frac{\psi_{L}^{\left(1\right)}}{\psi_{R}^{\left(1\right)}},\xi_{2}=\frac{\psi_{L}^{\left(2\right)}}{\psi_{R}^{\left(2\right)}},\label{eq:56}
\end{equation}
the Green function can therefore been written as,

\begin{equation}
G_{R}^{\left(1,2\right)}\left(k,\Lambda_{l}\right)=\mathcal{T}_{c}M_{KK}\lim_{Z\rightarrow\infty}Z^{\frac{4}{3}\Lambda_{l}}\xi_{1,2}\left(Z\right).\label{eq:57}
\end{equation}
satisfying the equation of motion,

\begin{equation}
\xi_{1,2}^{\prime}=-\frac{4}{3}\Lambda_{l}K^{-1/2}\xi_{1,2}-\frac{K^{-2/3}}{M_{KK}}\left(-\omega+h\cdot\mathrm{k}\right)-\frac{K^{-2/3}}{M_{KK}}\left(-\omega-h\cdot\mathrm{k}\right)\xi_{1,2}^{2},\label{eq:58}
\end{equation}
according to (\ref{eq:42}). Here we have set $k_{\mu}=\left(-\omega,\mathrm{k},0,0\right)$
with,

\begin{equation}
\frac{\psi_{R}^{\left(2\right)}}{\psi_{R}^{\left(1\right)}}=\frac{\psi_{L}^{\left(2\right)}}{\psi_{L}^{\left(1\right)}}=h.\label{eq:59}
\end{equation}
...
\begin{equation}
\psi_{R}\left(0\right)=\pm\psi_{L}\left(0\right).\label{eq:61}
\end{equation}
Therefore we can set $h=\pm1$ in (\ref{eq:59}) for $\xi_{1,2}$
respectively. Altogether, (\ref{eq:58}) could be numerically solved
...
\begin{equation}
\left(-\frac{4}{3}\pi TK^{-1/2}\Lambda_{l}+Z^{-1}K^{-1/6}\boldsymbol{\gamma}^{0}\partial_{0}+K^{-2/3}\boldsymbol{\gamma}^{i}\partial_{i}+2\pi T\boldsymbol{\gamma}\partial_{Z}+\pi TZK^{-1}\boldsymbol{\gamma}\right)\psi=0.\label{eq:62}
\end{equation}
Using the ansatz
...
\end{array}\right),
\end{equation}
with $k_{\mu}=\left(-\omega,\mathrm{k},0,0\right)$, (\ref{eq:62})
reduces to
...
\partial_{Z}\psi_{L}^{\left(1,2\right)}+\left(\frac{ZK^{-1}}{2}+\frac{2}{3}K^{-1/2}\Lambda_{l}\right)\psi_{L}^{\left(1,2\right)}+\left(\frac{h\cdot\mathrm{k}}{2\pi T}K^{-2/3}-\frac{\omega}{2\pi T}Z^{-1}K^{-1/6}\right)\psi_{R}^{\left(1,2\right)} & =0,\label{eq:64}
\end{align}
which can be solved analytically at $Z\rightarrow\infty$ as,

\begin{align}
\psi_{R}^{\left(1,2\right)} & =A\left(k,\Lambda_{l}\right)Z^{\frac{2}{3}\Lambda_{l}-\frac{1}{2}}+B\left(k,\Lambda_{l}\right)Z^{-\frac{2}{3}\Lambda_{l}-\frac{5}{6}},\nonumber \\
\psi_{L}^{\left(1,2\right)} & =C\left(k,\Lambda_{l}\right)Z^{-\frac{2}{3}\Lambda_{l}-\frac{1}{2}}+D\left(k,\Lambda_{l}\right)Z^{\frac{2}{3}\Lambda_{l}-\frac{5}{6}}.\label{eq:65}
\end{align}
The asymptotics (\ref{eq:65}) leads to that the onshell action (\ref{eq:39})
...
S_{f,d}^{\mathrm{D8}} & \supseteq2\pi T\int d^{4}xdZi\bar{\psi}\boldsymbol{\gamma}\partial_{Z}\psi=-2\pi T\int d^{4}x\psi_{L}^{\dagger}\psi_{R}+...\nonumber \\
 & =-2\pi T\int d^{4}xA\left(k,\Lambda_{l}\right)D\left(k,\Lambda_{l}\right)Z^{\frac{4}{3}\Lambda_{l}-\frac{4}{3}},
\end{align}
...
us use the ratio given in (\ref{eq:56}) so that the Green function
can be written as,

\begin{equation}
G_{R}^{\left(1,2\right)}\left(k,\Lambda_{l}\right)=\mathcal{T}_{d}\left(2\pi T\right)\lim_{Z\rightarrow+\infty}Z^{1/3}\xi_{1,2}\left(Z\right).
\end{equation}
And the equation for $\xi_{1,2}$ can be obtained from (\ref{eq:64})
as,

\begin{equation}
\xi_{1,2}^{\prime}=-\frac{4}{3}K^{-1/2}\Lambda_{l}\xi_{1,2}-\frac{h\cdot\mathrm{k}}{2\pi T}K^{-2/3}+\frac{\omega}{2\pi T}Z^{-1}K^{-1/6}+\left(\frac{\omega}{2\pi T}Z^{-1}K^{-1/6}+\frac{h\cdot\mathrm{k}}{2\pi T}K^{-2/3}\right)\xi_{1,2}^{2},\label{eq:70}
\end{equation}
...
by solving (\ref{eq:58}) (\ref{eq:70}) with incoming wave boundary
conditions.
...
\begin{equation}
G_{R}\left(k\right)=\frac{1}{ik_{\mu}\boldsymbol{\gamma}^{\mu}-M_{n}},\ M_{n}=m_{n}-\Sigma\left(k\right),\label{eq:71}
\end{equation}
...
energy of the various bound states consisted of the fundamental quarks.
And we believe this result is consistent with the property of the
bubble background (\ref{eq:20}) since its dual theory is expected
to be QCD with confinement states. Moreover, for $\omega>0$, the
location of the peak in the Green function can be read approximately
once we set $\mathrm{k}=0$, so that the peaks correspond to $\omega=m_{n}-\Sigma\left(k\right)$
as it is illustrated in Figure \ref{fig:5}. In this sense, we find
quantitatively, for $\Lambda_{l}=2,l=0$, the bound energy (the position
of the peak) is located at $\omega\simeq1.6,2.7,3.7...$ in $G_{R}^{\left(1\right)}$
...
Next, let us take a look at the Green function obtained in the deconfined
phase which is illustrated in Figure \ref{fig:7}. 
% Figure environment removed
 Since the relation of $G_{R}^{\left(1,2\right)}$ is given as $G_{R}^{\left(2\right)}\left(\omega,\mathrm{k},\Lambda_{l}\right)=-G_{R}^{\left(1\right)}\left(\omega,-\mathrm{k},\Lambda_{l}\right)$,
let us focus on $G_{R}^{\left(2\right)}\left(\omega,\mathrm{k},\Lambda_{l}\right)$
for convenience. First, we can see the particle onshell is very different
from the confined case which satisfies $\omega\simeq\mathrm{k}$ for
$\mathrm{k}\gg1$ and $\omega\simeq\mathrm{k}^{2}+...$ for $\mathrm{k}\ll1$.
This behavior is in qualitative agreement with the dispersion curves
obtained by the hard thermal loop (HTL) approximation in hot QCD,
up to one-loop calculation, as \cite{key-44},

\begin{align}
\omega\left(\mathrm{k}\right) & \simeq m_{f}+\frac{1}{3}\mathrm{k}+\frac{1}{3m_{f}}\mathrm{k}^{2},\ \mathrm{k}\ll1;\nonumber \\
\omega\left(\mathrm{k}\right) & \simeq\mathrm{k},\ \mathrm{k}\gg1,\label{eq:72}
\end{align}
...
\end{equation}
We note that chemical potential can also contribute to $m_{f}$ while
it is not turned on in the current holographic setup and $C_{F}$
is suggested to be $C_{F}=4/3$ for fundamental quarks. According
to our numerical calculations presented in Figure \ref{fig:7}, the
effective mass of quarks are evaluated to be $m_{f}=\omega\left(\mathrm{k}=0\right)\simeq2.9\times\left(2\pi T\right)$
...
is only valid in strong coupling region while it covers the result
from the perturbation method in HTL. The only consistent interpretation
is seemingly that the high order contribution by HTL perturbation
is suppressed in the large $N_{c}$ limit. Second, since the peaks
have a width, our numerical calculation may also include the dispersion
curves of plasmino which is expected to be $\omega\left(\mathrm{k}\right)\simeq m_{f}-\frac{1}{3}\mathrm{k}+\frac{1}{3m_{f}}\mathrm{k}^{2}$
for $\mathrm{k}\ll1$. However, it is out of reach to distinguish
exactly the dispersion curves of fundamental quarks from plasmino
due to the inevitable random error in our numerical calculation while
this may be indeed interesting. Overall, we believe our current results,
based on the holography with the principle in string theory, reveal
mostly the fundamental properties of both confinement and deconfinement
in QCD, while they are very different from some bottom-up approaches
or phenomenological approaches in this model as \cite{key-30,key-31}
where the features of QCD confinement is less clear. 
...
version of 1+2 dimensional QCD.
...
f\left(r\right) & =1-\frac{r_{KK}^{4}}{r^{4}},\!F_{5}=dC_{4}=4R^{4}g_{s}^{-1}\epsilon_{5},\ \alpha,\beta=0,1,2,\label{eq:74}
\end{align}
...
f_{T}\left(r\right) & =1-\frac{r_{H}^{4}}{r^{4}},\!F_{5}=dC_{4}=4R^{4}g_{s}^{-1}\epsilon_{5},\label{eq:75}
\end{align}
...
vanishes for $p=3$ due to (\ref{eq:2}) and since $x^{3}$ is conpactified
...
Then let us introduce the probe D7-branes as flavor embedded into
the background (\ref{eq:74}) (\ref{eq:75}) in order to investigate
...
\end{equation}
to the radial and $S^{5}$ part in (\ref{eq:74}) (\ref{eq:75}),
...
ds_{\mathrm{D7},d}^{2} & =\frac{r^{2}}{R^{2}}\left[-f_{T}\left(r\right)dt^{2}+\left(dx^{1}\right)^{2}+\left(dx^{2}\right)^{2}\right]+\frac{R^{2}}{\zeta^{2}}\left(d\rho^{2}+\rho^{2}d\Omega_{4}^{2}\right).\label{eq:78}
\end{align}
...
\begin{equation}
S_{f}^{\mathrm{D7}}=\frac{iT_{7}}{2}\int d^{8}x\sqrt{-g}\bar{\Psi}P_{-}\left(\Gamma^{\alpha}\nabla_{\alpha}-\frac{1}{2\cdot8\cdot5!}F_{KLMNP}\Gamma^{\alpha}\Gamma^{KLMNP}\Gamma_{\alpha}\right)\Psi,\label{eq:79}
\end{equation}
where we have imposed $\bar{\gamma}\bar{\Psi}=\bar{\Psi}$ and $F_{KLMNP}$
refers to the component of the R-R 5-form presented in (\ref{eq:74})
(\ref{eq:75}). After some straightforward calculations, the action
(\ref{eq:79}) can be written as,
...
S_{f,c}^{\mathrm{D7}}= & \frac{i}{2}T_{7}R^{2}\int d^{3}xd\rho d\Omega_{4}\bar{\Psi}P_{-}\bigg(\frac{\rho^{4}r^{2}R}{\zeta^{5}}\gamma^{\alpha}\partial_{\alpha}+\frac{3\rho^{5}r^{3}}{2\zeta^{6}}\sqrt{f}\gamma^{\rho}+\frac{\rho^{4}r^{3}}{\zeta^{4}R}\gamma^{\rho}\partial_{\rho}\nonumber \\
 & +\frac{\rho^{3}r^{3}}{\zeta^{4}R}\gamma^{m}D_{m}+\frac{\rho^{4}r^{3}}{\zeta^{5}R}\bigg)\Psi,\label{eq:80}\\
S_{f,d}^{\mathrm{D7}}= & \frac{i}{2}T_{7}R^{2}\int d^{3}xd\rho d\Omega_{4}\bar{\Psi}P_{-}\bigg[\frac{\rho^{4}r^{2}R}{\zeta^{5}}\gamma^{0}\partial_{0}+\frac{\rho^{4}\sqrt{f_{T}}r^{3}}{2\zeta^{5}R}\left(\frac{\zeta f^{\prime}}{2f}+\frac{3\rho\sqrt{f_{T}}}{\zeta}\right)\gamma^{4}\nonumber \\
 & +\frac{\rho^{4}\sqrt{f_{T}}r^{2}R}{\zeta^{5}}\gamma^{\alpha}\partial_{\alpha}+\frac{\rho^{4}\sqrt{f_{T}}r^{3}}{\zeta^{4}R}\gamma^{\rho}\partial_{\rho}+\frac{\rho^{3}\sqrt{f_{T}}r^{3}}{\zeta^{4}R}\gamma^{m}D_{m}+\frac{\rho^{4}\sqrt{f_{T}}r^{3}}{\zeta^{5}R}\bigg]\Psi,\label{eq:81}
\end{align}
...
\end{equation}
with respect to the bubble (\ref{eq:74}) and black brane (\ref{eq:75})
background. Follow the decomposition presented in (\ref{eq:32}) -
(\ref{eq:34}) with the Eigen equation on $S^{4}$ (\ref{eq:35})
and integrate the $S^{4}$ part, the action (\ref{eq:80}) can be
further written as a 5d form as,

\begin{align}
S_{f,c}^{\mathrm{D7}}= & i\mathcal{T}\int d^{3}xd\rho\bar{\psi}\left[\frac{1}{\zeta}-\frac{1}{\rho}\Lambda_{l}+\frac{R^{2}}{r\zeta}\boldsymbol{\gamma}^{\alpha}\partial_{\alpha}+\left(\frac{2\rho}{\zeta^{2}}-\frac{2}{\rho}\right)\boldsymbol{\gamma}+\boldsymbol{\gamma}\partial_{\rho}\right]\psi,\label{eq:83}
\end{align}
by rescaling $\psi\rightarrow\left(2\pi\alpha^{\prime}\right)\psi\frac{\zeta^{2}}{\rho^{2}}\frac{1}{r^{3/2}}$
and action (\ref{eq:81}) can be written as,
...
S_{f,d}^{\mathrm{D7}}= & i\mathcal{T}\int d^{3}xd\rho\bar{\psi}\bigg[\frac{1}{\sqrt{f_{T}}}\frac{R}{r\zeta}\boldsymbol{\gamma}^{0}\partial_{0}+\frac{R}{r\zeta}\boldsymbol{\gamma}^{\alpha}\partial_{\alpha}+\frac{1}{R}\boldsymbol{\gamma}\partial_{\rho}+\left(\frac{3\rho\sqrt{f_{T}}}{2R\zeta^{2}}+\frac{f_{T}^{\prime}}{4f_{T}R}\right)\boldsymbol{\gamma}\nonumber \\
 & +\frac{1}{R}\left(\frac{1}{\zeta}-\frac{f_{T}^{\prime}}{4f_{T}}\right)-\frac{1}{R\rho}\left(\Lambda_{l}+2\right)+\frac{\rho}{R\zeta^{2}}\left(\frac{5}{2}-\frac{3}{2}\sqrt{f_{T}}\right)\bigg]\psi,\label{eq:84}
\end{align}
...
\end{equation}
We will use these 4d fermionic actions (\ref{eq:83}) (\ref{eq:84})
...
$\psi$ can be obtained by varying (\ref{eq:83}) as, 
...
\end{equation}
Using the ansatz given in (\ref{eq:41}), we can obtain the coupled
equations as,
...
i\frac{R^{2}}{r\xi}\bar{\sigma}^{\alpha}\partial_{\alpha}\psi_{R}+\left(\frac{1}{\xi}-\frac{1}{\rho}\Lambda_{l}-\frac{2\rho}{\xi^{2}}+\frac{2}{\rho}-\partial_{\rho}\right)\psi_{L} & =0.\label{eq:87}
\end{align}
Setting $k_{\mu}=\left(-\omega,\mathrm{k},0,0\right)$, the equations
presented in (\ref{eq:87}) can be written as decoupled second order
...
\begin{align}
\psi_{R} & =A\left(k,\Lambda_{l}\right)\rho^{1+\Lambda_{l}}+B\left(k,\Lambda_{l}\right)\rho^{-2-\Lambda_{l}},\nonumber \\
\psi_{L} & =C\left(k,\Lambda_{l}\right)\rho^{\Lambda_{l}}+D\left(k,\Lambda_{l}\right)\rho^{-1-\Lambda_{l}},
\end{align}
...
\end{array}\right).
\end{equation}
Then the onshell action (\ref{eq:83}) will include a finite part
as,

\begin{align}
S_{f,c}^{\mathrm{D7}} & \supseteq-\mathcal{T}\int d^{3}x\left[\left(\psi_{L}^{\dagger}\psi_{R}\right)|_{\rho\rightarrow\infty}+h.c+...\right]\nonumber \\
 & =-\mathcal{T}\int d^{3}x\left[C\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)\rho^{2\Lambda_{l}+1}+D\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)+h.c\right]|_{\rho\rightarrow\infty},\label{eq:91}
\end{align}
thus it request for a holographic counterterm as,

\begin{equation}
S_{ct}=\mathcal{T}\int d^{3}x\left[C\left(k,\Lambda_{l}\right)^{\dagger}A\left(k,\Lambda_{l}\right)\rho^{2\Lambda_{l}+1}+h.c\right]|_{\rho\rightarrow\infty}.
\end{equation}
The existing finite part in (\ref{eq:91}) is due to the residual
isometry of $\mathrm{AdS}_{5}$ since the D3-brane solution (\ref{eq:74})
(\ref{eq:75}) as bulk geometry is $\mathrm{AdS}_{5}\times S^{5}$
asymptotically. Follow the discussion in Section 3, 4, the retarded
Green function is given by,

\begin{equation}
\mathcal{T}D\left(k,\Lambda_{l}\right)=G_{R}\left(k,\Lambda_{l}\right)A\left(k,\Lambda_{l}\right).
\end{equation}
Using the representation (\ref{eq:55}) and introducing the ratio
(\ref{eq:56}), the Green function can be written as,

\begin{equation}
G_{R}^{\left(1,2\right)}=\lim_{\rho\rightarrow\infty}\rho^{2\Lambda_{l}+2}\xi_{1,2},
\end{equation}
where the equation for $\xi_{1,2}$ can be obtained from (\ref{eq:87})
as,

\begin{equation}
\xi_{1,2}^{\prime}=\frac{R^{2}}{r\zeta}\left(\omega-h\cdot\mathrm{k}\right)+2\left(\frac{1}{\zeta}-\frac{1}{\rho}\Lambda_{l}\right)\xi_{1}+\frac{R^{2}}{r\zeta}\left(\omega+h\cdot\mathrm{k}\right)\xi_{1,2}^{2},\label{eq:95}
\end{equation}
...
$\psi$ is given by varying action (\ref{eq:84}) which is,

\begin{align}
\bigg[\frac{1}{\sqrt{f_{T}}}\frac{R}{r\zeta}\boldsymbol{\gamma}^{0}\partial_{0}+\frac{R}{r\zeta}\boldsymbol{\gamma}^{\alpha}\partial_{\alpha}+\frac{1}{R}\boldsymbol{\gamma}\partial_{\rho}+\left(\frac{3\rho\sqrt{f_{T}}}{2R\zeta^{2}}+\frac{f_{T}^{\prime}}{4fR}\right)\boldsymbol{\gamma}\nonumber \\
...
\end{align}
Using the ansatz (\ref{eq:41}), the coupled equations for $\psi_{L,R}$
are,
...
\bigg[-\frac{1}{R}\partial_{\rho}-\frac{3\rho\sqrt{f_{T}}}{2R\zeta^{2}}-\frac{f_{T}^{\prime}}{4f_{T}R}+\frac{1}{R}\left(\frac{1}{\zeta}-\frac{f_{T}^{\prime}}{4f_{T}}\right)-\frac{1}{R\rho}\left(\Lambda_{l}+2\right)\nonumber \\
+\frac{\rho}{R\zeta^{2}}\left(\frac{5}{2}-\frac{3}{2}\sqrt{f_{T}}\right)\bigg]\psi_{L}+i\frac{R}{r\zeta}\left(\frac{1}{\sqrt{f_{T}}}\partial_{0}+\bar{\sigma}^{\alpha}\partial_{\alpha}\right)\psi_{R} & =0,\label{eq:97}
\end{align}
...
\begin{align}
\psi_{R} & =\rho^{\Lambda_{l}-\frac{3}{2}}A\left(k,\Lambda_{l}\right)+\rho^{-\Lambda_{l}-\frac{3}{2}}B\left(k,\Lambda_{l}\right),\nonumber \\
\psi_{L} & =\rho^{-\Lambda_{l}-\frac{3}{2}}C\left(k,\Lambda_{l}\right)+\rho^{-\frac{5}{2}+\Lambda_{l}}D\left(k,\Lambda_{l}\right).
\end{align}
While the boundary value of $\psi$ remains to be $\psi_{R}$, the
onshell action (\ref{eq:84}) taking the form as,

\begin{align}
S_{f,c}^{\mathrm{D7}} & \supseteq-\mathcal{T}\int d^{3}x\left[\left(\psi_{L}^{\dagger}\psi_{R}\right)|_{\rho\rightarrow\infty}+h.c+...\right]\nonumber \\
 & =-\mathcal{T}\int d^{3}x\left[\rho^{2\Lambda_{l}-4}D^{\dagger}\left(k,\Lambda_{l}\right)A\left(k,\Lambda_{l}\right)+h.c\right]|_{\rho\rightarrow\infty}.
\end{align}
...
\begin{equation}
\bar{\Pi}_{0}=G_{R}\left(k,\Lambda_{l}\right)\psi_{0},\ G_{R}^{\left(1,2\right)}=\lim_{\epsilon\rightarrow0}\epsilon^{-1}\xi_{1,2},
\end{equation}
where the equation for $\xi_{1,2}$ defined in (\ref{eq:56}) can
be obtained from (\ref{eq:97}), as

\begin{align}
\xi_{1,2}^{\prime}= & \frac{R^{2}}{r\zeta}\left(\frac{\omega}{\sqrt{f}}-h\cdot\mathrm{k}\right)\xi_{2}+\frac{R^{2}}{r\zeta}\left(\frac{\omega}{\sqrt{f}}+h\cdot\mathrm{k}\right)\xi_{1,2}^{2}\nonumber \\
 & +2\left[\left(\frac{1}{\zeta}-\frac{f^{\prime}}{4f}\right)-\frac{1}{\rho}\left(\Lambda_{l}+2\right)+\frac{\rho}{\zeta^{2}}\left(\frac{5}{2}-\frac{3}{2}\sqrt{f}\right)\right]\xi_{1,2}.\label{eq:104}
\end{align}
...
(\ref{eq:95}) and (\ref{eq:104}) in the D3/D7 approach. The behavior
of two-point Green function in the confined phase is illustrated in
Figure \ref{fig:8} which is basically similar to the approach in
the D4/D8 model. The peaks in the Green function display the discreteness
representing the onshell bound state as confinement in 3d QCD, as
$\omega\simeq1.6,2.6,4.1...$ in $G_{R}^{\left(1\right)}$ and $\omega\simeq1.9,3.4,4.8...$
in $G_{R}^{\left(2\right)}$ which is only quantitatively different
from the results in the D4/D8 model. To specify the position of the
onshell energy, we have plotted out the relation of the Green function
$G_{R}$ as a function of $\omega$ which is illustrated in Figure
\ref{fig:9}. Note that, while we have set $L=0$ for the chirally
symmetric theory, the position of the peaks in the Green function
depends on the quark mass $L$ as it is illustrated in Figure \ref{fig:10}.
The dependence on $L$ in Green function is different from the D4/D8
approach, since there is not a direction perpendicular to both D4-
and D8-branes in the D4/D8 model (which means the VEV of a $\left(4,8\right)$
string is vanished i.e. the bare mass of fundamental quark is vanished.).
...
correction of the self-energy $\Sigma\left(k\right)$ in (\ref{eq:71}).
In the deconfined phase, we plot out the imaginary part of the Green
function as the spectral function in Figure \ref{fig:11} in which
the onshell condition is in qualitative agreement with the dispersion
curves obtained by the hard thermal loop approximation presented in
(\ref{eq:72}). The effective mass of fermion in hot medium is evaluated
as $m_{f}\simeq4.8\left(2\pi T\right)$ by the D3/D7 model which also
implies the high order contribution in HTL approximation might be
...
In this work, we study the correlation function of the fundamental
fermion in the $\mathrm{D}_{p}/\mathrm{D}_{p+4}$ model ($p=4,3$)
as the holographic top-down approach to QCD. Since our concern is
the fermionic correlator, the bulk spinor is identified to the worldvolume
fermion created by the $\left(p+4,p+4\right)$ string on the $\mathrm{D}_{p+4}$-brane.
...
prescription for two-point correlation function in AdS/CFT dictionary
into general D-brane background with respect to the fundamental fermion
...
the QFT approach is only valid in weak coupled field theory. Hence
...
of fermion while it is not included in this work. However, when the
...
approach. Therefore, we believe the Green function with non-vanished
...
Finally, in the confined phase, the interpretation of the bound states
presented in the Green function also need to be considered. According
to AdS/CFT, the bulk and dual theory should share the same Hilbert
space, hence both of them would include such fermionic bound states.
In the side of bulk theory, these bound states should correspond to
the supersymmetrically fermionic meson \cite{key-36} in holography
since they are created by the supersymmetric fermion on the $\mathrm{D}_{p+4}$-brane.
However the fermionic meson is absent in QCD as the dual theory. Nonetheless,
it may be possible to interpret these fermionic bound states as baryon
states if we take into account a baryon vertex in the bulk. For example,
consider a baryon vertex, which is identified as a $\mathrm{D}_{8-p}$-brane
wrapped on $S^{8-p}$ \cite{key-45,key-46,key-47}, located at $r=r_{KK}$
in the bubble background and the $\left(p,p+4\right)$ string is totally
inside the $\mathrm{D}_{p+4}$-brane terminating on the baryon vertex.
In this configuration, the $\left(p,p+4\right)$ string is also a
$\left(p+4,p+4\right)$ string connecting to a baryon vertex, therefore
the associated fermionic bound states are equivalently created by
the baryon vertex which can be identified as baryon states in the
dual theory. This viewpoint may support that the open strings on the
D-brane behaves somehow as baryons in the $\mathrm{D}_{p}/\mathrm{D}_{p+4}$
model as it is discussed in \cite{key-37}.
