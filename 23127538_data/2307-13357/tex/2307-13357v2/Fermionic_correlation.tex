...
hot medium. In this sense, we conclude remarkably that our top-down
approach in this work could reveal the fundamental properties of QCD
both in the confined and deconfined phase.
\end{abstract}
\newpage{}
...
somehow as baryons \cite{key-37}.
...
source of the flavored fermion $\chi$ living in the boundary which
...
on worldvolume of the $\mathrm{D}_{p+4}$-brane in order to continue
our discussion. 
...
\psi_{L} & =CZ^{-\frac{1}{3}+\frac{2}{3}\Lambda_{l}}+DZ^{-\frac{2}{3}\Lambda_{l}},\label{eq:45}
\end{align}
...
\begin{align}
S_{f,c}^{\mathrm{D8}} & \supseteq-\mathcal{T}_{c}M_{KK}\int d^{4}x\left[C^{\dagger}AZ^{-\frac{1}{3}+\frac{4}{3}\Lambda_{l}}+D^{\dagger}A+h.c\right]|_{Z\rightarrow+\infty}+...\label{eq:51}
\end{align}
...
\begin{equation}
G_{R}^{\left(1,2\right)}=\mathcal{T}_{c}M_{KK}\lim_{Z\rightarrow\infty}Z^{\frac{4}{3}\Lambda_{l}}\xi_{1,2}\left(Z\right).\label{eq:57}
\end{equation}
...
\psi_{L}^{\left(1,2\right)} & =CZ^{-\frac{2}{3}\Lambda_{l}-\frac{1}{2}}+DZ^{\frac{2}{3}\Lambda_{l}-\frac{5}{6}}.\label{eq:65}
\end{align}
...
\begin{equation}
G_{R}\left(\omega,\vec{k}\right)=\frac{1}{ik_{\mu}\boldsymbol{\gamma}^{\mu}-M_{n}},\ M_{n}=m_{n}-\Sigma\left(k\right),\label{eq:71}
\end{equation}
...
of the bubble background (\ref{eq:20}) since its dual theory is expected
...
implies the minus sign should be picked up in (\ref{eq:72}) for $\mathrm{k}\ll1$
while there are two branches given by HTL approximation in (\ref{eq:72}).
...
S_{f,c}^{\mathrm{D7}} & \supseteq-\mathcal{T}\int d^{3}x\left[\left(\psi_{L}^{\dagger}\psi_{R}\right)|_{\rho\rightarrow\infty}+h.c+...\right]\nonumber \\
 & =-\mathcal{T}\int d^{3}x\left[C^{\dagger}A\rho^{2\Lambda_{l}+1}+D^{\dagger}A+h.c\right]|_{\rho\rightarrow\infty},\label{eq:91}
\end{align}
...
\xi_{1,2}^{\prime}= & \frac{R^{2}}{r\zeta}\left(\frac{\omega}{\sqrt{f_{T}}}-h\cdot\mathrm{k}\right)\xi_{2}+\frac{R^{2}}{r\zeta}\left(\frac{\omega}{\sqrt{f_{T}}}+h\cdot\mathrm{k}\right)\xi_{1,2}^{2}\nonumber \\
 & +2\left[\left(\frac{1}{\zeta}-\frac{f_{T}^{\prime}}{4f_{T}}\right)-\frac{1}{\rho}\left(\Lambda_{l}+2\right)+\frac{\rho}{\zeta^{2}}\left(\frac{5}{2}-\frac{3}{2}\sqrt{f_{T}}\right)\right]\xi_{1,2}.\label{eq:104}
\end{align}
...
loop approximation presented in (\ref{eq:72}). The effective mass
...
Finally, in the confined phase, the interpretation of the fermionic
bound states presented in the Green function also need to be considered.
According to AdS/CFT, the bulk and dual theory should share the same
Hilbert space, hence both of them would include such fermionic bound
states. In the side of bulk theory, these bound states should be interpreted
as the fermionic super partner of the meson i.e. the mesinos \cite{key-36}
since they are created by the supersymmetric fermion on the $\mathrm{D}_{p+4}$-brane.
However such fermionic meson should be absent in non-supersymmetric
QCD as the dual theory. On the other hand, the bulk field is always
expected to be the source of a gauge invariant operator in the dual
theory according to the gauge-gravity duality \cite{key-14,key-38}.
Altogether, it may therefore be constructive to interpret these fermionic
bound states as baryon states by taking into account a baryon vertex
in the bulk. Specifically, let us consider a baryon vertex, which
is identified as a $\mathrm{D}_{8-p}$-brane wrapped on $S^{8-p}$
\cite{key-45,key-46,key-47}, located at $r=r_{KK}$ in the bubble
background and the $\left(p,p+4\right)$ string is totally inside
the $\mathrm{D}_{p+4}$-brane terminating on the baryon vertex. In
this configuration, the $\left(p,p+4\right)$ string is also a $\left(p+4,p+4\right)$
string connecting to a baryon vertex, so the associated fermionic
bound states are equivalently created by the baryon vertex which can
be identified as baryon states in the dual theory. Hence the peaks
presented in the confined Green function would refer to the baryon
spectrum and it is possible to fit the experimental data if we further
identify the quantum number $l,s$ in (\ref{eq:35}) to the isospin
and spin of the baryon. In this sense, we can identify, for examples,
the first three lowest states with same parity presented in Figure
\ref{fig:6} as proton, $N\left(1440\right)$ and $N\left(1710\right)$
for $l=1$ as \cite{key-48}. Thus our approach with D4/D8 model gives
the mass ratios of $N\left(1440\right)$ and proton, $N\left(1710\right)$
and proton as $M_{N\left(1440\right)}/M_{proton}\simeq1.51,M_{N\left(1710\right)}/M_{proton}\simeq1.95,$
