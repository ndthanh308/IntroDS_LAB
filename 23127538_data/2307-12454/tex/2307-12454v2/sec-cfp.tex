...
\label{ex-ind}
%\mps{\HT{p.13 examples and case studies -> I only see one running example and
%two simple remarks on N.}}
...
%\mps{\HT{p.13 l.-13: s.p. operators were defined already on the same page.}}
The application $\Phi(P)$,
where $\Phi = \lambda X\,Q$,
is defined as $Q[P/X]$. 
...
built from atomic formulas by the propositional operator $\land$,
the quantifiers $\forall$, $\exists$, and least and greatest fixed points.
Disjunction and other \emph{logical} operators introduced later are excluded.
%
...
% which is the base of the soundness of our realizability interpretation.
%
...
$\forall x \,.\, \NN(x) \to (x = 0 \to A) \land (x \neq 0 \to B)$. 
This is 
...
specifying which element of $\tau(A)$  
%\UB{$a\in\tval{\tau(A)}{\zeta}$} 
realize $A$.
We use types and programs as terms of RIFP to denote subdomains and elements of $D$, respectively.
%\UB{($\rea(A)$ is formalized in the system $\RIFP$ introduced below)}. 
%\item[(2)] 
\item\label{reali-prog} 
showing that 
from a proof of $A$ one can extract a program $M$ such that the formulas $M : \tau(A)$ and $\rea(A)(M)$
are provable in $\RIFP$ (and hence the program's denotation 
...
Therefore, we define for a Harrop formula $A$, an $\RIFP$-formula
 $\reah(A)$ that represents the realizability of $A$. For a predicate variable, $\rea(X)$ is a fresh type variable $\reali{X}$
...
%\HT{of the same section.}
\end{example}
%\mps{I added \HT{"of type $\bool$"}}
...
%
\begin{itemize}
%  \mps{\HT{R1:productive in Spreen\&Berger seems to be different from strict in the submission - how does it come that Spreen\&Berger can cite BT22? If there is an inclusion, it might be worth mentioning here.}}
...
not only disjunction and predicate variable, 
...
%
where    the formulas and predicates in the $\IFP$ rules run now over $\CFP$.
%
...
By inspecting the proof rules one easily sees:
%
\begin{lem}
\label{lem-cfp-wellformed}
If $\Gamma$ is a well-formed $\CFP$ assumption context, i.e.\ $\Gamma$
consists of wellformed $\CFP$-formulas, and $\Gamma\vdash A$ is derivable in $\CFP$,
then $A$ is a well-formed $\CFP$-formula.
\end{lem}
...
%
\item The rules Rest-return, Rest-efq, Conc-return, and Conc-mp 
only apply if $B$ is strict.
\end{itemize}
\end{minipage}
}
\medbreak
...
%\mps{\HT{(1) depends on (2). Shall we swap (1) and (2)?} \UB{Done.}}%    (1) 
(\ref{lem-strict-reg})
...
For the proof one also needs the following facts (a) and (b), 
...
then $A[\pcv{X}/X]$ is Harrop. Easy proof by induction on $A$.


%  (3) 
(\ref {lem-strict-amb})
follows from the fact that if $A$ is of the form 
$\Set(B)$,
then $B$
  must be strict.
\end{proof}
%\mps{\UB{I deleted ``identifying $M$ with $\val{M} \in D$'' since it doesn't fit.}}
...
%The following was  \item[(4)]  (\item\label{lem-strict-prod})
% of Lemma 4.6
\begin{lem}
\label{lem-prod-notbot}
For a program $M$ that realizes a formula $A$, 
              $M$ is productive iff $\val{M} \neq \bot$.
\end{lem}
\begin{proof}
%  (4) 
This follows from 
%(1), (3) and Corollary~\ref{cor:ddatabot}.   
Lemma~\ref{lem-strict}~(\ref{lem-strict-reg}) and~(\ref{lem-strict-amb}), 
Lemma~\ref{lem-realizers-typed}, and Theorem~\ref{cor:ddatabot}.  
% Detailed proof of (4):
% Thm 3.5, (1),(2): If M has regular type, then 
%                        M prod iff [[M]] neither bot nor Amb(bot,bot) 
% M r A, hence M : tau(A) by Lemma 4.7 ! which is regular, by (2). 
%        Also [[M]] not Amb(bot,bot), by (3).
% M productive => [[M]] not bot, by Thm 3.5 since M has regular type.
% [[M]] not bot => M productive, by Thm 3.5 since M is not Amb(bot,bot).
% We see that the proof needs Lemma 4.7
\end{proof}
...
%if $A$ is not realizable.
\end{rem}

%\mps{\HT{I separated the remark into two remarks.}}
\begin{rem}
  \label{rem-mult}
Next, let us discuss $\rea(\Set(B))(M)$ from an operational point of view:
%  (identifying $M$ with $\val{M} \in D$).
By the 
%Adequacy Lemma (\ref{lem:ssp}(5)), 
Adequacy Lemma (\ref{lem:ssp}(\ref{lem:ssp:ade})), 
%the formula $\rea(\Set(B))(M)$ 
this formula
means that    
...
%the operational semantics of which is referred to
%in the definition of realizability 
