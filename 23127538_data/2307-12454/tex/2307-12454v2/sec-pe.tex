...
%
%\mps{\UB{I deleted the sentence aboy Harrop cases since it is not needed.}}
...
\begin{proof}\quad\\
  ...
  Hence $b\in\{\Left,\Right\}$ and therefore $b \ne \bot$.
  %
  Now assume $b \ne \bot$.
...
of a 
%closeA%d 
%\mps{\HT{Do we need "closed"?}}
%\mps{\UB{No. I removed it.}}
formula $A$ from a set of axioms
one can extract a 
program $M$ 
such that 
%the typing rules prove 
$\vdash M : \tau(A)$
and
$\RCFP$ proves $\ire{M}{A}$ 
...
%\mps{\HT{p.23 l.7 occurs -$>$ only occurs.}}
%
%
...
and is realized by the identity function. %$\leftright$.
%
%
...
the rank of $a_n$: 
%(where the rank of a compact element of $D$ is defined 
...
%\mps{\UB{added forward reference to Faithfulness Thm}}
...
the functional implication $\NN(y)\to X(f(x,y))$ for which the minimal %only 
quasi-closed
...
\begin{lem}
\label{lem-respect}
%For any $\RCFP$ formula $A$, predicates $P,Q$ of the same arity, and predicate variable $X$ 
%that is not free in $P,Q$ and has the same arity as $P$, 
\begin{enumerate}
%\item[(a)] 
...
Then, by 
%Lemma~\ref{lem-data-compact}~(3), 
Lemma~\ref{lem-data-compact}~(\ref{lem-data-compact-reg}), 
(by 
%Lemma~\ref{lem-regular}~(1), 
...
%the required inclusion reduces to %
...
%
 ...
 %
 From a $\CFP$ proof of an admissible formula $A$ from a set of axioms one can extract a 
...
%\mps{\UB{I dropped part (2) of the remark.}}
\begin{rem}
