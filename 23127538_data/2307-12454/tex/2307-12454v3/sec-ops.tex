...
For a practical implementation, one might prefer a sequential version 
of (p-ii) and remove (p-iii), as done in the definition of 
the function \verb|ed| (`extract data') in Appendix~\ref{sub-program-data}.
...
We define 
${M}_{{D}} \in {D}$ by structural induction on programs $M$:
%
\begin{align*}
...
Note that $M_D \neq\bot$ iff $M$ is a deterministic w.h.n.f.

Since clearly $M \newprintp N$ implies $M_{D} \sqsubseteq_{D} N_{D}$,
for every computation $ M_0 \newprintp M_1 \newprintp  \ldots$,  
...
by coinduction.
 ...
 for some data constructor $C$.     \item[(5)] $a = \Fun(f)  \land d = a$
  ...
  Lemma~\ref{lem:ssp} (\ref{lem:ssp:det}) and (\ref{lem:ssp:ade}).
  ...
  (3) holds by Lemma~\ref{lem:ssp}~(\ref{lem:ssp:det}), 
 (\ref{lem:ssp:pres}) and~(\ref{lem:ssp:ade}).
...
We say that a program \emph{$M$ has a regular type}
if $M:\rho$, i.e.\ $M$ is a closed program such that 
$\val{M}\in\tval{\rho}{}$, 
for some closed regular type $\rho$.
%
...
\end{thm}
%
The following lemma is the essence of the proof:
%
...
$C \ne \Amb$ because $e \sqsubseteq d$ with $d \in \ddata(\val{M})$. We have 
%
$d = C(\vec{d'})$ with $d_i' \sqsubseteq e_i'$.
...
is semantically equal to a type of the form 
  ...
  Furthermore, by Lemma~\ref{lem:ssp}~(\ref{lem:ssp:pres}), $C(\vec{M'})$ has 
  ...
  and thus $M \newprintp^* K$ by (p-i).
  Note that   $K : \Am^{0}(\sigma)$ and
  $d \in \ddata(\val{K})$.  Therefore, the already proven case $k=0$ applies and 
...
%
\begin{proof}[Proof of Theorem~\ref{thm:dataconv}]
 %
...
Let $d_0 \sqsubseteq d_1 \sqsubseteq \ldots$ 
  be an infinite sequence of 
    ...
    Let $M_0 = M$.  By applying Lemma~\ref{lem-claim} to 
...
Then, by Theorem~\ref{thm:data}, $\ddata(\val{M})$ must contain a
  ...
  $\bot\not\in\ddata(\val{M})$.
  To show that $M$ is productive, consider a computation 
  $M = M_0 \newprintp M_1 \newprintp \ldots$.
  By Theorem~\ref{thm:data}, 
  $\sqcup_{i \in \NN} ((M_i)_{D}) \in\ddata(\val{M})$,
  hence $\sqcup_{i \in \NN} ((M_i)_{D})\neq\bot$.
  It follows that for some $i$, $(M_i)_{D}\neq\bot$ which, 
  as noted earlier, is equivalent to $M_i$ being a deterministic w.h.n.f.
  %
\end{proof}
%
  \end{thm}
...
Theorem~\ref{cor:ddatabot}  guarantees a similar property for our 
