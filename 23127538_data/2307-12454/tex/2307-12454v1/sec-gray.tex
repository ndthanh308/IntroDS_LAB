...
%We briefly sketch these representations and their realizability
%interpretations. For details we refer to \cite{IFP}.  
...
\begin{eqnarray*}
  ...
  p = d: p'\ \land 
  \ire{p'}{\myC(2x-d)})\, 
%%   \ire{p}{\myC(x)} \eqnu \exists\, d, p'\ (
%% \ire{d}{\SD(x)} \land p = \Pair(d, p')\ \land |2x-d| \leq 1 \land 
%%   \ire{p'}{\myC(2x-d)})\,
\end{eqnarray*}
%
...
%% like the sign function
%% but is multi-valued at $0$ in the sense that 
%%  where $\tent$ is the tent function 
...
prove $\myG \subseteq \myC_2$ in CFP (Thm.~\ref{thm-g-mc}).
...
\begin{proof}
  ...
  ${\sf f}_2(d:p) = 1:p$, and 
  ${\sf f}_3(p) = \nh\ p$ where $\nh\ (d:p) = \nnot\ d: p$.
  % The three extracted programs take \HT{
  % the tail of the input stream}, respectively set the first digit to $1$, respectively negate the
  % first digit.
  ...
  The extracted program is ${\sf f}_6(d:p) = d:1:\nh\ p$.
\end{proof} 
%
\begin{thm}
\label{thm-g-mc}
...
%% %% cases $x \leq 0$, $x \geq 0$, and $|x| \leq 1/2$,
%% alternatives of the premise and 
...
%% &  \caseof{c} \{\Left(d) \to 
%%                        \caseof{d} \{\\
%% &\hspace{3.5cm} \Left(\_) \to \Pair(-1, b:p); \\
%% &\hspace{3.5cm}               \Right(\_) \to \Pair(1,(\nnot\ b):p) \};\\
%% &\hspace{1.5cm}\Right(\_) \to    \Pair(0, a: (\nh\ p))\}\\
%% \nnot\ a \eqdef& \caseof{a} \{\Left(\_) \to \Right; \\
%% &\hspace*{1.6cm}\Right(\_) \to \Left\}\\
%% \nh\ (a: p) \eqdef& (\nnot\ a): p
%% \end{align*}
%
\begin{align*}
...
\begin{align*}
&\gtos \eqrec (\mon\ \gtos) \circ \mathsf{s}
\end{align*}
%
Using the equational theory of RCFP,
one can simplify $\gtos$ to the following program.
% \HT{Here, the axiom of \HT{RCFP} consists
% of axioms of RIFP~\cite{IFP} extended with the
% two rules $b \ne \bot \to \strictapp{a}{b} = a\ b$ 
% and $\strictapp{a}{\bot} = \bot$ for strict applications, and classical logic.
% }
...
%% \HT{(note, finally comment out)
%% After applying equational theory of RIFP,
%% \begin{align*}
%%   &\gtos \eqrec (\mon\ \gtos) \circ \mathsf{s}
%%   \end{align*}
%%   is equivalent to
...
%%   \gtos\ &(a:b:t) =
%%   \Amb(\\
%%   &  \caseof{(\strictapp{\Left}{(\leftright\ a)})} \{\Left(d) \to  
%%   \caseof{d} \{\\
%% &\hspace{6cm} \Left(\_) \to \Pair(-1, b:p); \\
%% &\hspace{6cm}               \Right(\_) \to \Pair(1,(\nnot\ b):p) \};\\
%% &\hspace{4.5cm}\Right(\_) \to    \Pair(0, a: (\nh\ p))\},\\
%% %
%% &  \caseof{(\strictapp{\Right}{(\caseof{b}\{\Left(\_) \to \bot; \Right(\_) \to \Nil\}})}\\
%% &\hspace{4.5cm}  \{\Left(d) \to  
%%   \caseof{d} \{\\
%% &\hspace{6cm} \Left(\_) \to \Pair(-1, b:p); \\
%% &\hspace{6cm}               \Right(\_) \to \Pair(1,(\nnot\ b):p) \};\\
%% &\hspace{4.5cm}\Right(\_) \to    \Pair(0, a: (\nh\ p))\})\\
%% \end{align*}
%% Then, we do case analysis whether $a$ is $\bot$ or not
%% and whether $b$ is $\Right(\_)$ or not using classical logic.
%% If $a$ is not $\bot$, $\leftright\ a$ is not $\bot$.
%% Therefore, 
%% $$\caseof {(\strictapp{\Left}{(\leftright\ a)})} \{\Left(d) \to \caseof{d} ...\}$$ 
%% is equal to
%% $$\caseof {({\Left}{(\leftright\ a)})} \{\Left(d) \to \caseof{d} ...\}$$
%% and then to
%% $\caseof{(\leftright\ a)} ...$ and then to
%% $$
%% \ \ (\caseof{a} \{\Left(\_) \to  -1: \gtos\ (b:t);\\
%% \ \                   \Right(\_) \to  1: \gtos((\nnot\ b):t)\}),\\
%% $$
%% On the other hand, if $a$ is $\bot$, then it is equivalent to $\bot$.
%% Similarly, if $b$ is $\Right(\_)$, then 
%% $\caseof {(\strictapp{\Right}{(...)})...}$  is $1: \gtos((\nnot\ b):t)$, and
%% it is $\bot$ otherwise.
%% Thus, with classical logic, for the four patterns of $a$ and $b$, compare this program with the following
%% program to see that they are equal.
%% }
%% \UB{That looks correct. Do you think we should give a few details of this reasoning in the paper?}
%% \HT{No, I do not think so.}
%
%
% \begin{align*}
% {\mathsf s}\ (a:b:t) &=  \Amb (\\
%     &(\caseof{a} \{\Left(c) \to  \Pair(-1,b:t);\\
% &\hspace*{1.8cm}                \Right(c) \to  \Pair(1, (\nnot\ b):t)\}), \\
%     &(\case\ b\ \of \{\Right(c) \to \Pair(0, a: (\nh\ t))\}))
% \end{align*}
%
\begin{align*}
  \gtos\ &(a:b:t) =\Amb(\\
