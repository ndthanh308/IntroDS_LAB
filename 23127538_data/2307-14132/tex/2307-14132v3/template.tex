...
\title{CIF-T: A Novel CIF-based Transducer Architecture for \\ Automatic Speech Recognition}
\name{
    \begin{tabular}{c}
       \large ~Tian-Hao Zhang$^{1, 2, *}$,~Dinghao Zhou$^{1, *}$\thanks{* Equal contribution.}, ~Guiping Zhong$^{1}$, ~Jiaming Zhou$^{1, 3}$, ~Baoxiang Li$^{1,} \textsuperscript{\Letter}$\thanks{\textsuperscript{\Letter} Corresponding author.}
   \end{tabular}
    \vspace{-5pt}
}
\address{
...
\end{abstract}
\begin{keywords}
Speech Recognition, RNN-T, Continuous Integrate-and-Fire
\end{keywords}
