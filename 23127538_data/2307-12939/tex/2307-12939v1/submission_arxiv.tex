...
\title[The width of curves in Riemannian manifolds]{The width of curves in Riemannian manifolds}
\author{Lucas Ambrozio, Rafael Montezuma and Roney Santos}
\address{L. Ambrozio: IMPA \\ Rio e Janeiro RJ 22460-320
Brazil}
\email{l.ambrozio@impa.br}

\address{R. Montezuma: Departamento de Matemática, UFC \\ Campus do Pici \\ Fortaleza, CE 0455-760 Brazil}
\email{montezuma@mat.ufc.br}

\address{R. Santos: Departamento de Matemática, UFC \\ Campus do Pici \\ Fortaleza, CE 0455-760 Brazil}
\email{roneysantos@mat.ufc.br}

\thanks{L.A. is supported by CNPq - Conselho Nacional de Desenvolvimento Cient\'ifico e Tecnol\'ogico (309908/2021-3 - Bolsa PQ) and by FAPERJ - Funda\c{c}\~ao Carlos Chagas Filho de Amparo \`a Pesquisa do Estado do Rio de Janeiro (grant SEI-260003/000534/2023 - BOLSA E-26/200.175/2023 and grant SEI-260003/001527/2023 - APQ1 E-26/210.319/2023)}
...
\thanks{R.S. is supported by Instituto Serrapilheira grant ``New perspectives of the min-max theory for the area functional".}
...
\end{equation*}
where $d : M\times M \rightarrow [0,+\infty)$ is the Riemannian distance of $(M^n,g)$. %Whenever necessary, the dependence on the metric $g$ will be indicated by the notation $\mathcal{D}_g$. \\
...
%\indent If $\Gamma$ is itself a geodesic, it may happen that a critical point $\{p,q\}$ of $\mathcal{D}$ bounds only minimising geodesics that are arcs of $\Gamma$ itself. (See Example \ref{examhemisphere} in Section \ref{sectionexamples}). \\
...
\begin{itemize}
	...
	\item[$iii)$] $p_t$ and $q_t$ bound an arc $C_t\subset \Gamma$ in such way that $t\in [0,1] \mapsto C_t$ is a continuous map with $C_0=\emptyset$ and $C_1=\Gamma$.
\end{itemize}
\end{defi}
...
\begin{defi}\label{defwidth}
	...
	\begin{equation*}
		\mathcal{S}(\Gamma) = \inf_{\{p_t,q_t\}\in \mathcal{V}_0} \,\, \max_{t\in [0,1]} d(p_t,q_t)
	\end{equation*}
	where $\mathcal{V}_0$ is the set of sweepouts $\{p_t,q_t\}$ of $\Gamma$. 	
\end{defi}
...
\subsection{The width of the boundary of discs} \label{introdiscs} 
	...
	Under these geometric conditions, critical points of $\mathcal{D}$ on $\partial \Omega$ and the minimising geodesics they bound enjoy further properties. For instance, a uniqueness property hold (Proposition \ref{propuniqu}), and local minima of $\mathcal{D}$ bound exactly one minimising geodesic (Proposition \ref{proplocalminima}). Moreover, Theorem A can be refined to yield much more precise information about critical points of $\mathcal{D}$ at the level $\mathcal{S}(\partial \Omega)$, at least under a natural geometric condition that we will introduce shortly.\\
...
\begin{thmC}
	...
	\begin{itemize}
		\item[$a)$] If there exists a continuous map $\phi : \Gamma \mapsto \Gamma$ such $d(x,\phi(x))=diam(\Gamma)$, then $\mathcal{S}(\Gamma)=diam(\Gamma)$.
		\item[$b)$] If $\mathcal{S}(\Gamma)=diam(\Gamma)$, then for every $x\in \Gamma$ there exists $y\in \Gamma$ such that $d(x,y)=diam(\Gamma)$.
	\end{itemize}
	Assume, moreover, that $\Gamma$ is the boundary of a totally convex smoothly embedded disc. Then $\mathcal{S}(\Gamma)=diam(\Gamma)$ if and only if there exists a continuous map $\phi : \Gamma \mapsto \Gamma$ such $d(x,\phi(x))=diam(\Gamma)$.
\end{thmC}

	Notice that a continuous map $\phi : \Gamma\mapsto \Gamma$ such that $d(x,\phi(x))=diam(\Gamma)$ is a \textit{monotone} homeomorphism of the circle $\Gamma$, \textit{i.e.} its lift $\hat{\phi} : \mathbb{R} \rightarrow \mathbb{R}$ is an increasing homeomorphism of the real line. \\
...
\end{thmD}
	...
	It is interesting to contrast the objects described in Theorem D to Riemannian fillings of the circle \cite{Gro1}. Recall that a Riemannian filling of the circle is a Riemannian surface $(M^2,g)$ with connected boundary such 
\begin{equation*}
...
% Figure environment removed
	...
	These examples show that $\mathcal{S}(\partial \Omega)$ is more suitable for the formulation of the results of this article than the other two min-max quantities.

\subsection{Perspectives} It would be interesting to know whether some refinement of the min-max Theorems A and B could prove that totally convex discs with smooth boundary always contain either a free boundary minimising geodesic with free boundary index at most one, or a pair of minimising geodesics, with extremities on the boundary, that are simultaneously stationary. (\textit{Cf}. Corollary \ref{corexistence}).
...
%\textcolor{red}{(CUIDADO!)} Another possible direction of investigation regards the study of distance-like functionals on the set of subsets with at most $k$ points in a circle, for a given $k\geq 3$. 

	We believe the methods of this paper can be generalised to the functional that computes the area of solutions to the Plateau problem for a given Jordan curve in a sphere inside a Riemannian three-manifold. We intend to describe this generalisation in a future work.

\subsection{Plan for the paper} In Section \ref{sectionbasicproperties}, we develop the basic theory of regular and critical points of the distance functional $\mathcal{D}:\mathcal{P}_*\mapsto [0,+\infty)$ along the lines of \cite{GroShi} and \cite{Gro}. (See also the excellent exposition of this theory W. Meyer \cite{Mey}. A related, but different notion of critical pairs of points of the distance function has been investigated in \cite{Ald}). We prove in particular the existence of gradient-like flows on the set of regular points (Proposition \ref{propgradientlikeflow}). We also prove that critical points satisfy special properties when the embedded circle bounds a totally convex smoothly embedded disc, see in particular Proposition \ref{propuniqu} and \ref{proplocalminima}.
	...
	The basic min-max Theorem A is improved in Section \ref{sectiondiscs}, under the assumption that the circle is the boundary of a totally convex disc. The key argument is to show that, under the extra assumption that no free boundary stable minimising geodesic exist in the disc, every critical point belongs to a sweepout restricted to which the functional attains a strict maximum exactly at this critical point, and which is moreover ``monotone" near this critical point (Propositions \ref{propstarcritical1} and \ref{propstarcritical2}). Birkhoff's curve shortening process, adapted to curves with boundary as in \cite{GluZil}, \cite{Zho} and \cite{DonMon}, is used here as a convenient technical device. This leads to Theorem B (Theorem \ref{thmBbis}).
...
\end{lemm}
%\begin{proof}
%	Let $\gamma_i:[0,a_i]\rightarrow M$ be a sequence of minimising geodesics joining $\gamma_i(0)=p_i$ and $\gamma_i(a_i)=q_i$. Since  $a_i=d(p_i,q_i)$, this sequence of numbers converges to $a=d(p,q)$. If $a=0$, the limit geodesic is the constant geodesic and there is nothing to prove. If $a\neq 0$, then, up to subsequence, the unit vectors $\gamma_i'(0)$ converge to some unit vector $v$, and by continuity of the geodesic flow, the geodesic $\gamma$ issuing from $p$ with speed $v$ satisfies $\gamma(a)=\lim_{i\rightarrow \infty}\gamma_i(a_i)=q$. As a limit of minimising geodesics, $\gamma$ is minimising as well.
%\end{proof}
...
\begin{equation*}
	(\star) \textit{ no free boundary stable geodesics exists on } (\Omega,g).
\end{equation*}
...
\end{proof}
	...
	Before we continue, we need to recall a few properties of unstable free boundary geodesics $\gamma$ in $(\Omega,g)$. By definition, a normal vector field $X$ exists along such $\gamma$ such that $Q(X,X)<0$. By virtue of the second variation formula \eqref{eqsecondvariation}, a small deformation of $\gamma$ by the flow of some extension of $X$ by a vector field that is tangent to $\partial \Omega$ is a deformation by curves in $\Omega$ with extremities in $\partial \Omega$ that are strictly shorter than $\gamma$.
...
If $\gamma$ is not free boundary stable, then there exists a smooth positive function $u$ on $\gamma$ such that $Q(uX,uX)<0$. %that solves 
%			\begin{align*}
%    			u'' + Ku + \lambda u & =0 \quad \text{on} \quad (0,a), \\ 
%			    -u'(0) &=  A_{\Gamma}(X(p),X(p))u(0), \\
%			    u'(a) &=  A_{\Gamma}(X(q),X(q))u(a)
%     		\end{align*} 
%		for a \textcolor{blue}{negative} constant $\lambda$, and therefore satisfies 
%		\begin{equation*}
%			\textcolor{blue}{Q(u X,u X)= \lambda \int_{\Sigma} u^2 <0.}
%		\end{equation*}
%		\item If $\gamma$ is free boundary stable, then any non-zero function $u$ on $\gamma$ such that $Q(u X,u X)=0$ does not change signs and satisfies the (free boundary) Jacobi equations
%			\begin{align*}
%    			u'' + Ku & =0 \quad \text{on} \quad (0,a), \\ 
%			    -u'(0) &=  A_{\Gamma}(X(p),X(p))u(0), \\
%			    u'(a) &=  A_{\Gamma}(X(q),X(q))u(a).
%     		\end{align*} 
%	\end{enumerate} 
...
\end{multline*} 
	By standard techniques, the minimisation of $Q(\phi,\phi)/\int_{0}^a \phi(s)^2 ds$ among non-zero functions in $C^{\infty}([0,a])$ yields a solution $u\neq 0$ of
     ...
     If $\gamma$ is not stable, then we clearly have $\lambda_1<0$, and the corresponding positive solution $u$ of \eqref{eqfirstjacobieigen} satisfies $Q(u,u)= \lambda_1 \int_0^a \phi^2(s)ds < 0$.
     
     %If, however, $\gamma$ is stable, then $\lambda_1\geq 0$. And if there exists a non-zero function $u$ on $\gamma$ such that $Q(u,u)=0$, then $\lambda_1=0$ and $u$ attains the minimum of $Q(\phi,\phi)/\int_{0}^a \phi(s)^2 ds$ . As observed above, this implies that such $u\neq 0$ satisfies \eqref{eqfirstjacobieigen} with $\lambda_1=0$ and, moreover, does not change signs. 
\end{proof}

%\begin{rmk}\label{rmkconvexity}
%	\textcolor{blue}{In case $(1)$ of the above Lemma, it is straightforward to argue using the formula 
%	\begin{equation*}
%		\frac{d}{dt}_{|_{t=0}}k= -u''- K u
%	\end{equation*}
%	for the first variation of the geodesic curvature $k$ of a geodesic in a variation with variational vector field $uX$, that a variation of the free boundary geodesic $\gamma$ in the direction $uX$ as in $(1)$ produces curves that have geodesic curvature vector pointing strictly towards the region of the disk which they bound that does not contain $\gamma$.} 
%\end{rmk}
...
\begin{prop}\label{propstarcritical2}
	...
	\begin{itemize}
		...
		\item[$iv)$] For all $t\geq 1/2$ sufficiently close to $1/2$, the sweepout is regulathe geodesicrly strictly monotone with $\{p_t,q_t\}=\partial c_t$ for a family of curves with  $c_{1/2}=\gamma_2$ and $\partial c_{t}$ belonging to the the arc of $\partial \Omega\setminus \{p,q\}$ into which $T(p)$ and $-T(q)$ points for $t<1/2$.
...
\begin{proof}
	...
	By construction, and by the second variation formula \eqref{eqsecondvariation}, $L(c_t)$ is a function that has a critical point at $t=1/2$, and a strictly negative second derivative at that point. Since the free boundary index of $\gamma$ is at least two, this negative subspace for the index form $Q$ belongs to a two-dimensional subspace of $\Gamma(N\gamma')$ where $Q$ is negative definite. Choose some $Y\in \Gamma(N\gamma')$ in this subspace in such way that 
	\begin{equation*}
		(Y(p),Y(q)) \quad \text{and} \quad \left(\frac{d}{dt}_{|_{t=1/2}}p_t,\frac{d}{dt}_{|_{t=1/2}}q_t\right)
	\end{equation*}
...
\section{Width, diameter and length}\label{sectionzoll}
	...
	The following general lemma about involutive homeomorphisms of the circle will be used several times in what follows. Recall that a homeomorphism $\phi: S^1\rightarrow S^1$ is called \textit{monotone} when $\phi(x)$ turns around $S^1$ in the same direction as $x$ does.

\begin{lemm}\label{lemcontinuousinvolution}
	Let $\phi : S^1\rightarrow S^1$ be a continuous involution without fixed points. Then $\phi$ is a monotone homeomorphism, and $\phi$ induces a homotopically non-trivial loop $p \in S^1 \mapsto \{p,\phi(p)\} \in \mathcal{P}_*$.
\end{lemm}
\begin{proof}
	...
	Monotonicity implies that $\phi$ interchanges the two arcs of $S^1$ bounded by any pairs $\{x,\phi(x)\}\in \Gamma$ as $x$ moves towards $\phi(x)$. Thus, $p\in S^1\mapsto \{p,\phi(p)\}\in \mathcal{P}_*$ lifts to a continuous open path in the oriented double cover of $\mathcal{P}_*$. In other words, it is homotopically non-trivial.
\end{proof}

\begin{prop}\label{propthmCvolta}
	Let $\Gamma$ be a smoothly embedded circle in a complete Riemannian manifold. Suppose that there exists a continuous map $p\in \Gamma \mapsto \phi(p)\in \Gamma$ be such that $d(p,\phi(p))=diam(\Gamma)$ for every $p\in \Gamma$. Then $\mathcal{S}(\Gamma)=diam(\Gamma)$
\end{prop}
\begin{proof}
	By Lemma \ref{lemcontinuousinvolution}, the continuous map $\overline{\phi}: p\in \Gamma \mapsto \{p,\phi(p)\}\in \mathcal{P}_*$ is a homotopically non-trivial loop, as much as any sweepout of $\Gamma$. Since any two homotopically non-trivial curves on a real projective plane must intersect, any sweepout of $\Gamma$ must intersect $\overline{\phi}(\Gamma)$ in $\mathcal{P}_*$. But this just means that any sweepout $\{p_t,q_t\}$ of $\Gamma$ must contain a pair $\{p_{t_0},\phi(p_{t_0})\}$. Thus, for any sweepout of $\Gamma$, we have $\max_{t\in[0,1]}d(p_t,q_t))\geq d(p_{t_0},\phi(p_{t_0})))= diam(\Gamma)$.
...
\begin{proof}
	...
	Since the pairs $\{p,d_{+}(p)\}$ and $\{p,d_{-}(p)\}$ are both critical points of $\mathcal{D}$ and $p\neq d_{-}(p)$ and $p\neq d_{+}(p)$, the equality $d_{+}(p)=d_{-}(p)$ is an immediate consequence of Proposition \ref{propuniqu}. Thus, a map $\phi:\Gamma \mapsto \Gamma$ with $d(p,\phi(p))=diam(\Gamma)$ for all $p\in \Gamma$ is well-defined. Notice also that this map satisfies $\phi(\phi(p))=p$ for all $p\in \Gamma$, for the same reason.
	...
	The last assertion of the lemma follows now immediately from Lemma \ref{lemcontinuousinvolution}.
\end{proof}
...
\begin{thm}\label{thmCbis}
	...
	\begin{itemize}
		...
		\item[$(b)$] If $\mathcal{S}(\Gamma)=diam(\Gamma)$, then for every $x\in \Gamma$ there exists $y\in \Gamma$ such that $d(x,y)=diam(\Gamma)$.
	\end{itemize}
	Assume, moreover, that $\Gamma$ is the boundary of a smoothly embedded totally convex Riemannian disc. Then $\mathcal{S}(\Gamma)=diam(\Gamma)$ if and only if there exists a continuous map $\phi : \Gamma \mapsto \Gamma$ map such $d(x,\phi(x))=diam(\Gamma)$.
\end{thm}
...
\noindent \textbf{Claim 1}: for every $\varepsilon>0$, there exists $\delta>0$ with the following property: For every minimising geodesic $\gamma:[0,a]\rightarrow M$ of $(M^2,g)$ such that $\gamma(0)\in \partial M$ and $L(\gamma,g)\geq \varepsilon$, there exists $t\in (0,a]$ such that $d(\gamma(t),\partial M)\geq \delta$.  \\
	...
	In order to prove the Claim, we argue by contradiction: if not, there exists some $\varepsilon>0$ and a sequence of minimising geodesics $\gamma_i:[0,a_i]\rightarrow M$ of $(M^2,g)$, with $\gamma_i(0)\in \partial M$ and $L(\gamma_i)\geq \varepsilon$, such that they lie entirely in the tubular neighbourhood $V_{1/i}$. Passing to the limit, these geodesics converge to an arc of $\partial M$ of length $\geq \varepsilon>0$ that is geodesic as a limit of geodesics. But this contradicts the assumption that $\partial M$ is strictly convex in $(M^2,g)$. \\
	...
	The next claim gives an estimate on the length of the piece of a minimising geodesic, joining sufficiently far apart boundary points, outside the tubular neighbourhood $V_\delta$. As expected, this piece is fairly long. \\
...
\noindent \textbf{Claim 3}: Let $p$, $q\in \partial M$ be such that $d_g(p,q)\geq \frac{4}{5}\mathcal{S}(\partial M,g)$. If $\gamma$ is a minimising geodesic in $(M^2,g)$ joining $p$ and $q$, then $L(\gamma\setminus V_\delta,g)\geq 3\mathcal{S}(\partial M,g)/5$. \\
	...
	In fact, let $\gamma:[0,\ell]\rightarrow M$ be a minimising geodesic of $(M^2,g)$ joining the boundary points $p$ and $q$ in $(M^2,g)$. Since $\ell=L(\gamma)\geq d_g(p,q)> 2\varepsilon$, we may apply Claim $1$ at both extremities and conclude that there exists $t_1\in (0,\varepsilon]$ and $t_2\in [\ell-\varepsilon,\ell)$ such that $d(\gamma(t_i),\partial M)\geq \delta$. By the strict convexity of $\partial M$, a sufficiently small tubular neighbourhood of $\partial M$ is foliated by strictly convex curves of $(M^2,g)$. Thus, by the maximum principle, 
	\begin{equation*}
...
\end{equation*}
	...
	In fact, by definition of $\mathcal{S}(\partial M,g)$, any sweepout of $\partial M$ must contain a pair $\{p,q\}\subset \partial M$ such that $d_g(p,q)\geq 4\mathcal{S}(\partial M,g)/5$. %Let $\gamma$ be a minimising geodesic of $(M^2,g)$ joining such points. It follows from Claim 3 that 
%\begin{equation*}
%	 L(\gamma\setminus V_\delta,g) \geq \frac{3}{5}\mathcal{S}(\partial M). 
%\end{equation*}
...
%\begin{lemm} \label{lemsdimpliesstar}
%	Let $(\Omega,g)$ be a totally convex disc with smooth boundary in a complete Riemannian surface. Assume $(\Omega,g)$ has property $(\star\star)$.\\
%	\indent \textcolor{blue}{If $(\Omega,g)$ satisfies $\textcolor{blue}{\mathcal{S}(\partial \Omega)=diam(\partial \Omega)}$. then, the non-trivial critical points of $\partial D$ on $\partial \Omega$ are all of the form
%	\begin{equation*}
%		\{x,\phi(x)\}\subset \partial \Omega
%	\end{equation*}
%	where $\phi : \partial \Omega \rightarrow \partial \Omega$ is the continuous involution with $d(x,\phi(x))=diam(\partial \Omega)$ given in Theorem C (Theorem \ref{thmCbis})}.
%\end{lemm}

\begin{thm}\label{thmFbis}
 Let $\Omega$ be a totally convex disc  with smooth boundary in a complete Riemannian surface $(M^2,g)$. Assume that every two points of $\partial \Omega$ are joined by a unique geodesic, that $\mathcal{S}(\partial \Omega)=diam(\partial \Omega)$, and that $(\Omega,g)$ admits an isometric involution $A$ with no fixed boundary points. %and
%	\begin{equation*}
%		\mathcal{S}(\partial \Omega)=diam(\partial \Omega),
%	\end{equation*}
...
\begin{proof} By Lemma \ref{lemcontinuousinvolution}, the restriction of the isometric involution $A : \Omega\rightarrow \Omega$ to $\Gamma$ is a monotone homeomorphism. As a continuous map from the disc to itself, $A$ has fixed points (by Brouwer Theorem), and none of them lies in the boundary (by assumption). Let $x_0\in \Omega$ denote a fixed point of $A$ that is closest to ${\partial \Omega}$. 
	...
	Using \eqref{eqaux1}, it is immediate to see that the concatenated curve $C$ is smooth at $x_0$. It is also orthogonal to $\partial \Omega$ at $y_0$ and $A(y_0)$. Since there exists, by property $(\star\star)$, only one geodesic joining these two boundary points, we conclude that $C$ is a free boundary minimising geodesic joining $y_0$ and $A(y_0)$. In particular, $\{y_0,A(y_0)\}\subset \partial \Omega$ is a non-trivial critical of $\mathcal{D}$, which moreover satisfies
...
\begin{equation}\label{eqaux3}
	2d(y,x_0) = L(\gamma_2)+L(A(\gamma_2))
...
\end{equation*}
	...
	Now that we know that the distance between $x_0$ and boundary points is constant, using the first variation formula \eqref{eqfirstvariation} it is straightforward to check that a minimising geodesic joining $x_0$ and any $y\in \partial \Omega$ is orthogonal to $\partial \Omega$ at $y$, and therefore unique.	
...
%
\bibitem{Don} S. Donato, \textit{The first p-widths of the unit disk.} J. Geom. Anal. 32 (2022), no. 6, Paper No. 177, 38 pp.% \url{https://doi.org/10.1007/s12220-022-00913-3}
%
...
%
\bibitem{Wei} A. Weinstein., \textit{Periodic orbits for convex Hamiltonian systems.} Ann. of Math. (2) \textbf{108} (1978), no. 3, 507-518.% \url{https://doi.org/10.2307/1971185}
%
