...
\end{bmatrix},
\end{equation*}
for $1\le i\le k$ , $k\in\N$ and we say that we have $k$ different environments corresponding to each set of equations above. 
Where the submatrix $B_X \in \R^{p\times p}$ are the interactions among the covariates $X^{A_i}$, $\beta_{PA} \in \R^p$ are the causal effects of $X^{A_i}$ on $Y^{A_i}$, and $\beta_{CH} \in \R^p$ are the downstream effects of $Y^{A_i}$ on $X^{A_i}$. The random vector $A_i$ is called the shift corresponding to environment $i$. We assume that $(I-B)^{-1}$ is non-singular, so that $X^{A_i}$ and $Y^{A_i}$ have unique solutions. In addition, we also consider the observational environment 
\begin{equation*}
\begin{bmatrix}
Y^{O}\\
X^{O}
...
\\
Let $\mathcal{W}=\left\{w\in\R^k: \n w\n=1\right\}$. Given the vector $w\in\mathcal{W}$, define $A_w=\sum_{i=1}^kw_iA_i$ and
$$C^\gamma_w=\left\{A'\in\mathcal{A}:\mathbb{C}(A')\preccurlyeq \gamma \mathbb{C}(A_w)\right\}.$$ 
For any shift vector $A$ denote $R_{A}(\beta)=\E\left[\left(Y^A-\beta X^{A}\right)^2\right]$. Letting
\begin{itemize}
...
$$C^\gamma(A)=\left\{A'\in\mathcal{A}:\exists w\in\mathcal{W}, \mathbb{C}(A')\preccurlyeq \gamma \mathbb{C}(A_w)\right\}.$$
It is worth noting that
$$C^\gamma(A)=\bigcup_{w\in\mathcal{W}}C_w^\gamma(A).$$
From now on we will remove the dependence on $A$ from $C^\gamma(A)$ and $C_w^\gamma(A)$, simply writing $C^\gamma$ and $C_w^\gamma$ respectively instead.
...
\end{prop}
\begin{proof}
First note that if we let $u=(I-B)^{-1}_{1:p,\textbf{.}}\beta-(I-B)^{-1}_{p+1,\textbf{.}}$
...
\end{align}
which implies that the supremum is attained (is a maximum) if and only if $\sup_{\tilde{A}\in C^{1+\tau}}u\E\left[\tilde{A}\tilde{A}^T\right]u^T$ attains a maximum. We now show that this is indeed the case. Note that since $C^\gamma=\bigcup_{w\in\mathcal{W}}C^\gamma_w$, any element in $C^\gamma$ can be identified with a $w\in\mathcal{W}$. Given some row vector $v$ of dimension $p$, consider $L(\tilde{A})=v\E\left[\tilde{A}\tilde{A}^T\right]v^T$ and $S=\sup_{\tilde{A}\in C^{\gamma}}L(A)$. There exists a sequence $\{B_n\}_n\subset C^\gamma$ such that $\lim_nL(B_n)=S$ and for each $n$, $B_n\in C^\gamma_{w_n}$ for some $w_n\in\mathcal{W}$. Since $\mathcal{W}$ is compact there exists a subsequence $\{w_{n_k}\}_k$ such that $w_{n_k}\to w^*$ for some $w^*\in\mathcal{W}$. Since $\lim_kL(B_{n_k})=S$ and $L(B_{n_k})\le \gamma L(A_{w_{n_k}})$ and $\lim_kL(A_{w_{n_k}})=L(A_{w^*})$ we must have that $S=L(A_{w^*})$, so the supremum is in fact a maximum attained at $A_{w^*}$ (that depends on $v$). 
Given an optimal $w^*$ as above,
\begin{align}\label{supeq2}
&\sup_{\tilde{A}\in C^{1+\tau}_{w^*}}R_{\tilde{A}}(\beta)=
\sup_{\tilde{A}\in C^{1+\tau}_{w^*}}\E\left[u\left(\epsilon+\tilde{A}\right)\left(\epsilon+\tilde{A}\right)^Tu^T\right]=\nonumber
\\
&u\E\left[\epsilon\epsilon^T\right] u^T
+
\sup_{\tilde{A}\in C^{1+\tau}_{w^*}}u\E\left[\tilde{A}\tilde{A}^T\right]u^T=R_O(\beta)+(1+\tau)u\E\left[A_{w^*}A_{w^*}^T\right]u^T=\nonumber
\\
&R_O(\beta)+(1+\tau)u\E\left[(\epsilon+A_{w^*})(\epsilon+A_{w^*})^T\right]u^T-(1+\tau)u\E\left[\epsilon\epsilon^T\right]u^T=\nonumber
\\
&(1+\tau)R_{A_{w^*}}(\beta)-\tau R_O(\beta).
\end{align}
Since
\begin{align*}
&R_{A_{w^*}}(\beta)=\E\left[\left(X^{A_{w^*}}\beta-Y^{A_{w^*}}\right)^2\right]=\E\left[u\left(\epsilon+\sum_{i=1}^kw^*_iA_i\right)\left(\epsilon+\sum_{i=1}^kw^*_iA_i\right)^Tu^T\right]
=\sum_{i=1}^k(w^*_i)^2R_{A_i}(\beta),
\end{align*}
we may plug this back into \eqref{supeq2} and get
$$ \sup_{\tilde{A}\in C^{1+\tau}_w}R_{\tilde{A}}(\beta)=(1+\tau)\sum_{i=1}^k(w_i)^2R_{A_i}(\beta)-\tau R_O(\beta)=\frac{1}{2}R_+(\beta)+\frac{1+2\tau}{2}R_\Delta(\beta).$$



The proof of the formula \eqref{supw}, given $w^*$ is the same as in Proposition \ref{suppropfixedw}.
...
Analogously to \eqref{supeq2} we have that 
