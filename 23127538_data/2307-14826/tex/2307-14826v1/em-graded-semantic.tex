\pdfoutput=1
\documentclass[a3paper,UKenglish,cleveref,autoref]{lipics-v2021}
\sloppy
...
\usepackage[author=anonymous,marginclue,footnote,draft]{fixme}
...
% Lipics wants us to give every author his own line, but this is way to messy.
\iffalse
\author{Jonas Forster}{Friedrich-Alexander-Universität Erlangen-Nürnberg,
  Germany}{}{}{}%{https://orcid.org/0000-0002-5050-2565}{Funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- project number 434050016}%SpeQT
\author{Lutz Schr{\"o}der}{Friedrich-Alexander-Universität Erlangen-Nürnberg,
  Germany}{}{}{}

\author{Paul Wild}{Friedrich-Alexander-Universität Erlangen-Nürnberg,
  Germany}{}{}{}

\author{Harsh Beohar}{University of Sheffield,
  United Kingdom}{}{}{}

\author{Sebastian Gurke}{Universität Duisburg-Essen,
  Germany}{}{}{}

\author{Barbara König}{Universität Duisburg-Essen,
  Germany}{}{}{}

\author{Karla Messing}{Universität Duisburg-Essen,
  Germany}{}{}{}
\fi

\author{Jonas Forster, Lutz Schr{\"o}der, Paul Wild}{Friedrich-Alexander-Universität Erlangen-Nürnberg,
  Germany}{}{}{}%{https://orcid.org/0000-0002-5050-2565}{Funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- project number 434050016}%SpeQT
\author{Harsh Beohar}{University of Sheffield,
  United Kingdom}{}{}{}%{https://orcid.org/0000-0002-5050-2565}{Funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- project number 434050016}%SpeQT
\author{Sebastian Gurke, Barbara K{\"o}nig, Karla Messing}{Universität Duisburg-Essen,
  Germany}{}{}{}%{https://orcid.org/0000-0002-5050-2565}{Funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- project number 434050016}%SpeQT

\authorrunning{J. Forster, L. Schr{\"o}der, P. Wild, H. Beohar, S. Gurke, B. K{\"o}nig, K. Messing}
...
\cite{DBLP:conf/fsttcs/SilvaBBR10}; it then equips $TF$-coalgebras
...
\subparagraph*{Contribution} By instantiating the expressivity
criterion of the graded semantics framework to Eilenberg-Moore
semantics, we show that it is sufficient to provide a set of modal
operators that separate the elements of $FX$, while the treatment
of~$T$ is automatically provided by the framework. Separation of~$FX$
is typically easy to ensure, justifying the slogan that
Eilenberg-Moore semantics essentially always admits an expressive
invariant logic. We state our results in quantalic generality,
allowing an instantiation to both the two-valued setting, where states
are either equivalent or not and formulae take binary truth values,
and to quantitative settings, where similarity of states is a
continuum and formulae may take intermediate values, for instance in
the real unit interval. We thus in particular cover notions of
\emph{behavioural distance}
...
\subparagraph{Related work} There has been a fair amount of work on
...
(e.g.~\cite{DBLP:conf/concur/KonigM18,DBLP:journals/lmcs/WildS22,kkkrh:expressivity-quantitative-modal-logics,DBLP:conf/csl/Forster0HNSW23}). In
recent work by some of the present
authors~\cite{bgkm:hennessy-milner-galois}, expressive logics for
coalgebraic trace-type behavioural distances have been approached by
setting up Galois connections between logics and distances. This
concept is highly general but requires a comparatively high amount of
effort for concrete instantiations, again contrasting with our present
results, whose instantiation typically requires only minor checks, if
any.
...
GC\xrightarrow{Gc_n}G^{n+1}1$. States $x,y\in C$ are
\emph{finite-depth behaviourally equivalent} if $c_n(x)=c_n(y)$ for
all $n<\omega$. For finitary set functors, finite-depth behavioural
...
\begin{definition} A (commutative unital) \emph{quantale}
  $\V = (V, \oplus, k, \leq)$ consists of a set $V$ that carries both
  the structure of a complete lattice $(V, \leq)$ and the structure of
  a commutative monoid $(V, \oplus, k)$ such that for all $v \in V$,
  the operation ${-} \oplus v$ is join-continuous; that is,
  \begin{equation*}\textstyle
    \left(\bigvee_{i\in I} u_i \right)\oplus v = \bigvee_{i\in I} \left(u_i \oplus v\right)
  \end{equation*}
...
every $b\in V$, the map ${-} \oplus b $ has a right adjoint
$[b, {-}]$, with defining property
\begin{equation*}
  a \oplus b \leq c  \Leftrightarrow a \leq [b,c].
\end{equation*}
...
\begin{definition}
  A \emph{$\V$-category} is a pair $(X, d_X)$ where $X$ is a set and a
  function $d_X\colon X\times X\to V$ such that for all
  $x, y, z \in X$ we have $d_X(x, y) \oplus d_X(y, z) \leq d_X(x, z)$,
...
generalized metric spaces.  The carrier of the quantale $\V$ itself
has the structure of an object in $\dpmet_\V$, where
$d(x, y) = [x, y]$ for all $x, y \in \V$. It may also be viewed as an
element in $\met_\V$ through symmetrization:
...
\begin{example}\label{expl:quantales}
  \begin{enumerate}[wide]
  \item\label{item:two} The lattice $2=\{\bot,\top\}$ carries a quantale
    $\mathbf{2} = (2, \wedge, \top, \leq)$. In this case, $[b,c]$ is
    just the Boolean implication $b\to c$. The category
    $\pmet_\mathbf{2}$ is isomorphic to the category of equivalence
    relations, while the category $\met_\textbf{2}$ is isomorphic to the
    category of sets and functions. We use this quantale to cover two-valued
    equivalences, used in situations where one is only interested in determining
    whether states behave in precisely the same way or not.
  \item We use the quantale $[0,1]_\oplus = ([0,1], \oplus, 0, \geq)$,
    ...
    is truncated subtraction ($[b,c]=\max(c-b,0)$). The category
    $\met_{[0,1]_\oplus}$ is isomorphic to the usual category of
    $1$-bounded metric spaces and non-expansive maps, while
    $\pmet_{[0,1]_\oplus}$ is isomorphic to the category of
    pseudometric spaces (that is, distinct elements may take distance
    $0$). Note that the ordering on the set $[0,1]$ is reversed
    compared to its natural ordering. This is necessary, since
    otherwise $\oplus$ does in general not distribute over the empty
    join.
	\end{enumerate}
\end{example}
We will use the concept of initiality (in the concrete case of $\V$-categories)
...
evaluations morphisms of formulae form an initial source.
\begin{definition}

	A source $\mathfrak{A}$ of $\V$-functors $f_i\colon (X, d_X) \to (Y_i,
	d_{Y_i})$ is \emph{initial}, if for all $x, y \in X$ it holds that $d_X(x, y) =
	\bigwedge_{i \in I}d_{Y_i}(f_i(x), f_i(y))$.
...
$\mu\colon TT\Rightarrow T$, $\eta\colon\Id\Rightarrow T$ (the \emph{multiplication}
and the \emph{unit} of~$T$) satisfying natural laws. Monads on~$\Set$
...
variables into terms. A monad~$T$ is \emph{semiaffine} if $T1=1$. For
instance, the \emph{distribution monad} $\dist$, given by $\dist X$
being the set
$\{\mu\colon X\to[0,1]\mid f(x)=0\text{ for almost all $x\in X$, }
\sum_{x\in X}f(x)=1\}$ of finitely supported discrete probability
distributions on~$X$, is semiaffine.  Monads induce a natural notion of
...
$(A, a)$ to their carrier $A$.
The category $\alg{T}$ has all limits that $\Ccat$ has. We occasionally need the $n$-fold product of an algebra with itself $(A, a)^n$, the carrier of which is the $\Ccat$ object $A^n$. We denote its algebra structure by $a^{(n)}\colon T(A^n) \to A^n$.
...
\begin{definition}
  \label{def:em-laws}
  An \emph{EM distributive law}, or just \emph{EM law}, of a monad
  ...
  \end{equation*}
\end{definition}
It is well-known (cf.\ \cite{MULRY2002:LiftingAlgebras}) that EM laws
...
$\eta(c),\eta(d)\in TC$ are behaviourally equivalent in the
...
The standard powerset construction for determinizing nondeterministic automata is then recovered by the following EM law:
\begin{example}\label{ex:powerset-construction}
	In $\textbf{Set}$ (i.e. $\met_\textbf{2}$), let $T = \mathcal{P}$ be that powerset monad and $F = 2 \times {-}^\Sigma$. % Coalgebras for
  % $F$ are deterministic automata, while $FT$-coalgebras are
  % nondeterministic automata (without a designated initial state).
  The determinization $(\mathcal{P}C, c^{\#})$ of a $FT$-coalgebra
...
\begin{example}
  ...
  $\delta(t)(\sigma) = T(\lambda f. f(\sigma))(t)$.  We the have an
  EM law
...
\begin{definition}[Graded semantics]\label{def:graded-sem}
  ...
  \end{equation*}
\end{definition}
We think of $c^{(n)}$ as assigning to a state in~$C$ its behaviour
...
\begin{definition}[Behavioural distance]
...
\begin{example}
  ...
  semiaffine (Section~\ref{sec:monads}). Otherwise, the information erased
  ...
  concrete example where~$T$ is semiaffine, consider $T=\dist$ (the
...
\begin{example}
  All graded monads described in~\Cref{ex:gradedMonads} are depth-1.
\end{example}
...
\begin{definition}[Canonical algebras]
  For $i \in \{0,1\}$, let
...
\begin{lemma}\label{lem:canonical}(\cite[Lemma 5.3]{DBLP:conf/concur/DorschMS19})
 ...
 is a coequalizer diagram in the category of $M_0$-algebras:
	...
	\begin{tikzcd}
		M_1M_0A_0 \arrow[r,swap, "\mu^{1,0}", shift right] \arrow[r, "M_1a^{0,0}", shift left] & M_1A_0 \arrow[r, "\mu^{1,0}"] & A_1
	\end{tikzcd}
	\end{equation*}
\end{lemma}

\noindent Combining \Cref{def:depthone} with \Cref{lem:canonical}
...
\begin{itemize}
	...
	\item For each $\lambda\in \Lambda$ a $\Ccat$-morphism $\sem{\lambda}\colon G\Omega \to \Omega$ %(\Omega^m)\to \Omega$
\end{itemize}
%
...
\begin{itemize}
	...
	\item For $\lambda \in \Lambda$ we define $\sem{\lambda \phi}_c = \sem{\lambda} \cdot G\sem{\phi} \cdot c$ %\sem{\lambda} \cdot G\langle\sem{\phi_1}_c, \ldots, \sem{\phi_m}_c \rangle \cdot c$
\end{itemize}

The following definition of logical distance quantifies over all
formulae~$\phi$ of \emph{uniform depth}, meaning that all occurrences
of truth constants in~$\phi$ are under the same number of nested modal
operators. This is a mild restriction; in fact, for the above version
of coalgebraic logic, truth constants can always be modelled as
$0$-ary propositional operators, for which there is no uniformity
restriction. Uniform depth only plays a role once we talk about graded
logics, where propositional operators are additionally required to be
$M_0$-algebra homomorphisms, while truth constants are not. But even
then, we often impose the requirement that $M_01 = 1$, making all
morphisms algebra homomorphisms.

For the rest of this section, assume that $\Ccat$ is one of $\met_\V$,
...
\begin{enumerate}
\item For every $n$-ary $p\in \mathcal{O}$, $\sem{p}$ is an
...
homomorphism of $M_0$-algebras $(M_n1, \mu^{0,n})$ and $(\Omega, o)$,
...
\begin{equation*}
  \begin{split}
    &\sem{\phi}_c = \sem{\lambda} \cdot G\sem{\phi_1}_c \cdot c\\
...
property in the step for modalities.
  ...
  The proof uses uniformity to enable the factorization of
  formula evaluation via a single $M_n1$, which in general is possible
  only for uniform-depth formulae. We leave the question of whether
  non-uniform-depth formulae are in general invariant under the graded
  semantics as an open problem. Recall that when~$M_0$ is semiaffine,
...
\begin{definition}
  ...
  the family of maps $\{\llbracket c \rrbracket \mid c \in \Theta\}$
...
\begin{proof}
  ...
  of \Cref{prop:invariance}.  It suffices to show that the family of
  maps
  \begin{equation*}
	  \{\sem{\phi}_\mathbb{M}: M_n1 \to \Omega \mid \phi \text{ is
      a depth-$n$  $\mathcal L$ formula}\}
  \end{equation*}
  ...
  \begin{equation*}
    \{\llbracket \lambda \rrbracket(\llbracket \phi \rrbracket) \colon M_{n+1} \to \Omega \mid \lambda \in \Lambda, \phi \text{ a depth-$n$ formula}\}
  \end{equation*}
...
From now on, fix a set functor~$F$, a monad $T$, and an EM law
$\zeta\colon TF\to FT$. The objective of this section is to show that
behavioural equivalences, respectively metrics, on a determinized
coalgebra agree with the equivalences/metrics induced by the graded
semantics (Lemma~\ref{lem:CharTracesFinBehDistance}), and that graded
logics for $FT$ may be reduced to coalgebraic logics for~$F$. We
recall the notion of predeterminization under a graded
semantics~\cite{DBLP:conf/lics/FordMSB022} and show that this is the
same concept as determinization under an EM law, under the mild
condition that the monad $T$ is semiaffine.

\subsection{Predeterminization in Graded Monads}
\label{sec:traces-graded-monads}
...
$M_0$-algebra~$A$ to the free $M_1$-algebra over~$A$ (which is then
canonical). This gives rise to a functor
\begin{equation*}
...
\end{lemma}
\begin{proof}
	Let $(A, a)$ be a $T$-algebra. Then $\tilde{F}(A, a) = (Fa, Fa \cdot \zeta_A)$. On the other hand, by \Cref{lem:canonical}, the 1-part of the canonical algebra of $\mbar(A, a)$ is given by the following (split) coequalizer:
\begin{equation*}
\begin{tikzcd}
...
Commutativity of all relevant paths is obvious from the algebra and monad axioms, implying that the diagram is a
	coequalizer diagram by virtue of being a split coequalizer. Then $(A, FA, a, Fa \cdot \zeta_A, Fa)$ defines a canonical $M_1$-algebra where coequalization, as well as canonicity (due to \Cref{lem:canonical}), are by the above coequalizer, and homomorphy instantiates to the outer paths of the following diagram:
\begin{equation*}
\begin{tikzcd}
...
\end{equation*}
	...
	On morphisms $h\colon (A, a) \to (B, b)$, the lifting $\tilde{F}$ acts by sending $h$ to $Fh$. Commutativity of the relevant diagram making $Fh$ a $T$-algebra morphism between $FA$ and $FB$ is easily checked, as is the fact that $(h, Fh)$ constitutes a morphism between the canonical $M_1$-algebras.
\end{proof}
...
\end{lemma}
\begin{proof}
	This follows from the fact that $c^\#$ can equivalently be defined as the adjoint transpose of $c$ under the free-forgetful adjunction of $\alg{T}$ \cite{DBLP:conf/fsttcs/SilvaBBR10}. Then $c^\#$ and $c^\dagger$ agree by definition.
\end{proof}

\noindent For the rest of this section, assume that $\Ccat$ is one of $\met_\V$, $\pmet_\V$ or $\dpmet_\V$.

\begin{definition}
	Let $T\colon \Ccat \to \Ccat$ be a monad and $H\colon \alg{T}\to \alg{T}$ a functor on the corresponding Eilenberg-Moore category. Further, let $c\colon ((A, d_A),a) \to H((A, d_A), a)$ be an $H$-coalgebra. The \emph{finite-depth behavioural distance} of two states $x, y \in A$ is given by $d^{H}(x, y) = \bigwedge_{i\in \nat} d_{H^i1}(f_i(x), f_i(y))$, where the $f_i\colon A\to H^i 1$ are the projections into the final $H$-chain.\todo{P: find a better notation for this, maybe $d^H_\omega$? L: Add full definition of the final chain}
\end{definition}

\begin{lemma}\label{lem:CharTracesFinBehDistance}
  Let $(\alpha\colon G \to M_1, \mathbb{M})$ be a graded semantics on
  $\Ccat$ with~$M_0$ semiaffine, and let $(C, c)$ be a
  $G$-coalgebra. Then $d^{\mbar}(\eta(x), \eta(y)) = d^b(x, y)$ for
  all $x, y \in C$.
\end{lemma}
\begin{proof}
  One shows by induction on~$n$ that $f_n \cdot \eta = c^{(n)}$ for
  all~$n$, where semiaffinity is needed for the base case $n=0$.
\end{proof}

\begin{remark}
  In the case where the graded monad is $\mathbb{M}_\zeta$, if $T$ is
  semiaffine, then the final chain of $\mbar$ lives over the final
...
\begin{remark}
  As noted in \Cref{ex:gradedMonads}.\ref{item:emgm}, finite-depth
  ...
  % $c^\#$.
\end{remark}
From now on, we notationally conflate modalities $\lambda\in\Lambda$
...
%We know that this coequalizer is given by the algebra structure by split coequalization.
	...
	To show that $ev_\lambda$ is a homomorphism of $T$-algebras consider the following diagram:\todo{J: This part is not necessary if we can argue that the coequalizer is taken in Eilenberg-Moore}
...
Conversely, given a morphism $ev_\lambda \colon \tilde F(\Omega, o) \to (\Omega, o)$ it is straight forward to check that $\lambda =ev_\lambda \cdot Fo$ satisfies the laws necessary to make it an $M_1$-algebra main structure.
\end{proof}
...
criterion is typically very easy to established and can be shown for
general classes of functors, which is what we mean by our slogan that
expressive graded logics for EM sematics come essentially for free.
...
\begin{proof}
  ...
  the homomorphy square of $h$. Since~$a^{0,0}$ is a split epimorphism
  ...
  is spearating by depth-$1$ separation for $\mathcal{L}'$.
\end{proof}
...
\ev{\top} \}$ of of modalities for~$F$, where
...
\begin{proof}
  ...
  $U \colon \alg{T} \to \textbf{C}$ creates limits~\cite[Proposition
	20.12]{AdamekHerrlich90}.
\end{proof}
\begin{corollary}
	Let $\zeta$ be defined as in \Cref{ex:distributive-machine}.
...
\end{lemma}
\begin{proof}
  By \Cref{canonical}, canonical $M_1$-algebras have the form
...
\begin{example}[deadlock-free nondeterministic automata]
  ...
  distributive law $\zeta$ as in \Cref{ex:distributive-machine}.
  Since, by \Cref{lem:L_separating_for_machines}, $\mathcal{L}$ is
  depth-1 separating for $2 \times {-}^\Sigma$, we have that by
  \Cref{thm:main_F_separation} the logic $\mathcal{L}$ is expressive
...
\begin{example}[Reactive probabilistic automata]
  ...
  \Cref{ex:distributive-machine} then yields a semantics where,
  ...
  \Cref{lem:L_separating_for_machines} and
  \Cref{thm:main_F_separation}. The logic remains invariant w.r.t.\
...
\begin{example}[eactive probabilistic automata with black hole
  ...
  (\Cref{ex:nd-automata}). Let $\Omega = [0,1]$, equipped with the
  ...
  \Cref{ex:distributive-machine} with the law
  $\lambda\colon \dist(- + 1) \Rightarrow (\dist-) + 1$ that maps
  $\mu \in \dist(X+1)$ to $*$ iff $\mu(*) \not = 0$, and to $\mu$
  otherwise, where $*$ denotes the unique element in $1$. The
  ...
  making them valid modalities according to \Cref{thm:validOperators}.

  To verify expressivity, it is sufficient by
  \Cref{thm:main_F_separation} to prove separation of elements of
...
type~\cite{DBLP:conf/fsttcs/SilvaBBR10}. A leading example are
...
expressiveness. Our first main results establishes an overview of all
