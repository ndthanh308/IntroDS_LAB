...
\paperid{369}        % paper id for double blind submission
...
\author[A]{\fnms{Fernandes Montesuma}~\snm{Eduardo}\thanks{Corresponding Author. Email: eduardo.fernandes-montesuma@cea.fr\\ Accepted as a conference paper at the 26th European Conference on Artificial Intelligence.}}
\author[A]{\fnms{Ngole Mboula}~\snm{Fred Maurice}}
\author[A]{\fnms{Souloumiac}~\snm{Antoine}}
\address[A]{Université Paris-Saclay, CEA, List, F-91120 Palaiseau France}
...
\begin{table}
    ...
    \begin{tabular}{ccccc}
         \toprule
        Concept & Symbol & Classic DiL & WDL~\cite{schmitz2018wasserstein} & DaDiL (ours) \\
         \midrule
...
In \gls{ot}, there are at least 2 ways of integrating labels, either by penalizing \gls{ot} plans that transport mass between different classes~\cite{courty2016optimal,montesuma2021cvpr}, or by defining a metric in the label space~\cite{alvarez2020geometric}. We choose to integrate labels in the ground-cost,
\begin{equation}
...
\end{equation}
Henceforth we denote $\pi = \text{OT}\biggr((\mathbf{X}^{(P)},\mathbf{Y}^{(P)});(\mathbf{X}^{(Q)},\mathbf{Y}^{(Q)})\biggr)$. As a consequence, we can interpolate between two point clouds, since $\hat{\mathbf{y}}_{i}^{(P)}$ corresponds to a soft-label (i.e., probabilities). We use equations~\ref{eq:supervised_ground_cost} and~\ref{eq:supervised_barycenter} for proposing a new barycenter strategy between labeled point clouds, shown in algorithm~\ref{alg:wbt_fixed_point_joint}.

\begin{algorithm}[ht]
    \caption{Free-Support Wasserstein Barycenter of Labeled Distributions}
    \begin{algorithmic}[1]
...
\begin{table}
    \caption{Classification accuracy (in \%) of \gls{da} methods on the Office 31 benchmark. Each column represents a target domain for which we report mean $\pm$ standard deviation over $5$ folds.}
    \centering
    \begin{tabular}{lcccc}
        \toprule
        Method & $A$ & $D$ & $W$ & Avg\\
...
% In the previous section, we performed \gls{msda} using the optimal inferred weights for the target domain, i.e., $\alpha_{T}=\alpha_{N_{S}+1}$. Now, we explore how general interpolations of atoms can be used for \gls{msda}, namely, the Wasserstein hull $\mathcal{M}(\mathcal{P})$ (see definition~\ref{def:wbary}), and the ensembling space $\mathcal{H}(\mathcal{P}) = \{h_{\alpha} = \sum_{k=1}^{K}\alpha_{k}\hat{h}_{k}:\alpha \in \Delta_{K}\}$. In figure~\ref{fig:dadil_interpolation_space}, we study the reconstruction loss and the classification accuracy of \gls{dadil}-E/R for weights in $\Delta_{K}$. We choose $K=3$ for better visualization.

% First, in some cases, the atoms themselves provide good predictors for the target domain (e.g., column 2, rows 2 and 3 in figure~\ref{fig:dadil_interpolation_space}), which implies that \gls{dadil} \emph{learns distributions} that can discriminate between classes in the target domain (i.e., low $\lambda_{k}$). Second, the reconstruction loss is correlated to the performance of \gls{dadil}-R and E, as regions with lower reconstruction error tend to adapt better. This remark agrees with our theoretical discussion (e.g., eq.~\ref{eq:bound_redko_otda}), and is illustrated in figure~\ref{fig:corr_acc_loss}, for $K \in \{3,\cdots,8\}$.

% Next, as theorems~\ref{thm:redko_dadil_r} and~\ref{thm:dadil_guarantee}, the classification performance depends on the reconstruction loss for a choice of $\alpha$. In figure~\ref{fig:corr_acc_loss} we analyze the correlation of between these two terms for $\alpha \in \Delta_{3}$, which shows that for \gls{dadil}-R and E, \gls{da} performance is moderately correlated with reconstruction loss.
...
% To give an overview of the performance of interpolations in the latent space we show in figure~\ref{fig:interpolation_avg} box plots corresponding to the adaptation performance of \gls{dadil}-E and \gls{dadil}-R on the Caltech-Office 10 benchmark, for each target and arbitrary $\alpha$. For each domain, $\alpha_{T}$ (dotted red line) is above average for most number of atoms $K$.
        ...
        \FOR{$it=1\cdots,N_{iter}$}
            \FOR{$batch=1,\cdots,M$}
                \FOR{$\ell=1,\cdots,N_{S}$}
                    \STATE Sample $\mathbf{X}^{(Q_{S_{\ell}})} = \{\mathbf{x}_{i_{m}}^{(Q_{S_{\ell}})}\}_{i_{m}=1}^{n_{b}}$
                \ENDFOR
                \STATE sample $\mathbf{X}^{(Q_{T})} = \{\mathbf{x}_{i_{m}}^{(Q_{T})}\}_{i_{m}=1}^{n_{b}}$
