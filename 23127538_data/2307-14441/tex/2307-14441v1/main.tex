...
\renewcommand{\arraystretch}{1.5}
    ...
    \begin{tabular}{|c|c|c|c|c|c|}
      \hline\hline
      \textbf{Theorem} & \textbf{Algorithm} & \textbf{Measurement} & \textbf{Complexity} & \textbf{Notes} \\
      \hline
      \thm{hs-ht} & Hamiltonian simulation & Hadamard test & $\Or(T^3/\epsilon^2)$ & Early fault-tolerant \\
      \hline
      \multirow{2}*{\thm{hs-ae}} & \multirow{2}*{Hamiltonian simulation} & Biased amplitude estimation & $\Or(T^3/\epsilon)$ & Fault-tolerant \\
      \cline{3-5}
      &  & Unbiased amplitude estimation & $\Or(T^{2.5}/\epsilon)$ & Fault-tolerant \\
      \hline
      \thm{lode-ae} & Quantum linear ODE solver & Amplitude estimation & $\Or(T^2/\epsilon)$ & Non-unitary relaxation \\
      \hline
      \thm{node-ae} & Quantum Carleman linearization & Padding, amplitude estimation & $\Or(\Gamma T/\epsilon)$ & Low-rank, linearization \\
      \hline\hline
    \end{tabular}
    }
...
There exist a quantum algorithm producing an observable approximating the cost functional $J(u)$ with error $\epsilon\in(0,1)$, succeeding with probability $1-\delta$, with query and gate complexities
\begin{equation}
...
\end{equation}
within precision $\epsilon$ with success probability at least $1-\delta$. 
...
We estimate $\Re{\bra{\psi(t_k)} O(t_k) \ket{\psi(t_k)}}$ based on the real Hadamard test~\cite[Section II]{LT22}. We introduce a random variable $X_k$ and set it to be $1$ when the measurement outcome is $0$, and set it to be $-1$ when the measurement outcome is $1$.
Similarly, we estimate $\Im{\bra{\psi(t_k)} O(t_k) \ket{\psi(t_k)}}$ based on the imaginary Hadamard test. We introduce a random variable $Y_k$ that depends in the same way on the measurement outcome.
...
\begin{equation}
\mathbb{P}\left(\abs{\wt{A}-\sum_{k=1}^{N_t}\omega_k \wt{f}_k}\ge \frac{\epsilon}{2}\right)\le 4 \exp\left(-\frac{N_s^2 \epsilon^2}{2 N_s\sum_{k=1}^{N_t} \omega_k^2}\right) = 4 \exp\left(-\frac{N_s \epsilon^2}{2 \sum_{k=1}^{N_t} \omega_k^2}\right).
...
\begin{equation}
\sum_{k=1}^{N_t} \omega_k^2=\Or(T/\log(1/\epsilon))=\Or(T).
\end{equation}
...
Since the cost for propagating $\ket{\psi(t_k)}$ is proportional to $\|H\|t_k =  \|H\|k\Delta t$, the total cost for evaluating $A$ is 
\begin{equation}
...
Given state $\ket{\psi}$ and reflections $R_{\psi} = 2 \ket{\psi}\bra{\psi} - I$ and $R = 2P-1$, and any $\eta>0$, there exists a quantum algorithm that outputs $\wt a$, an approximation to $a = \bra{\psi} P \ket{\psi}$, so that
\begin{equation}
...
Given state $\ket{\psi}$ and projection operator $\Pi$ with $p = \|\Pi \ket{\psi}\|^2$. Given $t\ge1$ and $\epsilon \in (0,1)$. There exists a quantum algorithm that outputs $\wt p$, so that 
\begin{equation}
| \E[\wt p] - p | \le \eta, \qquad \text{and} \qquad \V(\wt p) \le \frac{91p}{r^2} + \eta.
\end{equation}
The algorithm needs $O(r\log\log(r)\log(r/\eta))$ uses of the reflection operators $I - 2 \ket{\psi}\bra{\psi}$ and $I-2\Pi$, with probability at least $1-\eta$.
\end{lemma}
...
There exist quantum algorithms producing an observable approximating the cost functional $J(u)$ with error $\epsilon\in(0,1)$, succeeding with probability $1-\delta$, with query and gate complexities\\
(i) using Biased Amplitude Estimation: 
\begin{equation}
\Or\Bigl( \frac{\|H\|T^3\log(1/\delta)}{\epsilon} \Bigr);
\end{equation}
(ii) using Unbiased Amplitude Estimation: 
\begin{equation}
...
For the $k$-th iteration, we first propagate $\ket{\psi(0)}$ to obtain $U_k\ket{\psi(0)} = \ket{\psi(t_k)}$ by simulating $U_k = e^{-iHt_k}$, with cost $\wt O(\|H\|k\Delta t)$. We then perform the amplitude amplification and estimation for the correlation function
\begin{equation}
\braket{O}_{t_{k}} = \bra{\psi(t_k)} O(t_k) \ket{\psi(t_k)} = \bra{\psi(t_0)} U_k^{\dagger} O(t_k) U_k \ket{\psi(t_0)}.
\end{equation}

In all, we need to repeat quantum simulations with cost $\Or(\sum_{k=1}^{N_t} \|H\|k\Delta t)$. The total time complexity of producing such observables equals to the \emph{product} of the simulation cost and the measurement cost. 
...
We estimate $f_k$ using the standard amplitude estimation. It suffices to take $r = \Or(1/\epsilon)$ in \lem{nae} to obtain $\wt a$ that is $\epsilon$-close to $a$.
To ensure that $\sum_{k=1}^{N_t}f_k$ in \eq{quadrature-sum} can reach the precision $\epsilon$, it suffices to perform to estimate each $f_k$ within $\epsilon/T$, with
\begin{equation}
C_d = \Or(\frac{T}{\epsilon})
\end{equation} 
...
Using unbiased amplitude estimation in \lem{nuae}, each measurement of $f_k$ uses a circuit of depth 
\begin{equation}
...
\end{equation}
Since $|\omega_k \wt{f}_k^{(l)}|\le \omega_k\epsilon/\sqrt{T}$, we apply Hoeffding's inequality and obtain,
\begin{equation}
\mathbb{P}\left(\abs{\wt{A}-\sum_{k=1}^{N_t}\omega_k \wt{f}_k}\ge \frac{\epsilon}{2}\right)\le 4 \exp\left(-\frac{N_s^2 \epsilon^2}{2 N_s\sum_{k=1}^{N_t} \omega_k^2 \epsilon^2/T}\right)=4 \exp\left(-\frac{N_s T }{2\sum_{k=1}^{N_t}\omega_k^2 }\right).
\label{eq:hoeffding_ae}
\end{equation}
Now use $\sum_{k=1}^{N_t} \omega_k^2 =\Or(T)$, we can choose $N_s=\Or(\log(1/\delta))$ so that the success probability is at least $1-\delta$. The total cost is therefore
\begin{equation}
...
We can perform the quantum linear ODE solvers such as~\cite[Theorem 2]{BC22} to produce the history state 
\begin{equation}
...
\end{equation}
Here we use the fact that $g = \frac{\max_{t\in[0,T]}\|\Psi(t)\|}{\|\Psi(T)\|} = 1$ and $\frac{\|\psi_{\mathrm{in}}\|}{\max_{t\in[0,T]}\|\Psi(t)\|} = 1$.
...
\label{eq:matrixO}
\end{equation}
and we require that $\|O_{\text{sel}}\| \le \sum_{k=0}^{N_t} \omega_k \norm{O(t_k)} \le 1$, and $O_{\text{sel}}$ can be block-encoded by a unitary $U_{\text{sel}}$ as
\begin{equation}
...
\end{equation}
within precision $\epsilon/(N_t+1)$ with $N_t+1 = \Or(T\polylog(1/\epsilon))$, we need a circuit of depth 
\begin{equation}
C_d = \Or(\frac{N_t}{\epsilon}) = \Or(\frac{T}{\epsilon}).
\end{equation}
Here we can use the above standard estimator even it is biased, since we only measure it \emph{once} and avoid accumulating the biases.
The total cost for evaluating $A$ is 
\begin{equation}
...
\label{eq:QDCD}
\end{equation}
where $\ket{\psi}\in\C^n$ is a quantum state, $J\in\C$ is a classical cost function, $H \in \C^{n\times n}$ is a Hamiltonian, and $O\in\C^{n\times n}$ is the observable operator. We aim to develop quantum algorithm for the initial value problem of \eq{QDCD} to obtain the final state $J(T)$.
...
\end{aligned}
\end{equation}
Here $H(t)$ and $O(t)$ represent continuous Hermitian matrices with respect to $t$. We have access to unitaries that block encode $H(t)$ and $O(t)$ for all $t$, with $\|O(t)\|\le 1$. We assume each $O(t)$ is low-rank, i.e. 
$O(t)$ can be unitarily diagonalized as $O(t) = U^{-1}\Lambda(t) U$, $\Lambda = \text{diag}(\theta_{1},\ldots, \theta_{r}, 0, \ldots, 0)$, and $r = \polylog(n)$. Additionally, we are provided with a state preparation oracle that prepares the initial state $\ket{\psi_{\mathrm{in}}}$. Our objective is to estimate the value of $J$ with a desired precision of $\epsilon$, within a given time duration $T > 0$.
\end{problem}
...
\end{equation}
where we denote the operators $P\in\C^{1\times n^2}$ with
\begin{equation}
...
There exist a quantum algorithm producing an observable approximating the cost functional $J(u)$ with error $\epsilon\in(0,1)$, succeeding with probability $1-\delta$, with query and gate complexities 
\begin{equation}
\Or\Bigl( \frac{\|P\|(\|H\|+\|P\|)T\Gamma\log(1/\delta)}{\epsilon} \Bigr),
\end{equation}
where we denote
\begin{equation}
\Gamma \coloneqq \frac{\sqrt{|J(T)|^2+1}}{|J(T)|}
\label{eq:Gamma}
\end{equation}
\end{theorem}
...
\sum_{k=0}^{N_t}|k\rangle\Bigl[J^k|0\rangle|0\rangle + \sum_{j,l=1}^n\psi_j^k(\psi_l^{\ast})^k|j\rangle|l\rangle\Bigr] \\
&+ \sum_{k=N_t+1}^{2N_t+2}|k\rangle\Bigl[J(T)|0\rangle|0\rangle+\sum_{j,l=1}^n\psi_j(T)\psi_l^{\ast}(T)|j\rangle|l\rangle\Bigr]\Biggr\},
\end{aligned}
\end{equation}
which includes $N_t+1$ ($N_t = \Or(T\log(1/\epsilon))$) number of the final state $J = J(T)$ to boost the success probability. Here $J^k = J(t^k)$, $|\psi^k\rangle = \sum_{j=1}^n\psi_j^k|j\rangle = \sum_{j=1}^n\psi_j(t^k)|j\rangle$, $|\psi(T)\rangle = \sum_{j=1}^n\psi_j(T)|j\rangle$, and the normalizing constant is denoted by
\begin{equation}
Q = \sqrt{\sum_{k=0}^{N_t}(|J^k|^2 + 1) + \sum_{k=N_t+1}^{2N_t+2}(|J(T)|^2 + 1)}.
\end{equation}

According to \lem{kis}, we can solve the linear system \eq{linear_system_padding} with query complexity
\begin{equation}
...
Since the quantum linear ODE solver takes $\Or(\|P\|(\|H\|+\|P\|)T\Gamma)$,  the total cost for evaluating $A=J(T)$ is 
\begin{equation}
\Or\Bigl(\|P\|(\|H\|+\|P\|)T\Gamma \cdot C_d \cdot N_s  \Bigr) = \Or\Bigl( \frac{\|P\|(\|H\|+\|P\|)T\Gamma\log(1/\delta)}{\epsilon} \Bigr).
\end{equation}
\end{proof}

\textbf{Remark.} We notice that $\|P\|$ can be upper bounded by the Hilbert-Schmidt norm of $O$, i.e.
\begin{equation}
\|P\| \le \|O\|_{\text{HS}} \coloneqq \sqrt{\text{Tr}(O^2)}.
\end{equation}
If $O$ is explicitly low-rank, i.e. $O = \text{diag}(\lambda_{1},\ldots, \lambda_{r}, 0, \ldots, 0)$. In this case, the so-called few-body observable $O$ only acts locally on $k$ qubits of the system, with $k = \polylog(r)$. Then $P$ has at most $r$ nonzero entries in the row, and $\|P\| = O(r) = O(\polylog(n))$.

In more general, we just require the observable $O$ is low-rank, and it can be unitarily diagonalized as $O(t) = U^{-1}\Lambda(t) U$, $\Lambda = \text{diag}(\lambda_{1},\ldots, \lambda_{r}, 0, \ldots, 0)$ with $r = \polylog(n)$, and we are given a block encoding of $U$. Then we are able to exponentially fast-forward the quantum dynamics~\cite{ALWZ22}. In detail, we define
\begin{equation}
\ket{\phi(t)} = U\ket{\psi(t)},
\end{equation}
such that
\begin{equation}
\bra{\psi(t)}O(t)\ket{\psi(t)} = \bra{\psi(t)}U^{-1}\Lambda(t) U\ket{\psi(t)} = \bra{\phi(t)}\Lambda(t)\ket{\phi(t)}.
\end{equation}
So the quantum dynamics in \eq{low-rank-dense} can be reformulated as
\begin{equation}
\begin{aligned}
    \frac{\d }{\d t}\ket{\phi(t)} &= -iUH(t)U^{-1} \ket{\phi(t)}, \quad \ket{\phi(0)}=U\ket{\psi_{\mathrm{in}}},\\
    J &= \int_0^T \Bigl(\bra{\phi(t)}\Lambda(t)\ket{\phi(t)} + \frac{\mu}{2}u^2(t)\Bigr)  ~\d t.
\label{eq:explicit-low-rank-dense}
\end{aligned}
\end{equation}
where the classical dynamics $J$ remains unchanged. We then perform Carleman linearization to vectorize the explicit low-rank observable $\Lambda$ as $P$. with  $\|P\| = O(r) = O(\polylog(n))$. We refer to~\cite[Section 4]{ALWZ22} for the discussion of exponentially fast-forwardable quantum and non-quantum dynamics.
...
\end{equation} 
Here $T_{l}(x)$ is the $l$-th order Chebyshev polynomial, and $\delta$ is the Kronecker delta. When $M$ is large, the weights $\omega_k$ can be efficiently carried out using fast Fourier transform (FFT) (see e.g.,~\cite{Tre08}). We may use other efficient quadrature schemes, such as the Gauss-Legendre quadrature. However, we find that it is simpler to estimate the $2$-norm of the weights $\{\omega_k\}$ for the Clenshaw--Curtis quadrature needed for the tail bound: Using the fact that $\abs{T_{2l}(t_k)}\le 1$, we have
\begin{equation}
\sum_{k=0}^M \omega_{k}^2\le  \sum_{k=0}^M \frac{16}{M^2}\left(\sum_{l=0}^{M/2} \frac{1}{1-(2l)^2}\right)^2
\le  \frac{16}{M^2}\sum_{k=0}^M \left(\sum_{l=0}^{\infty} \frac{1}{(2l)^2-1}\right)^2=\frac{4(M+1)}{M^2}=\Or(M^{-1}).
\end{equation}

When $T=\Or(1)$, for simplicity assume that $\braket{O}_t$ is an analytic function in an open region including the interval $[0,T]$ on which $\abs{\braket{O}_t}\le C$ for some constant $C$. 
We may map the interval $[0,T]$ to $[-1,1]$ via a linear transformation. Then the error of Clenshaw--Curtis quadrature decreases exponentially in $M$. In other words, to achieve additive error $\epsilon$, number of quadrature points is $\Or(\log(1/\epsilon))$  For long-time integration, we may use a composite Clenshaw--Curtis quadrature (\cite[Theorem 4.3]{Tre08}). 
...
For \prb{qdcd}, we consider an instance of \eq{QDCD} with its Carleman linearization as defined in \eq{problem-kis}. 
Denote $\|P\| \coloneqq \max_{t\in[0,T]}\|P(t)\|$.
...
\wt O\Bigl(\|P\|(\|H\|+\|P\|)T\Gamma\cdot\polylog(1/\epsilon)\Bigr)
\end{equation}
query and gate complexities, where
\begin{equation}
\Gamma = \frac{\sqrt{|J(T)|^2+1}}{|J(T)|}
\end{equation}
...
\begin{equation}
\max_{t\in[0,T]}\|e^{At}\| = 1+2\|P\|\|Q^{-1}\| = O(\|P\|).
\end{equation}
When $P$ and $H$ (and hence $Q$) are time-dependent, we still have the above diagonalization for each $t$, and thus
\begin{equation}
\max_{t\in[0,T]}\|e^{At}\| = 1+2\max_{t\in[0,T]}\|P(t)\| = O(\max_{t\in[0,T]}\|P(t)\|).
\end{equation}

For the above ODE with a positive norm of the matrix exponential, we employ the quantum linear ODE solver with complexity discussed in~\cite{Kro22}
...
and define
\begin{equation}
C(A) \coloneqq \sup_{t\in[0,T]} \|\exp(At)\|.
\end{equation}
There exists a quantum algorithm that produces a quantum state $\epsilon$-close to the normalized solution with overall query complexity
\begin{equation}
\Or\Bigl(gT\|A\|C(A)\cdot \polylog(1/\epsilon)\Bigr),
\end{equation}
and gate complexity which is greater by polynomial factors.
\end{lemma}

Since $C(A) = O(\|P\|)$, $\|A\| = \|H\|+\|P\|$, and $g = 1$, there is a quantum algorithm for solving \eq{problem-kis} with query and gate complexity
\begin{equation}
\wt O\Bigl(\|P\|(\|H\|+\|P\|)T\Gamma\cdot\polylog(1/\epsilon)\Bigr).
\end{equation}
...
\begin{equation}
\Gamma = \frac{\sqrt{|J(T)|^2+1}}{|J(T)|} = \Theta(T).
\end{equation}
...
We compute the parameter $\Gamma$ as defined in \eq{Gamma}
\begin{equation}
\Gamma = \frac{\sqrt{|J(T)|^2+1}}{|J(T)|} = \Theta(1).
\end{equation}

Overall, we examine the time complexities of our algorithms for the two types of observables, as summarized in \tab{observables}. For (a) time-increasing observables, both the quantum linear ODE solver and the quantum Carleman linearization approaches with amplitude estimation can achieve the best scaling $\Or(T^2/\epsilon)$; For (b) time-oscillatory observables, the quantum Carleman linearization approach with amplitude estimation is superior to other approaches with complexity $\Or(T/\epsilon)$.
