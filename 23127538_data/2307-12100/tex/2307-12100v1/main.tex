\documentclass[prb,twoside,twocolumn,10pt,floatfix,showpacs,citeautoscript,]{revtex4-2}
...
\newacronym{sm}{SM}{Supplemental Material}
...
\newlabel{sfig:MAPI-parity}{{S1}{3}{Parity plots for energies, forces, and virials for training (top) and test (bottom) data for the \ce {MAPI3} model. A similar plot for the \ce {CsPbBr3} model is available in \cite {FraRosEri23}}{figure.1}{}}

\newlabel{sfig:CsPbBr3_mode_coordinate}{{S2}{4}{a) Distribution of the mode coordinate $Q_\text {M}$ (corsponding tilt-angles given in top x-axis) from large scale \gls {md} simulations at \SI {302}{\kelvin } in the NPT ensemble for \ce {CsPbBr3}. The histograms are collected from the same set of \gls {md} simulations, and are colored depending if the mode coordinate is around zero (cubic) or not (tetragonal). From the mode histogram we can see that there is a region between the cubic and tetragonal which is almost never visited in the \gls {md} simulations indicating a barrier between the two phases. b) Joint distribution over the energy and the mode coordinate, clearly indicating that the tetragonal phase has a different energy then the cubic at this temperature}{figure.2}{}}

\newlabel{sfig:CsPbBr3_distributions}{{S3}{5}{Distribution of the \gls {md} data and corresponding Gaussian fits for various temperatures for \ce {CsPbBr3}. The middle panel shows the alat lattice parameter, and the rightmost column the clat lattice parameter}{figure.3}{}}

\newlabel{sfig:MAPI_distributions}{{S4}{6}{Distribution of the \gls {md} data and corresponding Gaussian fits for various temperatures for \ce {MAPI3}. The middle panel shows the alat lattice parameter, and the rightmost column the clat lattice parameter}{figure.4}{}}
...
Here, we uncover the complete free energy landscape of two halide perovskites, \ce{CsPbBr3} and \ce{MAPbI3} by constructing \gls{mlp} that are both accurate and computationally very efficient on \glspl{gpu} \cite{FanWanYin22, FraWikErh2023}.
...
It achieved a \gls{rms} error of \SI{0.6}{\milli\electronvolt\per\atom} for energies, \SI{4.5}{\milli\electronvolt\per\atom} for virials, and \SI{38}{\milli\electronvolt\per\angstrom} for forces (see \gls{sm} for details).
...
The cell was constrained to remain orthorhombic throughout the simulations to simplify the analysis.
...
Again, we note that free energy landscape between the orthorhombic ($a^-a^-c^+$) and the tetragonal ($a^0a^0c^+$) phase is very flat.
The local free energy minimum closer to the center of Glazer space corresponds to the $a^0 a^0 c^+$ phase but with multiple domains separated by anti-phase boundaries.
...
For the lower (orthorhombic-to-tetragonal) transition we observe a continuous transition (\autoref{fig:CsPbBr3-MAPI-transitions}).
In the case of the upper transition, however, we find coexistence of the cubic and tetragonal phases, rather than a continuous transition in energy and lattice parameters.\footnote{
    The coexistence in \gls{md} simulations referred to here is due to the system switching between the two phases with time, i.e., the two phases \emph{do not coexist at the same time} in a simulation.
    We note that the width of the coexistence in temperature as it is observed here is affected by system size as well as the strength of the barostat.
    It should thus not be simply interpreted as an experimentally accessible quantity.
}

To quantify the character, a double Gaussian can be fitted to the histogram $p(x)$ of the energy and lattice parameters 
\begin{align}
\begin{split}
    p(x)
    &=
    \frac{g}{\sigma_\text{cub}\sqrt{2\pi}} \exp\left[-\frac{(x-\mu_\text{cub})^2}{2\sigma_\text{cub}^2}\right] \\
    &+
    \frac{1-g}{\sigma_\text{tet}\sqrt{2\pi}} \exp\left[-\frac{(x-\mu_\text{tet})^2}{2\sigma_\text{tet}^2}\right]
    \label{eq:double_gaussian}
\end{split},
\end{align}
where $\mu$ and $\sigma$ are the mean and standard deviation of the normal distributions, and $\text{cub}$ and $\text{tet}$ refer to the cubic and tetragonal phase respectively.
Here, $g$ represents the relative occurrence of the cubic phase.

The mean of the histograms changes continuously between the cubic and tetragonal in the temperature range from \SI{300}{\kelvin} to \SI{305}{\kelvin} (\autoref{fig:CsPbBr3-MAPI-transitions}; see \autoref{sfig:CsPbBr3_distributions} in the \gls{sm} for the complete data).
This is due to the coexistence of the two phases, i.e., the mixing parameter $g$ changes continuously, rather than the structure changing continuously.
The mode of the histogram, i.e., the point at which the probability is maximized and thus corresponds to thermodynamic equilibrium, changes discontinuously at \SI{302}{\kelvin}, revealing a latent heat of about \SI{0.5}{\milli\electronvolt\per\atom} and a discrete change in lattice parameter.
These findings are consistent with a first-order transition with a small free energy barrier separating the phases as the system can jump between the two phases on the size and time scale of the \gls{md} simulations.Furthermore, the mode coordinate of the M mode (\autoref{sfig:CsPbBr3_mode_coordinate}) clearly demonstrates a barrier between the cubic and tetragonal phases.

% Figure environment removed

Similar analysis for the orthorhombic-to-tetragonal transition does not reveal any discrete change in thermodynamic properties, indicating a continuous transition.
...
This is also consistent with our recent work on the phonon dynamics in this material \cite{FraRosEri23}, where the M-mode frequency in the cubic phase was observed to be non-zero at the tetragonal-to-cubic transition temperature.
...
On heating, starting from the orthorhombic ground-state structure ($a^-a^-c^+$), a transition occurs at around \SI{150}{\kelvin} to a tetragonal structure ($a^-a^-c^0$ \footnote{
...
}) followed by another transition at \SI{300}{\kelvin} to the cubic phase.
...
The full distributions and corresponding fits are shown in \autoref{sfig:MAPI_distributions} of the \gls{sm}.
...
\textbf{Impact of molecular rotations.}
...
At finite temperature, the orientations the MA molecules fluctuate and can even flip at a significant rate even at modest temperatures \cite{Mattoni2015, Lahnsteiner2016dynamics}, which is how entropy commonly causes a smoothing of the \emph{free} energy landscape with increasing temperature.
...
The \gls{nep} model for \ce{MAPbI3} constructed in this study is openly available via Zenodo at \url{https://doi.org/10.5281/zenodo.8138960}.
...
\bibitem [{\citenamefont {Wiktor}\ \emph {et~al.}(2023)\citenamefont {Wiktor},
  ...
  {author} {\bibfnamefont {P.}~\bibnamefont {Erhart}},\ }\bibfield  {journal}
  {\bibinfo  {journal} {arXiv:2304.07402}\ }\href
  {https://doi.org/10.48550/arxiv.2304.07402} {10.48550/arxiv.2304.07402}
  (\bibinfo {year} {2023})\BibitemShut {NoStop}%
...
\bibitem [{Note1()}]{Note1}%
  \BibitemOpen
  \bibinfo {note} {The coexistence in \gls {md} simulations referred to here is
  due to the system switching between the two phases with time, i.e., the two
  phases \protect \emph {do not coexist at the same time} in a simulation. We
  note that the width of the coexistence in temperature as it is observed here
  is affected by system size as well as the strength of the barostat. It should
  thus not be simply interpreted as an experimentally accessible
  quantity.}\BibitemShut {Stop}%
\bibitem [{Note2()}]{Note2}%
  \BibitemOpen
  \bibinfo {note} {We refer to this phase as $a^-a^-c^0$ throughout this work
  to emphasize its relation to the other phases and its place in \protect
  \autoref {fig:glazer-space-free-energy}a. We note, however, that in Glazer
  notation it could also be written as $a^0b^-b^-$.}\BibitemShut {Stop}%
