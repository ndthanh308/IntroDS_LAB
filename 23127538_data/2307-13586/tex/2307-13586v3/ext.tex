...
$$
	\mathtt{Q}_h^{\star}(s,a) = \min_{\pi}\mathtt{Q}_h^{\pi}(s,a)
	\qquad \text{and} \qquad
	\mathtt{V}_h^{\star}(s)=  \min_{\pi}\mathtt{V}_h^{\pi}(s).
$$
Given the definitions above, 
we overload the notation $\mathsf{Regret}(K)$ to denote the regret for the cost-based scenario as 
$$
	\mathsf{Regret}(K) \coloneqq \sum_{k=1}^K \Big( \mathtt{V}^{\pi^k}_{1}(s_1^k) -  \mathtt{V}_1^{\star}(s_1^k) \Big).
$$
%
