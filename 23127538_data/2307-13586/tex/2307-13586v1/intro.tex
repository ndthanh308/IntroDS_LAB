...
% 
More concretely, imagine that one seeks to learn a near-optimal policy of a time-inhomogeneous MDP with $S$ states, $A$ actions, and horizon length $H$, and is allowed to execute the MDP of interest $K$ times and collect $K$ sample episodes each of length $H$. 
This canonical problem is among the most extensively studied in the RL literature, 
with formal theoretical pursuit dating back to more than 25 years ago (e.g., \cite{kearns1998near}). 
...
%
Moreover, $\widetilde{O}\left(\cdot \right)$, $\widetilde{\Omega}\left(\cdot\right)$ and $\widetilde{\Theta}\left(\cdot\right)$ are defined analogously, except that all logarithmic factors in $\mathcal{X}$ are hidden. 
} 
%
\begin{align}
...
Given that a regret of $HK$ can be trivially achieved (as the sum of rewards in any $K$ episodes cannot exceed $HK$), 
we shall often drop the $HK$ term and write
%
\begin{align}
	\text{(minimax lower bound)} \qquad 
...
which came up with a model-based algorithm called UCBVI that enjoys a regret bound $\widetilde{O}\big(\sqrt{SAH^3K} + H^3S^2A\big)$. 
...
\end{align}
%
The interested reader is referred to Table~\ref{tab:comparisons} for more details about existing regret upper bounds and the associated sample complexity. 
...
$\mathtt{ORLC}$\\\citep{dann2019policy} }& $\min\big\{\sqrt{SAH^3K}+S^2AH^4,\,HK\big\}$ & $[S^3AH^5,\infty)$ & $\frac{SAH^3}{\varepsilon^2}+\frac{S^2AH^4}{\varepsilon}$\\
  ...
  \hline  
				\makecell{$\mathtt{UCB}\text{-}\mathtt{M}\text{-}\mathtt{Q}$\\\citep{menard2021ucb}} & $\min\big\{\sqrt{SAH^3K}+SAH^4,\,HK\big\}$ & $[SAH^5,\infty)$ & $\frac{SAH^3}{\varepsilon^2} + \frac{SAH^4}{\varepsilon}$\\
  \hline 
...
The results provided here account for all $K\geq 1$ and/or all $\varepsilon \in (0,H]$. 
...
%
In truth, the incentive to minimize the burn-in cost and improve data efficiency arises in multiple other settings beyond online RL.  
...
%
The main hurdle stems from the complicated statistical dependency; 
for instance, in online RL, the estimated transition probabilities at one time step and the value function estimates of the next step are oftentimes statistically dependent (unless we waste data), while in contrast, there is no such dependency in the simulator setting (as it typically assumes samples are taken completely independently). 
...
More in-depth technical discussions will be provided in Section~\ref{sec:tec}.
...
one can design an algorithm (see Algorithm~\ref{alg:main}) obeying
%
\begin{equation}
	\mathsf{Regret}(K) \leq \widetilde{O}\left(\min\big\{\sqrt{SAKH^3},\,HK\big\}\right)
...
The algorithm proposed herein is a modified version of {\em Monotonic Value Propagation (MVP)}. 
Originally proposed by \citet{zhang2020reinforcement},  
the MVP method falls under the category of the model-based approach, 
...
The full proof of Theorem~\ref{thm1} will be postponed to Appendix~\ref{app:thmmain}.
...
This motivates a recent line of works that goes beyond worst-case regret to investigate optimal performance 
...
%
\begin{align}
	\mathsf{Regret}(K) \leq \widetilde{O}\left(\min\big\{\sqrt{SAH^2K v^*}+SAH^2,\, Kv^* \big\}\right)
	\label{eq:optimal-regret-value}
\end{align}
%
with probability at least $1-\delta$, where $v^*$ is the value of the optimal policy averaged over the initial state distribution (see \eqref{eq:defn-vstar-formal} for the formal definition of $v^*$).
\end{theorem}


There is also no shortage of applications where the use of a cost function is preferred over a value function \citep{agarwal2017open,allen2018make,lee2020bias,wang2023benefits}. For this purpose, we provide another variation based upon the optimal cost. 
%
\begin{corollary}[Optimal cost-dependent regret]\label{thm:cost}
%
For any $K \ge 1$ and any $0<\delta<1$, 
Algorithm~\ref{alg:main} achieves 
\begin{align}
	\mathsf{Regret}(K) \leq \widetilde{O}\left(\min\big\{\sqrt{SAH^2K c^*}+SAH^2,\, K(H-c^*) \big\}\right)
	\label{eq:optimal-regret-cost}
\end{align}
with probability exceeding $1-\delta$, where $c^*$ denotes the cost of the optimal policy averaged over the initial state distribution (to be formally defined in \eqref{eq:defn-cstar-formal}).
\end{corollary}
%
\noindent 
It is worth noting that: despite the apparent similarity of the statements of Theorem~\ref{thm:first} and Corollary~\ref{thm:cost}, 
...
\begin{itemize}
	\item 
		In the worst-case scenarios, the quantities $v^*$, $c^*$ and $\mathrm{var}$ can all be as large as the order of $H$, 
		in which case Theorems~\ref{thm:first}-\ref{thm:var} readily recover Theorem~\ref{thm1}. 
%
In contrast, the advantages of Theorems~\ref{thm:first}-\ref{thm:var} over Theorem~\ref{thm1} become more evident 
		in those favorable cases (e.g., the situation where $v^* \ll H$ or $c^* \ll H$, or the case when the environment is nearly deterministic (so that $\mathrm{var}\approx 0$)).

	\item 
Interestingly, the regret bounds in Theorems~\ref{thm:first}-\ref{thm:var} and Corollary~\ref{thm:cost} all contain a lower-order term $SAH^2$, 
and one might naturally wonder whether this term is essential. 
To demonstrate the unavoidable nature of this term and hence the optimality of Theorems~\ref{thm:first}-\ref{thm:var} and Corollary~\ref{thm:cost}, 
we will establish matching lower bounds, to be detailed in Section~\ref{sec:extensions}.
%
\end{itemize}
 %
...
%to make $\|V-V^{\mathrm{ref}}\|_{\infty}$ small enough, 
 %to learn such a reference value function $V^{\mathrm{ref}}$, existing methods need to estimate the optimal value function $V^*$ up to a non-trivial accuracy, 
...
%\begin{remark}
%In recent, first-order dependent regret bound has been proposed, where the leading term in the regret is  $\widetilde{O}(\sqrt{SAH^2 V^*})$ where $V^*$ is the optimal value function at the initial state. It is easy to observe that we need  $\Omega(1/V^*)$ episodes to learn an $V^*$-optimal policy in the worst case. And in the case $V^*<0.1$, the burn-in time is trivially $1$. 
%\end{remark}
