...
which will then be executed to collect new data. 
...
and hence the updating rule \eqref{eq:Qh-Vh-UCB-informal} will be invoked multiple times as well.  
...
\begin{algorithm}[H]
	\DontPrintSemicolon
\caption{Monotonic Value Propagation ($\mathtt{MVP}$)~\citep{zhang2020reinforcement}\label{alg:main}}
...
%\textbf{initialization: } 
	\For{$k=1,2,\ldots,K$} {
		Set $\pi^k$ such that $\pi_h^k(s) = \arg\max_{a}Q_h(s,a)$ for all $s\in \mathcal{S}$ and $h\in [H]$. {\color{blue}\tcc{policy update.}}
		\For {$h=1,2,...,H$} {
			Observe $s_{h}^k$, 
