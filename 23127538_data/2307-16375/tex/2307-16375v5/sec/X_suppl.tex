...
\begin{theorem}
    A subgraph $\mathbb{V}_i=\{\forall u\in \mathbb{V}: \textbf{\textit{P}}_{ui}=1\}$ is contiguous if and only if there exists $\textbf{\textit{Z}}_{vi}$ such that Equation~\eqref{eqn:method:order-preserving:1}, \eqref{eqn:method:order-preserving:2}, and \eqref{eqn:method:order-preserving:3} are satisfied.
\end{theorem}
