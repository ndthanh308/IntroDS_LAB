...
When the radius of a spherical collapsing matter is 
...
the UV modifications of monotonic dispersion relations.
In Sec.~\ref{sec:nonmonotonic},
we consider generic non-monotonic dispersion relations,
and find that,
when the dispersion curve $g(p)$
approaches $0$ faster than $1/p$ in the large momentum limit $p \rightarrow \infty$,
...
considering generic monotonic dispersion curves $g(p)$ that are subluminal outside the horizon, 
...
in order for a wave packet outside the horizon to exhibit \emph{subluminal} propagation, 
...
this should not be viewed as a pathology of the model.
...
%
As $g(p) \simeq p$ is assumed to be a good approximation for $p \lesssim M_p$,
...
\begin{align}
\label{eq:dxdv0}
\frac{dx}{dv} \simeq \frac{g'(p) x}{2 a} \, ,
\\
\frac{dp}{dv} \simeq - \frac{g(p)}{2a} \, .
\end{align}
They lead to
\be 
\label{eq:x_exp}
x \simeq \frac{\mbox{const}}{g(p)} \, .
\ee 
%
...
\begin{align}
    \phi_\omega (v, r \to \infty) &\sim \mathcal{N}_{\omega} \, 
...
%
Note that,
since $g'_{\omega} \simeq 1$ so long as $\omega \ll M_p$,
...
For instance, 
the roton dispersion relation has a local energy minimum,
whose analogue of Hawking radiation was considered in Ref.~\cite{Jacobson:1991gr}.
%
...
can be evaluated as
\begin{align}
\ev{b_{\Psi}^{\dagger} b_\Psi}{0} &\simeq \frac{4 \pi a}{e^{4\pi a \omega_0 / g_1'(\omega_0)} - 1} \, \int\frac{d\omega}{2\pi} \int\frac{d\omega'}{2\pi} \, f_{\omega_0}(\omega) f_{\omega_0}^*(\omega') \, e^{i (\omega - \omega') u_0} \int_0^{F(-\infty)} \frac{dF}{2 \pi} \, e^{-2 i a (\omega - \omega') F} \nn \\
...
&\simeq \frac{1}{e^{4\pi a \omega_0 / g_1'(\omega_0)} - 1} \, \int_{u_0 - u_{\Delta}}^{u_0} du \, \big\lvert \mathcal{F}\{ f_{\omega_0} \} (u) \big\rvert^2 \, , 
\label{eq:VEV_nonmono}
\end{align}
where 
\begin{align}
...
&\simeq 2a \int_0^{-M_p} dp \, \frac{p}{(2\omega_0 - p)^2} + 2a \int_{-M_p}^{-\infty} dp \, \frac{g(p)}{\left[ 2\omega_0 - g(p) \right]^2} \nn \\
&\simeq 2a\ln \left( \frac{M_p}{2\omega_0} \right) + 2a \int_{-M_p}^{-\infty} dp \, \frac{g(p)}{\left[ 2\omega_0 - g(p) \right]^2} \, .
\label{eq:turnoff_t}
\end{align}
...
\be 
u_{\Delta} 
\simeq 2a \ln \left(\frac{M_p}{2\omega_0}\right) + \frac{\pi}{n^2 \sin(\pi/n)} \, 2 a \left(\frac{M_p}{2\omega_0}\right)^{1 \, + \, 1/n} \, .
\ee 
...
for non-monotonic dispersion relations with $g(p)$ decaying faster than $1/p$ as $p \rightarrow \infty$.
...
%
This phenomenon, where a positive-$\omega$ wave packet is partially converted into a negative-momentum wave packet, is known as ``mode conversion''~\cite{Corley:1996ar}. 
...
It is then evident from power counting that $\abs{\Delta v}$ is finite if $g(p)$ decreases faster than $1/p$ as $p$ approaches infinity (see Fig.~\ref{fig:traj_inside}).
%
...
if the dispersion curve $g(p)$ approaches zero faster than $1/p$ as $p$ approaches infinity.
In such cases, 
...
E.T.A. is supported by the Foundation for the Advancement of Theoretical Physics and Mathematics ``BASIS'' grant, by RFBR grants 19-02-00815 and 21-52-52004, and by Russian Ministry of education
and science.
