...
\]
corresponding to the equilibrium point \(0\) that verifies its stability. If \eqref{e:stability csip} is \emph{stricly} feasible (a certain Slater's condition mentioned below holds), then we shall obtain a numerical solution to \eqref{e:stability csip} in the following fashion:
...
Feasibility of the problem \eqref{e:asymptotic csip} is subject to the choice of the Lyapunov triplet (see Remark \ref{r:choice} for a discussion). However, the solution \((\lyapCoeff[\ast], \marginCoeff[\ast]) \in \R[\basisDim] \times \R[\marginDim]\) to \eqref{e:asymptotic csip}, if it exits, readily produces a Lyapunov function
\[
	\lyapfn(y) \Let \sum_{i=1}^{\basisDim} \basisFunc(y) \lyapCoeff[\ast, i] \quad\text{for \(y\in\nbhd\)}
\]
...
\begin{theorem}
	...
	\begin{equation}
		...
		\nbhd \ni z \mapsto \lyapfn(z) \Let \sum_{i=1}^{\basisDim} \basisFunc(z) \lyapCoeff[\ast, i] \in \R[]
	\end{equation}
	is a Lyapunov function for \(0 \in \R[\sysDim]\) and \(0\) is asymptoically Lyapunov stable with the stability margin characterized by
    \begin{equation}
...
The sequence of remarks given below document a number of distinctive features of the results presented above.
...
\begin{proof}[Proof of Theorem \ref{t:stability}]
    We begin with showing that the semi-infinite program \eqref{e:stability csip} satisfies the hypotheses of \cite[Theorem 1]{ref:DasAraCheCha-22}.
    \begin{enumerate}[label=(\roman*), align=left, widest=B, leftmargin=*]
		\item The neibhourhood \(\nbhd\) plays the role of the constraint index set and is compact (in view of the condition \ref{d:nbhd} of the Lyapunov triplet).
        \item The assumption of the feasible set having non-empty interior satisfies the Slater's condition on \eqref{e:stability csip}.% guaranteeing that its feasible set has a non-empty interior.
        ...
        \item The constraint function in \eqref{e:stability csip} can be written as an inequality of the form
            ...
            \]
            where the map \(\constraintMap: \R[\basisDim] \times \domain \lra \R[2]\) is defined as
            \[
