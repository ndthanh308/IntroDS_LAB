...
\author{
Hongxiang Li$^{1}$, Meng Cao$^{2,1}$, Xuxin Cheng$^{1}$, Yaowei Li$^{1,3}$, Zhihong Zhu$^{1}$, Yuexian Zou$^{1}$\footnotemark[2]\\
$^{1}$School of Electronic and Computer Engineering, Peking University \\ $^{2}$International Digital Economy Academy (IDEA) \quad $^{3}$Peng Cheng Laboratory\\
{\tt\small \{lihongxiang, mengcao, chengxx, zhihongzhu, ywl\}@stu.pku.edu.cn; zouyx@pku.edu.cn}
}
...
If a query and a video moment have strong semantic correspondence, then they tend to cooperate with each other and contribute to the coalition. Thus, we can take $\mathcal{H}^i\!=\!\mathcal{H}_i^V \!\cup\! \mathcal{H}_i^Q$ as the players in the same game. 
...
\begin{align}
\!\mathfrak{I}\left(\left[\mathcal{H}^i_{x y}\right]\right)
\!&\!=\phi\left(\left[\mathcal{H}_{x y}^i\right] \mid \mathcal{H}^i \backslash \mathcal{H}_{x y}^i \cup\left\{\left[\mathcal{H}_{x y}^i\right]\right\}\right) \nonumber \\
\!&-\phi\left(\mathbf{h}_{ix}^{V} \mid \mathcal{H}^i \backslash \mathcal{H}_{x y}^i \cup\left\{\mathbf{h}_{ix}^{V}\right\}\right) \nonumber \\
\!&-\phi\left(\mathbf{h}_{iy}^{Q} \mid \mathcal{H}^i \backslash \mathcal{H}_{x y}^i \cup\left\{\mathbf{h}_{iy}^{Q}\right\}\right) \\
\!&\!=\underset{\mathcal{C}}{\mathbb{E}}\big\{{\substack{\mathcal{U} \subseteq \mathcal{H}^i \backslash \mathcal{H}^i_{x y} \\|\mathcal{U}|=\mathcal{C}}}{\mathbb{E}}[f\left(\mathcal{U} \cup \mathcal{H}^i_{ x y}\right) \!-\!f\left(\mathcal{U} \cup\left\{\mathbf{h}_{ix}^{V}\right\}\right) \nonumber \\
...
This paper provides a novel perspective for cross-modal contrastive learning. We argue that there exist similar issues for cross-modal tasks, \eg cross-modal pre-training.
% In the future, we intend to apply this idea to other cross-modal tasks, \eg cross-modal pre-training.
