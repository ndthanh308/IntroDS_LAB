...
\begin{equation}
\begin{split}
    \log \left(\frac{k_{+1}}{k_{ -1}}\right) =  \mu_{O}^0- \mu_{U_+}^0\\massimo.bilancioni
...
Finally, we mention in advance two more properties that  will be used in the analysis of the Maxwell demon. They come from the fact that the stochastic process depicted in Eq.~\eqref{eq:rate choice simplified inverter} is a 1D linear jump-process~\cite{gardiner2009}. Firstly, the relaxation rate of the system is given by
\begin{equation}
...
\subsection{Setup}
\label{sec:setup}
In this paragraph, we assign the rates to the Maxwell demon; we describe how the macroscopic limit is performed; we mention the parameters that will allow  to rewrite the analytical results in a compact form; and, finally, we outline the strategy adopted to analytically solve the Maxwell demon.\\
...
and thus the free energy consumed by the demon  is  $\propto\alpha$.
To calculate $J_{S_-\to S_+}$, we need to be more careful. As a matter of fact, we will see that this current is intensive, it does not scale with the volume. 
Therefore, to obtain its small value, an analogous approximation $\langle n_S\rangle \simeq n_S^*$ would be incorrect as $\langle n_S\rangle$ is needed with higher accuracy.
The procedure we will follow involves two steps: first, we show that we can link $J_{S_-\to S_+}$ and thus all the quantities characterizing the Maxwell demon performances to the  covariance $\text{cov}(n_S,n_D) \equiv \langle n_S n_D \rangle - \langle n_S \rangle\langle n_D \rangle$; secondly, we calculate the latter through a Gaussian approximation of the rate function. \\
 Note that a more straightforward approach in which one uses the Gaussian approximation to directly calculate $\langle n_S\rangle$ is wrong. It turns out that  in chemistry, contrary to electronics, the Gaussian approximation is not accurate
...
These connections allow us to address the scaling behavior of those quantities from the known scaling
\begin{equation}
...
\label{sec:info flow}
In Section~\ref{sec:qualitative thermo}, we presented the basic Maxwell demon thermodynamics. In this Section, we dive deeper by analyzing the Maxwell demon through the lens of information thermodynamics in the context of bipartite systems~\cite{PhysRevX.4.031015,article}. This framework  allows us to understand the thermodynamics of the chemical demon through a quantity called information flow. \\
Firstly, we state that our Maxwell demon is  bipartite in the following sense: it is composed of two degrees of freedom, which are the state of the system $n_S$ and the state of the demon $n_D$; in addition,  the possible chemical reactions change either $n_S$ or $n_D$ but not both at the same time.  This last property  enables a decomposition of the time derivative of the mutual information 
\begin{equation}
...
For bipartite systems, it is possible to derive two refined second laws valid for each subpart that take into account this quantity~\cite{PhysRevX.4.031015}. They read
\begin{equation}
    \begin{split}
...
Thus, we realize that $\frac 1V I_1(x)$ and the third order expansion of $I_0(x)$ give relevant contributions of order $1/V$. On the other hand, if we are interested in $\langle \delta x^2\rangle$, $P_{\text{Gauss}}(x)$ is sufficient as the next order correction gives zero contribution:
    ...
    \end{split}
\end{equation}
Notice that if thanks to the symmetry of the problem, $P_{\rm ss}(x)$ is known to be even with respect to $x^*$, then $I_1'(x^*) =I_0'''(x^*) = 0 $ and $P_{\text{Gauss}}(x)$ is accurate enough for calculating $\langle x\rangle$. This is what happens in the electronic Maxwell demon \cite{electronic}.
...
\begin{equation}
     \dot{\mathcal{I}} =-\dot{\mathcal{I}}_S =- \sum_{n_D,n_S} j_{ss}(n_S+1\leftarrow n_S|n_D)\log \frac{P_{\rm ss}(n_D|n_S+1)}{P_{\rm ss}(n_D|n_S)}.
     \label{eq: app info flow expression}
\end{equation}
