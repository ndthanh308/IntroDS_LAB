...
s  = @(t)  2*1/2 * (2/pi * atan(1/2 * (t-Tsim/2)) + 1);

y = @(x,y,t)  - 1/2 * ((x-s(t)).^2 + (y - s(t)).^2); % static map
...
sat = @(x) sqrt(w)*2./(1 + exp(-x)) - sqrt(w); % Smooth Saturation
...
% Perfect Seeker  forward velocity
       ...
       eps = 0.0001;
       c = 1;
       kt = @(x5,t) sqrt(eps + c*x5.^2);
       K1 = 1;
       K2 = 1;
    end

u1 = @(x5,t)  sqrt(w).*kt(K2*x5,t);

fp = @(t,x)     [ u1(x(5),t)  * cos(x(3));...
                  u1(x(5),t)  * sin(x(3));...
                  (w -   k/mu * (y(x(1),x(2),t) - x(4)) );
                   1/mu *(y(x(1),x(2),t) - x(4));
                   wl*( K1 * (1/mu * (y(x(1),x(2),t) - x(4))).^2 - x(5) )];
...
grid on;
if i == 2
tt = linspace(0,Tsim,100);
plot(s(tt),s(tt),'o','MarkerSize',5,'LineWidth',1,'color',[0,0,0])
end
...
% axis equal;
xlabel( '$t$','Interpreter','latex','FontSize', fs)
ylabel('$c \eta(t)$','Interpreter','latex', 'Fontsize',fs)
set(gca,'TickLabelInterpreter', 'latex')
...
plot(t,etadot,'LineWidth',2);
grid on;
% axis equal;
xlabel( '$t$','Interpreter','latex','FontSize', fs)
legend('$\frac{1}{\mu^2}(y-z)^2 $','$\dot{\eta}(t)$','Interpreter','latex', 'Fontsize',fs)
set(gca,'TickLabelInterpreter', 'latex')
end
