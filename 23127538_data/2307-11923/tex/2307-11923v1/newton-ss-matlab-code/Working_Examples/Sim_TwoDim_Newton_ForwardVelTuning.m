...
f = @(t,x) [ q*w^(p)*cos(k*t*w)*cos(t*w0) + c*w^(1-p)*sin(k*t*w)*cos(t*w0)*(F(x(1),x(2)) - x(3));...
             q*w^(p)*cos(k*t*w)*sin(t*w0) + c*w^(1-p)*sin(k*t*w)*sin(t*w0)*(F(x(1),x(2)) - x(3));...
                                                                           h*(F(x(1),x(2)) - x(3)) ];
...
f = @(t,x) [ q*w^(p)*cos(k*t*w)*cos(t*w0) + c*x(4)*w^(1-p)*sin(k*t*w)*cos(t*w0)*(F(x(1),x(2)) - x(3));...
             q*w^(p)*cos(k*t*w)*sin(t*w0) + c*x(4)*w^(1-p)*sin(k*t*w)*sin(t*w0)*(F(x(1),x(2)) - x(3));...
                                                                               h*(F(x(1),x(2)) - x(3));...
                      w_z*  (1  -   x(4) *x(5));...
                      -w_d* (x(5) - 8*k^2*w^(2-2*p)*cos(2*k*t*w)*(F(x(1),x(2)) - x(3))/q^2 )];
...
if i == 2
figure(2);
title('$\dot{d} = \omega_d (1-\gamma d)$','Interpreter','latex')
set(gca,'TickLabelInterpreter', 'latex')
% subplot(1,2,1);
set(gca,'TickLabelInterpreter', 'latex')
hold on;
