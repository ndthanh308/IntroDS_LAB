...
In this section, we prove \Cref{thm:smooth_cor_general}.  We begin with an intuitive overview of the proof and partially construct the relevant intermediate trajectories used to define our coupling in \Cref{sec:proof_construction}.  In Appendix \ref{app:prop_of_deconv_replica}, we prove several prerequisite properties of the construction given in \Cref{sec:proof_construction}.  Finally, in Appendix \ref{app:proof_smooth_cor_general} we formally construct the coupling and rigorously prove \Cref{thm:smooth_cor_general}.
...
\newcommand{\Qtilreph}[1][h]{\tilde{\lawW}_{\circlearrowleft,#1}}
...
\begin{lemma}[Key Error Decomposition] \label{lem:putting_couplings_together} Let $\coup$ respect the construction (in the sense of \Cref{sec:proof_construction}). Then
\begin{align}
&\Pr_{\coup}[\exists h \in [H]: \max\{\dists(\srep_{h+1},\shat_{h+1}),\phiis(\arep_h,\ahat_h)\} > \epsilon] \\
&\quad\le \Pr_{\coup}[\Qall^c] + \sum_{h=1}^H\Pr_{\coup}[ \Ballh^c \cap \Callbarh \cap \Ballbarh[h-1]]
\end{align}
Hence, letting $\inf_{\coup}$ denote the infinum over couplings $\coup$ which respect the construction,
\begin{align}
&\gapjointvec\,(\pihatsig \parallel \pirep) \vee \gapmargvec[\epsvecmarg](\pihatsig \parallel \pist)  \\
&\le \inf_{\coup} \left\{\Pr_{\coup}[\Qall^c] + \sum_{h=1}^H\Pr_{\coup}[ \Ballbarh^c \cap \Callbarh \cap \Ballbarh[h-1]]\right\} \label{eq:Gamimit_decomp}
\end{align}
\end{lemma}
\begin{proof} Define the events $\cE_h := \Callbarh[h+1] \cap \Ballbarh$. Observe that the events are nested: $\cE_{h} \supset \cE_{h+1}$, and that on $\cE_H$, we have that for all $h \in [H]$
\begin{align}
...
Thus, 
\begin{align}
&\Pr_{\coup}[\exists h \in [H]: \distavec(\srep_{h+1},\shat_{h+1}) \vee \distavec(\arep_h,\ahat_h)\} \not \preceq \epsvec] \le \Pr_{\coup}[\cE_H^c ] \le \Pr_{\coup}[(\Qall \cap \cE_H)^c] \label{eq:first_pr_coup_big}
\end{align}
As $(\srep_{1:H+1},\arep_{1:H}) \sim \Dist_{\pistrep}$, this shows $\gapjointvec(\pihatsig \parallel \pirep) \le \Pr_{\coup}[(\Qall \cap \cE_H)^c]$. Moreover, on $\Qall \cap \cE_H$, we have that
\begin{align}
\max_h \distsvec(\srep_h,\stel_h) \le \gamipsvec(2r),
\end{align}
so that, by the inequality preceeding \eqref{eq:first_pr_coup_big}, the following holds for all $h \in [H]$ on $\Qall \cap \cE_H$.
\begin{align}
\distsvec(\srep_{h+1},\shat_{h+1}) \vee \distavec(\arep_h,\ahat_h) \le \distsvec(\srep_{h+1},\shat_{h+1}) \vee \distavec(\arep_h,\ahat_h) \le \epsvec. \label{eq:first_pr_coup_second}
\end{align}
By construction, for each $h$, $\atel_h \mid \cF_h \sim \pistreph(\stel_h)$. Moreover, \Cref{lem:replica_property} implies that $\stel_h$ has the marginal distribution of $\sstar_h \sim \Psth$. Thus, for each $h$, $\stel_{h+1}$ and $\atel_h$ have the same \emph{marginals} as the marginals under $\Dist_{\pist}$. Consequently, \eqref{eq:first_pr_coup_second} implies that,
\begin{align}
\gapmargvec[\epsvecmarg](\pihatsig \parallel \pist) &:= \max_{h \in [H]}\max\left\{\inf_{\coup_1}\Pr_{\coup_1}\left[\distsvec(\shat_{h+1},\sstar_{h+1})\not \preceq \epsvec\right],\, \inf_{\coup_1}\Pr_{\coup_1}\left[\distavec (\seqast_h,\seqahat_h)\not \preceq \epsvec\right]\right\}\\
 &\le \Pr_{\coup}[(\Qall \cap \cE_H)^c].
\end{align}
where above we take $\inf$ over $\coup_1 \in \couple(\Dist_{\pihatsig},\Dist_{\pist})$. Summarizing our findings thus far,
\begin{align}
\gapjointvec\,(\pihatsig \parallel \pirep) \vee \gapmargvec[\epsvecmarg](\pihatsig \parallel \pist)  &\le \Pr_{\coup}[(\Qall \cap \cE_H)^c].
\end{align}
...
\paragraph{Proof strategy.} Our proof follows along similar lines as that of \Cref{prop:IS_general}, although with the added complication of including the smoothing.  We will inductively construct $\coup$.  A useful schematic for the construction at each step is the following diagram:
\begin{align}
