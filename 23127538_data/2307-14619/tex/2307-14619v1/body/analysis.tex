...
Our analysis abstracts away the vector-valued dynamics into a deterministic MDP with \bfemph{composite-states} $\seqs \in \cS$ and \bfemph{composite-actions} $\seqa \in \cA$, with dynamics
\begin{align}
...
Define the \bfemph{marginal error} \iftoggle{arxiv}{
  \begin{align}
  \gapmarg(\polhat \parallel \pist) := \max_{h \in [H]}\left\{\inf_{\coup_1}\Pr_{\coup_1}[\dists(\sstar_{h+1},\shat_{h+1}) > \epsilon],\inf_{\coup_1}\Pr_{\coup_1}[\phia(\seqast_h,\seqahat_h) > \epsilon] \right\}
  \end{align}
...
\iftoggle{arxiv}{
  ...
  \end{align}
}{$\drob(\polhat_h(\seqs) \parallel \polst_h(\seqs)) := \inf_{\coup_2}\Pr_{\coup_2}\left[\phia(\seqahat_h,\seqast_h) \le \epsilon \right]$,} where the infimum is over $(\seqast_h, \hat \seqa_h) \sim \coup_2 \in \couple( \bpolhat_h(\seqs),\bpol_h^\star(\seqs))$. 
...
\Cref{app:related} for further discussion).
...
\nipspar{Smoothing.} In Appendix \ref{app:no_augmentation}, we show that under the strong condition that the learned policy $\pihat$ is $\gamma$-TVC, then $\toda$ with no data soothing ($\sigaug = 0$) learns the distribution.  Frequently, however, $\pihat$ may not satisfy this condition, such as when the ground truth $\pist$ is not also TVC. We circumvent this by introducing a \emph{smoothing kernel} $\Wsig: \cS \to \laws(\cS)$ that corresponds to the data smoothing; in $\toda$ we let the kernel be a Gaussian, sending $\pathm$ to $\cN(\pathm,\sigma^2 \eye) \in \laws(\scrP_{\pathm})$. We will thus be able to replace TVC of $\pihat$ with TVC of $\Wsig$. We now introduce a few key objects.
...
The operator $\pi \circ \Wsig$ composes $\pi$ with the smoothing kernel. The deconvolution policy $\pidec$ captures the distribution of actions under $\pist$ given a smoothed state, and corresponds to $\pidec = (\pidech)_{h=1}^H$. We argue that if a policy $\pihat$ approximates $\pidec$ at each step, then $\pihat \circ \Wsig$ imitates $\pirep = \pidec \circ \Wsig$. We explain the ``replica policy'', and importance of imitating it, after we state our main theorem. First, we define a notion of stability to smoothing, taking $\disttvc,\dists,\dista$ as given.%(\mscomment{again, differing probabilistic variants to the appendix}. 
...
\begin{itemize}
  \item[(a)] The sequence $(\seqs_{1:H+1},\seqa_{1:H})$ is input-stable w.r.t $(\dists,\dista)$.
...
\subsection{A Guarantee in the Composite MDP Stability, and the derivation of \Cref{thm:main}}\label{ssec:gen_guarantee}
