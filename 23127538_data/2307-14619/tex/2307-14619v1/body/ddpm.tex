...
\begin{theorem}\label{thm:samplingguarantee}  Fix $1 \leq h \leq H$, let let $q$ denoted $\Daugh$, and suppose that $(\seqa_i, \pathm[h,i]) \sim q$ are independent for $1 \leq i \leq n$.   Suppose that the projection of $q$ onto the first coordinate has support (as defined in \Cref{def:support}) contained in the euclidean ball of radius $R \geq 1$ in $\rr^d$.  For $\epsilon > 0$,
    set
    \begin{align}
...
We substantially simplify the notation in this appendix to suppress all dependence on $h$.  In particular, we fix some $h \in [H]$ and consider $\pathm[h] \sim \mathcal{D}_{\sigma, h, [0]}$.  We further fix some $\sigma > 0$ and let $\qOU{t}$ denote the law of $\seqa \mid \pathm[h]$ according to  $\mathcal{D}_{\sigma, h, [t]}$ for the sake of notational simplicity.  Furthermore, to emphasize that our analysis of the statistical learning theory decomposes accross DDPM time steps, we denote by $\Theta_j$ the function class $\scoref_{\theta}(\cdot, \cdot, \dpstep \dpind)$.  We (redundantly) keep the dependence on $t$ in the function evaluation for the sake of clarity.  All other notation is defined \emph{in situ}.
