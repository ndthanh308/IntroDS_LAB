...
\begin{align}
    ...
    c_1 + \I_{0,\infty}\{\cE\},
\end{align}
where we $\cE = \{\max_{1\le k \le \tauc}\max\{\|\bbaru_{k}-\bbaru_{k}'\|,\|\bbarx_{k}-\bbarx_{k}'\|, \|\bbarK_{k}-\bbarK_{k}'\|\} \le c_2\}$,  $\I_{0,\infty}$ is the indicator taking infinite value when the event fails to hold, and $c_1$ and $c_2$ are given in \Cref{defn:constants_for_stability_stuff}. We note the following fact.
...
\end{fact}
\begin{proof} This  follows because $\distA$ only an additional indicator function which returns $0$ for $\dmax(\seqa,\seqa') \le \epsilon \le c_2$).
\end{proof}
