...
\begin{proposition}\label{prop:ips_instant_app}  Assume $\eta \le \Lstab/2$, and let  $\cbeta,\cgamma,\cxi,\cbarbeta,\cbargamma$ be the constants defined in \Cref{prop:affine_inc_stable}, which are polynomial in the quantities in \Cref{asm:Jacobian_Stable,asm:traj_regular}. In terms of these,  define
\begin{align}
\alpha &=  \cbarbeta (4\cbargamma \min\{\cgamma,\cbeta/2\cbargamma\} +  \min\{\cbeta,\cxi\})\\
c_1 &:= 4\cbargamma\cbarbeta (2+\alpha\Lstab+2\Rdyn) \\
c_2 &:= \max\{1,c_1)^{-1}\min\{\cgamma,\cbeta/2\cbargamma\}\\
c_3 &:= \frac{\Lstab \log(4/\cbarbeta)}{\eta}\\
c_4 &:= \min\{\cbeta,\cxi\}\\
c_5 &:= 2\cbeta
\end{align}
...
The appendix is organized as follows. \Cref{sec:stabilitiy_prelims} recalls preliminaris. \Cref{sec:stab_of_trajectories} states \Cref{sec:prop:master_stability_lem}, the guarantee that expert trajectories satisfy our notion of incremental stability provided \Cref{asm:Jacobian_Stable,asm:traj_regular} hold. \Cref{proof:of_prop_ips_instant} uses this guarantee to finally prove \Cref{prop:ips_instant_app}, based on a lemma whose proof is given in \Cref{proof:of_prop_ips_instant}. 
\Cref{sec:prop:master_stability_lem} proves \Cref{prop:affine_inc_stable}, based on a lemma whose proof is given in \Cref{sec:lem:state_pert}. 


\Cref{sec:ric_synth} shows how a synthesis oracle satisfying \Cref{asm:Jacobian_Stable} can be obtained from the Riccati equation, assuming \Cref{asm:traj_regular} and the dynamics admit stabilizable linearizations. Finally, \Cref{sec:recursion_solutions} gives the solutions to various scalar recursions used in the proofs throughout.
...
\begin{definition}[Control Path Regularity]\label{defn:control_path_regular} A control path $\ctraj = (\bx_{1:K+1},\bu_{1:K})$  is $(\Rdyn,\Lf,\Mf)$-regular if for all $k \in [K]$ and all $(\bx'_k,\bu'_k) \in \R^{\dimx} \times \R^{\dimu}$ such that $\|\bx'_k-\bx_k\| \vee \|\bu_k-\bu'_k\| \le \Rdyn$,\footnote{Here, $\| \nablatwo \feta(\bx'_t,\bu'_t)\|_{\op} $ denotes the operator-norm of a three-tensor.}
	\begin{align}
...
\begin{align}
 \cxi &= \frac{1}{4\Rstab\Bstab}\min\left\{1,\frac{1}{4\Lstab \Mdyn\Rstab\Bstab}\right\}\\
 \cbeta &= \min\left\{\frac{1}{96\Bstab\Mdyn\Rstab^2},\frac{\Rdyn}{32\Rstab}\right\}\\
...
\end{align}
Then, the controllers $\sfk_k(\bx) = \bbarK_k(\bx-\bbarx_k) + \bbaru_k$ are incrementally stabilizing in the sense of \Cref{defn:tiss} with moduli $\gammaiss(\cdot)$ and $\betaiss(\cdot,\cdot)$ and constants $ \cxi,\cbeta,\cgamma$ as above. 
\end{proposition}
...
\begin{lemma}[Trajectory-tracking via $\tiss$]\label{lem:iss_ips} Consider a given sequence $(\btilx_{t_h})$, and suppose that $\seqa_h = \sfk_{t_h:t_{h+1}-1}$ is local-$\tiss$ at $\btilx_{t_h}$ for each $1 \le h \le H$ (with parameters as in \Cref{defn:tiss}. Consider consistent trajectories $(\bx_{1:T+1},\bu_{1:T})$, $(\bx_{1:T+1}',\bu_{1:T})$ satisfying 
	\begin{align}
	\bu_{t} =  \sfk_{t}(\bx_{t}), \quad \bu_{t+1}' = \sfk_{t}'(\bx_{t}'), \quad \bx_{1}=\bx_1', \quad \max_{h}\|\btilx_{t_h}-\bx_{t_h}\| \le r\le \min\{\cxi,\cbeta\}
	\end{align}
	...
	\item[(b)]  Suppose that $\epsilon > 0$  satisfies \eqref{eq:eps_cond_general}, and that one of the following hold
	\begin{align}
	&\max_{1 \le t \le T}\sup_{\|\delx\| \le \delR}\|\sfk_t(\bx_t+\delx)-\sfk_t'(\bx_t+\delx)\| \le \epsilon, \quad  \alpha(\epsilon) := 2\betaiss(2\gammaiss(\epsilon),0), \quad \text{ or } \label{eq:alpha_iss_eq}\\
...
\begin{itemize} 
	\item Each action $\seqa_h$ satisfies our notion of incremental stability (\Cref{defn:tiss}) with moduli $\upgamma(\cdot),\upbeta(\cdot,\cdot)$, constants $\cgamma,\cxi,\cbeta$
...
Finally, let $\epsilon_0 > 0$ satisfy \eqref{eq:eps_cond_general}, that is:
	\begin{align}
	\gammaiss^{-1}(\betaiss(2\gammaiss(\epsilon_0),\tauc) \le \epsilon_0 \le \min\{\cgamma,\gammaiss^{-1}(\cbeta/2)\}, \label{eq:eps_cond_control}
	\end{align}
	...
	\begin{itemize}
		...
		\end{align}
		\item[(b)] For any $\rips \le \min\{\cbeta,\cxi\}$, $\pist$ is $(\rips,\gamipsone,\gamipstwo,\distips)$- restricted-IPS (\Cref{defn:ips_restricted}) with
		\begin{align}
...
\begin{align}
2\upphi(\tauc)\cbarbeta \le 1, \quad \epsilon_0 := \min\{\cgamma,\cbeta/2\cbargamma\},
\end{align}
...
\begin{itemize}
		...
		\item[(b)] For any $\rips = \min\{\cbeta,\cxi\}$, $\pist$ is $(\rips,\gamipsone,\gamipstwo,\distips)$- restricted-IPS (\Cref{defn:ips_restricted}) with
		\begin{align}
...
\begin{proof}[Proof of \Cref{prop:ips_instant_app}] Note that, as a consequence of \Cref{prop:affine_inc_stable}, we are in the regime of \Cref{cor:linear_scaling}, with $\upphi(k) = \left(1 - \frac{\eta}{\Lstab}\right)^{k-1}$ and $\epsilon_0 := \min\{\cgamma,\cbeta/2\cbargamma\}$. We note that, since $\eta \le \Lstab/2$, we can always lower bound
\begin{align}
\upphi(k) \le 2\left(1 - \frac{\eta}{\Lstab}\right)^{k} \le 2e^{-k\eta \Lstab}. \label{eq:phisimplif}
\end{align}
Hence, $2\upphi(\tauc)\cbarbeta \le 1$ for $\tauc \ge c_3= \frac{\Lstab \log(4/\cbarbeta)}{\eta}$.Next, we develop $\Distalpha$. Express the primitive controllers $\sfk_i = (\bbaru_i,\bbarx_i,\bbarK_i)$ and  $\sfk_i' = (\bbaru_i',\bbarx_i',\bbarK_i')$. 
...
\begin{align}
	...
	& \max_{1 \le t \le \tauc} \|\bbaru_t - \bbaru_t'\| +  \|\bbarK_t-\bbarK_t)\|({\alpha}+\Rdyn) + \Lstab\|\bbarx_t - \bbarx_t'\|) + \|\bbarK_t-\bbarK_t\|\|\bbarx_t'\| \tag{$\|\bbarK_t\| \le \Lstab$}\\
	&\le  \max_{1 \le t \le \tauc} \|\bbaru_t - \bbaru_t'\| +  \|\bbarK_t-\bbarK_t)\|({\alpha}+2\Rdyn + \|\bbarx_t-\bbarx_t'\|) + \Lstab\|\bbarx_t - \bbarx_t'\| \tag{$\|\bbarx_t\| \le \Rdyn$}\\
	&\le  \dmax(\seqa,\seqa')(1+\Lstab {\alpha}+2\Rdyn + \dmax(\seqa,\seqa'));
	\end{align}
	that is, we can take
	\begin{align}
	\Distalpha(\seqa,\seqa')= \dmax(\seqa,\seqa')(1+\Lstab {\alpha}+2\Rdyn + \dmax(\seqa,\seqa')).
	\end{align}
	Now, set $	 \alpha = 4\cbarbeta \cbargamma \epsilon_0 + \cbarbeta \rips = \cbarbeta (4\cbargamma \min\{\cgamma,\cbeta/2\cbargamma\} +  \min\{\cbeta,\cxi\})$. For $c_1 = 4\cbargamma\cbarbeta (2+\alpha\Lstab+2\Rdyn)$ and $c_2 = \max\{1,c_1\}^{-1}\min\{\cgamma,\cbeta/2\cbargamma\}$. Then if $\dmax(\seqa,\seqa') \le  c_2$, then,
	\begin{align}
	\Distalpha( \seqa,\seqa') \le \dmax(\seqa,\seqa')(2+\Lstab {\alpha}+2\Rdyn) \le \min\{\cgamma,\cbeta/2\cbargamma\}.
	\end{align}
	and, in particular, 
	\begin{align}
	 \distAbar( \seqa,\seqa' \mid \alpha) &\le 4\cbarbeta \cbargamma((2+\Lstab {\alpha}+2\Rdyn)\dmax(\seqa,\seqa')) = c_1\dmax(\seqa,\seqa')
	 \end{align}
	 Hence, unconditionally, 
	 \begin{align}
	 \distAbar( \seqa,\seqa' \mid \alpha) \le c_1 \dmax(\seqa,\seqa') \I_{\infty}\{\dmax(\seqa,\seqa') \le c_2\}
	 \end{align}
	 Thus, $\pi^\star$ satisfies $(\rips,\gamipsone,\gamipstwo,\distips)$- restricted-IPS (\Cref{defn:ips_restricted}) with $\rips = \min\{\cbeta,\cxi\} = c_4$
		\begin{align}
		...
		\end{align}
		Using \eqref{eq:phisimplif} and recalling $c_5 =2\cbarbeta $, we conclude  $(\rips,\gamipsone,\gamipstwo,\distips)$- restricted-IPS (\Cref{defn:ips_restricted}) with $\rips = \min\{\cbeta,\cxi\} = c_4$ and
		\begin{align}
		\distA( \seqa,\seqa') &= c_1 \dmax(\seqa,\seqa') \I_{\infty}\{\dmax(\seqa,\seqa') \le c_2\}\\
		\gamipsone(r) &= c_5 e^{\eta(\tauc-\taum)/\Lstab},\quad
		\gamipstwo(r) = c_5 r.
		\end{align}
...
\subsection{Proof of \Cref{lem:iss_ips}}\label{proof:lem:iss_ips}
	...
	The inequality $\|\bx_{t_h+i} - \bhatx_{t_{h},i}'\| \le \betaiss(r,i)$ is an imediate consequence of local-$\tiss$ of $\seqa_h$ at $\bhatx_{t_h,0}$. Let us prove $\|\bx_{t_h+i} - \bx_{t_{h}+i}'\| \le \betaiss(2\gammaiss(\epsilon),i) + \gammaiss(\epsilon)$. We begin with the following simplifying observation, which follows from considering the definition of local $\tiss$ with $\delu_t \equiv 0$ at time $t=0$:
	\begin{observation}\label{obs:simplify} $\betaiss(m,0) \ge m$ for any $m \in [0,\cbeta)$.
	\end{observation}
	Next, define $\delu_{t} = \sfk_t'(\bx_t') - \sfk_t(\bx_t')$ and $\delx_{t} = \bx_t'-\bx_t$. We begin by fixing a chunk $h$ and arguing along the lines of \citet[Proposition 3.1]{pfrommer2022tasil}.
	\begin{claim}\label{claim:fixed_h} Fix $c_0 > 0$. Suppose that, at a given step $h$, $\|\delx_{t_h}\| \le c_0 \le \cbeta$, and that $ \betaiss(c_0,0) + \gammaiss(\epsilon) \le \alpha$. Then, for all $0 \le i \le \tauc - 1$, $\|\delu_{t_h+i}\| \le \epsilon \le \alpha$ and
	\begin{align}
	\forall 0 \le i \le \tauc, \quad \|\delx_{t_h+i}\| \le \betaiss(c_0,i) + \gammaiss(\epsilon)
	\end{align} 
	\end{claim}
	\begin{proof} 
	We perform induction over $t \ge t_h$. Assume inductively that  $\|\delx_{t}\|  \le \betaiss(c_0,t - t_h) + \gammaiss(\epsilon) \le \alpha$ and $\max_{1 \le s \le t-1}\|\delu_s\| \le \epsilon$; note that this base case $t = t_h$ holds as $c) \le \betaiss(c_0,0) \le \alpha$ by \Cref{obs:simplify}  and $\{1 \le s \le t-1\}$ is empty. From  the inductive hypothesis and the condition \eqref{eq:alpha_iss_eq},
	\begin{align}
	...
	\end{align}
	Note that from part (a) of the lemma, 
	Note also that $\epsilon \le \alpha$, $\eqref{eq:alphatil_iss_eq}$ also suffices for the above to hold.  
	Hence, $\max_{1 \le s \le t}\|\delu_s\| \le \Delta_h$.  As  $\epsilon \le \cgamma$ and as $\|\bx_{t_h}- \bhatx_{t_h}\| \le \cxi$, the definition of incremental stabiity in \Cref{defn:tiss} implies
	\begin{align}
...
\subsection{Proof of \Cref{prop:affine_inc_stable} (incremental stability of affine gains)}\label{sec:prop:master_stability_lem}
 ...
 \begin{lemma}[Stability to State Perturbation]\label{lem:state_pert} Let $\ctrajbar   = (\bbarx _{1:K+1},\bbaru _{1:K}) \in \scrP_K$ be an $(\Rdyn,\Ldyn,\Mdyn)$-regular and feasible path, and let $\bK_{1:K}$ be gains such that $(\ctrajbar  ,\bK_{1:K})$ is $(\Rstab,\Bstab,\Lstab)$-stable. Assume, that $\Rstab \ge 1$, $\Lstab \ge 2\eta$. Fix  another $\bxoff_1$ and define another trajectory $\trajoff$ via 
	...
	\end{align}
	Then,  if 
	\begin{align}\|\bxoff_1 - \bbarx _1\| \le \frac{1}{4\Rstab\Bstab}\min\left\{1,\frac{1}{4\Lstab \Mdyn\Rstab\Bstab}\right\},
	\end{align}
	then 
	\begin{itemize}
...
\begin{align}
\min\left\{\frac{1}{16CL},\frac{C'}{8}\right\}& \ge \underbrace{\min\left\{\frac{1}{96\Bstab\Mdyn\Rstab^2},\frac{\Rdyn}{32\Rstab}\right\}}_{=\cbeta} \ge \epsilon_1\\
