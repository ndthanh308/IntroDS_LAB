...
In this section, we consider a generalization of the theory to allow for general, nonlinear primitive controllers, as long as they obey the incremental stability considered in \citet{pfrommer2022tasil}. To avoid measure-theoretic headaches, we consider controllers with finite-dimensional parameters
\begin{align}
\sfk(\bx) = \sfk(\bx;\theta), \quad \theta \in \Theta. \label{eq:sfk}
\end{align}
To avoid measure-theoretic headaches, we assume that $\Theta \subset \R^{d_{\Theta}}$ is a measurable subset of a finite dimensional space and (2) that   $\sfk(\bx;\theta)$ is jointly piecewise-Lipschitz with at most countably many pieces. We define composite actions just as for linear primitive controllers: 
\begin{align}
\seqa = (\sfk_{1},\sfk_2,\dots,\sfk_{\tauc}).
\end{align}
We view $\cK := \{\sfk(\cdot;\theta) | \theta \in \Theta\}$ and $\cA = \cK^{\tauc}$ as Polish spaces with the Euclidean metric on the controller parameters $\theta$ (resp. sequences of control parameters). Lastly, 
given a primitive controller $\sfk$, we define the closed loop dynamic map $\fclkap(\bx,\delu) := f(\bx,\sfk(\bx)+\delu)$. Thus, composite action $\seqa$ is \emph{consistent} with $(\bx_{1:\tauc+1},\bu_{1:\tau})$ if $\bx_{t+1} = \fclkap[t](\bx,\bzero)$ for $1 \le t \le \tauc$.

\paragraph{Incremental Stability.} We now replace the smoothness conditions on the dynamical system with a variant of incremental stability, first proposed by \cite{angeli2002lyapunov}.  We recall definitions of classes of comparison functions in nonlinear control \cite{khalil2002nonlinear}: we say a univariate function $\gammaiss:\R_{\ge 0} \to \R_{\ge 0}$ is \emph{\classK}~if it is strictly increasing and satisfies $\gammaiss(0) = 0$. We say a bivariate function $\betaiss:\R_{\ge 0} \times \Z_{\ge 0} \to \R_{\ge 0}$ is \emph{\classKL}~if $x \mapsto \betaiss(x,t)$ is \classK~for each $t \ge 0$, and $t \mapsto \betaiss(x,t)$ is nonincreasing in $t$. 

\begin{definition}[Time-Varying Incremental Stability]\label{defn:tiss} 
Let $\gammaiss(\cdot)$ be \classK, $\betaiss(\cdot,\cdot)$ be \classKL, and let $\seqa = (\sfk_{1},\sfk_2,\dots,\sfk_{\tau})$ denote a sequence of primitive controllers (i.e. a composite action when $\tau = \tauc$). Given a sequence of input  perturbations $\delu_{1:\tau} \in (\R^{\dimu})^{\tau}$ and initial condition $\bxi \in \R^{\dimx}$, let $\xa_{i+1}(\delu_{1:\tau},\bxi) = \fclkap[i](\xa_{i}(\delu_{1:\tau},\bxi),\delu_i)$,  with $\xa_1 = \bxi$. We say that composite action $\seqa$ is time-varying incrementally input-to-state stable ($\tiss$) with moduli $\gammaiss(\cdot),\betaiss(\cdot,\cdot)$ if 
\begin{align}
\forall 0 \le i \le \tau,  \quad \|\xa_{i}(\bm{0}_{1:\tau},\bxi)-\xa_{i}(\delu_{1:\tau},\bxi')\| \le \betaiss(\|\bxi-\bxi'\|,\tau) + \gammaiss\left(\max_{1 s \le i-1}\|\delu_s\|\right) \label{eq:tiss}
\end{align}
Given parameters $\cgamma,\cbeta,\cxi>0$ we say that $\seqa$ is local-$\tiss$ at $\bxi_{0}$ if the above holds for all $\bxi,\bxi',\delu_{1:\tau}$ such that $\|\bxi-\bxi_0\| \le \cxi$ and $\|\bxi-\bxi'\| \le \cbeta$, and $\max_{t}\|\delu_{t}\| \le \cgamma$. 
\end{definition} 
In words, our notion of incremental stability means that as the inital conditions $\|\bxi - \bxi'\| \to 0$ and $\max_{0 \le s \le i-1}\|\delu_t\| \to 0$, the trajectories induced by taking rolling out $\seqa$ from $\bxi$, and rolling out $\seqa$ from $\bxi'$ with additive input perturbations $\delu_{1:\tau}$ tend to zero in norm. This behavior needs only hold for initial conditions in a small neighborhood of a nominal state $\bxi_0$.
Importantly, the perturbations $\delu_{1:\tau}$ are fixed inputs and \emph{not } perturbations of the controller. The salient point about incremental stability is that it considers perturbations of the \emph{closed loop behavior} under the controllers. This notion of incremental stability are similar too, but sublty different similar notions of past work. We provide an extended comparisons in \Cref{sec:comparison_to_prior}. 

Using the above definition of incremental stability, 
we replace \Cref{asm:traj_regular,asm:Jacobian_Stable}, which describe dynamical regularity of the system and stability of the controllers along Jacobian linearizations, with the more general assumption that the primitive controllers satisfy local $\tiss$. 
\begin{assumption}\label{asm:tis} Let  $\gammaiss(\cdot)$ be \classK, $\betaiss(\cdot,\cdot)$ be \classKL,  and let $\cxi,\cbeta,\cgamma > 0$ be postive constants. We assume access to a synthesis oracle $\synth: \scrP_{T} \to \cA^{H}$ such that, with probability $1$ over $\ctraj_T = (\bx_{1:T+1},\bu_{1:T})\sim \Dexp$, $\seqa_{1:H} = \synth(\ctraj_T)$ satisfies the following properties:
\begin{itemize}
...
To state our main results, we begin by defining distances on primitive controllers, as well as the induced imitation error of a policy $\pihat$. Throughout,  $\Daugh$  denotes the noise-smoothed expert data distribution  over $(\pathmtil,\seqa_h)$ as  in \Cref{defn:Daugh} (with $\seqa_h$ now representing sequence of general primitive controllers, not necessarily linear).
...
\end{align}
where $\couphatsighbar$ denotes the set of couplings of $(\bx_{t_h},\pathmtil,\seqa_h,\seqa_h')$, induced by drawing $\ctraj_T \sim \Dexp$ with induced observation chunk $\pathm$ and state at time $h$ $\bx_{t_h}$, noising $\pathmtil \sim \cN(\pathm,\sigma^2 \eye) $, letting $\seqa_h$ be induced by $\seqa_{1:H} \sim \synth(\ctraj_T)$, and finally letting  $\seqa_h' \mid \pathmtil$ is distributed according to $\pihat_h(\cdot \mid \pathmtil)$.\footnote{Note that this coupling of $\seqa_h'$ with $\seqa_h$ is the only degree of freedom for in selecting elements of $\couphatsighbar$.} 
\end{definition}
...
\begin{align}
\gammaiss^{-1}(\betaiss(2\gammaiss(\epsilon),\tauc) \le \epsilon \le \min\{\cgamma,\gammaiss^{-1}(\cbeta/2)\} \label{eq:eps_cond_general}
\end{align} 
Define
\begin{align}
...
\end{align} 
Then, if $\sigma \le \min\{\cbeta,\cxi\}/2\upomega$ and $\gammaiss(\epsilon) \le 2\sigma$, we have
\begin{align}
...
\begin{proposition}\label{prop:TVC_main_general} Suppose \Cref{asm:traj_regular,asm:Jacobian_Stable} hold, and suppose that $\epsilon > 0$ and $\tauc \in \mathbb{N}$  satisfies \eqref{eq:eps_cond_general}.  Then, for any non-decreasing non-negative $\gamma(\cdot)$ and $\gamma$-TVC chunking policy $\pihat$,
\begin{align}
...
\end{proposition}
The proofs of \Cref{thm:main_template_general,prop:TVC_main_general} are given in the \Cref{app:gen_controllers_proofs}, generalizing the proofs of \Cref{thm:main_template,prop:TVC_main} in the main text, respectively.
...
\begin{itemize}
    \item Our notion of stability,  \Cref{defn:tiss}, applying to fixed-length sequences of controllers $\seqa = (\sfk_{1},\dots,\sfk_{\tau})$; past notions of incremental stability are for time-varying controllers and are infinite horizon.
