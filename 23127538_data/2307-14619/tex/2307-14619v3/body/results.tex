...
\begin{definition}[Problem constants]\label{defn:prob_constants_body} Throughout, we refer to constants $c_1,c_2,c_3,c_4,c_5 > 0$, which are polynomial in the terms in \Cref{asm:traj_regular,asm:Jacobian_Stable}, and which are defined formally in  \Cref{app:control_stability}.
\end{definition}
...
This section presents our main theoretical result: if one learns a chunking policy $\pihat$ that can compute the conditional distribution of composite actions at time steps given observation-chunks, then a stochastically smoothed version of this policy, $\pihat_{\sigma}$, has low imitation error. In what follows, given two composite actions $\seqa = (\bbaru_{1:\tauc}, \bbarx_{1:\tauc}, \bbarK_{1:\tauc})$ and $\seqa' = (\bbaru_{1:\tauc}', \bbarx_{1:\tauc}', \bbarK_{1:\tauc}')$, define the distance 
\begin{align}
\dmax(\seqa,\seqa') := \max_{1\le k \le \tauc}(\|\bbaru_{k}-\bbaru_{k}'\| + \|\bbarx_{k}-\bbarx_{k}'\| +\|\bbarK_{k}-\bbarK_{k}'\|). \label{eq:dmax}
\end{align}
We also define, for any length $\tau \in \N$, the \emph{trajectory distance} between two trajectories $\ctraj = (\bx_{1:\tau+1},\bu_{1:\tau}),\ctraj' = (\bx'_{1:\tau+1},\bu'_{1:\tau}) \in \mathscr{P}_{\tau}$ the trajectory distance
\begin{align}
...
Lastly, we define a per-timestep restriction of the expert distribution.
\begin{definition}[Expert Distribution]\label{defn:Daugh} For $h \in [H]$, let $\cDh$ denote the joint distribution of $(\seqa_h,\pathm)$, induced by drawing a trajectory  $\ctraj_T = (\bx_{1:T+1},\bu_{1:T}) \sim \Dexp$ from the expert distribution, $\sfk_{1:T} = \synth(\ctraj_T)$ be the associated primitive controllers, letting $\pathm = (\bx_{t_h - \taum+1:t_h},\bu_{t_h - \taum+1:t_{h}-1}$ be the associated observation-chunk at time $h$,  and $\seqa_h  = \sfk_{t_h:t_{h+1}-1}$ the associated composite action.  
\end{definition}


\paragraph{Simplified guarantee under total variation continuity.} Before stating our full result, we begin with a much simpler-to-state guarantee which requires that the learner policy satisfies a total variation continuity (TVC) condition, defined below.
\begin{definition}[TVC of Chunking Policies]\label{defn:tvc_main}  We say that a chunking policy $\pi = (\pi_h)$ is total variation continuous with modulus $\gamma: \R_{\ge 0} \to \R_{\ge 0}$, written $\gamma$-TVC, if, for all $h \in [H]$ and any observation-chunks $\pathm,\pathm' \in \scrP_{\taum-1}$,
\begin{align}
\TV(\pi_h(\pathm),\pi_h(\pathm')) \le \gamma(\dtraj(\pathm,\pathm')).
\end{align}
\end{definition}
...
\begin{align}
\Imitmarg[\epsilon](\pihat) \le H\gamma(\epsilon) +  \sum_{h=1}^H \inf_{\coup \in \mathscr{C}_h(\pihat)}\Pr_{(\seqa,\seqa') \sim \coup}[\dmax(\seqa_h,\seqahat_h) > \epsilon/c_1] \label{eq:TVC_main},
\end{align}
where the $\mathscr{C}_h(\pihat)$ denotes the set of couplings over $(\pathm,\seqa,\seqahat)$ where $(\pathm,\seqa) \sim \cDh$ and $\seqahat \mid \pathm \sim \pihat_h(\pathm)$.  
\end{proposition}
The above proposition reduces the marginal imitation error of $\pihat$ to the sum over optimal transport errors between $\pihat$ and $\seqa \mid \pathm$ chosen by the expert demonstrators. Thus, if these are small, the regularity of the dynamics and local stabilization properties of the primitive controllers guaranteed by \Cref{asm:traj_regular,asm:Jacobian_Stable} ensure that errors compound at most linearly in problem horizon. We note that the optimal transport error on the right hand side of \eqref{eq:TVC_main} is necessary because our expert is stochastic, and thus point error estimates do not suffice, and corresponds to a relaxed  L\'evy-Prokhorov metric \citep{strassen1965existence}. A formal proof of the above proposition is given  in \Cref{app:end_to_end} based on the template outlined in \Cref{sec:analysis}. 
...
\paragraph{The general guarantee via stochastic smoothing.} To circumvent assuming that the learner's policy is TVC,   we study estimating the conditionals under a popular data augmentation technique \cite{ke2021grasping}, where the learner is trained to imitate the conditional sequence of $\seqa \mid \pathmtil$, where $\pathmtil \sim \cN(\pathm,\sigma^2 \eye)$ adds $\sigma^2$-variance Gaussian noise to the true observation-chunk. This leads to a generalization of the optimal transport distance considered in \Cref{prop:TVC_main}, defined as follows. 
\begin{definition}[Policy Error]\label{defn:policy_error}  For a given chunking policy $\pihat = (\pihat_h)$,  let $\couphatsigh$ denote the set of couplings of $(\pathm,\pathmtil,\seqa_h,\seqahat_h)$, where $(\pathm,\seqa_h) \sim \cDh$, $\pathmtil \sim \cN(\pathm,\sigma^2 \eye)$ and $\seqahat_h \mid \pathmtil$ is distributed according to $\pihat_h(\cdot \mid \pathmtil)$. Define the policy error  at scale $\epsilon > 0$ and step $h$ as
\begin{align}
\Delta_{\sigma,h}(\pihat;\epsilon) &:=  \inf_{\coup \in \couphatsigh}\Pr_{(\seqa,\seqa') \sim \coup}[\dmax(\seqa_h,\seqahat_h) > \epsilon] 
\end{align}
\end{definition}
...
In words, this error  measures an optimal transport distance between  the distribution over $\seqa_h \mid \pathm$ under $\cDh$, and $\pihat$ applying to the noised observation-chunk $\pathmtil$. By appropriate choice of coupling, this distance can be made to be zero precisely when $\pihat$ is the composition of the distribution of $\seqa_h \mid \pathm$ with the deconvolution of $\pathm$ from $\pathmtil$. That is, the optimal $\pihat$ is one which learns to deconvolve the smoothing noise. 

Analogously, we define a \emph{smoothed} version of $\pihat$, which ads the same noise $\sigma$ back in at inference time.
...
\begin{align}
    ...
    \epsilon_2 = \epsilon + \sigma e^{-\frac{\eta (\tauc-\taum)}{\Lstab}} \iota(\epsilon), 
    \end{align}
    and where  $\iota(\epsilon) = 6c_5\sqrt{5 \dimx + 2\log\left( \frac{4\sigma}{c_1 \epsilon} \right)}$ is logarithmic in $1/\epsilon$.
\end{restatable}

\Cref{thm:main_template} guarantees imitation of the distribution of marginals and final states of $\Dexp$ by replacing the explicit TVC assumption with smoothing noise. Our imitation guarantee applies to the \emph{smoothed policy} $\pihat_{\sigma}$ which adds smoothing noise back in. The motivation is that adding noise at inference time \emph{removes distribution shift} coming from training on smoothed data; this simple observation is crucial for our theoretical guarantees, and differs from past work which does not smooth at inference time. 
...
\nipspar{Learning-Theoretic Assumptions.} We now state the assumptions required for our theoretical guarantees on \toda{}. We require access to a class of score functions rich enough to represent the following deconvolution conditionals. To define these, we introduce the following distribution
\begin{definition}[Smoothed Data Distribution]\label{defn:Daugh} For $h \in [H]$ and $\sigma > 0$, let $\Daugh$ denote the joint distribution of $(\seqa_h,\pathmtil)$, induced by letting  $(\seqa_h,\pathm) \sim \cDh$, letting $\pathmtil \sim \cN(\pathm, \sigma^2 \eye)$ augment $\pathm$ with noise. 
\end{definition}
...
\begin{theorem}\label{thm:main} 
 ...
 \end{align}where $\epsilon_1 = \tilde\cO(\epsilon + \sigma)$ and $\epsilon_2 = \tilde\cO(\epsilon + \sigma e^{-\Omega(\tauc-\taum)})$ and $\iota(\epsilon) = \tilde{\cO}(1)$ are as in \Cref{thm:main_template}. 
