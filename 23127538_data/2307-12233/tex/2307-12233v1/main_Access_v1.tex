...
\title{Adaptive Consensus-based Regulation of Open-Channel Networks}
\author{Marco Fabris\authorrefmark{1}, Marco D. Bellinazzi\authorrefmark{1}, Andrea Furlanetto\authorrefmark{2}, and Angelo Cenedese\authorrefmark{1}, \IEEEmembership{Senior Member, IEEE},
...
\begin{abstract}
This paper deals with water management over open-channel networks subject to water
height imbalance. Specifically, it is devised a fully distributed adaptive consensus-based algorithm
within the discrete-time domain capable of (i) providing a suitable tracking reference that stabilizes
...
solving a guidance problem that guarantees to steer the regulated network - represented as a closedloop
...
exponentially fast towards the average consensus without violating the imposed constraints over
...
Furthermore, within graph theory, it is common opinion (see \cite{10.1007/978-3-642-36071-8_1},\cite{10.1145/2678280}) that theoretical computer science lays on a vantage point for the understanding of key emergent properties in complex interconnected systems. In this perspective, we model water distribution problems as distributed networked systems.
...
Due to the existence of state constraints, most existing consensus algorithms cannot be applied directly (see also~\cite{7170935}); hence, a novel consensus algorithm is here proposed to face such a challenge relatively to OCNs. 
...
In Sec.~\ref{sec:preliminaries}, we introduce mathematical preliminaries and briefly review the consensus theory. Sec.~\ref{chp:modeldesign} describes the setup for which our consensus-based reference design protocol for even water compensation is proposed. 
...
In this section, the preliminary notation and assumptions to model OCNs are reported. 
...
We now provide an overview of the discrete-time weighted consensus problem in the field of multi-agent systems (MASs). Let us consider a group of $n$ homogeneous agents, e.g. the $n$ pairs of actuators installed at the two endpoints of each channel in a water system modeled by an undirected and connected (adjoint) graph $\Gmc$. 
...
As schematically shown in Fig.~\ref{fig:ControlSchemeCavallinoDraft}, such a scheme is formed by each inner feedback loop (in gray) determined by the $i$-th inner controller $K_{i}(s)$ and the $i$-th plant $\Pi_{i}(s)$ and an outer feedback loop (in black). 
...
%%%%%%%%%%%%%%%%
\subsection{A consensus-based reference design protocol}
In the sequel, we present the dynamics of the proposed iterative water level regulation scheme introduced in Sec.~\ref{ssec:setup} in order to find a solution to Problem \ref{problem}. 
Drawing inspiration from~\cite{fabris2019distributed}, we include memory in the classic consensus dynamics~\eqref{eq:LAP} to provide a weighted correction to current water level, thus leading to the so called \textit{reference design protocol}~(RDP)
%
\begin{align} 
...
\begin{remark}
	Leveraging the structure of objective \eqref{eq:costfunction}, dynamics in \eqref{eq:3} can be easily rewritten as 
	\begin{equation}\label{eq:updaterulesd}
		x(k+1) = x(k) - (1-\eta(k)) ~ \nabla J(x(k)).
	\end{equation}
	The expression \eqref{eq:updaterulesd} can be considered a steepest descent update rule \cite{Argyros2022} with adaptive step-size $(1-\eta(k))$ and descent direction $-\nabla J(x(k)) = (P-I_{n})x(k)$. Therefore, the proposed RDP \eqref{eq:3} can be seen as a distributed direct method to minimize the objective \eqref{eq:costfunction}.
\end{remark}
...
Here, a proper tuning for parameter $\eta(k)$ is derived to ensure \eqref{eq:7}-\eqref{eq:equi} during the execution of protocol \eqref{eq:3}. In this direction, the following preliminary statements are formulated.
...
\textbf{Download regime}: Under this condition, we provide a value for $\eta(k)$ that is denoted with $\eta_{D}(k)$. 
By considering the $i$-th equation of RDP~\eqref{eq:3} and substituting $x_{i}(k+1)$ into the download constraint \eqref{eq:7} one has:
\begin{equation}
	\eta_{D}(k)\left(\!\!x_{i}(k)-\sum_{j=1}^n p_{ij}x_{j}(k)\!\!\right) \geq x_{i}(k)-\sum_{j=1}^n p_{ij}x_{j}(k)-c_{i}^{D}(k).
	\label{eq:11} 
\end{equation}
%
In order to state an explicit relation for $\eta_{D}(k)$, we first observe that $(x_{i}(k)-\sum_{j=1}^n p_{ij}x_{j}(k))$ is strictly positive $\forall i = 1,\ldots,n$; this is proven by substituting $x_{i}(k+1)$, as expressed in RDP~\eqref{eq:3}, into the download regime characterization~$\delta x_{i}(k)<0$, and by exploiting the fact that $\eta_{D}(k) \in (0,1)$ will be guaranteed.
Hence, one obtains
%
\begin{equation}
...
\end{equation}
%
holds by the submultiplicative property of the infinite norm and Lem.~\ref{omeg}. It is thus possible to impose
%
...
On the one hand, $\eta_{L}$ represents a suboptimal choice for the value of $\eta_{D}(k)$ whenever $\eta_{L} \geq 1- c_{D}(k) / (\omega\left\|x(k)\right\|_{\infty})$, since it ensures fast convergence for the corresponding static\footnote{Namely adopting a constant $\eta(k)=\eta_{L}$ in RDP \eqref{eq:3}.} consensus protocol. 
...
\textbf{Upload regime}: Under this condition, we provide a value for $\eta(k)$ that is denoted with $\eta_{U}(k)$ with a similar reasoning as for the download regime. 
\\Let us consider the $i$-th equation of RDP \eqref{eq:3}. Substituting $x_{i}(k+1)$ into the download constraint \eqref{eq:8} one has:
\begin{equation}
...
\textbf{Selection of} $\eta(k)$: Combining the results obtained for the download and upload regimes, the choice yielded by $\eta(k) := \max(\eta_{D}(k),\eta_{U}(k))$ now appears straightforward in order to guarantee constraints \eqref{eq:7}-\eqref{eq:8} to holds for all $k=0,1,2,\ldots$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Distributed implementation of the proposed reference design protocol}
\label{chp:models}
In this section, we present the main contribution of this work, namely a distributed algorithm leveraging a time-varying consensus-based RDP to evenly compensate water levels, along with its convergence analysis.
...
\begin{algorithm}[t]
	\caption{Adaptive distributed consensus-based RDP}
	\label{alg:1}
	\begin{algorithmic}[1]
		...
		\State $c^{i}_{D}(k) = -MCP_{\Gmc}(-c^{j}_{D}(k), ~\forall j \in \overline{\Nmc}_{i})$
		\State $c^{i}_{U}(k) = -MCP_{\Gmc}(-c^{j}_{U}(k), ~\forall j \in \overline{\Nmc}_{i} )$ \label{e:MC1}
...
\begin{table*}[t!]
	...
	\begin{tabular}{|c| c| c| c| c| c| c| c| c| c| c| c|}\hline %c| c| c|
		%	\rowcolor{gray}
		$    $ & $\varsigma_{P}$ & $d_{m}$ & $d_{M}$ & $\omega$ & $\eta_{L}$ & $\eta_{H}$ & $\underline{\xi}$ & $\overline{\xi}$ & $\phi$ & $\rho$ %& $\underline{r}$ & $\hat{r}$ & $1-\overline{r}$ 
		& $R$  \\
		\hline
		$\Gmc$ & $0.37$ & $2$ & $5$ & $1.67$ & $\zeta$ & $0.91$ & $0.17$ & $0.67$ & $7$ & $7$ %& $0.19$ & $0.9995$ & $1.5 \cdot 10^{-13}$ 
		& $1093.5$ \\\hline    
		$\Tmc$ & $0.22$ & $40$ & $40$ & $1.95$ & $\zeta$ & $0.92$ & $0.02$ & $0.02$ & $2$ & $2$ %& $0.14$ & $0.9994$ & $3.4 \cdot 10^{-6}$ 
		& $840.5$  \\\hline
	\end{tabular}
...
MCP can be parallelized. To be precise, quantities $c_{i}(k)^{D}$ and $c_{i}(k)^{U}$ could be determined beforehand, as these do not depend on the state $x(k)$. 
...
The second part of Alg.~\ref{alg:1} from line \ref{b:forxk1} to line \ref{e:forxk1} addresses the main body of the provided solution to the reference design. In particular, lines \ref{b:ifW}-\ref{e:ifW} implement the termination condition described in \eqref{eq:lyap} (exiting the {\bf while} loop), and lines \ref{b:update}-\ref{e:update} illustrate how the state $x(k)$ is locally updated into $x(k+1)$ according to equations \eqref{eq:3}, \eqref{eq:c_min} and \eqref{eq:3.33}.
As a final note, we highlight how the two {\bf for} loops at lines~\ref{b:forMCP} and \ref{b:forxk1} are indeed parallel executions on all nodes and not sequential operations.




To conclude, we point out that a measure of the convergence rate of Alg.~\ref{alg:1} can be established through the reasoning shown in App.~\ref{app:convergenceanalysis} and given by
\begin{equation}\label{eq:Rconvergence}
	R := 0.5 (1+d_{m})^{\rho} \geq 1.
\end{equation}
...
It is worth to note that the dynamics corresponding to $\Tmc$ converges to average consensus faster than that of $\Gmc$, as $\Tmc$ is a regular graph with higher degree $d_{M2}=d_{m2}=40$ w.r.t. to those of $\Gmc$ (ranging from $2$ to $5$); furthermore, $\Tmc$ has a total of $(n_{2}d_{M2}/2)=4620$ edges. 
...
It can be seen in Figs.~\ref{fig:traj1f}\textdiv\ref{fig:eta1f} that, under the application of Alg.~\ref{alg:1} also in such non-nominal conditions, the abrupt $c_U$-$c_D$ variations (Fig.~\ref{fig:constr1f}) yield some real-time adjustment of the $|\delta x^{\star}(k)|$ variable, which is reflected in the overall slower convergence ($\bar{k}=95$) with respect to the no-fail case of Figs.~\ref{fig:traj1}\textdiv\ref{fig:eta1}. Nonetheless, also in this situation, convergence is attained despite the violation of the theoretical $\eta$ bounds, as shown in Fig.~\ref{fig:eta1f}. 
...
The proposed approach leverages fully decentralized computations and it also considers the presence of water exchange capacity limits, which are embedded in the developed iterative procedure as download and upload constraints for each branch of the system. 

A potential research direction is represented by the analysis and design of the entire control depicted in Fig. \ref{fig:ControlSchemeCavallinoDraft}, according to the actual scheme implementation. Also, a couple of interesting investigation extensions are embodied by the development of the presented scheme exploiting directed topologies and by the improvement of its convergence properties.
...
The following theorem provides convergence guarantees towards average consensus for RDP \eqref{eq:3}, whose distributed implementation is performed through Alg. \ref{alg:1}. The related convergence rate is also discussed.


\begin{theorem}\label{thm:convergence}
	The RDP \eqref{eq:3}, with $\eta(k)$ selected as in \eqref{eq:3.33}, converges to average consensus as $k \rightarrow \infty$. In particular, by taking $W(x(k))$ in \eqref{eq:lyap} as a Lyapunov function, $W(x(k))$ vanishes with an approximate exponential decay rate $r \in (0,1)$ depending on the graph radius $\rho$ and the minimum nonzero entry of $Q_{\eta}(k)$ over time $k$. 
	...
	\end{equation}
	where $\underline{\xi}$, $\overline{\xi}$ and $\eta_{H}$ are respectively defined as in \eqref{eq:minentryP}, \eqref{eq:maxentryP}, and \eqref{eq:etaHdef}.
\end{theorem}
...
\begin{proof}
	...
	By means of the RDP~\eqref{eq:3} and noting that $A,B \in \mathrm{stoch}^{2}(\mathbb{R}^{n \times n})$, we observe that
	%
	\begin{align}
		W(x(k+\rho)) &= W(B(k)x(k))\nonumber \\
	...
	In this direction, we show that $\epsilon \in (0,1)$ leveraging Lem.~\ref{prop:metropolis}.\\
	...
	\end{equation}
	%
	can be related to the convergence of the RDP dynamics~\eqref{eq:3}, namely, the lower the value of $r$, the faster the convergence of RDP towards average consensus. 
	Indeed, inequality \eqref{eq:Wineq3} guarantees that the agreement for RDP \eqref{eq:3} is fulfilled with exponential decay dictated by \eqref{eq:rdef}: choosing a reference instant $k$, $\forall k \in \mathbb{N}$, it holds that 
	%
	...
	Now, we show that the consensus for the RDP \eqref{eq:3} is reached in terms of arithmetic mean of the initial conditions, i.e. the final value $x_{\infty}$ taken by $x(k)$, as $k$ goes to infinity, has the form
...
\begin{remark}
	...
	\begin{equation}\label{eq:rhat}
		\hat{r} := 1-(1+d_{m})^{-\rho} \in [0.5,1).
	\end{equation}
	Expression \eqref{eq:rhat} is obtained after averaging $\underline{\epsilon}$ and $\overline{\epsilon}$ that appear in the proof of Thm.~\ref{thm:convergence} through the convex combination $(1-\beta)\underline{\epsilon} + \beta \overline{\epsilon}$, choosing $\beta := (1+d_{m})^{-1}$.
	Notice that $\hat{r} \geq 0.5$, as $\Gmc$ is connected with at least $n\geq 2$ channels, implying that $d_{m} \geq 1$ and $\rho \geq 1$.
	%The lower the value of $r$, the faster the convergence of RDP~\eqref{eq:3} towards average consensus; 
	Thus, being an estimation of $r$, $\hat{r}$ serves as a purely topological index that can roughly measure how quickly this convergence takes place. Alternatively, index $R$ in \eqref{eq:Rconvergence}
	can be used for the same purpose, as $R \propto \hat{r}$. 
\end{remark}
%
\begin{remark}
...
\begin{IEEEbiography}[{% Figure removed}]{Marco Fabris} received the Laurea (M.Sc.)
	...
	formation control. In 2020-2021, he was post-doctoral fellow at the Technion-Israel Institute of Technology, Haifa. From January 2022 to July 2023 he was post-doctoral fellow at the University of Padua. He is now currently assistant professor with the University of Padua and his
	...
	systems and data-driven predictive control.
	\vspace{-12pt}
\end{IEEEbiography}
