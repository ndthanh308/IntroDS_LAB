\section{Missing Proofs of Section \ref{sec:4n-3}}\label{missing-proofs-4n-3}
\upperBoundLeft*
\begin{proof}
...
\begin{proof}
    We have
    \begin{align*}
        1 > v_i(B_{\ell^*+1}) = v_i(\ell^*+1) + v_i(2n-\ell^*) = v_i(\ell^*+1) + \frac{1}{3} + x.
    \end{align*}
    ...
    \begin{align*}
        1 > v_i(B_{s +1} \cup \{g\}) &= v_i(s+1) + v_i(n-s) + v_i(g) \\
...
\begin{restatable}{observation}{lessThanThree}\label{lessThan13}
    $x < 1/3$.    
\end{restatable}
\begin{proof}
...
\begin{restatable}{observation}{yBound}\label{y-bound}
    $y< 1/6$.    
\end{restatable}
\begin{proof}
...
\subsection{Proof of Lemma \ref{expowerful}}\label{lemma-expowerful-proof}
...
\subsection{Proof of Lemma \ref{difficult-bound}}\label{proof-sec-1}
The main idea is as follows. Recall that $s'=\min(|S'|,s)$. We consider two cases for $s'$. If $s' = s$, then in order to prove Lemma \ref{difficult-bound}, we must prove
...
in Claim \ref{claim-3}. Adding the two sides of the inequalities implies Lemma \ref{difficult-bound}. We prove this lemma in the rest of this section.
...
\section{Missing Proofs of Section \ref{sec:algo1}}\label{missing-proofs-2}
\thmRedn*
\begin{proof}
