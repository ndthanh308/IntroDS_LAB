...
The algorithm is described in Algorithm~\ref{algo}. Given an ordered $d$-normalized instance, we initialize $n$ bags (one for each agent) with the first $[2n]$ goods (highest valued) as follows.
\begin{equation}
    \label{eq:B_i}
...
We note that in contrast to \cite{garg2021improved, simple}, in the bag-filling phase we do not add arbitrary goods to the bags but we add the goods in the decreasing order of their values. 
