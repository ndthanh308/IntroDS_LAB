...
\title{Rotating detectors in dS/AdS}
...
\begin{widetext}
\begin{eqnarray}
Z^i(\tau)&=&a \left[\cosh\left(q_{+}\tau\right)-\cosh\left(q_{-}\tau\right)\right] \, N^i(0) - 
\frac{ a\,\Omega}{q_+ q_- q_0}\left[
...
\end{eqnarray}
\end{widetext}
Here, $q_0:=\left[(a^2-\Omega^2+\Lambda)^2+4\Lambda \Omega^2\right]^{1/4}$ and $q_{\pm}:=(1/\sqrt{2})\sqrt{(a^2-\Omega^2+\Lambda) \pm q_0^2}$. These constants are similar to the ones obtained for stationary motion with all curvature invariants. In fact, there seems to be mapping between these trajectories, which will be discussed soon.

The mapping between the embedding coordinates and the RNC is given by\cite{hari2020normal},
\begin{eqnarray}
X^i=\Delta^{-1/(D-1)}\hat{x}^i
\end{eqnarray}
where, $\Delta=\Delta(0,\hat x)$ is the van Vleck determinant with $D$ as the dimension of spacetime. The geodesic distance can be evaluated using the expression for flat spacetime, $-\Delta\tau_{\rm geod}=\eta_{ab}\hat{x}^a\hat{x}^b=\eta_{ab}Z^aZ^b\Delta^{2/(D-1)}$. For maximally symmetric spacetimes, we can use the formula, $\Delta^{2/(D-1)}=\Lambda\Delta\tau_{\rm geod}^2/\sin^2(-\Delta\tau_{\rm geod}\sqrt{\Lambda})$ for van Vleck determinant and simplify the equation for geodesic distance to obtain,
\begin{eqnarray}
\Delta\tau_{\rm geod}^2=-\frac{1}{\Lambda}\left[\sin^{-1}\left(\sqrt{-\Lambda \Delta\tau_{\rm ms}^2}\right)\right]^2
\label{sigma2-ms-1}
\end{eqnarray}
Here, $\Delta\tau_{\rm ms}^2:=-(\eta_{ab}Z^aZ^b)$ given by,
\begin{widetext}
\begin{eqnarray}
...
The equation for the hyperbolic trajectory in the ref. \cite{hari:2021gns} will be a special case when $\Omega=0$.

\subsection{Periodicity in $\Delta \tau_{\rm geod}^2$}
...
\begin{eqnarray}
    ...
    a^2 \Omega\, R_{n 0 b 0}\, \Delta \tau_{\rm traj}^7 + \frac{1}{20160} \left(a^6 - 2 a^4 \Omega^2  \right. \nn \\
    &&  \left. + a^2\Omega^4 + 18 a^2 R_{0 n 0 n} + \frac{128}{3} a^2 \Omega^2 R_{0 b 0 b} - 17 R_{0 n 0 \bullet}R^{\bullet}_{0 n 0} + 50 a^2 \Omega^2 R_{0n0n} + 32 a^3 \Omega R_{n b n 0} \right) \Delta \tau_{\rm traj}^8 + O(\Delta \tau_{\rm traj}^9) \nn \\
\end{eqnarray}
\end{widetext}
...
\section{Comment on rotating observers in BTZ blackhole}

In Ref. \cite{Hodgkinson:2012mr}, the authors discuss about the detector response of an observer who is corotating with the horizon of BTZ blackhole\cite{Banados:1992wn,Banados:1992gq,carlip19952+}. The BTZ blackhole metric can be constructed by a suitable coordinate transformation from 2+1 Anti-de Sitter($\text{AdS}_3$) spacetime. The $\text{AdS}_3$ metric is given by,
\begin{eqnarray}
...
H.K. thanks the Indian Institute of Technology Madras and the Ministry of Human Resources and Development (MHRD), India for financial support.
\end{acknowledgments}
