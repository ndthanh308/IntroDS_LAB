...
\title[Vanishing of $\shat(f/K)$ and its consequences for anticyclotomic Iwasawa theory]{Vanishing of the $\p$-part of the Shafarevich-Tate group of a modular form and its consequences for Anticyclotomic Iwasawa Theory}
\author{Luca Mastella}
\date{}
\subjclass[2020]{Primary 11R23, Secondary 11F11}
\thanks{}
\address{}
...
Let $f = \sum_{n > 0} a_n q^n$ be a cusp-newform of even weight $k > 2$ and level $\Gamma_0(N)$, fix an odd prime $p \nmid N$ 
...
Deligne \cite[see][]{deligne:formes-modulaire} attached to $f$ and $\p$ a $p$-adic representation $W_\p$ of $G_\Q = \Gal(\bar{\Q}/\Q)$, 
that is a $2$-dimensional $\K$-vector space endowed with a continuous $G_{\Q}$-action. Let $V = W_\p(k/2)$ be the $k/2$-Tate twist of $W_\p$ and let $T$ 
be the selfdual lattice inside $V$, defined in \cite{nekovar:kuga-sato}; set $A = V/T$. 
...
that is the unique $\Z_p$-extension of $K$ pro-dihedral over $\Q$. Put $\Gamma = \Gal(K_\infty/K) \cong \Z_p$ and let 
...
$\p$-ordinariety of $f$ and the big image property for $V_\p := W_\p^\ast$, that $\calX_\infty$ is pseusoisomorphic to $\Lambda \oplus M \oplus M$, 
...
(under some thechnical assumptions) if the \emph{basic generalized Heegner cycle} $z_{f, K}$ (see Sec.~\ref{sec:shafarevich-tate-mod-forms}) 
...
(see Def.~\ref{def:image-abel-jacobi-K}, \ref{def:shat-K-infty}) in terms of the $p$-adic Abel-Jacobi maps 
\[
...
the product of the Kuga-Sato variety $\kugasatoi{N}{k-2}$ of level $\Gamma_1(N)$ and weight $k-2$ and the $(k-2)$-fold selfproduct of 
a fixed CM elliptic curve $A$ defined over $K[1]$. 
\newpage
With this language the main theorem of this paper becomes:
\begin{theorem}[Th.~\ref{th:main}]\label{th:main-intro}
Under the assumptions of Sec.~\ref{sec:framework}, suppose moreover that the basic generalized Heegner cycle $z_{f, K}$ is non-torsion and that 
...
The proof of Th.~\ref{th:main-intro} goes along the same lines of \cite{nekovar:kolyvagin}, which proves an analogous result for elliptic curves. 
...
the \emph{(classic) Heegner cycles} of \cite{nekovar:kuga-sato}, while we need the generalized ones of \cite{bdp:generalized}.
\begin{theorem}[Th.~\ref{th:besser}]\label{th:besser-intro}
Let $p$ be a non-exceptional prime (Def.~\ref{def:admissible-primes}) and $z_{f, K}$ be non-torsion in $\hone(K, T)$. Then 
\[
...
Then in Sec.~\ref{sec:consequences} with an Iwasawa theoretical argument that follows that of \cite{nekovar:kolyvagin}, we show that  
that the vanishing of $\shat(f/K)$ implies the vanishing of $\shat(f/K_\infty)$, proving in this way Th.~\ref{th:main-intro}.
...
In a forthcoming paper \cite{pati:generalized-quaternionic} Pati shows that replacing the generalized Heegner cycles with the ones 
over Shimura curves one can prove the result of \cite{longo-vigni:generalized} under the relaxed Heegner hypothesis. 
It may therefore be possible to prove in the same way results similar to those in this paper in this more general context. 
\end{remark}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Content of this article}\label{sec:content}

Sec.~\ref{sec:selmer-groups-and-complexes}-\ref{sec:mod-forms-gen-heegner-cycles} deal with the background material: 
in particular in Sec.~\ref{sec:selmer-groups-and-complexes} we introduce the general notion of Selmer groups and Selmer complexes for $p$-adic representations;
in Sec.~\ref{sec:anticyclotomic-iwasawa-theory} we introduce the anticyclotomic $\Z_p$-extension $K_\infty$ and we recall some notion of the theory 
of Semer group and complexes over $K_\infty$; in Sec.~\ref{sec:mod-forms-gen-heegner-cycles} 
...
Sec.~\ref{sec:vanishing-of-sha} and Sec.~\ref{sec:consequences} contain the new results: 
in Sec.~\ref{sec:vanishing-of-sha} we prove Th.~\ref{th:besser-intro} generalizing \cite{besser:finiteness-sha} using generalized Heegner cycles,  
in Sec.~\ref{sec:consequences} we generalize the Iwasawa theoretic argument of \cite{nekovar:kolyvagin} 
and we combine it with Th.~\ref{th:besser-intro} in order to prove Th.~\ref{th:main-intro}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Acknowledgements}
The content of this article is part of my PhD Thesis at \emph{Università degli Studi di Padova}. I would like to express my deepest gratitude to my advisor, Matteo Longo, 
for the nice topic he proposed to me and for its constant help and encouragement.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
When $v$ is archimedean $E_v = \C$ or $\R$, define $I_v = G_v$, it is trivial or has order $2$. 
For $\ell \ne \infty$ we will use the notations $G_\ell = \Gal(\bar{\Q}_\ell/\Q_\ell)$ and $I_\ell$ for its inertia subgroup; 
$\Frob_\ell$ will denote an (arithmetic) Frobenius, i.e~(a lift of) a topological generator of $G_\ell/I_\ell$, while $\Frob_\ell^\geo$ a geometric Frobenius. 
In particular we fix the Frobenius automorphism $\Frob_p \in G_p$ such that $\chi_p(\Frob_p) =1$: this is possible since  $\Q_p(\mu_{p^n})/\Q_p$ is totally 
...
unramified outside $S$, $G_{E, S} = \Gal(E_S/E)$. Denote by $I_v^S$ and $G_v^S$ respectively the decomposition and the inertia group of $G_{E, S}$ at $v$.

For any local field $E_v$ with residue characteristic $\ell$, let $E_v^t$ denote the maximal tamely ramified extension of $E_v$, 
i.e.~the maximal algebraic extension of $E_v$ such that any $x \in E_v^t$ has ramification prime to $\ell$ over $E_v$.  
...
\]
denote the restriction and corestriction map. Recall that if $E'/E$ is finite and Galois 
\[
\res_{E'/E} \circ \cores_{E'/E} = \Tr_{E'/E} := \hspace{-18pt}\sum_{\sigma \in \Gal(E'/E)} \hspace{-10pt}\sigma, \qquad 
...
Let $R$ be a commutative ring, $\rmod$ denotes the category of $R$-modules; $\rmod_\ft$ (resp.~$\rmod_\coft$) the full subcategory of modules of finte type,
i.e.~notherian (resp.~of cofinite type, i.e.~artinian). 
...
In Sec.~\ref{sec:selmer-complexes} we will fix a noetherian complete local ring $R$ of dimension $d$, maximal ideal $\mfrak$ and finite residue field $k$ of characteristic $p$: for a profinite group $G$ we denote  by $\rgmod^\ad$ (resp.~$\rgmod^\indad$) the category of admissible (resp.~ind-admissible)
$R[G]$-modules as defined in \cite[Def.~3.2.1, 3.3.1]{nekovar:selmer-complexes}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Selmer groups and complexes}\label{sec:selmer-groups-and-complexes}

In this section we introduce the notion of $p$-adic representation and its Selmer groups.

\subsection{$p$-adic representations}\label{sec:p-adic-reps}
...
by $\pi$ a uniformizer and by $\kappa$ its residue field.
...
that is $G_E$-stable by the $\O$-linearity of the action. In particular $A[p^n] \cong T/p^nT$ and 
\[
T = \projlim_{n, p \cdot -} T/p^n \, T \cong \projlim_{n, p \cdot -} A[p^n]; \qquad A = \injlim_{n} A[p^n] = \bigcup_{n} A[p^n].
\]
...
\]
where $n$ is the rank of $T$ and $B = \O, \K$ or $\kappa$ respectively, depending on the fact that $M = T, V$ or $A[\pi]$.
\end{remark}
...
An important feature of many Galois $p$-adic representations, in particular of all these of interest for this article, 
...
A representation $\rho$ of $G_E$ is unramified outside $S$ if and only if it factorizes through $G_{E, S}$.
\end{proposition}

\begin{proof}
If $\rho$ factorizes as $\rho^S$ through $G_{E, S}$, then $\rho(I_v) = \rho^S \circ \pi(I_v) = \rho^S(I_v^S) = \rho^S(\set{1}) = \set{1}$ 
for any $v \notin S$. For the converse observe that applying \cite[Ch.~II, Prop.~9.4]{neu:ant} to the extensions $E_S/E$ and $\bar{E}/E$ it follows that $G_{E, S}$ is the quotient of $G_E$ by $H$, the minimal closed normal subgroup containing all $I_v$ such that $v \notin S$. Thus $\ker \rho = H$ by minimality and therefore $\rho$ factorizes through $G_E/H \cong G_{E, S}$.
\end{proof}

Let us turn to the notion of duality between Galois representations. 
\begin{definition}\label{def:linear-dual}
The linear dual of an $\O$-adic (resp.~$\K$-adic) representation $T$ (resp.~$V$) is defined to be the free $\O$-module $T^\ast = \Hom_{\O}(T, \O)$ 
...
In the litterature of $p$-adic representation the most common definition of Pontryagin dual is $\Hom_{\Z_p}^\cont(M, \Z_p)$, but this is harmless,
since it is (non-canonically) isomorphic to our, the isomorphism depending on the choice of a basis of $\O$ as finite free $\Z_p$-module.
\end{remark}

It is known (e.g.~\cite[Ch.~I, Th.~1.1.8]{neu:cnf}) that the $(\, -\,)^\vee$ functor defines a duality functor between the categories of compact and discrete $\O$-modules, i.e.~$M \to M^{\vee \vee}$ is an isomorphism for such modules. 
...
If $M$ is a finite free $\O$-module $\Hom_\O(M, \O) \otimes_\O \K/\O \cong \Hom_\O(M, \K/\O)$ .
\end{proposition}
...
In this paragraph $T$ will be an $\O$-adic representation of $G_{E, S}$ and let $V$ and $A$ be the induced representations as in the previous paragraph. 
...
we refer to \cite[B.2]{rubin:euler-systems} or \cite[Sec.~II.3]{neu:cnf}.


\begin{definition}\label{sec:local-conditions}
A local condition $\calF= (\calF_v)_{v \in S}$ is the choice, for any place $v \in S$ of a subspace $\hone_{\calF_v}(E_v, V) \subseteq \hone(E_v, V)$. 
...
The submodule $\hone_{\calF_v}(E_v, A[\pi^n])$ of $\hone(E_v, A[\pi^n])$ can be equally defined \cite[see][Rk.~I.3.9]{rubin:euler-systems} as the image of 
$\hone_{\calF_v}(E_v, T)$ under the map induced by 
\[
T \surj T/\pi^nT \xrightarrow[\sim]{\cdot \pi^{-n}} \pi^{-n}T/T = A[\pi^n]
\] 
...
If $V$ is unramified at $v$, then $c_v(A) = 1$ \cite[see][Lemma 3.5.\emph{iv}]{rubin:euler-systems}, i.e.~$\honef(E_v, A) = \honeur(E_v, A)$.
...
In this article we will consider mainly the Block-Kato Selmer groups defined in \cite{bloch-kato:l-funct-tamagawa}, usually denoted by $\honef(E, X)$, 
...
\hone(E_v, V) &\text{if $v | \infty$}.
\end{cases}\\
\end{align*}
where $B_\crys$ denote the Fontaine ring of $p$-adic crystalline periods. The group 
\[
...
$\K[G_v]$-submodules $V = F_v^0 V \supseteq V_v^+ = F_v^1 V \supseteq F_v^2 V \supseteq \dots$ and we may define for $X = T, V, A$ the Greenberg Selmer group 
...
Ex.~\ref{ex:unramified-finite} that $\honeur(E_v, A)$ contains $\honef(E_v, A)$, that is the image of $\honeur(E_v, V)$ via the natural map, 
...
and residue field $k$. Let $S_f \subseteq S$ be the subset consisting of finite primes.
...
Let $G$ be a profinite group and $M^\bullet$ a complex of admissible $R[G]$-modules \cite[Def.~3.2.1]{nekovar:selmer-complexes}.
...
Let $X$ a complex of $R[G_{E, S}]$-admissible modules \cite[Def.~3.2.1]{nekovar:selmer-complexes}. 
...
\]
Denote $i_S^+(X) = (i_v^+(X))$. The Selmer complex with local condition $\Delta(X)$ on $X$ is the complex
\[
...
\[
0 \to \tildecohomology^0_f(E, X) \to \cohomology^0(E, X) \to \bigoplus_{v | p} \cohomology^0(G_v, X) \to \honetildef(E, X) \to \hone_\str(E, X) \to 0.
\]
\end{proposition}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{proposition}[{\cite[Sec.~2.3.2]{nekovar:selmer-complexes}}]\label{prop:dualizing-functor}
Let $I$ be an injective hull of $k$ \cite[Def.~3.2.3]{bruns-herzog:cohen-macaulay-rings} and set $J = I[n]$, i.e.~$J$ the complex concentrated in degree $-n$ 
such that $J^{-n}= I$.
...
the converse, then we say that $\pi$ is a perfect duality if the adjunction morphism \cite[Sec.~1.2.6]{nekovar:selmer-complexes} 
$\adj(\pi) \colon X \to D_J(Y)(1)$ is a quasi isomorphism.
\end{definition}

For local conditions for $\Delta(X)$, $\Delta(Y)$ there is a notion of being orthogonal complements with respect to $\pi$ \cite[Lemma\,-Def.~6.2.7]{nekovar:selmer-complexes}. This notions has an important consequence:
\begin{theorem}[{\cite[Th.~6.3.4]{nekovar:selmer-complexes}}]\label{th:duality}
Let $X, Y, J$ as above, $\pi$  be a perfect duality and $\Delta(X)$ and $\Delta(Y)$ orthogonal complements. There is a map of complexes
\[
...
Let now consider the special case of $p$-adic representations: recall that, as we remarked in Sec.~\ref{sec:p-adic-reps}, $A^\ast = D(T)$, $T^\ast = D(A)$, 
the evaluation maps
\[
\ev_2 \colon T \otimes_\O A^\ast(1) \longrightarrow (\K/\O)(1), 	\qquad \ev_1 \colon   A \otimes_\O  T^\ast(1) \longrightarrow (\K/\O)(1)
\] 
are by definition perfect dualities (in the sense of Def.~\ref{def:perfect}) and the Greenberg local conditions are orthogonal complements with respect to them. Hence Th.~\ref{th:duality} holds and in this case its statement becomes:
...
\begin{proof}
We show only the first isomorphism, the proof of the latter one being analogous. Let $J = (\K/\O)[0]$. By Th.~\ref{th:duality} 
\[
...
& =  \Hom_\O\Bigl(\selmer\bigl(K, A^\ast(1)\bigr)^{3 - i}\hspace{-12pt}, \hspace{10pt}\K/\O\Bigr) = D \Bigl( \selmer\bigl(K, A^\ast(1)\bigr)^{3 - i} \Bigr)
\end{align*}
as  
\[
J^{ i + j - 3} = \begin{cases}
...
\end{cases}
\]
Therefore, since $\K/\O$ is injective and therefore $D = \Hom_\O( \, - \, , \K/\O)$ is exact and hence it commutes the $i$-th cohomology functor
\begin{align*}
...
Recall that $K$ denotes the imaginary quadratic field of Sec.~\ref{sec:notations}, and assume moreover that  
$p \nmid h_K$, the class number of $K$. Consider for any $n \ge 0$ the ring class field $K[p^{n+1}]$ of conductor $p^{n+1}$: $\Gal(K[p^{n+1}]/K)$ admits a canonical splitting
\[
\Gal(K[p^{n+1}]/K) \cong \Gal(K[p^{n+1}]/K[p]) \times \Gal(K[p]/K) \cong \Z/p^{n}\Z \times \Delta,
\]
where \cite[see][Th.~7.24]{cox:primes-of-the-form} 
\[
...
\]
Hence $K_\infty = (K[p^\infty])^\Delta$ is a $\Z_p$-extension of $K$, called the anticyclotomic $\Z_p$-extension of $K$: it is the unique $\Z_p$-extension of $K$ pro-dihedral over $\Q$, i.e.
\[
\Gal(K_\infty/\Q) \cong \Gal(K_\infty/K) \rtimes \Gal(K/\Q) = \Gal(K_\infty/K) \rtimes \set{1, \tau},
\]
...
Denote by $\Lambda_n = \O[\Gamma_n]$ the group algebra of $\Gamma_n$ and let  
$\Lambda = \projlim_n \Lambda_n = \O \llbracket \Gamma \rrbracket$ be the completed group algebra of $\Gamma$. 
It is well known \cite[Prop.~5.3.5]{neu:cnf} that $\Lambda \cong \O\llbracket X\rrbracket$ 
...
By general theory of $\Z_p$-extensions \cite[Prop.~13.2, 13.3]{washington:cyclotomic-fields} the ramification 
...
both places $\p$ and $\bar{\p}$ of $K$ over $p$ have the same beaviour in $K_\infty/K$, 
...
Moreover we may use the restriction and corestriction maps  \cite[see][Ch.~II, Sec.~5]{neu:cnf} in order to link the cohomology groups over the different layers $K_n$. First of all note that the formation of the Selmer groups defined in Sec.~\ref{sec:selmer-groups} is compatible with these maps, i.e.
\[
...
\]
which are naturally $\Lambda$-modules as $\hone_\ast(K_n, A)$ and $\hone_\ast(K_n, T)$ are are stable subgroup with respect of the $\Gamma_n$-action of Lemma \ref{lemma:action-quotient-group} and hence sub-$\Lambda_n$-modules. We will be in the following interested in particular in the structure as $\Lambda$-modules 
of the Pontryagin dual of $\honef(K_\infty, A)$. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Selmer complexes for $K_\infty$}\label{sec:selmer-complexes-iwasawa}

Let $R = \Lambda$ and $\mfrak = (p, \gamma - 1)$ its maximal ideal and suppose  that $K_\infty \subseteq K_S$.
Denote moreover by $\iota \colon \Lambda \to \Lambda$, the involution induced by $\gamma \mapsto \gamma^{-1}$.
Nekovar defines, for $T$ complex of admissible modules in $\rgksmod_\ft$ and $M$ complex of ind-admissible modules \cite[Def.~3.3.1]{nekovar:selmer-complexes} in $\rgksmod_\coft$ supported in $\set{\mfrak}$ (i.e.~$M^i = \bigcup_{n>0} M^i[\mfrak^n]$ for any $i$)
endowed with \emph{Greenberg local conditions}, Selmer complexes 
\[
\selmeriw(K_\infty/K, T) \qquad \selmer(K_S/K_\infty, M)
\]
such that (essentially by \cite[Prop.~8.8.6]{nekovar:selmer-complexes}) for ordinary $p$-adic representations
\[
\honetildef(K_S/K_\infty, A) \cong \injlim_{\res, n} \honetildef(K_n, A); \qquad \honetildefiw(K_\infty/K, T) \cong \projlim_{\cores, K_n} \honetildef(K_n, T).
\]
In the following we will use the simplified notations $\honetildef(K_\infty, A)$ and $\honetildef(K_\infty, T)$ for these objects.

The general theory of Selmer complexes in Iwasawa theory can be found in \cite[Ch.~8]{nekovar-plater:parity}. We will need only special versions for $p$-adic representations of the following results. Let us start with duality theory.

\begin{remark}
Note that, since $\Lambda$ has the same residue field as $\O$, then $\K/\O$ can be used in order to define the duality functor 
of Prop.~\ref{prop:dualizing-functor} for $R = \Lambda$, that we will call $\bar{D}$. Hence for any $\Lambda$-module $M$,
\[
...
In the following we denote $M^\vee$ toghether with the latter $\Lambda$-structure by $D(M)$. Let $N^\iota$, for any $\Lambda$-module $N$,  
...
\end{remark} 
\newpage
Th.~\ref{th:duality} becomes therefore:
\begin{theorem}[{\cite[Sec.~8.9.6.1]{nekovar:selmer-complexes}}]\label{th:duality-iwasawa}
Let $T, T^\ast(1)$ be two bounded complexes of admissible modules in $\ogksmod^\ad_{\O-\ft}$ endowed of Greenberg local structures. Define moreover the bounded complexes
\[
...
And the special case in which $T$ is a $p$-adic representation similarly to Th.~\ref{th:duality-rep}:
...
A classic tool in Iwasawa Theory for elliptic curves is Mazur's Control Theorem: it studies the relations between the Selmer groups over $K$ 
and $K_\infty$. In our context it generalizes to the following Exact Control Theorem.

\begin{theorem}[{\cite[Prop.~8.10.1]{nekovar:selmer-complexes}}]\label{th:exact-control-theorem-general}
Let $T$ be a bounded complexes of admissible modules in $\rgksmod^\ad_{\O-\ft}$ endowed of Greenberg local structures. There is a canonical isomorphism
\[
...
\]
\end{theorem}
In the case of a $p$-adic representation it implies the following statement:
...
\begin{proof}
Consider the first quadrant cohomological spectral sequence of Th.~\ref{th:exact-control-theorem-general}. 
Note that
\[
...
\]
But then by the Duality Th.~\ref{th:duality-iwasawa-rep}
\[
0 = D\bigl( \tildecohomology_f^0(K_\infty, A)^\Gamma \bigr) \cong \tildecohomology_f^3(K_\infty, T)_\Gamma
\]
and hence $\tildecohomology_f^3(K_\infty, T) = 0$ by Nakayama's lemma and then, being its Pontryagin dual, also $\tildecohomology_f^0(K_\infty, A) = 0$ and hence $E_2^{i, j} = 0$ for $j \le 0$. In fact $E_2^{i, j} = 0$ if $j \ne 1, 2$ 
(i.e.~the $E_2$-sheet has nonzero terms only in the horizontal half-lines $j = 1, 2$, $i \ge 0$) since for $j \ge 3$, by Th.~\ref{th:duality-iwasawa-rep},
\[
...
Suppose moreover that $f$ is a normalized newform (thus $a_1 = 1$, $T(\ell)f = a_\ell f$ for any rational prime $\ell$).
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Selfdual representation}\label{sec:selfdual-rep}
...
\[
\rho_{f, \p} \colon G_\Q \to \GL_2(\O_\p) \subseteq \GL_2(\K);
\]
$V_\p$ is unramified outside the finte set of rational primes
...
\]
for any prime $\ell \ne p$. Ribet \cite[Th.~2.3, Prop.~2.2]{ribet:nebentypus} proved moreover that $\rho_{f, \p}$ is irreducible and that $\det \rho_{f, \p} = \chi_p^{k-1}$.
...
the Tate module of an elliptic curve: it is selfdual, i.e.~$V^\ast(1)\cong V$. 
Indeed Nekovar in \cite{nekovar:kuga-sato} 
shows that $V$ has a $G_\Q$-equivariant lattice $T$ equipped with a $G_\Q$-equivariant, 
skew-symmetric, non-degenerate $\O$-linerar pairing $[\,-,-\,] \colon T \times T \to \O(1)$, in particular this means that $T^\ast(1) \cong T$, 
and tensoring with $\K$, this shows that $V$ is selfdual. 

Furthermore, since $f$ is $\p$-ordinary, we may describe a bit better this representation. 
A result of Wiles \cite[Th.~2.2.2]{wiles:lambda-adic} tell us that $V_\p$  
...
the invertible root of the polynomial $X^2 - i_p(a_p)X + p^{k-1} = 0$ (which exists since $f$ is $\p$-ordinary). 
...
From now on $V, T$ will denote respectively the selfdual representation attached to $f$ and its lattice as in Sec.~\ref{sec:selfdual-rep} and $A = V/T$. 
...
Let $W_{k-2}$ denote the Kuga-Sato variety of level $\Gamma_1(N)$ and weight $k$, 
i.e~the canonical desingularization \cite[see][Appendix]{bdlp:generalized} of the $(k-2)$-fold self-product of the universal generalized elliptic curve with 
$\Gamma_1(N)$-level structure. This is a scheme defined over $X_1(N)$ and the fiber at any noncuspidal point $P = (E, \eta_N) \in X_1(N)$ is $E^{k-2}$.
 Fix moreover a (complex) elliptic curve $A$ with complex multiplication by $\O_K$. By the theory of complex multiplication 
(see for instance \cite[Th.~II.4.1, Th.~II.2.2(b)]{silverman:advanced}) $A$ is defined over the Hilbert class field $K[1]$ of $K$ and there is an isomorphism
\[
...
The \emph{generalized Kuga-Sato variety} of level $\Gamma_1(N)$-level and weight $k$ is the $(4k - 3)$-dimensional variety 
...
$\pi \colon X_{k-2} \to X_0(N)$, whose fibers over a non-cuspidal point $P = (E, \eta_N)$ is $E^{k-2} \times A^{k-2}$.

Let $\Nfrak$ be an $N$-cyclic ideal of $\O_K$ (that there exists thanks to the Heegner hypothesis on $K$), and let $t_A \in A[\Nfrak](\C)$ be an $\Nfrak$-torsion point.
Consider the set
\[
...
A couple $(\phi, A') \in \Isog^{\Nfrak}(A)$ determines a point 
...
\]
supported on the fibre $X_P := \pi_W^{-1} (P_\phi) = (A')^{k-2} \times A^{k-2}$.
\end{definition}

\begin{proposition}[{\cite[Prop.~2.7]{bdp:generalized}}]
...
The next construction of Castella and Hsieh \cite[see][]{castella-hsieh:heegner-cycles-p-adic-l-functions} selects a subclass of these cycles that are 
...
`Shimura reciprocity law' and some `norm relations' \cite[see][Lemma 4.3 and Prop.~4.4]{castella-hsieh:heegner-cycles-p-adic-l-functions}, 
...
in the sense of Gross \cite[see][Th.~0.1]{yang:cm-av}, which is characterized by the 
following properties:
\begin{itemize}
    \item it has CM by $\O_K$;
    \item $A(\C) = \C/\O_K$;
    \item it is a $\Q$-curve \cite[see][]{gross:arithmetic-elliptic-curves-cm} defined over the real subfield $K[1]^+ = \Q\bigl(j(\O_K)\bigr)$ 
...
The map $\C/c^{-1}\O_c \to \C/\afrak^{-1}$ defined by $z \mapsto cz$ corresponds to an isogeny $\lambda_\afrak \colon A_c \to A_\afrak$ and we define 
\[
\phi_\afrak = \lambda_\afrak \circ \phi_c \colon A \to A_\afrak.
\]
For a suitable choice of $t_A$ (see \cite[Sec.~2.3]{castella-hsieh:heegner-cycles-p-adic-l-functions}, where is called $\eta_c$), we have therefore a  
...
For a general smooth variety $X$ over field $E$ of characteristic $0$,  Jannsen \cite[Rk.~6.15(c)]{jannsen:cont-et} defined a $p$-adic Abel-Jacobi map 
\[
...
obtained by pull-back in the rightmost square of the following diagram
\[
\begin{tikzcd}
...
\end{tikzcd},
\]
where $\lvert\bar{Z}\rvert$ is the support of $\bar{Z}$, $\bar{U} = \bar{X} \setminus \lvert\bar{Z}\rvert$ and $\bar{\cl}_X$ is the composition of the $p$-adic étale cycle map \cite[Lemma 6.14 and Th.~3.23]{jannsen:cont-et}
\[
...
In the following let $E$ be  number field containing the Hilbert class field $K[1]$ of $K$ so that $X = X_{k-2}$ is defined over $E$ and let $i=k-1$. Applying the projector $\epsilon_X$ to the diagram above and allowing coefficients of cycles in $\Z_p$ we obtain a $p$-adic Abel-Jacobi map  
\[
...
\end{tikzcd}.
\]
Castella and Hsieh show moreover \cite[Sec.~4.2, 4.4]{castella-hsieh:heegner-cycles-p-adic-l-functions} that for any 
...
\]
In particular we will be interested to $\AJ^\et_c := \AJ^\et_{K[c]}$, in fact the generalized Heegner cycle $\Delta_c$ is rational over $K[c]$, 
...
Let $\imajt(K[c]) = \im \AJ^\et_{\O, c}$, that we will take as an analogous in higher weight of the $K[c]$-rational points of an elliptic curve. 
Note that  $\imajt(K[c]) \subseteq \honef(K[c], T)$ by \cite[Sec.~II.1.4]{nekovar:p-adic-height-heegner-cycles} 
or \cite{niziol:p-adic-regulators} and hence $\imajt(K[c]) \otimes \K/\O$ injects into $\honef(K[c], A)$.
...
A similar definition could be given for $\shat(f/K)$, if we had a definition of $\imajt(K)$, but this cannot be defined as the image of the Abel-Jacobi map 
...
\]
under some standard assumptions this becomes an isomorphism of $\O$-modules. In particular if this is the case it restricts to an isomorphism
\[
...
Several papers \cite[see for instance][]{nekovar:kuga-sato, besser:finiteness-sha, longo-vigni:beilinson, masoero:sha},
...
\]
where $\kugasatoplain{N}{k-2}$ is the Kuga-Sato variety of level $\Gamma(N)$ and weight $k-2$, 
...
In \cite[Sec.~7.3]{castella-hsieh:heegner-cycles-p-adic-l-functions} Castella and Hsieh use the classes $z_{f, c}$ in order to construct an 
anticyclotomic Euler system as in the following definition. Let here $\squarefree$ denote the set of squarefree products 
of primes $\ell$ inert in $K$ such that $\ell \nmid 2pN$ and for any $\ell$ inert in $K$ let $\Frob_\ell := \Frob_\lambda$, 
...
and let $\sigma_{\bar{\Nfrak}}$ be the image by the Artin reciprocity map of $\overline{\Nfrak}$. 
...
\begin{theorem}[{\cite[Th.~7.7]{castella-hsieh:heegner-cycles-p-adic-l-functions}}]\label{th:selmer-group-V-castella-hsieh}
Let $z_{f, K}$ be non-torsion in $\hone(K, T)$, then 
\[
\honef(K, V) = \K \cdot z_{f, K}
\]
\end{theorem}

In fact they prove \cite[see][Th.~7.19]{castella-hsieh:heegner-cycles-p-adic-l-functions} that under the non-torsion hypothesis on $z_{f, K}$,
...
In the case of classical Heegner cycles \cite{besser:finiteness-sha} shows that the method of \cite{nekovar:kuga-sato} can 
...
\item the primes such that the image of $\rho_{f, p} \colon G_{\bar{\Q}} \to \GL_2(\O_F \otimes \Z_p)$ does not contain the set 
\[
\set{g \in \GL_2(\Z_p) : \det g \in (\Z_p^\times)^{k-1} }.		
\]
\end{itemize}
\end{definition}

\begin{remark}\label{rk:admissible-primes}
The previous definition slightly differs from \cite[Def.~6.1]{besser:finiteness-sha} as we need to exclude also $p \mid \phi(N)(k-2)!$ 
in order to get the integrality of the Abel Jacobi map and since we want to work with the selfdual lattice $T$ coming from \cite{nekovar:kuga-sato}; 
we exlude also the prime $3$ as $SL_2(\F_2) \cong S_3$ is solvable and therefore Lemma \ref{lemma:res-iso} does not 
hold anymore. However this last condition is not necessary in order to get Th.~\ref{th:besser}, 
...
\cite[Th.~3.1]{ribet:l-adic-ii}. It implies many technical properties on the reduced representations $A[p^M]$ and their cohomology, as the next lemma. 
...
\]
and therefore $\res_{K[1]/K}$ is an isomorphism by the inflation-restriction sequence.
\end{remark}
...
$\hone(K, T)[\p] = 0$. Since $p$ is by asumption unramified in $F$, $p$ is a uniformizer of $\O$ and therefore $\hone(K, T)[\p] = \hone(K, T)[p]$.
...
\end{tikzcd}
\]    
shows that $\hone(K, T)[p]$ is a quotient of $\ho(K, A[p])$, the latter beeing trivial by Lemma \ref{lemma:res-iso}.
\end{proof}
...
\begin{proof}
This is just \cite[Th.~1.2]{besser:finiteness-sha}, where we replace the generalized Heegner cycles to the classical ones in the definition of the classes 
$P(n)$. Note that the proof of \cite[Th.~1.2]{besser:finiteness-sha} is a formal consequence of the properties enjoyed by the classes $P(n)$ listed in 
\cite[Prop.~3.2]{besser:finiteness-sha} (see \cite[Sec.~6]{besser:finiteness-sha}), therefore in order to prove our result it is enough to show that these
properties still hold with the new definition of the classes $P(n)$. This will be done in Sec.~\ref{sec:euler-system-generalized-heegner-cycles}: 
they are respectively Prop.~\ref{prop:besser-property-complex-conjugation}, \ref{prop:besser-property-finite-condition} and 
...
Here we define the Kolyvagin classes $P(n)$ used in the proof of Th.~\ref{th:besser} and we establish their properties.
...
\begin{itemize}
    ...
    \item $p^M \mid a_\ell, \, \ell+1$;
    \item $p^{M+1} \nmid \ell + 1 \pm a_\ell$. 
\end{itemize}
Let $n$ be a squarefree product of primes $\ell \in S(M)$; recall that we defined $G(n) = \Gal(K[n]/K[1])$ and $\calG_n = \Gal(K[n]/K)$. 
...
Indeed:
\begin{itemize}
\item $\Tr_\ell(z_{f, n}) =  \res_{K[n]/K[m]} \circ \cores_{K[n]/K[m]}(z_{f, n}) = a_\ell\res_{K[n]/K[m]}(z_{f, m})$ by (E2);
\item $\ell +1, \, a_\ell \equiv 0 \mod p^{M}$ as $\ell \in S(M)$.
\end{itemize}
Thus, by Rk.~\ref{rk:identification-modulo-p-M}, the class 
...
We come now to the properties of the classes $P(n)$. For any $n$ squarefreee product of primes of $S(M)$ we define 
...
\begin{proof}
The proof follows essentially from the computations of \cite[Prop.~5.4(1)]{gross:kolyvagin}, replacing (E1) and (E3) to the analogous properties
for the Heegner points on an elliptic curve. We just sketch the argument for the convenience of the reader. 
Gross shows that, being $K[n]$ generalized dihedral over $\Q$, $\tau_c D_\ell = -\sigma_\ell D_\ell \tau_c + k \, \tau_c \Tr_\ell$, for $k \in \Z$: 
it follows that
\begin{align*}
...
\end{align*}
Let now $v \mid p$. The restriction map 
\[
...
\[
\Gal(K_\lambda^t/K_\lambda^\ur) \cong \hat{\Z}' = \prod_{q \ne \ell} \Z_q \cong \hspace{-5pt} \projlim_{\substack{n \ge 1, \, \ell \nmid n,\\m \mid n}} \hspace{-5pt} \Z/n\Z.
\]
\begin{remark}\label{remark:finite-singular-iso-proof}
Let just for this remark ler $\Frob_\ell$ denote the Frobenius automorphism of $G_\ell$ and $\Frob_\lambda$ the Frobenius of
...
\] 
where $\bar{u}_{\ell, n}$ is the reduction of $u_{\ell, n}$ to the residue field of $\O_{K, \lambda}$, in particular 
$\loc_\ell P(m) \ne 0$ if and only if $\bigl[\loc_\ell P(n)\bigr]_s \ne 0$.
\end{proposition}

\begin{proof}
The result follows by the formula of the following Lemma, since $\ell$ is $M$-admissible and hence the two coefficients there are $p$-adic units.
\end{proof}

\begin{lemma} \label{lemma:finite-singular}
If $n = m \cdot \ell$, then
\[
\biggl( \frac{(-1)^{k/2 -1} \epsilon_n a_\ell}{p^M} - \frac{\ell+1}{p^M} \biggr) \bigl[ \loc_\ell  P(n) \bigr]_s =
\biggl(\frac{\ell+1}{p^M} \epsilon_n + \frac{a_\ell}{p^M}\biggr) \phi^{\fs}_\ell\bigl(\loc_\ell P(m)\bigr).
\]
\end{lemma}

\begin{remark}
The proof of this formula is an application of the abstract theory of \cite[Sec.~9]{nekovar:kuga-sato} and the formula itself already appears in the 
...
For instance the original formula of \cite[Prop.~10.2(4)]{nekovar:kuga-sato} there is a problem in the definition of $\epsilon_n$, 
firstly noticed by Besser \cite[see][proof of Prop.~3.2]{besser:finiteness-sha}, and a sign error: in front of the term $a_\ell/p^{M'}$ there should be a 
plus sign. In particular the latter misprint is carried on by all the other versions of this formula that we found in the literature. 
...
\[
\biggl(\frac{(-1)^{k/2 - 1} w_f a_\ell}{p^M} + \frac{\ell+1}{p^M} \biggr)  \bigl[ \loc_\ell  P(\ell) \bigr]_s =
...
is completion at the prime above $\ell$ of the maximal totally real subfield $K[1]^+$ of $K[1]$,
\[
...
\item $\lambda_1$ is totally ramified in $K[\ell]/K[1]$ (that is cyclic of order $\ell + 1$), i.e.~$\lambda_1 = \lambda_\ell^{\ell + 1}$: it follows that 
$[K[\ell]_\lambda : K[1]_\lambda] = \ell +1$ and $\Gal(K[\ell]_\lambda/K[1]_\lambda) \cong \Gal(\kappa_\ell/\kappa_1)$,  
that is cyclic being the Galois group of an extension of finite fields, 
moreover the ramification of $K[\ell]_\lambda/\Q_\ell$ is prime to $\ell$, thus $K[\ell]^\ur \subseteq \Q_\ell^t$;
\item $K_\lambda \subseteq K[1]_\lambda \subseteq \Q_\ell^\ur$, therefore $K_\lambda^\ur = K[1]_\lambda^\ur = \Q_\ell^\ur$;
\item $K[\ell]_\lambda^\ur = \Q_\ell^\ur \cdot K[\ell]_\lambda$; %by the maximality of $K[\ell]_\lambda^\ur$, indeed 
%$\Q_\ell^\ur \cdot K[\ell]_\lambda$ is unramified over $K[\lambda]$
\item $K[\ell]_\lambda^\ur/\Q_\ell^\ur$ is cyclic of order $\ell + 1$, since 
\[
\Gal\biggl(\frac{K[\ell]_\lambda^\ur}{\Q_\ell^\ur}\biggr) = \Gal\biggl(\frac{\Q_\ell^\ur \cdot K[\ell]_\lambda}{\Q_\ell^\ur}\biggr) 
\cong \Gal\biggl(\frac{K[\ell]_\lambda}{\Q_\ell^\ur \cap K[\ell]_\lambda}\biggr) = \Gal\biggl(\frac{K[\ell]_\lambda}{K[1]_\lambda}\biggr);
\]
...
where $(\sigma_{\ell, 0})_{\vert K[\ell]} = \sigma_\ell$. %and $\Gal(K[\ell]_\lambda^\ur/K[1]_\lambda^\ur) = \langle \sigma_{\ell, 0}^\ur\rangle$,
%for $(\sigma_{\ell, 0}^\ur)_{\vert K[\ell]_\lambda} = \sigma_{\ell, 0}$. 
Write $\sigma = \sigma_\ell$ and $\sigma_0 = \sigma_{\ell, 0}$. %$\sigma_0^\ur = \sigma_{\ell, 0}^\ur$. 
...
\]
the induced projections $\pi \colon G_0 \surj \hat{\Z}'(1) \rtimes 2\hat{\Z}$, $\pi \colon H_0 \surj (\ell + 1)\hat{\Z}'(1) \rtimes 2\hat{\Z}$
have the same kernel. Hence $G_0/H_0 \cong  \pi(G_0)/\pi(H_0) \cong \hat{\Z}'(1)/(\ell + 1)\hat{\Z}'(1)$,
so that we may assume that $\sigma_\ell$ is such that $\tau \bmod (\ell + 1)\hat{\Z}'(1)$ correspond to $\sigma_{\ell, 0}$ in this isomorphism.

By \cite[Lemma 4.1]{nekovar:kuga-sato} and \cite[B.2.8]{rubin:euler-systems}, we have the following commutative diagram:
\[
\begin{tikzcd}
\hone(G_0, T) = \hone(K, T)   & \hone(K_\lambda^t/K_\lambda, T) \ar[l, "\infl", "\sim"'] \ar[d, "\rotdxsim"]   & \hone(K_\lambda^\ur/K_\lambda, T) \ar[l,    "\infl", "\sim"'{xshift=3pt}] \ar[r, "\sim"]  \ar[d, "\rotdxsim"] & T/(\phi^2 -1)T\\
...
\]
for $a, b \in T$ and $a \equiv F(\phi^2) \bmod (\phi^2-1)T$. Indeed, $F(\tau) = 0$ as the inflation is an isomorphism and $\tau$ acts trivially on $T$ (see Rk.~\ref{remark:finite-singular-iso-proof}) and hence
\[
F(\tau^v) = F(\tau) + \tau \cdot F(\tau^{v-1}) = F(\tau^{v-1}) = \dots = F(\tau) = 0,
\] 
and 
\begin{align*}{}
F(\tau^u \phi^{2v}) &= F(\tau^u) + \tau^u \cdot F(\phi^{2v}) = F(\phi^{2}\phi^{2(v-1)}) = F(\phi^2) + \phi^2 \cdot F(\phi^{2(v-1)}) \equiv\\
...
(and the computations of \cite[Sec.~7]{nekovar:kuga-sato} show that $z(\sigma_0) \equiv -\sigma_0 a = -a \bmod p^M$), such that for any $g \in G$,
\[
\cores^G_H(\tilde{y})(g) - a_\ell \tilde{x} = (g-1)a.
\]
...
\]
and since $T$ is torsion free, but $p^M \mid a_\ell, \ell + 1$, 
\[
...
\]
since $\charpol(\Frob_\ell \vert T) = X^2 - a_\ell/\ell^{k/2-1} \, X + \ell$ by Rk.~\ref{rk:charpol-frobenius-ell} and therefore the above formula becomes
\[
...
\[
\biggl(\frac{\ell + 1}{p^M} \tau_c - \frac{a_\ell}{p^M}\biggr)\red_{p^M}(a_x) = \biggl(\frac{a_\ell}{p^M} (-1)^{1 - k/2}\tau_c - \frac{(\ell + 1)}{p^M}\biggr)\red_{p^M}(a).
\]
In fact, since $\tau_c^2 = \id$, then its minimal polynomial over $A[p^M]$ is $X^2 -1$, therefore it coincides with the minimal polynomial 
of $\phi$ over $A[p^M]$, since $a_\ell \equiv 0 \bmod p^M$ and $\ell \equiv -1 \bmod p^M$. Now we want to express $\red_{p^M}(a)$ in terms of $z$: note that
...
\[
\biggl(\frac{\ell + 1}{p^M} \tau_c - \frac{a_\ell}{p^M}\biggr)\phi^{\fs}_\ell\bigl(\res^G_{G_0}(x)\bigr) = \biggl(-\frac{a_\ell}{p^M} (-1)^{1- k/2}\tau_c + \frac{(\ell + 1)}{p^M}\biggr)[\res^G_{G_0}(z)]_s.
\]
...
and $[\loc_\ell P(\ell)]_s$. Finally recalling that $\tau_c \cdot P(1) = w_f P(1)$ and $\tau_c \cdot P(\ell) = -w_f P(\ell)$ we obtain the desired formula.
...
In this section we prove our main result Th.~\ref{th:main} following \cite{nekovar:kolyvagin}, 
...
(that will be defined in Def.~\ref{def:shat-K-infty})
...
All the notations of the previous sections are in force, in particular recall from Sec.~\ref{sec:mod-forms-gen-heegner-cycles} that 
$f = \sum_{n \ge 1} a_n q^n$ is a $\p$-odinary cusp new-form of level $\Gamma_0(N)$ and weight $k > 2$, $V$ is the self-dual representation of
Sec.~\ref{sec:selfdual-rep}, $K_\infty$ denotes the anticyclotomic $\Z_p$-extension of $K$ introduced in Sec.~\ref{sec:anticyclotomic-extension} and $K_n$ its 
$n$-th layer. We will need moreover some technical assumptions.

\begin{assumption}\label{assumption:1}
Take the following hypothesis on the prime $p$.
\begin{enumerate}[label=(\emph{\roman*})]  
\item $p \nmid 6N\phi(N)(k-2)!$, where $\phi$ is the Euler function;
\item the image of $\rho_{f, p} \colon G_\Q \to \GL_2(\O_F \otimes \Z_p)$ contains the set 
\[
...
\item $p \nmid c_f = \big[\O_F : \O_f \big]$, where $\O_f = \Z[a_i]_{i > 0}$.
\end{enumerate}
\end{assumption}

\begin{remark}
Note that the first three hypothesis say that $p$ is a non-exceptional prime as in Def.~\ref{def:admissible-primes}. 
For a discussion of the significance of these hypothesis see Rk.~\ref{rk:admissible-primes}; 
(\emph{iv}) is a technical hypotesis in the construction of the generalized Heegner cycles;
(\emph{v}) is important for the properties of $K_\infty$, as described in Sec.~\ref{sec:anticyclotomic-extension}; 
(\emph{vi}) is a technical hypothesis coming from 
\cite{longo-vigni:generalized}: assuming it $\O_f \otimes \Z_p = \O_F \otimes \Z_p$. 
As observed in \cite[Rk.~2.2]{longo-vigni:generalized}, once $f$ and $K$ are given the restrictions of 
...
\begin{theorem}[{\cite[Th.~1.1]{longo-vigni:generalized}}]\label{th:longo-vigni}
There is a finitely generated torsion $\Lambda$-module $M$ such that $\calX_\infty$ is pseudoisomorphic to $\Lambda \oplus M \oplus M$, i.e.~there exists a 
...
then $\calX_\infty$ is in fact free of rank one over $\Lambda$: this is the content Th.~\ref{th:main}. We will obtain it following   
the method of \cite{nekovar:kolyvagin}, i.e.~putting together the result of the Euler system argument of Sec.~\ref{sec:vanishing-of-sha} 
with an abstract Iwasawa theoretical one.

We need moreover some other technical assumptions. Recall by Ex.~\ref{ex:unramified-finite} the definition of $c_v(A)$ for a place $v \nmid p$ of $K$. Since $V$ is unramified at $v \nmid pN$, then $c_v(A) = 1$ for those primes. We need to assume that this is the case also for the others.
...
We now make a comparison of the Selmer groups introduced in Sec.~\ref{sec:selmer-groups-and-complexes}.
In Sec.~\ref{sec:selfdual-rep} we saw that there is an exact sequence
\[
0 \to V^+ \to V \to V^- \to 0
\]
...
For any a number field $E$ and any place $v \mid p$ we have
\begin{lemma}\label{lemma_ho(Ev, V-)}
$\ho(E_v, V^-) = 0$
\end{lemma}

\begin{proof}
By \cite[(3.1.5)]{nekovar-plater:parity}, in the case $p \nmid N$. It can be done also directly observing that $\sigma \in I_p$ acts as 
$\chi_p(\sigma)^{1 - k/2 }$ and there is a $\sigma$ such that this $\chi_p(\sigma)^{1 - k/2 } \ne 1$, as $k > 2$. But by loc. cit.~this holds even for $k = 2$ 
if $p \nmid N$.
\end{proof}
...
\begin{proof}
The first equality follows from the exact sequence \cite[Prop.~12.5.9.2(\emph{iii})]{nekovar:selmer-complexes} 
...
\]
of Prop.~\ref{prop:comparison-generalized-greenberg} plus Lemma \ref{lemma_ho(Ev, V-)}. 
For the equality $\honef(E ,V) = \hone_\Gr(E, V)$ it is enough to prove that $\honef(E_v, V) = \ker\big(\hone(E_v, V) \to \hone(I_v, V^-)\big)$ 
for any place $v \mid p$ of $E$. This is shown in \cite[Sec.~3.3.3]{longo-vigni:control-theorems}.
\end{proof}
...
By Rk.~\ref{rk:ramification-anticyclotomic} the inertia degree of $K_{v, n}/\Q_p$ is $1$ and hence the residue field of $K_{v, n}$ is $\F_p$. 
...
\] 
where $\alpha$ be the $\p$-adic unit root of the polynomial $X^2 - i_p(a_p) X + p^{k-1}$ defined in Sec.~\ref{sec:selfdual-rep}. Indeed we may see $A^-$ as $\K/\O$ together with the action given by $\delta^{-1}\chi_p^{1 - k/2}$ and hence
\[
...
for the choice of $\Frob_p$ made in Sec.~\ref{sec:notations}. 
...
therefore $\mathcal{A}_{K_{n, v}} = \mathcal{A}_{K_v}$. Thus $\honeur(K_{n, v}, A) = {\honef(K_{n, v}, A)}$, too.
\end{proof}
...
\begin{proof}
Observe that $\honetildef(K_n, A) = \hone_\str(K_n, A)$, by the exact sequence of Prop.~\ref{prop:comparison-generalized-greenberg} applied to $E = K_n$
...
\]
shows that $\hone_\str(K_n, A) = \hone_\Gr(K_n, A)$. Finally $\hone_\Gr(K_n, A) = \honef(K_n, A)$ by comparing each local condition: 
...
\begin{proof}
(1) follows by induction: Prop.~\ref{prop:ho-A[p]} is the base case. As $\ho$ and taking torsion commute, for $m > 1$ and $x \in \ho(K_n, A[p^m])$, then 
$px \in \ho(K_n, A[p^{m-1}])$. Therefore $x \in \ho(K_n, A[p])$, since $\ho(K_n, A[p^{m-1}]) = 0$ by inductive hypothesis, and so $x = 0$ again by 
Prop.~\ref{prop:ho-A[p]}. 
...
For $V$ the selfdual Galois representation attached to a $\p$-ordinary newform  
\[
...
\begin{proof}
We have $\tildecohomology^i_f(K_n, A) = \honef(K_n, A)$ by Prop.~\ref{prop:comparison-h1}. By Prop.~\ref{th:duality-rep}, there is an isomorphism 
$\tildecohomology^i_f(K_n, A) \cong D\big(\tildecohomology^{3 - i}_f(K_n, T^\ast(1))\big)$. 
It follows that $\tildecohomology^i_f(K_n, A) \cong D\big(\tildecohomology^{3 - i}_f(K_n, T^\ast(1))\big)$ for any $i \in \Z$. 
Therefore, as $\tildecohomology^{j}_f(K_n, A) = 0$ if $j < 0$ by definition, it follows that $\tildecohomology^i_f(K_n, A) = 0$ for $i \ne 0, 1, 2, 3$. 
...
and the latter vanishes by Prop.~\ref{prop:ho}(5). Similarly $\tildecohomology_f^0(K_n, A) = 0$ as it is a submodule of $\ho(K_n, A)$, that vanishes 
by Prop.~\ref{prop:ho}(2).
\end{proof}

Taking the direct limit over $n$ we have:
...
In particular Th.~\ref{th:exact-control-theorem-representations} becomes:
...
\begin{proof}
Apply Th.~\ref{th:exact-control-theorem-representations} to $K_\infty/K_n$ and combine with Prop.~\ref{prop:comparison} and \ref{prop:comparison-iwasawa}.
\end{proof}
...
\]
as $D(M^\Gamma) \cong D(M)_\Gamma$ \cite[see for instance][Lemma 5.15]{longo-vigni:control-theorems}.
\end{remark}
...
\begin{proof}
By Th.~\ref{th:exact-control-th} and Rk.~\ref{rk:coinvariants-new} $(\calX_\infty)_{\Gamma} \cong \calX \cong \O$, i.e.~there is an $x \in \calX_\infty$
...
It is left to show that $\calX_\infty$ it is not $\Lambda$-torsion. Suppose by contraddiction that this is the case. 
Consider the morphism $\eta  \colon \calX_\infty \to \Lambda \oplus M \oplus M$ of Th.~\ref{th:longo-vigni}. The $\Lambda$-module   
...
\]
is infinite, contradicting the fact that $\eta$ is a pseudoisomorphism.
\end{proof}
...
\]
and $\imajt(K_\infty) = \projlim_{n, \cores} \imajt(K_n) \subseteq \honef(K_\infty, T)$.
\end{definition}
...
\begin{remark}
Note that under our assumptions the restriction is indeed an isomorphism: it follows by Lemma \ref{lemma:res-iso} with the same 
argument of Rk.~\ref{rk:res-iso-T} and therefore $\imajt(K_n)$ and $\shat(f/K_n)$ are well defined. 
In particular, since $\ho(K_n, A[p]) = 0$ by Prop.~\ref{prop:ho-A[p]}, 
the same argument of Lemma \ref{lemma:res-iso} shows that $\imajt(K_n)$ is a free $\O$-module of finite rank.
Moreover each $\imajt(K_n)$ is endowed of an action of $\Gal(K_n/K)$ and hence $\imajt(K_\infty)$ has a structure of $\Lambda$-module, 
compatible with the inclusion into $\honef(K_\infty, T)$.
\end{remark}

Using this language we may enhance Th.~\ref{th:main-selmer-version}.
\begin{theorem}\label{th:main-sha}
Suppose that $\imajt(K) \otimes \K/\O$  and $\imajt(K_\infty)$ are nontrivial. If $\calX$ is free of rank $1$ over $\O$, then 
\[
\shat(f/K) = \shat(f/K_\infty) = 0; \quad \honef(K_\infty, A) = \imajt(K_\infty) \otimes \K/\O,
\] 
the Pontryagin dual $\calX_\infty$ of the latter group being free of rank $1$ over $\Lambda$.  
\end{theorem}

\begin{proof}
If $\calX \cong \O$, then $\honef(K, A) \cong \K/\O$. By Cor.~\ref{cor:selmer-image-aj-free}, $\imajt(K) = \O^{j}$ for some $j > 0$, therefore
\[
...
Let us now pass to $K_\infty$. By Th.~\ref{th:main-selmer-version} we already know that $\calX_\infty$ has rank $1$ over $\Lambda$, let us show 
...
\]
we have by duality a short exact sequence
\[
\begin{tikzcd}
0 \ar[r] & \calZ_\infty \ar[r] & \calX_\infty \ar[r] & \calY_\infty \ar[r] & 0 
\end{tikzcd}
\]
and hence our vanishing claim is equivalent to show that $\calX_\infty/\calZ_\infty = \calY_\infty$ is not $\Lambda$-torsion: 
indeed being $\calY_\infty$ a quotient of $\calX_\infty \cong \Lambda$, it is $\Lambda$-torsion if and only if $\calZ_\infty \ne 0$. 
Observe that 
\begin{align*}
...
Indeed there would be a nonzero ideal $J$ of $\O\llbracket T \rrbracket$ such that $\calY_\infty \cong \O\llbracket T \rrbracket/J$, moreover since 
$\calY_\infty[p]  = 0$ all power series $f = \sum_{j=0}^{\infty} a_j T^j\in J$ are such that $a_j \notin \p$ for some $j$ 
(otherwise the power series $f' = \sum_{j=0}^{\infty} (a_j/p) T^j \in \O\llbracket T \rrbracket$ would be $p$-torsion $\bmod J$): fix such a series and  
let $s$ be the smallest index $j$ such that $a_j \notin \p$. 
By the division lemma \cite[Lemma 5.3.1]{neu:cnf}, for any $g \in \O \llbracket T \rrbracket$, $f = gq + r$ for 
$q \in \O\llbracket T \rrbracket$ and $r \in \O[T]^{\le s-1}$ and in particular the powers $T, \dots, T^{s-1}$ would be a set of generators of 
$\O\llbracket T \rrbracket/(f)$, and hence of its quotient $\O\llbracket T \rrbracket/J$, as an $\O$-module. 
Thus $\calY_\infty$ would be of finite type over $\O$ and free since $\calY_\infty[p] = 0$, 
following the same argument of the proof of Cor.~\ref{cor:selmer-image-aj-free}.

But since $\calY_\infty = \imajt(K_\infty)^\ast$, then also $\imajt(K_\infty)$ would be a free $\O$-module of finite rank; 
...
\end{align*}
for any $n'>0, n \ge m$. But then $\imajt(K_\infty) = 0$, contraddicting our hypothesis. Let indeed $(\epsilon_n)_n \in \imajt(K_\infty)$, for any $n \ge m$ 
this fact implies that $\epsilon_n = \cores_{K_{n+n'}/K_n} (\epsilon_{n+n'})$ is divisible by $p^{n'}$ for any $k > 0$, but then $\epsilon_n = 0$ since $\imajt(K_n)$ 
is finite free over $\O$ and therefore it does not have infinitely divisible nonzero elements.
Hence $\epsilon_n = 0$ for any $n$.
\end{proof}
...
\] 
moreover $\shat(f/K_\infty) = 0$ and $\honef(K_\infty, A) = \imajt(K_\infty) \otimes \K/\O$, the Pontryagin dual $\calX_\infty$ of the latter 
group beeing free of rank $1$ over $\Lambda$.
\end{theorem}

\begin{proof}
This is the combination of Th.~\ref{th:besser} and Th.~\ref{th:main-sha}. Indeed, since $z_{f, K} \in \imajt(K)$ is non-torsion, 
then it is nontrivial in $\imajt(K) \otimes \K/\O$ and $z_{f, K} \cdot \K/\O$ has corank $1$ over $\O$. Moreover $\imajt(K_\infty) \ne 0$, 
since it contains an element $\kappa_1$, constructed by Longo and Vigni in \cite{longo-vigni:generalized} from the classes 
$\alpha_n = \cores_{K[p^{n+1}]/K_n}(z_{f, p^{n+1}}) \in \imajt(K_n)$: $\tilde{\kappa}_1 \ne 0$ by \cite[Th.~4.12]{longo-vigni:generalized}. 
\end{proof}
