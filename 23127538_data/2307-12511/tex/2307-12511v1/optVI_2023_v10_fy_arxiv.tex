...
\end{align}
where $H:X\to \mathbb{R}$ is a continuous and merely monotone mapping. In fact, note that when $f$ is convex and differentiable, \cref{eqn:optVI} is an instance of~\cref{eqn:bilevelVI} where $H(x):=\nabla f(x)$.
...
$$\mathcal{S}\triangleq \left\{x \in \mathbb{R}^n \mid \sum_{\ell=1}^d {A_\ell x^\ell} =b, \   h_j(x^{1},\ldots,x^d) \leq 0, \  j=1,\ldots,q\right\},$$
...
\noindent {\bf (ii) New guarantees for strongly monotone bilevel VIs.} When $H$ is strongly monotone, we show that a variant of IR-EG, called R-EG, under a prescribed weighted averaging scheme, can be endowed with a linear convergence speed in terms of an error bound on the dual gap function of the outer VI problem. We also show that under a suitable choice of the regularization parameter, complexities of $\mathcal{O}\left(\epsilon^{-p}\right)$ and $\mathcal{O}\left(\epsilon^{-(p+1)}+\epsilon^{-1}\right)$ can be obtained for the outer and inner VI problem, respectively, for any arbitrary $p\geq 1$. These guarantees appear to be new for both \cref{eqn:optVI} and \cref{eqn:bilevelVI} formulations.  
...
This implies that at the best NE, the game attains full efficiency, while at the worst NE, the efficiency deteriorates by more than $16$ times. 
 \subsection*{The methods}  We implement our three proposed methods and compare them with the methods in~\cite{facchinei2014vi}, where two schemes are developed for solving VI-constrained problem~\cref{eqn:optVI} in convex and nonconvex cases. Note that complexity guarantees are not provided in~\cite{facchinei2014vi}. Among these, we refer to Algorithm 1 and Algorithm 3 in~\cite{facchinei2014vi} by ISR-cvx and ISR-ncvx, respectively, where ISR denotes an inexact sequential regularization technique that is employed in these methods to compute the Tikhonov trajectory~\cite{FacchineiPang2003,doi:10.1137/20M1357378}. Of these, ISR-cvx is suitable for addressing~\cref{eqn:optVI} with a convex objective and is essentially a two-loop regularized gradient scheme where in the outer loop, the regularization parameter is updated and decreased by a factor $\rho \in (0,1)$, while in the inner loop, a gradient method is employed for inexactly computing the unique solution to the regularized VI. The ISR-ncvx is suitable for the nonconvex case and employs ISR-cvx at each iteration to inexactly compute a projection onto the set of equilibria. ISR-ncvx is characterized by two parameters $\sigma$ and $\beta$~\cite[Algorithm~3]{facchinei2014vi}.  
 
 \subsection*{The experiments and set-up} We run three sets of experiments as follows. (E1) We implement IR-EG, R-EG, and ISR-cvx for computing the best NE. All these methods share the same constant stepsize $\gamma:=1/(2\|A\|_{\mathcal{F}})$ where $\|A\|_{\mathcal{F}}$ denotes the Frobenius norm of $A$. For IR-EG and ISR-cvx, we let $\eta_0=0.01$ and for R-EG, we utilize the self-tuned constant regularization parameter given by \cref{cor: convergance rate H is SC2}. For ISR-cvx, we consider the choices $\rho \in \{0.1,0.5,0.9\}$. (E2) In the second experiment, we implement IPR-EG and ISR-ncvx for computing the worst equilibrium where for IPR-EG, we follow the parameter choices outlined in \cref{alg:ncvx-IR-GD} and for ISR-ncvx, we use $\sigma=\beta=0.5$. (E3) Lastly, we employ R-EG for approximating the PoS and use IPR-EG for approximating PoA. In all the experiments, we start the methods from the initial vector $(40,40) \in X$.
 
\subsection*{Results and insights} The results of experiments (E1), (E2), and (E3) are presented in \cref{fig:E1}, \cref{fig:E2}, and \cref{fig:E3}, respectively. A summary of the key insights is provided in the following. 
...
In computing the best NE in \cref{fig:E1}, R-EG outperforms the other schemes. This observation is indeed aligned with the accelerated rate statements provided in \cref{cor: convergance rate H is SC2} for R-EG. The IR-EG method performs relatively fast, but slower than R-EG. This is because unlike in R-EG where we utilize the strong convexity of the objective function $f$, IR-EG is devised primarily for addressing merely convex objectives and it does not leverage the strong convexity property of $f$. As observed, the performance of ISR-cvx appears to be significantly sensitive to the decay factor $\rho$, emphasizing on the importance of the update rules for the regularization parameter. Indeed, one of the key contributions in our work is to provide prescribed update rules for the regularization parameter in the single-timescale methods IR-EG and R-EG. 

In computing the worst NE in \cref{fig:E2}, the sequence of iterates generated by IPR-EG appears to successfully converge to the unique stationary point $x^*_{{\tiny \mbox{W-NE}}} = (60,10)$. This is indeed aligned with the theoretical guarantees in \cref{theorem:NC}. We observe, however, that the sequence of iterates generated by ISR-ncvx appears to converge to a vector in the set $\mbox{SOL}(X,F)$ that deviates from the worst NE. A similar observation was made when we used different values for the initial point, $\beta$, and $\sigma$. To plot the error in terms of the residual mapping, we use $\ln(\bullet +\exp(1))$ to better present the performance of the metric when the residual mapping is approaching zero. Notably, even though both methods start from the same initial point, the initial residual mapping takes different values between the two methods. This is because, unlike in IPR-EG where a constant stepsize is used, ISR-ncvx leverages a line search technique to find a suitable stepsize at each iteration. 

Lastly, in \cref{fig:E3}, we observe that the PoS ratio approximated by R-EG and PoA ratio approximated by IPR-EG converge to their true values, i.e., $1$ and $16.74$, respectively, as computed earlier. We note that in this particular experiment, the stationary point to the worst-NE seeking problem is unique. In other instances where a stationary point is not necessarily among the worst equilibria, IPR-EG may not necessarily converge to the worst equilibrium. 
