...
\newcommand{\mappendix}[1]{Appendix \textcolor{red}{#1}}
...
\author{
Kai Liu$^{1}$\footnotemark[1], $\;$ Sheng Jin$^{2}$, $\;$ Zhihang Fu$^{2}$\footnotemark[2], $\;$ Ze Chen$^{2}$, $\;$ Rongxin Jiang$^{1}$, $\;$ Jieping Ye$^{2}$
\vspace{0.5em}\\
...
\footnotetext[1]{This work was done when Kai Liu worked as a research intern at Alibaba DAMO Academy. Email: kail@zju.edu.cn.}
\footnotetext[2]{Corresponding author. Email: zhihang.fzh@alibaba-inc.com.}
...
\begin{abstract}
    ...
    Code is available at \url{https://github.com/alibaba/u2mot/}.
\end{abstract}

%%%%%%%%% BODY TEXT

\input{content/1_introduction.tex}
\input{content/2_relatedwork.tex}
\input{content/3_method.tex}
\input{content/4_experiment.tex}
