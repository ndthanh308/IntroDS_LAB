...
Figures~\ref{fig:teaser} and \ref{fig:local_basis} illustrate the example results edited by the latent basis found by our method.
...
More examples are referred to Appendix.
...
This aligns with the existing understanding that, after a certain timestep, 
the reverse diffusion trajectories of different samples are isolated from each other\cite{zhang2022gddim, wu2022uncovering}.
% the reverse diffusion trajectories between different samples become disentangled \cite{zhang2022gddim, wu2022uncovering}.
% This is consistent with the assumption in \citet{zhang2022gddim} that generative process 가 초기 여러 sample 이 섞일 수 있는 구간과 하나의 sample 만을 생성하는 구간으로 나뉜다는 가정과 consistent 하다.
% \yh{This is consistent with the assumption in \citet{zhang2022gddim} that the generative process initially follows a mixture of different delta distributions, but after a certain number of timesteps, the process is dominated by one mode and the other modes are ignored.}
% Therefore, we conclude that there is homogeneity across local latent subspace discovered from various samples at $t \approx T$.

% For a more in-depth discussion about parallel transport across different tangent space, see the \aref{appendix:tangent_latent_space}.
...
\uh{To verify this trend, we measure the geodesic distances between tangent spaces of different timesteps and plot the average distances of the same difference in timestep in \fref{fig:timestep} (c)}.
