...
\author{Seraphim Jarov$^1$, Sarah Thiele$^2$, Siddharth Soni$^3$, Julian Ding$^{1,4}$, Jess McIver$^1$, Raymond Ng$^4$, Rikako Hatoya$^5$, Derek Davis$^6$}
...
To aid in event validation and potentially reduce the rate of retracted event candidates, we propose a quick and accurate method for determining whether a glitch is present in the data that is robust to loud astrophysical events. 
...
We restructured Gravity Spy's CNN to provide compact binary coalescence (CBC) GW signal-vs-glitch classifications which will allow for rapid rejection of false candidate events using GW strain data from a single detector. 
...
To match the accuracy of Gravity Spy's glitch classification accuracy for the signal-vs-glitch case, we introduced a new structure to Gravity Spy's CNN. 
...
and the high mass classifier on simulated GW signals with total mass between $50 M_\odot$ and $300 M_\odot$. 
...
We first identified a $64$ second segment of quiet LIGO Livingston detector data to act as baseline noise we can inject a simulated waveform into using the PyCBC package~\cite{PyCBC}. 
For the purpose of our study, we simulated signals drawing parameters from a total mass range of $3M_\odot$ to $350M_\odot$, a signal-to-noise-ratio (SNR) range of $3$ to $35$, and a range of $-0.95$ to $0.95$ for both component spins. 
...
Equipped with our GW simulation workflow, we retrained Gravity Spy's CNN on enriched training sets that increased representation of simulated GW signals outside the mass and SNR ranges present in the original training set. 
Our enriched training set contains an equal representation of each class, with roughly 750 examples per classifier where possible, as equal representation of classes been shown to increase the robustness of a training set~\cite{hastie_ml}. 
...
Of the $273$ false negatives in the low mass range (Figure \ref{fig:mass_range_plots} \textbf{A}), $80.5\%$ were classified as a scratchy glitch while $16.5\%$ were classified as Gaussian noise (`no glitch'). 
...
However $79.4\%$ of GW sources with a total mass within $25M_{\odot}$ and $150M_{\odot}$ were correctly classified by the original Gravity Spy model. 
...
\begin{multicols}{2}
\begin{tabular}{cr|c}
\multicolumn{3}{c}{GSpySVG\_LM} \\
  & & \# of Examples \\
  \hline
  \multicolumn{1}{c}{\multirow{5}{*}{\rotatebox[origin=c]{90}{Class}}} & \cellcolor{black!15}Blip & \cellcolor{black!15}150 \\
  & LF Blip& 150 \\

  & \cellcolor{black!15} Scratchy& \cellcolor{black!15}150\\

  & No Glitch  & 115 \\

  & \cellcolor{black!15}LM GW & \cellcolor{black!15}150 \\
\end{tabular}
...
\begin{tabular}{cr|c}
\multicolumn{3}{c}{GSpySVG\_HM} \\
  & & \# of Examples \\
  \hline
  \multicolumn{1}{c}{\multirow{5}{*}{\rotatebox[origin=c]{90}{Class}}} & \cellcolor{black!15}Blip & \cellcolor{black!15}150 \\
  & LF Blip& 150 \\

  & \cellcolor{black!15} Koi Fish& \cellcolor{black!15}150\\

  & Tomte  & 150 \\

  & \cellcolor{black!15}HM GW & \cellcolor{black!15}150 \\
\end{tabular}
...
While the original Gravity Spy model was only able to classify $52.3\%$ of simulated signals in the same mass range correctly, GSpySVG\_HM and GSpySVG\_LM were able to classify $97.2\%$ of them correctly.

% Figure environment removed

We also observed similarly accurate results for glitch classifications by GSpySVG\_HM and GSpySVG\_LM. We found that both GSpySVG\_HM and GSpySVG\_LM were able to correctly classify $98.5\%$ of the $200$ test glitches as belonging to the correct glitch class. 
This shows our method is able to increase signal classification accuracy while maintaining strong performance in glitch classification.
...
\rowcolor{black!15} \cellcolor{white}
 \rotnine{\rlap{~True Class}}
   & GW     & 4 &  15  &  0  &  0  & \hl{481}     \\
    ...
    Both GSpySVG\_LM and GSpySVG\_HM correctly classified $98.5\%$ of the test glitches while GW classification accuracy was 98.2$\%$ and 96.2$\%$ respectively. 
    Moreover, below a total mass of $200M_{\odot}$, GSpySVG\_HM was able to classify $98.0\%$ of simulated signals correctly.}
\end{table}
...
This supplemented training set increased classification accuracy from $83.0\%$ to $99.4\%$ on the same test set of time-translated images. 
...
In their target mass ranges, GSpySVG\_LM and GSpySVG\_HM achieved an overall simulated signal classification accuracy of $97.2\%$, an increase from $52.3\%$ with the original Gravity Spy model. 
Furthermore, GSpySVG\_LM and GSpySVG\_HM classified test sets consisting of 200 glitch examples that often mimic the form of signals with $98.5\%$ accuracy each. Both GSpySVG\_LM and GSpySVG\_HM were also tested on O3b data where they were able to correctly classify all confirmed events as non-glitches, an improvement from $6/21$ with the original Gravity Spy model. In addition to our main improvements in the $<250M_\odot$ range, we also investigated signals and glitches that may appear with a slight offset in time. For signals with a time offset in the range of $\pm0.1$ seconds, we were able to increase classification accuracy from $83.0\%$ to $99.4\%$ by adding random time offsets to each of the training images in the GSpySVG\_LM and GSpySVG\_HM training sets. 
