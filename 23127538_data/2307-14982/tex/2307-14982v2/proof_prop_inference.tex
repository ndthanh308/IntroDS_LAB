...
such that $y_{i,j} = 1 - y^\prime_{i,j}$ which implies
$\one(\norm{\bx_{i,j}}_1 > 0) \neq y^\prime_{i,j}$.
In this case, $h(\bx, \by^\prime) = 0$ is
contradicting the assumption that $\{(\bx, \by)\} \in \mA_{+}$. 
By \eqref{general_model},
...
The model form for $\sepmodel$ given in \eqref{general_model} implies 
\beno
\dfrac{\sepmodel(\bX = \bx, \, \bY = \by)}{\sepmodel(\bY = \by)}
\= \dfrac{f(\bx, \nat) \, g(\by) \, \psi(\nat, \by)}{g(\by)} \s \\
\= \exp(\log f(\bx, \nat) + \log \psi(\nat, \by)),
