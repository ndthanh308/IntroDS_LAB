\begin{lemma}
\label{lem:s_hetero}
Consider a multilayer network model following the form of equation \eqref{general_model} 
with maximum interaction term $H \leq K$
and 
is defined on a set of $N \geq 3$ nodes and $K \geq 1$ layers.  
...
\ee
the sufficient statistic vector $s : \mbX \mapsto \mbR^p$ and the natural parameter vector $\nat \in \mbR^p$. 
...
\ee
\item The exponential family outlined above is minimal, full, and regular. 
\een
\end{lemma}
...
\ee
provided $h(\bx, \by) = 1$.
The form of \eqref{general_model} 
...
From the form of $f(\bx, \nat)$ in \eqref{general_model},
\beno
\scalebox{0.9}{$\log f(\bx, \nat)$}
\,=\, \scalebox{0.8}{$\dsum_{\{i,j\} \subset \mN} \,
...
($\{k_1,\ldots, k_h\} \subseteq \{1, \ldots K\}$, 
$h \in \{1, \ldots, H\}$) 
...
Consider multilayer networks satisfying \eqref{general_model} 
with maximum interaction term $H \leq K$ 
...
note that the form of \eqref{general_model} and Proposition \ref{prop:inference} suggests that 
\be
