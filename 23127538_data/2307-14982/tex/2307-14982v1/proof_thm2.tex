...
belongs to a minimal exponential family with sufficient statistic vector $s(\bx) = (s_1(\bx), \ldots, s_p(\bx))$ 
...
\ee
Define $\tilde\bR \coloneqq  (\mcI(\truth) \, \norm{\bY}_1)^{-1/2} \, \bR$.
...
\beno
\mbP\left((\mcI(\truth) \, \norm{\bY}_1)^{1/2} \, (\mle - \truth) - \tilde\bR \in \mA\right)
...
Hence, 
\beno
&& \left|\mbP((\mcI(\truth) \, \norm{\bY}_1)^{1/2} \, (\mle - \truth) - \tilde\bR \in \mA) - \Phi(\bZ \in \mA)\right| \s\\
&\leq&\dfrac{83}{\xi_{\epsilon^\star}^{3/2}} \,
\sqrt{\dfrac{p^{7/2}}{\mbE \, \norm{\bY}_1}}
+  \dfrac{4}{\mbE \, \norm{\bY}_1} + \dfrac{8 \, \left[D_{g}\right]^{+}}{\left(\mbE \, \norm{\bY}_1 \right)^2}. \s 
\ee
We lastly handle the term $\tilde\bR$
by showing that $\norm{\tilde\bR}_2$ is small with high probability. 
We first use standard vector/matrix norm inequalities to bound 
\beno
\norm{\widetilde\bR}_2
\= \norm{(\mcI(\truth) \, \norm{\bY}_1)^{-1/2} \, \bR}_2 
&\leq& \dfrac{\mnorm{\mcI(\truth)^{-1/2}}_2}{\sqrt{\norm{\bY}_1}} \; \norm{\bR}_2
&\leq& \dfrac{\norm{\bR}_2}{\sqrt{\xi_{\epsilon^\star}  \norm{\bY}_1}},  
\ee
...
which is bounded below by $\sqrt{\xi_{\epsilon^\star}}$. 
...
\label{eq:Rbound1}
\norm{\tilde\bR}_2
&\leq& \dfrac{1}{\sqrt{\xi_{\epsilon^\star} \, \norm{\by}_1}} \; 
\sqrt{\dsum_{i=1}^{p} \, R_i^2}  
&\leq& \dfrac{1}{\sqrt{\xi_{\epsilon^\star} \, \norm{\by}_1}} \, 
\sqrt{\dsum_{i=1}^{p} \, \norm{\by}_1^2 \, \norm{\mle - \truth}_1^4} \s\s\\
&\leq& \dfrac{1}{\sqrt{\xi_{\epsilon^\star} \, \norm{\by}_1}} \,
\sqrt{p \, \norm{\by}_1^2 \, \norm{\mle - \truth}_1^4}  
&\leq& \dfrac{\sqrt{p} \, \norm{\by}_1 \, \norm{\mle - \truth}_1^2}{\sqrt{\xi_{\epsilon^\star} \, \norm{\by}_1}} \s\s\\
&\leq& \dfrac{\sqrt{p} \, \sqrt{\norm{\by}_1} \, p \, \norm{\mle - \truth}_2^2}{\sqrt{\xi_{\epsilon^\star}}} 
...
\ee
whereas Theorem \ref{thm1} established there exists $N_0 \geq 3$ such that,
for all $N \geq N_0$, 
the event 
\be
\label{eq:event2}
...
\ee
occurs with probability at least $1 - 3 \, (\mbE \, \norm{\bY}_1)^{-1}$.
...
and define $\mcR$ to be the corresponding values of $\tilde\bR$ in
the event $(\bX, \bY) \in \mE_1 \cap \mE_2$,
under which we have the bound
\be
\label{eq:Rbound2}
\norm{\tilde\bR}_2
&\leq& \dfrac{p^{3/2}\, \sqrt{\norm{\by}_1}}{\sqrt{\xi_{\epsilon^\star}}} \, 
\dfrac{3 \, p \, \log N}{\mbE \norm{\bY}_1} \; \dfrac{1 + [D_{g}]^{+}}{\xi_{\epsilon^\star}^2} \s\s\\
\= \dfrac{3 \, p^{5/2} \, \sqrt{2 \, \mbE \, \norm{\bY}_1} \, \log \, N}{\mbE \norm{\bY}_1} \, 
\dfrac{1 + [D_{g}]^{+}}{\xi_{\epsilon^\star}^2} \s\s\\
\= \dfrac{3 \, \sqrt{2} \, (1 + [D_{g}]^{+})}{\xi_{\epsilon^\star}^2} \; 
\dfrac{p^{5/2} \, \log N}{\sqrt{\mbE \, \norm{\bY}_1}}.  
%&\leq& \dfrac{\sqrt{2} \, p^2}{\sqrt{\xi_{\epsilon^\star} \, \mbE \, \norm{\bY}_1}} \,
%\dfrac{3 \, p \, \log N}{\mbE \, \norm{\bY}_1} \, \dfrac{(1 + [D_{g}]^{+})}{\xi_{\epsilon^\star}^2} \s\s\\
%\= \dfrac{3 \, \sqrt{2} \, (1 + [D_{g}]^{+})}{\xi_{\epsilon^\star}^{5/2}} \,
%\dfrac{p^3\, \log N}{(\mbE \, \norm{\bY}_1)^{3/2}},
\ee
combining the bounds in \eqref{eq:Rbound1}, \eqref{eq:event1}, and \eqref{eq:event2}
...
\begin{lemma}
\label{lem:Taylor}
Consider multilayer networks satisfying \eqref{general model} defined on a set of $N \geq 3$ nodes 
and $K \geq 1$ layers and denote by the log-likelihood function by $\ell(\nat; \bx,\by)$.
Then,
...
\ee
For further notation simplicity, denote by $L_i$ the set of layer indices $\{k_1, \ldots, k_h\} \subseteq \{1, \ldots, K\}$ that define the $i^{th}$ component $s_{i,v,w}(\bX_{v,w})$ of the sufficient statistic vector $\bs_{v,w}(\bX)$ 
...
& \dfrac{\partial\, P_{i,v,w}(\nat ; \bX, \by) }{\partial\,\theta_k} \s \\
 \leq & 
\dfrac{\dsum_{\mbX_{v,w }^{(-L_i)}} \, \exp\,\left(\sum_{j \in I_{S_{i,v,w}}}\,\theta_j + \sum_{j \in I_{S_{i,v,w}}^c}\, \theta_j\, s_{j,v,w}(\bx)\right) \, \left(Z(\nat) - \dfrac{\partial \, Z(\nat) }{ \partial \, \theta_k}\right)}{Z(\nat)^2} \s \\
 = & \dfrac{\dsum_{\mbX_{v,w }^{(-L_i)}} \, \exp\,\left(\sum_{j \in I_{S_{i,v,w}}}\,\theta_j + \sum_{j \in I_{S_{i,v,w}}^c}\, \theta_j\, s_{j,v,w}(\bx)\right) \, \left(\dsum_{\mbX_{v,w}} \, \exp\left( \sum_{j = 1}^p \, \theta_j \, s_{j,v,w}(\bx)\right) \, (1-s_{k,v,w}(\bx))\right)}{Z(\nat)^2} \s \\
 \leq  &
\dfrac{\dsum_{\mbX_{v,w }^{(-L_i)}} \, \exp\,\left(\sum_{j \in I_{S_{i,v,w}}}\,\theta_j + \sum_{j \in I_{S_{i,v,w}}^c}\, \theta_j\, s_{j,v,w}(\bx)\right) \, Z(\nat)}{Z(\nat)^2} \s \\
 \leq &
1.
\ee
The first inequality is obtained because $s_{k,v,w} (\bx) \leq 1$, and the last inequality is due to the fact that
\beno
\dsum_{\mbX_{v,w }^{(-L_i)}} \, \exp\,\left(\sum_{j \in I_{S_{i,v,w}}}\,\theta_j + \sum_{j \in I_{S_{i,v,w}}^c}\, \theta_j\, s_{j,v,w}(\bx)\right) & \leq  & \dsum_{\mbX_{v,w}} \, \exp\left( \sum_{j = 1}^p \, \theta_j \, s_{j,v,w}(\bx) \right).
\ee
...
& \cov_{\nat}(s_{i,v,w}(\bX_{v,w}), \, s_{j,v,w}(\bX_{v,w}) \,|\, \bY = \by) \s \\
 = & \mbE \,[ s_{i,v,w}(\bX) \, s_{j,v,w}(\bX) \,|\, \bY = \by] - \mbE\, [s_{i,v,w}(\bX)\,|\, \bY = \by] \, \mbE \, [s_{j,v,w}(\bX)\,|\, \bY = \by] \s \\
...
& \leq & \dsum_{\{v,w\} \subset \mN} \, \left| \dfrac{\partial \, \cov_{\nat}(s_{i,v,w}(\bX_{v,w}), \, s_{j,v,w}(\bX_{v,w}) \,|\, \bY = \by)}{\partial \, \theta_k} \, \right| \s \\
& \leq & 2 \, \norm{\by}_1
\ee
