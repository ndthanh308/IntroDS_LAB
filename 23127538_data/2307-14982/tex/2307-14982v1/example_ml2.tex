...
by considering a network separable multilayer network model using the Markov random field specification 
...
i.e.,
the edge variable between nodes $i$ and $j$ in layer $k$.  
The conditional log-odds of edge $X_{i,j}^{(k)}$ takes the form:    
\beno
\log \, \dfrac{\mbP(X_{i,j}^{(k)} = 1 \,|\, \bX_{i,j}^{(-k)} = \bx_{i,j}^{(-k)}, Y_{i,j} = 1)}
