...
For continuous transformations, their Jacobian determinant $|J|$ has to be positive everywhere to avoid folding of space.
...
In a recent study,~\citet{liu2022finite} showed that when approximating the Jacobian using forward or backward difference, it is implicitly assumed that the digital transformations are linearly interpolated on a tetrahedron mesh grid. 
Importantly, they showed that the Jacobian determinant, when approximated using central difference, results in the checkerboard and self-intersection problems. Consequently, it consistently underestimates non-diffeomorphic spaces.
