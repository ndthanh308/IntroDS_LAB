...
\leq 
\underbrace{\overbrace{\frac{L C(\bm{\theta}_0,f_*)}{\delta (1-c)(2 - L_n\overline{\alpha})}}^{C_1}
\frac{1}{K}}_{B(\bm{\theta}_0, K)} 
+ 
\underbrace{\overbrace{\frac{L_n \overline{\alpha} \sigma^2}{2-L_n \overline{\alpha}}}^{C_2} \frac{1}{b}}_{V(\sigma^2, b)},
\end{align}
...
\begin{table}[ht]
\caption{Relationship between batch size $b$ and number of steps $K$ to achieve an $\epsilon$--approximation defined by $\min_{k\in [0:K-1]} \mathbb{E}[\| \nabla f (\bm{\theta}_k)\|^2 ] \leq \frac{C_1}{K} + \frac{C_2}{b} = \epsilon^2$ for SGD with a constant learning rate $\alpha \in (0,\frac{2}{L_n})$ and for SGD with the Armijo-line-search learning rate ($[0:K-1] := \{0,1,\ldots,K-1\}$. $f := \frac{1}{n} \sum_{i\in[n]} f_i$ is bounded below by $f_*$, $L_i$ is the Lipschitz constant of $\nabla f_i$, $L_n := \frac{1}{n} \sum_{i\in [n]} L_i$, $C(\bm{\theta}_0, f_*)$ is a positive real number depending on the initial point $\bm{\theta}_0 \in \mathbb{R}^d$ and $f_*$, $\sigma^2$ is the upper bound of the variance of the stochastic gradient, and $\overline{\alpha}$ is the upper bound of the learning rates)}\label{table:1}
\centering
...
\midrule
\multirow{2}{*}{Armijo $(c,\delta \in (0,1))$} 
& $C_1$ & $\displaystyle{\frac{L C(\bm{\theta}_0,f_*)}{\delta (1-c)(2-L_n \overline{\alpha})}}$ & \multirow{2}{*}{$\displaystyle{K = \frac{C_1 b}{\epsilon^2 b - C_2}}$} & \multirow{2}{*}{$\displaystyle{N = \frac{C_1 b^2}{\epsilon^2 b - C_2}}$} & \multirow{2}{*}{$\displaystyle{b^\star = \frac{2 C_2}{\epsilon^2}}$} \\
& $C_2$ & $\displaystyle{\frac{L_n \overline{\alpha} \sigma^2}{2-L_n \overline{\alpha}}}$ & & & \\
\bottomrule 
\end{tabular}
\end{table}
...
b^\star = \frac{2 C_2}{\epsilon^2} = \frac{2 L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha}) \epsilon^2}
\end{align}
...
We assume that a stochastic first-order oracle (SFO) exists such that, for a given $\bm{\theta} \in \mathbb{R}^d$, it returns a stochastic gradient $\mathsf{G}_{\xi}(\bm{\theta})$ of the function $f$, where a random variable $\xi$ is supported on $\Xi$ independently of $\bm{\theta}$. We make the following standard assumptions.
...
\item[{\em (ii)}] {\em [Lower bound on learning rate determined by backtracking line search method]} If $\alpha_k$ can be computed by Algorithm \ref{algo:2}, then there exists a lower bound of $\alpha_k$ such that $0 < \underline{\alpha} := \frac{2 \delta (1-c)}{L} \leq \frac{2 \delta (1-c)}{L_{B_k}} \leq \alpha_k$. 
\end{enumerate}
\end{lem}
...
\underbrace{\overbrace{\frac{L_n \overline{\alpha} \sigma^2}{2-L_n \overline{\alpha}}}^{C_2} \frac{1}{b}}_{V(\sigma^2, b)},
\end{align*}
where $\delta, c \in (0,1)$, $L := \max_{i\in [n]} L_i$, $L_n := \frac{1}{n} \sum_{i\in [n]} L_i$ $(\leq L)$, $f_* := \frac{1}{n} \sum_{i\in [n]} f_{i,*}$, $\underline{\alpha} := \frac{2 \delta (1-c)}{L}$ $(< \frac{2}{L_n})$, $\overline{\alpha} < \frac{2}{L_n}$, and $C(\bm{\theta}_0,f_*)$ is defined by $\mathbb{E} \left[ f(\bm{\theta}_{0}) - f_* \right]$ or $\mathbb{E} \left[ f(\bm{\theta}_{0}) + f(\bm{\theta}_{k_0 +1}) - 2f_* \right]$ for some $k_0 \in [0:K-1]$.
\end{thm}
...
Here, we compare Theorem \ref{thm:1} with the convergence analysis of SGD using a constant learning rate. SGD using a constant learning rate $\alpha \in (0,\frac{2}{L_n})$ satisfies that
...
\end{align}
(The proof of (\ref{sgd_c}) is given in Appendix \ref{a5}). We need to set a constant learning rate $\alpha \in (0,\frac{2}{L_n})$ depending on the Lipschitz constant $L_n$ of $\nabla f$. However, since computing $L_n$ is NP-hard \citep{NEURIPS2018_d54e99a6}, it is difficult to set $\alpha \in (0,\frac{2}{L_n})$. Meanwhile, it is sufficient to set $c,\delta \in (0,1)$ in Algorithms \ref{algo:1} and \ref{algo:2} without computing the Lipschitz constant of $\nabla f$.
...
\end{align*}
where $c > 1 - \frac{L}{\rho L_n}$, $\overline{\alpha} < \frac{2}{\rho L_n}$, $\Delta := (\overline{\alpha} + \frac{2(1-c)}{L}) - \rho (\overline{\alpha} - \frac{2(1-c)}{L} + L_n \overline{\alpha}^2)$, and $\bm{\theta}^\star$ is a local minimizer of $f$. Theorem \ref{thm:1} is a convergence analysis of Algorithm \ref{algo:1} without assuming the strong growth condition or limiting the hyperparameter $c$. Moreover, Theorem \ref{thm:1} shows that using large batch size is appropriate for SGD using the Armijo line search (Algorithm \ref{algo:1}).
...
\subsection{Critical batch size minimizing SFO complexity}
...
\end{align}
where the positive constants $C_1$ and $C_2$ are defined as in Theorem \ref{thm:1}.
...
= \frac{2 L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})\epsilon^2}
\end{align}
...
b^\star \leq \frac{2 \delta (1-c) \overline{\alpha} \sigma^2}{\{\underline{\alpha} - \delta (1-c) \overline{\alpha} \} \epsilon^2}.
\end{align}
\end{enumerate}
\end{thm}
...
\label{sec:4}
The numerical results in \citep{NEURIPS2019_2557911c} showed that SGD with the Armijo-line-search learning rate performs better than other optimizers in training deep neural networks. Hence, here, we verify whether the numerical results match our theoretical results (Theorems \ref{thm:2} and \ref{thm:3}), that is, the relationship between $K$ and $b$ and the relationship between $N$ and $b$ for Algorithm \ref{algo:1}.

The metrics were the number of steps $K$ and the SFO complexity $N = Kb$ indicating that the training accuracy is higher than a certain score. We used Algorithm \ref{algo:1} with the Armijo-line-search learning rate computed by Algorithm \ref{algo:2} with $\gamma =2$, $\delta = 0.9$, $\overline{\alpha} = 10$ (see \url{https://github.com/IssamLaradji/sls} for the setting of parameters), and $c$ $(= 0.05, 0.1, 0.15, 0.2, 0.25, 0.3)$, where $n$ is the number of training samples. The stopping condition was 200 epochs. The experimental environment consisted of eight NVIDIA DGX A100 GPUs and two Dual AMD Rome7742 2.25-GHz, 128-Core CPUs. The software environment was Python 3.8.2, Pytorch 1.6.0, and CUDA 11.6. The code is available at \url{https://anonymous.4open.science/r/armijo_linesearch-C1C3}.
...
We trained a two-hidden-layer MLP with widths of 512 and 256 on the MNIST dataset ($n = 60000$). Figure \ref{fig5} plots the number of steps needed for the training accuracy to be more than $0.97$ for Algorithm \ref{algo:1} versus the batch size, and Figure \ref{fig6} plots the SFO complexities of Algorithm \ref{algo:1} versus the batch size. As in Figures \ref{fig1}--\ref{fig4}, these figures show that Algorithm \ref{algo:1} decreases the number of steps as the batch size increases and there are critical batch sizes that minimize the SFO complexities.
...
\begin{align*} 
b^\star \approx \frac{2 \delta (1-c) \overline{\alpha} \sigma^2}{\{\underline{\alpha} - \delta (1-c) \overline{\alpha} \} \epsilon^2}
\approx 
2 \delta (1-c) X, 
\text{ i.e., }
X \approx 18.71. 
\end{align*}
Let us estimate the critical batch size using $X \approx 18.71$ and Theorem \ref{thm:3}(iii). For example, when using Algorithm \ref{algo:1} with $c=0.25$ for training ResNet-34 on the CIFAR-100 dataset, the upper bound of the critical batch size is 
\begin{align*}
\frac{2 \delta (1-c) \overline{\alpha}}{\underline{\alpha} - \delta (1-c) \overline{\alpha}} X \approx 77.71 \approx 2^6 = b^\star,
\end{align*}
which implies that the estimated critical batch size $77.71$ is close to the measured critical batch size $b^\star = 2^6 = 64$ in Figure \ref{fig4}. 

\section{Conclusion}
This paper presented a convergence analysis of SGD using the Armijo line search for nonconvex optimization. The analysis indicates that the upper bound of the expectation of the squared norm of the full gradient becomes smaller as the number of steps and the batch size grow. Moreover, we investigated the relationship between the number of steps and the batch size needed for nonconvex optimization of SGD using the Armijo line search. We showed that the number of steps needed for nonconvex optimization is monotone decreasing and convex with respect to the batch size; i.e., the steps decrease in number as the batch size increases. We also showed that the SFO complexity needed for nonconvex optimization is convex with respect to the batch size and that there exists a critical batch size at which the SFO complexity is minimized. In addition, we gave an upper bound on the critical batch size and showed that it can be estimated by using some parameters. Finally, we provided numerical results that support our theoretical findings. We trained ResNet-34 on the CIFAR-10 and CIFAR-100 datasets and MLP on the MNIST dataset and found that SGD using the Armijo line search decreases the number of steps as the batch size increases. Moreover, we showed that the batch sizes estimated from the upper bound of the critical batch size are close to those of the numerical results.
...
(ii) Since $\frac{\alpha_k}{\delta}$ does not satisfy the Armijo condition (\ref{armijo}), we have that
...
\end{align*}
From $L_{B_k} = \frac{1}{b} \sum_{i\in [b]} L_{\xi_{k,i}} \leq L := \max_{i\in [n]} L_i$ $(k\in\mathbb{N})$, we also have that $\alpha_k \geq \frac{2 \delta (1-c)}{L_{B_k}} \geq \frac{2 \delta (1-c)}{L}$.
...
\|\nabla f_{B_k}(\bm{\theta}_k)\|^2.
\end{align}
From $0< \alpha_k \leq \overline{\alpha}$, we have that, for all $k\in\mathbb{N}$,
\begin{align}\label{ineq:3}
\langle \nabla f(\bm{\theta}_{k}),
\nabla f_{B_k}(\bm{\theta}_k) \rangle
\leq 
\frac{f(\bm{\theta}_{k}) - f(\bm{\theta}_{k+1})}{\alpha_k} + \frac{L_n \overline{\alpha}}{2} 
\|\nabla f_{B_k}(\bm{\theta}_k)\|^2.
\end{align}
...
+ 
\mathbb{E}\left[\| \nabla f(\bm{\theta}_k) \|^2 | \bm{\theta}_k \right]\\
&\leq \frac{\sigma^2}{b} + \mathbb{E}\left[\| \nabla f(\bm{\theta}_k) \|^2 | \bm{\theta}_k \right].
\end{split}
\end{align}
Taking the total expectation on both sides of (\ref{ineq:3}) thus ensures that, for all $k \in \mathbb{N}$,
\begin{align}\label{total}
\mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
\leq 
\mathbb{E} \left[ \frac{f(\bm{\theta}_{k}) - f(\bm{\theta}_{k+1})}{\alpha_k} \right]
+ \frac{L_n \overline{\alpha}}{2}
\left( 
\mathbb{E} \left[ \|\nabla f(\bm{\theta}_k)\|^2 \right]
+ \frac{\sigma^2}{b}
\right).
\end{align}
Let $L := \max_{i\in [n]} L_i$. A discussion similar to the one showing (\ref{lip}) leads to the finding that the Lipschitz constant of $\nabla f_{B_k}$ is $L_{B_k} = \frac{1}{b} \sum_{i\in [b]} L_{\xi_{k,i}} \leq L$ $(k\in\mathbb{N})$. Moreover, Lemma \ref{lem:1}(ii) implies that there exists $\underline{\alpha} > 0$ such that, for all $k\in \mathbb{N}$, $\underline{\alpha} := \frac{2\delta (1-c)}{L} \leq \alpha_k$. Let $K \geq 1$. Consider the case where, for all $k\in [0:K-1]$, $f(\bm{\theta}_{k}) - f(\bm{\theta}_{k+1}) \geq 0$. Then, (\ref{total}) guarantees that, for all $k \in [0:K-1]$,
\begin{align*}
\mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
\leq 
\frac{L}{2\delta (1-c)} \mathbb{E} \left[ f(\bm{\theta}_{k}) - f(\bm{\theta}_{k+1}) \right]
+ \frac{L_n \overline{\alpha}}{2}
\left( 
\mathbb{E} \left[ \|\nabla f(\bm{\theta}_k)\|^2 \right]
+ \frac{\sigma^2}{b}
...
which implies that
\begin{align*}
\left(1 - \frac{L_n \overline{\alpha}}{2} \right)
\mathbb{E} \left[ \|\nabla f(\bm{\theta}_k)\|^2 \right]
\leq
\frac{L}{2\delta (1-c)} \mathbb{E} \left[ f(\bm{\theta}_{k}) - f(\bm{\theta}_{k+1}) \right]
+ \frac{L_n \overline{\alpha} \sigma^2}{2b}.
\end{align*}
...
\leq 
\frac{L}{2\delta (1-c)} \mathbb{E} \left[ f(\bm{\theta}_{0}) - f(\bm{\theta}_{K}) \right]
+ \frac{L_n \overline{\alpha} \sigma^2 K}{2b}.
\end{align}
Since $f$ is bounded below by $f_* := \frac{1}{n} \sum_{i\in [n]} f_{i,*}$ and $\overline{\alpha} < \frac{2}{L_n}$, we have 
\begin{align*}
\frac{1}{K} \sum_{k=0}^{K-1} \mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
\leq 
\underbrace{\frac{L \mathbb{E} \left[ f(\bm{\theta}_{0}) - f_* \right]}{\delta (1-c)(2 - L_n\overline{\alpha})K}}_{A_K} 
+ \frac{L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})b}.
\end{align*}
Consider the case where there exists $k_0 \in [0:K-1]$ such that $f(\bm{\theta}_{k_0}) - f(\bm{\theta}_{k_0 +1}) < 0$. If $f(\bm{\theta}_k) - f(\bm{\theta}_{k +1}) < 0$ holds for all $k \in [0:K-1]$, (\ref{total}) ensures that
\begin{align*}
\mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
<
\frac{L_n \overline{\alpha}}{2}
...
\right),
\end{align*}
which, together with $\overline{\alpha} < \frac{2}{L_n}$ and $A_K \geq 0$, implies that 
\begin{align*}
\frac{1}{K} \sum_{k=0}^{K-1} \mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
<
\frac{L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})b}
\leq A_K + \frac{L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})b}.
\end{align*}
Hence, we may assume without loss of generality that, for all $k \neq k_0$, $f(\bm{\theta}_k) - f(\bm{\theta}_{k +1}) \geq 0$. A discussion similar to the one showing (\ref{sum}) implies that
\begin{align*}
\left(1 - \frac{L_n \overline{\alpha}}{2} \right)
\sum_{k=0}^{K-1} \mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
\leq 
\frac{L}{2\delta (1-c)} \mathbb{E} 
\left[ f(\bm{\theta}_{0}) - f(\bm{\theta}_{k_0}) + f(\bm{\theta}_{k_0 +1}) - f(\bm{\theta}_{K}) \right]
+ \frac{L_n \overline{\alpha} \sigma^2 K}{2b}.
\end{align*}
Since $f$ is bounded below by $f_* := \frac{1}{n} \sum_{i\in [n]} f_{i,*}$ and $\overline{\alpha} < \frac{2}{L_n}$, we have
\begin{align*}
\frac{1}{K} \sum_{k=0}^{K-1} \mathbb{E}\left[ \| \nabla f(\bm{\theta}_k)\|^2 \right]
\leq 
\frac{L \mathbb{E} \left[ f(\bm{\theta}_{0}) + f(\bm{\theta}_{k_0 +1}) - 2f_* \right]}{\delta (1-c)(2 - L_n\overline{\alpha})K} 
+ \frac{L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})b}.
\end{align*}
...
(iii) Lemma \ref{lem:1}(iii) ensures that 
\begin{align*}
L_n := \frac{1}{n} \sum_{i\in [n]} L_i \leq L = \frac{2\delta (1-c)}{\underline{\alpha}}.
\end{align*}
Hence, 
\begin{align*}
b^\star 
...
= \frac{2 L_n \overline{\alpha} \sigma^2}{(2-L_n \overline{\alpha})\epsilon^2}
\leq
\frac{4 \delta (1-c)\overline{\alpha} \sigma^2}{\underline{\alpha}}
\frac{\underline{\alpha}}{2 \{ \underline{\alpha} - \delta (1-c)\overline{\alpha} \}\epsilon^2}
= 
\frac{2 \delta (1-c) \overline{\alpha} \sigma^2}{\{ \underline{\alpha} - \delta (1-c) \overline{\alpha} \} \epsilon^2}.
\end{align*}
