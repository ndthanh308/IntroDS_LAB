...
\thanks{Corresponding author: Zhineng~Chen.}% <-this % stops an unwanted space
%\thanks{Manuscript received September, 2023.}}
}

% The paper headers

\markboth{For review only}%
...
\begin{abstract}
% Scene text recognition (STR) methods have struggled to attain high accuracy and fast inference speed. Autoregressive (AR)-based STR models use the previously recognized characters to decode the next character iteratively. They show superiority in terms of accuracy. However, the inference speed is slow also due to this iteration. Alternatively, parallel decoding (PD)-based STR models infer all the characters in a single decoding pass, offering faster inference speed but generally worse accuracy, as it is difficult to build a robust recognition context in such a pass. In this paper, we first present an empirical study of AR decoding in STR. In addition to constructing new AR models with top-tier accuracy, we find out that the success of AR decoder lies not only in modeling linguistic context, but also in providing guidance on visual context perception. Consequently, we propose the Context Perception Parallel Decoder (CPPD) to predict the character sequence in a single PD pass. CPPD devises a character counting module and a character ordering module. Given a text instance, the former infers the occurrence count of each character, while the latter deduces the content-free character reading order and placeholders. Meanwhile, the character prediction task associates the placeholders with characters. These components together robustly tell what the character sequence is and where the characters appear, providing a more complete context compared to AR decoding due to considering the entire character sequence. We construct a series of CPPD models and also plug the proposed modules into existing STR decoders. Experiments on both English and Chinese benchmarks demonstrate that the CPPD models achieve highly competitive accuracy while running approximately 8x faster than their AR-based counterparts. Moreover, the plugged models also achieve significant accuracy improvements on large and challenging datasets. Code is available at \url{https://github.com/PaddlePaddle/PaddleOCR}.
...
%It achieved highly competitive accuracy.
%encode the linguistic information, which directly
...
\begin{align}
    %&C_t^{AR} = y_{0:t-1} = \{c_0 + p_1, ..., c_{t-1} + p_{t-1}\} \\
    &C_t^{AR} = c_{y_0}:c_{y_{t-1}} \\
    &C_t^{AR-P} = c_{y_{k_0}}:c_{y_{k_{t-1}}} \\
    &C_t^{AR-L} = c_{y_{t-1}} \\
    &C_t^{AR-L-P} = c_{y_{k_{t-1}}}
    %&C_t^{AR} = y_{0:t-1} \\
    %&C_t^{AR-P} = y_{k_0:k_{t-1}} \\
    %&C_t^{AR-L} = y_{t-1} \\
    %&C_t^{AR-L-P} = y_{k_{t-1}}
    % &C_t^{PD} = p_{t} \\
    % &C_t^{PD-P} = \tilde{y}_{t}
\end{align}
...
\multirow{5}{*}{Ours}    & SVTR-T-NRTR & SVTR-Tiny & \multirow{4}{*}{$32\times100$}    & 97.3          & 93.4          & 95.0          & 85.5          & 88.2          & 89.6          & 91.49                                                                       & 8.56                                                                     & 45.5                                                              \\
                        %97.3 	93.4 	95.0 	85.5 	88.2 	89.6 	91.92 	91.49 
...
\multirow{4}{*}{CTC}  & CRNN  \cite{shi2017crnn} (2017)  & ResNet+BiLSTM  & $32\times100$       & 7.5                  & 0.9                                                       & 20.7                 & 25.6                 & 13.9                 & 25.6                                                   & 32.0                 & 18.03                                             \\
                      ...
                      %0.767518545888217,0.4521548542574518,0.5523385239160521,0.6302952422298429,0.7378395405063832,0.4890776639675041,0.6423040277457723
                      \hline
...
%71.92910110499133       22.132943592893035      60.57906391337345       67.90757294085272       78.2691087917302        63.8686123616957267.1468706246573        61.69046761859912
                      \hline
...
%This observation is in line with our anticipation.
