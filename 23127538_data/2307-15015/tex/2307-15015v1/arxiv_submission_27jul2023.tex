...
%%%%%%%%%%%%%%%%%%%%PREAMBLE%%%%%%%%%%%%%%%%%%%%
%%%BASIC Packages
\usepackage{microtype,subcaption,colortbl}
...
%\usepackage[normalem]{ulem} % for some combining accents & other exotic symbols
%\usepackage{sectsty} % to change title styles
\usepackage{graphicx,enumerate,bm} % for \includegraphics command and options
\DeclareGraphicsRule{*}{mps}{*}{}

\usepackage{hyperref} % for bookmarks in Acrobat Reader and using hyperlinks, ,pagebackref=true
\hypersetup{bookmarksnumbered=true, bookmarksopen=true,
  ...
  pdfkeywords={SU(2), }{two dimensions, }{QCD, }{large
    representations}, pdfstartview=FitH, pdfauthor={NSP},
  pdfsubject={SU(2) QCD2}, pdftitle={SU(2) QCD2}, plainpages=false,
  unicode=true, urlcolor=blue}
...
\usepackage[backend=bibtex8,sorting=none,style=numeric-comp,maxnames=8,maxalphanames=4,
minalphanames=4,giveninits=true,isbn=false]{biblatex}% style=ext-alphabetic,,articlein=false
...
\title{{\bf Confinement in $\text{SU}(2)$ QCD$_{1+1}$ with \\ Quarks in Large
  Representations}}

\author{Anurag Kaushal\footnote{anuragkaushal314@gmail.com}, Naveen S.~Prabhakar\footnote{naveen.s.prabhakar@gmail.com}, and Spenta R.~Wadia\footnote{spenta.wadia@icts.res.in}}
\date{}
...
\begin{center}
	{\it International Centre for Theoretical Sciences}\\
	{\it Tata Institute of Fundamental Research, Shivakote, Bengaluru 560089, India.}
\end{center}

\vspace{4ex}

\begin{abstract}
  We consider $\text{SU}(2)$ quantum chromodynamics in $1+1$
  ...
  integer spin representations of $\text{SU}(2)$ are real, and analyse
  the two cases separately.

  The theory is reformulated in terms of global color non-singlet
...
\cite{tHooft:1974pnl,tHooft:1973alw}, and was further analysed
...
solve the model in a 't Hooft-like large $J$ limit where
$J \to \infty$ and the gauge coupling $g \to 0$ with
$\lambda = g^2 J^2$ kept fixed.
...
variables.\footnote{Such non color-singlet variables have been used in
...
integral equation. We confirm that there is no bound state with zero
mass by solving the equation numerically.
...
$\tr(T^a T^b) = \frac{1}{2}\delta^{ab}$. The action can be written
...
\end{align}
We choose the direction $x^+$ as time. The component $A_+$ is then
non-dynamical and the Gauss' law is the field equation for $A_+$:
...
\begin{equation}\label{psipeom}
  2\i D_-\psi_+ + m \psi_- = 0 \quad\longrightarrow\quad \psi_+ = \frac{\i m}{2} \frac{1}{\partial_-}\psi_-\ \ \text{in}\ \ \text{$A_- = 0$ gauge.}% 2\i (D_-\psi_+)^\dag - m\psi_-^\dag = 0\ .
\end{equation}
...
\begin{equation}\label{psitldef}
  \tl\psi_-(u,x)\Big|_{u=u_m} = \sqrt{J}\,\psi^m_-(x)\ .
\end{equation}
Upon canonical quantization, the fermions $\psi^m_-(x)$ satisfy the
...
\begin{equation}
  \psi_-(\theta,x) := \tl\psi(u,x)\Big|_{u = \cos\theta} = \sqrt{J}\, \psi^m_-(x)\ ,
\end{equation}
where we have omitted the $\tl{}$ on the left-hand side to avoid
cluttering of notation. The equal time anticommutation relations
\eqref{contcomm} become
\begin{equation}\label{contcommfin}
  \{\psi_-(\theta,x), \psi^*_-(\theta',y)\} = \frac{1}{\sin\theta}\delta(\theta-\theta') \delta(x^- - y^-)\ .
\end{equation}
Thus, in the continuum limit, the fermion $\psi^m_-(x)$ has to be
replaced by
...
\end{equation}
Another way to motivate the prefactor of $\sqrt{J}$ in
\eqref{psitldef} is as follows. The fermion components $\psi^m_-$ are
discrete for finite $J$ and the neighbouring components need not be
close in any sense since we integrate over all possible configurations
of $\psi^m_-$ in the path integral. In the large $J$ limit, if we
assume that the components coalesce into a continuum field
$\tl\psi(u)$, then the difference $\psi^{m+1} - \psi^m$ is
parametrically suppressed by the $\sqrt{J}$ factor. The notion of a
derivative along the $u$ (or equivalently, the $\theta$) direction is
then conceivable.
...
\end{equation}
Equivalently, we have, in terms of the $\theta$ coordinate
\eqref{thetarepl},
\begin{equation}
...
\begin{equation}\label{hamil1}
  H  = \int \ud x^- \ud y^- \left(-\frac{g^2}{8} (\psi^\dag_- T^a \psi_-)(x) |x^- - y^-| (\psi^\dag_- T^a \psi_-)(y)  -\frac{\i m^2}{8} \psi^\dag_-(x)\sgn(x^- - y^-) \psi_-(y)\right)\ .
\end{equation}
...
We also note an identity satisfied by the equal-time bilocal operator
$M(x^+,x^-,x^+,y^-;\theta,\theta')$ by virtue of its definition
\eqref{Mdef}:
...
%  &\int \ud z^- \int_0^\pi\ud\varphi\sin\varphi\, M(x,z;\theta,\varphi)M(z,y;\varphi,\theta')\nonumber\\
   \int \ud z^- \int_0^\pi\ud\varphi\sin\varphi\, \psi(\theta,x)\psi^*(\varphi,z) \psi(\varphi,z) \psi^*(\theta',y)\\ = \int \ud z^- \int_0^\pi\ud\varphi\sin\varphi\, \psi(\theta,x)\psi^*(\varphi,z)\left[ \frac{1}{\sin\varphi}\delta(\varphi-\theta')\delta(z^--y^-) -  \psi^*(\theta',y)\psi(\varphi,z)\right]%\ ,\nonumber\\
...
\begin{equation}\label{conscharge}
  Q = \int \ud x^- \sum_m \psi^*_{m-}(x^+,x^-) \psi^m_-(x^+,x^-) = \int \ud x^- \int_0^\pi \ud\theta\sin\theta\,\psi^*_-(\theta,x^+,x^-) \psi_-(\theta,x^+,x^-)\ .
\end{equation}
This will be useful for our semiclassical analysis of the model in the
't Hooft-like large $J$ limit.


\subsection{The $W_\infty$ algebra and coadjoint orbits}
...
\cite{Dhar:1994ib,Dhar:1993jc,Dhar:1992rs}, we describe the coadjoint
orbit of the $W^{2J+1}_\infty$ algebra which is relevant for us. We
only give a summary here and refer the reader to the detailed
description in \cite{Dhar:1994ib,Dhar:1993jc,Dhar:1992rs}. The
coadjoint orbit is described by certain constraints on the bilocal
field $M(x^-,y^-;\theta,\theta')$. To describe these constraints, it
is convenient to introduce a matrix notation for the bilocal
...
\begin{equation}\label{olMconst}
  {\mM}{}^2 = {\mM}\ ,\quad \str(\bm{1} - {\mM}) = {Q}\ ,
\end{equation}
where ${Q}$ is the $\text{U}(1)$ charge defined in
\eqref{conscharge}.\footnote{Regarding the notation, ${\mM}{}^2$
  corresponds to matrix multiplication in the $(x^-,\theta)$ space,
  the $\bm{1}$ is the identity matrix in $(x^-,\theta)$ space and the
  $\str$ is a trace over the $x^-$ and $\theta$ labels.} The above
constraints can be shown to be invariant under the action of the
$W^{2J+1}_\infty$ algebra on the ${\mM}$. The coadjoint orbit we
choose corresponds choosing $Q = 0$.
...
\begin{equation}\label{hamilcoadj}
  H = \str\left[\frac{\i m^2}{8} \mS {\mM} + \frac{\lambda}{8} {\mM} \wt{\mM}\right]\ .
\end{equation}
where the trace is in the $(x,\theta)$ space, and we have defined
$\mS$ and $\wt{{\mM}}$ that have the matrix elements
\begin{align}
...
\begin{equation}\label{kinetic}
  \i \int_\Sigma \ud s \ud x^+ \str ({\mM} [\partial_+ {\mM}, \partial_s {\mM}])\ ,
\end{equation}
...
\end{equation}
where the $\str$ is on the $x^-$ and $\theta$ labels of
the field ${\mM}$ \eqref{Mmatrix}. In analogy to the usual action
$\int \ud t\, L = \int \ud t (p\dot{q} - H)$, the first term in
\eqref{Maction} is the $p\dot{q}$ term whereas the second term is the
Hamiltonian \eqref{hamilcoadj}.
...
\begin{equation}\label{actvar}
  \delta \mc{S} =  \i  \int_\Sigma \ud s \ud x^+ \str\left( \partial_s (\i\bm{\epsilon} \partial_+ {\mM}) - \partial_+ (\i\bm{\epsilon} \partial_s {\mM})\right) - \int \ud x^+ \str\left(\frac{\i m^2}{8} \i\bm{\epsilon} [{\mM}, \mS] + \frac{\lambda}{4}  \i\bm{\epsilon}[ {\mM}, \wt{{\mM}}]\right)\ .
\end{equation}
...
${\mM}^2 = {\mM}$ and $\str(\bm{1} - \mM) = 0$. The following choice
...
in them. Due to the trace $\str$ in the action \eqref{Maction} which
...
We have described the representation theory of $\text{SU}(2)$ in
Appendix \ref{SU2rep} and just state the relevant facts
here. Irreducible representations of $\text{SU}(2)$ with integer spin
$J$ are real, and hence fermions in such representations can be taken
to be Majorana. The Majorana condition on a Dirac fermion $\Psi$ in an
integer spin $J$ representation is
...
\int_0^\pi \ud\theta\sin\theta\,\tr\, \langle x |\log(\i \slashed\cD + m)| x\rangle
   = -\frac{\i}{2}\int_0^\pi \ud\theta\sin\theta \int_0^\infty \frac{\ud t}{t (2\pi t)^{d/2}} \e^{- m^2 t}\,  t\eta \coth t\eta\ ,
\end{equation}
where $\tr$ is the trace over Dirac spinor indices. Differentiating
once w.r.t.~$m$, we get
\begin{align}\label{deteval3}
  \int_0^\pi \ud\theta\sin\theta\,\tr\, \langle x |\frac{-\i}{\i\slashed\cD + m}| x\rangle  &=  m \int_0^\pi \ud\theta\sin\theta\int_0^\infty \frac{\ud t}{(2\pi t)^{d/2}} \e^{- m^2 t}\, t\eta \coth t\eta\ .
\end{align}
...
\footnote{We write this in analogy with the case of a single free
  ...
  \begin{equation}
    \tr\,\langle x | \frac{-\i}{\i \slashed{\partial} + m} | x\rangle = \tr\,\langle x | \frac{-\i(\i\slashed{\partial} - m)}{- \slashed{\partial}^2 - m^2} | x\rangle =  \int \frac{\ud^d p}{(2\pi)^d} \frac{2^{d/2} \i m}{p^2 - m^2 + \i\varepsilon} = \int \frac{\ud^d p}{(2\pi)^d} \int_0^\infty \ud\mu\frac{2^{d/2} \i \varrho(\mu)}{p^2 - \mu + \i\varepsilon}\ ,\nonumber
  \end{equation}
...
In this work, we have analysed $\text{SU}(2)$ gauge theory coupled to
...
effective action), the above results agree with well-known facts about
the the $\text{SU}(2)$ theory with quarks in the adjoint
representation: (1) the model with massive quarks is confining
...
massless quarks does not have a zero-mass bound state. The
$\text{SU}(2)$ theory coupled to an adjoint quark is known to have a
screening vs. confinement transition at zero quark mass. Does this
...
S.~R.~W.~would like to thank David Gross and Luis Alvarez-Gaume for
early discussions, and the Infosys Foundation Homi Bhabha Chair at
ICTS-TIFR for its generous support. The authors would also like to
thank R.~Loganayagam for discussions, and Gautam Mandal for comments
on the manuscript. This work is supported by the Department of Atomic
Energy under project no. RTI4001.
...
$C\gamma^\mu C^{-1} = -\gamma^{\mu,T}$, that is, $C^{\alpha\gamma} (\gamma^\mu)_\gamma{}^\delta (C^{-1})_{\delta\beta} = - (\gamma^{\mu,T})^\alpha{}_\beta$. It is numerically given by $C = \i\sigma_2$, that is,
\begin{equation}
...
\begin{equation}
  \sum_{m=-J}^J \psi^{(m)} \zeta^{J+m}\quad \longrightarrow\quad   \sum_{m=-J}^J (-1)^{-J-m} \psi^*_{(m)} \zeta^{J-m}\quad \longrightarrow \quad  \zeta^{2J} \sum_{m=-J}^J (-1)^{-J-m} \psi^{(m)} (-\zeta)^{-J+m}\ .
\end{equation}
...
Next, let us look at the third term in \eqref{intfluc}:
...
\begin{align}\label{III1}
  \text{III} &= -\frac{1}{J} \bigg[\int_{p<0} \int_{r>0} \int_{p}^r \frac{\ud k}{-2\pi k^2} (-1) \str\big( W(r-k,p-k) \wt{W}(p,r)\big)\nonumber\\
             &\qquad\qquad- \int_{p>0} \int_{r<0} \int_r^p\frac{\ud k}{-2\pi k^2} (+1) \str\big( W(r-k,p-k) \wt{W}(p,r)\big)\bigg]\ ,\nonumber\\
%             &= -\frac{1}{J} \int_{p>0} \int_{r>0}\bigg[ \int_{-p}^r \frac{\ud k}{-2\pi k^2} (-1) \str\big( W(r-k,-p-k) \wt{W}(-p,r)\big)\nonumber\\
%             &\qquad\qquad\qquad\quad -  \int_{-r}^p \frac{\ud k}{-2\pi k^2}(+1) \str\big( W(-r-k,p-k) \wt{W}(p,-r)\big)\bigg]\ ,\nonumber\\
             &= \frac{1}{J} \int_{p>0} \int_{r>0} \int_{-p}^r \frac{\ud k}{-2\pi k^2}  \bigg[ \str\big( W(r-k,-p-k) \wt{W}(-p,r)\big) + \str\big( W(-p-k,r-k) \wt{W}(r,-p)\big)\bigg]\ ,\nonumber\\
           &= \frac{1}{J} \int_{p>0} \int_{r>0} \int_{-p}^r \frac{\ud k}{-2\pi k^2}  \bigg[ \str\big( W^{\rm pn}(r-k,p+k) \wt{W}{}^{\rm np}(p,r)\big) + \str\big( W^{\rm np}(p+k,r-k) \wt{W}{}^{\rm pn}(r,p)\big)\bigg]\ .
\end{align}
...
$W^{\rm  np}(s,t)$: %switched the limits of integration of the commutator
             %term
\begin{multline}\label{Weomapp}
  -\i \partial_+ W^{\rm np}(t,s) - \frac{m^2}{4} \left(\frac{1}{s} + \frac{1}{t}\right) W^{\rm np}(t,s) 
  -\frac{\lambda}{4\pi} \int_{-t}^s \frac{\ud k}{-k^2} \Big(\wt{W}{}^{\rm np}(t+k,s-k) - W^{\rm np}(t,s)\Big) = 0\ .
\end{multline}
Displaying the $\theta$ labels explicitly, we get
...
\end{align}
where, in going to the second equality, we have used %B_1 was defined with wrong sign
\begin{equation}
...
\end{equation}
The finite part in \eqref{detevalx} as $\epsilon \to 0$ is %added the missing Euler gamma term in the THREE equations below
\begin{multline}
