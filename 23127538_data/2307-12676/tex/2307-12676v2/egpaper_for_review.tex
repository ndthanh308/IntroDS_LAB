...
In the study, we highlight one-class anomaly detection application whether anomalous class or not, and demonstrate clear examples on imbalanced vision datasets: blood smear, lung infection, wooden, concrete deterioration, and disaster damage. 
...
\item {\it Lung infection $N_2=1300, M_2=2000.$ }
	\begin{itemize}
		\item COVID-19 in chest X-ray images (\cite{Khan2022,Kaggle2021}). 
	\end{itemize}
\item {\it Wood deterioration $N_3=1300, M_3=2000.$ }
	\begin{itemize}
		\item decayed wooden sleeper in rural railway. 
	\end{itemize}
\item {\it Concrete damage $N_4=1300=650\times2, M_4=2000.$ }
	\begin{itemize}
		\item 2-classes : crack on pavement and deck (SDNET \cite{Dorafshan2018}). 
	\end{itemize}
\item {\it Disaster damage $N_5=1300=325\times4, M_5=2000.$ } 
	\begin{itemize}
...
Compared with the balanced case of positive ratio 1/1, we found that there was applicable range from the imbalanced ratio 1/2 to the ratio 1/16 where the accuracy of $F_1$ was consistently less than 95\%. 
However, extremely imbalanced range from 1/32 to 1/1300, that accuracy were inferior to those of applicable range, e.g. the $F_1$ was less than 95\%. The rare positive ratio 1/32 represent the imbalanced data that contains quite a little 41 anomalous images and relatively large 1300 normal images. In the situation, much more anomalous images should be acquired and added to the initial dataset.   
...
\begin{table}[h]
\caption{Imbalanced data studies using our deeper FCDD-VGG16 for Disaster damage detection $N_5=1300$.}
...
\textbf{Positive ratio} & \textbf{AUC} & \boldmath{$F_1$} & \textbf{Precision} & \textbf{Recall} \\
		\hline
\textbf{1/1(ano.$N_5$)} & \textbf{0.9844} & \textbf{0.9491} & \textbf{0.9410} & \textbf{0.9575} \\ \hline
...
\textbf{1/$N_5$(ano.1)} &\textbf{0.6870} & \textbf{0.5783} & \textbf{0.6721} & \textbf{0.5075} \\ \hline
\end{tabular}
\end{table}
...
% our tackle 
We are going to tackle the opportunities of unseen damage vision mining and improving the robustness of damage detection applications to overcome the combination of imbalance type in damage vision mining data, based on our initial damage vision studies on regression\cite{Yasuno2021L2norm,Yasuno2020RainCode}, 
image classification\cite{Yasuno2020Natural,Yasuno2022River}, 
contrastive metric learning\cite{Yasuno2023MNPair}, 
semantic segmentation\cite{Yasuno2019Popouts,Yasuno2020GeneraSyn,Yasuno2020Perpixel,Yasuno2021Road},
