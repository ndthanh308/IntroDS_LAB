...
Institute for Infrastructure Paradigm Shift, Yachiyo Engineering,Co.,Ltd.
{\tt\small tk-yasuno@yachiyo-eng.co.jp}
}
...
Finally, the applicability of the damage learning methods, limitations, and future works are mentioned. 
\end{abstract}
...
To address imbalanced image classification between majority classes and minority classes, a previous common preprocess is resampling-based approaches to the initial setting dataset. These are categorized into synthetic oversampling from minority classes \cite{Piras2012,Kim2019}, undersampling on majority classes \cite{Koziarski2020}, and a combination of both \cite{Batista2004,Zhu2020}. 
...
Object & Bounding Box & Object Detection & Handling scale \& objective imbalance\cite{Zou2018,Agarwal2018,Oksuzy2020} \\
...
\begin{enumerate}
\item {\it Wood deterioration $N_1=1300, M_1=2000.$ }
	\begin{itemize}
		\item decayed wooden sleeper in rural railway. 
	\end{itemize}
\item {\it Concrete damage $N_2=1300=650\times2, M_2=2000.$ }
	\begin{itemize}
		\item 2-classes : crack on pavement and deck (SDNET \cite{Dorafshan2018}). 
	\end{itemize}
\item {\it Disaster damage $N_3=1300=325\times4, M_3=2000.$ } 
	\begin{itemize}
...
% =============== Wooden Deterioration
\subsection{Wooden Deterioration}
\subsubsection{Backbone Studies of Supervised Detection}
The number of training images $N_1$ in each class is 1300, and the number for calibration and test are 300 and 400 images respectively on the dataset of wooden sleeper deterioration. 
...
\begin{table}[h]
\caption{Imbalanced data studies using our deeper FCDD-VGG16 for wooden sleeper deterioration $N_1=1300$.}
...
\textbf{Positive ratio} & \textbf{AUC} & \boldmath{$F_1$} & \textbf{Precision} & \textbf{Recall} \\
		\hline
\textbf{1/1(ano.$N_1$)} & \textbf{0.9463} & \textbf{0.8701} & \textbf{0.8379} & \textbf{0.9050} \\ \hline
...
\textbf{1/$N_1$(ano.1)} &\textbf{0.6136} & \textbf{0.5823} & \textbf{0.5724} & \textbf{0.5925} \\ \hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{Imbalanced data studies using our deeper FCDD-VGG16 for Concrete crack detection $N_2=1300$.}
...
\textbf{Positive ratio} & \textbf{AUC} & \boldmath{$F_1$} & \textbf{Precision} & \textbf{Recall} \\
		\hline
\textbf{1/1(ano.$N_2$)} & \textbf{0.9338} & \textbf{0.8265} & \textbf{0.9482} & \textbf{0.7325} \\ \hline
...
\textbf{1/$N_2$(ano.1)} &\textbf{0.7927} & \textbf{0.6908} & \textbf{0.8235} & \textbf{0.5950} \\ \hline
\end{tabular}
\end{table}
...
\begin{table}[h]
\caption{Imbalanced data studies using our deeper FCDD-VGG16 for Disaster damage detection $N_3=1300$.}
...
\textbf{Positive ratio} & \textbf{AUC} & \boldmath{$F_1$} & \textbf{Precision} & \textbf{Recall} \\
		\hline
\textbf{1/1(ano.$N_3$)} & \textbf{0.9844} & \textbf{0.9491} & \textbf{0.9410} & \textbf{0.9575} \\ \hline
...
\textbf{1/$N_3$(ano.1)} &\textbf{0.6870} & \textbf{0.5783} & \textbf{0.6721} & \textbf{0.5075} \\ \hline
\end{tabular}
\end{table}
...
We developed an imbalanced vision application to automate one-class anomaly detection. To ensure feasibility of imbalanced damage vision datasets on the targets of wooden/concrete deterioration and disaster damage. We assessed imbalanced anomaly detection approach for deeper FCDDs with promising backbones of VGG16, compared with the ResNet101 and Inceptionv3. Additionally, we visualized damage-mark heatmaps using direct Gaussian upsampling of the receptive field of the FCN.

Our experiments produced significantly high accuracy in terms of the AUC, $F_1$, precision, and recall using our deeper FCDD-VGG16 architecture under the imbalanced datasets with relatively small positive ratio. 
...
Thus, our imbalanced studies presented an applicability for deeper FCDDs that offers an imbalanced damage detection tool for visual inspections and damage-explainability.

\subsection{Limitation and Robustness for Unseen Damage}
% limitations
This study discovered the feasibility of imbalanced anomaly detection highlighting wooden/concrete deterioration and disaster damage of 4 classes: building collapse, traffic incidents, fire/smoke, and flooding area. However, this is limited on specific target of narrow damage variation. The wooden deterioration images had few variations collected at spring season. Winter season may influence the unseen background noise: shadow from west sunshine, decayed grass, iced, and snowy. 

% future works
Several promising directions exist for future research to improve the usability of visual inspection applications. To address the challenges of unseen data, this issue remains for infrequent defects such as cracks of concrete sleepers, and holes on ballast tracks. 
To overcome this challenge, the anomaly score generated by our deeper FCDDs can be used in edge devices for effective data acquisition of rare classes. By unseen data mining only the anomalous vision that have damage-marks with significantly higher anomaly scores than a particular threshold, the data acquisition process can be made more efficient.

% open discussion
From our key results of imbalanced studies, we found that the positive ratio of over $1/128$ to $1/8$ brought a promising accuracy improvement as a damage vision mining merit. 
In contrast, ranging with the positive ratio over $1/4$, the accuracy has not been improved. This implies that over-mining phase has not been beneficial that we should early stop further data acquisition.  
% our tackle 
We are going to tackle the opportunities of unseen damage vision mining and improving the robustness of damage detection applications to overcome the combination of imbalance type in damage vision mining data, based on our initial damage vision studies on regression\cite{Yasuno2020RainCode,Yasuno2021L2norm}, 
image classification\cite{Yasuno2022River,Yasuno2020Natural}, 
contrastive metric learning\cite{Yasuno2023MNPair}, 
semantic segmentation\cite{Yasuno2019Popouts,Yasuno2020Perpixel,Yasuno2021Road},
 and anomaly detection\cite{Yasuno2019Color,Yasuno2021Bridge,Yasuno2020Generative}.    

\section*{Acknowledgment}
The authors wish to thank MathWorks and Computer Vision Toolbox Team, Takuji Fukumoto for providing helpful MATLAB resources for Automated Visual Inspection. We also thank Nakasha Creative Co.,Ltd., for providing the opportunity of a feasibility study in rural railway track. 
%-------------------------------------------------------------------------
% Rererence
{\small
