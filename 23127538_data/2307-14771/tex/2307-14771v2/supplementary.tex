...
\title[Supplementary]{Supplementary Material}
...
\section{Proof of gap closing in the $N_y=4M+2$ case}
Given the Hamiltonian with PBC along the $\Vec{a}_1$ direction in Eq.~(4) of the main text, the corresponding Schr\"odinger equation
\begin{equation}
...
\begin{align*}
    ...
    &\vdots
\end{align*}
so that in the end $\psi_{2\ell}^{A/B}=0$ for any $\ell \in \mathbb N$. On the other hand, the boundary condition $\psi_{\Tilde{N}+1}^{A/B}(k)=0$ imposes no more conditions if $\Tilde{N}=2M+1$, since then $\Tilde{N}+1$ is even, while for $\Tilde{N}=2M$ it forces $\psi_{2\ell+1}^{A/B}=0$ for any $\ell \in \mathbb N$ as well. Thus we conclude that if $N_y=4M$ no zero energy solution exists for $m=0$\footnote{Note that the dispersion relation can only be gapless at $k=\pi$, since that is the position of the Dirac point for the edge states of the Haldane model on a strip with zigzag edges (see Fig.~1\textbf{b} of the main text).}.
...
\section{Large $m$ limit and jump of the Zak phase}
...
\begin{equation}
    \Psi^{m>0}_{n_\ell}(k_N) = e^{-i\frac{2\pi}{a}t_\ell}  \Psi^{m>0}_{n_\ell}(k_0),
\end{equation}
where $t_\ell$ is the $x$-position of the $\ell$-th site in the strip unit cell. With our conventions (see main text) we get
\begin{align*}
    e^{-i\frac{2\pi}{a}t_{4r}} &= 1, \\
    e^{-i\frac{2\pi}{a}t_{4r+1}} &= 1, \\
    e^{-i\frac{2\pi}{a}t_{4r+2}} &= -1, \\
    e^{-i\frac{2\pi}{a}t_{4r+3}} &= -1.
\end{align*}
Thus, for $m>0$ we get
\begin{align*}
...
\]
The resulting values of the Zak phase in the large (positive and negative) $m$ limit are reported as a function of $N_y$ ($N_{y}=2\Bar{n}$) in Tab.~\ref{tab:tab1}. In particular, by inspecting Tab.~\ref{tab:tab1} we conclude that for $N_y=4M+2$ the Zak phase differs of $\pi$ between the $m\to \pm \infty$ limits, while this is not the case for $N_y=4M$.
...
In Panels \textbf{a} and \textbf{b} of Fig.~\ref{fig:noise_N_2} (Fig.~\ref{fig:noise_N_3}) we report a graphical representation of the on-site potential in absence and presence of the random noise term for the strip with $N_y=4$ ($N_y=6$). In Panels \textbf{c} and \textbf{d} of Fig.~\ref{fig:noise_N_2} (Fig.~\ref{fig:noise_N_3}) we compare the corresponding low energy spectra: even in the presence of noise, two eigenmodes (marked in red and orange) are present inside the energy gap. However, in contrast to the unperturbed case, the in-gap eigenvalues in Fig.~\ref{fig:noise_N_2}\textbf{d} (Fig.~\ref{fig:noise_N_3}\textbf{d}) are slightly split in energy. In Panels \textbf{e} and \textbf{g} of Fig.~\ref{fig:noise_N_2} (Fig.~\ref{fig:noise_N_3}) are reported the probability density plots of the eigenstates corresponding to the eigenvalues marked in red and orange in Panel \textbf{c}, while in Panels \textbf{f} and \textbf{h} of Fig.~\ref{fig:noise_N_2} (Fig.~\ref{fig:noise_N_3}) are reported the ones corresponding to the eigenvalues marked in red and orange in Panel \textbf{d}. By comparing them one can see that, as a consequence of the energy splitting, the two bound states, which in absence of noise are degenerate in energy and equally localized on both ends, localize themselves on one end of the strip each.
...
\section{Low energy theory and effective mass term}
{\color{black}
...
Also, we set a sharp ``first-neighbor'' cutoff, effectively limiting the interaction to the edge sites that in the full strip are connected by the minimum amount of first neighbor hoppings (see Fig.~\ref{fig:eff_coupl_scheme}). According to this criterion, the number of coordination is easily found to be $n_c = \frac{N_y}{2}+1$. Thus, the interaction Hamiltonian is written as
\begin{align*}
    &N_y=4M: && H_{\text{int}} = \Tilde{\Delta}\sum_{\ell=1}^{N}\sum_{j=-(n_c-1)/2}^{(n_c-1)/2} e^{-(\sqrt{w^2+(ja)^2}-w)/\xi} a^\dagger_\ell b_{\ell+j} + \text{h.c.}, \\
    &N_y=4M+2: && H_{\text{int}} = \Tilde{\Delta}\sum_{\ell =1}^{N}\sum_{j=-n_c/2}^{n_c/2-1} e^{-(\sqrt{w^2+(ja+a/2)^2}-w)/\xi} a^\dagger_\ell b_{\ell+j} +\text{h.c.},
\end{align*}
...
\begin{align*}
    &N_y=4M: && H_{\text{int}} = \Tilde{\Delta}\sum_{\ell =1}^{N}\sum_{j=-M}^{M} e^{-(\sqrt{w^2+(ja)^2}-w)/\xi} a^\dagger_\ell b_{\ell +j} + \text{h.c.}, \\
    &N_y=4M+2: && H_{\text{int}} = \Tilde{\Delta}\sum_{\ell =1}^{N}\sum_{j=-(M+1)}^{M} e^{-(\sqrt{w^2+(ja+a/2)^2}-w)/\xi} a^\dagger_\ell b_{\ell+j} +\text{h.c.}
\end{align*}
The Fourier transformation is defined as
\begin{align}
    a_{\ell}&= \dfrac{1}{\sqrt{N}}\sum_{k} e^{-ik\ell a}a(k), \\
    b_{\ell}&= \dfrac{1}{\sqrt{N}}\sum_{k} e^{-ik(\ell a+\delta)}b(k) ,
\end{align}
with $\delta = 0$ if $N_y=4M$ and $\delta = 1/2$ if $N_y=4M+2$. Thus
\begin{align*}
    &N_y=4M: && H_{\text{int}} = \frac{\Tilde{\Delta}}{N}\sum_{\ell=1}^{N} \sum_{k,q}\sum_{j=-M}^{M} e^{-(\sqrt{w^2+j^2}-w)/\xi} e^{ik\ell}e^{-iq(\ell+j)}a^\dagger(k)b(q) + \text{h.c.} \\
    &N_y=4M+2: && H_{\text{int}} = \frac{\Tilde{\Delta}}{N}\sum_{\ell=1}^{N}\sum_{k,q}\sum_{j=-(M+1)}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} e^{ik\ell}e^{-iq(\ell+j+1/2)} a^\dagger(k)b(q)+\text{h.c.}
\end{align*}
Summing over $\ell$ and $q$:
\begin{align*}
    &N_y=4M: && H_{\text{int}} = \Tilde{\Delta}\sum_{k}\sum_{j=-M}^{M} e^{-(\sqrt{w^2+j^2}-w)/\xi} e^{-ikj}a^\dagger(k)b(k) + \text{h.c.} \\
    &N_y=4M+2: && H_{\text{int}} = \Tilde{\Delta}\sum_{k}\sum_{j=-(M+1)}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} e^{-ik(j+1/2)} a^\dagger(k)b(k) +\text{h.c.}
\end{align*}
Now we consider the $N_y=4M$ case first. We have
\[
    H_{\text{int}} = \Tilde{\Delta}\sum_{k} [1 + \sum_{j=1}^{M} 2\cos(kj)e^{-(\sqrt{w^2+j^2}-w)/\xi}]a^\dagger(k)b(k) + \text{h.c.},
\]
yielding
\begin{equation}
    \mathcal{H}^{\text{edge}}_{\text{int}}(k)= \Tilde{\Delta} [1 + \sum_{j=1}^{M} 2\cos(kj)e^{-(\sqrt{w^2+j^2}-w)/\xi}] \tau_x.
\end{equation}
Proceeding analogously for the $N_y=4M+2$ case
\[
    \begin{split}
        H_{\text{int}} &=\Tilde{\Delta}\sum_{k}\left\{\sum_{j=0}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} e^{-ik(j+1/2)} a^\dagger(k)b(k) +\text{h.c.} \right.\\
        &\left.+ \sum_{j=1}^{M+1} e^{-(\sqrt{w^2+(-j+1/2)^2}-w)/\xi} e^{-ik(-j+1/2)} a^\dagger(k)b(k) +\text{h.c.}\right\}\\
        &= \Tilde{\Delta}\sum_{k}\left\{\sum_{j=0}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} e^{-ik(j+1/2)} a^\dagger(k)b(k) +\text{h.c.} \right.\\
        &\left.+ \sum_{j=0}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} e^{ik(j+1/2)} a^\dagger(k)b(k) +\text{h.c.}\right\}\\
        &= \Tilde{\Delta}\sum_{k}\sum_{j=0}^{M} e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi} 2\cos(k(j+1/2)) a^\dagger(k)b(k) +\text{h.c.} \\
    \end{split}
\]
This yields
\begin{equation}
    \mathcal{H}^{\text{edge}}_{\text{int}}(k)= \Tilde{\Delta} \sum_{j=0}^{M} 2\cos(k(j+1/2)) e^{-(\sqrt{w^2+(j+1/2)^2}-w)/\xi}  \tau_x.
\end{equation}
...
\section{Jackiw-Rebbi like states}
