\begin{tikzpicture}[node distance={20mm},thick,
  ...
  \node[box,fit=(x0)(x1)(x2)] (gx) [label=above:$G(\psi_1)$] {};
  \node[box,fit=(y0)(y1)(y2)] (gy) [label=above:$G(\psi_0)$] {};
  ...
  \draw[->] (x0) to node[above] {$h_1$} (z0);
  \draw[->] (x1) to node[below] {$h_1$} (z1);
  \draw[->] (x2) to[bend right=25, pos=.66] node[above] {$h_1$} (z0);

  \draw[->] (y0) to node[above] {$h_0$} (z0);
  \draw[->] (y1) to node[above] {$h_0$} (z1);
  \draw[->] (y2) to node[above] {$h_0$} (z2);
\end{tikzpicture}
