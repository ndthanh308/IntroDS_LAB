...
$d\fapprel d'$) we say that $d$
\textit{approximates} $d'$ with degree $\beta$.
\end{definition}
...
Let
\begin{align*}
  g_0 &= G(\Xj[0]:\slasher \left(\dirby \to \Yj[0]:\director,
  \ttle\to\Zj[0]:\s[string] \right))~\text{and}\\
    g_1 &= G(\Xj[1]:\thriller \left( \dirby \to \Yj[1]:\director \right)).
\end{align*}
...
In particular $\gamma(g_0) = g_1$. The function $\gamma$ is a
$\appdegree$-morphisms witnessing $\fapproximates(g_0, g_1) = 0.5$.
\end{example}
...
\myrestatable%
  {Endomorphic approximation fuzzy preorder}
  {prop}
  {prop:preorder}
  {proof:proppreorder}
  {proppreorder}
...
\myrestatable%
  {Semantic and syntactic subsumption}
  {theorem}
  {prop:synsem}
  {proof:synsem}
  {propsynsem}
