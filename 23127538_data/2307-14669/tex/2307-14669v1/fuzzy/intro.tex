...
$\beta_k$ with each pair of sort symbols $\s, \su$ (and satisfying
every constraint of \cref{def:fuzzy_poset} in
\cref{app:fuzzy}) and having the following semantics:
\begin{align}
  \label{eq:fuzzy_subsumption}
  \text{if}~\fisop(\s, \su)=\beta_k,~\text{then}~\forall d\in\dom:
  \s^\ii(d) \land \beta_k \leq \su^\ii(d).
\end{align}
That is, any object $d$ which is an instance of $\s^\ii$ with degree
$\beta$ must also be an instance of $\su^\ii$ with a degree $\beta'$
