\subsection{Uniform superposition of size $q$\label{sec:superposition_modulo_q}}
...
There is a deterministic $\LAQCC$ circuit that prepares the uniform superposition of size $q$. This circuit requires $\mathO(\log_2(q)^2)$ qubits.
\end{theorem}
\begin{proof}
Let $n = \left\lceil \log_2(q) \right\rceil$ and define $\mathcal{G}=\{i\mid 0\le i<q\}$ and $\mathcal{B}=\{i\mid q\le i\le2^n-1\}$. 
