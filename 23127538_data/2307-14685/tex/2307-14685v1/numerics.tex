...
All the schemes we consider use the third order CWENOZ reconstruction without ghost cells, see~\cite{STP23:cweno:boundary}, described in Definition~\ref{def:CWENOZb} and \ref{def:CWENOZAO} with parameters $q=p=2$ in \eqref{eq:omegaZ} and
\begin{equation*}
	\begin{cases}
...
\end{equation}
with $\alpha = \max\{\|\mathbf{f}'(\mathbf{v})\|,\|\mathbf{f}'(\mathbf{w})\|\}$, where $\|\mathbf{f}'(\cdot)\|$ denotes the spectral norm of the Jacobian of the flux function $\mathbf{f}$.
...
\begin{equation*}
    ...
    - \tfrac12\alpha \mathbb{I}_m,
\end{equation*}
where $\mathrm{J}_{\mathbf{f}}$ denotes the Jacobian of the exact flux function and $\mathbb{I}_m$ is the $m\times m$ identity matrix.

For the time integration, we employ the three stage third order L-stable DIRK scheme of~\cite{Alexander1977} with Butcher tableau
\begin{equation*}
	...
	\end{array}
\end{equation*}
where $\lambda=0.4358665215$. As a consequence, the Butcher tableau~\eqref{eq:tableau:be} of the composite backward Euler becomes
\begin{equation*}
	\setlength\arraycolsep{10pt}
	\begin{array}{c|ccc}
		\lambda & \lambda & 0 & 0 \\[1.5ex]
...
\begin{table}[t]
	\caption{List of the tested schemes, labels and linetype (for those that appear in the plots)}
	\begin{center}
		\begin{tabular}{c|c|c}
			%\hline
			...
			$\QI{1}$ & Quinpi solution with $\mathcal{I}_1$ time-limiting strategy & \begin{tikzpicture}\draw[blue] (1,0)--(2,0);\node[blue] at (1.5,0) {\pgfuseplotmark{square*}};\end{tikzpicture} \\[2ex]
			$\QI{2}$ & Quinpi solution with $\mathcal{I}_2$ time-limiting strategy & %\begin{tikzpicture}\draw[magenta] (1,0)--(2,0);\node[magenta] at (1.5,0) {\pgfuseplotmark{diamond*}};\end{tikzpicture} 
			\\[2ex]
			$\QI{3}$ & Quinpi solution with $\mathcal{I}_3$ time-limiting strategy & \begin{tikzpicture}\draw[red] (1,0)--(2,0);\node[red] at (1.5,0) {\pgfuseplotmark{triangle*}};\end{tikzpicture} \\[2ex]
			...
			$\QIexp{1}$ & $\QI{1}$ with CFL set as in explicit schemes & \begin{tikzpicture}\draw[dashed,blue] (1,0)--(2,0);\node[blue] at (1.5,0) {\pgfuseplotmark{square}};\end{tikzpicture} \\[2ex]
			$\QIexp{2}$ & $\QI{2}$ with CFL set as in explicit schemes & 
			%\begin{tikzpicture}\draw[dashed,magenta] (1,0)--(2,0);\node[magenta] at (1.5,0) {\pgfuseplotmark{diamond}};\end{tikzpicture}
			\\[2ex]
			$\QIexp{3}$ & $\QI{3}$ with CFL set as in explicit schemes & \begin{tikzpicture}\draw[dashed,red] (1,0)--(2,0);\node[red] at (1.5,0) {\pgfuseplotmark{triangle}};\end{tikzpicture} \\[2ex]
			$\CW$ & Explicit CWENOZ scheme of~\cite{STP23:cweno:boundary} & \begin{tikzpicture}\draw[orange] (1,0)--(2,0);\node[orange] at (1.5,0) {\pgfuseplotmark{star}};\end{tikzpicture} \\
			%\hline
		\end{tabular}
	\end{center}
	\label{tab:labels}
...
All numerical simulations are performed with the schemes described in Table~\ref{tab:labels}. For the time-limited schemes, we choose the threshold parameters in \eqref{eq:strategy1}, \eqref{eq:strategy2} and \eqref{eq:strategy3} as $\gamma_1 = h$, $\gamma_2 = 0.1$ and $\sigma = 10^{-10}$.
...
\subsection{Gas-dynamics problems}

In this section, we test the third order Quinpi scheme on several problems based on the the one-dimensional nonlinear Euler system for gas-dynamics
...
\end{equation}
We run the problem up to the final time $t=1$ with periodic boundary conditions on the domain $[0,1]$.
...
$$
corresponding to the Courant numbers $C = \lambda_{\max}\tfrac{\DT}{h} = 10.7$ and $C = 41.8$, respectively.

The $L^1$- and $L^\infty$-norm errors computed on the density component and the corresponding experimental orders of convergence are listed in Table~\ref{tab:rates:cfl10} and Table~\ref{tab:rates:cfl40} for the two different Courant numbers. While the schemes $\QI{1}$ and $\QI{3}$ achieve the theoretical order of convergence, the scheme $\QI{2}$ exhibits, instead, first order accuracy. This behavior is explained by investigating the time-limiting procedure for $\QI{2}$. In fact, one can observe that the marking strategy $\mathcal{I}_2$ causes the limiting of all the numerical fluxes, and thus computed solution ends up coinciding with the first order predictor. Instead, the other detectors for time-limiting are able to signal the regularity of the solution, avoiding the activation of the time-limiting. For these reasons, in the following we will not consider the scheme $\QI{2}$ anymore.

%\addtolength{\tabcolsep}{3pt}
\begin{table}[th!]
	\caption{Comparisons of the orders of convergence of the Quinpi schemes with Courant number $C=10.7$.\label{tab:rates:cfl10}}
	\centering
	\vspace{0.25cm}
...
\begin{table}[th!]
	\caption{Comparisons of the orders of convergence of the Quinpi schemes with Courant number $C=41.8$.\label{tab:rates:cfl40}}
	\centering
	\vspace{0.25cm}
	\pgfplotstabletypeset[
	font=\small,
...
$$
where $v_{\text{cw}}$ denotes the speed of the contact wave. The ratio between the time-step $\DT_{\text{stab}}$ required for stability and the time-step $\DT_{\text{acc}}$ required for accuracy on the material wave, is a measure of the stiffness of the problem. The Courant number is thus
$$
...
\begin{description}
	...
	\begin{equation} \label{eq:tworar:ic}
		\left(\rho,u,p\right) = \begin{cases}
			...
			\end{cases}
	\end{equation}
	The exact solution at time $t=1$ is depicted in the left panel of Figure~\ref{fig:exact} and is characterized by a 1-rarefaction moving with negative speed, a 3-rarefaction moving with positive speed, and a 2-contact wave with speed $v_{\text{cw}}=-2.57\cdot10^{-2}$.
	...
	$$
	With an implicit scheme, it is possible to overcome this stability restriction. In particular, choosing the time-step $\DT_{\text{stab}}$ to meet the accuracy on the contact wave, one has
	$$
		\frac{\DT_{\text{acc}}}{h} = 6.66,
	$$
	which implies a Courant number $C = 12.1$.
	...
	\begin{equation} \label{eq:twoshocks:ic}
		\left(\rho,v,p\right) = \begin{cases}
			...
			\end{cases}
	\end{equation}
	The exact solution at time $t=1$ is depicted in the center panel of Figure~\ref{fig:exact} and is characterized by a 1-shock wave moving with negative speed, a 3-shock wave moving with positive speed, and a 2-contact wave with speed $v_{\text{cw}}=0.13$.
	For this problem, one has
	$
	...
	$$
	which implies a Courant number $C = 11.6$.
	...
	\item[Test (c): modified Lax shock tube problem.] \begin{equation} \label{eq:stiffLax:ic}
		...
		\end{cases}
	\end{equation}
	The exact solution at time $t=0.15$ is depicted in the right panel of Figure~\ref{fig:exact} and is characterized by a 1-rarefaction moving with negative speed, a 3-shock moving with positive speed, and a 2-contact wave with speed $v_{\text{cw}}=0.35$.
	For this problem, one has
	$
	\max_{(x,t)} |v|+c = 3.61,
	$
	...
	$$
	which implies a Courant number $C = 10.2$.
\end{description}
...
\begin{table}[t!]
\caption{Statistics on the time-limiting procedure of the Quinpi schemes.\label{tab:stat}}
	...
	every last row/.style={after row=\bottomrule},
	columns/0/.style={column name={$N$}},
	columns/4/.style={column name={\shortstack{max.~limited \\ fluxes}}},
	columns/5/.style={column name={\shortstack{\% limited \\ time-steps}}},
    columns/9/.style={column name={\shortstack{max.~limited \\ fluxes}}},
...
Next, we study the effect of the choice of time-step. Schemes $\Qexp$ and $\QIexp{3}$ are obtained with a time-step $\DT=\DT_{\text{stab}}$, instead schemes $\Q$ and $\QI{3}$ are obtained with a time-step $\DT=\DT_{\text{acc}}$, i.e.~centered on the material wave. In Figure~\ref{fig:implicit} we show the results and we observe that
	\begin{itemize}
	    ...
	    \item time-limiting is needed also when $\DT=\DT_{\text{stab}}$, as can be observed also in Figure~\ref{fig:xt:timelim} and in Table~\ref{tab:stat}. Of course, the percentage of limited steps is larger when $\DT=\DT_{\text{acc}}$ but in this case the number of time-steps is roughly $10$ times smaller.
	\end{itemize}
...
$$
Here, the aim is to show that the implicit framework we propose has the ability to keep order and stability on such problems. We do not claim that the Quinpi approach is asymptotic preserving, e.g.~see~\cite{2017DimarcoLoubereVignal}.

In order to the describe the low-Mach number limit, we consider the rescaled compressible Euler system
...
\begin{table}[th!]
	\caption{Experimental order of convergence of the Quinpi scheme on the low-Mach problem with Courant number $C=20$.\label{tab:ratesLM:cfl20}}
	\centering
	\vspace{0.25cm}
	\pgfplotstabletypeset[
	font=\small,
...
\end{align*}
on the computational domain $[-L,L]$, $L=\nicefrac{2}{\varepsilon}$, with periodic boundary conditions.
