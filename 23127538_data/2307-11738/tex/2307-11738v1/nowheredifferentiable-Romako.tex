...
\end{cases}
\]
Fonf, Guraiy and Kadets\cite{FGK} constructed an infinite dimensional space $V\leq C^0([0,1])$, such that every non-zero element of $V$ is nowhere differentiable. Girgensohn \cite{Girgensohn} and Bobok \cite{Bobok} gave  construction of an infinite dimensional space such that every non-zero element has at no point a one-sided derivative.
...
\item for every non-constant polynomial $P$ we have that $P(f)$ does not have a two-sided derivative equal to 0 at some point;
...
Note that Bayart and Quarta \cite{BQ} already constructed an algebra of nowhere differentiable functions countably infinite transcendence degree, clearly the algebra given in the corollary has uncountable transcendence degree.
...
We finally give examples of function that generate algebras of functions that are differentiable at some points, but where such points are quite rare.
...
\[
\limsup_{y\searrow x_0}\frac{|f(y)-f(x_0)|}{|y-x_0|} <\infty.
\]
...
\leq\underbrace{\left(\limsup_{y\searrow x_0}\frac{|f(y)-f(x_0)|}{|y-x_0|^{\alpha}}\right)}_{<\infty}\underbrace{\left(\limsup_{y\searrow x_0}|f(y)-f(x_0)|^{n-\frac{1}{\alpha}}\right)}_{=0} = 0,
\end{multline*}
...
To give an explicite example of a completely non-Hölder function we use the Faber-Schauder expansion of a continuous function. Every continuous function $f\in C([0,1])$ has a unique expansion in the form
\[
