...
If a totally real submanifold has half of the dimension of the ambient manifold, it is called a Lagrangian submanifold. 
...
We add the condition of being totally geodesic, and we find that these submanifolds can only be part of the first group, also called Lagrangian submanifolds of diagonalizable type.
...
\subsection{Nearly Kähler manifolds}  A nearly Kähler manifold is an almost Hermitian manifold for which the tensor $G(X,Y)=(\nabla_XJ)Y$ is skew symmetric. Here, $J$ is the almost complex structure and $\nabla$ is the Levi-Civita connection. For a (pseudo-)nearly Kähler manifold, the tensor $G$ satisfies
\begin{equation}
...
\]
thus the metric has constant sectional curvature $-1$. This way, we can think $\Sl$ as the anti-de Sitter space $H_1^{3}(-1)$. The tangent space of $\Sl$ at a matrix $a$ can be written as
\begin{equation}
...
\end{equation}
where $a$, $b$ and $c$ are matrices in $\Sl$. 
...
\begin{equation}
    ...
    \label{constanttype}
    \end{equation}
The curvature tensor $\tilde{R}$ of $\Sl\times\Sl$ associated to $\tilde{\nabla}$ is given by
\begin{equation}
        \begin{split}
...
We will also assume that the submanifold is non-degenerate.
...
\begin{table}[H] %this centers the table on the page
  ...
  \begin{tabular}{l l l l}
    ...
    and $\theta_1,\theta_2,\theta_3\in[0,\pi)$,}
    \\[2ex]
    \columntag{(2)}{case:8.2} \label{cc2} &
    $A = \begin{pmatrix}
    \cos 2\theta_1 & 1 & 0 \\
    0 & \cos 2\theta_1 & 0 \\
    0 & 0 & \cos 2\theta_2
    \end{pmatrix}$, &
    $B = \begin{pmatrix}
    \sin 2\theta_1 & \frac{-(c^2 + 2 \cos 2\theta_1)}{2 \sin 2\theta_1} & c \\
    0 & \sin 2\theta_1 & 0 \\
    0 & c & \sin 2\theta_2
    \end{pmatrix}$, 
    \\[4ex]
    \multicolumn{4}{l}{  with $\Delta_i=\Delta_2$, 
    $\theta_1,\theta_2 \in [0,\pi)$, $\theta_1 \ne 0,\pi/2$ and $c \in \mathbb R$.
    If $c \ne 0$, then $\cos 2\theta_1 = \cos 2\theta_2$}\\[1ex] \multicolumn{4}{l}{and $\sin 2\theta_1 = -\sin 2\theta_2$,}
    \\[2ex]
    \columntag{(3)}{case:8.3} \label{cc3} &
    $A = \begin{pmatrix}
    -1 & 1 & 0 \\ 0 & -1 & 0 \\ 0 & 0 & -1
    \end{pmatrix}$, &
    $B = \begin{pmatrix}
    ...
    \end{pmatrix}$,
    \\[4ex]
    \multicolumn{4}{l}{ with $\Delta_i=\Delta_2$ and $t \in \R$,}
    % \\[2ex]
    \end{tabular}
  }
  \end{table}
 \begin{table}[H] %this centers the table on the page
  ...
  \makebox[\textwidth]{
  \begin{tabular}{l l l l}
    \columntag{(4)}{case:8.4}  \label{cc4}&
    $A = \begin{pmatrix}
    \cos 2\theta & 0 & 1 \\
    0 & \cos 2\theta & 0 \\
    0 & 1 & \cos 2\theta
    \end{pmatrix}$, &
    $B = \begin{pmatrix}
    \sin 2\theta & -\csc (2\theta)^3 & -\cot 2\theta \\
    0 & \sin 2\theta & 0 \\
    0 & -\cot 2\theta & \sin 2\theta
    \end{pmatrix}$,
    \\[4ex]
    \multicolumn{4}{l}{with $\Delta_i=\Delta_2$, and $\theta \ne 0,\pi/2$,}
    \\[2ex]
    \columntag{(5)}{case:8.5} \label{cc5} &
    $A = \begin{pmatrix}
    s \cos 2\theta_1 & x & 0 \\
    -x & s \cos 2\theta_1 & 0 \\
    0 & 0 & \cos 2\theta_2
...
From the first equation of (\ref{lagrprop}) follows that $G(X,Y)$ is a normal vector to $M$ for any tangent vectors $X,Y$ on $M$. Let $\{E_1,E_2,E_3\}$ be a $\Delta_i$-orthonormal frame of the tangent space of $M$.
...
With this information we can state a stronger version of Lemma $\ref{propABcruda}$.
 \begin{lemma}
 Let $M$ be a Lagrangian submanifold of $\Sl\times\Sl$ and $P$ the almost product structure given in $(\ref{prodstructuredef})$. Then the restriction of $P$ to $M$ can be written as $P|_M=A+JB$, where $A,B:TM\to TM$ must have one of the following forms, with respect to a $\Delta_i$-orthonormal frame $\{E_1,E_2,E_3\}$:
\begin{table}[H]
  \centering
  \makebox[\textwidth]{
  \begin{tabular}{l l l}
...
\end{equation}
where $\alpha$ can be either $\sqrt{\tfrac{3}{2}}$ or $-\sqrt{\tfrac{3}{2}}$, depending on which case of (\ref{tabla}) we are in.
We consider the five cases of Lemma \ref{propABcruda} applied to Equation (\ref{pei}), one by one.

\textit{Case \emph{(1)}.}
 Using a similar procedure as in \cite{Dioos}, we get $\theta_1+\theta_2+\theta_3=0$ modulo $\pi$.
...
Looking at the component of $E_3$ in (\ref{pei}) when taking $i=2$, $j=2$ and $k=3$ we obtain $c\sin(\theta_1)=0$, therefore $c=0$ as $\sin(\theta_1)\neq 0$.
...
with $i=2$, $j=2$ and  $k=3$ yields $-2\cos(2\theta)=1$.

So $\theta=\tfrac{\pi}{3}$ or $\theta=\tfrac{2}{3}\pi$. That is
\[
    A=\begin{pmatrix}
    -\frac{1}{2} & 0 & 1 \\
    0 & -\frac{1}{2} & 0 \\
...
\textit{Case \emph{(5)}}.
Taking $i=1$ in (\ref{pei}) gives us
\begin{align*}
    \cos2\theta_1&=\cos2(\theta_1+\theta_2),\\
       \sin2\theta_1&=-\sin2(\theta_1+\theta_2).
\end{align*}
So as in the previous cases we have that $\cos2(2\theta_1+\theta_2)=1$. We take $i=2$ in (\ref{pei}) and we look at the components of $E_1,JE_1$:
\begin{equation}
    \begin{split}
...
Notice that the equations in (\ref{reflection}) can be written as the equation $Rv=v$ where $v=(x,y)^t$ and
\begin{equation*}
    R=\begin{pmatrix}
...
Finally, replacing $x$ and $y$ in the matrices of Case \ref{cc5} in Lemma \ref{propABcruda} yields what we desired.
\end{proof}
...
Equation (\ref{nablap}) for case 1 yields the following lemma.
...
\end{align*}
Since sine and cosine never vanish at the same time (\ref{anglederi}) and (\ref{sffc}) hold for $i=1$ and $j=2$. The other equations follow in a similar way, y choosing other $X$ and $Y$ in (\ref{nablap}).
\end{proof}


\begin{proposition}
If the angle functions are constant and $h_{12}^3=0$, $M$ is totally geodesic. Conversely, if the submanifold is totally geodesic then the angles are constant.    
\end{proposition}
\begin{proof}
...
If two angles are equal modulo $\pi$, $M$ is totally geodesic.
\end{proposition}
\begin{proof}
...
\begin{lemma}\label{isometrias}
 ...
 Then the Lagrangian immersions $\phi_1\circ f$ , $\phi_{2}\circ f$ where $\phi_i$ are the isometries given in \emph{(\ref{isoslsl})} are also of diagonalizable type and their respective angle functions are
\[
...
\begin{proof}
      ...
      \vspace{0.2 cm}
        ...
        Suppose that $M$ is a Lagrangian submanifold of diagonalizable type with angles as in \ref{c3}. Then by Lemma \ref{isometrias}, applying the isometry $\phi_2$ in (\ref{isoslsl}) produces a congruent Lagrangian submanifold with angles as in \ref{c1}. We can apply a similar argument to see that cases \ref{c2}, \ref{c3}, \ref{c6}, \ref{c7}, \ref{c8} and \ref{c9} are congruent to one of the cases \ref{c1}, \ref{c4} or \ref{c5}. Hence we may restrict our attention to the latter cases. Notice that in cases \ref{c4} and \ref{c5} we cannot do a permutation with $E_1$ since, unlike $E_2$ and $E_3$, this vector is timelike. Later on we will prove that these cases are not isometric.
\end{proof}
...
\]
hence the angle functions are constant and all are equal to $\tfrac{4}{3}\pi$, as in Case (1) of Lemma \ref{trescasos}. Using Proposition \ref{twoanglesequal} we conclude that $f$ is totally geodesic.
\end{example}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
 Consequently, the angle functions are constant and equal to $(0,\pi,\pi)$, as in Case (2) of Lemma \ref{trescasos}. By Proposition (\ref{twoanglesequal}) $f$ is totally geodesic.
\end{example}
...
\begin{equation*}
    ...
    \end{split}
\end{equation*}
Therefore, the angle functions are constant and equal to $(\pi,\pi,0)$, corresponding to Case 3 of Lemma~\ref{trescasos}. By Proposition (\ref{twoanglesequal}) $f$ is totally geodesic.
\end{example}
...
\]
Using (\ref{equationstg0pipi}) we can prove that $\omega$ is closed. Hence there exist a local function $\varphi$ such that $d\varphi=\omega$.
...
The authors would like to thank Professor Luc Vrancken for his valuable input to this work.
