...
%
%
% the stress and internal variables but 
%
%By extending the concept of neural additive model (NAM), we introduce a neural network architecture that generate feature space and 
%
%
%aggregate those models in a polynomial form, that enables us to obtain the analytical expression of the yield function. 
%
%
%
%By comparing with the benchmark state-of-the-art algorithms, the proposed framework is capable of delivering more robust and accurate predictions, 
%
%while the divide-and-conquer approach significantly improves the computational efficiency, especially for high-dimensional models aimed to capture material behaviors that lack the material symmetry or exhibit size-dependent effects. 
%

%Based on the portability of symbolic regression, we also showcase that the resultant models can be easily deployed to continuum-scale simulations such as finite element analysis. 
%We also make all the data sets and the source code to train and utilize our proposed model publicly available (\href{https://github.com/XXX/XXX}{https://github.com/XXX/XXX}),  to enable third-party validation and extension. 
%By introducing neural network architectures that generate feature space and aggregate those data in a polynomial form, we enable the yield function to be determined analytically. 
%By comparing with the benchmark state-of-the-art algorithms, the proposed method is capable of delivering more robust and accurate predictions, while the divide-and-conquer approach significantly improves the computational efficiency, especially for high-dimensional models aimed to capture material behaviors that lack material symmetry, exhibit size-dependent effect or complex hardening/softening mechanisms. 
%By leveraging the portability of symbolic regression, the resultant models can be easily deployed to third-party software such as UMAT in ABAQUS. 
\end{abstract}

%\keywords{neural additive model; symbolic regression; level-set meta-model; computational plasticity; ABAQUS subroutine}
...
the adaptation of these models on high-consequence engineering applications has not yet been considered mainstream. Successful adaptation to production software has been demonstrated, for instance, by \citet{li2019machine}.
...
%
% BB: I think intelligence is an ambiguous word that is not our focus; for example, a system can be very intelligent but not interpretable. Like human brain is intelligent, but right now, it is not explainable why/how it works.
%
...
%
% BB: I think an "over-parameterized" network requires a rigorous defintion and is out of the scope of our work.  Also, our univariaet MLP can be over-parameterized so there is not neccassary direct connection between over-parameterized and interpretability.
%
%However, over-parameterized neural network has found to have 
%superior expressivity \citep{muthukumar2021classification}, 
%modern neural network constitutive laws often use 
%larger number of parameters such as neurons than those used in the 90s \citep{xiao2022geometric}, which makes directly interpretations of the learned function not feasible \citep{murdoch2019definitions}. 
%
% SUN: I see your point. Let's keep it simple and try to move forward. 
...
\citet{vlassis2022component}, for instance, adopt a component-based design, which is introduced where the ingredients of elastoplasticity models are represented by individual neural networks to distinguish the properties of the elasticity models 
from the plastic responses, a departure from the recurrent neural network approach or multi-step feed-forward neural network approaches in \citep{ghaboussi1991knowledge, logarzo2021smart, mozaffar2019deep}. Coincidentally, this concept has later reintroduced as modular machine learning elastoplasticity by \citet{fuhg2023modular}.
...
These approaches may provide relational and structural knowledge about the learned material models and facilitate the model validation against physical constraints,  such as thermodynamic consistency \citep{vlassis2021sobolev}. However, the lack of analytical expression makes it difficult to provide definite proof of basic properties such as convexity and material symmetry.
Other approaches attempted in the literature involves performing symbolic regression directly to learn a portion or all of the plasticity models \citep{versino2017data, wang2022establish, bomarito2021development}. The advantage of this approach is that it may lead to a mathematical expression of the learned function that is much shorter than the neural network counterparts and hence, suitable for analysis. 
...
%
% BB: I think the first ref has also multi dimesnional problem.  Also the second ref has porous metal plasticity which we also use that and I am not sure that example is isotropic.
%As such, many of the examples involved in symbolic regression for plasticity model are often limited to one-parameter learning hardening laws (\citet{versino2017data}. )or isotropic yield functions  (bomarito2021development). 
%
...
It should be, however, noted that there is no universal consensus on whether expressing models in compact mathematical equations is necessary. In particular, 
\citet{jenab2016use,li2019machine}, and \citet{jordan2020neural}  argue that \textit{a prior assumption of the nature of the mathematical function is not necessary} (cf. \citet{jordan2020neural}) and the neural network approach may avoid introducing bias by not limiting the search of expression within a pre-defined equation form. 
...
%
%
Due to the high expressivity\footnote{The expressivity of a neural network architecture refers to the size of the set (or cardinality) of all possible functions a specific neural network architecture is capable of approximating (cf.  \citep{raghu2017expressive,lin2018generalization}).} of deep neural networks and the inherent nonlinear interactions among the input features,  a consistently high accuracy has been reported by those successfully trained MLP models. 
...
In theory, a symbolic regression may potentially achieve both the desirable level of expressivity and interpretability if the combinatorial optimization that searches the optimal expression tree is successful. 
...
Hence, a strategy that can enable us to effectively address the NP-hard symbolic regression problem for high-dimensional contexts is not only a potential breakthrough for discovering analytical plasticity models but could also be significant for advancing the state-of-the-art of symbolic regression through approximations, heuristics, or specialized algorithms.
...
can be ranked by examining the coefficients of the feature $w_{i}$. In other words, the NAM approach maintains the interpretability of the linear regression (in the feature space) with enhanced expressivity afforded by the neural networks. 
...
% BB: I think maybe there is typo in the last part after comma ????

% SUN: This is true but you will also show the results at the end, so I would suggest taking it out for simplicity. 
%
%For example,  let's consider a data generating process $\phi = x_1  x_2$.  
%There could be some parts of the data space such that the NAM model with the form $\bar{\phi} = w_1 \cdot MLP(x_1; \vec{\beta}_1) + w_2 \cdot MLP(x_2; \vec{\beta}_2)$ cannot approximate the data with the desirable accuracy since it does not take into account any possible interactions among the input features,  no matter how expressive are those single-variable MLPs.  In the next section, we introduce our model assumption to increase the NAM expressivity.

\subsection{Proposed strategy for interpretable model recovery}
%\alert{TODO: they asked to rewrite the first paragraph, I have not written this so not sure what to do.}
% SUN: I will just neglect them. They just don't want to admit that their method lacks interpretability. 
Given the fact that constitutive laws are often used for high-consequence engineering applications, making the machine learning generated constitutive laws interpretable is necessary but not sufficient to make those models trustworthy and hence has the potential to have impacts on engineering applications. 

The purpose of this article is to propose a two-step machine learning approach that strikes a balance between expressivity 
...
However, instead of directly using these shape functions as the bases for the yield function, we introduce additional quadratic terms of the feature shape function to further enhance the expressivity, which then lead to more accurate models (see Section \ref{sec:qnm}). 
...
Here, we introduce a specific neural network architecture for the quadratic neural model  such that a set of neural networks
are used to generate the feature space spanned by scalar-valued shape functions. 
...
%SUN: I have mentioned that in the introduction already.
%Various parametric model classes can be assumed for $\bar{\phi}$ from the linear regression (the least expressive) to the deep neural networks (the most expressive).  
...
% In the next sections, our model hypothesize class is declared, particularly designed to balance interpretability and expressivity, 
...
$\vec{w} \in \mathbb{R}^{D + D (D-1)/2}$ denotes the concatenation of all parameters $\vec{w} = \{ w_i,  \hat{w}_{ij}\} $.
...
%Another way to incorporate the interaction between input features without losing too much interpretability is to add additional learnable functions with scalar-valued inputs, which are pairwise multiplication of the input features as follows:
%
%\begin{equation}
%\bar{\phi}(\vec{\sigma}; \vec{\beta},  \vec{\omega}) = \sum_{i=1}^D w_{i} f_i(\sigma_i; \vec{\beta}_i)
%+
%\sum_{i=1}^D \sum_{j=i+1}^D w_{ij} g_{ij}(\sigma_i \sigma_j; \vec{\beta}_{ij}), 
%\end{equation}
%
%where $g_{ij} : \mathbb{R} \to \mathbb{R}$ are higher order shape functions parameterized by separate neural networks with trainable parameters $\vec{\beta}_{ij}$.  
%Similarly,  $\vec{\beta}$ denotes the concatenation of all neural network parameters $\vec{\beta} = \{ \vec{\beta}_i, \vec{\beta}_{ij} \}$.  We call this functional form \textbf{feature multiplication} extension of NAM. 

%Although single variable functions are the most desirable function forms, in terms of interpretability,  there could be cases where our two previously introduced extensions of NAM may still fail to approximate the underlying ground truth data accurately. 
%For example, there could be cases where the underlying ground truth data may have sophisticated interactions among different features that the previous two functional forms cannot capture. 
%To add more flexibility regarding this limitation,  we consider the following functional form
%\begin{equation}
%
%\bar{\phi}(\vec{\sigma}; \vec{\beta},  \vec{\omega}) = \sum_{i=1}^D w_{i} f_i(\sigma_i; \vec{\beta}_i)
%+
%\sum_{i=1}^D \sum_{j=i+1}^D w_{ij} h_{ij}(\sigma_i,  \sigma_j; \vec{\beta}_{ij}), 
%\end{equation}
%where $h_{ij} : \mathbb{R}^2 \to \mathbb{R}$ are higher-order shape functions parameterized by separate neural networks. 
%We call this functional form \textbf{two variables} extension of NAM. 
%Notice that, unlike previous formulations,  the higher order term is built by $h_{ij}$ functions with two-dimensional inputs.  
%This will considerably reduce the (visual) interpretability advantage of single variable functions.  
%However,  we will recover another layer of interpretability by extracting symbolic equations associated with these single and double-variable functions.  
 
%TO BE CONTINUED ...  compare advantage/disadvantage of each of these methods


%

%\subsection{Polynomial expression for nonlinear feature sub-manifold }
%Geometrical interpretation of the NAM. Consider the admissble space spanned by the feature and yield function as the ambient space of dimension N. Adding a linear constraint leads to a N-1. 

%Our contribution is to use the same feature but instead of aggregating those feature 
%with a linear combination, we use polynomial to give us more expressibility. 
...
As our learning task involves a univariate neural network, we must ensure that each shape function can handle such scenarios. In the NAM work by \citet{agarwal2021neural}, the authors propose a new activation function to address this issue. However, in our work, we build upon recent developments by enriching classical neural networks with Fourier layers, as described in \citep{rahimi2007random,tancik2020fourier}. This approach can better capture high-frequency content and improve the overall performance of our model.


% Figure environment removed

Each shape function $f_i(x_i)$ shown in Fig.~ \ref{fig:nn-arch} is parameterized by an MLP enriched with the Fourier layer as follows:
...
%We leverage the advantages of the series of scalar mapping in NAM to simplify the symbolic regression and avoid directly dealing with high-dimensional space. This bypasses the curse of dimensionality, which is particularly difficult to overcome for combinatorial optimization problems. Other research efforts have also utilized scalable models (e.g., neural networks and decision trees) in combination with symbolic regression algorithms to handle the curse of dimensionality while maintaining interpretability\citep{icke2013improving,cranmer2020discovering,wadekar2020modeling}.



%In our proposed framework, finding the best approximation under separability constraints is performed via scalable gradient-descent-based optimization and neural network parameterization. Then, several symbolic regression problems are solved, in parallel, for each single-variable neural network function. In this way, we can significantly reduce the computational complexity of the discrete search associated with the symbolic regression of an otherwise multivariate function. Other research efforts have also combined more scalable models (e.g., neural networks and decision trees) with symbolic regression algorithms to handle the curse of dimensionality while maintaining interpretability.

%Symbolic regression algorithms aim to find analytical equations describing a given data set. Unlike black-box neural network models, analytical expressions are transparent and explainable for the entire data domain. These symbolic equations are more lightweight than other standard machine learning models, such as neural networks needed to store a large number of parameters, hence more transportable. Discovered equations by symbolic regression are shown to generalize well outside the train data support \citet{kim2020integration}, providing successful training.

%A symbolic regression algorithm searches the space of mathematical expressions to find the best equation describing the data. Usually, this search is performed by genetic programming, where evolutionary-based algorithms are utilized to optimize tree data structures associated with mathematical expressions \citep{schmidt2009distilling,wang2019symbolic,udrescu2020ai}. Recently, methods based on reinforcement learning are also developed as alternative ways for conducting an efficient discrete search in the space of tree data structures \citep{petersen2019deep,landajuela2021discovering}. 

% BB: already mentioned
%Due to the combinatorial nature of the discrete search associated with the symbolic regression, they do not scale well for high-dimension data \citep{icke2013improving,de2018greedy}; their computational time scales exponentially with respect to data dimensionality. However, in our proposed framework, we extract a symbolic equation for each shape function: single-variable to single-variable data.  This feature enables easy application of parallel computing; enabling SR algorithms to be executed simultaneously for each shape function.
%
...
% BB: our devide-and-conqure method shares similarity with AI-Feymann
}
% SUN: This session is not the main focus of the paper and should be as concise as possible. 
...
%While the classical theory of elastoplasticity requires the introduction of a yield function that oftentimes involves handcrafting sophisticated mathematical expressions designed to incorporate the physics and the desired material behavior, recent studies \citep{vlassis2021sobolev, vlassis2022component} have demonstrated that a data-driven model can discover the yield criterion while bypassing such endeavors. 
%In their approach,  they enhance the robustness of the data-driven scheme by separating the elastic and plastic behaviors as two separate data-driven tasks.  Particularly,  they focus on learning the yield surface in a data-driven manner as a level-set function parameterized by MLP where the zero levels are the yield surfaces. 
%The resultant yield function based on their framework relies on a multivariate MLP which is not an interpretable model as mentioned earlier.
%To overcome this limitation, we demonstrate the implication of our proposed framework for computational plasticity while being completely interpretable. 
...
model into the user-defined material subroutine (UMAT) for finite element simulations. Unlike the previous approach in \citet{suh2023publicly} where yield functions are parametrized via neural networks across different programming languages, the implementation of the analytical model 
requires only the correction expression of the learned model and hence simpler. 

%
%
% we first briefly summarize the theory of level-set plasticity that requires a set of univariate multi-layer perceptrons that learn the implicit representation of the yield surface (Section \ref{sec:level_set}). 
%We then provide details on the implicit stress integration algorithm for the proposed interpretable-ML-based constitutive relation in Section \ref{sec:stress_integration}, while the results shown in Section \ref{sec:num_example} highlight the applicability of our framework on computational plasticity. 
...
\end{equation}
Based on the theory of elastoplasticity, this means that a dataset of stress points in the plastic regime measured either from the experiments or from sub-scale simulations always reside on the yield surface $f_y(\tensor{\sigma}) = 0$. 
...
Although its effect will be further discussed in Section \ref{sec:low_order_NAM},  this section focuses on a cylindrical coordinate system for the $\pi$-plane orthogonal to the hydrostatic axis [i.e., $\hat{\tensor{x}} = \hat{\tensor{x}}(p, \rho, \theta)$, where $p$ denotes the mean pressure while $\rho$ and $\theta$ are the Lode's radius and angle, respectively], rather than directly adopting the Cartesian coordinates spanned by the principal stresses ($\sigma_1$, $\sigma_2$, $\sigma_3$). 
...
%These auxiliary data points are added, as the yield function is an implicit function that is defined everywhere in the parametric space.  Adding these auxiliary data may help us reach the intended inductive bias with more support. 
...
%In this section, we discuss and validate different aspects of our introduced interpretable framework for the data-driven discovery of plastic yield surfaces. The first example focuses on a pressure-insensitive dataset and examines how the proposed method can discover the symbolic yield surface. We also examine the method's extrapolation capability compared to previous methods in the literature. In the second example, we study the efficacy of the QNM-based symbolic model in dealing with higher-dimensional data of metal plasticity in a five-dimensional space. We also discuss the enforcement of simplicity through sparsity control. The last example illustrates how the proposed QNM-based model can discover the correct symbolic equation for pressure-sensitive data. We demonstrate that the found symbolic equations can be readily used in classical FEM codes without significant changes compared to neural network-based plasticity models.
% 
%=======
...
In the second example,  Section \ref{sec:high_order_NAM}, we study the efficacy of the QNM-based symbolic regression method in dealing with higher-dimensional data of metal plasticity in a five-dimensional space. We also discuss the enforcement of simplicity through sparsity control. The last example in Section \ref{sec:FE_simulation} illustrates how the proposed QNM can discover the correct symbolic equation for pressure-sensitive data. We demonstrate that the found symbolic equations can be readily used in classical FEM codes without significant changes compared to neural network-based plasticity models.


\remark{
In Section \ref{appendix:toy_problem}, we compare different modeling assumptions for solving a general regression task when dealing with sparse data, considering the problem dimensionality. The limitations of the proposed scheme are discussed in Appendix \ref{appendix:limit}.
}
...
%The symbolic regression algorithm generates a list of equations ranked by accuracy and complexity. However, simpler equations tend to have better generalization, making them the preferred choice.
...
% I think there are many instant that Occams razor is either misused or just not work. So, I woujld avoid dealing with it altogether, even though I know T Oden likes it a lot. For instance, Francis Crick strongly oppose the use of it. See below: 
% https://www.theatlantic.com/science/archive/2016/08/occams-razor/495332/
% Hence, I would prefer not adding the content below. 

%In the context of plasticity applications, accuracy may be more valued than simplicity since the return mapping algorithm requires both function values and their gradients to be accurate, consistent and stable. Failure to meet these requirements can lead to the Newton-Raphson algorithm's failure in an implicit method. 
%However, as per Occam's Razor, simplicity's generalization power suggests that simpler models may generalize better \citep{thorburn1918myth}.
% However, there are no extrapolation concerns for this shape function since it is only defined for the angle $\theta$, which falls within the training data range of zero to $2\pi$. Therefore, we can focus solely on finding a sufficiently accurate function in the interpolation regime. It is worth noting that we already considered Occam's Razor principle at the start by restricting the approximation function to be univariate and separable.
...
% BB: since now it is in the main text
%Computational time for discovering symbolic equations with and without guidance from the proposed neural network are reported in Appendix \ref{appendix:symb_reg_comparison}.
...
Here, we add uniformly distributed noise along the radial direction where its magnitude ranges from $-4$ \% to $4$ \% of Lode's radius to test the performance of the model trained by a dataset that may possibly contain noise. Similar to the previous example, the original dataset is then pre-processed via level-set augmentation by setting $N_{\phi} = 11$ that covers a narrow band region of [0.85$\rho$, 1.15$\rho$] such that our full dataset consists of 66,000 stress points. 
...
%Figure \ref{fig:bomarito_shape_func} shows the shape functions learned by QNM with and without sparsity control. Notably, the learned shape functions for each feature are similar in their overall shape. The translational difference between two shape functions is not important since it relates to the constants (i.e., the bias) in each MLP, which can be represented by different weighted averages over all MLPs. This aligns with the principle that the same function can be represented in multiple ways, although some may be simpler and more preferable even if the difference is marginal. 
...
%\renewcommand{\arraystretch}{2.5} % Increase the space between rows
...
%\subsection{Petal shape Von Mises yield function}
%\label{sec:petalJ2}
%summarize the main purposes of this problem TO BE CONTINUED ....
%
%In this problem, we synthesize the yield surface point cloud from the following equation,  
%\begin{equation}
%\phi(p, \rho, \theta) = 
%\sqrt{\frac{3}{2}} \rho
%(1 + A \sin(k_p \theta)) - \sigma_y,
%\end{equation}
%where $k_p = 3$ and $A=0.325$ are parameters controlling the number of petals and their height in the $\pi$-plane as shown in Fig. ~\ref{fig:petalJ2Data}.
%The initial yield stress is set to $\sigma_y= 250$ MPa.
%
%
%% Figure environment removed
%
...
%\begin{table}
%  \centering
%  \caption{symbolic shape functions for $p$}\vspace{-10pt}
%  \resizebox{\columnwidth}{!}{
%        \begin{tabular}{|c|c|c|}
%        \hline
%        Complexity & Loss & Expression \\
%        \hline
%        3 & 1.069e-03 & $\begin{aligned}- 0.17 x_{0}\end{aligned}$\\ \hline19 & 4.696e-04 & $\begin{aligned}\left(x_{0} - 0.18\right) \sin{\left(\left(\sin{\left(10.94 \cos{\left(x_{0} \right)} \right)} - 0.25\right) e^{- \sin{\left(x_{0} - 0.92 \right)}} \right)}\end{aligned}$\\ \hline56 & 1.864e-04 & $\begin{aligned}\left(- \frac{0.34 \cdot \left(2 \sin{\left(x_{0} \right)} - 0.29\right)}{\cos{\left(\sin{\left(2.02 x_{0} \right)} \right)}} + 0.4\right) \left(1.29 x_{0} + 1.29 \sin{\left(x_{0} - \sin{\left(15.46 x_{0} - 28.91 \right)} + 0.8 \right)} + 0.5\right) \sin{\left(\sin{\left(x_{0} \right)} - 2.12 \sin{\left(9.94 x_{0} - 18.6 \right)} \right)}\end{aligned}$\\ \hline\end{tabular}
%}
%\label{Tab::petal-symb-eqs-p} 
%\end{table}
%
%
%\begin{table}
%  \centering
%  \caption{symbolic shape functions for $\rho$}\vspace{-10pt}
%        \begin{tabular}{|c|c|c|}
%        \hline
%        Complexity & Loss & Expression \\
%        \hline
%        3 & 1.684e-02 & $\begin{aligned}1.0 x_{0}\end{aligned}$\\ \hline8 & 2.986e-04 & $\begin{aligned}x_{0} + 0.14 \cos{\left(x_{0} \right)} - 0.08\end{aligned}$\\ \hline20 & 5.712e-06 & $\begin{aligned}- 0.06 x_{0} \left(x_{0} + 0.26 \cos{\left(1.14 x_{0} \right)}\right) + x_{0} + 0.06\end{aligned}$\\ \hline\end{tabular}
%\label{Tab::petal-symb-eqs-rho} 
%\end{table}
%        
%        
...
%\begin{table}
%  \centering
%  \caption{symbolic shape functions for $\theta$}\vspace{-10pt}
%\begin{center}
%        \begin{tabular}{|c|c|c|}
%        \hline
%        Complexity & Loss & Expression \\
%        \hline
%        3 & 2.375e+00 & $\begin{aligned}- 0.17 x_{0}\end{aligned}$\\ \hline16 & 4.151e-03 & $\begin{aligned}\frac{0.16 - \sin{\left(4.83 x_{0} \right)}}{\cos{\left(\sin{\left(2.41 x_{0} + 0.79 \right)} \right)}}\end{aligned}$\\ \hline47 & 4.345e-04 & $\begin{aligned}\frac{- \sin{\left(4.83 x_{0} \right)} + \sin{\left(0.21 \cos{\left(\sin{\left(\sin{\left(\frac{0.28 - \sin{\left(4.83 x_{0} \right)}}{\cos{\left(\sin{\left(2.41 x_{0} + 0.79 \right)} \right)}} \right)} \right)} - 0.1 \right)} \right)}}{\cos{\left(\sin{\left(2.41 x_{0} + 0.79 \right)} \right)}}\end{aligned}$\\ \hline\end{tabular}
%        \end{center}
%\label{Tab::petal-symb-eqs-teta} 
%\end{table}
%
%
%% Figure environment removed
...
%
%
%\subsection{Matsuoka-Nakai yield function}
%\label{sec:MNdata} 
%
%% Figure environment removed
%
...
%\begin{table}
%  \centering
%  \caption{discovered symbolic shape functions}\vspace{-10pt}
%\begin{center}
%        \begin{tabular}{|c|c|c|c|}
%        \hline
%        Quantity & Complexity & Loss & Expression \\
%        \hline
%        $p$ & 3 & 6.960e-01 & $\begin{aligned}- 1.0 x_{0}\end{aligned}$\\ 
%        \hline
%        $\rho$ & 8 & 4.324e-03 & $\begin{aligned}x_{0} + 0.12 \cos{\left(x_{0} \right)} - 0.07\end{aligned}$\\
%        \hline
%        $\theta$ & 39 & 1.192e-04 & $\begin{aligned}0.29 - \frac{\sin{\left(4.79 x_{0} - 0.07 \right)}}{0.86 - 0.37 \sin{\left(4.79 x_{0} - \frac{0.05}{\cos{\left(1.23 \cos{\left(2.48 x_{0} \right)} \right)}} \right)}}\end{aligned}$\\ 
%        \end{tabular}
%        \end{center}
%\label{Tab::MK-symb-eqs}
%\end{table}
%
%
%
...
%\label{sec:micropolar}
%Extending this framework into higher-order continua based on Zeyu's data.
...
%The total computational time allotted to direct symbolic search ($\approx 43$min) is limited to that required by our two-step framework, comprising neural network training ($\approx 83$min) and single-variable symbolic regression (1 min). Comparing the two processes may be partially fair, as the neural network training is conducted using \texttt{PyTorch}, an industry-optimized tool for this purpose developed over many years by expert professionals, while \texttt{PySR}, a research-based package, is used for symbolic regression.



%The best found equation, in terms of MSE,  is $\phi = 1.88 \sin{\tilde{\phi}}$ where:
%
%
%
...
%The contribution of each variable in the final yield surface is less apparent than the results shown in Fig.\ref{fig:bomarito_shape_func}. 
...
%For example, one may realize that an exponential function is sufficient to express the data in Fig.\ref{fig:bomarito_shape_func}(c) rather than the complex equation found in Table \ref{Tab::porous-symb-eqs-sparse}.
...
% SUN: need a sentence or two to describe the purpose of this subsection. :
...
Note that the availability of the analytical expression of the yield function also enables us to obtain the analytical expression of the plastic flow direction (assuming associative flow rule) and its Hessian.  As such, convexity can be established by checking the positivity of the eigenvalues of the Hessian \citep{borja2013plasticity},  which can be obtained from the characteristic polynomial,  which can be again obtained analytically.  In this case, and thanks to the separability of the discovered equation, we can represent yield surface (the locus of points that has zero level-set) by writing the radius as a function of angle $\rho(\theta) = c_1\sin(c_2\theta + c_3) + c_4$.  For such representation, the positivity of Hessian is equivalent to the positivity of the curvature of the polar base vector $\rho(\theta)$\citep{gluge2018does},  
\begin{equation}
...
%Note that the availability of the analytical expression of the yield function also enables us to obtain the analytical expression of the plastic flow direction (assuming associative flow rule) and the Hessian, i.e,
...
%As such, convexity can be established by checking the positivity of the eigenvalue of the Hessian, which can be obtained from the characteristic polynoimal, which can be again obtained analytically,.

%\begin{equation}
%\label{eq:eigenvalues}
%\end{equation}

%Hence, we proof that the model in Eq. ???? is convexity. Notice that similiar analysis can be conducted for other properties and in some cases, it can be completed with symoblic software like Mathematica or PySYM?????. For breivty, we would not exhaust those options . 

%\subsubsection{Necessary conditions for compaction banding}
%This requires getting the expression from the three-invariant plasticity model

\subsubsection{Symmetry on the $\pi$-plane for yield surface}
%The John Argyris model is supposed to have the symmetry in the $\pi-$ plane if the yield surface is rotated by $120^{\circ}$ degrees. In the neural network representation, this can be checked through numerical tests. In our case, however, this can be proved by checking whether $\phi(\rho, \theta)  - \phi(\rho \theta \pm 2*\pi*/3 =0$.
...
In both cases, we found that the proposed method is feasible to train, and the generated model is capable of discovering yield functions with superior accuracy than those obtained from the neural additive model. To ensure third-party validation, the source code is open-sourced. 
