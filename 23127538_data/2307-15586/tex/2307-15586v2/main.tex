...
They observed that, even though the rule that maximizes the utilitarian social welfare is known to be strategyproof (for a specific tie-breaking convention) \cite{goel2019knapsack,lindner2008allocating}, it tends to put too much weight on majority preferences.
...
However, agents are rarely single-minded in several applications of portioning, so an appropriate notion of proportionality (or, more broadly, representation) for general preferences is needed.
...
\begin{definition}[Pareto optimality]
    An aggregation rule $F$ is \emph{Pareto optimal} (PO) if, for every instance $\mathcal I$ and the outcome $\mathbf{x}=F({\mathcal I})$, there does not exist another outcome $\mathbf{x}'$ such that (1) $d_i(\mathbf{x}') \leq d_i(\mathbf{x})$ for all $i \in N$ and (2) $d_i(\mathbf{x}') < d_i(\mathbf{x})$ for some $i \in N$.
\end{definition}

The last two axioms we consider were studied by Freeman et al.~\cite{freeman2021truthfulbudget}.
\begin{definition}[Range-respect]
    An aggregation rule $F$ is \emph{range-respecting} (RR) if
    for every instance $\mathcal I$, the outcome $\mathbf{x}=F({\mathcal I})$, and
...
In particular, we focus on two popular welfare criteria: (1) maximizing the utilitarian welfare (\util{}), i.e., minimizing $\sum_{i\in N}d_i(\mathbf{x})$, and (2) maximizing the egalitarian welfare (\egal{}), i.e., minimizing $\max_{i\in N}d_i(\mathbf{x})$. 
...
\begin{proof}
    ...
    Let $\mathbf{x}$ be the output of the sum rule. Then $x_j = \frac{s_{1j}+s_{2j}}{2}$ for each $j \in [m]$, so
    $d_1(\mathbf{x}) = d_2(\mathbf{x})$, and $d_1(\mathbf{x}) + d_2(\mathbf{x}) = \sum_{j\in [m]} |s_{1j} - s_{2j}|$. 
...
While the IM mechanism satisfies both strategyproofness and proportionality, it fails other intuitive properties such as score-unanimity, range-respect, and Pareto-optimality; these failures can lead to highly counterintuitive outcomes for the voters and cast doubt on the IM mechanism as an aggregation method.

These trade-offs between various rules may be used to inform decision-making in a wide range of settings.
...
\begin{proof}
    ...
    For the upper bound, we know that $z_1^\text{min} \leq z_1^\text{max}$. 
    Adding $1 - z_1^\text{min}$ to both sides, we get 
    \[
...
While the general positive result only holds for the sum rule, we show that the max and med rules satisfy score-representation when $n=m=2$, whereas the min and prod rules may fail to do so even in the simplest setting.

\begin{thm} \label{thm:score_representation-max}
    The max rule satisfies score-representation when $n=m=2$, but may fail to do so when  $m \geq 3$ (even for $n=2$) or $n\ge 3$ (even for $m=2$).
    It also satisfies proportionality when $n=2$, but may fail to do so when $n \geq 3$ (even for $m = 2$).
\end{thm}
\begin{proof}
    ...
    Then, score-representation demands that (i) $x_1 \geq \frac{s_{11}}{2}$ and $x_2 \geq \frac{s_{22}}{2}$, and (ii) $x_1 \geq s_{21}$ and $x_2 \geq s_{12}$.
    ...
    However, the max rule returns $\mathbf{x} = (0.5,0.25,0.25)$ with $x_3 = 0.25 < \frac{1}{3}$, failing this condition.
...
\begin{thm} \label{thm:score_representation-min_prod}
    The min and prod rules satisfy proportionality when $n=m=2$, but may fail to be so when $n \geq 3$ or $m \geq 3$.
...
IM and \util{} are known to satisfy participation \cite{freeman2021truthfulbudget}.
...
\begin{proof}
    ...
    Let $[m] = S \cup T$, where $S \subseteq [m]$ is the set of indices where for each $\alpha \in S$,
    \begin{equation*}
        x'_\alpha > x_\alpha,
    \end{equation*}
    and $T \subseteq [m]$ is the set of indices where for each $\beta \in T$,
    \begin{equation*}
    ...
    It follows that $y'_\alpha = y_\alpha$.
...
\ack This work was partially supported by the AI Programme of The Alan Turing Institute, by the Singapore Ministry of Education under grant number MOE-T2EP20221-0001, and by an NUS Start-up Grant.
