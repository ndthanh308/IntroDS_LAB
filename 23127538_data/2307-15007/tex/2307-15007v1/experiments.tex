...
\textbf{Chest X-ray:} Second, we consider a semi-synthetic chest x-ray dataset for pneumonia classification \cite{kermany2018labeled}. To control exactly what information the model leverages such that we can create ground truth signal-distractor decompositions, we inject a spurious correlation into this dataset. We place a small, barely perceptible noise patch randomly at the top of each image in the ``normal" class. We confirm that the model relies only on the spurious signal during classification by testing the model's drop in performance when flipping the correlation (adding the noise patches to the ``pneumonia'' class) and seeing that the accuracy goes from 100\% to 0\%. As such, for this dataset, the signal is simply the noise patch and the distractor is the rest of the xray. 
...
VerT (Ours) & \textbf{0.461 $ \pm0.08$} & \textbf{0.821$ \pm 0.05$}\\
...
We first explore how well VerT recovers signal-distractor decompositions. 
