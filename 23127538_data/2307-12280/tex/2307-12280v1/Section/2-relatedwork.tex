...
Some efforts investigated privacy risks against pre-trained encoders, such as membership inference attacks~\cite{liu2021encodermi,he2021quantifying}, model extraction ~\cite{liu2022stolenencoder,dziedzic2022difficulty,sha2022can}.
At the same time, backdoor attacks and poisoning attacks, the common security threats that usually occur in the training phase, have been shown to be deleterious to pre-trained encoders~\cite{carlini2021poisoning,jia2022badencoder,saha2022backdoor,liu2022poisonedencoder} . 
...
On the contrary, our work aim to achieve effective attacks from the perspective of directly changing the intrinsic texture features of the samples under more demanding conditions that better reflect 
realistic scenarios.


\subsection{Universal Adversarial Examples}
It is well known that deep neural networks are vulnerable to adversarial examples, where an attacker can fool the model by adding minor noise to the image, usually in the form of perturbation~\cite{goodfellow2014explaining,carlini2017towards,hu2023pointca, hu2022protecting} and patch \cite{yang2020design, liu2020bias,hu2021advhash}.
...
Existing works can be divided into optimization-based universal adversarial attacks~\cite{moosavi2017universal,mopuri2017fast,mopuri2018generalizable} and generative universarial adversarial attacks~\cite{hayes2018learning, mopuri2018nag, mopuri2018ask}.
...
Some works also proposed different defenses against adversarial examples, such as data pre-processing, adversarial training~\cite{madry2017towards,tramer2019adversarial}, pruning~\cite{zhu2017prune,ye2018defending}, and fine-tuning~\cite{ye2018defending,peng2022fingerprinting}. These methods can defend against adversarial samples at different phases.
