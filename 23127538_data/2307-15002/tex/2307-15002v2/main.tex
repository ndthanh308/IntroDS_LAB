...
\begin{enumerate}
    \item  After correcting optimistic results in \cite{jiang-etal-2023-low}, we observe accuracy loss of \gzip (but not of trained neural models). However, KNN overall seems useful in few-shot setups.
    
    \item We show that a much simpler distance function based on `bag-of-words' works equally well as \gzip, if not better.
\end{enumerate}
