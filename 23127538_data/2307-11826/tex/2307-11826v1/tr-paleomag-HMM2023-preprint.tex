...
\\&\hspace*{-1.5em} 
=\frac{\psi_\mm'(\FF,\mm,\theta)\!\!}{\det\FF}
...
\hspace*{-1em}\varrho\DT\vv-{\rm div}\big(\TT{+}\DD{+}\TT_{\!\rm mag}^{}
        {-}\,{\rm div}\,\mathscr{S}\big)=\varrho\GRAVITY+\ff_{\!\rm mag}^{}
...
${\rm skw}\big((\mu_0\hh {-}
\psi_\mm'(\FF,\mm,\theta)/\!\det\FF){\otimes}\mm\big)$ while $\mathscr{S}$
will be a ``magnetic exchange hyperstress''
$$
{\rm Skw}\big(\mm{\otimes}[\psi_{\rm G}^{}]_{\nabla\mm}'(\FF,\nabla\mm)\big)
=\frac{\upkappa(\FF)}{\det\FF}{\rm Skw}(\mm{\otimes}\nabla\mm)\,,
$$
...
L.D.\,Landau \cite{Land37TPT} devised a pioneering theory of phase
...
$|\mm|=m_\text{\sc s}(\theta)$ with the radius
...
\begin{align}
{\rm Dir}(\rr)=\begin{cases}\{\rr\in\R^d;\ |\rr|\le1\}&\text{if }\rr={\boldsymbol0}\,,\\
...
\cite{Gilb55LFGE} and the Landau-Lifshitz \cite{LanLif35TDMP}
...
\begin{align}
  \hspace*{-1.9em}\ZJ\mm=\DT\mm-{\rm skw}(\nabla\vv)\mm
\ \text{ with }\ 
...
is set-valued at 
the zero rate. Moreover, in a deforming continuum, we can 
...
variant by \cite{BFLS18EWSE} or \cite{Zhao18LWPB} or in a nanoparticle
transport in fluids by \cite{GruWei21FITM}.
...
\begin{align}\nonumber
  ...
  \DT\theta
=\xi+\theta\frac{\psi_{\FF\theta}''(\FF,\mm,\theta)\!}{\det\FF}{:}\DT\FF
...
=\!\!\!\!\!\linesunder{\frac{\psi(\FF,\mm,0)}{\det\FF}}{actual stored}{energy}
\!\!\!\!\!
+\!\!\!\!\!\!
...
\begin{align}\nonumber
&\hspace*{-2em}\pdt\W+{\rm div}(\vv\W)=\xi
...
\hspace*{-2em}
\alpha\ZJ\mm+\HC(\theta){\rm Dir}(\ZJ\mm)
\\[-.5em]&\hspace*{-.7em}-\frac{\mm{\times}\ZJ\mm}{\gamma(\mm,\theta)}
\ni\mu_0\hh-\tt
...
$\SS$ see \cite{DeSPoG95ISIS,DeSPoG96CTDF} while the skew-symmetric hyperstress
...
will be based on the Kr\"oner-Lie-Liu \cite{Kron60AKVE,LeeLiu67FSEP}
...
\\&\nonumber\hspace{-1em}
\text{ with }\
\xi(\Fe,\theta;\ee,\rr,\Lp)=\DIS\ee{:}\ee+\mu_0\alpha|\rr|^2
...
{power of}{external field}\!\!\!\!
  \\&
  \hspace*{-1.2em}\lineunder{-\,
  \frac{\COUPLING_{\Fe}'\!\!(\Fe,\mm,\theta)\Fetop\!\!\!}{\det\Fe}{:}\ee(\vv)
-\frac{\COUPLING_\mm'(\Fe,\mm,\theta)\!}{\det\Fe}{\cdot}\DT\mm
...
\nabla\FF$ has been added into the kinematic equation \eq{Euler-thermodynam2}.}
 ...
 extended appropriately, cf.\ \cite{Roub22QHLS,Roub23LTFP,RouTom123IFST}
...
Maxwellian creep modulus $\GM$ in 
...
\vspace*{-.3em}\bibitem{KaKoSc21MAWS}
...
\vspace*{-.3em}\bibitem{NeNoSi89GSIC}
...
\newblock {\em Disc. Cont. Dynam. System - S, submitted}, 2023.
\newblock (Preprint arXiv no.2304.05918).

\vspace*{-.3em}\bibitem{RoToZa09GEDF}
