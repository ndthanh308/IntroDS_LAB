...
%  theorem concerning stable invertible categories: if $R$ is an Artinian $E_\infty$-ring then every invertible $R$-linear cocomplete stable category is the category of modules over a (non necessarily connective) Azumaya $R$-algebra.
...
We finish with a classification of invertible stable categories over Artinian $E_\infty$-rings. In what follows, we say that a connective $E_\infty$-ring $R$ is Artinian if it is truncated, Noetherian, and $\pi_0(R)$ is Artinian.

\begin{theorem}\label{theorem stable over R}
Let $R$ be an Artinian $E_\infty$-ring. Let $\Ccal$ be an invertible $R$-linear cocomplete stable category. Then $\Ccal = \LMod_A(\Mod_R)$ for some Azumaya algebra $A$ in $\Mod_R$.
\end{theorem}
...
By proposition \ref{proposition dualizable is presentable} we see that $\Ccal$ and $\Ccal^\vee$ are presentable. By \cite{AGBrauer} theorem 6.16 it suffices to show that there exists an \'etale cover $\lbrace \Spec(R_i) \rightarrow \Spec(R) \rbrace$ such that $\Ccal \otimes_R R_i$ is equivalent to $\Mod_{R_i}$ for all $i$. We may in particular reduce to the case where $\pi_0(R)$ is local. Let $k$ be the residue field of $R$. By theorem \ref{theorem stable} there exists a finite separable field extension $k'$ of $k$ such that $\ccal \otimes_R k'$ is equivalent to $\Mod_{k'}$ as a $k'$-linear category. Pick an \'etale morphism $R \rightarrow R'$ such that $R' \otimes_R k = k'$. Then $\ccal \otimes_R R'$ is equivalent to $\Mod_{R'}$ by an inductive application of lemma \ref{lemma deal with square zero para stable}.
\end{proof}
