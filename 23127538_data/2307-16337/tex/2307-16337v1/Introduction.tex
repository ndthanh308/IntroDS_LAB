...
If we focus on invertible objects of $\lincat_{R, \infty, \text{st}}$ instead, one may expect that they all arise from (non necessarily connective) Azumaya $R$-algebras. As already remarked, this holds under an additional compact generation hypothesis. Our next result removes this compact generation hypothesis in the case when $R$ is Artinian:

\begin{theorem}\label{teo principal stable introduction}
Let $R$ be an Artinian $E_\infty$-ring. Then every invertible object of $\lincat_{R, \infty, \normalfont{\text{st}}}$ is the $\infty$-category of left modules over an Azumaya $R$-algebra.
\end{theorem}

In the language of \cite{ToenAzumaya} section 2, this shows that the derived algebraic Brauer group and the big derived categorical Brauer group agree in the Artinian case.
