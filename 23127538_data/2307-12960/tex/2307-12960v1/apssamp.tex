...
\label{ eq:l}
 ...
 \end{split}
\end{align} 
where  $x(t)$ and $y(t)$ represents the positions of these particles at different times. The parameters $k_1$ and $k_2$ denote the optical stiffness of the two traps. The constants $H_{11} = H_{22} = 1/(6\pi \eta a) = 1/\gamma$ \; and \; $H_{12} = H_{21} = 1/(4\pi \eta R)$, where $R$ is the center - center distance between the two traps and $a$ is the radius of the particle, are the lowest order components, in $1/R$, of the Oseen Tensor \cite{doi1988theory} for motions in the longitudinal directions. The value of $R$ determines the interaction between the colloidal particles. As $R\to \infty$, the interaction between the colloidal particles vanishes and our system turns to a non-interacting system. The terms $f_1(t)$ and $f_2(t)$ are the random Brownian forces given by, $f_1(t) = \sqrt{2k_BT/\gamma}\;\epsilon_1(t)$ \; and \;  $f_2(t) = \sqrt{2k_BT/\gamma}\;\epsilon_2(t)$,\; where $k_B$ is the Boltzmann constant, $\gamma$ is the viscous drag coefficient given by $6\pi \eta a$ with $\eta$ being the viscosity of the medium. The noise terms $\epsilon_1(t)$  and $\epsilon_2(t)$ obey, $\langle \epsilon_i(t) \rangle =0$ and $\langle \epsilon_i(t) \epsilon_j(s) \rangle =\delta_{i,j}\delta(t-s)$.
...
For the model we consider, leveraging the fact that it is a linear system of stochastic differential equations, it is possible to analytically compute the instantaneous probability distribution $P({\bm x}_t)$ in terms of all the parameters in the system, for any value of time $t$ (See Appendix A). Verifying that the variables $x$ and $y$ are always correlated is straightforward using these solutions. At equilibrium, the correlations are given by,
\begin{align}
    \langle x  y\rangle = \frac{\gamma  k_B T}{2 \pi  \eta R(  k_1 + k_2 )}.
\end{align}
Note that the correlation increases when $R$ decreases and vice versa.
...
\begin{equation}
    \dot {{\bm r}}(t) = - {\bm A} {\bm r}(t) + {\bm B} {\bm \epsilon}(t)
\end{equation}\\
where 
\begin{align}
\begin{split}
...
\frac{\sqrt{2k_BT\gamma}}{\gamma} & \frac{\sqrt{2k_BT\gamma}}{4\pi \eta R} \\
\frac{\sqrt{2k_BT\gamma}}{4\pi \eta R}  & \frac{\sqrt{2k_BT\gamma}}{\gamma}
\end{bmatrix}, \\
...
\end{equation}
where $P(t,{\bm r}|t_0,{\bm r}_0)$ is  the conditional probability that the system is in a position ${\bm r}$ at time $t$, given that it was at ${\bm r}_0$ at time $t_0$. Here ${\bm D}$ is the diffusion matrix defined as ${\bm D} = \frac{1}{2}{\bm B}{\bm B}^T$. \\
...
\begin{align}
    {\bm \Sigma}(\infty) =\left(
\begin{array}{cc}
 \frac{k_B T \left(\gamma ^2 k_1+16 \pi ^2 \eta ^2 k_1 R^2-\gamma ^2 k_2+16 \pi ^2 \eta ^2 k_2 R^2\right)}{16 \pi ^2 \eta ^2 k_1 R^2 (k_1+k_2)} & \frac{\gamma  k_B T}{2 \pi  \eta  k_1 R+2 \pi  \eta  k_2 R} \\
 \frac{\gamma  k_B T}{2 \pi  \eta  k_1 R+2 \pi  \eta  k_2 R} & \frac{k_B T \left(\gamma ^2 (-k_1)+16 \pi ^2 \eta ^2 k_1 R^2+\gamma ^2 k_2+16 \pi ^2 \eta ^2 k_2 R^2\right)}{16 \pi ^2 \eta ^2 k_2 R^2 (k_1+k_2)} 
\end{array}
\right)
\end{align}
\end{widetext}
