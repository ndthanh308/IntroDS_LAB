\documentclass[9pt,twocolumn,twoside]{pnas-new1}
...
\begin{document}
\title{Quantum jamming brings quantum mechanics to macroscopic scales}

% Use letters for affiliations, numbers to show equal authorship (if applicable) and to indicate the corresponding author
\author[a,1]{Maurizio Fagotti}
%\author[b,1,2]{Author Two}
%\author[a]{Author Three}

\affil[a]{Universit\'e Paris-Saclay, CNRS, LPTMS, 91405, Orsay, France}
...
Few particles in a quantum many-body system do not generally reach the realm of the macroscopic world. 
We show that quantum jamming is a mechanism to make microscopic  phenomena visible at a macroscopic scale: a single particle produces a permanent change in the state, and few particles leave an imprint of their scattering properties and bound states.
One of the most noteworthy phenomena is that a single observation of a local observable can turn a jammed separable state into %quantum superposition of few macroscopically different states, similarly to the ``burleske'' cases pictured by Schr\"odinger to shed light on the interpretation of quantum mechanics. 
a state in which quantum mechanics is necessary to describe macroscopic properties. 
}
...
\correspondingauthor{\textsuperscript{1}%To whom correspondence should be addressed. E-mail: 
maurizio.fagotti@university-paris-saclay.fr}

% At least three keywords are required at submission. Please provide three to five keywords, separated by the pipe symbol.
%\keywords{Quantum jamming  $|$ Macroscopic entanglement $|$ Lightcone dynamics $|$ Quantum spin chains }

\begin{abstract}
A quantum spin-$\frac{1}{2}$ chain with an axial symmetry is normally described by quasiparticles associated with the spins oriented along the axis of rotation. Kinetic constraints can enrich such a description by setting apart different species of quasiparticles, which can get stuck at high enough density, realising the quantum analogue of jamming. %We consider a duality mapping that has the practical effect of removing the kinetic constraints. 
We identify a family of interactions satisfying simple kinetic constraints and consider generic translationally invariant models built up from them. 
We study dynamics following  a local unjamming perturbation in a jammed state. 
%We consider a duality transformation that has the practical effect of removing the kinetic constraints, 
...
%As a by-product, we show that kinetic constraints allow for the generation of macroscopic quantum states by a single measurement of a local observable. 
\end{abstract}
...
\firstpage[7]{3}%this is a weird number to set in order to make it work...
...
\dropcap{L}ow dimensional quantum many-body systems easily capture the interest of theoretical physicists as a result of their unusual properties, e.g., strong correlations~\cite{Giamarchi2003Quantum}, integrability in the presence of interactions~\cite{Essler2005The} in 1D, topological order~\cite{Zeng2019Quantum} in 2D. Since nowadays such systems have become experimentally  accessible~\cite{Bloch2008Many-body,Cazalilla2011One}---turning the concept of quantum simulator~\cite{Buluta2009Science,Cirac2012Goals,Bernien2017Probing} into a reality---the interest in them has sparked far beyond the academic level. One dimensional systems, in particular, have been intensively investigated in nonequilibrium settings~\cite{Polkovnikov2011Colloquium,Gogolin2016Equilibration,Gring2012Relaxation, Schemmer2019Generalized}. On the one hand, they can  exhibit exotic behaviours such as relaxation to non-thermal states~\cite{Rigol2007Relaxation,Essler2016Quench,Ilievski2016Quasilocal}; on the other hand, there are interacting 1D models that allow for non-perturbative solutions, either exact~\cite{Ilievski2015Complete} or asymptotic~\cite{Castro-Alvaredo2016Emergent,Bertini2016Transport}, and, especially in quantum spin chains, numerical approaches based on tensor networks~\cite{Ran2020Tensor} are particularly efficient. %(though not always effective as some theoretical questions clash with the complexity of nonequilibrium time evolution). 

Among the most remarkable nonequilibrium phenomena observed in 1D systems, we point up
%Symmetries, hidden or explicit, exact or approximate, are at the basis of remarkable nonequilibrium phenomena, including 
prethermalization~\cite{Moeckel2008Interaction,Rigol2009Breakdown,Bertini2015Prethermalization,Babadi2015Far,Langen2016Prethermalization,Abanin2017A,Alba2017Prethermalization,Reimann2019Typicality,Durnin2021Nonequilibrium} and prerelaxation~\cite{Fagotti2014On,Bertini2015pre-relaxation}. Such slow dynamics appear, for example, in strong coupling limits %, where there are emergent symmetries that significantly affect the dynamics~\cite{Fagotti2014On, zadnik2023slow}. 
as a result of emergent symmetries~\cite{Fagotti2014On, zadnik2023slow}. 
...
%While similar in essence, this definition tempers the traditional connection between quantum and classical jamming~\cite{Nussinov2013Mapping,Artiaco2021Quantum}, which is based on mapping classical dissipative systems at finite temperature into  quantum systems at zero temperature~\cite{Biroli2008Theory}.
The interest in it had come from the idea that jamming %was inspected as 
%a possible 
might be %a critical phenomenon characterised by spatial heterogeneity and rigidity %, manifesting itself in the impossibility of a system to relax
 %This ... the 
%and was 
linked to a new kind of phase transition~\cite{Biroli2007A}, akin to the more celebrated glass transition, which could be approached by varying a thermodynamic quantity such as the density. 

The key question we address here is which phenomenology should be expected %in the vicinity of 
close to quantum jamming when the system is  %when the system exhibits 
%is prepared in an unstable state far from equilibrium rather than  
perturbed out of unstable equilibrium %. %not from thermodynamic equilibrium but 
rather than driven from or to thermodynamic equilibrium. 
In the systems we consider, indeed, the space of jammed states is spanned by product states, which could be reproduced, for example, in optical lattices with ultracold atoms~\cite{Weitenberg2011Single}. %, which can be prepared in an independent way
...
%an alternative examination: we assume that the system has been already prepared in a low-entangled jammed state and investigate the dynamical effects of perturbations. 
...
%Just as strong-weak dualities help identify emergent symmetries and constraints, so `high-low dualities'', mapping high density into low density, will help us clarify dynamics close to quantum jamming. The simplest example of such a transformation is a particle-hole duality, and in fact we are going to consider a generalisation of it for systems with %in which there are more than one species of particles. 
...
%, which is an instance of the duality we are going to consider, 
In the specific case of the dual folded XXZ, it translates trivial noninteracting dynamics in XX % (which does not affect local observables in the limit of infinite time) 
...
%Quantum states that are capable of inducing macroscopic quantum effects have been called ``macroscopic quantum states''~\cite{Frowis2012Measures}. 
Specifically, MELP can be observed even %in the non-symmetric case 
in the absence of that symmetry whenever the jammed state %consists of a repeated pattern of species (hence, it 
is (almost) homogeneous; the more disordered the sequence of species, however, the more hindered the spreading of the perturbation. %; and this holds true even if  disorder is confined into a small part of the sequence. 
%Finally, following Ref.~\cite{Frowis2012Measures}, we show that  the unusual lightcone emerging from the position of a local unjamming perturbation can not be fully explained without assuming the validity of quantum mechanics. 
...
Since the sequence of species $\underline b$ is conserved, it is convenient to split the representation of the state  in two parts, one of which stores just the information about the sequence $\underline b$ and hence is stationary. By removing the information about the species, particles become indistinguishable; let us associate each of them with a pseudospin down.  A pseudospin up represents instead the absence of a particle, so the number of pseudospins up between two pseudospins down should match the macrodistance between the corresponding particles minus one--- see Fig.~\ref{f:mapping}. 
...
The prescription that we described could look plain at first sight but belies an important complication: macropositions, pseudopositions (i.e. the position of the pseudospins) and the relative position of a particle in the sequence are not trivially related to one other. 
...
\end{cases}
\end{equation}
In the following we will denote the state with the sequence of species $\underline b$ and impurities at $n_1,\dots,n_\nu$ by $\ket{n_1,\dots,n_\nu;\underline b}$ (the absence of impurities will be denoted by  $\ket{\emptyset;\underline b}$)---see Fig.~\ref{f:mapping}. 

For given $\underline b$, the dynamics can be projected onto the space of pseudospins by applying the duality transformation to the Hamiltonian. A procedure is described in `Material and Methods'.
We find that symmetric translationally invariant  operators with local densities are represented in the pseudospace by  operators with densities that are still local and, in addition, are independent of the sequence of species. Non-symmetric operators, instead, have a sequence-dependent semilocal representation\footnote{We are using the term ``semilocal'' to indicate representations of local observables that do not commute with all local operators acting on the pseudospins at arbitrarily large pseudodistance; an example follows with \eqref{eq:HI}.}. 
...
In the basis $\ket{n_1,\dots,n_\nu;\underline b}$, the local magnetisation is a diagonal operator. Using~\eqref{eq:ellp}, we find the matrix elements shown in \eqref{eq:szn} and \eqref{eq:sznnu}---see \hyperlink{SI0}{SI} for more details.
% Figure environment removed
Roughly speaking, the local magnetisation in a state with a single impurity is identified with either one local magnetisation or another ($y$ spins apart) in the underlying jammed state, depending on whether the impurity is on the left or on the right. The matrix elements with more impurities have a similar interpretation and can be reduced to those with a single impurity, \eqref{eq:sznnu}. Time evolution is readily obtained 
\begin{equation}\label{eq:szt}
\braket{: \sigma^z_{\ell}:}_t=\sum_{k=1}^\nu\sum_n p_{n+k-1,t}^{(k)}\braket{n;\underline b|:\sigma_{\ell-y(k-1)}^z:|n;\underline b}
\end{equation}
where $p^{(j)}_{n,t}$ is the probability that the $j$-th pseudospin up is at pseudoposition $n$ at time $t$, and it is given by
\begin{equation}
 p^{(j)}_{n,t}=\sum_{n'}|\braket{n_1',\dots,n'_{j-1},n,n'_{j+1},\dots,n'_\nu|e^{-i\tilde{ H} t}|\tilde \Psi_0}|^2.
\end{equation}
Here $\ket{\tilde \Psi_0}$ is the projection of the initial state onto the pseudospin space, $\tilde{H}$ is the projected Hamiltonian,
and the sum is over all   integers $n'_1,\dots,  n'_{j-1},n'_{j+1},\dots,n'_{\nu} $ satisfying $n_1'<\dots<n'_{j-1}<n$ and $n<n'_{j+1}<\dots<n'_\upsilon$. For finite $\nu$, the qualitative behaviour of the magnetisation can be readily understood if $\tilde H$ is local (i.e., the original Hamiltonian is symmetric) and doesn't exhibit exceptional  localisation properties in the eigenstates. It is then reasonable to expect $p^{(j)}_{n,t}$ to approach zero in time. Consequently, the terms in the second line of \eqref{eq:szn} give a subleading contribution to \eqref{eq:szt}, and the argument of the step functions on the first line of  \eqref{eq:szn} can be shifted without affecting the asymptotic behaviour of the local magnetisation. We then end up with the following simplified expression
\begin{equation}\label{eq:sigmaasymp}
\braket{:\sigma^z_{\ell}:}_t\sim 
\sum_{k=0}^{\nu-1} 
\braket{\emptyset;\underline b|\sigma_{\ell-y(k+1)}^z-\sigma_{\ell-y k}^z|\emptyset;\underline b}F_{t}^{(1+k)}(x_\ell)
\end{equation}
where  
$F_{t}^{(k)}(x)=\sum_{n\lesssim x}p_{n,t}^{(k)}$ is the cumulative distribution and  $x_\ell$ is the effective pseudoposition associated with site $\ell$, i.e.,  $\frac{2}{\ell}(x_{\ell}+\delta x_{\ell})\sim 1+O(\ell^{-1})$.
...
For its generality and transparency, \eqref{eq:sigmaasymp} is the most important formula of this paper. Firstly, by modifying the underlying jammed state, the coefficients of $F_{t}^{(k)}(x_\ell)$ in \eqref{eq:sigmaasymp} (which belong to $\{-2,0,2\}$) can be changed in an arbitrary way without even affecting the asymptotics of $x_\ell$. 
This allows us to ignore the coefficients altogether and focus on the cumulative distributions.
Secondly, 
as clarified in the following examples, $p_{n,t}^{(k)}$ typically  decays as $1/t$ (at least for small enough $\nu$), and hence the typical variation of the cumulative distribution is over ballistic scales $x\sim t$. Thus, we predict a general nontrivial profile for the local magnetisation as a function of $x/t$, which is the unusual lightcone behaviour pointed out in Refs~\cite{Bidzhiev2022Macroscopic,Zadnik2022Measurement} in a specific model with a single impurity.  
Corrections should be expected if there is a finite number of localised eigenstates, whose more explicit effect would be to create effective discontinuities in the aforementioned relevant scaling limit of $F_t^{(k)}(x)$.

On the contrary, in order for the impurities not to change the local magnetisation in a region that grows in time for a generic jammed product state, the probabilities  $p_{n,t}^{(k)}$ should remain localised around the original positions of the impurities, which is possible only if the sector with $\nu$ impurities is spanned by almost only localised excited states. This is practically impossible when the Hamiltonian is symmetric and the interactions are local, but we will show in the examples below that it becomes an option in the presence of non-symmetric interactions. 
...
In order to prove the robustness of the phenomenology under interacting and integrability breaking  terms, we consider the following Hamiltonian with $y=2$
...
\end{multline}
The first line is a hard-rod deformation of the XXZ model and is  interacting integrable. % (whether or not the case $\Delta=0$ could be considered interacting is questionable). 
...
\frac{ 1+ \tau^z_j}{2}\, ,
\end{multline}
where we indicated the pseudospins operators by $\frac{1}{2}\vec \tau_\ell$ %rather than $\frac{1}{2}\vec \sigma_\ell$ 
to avoid confusion with the spin operators. %over the spaces where the operators act.
\subsubsection*{Single impurity: noninteracting representation}
Quite generally, the physics of a single impurity can be described by an effective noninteracting Hamiltonian. Our example is no exception and, in fact, we find that the coupling constants $\Delta$ and $g$ that set this example apart from the problem studied in Ref.~\cite{Zadnik2022Measurement} are irrelevant. Specifically, %the term coupled by $g$ doesn't modify the excitation energy, whereas the term coupled by $\Delta$ 
the additional interactions results in just a shift of the excitation energy, given by $E(p)=4 J(\cos p-\Delta)$, that doesn't have any impact on the local magnetisation. %Thus, we refer the reader to Ref.~\cite{Zadnik2022Measurement} for plots and discussions. 
...
The large time limit of $F_t^{(k)}(x)$ takes contributions from both magnons (m) and bound states (b), $F_t^{(k)}(x)=F_{t,\mathrm{m}}^{(k)}(x)+F_{t,\mathrm{b}}^{(k)}(x)$, which are reported in \eqref{eq:Fmagnon} and \eqref{eq:Fbound}.
% Figure environment removed
There, $n_1^{(0)}<n_2^{(0)}$ denote the pseudopositions of the impurities at the initial time (note that we took the limit $x,t\rightarrow \infty$ for given $n_1^{(0)}, n_2^{(0)}$, so the impurities in the initial state correspond to $x\sim 0$). Figure~\ref{f:1}  highlights the effect of the interactions by comparing the cumulative distributions with those obtained by trivialising the scattering phase, i.e., \mbox{$S(p_1,p_2)\rightarrow -1$}. The bound state contributes significantly only when the impurities at the initial times are close enough. The dissimilarity between the maximal velocities of magnons and bound states  is manifested in the appearance of cusps within the lightcone.
...
$,%\end{equation}
where the left (right) hand side  shows the (pseudo)spins and (pseudo)position $0$ ($\Downarrow$ stands for a semi-infinite string of pseudospins down). The sequence of species reads
\begin{equation}\label{eq:b}
\underline b= \dots,(1,0,0,1),(1,0,0,1_0),(0,0,1,1),(0,0,1,1),\dots
\end{equation}
where we displayed relative position $0$ and grouped the species to highlight the repeating pattern.
...
\centering
% Figure removed
\caption{The cumulative distribution $F_t^{(1)}(x)$ for a single impurity as a function of  pseudoposition ($x$ axis) and time $Jt$ ($y$ axis) in the (non-symmetric) example described by  \eqref{eq:Hnonsym1} starting from $\ket{0;\underline b}$ with the sequence in \eqref{eq:b}. Each column corresponds to a different value of $V=\{0.1,0.3,0.5\}$ (the values of $\Delta$ and $g$ are irrelevant). The first row is for the configuration without disorder. The second raw is for a configuration with a small disordered region delimited by the two orange vertical lines.
}
\label{f:disorder}
\end{SCfigure*}
\subsection*{Macroscopic quantum states}
...
Following Ref.~\cite{bocini2023growing}, let us then 
...
Let us then consider an extensive operator of the form $S_{\vec s}^z=\frac{1}{2}\sum_\ell s_\ell\sigma_\ell^z$, with $s_\ell=\pm 1$, which has the property not to fluctuate outside the lightcone. Because of that, its variance matches  the variance of its restriction to $\Omega_t$. Denoting its expectation value and its variance at time $t$ by $\braket{S^z_{\vec s}}_t$ and $\braket{S^z_{\vec s},S^z_{\vec s}}_t$, respectively, we have 
...
\end{multline}
Assuming \eqref{eq:sigmaasymp}, the right hand side of this equation grows as $t^2$ because the difference in the expectation values grows linearly in time. In fact, we find that even the fluctuations on the second line grow as $t^2$, e.g., if  $s_\ell=\braket{\emptyset;\underline b|\sigma_\ell^z|\emptyset;\underline b}$---see \hyperlink{SI3}{SI}---hence the same conclusion applies also to the marginal case with $p=0$, which is the basic problem studied before. 
%% Figure environment removed
Following Ref.~\cite{Frowis2012Measures}, 
the square root of the variance provides a lower bound to the ``minimal system size for which one has to assume validity of quantum mechanics in order to explain a non-classical phenomenon''. 
...
A related question that we have not addressed is about the first signatures of integrability breaking, which we expect to appear in the sector with three impurities. 

In the integrable case it is instead still unclear the relationship between the lightcone dynamics emerging because of quantum jamming and those emerging because of the existence of infinitely many conservation laws, which have become the signature of the theory of generalised hydrodynamics. %~\cite{Alba2021Generalized}.    
...
%Figure \ref{fig:frog} shows an example of how to insert a column-wide figure. To insert a figure wider than one column, please use the \verb|% Figure environment removed| environment. Figures wider than one column should be sized to 11.4 cm or 17.8 cm wide. Use \verb|\begin{SCfigure*}...\end{SCfigure*}| for a wide figure with side legends.
...
The Hamiltonians in the examples have both a linear and a quadratic dependency on the building blocks in \eqref{eq:build_blocks}. Specifically, up to a constant and a term proportional to $S^z$, which is conserved and irrelevant, the Hamiltonian in \eqref{eq:H} reads
\begin{multline*}
H\sim 2J\sum\nolimits_\ell  \bar T^{+-}_{\ell-1,\ell+1}+ \bar T^{-+}_{\ell-1,\ell+1}+2\Delta\{\bar T^{+-}_{\ell-1,\ell+2},\bar T^{-+}_{\ell-1,\ell+2}\}\\
...
Concerning the non-symmetric example, the interaction in \eqref{eq:HI0} was already expressed in terms of the building blocks $\sigma_\ell^z$. 

\subsection*{Duality mapping of shift invariant operators}
...
$,
where arrows represent here pseudospins. It is then easy to identify the pseudospin operator that acts on the dual fragment as the original spin operator does on the original fragment %, which is exactly the dual operator we were looking for 
(see \hyperlink{SI1}{SI} for explicit expressions).
\subsection*{Bethe Ansatz with two impurities}
In order to solve the two-impurity sector we have used the  Bethe Ansatz
\begin{equation*}
\ket{p_1,p_2}%_{\mathrm m}
\propto\!\!\!\! \sum_{n_1<n_2} [e^{in_1 p_1+in_2 p_2}+S(p_1,p_2)e^{in_1 p_2+in_2 p_1}]\ket{n_1,n_2;\underline b}\, .
\end{equation*}
...
\begin{equation*}
\ket{p}_{\mathrm{b}}=\sum_{n_1<n_2}e^{i\frac{n_1+n_2}{2}p} \Bigl(\frac{\cos\frac{ p}{2}}{ \Delta +g  \cos p}\Bigr)^{n_2-n_1}\ket{n_1,n_2;\underline b}
\end{equation*}
provided that $1+\cos p<2(\Delta + g \cos p)^2$. Their energy reads $E_2(p)=4J(\cos^2\frac{ p}{2}+g^2\cos^2p-\Delta^2)/(\Delta +g\cos  p)$. The reader can find more details in \hyperlink{SI2}{SI}, which also includes the derivation of \eqref{eq:Fmagnon} and \eqref{eq:Fbound}  
...
\acknow{I am grateful to Saverio Bocini for sharing some data of tDMRG simulations (not included here); I also thank him for useful discussions.
...
\appendix
\onecolumn
\section*{\Huge\hypertarget{SI}{Supplemental Information}}
In the next four sections we have collected  some details that might help the reader grasp the results reported in the main text. 
\section*{\hypertarget{SI0}{Expectation values of the local magnetisation}}
Equation~\ref{eq:ellp} allows us to infer a representation of local operators in the pseudospace. In particular we have
...
\end{equation}
where we used the notation introduced  in \eqref{eq:notation}.
...
\section*{\hypertarget{SI1}{Mapping of diagonal operators}}
...
\end{table*}
The duality mapping acts as follows
\begin{equation}
...
\section*{\hypertarget{SI2}{Integrable representation in the presence of two impurities}}
...
:\tilde{ H}:\ket{n_1,n_2}=
 ...
 +2g\delta_{n_2,n_1+1}(\ket{n_1-1,n_1}+\ket{n_1+1,n_1+2})\, ,
\end{multline}
...
\begin{equation}
S(p_1,p_2)=-\frac{1-2[\Delta +\frac{g}{J}\cos(p_1+p_2)]e^{i p_2}+e^{i (p_1+p_2)}}{1-2[\Delta+\frac{g}{J}\cos(p_1+p_2)]e^{i p_1}+e^{i (p_1+p_2)}}\, .
\end{equation}
...
F_n^{(1,2)}(t)=\sum_{n_1\leq n }p_{n_1}^{(1)}(t)=\sum_{n_1\leq n}\sum_{n_2>n_1}|\braket{n_1,n_2|e^{-i\tilde{ H}_{eff} t }\tilde{ P}|\Psi_0}|^2\, .
\end{equation}
\begin{equation}\label{eq:cumdist22}
F_n^{(2,2)}(t)=\sum_{n_2\leq n }p_{n_2}^{(2)}(t)=\sum_{n_1<n_2\leq n}|\braket{n_1,n_2|e^{-i\tilde{ H}_{eff} t }\tilde{ P}|\Psi_0}|^2\, .
\end{equation} 
...
where
\begin{equation}
A_{p_1,p_2}=1-2[\Delta+\frac{g}{J}\cos(p_1+p_2)]e^{i p_1}+e^{i (p_1+p_2)}\, .
\end{equation}
\begin{equation}
...
We can conclude that the density is flat observing that the correction associated with the scattering phase approaches zero as $O(L^{-1})$. We are now in a position to compute the contribution $F_{n,1}^{(j,2)}(t)$ to $F_n^{(j,2)}(t)$ of the excitations with real momenta.
The antisymmetry of the state under the interchange of $p_1$ and $p_2$ allows us to express them as follows
\begin{multline}
F_{n, 1}^{(1,2)}(t)= \frac{1}{4}\sum_{m_1= 0}^\infty\sum_{m=1}^\infty
...
\begin{multline}
F_{n, 1}^{(1,2)}(t)= \frac{1}{2}\int_{-\pi}^\pi\frac{\mathrm d^2 p\mathrm d ^2p'}{(2\pi)^4}\frac{e^{-i[E(p_1)+E(p_2)-E(p_1'+p_1-p_2')-E(p_2'+p_2)]t}e^{-in p_1'}}{1-e^{i(p_1'+i\epsilon)}}\\
...
Thus we have
\begin{equation}
F_{n, 1}^{(1,2)}(t)\sim \frac{1}{2}\int_{-\pi}^\pi\frac{\mathrm d^2 p}{(2\pi)^2}\theta_H(n-\min(v(p_1),v(p_2))t)|e^{i(n_1^{(0)} -n_2^{(0)})(p_1-p_2)}+S(p_1,p_2)|^2
\end{equation}
and analogously for $F_{n, 1}^{(2,2)}(t)$
\begin{equation}
F_{n, 1}^{(2,2)}(t)\sim \frac{1}{2}\int_{-\pi}^\pi\frac{\mathrm d^2 p}{(2\pi)^2}\theta_H(n-\max(v(p_1),v(p_2))t)|e^{i(n_1^{(0)} -n_2^{(0)})(p_1-p_2)}+S(p_1,p_2)|^2
\end{equation}
The incompleteness of the real solutions is evident if we take the limit $n\rightarrow\infty$, which gives
\begin{equation}\label{eq:displacement}
\lim_{n\rightarrow\infty}F_{n, 1}^{(j,2)}(t)\sim 1+\mathrm{Re}\int_{-\pi}^\pi\frac{\mathrm d^2 p}{(2\pi)^2}e^{i(n_2^{(0)} -n_1^{(0)})(p_1-p_2)}S(p_1,p_2)
\end{equation}
We will see before long that the bound state contribution cancels out the displacement from $1$. 


\subsection*{Bound states}
Except for the region $g=J\Delta\in(-\frac{1}{2},\frac{1}{2})$, the Bethe equations exhibit always complex solutions. In the simple case we are considering, such solutions can be easily worked out imposing that, in the Bethe equations , $|e^{iLp_j}|$  approaches either $0$ or $\infty$. 
...
within the region
\begin{equation}
1+\cos p<2(\Delta + \frac{g}{J} \cos p)^2\, ,
\end{equation}
i.e. 
\begin{equation}
\cos p>\frac{1-4 \frac{g}{J}\Delta+\sqrt{1+8\frac{g^2}{J^2}-8 \frac{g}{J}\Delta}}{4 \frac{g^2}{J^2}}\quad \vee\quad  \cos p<\frac{1-4 \frac{g}{J}\Delta-\sqrt{1+8\frac{g^2}{J^2}-8 \frac{g}{J}\Delta}}{4 \frac{g^2}{J^2}}\, .
\end{equation}
Note that this condition is always satisfied when $\frac{g}{J}\Delta>\frac{1}{8}+ \frac{g^2}{J^2}$ or $1<(\Delta+\frac{g}{J})^2\wedge 4g(\Delta+\frac{g}{J})<1$. 
...
\begin{equation}
E_2(p)=4g\cos p+4J[\frac{\cos^2\frac{ p}{2}}{\Delta +\frac{g}{J}\cos  p}-\Delta]
\end{equation}
We can now compute the bound-state contribution to the cumulative distributions. We have
\begin{multline}
F_{n,2}^{(1,2)}(t)=\sum_{m_1= 0}^\infty\sum_{m=1}^\infty|\int_{-\pi}^\pi\frac{\mathrm d p}{2\pi}\theta_H(1+\cos p<2(\Delta + \frac{g}{J} \cos p)^2)e^{-i E_2(p)t}
\braket{-m_1+n,-m_1+n+m|\Psi_2(p)}\braket{\Psi_2(p)|n_1^{(0)},n_2^{(0)}}|^2=\\
 \int_{-\pi}^\pi\frac{\mathrm d p\mathrm d p'}{(2\pi)^2}\theta_H(1+\cos p<2(\Delta + \frac{g}{J} \cos p)^2)\theta_H(1+\cos (p'+p)<2(\Delta + \frac{g}{J} \cos (p'+p))^2)\\
...
\braket{\Psi_2(p)|n_1^{(0)},n_2^{(0)}}
\braket{n_1^{(0)},n_2^{(0)}|\Psi_2(p'+p)}\sim\\
%
\int_{-\pi}^\pi\frac{\mathrm d p}{2\pi}\theta_H(1+\cos p<2(\Delta + \frac{g}{J} \cos p)^2)\theta_H(n-v_2(p))\braket{\Psi_2(p)|n_1^{(0)},n_2^{(0)}}
\braket{n_1^{(0)},n_2^{(0)}|\Psi_2(p)}
\end{multline}
Thus we have
\begin{equation}
F_{n,2}^{(1,2)}(t)\sim \int_{-\pi}^\pi\frac{\mathrm d p}{2\pi}\theta_H(n-v_2(p)t)\max\left[\Bigl(\frac{\Delta +g \cos  p}{\cos \frac{p}{2}}\Big)^2-1,0\right] \Bigl(\frac{\cos^2\frac{ p}{2}}{[\Delta +g  \cos p]^2}\Bigr)^{n_2^{(0)}-n_1^{(0)}}\, .
\end{equation}
Concerning $F_{n,2}^{(2,2)}(t)$, as one could expect from physical arguments based on the fact that the impurities are bounded, we find $F_{n,2}^{(2,2)}(t)\sim F_{n,2}^{(1,2)}(t)$, which we will therefore simply call $F_{n,2}^{(2)}(t)$.
...
\section*{\hypertarget{SI3}{Macroscopicity of the quantum state}}
In this section we work out the variance of $S^z_{\vec s}$ when the system is prepared in a product state with a single impurity. We have
\begin{equation}
\braket{S_{\vec s}^z,S_{\vec s}^z}_t=\frac{1}{4}\sum_\ell (1-\braket{\sigma_\ell^z}^2_t)+\frac{1}{4}\sum_{\ell_1\neq \ell_2}s_{\ell_1} s_{\ell_2}[\sum_{n}p_{n,t}(1-p_{n,t})\braket{n|:\sigma_{\ell_1}^z::\sigma_{\ell_2}^z:|n}-\sum_{n\neq n'}p_{n,t}p_{n',t}\braket{n|:\sigma_{\ell_1}^z:|n}\braket{n'|:\sigma_{\ell_2}^z:|n'}]
\end{equation}
%$$
%\frac{1}{2}\sum_{\ell_2\neq \ell} s_{\ell_2}[\sum_{n}p_{n,t}(1-p_{n,t})\braket{n|\sigma_{\ell}^z\sigma_{\ell_2}^z|n}-\sum_{n\neq n'}p_{n,t}p_{n',t}\braket{n|\sigma_{\ell}^z|n}\braket{n'|\sigma_{\ell_2}^z|n'}]\propto  s_\ell
%$$
In the symmetric case the asymptotic behaviour can be obtained using the simplified expression
\begin{equation}
\braket{n;\underline b|:\sigma_{\ell_1}^z:|n; \underline b}\sim 
\braket{\emptyset;\underline b|\sigma_{\ell_1-y}^z-\sigma_{\ell_1}^z|\emptyset;\underline b}\theta_H(n<x_{\ell_1})\, ,
\end{equation}
indeed the correction is suppressed by $O(p)$, which is assumed to approach zero in time. 
We then find
\begin{equation}
\braket{S_{\vec s}^z,S_{\vec s}^z}_t\sim \frac{1}{4}\sum_{\ell_1\neq \ell_2}[F_{t}(\min(x_{\ell_1},x_{\ell_2}))-F_{t}(x_{\ell_1})F_{t}(x_{\ell_2})]s_{\ell_1} s_{\ell_2}\braket{\emptyset;\underline b|\sigma_{\ell_1-y}^z-\sigma_{\ell_1}^z|\emptyset;\underline b}
\braket{\emptyset;\underline b|\sigma_{\ell_2-y}^z-\sigma_{\ell_2}^z|\emptyset;\underline b}\, .
\end{equation}
Choosing, for example, $s_\ell=\braket{\emptyset;\underline b|\sigma_\ell^z|\emptyset;\underline b}$ gives
\begin{equation}
\braket{S_{\vec s}^z,S_{\vec s}^z}_t\sim \sum_{\ell_1\neq \ell_2}[F_{t}(\min(x_{\ell_1},x_{\ell_2}))-F_{t}(x_{\ell_1})F_{t}(x_{\ell_2})]\braket{\emptyset;\underline b|\frac{1-\sigma_{\ell_1-y}^z\sigma_{\ell_1}^z}{2}|\emptyset;\underline b}
\braket{\emptyset;\underline b|\frac{1-\sigma_{\ell_2-y}^z\sigma_{\ell_2}^z}{2}|\emptyset;\underline b}\, .
\end{equation}
Generally this grows as $t^2$, indeed $F_t(x)$ approaches a function of $x/t$ and $\braket{\emptyset;\underline b|\frac{1-\sigma_{\ell-y}^z\sigma_{\ell}^z}{2}|\emptyset;\underline b}$ has a strictly positive local average. This implies that also the second line of \eqref{eq:variance} grows quadratically in time.  
