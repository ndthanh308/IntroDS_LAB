...
provide for a physicist understanding of their content, nor on the basic mechanism(s) at the
...
rest frame SO(1,3) symmetric vector spectra and decay constants are recovered dynamically.
...
scalar and vector channels are made explicit, and solved. We show that the light front eigenfunctions are
emenable to their covariant counterparts  by integration over the light front energy. In section~\ref{SECV},  all
...
symmetry (spontaneously). This is manifest through the emergence  multi-flavored interactions,
...
\begin{equation}
    G_I=\int d\rho n(\rho)\rho^2(2\pi\rho)^4=\frac{n_{I+\bar{I}}}{2}(4\pi^2\rho^3)^2\left(\frac{1}{m^*\rho}\right)^2
\end{equation}
is fixed by the mean-instanton density 
\begin{equation}
    \frac{n_{I+\bar{I}}}{2}=\int d\rho n(\rho) \prod_{f=1}^{N_f}(m^*_f\rho)
\end{equation}
where $n(\rho)$ denotes the quark-quenched instanton density. Here $m^*_f$ is the determinantal mass,  which accounts for the suppression 
of the tunnelings in the presence of light quarks. At low resolution, the instanton distribution is sharply peaked around the average instanton size $\rho\approx0.31$ fm, with a density $n_{I+\bar{I}}\sim 1$ $\mathrm{fm^{-4}}$. 
...
understtod as the unquenched tunneling density for a molecular configuration, whereby a pair of quark lines is removed by the division $T_{I\bar I}^2$
...
\label{hopping}
%\label{GII}
G_{I\bar{I}}=\frac{G_I^2}{128\pi^2\rho^2N_c}\xi
\end{equation}
where the dimensionless and positive hopping parameter is defined as 
\bea
\label{HOPPING}
\xi=\frac{N_c}{\pi^2\rho^4}\int dud^4R[\rho T_{I\bar{I}}(u,R)]^{2N_f-2}
\eea
...
In sum, we will use  the effective action
\begin{equation}
\label{ACTIONX}
...
the critical coupling $g^{\mathrm{cr}}_{S,\mathrm{ILM}}$, which is set by
\begin{equation}
\label{CRIX}
g^{\mathrm{cr}}_{S,\mathrm{ILM}}=2\pi^2\rho^2\left[8\int_0^{\infty} dz z (zF'(z))^4\right]^{-1}\approx2.981\pi^2\rho^2
\end{equation}
...
\begin{widetext}
\begin{equation}
   \frac{m}{M}=1-\frac{g_S}{2\pi^2\rho^2}\left[8\int_0^{\infty} dz z |z(I_0(z)K_0(z)-I_1(z)K_1(z))'|^4+\rho^2M^2\ln\rho^2M^2+\mathcal{O}(\rho^2 M^2)\right]
\end{equation}
...
\begin{equation}
    \rho^3\langle\bar{\psi}\psi\rangle=-\frac{4N_c}{\pi^2}\rho M\int_0^\infty dz \frac{z^3}{z^2+\frac{\rho^2M^2}{4}}|z(I_0(z)K_0(z)-I_1(z)K_1(z))'|^2
\end{equation}
\end{widetext}



In the standard 2-flavor QCD  instanton vacuum with $G_{I+\bar I}=0$ and $\rho\approx (636\mathrm{MeV})^{-1}$, 
$g^{\mathrm{cr}}_{S,\mathrm{ILM}}$ is approximately $72.64$ $\mathrm{GeV}^{-2}$.  
...
\begin{equation}
\frac{G_{I}}{4N_c}\left(1+\frac{G_I}{8\pi^2\rho^2N_c}\xi\right)\geq g^{\mathrm{cr}}_{S,\mathrm{ILM}}\approx2.981\pi^2\rho^2
\end{equation}
...
in the chiral limit. The larger $\xi$, the snaller the instanton density required for the on-set of chiral symmetry breaking. This effect is also
...
non-propagating or constraint. The elimination of the constaint, induces multi-fermion interactions in terms of the good component. In the mean-field 
approximation, these interactions account for the spontaneous breaking of chiral symmetry on the light front through tadpoles. 
...
\begin{align}
    &\left\langle\bar{\psi}\gamma^+\frac{-i}{\partial_-}\psi\right\rangle\rightarrow  \left\langle\bar{\psi}\mathcal{F}(i\partial)\gamma^+\frac{-i}{\overleftrightarrow{\partial_-}}[\mathcal{F}(i\partial)\psi]\right\rangle\\
...
\begin{widetext}
\begin{equation}
    \frac{1}{2N_c}\left\langle\bar{\psi}\mathcal{F}(i\partial)\gamma^+\frac{-i}{\overleftrightarrow{\partial_-}}[\mathcal{F}(i\partial)\psi]\right\rangle \rightarrow w_+(P^+)=\int\frac{dk^+d^2k_\perp}{(2\pi)^3}\frac{\epsilon(k^+)}{P^+-k^+}\mathcal{F}(k)\mathcal{F}(P-k)
\end{equation}
...
\textbf{scalar modes}
\begin{equation}
    1-\frac{g_{\sigma,a_0}}{g_\sigma}\left(1-\frac{m}{M}\right)=-2g_{\sigma,a_0}(m^2_{\sigma,a_0}-4M^2)\int_0^1dx\int\frac{d^2k_\perp}{(2\pi)^3}\frac{1}{x\bar{x}m_{\sigma,a_0}^2-(k_\perp^2+M^2)}\mathcal{F}(k)\mathcal{F}(P-k)
\end{equation}

\textbf{pseudoscalar modes}
\begin{equation}
    1-\frac{g_{\pi,\eta'}}{g_\sigma}\left(1-\frac{m}{M}\right)=-2g_{\pi,\eta'}m^2_{\pi,\eta'}\int_0^1dx\int\frac{d^2k_\perp}{(2\pi)^3}\frac{1}{x\bar{x}m_{\pi,\eta'}^2-(k_\perp^2+M^2)}\mathcal{F}(k)\mathcal{F}(P-k)
\end{equation}
...
\begin{equation}
\label{t_mass}
    1-\frac{g_V}{g_S}\left(1-\frac{m}{M}\right)=-2g_V\int_0^1dx\int\frac{d^2k_\perp}{(2\pi)^3}\frac{m_{\omega,\rho}^2-2k_\perp^2}{x\bar{x}m_{\omega,\rho}^2-(k_\perp^2+M^2)}\mathcal{F}(k)\mathcal{F}(P-k)
\end{equation}

\textbf{longitudinal modes}
\begin{equation}
\label{l_mass}
1=-8g_V\int_0^1dx\int\frac{d^2k_\perp}{(2\pi)^3}\frac{k_\perp^2+M^2}{x\bar{x}m_{\omega,\rho}^2-(k_\perp^2+M^2)}\mathcal{F}(k)\mathcal{F}(P-k)
\end{equation}
...
\label{INTX}
    ...
    &I_3(P^2)=\int\frac{d^4k}{(2\pi)^4}\frac{4i(k\cdot P)^2/P^2}{[(k-P/2)^2-M^2][(k+P/2)^2-M^2]}\mathcal{F}(k-P/2)\mathcal{F}(k+P/2)
\end{align}
\end{widetext}
...
However, these mixing contributions are suppressed in $1/N_c$ or $G_V/G_I$, much like in the light front case. 
...
For $g_S=2.544\times2\pi^2\rho^2$, the $\sigma$ mass is $m_\sigma=2M=752.5$ MeV in the chiral limit.
...
\end{equation}
where the pion decay constant follows as
\begin{equation}
...
This is manifest if we average over the transverse eigenvalue or gap-like equation \eqref{t_mass} and longitudinal eigenvalue equation \eqref{l_mass},
...
\end{equation}
in agreement with the covariant gap-like equation for the vector mesons obtained in the covariant frame. With the explicit form factors, 
(\ref{AVERAGEX}) is 
\begin{equation}
...
in the vector channels occur only for a finite range of $g_V$.  In the chiral limit with a constituent mass $M=376.2$ MeV, the range is
...
\begin{tabular}{|c|c|c|c|c|}
    ...
    \hline
     ILM (this work)& $133.0$ & $133.0 $ & $772.0  $ & $772.0  $  \\
...
\bea
\label{PARAX}
g_S&=&2.544 \times 2 \pi^2\rho^2=123.97~ \mathrm{GeV}^{-2} \nonumber\\
g_V&=&0.448 \times 2 \pi^2\rho^2=21.82~ \mathrm{GeV}^{-2}
\eea
with the current quark mass $m=13.6~\mathrm{MeV}$.
...
and anti-instanton, is found to be $m_{det}\approx127.9$ MeV, which is close to the value of 103 MeV in~\cite{Shuryak:2021fsu,Faccioli:2001ug}.
The fixed parameters (\ref{PARAX}) translate to single and molecular induced couplings  as
$$G_I=440.04 \mathrm{GeV}^{-2}\qquad\qquad G_{I\bar{I}}=65.47 \mathrm{GeV}^{-2}$$
The dimensionless hopping parameter   $\xi=3.164$ is fixed by \eqref{hopping}. 
...
\label{LF_L}
    ...
    =&\sqrt{2}\pi\left[\int_0^1dx\int_0^\infty dz x\bar{x}z\left(\frac{4\lambda_V^2}{\rho^2m_{\omega,\rho}^2}+z\frac{4x\bar{x}\left(x\bar{x}z^2+\frac{\rho^2M^2}{4\lambda_V^2}\right)}{\left(x\bar{x}z^2-\frac{\rho^2}{4\lambda_V^2}\left(x\bar{x}m_{\omega,\rho}^2-M^2\right)\right)^2}\right)\left(zF'(z)\right)^4\right]^{-1/2}
\end{aligned}
\end{equation}
\end{widetext}
...
\label{vec_norm}
\begin{aligned}
\frac{1}{C^2_{\omega_L,\rho_L}}=&\frac{1}{C_{\omega_T,\rho_T}^2}+\frac{2}{m_{\omega,\rho}^2}\int_0^1 dx\int \frac{d^2k_\perp}{(2\pi)^3}\mathcal{F}(k)\mathcal{F}(P-k)
\end{aligned}
\end{equation} 
which reflects on the irremediable loss of Lorentz symmetry on the light front. Fortunately,
our book-keeping in $1/N_c$ suggests that
$$
\frac{C^2_{\omega_T,\rho_T}}{g_{\omega,\rho}m^2_{\omega,\rho}}\sim\mathcal{O}(N_c^0)
$$
...
\begin{equation}
\begin{aligned}
\frac{1}{C^2_{\omega_L,\rho_L}}
\simeq&\frac{1}{C_{\omega_T,\rho_T}^2}\left[1+\mathcal{O}\left(\frac{g_V}{2\pi^2\rho^2}\right)\right]
\end{aligned}
\end{equation}
...
\begin{tabular}{|c|c|c|c|c|c|c|}
   \hline
   Model  &  $|C_{\sigma}|$ & $|C_{\pi}|$ & $|C_{\eta'}|$ & $|C_{\omega_T,\rho_T}|$ & $|C_{\omega_L,\rho_L}|$ & $|C_{\omega,\rho}|$\\
   \hline
   ILM  & 4.227 & 4.706 & 7.395 & 2.554 & 2.202 & 2.560  \\
   \hline
\end{tabular}
\end{center}
\vskip 0.5cm
...
The covariance-violating term in $C_{\omega_L,\rho_L}$ is numerically estimated to be $0.369$ which is of order $\mathcal{O}\left(\frac{g_{V}}{2\pi^2\rho^2}\right)$,
...
covariant frame, can be shown to agree with our derivation from the light front after a pertinent integration over the light front energy. Indeed, in the covaiant
...
The effective quark-meson couplings $g_{Xqq}$  follow as
 \begin{align}
 \label{Xqq_coupling}
     &g_{\sigma qq}^2=\left(\frac{\partial \Pi_{SS}}{\partial P^2}\right)^{-1}\bigg|_{P^2=m^2_{\sigma}}\nonumber\\
...
In Fig.~\ref{TLXX1}  we show our results for the vector mesons PDFs versus $x$, in the QCD instanton vacuum for $\rho=0.31$ fm, $m_{\omega, \rho}= 772$ MeV 
and a constituent mass $M=398.2$ MeV. The transverse PDF for the vectors is in solid-blue, the longitudinal PDF for the vectors is in solid-brown and the 
unpolarized PDF in solid-green. Note the small differences due to the parametrically small value of $g_V/g_S$, with the exception of the end-points for the
longitudinal polarization. The unpolarized PDF is identical to that from the covariant analysis. 
In Fig.~\ref{TLXX2}  we show the parton density functions versus parton-$x$, for the sigma meson in solid-blue, the pion in solid-green and the rho meson
in solid-brown in the chiral limit. In this limit, the constituent mass is $M=372.3$ MeV.  
...
\end{equation}
To enforce current conservation, we approximate
$$
x\bar{x}m_{\omega,\rho}^2+k^2_\perp+M(k)M(P-k)\approx(x\bar{x}m_{\omega,\rho}^2+k^2_\perp+M^2)\mathcal{F}^2\left(\frac{k_\perp}{2\lambda_V\sqrt{x\bar{x}}}\right)
$$
which amounts to 
\begin{equation}
\begin{aligned}
        f_{\omega,\rho}
        &=-\frac{\sqrt{2N_c}}{2\pi^2m_{\omega,\rho}}C_{\omega_L,\rho_L}\frac{4\lambda_V^2}{\rho^2}\int_0^1dx\int_0^{\infty} dzz  \frac{2x\bar{x}\left[\frac{\rho^2}{4\lambda_V^2}(x\bar{x}m_{\omega,\rho}^2+M^2)+x\bar{x}z^2\right]}{\left[\frac{\rho^2}{4\lambda_V^2}(x\bar{x}m^2_{\omega,\rho}-M^2)-x\bar{x}z^2\right]} (zF'(z))^6 \\
\end{aligned}
\end{equation}
...
\begin{aligned}
    \phi^{V}_{\omega,\rho}(x)
    =&-\frac{\sqrt{2N_c}}{2\pi^2f_{\omega,\rho}m_{\omega,\rho}}C_{\omega_L,\rho_L}\frac{4\lambda_V^2}{\rho^2}\int_0^{\infty} dz z\frac{2x\bar{x}\left[\frac{\rho^2}{4\lambda_V^2}(x\bar{x}m_{\omega,\rho}^2+M^2)+x\bar{x}z^2\right]}{\left[\frac{\rho^2}{4\lambda_V^2}(x\bar{x}m^2_{\omega,\rho}-M^2)-x\bar{x}z^2\right]} \left(zF'(z)\right)^6
\end{aligned}
\end{equation}
...
solid-blue, th evolved DA with $\mu=2$ GeV in solid-brown, and the QCD asymptotic result $6x\bar x$ in dashed-black. 
We note that $\phi_\pi^A(x\rightarrow1,\mu=0.313 \mathrm{GeV})\sim6(1-x)^{0.953}$  near the  end point, and asymptotes the QCD result at
infinite resolution.
...
solid-blue, the evolved DA with $\mu=2$ GeV in solid-brown, and the QCD asymptotic result $6x\bar x$ in dashed-black. 
The longitudinal vector DA  at the end point scales as $\phi_\rho^V(x\rightarrow1,\mu=0.313 \mathrm{GeV})\sim6(1-x)^{1.03}$ asymptotically. 
...
$\phi_\rho^T(x\rightarrow1,\mu=0.313 \mathrm{GeV})\sim6(1-x)^{0.93}$ at the initial scale. 
...
\begin{align*}
    f_\pi =&130.3~\mathrm{MeV} & f_\rho =&229.4~\mathrm{MeV}  & f^T_\rho =&166.26~\mathrm{MeV} & f^T_\rho/f_\rho =0.753
\end{align*}
...
Our results are compared with the lattice calculations \cite{Braun:2016wnx}, and the values quoted by the Particle Data Group \cite{ParticleDataGroup:2018ovx} in the table below. The transverse $\rho$ decay constant $f^T_\rho$ is evolved to $2$ GeV starting from $0.226$ GeV using \eqref{fT_evol} when  compared with the lattice. 
...
\begin{tabular}{|l|c|c|c|c|}
    ...
    \hline
    ILM(this work) & $130.3$ & $220.6$ & $122.32$ & $0.554$ \\
...
the Hadron Spectrum Collaboration~\cite{Shultz:2015pfa}. The results from the Bethe-Salpeter in the NJL model are shown in dashed-orange. Clearly, the
leading Fock-component in the QCD instanton vacuum, agrees well with the reported lattice results. This is in sharp contrast to the pion EM form factor.
...
\bea
    ...
    r^{\mathrm{VDM}}_\pi&=&0.626~\mathrm{fm} \nonumber\\
    r^{exp}_\pi&=&0.659\pm0.004~\mathrm{fm}
\eea
Without the rho-cloud, the bare EM size of the pion $r_\pi$, is slightly larger than the size of an instanton $\rho=0.31$ fm. Pions
are collective Goldstone modes, strongly bound by single instantons (anti-instantons) of size $\rho$. In contrast, the EM size
 of the rho and omega $r_{\omega,\rho}=0.807~\mathrm{fm}$
