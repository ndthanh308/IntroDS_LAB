...
The paper is structured as follows: In Section 2, we introduce weak Dirichlet processes, generalized stochastic integration, and stochastic flows. Section 3 gives our result, namely the extension of the It{\^o}-Ventzell formula to stochastic flows in $C^{0,1}$, while the proof of this extension is shown in Section 4.
...
\begin{definition} \label{def: quadratic variation alternative definition}
	Let $X$ and $Y$ be two continuous processes. Then, we define for all $t \in [0,T]$:
	\begin{align*}
...
Opposed to the classical It{\^o} formula, the It{\^o}-Ventzell formula considers stochastic flows, which are defined in the following definition. Similar, but slightly different definitions can be found, e.g., in Kohlmann, Siyuan, and Xiong \cite[p.7]{Kohlmann} or dos Reis and Platonov \cite[p.6]{dos2022ito}.
\begin{definition} \label{def: stochastic flow}
	...
	\begin{enumerate}[(a)]
		...
		\item $F(t,\cdot)$ is a progressively measurable process for all fixed $x \in \Real^d$,
		\item $\beta: \Omega \times \Real_{\geq 0} \times \Real^d \to \Real$ and $\gamma: \Omega \times \Real_{\geq 0} \times \Real^d \to \Real^d$ are progressively measurable random fields,
		\item $\gamma(t,x)$ is $\diff \PP \times \diff t$-a.s. locally $\alpha$-H{\"o}lder continuous in $x$ for an $\alpha >0$, and
		\item \label{def, item: integrability}for any $K \subset \Real^d$ compact and for any $S \geq 0$ it holds a.s.:
			\begin{align*}
				&\int_0^S  \sup_{x \in K} |\beta(t,x)| \diff t < \infty, &
				&\int_0^S  \sup_{x \in K} |\gamma(t,x)|^2 \diff t < \infty.
			\end{align*}
	\end{enumerate}
\end{definition}
...
\begin{theorem} \label{th: main result}
	...
	\end{align*}
	where $\BB^X$ is linear (in $F$) and continuous. Moreover,
	\begin{enumerate}[(a)]
...
The assumptions of Theorem \ref{th: main result} are satisfied (through Assumption \ref{th, item: beta integrability} and \ref{th, item: gamma bounded}) if the mappings $(t,x) \mapsto \beta(t,x)$ and $(t,x) \mapsto \gamma(t,x)$ are a.s. continuous. %c{\`a}dl{\`a}g.
...
\section{Proof of Theorem \ref{th: main result}}

The theorem generalizes the result of Gozzi and Russo \cite[pp.1571-1574]{Gozzi} to stochastic flows. Before proving it, we need some preliminary results. In the whole section, we use the convention from Remark \ref{rem: constant continuation} for the constant continuation of processes and functions. 

By Russo and Vallois \cite[p.89]{russo1995generalized} it holds:
\begin{remark} \label{rem: cadlag ucp}
...
\begin{lemma} \label{lem: ucp convergence single}
	Let $X$ be a FQV process, $t \in [0,T]$, and $Z_r^{\varepsilon}$ be such that $\int_0^T \left|Z_r^{\varepsilon}\right|^2 \diff r \xrightarrow[\PP]{\varepsilon \to 0} 0$. Then:
	\begin{align*}
...
\begin{definition}
	A set of random variables $(X_n)_{n \in \mathbb{N}}$ is bounded in probability (also known as uniformly tight) if there exists for all $\varepsilon>0$ an $M>0$ such that $\sup_{n \in \mathbb{N}} \PP (|X_n|>M)<\varepsilon$.
\end{definition}
...
\begin{lemma} \label{lem: ucp convergence single L1}
	Let $X$ be a continuous FQV process, $t \in [0,T]$, and $Z_r^{\varepsilon}$ be bounded in probability for $\varepsilon>0$. Then:
	\begin{align*}
...
\begin{proof}
	Let $\omega \in \Omega$ be arbitrary such that $X(\omega)$ is continuous. Then, there exists a $K$ compact such that $X_r (\omega) \in K$ for all $r \in [0,T]$. Thus, $X$ is uniformly continuous on $[0,T]$, i.e., $\sup_{r \in [0,T]} \left| X_{r+\varepsilon} (\omega)-X_{r} (\omega) \right| \xrightarrow[a.s.]{\varepsilon \to 0} 0$. Hence, we get:
	\begin{align*}
...
\begin{lemma} \label{lem: ucp convergence double}
	Let $X$, $Y$ be two FQV processes, $t \in [0,T]$. Moreover, let $Z_r^{\varepsilon}$ be such that  $\sup_{r \in [0,T]} \left|Z_r^{\varepsilon} \right| \xrightarrow[\PP]{\varepsilon \to 0} 0$. Then:
	\begin{align*}
...
\begin{proof}
	...
	\end{align*}
	and the lemma follows from Remark \ref{rem: cadlag ucp}.
...
\begin{myproof}[Proof of Theorem \ref{th: main result}]
	...
	\begin{align*}
		\BB^X (F)_t = F(t,X_t) - F(0,X_0) - \int_0^t \gamma (r,X_r) \diff W_r - \int_0^t F_x (r,X_r) \diff M_r.
	\end{align*}
	Hence, $\BB^X$ is linear (in $F$) and continuous. 
	...
	\end{align*}
	For $I_{2,1}$, we get due to Remark \ref{rem: cadlag ucp} and Assumption \ref{th, item: gamma continuity}:
	\begin{align*}
	...
	\end{align}
	Thus, $\widetilde{Z}^{\varepsilon}_r \xrightarrow[\LL^2(\diff \PP)]{\varepsilon \to 0} 0$ for all $r \in [0,T]$ in both cases. Analogously to \eqref{eq: 2rho moment 2a} or \eqref{eq: 2rho moment 2b} respectively, we also get $\sup_{r \in [0,T]} \EX |\widetilde{Z}^{\varepsilon}_r |^{2} \leq C \cdot \max \{ \tilde{B}_2^2, B_2\}$. 
