...
\maketitle
\begin{abstract}
	This paper proves an extension of the It{\^o}-Ventzell formula that applies to stochastic flows in $C^{0,1}$ for continuous weak Dirichlet processes. 
\end{abstract}
...
\begin{definition}
	Let $X$ be a continuous process. If $[X]_t < \infty$ for all $t \in [0,T]$, then we call $X$ a finite quadratic variation (FQV) process.
\end{definition}
...
\begin{definition}
	A real continuous process $X$ is called a weak Dirichlet process if $X$ is decomposable into a local martingale $M$ and a weak zero energy process $A$, i.e., $X=M+A$. \\
...
\begin{definition} \label{def: stochastic flow}
	...
	\begin{enumerate}[(a)]
		...
		\item $\beta: \Omega \times \Real_{\geq 0} \times \Real^d \to \Real$ and $\gamma: \Omega \times \Real_{\geq 0} \times \Real^d \to \Real^d$ are progressively measurable and previsible random fields,
...
The assumptions of Theorem \ref{th: main result} are satisfied (through Assumption \ref{th, item: beta integrability}) %and \ref{th, item: gamma bounded}) 
...
\begin{lemma} \label{rem: cadlag ucp}
	Let $X$ and $Y$ be two continuous FQV processes and $H$ be a c{\`a}dl{\`a}g process defined on the interval $[0,T]$. Then, it holds:
	\begin{align*}
...
\begin{lemma} \label{lem: ucp convergence single}
	Let $X$ be a FQV process, $t \in [0,T]$, and $Z_r^{\varepsilon}$ be a progressively measurable process for all $\varepsilon>0$ such that $\int_0^T \left|Z_r^{\varepsilon}\right|^2 \diff r \xrightarrow[\PP]{\varepsilon \to 0} 0$. Then:
	\begin{align*}
...
\begin{proof}
	...
	\begin{align*}
		\sup_{t \in [0,T]} \left| \int_0^t Z^{\varepsilon}_r \frac{X_{r+\varepsilon}-X_{r}}{\sqrt{\varepsilon}} \diff r \right| \leq \int_0^T \left| Z^{\varepsilon}_r \frac{X_{r+\varepsilon}-X_{r}}{\sqrt{\varepsilon}} \right| \diff r \leq \sqrt{\int_0^T \left| Z^{\varepsilon}_r \right|^2 \diff r} \sqrt{\int_0^T \frac{(X_{r+\varepsilon}-X_{r})^2}{\varepsilon} \diff r}.
	\end{align*}
...
\begin{lemma} \label{lem: ucp convergence single L1}
	Let $X$ be a continuous FQV process, $t \in [0,T]$, and $Z_r^{\varepsilon}$ be a progressively measurable process for all $\varepsilon>0$ such that $\int_0^T |Z_r^{\varepsilon}| \diff r$ is bounded in probability for $\varepsilon>0$. Then:
	\begin{align*}
...
\begin{lemma} \label{lem: ucp convergence double}
	Let $X$, $Y$ be two FQV processes, $t \in [0,T]$. Moreover, let $Z_r^{\varepsilon}$ be a progressively measurable process for all $\varepsilon>0$ such that  $\sup_{r \in [0,T]} \left|Z_r^{\varepsilon} \right| \xrightarrow[\PP]{\varepsilon \to 0} 0$. Then:
	\begin{align*}
...
\begin{proof}
	We get: % with Hölder's inequality:
	\begin{align*}
...
\begin{myproof}[Proof of Theorem \ref{th: main result}]
	...
	First of all, since $X$ is continuous, we can use a classical localization argument to choose a compact set $K \subset \Real$ such that $X_t (\omega) \in K$ for all $t \in [0,T]$ and almost all $\omega \in \Omega$. Then, by using again classical localization arguments%(see, e.g., Steele \cite[pp.95-110]{steele2001stochastic})
	, we can assume that $\sup_{x \in K} |\beta (t,x)| \in \LL^{1}(\Omega \times [0,T],\diff \PP \times \diff t)$, $\sup_{x \in K} |\gamma (t,x)| \in \LL^{2}(\Omega \times [0,T],\diff \PP \times \diff t)$ both from Definition \ref{def: stochastic flow}(\ref{def, item: integrability}). Now, the prerequisites of Lemma \ref{lem: substitution} are fulfilled, and, in combination with the Assumption \ref{th, item: beta integral expected value} and Lemma \ref{lem: show old assumption}, %\ref{th, item: gamma expected value}, 
	...
	In particular, we get for any arbitrary $L>0$ a $0<\rho_{\omega}\leq1$ such that $\sup_{r \in [0,T]} \left| Y_r^{\varepsilon} (\omega) \right| \leq L$ for all $0<\varepsilon \leq \rho_{\omega}$. Hence, $\sup_{r \in [0,T]} \left| Y_r^{\varepsilon} (\omega) \right| \xrightarrow[a.s.]{\varepsilon \to 0} 0$ and therefore also in probability. \\
	Next, note that it holds that $X_{r}(\omega)+\lambda(X_{r+\varepsilon} (\omega) -X_{r}(\omega)) \in K$ for all $\lambda \in [0,1]$ since $X_r(\omega) , X_{r+\varepsilon}(\omega) \in K$. Moreover, $X_r (\omega)$ is uniformly continuous in $r$ for $r \in [0,T]$, i.e., for all $\tilde{L}>0$ there exists a $0<\tilde{\rho}_{\omega}\leq1$ such that for all $r \in [0,T]$ and for all $0<\varepsilon \leq \tilde{\rho}_{\omega}$ it holds: $\left| X_{r+\varepsilon}(\omega) -X_r(\omega) \right| \leq \tilde{L}$. Let $L>0$ arbitrary. 
	...
	Since we chose $L>0$ and $\omega \in \Omega$ arbitrary, $\sup_{r \in [0,T]} \left| Z_r^{\varepsilon} \right| \xrightarrow[a.s.]{\varepsilon \to 0} 0$ and therefore also in probability. Now, Lemma \ref{lem: ucp convergence double} yields that $\lim_{\varepsilon \to 0} R_{1,1} (t,\varepsilon) = 0$ and $\lim_{\varepsilon \to 0} R_{1,2} (t,\varepsilon) = 0$ in $ucp$.
	...
	\end{align*}
	The first fraction is $\mathcal{N} (0,1)$-distributed for all $\varepsilon$, and hence converges in distribution to $Z \sim \mathcal{N} (0,1)$ for $\varepsilon \to 0$. The second part converges in probability to $0$ for $\varepsilon \to 0$ due to Protter \cite[p.96]{Protter} and the continuity in the first argument and adaptedness of $\gamma$. Hence, Slutsky implies that $\widetilde{Z}^{\varepsilon}_r \xrightarrow[d]{\varepsilon \to 0} 0$. Due to $0$ being constant, this convergence also holds in probability.\\
...
In this section, we apply Theorem \ref{th: main result} to derive novel stochastic calculus formulas for quadratic variations and a representation result for strong solutions of SPDEs.
...
Note that in the previous proof, we could not use the It{\^o}-Ventzell formula in $C^{0,2}$ or $C^{1,2}$ since $F$ is not twice continuously differentiable in the space direction.

\begin{proposition}
	Let $W$ be a one-dimensional Brownian Motion. Then, it holds that
	\begin{align*}
		[((W_t)_+)^2 W_t,W_t] = \int_{0}^{t} ((W_r)_+)^2 \diff [W]_r + 2 \int_{0}^{t} ((W_r)_+) W_r \diff [W]_r,
	\end{align*}
	where $x_+ := \max \{x,0\}$.
\end{proposition}
...
In this subsection, the overall target is to give a representation result for strong solutions of stochastic elliptic PDE for a special type of weak Dirichlet processes. Therefore, we start with stating the SPDE giving a generalization of the solution theory (naturally generalizing the theory of Gozzi and Russo \cite{Gozzi} who considered PDEs). As in Gozzi and Russo \cite{Gozzi}, in the elliptical case all of the relating function are independent of $t$ due to the elliptical problem being time-independent.

Let $b: \Real^d \to \Real^d$ and $\sigma: \Real^d \to \Real^{d \times d}$ be two continuous functions. Then, we define the elliptic operator $L_0: D(L_0) \subset C^0 (\Real^d) \to C^0 (\Real^d)$ with $D(L_0) = C^2 (\Real^d)$ by $L_0 F(x) := b(x) F_x(x) + \frac{1}{2} tr(\sigma^\intercal(x) F_{xx}(x) \sigma(x)$, where $tr$ denotes the trace of a matrix. In total, we then consider the stochastic inhomogeneous elliptic problem for all $x \in \Real^d$ and almost all $\omega \in \Omega$:
\begin{align} \label{def: elliptic pde}
	L_0 F(\omega,x) = h(\omega,x), 
\end{align}
where $h(\omega,\cdot) \in C^0(\Real^d)$ almost surely.

\begin{definition}
	We say that $F$ with local characteristics $(\beta(x),\gamma(x))$ is a strict solution to the stochastic elliptic problem \eqref{def: elliptic pde} if for a.e. $\omega \in \Omega$ it holds that $F(\omega,\cdot) \in D(L_0)$, $\beta(\omega,\cdot) \in C^{1}(\Real)$, $\gamma(\omega,\cdot) \in C^{1+\alpha}(\Real^d)$ for an $\alpha \in (0,1)$, and \eqref{def: elliptic pde} holds.
\end{definition}
...
\begin{definition} \label{def: strong solution}
	We say that $F$ is a strong solution to the stochastic elliptic problem \ref{def: elliptic pde} if there exists a sequence $(F^n)_{n \in \N}$ with local characteristics $(\beta^n(\omega,x),\gamma^n(\omega,x))$ and $F^n (\omega,\cdot) \in D(L_0)$ for all $n \in \N$ and a sequence $(h^n)_{n \in \N}$ with $h^n (\omega,\cdot) \in C^0(\Real^d)$ for all $n \in \N$ such that for every $n \in \N$ and a.e. $\omega \in \Omega$ holds:
	\begin{enumerate}
		\item $\beta^n(\omega,\cdot) \in C^{1}(\Real)$, and $\gamma^n(\omega,\cdot) \in C^{1+\alpha}(\Real^d)$ for an $\alpha \in (0,1)$,
		\item $F^n$ is a strict solution of $L_0 F^n(\omega,x) = h^n(\omega,x)$ for all $x \in \Real^d$ and a.e. $\omega \in \Omega$,
		\item $F^n (\omega,\cdot) \xrightarrow{n \to \infty} F(\omega,\cdot)$ and $h^n (\omega,\cdot) \xrightarrow{n \to \infty} h(\omega,\cdot)$ in $C^0(\Real^d)$ (equipped with the supremum-norm on compact sets), and
		\item $\beta^n (\omega,\cdot) \xrightarrow{n \to \infty} \beta (\omega,\cdot)$ in $\LL^1(K;\diff t)$ for all compact sets $K \subset \Real$.
	\end{enumerate}
\end{definition}

Note that this a real generalization of the notion of a strict solution since every strict solution is also a strong solution by taking the constant sequences $F^n := F$ and $h^n := h$. The representation theorem states then:

\begin{theorem}
	Let $b: \Real^d \to \Real^d$ and $\sigma: \Real^d \to \Real^{d \times d}$ be two continuous functions, and $F$ (with local characteristics $(\beta(\omega,x),\gamma(\omega,x))$) be a strong solution of problem \eqref{def: elliptic pde} where $F^n$ (with local characteristics $(\beta^n(\omega,x),\gamma^n(\omega,x))$) denotes the approximating sequence. Define $X_t := x + \int_{0}^t \sigma (X_r) \diff r + A_t$, where $x \in \Real^d$ and $A$ is a continuous weak zero energy process such that $X$ is a continuous FQV process. Furthermore, we assume:
	\begin{enumerate}
		\item $F(\omega,\cdot) \in C^1(\Real^d)$ for a.e. $\omega \in \Omega$,
		\item $\beta$ and $\gamma$ fulfill the assumptions of Theorem \ref{th: main result} with $\gamma(\omega,\cdot) \in C^1 (\Real^d)$,
		\item It holds in $ucp$ that
		\begin{align} \label{eq: elliptic pde assumption}
			\lim_{n \to \infty} \int_0^t (F_x (X_r)-F^n_x (X_r)) \diff^- A_r &+ \int_0^t (\gamma_x (X_r)-\gamma^n_x (X_r)) \sigma(X_r) \diff r \notag \\
			&+ \int_0^t (F_x (X_r)-F^n_x (X_r)) b(X_r) \diff r = 0.
		\end{align}
	\end{enumerate}
	Then, it holds that (suppressing the dependence on $\omega$)
	\begin{align*}
		F(X_t) =& F(X_0) + \int_0^t \gamma(X_r) \diff W_r + \int_0^t F_x(X_r) \sigma(X_r) \diff W_r + \int_0^t \beta (X_r) \diff r + \int_0^t F_x (X_r) \diff^- A_r \\
		&+ \int_0^t \gamma_x (X_r) \sigma(X_r) \diff r
		+ \int_0^t h (X_r) \diff r - \int_0^t b(X_r) F_x (X_r) \diff r.
	\end{align*}
\end{theorem}
...
\begin{proof}
	...
	\begin{align}
		F^n(X_t) =& F^n (X_0) + \int_0^t \gamma^n (X_r) \diff W_r + \int_0^t F^n_x(X_r) \sigma(X_r) \diff W_r + \int_0^t \beta^n (X_r) \diff r + \int_0^t F^n_x (X_r) \diff^- A_r \notag \\
		&+ \int_0^t \gamma^n_x (X_r) \sigma(X_r) \diff r + \frac{1}{2} \int_0^t \sigma^\intercal (X_r) F^n_{xx} (X_r) \sigma(X_r) \diff r \notag \\
		=& F^n (X_0) + \int_0^t \gamma^n (X_r) \diff W_r + \int_0^t F^n_x(X_r) \sigma(X_r) \diff W_r + \int_0^t \beta^n (X_r) \diff r + \int_0^t F^n_x (X_r) \diff^- A_r \notag \\
		&+ \int_0^t \gamma^n_x (X_r) \sigma(X_r) \diff r + \int_0^t L_0 F^n (X_r) \diff r - \int_0^t b(X_r) F^n_x (X_r) \diff r. \label{eq: elliptic pde c2}
	\end{align}
	Now, we define:
	\begin{align}
		M_t^n :=& F^n(X_t)-F^n (X_0) - \int_0^t \beta^n (X_r) \diff r - \int_0^t F^n_x (X_r) \diff^- A_r - \int_0^t \gamma^n_x (X_r) \sigma(X_r) \diff r \notag \\
		&- \int_0^t L_0 F^n (X_r) \diff r + \int_0^t b(X_r) F^n_x (X_r) \diff r, \notag \\
		M_t :=& F(X_t)-F (X_0) - \int_0^t \beta (X_r) \diff r - \int_0^t F_x (X_r) \diff^- A_r - \int_0^t \gamma_x (X_r) \sigma(X_r) \diff r \notag \\
		&- \int_0^t h (X_r) \diff r + \int_0^t b(X_r) F_x (X_r) \diff r. \label{eq: mt equation}
	\end{align}
	Note that for almost all $\omega \in \Omega$, it holds that $L_0 F^n (X_r(\omega)) = h^n(X_r(\omega)) \xrightarrow{n \to \infty} h (X_r(\omega))$ uniformly in $r$ due to 5. in Definition \ref{def: strong solution} and $X_r(\omega)$ taking values in a compact set for $r \in [0,t]$. Then, by the assumptions of a strong solution and by \eqref{eq: elliptic pde assumption}, it holds that $M_t^n \xrightarrow{n \to \infty} M_t$ in $ucp$. Moreover, it follows from \eqref{eq: elliptic pde c2} that $M_t^n$ is a local martingale. Proposition 4.4 of Gozzi and Russo \cite{Gozzi} implies then that $M$ is also a local martingale. Furthermore, note that $M_t$ is continuous in $t$ since $X$, $F$, and $A$ are continuous. Then, we define the weak zero energy process
	\begin{align*}
		\tilde{\mathcal{B}}^X (F)_t :=& \int_0^t \beta (X_r) \diff r + \int_0^t F_x (X_r) \diff^- A_r + \int_0^t \gamma_x (X_r) \sigma(X_r) \diff r + \int_0^t h (X_r) \diff r - \int_0^t b(X_r) F_x (X_r) \diff r.
	\end{align*}
	Rearranging \eqref{eq: mt equation} implies that $F(X_t) = F(X_0) + M_t + \tilde{\mathcal{B}}^X (F)_t$.	Next, we apply Theorem \ref{th: main result} on $F$ to get that $F(X_t) = F(X_0) + \int_0^t \gamma(X_r) \diff W_r + \int_0^t F_x(X_r) \sigma(X_r) \diff W_r + \mathcal{B}^X (F)_t$, where $\tilde{M}_t := \int_0^t \gamma(X_r) \diff W_r + \int_0^t F_x(X_r) \sigma(X_r) \diff W_r$ is a continuous local martingale. Finally, since $M_t$ and $\tilde{M}_t$ are continuous, the decomposition of the weak Dirichlet process $F(X_t)$ is unique. In particular, we get that $\mathcal{B}^X (F)_t = \tilde{\mathcal{B}}^X (F)_t$ which implies the claim.
\end{proof}
