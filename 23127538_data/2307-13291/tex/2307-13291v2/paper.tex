...
%Title of paper
\title{Different versions of soft-photon theorems. Leading and next-to-leading terms for pion-pion and pion-proton scattering}
...
In the present paper we reconsider the leading and next-to-leading terms 
...
We use only exact QFT methods in this framework:
%
\begin{itemize}
...
\begin{eqnarray}
\tilde{s} = (\tp_{a} + \tp_{b})^{2} = 
\tilde{\nu} + \frac{1}{2} \left(m_{a}^{2} + m_{b}^{2} + m_{1}^{2} + m_{2}^{2} \right)\,. \quad
\label{3.4}
\end{eqnarray}
%
...
the following (see (3.27) of \cite{Lebiedowicz:2021byo}) where we neglect
gauge terms $\propto k_{\lambda}$:
%
\begin{eqnarray}
...
Of course, as starting point he considers again the diagrams
(a), (b), (c) of Fig.~\ref{fig:4} for ${\cal M}_{\lambda}$.
...
to $\hat{k}$, $\hat{p}_{1}'$ and $\hat{p}_{2}'$ in
the overall c.m. system, respectively.
...
always staying close to $(\bm{\hat{p}_{1}'}, \bm{\hat{p}_{2}'})$.
Referring always to the phase-space variables
$(k, \bm{\hat{p}_{1}'})$ we choose as starting point for
...
Therefore, in the schematic Fig.~\ref{fig:5} 
and Fig.~\ref{fig:8}
we move
...
The expansion of ${\cal M}_{\lambda}$ (\ref{6.4})
...
Note also that the expansion (\ref{4.12}) which is use here is,
...
+ \, \omega \int
\dv \Omega_{\hat{k}} \, \dv \Omega_{\hat{p}_{1}'}
{\cal A}^{(1,1)}(s, \bm{\hat{p}\ua}, \bm{\hat{p}\uu'}, \bm{\hat{k}})
+
{\cal O}(\omega^{2})\,.
\label{6.12}
\end{eqnarray}
...
Similarly to (\ref{6.7}) we define here
(see (\ref{6.2c})) with
%%
\begin{eqnarray}
t'' &=& (p_{a} - p_{1}'')^{2}\,,\nn\\
\bm{p_{1}''} &=& \bm{\hat{p}_{1}''} \,|\bm{p_{1}}|
= \bm{\hat{p}_{1}''} \sqrt{ \frac{s}{4} - m_{\pi}^{2}}\,,\nn\\
p_{1}''^{0} &=& \frac{\sqrt{s}}{2}\,,
\label{6.12a}
\end{eqnarray}
%
the following matrix elements using (\ref{4.13}):
%
\begin{eqnarray}
...
\quad p_{2}^{0} \to p_{1}''^{0}, \quad \bm{\hat{p}_{2}} \to -\bm{\hat{p}_{1}}'',  \nn \\
&&\bm{l_{1 \perp}} \to \bm{l_{1 \perp}}'' = \bm{k} (\bm{\hat{p}_{1}}'')^{2}
- (\bm{k} \cdot \bm{\hat{p}_{1}}'') \bm{\hat{p}_{1}}'' \,.
\label{6.15}
\end{eqnarray}
%
We get then $\bm{l_{1 \perp}}'' = 0$, as we require here.
...
&& \qquad  \times (-1) 
\left( \widehat{{\cal M}}_{\lambda}^{(0,2)} + \omega \widehat{{\cal M}}_{\lambda}^{(1,2)} \right)
\left( \widehat{{\cal M}}^{(0,2)\lambda}    + \omega \widehat{{\cal M}}^{(1,1)\lambda} \right)^{*} \nn\\
&& \qquad  +
{\cal O}(\omega^{2})\,.
\label{6.17}
\end{eqnarray}
...
and (\ref{6.13}), (\ref{6.14}), we find, indeed, 
...
and time reversal ($T$).
These give us restrictions for the propagators, vertices, and amplitudes. 
...
We work again in the c.m. system where we have
%
\begin{eqnarray}
...
And~this expansion is \textit{different} 
...
were finished we learned about Refs.~\cite{Liou:1977qv,Liou:1978sz,Liou:1982fm}.
There, as in our work, it is emphasized that one has to make
a consistent expansion of all terms
in the amplitude for $k \to 0$.
...
For $\pi^{0} p \to \pi^{0} p \gamma$ we are left with the diagrams (c), (d) and (e) 
...
\end{align}
%
using expressions similar to (\ref{26}) and (\ref{27})
replacing, of course, the $\pi^{\mp} p$
...
presented in \cite{Low:1958sn} and \cite{Lebiedowicz:2021byo} must be different,
...
radiative amplitude ${\cal M}_{\lambda}$
...
in $\omega$ for $\omega \to 0$ we get indeed the result expected from
(3.27), (3.28) of \cite{Lebiedowicz:2021byo} which we reproduce in 
...
where the expansion in $\omega$ agrees with our general results
...
to the orders $\omega^{-1}$ and $\omega^{0}$.
But these calculations certainly will not be easy.

\begin{acknowledgments}
We are very grateful to an anonymous referee of a first version
of this paper and a related paper of us for pointing out
...
\right], \qquad
\label{B3} \\
&& {\rm det} \left( \frac{\partial \hat{p}_{i}'}{\partial \hat{p}_{e}''} \right) = 1 + \frac{2 \omega}{|\bm{{p}_{1}}|} 
\bm{\hat{p}_{1}''} \cdot \bm{\hat{k}} +
{\cal O}(\omega^{2})\,.
\label{B4} 
\end{eqnarray}
%
Now we consider an arbitrary function $f(\bm{\hat{p}_{1}'})$
setting
%
\begin{align}
...
f(\bm{\hat{p}_{1}'}) \nn \\
&& = 
2 \int \dv^{3} \hat{p}_{1}' \, \delta(\hat{p}_{1}'^{2} - 1)
J^{(1)}(s, \omega, \bm{\hat{p}\uu'}, \bm{\hat{k}}) \,
f(\bm{\hat{p}_{1}'}) \nn \\
&&  =
2 \int \dv^{3} \hat{p}'' \, 
{\rm det} \left( \frac{\partial \hat{p}_{i}'}{\partial \hat{p}_{e}''} \right)
\delta(\hat{p}''^{2} - 1)
J^{(1)}(s, \omega, \bm{\hat{p}\uu'}, \bm{\hat{k}}) \,
\tilde{f}(\bm{\hat{p}_{1}''}) \nn \\
&&  =
2 \int \dv^{3} \hat{p}'' \, 
\delta(\hat{p}''^{2} - 1)
\left[ 1 + \frac{2 \omega}{|\bm{\hat{p}_{1}}|} \bm{\hat{p}_{1}''} \cdot \bm{\hat{k}} \right]
J^{(1)}(s, \omega, \bm{\hat{p}\uu'}, \bm{\hat{k}}) \nn \\
&& \quad  \times 
\tilde{f}(\bm{\hat{p}_{1}''}) +
{\cal O}(\omega^{2})\,.
...
\frac{2 m_{\pi}^{2}}{\sqrt{s(s-4 m_{\pi}^{2})}} + \bm{\hat{p}\uu'} \cdot \bm{\hat{k}}
\bigg{)} \nn \\
&& \qquad + \frac{1}{2} \sqrt{1 - \frac{4 m_{\pi}^{2}}{s}}
...
\end{eqnarray}
%
Since the function $f(\bm{\hat{p}_{1}'})$
was arbitrary we get the following transformation of the measures
%
\begin{eqnarray}
&&\dv \Omega_{\hat{p}_{1}'}
J^{(1)}(s, \omega, \bm{\hat{p}\uu'}, \bm{\hat{k}}) \nn\\
