...
Hersch \cite{He} showed that in the case of a sphere the only maximizers are round metrics. After that, Li-Yau \cite{LY} characterized the round metrics as the unique maximizers when $M = \mathbb{R}\mathrm{P}^2$. Nadirashvili \cite{N} proved that metrics maximizing $\lambda_1(g)|M|_g$ are induced by minimal immersions $M \to \mathbb{S}^n$, for some $n$, and this led to the proof that the flat equilateral metric is the unique maximizer on the case of the $2$-torus \cite{N,Gi,CKM}. Moreover, by the combined work of Matthiesen-Siffert \cite{MS} and Petrides \cite{Pe1}, for any closed surface $M$, there is a metric (possibly with conical singularities) achieving $\lambda_{1}^{\ast}(M)$, which is induced by a branched minimal immersion $M \to \mathbb{S}^n$. Besides the three cases mentioned above, the maximizers were also determined for the Klein bottle \cite{EGJ,JNP,CKM}, and the orientable closed surface of genus $2$ \cite{JLNP,NS}. Also, instead of \eqref{eq:Lap.funct}, we can consider $\sup_{g \in \mathfrak{C}} \lambda_1(g)|M|_g$, for a fixed conformal class $\mathcal{C}$, and it is known that maximizers exist and are induced by harmonic maps $(M,\mathfrak{C}) \to \mathbb{S}^{n}$ \cite{EI,Pe1}.
...
\end{equation}
where $|\pa\si|_{g}$ is the length of $(\pa\si, g)$ and $\sigma_{1}(g)$ denotes the first nontrivial Steklov eigenvalue, and they proved that $\sigma_{1}^{\ast}(\si) < \infty$ if $\si$ is orientable (see the work of Medvedev \cite{Me} for the non-orientable case). In \cite{F.S2}, they also showed that any metric achieving the supremum is induced by a free boundary conformal minimal immersion of $\si$ into a Euclidean unit ball $\mathrm{B}^{n}$. Moreover, by \cite{Pe3,MP}, $\sigma_{1}^{\ast}(\si)$ is always achieved by a metric (possibly with conical singularities) on $\si$, induced by a branched free boundary conformal minimal immersion $\si \to \mathrm{B}^{n}$. Also, Karpukhin-Stern  \cite{KS1,KS2} developed an alternative approach to the existence of maximizers of both \eqref{eq:Lap.funct} and \eqref{intro:Stek.funct}.


The maximizers of \eqref{intro:Stek.funct} are known in the case of the disk by the work of Weinstock \cite{We}, and in the cases of the annulus and the M\"obius band by \cite{F.S2}. Moreover, combining \cite[Proposition 8.1]{F.S2} with the results in \cite{KS2}, we know that in the case of orientable surfaces of genus zero, the maximizers are realized by embedded free boundary minimal surfaces in $\mathrm{B}^3$, which converge to $\mathbb{S}^2$ as the number of boundary components goes to infinity.


Inspired by the previous two problems, in this work we introduce an eigenvalue optimization problem which is related to free boundary minimal surfaces in \emph{spherical caps}, i.e., geodesic balls of round spheres. In order to state our results, let us first introduce some terminology.
...
\end{align*}
and let $\mathbb{B}^{n}(r) = \left\{x \in \mathbb{S}^{n};\, x_{0} \geq \cos r\right\}$ be its geodesic ball of center $\big(1,0,\ldots,0\big)$ and radius $0 < r < \frac{\pi}{2}$.  %Observe that
%$$\mathbb{B}^{n}(r) = \left\{x \in \mathbb{S}^{n};\, x_{0} \geq \cos r\right\}.$$
...
We denote by $\mathcal{M}(\si)$ the space of smooth Riemannian metrics $g$ on $\si$ such that $2$ is not an eigenvalue of $-\Delta_g$ with Dirichlet boundary condition (see Section \ref{sec:Steklov} for a justification on this restriction on the metrics). Then, for any $g \in \mathcal{M}(\si)$, we define
\begin{align*}
...
\end{equation*}
Moreover, if $\si$ is a disk, the equality holds if, and only if, $(\si,g)$ is isometric to $\mathbb{B}^{2}(r)$.
\end{thm1}
...
\begin{itemize}
\item[(i)] If $g\in \mathcal{M}(\si)$ satisfies $\Theta_{r}(\si,g) = \Theta_{r}^{\ast}(\si)$, then there exist a $\sigma_{0}(g,2)$-eigenfunction $v_0$ and independent $\sigma_{1}(g,2)$-eigenfunctions $v_1,\ldots,v_n$, which induce a free boundary minimal isometric immersion $v = (v_0,v_1,\ldots,v_n):(\si,g) \to \mathbb{B}^{n}(r)$.
\item[(ii)] If $g\in \mathcal{M}(\si)\cap \, \mathfrak{C}$ satisfies $\Theta_{r}(\si,g) = \Theta_{r}(\si,\mathfrak{C})$, then there are a $\sigma_{0}(g,2)$-eigenfunction $v_0$ and independent $\sigma_{1}(g,2)$-eigenfunctions $v_1,\ldots,v_n$, which induce a free boundary harmonic map $v = (v_0,v_1,\ldots,v_n):(\si,g) \to \mathbb{B}^{n}(r)$.

\end{itemize}
\end{thm2} 

 In \cite{dCD}, do Carmo and Dajczer studied the concept of rotational hypersurfaces in $\mathbb{S}^n$ and characterized in the case $n=3$ the ones having constant mean curvature. The minimal surface case was done by Otsuki \cite{Ot}. These surfaces were considered by Li and Xiong \cite{LX} in the context of free boundary minimal surfaces in spherical caps. We will use the setting of \cite{dCD}.

 In our next result, we characterize the immersions from Theorem B in the case of an annulus. 

\begin{thm3}\label{thm-intro3}
Let $\si$ be an annulus and consider a free boundary minimal immersion $\Phi = (\phi_0,\ldots,\phi_n):(\Sigma,g) \to \mathbb{B}^n(r)$. Suppose $\phi_i$ is a $\sigma_{1}(g)$-eigenfunction, for $i=1,\ldots,n$. Then $n=3$ and $\Phi(\Sigma)$ is one of the rotational examples given in \cite{dCD, Ot}.
\end{thm3}

Observe that we do not need to have as hypothesis that $\phi_0$ is a $\sigma_0(g)$-eigenfunction since this is always true for any free boundary minimal immersion into $\mathbb{B}^n(r)$ (see Remark \ref{rem-phi0}).
...
\noindent
{\bf Acknowledgments}: 
The first author thanks the University of Chicago and Princeton University for the hospitality, where part of the research in this paper was developed. The first author was supported by the Simons Investigator Grant of professor André Neves, and by the Brazilian National Council for Scientific and Technological Development (CNPq Grant 405468/2021-0).
...
\item Suppose $u \in H^{1}(\si,g)$ and $\Delta_{g}u = f$ in the weak sense. We say that $\displaystyle\frac{\partial u}{\partial\nu_{g}} \in L^{2}(\si,g)$ if there exists $\varphi \in L^{2}(\pa\si,g)$ such that
$$\int_{\si} g\big(\nabla^{g} u,\nabla^{g} v\big)\,dA_{g} - \int_{\si} fv\,dA_{g} = \int_{\pa\si} \varphi v\,dL_{g}, \quad \forall\, v \in H^{1}(\si,g).$$
\end{itemize}
Then, we define
\begin{align*}
\mathrm{dom}(\mathcal{D}_{\alpha}) &= \biggl\{u \in L^{2}(\partial\si,g);\, \exists\, \widehat{u} \in H^{1}(\si,g)\ \textrm{such that}\ \tr\,\widehat{u} = u,\, \Delta_{g} \widehat{u} + \alpha \widehat{u} = 0\\ &\quad\quad \textrm{in the weak sense, and}\ \frac{\partial u}{\partial\nu_{g}} \in L^{2}(\partial\si,g)\biggr\},\\
...
The eigenvalue $\sigma_{0}(g,\alpha)$ has a variational characterization given by
\begin{align}\label{var-sigma0}
\displaystyle\sigma_{0}(g,\alpha) = \inf\left\{\frac{\displaystyle\int_{\si} |\nabla^{g} \widehat{u}|^{2}_{g}\,dA_{g} - \alpha\int_{\si} \widehat{u}^{\,2}\,dA_{g}}{\displaystyle\int_{\partial\si} u^2\,dL_{g}};\,u \in \mathrm{dom}(\mathcal{D}_{\alpha})\setminus\{0\}\right\},
\end{align}
and it is simple. Denote by $\phi_0$ a first eigenfunction, which we can choose to be positive. Then, applying the min-max principle to $\mathcal{D}_{\alpha}$, it follows that
\begin{equation*}\label{var-sigma1}
\sigma_{1}(g,\alpha) = \inf \left\{\frac{\displaystyle\int_{\si} |\nabla^{g} \widehat{u}|^{2}_{g}\,dA_{g} - \alpha\int_{\si} \widehat{u}^{\,2}\,dA_{g}}{\displaystyle\int_{\partial\si} u^2\,dL_{g}};\, u \in \mathrm{dom}(\mathcal{D}_{\alpha})\setminus\{0\}\, \textrm{and} \int_{\partial\si} u \phi_{0}\,dL_{g} = 0\right\}.
\end{equation*} 

\begin{remark}
If $\alpha$ is an eigenvalue of $-\Delta_g$ with Dirichlet boundary condition, a priori $\mathcal{D}_{\alpha}$ is a multi-valued operator. However, we can still define it as single-valued operator by projecting in a certain subspace of $L^{2}(\pa\si,g)$ (for the details, see \cite{AM1}). Although we will not work with such eigenvalues from the next section on, we observe that the quotient
$$\frac{\displaystyle\int_{\si} |\nabla^{g} u|^{2}_{g}\,dA_{g} - \alpha\int_{\si} u^{2}\,dA_{g}}{\displaystyle\int_{\partial\si} u^2\,dL_{g}}$$
is unbounded on the class of functions $u \in H^{1}(\si,g)$ such that $u\vert_{\pa\si} \neq 0$, so in such cases we can define $\sigma_{0}(g,\alpha) = \pm\infty$ (depending on the sign of $\alpha$).
\end{remark}
...
Let $(\si,g)$ be a compact Riemannian manifold with boundary such that the following Dirichlet eigenvalue problem does not admit non-positive eigenvalues $\lambda$:
\begin{equation}\label{eq:eing.aux1}
\begin{split}
-\Delta_{g}u - \alpha u &= \lambda u,\ \textrm{in}\ \si,\\
u &= 0,\ \textrm{on}\ \partial\si.
\end{split}
\end{equation}
Then, for any $\sigma_{j}(g,\alpha)$-eigenfunction, its number of nodal domains $N_j$ in $\si$ satisfies
$$N_j \leq j + 1.$$
\end{theorem}
...
Let $(\si,g)$ be a compact Riemannian surface of genus $\gamma$ with
$\ell$ boundary components such that the Dirichlet eigenvalue problem \eqref{eq:eing.aux1} does not admit non-positive eigenvalues.
...
Consider a function $\phi \in C^{\infty}(\si)$ satysfying
...
\begin{equation*}
\Delta_{\delta}\,v + \langle b,\nabla_{\delta}\,v\rangle + c v = 0\ \textrm{on}\ U,\, \textrm{and}\ \ \frac{\partial v}{\partial y}(x,0) = 0,
\end{equation*}
...
Let $p \in \si\setminus\partial\si$ and consider isothermal coordinates $x,y$ around $p$ such that $p$ is mapped to $(0,0)$ and $g = \mu\delta$. Since $\Delta_{g}\,\phi_{j} + \alpha\phi_{j} = 0$ and $0 \leq l \leq q$, we have
...
\end{cases}
$$ 
has no solution for any $\lambda \leq 0$. In particular, for any function $u \in C^{\infty}(\si)$ that satisfies $u=0$ on $\partial \si$, it holds that
$$
...
\end{align*}
where in the last equality we use that $\phi_0\big\vert_{\partial\si}=\cos(r)$ and the fact that $\displaystyle\sum_{i=1}^3\langle v,\partial_i\rangle\phi_i$ is a $\sigma_j$-eigenfunction with $j \geq 1$ and, therefore, it is $L^2(\partial\si)$-orthogonal to $\phi = 1$ which is a
...
Let $\si$ be a compact surface with  boundary. We will denote by $\mathcal{M}(\si)$ the space of smooth Riemannian metrics $g$ on $\si$ such that $2$ is not an eigenvalue of $-\Delta_g$ with Dirichlet boundary condition. From now on, we will only be interested in metrics satisfying this property; hence, for simplicity, on the remainder of the paper we will denote $\sigma_{k}(g) = \sigma_{k}(g,2)$, $k \geq 0$.
...
We claim that, if $f$ is a $\sigma_1$-eigenfunction then the functions $a_0(t)$, $a_k(t)\cos(k\theta)$, $b_k(t)\sin(k\theta)$ are $\sigma_1$-eigenfunctions as well. In fact, we just need to observe that 
$$
...
Observe that $a_0$ and $a_1$ satisfy:
\begin{align}
...
On the other hand, $\sin t$ is a solution of \eqref{eq:aone}. Thus, a second solution linearly independent to $\sin t$ is of the form $z = h(t)\cos t$, where $h$ satisfies
...
\Theta_{r}(\si,g) \leq 4\pi(1 -\cos r)(\gamma + \ell).
\end{equation}
Moreover, if $\si$ is a disk, then the equality in \eqref{eq:bound.func} holds if, and only if, $(\si,g)$ is isometric to $\mathbb{B}^{2}(r)$.
\end{thm1}
...
Fix $r \in (0,\frac{\pi}{2})$. Recall that the area of $\mathbb{B}^{2}(r)$ with respect to the round metric is $2\pi(1-\cos r)$. Since $\mathbb{D}^2$ is conformally equivalent to $\mathbb{B}^{2}(r)$, there is also a proper conformal branched cover $u:(\Sigma,g) \to \mathbb{B}^{2}(r)$ of the same degree as $w$.
...
\end{equation}
Moreover, by using conformal diffeomorphisms of $\mathbb{B}^{2}(r)$, we can also assume (see \cite{LY})
$$\int_{\partial\Sigma}\ u_j \phi_0\,dL=0, \quad j=1,2,$$ 
where $\phi_0$ is a positive eigenfunction associated to $\sigma_{0}$.

Since $u: \si \to \mathbb{B}^{2}(r)$ is conformal, there is a positive function $f \in C^{\infty}(\si)$ such that, for all $X,Y \in \mathfrak{X}(\si)$, we have
$$g_{\mathbb{B}^{2}(r)}\big(u_{*}X,u_{*}Y) = f^2 g(X,Y).$$  
So, by abuse of notation, 
$$\nabla^{g}u_j = f^2\nabla^{\mathbb{B}^{2}(r)} x_j = f^2\big(\partial_j - x_{j} x\big),$$ 
which implies
...
First let us suppose $\Sigma=\mathbb{B}^{2}(r)$. Then, by Theorem \ref{thm-disk}, we have 
\begin{align*}
...
Now suppose the equality occurs in $(\ref{eq:bound.func})$ and $\gamma=0$ and $\ell=1$. Then, in particular, we have equality in (\ref{ineq1}) and (\ref{ineq2}), which implies that $u_0$ is an eigenfunction with eigenvalue $\sigma_{0}$, while $u_1$ and $u_2$ are eigenfunctions with eigenvalue $\sigma_{1}$. So, using that $\displaystyle\sum_{i=0}^{2}u_{i}^{2} = 1$, we conclude
\begin{align*}
...
Combining equations \eqref{eq:conformal.grad} and \eqref{eq:disk1}, it follows that $f\equiv 1$. Hence, using the fact that $\gamma=0$ and $\ell=1$ in $(\ref{eq:Hersch.Dir})$, we conclude that $\mathrm{deg}(u)=1$. Therefore, $(du)^{*}g_{\mathbb{B}^{2}(r)} = g.$
...
Let $\si$ be a compact orientable surface with  boundary and fix $r \in (0,\frac{\pi}{2})$. Recall we define
\begin{align*}
\Theta_{r}^{\ast}(\si) &= \sup_{g \in \mathcal{M}(\si)} \Theta_{r}(\si,g),
\end{align*}
where $\Theta_{r}(\si,g)$ was defined in \eqref{eq:funct_r}. We want to characterize the metrics realizing $\Theta_{r}^{\ast}(\si)$. Hence, we first need to study the regularity of $\Theta_{r}(\si,g)$ with respect to $g$.

For simplicity, given $u \in C^{\infty}(\pa\si)$, we will also denote by $u$ its extension to $\si$ satisfying $\Delta_g\,u + 2u = 0$.
...
Observe that by the definition of the eigenvalue, we have $f_1(t)\geq 0$ for any $t$, and $f_1(0)=0.$ So, if we assume that $\Theta_{r}(\si,g(t))$ is differentiable at $t=0$, then, in particular, $\sigma_1(t)$ is also differentiable at $t=0$ and $f_1$ has a critical point at $t=0.$ Thus,
\begin{align*}
...
Let $\si$ be a compact surface with boundary.
\begin{itemize}
\item[(i)] Suppose $g \in \mathcal{M}(\si)$ satisfies $\Theta_{r}(\si,g) = \Theta_{r}^{\ast}(\si)$. Then, there exist independent eigenfunctions $v_0 \in V_{0}(g)$ and $v_1,\ldots,v_n \in V_{1}(g)$ which induce a free boundary  minimal isometric immersion $v = (v_0,v_1,\ldots,v_n):(\si,g) \to \mathbb{B}^{n}(r)$.
\item[(ii)] Suppose $g \in \mathcal{M}(\si)\cap \mathfrak{C}$ satisfies $\Theta_{r}(\si,g) = \Theta_{r}(\si,\mathfrak{C})$. Then, there exist independent eigenfunctions $v_0 \in V_{0}(g)$ and $v_1,\ldots,v_n \in V_{1}(g)$ which induce a free boundary harmonic map $v = (v_0,v_1,\ldots,v_n):(\si,g) \to \mathbb{B}^{n}(r)$.
...
We claim that $(0,0) \in \mathcal{K}$. 

Suppose this is not true. Since $\mathcal{K}$ is a closed convex subset of $\mathcal{H}_{g}$, by the geometric form of the Hahn-Banach Theorem, there exists $(h,\psi) \in \mathcal{H}_{g}$ such that for any $u_{1} \in V_1(g)$ with $||u_{1}||_{L^2(\pa\si,g)}=1,$ we have
\begin{align*}
...
\end{equation*}
Therefore, $v(\si) \subset \mathbb{B}^{n}(r)$.  
...
(ii) Consider a smooth path $t \mapsto f(t)$ of positive functions such that $f(0) \equiv 1$ and $g(t) = f(t)g \in \mathcal{M}(\si)$ for all $t$. Denote $w = f^{\prime}(0)$ so that $\displaystyle\frac{dg}{dt}\Big\vert_{t=0} = wg$.

Following the previous notation and using the previous calculations, we have
\begin{align*}
...
&= -  2 \int_{\si}u_{k}^{2}\,w\, dA_{g} - \frac{1}{2}\,\sigma_{1}(g) \int_{\partial\si} u_{k}^{2}\,w\,dL_{g},
\end{align*}
hence,
\begin{align*}
\Theta_{r}'(0) = Q_w(u_1) &:= -2|\pa\si|_{g}\int_{\si} \big[(\cos^2 r)u_{0}^{2}+ (\sin^2 r)u_{1}^{2}\big]w\, dA_{g}\\ 
&\quad\  + \int_{\partial\si} F\big((\cos r)u_0,(\sin r)u_1\big)w\,dL_{g}.
\end{align*}
...
$$\sum_{j=0}^{n}v_{j}^{2} = 1,$$
and, hence, $v(\si) \subset \mathbb{S}^n$. Also, $v: (\si,[g]) \to \mathbb{S}^n$ is a harmonic map, since $\Delta_{g}\,v_j + 2v_j =0$, $j=0,1,\ldots,n$. Finally, using \eqref{eq:extremal4} and arguing as in the proof of (i), we conclude that $v(\si) \subset \ba$, $v(\pa\si) \subset \pa\ba$, and the map satisfies the free boundary condition.
\end{proof}
...
As observed in \cite{LX}, it is possible to prove that for any $0<r\leq\frac{\pi}{2}$, there exist $-\frac12<a\leq0$ and $s_0\in\mathbb R$ such that $\Phi_a: [-s_0,s_0]\times\mathbb S^1\to  \mathbb{B}^3(r)$ is a free boundary minimal annulus in  $\mathbb{B}^3(r)$.
...
In this section we will prove Theorem C stated in the introduction. %are the only free boundary minimal annuli that are immersed by the first and second eigenfunctions. 

Let $\si^2$ be a compact surface and suppose there exists a free boundary minimal immersion $\Phi: \Sigma \to \mathbb B^{n}(r)$. We can write $\Phi^{*}T\mathbb{S}^{n} = \Phi_{\ast}T\si\oplus N\si$, where $N\si$ is the normal bundle of $\Phi$. Thus, for $V \in \Gamma(\Phi^{\ast}T\mathbb{S}^{n})$ we can write 
$$V = V^{\top} + V^{\perp},$$
where $V^{\top}_{p} \in \Phi_{\ast}T_p\si$ and $V^{\perp}_{p} \in (N\si)_{p}$, $\forall\, p \in \si$.
...
Then, there is $T>0$ susch that $(\Sigma,g)$ is conformal to $[-T,T]\times\mathbb S^1$ endowed with the cylindrical metric $g_{\textrm{cyl}}=ds^2+g_{\mathbb{S}^{1}}$, i.e., $g=\lambda g_{\textrm{cyl}}$ for some positive function $\lambda$. Let $\partial_\theta$ be the (globally defined) vector field associated to any angular coordinate. Observe that $\partial_{\theta}$ spans $T_{p}\partial\si$, $\forall\,p \in \partial\si$, and the outward pointing unit $g$-normal of $\partial\si$ is $\pm\lambda^{-1/2}\partial_s$.
\begin{lemma} \label{kernel}
Let $\Phi = (\phi_0,\ldots,\phi_n):(\Sigma,g) \to \mathbb{B}^n(r)$ be a free boundary conformal minimal immersion, where $\si$ is an annulus. Then, for any $V \in \Gamma(\Phi^{\ast}T\mathbb{S}^{n})$ such that $V_p \in T_p\pa\ba$, $\forall\,p\in\pa\si$, we have
$$
Q(V,\Phi_{\theta})=0.
$$
\end{lemma}
...
\begin{proof}
First observe that $\langle V, \Phi\rangle =0$.   Since $\Delta_g =\lambda^{-1}\Delta_{\textrm{cyl}},$ using Proposition \ref{prop-eigen} we have 
\begin{align*}
...
Now let us specialize to the case $n=3$, and suppose $\si$ is is any compact orientable surface. We know the bundle $N\si$ is trivial, and there is a globally defined unit normal vector $N$ in $\si$.
\begin{lemma}
\label{lemma:perp2}
...
Suppose that $\Phi: (\Sigma,g) \to \mathbb B^3(r)$ is a free boundary minimal immersion of an annulus. Then, there is a subspace $\mathcal C_1$ of $\mathcal{C}$ of dimension at least three such that for all $\psi\in\mathcal C_1$ there is $Y^{\top} \in \Gamma(T\si)$ such that $Y^{\top}$ is tangential to $\partial \mathbb B^3(r)$ along $\partial \Sigma$ and $Y(\psi)=Y^{\top}+\psi N$ is conformal. Moreover, for any two such vector fields $Y_{1}^{\top}$ and $Y_{2}^{\top}$, there exist a constant $c \in \R$ such that
...
By the result of do Carmo and Dajczer \cite{dCD} (see also \cite{LX}), $\si$ is one of the rotational examples given by \eqref{eq-annulus}.
...
\bibitem{HS} A. Hassannezhad and D. Sher, {\it Nodal count for Dirichlet-to-Neumann operators with potential}, arXiv:2107.03370v3.
...
\bibitem{KS1} M. Karpukhin and D. L. Stern, {\it Min-max harmonic maps and a new characterization of conformal eigenvalues}, Preprint arXiv:2004.04086.

\bibitem{KS2} M. Karpukhin and D. L. Stern, {\it  From Steklov to Laplace: free boundary minimal surfaces with many boundary components}, Preprint arXiv:2109.11029.
...
\bibitem{Lo} E. Longa, {\it Extremal metrics for the first eigenvalues of the Laplacian on manifolds with boundary}, Preprint arXiv:2305.05897.
...
\bibitem{MoRo} S. Montiel and A. Ros, {\it inimal immersions of surfaces by the first eigenfunctions and conformal area}, Invent. Math. {\bf 83} (1985), no. 1, 153--166.
...
\bibitem{Pe4} R. Petrides, {\it Non planar free boundary minimal disks into ellipsoids}, Preprint arXiv:2304.12111.

\bibitem{Pe5} R. Petrides, {\it Laplace eigenvalues and non-planar minimal spheres into 3-dimensional ellipsoids}, Preprint arXiv:2304.12119. 
