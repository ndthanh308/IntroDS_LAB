...
Then, the matroids in $\mathcal{N}_i$ are \emph{strong stabilizers} for the class of $\mathbb{H}_{i}$-representable matroids \cite{vanZwam2009}; loosely speaking, this means that when considering a matroid $M$ in this class with some matroid~$N$ in $\mathcal{N}_i$ as a minor, then after fixing a representation of $N$, one need not worry about inequivalent representations of $M$.
...
As a slightly more subtle point, note that when an excluded minor for the class of $\mathbb{H}_{i}$-representable matroids is $\GF(5)$-representable, for some $i \in \{2,3,4,5\}$, it need not be $\mathbb{H}_{i-1}$ representable.
...
For \cref{thm2}, the more advanced techniques employed in \cite{BP20} become useful; in particular, \emph{splicing} is used to dramatically speed up the computations when considering matroids of size at least 11.
In order to use splicing, while only keeping track of $3$-connected matroids with a certain $N$-minor, we require a splitter theorem for $N$-detachable pairs~\cite{bww3}.
...
Let $\mathbb{P}=(R, G)$ be a partial field, and let $A$ be an $X\times Y$ matrix with entries from $\mathbb{P}$. Then $A$ is a $\mathbb{P}$-\textit{matrix} if every non-zero subdeterminant of $A$ is contained in $G$. If $X'\subseteq X$ and $Y'\subseteq Y$, then we write $A[X',Y']$ to denote the submatrix of $A$ with rows labelled by $X'$ and columns labelled by $Y'$.
...
%\begin{definition}
  Let $M$ be a matroid.  We say that a partial field is the \emph{universal partial field of $M$}, and denote it $\mathbb{P}_M$, if for every partial field $\mathbb{P}$, the matroid $M$ is $\mathbb{P}$-representable if and only if there exists a %partial-field
  homomorphism $\phi : \mathbb{P}_M \rightarrow \mathbb{P}$.
%\end{definition}
  Pendavingh and Van Zwam described %the construction of a unique
  ...
  %\todo{maybe.  Might want a formal definition.}
...
If $A'$ can be obtained from $A$ by scaling, pivoting, and permuting rows and columns (permuting labels at the same time), then we say that $A$ and $A'$ are \emph{projectively equivalent}.
...
%
Let $F \subseteq \mathfrak{F}(\mathbb{P})$, and let $A$ be a $\mathbb{P}$-matrix. %, and let $M$ be the matroid $M[A]$.
 We say that the matrix $A$ is {\em $F$-confined} if $\Cr(A) \subseteq F \cup \{0,1\}$.
%
...
for any $F_\phi$-confined $\mathbb{F}$-matrix, there exists a $\mathbb{P}$-matrix~$A'$ such that $\phi(A')$ is scaling-equivalent to $A$.
...
\begin{lemma}[{\cite[Corollary~2.8]{BP20}}]
  ...
  Then, for $\{M',N'\} \in \{(M,N),(M^*,N^*)\}$, there exists an element $e \in E(M')$ such that $M' \ba e$ is $3$-connected and has an $N'$-minor.
\end{lemma}
...
To find the excluded minors for $\mathbb{H}_5$-representability on up to $13$ elements, we use ``splicing'', as described in \cite[Section~4.5]{BP20}.
Briefly, when $M'$ is a matroid, $M_e$ is an extension of $M'$ by an element $e$, and $M_f$ is an extension of $M'$ by an element $f$, then we say that $M$ is a \emph{splice} of $M_e$ and $M_f$ if $M \ba f = M_e$ and $M \ba e = M_f$.
...
That is, for some pair $\{a,b\} \subseteq E(M)$, the matroid $M \ba a,b$ is $3$-connected and has an $N$-minor;
...
%
\begin{definition}
  Let $P$ be an exactly $3$-separating set in a matroid $M$, with $|P| \ge 6$.
...
We introduce notation to refer to the latter sets: for $J \subseteq \{1,2,3\}$, let $X_J$ be the $6$-element set such that $\{p_i,t_i\} \subseteq X_J$ for each $i \in J$, and $\{q_i,s_i\} \subseteq X_J$ for each $i \in \{1,2,3\}-J$.
...
\begin{proof}
  ...
  Now we assume that $\mathbf{X}$ is an stable set in $Q$.
  We note that $M$ is one of four \quadflower s, up to isomorphism (recalling that $M$ is not a near-regular or free \quadflower).
  ...
  If $M'$ has an $\mathbb{F}$-representation $A'$ for some field $\mathbb{F}$, then it is easy to verify that $A'$ is projectively equivalent to the matrix
  $$\kbordermatrix{
...
q_3 &     1 &     1 &     0 &     0 &      0 & -\alpha},$$
  ...
  (Alternatively, the above matrix is a $\mathbb{K}_2$-representation, and there is a homomorphism from $\mathbb{K}_2$ to any field with size at least four.)
...
%
The following thereom is the recently obtained excluded-minor characterisation of $2$-regular matroids \cite{BOSW2023b,BP20}.
...
For $i \in \{1,2,3,4,5\}$, let $\mathcal{X}_i$ be the excluded minors for the class of $\mathbb{H}_i$-representable matroids.
For $i \in \{1,2,3,4\}$, let $\mathcal{N}_i$ be the matroids in $\mathcal{X}_{i+1}$ that are $\mathbb{H}_i$-representable.
%
...
The matroid $\pappus \ba e$ has a pair of free bases; tightening either one of these results in a matroid that we denote $(\pappus \ba e)^+$ (the two possible matroids are isomorphic).
...
%\section{Excluded minors for \texorpdfstring{$\GF(5)$}{GF(5)}-representable matroids}
%\label{hendsec}

%%\begin{lemma}
  %%\label{gf5minors}
  %%Let $M$ be an excluded minor for the class of $\mathbb{P}$-representable matroids, for $\mathbb{P} \in \{\GF(5), \mathbb{D}\}$.
  %%Then, either
  %%\begin{enumerate}
    %%\item $M$ has a $\{U_{2,5}, U_{3,5}, F_7^-, (F_7^-)^*,P_8\}$-minor, or
    %%\item $M$ is isomorphic to one of $F_7$, $F_7^*$, $\AG(2,3)\ba e$, $(\AG(2,3)\ba e)^*$, $(\AG(2,3)\ba e)^{\Delta Y}$.
  %%\end{enumerate}
%%\end{lemma}
%%\begin{proof}
  %%Suppose that $M$ has no $\{U_{2,5},U_{3,5},F_7^-,(F_7^-)^*,P_8\}$-minor.
  %%As $M$ is not near-regular, \cref{nr_exminors} implies that $M$ has a minor isomorphic to one of $F_7$, $F_7^*$, $\AG(2,3)\ba e$, $(\AG(2,3)\ba e)^*$, $(\AG(2,3)\ba e)^{\Delta Y}$.
  %%But each of these five matroids is not $\GF(5)$-representable, so $M$ does not contain one of these matroids as a proper minor; hence (ii) holds.
%%\end{proof}

%\begin{lemma}
  %\label{gf5minors}
  %Let $M$ be an excluded minor for the class of $\GF(5)$-representable matroids.
  %%such that $|E(M)| \le 12$.
  %Then, either
  %\begin{enumerate}
    %%\item $M$ has an $\{U_{2,6}, U_{4,6}, P_6, F_7^-, (F_7^-)^*, P_8,P_8^=\}$-minor, or
    %\item $M$ has a $\{U_{2,6}, U_{4,6}, P_6, P_8^=\}$-minor, or
    %\item $M$ has both a $\{U_{2,5}, U_{3,5}\}$-minor %, F_7, F_7^*\}$-minor
      %and a $\{F_7^-, (F_7^-)^*, P_8\}$-minor, or
    %\item $M$ is an excluded minor for 
      %$\mathbb{D}$- or
      %$\mathbb{G}$-representable matroids.
  %\end{enumerate}
  %%Moreover, if $M$ has a proper $\{F_7^-, (F_7^-)^*, P_8\}$-minor, then $M$ has a $\{U_{2,5},U_{3,5}\}$-minor.
%\end{lemma}
%\begin{proof}
  %Let $M$ be an excluded minor for quinary matroids.
  %%If $M$ is ternary or quaternary, then (ii) holds; so assume otherwise.
  %As $M$ is not, in particular, golden mean, it has as a minor some excluded minor $N$ for golden-mean matroids.
  %We may assume $N$ is a proper minor of $M$, otherwise (iii) holds.
  %Note that $N$ is quinary, since $M$ is an excluded minor for quinary matroids.
  %It follows that $N$ is an excluded minor for quaternary matroids,
  %so $N$ is in $\{U_{2,6}, U_{4,6}, P_6, F_7^-, (F_7^-)^*, P_8,P_8^=\}$. %, as required.

  %We may now assume that $M$ has a $\{F_7^-, (F_7^-)^*, P_8\}$-minor; it remains to show that $M$ has a $\{U_{2,5}, U_{3,5}\}$-minor.
  %Using a similar argument as in the previous paragraph, $M$ properly contains an excluded minor $N'$ for dyadic matroids, where $N'$ is quinary.
  %It follows that $N'$ is an excluded minor for ternary matroids, so $N' \in \{U_{2,5},U_{3,5},F_7,F_7^*\}$.  But neither $F_7$ nor $F_7^*$ is quinary, so the result follows.
  %%As $|E(N')| \le 14$, we have $N' \in \{U_{2,5},U_{3,5}\}$.
%\end{proof}
