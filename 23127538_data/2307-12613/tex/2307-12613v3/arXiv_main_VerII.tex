...
\subsection{Effective rank-aware error bounds}

\newS{After the first version of this work appeared, it was observed in \cite{chen2023parameter} that improved operator norm estimation error bounds can be obtained by using a different dithering scale for each entry of the samples. Although this was developed for a different quantizer (a two-bit quantizer with triangular dithering), an analogous approach can be pursued in our setting. The idea is to consider quantized samples of the form
\begin{align*} %\label{eq:TwoBitSamplesAdaptiveEntrywise}
	\sign(\X_k + \LAMBDA_{\max} \Tau_k) \quad \text{ and } \quad \sign(\X_k + \LAMBDA_{\max} \bar{\Tau}_k),
\end{align*}
for $1 \le k \le n$, where $\Tau_k,\bar{\Tau}_k$ are independent and uniformly distributed in $[-1,1]^p$ and $\LAMBDA_{\max}$ is the diagonal matrix with diagonal entries
$$(\LAMBDA_{\max})_{ii} = \max_{1\leq k\leq n}|(\X_k)_i|.$$
Define the estimator
\begin{align*} %\label{eq:TwoBitEstimatorAdaptiveEntrywise}
	\SIGMAMAX_n = \frac{1}{2}(\SIGMAMAX)'_n + \frac{1}{2}((\SIGMAMAX)'_n)^T,
\end{align*}
where
\begin{align*} %\label{eq:AsymmetricEstimatorAdaptiveEntrywise}
	(\SIGMAMAX)'_n = \frac{1}{n} \sum_{k=1}^n  \big( \LAMBDA_{\max} \sign(\X_k + \LAMBDA_{\max}\Tau_k) \big) \big( \LAMBDA_{\max} \sign(\X_k + \LAMBDA_{\max} \bar{\Tau}_k) \big)^T.
\end{align*}
Similarly to \cite{chen2023parameter}, one can show that with high probability, 
$$\|\SIGMAMAX_n - \SIGMA\|\lesssim_K \text{polylog}(p)\| \SIGMA \| \left( \sqrt{\frac{\operatorname{r}(\SIGMA)}{n}} +\frac{\operatorname{r}(\SIGMA)}{n} \right),
$$
where $\operatorname{r}(\SIGMA)=\tr(\SIGMA)/\|\SIGMA\|$ is the \emph{effective rank} of $\SIGMA$. This bound is qualitatively better than the ones in Theorems~\ref{thm:OperatorDitheredMask_Old} and \ref{thm:OperatorDitheredMask}, as clearly $\tr(\SIGMA)\leq p\|\SIGMA\|_{\infty}$. Although this approach looks appealing, there is an important practical downside. Since $\Lambda_{\max}$ depends on all collected samples, \emph{all samples $\X_1,\ldots,\X_n$ must be observed and stored before they can be quantized}. In a typical signal processing scenario, the samples are collected over time and need to be quantized as they arrive at a sensor, prohibiting the aforementioned approach. Even if one is able to obtain all samples before quantization, then it seems better to directly construct the sample covariance matrix (which satisfies the same estimation error bound \cite{koltchinskii2017}).}\par 
\newS{To mend this, we show in this section that improved error estimates can still be obtained by using \emph{adaptive} dithering levels which are determined only by previously observed samples.} 
\newJ{
...
Specifically, we consider now quantized samples of the form
\begin{align} \label{eq:TwoBitSamplesAdaptiveEntrywise}
...
%\newJ{
%Based on the samples in \eqref{eq:TwoBitSamplesAdaptiveEntrywise}, we define an according covariance estimator by
\newS{Now consider the estimator}\newJ{
\begin{align} \label{eq:TwoBitEstimatorAdaptiveEntrywise}
...
\end{align}
%
\newS{By slightly adapting the proof of Theorem \ref{thm:OperatorDitheredMask}, we can derive the following effective rank-aware bounds for our data-adaptive estimator. For simplicity, we state a version without an additional mask $\M$}. 
...
\end{theorem}
%
We provide the proof in Appendix~\ref{sec:ProofAppendix}. To illustrate Theorem \ref{thm:OperatorDitheredMaskDecoupled} numerically as well, we compare the performance of $\SIGMAADAP_n$ and $\hat{\SIGMA}^{\operatorname{adap}}_n$ in the setting of Figure \ref{fig:Comparison} for two different covariance matrices. The first covariance matrix $\SIGMA_1$ is the one from Figure \ref{fig:Comparison}, i.e., it has ones on the diagonal and all remaining entries equal to $0.2$; clearly, $\mathrm{tr}(\SIGMA_1) = p \| \SIGMA_1 \|_\infty$ such that Theorems \ref{thm:OperatorDitheredMask} and \ref{thm:OperatorDitheredMaskDecoupled} yield similar bounds. This is confirmed by the simulation. The second covariance matrix $\SIGMA_2 = \mathbf D \SIGMA_1 \mathbf D$, where $\mathbf D \in \R^{p\times p}$ is a diagonal matrix with $D_{11} = 1$ and $D_{ii} = 1/10$, for $i > 1$; in this case, all variances but the first are scaled by $\tfrac{1}{100}$ such that $\mathrm{tr}(\SIGMA_2) \ll p \| \SIGMA_2 \|_\infty$ and Theorem \ref{thm:OperatorDitheredMaskDecoupled} yields way better approximation rates for $\hat{\SIGMA}^{\operatorname{adap}}_n$ than Theorem \ref{thm:OperatorDitheredMask} for $\SIGMAADAP_n$. Figure \ref{fig:ComparisonDecoupled} illustrates that in the second case the error of $\SIGMAADAP_n$ grows in $p$ whereas the error of $\hat{\SIGMA}^{\operatorname{adap}}_n$ stays constant and aligns with the behavior of the sample covariance matrix $\hat\SIGMA_n$ which requires the unquantized samples.
}
%
% Figure environment removed
