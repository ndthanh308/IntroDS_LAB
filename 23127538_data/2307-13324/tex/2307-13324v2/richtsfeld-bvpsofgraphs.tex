...
\textbf{Acknoledgements}
I would like to thank Christian Bär, Lashi Bandara and Klaus Ecker for their supervision and for making this project possible. I would also like to thank the anonymous referees for their helpful comments.
...
where $x\sim y$ iff $\mathrm{d}(x)=\mathrm{d}(y)$ with 
\[\mathrm{d} : \bigsqcup_{e\in E}\{0_e,l_e\}\rightarrow V\qquad 0_e\mapsto \partial_-e, \quad l_e\mapsto \partial_+ e.\]
We denote the coordinate on the edge $e$ by $x_e$. For our purposes, we will always assume that our metric graph is finite and has no isolated vertices.
...
Standard Sobolev estimates tell us that each function in $H^1(G,\mathbb{C}^r)$ is continuous when restricted to a single edge and that there is a well-defined bounded operator 
\[\tr : H^1(G,\mathbb{C}^r)\rightarrow \mathbb{C}^{r,\bar{E}},\qquad f\mapsto \sum_{e\in E}f(0_e)\otimes e_-+f(l_e)\otimes e_+.\]
...
\begin{defin}
	A boundary condition $B$ is \textbf{local} iff there are linear subspaces $B_v\subseteq \mathbb{C}^{r,\bar{E}_v}$, such that 
	\[B=\bigoplus_{v\in V}B_v.\]
\end{defin}

\begin{rmk}
	This notion of locality is different from the notion of locality introduced by Bär and Ballmann \cite[Definition 7.9]{bb}. In fact, every boundary condition on the manifold $\bigsqcup_{e\in E}[0,l_e]$ is local in the sense of Bär and Ballmann, and non-locality is a phenomenon that occurs only in higher dimensions. Locality in our sense means that the boundary condition sees the structure of the graph and imposes a condition per vertex such that the conditions at two different vertices are independent of each other. 
\end{rmk}
...
\end{align*}
as well as the boundary maps $\partial_\pm : E\rightarrow V$, mapping $e \rightarrow \partial_\pm e$. The subsets $E_v^\pm := \partial_\pm^{-1}(\{v\})\subseteq E$ induce subspaces 
$$\mathbb{C}^{r,E_v^\pm}\subseteq \mathbb{C}^{r,E}.$$ 
Note that there are canonical inclusions \(\iota_\pm:\mathbb{C}^{r,E}\rightarrow \mathbb{C}^{r,\bar{E}}\) sending \(e\) to \(e_\pm\). For $v\in V$, the subspace \(\mathbb{C}^{r,\bar{E}_v}\) is equal to the direct sum \(\iota_+(\mathbb{C}^{r,E_v^+})\oplus \iota_-(\mathbb{C}^{r,E_v^-})\). 
 ...
 \begin{exa}
 	...
 	\begin{align*}
 		\mathbb{C}^{E}&= \mathbb{C}\cdot e\\
 		\mathbb{C}^{\bar{E}} &= \mathbb{C}\cdot e_-\oplus \mathbb{C}\cdot e_+\\
 		C^\infty(G,\mathbb{C}) &= C^\infty([0,l],\mathbb{C}).
 	\end{align*}
 	For $f\in C^\infty(G,\mathbb{C})$,
 	\begin{equation*}
 		\mathrm{tr}(f)= f(0)\cdot e_- + f(l)\cdot e_+
 	\end{equation*}
 	holds. 
 	
 	There are two cases: Either we have $\partial_-e \neq \partial_+ e$, or $\partial_-e= \partial_+ e$.\\
 	In the first case, we have
 	\begin{align*}
 		\mathbb{C}^{\bar{E}_{\partial_- e}}&= \mathbb{C}\cdot e_- & \mathbb{C}^{\bar{E}_{\partial_+ e}}&= \mathbb{C}\cdot e_+\\
 	...
 	\begin{align*}
 		\mathbb{C}^{\bar{E}_v}&=\mathbb{C}^{\bar{E}} &
...
Given  two endomorphism fields $\sigma, \mathbb{V}\in C^\infty(G,\mathbb{C}^{r\times r})$ we can define a first order differential operator 
\[D: C^\infty(G,\mathbb{C}^r)\rightarrow C^\infty (G,\mathbb{C}^r), \qquad f\mapsto \sigma f' +\mathbb{V}f.\]
...
\begin{lem}
	...
	\[\sigma_0:\mathbb{C}^{r,\bar{E}}\rightarrow \mathbb{C}^{r,\bar{E}}, \qquad \sum_{e\in E} a_e\otimes e_- + b_e\otimes e_+\mapsto \sum_{e\in E} -\sigma(0_e)a_e\otimes e_- + \sigma(l_e)b_e \otimes e_+ .\]
\end{lem}

We directly obtain 
\begin{prop}\label{prop:sa}
	Let $B$ be a boundary condition, define the adjoint boundary condition by
	\[B^{\mathrm{ad}}:=\{y\in \mathbb{C}^{r,\bar{E}}\mid \forall b\in B: \langle \sigma_0 b, y\rangle =0\}=(\sigma_0 B)^\perp.\]
...
\section{The index of $D_B$}\label{section:index}
...
\begin{lem}\label{cor:fred}
	Let $B$ be a subspace of $\mathbb{C}^{r,\bar{E}}$, $B^\perp$ be the orthogonal complement to $B$ and let $P:\mathbb{C}^{r,\bar{E}}\rightarrow\mathbb{C}^{r,\bar{E}}$ be the orthogonal projection onto $B^\perp$. Then, 
	\begin{align*}
...
\begin{proof}
	...
	H^1(G,\mathbb{C}^r)  \ar[ru]^{(D,P_2)} \ar[rd]^{(D,P_1)} \\
...
Let \(A: \mathbb{C}^E\rightarrow \mathbb{C}^E\) be a linear map, we can then define an associated boundary condition 
\[\mbox{\boldmath$\Gamma(A)$}:=\left\{\iota_+(x)+\iota_-(Ax)\mid x\in\mathbb{C}^E\right\}\subseteq \mathbb{C}^{\bar{E}}\]
which we call the \textbf{graph of }{\boldmath\(A\)}.  We say that \(A\) is a {\boldmath \(G\)}\textbf{-endomorphism}, if \(\Gamma(A)\) is a local boundary condition. In this case \(A\) restricts to a homomorphism $$A_v :\mathbb{C}^{E_v^+}\rightarrow \mathbb{C}^{E_v^-}$$ for all \(v\in V\).

\begin{thm}\label{thm:unit}
	Let \(B\) be a boundary condition on \(G\). Then, \(D_B\) is self-adjoint iff there exists a unitary map \(A:\mathbb{C}^E\rightarrow\mathbb{C}^E\), such that \(B=\Gamma(A)\).
\end{thm}
\begin{proof}
	Let $B$ be such that $D_B$ is self-adjoint. Since the index of a self-adjoint operator is zero, by Theorem \ref{thm:ind}, the dimension of $B$ is equal to the number of edges, denoted by $n$. Choosing a basis $b_1,...,b_n$ of $B$, let 
	$$b_i^\pm=\pi_\pm(b_i)\in \mathbb{C}^E$$
	for $i=1,...,n$, where \(\pi_\pm: \mathbb{C}^{\bar{E}}\rightarrow \mathbb{C}^E\) is the map sending \(e_\pm\) to \(e\) and \(e_\mp\) to \(0\). Now, assume that there exist $\lambda_1,...,\lambda_n$, such that
	...
	Considering that the dimension of $\mathbb{C}^E$ is equal to $n$, it follows that $b^+_1,...,b^+_n$ forms a basis of $\mathbb{C}^E$. W.l.o.g., $b_1,...,b_n$ can be chosen such that $b^+_1,...,b^+_n$ is an orthonormal basis of $\mathbb{C}^E$. Define $A$ as the linear map sending $b^+_i$ to $b^-_i$ for $i\in\{1,...,n\}$. Since $B=B^{\ad}$, we have 
	...
	Conversely, it is easily checked that for a given unitary map $A:\mathbb{C}^E\rightarrow \mathbb{C}^E$, the graph $\Gamma(A)$ is a boundary condition such that $\Gamma(A)^{\ad}=\Gamma(A)$.
\end{proof}
...
\begin{align*}
	x_e : \mathbb{C}^E\rightarrow \mathbb{C},\quad \quad x_e\left(\sum_{e^\prime\in E}a_{e^\prime}e^\prime \right)= a_e.
\end{align*}

\begin{defin}
	For a \(G\)-endomorphism $A$ define \textbf{the multi-dimensional characteristic polynomial} {\boldmath$P_A$} of $A$ to be the polynomial on $\mathbb{C}^E$ given by
	$$P_A(x)=\det\left(\diag(x)-A\right),$$
	where \(diag(x)\) is the linear map on \(\mathbb{C}^E\) given by \(\sum_{e\in E} x_e e\otimes e^\ast\).
	The characteristic function {$\mbox{\boldmath $P_{A,l}$}:\mathbb{C}\rightarrow\mathbb{C}$} with respect to the length function $l$ is given by 
	$$P_{A,l}(\lambda)=P_A(\exp(i\lambda l)),$$
...
\begin{rmk}
	Similar results exist for the Laplace operator on metric graphs, see for example Theorem 2.1.8 in \cite{BerKuch}, Theorem 19 in \cite{Kuchment2014} and the main theorems in \cite{VONBELOW}.
\end{rmk}


\begin{proof}
	By Theorem \ref{thm:spec}, $\lambda$ is an eigenvalue of $D_{\Gamma(A)}$ if and only if $e^{i\lambda l}\in\mathbb{C}$ is an eigenvalue of $A$. This proves the corollary.
\end{proof}
...
Consider the following setup: Let $G$ be a metric directed multigraph and assume that there exists a vertex $v\in V$ with in- and out-degree being one. Furthermore, assume that the ingoing edge at $v$ is different from the outgoing edge. Enumerate the edges of $G$ by $e_1,e_2,...,e_n$ such that $e_1$ is the ingoing and $e_2$ is the outgoing edge at $v$, defining a basis of $\mathbb{C}^E$. Let $\tilde{G}=(\tilde{V},\tilde{E})$ be the graph obtained out of $G$ by replacing $e_1$ and $e_2$ with a single edge $e$ running from the tail of $e_1$ to the head of $e_2$. We define a length function $\tilde{l}: \tilde{E}\rightarrow (0,\infty)$ by 
\begin{align*}
	\tilde{l}_e&=l_{e_1}+l_{e_2},\\
	\tilde{l}_{e_i}&= l_{e_i} \quad \forall i>2.
\end{align*}
% Figure environment removed

\begin{lem}\label{lem:reduction}
	Consider the above setup and let $A$ be a $G$-endomorphism. The transformation matrix of $A$ with respect to the basis $e_1,...,e_n$ of $\mathbb{C}^E$ is then of the form
	\[
	A=\scalebox{0.9}{$
		...
		\end{array}\right)$}.
	\]
	Let $\tilde{A}$ be the endomorphism on $\mathbb{C}^{\tilde{E}}$ that with respect to basis $e,e_3,...,e_n$ has the transformation matrix
	\[
	\tilde{A}=
	\left(\begin{array}{@{}c c@{}}
		\alpha\alpha_{12}
...
\begin{proof}
	...
	\begin{align*}
		P_A(x_1,x_2,...,x_n)&=\det \scalebox{0.8}{$
			\left(\begin{array}{@{}c c@{}}
				\begin{matrix}
					x_1 & -\alpha_{12} \\
					-\alpha & x_2
...
\begin{defin}
	A \(G\)-endomorphism $A$ is said to be \textbf{reducible} with respect to a proper subset $\Delta$ of $E(G)$ if \(A\) maps $\mathbb{C}^{\Delta}:=\{\sum_{e\in \Delta}c_e \cdot e \mid c_e\in \mathbb{C}\}$ into itself. If no such $\Delta$ exists, $A$ is said to be \textbf{irreducible}.
\end{defin}
...
$$\pi_{\Delta}\circ A $$
to $\mathbb{C}^{\Delta}$, where $\pi_{\Delta}:\mathbb{C}^{E}\rightarrow \mathbb{C}^{\Delta}$ is the canonical projection map.

\begin{thm}\label{prop:decomp}
...
\begin{exa}
	...
	\begin{equation*}
		E_{ij}= \{e\in E\mid \partial_- e\in V_i, \partial_+ e\in V_j\}.
	\end{equation*}
	By the assumptions on $G$ we have $E_{12}=\emptyset$ and a partition
	\begin{equation*}
	...
	\begin{align*}
		\sigma(D_{\Gamma(A)})&= \sigma(D^1_{\Gamma(A_1)})\cup  \underbrace{\sigma(D^{21}_{\Gamma(A_{21})})}_{=\emptyset} \cup  \sigma(D^{22}_{\Gamma(A_{22})})\\
		&= \sigma(D^1_{\Gamma(A_1)}) \cup \sigma(D^{22}_{\Gamma(A_{22})}),
	\end{align*}
...
\begin{defin}
	Let $G$ be a directed multigraph. We call a bijective map $P:E(G)\rightarrow E(G)$ a \textbf{permutation of the edges} of $G$, the set of permutations of its edges are denoted by $\mathfrak{S}(E(G))$. If $P$ satisfies that $P(e)=e^\prime$ implies $\partial_+e= \partial_- e^\prime$, then $P$ is called a {\boldmath$G$}\textbf{-permutation}. We denote the set of all \(G\)-permutations by $\mathfrak{S}(G)$. 
\end{defin}
...
\begin{thm}\label{thm:perm}
	Let $G$ be a Eulerian directed multigraph. Every decomposition of the directed multigraph into directed closed trails corresponds to a $G$-permutation $P$ and vice versa. Each trail of length $k$ then corresponds to a $k$-cycle of $P$, i.e. the trail containing the edge $e$ is given by
	\begin{equation*}
		e,P(e),P^2(e),...,P^{k-1}(e).
	\end{equation*}
\end{thm}
...
\begin{proof}
	Given a decomposition of the directed multigraph into closed directed trails, define a map $P:E(G)\rightarrow E(G)$ by $P(e)=e^\prime$, where $e^\prime$ is the edge that comes after $e$ in the closed trail containing $e$. In order to show that $P$ is a permutation, it remains to be shown that it is injective. Hence, assume that 
	\begin{equation*}
...
Given a closed trail $T=e_1,...,e_k$, let its length be
...
\begin{thm}\label{thm:perm2}
	Let $G$ be a Eulerian directed multigraph and let $P$ be a linear $G$-permutation. If $T_1,...,T_m$ are the closed trails of the decomposition defined by $P$, then the spectrum of $D_{\Gamma(P)}$ is given by
	\begin{equation*}
...
\begin{proof}
	...
	\begin{equation*}
		\sigma(D_{\Gamma(P)})=\bigcup_{i=1}^m\sigma(D_{\Gamma(P_i)}),
	\end{equation*}
	...
	The transformation matrix of $P$ with respect to the basis $e_{0},...,e_{\lvert E(T)\rvert-1}$ is equal to
	\begin{equation*}
		\left(
...
As we have seen, the spectrum of the Dirac operator is closely tied to the boundary condition it is subjected to. Boundary conditions involve choices which are a priori not canonical. In order to obtain a theory that ties the spectrum of the Dirac operator to properties of the underlying graph, we need to come up with an intrinsic boundary condition. By Proposition \ref{prop: s.a. implies eulerian}, this boundary condition will in general not be self adjoint. However, canonical matrices on graphs are given by directed adjacency matrices:

\begin{defin}
	The \textbf{directed adjacency matrix} {\boldmath$\mathcal{A}(G)$} of a directed multigraph $G$ is given by
	\begin{align*}
		\mathcal{A}(G) &= \sum_{e, e^\prime \in E} I_{e, e^\prime} e\otimes (e^\prime)^\ast \in \mathbb{C}^E\otimes(\mathbb{C}^E)^\ast = \mathrm{End}(\mathbb{C}^E), \\
...
\begin{defin}
	A \textbf{disjoint collection of cycles} is a directed multigraph $C$, whose connected components consist of directed cycles. The \textbf{connectedness} {\boldmath$\mathbf{\alpha(C)}$} of $C$ is the number of connected components of $C$, the \textbf{size} {\boldmath$\mathbf{\eta(C)}$} of $C$ is the number of edges of $C$. For a directed multigraph $G$, set
	$$\mbox{\boldmath$\mathbf{\mathcal{C}(G)}$}:=\{C\subseteq G\mid C \text{ is a disjoint collection of cycles} \}.$$
\end{defin}


\begin{rmk}
	The empty subgraph $\varnothing$ with no vertices and edges is always an element of $\mathcal{C}(G)$. Its connectedness and size equal zero.
\end{rmk}
...
\begin{lem}\label{lem:idod}
	If $G$ is a directed multigraph such that for every vertex $v\in V(G)$ the in-degree $id(v)$ and the out-degree $od(v)$ are both equal to one, then $G$ is a disjoint collection of cycles.
\end{lem}
...
\begin{proof}
	...
	where $\alpha(G)=s$ and $\eta(G)=k_1+...+k_s$, it follows that $\mathcal{A}(G)$ is the product of cycles of size $k_1,...,k_s$. Since the signature of a cycle of size $k$ is given by $(-1)^{k-1}$, the signature $\sgn(\mathcal{A}(G))$ of $\mathcal{A}(G)$ is given by $-1$ to the power of
...
\begin{cor}
	...
	\begin{itemize}
		...
		\item $a_{n-3}=-\#\{3\text{-cycles}\} + \#\{\text{pairs of loops and }2\text{-cycles that are disjoint}\}\\-\#\{\text{triples of loops that are disjoint}\}$
	\end{itemize}
\end{cor}
\begin{proof}
