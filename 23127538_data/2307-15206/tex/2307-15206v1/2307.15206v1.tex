...
%\thanks{corresponding author:Masato Kobayashi, masato210@gmail.com}
...
proves a system of differential equations for Eisenstein series $(E_{2k}^{*})$ of level 2. This is an analog of Theorem \ref{t1} and \ref{t2}. 
...
Maier (2011) \cite{maier} and Toh (2011) \cite{toh} for other equivalent descriptions of such a system.
...
$(\cotz)'=-1-\cot^{2}z$. Since proofs are elementary, we omit such details here.
...
For $\ta\in \mathbf{H}=\{\ta\in \cc\mid \text{Im}(\ta)>0\}$, 
let $q=e^{2\pii \ta}$. Note that although we deal with various infinite sums below, we will not specify those radii of convergence.
\begin{defn}
\[
\th_{1}(z|\ta)=
2
...
\]
and $G_{0}(\ta)=-1$ for all $\ta\in \mathbf{H}$ for convenience.
\end{defn}
...
\subsection{Differential equations for Eisenstein series of level 2}
 Our next idea is to consider an analog of \er{ldth1} with 
$\th_{2}$.
\begin{defn}
\[
\th_{2}(z|\ta)=
...
\]
for all $n\ge0$. Ramanujan-Shen differential equation  for $2m-2=12$ is particularly simple due to the relations 
...
\begin{thm}[Mordell 1917 \cite{mordell}]\hf
\begin{enumerate}
...
\end{cj}
Deligne \cite{de} proved it in 1974.
...
\begin{fact}[Garvan's identity {\cite[Theorem 1.4]{milne}}]
\[
\left|\begin{array}{ccc}
	E_{4}&   E_{6}&E_{8}   \\
	E_{6}&   E_{8}&E_{10}   \\
...
\]
forms its basis. Note that $E_{2}^{*}$ is \eh{not} a 
modular, but a quasi-modular form of weight 2 on 
$\Gam_{0}(2)$. As a matter of fact, $E_{2k}^{*}\in M_{2k}$ for each $2k\ge4$.
%belongs to $M_{2}$.
...
%	\ff{1}{17}(9B^{2}+8BC^{2}).
%	\\
	E_{10}^{*}&=\ff{1}{31}(27B^{2}C+4BC^{3}).
	\label{e10*d}
	\\E_{12}^{*}&=\ff{1}{691}(
	189B^{3}+486B^{2}C^{2}+16BC^{4}).\label{e12*d}
\end{align}
\end{ex}
...
\del C=-\ff{1}{2}B.
\]
As a consequence, $f \in \cc[A, B, C]_{2k}\then 
\del f \in \cc[A, B, C]_{2k+2}$ for $2k\ge2$.
\end{prop}
\begin{proof}
\begin{align*}
	\del A&=qA'-\ff{2}{4}A^{2}=-\ff{1}{4}B,
	\\\del B&=qB'-\ff{4}{4}AB=-BC,
	\\\del C&=q\der{\ff{E_{6}^{*}}{E_{4}^{*}}}-\ff{2}{4}AC
	\\&=
\ff{\tf{1}{2}\k{3ABC-B^{2}-2BC^{2}}B-BC(AB-C)
}{B^{2}}
-\ff{1}{2}AC=
-\ff{1}{2}B.
\end{align*}
...
\begin{fact}
$f\in M_{2k}\then \del f\in M_{2k+2}$.
\end{fact}
...
Further, denote $\qq_{+}(x_{1}, \ds, x_{n})$
 ($\qq_{-}(x_{1}, \ds, x_{n})$) 
the set of polynomials in $x_{1}, \ds, x_{n}$ with all coefficients positive (negative) rational numbers.

\begin{lem}\label{neg}Let $2k\ge2$.
...
\begin{enumerate}
\item 
Let $S_{2k}$ denote the set of \eh{cusp forms} on 
$\Gam_{0}(2)$. We have 
$\dim S_{2}=0$ and for $2k\ge4$, $\dim S_{2k}=t-1$ where $t=[\tf{2k}{4}]$.
...
\]
forms a basis of $S_{2k}$ as Imamo$\bar{\text{g}}$lu-Kohnen discussed \cite[p.828]{ik}.
...
In the literature, authors often discuss these equations  equivalently as $q\d'=E_{2}\d$ and $qD'=E_{2}^{*}D$. 
	\item Coefficients of $D$ has also a nice arithmetical interpretation: 
\[
D=
q 
...
\end{fact}
Obviously, this comes from the calculus of $\th_{3},  E_{12}$ and $\d$. Let us figure out some relation among $\th_{3}, E_{2k}^{*}$ and $\d$. We then come across certain coincidence of coefficients so that we find  nice expressions of $r_{24}(n)$ with $\ta(n)$.
\begin{thm}\label{t10}
\begin{align*}
	r_{24}(n)&=(-1)^{n}64
\k{
...
$\th_{3}^{24}(-q)=E_{4}^{*3}(q)=E_{6}^{*2}(q)-64\d(q)$, that is, 
\[
(-1)^{n}r_{24}(n)=
