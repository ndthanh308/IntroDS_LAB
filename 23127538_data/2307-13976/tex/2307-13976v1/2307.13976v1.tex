...
First note that $O_r(G)$ is contained in every Sylow $r$-subgroup of $G$ and so one can pass to $G/O_r(G)$. Similarly, the Frattini subgroup $\Phi(G)$ is contained in every maximal subgroup of $G$, which allows us to work with 
$G/O_r(G)\Phi(G)$. Of course, this quotient may have a nontrivial $r$-core, and possibly a nontrivial Frattini subgroup. For simplicity, we state the next result under the assumption $\Phi(G)=O_r(G)=1$ and we refer the reader to Theorem \ref{t:main2gen} for the general case. Note that the condition $\Phi(G) = 1$ implies that $E(G)$ is a direct product of nonabelian simple groups.

\begin{theorem}\label{t:main2}  
Let $G$ be a finite group, let $r$ be a prime divisor of $|G|$ and let $R$ be a Sylow $r$-subgroup of $G$. Assume that $\Phi(G) = O_r(G)=1$.  Then $R$ is contained in a unique maximal subgroup of $G$ if and only if one of the following holds:
\begin{itemize}\addtolength{\itemsep}{0.2\baselineskip}
...
\item[{\rm (iii)}] $G=E(G)R$, where $E(G)$ is a direct product of two or more  nonabelian simple groups, and $R$ acts transitively on the set of components of $G$. Moreover, if $L$ is a component of $G$, then $N_R(L)/C_R(L)$ is a Sylow $r$-subgroup of the almost simple group $J = N_G(L)/C_G(L)$ and it is contained in a unique maximal subgroup $H$ of $J$, so $(J,r,H)$ is recorded in Theorem \ref{t:prime2} or \ref{t:main1}.
\end{itemize} 
\end{theorem} 
...
In \cite{Baum}, Baumann determines the structure of insoluble finite groups $G$ with a nilpotent maximal subgroup, showing that $O^2(G/F(G))$ is a direct product of nonabelian simple groups with dihedral Sylow $2$-subgroups (here the simple groups that can arise are of the form ${\rm L}_2(q)$, where $q=9$ or $q = 2^m \pm 1 \geqs 7$ is a prime). As an application of Theorem \ref{t:main2}, we can describe the structure of a finite group with a maximal Sylow $r$-subgroup. 

To do this, first note that a Sylow $r$-subgroup $R$ is maximal in $G$ if and only if $R/O_r(G)$ is maximal in $G/O_r(G)$, so we are free to assume that $O_r(G)=1$. In addition, if $R$ is maximal then it contains $\Phi(G)$, so $\Phi(G) \leqs O_r(G)$ and thus $\Phi(G) = O_r(G)=1$ as in Theorem \ref{t:main2}.
...
$\mathbb{M}$. We refer the reader to Wilson's survey article \cite{Wil} for further details, noting that $\mathbb{M}$ has $44$ known conjugacy classes of maximal subgroups and any additional maximal subgroup must be almost simple with socle ${\rm L}_{2}(q)$ or ${\rm U}_3(q')$, where $q \in \{8,13,16\}$ and $q' \in \{4,8\}$. 
...
Next we consider the case of coprime subgroups. The proof of the following result depends on the Schur-Zassenhaus Theorem (which in turn relies on the Feit-Thompson Theorem). However, we only need to apply this when one of the  subgroups is an $r$-group and this only requires Sylow's Theorem.
  
\begin{lem} \label{l:coprime}   
Let $G = NK$ be a finite group, where $N$ and $K$ are subgroups of $G$ with $N$ normal, $N \not\leqs \Phi(G)$ and $(|N|, |K|)=1$. Then $K$ is contained in a unique maximal subgroup of $G$ if and only if $N$ is a $p$-group for some prime $p$ and $K$ acts irreducibly on $N/\Phi(N)$.
\end{lem}
 
\begin{proof}  
First assume that $N$ is a $p$-group and $K$ acts irreducibly on $N/\Phi(N)$. We claim that $K$ is contained in a unique maximal subgroup of $G$. This is clear if $K$ itself is a maximal subgroup, so we may assume $H$ is a subgroup of $G$ such that $K<H<G$. Set $A = N \cap H$ and note that $A \ne 1$. Then $A\Phi(N)/\Phi(N)$ is a $K$-invariant subgroup of $N/\Phi(N)$, so the irreducibility of $K$ on $N/\Phi(N)$ implies that $A\Phi(N) = \Phi(N)$ or $N$. If $A\Phi(N) = \Phi(N)$ then $A \leqs \Phi(N)$ and we deduce that $H \leqs K\Phi(N)$. On the other hand, if $A\Phi(N) = N$ then $A = N$ and $H = G$, which is a contradiction. We conclude that $K\Phi(N)$ is the unique maximal subgroup of $G$ containing $K$. 

For the remainder, we will assume $K$ is contained in a unique maximal subgroup $H$ of $G$. By passing to $G/(N \cap \Phi(G))$, we may assume that $N \cap \Phi(G)=1$. Therefore, $\Phi(N)=1$ and $F(N)$ is abelian. 

Let $r_1, \ldots, r_k$ be the distinct prime divisors of $|N|$ and let
$R_i$ be a Sylow $r_i$-subgroup of $N$. Then for each $i$ we have
$G=NN_G(R_i)$ by the Frattini argument, hence $|N_G(R_i)|$ is divisible
by $|K|$ and by applying the Schur-Zassenhaus Theorem we may assume that
$K \leqs N_G(R_i)$. Note that $N = \la R_1, \ldots, R_k \ra$ and thus $G = \la N_G(R_1), \ldots, N_G(R_k) \ra$.

If each $R_i$ is non-normal in $G$, then $N_G(R_i) \leqs H$ for all $i$ and thus $H=G$, which is a contradiction. Therefore, we may assume $R_1$ is a normal subgroup of $G$, so $R_1 \leqs F(N)$ and thus $R_1$ is abelian. Then Lemma \ref{l:gas} implies that there
exists a subgroup $L$ of $G$ such that $G = R_1L$ and $R_1 \cap L =1$.
By Dedekind's modular law, we have $R_1K= R_1K_0$ for some subgroup
$K_0$ of $L$, so $K$ and $K_0$ are conjugate by the Schur-Zassenhaus
Theorem and therefore we may assume
that $K \leqs L \leqs H$. If $G \ne R_1K$, then we must have $R_1K \leqs
H$, but this implies that $H=G$ and we have reached a contradiction.
Therefore, $G = R_1K$ and we deduce that $N=R_1$.  The result now follows since $K$ acts irreducibly on $N$ by Lemma \ref{l:normal}.
\end{proof}
...
For the remainder of this section, let $G$ be a finite group, let $r$ be a prime divisor of $|G|$ and let $R$ be a Sylow $r$-subgroup of $G$. Let $\mathcal{M}(R)$ be the set of maximal subgroups of $G$ containing $R$.

Let us assume $R$ is contained in a unique maximal subgroup $H$ of $G$, so $R$ is a proper subgroup of $G$ (at this point, we are making no
further assumptions on $G$, other than finiteness). Note that if $R$ is normal in $G$, then $G/R$ must be cyclic of prime order $s \ne r$ and thus $G = R \times S$, with $S$ cyclic of order $s$. The next result is a noteworthy observation.
...
Suppose $O_{r'}(G) \not\leqs \Phi(G)$. Then $R$ is contained in a unique maximal subgroup of $G$ if and only if $G = P{:}R$, where $P$ is  $p$-group for some prime $p \ne r$ and $R/O_r(G)$ acts faithfully and irreducibly on $P/\Phi(P)$. 
\end{lem} 

\begin{proof}    
If $G=P{:}R$ and $R$ acts on irreducibly on $P/\Phi(P)$, then $R\Phi(P)$ is clearly the unique maximal subgroup of $G$ containing $R$. 
...
$G=N{:}R$ and thus Lemma \ref{l:coprime} implies that $N$ is a $p$-group for some prime $p \ne r$ and $R$ acts irreducibly on $N/\Phi(N)$. It just remains to show that $R/O_r(G)$ acts faithfully on $N/\Phi(N)$. To see this, set $C=C_R(N/\Phi(N))$ and note that $C$ centralises $N$ (indeed, we have $N=[C,N]C_N(C)$ and $[C,N] \leqs \Phi(N)$). Therefore, $C=O_r(G)$ and the result follows.
\end{proof}  

\begin{lem}\label{l:case2}
Suppose $O_r(G)=1$ and $O_{r'}(G) \leqs Z(G)$. Set $E = E(G)$. Then $R$ is contained in a unique maximal subgroup of $G$ if and only if the following hold:
\begin{itemize}\addtolength{\itemsep}{0.2\baselineskip}
\item[{\rm (i)}] $G=ER$ and $R$ acts transitively on the set of components of $G$. 
\item[{\rm (ii)}] Moreover, if $L$ is a component of $G/\Phi(E)$, then 
\[
N_R(L)C_G(L)/C_G(L) \cong N_R(L)/C_R(L)
\]
is a Sylow $r$-subgroup of the almost simple group 
$J = N_G(L)/C_G(L)$ and it is contained in a unique maximal subgroup of $J$.    
\end{itemize}
\end{lem} 

\begin{proof}
Since $O_r(G)=1$, let us first observe that $R$ acts faithfully on $F^*(G) = Z(G)E$ and hence also on $E$.

Suppose that (i) and (ii) hold. By passing to $G/\Phi(E)$, we may assume that $E = L_1 \times \cdots \times L_t$ is a direct product of isomorphic nonabelian simple groups. Then $R$ is contained in a unique maximal subgroup of $G$ by Lemmas  \ref{l:normal} and \ref{l:simpleprod}. 

For the remainder, let us assume $\mathcal{M}(R) = \{H\}$. Then $G = N_G(E \cap R)E$ by the Frattini argument, so $G = ER$ since $R \leqs ER$ and $R \leqs N_G(E \cap R)$. In particular, we have 
\[
O_{r'}(G) \leqs E \cap Z(G) \leqs Z(E) \leqs \Phi(E) \leqs \Phi(G).
\]   
Let $X = \{L_1, \ldots, L_s\}$ be the set of components of $G$ and suppose $R$ acts intransitively on $X$. Then we can write $E = E_1E_2$, where the $E_i$ are proper normal subgroups of $G$. Then $R \leqs RE_i < G$, so $RE_1,RE_2 \leqs H$ and thus $G = \la R, E_1, E_2 \ra \leqs H$, which is a contradiction. We conclude that (i) holds. Finally, in order to show that (ii) holds, we can pass to $G/\Phi(E)$, which means that we may assume $O_{r'}(G)=1$ and each $L_i$ is simple. In addition, $R \cap L_i$ is nontrivial for each $i$ and the result now follows by Lemma \ref{l:simpleprod}.  
\end{proof}

\begin{proof}[Proof of Theorem \ref{t:main2}]
This is now entirely straightforward: apply Lemma \ref{l:case1} if $O_{r'}(G) \ne 1$, otherwise Lemma \ref{l:case2} gives the desired conclusion.
\end{proof}

Let $G$ be a finite group and let $r$ be a prime divisor of $|G|$. In Theorem \ref{t:main2} we assume that $\Phi(G) = O_r(G) = 1$ and we conclude this section by considering the general case. To do this, set $W(G) = \Phi(G)O_r(G)$ and  inductively define an ascending chain of subgroups of $G$, where $W_1(G) = W(G)$ and $W_{i+1}(G)/W_i(G) = W(G/W_i(G))$ for $i \geqs 1$. Let $D$ be the final subgroup in this chain. Then it is easy to see that $D$ is a normal 
soluble subgroup of $G$ with $W(G/D) = 1$. Moreover, $D$ is the smallest normal subgroup of $G$ with these properties.

By combining the following result with Theorem \ref{t:main2} we can describe the structure of an arbitrary finite group with a Sylow $r$-subgroup contained in a unique maximal subgroup. Note that $\Phi(G/D) = O_r(G/D) = 1$, so the pairs 
$(G/D,r)$ arising in part (ii) are determined in Theorem \ref{t:main2}.

\begin{thm} \label{t:main2gen}   
Let $G$ be a finite group, let $r$ be a prime divisor of $|G|$ and let $R$ be a Sylow $r$-subgroup of $G$.
\begin{itemize}\addtolength{\itemsep}{0.2\baselineskip}
\item[{\rm (i)}] The map $M \mapsto MD/D$ defines a bijection from the set of maximal subgroups of $G$ containing $R$ to the set of maximal subgroups of $G/D$ containing the Sylow $r$-subgroup $RD/D$.
\item[{\rm (ii)}] In particular, $R$ is contained in a unique
maximal subgroup of $G$ if and only if $RD/D$ is contained in a unique maximal
subgroup of $G/D$.
\end{itemize}
\end{thm}

\begin{proof}   
As above, set $W=O_r(G)\Phi(G)$ and observe that (ii) follows immediately from (i). Also note that $D=1$ if $W=1$, so we may assume $W$ is nontrivial. Since $O_r(G) \leqs R$ and $\Phi(G)$ is contained in every maximal subgroup of $G$, it follows that $\mathcal{M}(R) = \mathcal{M}(RW)$. By passing to $G/W$, the result now follows easily by induction. 
\end{proof}

\begin{rem}\label{r:neww}
Let $G$ be a finite group, let $r$ be a prime divisor of $|G|$ and let $R$ be a Sylow $r$-subgroup of $G$. Suppose $G$ has a unique conjugacy class of maximal subgroups of $r'$-index. 
\begin{itemize}\addtolength{\itemsep}{0.2\baselineskip}
\item[{\rm (a)}] If $G$ is soluble, then $|G|$ is divisible by precisely two primes.
\item[{\rm (b)}] If $E(G) \ne 1$, then $G = E(G)R$ and $D = Z(E(G))O_r(G)$, where $D$ is the subgroup defined above.
\end{itemize}
\end{rem}
...
\item[{\rm (b)}] In the first row of Table \ref{tab:NGR}, we write $\mathcal{P}$ for the set of primes $r$ that are not of the form $(q^d-1)/(q-1)$ for some prime power $q$ and integer $d \geqs 2$. Note that the first $240$ prime numbers of the form $(q^d-1)/(q-1)$ are recorded in \cite[Table II]{BS}.
\end{itemize}
\end{rem}
...
\bibitem{Craven}
D.A. Craven, \emph{The maximal subgroups of the exceptional groups $F_4(q)$, $E_6(q)$ and ${}^2E_6(q)$ and related almost simple groups}, submitted (arXiv:2103.04869).
...
\bibitem{Craven3}
D.A. Craven, \emph{On medium-rank Lie primitive and maximal subgroups of exceptional groups of Lie type}, Mem. Amer. Math. Soc., to appear.
...
\bibitem{GHT}
R.M. Guralnick, H.P. Tong-Viet and G. Tracey, \emph{Weakly subnormal subgroups and variations of the Baer-Suzuki theorem}, in preparation.
...
\bibitem{Is}
I.M. Isaacs, \emph{Finite group theory}, Graduate Studies in Mathematics, vol. 92, American Mathematical Society, Providence, RI, 2008.
