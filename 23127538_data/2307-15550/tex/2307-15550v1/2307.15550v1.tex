...
\title{Complex hyperbolic Gromov-Thurston metrics and almost $\frac{1}{4}$-pinched K\"{a}hler manifolds}
...
%    General info
%\subjclass[2010]{Primary 53C20, 53C2; Secondary 20F65, 57R19, 57R25}
...
\begin{abstract}
In this paper we construct an almost negatively $\frac{1}{4}$-pinched Riemannian metric on a class of compact manifolds that, via previous work, was already known to be K\"{a}hler and not locally symmetric.  
This is the first known example of such manifolds and, via the result of Hernandez \cite{Hernandez} and Yau and Zheng \cite{YZ}, these manifolds cannot admit a negatively quarter-pinched Riemannian metric.
This metric is also interesting because it is the complex hyperbolic analogue to the famous pinched metric constructed by Gromov and Thurston in \cite{GT}.  
\end{abstract}
...
Via Mostow rigidity \cite{Mostow} one knows that any compact K\"{a}hler manifold with negatively $\frac{1}{4}$-pinched holomorphic sectional curvature (meaning the curvatures of all complex lines lie in the interval $[-1,-1/4]$) must be isometric to a quotient of the complex hyperbolic space $\C \H^n$.  
Even more surprisingly, by Hernandez \cite{Hernandez} and Yau and Zheng \cite{YZ} it holds that any K\"{a}hler manifold with a negatively $\frac{1}{4}$-pinched {\it Riemannian} metric must likewise be isometric to a locally symmetric manifold.  

Mostow rigidity of course holds in the hyperbolic case, but there are now various examples of Riemannian manifolds whose sectional curvatures are pinched arbitrarily close to $-1$, but which are not homotopy equivalent to a locally symmetric manifold.  
The first known examples were the branched cover manifolds constructed by Gromov and Thurston in \cite{GT}, but one now obtains many more examples from Ontaneda's smooth hyperbolization \cite{Ontaneda}.  

Very little is known about pinched metrics in the K\"{a}hler setting.
If a K\"{a}hler metric has holomorphic sectional curvatures that are $\e$-close to being negatively $\frac{1}{4}$-pinched, meaning that all curvatures lie in the $\e$ neighborhood of $[-1,-1/4]$ for some sufficiently small $\e > 0$, then the underlying manifold must be ``close" to being locally symmetric in a sense made explicit by Dereax and Seshadri in \cite{DS}.  
But it is unknown if any such manifolds exist (outside of the locally symmetric ones). 
In light of the results in \cite{Hernandez} and \cite{YZ}, it is natural to ask if there are even any K\"{a}hler manifolds which admit an almost negatively $\frac{1}{4}$-pinched Riemannian metric but are not locally symmetric?  
The main result of this paper is the construction of the first known examples in this latter category.  
%More explicitly, the main result of this paper is part (a) of the following Theorem.  
%Most of the remainder of this Theorem is due to Stover and Toledo in \cite{ST} and Zheng in \cite{Zheng}.  
...
For any $\e > 0$, there exist K\"{a}hler manifolds that admit a Riemannian metric that is $\e$-close to being negatively $\frac{1}{4}$-pinched and are not homotopy equivalent to a locally symmetric manifold.  
\end{theorem}
...
A few quick remarks about the Theorem.  
As mentioned in the abstract, the Riemannian metric in Theorem \ref{thm:main theorem} is the complex hyperbolic analogue to the famous Gromov-Thurston metric in \cite{GT}.  
The Riemannian metric from the Theorem is generally not K\"{a}hler.  
%Note that the submanifold $\tilde{N}$ may be disconnected.  
Also, for the remainder of this paper we use the interval $(-4-\e, -1+\e)$ instead of $(-1-\e, -1/4 + \e)$ purely for notational convenience.  
One can adjust the curvatures between these two intervals by appropriately scaling the metric.

%As stated above, the new result developed in this paper is part (a) in Theorem \ref{thm:main theorem}.  
The new construction in this paper is the almost negatively $\frac{1}{4}$-pinched metric in Theorem \ref{thm:main theorem}.
...
Let $M' = \Gamma' \setminus \C \H^n$.   
Our almost $\frac{1}{4}$-pinched Riemannian metric will require a large normal injectivity radius about the totally geodesic submanifold.  
...
Stover and Toledo in \cite{ST} also prove that $X$ is not homotopy equivalent to a locally symmetric manifold (Theorem 1.5 in \cite{ST}).  
The fact that $X$ admits a K\"{a}hler metric is due to Zheng in \cite{Zheng}.  
...
%Also, while the metric developed by Zheng is negatively curved, it is not clear to the author that these curvatures are bounded above by a negative constant.  
%In this case, it is possible that the metric constructed in this paper is the first example of a negatively curved Riemannian metric on a non-symmetric K\"{a}hler manifold whose curvatures are bounded above by a negative constant.  
...
It seems likely to the author that there exists an example of a branched cover $X$ which satisfies Theorem \ref{thm:main theorem} but whose ratio of chern numbers is not close to that of a complex hyperbolic manifold, but again this is not known.  
...
Let
	...
	\end{equation*}
where $\y_{n-2}$ denotes the hyperbolic metric on the core copy of $\H^{n-2}$.
...
The associated warped product metric is
	\begin{equation*}
	\mu =  h^2 \c_{n-1} + \frac{1}{4} v^2 d \th^2 + dr^2.
	\end{equation*}
where $\c_{n-1}$ denotes the complex hyperbolic metric on the core copy of $\C \H^{n-1}$.  
A major difference between the hyperbolic and complex hyperbolic metrics is that the horizontal fiber is now not integrable, meaning that it is not everywhere tangent to the $r$-tube about $\C \H^{n-1}$.  
...
There now exist terms in various components of the curvature tensor where $v$ appears without a corresponding $v'$ or $v''$.  
But each of these terms contains a structure constant.  
...
This is equivalent to slowly turning the horizontal fiber of the metric until it is tangent to the $r$-tube about the core copy of $\C \H^{n-2}$.  
By turning this fiber slowly we can keep the curvatures pinched near $[-4,-1]$.  
The almost $\frac{1}{4}$-pinched metric in Theorem \ref{thm:main theorem} is not K\"{a}hler, and this is the part of the process that is the reason for this.  
...
We can then increase the angle with this metric in an almost identical way as to what was described above with the Gromov-Thurston metric while keeping the curvature pinched (Theorem \ref{thm:warping construction 2}).  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{comment}
\subsection*{Layout of this article} 
In Section 2 below we review curvature formulas for the warped product metric for hyperbolic and complex hyperbolic space written in polar coordinates about a (real) codimension two totally geodesic submanifold.  
We then prove ``The Pinching Lemma" (Lemma \ref{thm:pinched curvature}) which is the key technical tool that allows us to keep the curvature almost $\frac{1}{4}$-pinched during the unwinding and winding processes described above.  

In Section 3 we describe the integrable complex hyperbolic metric $g_I$, the $d$-fold integrable complex hyperbolic metric $g_d$, and prove that each of the three warping procedures outlined above can be completed in a pinched way (Theorem \ref{thm:warping construction 1}, Corollary \ref{cor:warping construction 3}, and Theorem \ref{thm:warping construction 2}).  
We then give a formal proof of part (a) of Theorem \ref{thm:main theorem}.  
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{comment}
{\color{red} Beginning of second introduction}

In \cite{GT} Gromov and Thurston discover new examples of negatively curved Riemannian manifolds.  
The underlying manifolds $X^n$ in these examples are $d$-fold cyclic ramified covers of a hyperbolic manifold $M^n$ about a codimension $2$ totally geodesic submanifold $N^{n-2}$.  
It is proved that these manifolds can be constructed so that they admit a Riemannian metric whose sectional curvatures lie in the interval $(-1-\e, -1+\e)$ for any prescribed $\e > 0$ and that, for all but finitely many values of $d$, this manifold is not homeomorphic to a locally symmetric manifold.  

It is a classical question if this process can be extended to complex hyperbolic manifolds.  
The three difficult steps in this process are
	\begin{enumerate}
	\item  finding a pair $(M,N)$ modeled on $(\C \H^n, \C \H^{n-1})$ whose $d$-fold cyclic ramified covering $X$ is a manifold
	\item  constructing a negatively curved Riemannian metric on $X$, and 
	\item  proving that this manifold is not homeomorphic to a locally symmetric one.  
	\end{enumerate}
With regards to (1) above, we say that the pair $(M,N)$ is {\it modeled on} $(\C \H^n, \C \H^{n-1})$ if there exist lattices $\Gamma \subset \text{Isom}(\C \H^n)$ and $\Lambda \subset \text{Isom}(\C \H^{n-1})$ such that $M = \C \H^n / \Gamma$, $N = \C \H^{n-1} / \Lambda$, and $\Lambda < \Gamma$.  	
Note that it is possible for $N$ to be disconnected.  

In a recent paper \cite{ST}, Stover and Toledo construct infinitely many such pairs $(M, N)$ that satisfy conditions (1) and (3) above.  
The fact that these manifolds admit a metric of strictly negative sectional curvature is then a result of Zheng in \cite{Zheng}, answering question (2) as it is stated.  

It is still an open question whether or not the manifolds constructed in \cite{ST} admit Riemannian metrics whose sectional curvatures are pinched near the interval $[-4,-1]$.  
No strict curvature bounds are proved for the metric developed in \cite{Zheng} other than the fact that all sectional curvatures are negative.  
In particular, if the totally geodesic submanifold $N$ is not compact, then it is possible that there are sectional curvatures which attain values arbitrarily close to zero.  
The main result of the present work is proving that all but finitely-many of the ramified covers constructed in \cite{ST} admit such a pinched negatively curved metric.  
...
\begin{comment}
\begin{theorem}\label{thm:main theorem}
For any given $\e > 0$, there exist infinitely many pairs $(M, N)$ modeled on $( \C \H^n, \C \H^{n-1})$ whose $d$-fold cyclic ramified cover $X$ is a manifold and which admits a Riemannian metric with all sectional curvatures lying in the interval $(-4-\e, -1+\e)$.  Moreover, none of these manifolds $X$ is homeomorphic to a locally symmetric manifold.  
\end{theorem}
\end{comment}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{comment}
To construct the smooth negatively curved Riemannian metric on the $k$-fold ramified covering of $M$ about $N$, we begin by putting the ``hyperbolized complex hyperbolic" metric $\hch$ on a large neighborhood about the core $N$.  
We then slowly ``unbend" this metric to obtain the hyperbolic metric ${\bf h_{2n}}$ on $\H^{2n}$ (see Theorem \ref{}).
We then vary this metric to $k$-copies of ${\bf h_{2n}}$ via the exact same construction used by Gromov and Thurston in Section 2 of \cite{GT}.
At this point we apply the following Theorem, which may be of its own independent interest, to each copy of $N$ individually to in order to warp the hyperbolic metric ${\bf h_{2n}}$ to the complex hyperbolic metric ${\bf c_n}$.

\vskip 20pt

\begin{theorem}\label{thm:h4 to ch2}
Let $P$ be an $n$-plane in $\mathbb{R}^{2n}$, $E = P \times \mathbb{S}^{n-1}$, and write $\mathbb{R}^{2n} \setminus P \cong E \times (0,\infty)$.  
Then for all $\e > 0$ there exist positive real numbers $r_1$ and $r_2$ with $0 < r_1 < r_2$, and a Riemannian metric $g$ on $\mathbb{R}^{2n} \setminus P$, such that
	\begin{enumerate}
	\item the metric $g$ restricted to $E \times (0,r_1)$ is the real hyperbolic metric ${\bf h_{2n}}$ on $\H^{2n}$.  
	\item the metric $g$ restricted to $E \times (r_2, \infty)$ is the complex hyperbolic metric ${\bf c_n}$ on $\C \H^n$.  
...
The metric in Theorem \ref{thm:h4 to ch2} is constructed in two steps, which are mostly analogous to those in \cite{LMMT}.  
In the first step, we vary the metric from the hyperbolic metric ${\bf h_{2n}}$ to what we call the {\it warped complex hyperbolic metric} ${\bf w_{ch}}$ (see \ref{subsection:warped ch2} for details).  
Of course, in the second step we then vary $g$ from ${\bf w_{ch}}$ to ${\bf c_n}$.  
These two steps are recorded in Theorems \ref{thm:h4 to wch2} and \ref{thm:wch2 to gch}, respectively, and so the combination of these Theorems proves Theorem \ref{thm:h4 to ch2}.

It should be noted that in \cite{FJ} Farrell and Jones construct a pinched negatively curved metric which varies from the hyperbolic metric $\gh$ to the complex hyperbolic metric $\gch$.  
But their metric varies radially from a point, whereas our metric varies radially from a copy of $\h2$.  
While there are certainly some similarities in the constructions, it is not clear to the authors how to use the work in \cite{FJ} to aid in the proof of Theorem \ref{thm:h4 to ch2}.
\end{comment}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We then prove a key ``Pinching Lemma" which will be very important when we want to prove that our metric that ``unwinds" the complex hyperbolic metric has almost $\frac{1}{4}$-pinched curvature.
...
The following Lemma is our main technical result which ensures that the metric constructed below in Theorem \ref{thm:warping construction 1} and Corollary \ref{cor:warping construction 3} is almost negatively $\frac{1}{4}$-pinched.  
In the statement below, $K_g(\sigma)$ denotes the sectional curvature of the $2$-plane $\sigma$ with respect to the metric $g$.  

\vskip 20pt

\begin{lemma}[The Pinching Lemma]\label{thm:pinched curvature}
...
In this Section we define two metrics on the product $E \times (0, \infty)$ and prove that we can interpolate between various metrics in an almost $\frac{1}{4}$-pinched way as discussed in the Introduction (see Theorem \ref{thm:warping construction 1}, Corollary \ref{cor:warping construction 3}, and Theorem \ref{thm:warping construction 2} below).  
...
The next Theorem proves that there exists an almost $\frac{1}{4}$-pinched metric that interpolates between the complex hyperbolic metric $\c_n$ and the metric $g_I$.  
This Theorem and the following Corollary provide two of the three steps in the construction of the almost $\frac{1}{4}$-pinched Riemannian metric in Theorem \ref{thm:main theorem}.  
...
Let $\e > 0$.  
Then there exist positive constants $r_1 < r_2$ and a Riemannian metric $g$ defined on $E \times (0, \infty)$ such that
	\begin{enumerate}
...
To show that the metric $g$ is almost $\frac{1}{4}$-pinched, we need to compute the Lie brackets of the basis $(X_i)$ of the horizontal fiber.
...
Via the discussion in the Introduction, all that is left to prove is the construction of the metric $g$ on the $d$-fold ramified cover of $\C \H^n$ about $\C \H^{n-1}$ with all curvatures almost $\frac{1}{4}$-pinched.
...
As long as the ramification locus of $X$ has a normal injectivity radius of at least $r_3$, this metric $g$ will descend to a well-defined Riemannian metric on $X$ that is $\e$-close to being negatively $\frac{1}{4}$-pinched.  
