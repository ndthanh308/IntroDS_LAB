...
For the other data combination shown in Figure~\ref{fig:fnlST_AllOs}, that is, including the CDF $W$ mass measurement while leaving all the other observables unchanged, kinetic mixing is unable to completely explain the data.  This is clear in the $M_{Z_D} \gg M_Z$ limit as the blue ellipse does not intersect the cyan dark photon line in Figure~\ref{fig:fnlST_AllOs}.  We have also explicitly checked that this is true when we extend to dark photons masses comparable to $M_Z$ by allowing for nonzero $U$ in the fit. As a result, there is no contour that corresponds to this scenario in Fig.~\ref{fig:fnlMzPrEta_AllOs}. 
