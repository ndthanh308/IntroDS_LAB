...
%\newcommand{\ThisIsTheAffiliation}{Yau Mathematical Sciences Center, Tsinghua University, Beijing 100084, China}
%\newcommand{\ThisIsTheAffiliation}{Department of Physics \& The Oskar Klein Centre, Stockholm University, \\ AlbaNova University Centre, SE-106 91 Stockholm, Sweden}
%\newcommand{\ThisIsTheAffiliation}{Center for Cosmology and Particle Physics, Department of Physics, New York University, \\4 Washington Place, New York, NY 10003, USA}
...
%\affiliation[c]{\ThisIsTheAffiliation}
...
%\affiliation[\mbox{\fontsize{7}{8.4}{$F$}}]{Physique Mathematique des Interactions Fondamentales, Universite Libre de Bruxelles and International Solvay Institutes, Campus Plaine -- CP 231, 1050 Bruxelles, Belgium}
%\affiliation[\mbox{\fontsize{7}{8.4}{$T$}}]{Institute for Advanced Study, 1 Einstein Drive, Princeton, NJ 08540, USA}
...
As we assume that the quantum correction is small, $\ell \ll \ell_3$, we may first consider the small $\ell$ limit of $C_V$.  Keep in mind that $G_4\sim2\ell G_3$, so the prefactor of \eqref{CV} is of order 1. For  the first factor of the integrand in \eqref{CV},  we claim that in the small $\ell$ limit, the following identity holds
 ...
 To prove this fact, we note that when
    ...
    \be\ba
     ...
     &\sim -\frac{l^2 f(z)}{(\ell+z)^2}\bigg\vert_{0}^{x_{1} r }-\frac{\ell^{2} f'(z) }{\ell+z}\bigg\vert_{0}^{x_{1} r}+\int_{0}^{x_{1}r} \frac{\ell^2 f''(z)dz}{\ell+z}. 
     \ea
     \ee
    In the last step we have used the integration by parts and ignored the $dr$ part of the integration. We then find
    \be I\sim f(0)+\ell f'(0)+\(-2\ell^2 (x_{1} r)^2+l^2 \log{(\ell+z)}|_{0}^{x_{1} r}\), \ee
    where we have used the large-$z$ behavior of $f(z)$, \eqref{largezf}. We see that although there are two divergent terms,  one being quadratically divergent and the other being  logarithmically divergent,  both of them are of order $\ell^2$,  whose effects will be ignored in this work.  Also it can be directly checked that the functional $\mathcal V$ satisfies
