%\renewcommand{\arraystretch}{0.9}
\begin{table}%[t!]
\centering
\tabcolsep 4pt
\caption{
Object detector performance on LMO~\cite{lmo}. On this dataset, failures and successes of $\Delta_{S \to F}$ and $\Delta_{F \to S}$ are measured in terms of ADD(S). Key: $^\ast$: symmetric object.
}
\label{tab:detection_lmo}
\vspace{-3mm}
% \resizebox{1.0\columnwidth}{!}{
    \begin{tabular}{lcccc}
        \toprule
        Object & AP & $\text{AP}_{50}$ & $\Delta_{S \to F}$ & $\Delta_{F \to S}$ \\
        \midrule
        Ape & 64.9 & 95.8 & 3.3 & 1.5\\
        Can & 82.8 & 99.3 & 1.9 & 0 \\
        Cat & 69.6 & 90.8 & 1.5 & 0.1 \\
        Driller & 80.0 & 98.7 & 0.8 & 0.4 \\
        Duck & 77.9 & 98.2 & 0.7 & 2.4 \\
        Eggbox$^\ast$ & 58.8 & 85.9 & 4.3 & 24.8 \\
        Glue$^\ast$ & 55.1 & 87.3 & 5.1 & 2.3\\
        Holepuncher & 82.3 & 99.5 & 2.0 & 1.4\\
        \midrule
        Average & 71.4 & 94.4 & 1.2 & 2.7 \\
        \bottomrule
    \end{tabular}
% }
\end{table}
%\renewcommand{\arraystretch}{1}
