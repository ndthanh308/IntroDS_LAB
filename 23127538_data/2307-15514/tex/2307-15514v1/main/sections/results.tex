...
contains RGBD images of real scenes with different configurations of everyday objects placed on a table.
...
Differently, we only use the Photo Realistic Rendering (PBR) set of 50K synthetic images provided by the BOP challenge~\cite{bop-challenge} as it contains a large variety of pose configurations.
...
Objects have similar geometry (boxes or cans) and are placed in various poses (i.e.~some objects are placed on top of others).
...
For training, we choose 4K synthetic, 4K real, and 4K PBR data provided by the BOP challenge~\cite{bop-challenge} because we found that using only the PBR data gives unsatisfactory results.
As for LMO, we adopt an hole filling algorithm~\cite{hole_filling} on both train and test depth images as done in~\cite{pvn3d}.
...
The input pair $(O, S)$ is first sampled to $V_O=4096$ and $V_S=50000$ points, respectively, and then quantised with a step of $Q=2$mm.
As feature extractor we use a MinkUNet34 \cite{minkowski} with output dimension of $F=32$.
The correspondence estimation threshold used for the positive mining is set to $\tau_P=4$mm, and the maximum number of correspondences extracted is set to 1000 to reduce computational costs.
As depicted in Fig.~\ref{fig:loss_detail}, the safety threshold $\tau_{NO}$ is defined proportionally to the object $O$ diameter by setting $t_{\text{scale}}=0.1$.
The hardest negative mining on $\mathcal{X}_O$ is performed in $\widetilde{\mathcal{N}}_i$.
When mining the hardest negatives on $\mathcal{X}_S$, instead of considering the full negative candidates set $\widetilde{\mathcal{N}}_j$ we randomly sample 10000 points from it to reduce the spatial complexity required by the distance matrix.
HC loss margins are set as $\mu_P=0.1$, $\mu_N=10.0$, and coefficients are set to $\lambda_P = 1$, $\lambda_{NO} = 0.6$, and $\lambda_{NS} = 0.4$.
The feature extractor is trained on 50K PBR images for 12 epochs.
...
We reduce the number of points of the scene to $V_S=20000$, as we found that this value works on par with the original $V_S=50000$ of LMO.
We believe this happens because YCBV objects are less occluded and their geometries are less complex than LMO objects.
As feature extractor we use a MinkUNet50 model \cite{minkowski}, trained on 12K mixed images for 110 epochs.
...
Unlike ADD(S)-0.1d, where the success threshold is relative to the object diameter, in ADD-S AUC the success thresholds are absolute.
...
For Ape, Can and Glue (the ones most negatively affected by detections), this percentage is 3.3\%, 1.7\%, and 5.1\%, respectively. 
...
However, our model fails in case of partial objects with ambiguities (the duck in the second image), or of atypical occlusions (the eggbox in the second image).
This type of occlusion is not present in the training set.

Fig.~\ref{fig:ycbv_qualitative} shows some examples of successes and failures on the test set of YCBV. FCGF6D appears prone to rotation errors (the large clamps can in the first image), especially in the case of partially occluded objects (the bleach cleanser in the second image). However, the poses are generally accurate.
...
We conduct an ablation study on the LMO dataset by training the Drill object for five epochs.
We choose the closest setting to FCGF as baseline: 
no safety threshold in the loss; 
shared network weights; 
no RGB information; 
SGD optimiser with $\text{lr}_{\text{init}}=1e^{-3}$;
...
RRE and RTE show how the two pose components (rotation and translation) are ffected.
...
We set the inlier distance threshold $\tau_1$ = 5 voxels, and the inlier recall ratio $\tau_2$ = 5\%.
...
Using independent weights and applying random erasing on the scene are important too, contributing +10.9 and +10.2, respectively.
...
The training time takes about one week for each dataset using 2$\times$ Nvidia A40 GPUs.
Tab.~\ref{tab:times} reports the comparison of the number of parameters, inference GPU memory footprint, and inference time (using a GeForce RTX 3050) on YCBV. 
We were unable to test E2EK \cite{e2ek} as the code is unavailable, but we used the authors' original code for the other papers.
\ourmethod has a significantly smaller memory footprint than the main competitors, and the inference time is comparable. 
In a scenario where multiple objects are expected, our closest competitor~\cite{geometricaware6d} uses a single model for each object, thereby requiring more memory. 
Our method requires less memory because we train a single model with multiple objects.
...
Time is for a single image, and includes network inference (inf.) and registration (reg.) times.}
\vspace{-3mm}
