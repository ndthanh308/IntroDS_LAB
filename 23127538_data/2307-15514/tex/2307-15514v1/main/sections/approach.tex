...
The quantisation procedure splits the volume occupied by $\mathcal{X}$ in a regular grid of voxels of size $Q$ and assigns a single representative vertex $\textbf{x}_i \in \mathbb{R}^3$ to each voxel $i$.
This reduction is typically computed with random sampling or by average pooling (barycenter)~\cite{minkowski}.
The resulting sparse tensor representation is obtained by discarding voxels corresponding to a portion of the empty space and is significantly more efficient in terms of memory utilisation.
...
The hardest contrastive (HC) loss is defined as $\ell_\text{HC} = \lambda_P \ell_P + \lambda_N \ell_N$, where $\ell_P$ promotes similarity between features of positive samples, $\ell_N$ promotes dissimilarity between features of negative samples, and $\lambda_P, \lambda_N$ are hyperparameters.

% Positive mininig
Given a pair of 3D scenes $( \mathcal{X}_1, \mathcal{X}_2 )$ as inputs, the set of positive pairs is defined as $\mathcal{P} = \{ (i, j): \textbf{x}_i \in \mathcal{X}_1, \textbf{x}_j \in \mathcal{X}_2, \phi(\textbf{x}_i) = \textbf{x}_j \}$, where $\phi \colon \mathcal{X}_1 \to \mathcal{X}_2$ is a correspondence mapping between $\mathcal{X}_1$ and $\mathcal{X}_2$ voxels.
% Positive term
$\ell_P$ is defined as
%+++++++++++++++++++++++++++++++++++++++
...
Computing $\mathcal{N}_i, \mathcal{N}_j$ scales quadratically with the minibatch size, for this reason random subsets of $\mathcal{N}_i$ and $\mathcal{N}_j$ with fixed cardinalities are used in practice.
% Negative term
$\ell_N$ is defined as
...
% A major difference between our setting and the one addressed by FCGF concerns input data.
...
Therefore, their input data 
% have homogeneous representations and 
belongs to the same distribution, i.e.~real-world data captured with the same LiDAR sensor.
This is why authors in \cite{FCGF} use a Siamese approach.
Unlike FCGF, our input data is heterogeneous, therefore we process it with two independent deep networks.

Formally, given an object $O$ and a scene $S$, the input of our pipeline is the pair $(\mathcal{M}_O, \mathcal{I}_S)$, where $\mathcal{M}_O$ is a textured 3D model of $O$ and $\mathcal{I}_S$ is a RGBD capture of $S$ from a viewpoint.

We transform $(\mathcal{M}_O, \mathcal{I}_S)$ into a pair of point clouds.
For $O$, we produce a point cloud $\mathcal{X}_O \in \mathbb{R}^{V_O\times6}$ by sampling $V_O$ vertices on the triangular faces of $\mathcal{M}_O$ and extracting the corresponding RGB colors from its texture.
...
We quantise $\mathcal{X}_O$ and $\mathcal{X}_S$ by a factor $Q$ to process the data with two independent networks implemented with Minkowski engine~\cite{minkowski}.
...
Instead, we found that this addition significantly improves the performance. 
This is particularly beneficial in the case of symmetric objects, as the object pose can be predicted correctly only by relying on its texture patterns and colours.
...
Because of occlusions with other objects and/or self-occlusions, some of the correspondences may be spurious, e.g.~associating points of different surfaces.
Therefore, we consider a correspondence valid if the distance between $\tilde{\textbf{x}}_i \in \widetilde{\mathcal{X}}_O$ and $\textbf{x}_j \in \mathcal{X}_S$ is less than a threshold $\tau_P$ and if the other points on the scene are farther away, i.e. $(i, j) \in \mathcal{P} \text{ iff } \lVert \tilde{\textbf{x}}_i - \textbf{x}_j \rVert < \tau_P \text{ and } \lVert \tilde{\textbf{x}}_i - \textbf{x}_j \rVert < \lVert \tilde{\textbf{x}}_i - \textbf{x}_k \rVert $ for every $k = 1, \dots, V_S$.
...
In this way, we are able of mantaining a good quantity of negatives while avoiding the mining of spurious hardest negatives. 
We also experimented with using different thresholds for the object and the scene points clouds but this solution underperformed our final choice.
...
Avoiding these augmentations helps the deep network 
% $\boldsymbol{\Phi}_O$ 
in learning specialised features for each object. Therefore, our data augmentations are significantly different from those of FCGF and consist of:

\setlist{nolistsep}
\begin{itemize}
\item Point re-sampling of $O$ and $S$, i.e.~unlike FCGF, we randomly downsample point clouds at each epoch to mitigate overfitting. This allows the model to be more robust to depth acquisition noise.
%
\item Colour jittering on $O$, i.e.~we randomly perturb brightness, contrast, saturation, and hue of $O$.
This is particularly effective to make the model robust to different lighting conditions and shadows cast by other objects.
%
\item Random erasing on $S$, i.e.~unlike FCGF, we simulate occlusions at training time. For each point of $\widetilde{\mathcal{X}}_O$ we compute its nearest neighbour in $\mathcal{X}_S$ and randomly select a point on $\mathcal{X}_S$ within such correspondence set. We then erase all the points that fall within a distance threshold $\rho$ from it. This allows the model to be more robust to occlusions in the input scene.
\end{itemize}
...
FCGF uses an SGD optimiser with an initial learning rate $\text{lr}_{\text{init}}=1e^{-1}$ decreased during training with an exponential scheduler with $\gamma = 0.99$.
In our setting, these hyperaparameters do not lead to convergence.
Instead, in our baseline we set $\text{lr}_{\text{init}}=1e^{-3}$.
We experiment with Adam~\cite{adam} and AdamW~\cite{adamw}, and notice improvements in both cases.
We also switch to a Cosine Annealing scheduler~\cite{cosine} that lowers the learning rate from $1e^{-3}$ to $1e^{-4}$ across the epochs.
