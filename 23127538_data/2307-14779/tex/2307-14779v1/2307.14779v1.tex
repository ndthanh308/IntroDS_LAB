...
Partial regularity for parabolic systems using caloric approximations has been studied in \cite{DuzMin05,BoDuzMin10_1,BoDuzMin10_2,DuzMinSte11,BoDuzMin13,Scheven11}. In particular, in \cite{BoDuzMin13} B\"ogelein, Duzaar and Mingione obtained the $\A$-caloric and $p$-caloric approximations, and using these proved partial H\"older continuity of the gradient of weak solutions to the degenerate parabolic system \eqref{system1} with standard $p$-growth.
...
\textit{Overview of the paper.}
 In Section~\ref{Sec2}, we fix the basic notation and collect some definitions and results on Orlicz functions. In Section~\ref{Sec:caloricapproximation}, we prove the $\mathcal{A}$-caloric approximation and recall the $\phi$-caloric one. In Section~\ref{sec3}, we obtain the Caccioppoli inequality and the higher integrability. In Sections~\ref{Sec:nondegenerate} and ~\ref{Sec:degenerate}, we consider the nondegenerate and degenerate regimes, respectively. 
...
\end{equation}
holds true. From now on we always assume that $\phi$ and $\phi^*$ satisfy the $\Delta_2$ condition and  this is denoted by $\Delta(\phi,\phi^*)<\infty$, where $\Delta(\phi,\phi^*)$ denotes the relevant constants $K$.  We note that the exact value of $\phi^*$ is not always explicitly computable and instead the estimate
\begin{equation}
...
$$ 
 $L^\phi(\Omega,\R^N)$ and $W^{1,\phi}(\Omega,\R^N)$ are endowed with the usual Luxembourg type norms. Then they are reflexive Banach spaces. Moreover, for an interval $I$ in $\R$, the parabolic space
$L^\phi(I;W^{1,\phi}(\Omega,\R^N))$ or $L^\phi(I;W^{1,\phi}_0(\Omega,\R^N))$ denotes the set of all functions $f:\Omega\times I\to \R^N$ such that $f(\cdot,t)\in W^{1,\phi}(\Omega,\R^N)$ or $f(\cdot,t)\in W^{1,\phi}_0(\Omega,\R^N)$  for a.e. $t\in I$ and 
$$
...
We recall Jensen type inequality in \cite[Lemma 2.2]{HasOk21}.
...
\[
\psi\bigg( \frac{1}{L^2} \fint_U |f|\, dz\bigg) \le \fint_U \psi(|f|)\, dz.
\] 
\end{lemma}
...
We note that the assumption (i) implies the following monotonicity
...
$$%\begin{equation}
-\int_{\Omega_T} \bfu \cdot \bfzeta_t \,\d z + \int_{\Omega_T}   \bA(D\bfu) : D\bfzeta \, \d z =0 
\quad  \text{for all }\ \bfzeta\in C^\infty_{\mathrm c}(\Omega_T,\R^N)\,,
$$%\label{eq:weakformul}
%\end{equation}
...
$$
is called an \textit{$\A$-caloric map}. By standard regularity theory, see for instance \cite{Cam66}, $\bfv \in C^\infty(Q_r,\R^N)$, and in particular we have the following Lipschitz estimate and excess decay estimate, which will be used in Section~\ref{Sec:nondegenerate}.
...
\int_{Q_r} \psi(|D\bfw |)\,dz \le c \int_{Q_r} \psi(|\mathbf G |) \,dz\,,
\end{equation}
...
$$\begin{aligned}
 \int_{Q_r}\psi (|D \bfw |)\,dz   \le   \sup_{{\bf G}\in L^{\psi^*}\cap C^{\infty}(Q_r,\R^{Nn})}  \left( \int_{Q_r} \bfw \cdot ( \bfv_{{\bf G}} )_t -\langle \A D\bfw ,  D\bfv_G \rangle       - \psi^*( | {\bf G} | ) \, \d z \right),
\end{aligned}$$
...
\bfv_t = \div {\bf G} \quad \text{in}\ \ Q_r , \qquad \bfv=0 \quad \text{on }\ \partial_{\mathrm p} Q_r,
\end{equation}
...
\frac{1}{|Q_r|}\left|  \int_{Q_r} \bfu \cdot \bm\zeta_t - \langle\mathcal A D\bfu , D\bm\zeta\rangle \,dz - \left[\int_{B_r} \bfu \cdot \bm\zeta_t \, \d x\right]^{t=r^2}_{t=-r^2}\right| \le \delta \mu  \|D\bm\zeta\|_{L^\infty(Q_r,\R^{Nn})},
\end{equation}
...
$$
\begin{cases}
\partial_t \bfh- \div (\A D\bfh) = 0  \quad \text{in}\ \ Q_r, \\
...
can be obtained by scaling argument with the functions $\tilde \bfu = \mu^{-1}\bfu$, $\tilde {\bf H} = \mu^{-1} {\bf H}$ and $\tilde\psi(\tau)=\frac{\psi(\mu \tau)}{\psi(\mu)}$.
...
Let $\bfu$ be a weak solution to \eqref{system1}. We always assume that $\phi$ and $\bA$ satisfies Assumption~\ref{Ass}. Let  ${\bm \ell}: \R^n \to \R^N$ be any fixed linear map of the form
...
There exists $\sigma=\sigma(n,N,p,q,L,\nu)>0$ such that $\phi_{|D{\bm \ell}|}(|D\bfu_{\bm\ell}|)\in L^{1+\sigma}_{loc}(\Omega_T)$ with the following estimate: for any $Q_{4\rho}\Subset\Omega_T$,
\[
...
Let $\bfu$ be a 
local weak solution to \eqref{system1}.
There exists $\sigma=\sigma(n,N,p,q,L,\nu)>0$ such that  if  $Q^\lambda_{4\rho}\Subset\Omega_T$ and 
\begin{equation}
\fint_{Q^\lambda_{4\rho}} \phi (|D\bfu|) \, \d z \le \phi(\lambda)
\quad\text{and}\quad
|D\bm\ell|\le \lambda,
\label{eq:assumpt}
\end{equation}
...
$$
\partial_t  \tilde \bfu - \mathrm{div}\, \tilde{\bA}(D\tilde \bfu) = {\bf 0} \quad \text{in }\ Q_{4\rho}\,.
$$
...
$$
\fint_{Q_{4\rho}} \tilde \phi (|D\tilde \bfu|) \, \d z \le 1
\quad \text{and}\quad
|D\tilde {\bm\ell}| \le  1\,,
$$
whence, taking into account \eqref{eq:approx},  %by Theorem~\ref{thm:high},
$$
\fint_{Q_{4\rho}} \tilde \phi_{{|D \tilde{\bm \ell}|}} (|D\tilde \bfu_{\tilde {\bm\ell}}|) \, \d z \le c \fint_{Q_{4\rho}} \tilde \phi (|D\tilde \bfu|+ |D \tilde {\bm\ell}|) \,\d z \le c \,.
$$
...
\end{aligned}\end{equation}
where $\bm\ell$ and $\bfu_{\bm\ell}$ are from \eqref{def_l} and \eqref{def_ul} and 
...
If $z\in S_1$, using \eqref{DE08lem20} and the fact that $ |D\bfu|+|D{\bm\ell}|\le |D\bfu_{\bm\ell}| + 2|D{\bm\ell}| \le 5 |D\bfu_{\bm\ell}|$,
$$\begin{aligned}
...
and 
$$\begin{aligned}
\frac{|D\bfu_{\bm\ell}(z)|^2}{|D{\bm\ell}|^2} &\le \frac{\phi'(|\bfu_{\bm\ell}(z)|+|D{\bm\ell}|)}{\phi'(|D{\bm\ell}|)|D{\bm\ell}|}\frac{|D\bfu_{\bm\ell}(z)|^2}{|D{\bm\ell}|} \\
...
\end{aligned}$$
Then using these estimates and the fact that $\omega(\cdot)\le 1$ and applying H\"older's inequality and Jensen's inequality to the concave function $\tau\mapsto\omega(\tau^{1/2})$, we have
\begin{equation}
\begin{aligned}
...
&\le c \phi'(|D{\bm\ell}|) \left[  \fint_{Q^\lambda_r}  \frac{|D\bfu_{\bm\ell}(z)|^2}{|D{\bm\ell}|^2} \, \d z \right]^{\frac{1}{2}}  \left[  \fint_{Q^\lambda_r} \left(\frac{|D\bfu_{\bm\ell}(z)|}{|D{\bm\ell}|}\right)^{2\gamma}\, \d z\right]^{\frac{1}{2}}\\
...
Now, we derive an excess decay estimate in the non-degenerate regime.

\begin{lemma} \label{Lem:nondegenerate1}
Let $Q_{2r}^\lambda = Q_{2r}^\lambda(z_0)\Subset \Omega_T$, $\beta\in(0,1)$, and $\bfu$ be a weak solution to \eqref{system1}.
Suppose that
\begin{equation}\label{Lem:nondegenerate1_Ass1}
\frac{\lambda}{2K}\le |(D\bfu)^\lambda_{2r}| \le 2K \lambda
\end{equation}
...
$$
\fint_{Q^\lambda_{2r}} \phi_{|D\bm\ell|} (|D\bfu_{\bm\ell}|) \, \d z =\fint_{Q^\lambda_{2r}} \phi_{|(D\bfu)_{r}^\lambda|} (|D\bfu - (D\bfu)_{r}^\lambda|) \, \d z \,.
$$
We divide the proof into three steps.

\vspace{0.3cm}

\textit{Step 1. (Scaling)} We first observe from \eqref{eq:approx}, \eqref{Lem:nondegenerate1_Ass1} and \eqref{Lem:nondegenerate1_Ass2} that
$$
...
%\end{aligned}$$
Now, we consider the following scaled functions: 
$$
\tilde \bfu(x,t) :=\frac{1}{\lambda} \bfu(x, {t}\lambda^2/{\phi(\lambda)}), 
\quad \tilde{\bA}({\bf P}) := \frac{\lambda \bA(\lambda {\bf P} )}{\phi(\lambda)}, 
...
\begin{equation*}
\tphi_{\frac{a}{\lambda}}(t) = \frac{\phi_{a}(\lambda t)}{\lambda \phi'(\lambda)}  \quad \mbox{ and } \quad \tilde{\phi}_{\frac{a}{\lambda}}'(t) = \frac{ {\phi}_{{a}}'(\lambda t)}{\phi'(\lambda)}\,,
...
Moreover, by Corollary~\ref{cor:high} and the estimate \eqref{Acalori_pf1} we also have that
...
$$
\partial_t \tilde \bfu_{\bm\ell} = \partial_t \tilde \bfu  = \div \tilde{\bA}(D\tilde \bfu) =: \div \, {\bf H} \quad \text{in } \ Q_r\,,
$$
i.e., ${\bf H}:=\tilde{\bA}(D\tilde \bfu)$, in the distributional sense, and writing $\tilde{\phi}_{|D\tilde {\bm\ell}|}^*:=(\tilde{\phi}_{|D\tilde {\bm\ell}|})^*$
\begin{equation}
\fint_{Q_r} \tilde{\phi}_{|D\tilde {\bm\ell}|}^*(|{\bf H}|)\, \d z =  \fint_{Q_r} \tilde{\phi}_{|D\tilde {\bm\ell}|}^*(|\tilde{\bA}(D\tilde \bfu)|)\, \d z \le c \fint_{Q_r} \tilde{\phi}_{|D\tilde {\bm\ell}|}(|D\tilde \bfu|)\, \d z \le c\mu^2\,.
\label{Duphideltabis}
\end{equation} 
Set 
$$
...
\end{equation}
Therefore, by H\"older's inequality, the Jensen inequality in Lemma~\ref{lem:Jensen} with $\psi^{-1}(t):=\tilde\phi_{|D\tilde {\bm\ell}|}(t)^{\frac{1}{q}}$  and the estimates \eqref{Duphisigma}, \eqref{comparisonp0}, \eqref{Dhphisigma} and \eqref{gammaequiv}, we have that  with  $\kappa_0\in(0,1)$ satisfying $\frac{\kappa_0}{q}+(1-\kappa_0)(1+\sigma_0)=1$,
...
$$
\tilde\phi_{|D \tilde{\bm\ell}|}(\theta |(D\bfh)_{r/4}|) \sim \frac{\phi'(|(D\tilde \bfu)_{r}| + \theta |(D \bfh)_{r/4}|)}{|(D\tilde \bfu)_{r}| + \theta |(D \bfh)_{r/4}|} \theta^2 |(D \bfh)_{r/4}|^2 \sim    \theta^2 |(D\bfh)_{r/4}|^2 \lesssim  \theta^2\mu^2\,,
$$
...
\end{aligned}$$
Finally, choosing $\theta$ small so that $c \theta^{1-\beta}\le \frac{1}{2}$ and then $\epsilon$ small so that $c\theta^{-(n+2)} \epsilon^{\frac{p\kappa_0}{p_0q}}\le \frac{1}{2}\theta^{\beta+1}$, we obtain 
$$
\fint_{Q_{\theta r}} \tilde\phi_{|(D\tilde \bfu)_{\theta r}|}(|D\tilde\bfu-(D\tilde \bfu)_{\theta r}|)|)\, \d z  \le \theta^{1+\beta}\mu^2\,,
$$
whence, by scaling back,
$$
\fint_{Q_{\theta r}} \phi_{|(D\bfu)_{\theta r}^\lambda|}(|D\bfu-(D \bfu)^\lambda_{\theta r}|)\, \d z \le \theta^{1+\beta} \lambda \phi'(\lambda) \mu^2 \,.
$$ 
...
Let $\lambda>0$, $\beta\in(0,1)$, $Q_{2R}^\lambda = Q_{2R}^\lambda(z_0)\Subset \Omega_T$ and $\bfu$ be a weak solution to \eqref{system1}.
Suppose
\begin{equation*}\label{Lem:nondegenerate2_ass1}
\frac{\lambda}{K_0}\le |(D\bfu)^\lambda_{2R}| \le K_0 \lambda
\end{equation*}
...
\end{aligned}\end{equation}
Since  $\frac{K_0}{2} \le |(D\bfu)^\lambda_{\theta^{j-1}R}| \le 2K_0$, applying Lemma~\ref{Lem:nondegenerate1} with $r=\theta^{j-1}R$ we obtain 
$$ \begin{aligned}
...
\leq  \frac{q{\tilde \sigma}^2 \lambda_{i}}{\varphi'(\lambda_{i})\nu^{q-2}} r_i^2
 ...
 = \left(\frac{\sigma}{2}\right)^2 \frac{r_i^2\lambda_{i}^2}{\varphi(\lambda_{i})} 
...
Finally, we are left to prove the first inequality in \eqref{aveDulowerbound}. Note that, since in case (iii) $r_s=0$, there is nothing to prove. Then we consider  the cases (i) and (ii) where  $r_s>0$.
...
\begin{equation*}
\begin{split}
\fint_{Q^\lambda_{r/2}} \phi_{|D\bfh|}^{1+\sigma}(|D\bfu-D\bfh|)\,\mathrm{d}z & \leq c\left(c_\eta \fint_{Q^\lambda_{r/2}} \phi^{1+\sigma}(|D\bfu-D\bfh|)\,\mathrm{d}z + c\eta \phi^{1+\sigma}(|D\bfh|)\right) \\
& \leq \tilde{c}\left(\fint_{Q^\lambda_{r/2}} \phi^{1+\sigma}(|D\bfu|)\,\mathrm{d}z + \phi^{1+\sigma}(|D\bfh|)\right)\,,
\end{split}
\end{equation*}
whence
\begin{equation*}
\begin{split}
\left(\fint_{Q^\lambda_{r/2}} \phi_{|D\bfh|}^{1+\sigma}(|D\bfu-D\bfh|)\,\mathrm{d}z\right)^\frac{1}{1+\sigma}  \leq {c}\left(\left(\fint_{Q^\lambda_{r/2}} \phi^{1+\sigma}(|D\bfu|)\,\mathrm{d}z\right)^\frac{1}{1+\sigma} + \phi(|D\bfh|)\right)\,.
\end{split}
\end{equation*}
...
\textit{Step 2.} Let $0< \theta \le 1$. Applying Lemma~\ref{Lem:phiLaplacesystem} with $R=r/2$ and writing $\lambda_{\theta}:=\lambda_{\theta r}$ for each $\theta\in(0,1]$ and $\theta_s:=\frac{2r_s}{r}$, we have  that 
\begin{equation}\label{Lem:degenerate_pf1}
\lambda_\theta = \lambda_{\theta_s}  \  \ \text{if}\ \ \theta \in(0,\theta_s ]
\quad\text{and}\quad 
\theta^{\alpha_1}\lambda 
\le  \lambda_\theta
\le 2\theta^{\alpha_1}\lambda \  \ \text{if}\ \ \theta\in(\theta_s,1]\,,   
\end{equation}
...
& \le  c \frac{\phi(\lambda_1)\lambda_\theta^2 \vartheta^{n+2}}{\phi(\lambda_\theta)\lambda_1^2\theta^{n+2}}\theta^{-(n+2)-(q-2)\alpha_1}\fint_{Q^{\lambda}_{r}} |\bV(D\bfu)- \bV(D\bfh)|^{2}  \, \dz +  c \sup_{Q^{\lambda_\theta}_{\theta r}} \phi(|D \bfh|) \,.
\end{aligned}$$
...
& \le  c C_0^{q-2+\frac{(2-p)(2n+4+(q-2)\alpha_1)}{2}}\vartheta^{-(n+2)-(q-2)\alpha_1}\fint_{Q^{\lambda}_{r}} |\bV(D\bfu)- \bV(D\bfh)|^{2}  \, \dz \\
& \le  c C_0^{q-2+\frac{(2-p)(2n+4+(q-2)\alpha_1)}{2}-p}\vartheta^{-(n+2)-(q-1)\alpha_1}  \epsilon^{\frac{1-\tau}{2}} \phi(\lambda_1)\,.
\end{aligned}$$
...
&\le  c C_0^{q-2+\frac{(2-p)(2n+4+(q-2)\alpha_1)}{2}-p}\vartheta^{-(n+2)-(q-1)\alpha_1}  \epsilon^{\frac{1-\tau}{2}} \phi(\lambda_1)+  c C_0^{-p} \phi(\lambda_1) \le  \phi(\lambda_1)\,,
\end{aligned}$$
...
%provided $C_0$ large and then $\epsilon=\epsilon(\theta)$ small, which together with the monotonicity of $\phi$ implies the second estimate in  \eqref{Lem:degenerate_decay}. 
 ...
 If $\theta \in [\theta_s , 1)$, by \eqref{Lem:degenerate_pf1} and \eqref{Lem:degenerate_pf2}, we have that for each $\alpha\in(0,\alpha_1)$
$$
...
$$
where we  chose $\vartheta$  small to satisfy the last inequality. Therefore, we obtain \eqref{Lem:degenerate_lambda1-1} without considering \eqref{Lem:degenerate_ass4}.
...
%& \le c \vartheta^{-\kappa_1} \epsilon^{\kappa_2} \lambda_1 +c \left(\frac{\theta}{\theta_s}\right)^{\alpha_1} \lambda_1\,,
%\end{aligned}$$
where $\kappa_1= -n-2-\alpha_1(q-2)-\alpha_1$ and $\kappa_2=\frac{1-\tau}{4q} $. Then using \eqref{Lem:degenerate_pf5}  and choosing $\epsilon=\epsilon(\vartheta)$ sufficiently small we have 
$$\begin{aligned}
...
$$ \begin{aligned}
|(D\bfu)^{\lambda_1}_{\vartheta r}|  & \le \chi_1^{-1} \fint_{Q^{\lambda_1}_{\vartheta r}} \phi_{|(D \bfu)^{\lambda_1}_{\vartheta r}|} (|D \bfu-(D \bfu)^{\lambda_1}_{\vartheta r}|) \, \dz 
...
which implies that
$$\begin{aligned}
& \left(\frac{1}{2C_0C_s}  - c \left(\frac{\theta}{\theta_s}\right)^{\alpha_1} \right)\lambda_1  \le c \chi_1^{-q}\left(\frac{\theta}{\theta_{s}}\right)^{\frac{3}{4q}} \lambda_\theta  \le c \chi_1^{-q} \left(\frac{\theta}{\theta_s}\right)^{\alpha_1}\lambda_1,
\end{aligned}$$
hence
\begin{equation}\label{thetastheta}
\theta_s^{\alpha_1} \le c \chi_1^{-q} \theta^{\alpha_1} \,.
\end{equation}
...
$$
\lambda_1  = C_0\lambda_\theta = 2 C_0 \theta_s^{\alpha_1}\lambda \le c \chi_1^{-q}  \theta^{\alpha_1}\lambda  \le c \chi_1^{-q} \vartheta^{\alpha_1-\alpha}\vartheta^\alpha \lambda \le \vartheta^\alpha \lambda\,,
$$
...
\end{equation}
If this condition holds, then by Lemma~\ref{Lem:degenerate} with $\lambda_j= \lambda$ there exists $\lambda_{j+1}\in[\vartheta^{\alpha_1}\lambda_j, C_1\lambda_j]$  such that $Q^{\lambda_{j+1}}_{\vartheta^{j+1}R} \subset Q^{\lambda_j}_{\frac{\sigma}{2}\vartheta^j R}$,
...
\end{equation}
and the other case. Set $\widetilde{\mathbb N}:=\{j\in \mathbb N_0: \eqref{induction2}\ \text{with $j$ does not holds.}\}$ and 
...
\end{aligned}\end{equation}
Therefore, we can inductively apply Lemma~\ref{Lem:degenerate} with $r=\theta^jR$ for $j=0,1,\dots,m-1$. 
...
Moreover, since 
$$
\frac{r}{R} \ge \vartheta^{j+1} \min \left\{ 1, (\phi(\lambda_{j+1})/\lambda_{j+1}^2)^{-1/2} \right\} \ge  \vartheta^{j+1} \lambda_{j+1}^{\frac{2-p}{2}} \ge c \lambda_j^{\frac{1}{\alpha_1}+\frac{2-p}{2}}
$$ 
by   \eqref{Lem:iteration1_result2}  we have 
$$
|(D\bfu)_{r}|   \le c \left(\frac{r}{R}\right)^{\frac{1}{q}(\frac{n+2}{2}p-n)/(\frac{1}{\alpha_1}+\frac{2-p}{2})} \longrightarrow 0 \quad \text{as }\ r\to 0 \,,
$$
hence 
$$
...
$$
which proves \eqref{Lem:iteration1_result5} with $m=\infty$.
\end{proof}
...
J. Ok was supported by the National Research Foundation of Korea by the Korean Government (NRF-2022R1C1C1004523) and by GNAMPA visiting program. G. Scilla has been supported by the Italian Ministry of Education, University and Research through the MIUR – PRIN project 2017BTM7SN “Variational methods for stationary and evolution problems with singularities and interfaces”. The research of B. Stroffolini was supported by PRIN Project 2017TEXA3H “Gradient flows, Optimal Transport and Metric Measure Structures”.
...
{Regularity theory for parabolic systems with Uhlenbeck structure},  arXiv:2302.05649.
