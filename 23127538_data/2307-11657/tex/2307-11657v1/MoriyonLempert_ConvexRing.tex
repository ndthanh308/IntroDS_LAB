...
\begin{problem}
	...
	Suppose $\Omega_0, \Omega_1$ are bounded domains with smooth boundaries in $\ER^n$, with $\overline{\Omega_0}\subset \Omega_1$.  Let $\MR=\Omega_1\backslash\overline{\Omega_0}$. Find $\Phi$ satisfying
	\begin{align}
		\MF(\Phi)=0\ \ \ \ \ \ &\text{ in }\MR,\\
   		\Phi=0\ \ \ \ \ \  &\text{ on }\partial \Omega_0,\\
		\Phi=1\ \ \ \ \ \  &\text{ on }\partial \Omega_1.
	\end{align}
	Here $\MF$ is a partial differential operator.
\end{problem}
...
In \cite{RosayRudin1989Michigan}, they considered the following function 
\begin{align}
...
\begin{problem}
	...
	Suppose $\Omega_0, \Omega_1$ are two strongly \cconvex \ domains with smooth boundaries in $\EC^n$, $n\geq 2$, with $\overline{\Omega_0}\subset \Omega_1$.  Let $\MR=\Omega_1\backslash\overline{\Omega_0}$. Find $\Phi$ satisfying
	\begin{align}
...
\begin{remark} 	The discussion about domains with $C^{1,1}$ boundaries is necessary since the solution $\Phi$ to the Dirichlet problem above may only be a $C^{1,1}$ function; so, its level sets may only have $C^{1,1}$ regularity.
\end{remark}
...
\begin{theorem}
	...
	Suppose $\Phi$ is a solution to Problem \ref{prob:Main_Problem_HCMA_ring}. Let $\MG_{\ijbar} dz^i\otimes \overline{dz^j}$ be the Hermitian metric on $\EC^n$. Then $|\nabla\Phi|_\MG$ has a positive lower bound, and $\sqrt{-1}\ddbar\Phi$ has rank $n-1$ in the weak sense. Let
	\begin{align}
...
\begin{problem}
	...
	\label{prob:Perturbed_Problem_HessianQuotient}  
	Find $\Phi$ satisfying
	\begin{align}
		...
		\Phi&=1\ \ \ \ \ \  &\text{\ \ on }\partial \Omega_1.
	\end{align}
Here $\bbform =\sqrt{-1}\bb_{ij}dz^i\wedge \overline{dz^j}$ is a constant coefficient non-degenerate K\"ahler form on $\EC^n$, and $\eqe$ is a positive constant.
\end{problem}
...
\begin{proof}
	...
	Given $E_1$, we can construct the pluricomplex Green's function $G$ of  $E_1$ with a logarithm singularity at $0\in E_1$ \cite{Lempert84Bulgar}.  Then $G$ is smooth with $\nabla G\neq 0$ everywhere in $E_1\backslash \{0\}$, and all the level sets of $G$ are smooth and \scconvex, according to Lemma 5.3 of \cite{Lempert85_Duke_Symmetries}. 
	...
	\end{align} Moreover, it's also true that
	\begin{align}
	...
	\begin{align}
		...
		\lim_{\tau\rightarrow 0}\frac{|f_\zeta(\tau)|^2}{|\tau|^2}=|f_{\zeta}'(0)|.
	\end{align}
	So 
	$\mu \left\{
	|z|^2\leq \frac{1}{H(0, \zeta)} 
...
978-0-85729-029-8.
   ...
   \bibitem[RR89]{RosayRudin1989Michigan} J. Rosay,  W. Rudin, {\em  A Maximum Principle for Sums of Subharmonic Functions and the Convexity of Level Sets}. Michigan Math. Journal. 36: 95-111. 
