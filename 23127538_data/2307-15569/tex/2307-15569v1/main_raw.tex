...
However, with the current substantial advancements in multi-modal learning~\cite{radford2021learning,wang2023image,girdhar2023imagebind}, we identify novel opportunities to enhance SSRL's effectiveness using contrastive objectives.
...
% Recon utilised text labels during self-supervised training, thereby introducing additional supervisory signals. While it is currently the SOTA, it is surprising that our results are still comparable. Furthermore, it is noteworthy that our model size is only around 15% of theirs.
\begin{table*}[!t]
