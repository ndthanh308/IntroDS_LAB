...
\thanks{The research of Q.W. has been supported by the National Key Research and Development Program of China (Grant No. 2020YFA0713200) and the NSFC (Grant No. 11771085). The research of Z.M. and M.Y. has been supported by NSF grants DMS-2131243 and DMS--2200762.}
...
\begin{abstract} Discriminant ideals of noncommutative algebras $A$, which are module finite over a central sublagebra $C$,
...
\[
D_n(A/C, \tr) := \big( \det [ \tr(a_i a_j) ]_{i,j =1}^n \mid (a_1, \ldots, a_n) \in R^n \big)
\]
and the {\em{modified discriminant ideals}} \cite{CPWZ}
\[
MD_n(A/C, \tr) := \big( \det [ \tr(a_i a'_j) ]_{i,j =1}^n \mid (a_1, \ldots, a_n), (a'_1, \ldots, a'_n) \in R^n \big),
\]
...
\end{equation}
Discriminants were first defined by Dedekind in the case when $A$ is the of ring of integers of an algebraic
...
evaluation: smash product techniques \cite{GKM}, Poisson geometry \cite{NTY1,LY}, cluster algebras \cite{NTY2}, reflexive hulls
...
this assumption is in fact very often violated. Discriminant ideals even carry more information than discriminants in the case when
...
\begin{enumerate}
\item[(1)] $A$ is a {\em{maximal order}} of PI degree $n$ with $C = Z(A)$ or
\item[(2)] $(A,C, \tr)$ is a {\em{Cayley--Hamilton algebra of degree $n$}} in the sense of Procesi \cite{P,DP}, see
...
By \cite[Theorem 0.2(1)]{WZ}, in the setting of Theorem B, $H$ is an {\em{AS-Gorenstein algebra}}, see \deref{AS-Gorenstein}. In this setting,
...
\noindent
{\bf{Notation.}} For a $\kk$-algebra $A$, we will denote by $\modd(A)$ the category of its finite dimensional modules
...
$\tr : H \to C$ such that $(R, C, \tr)$ is a Cayley--Hamilton algebra.
\ede
...
that filter through the quotients $H \to H/ \mm H$ for $\mm \in \MaxSpec C$. Furthermore, the homomorphisms \eqref{Deltaphipsi}
...
$W \otimes V^* \in \modd(H/ \mm_{\ol{\ep}})$. Choose an irreducible module $M' \in \modd(H/ \mm_{\ol{\ep}})$ in the socle
...
\]
for some subset $T \subseteq G_0$. \leref{stab}(ii) implies that $\Stab_{G_0}(V) =T$. Therefore,
\[
...
\gamma (a^i b^j, a^k b^m) = \ep^{jk}, \quad \forall \, i, j, k, m \in \Zset/ \ell,
\end{equation}
where $\ep \in \kk$ is a primitive $\ell$-th root of unity.
The corresponding twisted group algebra is isomorphic to a truncated quantum torus:
\[
\kk_\gamma \Lambda \cong \frac{\kk \langle a, b \rangle}{( a^\ell -1, b^\ell-1, ba - \ep ab )} \cdot
\]
We have the well known isomorphism $\kk_\gamma \Lambda \cong M_\ell(\kk)$, given by
\[
b \mt \diag(1, \ep, \ldots, \ep^{\ell-1}), \quad a \mt (g_{st})_{s,t=1}^\ell,
\; \; g_{st} :=
\begin{cases}
...
\end{enumerate}
\ede
Every Noetherian, finitely generated, PI algebra $A$ over a field $\kk$ is AS-Gorenstein by
\cite[Theorem 0.2(1)]{WZ}. The assumptions in Theorems A-C imply that the Hopf algebras
...
\[
\textstyle \int^l \cong {}^{w_l(\chi)} \kk^1
\]
for some character $\chi$ of $H$, where the right hand side denotes the trivial $H$-bimodule
given by the counit $\ep$, twisted on the left side by the winding automorphism $W_l(\chi)$.
By \cite[Theorem 0.3]{BZ}, the Nakayama automorphism $\nu$ of $H$ equals
\[
\nu = S^2 W_l(\chi).
\]
We now apply \cite[Proposition 4.5(c)]{BZ}, stating that $\nu$ acts trivially on $Z(H)$,
...
\]
Equation \eqref{V-discr} now applies the second statement in Theorem B(d).
\qed
%%%%%%%%%%
...
i.e.,
\[
\Sigma:= \{c^s a^i b^j \mid a^i b^j \in G, c^s \in \Sigma \}
\]
with the product
\[
...
Denote by $\ep \in \kk$ a primitive $\ell$-th root of 1. We have the isomorphism
\[
C \cong \kk[c]/(c^\ell -1),
\]
and thus,
...
\beta_s (a^i b^j, a^k b^m) = \ep^{jks}, \quad \forall \, i, j, k, m \in \Zset/ \ell.
\]
...
\[
H / \mm_s H =  \frac{\kk \langle a, b \rangle}{(a^\ell -1, b^\ell -1, ba - \ep^s ab)} \cong \kk_{\beta_s} \Lambda,
...
\quad \chi_{i,j}(a) = \ep^i, \chi_{i,j}(b) = \ep^j.
\]
...
It is easy to verify that the map
\[
b \mt \diag(1, \ep^s, \ldots, \ep^{(\ell/d-1)s}), \quad a \mt (g_{st})_{s,t=1}^\ell,
\; \; g_{st} :=
\begin{cases}
1, &\mbox{if} \; \; t \equiv s + 1 (\modd \ell/d)
\\
0, &\mbox{otherwise}
\end{cases}
\]
is an irreducible $H / \mm_s H$-module of dimension $\ell/d$. It will be denoted by $V_s$. Another
direct calculation yields
\[
\chi_{i,j} \otimes V_s \cong V_s, \quad \forall i, j \in \Zset/ \ell \; \; \mbox{such that} \; \; d | i, j.
\]
The number of these elements $\chi_{i,j}$ of $G_0$ is $\ell^2/d^2$. \prref{stab}(b) implies that $|\Stab_{G_0}(V_s)| \leq \dim(V_s)^2 = \ell^2/d^2$, and so, 
\[
...
\]
while the zero set of the lowest discriminant ideal of $(H,C, \tr)$ is the whole $\MaxSpec C$.
\subsection{Infinite dimensional group algebras of central extensions} The phenomena from the previous
...
\mbox{where} \quad \mm_{u, v, s} := (x^\ell - u, y^\ell -v, c - \ep^s).
\]
...
\begin{align*}
\widetilde{H} / \mm_{u, v, s} \widetilde{H} &= \frac{\kk \langle x, y \rangle}{(x^\ell -u, y^\ell - v, y x  - \ep^s x y)}
\\
& \cong \frac{\kk \langle a, b \rangle}{(a^\ell -1, b^\ell -1, ba - \ep^s ab)} \cong \kk_{\beta_s} \Lambda,
...
\quad \chi_{i,j}(x) = \ep^i, \chi_{i,j}(y) = \ep^j.
\]
In the notation $d := \gcd (\ell, s)$, the map
\[
y \mt \sqrt[\ell]{v} \diag(1, \ep^s, \ldots, \ep^{(\ell/d-1)s}), \; \;  x \mt (g_{st})_{s,t=1}^\ell,
\; g_{st} :=
\begin{cases}
\sqrt[\ell]{u}, &\mbox{if} \; \; t \equiv s + 1 (\modd \ell/d)
\\
0, &\mbox{otherwise}
\end{cases}
\]
...
generated by the nonnegative Chevalley generators $E_i, K_i^{\pm 1}$,
...
/ \rad \Big( \UU_\ee(\g)^{\geq} / \mm_{\ol{\ep}} \,  \UU_\ee(\g)^{\geq} \Big).
\end{multline*}
This homomorphism is an isomorphism since $\Cset [K_1, \ldots, K_r]/ (K_i^{\ord(\ee)} -1 \mid 1 \leq i \leq r)$ is a semisimple sublagebra of
...
\bth{quant-Borel} Let $\g$ be a finite dimensional complex simple Lie algebra of rank $n$ and $\ee \in \Cset$ be a root of unity of odd order,
...
Assume that $V$ is an irreducible $\UU_\ee(\g)^{\geq}$-module of dimension $>1$. \thref{rep-B} implies that
\[
...
\]
Hence, $|\Stab_{G_0} (V)| < \dim(V)^2$ because $l(w) >1$, and thus, $V$ is not maximally stable.
...
imply that for all $c = (c_1, \ldots, c_n) \in (\Cset^*)^n$,
\[
W_l(\chi_c) \, . \, \mm_{\ol{\ep}}  =
W_l(\chi_c) \, . \, \mm_{\ol{\ep}} =
...
\[
W_l \big( G( (\UU_\ee(\g)^{\geq})^\circ)  \big)  \, . \, \mm_{\ol{\ep}} = W_l \big( G( (\UU_\ee(\g)^{\geq})^\circ)  \big) \, . \, \mm_{\ol{\ep}} = T.
\]
...
\]
So, we have $\OO_\ee(G) \cong \OO_q(G)/(q-\ee)$. 
...
that $\OO_\ee(G)$ is a free module over $\CC_\ee$ of rank $\ell^{\dim G}$. This gives rise to the regular trace function
\[
...
\bth{O-eps} Let $\g$ be a finite dimensional complex simple Lie algebra of rank $n$ and $\ee \in \Cset$ be a root of unity of odd order,
coprime to $3$ if $\g$ is of type $G_2$. The following hold:
\begin{enumerate}
...
Textbooks in Pure and Appl. Math., vol. 94, Marcel Dekker, Inc., New York, 1985.
    ...
    \bibitem{LY} J. Levitt and M. Yakimov, {\em{Quantized Weyl algebras at roots of unity}}, Israel J. Math. {\bf{225}} (2018), 681--719.
