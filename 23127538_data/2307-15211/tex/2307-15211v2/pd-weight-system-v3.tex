...
I am grateful to Zhiyun Cheng and Sergei Lando for very interesting 
discussions and the anonymous referee valuable comments.
...
Sliding the left loop in example (c)\\ along the middle loop gives a different graph\\ on a torus:
...
\medskip
\noindent
{\bf Examples.} Here are the chord diagrams with with at most 3 chords.
...
should be the same in all the four diagrams. We say that a function on chord diagram satisfies the four-term relation if the alternating sum of its values on the four diagrams of \eqref{eq:4T} equals zero.
...
Here is the chord diagram on two circles corresponding to the two-vertex ribbon graph of example (a).\quad $\risS{-35}{cd-rg-ex1}{}{30}{0}{0}$

To perform the partial duality relative to a single edge-ribbon
(that is a chord),\\ 
we need to break the corresponding circles at the end of the chord and reconnect\\ 
them by two arcs parallel to the chord, and then place a new chord connecting\\
 these two arcs.
...
also extended the partial-dual genus polynomial to binary delta-matroids. Consequently, it can be considered as a new polynomial for all ({\it framed} in terminology of \cite{Lan-J}) graphs. It is interesting whether the recurrent formula from the Theorem 5.2. is also valid for all such graphs. Are there any relation of this polynomial with other graph polynomials?
%
%One can try to extend the partial-dual genus polynomial to a polynomial on all ({\it framed} in terminology of \cite{Lan-J}) graphs using this recurrence relation. But for arbitrary such graphs we will need to prove the correctness of this definition. If so, then we have a new polynomial for all (framed) graphs
%
...
\bibitem[Lando00]{Lan} S.~Lando, {\it On a Hopf algebra in graph theory},
  J. of Comb. Theory, Series B, {\bf 80}(1) (2000) 104--121.
	
\bibitem[Lando06]{Lan-J} S.~Lando, {\it $J$-invariants of plane curves and  
  framed chord diagrams},
  Functional Analysis and Its Applications, {\bf 40}(1) (2006) 1--10.	
	
\bibitem[Kar23]{Kar} M.~Karev, {\it On the primitive subspace of Lando 
  framed graph bialgebra}.
	Preprint \verb#	arXiv:2307.00468 [math.CO]#.
