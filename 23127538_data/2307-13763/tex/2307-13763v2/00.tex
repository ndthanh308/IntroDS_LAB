...
\icmltitlerunning{Sobolev Space Regularised Pre Density Models}
...
\twocolumn[
\icmltitle{Sobolev Space Regularised Pre Density Models}
...
As mentioned earlier, the solutions of \eqref{eq:estimate_def1} do not necessarily integrate to 1, and the normalising constant may not be easy to compute. 
This introduces a particular nuance in the context of hyperparameter tuning, as it prevents the utilization of the maximum likelihood measure to establish the optimal parameter. To overcome this  normalization challenge, we apply a score-based method, consisting  of measuring the Fisher Divergence (FD), which uses log-likelihood \emph{gradients} for divergence measurement, thereby eliminating the need for normalization. More specifically, we incorporate the concept of score-matching \citep{hyvarinen2005estimation,song2020sliced,song2019generative}, a technique that has recently garnered renewed interest. 


In addition to the above contributions, we provide two theoretical results. First,  we prove the asymptotic consistency of the SOSREP estimator for the SDO kernels in any fixed dimension $d$, under mild assumptions on the ground truth density generating the $x_i$'s. Second, we present a family of examples in which the SOSREP and the standard Kernel Density Estimator (with the same kernel) provably  arbitrarily differ. Thus, SOSREP is a genuinely new estimator, non equivalent to KDE, and having some different properties. We also show that examples as above occur naturally in real datasets.  Due to space constraints, the discussion of these examples is deferred to the Supplementary Material, Section \ref{sec:two_regions}.


Note that as a result of consistency, it can also be shown that the estimator $(f^*)^2$ becomes normalized, at least asymptotically with $N$ (see Section \ref{sec:consistency_statement}). However, as mentioned above, for finite $N$, computing or even estimating the normalization constant is not straightforward, and is outside the scope of this paper. Instead, we will focus on the Anomaly Detection (AD) applications, that do not require a normalization. 

Indeed, building on the above steps, we show that SOSREP achieves the remarkable performance of scoring \emph{second best} on a recent comprehensive Anomaly Detection benchmark, \cite{han2022adbench} for tabular data, which includes more than 45 datasets and more than 15 specialized AD methods. It is noteworthy that Anomaly Detection is widely regarded as a particularly arduous task, specifically for deep density estimators  \cite{nalisnick2019deep, choi2019waic}, a difficulty that persists even in the presence of substantial data. Furthermore, the task of learning from tabular data has long been recognized as a significant challenge for deep learning-based models \cite{grinsztajn2022treebased}.
...
The Fisher Divergence (FD) is a similarity measure between distributions, which is based on the score function -- the gradient of the log likelihood. In particular, it does not require the normalization of the density. While the link between FD and maximum likelihood estimation has been previously studied in the context of score matching \cite{DBLP:journals/corr/abs-1205-2629}, to the best of our knowledge, this represents the first application of FD for hyperparameter tuning. The divergence between data and a model can be approximated via the methods of \cite{hyvarinen2005estimation}, which have been recently computationally improved in \cite{song2020sliced} in the context of score based generative models, \cite{song2019generative}. In particular, we adapt the Hutchinson trace representation based methods used in \cite{song2020sliced} and \cite{grathwohl2018ffjord} to the case of models of the form \eqref{eq:main_optimization_problem}. Full details are given in the Supplementary Material Section \ref{SEC:FD}. 
...
Consistency of the SOSREP in one dimension was shown in \cite{klonias1984class}, for kernels that coincide with SDO in one dimension. Our results provide $L_2$ consistency of the SOSREP in any fixed dimension $d$, for compactly supported ground truth densities. Our approach generally follows the same lines as that of \cite{klonias1984class}. However, some of the estimates are done differently, since the corresponding arguments in \cite{klonias1984class} were intrinsically one dimensional. 



\section{The SOSREP Desnity Estimator}
\label{sec:INER_density_estimator}
In this Section we describe the general SOSREP Density Estimation Framework, formulated in an abstract Reproducing Kernel Hilbert Space. We first introduce the general optimization problem and discuss a few of its properties. In Section \ref{sec:gradients} we discuss the gradient descent optimization and introduce the natural gradient. 
...
\label{eq:cone_c_prime_definition_main}
    \mcC' = (\mcC^a)' = \Set{ f \in span\Set{k_{x_i}}_{i=1}^N \setsep f(x_i) >0 }.
\end{equation}
Observe also that compared to Section \ref{sec:gradients}, here we require only positivity on the data points $x_i$ rather than on all $x\in \RR^d$, a weaker requirement. The convexity of $L$ still holds, due to similar considerations.  
%\notemk{Perhaps elaborate here, depending on what we decide with non-SDO.} 
In addition, we restrict the cone to the span of $x_i$ since the optimal solutions belong to that span, and our algorithms operate inside the span too. Note that $f\in \mcH^1$ if and only if $f\in \mcH^a$ for every $a>0$. With these remarks in mind, the consistency result is as follows: 
...
ADbench, \cite{han2022adbench}, is a recent Anomaly Detection (AD) benchmark in which 15 state of the art AD algorithms are evaluated on over 47 datasets. In addition to these baselines, we evaluate also the KDE estimators, using both Gaussian and Laplace kernels. The performance of all these algorithms is compared to that of SOSREP with the SDO kernel.
...
For all density-based approaches (KDE,SOSREP), we employ the negative of the density as the 'anomaly score'. 
In accordance with the ADbench paper, we evaluate success on each dataset by calculating the average AUC-ROC across four runs, each with a different seed. It's important to note that while the problem is convex, the seed also determines the train-test split of the ADbench data, which may lead to variations in results, especially when there is only a small fraction of anomalies.

In Figure \ref{fig:ADbench_aucs}, for each algorithm, we present a box plot where the average AUC-ROC across all datasets is indicated by an orange dot. Additionally, the boundaries of the box represent the 25th and 75th quantiles, while the line inside the box denotes the median. The algorithms are sorted by the average AUC-ROC. 
An extended evaluation, which compares the relative ranks of the algorithms for each dataset and thereby aims to mitigate potential bias arising from variations in dataset difficulty (reflected in generally higher mean AUC-ROC for all algorithms on easier datasets), is detailed in the Supplementary Material, Section \ref{sec:supp_aucroc}.
...
As noted in Section \ref{sec:intro}, dealing with unnormalized densities adds a layer of complexity to hyperparameter tuning since it prevents the use of maximum likelihood for determining optimal parameters. Consequently, for tuning the smoothness parameter $ a > 0$ in both SOSREP and KDE with SDO kernel we employ a score-based approach. This approach measures the Fisher Divergence (FD) between a dataset sampled from an unknown distribution and a proposed distribution. Specifically, in our case, the FD between the density learned on the training set and the density inferred on the test set. Hence, the hyperparameters tuning procedure simply picks the parameters that resolve with the lowest FD. 
Figure \ref{fig:enter-label} illustrates the FD for different 'a' values for a single dataset. The x-axis represents the parameter values on a logarithmic scale. For visual clarity, we also scaled the FD to be strictly positive and applied a logarithmic transformation.

Observe that for small 'a' values, the majority of the distribution concentrated around the training samples, suggesting overfitting. This poses challenges for gradient computation and might lead to arbitrary extreme FD values. As a result, we concentrate on the stable minimum — defined as a point lower than six of its neighbors, three on each side — with the largest 'a' value. In most of the datasets, there was only a single stable minimum.
A thorough explanation of this approach, addressing important computational aspects and featuring a dedicated algorithm figure, can be found in Supplementary Material Section \ref{SEC:FD}.
...
of $f_{\alpha}$ (on the train set) was measured at initialization, and in the end of each run. In Figure \ref{fig:pos_neg_frac}, for each dataset and for each method, we show an average of the worst 5 fractions among the 50 initializations. Thus, for instance, for the 'shuttle' data, the initial fraction is negligible, and is unchanged by the natural gradient. However, the standard gradient (``alpha GD'' in the Figure, blue) for this dataset yields about 70\% negative values in the 5 worst cases (i.e. 10\% of initializations). We conclude that the natural gradient is considerably better at preserving non-negativity compared to the standard gradient descent.


\section{Conclusion}
\label{sec:conclusion}
In this paper we have developed the theory and the optimisation methods for a multi dimensional unnormalised density estimator, which is regularised by an $L_2$ Sobolev norm. This is the first estimator, among those broadly based on variants of functional space normalisation, that is  computationally tractable, can be applied in higher dimensions, and performs well on a useful and difficult task. 
...
\begin{itemize}
    ...
    \item Proofs related to the SOSREP vs KDE comparison: Section \ref{sec:kde_vs_iner_comparison_proofs}
