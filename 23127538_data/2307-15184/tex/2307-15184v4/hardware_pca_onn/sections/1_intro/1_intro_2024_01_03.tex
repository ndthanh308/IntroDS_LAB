...
\YLnote{poisson is not considered before. we developed a simple vision system to address that and compare the noise models impacts}
\YLnote{https://ieeexplore.ieee.org/document/6637874, https://www.nature.com/articles/srep41435.pdf, https://compphotolab.northwestern.edu/wordpress/wp-content/uploads/2015/04/BeatHadamard.pdf}

\IEEEPARstart{T}{}he projection of \checkednote{information} information from a high-dimensional scene space involving color, timing, polarization, and phase onto a lower-dimensional reconstruction space is crucial step in any imaging and vision device. This projection serves the dual purpose of data reduction to fit the limited number of values that can be digitized and processed by the hardware and the extraction of actionable patterns. This process, often termed coding or multiplexing, is a fundamental and essential aspect of both physical and computational considerations in imaging and vision devices. 
In this project, the primary objective is to optimize coding schemes, with a particular emphasis on selecting the appropriate noise model.

\Xhide{Mathematically, coding, it can be described as a linear operator, $\B{M}$ linking a vectorized incoming light field $\B{x}$ to a vector of light measurements $\B{y}$. }
...
Coding schemes, often developed under signal independent Additive Gaussian Noise (AGN), provide a mathematically tractable framework. While AGN is suitable to model certain types of noise introduced by the measurement device, it does not correctly model Poisson noise\YLnote{\Xhide{even after quantization }\cite{yang2015poisson}}\Xhide{ which is signal-dependent and not avoidable via better sensor fabrication techniques. \Xhide{In almost all modern cameras and sensors, however, Poisson noise is predominant, especially in advanced sensors like Single-photon Avalanche Diode (SPAD), Quanta Image Sensors (QiS), and qCMOS cameras \cite{foord1969PMT, prescott1966PMT, lombard1961PMT}.  }This necessitates a reevaluation of coding strategies tailored to this noise type}.
...
Poisson noise, a fundamental property of light, arises due to the discrete and random nature of light measurements. \AVnote{\checkednote{ This sentence doesn't seem to make sense Jan 18, 2024 9:41 AM}}\YLnote{\Xpolish{Unlike the assumptions of the linear model in Eq.  \ref{eqn:NoiselessMeasurement},}{Different from AGN, Poisson noise is discrete}} and it is impossible to distribute a photon measurement across all open pixels in a mask. Consequently, linear reconstruction approaches based on the measurements $\B{y}$ called for by the \YLnote{\Xpolish{quation}{Eq.  \ref{eqn:NoiselessMeasurement}}} are \Xpolish{\Xpolish{unattainable}{unsatisfactory}}{unattainable}\Xhide{ by linear reconstruction approaches}. This renders coding and compressed sensing approaches\YLnote{\checkednote{more about  compressed sensing}}, designed for effectiveness under the AGN or noiseless conditions unsuitable. The literature discourages their use in such scenarios \cite{harwit1979hadamard, swift1976hadamard, willet2009CSPoisson, scotte2022photon_noise, vanden2019various}. \Xhide{The optimal code, denoted as $\B{M}_P$, for measurements limited by real Poisson noise must converge to $\B{M}_G$ applicable to the optimal code under Additive Gaussian Noise (AGN) or under no noise as the number of measured photons tends toward infinity. However, it is crucial to highlight that, as demonstrated below, $\B{M}_P$ does not necessarily exhibit similarity to $\B{M}_G$ for any finite number of photons.}
...
{\Xhide{In this project, we use the popular single pixel camera design to analyze and design effective coding models under Poisson noise. } Therefore, we propose a coding methodology called \textbf{Selective Sensing} (SS) \Xpolish{that relies on finding codes in basis where the signal is sparse and compressible to be used at capture time}{specifically optimized for Poissonian data}. This approach proves particularly effective for extremely sparse and compressible signals such as character recognition, as it enables the retention of key information while reducing the impact of Poisson noise on downstream analyses.}
}%hide old versions
\AVnote{\checkednote{velten: This still comes out of nowhere. So far we have mostly talked about image reconstruction and reconstructing and image x. Now we talk about a vision problem and character recognition.
Maybe Figire 1 should have another component after x that is the output of the vision system or the pattern. That could also be an image. Or we have to say that x is not necessarily and image but could be something else.
Jan 18, 2024 9:50 AM}}
\YLnote{Alternatively, we propose a coding methodology called \textbf{Selective Sensing} (SS) specifically optimized for Poissonian data acquisition. SS focuses on the retention of the key information from some prior knowledge and reducing the impact of Poisson noise for the downstream computer vision tasks, e.g. classification, instead of general reconstruction of $\B{x}$. Its classification performance most closely approaches the theoretical upper limit among all imaging strategies.
}
...
\item Similar to the conclusion of Cossairt et al. \cite{cossairt2012does}, Mitra et al. \cite{mitra2014can}, Harwit et. al. \cite{harwit1979hadamard}, Swift et. al. \cite{swift1976hadamard}, and Raginsky et. al. \cite{willet2009CSPoisson}, we find that general prior free  coding provides no benefit over a trivial raster scanning approach using a diagonal matrix $\B{M}$ {\cite{scotte2022photon_noise, wuttig2005optimal, schumann2002SNR_poisson, larson1974hadamard_poisson, streeter2009optical_poisson}}. We find, however, that in applications like compressed sensing and pattern recognition, where the collected data is compressible, similar benefits to those seen in compressed sensing and computational imaging can be seen if appropriate adjustments to the measurement matrices are made. 
...
\item We propose the concept of selective sensing (SS) which includes coding methods selectively extracting the features directly for downstream tasks.
