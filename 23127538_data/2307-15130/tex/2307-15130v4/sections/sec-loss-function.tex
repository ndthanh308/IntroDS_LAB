...
However, this version of the bound requires checking diagrams for all possible open sets $U \in \Open (K)$ which creates a combinatorial explosion that is counterproductive in practice.
...
\end{equation*}
If no such $n$ exists, then we  set $d_U^{F}(A,B) = \infty$.
\end{definition}
...
Then $d_U^F(A,B) = 1$, since thickening the set $U$ by $1$ puts $a$ and $b$ in the same connected component.
...
For an example, consider $W,Z \in F(V)$ representing points $w$ and $z$ in Fig.~\ref{fig:examplegraph-distance}.
...
Normally, a natural transformation $\eta:H \Rightarrow H'$ is a collection of component morphisms ${\eta:H(U) \to H'(U)}$ which commute with the inclusions: 
\[
...
Given functors $H,H':\Open(K) \to Set$, an \emph{unnatural transformation}\footnote{A  natural transformation is an unnatural transformation which just happens to follow commutativity properties. In other words, natural and unnatural transformations are not mutually exclusive. This vocabulary follows from~\cite{nlab:unnatural_transformation} so please do not blame us for the linguistic implications.}   $\eta:H \rightarrow H'$ is a collection of maps $\eta_U:H(U) \to H'(U)$ with no additional promise of commutativity. 
...
In the spirit of  \cite{Robinson2020}, we measure the quality of a choice of an  $n$-assignment $\phi, \psi$ using the collections of distances $\{d_U^F \mid U \in \Open(K)\}$ and $\{d_U^G \mid U \in \Open(K)\}$.  
...
\end{equation*}
commute. 
As we use them repeatedly, we will denote these diagrams by $\Parallelograml_\phi(U,V)$ and $\Parallelogramr_\psi(U,V)$, dropping the subscript when it is clear from context.
...
\end{tikzcd}
\end{equation}
the image of a point from $F(U)$ is the same in $G(V^{n+k})$ following both paths. 
...
\label{eqn:dgm:triExtendK}
    ...
    \end{tikzcd}
\end{equation}
the image of a point in $F(U)$ is the same (following both paths) in $F(U^{2(n+k)})$ even if not in $F(U^{2n})$.
...
For this particular example, no $n=1$ interleaving is possible so any choice of assignment will have a non-zero loss (the easiest check is to see that any choice of assignment will force $\Ltu^U =1$). 
...
\begin{align*}
    L(\phi,\psi) \leq 
    \max \Bigg( &
    \left\{\mathrm{diam}(F(U^{k}),d_F^{U^{k}}) \mid U \in \Open(K), k \in \{ n, 2n\}\right\} \\
    &\cup \left\{\mathrm{diam}(G(U^{k}),d_G^{U^{k}})\mid U \in \Open(K), k \in \{ n, 2n\}\right\}
    \Bigg) .
\end{align*}
...
\begin{proof}
The parallelogram portions of the loss function $\Lpl$ and $\Lpr$ take values from distances in $F(U^n)$ and $G(U^n)$, . 
The triangle portions $\Ltd$ and $\Ltu$ take values  from distances in $F(U^{2n})$ and $G(U^{2n})$. 
...
We have so far measured the loss function by studying all possible open sets $U$. 
...
\begin{equation*}
\{\Phi_{U_\sigma}: F(U_\sigma) \to G(U_\sigma^N) \mid \sigma \in K\} 
\end{equation*}
with $\Lpl^{U_\tau, U_\sigma} = 0$ for all $\sigma \leq \tau$, we can extend this to a full natural transformation $\Phi$; i.e.~we can define $\Phi_U$ for all $U$ such that $\Lpl^{U,V} = 0 $ for all $U \subseteq V$. 
\end{restatable}
...
\label{lem:extendTriangles}
    Given natural transformations $\Phi:F \Rightarrow G^N$ and $\Psi:G^N \Rightarrow F$ such that $\Ltd^{U_\sigma} = 0$ for all $\sigma \in K$, then $\Ltd^{U} = 0$ for all open sets $U$. 
\end{restatable}
