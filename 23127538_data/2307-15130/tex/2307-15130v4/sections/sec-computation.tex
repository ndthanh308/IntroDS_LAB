...
For clarity, we use $\phitt$ and $\psitt$ to denote the data structures that store information for $\phi$ and $\psi$, respectively.
These encode set maps $\phitt:(V_F,E_F) \to (V_G,E_G)$ and $\psitt:(V_G,E_G) \to (V_F,E_F)$, which map each vertex to a vertex in the other graph and each edge to an edge in the other graph. 



When $d=1$, recall that $K$, i.e.,~the discretization of $\R$, consists of vertices  $\sigma_{-L},\cdots,\sigma_L$ with heights in our bounding box $[-L\delta,L\delta]$, and with edges $\tau_j = (\sigma_j, \sigma_{j+1})$. 
...
The endpoints of any edge $e \in F(U_{\tau_i}) \subseteq E_F$ can be found via the attaching maps: 
\begin{align*}
F[U_{\tau_i} &\subseteq U_{\sigma_{i}}](e) \in F(U_{\sigma_i}) 
    \text{ and} \\
F[U_{\tau_i} &\subseteq U_{\sigma_{i+1}}](e) \in F(U_{\sigma_{i+1}}). 
\end{align*}
...
In addition, each vertex also keeps track of its height, so a vertex $v \in F(U_{\sigma_i})$ also stores the value $i$ as a representation of its height.
...
To see how these maps arise from an input assignment $\phi$ and $\psi$, we start by focusing on the vertex set. 
...
\]
The elements of $G(U_{\sigma_i}^n)$ are the connected components of the subset of the graph $(V_G,E_G)_{\sigma_i,n}:=(V_{G,\sigma_i,n}, E_{G,\sigma_i,n})$. 
...
So, for each $v \in F(U_{\sigma_i})$, we store a vertex $\phitt(v) \in V_{G,\sigma_i,n}$, where $\phitt(v)$ is in the connected component of $(V_{G}, E_G)_{\sigma_i,n}$ represented by $\phi_{U_{\sigma_i}}(v) \in G(U_{\sigma_i}^n)$.
For instance, consider the example of Fig.~\ref{fig:Assignment} where we assume $n=1$. 
If  $\phitt(b) = w$, then $\phi_{U_{\sigma_i}}(b)$ is the connected component that includes $w$ of $(V_G,E_G)_{\sigma_i,1}$ as shown on the right.  
...
For a fixed $k$, we will determine if $L_B(\phi,\psi) \leq k$. 
...
Following the top of diagram  Eq.~\eqref{eqn:dgm:parallel_extend_basis}, we know that $e$ has a unique endpoint vertex  $v \in F(U_{\sigma_\ell})$, and that vertex has an image under $\phi_{U_{\tau_i}^n}$,  which is a connected component represented by  $\phitt(v) = w \in V_G$. 
...
As we do this once per grid element, we get a total time (when $d=1$) of $O(V_G+E_G)$ time per parallelogram. 
...
Then  for the diagram of Eq.~\eqref{eqn:dgm:parallel_extend_basis} with $\ell = j$ and chasing $bc \in F(U_{\tau_j})$,
...
Similar to the parallelogram case, we take a strip of the graph and check this connectivity question in $O(d(V_G +E_G))$ time. 
...
To count the number of (complected) diagram checks, a vertex $v \in F(U_{\sigma_i})$  is checked for one triangle loss $\Ltd^{U_{\sigma_i}}$; and a vertex $w \in G(U_{\sigma_i})$ is checked for one triangle loss $\Ltu^{U_{\sigma_i}}$. 
An edge 
$e \in F(U_{\tau_i})$ 
...
\begin{equation*}
    O\Bigg( [(V_F+V_G) + 3 (E_F + E_G) ] \cdot \max\{(V_F+E_F),(V_G+E_G)\} \Bigg).
\end{equation*}
