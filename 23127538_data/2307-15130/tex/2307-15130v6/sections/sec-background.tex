...
Assume we are given as input a pair of compact topological spaces with valued functions $f: \X \to \R^d$ and $g: \Y \to \R^d$.
...
for any $U \in S$ and any $V \subseteq U$, $V \in S$. 
...
When the sets involved are obvious in the notation, we will  write the induced map as $F[\subseteq]:F(S) \to F(T)$. 
...
We can use this thickening to build an interleaving distance on cosheaves of the form 
${F:\Open(\cU) \to \Set}$.
