...
\end{equation}
are the same. 
Note that we use $[-]$ to  represent the connected component in the relevant slice of the graph containing that edge or vertex. 
...
So the question becomes: are $e'$ and $w$ in the same connected component of the slice $(V_G,E_G)_{\sigma_\ell,n+k}$, whose components represent the elements of $G(S_{\sigma_\ell}^{n+k})$? 
In order to answer this question easily for all starting edges in $F(S_{\tau_i})$, we use a breadth first search  algorithm to label all the connected components of the slice once. Then the two images of each edge starting from $F(S_{\tau_i})$ can be checked in constant time~\cite[Sec.~5.6]{Jeffe-book}. 
...
Similar to the parallelogram case, we take the relevant slice of the graph and check this connectivity question by finding connected components once in the slice in  $O(|V_G| +|E_G|)$ time, and then the check for each vertex in $F(S_{\sigma_i})$ is done in $O(1)$ time. 
...
Each of these checks involves determining if two vertices are in the same connected component of the higher dimensional analogue of a slice for $\sigma_{\overrightarrow{\ell}}$ with indices $\overrightarrow\ell \in \Z^d$ involves checking  
