...
In this section, we  introduce a loss function for interleavings on geometric graphs.  
%
We will work with a fixed grid and consider two functors $F, G$ as defined in Section \ref{ssec:functorGraphs} arising from two embedded graphs $f:X \to \R^{2}$ and $g:Y \to \R^2$.
In Section \ref{ssec:LossFunction} we give the definition of the loss function (Defn.~\ref{def:Loss_v1}) and provide an example of its calculation.
We use this to give our first version of the bound as Thm.~\ref{thm:bound} in Sec.~\ref{ssec:bound_v1}.
...
This setting can be viewed in parallel to the work of \cite{Robinson2020}, where the input data was viewed as assignments $\Open(X) \to \mathbf{Pseud}$ taking values in the category of pseudometric spaces. 
However, in that work, the final measurements were done to measure the quality of a single input sheaf, rather than a pair of natural transformations as seen here.
...
Consider the example of Fig.~\ref{fig:examplegraph-distance}.
In this case we have a single input graph, and we denote its cosheaf by $F:\Open(K) \to \Set$. 
The set $F(U)$ has two elements, which we will denote by $A$ and $B$ as they represent the connected components containing the points $a$ and $b$ respectively. 
Then $d_U^F(A,B) = 1$ since thickening the set $U$ by one puts $a$ and $b$ in the same connected component.
Likewise, denoting the elements of $F(V)$ by $W$ and $Z$, we see that $d_V^F(W,Z) = 2$ since we must expand the set $V$ by two before they are in the same connected component. 
% Figure environment removed


As a first useful property of this distance, we note that if you thicken your set, the distance between components will only decrease.
...
We use this as follows: first, assume our attempts at finding an interleaving  do not necessarily satisfy the requirements of a natural transformation. 
...
\end{tikzcd}
\]
The following definition, inspired by~\cite{Robinson2020}, gives a name to collections of component morphisms used to define an interleaving where the square might not commute. 

\begin{definition}
\label{def:assignment}
Fix an $n > 0$ and cosheaves $F$ and $G$. 
We define an \emph{assigment}, or more specifically an \emph{$n$-assignment}, to be a collection of maps $\phi \colon F \Rightarrow G^n$ where $\phi_U \colon F(U) \to G(U^n)$. 
\end{definition}
...
This definition is a special case of what is called an ``unnatural transformation" on ncatlab\footnote{\url{https://ncatlab.org/nlab/show/unnatural+transformation}}. 
In order to clarify notation, for the remainder of the paper, we will be using $n$-assignments to to build $(n+k)$-interleavings, which by definition will be required to be natural transformations. 
...
In the spirit of  \cite{Robinson2020}, we will measure the quality of a choice of a pair of $n$-assignments $\phi, \psi$ using the collections of distances $\{d_U^F \mid U \in \Open(K)\}$ and $\{d_U^G \mid U \in \Open(K)\}$.  
...
\begin{definition}
\label{def:Loss_v1}
Fix a pair of assignments 
$\phi:F \Rightarrow G^n$
and 
$\psi:G \Rightarrow F^n$.
From the above diagrams, we define four \emph{loss functions}:
%
%
%
%
...
\end{align*}%
%
Then the loss for a given pair of assignments is defined to be
\begin{equation*}
...
\end{tikzcd}
\end{equation*}
the image of a points from $F(U)$ in $G(V^{n+k})$ is the same no matter which path is followed. 
Similarly, if $\Ltu^U (\phi,\psi)=k$, then in the diagram 
\begin{equation*}
    \begin{tikzcd}
        F(U) 
            \ar[rr, "{F[U \subseteq U^{2n}]}"]   
    ...
    \end{tikzcd}
\end{equation*}
the image of a point in $F(U)$ is the same in $F(U^{2(n+k)})$.
...
%
\paragraph{An Example:} Consider the example of Fig.~\ref{fig:nonzero_loss_finite} and fix $n=1$. 
...
The interleaving diagram we are working with is 
\begin{equation}
...
\begin{equation*}
    ...
    \end{matrix}
\end{equation*}
We will fix an assignment to compute its loss. 
To this end, define the following maps:
\begin{equation*}
\begin{matrix}
    \phi_U: A \mapsto W'& & 
    \psi_U: W,Z \mapsto A'\\
    \phi_{U^1}:A' \mapsto W''& &
    \psi_{U^1}: W',Z' \mapsto A''\\
\end{matrix}
\end{equation*}
In this case, we then have that 
$\Lpl^U = 0$, $\Lpr^U = 0$, $\Ltd^U = \lceil \tfrac{1}{2} \cdot 0\rceil = 0$, and $\Ltu^U= \lceil \tfrac{1}{2} \cdot 1\rceil = 1$
%
%
%
%
...
Just to have a second example, we could instead define the maps 
\begin{equation*}
\begin{matrix}
    \phi_U: A \mapsto W'& & 
    \psi_U: W,Z \mapsto A'\\
...
We use the framework built to give an upper bound for the interleaving distance using the loss function. 
While everything here is done in the context of an embedded graph, we note that the proof of the theorem is entirely dependent on category theoretic constructions. 
\begin{restatable}{theorem}{FirstLossBound}
\label{thm:bound} 
    For $n$-assignments  $\phi\colon F \Rightarrow G$ and $\psi\colon G \Rightarrow F$, 
    \begin{equation*}
        d_I(F, G) \leq  n+L(\phi, \psi). %
    \end{equation*}
\end{restatable}

To prove this, we require the following technical lemma, proved in Section \ref{sec:technicalProofs}.
...
Assume we are given $n$-assignments 
...
\end{enumerate}
In particular, if a pair of $n$-assignments $\phi$ and $\psi$ have $L(\phi,\psi) = 0$, then $\phi$ and $\psi$ constitute an interleaving, and so $d_I(F,G) \leq n$.
\end{restatable}
...
Set $k = L(\phi,\psi)$, and as in Lem.~\ref{lem:lossimpliescommutes}, construct two $(n+k)$-assignments: 
$\Phi$ given by 
$\Phi_U = G[U^n \subseteq U^{n+k}] \circ \phi$,  and
$\Psi$ given by 
$\Psi_U = F[U^n \subseteq U^{n+k}] \circ \psi$.
By definition of the loss function,  $\Lpl^{U,V}(\phi) \leq k$, $\Lpr^{U,V}(\psi) \leq k$, $\Ltd^{U}(\phi,\psi) \leq k$, and $\Ltu^{U}(\phi,\psi) \leq k$. 
...
Note that we have no reason to believe that this bound is tight. In particular, in Section \ref{ssec:BasisBound} we will find an improved bound by extending from a subset of the input assignment to ensure as much commutativity as possible prior to computing the bound.
...
\end{equation*}
and note that the $\sup$ can be replaced with a $\max$ since we are working in finite metric spaces.
\begin{lemma}
The loss function for $n$-assignments $\phi$ and $\psi$ is bounded above by the distances in $F(U^{2n})$ and $G(U^{2n})$; specifically,
\begin{equation*}
    L(\phi,\psi) \leq 
    \max\; \left\{\mathrm{diam}(F(U^n),d_F^{U^n}) \mid U \in \Open(K)\right\} \cup \left\{\mathrm{diam}(G(U^n),d_G^{U^n})\mid U \in \Open(K)\right\}.
\end{equation*}
In particular, if the original graphs have one connected component each, then $L(\phi,\psi)$ is finite.
\end{lemma}
...
To this point, we have measured the loss function by studying all possible open sets $U$ in the poset. 
While this is helpful for definitions, it does not make for a reasonable computational setting. 
To that end, we will focus on a basis of the topology, and prove that this basis suffices.

\begin{definition}
    An open set defined by the upset of a cell $\sigma \in K$ (that is, a vertex, edge, or square), $U_\sigma = \{ \tau \mid \tau \geq \sigma\}$, is called a \emph{basic open set}.
    We denote this collection by $B = \{U_\sigma \mid \sigma \in K \}$.
\end{definition}

Note that  $B$ is a basis for the Alexandroff topology. 
...
\begin{definition}
    A \emph{basis $n$-assignment} (or simply basis assignment) is a collection of maps 
...
This can be seen in the following lemma, proved in Section \ref{sec:technicalProofs}. 

\begin{restatable}{lemma}{extendToNatTrans}
\label{lem:extendToNatTrans}
Given a basis assignment 
...
Note that the symmetric version extending a partial assignment to a natural transformation $\Psi: F \Rightarrow G^N$ is obtained in exactly the same way. 
...
\begin{prop}
\label{prop:zeros}
Fix a pair of basis $N$-assignments $\Phi$ and $\Psi$. 
...
Finally, we arrive at our main theorem, where we can use the basis $n$-assignment and the calculated loss function to give a bound for the interleaving distance. 
...
Then by Lem.~\ref{lem:extendToNatTrans}, we can extend $\Phi$ and $\Psi$ to full natural transformations defined for all $U \in \Open(K)$ rather than just basis elements. 
...
With the goal of using part 3 of Lem.~\ref{lem:lossimpliescommutes}, first note that for the original $n$-assignments, we know that $\Ltd^{U_\sigma} (\phi,\psi) \leq k$ for basis elements. 
...
\end{tikzcd}
\end{equation*}
The leftmost and rightmoost triangles commute by definition of $\Psi$, and the orange parallelogram commutes because $\Psi$ is a natural transformation. 
...
The proof of $\Lpl^{U_\sigma^n, U_\sigma^{n+k}} \leq k$ is similar.
Thus using Lem.~\ref{lem:extendTriangles} for $\Phi$ and $\Psi$, $\Ltd^{U} (\Phi,\Psi) =0$ for all open sets $U$. 
