...
%%\usepackage{backref} %para depois tirar
%%\usepackage[notref,notcite]{showkeys} % to be removed
%%%
%%%%%%%%%%% Macros %%%%%%%%%%%
...
say $\Omega=\Omega_n=\{1,2,\ldots,n\}$, we denote, as usual,
...
$$
P_n=\left(\Omega_n,\{\{i,i+1\}\mid 1\<i\<n-1\}\right).
$$
...
$$
S_n=\left(\{0,1,\ldots,n-1\},\{\{0,i\}\mid 1\<i\<n-1\}\right), 
$$
...
$$
C_n=(\Omega_n, \{\{i,i+1\}\mid 1\<i\<n-1\}\cup\{\{1,n\}\})
$$
with $n$ vertices.  
...
Moreover, $\DPW_n^+$ is a submonoid of $\DPW_n$ and $\DPW_n^-$ is a monoid but not a submonoid of $\DPW_n$. 
...
Moreover, for $n\geqslant3$, $g$ and $h$ generate the well-known \textit{dihedral group} $\D_{2n}$ of order $2n$ 
...
of $M$ can be described as follows: for $\alpha, \beta \in M$,
\begin{itemize}
...
\end{smallmatrix}\right)$. 
Let $\tau$ and $\tau'$ be permutations of $\{1,\ldots,\ell\}$ and $\{1,\ldots,\ell'\}$, respectively, such that 
...
It is easy to show that: if $\alpha\in J_2\setminus(\DPW_n^-\cup\DPW_n^+)$ (and so $0\in\dom(\alpha)\cup\im(\alpha)$ with $0\alpha\neq0$) then 
$$
...
%% 
%% n>=4 1 rank 0 + 1 rank 1 + 2 rank 2
%% 
%% n=4 2 rank 3 + 2 rank 4
%% n=5 3 rank 3 + 3 rank 4
%% n=6 4 rank 3 + 6 rank 4
%% n=7 4 rank 3 + 7 rank 4
%% n>7 4 rank 3 + 8 rank 4
...
The author would like to thank Ricardo Guilherme for pointing out several typos in the first version of this paper.
