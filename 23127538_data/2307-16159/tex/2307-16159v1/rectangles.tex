...
\begin{definition}[Factorization norm]
	...
	\[
		\gamma_2(\mm) \defeq \min \{ R \geq 0 : \text{for all $a \in A, b \in B, \exists \vu_a, \vv_b$ so that $\mm_{a,b} = \inner{\vu_a, \vv_b}$ and $\norm{\vu_a}_2 \norm{\vv_b}_2 \leq R$}\}.
	\]
\end{definition}
...
\begin{proof}[Proof of \cref{lem:almost-monochrom-rects}]
	...
	We observe that on $[0,1]$, the function $h$ is convex, monotone increasing, lowerbounded by $h(0) = 1/4$, and $h'(\alpha) = \frac{1}{2\pi \sqrt{1-\alpha^2}} \geq \frac{1}{2\pi}$.
	Additionally, the following claim about $c$ will be useful for our calculations later:
	\begin{claim} \label{lem:c-bounds}
		$\frac{c(j)}{c(j-1)} \geq 1 + \frac{2}{\pi \Delta \sqrt{r}}$ for $1 \leq j \leq \Delta$.
		Also, 
		$\frac{c(\Delta)}{c(0)} \leq 1 + \frac{4}{\pi \sqrt{r}}.$
	\end{claim}
	\begin{proof}
		We use first order approximations for $c$. For the first inequality, we also use the fact that $c(j-1) \geq 1/4$:
		\begin{align*}
			\frac{c(j)}{c(j-1)} &\geq \frac{c(j-1) + c'(j-1)}{c(j-1)} \geq 1 + \frac{1}{2 \pi \Delta \sqrt{r} \cdot c(j-1)} \geq 1 + \frac{2}{\pi \Delta \sqrt{r}}.
		\end{align*}
		For the second inequality, we may assume $r \geq 2$:
		\begin{align*}
	...
	\begin{proof}
		...
		\begin{enumerate}[(1)]
			...
			\end{align*} 
			Using the lower bound from \cref{lem:c-bounds}, along with $\log(1+2x) \geq x$ for small $x$, we conclude it suffices to choose $k$ to satisfy
			\begin{equation}\label{eq:k-lowerbound}
			k \geq ((8 \Delta)^i+1) \log (64 r \Delta) \cdot \pi \Delta \sqrt{r}.
			\end{equation}
		...
		Indeed when $i \neq 0$, we have
		\[
			((8 \Delta)^i+1) \Delta \leq \frac14 ((8\Delta)^{i+1} - (8\Delta)^{i}-1),
		\]
	...
	\end{proof}
	For any established choice of $k$, we always have $k \leq 2 \pi \Delta (8 \Delta)^{\Delta} \log(64 r \Delta) \sqrt{r}$.
