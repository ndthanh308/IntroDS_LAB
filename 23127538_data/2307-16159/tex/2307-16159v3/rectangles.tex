...
\begin{definition}[Factorization norm]
	...
	\begin{align*}
		\gamma_2(\mm) \defeq \min \{ R \geq 0 &: \text{there are families of vectors $\{\vu_a\}_{a \in A}, \{\vv_b\}_{b \in B}$ so that} \\
			 &\; \mm_{a,b} = \inner{\vu_a, \vv_b} \text{ and } \norm{\vu_a}_2 \norm{\vv_b}_2 \leq R \text{ for all } a, b\}.
	\end{align*}
\end{definition}
In other words, $\gamma_2(\mm)$ gives the Euclidean length needed to factor the matrix $\mm$. 
Note that by rescaling the $\vu_a$'s and $\vv_b$'s, it is straightforward to guarantee $\norm{\vu_a}_2 \leq \sqrt{R}$ and $\norm{\vv_b}_2 \leq \sqrt{R}$ for all $a, b$ in the definition.
...
\end{lemma}
\begin{proof}
	Construct vectors $\vu_a, \vv_b$ each with 2-norm $\norm{\vu_a}_2, \norm{\vv_b}_2 \leq s$ and $\mm_{a,b} = \inner{\vu_a, \vv_b}$ using \cref{lem:mm-gamma2-norm-bound}.
...
\begin{proof}[of \cref{lem:almost-monochrom-rects}]
	...
	\begin{proof}
		...
		\begin{align*}
			...
			1 +  \frac{4 \Delta}{\Delta \sqrt{r} \cdot 2\pi \sqrt{1 - 1/r}} \leq 1 + \frac{4}{\pi \sqrt{r}}.
		\end{align*}
	\end{proof}
...
%	Now we can complete the lower bound on the expected size of $\mr$:
	\begin{align*}
%          \E_{\mr \sim \mathcal{D}_k} \left[ |\mr| \right] &\geq
                                                             \E_{\mr \sim \mathcal{D}_k} \left[ |\cE_i \cap \mr| - 16r \sum_{j \neq i} |\cE_j \cap \mr| \right] 
	&\geq \frac{|A| |B|}{m_i} \frac{1}{2 \cdot 4^k} \\
	&\geq \exp(-\Delta^{O(\Delta)}\cdot  \sqrt{r} \cdot \log r) |A| |B|.
	\end{align*}
	% as claimed.
        Then any $\mr$ attaining this expectation will simultaneously satify
        $|\mr| \geq \exp(-\Delta^{O(\Delta)}\cdot  \sqrt{r} \cdot \log r) |A| |B|$ and be
        $(1-\frac{1}{16r})$-monochromatic.
\end{proof}