...
\subsection{photomeson ($p\gamma$) process}
 The decay chains of inelastic photomeson interactions are 
\begin{eqnarray}
\rm p+\gamma\rightarrow n +\pi^0, \label{eq:decay0}\\
\rm \pi^0\rightarrow\gamma+\gamma,
\end{eqnarray}
and 
\begin{eqnarray}
\rm p+\gamma\rightarrow p +\pi s, \label{eq:decay1}\\
\rm \pi^+\rightarrow\mu^+ + \nu_\mu ,\label{eq:decay10}\\
\rm \pi^-\rightarrow \mu^- + \overline{\nu}_\mu,\\
\rm \mu^+\rightarrow e^+ + \nu_e +\overline{\nu}_\mu,\\
\rm \mu^-\rightarrow e^-+\overline{\nu}_e + \nu_\mu. \label{eq:decay2}
\end{eqnarray}
...
\end{eqnarray}
 where $\chi$ is the ratio of Lorentz Factor of proton to that of neutron and $\chi_\pi\simeq2.15$ is the threshold for pion production~\cite[e.g.][]{1971ctf..book.....L,2007A&A...471..395K}. For the Poynting-flux-dominated (PFD) case, $m=1/3$ and the saturation radius is much larger, $\tau_{np}\simeq0.01\zeta_n^{1/2}$ is several times smaller that that of fireball ($\tau_{np}\simeq0.05\mbox{--}0.1$). Thus, the corresponding U.L. of $\xi_N$ is even much larger than that obtained in the fireball scenario, which seems too large for the assumption that the Poynting flux is dominated. Therefore, the U.L. can not provide a stringent constraint in the PFD scenario.

A third scenario of a hybrid jet with $1\lesssim\sigma_0<10$, could be considered as well. If the decoupling occurs during the rapid acceleration, the case is similar to the fireball scenario; similar to the above discussion, the U.L. of $\xi_N\sim$ a few could be also obtained, which seems consistent with the hybrid scenario if we assume there exists a Poynting flux of which the energy is compatible with $E_{\gamma, \rm iso}$. Otherwise, it is similar to the PFD scenario. 
...
This may be caused by the enhanced dissipation below the photosphere with time. In summary, an significant existence of the photospheric emission is observed in the prompt phase of GRB 230307A, which is greatly different from that in GRB 221009A.

%The details of the spectral evolution are not the main goal in this letter and will not be further discussed.
...
% Figure environment removed
 ...
 If the jet is PFD, it is inferred that there must be a thermalization below the photosphere, to interpret the observed photospheric emission. 
...
%\tabletypesize{\small}
\tablewidth{0pt}
\center{Table. \ref{tab:fitresTR}--- Continued.}
\tablehead{
\colhead{Time bins}
& Favoured Model
&\colhead{$\alpha$}
