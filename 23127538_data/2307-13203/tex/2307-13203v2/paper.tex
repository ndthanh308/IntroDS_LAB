...
\ifdefined\iros
\newcommand\shortenXor[2]{#2}
%\AtBeginEnvironment{proof}{\small}
%\AtEndEnvironment{proof}{\par\vspace*{-9pt}}
\newcommand{\gobble}[1]{}
\newcommand{\gobblexor}[2]{#2}  %Long is first argument; short is second
\else
\newcommand\shortenXor[2]{#1}
\newcommand{\gobble}[1]{#1}
\newcommand{\gobblexor}[2]{#1} 
\fi

% To use: \shortenXor{Long text to appear in ArXiV version}{Short for IROS}
...
plausibly concealed by (or mistaken for) another.
%
%Using ambiguity  to limit the acquisition of knowledge is a strong  privacy guarantee, as recognized in some prior work---but our formulation of sensor selection with  inter-itinerary conflation is novel.
Using sensor ambiguity to limit the acquisition of knowledge is a strong  privacy guarantee, a form of guarantee which some earlier work examined under formulations
distinct from our inter-itinerary conflation approach.
%Using sensor ambiguity  to limit the acquisition of knowledge is a strong  privacy guarantee, and one which some earlier work has examined. 
By concretely
formulating privacy requirements for sensor selection, 
this paper
connects both lines of work in a novel fashion: privacy---where there is a bound from above, and
behavior verification---where sensors choices are bounded from below.  
%
We examine the
worst-case computational complexity that results from both types of bounds,
proving that upper bounds are 
more challenging 
under standard computational
complexity assumptions.  The problem is intractable in general, but we introduce an
approach to solving this problem that can exploit interrelationships
between constraints, and identify opportunities for optimizations.  Case studies
...
%\textcolor{red}{One of the comments here was that we need to explain more why the problem and our solution is novel and challenging. I think our reference to challenging was meant to convey the PSPACE completeness, which I'm assuming the reviewer misinterpreted? Though this work can be seen as an extension to the work of Rahmani, may be we need to explain more how privacy in this specific model was never considered before.}
...
%\textcolor{red}{Another comment mentions that a `deep analysis of privacy' is needed. I'm not sure what this means since we did consider how privacy has been considered by other authors in different settings.}

\shortenXor{}{Some detail has necessarily been omitted,
the reader is encouraged to refer to \cite{phatak23sensorfull} for the full
...
$E$ is a set of edges;
$\src : E \to V$ and $\tgt : E \to V$ are source and target
functions, respectively, identifying a source vertex
and target vertex for each edge;
$v_0 \in V$ is an initial vertex;
$S = \{s_1, s_2,\dots , s_k\}$ is a nonempty finite set of sensors;
$\mathbb{Y} = \{Y_{s_1}, Y_{s_2},\dots , Y_{s_k}\}$ is a collection of mutually
disjoint event sets associated to each sensor; 
$\lambda : E \to \pow{Y_{s_1} \cup Y_{s_2} \cup\dots\cup Y_{s_k}}$ is a labelling function, which assigns to each edge, a world-observation a set of events.
(Here powerset $\pow{X}$ denotes all the subsets of $X$.)}
...
that can be confused with one from the other itinerary. 
When there is a conflation constraint, any walk from the first itinerary has
at least one from the second that appears identical. 
Conflation models privacy in the following sense: any putative claim that the agent followed one itinerary can be countered by arguing,  just as plausibility on the basis of the sensor readings, that it followed the other itinerary.
While the  discrimination constraint
...
$\delta_{\mathcal{P}}: Q_{\mathcal{P}} \times E \rightarrow Q_{\mathcal{P}} \cup \{\perp\}$ is a function such that for each $(q, v) \in Q_{\mathcal{P}}$ and $e \in E$; 
   $\delta_{\mathcal{P}}((q, v), e)$ is defined to be $\perp$ if 
   $\srcfunc{e} \neq v$, otherwise, 
   $\delta_{\mathcal{P}}((q, v), e) = (\delta(q, e), \tgtfunc{e})$;
...
{
\begin{proof}
The full detailed proof appears in \cite{phatak23sensorfull}.
\end{proof}
}
...
% \textcolor{red}{
% Another comment suggested we use a reduction to do this proof. I'm really not sure what that meant, since we're already using reductions to prove PSPACE-Completeness.\\
% Also, related, a minor comment was that the math notation was hard to understand. We iterated on the proof to make it simpler and consistent, and I think the math is necessary for rigor and to avoid ambiguities.
% }
...
% \textcolor{red}{
% There were 2 comments -- one regarding the real world application and the other about providing an experiment and short demo.\\
% For the first, it was really hard to find a test case that was parameterizeable. May be we can cast this test case we constructed as a large public space divided into blocks (where privacy is needed somehow)? Alternatively, we may also try to connect it back to the elder care scenario.\\
% For the second, our paper is based in theory, so it doesn't make sense to provide a physical demo. I'm not sure if we should mention it somewhere? 
% }
...
% \textcolor{red}{One of the minor comments said that more comparisons are needed in simulation. It was unclear what to compare to what. We have compared the 2 optimizations with each other and the baseline.}
...
grant~\href{http://nsf.gov/awardsearch/showAward?AWD_ID=2034097}{IIS-2034097}
and DoD Army Research Office under
award~W911NF2120064.
