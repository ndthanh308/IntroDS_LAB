...
In this section, we compare HiHGNN to baselines and provide the optimization analysis in detail.

\subsection{Overall Results} \label{sec:speedup} 
% \todo{
%% 1. 一句话告诉大家为什么做实验，引出图
%% 2. 给出最直接的结论
%% 3. 给出支撑结论的技术点
...
% }
\textbf{Speedup.} 
Fig.~\ref{fig:overall_speedup} shows the speedup of HiHGNN to GPU T4. The last set of bars, labeled AVG, indicates the arithmetic mean across all models.
HiHGNN achieves an average speedup of 41.5$\times$ and 8.6$\times$ compared to GPU T4 and A100, respectively. 
...
Second, an independency-aware parallelism exploitation optimization help leverage more hardware resources to improve performance.
...
\textbf{Energy and Its Breakdown.} Fig.~\ref{fig:overall_detailed_results} (a) shows that HiHGNN improves the average energy efficiency of 106$\times$ and 73$\times$ compared to GPU T4 and GPU A100, respectively. The energy consumption of all platforms includes the DRAM.
...
HiHGNN reaches 2.2$\times$ of GPU T4 and 4.9$\times$ of GPU A100 in bandwidth utilization, respectively.
...
HiHGNN dramatically reduces DRAM accesses by 78\% and 66\% compared to GPU T4 and GPU A100 on average, respectively.
%
