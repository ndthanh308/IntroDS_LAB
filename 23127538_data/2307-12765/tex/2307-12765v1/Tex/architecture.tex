...
%                 \For{$e_{v,u}$ $\in$ $E^{\mathcal{P}}$}{
%                     % \tcp{Edge-Centric Scheduling}
%                     \eIf{$x_v$ and $x_u$ is projected}{
%                         $\alpha_{v,u}^{\mathcal{P}}$=CImpNA($\theta_{v,*}^{\mathcal{P}}$, $\theta_{*,u}^{\mathcal{P}}$) \;
%                         $\alpha_{v}^{\mathcal{P}}$$\gets$Accumulate($\alpha_{v,u}^{\mathcal{P}}$)\;
%                         $z_v^{\mathcal{P}}$$\gets$Aggregate($\alpha_{v,u}^{\mathcal{P}}$, $h_u'$)\;
...
\begin{algorithm}[!t]
    \SetAlgoLined
    % \caption{\textbf{Bound-aware Stage Fusion Algorithm}}
    \caption{\textbf{\revise{Stage-fusion Programming Model}}}
    \footnotesize
    \label{alg:Programming Model}
    \textbf{Input:} semantic graphs of G = \{$G^{\mathcal{P}_1}$, $G^{\mathcal{P}_2}$ ... $G^{\mathcal{P}_n}$\};\\
    \textbf{Output:} the embedding $h_v$ of each vertex $v$;\\
    ...
    \For{semantic graph $G^{\mathcal{P}}$ $\in$ G}{
        ...
        % \For{each task list \textbf{in parallel}}{
            ...
            \centerline{{\underline{\color{blue} $\triangleleft \quad \textbf{NA Stage}$}}}
                    \For{each edge $e_{v,u}$ in NA\_task\_list}{
                        % \tcp{Edge-Centric Scheduling}
                        \eIf{$x_v$ and $x_u$ is projected}{
                            $z_v^{\mathcal{P}}$$\gets$Aggregate($\theta_v^{\mathcal{P}}$, $\theta_u^{\mathcal{P}}$, $h_u'$)\;
                            \If{all neighbors of $v$ have finished NA}{
                                Send\_to\_LSF\_task\_list($v$);
                            }
                        }
...
%\delete{Furthermore, we also apply optimizations to prevent redundant computations and alleviate the execution stalls. Specifically, the computation of the attention importance for each vertex pair in the NA stage is rearranged to convert the vector product of two concatenated long vectors in Equation~\ref{eq:attn} into the summation of two partial short vector products, enabling the reuse of attention coefficients. To alleviate the execution stall caused by the accumulation of the denominator, we decompose the computation of the \textit{Softmax} function and fuse it into subsequent computation processes (lines 15-16 and 33-36).} 
...
\setlength{\belowdisplayskip}{2pt}
    \begin{aligned}  \label{eq:softmax decompose}
        \displaystyle \sum_{u\in \mathcal{N}_v^{\mathcal{P}}} \alpha_{v,u}^{\mathcal{P}} \cdot h_u^{'}=\frac{\displaystyle \sum_{u\in \mathcal{N}_v^{\mathcal{P}}} exp(\theta_{v,u}^{\mathcal{P}}) \cdot h_u^{'}}{\displaystyle \sum_{k\in \mathcal{N}_v^{\mathcal{P}}} exp(\theta_{v,k}^{\mathcal{P}})}
    \end{aligned}
\end{equation*}
...
%         \displaystyle \sum_{u\in \mathcal{N}_v^{\Phi}} \alpha_{v,u}^{\mathcal{P}} \cdot h_u^{'}=\displaystyle \sum_{u\in \mathcal{N}_v^{\Phi}} \frac{exp(\theta_{v,u}^{\mathcal{P}})}{\displaystyle \sum_{k\in \mathcal{N}_v^{\mathcal{P}}} exp(\theta_{v,k}^{\mathcal{P}})} \cdot h_u^{'}=\frac{\displaystyle \sum_{u\in \mathcal{N}_v^{\mathcal{P}}} exp(\theta_{v,u}^{\mathcal{P}}) \cdot h_u^{'}}{\displaystyle \sum_{k\in \mathcal{N}_v^{\mathcal{P}}} exp(\theta_{v,k}^{\mathcal{P}})}
%     \end{aligned}
% \end{equation}
...
The resulting datapath is depicted in Fig.~\ref{fig:datapath}. For each edge $e_{v,u}$, the raw features of the source vertex $u$ and the target vertex $v$ are projected into the same vector space through a linear transformation function performed by the \textit{Systolic Module} (indicated by SYST in Fig.~\ref{fig:datapath}). A bitmap-based data reuse mechanism is employed here to remove computation redundancy. Since $h_v'$ and $h_u'$ are globally reusable while $\theta_{v,*}^{\mathcal{P}}$ and $\theta_{*,u}^{\mathcal{P}}$ ($\theta^{\mathcal{P}}_{v, *}$ for destination vertex and $\theta^{\mathcal{P}}_{*, u}$ for source vertex) are only reusable within the same semantic graph, a total of three cases arise for the reuse mechanism, as indicated by the different colored lines in Fig.~\ref{fig:datapath}. More details are given in Section~\ref{sec:basic_implementation_of_data_reuse}. 

Unlike the traditional HGNN execution process, after obtaining $h'_{v}$ and $h'_{u}$, instead of writing them back and keeping them until aggregation in the NA stage, they are immediately sent to the \textit{Systolic Module} for the computation of $\theta_{v,*}^{\mathcal{P}}$ and $\theta_{*,u}^{\mathcal{P}}$ in NA stage, and the weighted aggregation operation from $u$ to $v$ is completed by the \textit{SIMD Module} afterward. 
