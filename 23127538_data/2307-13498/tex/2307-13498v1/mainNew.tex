...
Their method was later shown to generate all one-dimensional Fourier quasicrystals with $\N$-valued coefficients ($ \N $-FQ). 


In this paper, we characterize which Lee-Yang polynomials %that 
give rise to 
%measures
non-periodic $ \N $-FQs with unit coefficients and uniformly discrete support, 
and show that this property is generic among Lee-Yang polynomials. 
We also show that the infinite sequence of gaps between consecutive atoms of any $\N$-FQ 
has a well-defined distribution, which, under mild conditions, is absolutely continuous. This generalizes previously known results for the spectra of quantum graphs to arbitrary $\N$-FQs.
Two extreme examples are presented: first, a sequence of $\N$-FQs whose gap distributions converge to a Poisson distribution. Second, a sequence of random Lee-Yang polynomials that results in random  $\N$-FQs whose empirical gap distributions converge to that of a random unitary matrix (CUE).%%PREVIOUS ABSTRACT
%A recent sequence of works \cite{KurasovSarnak20jmp,OlevUlan20,AloCohVin} established that one-dimensional Fourier quasicrystals with $ \N $-valued coefficients (abbreviated $ \N $-FQ) are exactly all measures of the form $ \mu=\sum m(x)\delta_{x} $, where the summation is over the zeros of $x\mapsto p(e^{ix\lv_{1}},\ldots,e^{ix\lv_{n}})$, $m(x)$ represents the multiplicity or degree of each zero, $p$ denotes a Lee-Yang polynomial in $n$ variables, and $\lv=(\lv_{1},\ldots,\lv_{n})\in\R_{+}^{n}$ is a positive vector. Notably, both works \cite{KurasovSarnak20jmp,OlevUlan20} provide explicit examples of non-periodic $\N$-FQs with unit coefficients ($m(x)\equiv 1$) and uniformly discrete support, thereby answering a long-standing question posed by Meyer.
%
%
%This paper provides necessary and sufficient conditions for a Lee-Yang polynomial $p$ such that for any $\lv$ with $\Q$-linearly independent entries, the measure $\mu$ will be non-periodic with unit coefficients and uniformly discrete support. These conditions are shown to be generic among Lee-Yang polynomials. Additionally, it is shown that the infinite sequence of gaps between consecutive atoms of $\mu$ possesses a well-defined distribution, which, under certain conditions on $p$, is absolutely continuous. Two extreme examples are presented: the first involves a sequence of $\N$-FQs whose gap distributions converge to a Poisson distribution, while the second involves randomly selected polynomials for each $n\in\N$ and deterministic choices of $\lv$, resulting in the empirical gap distributions converging to that of a random $n\times n$ unitary matrix (CUE).
\end{abstract}
...
Furthermore, we provide explicit perturbation taking any $ p\in\A_{\bd}(n)  $ to a one parameter family polynomials $ p_{\lambda}=p+\sum_{j=1}^{|\bd|-1}\lambda^{j}q_{j} $, such that $ p_{0}=p $ and $ p_{\lambda} $ satisfies (i) for any $ \lambda>0 $.
\end{thm}
...
They would like to thank Peter Sarnak and Pavel Kurasov for many enlightening discussions.  The first author is supported by the Simons Foundation Grant 601948, DJ.
...
To see that $U\subseteq \mathfrak{S}_{\bf d}^{\circ}$, let ${\bf a}\in \R^n$, ${\bf b}\in \R_+^n$ and let $\{\lambda_j\}_j$ denote the roots of $\mathcal{D}_1q({\bf 1}, {\bf a}+{\bf b}t)$ and $\{s_j\}_j$ denote the roots of $q({\bf 1}, {\bf a}+{\bf b}t)$. These roots are distinct by the argument above. 
...
The proof of \Cref{prop: non-binomial} part (2) is a consequence of \cite[Theorem 1.2]{Evertse00},  often known as Lang's GM Theorem. To state we consider $ (\C^{*})^{n} $ as multiplicative group, and it will be convenient to define the notions of \emph{rank, division group,} and \emph{algebraic torus cosets} in terms of the exponent map $ \exp:\C^n\to(\C^{*})^{n} $. 
\begin{Def}
...
\end{Def}
\begin{thm*}\cite[Theorem 1.2]{Evertse00}
