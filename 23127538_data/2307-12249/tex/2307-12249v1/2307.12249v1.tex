%**************************************************************************
%***                            Last Change: Sun 23 Jul 2023 00:13
%***   < KARA  ¤ ¤ ¤  arXiv >
%***
%**************************************************************************
...
\newcommand{\DP}{{.\kern7pt}}				% delimiter for predicate formula
...
\begin{flushleft}
	{\Large\bf Karamata's theorem for regularised Cauchy\\[2mm] transforms}
	\\[5mm]
	\textsc{
	Matthias Langer
	\,\ $\ast$\,\
...
\begin{equation}\label{L4}
	...
	\qquad z\in\bb C^+,
\end{equation}
of $\mu$ plays an important role in many areas, such as spectral theory, 
moment problems, complex analysis and random matrix theory.
It is well known that the measure $\mu$ can be reconstructed from $C[\mu]$
via the Stieltjes inversion formula.
In many applications it is important to relate properties of $\mu$ to
properties of $C[\mu]$.  One question that has attracted a lot of attention
is the relation between the asymptotics of $\mu$ at $\pm\infty$ and
the asymptotics of $C[\mu](z)$ as $z\to\infty$.
In particular, for measures that are supported on $[0,\infty)$,
results were obtained in the early 20th century:
G.~Valiron \cite{valiron:1913}, E.C.~Titchmarsh \cite{titchmarsh:1927},
and G.H.~Hardy and J.E.~Littlewood \cite{hardy.littlewood:1930}
proved that $C[\mu](-x)\sim Cx^\alpha$ as $x\to\infty$ (where $C[\mu]$
is analytically continued to $(-\infty,0)$) if and only if the
distribution function of the measure $\mu$ satisfies $\mu([0,t))\sim C't^{\alpha+1}$
for $\alpha\in(-1,0)$ and some constants $C,C'$.
In \cite{karamata:1931a} J.~Karamata generalised the results to growth
of regular variation instead of power asymptotics;
for the definition and some examples of regularly varying functions
see the beginning of \cref{L115}, in particular, \cref{L37}.

For measures that are supported on $\bb R$ there is much less known;
see, e.g.\ \cite{pleijel:1962,selander:1963}.
One of the main difficulties is that contributions from the positive and
negative half-axes can cancel each other.
Moreover, in many applications, e.g.\ spectral theory of Sturm--Liouville
and Schr\"odinger operators, measures are used that grow faster 
at infinity: instead of $\int_{\bb R}(1+|t|)^{-1}\DD\mu(t)<\infty$
they are only Poisson integrable, i.e.\ they satisfy 
$\int_{\bb R}(1+t^2)^{-1}\DD\mu(t)<\infty$.
In this case one has to regularise the integral in \cref{L4};
a common regularisation is
\begin{equation}\label{L5}
	\widetilde C[\mu](z)\DE\int_{\bb R}\Big(\frac 1{t-z}-\frac t{1+t^2}\Big)\DD\mu(t),
	\qquad z\in\bb C^+.
\end{equation}
A Tauberian theorem for such measures is stated in \cite{bennewitz:1989};
however, the proof given is false; see the discussion at the beginning of \cref{L112}.
For measures that grow even faster at infinity, one has to use more regularising terms.
In the current paper we study measures that satisfy $\int_{\bb R}(1+t^2)^{-\kappa-1}\DD\mu(t)<\infty$
for some $\kappa\in\bb N_0$.  Such measures appear, for instance, in connection with
the spectral theory of differential operators with two singular endpoints.
In \cref{L111} we discuss in detail a regularisation of the Cauchy transform 
for such measures; see, in particular, \eqref{L38}.

The main results of the paper (see \cref{L28} and \cref{L40}) are Abelian and 
Tauberian theorems for such regularised Cauchy transforms, 
i.e.\ we relate the asymptotics of the distribution functions $\mu([0,t))$ 
and $\mu((-t,0))$ of the measure $\mu$ at $+\infty$ and $-\infty$ 
to the asymptotics of the regularised Cauchy transform at infinity.  
In the generic case a full characterisation is possible.
There are some boundary cases, namely when the index of the regularly varying function
...
In \cref{L111} we study basic properties of regularised Cauchy transforms;
in particular, we characterise the range of the transform and
we prove an analogue of the Grommer--Hamburger theorem that relates convergence
of a sequence of measures to convergence of the generalised Cauchy transforms.
We use the latter theorem to prove a Tauberian theorem in \cref{L112}.
...
and a reduction to Stieltjes transforms of measures supported on $[0,\infty)$.
Here we have to treat real and imaginary parts of the regularised Cauchy transform
separately.
In \cref{L114} we combine the results from \cref{L112,L113} to prove
our main theorems.  We also provide counterexamples for some boundary cases.
...
\noindent
\textbf{Notation.} \\
Throughout the paper, we always use the branches of the logarithm and complex powers
which are analytic on $\bb C\setminus(-\infty,0]$ and take the value $0$ or $1$,
respectively, at the point $1$.
Moreover, we set $\bb C^+\DE\{z\in\bb C:\Im z>0\}$ and $\bb C^-\DE\{z\in\bb C:\Im z<0\}$.
...
Throughout the paper we use the notation $f\sim g$ to express that $\frac fg\to 1$,
...
In this section we consider regularised Cauchy integrals in more detail.
First let us recall characterisation of the ranges of the transforms
...
for a comprehensive account see, e.g.\ \cite{kac.krein:1968a}.
...
\begin{proposition}\label{L6}
	...
	\item 
		...
		\begin{equation}\label{L7}
			q(z)=a+bz+\widetilde C[\mu](z), \qquad z\in\bb C^+,
		\end{equation}
...
%Thereby, for a positive Borel measure on $\bb R$, we set
%\[
%	\ms p(\mu)\DE \inf\biggl\{n\in\bb N_0\DS
%	\int_{\bb R}\frac{\D\mu(t)}{(1+t^2)^{n+1}}<\infty\biggr\}
%	\in\bb N_0\cup\{\infty\}.
%\]
...
\cite{dijksma.langer.shondin:2004,dijksma.luger.shondin:2009,dijksma.luger.shondin:2010,
...
\begin{remark}\label{L68}
	...
	\item
		...
		(see, e.g.\ \cite{dijksma.langer.shondin:2004,kaltenbaeck.woracek:polya}):
		\begin{Enumeratealph}
		\item
...
\begin{theorem}\label{L9}
	...
	holds.
%	Let $q\in\Hol(\bb C^+)$.  Then $q\in\ran\ms C_\kappa$
%	if and only if $q\in\mc N_{\le\kappa}^{(\infty)}$.
\end{theorem}
...
Next we give a supplement to \cref{L9}, namely a continuity property
of $\ms C_\kappa$; see \cref{L71} below.
This result is a variant of a classical theorem of J.~Grommer and H.~Hamburger;
see the discussion below \cref{L71}.
...
\begin{proof}
	...
	\end{equation}
	and that $\lim_{n\to\infty}\mu_n=\mr\mu$ w.r.t.\ $w^*$ in $C_0(\bb R,\omega_\kappa)'$.
...
The following functions play also an important role in the current section,
...
\begin{theorem}\label{L16}
	...
	\end{equation}
%	\item
%		$\alpha$ and $\omega$ satisfy % either \eqref{L89} or \eqref{L90}.
%		\begin{alignat}{2}
%			& \alpha\notin 2\bb Z+1
%			\quad\text{and}\quad &
...
%	If $\alpha\in(-1,1)$, then again $q\in\mc N_0$.
	Now assume that $\alpha>-1$.  The function $Q_{\alpha,\omega}$ can be extended
...
In the following lemma we use the standard notation $z_0M=\{z_0z:z\in M\}$
...
\begin{proof}[Proof of \cref{L91}.]
	...
	\]
	exists and is non-zero.  Hence (i) is satisfied with $M=\{sz_0:s\in(0,\infty)\}$.
	...
	Since the sequence $(r_n)$ was arbitrary, it follows again from \eqref{L96} that
	\[
...
\begin{proof}[Proof of \cref{L16}.]
	...
	\end{equation}
	Since the sequence $(r_n)$ was arbitrary, relations \eqref{L99} and \eqref{L100}
	imply \eqref{L98}.
...
e.g.\ \cite{pleijel:1962, selander:1963},
...
\begin{equation}\label{L60}
	\kappa(\mu) \DE \inf\biggl\{n\in\bb N_0: \int_{\bb R}\frac{\D\mu(t)}{(1+|t|)^{2n+2}}<\infty\biggr\} < \infty.
\end{equation}
...
\begin{remark}\label{L204}
	Suppose that \cref{L171} holds and that $\kappa=0$, 
...
\begin{equation}\label{L161}
	\ell(\mu) \DE \inf\biggl\{n\in\bb N_0: \int_{\bb R}\frac{\D\mu(t)}{(1+|t|)^{2n+1}}<\infty\biggr\}.
\end{equation}
...
\item
	...
	\end{alignat*}
\item
	Instead of the assumption $\ell=\ell(\mu)$ in the lemma, 
...
\begin{proof}[Proof of \cref{L167}]
	...
	which implies that $\beta=0$.  From \cref{L3}\,(ii) we obtain that
	\[
	...
	The proves of \eqref{L168} when $\beta=0$ the integrals in \eqref{L188} are infinite 
...
\begin{theorem}\label{L28}
	Let $\mu$ and $q$ be as in \cref{L171,L173,L176}.
	Further, let $\beta\in[0,\infty)$ and consider the following statements:
	\begin{Enumeratealph}
	\item
	...
	\item
		...
		\begin{equation}\label{L32}
			\lim_{r\to\infty}\frac{q(rz)}{f(r)} = Q_{\beta-1,\omega}(z)
...
\begin{Enumerate}
\item
	Note that, in the case $\beta\in\bb N$, one has $f(r)\gg\frac{\mu((-r,r))}{r}$.
\item
...
\begin{example}\label{L212}
	...
	\[
		...
		= \tilde\tau([0,t)).
	\]
	By \cref{L54}\,(i) and \cref{L3}\,(i) we have
	\begin{align}
...
%
%
\section{Regularly varying functions and \newline some theorems of Karamata}
\label{L115}
%
%
...
\begin{proposition}\label{L33}
	...
	\item
		...
		is regularly varying with index $\alpha-\rho$,
		and the same holds for $x\mapsto\int_x^\infty\frac{\D\sigma(t)}{t^\rho}$
		unless $\alpha=0$.
	\end{Enumerate}
\end{proposition}
...
\item
	...
	The integral $\int_1^x t^{\gamma-1}\sigma([1,t))\DD t$ converges
	and $\lim_{x\to\infty}x^\gamma\sigma([1,x))=0$.
	It follows from \eqref{L104} that
	\[
...
\begin{example}\label{L69}
	Define the discrete measure $\sigma$ supported on $\{e^k:k\in\bb N\}$ 
...
\noindent
We use Karamata's theorem about the Stieltjes transform
\cite{karamata:1931a,karamata:1931} in a formulation that includes a boundary case;
...
\begin{theorem}[Karamata]\label{L1}
	...
	and consider its Stieltjes transform
...
\begin{remark}\label{L3}
	...
	\item
		If $\mu$ is a finite measure, then the function $x\mapsto\mu([0,x))$
		is slowly varying for trivial reasons.
		\Cref{L1} does apply in this situation, but gives
		only the --- anyway obvious --- fact that
		\begin{equation}\label{L39}
			\lim_{x\to\infty}x\int_{[0,\infty)}\frac{\D\mu(t)}{t+x}
			= \mu\bigl([0,\infty)\bigr),
		\end{equation}
		which holds also when $\mu$ is an infinite measure.
	\item
		It follows from item \eqref{L39} and \eqref{L105} that, if the measure $\mu$ is infinite,
		then
		\[
			\frac{1}{x} \ll \ms S[\mu](x) \ll 1
		\]
		as $x\to\infty$.
	\end{Enumerate}
\end{remark}
...
\begin{proof}
	...
	Hence \eqref{L189} follows from \cref{L3}\,(iii).
	...
	\[
		...
		\gg \sigma([0,x)).
	\]
	Hence we can apply \cref{L54} and \cref{L3}\,(i) to deduce that
	\begin{align*}
...
\bibitem[Ple62]{pleijel:1962}
A.~Pleijel, 
A bilateral Tauberian theorem, 
\textit{Ark. Mat.} 4 (1962), 561--571
