%**************************************************************************
%***                            Last Change: Sun 21 Jan 2024 11:40
%***   < KARA.TEX >
%***
%**************************************************************************
...
%\begin{flushleft}
	\noindent
	{\Large\textbf{Karamata's theorem for regularised Cauchy transforms}}
	\\[5mm]
	\textsc{
	\hspace*{1ex}
	Matthias Langer
...
\begin{equation}
\label{L61}
	\mu\bigl((\alpha,\beta)\bigr)=
	\lim_{\delta\downarrow0}\lim_{\varepsilon\downarrow 0}
	\frac{1}{\pi}\int_{\alpha+\delta}^{\beta-\delta}\Im C[\mu](x+i\varepsilon)\DD x
\end{equation}
for $-\infty<\alpha<\beta<\infty$. 
This formula can be seen as relating the local behaviour of $\mu$ at a point or on a bounded interval 
in $\bb R$ with the local behaviour of $C[\mu]$ around this point or interval: 
in order to evaluate the right-hand side of \cref{L61} only the values of $C[\mu](z)$ for $z$ in some rectangle 
$(\alpha,\beta)\times(0,\varepsilon)\subseteq\bb C^+$ have to be known. 
...
and G.H.~Hardy and J.E.~Littlewood \cite{hardy.littlewood:1930} proved that, for each $\gamma\in(-1,0)$,
\begin{equation}
\label{L117}
	S[\mu](-x)\sim cx^\gamma\quad\Leftrightarrow\quad\mu([0,t))\sim c't^{\gamma+1},
\end{equation}
...
or \cite{mcclure.wong:1978} where asymptotic expansions with infinitely many terms instead of a monomial 
on the right-hand side of \cref{L117} is considered. 
...
grow faster at infinity: instead of satisfying $\int_{\bb R}(1+|t|)^{-1}\DD\mu(t)<\infty$ they have only power bounded tails, meaning that
...
\cite{bennewitz:1989}; however, the proof given contains a mistake. 
...
\begin{Ilist}
\item 
	We set $\bb N\DE\{1,2,\ldots\}$, $\bb N_0\DE\bb N\cup\{0\}$, and let $\bb C$ be the field of complex numbers.
\item 
...
\item 
	When we speak of a ``measure'', we always mean positive Borel measure (unless explicitly specified differently). 
\item
	Throughout the rest of the paper we use the Stieltjes transform $\ms S[\mu]$ as defined in \eqref{L22},
	where we use a different sign convention from the one used in \eqref{L300}.
\end{Ilist}
...
\begin{proposition}\label{L6}
	...
	\item 
		...
		$\int_{\bb R}(1+|t|)^{-1}\D\mu(t)<\infty$ if and only if
		\[
	...
	\item 
		...
		$\int_{\bb R}(1+t^2)^{-1}\D\mu(t)<\infty$ if and only if
		\[
			\forall z\in\bb C^+\DP \Im q(z) \ge 0.
		\]
	\end{Enumerate}
...
\item
	...
	\[
		\forall \alpha,\beta\in\bb R,\,\alpha<\beta\DP
		\lim_{\delta\downarrow0}\lim_{\varepsilon\downarrow 0}
		\frac{1}{\pi}\int_{\alpha+\delta}^{\beta-\delta}\Im q(x+i\varepsilon)\DD x
		= \mu\bigl((\alpha,\beta)\bigr);
	\]
...
\begin{definition}\label{L8}
	...
	\item
		...
		\begin{Itemize}
		\item
			$\mu$ is a measure on $\bb R$ that satisfies
...
\medskip\noindent
In order to represent polynomials with real coefficients as regularised Cauchy transforms, 
we have to include pairs such as $(0,z^{2\kappa})$ in $\bb E_{\leq\kappa}$. 
For this reason we cannot speak of ``the leading coefficient of $p$'' in \cref{L77}.
...
\label{L39}
	...
	$\int_{\bb R}(1+|t|)^{-(2\kappa+1)}\D\mu(t)<\infty$ is satisfied,
...
Let $\sigma$ be a finite measure on $\bb R$, let $\alpha,\beta\in\bb R$ with $\alpha<\beta$,
and let $f$ be an analytic function on a neighbourhood of $[\alpha,\beta]$.
...
\begin{equation}\label{L75}
	\alpha+\delta-i\varepsilon\rightsquigarrow \beta-\delta-i\varepsilon
	\quad\text{and}\quad
	\beta-\delta+i\varepsilon\rightsquigarrow \alpha+\delta+i\varepsilon.
\end{equation}
Then
\begin{equation}\label{L74}
	...
	= \int_{(\alpha,\beta)}f(t)\DD\sigma(t).
\end{equation}
...
\begin{lemma}\label{L21}
	...
	\item
		...
		for $\alpha,\beta\in\bb R$ with $\alpha<\beta$ we have
		\begin{equation}\label{L73}
			\mu\bigl((\alpha,\beta)\bigr)
			= \lim_{\delta\downarrow 0}\lim_{\varepsilon\downarrow 0}
			\frac{1}{\pi}\int_{\alpha+\delta}^{\beta-\delta} \Im q(t+i\varepsilon)\DD\mu(t).
		\end{equation}
	\item
		Let $\kappa'>\kappa$.
...
(ii)
	...
	Moreover, let $\alpha,\beta\in\bb R$ with $\alpha<\beta$ and let
	...
	\begin{align*}
		...
		\\
		&=
		\int_{(\alpha,\beta)}(1+t^2)^{\kappa+1}\cdot\frac{\D\mu(t)}{(1+t^2)^{\kappa+1}}
		= \mu\big((\alpha,\beta)\big).
	\end{align*}
...
\begin{remark}\label{L68}
	...
	\item
		...
		\item
			there exist $m\in\bb N_0$, a real polynomial $p$ and a measure $\sigma$
			on $\bb R$ such that $\int_{\bb R}(1+t^2)^{-1}\D\sigma(t)<\infty$ and
		...
		\item
			...
			\[
				q(z) = q_0(z)\prod_{j=1}^n(z-\beta_j)^{\rho_j}(z-\qu{\beta_j})^{\rho_j}.
			\]
		\end{Enumeratealph}
...
\begin{proof}
	...
	and a measure $\sigma$ such that $\int_{\bb R}(1+t^2)^{-1}\D\sigma(t)<\infty$.
...
\begin{proof}
	...
	\Enumref{3} The statement follows from what we have shown so far and from the fact
...
\item
	...
	let $(\mu_n)_{n\in\bb N}$ be a sequence of finite measures on $\bb R$ whose total variations
...
\item
	...
	The condition in \eqref{L72} says that $(\mu_n)_{n\in\bb N}$ converges vaguely,
	i.e.\ w.r.t.\ $w^*$ in $C_{00}(\bb R)'$.
	However, at some places in the literature
	it is claimed that (a) in item (i) implies that $(\mu_n)_{n\in\bb N}$ converges weakly,
...
\item
	...
	$\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu_n(t)$ are automatically
...
The proof given in that paper contains a mistake\footnote{%
In particular, in that paper it is claimed that the relation $\int_{\bb R}(t-z)^{-3}\D\tau(t)=0$
...
\begin{theorem}\label{L91}
	...
	and let $f:[x_0,\infty)\to(0,\infty)$ with $x_0>0$ be measurable.
	Then the following statements are equivalent:
	\begin{Enumerate}
	\item
...
\begin{proof}
	...
	for all $\alpha,\beta\in\bb R$ with $\alpha<\beta$, we have
	\begin{align*}
		\mu_{\langle r\rangle}\bigl((\alpha,\beta)\bigr)
		&= \lim_{\delta\downarrow 0}\lim_{\varepsilon\downarrow 0}
		\frac{1}{\pi}\int_{\alpha+\delta}^{\beta-\delta} \Im q_{\langle r\rangle}(t+i\varepsilon)\DD t
		= \lim_{\delta\downarrow 0}\lim_{\varepsilon\downarrow 0}\frac{-1}{2\pi i}
		...
		\\[1ex]
		&= \int_{(\alpha,\beta)}\big(1+(rs)^2\big)^{\kappa+1}\cdot
...
distribution functions $t\mapsto\mu([0,t))$ and $t\mapsto\mu((-t,0))$.
...
\begin{lemma}\label{L159}
	Let $\mu$ be a measure on $\bb R$.  For $\gamma>0$ we have
	\[
...
\begin{assumption}\label{L171}
	Let $\mu$ be a measure on $\bb R$ such that $\ms p(\mu)<\infty$
...
\medskip\noindent
Note that, by \cref{L8}\,(i), we have $c_{2\kappa+1}\ge\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu(t)$.
...
\begin{proposition}\label{L172}
	...
	\begin{Enumerate}
	\item
		If $c_{2\kappa+1}>\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu(t)$, then
		\[
...
\item
	...
	when $c_{2\kappa+1}>\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu(t)$, 
	and the first relation in \eqref{L210} 
	when $c_{2\kappa+1}=\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu(t)$.

\item
	For the rest of the proof assume 
	that $c_{2\kappa+1}=\int_{\bb R}(1+t^2)^{-(\kappa+1)}\D\mu(t)$.
...
\item	
	...
	$\int_{\bb R}(1+t^2)^{-\kappa}\D\mu(t)=\infty$.
...
\begin{lemma}\label{L217}
	...
	and define the measure $\tau_\kappa$ on $[0,\infty)$ by
...
to the symmetrised distribution function $\mu((-t,t))$ of the measure $\mu$.
...
\begin{proposition}\label{L50}
	Let $\mu$ be a measure on $\bb R$ such that $\int_{\bb R}(1+t^2)^{-1}\D\mu(t)<\infty$,
	let $c_0\in\bb R$ and set $q\DE c_0+\widetilde C[\mu]$.
...
Let $\mu$ be a measure on $\bb R$ such that there exists $\ell\in\bb N_0$ with
...
\begin{proof}
	...
	(i)
	When $\ms p(\mu)$ is odd, then $\int_{\bb R}(1+|t|)^{-(2\kappa+1)}\D\mu(t)<\infty$.
...
\begin{remark}\label{L154}
	Let $\mu$ be a measure on $\bb R$ and define $\ms p(\mu)$, $\kappa(\mu)$ 
...
\begin{lemma}\label{L163}
	Let $\mu$ be a measure on $\bb R$ and define the
	measures $\mu^+$ and $\mu^-$ on $\bb R$ by
...
\begin{proposition}\label{L167}
	Let $\mu$ be a measure on $\bb R$, set $\ell\DE\ell(\mu)$
...
\begin{proof}[Proof of \cref{L167}]
	...
	The proofs of \eqref{L168} when $\beta=0$ and the integrals in \eqref{L188} are infinite 
...
\begin{lemma}\label{L155}
	Let $\mu_1$ and $\mu_2$ be measures on $\bb R$ and set $\ell\DE\ell(\mu_2)$.
...
\begin{theorem}\label{L28}
	...
	\item
		...
		\]
		If $\beta=0$ or $\int_{\bb R}(1+|t|)^{-\beta}\D\mu(t)<\infty$, then
		\[
...
\begin{example}\label{L212}
	...
	\[
		...
		\qquad k\in\bb N.
	\]
	Let $\tilde\tau$ be the measure on $[0,\infty)$ 
...
\begin{example}\label{L53}
	Let $\gamma\in(0,1)$ and define the measure $\mu=\lambda+\nu$ on $\bb R$
...
\begin{theorem}\label{L40}
	...
	\begin{Enumerate}
	\item
		Assume that $\mu=\mu_0+\mu_1+\mu_2$ with measures $\mu_0$, $\mu_1$ and $\mu_2$ 
		...
		\item
			if $\int_{\bb R}(1+|t|)^{-\beta}\D\mu_1(t)<\infty$, then
			\[
...
\item
	...
	when $\int_{\bb R}(1+|t|)^{-\beta}\D\mu_1(t)=\infty$,
	\[
	...
	show that also in the case when $\int_{[0,\infty)}(1+|t|)^{-\beta}\D\mu_1(t)<\infty$,
...
\item
	...
	that $\int_{\bb R}(1+|t|)^{-\beta}\D\mu_2(t)<\infty$
...
\item
	...
	when $\int_{\bb R}(1+|t|)^{-\beta}\D\mu_1(t)=\infty$, i.e.\ $\ell=\kappa+1$,
	\[
		...
		= -(-1)^\kappa\eta_+;
	\]
	a similar calculation proves relation \eqref{L135} also in the case 
	when $\int_{\bb R}(1+|t|)^{-\beta}\D\mu_1(t)<\infty$.
...
\item
	...
	Choose $\rho\in(\max\{\gamma,\beta-1\},\beta)$ and define the measure $\nu$ 
	by $\nu((-\infty,0))=0$ and $\nu([0,t))=t^\rho$ for $t>0$.
...
\begin{example}\label{L51}
	Let $\mu$ be a measure on $\bb R$ that satisfies
	...
	\end{aligned}
	\end{equation}
	as $t\to+\infty$ with $a_+,a_-\ge 0$ and $\gamma<1$.
	...
	\end{alignat*}
	Since $\int_{\bb R}(1+|t|)^{-1}\D\mu(t)=\infty$ (see \cref{L159}), assumptions (a)--(d)
...
\begin{proposition}\label{L33}
	Let $\sigma$ be a measure on $[1,\infty)$, which is not the zero measure.
	...
	\item
		Let $\rho>0$ and assume that $\int_{[1,\infty)}t^{-\rho}\D\sigma(t)<\infty$.
		...
		\[
			...
			= \alpha.
		\]
		The function $x\mapsto\int_x^\infty t^{-(\rho+1)}\sigma([1,t))\DD t$
		is regularly varying with index $\alpha-\rho$; if $\alpha\neq 0$, 
		the same holds for $x\mapsto\int_x^\infty t^{-\rho}\D\sigma(t)$.
	\end{Enumerate}
\end{proposition}
...
\item
	...
	\[
		...
		\bigg).
	\]
	By \cref{L36}\,(ii) the integral $\int_{[1,x)}t^{-(\rho+1)}\sigma([1,t))\DD t$
...
let $\mu$ be a measure on $[0,\infty)$ which 
satisfies $\int_{[0,\infty)}(1+t)^{-1}\DD\mu(t)<\infty$;
...
from the one in \eqref{L300} (i.e.\ different from the convention for the Cauchy transform) 
in order to obtain functions that are defined on the positive half-line.

\begin{remark}
\label{L26}
	Let $\mu$ be a measure on $[0,\infty)$
	which satisfies $\int_{[0,\infty)}(1+t)^{-1}\DD\mu(t)<\infty$. 
...
\begin{theorem}[Karamata]\label{L1}
	Let $\mu$ be a measure on $[0,\infty)$, which is not the
	zero measure and satisfies $\int_{[0,\infty)}(1+t)^{-1}\DD\mu(t)<\infty$.
...
\hfill
	...
	\item
		...
		\begin{align}
			...
			\label{L150}
		\end{align}
		Let $\sigma$ be the measure on $[0,\infty)$ with density $\ms L[\mu]$, i.e.\
		\[
		...
		\]
		the finiteness of the last integral follows easily from the
		assumption $\int_{[0,\infty)}(1+t)^{-1}\D\mu(t)<\infty$.
...
\begin{lemma}\label{L54}
	Let $\mu_1$ and $\mu_2$ be measures on $[0,\infty)$
	such that $\mu_2$ is not the zero measure and that
	$\int_{[0,\infty)}(1+t)^{-1}\D\mu_i(t)<\infty$ for $i\in\{1,2\}$.  
	\begin{Enumerate}
	\item
...
\begin{proof}
	...
	Let $r_0>0$ be such that $\mu_i([0,r_0))>0$ for $i\in\{1,2\}$.
...
\begin{proposition}\label{L122}
	...
	\end{equation}
	and define the measure $\sigma$ on $[0,\infty)$ by 
	\[
...
\begin{proof}
	...
	\end{equation}
	Define the measure $\mu$ on $[0,\infty)$
	by $\D\mu(t)=t^{\gamma-1}\nu([0,t))\DD t$.  Then
	\[
...
\begin{lemma}\label{L121}
	Let $\nu_1,\nu_2$ be measures on $[0,\infty)$, let $h:[0,\infty)\to[0,\infty)$
	...
	\begin{equation}\label{L192}
		...
		\int_{[0,\infty)}\frac{h(t)}{1+t}\DD\nu_2(t) < \infty
	\end{equation}
	and define the measures $\sigma_i$ on $[0,\infty)$ by
...
\bibitem{derkach.hassi.snoo:1999}
...
\bibitem{dijksma.langer.luger.shondin:2000}
...
\bibitem{dijksma.langer.shondin:2004}
...
\bibitem{dijksma.luger.shondin:2009}
...
\bibitem{dijksma.luger.shondin:2010}
A.~Dijksma, A.~Luger and Y.~Shondin, 
...
{\cyr Diskretnye i nepreryvnye graniqnye zadaqi} (Russian translation) Izdat. ``Mir'', Moscow, 1968, pp.~ 629--647. 
English translation: Amer. Math. Soc. Transl. (2) 103 (1974), 1--19 


\bibitem{kaltenbaeck.woracek:polya}
...
\bibitem{langer.woracek:ninfrep}
M.~Langer and H.~Woracek, 
Distributional representations of $\mathcal N_\kappa^{(\infty)}$-functions, 
\textit{Math. Nachr.} 288 (2015), 1127--1149

\bibitem{langer.woracek:sinham}
M.~Langer and H.~Woracek, 
Direct and inverse spectral theorems for a class of
canonical systems with two singular endpoints, 
in: \textit{Function Spaces, Theory and Applications}, 
Fields Institute Communications, vol.~87, Springer, Cham, 2023, pp.~105--205.

\bibitem{mcclure.wong:1978}
