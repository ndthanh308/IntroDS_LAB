...
The existence of the Beilinson--Bloch height pairing is still conjectural. It is expected, however, that it decomposes into a sum of local heights whose non-archimedean terms are closely related to intersection numbers on integral models. For this reason, we consider integral models $\wt{\mr{Sh}_H}\hookrightarrow \wt{\mr{Sh}_G}$ of the given Shimura varieties. Our interest then lies with the intersection numbers $I(f) = \langle \wt{\mr{Sh}_H},\, f*\wt{\mr{Sh}_H} \rangle$ for varying Hecke operators $f\in C^\infty_c(G(\mbA_f))$, and their relations with leading terms of $L$-functions.
...
\begin{conj}[Fundamental Lemma]\label{conj:FL_intro}
...
Conjecture \ref{conj:FL_intro} specializes to the Guo--Jacquet fundamental lemma (FL) from \cite{Guo} if $D = M_{2n}(F)$. By work of N. Hultberg and the second author \cite{HM}, Conjecture \ref{conj:FL_intro} is also known if the Hasse invariant of $D$ is $1/2$: The proof in this case is by reduction to the base change FL and to the Guo--Jacquet FL. Our main result on Conjecture \ref{conj:FL_intro} in the present paper is as follows.

\begin{thm}\label{thm:FL_intro}
Assume that $D$ is a division algebra of degree $4$. Then Conjecture \ref{conj:FL_intro} holds.
\end{thm}

Note that if $D$ is a division algebra of degree $2n$, then $\Orb(g, f_D)$ is either $0$ or an integer divisor of $n$ (Proposition \ref{prop:orb_int_division}).
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*), topsep=2pt, itemsep=2pt]
\item Concerning the orbital integral side, we combine three techniques to explicitly determine all occurring $\Orb(γ, f'_\Par)$, $\Orb(γ, f'_\Iw)$ and $\del(γ, f'_\Iw)$. First, we work out these orbital integrals for all hyperbolic orbits. In this situation, the computation can be reduced to a Levi subgroup and hence to $GL_2$. Second, we establish a germ expansion principle that allows to write each orbital integral as a linear combination of a principal germ and a unipotent germ. The principal germ can be described explicitly in all situations. Third and finally, we use the results for hyperbolic orbits and the linear relations amongst the various germs to also determine the remaining orbital integrals. A summary of the final results can be found in \S\ref{s:main_analytic}.
...
\end{prop}
Set $\mcI(g) = \mcM_C \cap g \cdot \mcM_C$. We are left to describe $\mcI(g)^{\mr{pure}}$ and $\mcI(g)^{\mr{art}}$, where the notation is meant in the sense of Proposition \ref{prop:intro_int_formula}.
\begin{enumerate}[wide, resume, labelindent=0pt, labelwidth=!, label=(\arabic*), topsep=2pt, itemsep=2pt]
...
\end{equation}
One of our main auxiliary results (Theorem \ref{thm:classification_multiplicity_function}) describes this function $Λ\mapsto m(g, Λ)$ in terms of $g$.
...
We are grateful to Michael Rapoport and Wei Zhang for their continued interest in this project and comments on an earlier version of our text. We also thank Johannes Anschütz and Mingjia Zhang for helpful discussions.
...
Let $F$ be a field, let $E/F$ be an étale quadratic extension and let $D/F$ be a CSA of degree $2n$. Assume that there exists, and fix, an $F$-algebra embedding $E\to D$ that makes $D$ into a free $E$-module. (The latter condition is only relevant if $E\iso F\times F$.) Since $E/F$ is étale, the left and right multiplication actions of $E$ on $D$ provide an eigenspace decomposition $D = D_+ \oplus D_-$ into $E$-linear and $E$-conjugate linear elements. It satisfies $\dim_F(D_+) = \dim_F(D_-)$. We denote the two components of an element $g\in D$ by $g_+$ and $g_-$.
...
Let $g\in G$ be an element such that also $g_+$ lies in $G$. It is easily checked after base change to $\ov{F}$ that the reduced characteristic polynomial of $(g_+^{-1}g_-)^2$ is a square, see \eqref{eq:matrix_example_central} below. We define the invariant of $g$ as its unique monic square root:
\begin{equation}\label{eq:def_invariant}
\Inv(g; T) = \mr{charred}_{D/F}\left((g_+^{-1}g_-)^2;\ T\right)^{1/2} \in F[T].
\end{equation}
This is a monic polynomial of degree $n$. It satisfies $\Inv(g; 1) \neq 0$ because $g_+^{-1}g = 1 + g_+^{-1}g_-$ is invertible.
\end{defn}
...
A slightly different definition of invariant is given in \cite[Definition 1.1]{Li}. It is defined for every $g\in G$ and again a monic polynomial of degree $n$; let us call it $\Inv'(g)$. Assuming that $g_+$ lies in $G$, the two definitions are related by the Moebius transformation
...
Let $g\in G_{\mr{rs}}$ be regular semi-simple. We denote by $B_g\subseteq D$ the $F$-subalgebra that is generated by $E$ and $g^{-1}Eg$. We denote by $L_g \subset B_g$ its center. Set $z_g = g_+^{-1}g_-$. Up to isomorphism, these objects only depend on the orbit $HgH$ because
...
\emph{Claim: The elements $1, z, \ldots, z^{2n-1}, ζ, ζz, \ldots, ζz^{2n-1}$ form an $F$-vector space basis of $B_g$.} This may be shown after base change to $\ov{F}$ which puts us into the situation of Example \ref{ex:invariants}. Then we may assume that $g$ is given as a block matrix $g = \left(\begin{smallmatrix} 1 & w \\ 1 & 1\end{smallmatrix}\right)$ as in \eqref{eq:simple_representative}. In this specific case we have
\begin{equation}
z_g = \begin{pmatrix}
 & w \\ 1 &
\end{pmatrix} \quand B_g = (F\times F)[z_g].
\end{equation}
The claim then follows from the fact that the characteristic polynomial of $w$, which equals $\Inv(g; T)$, is separable by assumption and hence agrees with the minimal polynomial. The identities $L_g = F[z_g^2]$ as well as $EL_g \iso E\tensor_FL_g$ and $B_g = EL_g[z_g]$ all follow from the claim.
...
For example, the element $1 + z\in B_δ$ from Definition \ref{def:universal_quaternion} has invariant $\Inv(1+z) = δ$ with respect to the embedding $E\to B_δ$ that comes by construction.
...
\item We denote by $D$ a CSA of degree $2n$ over $F$. We fix an embedding $E\to D$ and then define $C = \mr{Cent}_D(E)$. We use the notation $D_-$ and $g = g_+ + g_-$ in the same way as before. We define
...
The stabilizer of a regular semi-simple element $γ$ is isomorphic to the torus $L_γ^\times$, where $L_γ = L[z_γ^2]$ is the étale $F$-algebra of degree $n$ from Proposition \ref{prop:quaternion_algebra} (1). In this situation, we endow $(H'\times H')_γ$ with the Haar measures such that $O_{L_γ}^\times$ has volume $1$. We also normalize the Haar measure on $H'\times H'$ such that a maximal compact subgroup has volume $1$.
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
\item Two regular semi-simple elements $γ\in G'_{\mr{rs}}$ and $g\in G_{\mr{rs}}$ (resp. their orbits) are said to match if $\Inv(γ) = \Inv(g)$. Note that in this case also $L_γ \iso L_g$ so that we have chosen compatible Haar measures on the stabilizers $(H'\times H')_γ$ and $(H\times H)_g$.

\item A test function $f'\in C^\infty_c(G')$ is called a transfer of $f\in C^\infty_c(G)$ if, for all $γ\in G'_{\mr{rs}}$,
\begin{equation}
...
\subsection{The Fundamental Lemma Conjecture}
\label{ss:FL}
In the following we write $C = \mr{Cent}_D(E)$.
...
\end{equation}
Let $h_1\in H'$ be any element with $|h_1|^{-s} = q^{-2ns/\ell}$ and define $f'_D \in C^\infty_c(G')$ by
\begin{equation}
f_D'(γ) := f_D'^\circ(h_1γ).
\end{equation}
By \eqref{eq:orb_int_trafo}, the orbital integrals of $f_D'^\circ$ and $f_D'$ are related by
\begin{equation}\label{eq:norm_orb_int}
\Orb(γ, f_D', s) = q^{-2ns/\ell}\Orb(γ, f_D'^\circ, s)
\end{equation}
...
\end{prop}
\begin{proof}
In the given situation, $f_D$ is the indicator function of the units $O_D^\times$ of the unique maximal order in $D$. The centralizer $C = \mr{Cent}_E(D)$ is a CDA of degree $n$ over $E$, so $v_D(C^\times) = 2\mbZ$. It moreover holds that $v_D(D_-\backslash \{0\}) = 2\mbZ + 1$ and hence follows that $(O_C^\times\, g\,O_C^\times) \cap O_D^\times \neq \emptyset$ if and only if $v_D(g) \in 2\mbZ$. This is also equivalent $v_D(1 + z_g) = 0$ and we obtain from Definition \ref{def:orb_int_geometric} that
...
Let $g\in G_{\mr{rs}}$ be a regular semi-simple element. Then $ε_D(\Inv(g)) = 1$.
\end{lem}
\begin{proof}
...
\Orb(γ, f'_D, s) = q^{-2ns/\ell} \sum_{Λ_\bullet \in L_γ^\times \backslash \mcL(γ)} [O_{L_γ}^\times : \mr{Stab}(Λ_\bullet)]\ \Omega(γ, Λ_0, s).
\end{equation}
The next few lemmas study this expression in more detail.
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
\item The following operator $Z_γ$, defined on lattice chains in $F^{2n}$, defines an automorphism of $\mcL(γ)$:
...
\end{equation}
\item Moreover, $Z_γ$ commutes with the $L_γ^\times$-action on $\mcL(γ)$ and satisfies
...
\end{lem}
\begin{proof}
This follows directly from the definition of $\Omega(γ, Λ, s)$: Assume that $Λ = h_2O_F^{2n}$ and that $γΛ = h_1O_F^{2n}$. Let $\left(\begin{smallmatrix} a & b \\ c & d \end{smallmatrix}\right) = h_1^{-1}γh_2$. Then
$$\begin{array}{rlcrl}
v(a) \!\!\!\! & = [(γΛ)_+ : Λ_+] & \ \ & v(b) \!\!\!\! & = [(γΛ)_+ : zΛ_-]\\
v(c) \!\!\!\! & = [(γΛ)_- : zΛ_+] & \ \ & v(d) \!\!\!\! & = [(γΛ)_- : Λ_-].
\end{array}$$
Applying the definition \eqref{eq:transfer_lattice} and substituting this in the definition of $\Omega$ (see Definition \ref{def:transfer_factor}) proves the lemma.
\end{proof}
...
\end{equation}
We are left to take care of the shift in \eqref{eq:def_Z_gamma}. Assume that $Λ'\subseteq Λ$ is a sublattice that also has the property that both $Λ'$ and $γΛ'$ are $O_K$-stable. Define integers $a_{\pm}$ and $b_{\pm}$ by the identities
...
\end{cases}
\end{equation}
The sign $(-1)^{2n/\ell}η(\det(z))$ equals $ε_D(γ)$ by Lemma \ref{lem:sign_alternative}, so the proof of \eqref{eq:omega_Z_gamma} is complete.
\end{proof}
...
\end{equation}
Note that $G$ and $G^{\mr{op}}$ have the same underlying set which implies that $C^\infty_c(G) = C^\infty_c(G^{\mr{op}})$. Moreover, $H$ and $H^{\mr{op}}$ have the same underlying set as well and the definitions of $g\in G$ being regular semi-simple, of the invariant $\mr{Inv}(g;T)$, and of the orbital integral $\Orb(f, g)$ are all unchanged when taking them for opposed CSAs.

\subsection{Local Shimura Data}

Let $\breve F$ be the completion of a maximal unramified extension of $F$. Let $O_{\breve F}$ denote its ring of integers and let $\mbF$ be its residue field. The Frobenius automorphism of $\breve F$ is denoted by $σ:\breve F\to \breve F$.

By $F$-isocrystal, or simply isocrystal, we mean a pair $\bN = (N, \bF)$ that consists of a finite-dimensional $\breve F$-vector space $N$ and a $σ$-linear automorphism $\bF$. The Verschiebung of $\bN$ is defined as $\bV = π\bF^{-1}$. Height, dimension and slope of $\bN$ are all meant in the relative sense with respect to $F$: The height $\mr{ht}(\bN)$ is the $\breve F$-dimension of $N$, the dimension $\dim(\bN)$ is the integer $v_{\breve F}(\bV)$, and the slope is their ratio $\dim(\bN)/\mr{ht}(\bN)$. Note that $\dim(\bN)$ might be negative.
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
\item By $C$-isocrystal, we mean a pair $(\bN_+, ι)$ that consists of an isocrystal $\bN_+$ and an $F$-linear $C$-action $ι:C\to \End(\bN_+)$ with the following numerical conditions: The height of $\bN_+$ is $2n^2 = \dim_F(C)$, the dimension of $\bN_+$ is $n$, and the slopes of all subisocrystals of $\bN_+$ lie in the interval $[0,1]$.
\item By $D$-isocrystal, we mean a pair $(\bN, κ)$ that consists of an isocrystal $\bN$ and an $F$-linear $D$-action $κ:D\to \End(\bN)$ with the following numerical conditions: The height of $\bN$ is $4n^2 = \dim_F(D)$, the dimension of $\bN$ is $2n$, and the slopes of all subisocrystals of $\bN$ lie in the interval $[0,1]$.
\end{enumerate}
\end{defn}
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
\item Two $C$-isocrystals (resp. two $D$-isocrystals) are isomorphic if and only if the underlying isocrystals are isomorpic. In particular, the functor \eqref{eq:Serre_tensor} defines an injective map on isomorphism classes.
\item A $D$-isocrystal $(\bN, κ)$ lies in the essential image of \eqref{eq:Serre_tensor} if and only if there exists an $F$-algebra map $E\to \End_D(\bN, κ)$.
\end{enumerate}
\end{lem}
\begin{proof}
(1) Let $\bN$ be any isocrystal. By the Dieudonné--Manin classification, $\End(\bN)$ is a product of CSAs over $F$. It then follows from the Skolem--Noether Theorem that any two $F$-algebra homomorphisms $C\to \End(\bN)$ are conjugate. In other words, there is at most one way (up to $C$-linear isomorphism) to define a $C$-action on $\bN$. The same argument applies to $D$-isocrystals. The injectivity of \eqref{eq:Serre_tensor} on isomorphism classes follows directly because $D\tensor_C \bN_+ \iso \bN_+^{\oplus 2}$ as isocrystal.
...
\begin{aligned}
B(G) &\ \overset{\iso}{\lr} \left\{(\bN, κ) \left\vert \text{\begin{varwidth}{\textwidth} $\bN$ an isocrystal of height $\dim_F(D)$\\
...
\end{equation}
Via \eqref{eq:Kottwitz_bijection}, the subset $B(G, μ_G)\subset B(G)$ of $μ$-admissible elements is in bijection with the isomorphism classes of the $D$-isocrystals from Definition \ref{def:isocrystals}.
...
Let $b\in B(G, μ_G)$ be any isomorphism class; denote by $(\bN_b, κ)$ the corresponding $D$-isocrystal. Giving an embedding $E\to D_b$ and an orbit $H_bgH_b \subset G_b$ of invariant $δ$ is the same as lifting $κ$ to a faithful action $\wt{κ}:P\to \End(\bN)$ up to $G_b$-conjugacy, cf. \eqref{eq:conjugacy_orbit}. The uniqueness of the pair $(b,\ H_b g H_b)$ is thus equivalent to the uniqueness (up to $P$-linear isomorphism) of an isocrystal $\bN$ of height $4n^2$, of dimension $2n$, with all slopes within $[0,1]$ and with a faithful $P$-action $\wt{κ}$.

Assume $(\bN, \wt{κ})$ is is such a pair. Then $\bN$ decomposes, $\bN = \prod_{i\in I}\bN_i$, into a product of isocrystals with faithful $P_i$-action. The $i$-th factor $P_i$ is a CSA over $L_i$ of degree $4n$ so the height of $\bN_i$ has to be an integer multiple of $4nn_i$. Since $\sum_{i\in I} 4nn_i = 4n^2 = \mr{ht}(\bN)$, the height of $\bN_i$ has to be exactly $4nn_i$. Furthermore, $\bN_i$ is necessarily isoclinic, say of slope $μ_i = d_i/4nn_i$. The uniqueness of $(\bN, \wt{κ})$ up to $P$-linear isomorphism is then equivalent to the vector $(d_i)_{i\in I}$ being uniquely determined by $P$.
...
An equivalent way to formulate \eqref{eq:special_C} and \eqref{eq:special_D} is as follows. Let $L/E$ be an unramified field extension of degree $n$ and fix an embedding $O_L\to O_C$. Then, given an action $ι:O_C\to \End(Y)$ or $κ:O_D\to \End(X)$, the Lie algebra $\Lie(Y)$ resp. $\Lie(X)$ becomes an $O_L\tensor_{O_F}\mcO_S$-module. Since $S$ was assumed to be an $O_{\breve F}$-scheme, there is an eigenspace decomposition
$$\Lie(Y) = \bigoplus_{φ\in \Hom_E(L, \breve F)} \Lie(Y)_φ\quad \text{resp.}\quad \Lie(X) = \bigoplus_{φ\in \Hom_F(L, \breve F)} \Lie(X)_φ.$$
Then \eqref{eq:special_C} resp. \eqref{eq:special_D} holds for all $x\in O_C$ (resp. all $x\in O_D$) if and only if each summand $\Lie(Y)_φ$ (resp. each summand $\Lie(X)_φ$) is locally free of rank $1$ as $O_S$-module. This equivalence can be checked on the local model.
\end{rmk}
...
$ρ:\ov{S}\times_{\Spec \mbF} \mbY \lr \ov{S}\times_S Y$ an $O_C$-linear quasi-isogeny\end{varwidth}}\right.\right\}.$$
...
$ρ:\ov{S}\times_{\Spec \mbF} \mbX \lr \ov{S}\times_S X$ an $O_D$-linear quasi-isogeny\end{varwidth}}\right.\right\}.$$
\end{defn}
...
\end{equation}
Note that $Π^2\in O_C$ and that $Π^{-1}O_CΠ = O_C$. The $O_D$-action on $O_D\tensor_{O_C}Y$ has the matrix description
...
\end{equation}
Here, the condition is meant in the sense that $ρTρ^{-1}$ is always a subset of the \emph{quasi}-endomorphisms $\End^0(Y)$ because $ρ$ is a quasi-isogeny. The functor $\mcZ(T)$ is representable by a closed formal subscheme of $\mcM_C$, see \cite[Proposition 2.9]{RZ1}. In exactly the same way, we define a closed formal subscheme $\mcZ(T)\subseteq \mcM_D$ whenever $T\subseteq \End^0(\mbX)$. We apply this construction to the subring $ι(O_E)\subseteq \End(\mbX)$ to obtain the closed formal subscheme $\mcZ(ι(O_E))\subset \mcM_D$. The purpose of these definitions was that we can now give a description of the image of $\mcM_C\to \mcM_D$.

\begin{prop}\label{prop:closed_immersion}
The morphism $\mcM_C\to \mcM_D$ is a closed immersion. Its image consists of all those points $(X, κ, ρ)\in \mcZ(ι(O_E))$ with the following two additional properties. Let $X = X_+ \oplus X_-$ be the eigenspace decomposition with respect to the $κ(O_E)\tensor_{O_F} (ρι(O_E)ρ^{-1})$-action.
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
...
%\end{equation}
The key observation for this is that, by Proposition \ref{prop:char_isogeny_class}, the geometric isogeny class of the generic fiber $X_η$ is uniquely determined by $\Inv(g;T)$ and hence equal to that of $\mbX$. In other words, the point-wise slope vector of $X$ on $\Spec A$ is constant. The perfection $A^{\mr{perf}} = \colim_{x\mapsto x^p} A$ of $A$ is again strictly henselian. In this situation, \cite[Lemmas 4.3.15]{CS} applies and shows that the pullback of $ρ$ to $A^{\mr{perf}}$ algebraizes. The map $A\to A^{\mr{perf}}$ is faithfully flat, so this implies that $ρ$ algebraizes.
\end{proof}
%
%
...
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*)]
\item Assume that $[b]\in B(H, μ_H)$ is such that $\bN_b$ has no étale part and assume that $g\in G_{b, \mr{rs}}$ is regular semi-simple. If $\mcI(g)\neq \emptyset$, then $z_g$ is topologically nilpotent.

\item Let $[b]\in B(H, μ_H)$ be any and let $g\in G_{b, \mr{rs}}$ be an element such that $z_g$ is topologically nilpotent. Then $\mcI(g) = \mcM_C \cap \mcZ(z_g)$.
\end{enumerate}
\end{prop}
\begin{proof}
...
\item Consider next the case that $D\iso M_n(D_{1/2})$. Then the main result of \cite{HM} reduces Conjecture \eqref{conj:ATC_explicit} to the linear AFL conjecture for $M_{2n}(F)$. In particular, the case $D\iso M_2(D_{1/2})$ is known by \cite{Li_future}.
...
O_F^\times & O_F & (π) & O_F\\
(π) & O_F^\times & (π) & (π)\\
O_F & O_F & O_F^\times & O_F\\
(π) & O_F & (π) & O_F^\times
\end{pmatrix}.
\end{equation}
Define $f'_\Par = 1_\Par$ and $f'_\Iw = (q+1)^4\,1_\Iw.$ These define the functions $f_D'^\circ$ from Definition \ref{def:test_function}
\begin{equation}
...
Let $δ = T^2 + δ_1 T + δ_0\in F[T]$ be a regular semi-simple invariant of degree $2$. The numerical invariant of $δ$ is the triple $(L, r, d)$ where
...
Let $γ\in G'_{\mr{rs}}$ be regular semi-simple with numerical invariants $(L, r, d)$. The parahoric orbital integral $\Orb(γ, f'_\Par)$ vanishes if $r$ is odd, or if $r \leq 0$, or if $r/2 + d \leq 0$. In all other cases, it is given by
...
\end{equation}
\end{thm}
Let $(L, r, d)$ be the numerical invariant of an element $γ\in G'_{\mr{rs}}$. We remark that the matching element $g$ in \eqref{eq:FL_intermediate_teaser} exists if and only if $L/F$ is a ramified field extension and $r$ odd.
...
A regular semi-simple element $γ\in G'_{\mr{rs}}$ is called hyperbolic if $L_γ\iso F\times F$. In this situation, the orbital integrals $\Orb(γ, f'_\Iw, s)$ and $\Orb(γ, f'_\Par, s)$ can be expressed in terms of much simpler orbital integrals for the Levi that is defined by $L_γ$. 

In the following we fix a hyperbolic element $γ\in G'_{\mr{rs}}$ of the form $γ = 1+z_γ$; set $z = z_γ$. We also fix an isomorphism $L_γ \iso F\times F$. Recall from \S\ref{ss:setting} that $K = \{\diag(a, a, b, b) \in M_4(F) \mid a, b\in F\}$ denotes the diagonal copy of $F\times F$, and recall from Proposition \ref{prop:quaternion_algebra} that $V$ is free as $K\tensor_FL_γ$-module.
...
We now extend Proposition \ref{prop:count_lattice_gluing} to the lattice chains from Definitions \ref{def:lattice_chains} and \ref{def:lattice_chains_gamma} when $n \leq 2$. More precisely, we are interested in the cases $(n,\ell) = (2, 2)$, $(2, 4)$ or $(1, 2)$ from these definitions.
...
\end{equation}
We remark that the original definitions of $\mcL(γ)$, $\mcP(γ)$ and $\mcL^j_\pm(γ^j)$ in Definitions \ref{def:lattice_chains} and \ref{def:lattice_chains_gamma} are slightly different. The above formulas \eqref{eq:lattice_chains_Iw}, \eqref{eq:lattice_chains_Par} and \eqref{eq:lattice_chains_Levi} are equivalent by Lemma \ref{lem:lattice_combinatorics} (3).
\end{enumerate}
\end{defn}
...
\end{enumerate}
\end{defn}
For the next statement, choose a free discrete subgroup $Γ\subset L^\times$ such that $\mr{vol}(L^\times/Γ) = 1$. Concretely, $Γ = \varpi^{\mbZ}$ with some uniformizer $\varpi \in L$ if $L$ is a field or $Γ \iso (\varpi_1^\mbZ, \varpi_2^\mbZ)$ for two uniformizers $\varpi_1, \varpi_2\in F$ if $L\iso F\times F$.
...
P(w, s) := \sum_{(Λ_0, Λ_0^\flat, Λ_1, Λ_1^\flat) \in \mcL(w),\ \mr{cond}(Λ_0, Λ_0^\flat) = (0,0)} (-q^{2s})^{[Λ_0:Λ_1]}.
\end{equation}
...
U(w, s) := i(L)^{-1}\sum_{(Λ_0, Λ_0^\flat, Λ_1, Λ_1^\flat) \in \mcL(w),\ \mr{cond}(Λ_0, Λ_0^\flat) \neq (0,0)} (-q^{2s})^{[Λ_0:Λ_1]}.
\end{equation}
\end{defn}
...
This is proved by an easy case-by-case analysis; we omit these details.
...
The purpose of this section is to explicitly compute the principal germ. The relative position $(M_0 : M_1) \in \mbZ^2$ of two lattices $M_0, M_1\subset F^2$ is, by definition, the pair $(a, b)$ with $a\leq b$ that consists of the valuations of their elementary divisors (Cartan decomposition). For example, $(M_0 : M_1) = (0, k)$ withp $k\geq 0$ if and only if $M_0\supseteq M_1$ with cyclic quotient $M_0/M_1$ of length $k$. By lattice pair in $F^2$, we mean a pair of lattices $(M, M^\flat)$ such that $(M : M^\flat) = (0, 1)$.
...
\end{equation}
The task is thus to count the set of lattice diagrams of the form $(O_L, Λ_0^\flat, Λ_1, Λ_1^\flat)\in \mcL(w)$ with $\mr{cond}(Λ_0^\flat) = 0$ and $[O_L:Λ_1] = k$. This counting problem was the content of Lemma \ref{lem:lattice_count} and we are only left to evaluate this lemma in dependence on $(L, r, d)$.
...
\end{prop}
\begin{proof}
Let $O_{L,c} = O_F + π^cO_L$ denote the order of conductor $c$ in $L$. By Lemma \ref{lem:changing_L}, the relative position $(O_{L,c}:w O_{L,c})$ only depends on $(r,d)$. Moreover, for every $c \geq 1$, the number
...
\end{cases}
\end{equation}
We are left to substitute \eqref{eq:para_princ_germ_value} and \eqref{eq:para_uni_germ_value} in \eqref{eq:germ_exp_par_value} with $i(L) = q$ for $L$ ramified and $i(L) = q+1$ for $L$ inert. This proves the proposition in the remaining four cases.
\end{proof}
...
\end{cases}
$$
Here, $v_D:D^\times\to \mbZ$ denotes the normalized valuation of $D$. The condition $v_D(g) \in 2\mbZ$ holds if and only if $v_D(z_g) \geq 1$. Moreover, $v_D(z_g)$ is always odd and thus $v_F(N_{L_g/F}(z_g^2)) \in 2\mbZ + 1$. In this way, the FL for $f'_\Iw$ becomes Identity \eqref{eq:fund_lem_gl4}.

We turn to the computation of $\Orb(γ, f'_\Iw)$. The sign of the functional equation of $\Orb(γ, f'_\Iw, s)$ is $(-1)^{r+1}$, see Lemma \ref{lem:sign_alternative} and Proposition \ref{prop:functional_equation}. It follows that $\Orb(γ, f'_\Iw) = 0$ whenever $r$ is even. Moreover, Proposition \ref{prop:hyperbolic} in particular implies that $(X+1)$ divides $\Orb(γ, f'_\Iw, s)$ if $L$ is split, and hence that $\Orb(γ, f'_\Iw) = 0$ in all such cases. Since $r$ is always even when $L$ is an unramified field extension, the only remaining possibility for $\Orb(γ, f'_\Iw)$ to be non-zero is when $L$ is ramified and $r$ odd. In this case $d = -1/2$ by Lemma \ref{lem:std_form_r_d} (3). If $r \leq 0$, then \eqref{eq:fund_lem_gl4} holds for the trivial reason that $\Orb(γ, f'_\Iw, s) = 0$ by Lemma \ref{lem:lattice_combinatorics} (3). Thus we are left to consider the case $L$ ramified, $r> 0$ odd, $d = -1/2$.
...
\end{prop}
\begin{proof}
Identity \eqref{eq:derivative_trivial_case} follows immediately from the functional equation (Proposition \ref{prop:functional_equation}): If its sign $ε_D(γ)$ is positive, then $\Orb(γ, f'_D, s)$ has even vanishing order at $s = 0$, so $\del (γ, f'_D) = 0$. Applying \eqref{eq:norm_orb_int_Iw}, we also have the functional equation
...
\end{cases}
\end{equation}
Here, the first line occurs if $r\in 4\mbZ$ and the second if $r\in 2 + 4\mbZ$. We are left to evaluate the expression
...
$ρ:\ov{S} \times_{\Spec \mbF} \mbY \to \ov{S}\times_S Y$ an $O_C$-linear quasi-isogeny\end{varwidth}}\right\}\right..
\end{equation}
...
$ρ:\ov{S} \times_{\Spec \mbF} \mbX \to \ov{S}\times_S X$ an $O_D$-linear quasi-isogeny\end{varwidth}}\right\}\right..
\end{equation}
...
$$\mcI^{\mr{pure}}(U) = \{f\in A\mid \text{$f_\eta = 0$ in $(A/I)_\eta$ for every $1$-dimensional generic point $η\in \Spec A/I$}\}.$$
...
\end{equation}
Then $n(z, Λ') = n(z, Λ) + 1$ occurs if and only if all four entries in \eqref{eq:mod_pi_cases_z_II} have valuation $\geq 1$. This never happens in the second case of \eqref{eq:mod_pi_cases_z} because here $v(a) = 0$. We are left with the first case in \eqref{eq:mod_pi_cases_z} and find that $n(z, Λ') = n(z, Λ) + 1$ if and only if $v(π^{-1}c) \geq 1$, which is equivalent to the stated condition $v(\det_E(z^2)) \geq 4$.
\end{proof}
...
The ``only if'' direction is clear because $zΛ\subseteq Λ$ implies that $z^2$ has an integral characteristic polynomial. To prove the ``if'' direction, observe that $W$ is a free module of rank $1$ over $E\tensor_FL$ by Proposition \ref{prop:quaternion_algebra}. Pick any lattice $Λ' \subseteq W$ that is stable under $O_E\tensor_{O_F} O_L$. If $z^2\in O_L$, then $Λ = Λ' + zΛ'$ is preserved by $z$.
\end{proof}
...
Assume that $z^2 \in O_L^\times$. Then there exists an $L$-linear, $E$-conjugate linear involution $τ$ on $W$ such that
$$O_E[z] = O_E[τ, z^2].$$
...
is easily checked to be surjective. It equals the map on $(R/\mfm)$-points of the  smooth morphism $N_{E/F}:\mr{Res}_{O_E/O_F} \mbG_m \to \mbG_m$ of smooth $O_F$-group schemes. (The norm morphism is smooth because $E/F$ is unramified.) Using completeness and a deformation argument, it follows that the map
...
\end{enumerate}
These possibilities are the ones that lead to rows 1 and 3 in Table \ref{table:matching}.
\end{rmk}
...
\end{equation}
Here, we let $G_b$ act from the left of $\mcM_D$ (instead of as from the right) by $g\mapsto g^{-1}$.
\end{thm}
...
\end{equation}
Then one can define a decomposition $S = S^+ \sqcup S^- \sqcup S^{\neq}$ into open and closed subschemes in the following way: $S^+$ is the locus where both $φ_0^-$ and $φ_0^-$ vanish. Similarly, $S^-$ is the locus where both $φ_0^+$ and $φ_1^+$ vanish. Finally, $S^{\neq}$ is the complement. This decomposition is functorial and hence defines a decomposition
...
\end{equation}
Then $(e_1^\pm,e_2^\pm)$ forms an $O_E$-basis of $Λ^\pm$. Let $φ:\mcO_{U_Λ} \tensor_{O_F} Λ \to \mcL$ be the universal quotient. For every $λ\in Λ\setminus πΛ$, the image $φ(λ)\in \mcL$ is a generator. By inspection of \eqref{eq:U_L_over_E_to_F_II}, one sees that the functions $φ(e_1^-)/φ(e_1^+)$ and $φ(e_2^-)/φ(e_2^+)$ are defined on a Zariski open neighborhood of $\breve U_{E,Λ}$ and generate the ideal defining $\breve U_{E,Λ}\subset \breve U_Λ$. Their wedge product
$$c_{(e_1,e_2)} := \frac{φ(e_1^-)}{φ(e_1^+)} \wedge \frac{φ(e_2^-)}{φ(e_2^+)}$$
is thus a generator of $\det \mcC\vert_{\breve U_{E,Λ}}$.
...
Let $Λ\subseteq W$ be an $O_E$-lattice such that $zΛ\subseteq Λ$. Then $z$ satisfies $zΛ^+ \subseteq Λ^-$ and $zΛ^- \subseteq Λ^+$.
...
Given an $S$-valued point $(\mcL, (φ,0))$ resp. $(\mcL_i, (φ_i,0))_{i = 0,1}$ of $\breve U_{E, \mcL}$ as in \eqref{eq:U_L_over_E_to_F_II}, we obtain that
...
Definition \ref{def:purification} also provides a definition of the artinian locus $(\bOm_E(W)\cap \bOm_E(W))^{\mr{art}}$. Furthermore, recall that we defined $\mcT(z)$ as the set of homothety classes of $O_E$-lattices in which $n(z,-)$ takes its maximum (Definition \ref{def:Tz}). Also recall the following terminology for points on $\Omega_E(W)$:

\begin{defn}\label{def:superspecial}
A closed point of $\bOmega_E(W)$ is called special if it is defined over $\mbF_{q^4}$. It is called superspecial if it is defined over $\mbF_{q^2}$. The superspecial points are hence precisely the intersection points $P_Λ\cap P_{Λ'}$ for lattice chains $πΛ\subset Λ' \subset Λ$ and in bijection with the edges of $\mcB$.
\end{defn}
...
\end{equation}
where $\breve U_{E,\mcL} = \Spf O_E\langle \bu, \bv\rangle/(\bu\bv-π)$. Write $z = \left(\begin{smallmatrix} a & b\\ πc & d\end{smallmatrix}\right)σ \in M_2(O_E)σ$ with respect to the basis $(e_1,e_2)$. Here, $σ\in \mr{Gal}(E/F)$ denotes the Galois conjugation. Note that $σ(e_i^+) = e_i^-$ and $σ(e_i^-) = e_i^+$. Thus the map from $Λ_0^-$ to $Λ_0^+$ defined by $z$ is given by $\left(\begin{smallmatrix} a & b\\ πc & d\end{smallmatrix}\right)$ with respect to the bases $(e_1^+, e_2^+)$ and $(e_1^-, e_2^-)$. The vanishing conditions defining $\breve U_{E, \mcL}^z$ in $\breve U_{E, \mcL}$ then become
...
where $m = m(z,Λ_0)$ is chosen maximally. We claim that the ideal $(a+c\bv, b\bu+d)$ is principal unless $a',d' \in πO_E$ and $b',c' \in O_E^\times$. In this latter case, the ideal is $π^m(\bu, \bv)$.
In order to prove the claim, consider the complete local ring at $P_{Λ_0}\cap P_{Λ_1}$ which is $R = O_{\breve F}[\![\bu,\bv]\!]/(\bu\bv-π)$. Then
...
\end{cases}$$
This proves our claim and we are left to connect everything with the geometry of $\mcT(z)$. The condition in question ($a',d'\in πO_E$ and $b',c' \in O_E^\times$) implies that $m = \max n(z,-)$ and that $r - 4m = v(\det_E(π^{-2m}z^2)) = 2$. This shows that we are in case (2) or (4) of Theorem \ref{thm:classification_multiplicity_function} as claimed.
\end{proof}
...
Let $(Y, ι, ρ)$ be the universal point over $\mcM_C$ and let $\mcD$ be the $O_F$-Grothendieck--Messing crystal of $Y$ evaluated at the thickening $V(\mcI^2)$, viewed with trivial PD-structure. It is endowed with an $O_C = O_{F_4}[\varpi]$-action $ι$ by functoriality. This provides a $\mbZ/4$-grading $\mcD = \bigoplus \mcD_i$ where
...
Write $\ob{\mcD} = \mcD/\mcI\mcD$, $\ob{\mcD_i} = \mcD_i/\mcI \mcD_i$ and denote by $\ob{\mcF_i} \subset \ob{\mcD_i}$ the Hodge filtration of $Y$. The special condition (see Definition \ref{def:special}) implies that $\ob{\mcF_i} = \ob{\mcD_i}$ for $i = 1,3$.
...
\end{array}
\end{equation}
where $\mcP_i = \mcD_i \oplus Π\mcD_{i+4λ}$. The operator $Π$ acts homogeneously of degree $-4λ$. Let $\mcQ\subset \mcP$ denote the Hodge filtration of the (restriction to $V(\mcI^2)$ of the) universal point of $\mcM$. It is $O_D$-stable, meaning it is $Π$-stable and graded ($\mcQ = \bigoplus \mcQ_i$ with $\mcQ_i \subset \mcP_i$). The ideal $\mcI/\mcI^2$ is defined by the condition that $\mcQ$ is vertically graded in the sense of \eqref{eq:Grothendieck_Messing_3/4}, meaning that for all $i$
$$\mcQ_i = (\mcQ_i\cap \mcD_i) \oplus (\mcQ_i \cap Π\mcD_{i+4λ}).$$
Being vertically graded in this sense is equivalent to $\mcD_i \subset \mcQ_i$ and $Π\mcD_{i+1+4λ} \subset \mcQ_{i+1}$ for $i = 1,3$, because these inclusions hold modulo $\mcI$. Since $\mcQ$ is $Π$-stable, it is equivalent to just have $\mcD_i\subset \mcQ_i$ for $i = 1,3$. This shows that $\mcI/\mcI^2$ is defined by the vanishing of the two maps
...
\end{equation}
The automorphism $c\mapsto Π^{-1}cΠ$ of $C$ satisfies $Π^{-1}\varpi Π = \varpi$ but its effect on $F_4$ depends on $λ$: It is given by $Π^{-1}a Π = σ^{-1}(a)$ if $λ = 1/4$ and by $Π^{-1}aΠ = σ^{-3}(a)$ if $λ = 3/4$. For both choices of $λ$ we define, with $Π\in D_λ$,
\begin{equation}\label{eq:def_set_S_of_endomorphisms}
S_λ = \{ y\in \End^0_F(\mbY)^\times\ \vert\ ycy^{-1} = Π^{-1}cΠ\ \text{for}\ c\in C\}.
\end{equation}
Let $S$ be the union $S_{1/4}\sqcup S_{3/4}$. Then $\varpi S_λ = S_{λ + 1/2}$ and, for every $y\in S$,
$$\mcZ(\varpi y) \subseteq \mcZ(y) \subseteq \mcZ(\varpi^{-1} y).$$
...
Let us now bring the framing object $(\mbY, ι)$ into play. Denote its isocrystal by $(N, F, V, ι)$. We have seen that $τ := V^{-2}\varpi$ is of degree $0$ and $σ^2$-linear with all slopes $0$. Put $W = N_0^{τ = \mr{id}}$, which is a $2$-dimensional $E$-vector space. We may define a map $\mcM_C(\mbF)\to \bOmega_E(W)(\mbF)$ by the following construction: An $ι(O_C)$-stable and special Dieudonné lattice $M\subseteq N$ with $Λ_i = M_i^{τ = \mr{id}}$ as above is sent to
...
\end{equation}
Also let $\mcZ(y)^0 := \mcZ(y)^{0}$. Lemma \ref{lem:descript_Z_y_Dieudonne} shows that, under the isomorphism $\mcM_C^{0}\iso \bOmega_E(W)$,
...
By Corollary \ref{cor:support_intersection_set_theoretic}, if $z\in P_Λ(\mbF)$, then $n(w(y), Λ) = 0$. It follows that $n(w(\varpi y), Λ) = 1$. Moreover, by \eqref{eq:squares_w}, the assumption $w(y)^2 \in O_L^\times$ implies that $w(\varpi y)^2 \in π^2O_L^\times$. By Propositions \ref{prop:multiplicities_1_4} and \ref{prop:embedded_components_1_4} as well as \eqref{eq:isom_1_4}, $\mcZ(\varpi y)$ equals the vanishing locus $V(π)$ in a neighborhood of $z$. Since $\mcZ(\varpi y)\subseteq \mcZ(y)$, this implies $π\in I$ and we are left to show that the structure map $O_{\breve F} \to R/I$ is formally unramified.
...
The coefficients in \eqref{eq:V_series_y_generic} have the following properties.
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*), topsep=2pt, itemsep=2pt]
...
Consider now any sequence of elements $a_1,b_1,a_3,b_3,\ldots \in \ob R$ that lift the coefficients in \eqref{eq:V_series_y_generic}. (It will not lead to confusion that we denote them by the same symbols.) Let $\wt y: \ob M \to \ob M$ be the map of sets that is given by the $V$-series substitution
...
\end{cases}
\end{equation}
It lifts of the map $y\in \End(\mbM)$ by definition. By Corollary \ref{cor:char_endos}, $\wt y \in \End(E(\ob{R}/J)\tensor_{E(R)} M)$ for the ideal $J\subseteq \ob{R}$ that is generated by all coefficients of the two $V$-series $(\wt y\circ \varpi)(γ_0) - (\varpi \circ \wt y)(γ_0)$ and $(\wt y\circ \varpi)(γ_2) - (\varpi \circ \wt y)(γ_2)$.
...
\end{cases} 
\end{equation}
By definition of $I(y)$ and by Proposition \ref{prop:endos_general}, $I(y)$ is precisely the ideal $(c_1,c_3,\ldots,d_1,d_3,\ldots)$. Now $π\circ \wt y$ defines a lift to $R$ of the power series expression for $π\circ y$. Its obstruction, evaluated on $γ_0,γ_2$, is given by $π \circ \mr{ob}$ because $[π,\varpi] = 0$. The crucial observation now is that
...
\end{cases}
\end{equation}
modulo $I(y)^{q^2}$ because $[a]V^{2k} = V^{2k}[a^{q^{2k}}]$. Thus we obtain that
\begin{equation}\label{eq:ideal_mod_power_m}
I(πy) = πI(y) \mod I(y)^{q^2}.
\end{equation}
We are left to show that this already implies $I(πy) = πI(y)$.
...
Here, there either one or two curves $P_Λ$ that contain $z$. Since $n(\varpi y, Λ) = n(y, Λ) + 1$ for every $y\in S_{3/4}$,
$$\max_{z \in P_Λ} n(\varpi y, Λ) = n + 1.$$
By Propositions \ref{prop:multiplicities_1_4} and \ref{prop:embedded_components_1_4}, we find that
...
\end{array}
\end{equation}
Assuming that all these relations hold, we claim that none of the $16$ variables is a unit. (This is equivalent to claiming that all $16$ coefficients are divisible by $π$ after base change to $W_{O_F}(R/π) = O_{\breve F}$ which precisely means that $z\in \mcZ(π^{-1}y)$.) The proof of the claim is as follows. We use the matrix notation $(α_{ij})$, $(β_{ij})$ etc. to refer to the individual identities in \eqref{eq:display_final_identities}.
\begin{enumerate}[wide, labelindent=0pt, labelwidth=!, label=(\arabic*), topsep=2pt, itemsep=2pt]
...
\end{lem}
\begin{proof}
First consider the divisors $\mcZ(y)^{0, \mr{pure}},\, \mcZ(\wt y)^{0, \mr{pure}} \subseteq \mcM^0_C$. Because of the identity $w(\varpi^{-1} y) = w(y)$, the multiplicity formula for the invariant $1/4$ case (Proposition \ref{prop:multiplicities_1_4}) and the analogous formula for invariant $3/4$ (Proposition \ref{prop:structure_3_4}), we obtain that
...
0 & \text{$L/F$ split}\end{cases}
\end{equation}
where $δ = 2$ if $L/F$ is inert and $δ = 1$ otherwise. We are left to verify the arithmetic transfer identity \eqref{eq:ATC_3_4}.
...
\end{lem}
\begin{proof}
The left hand side only depends on the triple $(L, r, d)$ and is given by Proposition \ref{prop:orb_int_para_teaser}. Let $m = \max_{Λ\subset W} n(y, Λ)$ and assume that $m \geq 0$. The right hand side of the lemma is described by Proposition \ref{prop:multiplicities}: It equals the ball of radius $m$ around the center $\mcT(w(y))$,
...
\bibitem{HSY}
Q. He, Y. Shi, T. Yang, \emph{Kudla--Rapoport conjecture for Krämer models}, Preprint (2022), arXiv:2208.07992v3.

\bibitem{HLSY}
Q. He, C. Li, Y. Shi, T. Yang, \emph{A proof of the Kudla--Rapoport conjecture for Krämer models}, Preprint (2022), arXiv:2208.07988.
...
\bibitem{HL}
B. Howard, Q. Li, \emph{Intersections in Lubin--Tate space and biquadratic Fundamental Lemmas}, Preprint (2020), arXiv:2010.07365.

\bibitem{HM}
N. Hultberg, A. Mihatsch, \emph{A linear AFL for Hasse invariant $1/2$}, in preparation.
...
\bibitem{Li}
Q. Li, \emph{An intersection number formula for CM cycles in Lubin--Tate towers}, Preprint (2018), arXiv:1803.07553, to appear in Duke Math. J.

\bibitem{Li_GL4}
Q. Li, \emph{A computational proof of the linear Arithmetic Fundamental Lemma for $GL_4$}, Preprint (2019), arXiv:1907.00090, to appear in Canad. J. Math.

\bibitem{Li_future}
Q. Li, \emph{A computational proof for the biquadratic Linear AFL for $GL_4$}, in preparation.

\bibitem{LM}
Q. Li, A. Mihatsch, \emph{On the Linear AFL: The Non-Basic Case}, preprint, \href{https://arxiv.org/abs/2208.10144}{arxiv:2208.10144}.
...
%
\bibitem{MZ}
A. Mihatsch, W. Zhang, \emph{On the Arithmetic Fundamental Lemma Conjecture over a general $p$-adic field}, Preprint (2021).
...
\bibitem{ZZ}
Z. Zhang, \emph{Maximal parahoric arithmetic transfers, resolutions and modularity} Preprint (2021), arXiv:2112.11994.
