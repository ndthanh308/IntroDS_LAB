...
and couplings to an ultra-light, axion-like scalar field. In particular we study the impact on the 
number of additional, light, fermionic degrees of freedom and primordial nucleosynthesis.
While the ADR leads to a new potential term in the Hamiltonian, the coupling to the scalar field results in a 
time dependent, effective mass contribution.
We solve the quantum kinetic equations (QKEs) for the neutrino density matrix and find that in certain parameter regions both new physics effects can individually yield a suppressed population of sterile neutrino species and the correct observed amount of helium in nucleosynthesis.
Combining both effects opens up new patches of parameter space excluded by experimental bounds applying to models 
featuring only one of the effects.
\end{abstract}
...
Another idea that has been proposed to explain the SBL oscillation anomalies  employs an axion-like particle (ALP) coupling to the sterile neutrinos via a Yukawa interaction term~\cite{Farzan:2019yvo, Cline:2019seo}. 
This additional, ultra-light scalar field \(\phi\) is assumed to be an approximately homogeneous condensate behaving like a classical field.
...
(Note that a similar idea~\cite{Davoudiasl:2023uiq} has been employed recently to suppress the population of sterile degrees of freedom in the early universe by means of an additional sterile mass generated by matter effects.
Here we focus on models where the  additional mass contribution originates exclusively from the evolution of the ALP condensate and independent from other background matter densities.)

In this paper we discuss both, the individual {cosmological} effects of and the interplay between altered dispersion relations and interactions with a light scalar field.
In order to estimate the influence of the full model on the chosen set of cosmological quantities, i.e. the effective number of additional neutrino generations \(\Delta N_{\mathrm{eff}}\) and the helium fraction \(Y_{\mathrm{^4He}}\), we solve the Boltzmann equation for the neutrino density matrix \(\varrho\).
...
This paper is organized as follows: In chapter~\ref{sec:model}, we describe the model in detail and define the parameter space and crucial parameters.
Chapter~\ref{sec:density_matrix} is concerned with setting up and numerically solving the Boltzmann equations.
Furthermore, we discuss the proper definition of the density matrix.
In chapter~\ref{sec:results}, we present results for the final helium abundances and \(\Delta N_{\mathrm{eff}}\) for various benchmark points in the model parameter space.
Finally in chapter~\ref{sec:conclusions}, we draw our conclusions.
...
\begin{align}
    \theta \rightarrow \frac{\pi}{4} &\Leftrightarrow \tan(2\theta(p)) \rightarrow \pm \infty \nonumber\\
...
\end{align}
Therefore, the sterile neutrinos gain an additional time dependent mass term
\begin{align}
    m_{\mathrm{eff}}(t) &= m_{ss} + \lambda \phi_{0} \eta(t) \,,
\end{align}
which modifies the mixing between electron and sterile neutrinos.
...
\begin{enumerate}
    ...
    \end{align}
    with the helium and {b}aryon number densities \(n_{\mathrm{He}}\), \(n_{\mathrm{B}}\).
\end{enumerate}
...
\begin{align}
    (2 \pi)^3 \delta^{3}(\vec{p} - \vec{q}) \varrho_{jk}(\vec{p}) := \langle a_k(\vec{p})^{\dagger} a_j(\vec{p}) \rangle
\end{align}
...
The time evolution of \(\varrho\) in an expanding, homogeneous and isotropic universe is governed by a Boltzmann-like, quantum kinetic equation (QKE)~\cite{Sigl:1993ctk,Vlasenko:2013fja,deSalas:2016ztq,Blaschke:2016xxt}
\begin{align}
...
\end{align}
where the set of all processes \(\mathcal{P}\) contains the interactions given in Tab.~\ref{tab:processes}.
...
For more discussions see App.~\ref{app:x0}.
...
Furthermore, decomposing the Hermitian density matrix into its 4 {independent}, real components yields a total of \(4 N_{y} + 1\) coupled differential equations which need to be solved\footnote{For the numerical details see App.~\ref{app:num}}.
...
They are given and discussed in App.~\ref{app:gamma_n}.
...
The values of \(\dNeff\) obtained after the full integration of QKEs for different ADR parameters are shown in Tab.~\ref{tab:dNeff_sc_only}.
...
In Fig.~\ref{fig:f_nu_s_sc_only}, we show the final sterile neutrino distributions compared to the equilibrium distribution for four \(b\) values differing by many orders of magnitude to emphasize this statement.
...
In Fig.~\ref{fig:y_res}, we show the resonance momentum \(y_{\mathrm{res}}\) for multiple ADR parameters in the temperature range \(T_{\nu} \in \mathcal{T}_{\nu} := [3, 100] \, \mathrm{MeV}\).
% Figure environment removed
...
This can be seen in Fig.~\ref{fig:rho_es_sc_only} which shows the temperature evolution of the modulus \(\vert \varrho_{es} \vert := \sqrt{\mathrm{Re}(\varrho_{es})^2 + \mathrm{Im}(\varrho_{es})^2}\) for \(y = 5\).
...
To do so, we now estimate the impact of the \(\nu_e\) depletion on nucleosynthesis by proceeding as described in Sec.~\ref{ssec:He} and solve the Boltzmann equation for the neutron fraction \(X_n = n_n / (n_n + n_p)\). 
...
The final helium abundances for different ADR parameters are shown and compared to the expected standard value of \(Y_{^4\mathrm{He}}^{\mathrm{std}} \approx 0.227\) in Tab.~\ref{tab:He_sc_only}.
\begin{table}
    \centering
...
In Fig.~\ref{fig:X_n_sc_only}, we compare the temperature evolution of \(X_{n}\) for two different scenarios, i.e. for \(b = 10^{-17}\) and for \(b = 10^{-4}\).
% Figure environment removed
...
We integrate the QKEs for different parameter values shown in Tab.~\ref{tab:dNeff_alp_only}.
...
\begin{table}
    ...
    \begin{tabular}{c|ccccccccc}
        $m_{s0}$ / $\mathrm{eV}$ & \multicolumn{3}{l}{$50$} & \multicolumn{3}{l}{$100$} & \multicolumn{3}{l}{$250$} \\
        $m_{\phi}$  / $\mathrm{eV}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ \\
        \hline
        $\Delta N_{\mathrm{eff}}$ & \(1.23\) & \(1.22\) & \(1.30\) & \(0.80\) & \(0.80\) & \(0.93\) & \(0.26\) & \(0.26\) & \(0.30\)
    \end{tabular}%}
\end{table}

% discuss mixing suppression 
% plot effective mixing of nu_e and nu_s
We can explain this by looking at the behavior of the time dependent part \(\propto \eta\) of the sterile mass matrix element which is shown in Fig.~\ref{fig:phi_hat} and the \(\tan(2\theta)\) of the effective mixing angle shown in Fig.~\ref{fig:tan2th_alp_only}.
...
These conjectures are supported by Figs.~\ref{fig:rho_es_mphiFix} and~\ref{fig:rho_es_ms0Fix} showing the off-diagonal element \(\vert \varrho_{es}(y = 5) \vert\).
...
In Tab.~\ref{tab:He_alp_only} the resulting values for \(Y_{^4\mathrm{He}}\) are shown.
...
% Its time evolution for \(y = 5\) is shown in Fig.~\ref{fig:df_nu_e_ms0Fix} for different parameter configurations with the corresponding temperature when the ALP condensate starts to oscillate.
The earlier this oscillation occurs, the more \(f_{\nu_e}\) is depleted and the bigger the deviation of the final helium abundance from the SM value becomes as we can see from Tab.~\ref{tab:He_alp_only}.
...
\begin{table}
    ...
    \begin{tabular}{c|ccccccccc}
        ...
        $m_{\phi}$  / $\mathrm{eV}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ & $10^{-20}$ & $10^{-16}$ & $10^{-12}$ \\
        \hline
        \(Y_{^4\mathrm{He}}\) & \(0.234\) & \(0.234\) & \(0.235\) & \(0.232\) & \(0.232\) & \(0.233\) & \(0.229\) & \(0.229\) & \(0.229\)
    \end{tabular}%}
\end{table}

To underline this statement we also show the evolution of the neutron fraction for the benchmark points which are least and most compatible with observations in Fig.~\ref{fig:X_n_alp_only}.
...
We expect that in the combined scenario even small ADR parameters can be brought into agreement with experiment by invoking the mixing suppression by the scalar field \(\phi\) from Sec.~\ref{ssec:model_alp}.
This expectation is further substantiated from inspecting Fig.~\ref{fig:rho_es_combined}, which shows a (on average) smaller \(\vert \varrho_{es} \vert\) than for small \(b\) values in the ADR only case.
...
In Tab.~\ref{tab:dNeff_sc_alp}, we display the values of \(\dNeff\) again for \(b \in \{0, 10^{-17}, 10^{-15}, 10^{-12}\}\) with the addition of the scalar field.
...
For comparison with the pure ADR case, we show the final neutrino distributions for \(b = 10^{-17}\) and \(b = 10^{-12}\) in Fig.~\ref{fig:f_nu_s_combined}.
% Figure environment removed
...
Here, we expect the same mechanism to apply as in Sec.~\ref{ssec:results_alp_only}:
...
We can observe this effect in Fig.~\ref{fig:X_n_combined} showing the evolution of \(X_n(T)\) for \(m_{\phi} = 10^{-22}\,\mathrm{eV}\), \(m_{s0} = 250\,\mathrm{eV}\) and \(b = 10^{-17}\) compared to the evolution in the pure ADR case with the same \(b\) value.
% Figure environment removed
...
This holds for all chosen ADR parameter configurations with low \(m_{\phi}\) and high \(m_ {s0}\), c.f Tab.~\ref{tab:He_sc_alp}.
\begin{table}
    \centering
...
This effect can be seen in Fig.~\ref{fig:df_nu_combined}.
% Figure environment removed
...
Moreover, due to our findings described in Sec.~\ref{ssec:qke_sol} and~\ref{sec:results}, we don't expect that our conclusions will be different if such an analysis is carried out.
...
%As we have shown in Sec.~\ref{sec:results}, active-sterile oscillations can have a much larger impact \(\dNeff = \mathcal{O}(1)\) rendering it the more significant effect.
...
% Finally, we want to mention that for those parameter configurations leading to an equilibrated sterile species at \(T = \mathcal{O}(100\,\mathrm{MeV})\), one needs to start the integration of QKEs much earlier at \(T = \mathcal{O}(1\,\mathrm{GeV})\) and take the presence of muons and hadrons into account.
% This would yield more reliable estimates of the quantities of interest.
% Nevertheless, these scenarios are not the ones we are interested in since they are experimentally excluded anyway.
% Moreover, due to our findings described in Sec.~\ref{ssec:qke_sol} and~\ref{sec:results}, we don't expect that our conclusions regarding the behavior with respect to the model parameters will be different if such an analysis is carried out.
% Furthermore, we neglected thermal corrections to the QED plasma which can have a bigger impact on our chosen observables than oscillations if only active neutinos are considered as shown in~\cite{Bennett:2020zkv}.
% Such effects can lead to an increase in \(\dNeff\) on the order of magnitude of \(\dNeff = \mathcal{O}(0.1)\).
% As we have shown in Sec.~\ref{sec:results}, active-sterile oscillations can have a much larger impact \(\dNeff = \mathcal{O}(1)\) rendering it the more significant effect.
% In addition to that, we do not expect these thermal corrections to affect the population of the sterile species significantly since they do not interact with the plasma directly.
% Only the allowed parameter region might be shifted to slightly higher ADR parameters, higher \(m_{s0}\) and lower \(m_{\phi}\) as one takes these thermal corrections into account.
...
\label{app:x0}
In Sec.~\ref{ssec:qke_sol}, we state our choice of the initial dimensionless scale factor to be
\begin{align}
    x_0 = 0.01 \,,
\end{align}
...
In the following, we present an argument why it is still sufficient to consider only the reactions listed in Sec.~\ref{ssec:Boltzmann} for these test runs.
...
In Fig.~\ref{fig:app:rho_nu}, we show all components of the density matrix at \(x_0 = 0.01\) for a set of different models.
Inspecting Fig.~\ref{fig:app:rho_nu_ee} shows that the electron neutrino phasespace distribution perfectly aligns with its equilibrium value as expected.
Furthermore, we observe that for models with high ADR parameters the sterile neutrino distribution is very close to 0 (c.f.~\ref{fig:app:rho_nu_ss}) such that numerical oscillations below machine precision take over.
...
In Fig.~\ref{fig:app:comp}, we compare the final sterile neutrino distribution from the test run at \(x_0 = 0.01\) to the distribution obtained in the full run at \(x = 0.010001\) using the assumption that the sterile neutrino density vanishes at \(x_0\).
% Figure environment removed
...
However, since these models are already excluded for the starting point \(x_0\) (c.f.~\ref{sec:results}) choosing an earlier \(x_0\) would even lead to higher or equal sterile neutrino densities.
