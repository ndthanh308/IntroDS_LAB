...
The method takes full advantage of both uniform-grid finite difference approximation in efficient matrix-vector
multiplication via the fast Fourier transform and unstructured meshes for complex geometries.
...
% REQUIRED
\textbf{keywords}
Fractional Laplacian, finite difference, arbitrary domain, mesh adaptation, overlay grid, nonlocal

% REQUIRED
\textbf{AMS 2020 Mathematics Subject Classification.}
65N06, 35R11
...
\label{FL-1}
\begin{cases}
(-\Delta)^{\alpha} u  = f, & \text{ in } \Omega
\\
u = 0, & \text{ in } \Omega^c
\end{cases}
\end{equation}
where $(-\Delta)^{\alpha}$ is the fractional Laplacian with the fractional order $\alpha \in (0,1)$, $\Omega$ is a bounded
domain in $\mathbb{R}^d$ ($d \ge 1$), $\Omega^c \equiv \mathbb{R}^d\setminus \Omega$, and $f$ is a given function.
Given an unstructured simplicial mesh $\mathcal{T}_h$ that fits or approximately fits $\Omega$,
we want to approximate the solution of (\ref{FL-1}) on $\mathcal{T}_h$.
A uniform grid
$\mathcal{T}_{\text{FD}}$ (with spacing $h_{\text{\text{FD}}}$) that overlays $\bar{\Omega}$ (see Fig.~\ref{fig:gridoverlay-1}) is first created
and a uniform-grid FD approximation $h_{\text{\text{FD}}}^{-2 \alpha} A_{\text{FD}}$ for the fractional Laplacian is constructed thereon.
Then, the GoFD approximation of the fractional Laplacian on $\mathcal{T}_h$ is defined as
$h_{\text{\text{FD}}}^{-2 \alpha} A_{h}$, where $A_h$ is given by
\begin{equation}
\label{Ah-1}
A_h = D^{-1} (I_{h}^{\text{FD}})^T A_{\text{FD}} I_{h}^{\text{FD}}.
\end{equation}
Here, $I_{h}^{\text{FD}}$ is a transfer matrix from $\mathcal{T}_h$ to $\mathcal{T}_{\text{FD}}$ and $D$ is the diagonal matrix
...
is guaranteed if $h_{\text{\text{FD}}}$ is smaller than or equal to a positive bound
...
\begin{align}
\label{FL-3}
\widehat{(-\Delta)^\alpha u}(\xi,\eta) = (\xi^2+\eta^2)^{\alpha} \hat{u}(\xi,\eta) .
\end{align}
Accordingly, the fractional Laplacian is given by
\begin{align}
\label{FL-2}
(-\Delta)^{\alpha} u (x,y) = \frac{1}{(2\pi)^2} \int_{-\infty}^{\infty} \int_{-\infty}^{\infty}  \widehat{(-\Delta)^\alpha u}(\xi,\eta)
e^{i x \xi} e^{i y \eta} d \xi d \eta .
\end{align}

A uniform-grid FD approximation for the fractional Laplacian can be defined in a similar manner. Consider a uniform infinite grid (lattice)
\[
...
\begin{align}
\label{FL-4}
\widecheck{(-\Delta_h)^\alpha u} (\xi, \eta) & = \frac{1}{h_{\text{\text{FD}}}^{2\alpha}}
\left (4 \sin^{2}(\frac{\xi h_{\text{\text{FD}}}}{2}) + 4 \sin^{2}(\frac{\eta h_{\text{\text{FD}}}}{2})\right )^{\alpha} \check{u}(\xi,\eta) .
\end{align}
Then, the FD approximation of the fractional Laplacian reads as
\begin{align}
&(-\Delta_h)^\alpha u(x_j,y_k) = \frac{1}{ (2\pi)^2}
\int_{-\pi}^{\pi} \int_{-\pi}^{\pi}
\widecheck{(-\Delta_h)^\alpha u} (\frac{\xi}{h_{\text{\text{FD}}}},\frac{\eta}{h_{\text{\text{FD}}}}) e^{i j \xi} e^{i k \eta} d \xi d\eta
\notag \\
% =& \frac{1}{(2\pi)^2 h_{\text{\text{FD}}}^{2\alpha}} \int_{-\pi}^{\pi} \int_{-\pi}^{\pi}
%\left (4 \sin^{2}(\frac{\xi}{2}) + 4 \sin^{2}(\frac{\eta}{2})\right )^{\alpha} \check{u}(\frac{\xi}{h_{\text{\text{FD}}}},\frac{\eta}{h_{\text{\text{FD}}}}) e^{i j \xi} e^{i k \eta} d \xi d\eta ,
%\end{align*}
%or,
%\begin{align}
%& (-\Delta_h)^\alpha u(x_j,y_k)
%\notag \\
& = \frac{1}{(2\pi)^2 h_{\text{\text{FD}}}^{2\alpha}} \sum_{m=-\infty}^{\infty} \sum_{n=-\infty}^{\infty} u_{m,n} \int_{-\pi}^{\pi} \int_{-\pi}^{\pi}  \psi(\xi,\eta) e^{i (j-m) \xi} e^{i (k-n) \eta} d \xi d\eta ,
\label{FL-5}
\end{align}
where
\begin{align}
\label{FL-6}
\psi(\xi,\eta) = \left (4 \sin^{2}(\frac{\xi}{2}) + 4 \sin^{2}(\frac{\eta}{2})\right )^{\alpha} .
\end{align}
If we define
\begin{align}
\label{T-1}
...
\begin{align}
\label{FL-8}
(-\Delta_h)^\alpha u(x_j,y_k) = \frac{1}{h_{\text{\text{FD}}}^{2\alpha}}
...
$\Omega$ is covered by the square. For a given positive integer $N$, we choose $h_{\text{\text{FD}}} = \frac{R}{N}$.
...
\end{equation}
Notice that $h_{\text{\text{FD}}}^{-2\alpha} A_{\text{FD}}$ is an FD approximation matrix of the fractional Laplacian on $\mathcal{T}_{\text{FD}}$.
...
\begin{equation}
\int_{-\pi}^{\pi} \int_{-\pi}^{\pi} \big (|\xi|^2 + |\eta|^2\big )^{\alpha} |\check{u}(\frac{\xi}{h_{\text{\text{FD}}}}, \frac{\eta}{h_{\text{\text{FD}}}})|^2 d \xi d\eta
\ge C\, h_{\text{\text{FD}}}^{2\alpha} \int_{-\pi}^{\pi} \int_{-\pi}^{\pi} |\check{u}(\frac{\xi}{h_{\text{\text{FD}}}}, \frac{\eta}{h_{\text{\text{FD}}}})|^2 d \xi d\eta  ,
...
We remark that the left-hand and right-hand sides of (\ref{lem:poincare:1}) can be viewed as the $H^{2\alpha}$ semi-norm
...
\begin{equation}
\vec{u}_{\text{FD}}^T A_{\text{FD}} \vec{u}_{\text{FD}} \ge C\, h_{\text{\text{FD}}}^{2\alpha} \; \vec{u}_{\text{FD}}^T \vec{u}_{\text{FD}},
...
\cdot \sum_{j=-N}^N \sum_{k=-N}^N u_{j,k} e^{i j \xi} e^{i k \eta}
\cdot \sum_{m=-N}^N \sum_{n=-N}^N u_{m,n} e^{-i m \xi} e^{-i n \eta} d \xi d\eta
\\
 =& \frac{1}{(2\pi)^2} \int_{-\pi}^{\pi} \int_{-\pi}^{\pi}  \psi(\xi,\eta)
...
is given by
\begin{equation}
\frac{1}{h_{\text{\text{FD}}}^{2\alpha}} A_{\text{FD}} \vec{u}_{\text{FD}} = \vec{f}_{\text{FD}},
\label{FD-1}
\end{equation}
...
\begin{align}
\label{pro:FD-stab:1}
& \vec{u}_{\text{FD}}^T A_{\text{FD}} \vec{u}_{\text{FD}} \le C_1\, h_{\text{\text{FD}}}^{2\alpha} \, \vec{f}_{\text{FD}}^T \vec{f}_{\text{FD}} ,
\\
\label{pro:FD-stab:2}
...
\begin{align*}
\vec{u}_{\text{FD}}^T A_{\text{FD}} \vec{u}_{\text{FD}} & \le h_{\text{\text{FD}}}^{2\alpha} \vec{u}_{\text{FD}}^T \vec{f}_{\text{FD}}
\le h_{\text{\text{FD}}}^{2\alpha} ( \vec{u}_{\text{FD}}^T A_{\text{FD}} \vec{u}_{\text{FD}} )^{\frac 12}
( \vec{f}_{\text{FD}}^T A_{\text{FD}}^{-1} \vec{f}_{\text{FD}} )^{\frac 12} .
\end{align*}
Thus, we have
\[
\vec{u}_{\text{FD}}^T A_{\text{FD}} \vec{u}_{\text{FD}} \le h_{\text{\text{FD}}}^{4\alpha} \vec{f}_{\text{FD}}^T A_{\text{FD}}^{-1} \vec{f}_{\text{FD}} .
\]
Notice that Proposition~\ref{pro:AFD} implies $\| A_{\text{FD}}^{-1}  \| \le C h_{\text{\text{FD}}}^{-2 \alpha}$.
...
$H^{\alpha + 1/2-\epsilon}(\Omega)$ for any $\epsilon >0$ \cite{Borthagaray-2018}.
Nevertheless, for those solutions it has been observed numerically (e.g., see \cite{Duo-2018,Hao2021})
that the FD approximation converges at $\mathcal{O}(h_{\text{\text{FD}}}^{\alpha})$ in $L^\infty$ norm.
This is confirmed by our numerical results (cf. Example~\ref{ex:5}).
Our results also suggest that the error is $\mathcal{O}(h_{\text{\text{FD}}}^{\min (1,\alpha + 1/2)})$ in $L^2$ norm.
\qed
\end{rem}
...
\\
 =  & \sum_{m=-N}^{3N-1} \sum_{n=-N}^{3 N-1} \tilde{u}_{m,n}
e^{-\frac{i 2 \pi (p+2N)(m+N)}{4N}} e^{-\frac{i 2 \pi (q+2N)(n+N)}{4N}},
\end{align*}
...
\begin{equation}
\label{psi-2}
\tilde{\psi}(\xi,\eta) = \big ( 4 \cos^2(\pi \xi) + 4 \cos^2(\pi \eta) \big )^{\alpha} .
\end{equation}
...
\begin{equation}
\label{T-1D-0}
T_p =  \frac{(-1)^p \Gamma(2\alpha+1)}{\Gamma(p + \alpha + 1) \Gamma(\alpha-p+1)}, \quad p = 0, ..., 2N
\end{equation}
...
Then, a uniform-grid FD approximation $h_{\text{\text{FD}}}^{-2\alpha} A_{\text{FD}}$ (of size $(2N+1)^d\times (2N+1)^d$)
...
We define $h_{\text{\text{FD}}}^{-2\alpha} A_h$ as the GoFD approximation of the fractional Laplacian on $\Omega$,
where $A_h$ is defined in (\ref{Ah-1}).

\begin{rem}
\label{rem:Ah-1}
The matrix $D^{-1} (I_{h}^{\text{FD}})^T$ represents a data transfer from grid $\mathcal{T}_{\text{FD}}$ to mesh $\mathcal{T}_h$.
...
Moreover,  $D^{-1}$ is included in the definition so that the row sums of $D^{-1} (I_{h}^{\text{FD}})^T$ are equal to one
and constants are preserved by the transfer.
\qed
\end{rem}
...
\begin{equation}
\label{Ah-2}
A_h = D^{-\frac{1}{2}}  \cdot \big (I_{h}^{\text{FD}} D^{-\frac{1}{2}}\big )^T A_{\text{FD}} \big (I_{h}^{\text{FD}} D^{-\frac{1}{2}}\big ) \cdot D^{\frac{1}{2}}.
\end{equation}
As a consequence, $A_h$ is invertible.
\end{thm}

\begin{proof}
When $I_{h}^{\text{FD}}$ has positive column sums, $D$ is invertible and thus, the definition (\ref{Ah-1}) is meaningful. Moreover,
it is straightforward to rewrite (\ref{Ah-1}) into (\ref{Ah-2}), indicating that $A_h$ is similar to
\begin{equation}
\label{Ah-3}
\tilde{A}_h = \big (I_{h}^{\text{FD}} D^{-\frac{1}{2}}\big )^T A_{\text{FD}} \big (I_{h}^{\text{FD}} D^{-\frac{1}{2}}\big ) .
\end{equation}
...
$
I_{h}^{\text{FD}} D^{-\frac{1}{2}} \vec{u}_h = 0.
$
The full column rank assumption of $I_{h}^{\text{FD}}$ and $D$ being diagonal mean $\vec{u}_h = 0$. Thus, $\tilde{A}_h$ is nonsingular and,
...
\]
\item[(iv)] The column sums, $D_j = \sum_{k=1}^{N_v^{\text{FD}}} (I_h^{\text{FD}})_{k,j}$, $j=1, ..., N_v$, are bounded by
\begin{equation}
\label{lem:IhFD-1:1}
\max_{\V{x}_k^{\text{FD}} \in \omega_j} \phi_j(\V{x}_k^{\text{FD}}) \le D_j \le N_{val} N_{\text{FD}}^h .
\end{equation}
\item[(v)]
\begin{equation}
\label{lem:IhFD-1:2}
...
\[
D_j = \sum_{k=1}^{N_v^{\text{FD}}} \phi_j(\V{x}_k^{\text{FD}})
...
to guarantee that $D$ is invertible and $I_h^{\text{FD}}$ has full column rank.
...
\item[(i)]
\begin{equation}
\frac{1}{(d+1)\sqrt{d}} \cdot \frac{a_h}{h} \le D_j \le N_{val} N_{\text{FD}}^h,\quad \forall j = 1, ..., N_v
\label{thm:IhFD:0}
\end{equation}
and thus, $D$ is invertible. Here, $h$ is the maximum element diameter of $\mathcal{T}_h$.
...
Then, (\ref{thm:IhFD:0}) follows from (\ref{lem:IhFD-1:1}) and $D$ is invertible.
...
\begin{rem}
\label{rem:Ah-3}
The choice (\ref{hFD-1}) is needed for the theoretical guarantee of the full column rank of $I_h^{\text{FD}}$ and the invertibility of $D$.
However, the requirement is conservative. Numerical experiment shows that we can use much larger $h_{\text{\text{FD}}}$, for instance,
...
\begin{equation}
\label{GoFD-1}
\frac{1}{h_{\text{\text{FD}}}^{2\alpha}} D^{-1} (I_{h}^{\text{FD}})^T A_{\text{FD}} I_{h}^{\text{FD}} \vec{u}_h = D^{-1} (I_{h}^{\text{FD}})^T\vec{f}_{\text{FD}},
\end{equation}
where
\begin{align*}
...
\begin{equation}
\label{GoFD-3}
\frac{1}{h_{\text{\text{FD}}}^{2\alpha}} D^{-1} (I_{h}^{\text{FD}})^T A_{\text{FD}} I_{h}^{\text{FD}} \vec{u}_h = \vec{f}_{h}.
\end{equation}
...
\begin{equation}
\label{GoFD-2}
(I_{h}^{\text{FD}})^T A_{\text{FD}} I_{h}^{\text{FD}}  \vec{u}_h = h_{\text{\text{FD}}}^{2 \alpha} (I_{h}^{\text{FD}})^T \vec{f}_{\text{FD}} .
\end{equation}
Since $I_{h}^{\text{FD}} $ is sparse and the multiplication of $A_{\text{FD}}$
...
\vec{u}_{h}^T \vec{u}_{h} \le C \, (N_{val}  N_{\text{FD}}^h)^2  \left (\frac{h}{a_h}\right )^{4d} \, \vec{f}_{\text{FD}}^T \vec{f}_{\text{FD}} .
\end{equation}
\end{thm}

\begin{proof}
The estimate follows from Proposition~\ref{pro:AFD}, Lemma~\ref{lem:IhFD-1}, Theorem~\ref{thm:IhFD}, and
...
\begin{equation}
\label{GoFD-lte}
\vec{\tau}_{\text{FD}} = \vec{f}_{\text{FD}} - \frac{1}{h_{\text{\text{FD}}}^{2 \alpha}} A_{\text{FD}} I_h^{\text{FD}} \vec{u}_h^{\,e} ,
\end{equation}
where
\[
...
\begin{equation}
\label{GoFD-lte-2}
\vec{\tau}_{\text{FD}} = \vec{f}_{\text{FD}} - \frac{1}{h_{\text{\text{FD}}}^{2 \alpha}} A_{\text{FD}} \vec{u}_{\text{FD}}^{\,e}
+ \frac{1}{h_{\text{\text{FD}}}^{2 \alpha}} A_{\text{FD}} \big (\vec{u}_{\text{FD}}^{\,e} - I_h^{\text{FD}} \vec{u}_h^{\,e} \big ).
\end{equation}
...
\[
(I_h^{\text{FD}})^T A_{\text{FD}} I_h^{\text{FD}} \vec{u}_h^{\,e} = h_{\text{\text{FD}}}^{2 \alpha} (I_h^{\text{FD}})^T \vec{f}_{\text{FD}} - h_{\text{\text{FD}}}^{2 \alpha} (I_h^{\text{FD}})^T \vec{\tau}_{\text{FD}} .
\]
Subtracting (\ref{GoFD-2}) from the above equation, we obtain the error equation as
\begin{equation}
\label{GoFD-error-1}
 (I_h^{\text{FD}})^T A_{\text{FD}} I_h^{\text{FD}} \vec{e}_h = - h_{\text{\text{FD}}}^{2 \alpha} (I_h^{\text{FD}})^T \vec{\tau}_{\text{FD}} ,
 \end{equation}
...
\vec{e}_h^T \vec{e}_h
\le C (N_{val} N_{\text{FD}}^h )^2 \left (\frac{h}{a_h}\right )^{4 d}
...
is $\mathcal{O}(h^{\min(1,\alpha+1/2-\epsilon)})$
for functions in $H^{\alpha+1/2-\epsilon}(\Omega)$ for any $\epsilon > 0$.
Moreover, it can be proved that $h_{\text{\text{FD}}}^{-2\alpha} A_{\text{FD}}$ is bounded in $H^{\alpha}(\Omega)$.
Thus, we can expect that the local truncation error (and thus the error by Corollary~\ref{co:GoFD-error})
for the GoFD scheme (\ref{GoFD-1}) is $\mathcal{O}(h^{\min(1,\alpha+1/2-\epsilon)})$ in $L^2$ norm
...
of (\ref{FL-1}) in $L^2$ norm is $\mathcal{O}(h^{\min(1,\alpha+1/2)-\epsilon})$ for uniform meshes and
$\mathcal{O}(\bar{h}^{1 + \alpha})$ for graded meshes.
Here, $\bar{h} \equiv N^{-\frac{1}{d}}$ is the average element diameter commonly used
to measure convergence order in mesh adaptation.
The convergence order, $\mathcal{O}(\bar{h}^{1 + \alpha})$,
...
Notice that the fractional Laplacian approaches to the Laplacian operator as $\alpha \to 1$ and the identity operator
as $\alpha \to 0$. Thus, it is reasonable to build an efficient preconditioner
based on the Laplacian at least when $\alpha$ is close to 1.
...
Thus, it is useful to use mesh adaptation in the numerical solution of (\ref{FL-1}).
...
\sum_{K}|K|\, \det\left(I+\frac{1}{\alpha_h}|H_K(H_K(u_h^{(\ell)}))|\right)^{\frac{2}{d+4}}
 =2\sum_{K}|K|\,  \det\left(|H_K(H_K(u_h^{(\ell)}))|\right)^{\frac{2}{d+4}}.
\]
...
\begin{align}
  \sqrt{\det(\M_K)}\; |K| &=\frac{\sigma_h}{N},\;\qquad \forall K\in \mathcal{T}_h
\label{C-1}
\\
...
\label{C-2}
\end{align}
where $F'_K$ is the Jacobian matrix of the affine mapping
...
contributed by element $K$ to the vertex $\V{x}_i$. The interested reader is referred to \cite[(38), (40), and (41)]{HK2015}
...
\label{exam-0}
\Omega = B(0,1), \quad f =
\frac{2^{2 \alpha} \Gamma(1+\alpha + k) \Gamma(\frac{n}{2}+\alpha + k)} {k! \; \Gamma(\frac{n}{2}+k)} \cdot
P_{k}^{\alpha,\frac{n}{2}-1}\big(2 |\V{x}|^2-1\big ) ,
\end{equation}
where $P_{k}^{\alpha,\frac{n}{2}-1}(\cdot)$ is the Jacobi polynomial of degree $k$ with parameters $(\alpha,\frac{n}{2}-1)$
...
\begin{equation}
\label{exam-2}
u = (1-|\V{x}|^2)^\alpha_+ P_{k}^{\alpha,\frac{n}{2}-1}\big(2 |\V{x}|^2-1\big ).
\end{equation}
...
in terms of $\bar{h} \equiv N^{-1/d}$, the average element diameter for both fixed and adaptive meshes. For a fixed (and almost
...
For fixed (uniform) meshes, the error behaves like $\mathcal{O}(h^{\alpha})$ in $L^\infty$ norm
and $\mathcal{O}(h^{\min(1,0.5+\alpha)})$ in $L^2$ norm. This is consistent with the observations made
...
$\mathcal{O}(\bar{h}^{0.5+\alpha})$ in $L^\infty$ norm and $\mathcal{O}(\bar{h}^{2})$ in $L^2$ norm for adaptive meshes.
...
We consider two cases with $k = 0$ and $5$ and $\alpha = 0.5$.
...
This is consistent with finite element approximations (cf. Remark~\ref{rem:FEM-rate}) since in this case with $\alpha = 0.5$,
$\mathcal{O}(h^{\min(1,0.5+\alpha)}) = \mathcal{O}(h)$.
...
$\mathcal{O}(\bar{h}^{1+\alpha})=\mathcal{O}(\bar{h}^{1.5})$ (cf. Remark~\ref{rem:FEM-rate}). Higher accuracy with mesh adaptation
...
reduces the number of iterations significantly. Moreover, the preconditioner is more effective when $\alpha$ is closer to 1.
Meanwhile, a smaller number of iterations is required to reach the same accuracy for $\alpha = 0.5$ than $\alpha = 0.9$.
These observations are consistent with the fact that the fractional Laplacian approaches
to the Laplacian as $\alpha \to 1$ and the identity operator as $\alpha \to 0$. As a result,
the stiffness matrix of the FD approximation has a smaller condition number and the corresponding
linear system is easier to solve for smaller $\alpha$.
...
Next we consider the 3D version of (\ref{exam-0}) for $k = 0$ and $\alpha = 0.5$.
...
This example is (\ref{FL-1}) with $f = 1$ and $\Omega$ as shown in Fig.~\ref{fig:gridoverlay-1} with $\alpha = 0.75$.
The geometry of $\Omega$ is complex, with the wavering outside boundary and two holes inside.
An analytical exact solution is not available for this example. A computed solution with an adaptive mesh of $N=250948$
...
This example is (\ref{FL-1}) with $f = 1$ and $\Omega$ being $L$-shaped with $\alpha = 0.5$.
An analytical exact solution is not available for this example. A computed solution obtained with an adaptive mesh of $N=417508$
...
of $\mathcal{O}(h^{\min(1,0.5+\alpha)})$ in $L^2$ norm for fixed meshes.
...
We can use any other boundary fitted meshes or even meshless points. Moreover,
...
W. Huang was supported in part by the University of Kansas General Research Fund FY23 and J. Shen was supported in part by the National Natural Science Foundation of China through grant [12101509].
