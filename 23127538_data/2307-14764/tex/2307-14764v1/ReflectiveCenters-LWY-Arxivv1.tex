...
\begin{abstract}
Our work is motivated by obtaining solutions to the quantum reflection equation (qRE). To start, given a braided monoidal category $\cC$ and  $\cC$-module category $\cM$, we introduce  a version of the Drinfeld center $\cZ(\cC)$ of $\cC$ adapted for $\cM$;  we refer to this category as the {\it reflective center} $\cE_\cC(\cM)$ of $\cM$.
Just like $\cZ(\cC)$ is a canonical braided monoidal category attached to $\cC$, we show that $\cE_\cC(\cM)$ is a canonical braided module category attached to $\cM$. We also study when $\cE_\cC(\cM)$ possesses nice properties such as being abelian, finite, and semisimple.

Our second goal pertains to when  $\cC$ is the  category of modules over a quasitriangular Hopf algebra $H$, and $\cM$ is the  category of modules over an $H$-comodule algebra $A$. We show that the reflective center $\cE_\cC(\cM)$ here is equivalent to a category of modules over (or, is represented by) an explicit algebra, denoted by $R_H(A)$, which we call the {\it reflective algebra} of $A$. This result is akin to $\cZ(\cC)$  being represented by the Drinfeld double $\Drin(H)$ of~$H$. We study algebraic properties of reflective algebras as well.
...
Lastly, we study the reflective center $\cE_\cC(\cM)$ as a module category over $\cZ(\cC)$ in the Hopf setting. This action induces actions of $\cC$ on  $\cE_\cC(\cM)$ (including that above), and it gives the reflective algebra $R_H(A)$ the structure of a $\Drin(H)$-comodule algebra. 
\end{abstract}
...
The $H$-module coalgebra $\widehat{H}$ is defined in Lemma~\ref{lem:hatH}, and as discussed in Remark~\ref{rem:Majid},  it is related to Majid's {\it transmuted Hopf algebra} constructed in \cite{Majid1991}. 

\medskip

Moreover, we refer to $R_H(A)$ as the {\it reflective algebra of $A$ with respect to $H$}.  It is defined as a crossed product algebra, $A \rtimes_H (\widehat{H}^*)^{\textnormal{op}}$, as described at the beginning of Section~\ref{sec:double-DH}. We will see later that it plays an analogous role for reflective centers as the Drinfeld double $\Drin(H)$ of $H$ does for the Drinfeld center $\cZ(\lmod{H})$.
...
As a consequence of Theorem~B, we obtain that the reflective algebra $R_H(A)$ is a quasitriangular $H$-comodule algebra and is equipped with a quantum $K$-matrix in the sense of Definition~\ref{def:quastr-coalg}.

\vspace{7pt}

\noindent {\bf Proposition~C} (Proposition~\ref{prop:RHA-props}, Corollary~\ref{cor:RHA-comod}){\bf.} {\it The reflective algebra $R_H(A) \; (=A \rtimes_H (\widehat{H}^*)^{\textnormal{op}})$ is a quasitriangular left $H$-comodule algebra, with }
\begin{itemize}
\item {\it the given left $H$-comodule structure on $A$,} 
\smallskip
\item {\it an explicit left $H$-comodule structure on $(\widehat{H}^*)^{\textnormal{op}}$ in terms of the quantum $R$-matrix of $H$,}
\smallskip
\item {\it an explicit quantum $K$-matrix  given in terms of a dual basis of $H$. }
\end{itemize}
{\it Moreover, $R_H(A)$ is a finite-dimensional (and semisimple) $\Bbbk$-algebra when $A$ is finite-dimensional (resp., and semisimple). In this case, $\lmod{R_H(A)}$ is abelian and finite (resp., and semisimple).} \qed
...
\noindent {\bf Theorem~D} (Theorem~\ref{thm:init-obj}){\bf .} {\it The reflective algebra $R_H(\Bbbk)$ and its quantum $K$-matrix from Proposition~C is an initial object in the category of quasitriangular left $H$-comodule algebras.} \qed
...
Next, following up with Theorem~A(c), we extend the actions of Theorem~B to obtain various module category actions by the isomorphic braided monoidal categories: 
\begin{equation} \label{eq:Z(H)-intro}
\cZ(\lmod{H}) \; \cong \; \lYD{H} \; \cong \; \lmod{\Drin(H)}.
\end{equation}
Here, $\lYD{H}$ is the left $H$-Yetter--Drinfeld category. 



\vspace{7pt}

\noindent {\bf Theorem~E} (Propositions~\ref{prop:RC-DC},~\ref{prop:DH-YD},~\ref{prop:RA-DD}, Corollaries~\ref{cor:YDactEHA},~\ref{cor:DrinactDH}){\bf .} {\it The isomorphic categories from \eqref{eq:EHA-intro} are each left module categories over the monoidal categories in \eqref{eq:Z(H)-intro}, as discussed in Figure~\ref{fig:thm-sec7}.} \qed
...
In fact, there are  monoidal embeddings $I_+: \lmod{H} \to \cZ(\lmod{H})$ and $I_-: \overline{\lmod{H}} \to \cZ(\lmod{H})$, for the  mirror category $\overline{\lmod{H}}$ of $\lmod{H}$, and these induce actions on the categories in~\eqref{eq:EHA-intro}.


\vspace{7pt}

\noindent {\bf Proposition~F} (Proposition~\ref{prop:induced}){\bf .} 
{\it Retain the notation above and in Figures~\ref{fig:thm-sec6} and~\ref{fig:thm-sec7}. }
\begin{enumerate}[\upshape (a)]
\item {\it The embedding $I_+$ recovers the module category action $\blkact$ of $\lmod{H}$ on the categories in~\eqref{eq:EHA-intro} from the module category action $\widetilde{\blkact}$ of $\cZ(\lmod{H})$ on the categories in \eqref{eq:EHA-intro}.} 
\smallskip
\item {\it The embedding $I_-$ induces a module category action $\blkact'$ of $\overline{\lmod{H}}$ on the categories in \eqref{eq:EHA-intro} from the module category action $\widetilde{\blkact}$ of $\cZ(\lmod{H})$ on the categories in \eqref{eq:EHA-intro}. Here, $\blkact'$ is a special case of an action on Doi--Hopf modules examined in Proposition~\ref{prop:DH-modcat-gen}.} \qed
\end{enumerate}


\vspace{7pt}

Finally, using the module category action of $\lmod{\Drin(H)}$ on $\lmod{R_H(A)}$ [Figure~\ref{fig:thm-sec7}], we achieve the following result.

\vspace{7pt}

\noindent {\bf Corollary~G} (Corollary~\ref{cor:RHA-comod}){\bf .} {\it The reflective algebra $R_H(A)$ is a left $\Drin(H)$-comodule algebra, with an explicit comodule structure given in terms of the quantum $R$-matrix of $H$.} \qed
...
We refer the reader to  \cite{EGNO}*{Sections~1.8, 2.1, 2.4, 2.10, 4.1, 4.2} and \cite{TV}*{Sections~1.1, 1.4, 1.6, 4.1, 4.2, 4.6} for further details. 

\medskip

{\bf Monoidal categories.} A {\it monoidal category} consists of a category $\cC$ equipped with 
\begin{itemize}
\item a bifunctor $\otimes\colon  \cC \times \cC \to \cC$, 
\item a natural isomorphism $a_{X,Y,Z}\colon  (X \otimes Y) \otimes Z \overset{\sim}{\to} X \otimes (Y \otimes Z)$ for $X,Y,Z \in \cC$, 
\item an object $\one \in \cC$, and 
\item natural isomorphisms $l_X\colon  \one \otimes X \overset{\sim}{\to} X$ and $r_X\colon  X  \otimes \one \overset{\sim}{\to} X$ for  $X \in \cC$, 
\end{itemize}
such that the following associativity and unitality axioms hold for each $W,X,Y,Z \in \cC$:
\begin{eqnarray}
a_{W,X,Y \otimes Z} \circ a_{W \otimes X, Y,Z} &=& (\ide_W \otimes a_{X,Y,Z})\circ a_{W, X \otimes Y, Z}\circ (a_{W,X,Y} \otimes \ide_Z),\\
r_X \otimes \ide_Y &=& (\ide_X \otimes l_Y) \circ a_{X, \one, Y}.
\end{eqnarray}
We sometimes write $\cC$ or $(\cC, \otimes, \one)$ to denote $(\cC, \otimes, \one, a, l, r)$ for brevity.

\smallskip

A {\it (strong) monoidal functor}  between monoidal categories $(\cC, \otimes, \one, a, l, r)$ and $(\cC', \otimes', \one', a', l', r')$ is a functor $F\colon  \cC \to \cC'$ equipped with 
\begin{itemize}
\item a natural isomorphism $F_{X,Y}\colon  F(X) \otimes' F(Y) \isomorph F(X \otimes Y)$ for all $X,Y \in \cC$, and 
\item an isomorphism $F_0\colon  \one' \isomorph F(\one)$ in $\cC'$, 
\end{itemize}
that satisfy the associativity and unitality constraints below for each $X,Y,Z \in \cC$: 
\begin{eqnarray}
\quad \quad F_{X,Y \otimes Z} \circ (\ide_{F(X)} \otimes' F_{Y,Z}) \circ a'_{F(X),F(Y),F(Z)} &=& F(a_{X,Y,Z}) \circ F_{X\otimes Y, Z} \circ (F_{X,Y} \otimes' \ide_{F(Z)}), \label{eq:monfun-ass} \\
F(l_X)^{-1} \circ l'_{F(X)} &=& F_{\one, X} \circ (F_0 \otimes' \ide_{F(X)}), \label{eq:monfun-lunit} \\
F(r_X)^{-1} \circ r'_{F(X)} &=& F_{X,\one} \circ (\ide_{F(X)} \otimes' F_0). \label{eq:monfun-runit}
\end{eqnarray}
...
{\bf Linearity over $\Bbbk$, finiteness.} We now discuss certain $\Bbbk$-linear monoidal categories. A $\Bbbk$-linear category $\cC$ is {\it locally finite} if, for any two objects $V,W$ in $\cC$, $\Hom_{\cC}(V,W)$ is a finite-dimensional $\Bbbk$-vector space and every object has a finite filtration by simple objects. A locally finite category $\cC$ is {\it finite} if there are enough projectives and finitely many isomorphism classes of simple objects. Equivalently, a $\Bbbk$-linear category $\cC$ is  finite if it is equivalent to the category of finite-dimensional modules over a finite-dimensional $\Bbbk$-algebra.

 
\medskip

{\bf Tensor and fusion categories.} A  {\it tensor category} is an abelian, locally finite, rigid, monoidal category $(\cC, \otimes, \one)$ such that $\otimes$ is $\Bbbk$-linear in each slot and $\End_\cC(\one) \cong \Bbbk$.
...
{\bf Deligne tensor product.} Let $\cC$ and $\cC'$ be two tensor categories. Then the {\it Deligne tensor product} of $\cC$ and $\cC'$ is a category $\cC \boxtimes \cC'$ consisting of objects $X \boxtimes X'$ for $X \in \cC$, $X' \in \cC'$, and morphisms $f \boxtimes f'$ for $f \in \cC$, $f' \in \cC'$. We also have that $\cC \boxtimes \cC'$ is a tensor category where $(X \boxtimes X') \otimes^{\cC \boxtimes \cC'} (Y \boxtimes Y') := (X \otimes^{\cC} Y) \boxtimes (X' \otimes^{\cC'} Y')$, for $X, Y \in \cC$, $X', Y' \in \cC'$, and $\one^{\cC \boxtimes \cC'} := \one^{\cC} \boxtimes \one^{\cC'}$. 
...
{\bf Braided categories.}  A  monoidal category  $(\cC, \otimes, \one, a, l, r)$ is {\it braided} if it is a equipped with 
\begin{itemize}
\item a natural isomorphism $c_{X,Y}\colon  X \otimes Y \overset{\sim}{\to} Y \otimes X$ for $X,Y \in \cC$\; ({\it braiding}),
\end{itemize}
...
\end{equation}
for all $X,Y \in \cC$. An {\it equivalence (resp., isomorphism) of  braided monoidal categories} is a braided monoidal functor that yields an equivalence (resp., isomorphism) of the underlying categories. 

\smallskip

Similar notions hold for tensor categories and tensor functors.
...
{\small
\begin{array}{rl}
\cZ(\lmod{H}) \ni (V, \varodot, c^V) & \hspace{-.05in} \mapsto (V, \varodot, \partial^V) \in \lYD{H} \text{ for } \partial^V(v) := (c^V_H)^{-1}(v \otimes_\Bbbk 1_H) \\[.8pc]
...
{\small
\begin{array}{rl}
\lYD{H} \ni  (V, \varodot, \partial^V) & \hspace{-.05in} \mapsto  (V, \varodot, \diam) \in \lmod{\Drin(H)} \text{ for } \xi \diam v := \langle \xi, v_{\langle -1 \rangle} \rangle v_{\langle 0 \rangle}\\[.8pc]
...
\end{equation} 
with $\{h_d, \xi_d\}_d$ a dual basis of $H$. (The latter assignment is independent of choice of dual basis of~$H$.)
...
\label{eq:QT1} \textstyle \sum_i (s_i)_{(1)} \otimes (s_i)_{(2)} \otimes t_i
 \; &=  \textstyle\; \sum_{j,k} s_j \otimes s_k \otimes t_j t_k,\\[.3pc]
\label{eq:QT2}  \textstyle\sum_i s_i \otimes (t_i)_{(1)} \otimes (t_i)_{(2)}
 \;  &= \textstyle\;  \sum_{j,k} s_j s_k \otimes t_k \otimes t_j,\\[.3pc]
...
\begin{itemize}
\item an {\it action bifunctor} $\act: \cC \times \cM \to \cM$, 

\vspace{.02in}

\item a natural isomorphism $m_{X,Y,M}\colon  (X \otimes Y) \act M \overset{\sim}{\to} X \act (Y \act M)$ for $X,Y \in \cC$, $M \in \cM$, and 

\vspace{.02in}

\item a natural isomorphism $\lambda_M\colon  \one \act M \overset{\sim}{\to} M$ for $M \in \cM$, 
\end{itemize}
such that the following associativity and unitality axioms hold for each $X,Y,Z \in \cC$ and $M \in \cM$:
\begin{eqnarray}
...
such that the following compatibility axioms hold for each $X,Y \in \cC$ and $M \in \cM$:
\begin{eqnarray}
...
Similarly, a  \emph{right $\cC$-module category} is a category $\cM$ equipped with  
\begin{itemize}
\item an {\it action bifunctor} $\triangleleft\colon  \cM \times \cC \to \cM$, 

\vspace{.02in}

\item a natural isomorphism $n_{M,X,Y}\colon  M \triangleleft (X \otimes Y) \overset{\sim}{\to} (M \triangleleft X) \triangleleft Y$ for $X,Y \in \cC$, $M \in \cM$, and 

\vspace{.02in}

\item a natural isomorphism $\rho_M\colon M\triangleleft \one  \overset{\sim}{\to} M$ for $M \in \cM$, 
\end{itemize}
such that the following associativity and unitality axioms hold for each $X,Y,Z \in \cC$ and $M \in \cM$:
\begin{eqnarray*}
(n_{M, X,Y}\triangleleft \ide_Z) \circ n_{M,X\otimes Y,Z} &=& (n_{M\triangleleft X,Y,Z})\circ n_{M,X, Y \otimes Z}\circ (\ide_M\triangleleft a_{X,Y,Z}), \\
\ide_M \triangleleft l_X &=& (\rho_M \triangleleft \ide_X) \circ n_{M, \one, X}. 
\end{eqnarray*}
...
such that the following compatibility axioms hold for each $X,Y \in \cC$ and $M \in \cM$:
\begin{eqnarray*}
n'_{F(M),X,Y} \circ t_{M,X \otimes Y} &=& ( t_{M,X} \ract' \ide_Y)\circ t_{M \ract X, Y}\circ F(n_{M,X,Y}),\\
F(\rho_M) &=& \rho'_{F(M)} \circ t_{M,\one}.
\end{eqnarray*}



A {\it left module category over a tensor category $\cC$} is a left $\cC$-module category $(\cM, \act)$ that is abelian, locally finite, bilinear on morphisms, such that $- \act M: \cC \to \cM$ is exact for all $M \in \cM$. A similar notion holds for right module categories.
...
A  subcollection of $\mathsf{Fun}_{\cC}(\cM,\cM')$ that is better behaved is the collection of right exact $\cC$-module functors between left $\cC$-module categories $\cM$ and $\cM'$; this forms a full subcategory of $\mathsf{Fun}_{\cC}(\cM,\cM')$, which we denote by $\mathsf{Rex}_{\cC}(\cM,\cM')$. 
...
satisfying the compatibility conditions below for all $X,Y, Z \in \cC$ and $M\in \cM$:
\begin{align*}
    m_{X,Y,M\triangleleft Z}\circ b_{X\otimes Y,M,Z} \; &= \;(\ide_X\act b_{Y,M,Z})\circ b_{X,Y\act M,Z}\circ (m_{X,Y,M}\triangleleft \ide_Z),\\[.2pc]
    (\ide_X\act n_{M,Y,Z})\circ b_{X,M,Y\otimes Z} \;&= \;b_{X,M\triangleleft Y,Z}\circ(b_{X,M,Y}\triangleleft \ide_Z)\circ n_{X\act M,Y,Z},\\[.2pc]
    \rho_M\circ (\lambda_M\triangleleft \ide_\one) \;&=\; \lambda_M\circ (\ide_\one \act \rho_M)\circ b_{\one,M,\one}.
\end{align*}
...
\item If $(\cC,c)$ and $(\cC', c')$ are  braided monoidal categories and  $(F, F_{-,-},F_0): \cC \to \cC'$ is a braided monoidal functor, then the left $\cC$-module category $\cC'$ from part (a) (induced by the regular left $\cC'$-module category structure) is braided with
...
\begin{proof} (a) The module associativity axiom \eqref{eq:modassoc}  for $(\cM', \act', m', \lambda')$ holds by that for $(\cM', \act, m, \lambda)$, by the naturality of $m'$, and by \eqref{eq:monfun-ass}. The left module unitality axiom \eqref{eq:modunit}  for $(\cM', \act', m', \lambda')$ holds by that for $(\cM', \act, m, \lambda)$, by the naturality of $m'$, and by \eqref{eq:monfun-runit}.
%%%%%%%%%%%%%%%%%%%%%%%%
...
&\quad \circ a'^{-1}_{F(Y),M,F(X)} \; (\ide_{F(Y)} \otimes'  c'_{F(X),M}) \; a'_{F(Y), F(X), M}  \; (c'_{F(X),F(Y)} \otimes' \ide_M)  \; a'^{-1}_{F(X), F(Y), M} \\[6pt]
...
In fact, if a braided $\cC$-module functor gives an equivalence of categories, then its quasi-inverse is also a braided $\cC$-module functor.

\begin{proposition}\label{prop:inverseCbraided}
    Let $(F,s): (\cM,\act,e) \to (\cM',\act',e')$ be a functor of braided left $\cC$-module categories and let $G\colon \cM'\to \cM$ be a quasi-inverse  of $F$. Then there exists a natural isomorphism $s'$ making $(G,s')$ a functor of braided left $\cC$-module categories.
\end{proposition}

\begin{proof}
Since $F$ is an equivalence, $G$ is a left and right adjoint of $F$. Given the adjunction $G \dashv F$, consider its counit and unit below:
$$\epsilon_M\colon GF(M)\to M, \qquad \eta_N\colon N\to FG(N),$$
for objects $M \in \cM$, $N \in \cM'$, which we can choose to be isomorphisms.
%\footnote{Such adjunction data can always be chosen. A proof of this fact can e.g. be found here \url{https://ncatlab.org/nlab/show/adjoint+equivalence}.}  
Now we can define 
$s'_{X,N}$ to be the isomorphism making the diagram below commute.
{\small
\[
\xymatrix
{G(X\triangleright' N)\ar[d]_{G(X\triangleright' \eta_N)}\ar[rr]^{s'_{X,N}}&&X\triangleright G(N)\\
G(X\triangleright' FG(N))\ar[rr]^{G(s_{X,G(N)}^{-1})}&&GF(X\triangleright G(N))\ar[u]_{\epsilon_{X\triangleright G(N)}
}}
\]
}

 We first check that $(G,s')$ defines a $\cC$-module functor. To start, we check that  \eqref{eq:modfuncassoc} holds for~$s'$. We compute: 
{\small
\begin{align*}
  s'_{X\otimes Y,N}&=  \epsilon_{(X\otimes Y)\triangleright G(N)} \; G(s_{X\otimes Y,G(N)}^{-1}) \; G((X\otimes Y)\triangleright'\eta_N)\\[.2pc]
  & =
  \epsilon_{(X\otimes Y)\triangleright G(N)} \; GF(m_{X,Y,G(N)}^{-1})\;  G(s^{-1}_{X,Y\triangleright G(N)}) \; G(X\triangleright' s_{Y,G(N)}^{-1}) \; G(m'_{X,Y,FG(N))}) \; G((X\otimes Y)\triangleright'\eta_N)
  \\[.2pc]
  &= m_{X,Y,G(N)}^{-1} \; \epsilon_{X\triangleright (Y\triangleright G(N))} \; G(s^{-1}_{X,Y\triangleright G(N)}) \; G(X\triangleright' s_{Y,G(N)}^{-1})\; G(X\triangleright' (Y\triangleright'\eta_N))\; G(m'_{X,Y,N}),
\end{align*}
}

\noindent where the second equality uses \eqref{eq:modfuncassoc} for $s$, and 
the third equality uses the naturality of $m'$ and of $\epsilon$. Now we compute that
{\small
\begin{align*}
&G(s^{-1}_{X,Y\triangleright G(N)})\; G(X\triangleright' s^{-1}_{Y,G(N)})\\[.2pc]
&=  G(s^{-1}_{X,Y\triangleright G(N)}) \; G(X\triangleright' F(\epsilon_{Y\triangleright G(N)})) \; G(X\triangleright' \eta_{F(Y\triangleright G(N))}) \;
G(X\triangleright' s^{-1}_{Y,G(N)})\\[.2pc]
&= 
GF(X\triangleright \epsilon_{Y\triangleright G(N)})\; G(s^{-1}_{X,GF(Y\triangleright G(N))}) \; G(X\triangleright' FG(s^{-1}_{Y,G(N)})) \; G(X\triangleright' \eta_{Y\triangleright FG(N)})
\\[.2pc]
&=GF(X\triangleright \epsilon_{Y\triangleright G(N)}) \; GF(X\triangleright' G(s^{-1}_{Y,G(N)})) \; G(s^{-1}_{X,Y\triangleright FG(N))})\; G(X\triangleright' \eta_{Y\triangleright FG(N)}).
\end{align*}
}
\noindent Here, the adjunction triangle identity for the first equality, the naturality of $\eta$ and of $s$ in the second equality, followed by naturality of $s$ once more  in the third equality. Putting together the two equations above, we obtain that:
{\small
\begin{align*}
    s'_{X\otimes Y,N} &= m_{X,Y,G(N)}^{-1} \; \epsilon_{X\triangleright (Y\triangleright G(N))}\;
    GF(X\triangleright \epsilon_{Y\triangleright G(N)}) \; GF(X\triangleright' G(s^{-1}_{Y,G(N)})) \; G(s^{-1}_{X,Y\triangleright' FG(N))})\\
    & \quad \circ G(X\triangleright' \eta_{Y\triangleright' FG(N)}) \;
    G(X\triangleright' (Y\triangleright'\eta_N)) \; G(m'_{X,Y,N})\\[.4pc]
     &=  m_{X,Y,G(N)}^{-1} \; \epsilon_{X\triangleright (Y\triangleright G(N))} \;
    GF\big((X\triangleright \epsilon_{Y\triangleright G(N)})(X\triangleright' G(s^{-1}_{Y,G(N)})\big) \; G(s^{-1}_{X,Y\triangleright' FG(N)})\\
    &  \quad \circ G(X\triangleright' FG(Y\triangleright'\eta_N)) \; G(X\triangleright' \eta_{Y\triangleright'N}) \;
    G(m'_{X,Y,N})\\[.4pc]
     &= m_{X,Y,G(N)}^{-1} \;  \epsilon_{X\triangleright (Y\triangleright G(N))} \;
    GF\Big((X\triangleright \epsilon_{Y\triangleright G(N)})(X\triangleright' G(s^{-1}_{Y,G(N)}))(X\triangleright' G(Y\triangleright'\eta_N))\Big)\\
    &  \quad \circ G(s^{-1}_{X,G(Y\triangleright' N)}) \; G(X\triangleright' \eta_{Y\triangleright'N}) \;
    G(m'_{X,Y,N})\\[.4pc]
      &=  m_{X,Y,G(N)}^{-1} \; 
    (X\triangleright \epsilon_{Y\triangleright G(N)}) \; (X\triangleright' G(s^{-1}_{Y,G(N)})) \; (X\triangleright G(Y\triangleright'\eta_N)) \; \epsilon_{X\triangleright G(Y\triangleright' N)} \\
    &  \quad \circ G(s^{-1}_{X,G(Y\triangleright' N)}) \; G(X\triangleright' \eta_{Y\triangleright'N}) \; 
    G(m'_{X,Y,N})\\[.4pc]
    &= m_{X,Y,G(N)}^{-1} \; (X\triangleright s'_{Y,N}) \; s'_{X,Y\triangleright' N} \; G(m'_{X,Y,N}).
\end{align*}
}
\noindent Here, the second, third, and fourth equalities uses naturality of $\eta$, $s$, and $\epsilon$, respectively. Finally, using the definition of $s'$ we confirm that \eqref{eq:modfuncassoc} holds for $(G,s')$.

\smallskip


Next, we show that \eqref{eq:modfuncunit} holds for $(G,s')$. Indeed,
{\small
\[
\begin{array}{rll}
\lambda_{G(N)} \; s'_{\one, N}&=\lambda_{G(N)} \; \epsilon_{\one\triangleright G(N)} \; G(s^{-1}_{\one,G(N)}) \; G(\one\triangleright' \eta_N)
&=\epsilon_{G(N)} \; GF(\lambda_{G(N)}) \; G(s^{-1}_{\one,G(N)}) \; G(\one\triangleright' \eta_N)\\[.4pc]
&=\epsilon_{G(N)}\;G(\lambda'_{FG(N)}) \; G(\one\triangleright' \eta_N)
&=\epsilon_{G(N)} \; G(\eta_N)\; G(\lambda'_{N})\\[.4pc]
    &=G(\lambda'_{N}).
\end{array}
\]
}
\noindent Here, the first equality uses the definition of $s'$, the second equality uses the naturality of $\epsilon$, the third equality uses 
$$GF(\lambda_{G(N)}) \; G(s^{-1}_{\one,G(N)}) \; = \; G(\lambda'_{FG(N)}),$$
which we derive from \eqref{eq:modfuncunit} for $(F,s)$ by applying $G$, while the fourth equality uses naturality of $\lambda'$, followed by an adjunction triangle identity  in the last equality.
Hence, we have shown that $G$ is a functor of $\cC$-module categories.



Now assume that $\cM, \cM'$ are braided $\cC$-module categories and $F\colon \cM\to \cM'$ a braided $\cC$-module functor. It remains to show compatibility with the braiding of $G$, cf. \eqref{eq:braidmodfunc}. We compute:
{\small
\begin{align*}
e_{X,G(N)} \; s'_{X,N}\; G(e'_{X,N})^{-1}&= e_{X,G(N)} \; \epsilon_{X\triangleright G(N)}\; G(s^{-1}_{X,G(N)})\; G(X\triangleright' \eta_N)\; G(e'_{X,N})^{-1}\\
    &=e_{X,G(N)} \; \epsilon_{X\triangleright G(N)}\; G(s^{-1}_{X,G(N)})\; G(e'_{X,FG(N)})^{-1}\; G(X\triangleright' \eta_N)\\
    &=\epsilon_{X\triangleright G(N)}\; GF(e_{X,G(N)})\; G(s^{-1}_{X,G(N)})\; G(e'_{X,FG(N)})^{-1}\; G(X\triangleright' \eta_N)\\
    &=\epsilon_{X\triangleright G(N)}\; G(s^{-1}_{X,G(N)})\; G(X\triangleright' \eta_N)\\
    &=s'_{X,N}.
\end{align*}
}
\noindent The first equality uses the definition of $s'$, followed by naturality of $e'$, naturality of $\epsilon$,  \eqref{eq:braidmodfunc} for $F$ in the fourth equality, and the definition of $s'$ again in the last equality.   Thus, $(G,s')$ is a morphism of braided $\cC$-module categories as claimed.
\end{proof}
...
\end{corollary}
 ...
 We now discuss how to transfer structure for (braided) module categories, particularly across an equivalence of categories.
...
with the left $\cC$-module category structure from part (a) such that $F,G$ preserve the braiding. 
\end{enumerate}
\end{proposition}

\begin{proof}
 Since $F$ is an equivalence, $G$ is a left and right adjoint of $F$. For the adjunction $F \dashv G$, consider the unit components  $\eta_M\colon M\to GF(M)$ and counit components $\epsilon_N\colon FG(N)\to N$, for  $M \in \cM$ and $N \in \cM'$, which we can choose to be isomorphisms.  We now obtain that $\cM'$ is a left $\cC$-module category with the action and natural isomorphisms below,
\begin{align*}
X\triangleright' N &:=F(X\triangleright G(N)),
\end{align*}
\[
...
\xymatrix@C=5pc{
m_{X,Y,N}':
F((X\otimes Y)\triangleright G(N))
\ar[r]^(.54){F(m_{X,Y,G(N)})}
& F(X\triangleright (Y\triangleright G(N)))
\ar[r]^(.46){F(X\triangleright (\eta_{Y\triangleright G(N)}))}
& F(X\triangleright GF(Y\triangleright G(N))),  
}
}
\]
\[
...
\xymatrix@C=5pc{
\lambda_N':
F(\one\triangleright G(N))
\ar[r]^(.58){F(\lambda_{G(N)})}
& FG(N)
\ar[r]^(.52){\epsilon_{N}}
& N,  
}
...
\noindent for all $X,Y \in \cC$ and $N \in \cM'$.
We leave it to the reader to verify coherence axioms \eqref{eq:modassoc} and \eqref{eq:modunit}.

Now, we claim that the isomorphisms
\begin{gather*}
    s_{X,M} \; := \; F(X\act \eta_M):\;  F(X\act M)\isomorph X\act' F(M),\\[.2pc]
    s'_{X,N} \; := \; \eta^{-1}_{X\act G(N)} \; \colon G(X\act'N)\isomorph X\act G(N),
\end{gather*}
make $(F,s)$ and $(G,s')$ morphisms of left $\cC$-module functors. By \Cref{prop:inverseCbraided}, it suffices to check this for $(F,s)$. Equation \eqref{eq:modfuncassoc} follows since 
{\small
\begin{align*}
  m'_{X,Y,F(M)} \; s_{X\otimes Y,M}&= F(X\act (\eta_{Y\act GF(M)})) \; F(m_{X,Y,GF(M)}) \; F((X\otimes Y)\act \eta_{M})\\[.2pc]
    &= F(X\act (\eta_{Y\act GF(M)})) 
    \; F(X\act (Y\act \eta_{M})) \; F(m_{X,Y,M})\\[.2pc]
     &= F(X\act (Y\act GF(\eta_{M})))\; F(X\act \eta_{Y\act M}) \; F(m_{X,Y,M})\\[.2pc]
     &=(X\act' s_{Y,M}) \; s_{X,Y\act M} \; F(m_{X,Y,M}),
\end{align*}
}

\noindent using definitions in the first equality, naturality of $m$ in the second equality,  naturality of $\eta$ in the third equality, and the definition of $s$ in the last equality. 
Moreover, we verify \eqref{eq:modfuncunit}. Indeed, 
\begin{align*}
\lambda'_{F(M)} \; s_{\one,M} \; = \; \epsilon_{F(M)} \; F(\lambda_{GF(M) }) \; F(\one\act \eta_M)
\; = \; \epsilon_{F(M)} \; F(\eta_M) \; F(\lambda_{M})
\; = \; F(\lambda_M),
\end{align*}
using the definitions in the first equality, naturality of $\lambda$ in the second equality, and an adjunction triangle identity in the third equality. Therefore, we have shown that $F$ is a left $\cC$-module functor. This concludes the proof of part (a).

To prove part (b), assume that $\cM$ has a braiding 
$e_{X,M}\colon X\act M\isomorph X\act M$. Define: 
\begin{align*}
 e'_{X,N}:=F(e_{X,G(N)})\colon \;  X\act'N\isomorph X\act'N.
\end{align*}
for $X \in \cC$ and $N \in \cM'$. We have to check that $e'$ satisfies \eqref{eq:brmod2} and \eqref{eq:brmod1}.
First, we check \eqref{eq:brmod2}:
{\small
\begin{align*}
    (m'_{X,Y,N})&^{-1} \; (X \act' e'_{Y,N}) \; m'_{X,Y,N} \; (c_{Y,X} \act' N) \; (m'_{Y,X,N})^{-1} \; (Y \act' e'_{X,N}) \;  m'_{Y,X,N}\\[.2pc]
    &= F\Big(m_{X,Y,G(N)}^{-1} \; [X\act (\eta^{-1}_{Y\act G(N)} \; GF(e_{Y,G(N)}) \;  \eta_{Y\act G(N)})] \; m_{X,Y,G(N)} \;(c_{Y,X} \act {G(N)})\\
    & \quad \circ \; m_{Y,X,G(N)}^{-1} \; [Y\act (\eta^{-1}_{X\act G(N)} \; GF(e_{X,G(N)})\; \eta_{X\act G(N)})] \; m_{Y,X,G(N)}\Big)\\[.2pc]
    &= F\Big(m_{X,Y,G(N)}^{-1} \; (X\act e_{Y,G(N)}) \; m_{X,Y,G(N)} \; (c_{Y,X} \act {G(N)}) \; m_{Y,X,G(N)}^{-1} \; (Y\act e_{X,G(N)}) \; m_{Y,X,G(N)}\Big)\\[.2pc]
    &=  F\big((e_{X \otimes Y, G(N)}) \; (c_{Y,X} \act {G(N)})\big)\\[.2pc]
    &= e'_{X \otimes Y, N} \; (c_{Y,X} \act' N).
\end{align*}
}

\noindent Here, the first and last equalities use definitions, the second equality uses naturality of $\eta$, and 
the third equality uses \eqref{eq:brmod2} for $e$.
Next, we check \eqref{eq:brmod1}:
{\small
\begin{align*}
    m'_{X,Y,N}& \; (c_{Y,X} \act' N) \; (m'_{Y,X,N})^{-1} \; (Y \act' e'_{X,N}) \; m'_{Y,X,N} \; (c_{X,Y} \act' N) \; (m'_{X,Y,N})^{-1}\\[.2pc]
    &=F\Big((X\act (\eta_{Y\act G(N)})) \; m_{X,Y,G(N)}\; (c_{Y,X} \act {G(N)}) \; m_{Y,X,G(N)}^{-1} \; (Y \act \eta^{-1}_{X\act G(N)}) \; GF(e_{X,G(N)}) \\
    &\quad \circ \; \eta_{X\act G(N)} \; m_{Y,X,G(N)} \; (c_{X,Y} \act {G(N)}) \; m_{X,Y,G(N)}^{-1} \; (X \act (\eta_{Y\act G(N)}^{-1}))\Big)\\[.2pc]
    &= F\Big((X\act (\eta_{Y\act G(N)})) \; m_{X,Y,G(N)} \; (c_{Y,X} \act {G(N)}) \;  m_{Y,X,G(N)}^{-1} \; (Y\act e_{X,G(N)})\\
    &\quad \circ \; m_{Y,X,G(N)} \; (c_{X,Y} \act {G(N)}) \; m_{X,Y,G(N)}^{-1} \; (X \act \eta^{-1}_{Y\act G(N)})\Big)\\
    &= F\Big((X \act (\eta_{Y\act G(N)})) \; e_{X,Y\act G(N)} \; (X\act \eta^{-1}_{Y\act G(N)})\Big)\\[.2pc]
    &= F(e_{X,GF(Y\act G(N))})\\[.2pc]
    &= e'_{X,Y\act' N}.
\end{align*}
}
Again, the first equality uses definitions, the second one uses the naturality of $\eta$,  the third equality uses~\eqref{eq:brmod1} for $e$, while the final two equalities follows from naturality of $e$ in the second component and the definition of $e'$ and $\act'$. Thus, $e'$ makes $\cM'$ a braided module category as claimed.

Finally, we check that $F$ is a functor of braided left $\cC$-module categories; then, by \Cref{prop:inverseCbraided}, the same holds for $G$. Indeed, using the definition of $s'$, $\act'$, and $s$, condition \eqref{eq:braidmodfunc} reduces to naturality of $e_{X,M}$ applied to $\eta_M$ in the second component. This concludes the proof.
\end{proof}
...
\item The morphisms $(M, e^M) \to (N, e^N)$ are given by morphisms $f \in \Hom_{\cM}(M,N)$ such that, for all $Y \in \cC$: 
$$(\ide_Y \act f) \circ e^M_Y = e^N_Y \circ (\ide_Y \act f).$$ 
\end{enumerate}
\end{definition}
...
and $e^{Y \act M}_X \; (= e_{X, Y \act M})$ is defined by \eqref{eq:brmod1} 
for all $X,Y \in \cC$.
\end{lemma}
...
\noindent Here, the first and last equations hold by \eqref{eq:brmod1}; the second equation by Definition~\ref{def:ECM}(b) for $e^M$; and the third equation follows from  \eqref{eq:modassoc}, from the braid axiom \eqref{eq:braid1}, and the naturality of $c$.
...
Retain the notation above.  Then we have that  $\cE_{\cC}(\cM)$ and $\cZ_{\cC}(\cM_{\textnormal{bim}})$ are isomorphic as left $\cC$-module categories.
\end{proposition}
...
\begin{proof}
It follows from Proposition~\ref{prop:ECM-ZCM} that it suffices to establish the statements for  $\cZ_{\cC}(\cM_{\textnormal{bim}})$. Part~(a) then holds by \cite{Greenough}*{Lemma~7.8}.
...
Here, we will consider a category of left Doi--Hopf modules, ${}^{\widehat{H}}_A \mathsf{DH}(H)$, consisting of vector spaces which are modules over the left $H$-comodule algebra $A$ and comodules over a particular left $H$-module coalgebra $\widehat{H}$. Our main goal is to show that 
...
\label{sec:hatH}
Let us define the $H$-module coalgebra $\widehat{H}$ mentioned above in~\eqref{eq:ECM-DH} in the result below.

\begin{lemma} \label{lem:hatH}
...
\noindent for all $h \in \widehat{H}$ and $\ell \in H$. These operations make $\widehat{H}$ a left $H$-module coalgebra.
\end{lemma}

\begin{proof} First, we show that $\widehat{\Delta}$ is coassociative. For all $h \in \widehat{H}$, we have that:
{\small
\begin{align*}
&(\widehat{\Delta} \otimes \ide)\widehat{\Delta}(h) \\[.3pc]
&  \; = \;
\textstyle \sum_{i,j} \widehat{\Delta}(t_j  h_{(1)}  t_i) \; \otimes \; h_{(2)}  s_i  S^{-1}(s_j)\\[.2pc]
&  \; = \;
\textstyle \sum_{i,j,k,l} t_l  (t_j)_{(1)}   h_{(1)}  (t_i)_{(1)}  t_k \; \otimes \;(t_j)_{(2)}   h_{(2)}  (t_i)_{(2)}  s_k S^{-1}(s_l) \; \otimes \; h_{(3)}  s_i  S^{-1}(s_j)\\[.2pc]
&  \; \overset{\textnormal{\eqref{eq:QT2}}}{=} \;
\textstyle \sum_{i,j',j'',k,l} t_l  t_{j''}   h_{(1)}  (t_i)_{(1)}  t_k \; \otimes \;t_{j'}   h_{(2)}  (t_i)_{(2)}  s_k S^{-1}(s_l) \; \otimes \; h_{(3)}  s_i  S^{-1}(s_{j'}s_{j''})\\[.2pc]
&  \; \overset{\textnormal{\eqref{eq:QT2}}}{=} \;
\textstyle \sum_{i',i'',j',j'',k,l} t_l  t_{j''}   h_{(1)}  t_{i''}  t_k \; \otimes \;t_{j'}   h_{(2)}  t_{i'}  s_k S^{-1}(s_l) \; \otimes \; h_{(3)}  s_{i'} s_{i''}  S^{-1}(s_{j''})S^{-1}(s_{j'})\\[.2pc]
&  \; \overset{\textnormal{\eqref{eq:QT1}}}{=} \;
\textstyle \sum_{i',j',m,n} t_m   h_{(1)}  t_n \; \otimes \;t_{j'}   h_{(2)}  t_{i'}  (s_n)_{(2)} S^{-1}((s_m)_{(1)}) \; \otimes \; h_{(3)}  s_{i'} (s_n)_{(1)}  S^{-1}((s_m)_{(2)})S^{-1}(s_{j'})\\[.2pc]
&  \; \overset{\textnormal{\eqref{eq:QT3}}}{=} \;
\textstyle \sum_{i',j',m,n} t_m   h_{(1)}  t_n \; \otimes \;t_{j'}   h_{(2)} (s_n)_{(1)} S^{-1}((s_m)_{(2)}) t_{i'}    \; \otimes \; h_{(3)} (s_n)_{(2)} S^{-1}((s_m)_{(1)}) s_{i'}  S^{-1}(s_{j'})\\[.2pc]
&  \; = \;
\textstyle \sum_{i',j',m,n} t_m   h_{(1)}  t_n \; \otimes \;t_{j'} \;  (h_{(2)} s_n S^{-1}(s_m))_{(1)} \; t_{i'}    \; \otimes \; (h_{(2)} s_n S^{-1}(s_m))_{(2)} \; s_{i'}  S^{-1}(s_{j'})\\[.2pc]
&  \; = \;
\textstyle \sum_{m,n} t_m   h_{(1)}  t_n \; \otimes \; \widehat{\Delta}(h_{(2)} s_n S^{-1}(s_m))\\[.2pc]
&  \; = \;
\textstyle (\ide \otimes \widehat{\Delta})\widehat{\Delta}(h).
\end{align*}
}

Next, we verify that $\widehat{\Delta}$ is counital via $\widehat{\varepsilon}$ as follows. For all $h \in \widehat{H}$, we have that:
{\small
\[
(\widehat{\varepsilon} \otimes \ide)\widehat{\Delta}(h) \; = \; \textstyle \sum_{i,j} \varepsilon(t_j  h_{(1)}  t_i) \; \otimes \; h_{(2)}  s_i  S^{-1}(s_j)
\; = \; \textstyle \sum_{i,j}      \varepsilon(h_{(1)}) h_{(2)} \;  s_i \varepsilon(t_i) \;  S^{-1}(s_j \varepsilon(t_j)) \overset{\textnormal{\eqref{eq:QT4}}}{=} h.
\]
}

\noindent Likewise, we have that $(\ide \otimes \widehat{\varepsilon} )\widehat{\Delta}(h) = h$, for all $h \in \widehat{H}$.

\smallskip

Now, $\widehat{H}$ is a left $H$-module via $\rightharpoonup$ because, for all $h \in \widehat{H}$ and $\ell \in H$, we get that $1_H \rightharpoonup h = h$ and 
{\small
\begin{align*}
\ell' \rightharpoonup (\ell \rightharpoonup h) & \; = \;\ell' \rightharpoonup (\ell_{(2)}  h  S^{-1}(\ell_{(1)}))\; = \;\ell'_{(2)} \ell_{(2)}  h  S^{-1}(\ell_{(1)})  S^{-1}(\ell'_{(1)}) \\
&\;=\; \ell'_{(2)} \ell_{(2)}  h  S^{-1}(\ell'_{(1)} \ell_{(1)})
\;=\; (\ell' \ell)_{(2)}  h  S^{-1}((\ell' \ell)_{(1)})\; =\; \ell'\ell \rightharpoonup h.
\end{align*}
}

\noindent Next, we see that $\widehat{\Delta}$ is an left $H$-module map via $\rightharpoonup$ due to the computation below:
{\small
\begin{align*}
\widehat{\Delta}(\ell \rightharpoonup h)
& \; = \; 
\textstyle \sum_{i,j} t_j \; (\ell_{(2)}hS^{-1}(\ell_{(1)}))_{(1)} \; t_i \; \otimes \;(\ell_{(2)}hS^{-1}(\ell_{(1)}))_{(2)} \; s_i \; S^{-1}(s_j)
\\[.2pc]
& \; = \; 
\textstyle \sum_{i,j} t_j (\ell_{(2)})_{(1)}h_{(1)}(S^{-1}(\ell_{(1)}))_{(1)}  t_i \; \otimes \;(\ell_{(2)})_{(2)}h_{(2)}(S^{-1}(\ell_{(1)}))_{(2)}  s_i S^{-1}(s_j)
\\[.2pc]
& \; \overset{\textnormal{\eqref{eq:QT3}}}{=} \; 
\textstyle \sum_{i,j} t_j  (\ell_{(2)})_{(1)}h_{(1)}  t_i (S^{-1}(\ell_{(1)}))_{(2)}\; \otimes \;(\ell_{(2)})_{(2)}h_{(2)}  s_i (S^{-1}(\ell_{(1)}))_{(1)} S^{-1}(s_j)
\\[.2pc]
& \; = \; 
\textstyle \sum_{i,j} t_j  \ell_{(3)}h_{(1)}  t_i S^{-1}(\ell_{(1)})\; \otimes \;\ell_{(4)}h_{(2)}  s_i S^{-1}(s_j\ell_{(2)})
\\[.2pc]
& \; \overset{\textnormal{\eqref{eq:QT3}}}{=} \; 
\textstyle \sum_{i,j} \ell_{(2)} t_j  h_{(1)}  t_i S^{-1}(\ell_{(1)})\; \otimes \;\ell_{(4)}h_{(2)}  s_i S^{-1}(\ell_{(3)} s_j)
\\[.2pc]
& \; = \; 
\textstyle \sum_{i,j} [\ell_{(1)} \rightharpoonup  (t_j \; h_{(1)} \; t_i)] \; \otimes \; [\ell_{(2)} \rightharpoonup (h_{(2)} \; s_i \; S^{-1}(s_j))] \\[.3pc]
& \; = \; \ell \rightharpoonup \widehat{\Delta}(h).
\end{align*}
}

Lastly, $\widehat{\varepsilon}$ is a left $H$-module via $\rightharpoonup$ since 
{\small
\begin{align*}
\widehat{\varepsilon}(\ell \rightharpoonup h) &\; = \; \varepsilon(\ell_{(2)}hS^{-1}(\ell_{(1)})) 
\; = \; \varepsilon(S^{-1}(\ell_{(1)}))\varepsilon(\ell_{(2)})\varepsilon(h)\\[.2pc]
&\; = \; \varepsilon(\ell_{(1)}\varepsilon(\ell_{(2)}))\varepsilon(h)\; = \; \varepsilon(\ell) \varepsilon(h) \; = \; \ell \rightharpoonup \varepsilon(h) \; = \; \ell \rightharpoonup \widehat{\varepsilon}(h).
\end{align*}
}

\noindent Thus, $\widehat{H}$ is a left $H$-module coalgebra.
\end{proof}

\begin{remark} \label{rem:Majid}
We have that $\widehat{H}$ above is related to Majid's procedure of {\it transmuting} Hopf algebras \cite{Majid1991}, which starts with a quasitriangular Hopf algebra $H$ and builds a Hopf algebra $H_{\textnormal{trm}}$ in the braided monoidal category $\lmod{H}$.  In particular, note that for a quasitriangular Hopf algebra $H:=(H, R:=\sum_i s_i \otimes t_i)$, we have that its coopposite Hopf algebra $H^{\textnormal{cop}}$ is a quasitriangular Hopf algebra with $R^{\textnormal{cop}}:=\sum_i t_i \otimes s_i$.
With this, and by inspecting the proof of \cite{Majid1991}*{Theorem~3.1}, one can see that $(\widehat{H})^{\textnormal{cop}} = (H^{\textnormal{cop}})_{\textnormal{trm}}$. Under this comparison, \Cref{lem:hatH} corresponds to \cite{Majid}*{Theorem~7.4.2}.
\end{remark}
...
& \; \overset{\textnormal{Lem.\;\ref{lem:hatH}}}{=} \; 
...
To verify (i), recall the invertible linear map $\omega$ from \eqref{eq:omega} and \eqref{eq:omega-inv}, and recall that $\widehat{\Delta} = \omega \circ \Delta$ for the coproduct $\widehat{\Delta}$ in Lemma~\ref{lem:hatH}. We use the Sweedler notation $\widehat{\Delta}(h):= h_{\widehat{(1)}} \otimes h_{\widehat{(2)}}$ for $h \in \widehat{H}$,  along with $\Delta(h):= h_{(1)} \otimes h_{(2)}$ for $h \in H$. Also, for $X,Y \in \lmod{H}$, and $x \in X$, $y \in Y$, we have that:
...
Define the {\it crossed product algebra}: 
\[
...
On the other hand, we have the following facts, the first one of which is standard.
...
\[
\Omega_{L;B,C}: {}^C_B  \mathsf{DH}(L) \to \lmod{(B \rtimes_L (C^*)^{\textnormal{op}})}, \quad
(M, \ast, \varphi) \mapsto (M, \ast, \star),
\]
where $\star:= \star_\varphi$ is given in Lemma~\ref{lem:C-comod}.
\end{proposition}
...
\noindent for $\xi \in C^*, b \in B, m \in M$. Therefore, the actions $\ast$ (resp., $\star$) of $B$ and (resp., $(C^*)^{\textnormal{op}}$) on $M$ induce an action of the crossed product $B \rtimes_L (C^*)^{\textnormal{op}}$ on $M$, that is, 
...
Assume that $H$ is finite-dimensional. Then, given the left $H$-module coalgebra $\widehat{H}$ in Lemma~\ref{lem:hatH}, we have the following statements.
\begin{enumerate}[\upshape (a)]
...
\;  \overset{\textnormal{Lem.\;\ref{lem:hatH}}}{=} \; \langle \xi, \;\ell_{(2)} h S^{-1}(\ell_{(1)}) \rangle 
...
In this part, we examine algebraic properties of reflective algebras and their categories of modules. First, we recall some terminology about left $H$-comodule algebras $A$ below: 
\begin{itemize}
...
\begin{enumerate}[\upshape (a)]
\item $R_H(A)$-$\mathsf{mod}$ is abelian.

\smallskip

\item If $A$ is finite-dimensional, then $R_H(A)$ is finite-dimensional. Here, $R_H(A)\text{-}\mathsf{fdmod}$ is finite.

\smallskip

\item Consider the conditions below:
\begin{enumerate}[\upshape (i)]
\item $A$ is a left coideal subalgebra of $H$ (so, is finite-dimensional);
\item $A$ is $H$-simple and finite-dimensional;
\item $A$ is exact.
\end{enumerate}

\smallskip

\noindent Then (i) $\Rightarrow$ (ii) $\Rightarrow$ (iii). In either case,  $R_H(A)\text{-}\mathsf{fdmod}$ is finite.
...
\begin{proof}
Part (a) is well-known. The first of part (b) follows from Lemma~\ref{lem:cross-prod}; the second part is then well-known.
For part (c), we have that (i) $\Rightarrow$ (ii) by \cite{AndruskiewitschMombelli}*{Proposition~1.6} (or \cite{Skryabin}*{proof of Theorem~6.1}), and (ii) $\Rightarrow$ (iii) by \cite{AndruskiewitschMombelli}*{Proposition~1.20}. The final statement holds of part (c) by Corollary~\ref{cor:ECMprops}(d).
Part (d) follows from Corollary~\ref{cor:ECMprops}(e).
\end{proof}
...
\begin{itemize}
    ...
    \medskip
    \item $\widehat{H}:=(\widehat{H}, \widehat{\Delta}, \widehat{\varepsilon}, \rightharpoonup)$ is the left $H$-module coalgebra from Lemma~\ref{lem:hatH}; here $\widehat{\Delta}(h):= h_{\widehat{(1)}} \otimes   h_{\widehat{(2)}}$.
...
Here, we upgrade  the category isomorphism \eqref{eq:ECM-DH},
\[
\cE_{\lmod{H}}({\lmod{A}}) \; \cong \;{}_A^{\widehat{H}}  \mathsf{DH}(H),
\]
to an isomorphism of braided module categories [Definition~\ref{def:brmodcat-equiv}]. In particular, we use the braiding of the left $({\lmod{H}})$-module category $\cE_{\lmod{H}}({\lmod{A}})$ to induce a braiding on the left $({\lmod{H}})$-module category ${}_A^{\widehat{H}}  \mathsf{DH}(H)$. This will be akin to the isomorphism of braided categories, $\cZ(\lmod{H}) \; \overset{\otimes}{\cong} \; \lYD{H}$ mentioned in  \eqref{eq:Drin}.
...
\]
as braided left $({\lmod{H}})$-module categories. In particular, we have  the following statements. 
\begin{enumerate}[\upshape (a)]
...
Now we continue the work in the previous section and upgrade  \eqref{eq:ECM-DH-2},
\[
\cE_{\lmod{H}}({\lmod{A}}) \; \cong \;{}_A^{\widehat{H}}  \mathsf{DH}(H)
\; \cong \; \lmod{R_H(A)},
\]
to an isomorphism of braided module categories. Namely, we use Proposition~\ref{prop:ECM-DH-brmod} to induce a braiding on the left $({\lmod{H}})$-module category $\lmod{R_H(A)}$. This will be akin to the isomorphism of braided categories, $\cZ(\lmod{H})  \; \overset{\otimes}{\cong} \;  \lYD{H} \; \overset{\otimes}{\cong} \; \lmod{\Drin(H)}$ mentioned in  \eqref{eq:Drin}.
...
\[
\cE_{\lmod{H}}({\lmod{A}}) \; \overset{\textnormal{br.mod}}{\cong} \; {}_A^{\widehat{H}}  \mathsf{DH}(H) \; \overset{\textnormal{br.mod}}{\cong} \;
\lmod{R_H(A)},
\]
as braided left $({\lmod{H}})$-module categories. In particular,  we have  the following statements.
\begin{enumerate}[\upshape (a)]
...
\item $R_H(A)$ is also quasitriangular (as an $H$-comodule algebra) with $K$-matrix:
\begin{equation}
\label{eq:K-A}
...
\begin{proof}
(a) It follows from 
Lemma \ref{lem:cross-prod} that 
\begin{equation} \label{eq:delta-ref}
r \cdot (x \otimes m) = \delta_{\textnormal{ref}}(r) (x \otimes m),
\end{equation}
for all $r \in R_H(A)$, $x \in X$, $m \in M$. 
So, the formulas for $\delta_{\textnormal{ref}}(a)$ and $\delta_{\textnormal{ref}}(\xi)$ follow from the formulas for $\widetilde{\ast}$ and $\widetilde{\star}$, respectively, in Theorem~\ref{thm:ECM-DH-brmod-2}(a), 
applied to the left regular modules $X=H, M=A$ and the elements $x=1_H, m=1_A$.
One can see directly that  $\delta_{\textnormal{ref}}(\xi)$ is  coassociative as shown below:
{\small
\begin{align*}
(\ide \otimes \delta_{\textnormal{ref}})\delta_{\textnormal{ref}}(\xi) & \; = \; \textstyle \sum_{i,j,d} \langle \xi,\;  t_j h_d s_i \rangle  s_j t_i \; \otimes \; \delta_{\textnormal{ref}}(\xi_d)\\[.2pc]
&\; = \; 
\textstyle \sum_{i,j,d,i',j',d'} \langle \xi,\;  t_j h_d s_i \rangle  \langle \xi_d,\;  t_{j'} h_{d'} s_{i'} \rangle s_j t_i \; \otimes \; s_{j'} t_{i'} \; \otimes \; \xi_{d'} \\
&\; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; 
\textstyle \sum_{i,j,d,i',j',d'} \langle \xi,\;  t_j t_{j'} h_{d'} s_{i'}  s_i \rangle   s_j t_i \; \otimes \; s_{j'} t_{i'} \; \otimes \; \xi_{d'}  \\
&\; \overset{\textnormal{\eqref{eq:QT1}},\textnormal{\eqref{eq:QT2}}}{=} \; 
\textstyle \sum_{i,j,d} \langle \xi,\;  t_j h_{d} s_i \rangle   (s_j)_{(1)} (t_i)_{(1)} \; \otimes \; (s_{j})_{(2)} (t_{i})_{(2)} \; \otimes \; \xi_{d}\\[.2pc]
& \; = \; (\Delta \otimes \ide)\delta_{\textnormal{ref}}(\xi).
\end{align*}
}

\noindent Moreover, to have that \eqref{eq:delta-ref} is an action, we must have that  $\delta_{\textnormal{ref}}(a \; \xi)= \delta_{\textnormal{ref}}(a)\; 
\delta_{\textnormal{ref}}(\xi)$. 
...
(b) This follows from Lemma~\ref{lem:braid-Kmatr}(b) and Theorem~\ref{thm:ECM-DH-brmod-2}(b). We also provide a direct proof for the reader's convenience; that is we verify \eqref{eq:K2}-\eqref{eq:K1}.

For \eqref{eq:K2}, we need to verify that 
$$\textstyle \sum_d (h_d)_{(1)} \otimes (h_d)_{(2)} \otimes \xi_d \; = \; 
\sum_{j,k,l,m} t_k h_l t^m \otimes h_j s_k s^m \otimes \xi_l \xi_j.$$
Here, we use \eqref{eq:BC-prod} to get $\text{mult}^{R_H(A)}(1_A \circledast \xi_j,  1_A \circledast \xi_l) = \xi_l \xi_j$ for the last factor. It suffices to show that for all $\zeta, \zeta' \in H^*$ and $h \in \widehat{H}$, we have:
$$\textstyle \sum_d \langle \zeta, (h_d)_{(1)} \rangle \; \langle \zeta', (h_d)_{(2)} \rangle \; \langle \xi_d, h \rangle \; = \; \sum_{j,k,l,m} \langle \zeta, t_k h_l t^m \rangle \; \langle \zeta', h_j s_k s^m \rangle \;  \langle \xi_l \xi_j, h \rangle.$$
The axiom \eqref{eq:K2} now holds by the computation below:
{\small
\begin{align*}
&\textstyle \sum_{j,k,l,m} \langle \zeta, t_k h_l t^m \rangle \; \langle \zeta', h_j s_k s^m \rangle \;  \langle \xi_l \xi_j, h \rangle \\[.2pc]
& \; \overset{\textnormal{\eqref{eq:hatHpair}}, \; \textnormal{Lem.\ref{lem:hatH}}}{=} \; 
\textstyle \sum_{j,k,l,m,p,q} \langle \zeta, t_k h_l t^m \rangle \; \langle \zeta', h_j s_k s^m \rangle \;  \langle \xi_l, t_q h_{(1)}t_p \rangle \;
\langle \xi_j, h_{(2)} s_p S^{-1}(s_q) \rangle\\
&\; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; \textstyle \sum_{k,m,p,q} \langle \zeta, t_k t_q h_{(1)} t_p t^m \rangle \; \langle \zeta', h_{(2)} s_p S^{-1}(s_q) s_k s^m \rangle\\
&\; \overset{\textnormal{\eqref{eq:QT5}}}{=} \; \textstyle \sum_{m,p} \langle \zeta, h_{(1)} t_p t^m \rangle \; \langle \zeta', h_{(2)} s_p s^m \rangle\\[.2pc]
& \; \overset{\textnormal{\eqref{eq:Hopfpair}}}{=} \; \langle \zeta \otimes \zeta', \Delta(h) \rangle\\
& \; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; \textstyle \sum_d \langle \zeta \otimes \zeta', \Delta(h_d) \rangle \; \langle \xi_d, h \rangle \\[.2pc]
& \; \overset{\textnormal{\eqref{eq:Hopfpair}}}{=} \; \textstyle \sum_d \langle \zeta, (h_d)_{(1)} \rangle \; \langle \zeta', (h_d)_{(2)} \rangle \; \langle \xi_d, h \rangle.
\end{align*}
}

Next, we verify \eqref{eq:K3} as follows:
{\small
\begin{align*} 
\textstyle \sum_d h_d \otimes (\xi_d)_{[-1]} \otimes (\xi_d)_{[0]}  &\; \overset{\textnormal{Cor.\;\ref{cor:RHA-comod}(a)}}{=} \; 
\textstyle \sum_{d,j,k,l} h_d \otimes  \langle \xi_d,\; t_j h_{k} s_l \rangle \; s_j t_l  \otimes  \xi_{k}  \\
& \; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; \textstyle \sum_{j,k,l} t_j h_k s_l \otimes s_j t_l \otimes \xi_k.
\end{align*}
}

\noindent Finally, we establish \eqref{eq:K1}, that is, for $r = a \xi \in R_H(A)$ we should get:
$$ \textstyle \sum_d h_d \; a_{[-1]} \;  \xi_{[-1]} \; \otimes \;  \text{mult}^{R_H}(\xi_d, a_{[0]},  \xi_{[0]}) \; = \; \textstyle \sum_d a_{[-1]} \;  \xi_{[-1]} \; h_d  \; \otimes \; \text{mult}^{R_H}( a_{[0]}, \xi_{[0]}, \xi_d).$$
The left-hand side is equal to:
{\small
\begin{align*}
&\textstyle \sum_d h_d \; a_{[-1]} \;  \xi_{[-1]} \otimes \text{mult}^{R_H}(\xi_d, a_{[0]},  \xi_{[0]})
\; \overset{\textnormal{\eqref{eq:BC-prod}}}{=} \; 
\textstyle \sum_d h_d \; a_{[-2]} \;  \xi_{[-1]} \otimes a_{[0]} \; \xi_{[0]} \; (\xi_d \leftharpoonup a_{[-1]})\\
&\; \overset{\textnormal{Cor.\;\ref{cor:RHA-comod}(a)}}{=} \; 
\textstyle \sum_{d,i,j,d'} \langle \xi, t_j h_{d'} s_i \rangle \; h_d  a_{[-2]}   s_j   t_i \; \otimes \; a_{[0]}  \xi_{d'}  (\xi_d \leftharpoonup a_{[-1]}).
\end{align*}
}

\noindent The right-hand side is equal to:
{\small
\begin{align*}
& \textstyle \sum_d a_{[-1]} \;  \xi_{[-1]} \; h_d  \; \otimes \; \text{mult}^{R_H}( a_{[0]}, \xi_{[0]}, \xi_d) \; \overset{\textnormal{\eqref{eq:BC-prod}}}{=}  \; \textstyle \sum_d a_{[-1]} \;  \xi_{[-1]} \; h_d  \; \otimes \; a_{[0]} \; \xi_d \;\xi_{[0]}\\
&\; \overset{\textnormal{Cor.\;\ref{cor:RHA-comod}(a)}}{=} \; \textstyle \sum_{d,i,j,d'} \langle \xi, t_j h_{d'} s_i \rangle \; a_{[-1]} s_j t_i  h_d  \; \otimes \; a_{[0]} \xi_d \xi_{d'}.
\end{align*}
}

\noindent Now we establish the two sides are equal via the computation below, for $\zeta \in H^*$, $h \in H$, and for any $\bar{h} \in H$ (for which $a_{[-2]} \otimes a_{[-1]} \otimes a_{[0]} = (\Delta \otimes \ide) \delta(a) =: \bar{h}_{(1)} \otimes \bar{h}_{(2)} \otimes a_{[0]}$):
{\small
\begin{align*}
&\textstyle \sum_{d,i,j,d'} \langle \xi, t_j h_{d'} s_i \rangle \; \langle \zeta, h_d  \bar{h}_{(1)}   s_j   t_i \rangle \; \langle \xi_{d'}  (\xi_d \leftharpoonup \bar{h}_{(2)}), h \rangle  \\
&\; \overset{\textnormal{Lem.\;\ref{lem:hatH}}}{=} \; \textstyle \sum_{d,i,j,d', p, q} \langle \xi, t_j h_{d'} s_i \rangle \; \langle \zeta, h_d  \bar{h}_{(1)}   s_j   t_i \rangle \; 
\langle \xi_{d'}, t_q h_{(1)} t_p \rangle 
\langle \xi_d \leftharpoonup \bar{h}_{(2)}, h_{(2)} s_p S^{-1}(s_q) \rangle\\
&\; \overset{\textnormal{\eqref{eq:LactC*}},~ \textnormal{Lem.\;\ref{lem:hatH}}}{=} \; \textstyle \sum_{d,i,j,d', p, q} \langle \xi, t_j h_{d'} s_i \rangle \; \langle \zeta, h_d  \bar{h}_{(1)}   s_j   t_i \rangle \; 
\langle \xi_{d'}, t_q h_{(1)} t_p \rangle \;
\langle \xi_d, \bar{h}_{(3)} h_{(2)} s_p S^{-1}(s_q) S^{-1}(\bar{h}_{(2)})\rangle\\
&\; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; \textstyle \sum_{i,j, p, q} \langle \xi, t_j t_q h_{(1)} t_p s_i \rangle \; \langle \zeta, \bar{h}_{(3)} h_{(2)} s_p S^{-1}(s_q) S^{-1}(\bar{h}_{(2)})  \bar{h}_{(1)}   s_j   t_i \rangle\\[.2pc]
&\; = \; \textstyle \sum_{i,j, p, q} \langle \xi, t_j t_q h_{(1)} t_p s_i \rangle \; \langle \zeta, \bar{h} h_{(2)} s_p S^{-1}(s_q)    s_j   t_i \rangle\\
&\; \overset{\textnormal{\eqref{eq:QT5}}}{=} \; \textstyle \sum_{i, p} \langle \xi, h_{(1)} t_p s_i \rangle \; \langle \zeta, \bar{h} h_{(2)} s_p   t_i \rangle\\
&\; \overset{\textnormal{\eqref{eq:QT3}}}{=} \; \textstyle \sum_{j, p} \langle \xi,  t_j h_{(2)} s_p \rangle \; \langle \zeta, \bar{h}  s_j h_{(1)}  t_p \rangle\\
&\; \overset{\textnormal{\eqref{eq:QT5}}}{=} \; \textstyle \sum_{i,j, p,q} \langle \xi,  t_j h_{(2)} s_p S^{-1}(s_q) s_i\rangle \; \langle \zeta, \bar{h}  s_j t_i t_q h_{(1)}  t_p \rangle\\
&\; \overset{\textnormal{\eqref{eq:dual-iden}}}{=} \; \textstyle \sum_{i,j, p,q, d, d'} \langle \xi,  t_j h_{d'} s_i\rangle \; \langle \zeta, \bar{h}  s_j t_i h_d \rangle \;
\langle \xi_d, t_q h_{(1)}  t_p \rangle \;
\langle \xi_{d'}, h_{(2)} s_p S^{-1}(s_q) \rangle \\
&\; \overset{\textnormal{Lem.\;\ref{lem:hatH}}}{=} \; \textstyle \sum_{d,i,j,d'} \langle \xi, t_j h_{d'} s_i \rangle \; \langle \zeta, \bar{h}  s_j t_i  h_d  \rangle  \; \langle  \xi_d \xi_{d'}, h \rangle.
\end{align*}
}

\noindent Thus, we have shown that $K_{\textnormal{ref}}(A)$ satisfies the quantum $K$-matrix axioms, \eqref{eq:K2}-\eqref{eq:K1}.
\end{proof}
...
$$\kappa:= \kappa_{(Q,K)}: (R_H(\Bbbk), K_{\textnormal{ref}}(\Bbbk)) \to (Q, K)$$ in ${}^H \mathsf{QT}$. Towards this, recall that $R_H(\Bbbk) \cong (\widehat{H}^*)^{\textnormal{op}}$ as left $H$-comodule algebras [Example~\ref{ex:specialA2}]. So, $R_H(\Bbbk) \cong H^*$ as vector spaces [Lemma~\ref{lem:hatH}]. Now we use the element $K:= \sum_i g_i \otimes p_i \in H \otimes Q$ to yield a linear map:
...
\; & \overset{\textnormal{\eqref{eq:hatHpair}}, \; \textnormal{Lem.~\ref{lem:hatH}}}{=} \textstyle \sum_{i,j',k'} \langle \zeta, t_{k'}  (g_i)_{(1)}  t_{j'} \rangle \; \langle \xi, \; (g_i)_{(2)} s_{j'}  S^{-1}(s_{k'}) \rangle \;p_i\\
...
\section{Reflective algebras as comodule algebras over Drinfeld doubles} \label{sec:ref-D(H)}

We maintain the setting and notation of Section~\ref{sec:setting-repr} here. The goal of this section is to establish how reflective algebras $R_H(A)$ are comodule algebras over the Drinfeld doubles $\Drin(H)$. We start by providing the details of Corollary~\ref{cor:ECMprops}(a) on the action of the Drinfeld center $\cZ(\cC)$ on the reflective center $\cE_\cC(\cM)$ in Section~\ref{sec:RC-DC}. Then in Section~\ref{sec:DH-YD}, we obtain an action of the Yetter--Drinfeld category $\lYD{H}$ on the category of Doi--Hopf modules ${}_A^{\widehat{H}}  \mathsf{DH}(H)$. As a consequence, we achieve the main goal of the section, which is described in Section~\ref{sec:RA-DD}. 

After this we show how the action of $\cZ(\cC)$ on $\cE_\cC(\cM)$  and the coaction of $\Drin(H)$ on $R_H(A)$ above recover the action of $\cC$ on $\cE_\cC(\cM)$ and the coaction of  $H$ on $R_H(A)$ from Sections~\ref{sec:rep-refl-cent} and~\ref{sec:R-comod-RHA}, as well as induce additional (co)actions of a similar flavor. This is done in Section~\ref{sec:ind-RC-RA}. 



\medskip

\noindent {\bf Standing notation.} Along with the notation from Section~\ref{sec:setting-repr} and  from the beginning of Section~\ref{sec:R-comod-RHA}, consider the standing notation below. The objects of $\cZ(\cC)$ will be denoted by $(V, \varodot, c^V)$ where 
\begin{itemize}
\item the left $H$-action on $V$ is given by $\varodot: H \otimes V \to V$,  \smallskip
\item for $(X, \cdot) \in \cC$, we denote $c_X^V(x \otimes v) :=  \textstyle  \sum_i (w_i \varodot v) \otimes (u_i \cdot x)$, with $x \in X$, $ \textstyle  \sum_i w_i \otimes u_i \in H \otimes H$.
\end{itemize}
 \smallskip
The objects of $\lYD{H}$ will be denoted by $(V, \varodot, \partial^V)$, with $\varodot$ as above, and  
\begin{itemize} 
\item the left $H$-coaction on $V$ given by $\partial^V(v) :=   v_{\langle -1 \rangle} \otimes v_{\langle 0 \rangle}\; \in H \otimes V$. 
\end{itemize}
Moreover, the objects of $\lmod{\Drin(H)}$ will be denoted by $(V, \varodot, \diam)$, with $\varodot$ as above, and 
\begin{itemize} 
\item the action of the Hopf subalgebra $(H^*)^{\text{op}}$ of $\Drin(H)$ on $V$ given by $\diam: (H^*)^{\text{op}} \otimes V \to V$.
\end{itemize}


%%%%%%%%%%%%%%%%

\subsection{Reflective centers as module categories over Drinfeld centers} \label{sec:RC-DC}
Note the results below. 


\begin{proposition} \label{prop:RC-DC}
The reflective center $\cE_{\lmod{H}}({\lmod{A}})$ is a left $\cZ(\lmod{H})$-module category via:
\[
\begin{array}{rl}
\widetilde{\act}: \cZ(\lmod{H}) \;\times \;\cE_{\lmod{H}}({\lmod{A}}) &\longrightarrow \cE_{\lmod{H}}({\lmod{A}}) \\[.4pc]
\left( (V, \varodot, c^V), \; (M, \ast, e^M) \right) & \mapsto 
\left( V \otimes M, \; \widetilde{\ast}, \; e^{V \otimes M}\right),
\end{array}
\]
for $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$, and $e^{V \otimes M}$ given by
\begin{align} \label{eq:ECM-ZCmod}
e^{V \otimes M}_X(x \otimes v \otimes m) =  \textstyle \sum_{i,j,k} (t_k g_j u_i \cdot x) \otimes (s_k w_i \varodot v) \otimes (p_j \ast m),
\end{align}
for $x \in (X, \cdot) \in \lmod{H}$.
\end{proposition}

\begin{proof}
The action of $\cZ(\cC)$ on $\cZ_\cC(\cM_\textnormal{bim})$ is given by \cite{Greenough}*{Lemma~7.8}. We can then apply Proposition~\ref{prop:ECM-ZCM} to obtain the action of $\cZ(\cC)$ on $\cE_\cC(\cM)$ below:
{\small
\begin{align*}
...
\noindent where,  for any $X \in \cC$, we have
{\small
\begin{align*}
 e^{V \act M}_X &:= 
...
\noindent Now for the case when $\cC = \lmod{H}$ and $\cM = \lmod{A}$, we get that 
{\small
\begin{align*}
 e^{V \otimes M}_X(x \otimes v \otimes m) 
 & = (c_{V,X} \otimes \ide_M)(\ide_V \otimes e_X^M)(c_X^V \otimes \ide_M)(x \otimes v \otimes m)\\[.2pc]
 & = \textstyle \sum_i (c_{V,X} \otimes \ide_M)(\ide_V \otimes e_X^M)((w_i \varodot v) \otimes (u_i \cdot x) \otimes m)\\[.2pc]
 & = \textstyle \sum_{i,j} (c_{V,X} \otimes \ide_M)((w_i \varodot v) \otimes (g_j u_i \cdot x) \otimes (p_j \ast m))\\[.2pc]
 & = \textstyle \sum_{i,j,k} (t_k g_j u_i \cdot x) \otimes  (s_k w_i \varodot v) \otimes (p_j \ast m).
\end{align*}
}

\noindent Thus, \eqref{eq:ECM-ZCmod} is justified. The formula for $\widetilde{\ast}$ follows from Lemma~\ref{lem:braid-Kmatr}(a). 
\end{proof}

Now we get the following consequence via the isomorphism of braided monoidal categories, $\cZ(H\text{-}\mathsf{mod})\overset{\otimes}{\cong}  \lYD{H}$ from \eqref{eq:Drin} and \eqref{eq:ZC-YD}.


\begin{corollary} \label{cor:YDactEHA}
The reflective center $\cE_{\lmod{H}}({\lmod{A}})$ is a left $\lYD{H}$-module category via:
\[
\begin{array}{rl}
\widetilde{\act}: \lYD{H} \;\times \;\cE_{\lmod{H}}({\lmod{A}}) &\longrightarrow \cE_{\lmod{H}}({\lmod{A}}) \\[.4pc]
\left( (V, \varodot, \partial^V), \; (M, \ast, e^M) \right) & \mapsto 
\left( V \otimes M, \; \widetilde{\ast}, \; e^{V \otimes M}\right),
\end{array}
\]
for $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$,  and $e^{V \otimes M}$ given by
\begin{align} \label{eq:ECMYDmod}
e^{V \otimes M}_X(x \otimes v \otimes m) =  \textstyle \sum_{j,k} (t_k g_j S^{-1}(v_{\langle -1 \rangle}) \cdot x) \otimes (s_k \varodot v_{\langle 0 \rangle}) \otimes (p_j \ast m).
\end{align}
...
\subsection{Doi--Hopf modules as module categories over Yetter--Drinfeld modules} \label{sec:DH-YD}


\begin{proposition} \label{prop:DH-YD}
The category of Doi--Hopf modules ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ is a left $\lYD{H}$-module category via:
\[
\begin{array}{rl}
\widetilde{\blkact}: \lYD{H} \;\times \; {}_A^{\widehat{H}}  \mathsf{DH}(H) &\longrightarrow {}_A^{\widehat{H}}  \mathsf{DH}(H) \\[.4pc]
\left( (V, \varodot, \partial^V), \; (M, \ast, \varphi) \right) & \mapsto 
\left( V \otimes M, \; \widetilde{\ast}, \; \widetilde{\varphi} \right),
\end{array}
\]
for $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$, and
\begin{align} \label{eq:DH-YDmod}
\widetilde{\varphi}(v \otimes m) =  \textstyle \sum_{k} t_k m_{-1} S^{-1}(v_{\langle -1 \rangle}) \otimes (s_k \varodot v_{\langle 0 \rangle}) \otimes m_0.
\end{align}
Here, $\varphi(m):=m_{-1} \otimes m_0$.
\end{proposition}

\begin{proof}
Take the action of $\lYD{H}$ on $\cE_{\lmod{H}}({\lmod{A}})$ from Corollary~\ref{cor:YDactEHA}, and recall the category isomorphism $\cE_{\lmod{H}}({\lmod{A}}) \cong  {}_A^{\widehat{H}}  \mathsf{DH}(H)$ from Proposition~\ref{prop:ECM-DH}. Then \eqref{eq:DH-YDmod} follows from \eqref{eq:ECMYDmod} by way of Proposition~\ref{prop:structuretransfer}(a). Namely, the functor $G$ in Proposition~\ref{prop:functorG} gives us 
$$e_X^M(x \otimes m) = \textstyle \sum_j (g_j \cdot x) \otimes (p_j \ast m) = (m_{-1} \cdot x) \otimes m_0 \; \in X \otimes M,$$
and  the functor $F$ in Proposition~\ref{prop:functorF} gives us $\widetilde{\varphi}(v \otimes m) = e_H^{V \otimes M}(1_H \otimes v \otimes m)$. Thus, \eqref{eq:DH-YDmod} is justified. The formula for $\widetilde{\ast}$ follows from Lemma~\ref{lem:braid-Kmatr}(a). 
\end{proof}

Next, by way of the isomorphism of braided monoidal categories, $\lYD{H} \overset{\otimes}{\cong} \lmod{\Drin(H)}$ from~\eqref{eq:Drin} and \eqref{eq:YD-Drin}, we get the consequence below.


\begin{corollary} \label{cor:DrinactDH}
When $H$ is finite-dimensional, we get that the category of Doi--Hopf modules ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ is a left module category over $\lmod{\Drin(H)}$ via:
\[
\begin{array}{rl}
\widetilde{\blkact}: \lmod{\Drin(H)} \;\times \; {}_A^{\widehat{H}}  \mathsf{DH}(H) &\longrightarrow {}_A^{\widehat{H}}  \mathsf{DH}(H)\\[.4pc]
\left( (V, \varodot, \diam), \; (M, \ast, \varphi) \right) & \mapsto 
\left( V \otimes M, \; \widetilde{\ast}, \; \widetilde{\varphi}\right),
\end{array}
\]
for $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$,  and 
\begin{align} \label{eq:DH-Drinmod}
\widetilde{\varphi}(v \otimes m) =  \textstyle \sum_{k} t_k m_{-1} S^{-1}(h_d) \otimes (s_k \varodot (\xi_d \diam v)) \otimes m_0.
\end{align}
Here, $\{h_d, \xi_d\}_d$ is a dual basis of $H$.
\qed 
\end{corollary}
...
\subsection{Reflective algebras as comodule algebras over Drinfeld doubles}  \label{sec:RA-DD}
Take $H$ to be a finite-dimensional quasitriangular Hopf algebra for this section.

\begin{proposition} \label{prop:RA-DD}
The category of modules over the reflective algebra $R_H(A)$ is a left module category over $\lmod{\Drin(H)}$ via:
\[
\begin{array}{rl}
\widetilde{\grayact}: \lmod{\Drin(H)} \;\times \; \lmod{R_H(A)} &\longrightarrow \lmod{R_H(A)}  \\[.4pc]
\left( (V, \varodot, \diam), \; (M, \ast, \star) \right) & \mapsto 
\left( V \otimes M, \; \widetilde{\ast}, \; \widetilde{\star} \right),
\end{array}
\]
for $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$, and
\begin{align} \label{eq:RHA-Drinmod}
\xi \; \widetilde{\star} \; (v \otimes m) := \textstyle \sum_{k,d,d'} \langle \xi, t_k h_{d'} S^{-1}(h_d) \rangle \; (s_k \varodot (\xi_d \diam v)) \otimes (\xi_{d'} \star m).
\end{align}
Here, $\{h_d, \xi_d\}_d$, $\{h_{d'}, \xi_{d'}\}_{d'}$ are dual bases of $H$.
\end{proposition}



\begin{proof}
Take the action of $\lmod{\Drin(H)}$ on ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ from Corollary~\ref{cor:DrinactDH}, and recall the category isomorphism ${}_A^{\widehat{H}}  \mathsf{DH}(H) \cong \lmod{R_H(A)}$ from Corollary~\ref{cor:isom-refl-alg}. Then \eqref{eq:RHA-Drinmod} follows from \eqref{eq:DH-Drinmod} by way of Proposition~\ref{prop:structuretransfer}(a). Namely, the functor $\Omega^{-1}$ in Corollary~\ref{cor:isom-refl-alg} gives us 
$$\varphi(m) \; =  \; m_{-1} \otimes m_0  \; =  \; \textstyle \sum_{d'} h_{d'} \otimes (\xi_{d'} \star m) \; \in H \otimes M.$$
Then, for $\widetilde{\varphi}(v \otimes m) := (v \otimes m)_{\widetilde{-1}} \otimes (v \otimes m)_{\widetilde{0}}$, the functor $\Omega$ in Corollary~\ref{cor:isom-refl-alg}  gives us 
$$\xi \; \widetilde{\diam} \; (v \otimes m) = \langle \xi, (v \otimes m)_{\widetilde{-1}} \rangle (v \otimes m)_{\widetilde{0}}.$$
Thus, \eqref{eq:RHA-Drinmod} is justified. The formula for $\widetilde{\ast}$ follows from Lemma~\ref{lem:braid-Kmatr}(a). 
\end{proof}

Now we obtain that $R_H(A)$ is a left $\Drin(H)$-comodule algebra. 

\begin{corollary} \label{cor:Drin-RHA-comod}
The reflective algebra $R_H(A)$ is a left $\Drin(H)$-comodule algebra  as follows:
\begin{align*}
...
\end{align*}
for $a \in A$ and $\xi \in (\widehat{H}^*)^{\textnormal{op}}$. Here, recall that $\sum_k s_k \otimes t_k$ is the $R$-matrix of $H$, and that $\{h_d, \xi_d\}_d$,  $\{h_{d'}, \xi_{d'}\}_{d'}$ are dual bases of $H$. 
\end{corollary}

\begin{proof}
It follows from Lemma \ref{lem:cross-prod} that 
\begin{equation*} \label{eq:delta-Drinref}
r \cdot (x \otimes m) = \delta^{\textnormal{{\tiny Drin}}}_{\textnormal{ref}}(r) (x \otimes m),
\end{equation*}
for all $r \in R_H(A)$, $x \in X$, $m \in M$. So, the formulas for $\delta^{\textnormal{{\tiny Drin}}}_{\textnormal{ref}}(a)$ and $\delta^{\textnormal{{\tiny Drin}}}_{\textnormal{ref}}(\xi)$ follow from $\widetilde{\ast}$ and $\widetilde{\star}$, respectively, in Proposition~\ref{prop:RA-DD}. In particular, the formula for $\delta^{\textnormal{{\tiny Drin}}}_{\textnormal{ref}}(\xi)$  holds by \eqref{eq:RHA-Drinmod} via \eqref{eq:Drin-prod}.
\end{proof}
...
\subsection{Induced (co)actions on reflective centers and reflective algebras}  \label{sec:ind-RC-RA}
Recall the embeddings of braided monoidal categories, $I_+$ and $I_-$, from Section~\ref{sec:braided}. We then obtain the braided monoidal categories embeddings below:
\[
\begin{array}{lccl}
I_+: \lmod{H} \to \cZ(\lmod{H}) 
&&& 
I_-: \overline{\lmod{H}} \to \cZ(\lmod{H}) \\[.2pc]
\hspace{.32in} (V, \varodot) \mapsto (V, \varodot, c^V) \;  \text{for } c^V_{X} := c_{X,V},
&&&
\hspace{.32in}  (V, \varodot) \mapsto (V, \varodot, (c^{-1})^V) \; \text{for } (c^{-1})^V := c^{-1}_{V,X}.
\end{array}
\]

\noindent It now follows from \eqref{eq:ZC-YD} that we get such embeddings into categories of Yetter--Drinfeld modules.

\begin{lemma}
We have the following embeddings of braided monoidal categories: 
\[
\begin{array}{lccl}
I_+: \lmod{H} \to \lYD{H}
&&& 
I_-: \overline{\lmod{H}} \to \lYD{H} \\[.2pc]
\hspace{.32in} (V, \varodot) \mapsto (V, \; \varodot, \; \partial^V_+),
&&&
\hspace{.32in}  (V, \varodot) \mapsto (V, \; \varodot, \; \partial^V_-).
\end{array}
\]
Here, $\partial^V_+(v) = \textstyle \sum_i s^i \otimes (t^i \varodot v)$ and 
$\partial^V_-(v) = \textstyle \sum_i t_i \otimes (s_i \varodot v)$. \qed
\end{lemma}

Now we use the monoidal structure of the embeddings above and Proposition~\ref{prop:brmodsource}(a) to achieve the result below.

\begin{proposition} \label{prop:induced}
The action $\widetilde{\blkact}$ of $\lYD{H}$ on ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ from Proposition~\ref{prop:DH-YD} induces the following module category actions:
\[
\begin{array}{rlrl}
\widetilde{\blkact}_+: \lmod{H} \;\times \; {}_A^{\widehat{H}}  \mathsf{DH}(H) & \hspace{-.1in} \longrightarrow {}_A^{\widehat{H}}  \mathsf{DH}(H)
&
\widetilde{\blkact}_-: \overline{\lmod{H}} \;\times \; {}_A^{\widehat{H}}  \mathsf{DH}(H) & \hspace{-.1in}  \longrightarrow {}_A^{\widehat{H}}  \mathsf{DH}(H)\\[.4pc]
\left( (V, \varodot), \; (M, \ast, \varphi) \right) &\hspace{-.1in}  \mapsto 
\left( V \otimes M, \; \widetilde{\ast}_+, \; \widetilde{\varphi}_+ \right),
&
\left( (V, \varodot), \; (M, \ast, \varphi) \right) &\hspace{-.1in}  \mapsto 
\left( V \otimes M, \; \widetilde{\ast}_-, \; \widetilde{\varphi}_- \right),
\end{array}
\]
for $\widetilde{\ast}_{\pm}:=\widetilde{\ast}$ with $a \; \widetilde{\ast} \; (v \otimes m) = (a_{[-1]} \varodot v) \otimes (a_{[0]} \ast m)$ with $a \in A$, $v \in V$, $m \in M$,  and 
\begin{align} \label{eq:induced+}
\widetilde{\varphi}_+(v \otimes m) &=  \textstyle \sum_{i,j} t_j m_{-1} s_i \otimes (s_j t_i \varodot v) \otimes m_0,\\[.2pc]
\label{eq:induced-}
\widetilde{\varphi}_-(v \otimes m) &=  \textstyle \sum_{j} (t_j)_{(2)} m_{-1} S^{-1}((t_j)_{(1)}) \otimes (s_j \varodot v) \otimes m_0.
\end{align}
Here, $\varphi(m):= m_{-1} \otimes m_0$. \qed
\end{proposition}

In particular, we used \eqref{eq:QT5} to get \eqref{eq:induced+}, and \eqref{eq:QT2} to get \eqref{eq:induced-} above.

\begin{remark}
Note that the induced action $\widetilde{\blkact}_+$ is the same as the action $\blkact$ of $\lmod{H}$ on ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ from Proposition~\ref{prop:ECM-DH-brmod}(a). On the other hand, the induced action $\widetilde{\blkact}_-$ of $\overline{\lmod{H}}$ on ${}_A^{\widehat{H}}  \mathsf{DH}(H)$ appears as a special case of a module category action  $\blkact'$ on Doi--Hopf modules described in Appendix~\ref{sec:appendix}.
 \end{remark}
...
\section{An alternate way Doi--Hopf modules are module categories} \label{sec:appendix}

Recall the notation of Section~\ref{sec:Doi--Hopf}. In this part, we show that the category of left $(L,B,C)$-Doi--Hopf modules admits the structure of a left module category over the braided monoidal category $\lmod{L}$ when $L$ is quasitriangular, with $R$-matrix $\sum_i s_i \otimes t_i$.

\begin{proposition} \label{prop:DH-modcat-gen} 
Retain the notation above. Then we get that  ${}^C_B  \mathsf{DH}(L)$ is a left  module category over $\lmod{L}$ via the action:
\begin{align*}
\blkact' \; : \lmod{L} \; \times \;  {}^C_B  \mathsf{DH}(L) \; &\longrightarrow \;  {}^C_B  \mathsf{DH}(L)\\
\quad \left((X, \cdot),(M, \ast, \varphi)\right) &\mapsto \left(X \otimes M, \;  \widetilde{\ast}, \; \widetilde{\varphi} \right),
\end{align*}
where $\varphi: M \to C \otimes M$ for $\varphi(m):=m_{-1} \otimes m_0$, where $b \; \widetilde{\ast} \; (x \otimes m):=(b_{[-1]} \cdot x) \otimes (b_{[0]} \ast m)$, and where $\widetilde{\varphi}(x \otimes m):= \textstyle \sum_i (t_i \rightharpoonup m_{-1}) \otimes (s_i \cdot x) \otimes m_0$, for $m \in M$, $b \in B$, $x \in X$.
\end{proposition}

\begin{proof}
First, we see that $(X \otimes M, \;\widetilde{\ast})$ is a left $B$-module via the following calculation:
{\small
\begin{align*}
b' \; \widetilde{\ast} \; (b \; \widetilde{\ast} \; (x \otimes m)) 
& \; = \; b' \ast ((b_{[-1]} \cdot x) \otimes (b_{[0]} \ast m)) \\
& \; = \; ((b'_{[-1]}b_{[-1]}) \cdot x) \otimes ((b'_{[0]}b_{[0]}) \ast m)
\; = \; (b'b) \; \widetilde{\ast} \; (x \otimes m),
\end{align*}
}

\noindent for all $b,b' \in B$, $x \in X$, and $m \in M$.
Next, $(X \otimes M, \;\widetilde{\varphi})$ is a left $C$-comodule via the  calculation:
{\small
\begin{align*}
(\ide_C \otimes  \widetilde{\varphi})\widetilde{\varphi}(x \otimes m) 
&\; = \; 
\textstyle \sum_i (t_i \rightharpoonup m_{-1}) \otimes \widetilde{\varphi}((s_i \cdot x) \otimes m_0)\\[.2pc]
&\; = \; 
\textstyle \sum_{i,j} (t_i \rightharpoonup m_{-2}) \otimes (t_j \rightharpoonup m_{-1}) \otimes (s_j s_i \cdot x) \otimes m_0 \\[.2pc]
&\; \overset{\textnormal{\eqref{eq:QT2}}}{=} \; 
\textstyle \sum_{i} ((t_i)_{(1)} \rightharpoonup m_{-2}) \otimes ((t_i)_{(2)} \rightharpoonup m_{-1}) \otimes (s_i \cdot x) \otimes m_0 \\[.2pc]
&\; = \; 
\textstyle \sum_{i} (t_i \rightharpoonup \Delta_C(m_{-1})) \otimes (s_i \cdot x) \otimes m_0 \\[.2pc]
&\; = \;
 (\Delta_C \otimes \ide_{X \otimes M})\widetilde{\varphi}(x \otimes m) 
\end{align*}
}

\noindent for all $x \in X$ and $m \in M$. Finally, $(X \otimes M, \; \widetilde{\ast},\; \widetilde{\varphi})$ satisfies \eqref{eq:Doi-Hopf}, which we see as follows:
{\small
\begin{align*}
&(b \; \widetilde{\ast} \; (x \otimes m))_{\widetilde{-1}}\; \otimes \; (b\; \widetilde{\ast} \;  (x \otimes m))_{\widetilde{0}} \\[.2pc]
& \; = \;
((b_{[-1]} \cdot x) \otimes (b_{[0]} \ast m))_{\widetilde{-1}} \; \otimes \;
((b_{[-1]} \cdot x) \otimes (b_{[0]} \ast m))_{\widetilde{0}} \\[.2pc]
& \; = \;
\textstyle \sum_i (t_i \rightharpoonup (b_{[0]} \ast m)_{-1}) \otimes (s_i b_{[-1]} \cdot x) \otimes (b_{[0]} \ast m)_{0}\\
& \; \overset{\textnormal{\eqref{eq:Doi-Hopf}}\text{ for $M$}}{=} \;
\textstyle \sum_i (t_i b_{[-1]} \rightharpoonup  m_{-1}) \otimes (s_i b_{[-2]} \cdot x) \otimes (b_{[0]} \ast m_0)\\
& \; \overset{\textnormal{\eqref{eq:QT3}}}{=} \;
\textstyle \sum_i (b_{[-2]} t_i  \rightharpoonup  m_{-1}) \otimes (b_{[-1]} s_i  \cdot x) \otimes (b_{[0]} \ast m_0)
\\[.2pc]
& \; = \;
\textstyle \sum_i (b_{[-1]} t_i  \rightharpoonup  m_{-1}) \otimes (b_{[0]} \; \widetilde{\ast} \; ((s_i  \cdot x) \otimes m_0)) \\[.2pc]
& \; = \;
(b_{[-1]} \rightharpoonup  (x \otimes m)_{\widetilde{-1}}) \otimes (b_{[0]} \; \widetilde{\ast} \; (x \otimes m)_{\widetilde{0}}),
\end{align*}
}

\noindent for all $b \in B$, $x \in X$, and $m \in M$.
\end{proof}

\begin{remark}
As a special case, when $\lmod{L} = \overline{\lmod{H}}$, $B = A$, and $C = \widehat{H}$, we obtain that  
$\blkact'$
recovers the action $\blkact_-$ from Proposition~\ref{prop:induced}.
\end{remark}
...
R.~Laugwitz was supported by a Nottingham Research Fellowship. C.~Walton was partially supported by the Alexander Humboldt Foundation, and the US National Science Foundation grant \#DMS-2100756. C. Walton was also hosted by the University of Hamburg during many phases of this project, and she would thank to her hosts for their great hospitality and for providing excellent working conditions.
