...
We start by reviewing Gonzalez's algorithm, a classical solution to the problem in the static setting, and then describe how to adapt it to the fully-dynamic setting by using fully-dynamic approximate SSSP algorithms.
...
\bigskip
The update procedure requires that $\mathcal{D}$ is updated $2k+1$ times (Lines 15, 5, 8), with a worst-case time of $T(n,m, \epsilon)$ per update, and additionally look for the approximate furthest neighbor $k$ times (Line~4), each requiring time $O(n)$, i.e., querying the approximate distance $\delta_G(s,v)$, $\forall v\in V$.
\end{proof}
