...
\begin{proof}
    ...
    \tbrs in $G_r$, with $r \geq 2R^*$. We can assume WLOG that the set
    ...
    $C_1^* \cup \dots \cup C_{i-1}^*$. As a result, we can assume WLOG that $v_i \in C_i^*$. By following the same approach as in the base case, we have that every vertex $u \in C_i^*$
...
\begin{proof}
    ...
    and let $M'$ be an MIS in $G_r$ such that 
