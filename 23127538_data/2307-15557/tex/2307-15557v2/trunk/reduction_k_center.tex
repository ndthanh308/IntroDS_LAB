...
It is well-know that the $k$-center problem can be reduced to the problem of finding an MIS on a graph. This reduction was first given by Hochbaum and Schmoys~\cite{HochbaumS86}, 
in order to get a $2$-approximation algorithm, and also it has been used by \cite{chan2018fully, BateniEFHJMW23} 
for the fully dynamic $k$-center problem on point sets in arbitrary metric spaces.
...
A similar definition was also given in \cite{BateniEFHJMW23} for the MIS. Recall that an MIS is a $(2, 1)$-ruling set. 
...
\begin{lemma}\label{lem:2b_Gr_H}
    ...
    Assume that there is an algorithm $\mathcal{A}$ such that, given $G, r, \epsilon',\beta$, 
