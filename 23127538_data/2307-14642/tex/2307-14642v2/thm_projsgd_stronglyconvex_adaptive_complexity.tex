...
\begin{theoremEnd}[category=complexityprojsgdadaptiveqvcfixed]{lemma}[\textbf{Strongly convex \(F\) with adaptive QV and Fixed Stepsize}]\label{thm:projsgd_stronglyconvex_adaptive_complexity}
  For a \(\mu\)-strongly convex \(F : \Lambda \to \mathbb{R}\) on a convex set \(\Lambda\) with a unique global minimizer \(\vlambda^* \in \Lambda\), the last iterate \(\vlambda_T\) of projected SGD with a gradient estimator satisfying an adaptive QV bound (\cref{assumption:adaptiveqvc}) and a fixed stepsize satisfies a suboptimality of \(\norm{\vlambda_T - \vlambda_{*}}_2^2 < \epsilon\) if
{%
