...
\begin{proofEnd}
  ...
  \begin{align*}
    ...
    =
    -
    {\left( \mC\mC^{\top} \right)}^{-1}
    +
    \operatorname{diag}\left(\mSigma^{-1}\right).
  \end{align*}
  The first-order optimality condition with respect to \(\mC\) is then
  \[
    {\left( \mC\mC^{\top} \right)}^{-1} = \operatorname{diag}\left(\mSigma^{-1}\right).
  \]
  Since, by assumption, both sides are invertible, the unique solution \(\mC^*\) is
  \[
    \mC_* = {\operatorname{diag}\left(\mSigma^{-1}\right)}^{\nicefrac{1}{2}}.
  \]
\end{proofEnd}
