...
\begin{theoremEnd}[all end, category=complexityprojsgdqvcdec]{theorem}[\textbf{Strongly convex \(F\) with a decreasing stepsize schedule}]\label{thm:projsgd_stronglyconvex_decstepsize}
  For a \(\mu\)-strongly convex \(F : \Lambda \to \mathbb{R}\) on a convex set \(\Lambda\) with a unique global minimizer \(\vlambda^* \in \Lambda\), the last iterate \(\vlambda_{T}\) of projected SGD with a descreasing stepsize satisfies \( \mathbb{E}{\lVert \vlambda_T - \vlambda^* \rVert}_2^2 \leq \epsilon \) if
{%
...
\end{theoremEnd}
\begin{proofEnd}\label{proof:projsgd_stronglyconvex_decstepsize}
  ...
  \]
  Following a similar strategy to \citet{kim_convergence_2023}, we can obtain a computational complexity by solving for the smallest \(T\) that achieves
  \[
    \frac{64 \alpha}{\mu^2} \frac{ \norm{\vlambda_0 - \vlambda^*}_2^2 }{T^2} 
    +
...
\shortintertext{applying the inequality \(\sqrt{a + b} \leq \sqrt{a} + \sqrt{b}\) for \(a, b \geq 0\),}
    ...
    = \frac{\left(-B\right)}{A} + \frac{ \sqrt{ \left( -C \right) }  } { \sqrt{A} }
    \\
    &= \frac{ 16 \beta }{ \mu^2 \epsilon} + \frac{ \sqrt{  \frac{64 \alpha^2}{\mu^4} \norm{\vlambda_0 - \vlambda^*}_2^2  }  } { \sqrt{\epsilon} }
    \\
    &= \frac{ 16 \beta }{ \mu^2 \epsilon} + \frac{ 8 \alpha \norm{\vlambda_0 - \vlambda^*}_2 } { \mu^2 \sqrt{\epsilon} }.
  \end{align*}
\end{proofEnd}
