...
\begin{enumerate}
    ...
    \item We bound the family-misspecification term \(T_{\text{\ding{184}}}\) with the Fisher-Hyv\"arinen divergence \(D_{\mathrm{F}^4}\left(q_{\vlambda^*}, \pi\right)\), while \citet{domke_provable_2023a} bound it with a quadratic involving the smoothness constant of the residual function \(r\left(\vz\right) \triangleq \log q_{\vlambda^*}\left(\vz\right) - \log \pi\left(\vz\right)\).
    
    \item For the Gaussian posterior case, the constants of \cref{thm:stl_upperbound} are tighter that of \citet{domke_provable_2023a}.
    
    \item We also provide an upper bound for the mean-field variational family in \cref{thm:stl_upperbound_mf}.
    
    \item We establish a lower bound on the gradient variance of STL, quantifying the tightness of the bounds.
\end{enumerate}
