...
\setlength{\abovedisplayskip}{1.5ex} \setlength{\abovedisplayshortskip}{1.5ex}
  \begin{align*}
    ...
    \leq \alpha_{\mathrm{CFE}} {\lVert \vlambda - \vlambda^* \rVert}_2^2 + \beta_{\mathrm{CFE}}
  \end{align*}
  }%
  where 
{\setlength{\belowdisplayskip}{1.5ex} \setlength{\belowdisplayshortskip}{1.5ex}
\setlength{\abovedisplayskip}{1.5ex} \setlength{\abovedisplayshortskip}{1.5ex}
  \begin{align*}
    \alpha_{\mathrm{CFE}} &= L^2 \left( d + k_{\varphi} \right) \left(1 + \delta\right) + {\left(L + S \right)}^{2} \\
    \beta_{\mathrm{CFE}} &=  L^2 \left( d + k_{\varphi} \right) \left(1 + \delta^{-1} \right) {\lVert \vlambda^* - \bar{\vlambda} \rVert}_2^2
  \end{align*}
  }%
  for any \(\vlambda, \vlambda^* \in \Lambda_S\) and \(\delta > 0\), where \(\bar{\vlambda} = \left(\bar{\vz}, \mathbf{0}\right)\) and \(\bar{\vz}\) is any stationary point of \(f\).
\end{theoremEnd}
\vspace{-1ex}
\begin{proofEnd}\label{proof:cfe_upperbound}
  Following the notation of \citet{domke_provable_2023}, we denote \( \log \ell = f \).
...
\shortintertext{and \cref{eq:thm_cfe_entropy},}
    ...
    \\
    &=
    \left( L^2 \left( d + k_{\varphi} \right) \left(1 + \delta\right) + {\left(L + S \right)}^{2} \right) {\lVert \vlambda - \vlambda^* \rVert}_2^2 
    +
...
\end{theoremEnd}
\begin{proofEnd}\label{proof:cfe_upperbound_mf}
...
\shortintertext{applying \cref{thm:jacobian_reparam_inner},}
    ...
    \nonumber
\shortintertext{from \(L\)-smoothness of \(f = \log \ell\),}
    &\leq
...
\shortintertext{We finally apply \cref{eq:peterpaul} as}
  ...
  \begin{align*}
    ...
    +
    \norm{ \nabla F\left(\vlambda\right) }_2^2
\shortintertext{and applying \cref{eq:cfe_meanfield_energy},}
    &\leq
    \mathbb{E}_{\rvvz \sim q_{\vlambda}} \norm{ \nabla_{\vlambda} f\left( \rvvz \right) }_2^2
    +
    {\left(L + S \right)}^{2} \norm{ \vlambda - \vlambda^* }_2^2
