...
\begin{remark}
    Equivalently, we say a function \(f\) is \(L\)-log-smooth if \(\log f\) is \(L\)-smooth.
\end{remark}
...
\begin{remark}
    Equivalently, we say a function \(f\) is only convex if it satisfies the strong convexity inequality with \(\mu = 0\).
\end{remark}

\begin{remark}
    We say a probability measure is strongly log-concave if the log of its probability density function is strongly convex.
\end{remark}
