...
\begin{proofEnd}
  ...
  \mathbb{E} 
  \left(1 + \norm{\rvvu}_2^2 \right)
  \left( \mC \rvvu + \vm \right)
  =
  \mC \mathbb{E} \left(1 + \norm{\rvvu}_2^2 \right) \rvvu + \vm
  =
  \vm,
  \]
...
\begin{theoremEnd}[category=stllowerboundunimprovability]{theorem}\label{thm:stl_lowerbound_unimprovability}
 Assume \cref{assumption:variation_family}.
  There exists a strongly-convex, \(L\)-log-smooth posterior and some variational parameter \(\widetilde{\vlambda} \in \Lambda_{L}\) such that
  \[
    \mathbb{E} {\lVert \rvvg_{\mathrm{STL}}\left(\widetilde{\vlambda}\right) \rVert}_2^2
    \geq
    2 \left( d L^2 - 2 d\right) \, \norm{\widetilde{\vm} - \bar{\vz}}_2^2
    +
    \left( d L^2 + k_{\varphi} L^2 - 3 d \right) {\lVert \widetilde{\mC} \rVert}_{\mathrm{F}}^2
    -4 d {\left(L - L^{-1} \right)},
  \]
...
\begin{proofEnd}
  ...
  \begin{enumerate}[label=\textbf{(\roman*)}]
  \item \textbf{\(\pi\) ill-conditioned such that the smoothness constant is large.} \\
    This results in the domain \(\Lambda_{L}\) to include ill-conditioned \(\mC\), which has the largest impact in the gradient variance. Furthermore, 
...
\shortintertext{applying \cref{lemma:unorm_times_reparam,thm:u_identities} to the last two inner product terms,}
    &=
    \mathbb{E} 
    \left(1 + \textstyle{\sum^{d}_{i=1} \rvu_i^2} \right)
    \Big(
...
\shortintertext{applying \cref{thm:normdist_1pnormu},}
    ...
    \left(d - 1\right) L^2 \norm{\widetilde{\vm} - \bar{\vz}}_2^2,
    \nonumber
\shortintertext{and since \(L^{-2} > 0\) and is negligibly,}
    &\geq
    L^2
    \left(
      \left( d + 1 \right) \norm{\widetilde{\vm} - \bar{\vz}}_2^2
...
\paragraph{Lower Bound on \(T_{\text{\ding{174}}}\)}
  ...
  \begin{align*}
    \mathbb{E} \norm{\rvvg_{\mathrm{STL}}\left(\widetilde{\vlambda}\right)}_2^2
    &\geq
    ...
    \\
    &=
    2 \left( d L^2 - d - k_{\varphi} - 1\right) \, \norm{\widetilde{\vm} - \bar{\vz}}_2^2
    +
    \left( d L^2 + k_{\varphi} L^2 - 2 d - 2 \right) {\lVert \widetilde{\mC} \rVert}_{\mathrm{F}}^2
    \\
    &\quad-2
    \left( d + k_{\varphi} \right)
    {\left(L - L^{-1} \right)}.
\shortintertext{We can further simplify by assuming \(d \geq k_{\varphi} + 1 = 4\), since \(k_{\varphi} = 3\) for a Gaussian \(\varphi\), as}
    &\geq
    2 \left( d L^2 - 2 d\right) \, \norm{\widetilde{\vm} - \bar{\vz}}_2^2
    +
    \left( d L^2 + k_{\varphi} L^2 - 3 d \right) {\lVert \widetilde{\mC} \rVert}_{\mathrm{F}}^2
