...
\begin{assumption}[\textbf{Adaptive QV}]\label{assumption:adaptiveqvc}
  ...
  \]
  for any \(\delta > 0\), any \(\vlambda \in \Lambda_S\), and some \(0 < \widetilde{\alpha}, \widetilde{\beta} < \infty\).
\end{assumption}
%
...
As mentioned in \citet[Remark 6]{kim_practical_2023}, adjusting \(\delta\) can occasionally tighten the analysis by a constant factor.
...
Here, \(J_{\mathcal{T}}\) is a term that stems from the Jacobian of the parameterization function \(\mathcal{T}\).
Thus, \(J_{\mathcal{T}}\) contains the properties unique to the chosen variational family.
\(T_{\text{\ding{182}}}\) and \(T_{\text{\ding{183}}}\) measure how far the current variational approximation \(q_{\vlambda}\) is from the optimal variational approximation \(q_{\vlambda^*}\).
...
Without further due, we present our upper bound on the expected-squared norm of the STL gradient estimator.
%
...
\begin{remark}[\textbf{Interpolation Condition}]
  This bound is general, encompassing both settings where the variational family is either well specified or misspecified.
  That is when the variational family is well specified such that \( \mathrm{D}_{\mathrm{F}^4}\left(q_{\vlambda^*}, \pi\right) = 0 \), we obtain interpolation such that \(\beta_{\mathrm{STL}} = 0\).
\end{remark}
...
\begin{remark}\label{remark:stl_tightness}
  \cref{thm:stl_lowerbound_unimprovability} implies that \cref{thm:stl_upperbound_corollary} is tight with respect to the dimension dependence \(d\) and the log-smoothness \(L\) except for a factor of 2.
\end{remark}

\begin{remark}[\textbf{Room for Improvement}]
  The factor of \(2\) looseness is due to the extreme worst case: when \(\nabla \log \pi\) and \(\nabla \log q_{\vlambda}\) are perfectly anti-correlated.
  For the score of probability distributions, this worst case is unlikely to appear in practice, thus making a tighter lower bound challenging to obtain.
...
\begin{remark}[\textbf{Comparison with STL}]\label{remark:variance_comparison}
    ...
    Considering \cref{thm:stl_lowerbound_unimprovability}, the difference should be closer to a factor of 2 in practice.
    This means that the STL estimator will have a larger variance when \({\lVert \bar{\vlambda} - \vlambda^* \rVert}_2^2\) large.
...
This type of convergence analysis, which has been popular in the ERM sample selection strategy literature~\citep[\S 1.1]{csiba_importance_2018}, is convenient for comparing the lower-order or even factor improvements of different gradient estimators.
...
\subsection{Non-Asymptotic Complexity of Black-Box Variational Inference}
...
Although the bounds for the mean-field parameterization have better dependences on \(d\), they are not tight~\citep{kim_practical_2023}. (See also \citealp[Conjecture 1]{kim_blackbox_2023}.)
...
\paragraph{Complexity of BBVI on Strongly-Log-Concave Posteriors}
...
\begin{remark}
  The overall complexity of the STL estimator is larger by a constant factor of 4 compared to that of the CFE estimator.
...
%%   where 
%%   \[
%%      \mu \boldupright{I} \leq {\mSigma}^{-1} \leq L \boldupright{I}.
%%   \]
