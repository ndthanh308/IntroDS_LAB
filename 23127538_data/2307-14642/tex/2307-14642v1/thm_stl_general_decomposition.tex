...
\begin{theoremEnd}[category=gradvarlemmas,all end]{lemma}\label{thm:peterpaul}
  ...
  \[ {(a+b+c)}^2 \leq (2 + \delta ) a^2 + (2 + \delta ) b^2 + (1 + 2 \delta^{-2})  c^2, \]
  for any \(\delta \geq 0\).
\end{theoremEnd}
\begin{proofEnd}
...
%
  ...
  \begin{align*}
    ...
    {\rVert}_2^2,
  \end{align*}
  for any \(\delta \geq 0\) and \(\vlambda \in \mathbb{R}^p\).
  \(J_{\mathcal{T}} : \mathbb{R}^d \to \mathbb{R}\) is a function depending on the variational family as
  \begin{alignat*}{2}
    J_{\mathcal{T}}\left(\rvvu\right) &= 1 + {\textstyle\sum^{d}_{i=1} \rvu_i^2} \quad                        &&\quad \text{for the full-rank and} \\
    J_{\mathcal{T}}\left(\rvvu\right) &= 1 + {\textstyle\sqrt{\sum^{d}_{i=1} \rvu_i^4}} &&\quad \text{for the mean-field parameterizations.}
  \end{alignat*}
 }%
\end{theoremEnd}
\begin{proofEnd}
