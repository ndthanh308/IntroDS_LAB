...
%Edited and submitted  on March 4, 2025 by Shabnam.
...
\sigma_w(k) \subseteq \R,\quad\text{and}\quad \sigma_w(k) = \sigma_w(K) \cap \R.
\end{equation}
...
It also follows from the equality in (\ref{extra558}) that the completion $k_x$ at the unique place $x$ such that $w | x$ is isomorphic 
...
(\ref{extra523}) fixes the subfield $\sigma_w\bigl(k\bigr)$.
\end{proof}
...
\begin{corollary}\label{corhaar1}  Let $K$ be an algebraic number field that satisfies the hypotheses of  Lemma \ref{lemhaar2}.  For
each archimedean place $w$ of $K$ let $\xi^{(w)}$ be a nonzero element of $O_K$
that satisfies (\ref{extra498}) from Lemma \ref{lemhaar2}, and suppose
\begin{equation*}
...
For the remainder of the proof we assume that the totally complex number field  $K$ has at least two archimedean places.  Let $w$ be an archimedean 
...
The authors are  grateful to the anonymous referee for the careful reading and  valuable  suggestions.
