\begin{table}[ht!]
\small
\centering
\resizebox{\columnwidth}{!}{%
\setlength{\tabcolsep}{4pt}
\begin{tabular}{cc|c|cc}
&& \textbf{best} & \multicolumn{2}{|c}{\textbf{supervised}}
\\
\multicolumn{2}{c|}{\textbf{Task/Language}} & \textbf{ZSL} & \textbf{Standard ML}   & \textbf{Transformer} \\
\toprule
\multirow{3}{*}{SA} 
& EN & 0.553 & 0.610 & \textbf{0.680}
\\
& DE & 0.517 & 0.610 & \textbf{0.677}
\\
& FR & 0.528 & 0.612 & \textbf{0.706}
\\
\midrule
\multirow{3}{*}{AC-Gender} 
& EN & 0.624 & 0.601 & \textbf{0.638}
\\
& DE & 0.497 & 0.540 & \textbf{0.629}
\\
& FR & 0.576 & 0.546 & \textbf{0.650}
\\
\midrule
\multirow{3}{*}{AC-Age} 
& EN & 0.590 & 0.620 & \textbf{0.636}
\\
& DE & 0.528 & 0.602 & \textbf{0.611}
\\
& FR & 0.545 & 0.540 & \textbf{0.568}
\\
\midrule
\multirow{3}{*}{TD} 
& EN & 0.596 & 0.656 & \textbf{0.705}
\\
& DE & 0.520 & 0.605 & \textbf{0.671}
\\
& FR & 0.314 & 0.385 & \textbf{0.444}
\\
\midrule
\multirow{2}{*}{HS} 
& EN & \textbf{0.732} & 0.451 & 0.416
\\
& ES & 0.603 & 0.701 & \textbf{0.752}
\\
\bottomrule
\end{tabular}
}
\caption{Macro-F1 results for best ZSL model (Table \ref{tab:results_trustpilot}), compared to previous supervised results on the same datasets.} \label{tab:results_trustpilot_comp}
\end{table}