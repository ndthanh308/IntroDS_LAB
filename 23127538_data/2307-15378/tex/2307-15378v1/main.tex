...
\newcommand{\newtextAS}[1]{\textcolor{black}{#1}}
\newcommand{\newtextPB}[1]{\textcolor{black}{#1}}
...
\label{Intro}
Experminelatlly, we can study partons (quarks and gluons) by analyzing jets 
...
~\cite{Gallicchio:2011xq,FerreiradeLima:2016gcz,Whitmore:2019cri,Bhattacherjee:2016bpy}.
...
\newtext{In this paper, we study a variety of angularity observables as measures of quark/gluon jet differences.
...
\label{sec:strategy}
The LHC has collected data at many different energies: $900$~GeV, $2.36$~TeV, \newtext{$5.02$~TeV}, $7$~TeV, $8$~TeV and 
...
\herwig{ 7}~\cite{Bellm:2015jjp, Bellm:2019zci},
\pythia{ 8}~\cite{Sjostrand:2014zea,Bierlich:2022pfr} and 
...
% Figure environment removed
As we can see from the figure,  despite a large separation power, this observable suffers from various problems. First, it is not robust to MPI and ISR effects, which leads to unphysical negative bins of the probability distributions. It is also very energy dependent.  For this reason, it is clear that this single measure is not suitable for our problem; therefore, we introduce additional measures to evaluate the robustness to the important systematic effects. 
...
\newtextAS{call it quark (gluon) negativity.}
We show the distributions of all of these measures in the Appendix (including repetition of Figs.~\ref{fig:separation} and~\ref{fig:separation_ang} for completeness). We see that the MPI and ISR affect larger radius jets more than smaller (Fig.~\ref{fig:negativity_R_MPI}), as would be expected, and multiplicity much more than the other angularities (Fig.~\ref{fig:negativity_ang_MPI}). Its effect gets somewhat less important at higher \ptcutwb\ (Fig.~\ref{fig:negativity_Q_MPI}). The energy-dependence shows a rather similar behaviour (Figs.~\ref{fig:negativity_R_E}--\ref{fig:negativity_Q_E}), except that the smallest jet radius, 0.2, shows a strong dependence (Fig.~\ref{fig:negativity_R_E}). The energy-dependence of the multiplicity distributions is again strong (Fig.~\ref{fig:negativity_ang_E}). The negativity tends to follow these same patterns (Figs.~\ref{fig:negativity_R_neg}--\ref{fig:negativity_Q_neg}). Thus, avoiding combinations that have strong energy dependence or MPI and ISR dependence tends also to avoid the problem with negativity. It is possible that strong MPI and ISR dependence leads to strong energy dependence and negativity.
...
\newtextAS{in Figure~\ref{fig:multi_bad_example} is the highest (100\%) it means that no other angularity has a higher separation power $\Delta_{[q,g]}$}.
Similarly, if the other columns are high, it means that the corresponding measures are good, i.e.\
\newtext{have low values}.
Successively, the columns from left to right show \newtextAS{the percentiles of $\Delta_{[q,g]}$ (1st column -- Delta), the percentiles of $\Delta_{[q,~q~\mathrm{no~MPI~no~ISR}]}$ 
...
Moreover, recently ALICE published results~\cite{Lesser:2022lpc} 
...
Knowing the flavour of jets can be used to construct the fraction of jets after the evolution of the parton shower,
can help to trace the origin of the jets through hadronisation and parton showering, as a clue to understanding the ``negativity''
problem, and finally be used to develop a Machine Learning method to optimise the q/g 
...
\newtextPB{In this appendix we show scatter plots of the absolute values of the measures $\Delta_{[q,g]}$, $\Delta_{[q,~q~\mathrm{no~MPI~no~ISR}]}$ 
