...
% Making comments
\newcommand{\CV}[1]{{\color{red}{CV: #1}}}
\newcommand{\CVCV}[1]{{\color{red}{#1}}}
\newcommand{\ST}[1]{\textcolor{blue}{ #1}}
\newcommand{\RT}[1]{{\color{violet}{#1}}}
...
\begin{abstract}
Canonical typicality is the known fact in quantum statistical mechanics that for most wave functions $\psi$ from the unit sphere in a high-dimensional subspace $\Hilbert_R$ (such as a micro-canonical subspace) of the Hilbert space $\Hilbert_S=\Hilbert_a \otimes \Hilbert_b$ of a macroscopic quantum system $S$ consisting of two subsystems $a$ and $b$, the reduced density matrix $\rho_a^\psi := \tr_b |\psi\rangle\langle\psi|$ is close to $\tr_b \rho_R$ and thus nearly deterministic, provided that $a$ is not too large.
Here $\rho_R$ is the projection to $\Hilbert_R$ normalized to trace $1$, and the word ``most'' refers to the uniform distribution over the unit sphere in $\Hilbert_R$, which for a micro-canonical subspace can be regarded as an analog of the micro-canonical ensemble in classical statistical mechanics. In this paper, we generalize canonical typicality to other ensembles, in particular to an analog of the canonical ensemble, so our result expresses a kind of equivalence of ensembles. For a general density matrix $\rho$, the measure over the unit sphere that forms the analog of the uniform measure but has density matrix $\rho$ is known as $\GAP(\rho)$. We show that for any density matrix $\rho$ on $\Hilbert_S$ with small eigenvalues, most wave functions $\psi$ according to $\GAP(\rho)$ are such that $\rho_a^\psi$ is close to $\tr_b \rho$. Our proof is based on a generalization of L\'evy's lemma (concentration of measure)  to $\GAP(\rho)$. We also prove a variant of dynamical typicality for $\rho_a^{\psi_t}$ with $\GAP(\rho)$-typical $\psi_0$ and that the conditional wave function $\psi_a$ has distribution close to $\GAP(\tr_b \rho)$ for typical bases of $\Hilbert_b$ and $\GAP(\rho)$-typical~$\psi$.

\medskip

Key words: Levy's lemma; equivalence of ensembles; thermalization; quantum statistical mechanics; concentration of measure; Gaussian adjusted projected (GAP) measure; Scrooge measure; dynamical typicality; random wave function.
\end{abstract}
...
For any density matrix $\rho$ replacing $\rho_R$ in $\Hilbert_S$, $\GAP(\rho)$ is the most spread-out distribution over $\SSS(\Hilbert_S)$ with density matrix $\rho$; the acronym stands for \emph{Gaussian adjusted projected} measure \cite{JRW94,GLTZ06b}. For $\rho=\rho_\mathrm{can}$, it arises as the distribution of wave functions in thermal equilibrium \cite{GLTZ06b,GLMTZ15}. For general $\rho$, we think of $\GAP(\rho)$ as the natural ensemble of wave functions with density matrix $\rho$; for a more detailed description, see Section~\ref{sec:background}.  
...
As a direct consequence of generalized canonical typicality let us mention that, just as canonical typicality implies that most pure states $\psi\in \SSS(\Hilbert_S)$ have nearly maximal entanglement entropy $-\tr(\rho_a^\psi \log \rho_a^\psi)$ \cite{hayden2006} (because they have $\rho_a^\psi\approx \tr_b I_S/D = I_a/d_a$ with $I$ the identity operator, $d_a=\dim\Hilbert_a$, and $D=d_ad_b=\dim\Hilbert_S$), generalized canonical typicality implies that $\GAP(\rho)$-typical $\psi$ have entanglement entropy $\approx -\tr(\rho_a\log\rho_a)$ with $\rho_a=\tr_b\rho$. 
...
Furthermore, we obtain as a corollary that a form of \emph{dynamical typicality} \cite{BG09} holds for~$\rho_a^\psi$ (Corollary \ref{cor: polybound}): dynamical typicality means that for typical initial wave functions, the time evolution ``looks'' the same; here, ``typical'' refers to the $\GAP(\rho)$ distribution, and ``look'' (which meant the macroscopic appearance in \cite{TTV22-physik}) refers to $\rho_a^\psi$. Hence, the statement here is that for $\GAP(\rho)$-most $\psi$ and most $t\in[0,T]$,
\be
\rho_a^{\psi_t} \approx \tr_b \rho_t\,,
\ee
where $\psi_t=e^{-iHt}\psi$ and $\rho_t=e^{-iHt}\rho e^{iHt}$ ($\hbar=1$). In the original version
of canonical typicality, one particularly considers for $\rho_R$ the micro-canonical density matrix $\rho_\mc$, for which the time evolution yields nothing interesting because it is invariant anyway; but if we consider arbitrary $\rho$, then $\rho$ can evolve in a non-trivial way. 

Another corollary (Corollary \ref{cor:cond}) concerns the conditional wave function $\psi_a$ of $a$ (see Section~\ref{sec:background} for the definition): It is known that if $d_R$ is large, then for $u_R$-most $\psi$ and most bases of $\Hilbert_b$, the Born distribution of $\psi_a$ is approximately $\GAP(\tr_b \rho_R)$. 
...
For a general density matrix $\rho$ on a Hilbert space $\Hilbert$, the probability measure that forms the analog of the uniform distribution over the unit sphere $\mathbb{S}(\Hilbert)$ with density matrix $\rho$ is known as $\GAP(\rho)$. 
Before giving more details about it, we first clarify the notion ``distribution with density matrix $\rho$.'' 
...
\end{align}
Note that if $\mu$ has mean zero then $\rho_\mu$ is the covariance matrix of $\mu$. For $\mu = \GAP(\rho)$ we have that $\rho_{\mu} = \rho$. 

For any $\mu$, random $\psi\sim \mu$, and $\Hilbert=\Hilbert_a \otimes \Hilbert_b$, $\rho_a^\psi$ is \emph{on average} equal to $\tr_b\rho_\mu$,
...
\end{align}
\end{subequations}
The statement of our generalized canonical typicality for $\mu=\GAP(\rho)$ differs in that it concerns \emph{approximate} equality and holds for the individual $\rho_a^\psi$, not only for its average. 


\paragraph{GAP measure.}
The gap measure $\GAP(\rho)$ was first introduced by Jozsa, Robb, and Wootters \cite{JRW94}, who named it \textit{Scrooge measure}\footnote{Named after Ebenezer Scrooge, a fictional character in and the protagonist of Charles Dickens' novella \textit{A Christmas Carol} (1843) who is known for being very stingy. As Jozsa et al. argue, the gap measure is in some sense the most spread-out distribution on $\mathbb{S}(\Hilbert)$ with density matrix $\rho$ and they choose the name ``Scrooge measure'' because the measure is ``particularly stingy with its information.''}; it can be defined in several ways, see \cite{GLMTZ15} for four different definitions. One is that $\GAP(\rho)$ is the distribution of $\phi/\|\phi\|$ if the distribution of the random vector $\phi$ in $\Hilbert$ has density $\|\phi\|^2$ relative to the Gaussian distribution with mean 0 and covariance $\rho$. $\GAP(\rho)$ can also be characterized as the minimizer of the ``accessible information'' functional under the constraint that its density matrix is $\rho$ \cite{JRW94}. If all eigenvalues of $\rho$ are positive and $D:=\dim\Hilbert<\infty$, then $\GAP(\rho)$ possesses a density relative to the uniform distribution $u$ on $\mathbb{S}(\Hilbert)$ \cite{GLTZ06b,GLMTZ15},
\begin{align}\label{GAPdensity}
	\frac{d\GAP(\rho)}{du}(\psi) = \frac{D}{\det\rho}\langle\psi|\rho^{-1}|\psi\rangle^{-D-1}\,.
\end{align}

It was argued in \cite{GLTZ06b} and mathematically justified in \cite{GLMTZ15} that GAP measures describe the thermal equilibrium distribution of the wave function of the system if $\rho$ is a canonical density matrix. 


\paragraph{Norms.}
The distance between the two density matrices will be measured in the \textit{trace norm}
\begin{align}
...
In this section, we present and discuss our main results about generalized canonical typicality. 

\subsection{Statements}

We now formulate theorems, first with polynomial bounds (which can be obtained with easier proofs) and then with tighter exponential bounds. The known bounds for uniformly distributed $\psi$ will be stated in Remark~\ref{rmk:comparison} in Section~\ref{sec:discussion} for comparison. We also state a variant of L\'evy's lemma for GAP measures, two corollaries about dynamical canonical typicality, and one about conditional wave functions. The proofs will be given in Section~\ref{sec: proofs}.


\begin{thm}[Generalized canonical typicality, polynomial bounds]\label{thm:1}
Let $\Hilbert_a$ and $\Hilbert_b$ be Hilbert spaces with $\Hilbert_a$ having finite dimension $d_a$ and $\Hilbert_b$ being separable (i.e., having either finite or countably infinite ONB). 
...
\begin{rmk}
Equivalently, we can express Theorem~\ref{thm:1} as a bound on the confidence level $1-\delta$ for any given allowed deviation of $\rho_a^\psi$ from $\tr_b\rho$: {\it For every $\rho$ with $\|\rho\|<1/4$ and every $\varepsilon>0$,}
...
\begin{thm}[Generalized canonical typicality, exponential bounds]\label{thm: GCT exp bound}
    ...
    \begin{align}\label{expbounddelta}
        \GAP(\rho)\Biggl\{\psi\in\mathbb{S}(\Hilbert): \bigl\|\rho_a^\psi-\tr_b\rho\bigr\|_{\tr}\leq c d_a \sqrt{\ln\left(\frac{6d_a^2}{\delta}\right)\|\rho\|}\Biggr\} \geq 1-\delta,
    \end{align}
    where $c=24\sqrt{2}\pi$.
\end{thm}



\begin{rmk}\label{rmk: exp bound}
Again, the relation can equivalently be formulated as a bound on the confidence level, given the allowed deviation: {\it For every $\varepsilon\geq 0$,}
\begin{align}\label{expboundepsilon}
    \GAP(\rho)\Bigl\{\psi\in\mathbb{S}(\Hilbert): \bigl\|\rho_a^\psi-\tr_b\rho\bigr\|_{\tr} > \varepsilon\Bigr\} \leq 6d_a^2 \exp\left(-\frac{\tilde{C}\varepsilon^2}{d_a^2 \|\rho\|}\right),
\end{align}
{\it where} $\tilde{C}=\frac{1}{1152\pi^2}$. This form makes it visible why we call Theorem~\ref{thm: GCT exp bound} an ``exponential bound'': because the bound on the probability of too large a deviation is exponentially small in $1/\|\rho\|$. In contrast, the bound \eqref{polyboundepsilon} is polynomially small in $\tr\rho^2$.\hfill$\diamond$
\end{rmk}


A key tool for proving Theorem~\ref{thm: GCT exp bound} is Theorem~\ref{thm: LM for GAP} below, a variant of L\'evy's lemma for GAP measures.
In the following, we use the notation $\mu(f)$ for the average of the function $f$ under the measure $\mu$,
\be\label{mufdef}
\mu(f) := \int \mu(d\psi) \, f(\psi) \,.
\ee


\begin{thm}[L\'evy's lemma for GAP]\label{thm: LM for GAP}
  Let $\Hilbert$ be a separable Hilbert space, let $f:\mathbb{S}(\Hilbert)\to\mathbb{R}$ be a function with Lipschitz constant\footnote{A Lipschitz constant refers to a metric on the domain, and two metrics are often considered on the sphere: the spherical metric (distance along the sphere, $d_\mathrm{sph}(\psi,\phi)=\arccos \bigl|\scp{\psi}{\phi}\bigr|$) and the Euclidean metric (distance in the ambient space across the interior of the sphere, $d_\mathrm{Eucl}(\psi,\phi)=\|\psi-\phi\|$). We use the spherical metric, as did \cite{MS86,PSW05,PSW06}, but since $d_\mathrm{Eucl}(\psi,\phi)\leq d_\mathrm{sph}(\psi,\phi)\leq \tfrac{\pi}{2} \, d_\mathrm{Eucl}(\psi,\phi)$, using the Euclidean metric would at most change the Lipschitz constants by a factor of $\tfrac{\pi}{2}$.} $\eta$, let $\rho$ be a density matrix on $\Hilbert$, and let $\varepsilon\geq 0$. 
  Then
  \be\label{LMGAP}
...
In contrast to the uniform distribution on the sphere, which is invariant under the unitary time evolution, $\GAP(\rho_0)$ will in general evolve, in fact to $\GAP(\rho_t)$ \cite{GLTZ06b} with $\rho_t=e^{-iHt}\rho_0 e^{iHt}$. This leads to the question whether, for $\GAP(\rho_0)$-typical $\psi_0$, the reduced density matrix $\rho_a^{\psi_t}$ remains close to $\tr_B \rho_t$ over a whole time interval $[0,T]$. The following corollary answers this question affirmatively for most times in this interval.


\begin{cor}\label{cor: polybound}
Let $\Hilbert_a$ and $\Hilbert_b$ be Hilbert spaces with $\Hilbert_a$ having finite dimension $d_a$ and $\Hilbert_b$ being separable, and let $\rho$ be a density matrix on $\Hilbert = \Hilbert_a \otimes \Hilbert_b$ with $\|\rho\|<1/4$. Then for every $\varepsilon, T>0$,
   \begin{align}
        \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\frac{1}{T}\int_0^T \bigl\|\rho_a^{\psi_t} - \tr_b \rho_t\bigr\|_{\tr}^2\, dt > \varepsilon\right\} \leq \frac{28 d_a^3 \tr\rho^2}{\varepsilon} .\label{ineq: GAP time}
    \end{align}
\end{cor}


\begin{rmk}\label{rmk: expbound}{\it Alternative bound for \eqref{ineq: GAP time}.}
The proof of Corollary~\ref{cor: polybound} makes use of Proposition~\ref{prop: var} which is also used to prove Theorem~\ref{thm:1}. Another way to obtain bounds for the left-hand side of \eqref{ineq: GAP time} (and for the same expression with $\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}$ instead of its square) is to exploit Theorem~\ref{thm: GCT exp bound} and the fact that $\rho\mapsto \GAP(\rho)$ is covariant. This leads to the following result: 
Let $\Hilbert_a$ and $\Hilbert_b$ be Hilbert spaces with $\Hilbert_a$ having finite dimension $d_a$ and $\Hilbert_b$ being separable, and let $\rho$ be a density matrix on $\Hilbert = \Hilbert_a \otimes\Hilbert_b$. Then for every $\alpha,\varepsilon,T>0$,
\begin{multline}
    \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert): \frac{1}{T}\int_0^T \|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr} \, dt > \varepsilon \right\} \\
    \leq \frac{1}{\varepsilon} \left(12d_a^2 \exp\left(-\frac{\tilde{C}}{(d_a^2 \|\rho\|)^{\alpha}}\right) + (d_a^2 \|\rho\|)^{(1-\alpha)/2}\right),\label{cor expbound ineq1}
\end{multline}
where $\tilde{C}=\frac{1}{1152\pi^2}$. Moreover,
\begin{multline}
    \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert): \frac{1}{T}\int_0^T \|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}^2 \, dt > \varepsilon \right\} \\
    \leq \frac{1}{\varepsilon} \left(24d_a^2 \exp\left(-\frac{\tilde{C}}{(d_a^2 \|\rho\|)^{\alpha}}\right) + (d_a^2 \|\rho\|)^{1-\alpha}\right)\label{cor expbound ineq2}.
\end{multline}
The proof of the two bounds can be found in Section~\ref{proof: cor1, rmk2}.\hfill$\diamond$
\end{rmk}
...
\begin{cor}\label{cor:cond}
    ...
    Then there is $p>0$ such that for every density matrix $\rho$ on $\Hilbert=\Hilbert_a \otimes \Hilbert_b$ with $\tr\rho^2 <p$,
...
4\exp\left(-\frac{\tilde{C} D\varepsilon^2}{\eta^2}\right)\,,
\ee
where $\tilde C=\frac{2}{9\pi^3}$.
\end{thm}

When we apply our Theorem~\ref{thm: LM for GAP} to $\rho=I/D$, we obtain that $\GAP(\rho)=u$, $\|\rho\|=1/D$, and exactly the bound \eqref{Levy} except for worse constants. Note that Theorem~\ref{thm: LM for GAP} holds also in infinite dimension.\hfill$\diamond$
\end{rmk}
...
Here is a counter-example. Let $\rho=\sum_{n=1}^D p_n |n\rangle \langle n|$ have eigenvalues $p_n$ and eigen-ONB $\{|n\rangle\}_n = \{|n\rangle:1\leq n \leq D\}$, and let
\be
\mu=\sum_{n=1}^D p_n \, \delta_{|n\rangle}
\ee
be the measure that is concentrated on the finite set $\{|n\rangle\}_n$ and gives weight $p_n$ to each $|n\rangle$. This measure is the narrowest, most concentrated measure with density matrix $\rho$, and thus a kind of opposite of $\GAP(\rho)$, the most spread-out measure with density matrix $\rho$. A random vector $\psi\sim \mu$ is a random eigenvector $|n\rangle$. What the reduced density matrix $\rho_a^{|n\rangle}$ looks like depends on the vectors $|n\rangle\in\Hilbert=\Hilbert_a\otimes \Hilbert_b$. Suppose that the eigenbasis of $\rho$ is the product of ONBs of $\Hilbert_a$ and $\Hilbert_b$, $|n\rangle=|\ell\rangle_a \otimes |m\rangle_b$; then $\rho_a^{|n\rangle}=\tr_b |n\rangle \langle n| = |\ell\rangle_a\langle \ell|$ (in an obvious notation), so $\rho_a^{|n\rangle}$ is always a pure state and thus far away from $\tr_b \rho= \sum_{\ell,m}p_{\ell m} |\ell\rangle_a\langle\ell|$ if that is highly mixed. Note, however, that if instead of a product basis, we had taken $\{|n\rangle\}_n$ to be a purely random ONB of $\Hilbert$, then (with overwhelming probability if $d_b\gg 1$) $\rho_a^{|n\rangle}\approx d_a^{-1} I_a$ and thus also $\tr_b \rho$ (which by \eqref{mu-avg-rhopsi} is the $\mu$-average of $\rho_a^\psi$) is close to $d_a^{-1} I_a$, so $\rho_a^{\psi}\approx \tr_b \rho$ for $\mu$-most $\psi$, despite the narrowness of $\mu$.\hfill$\diamond$
\end{rmk}

\begin{rmk}\label{rmk:BEC}{\it Canonical typicality with respect to $\GAP(\rho)$ does not hold for every $\rho$.}
    Let us consider the special case in which $\rho$ has one eigenvalue that is large (e.g., $10^{-1}$), while all others are very small (e.g., $10^{-1000}$). Such a situation occurs for example for $N$-body quantum systems with a gapped ground state $|0\rangle$ at very low temperature, $T\sim (\log N)^{-1}$. 
...
the distribution of $\|\rho_a^\psi-\tr_b \rho\|_{\tr}$ in $\RRR$ for $\psi\sim \GAP(\rho)$. However, our situation does not quite fit the standard framework of large deviations because we do not necessarily consider a sequence $\rho_N$ of density matrices, but rather a fixed $\rho$ with small $\|\rho\|$. That is why we have provided error bounds in terms of the given $\rho$.\hfill$\diamond$
\end{rmk}
...
Let $\rho$ be a density matrix on a separable Hilbert space $\Hilbert$ with positive eigenvalues $p_n$ such that  $p_{\max}=\|\rho\|<1/4$ and let $\dim\Hilbert \geq 4$. For any bounded operator $A:\Hilbert\to\Hilbert$, 
\begin{align}
...
Let $\{|n\rangle: n=1,\dots,D\}$ be an orthonormal basis of $\Hilbert$ consisting of eigenvectors of $\rho$. For $\psi\in\mathbb{S}(\Hilbert)$ we write
\begin{align}
...
Now suppose that $\Hilbert$ has a countably infinite ONB. The expectation can be computed as before if we show that $\mathbb{E}|\psi\rangle\langle\psi|=\rho$ remains true in the infinite-dimensional setting. This can be seen as follows: Let $\phi\in\Hilbert$. Then
\begin{align}
    \langle\phi|\left(\mathbb{E}|\psi\rangle\langle\psi|\right)|\phi\rangle &= \int_{\mathbb{S}(\Hilbert)} \langle\phi|\psi\rangle\langle\psi|\phi\rangle \, \GAP(\rho)(d\psi) = \langle\phi|\rho|\phi\rangle, 
\end{align}
where we used the definition of $\GAP(\rho)$ in infinite dimensions in the last step. Since $\rho$ is uniquely determined by the quadratic form $\phi\mapsto \langle\phi|\rho|\phi\rangle$, it follows that $\mathbb{E}|\psi\rangle\langle\psi|=\rho$. Moreover, note that $\mathbb{E}$ and $\tr$ can be interchanged by Fubini's theorem.
...
\end{align}
\end{subequations}
Let $\{|l\rangle_a: l=1,\dots,d_a\}$ and $\{|n\rangle_b: n=1,\dots,d_b\}$, where $d_a := \dim\Hilbert_a \in \mathbb{N}$ and $d_b:=\dim\Hilbert_b \in \mathbb{N}\cup \{\infty\}$, be an orthonormal basis of $\Hilbert_a$ and $\Hilbert_b$ respectively. For
\begin{align}
...
The proof of Theorem~\ref{thm: GCT exp bound} follows largely the one of canonical typicality given in \cite{PSW05}; the main difference concerns, apart from our generalization of the L\'evy lemma, the steps needed for covering infinite dimension.

Let $U_a$ be a unitary operator on $\Hilbert_a$. Then the function $f:\mathbb{S}(\Hilbert)\to \mathbb{R}$, $f(\psi) = \tr_a(U_a\rho_a^\psi)$ is Lipschitz continuous with Lipschitz constant $\eta \leq 2\|U_a\| = 2$ (see, e.g., Lemma 5 in \cite{PSW05}), and Theorem~\ref{thm: LM for GAP} implies that 
\begin{align}
\GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\bigl|\tr_a(U_a\rho_a^\psi)-\GAP(\rho)(\tr_a(U_a \rho_a^\psi)) \bigr| >\varepsilon\right\} \leq 6\exp\left(-\frac{C\varepsilon^2}{4\|\rho\|}\right).
\end{align}
Next note that
\begin{align}
    \GAP(\rho)(\tr_a(U_a\rho_a^\psi)) = \tr_a\left(U_a \GAP(\rho)(\rho_a^\psi)\right) = \tr_a\left(U_a \tr_b\rho\right).
\end{align}
If $\Hilbert_b$ is finite-dimensional, the last step follows immediately from 
\begin{subequations}
\begin{align}
    \GAP(\rho)(\rho_a^\psi) 
    &= \int_{\mathbb{S}(\Hilbert)} \tr_b |\psi\rangle\langle\psi|\, \GAP(\rho)(d\psi) \\
    &= \tr_b \int_{\mathbb{S}(\Hilbert)} |\psi\rangle\langle\psi| \, \GAP(\rho)(d\psi) \\
    &= \tr_b \rho.\label{eq: Gap(rho)(rho_a^psi)=tr_b rho}
\end{align}
\end{subequations}
In the case that $\Hilbert_b$ is infinite-dimensional with a countable ONB, the relation $\GAP(\rho)(\rho_a^\psi)=\tr_b\rho$ remains true but one has to argue slightly differently, here is how: Let $\{|l\rangle_b: l\in\mathbb{N}\}$ be an orthonormal basis of $\Hilbert_b$ and let $|\phi\rangle_a\in\Hilbert_a$. Then
\begin{subequations}
\begin{align}
    {}_a\langle\phi|\GAP(\rho)(\rho_a^\psi)|\phi\rangle_a &= \int_{\mathbb{S}(\Hilbert)} {}_a\langle\phi|\tr_b\left(|\psi\rangle\langle\psi|\right)|\phi\rangle_a\, \GAP(\rho)(d\psi)\\
    &=\int_{\mathbb{S}(\Hilbert)} \sum_l |\langle\phi,l|\psi\rangle|^2\, \GAP(\rho)(d\psi)\\
    &= \sum_l \int_{\mathbb{S}(\Hilbert)} \langle\phi,l|\psi\rangle\langle\psi|l,\phi\rangle \, \GAP(\rho)(d\psi)\\
    &=\sum_l \langle\phi,l|\rho|l,\phi\rangle\\
    &= {}_a\langle\phi|\tr_b\rho|\phi\rangle_a,
\end{align}
\end{subequations}
where we used Fubini's theorem in the third and the definition of $\GAP(\rho)$ in infinite dimensions in the fourth line.
Since $\tr_b\rho$ is uniquely determined by the quadratic form $\phi\mapsto {}_a\langle\phi|\tr_b\rho|\phi\rangle_a$, it follows that $\GAP(\rho)(\rho_a^\psi)=\tr_b\rho$.

Therefore, we have that
\begin{align}
   \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\bigl|\tr_a(U_a\rho_a^\psi)-\tr_a(U_a\tr_b\rho) \bigr| >\varepsilon\right\} \leq 6 \exp\left(-\frac{C\varepsilon^2}{4\|\rho\|}\right). 
\end{align}
 Let $(U_a^j)_{j=0}^{d_a^2-1}$ be an orthogonal operator basis for $\Hilbert_a$ such that\footnote{One possible choice is given by
  \begin{align}
      U_a^j = \sum_{k=0}^{d_a-1} e^{2\pi i k (j-(j\mathrm{\, mod \,} d_a))/d_a^2} |(k+j)\mbox{ mod } d_a\rangle \langle k|,
  \end{align}
  where $\{|k\rangle: k=0,\dots,d_a-1\}$ is an orthonormal basis of $\Hilbert_a$, see \cite{PSW05}.}
  \begin{align}
      \tr_a(U_a^{j*} U_a^k) = d_a \delta_{jk}.
  \end{align}
    Then
  \begin{align}
     \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\exists j: \bigl|\tr_a(U_a^j\rho_a^\psi)-\tr_a(U_a^j\tr_b\rho) \bigr| >\varepsilon\right\} \leq 6d_a^2 \exp\left(-\frac{C\varepsilon^2}{4\|\rho\|}\right). \label{ineq: GAP all j}  
  \end{align}
  As in \cite{PSW05}, the density matrix $\rho_a^\psi$ can be expanded as
  \begin{align}
      \rho_a^\psi = \frac{1}{d_a} \sum_j C_j(\rho_a^\psi) U_a^j,
  \end{align}
  where $C_j(\rho_a^\psi) = \tr_a(U_a^{j*}\rho_a^\psi)$ and \eqref{ineq: GAP all j} becomes
  \begin{align}
    \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\exists j: \bigl| C_j(\rho_a^\psi)-C_j(\tr_b\rho) \bigr| >\varepsilon\right\} \leq 6d_a^2 \exp\left(-\frac{C\varepsilon^2}{4\|\rho\|}\right).  
  \end{align}
  If $|C_j(\rho_a^\psi)-C_j(\tr_b\rho)|\leq \varepsilon$ for all $j$, then
\begin{subequations}
\begin{align}
      \|\rho_a^\psi-\tr_b\rho\|_{\tr}^2 &\leq d_a\|\rho_a^\psi-\tr_b\rho\|^2_2\\
      &=d_a\left\| \frac{1}{d_a}\sum_j\left(C_j(\rho_a^\psi)-C_j(\tr_b\rho)\right)U_a^j\right\|_{2}^2\\
      &= \frac{1}{d_a} \tr_a\left|\sum_j \left(C_j(\rho_a^\psi)-C_j(\tr_b\rho)\right)U_a^j\right|^2\\
      &= \sum_j \left|C_j(\rho_a^\psi)-C_j(\tr_b\rho)\right|^2\\
      &\leq d_a^2\varepsilon^2.
\end{align}
\end{subequations}
   This implies that
  \begin{align}
      \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\|\rho_a^\psi-\tr_b\rho\|_{\tr} > d_a\varepsilon\right\} \leq 6d_a^2 \exp\left(-\frac{C\varepsilon^2}{4\|\rho\|}\right)
  \end{align}
  and, after replacing $\varepsilon$ by $\varepsilon d_a^{-1}$,
  \begin{align}
      \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\|\rho_a^\psi-\tr_b\rho\|_{\tr} > \varepsilon\right\} \leq 6d_a^2 \exp\left(-\frac{C\varepsilon^2}{4d_a^2\|\rho\|}\right).
  \end{align}
  Setting  
  \begin{align}
      \delta = 6d_a^2 \exp\left(-\frac{C\varepsilon^2}{4d_a^2\|\rho\|}\right)
  \end{align}
...
We start with quoting the following cousin of L\'evy's lemma: 
...
Now let $\rho$ be a density matrix on $\Hilbert$ (identified with $\mathbb{C}^D$) with eigenvalues $p_1,\dots,p_D$ and let $Z=(Z_1,\dots,Z_D)$ be a vector of independent complex mean zero Gaussian random variables with variances
\begin{align}
    \mathbb{E}|Z_n|^2 = p_n
\end{align}
in a basis in which $\rho$ is diagonal. Then we can write $Z=\sqrt{\rho/2} \tilde{Z}$, where $\tilde{Z}$ is a vector of $D$ complex mean zero Gaussian random variables with variances $\mathbb{E}|\tilde{Z}_n|^2=2$ which can be in a natural way identified with a vector of $2D$ independent real standard Gaussian variables.

If $F:\Hilbert\to\mathbb{R}$ is Lipschitz with constant $\eta$, then $F\circ \sqrt{\rho/2}: \mathbb{C}^D \to \mathbb{R}$ is also Lipschitz with constant $\eta \sqrt{\|\rho\|/2}$. This function can also naturally be considered as a function on $\mathbb{R}^{2D}$ and then an application of Lemma~\ref{lemma:LevyGauss} immediately proves the following theorem: 


\begin{thm}
    Let $\rho$ be a density matrix on $\Hilbert$ with eigenvalues $p_1,\dots,p_D$ and 
    let $F:\Hilbert\to\mathbb{R}$ be a Lipschitz function with constant $\eta$. Let $Z=(Z_1,\dots,Z_D)$ be a vector of independent complex mean zero Gaussian random variables with variances
    \begin{align}
        \mathbb{E}|Z_n|^2 = p_n.
    \end{align}
    Then for every $\varepsilon>0$,
    \begin{align}
        \mathbb{P}\left\{|F(Z)-\mathbb{E}F(Z)|>\varepsilon\right\} \leq 2\exp\left(-\frac{4\varepsilon^2}{\pi^2\eta^2 \|\rho\|}\right).
    \end{align}
\end{thm}


Next we show that a similar result also holds for the measure $\GA(\rho)$. Its proof closely follows the proof of L\'evy's Lemma in \cite{MS86}; for convenience of the reader we provide all the details.


\begin{thm}\label{thm: bound GA}
    Let $F:\Hilbert\to\mathbb{R}$ be a Lipschitz function with Lipschitz constant $\eta$. Let $Z=(Z_1,\dots,Z_D)$ be a vector of independent, mean zero complex Gaussian variables with variances
    \begin{align}
        \mathbb{E}|Z_n|^2 = p_n.
    \end{align}
    Then for every $\varepsilon>0$,
    \begin{align}
        \GA(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\bigl|F(\psi)-\GA(\rho)(F) \bigr|>\varepsilon\right\} \leq 4\exp\left(-\frac{2\varepsilon^2}{\pi^2\eta^2\|\rho\|}\right). \label{ineq: thm GA bound}
    \end{align}
\end{thm}    


\begin{proof}
    Let $\varphi: \mathbb{R}\to\mathbb{R}$ be a convex function and let $\tilde{Z}=(\tilde{Z_1},\dots,\tilde{Z}_D)$ be a vector with the same distribution as $Z$ but independent of it. With the help of Jensen's inequality and H\"older's inequality we find that
    \begin{subequations}
    \begin{align}
...
\end{subequations}
   ...
   \end{align}
   Next note that $2(\RE \tilde{Z}_n)^2$ and $2(\IM \tilde{Z}_n)^2$ are chi-squared distributed random variables with one degree of freedom and that the moment generating function of a random variable $Y\sim \chi^2_1$ is given by
   \begin{align}
...
\end{align}
is open in $\SSS(\Hilbert)$, it follows from the weak convergence of the measures $\GAP(\rho_n)$ to $\GAP(\rho)$ by Portmanteau's theorem \cite[Thm.~2.1]{Bill99} that 
\begin{align}
...
\subsection{Proofs of Corollaries~\ref{cor: polybound}, \ref{cor:cond} and Remark~\ref{rmk: expbound} \label{proof: cor1, rmk2}}

\begin{proof}[Proof of Corollary~\ref{cor: polybound}]
    Without loss of generality we assume that all eigenvalues of $\rho$ are positive. Let $\{|l\rangle_a: l=1,\dots,d_a\}$ be an orthonormal basis of $\Hilbert_a$ and let $t\in[0,T]$. For $A^{lm}$ as in the proof of Theorem~\ref{thm:1} we find $\|A^{lm}\|=1$ and, as it was shown in the proof of Theorem~\ref{thm:1}, 
    \begin{align}
        \langle\psi_t|A^{lm}|\psi_t\rangle &= {}_a\langle m|\rho_a^{\psi_t}|l\rangle_a,\quad \tr(A^{lm}\rho_t) = {}_a\langle m|\tr_b\rho_t |l\rangle_a.
    \end{align}
    Using again the fact that $\|M\|_{\tr} \leq \sqrt{d_a} \|M\|_2$ for any $d_a\times d_a$ matrix $M$, we obtain that
\begin{subequations}
    \begin{align}
         \mathbb{E}\left(\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}^2\right) &\leq d_a \sum_{l,m=1}^{d_a} \mathbb{E}\left(\bigl|{}_a\langle m|\rho_a^{\psi_t}-\tr_b\rho_t|l\rangle_a \bigr|^2\right)\\
    &= d_a\sum_{l,m=1}^{d_a} \mathbb{E}\left(|\langle\psi_t|A^{lm}\bigl|\psi_t\rangle - \tr(A^{lm}\rho_t)\bigr|^2\right)\\
    &= d_a\sum_{l,m=1}^{d_a} \mathbb{E}\left(\bigl|\langle\psi|A^{lm}_t|\psi\rangle - \tr(A^{lm}_t \rho) \bigr|^2\right)\\
    &= d_a \sum_{l,m=1}^{d_a} \Var\left(\langle\psi|A^{lm}_t|\psi\rangle\right)\\
    &\leq \frac{d_a^3 \tr\rho^2}{1-p_{\max}} \left(1+\frac{4\sqrt{\tr \rho^2} + 2\tr \rho^2}{(1-2p_{\max})(1-3p_{\max})}\right)\\
    &\leq 28 d_a^3 \tr\rho^2,
    \end{align}
\end{subequations}
    where we used that $\|A_t^{lm}\|\leq 1$. With the help of Fubini's theorem we find that
\begin{subequations}
\begin{align}
    \mathbb{E}\left(\frac{1}{T}\int_0^T \|\rho_a^{\psi_t}-\tr_b \rho_t\|_{\tr}^2\, dt\right) &= \frac{1}{T}\int_0^T \mathbb{E}\left(\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}^2\right) \, dt\\
    &\leq 28 d_a^3 \tr\rho^2.
\end{align}
\end{subequations}
...
\begin{proof}[Proof of Remark~\ref{rmk: expbound}]
        As $\rho\mapsto \GAP(\rho)$ is covariant, it follows from Theorem~\ref{thm: GCT exp bound} (or, more precisely, Remark~\ref{rmk: exp bound}) that for all $t\geq 0$ and all $\varepsilon'>0$,
 \begin{subequations}
   \begin{align}
        &\GAP(\rho_t)\left\{\psi_t \in \mathbb{S}(\Hilbert): \|\rho_a^{\psi_t}-\tr_b \rho_t\|_{\tr}>\varepsilon'\right\} \nonumber\\[4mm]
        &\qquad = \GAP(\rho)\left\{\psi \in \mathbb{S}(\Hilbert): \|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}>\varepsilon'\right\}\\
        &\qquad\leq 6 d_a^2 \exp\left(-\frac{\tilde{C}\varepsilon'^2}{d_a^2\|\rho\|}\right). \label{ineq: measure at most}
    \end{align}
\end{subequations}
    Noting that $\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}\leq 2$, we obtain for all $t\geq 0$ that
 \begin{align}
     \GAP(\rho)\left(\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}\right) \leq 12 d_a^2 \exp\left(-\frac{\tilde{C}\varepsilon'^2}{d_a^2\|\rho\|}\right) + \varepsilon',
 \end{align}
 where we used that $\|\rho_t\| = \|\rho\|$ for all $t\geq 0$ and that $\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}$ can be bounded by 2 on a set of measure at most \eqref{ineq: measure at most} and by $\varepsilon'$ on the complement of this set (whose measure is bounded by 1). 
 
Applying Fubini's theorem shows that
\begin{subequations}
\begin{align}
    \GAP(\rho)\left(\frac{1}{T}\int_0^T \|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}\, dt\right) &= \frac{1}{T} \int_0^T \GAP(\rho)\left(\|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}\right)\, dt\\
    &\leq  12 d_a^2 \exp\left(-\frac{\tilde{C}\varepsilon'^2}{d_a^2\|\rho\|}\right) + \varepsilon'.
\end{align}
\end{subequations}
Finally, it follows from Markov's inequality that
\begin{align}
    \GAP(\rho)\left\{\psi\in\mathbb{S}(\Hilbert):\frac{1}{T}\int_0^T \|\rho_a^{\psi_t}-\tr_b\rho_t\|_{\tr}\, dt > \varepsilon\right\} \leq \frac{1}{\varepsilon} \left( 12 d_a^2 \exp\left(-\frac{\tilde{C}\varepsilon'^2}{d_a^2\|\rho\|}\right) + \varepsilon'\right)
\end{align}
and choosing $\varepsilon' = (d_a^2 \|\rho\|)^{(1-\alpha)/2}$ finishes the proof of \eqref{cor expbound ineq1}. The second inequality can be shown using similar arguments.
\end{proof}
...
\begin{proof}[Proof of Corollary~\ref{cor:cond}.]
    ...
    \ee
    whenever $\|\rho_a^\psi-\tr_b\rho\|_{\tr} < r$. By Theorem~\ref{thm:1} above in the form \eqref{polyboundepsilon}, the latter condition is fulfilled with probability $\geq 1-28d_a^5 \tr \rho^2/r^2 \geq 1-\delta/2$ for $\tr\rho^2\leq p:=r^2\delta/56d_a^5$. Now \eqref{typcond} follows. 
\end{proof}
...
As discussed after Theorem~\ref{thm:4}, applying Theorem~\ref{thm:1} to $\rho=\rho_R$ yields the worse factor $d_a^{2.5}$ instead of $d_a^2$. Here we want to give some details why in this special case of Theorem~\ref{thm:1} better slightly bounds can be obtained.

First suppose that $\Hilbert_R = \Hilbert$. Similarly to the proof of Theorem~\ref{thm:1} one finds that
\begin{align}
    u\left\{\psi\in\mathbb{S}(\Hilbert): \|\rho_a^\psi-\tr_b\rho\|_{\tr}>\varepsilon\right\} \leq \frac{d_a^3}{\varepsilon^2} \sum_{l,m}\Var\langle\psi|A^{lm}|\psi\rangle,
\end{align}
where $A^{lm} = |l\rangle_a\langle m|\otimes I_b$ and $\{|l\rangle_a : l=1,\dots,d_a\}$ is an orthonormal basis of $\Hilbert_a$. Instead of bounding the sum by $d_a^2$ times a uniform bound on the variances $\Var\langle\psi|A^{lm}|\psi\rangle$, one can now make use of the fact that for uniformly distributed $\psi\in\mathbb{S}(\Hilbert)$, the second and fourth moments of the coefficients $c_l$ of $\psi$ in an orthonormal basis $\{|n\rangle: n=1,\dots,D\}$ of eigenvectors of $\rho$ can be computed explicitly. More precisely, they satisfy
\begin{align}
...
Specifically, we are concerned with a bi-partite quantum system $a\cup b$, where $b$ is macroscopically large. Canonical typicality is the fact that for most unit vectors $\psi$ from a high-dimensional subspace $\Hilbert_R$ of $\Hilbert=\Hilbert_a \otimes \Hilbert_b$, the reduced density matrix $\rho_a^\psi = \tr_b |\psi\rangle\langle\psi|$ is close to its average, which is $\tr_b P_R/d_R$. We have shown that for most $\psi$ from the $\GAP(\rho)$ ensemble, $\rho_a^\psi$ is close to its average, which is $\tr_b \rho$, assuming that the largest eigenvalue or at least the average eigenvalue of $\rho$ is small. That is, we have established an extension of canonical typicality to GAP measures. This family of measures is particularly natural in this context because it arises anyway in the context of bi-partite systems as the typical asymptotic distribution of the conditional wave function \cite{GLTZ06b,GLMTZ15}.

These results contribute to the picture of how an individual, closed quantum system in a pure state can display thermodynamic behavior, and thus help clarify the role of ensembles as defining a concept of typicality, while thermal density matrices arise from partial traces. We have further extended our generalization of canonical typicality in several ways: First, concerning the time evolution, if the initial wave function $\psi_0$ is $\GAP(\rho)$-typical (and $\rho$ has small eigenvalues), then the whole curve $t\mapsto \rho_a^{\psi_t}$ is nearly deterministic (and given by $\tr_b \rho_t$) for not-too-large $t$. Second, the conditional wave function $\psi_a$ has Born distribution close to $\GAP(\tr_b \rho)$ for $\GAP(\rho)$-most $\psi$ and most ONBs in $\Hilbert_b$.

We have provided explicit error bounds by means of two strategies: one based ultimately on Chebyshev's inequality (and certain estimates by Reimann \cite{Reimann08}), the other on a variant of L\'evy's lemma for GAP measures that we prove. For $\rho=P_R/d_R$ with large $d_R$, our error bounds reduce to the known ones for canonical typicality.

In sum, our results describe a simple way in which the following concepts fit together: reduced density matrix, many participating dimensions, and GAP measures. That is, if many dimensions participate in $\rho$, then for $\GAP(\rho)$-most $\psi$, the reduced density matrix $\rho_a^\psi$ is nearly independent of $\psi$. 
...
We thank Tristan Benoist and Marius Lemm for helpful discussion. C.V.\ acknowledges financial support by the German Academic Scholarship Foundation. S.T.\ acknowledges financial support by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) -- TRR 352 -- Project-ID 470903074.  
