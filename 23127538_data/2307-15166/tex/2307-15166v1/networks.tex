...
%The dimensions of varieties corresponding to tree mixtures are While containing tree varieties and being contained in tree mixture varieties, The varieties of group-based network-based Markov models contain tree varieties as subvarieties, and in turn are subvarieties of varieties corresponding to tree mixture models \cite{}.  While the dimensions of mixture models for group-based tree models are given in \cite{}, the dimensions of group-based networks have been less understood.
 ...
 As described in Section \ref{sec:prelim}, a group-based model is defined with an abelian group $G$ of order $n$ and $B$ a subgroup of the automorphism group of $G$, denoted $\Aut(G)$. In a network-based Markov model, each edge of the network has a transition matrix associated to it, and the parameters of the model are the entries of these transition matrices along with a mixing parameter for each cycle. In a group-based model, the dimension of the parameter space is cut significantly by placing constraints on the transition matrices.   In particular, in a group-based model, if $l+1$ is the number of $B$-orbits in $G,$ the number of free parameters for each edge is $l$. For a phylogenetic network $\C N$ with $m$ edges and $c$ cycles, the {\em expected dimension} of the group-based  network variety $\dim V_{\C N}^M$ is $l(m-c)+1$, and Proposition \ref{prop:upper} shows that it is indeed an upper bound.  The main theorem of this paper shows that most level-one group-based network varieties have the expected dimension.
 

 
 \begin{theorem} \label{thm:main} 
 Let $\C N$ be a level-one triangle-free phylogenetic network with $n$ leaves, $m$ edges, and $c$ cycles. Let $G$ be a finite abelian group of order at least $3$ and $B$ a subgroup of $\Aut(G)$.  Let $l+1$ be the number of $B$-orbits in $G$. Then the group-based network variety $V^{(G,B)}_{\C N}$ has dimension $l(m-c)+1$.
 \end{theorem}
 
 When $G=\mathbb{Z}/2\mathbb{Z}$, the $3$ and $4$-sunlet networks do not have the expected dimension. In this case, since $\Aut(G)$ is the trivial group, there is only a single group-based model. This is the Cavender-Farris-Neyman (CFN) model, and has biological relevance, so we give the result for this group separately. Note that here we are able to give a full result for level-one phylogenetic networks.
 
 \begin{theorem}\label{thm:zmod2z}
 Let $G=\mathbb{Z}/2\mathbb{Z}$ and let $\C N$ be a level-one phylogenetic network with $n$ leaves, $m$ edges, $c_{\geq 5}$ cycles of length at least $5$, $c_4$ 4-cycles, and $c_3$ 3-cycles. Then the group-based network variety $V^{G}_{\C N}$ has dimension $m- (c_{\geq 5} + 2c_4 + 3 c_3) +1$.
 \end{theorem}
...
We will not define here the tropical variety $\Trop(C) \subseteq \mathbb{R}^n$, but we note two relevant facts: $\Trop(C)$ is a polyhedral complex with the same dimension as $C$, and $\im(\Trop(f)) \subseteq \Trop(C)$ (see e.g. \cite{maclagan2021introduction}). Therefore the Jacobian of $\Trop(f)$ at a point $\lambda \in \mathbb{R}^m$ where the map is differentiable gives a lower bound on $\dim(C)$ (although it is no longer true that equality necessarily holds when $\lambda$ is generic).
...
With notation as above, the \emph{toric fiber product} of $I$ and $J$ is
 ...
 \end{definition}
 Note that since the toric fiber product $I\times_{\C A} J$ is the kernel of a ring homomorphism, it is a prime ideal. It will be helpful for us to also consider the monomial homomorphism
  \begin{equation*}
  \begin{aligned} 
...
Samuel Martin is grateful for funding from EPSRC (grant number EP/W007134/1) and BBSRC (grant number BB/X005186/1).
