...
An overview of \textbf{AntGPT} is shown in Figure~\ref{fig:model}.
...
The long-term action anticipation (LTA) task requires predicting a sequence of $Z$ actions in a long future time horizon based on a video observation. In the LTA task, a long video $V$ is split into an ordered set of $N$ annotated short segments $\{S^j, a^j\}_{j=1}^N$, where $S^j$ denotes the $j$-th segment in video $V$ and $a^j$ denotes the corresponding action label in the form of noun-verb pair $(n^j, v^j)$. The video is also specified with a stop time $T$, which is essentially the index of the last observed segment. In this way, a video is split into the observed segments $V_{o}$ and the future segments of the video $V_{f}$ whose labels $\{\hat{a}^{(T+1)}, ..., \hat{a}^{(T+Z)}\}$ are to be predicted. A hyper-parameter $N_\text{seg} $ controls how many segments the model can observe. Concretely, we take the observable video segments $\{S^j\}_{j=T-N_\text{seg}+1 }^T$ from $V_{o}$ as input and output action sequence $\{\hat{a}^{(T+1)}, ..., \hat{a}^{(T+Z)}\}$ as predictions. Alternatively, Ego-Topo~\cite{ego-topo} takes a simplified approach, which only requires predicting the set of future actions, but not their ordering.
...
We first consider the standard approach to represent video frames as distributed embedding representations, computed with pre-trained vision backbone models, such as the CLIP visual encoder~\cite{radford2021clip}. For each video segment $S^j$ in $V_o$, the backbone extracts the representations of uniformly sampled frames from this segment to obtain $E^j = \{e_1, e_2, \ldots, e_n\}$. A neural network can then take the embedding representation and predict action labels for the observed frames (action recognition), or the future timesteps (action anticipation).
...
\noindent\textbf{Few-shot Goal Generation.} In order to perform top-down long-term action anticipation, we use in-context learning to generate the goals by taking the recognized action labels as inputs, as illustrated in Figure~\ref{fig:model} (b). We perform ICL with OpenAI GPT-3.5-turbo. The ICL prompts $q_\text{goal}$ is formulated with examples in the format of \texttt{"<observed actions> => <goal>"} and the final query in the format of \texttt{"<observed actions> =>"}. The observed actions for the in-context examples are based on ground-truth annotations, and the observed actions in the final query are generated by recognition models.
Since no ground truth goals are available, we either use the video metadata as \textit{pseudo goals} when it is available, or design the goals manually. Figure~\ref{fig:vis2} (a) shows an example for in-context goal inference with the LLM. We treat the raw output of the LLM $T_\text{goal} = \mathcal{\pi}(q_\text{goal})$ as the high-level goal.
...
Thanks to few-shot goal generation with in-context learning, implementing the top-down model $\mathcal{F}_\text{td}$ is straightforward: We first embed the inferred goals $T_\text{goal}$ with a pre-trained CLIP text encoder. The goal token is then prepended at the beginning of the visual embedding tokens to perform goal-conditioned action anticipation. During training, we use ground-truth action labels to infer the goals via in-context learning. Durining evaluation, we use the recognized action labels to infer the goals.



\noindent\textbf{Modeling Temporal Dynamics with LLMs.}
We further investigate if LLMs are able to model temporal dynamics via recognized action labels and perform action anticipation via autoregressive sequence completion. We first study the fully supervised scenario, where we fine-tune an LLM (GPT-3-curie) on the training set of an LTA benchmark. As illustrated in Figure~\ref{fig:model} (d), both the input prompt and the target sequence are constructed by concatenating the action labels separated with commas. During training, we apply teach forcing by forming the input prompts from ground-truth action labels. The LLM is optimized with the standard sequence completion objective. During inference, we use the action recognition model $\mathcal{E}$ to form (noisy) input prompts from the recognized action labels. We perform postprocessing to convert the output sequence into action labels. Details of the postprocessing can be found in Section~\ref{sec:postprocessing}. To perform top-down LTA, we simply prepend an inferred goal at the beginning of each input prompt. The goals are again generated from ground-truth actions during training, and recognized actions during evaluation.





\noindent\textbf{Few-shot Learning with LLMs.} Finally, we are interested in understanding if LLM's in-context learning capability~\cite{gpt3, touvron2023llama, ouyang2022training, wei2021finetuned} generalizes to the LTA task. Compared with fine-tuning model with the whole training set, in-context learning avoids updating the weights of a pre-trained LLM. We use the same GPT-3.5-turbo model as for few-shot goal generation, which exhibits competitive in-context learning performance after instruction tuning~\cite{ouyang2022training}.
As illustrated in Figure~\ref{fig:model} (d), an ICL prompt consists of three parts: First, an instruction that specifies the anticipating action task, the output format, and the verb and noun vocabulary. Second, the in-context examples randomly sampled from the training set. They are in the format of \texttt{"<observed actions> => <future actions>"} with ground-truth actions. Finally, the query in the format \texttt{"<observed actions> => "} with recognized actions. An example of the model's input and output is shown in Figure~\ref{fig:vis2} (b). 

While few-shot top-down LTA can be performed in two stages, by first inferring the goals and then forming the in-context prompt, a more elegant approach is based on chain-of-thoughts prompts~\cite{wei2022chain} (CoT). As shown in Figure~\ref{fig:model} (e), a CoT prompt also consists of three parts: task definition and rule specification, example CoT demonstrations, and the CoT queries. We construct two CoT questions: \texttt{Q1.What's the goal according to previous actions? Q2.What are the future actions based on the goal and previous actions?} To generate the CoT examples, we choose samples from training set and then use the LLM-generated goal as the answers of \texttt{Q1}, the ground-truth future actions as the answer of \texttt{Q2}. Finally, we formulate CoT queries in the format of  \texttt{"<observed actions> Q1 Q2 => "}. An example of CoT LTA is shown in Figure~\ref{fig:vis2} (c).
