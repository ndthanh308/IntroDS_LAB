...
We demonstrate that this group of suspended accounts did influence (in a causal sense)
the retweet activity of undecided users and weak supporters—individuals
uncertain about their voting choices—in terms of political polarization. 

The paper is structured as follows: In Section \ref{data}, we provide
information about the data utilized and the methods employed. Section
\ref{Section1} delves into the results of our analyses, following these
steps: {\bf a)} Characterizing users in our dataset based on the
information they disseminate; {\bf b)} Employing the IRA ego network
as a proxy to identify a subset of suspended users exhibiting similar
patterns; and {\bf c)} Assessing the impact of this group of suspended
accounts on Twitter discourse through Granger causality. The paper
concludes with a comprehensive discussion and conclusion that encapsulates
insights derived from our analyses.
...
{\bf Version two-sided}
Similarly to the one-sided version, to test for differences in the in/out-degree
activity of suspended, not found,
...
In terms of in-degree results, verified
accounts differ from suspended, not found, and not verified accounts, 
but align with IRA users.
...
we delve into the IRA ego network. This network encompasses all users
...
remaining 2.3\% of users are categorized as undecided.

The majority of suspended accounts fall into the category of Trump
supporters, whether weak or strong, as shown in Fig. \ref{table:five}.
...
from the content shared by users, as shown in Fig. \ref{fig:11}. 
...
as indicated in Table~\ref{table:eightb} and Supplementary Figure 6. 
...
This section delves into measuring the impact that suspended accounts and IRA 
have on shaping the Twitter discourse during the 2016 US presidential elections. 
Specifically, we scrutinize the causal relationships between IRA
...
This is achieved through the application of Granger causality to the
tweet activity produced by IRA suspended accounts, and each supporting
...
regarding changes in users' vote intentions require longitudinal data.
...
presidential elections. Further research is required to delve
deeper into their impact on political user preferences.
...
\subsection*{Funding}
HAM was supported by NSF-HNDS Award 2214217. JSA gratefully acknowledges the Brazilian agencies FUNCAP, CNPq and CAPES, the National Institute of Science and Technology for Complex Systems in Brazil, and the PRONEX-FUNCAP/CNPq Award PR2-0101-00050.01.00/15 for financial support. Matteo Serafino gratefully acknowledges the Brazilian agency CAPES Award 88887.899221/2023-00.
...
\end{tabular}
\caption{{\bf IRA ego network: Interaction between IRA accounts and other group of users.}
...
and other users. The breakdown is per account type and direction of the interaction. $\rightarrow$X indicated that the X-type accounts interacted with ego nodes, while X$\rightarrow$ has an opposite meaning. Values are normalized over the total weighted edges in each community. }
\label{table:ten}
\end{table}
