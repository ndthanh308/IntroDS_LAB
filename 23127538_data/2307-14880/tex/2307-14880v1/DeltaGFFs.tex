...
\begin{equation}\label{eq:deltainterpolating}
	J_{\alpha}(x)=\frac{1}{\sqrt{3}}\varepsilon^{abc}\left[\vphantom{\int_0^{x_2}}2(u^{aT}C\gamma_{\alpha}d^{b})u^{c}+(u^{aT}C\gamma_{\alpha}u^{b})d^{c}\right],
\end{equation}
...
&&- i g_{\mu\nu} \Big[\bar{u}(z)  \big(\overleftrightarrow{\dslash} - m_u\big)u(z)
+\bar{d}(z)\Big( \overleftrightarrow{\dslash} -  m_d\Big)d(z)  \Big],\\
...
g_{\mu\nu} \Big[\bar{u}(z)  \big(\overleftrightarrow{\dslash}-m_u\big)u(z)+\bar{d}(z)\Big( \overleftrightarrow{\dslash}-m_d\Big)d(z) \Big] 
\simeq  
...
+\cdots,
\end{equation}
where $m = m_{\Delta}$ and the dots indicate the higher states and continuum contributions. The matrix element of the EMT current between $\Delta$ states can be expressed in terms of ten form factors ~\cite{Kim:2020lrs},
...
\begin{equation}\label{eq:totalGamma}
	...
	+ \Gamma_{\alpha\mu\nu\beta}^{(4D,g)}.
\end{equation}
where the correlation function has a perturbative part $\Gamma^{(P)}$ and non-perturbative parts $\Gamma^{(3D)}$, $\Gamma^{(4D)}$ and $\Gamma^{(5D)}$ in three, four and five dimensions, respectively, which are shown in the Appendix~\ref{Calcu}. We transform the calculations to momentum space, by  employing ~\cite{Azizi:2017ubq},
...
\langle r^2_E\rangle = 6 \frac{d \varepsilon_0(Q^2)}{dQ^2} |_{Q^2=0}.
\end{equation}
In the following section, we will perform numerical analysis of the obtained GFFs and other observables made of these GFFs and discuss their values at the static limit, which corresponds to zero transferred momentum. 
...
As expected, we observe that the $Q^2$ dependencies of the GFFs are smoothly change and decrease with increasing of the $Q^2$. 
...
\begin{table}[!htb]
	...
	\begin{tabular}{c*{12}{c}r}	
		...
		\hline
	\end{tabular}
	\caption{\small The numerical values of p-pole fit parameters ${\cal F}(0)$,  $m_{p}$ and $ p $ of the GFFs in Fig.~\ref{fig:Qsqfigs} at mean values of the continuum threshold. }
...
We present and compare some mechanical properties extracted from our work and other studies in the rest of this section. Some $\Delta$'s GMFFs in the static limit from our calculations are shown in Table.~\ref{table:CompareResultsZeroQ2} and compared with results of Ref.~\cite{Kim:2020lrs}. The normalization condition for $\Delta$ mass is $1$, which is consistent with $\varepsilon_0(0) = F_{1,0}(0) = 1.01 \pm 0.15$ from our calculations. We obtain $\mathcal{J}_1(0) = \frac{1}{3} F_{4,0}(0) =  0.46 \pm 0.06$ for the dipole angular momentum where $F_{4,0}(0) = 1.38 \pm 0.19$ corresponds to spin of $\Delta$ baryon which is $3/2$. This result is well consistent with the prediction of Skyrme model within the presented errors.
%
...
In this appendix, we collect some parts of QCD side solutions of three-point correlation function. In Eq.~\eqref{eq:contractions}, after using Wick's theorem and calculating all possible contractions, $\Gamma^q$ and $\Gamma^g$ are obtained as below,
...
Z. Dehghan and K. Azizi are thankful to Iran Science Elites Federation (Saramadan) for the partial financial support provided under the grant number ISEF/M/401385.
