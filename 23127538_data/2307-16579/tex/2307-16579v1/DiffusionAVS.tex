...
%   {% Figure removed}\\
%     % \footnotesize{Image} &
%     % \footnotesize{Fixation} & \footnotesize{Binary} & \footnotesize{Ranking} \\
%   \end{tabular}
%   \end{center}
%     \captionof{figure}{AVS prediction of the same image with different audio as guidance, where the first column shows the original audio, the RGB image, and the corresponding ground-truth, and the second to fourth columns show predictions of AVSBench~\cite{zhou_AVSBench_ECCV_2022} (top) and ours (bottom) with different audio as guidance.
%     } 
%     \label{fig_model_prediction_with_different_audio}
% \end{center}%
...
% %   \end{center}
% %     \captionof{figure}{
% %     % Illustrating the ranking and discriminative region of camouflaged objects. 
% %     The conventional \enquote{Binary} ground truth only provides the scope of the camouflaged objects. We present additional %NB the extra
% %     fixation (\enquote{Fixation}) and ranking (\enquote{Ranking}) annotations,
% %     % lead to better understanding about    camouflage, 
% %     where the former discover regions that make camouflaged objects detectable and the latter highlight the level of camouflage. Blue color in \enquote{Ranking} indicates higher rank (harder) of camouflage.
% %     % more explanatory camouflaged object detection model.
% %     } 
% %     \label{sample_show_front_page}
% % \end{center}%
% }]
%%%%%%%%% ABSTRACT
\begin{abstract}
...
In this way, our latent diffusion model via contrastive learning explicitly maximizes the contribution of audio for AVS. Experimental results on the benchmark dataset verify the effectiveness of our solution. 
...
Audio-visual segmentation (AVS) aims to accurately segment the region in the image that produces the sound from the audio, which is also defined as the sound source segmentation task. 
...
Diffusion models have also been extended in 3D vision~\cite{cheng_3DDiffusion_arxiv_2022}. For multimodal generation~\cite{weinbach_M_VADER_arxiv_2022,gopalakrishnan2022image}, each unimodal is treated equally, which is different from our setting that one modality serves as guidance for the conditional generation process.
...
of size $D=24$. Ablation on the size of the latent space can be found in the supplementary material.
...
For the latent diffusion model, we use the cosine noise schedule and the noise prediction objective in \equref{ddpm_loss} for all experiments. The diffusion steps $K$ is set as 20. To accelerate sampling, we use the DDIM~\cite{song_DDIM_ICLR_2020} with 10 sampling steps.~\footnote{The code will be released publicly to ensure reproducibility.}
