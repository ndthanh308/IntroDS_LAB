...
\begin{document}
\title{Cascaded Coded Distributed Computing With Low Complexity and Improved Flexibility
...
In this paper, we focus on the cascaded case of $K/s\in\mathbb{N}$, deliberately designing the strategy of data placement and output functions assignment based on a grouping method, such that a low-complexity Shuffle strategy of two rounds is achievable.
The main advantages of the proposed scheme include: 1) the multicast gains equal to $(r+s-1)(1-1/s)$ and $r+s-1$ which is approximate to $r+s-1$ when $s$ is relatively large, and the communication load is quite approximate to or surprisingly better than the optimal state-of-the-art scheme proposed by Li \emph{et al.}; 2) the proposed scheme requires significantly less number of input files and output functions; 3) all the operations are implemented over the minimum binary field $\mathbb{F}_2$ in the one-shot fashion.
Finally, we derive a new information-theoretic converse bound for the cascaded coded distributed computing framework, under the given strategies of data placement and output functions assignment.
...
In the pioneering paper \cite{LMYA}, Li \emph{et al.} extended the coding technique to distributed computing and proposed the coded distributed computing (CDC) scheme, in order to reduce the communication load by increasing the computations of input data. In the considered MapReduce setting, $K$ distributed computing nodes are utilized to compute $Q$ output functions from $N$ input files by three phases: Map, Shuffle, and Reduce.
...
Note that the above-mentioned cascaded CDC scheme need exponentially large quantities of both input files and output functions, which are increased with  $\binom{K}{r}$ and $\binom{K}{s}$, respectively.
As a consequence, practical implementations encounter unexpected performance degradation, particularly when $K$ assumes relatively large values.  To reduce the required numbers of both input files and output functions, the authors in \cite{JQ} generated cascaded CDC schemes by using the Partition placement delivery array (which is originally proposed in \cite{YCTC}) under the binary field $\mathbb{F}_2$.
...
In \cite{WCJ}, the authors proposed a scheme based on hypercube structure under the constraint of $r=s$ with less number of input files and output functions, and a high-performance communication load that is asymptotically optimal.
However, both the number of input files and output functions are still exponentially in the form of $(\frac{K}{r})^{r}$.
In \cite{JWZ}, the authors employed a symmetric balanced incomplete block design (SBIBD) to present a strategy of  data placement and output functions assignment,  ultimately generating an asymptotically cascaded scheme with $N=Q=K$, while under the constraint of $r=s$ or $r+s=K$.
In \cite{CWL}, the authors proved that the maximum multicast gain under the one-shot linear delivery is $r+s-1$.
...
In \cite{CLCEL}, the authors proposed two asymptotically optimal cascaded schemes with $N=Q=K$. One of them used symmetric design (SD) for the case of $r+s=K$, while achieving a lower communication load than that of \cite{JWZ}; the other one uses almost difference (AD) sets.
In \cite{CLCEL2}, focusing on the case of $r=s$, the authors proposed an improved asymptotically optimal cascaded scheme relying on another special $t$-design.
However, from \cite{CD}, the AD sets and the specific $t$-design in \cite{CLCEL} and \cite{CLCEL2} are rare at present.
%sparse entities.
These schemes are summarized in Table~\ref{tab-comp}.
\begin{table}
\centering
\vspace{-0.8cm}
\caption{Comparisons of the proposed and existing cascaded CDC schemes}\label{tab-comp}
...
\tabincell{c}{\cite{LMYA}: \\ $K,r,s\in\mathbb{N}$,\\ $1\leq r$,\\ $s\leq K$} & $K$ & $r$ & $s$ & ${K\choose r}$ & ${K\choose s}$ & \tabincell{c}{$\sum\limits_{l=\max\{r+1,s\}}^{\min\{r+s,K\}}$\\ $\left(\frac{{K-r\choose K-l}{r\choose l-s}}{{K\choose s}}\cdot\frac{l-r}{l-1}\right)$} & No \\ \hline
...
It is worth noting that the aforementioned schemes that reduce the numbers of input files and output functions impose stringent requirements on system parameters $(r,s, K,N,Q)$, which poses obstacles to the implementation of existing schemes in practical scenarios. More specifically,
\begin{itemize}
...
From  Table \ref{tab-comp}, we observe that only the schemes in \cite{JQ} and \cite{WCJ} can support for flexible $K$ among all existing schemes reducing coding complexity in terms of  $(N,Q)$.
It is interesting that  these two schemes both are designed based on the grouping approach from the view point of combinatorial design. More precisely, both \cite{JQ} and \cite{WCJ}   adopt the same  data placement strategy based on the combinatorial structure named Partition or hypercube, which can be seen as an utilization of \emph{Cartesian Product method} in \cite{WCWC} such that the strategy of each group is distinct from each other. The difference between \cite{JQ} and \cite{WCJ}  lies in the  output functions assignment  and signal transmission strategies,  inducing their individual strengths and weaknesses:
...
\begin{itemize}
	...
	\item In \cite{WCJ}, the output function assignment is the same as its file storage. During the Shuffle phase, each node multicasts linear combinations of IVs segments, and the resulting scheme with low communication load is proved to be order optimal under the specific output function assignment.  Particularly, when $r=s=2$, the communication load is strictly less than that of \cite{LMYA}, where the multicast gain is $\frac{3K}{2(K+1)}$. However, the value of $Q$ still grows exponentially with $K$, and the scheme is not feasible in a binary field.
\end{itemize}
...
\emph{An interesting question is whether one can achieve low communication load approaching to or even better than \cite{WCJ}, while simultaneously keeping small $N,Q$ and working in binary field with low complexity. }


In this paper, we give an affirmative answer by proposing   communication-efficient, flexible, and low-complexity cascaded CDC scheme that reduce both the communication load and required values of  $(N,Q)$, while breaking the stringent requirements in existing works, including 1) $r=s$ or $r+s=K$; 2) $N=Q$; 3) $K$   takes the form of combinations or as a polynomial of primes.  %Our main contribution of this paper are outlined below.
...
\item Unlike   existing schemes in \cite{WCJ,JWZ,CWL,CLCEL,CLCEL2} requiring   $r\in\{s,K-s\}$ and $N=Q$,  we propose a novel CDC scheme that not only supports   flexible setting   with  $K/s\in\mathbb{N}$ and $N$ being independent of $Q$, but also greatly reduces the communication load. %In our scheme, we divide the computing nodes into $s$ equal-sized groups, and apply asymmetric data placement among $s$ groups (the existing scheme use symmetric assignment).
%Compared to the existing schemes which
Our scheme involves a Shuffle strategy of two rounds that achieve multicast gains   $(r+s-1)(1-1/s)$ and $r+s-1$, respectively, much larger than that of the existing schemes \cite{JQ,WCJ}.  When $s$ is relatively large, the multicast gain is approximate to $r+s-1$, which is  the maximum under one-shot linear delivery  \cite{CWL}.% where the data placement in $s-1$ groups are the same, and the remaining   one group is assigned more data to  provide  larger  multicast opportunities. %,  equal to $(r+s-1)(1-1/s)$ and $r+s-1$.
...
\item For any positive integers $a$, $b$, $t$ with $a<b$ and $t\leq b $,   non-negative set $\mathcal{V},\mathcal{J}$,
	\begin{itemize}
		...
		\item Assume that all the sets are in increasing order.
...
In the coded distribute computing model \cite{LMYA}, the task is to compute $Q$ output functions from $N$ input files  by using $K$ computing nodes.
The $N$ input files are denoted by $w_1,\ldots,w_N$;
the $Q$ output functions are denoted by $\phi_1,\ldots,\phi_Q$.
For each $q\in[Q]$,  define $\mathcal{A}_q\subseteq[K]$ to be the set of nodes that are assigned to compute the output function $\phi_q$, which maps all the $N$ input files into a stream
\begin{align}
	u_q&\triangleq\phi_q(w_{1},w_{2},\ldots,w_{N})\\
	&\triangleq h_q(g_{q,1}(w_{1}),g_{q,2}(w_{2}),\ldots,g_{q,N}(w_{N})) \in \mathbb{F}_{2^E},
\end{align}
for some integer $E$. For any $q\in [Q]$ and $n\in [N]$,  $g_{q,n}(\cdot)$ is  called Map function and $h_q(\cdot)$ is called Reduce function. The parameter $v_{q,n}\triangleq g_{q,n}(w_{n})$ where $q\in [Q]$ and $n\in [N]$ is called intermediate value (IV). We assume that each IV has $T$ bits for some positive $T$. In order to support multiple-round computing, where the reduced results of the previous round are the inputs of the next round Map operation, each Reduce function is assumed to be computed by $s\in[K]$ nodes. The cascaded CDC problem consists of the following three phases.

\textit{Map Phase}:
For each $n\in[N]$,  define $\mathcal{D}_n\subseteq[K]$ as the set of nodes that  placed  the input file $w_n$.
...
\textit{Shuffle Phase}:
According to the output functions arranged, for each node $k\in[K]$, the set of assigned output functions is denoted by
\begin{align}
...
\end{align}
and $L(K,s)=0$.
The proposed scheme requires input files $N/\left({K/s\choose (r-1)/s+1}(r+s-1)\right)\in\mathbb{N}$  and output functions $Q/(K/s)\in\mathbb{N}$.
\end{theorem}

\begin{corollary}
\label{corol-multicasting gain}
For the scheme in Theorem~\ref{th-proposed scheme}, the multicasting gain is consisted of $(r+s-1)(1-1/s)$ and $r+s-1$ (detailed by \eqref{eq-g1} and \eqref{eq-g2} in Section~\ref{sec-proof}).
...
\label{th-converse}
In the cascaded CDC framework with $s>1$ and $K/s\in\mathbb{N}$,  under the given strategies of data placement and output functions assignment, the converse bound satisfies that
\begin{align}
...
In this section, we present some numerical comparisons of the proposed scheme in Theorem~\ref{th-proposed scheme}, the new converse bound in Theorem~\ref{th-converse} compared to the optimal scheme in \cite{LMYA}, and the other grouping schemes in \cite{JQ} and \cite{WCJ}.
...
\begin{table}
	\center
	\vspace{-0.8cm}
	\caption{Communication load for the cascaded CDC with $K=30$ and $s=5$}\label{tab-K30s5}
	\resizebox{\linewidth}{!}{
		\begin{tabular}{cccccccccc}
			\toprule[1.5pt]
			& {$r=5$} & {$r=6$} & {$r=10$} & {$r=11$} & {$r=15$}  & {$r=16$} & {$r=21$} & {$r=26$}  \\ \hline
			Scheme in \cite{LMYA}  & $0.505$ & $0.439$ & $0.267$ & $0.236$ & $0.1481$ & $0.132$ & $0.068$ & $0.025$  \\
			Scheme in \cite{JQ}  & -& $0.8$ & $0.370$ & -& $0.179$ & -& -& -\\
			Scheme in \cite{WCJ}  & $0.522$ & -& -& -& -& -& -& -\\
			Proposed scheme  & -& $0.417$  & -& $0.222$ & -& $0.125$ &  $0.067$ & $0.028$ \\
			Converse bound  & - & $0.407$ & - & $0.214$ &-  & $0.118$ &$0.061$ & $0.023$\\
			\bottomrule[1.5pt]
	\end{tabular}}
\end{table}
...
Here, we provide the numerical results for the case of $K=30$ and $s=5$, as illustrated in Table~\ref{tab-K30s5}\footnote{Considering the communication load $L(r,s)$, since all schemes satisfy that $L(1,s)=1$ and $L(K,s)=0$, in Table~\ref{tab-K30s5} the points $r=1$ and $r=30$ are omitted} and Fig.~\ref{fig-K30s5}.
Notice that, these schemes have their own applicable computation load $r$. From Table~\ref{tab-comp}, we have:
\begin{itemize}
\item In \cite{LMYA}, $r\in[K]=[30]$. Due to space limitations, in Table~\ref{tab-K30s5} we only listed the points that need to be compared with other schemes.
\item In \cite{JQ}, when $\frac{s}{r-1}(1-\frac{r}{K})<1$ and $K/r\in[N]$, the $r$ is available. Thus, we have $r\in\{6,10,15\}$.
\item In \cite{WCJ}, since $r=s$ is required, only $r=5$ is available.
\item In the proposed scheme, $r\in\{1,s+1,2s+1,\ldots,(K/s-1)s+1\}$ by Theorem~\ref{th-proposed scheme}. Thus, we have $r\in\{6,11,16,21,26\}$
\end{itemize}
For the other values of  $r$, it can be generalized by \cite[Section V-E]{LMYA}.
In Fig.~\ref{subfig-K30s5L}, the communication loads of the proposed scheme are clearly better than those of \cite{JQ} and the most region of \cite{WCJ}, and quite close to those of \cite{LMYA}. Surprisingly, some of the points are better than \cite{LMYA}, for instance when $6\leq r\leq21$.
...
Therefore, the proposed scheme has the potential to outperform other grouping schemes in terms of both the communication load and the required number of files, and it is implemented with one-shot delivery over the binary field.
...
For simplicity of expression, for each $n\in[N_1]$, the subscript $n$ of input files $w_n$ is denoted by a tuple $(i,\mathcal{C},\mathcal{T})\in\mathcal{N}$, and formulated by
\begin{align}
...
The file $w_{(i,\mathcal{C},\mathcal{T})}$ is stored by nodes in
\begin{align}
...
There are totally $N_1\times Q_1$ different IVs, $\{v_{q,(i,\mathcal{C},\mathcal{T})}: q\in[Q_1],(i,\mathcal{C},\mathcal{T})\in\mathcal{N}\}$, where $v_{q,(i,\mathcal{C},\mathcal{T})}$ is locally mapped by the nodes in $\mathcal{D}_{(i,\mathcal{C},\mathcal{T})}$ and requested by the nodes in $\mathcal{A}_q$ during the Shuffle phase.
For these IVs, based on the number of nodes requesting them during the Shuffle phase, they are classified into 3 categories: \textit{Type I} requested by $0$ node; \textit{Type II} requested by $s-1$ nodes; and \textit{Type III} requested by $s$ nodes.
...
\begin{remark}
\label{rem-s=1 F0'}
When $s=1$, we have $\Lambda=K/s=K$ and $t=(r-1)/s=r-1$. Then, \eqref{eq-n index} is reduced to $\mathcal{N}'=\left\{(1,\mathcal{C},\mathcal{T}):\mathcal{C}\in{[K]\choose r},\mathcal{T}\in{\mathcal{C}\choose r-1}\right\}$, thus $N_1'=|\mathcal{N}'|=r{K\choose r}$.
...
Hence, there are $F_0'=|\mathcal{V}_{\text{I}}'|=N_1'r$ distinct IVs belonged to Type I, since  $\mathcal{V}_{\text{I}}'=\{v_{q',(1,\mathcal{C},\mathcal{T})}:\forall{(1,\mathcal{C},\mathcal{T})\in\mathcal{N}'},q'\in\mathcal{C}\}$.
\end{remark}
...
We can check that, $F_0+F_1+F_2=N_1t+N_1+N_1(\Lambda-(t+1))=N_1\Lambda=N_1\times Q_1$.
...
v_{1,(1,\{1,3\},\{1\})} \ \ \ \ \ \ \text{since} \ \mathcal{A}_1=\{1,4\}\in\mathcal{D}_{(1,\{1,2\},\{1\})}=\{1,3,4\}, \nonumber \\
...
\begin{align}
X_{\mathcal{K}}=\left(\bigoplus_{q\in\mathcal{L}} v_{q,(i,\mathcal{C},\mathcal{T})} \ : \ \mathcal{L}=\mathcal{C},\mathcal{T}=\mathcal{L}\setminus\{q\}\right), \label{eq-X1-sum}
\end{align}
to the nodes in  $\{\mathcal{A}_q\setminus \mathcal{K}:q\in\mathcal{L}\}$ over the broadcast channel. Thus the number of nodes served by one message is
\begin{align}
g_1=|\mathcal{A}_q\setminus \mathcal{K}|\cdot|\mathcal{L}|=(s-1)(t+1). \label{eq-g1}
\end{align}
To be average, each IV included in \eqref{eq-X1-sum} is divided into $|\mathcal{K}|=t+1$ disjoint equal-size packets. For each node $k=\mathcal{K}[j],j\in[t+1]$, it multicasts the following message,
\begin{align}
X_k=X_{\mathcal{K}[j]}=\left(\bigoplus_{q\in\mathcal{L}} v_{q,(i,\mathcal{C},\mathcal{T})}^{(j)} \ : \ \mathcal{L}=\mathcal{C},\mathcal{T}=\mathcal{L}\setminus\{q\}\right), \label{eq-X1}
\end{align}
to the nodes belonged to $\{\mathcal{A}_q\setminus \mathcal{K}:q\in\mathcal{L}\}$.
...
\begin{align}
X_{\{1,2\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(1,\mathcal{C},\mathcal{T})}:  \mathcal{L}=\{1,2\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
=&v_{1,(1,\{1,2\},\{2\})}\bigoplus v_{2,(1,\{1,2\},\{1\})}\overset{\text{Table \ref{tab-mapping}}}{=}v_{1,2}\bigoplus v_{2,1}, \nonumber \\
X_{\{1,3\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(1,\mathcal{C},\mathcal{T})}  :  \mathcal{L}=\{1,3\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
=&v_{1,(1,\{1,3\},\{3\})}\bigoplus v_{3,(1,\{1,3\},\{1\})}\overset{\text{Table \ref{tab-mapping}}}{=}v_{1,4}\bigoplus v_{3,3}, \nonumber \\
X_{\{2,3\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(1,\mathcal{C},\mathcal{T})}  :  \mathcal{L}=\{2,3\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
=&v_{2,(1,\{2,3\},\{3\})}\bigoplus v_{3,(1,\{2,3\},\{2\})}\overset{\text{Table \ref{tab-mapping}}}{=}v_{2,6}\bigoplus v_{3,5}, \nonumber \\
X_{\{4,5\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(2,\mathcal{C},\mathcal{T})}  :  \mathcal{L}=\{1,2\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
=&v_{1,(2,\{1,2\},\{2\})}\bigoplus v_{2,(2,\{1,2\},\{1\})}\overset{\text{Table \ref{tab-mapping}}}{=}v_{1,8}\bigoplus v_{2,7}, \nonumber \\
X_{\{4,6\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(2,\mathcal{C},\mathcal{T})}  :  \mathcal{L}=\{1,3\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
=&v_{1,(2,\{1,3\},\{3\})}\bigoplus v_{3,(2,\{1,3\},\{1\})}\overset{\text{Table \ref{tab-mapping}}}{=}v_{1,10}\bigoplus v_{3,9}, \nonumber \\
X_{\{5,6\}}=&\left(\bigoplus_{q\in\mathcal{L}} v_{q,(2,\mathcal{C},\mathcal{T})}  :  \mathcal{L}=\{2,3\},\mathcal{T}=\mathcal{L}\setminus\{q\}\right) \nonumber \\
...
Compared with the optimal scheme in \cite{LMYA}, the communication load of our proposed scheme can outperform or quite approximate to that of \cite{LMYA}, the number of requested input files and output functions are significantly less, and all the operations of the proposed scheme are implemented over the binary field.
Compared with the other grouping schemes in \cite{JQ} and \cite{WCJ}, the proposed scheme has the potential to improve both the communication load and the required number of files.
