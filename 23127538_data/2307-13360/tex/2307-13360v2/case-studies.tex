...
%\subsection{Reversible Calculus of Communicating Systems (RCCS)}\label{sec:rccs}
\subsection{Reversible CCS (RCCS)}\label{sec:rccs}
We consider here the semantics of RCCS %(\iu{Reversible Calculus of Communicating Systems})
in~\cite{DK04}, and restrict the attention to coherent processes~\cite[Definition 2]{DK04}. 
...
\subsection{CCS with Communication Keys (CCSK)}
...
\subsection{Higher-Order $\pi$-calculus (HO$\pi$)}\label{sec:hopi}
...
\subsection{Reversible $\pi$-calculus (R$\pi$)}\label{sec:pi}
...
\subsection{Reversible Internal $\pi$-calculus with Extrusion Histories ($\pi$IH)}
%The reversible internal $\pi$-calculus \iu{with extrusion histories}
The calculus $\pi$IH~\cite{GPY21} is based on the work of Hildebrandt \emph{et al.}~\cite{HJN19},
...
(SP, BTI, WF, PCI and IRE); the only exception is that IEC fails,
...
This is for instance the case for Janus programs~\cite{YokoyamaG07} or CCSK processes without parallel composition such as the ones studied in~\cite{BR23}.
%}
