...
Thus, we adopt initially only the core of the notion of \emph{labelled
 transition system with independence} (LTSI)~\cite[Definition~3.7]{SNW96},
and explore what can be achieved by adding 
various axioms on the independence relation. This is in contrast with the approach
taken in ~\cite{SNW96}, which requires a fixed number of axioms to hold in LTSIs.
Also, we extend LTSIs with reverse transitions, since we
study reversible systems. We first define labelled transition
systems (LTSs).
...
is a transition relation. %{\em transition relation\/}. 
\end{definition}
We let $P,Q,\ldots$ range over processes,
$a,b,c,\ldots$ range over labels,
and $t,u,v,\ldots$ range over transitions (namely, elements of the transition relation).
...
$\revw\Lab = \{\rev a : a \in \Lab\}$,
and define the \emph{combined LTS} to be
${\tran{}}\subseteq \Proc \times (\Lab \cup \revw\Lab) \times \Proc$
by $P \tran a Q$ iff $P \ftran a Q$ and $P \tran{\rev a} Q$ iff $P \rtran a Q$.
\end{definition}
We stipulate that the union $\Lab \cup \revw\Lab$ is disjoint.
We let $\alpha, \beta, \ldots$ range over $\Lab \cup \revw\Lab$.
For $\alpha \in \Lab \cup \revw\Lab$, the \emph{underlying} action label
...
We define a labelling function $\lab$ from transitions to $\Lab\cup \revw\Lab$ by setting
$\lab((P,\alpha,Q)) = \alpha$.
...
%Given an LTS,
A \emph{path} is a sequence of forward or reverse transitions
of the form
...
Paths $P \ptran \rho Q$ and $R \ptran \sigma S$ are
...
%Let $(\Proc,\Lab,\tran{})$ be an LTS.
The irreversible processes in an LTS $(\Proc,\Lab,\tran{})$ are
$\Irr = \{P \in \Proc: P \not \rtran{}\}$.
A \emph{rooted path} is a
path $P \ptran \rho Q$ such that $P \in \Irr$.
...
\begin{remark}
From now on, unless stated otherwise, we consider a combined LTSI $(\Proc,\Lab,\tran{},\ind)$. We will refer to it simply as an LTSI.     
\end{remark}
