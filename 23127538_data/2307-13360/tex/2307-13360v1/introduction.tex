...
characterise reversible computation in the sequential setting.  Thus
in reversible finite state automata~\cite{Pin87}, reversible
cellular automata~\cite{Kari18}, reversible Turing machines~\cite{Bennett:1973} and
reversible programming languages such as Janus~\cite{YokoyamaG07} the
main point is that the mapping from inputs to
outputs is injective, and the reverse computation is deterministic.

Matters are less clear when it comes to reversible computation in the
concurrent setting. Indeed, various reversible concurrent models have
been studied, most notably in the areas of process
calculi~\cite{DK04,PU07,LaneseMS16}, event
structures~\cite{UlidowskiPY18}, Petri nets~\cite{BarylskaKMP18,MMU19}
and programming languages such as Erlang~\cite{LaneseNPV18}.

A main result of this line of research is that the
notion of reversibility most suited for concurrent systems is
\emph{causal-consistent reversibility} (other notions are also used, e.g., to model biological systems~\cite{PhillipsUY12}). According to
an informal account of causal-consistent reversibility, any action can be undone provided
...
$t$ can be undone after a derivation if and only if all its consequences, if any,
...
(Examples~\ref{ex:prerev not CSi},~\ref{ex:prerev not CL},~\ref{ex:CS CL not CC}), and that there are slightly
...
axioms (Sections~\ref{sec:basic}, \ref{sec:events}, \ref{sec:CSCL},
\ref{sec:coinitial}). This is in sharp contrast with the large part of
...
`Str.\ Ax.' abbreviates `Structural Axiom' and `Pre-rev.'  abbreviates `Pre-reversible', namely SP, BTI, WF, PCI (cf.~Def.~\ref{def:prerev}).}
    \label{t:list}  
\end{table}
}
...
a causal-consistent reversible setting, consider the following LTSs in
CCS (see Figure~\ref{fig:ccs1}):
% Figure environment removed
...
\item[$P \tran a P$ where $P = a.P$:] one can go back forever, against the idea that a state models a process reachable after a finite computation.
\end{description}
