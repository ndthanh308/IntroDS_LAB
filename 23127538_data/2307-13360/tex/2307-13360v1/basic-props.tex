...
\begin{definition}\label{def:PL}
  {\bf Parabolic Lemma (PL)}: for any path $r$ there are forward-only paths $s,s'$ such that 
...
Clearly $r$ is parabolic iff $d(r) = 0$.
...
There are various cases to check, depending on the initial and final states of the path.
Let $r:R \tran\rho R$ be any path from $R$ to $R$.
...
Now let $r:P \tran\rho R$ be any path from $P$ to $R$.
Then $r = tr'$ where $r'$ is a path from $R$ to $R$.
Hence $r \ceqt t$.
Now let $r:P \tran\rho P$ be any path from $P$ to $P$.
Then $r = tr'\rev t$ where $r'$ is a path from $R$ to $R$.
Hence $r \ceqt t\rev t \ceqt \es$.
Next let $r:P \tran\rho Q$ be any path from $P$ to $Q$.
