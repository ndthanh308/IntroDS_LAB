...
We have just seen that existing axioms are sufficient to show
...
We add coinitial independence as given by BTI and PCI.
The LTSI is pre-reversible.
However CS$\indt$ fails.
% \todo{can deduce this from Proposition~\ref{prop:IC CSi}}
We have $t_0:P \tran a Q$, $Q \ptran {bc} R$ and $t_0\op:S \tran a R$
with $t_0 \sqeqt t_0\op$.
...
Note, however, that CS$\indt$ holds since the only way to undo transitions is with transitions on the opposite side of the same diamond, and the path connecting them is the third side of the same diamond. Hence, the condition on independence holds thanks to BTI and PCI.
...
w.r.t.~reversing.
\begin{definition}[Reversing preserves independence (RPI)]\label{def:rpi}
...
\begin{definition}[Event Coherence (ECh)]\label{def:ECh}
If $t \sqeqt t'$ then $t \not\ind t'$.
\end{definition}
...
The axioms 
SP, BTI, WF, PCI, IRE, IEC are independent of each other.
% and do not imply ED.
% \todo{can omit ED}
\end{proposition}
%\end{restatable}
...
SP: Let $t:P \tran a Q$ and $u:P \tran b R$ with $t \ind u$.

BTI: Let $P \tran a R$ and $Q \tran b R$ with an empty independence relation
(Example~\ref{ex:notPL}).

WF: Let $P_{i+1} \tran a P_i$ for $i = 0,1,\ldots$ with an empty independence relation.

PCI: Let $t:P \tran a Q$, $u:P \tran b R$,
$u':Q \tran b S$, $t':R \tran a S$,
with $\rev{t'} \ind \rev{u'}$.

IRE: See Example~\ref{ex:CLG CSi}.

IEC: See Example~\ref{ex:IRE1} or Example~\ref{ex:IRE2}.
...
CIRE is only used in the last step. Hence, the result could be rephrased by stating that any prereversible LTSI satisfies (\ref{item:CDI}),
...
The axioms 
SP, BTI, WF, PCI, CIRE, IEC are independent of each other.
\end{proposition}
\begin{proof}
...
%\end{restatable}
\begin{proof}
(\ref{item:L BFCIRE}) $\Rightarrow$ (\ref{item:L CLci})
  Suppose $t_0:P \tran a Q$, $r:Q \ptran \rho R$ and $\cte(r,[t_0]) =
...
(\ref{item:L CLci}) $\Rightarrow$ (\ref{item:L BFCIRE})
...
The axioms 
SP, BTI, WF, PCI, BFCIRE, IEC are independent of each other.
\end{proposition}
\begin{proof}
%-short version-
