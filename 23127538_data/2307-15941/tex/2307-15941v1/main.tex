...
\renewcommand{\shortauthors}{Hao, et al.}
...
The common solution, as shown in Fig.~\ref{fig:training_modes}, is periodically re-training the model from scratch with a long range of historical data, however, with high costs in term of both computation and memory consumption.
...
One solution is to fine-tune the previously-learned model and to make it adaptable to the changes in the data distribution. 
...
Inspired by the application of continual learning (CL) in computer vision tasks, which learns from a stream of incoming data while avoiding forgetting prior knowledge, we combine replay-based CL with a regression model to solve our problem. As shown in Fig.~\ref{fig:training_modes}, CL maintains a memory set to store informative historical samples and concatenates it with the current data set to serve as the training set. The storage of the memory set is much smaller than that of the single periodic data set. 
In our practical scenario, the resource utilization is decreased by almost a quarter of the initial consumption,
...
In our industrial practices, we observe that the consumption of storage and computing resources is very high, due to the large number of applications and zones, incentivicing further optimization of resource usage and prediction accuracy, for which we devise new scheme for regression tasks based on continual learning in this paper.
...
As shown in Fig.~\ref{fig:framework}, our overall goal is to use weight sampling to select samples from $\mathcal{M}^{n-1}$ and $S^{n}$ as the memory set of the next period, so we focuse on the design of weights. In order to overcome the problem of sample overlap, while incorporating the sample in the new distribution into the memory set, we designed two scoring functions, \textbf{DensityScore} and \textbf{ShiftLevelScore}. 
...
Note that the memory set stores not only the selected samples and labels, but also the outputs of intermediate representation
from the current model, whose intention is explained in Sec.~\ref{sec:param_train} later.
...
\begin{itemize}[leftmargin=*]
    ...
    \end{align}
    % i.e., the probability of all samples ⋃n+1i=1Si \bigcup_{i=1}^{n+1}S^{i} is MAk+1+Nk+1\frac{M}{A^{k+1}+N^{k+1}}
...
\section{Experiments}
\subsection{Datasets} 
To evaluate the effectiveness of our proposed method, we conduct experiments on ATEC's\footnote{https://github.com/TRaaSStack/Forecasting} public industrial dataset, which is for workload forecasting competition.  We select one traffic series and split all data into 8 periodic datasets in chronological order to perform the evaluation on the workload forecasting task. Furthermore, we also perform the evaluation of CPU utilization estimation (CUE) on a zone of a main application whose data was collected from an industrial scenario. We select datasets of 8 days, whose distributions are significantly different from each other, which can be observed intuitively after being projected onto a 2-dimensional plane after dimension reduction, as shown in Fig.~\ref{fig:cpu_dist}.
% \footnote{Code will be released at https://github.com/anonymousaccountx/DMSHM.git.} 
...
To measure memorization and forecast performance of the model in continual learning setup, we regard the first dataset as a historical set, and the samples compose of random selection from all 8 datasets as a future set.
After every period, we record the mean square error (MSE) of the model on both the historical set and future set. 
...
% RSHM    & 0.0433 & 0.0302 & 17.85 & 16.91 \\ 
...
To explore the performance of the model in each period, we demonstrate MSE of the four models on the historical set and future set in 8 days, as illustrated in Fig.~\ref{fig:mse_history_point} and Fig.~\ref{fig:mse_future_point}. The MSE on the historical set of all models appears to have a tendency to decrease before the fifth day, due to the fact that the datasets of these days are of high similarity.  However, on the sixth day, when a configuration
change takes place, the sample distribution has a somewhat larger change, as shown in Fig.~\ref{fig:cpu_dist}. 
