...
different arrangement of protons and neutrons. The surface is covered by an ocean 
of $^{56}$Fe nuclei while the layers beneath are solid and composed of 
...
all nuclear reactions and decays, a primary goal of these models is to reproduce
as accurately as possible all known nuclear masses of nuclei with $N,Z\geq 8$. 
...
Although dedicated mass models augmented with machine learning techniques can
reach rms deviations lower than 0.1~MeV~\cite{Niu22}, such approaches lack the reach 
in terms of additional observables that EDF-based models can offer. The 
BSkG-series combines the reproduction of known masses with, among other things, 
...
\end{align}
%
where $t=0,1$ is an isospin index. Different terms in the energy densities are
bilinear combinations of different local densities and their derivatives: 
...
To adress these deficiencies, we discard the phenomenological ansatz for 
...
%
%
We now start from the following ansatz
%
\begin{align}
g_{q}(\rho_n, \rho_p) &=
...
This tool iterates $N_n$ single-neutron and $N_p$ single-proton wavefunctions on a 
...
discussion in Ref.~\cite{Ryssens23b}.}. In our most general calculations, all
moments $\beta_{\ell m}$ for arbitrary values of $\ell$ but even values of $m$
are generally non-zero. In particular, this includes two quadrupole deformations 
...
but these variables are not widely used. } in terms of $(\beta, \gamma)$:
%
\begin{subequations}
\begin{align}
\label{eq:beta}
\beta  &= \sqrt{\beta_{20}^2 + 2 \beta_{22}^2 } \, , \\
...
We imposed reflection symmetry at small deformation (typically $\beta_{20} < 1$) 
but accounted for the role of octupole deformation at larger elongations. 
...
($V_W$, $\lambda$, $V^{'}_W$ and $A_0$). BSkG2 relied on 25 parameters;
...
We include these quantities in the objective function to control the overall
...
chains. As discussed briefly in Sec.~\ref{sec:BSk_comparison}, current 
...
The results shown in Fig.~\ref{fig:oct} are comparable to earlier global 
 ...
 relativistic EDF~\cite{Robledo11,Agbemava2017,cao2020}. When calculations
...
intermediate-high densities of a given model. The parameterisations BSk31 and 
...
\hline
$k_F$~[fm]             &    1.329      &    1.327   & 1.327  \\
$\rho_{\sat}$~[fm$^{-3}$]      &    0.159      &    0.158   & 0.158  \\
$a_v$~[MeV]            &  -16.110      &  -16.070   & -16.082 \\
$J$~[MeV]              &   31.00       &   32.00    & 31.00    \\
$L$~[MeV]              &   53.1        &   53.0     & 55.06 \\
$M_s^*/M$              &    0.840      &    0.860   & 0.859 \\
$M_v^*/M$              &    0.730      &    0.773   & 0.722 \\
$K_v$~[MeV]            &  244.0        &  237.5     & 242.6 \\
$K_{\text{sym}}$~[MeV] &  -15.80       & -150.6    & -21.25 \\
$K^\prime$~[MeV]       &  303.0        &  376.3     &  304.2 \\
$G_0$                  &    0.37       &    0.36    & 0.20 \\
$G_0^\prime$           &    0.97       &    0.98    & 0.98 \\
\hline
\end{tabular}
%\end{ruledtabular}
\label{tab:nucmatter}
...
Table~\ref{tab:nucmatter} shows that BSkG3 values for $M_s^*/M$ and $M_v^*/M$ taken at saturation present the hierarchy $M_s^* > M_v^*$, which implies that the neutron effective mass is higher then the proton one in neutron-rich matter. This hierarchy between neutron and proton effective mass is consistent with measurements of the isovector giant dipole resonance~\cite{Lesinski06} and ab-initio calculations~\cite{Cao06,Cao06b}. In particular, the BSkG3 magnitude of the splitting at saturation density is in excellent agreement the realistic calculation of Ref.~\cite{Cao06b}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% V3 
This work adresses the extrapolation of the BSkG to high nucleon densities, 
...
of both files follow the conventions of the the supplementary files of 
