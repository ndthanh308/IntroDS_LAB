...
\item A quantum walk starting at supervertex 1 can reach supervertex $n$ in time $T$ and with probability $p$, with $T$ and $1/p$ both bounded by $\exp(O(\abs{\sum_i \log e_{2i} - \log e_{2i+1}}))\cdot \poly(n)$.
  \eit
\end{theorem}
...
\begin{definition}[Balanced hierarchical graph]
...
\begin{lemma}[Hamiltonian of hierarchical graph]
    The ``graph Hamiltonian'' $H$ is defined to be negative of the adjacency matrix of a balanced hierarchical graph.  When $H$ is projected onto the supervertex subspace $\mathcal{V} = \text{span}\left\{\ket{S_u}, u \in V\right\}$, the matrix elements of the Hamiltonian are:
\begin{equation}
...
So far we have discussed the behavior of quantum walks and classical algorithms on hierarchical graphs.  But how can such graphs be constructed? In this section, we introduce general frameworks for doing so.  On the one hand, we propose a framework based on an explicit construction utilizing factors of the graph degree $D$, which can be applied to both 1D and higher-dimensional graphs. On the other hand, we can construct hierarchical graphs by converting a Hamiltonian into a graph using sparsification.
...
random graph ensemble.  Unlike the previous sections in which the supergraph is a regular lattice, this approach will allow an arbitrary choice of supergraph.  The idea is to construct a densely connected hierarchical graph with the desired structure, and then to ``sparsify'' it, meaning to construct a lower-degree graph that has approximately the same spectrum.  The resulting graphs will no longer have an exact supervertex subspace, which allows us to generalize beyond the more rigid constraints in the previous sections.  

We will start with a set of vertices $V$, corresponding to the
supervertices in the hierarchical graph we will construct.  Start also
with desired hopping amplitudes $t_{uv}\geq 0$ for $u,v \in V$; these
correspond to the effective Hamiltonian we will obtain in the supervertex
...
We choose supervertex sizes $s_v = N \pi(v)$ where $\pi$ is a probability distribution and $N$ is the total number of vertices.   We determine $\pi$, and thereby $s$, according to the eigenvalue equation
...
\ee
Thus we can create a regular but weighted graph by adding an edge between each $(u,x)\in S_u$ and each $(v,y)\in S_y$ with weight $\frac{e_{uv}}{s_u s_v}$.  The weighted degree of a vertex $(u,x)$ is
\be
...
Let $A$ denote the weighted adjacency matrix of the hierarchical graph described above.  Each row and column of $A$ sums to $\lambda$, the entries are nonnegative with zeroes on the diagonal, and the supervertex subspace is an invariant subspace.
...
We present two approaches for sparsifying this graph.  Sparsification is necessary to make $e^{-iHt}$ efficiently implementable on a quantum computer.  The idea is to construct a lower degree graph with adjacency matrix $\tilde A$ and corresponding Hamiltonian $\tilde H = - \tilde A$.  If we run for time $T$, then $\norm{e^{-iH T} - e^{-i\tilde H T}} \leq T \norm{ H - \tilde H}$.  If the ideal Hamiltonian results in a success probability $a^2$,  then it suffices to find $\tilde A$ satisfying $\norm{A - \tilde A} \leq \frac{a}{2T}$.  We will see that this can be achieved with degrees that are polynomial in $T / a$.

{\em Poisson sparsification.}  One method for sparsification results
in graphs that are irregular, but where each vertex's degree is drawn
...
{\em Sparsifying to a regular (multi-)graph.}  Another approach to sparsifying will construct a graph with a desired degree $D$.
...
\end{itemize}
The presence of these multi-edges does not affect the convergence of $\tilde A$ to $A$ in operator norm (as we will discuss below) but it might affect the classical lower bound.  However, we want to argue that the multi-edges are sufficiently rare in the large supervertices and only prevalent inside small supervertices.  As a result, by paying only a small change in operator norm we can get rid of multi-edges in large supervertices, thereby resulting in a random graph ensemble that classical algorithms cannot traverse.  To formalize this intuition, we upper bound the expected number of multi-edges in $\widetilde{A}$.

First, we consider the case where some of the $P_i$ have one or more
...
The second source of multi-edges is from sampling.  If two different samples from $\mu$ share an edge, then their sum will have a multi-edge.  At this point, it is too late to change the distribution we're sampling from, but we can simply argue that these multi-edges are rare.

We can bound the number of multi-edges from each source by using the symmetry of $\mu$.  Suppose we sample $P_1,\ldots,P_D$ from $\mu$.  Let $\widetilde P_i \triangleq (P + P^{-1} + f(P) + f(P)^{-1})/2$.  We will argue that $\sum_i \widetilde P_i$ is $4D$-regular almost everywhere.
...
\end{blockarray}
\end{equation}
and thus $\norm{A' - A} \leq \frac{\lambda}{D}$.  All other rewirings have similar corrections with disjoint supports, so after all rewirings, $\norm{A' - A} \leq \frac{\lambda}{D}$ still holds.  (Here we use the assumption that there are no triple or higher collisions.  Of course the argument could be extended to accommodate $o(D)$ collisions if necessary.)  So long as $\lambda/D$ is much smaller than the gap away from the zero-energy eigenstate in $t$, then the quantum algorithm still remains efficient.

Since we sparsify the graph so that $D = \text{poly}(N)$ or even
$\exp(O(\sqrt{N}))$ and there are no multi-edges in large supervertices, then the analysis of our classical lower bound still holds and any classical algorithm will still require exponential time to traverse such graphs.

{\em Quantum algorithm performance on sparse graphs.}
Recall from earlier in this subsection that if $A$ is the adjacency matrix of the dense graph and $\tilde A$ is the sparse approximation and we run the algorithm for time $T$ with success probability $a^2$ then the algorithm will still succeed on the sparsified graph if $\norm{A - \tilde A } \leq \frac{a}{2T}$.  For instance, in the 1D hierarchical graph models, we had $T = \exp(O(\sqrt{N}))$ and $a = \exp(-O(\sqrt{N}))$.  Thus, when sparsifying, we need to choose the degree $D$ large enough so that the $\norm{A - \tilde A } \leq \exp(-O(\sqrt{N}))$.  For higher dimensional hierarchical graphs (where for simplicity the dimension is $O(1)$), the same statement holds with $T$ and $1/a$ both $\text{poly}(N)$.  To determine the value of $D$ to pick, we will use the Matrix Bernstein inequality, which is Theorem 1.4 of \cite{Tropp-LD}.
...
For the permutation-based sparsification above we have $d = |\mathcal{V}|$, $n=D$, $X_i = \frac{\lambda \widetilde{P_i} - A}{D}$ with $\tilde P_i = (P_i + P_i^{-1} + f(P_i) + f(P_i)^{-1})/2$.  Then $\tilde A = A + \sum_{i=1}^D X_i$ and $\bbE[X_i]=0$.  We can bound
\begin{equation}
...
If the quantum algorithm runs for time $T$ and we require $ \norm{\sum_i X_i} = \norm{A - \tilde A } \leq \frac{a}{2T}$, then we will choose $\delta = \frac{a}{2 T}$.  Substituting into the matrix Bernstein bound and assuming $D \gg \lambda$, we find that
\be
\mathbb{P}\left[\lambda_{\max}\qty(\sum_k X_k) \geq  \frac{a}{2 T}\right]
\leq |\mathcal{V}| \cdot \exp(-c \frac{a^2 D}{\lambda^2 T^2}).
  \ee
for some constant $c$.  If $D$ is large relative to $T^2\lambda^2a^{-2} \log(|\cV|)$ then most of the sparsified matrices drawn from this distribution will be close in norm to the true adjacency matrix.  As an example, when $T = \exp(O(\sqrt{N}))$ and the supergraph is a 1D line, we require that $D = N \exp(O(\sqrt{N}))$.  For higher dimensional supergraphs with $d = O(1)$, $T = N^{\gamma(d)}$ for some $\gamma(d)$ and we find that $D = O(N^{\gamma(d) + 1})$.
...
\be
\mathbb{P}[\norm{\frac{\lambda}{D} \tilde A - A}\geq \eps\lambda]
\leq
|\cV| \cdot \exp( -\frac{D^2\eps^2/2}{2D + D\eps/3})
\leq
|\cV| \cdot \exp( -cD \eps^2).
\ee
Setting $\eps = a/2T$ we find a similar  bound to the permutation-based sparsification case.

We will remark briefly on the proof technique.
To handle random sums of permutations (or more generally unitary matrices), it often suffices to use a simpler concentration method, such as the matrix Chernoff or Hoeffding bounds (see Thms 1.1-1.3 of \cite{Tropp-LD}).  These bounds would work for the permutation-based sparsification.  However, these methods often perform badly on sums of low-rank matrices, which we encounter in  Poisson sparsification.  For example, the corresponding value of $\sigma^2$ arising in the matrix Hoeffding bound would scale with $\max_v s_v$.  In this case, the matrix Bernstein inequality is necessary, and for ease of exposition, we use it in both cases.

{\em Concentration of the Poisson approximation.} 
We need to also establish two other facts about the Poisson sparsification technique.  First, for the quantum algorithm to work, we need the degree to not be too high.  It is well-known that the moment-generating function (MGF) of the Poisson distribution is
\be \bbE[e^{tX}] = e^{D\qty(e^t-1)}\ee
where $X$ is a Poisson-distributed random variable with mean $D$.  However, the degrees are not precisely Poisson distributed.  A vertex in $S_u$ has degree that is a sum of $s_v$ copies of $\text{Bern}(p_{u,v})$ for each $v\in V$.  If $X$ denotes the resulting degree then the moment generating function is
\ba
...
& \leq \exp( \sum_v s_v p_{u,v} (e^t-1)) \nonumber\\
 & =  \exp( D (e^t-1))
\ea
This distribution is thus stochastically dominated by the Poisson distribution, meaning that we can upper bound the probability of a large fluctuation by using the corresponding bound from the Poisson distribution. Taking $t=1$ we  have
\be \mathbb{P}[X \geq \alpha D]  \leq
e^{D(e-1 - \alpha)}\ee
If $D \gg \log|\cV|$ (as we have assumed above) then we can choose $\alpha=3$ and find that the probability that $X\geq 3D$ is $\ll 1/|\cV|$, meaning that with high probability no vertex has degree higher than $3D$.  Even if $D$ were smaller we could still find an efficient quantum algorithm by letting $\alpha$ scale with $\log|\cV|$.

We also need to show that vertices in large supervertices have a degree distribution that is nearly Poisson.  This will prevent the degree distribution from leaking information to the classical algorithm.  Here we can use a result of Le Cam from 1960 stating that the variational distance between the two degree distributions (for a fixed vertex in $S_u$) is $O(\max_v p_{u,v}) \ll D\delta /Q$; see Eqn.~(1.6) of \cite{Poisson-approximation}.  Our lower bounds are written from the perspective of constant-degree graphs for simplicity. However, the bounds apply equally well to graphs where the degrees of all vertices in hard supervertices is identically distributed.  This guarantees that a classical algorithm cannot gain any information from its queries to the hard vertices.  In our case, the variational distance to the Poisson distribution contributes a further source of error, but it is negligible when $D\delta/Q \ll 1$.

{\em An example.}  We briefly provide an explicit example where the sparsification procedure results in hierarchical graphs which are efficiently traversable by a quantum walk.  The example we will choose is simply a sparsification of the original welded trees graph, where the supergraph is a line graph with $2n$ nodes.  The matrix $t$ is a hopping Hamiltonian where $\mel{S_i}{t}{S_{i+1}} = \mel{S_{i+1}}{t}{S_{i}} = 1$ for $i \neq n$, $\mel{S_i}{t}{S_{i+1}} = \mel{S_{i+1}}{t}{S_{i}} = \sqrt{2}$ for $i = n$, and all other matrix elements are zero.  When we solve for the spectrum of this Hamiltonian, we find that $2n-2$ of the $2n$ eigenstates are scattering states and are delocalized.  However, the bottom and top eigenvectors are bound states \cite{Childs}.  In particular, the amplitudes of the top eigenvector $\ket{\psi}$ satisfy $\braket{j}{\psi} \propto \sinh (k j)$ for $1 \leq j \leq n$ and $\braket{j}{\psi} \propto \sinh (k (2n+1-j))$ for $n+1\leq j \leq 2n$.  The solution for $k$ satisfies
\begin{equation}
\frac{\sinh((n+1)k)}{\sinh nk} = \sqrt{2}.
\end{equation}
Therefore, since the top eigenvector is $\sqrt{\pi}$, the size of each supervertex $N \pi$ grows and shrinks like $1,2,4,\cdots,2^{n-1}, 2^n, 2^{n}, 2^{n-1},\cdots, 4,2,1$.  We connect edges in an all-to-all manner between supervertices, and place weights $1/\sqrt{s_{j} s_{j+1}}$ on all edges where $j \neq n$ and $\sqrt{2/(s_{j} s_{j+1})}$ for $j = n$.  We may then either choose to sparsify this graph using the Poisson or Birkhoff methods.

{\em Remark on symmetry.}
The random graph instances we construct {no longer} have a small Krylov subspace, although their time evolution does approximately stay within a low-dimensional subspace long enough for our algorithm.
...
\section{Quantum speedups with onsite potentials}\label{sec:Anderson}
...
\begin{conjecture}[Anderson transition]\label{cor:Anderson-transition}
...
\end{conjecture}
If this conjecture were proved, this immediately allows us to claim the following corollary:
\begin{corollary}
Assuming the Anderson transition occurs at $d\geq 3$, the class random graphs $A_d$ for $d \geq 3$ will exhibit a superpolynomial quantum-classical separation.
\end{corollary}
...
\end{equation}
where $b = \frac{1}{2}(\sqrt{5}+1)$.  The equations for the boundaries of the mobility edges are $\alpha E = 2 \text{ sgn}(\lambda) (1 - |\lambda|)$.  Although expect a superpolynomial quantum speedup in such graphs, the irrationality of the onsite potential makes an explicit construction of such a random graph ensemble challenging.
...
Fellowship under Grant No.~1745302. TL was supported by the National
