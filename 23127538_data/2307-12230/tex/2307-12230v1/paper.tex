...
Similar conclusion is made in a talk~\cite{Smirnov:2022aab}.
...
where $\h1$ is the identity operator in the one-particle subspace.
...
The second-last line is nothing but the ordinary plane-wave oscillation probability.
...
The last line exhibits the wave-packet effects: The first term in the exponent shows how the deviation from the momentum conservation is exponentially suppressed, reducing to the momentum delta function,
\al{
\sigma_\tx{red}^{d\ov2}e^{-\sigma_\tx{red}\pn{\bs P_\D-\bs P_\s}^2}
	&\to	\pi^{d\ov2}\delta^d\fn{\bs P_\D-\bs P_\s},
}
in a plane-wave limit $\sigma_\tx{red}\to\infty$. Meanwhile, the second and last terms exhibit the localization of the wave packets at $\bs L=\bar{\bs v}_IT$ and $\bar{\bs v}_JT$ for the $I$th and $J$th mass eigenstates, respectively.

\subsection{Decoherence effect}
Here we show how the decoherence effect appears in two different approaches.
Firstly, we square-complete the real part of the exponent, which represent the wave-packet effects, with respect to $\bs L$:
\al{
P\fn{\alpha\to\beta}
	&=	
		\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}
...
\subsection{Decoherence length might become shorter}
For completeness, we also write down the ultra-relativistic limit of Eq.~\eqref{L square completed} or \eqref{T partly square-completed}:
\al{
P\fn{\alpha\to\beta}
	&=
		e^{-\sigma_\tx{red}\pn{\bs P_\D-\bs P_\s}^2}
	...
	&\quad\times
		...
		\exp\Bigg[
			...
			\nn
	&\phantom{
		\approx\times\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}\exp\Bigg[
		}
			-{\pn{m_I^2+m_J^2}\pn{T-L_\parallel}T\ov2\bar P^2\sigma_\tx{sum}}
				\pn{1-{3\pn{m_I^4+m_J^4}\ov4\bar P^2\pn{m_I^2+m_J^2}}+\cdots}
			\nn
	&\phantom{
		\approx\times\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}\exp\Bigg[
		}
			-{m_I^4+m_J^4\ov8\bar P^4\sigma_\tx{sum}}T^2
			+\cdots
			\Bigg].
}
We point out that if we manage to have an experiment to adjust the timing $T$ at $L_\parallel$, the second-last line disappears, and the decoherence time from the last line becomes
\al{
...
We have obtained the (de)coherence length in several ways, all of which coincides in the ultra-relativistic limit.
...
P\fn{\alpha\to\beta}
	...
	&\quad\times\exp\!\Bigg\{
		...
		-\sigma_\tx{sum}\pn{\bar E_J-\bar E_I}^2\nn
	&\phantom{\quad\times\exp\!\Bigg\{}
		-{\bs L_\perp^2\ov\sigma_\tx{sum}}
		-{\pn{\bar v_I-\bar v_J}^2
			\sqbr{
			L_\parallel
			+i{\sigma_\tx{sum}\pn{\bar v_I+\bar v_J}\pn{\bar E_J-\bar E_I}\ov
				\pn{\bar v_I-\bar v_J}^2}
			}^2
...
T_{\fs\,IJ}
	&:=	{L_\parallel\pn{\bar v_I+\bar v_J}
				-i\sigma_\tx{sum}\pn{\bar E_J-\bar E_I}\ov\bar v_I^2+\bar v_J^2}.
}
Its further analysis will be presented elsewhere; see footnote~\ref{emergence of integral}.

In the ultra-relativistic limit, this reduces to
\al{
P\fn{\alpha\to\beta}
	&\approx
		\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}
...
P\fn{\alpha\to\alpha}
	&=	\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}
		e^{-\sigma_\tx{red}\pn{\bs P_\D-\bs P_\s}^2}
		\sum_{I,J}\ab{U_{\alpha I}}^2\ab{U_{\alpha J}}^2e^{-i\pn{\bar E_I-\bar E_J}T
			-{\bs L_\perp^2\ov\sigma_\tx{sum}}
			-{\pn{L_\parallel-{\bar v_I+\bar v_J\ov2}T}^2\ov\sigma_\tx{sum}}
			-{\pn{\bar v_I-\bar v_J}^2\ov4\sigma_\tx{sum}}T^2
				}\nn
	...
	&\quad\times\Bigg(
			e^{-{\pn{L_\parallel-\bar v_1T}^2\ov\sigma_\tx{sum}}}\cos^4\theta
			+e^{-{\pn{L_\parallel-\bar v_2T}^2\ov\sigma_\tx{sum}}}\sin^4\theta\nn
...
and from Eq.~\eqref{T partly square-completed},
\al{
P\fn{\alpha\to\alpha}
%	&=	\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}
%		e^{-\sigma_\tx{red}\pn{\bs P_\D-\bs P_\s}^2}
...
In the ultra-relativistic limit, they reduce to
\al{
P\fn{\alpha\to\alpha}
	&\approx
		\pn{\sigma_\tx{red}\ov\sigma_\tx{sum}}^{d\ov2}e^{
			-\sigma_\tx{red}\pn{\bs P_\D-\bs P_\s}^2
			-{\bs L_\perp^2\ov\sigma_\tx{sum}}
			-{\pn{L_\parallel-T}^2\ov\sigma_\tx{sum}}}\nn
	&\quad\times
	\Bigg(
			e^{
				-{m_1^2\pn{T-L_\parallel}T\ov\bar P^2\sigma_\tx{sum}}
					\pn{1-{3m_1^2\ov4\bar P^2}}
				-{m_1^2\ov4\bar P^2\sigma_\tx{sum}}T^2
				}\cos^4\theta\nn
	&\phantom{\quad\times\Bigg(}
			+e^{
				-{m_2^2\pn{T-L_\parallel}T\ov\bar P^2\sigma_\tx{sum}}
					\pn{1-{3m_2^2\ov4\bar P^2}}
				-{m_2^2\ov4\bar P^2\sigma_\tx{sum}}T^2
				}\sin^4\theta\nn
	&\phantom{\quad\times\Bigg(}
			+e^{
				-{m_1^2+m_2^2\pn{T-L_\parallel}T\ov2\bar P^2\sigma_\tx{sum}}
					\pn{1-{3\pn{m_1^2+m_2^2}\ov8\bar P^2}}
				-{m_1^2+m_2^2\ov8\bar P^2\sigma_\tx{sum}}T^2
				}
			2\cos\fnl{\pn{\bar E_1-\bar E_2}T}
					\cos^2\theta\sin^2\theta
		\Bigg).
