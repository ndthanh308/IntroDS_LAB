...
\endgroup
    ...
    \begin{lemma}\label{app-obs:v-Even-Odd-inequalities} 
        ...
        \end{align*}
        where $\rank{v} \geq_b \rank{w}$ denotes the $\geq$ relation in the lexicographic ordering, restricted to the first b elements of the tuples $\rank{v}$ and $\rank{w}$. If $\chi(v)$ is even, the inequalities are strict. 
    \end{lemma}
    \begin{proof}
        ...
        By Lem.~\ref{app-lem:intersection_of_Y_m}, $v \in Y_m^{r_l-1, 0, \ldots, r_{m-2}-1, 0, r_m}$. If we look at the flattening of this formula in Lem.\ref{app-obs:flat-Z}, $v$ is in particular, inside the middle term of this formula. That is,
         \\$v \in \B_m[Y_m^{r_l-1, \hdots, r_m-1}, Y_m^{r_l-1,\hdots, r_m }]$. If we go through the definition of this term we get,
            ...
            That gives us,
            \begin{align*}
                &\text{if } \chi(v) = m, \quad \quad \quad\quad\quad \,\,\, v \in \Cpre_\Odd(Y_m^{r_l-1, 0, \ldots, r_m-1}) \\
        ...
        \end{align*}
            ...
            Now, claim of the lemma follows from the definition of $\Cpre_\Odd$ together with the observation $\rank{v} \geq_{l+1-m} \rank{w} \iff \rank{v} \geq_{l+1-(m-1)} \rank{w}$ due to the odd indices in the ranks having $0$.
    ...
    For the second part, observe that in the first iteration of the formula, for all $j$, $Y_j = \emptyset$. Also note that $\Cpre_\Odd(\emptyset) = \emptyset$. 
    Then from formula~\ref{app-eq:fp-odd}, we can see that $Z$ does not contain any $v$ with even priority in the first iteration. 
    ...
    To see why Prop.~\ref{app-prop:cycle-through-M} holds, we need to make an observation. 
    ...
    %which is the final result of the computation. That is, 
    Since for all $j$, $Y_j^0 = \emptyset$ and $ X^0_{j-1} = V$, Lem.~\ref{app-obs:flat-Z} gives us,
    ...
    $\delta$ cannot be an \Even winning cycle: Assume $b := \max\{ \chi(w) \mid w \in \delta\} $ is even. Let $w_i\in \delta$ have priority $b$. By Obs. \ref{app-obs:v-Even-Odd-inequalities}, $\rank{w_i} >_{l+1-b} \rank{w_{i+1}} \geq_{l+1 - \chi(w_{i+1})} \ldots \geq_{l+1-\chi(w_{i-1})} \rank{w_i}$. Since for all $w_j \in \delta$, $\chi(w_{j})\leq b$, the previous inequality yields $\rank{w_i} >_{l+1-b} \rank{w_i}$, which is a contradiction.
    ...
    Since $\delta$ is minimal, Lem.~\ref{app-obs:v-Even-Odd-inequalities} gives us $\rank{w_i} \geq_{l+1 - \chi(w_i)} \rank{w_{i+1}}$ for all $w_i \in \delta$. Since $\chi(w_i) \leq \chi(w_k)$ for all $i$ and $\chi(w_k) < j$; $\rank{w_i} \geq_{l+1-j} \rank{w_{i+1}}$ for all $w_i \in \delta$. 
     ...
     \begin{proof}[Proof of Thm. \ref{prop:mainresult}]
          ...
          $\pi$ visists $M$ infinitely often. By pigeonhole principle, there exists an $x \in M$ that $\pi$ visits infinitely often. 
