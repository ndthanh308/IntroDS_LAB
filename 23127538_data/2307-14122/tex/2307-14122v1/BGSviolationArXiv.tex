...
\title{Violation of Bohigas-Giannoni-Schmit conjecture using an integrable many-body Floquet system}
...
Earlier studies have given enough evidence in support of the BGS conjecture, with few exceptions violating it. Here, we provide one more
counterexample using a many-body system popularly known as the model of quantum kicked top consisting of $N$ qubits with all-to-all 
interaction and kicking strength $k=N\pi/2$. We show that it is quantum integrable even though the corresponding semiclassical 
phase-space is chaotic, thus violating the BGS conjecture.
We solve the cases of $N=5$ to $11$ qubits analytically, finding its eigensystem, the dynamics of the entanglement, and the 
unitary evolution operator. For the general case of $N>11$ qubits, we provide numerical evidence of integrability using degenerate 
spectrum, and the exact periodic nature of the time-evolved unitary evolution operator and the entanglement dynamics.
...
{\it Introduction.---}
The celebrated Bohigas-Giannoni-Schmit (BGS) conjecture states that the energy spectrum of a classically chaotic and ergodic 
system when quantized has universal correlations described by random matrix theory (RMT) \cite{Bohigas84}.
In short, one can say that: classical chaos implies RMT. Semiclassical techniques are invoked to investigate this connection 
\cite{SemiclassicalFritzHaake2004,SemiclassicalFritzHaake2005,keating2007resummation,heusler2007periodic}. This conjecture has played an important 
role in advancing our understanding of complex systems. It includes systems from nuclear, atomic, and condensed matter physics,
billiards, periodically driven quantum systems, number theory \cite{Forresterbook,akemann2011oxford,guhr1998random,ProsenRMTkicked}, etc. 
Even some spin chains from condensed matter physics which do not necessarily have a classical limit also can be characterized 
very well using RMT \cite{keating2004random,Karthik07,TransitionThomasGuhr2020,wells2014quantum}. 

A natural question that one can ask: {\it Is there any instance where BGS conjecture is violated?}
The answer to this question is yes and there are few known instances where the BGS conjecture is shown to be violated.
They are 1) arithmetic, chaotic billiard models \cite{sarnak1993arithmetic,bogomolny1997arithmetical}  and 2) a  symmetrical 
billiard \cite{leyvraz1996anomalous}, both being single body chaotic systems;
3) the quantum modular multiplication operator \cite{shor1994algorithms,shor1999polynomial,NielsenChuang,patoary2023chaotic} 
and finally 4) the Arnold's cat map \cite{keating1991cat}.
In the first case \cite{sarnak1993arithmetic,bogomolny1997arithmetical}, the free motion on a certain constant negative curvature 
surface with finite area generated by arithmetic groups is shown to be chaotic 
but the spectral statistics are closer to that of a typical integrable system.
While in the second case  \cite{leyvraz1996anomalous}, a billiard with threefold symmetry and time-reversal invariance displays 
spectral statistics of a Gaussian unitary ensemble. But as per the BGS conjecture, it should have displayed the 
spectral statistics of Gaussian orthogonal ensemble \cite{Mehtabook,Forresterbook}. 
The third and fourth examples shows quantum integrability even though the classical limit is chaotic 
 \cite{shor1994algorithms,Ott,hannay1980quantization,keating1991cat,vikram2022dynamical,patoary2023chaotic}.
Studies have shown that the converse of this conjecture is not true as well \cite{SchumayerQuantumChaos2011,ConverseWignerDyson}. 
There is a class of spin chains for which the classical limit is known in the semiclassical limit but as of now none of them are shown 
to violate the BGS conjecture when the parameter values are such that the classical limit is fully chaotic 
\cite{pappalardi2018scrambling,Hakke87,ShohiniGhose2008,ExactDogra2019,VaibhavMadhok2015,munoz2021nonlinear,Wang04}.

In this paper, we give an example of a many-body quantum system that violates the BGS conjecture. To be specific, the classical 
limit of the many-body model under consideration and for certain parameter values is chaotic while it is still a quantum integrable system 
\cite{yuzbashyan2013quantum}. 
Its signatures can be seen in spectral statistics being Poissonian or degenerate spectrum or level crossings, exact periodicity 
of the time-evolution operator, etc. 
\cite{LevelCrossingsYuzbashyan2008,yuzbashyan2013quantum,BogomolnyDistribution2013,naik2019controlled,patoary2023chaotic}.
We consider a Floquet 
system consisting of $N$ qubits collectively modeled 
and popularly known as a quantum kicked top (QKT) \cite{Hakke87}. The Hamiltonian of the model consists of a top having spin $j=N/2$ 
rotating about an axis with a periodic kick of strength $k$. Its classical limit is known very well and shows a transition from 
regular to chaotic behavior as $k$ is increased \cite{Hakke87,Wang04,BandyoArulPRE}. The model has been implemented in various 
experimental beds for values of $j$ up to three \cite{Chaudhury09,neill2016ergodic,KrithikaKickedTop2019}. For larger values of $j$ of the order 
of $100$ use of ion traps \cite{monroe2021programmable,defenu2021long} has been proposed in the Ref.\cite{sieberer2019digital}. Previous studies 
have shown the integrability of this 
model {\it only} up to four qubits \cite{ExactDogra2019}. There, an exact analytical expression for the entanglement 
dynamics was obtained for all values of $k$. In our present work, we first analytically solve the model for the cases of five 
to eleven qubits and $k=j\pi$. We obtain the eigenvalues, eigenvectors and, entanglement and operator dynamics exactly, even though
the corresponding phase-space is chaotic, implying the integrable nature. 
We analytically show the time-periodic nature of both, the entanglement and the operator itself.
...
model for a range of parameters are considered. This is an important observation as our model falls 
in the same category of integrability class as that of these models (refer Eq.(\ref{uni})) \cite{caux2011remarks}. 
Thus, we can use these signatures to quantify integrability in our model too.

Thus, we can 
quantify signatures of integrability for 
our model too. In Refs.\cite{ArulLakshminarayan2005,naik2019controlled,mishra2015protocol,pal2018entangling}, using numerical and 
...
eigenvalues are lacking the repulsion among themselves. 
In Ref.\cite{naik2019controlled} a highly degenerate spectra is observed in the periodically driven Ising system. Thus, we can 
conclude that a system with Ising-like interaction shows quantum integrability if its spectra is degenerate; the entanglement and 
the evolution operator dynamics are time-periodic.
  
For the general case of $N>11$ qubits, we provide numerical evidence of the integrability for $k=j\pi$ 
(throughout this paper $k=j\pi$) using the degeneracy in the spectra, the periodic nature of entanglement dynamics and 
the corresponding Floquet operator itself.
To the best of our knowledge, this is the first example of a many-body quantum system that violates the BGS conjecture.

{\it Model.---}
The QKT Hamiltonian \cite{Hakke87,Haakebook} is given by 
\begin{equation}
\label{Eq:QKT}
H= \frac{p}{\tau} \, {J_y} + \frac{k}{2j}{J_z}^2 \sum_{n = -\infty}^{ \infty} \delta(t-n\tau).
\end{equation}
The first term represents a rotation and the second one is torsion applied at periodic $\delta$ kicks.
Here, $J_{x,y,z}$ are components of the angular momentum operator $\mathbf{J}$. The time between periodic kicks is $\tau$. 
The corresponding Floquet operator is as follows:
\begin{equation}
 \mathcal{U} = \exp\left [-i (k/2j \hbar) J_z^2 \right]\exp\left[-i (p/\hbar) J_y\right].
\end{equation}
It is unitary with a Hilbert space dimension of $2j+1$ and gives the evolution of states just after a kick to just after the next one.
Here, $p$ measures rotation about $y$ axis, and we set $p=\pi/2$ and $\hbar=1$.
Value of $k$ controls the degree of chaos in the classical limit, where $k=0$ gives integrable nature while for $k>0$
it becomes increasingly chaotic.
When $j\rightarrow\infty$, the classical limit is a map on the unit sphere and at the $i$th iteration is as follows:
\begin{eqnarray}
 X_{i}&=&Z_{i-1}\cos(k X_{i-1})+Y_{i-1}\sin (k X_{i-1}),\nonumber \\
 Y_{i}&=&-Z_{i-1}\sin(k X_{i-1})+Y_{i-1}\cos (kX_{i-1}),\nonumber \\
 Z_{i}&=&-X_{i-1}.
\end{eqnarray}
It's clear that for $k=0$ the map is integrable while as $k$ increases the phase space becomes more and more chaotic
\cite{Hakke87,Haakebook}. For given $j$, the top can be decomposed in $N=2j$ spin-half particles \cite{milburn1999simulating,Wang04}. 
Thus, this model is connected to a many-qubit system via the transformation $J_{x,y,z}=\sum_{l=1}^{2j}\sigma_l^{x,y,z}/2$,
where $\sigma_l^{x,y,z}/2$ are the standard Pauli matrices. 
The Floquet operator then simplifies to:
\begin{equation}
\label{uni}
{\mathcal U}=\exp\left(-i \frac{k}{4j}  \sum_{ l< l'=1}^{2j} \sigma^z_{l} \sigma^z_{l'}\right)
\exp\left( -i \frac{\pi}{4} \sum_{l=1}^{2j}\sigma^y_l \right).
\end{equation}
It is an infinite-range or all-to-all interaction periodically kicked spin model with Ising interaction very much similar to the spin 
glasses without kicking \cite{kirkpatrick1978infinite,erzan1983infinite,thirumalai1989infinite,milotti2001exactly,milotti2002exactly}.
Various entanglement content has been studied in this model 
\cite{Arjendu2017,UdaysinhPeriodicity2018,Udaysinhbifurcation2017,ShohiniGhose2008,ExactDogra2019,Lombardi2011}.
...
\begin{equation}
 |\theta_0,\phi_0\rangle = \otimes^{2j} \left(\cos(\theta_0/2) |0\rangle + e^{-i \phi_0} \sin(\theta_0/2) |1\rangle\right).
\end{equation}
...
and four qubits in Ref.\cite{ExactDogra2019}. The general basis for the odd number of qubits is: 
\begin{equation}\label{Eq:oddBasis1}
\ket{\phi_q^{\pm}}=\frac{1}{\sqrt{2}}\left(\ket{w_q}\pm {i^{\left(N-2q\right)}} \ket{\overline{w_q}}\right),
0\leq q\leq \dfrac{2j-1}{2}.
\end{equation}
 For the even case:
\begin{eqnarray}\label{Eq:evenBasis1}
\begin{split}
\ket{\phi_r^{\pm}}&=\frac{1}{\sqrt{2}}\left(\ket{w_r}\pm {(-1)^{\left(j-r\right)}} \ket{\overline{w_r}}\right), 0\leq r\leq j-1\\
\mbox{and} &\;\;
\ket{\phi_j^+}=\left({1}/{\sqrt{\binom{N}{j}}}\right)\sum_{\mathcal{P}}\left(\otimes^{j}\ket{0}\otimes^{j}\ket{1}\right)\mathcal{P}
\end{split}
\end{eqnarray}
  where 
...
These basis states are parity symmetric and follows $\otimes_{l=1}^{2j}\sigma_l^y\ket{\phi_j^{\pm}}=\pm\ket{\phi_j^{\pm}}$.
In this basis, $\mathcal U$ becomes block-diagonal simplifying further analysis. We derive entanglement dynamics at $k=j\pi$ and for 
the coherent states $|\theta_0=0,\phi_0=0\rangle$ and $|\theta_0=\pi/2,\phi_0=-\pi/2\rangle$. Here, the first state is on the 
period-4 orbit whereas the second one is on the fixed point in the classical phase space. In terms of qubit representation, 
these coherent states are given by $\otimes^{N}\ket{0}$ and $\otimes^{N}\ket{+}$ respectively \cite{ExactDogra2019}.
...
\begin{equation}
  \mathcal{U}_{\pm}= \frac{1}{4} e^{\mp{\frac{ i \pi }{4}}}
\begin{pmatrix}
 e^{-i k} & \mp i\sqrt{5} \text{  }e^{-i k} &  \sqrt{10}\text{  } e^{-i k} \\
\pm i\sqrt{5}\text{  }e^{-\frac{i k}{5}} & -3\text{  } e^{-\frac{i k}{5}} & \pm i\sqrt{2}\text{  } e^{-\frac{i k}{5}} \\
  \sqrt{10}\text{  } e^{\frac{i k}{5}} & \mp i\sqrt{2}\text{  } e^{\frac{i k}{5}} & - 2\text{  }e^{\frac{i k}{5}} \\
\end{pmatrix}.
\end{equation}
At $k=5\pi/2$, the eigenvalues  of $\mathcal{U_+}$ and $\mathcal{U_-} $ are 
...
In the context of QKT time-periodicity has been reported earlier for $j=1$ \cite{UdaysinhPeriodicity2018};
$j=3/2$ and $j=2$ for special values of $k$ \cite{ExactDogra2019}.
% UdaysinhPeriodicity2018
...
same figure \cite{supplementry2023}. This shows that the entanglement is of multipartite nature.\\ 
...
But obtaining a general solution as a function of $N$ is mathematically challenging. Thus, we resort to numerical simulations and 
find various signatures of integrability for $N>11$. 
For this purpose we use the same signatures obtained for $N=5$ to $11$ and claim integrability for any $N>11$ and $k=N\pi/2$.
To our surprise, we find the same signatures (depending only on the parity of $N$) as shown in the 
Fig. \ref{fig:correlations1}. Similar $N$ independence was observed in the Ref.\cite{ArulLakshminarayan2005} for the case of the integrable
kicked-Ising model.
...
$\delta(n)=\sum_{p,q}|{\mathcal U}^n_{p,q}-{\mathcal U}_{p,q}|$. It is zero for any $n>1$ if and only if ${\mathcal U}^n={\mathcal U}$,
thus confirming the time-periodic nature of $\mathcal{U}$. The results are plotted in Fig.~\ref{fig:periodicUnitary} for even $N$. 
We find that for 
$k=j\pi$ and $j$ upto $200$, time evolution of $\mathcal U$ is periodic (checked for $n$ as large as $5000$), whereas for $k\neq j\pi$ 
it isn't. For odd $N$ (results not plotted 
...
for $N=5$, $7$, $9$, and $11$. Another signature of integrability is found from the eigenangle spectrum of $\mathcal U$ at $k=j\pi$. 
We find it to be highly degenerate taking values from the set $\{0,\pm \pi/4, \pm \pi/2, \pm 3 \pi/4, \pm \pi \}$ for even $N$ 
(checked for $j$ upto $200$, refer Fig.~\ref{fig:degenracy}). Similarly, for odd $N$, we numerically find a degenerate spectrum. 
...
and the kicking strength such that $k=N\pi/2$.\\
{\it Conclusions.---}
...
We have shown that the QKT model shows integrability for any number of qubits and the special value of kicking strength.
It shows quantum integrability even though the phase space in the semiclassical limit is chaotic and thus violates the BGS conjecture.
Previous to our work, it was believed to be quantum chaotic for the special parameter considered here using the BGS conjecture.
...
$N>=11$. Previous work as far as integrability is concerned was limited only up to $4$ qubits.
We have now generalized it to any $N$. Our work can be studied further on the lines of recent work in the 
Ref.\cite{patoary2023chaotic}. In Ref.\cite{patoary2023chaotic}, Shor's quantum modular multiplication operator is studied 
which is quantum integrable but has a semiclassical limit to be chaotic, thus violating the BGS conjecture.
There, it is shown analytically that the operator can be expressed as a superposition of quantized A-baker's maps,
where these maps shows the signatures of quantum chaos and ergodicity. Our results (for the smaller number of qubits) can be 
verified experimentally in various setups from NMR \cite{Krithika2019}, superconducting qubits \cite{neill2016ergodic} and laser 
cooled atoms \cite{Chaudhury09}, where QKT is implemented readily. Whereas for the larger number of qubits (of the order of $100$s), 
one can use ion traps \cite{monroe2021programmable,defenu2021long}. Our conjecture can be tested in this setup as well. This is the 
first example of an integrable many-body Floquet spin system with a chaotic classical
limit and which violates the BGS conjecture. With our findings, a search for similar spin systems, if any, with classically chaotic 
phase space can be initiated which are otherwise considered to be quantum chaotic due to the BGS conjecture.

 
\bibliography{references,reference22,reference221,reference22013}
...
Eq. (\ref{uni}), of the main text, unitary operator acting on a system of $5$-qubits, that simulates the dynamics of a spin-$5/2$
under a kicked top Hamiltonian is given by, 
\begin{eqnarray}\label{Eq:5q87}
 \mathcal{U} &=& \exp\left[{-i \frac{k}{10} (\sigma_1^z \sigma_2^z +\sigma_1^z \sigma_3^z +\sigma_1^z \sigma_4^z+\sigma_1^z \sigma_5^z+\sigma_2^z \sigma_3^z+\sigma_2^z \sigma_4^z+\sigma_2^z \sigma_5^z+\sigma_3^z \sigma_4^z+\sigma_3^z \sigma_5^z+\sigma_4^z \sigma_5^z)} \right]\times
 \nonumber \\
 &&  \exp \left[{-i \frac{\pi}{4}(\sigma_1^y+\sigma_2^y+\sigma_3^y+\sigma_4^y+\sigma_5^y)} \right], 
\end{eqnarray}
where $k$ is the chaoticity parameter and $\sigma_l^z$ and $\sigma_l^y$  are  Pauli operators. The solution to the 5-qubit case proceeds from the general observation that,
\begin{equation}
[\mathcal{U},\otimes_{l=1}^{2j} \sigma^y_l]=0,
\end{equation} 
 {\it i.e.,} there is
an ``up-down" or parity symmetry. Here, we are confined to $(2j+1=6)$ dimensional permutation symmetric subspace of the total 
$2^{2j}$= $2^{5}$= $32$ dimensional Hilbert space. The basis vectors are given as follows:
\begin{eqnarray}
...
\end{eqnarray}
We calculated the linear entropy at a particular value of kicking strength $k=j\pi$, where $j=5/2$ in which we find that it attains the maximum upper bound value i.e. $1/2$.
From the Eqs.~(\ref{Eq:Ap5Q1}) and (\ref{Eq:Ap5Q2}), we can see that its entanglement dynamics is periodic in nature with $S_{(0,0)}^{(5)}(n)=S_{(0,0)}^{(5)}(n+6)$ and the   linear entropy for consecutive odd \mbox{and} even values of $n$ are same (Fig.(\ref{fig:correlations1})) i.e.
\begin{equation}
...
the standard ($\sigma_z$) basis. For even time $n=2m$, the two-qubit RDM $\rho_{12}(2m)$ is an ``$X$ state" \cite{Yu2005EvolutionFE} given as follows:
\begin{equation}
\rho_{12}(2m) ={\left(
\begin{array}{cccc}
 a_n & 0 & 0 & b_n \\
...
see that the concurrence remains zero for every value of $n$. This is shown in the Fig.(\ref{fig:correlations1}).
...
\end{equation} 
 ...
 This is shown in the Fig.(\ref{fig:correlations1}).
\subsubsection{\bf{Concurrence}}
...
% As we discussed earlier due to the periodic nature of the matrix and coefficients its eigenvalue also shows periodic nature having the same periodicity, which we can see by using the simplified eigenvalues of $(\sigma_y \otimes \sigma_y)\rho_{12} (\sigma_y \otimes \sigma_y) \rho_{12}^*$ for  $n$ going from $0$ to $6$  are $\left\lbrace(0, 0, 0, 0), \left(1/4, 1/4, 0, 0\right),\left(1/4, 1/4, 0, 0\right),(0, 0, 0, 0),\left(1/4, 1/4, 0, 0\right),\left(1/4, 1/4, 0, 0\right), (0, 0, 0, 0)\right\rbrace$. These eigenvalues repeat after $n\rightarrow n+3$. By using these  $\lambda_j$ in Eq.~(\ref{Eq:Ap5Q3}) we can easily see that the concurrence vanishes  for each value of $n$, which is shown in  Fig~(\ref{}).
 ...
 vanishes  for each value of $n$. This is shown in the Fig.(\ref{fig:correlations1}).
 
\section{Exact analytical solution for six-qubits case}
Consider a spin-$3$ system, which can be effectively simulated by a six-qubit system confined into a seven-dimensional symmetric subspace. The eigenbasis of operator $\otimes_{l=1}^6\sigma_y$ in which $\mathcal{U}$  is block diagonal are given below:
\begin{eqnarray}
...
$\ket{\overline{w_0}}=\ket{111111}$, $\ket{w_1}=\frac{1}{\sqrt{6}} \sum_\mathcal{P} \ket{000001}_\mathcal{P}$, $\ket{\overline {w_1}}=\frac{1}{\sqrt{6}} \sum_\mathcal{P} \ket{011111}_\mathcal{P}$, $\ket{w_2}=\frac{1}{\sqrt{15}} \sum_\mathcal{P} \ket{000011}_\mathcal{P}$ and $\ket{\overline{w_2}}=\frac{1}{\sqrt{15}} \sum_\mathcal{P} \ket{001111}_\mathcal{P}$. The unitary operator $\mathcal{U}$ in this basis are diagonal in $\mathcal{U_+} \left( \mathcal{U_-}\right)$  having dimensions $4\times 4\left(3\times3\right)$ respectively. They are given as follows:
 \begin{equation}
\mathcal{U_+}= \frac{1}{2\sqrt{2}}\left(
\begin{array}{cccc}
 0 & -\sqrt{3}\text{  }e^{-\frac{5 i k}{4}} & 0 & -\sqrt{5}\text{  }e^{-\frac{5 i k}{4}} \\
\sqrt{3}\text{  } e^{-\frac{5 i k}{12}} & 0 & \sqrt{5}\text{  } e^{-\frac{5 i k}{12}} & 0 \\
 0 & -\sqrt{5}\text{  } e^{\frac{i k}{12}} & 0 & \sqrt{3}\text{  } e^{\frac{i k}{12}} \\
 \sqrt{5}\text{  } e^{\frac{i k}{4}} & 0 & -\sqrt{3}\text{  } e^{\frac{i k}{4}} & 0 \\
\end{array}
\right)\mbox{and}
 \end{equation}
 \begin{equation}
\mathcal{U_-}= \frac{1}{4}{\left(
\begin{array}{ccc}
  e^{-\frac{5 i k}{4}} & 0 &  \sqrt{15} e^{-\frac{5 i k}{4}} \\
 0 & -4~e^{-\frac{5 i k}{12}} & 0 \\
 \sqrt{15} e^{\frac{i k}{12}} & 0 & - e^{\frac{i k}{12}} \\
\end{array}
\right)}.
 \end{equation}
For $k=3\pi$, the eigenvalues of the $\mathcal{U_+} \left( \mathcal{U_-}\right)$  is ${\left\{-1,-1,1,1\right\}}\left(\left\{-(-1)^{1/4},-(-1)^{3/4},(-1)^{1/4}\right\}\right)$  and the eigenvectors are  $\left\lbrace\left[\frac{2+2 i}{\sqrt{5}},\sqrt{\frac{3}{5}},0,1\right]^T, \left[\sqrt{\frac{3}{5}},\frac{2-2 i}{\sqrt{5}},1,0\right]^T, \left[-\frac{2+2 i}{\sqrt{5}},\sqrt{\frac{3}{5}},0,1\right]^T, \left[\sqrt{\frac{3}{5}},-\frac{2-2 i}{\sqrt{5}},1,0\right]^T\right\rbrace\left(\left\lbrace\left[-\sqrt{\frac{3}{5}},0,1\right]^T, \left[0,1,0 \right]^T, \left[\sqrt{\frac{5}{3}},0,1\right]^T\right\rbrace\right)$. The $n$th time evolution of the  blocks $\mathcal{U_{\pm}}$ is given as follows:
 \begin{equation}
...
\end{eqnarray}
By observing Fig.(\ref{fig:correlations1}) and Eq.~(\ref{Eq:Ap5Q4}), we can see that the entanglement dynamics of the system exhibit a periodic pattern. This periodicity is expressed as $S_{(0,0)}^{(6)}(n) = S_{(0,0)}^{(6)}(n+4)$. We also find that the values of linear entropy for consecutive odd \mbox{and} even values of $n$  are identical. It attains the maximum upper bound value i.e. $1/2$.
\subsubsection{\bf{Concurrence}}
...
$n$ and zero for even values of $n$. This is shown in the Fig.(\ref{fig:correlations1}). 
\subsubsection{\bf{Concurrence}}
...
For a spin-$7/2$ the permutation symmetric basis is given as follows:
\begin{eqnarray}
...
\begin{equation}
\mathcal{U}_{\pm} ={\frac{e^{{\pm}\frac{i \pi }{4}}}{8}\left(
\begin{array}{cccc}
  e^{-\frac{3 i \text{k}}{2}} & {\pm} i \sqrt{7} e^{-\frac{3 i \text{k}}{2}} &  \sqrt{21} e^{-\frac{3 i \text{k}}{2}} &{\pm} i \sqrt{35} e^{-\frac{3 i \text{k}}{2}}
\\
 {\mp}i \sqrt{7} e^{-\frac{9 i \text{k}}{14}} & - 5 e^{-\frac{9 i \text{k}}{14}} & {\mp} 3i\sqrt{3} e^{-\frac{9 i \text{k}}{14}} & - \sqrt{5} e^{-\frac{9
i \text{k}}{14}} \\
  \sqrt{21} e^{-\frac{i \text{k}}{14}} & {\pm} 3 i \sqrt{3} e^{-\frac{i \text{k}}{14}} &  e^{-\frac{i \text{k}}{14}} & {\mp}i\sqrt{15} e^{-\frac{i \text{k}}{14}}
\\
 {\mp}i \sqrt{35}  e^{\frac{3 i \text{k}}{14}} & - \sqrt{5} e^{\frac{3 i \text{k}}{14}} & {\pm} i \sqrt{15} e^{\frac{3 i \text{k}}{14}} & 3 e^{\frac{3 i
\text{k}}{14}} \\
\end{array}
\right)}.
\end{equation}
For $k={7\pi}/{2}$, the eigenvalues for $\mathcal{U_{+}}$($\mathcal{U_{-}}$) are 
...
\end{eqnarray}
 ...
 the linear entropy for consecutive odd \mbox{and} even values of $n$  remains same (which is shown in the Fig.(\ref{fig:correlations1})) 
 i.e.
\begin{equation}
S_{(0,0)}^{(7)}(2m-1)=S_{(0,0)}^{(7)}(2m).
\end{equation} 
\subsubsection{\bf{Concurrence} }
For even $n=2m$, the two qubit RDM, $\rho_{12}(n)$, is an ``$X$ state" \cite{Yu2005EvolutionFE} and is  given as follows:
\begin{equation}
\rho_{12}(2m)={\left(
\begin{array}{cccc}
 s_n& 0 & 0 & y_n \\
...
S_{(\pi/2,-\pi/2)}^{(7)}(n+3)$. This is shown in the Fig.(\ref{fig:correlations1}). 
...
\mathcal{U}_{+}={\left(
\begin{array}{ccccc}
 \frac{1}{8} e^{-\frac{7 i k}{4}} & 0 & \frac{1}{4} \sqrt{7} e^{-\frac{7 i k}{4}} & 0 & \frac{1}{8} \sqrt{35} e^{-\frac{7 i k}{4}} \\
 0 & -\frac{3}{4} e^{-\frac{7 i k}{8}} & 0 & -\frac{1}{4} \sqrt{7} e^{-\frac{7 i k}{8}} & 0 \\
 \frac{1}{4} \sqrt{7} e^{-\frac{i k}{4}} & 0 & \frac{1}{2} e^{-\frac{i k}{4}} & 0 & -\frac{1}{4} \sqrt{5} e^{-\frac{i k}{4}} \\
 0 & -\frac{1}{4} \sqrt{7} e^{\frac{i k}{8}} & 0 & \frac{3}{4} e^{\frac{i k}{8}} & 0 \\
 \frac{1}{8} \sqrt{35} e^{\frac{i k}{4}} & 0 & -\frac{1}{4} \sqrt{5} e^{\frac{i k}{4}} & 0 & \frac{3}{8} e^{\frac{i k}{4}} \\
\end{array}
\right)}~ \mbox{and}
\end{equation}
\begin{equation}
\mathcal{U}_{-}={\left(
\begin{array}{cccc}
 0 & -\frac{e^{-\frac{7 i k}{4}}}{2 \sqrt{2}} & 0 & -\frac{1}{2} \sqrt{\frac{7}{2}} e^{-\frac{7 i k}{4}} \\
 \frac{e^{-\frac{7 i k}{8}}}{2 \sqrt{2}} & 0 & \frac{1}{2} \sqrt{\frac{7}{2}} e^{-\frac{7 i k}{8}} & 0 \\
 0 & -\frac{1}{2} \sqrt{\frac{7}{2}} e^{-\frac{i k}{4}} & 0 & \frac{e^{-\frac{i k}{4}}}{2 \sqrt{2}} \\
 \frac{1}{2} \sqrt{\frac{7}{2}} e^{\frac{i k}{8}} & 0 & -\frac{e^{\frac{i k}{8}}}{2 \sqrt{2}} & 0 \\
\end{array}
\right)}.
\end{equation}
For $k={4\pi}$ the eigenvalues of $\mathcal{U_{+}}$ $\left(\mathcal{U_{-}}\right)$ are $\left\lbrace-1,-1,i,-i,1\right\rbrace$ $\left(e^{\frac{i \pi }{4}}\left\lbrace-1,-1,1,1\right\rbrace\right)$ and the eigenvectors are\\
...
remain the same. This is shown in the Fig.(\ref{fig:correlations1}).
\subsubsection{\bf{Concurrence}}
...
we can easily see that the concurrence remains  zero for every value of $n$. This is shown in the Fig.(\ref{fig:correlations1}).
...
$S_{(\pi/2,-\pi/2)}^{(8)}(n)=S_{(\pi/2,-\pi/2)}^{(8)}(n+2)$. This is shown in the Fig.(\ref{fig:correlations1}).
\subsubsection{\bf{Concurrence}}
 The two qubit RDM, $\rho_{12}(n)$, for this state is given as follows:
\begin{equation}
...
\end{equation}
 ...
 concurrence remains zero for every value of $n$. This is shown in the Fig.(\ref{fig:correlations1}).
...
\begin{array}{ccccc}
 { e^{-2 i k}} & \mp {3i~ e^{-2 i k}} & {6} e^{-2 i k} & \mp{2i} \sqrt{21} e^{-2 i k} & {3} \sqrt{7} e^{-2 i
k} \\
 \pm 3 i~e^{-\frac{10 i k}{9}} & -{7} e^{-\frac{10 i k}{9}} & \pm {10 i} e^{-\frac{10 i k}{9}} & -2\sqrt{21} e^{-\frac{10i k}{9}} & \pm {i}\sqrt{7} e^{-\frac{10 i k}{9}} \\
 6 e^{-\frac{4 i k}{9}} & \mp {10 i}e^{-\frac{4 i k}{9}} & 8 e^{-\frac{4 i k}{9}} & 0 & -2{\sqrt{7}}\text{  }e^{-\frac{4i k}{9}} \\
\pm {2i} \sqrt{21} & - 2\sqrt{21} & 0 & 8 & \mp {2i}\sqrt{3} \\
 3\sqrt{7} e^{\frac{2 i k}{9}} & \mp i\sqrt{7} e^{\frac{2 i k}{9}} & -2{\sqrt{7}} e^{\frac{2 i k}{9}} & \pm {2i} \sqrt{3} e^{\frac{2 i k}{9}} & {6} e^{\frac{2 i k}{9}} \\
\end{array}
\right).
\end{equation}
For $k={9\pi}/{2}$, the eigenvalues are $\mathcal{U_{+}}\left(\mathcal{U_{-}}\right)$ is given as ${e^{\frac{3i \pi }{4}}\left\{1,e^{-\frac{2i \pi }{3}},e^{-\frac{2i \pi }{3}},e^{\frac{2i \pi }{3}},e^{\frac{2i \pi }{3}}\right\}}\left(e^{\frac{i \pi }{4}}\left\{-1,e^{\frac{i \pi }{3}},e^{\frac{i \pi }{3}},e^{-\frac{i \pi }{3}},e^{-\frac{i \pi }{3}}\right\}\right)$
...
$1/2$ which is shown in Fig.(\ref{fig:correlations1}).
\subsubsection{\bf{Concurrence}}
...
Considering a spin-$5$ system that is confined in an eleven-dimensional symmetric subspace. The eigenbasis  in which the unitary operator becomes block diagonal is given as follows:
\begin{eqnarray}
\ket{\phi_0^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_0} \mp \ket{\overline{w_0}}),\\
\ket{\phi_1^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_1} \pm  \ket{\overline{w_1}}),\\
\ket{\phi_2^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_2} \mp  \ket{\overline{w_2}}),\\
...
\begin{array}{cccccc}
 0 & - \sqrt{{5}}~ e^{-\frac{9 i k}{4}} & 0 & -2 \sqrt{{15}}~ e^{-\frac{9 i k}{4}} & 0 & -{3}\sqrt{{7}}~
e^{-\frac{9 i k}{4}} \\
  \sqrt{{5}} ~e^{-\frac{27 i k}{20}} & 0 & {9~ e^{-\frac{27 i k}{20}}} & 0 &  \sqrt{42} ~e^{-\frac{27i k}{20}} & 0 \\
 0 & -{9~ e^{-\frac{13 i k}{20}}} & 0 & -2\sqrt{{3}}~ e^{-\frac{13 i k}{20}} & 0 &  \sqrt{{35}}
~e^{-\frac{13 i k}{20}} \\
 2 \sqrt{{15}}~ e^{-\frac{3 i k}{20}} & 0 &  2\sqrt{3} e^{-\frac{3 i k}{20}} & 0 & - 2\sqrt{14}~ e^{-\frac{3
i k}{20}} & 0 \\
 0 & -\sqrt{42}~ e^{\frac{3 i k}{20}} & 0 & 2\sqrt{14}~ e^{\frac{3 i k}{20}} & 0 & - \sqrt{30}~ e^{\frac{3 i k}{20}}
\\
 {3}\sqrt{{7}}~ e^{\frac{i k}{4}} & 0 & - \sqrt{{35}}~ e^{\frac{i k}{4}} & 0 &  \sqrt{30}~ e^{\frac{i
k}{4}} & 0 \\
\end{array}
\right),
\end{equation}
...
\begin{array}{ccccc}
 e^{-\frac{9 i k}{4}} & 0 & {3} \sqrt{5}~ e^{-\frac{9 i k}{4}} & 0 &  \sqrt{{210}}~ e^{-\frac{9 i k}{4}} \\
 0 & -8~ e^{-\frac{27 i k}{20}} & 0 & -8 \sqrt{3}~ e^{-\frac{27 i k}{20}} & 0 \\
 {3} \sqrt{5}~ e^{-\frac{13 i k}{20}} & 0 & {13}~ e^{-\frac{13 i k}{20}} & 0 & - \sqrt{{42}}~ e^{-\frac{13 i k}{20}}
\\
 0 & -8 \sqrt{3} ~e^{-\frac{3 i k}{20}} & 0 & 8~ e^{-\frac{3 i k}{20}} & 0 \\
 \sqrt{{210}}~ e^{\frac{3 i k}{20}} & 0 & -\sqrt{{42}}~ e^{\frac{3 i k}{20}} & 0 & 2~ e^{\frac{3 i
k}{20}} \\
\end{array}
\right)}.
\end{equation}
For $k={5\pi}$, the eigenvalues for $\mathcal{U_{+}}\left(\mathcal{U_{-}}\right)$ are ${\{i,i,i,-i,-i,-i\}}\left(\left\{(-1)^{1/4},-(-1)^{3/4},(-1)^{3/4},(-1)^{3/4},-(-1)^{1/4}\right\}\right)$\\ and the eigenvectors are $\left\lbrace\left[-\frac{1+i}{\sqrt{7}},-\sqrt{\frac{5}{7}},(1+i) \sqrt{\frac{5}{7}},0,0,1 \right]^T,\left[\sqrt{\frac{5}{42}},(-4+4 i) \sqrt{\frac{2}{21}},3 \sqrt{\frac{3}{14}},0,1,0 \right]^T,\right.$\\ $\left. \left[\left(-\frac{1}{2}-\frac{i}{2}\right) \sqrt{\frac{5}{3}},\frac{2}{\sqrt{3}},\left(-\frac{1}{2}-\frac{i}{2}\right) \sqrt{3},1,0,0 \right]^T,
...
Considering a spin-$11/2$ system which is confined in a twelve-dimensional symmetric subspace. The eigenbasis  in which the unitary operator becomes block diagonal is given as follows:
\begin{eqnarray}
\ket{\phi_0^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_0} \mp i \ket{\overline{w_0}}),\\
\ket{\phi_1^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_1} \pm i \ket{\overline{w_1}}),\\
\ket{\phi_2^{\pm}}&=&\frac{1}{\sqrt{2}}(\ket{w_2} \mp i \ket{\overline{w_2}}),\\
...
\begin{equation}
\mathcal{U}_{\pm}={\frac{e^{\pm  \frac{i \pi }{4}}}{32}} \left(
\begin{array}{cccccc}
  e^{-\frac{5 i k}{2}} & \pm i\sqrt{11} e^{-\frac{5 i k}{2}} & \sqrt{55} e^{-\frac{5 i k}{2}} & \pm i\sqrt{165} e^{-\frac{5 i k}{2}} &  \sqrt{330} e^{-\frac{5
i k}{2}} & \pm  i\sqrt{462} e^{-\frac{5 i k}{2}} \\
  \mp i \sqrt{11} e^{-\frac{35 i k}{22}} & - 9 e^{-\frac{35 i k}{22}} & \mp 7i \sqrt{5} e^{-\frac{35 i k}{22}} & -5 \sqrt{15} e^{-\frac{35 i k}{22}} &
\mp 3i\sqrt{15} e^{-\frac{35 i k}{22}} & -\sqrt{42} e^{-\frac{35 i k}{22}} \\
 \sqrt{55} e^{-\frac{19 i k}{22}} & \pm 7 i \sqrt{5} e^{-\frac{19 i k}{22}} & 19 e^{-\frac{19 i k}{22}} & \pm 7i \sqrt{3} e^{-\frac{19 i k}{22}} &
-\sqrt{6} e^{-\frac{19 i k}{22}} & \mp i\sqrt{210} e^{-\frac{19 i k}{22}} \\
  \mp i \sqrt{165} e^{-\frac{7 i k}{22}} & - 5 \sqrt{15} e^{-\frac{7 i k}{22}} & \mp 7i \sqrt{3} e^{-\frac{7 i k}{22}} & 5 e^{-\frac{7 i k}{22}} & \pm 11 i\sqrt{2}
e^{-\frac{7 i k}{22}} & \sqrt{70} e^{-\frac{7 i k}{22}} \\
  \sqrt{330} e^{\frac{i k}{22}} &\pm  3 i\sqrt{15} e^{\frac{i k}{22}} & -\sqrt{6} e^{\frac{i k}{22}} & \mp 11i\sqrt{2}e^{\frac{i k}{22}} & - 6 e^{\frac{i
k}{22}} & \pm 2i \sqrt{35} e^{\frac{i k}{22}} \\
 \mp i \sqrt{462} e^{\frac{5 i k}{22}} & -\sqrt{42} e^{\frac{5 i k}{22}} &  \pm i \sqrt{210} e^{\frac{5 i k}{22}} & \sqrt{70} e^{\frac{5 i k}{22}} &  \mp 2i
\sqrt{35} e^{\frac{5 i k}{22}} & -10 e^{\frac{5 i k}{22}} \\
\end{array}
\right).
\end{equation}
For $k={11\pi}/{2}$, the eigenvalues for $\mathcal{U_{+}}\left(\mathcal{U_{-}}\right)$ is given as ${\left\{i\left(-1,-1,e^{\frac{i \pi }{3}},e^{\frac{i \pi }{3}},e^{-\frac{i \pi }{3}},e^{-\frac{i \pi }{3}}\right)\right\}}\left(\left\{-1,-1,e^{\frac{i \pi }{3}},e^{\frac{i \pi }{3}},e^{-\frac{i \pi }{3}},e^{-\frac{i \pi }{3}}\right\}\right)$ and eigenvectors are $\left\lbrace\left[0,-\sqrt{\frac{7}{6}},\mp i \sqrt{\frac{35}{6}},0,0,1 \right]^T,\left[-\sqrt{\frac{15}{22}},0,0,\mp \frac{i}{\sqrt{2}},1,0 \right]^T,\left[\pm\sqrt{\frac{11}{14}},-2 \sqrt{\frac{2}{21}},\pm i \sqrt{\frac{10}{21}},-i \sqrt{\frac{15}{14}},0,1 \right]^T \right.$\\$\left.,\left[0,\pm\sqrt{\frac{5}{2}},-\frac{i}{\sqrt{2}},\pm i \sqrt{2},1,0 \right]^T,\left[\mp\sqrt{\frac{11}{14}},-2 \sqrt{\frac{2}{21}},\pm i \sqrt{\frac{10}{21}},i \sqrt{\frac{15}{14}},0,1 \right]^T,\left[0,\mp\sqrt{\frac{5}{2}},\frac{i}{\sqrt{2}},\pm i \sqrt{2},1,0 \right]^T\right\rbrace$. The $n$th time evolution of $\mathcal{U}_{\pm}$ is given as follows:
\begin{equation}
...
values and its entanglement dynamics is periodic in nature having periodicity $6$. This is shown in the Fig.(\ref{fig:correlations1}).
\subsubsection{\bf{Concurrence }}
...
we found that its concurrence vanishes for all $n$. This is shown in the Fig.(\ref{fig:correlations1}).
