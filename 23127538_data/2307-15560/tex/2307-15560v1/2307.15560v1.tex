...
\begin{equation}
\tilde J_k(t) = \frac{1}{N \, R^n} \sum_{k=1}^N K \Big( \frac{|X_k(t) - X_\ell(t)|}{R} \Big) \, A_\ell(t), 
...
%The model would still make sense without the projection operator ${\mathcal P}$, namely if \eqref{eq:part_dAkdt} was replaced by 
%$$ d A_k = \nu \, P_{T_{A_k}} \tilde J_k \, dt + \sqrt{2D} \, P_{T_{A_k}} \circ dW_t^k, $$ and would lead to \eqref{eq:mf_kin} replaced by 
%$$ \partial_t f + c_0 A {\mathbf e}_1 \cdot \nabla_x f + \nu \nabla_A \cdot \big( P_{T_A} (\tilde J_f) f \big) = D \Delta_A f. $$
%This setting would then be the analog of that considered in \cite{bolley2012mean} for the Vicsek model. If we assume that their result applies to the body-orientation case, we obtain that the random measure 
%$$ \mu^N(x,A,t) = \frac{1}{N} \sum_{k=1}^N \delta_{(X_k(t), A_k(t))} (x,A), $$
%where $\delta_{(X_k(t), A_k(t))} (x,A)$ is the Dirac delta in ${\mathbb R}^n \times \mathrm{SO}_n$ located at $(X_k(t), A_k(t))$ converges in the weak sense towards a deterministic absolutely continuous measure whose density is $f$. A similar although local-in-time result is proved in \cite{briant2022cauchy} for the unnormalized Vicsek system whose setting is analog to our original system \eqref{eq:part_dXkdt}, \eqref{eq:part_dAkdt}. 
...
$$ - \frac{\partial \alpha_1}{\partial \theta_1} \Big( m \frac{\partial \alpha_1}{\partial \theta_1} \Big) + \frac{m \, \alpha_1}{1 - \cos \theta_1} = m \sin \theta_1. $$
...
\label{eq:equi_kin}
\end{equation}
It is clear that if $f^\varepsilon \to f_0$ as $\varepsilon \to 0$ strongly as well as all its first-order derivatives with respect to $x$ and second-order derivatives with respect to $A$, then, we must have $Q(f^0) = 0$. Solutions of this equation are called equilibria.  
...
\begin{corollary}
Assume that $f^\varepsilon \to f_0$ as $\varepsilon \to 0$ strongly as well as all its first-order derivatives with respect to $x$ and second-order derivatives with respect to $A$, such that $\int f_0(x,v,t)  dv >0$, $\forall (x,t) \in {\mathbb R}^n \times [0,\infty)$. Then, there exists two functions $\rho$: ${\mathbb R}^n \times [0,\infty) \to (0,\infty)$ and $\Gamma$: ${\mathbb R}^n \times [0,\infty) \to \mathrm{SO}_n$ such that \eqref{eq:equi_f0express} holds. 
\label{cor:equi}
\end{corollary}
\noindent
...
for all $X, \, Y \, X', \, Y' \, \in \mathfrak{so}_n$. With this identification, if 
$(F_i)_{i=1}^{\mathcal N}$ and $(G_i)_{i=1}^{\mathcal N}$ (with ${\mathcal N} = \frac{n(n-1)}{2}$) are two orthonormal bases of $\mathfrak{so}_n$, then $(F_i \otimes A G_j)_{i,j = 1}^{\mathcal N}$ is an orthonormal basis of $\mathfrak{so}_n \otimes T_A$ and we can write 
$$ \nabla \chi (A) = \sum_{i,j=1}^{\mathcal N} \big( \nabla (\chi \cdot F_i)(A) \cdot (A G_j) \big) \, F_i \otimes A G_j. $$ 
...
\begin{eqnarray} 
\nabla \mu(A) \cdot \nabla \chi(A) &=& \sum_{i,j=1}^{\mathcal N} \big( \nabla (\mu \cdot F_i) (A) \cdot AG_j \big) \, \big( \nabla (\chi \cdot G_i) (A) \cdot AG_j \big) \nonumber \\
&=& \sum_{i,j=1}^{\mathcal N} \Big( \big( \varrho(G_j) (\mu \cdot F_i) \big) (A) \Big) \, \Big( \big( \varrho(G_j) (\chi \cdot F_i) \big) (A) \Big). \label{eq:GCIId_nabmu_nabchi_def}
\end{eqnarray}
In general, we will use \eqref{eq:GCIId_nabmu_nabchi_def} with identical bases $(F_i)_{i=1}^{\mathcal N} = (G_i)_{i=1}^{\mathcal N}$, but this is not necessary. The construction itself shows that these formulae are independent of the choice of the orthonormal bases of $\mathfrak{so}_n$. 
...
\tau_i \big( D_{ij} \Theta \big) &=& - \tau_i ( \Theta ),  \qquad \alpha_j \big( D_{ij} \Theta \big) = - \alpha_j (  \Theta ). 
\end{eqnarray*}
Hence, we find
...
\begin{proposition}
$\chi \in  H^1_{\mathrm{inv}}(\mathrm{SO}_n, \mathfrak{so}_n)$ if and only if $\chi$ is given by \eqref{eq:GCIId:mu_generic} with $\tau \in {\mathcal V}$. 
\label{prop:GCIId_regularity_tau}
\end{proposition}

\noindent
\textbf{Proof.} Let $\chi \in C^{\infty}_\mathrm{inv}(\mathrm{SO}_n, \mathfrak{so}_n)$, an element of the space of smooth functions satisfying the invariance relation \eqref{eq:GCIId_conjug_invar_mu}. Then, $\tau$ associated with $\chi$ through \eqref{eq:GCIId:mu_generic} belongs to $C_{\mathrm{per}}^{\infty,\mathfrak{W}}({\mathcal T}, {\mathbb R}^p)$. 
...
\end{equation}
We also temporarily assume that 
\begin{equation}
...
\end{equation}
Let now $\chi \in H^1_{\mathrm{inv}}(\mathrm{SO}_n, \mathfrak{so}_n)$ and $(\chi^q)_{q \in {\mathbb N}}$ a sequence of elements of $C^{\infty}_\mathrm{inv}(\mathrm{SO}_n, \mathfrak{so}_n)$ which converges to $\chi$ in $H^1_{\mathrm{inv}}(\mathrm{SO}_n, \mathfrak{so}_n)$. Then, the associated sequence $(\tau^q)_{q \in {\mathbb N}}$ is such that $\tau^q \in C_{\mathrm{per}}^{\infty,\mathfrak{W}}({\mathcal T}, {\mathbb R}^p)$ and by \eqref{eq:GCIId_regularity_tau_prf0} $(\tau^q)_{q \in {\mathbb N}}$ is a Cauchy sequence in ${\mathcal V}$. Thus, there is an element $\tau \in {\mathcal V}$ such that $\tau^q \to \tau$ in ${\mathcal V}$. Now, up to the extraction of subsequences, we have $\chi^q \to \chi$, a.e. in $\mathrm{SO}_n$ and $\tau^q \to \tau$, a.e. in ${\mathcal T}$. Hence 
$$ \chi^q = \sum_{i=1}^p \tau^q_i F_{2i-1 \, 2i} \to \sum_{i=1}^p \tau_i F_{2i-1 \, 2i} \quad \mathrm{a.e.}, $$ 
showing that $\chi = \sum_{i=1}^p \tau_i F_{2i-1 \, 2i} $. Hence, $\chi \in H^1_{\mathrm{inv}}(\mathrm{SO}_n, \mathfrak{so}_n)$ if and only if $\chi$ is associated with $\tau \in {\mathcal V}$ through \eqref{eq:GCIId:mu_generic}. 

We now show \eqref{eq:GCIId_regularity_tau_prf-1}. Let $\chi \in H^1_{\mathrm{inv}}(\mathrm{SO}_n, \mathfrak{so}_n)$. Since $C^\infty(\mathrm{SO}_n, \mathfrak{so}_n)$ is dense in $H^1(\mathrm{SO}_n, \mathfrak{so}_n)$, there exists a sequence $(\chi^q)_{q \in {\mathbb N}}$ in $C^\infty(\mathrm{SO}_n, \mathfrak{so}_n)$ such that $\chi^q \to \chi$. Now, $\bar \chi^q$ obtained from $\chi^q$ through \eqref{eq:GCIId_barchi_def} belongs to $C^{\infty}_\mathrm{inv}(\mathrm{SO}_n, \mathfrak{so}_n)$. And because the map $\chi \mapsto \bar \chi$ is continuous on $H^1(\mathrm{SO}_n, \mathfrak{so}_n)$ (see proof of Prop. \ref{prop:GCIId_varform_invar}), we get $\bar \chi^q \to \bar \chi$ as $q \to \infty$. But since $\chi$ satisfies \eqref{eq:GCIId:mu_generic}, we have $\bar \chi = \chi$, which shows the requested density result. 

We now show \eqref{eq:GCIId_regularity_tau_prf0}. Let $\chi \in C^{\infty}_\mathrm{inv}(\mathrm{SO}_n, \mathfrak{so}_n)$. Since the function $A \mapsto |\chi(A)|^2$ is a class function, we have, thanks to Weyl's integration formula \eqref{eq:WIF}:
\begin{equation}
...
\end{equation}
which shows that $\| \chi \|_{L^2} = \frac{\gamma_n}{(2 \pi)^p} \| \tau \|_{\mathcal V}$. 
...
which leads to 
\begin{eqnarray*}
\big( \varrho(G^+) \chi \big) (A_\Theta) &=& - \frac{\tau_1(\theta_1)}{2 (1 - \cos \theta_1)} \big( \sin \theta_1 G^+ + (\cos \theta_1 - 1) G^- \big), \\
\big( \varrho(G^-) \chi \big) (A_\Theta) &=& \frac{\tau_1(\theta_1)}{2 (1 - \cos \theta_1)} \big( (\cos \theta_1 - 1) G^+ - \sin \theta_1 G^- \big).  \\
\end{eqnarray*}
...
\medskip
This finishes the proof of \eqref{eq:GCIId_regularity_tau_prf2} and of the proposition. \endproof
...
for some $C >0$. Since $1 - \cos (\theta_i \pm \theta_j) \leq 2$, we have 
\begin{eqnarray*} 
...
\end{equation}
Thus, up to a constant which will factor out from \eqref{eq:GCIId_varform_alpha}, the integrals over ${\mathcal T}$ which are involved in the definitions of ${\mathcal A}$ and ${\mathcal L}$ in \eqref{eq:GCIId_bilinform_A} and \eqref{eq:GCIId_linform_L} can be replaced by integrals over ${\mathcal W}_{\mathrm{per}}$. 
...
Thus, 
$$ \Circled{3} = \kappa \rho \sum_{i,j,m,q = 1}^n \int_{\mathrm{SO}_n} \Big( \frac{A - A^T}{2} \Big)_{ij}  \Big( \frac{A + A^T}{2} \Big)_{mq} \, M(A) \, \mu(A) \, dA \, \, {\mathbb S}_{ijmq}. $$
...
\begin{eqnarray} 
\Circled{3}_{rs} &=& \kappa \rho \sum_{m,q = 1}^n \int_{\mathrm{SO}_n} (A \cdot {\mathbb S}_{\cdot \cdot mq}) \, (\mu(A) \cdot F_{rs}) \, \Big( \frac{A + A^T}{2} \Big)_{mq} \, M(A)  \, dA \nonumber \\
...
\end{eqnarray}
where $\epsilon_n$ is given by \eqref{eq:Gamma_eps_def} and $m$ by \eqref{eq:GCIId_m_def}. 
...
\end{equation}
Now, summing \eqref{eq:Gamma_L(P)_B(P,Q)_express_prf24} over $\ell \not =j$ and adding \eqref{eq:Gamma_L(P)_B(P,Q)_express_prf25}, we find
$$
2 C_3 + C_4 \frac{n^2 + n - 4}{2n} = \sum_{\ell=1}^n \int_{\mathrm{SO}_n} \mu(A)_{i \ell} \Big(\frac{A+A^T}{2} \Big)_{\ell j} \Big(\frac{A-A^T}{2}\Big)_{ji} \, M(A) \, dA. 
$$
...
We apply this equality with $P=F_{ij}$ for $i \not = j$ and we note that $(\beta(F_{ij}) \cdot F_{ij})=0$, $\forall i, j$, and that $(\beta(P). \beta(P)) = (P.P)$ and $(\mu(A) \cdot \beta(P)) = 
(\beta \circ \mu(A) \cdot P)$ due to the fact that $\beta$ is an orthogonal self-adjoint transformation of $\mathfrak{so}_4$. This leads to 
...
Taking the trace of \eqref{eq:dim4_Bs_prf1} and applying the resulting formula with $Q = \beta(P)$, we get, with \eqref{eq:Gamma_L(P)_B(P,Q)_express_prf9} and \eqref{eq:Gamma_Bdef}: 
\begin{eqnarray*}
&&\hspace{-1cm}
\frac{1}{2} \int_{\mathrm{SO}_4} \Big\{  \big( \frac{A-A^T}{2} \cdot P \big) \big( \beta \circ \mu(A) \cdot P \big) + 
\Big( \beta \big( \frac{A-A^T}{2} \big) \cdot P \Big) \big( \mu(A) \cdot P \big) \Big\} \mathrm{Tr}(A) \, M(A) \, dA \\
&&\hspace{9cm}
= 4 C_5 (P \cdot P), \quad \forall P \in \mathfrak{so}_4. 
\end{eqnarray*}
Applying this relation with $P=F_{ij}$ and averaging the result over $i$, $j$, such that $i \not = j$, we get 
\begin{eqnarray*}
...
$$ \mathrm{Tr} \big\{  {\mathcal R} \big( (v_1 \wedge v_2) \wedge (v_3 \wedge v_4) \big)  \big\} = 0. $$
...
$$
It follows that 
$$ B_a(F_{ij},F_{ik})_{i \ell} = C_6 {\mathcal R}\big( (e_i \wedge e_j) \wedge (e_i \wedge e_k) \big)_{i \ell} = 2 C_6 \varepsilon_{ikj\ell}. $$
Thus, 
$$ \sum_{i,j,k, \ell} \epsilon_{i j k \ell} B_a(F_{ij},F_{ik})_{i \ell} = 2 C_6 \sum_{i,j,k, \ell} \epsilon_{i j k \ell} \varepsilon_{ikj\ell} = -2 C_6 \mathrm{Card}(\mathfrak{S}_4) = - 48 C_6. $$
On the other hand, 
\begin{eqnarray*}
&&\hspace{-1cm}
...
with $u_i$ the $i$-th coordinates of $u$ in the basis $(e_i)_{i=1}^n$ and similarly for $v_j$ and $w_k$. Now, for a given $A \in \mathrm{SO}_4$, there exists $g \in \mathrm{SO}_4$ and $\Theta = (\theta_1, \theta_2) \in {\mathcal T}_2$ such that $A = g A_\Theta g^T$. Then,  
\begin{eqnarray}
&&\hspace{-1cm}
\mathrm{det} \Big(  \frac{A-A^T}{2} e_i,  \,  \mu(A) e_i, \,  \frac{A+A^T}{2} e_i, \,   e_i  \Big) \nonumber \\
&&\hspace{1cm}
= \mathrm{det} \Big(  g \frac{A_\Theta-A_\Theta^T}{2} g^T e_i,  \,  g \mu(A_\Theta) g^T e_i, \,  g \frac{A_\Theta+A_\Theta^T}{2} e_i, \,   e_i  \Big) \nonumber \\
&&\hspace{1cm}
= \mathrm{det} \Big( \frac{A_\Theta-A_\Theta^T}{2} g^T e_i,  \, \mu(A_\Theta) g^T e_i, \,  \frac{A_\Theta+A_\Theta^T}{2} g^T e_i, \,  g^T e_i  \Big) \nonumber \\
&&\hspace{1cm}
= \sum_{j,k,\ell,m} g_{ij} \, g_{ik} \, g_{i\ell} \, g_{im} \, \mathrm{det} \Big( \frac{A_\Theta-A_\Theta^T}{2} e_j,  \, \mu(A_\Theta) e_k, \,  \frac{A_\Theta+A_\Theta^T}{2} e_\ell, \,  e_m  \Big). \label{eq_proof_of_Ba_prf1}
\end{eqnarray}
Now, thanks to \eqref{eq:GCIId_tildmuk_equation_prf2}, \eqref{eq:GCIId:mu_generic}, \eqref{eq:Gamma_L(P)_B(P,Q)_express_prf26}, detailed inspection shows that the determinant in~\eqref{eq_proof_of_Ba_prf1} is non-zero only if and only if $i, \, j, \, k, \ell$ are all distinct and 
(($\{i,j\}= \{1,2\}$ and $\{k,\ell\}= \{3,4\}$) or ($\{i,j\}= \{3,4\}$ and $\{k,\ell\}= \{1,2\}$)). Let us consider for instance the cases $(i,j,k,\ell) = (1,2,3,4)$ and $(2,1,3,4)$. We have 
\begin{eqnarray}
&&\hspace{-1cm}
\mathrm{det} \Big( \frac{A_\Theta-A_\Theta^T}{2} e_1,  \, \mu(A_\Theta) e_2, \,  \frac{A_\Theta+A_\Theta^T}{2} e_3, \,  e_4  \Big) = - \alpha_1(\Theta) \sin \theta_1 \cos \theta_2 
\label{eq_proof_of_Ba_prf2}
\\
&&\hspace{1cm}
= - \mathrm{det} \Big( \frac{A_\Theta-A_\Theta^T}{2} e_2,  \, \mu(A_\Theta) e_1, \,  \frac{A_\Theta+A_\Theta^T}{2} e_3, \,  e_4  \Big). \label{eq_proof_of_Ba_prf3}
\end{eqnarray}
On the other hand, the expression $g_{ij} \, g_{ik} \, g_{i\ell} \, g_{im}$ is symmetric under permutations of $(j,k,\ell,m)$. Thus, the contributions of the two determinants \eqref{eq_proof_of_Ba_prf2} and \eqref{eq_proof_of_Ba_prf3} to the sum \eqref{eq_proof_of_Ba_prf1} cancel each other. We make the same observations for the pairs of quadruples $((1243),(2143))$, $((3412),(4312))$ and $((3421),(4321))$. As observed before, the terms corresponding to other quadruples are zero. Hence, the sum \eqref{eq_proof_of_Ba_prf1} is equal to zero which results in the fact that $C_6=0$ and ends the proof. \endproof 
