...
\end{equation}
 ...
 For our DLR calculations, the values for the $a, b, \phi$ parameters are the \texttt{A\_{IMAGE}}, \texttt{B\_{IMAGE}} (converted into arcsec), and \texttt{THETA\_{IMAGE}} parameters output by Source Extractor \citep[][\texttt{sextractor}]{psfex}. The galaxy with the lowest $d_{\mathrm{DLR}}$ value is chosen as the host galaxy.% \todo{i also defined snsep as $\Delta \theta$, oops...} 
...
For simulations with SNe Ia only, we find $\Delta w = 0.0013 \pm 0.0026$, which is consistent with zero. In Figure~\ref{fig:Ia_delta-mu}, we show the biases on the binned Hubble diagram comparing distance moduli $\mu$ from simulations with DLR matched hosts ($\mu_{\text{match}}$) and true hosts ($\mu_{\text{truehost}}$). We define 
\begin{equation}
\label{eq:delta-mu}
