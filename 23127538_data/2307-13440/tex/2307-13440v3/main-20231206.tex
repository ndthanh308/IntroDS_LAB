...
\title{Black holes with scalar hair: Extending from and beyond the Schwarzschild solution}
...
This paper aims to unveil the existence of scalarized black holes, their profiles, and thermodynamic stability in Einstein-scalar-Weyl gravity \eqref{action} by directly solving the fourth-order EOMs.
...
Although the dilaton-like coupling function $F(\varphi)=e^{-\beta\varphi}$ only allows nontrivial solutions for the scalar field $\varphi$ according to the EOMs, we propose that as $\beta$ approaches zero, the hairy black hole gradually converges to the Schwarzschild solution with a vanishing scalar field.
...
In this paper, we present novel scalarized BH solutions beyond the GR framework. These scalarized BH solutions are extended from the Schwarzschild one and non-Schwarzschild one found in the pure Einstein-Weyl gravity \cite{Lu:2015cqa}.
