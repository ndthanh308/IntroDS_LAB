...
\author[2]{Jo\"el \snm{Lavanchy}}
\author[2]{Pietro \snm{Mascagni}}
...
\address[1]{ICube, University of Strasbourg, CNRS, Strasbourg, France}
...
\begin{itemize}
    ...
    \item We propose to leverage text transcription from two complementary ASR systems, Whisper and AWS, to enhance the learning process by addressing limitations and inaccuracies inherent in each ASR system.
    ...
    \item We demonstrate the zero-shot transferability of our learned multi-modal representations by showing superior performance on various vision-and-language and vision-only surgical tasks. 
\end{itemize} 


\section{Related Works}

\subsection{Representation learning from unlabeled videos}
...
modalities to efficiently optimize the model using large-scale image-text paired datasets~\citep{li2021supervision}. 
...
In particular, AWS Medical Transcribe is used to understand medical terminology and surgery-specific terms, and Whisper is used to understand the overall sentence structure and common words. 
...
\end{itemize}
We give below our intuitions behind the choices made for the video clip sampling. 
...
Our final multiple text-views contrastive loss $\mathcal{L}$ combines these two loss functions, $\mathcal{L}_{InfoNCE}$ and $\mathcal{L}_{MIL-NCE}$, scaled by the weighting coefficients $\epsilon$, as shown below:
\begin{equation}
    \centering
...
\subsection{Zero-shot transfer to downstream tasks}
...
\subsubsection{Vision-only surgical tasks}
...
Fig.~\ref{activatioin map2} shows the impact of novel text queries on the temporal activity grounding task. Specifically, we simulate the open-vocabulary scenario by decomposing the sentence into sub-sentence. For example, we remove the sub-sentence \textit{``and bloodless field could be obtained''} from the complete sentence \textit{``bipolar is very helpful in this task and bloodless field could be obtained''}, resulting in a novel text query \textit{``bipolar is very helpful in this task''}. We plot the visual-textual activation map by comparing the text embedding to all visual frame embeddings of the video. It shows that the model is triggered by precise sentences, such as \textit{``bipolar is very helpful in this task''} rather than abstract or noisy ones, such as \textit{``bloodless field could be obtained''}. Therefore, removing a noisy sub-sentence creates a more precise activation map. Fig.~\ref{activation map1} also demonstrates that \textit{SurgVLP} with two text views generates a focused attention map, highlighting relevant frames and ignoring adjacent ones. This demonstrates that multiple text view supervision effectively connects the visual and the textual content.
...
This work was supported by French state funds managed by the ANR within the National AI Chair program under Grant ANR-20-CHIA-0029-01 (ChairAI4ORSafety) and within the Investments for the future program under Grant ANR-10-IAHU-02 (IHU Strasbourg). This work was granted access to the HPC resources of IDRIS under the allocation AD011013704R1 and AD011011631R2 made by GENCI.
