...
\begin{abstract}
	The real power of artificial intelligence appears in reinforcement learning, which is more dynamic and can adapt itself to real-world fluctuations. Rotation and injection are some of the proven ways in active flow control for drag reduction on blunt bodies. In this paper, rotation will be added to the cylinder alongside the deep reinforcement learning (DRL) algorithm, which uses multiple controlled jets to reach the maximum possible drag suppression. Characteristics of the DRL code, including controlling parameters, their limitations, and optimization of the DRL network for use with rotation will be presented. This work will focus on optimizing the number and positions of the jets, the sensor’s location, and the maximum allowed flow rate to jets in the form of the maximum allowed flow rate of each actuation and the total number of them per episode. It is found that combining the rotation and DRL is promising since it suppresses the vortex shedding, stabilizes the Karman vortex street, and reduces the drag coefficient by up to $49.75\%$. Also, it will be shown that having more sensors at more locations is not always a good choice and the sensor number and location should be determined based on the need of the user and corresponding configuration. Also, allowing the agent to have access to higher flow rates, mostly reduces the performance, except when the cylinder rotates. In all cases, the agent can keep the lift coefficient at a value near zero, or stabilize it at a smaller number.
\end{abstract}
...
The key idea consists of performing computation using a network of simple processing units, called artificial neurons. The output value of each neuron is computed by some non-linear function of the sum of its inputs. The connection between each neuron is called the edge. Neurons and edges typically have a weight that adjusts using an algorithm as learning proceeds. The weight increases or decreases the strength of the signal at each connection. As an example of performing supervised learning, an algorithm such as stochastic gradient descent is used for tuning the neuron's weights to minimize a cost function on a training set \cite{Goodfellow}. Given the success of this method, ANNs can in theory solve any problem since they are universal approximators. A large enough feed-forward neural network using a non-linear activation function can fit arbitrarily well any function \cite{Hornik}. Therefore, virtually any problem or phenomenon that can be represented by a function could be a field of experimentation with ANNs. However, the problem of designing the ANNs, and designing the algorithms that train and use them, is still the object of active research.
...
The geometry of the simulation, adapted from the 2D test case of Schäfer et al.\cite{Schäfer}, consists of a cylinder with the non-dimensional diameter of D = 1 immersed in a box of total non-dimensional length L = 22 (along the X-axis) and height H = 4.1 (along Y-axis). The origin of the coordinate system is located at the center of the cylinder, and the cylinder is 0.05 off-axis in y direction, as shown in figure~\label{Geo2}\ref{Geo2}. The inflow profile is parabolic with the formula as follows:

\begin{equation}
	U_{(y)}={5(H⁄2-y)(H⁄2+y)}/H^2
\end{equation}

The no-slip boundary condition is imposed on the top, bottom walls, and solid walls of the cylinder. In the case of rotation, the speeds are applied over the boundary of the cylinder's wall. An outflow boundary condition is imposed on the right side of the domain. The Reynolds number is defined based on the mean velocity magnitude and the cylinder diameter $(Re = \bar{U}D/\nu )$, with $\nu$ being the kinematic viscosity, $\bar{U} = 2U(0)/3=1$ and $Re = 100$. Computations are performed on an unstructured mesh generated using Gmsh \cite{Geuzaine}. The mesh is refined around the cylinder and is composed of $9252$ triangular elements. A constant non-dimensional time step $dt = 5\times10^{-4}$ has been used. The total instantaneous drag on the cylinder is computed as follows:

\begin{equation}
	F_D=\int\limits_c (\sigma.n).e_x \ dS \
\end{equation}
where $\sigma$ is the Cauchy stress tensor, $n$ is the unit vector normal to the outer cylinder surface, and $e_x = (1, 0)$. Then, the drag is normalized into the drag coefficient:
\begin{equation}
...
The Navier-Stokes equations are solved in a segregated manner based on the incremental pressure correction scheme or IPCS method \cite{Goda}, with an explicit treatment of the non-linear term. The spatial discretization relies on the finite-element method implemented within the FEniCS framework \cite{Logg}. A maximum of six jets (numbered 0 to 5) normal to the cylinder wall is implemented on the surface of the cylinder, at angles $\theta_0$ to $\theta_5$ relative to the flow direction. The jets are controlled through their non-dimensional mass flow rates, $Q_i$, i = 0 to 5 and are set through a parabolic-like velocity profile going to zero at the edges of the jet. The jet widths are fixed at $10^{\circ}$.

In addition, the control is set up in such a way that the total mass flow rate injected by the jets is zero, i.e. $Q_0+...+Q_i = 0$. This synthetic jet condition is chosen as it is more realistic than a case when the mass flow rate is added or subtracted from the flow. In total, we ran 34 simulations (excluding the validation and test simulations). To summarize the results of these simulations, we grouped them based on the jet positions as shown in the table~\ref{Configurationk}.
...
%----------------------------------------------------------
\subsubsection{\label{sec:level2}The network and the reinforcement learning framework}
Deep reinforcement learning (DRL) sees fluid mechanics simulation as another environment to interact with through three simple channels: observation, action, and reward. Here, the reward is the time-averaged drag coefficient provided by the environment and penalized by the mean lift coefficient.
Based on this limited information, the DRL trains a deep neural network to find a closed-loop control strategy, deciding the action (here jets flow rate) based on observation at each time step, to maximize the reward (here the drag reduction).
As stated before, the DRL agent uses the proximal policy optimization \cite{Schulman} method. PPO is a reinforcement learning algorithm that belongs to the family of policy gradient methods. This method was chosen for several reasons. In particular, it is less complex mathematically and faster than concurring trust region policy optimization methods \cite{Schulman}. In addition, it requires little hyperparameter tuning and is easy to configure. It also adapts well to continuously controlled problems in comparison to the deep Q-learning \cite{Mnih} and its other variations \cite{Gu}.

The PPO method is episode-based, which means that it learns from performing active control for a limited amount of time before analyzing the results obtained and resuming learning with a new episode. In our cases, the simulation is first performed without conducting active control until a developed unsteady wake is obtained at about 5 seconds. Then, the corresponding state is saved and used as a start position for each subsequent learning episode. The instantaneous reward function, $r_t$, is computed as follows.
\begin{equation}
	r_t=-⟨C_D ⟩_T-0.2|⟨C_L ⟩_T |
\end{equation}
where $⟨.⟩T$ indicates the average sliding back in time over a duration corresponding to one vortex shedding cycle. ANN tries to maximize this function $r_t$, i.e. to make it as little negative as possible therefore minimizing drag and mean lift.

This specific reward function has several advantages compared to utilizing the plain instantaneous drag coefficient. Firstly, using values averaged over one vortex shedding cycle leads to less variability in the value of the reward function, which was found to improve learning speed and stability. Secondly, the use of a penalization term, based on the lift coefficient is necessary to prevent the network from cheating. As in the absence of this penalization, the ANN manages to find a way to modify the configuration of the flow in such a way that a larger drag reduction is obtained, but at the cost of a large induced lift which is damageable in most practical applications.
...
The ANN is composed of two dense layers of 512 fully connected neurons, plus the layers required to acquire data from the probes and generate data for jets. This network configuration was found empirically through trial and error, as is usually done with ANNs. Results obtained with larger networks are less appropriate, as they are harder to train and have significantly longer run times. In addition, as is discussed below in the validation section, there was no noticeable improvement. Larger networks are also less successful. In total, this network has slightly over 300000 weights. A learning rate of 1e-3, a sub-sampling fraction of 0.2, and a batch size of 20 are used. Our code is based on the original paper of Rebault et al.\cite{Rebault19} with some tweaks and changes, as we started our work right after their publication.
...
Thirdly, adding rotation alongside the DRL-controlled jets can lower the drag coefficient from 3.2416 to 1.6288 in the best case, which is almost a $50\%$ reduction. In this case, the vortex shedding is almost suppressed. Contrary to the previous case, giving access to higher flow rates for the agent is usually beneficial, since in this case, the constraint is partly jet outputs. This was true for configuration III with 0.2 rotation speed as in this case, configuration II could not control the flow and reduce the drag coefficient. But, this configuration could decrease the drag force at smaller rotation speeds of 0.1 and 0.08, although its performance was worse than that of configuration III. Again we believe it's partly because of the lower maximum output of 2 jets compared to the 5 and partly because of the jet at $0^{\circ}$. Lastly, in all the cases, the agent lowers the lift coefficient to zero or stabilizes it on a near-to-zero number (lower than its value at the beginning of the process).
