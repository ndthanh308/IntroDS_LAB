...
Moreover, the linear constraint representation of CCEs and CEs is particularly useful as one may define a linear program with a corresponding linear objective function in order to test certain hypotheses; as we do in the proof of Lemma \ref{lem: CCE non uniform winning}. %, such as convergence to uniform winning bids. 
...
\end{theorem}
 It is worth noting that both the time and space complexity exhibit polynomial scaling with $\Nitem$ and $|\mathcal{B}|$. {\color{black}For the special case of $M=1$ corresponding to the standard first price auction, we obtain the regret of $\tilde{O}(\sqrt{\Nround})$.}
