...
\begin{table}[ht]
    ...
    \begin{minipage}[t]{0.48\linewidth} % Right table
        ...
        \midrule
        \multicolumn{5}{c}{\textbf{For \( M = 1 \)}} \\
        % Regret & 9.502 (11.2415) & 11.6405 (13.2965) & 9.7865 (13.1154) & 12.1262 (15.0636)\\
...
\begin{enumerate}
    ...
    \item \textbf{Run-time Increases in $M, N, |\mathcal{B}|$.} The run-time of our algorithms is approximately linear in each of $M$ and $|\mathcal{B}|$, as predicted in Section~\ref{sec: path kernels regret}. The algorithm for uniform price is approximately linear in $M$ and quadratic in $|\mathcal{B}|$, consistent with \cite{brânzei2023online}. As there are $N$ agents running the learning algorithm in each trial, the run-time is also both theoretically and empirically linear in $N$.
