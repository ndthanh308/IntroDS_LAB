...
This is difficult since precise identification requires correct classification of false positives (bugs caused by the driver) and false negatives (ineffective usage).
...
The evaluation results show that some questions cannot be answered by any strategy, indicating the difficulty of generating effective fuzz drivers without a global understanding of the context requirements.
For instance, the \texttt{tmux}~\cite{tmux-ossfuzz-driver-link} driver requires a comprehensive understanding of the relationships between session, window, pane, etc.
...
Ignoring these contextual requirements can result in failed generation of effective drivers.
% Thus, future research can focus on exploring more advanced strategies to address these challenges.
Therefore, more advanced strategies can be explored on solving these challenges.
...
Many generated drivers contain APIs that do not appear in the OSS-Fuzz drivers, especially for those drivers hinted by example usage snippets.
...
In this study, we discussed the effectiveness of strategies on C projects.
The strategies or improvements may not be necessarily generalizable on other languages.
