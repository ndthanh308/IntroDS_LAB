...
\tab
The effectiveness of two types of \textbf{Extended} information are studied: API documentation and example code snippets. 
Table~\ref{tab:advanced-strategies-detail} details two types of strategies which are extended from \texttt{BACTX-K} by adding additional usage information into the prompt template.
% For \texttt{DOCTX-K}, it additionally provides the documentation before function declaration.
% the only difference with \texttt{BACTX-K} is the inclusion of the target API documentation before the function declaration.
Note that, for \texttt{DOCTX-K}, not all APIs have associated documentation (49/86 questions have in our quiz).
The documentation of 20 questions was automatically extracted from the header files, while the remaining 29 were manually collected from sources like project websites, repositories, and developer references. 
% For \texttt{UGCTX-K}, the example code snippet is added after the \texttt{\#include} statement, followed by the declarations of all APIs used in the example, and the rest are the same as those of \texttt{BACTX-K}. 
For \texttt{UGCTX-K}, example code snippets of an API is collected as follows:
...
\ding{183} identifing and excluding the files that are fuzz drivers;
\ding{184} extracting functions which directly calls the target API as example code snippets via \texttt{ANTLR};
% each function inside the files that directly calls the target API is extracted as one example using \texttt{ANTLR};
\ding{185} deduplicating the snippets by comparing their Jaccard Similarity~\cite{jaccard-similarity} ($\geq$95\%).
...
\texttt{UGCTX-K} will randomly pick one snippet from its collection when generating prompt.
For snippets that were too long to be included into prompt, they are truncated line by line satisfying token length limitation.
...
\tab
\texttt{DOCTX-K} is compared with \texttt{BACTX-K} on the 49 questions which have API documentation.
...
This result is reasonable since API documentation usually provides a limited description of the usage, typically a summary of main functionality with one-sentence explanations for arguments. 
However, the blocker-solving usage information discussed in Section~\ref{sec:failure-analysis}, like control flow dependencies or usages of its dependent APIs, is usually not included.
...
We further investigated the contributions of different example sources.
Figure~\ref{fig:statistics-on-different-example-sources} compares sources in two kinds of file path based divisions:
\ding{182} \textit{External} and \textit{Internal}.
The internal source includes the target project and its variants, while the external source covers the rest.
\ding{183} \textit{Test \& Example} and \textit{Others}.
...
One key observation is that both \textit{Internal} and \textit{Test \& Example} are higher quality sources than their counterparts, especially for these questions which are not likely to be solved without usage snippets (\textit{i.e.}, not solved by \texttt{BACTX-K}).
...
Besides, "test/example files", "code files from the target/variant projects" are high quality sources.
...
Specifically, for initial query, it can be one of \texttt{BACTX-K}, \texttt{DOCTX-K}, and \texttt{UGCTX-K}.
% can be used for generating diverse startup drivers.
% we first detail its design then point out the divergence point.
For fix queries, seven fix templates are designed towards fixing different errors observed from the driver.
The errors are categorized according to the taxonomy covering compile, link, and runtime fuzzing errors.
Note that only the automatically identifiable symptoms are used in iteration and the manually added feedback discussed in Section~\ref{sec:methodology} will only be used for quiz validation.
Table~\ref{tab:fix-prompt-general-template} shows the general fix template while all full details are put at ~\cite{fuzz-drvier-study-website}.
% are recorded in \appe\ref{sec:fix-templates-detail} Table~\ref{tab:fix-templates-detail}.
In template, "\texttt{\$\{ERR\_XXX\}}" are error type specific content.
"\texttt{\$\{SUPPLEMENTAL\_INFO\}}" will contain additional root cause API usage information.
For grammatical errors, it is easy to locate the root cause API and provide correct usage accordingly.
...
For semantic errors, the root cause API is located by identifying the first API found starting from the error line.
...
We developed two strategies \texttt{BA-ITER-K} and \texttt{EX-ITER-K} (collectively referred to as \texttt{ITER-K}).
...
\textit{EX-ITER-K} selects the choice randomly.
% \texttt{K} represents the times of iterative queries will be conducted for one question.
Both strategies are evaluated with max search depth = 20 and max \texttt{K} = 40.
