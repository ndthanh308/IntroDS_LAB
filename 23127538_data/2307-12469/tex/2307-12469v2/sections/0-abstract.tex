...
Compared to traditional program analysis based generators, it is a text-based approach which is more lightweight and general.
It can easily leverage various dimensions of API usage information during the generation and generate human-friendly code.
Nonetheless, there still lacks the basic understanding on this direction.

To fill this gap, we conducted a study targeting the fundamental issues of using LLMs on effective fuzz driver generation.
For systematic understanding, five query strategies are designed and analyzed from basic to enhanced.
For evaluation in scale, we built a semi-automatic framework, containing a quiz with 86 driver generation questions collected from 30 popular C projects, and a set of criteria for precise driver effectiveness validation.
In total, 189,628 fuzz drivers using 0.22 billion tokens are generated and evaluated.
% Based on the framework, five query strategies are designed and evaluated from basic to enhanced, involving the validation of more than .
Besides, generated drivers were compared with industrial used ones to obtain practical insights (3.12 CPU year fuzzing experiments).
Our study revealed that:
\ding{182} LLM-based generation has shown promising practicality.
...
\ding{183} LLMs struggle to generate fuzz drivers that require complex API usage specifics.
...
\ding{184} Significant rooms for improvement are still left, such as automatic semantic correctness validation, API usage expansion, and semantic oracle generation.
