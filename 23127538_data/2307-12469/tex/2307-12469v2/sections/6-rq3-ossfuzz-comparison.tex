...
All drivers of \texttt{gpt4-ITER-K} are compared with OSS-Fuzz's, covering 78 questions.
...
Specifically, it uses a switch structure to determine which logic it will execute based on a part of the input data.
...
In total, 57 drivers for both \texttt{gpt4-ITER-K} and OSS-Fuzz are prepared.
...
In total, the fuzzing experiments take 3.12 CPU year.
Due to the page limit, Figure~\ref{fig:rq3-cmps} shows overall comparison results in certain metrics while the full detailed data for 57 pairs of drivers are posted at~\cite{fuzz-drvier-study-website}.
...
The API usage are measured by the number of unique project APIs used in the fuzz driver.
...
Overall, 49\% (17/35) \texttt{gpt-BACTX-K} drivers have used less project APIs than OSS-Fuzz's while 14\% for \texttt{gpt4-ITER-K}.
...
For example, for \texttt{gpt4-BACTX-K}, it tends to provide a fuzz driver with only necessary usages such as argument initialization.
The API usage is hardly extended such as adding APIs to use an object after parsing it.
...
The examples provided in \texttt{gpt4-ITER-K} alleviate this situation.
...
in all 78 questions, OSS-Fuzz drivers of 15 questions contain at least one oracle which can detect semantic bugs, while there are no LLM-generated drivers have oracles.
...
While example code can help LLM generate drivers containing more usages than OSS-Fuzz drivers, it still have space for prompting to expand API usages or semantic oracles.
...
Instead of presenting every detail of the experiments involving 228 different drivers, the plots directly list the comparison between their final results while the full experiment details for each driver can be found at~\cite{fuzz-drvier-study-website}.
Overall, in most questions, the LLM-generated drivers demonstrate similar or better performance in metrics of both coverage and the number of uniquely found crashes.
...
If only the fully automatic validation process are adopted, \textit{i.e.}, removing the last two checkers in Figure~\ref{fig:validation-checker}, the fuzzing outcome of 23 questions will be messed with huge number of false positives, incurring significant manual efforts for post-processing.
...
% \begin{tcolorbox}[size=title, opacityfill=0.1, nobeforeafter, breakable]
\begin{tcolorbox}[size=title, opacityfill=0.1, breakable]
LLM-generated drivers can have better fuzzing outcomes than OSS-Fuzz drivers.
