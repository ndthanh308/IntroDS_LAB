...
The direct failure reason of the driver is collected to reveal the generation blockers.
In total, 52,824 ineffective drivers were analyzed.
The runtime errors of 11,095 drivers are semi-automatically analyzed while the compilation and link errors are categorized based on the compiler outputs.
...
%The taxonomy represents the union results of all BACTX-K configurations.
...
%Grammatical mistakes usually root in misuse of indirect usages for target API, like passing mismatched number of arguments to a dependent API or referencing a non-existing \texttt{struct} member.
...
%Semantic errors are challenging for locating root causes and correction usages based on the code semantics.
%% However, reporting the existence of such error relevant usage description may still be helpful.
