...
\thanks{H. Zhang is with the School of Computer Science, China University of Geosciences, Wuhan, 430074, PR China (zhanghongyan@whu.edu.cn).}% <-this % stops a space
}
...
% the encoder-decoder structure and adopts VGG-16 as the encoder to extract multi-level features. On the basis of extracted multi-level features, we divide the whole decoder into three parts. Firstly, we adopt a conventional decoding strategy to output a relatively uncertain extraction map. 
...
% Deeplab v3+& 2016 &
% 84.33 &91.96 &91.23 &92.70
% &63.39 &77.59 &76.96 &78.24
% &73.58	&85.07 &85.99	&84.16 \\ 
HRNet \#& 2019 &
...
% \textbf{Ours—ISPRS} &  &
% {\color{red}\textbf{91.81}} &{\color{red}\textbf{95.73}} &95.76 &95.69 

% & 76.02 &86.37 &{\color{red}\textbf{87.76}} &85.04

% & 82.71 &90.54 &91.76 &89.35 \\
\textbf{Ours-UANet} &  &
% \color{red}\textbf{91.80} &{\color{red}\textbf{95.73}} &{\color{red}\textbf{96.06}} &\underline{95.39}
\color{red}\textbf{92.15} &{\color{red}\textbf{95.91}} &{\color{red}\textbf{95.96}} &\color{red}\textbf{95.86}

% & \color{red}\textbf{{76.20}} &\color{red}\textbf{{86.50}} &\color{red}\textbf{87.61} &\color{red}\textbf{{85.41}}
...
\par
%To eliminate the uncertainty of the building extraction map as much as possible, we proposed the UAFM, which separately converts the original features into foreground uncertainty features and the background uncertainty features, and concatenate the two together to achieve the uncertainty-aware feature.

%Through such an operation, we imitated the fusion way level by level and achieved the fusion of all features. Finally, the final extraction map with low uncertainty is achieved. As shown in Table \ref{tab:table_ablation}, we can easily find that the usage of the UAFM can bring the highest gain than the other two modules.
...
&69.73 &82.17 &85.41 &79.16
&79.08 &88.32 &87.77 &88.88\\
% 68.58 &81.36 &81.25 &81.47
% 75.42 &85.99 &88.32 &83.78\\
$M_{4}$&
...
% we can easily find that our proposed uncertain-aware strategy can achieve great extraction on all encoders.
...
% In order to verify the effectiveness of our proposed strategy, we also compared our method with other uncertainty strategies used in other vision tasks, $i.e.,$ GAN \cite{aleatoric_uncertainty} and Conditional Variational Autoencoder (CAVE)\cite{CVAE_COD}.
% %Regarding the compared uncertainty strategies, we directly selected two typical uncertainty-related works which solve the uncertainty problems via GAN \cite{aleatoric_uncertainty} and Conditional Variational Autoencoder (CAVE)\cite{CVAE_COD}, respectively.

% As shown in Table\ref{tab:table_us}, we can clearly see the superiority of our UANet. Meanwhile, unlike other uncertainty strategies that introduce complex networks to eliminate uncertainty, our proposed UAFM is simple and does not impose a large computational burden.
