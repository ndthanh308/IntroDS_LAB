...
\newcommand{\thomas}[1]{{\bf \color{green} [Thomas: #1]}}
\newcommand{\elisa}[1]{{\bf \color{magenta} [Elisa: #1]}}
\newcommand{\fran}[1]{{\bf \color{blue} [Fran.: #1]}}
...
\author[Maion et. al]{
Francisco Maion$^{1}$\thanks{E-mail: francisco.maion@dipc.org},
Raul E. Angulo$^{1,2}$,
Thomas Bakx$^{3}$,
Nora Elisa Chisari$^{3}$,
Toshiki Kurita$^{4}$,
\newauthor{Marcos Pellejero-Ibañez$^{1}$}
\\
% List of institutions
$^{1}$Donostia International Physics Center, Manuel Lardizabal Ibilbidea, 4, 20018 Donostia, Gipuzkoa\\
...
$^{4}$Kavli Institute for the Physics and Mathematics of the Universe (WPI), The University of Tokyo Institutes for Advanced Study (UTIAS), \\
...
They key assumption behind our work is that the shape field will evolve from Lagrangian to Eulerian space only by having its amplitude rescaled by the local volume contraction or expansion, but without suffering any shear. This can be expressed by the equation
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:advect}
\end{equation}
which is simply a reflection of the conservation law in Eq. (\ref{eq:shape_conservation}); in this expression, $\mathbf{\psi}$ is the displacement field, obtained through any calculation method available (e.g.: perturbation theory or simulations). Our goal is to test how well such a model will be able to reproduce realistic measurements of shape correlations, thus implicitly testing the assumptions made earlier.
...
\end{equation}
in which the sum over $\alpha$ is in fact over all objects available, making it clear that $\Tilde{g}$ is in fact the density-weighted reduced shape field. Let us assume a halo population with density field 1+$\delta_h$ and reduced-shape field $g^h_{ij}$, then the density-weighted reduced shape field is given by
\begin{equation}
...
To study the intrinsic alignment signal we will decompose the reduced shape tensor $g_{ij}$ into two measures of ellipticity, equivalent to the Stokes' parameters in the study of light polarization,
\begin{equation}
    \begin{split}
    ...
    \label{eq:proj}
\end{equation}
but these definitions still have the disadvantage of being dependent on the particular coordinate system chosen. Fourier transforming these quantities one can define the following combinations,
\begin{equation}
    \begin{split}
...
We begin by reviewing the linear-alignment (LA) model for intrinsic alignments \citep{Catelan_2001, Hirata_2004}. This model assumes that the intrinsic shear of galaxy shapes is proportional to the line-of-sight projected tidal tensor, 
\begin{equation}
...
\begin{equation}
    ...
    \end{split}
\end{equation}
leading to the following predictions for the power spectra of $E$- and $B$- modes of the intrinsic alignment field
\begin{equation}
    \begin{split}
...
\begin{enumerate}
    ...
    \item W subtract the Lagrangian positions of particles from their Eulerian positions at the relevant redshift, thus obtaining the displacement field $\mathbf{\psi}_i = \mathbf{x}_i(z) - \mathbf{q}_i$ of the $N$-body simulation;
...
The simulations used in this work are summarized in Table \ref{tab:cosmologies}, and are part of the BACCO simulations, described in \citep{Contreras_2020, Angulo_2021, Zennaro_2021}. These are gravity-only simulations run using a modified version of \verb|L-GADGET-3| \citep{Springel_2005, Angulo_2021}. The initial conditions are computed at $z=49$, and the simulation is evolved until $z=0$. At each output redshift we have stored FoF groups and \verb|SUBFIND| subhalos. 
...
In this section we discuss briefly the methods used to measure the $E$- and $B$- mode shape power spectra of IA \citep{Kurita_2020}. Let $S$ be the shape tensor of a halo, then we can define its ellipticity as
\begin{equation}
    \begin{split}
...
\end{equation}
in which $\mu=\hat{\mathbf{k}}\cdot \hat{\mathbf{n}}$ is the cosine of the angle between the wavevector $\mathbf{k}$ and the line-of-sight direction and $\Sigma_i$ represents the region of a Fourier-space shell with radius in the range $[k_i-\frac{\Delta k}{2}, k_i+\frac{\Delta k}{2}]$.
...
\begin{equation}
    \begin{split}
        \rm{Cov}&\left(P_{XY}^{(\ell)}(k_i), P_{X'Y'}^{(\ell)'}(k_j)\right) = \frac{(2\ell+1)(2\ell'+1)}{N_i^2}  \delta_{ij} \\
...
As previously stated in the text, the values of the bias parameters obtained in this work cannot be directly compared to those reported in \cite{Akitsu_2021, akitsu2023quadratic}; let us explore in more detail why that is. In these works, their parameter of interest, $b_K$, is defined via the expression
\begin{equation}
...
\end{equation}
and there is a factor of $2$ difference in the definitions of the bias parameters. Therefore, our bias parameter will be related to the one of \cite{Akitsu_2021, akitsu2023quadratic} by
\begin{equation}
    c_s = 3 b_K.
...
In this work we will employ three closely connected quantities, which can therefore be confused. To avoid this from happening, we will clarify their definitions and make explicit their relations to each other. The shape tensor, $S_{ij}$ has already been defined in equation \eqref{eq:shape_tensor}. It is closely related to $g_{ij}$, which we label the reduced shape tensor, and which is defined by
\begin{equation}
    g_{ij}(\mathbf{x}) = \frac{1}{S_0^{2\rm{D}}}\left( S_{ij} - \frac{1}{2}\delta^K_{ij}S^{2\rm{D}}_0 \right),
\end{equation}
...
\begin{equation}
    \gamma^I(\mathbf{x}) = \left( g_{11}-g_{22}, 2g_{12} \right).
\end{equation}

\section{Fiducial Parameter Estimates}

The fiducial values of $c_s$ and $A_{SN}$, measured according to what was described in \ref{sec:covariance_matrix} are summarised in tables \ref{tab:nenya_fid_bias_parameters}, \ref{tab:narya_fid_bias_parameters} and  \ref{tab:theone_fid_bias_parameters}, corresponding to the halo populations extracted from simulations with Nenya, Narya, and TheOne cosmologies respectively.
...
\begin{table}
 ...
 \begin{tabular}{ccccc}
  \hline
   & $c_s$ & $\sigma_{c_s}$ & $A_{SN}$ & $\sigma_A$ \\
   \hline
   $M_1$ & $-0.0352$ & $0.0047$ & $5.459$ & $0.093$ \\
   $M_2$ & $-0.057$ & $0.013$ & $16.71$ & $0.37$ \\
   $M_3$ & $-0.113$ & $0.042$ & $56.1$ & $1.1$ \\
   $M_4$ & $-0.202$ & $0.057$ & $165.9$ & $2.8$ \\ 
  \hline
 \end{tabular}
 \label{tab:nenya_fid_bias_parameters}
\end{table}

\begin{table}
 \caption{Fiducial model parameters for each of the halo populations extracted from the mid-sized Narya simulation. The mass ranges of these samples are defined in Table \ref{tab:mass_samples}.}
 \begin{tabular}{ccccc}
  \hline
   & $c_s$ & $\sigma_{c_s}$ & $A_{SN}$ & $\sigma_A$ \\
   \hline
   $M_1$ & $-0.0286$ & $0.0067$ & $4.142$ & $0.090$ \\
   $M_2$ & $-0.041$ & $0.016$ & $11.69$ & $0.16$ \\
   $M_3$ & $-0.063$ & $0.030$ & $36.91$ & $0.5$ \\
   $M_4$ & $-0.097$ & $0.023$ & $102.6$ & $1.6$ \\ 
  \hline
 \end{tabular}
 \label{tab:narya_fid_bias_parameters}
\end{table}

\begin{table}
 \caption{Fiducial model parameters for each of the halo populations extracted from the mid-sized TheOne simulation. The mass ranges of these samples are defined in Table \ref{tab:mass_samples}.}
 \begin{tabular}{ccccc}
  \hline
   & $c_s$ & $\sigma_{c_s}$ & $A_{SN}$ & $\sigma_A$ \\
   \hline
   $M_1$ & $-0.0319$ & $0.0049$ & $5.170$ & $0.072$ \\
   $M_2$ & $-0.0399$ & $0.0093$ & $15.28$ & $0.11$ \\
   $M_3$ & $-0.067$ & $0.019$ & $48.8$ & $0.4$ \\
   $M_4$ & $-0.125$ & $0.052$ & $138.6$ & $1.8$ \\ 
  \hline
 \end{tabular}
 \label{tab:theone_fid_bias_parameters}
\end{table}
