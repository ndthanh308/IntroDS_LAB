...
\begin{document}
	...
	Having nearly confirmed that the measurements have a GW origin, the exciting possibility that this background signal is being sourced by fluctuations in the early Universe has been discussed in the literature \cite{10.1093/mnrasl/slaa203,Vagnozzi:2023lwo}. 
	...
	In \cite{Vagnozzi:2023lwo} the author explores the possibility of inflationary origins of the nHz GW signal and concludes that an inflationary origin is less tenable. He concludes that for the nHz GW signal to have primordial origins power spectrum has to be blue-tilted with a spectral index of $n_t=1.8\pm 0.3 $. However, this analysis is assuming instant reheating of the Universe after the inflationary phase. The effects of reheating on the GW spectrum of inflation have been discussed in literature over the years \cite{Martin:2014nya,Dai:2014jja,Lozanov:2019jxc}. It has been observed that the spectral index and amplitude of the GW signal are affected by the duration of the reheating era and also by the equation of state $w_{re}$, during the reheating phase \cite{Boyle:2007zx}. In this article, we examine the possibility of a primordial origin of the nHz GW signal in the presence of a non-trivial reheating phase, while remaining agnostic about the early universe paradigm that had been realized in Nature. 
	
	We discuss the tenability of a primordial origin of the nHz signal in this renewed context. We perform a likelihood analysis of the data based on compressed likelihoods from \cite{Vagnozzi:2023lwo}. Slow-roll inflation, where $n_t\simeq 0$ will require a stiff reheating phase of $w_{re}\simeq 19/3$. A blue tilted spectrum with $n_t\sim 1.77$ is obtained for instant reheating in accordance with what we expect from previous works. Finally, for open reheating equation of state $w_{re}$ and tensor tilt $n_t$ results in $n_t=1.94^{+0.43}_{-0.88}$ and $w_{re}=0.36^{+0.15}_{-0.28}$. In all cases, the reheating temperature is expected to be ``low", $T_{re}\leq 10^5 GeV$. Thus,  within the primordial interpretation of the signal, it seems instantaneous reheating is favored, and a primordial blue-tilted tensor spectrum. This disfavors slow-roll inflationary scenarios and favors other paradigms like a bounce \cite{Battefeld:2014uga,Khoury:2001wf}, or GW spectrum generated by sourced fluctuations \cite{Barnaby:2011qe,Ben-Dayan:2016iks,Ben-Dayan:2018ksd}. We briefly discuss the possibility of bounce being the origin of GW signal in the future directions section.
	...
	\end{eqnarray}
	where $f = \frac{1}{2 \,\pi} \frac{k}{a_0}$ is the present-day physical frequency of a GW associated with the comoving wavenumber $k$. The present-day GW energy spectrum $\Omega_{GW}^{\text{prim}}(f)$ is then related to the primordial tensor power spectrum  $ \mathcal{P}_{T}^{\text{prim}} (f)$ via
	\begin{eqnarray}
	...
	\end{eqnarray}
	Using the tensor transfer function $T_{h} (f)$ mentioned in eq. (\ref{eq: tensortransfer}), the present-day GW energy spectrum is written as 
	\be
	...
	\ee 
	where the factors $C_2 (f)$ and $C_3 (f)$ are defined as 
	\begin{eqnarray}
	...
	\end{eqnarray}
	with $\Omega_{fs} \equiv \frac{\rho^{fs}}{\rho^{crit}} $ is the fraction of the  relativistically free-streaming critical density at redshift $z_{re}$.
	...
	\begin{eqnarray}
		\Omega_{GW}^{\text{PTA}}(f) &=& A^{2} \frac{2\pi^{2}}{3H_0^2} \, \frac{f^{5 -\gamma}}{yr^{\gamma -3}} \,.
		\label{eq: GWPTA}
	\end{eqnarray}
	
	PTA data from NANOGrav gives constraints on $A$ and $\gamma$ as a joint posterior distribution. The collaboration inferred the values of $A = 6.4 ^{+4.2}_{-2.7} \times 10^{-15}$ and $\gamma = 3.2 \pm 0.6$ at 2 $\sigma$ CL. Let us connect these quantities to early universe parameters. Equating the eq.(\ref{eq: GWInfl}) with eq.(\ref{eq: GWPTA}) gives the following relation of $\gamma$ with $n_t$ and $w_{re}$ 
	\begin{eqnarray}
	...
	So far in the analysis, the spectrum of gravitational waves is assumed to have a constant spectral index, i.e. the spectral index is independent of the scale. The tensor spectrum of inflation typically has a running of the spectral index, albeit extremely small. The running of the spectrum for tensor perturbations $\alpha_{n_t}$ is defined as 
	\begin{equation}
		\alpha_{n_t} = \dfrac{d^2\log \mathcal{P}^{\text{prim}}_T}{d\log k^2}|_{k=k_*}. 
		\label{eq:running}
	\end{equation}
	...
	%at these scales, $\Omega_{\text{GW}} \leq 1.7 \times 10^{-8}$ \cite{KAGRA:2021kbb}. 
	...
	\begin{equation}
		n_t(f) = n_t(f_{\text{NG}}) + \alpha_{n_t} \log\left( \frac{f}{f_{\text{NG}}}\right)
	\end{equation}
	where $f_{\text{NG}}$ is the frequency mode corresponding to the NANOGrav data ($f_{\text{NG}} \sim 1 yr^{-1} \sim 3.17 \times 10^{-8}$Hz). We solve equation \eqref{eq:gwligo} with parameters obtained from the likelihood analysis performed in section \ref{sec:Discussion}. The results of our analysis are demonstrated in the table (\ref{tab:alpha}).
	\begin{table}[H]
		\centering
		...
		\vspace{1 em}  \\
		{
			\begin{tabular}{|c|c|c|c|c|}
				...
				$ w_{re}$        & $ 0.36^{+0.15}_{-0.28} $ &$0 $ & $1/3$ & $ 1$  \\
				$\alpha_{n_t}$ & $ -0.049^{-0.048}_{+0.019} $ &$ -0.040^{-0.013}_{+0.005} $ & $-0.043^{-0.008}_{+0.006}$ & $ -0.045^{-0.009}_{+0.084}$  \\
				\hline
				\hline
