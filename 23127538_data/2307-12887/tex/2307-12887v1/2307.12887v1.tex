...
\hspace*{6mm}
In sec.\,\ref{CPOLMSB} we construct the POL for a massive scalar boson.  The result is the explicit formula in theorem (\ref{APOL}), which  furnishes myriads of  different POL.  
In order to  recognize  their  features it is very effective to describe a POL  in a concise manner by its kernel.  A useful characterization  and structural analyzes of the POL  kernels are given by the theorem (\ref{CKPOLMB}).\\
\hspace*{6mm}
...
that their POL  are causal.  Moreover, we mention particularly    the considerations by Werner  \cite{W21}, which  suggest  that most probably these are the only causal POL. Therefore we refer quite simply to their kernels (\ref{KKK}) as the causal kernels.\\
\hspace*{6mm}
...
\hspace*{6mm}
The appendices \ref{AppA} and \ref{EQOD} provide space to  some cumbersome technical details, whereas  the appendices \ref{AppB} and \ref{AppC} house all which  regards the CT criterion (\ref{GCCT}) and the result on  CC  (\ref{GCCPOL}), repectively.
...
A projection valued measure (PM), which is Euclidean covariant, is called a Wightman localization (WL). By Mackey's imprimitivity theorem  every WL is  Hilbert space isomorphic to a rep of $\tilde{\mathcal{E}}$ induced from the subgroup $SU(2)$ together with the related  system of imprimitivitiy.\\
...
given by\footnote{Often one uses $ \e^{-\i \mathfrak{a}\cdot \,\mathfrak{p}^{\eta}}$.}
\begin{itemize}
...
for $p\ne 0$, where $\epsilon(p)=\sqrt{m^2+p^2}$,  $\mathfrak{p}^{\eta}=(\eta\epsilon(p),p)$, $\mathfrak{q}^\eta= (q_0^\eta,q^\eta):=A^{-1}\cdot \mathfrak{p}^\eta$, and $R$ the Wigner rotation with respect to the canonical cross section satisfying  $R( \mathfrak{p}^\eta,B)=B$ for $B\in SU(2)$, see \cite[sec.\,3.3 (1)]{C17}.
...
A positive operator valued measure $T$  on the Borel sets $\mathcal{B}(\R^3)$ (POM), which is Euclidean covariant, is called a PO-localization (POL). Recall that a POM  $E$ constituted by othogonal projections is denoted by PM, and a Euclidean covariant PM is called a Wightman localization (WL).\\
\hspace*{6mm}
...
sec.\,\ref{IRPP}.\\
\hspace*{6mm}
Regarding parts of  (\ref{CPCPOL}) see e.g. \cite[sec. II]{W86}  for the general theory.\\
...
Obviously the invariant subspaces $V_{\e}$ and  $V_{\e'}$ for the spinor choices $\e$ and $\e'$ are equal if and only if $\e$ and $\e'$ are equivalent, i.e.,
 $\e'(\rho)=s(\rho)\e(\rho) \text{ for some } s(\rho)\in \C\setminus\{0\} \text{ for almost all } \rho>0$.\\
\hspace*{6mm}
...
If $\e$ and $\e'$ are equivalent, then $(W^{m,0,\eta}|_{\tilde{\mathcal{E}}}, T_{\e})$ and $(W^{m,0,\eta}|_{\tilde{\mathcal{E}}}, T_{\e'})$ are unitarily equivalent. Indeed, by assumption $\e'(\rho)=s(\rho)\e(\rho) \text{ for some } s(\rho)\in \C\setminus\{0\} \text{ for almost all } \rho>0$. Since
$\norm{\e(\rho)}=\norm{\e'(\rho)}=1$,  one has  $|s(\rho)|=1$ and $s(\rho)=\langle \e(p),\e'(\rho)\rangle$ for almost all $\rho$, whence $s$ is measurable. Therefore 
...
The NWL  $E^{NW}$ is given  by $\e=1$. About the uniqueness of $E^{NW}$ see \cite[sec.\,2.2]{W21}, \cite[sec.\,2]{C17}.
...
on $\R^3\setminus\{0\}$ with $\textsc{t}_{\e}(p,p)=1$ for all $p$. It is also rotation invariant, i.e.,  $\textsc{t}_{\e}(Rk,Rp)= \textsc{t}_{\e}(k,p)$ for all rotations $R$, as we will see explicitly below. Clearly  $T$ and its kernel $\textsc{t}$ determine each other uniquely.\\
...
\end{Cor}\\
 ...
 It remains to show (\ref{POLMB4}).  Note $D^{(j)}(M)_{00}=\sum_{i=0}^{j} {j\choose i}^2(M_{11}M_{22})^{j-i}(M_{12}M_{21})^i $ for $j \in \N_0$ and $M\in M_2(\C)$, whence $D^{(j)}(M)_{00}=\sum_{i=0}^{j} {j\choose i}^2|M_{11}|^{2(j-i)}\big(|M_{11}|^{2}-1\big)^i$ for $M\in SU(2)$. So using the explicit expression for $B(p)$ \cite[Eq.\,(27.8)]{C17}, one finds $|M_{11}|^2=\frac{1}{2}\big(1+\frac{kp}{|k||p|}\big)$ for $M:=B(k)^{-1}B(p)$.\qed\\
...
In particular  with respect to the shell representation the kernel of the NWL $E^{NW}$ reads $\sqrt{\epsilon(k)\epsilon(p)}$, see \cite[sec.\,2.2]{W21}. 
...
\end{equation} 
consider the properties    (i) to be  measurable,   (ii)  to be a positive definite kernel, (iii) to be normalized, i.e.,  $K(p,p)=1$ for all $p$, and (iv)  to be  rotation invariant, i.e., $K(Rk,R p)=Kk,p)$ for all $k,p$ and rotations $R$.  
Recall that (ii) implies 
\begin{equation}\label{CSK}
|K(k,p)|^2\le K(k,k)K(p,p)
\end{equation}
 whence, if (iii) holds, $|K|\le 1$.

\begin{Pro}\label{TCK} Let $K$ satisfy \emph{(i)-(iii)}. Then
...
{\it Proof.} Let $\mathcal{H}_J$  be the RKHS associated to the kernel $K$ (see e.g.\,\cite{F08}) and put $J_p:=K(\cdot, p)$. Since $\norm{J_p}^2=K(p,p)=1$, $j$ is an injection. Hence $T$ is a POM. Its kernel is $K$, since 
$\langle \phi',T[g]\phi\rangle=(2\pi)^{-3}\int \d^3x \,g(x)\int\int \d^3p\,\d^3k\,\langle J_k,J_p\rangle\,\e^{\i(p-k)x}\overline{\phi'(k)}\phi(p)$ for $\phi,\phi'\in L^{2}\cap L^1$, $g\in L^\infty$,
as is easily verified. By this formula for $g=1_\Delta$  translation covariance of $T$ is  obvious.\qed\\

 Obviously every POL kernel $\textsc{t}_{\e}$ satisfies (i)-(iv). The converse holds true, too. 


\begin{Cor}\label{EPDKKPOL} Let $K$ satisfy  \emph{(i)-(iv)}. Then \emph{(\ref{AKPOLMB})} is a Euclidean covariant POM with respect to $U^{(0)}$. Hence  $(W^{m,0,\eta}|_{\tilde{\mathcal{E}}},T)$ 
...
\textsc{k}:\;]0,\infty[\times  ]0,\infty[ \times [-1,1]\to \C,\quad \textsc{k}(\sigma,\rho,x):=K\big(\sigma(1,0,0),\rho(x,\sqrt{1-x^2},0)\big)
\end{equation}
Due to rotation invariance  one has $K(k,p)=\textsc{k}\big(|k|,|p|,\frac{kp}{|k||p|}\big)$  for all $k,p\ne 0$. 

\begin{The}\label{CKPOLMB}  The kernels of the POL of the massive scalar boson are exactly the measurable rotational invariant positive definite normalized  kernels $K$ on $\R^3\setminus\{0\}$. The latter  are given by 
\begin{equation}\label{PDEC}
K(k,p)=\sum_{j=0}^\infty k_j(|k|,|p|)P_j\left(\frac{kp}{|k||p|}  \right)
\end{equation}

for all $k,p\ne 0$ with $(k_j)_j$ any sequence  of measurable positive definite kernels $k_j$  on $]0,\infty[$  (not excluding $k_j=0$ for some $j$) satisfying $\sum_jk_j(\rho,\rho)=1$ for all $\rho$. 
...
\end{equation}
{\it is the kernel of the finite POL  with CT} 
...
The proof is postponed to sec.\,\ref{AppA}.
...
\subsubsection{Two spinor choices for $T^{tct}$}\label{TVFTCT}
 (a) {\it The spinor choice $\e$  for which $T^{tct}=T_{\e}$  according  to the construction in \emph{sec.\,\ref{CPOLMSB}} is 
...
(b) Following (\ref{AKPOL}) one obtains immediately $T^{tct}=T_{\e'}$ for
...
CT for a POL  is a requirement of the fact that there is no propagation faster than light. Clearly limited propagation affects  all kinematical transformations. For instance, according to NWL, if   a massive particle localized in a bounded region  is subjected to a boost of however small rapidity then  there is a non-vanishing probability to observe the particle arbitrarily far away. This  frame-dependence of NWL (already described by Weidlich, Mitra 1964 \cite{WM63}  and still currently discussed, see e.g. \cite[3]{C17})  is an acausal behavior. In fact,  causality  requires from 
 localization that the probability of localization in a region of influence can not be less than that in the region of actual localization. \\
\hspace*{6mm}
...
T(\Delta)\le T(\Delta_\sigma) \tag{CC}
\end{equation*}
holds  for all $\Delta\in\mathfrak{S}$, $\sigma$ spacelike hyperplane. The causality condition CC is thoroughly studied in \cite{C17}. There causal POL for the Dirac electron and the Weyl fermions are revealed.  Apparently the causality condition  has  not  been analysed  elsewhere.  Recently, along a tentative \textquotedblleft new and different  operational interpretation of the  notion  of spatial position\textquotedblright\,   in \cite{M23} a causality condition corresponding to CC is shown to hold.
\\
\hspace*{6mm}
Let us verify explicitely that CT    is a special case CC. Indeed,  for $\Delta\subset \{0\}\times \R^3$, $t\in\R$, and $\sigma:=\{-t\}\times \R^3$ check $\Delta_\sigma=(-t,0,I_2)\cdot \Delta_t$, whence  CT by covariance of $T$.

\subsection{Causal kernels}\label{KKO}


 As argued above CC, which implies CT, is an  indispensable property for a localization. For this  one is highly  interested in the  kernels of causal POL.  We study  the promising class  $\mathcal{K}$ of  conserved covariant POL kernels (\ref{CKKK})  introduced   by Petzold et al.\,\cite{GGP67}. Indeed,   (\ref{GCCPOL}) shows that a POL with kernel $K\in\mathcal{K} $ is causal. Moreover, as emerges from the considerations by Werner  \cite{W21}, most probably $\mathcal{K}$ comprises even all kernels of causal POL. 
...
\hspace*{6mm} 
 ...
 Let us report the results, which in the literature are  achieved so far.   Put 
 \begin{equation}\label{KKKE}
 g_r(t):=(2m^2)^r(m^2+t)^{-r}, \;t\ge1, \;r>0
 \end{equation}
...
Further solutions are the pointwise limits of their convex  combinations as  e.g. $K$ determined by $g(t):=\big(\frac{(1+c)m^2}{cm^2+t}\big)^n$ for $-1<c\le1$, $n=2,3,\dots$. Note  that $\frac{(1+c)m^2}{cm^2+t}\le g_1(t)$.
 \item[(b)] 
 One observes \cite{W21}  that 
 \begin{equation}\label{GMWRTK}
 K\breve{h}\in\mathcal{K}
 \end{equation} 
 for every   $K\in\mathcal{K}$ and continuous  $h:[m^2,\infty[\to \R$ with $h(m^2)=1$, $\breve{h}$ positive definite.
\item[(c)] 
By  \cite{W21} one has the important implication
\begin{equation}\label{KIPDG}
 K\in\mathcal{K} \Rightarrow \;\breve{g} \text{ \,positive definite}
 \end{equation}
Indeed,
$\breve{g}(k,p)
=\frac{2\,\sqrt{\epsilon(k)\epsilon(p)}}{\epsilon(k)+\epsilon(p)}K(k,p)$ and by  the formula from  \cite{W21} 
...
\end{equation}
 ...
 \item[(d)] 
 For $K$ in (\ref{KKK}), one has $|K|\le 1$  if and only if  $|g|\,\le \,g_{1/2}$.  This is shown in \cite{W21} and follows from  (\ref{PDDOO})(b). Hence
...
As recommended by \cite[footnote to (3.2)]{GGP67} and since there is still some interest in this topic  \cite{KHB18}, we look first at the instructive case of one spatial dimension.
 Here, indeed,   $g_{1/2}$ arises naturally and turns out to furnish  the maximal solution $K^{1}_{1/2}$ (\ref{PDDOO})(c). A complete description of the  causal kernels is given in  (\ref{MROD}). Accordingly,  the product of $K^{1}_{1/2}$ with a normalized positive definite kernel is a causal kernel and every causal kernel is of this kind.
...
\end{equation}
 ...
 is called a \textbf{causal kernel } if it is positive definite. Put $\tilde{g}(k,p):=g\big(\epsilon(k)\epsilon(p)-kp\big)$.\\
 \hspace*{6mm}
...
(c) $G\in\mathcal{G}$ is normalized, i.e., $G(p,p)=G(0,0)=1$ for all $p\in \R^3$, whence $G$ is bounded by $1$ (\ref{CSK}). \\
 \hspace*{6mm}  
(d)    iI $G$, $G' \in\mathcal{G}$, then also $GG'\in\mathcal{G}$. Moreover, if  $G$ is the pointwise limit of  a sequence  in $\mathcal{G}$, then      $G\in\mathcal{G}$.\\
 \hspace*{6mm}
...
Note that in (\ref{IKFR})  the carrier space $\mathcal{H}_U$ of $U$ need not be separable.  One concludes
...
\end{itemize}
{\it Proof.} By (e) let without restriction $\mu^\textsc{p}(]0,\infty[)=0$. Fix $\kappa>0$. Recall $\operatorname{sinch}(y)=\frac{\sinh y}{y}$ for $y\ge 0$  is strictly increasing. It suffices to schow $\int_{]0,1]}\frac{\operatorname{sinch}x \kappa}
...
Then going backwards the prior proof, $\frac{\Psi(\kappa)}{\sinh (\kappa)}$ equals $g(t)$ in $(\beta)$ for $\mu$ given in $(\delta)$.\\
\hspace*{6mm} Finally, obviouly $g$ and $\psi$ determine each other. Hence so do  $g$, $\psi$, $\mu_{even}$ and, by ($\delta)$, also $\mu$.\qed
...
\begin{The}\label{MTKK} $|K|\le K_{3/2}$ pointwisely for all  kausal kernels $K$.
\end{The}\\
...
\begin{Cor}\label{SPOL} The POL  with CT  $T^{tct}$   \emph{(sec.\,\ref{TCT})} and $T^{TM}$   \emph{(sec.\,\ref{POLTM})},
are separated. Every $k_0\in\R^3\setminus\{0\}$ yields a point localized sequence of states by \emph{(\ref{PLSS})}.
\end{Cor}

{\it Proof.}
(a) As easily checked $\textsc{t}^{tct}$  (\ref{KPOLCT}) satisfies   (\ref{SSL0})(i)-(iii). In particular one has $\lim_{\lambda\to\infty}\textsc{t}^{tct}(k,\lambda p)=\frac{1}{2}\big(1+m/\epsilon(k)\big)^{1/2}+\frac{1}{2|p|}\big(\epsilon(k)(m+\epsilon(k))\big)^{-1/2}kp>0$ for all $k,p$.\\
(b) Similarly, $\textsc{t}^{TM}$  (\ref{KTM}) satisfies   (\ref{SSL0})(i)-(iii). In particular,  $\lim_{\lambda\to\infty}\textsc{t}^{tct}(k,\lambda p)=
...
\hspace*{6mm}
In search of the best POL we recall the observation that $KG\in\mathcal{K}$ if $K\in\mathcal{K}$ and $G\in\mathcal{G}$ (\ref{LIPGK}). Probably multiplying $K$ by  $G$ deteriorates the localization features of the corresponding  POL. \cite{W21} call $G$  a noise factor. More generally, if $K,K'\in\mathcal{K}$ with $K'\le K$, then $K$ is better than $K'$? 
...
\hspace*{6mm}
 The  physically reasonable result would be that a more massive scalar boson is better localizable in bounded regions than a less massive scalar boson.
...
\textbf{Acknowledgements} 
 ...
 I am indebted to R.F. Werner having drawn my attention to this topic and having shared  his results with me, and I am very grateful to V. Moretti  for  valuable discussions and suggestions, and above all because of  his work \cite{M23} which enabled me to accomplish this work.
...
(e) Finally let $\Delta$ be any measurable set. Since $\mu$ is tight, there are compact $K^{(i)}\subset \Delta$ satisfying $\mu(\Delta\setminus C)=0$ for $C:=\bigcup_i K^{(i)}$. By (d), (1) holds for $C^{(n)}:=\bigcup_{i=1}^nK^{(i)}$. Now proceed as in (c). Hence (1) holds for $\Delta$.\qed\\
 ...
 The applications  of (\ref{MLCC}) in (\ref{GCCT}) and (\ref{GCCPOL}) regard the case $T'=T$ as they  concern  a POL $T$ of the massive scalar boson, which due to the Poincar\'e covariance  (\ref{PCEPOL}) are defined on all spacelike hyperplanes. Moreover the particular case  in (\ref{MLCC})  of parallel hyperplanes $\varepsilon$ and $\sigma=\{t\}\times \R^3$ for $t\in\R$  and for  $V=\R^3$ is used for the CT criterion below.
...
\section{Result on CC}\label{AppC}
...
\begin{itemize}
\item[(i)]  $(\mathfrak{k}-\mathfrak{p})\mathfrak{v}(k,p)=0$
...
\item[\emph{(c)}] $\mathfrak{v}$ is conserved, iff  \,   $\mathfrak{v}(k,p)=\gamma(\mathfrak{k}\cdot \mathfrak{p}) \,(\mathfrak{k} + \mathfrak{p})$ for some unique $\gamma:[0,\infty[\to \C$.
\item[\emph{(d)}] $v_0$ is Hermitian, iff \,$\mathfrak{v}(k,p)= g(\mathfrak{k}\cdot \mathfrak{p})(\mathfrak{k} + \mathfrak{p}) +\i f(\mathfrak{k}\cdot \mathfrak{p})(\mathfrak{k} - \mathfrak{p})$ with unique  $g,f:[0,\infty[\to \R$.
...
{\it Proof.} (a) Verify $v_i(k,p)=\sqrt{2}\,v_0(p,k) - v_0\big(L_i^{-1}\mathfrak{k},L_i^{-1}\mathfrak{p}\big)$ with 
$L_i$  the boost along the $\i$th     direction with velocity $1/\sqrt{2}$ evaluating the equation $v_0(k,p)=L(1,0,0,0)\,\cdot \mathfrak{v}(L\mathfrak{k},L\mathfrak{p})$ for $L=L_i$.\\
...
$$\mathfrak{z}:=\sum_{a,b}\overline{c}_ac_b\,\mathfrak{v} (p_a, b_b)$$
...
Then there  is a continuous $g:[1,\infty] \to \R$ such that 
...
\hspace*{6mm}
Conversely let $g:[1,\infty] \to \R$  be measurable such that the right hand side of \emph{(\ref{CKKK})} is positive definite. Then the latter is a conserved covariant POL  kernel and $g$ is continuous.
\end{Cor}\\
...
\emph{(b)}  Let $k\in L^1(\R)$ with its Fourier transform $\hat{k}\in L^1(\R)$, $\hat{k}\ge 0$; then $k$ is even and $\tilde{g}$ for $g:=k\circ \cosh^{-1}$  is positive definite.\footnote{$\cosh^{-1}(t) =\ln\big(t+\sqrt{t^2-1}\,\big)), \,t\ge 1$}\\
 \hspace*{6mm}
\emph{(c)}   Consider $ k\in L^1(\R)\cap L^2(\R)$. Then $k$ is equal a.e. to a continuous function of  positive type if and only its Fourier transform is real nonnegativ.\\
 \hspace*{6mm}
...
Hence $\langle\phi,k\ast \psi\rangle=\langle\mathcal{F}^{-1}\phi,\mathcal{F}^{-1}(k\ast \psi)\rangle=\langle u, (\mathcal{F}^{-1}k)v\rangle$. For $v=u$ this implies $(2\pi)^{-1/2}\int\int k(x-y)\overline{\phi(y)}\phi(x)\d x\d y=\int |u(x)|^2\,(\mathcal{F}^{-1}k)(x)\d x$.  The right hand side is non-negative for all $u$ if and only $\mathcal{F}^{-1}k\ge 0$ a.e. Therefore, looking at the left hand side, $k$ is of weak positive type if and only if $\mathcal{F}^{-1}k\ge 0$ a.e. or equivalently $\mathcal{F}k\ge 0$ a.e.  Recall \cite[Proposition after Theorem IX.10]{RS75}.
...
\bibitem{K76}  Kirillov, A.A.: {\it Elements of the Theory of Representations}. Springer-Verlag,  Berlin  (1976)

\bibitem{K15} Kuznetsova,  Y.: {\it On Continuity of Measurable Group Representations and Homomorphism}. arXiv: 1010.0999v3 (2015)

\bibitem{M95} Mattila, P.: {\it Geometry of Sets and Measures in Euclidean Spaces}. Cambridge University Press (1995)
...
\bibitem{W21} Werner, R.F.: {\it Some ideas about relativistic localization} (30.12.2021)
