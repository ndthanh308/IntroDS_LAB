...
\end{equation}
where, e is the charge of the electron and \textbf{r} being the spatial coordinates. The position operator in the length gauge approach is linked to the Berry connection \bm{$(\xi)$} when the bands are distinct under external electrical field.
In the context of distinct bands, the Berry connection may capture the phase relationship between them when system undergoes changes in k-space.
...
-e\textbf{E}(t).[\textbf{r},\rho_{n_1 n_2}(t)] |n_1\rangle\langle n_2|\\
...
\widetext
\maketitle
\definecolor{dimmedtext}{RGB}{70,70,70}
\begin{center}
