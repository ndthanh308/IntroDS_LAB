...
\title{Unconventional optical response in monolayer graphene \\upon dominant intraband scattering}% Force line breaks with \\
...
\textit{Semi-classical Approach.\textbf{\textemdash}}The SBE consists of perturbed Hamiltonian with radiation-interaction and scattering terms to study many-body interactions. For the graphene, unperturbed tight binding Hamiltonian ($H_0$) deals with the nearest neighbour hopping with zero onsite energies. 
\begin{equation}
\hat{H}_0=\sum\limits_{n}^{}\sum\limits_{k}\varepsilon_{nk}c_{nk}^{\dag}c_{nk}
\end{equation}
The energy eigenvalues are denoted by $\varepsilon_{nk}$ and the coefficients $c^\dag$,$c$ are creation and annihilation operators with states n=[1,2] in the momentum space (k). Previously, the light matter interactions were treated with velocity gauge, but challenges arise for many body interactions because of the spurious divergences at lower frequencies due to its sensitivity and susceptibility to numerical inaccuracies \cite{Peres17}. These divergence issues can be easily tackled by using the length gauge through sum rules \cite{sipe_gahramani} and gauge invariant electric field, which emerged as a pivotal solution to the problem. However, it does possess an additional scalar dependence that disturb the diagonal property of the perturbed Hamiltonian in k-space. Because of that, there will be some overlap or coupling contributions which can be eliminated by the band isolation technique \cite{Aversa_Sipe}. 

The light-matter interacting term in the Hamiltonian $(\hat{H}_{eR})$ possesses interaction between electric field of the light  and  charge distribution of the matter. By considering electric dipole approximation along with long wavelength limit for uniform illumination, we have employed the length gauge approach with explicit dependence on external electric field \textbf{E}(t) given by,
\begin{equation}  
\hat{H}_{eR}=-e\textbf{E}(t).\textbf{r}
\end{equation}
where, e is the charge of the electron and \textbf{r} being the spatial coordinates. The position operator in the length gauge approach is linked to the Berry connection $(\xi)$ when the bands are distinct under external electrical field. Close proximity of the Dirac cones (Fig.\ref{Energy_near_Dirac_points}) is no longer valid due to presence of external electric field radiation within the light-matter Hamiltonian \cite{CSV15} given by,
\begin{multline}
\hat{H}_{eR}=-e\textbf{E}(t).\sum\limits_{n_1}^{}\sum\limits_{n_2}\sum\limits_{k}c_{nk}^{\dag}\xi_{n_1n_2k}c_{nk}-\\ie\textbf{E}(t).\sum\limits_{n}^{}\sum\limits_{k}\nabla_{k}c_{nk}^{\dag}c_{nk}
\label{light-matter}
\end{multline}
...
To depict this different band characteristics, the above Eq.(\ref{Blount}) is extended for each k values to perceive the required collisional multiband Boltzmann equation (CMBE),
%\begin{widetext}
\begin{multline}
i\hbar\frac{\partial {\rho_{n_1 n_2k}}(t)}{\partial t}=(\varepsilon_{n_1k}-\varepsilon_{n_2k})\rho_{n_1 n_2k}(t) \\
-ie\textbf{E}(t)\frac{\partial\rho_{n_1n_2k}}{\partial k} - e\textbf{E}(t)\sum\limits_{n}\left(\xi_{n_{1}nk}\rho_{nn_2k} \right. \\
\left.- \rho_{n_1nk}\xi_{nn_2k}\right)-i\Gamma_{n_1n_2k}\rho_{n_1n_2k}(t)
\label{CMBE}
\end{multline}
...
\textit{Conductivity Expression.\textbf{\textemdash}}Composition of the j$^{th}$ order CMBE is possible if the density matrix is considered to be sum of all orders,
 \begin{gather}
    \hbar \frac{\partial \rho_{n_1n_2k}^{\!(j)}(t)}{\partial t}=-i\left[\varepsilon_k,\rho_k^{\!(j)}{\!(t)}\right]_{n_1n_2}-\Gamma_{n_1n_2k}^{\!(j)}\rho_{n_1n_2k}^{\!(j)}(t)\nonumber\\ +ie\textbf{E}(t).\left[\xi_k,\rho_k^{\!(j-1)}\right]_{n_1n_2}-eE(t).\nabla_k\rho_k^{\!(j-1)}
\label{jSBE}
\end{gather}
...
\begin{multline}
\hbar \frac{\partial \rho_{n_1n_2k}^{\!(1)}(t)}{\partial t}=-i\left[\varepsilon_k,\rho_k^{\!(1)}{\!(t)}\right]_{n_1n_2}-\Gamma_{n_1n_2}^{\!(1)}\rho_{n_1n_2k}^{\!(1)}(t)\\
+ie\textbf{E}(t).\left[\xi_k,\rho_k^{\!(0)}\right]_{n_1n_2}-eE(t).\nabla_k\rho_k^{\!(0)}
\label{1st_CMBE}
\end{multline}
...
+ie\textbf{E}(t).\left[\xi_k,\rho_k^{\!(0)}\right]_{n_1n_2}-eE(t)\frac{\partial}{\partial k}\rho_{n_1n_2k}^{\!(0)}
\end{multline}
...
\end{equation}
The proposition of the cold semiconductor approximation is put forward with the aim of attaining outcomes that closely converge towards temperatures nearing absolute zero. Similarly, for the interband term the required velocity approximations are listed $v_{12k}^x$=$iv_{F}/{k_y}{k}$ and $v_{12k}$=$-iv_{F}{k_y}/{k}$ along with the Berry connection elements $\xi_{12k}^x$=${k_y}/{2k^2}$ and $\xi_{21k}^x$=${k_y}/{2k^2}$. From Eq.(\ref{formula_conductivity}) the interband linear conductivity expression is written as,
\begin{equation}
...
Solving the Eq.(\ref{Kubo_intra} and Eq.(\ref{Kubo_inter}) produces the final expression which depends upon frequency and chemical potential ($\mu$) given by,
\begin{equation}
...
\label{inter_con}
\end{multline}
where, $\Gamma_e$ is the interband constant, $\Gamma_i$ is the intraband constant and $\sigma_0=(e^2/4\hbar)$ is the universal conductivity.
...
%\appendix*
%\section{Some Important Approximations}
%\label{app:section1}


%\section{Conductivity Expression}
%\label{app:section1}
