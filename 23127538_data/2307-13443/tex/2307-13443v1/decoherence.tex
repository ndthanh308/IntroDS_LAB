...
% In this work we chiefly need to deal with   various two-point functions. We first define the scalar two point functions,
% %
% \begin{eqnarray}
% &&i\Delta^{\phi}_{-+}(x,x')= \langle 0 | \phi(x)\phi(x') |0\rangle \qquad i\Delta^{\phi}_{+-}(x,x')= \langle 0 | \phi(x')\phi(x) |0\rangle \nonumber\\
% &&i\Delta^{\phi}_{++}(x,x') =  \theta(\eta -\eta')i\Delta^{\phi}_{-+}(x,x') + \theta(\eta' -\eta)i\Delta^{\phi}_{+-}(x,x')\nonumber\\
% &&\Delta^{\phi}_{--}(x,x') =  \theta(\eta' -\eta)i\Delta^{\phi}_{-+}(x,x') + \theta(\eta -\eta^\prime)i\Delta^{\phi}_{+-}(x,x')
% \label{props}
% \end{eqnarray}
% %
%  where the first two above are the Wightman functions, whereas the third and the fourth are respectively the Feynman and anti-Feynman propagators. 
...
%
%\begin{eqnarray}
%\delta F_{\phi}(\eta, \eta',k) =  F_{\phi}(\eta, \eta',k;h^2)- F_{\phi}(\eta, \eta',k;h^2=0)   
%\end{eqnarray}
%
...
%However, the statistical propagator in an open quantum system is not directly related to phase space area, as in  closed quantum systems. Instead, it is related to the evolution of the system's density matrix, capturing the non-unitary and dissipative dynamics arising from interactions with the environment. 
...
\end{eqnarray}
%
The other renormalized self energyes, $i M^{ss^{\prime}}_{\phi, \text{ren}}(x,x')$ ($s,s^{\prime}=\pm$), can be found in a similar manner. However,  the self-energies of mixed kinds, i.e. those associated with the Wightman functions are free of divergences and hence do not require any renormalisation. 
...
%
% \begin{equation}
% \frac{\Box}{H^2}=-\eta^2\left(\partial_\eta^2 - \frac{2}{\eta}\partial_\eta+k^2\right)
% \label{d'Alembertian in k space}
% \end{equation}
% 
...
\end{eqnarray}
%
where we have used \ref{energies} and \ref{energies1} given in \ref{defAndConv}. We note that while utilising \ref{energies1} into \ref{statEQX}, it becomes evident that $\eta \gtrsim \eta^{\prime}$ and $\eta^{\prime} \lesssim \eta^{\prime \prime}$, consequently determining the upper limits of the above integrations. The lower limit is set to be $-1/H$, as explained in \ref{basic}. Using now \ref{newenergies} and \ref{newenergies1} in above equation, we obtain
%
\begin{multline}
...
%
\begin{multline}
F_{\phi} (\eta,\eta^{\prime \prime},k)_{ k | \Delta \eta| \ll 1}   =  \frac{1}{H^2}\left[-\frac{1}{4} \Bigg(\frac{\square_k}{H^2} \Bigg)^2 + \Bigg(\frac{\square_k}{H^2} \Bigg)  - 1 \right]\mathcal{O}(\eta, \eta^{\prime},k)_{ k | \Delta \eta| \ll 1} + H \int_{-\infty}^{\infty}d\tau \frac{G_{\text{ret}}(\eta, \tau,k)_{ k | \Delta \eta| \ll 1} }{(\tau H)^4}\\+ F_{\text {free}} (\eta ,\eta^{\prime \prime},k)   \label{eqnFInv}
\end{multline}
%
...
%
\begin{multline}
F_{\phi} (\eta,\eta^{\prime \prime},k)_{ k | \Delta \eta|\ll 1}\vert_{\rm 1\,loop} \approx \frac{g^{2} H^2}{768 \pi ^2 k^3}\Bigg[ \ln \frac{\eta  H^2 k^2 \eta ''}{\mu ^2} \left(\left(\eta ^4 k^4+2 \eta ^2 k^2+4\right) \ln \frac{\eta  H^2 k^2 \eta ''}{\mu ^2}-4
   \left(\eta ^2 k^2+6\right)\right)\\
...
\end{multline}
%
We now substitute it into \ref{phase} in order to obtain the increase in the phase space area 
%
\begin{eqnarray}
...
\end{eqnarray}
%
Converting $\eta$ now in terms of the scale factor $a$,  and keeping the dominant terms only, we have
%
\begin{eqnarray}
\delta \Xi_\phi ^2\approx \frac{ g^{2} H^6 a^6}{48 \pi ^2 k^6}  
\end{eqnarray}
%
By taking (for example) $k/H\sim1$, we have  plotted the above variation  in \ref{fig:phasespace} for different values of the Yukawa coupling.    Similar behaviour was reported in \cite{Friedrich:2019hev} for cubic interactions in inflation in the super-Hubble limit.
%
% Figure environment removed
%
Now we use the \ref{entropyPhi} to compute the von Neumann entropy, which is also plotted in \ref{fig:phasespace}. Expectedly, the von Neumann entropy also increases monotonically with increasing scale factor and the coupling constant,  similar to that of the phase space area.
...
In this work, we have studied the decoherence and subsequent entropy generation using the non-equilibrium effective field theory for the Yukawa interaction in the inflationary de-Sitter spacetime, using the correlator approach developed in~\cite{ Friedrich:2019hev, JFKTPMGS, koksma, kok}. The present paper is  an extension of our previous analysis regarding the decoherence mechanism in a fermion and scalar quantum field theory with the Yukawa interaction in the Minkowski spacetime \cite{Bhattacharya:2022wpe}. In this paper, we have also treated the scalar as our system, whereas the fermions as the surrounding or environment. Our objective was to compute the perturbative von Neumann entropy for the scalar at one loop.  The entropy turns out to be monotonically increasing with respect to the time (the scale factor), as well as the Yukawa coupling, \ref{fig:phasespace}.  This is in the qualitative agreement with the case when both the system and the surrounding are scalar fields~\cite{Friedrich:2019hev}.  Our result is also in the qulitative  agreement with the result of~\cite{Boyanovsky:2018soy} where such entropy generation was computed using the Feynman-Vernon influence functional technique. The increase in the entropy with the scale factor corresponds to the fact that the phase space area must increase with time in an expanding spacetime. The increase of the entropy with respect to the Yukawa coupling must correspond to the increase or opening up of new phase space areas due to the interaction. This feature is qualitatively similar to that of the Minkowski spacetime \cite{Bhattacharya:2022wpe}, even though this result in the Minkowski spacetime was non-perturbative.  

 
The above analysis is perturbative and one loop. Going to higher order of the perturbation theory is expected to bring in the secular logarithms.  The non-perturbative resummation of these effects using the Kadanoff-Baym equations poses a challenging task. We intend to delve into this issue in our future publications.
...
%i\Delta^{\pm \pm}_{\phi}(\eta, \eta^{\prime},k) &=& \int d^3\vec{r} \,{\rm  e}^{- i \vec k\cdot \vec{r} } i\Delta^{\pm \pm}_{\phi}(x;x^\prime),\\
%F_{\phi}(\eta,\eta^\prime,k) &=& \int d^3\vec{r}\, {\rm  e}^{- i \vec k\cdot \vec{r}} F_\phi(x;x^\prime) 
%\,,\\
%i \Delta_{\phi}^c(\eta,\eta^\prime,k) &=& \int d^3\vec{r} \,{\rm  e}^{- i \vec k\cdot \vec{r} } i \Delta_\phi^c(x;x^\prime),
\end{eqnarray}
% 
...
\end{eqnarray}
%
Introducing a renormalisation scale $\mu$ with mass dimension one, we can modify \ref{deSitterRel} by including a term dependent on $\mu$. This modification allows us to eliminate the divergence in the self energy using appropriate counterterms in the action.
...
%  recall that
% \begin{align}
% \ln \big( \Delta x_{++}^2 \big) = \ln \Big( |\Delta \eta^2 - |\vec{x}-\vec{x}^{\prime}|^2 |\Big) + i \pi \theta \big( \Delta \eta^2 - |\vec{x}-\vec{x}^{\prime}|^2 \big) \, ,
% \end{align}
...
%=  \frac{8 \pi^2 }{k^3}  \Bigg[-2\cos \big(k |\Delta \eta | \big)+ \Big[   \cos \big(k |\Delta \eta | \big)+ k |\Delta \eta | \sin \big(k |\Delta \eta | \big)  \Big]\Big[  \text{ci}\Big(2 k |\Delta \eta |\Big)  + \gamma_E - \ln \Bigg(\frac{2}{k |\Delta \eta | }\Bigg) \Big] \\ +\Big[    \sin \big(k |\Delta \eta | \big)  -k |\Delta \eta | \cos \big(k |\Delta \eta | \big) \Big]  \text{si}\Big( 2 k |\Delta \eta |\Big)\Bigg]
%\\
%- \frac{8 \pi^2 }{k^3} \ln \Big( \frac{a a^{\prime} H^2 \Delta \eta^2}{4} \Big)  \Big[   \cos \big(k |\Delta \eta | \big)+ k |\Delta \eta | \sin \big(k |\Delta \eta | \big)  \Big]  \\
%+   \frac{8 \pi^2i }{k^3}\Bigg[2\sin \big(k |\Delta \eta | \big)+ \Big[\sin \big(k |\Delta \eta | \big)  -    k |\Delta \eta |  \cos \big(k |\Delta \eta | \big)  \Big]\Big[  \text{ci}\Big(2k |\Delta \eta |\Big)  - \gamma_E + \ln \Bigg(\frac{2}{k |\Delta \eta | }\Bigg) \Big]  -\Big[    \cos \big(k |\Delta \eta | \big)  +k |\Delta \eta | \sin \big(k |\Delta \eta | \big) \Big] \Big[ \text{si}\Big( 2k |\Delta \eta | \Big) + \frac{\pi}{2} \Big]\Bigg]
%- \frac{4 \pi^2}{k^3} \Bigg[\pi - 2i\ln \Big( \frac{a a^{\prime} H^2 \Delta \eta^2}{4} \Big)  \Bigg] \Big[\sin \big[ k|\Delta \eta|   \big]- k | \Delta \eta| \cos\big[ k|\Delta \eta|   \big] \Big]  + \\
...
\begin{eqnarray}
 ...
 %\nonumber\\
%&& \approx \frac{H^2 \theta(Ha'-k)\theta(Ha''-k)}{2k^3} \left(1-\frac{ik^3}{3H^3 a'^3} \right)\qquad (\eta'' \gtrsim \eta')\label{nc4'}
\end{eqnarray}
%
...
%\left[ \frac{a'}{a} \left(1-\frac{a'}{a}\right) 
%+\frac{1}{3}\left( 1- \frac{a'}{a}\right)^3\right] 
%\\& \approx &\frac{i \theta(Ha-k)\theta(Ha'-k)}{H a^3}\left[ \frac{a}{a'} \left(1-\frac{a}{a'}\right) +\frac{1}{3}\left( 1- \frac{a}{a'}\right)^3\right]\nonumber\\
