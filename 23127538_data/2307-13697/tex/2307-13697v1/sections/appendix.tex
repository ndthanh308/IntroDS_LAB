...
\subsection{Correlation with Finetuned Accuracy}

In Figure~\ref{fig:finetuned_correlation}, we investigated the relationship between finetuned accuracy and CLER score for both generative and original data on four representative datasets. Notably, for the rare concept datasets in the original data, a certain gap was observed between the finetuned accuracy and CLER score, with the former being significantly higher than the latter (e.g., in FER-2013). This trend was also observed in the generative data. However, despite the presence of this gap, a positive correlation was observed between CLER score and finetuned accuracy, since the latter increased with the former. Nevertheless, although CLER score showed a strong positive correlation with linear probing accuracy for all types of external data, we believe that further research is necessary to fully investigate the capability of CLER score in representing finetuned accuracy.
