...
\author{I.R. Gabdrakhmanov, N.A Gramotkov, A.V.~Kotikov, D.A. Volkova and I.A.~Zemlyakov}
...
\begin{abstract}
  ...
  these data and the results obtained in the framework of standard QCD.
...
Polarized Bjorken sum rule (BSR) $\Gamma^{p-n}_1(Q^2)$ \cite{Bjorken:1966jh}, i.e. the difference between the first moments of the spin-dependent
structure functions (SFs) of a proton and neutron, is a very important space-like QCD observable \cite{Deur:2018roz}.
...
\cite{Deur:2021klh,E143:1998hbs,E142:1996thl,SpinMuon:1993gcv,COMPASS:2005xxc,HERMES:1997hjr,Deur:2004ti}.
...
\cite{Pasechnik:2008th,Ayala:2017uzx,Ayala:2018ulm}.



Following \cite{Cvetic:2006mk}, we introduce here the derivatives (in the $k$-order of perturbation theory (PT))
\be
...
The series of derivatives $\tilde{a}_{n}(Q^2)$ can successfully replace the corresponding series of $\ar$-degrees (see, e.g. \cite{Kotikov:2022swl}). Indeed, each
derivative reduces the $\ar$ degree but is accompanied by an additional $\beta$-function $\sim \ar^2$.
...
(see Ref. \cite{Kotikov:2022vnx} for a brief introduction).
...
It is much better to use the so-called ``massive'' representation for the HT part (introduced in Ref. \cite{Teryaev:2013qba}):
\be
\Gamma_1^{p-n}(Q^2)=
%\left|\frac{g_A}{g_V}\right| \,
...
~~\hat{\mu}_{\rm{MA},4}=\tilde{\mu}_{\rm{MA},4}/M^2=-0.196 \, .
\label{M,mu} 
\ee
...
%{\bf We will use this expession below but with arbitrary velues of $\sigma$.}
...
\label{DBS} 
\ee
where $d_1$, $d_2$ and $d_3$ are known from exact calculations (see, e.g., \cite{Chen:2006tw}). The exact $d_4$ value is not known, but it was recently
...
\footnote{The coefficients $\beta_i$ $(i\geq 0)$ of the QCD $\beta$-function (\ref{bQCD})
  ...
  the game at a certain threshold $Q^2_f$ according to \cite{Chetyrkin:2005ia}. The corresponding QCD parameters $\Lambda^{(f)}$ in N$^i$LO of PT can be found
...
The results of calculations are presented in Table \ref{Tab:BSR} and in Fig. \ref{fig:APTHT}.
...
twist-four term.
Without it, the value of $\Gamma_1^{p-n}(Q^2)$ is about 0.16, whic is very far from the experimental data.

At $Q^2 \leq 0.3$ GeV$^2$
we also see good agreement with the phenomenological models: Burkert-Ioffe one \cite{Burkert:1992tg} and especially
...
the deterioration increases with the PT order in this case (see \cite{Pasechnik:2008th,Ayala:2017uzx,Ayala:2018ulm,Kotikov:2022sos}).
...
We have considered the Bjorken sum rule in the framework of MA and conventional QCD and obtained results similar to those obtained in previous
studies \cite{Pasechnik:2008th,Ayala:2017uzx,Ayala:2018ulm,Kotikov:2022sos} for the first 4 orders of PT.
%perturbation theory.
...
\bibitem{Deur:2021klh}
  A.~Deur,
  %J.~P.~Chen, S.~E.~Kuhn, C.~Peng, M.~Ripani, V.~Sulkosky, K.~Adhikari, M.~Battaglieri, V.~D.~Burkert and G.~D.~Cates,
  \textit{et al.}
...
%B.~Adeva \textit{et al.} [Spin Muon (SMC)],
...
D.~Adams \textit{et al.} [Spin Muon (SMC)],
...
M.~G.~Alekseev \textit{et al.} [COMPASS],
...
M.~Aghasyan \textit{et al.} [COMPASS],
...
\bibitem{Deur:2004ti}
  A.~Deur,
  %P.~E.~Bosted, V.~Burkert, G.~Cates, J.~P.~Chen, S.~Choi, D.~Crabb, C.~W.~de Jager, R.~De Vita and G.~E.~Dodge,
  \textit{et al.}
...
%\cite{Deur:2008ej}
%\bibitem{Deur:2008ej}
%A.~Deur, P.~Bosted, V.~Burkert, D.~Crabb, V.~Dharmawardane, G.~E.~Dodge, T.~A.~Forest, K.~A.~Griffioen, S.~E.~Kuhn and R.~Minehart, \textit{et al.}
...
K.~Slifer \textit{et al.} [Resonance Spin Structure],
...
\bibitem{PDG20}
Particle Data Group collaboration, P.A. Zyla et al., Review of Particle Physics, PTEP {\bf 2020} (2020) 083C01.
