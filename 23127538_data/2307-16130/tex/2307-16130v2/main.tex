...
We next read the body of the remaining preprints to identify whether the authors indeed employed QCs of the five vendors (based on the gate model, not on the quantum annealing or quantum simulator) and extracted the following attributes: a) topics of the preprint (up to 2), b) presence of authors affiliated with the vendors, c) the names of the QPUs employed and their vendors (up to 3), and d) the maximal number of the employed qubits in the preprint.
In these elimination steps, we singled out 748 preprints.
...
In particular, the increase in speed was accelerating since 2018: the slope from 2018 to 2021 is about 1.16 times larger than that from 2016 to 2018.
...
If we compensate for the influence of this lacking two months, then the number of the estimated submissions in 2022 is approximately 206.4 (=$12/10\times 172$, where 172 is the number of the submissions  in 2022 observed through this survey), which is comparable with the number of the submissions 217 in 2021.
...
The box-and-whisker plots in Fig. \ref{general} (c) for the number of the employed qubits show that the upper end of the whisker reaches 20 in 2022 whereas the lower end has been one except two in 2016 and 2017.
...
On the other hand, when examining Fig. \ref{general} (d), it becomes evident that preprints associated with vendor companies have contributed to an increase in the median number of employed qubits, reaching 8 in 2022.
...
The average number of employed qubits has increased almost 2.5 times from 4 in 2016 to 10.46 in 2022 (Fig. \ref{general}(c)).
...
Indeed, the maximal number of employed qubits rapidly increases from 5 in 2016 to 102 in 2022 (Fig. \ref{general} (e)).
...
The average number of employed qubits in the preprints using the QPUs of Quantinuum \cite{PhysRevA.101.010301,PhysRevApplied.16.044057,Amaro_2022,kikuchi2023realization} and Rigetti are larger than those of IBM and IonQ.
...
Of the 113 preprints submitted in 2022, 102 preprints (90.3\%) are in the top eight subtopics (F1: Quantum machine learning, F2: Condensed matter physics, F3: Quantum chemistry, F5: Optimization (QAOA), F7: Primitives, F8: Simulating quantum mechanics, and F9: High energy physics, F13: Others).
...
Similarly, the subtopics F2, F5, F7 (Primitives), and F13 have an average larger than the overall average of 10.46 in 2022 (c.f. Fig. \ref{general} (c)).
Therefore, the common subtopics F2, F5, and F13 tend to use more qubits than all the (sub)topics. 
The subtopic F2 (Condensed matter physics) employs more qubits than the subtopic F3 (Quantum chemistry), although both the subtopics resemble each other in the energy scale of the systems to be treated.
