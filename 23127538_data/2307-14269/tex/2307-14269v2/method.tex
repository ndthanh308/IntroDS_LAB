...
Given $N+1$ observations of $y(\cdot)$ at $N+1$ samples $\tau_i \in \mathcal{T}_N \cup \{\tau_\xid\}$ for $i \in \setS$, where $\tau_\xid$ denotes the abscissa of the exceptional sample, an approximation of $y(\cdot)$, denoted $\tilde{y}(\cdot)$, can be constructed based on a polynomial of degree $N$ through the use of Lagrange polynomial interpolation, see, \emph{e.g.}, \cite{Berrut2004,Ross2003,Fahroo2008,Rao2010,Garg2011}, such that
...
\begin{thm} \label{thm:new-matrix-is-inversion-ready}
    Matrix $\bm{D} \in \mathbb{R}^{N \times (N+1)}$ specified in \eqref{eq:diff-matrix} is an inversion-ready differentiation matrix over the set of abscissas $\mathcal{T}_N \cup \{ \tau_\xid\}$ with order of accuracy $N$, according to Definition \ref{defn:inversion-ready}.
\end{thm}
...
To determine an appropriate location $\tau_\xid$ the following metric is adopted:
...
\begin{lem} \label{lem:lagrange-inner-prod}
    Let $l_k (\tau)$ with $\tau \in \closedinterval{-1}{1}$ denote the Lagrange interpolating polynomial of index $k$, obtained with \eqref{eq:poly-interp}, furthermore, let $p(\tau)$ denote some polynomial of degree $N-3$ or less. Then the following holds:
...
\begin{proof}[Proof of Lemma \ref{lem:lagrange-inner-prod}]
    ...
    \begin{equation}
        \int_{-1}^{1} l_k(\tau) p(\tau) \dl \tau = \sum_{i \in \setC} w_i l_k(\tau_i) p(\tau_i) \,, \quad \forall k \in \setC \,,
    \end{equation}
