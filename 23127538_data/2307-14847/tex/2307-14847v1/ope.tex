...
\bar\psi \gamma_\mu \partial^{\mu_1} \cdots \partial^{\mu_J}   \partial^{2n} \psi  + {\rm cyclic} \right) -{\rm Traces},  \qquad l = J+1,\\ \nonumber
 ...
 These are the possible  symmetric traceless tensors tensors formed out of bi-linears of fermions which 
...
\end{equation}
The contour in (\ref{contrep}) is chosen to encircle the right-half of the $\Delta$ plane plane when $|x|<1$ and
...
\begin{align}
	...
	\Gamma( \frac{1}{2} + \Delta_\psi) \Gamma \left(\Delta -\Delta_\psi+\frac{3}{2}\right)} 
	 \int_{0}^{1}dz\sqrt{\frac{1}{\bar z}}\, (-\bar z)^{-\Delta_\psi-\frac{1}{2}} (1-\bar z)^{2 \nu }  \bar z^{-\frac{\Delta }{2}+\Delta_\psi-\nu -1} \nonumber \\ 
...
&\equiv& I_1 + I_2 +I_3.
\end{eqnarray}
Just as before the last line defines the 3 integrals required to obtain $\hat a(\Delta , j)$. 
...
\appendix
\section{
Gross-Neveu model: parition function, gap equation}
\label{appendgap}
