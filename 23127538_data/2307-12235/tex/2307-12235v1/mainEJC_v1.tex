...
\author{Marco Fabris\fnref{myfootnote} and Angelo Cenedese}
\address{Dept. of Information Eng., Univ. of Padova, via Gradenigo 6/B, Padua, 35131, Italy.}
\fntext[myfootnote]{M.~Fabris and A.~Cenedese are with the Department of Information Engineering at the University of Padova (UNIPD - DEI), Padova, 35131, Italy (e-mails: marco.fabris.1@unipd.it, angelo.cenedese@unipd.it). A.~Cenedese is also with the Institute of Electronics, Information and Telecommunication Engineering, National Research Council (CNR - IEIIT).}
...
%
For example, the specific features of FC depend on the level of interaction within the MAS, namely if the approach is centralized~\cite{HauserCook2016}, distributed~\cite{zhao2014distributed, Hu2021ADecentralized, bishop2015distributed}, or leader-follower~\cite{trinh2018bearing, Franchi2019Online}, and on the control and sensing capabilities of the agents, basically distinguishing among position/distance information~\cite{OhParkAhn2015, ChuangHuangDOrsogna2007}, orientation/bearing information~\cite{eren2012formation, michieletto2021unified}, or both~\cite{bishop2015distributed}.
%
...
%\MFa{
	...
	%
	%
	However, in the latter works, as well as in the majority of the surveyed literature, the energy consumption of the system is not taken into consideration, even though it may represent a crucial design principle impacting both the task completion effectiveness and the control action efficiency.
	%
	%
	%
	%\todo[inline]{               }
...
\textit{Contributions}: Given this premise, in this paper we propose a novel approach to FT in which the problem is formulated based on a time-invariant optimal control objective where three different tasks are considered, namely that of tracking a given path at the group level, attaining a desired (and feasible) geometric formation while minimizing the input energy. Also, the approach we pursue is fully distributed.
%
More specifically, we build on our previous work~\cite{FabrisCenedeseHauser2019}, where the optimization framework called PRONTO (PRojection Operator based Newton's method for Trajectory Optimization) was successfully adopted to obtain inverse dynamics to steer a MAS towards the desired formation along the chosen path, and we devise in the present work an online networked strategy based on a practically optimal distributed feedback control law.
%
...
The first part of Sec.~\ref{sec:numerical_methodologies_for_OIFT} introduces, in a general way, how the FT problem of interest can be formulated and optimally solved in a centralized fashion by using PRONTO; whereas, in the second part, the derivation of an online distributed feedback controller is presented. 
...
The set of real numbers is indicated by $\mathbb{R}$ and a similar symbology is straightforwardly employed for further subsets or extensions of $\mathbb{R}$ itself. The class of $k$-continuous functions is identified with $\mathscr{C}^{k}$.

The identity matrix of dimension $\natural \in \mathbb{N}$ is denoted by $\Ii_{\natural}$, while $\Zz_{\natural_{1}\times \natural_{2}}$ addresses the zero matrix with dimensions $\natural_{1}\times \natural_{2} \in \mathbb{N} \times \mathbb{N}$. Moreover, if $\natural_{1}= \natural_{2}=\natural$ symbol $\Zz_{\natural}$ is used instead of $\Zz_{\natural_{1}\times \natural_{2}}$, while the zero vector of dimension $\natural$ is denoted by $\mathbf{0}_{\natural}$. The rank of a matrix is expressed by the operator $\rk$. Symbols $\succeq 0$, $\succ$, and $\left\|\cdot \right\|_{2}$ stand for positive semidefinite, positive definite, and Euclidean norm, respectively. Whereas, $\left\|\psps\right\|_{\Omom}^{2} $ defines the quadratic form $\psps^{\top}\Omom \psps$, where $\psps$ and $\Omom$ generically indicate real-valued vector and a real-valued matrix, respectively.
...
%\mathbf{x}_{B} = \begin{bmatrix}
	...
	\end{equation}
	%
	where state matrix $\mathbf{A}\in \mathbb{R}^{2N\times 2N}$ and input matrix $\mathbf{B}\in \mathbb{R}^{2N\times N}$ take the forms
	%
	%\begin{equation*}
	%\left(\mathbf{A},\mathbf{B}\right) = \left(\begin{bmatrix}
		...
		%\mathbf{C} = \dfrac{1}{n} \begin{bmatrix}
			...
			\begin{equation*}
				\mathbf{C} = \dfrac{1}{n} \begin{bmatrix}
					\mathbf{I}_{M} & \dots & \mathbf{I}_{M} & \Zz_{M} & \dots & \Zz_{M}\\
					\Zz_{M}  & \dots &  \Zz_{M} & \mathbf{I}_{M} & \dots & \mathbf{I}_{M} 
				\end{bmatrix}.
			\end{equation*}
			%
...
The purpose of this study is the design and analysis of control strategies in order for the group of agents to track a desired path 
%\begin{equation*}
...
\end{equation}
represents the cost functional to be minimized over the time interval $[0,T]$ in order to achieve the three objectives previously defined. Generalizing the approach in \cite{FabrisCenedeseHauser2019}, two different terms explicitly appear in \eqref{eq:cost}:
...
\end{equation}
It is worth observing that terms \eqref{eq:track_inst_cost} and \eqref{eq:form_inst_cost}-\eqref{eq:align_inst_cost} lead the system to achieve second-order velocity alignment and cohesion/separation, respectively, according to the Boids model principles~\cite{Reynolds1987}.
...
\end{equation*}
%
where $l^{fo}(\mathbf{x}) = l^{fo1}(\mathbf{p})+l^{fo2}(\dot{\mathbf{p}})$. Similarly, we assume that $\nabla l^{tr}(\xx) = \nabla l^{tr1}(\pp)+\nabla l^{tr2}(\dot{\pp})$ by trivially splitting \eqref{eq:track_inst_cost} into the additive terms $\nabla l^{tr1}(\pp)$ and $\nabla l^{tr2}(\dot{\pp})$ referred to the control of $\pp_{c}$ and $\dot{\pp}_{c}$, respectively. %where $\Thth_{ij} \succeq 0$ for all $(i,j) \in \mathcal{E}$. 
...
The qualitative properties of functions in \eqref{eq:potential} and their derivatives can be summarized as follows:
\begin{enumerate}
...
%\end{equation*}
and denoting with a subscript the partial differentiation w.r.t. $\xx$ or $\uu$, the list of relations is immediately obtained:  
%
\begin{alignat}{3}
	& \bar{\mathbf{A}}(t) &&= \ff_{\mathbf{x}}           &&= \mathbf{A} \nonumber\\
	& \bar{\mathbf{B}}(t) &&= \ff_{\mathbf{u}}           &&= \mathbf{B} \nonumber\\
	& \mathbf{a}(t)       &&= l^{\top}_{\mathbf{x}}    &&= \mathbf{C}^{\top} \mathbf{Q} \left(\mathbf{x}_{c}-\mathbf{x}_{c,des}\right) + \nabla_{\mathbf{x}}l^{fo1} + \nabla_{\mathbf{x}}l^{fo2} \label{eq:a}\\
	& \mathbf{b}(t)       &&= l^{\top}_{\mathbf{u}}    &&= \mathbf{R} \mathbf{u} \nonumber\\
	& \mathbf{r}_{1}(T)   &&= m^{\top}_{\mathbf{x}}    &&= \mathbf{C}^{\top} \mathbf{Q} \left(\mathbf{x}_{c}(T)-\mathbf{x}_{c,des}(T)\right) +  \nonumber\\
	& && && ~~~~\nabla_{\mathbf{x}}l^{fo1}(T) + \nabla_{\mathbf{x}}l^{fo2}(T) \nonumber\\
	& \mathbf{Q}_{o}(t)   &&= l_{\mathbf{x}\mathbf{x}} &&= \mathbf{C}^{\top} \mathbf{Q} \mathbf{C} + \mathcal{H}_{\mathbf{x}\mathbf{x}} l^{fo1} + \mathcal{H}_{\mathbf{x}\mathbf{x}} l^{fo2} \label{eq:Qo}\\
	& \mathbf{S}_{o}(t)   &&= l_{\mathbf{x}\mathbf{u}} &&= \Zz_{2N\times N} \nonumber\\
	& \mathbf{R}_{o}(t)   &&= l_{\mathbf{u}\mathbf{u}} &&= \mathbf{R} \nonumber\\
	& \mathbf{P}_{1}(T)   &&= m_{\mathbf{x}\mathbf{x}} &&= \mathbf{C}^{\top}\mathbf{Q}\mathbf{C} + \mathcal{H}_{\mathbf{x}\mathbf{x}} l^{fo1}(T) + \mathcal{H}_{\mathbf{x}\mathbf{x}} l^{fo2}(T) \nonumber
\end{alignat}
%
...
\end{equation}
turns out to be one of the most efficient options to adopt.
...
\begin{equation*}\label{eq:gradal}
	\nabla_{\mathbf{p}_{i}}l^{fo2}(t) = k_{A} \sum\limits_{ j \in \mathcal{N}_{i}} \Thth_{ij} \dot{\mathbf{e}}_{ij} 
\end{equation*}
and for all $(i,j) \in \mathcal{E}$,
...
Now, to make $\widehat{\uu}$ comply with optimality criteria, a weak version of the Pontryagin Minimum Principle (PMP) \cite{Pontryagin2018} applied to the specific framework under analysis is provided in the next lemma.

\begin{Lem}[PMP applied to \eqref{eq:dyn_sys}-\eqref{eq:minimizationproblemOIFT}] \label{lem:PMP}
...
\end{Lem}
\begin{proof}
	The statement is a direct consequence of the PMP applied to \eqref{eq:dyn_sys}-\eqref{eq:cost}, as shown by each double implication in \eqref{eq:PMPstates}-\eqref{eq:PMPinput} and \eqref{eq:PMPsuff}. In particular, sufficient condition \eqref{eq:PMPsuff} is equivalent to require convexity for $l$ w.r.t. variables $\mathbf{x}$ and $\boldsymbol{\lambda}$, leveraging the fact that \eqref{eq:dyn_sys} is linear (see also \cite{Locatelli2001}).
\end{proof}

We now address point $(ii)$. To this purpose, the derivation of a local estimator $\widehat{\mathbf{x}}_{c}^{i}(t)$, for each agent $i=1,\dots,n$, that reconstructs the information about the centroid $\mathbf{x}_{c}(t)$ will be crucial, since one term of controller $\uu^{\star}$ is devoted to satisfy the minimization of the tracking part of $h$ influenced by $l^{tr}$. Such estimator $\widehat{\mathbf{x}}_{c}^{i}$ is of second-order type, as it mimics dynamics \eqref{eq:dyn_sys}. However, the next proposition focuses more on the working principles to design a first-order estimator, since this will be used to obtain $\widehat{\mathbf{x}}_{c}^{i}$ in the following.

\begin{prop}[Local first-order centroid estimator] \label{prop:loccentrest}
	Given an undirected and connected graph $\mathcal{G}=(\mathcal{V},\mathcal{E})$, assume that the dynamics of the whole system associated to $\mathcal{G}$ is described by the single integrator
	\begin{equation}
		\dot{\mathbf{y}}(t) = \mathbf{w}(t)
	\end{equation}
	where $\mathbf{y}, \mathbf{w} \in \mathbb{R}^{N}$, $N=nM$, with $\yy = \vci{\yy_{i}}{n}$ and $\ww = \vci{\ww_{i}}{n}$. For $i=1,\dots,n$, let $\mathbf{N}_{i} \in \mathbb{R}^{N\times N}$ be a projection matrix such that for a vector $\psps \in \mathbb{R}^{N}$, $\psps = \vci{\psps_{i}}{n}$, it holds that $\mathbf{N}_{i} \psps = \begin{bmatrix}
	...
	\begin{equation}\label{eq:ssestim}
		\begin{cases}
			...
			\widehat{\mathbf{y}}_{c}^{i} = n^{-1} \sum_{i=1}^{n} \widehat{\mathbf{y}}^{i}, \quad \widehat{\mathbf{y}}^{i}(0) = \widehat{\mathbf{y}}^{i}_{0}
		\end{cases},
	\end{equation}
	such that $k_{y}$ is a positive gain, $\widehat{\mathbf{y}}^{i}_{0} \in \mathbb{R}^{N}$ is an arbitrary initial condition, $\widehat{\yy}^{i}(t), \widehat{\ww}^{i}(t) \in \mathbb{R}^{N}$ for $i = 1,\ldots,n$, and
	\begin{equation}\label{eq:inputestassignment}
		...
		\end{cases}
	\end{equation}
	with $\widetilde{\ww}_{j}^{i} \in \mathbb{R}^{M}$.
	Also, let us define the $i$-th centroid estimation error $\ee_{y_{c}}^{i} = \mathbf{y}_{c}- \widehat{\mathbf{y}}_{c}^{i}$ between the actual centroid $\mathbf{y}_{c} = n^{-1} \sum_{i=1}^{n} \yy_{i}$ and its $i$-th estimate $\widehat{\mathbf{y}}_{c}^{i}$. If, for all $i = 1,\ldots,n$
	\begin{equation}\label{eq:hypconvinpest}
		\underset{t\rightarrow +\infty}{\lim} \ww_{j}(t)-\widetilde{\ww}_{j}^{i}(t) = \mathbf{g}_{ij}, ~ \forall j \notin \overline{\mathcal{N}}_{i}
	\end{equation}
	with $\mathbf{g}_{ij} \in \mathbb{R}^{M}$ finite constant then, $\forall i = 1,\ldots,n$, one has
	\begin{equation}\label{eq:centrerrest}
		\left\|\ee_{y_{c}}(t)\right\|_{2}^{2} \leq \varpi_{y_{c}}^{2}(t) := \dfrac{\mathfrak{c}(\mathcal{G})^{2}}{n^{2}k_{y}^{2}} \sum\limits_{i=1}^{n} \sum\limits_{j \notin \overline{\mathcal{N}}_{i} } \left\|\ww_{j}(t)-\widetilde{\ww}_{j}^{i}(t) \right\|^{2}_{2}
	\end{equation}
	as $t\rightarrow +\infty$, where $\ee_{y_{c}} = \vc{\ee^{i}_{y_{c}}}$, $j\notin\overline{\mathcal{N}}_{i}$ stands for $j \in \mathcal{V} \setminus \overline{\mathcal{N}}_{i}$ and $\mathfrak{c}(\mathcal{G}) > 0$ is a constant that only depends on the topology of $\mathcal{G}$.
\end{prop}
\begin{proof}
	...
	\begin{equation}\label{eq:errdynest}
		\dot{\ee}_{y} = -k_{y} \left[(\Ll \otimes \Ii_{N}) + \Nn \right] \ee_{y} + \ee_{w}
	\end{equation}
	where $\Nn = \Diag(\Nn_{i})$ and $\otimes$ denotes the Kronecker product. Indicating the update matrix of \eqref{eq:errdynest} with $\Mm = -k_{y} \left[(\Ll \otimes \Ii_{N}) + \Nn \right] \in \mathbb{R}^{N^{2}\times N^{2}}$, it is possible to observe that $\rk(\Mm)$ is full. This is due by the fact that, in $\Mm$, the diagonal projection matrix $\Nn$, with $\rk(\Nn) = N$, adds to the extended Laplacian $(\Ll \otimes \Ii_{N})$. The latter is a positive semidefinite matrix with exactly $N$ zero eigenvalues by the Kronecker product properties and thus $\rk(\Ll \otimes \Ii_{N}) = N^{2}-N$. As a consequence, the diagonal elements of $\Nn$ contribute to form $N$ self loops in the graph associated to $(\Ll \otimes \Ii_{N})$; hence, $\rk(\Mm) = N^{2}$ is achieved. Moreover, since matrix $\Nn$ is also positive semidefinite and $\rk(\Mm) = N^{2}$, matrix $\Mm$ is negative definite. Therefore, by also leveraging hypothesis \eqref{eq:hypconvinpest}, the linear dynamics in \eqref{eq:errdynest} stabilizes around
	\begin{equation}
		\ee_{y}^{\star}(t) = k_{y}^{-1} \left[(\Ll \otimes \Ii_{N}) + \Nn \right]^{-1} \ee_{w}(t)
	\end{equation} 
	for large values of $t$. Indeed, by substituting $\ee_{y}^{\star}(t)$ in \eqref{eq:errdynest}, one has $\dot{\ee}_{w}(t) \rightarrow \mathbf{0}_{N^{2}}$ as $t\rightarrow +\infty$, namely each $\ee^{i}_{w_{j}}(t)$ tends to some finite constant (either $\mathbf{0}_{M}$, if $j\in \overline{\mathcal{N}}_{i}$, or $\mathbf{g}_{ij}$, otherwise). In conclusion, the following inequality can be derived:
	\begin{equation}\label{eq:ineqerrbase}
		\left\|\ee_{y}\right\|_{2} = k_{y}^{-1} \left\| \left[(\Ll \otimes \Ii_{N}) + \Nn \right]^{-1} \ee_{w}\right\|_{2} \leq \dfrac{\mathfrak{c}(\mathcal{G})}{k_{y}} \left\|\ee_{w}\right\|_{2}
	\end{equation}
	where constant $\mathfrak{c}(\mathcal{G}) > 0$ is the spectral radius of matrix $\left[(\Ll \otimes \Ii_{N}) + \Nn \right]^{-1} \succ 0$. From
...
\begin{equation}
	\widehat{\mathbf{x}}_{c}^{i}(t) = \vc{\widehat{\dot{\mathbf{p}}}_{c}^{i}(t),\widehat{\mathbf{p}}_{c}^{i}(t)} \in \mathbb{R}^{2N}, \quad i = 1,\ldots,n
\end{equation}
in the following lemma.

\begin{Lem}[Local second-order estimator]\label{lem:secordest}
	Let $k_{py}$ and $k_{dy}$ be two positive gains. Each node $i = 1,\ldots,n$ can compute the $i$-th estimates $(\widehat{\pp}_{c}^{i},\widehat{\dot{\pp}}_{c}^{i}) = \left(n^{-1}\sum_{i=1}^{n}\widehat{\pp}_{j}^{i}, n^{-1}\sum_{i=1}^{n}\widehat{\dot{\pp}}_{j}^{i} \right)$ for the position and velocities centroids $(\pp_{c},\dot{\pp}_{c})$ in the OIFT problem \eqref{eq:minimizationproblemOIFT} constrained to dynamics \eqref{eq:dyn_sys} exploiting the $i$-th second-order estimator
	\begin{equation}\label{eq:estimator2}
		\begin{cases}
			\dfrac{\mathrm{d}}{\mathrm{d}t}\widehat{\pp}^{i} = -k_{py}  \sum\limits_{j\in \mathcal{N}_{i}} (\widehat{\pp}^{i}-\widehat{\pp}^{j}) -k_{py} \mathbf{N}_{i} (\widehat{\pp}^{i}-\pp ) + \widehat{\uups}^{i} \\
	...
	\begin{equation}\label{eq:estinupt2ord}
		...
		\end{cases}
	\end{equation}
	Furthermore, if $\mathbf{u}$ is a stabilizing input for system \eqref{eq:dyn_sys} and such that $\mathbf{u} \rightarrow \mathbf{0}_{N}$ as $t\rightarrow +\infty$ then, for large values of $t$, each velocity estimation error $(\dot{\pp}_{c} - \widehat{\dot{\pp}}_{c}^{i} )$ vanishes, while each position estimation error $(\pp_{c} - \widehat{\pp}_{c}^{i} )$ converges to some constant vector.
\end{Lem}
\begin{proof}
	The estimator developed in this lemma is the natural second-order extension of the results illustrated in Prop. \ref{prop:loccentrest}. As pointed out in Rem. \ref{rm:desiredinputest}, the exogenous inputs for the estimator chosen in \eqref{eq:estinupt2ord} are such that the velocity estimation error vanishes for large values of $t$, by virtue of \eqref{eq:centrerrest}.
\end{proof}

\subsection{Online distributed feedback control law}
...
\begin{align}\label{eq:distrOIFTsol}
	...
	&k_{d}^{fo1} \dot{\nabla}_{\pp_{i}}^{safe} l^{fo1} + k_{p}^{tr2} \nabla_{\dot{\pp}_{i}} l^{tr2}  + k_{d}^{fo2} \nabla_{\dot{\pp}_{i}} l^{fo2}) ,
\end{align}
where terms $ \nabla_{\pp_{i}} l^{tr1}, \dot{\nabla}_{\pp_{i}} l^{tr1}, \nabla_{\dot{\pp}_{i}} l^{tr2} $ in \eqref{eq:distrOIFTsol} are computed through the second-order local estimator in Lem. \ref{lem:secordest}. 
...
The design criterion $(i)$ is satisfied, since the structure of $\widehat{\uu} = -\Rr^{-1}\Bb^{\top} \widehat{\lamlam}$ mimics the one provided by the optimal control $\uu^{\star} = -\Rr^{-1}\Bb^{\top} \lamlam$ in Lem. \ref{lem:PMP}. Indeed, the solution of the co-state equation in \eqref{eq:PMPcostates} can be reproduced by means of
\begin{equation}\label{eq:lamdynest}
	...
	\end{cases},
\end{equation}
since $(\widehat{\lamlam}_{i}, \widehat{\lamlam}_{i+n}) \approx (\lamlam_{i}, \lamlam_{i+n})$ represents a first-order approximation for the Lagrangian multipliers whenever the control gains can be selected to satisfy\footnote{Up to some maximum tolerance established by design.} the dynamics
...
Then, the design criterion $(ii)$ is obeyed, since the structure of $\uu_{i}$ is distributed by construction, i.e. node $i$ uses the information of nodes $j \in \overline{\mathcal{N}}_{i} = \mathcal{N}_{i} \cup \{i\}$ only. In particular, the second-order local estimator in Lem. \ref{lem:secordest} is exploited by each agent to reconstruct the centroid information of the system. 
Lastly, the design criterion $(iii)$ is observed, since $\uu_{i}$ is a function of the current state $\xx(t)$ only and does not require prior computations, e.g. trajectory optimization techniques are not needed.
...
\end{comment}
%		\begin{rmk}
	%			Out of their context, equations in \eqref{eq:diffeqlamapprox} can be interpreted as an asymptotically stable dynamics, whenever the constant gains $k_{p}^{tr1},k_{p}^{fo1},k_{d}^{tr1},k_{d}^{fo1},k_{p}^{tr2},k_{p}^{fo2}$ are positive. It is worth, indeed, to recall that in this framework the evolution over the time of each variable $(\xx^{\star},\lamlam^{\star},\uu^{\star})$ depends primarily on $\nabla l^{st}(\xx^{\star})$, since $\xx^{\star}$ linearly depends on $\uu^{\star}$ that linearly depends $\lamlam^{\star}$, whose behavior is dictated by structure of $\nabla l^{st}(\xx)$. With such a perspective, the practical co-state optimality condition can be satisfied in any scenario, implying that \eqref{eq:lamdynest} is a valid approximation for the Lagrangian multipliers for large values of $t$. Nonetheless, the challenging aspect here is represented by the feedback gain tuning in order to achieve \eqref{eq:practfoc} as $t$ approaches $T$, while maintaining the magnitude of $\widehat{\uu}$ big enough to be effective. To some extent, the backward integration that emerges from the co-state computation in \eqref{eq:PMPcostates} boils down to the adjustment gains $k_{p}^{tr1},k_{p}^{fo1},k_{d}^{tr1},k_{d}^{fo1},k_{p}^{tr2},k_{p}^{fo2}$. This is a very remarkable fact, since the need for the computation of $2N$ scalar Lagrangian multipliers depending on time is downsized to the choice of a finite number of positive constants.
	%		\end{rmk}

To conclude, the adoption of a saturated version $\bar{\uu}$ of input $\widehat{\uu}$ can be employed to refine approximation errors made by \eqref{eq:lamdynest}, since high values for the feedback gains imply high values for the instantaneous input energy $l^{in}(\widehat{\uu})$. In formulas, for all individual scalar components $j = 1,\ldots,N$, assign
...
In this section, we focus on a singular case study where desired distances $d_{ij}$ are imposed such that the formation assumes a cubic shape in a space of dimension $M=3$. Precisely, the desired cube has side equal to $d = 5~\si{\meter}$ and each of the $n=8$ agents have a neighborhood with degree $|\mathcal{N}_{i}| = 5$, $i=1,\ldots, n$. This setup allows us to present an example where the formation to be achieved is globally rigid. 
Thus, the convergence for the whole system towards a unique stable equilibrium is ensured by such topology and geometry. Also, the group of agents have to track the straight line $\boldsymbol{\Gamma}(t) = \mathrm{v}t$ with a constant desired velocity $\mathrm{v}_k = 1 ~\si{\meter\second^{-1}}$ for each component $k = 1,2,3$. The initial state $\xx_{0} = \vc{\pp_{0},\dot{\pp}_{0}}$ of the system is assigned as follows:
\begin{align*}
...
by taking two actions: either the algorithm stops when the iteration $k$ exceeds the preset number of iterations $MaxIter \in \mathbb{N}$ or, given a threshold $\varepsilon_{g} > 0$, when the inequality $-Dg\left(\boldsymbol{\xi}_{t}\right) \cdot \boldsymbol{\zeta}_{t} < \varepsilon_{g}$ is verified (i.e. the search direction becomes sufficiently flat). In particular, we set $MaxIter = 80$ and $\varepsilon_{g} = 10^{-8}$.
...
\begin{itemize}[]
	...
	\item for globally rigid geometrical shapes the convergence is attained in a finite time, provided that gains of the distributed PD controller are properly tuned.
\end{itemize}
...
\subsection{Distributed control law seen as a consensus protocol}
...
the entire dynamics controlled by law in \eqref{eq:distrOIFTsol} approximately attains a stable equilibrium after $T/2= 10~\si{\second}$, for $t > t_{1\%}$.
...
Considering a second-order integrator MAS, we have introduced a complete version of the OIFT problem whose minimum-energy solution can be obtained by the numerical tool PRONTO (offline) and a distributed feedback control law (online). In order to fulfill the required final configurations for the formation to be driven, generic and versatile potential functions have been proposed. In particular, the reported simulations show the validity of the online distributed feedback control devised w.r.t. the trajectory and the energy profiles yielded by PRONTO, where the latter has been accounted as a reference for the former. %A short yet detailed analysis of the equilibria has also been provided in order to give an overview on the working principles of developed input law. 
Furthermore, the flexibility of the decentralized paradigm is tested under the presence of communication constraints on the information sharing.
...
The investigation of an optimal time-varying formation tracking problem also provides a new challenging follow-up for this study.
