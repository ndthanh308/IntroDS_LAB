...
\createtodoauthor{AP}{pairedTwoDarkBlue}
...
Hoare logic has been extended to probabilistic programs~\cite{probabilisticCBC2020,Morgan1998pGCLFR}, where the properties are probabilistic, and correctness is defined in terms of their expectations.
...
This has given rise to, e.g., quantum Hoare logic~\cite{chadha2006hoare,kakutani2009hoare,ying2012quantum} and quantum incorrectness logic~\cite{peng2022_quantum_incorrectness_logic}.
...
We provide refinement systems for both partial and total correctness.
...
\item Prove that our refinement systems are sound and complete: Any program constructed from an initial specification by using the refinement rules must satisfy the specification, and any program satisfying a specification can always be constructed using the refinement rules.
...
\Cref{app:coin-toss-till-zero} discusses an example that we hope can be helpful for readers with less familiarity with quantum computing.

\paragraph{Independent concurrent work}
Shortly after our work had appeared as a preprint, \citet{feng2023refinement} reported on independent work on a refinement system that is similar to ours.
The two works are independent and each has its merits:
\Citet{feng2023refinement} restrict to projective predicates, whereas our formalism allows for arbitrary linear predicates.
Therefore our formalism captures a larger class of interesting properties, in particular success probabilities of algorithms as described in \cref{subsec:examples:qsearch} and \cref{subsec:examples:boosting}.
In addition, while their refinement system only ensures partial correctness, we also provide refinement system that ensures total correctness.
This gives stronger guarantees and in particular enables reasoning about termination, which is impossible otherwise.
Finally, we also offer a new formalization of the correctness by construction methodology in terms of refinement of ``programs with holes'', which can be of interest beyond quantum verification.
On the other hand, restricting to projective predicates enables \citet{feng2023refinement} to reason about strongest postconditions.
% , which are not known to exist for arbitrary predictes.
\Citet{feng2023refinement} also provide a proof-of-concept implementation of their refinement system, which will prove insightful for future case studies.
%=============================================================================
...
S~::=~
    ...
    &|\; \qpselect{\{\omega_1 \colon M_{\omega_1}, \omega_2 \colon M_{\omega_2}, \ldots\}}{\vec{q}}{\\
       &\quad\quad \omega_1 \colon S_{\omega_1}, \ \omega_2 \colon S_{\omega_2}, \ \ldots}~\\
    &|\; \qpwhile{B}{\vec{q}}{S} ~
}}
...
We also allow leaving out the measurement in \textbf{if}, \textbf{case}, and \textbf{while}, in which case we default to standard basis measurements:
\begin{gather*}
...
The semantics for \textbf{while} is well-defined because the partial states~$\sigma_n = \sum_{k=0}^n \parens*{\cB_{0,\vec q} \circ \parens{ \Bracks{S} \circ \cB_{1,\vec q} }^k}(\rho)$ form an increasing sequence ($\sigma_{n+1} \succeq \sigma_n$ for all~$n$) that is bounded from above.
...
\end{align}
%
For a program to be physically realizable on a quantum computer, its semantics must be \emph{completely positive and trace non-increasing}.
...
For example, the program~$S$ \emph{terminates almost surely} (that is, with probability one) when started in state~$\rho$ if, and only if $\tr(\Bracks{S}(\rho))=1$.
When a program is \emph{trace preserving}, it terminates on any input state.
...
%-----------------------------------------------------------------------------
\subsection{%Specifying Program Properties --
Quantum Hoare Logic}
...
As in probabilistic Hoare logic~\cite{Morgan1998pGCLFR}, instead of assigning a definite truth value to a given predicate and state, we rather assign an \emph{expectation} or degree to which the predicate holds in the given state -- namely the probability of getting outcome~$1$ if one were to apply the measurement~$\vec B$.
...
As explained below \cref{def:semantics}, the term $[\tr(\rho) - \tr(\Bracks{S}(\rho))]$ in \cref{eq:def par cor} is the \emph{probability of non-termination} of the program~$S$ when started in the initial state~$\rho$.
...
% it follows from \cref{lem:projections as predicates} that
...
Thus:
%
% By plugging $S = \qpenv{\qpskip}$ into \cref{lem:projections as predicates}, we obtain the following:
%
...
% , as well as the pre- and postconditions that specify what it means for the search algorithm to be correct,
...
% , except for the last line, which defines the hole construct.
...
%
  ...
  $\qpenv{\qphole{P}{Q}} \parref \qpenv{
    \begin{aligned}
    &\qpselect {\vec M} {\vec q} {\\&\quad \{\omega \colon \qphole{P_\omega}{Q}\}_{\omega\in\Omega}\\&}
    \end{aligned}
  }$
...
%
  \ruleitem{HP.while}
    $\qpenv{\qphole{P}{Q}} \parref \qpenv{
      \begin{aligned}
      &\qpwhile{B}{\vec q}{\\&\quad \qphole{R}{\cB_{0,\q}(Q) + \cB_{1,\q}(R)} \\&}
      \end{aligned}
    }$,
...
We explain the \ruleref{HP.split}, \ruleref{H.repeat}, \ruleref{H.case} and \ruleref{HP.while} rules.
%
...
\begin{enumerate}[align=right,leftmargin=1.4cm]
\ruleitem{H.ifElse}
  $\qpenv{\qphole{P}{Q}} \parref \qpenv{
    \begin{aligned}
...
\begin{enumerate}[align=right,leftmargin=.9cm]
\ruleitem{H.if}
...
\begin{enumerate}[align=right,leftmargin=1cm]
  \ruleitem{H.sw}
...
The quantum setting poses some interesting new challenges. % that do not arise in the classical setting.
...
We leave the problem of finding suitable heuristics % for deriving such predicates
to future work.
...
\ruleitem{HT.while}
  ...
  \begin{aligned}
      &\qpwhile{B}{\q}{\\
      &\quad \qphole{R_{n+1}}{\cB_{0,\q}(Q) + \cB_{1,\q}(R_n)} \\
      &}
  \end{aligned}
  }
  $, \\
...
See \cref{sec:examples} for examples and \cref{sec:related-work} for related work on proving termination of probabilistic programs.
...
Additionally, we also discuss a pedagogical example of tossing a quantum coin until zero in \cref{app:coin-toss-till-zero}.
...
\begin{enumerate}
  ...
  \item Alice measures her qubits~$q,a$ in the standard basis (without loss of generality) and obtain as outcomes two classical bits~$x,y$, which we imagine she communicates to Bob.
...
% We can label the two remaining holes as
% $S_\text{Alice} = \qpenv{\qphole{\proj{\phi^+}_{qr} \ot \proj{\phi^+}_{ab}}{P}}$ and
% $S_\text{Bob} = \qpenv{ \qphole{Q(x, y)}{\proj{\phi^+}_{br}} }$.
...
The second refinement is valid assuming $U_b(x,y) Q(x,y) U_b^\dagger(x,y) \imp \proj{\phi^+}_{br}$, which by \cref{eq:teleport form of Q} we can satisfy by picking~$R_{br}(x,y) = U^\dagger_b(x,y) \proj{\phi^+}_{br} U_b(x,y)$.
...
\end{enumerate}
Second, we give a rule to reduce the construction of programs that succeed with probability one to ones that succeed with some finite probability~$\eps\in(0,1)$, by repeating the program an \emph{unbounded} number of times until it succeeds:
\begin{enumerate}[align=right,leftmargin=1.9cm]
...
The relations \ruleref{H.boostRep} and \ruleref{H.boostWhile} hold.
\end{restatable}
...
The Correct\-ness-by-Construction (CbC) approach to programming was initially introduced by \citet{10.5555/550359} and \citet{gries_science_1981}
...
in the spirit of the concept of \emph{typed holes} in functional programming~\cite{wiki2014typedholes,10.1145/3290327}.
...
We presented refinement rules that iteratively refine such quantum programs and proved that these rules are sound and complete: every program is guaranteed to satisfy the specification it was constructed from and every correct program can always be constructed from the specification.
...
We take these findings to suggest that QbC could meaningfully support the design of novel quantum algorithms, their taxonomization, and the construction and verification of larger quantum software.
...
Second, it would be interesting to extend the QbC methodology to other program semantics as a natural next step that might also inform other challenges,
such as the expected-runtime calculus introduced by~\citet{Liu2022qweakest} to construct programs that are \emph{efficient by construction}.
...
As $S' \parref^{k-1} S$, by \cref{lem:chain_split} we must have $S = \qpenv{\qpwhile{B}{\vec q}{\qpabsprog{C}}}$ for some program~$C$ such that $\qpenv{\qphole{R}{\cB_{0,\q}(Q) + \cB_{1,\q}(R)}} \parref^{k-1} C$.
...
\begin{proof}
  ...
  \begin{proofcase}
    ...
    We can thus construct~$S$ by first using the refinement rule \ruleref{HT.while} with the above family~$\{R_n\}$, to obtain $\qpenv{\qphole{P}{Q}} \totref \qpenv{\qpwhile{\q}{B}{\qphole{R_{n + 1}}{\cB_{0,\q}(Q) + \cB_{1,\q}(R_n)}}}$, and then \ruleref{C.while}, proving this case.
  \end{proofcase}
...
\end{align*}
Thus we see that we can refine the loop body using %\ruleref{C.while} and -- we don't mention these earlier
...
In this section we discuss a worked example that could be helpful for readers with less familiarity with quantum computing.
It is meant to illustrate both the preliminaries of \cref{sec:prelims} and the new concepts introduced in our paper.
...
\begin{align*}
  ...
  = \proj0.
\end{align*}
Therefore the semantics of the coin toss until zero program~(\cref{ex:coin-toss:program}) is given by
...
As such, \cref{eq:orig toss until zero} is not a very interesting specification, as much simpler programs than our original program~\eqref{ex:coin-toss:program} also satisfy it, e.g.,\ $\qpenv{\qpinit{q}}$.
...
Putting it all together, we have re-constructed \cref{ex:coin-toss:program} from a specification:
\begin{gather*}
