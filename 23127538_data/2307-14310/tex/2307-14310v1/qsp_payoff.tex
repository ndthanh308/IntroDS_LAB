...
Financial derivatives can be priced on quantum computers using Quantum Amplitude Estimation (QAE) \cite{brassard2002quantum} with a quadratic speedup compared to classical Monte Carlo methods \cite{montanaro2015quantum,rebentrost2018quantum, Woerner_2019, Stamatopoulos_2020, doriguello_et_al:LIPIcs.TQC.2022.2}.
...
We focus our attention on payoffs involving the calculation of $f(x)=\left(Ae^x - B\right)/C$ because this particular form is ubiquitous in derivative pricing \cite{Hull}.
...
\end{equation}
where $\ket{\psi_0}_n$ and $\ket{\psi_1}_n$ are normalized quantum states, we are looking for a unitary which prepares the state
...
Defining projectors $\tilde{\Pi} \equiv \identity^{\otimes 2n} \otimes \ket{0}\bra{0}$ and $\Pi \equiv \identity^{\otimes n} \otimes \left(\ket{0}\bra{0} \right)^{\otimes n+1}$, we observe that $\tilde{\Pi}U\Pi$ is a rank-1 matrix with a single non-trivial singular value $g(x)$ \cite{gilyen2019quantum}.
Consecutive invocations of $U$ and $U^{\dagger}$, interleaved with projector-controlled rotation operators $\Pi_{\phi}=e^{i\phi(2\Pi - \identity)}$ and $\tilde{\Pi}_{\phi}=e^{i\phi(2\tilde{\Pi} - \identity)}$, allow us to apply polynomial transformations to this singular value, using the Quantum Signal Processing (QSP) framework.
Given phase factors $\vec{\phi}=(\phi_1, \phi_2, \cdots, \phi_d)$, define
...
\end{equation}
Then, for any polynomial $P$ satisfying $\textrm{deg}(P) \le d$, $|P(a)| \le 1, \forall a \in [-1, 1]$, and $P$ either even or odd, there exist phase factors $\vec{\phi}$ \cite{Haah2019product, chao2020finding, dong2021efficient} such that the unitary
...
%	\begin{cases}
%			\left(P(\sqrt{x})\ket{\psi'_0}_n\ket{0}_2 + \sqrt{1-P(\sqrt{x})^2}\ket{\psi'_1}_{n}\ket{0_{\perp}}_2 \right), & \text{for odd $d$,} \\
%			& \\
%			\left(P(\sqrt{x})\ket{0}_{n+2} + \sqrt{1-P(\sqrt{x})^2}\ket{0_{\perp}}_{n+2} \right), & \text{for even $d$.}
%	\end{cases}
%\end{equation}
\begin{equation}
	\label{eqn:qsp_general_transformation}
	...
	\end{cases}
\end{equation}
With an appropriately chosen polynomial $P$ which approximately performs

\begin{equation}
	P(x) \rightarrow \sqrt{\frac{Ae^{g^{-1}(x)} - B}{C}},
\end{equation}
we can then use the unitary $U^{\vec{\phi}}$ to approximately prepare the target state in Eq.~\eqref{eqn:general_exp_state}. A circuit description of the operator $U^{\vec{\phi}}$ is shown in Fig.~\ref{fig:qsp_circuit}.

This idea of encoding a function $g(x)$ into a quantum amplitude and then transforming it into another function $P(x)$ by virtue of approximating $P(g^{-1}(x))$ using QSP was considered in Ref.~\cite{mcardle2022quantum} for quantum state preparation, where the unitary in Eq.~\eqref{eqn:general_amplitude_oracle} is taken to be
...
This unitary can also be used to approximately generate our target state in Eq.~\eqref{eqn:general_exp_state}, by picking the polynomial transformation $P(x) \rightarrow \sqrt{\frac{Ae^{\arcsin(x)} - B}{C}}$.
...
\end{equation}
such that QSP needs to produce the transformation $P(x) \rightarrow \sqrt{\frac{Ae^{x^2} - B}{C}}$ in order to construct Eq.~\eqref{eqn:general_exp_state}.
...
%One way this unitary can be constructed is with the ripple-carry approach described in Ref.~\cite{draper2006logarithmic}.
...
\begin{enumerate}
    \item A set of $k$ binary payoffs $\{K_i, t_i, f_i \}$, such that if the asset returns at time $t_i$ exceed $K_i$, the holder of the contract receives a return $f_i$ and the contract terminates.
...
\end{enumerate}
Because an autocallable is typically defined in terms of an asset's performance, it also includes a notional value $V$ on which the payoff returns are evaluated to determine a dollar amount to be exchanged.
...
%We add $k$ qubits to evaluate whether any of the binary payoffs are triggered on path $\ket{r}$
%
%\begin{equation}
%	\ket{r}\bigotimes_{i=1}^{k}\ket{s_i},
%\end{equation}
...
%Then, we apply the transformation of Eq.~\eqref{eqn:general_exp_state} using QSP, controlled on the ancilla qubit to approximately prepare the state
...
%A schematic representation of the quantum circuits corresponding to these two approaches of pricing an autocallable contract is shown In Fig.~\ref{fig:reparam_circuits}.
...
%In order to satisfy the target accuracy requirements, the register $\ket{r_T}$ in the fixed-point representation of Eq.~\eqref{eqn:fixed_point_repr} was chosen in Ref.\cite{chakrabarti2021threshold, qsharpblog} to use $(n, p)=(15, 5)$ \cite{chakrabarti2021threshold, qsharpblog}, such that $r_T \in [-R, R]$ with $R=2^5$.
...
%, which
%In order to satisfy the target accuracy requirements, Ref.~\cite{chakrabarti2021threshold, qsharpblog} choose $(n, p)=(15, 5)$ for the fixed-point representation of Eq.~\eqref{eqn:fixed_point_repr} for the register $\ket{r_T}$, such that $r_T \in [-R, R]$ with $R=2^5$.
...
\begin{table*}
  \begin{center}
    ...
    \begin{tabular}{ccccc}
       ...
       &  &$\Usin$ [Eq.~\eqref{eqn:sine_amplitude_oracle}] & $\Usqrt$ [Eq.~\eqref{eqn:sqrt_amplitude_oracle}]  & \\
      \hline
      \textbf{$T$-depth} & $36$k & $75$k & $7.8$k  & 4.7x\\
      \textbf{$T$-count} & $6.6$M & $605$k & $414$k  & $16$x\\
	  \textbf{\# Qubits}  & $19.2$k & $4.7$k & $4.7$k & $4.1$x\\
    \end{tabular}
	\end{ruledtabular}
...
%Example polynomial approximations generated using this method for the threshold function $\theta_{\mu}(x)$ at $\mu=0.5$ are shown in Fig.~\ref{fig:thres_approximation}.
