...
Table \ref{tab:models_results} shows each model's log loss and accuracy. As we can see, the spectral ranking model exhibits a lower accuracy than the remaining models, closer to a random predictor. It showcases that, with this number of available comparisons, it cannot learn cycling safety perceptions from these environments. TrueSkill reveals the lowest log loss but with values close to those of the Elo rating model. In turn, GP showcases the highest accuracy but with a log loss much higher than that of TS, meaning that, while it is more accurate, its probability of choosing the winning environment is usually much lower than that of TS or Elo.
