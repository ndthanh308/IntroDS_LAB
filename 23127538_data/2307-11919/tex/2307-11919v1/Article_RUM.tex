...
\AFF{Léonard de Vinci Pôle Universitaire, Research Center, 92 916 Paris La Défense, France and Laboratoire de Mathématiques de Reims, UMR9008 CNRS and Université de Reims
Champagne-Ardenne, France \EMAIL{laurence.carassus@devinci.fr}}
\AUTHOR{Massinissa Ferhoune}
\AFF{Léonard de Vinci Pôle Universitaire, Research Center, 92 916 Paris La Défense, France and Laboratoire de Mathématiques de Reims, UMR9008 CNRS and Université de Reims
Champagne-Ardenne, France, \EMAIL{massinissa.ferhoune@devinci.fr}}
} 
...
In this article, we are interested in the following question : In a frictionless discrete-time market, does an investor, who tolerates negative wealth and who faces  model ambiguity, have an optimal investment strategy ? %i.e. concave and non-decreasing functions. 
Model uncertainty refers to random phenomena whose probability of occurrence are uncertain. For example, the probability of having a tail in a coin flip may not be $1/2$ but unknown. As the world becomes more and more unpredictable, model ambiguity has become a prime issue especially in financial economics. This notion of model ambiguity, sometimes referred as ``unknown unknowns",  goes back to Knight, see \cite{refkni}, and is therefore called Knightian uncertainty. To address our research question, we will model the preference of the investor with a utility function. The classical utility maximization problem i.e. without Knightian uncertainty, %i.e. the set of priors is reduced to a singleton, 
has a rich literature and we refer to \citep{refFS} and the references therein for a detailed survey. In a general semimartingale setting for concave utility functions defined on $\mathbb{R}$, \citep{refAE2} shows using dual methods that Asymptotic Elasticity (AE) constraints, both on $-\infty$ and $+\infty$, are necessary in order to obtain existence of an optimal investment strategy. As a result, constraints on AE (which was first introduced in \citep{refAE1}) has become an essential part of the usual assumptions postulated for solving utility maximization problems. Loosely speaking, the AE constraint at $+\infty$ (resp. $-\infty$) states that for positive (resp. negative) large enough  wealth, the marginal (resp. average) utility is small (resp. big) enough in comparison to the average (resp. marginal) utility. In discrete time, \citep{ref2} proves similar results using primal methods and postulating AE constraint only on one side. Later, \citep{ref7} extends those results to non-concave utility functions, see also \citep{ref10}. 

Going back to model uncertainty,  a set $\mathcal{Q}$ of probability measures, also called priors, usually models all the investor's beliefs about the market. The earliest literature assumed that $\mathcal{Q}$ is dominated. We refer to \citep{refdoml} for an extensive survey on the dominated case. This setting excludes models with uncertainty about volatility and to overcome this issue, Bouchard and Nutz consider the notion of quasi-sure uncertainty, see \citep{refbn1}. Random sets of ``local" priors are first defined. These probability measures are ``local" in the sense that they represent the investor's belief between two consecutive moments. The set of priors is then constructed by taking the Fubini product of those ``local" priors. The setting of \citep{refbn1} assumes that the graph of these random sets are analytic set, see \cite{ref1} for a comprehensive presentation of these sets. This is a sufficient level of measurability to ensure that measurable selection technics can be applied. Note that the set of probability measures $\mathcal{Q}$ is neither assumed to be compact or to be dominated by a specific measure. In the case of model ambiguity, Gilboa and Schmeidler \citep{refgs} characterize the preference of the agents with the following numerical representation $X\mapsto \inf_{Q\in\mathcal{Q}} \mathbb{E}_Q U(X)$. So, optimal investment amounts to proving existence in a maxmin expected utility problem. 
In the discrete time quasi-sure setting, \citep{ref9} solves this maxmin problem for concave and bounded from above utility functions, when only positive wealth are admissible i.e. the utility functions are defined on the positive axis. The same result is proved in \citep{ref4} for unbounded functions.  The literature is rather new when dealing with potentially negative wealth in the discrete time nondominated quasi-sure setting. We first mention papers using a primal approach. The existence of an optimal investment strategy is proved in \cite{ref11} for a one-period market under quasi-sure uncertainty. Then, \citep{ref8} have derived existence results in a different framework where the uncertainty is represented by a set of stochastic processes. There doesn't seem to be an obvious link between their framework and the quasi-sure framework of \citep{refbn1}. Then,  \citep{refns} obtains existence for non-concave and bounded from above utility functions but assuming that the assets's positions the investors can take  are in a discrete set. Further results were also obtained using a dual approach. First, \citep{refbar1} solves the utility maximization problem for an exponential utility function assuming that the local no-arbitrage condition holds true on the whole probability set and not only on universally-measurable sets of $\mathcal{Q}$-full-measure. Some of these results have been extended in \citep{refbar3} but using the rather strong assumption that medial limits exist. 
To the best of our knowledge, no general results have been obtained concerning the existence of an optimal investment strategy in the discrete-time nondominated quasi-sure setting of \citep{refbn1}, when the wealth of the investor can be negative. In this article, %we rely on \citep{ref3}, \citep{ref4}, \citep{ref7} and \citep{ref11} and 
we show that under technical conditions (involving no medial limits) on the market and on the utility function, an optimal investment strategy exists. Since beliefs are uncertain, we consider random utility functions. 

Our dynamic programming procedure is inspired by \citep{ref3} but is adapted to take into account negative wealth. Some ideas on the formulation of the one-period problem are similar to the one of \cite{ref11}. Nevertheless, we introduce here some specific set $\mathcal{H}$ of priors $P$ for which the $P$ no-arbitrage condition holds true in a quasi-sure sense. The existence of such priors has been proved in \citep{ref4} under the quasi-sure no-arbitrage condition of \citep{refbn1}. 
%Then, we follow the uni-prior approach of \citep{ref7} and adapt  their one-period assumptions for the priors in $\mathcal{H}$. 
Most of the assumptions that we postulate in the one-period case are taken from \citep{ref7} (which ignores ambiguity) and are stated on each prior of $\mathcal{H}$
...
Note, however, that in our quasi-sure setting, we have to assume that the utility function is concave (which is not the case in \citep{ref7}) in order to obtain the regularity of the value functions $U_t$ introduced in the dynamic programming. 
We first state assumptions which can be directly tested when the utility and the market are specified. The first one is the classical AE constraints. The second one requires that $U$ is ``negative enough"  and is automatically satisfied for deterministic non-constant concave  utility functions. The third one states that some simple strategies are admissible and is the single assumption which is postulated in a strong sens, i.e. considering a  supremum over all priors. This assumption ensures a control from below for the value functions. 
Our last condition cannot be directly checked. It requires that the value function $U_0^{P}$ at time 0  for each prior $P\in \mathcal{H}$ is finite. This is the well-accepted assumption of \citep{ref2}. What is nice here is that the assumption is stated  on  a prior-by-prior basis  not for the supremum over all priors. So, it can be verified as in the  setting without ambiguity. 
This assumption provides a control from above for the value functions. We will give an example where there is some prior $P^*\in \mathcal{H}$   such that $U_0^{P^*}=+\infty$ and there does not exist an optimal strategy though the value function is finite. 
As Rasonyi and Stettner, we propose a general setting where this assumption is automatically satisfied. For that, we introduce a new kind of random utility functions called of type (A). We show that the random utility with benchmark introduced by \cite{refkahn} are of type (A). 


From this assumptions, we construct in Theorem \ref{one_step_strategy} the optimal strategies following the idea of \citep{ref2} and \citep{ref9}. We prove existence of one-period optimal strategies on $\mathbb{R}^d$. Then, we ``glue" together the solutions found in the one-period case using dynamic programming together with measurable selection technics. This is where the analycity of the graph of priors is required. These ``glued" strategies may fail to be admissible. But, we show in Theorem \ref{one_step_strategy} that when these strategy are admissible, they are optimal. 
In Theorem \ref{optimality_M_typeA}, we show that for random utility of type (A)  such ``glued" solutions are automatically  admissible and, thus optimal, under additional integrability assumptions on the market. 

The remainder of this article is organized as follows. In Section \ref{sett_main}, we introduce the financial model, the assumptions as well as the main results. In Section \ref{op}, we solve the utility maximization problem in a one-period market, while in Section \ref{muti_per}, we prepare the dynamic programming procedure. Section \ref{proof_th1} presents the proof of Theorem \ref{one_step_strategy}, while Section \ref{proof_optimality_M} the one of Theorem \ref{optimality_M_typeA}. The appendix contains further proofs and results, in particular on AE and on a specific integral that we introduce in order to preserve concavity through the dynamic programming procedure.
...
Let $S:=(S_t)_ {0\leq t\leq T}$ be a $\mathbb{R}^d$-valued process representing the discounted price of $d$ risky assets over time. We first assume that $S$ is Borel-adapted. This assumption is usual in the quasi-sure financial literature (see \citep{refbn1}) and aims at solving measurability issues.
\begin{assumption}
...
In presence of uncertainty about the law of nature, we expect that any characterization of the investor\rq{}s behavior  should also be uncertain. Hence, the choice of a random utility. The concavity and monotonicity of $U$ ensure that the investor is risk-averse and always seeks for more money. Remark that U is not necessarily finite and thus the usc condition is not vacuous. %Remark that U is not necessarily finite and so that the usc condition is not superfluous.
...
\begin{remark}
The random utility $U$ is $\mathcal{B}(\Omega^T) \otimes \mathcal{B}(\mathbb{R})$-measurable (see \citep[Lemma 5.13]{ref10}) as $U(\w,\cdot)$ is right-continuous using \citep[Lemma 5.10]{ref10}).
\label{joint_mes_U}
\end{remark}
...
If $P:= q_1 \otimes q_2 \otimes \cdot\cdot\cdot \otimes q_T \in\mathcal{Q}^T$, we write for any $1\leq t\leq T$, $P^t:= q_1 \otimes q_2 \otimes \cdot\cdot\cdot \otimes q_t$ and $ P^t\in\mathcal{Q}^t$. In this paper, most of the time, we work directly on the disintegration of $P$ rather than $P$. So, from now, we will precise the fixed disintegration for which the required result holds true.
...
In this paper, we will use an adhoc integral called $\int_{-}$ in order to preserve the concavity of the value functions. When this integral is finite, it is just the usual intagral. Else, it is computed using the convention :
\begin{eqnarray}
\label{cvt_inf}
-\infty+\infty=+\infty-\infty=-\infty.
\end{eqnarray}
Unfortunately, \citep{ref1} uses the opposite convention. Nevertheless, we will show in Appendix \ref{inf_cvt_sub} that some importants results of \citep{ref1} still hold true for $\int_{-}$. From now, we will simply write $\int$ (or $\mathbb{E}$) instead of $\int_{-}$ except when we need to clarify the difference between both integrals. We now introduce the set of admissible strategies for the utility maximization problem. 
...
We aim to solve the following max-min utility problem of an investor looking for an optimal hedging strategy when the uncertainty about the true probability of future events is modeled by $\mathcal{Q}^T$ :
\begin{eqnarray}
u(x):=\sup_{\phi\in \Phi(x,U,\mathcal{Q}^T)}\inf_{P\in \mathcal{Q}^T}\mathbb{E}_P U(\cdot,V_T^{x,\phi}(\cdot)).
\label{RUMP}
\end{eqnarray} 
...
We start with the assumptions which can be directly and easily checked. The first one is on the asymptotic behavior of the random utility $U$. This condition already appeared in \citep[Proposition 4]{ref7} for a finite nondecreasing continuous  and non necessarily concave function in the uni-prior setting. See also \citep[Proposition 5.1]{ref2} and \citep[Proposition 3.24]{ref3}. 
...
\label{AE}
\end{assumption}
A consequence of Assumption \ref{AE} is that a one-step strategy must be bounded in order to be optimal (see Proposition \ref{sub_optimal}) and this compactness result will allow us to prove existence of an optimal strategy. 
...
The following assumption requires some strong admissibility of simple strategies and is discussed in the one-period setting after Assumption \ref{integ_V+}. 
\begin{assumption}
...
We now introduce the dynamic programming procedure and the associated value functions. We do it for the multiple-priors utility maximization problem \eqref{RUMP} with the value functions $U_t$ and also for the utility problem \eqref{RUMP_one_prior} related to a given prior $P$ with the value functions $U_t^P$. Fix $P\in\mathcal{Q}^T$ with fixed disintegration $q_1^P\otimes \cdots \otimes q_T^P$. Let  
\begin{eqnarray}
...
We first comment on the set $\mathcal{H}^T$.
\begin{lemma}
...
So, the set $\mathcal{H}^T$ shares the same properties as the set $\mathcal{P}^T$ of \citep[Theorem 3.6]{ref4}. This set $\mathcal{P}^T$ can be defined as the set of kind of convex combinations of some fixed $P^*:= q_1^* \otimes \cdots \otimes q_T^*\in\mathcal{H}^T$ and of the $Q:= q_1^Q \otimes \cdots \otimes q_T^Q\in\mathcal{Q}^T$ i.e.
\begin{eqnarray}
...
\label{U0}
\end{assumption}
Before commenting on Assumption \ref{U0}, we first link it to the existence of kind of admissible one-step optimal strategies for all priors in $\mathcal{H}^T$ and to the finiteness of the uni-prior value functions.
\begin{lemma}
...
\begin{theorem}
Let $U$ be a random utility (see Definition \ref{U_hp}). Assume that the $NA(\mathcal{Q}^T)$ condition as well as Assumptions \ref{S_borel}, \ref{analytic_graph}, \ref{AE}, \ref{nncst}, \ref{simple_adm} and \ref{U0} hold true. 
Then, one can find some $(\phi^{*,x})_{x\in\mathbb{R}}\subset \Phi$ and for all $0\leq t \leq T-1$, some $\mathcal{Q}^t$-full-measure set $\widehat{\Omega}^t \in \mathcal{B}_c(\Omega^t)$ such that for all $x\in\mathbb{R}$ and $\w^t\in\widehat{\Omega}^t$, we have that $\phi^{*,x}_{t+1}(\w^t)\in \textup{Aff}(D_{t+1})(\w^t)$ and that
%\begin{eqnarray}
%U_{t}(\w^t,V_t^{x,\phi^{*,x}}(\w^t))&=&\sup_{h\in\mathbb{R}^d}\inf_{p\in\mathcal{Q}_{t+1}(\w^t)}\int_{\Omega_{t+1}}{U_{t+1}(\w^t,\w_{t+1},V_{t}^{x,\phi^{*,x}}(\w^t)+h \Delta S_{t+1}}(\w^t,\w_{t+1}))\;p(d\w_{t+1}),\label{one_step_strategy_temp1}\\
%U_{t}(w^t,V_t^{x,\phi^{*,x}}(w^t))%
%&=&\inf_{p\in\mathcal{Q}_{t+1}(\w^t)}\int_{\Omega_{t+1}}{U_{t+1}(\w^t,\w_{t+1},V_{t+1}^{x,\phi^{*,x}}(\w^t,\w_{t+1})})\;p(d\w_{t+1}).
...
%\end{eqnarray}
\begin{eqnarray}
U_{t}(\w^t,V_t^{x,\phi^{*,x}}(\w^t))&=&\sup_{h\in\mathbb{R}^d}\inf_{p\in\mathcal{Q}_{t+1}(\w^t)}\mathbb{E}_p U_{t+1}\left(\w^t,\cdot,V_{t}^{x,\phi^{*,x}}(\w^t)+h \Delta S_{t+1}(\w^t,\cdot)\right)\label{one_step_strategy_temp0}\\ %\label{one_step_strategy_temp1}\\
...
The proof of Theorem \ref{optimality_pp} is quite involved and is delayed to Section \ref{proof_th1}. To construct $\phi^{*,x}$, one glues all the one-step optimal strategies constructed in Section \ref{op} together. Indeed, the one-step strategy $\phi^{*,x}_{t+1}$ will be the optimal strategy to adopt between time $t$ and $t+1$ starting from an initial wealth equal to $\sum_{s=1}^t \phi^{*,x}_s \Delta S_s$ i.e. if one have already followed the strategies $(\phi^{*,x}_{1},\cdot\cdot\cdot,\phi^{*,x}_{t})$ until time $t$. Note that the strategy $\phi^{*,x}$ belongs to $\Phi$ but may fail to be admissible and thus to be a solution of (\ref{RUMP}). However, when $\phi^{*,x}$ is admissible, it achieves the supremum in (\ref{RUMP}). Note that if $\phi^{*,x}$ is not admissible, $\inf_{P\in\mathcal{Q}^T}\mathbb{E}_P U(\cdot,V_T^{x,\phi^{*,x}}(\cdot))=-\infty$ by convention \eqref{cvt_inf} and $\phi^{*,x}$ is not optimal. The condition that $\phi^{*,x}\in \Phi(x,U,\mathcal{Q}^T)$ is obviously not easy to verify. So, we now introduce random utility functions for which this holds true. For that, we first define some sets of random variables which are integrable enough.
\begin{definition}
...
\label{M_tP}
\end{definition}
Note that $\mathcal{W}^0=\mathcal{M}^0=\mathcal{M}^{0}(P)=\mathbb{R}$. It is also clear that $\mathcal{W}^t \subset \mathcal{M}^t\subset \mathcal{M}^t(P)$ for all $P\in\mathcal{Q}^t$. The set $\mathcal{W}^T$ has already been introduced in \citep[p1866]{ref3} (see also \citep[p6667]{ref4}) but for Borel-measurable random variables. 
...
\end{eqnarray}
Let $Z : \Omega^T \to \mathbb{R}$ be $\mathcal{B}(\Omega^T)$-measurable and in $\mathcal{W}^T$. A function $U : \Omega^T\times \mathbb{R} \to \mathbb{R}$ is a  utility function with random benchmark if $U(\w^T,x):=\widetilde{U}(x-Z(\w^T))$ for all $\w^T\in\Omega^T$ and $x\in\mathbb{R}$.
%\begin{eqnarray*}
...
\begin{theorem}
Let $U$ be a random utility of type $(A)$ (see Definition \ref{typeA}). Assume that the $NA(\mathcal{Q}^T)$ condition as well as Assumptions \ref{S_borel} and \ref{analytic_graph} hold true. Moreover, suppose that for all $P\in \mathcal{H}^T$ and for all $0\leq t \leq T-1$, $1/\alpha_{t}^P\in \mathcal{M}^{t}$ and  $|\Delta S_{t+1}|\in\mathcal{W}^{t+1}$.
...
\end{eqnarray}
Then, $v(x)=\sup_{h\in\mathbb{R}^d}\Psi(x,h)$, see \eqref{v}. Recall that the expectations in \eqref{Phi} are defined in the generalized sense using the convention $+\infty-\infty=-\infty+\infty=-\infty$ and that $\Psi_p$ and $\Psi$ are well-defined although being potentially infinite. The function $\Psi_p$ is introduced as a tool to prove properties on $\Psi$ and $v$ but also to pass Assumption \ref{U0} through the dynamic programming procedure. The results on $\Psi_p$ can be find in \citep{ref2} or \citep{ref11} but under a different set of assumptions.

 
\begin{proposition}
Fix $p\in\mathcal{Q}$. Assume that Assumption \ref{V} holds true. Then, $\Psi_p$ and $\Psi$ are concave. Assume now that Assumption \ref{integ_V-} holds true. Then, for all $x\in\mathbb{R}$ and $h\in\mathbb{R}^d$, 
$\sup_{p\in\mathcal{Q}} \mathbb{E}_p V^-(\cdot,x+h Y(\cdot)) <+\infty$ and $\Psi_p>-\infty$ and $\Psi>-\infty$. Assume furthermore that Assumptions \ref{P^*}, \ref{AE_one} and \ref{integ_V+} hold true.  Then, $\Psi$ is finite and continuous and for all $x\in\mathbb{R}$ and $h\in\mathbb{R}^d$,
\begin{eqnarray}
...
\end{proposition}
\proof{Proof.}
Fix some $p\in\mathcal{Q}$. First, assume that only Assumption \ref{V} holds true. Recall that a (extended) function $f : \mathbb{R} \to \mathbb{R}\cup\{-\infty,+\infty\}$ is said to be concave if for all $0<t<1$ and $x,y\in\mathbb{R}$,
$$f(tx+(1-t)y)\geq tf(x)+(1-t)f(y),$$ using convention \eqref{cvt_inf}. Let $(x_1,h_1)$ and $(x_2,h_2)$ in $\mathbb{R}\times \mathbb{R}^d$ and $0<t<1$. Using the concavity of $V$ and in \eqref{eq_ccv_psi}, \citep[Lemma 7.11, (a), p139]{ref1} adapted to convention \eqref{cvt_inf}, we get that
\begin{eqnarray}
...
\end{eqnarray}
and $\Psi_p\geq \Psi > -\infty$ for all $p\in\mathcal{Q}$.\\
Assume now that Assumptions \ref{P^*}, \ref{AE_one} and \ref{integ_V+} also hold true. 
...
\begin{proposition}
Suppose that Assumptions \ref{P^*}, \ref{V}, \ref{AE_one}, \ref{integ_V-}, \ref{integ_V+} and \ref{pb_inequality} hold true.
...
Suppose that Assumptions \ref{P^*}, \ref{V}, \ref{AE_one}, \ref{integ_V-}, \ref{integ_V+} and \ref{pb_inequality} hold true and
let 
\begin{eqnarray}
...
For the rest of this part, we fix a random utility function $U$ in the sense of Definition \ref{U_hp}. For $0\leq t\leq T$, we define by induction $J_t : \Omega^t \times \mathbb{R} \to \mathbb{R}\cup\{-\infty,+\infty\}$ and $C_t : \Omega^t \to \mathbb{R}\cup\{-\infty,+\infty\}$ as follows : for all $\w^t\in\Omega^t$ and $x\in\mathbb{R}$, 
\begin{eqnarray}
&&\left\{
    \begin{array}{ll}
    J_T(\w^T,x)&:=U^-(\w^T,x)\\
...
\noindent\textit{$J_t$ and $C_t$ are non-negative, usa and satisfy for all $1\leq k\leq t$, $x\in\mathbb{R}$, $h\in\mathbb{R}^d$ that \begin{eqnarray}
\sup_{P\in \mathcal{Q}^t} \mathbb{E}_P J_t(\cdot,x+h\Delta S_k(\cdot))<+\infty \quad \mbox{and} \quad \sup_{P\in\mathcal{Q}^t}\mathbb{E}_P C_t<+\infty.
\label{property_temp}
\end{eqnarray}}\\ 
...
Let $0\leq t\leq T-1$. Assume that $J_{t+1}$ and $C_{t+1}$ are non-negative, usa and that \eqref{property_temp} holds at time $t+1$. Let ${\jmath}_t : \Omega^t\times \mathbb{R}\times  \mathfrak{P}(\Omega_{t+1})\times \mathbb{R}^d \to \mathbb{R}\cup\{-\infty,+\infty\}$ and $\widetilde{\jmath}_t : \Omega^t\times \mathbb{R}\times  \mathbb{R}^d \to \mathbb{R}\cup\{-\infty,+\infty\}$ be for all $(\w^t,x,p,\overline{h})\in \Omega^t\times \mathbb{R}\times \mathfrak{P}(\Omega_{t+1}) \times \mathbb{R}^d$ defined by 
\begin{eqnarray}
...
We now show some fundamental properties of the value functions $U_t$ and $U_t^P$.
...
Now, the counterpart of $c^*$, $l^*$ and $n_0^*$ in the $P$-prior $(t+1)$ context are $c_t^P(\w^t)$, 
\begin{eqnarray}
...
Now, for $\w^t\in\Omega^t_{12}$, $N_t^*(\w^t)<+\infty$, so that $$q^{P^*}_{t+1}\left (U_{t+1}(\w^t,\cdot, -N_t^*(\w^t))\leq - \left(1+2 \frac{c_t^{P^*}(\w^t)}{\alpha_t^{P^*}(\w^t)}\right) \bigg| \w^t \right)\geq 1-\frac{\alpha_t^{P^*}(\w^t)}{2}.$$ The fact that $\w^t\in\Omega_{qNA}^{t,P^*}$ shows \eqref{qNA} and Assumption \ref{pb_inequality} holds true in robust $(t+1)$ context.
\Halmos \endproof

We now prove that the $\Omega^t_{i}$ are $\mathcal{Q}^t$-full-measure sets while the $\Omega^{t,P}_{i}$ are $P^t$-full-measure sets. The proof need the technical Lemmata \ref{lemma1+} and \ref{lemma_N_finite} which are relegated to Appendix \ref{miss_proof}.

\begin{proposition}%ajouter U_t^P ccv et tt ?
Assume that the $NA(\mathcal{Q}^T)$ condition as well as Assumptions \ref{S_borel}, \ref{analytic_graph}, \ref{AE} and \ref{simple_adm} hold true. Let $P\in\mathcal{H}^T$ and $0\leq t \leq T-1$. Then, for all $7\leq i \leq 10$, $\Omega_{i}^t$ is a $\mathcal{Q}^t$-full-measure set and $\Omega_{i}^{t,P}$ is a $P^t$-full-measure set. Assume furthermore that Assumptions \ref{nncst} and \ref{U0} hold true. Then, $\Omega_{11}^t$ and $\Omega_{12}^t$ are $\mathcal{Q}^t$-full-measure sets while $\Omega_{11}^{t,P}$ and $\Omega_{12}^{t,P}$ are $P^t$-full-measure sets. So, there exists a $\mathcal{Q}^t$-full-measure set $\widehat{\Omega}^t \in\mathcal{B}_c(\Omega^t)$ that satisfies $\widehat{\Omega}^t\subset\widetilde{\Omega}^t$.
\label{U_t^-&C}
\end{proposition}
\proof{Proof.}
Fix $P:=q_1^P\otimes \cdots \otimes q_T^P \in\mathcal{H}^T$.\\
\noindent\textit{Let $0\leq t\leq T-1$ and $7\leq i\leq 10$. The sets $\Omega_{i}^t$ and $\Omega_{i}^{t,P}$ are of $\mathcal{Q}^t$-full-measure and also of $P^t$-full-measure under $NA(\mathcal{Q}^T)$ and Assumptions \ref{S_borel}, \ref{analytic_graph}, \ref{AE} and \ref{simple_adm}.}\\ 
By Definitions \ref{def_ctxt_gen} and \ref{def_ctxt}, we see that
\begin{eqnarray*}
\Omega^{t,P}_{7}&:=& \{\w^t\in\Omega^t,\; 0 \in \textup{ri}(\textup{conv}(D_{P}^{t+1})(\w^t))\}\\
\Omega^{t}_{7}&:=& \{\w^t\in\Omega^t,\; 0 \in \textup{ri}(\textup{conv}(D_{P^*}^{t+1})(\w^t)), \textup{Aff}(D_{P^*}^{t+1})(\w^t)=\textup{Aff}(D^{t+1})(\w^t)\},
\end{eqnarray*}
and $\Omega^{t}_{7} \subset \Omega^{t,P^*}_{7}.$ As $P^*$ and $P$ belong to $\mathcal{H}^T$, \eqref{H_def} shows that $\Omega_{7}^t$, $\Omega^{t,P^*}_{7}$ and $\Omega_{7}^{t,P}$ are $\mathcal{Q}^t$-full-measure sets. %As $\mathcal{H}^T \subset \mathcal{Q}^T$, $\Omega_{3.1}^{t,P}$ is also a $P^t$-full-measure set. 
Proposition \ref{U_t_well} at $t+1$ is now in force. Assertions (i) and (ii) together with \citep[Lemma 7.29, p174]{ref1} and Assertion (iii) show that $\Omega^{t}_{8}=\Omega_{8}^{t,P}=\Omega^t$, which is of course of $\mathcal{Q}^t$-full-measure. Using now (iv) and (v), we have that $\bigcap_{(x,h)\in\mathbb{Q}\times\mathbb{Q}^{d}}\Omega^t_{J,x,h} \subset \Omega_{10}^t\subset \Omega_{10}^{t,P}$ and Proposition \ref{CJ_t} shows that $\Omega_{10}^t$ and $\Omega_{10}^{t,P}$ are $\mathcal{Q}^t$-full-measure sets. Fix $\w^t\in\Omega^t$ such that $J_t(\w^t,0)<+\infty$ and $C_t(\w^t)<+\infty$. Let $\w_{t+1}\in\Omega_{t+1}$ such that $J_{t+1}(\w^t,\w_{t+1},0)<+\infty$ and $C_{t+1}(\w^t,\w_{t+1})<+\infty$. Let $\lambda \geq 1$ and $x\in\mathbb{R}$. Lemma \ref{lemmaAE1} together with Assertions (iii) and (iv) and the fact that $C_{t+1}\geq 0$ show that
\begin{eqnarray*}
...
\end{eqnarray*}
Let $C(\w_{t+1}):= J_{t+1}(\w^t,\w_{t+1},0)+C_{t+1}(\w^t,\w_{t+1}),$ then  $c_t^{P^*}(\w^t)= \mathbb{E}_{q_{t+1}^*(\cdot|\w^t)} C(\cdot) \leq C_{t}(\w^t)+J_{t}(\w^t,0)<+\infty$  using \eqref{ineq_ctp2}. So, the inequalities in \eqref{V_pos} are satisfied in the robust $(t+1)$ context with $\underline{\gamma}:=\min(1,\gamma)$ and $\overline{\gamma}:=\max(1,\gamma)$. Thus, $\{\w^t\in\Omega^t,\; J_{t}(\w^t,0)<+\infty,\; C_t(\w^t)<+\infty\} \subset \Omega^t_{9}$. So, \eqref{finite_CJ} in Proposition \ref{CJ_t} shows that $\Omega^t_{9}$ is a $\mathcal{Q}^t$-full-measure set. The same arguments apply for $U_{t+1}^P$ (using \eqref{elas_gammafPt} instead of \eqref{elas_gammaft} and $c_t^P(\w^t)$ instead of $c_t^{P^*}(\w^t)$) and $\Omega^{t,P}_{9}$ is also a $\mathcal{Q}^t$-full-measure set.\\ Note that, as $\mathcal{H}^T \subset \mathcal{Q}^T$, $\Omega^t_{i}$ and $\Omega^{t,P}_{i}$ are also $P^t$-full-measure sets for all $7\leq i\leq 10$.\\
...
%\end{eqnarray*}
\noindent\textit{Let $0\leq t\leq T-1$ and $i \in \{11, 12\}$. The set $\Omega_{i}^t$ is of $\mathcal{Q}^t$-full-measure and $\Omega^{t,P}_{i}$ is of $P^t$-full-measure if we also assume Assumptions \ref{nncst} and \ref{U0}.}\\
The assertions for $\Omega^t_{11}$ and $\Omega^{t,P}_{11}$ are proved in Lemma \ref{lemma1+} in the Appendix. Recall $\Omega^t_{12}$ and $\Omega^{t,P}_{12}$ from Definition \ref{def_ctxt}.
...
In this part, we prove Theorem \ref{optimality_M_typeA}. We will apply Theorem \ref{one_step_strategy} and verify the different conditions needed for that. More precisely, we prove that if Assumptions \ref{S_borel} and \ref{analytic_graph} as well as $NA(\mathcal{Q}^T)$ hold true and if $U$ is a random utility of type $(A)$, then Assumptions \ref{simple_adm} and \ref{U0} hold true. Note that Assumptions \ref{AE} and \ref{nncst} hold true by definition of a random utility of type $(A)$. For that, we use the result of Section \ref{muti_per} for $\Omega^{t,P}$ and $U_t^P$.\\
...
we deduce from \eqref{temp_iCP} and Lemma \ref{lemme_fdmt_W} that $(U_{t+1}^P)^+(\cdot,1+\theta \Delta S_{t+1}(\cdot))\in\mathcal{M}^{t+1}(P)$. Thus, $\w^t\mapsto \mathbb{E}_{q_{t+1}^P(\cdot|\w^t)} (U_{t+1}^P)^+(\w^t,\cdot,1+\theta \Delta S_{t+1}(\w^t,\cdot))$ belongs to $\mathcal{M}^t(P)$ for all $\theta\in\{-1,1\}^d$ thanks to Lemma \ref{M_hered} (recall Assumption \ref{analytic_graph}). So, Lemma \ref{lemme_fdmt_W} shows that $l_t^P\in\mathcal{M}^t(P)$, see \eqref{l_tP}.\\
We now prove that the set $\Omega_{12}^{t,P}$ is a $P^t$-full-measure set. Indeed, by the induction hypothesis, $N_t^P\in\mathcal{M}^t(P)$, which implies that $N_t^P<+\infty\; P^t-\mbox{a.s.}$ Lemma \ref{simi_qt_na} shows that $\Omega^{t,P}_{qNA}$ is a $\mathcal{Q}^{t}$-full-measure set and also a $P^t$-full-measure set as $\mathcal{H}^T \subset \mathcal{Q}^T$. Thus, $\Omega^{t,P}_{12}$ is a $P^t$-full-measure set. %Let $\w^t\in\Omega^t_{\alpha}$ such that $N_t(\w^t)<+\infty$. Then by definition of $N_t$, $$q^{P^*}_{t+1}\left (U_{t+1}(\w^t,\cdot, -N_t(\w^t))\leq - I_t(\w^t) | \w^t \right)\geq 1-\frac{\alpha_t(\w^t)}{2}$$
%where for all $h\in\textup{Aff}(D^{t+1})(\w^t)$ with $h \neq 0$,
%$$q^{P^*}_{t+1}(h \Delta S_{t+1}(\w^t,\cdot) < - \alpha_t(\w^t)|h| | \w^t)\geq \alpha_t(\w^t).$$ 
Moreover, using the first part of Proposition \ref{U_t^-&C}, which do not require Assumption \ref{U0}, we also have that $\Omega_{i}^{t,P}$ is a $P^t$-full-measure set for all $7\leq i \leq 10$. As $(U_{t+1}^{P})^+(\cdot,1)\in \mathcal{M}^{t+1}(P)$, Assumption \ref{analytic_graph} and Lemma \ref{M_hered} show that $\w^t\mapsto \mathbb{E}_{q_{t+1}^P(\cdot|\w^t)} (U_{t+1}^P)^+(\w^t,\cdot,1)$ belongs to $\mathcal{M}^t(P)$. As a result, $\Omega^{t,P}_{11}$ and also $\widetilde{\Omega}^{t,P}$ are $P^t$-full-measure sets. Thus, we can find a $P^t$-full-measure set $\widehat{\Omega}^{t,P} \in \mathcal{B}_c(\Omega^t)$ such that 
$\widehat{\Omega}^{t,P}\subset \widetilde{\Omega}^{t,P}$.\\
We now turn to the proof of $(U_t^P)^+(\cdot,1)\in \mathcal{M}^t(P)$. We define $K_t^P(\w^t)$ as follows. If $\w^t\in\widehat{\Omega}^{t,P}$, let $K_t^P(\w^t):= K_1(1)$ where $K_1$ is defined in Proposition \ref{sub_optimal} applied in the $P$-prior $(t+1)$ context (see Definition \ref{def_one_prior}). This is possible as $\w^t\in \widetilde{\Omega}^{t,P}$ and Assumptions  \ref{P^*} to 
...
\end{eqnarray}
see \eqref{J_t_eq} for the last equality. As $J_{t}(\cdot,x)\in\mathcal{M}^t$ for all $x\in\mathbb{R}$ (see Heredity step 1), we get that $\w^t\mapsto \mathbb{E}_{q_{t+1}^P(\cdot|\w^t)} (U_{t+1}^P)^-(\w^t,\cdot,x)$ belongs to $\mathcal{M}^t$, see Lemma \ref{lemme_fdmt_W}. So, we deduce again from Lemma  \ref{lemme_fdmt_W} that $K_t^P \in \mathcal{M}^t(P)$. We now prove that $(U_t^P)^+(\cdot,1)\in \mathcal{M}^t(P)$. As already mentioned, for all $\w^t\in\widehat{\Omega}^{t,P}$,  Assumptions  \ref{P^*} to 
...
\ref{pb_inequality}  are satisfied in the robust $(t+1)$ context (see Lemma \ref{lemma_Assumption_true} and Definition \ref{def_ctxt}), %and there exists some $\widehat{\Omega}^{t,P}\in\mathcal{B}_c(\Omega^t)$ such that $\widehat{\Omega}^{t,P}\subset \widetilde{\Omega}^{t,P}$ and which is a $P^t$-full-measure set. 
...
For that, we use $\widehat{P} \in\mathcal{H}^T$ defined in \eqref{hatP_lemmaN} in the Appendix. Note that Lemma \ref{lemma_inc_M} shows that $\mathcal{M}^t(\widehat{P})\subset \mathcal{M}^t(P)$. Recall that in the robust $(t+1)$ context, $\alpha^* = \alpha_{t}^{P^*}(\w^{t})$, $n_0^* = N_{t}^*(\w^{t})$, $c^* = c_{t}^{P^*}(\w^{t})$ and $l^* = l_{t}^{*}(\w^{t})$. By assumption of Theorem \ref{optimality_M_typeA}, $1/\alpha_t^{P^*}\in\mathcal{M}^t \subset \mathcal{M}^t(P)$. Note that we also have that $1/\alpha_t^{P^*}\in\mathcal{M}^t(\widehat{P})$.  We first prove that $l_{t}^*\in \mathcal{M}^{t}(\widehat{P})\subset \mathcal{M}^{t}(P)$. Using that $U_{t}\leq U_{t}^{\widehat{P}}$ (see Proposition \ref{U_t_well}), we have that $l_{t}^*\leq l_{t}^{\widehat{P}}$. So, as $l_{t}^{\widehat{P}}\in\mathcal{M}^{t}(\widehat{P})$ (see Heredity step 2) and $l_{t}^*$ is $\mathcal{B}_c(\Omega^t)$-measurable (see Lemma \ref{lemma_mes_N}), we  get that $l_{t}^*\in \mathcal{M}^{t}(\widehat{P})$ by Lemma \ref{lemme_fdmt_W}. We now prove that $N_{t}^*\in\mathcal{M}^{t}(P)$. Recall assertions (B1) and (B2) from Lemma \ref{lemma_N_finite}. Assertion (B1) applies if $t=T-1$ and thus shows that $N_{T-1}^*\in\mathcal{M}^{T-1}(P)$ as $1/\alpha_{T-1}^{P^*} \in \mathcal{M}^{T-1}\subset \mathcal{M}^{T-1}(\widehat{P})$ and $\underline{X}$, $1/|U(\cdot,\underline{X}(\cdot)) + C(\cdot)|$, $C$, $U^-(\cdot,0)$ belong to $\mathcal{M}^T(\widehat{P})$, see initialization step for $P=\widehat{P}$. Assertion (B2) applies and shows that $N_t^* \in \mathcal{M}^t(P)$ as $\widetilde{\Omega}^{t+1,P}$ is a $P^{t+1}$-full-measure set for all $P\in\mathcal{H}^T$, $1/\alpha_{t}^{P^*}\in\mathcal{M}^{t}\subset \mathcal{M}^{t}(\widehat{P})$, $1/\alpha_{t+1}^{\widehat{P}}$, $C_{t+1}$, $J_{t+1}(\cdot,0) \in\mathcal{M}^{t+1} \subset \mathcal{M}^{t+1}(\widehat{P})$ and  $l_{t+1}^{\widehat{P}}$, $N_{t+1}^{\widehat{P}}\in \mathcal{M}^{t+1}(\widehat{P})$, see Heredity steps $1$ and $2$ for $P=\widehat{P}$. Now, as $U_{t+1}$ is lsa (see Proposition \ref{U_t_well}), Lemma \ref{lemma_mes_u} (iii) with $f=U_{t+1}^-$ alongside \citep[Lemma 7.30 (3), p177]{ref1} show that $\w^{t}\mapsto \sup_{p\in\mathcal{Q}_{t+1}(\w^{t})}\mathbb{E}_p U_{t+1}^- (\w^{t},\cdot,-x^-)$ is usa and so $\mathcal{B}_c(\Omega^{t})$-measurable. We deduce then from \eqref{J_t_eq}, $U_{t+1}^-\leq J_{t+1}$ (see Proposition \ref{U_t_well}) and the fact that $J_{t}(\cdot,-x^-)\in\mathcal{M}^{t}$ (see Heredity step 1) that $\w^{t}\mapsto \sup_{p\in\mathcal{Q}_{t+1}(\w^{t})}\mathbb{E}_p U_{t+1}^- (\w^{t},\cdot,-x^-)$ belongs to $\mathcal{M}^{t}$, see Lemma \ref{lemme_fdmt_W}. % as for all $Q\in\mathcal{Q}^t$, $1/\alpha_t^{P^*} \in\mathcal{W}^t$, $N_t\in\mathcal{M}_t(Q)$, $l_t\in\mathcal{M}_t(Q)$, $c_t^*\in\mathcal{W}^t$ and $\w^t\to \sup_{p\in\mathcal{Q}_{t+1}(\w^t) }\mathbb{E}_p U_{t+1}^- (\w^t,\cdot,-x^-) \; \in \mathcal{W}^t$, 
Finally, as $C_{t+1}$ and $J_{t+1}(\cdot,0)$ belong to $\mathcal{M}^{t+1}$, $c_{t}^{P^*}\in\mathcal{M}^t$ using \eqref{C_t_eq} and Lemma \ref{M_hered}. Thus, recalling again that $1/\alpha_{t}^{P^*} \in \mathcal{M}^{t}$, we deduce that $K_{t}\in\mathcal{M}^{t}(P)$. \Halmos
...
$U(\lambda x)\leq \lambda (U(x)+U^-(0)) \leq \lambda^{\gamma}(U(x)+C).$\\ Assume now that $\underline{x}<x\leq 0$. We have that $$\lambda^{\gamma}(U(x)+C)\geq \lambda^\gamma (U(\underline{x})+C)=\lambda^\gamma (U^+(\underline{x})+U^+(0)+U^-(0))\geq U^+(0)\geq U(0)\geq U(\lambda x),$$ as $x\leq 0$. Recalling \eqref{temp_AE_ASSUMP_gamma}, \eqref{det_AE} is proved.\\ 
...
The following proposition is important to prove that Assumption \ref{U0} is conserved by dynamic programming. Let $p,q\in\mathfrak{P}(X),$ we write that $p\ll q$ if for all $A\in \mathcal{B}(X)$, $q(A)=0$ implies that $p(A)=0$.
...
\subsection{The sets $\mathcal{M}^t(P)$, $\mathcal{M}^t$ and $\mathcal{W}^t$}
...
The following lemma shows that $\Omega^t_{11}$ is a $\mathcal{Q}^t$-full-measure set and $\Omega^{t,P}_{11}$ is a $P^t$-full-measure set. %The result of Proposition \ref{U_t^-} will come in handy to ensure that 

\begin{lemma}
Assume that Assumptions \ref{S_borel}, \ref{analytic_graph}, \ref{AE} and \ref{simple_adm} hold true. Let $P:=q^P_1\otimes \cdot \cdot \cdot \otimes q^P_{T}\in\mathcal{Q}^T$ and $0\leq t\leq T-1$. Then, $\Omega^{t}_{11}$ and $\Omega_{11}^{t,P}$ belong to $\mathcal{B}_c(\Omega^t)$. Assume furthermore that $U_0^P(1)<+\infty,$ then $P^t(\Omega_{11}^{t,P})=1$. Moreover, if Assumption \ref{U0} holds true, $\Omega^{t}_{11}$ is a $\mathcal{Q}^t$-full-measure set.
...
\end{lemma}
\proof{Proof.}
Definition \ref{def_ctxt} implies that 
\begin{eqnarray*}
\Omega^{t,P}_{11} &=& \left\{\w^t\in\Omega^t,\; \mathbb{E}_{q^{P}_{t+1}(\cdot|\w^t)} (U_{t+1}^P)^+(\w^t,\cdot,1)<+\infty \right\} \quad 
\Omega^t_{11} =\left \{\w^t\in\Omega^t,\; \mathbb{E}_{q^{P^*}_{t+1}(\cdot|\w^t)} U_{t+1}^+(\w^t,\cdot,1)<+\infty \right\}.
\end{eqnarray*}
Recalling that $U_{t+1}^P$ is $\mathcal{B}_c(\Omega^{t+1}\times \mathbb{R})$-measurable (see Proposition \ref{U_t_well} (ii)), \citep[Proposition 7.44, p172]{ref1} shows that $(\w^t,\w_{t+1})\mapsto (U_{t+1}^P)^+(\w^t,\w_{t+1},1)$ is $\mathcal{B}_c(\Omega^t\times \Omega_{t+1})$-measurable. So, Proposition \ref{univ_cvt} (iii) shows that $\w^t\mapsto \mathbb{E}_{q_{t+1}^P(\cdot|\w^t)} (U_{t+1}^P)^+(\w^t,\cdot,1)$ is $\mathcal{B}_c(\Omega^t)$-measurable, which implies that $\Omega^{t,P}_{11}\in\mathcal{B}_c(\Omega^t)$. Similarly, using Proposition \ref{U_t_well} (i), we get that $\Omega^{t}_{11}\in\mathcal{B}_c(\Omega^t)$.\\

\noindent\textit{If $U_0^P(1)<+\infty$, then $P^t(\Omega^{t,P}_{11})=1$.}\\
\noindent Assume now that $U_0^P(1)<+\infty$. We proceed by contraposition. Assume that there exists some $0\leq t\leq T-1$ such that $P^t(\Omega^{t,P}_{11})<1$. Then, for all $\w^t\notin \Omega^{t,P}_{11}$, 
\begin{eqnarray}
...
\end{eqnarray}
Let $\widetilde{\Omega}^t_+ := (\Omega^{t,P}_{11})^c \cap \Omega^t_{J,1,0}$ (see \eqref{set_J_t} for the definition of $\Omega^t_{J,1,0}$). Then, Proposition \ref{CJ_t} shows that $\widetilde{\Omega}^t_+ \in \mathcal{B}_c(\Omega^t)$ and $P^t(\widetilde{\Omega}^t_+)=P^t((\Omega^{t,P}_{11})^c)>0$ as $P^t(\Omega^t_{J,1,0})=1$. Let $\w^t\in \widetilde{\Omega}^t_+$. Using that $U_{t+1} \leq U_{t+1}^{P}$ and $U_{t+1}^- \leq J_{t+1}$ (see Proposition \ref{U_t_well}), we get tha
...
\noindent\textit{If Assumption \ref{U0} holds true, $\Omega^t_{11}$ is a $\mathcal{Q}^t$-full-measure set.}\\
Let $\widetilde{P} := q_{1}^{\widetilde{P}} \otimes \cdots \otimes q_{T}^{\widetilde{P}} \in\mathcal{Q}^T$ and $\widehat{P} := \frac{q_{1}^{P^*}+q_1^{\widetilde{P}}}{2}\otimes \cdot\cdot\cdot \otimes\frac{q_T^{P^*}+q_T^{\widetilde{P}}}{2}.$ As $\widehat{P}\in \mathcal{H}^T$ (see \eqref{setPT}), we have that $U_0^{\widehat{P}}(1)<+\infty$ by Assumption \ref{U0}. So, the preceding step shows that $\Omega^{t,\widehat{P}}_{11}$ is a $\widehat{P}^t$-full-measure set. Using now (v) in Proposition \ref{U_t_well} at $t+1$, $U_{t+1}\leq U_{t+1}^{\widehat{P}}$, $\Omega^{t,\widehat{P}}_{11}\subset \Omega^{t}_{11}$ and 
...
%\label{set_temp_U_t^+}
%\end{eqnarray}
$\Omega_{11}^t$ is also a $\widehat{P}^t$-full-measure set. So, $\Omega^t_{11}$ is a ${\widetilde{P}}^t$-full-measure set using Proposition \ref{domin_lcl} and $\widetilde{P} \ll  \widehat{P}$. As ${\widetilde{P}}$ is arbitrary and $\Omega_{11}^t\in\mathcal{B}_c(\Omega^t)$, $\Omega_{11}^t$ is a $\mathcal{Q}^t$-full-measure set. \Halmos \endproof
...
%\end{lemma}
\begin{lemma}
Assume that the $NA(\mathcal{Q}^T)$ condition as well as Assumptions  \ref{S_borel}, \ref{analytic_graph}, \ref{AE}, \ref{nncst} and \ref{simple_adm} hold true. For all $P:= q_1^P \otimes \cdots \otimes q_T^P\in\mathcal{Q}^T$, let 
\begin{eqnarray}
\widehat{P}:= \frac{q_{1}^{P^*}+q_1^{P}}{2} \otimes \cdots \otimes q_{t}^{P^*} \otimes \cdots \otimes \frac{q_T^{P^*}+q_T^{P}}{2}. \label{hatP_lemmaN}
\end{eqnarray}
Then, $\widehat{P} \in\mathcal{H}^T$ and we have that
\begin{eqnarray}
...
\noindent (B1) : If $1/\alpha_{T-1}^{P^*}\in\mathcal{M}^{T-1}(\widehat{P})$ and $\underline{X}$, $1/|U(\cdot,\underline{X}(\cdot))+C(\cdot)|$, $C$, $U^-(\cdot,0) \in \mathcal{M}^T(\widehat{P})$, then $N^*_{T-1} \in \mathcal{M}^{T-1}(P)$.\\
...
\noindent (B2) : If $1/\alpha_{t-1}^{P^*}\in\mathcal{M}^{t-1}(\widehat{P})$ and $1/\alpha_t^{\widehat{P}}$, $N_{t}^{\widehat{P}}$, $l_t^{\widehat{P}}$, $C_t$, $J_t(\cdot,0)\in\mathcal{M}^t(\widehat{P})$, then $N^*_{t-1}\in\mathcal{M}^{t-1}(P)$.
...
%\label{eq_NtPAB}
\end{eqnarray*}
Recalling \eqref{eq_NtP},
\begin{eqnarray}
N_{t}^P(\w^t) = N_{t}^{P,P}(\w^t). \label{templemmeN}
\end{eqnarray}
...
(C1) :  If $1/\alpha_{t-1}^R\in \mathcal{M}^{t-1}(P)$ and $1/\alpha_{t}^P$, $N_t^P$, $l_t^P$, $C_t$, $J_t(\cdot,0)\in \mathcal{M}^{t}(P)$, then $N_{t-1}^{P,R}\in\mathcal{M}^{t-1}(P)$.\\
%\begin{eqnarray}
...
As $\widetilde{\Omega}^{t,P}$ is a $P^t$-full-measure set, there exists $\widehat{\Omega}^{t,P}\in\mathcal{B}_c(\Omega^t)$ such that $\widehat{\Omega}^{t,P}\subset \widetilde{\Omega}^{t,P}$ and  $P^t(\widehat{\Omega}^{t,P})=1$. Define $$\overline{\Omega}^{t,P} := \widehat{\Omega}^{t,P}\cap \{\w^{t}\in\Omega^{t},\; C_{t-1}(\w^{t-1})<+\infty,\quad J_{t-1}(\w^{t-1},0)<+\infty\}.$$
...
\ref{pb_inequality}  hold true in the $P$-prior $(t+1)$-context (see Definition \ref{def_ctxt}) and \eqref{v_v_q} in Proposition \ref{existence_uni} applies : 
\begin{eqnarray}
...
We get that $\overline{N}_t^{P}(\w^t)<+\infty$. \\ We now provide conditions for $\overline{N}_t^{P} \in \mathcal{M}^t(P)$. Lemma \ref{lemma_mes_N} shows that $\widehat{N}_t^P$ and thus $\overline{N}_t^P$ are $\mathcal{B}_c(\Omega^t)$-measurable. Moreover, if $1/\alpha_{t-1}^R$ and $c_{t-1}^R$ belong to $\mathcal{M}^{t-1}(P)$ and $1/\alpha_t^P$, $N_t^P$, $l_t^P$, $c_t^P$ to $\mathcal{M}^t(P)$, then $\overline{N}_t^P$ also belongs to $\mathcal{M}^t(P)$, see Lemma \ref{lemme_fdmt_W}. Note first if $C_t$ and $J_t(\cdot,0)$ belong to $\mathcal{M}^t(P)$, then $c_t^P\in\mathcal{M}^t(P)$ and also $c_{t-1}^R$ and $i_{t-1}^R$ belong to $\mathcal{M}^{t-1}(P)$. Indeed, Lemma \ref{lemma_mes_N} shows that $c_t^P$ is $\mathcal{B}_c(\Omega^t)$-measurable. So, \eqref{ineq_ctp2} and Lemma \ref{lemme_fdmt_W} imply that $c_t^P\in\mathcal{M}^t(P)$. In addition, \eqref{ct^P} at $t-1$, Lemma \ref{M_hered} and $1/\alpha_{t-1}^R\in\mathcal{M}^{t-1}(P)$ imply that $c_{t-1}^R$ and $i_{t-1}^R$ belong to $\mathcal{M}^{t-1}(P)$.\\
...
\begin{eqnarray*}
q_t^P(\overline{N}_t^P(\w^{t-1},\cdot)\leq \widehat{N}_{t-1}^P(\w^{t-1}) | \w^{t-1}) &=& 1-q_t^P(\overline{N}_t^P(\w^{t-1},\cdot)> \widehat{N}_{t-1}^P(\w^{t-1}) | \w^{t-1})\\
...
So, by definition of $\widetilde{N}_{t-1}^P$, we have that $\widetilde{N}_{t-1}^P(\w^{t-1})\leq \widehat{N}_{t-1}^P(\w^{t-1})$ and also using \eqref{NtleqNprime0} that
\begin{eqnarray}
...
\end{eqnarray*}
As a result, if $1/\alpha_{t-1}^R\in\mathcal{M}^{t-1}(P)$ and $\overline{N}_t^P\in\mathcal{M}^t(P)$, then $N_{t-1}^{P,R}\in\mathcal{M}^{t-1}(P)$. Recalling the conditions for $\overline{N}_t^P\in\mathcal{M}^t(P)$, this shows assertion (C1).\\%(indeed, $\mathbb{E}_{P^t}\left(\left(\frac{1}{\alpha_{t-1}^R}\right)^{2r}\right) = \mathbb{E}_{P^{t-1}}\left(\left(\frac{1}{\alpha_{t-1}^R}\right)^{2r}\right)$
...
(C2) : If $1/\alpha_{T-1}^R\in\mathcal{M}^{T-1}(P)$ and $\underline{X}$, $1/|U(\cdot,\underline{X}(\cdot))+C(\cdot)|$, $C(\cdot)$, $U^-(\cdot,0) \in \mathcal{M}^{T}(P)$, then $N_{T-1}^{P,R}\in\mathcal{M}^{T-1}(P)$.
\noindent Let
\begin{eqnarray*}
...
\end{eqnarray*}
and $\widetilde{N}_T^{P}(\w^T):=1$ otherwise. For all $\w^T\in\Omega^T$, we set $\overline{N}_T^{P}(\w^T):= \lceil \widetilde{N}_T^{P}(\w^T) \rceil$. We see easily that $\overline{N}_T^{P}$ is $\mathcal{B}_c(\Omega^T)$-measurable (recall that $i_{T-1}^R$ is $\mathcal{B}_c(\Omega^{T-1})$-measurable from Lemma \ref{lemma_mes_N}).\\ We now give conditions for $\overline{N}_{T}^P \in\mathcal{M}^{T}(P)$. For that, it is enough that $c_{T-1}^R$, $1/\alpha_{T-1}^R \in \mathcal{M}^{T-1}(P)$ and $\underline{X}$, $1/|U(\cdot,\underline{X}(\cdot)) + C(\cdot)| \in\mathcal{M}^T(P)$. Remark that if $C$ and $U^-(\cdot,0)$ belong to $\mathcal{M}^T(P)$ then $C_{T-1}^R\in\mathcal{M}^{T-1}(P)$, see Lemma \ref{M_hered} and \eqref{ct^P}.\\ Let $\w^T\in\overline{\Omega}^{T,P}$. Note that $c_{T-1}^R(\w^{T-1})<+\infty$ using \eqref{ineq_ctp2} at $T-1$. Moreover, $\alpha_{T-1}^R(\w^{T-1})>0$ and $U(\w^T,\underline{X}(\w^T))+C(\w^T) < 0$. So, $i_{T-1}^R(\w^{T-1})<+\infty$ and also $\overline{N}_T^{P}(\w^{T})<+\infty$. As $U(\w^T,\cdot)$ is nondecreasing and using \eqref{elas_gammaf} with $x=-|\underline{X}(\w^T)|=\underline{X}(\w^T)$ and $\lambda = \frac{\widetilde{N}_T^{P}(\w^T)}{|\underline{X}(\w^T)|} \geq 1$, we find that
\begin{eqnarray*}
...
So, \eqref{NtoPfiniteT} for $N_{T-1}^P$, \eqref{NtoPfinite} for $N_{t-1}^P$ and Assertions (A1) and (A2) follows from \eqref{templemmeN} and Steps 1 and 2 (recall \eqref{NtleqNprime0}).\\
...
%\end{eqnarray}
\noindent\textit{Step 3 : \eqref{Nt_finiteT} for $N_{T-1}^*$, \eqref{Nt_finite} for $N_{t-1}^*$ and Assertions (B1) and (B2) hold true for all $1\leq t\leq T$ and $P\in\mathcal{H}^T$.}\\
Let $P:=q_1^P\otimes \cdots \otimes q_T^P \in\mathcal{H}^T$. Recall $P^*$ from \eqref{P^*_exp} and $\widehat{P}\in\mathcal{H}^T$ from \eqref{hatP_lemmaN} and \eqref{setPT}. %let $\widehat{P}:= \frac{q_{1}^{P^*}+q_1^{P}}{2} \otimes \cdots \otimes q_{t}^{P^*} \otimes \cdots \otimes \frac{q_T^{P^*}+q_T^{P}}{2}.$ 
%Note that $\widehat{P}\in\mathcal{H}^T$, see \eqref{setPT}. 
Let $1\leq t\leq T$. Steps 1 and 2 show that for all $\w^{t-1}$ in a $\widehat{P}^{t-1}$-full-measure set, $N_{t-1}^{\widehat{P},P^*}(\w^{t-1})<+\infty$ and thus 
\begin{eqnarray*}
q_{t}^{\widehat{P}}\left(U_{t}^{\widehat{P}}(\w^{t-1},\cdot,-N_{t-1}^{\widehat{P},P^*}(\w^{t-1}))\leq - i_{t-1}^{P^*}(\w^{t-1}) | \w^{t-1} \right)\geq 1-\frac{\alpha_{t-1}^{P^*}(\w^{t-1})}{2}.
\end{eqnarray*}
As $U_{t}\leq U_{t}^{\widehat{P}}$ (see Proposition \ref{U_t_well}) and $q_t^{\widehat{P}}=q_t^{P^*}$, we get that
\begin{eqnarray*}
q_{t}^{P^*}\left(U_{t}(\w^{t-1},\cdot,-N_{t-1}^{\widehat{P},P^*}(\w^{t-1}))\leq - i_{t-1}^{P^*}(\w^{t-1}) | \w^{t-1} \right)\geq 1-\frac{\alpha_{t-1}^{P^*}(\w^{t-1})}{2}.
\end{eqnarray*}
So, recalling the definition of $N^*_{t-1}$ in \eqref{eq_Nt}, we deduce that %\footnote{Recall that $\widehat{P}^{0}=\delta_\{\w_0\}$ (see Section \ref{sett}).} 
$N^*_{t-1} \leq N_{t-1}^{\widehat{P},P^*}<+\infty\;\; \widehat{P}^{t-1}-\mbox{a.s.}$\\ 
%\begin{eqnarray*}
%N^*_{t-1} \leq N_{t-1}^{\widehat{P},P^*}<+\infty\;\; \widehat{P}^{t-1}-\mbox{a.s.}
%\end{eqnarray*}
If $t=1$, then, $P^{0} = \delta_{\w_0} \ll \delta_{\w_0} =\widehat{P}^{0}.$ If $t>1$, Proposition \ref{domin_lcl} for $l=t-1$ shows that $P^{t-1}\ll \widehat{P}^{t-1}$. So, $N^*_{t-1} <+\infty\;\; P^{t-1}-\mbox{a.s.}$ %As $N^*_{t-1}$ is $\mathcal{B}_c(\Omega^{t-1})$-measurable (see Lemma \ref{lemma_mes_N}), the set $\{\w^t\in\Omega^{t-1}, \; N^*_{t-1}(\w^{t-1})<+\infty\}$ belongs to $\mathcal{B}_c(\Omega^{t-1})$. Recalling that this set is a $P^{t-1}$-full measure set with $P$ arbitrary, we deduce that it is a $\mathcal{Q}^{t-1}$-full-measure set. 
As $P$ is arbitrary, \eqref{NtoPfiniteT} and \eqref{Nt_finite} hold true. \\If $t=1$, we have that $\mathcal{M}^{0}(\widehat{P}) = \mathcal{M}^{0}(P) = \mathbb{R}$. Now, if $t>1$, Lemma \ref{lemma_inc_M}, again with $l=t-1$, shows that $\mathcal{M}^{t-1}(\widehat{P}) \subset \mathcal{M}^{t-1}(P)$. Thus, Assertions (C1) and (C2) imply that Assertions (B1) and (B2) hold true. %\label{temp_X_Phat}
\Halmos \endproof

\subsection{Utility function with random benchmark}
Finally, we show that it is indeed possible to apply Theorem \ref{optimality_M_typeA} to a random utility with benchmark.
\begin{proposition}
...
\geq -\widetilde{b}2^{p-1}\left(1+|Z(\w^T)|^{p}+|A(\w^T)|^{p}\right)\nonumber\\
&\geq &-C_1(\w^T)\left(1+|A(\w^T)|^{p}\right),\label{temp_rand_ben_ineq1}
\end{eqnarray}
where $C_1(\cdot):= \widetilde{b} 2^{p-1}(1+|Z(\w^T)|^{p})$. We obtain that $C_1\in\mathcal{W}^T$ as $A, Z\in\mathcal{W}^T$. So, for all $x\in\mathbb{R}$, \eqref{ineq_b_inf_det_A} holds true choosing $A(\cdot)=x$. Now, as $A$ is $\mathcal{B}_c(\Omega^T)$-measurable, Remark \ref{joint_mes_U} shows that $U^-(\cdot,A(\cdot))$ is $\mathcal{B}_c(\Omega^T)$-measurable and thus belongs to $\mathcal{W}^T$, see Lemma \ref{lemme_fdmt_W} and \eqref{temp_rand_ben_ineq1}.\\
...
We show first that for all $\w^T\in\Omega^T$, $AE_{\pm\infty}(U(\w^T,\cdot))=AE_{\pm\infty}(\widetilde{U})$. Fix $\w^T\in\Omega^T$. Then,
\begin{eqnarray*}
...
\end{eqnarray}
Now, as $x \leq \underline{X}(\w^T)\leq -\frac{\eta}{1-\eta}Z(\w^T)$ and $\eta<1$, we get that $(1-\eta)x \leq -\eta Z(\w^T)$. As $x-Z(\w^T) \leq \underline{x} <0$, we have that $\frac{x}{x-Z(\w^T)}\geq \eta.$ So, using \eqref{temp_tilde_-inf}, we deduce that 
\begin{eqnarray}
...
\end{eqnarray}
As $AE_{-\infty}(U(\w^T,\cdot))=AE_{-\infty}(\widetilde{U})>1$, $\gamma\in (1, AE_{-\infty}(U(\w^T,\cdot))$, $\underline{X}(\w^T)<0$, $U(\w^T,\underline{X}(\w^T))<0$ and \eqref{temp_ineq_AE11} holds true for all $x\leq \underline{X}(\w^T)$, Proposition \ref{annex_AE2} for $U(\w^T,\cdot)$ applies and \eqref{det_AE} holds true for $U(\w^T,\cdot)$ with $\gamma$ and $C(\w^T) := U^+(\w^T,0) + U^-(\w^T,0)+ U^-(\w^T,\underline{X}(\w^T))$. As $Z\in\mathcal{W}^T$, $\underline{X}\in\mathcal{W}^T$ (see Lemma \ref{lemme_fdmt_W}). Now, Step 1 shows that $U^-(\cdot,\underline{X}(\cdot)), U^-(\cdot,0)\in\mathcal{W}^T$ and Step 2 that $U^+(\cdot,0)\in \mathcal{W}^T$. So, we conclude that $C\in\mathcal{W}^T$ and that Assumption \ref{AE} holds true with $C\in\mathcal{W}^T$.\\
...
\newblock {\em The Annals of Applied Probability,}{29\bf(1)} 577-612.
...
\newblock {Phd Thesis, \em Universit\\rq{}e de Reims Champagne-Ardenne}.
...
\newblock {\em The Annals of Applied Probability} {28\bf(3)}, 1856-1892.
...
\newblock {\em The Annals of Applied Probability,}{25} 823-859.
...
\newblock {\em The Annals of Applied Probability} {15(\bf 2)}, 1367-1395.
