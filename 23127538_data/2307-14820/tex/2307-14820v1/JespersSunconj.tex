...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%functions maybe used in other articles %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%\newcommand\Coim{{\rm Coim}}
%\newcommand\Ext{{\rm Ext}}
%\newcommand\Tor{{\rm Tor}}
%\newcommand\Transpose{\rm Tr}
%\newcommand\Tr{\rm Tr}
%\newcommand\Rad{\rm Rad}
%\newcommand\Glid{\operatorname{Glid}}
%\newcommand\glid{\operatorname{glid}}
%\newcommand\Preglid{\operatorname{Preglid}}
...
%\newcommand\Mapleft[1]{\smash{\mathop{\Longleftarrow}\limits^{#1}}}
%\newcommand\hookmapright[1]{\smash{\mathop{\hookrightarrow}\limits^{#1}}}
%\newcommand{\veq}{\mathrel{\rotatebox{90}{$=$}}}
%\newcommand\checkmarkz{\tikz\fill[scale=0.4](0,.35) -- (.25,0) -- (1,.7) -- (.25,.15) -- cycle;}
...
Already in the 1860's Weierstrass and Jordan introduced what students today learn to call the Jordan normal form of a matrix, cf. \cite{Hawkins} for a historic overview. Reformulating this theory in a more general context for $A$ an algebra over a field $F$ any invertible element $a \in A$ can be uniquely decomposed as $a = a_u a_s$ such that $a_u$ is unipotent, $a_s$ is semisimple and $a_u a_s = a_sa_u$. When $A$ contains a substructure $B$ of interest, e.g. when $F$ is a number field and $B$ an order in $A$, one could ask, if for every invertible $b \in B$ one can still achieve this Jordan decomposition in $B$, i.e. whether $u_n,u_s \in B$ holds. Motivated by the study of units in integral group rings Hales, Luthar and Passi asked when the above will happen for $A= \mathbb{Q}G$ the rational group algebra of a finite group $G$ and $B = \mathbb{Z}G$ the integral group ring therein. Namely they defined a finite group $G$ to have \emph{Multiplicative Jordan Decomposition}, if for any unit $u \in \mathbb{Z}G$ the elements $u_n$ and $u_s$, which a priori are defined in $\mathbb{Q}G$, actually live in $\mathbb{Z}G$, and asked which groups satisfy this property. Though quiet some research has been developed to this, the problem remains open in general, see \Cref{sec:MJD} for more details and references. 

A major breakthrough in this investigation came when it was observed in \cite{HPW} that a group which has Multiplicative Jordan Decomposition also has the \emph{Nilpotent Decomposition} (\emph{ND} for short). Namely, $G$ is said to have ND, if for any nilpotent element $n \in \mathbb{Z}G$ and any central idempotent $e \in \mathbb{Q}G$, the product $ne$ still lies in $\mathbb{Z}G$. Looking on the Wedderburn-Artin decomposition 
\begin{equation}\label{eq:QGWedderburn}
\mathbb{Q}G = M_{n_1}(D_1) \oplus ... \oplus M_{n_\ell}(D_n),
\end{equation}
 where $M_{n_i}(D_i)$ denotes the $n_i\times n_i$-matrix ring over a division algebra $D_i$, one sees that this property will hold, if at most one of the $n_i$ is bigger than $1$: indeed, then either $ne = n$ or $ne = 0$ holds. This observation during the search for groups having ND led Jespers and Sun to define a group  $G$ as having \emph{at most one matrix component}, if at most one of the $n_i$ in \eqref{eq:QGWedderburn} is bigger than $1$ \cite{JespersSun}. 
 %Recall that $n$ is called the reduced degree of the Wedderburn component $M_n(D)$. 
...
\Cref{Summary theorem for SN} is proven in \Cref{sec:SN} by dividing it into three separate subcases: nilpotent groups (which are handled in  \Cref{prop:NilpotentSNNotSSN}), solvable non-nilpotent groups (\Cref{prop:SolvSNGroups}) and non-solvable groups (\Cref{prop:NonSolvableSNGroups}). The classification of nilpotent groups with SN turns out to be the hardest of those.  With those preparations we then prove \Cref{Summary theorem ND} and \Cref{cor:QGKnowsND} in \Cref{Section ND}, where the former is obtained by again dividing into three similar cases. Also here  the nilpotent groups again require most of our attention.

After the proofs of these results we devote the last two sections to other group, ring and representation theoretic properties which are connected with the ND property. 

First, in \Cref{sec:GeneralOrders} we investigate the group $q(G)$ which measures how far a given group $G$ is from having ND. This group was also introduced by Jespers-Sun in the last section of \cite{JespersSun} and is obtained by factoring out the unipotent units of $\Z G$ which project trivially to all but one simple component of $\Q G$ from the group generated by all the unipotent units of $\Z G$. Clearly $q(G)$ is trivial if and only if $G$ has ND, so that it can be regarded as an obstruction for the property ND. Jespers and Sun posed two problems about $q(G)$: to classify the groups $G$ for which it is finite and to find a connection between the structure of it and the simple components of $\Q G$. In \Cref{measure subsection} we give answers to both of their questions by showing first that in the absence of so-called exceptional simple components of $\Q G$ the group $q(G)$ is finite, while in the presence of a simple component $M_2(\Q)$ and under an additional group-theoretical condition it is an infinite non-torsion group, cf. \Cref{when q(G) finite for grp rings} for the details. Interestingly the group $C_4 \rtimes C_8$ in \Cref{Summary theorem ND} is the smallest group not  satisfying that additional group-theoretical condition. Our arguments heavily rely on solutions of the Congruence Subgroup Problem. In \Cref{the obstruction for general smeisimple} we also point out that studying the obstruction group $q(G)$ can be interesting in the context of arithmetic subgroups of general semisimple algebraic groups and ask for a topological interpretation. 
...
\vspace*{.5cm}
\noindent \underline{\it Conventions and Notations.} $G$ will always denote a finite group. If $\Q G \cong \prod_{i} \Ma_{n_i}(D_i)$ is the Wedderburn-Artin decomposition of the semisimple algebra $\Q G$, then we call the factors $\Ma_{n_i}(D_i)$ simple components of $\Q G$. Recall that $n$ is called the \emph{reduced degree} of the simple component $M_n(D)$. If a component has reduced degree $2$ or more we will speak of a {\it matrix component}. The set $\PCI(G)$ denotes the primitive central idempotents of $\Q G$. 

Moreover we use standard group-theoretical notation: for a group $G$ we denote by $G'$ the derived subgroup of $G$, by $\ZZ(G)$ the center of $G$, by $\Phi(G)$ the Frattini subgroup of $G$, by $g^G$ the conjugacy class of an element $g\in G$ in $G$ and by $\Soc(G)$ the socle of $G$. Moreover for $g,h \in G$, we set $[g,h] = g^{-1}h^{-1}gh$. A cyclic group of order $n$ is denoted $C_n$, a dihedral group of order $2n$ by $D_{2n}$, an alternating group of degree $n$ by $A_n$ and $Q_{2^n}$ denotes the generalized quaternion group of order $2^n$, i.e. 
\[Q_{2^n} = \langle a, b \ | \ a^{2^{n-1}} = b^4 = 1, \ b^2 = a^{2^{n-1}} \ a^b = a^{-1} \rangle \]
...
We thank B. Sury and Amir Behar for useful conversations concerning \Cref{unipotent in congruence of SL2Z}.

\section{Description of groups with SN}\label{sec:SN}
Recall that a group $G$ has the {\it SN property} if for any normal subgroup $N$ of $G$ and any subgroup $Y \leq G$ either $N \subseteq Y$ or $N Y \unlhd G$. 
The main goal of this section is to prove \Cref{Summary theorem for SN}. We separate this in essentially three steps: the nilpotent groups with SN (which are handled in  \Cref{prop:NilpotentSNNotSSN}), the solvable non-nilpotent groups with SN (\Cref{prop:SolvSNGroups}) and the non-solvable groups with SN (\Cref{prop:NonSolvableSNGroups}).  The combination of this cases then gives exactly \Cref{Summary theorem for SN}. 
...
\subsection{Background results on groups with SN}
...
$G$ is a Dedekind group if and only if it is abelian or $G \cong Q_8 \times C_2^n \times A$ for some $n \in \mathbb{N}_0$ and $A$ and abelian group of odd order.
\end{theorem}
...
&= \frac{k}{|G|}\sum_{g \in G} \left(\sum_{h \in G} \alpha(gh^{-1}) \chi(h^{-1})\right)g = \frac{k}{|G|}\sum_{g \in G} \left(\sum_{h \in G} \alpha(h) \chi(h^{-1}g)\right)g
\end{align*}
...
\end{lemma}
\begin{proof}
Assume $P$ and $Q$ are a non-cyclic $p$-Sylow and a $q$-Sylow subgroup of $G$ respectively. Then $P \unlhd G$ and so by \Cref{lem:LiuPassma2.1} we know that $Q$ is a Dedekind group. But as this argument applies to any prime, this means that $G$ is Dedekind, contradicting the assumption.
\end{proof}
...
Let $g,h \in G$ such that $[g,h] \neq 1$. Assume first that $z \notin \langle g \rangle$. Note that we can assume this without changing the value of $[g,h]$ when $p$ is odd. As $G$ is a group with SN, this implies $\langle g, z \rangle = \langle g \rangle \times \langle z \rangle \unlhd G$. As $z$ is central and $G$ is a $p$-group we hence get $[G,g] \in \langle g^p \rangle \times \langle z \rangle$. If $g^p \neq 1$, then we have $[G, g^p] \in \langle g^{p^2} \rangle$, $[G, g^{p^2}] \in \langle g^{p^3} \rangle$, ...,$[G, g^{\circ(g)/p}] = 1$, implying $g^{\circ(g)/p} \in \ZZ(G)$ which contradicts Claim 1. Hence $g^p = 1$ and $[G,g] \in \langle z \rangle$, in particular $[g,h] \in \langle z \rangle$. Now suppose that $z$ lies in every non-trivial subgroup of $\langle g, h \rangle$, i.e. $\langle g,h \rangle$ is a generalized quaternion group. If $\langle g,h \rangle$ has order $8$, then $[g,h] = z$ and there is nothing more to prove. So assume $\langle g,h \rangle \cong Q_{2^m}$ for some $m \geq 4$. Say $g^{2^{m-1}} = h^4 = 1$ and $g^h = g^{-1}$. Note that $h^g = h^{-1}g^2$. As $Q$ is elementary abelian and maximal non-normal, we get $\langle Q, h \rangle \unlhd G$. As also $Q  \times\langle z \rangle \unlhd G$, we have that $[h,q]$ has order at most $2$ and so $hq$ has order at most $4$ for any $q \in Q$. Note for this that $h^2 = z$ is central in $G$. But as $h^g = h^{-1}g^2 \in \langle Q, h\rangle$, we have $g^2 \in \langle Q, h \rangle$, a contradiction, since $g^2$ has order at least $8$.

In particular Claim 2 implies that $G$ is metabelian. Moreover, there exists a cyclic subgroup $C$ of $G$ containing $z$ such that $A = C \times Q$ is a maximal abelian subgroup of $G$. We are now finally ready to prove that $G$ has at most one matrix component by applying \Cref{lem:IdempotForMetabelian}. So assume $\mathbb{Q}e(G,H,K)$ is a non-commutative component of $\mathbb{Q}G$. As $K$ lies in the kernel of a representation corresponding to this component, we have $G' \not\leq K$, i.e. $z \notin K$ by Claim 2. Hence, again by Claim 2, the unique maximal element of $\{B \leq G \ | \ A\leq B \ \ \text{and} \ \ B' \leq K \leq B \}$ is $A$ and $e(G,H,K) = e(G,A,K)$ with $A/K$ a cyclic group. By \Cref{lem:IdempotForMetabelian} it is thus sufficient to prove that all subgroups of $A$ which do not contain $z$ and have cyclic quotients are conjugate. We call such subgroups ``good''. Note that good subgroups are elementary abelian, so contained in $\langle z \rangle \times Q$. A good subgroup $U$ is determined by $(\langle z \rangle \times Q)/U$ and these quotients are exactly the images of the groups $\langle z q\rangle$, where $q$ runs through the elements of $Q$. Hence there are $|Q|$ good subgroups. It is clear that $Q$ itself is a good subgroup. So we need to prove that $Q$ has $|Q|$ conjugates in $G$, i.e. $[G:N_G(Q)] = |Q|$. By the maximality of $Q$, and since $z \in N_G(Q)$, we have $N_G(Q) \unlhd G$. As $G'$ is a central group of order $p$, the group $G/N_G(Q)$ is elementary abelian. So we can view $V = Q \times G/N_G(Q)$ as an $\mathbb{F}_p$-vector space of dimension $|Q| + |G/N_G(Q)|$. We define a non-degenerate symplectic bilinear form
...
Let $G$ be a $2$-group which has SN, but not SSN. Assume that every involution of $G$ is central. Then $G \cong Q_8 \times Q_8$.
\end{lemma}
\begin{proof}
We first note that $G$ is not a generalized quaternion group. Indeed $Q_8$ and $Q_{16}$ have SSN \cite[Theorem 2.3, BJ6]{Liu} and if $G = \langle g, h \ | \ g^{2^n} = h^4 = 1, \ g^{2^{n-1}} = h^2, \ g^h = g^{-1} \rangle$ for some $n \geq 4$, then $G$ does not have SN. This can be observed by taking $N = \langle g^4 \rangle$, $Y = \langle h \rangle$, so that $N \not\subseteq Y$ and $NY \ntrianglelefteq G$ as $h^g = hg^2 \notin NY$. In particular, the center of $G$ is not cyclic.
...
\item[(viii)] If $g \notin N_G(\langle a \rangle)$ and $g \notin N_G(\langle b \rangle)$, then $g \in N_G(\langle ab \rangle)$. The same holds for any permutation of $a$, $b$ and $ab$:\\
...
\item[(xvii)] $\circ(x) = 4$:\\
Assume $\circ(x) = 2^n > 4$. Then by (v) and (xiv) we have $x^4 \in \langle \tilde{z} \rangle$ and so $z \in \langle x^4 \rangle$. As $[x^2,y] = [x,y^2] \in \langle y^4 \rangle = 1$ by (xii) we then get $(x^{2^{n-2}}y)^2 = zz=1$ and $x^{2^{n-2}}y$ is an involution. But as it is not centralizing $b$ this gives a contradiction.

\item[(xviii)] $\tilde{z} = z$:\\
If $\circ(\tilde{z}) > 2$ holds, then $G/(\langle c \rangle \times \langle \tilde{z}^2 \rangle)$ is an elementary abelian group where the images of $a$, $b$, $x$, $y$ and $\tilde{z}$ are independent elements. To see this use the fact that $\circ(x) = \circ(y) = 4$ by (xvi) and (xvii). But this cannot be true, as $G$ is $4$-generated by (x).
...
Assume $a^x \neq az$. Then $a^x = a^{-1}x$ by the properties of $x$ and (iv). Then using (xi) we get $(ax)^2 = a^2x^2[x,a] = czcz = 1$, so that $ax$ would be an involution not centralizing $b$. Similarly, if $b^y \neq bz$ we would have $by$ an involution not centralizing $a$.
...
%This is moreover the only element of order $4$ acting trivially, as $Q$ in total acts non-trivially on $P_0$. Moreover if $n \geq 3$ and $g$ acts non-trivially, then $g^\alpha h$ acts trivially for some $\alpha$ not divisible by $2^{n-1}$. But this would be another element of order $4$ acting trivially on $P_0$, so that we can assume $g$ to always act trivially on $P_0$ and $h$ non-trivially. 
...
Conversely, suppose $S \leq G $ is the unique minimal normal subgroup of $G$, that $S$ is non-abelian and $G/S$ is Dedekind. Now, any $N \unlhd G$ contains the unique minimal normal subgroup $S$. Hence if $Y \leq G$, then $S \leq NY$ and so $NY / S \leq G/S$ is normal. Therefore, $NY \unlhd G$ as needed.
...
Let $G$ be a finite group and $n \in \Z G$ nilpotent. Then $G$ {\it has ND} if $ne \in \Z G$ for every primitive central idempotent $e$ of $\Q G$. In this section we answer property ND for all finite groups which are not as in \Cref{Defintion solv SSN unfaithful type}. In particular for such groups we show that there is a unique counterexample to Jespers-Sun's \cref{conj:EricWei}. 
...
%It can be checked using GAP \cite{GAP} that there are exactly three groups of order $32$ with this property and by wedderga \cite{wedderga} the only one of them having rational group algebra isomorphic to $\mathbb{Q}G$ is $G$ itself.
...
As could be expected from the content of \Cref{conj:EricWei}, we need to recall some methods to construct primitive central idempotents of $\Q G$. These methods were introduced by Olivieri-del R\'io-Sim\'on \cite{OlRiSi}, see \cite[Chapter 3]{GRG1} for a good introduction. To start, recall that if $ H\unlhd G$, then $\wh{H}$ is a central idempotent in $\Q G$. Now, set $\epsilon(H,H) = \wh{H}$ and for for a strict normal subgroup $K$ of $H$ define
...
With notations as above, $e(G,H,K)$ is a primitive central idempotent of $\Q G$ if and only if $(H,K)$ is a strong Shoda pair. Moreover, in that case $\Cen_G(\epsilon(H,K)) \cong N_G(K)$ and $\ker(\varphi_{e(G,H,K)}) = \text{core}_G(K) = \bigcap_{g \in G} K^g$
\end{theorem}
...
\end{lemma}
\begin{proof}
Following \cite[Theorem 3.5.5.]{GRG1}, $\Q G e(G,H,K) \cong M_{[G : N_G(K)]}(\Q(\zeta_{[H:K]}) \star N_G(K)/H)$ for some crossing that can be made explicit (see \cite[Remark 3.5.6]{GRG1}). Therefore, denoting by $\phi(\cdot)$ the phi-Euler function, one has that
$$\begin{array}{ll}
...
\end{equation}
Note that the center of $G(p,m,n)$ is $\langle a^p \rangle \times \langle b^p \rangle$. When working with a group $G(p,m,n)$ we will always assume that is has generators and relations exactly as given in \eqref{definition BJ1 class}.
...
\end{lemma}
\begin{proof} We first calculate
\[A^2 = \begin{pmatrix} x^2 + yz & xy + yw \\ xz + zw & w^2 + yz \end{pmatrix} = \begin{pmatrix} x^2 + yz & y(x + w) \\ z(x + w) & w^2 + yz \end{pmatrix}. \]
As $R$ is a domain, $A$ has a Jordan normal form. So it is nilpotent if and only if $A^2=0$ which by the previous is equivalent to one of the following
\begin{itemize}
\item $y = x = w = 0$
\item $z = x = w = 0$
\item $y \neq 0$, $z \neq 0$, $x = -w$, $x^2 = -yz$
\end{itemize}
Analyzing these conditions one sees that actually the condition $x = -w$, $x^2 = -yz$ is sufficient as it also holds in the first two cases.
\end{proof}
...
Next we consider the $\mathbb{H}_\mathbb{Q}$-component where the projection of $n$ must equal $0$. With the representation above $n$ is send to
\begin{align*}
...
\end{equation}
Together with \eqref{eq:gSignga^2}, \eqref{eq:gCoefModb4} and \eqref{eq:1eq0} the congruences \eqref{eq:congab^2}, \eqref{eq:congab3} and \eqref{eq:congab3} can be compactly written as 
...
Our next goal is to show that $G(2,2,3)$ is in fact the only nilpotent counterexample to \Cref{conj:EricWei}. In the nilpotent case, by \Cref{prop:NilpotentSNNotSSN} we need to understand nilpotent groups with SSN and the groups $G(p,m,n)$ from (\Cref{definition BJ1 class}) in particular. 
...
\end{lemma} 
\begin{proof}
By (i) we have $rs = sr$, so that by (i) $(y(r+s)/p)^2 = y^2(r+s)^2/p = 0$. So by (iii) $y(r+s)/p$ is a nilpotent element in $\mathbb{Z}G$ and moreover $ey(r+s)/p = yr/p$ by (ii). So by (iv) $G$ does not have ND.
\end{proof}

The property of having one matrix component was systematically studied for groups with SSN by Jespers and Liu. We record the result relevant for this section which motivates the following lemmas.
...
is $0$ for any $1 \leq i \leq p-1$. Indeed, using $\prod_{j=0}^{p-1}(X-\zeta^j) = X^p - 1$, as an equation in the polynomial ring $\mathbb{Z}[X]$, up to the factor $b^i$ and possibly a sign this coefficient is the same as in $\prod_{j=0}^{p-1} (a^{p^{m-2}} - \zeta^j) = (a^{p^{m-2}})^p - 1$. So,
...
We already know by \Cref{th:counterex} that $G(2,2,3)$ does not have ND. So to exclude the cases of $G$ having one matrix component by \Cref{lem:GpmnOneMatrixComponent} we can assume that $n \geq 1$ and $(m,n) \notin \{(2,2),(2,3) \}$. Then $G$ does not have ND for any of the remaining cases by Lemmas~\ref{lem:GpmnEvenCase}, \ref{lem:GpmnOddCase1}, \ref{lem:GpmnOddCase2}, \ref{lem:GpmnOddCase3}.
\end{proof}
...
For any finite group $G$ whenever $\Q G$ has at most one  matrix component then $G$ has ND. Conversely, assume that $G$ has ND and hence property SN. As $G$ is assumed to not be an SSN group of unfaithful type,  \Cref{prop:SolvSNGroups} says that $G \cong P \rtimes H$ for an elementary abelian $p$-group $P$, where the action is faithful, irreducible and $[x,h] \neq 1$ for every non-trivial $x \in P$ and  $h \in H$. Moreover $H$ is Dedekind. In other words, by \Cref{th:DedekindGroups}, either $H$ is abelian or $H \cong Q_8 \times D$ with $D$ and odd order abelian group. In the latter case we denote by $c \in Q_8$ the unique (central) element of order $2$.
...
Next let $\mathcal{T}_K$ be a left transversal for $N_H(K)$ in $H$. Then as any $K \in \mathcal{S}_d$ is a maximal subgroup, one has by \Cref{form of SSP idempotent result} that 
...
Next notice that by \cite[Lemma 3.3.]{JespersSun}, $H$ has no nonzero nilpotent elements. Therefore, when decomposing $\Q G$ as
$$\Q G \cong \Q G \widehat{P} \oplus \Q G (1-\widehat{P})$$
the piece $\Q G \widehat{P} \cong \Q G/P \cong \Q H$ has no matrix components. So, it remains to prove that $\Q G (1-\widehat{P})$ is simple. For this let $\zeta_p$ be a primitive $p$-th root and recall that 
$\Q G e_K \cong \Ma_{[G : N_G(K)]}(\Q(\zeta_p) \star N_G(K)/P).$
Thus,
$$\dim_{\Q }(\Q G e_K) = [G : N_G(K)]^2 (p-1) [N_G(K):P] = |H| (p-1) [G:N_G(K)].$$
...
It now remains to consider finite non-solvable groups. In this case we prove that none of the groups as in \Cref{prop:NonSolvableSNGroups} have ND. This will be done by proving that for any such group there is always a bicyclic nilpotent element which does not have ND. 
...
Denote by $\SL_1(\Z G)$ the group of elements in $\U(\Z G)$ whose projections to any simple component of $\Q G$ all have reduced norm $1$ over the centre of that component. Note that $\langle  \U(\Z G)_{un} \rangle $ and $\langle \mathcal{E}_G(e) \rangle$ are normal subgroups of $\U (\Z G)$ which are contained in $\SL_1(\Z G)$. The measure is the following quotient group:
...
$$E_n(I) := \langle e_{ij}(r) \mid 1 \leq i \neq j \leq n, \, r \in J \rangle $$
...
Next note that $\SL_1(\Z G) (1-f)$ is the projection of $\SL_1(\Z G)$ onto the simple components of $\Q G$ of reduced degree at least $2$. Also notice that a unipotent unit $\alpha$ in $\Z G$ projects in any simple component to a unipotent element and in particular has reduced norm $1$ there. Thus $\langle  \U(\Z G)_{un} \rangle$ can be viewed as a subgroup of $\SL_1(\Z G) (1-f)$. Moreover, by definition $\SL_1(\Z G) (1-f)$ is a subgroup of $\prod_{e \in \PCI (\Q G)_{\geq 2}} \SL_{n_e}(\O_e).$ Furthermore, $U_e \leq \mathcal{E}_G(e)$ as elementary matrices are unipotent. Altogether, as we are both increasing the nominator and decreasing the denominator, this yields the desired inequalities.
\end{proof}
...
We will need the following result which seems to be known to experts, but which we could not find explicitly in the literature.

\begin{lemma}\label{unipotent in congruence of SL2Z}
Let $u$ be a unipotent matrix in $\SL_2(\Z)$ and $H$ a finite index subgroup of  $\Gamma(n)$ for some $n$. Then $u$ is conjugate inside $\SL_2(\Z)$ to a matrix of the form $\begin{pmatrix}
1 & m \\ 0 & 1
\end{pmatrix}$ with $m \in \Z$. Moreover, if $n$ is largest such that $H \leq \Gamma(n)$, then the quotient $H/ \langle B \in H \mid B \text{ is unipotent } \rangle$ is infinite if and only if $n \geq 6$. In that case, it is a non-torsion group.
\end{lemma}
\begin{proof}
...
y \\ -x
\end{pmatrix}$ are equal. In other words both columns of $u-1$ are eigenvectors and hence even any vector is a generalized eigenvector. \vspace{0.2cm}


Next let $n$ be a the largest positive integer such that $H$ a finite index subgroup of $\Gamma(n)$. Consider the normal subgroup 
$$N = \langle B \in H \mid B \text{ is unipotent } \rangle$$ of $H$. As $\Gamma(n)$ is a normal subgroup of $\SL_2(\Z)$ the first part yields that every generator of $N$ is conjugate to an element of the form $\begin{pmatrix}
1 & m \\ 0 & 1
\end{pmatrix}$ with $m \equiv 0 \mod n$.  Moreover by the maximality of $n$ there is an element of $N$ that is conjugate to $\begin{pmatrix}
1 & n \\ 0 & 1
\end{pmatrix}$. Thus $N$ is a subgroup of $cl_{\SL_2(\Z)}\left( \langle \begin{pmatrix}
1 & n \\ 0 & 1
\end{pmatrix}\rangle\right)$, the normal closure of the subgroup generated by $\begin{pmatrix}
1 & n \\ 0 & 1
\end{pmatrix}$ inside $\SL_2(\Z)$.  Denote the image of this normal closure in $\PSL_2(\Z)$ by $K(n)$.  It is known, e.g. see \cite[Chapter VIII, Section 13]{NewmanBook}, that $\PSL_2(\Z) / K(n)$ is isomorphic to the triangle group $\langle x_1, x_2 \mid x_1^2 = x_2^2 = (x_1 x_2)^n = 1 \rangle$ with parameters $(2,3,n)$. Moreover, this group is infinite if and only if $n \geq 6$. Since $\Gamma(n)$ is of finite index in $\SL_2(\Z)$ the same conclusion holds for $\Gamma(n)/ cl_{\SL_2(\Z)}\left( \langle \begin{pmatrix}
1 & n \\ 0 & 1
\end{pmatrix} \rangle \right)$ and subsequently also for $H/N$ as $H$ is of finite index in $\Gamma(n)$. The description of the conjugacy classes of torsion elements in $\PSL_2(\Z) / K(n)$, see \cite[Theorem 2.10]{Magnus}, yields that $H/N$ is non-torsion when $n \geq 6$.  This finishes the proof of the second part.
\end{proof}
...
%Using the proof of \cite[Theorem 10.6]{JJS} and \cite[Proposition 8.1]{JJS} one has a ring monomorphism
...
That the image is the latter can be directly verified and is also recorded in \cite[Proposition 8.1]{JJS}. The triple $(gh,h,Q)$ obtained above satisfies the conditions from \cite[Definition 10.5 \& Remark 10.7]{JJS} and thus one has the associated non-trivial group of $H$-units $\mathcal{H}(gh,h,Q)$. As $G = \langle gh, h, Q \rangle$ we can use \cite[Theorem 10.6]{JJS}, saying that $\mathcal{H}(gh,h,Q) = \SL_1(\Z G) \cap 1-e + \Q Ge.$ In other words $\mathcal{H}(gh,h,Q)$ is the largest subgroup of $\SL_1(\Z G)$ fully contained in $\SL_{2}(\Z)$ (with contained we mean that the subgroup projects trivially on all the other simple components of $\Q G$). Furthermore, $\mathcal{H}(gh,h,Q) = 1 - e + V_{m_e}$ with $m_e = 2 |Q|$ and where 
$$V_{m_e} = \left\{ \left(\begin{array}{cc}
1 + m_e \, l_2 & m_e \, t_1  \\
 m_e \,t_2 & 1 + m_e\, l_1
\end{array} \right) \in \SL_2(\Z) \mid l_1 \equiv l_2 \text{ and } t_1 \equiv t_2 \bmod 2 \right\}$$
 is a subgroup of index $2$ in $\Gamma(m_e)$. Moreover, $V_{m_e}$ is a normal subgroup of $\SL_{2}(\Z).$ By \Cref{unipotent in congruence of SL2Z}, the subgroup 
...
%Denote the composition of the isomorphisms $\Z Ge \cong \Z^{\alpha}[\langle \ov{a}, \ov{b} \rangle] \cong \Ima(\phi)$ by $\Phi$. 
...
Using the procedure from the proof of \Cref{unipotent in congruence of SL2Z}, we find that $S^{-1} \phi(ue) S = \begin{pmatrix}
...
{\it Claim: } Let $K \leq H \leq \Gamma \leq \wh{\Gamma}$ with $K$ normal in $\wh{\Gamma}$ and $H$ normal in $\Gamma$. Denote by $cl_{\wh{\Gamma}}(H)$ the normal closure of $H$ in $\wh{\Gamma}$.  If $[H:K]$ and $[\wh{\Gamma}: \Gamma]$ are finite, then also $[cl_{\wh{\Gamma}}(H): K]$ is finite.

We apply this to $K = N_e$ defined in (\ref{unipotent in the H-unit sbgrp}) which is a normal subgroup in $\mathcal{H}(gh,h,Q)e = V_{m_e}$. As unipotent matrices stay unipotent under conjugation, it is also normal in $\wh{\Gamma} = \SL_2(\Z)$.
 We also take $H = \varphi_e(\langle  \U(\Z G)_{un} \rangle)$ which is normal in $\Gamma = \SL_1(\Z G)e$. Now, since $\Z G$ is an order contained in the order $\prod_{f \in \PCI( \Q G)_{div}} \Z G f \times \prod_{e \in \PCI (\Q G)_{\geq 2}} \Ma_{n_e}(\mc{O}_e)$, it follows that the corresponding $\SL_1$ are subgroups of finite index \cite[Lemma 4.6.9 \& Proposition 5.5.1]{GRG1}. Therefore $\Gamma = \SL_1(\Z G)e$ is of finite index in $\wh{\Gamma}=\SL_2(\Z)$. Suppose that $N_e$ would be of finite index in $\varphi_e(\langle  \U(\Z G)_{un} \rangle$. Then by the above claim $N_e$ would also be of finite index in $cl_{\SL_2(\Z)}(\varphi_e(\langle  \U(\Z G)_{un} \rangle).$ The latter group contains $cl_{\SL_2(\Z)}(\langle \phi(ue) \rangle) = cl_{\SL_2(\Z)}\left(\langle  \begin{pmatrix}
1 & -4 \\ 0 & 1
...
The last two groups have SmallGroup ID $[16,8]$, resp. $[16,3]$. The latter is sometimes given in the literature with the presentation $\langle a,b,c \mid a^2= b^2 = c^4 = [a,b] = [b,c]= 1, c^{a}= bc\rangle.$ 
...
\begin{equation*}
\text{If }  S\text{-rank}(\mathbf{G}_i(F)) \geq 2 \text{ for all anisotropic }\mathbf{G}_i(F), \text{ then } |q(\Gamma)|< \infty 
\end{equation*}
In particular, under these conditions finiteness of $q(\Gamma)$ does not depend on the chosen $S$-arithmetic subgroup $\Gamma$.
...
\section{Further related properties:  nilpotent decomposition, different kernels and bicyclic resitance}\label{sec:DK prop section}
...
\begin{definition}
A finite group $G$ is said to have the \emph{Different Kernel property}, \emph{DK} in short, if for any orthogonal pair $e,f \in \PCI(\Q G)$ one has $\ker(\varphi_e) \neq \ker(\varphi_f)$. In other words, any two non-equivalent irreducible $\mathbb{Q}$-representations of $G$ have different kernels.
\end{definition}
...
\end{proposition}
\begin{proof}
Given a normal subgroup $N$ of $G$ we define the set $\mc{I}(N) = \{ e \in \PCI(\Q G) \mid N \subseteq \ker(\varphi_e) \}$ which corresponds to the irreducible $\Q$-representations containing $N$ in their kernel. Note that $\Q G \wh{N} = \bigoplus\limits_{e \in \mc{I}(N)} \Q G e$ and hence $\Q G (1 - \wh{N}) = \bigoplus\limits_{e \in \PCI(\Q G) \setminus \mc{I}(N)} \Q G (1-e)$.
...
Next assume that $e$ is the unique idempotent with $\Q Ge$ not commutative. In other words, $G'  \not \subseteq \ker(\varphi_e)$ but $G' \subseteq \ker(\varphi_f)$ for any other $f \in \PCI(\Q G)$.  Moreover the first part tells that the primitive central idempotents different from $e$ have also different kernels, hence altogether $G$ has DK.
\end{proof}
...
\end{proposition}
\begin{proof}
Any irreducible $\Q$-representation of $G/N$ is also an irreducible $\Q$-representation of $G$, so the first item follows.
...
\end{align}
As we saw above any primitive summand of $e \otimes f$ is orthogonal with any primitive summand of $e' \otimes f$. So if $e \otimes f$ would not be primitive, this would contradict \eqref{eq:PCIDirectProduct}.
...
\end{proposition}
\begin{proof}
The finite subgroups of division algebras in characteristic $0$ were obtained by Amitsur, we refer to \cite[Theorem 2.1.4, 2.1.5]{ShirvaniWehrfritz} for a full account. It follows that when $G$ is not one of the three possibilities listed explicitly in the statement, then $G$ is the direct product of groups of comprime orders such that each factor has the shape given in \Cref{lem:DKForCmQ}. So by \Cref{lem:DKForCmQ} and \Cref{prop:DKDirectProductsAndQuotients} we conclude that $G$ has DK. It remains to show that this is not the case for the three cases listed.
...
We call $G$ \emph{bicyclic resistant}, if for any bicyclic nilpotent element $n \in \mathbb{Z}G$ and any central idempotent $e \in \Q G$ one has $ne \in \mathbb{Z}G$.
\end{definition}
...
for certain integers $a_M$. If (3) holds, then $n\wh{M} \in \mathbb{Z}G$ for any $M\unlhd G$ and consequently (2) holds. To see that (2) implies (3) we argue by induction on the minimal length of a chain of normal subgroups from $N$ to $G$. For the induction start notice $n\epsilon(G,G) = n\wh{G}$. Now let $N \unlhd G$. Then
...
\end{question}
%\ChLeo{I know it is my formulation, but maybe you have an idea, why I put the condition on the kernel? Now it seems to me it makes more sense to ask for all unfaithful}
