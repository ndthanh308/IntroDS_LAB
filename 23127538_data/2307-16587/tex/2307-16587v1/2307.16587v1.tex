...
%
\begin{document}
\title{Note on generating operators
\\
...
\begin{abstract}
By using the \lq\lq{generating operator}\rq\rq\ of the Rankin--Cohen brackets
 introduced in Kobayashi--Pevzner 
 [arXiv:2306.16800], 
 we present a trick to produce 
 various non-local intertwining operators
...
In \cite{KPgen} we initiated 
a new line of investigation 
 on branching problems in representation theory
 through \lq\lq{generating operators}\rq\rq\
 between two manifolds.  
By definition \lq\lq{generating operators}\rq\rq\
...
In this note, 
 we illustrate a new trick 
 about how differential SBOs
 ({\bf{countable data}}) 
 yield a meromorphic family of non-local intertwining operators
 ({\bf{continuous data}})
 in different geometric settings
 via \lq\lq{generating operators}\rq\rq.  
For this, 
 we bring the idea of hyperfunctions \cite{xsato}
 into the analysis of generating operators.  



As a biproduct, 
 we prove in Theorem \ref{thm:23062904}
 that the Rankin--Cohen brackets give a geometric embedding
 of discrete series representations
 for the de Sitter space $\operatorname{d S}^2$
 into principal series representations
 through the analytic continuation 
of eigenfunctions
 which is guaranteed by the theory of discretely decomposable restriction
\cite{xkInvent98}.  
...
In defining the generating operator of SBOs, 
 we wish to have some control of {\it{multiplicities}}
...
In \cite{KPgen}, 
 ...
 in a special case
 of the general framework of Fact \ref{fact:2.2}
...
Our approach is formulated by viewing elements
 ...
 as \lq\lq{boundary values}\rq\rq\
...
~~~
\newline
This section defines a \lq\lq{meromorphic continuation}\rq\rq\ 
 of $\{R_{\ell}\}$ from 
 ${\mathbb{N}}$ to ${\mathbb{C}}$
...
Then $\operatorname{d S}^2$ is identified 
 ...
 by $g H \mapsto \operatorname{Ad}(g)E_{1,1}
...
The tensor product 
 ...
 to $L^2(G/K)$, 
 and does not contain a discrete series.  
On the other hand, 
 any discrete series for $\operatorname{d S}^2$
 arises from the $K$-admissible tensor product 
 ${\mathbb{H}}(\Pi_\varepsilon) \,\widehat \otimes\, {\mathbb{H}}(\Pi_\varepsilon)$
 (\cite{xkAnn98}), 
 hence any $K$-finite vector $f$ is $(K \times K)$-finite
 by \cite{xkInvent98}.  
Therefore $f \in L^2(G/H)$ extends to a real analytic section $\widetilde f$
 over $G/P \times G/P$.  
\end{proof}
...
Then $f \mapsto R_{\ell} \widetilde f$ is a well-defined 
...
This work was partially supported by the JSPS
 under the Grant-in Aid for Scientific Research (A) (JP18H03669).  
