...
A practical version of the algorithm appears via a \emph{saddle-point} reformulation of the quasilinear subproblem~\cref{eq:EPEIntro}, leading to the finite element method that gives this paper its name.
...
\begin{algorithm2e}\DontPrintSemicolon
	...
	\Output{Two approximate solutions, $u_{h}$ and $\widetilde{u}_h = \exp\psi_h$, and an approximate Lagrange multiplier, $\lambda_h = (\omega_h - \psi_h)/\alpha$.}
		\BlankLine
	\Repeat{a convergence test is satisfied}
	{
...
The proximal Galerkin finite element method is as much a finite element method as it is an optimization algorithm.
...
Readers uninterested in the historical context are encouraged to move directly to~\Cref{sub:contributions}.
...
Yet, \Cref{sec:maximum_principles,sec:new_algorithms_for_topology_optimization} provide further sample applications with an eye towards future work.
...
\begin{itemize}[leftmargin=0cm,itemindent=1cm,itemsep=3pt,topsep=3pt]
	...
	In this case, the correct quadrature rule induces a nodally bound-preserving primal solution; cf.~\Cref{rem:MassLumping}.
...
\label{eq:ObstacleSet}
	\mathcal{O} := \{ \varphi \in H^1(\Omega) \cap C(\overline{\Omega}) \mid \Delta \varphi \in L^{\infty}(\Omega) \}.
\end{equation}
		...
		\label{eq:AdvectionDiffusionDiscreteNonlinearSaddlePoint}
			...
			\begin{aligned}
				...
				&\begin{alignedat}{4}
					...
					\,,
					\\
					(u_h, \varphi) - (\sigmoid\psi_h, \varphi) &= 0
					&&~\fa \varphi \in W_h
					\,.
				\end{alignedat}
			\end{aligned}
...
\begin{remark}[Stability]
	...
	Therefore, if we assume $\{\mathcal{T}_h\}$ is quasi-uniform and we are using any of the equal-order pairs in~\cref{eqs:SubspacePairs_EqualOrder}, then $\mathcal{P}_h(H^1_0(\Omega)) \subset V_h = W_h$ and~\cref{eqs:GlobalL2OrthogonalProjection} imply that there exists $\beta_0 = 1/c > 0$ such that
	\begin{equation}
...
The first result of this section is that all linearizations of the subproblems in~\Cref{alg:ObstacleProblem} are stable at the continuous level.
...
\label{thm:ContinuousStability}
	...
	Then, for every $f \in L^2(\Omega)$ and $\phi \in H^1(\Omega)$, the saddle-point problem
	\begin{equation}
	\label{eq:ContinuousStability}
		...
		\begin{aligned}
			...
			&&~\fa v \in H_0^1(\Omega),
			\\
			(u, \varphi) - (\delta \exp\psi, \varphi) &= (\phi, \varphi)
			&&~\fa \varphi \in L^2(\Omega),
			\end{alignedat}
		\end{aligned}
		\right.
...
\begin{proof}[Proof of~\Cref{thm:ContinuousStability}]
	...
	\label{eq:LinearizedSaddleRewrite}
		B((u,\delta),(v,\varphi)) = (f, v) - ( \phi, \varphi)
		~
		\fa v \in H_0^1(\Omega)
		\text{ and }
		\varphi \in L^2(\Omega)
	...
	$
	is bounded bilinear map $B \colon H_0^1(\Omega) \times L^2(\Omega) \to \mathbb{R}$.
	...
	Here, linearity of~\cref{eq:ContinuousStability} allows us to consider the cases $\phi = 0$ and $f = 0$ separately.
	\medskip

	\noindent\textit{Case 1: $\phi = 0$}.
...
\begin{proof}[Proof of~\Cref{lem:Fortin}]
	...
	We now analyze $\tilde{\Pi}_{h}$ more closely element-wise.
...
This subsection is devoted to a proof that the $(\mathbb{P}_{p}\text{-bubble},\mathbb{P}_{p-1}\text{-broken})$ and $(\mathbb{Q}_{p}\text{-bubble},\mathbb{Q}_{p-1}\text{-broken})$ elements defined in~\cref{eq:SubspacePairs} converge optimally toward the solutions of the linearized subproblems~\cref{eq:ContinuousStability}.
...
\label{thm:APrioriLinearized}
	...
	for all $1 \leq s \leq \min\{r,p\}$, where $C_1$ is a mesh-independent constant that remains bounded as $\essinf \psi \to -\infty$.
		Moreover, if in addition $\delta \in H^{r}(\Omega)$, then there exists a mesh-independent constant $C_2$ such that
	\begin{equation}
...
\begin{proof}
	...
	\begin{equation}
		...
		\sup_{v \in V}
		\sup_{w \in W}
		\frac{|d(v,w)|}{\|v\|_V \|w\|_W}
	\end{equation}
	denote the norm of a continuous bilinear form $d \colon V \times W \to \mathbb{R}$.
	A classical analysis given in, e.g., \cite[Section~4.3]{boffi2013mixed}, shows that $C$ in~\cref{eq:BAE} depends at most on $\|a\|$, $\|b\|$, $\|c\|$, the coercivity constant for $a$, and $\beta_0$, and that $C$ remains bounded as $\|c\| \to 0$.
...
The authors gratefully thank Jesse Chan, J\'er\^{o}me Darbon, Alexandre Ern, Patrick Farrell, Caroline Geiersbach, Dohyun Kim, Tarik Dzanic, Boyan Lazarov, Michael Hinterm\"uller, Thomas J.R.~Hughes, Karl-Hermann Neeb, Chi-Wang Shu, and N.~Sukumar for their time and helpful discussions during the development of this work.
