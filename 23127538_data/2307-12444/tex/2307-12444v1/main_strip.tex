...
The paper begins with a derivation of the latent variable proximal point (LVPP) method: an unconditionally stable alternative to the interior point method.
LVPP is an infinite-dimensional optimization algorithm that may be viewed as having an adaptive (Bayesian) barrier function that is updated with a new informative prior at each (outer loop) optimization iteration.
One of the main benefits of this algorithm is witnessed when analyzing the classical obstacle problem.
...
\noindent\fbox{%
    \parbox{0.965\textwidth}{
        \begin{center}
        \medskip
            \emph{\bfseries This is a ``working paper.'' Sections 5 and 6 will be included in a future edition.}
        \medskip
        \end{center}
    }
}
...
When partial differential equations (PDE) are first written down, their essential boundary conditions are often the only explicit constraints that appear on the space of solutions.
...
\begin{equation}
	...
	,
\end{equation}
under the additional condition $\int_\Omega \nabla u^\ast \cdot \nabla u^\ast \dd x = \int_\Omega f u^\ast \dd x$; see, e.g., \cite[Theorem~1.1.2]{ciarlet2002finite}.
...
\[
L^p_+(\Omega) := \left\{ u \in L^p(\Omega) \mid u \ge 0 \text{ a.e. } \Omega \right\}
\]
...
\begin{equation}\label{eq:complementarity}
	...
	\,.
\end{equation}
Another perspective uses a dual variable $\lambda^\ast$, also known as a Lagrange multiplier, to formulate \eqref{eq:complementarity} as 
...
Since the range of this isomorphism is contained in the $H^1(\Omega) \cap L^\infty(\Omega)$-interior of the set of essentially bounded non-negative $H^1(\Omega)$ functions, we find that $$u^k = \exp \psi^k \in H^1(\Omega) \cap \interior L^\infty_+(\Omega) \subset \interior (H^1(\Omega) \cap L^\infty_+(\Omega))$$ will always be an \emph{interior point}.
In the next motivational subsection, we explain that an idenitical sequence of interior points $u^k\to u^\ast$ can be found by regularizing the Dirichlet energy with an appropriate \emph{entropy} functional.
...
\subsection{Dirchlet free energy} \label{sub:dirchlet_free_energy}
...
\label{eq:EPE_ConvergenceWRTTemperature}
	\|\nabla u^\ast - \nabla u\|_{L^2(\Omega)}^2 \leq \theta (S(u^\ast) + 1)
	\,,
\end{equation}
...
See also \cite{dahlke2022wavelet}.
...
\begin{itemize}[leftmargin=0cm,itemindent=1cm,itemsep=3pt,topsep=3pt]
	...
	Moreover, the nature of the functionals involved in our approach indicate that we need to work in a non-standard, \emph{non-reflexive} setting that is nevertheless natural for entropy regularization in infinite dimensions.
		\item
		The adaptive entropy regularization technique explored in this paper indicates the potential for a broad methodology in which the nonlinearity arising from the variational derivative of the entropy term can be replaced by a slack variable, which can in turn by transformed back into a primal variable, which we term the ``latent'' variable. This ultimately allows a greater degree of flexibility in the choice of discretization scheme as it naturally facilitates structure-preserving discretizations. We coin this framework the latent variable proximal point (LVPP) methodology.
	\item
	...
	\begin{equation}\label{eq:entrop-poisson}
		...
		\,,
			\end{equation}
	and the \emph{binary-entropic Poisson equation} (to appear in \Cref{sec:maximum_principles}),
	\begin{equation}
				-\Delta u + \atanh u = f
				\,.
	\end{equation}
	...
	\item
	We present a new algorithm for topology optimization (to appear in \Cref{sec:new_algorithms_for_topology_optimization}) to showcase the breadth of applicability of geometric optimization techniques developed in this work.
...
Let  $S: L^{p}(\Omega) \to  \mathbb R \cup \left\{+\infty\right\}$, $p \in [1,\infty]$, be the negative entropy functional defined by 
	...
	Fr\'echet differentiable on $\interior L^\infty_+(\Omega)$
	with respect to the norm topology on $L^{\infty}(\Omega)$.
	In particular, the Fr\'echet derivative of $S$ can be uniquely characterized by the variational equation
	\begin{equation}
...
Recall that in~\Cref{sub:dirchlet_free_energy} we proposed the regularized Dirichlet free energy functional
\begin{equation}
	A(u) = E(u) + \theta S(u)
	\,,
\end{equation}
...
From now on, we mainly focus inhomogeneous obstacle problems; i.e., $\phi \neq 0$.
...
Here and throughout, $\mathcal{T}_h$ always denotes a shape-regular partition of the domain $\Omega \subset \mathbb{R}^2$ into finitely many open connected tringular or quadrilateral mesh cells $T$ with Lipschitz boundaries $\partial T$ such that $\Omega$ is the union of the closure of all mesh cells $T$ in $\mathcal{T}_h$.
...
\noindent\fbox{%
    \parbox{0.965\textwidth}{
        \begin{center}
        \smallskip
            \emph{\bfseries This is a ``working paper.''  Section 5 will be included in a future edition.}
        \smallskip
        \end{center}
    }
}
...
\section{Extensions II: Non-convex objective functions and a simple algorithm for topology optimization}
...
\noindent\fbox{%
    \parbox{0.965\textwidth}{
        \begin{center}
        \smallskip
            \emph{\bfseries This is a ``working paper.'' Section 6 will be included in a future edition.}
        \smallskip
        \end{center}
    }
}
...
\begin{proof}
	...
	The proof is completed by setting $u = u^\ast$ and exploiting the strong convexity of $E$, as done in~\cref{eq:ConvergenceContinuousLevel_StrongConvexity}, and noting that $D(u,1) = S(u) + 1$.
\end{proof}
...
The authors gratefully thank J\'er\^{o}me Darbon, Alexandre Ern, Caroline Geiersbach, Dohyun Kim, Tarik Dzanic, Boyan Lazarov, Michael Hinterm\"uller, Thomas J.R.~Hughes, Karl-Hermann Neeb, Chi-Wang Shu, and N.~Sukumar for their time and helpful discussions during the development of this work.
...
{
\section*{\small Extended dedication from B.~Keith} \label{sec:dedication}
\it
