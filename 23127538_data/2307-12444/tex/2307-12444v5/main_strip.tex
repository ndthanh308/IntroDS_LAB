...
This paper introduces the proximal Galerkin method and applies it to solve free boundary problems, enforce discrete maximum principles, and develop a scalable, mesh-independent algorithm for optimal design problems with pointwise bound constraints.
This paper also provides a derivation of the latent variable proximal point (LVPP) algorithm, an unconditionally stable alternative to the interior point method.
...
One of its main benefits is witnessed when analyzing the classical obstacle problem.
Therein, we find that the original variational \emph{inequality} can be replaced by a sequence of second-order partial differential \emph{equations} (PDEs) that are readily discretized and solved with, \textit{e.g.}, high-order finite elements.
...
As we show in \Cref{thm:PrimalProblem}, all minimizers of~\cref{eq:DirichletFreeEnergy} lie in $K\cap \interior L^\infty_+(\Omega)$, i.e., for all $\theta > 0$,
\begin{equation}
\label{eq:DirichletFreeEnergyMinimization}
	...
	\,,
\end{equation}
is essentially bounded away from zero in $\Omega$, and $u = u(\theta)$ converge linearly with respect to $\theta$ to the unique non-negative minimizer of~\cref{eq:DirichletEnergy}, $u^\ast = \argmin_{v \in K} E(v)$.
More specifically, each $u$ is an interior point of $K\cap L^\infty(\Omega)$ and
\begin{equation}
...
\begin{itemize}[leftmargin=0cm,itemindent=1cm,itemsep=3pt,topsep=3pt]
	...
	These finite elements lead to a primal solution $u_h$ with a feasible cell average; cf.~\Cref{rem:PositiveCellAverage}.
	The second type uses a $C^0(\Omega)$-\emph{continuous} latent variable; cf.~\Cref{sub:StableElement_ContinuousLatentVariable}.
	In this case, the correct quadrature rule induces a nodally-feasible primal solution; cf.~\Cref{rem:MassLumping}.
	Both types of proximal Galerkin discretizations lead to a secondary solution variable $\tilde{u}_h$ that is feasible \emph{pointwise} throughout the domain.
	\item
...
\DontPrintSemicolon
		...
		\Repeat{$\mathtt{tol}_{\mathrm{Newton}} < \mathtt{tol}_{\mathrm{exit}}$}
	{
		...
		\Repeat{$\|u_h - w_h\|_{L^2(\Omega)}^2 \leq \mathtt{tol}_{\mathrm{Newton}}^2$}
			...
			\label{eq:ObstacleDiscreteNonlinearSaddlePoint2}
				...
				\begin{aligned}
					...
					&\begin{alignedat}{4}
						...
						\,,
						\\
						(u_h, \varphi) - c_\varepsilon(\psi_{h},\delta_{h},\varphi) &= (\phi + \exp\psi_{h}, \varphi)
												&&~\fa \varphi \in W_{h}
						\,,
					\end{alignedat}
				\end{aligned}
			...
			where
			\begin{equation*}
				c_\varepsilon(\psi_{h},\delta_{h},\varphi)
				=
				\begin{cases}
					(\delta_{h}\exp\psi_{h}, \varphi) + \varepsilon (\delta_{h}, \varphi)
					& \text{if~} W_h \in \mathrm{ker}(\nabla_h)\,,\\
					(\delta_{h}\exp\psi_{h}, \varphi) + \varepsilon (\nabla_h \delta_{h}, \nabla_h \varphi)
					& \text{otherwise.}
				\end{cases}
			\end{equation*}
			\;
...
\begin{remark}[Practical aspects of the implementation: Stopping criteria]
	...
	convergence rate, which up to a constant could be used to predetermine a maximum number of step sizes to reach a 
	desired tolerance. Furthermore, if we set $\alpha_k = \alpha > 0$ for all $k$, then we may view the outer loop as a 
	\textit{globally} convergent fixed point iteration. Since these observations are for the ideal setting, we still should 
	check for some indication of convergence. For this 
...
All results from this experiment can be reproduced by running the FEniCSx code \texttt{obstacle.py} or the MFEM code \texttt{obstacle.cpp} available at \cite{ZenodoCode}.
...
This experiment is an official part of MFEM 4.6; in particular, see MFEM Example~36 \cite{Keith2023ObstacleCode}.
...
\label{eq:fixed_point_VI}
	...
	\begin{aligned}
		...
		\text{~such that}
		\\
		&(w,v-w)_{V} \ge (u,v-w)_{V} - \rho[a(u,v-w) - (f,v-w)]
		~\fa v \in K
		\,,
	\end{aligned}
	\right.
...
As  also explained in this remark, a particularly interesting consequence of mass lumping is that it induces a \emph{nodally}-feasible primal solution $u_h$.
Moreover, because nodal boundedness extends to pointwise boundedness when $p=1$, the primal solution $u_h$ will be \emph{pointwise}-feasible for any lowest-order $(\mathbb{P}_1,\mathbb{P}_1)$ or $(\mathbb{Q}_1,\mathbb{Q}_1)$ proximal Galerkin discretization with pointwise box constraints.
In contrast, the lowest-order elements in~\cref{eq:SubspacePairs} only induce a primal discretization with a feasible cell average; cf.~\Cref{rem:PositiveCellAverage}.
...
\begin{remark}[Nodal feasibility]
\label{rem:MassLumping}
	...
	We now employ the nodal quadrature technique \cite{fried1975finite} that is commonly used in, e.g., spectral element methods \cite{duczek2019mass}.
	Namely, if we assume $M=N$ and that $\varphi_j$ are formed from a nodal basis with nodes corresponding to the quadrature points $x_j$ (e.g., a Lagrange basis with Gauss--Lobatto nodes \cite{pozrikidis2005introduction}), then $\varphi_j(x_l) = \delta_{jl}$ for all $j,l = 1,\ldots, N$.
								In turn, we find that~\cref{eq:NodalFeasibilityQuadrature} reduces to
	\begin{equation}
						\mathsf{c}_i
		=
		\sigmoid(\mathsf{d}_i)
						\,.
	\end{equation}
	Finally, we note that $\mathsf{c}_i = u_h(x_i)$ and $\mathsf{d}_i = \psi_h(x_i)$ since we have assumed the basis $\{\varphi_i\}_{i=1}^N$ is nodal.
	Thus, the primal variable $u_h$ is nodally feasible; i.e., $0 \leq u_h(x_i) \leq 1$ at all nodes $i = 1,\ldots, N$.
...
Note that the two discrete solutions are similar, except the proximal Galerkin solution preseves the maximum principle $0 \leq u \leq 1$.
...
Note that neither of the numerical approximations depicted in~\Cref{fig:EJProblem,fig:EJProblem2} were obtained with numerical stabilization techniques that are common for this class of singularly-perturbed problems and usually required to consider smaller values of $\epsilon > 0$.
...
This experiment is an official part of MFEM 4.6; in particular, see MFEM Example~37 \cite{Keith2023TopOptCode}.
...
We have introduced a new nonlinear finite element method that hinges on a mathematical technique called entropy regularization.
We refer to the method as the latent variable proximal Galerkin finite element method or, simply, ``proximal Galerkin'' for short.
...
We hope that the proximal Galerkin methods that arise from this framework will lead to new challenges and opportunities in optimization theory, analysis of PDEs, and numerical analysis, as well as provide promising alternatives to the more classical procedures used in industry for industrial-scale problem solving.
...
\noindent\textsl{Step 5.} Prove that the entropic Poisson equation has a unique solution in $H^1_g(\Omega) \cap \interior L^{\infty}_+(\Omega)$.
	...
	\[
	A_{\alpha}(y) \ge A_{\alpha}(u) + A'_{\alpha}(u)(y -u ) = A_{\alpha}(u)\quad \forall y \in H^1_g(\Omega)\cap L^{\infty}(\Omega).
	\]
...
\subsection{Towards an entropic Poisson equation with homogeneous boundary conditions} \label{sub:the_entropic_poisson_equation_with_homogeneous_boundary_conditions}


In this subsection, we contemplate the mathematical meaning of an entropic Poisson equation with homogeneous boundary conditions.
\Cref{thm:PrimalProblem} intentionally avoids this setting due to the strict positivity requirement used to argue that $\max\{\epsilon, u\} \in H^1_g(\Omega) \cap \interior L^\infty_+(\Omega)$ in step 3 of the proof.
Nevertheless, as the following result demonstrates, we can still provide important insight into the Dirichlet free energy minimization problem~\cref{eq:subprob_obs} in the situation where $g \equiv 0$ on $\partial \Omega$ by applying \Cref{thm:PrimalProblem} sequentially.
...
\begin{proof}
According to \Cref{rem:ExplicitBounds}, we have the bounds 
\[
...
$
Since $L^{\infty}(\Omega)$ is the topological dual of a separable Banach space, there exists a subsequence $\{k_l\}$ and $\tilde{u} \in L^{\infty}(\Omega)$ such that $u_{k_l} \to \tilde{u}$ weak-$*$ in 
...
Continuing on, we note that $v = u_k - \varepsilon_k \in H^1(\Omega)$ and $\gamma(v) = 0$. Using  $v$ as a test function in \cref{eq:PrimalProblemVE}, we deduce that
\[
\alpha (\nabla u_k,\nabla u_k) \le |\Omega|/\exp(1) + \| \alpha f + \ln w \|_{L^{1}(\Omega)} \exp(\| \ln w + \alpha f \|_{L^{\infty}(\Omega)}).
\]
This follows from the fact that $ (\nabla u_k,\nabla (u_k-\varepsilon_k)) =  (\nabla u_k,\nabla u_k)$, 
\[
0 \le u_k - \varepsilon_k \le \exp(\| \ln w + \alpha f \|_{L^{\infty}(\Omega)}),
\] 
and
\[
( \ln(u_k),u_k - \varepsilon_k)
= 
( \ln(u_k - \varepsilon_k + \varepsilon_k),u_k - \varepsilon_k) 
\ge
( \ln(u_k - \varepsilon_k),u_k - \varepsilon_k),
\]
along with $x \ln x \ge -1/\exp(1)$. Consequently, there exists a further subsequence $\left\{k_{l_m}\right\}$ such that
$u_{k_{l_m}} \to \tilde{u}$ weakly in $H^1(\Omega)$ and we deduce that $\tilde{u}$ is feasible to \cref{eq:subprob_obs}. 
...
\[
\frac{1}{2}\| \nabla \bar{u}_{\varepsilon_k}  \|^2_{L^2(\Omega)} =
\frac{1}{2}\int_{\{\bar{u} \ge \varepsilon_k\}} |\nabla \bar{u}|^2 \dd x \to 
\frac{1}{2}\int_{\Omega} |\nabla \bar{u}|^2 \dd x
\,,
\]
by monotone convergence and $\bar{u} \ge 0$. Similarly, we see that
$
\| \bar{u}_{\varepsilon_k} - \bar{u} \|^2_{L^2(\Omega)} 
=O(\varepsilon^2).
$
...
Finally, we return to the $L^{\infty}$-statements. By the linearity of the trace, we have $\gamma(u_k - \bar{u}) = \varepsilon_k$, which clearly converges to 0 in $L^{\infty}(\partial \Omega)$. Likewise, if we assume that $u_k$ possess a subsequence that does not weak-$*$ converge to $\bar{u}$ in $L^{\infty}(\Omega)$, then we can find a further subsequence that does, which leads to a contradiction and completes the proof.
\end{proof}

Without a deeper analysis of the properties of $\bar{u}$ in \Cref{thm:homog-case}, it is difficult to say whether a type of entropic Poisson equation can be derived for the fully homogeneous case. Nevertheless, we do know that $\bar{u}$ is an optimal solution and is, therefore, feasible. On the active set, $\bar{u} = 0$ is fully determined.
...
\begin{proof}[Proof of~\Cref{thm:ContinuousStability}]
	...
	We now consider the cases $\phi = 0$ and $f = 0$ separately.
	\medskip
