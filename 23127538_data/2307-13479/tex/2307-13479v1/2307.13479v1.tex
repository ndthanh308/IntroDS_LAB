...
$$ 
of $R$-modules such that $\mu_R(C) = \rme^0_\m(C)$ (\cite[Definition 3.3]{GTT}). Here, $\mu_R(-)$ (resp. $\rme^0_\m(-)$) denotes the number of elements in a minimal system of generators (resp. the multiplicity with respect to $\m$). When $\dim R=1$, if $R$ is an almost Gorenstein local ring in the sense of \cite{GTT}, then $R$ is almost Gorenstein in the sense of \cite{GMP}. The converse does not hold in general (\cite[Remark 3.5]{GTT}, see also \cite[Remark 2.10]{GMP}). 
...
As we mentioned, by \cite[Proposition 3.4]{GTT} and the definition of almost Gorenstein local/graded rings, we only need to verify the implication $(2) \Rightarrow (1)$. What makes $(2)\Rightarrow (1)$ interesting and difficult is that the implication is not true in general.
...
We set $A=R_M$, $I=JA$, and $Q = fA$. Since $I^{r+1} = QI^r$, we see that $Q$ is a reduction of $I$. We denote by $\m$ the maximal ideal of $A$. 
By \cite[Proposition 3.4]{GTT} and the definitions of almost Gorenstein rings (\cite[Definitions 3.3, 8.1]{GTT}), we only need to verify the implication $(2) \Rightarrow (1)$. 
...
Since every non-zero ideal in numerical semigroup rings admits its minimal reduction, the almost Gorenstein property for local rings are equivalent in both senses \cite[Definition 3.1]{GMP} and \cite[Definition 3.3]{GTT}. In addition, the local ring $k[H]_M$ is almost Gorenstein if and only if $k[[H]]$ is an almost Gorenstein local ring; equivalently, the semigroup $H$ is almost symmetric (\cite[Proposition 29]{BF}, \cite[Theorem 2.4]{N}). %Here, a numerical semigroup 
...
\begin{rem} 
When the almost symmetric semigroup $H$ is minimally generated by four elements, Eto provided an explicit generators of defining ideals of the semigroup rings $k[H]$ by using the notion of RF-matrices (\cite[Section 5]{Eto}). 
\end{rem}
...
\end{smallmatrix}
\right) =V$. 
If we consider $T$ as a $\bbZ$-graded ring under the grading $k=T_0$, $Y \in T_4$, $Z \in T_5$, and $W \in T_6$, as we have shown in Example \ref{3.1} the ring $V$ is not almost Gorenstein as a graded ring. Whereas, we regard $T$ as a $\bbZ$-graded ring with 
