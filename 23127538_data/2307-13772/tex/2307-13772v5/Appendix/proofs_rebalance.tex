...
\begin{align}\label{eq:pi_diff_v2}
    \pi_L-\pi_H    &=\frac{1}{\lambda d_H d_L}\left[q_i\left(h \times \left(\frac{\qmg}{Q}\right)^{\frac{\lambda}{\theta}\frac{Q}{Q-1}} -\left(h-l\right)\right) -\Gamma \left(\frac{\qmg}{Q}\right)^{\frac{\lambda}{\theta}\frac{Q}{Q-1}} \right]. 
\end{align}

If the coefficient of $q_i$ in \eqref{eq:pi_diff_v2} is negative, that is $h \times \left(\frac{\qmg}{Q}\right)^{\frac{\lambda}{\theta}\frac{Q}{Q-1}} -\left(h-l\right)\leq 0$, then $\pi_L-\pi_H<0$ and all $\LP$s optimally choose to post liquidity on pool $H$. A necessary condition for a fragmented equilibrium is therefore that
...
Second, in any fragmented market equilibrium, the marginal $\LP$ is indifferent between providing liquidity on the $L$ or $H$ pool. Equivalently, the marginal trader solves $\pi_L-\pi_H=0$, which from equation \eqref{eq:mg_eq} translates to a solution of
...
\begin{lem}\label{cor:comp_stat}
\begin{leftbar} \setlength{\parskip}{0ex}
In equilibrium, the endowment marginal trader $\qmg^\star$:
\begin{itemize}
    \item[(i)] increases in the gas cost ($\Gamma$), the fee on pool $H$ ($h$), and the arrival rate of large trades ($\lambda$).
    \item[(ii)] decreases in the fee on pool $L$ ($\ell$) and the arrival rate of small trades ($\theta$).
\end{itemize}
\end{leftbar}    
\end{lem}

\noindent \textbf{\large Lemma \ref{cor:comp_stat}}
\begin{proof}
From the implicit function theorem, 
\begin{equation}
    \frac{\partial \qmg^\star}{\partial x}=-\left(\frac{\partial f_2}{\partial \qmg}\right)^{-1} \frac{\partial f_2}{\partial x}.
\end{equation}
...
\end{equation}
which implies that $\qmg^\star$ decreases in $\theta$. 
% Fourth,
% \begin{equation}
...
\begin{proof}
The comparative statics with respect to $\lambda$, $\theta$, and $\ell$ follow immediately from Lemma \ref{cor:comp_stat} and the fact that $w_L$ decreases in $\qmg$.

\paragraph{Comparative statics for $h$.} If $\frac{\Gamma}{h}>1$ such that $\underline{q}=\frac{\Gamma}{h}$, then the partial derivative of $w_L$ with respect to $h$ is
\begin{equation}
...
\end{equation}
which is positive since from Lemma \ref{cor:comp_stat} we have that $\frac{\partial \qmg}{\partial h}>0$.
...
\paragraph{Comparative statics for $\Gamma$.} If $\underline{q}=\frac{\Gamma}{h}$ (i.e., if $\frac{\Gamma}{h}>1$), then the partial derivative of $w_L$ with respect to $\Gamma$ is
\begin{equation}\label{eq:partial_gamma}
    \frac{\partial w_L}{\partial \Gamma}=\frac{\Gamma  \frac{\partial \qmg}{\partial \Gamma} \left(\log \left(\frac{\Gamma }{h}\right)-\log (Q)\right)+\qmg (\log (Q)-\log (\qmg)))}{\Gamma  \qmg \left(\log (Q)-\log \left(\frac{\Gamma }{h}\right)\right)^2}.
\end{equation}
From equation \eqref{eq:mg_eq}, $\qmg$ is linear in $\Gamma$ and therefore $\qmg=\Gamma\frac{\partial \qmg}{\partial \Gamma}$. We can rewrite \eqref{eq:partial_gamma} as
\begin{equation}
    \frac{\partial w_L}{\partial \Gamma}=-\frac{\log\left(\qmg\right) - \log \left(\frac{\Gamma }{h}\right)}{\Gamma \left(\log \left(Q\right)-\log \left(\frac{\Gamma }{h}\right)\right)^2 }<0.
\end{equation}
Further, if $\underline{q}=1$, then it is easy to see that $w_L$ decreases in $\Gamma$ since $\qmg$ increases with $\Gamma$ and the denominator is not a function of $\Gamma$.

% \paragraph{Comparative statics for $Q$.} The partial derivative of $w_L$ with respect to $Q$ is
% \begin{equation}
...
\begin{proof}
The two-pool implementation shortfall is
\begin{equation}
    \text{IS}\left(\left\{h,\ell\right\}\right)=\ell \int_{\qmg}^Q q_i \varphi\left(q_i\right) \diff i + h \int_{\frac{\Gamma}{h}}^{\qmg}  q_i \varphi\left(q_i\right) \diff i + g\left(\Theta - \int_{\frac{\Gamma}{h}}^{Q}  q_i \varphi\left(q_i\right) \diff i\right).
\end{equation}
We set $h=f$ such that the marginal $\LP$ entering the market is the same as in the single-fee pool, which equates the opportunity cost IS component across the two setups. It follows that:
\begin{equation}
    \text{IS}\left(\left\{f,\ell\right\}\right)-\text{IS}\left(\left\{f\right\}\right)=\left(\ell-f\right) \int_{\qmg}^Q q_i \varphi\left(q_i\right) \diff i \leq 0,
\end{equation}
with strict inequality if $\qmg<Q$ such that the low-fee pool attracts a positive mass of $\LP$s.\end{proof}