\noindent\textbf{\large Lemma \ref{lem:sort}}
\begin{proof}
Suppose that $\LP$s with $q_i\leq\qmg$ post liquidity on the low-fee pool and all $\LP$s with $q_i>\qmg$ choose the high-fee pool. For an $\LP$ with endowment $\tilde{q}<\qmg$, submitting liquidity to pool $L$ yields a lower expected revenue since
\begin{equation*}
...
If the coefficient of $q_i$ in \eqref{eq:pi_diff_v2} is negative, that is $h \times \left(\frac{\qmg}{Q}\right)^{\frac{\lambda}{\theta}\frac{Q}{Q-1}} -\left(h-l\right)\leq 0$, then $\pi_\ell-\pi_h<0$ and all $\LP$s optimally choose to post liquidity on pool $H$. A necessary condition for a fragmented equilibrium is therefore that
