...
\subsection{Discussion \label{sec:model_discussion}}
% \mz{Not 100\% sure where best to place this. A natural place is immediately after the model primitives, to convince a reader our assumptions are reasonable.}

Our model is clearly stylized. First, we assume a linear bonding curve, meaning trades do not have price impact. In reality, decentralized exchanges like Uniswap V3 use convex bonding curves, such as ``constant product'' functions, for which larger trades generate higher price impact. Akin to how limit order markets operate, the price impact of a trade decreases in the liquidity available on high- and low-fee pools, and can therefore vary over time. Second, a linear bonding curve implies that all trades execute at the same price, precluding the need to model the market makers' choice of a price range for providing concentrated liquidity.

Our assumptions significantly streamline the analysis, enabling us to highlight the key economic trade-offs related to the cost structure of liquidity supply. In practice, small orders in both low- and high-fee markets typically have a minimal price impact. Therefore, we assume without loss of generality that small orders are routed to the low fee pool. Large orders, which have a more substantial price impact, are strategically routed in practice across high- and low-fee pools for optimal execution. Figure \ref{fig:routing} offer empirical support for this claim. Using liquidity snapshot data from Kaiko, we compute the optimal routing strategy for trades of different sizes. We show that while small trades are almost fully routed to low-fee pools, large trades are optimally split between low- and high-fee pools.

% Figure environment removed

Large orders in our model also trade on both pools. For simplicity, we assume they are routed sequentially across markets: first on the low- and then on the high-fee pool. Further, we assume that the large trader depletes liquidity in the high fee pool. This assumption mirrors the modelling approach in \citet{foucault2013liquidity}, where a limit order book cycles between ``empty'' and ``full'' states.  
The economic mechanism we aim to illustrate is that due to the routing of liquidity demand across pools, market makers in high-fee pools need to adjust their liquidity less frequently than those in low-fee pools, resulting in reduced fixed costs over time.

We streamlined liquidity demand in our model to better focus on the effect of fixed and variable costs on liquidity supply. A richer model with realistic price impact functions would need to account for the optimal order splitting across pools with different liquidity fees. This would add mathematical complexity to the model without necessarily bringing new insight on the economics of liquidity supply. Similar to the theoretical literature on limit order markets \citep{Parlour1998,Foucault1999,rocsu2009dynamic}, restrictive assumptions are required to obtain analytical solutions.  Our approach ensures a tractable framework that produces clear predictions to direct our empirical analysis. 
