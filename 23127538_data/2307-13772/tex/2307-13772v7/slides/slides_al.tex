...
\smallskip
\item[\textbullet] Fragmented liquidity can be beneficial
\end{itemize}
\bigskip
...
\item[\textbullet] Transaction prices automatically calculated as a function of liquidity demand and supply.
...
\medskip
\item From any single fee pool, you can find a fragmented pool that improves the large trader's shortfall
\item Set $f=h$, and introduce a lower fee pool.
\end{itemize}
...
\begin{frame}{$\Uparrow$ gas $\Uparrow$ cost of liquidity management:  Smaller LPs switch to $h$ fee pool}
    \begin{center}
        % Figure removed
    \end{center}
\end{frame}
