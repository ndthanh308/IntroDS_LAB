...
\item[\textbullet] Transaction price is automatic function of liquidity demand and supply.
...
\bigskip
\item Traditionally, a spread compensates market makers for adverse selection (``insider trading")
...
\begin{block}{Full participation benchmark}
    \begin{itemize}
        \item \textbf{LP} $i$ participates if $q_if>\Gamma$, or fee greater than fixed cost.
        \item Maximal {\bf LP} participation if fee=fixed cost.
        \item Difficult to implement; also costly for liquidity takers
    \end{itemize}
\end{block}

\begin{block}{Implementation shortfall benchmark \citep{perold1988}}
\begin{minipage}{0.55\textwidth}%
\begin{itemize}
...
\begin{frame}{Fragmented liquidity leads to lower implementation shortfall}

\begin{block}{Proposition}
From any single fee pool, you can find a fragmented pool that reduces the large trader's implementation shortfall.
\end{block}
