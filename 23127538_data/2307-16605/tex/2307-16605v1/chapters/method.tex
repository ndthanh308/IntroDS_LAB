...
\end{align}
where $\mathcal{L}_{CD}$ denotes the $\ell_1$ Chamfer Distance (CD)~\citep{ChamferDistance17} as geometry disparity loss, $\mathcal{U}$ is the uniform distribution.
...
\begin{align}\label{eq:dVAE}
    \!\!\!\sum_{(z_i, \tilde{z}_i)\in\mathcal{D}}\!\log \mathrm{P}(x_i|\tilde{x}_i)\geq\!\!\!
    ...
    D_{\mathrm{KL}}
    \big[
    \mathrm{Q}_{\phi}(\mathbf{z}|x_i), \mathrm{P}_{\theta} (\mathbf{z}|\hat{z_i})
    \big]
    \Big),
\end{align}
where (1) $\mathrm{Q}_{\phi}(z|x)$ denotes the discrete semantic tokenizer; (2) $\mathrm{P}_{\psi}(p|z)$ is the tokenizer decoder to recover origin data; (3) $\hat{z_i}=\mathrm{Q}_{\phi}(\mathbf{z}|\tilde{x}_i)$ denotes the masked semantic tokens from masked data; (4) $\mathrm{P}_{\theta}(z|\hat{z_i})$ reconstructs masked tokens in an autoencoding fashion. 
In the following sections, we extend the mask generative Transformer to voxel and point representations, respectively.
...
Point clouds are unordered that cannot be divided into regular grids like pixels or voxels. 
