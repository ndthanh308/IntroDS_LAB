...
We begin with an informal discussion of scaling rules and motivate the existence of an exponential scaling rule for momentum values of model \glspl{ema}.
...
Let us now formally introduce the \gls{ema} Update (\Cref{def:ema}).
\begin{definition}[EMA Update]
\label{def:emaUpdateDefinition}
     The \gls{ema} update for the model \gls{ema} $\rvzeta_t$ following a target model $\rvtheta_t$ at iteration $t$ with momentum $\rho\equiv1-\beta_\rho$ is
    \label{def:ema}
    \begin{equation}
    \rvzeta_{t+1}
    =
...
\end{bmatrix}.
\end{align}
The first row is the \gls{sgd} Scaling Rule (\Cref{def:lsr}).
...
$\mathcal{O}(\eta \times \mathcal  \beta_\rho)$, although its form may be different for different optimizers.
...
\glspl{sde} are a tool typically use to obtain scaling rules from first principles~\citep{DBLP:conf/nips/LiMA21,DBLP:conf/nips/MalladiLPA22}.
...
\begin{align}
    ...
    \end{split}
\end{align}
The SDE~\eqref{eq:sde-sgd} is an approximation of the discrete iterations~\eqref{eq:iterations} when the learning rate $\eta$ goes to zero.
An easy way to see this is that an Euler-Maruyama discretization of the SDE with learning rate $\eta$ exactly recovers the discrete iterations.
...
\begin{theorem}[SDE for SGD + EMA; informal see~\Cref{thm:app:sde}]
    ...
    \begin{equation}
        \max_{k=0,\dots, \lfloor T /\eta \rfloor} |\mathbb{E}[g(\Theta_{\eta k}, Z_{\eta k})] - \mathbb{E}[g(\rvtheta_k, \rvzeta_k)]| \leq c\times  \eta \enspace.
    \end{equation}
...
\begin{corollary}[Validity of the EMA Scaling Rule]
     ...
     Then, for any time horizon $T >0$ and function $g\in G^2$, there exists a constant $c>0$ independent of $\eta$ such that 
    \begin{equation}
        \max_{k=0,\dots, \lfloor T /\eta \rfloor} |\mathbb{E}[g(\rvtheta_{\lfloor k / \kappa \rfloor}^{(\kappa B)}, \rvzeta_{\lfloor k / \kappa \rfloor}^{(\kappa B)})] - \mathbb{E}[g(\rvtheta_k^{(B)}, \rvzeta_k^{(B)})]| \leq c\times  \eta \enspace.
    \end{equation}
...
\citet{DBLP:conf/nips/MalladiLPA22} study this without an \gls{ema} and characterize how the hyperparameters change with the noise scale.
