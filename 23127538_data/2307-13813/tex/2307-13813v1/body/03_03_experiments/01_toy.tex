...
We begin by showing the \gls{ema} Scaling Rule is required to match parameter trajectories in a simple setting.
Consider the optimization of $\rtheta$ in a \emph{noisy parabola} at the origin:
\begin{align}
    \Ls(\rtheta)
    &=\frac a2\,\rtheta^2,
...
Let $\rtheta\in\R$ be optimized with \gls{sgd} (\Cref{def:sgd}) and let there be a Polyak-Ruppert average (\Cref{def:polyak-ruppert-average}) $\rzeta\in\R$ with momentum $\rho=1-\beta$ for $\rtheta$.
