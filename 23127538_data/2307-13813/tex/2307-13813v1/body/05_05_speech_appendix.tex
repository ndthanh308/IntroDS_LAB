...
\ifthenelse{\equal{\anonymous}{0}}{{\bf Compute}~~~We were running all experiments on A100 80GB 8GPU nodes with bfloat16 precision training. While for supervised training evaluation of different EMA decay values is done in parallel during a single run, for pseudo-labeling every EMA decay value needs separate training. Final models training took 4.6k GPU hours for supervised learning and 11.4k GPU hours for pseudo-labeling.
}{\compute}
