...
\begin{definition}[Stop Gradient/$\text{StopGrad}(\,\cdot\,)$] 
    ...
    \end{equation}
    \label{def:stop-grad}
    for all parametric functions $h$ and $f$ and for all parameters $\rvtheta$ and $\rvomega$.
\end{definition}
...
\subsection{A ResNet-18 recipe for BYOL}
...
MoCo-v3 \citep{DBLP:conf/iccv/ChenXH21} enables the use of \gls{vit}s with contrastive learning, but achieves this through modifications of the \gls{vit} training procedures, including gradient freezing on the image patching layer, and re-introducing Batch Normalization to post-attention MLP layers.
...
and using either LayerNorm or \emph{no normalization} significantly decreases model performance.
