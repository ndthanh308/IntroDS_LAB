...
We now turn to real-world classification.
Here, the target model $\rvtheta$ optimizes a parametric log-likelihood, 
$\max_{\rvtheta} \log p(\rvy | \rvx; \rvtheta)$ 
with labels $\rvy$ and inputs $\rvx$ drawn from the joint distribution $p(\rvy, \rvx)$.
...
on ImageNet1k \citep{DBLP:journals/corr/RussakovskyDSKSMHKKBBF14} (\Cref{fig:r50-polyak}) using a three stage step learning rate schedule. 
...
We also observe
a growing gap between the baseline and \gls{ema}-scaled trajectories as we increase~$\kappa$. Inspecting the train loss leads us to conclude this is due to a breakdown of the Adam Scaling Rule.
\emph{In summary, ASR task shows that EMA Scaling Rule holds in practice for sequential data with {\bf dynamic batch sizes}, as well as when using adaptive optimization.}
