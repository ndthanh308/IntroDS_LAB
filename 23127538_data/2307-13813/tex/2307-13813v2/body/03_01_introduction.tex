...
Many modern \gls{ml} optimization methods incorporate a parameter \gls{ema}, which is functionally a copy of the \emph{target model}, except its parameters do not receive gradient updates and are instead a weighted moving average of the target model parameters \citep{Polyak92,Ruppert1988EfficientEF}.
These weights are updated through by a \emph{momentum} hyperparameter. 
This functional copy, which we call the \emph{model \gls{ema}}, has a number of desirable properties:
i) 
the model \gls{ema} inhabits wider minima than the target model, reducing overfitting and improving generalization
\citep{DBLP:conf/iclr/HuangLP0HW17,DBLP:conf/uai/IzmailovPGVW18,DBLP:conf/cvpr/HeCXLDG22};
ii)
...
Despite its significant role in optimization, a recipe for adapting model \glspl{ema} when changing batch size has,
...
\begin{enumerate}[leftmargin=0.75cm]
    ...
    \item We empirically validate the \gls{ema} Scaling Rule, first in a synthetic setting (\Cref{subsec:toy-experiment}), 
    followed by practical ones where the model \gls{ema} plays an increasingly significant role in optimization: 
    i) 
    ...
    and iii)
    \gls{ssl}, which is the same as the semi-supervised case, except there is no labeled data (\Cref{subsec:self-supervised}).
    \item We observe that pseudo-labeling and \gls{ssl} training dynamics during optimizer warm-up are not always able to replicate at large batch sizes using \emph{only} the \gls{ema} Scaling Rule.
    We propose and verify practical methods to overcome this limitation, enabling us to scale to a batch size of 24,576 with BYOL \glspl{vit}, enabling an ideal wall-clock training time reduction of 6$\times$, while maintaining performance of the batch size 4096 baseline.
\end{enumerate}
