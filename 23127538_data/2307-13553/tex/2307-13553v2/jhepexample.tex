...
\label{e21}
\end{equation}
where, $r_e$ is the characteristic radius and its logarithmic slope is $\text{d ln} \rho / \text{d ln r}=-2$, $\rho_e$ is the halo density and $\alpha$ is a shape parameter\footnote{The authors in Ref. \cite{Wang:2019ftp} fixed this shape parameter $\alpha$ at $\alpha=0.16$, but others are not \cite{Udrescu:2018hvl,Quintana:2022yky,Figueiredo:2023gas,Rahaman:2023tkm}. Therefore, in our work, we reserve $\alpha$ as a free parameter to participate in the calculation later.}. Now, based on the Eq. (\ref{e21}), the mass distribution of the dark matter halo can be given by the following form,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where, $f(r)$ is the redshift function and $g(r)$ is the shape function. Here, we mainly consider the simpler case in the spherical symmetric spacetime, that is, $f(r)=g(r)$. The relationship between the redshift function $f(r)$ and the tangential velocity $V$ is as follows \cite{Matos:2003nb},
\begin{equation}
V^2=\frac{r}{\sqrt{f(r)}}\frac{d\sqrt{f(r)}}{dr}=r\frac{dln\sqrt{f(r)}}{dr}.
\label{e25}
\end{equation}
According to Eqs. (\ref{e23}) and (\ref{e25}), we can get an analytical solution of the metric coefficient $f(r)$ in a dark matter halo, and it has the following form,
\begin{equation}
...
\end{aligned}
\end{equation}
Eq. (\ref{e35}) is the energy-momentum tensors that can be described by two different metrics. The first equation of Eq. (\ref{e35}) is only related to function $G1$. The second equation is related to functions $G1,F1$ and it can be simplified to the following form
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where, $M_\text{BH}$ is the mass of the black hole. Here, using the case of $f(r)=g(r)$ we assumed before in Eq. (\ref{e35}), it is not difficult to find that $F1(r)=G1(r)=-2M_\text{BH}/r$. Therefore, the new metric for the Schwarzschild-like black hole in a dark matter halo is given by the following form,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where, $F(r)=f(r)+F1(r), G(r)=g(r)+G1(r), H(r)=r^{2}$. The terms $f(r)$ and $g(r)$ represent the factor terms for considering pure dark matter halo in Eq. (\ref{e26}). Finally, the metric coefficients for the Schwarzschild-like black hole in a dark matter halo are as follows
\begin{equation}
...
\label{e42}
\end{equation}
where, $F(r), G(r), H(r)=r^2$ are the metric functions. In the null tetrad, the inverse metric of Schwarzschild-like black hole can be composed of basis vectors $l^{\mu},n^{\mu},m^{\mu},\bar{m}^{\mu}$, and it has the following form,
\begin{equation}
g^{\mu\nu}=-l^{\mu}n^{\nu}-l^{\nu}n^{\mu}+m^{\mu}\bar{m}^{\nu}+m^{\nu}\bar{m}^{\mu},
\label{e43}
\end{equation}
...
\end{aligned}
\end{equation}
where, $\overline{m'^\mu}$ means the complex conjugate of $m'^\mu$, $\Sigma_1 =r-ia\cos \theta$ and $\Sigma_2=r + ia\cos \theta$. Taking the results of Eq. (\ref{e46}) into the equation of new contravariant metric,
\begin{equation}
\begin{aligned}
g'^{\mu\nu}=-l'^{\mu}n'^{\nu}-l'^{\nu}n'^{\mu}+m'^{\mu}\bar{m}'^{\nu}+m'^{\nu}\bar{m}'^{\mu}, 
\end{aligned}
...
\end{aligned}
\end{equation}
where, $\Sigma=\Sigma_1\Sigma_2=r^2+a^2\cos^2\theta$. Based on these inverse metric and then taking the inverse of Eq. (\ref{e47}), the new metric of the Kerr-like black hole in EF coordinates has the following form,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where, $G, H, K$ are the function of $r$ we introduced before and $\Sigma=r^2+a^2\cos^2\theta$. With the case $F(r)=G(r), H(r)=r^2$ we introduced previous section, we find that $K(r)=H(r)=r^2$. Finally, the Eq. (\ref{e411}) can be written as Kerr-like form,
\begin{equation}
\begin{aligned}
...
\label{e514}
\end{equation}
where, $m$ is the mass of the particle (here the photon takes $m=0$). Taking Eqs. (\ref{e511}) and (\ref{e514}) into account, the Eq. (\ref{e513}) can be divided into the following two equations,
that is
\begin{equation}
...
\end{aligned}
\end{equation}
where, the function $ \Delta$ can be obtained in Eq. (\ref{e512}). For a circular photon orbit, it satisfies the following form,
\begin{equation}
\begin{aligned}
V_\text{eff}(r)\mid _{r=r_c}=0, \quad \frac{\partial V_\text{eff}(r)}{\partial r}\mid _{r=r_c}=0,
\label{e519}
\end{aligned}
\end{equation}
where, $r_c$ is the orbital radius of photon sphere and then we can obtain the following form,
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where, $\Delta '(r)$ represents the derivative of $\Delta$ with respect to $r$. Taking Eq. (\ref{e520}) into the third equation of Eq. (\ref{ne520}) can be obtained
\begin{equation}
V_{\text{eff}}''(r)=8 E^2 \left(r^2+\frac{2 r\Delta (r) \left(\Delta '(r)-r \Delta ''(r)\right)}{\Delta'(r)^2}\right) \mid _{r=r_c},
\label{e521}
\end{equation}
In particular, if $V_\text{eff}''(r_c)>0$, the radial equation corresponding to the lightlike geodesics is unstable, but $V_\text{eff}''(r_c)<0$ is stable. The unstable orbit of the photon will determine the shape of shadow of the black hole. The range of the photon regions $r_c$ can be obtained by the equation $\Theta \geq 0$ with case of $\theta=\pi/2$, and then taking Eq. (\ref{e520}) into the last equation of Eq. (\ref{e518}) and then we can obtain the following form
\begin{equation}
\begin{aligned}
%\Theta = E^2\left (\eta -\lambda ^2\cot^2\theta +a^2\cos^2\theta \right ) &\geq 0,\\
%\frac{E^2 r^2(-16\Delta(r)^2-r^2\Delta'(r)^2+8\Delta(r)(2a^2+r\Delta'(r)) \mid _{r=r_c}}{a^2\Delta'(r)^2} &\geq 0,\\
16\Delta(r)^2 + r^2\Delta'(r)^2 - 8\Delta(r)(2a^2+r\Delta'(r)) \mid _{r=r_c} &\leq 0,
\label{e522}
\end{aligned}
\end{equation}
From Eq. (\ref{e522}), once the parameters of the black hole are determined, the range of photon region $r_c$ may be $r_{\text{cmin}}\leq r_c \leq r_{\text{cmax}}$. Here, $r_{\text{cmin}}, r_{\text{cmax}}$ are the minimum and maximum positions of the photon regions, respectively. It can be seen that there are maximum and minimum values of the photon sphere.
...
\acknowledgments
%We would like to acknowledge the anonymous referees for the constructive reports, which significantly improved this paper. 
