...
\begin{abstract}{The principal paradigm for determining the thermoelectric properties of materials is based on the Boltzmann transport equations (BTEs) or Landauer equivalent. These equations depend on the electron and phonon density of states (e-DOS and p-DOS) derived from $ab \; intio$ calculations performed using density functional theory and density functional perturbation theory. Recent computational advances have enabled consideration of phonon-phonon and electron-phonon interactions in these calculations. Leveraging these density of states, the single species BTE or Landauer equivalent can ascertain key thermoelectric properties. This, however, overlooks the intrinsic electron-phonon coupling. Despite the substantial computational burden, the BTE paradigm has successfully incorporated this coupling, yielding excellent agreement with experimental observations. Nonetheless, to alleviate the computational burden, an alternative paradigm is proposed: the steepest-entropy-ascent quantum thermodynamic (SEAQT) framework. This framework offers an equation of motion (EOM) for the evolution of non-equilibrium system states. Based on a gradient dynamics of entropy in state  space, the SEAQT EOM inherently satisfies both the postulates of quantum mechanics and thermodyanmics. By incorporating both e-DOS and p-DOS data as input, this EOM accurately computes the transport properties of materials, accounting for electron-phonon coupling. Importantly, it achieves this at a significantly reduced computational cost and can do so across multiple spatial and temporal scales in a single analysis. A succinct overview of this framework and its EOM with comparisons to measured data of its predictions for the transport properties of Si and doped Si is given.}
\end{abstract}
...
Results are presented for the electrical and thermal conductivities, the Seebeck coefficient, and the figure of merit for both Si and doped Si. In addition, supplementary materials demonstrate SEAQT's ability to predict the properties of anisotropic materials, as evidenced by the results obtained for Bi$_2$Te$_3$ given in the \textit{Supplementary Materials}.
