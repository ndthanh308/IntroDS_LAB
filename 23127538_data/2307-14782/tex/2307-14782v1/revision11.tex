...
and several key properties such as  gonality of a general element of any component (Proposition \ref{e2} and Remark \ref{e3}) as well as identifying all smooth elements of each components ; e.g. Proposition \ref{b1} and Remark 
\ref{e31}.
...
\item[(ii)] Let $\xi$ be the natural functorial map $\xi: \h{H}_{15,14,5}\rightarrow\h{M}_{14}$. Let $\Gamma\subset  \h{H}_{15,14,5}$ be and irreducible component. Let  $\h{G}_\Gamma$ be the  irreducible family consisting of pairs 
...
%Usually we will work with, perhaps singular, curves in $\PP^3$ and we consider the set of all dual curves $u_X(X)$. ß†
...
\item[(ii)]
Fix an integer $g$ such that $0\le g\le q$. Let $\Sigma_g$ be the locus of all integral curves $Y\in |L|$ with geometric genus $g$. Let $\w{\Sigma}_g$ be the locus of all nodal $Y\in \Sigma_g$ with $\delta:=q-g$ nodes as its only singularities. By \cite[Cor. 4.4]{dedse} a general member of every irreducible component of  $\Sigma_g$ is nodal curve. For a fixed $p\in\mathbb{F}_e$, being nodal at $p$ imposes three 
...
Let $S$ be a smooth Del Pezzo surface. A line bundle on $S$ is spanned (resp. very ample) if and only if it is nef (resp. ample) (\cite[Cor. 4.7]{sandra}).  $T\subset S$ an integral projective curve such that $T^2>0$. Since $T$ is integral and $T^2>0$, $T\cdot D\ge 0$ for all curves $D$. Thus the line bundle $\Oo_S(T)$ is nef and hence it is spanned. Thus a general element of $|T|$ is smooth by the Bertini theorem.
\end{remark}
...
has very ample  residual series $|K_{\tilde{C}}(-1)|$; if this the case, we then may conclude
...
Choose a node $q_0\in C$ and blow $Q$ at $q_0$ - which we again denote by $Q_{q_0}$ - and then blow up successively at the remaining three 
...
Choice of a triple covering $X$ over  a fixed $E$ depends on $2g-3$ parameters by Riemann's moduli count; \cite [Theorem 8.23, p828]{ACGH2}.
Choice of degree two effective divisors $B\in X_2$ depends on at most one 
parameters; note that $|\pi^*(\h{O}_E(1))+B|=g^2_{11}$ for general $B\in X_2$.
...
\quad (a) Supose $K_X(-1)=g^3_{11}$ is very ample. Recall that a complete linear series $g^r_d$ on a $4$-gonal curve $C$ is of type 1 (type 2, resp.) if $g^r_d$ is composed of $g^1_4$ (if the residual series $|K_C-g^r_d|$ is composed of $g^1_4$, resp.) according to Coppens-Martens \cite{cm}. In the current situation, the linear series $K_X(-1)$ is neither type 1 nor type 2 and hence by \cite[Th. 1.9]{cm}, $K_X(-1)=g^3_{11}$ is of the form $|2g^1_4+F|$ with $\deg (F)=3$ i.e. there is $m\in \{0,2\}$
...
\quad (b) Recall $p_aC)\le 15$ by the assumption that $C$ is not contained in a quadric. Now assume $p_a(C)=15$. We exclude the case $g^3_{11} =|2g^1_4+F|$. Since $p_a(C)=15$ and $g=14$, $C$ can  have only one node or only one simple cusp (a double point). On the other hand,
...
$|\Ii_G(2,2)| = |\Ii_{G\cup \{p\}}(2,2)|$. Since $G$ is general, no two points among $G$ lie on on a line in $Q$. For any $E\subset G$ such that $\#E=3$ the non-empty linear system $|\Ii_E(1,1)|$  has a unique element $C_E$ which  is smooth.
...
Note that $C\cap J$ is the base locus of $\Oo_C(1)(-E)$. Since $E =\phi^{-1}(E_1)$ and $J_1\cap C_1$ is the base locus of $|\Oo_{C_1}(1)(-E_1)|$ and  $\phi^{-1}(J_1\cap C_1) = J\cap C$. Thus $$\phi^{-1}((J_1\cap C_1)+E_1)) = (J\cap C)+E\in |\Oo_C(1)|$$ and we have $\phi^\ast (\Oo_{C_1}(1)) \cong \Oo_C(1)$.
\end{proof}
...
J. Pure Appl. Algebra \textbf{224} 1115--1123 (2020)
\bibitem{Beauville}
{A. Beauville},
...
\bibitem{Brevik}
{J. Brevik},
\textit{Curves on normal rational cubic surfaces}, Pacific J. Math. \textbf{230} no. 1, 73--105 (2007)
\bibitem{cm} M. Coppens and G. Martens, \textit{Linear series on 4-gonal curves}, Math. Nachr. \textbf{213}, 33--55 (2000)
\bibitem{dedse} T. Dedieu and E. Sernesi, \textit{Equigeneric and equisingular families of curves on surfaces}, Publ. Mat. \textbf{61}, 175--212 (2017)% it is Corollary 4.4 in the published version;
%DOI: 10.5565/PUBLMAT 61117 07
\bibitem{sandra}
{S. Di Rocco},
\textit{$k$-very ample line bundles on Del-Pezzo surfaces}.
Math. Nachr.  \textbf{179}, 47--56 (1996)
\bibitem{E1}
{L. Ein},
\textit{Hilbert scheme of smooth space curves}.
Ann. Scient. Ec. Norm. Sup. (4) \textbf{19} no. 4, 469--478 (1986)
\bibitem{E2}
{L. Ein},
{\it The irreducibility of the Hilbert scheme of complex space curves.}
Algebraic geometry, Bowdoin, 1985, Proc. Sympos. Pure Math. \textbf{46} Part 1, Providence, RI: Amer. Math. Soc. 83--87 (1985)
%\bibitem{EH}
...
\bibitem{gp1}
L. Gruson and C. Peskine, \textit{Genre des courbes de l’espace projectif. II}
Annales scientifiques de l’\'{E}.N.S. 4e s\'{e}rie, \textbf{15} no 3, 401--418 (1982)
\bibitem{gp2}
L. Gruson and C. Peskine,
\textit{Section plane d'une courbe gauche: postulation.(French)} In: Enumerative geometry and classical algebraic geometry,  33–35,  Nice--1981,
Progr. Math. \textbf{24},
Birkh\"{a}user, Boston, Mass. (1982)

\bibitem{he} J.~Harris, Curves in projective space, S\'eminaire de Math\'ematiques Sup\'erieures, vol.~\textbf{85}, Presses de
l'Universit\'e de Montr\'eal, Montreal, Que. (1982) 
\bibitem{hs} R. Hartshorne and E. Schlesinger, Gonality of a general ACM curve in $\PP^3$, Pacific J. Math. \textbf{251} no. 2, 269--313 (2011)
\bibitem{Hartshorne}
{R. Hartshorne},
...
\textit{On the irreducibility of the {H}ilbert scheme of curves in
              {$\Bbb P^5$.}} Comm. Algebra., \textbf{36}  no. 4, 1550--1564 (2008)
\bibitem{lengthy}
{C. Keem},
\textit{Existence and the reducibility of the Hilbert scheme of linearly normal
  curves in $\mathbb{P}^r$ of relatively high degrees,} J. Pure Appl. Algebra \textbf{227}, 1115--1123 (2023)
 % available at
%https://arxiv.org/abs/2101.00559.
\bibitem{speciality}
{C. Keem},
\textit{On the Hilbert scheme of  linearly normal curves in $\mathbb{P}^r$ with small index of speciality.} Indag. Math.(N.S.), \textbf{33}, 1102--1124 (2022)% available at 
...
\bibitem{ms} G. Martens and F.-O. Schreyer, Line bundles and sygygies of trigonal curves, Abh. Math. Sem. Univ. Hamburg \textbf{56}, 169--189 (1985)
\bibitem{Sakai} 
    {M, Ohkouchi and F. Sakai},
     \textit{The gonality of singular plane curves},
    {Tokyo J. Math.}
 % FJOURNAL = {Tokyo Journal of Mathematics},
    {\textbf{27}},
     {137--147} (2004)
\bibitem{ps}  C. Peskine and L. Szpiro, \textit{Liaison des vari\'{e}t\'{e}s alg\'{e}briques. I}, Invent. Math. \textbf{26}, 271--302 (1974) 
...
\bibitem{ty}
{I. Tyomkin},
\textit{On Severi varieties on Hirzebruch surfaces} 
Int. Math. Res. Not. \textbf{23} Art. ID rnm109  (2007)
