...
\label{eq:result_rate}
    B(f_{\x,\x'}^{\lambda^\ast})\leq O\left( (m+n)^{-\frac{2 r \alpha+\alpha}{2 r \alpha +\alpha+1}}\right)
\end{align}
for a minimizer $f_{\x,\x'}^{\lambda^\ast}$ of the empirical estimation Eq.~\eqref{eq:empirical_loss_objective} of Eq.~\eqref{eq:regularized_Bregman_objective}.
...
In particular, the error rate discovered in~\citep{kanamori2012statistical} for RKHS with \textit{bracketing entropy} $\gamma\in(0,2)$:
\begin{align}
...
\end{align}
with $r'$ originating from a source condition similar to our Assumption~\ref{ass:source_condition} but for the covariance operator $T=\mathbb{E}[K_x\otimes K_x]$ of $Q$ instead of $\rho$.
...
\begin{assumption}
    ...
    The function $\ell:\Y\times\mathbb{R}\to\mathbb{R}$ has an associated link function $\Psi:[0,1]\to\mathbb{R}$ such that
