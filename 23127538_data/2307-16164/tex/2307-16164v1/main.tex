...
\usepackage[a4paper,bindingoffset=15pt,%
...
\author[1]{Werner Zellinger}
\author[2]{Stefan Kindermann}
\author[1]{Sergei V.~Pereverzyev} 
\affil[1]{Johann Radon Institute for Computational and Applied Mathematics (RICAM), Austrian Academy of Science, Linz, Austria (\textit{werner.zellinger@oeaw.ac.at, sergei.pereverzyev@oeaw.ac.at}).}
\affil[2]{Industrial Mathematics Institute, Johannes Kepler University Linz, Austria.
(\textit{kindermann@indmath.uni-linz.ac.at}).}
\date{}
...
\noindent
    ...
    In the special case of quadratic loss, our method adaptively achieves a minimax optimal error rate.
    %our method adaptively achieves the best possible error rate.
...
plays a central role in statistics and machine learning~\cite{sugiyama2012density}; its applications include anomaly detection~\cite{smola2009relative,hido2011statistical}, two-sample testing~\cite{keziou2005test,kanamori2011f}, divergence estimation~\cite{nguyen2007estimating,nguyen2010estimating}, covariate shift adaptation~\cite{shimodaira2000improving,gizewski2022regularization,dinu2022aggregation}, generative modeling~\cite{mohamed2016learning}, conditional density estimation~\cite{schuster2020kernel}, and classification from
positive and unlabeled data~\cite{kato2019learning}.
Note, that this problem is different from supervised learning~\cite{Cucker2001OnTM} as the given data do not include point evaluations $\beta(x)$ from the desired target $\beta$.

It has been first observed by M.~Sugiyama, T.~Suzuki and T.~Kanamori in~\cite{sugiyama2012densitybregman} that many algorithms for density ratio estimation compute finite-sample approximations of the objective $\beta$ by discretizing the minimization problem
\begin{align}
...
~
    \begin{itemize}
        \item As can be seen from~\cite[Sec.~3.2.1]{sugiyama2012densitybregman}, the kernel unconstrained least-squares importance fitting procedure (KuLSIF)~\cite{kanamori2009least} uses Eq.~\eqref{eq:regularized_Bregman_objective} with $F(h)=\int_\X (h(x)-1)^2/2\diff Q(x)$ and $g=\mathrm{id}$, where $\mathrm{id}(f)=f$ is the identity (cf.~\cite[Eq.~(4)]{kanamori2012statistical}).
        This leads to a discretization of the minimization problem
        \begin{align}
            \label{eq:kulsif}
            \min_{f\in\mathcal{H}} \frac{1}{2} \norm{\beta-f}_{L^2(Q)}^2 + \frac{\lambda}{2} \norm{f}^2.
        \end{align}
        \item The Kullback-Leibler estimation procedure (KLest) introduced in~\cite[Eq.~(41)]{nguyen2010estimating} can originate from Eq.~\eqref{eq:regularized_Bregman_objective} with $F(h)=\int_\X h(x)\log(h(x))-h(x)\diff Q(x)$ and $g=\mathrm{id}$; see~\cite[Sec.~3.2.4]{sugiyama2012densitybregman} for the derivation of $F$, which is also used in~\cite{sugiyama2008directest}.
        \item The logistic regression approach (LR) employed in~\cite[Sec.~7]{bickel2009discriminative} can be obtained from Eq.~\eqref{eq:regularized_Bregman_objective} using $F(h)=\int_\X h(x)\log(h(x))-(1+h(x))\log(1+h(x))\diff Q(x)$ and $g(f)=e^{f}$ with $f(x)=\theta^\text{T} \varphi(x)$ for some $\varphi:\X\to\mathbb{R}^d$; see~\cite[Sec.~3.2.3]{sugiyama2012densitybregman}.
        % \item The power function family (Power) is suggested in~\cite[Sec.~3.4.1]{sugiyama2012densitybregman} with $\phi(t)=\frac{t^{1+\alpha}-t}{\alpha}$ for some $\alpha\in(0,1]$ and $g_f=f\in\mathcal{H}$.
        \item The exponential function approach (Exp) introduced in~\cite[Sec.~7]{menon2016linking} can be obtained using $F(h)=\int_\X h(x)^{-3/2}\diff Q(x)$ and $g(f)=e^{2 f}$; see~\cite[Table~1]{menon2016linking}.
        \item The squared hinge function (SH) is used in~\cite[Sec.~8,.2]{menon2016linking} with $F(h)=\int_X 1/(2 h(x) + 2)\diff Q(x)$ and $g(f)=\frac{-1+2 f}{2-2 f}$; see~\cite[Table~1]{menon2016linking}.
    \end{itemize}
\end{example}
To the best of our knowledge, no error bounds are known for estimating Eq.~\eqref{eq:bregman_identity_for_density_ratio} for the functionals in Example~\ref{ex:example_in_introduction} that take into account the regularity of the density ratio $\beta$, except~\cite[Sec.~3.1]{gizewski2022regularization} and~\cite{nguyen2023regularized} which hold only for quadratic $F$, i.e., KuLSIF.
Moreover, no method for choosing the regularization parameter $\lambda$ in Eq.~\eqref{eq:regularized_Bregman_objective} for non-quadratic $F$ is known that adapts to the regularity of $\beta$, i.e., a method with error bounds provably decreasing for increasing regularity.
...
Following~\cite{menon2016linking}, one can relate the minimization of the data fidelity term $B(f):=B_F(\beta,g(f))$ from Eq.~\eqref{eq:regularized_Bregman_objective}, to the minimization of the expected loss with a loss function $\ell:\{-1,1\}\times\mathbb{R}\to\mathbb{R}$ measuring by $\ell(-1,f(x))$ and $\ell(1,f(x))$, respectively, the error of a classifier $f(x)$ which predicts whether $x$ is drawn from $P$ or $Q$.%

Our first result (Example~\ref{ex:self-concordant_losses}) shows that all loss functions $\ell$ corresponding to Example~\ref{ex:example_in_introduction}, satisfy a property called \textit{generalized self-concordance}~\cite{marteau2019beyond}.
...
\label{eq:result_rate}
    B(f_{\x,\x'}^{\lambda^\ast})\leq C (m+n)^{-\frac{2 r \alpha+\alpha}{2 r \alpha +\alpha+1}}
\end{align}
for a minimizer $f_{\x,\x'}^{\lambda^\ast}$ of the empirical estimation Eq.~\eqref{eq:empirical_loss_objective} of Eq.~\eqref{eq:regularized_Bregman_objective} and a generic constant $C$ not depending on $m, n$.
...
In the special case of quadratic loss, as used in Eq.~\eqref{eq:kulsif}, the error rate in Eq.~\eqref{eq:result_rate} is minimax optimal in the same sense as it is minimax optimal in supervised learning, see~\cite{caponnetto2007optimal,blanchard2018optimal}.
...
Our third result (Eq.~\eqref{eq:balancing_principle_estimate}) is a new parameter choice method for $\lambda$ that does not need access to the regularity index $r$ but nevertheless achieves the same rate as in Eq.~\eqref{eq:result_rate} (see Theorem~\ref{thm:balancing_principle_empirical}).
...
Our parameter choice strategy achieves the error rate in Eq.~\eqref{eq:result_rate} and is thus optimal in the case of quadratic loss.
A numerical example illustrates our theoretical findings.

\subsection{Related Work}

The representation of density ratio estimation methods as minimizers of empirical estimations of Eq.~\eqref{eq:regularized_Bregman_objective} has been first observed in~\cite{sugiyama2012densitybregman} and later characterised in more detail in~\cite{menon2016linking}.
Our work extends~\cite{sugiyama2012densitybregman,menon2016linking} by taking into account the effect of the regularization term $\norm{f}^2$ in Eq.~\eqref{eq:regularized_Bregman_objective}.

In general, to the best of our knowledge, there are no error bounds similar to Eq.~\eqref{eq:result_rate} for a class of discretized minimization problems Eq.~\eqref{eq:regularized_Bregman_objective}.
For the special case of quadratic loss, i.e., KuLSIF in Eq.~\eqref{eq:kulsif}, error rates have been discovered in~\cite{kanamori2012statistical,gizewski2022regularization,nguyen2023regularized}.
In particular, the error rate discovered in~\cite{kanamori2012statistical} for RKHS with \textit{bracketing entropy} $\gamma\in(0,2)$:
\begin{align}
...
We can also mention~\cite[Remark~3]{gizewski2022regularization} which leads to the error bound
\begin{align}
...
\end{align}
Our result in Eq.~\eqref{eq:result_rate} improves Eq.~\eqref{eq:gizewski_fast_rate_intro} for $\alpha\in(1,\infty)$ and $r\in (0,\frac{1}{2}]$.
In the recent work~\cite{nguyen2023regularized}, Eq.~\eqref{eq:gizewski_fast_rate_intro} is improved by additionally taking into account the capacity of the space $\mathcal{H}$ in terms of a new pointwise smoothness condition on the reproducing kernel of $\mathcal{H}$ that allows to bound the regularized Christoffel function.
...
As a result, an error bound similar to our Eq.~\eqref{eq:result_rate} is obtained in~\cite{nguyen2023regularized}. However, the error is measured in the $\mathcal{H}$-norm and the bound holds only for the special case of quadratic loss.
...
% For example consider the fastest error rate of KuLSIF discovered in~\cite{kanamori2012statistical} for RKHS with \textit{bracketing entropy} $\gamma\in(0,2)$:
% \begin{align}
%     B(f_{\z}^\lambda)=O\!\left(\min\{m,n\}^{-\frac{2}{2+\gamma}}\right)
% \end{align}
% This rate is larger than the one in Eq.~\eqref{eq:result_rate}, whenever $\alpha (2 r+1)>\frac{2}{\gamma}$.
% We can also mention~\cite[Remark~3]{gizewski2022regularization} which leads to the error bound (cf.~Remark~\ref{remark:fast_rate})
% \begin{align}
%     \label{eq:gizewski_fast_rate_intro}
%     B(f_{\z}^\lambda)
%     % =O\!\left(\left(\phi(\theta_\phi^{-1}(m^{-\frac{1}{2}}+n^{-\frac{1}{2}}))\right)^2\theta_\phi^{-1}(m^{-\frac{1}{2}}+n^{-\frac{1}{2}})\right)
...
We can mention variants of cross-validation~\cite{kanamori2012statistical}, the quasi-optimality criterion~\cite{nguyen2023regularized}, and hints towards the use of balancing principles~\cite{gizewski2022regularization}, but, even in the special case of quadratic loss, we are not aware of any finite-sample bounds related to these approaches.

Some of our proof techniques rely on~\cite{marteau2019beyond}, where  self-concordant loss functions in the context of kernel learning were studied first.
Our analysis extends~\cite{marteau2019beyond} to density ratio estimation and adaptive parameter choice.
...
A.K.~Menon and C.S.~Ong discuss in~\cite{menon2016linking} properties of $F$ and $g$, which allow an intuitive estimation of Eq.~\eqref{eq:regularized_Bregman_objective} by discriminating i.i.d.~observations $\x=(x_i)_{i=1}^m$ from $P$ and $\x'=(x_i')_{i=1}^n$ from $Q$.
In the following, we will summarize these properties.

Let us start by introducing the underlying model: a probability measure $\rho$ on $\X\times\Y$, $\Y:=\{-1,1\}$, with conditional probability measures\footnote{Existence follows from $\X\times\Y$ being Polish (cf.~\cite[Theorem~10.2.1]{dudley2018real}, the product case).} defined by $\rho(x|y=-1):=P(x)$ and $\rho(x|y=1):=Q(x)$ and a marginal (w.r.t.~$y$) probability measure $\rho_\Y$ defined by a Bernoulli measure with probability $\frac{1}{2}$ for both events $y=-1$ and $y=1$.
...
\begin{assumption}[iid data]
    ...
    The data
    \begin{align}
    \z:=(x_i,-1)_{i=1}^m\cup(x_i',1)_{i=1}^m \in \X\times\Y   
    \end{align}
...
% Menon and Ong showed in~\cite{menon2016linking}, that all Bregman distances discussed in Table~\ref{tab:methods} can be equivalently written using the expected risk $\mathcal{R}(f)$.
In this work, we focus on \textit{strictly proper composite} loss functions, which are characterized by the following assumption.

\begin{assumption}[strictly proper composite loss]
    \label{ass:strictly_proper_composit_and_diff_bayes_risk}
    The loss function $\ell:\Y\times\mathbb{R}\to\mathbb{R}$ has an associated link function $\Psi:[0,1]\to\mathbb{R}$ such that
    ...
    \end{enumerate}
\end{assumption}
The main result of~\cite{menon2016linking} is the following representation of the data fidelity in Eq.~\eqref{eq:regularized_Bregman_objective} in terms of a loss function satisfying Assumption~\ref{ass:strictly_proper_composit_and_diff_bayes_risk}.
% \begin{lemma}[{\cite[Proposition~3]{menon2016linking}}]
...
\end{align}
for some link function $\Psi$ and the Bayes risk $G$ of a loss function $\ell$ satisfying Assumption~\ref{ass:strictly_proper_composit_and_diff_bayes_risk}. Then\begin{align}
    \label{eq:bregman_identity_for_density_ratio}
    B_F(\beta,g(f)) = 2 \left(\mathcal{R}(f)-\mathcal{R}(f^\ast)\right).
\end{align}
\end{lemma}
Indeed, for all approaches in Example~\ref{ex:example_in_introduction}, we can find $F$ and $g$ as required by Lemma~\ref{lemma:bregman_form_of_loss} (cf.~\cite[Table~1]{menon2016linking}).
We refer to~\cite{menon2016linking} for a recipe to construct new losses for Eq.~\eqref{eq:regularized_Bregman_objective} satisfying the requirements of Lemma~\ref{lemma:bregman_form_of_loss}.
\newpage
\begin{example}
\label{ex:associated_loss_functions}%
~
\begin{itemize}
    \item KuLSIF~\cite{kanamori2009least} uses $\ell(-1,v)=\frac{1}{2}v^2, \ell(1,v)=-v$ and $\Psi(v)=\frac{x}{1-x}$.
    \item KLest~\cite{nguyen2007estimating,nguyen2010estimating} uses $\ell(-1,v)=v, \ell(1,v)=-\log(v)$ and $\Psi(v)=\frac{x}{1-x}$.
    \item LR~\cite{bickel2009discriminative} uses $\ell(-1,v)=\log(1+e^v), \ell(1,v)=\log(1+e^{-v})$ and $\Psi(v)=-\log(\frac{1}{v}-1)$.
    % \item Power~\cite{sugiyama2012densitybregman} uses $\ell(-1,v)=\frac{v^{1+\alpha}-1}{1+\alpha}, \ell(1,v)=\frac{1-v^\alpha}{\alpha}, \alpha\in(0,1]$ and $\Psi(v)=\frac{x}{1-x}$.
    \item Exp~\cite{menon2016linking} uses $\ell(-1,v)=e^v, \ell(1,v)=e^{-v}$ and $\Psi(v)=-\frac{1}{2}\log(\frac{1}{v}-1)$.
    \item SH~\cite{menon2016linking} uses $\ell(-1,v)=(1+v)^2, \ell(1,v)=(1-v)^2$ and $\Psi(v)=2 v -1$. 
\end{itemize}
\end{example}
...
\end{align}
Throughout this work, we will denote a minimizer of Eq.~\eqref{eq:empirical_loss_objective} by $f_{\z}^{\lambda}$. The goal of this work is to analyse the error of estimating the density ratio $\beta$ by some $g(f_{\z}^{\lambda})$ in the data fidelity $B(f_{\z}^{\lambda})=B_F(\beta,g(f_{\z}^{\lambda}))$.
...
We will see in Section~\ref{sec:error_bounds} that these loss functions satisfy the following property when used with functions $f\in\mathcal{H}$ from an RKHS $\mathcal{H}$.%
\begin{assumption}[generalized self-concordance~\cite{marteau2019beyond}]
    ...
    \end{align}%
\end{assumption}%
The definition of self-concordance originates from the analysis of Newton's method~\cite{nesterov1994interior} and has been generalized in~\cite{10.1214/09-EJS521,marteau2019beyond}.
Before we review error bounds, we follow~\cite{marteau2019beyond} and make the following assumption.
...
\begin{enumerate}
    \item There exists $R\geq 0$ such that for any realization $z$ of the random variable $Z$, $\sup_{g\in\varphi(z)}\norm{g}\leq R$ almost surely.
...
The first point of Assumption~\ref{ass:boundedness_defin0_existence} is standard in learning theory~\cite{caponnetto2007optimal,steinwart2009optimal} and, together with point~2, assures that $\mathcal{R}(f)$ is well defined for all $f\in\mathcal{H}$.
...
Under these assumptions, Theorem~3 in~\cite{marteau2019beyond} holds, which we state in our notation as follows:
\begin{lemma}
[{\cite[Theorem~3]{marteau2019beyond}}]
\label{lemma:marteau_theorem3}
    ...
    \end{align*}
    Then, whenever $\lambda\leq B_2$ and
    \begin{align}
...
Such regularity can be encoded by so-called \textit{source conditions} as used in the regularization theory for inverse problems~\cite{engl1996regularization,bauer2007regularization,caponnetto2007optimal,steinwart2009optimal,rudi2017generalization,blanchard2018optimal}.%
\begin{assumption}[source condition]
    \label{ass:source_condition}
    It exists $r\in (0,\frac{1}{2}]$ and $v\in\mathcal{H}$ such that $f_{\mathcal{H}}=\Hess(f_{\mathcal{H}})^r v$ with the expected Hessian $\Hess(f):=\mathbb{E}[\nabla^2 \ell_Z(f)]$.
    % \begin{enumerate}
    ...
    % \end{enumerate}
\end{assumption}
In the case of quadratic loss, Assumption~\ref{ass:source_condition} recovers the polynomial source condition $f_{\mathcal{H}}=T^r v$ with covariance operator $T=\mathbb{E}[K_x\otimes K_x]$ and feature map $\Phi:\X\to\mathcal{H}$ of the RKHS $\mathcal{H}$.
...
It is well known in learning theory~\cite{zhang2002effective,caponnetto2005empirical,caponnetto2007optimal} that, the rate of convergence of regularized learning algorithms is not only influenced by the regularity of $f_\mathcal{H}$ but also by the \textit{capacity} of the space $\mathcal{H}$ measured by the so-called \textit{effective dimension}.
We therefore make the following assumption of~\cite[Assumption~7]{marteau2019beyond}, which generalizes~\cite[Definition~1]{caponnetto2007optimal}.%
\begin{assumption}[capacity condition]
    \label{ass:capacity_condition}
    There exist $\alpha\geq 1, Q\geq 0$ such that $\mathrm{df}_\lambda\leq Q \lambda^{-\frac{1}{\alpha}}$ for
    \begin{align}
        \mathrm{df}_\lambda:=\mathbb{E}\left[\norm{\Hess_\lambda(f_\mathcal{H})^{-\frac{1}{2}}\nabla\ell_Z(f_\mathcal{H})}^2\right]
    \end{align}
    with $\Hess_\lambda(f):=\Hess(f)+\lambda I$.
\end{assumption}
The term $\mathrm{df}_\lambda$ is called \textit{degrees of freedom}, appears as \textit{Fisher information} term in the asymptotic analysis of M-estimation~\cite{van2000asymptotic,lehmann2006theory}, appears similarly in spline smoothing~\cite{wahba1990spline}, and reduces to the \textit{effective dimension}~\cite{caponnetto2005empirical,caponnetto2007optimal} $\mathrm{df}_\lambda=\mathrm{Tr}(T T_\lambda^{-1})$ with $T_\lambda:=T+\lambda I$ for the quadratic loss.
In the latter case, Assumption~\ref{ass:capacity_condition} is equivalent to the condition $\sigma_j=O(j^{-\alpha})$ for a sequence $\sigma_1,\sigma_2,\ldots$ of non-increasing positive eigenvalues of $T$~\cite[Theorem~5]{guo2022capacity}.
That is, a bigger $\alpha$ induce that fewer eigenvectors are needed to approximate elements from the image space of $T$ with a given capacity.
On the other hand, from a regularization theory perspective,
Assumption~\ref{ass:capacity_condition} can be interpreted as specifying the \textit{ill-posedness} of the problem of inverting $T$ (cf.~\cite{blanchard2018optimal}).

Under Assumptions~\ref{ass:source_condition} and~\ref{ass:capacity_condition}, the following result is stated in~\cite{marteau2019beyond} and rephrased here in our notation.
See Subsection~\ref{subsec:proofs_balancing_principle_empirical} for more details on the derivation.%
% More precisely, it follows from their proofs of Thm.~28 (first inequality in Eq.~\eqref{eq:marteau_bound_theorem38}) and Thm.~38 (second inequality in Eq.~\eqref{eq:marteau_bound_theorem38}) and the fact that $\psi(\mathrm{t}_\lambda+\log 2)\leq 1$ since~\cite[Lemma~33]{marteau2019beyond} holds with $\max\{\mathrm{df}_\lambda, (Q^\ast)^2\}\leq (B_1)^2/\lambda$ (cf.~\cite[Proposition~18]{marteau2019beyond}) and $r>0$ implies~\cite[Eq.~(45)]{marteau2019beyond}.%
% is shown in Point 1) of the proof of Theorem~38 in~\cite{marteau2019beyond}(which applies since~\cite[Eq.~(38)]{marteau2019beyond} follows from $r>0$) in our notation.
\begin{lemma}[{\cite[Theorem~38]{marteau2019beyond}}]
    ...
    Whenever $0<\lambda\leq \min\{B_2^\ast,(2 L R \log\frac{2}{\delta})^{-1/r}, Q^{2\alpha} (B_2^\ast)^\alpha (B_1^\ast)^{-2 \alpha}\}$ and
    \begin{align}
        \label{eq:sample_size_condition_marteau_thm38}
        m+n\geq \max\left\{5184 \frac{B_2^\ast}{\lambda}\log\left(\frac{8 \cdot 414^2 B_2^\ast}{\lambda \delta}\right), \frac{1296 Q^2}{L^2 \lambda^{1+2 r+1/\alpha}} \right\},
    \end{align}
    ...
    \begin{align}
        ...
        \norm{f_{\z}^{\lambda}-f_{\mathcal{H}}}_{\Hess_\lambda(f_{\mathcal{H}})}^2\nonumber\\
        &\leq
        414 \frac{Q^2}{(m+n) \lambda^{1/\alpha}}\log\left(\frac{2}{\delta}\right) + 414 L^2 \lambda^{1+2 r}
        \label{eq:marteau_bound_theorem38}.
    \end{align}
\end{lemma}
In the special case of quadratic loss, it is possible to find $\lambda$ in
Lemma~\ref{lemma:marteau_main_result} that recovers the minimax optimal error rate discovered in~\cite{caponnetto2007optimal}; see~\cite[Corollary~39]{marteau2019beyond}.
However, \cite[Corollary~39]{marteau2019beyond} uses an a priori choice of $\lambda$ that depends on the regularity level $r$ and cannot be computed from the available samples.
...
The goal of this section is to analyse the error of a density ratio model $g(f_{\z}^{\lambda})$ with a minimizer $f_{\z}^{\lambda}$ of Eq.~\eqref{eq:empirical_loss_objective}, in terms of the data fidelity $B(f):=B_F(\beta, g(f))$ as specified in Eq.~\eqref{eq:Bregman_generator_and_density_ratio_model}.
...
~
    \begin{itemize}
    \item KuLSIF~\cite{kanamori2009least} uses the losses $\ell_{(x,-1)}(f)=\frac{1}{2}f(x)^2, \ell_{(x,1)}(f)=-f(x)$ which are three times differentiable and Eq.~\eqref{eq:self_concordant} holds with $\varphi((x,y))=\{0\}\subset\mathcal{H}$.
    \item\sloppy KLest~\cite{nguyen2007estimating,nguyen2010estimating} uses the losses $\ell_{(x,-1)}(f)=f(x), \ell_{(x,1)}(f)=-\log(f(x))$ which satisfy $\nabla^2 \ell_{(x,-1)}(f)=\nabla^3 \ell_{(x,-1)}(f)=0$ and $\nabla^3 \ell_{(x,1)}(f)=-2 f(x)^{-3} K_x^3=-2 \nabla^2 \ell_{(x,1)}(f) K_x$ for the unique $K_x\in\mathcal{H}$ such that $f(x)=\langle f, K_x\rangle_\mathcal{H}$. That is, self-concordance is satisfied with $\varphi(z)=\{-2 K_x\}$.
    \item LR~\cite{bickel2009discriminative} is self-concordant with $\varphi((x,y))=\{y K_x\}$, see~\cite[Example~2]{marteau2019beyond} and~\cite{bach2010self}.
    % \item Power~\cite{sugiyama2012densitybregman} ...
    \item Exp~\cite{menon2016linking} uses $\ell_{(x,y)}(f)=e^{y f(x)}$ which is self-concordant with $\varphi((x,y))=\{y K_x\}$.
    \item SH~\cite{menon2016linking} uses losses which are three times differentiable and Eq.~\eqref{eq:self_concordant} holds with $\varphi((x,y))=\{0\}\subset\mathcal{H}$.
\end{itemize}
\end{example}
...
As an intermediate result, we will provide first error rates for regularized losses beyond the quadratic loss, i.e., Eq.~\eqref{eq:kulsif}.
...
From Lemma~\ref{lemma:marteau_theorem3} we know that, for large enough sample sizes $m+n$, with probability larger than $1-2\delta$, a minimizer $f_{\z}^{\lambda}$ of Eq.~\eqref{eq:empirical_loss_objective}
...
% and $S(m+n,\delta,\lambda):=\frac{414 Q^2}{(m+n)\lambda^{1/\alpha}}\log\left(\frac{2}{\delta}\right)$, $A(\lambda):=414 L^2\lambda^{1+2 r}$ in case of Lemma~\ref{lemma:marteau_main_result}.
The question is: \textit{how to choose $\lambda$?}

One strategy in learning theory is to
use the value $\lambda^\ast$ which \textit{balances}~\cite{lepskii1991problem,goldenshluger2000adaptive,birge2001alternative,mathe2006lepskii,de2010adaptive,mucke2018adaptivity,blanchard2019lepskii,lu2020balancing,zellinger2021balancing}
\begin{align}
\label{eq:balance}
...
\begin{prop}
    ...
    Then there exist generic constants $C,c>0$ independent of $m,n,\delta$, such that the minimizer $f_{\z}^{\lambda^\ast}$ of Eq.~\eqref{eq:empirical_loss_objective} satisfies, for $m+n>c$, with probability at least $1-2 \delta$:
    \begin{align}
    ...
    \end{align}
\end{prop}
Explicit bounds on the constants $c, C$ are given in Eq.~\eqref{eq:sample_condition_a_priori_bound} and Eq.~\eqref{eq:exact_form_of_constant_a_priori_bound}, respectively.
...
\begin{remark}
    Together with Example~\ref{ex:self-concordant_losses}, our Proposition~\ref{prop:first_rate} provides (to the best of our knowledge) the first error rate for regularized density ratio estimation methods beyond quadratic data fidelity $B(f)$.
    At the same time, for the special case of quadratic data fidelity {$B(f_{\z}^\lambda)=\norm{\beta-f_{\z}^\lambda}_{L^2(Q)}^2$}, as used in KuLSIF, the error has been analyzed in~\cite{kanamori2012statistical} in terms of the so-called bracketing entropy $\gamma$ of $\mathcal{H}$.
    ...
    For $m\approx n$ as motivated by Assumption~\ref{ass:iid_of_classification_data}, our result therefore recovers the state of the art~\cite{kanamori2009least} for the special case of quadratic data fidelity and RKHS with bracketing entropy $\gamma\to 2$.
    In the same case, the rate has been recently improved in~\cite{gizewski2022regularization} for smooth $\beta$, which we discuss in the next Section~\ref{subsec:improved_error_bound}.
\end{remark}
...
\begin{prop}
    ...
    Let Assumptions~\ref{ass:iid_of_classification_data}--\ref{ass:capacity_condition} be satisfied, and let $\delta\in [\frac{2}{e^{1296}},\frac{1}{2}]$.
    Let further
    $\lambda^\ast$ be a solution of Eq.~\eqref{eq:balance} with $\eta:= 1296 \log^{-1}\!\left(\frac{2}{\delta}\right)$.
    Then there exist generic constant $C,c>0$ not depending on $m,n,\delta$, such that the minimizer $f_{\z}^{\lambda^\ast}$ of Eq.~\eqref{eq:empirical_loss_objective} satisfies, for $m+n>c$, with probability at least $1-2 \delta$:
    \begin{align}
...
\begin{remark}
    ...
    As a result, even in the special case of a quadratic functional $B(f_{\z}^\lambda)=\norm{\beta-f_{\z}^\lambda}_{L^2(Q)}^2$, Proposition~\ref{prop:fast_rate} allows to refine existing results.
    To see this, consider the fastest error rate of~\cite{kanamori2012statistical} discovered for RKHS with bracketing entropy $\gamma\in(0,2)$ (cf.~Remark~1):
    \begin{align}
        B(f_{\z}^\lambda)=O\!\left(\min\{m,n\}^{-\frac{2}{2+\gamma}}\right).
    \end{align}
    ...
    \begin{align}
        ...
        = O\!\left((m+n)^{-\frac{2r+1}{2r+2}}\right)
    \end{align}
    under the source condition in Assumption~\ref{ass:source_condition}.
    The rate in Eq.~\eqref{eq:gizewski_fast_rate} is slower than the one in Eq.~\eqref{eq:balancing_value_rate_fast} for $\alpha\in(1,\infty)$ and $r\in (0,\frac{1}{2}]$.
    In general, for quadratic data fidelity, the error rate in Eq.~\eqref{eq:balancing_value_rate_fast} is optimal in a minimax sense, see~\cite{caponnetto2007optimal,steinwart2009optimal,blanchard2018optimal}.
...
One solution to this problem is to estimate $\lambda^\ast$ with Lepskii's \textit{balancing principle}~\cite{lepskii1991problem,goldenshluger2000adaptive,birge2001alternative,mathe2006lepskii,de2010adaptive,lu2020balancing,zellinger2021balancing}.
However, no representation of $B(f_\z^\lambda)-B(f_\mathcal{H})$ as squared norm is available.
This prevents us from using the above referenced approaches directly, e.g., by using the norm estimates~\cite[Proposition~1]{de2010adaptive} or~\cite[Proposition~4.1]{lu2020balancing}. 
...
Since the estimation of $\norm{\cdot}_{\Hess_\lambda(f_\mathcal{H})}$ requires some care, we postpone it to the next Section~\ref{subsec:balancing_principle_empirical_risk}.
...
Namely, whenever $\lambda_j\leq\lambda_i\leq\lambda^\ast$, Lemma~\ref{lemma:marteau_main_result} and the monotonicity of $$S(m+n,\delta,\lambda):=\eta\cdot \frac{414 Q^2}{(m+n)\lambda^{1/\alpha}}\log\!\left(\frac{2}{\delta}\right) \quad \text{ and }  \quad A(\lambda):=414 L^2\lambda^{1+2 r},$$ with $\eta$ as in Proposition~\ref{prop:fast_rate}, grant us the inequality
\begin{align}
...
\end{align}
where we have used that $\Hess_{\lambda_j}(f)=\Hess(f)+\lambda_j I\leq \Hess(f)+\lambda_i I=\Hess_{\lambda_i}(f)$.
...
\begin{align}
    ...
    \lambda_{+}:=\max\!\left\{\lambda_i: \norm{f_\z^{\lambda_i}-f_\z^{\lambda_j}}_{\Hess_{\lambda_j}(f_\mathcal{H})}^2\leq 8 S(m+n,\delta,\lambda_j), j\in\{1,\ldots,i-1\}\right\}.
\end{align}
Importantly, the term $S(m+n,\delta,\lambda)$ does not depend on the unknown regularity index $r\in (0,\frac{1}{2}]$ but only on the quantities $m,n,Q,\alpha$ which are either given or an upper bound can be easily estimated from data.
...
\begin{thm}
    ...
    \end{align*}%
    for some $\xi>1$ and $\lambda_0\leq \xi^{-l} \min\left\{B_2^\ast,\left(2 L R \log\frac{2}{\delta}\right)^{-\frac{1}{r}}, Q^{2\alpha} (B_2^\ast)^\alpha (B_1^\ast)^{-2 \alpha}\right\}$.
    Then there exist generic constants $C,c>0$ not depending on $m,n,\delta$, such that the minimizer $f_{\z}^{\lambda_+}$ of Eq.~\eqref{eq:empirical_loss_objective} satisfies, for $m+n>c$, with probability at least $1-2 \delta$:
    \begin{align}
    ...
    \end{align}
\end{thm}
Exact bounds on the constants $c$ and $C$ are given in Eq.~\eqref{eq:sample_size_known_norm} and Eq.~\eqref{eq:const_in_known_risk_balancing_bound}, respectively.
...
The following concentration result is proven in Subsection~\ref{subsec:proofs_balancing_principle_empirical} and essentially combines analytical arguments for self-concordance losses~\cite{marteau2019beyond} with concentration inequalities for Hermitian operators~\cite{rudi2017generalization}.%
\begin{lemma}
    \label{lemma:Hessian_concentration}
    Let the assumptions of Lemma~\ref{lemma:marteau_main_result} be satisfied, denote the empirical Hessian by $\widehat{\Hess}(f):=\frac{1}{m+n}\sum_{z\in \z} \nabla^2 \ell_z(f)$, and let us write $\mathbf{B}\preccurlyeq\mathbf{A}$ iff $\mathbf{A}-\mathbf{B}$ is positive semi-definite.
...
\begin{align}
    ...
    \lambda_{\mathrm{BP}}:=\max\!\left\{\lambda_i: \norm{f_\z^{\lambda_i}-f_\z^{\lambda_j}}_{\widehat{\Hess}_{\lambda_j}(f_\z^{\lambda_j})}^2\leq 48 S(m+n,\delta,\lambda_j), j\in\{1,\ldots,i-1\}\right\}
\end{align}
which uses only given data or information for which upper bounds are easily estimable.
Moreover, the following error rates result holds.
\begin{thm}
    \label{thm:balancing_principle_empirical}
    Let the assumptions of Theorem~\ref{thm:balancing_principle_known_norm} be satisfied and $(\lambda_i)_{i=1}^l$ satisfy Assumption~\ref{ass:Hessian_of_sequence}.
    Then there exist generic constant $C,c>0$ not depending on $m,n,\delta$, such that the minimizer $f_{\z}^{\lambda_{\mathrm{BP}}}$ of Eq.~\eqref{eq:empirical_loss_objective} satisfies, for $m+n>c$, with probability at least $1-2 \delta$:
    \begin{align}
    ...
    \end{align}
\end{thm}
Exact bounds on $c$ and $C$ can be found in Eq.~\eqref{eq:sample_size_empirical_norm} and Eq.~\eqref{eq:const_in_empirical_risk_balancing_bound}, respectively.
...
\begin{remark}
    ...
    Even if similar adaptivity results hold for cross-validation in supervised learning~\cite{caponnetto2010cross} and the aggregation method in covariate shift domain adaptation~\cite{gizewski2022regularization}, their extension to density ratio estimation is not straight forward and an open future problem.
    In the special case of quadratic loss, e.g., for KuLSIF, the order of the error rate is optimal (see Remark~\ref{remark:fast_rate}),  and Eq.~\eqref{eq:balancing_principle_estimate} thus provides, in this sense, adaptivity at optimal rate.
\end{remark}
...
However, as already noted in~\cite[Remark~6.1]{lu2020balancing}, this choice is too rough for the low number of samples considered in practical applications.
We therefore follow~\cite[Remark~6.1]{lu2020balancing} and choose $M=M_j:=\mathrm{Tr}(\widehat{\Hess}_{\lambda_j}(f_\z^{\lambda_j}))^{-2}$.

The results of our experiment can be found in Figure~\ref{fig:lepskii}.
The choice $\lambda^\circ$ in Eq.~\eqref{eq:numerical_example_bp} corresponds to $\lambda=10^{-2}$ for KuLSIF in all three cases of dataset size $m+n=10,20,200$.
In case of the Exp method, it chooses $\lambda=10^{-1}$ for $m+n=10,20$ and $\lambda=10^{-2}$ for $m+n=200$.
...
The value $\lambda^\ast$ satisfying Eq.~\eqref{eq:balance} minimizes Eq.~\eqref{eq:bias_variance_simple} up to a constant factor (cf.~\cite[Sec.~4.3]{lu2013regularization}).
To see this, assume for simplicity that a minimizer $\lambda_1:=\argmin_{\lambda\geq 0} S(m+n,\delta,\lambda)+A(\lambda)$  exists. Then, either $\lambda^\ast<\lambda_1$, in which case
\begin{align*}
    A(\lambda^\ast)\leq A(\lambda_1)\leq \inf_{\lambda\geq 0}\left[ S(m+n,\delta,\lambda)+A(\lambda)\right],
\end{align*}
or $\lambda_1\leq \lambda^\ast$, and
\begin{align*}
    \eta S(m+n,\delta,\lambda^\ast)\leq \eta S(m+n,\delta,\lambda_1)\leq \eta \inf_{\lambda\geq 0} \left[ S(m+n,\delta,\lambda)+A(\lambda)\right],
\end{align*}
which gives, for large enough sample sizes and with high probability,
\begin{align}
\label{eq:optimality_of_exact_balancing_value}
    B(f_{\z}^{\lambda^\ast})-B(f_\mathcal{H})\leq 2 \inf_{\lambda\geq 0}  \left[ S(m+n,\delta,\lambda)+A(\lambda)\right]\leq 2 \eta \inf_{\lambda\geq 0}  \left[ S(m+n,\delta,\lambda)+A(\lambda)\right].
\end{align}
We will need the following technical lemma.
\begin{lemma}
    \label{lemma:technical}
    If $A,B\geq 0$ and $k\geq 2 A \log(2 A B)$, then $A\log(B k)\leq k$.
\end{lemma}
...
The following proof of Proposition~\ref{prop:first_rate} follows the essential steps of~\cite[Corollary~4]{marteau2019beyond} applied to Eq.~\eqref{eq:regularized_Bregman_objective} with explicit treatment of the balancing property Eq.~\eqref{eq:balance} of $\lambda^\ast$.
\begin{proof}[Proof of Proposition~\ref{prop:first_rate}]
    ...
    Then, under Assumptions~\ref{ass:generalized_self_concordance} and~\ref{ass:boundedness_defin0_existence}, Lemma~\ref{lemma:marteau_theorem3} gives, for large enough sample sizes $m+n$ (as specified later), with probability larger $1-2\delta$,
    \begin{align}
        % \nonumber
        B(f_{\z}^{\lambda})-B(f_\mathcal{H})&\leq S(m+n,\delta,\lambda)+A(\lambda)
        % \\
    ...
    \end{align}
    for $S(m+n,\delta,\lambda)=168 \frac{B_1^2}{\lambda (m+n)}\log(\frac{2}{\delta})$, $A(\lambda)=4\lambda \norm{f_\mathcal{H}}^2$.
    Solving
    \begin{align*}
        \eta \cdot S(m+n,\delta,\lambda^\ast) = A(\lambda^\ast)
    \end{align*}
    ...
    \label{eq:sample_condition_a_priori_bound}
        ...
        \frac{256 B_1^2 R^2}{B_2}\log\!\left(\frac{2}{\delta}\right),
        \frac{B_1^2 R^2\log\!\left(\frac{6 B_2^2}{4 \delta B_1^2 R^2}\log^{-1}\!\left(\frac{2}{\delta}\right)\right)}{9 B_2^2\log\!\left(\frac{2}{\delta}\right)}
    ...
    \begin{enumerate}
        \item The condition $\lambda^\ast\leq B_2$ is satisfied since $m+n\geq \frac{256 B_1^2 R^2}{B_2}\log\!\left(\frac{2}{\delta}\right)$.
        ...
        \label{eq:proof_first_bound_split1}
            m+n\geq 24\frac{B_2}{\lambda}\log\!\left(\frac{8 B_2}{\lambda \delta}\right)
            = 3 c (m+n)^{\frac{1}{2}}\log\!\left(\frac{1}{\delta} c (m+n)^{\frac{1}{2}}\right)
        \end{align}
        with $c:=\frac{8 B_2}{\lambda (m+n)^{\frac{1}{2}}}=\frac{B_2}{2 B_1 R}\log^{-\frac{1}{2}}\!\left(\frac{2}{\delta}\right)$.
        Eq.~\eqref{eq:proof_first_bound_split1} is guaranteed by Lemma~\ref{lemma:technical} (with $k:=(m+n)^{\frac{1}{2}}$, $A:=3 c$ and $B:=\frac{1}{\delta} c$), whenever $2 A\log(2 A B)\leq k$. That is, whenever $m+n$ is larger than
        \begin{align*}
            36 c^2 \log^2\left(\frac{6 c^2}{\delta}\right)
    ...
    \end{enumerate}
    Evaluating Eq.~\eqref{eq:bias_variance_simple_proof} at $\lambda^\ast$ as defined in Eq.~\eqref{eq:exact_balance_value_simple_bound} gives the desired result for
        \begin{align}
...
Using Assumptions~\ref{ass:iid_of_classification_data} and~\ref{ass:strictly_proper_composit_and_diff_bayes_risk}, we obtain, from Lemma~\ref{lemma:bregman_form_of_loss},
    ...
    \end{align}
    for $S(m+n,\delta,\lambda)=414 \frac{Q^2}{\lambda^{1/\alpha} (m+n)}\log(\frac{2}{\delta})$ and $A(\lambda)=414 L^2 \lambda^{1+2 r}$.
% Let us denote by
% $\gamma:=1+2 r +\frac{1}{\alpha}$,
...
\begin{align}
    ...
    \lambda^\ast=\left(\frac{1296 Q^2}{(m+n) L^2}\right)^{\frac{\alpha}{1+2 r\alpha +\alpha}}.
\end{align}
...
\begin{align}
    ...
    \begin{split}
        1296\cdot \max\!\Bigg\{&\frac{Q^2}{L^2 (B_2^\ast)^{1+2 r +\frac{1}{\alpha}}},
        \frac{Q^2}{L^2} \left(2 L R \log\frac{2}{\delta}\right)^{\frac{\alpha+2\alpha r+1}{\alpha r}},
        \frac{(B_1^\ast)^{2\alpha r+2\alpha}}{L^2 Q^{2\alpha r+2\alpha} (B_2^\ast)^{1+\alpha r+\alpha}},\\
        &\frac{L^2 \left(16\cdot 648 B_2^\ast\right)^{1+2 r+\frac{1}{\alpha}}}{1296^2 Q^2}
        \log^{1+2 r+\frac{1}{\alpha}}\!\left(\frac{48 (B_2^\ast)^2 1296\cdot 414^2}{\delta} \left(\frac{1296 Q^2 }{L^2}\right)^{-\frac{2 \alpha}{1+2 r\alpha +\alpha}}\right)
        \Bigg\}.
    \end{split}
\end{align}
\begin{enumerate}
    ...
    \begin{align*}
        m+n \geq \frac{1296 Q^2 }{L^2 (B_2^\ast)^{1+2 r +\frac{1}{\alpha}}}.
    \end{align*}
    ...
    % \end{align*}
    \begin{align*}
        m+n\geq \frac{1296 Q^2}{L^2} \left(2 L R \log\frac{2}{\delta}\right)^{\frac{\alpha+2\alpha r+1}{\alpha r}}.
    \end{align*}
    \item The requirement $\lambda^\ast\leq Q^{2\alpha} (B_2^\ast)^\alpha (B_1^\ast)^{-2 \alpha}$ is equivalent to
    \begin{align*}
        % m+n\geq \frac{L^2 Q^{4+2\alpha+2 r} (B_2^\ast)^{\alpha+2\alpha r}}{ (B_1^\ast)^{\frac{2\alpha+4 r\alpha + 2}{\alpha^2}}}.
        m+n\geq \frac{1296  (B_1^\ast)^{2\alpha r+2\alpha}}{L^2 Q^{2\alpha r+2\alpha} (B_2^\ast)^{1+\alpha r+\alpha}}.
    \end{align*}
    \item The requirement $m+n\geq \frac{1296 Q^2}{L^2 (\lambda^\ast)^{1+2 r+1/\alpha}}$ follows directly from the definition of $\lambda^\ast$.
    ...
    \begin{align}
        ...
        &= 648\cdot 8 (m+n)^{\frac{\alpha}{1+2 r\alpha +\alpha}} c \log\!\left(\frac{414^2}{\delta} c (m+n)^{\frac{\alpha}{1+2 r\alpha +\alpha}}\right)
    \end{align}
    \sloppy with $c:=8 B_2^\ast\left(\frac{1296 Q^2 }{L^2}\right)^{-\frac{\alpha}{1+2 r\alpha +\alpha}}$.
    Eq.~\eqref{eq:proof_fast_rate_technical_mn_condition1} follows from Lemma~\ref{lemma:technical} ($k:=(m+n)^{\frac{\alpha}{1+2 r\alpha +\alpha}}$, $A:=648 c$ and $B:=\frac{414^2}{\delta} c$), whenever
    % {\small
    \begin{align*}
        m+n\geq
        % \left(\frac{1296 Q^2}{L^2}\right)^{-1}
        \frac{L^2 \left(16\cdot 648 B_2^\ast\right)^{1+2 r+\frac{1}{\alpha}}}{1296 Q^2 }
        \log^{1+2 r+\frac{1}{\alpha}}\!\left(\frac{48 (B_2^\ast)^2 1296\cdot 414^2}{\delta} \left(\frac{1296 Q^2 }{L^2}\right)^{-\frac{2 \alpha}{1+2 r\alpha +\alpha}}\right)
        % 2 A \log(2 A B) 
    \end{align*}
    noting that $(m+n)^{\frac{\alpha}{1+2 r\alpha +\alpha}}\leq (m+n)^{\frac{1}{2}}$.
\end{enumerate}
Evaluating Eq.~\eqref{eq:bias_variance_fast_proof} at $\lambda^\ast$ as defined in Eq.~\eqref{eq:lambda_star_in_fast_rate_proof} gives the desired result for
        \begin{align}
        \label{eq:exact_form_of_constant_fast_bound}
        C:= 828  L^2 \left(\frac{1296 Q^2}{L^2}\right)^{\frac{\alpha+2 r\alpha}{1+2\alpha r + \alpha}}.
    \end{align}
...
\begin{proof}[Proof of Theorem~\ref{thm:balancing_principle_known_norm}]
    Using Assumptions~\ref{ass:iid_of_classification_data} and~\ref{ass:strictly_proper_composit_and_diff_bayes_risk}, we obtain, from Lemma~\ref{lemma:bregman_form_of_loss},
    \begin{align*}
    ...
    \end{align*}
    where $\mathcal{R}(f_{\z}^{\lambda_+})$ is an expectation w.r.t.~the loss function $\ell$ inducing $B(f_{\z}^{\lambda_+})=B_F(\beta,f_{\z}^{\lambda_+})$ as defined in as defined in Lemma~\ref{lemma:bregman_form_of_loss}.
    From Lemma~\ref{lemma:marteau_main_result} we further know that
    \begin{align}
    ...
    \label{eq:sample_size_known_norm}
        m+n\geq \max\left\{5184 \frac{B_2^\ast}{\lambda_0}\log\left(\frac{8 \cdot 414^2 B_2^\ast}{\lambda_0 \delta}\right), \frac{1296 Q^2}{L^2 \lambda_0^{1+2 r+1/\alpha}} \right\}.
    \end{align}
    We now define
    \begin{align*}
        \overline{\lambda}:=\max\{\lambda_i:A(\lambda_i) &\leq S(m+n,\delta,\lambda_i)\}
    \end{align*}
    with $S(m+n,\delta,\lambda):=\eta \frac{414 Q^2}{(m+n)\lambda^{1/\alpha}}\log\!\left(\frac{2}{\delta}\right)$ and $A(\lambda):=414 L^2\lambda^{1+2 r}$ as used in Lemma~\ref{lemma:marteau_main_result}, and $\eta:=1296\log^{-1}\!\left(\frac{2}{\delta}\right)$ as in Proposition~\ref{prop:fast_rate}.
    ...
    \end{align}
    where the last inequality follows from the fact that $\Hess_\lambda(f)=\Hess(f)+\lambda I\succcurlyeq \Hess(f)$.
    
    We will now prove bounds on the two terms in Eq.~\eqref{eq:proof_known_norm_helper1} separately.
    To bound the first term, we note that $\overline{\lambda}\leq\lambda^\ast$ and Eq.~\eqref{eq:necessary_balancing_condition} implies, for any $\lambda_j\leq \overline{\lambda}\leq\lambda^\ast$,
    \begin{align*}
        \norm{f_{\z}^{\overline{\lambda}}-f_{\z}^{\lambda_j}}_{\Hess_{\lambda_j}(f_{\mathcal{H}})}^2\leq 8 S(m+n,\delta,\lambda_j).
    \end{align*}
    As a consequence, we get $\overline{\lambda}\leq \lambda_+$ from the maximizing property of $\lambda_+$ in Eq.~\eqref{eq:balancing_principle_estimate_known_norm}. Another consequence of $\overline{\lambda}\leq \lambda_+$ and Eq.~\eqref{eq:balancing_principle_estimate_known_norm} is that
    \begin{align}
    \label{eq:proof_helper2}
        \norm{f_{\z}^{\lambda_+}-f_{\z}^{\overline{\lambda}}}_{\Hess_{\overline{\lambda}}(f_{\mathcal{H}})}^2
        \leq 8 S(m+n,\delta,\overline{\lambda}).
    \end{align}
    The bound
    \begin{align}
    \label{eq:proof_helper3}
        ...
        &\leq 2 S(m+n,\delta,\overline{\lambda})
    \end{align}
    ...
    \begin{align*}
        B(f_{\z}^{\lambda_+})-B(f_\mathcal{H})
        % &\leq 36 S(m+n,\delta,\overline{\lambda})\\
        &\leq 36 \xi^{1/\alpha} S(m+n,\delta,\lambda^\ast)
    ...
    \begin{align}
        ...
        C:=1296\cdot 36 \xi^{1/\alpha}\cdot 828  L^2 \left(\frac{1296 Q^2}{L^2}\right)^{\frac{\alpha+2 r\alpha}{1+2\alpha r + \alpha}}
    \end{align}
    is obtained from Eq.~\eqref{eq:exact_form_of_constant_fast_bound}.
\end{proof}

\subsection{Parameter Choice with Empirical Norm}
\label{subsec:proofs_balancing_principle_empirical}

In the following, let $f^\lambda:=\argmin_{f\in\mathcal{H}} \mathcal{R}(f)+ \frac{\lambda}{2}\norm{f}^2$
...
\begin{lemma}[{\cite[Proposition~15]{marteau2019beyond}}]
...
\begin{lemma}[{\cite[Remark~37, Lemma~33, Proposition~16]{marteau2019beyond}}]
...
\end{lemma}
\begin{proof}
    Lemma~33 in~\cite{marteau2019beyond} shows that (noting that $\max\{\mathrm{df}_\lambda,(Q^\ast)^2\}\leq (B_1)^2/\lambda$ from their Proposition~18 and $1/r_\lambda(\theta^\ast)\leq R/\sqrt{\lambda}$ by the remark after their Assumption~6) the conditions on $n$ in Lemma~\ref{lemma:marteau_main_result} allow to apply their Proposition~16, which gives the desired result.
\end{proof}
\begin{lemma}[{\cite[Lemma~30]{marteau2019beyond},~\cite[Prop.~6, Prop.~8]{rudi2017generalization}}]
...
\end{lemma}
\begin{proof}
    Eq.~\eqref{eq:technical_lemma1} is proven in~\cite[Lemma~30]{marteau2019beyond}.
    Eq.~\eqref{eq:technical_lemma2} follows from~\cite[Prop.~47]{marteau2019beyond} (attributed to~\cite[Prop.~6, Prop.~8]{rudi2017generalization}) and~\cite[Remark~48]{marteau2019beyond}.
\end{proof}
We are now ready to provide more precise steps on the derivation of Lemma~\ref{lemma:marteau_main_result}, which is essentially a re-phrasing of the proof of~\cite[Theorem~38]{marteau2019beyond} in our notation.
\begin{proof}[Proof of Lemma~\ref{lemma:marteau_main_result}]
    % We will just show how to obtain the inequality
    % \begin{align}
    %     \label{eq:start_reference_for_marteau}
    %     \mathcal{R}(f_{\z}^{\lambda})-\mathcal{R}(f_{\mathcal{H}})
    %         &\leq
    %         \norm{f_{\z}^{\lambda}-f_{\mathcal{H}}}_{\Hess(f_{\mathcal{H}})}^2
    %         \leq
    %         \norm{f_{\z}^{\lambda}-f_{\mathcal{H}}}_{\Hess_\lambda(f_{\mathcal{H}})}^2.
    % \end{align}
    % The rest follows then line by line from the proof of~\cite[Theorem~38]{marteau2019beyond}.
    ...
    \end{align}
    which is exactly how the proof of~\cite[Theorem~38]{marteau2019beyond} works.
    Indeed, they proceed by showing how Eq.~\eqref{eq:blabla} can be upper bounded by $414 \frac{Q^2}{(m+n) \lambda^{1/\alpha}}\log\left(\frac{2}{\delta}\right) + 414 L^2 \lambda^{1+2 r}$.
...
\begin{proof}[Proof of Lemma~\ref{lemma:Hessian_concentration}]
    From Lemma~\ref{lemma:technical_concentration_proof} we get
    \begin{align*}
    \widehat{\Hess}_\lambda(f_\z^\lambda)
    \preccurlyeq
    \frac{3}{2}\Hess_\lambda(f_\mathcal{H}).
    \end{align*}
    From Lemma~\ref{lemma:marteau_prop15}, we further get
    \begin{align*}
        \frac{3}{2}\Hess_\lambda(f_\mathcal{H})
        \preccurlyeq
    ...
    \end{align*}
    where the last inequality uses also Lemma~\ref{lemma:t_concentration}. Proceeding with Eq.~\eqref{eq:technical_lemma1} in Lemma~\ref{lemma:technical_concentration_proof} gives
    \begin{align*}
        6 \Hess_\lambda(f_\mathcal{H})
        \preccurlyeq
...
\begin{proof}[Proof of Theorem~\ref{thm:balancing_principle_empirical}]
    Following the proof of Theorem~\ref{thm:balancing_principle_known_norm}
    line by line up to Eq.~\eqref{eq:proof_known_norm_helper1} and replacing $\lambda_+$ by $\lambda_\mathrm{BP}$ gives
    \begin{align}
    ...
    \label{eq:sample_size_empirical_norm}
        % m+n\geq \max\left\{5184 \frac{B_2^\ast}{\lambda_0}\log\left(\frac{8 \cdot 414^2 B_2^\ast}{\lambda_0 \delta}\right), \frac{1296 Q^2}{L^2 \lambda_0^{1+2 r+1/\alpha}},\frac{16 B_2^\ast}{1296 b^\ast}\log\!\left(\frac{2}{\delta}\right) \right\},
        m+n\geq \max\left\{5184 \frac{B_2^\ast}{\lambda_0}\log\left(\frac{8 \cdot 414^2 B_2^\ast}{\lambda_0 \delta}\right), \frac{1296 Q^2}{L^2 \lambda_0^{2+1/\alpha}},\frac{16 B_2^\ast}{1296 b^\ast}\log\!\left(\frac{2}{\delta}\right) \right\},
    \end{align}
    where
    \begin{align*}
        \overline{\lambda}:=\max\{\lambda_i:A(\lambda_i) &\leq S(m+n,\delta,\lambda_i)\},
    \end{align*}
    and $S(m+n,\delta,\lambda):=\eta \frac{414 Q^2}{(m+n)\lambda^{1/\alpha}}\log\!\left(\frac{2}{\delta}\right)$ and $A(\lambda):=414 L^2\lambda^{1+2 r}$ are as used in Lemma~\ref{lemma:marteau_main_result} with $\eta:=1296\log^{-1}\!\left(\frac{2}{\delta}\right)$ as in Proposition~\ref{prop:fast_rate}.
    Note that we used the requirement $\frac{1296 Q^2}{L^2\lambda_0^{2+1/\alpha}}\geq \frac{1296 Q^2}{L^2\lambda_0^{1+2 r+1/\alpha}}$ in Eq.~\eqref{eq:sample_size_empirical_norm} that is independent of $r$, and, we added a requirement based on $b^\ast$ from Assumption~\ref{ass:Hessian_of_sequence} to satisfy all conditions needed to apply Lemma~\ref{lemma:Hessian_concentration}.
    % $m+n$ is larger than
% \begin{align}
...
% \end{align}
    ...
    \end{align}
    Now note that $\overline{\lambda}\leq\lambda^\ast$, and the same reasoning as for Eq.~\eqref{eq:necessary_balancing_condition} can be applied, together with Lemma~\ref{lemma:Hessian_concentration}, to obtain, for any $\lambda_j\leq \overline{\lambda}\leq\lambda^\ast$,
    \begin{align*}
        \norm{f_{\z}^{\overline{\lambda}}-f_{\z}^{\lambda_j}}_{\widehat{\Hess}_{\lambda_j}(f_{\z}^{\lambda_j})}^2\leq 48 S(m+n,\delta,\lambda_j).
    \end{align*}
    ...
    \label{eq:proof_helper_mepirical2}
        \norm{f_{\z}^{\lambda_\mathrm{BP}}-f_{\z}^{\overline{\lambda}}}_{\widehat{\Hess}_{\overline{\lambda}}(f_{\z}^{\overline{\lambda}})}^2
        \leq 48 S(m+n,\delta,\overline{\lambda}).
    \end{align}
    Combining the above results gives
    \begin{align*}
        B(f_\z^{\lambda_\mathrm{BP}})-B(f_\mathcal{H})
        \leq 32\cdot 48 S(m+n,\delta,\overline{\lambda})+2\cdot 4 S(m+n,\delta,\overline{\lambda}) = 1544 S(m+n,\delta,\overline{\lambda}).
    \end{align*}
    ...
    \begin{align*}
        B(f_{\z}^{\lambda_\mathrm{BP}})-B(f_\mathcal{H})
        % &\leq 36 S(m+n,\delta,\overline{\lambda})\\
        &\leq 1544 \xi^{1/\alpha} S(m+n,\delta,\lambda^\ast)
    ...
    \begin{align}
        ...
        C:=1296\cdot 1544 \xi^{1/\alpha}\cdot 828  L^2 \left(\frac{1296 Q^2}{L^2}\right)^{\frac{\alpha+2 r\alpha}{1+2\alpha r + \alpha}}.
    \end{align}
\end{proof}
...
\end{align*}
For the minimizer $f_\z^\lambda=\sum_{j=1}^{m+n} \alpha_j k(x_j,\cdot)$ of Eq.~\eqref{eq:empirical_loss_objective} as determined by the representer theorem, see, e.g.~\cite{scholkopf2001generalized}, it follows that
\begin{align*}
    \widehat{\Hess}_\lambda(f_\z^\lambda)=
...
We considered the problem of deriving error bounds for a large class of kernel methods for density ratio estimation for which no error bounds were known before (to the best of our knowledge).
...
As a consequence it achieves a minimax optimal error rate in the special case of quadratic loss.
...
The minimax optimality of our error bounds for non-quadratic losses is an open problem.
...
%  \hline
%  Method & Bregman generator $\phi(t)$ & Density ratio model $g$ & Penalty\\% & Loss $\ell(1,\cdot)$ & Loss $\ell(-1,\cdot)$\\
%  \hline
%  \hline
%  \rule{0pt}{3ex}KuLSIF~\cite[Eq.~(4)]{kanamori2012statistical} & $(t-1)^2/2$ & $g\in\mathcal{H}$  & $\norm{g}_{\mathcal{H}}^2$\\
%  % \rule{0pt}{3ex}KMM~\cite[Sec.~3.3]{huang2006correcting} & $(t-1)^2/2$ & $g\in\mathcal{H}$ & Ivanov (cf.~\cite[Sec.~3.1]{gizewski2022regularization},~\cite{page2019ivanov})  \\
%  \rule{0pt}{3ex}KL-est.~\cite[Eq.~(41)]{nguyen2010estimating} & $t\log(t)-t$ & $g\in\mathcal{H}$ & $\norm{g}_{\mathcal{H}}^2$\\
%  \rule{0pt}{3ex}LR~\cite[Sec.~7]{bickel2007discriminative}, & $t\log(t)-(1+t)\log(1+t)$ & $g(x)=e^{\theta^\text{T} K_x}$,~\cite[Sec.~3.2.3]{sugiyama2012densitybregman} & $\sum_{i=1}^{d+1}\theta_i^2$ \\
%  % \rule{0pt}{4ex}KLIEP~\cite{sugiyama2008direct} & $t\log(t)-t$ & - & - \\
%  \rule{0pt}{4ex}Power~\cite[Sec.~3.4.1]{sugiyama2012densitybregman} &
%  $\frac{t^{1+\alpha}-t}{\alpha}, \alpha\in (0,1]$ &
%  \begin{tabular}{@{}c@{}}
% \noindent
%  $g(x)=\sum_{i=1}^{m+n} c_i k(x,x_i)$, $x_{i+m}=x_i'$\\ for $i\in\{m+1,\ldots, m+n\}$
% \end{tabular} & $\sum_{i=1}^{m+n} c_i^2$,~\cite[Sec.~8.2]{menon2016linking} \\
%  % \rule{0pt}{3ex}General~\cite{gizewski2022regularization} & -  & - & -\\
%  \rule{0pt}{3ex}Exponential~\cite[Sec.~7]{menon2016linking} & $t^{-3/2}$ & 
%  $g(x)=e^{2\cdot \sum_{i=1}^{m+n} c_i k(x,x_i)}$& $\sum_{i=1}^{m+n} c_i^2$\\
%  \rule{0pt}{3ex}Square hinge~\cite[Sec.~6.1]{menon2016linking} & $(2 t + 2)^{-1}$ & 
%  $g(x)=\frac{-1+2\cdot \sum_{i=1}^{m+n} c_i k(x,x_i)}{2-2 \cdot \sum_{i=1}^{m+n} c_i k(x,x_i)}$& $\sum_{i=1}^{m+n} c_i^2$\\
%  \hline
%  \end{tabular}
%  }
%  \caption{Examples of Bregman distance losses and penalties for density ratio estimation.%Table~1 of~\cite{sugiyama2012densitybregman} (cf.~also \cite{kanamori2012statistical,menon2016linking,kato2021non}) extended by Tikhonov regularizers.
%  }
%  \label{tab:methods}
% \end{table}

We will also need the following definitions.
% \begin{defi}[Bias and degrees of freedom]
%     \label{def:bias_and_degrees_of_freedom}
%     Let $\mathcal{R}_\lambda(f):=\mathcal{R}(f)+\lambda\norm{f}$; define the Hessian $\Hess(f)$, the regularized Hessian $\Hess_\lambda(f)$, the Bias $\mathrm{Bias}_\lambda$ and the degrees of freedom $\mathrm{df}_\lambda$ by
%     \begin{align}
%         \label{eq:bias_hessian_degrees_of_freedom}
%         \Hess(f)&:=\mathbb{E}[\nabla^2\ell_Z(f)],\quad\Hess_\lambda(f):=\Hess(f) + \frac{\lambda}{2} I\\
%         \mathrm{Bias}_\lambda&:=\norm{\Hess_\lambda(f_{\mathcal{H}})^{-\frac{1}{2}}\nabla\mathcal{R}_\lambda(f_{\mathcal{H}})}\\
%         \mathrm{df}_\lambda&:=\mathbb{E}\!\left[\norm{\Hess_\lambda(f_{\mathcal{H}})^{-\frac{1}{2}}\nabla\ell_Z(f_{\mathcal{H}})}^2\right]
%     \end{align}
% \end{defi}
...
% \begin{lemma}
% [{\cite[Theorem~3]{marteau2019beyond}}]
% \label{thm:marteau_theorem3}
%     Let $\delta\in (0,1]$ and $\lambda>0$. Under Assumptions~\ref{ass:iid_of_classification_data},~\ref{ass:generalized_self_concordance} and~\ref{ass:boundedness_defin0_existence}, whenever
%     \begin{align*}
%         m+n\geq C_0\frac{R^2\mathrm{df}_\lambda \log\frac{2}{\delta}}{\lambda},
%     \end{align*}
%     then, with probability at least $1-2\delta$, the empirical risk minimizer $\widehat{f}_\lambda$ of Eq.~\eqref{eq:empirical_loss_objective} satisfies
%     \begin{align}
%         \label{eq:marteau_bound_theorem2}
%         \mathcal{R}(\widehat{f}_\lambda)-\mathcal{R}(f_{\mathcal{H}})\leq C_\mathrm{bias} \mathrm{Bias}_\lambda^2 + C_\mathrm{var} \frac{\mathrm{df}_\lambda\log\frac{2}{\delta}}{n},
%     \end{align}
%     where $C_0, C_\mathrm{bias}$ and $C_\mathrm{var}$ are either universal or depend only on $R\norm{f_{\mathcal{H}}}$.
% \end{lemma}
