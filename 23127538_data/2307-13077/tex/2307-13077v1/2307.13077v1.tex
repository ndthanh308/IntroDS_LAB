...
\author{\textsc{Marco Castrill\'{o}n$^1$}
\and \textsc{M. Eugenia Rosado Mar\'{\i}a$^2$}
\and \textsc{Alberto Soria}$^3$ \vspace{0.2cm} \\
$^1$ Departamento de Álgebra, Geometría y Topología, \\  
...
$^3$ Departamento de Matemática Aplicada a las TIC, \\
...
In this work ruled surfaces in $3$-dimensional Riemannian manifolds are studied. We determine the expression for the extrinsic and sectional curvature of a parametrized ruled surface, where the former one is shown to be non-positive. We also quantify the set of ruling vector fields along a given base curve which allow to define a relevant reference frame along it and that we refer to as \emph{Sannia}. The fundamental Theorem of existence and equivalence of Sannia ruled surfaces in terms of a system of invariants is given. The second part of the article tackles the concept of striction curve, which is proven to be
the set of points where the so-called \emph{Jacobi evolution function} vanishes on a ruled surface. This provides independent proofs for their existence and uniqueness in space forms, and to disprove its existence or uniqueness in some other cases.  
...
surfaces in $3$-dimensional space forms with vanishing Gauss curvature are necessarily ruled. However, this question is still open for more general contexts. Ruled surfaces have also been studied in Lorentzian contexts as in 
\cite{DillenKuhnel1999}, where all Weingarten surfaces are characterized in the $3$-dimensional Minkowski spacetime. Other results concerning ruled surfaces in the $3$-dimensional Minkowski spacetime have been established in \cite{Choi1995,KimYoon2000,KimYoon2004}. Lightlike ruled hypersurfaces have also been of great utility to address the problem of some geometric inequalities as in \cite{MarsSoria2016}, where some cases of the so-called null Penrose inequality for the Bondi energy of a spacetime are proven.    



All Euclidean ruled surfaces except cylinders admit a unique special curve called \emph{striction}, which is made up of the so-called central points of each generator \cite{PottmannWallner}. In the case of the cylinder the uniqueness property is not fulfilled for every curve in it is a striction. On the other hand, striction curves can degenerate to a single point, as occurs with the cone, where it is made up exclusively of its vertex. A central point in a geodesic generator of the surface furnish a critical point to the distance from points on that generator to neighboring geodesics.
...
functions is known is presented in Theorem 5.3.8 in \cite{PottmannWallner}. In this work we establish an analogous
...
Let $(M,g)$ be a $3$-dimensional Riemannian manifold and let $\psi \colon\Sigma\rightarrow (M,g)$ be an immersed orientable surface in $(M,g)$. We recall some basic concepts of the geometry of hypersurfaces (see [5, Volume II, Chapter VII, Section 3]). We denote by $\nabla$ and $\nabla ^\Sigma$ the Levi-Civita connections of $(M,g)$ and
...
Ruled surfaces are a remarkable family of hypersurfaces in $3$-dimen\-sional Riemannian manifolds. They are generally defined as follows:
\begin{definition}
	We say that the immersed hypersurface $\psi\colon \Sigma\rightarrow (M,g)$ in $(M,g)$
	is ruled if there exists a foliation of $\Sigma$ by complete curves which are geodesics in the ambient
	space $(M,g)$.
\end{definition}
...
\begin{remark}
		...
		an embedded surface in $(M,g)$. But if we require that rank of $d\mathbf{X}$ to be 2, it will be an immersed surface. On the other hand, if $(M,g)$ is not complete, the definition above is still valid by restricting the domain of $\mathbf{X}$.
\end{remark}
...
$\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$, where $d\mathbf{X}$ is of rank $2$. Then, we have
\begin{equation}
	\label{intcurvature}
...
Given any immersed surface $\psi\colon \Sigma\rightarrow (M,g)$, 
the sectional curvatures $K(T_p\Sigma)$ in $(M,g)$ and $K^\Sigma _p$ in $(\Sigma,g_\Sigma)$ defined by $T_p\Sigma$ respectively are related as
\[
K^\Sigma _p = K(T_p\Sigma) + K^\Sigma _{ext},
\]
where $K^\Sigma_{ext}$ is the extrinsic or Gauss curvature (the determinant of the second fundamental form endomorphism). By virtue of (\ref{intcurvature}), it follows that 
\begin{eqnarray}
	K(T_{p}\Sigma)
...
\begin{equation*}
	\lambda(u)=\frac{\mathrm{vol}_g(\alpha',Z,\nabla_{\alpha'}Z)}{\Vert\nabla_{\alpha'}Z\Vert^2}, 
\end{equation*}
where $\alpha: I\rightarrow (\Sigma,g_\Sigma)$ is the striction base curve and $Z$ is the ruling vector field along $\alpha$.
This function plays an important role in many results for classical ruled surfaces in $\mathbb{R}^3$. We next define a function defined on a parametrized ruled surface
$\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ motivated by the classical concept of distribution parameter: 

\begin{definition}[\bf Extended distribution parameter]
\label{Defdistribparam}	
Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface as in (\ref{rulparam}) in a Riemmanian $3$-manifold $(M,g)$ with $d\mathbf{X}$ of rank $2$. We define the \emph{extended distribution parameter function} of $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ as
...
\end{definition}
\begin{remark}
As we will illustrate in Section \ref{Strictioncurves}, for $v=0$ the above formula reduces to the classical distribution parameter $\lambda(u,0)=\lambda(u)$ provided $\alpha: I\rightarrow \mathbf{X}(I\times\mathbb{R})$ is an striction base curve. 
\end{remark}
...
\begin{definition}
	    \label{anglefunction}
		Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ a parametrized ruled surface in $(M,g)$ as in (\ref{rulparam}).
		The function $\sigma\colon I\rightarrow\mathbb{R}$
		satisfying
	\[
	\cos\sigma_{\alpha(u)}=\frac{g(\alpha^{\prime}(u),Z(u))}{\Vert\alpha'(u)\Vert },
	\]
	is called {\rm base angle} along the curve $\alpha$.
\end{definition}


In the following result we prove that the inner product of the vectors $\xu$ and $\xv$ remains constant along the rulings of a parametrized ruled surface.
\begin{proposition}
	\label{Propcoordframeangle}
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ a parametrized ruled surface in $(M,g)$ as in (\ref{rulparam}) by a unitary vector field $Z$. Then $g(\xu,\xv)$ is constant along its rulings. Moreover, the angle $\sigma$ between $\xu$ and $\xv$ at any point $q$ of the ruling containing $p=\alpha(u)$ is given by
	\begin{equation}
		\label{coordframeangle}
		\cos{\sigma}_q=\frac{\Vert\alpha'(u)\Vert\cos{\sigma_p}}{\Vert\xu\Vert_q}.
	\end{equation}
\end{proposition}
\begin{proof}
	...
	\end{equation*}
	where we have taken into account that $[\xu,\xv]=0$ and $\xv$ is geodesic. This means that $g(\xu,\xv)=\Vert\xu\Vert\cos{\sigma}$ is constant along the rulings of $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$.
	This value can be obtained by evaluating it at the point $p=\alpha(u)$. Indeed,
	given any $q$ of the ruling at $p=\alpha(u)$,
	\begin{equation*}
		g(\xu,\xv)|_q=\Vert\xu\Vert_q\cos{\sigma_q}=\Vert\xu\Vert_p\cos{\sigma}_p=\Vert\alpha'(u)\Vert\cos{\sigma_p},
	\end{equation*}
	from where relation (\ref{coordframeangle}) follows.
	\end{proof}

\begin{remark}
		Relation (\ref{coordframeangle}) shows that whenever the vectors  $\xu(u,0)=\alpha'(u)$ and $Z_p=\xv(u,0)$ are orthogonal at the base curve, they remain orthogonal along the ruling $\gamma_{Z(u)}(v)=\mathbf{X}(u,v)$. Nevertheless, the coordinate basis $(\xu,\xv)$ associated to the parametrization (\ref{rulparam}) with $d\mathbf{X}$ of rank $2$ is not necessarily orthogonal.
\end{remark}
...
\begin{proposition}
	...
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface in $(M,g)$ as in (\ref{rulparam}). Then the decomposition of the Jacobi field $\xu$ into its tangential and normal part with respect to the ruling is
	\begin{equation}
		\label{Jacdecomposition}
		\xu=\Vert\alpha'(u)\Vert(\cos{\sigma_p})\xv+\xu^{\bot},
	\end{equation}
...
In this background, the Jacobi field $\xu$ evaluated at $p $ reads $\xu|_p=\alpha'(u)$, so
	\begin{equation*}
		a=g(\alpha'(u),Z_p)=\Vert\alpha'(u)\Vert\cos{\sigma_p},
	\end{equation*}	
	and
	\begin{equation*}
...
\begin{definition}
	...
	vector field $Z\in\mathfrak{X}(\alpha)$ is in general position up to order $2$
	if it is in general position up to this order for every $u\in(a,b)$.
\end{definition}
...
\begin{proof}
	...
	\begin{align*}
		S_{1}  &  =\{(Z_{1},Z_{2})\in E\oplus E:Z_{2}=fZ_{1}\}\simeq E\times
		\mathbb{R},\\
...
\begin{definition}
	Let $(M,g)$ be a Riemannian $3$-manifold. A parametrized ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ is said to be a \emph{Sannia ruled surface} if
...
\begin{proposition}
   \label{propSanniaderivatives}
	...
	and let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a Sannia ruled surface
...
\label{definv}
	...
	ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$.
...
Let $\theta$ and $\varphi$ denote the spherical angles of $\alpha '$ with respect to $(X_{1},X_{2},X_{3})$; that is,
 \begin{equation}
		...
		\frac{\alpha'}{\Vert\alpha '\Vert}=\cos{\varphi}\cos{\theta}\, X_1+\sin{\varphi}\,X_2+\cos{\varphi}\sin{\theta}\,X_3.
	\end{equation}
 If $\varphi =0$, then $\theta$ is the base angle (see Definition \ref{anglefunction}). We will show in Section \ref{Strictioncurves} that this case corresponds to the base curve being a striction one.
...
to a base curve which does not have to be necessarily a striction curve. 
\begin{proposition}
	If $( X_{1},X_{2},X_{3} )  $ is the Sannia frame of a Sannia ruled surface  $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$, then the value of the distribution parameter function $\lambda$ 
	on the base curve $\alpha$ is 
	\[
 \lambda(u,0)=
 \frac{\Vert\alpha' \Vert\kappa_{0}^{2}\kappa_1 \cos{\varphi}\sin{\theta}}{\left(\frac{d\kappa_0}{du}\right)^2+\kappa_0^{2}\kappa_1^{2}}.
	\]
\end{proposition}
...
\begin{remark}
	Note that it is always possible to consider a unit ruling $Z$ along an arc-length parametrized curve $\alpha$, i.e $\kappa_0=1$ and $\Vert \alpha' \Vert=1$. In such case the value of the distribution parameter function along $\alpha$ reduces to 
	\begin{equation}
		\label{basecurvdistribparam}		
		\lambda(u,0)=\frac{\cos{\varphi}\sin{\theta}}{\kappa_1}. 
	\end{equation}
\end{remark}
...
\begin{theorem}[Fundamental theorem of ruled surfaces]
	...
	$\alpha\colon(u_{0}-\delta,u_{0}+\delta)\rightarrow (M,g)$, parametrized by its arc length, and a vector field $Z\in \mathfrak{X}(\alpha)$ such that the Sannia ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$, $\mathbf{X}(u,v)=\exp_{\alpha(u)}(vZ(u))$ satisfies:
	...
	\end{enumerate}
where  $\kappa_{i}$, $0\leq i\leq2$, $\theta$ and $\varphi$ are the invariant of Definition \ref{definv}
...
\begin{proof}
	...
	to the orthonormal basis $(e_i)_{i=1}^3$. We define the natural
	...
	\end{equation*}
	    Without loss of generality, the ruling can be considered to be unit, i.e. $\okap_0=1$
	...
	delta. We consider the ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$, $\mathbf{X}(u,v)=\exp_{\alpha(u)}(vZ(u))$
	with
	\[
	...
	constant function $\hat{\phi}_{ij}(u)=\hat{\delta}_{j}^{i}$ also satisfy these
	system of differential equations with these initial conditions. By virtue of the
	unicity theorem of EDOs, it follows that $\phi_{ij}(u)=\hat{\delta}_{j}^{i}$, for
	...
	surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$. Finally, again from (\ref{sistemaEDO}), it follows that
	$\okap_{1}$ and $\okap_{2}$ are the Sannia invariants.
	
	We now prove the uniqueness of the ruled surface up to isometry. First, if
	...
	the space is locally isotropic, and hence of contant curvature (\emph{cf.} \cite{KN}).
\end{proof}
...
Euclidean ruled surfaces contain a special curve called \emph{striction} which is the locus of the points whose distance with respect to neighbouring geodesic is extremal. In particular striction curves are divided into \emph{expanding-contracting} and \emph{contracting-expanding}  
...
\begin{definition}  
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface. A curve $s\colon I \to \mathbf{X}(I\times \mathbb{R})$  
...
\begin{proof}
	The above relation holds since the tangent plane to  $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ along $s$ is spanned by $s^{\prime}$ and $Z$, and
...
\end{corollary}	
\begin{proof}
	The Gauss identity on the striction reads
	\begin{equation*}
...
\begin{remark}
    ...
    in the Euclidean space $(\mathbb{R}^3,\delta)$ an striction curve can always be taken as the base curve in the parametrization for it always exists. By virtue of the above proposition, in such a context $\varphi=0$ and $\theta=\sigma$, i.e. the base angle on the striction. 
	In particular, the distribution parameter formula (\ref{basecurvdistribparam}) on a general base curve reduces to  
    \begin{equation*}
  	\lambda(u,0)=\frac{\sin{\sigma_{s(u)}}}{\kappa_1}, 
    \end{equation*}
...
\begin{proposition}
	\label{criticalJac}
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a 
	parametrized ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ with $\alpha\colon I \to \mathbf{X}(I\times \mathbb{R})$ an associated base curve, and with a unitary ruling vector field $Z\in \mathfrak{X}(\alpha)$. A curve $s\colon I \to \mathbf{X}(I\times \mathbb{R})$ on such a surface is a   
...
\begin{proof}
Given any curve of the form $s:I\rightarrow \mathbf{X}(I\times \mathbb{R})$, $s(u)=\mathbf{X}(u,v(u))$, where $v:I \rightarrow \mathbb{R}$ is smooth, we have $s^{\prime}=\xu+v^{\prime}\xv \label{tanstrict}$. Then
\begin{eqnarray*}
g(s^{\prime} ,\nabla _{s^{\prime}}\xv)&=& g (\xu+v^{\prime}\xv, \nabla _{\xu+v^{\prime}\xv} \xv)= g (\xu+v^{\prime}\xv, \nabla _{\xu} \xv )\\
&=& g (\xu, \nabla _{\xu} \xv ) = g (\xu, \nabla _{\xv} \xu )
= \tfrac{1}{2}\xv(\Vert \xu\Vert ^{2}),
\end{eqnarray*}
where we have taken into account that $g(\xv,\xv)$ is constantly $1$, and that  that $\nabla _{\xu}\xv-\nabla _{\xv}\xu=[\xu,\xv]=0$. The proof is complete by the definition of striction curve. 
\end{proof}
...
\begin{definition}
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface in a $3$-dimensional Riemannian manifold $(M,g)$. The function
...
Just the opposite happens in case that $F$ is strictly negative at some point. Since $(u,v)$ is a system of coordinates associated to the parametrization
...
\begin{theorem}
	...
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface in a Riemannian $3$-manifold $(M,g)$. The first and second derivative of the
...
\begin{proof}
	...
	\begin{eqnarray*}
		\frac{\partial F}{\partial v}&=&-g(R(\xv,\xu)\xv,\xu)+\Vert\conv \xu\Vert^2  \nonumber \\
...
\begin{corollary}
	...
	Let $(M(k),g)$ be $3$-Riemannian manifold of constant sectional curvature $k$ and  $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface with $Z$ a unitary vector field. Then the first and second derivatives of the Jacobi evolution function $F$ read
	\begin{eqnarray}
		\label{Jacfirstspaceform}
		\frac{\partial F}{\partial v}&=&-k(\Vert\xu\Vert^2-\Vert\alpha'(u)\Vert^2\cos^2\sigma_p)+\Vert\conv \xu\Vert^2  \\
		\label{Jacsecondspaceform}
		\frac{\partial^2 F}{\partial \, v^2}&=& (-2k)\xv(\Vert\xu\Vert^2)=-4kF(u,v)
	\end{eqnarray}
	where $p=\alpha(u)=\mathbf{X}(u,0)$ and $\sigma_p$ is the base angle between $\alpha'(u)$ and $Z$ at $p$.
\end{corollary}
\begin{proof}
	...
	\end{equation}	
	since $\xv$ is a unitary vector field. On the other hand, we know by Proposition \ref{Propcoordframeangle} that $g(\xu,\xv)=\Vert\alpha'(u)\Vert\cos{\sigma_p}$ is constant along the associated ruling. Hence, (\ref{seccurvature}) can be rewritten as
	\begin{equation}
		\label{seccurvaturetwo}
		\Riem(\xv,\xu,\xv,\xu)=-k\left(\Vert\xu\Vert^2-\Vert\alpha'(u)\Vert^2 \cos^2 \sigma_p \right),
	\end{equation}
	...
	\end{equation}
	Using the splitting (\ref{Jacdecomposition}) given in Proposition \ref{Propjacdecomposition}  for the Jacobi field $\xu$, we obtain
	\begin{eqnarray}
		\label{splittingterm}
		g(\xu^{\bot},\nabla_{\xv}\xu)&=&g(\xu,\nabla_{\xv}\xu)-\Vert\alpha'(u)\Vert(\cos{\sigma_p})g(\xv,\nabla_{\xv}\xu)\\ &=&\frac{1}{2}\xv(\Vert\xu\Vert^2), \nonumber 
	\end{eqnarray}
...
\begin{theorem}
	 Let  $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a parametrized ruled surface in a complete manifold $(M,g)$ of constant sectional curvature $k$, with $Z$ a unitary ruling vector field.  Then the expression of the Jacobi evolution function $F$ is
	\begin{eqnarray}
...
\begin{eqnarray}
		...
		C_2&=&\tfrac{1}{\sqrt{|4k|}}\left( -k\Vert\alpha'\Vert_p^2\sin^2{\sigma_p}+\Vert\nabla_{\alpha'}Z\Vert_p^2\right) , \quad \text{if $k>0$ or $k<0$}, \\
		\label{Ceuclid}
		 C_2&=&\Vert\nabla_{\alpha'}Z\Vert^2_p,   \quad  \text{if $k=0$}. 
\end{eqnarray}
...
\begin{proof}
	...
	\begin{equation}
		...
		\frac{\partial F}{\partial v}(u,0)=-k\Vert\alpha'(u)\Vert_p^2\sin^2{\sigma_p}+\Vert\nabla_{\alpha'} Z\Vert^2_p
	\end{equation}
	since $\xu(u,0)=\alpha'(u)$. Differenciating (\ref{Jacobifuncsphere}) at $v=0$ from (\ref{Fderivativebasecurve}) we obtain
	\begin{equation*}
		C_2 \sqrt{4k}=-k\Vert\alpha'(u)\Vert^2\sin^2{\sigma_p}+\Vert\nabla_{\alpha'} Z\Vert^2_p,
	\end{equation*}
	which is none other than $C_2$ in (\ref{Cspherehyperbolic}) for $k>0$. The respective value of $C_2$ for $k<0$ is obtained in an analogous way differenciating  (\ref{Jacobifunchyperbolic})  at $v=0$.
	Likewise, in the Euclidean context $k=0$, $(\partial F/\partial v)(0)=\Vert\nabla_{\alpha'} Z\Vert^2_p$, which implies the value of $C_2$ in (\ref{Ceuclid}) for the Jacobi evolution function is $F(u,v)=C_1+C_2 v$  in the flat case.
\end{proof}
...
\begin{theorem}
	Let $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ be a 
	Sannia ruled surface $\mathbf{X}:I\times\mathbb{R}\rightarrow (M,g)$ in a complete manifold $(M,g)$ of constant sectional curvature $k$, where $\alpha\colon I \to \mathbf{X}(I\times \mathbb{R})$ is an associated base curve, and with a unitary ruling vector field $Z\in \mathfrak{X}(\alpha)$. If $s\colon I \to \mathbf{X}(I\times \mathbb{R})$, $s(u)=\mathbf{X}(u,v(u))$ is an striction curve, then 
	\begin{eqnarray}
		\label{strictionsphere}
		v(u)&=&\frac{1}{\sqrt{4k}}\arctan{\left(\frac{-\sqrt{4k}\,g_p(\nabla_{\alpha'}Z,\alpha')}{-k\Vert\alpha'\Vert_p^2\sin^2{\sigma_p}+\Vert\nabla_{\alpha'} Z\Vert^2_p}\right)} \quad \text{if $k>0$}, \\
		\label{strictioneuclid}
		v(u)&=& -\frac{g_p(\nabla_{\alpha'}Z,\alpha')}{\Vert\nabla_{\alpha'}Z\Vert^2_p}  \quad \text{if $k=0$},  \\
		\label{strictionhyperbolic}
		v(u)&=&\frac{1}{\sqrt{-4k}}\mathrm{arctanh}{\left(\frac{-\sqrt{-4k}\,g_p(\nabla_{\alpha'}Z,\alpha')}{-k\Vert\alpha'\Vert_p^2\sin^2{\sigma_p}+\Vert\nabla_{\alpha'} Z\Vert^2_p}\right)} \quad \text{if $k<0$},
	\end{eqnarray}
	where $\sigma_p$ is the angle betwen the vectors $\alpha'$ and $Z$ at $p=\alpha(u)$. In addition, if $M$ is simply connected (that is, $M=S^3$ for $k>0$, $M=\mathbb{R}^3$ for $k=0$ or $M=\mathbb{H}^3$ for $k<0$ with their standard metrics scaled by $1/\sqrt{|k|}$ when  $k\neq 0$), then for $k\geq 0$, every Sannia surface has a unique striction curve. For $k<0$, every Sannia surface has at most one striction curve.
\end{theorem}
...
&& \mathbf{X}(u,v)=(v, (2+\sin{v})\cos{u}, (2+\sin{v})\sin{u}) . \label{paramseveralstriction}
	...
	\end{equation*}
	in local coordinates associated to the parametrization (\ref{paramseveralstriction}). It is a well known fact that
	the generating curves of a surface of revolution in $(\mathbb{R}^3,\delta)$ are geodesics of the surface.
	...
	$\alpha'(u)=\xu |_\alpha$ is the initial $v=0$ value  of the Jacobi vector field $\xu$ along the rulings, and $Z=(1/\sqrt{1+\cos^2{v}})\xv$ is a unit geodesic vector tangent to them. Let us compute the Jacobi evolution function $F$  in this context. A straightforward calculation gives
	\begin{equation*}
		\nabla_{\alpha'}Z=\frac{\cos{v}}{(1+\cos^2{v})(2+\sin{v})}\xu.
	\end{equation*}
	As a consequence
	\begin{equation*}
...
\bibitem{Castrillonetal2015}M.\, Castrillón, V.\, Fernández Mateos, J.\, Muñoz, {\it The equivalence problem of curves in a Riemannian manifold},  Annali di Matematica Pura ed Applicata (1923-) \textbf{194(2)} (2015), 343--367.
...
\bibitem {DaSilvaDaSilva2022}L.C.B da Silva, J.D. da Silva, {\it Characterization of manifolds of constant curvature by ruled surfaces}, S\~ao Paulo Journal of Mathematical Sciences (2022),  1--25. 


\bibitem {DillenKuhnel1999}F.\, Dillen, W.\, Kühnel, {\it Ruled Weingarten surfaces in Minkowski $3$-space}, Manuscripta.\ Math. {\bf 98} (1999), 307--320.
...
\bibitem {LiuYuJung2014} H.\, Liu, Y.\, Yu, S.D.\, Jung {\it Invariants of non-developable ruled surfaces in Euclidean 3-space},  Beiträge zur Algebra und Geometrie/Contributions to Algebra and Geometry. {\bf 55.1} (2014), 189--199.
...
\bibitem{Muller1941} H.R. Müller,  \emph{Über die Striktionslinien von Kurvenscharen}, 50, Monatsh. für Math. und Phys., 1941, 101–110.   

\bibitem{Muller1948} H.R. Müller,  \emph{Über Striktionslinien von Kurven- und Geradenscharen im elliptischen Raum}, 52, Monatsh. für Math., 1948, 138–161.
