...
multiscale approximations with error estimates 
...
%in the course of 
%reformulating, via 
a rescaled Floquet-Bloch-Gelfand transform to resolvent problems for high-contrast elliptic PDEs with highly oscillatory 
...
is the bivariate operator sandwiched %accompanied 
by a connecting operator which for a broad class of periodic %high-contrast 
problems specialises to a new two-scale version of the classical Whittaker-Shannon interpolation. 
...
x=(x_1,x_2,...,x_n)^T\,\in\mathbb{R}^n, \,\, n\ge 1.  
	\end{equation}
...
\cite{Av08,BaKaSm,Bell,BoFe,Br,CaEd05,
cherd,ChErKi,ChKiVeZu23,
Co,IVKVPS18,IVKVPS13,
IVKVPS19,Pet-Bohm-08,ZhiPa,VPS}. % for a very short, non-exhaustive, list of such works. 
Somewhat similar effects are observed in %homogenisation of 
thin periodic structures, see e.g. recent publications 
\cite{Ch-V-2021, Ch-V-Z-2023} and further references therein. 

Problem \eqref{th-hc} is mathematically a spectral problem for unbounded self-adjoint operator $\mathcal{A}u=\,-\,{\rm div}A_{\ep}\nabla u_\ep$ in Hilbert space $L^2\left(\RR^n\right)$, 
with spectral parameter $\lambda=\rho\,\omega^2$. (Here we regard $A_\ep(x):=a_{\ep,\,\ep^2}(x)$ i.e. we set for simplicity 
$\delta=\ep^2$.) 
...
Moreover, they often also hold keys for analysing related evolution problems, both parabolic and hyperbolic, cf. e.g. 
\cite{ZhP07,Pas05,IVKVPS13}. 
...
can one establish tractable leading-order approximations for the asymptotically degenerating equations like \eqref{hom}, %, with respect to $\ep$,  for 
...
not even attempt here to provide a review of all these different methods  
except to mention one approach of particular relevance to this work: the so-called {\it spectral method}, see for example \cite{Zh89,CoVa97,BiSu,ZhSpectr}, and also \cite{DGR23} and further references therein for most recent developments of the spectral approach for long-time homogenisation problems in both periodic and random settings. Our approach here is conceptually somewhat similar to 
\cite{BiSu} 
but we believe bears at the same time fundamental novelties allowing to obtain new results for much wider classes of examples, both 
non-degenerate and degenerate,  see Section \ref{sec:examples}. 
Namely, our method does not employ analytic perturbation theory or alike but (as clarified below) performs instead a robust generic asymptotic analysis of variational problems and related operators near 
%the related key 
...
A leading-order approximation with $L^2$ error estimates was established in \cite{ChCo}, for multiple spatial dimensions, using the spectral method as a basis; however the techniques employed therein appear problem specific and not readily generalisable. 
 ...
 We emphasise here that upon applying our  general method to the above key example of multi-dimensional double porosity model, see Section \ref{e.dp}, we provide new operator estimates between the initial problem and a novel approximation via the two-scale limit operator. 
...
$\mathcal{I}_\ep$ is an $L^2$-isometry and a key tool for recasting, for any final $\ep>0$, an input function $F(x)$ of 
\eqref{hom} as  a two-scale function $f_0(x,y)$.  %which 
This in turn serves 
as an input for the two-scale limit problem, which ultimately ensures the desired operator error estimate, see \eqref{dpcompe3}, with further implications e.g. for 
error estimates on Floquet-Bloch spectrum and eigenfunctions (Bloch waves), Theorem \ref{EVestDP} and Remark \ref{EFestDP}. 
Operator $\mathcal{I}_\ep$ 
is a specialisation of a naturally constructed abstract connecting operator $A_\ep$ (Theorem \ref{thm.bivariate}), 
and 
appears to be a 
 novel two-scale analogue of classical 
Whittaker-Shannon interpolation formula, 
see Remark \ref{RemShann}. 
It plays a role somewhat similar to the periodic unfolding operator see e.g. \cite{CDG}, which has %a proven record for 
proven capable of 
establishing 
operator-type estimates for certain classical (moderate contrast at low frequencies) homogenisation problems, e.g. \cite{griso}. 
We argue however 
%It replaces in a sense the periodic unfolding operator \cite{CDG, griso}, 
%and we argue 
(see Remark \ref{ShannVsUnf}) that the new two-scale interpolation operator $\mathcal{I}_\ep$ is the most natural one 
for a wide class of models, including 
%The latter, while proven appropriate for analysing classical (moderate contrast at low frequencies) homogenisation problems, see e.g. \cite{griso}, 
% seems insufficient for 
the high-contrast problems at micro-resonant frequencies. 
...
That all being said, one of the main aims of this article is  
%not to rediscover or improve the previously obtained %mentioned available 
%results. Rather, we shall  
to demonstrate that a large class of the above mentioned problems (as well as many others) are all examples of  one particular 
...
parametrised by quasi-periodicity variable (``quasimomentum'') $\t$:  
\begin{equation}
...
-- `Inverted' high-contrast model (Section \ref{e:idp}), with resulting approximation (accompanied by operator error bounds) by an infinite contrast `stiff inclusions problem' 
...
-- Schr\"{o}dinger equations  with a strong periodic magnetic field (Section \ref{magnschrod}); 
%\[
...
-- Differential-difference equations (Section \ref{sec:nonloc});
 ...
 -- Difference equations (Section \ref{sec:nonloc2}).
...
(cf. \cite{IVKVPS13} for some related background details); %for some further related details); 
...
The article is organised as follows. In Section \ref{sec:pf} we formulate the abstract problem and introduce our 
main assumption \eqref{KA} 
%of the article 
that can be regarded as a version of a  \textit{spectral gap} condition. Namely, for every $\t\in\Theta$ the form $a_\t$ is coercive (generally 
...
see Theorem \ref{thm:contV}. This simple result not only forms the basis for further investigation, it appears applicable to certain physically relevant models, for example,  the { \it inverted } double porosity model ( Example \ref{e:idp}) and in the study of certain polarisations of electromagnetic waves in photonic crystal fibres, cf. \cite{Cothesis,CoKaSmPCF}. 
...
high-contrast problems \eqref{hom}, and  corresponds to loss of the $\t$-uniformity of the spectral gap which requires %as a result 
 a much more subtle asymptotic analysis of the solutions near related %such a 
singular points. 
  In particular, in Theorem \ref{thm1.all} we construct a leading-order approximation to problems \eqref{pr}  when the null-space $V_\t$ possesses an isolated singularity (say at the origin $\t=0$) that is removable in the following sense:  there exists a subspace $V_\star$ such that 
$	
	V^\star_\t  = \left\{ \begin{array}{cc}
		V_\t, & \t \neq 0, \\ V_\star, & \t = 0
	\end{array} \right.  
$ is Lipschitz continuous, see \eqref{contVs}. 
The resulting approximate problem \eqref{coupledbest} is on a ``sum'' of $V^\star_\t$ and the 
`defect subspace' $Z$ (describing the discontinuity gap between $V_\star$ and $V_0$), with  
$\M z=z+\N z$  where $\N z$ is an abstract version of the classical corrector, 
see \eqref{IliaN}. 
The results of Section \ref{section:discV} are found useful for certain applications, see e.g. the example with 
concentrated perturbations in Section \ref{sec:concpert}. 

Problem \eqref{coupledbest} 
%It 
is simpler than \eqref{pr}, 
but still depends on $\ep$ and $\t$. In Section 
...
%additional 
mild regularity assumptions \eqref{H4} and \eqref{H5} on $a_\t$ and $b_\t$, %and some $\t$-nondegeneracy condition for the spectral gap, 
that are readily observed in many (even if not all, cf the example in Section \ref{sec:concpert} mentioned above) practical examples. 
%nder some additional mild regularity assumptions on $a_\t$ and $b_\t$ that are readily observed in practice. 

This leads us to one of the main results, Theorem \ref{thm.IKunifest2}, that provides a uniform approximation to the two-parameter solution $u_{\ep,\t}$ of \eqref{pr} in terms of a solutions to a one-parameter family of variational problems 
\eqref{IKz3prob88} 
on the smaller space $V_0$ with sesquilinear forms $a^{\rm h}_\xi \,+\, b_0$, $\xi=\t/\ep \in \RR^n$. 
...
Here the non-negative form $a^{\rm h}_\xi\left(z,\tilde z\right)$ acts on the even smaller 
defect subspace  
$Z$, and 
...
is found to be finite-dimensional under a stronger version \eqref{KA2} of the spectral gap condition which typically holds in many practical examples, see Section \ref{sec.newKA}. 
(Notice however a simple example of a difference equation in Section \ref{sec:nonloc2}, where \eqref{KA2} does not hold. 
Nevertheless our general 
scheme is  still applicable and yields, despite infinite-dimensional $Z$, explicit approximations with error estimates in terms of a two-scale limit operator sandwiches by the two-scale interpolation operator, 
see Theorem \ref{thm.2scOpResnonloc}.) 
\vspace{.1in} 

The significance of the dependence of the approximating problem \eqref{IKz3prob88} only  on single parameter $\xi=\t/\ep$ manifests itself  in Section \ref{s:resolv} were $\ep$-independent approximations in terms of an abstract version of a `two-scale' limit operator, with principal symbol  $a^{\rm h}_\xi$,  are constructed.  
...
Namely, Section \ref{s:resolv} focuses on the associated abstract spectral problem in an ambient Hilbert space $\mathcal{H}\supset H$: 
\begin{equation}
	...
	\right.\label{prspec}
\end{equation}
where $d_\t$ is an inner product in $\mathcal{H}$ and a positive compact sesquilinear form on $H$, satisfying some isometry conditions \eqref{H6} that are trivially observed in examples. Using the results of the previous sections we establish that the union of the spectra $\bigcup_{\t \in \Theta} \lambda_{\ep,\t} $  converges in appropriate sense, with rate 
$\ep$. % (Corollary \ref{c.collspec}). 
Moreover, in Theorem \ref{thm.bivariate} we approximate the resolvent problem in terms  of certain self-adjoint `bivariate'  operator $\mathcal{L}$, which is an abstract version of two-scale limit operator 
 and is related to the `homogenised' form $a^{\rm h}_\xi \,+\, b_0$ via 
(inverse) 
Fourier transform (i.e. $\xi \,\mapsto\, -\,\i\,\nabla$). 
An important role is played here 
by the already mentioned %above 
abstract $L^2$-isometric connecting operator $A_\ep$.  
%As a result w
We then show 
(Theorem \ref{bivariate.spec}) 
that the limit spectrum converges to the spectrum of $\mathcal{L}$, with a rate $\ep$. 
%certain self-adjoint `bivariate'  operator, which is an abstract version of two-scale limit operator 
% and is related to the `homogenised' form $a^{\rm h}_\xi \,+\, b_0$ via 
%Fourier transform (i.e. $\xi \,\mapsto\, -\,\i\,\nabla$).  
The bivariate operator $\mathcal{L}$, specified by form \eqref{Q}, can be viewed as a second-order constant-coefficient differential operator acting in the Bochner space 
$L^2\left(\RR^n ;\, \overline{V_0} \right)$, where $\overline{V_0}$ is the closure of $V_0$ in $\mathcal{H}$. 
...
Extensive Section \ref{sec:examples} aims at demonstrating the power and versatility of our abstract results by 
applying them 
to a diverse set of physically motivated examples. 
%for various  problems of interest. 
We thereby not only  re-establish, in the new light, some previously known results 
but obtain a number of 
...
present a simpler example displaying a particular feature. 
\vspace{.1in} 

For all the above reasons, we expect the proposed approach to have a significant potential for further developments and applications, possibly going far beyond those discussed in the present work. 
In particular, the generality of the abstract scheme and versatile features of the emerging accompanying tools like the 
two-scale interpolation operator and its more abstract version, are expected to allow treating various non-periodic and time-dependent problems. 
...
As we shall see, the asymptotics of the solution to \eqref{p1} crucially depend on certain continuity properties of 
...
Theorem \ref{thm:contV} also holds `locally', i.e. with $\Theta$ replaced by any of its subsets $\Theta'$ such that 
...
b_\t[r ]\,\,\le\,\, \| w\|_\t^2\,.
\end{equation} 
Since \eqref{w1prob0} is a restriction of \eqref{p1} to $\M V_0$, $r$ is orthogonal to $\M V_0$ with respect to $A$ (indeed setting $\tilde{u} = \M \tilde{v}$ in \eqref{p1} and subtracting \eqref{w1prob0}  gives $A\left(r,\M \tilde{v}\right) =0$ for any $\tilde{v} \in V_0$). Consequently, since $w = r - \M v_r$, one infers $A[r] \le A[w]$. This inequality along with \eqref{IliaN4.1} yields $b_\t[r] \le b_\t [w]$ which clearly implies  \eqref{IliaN6}, see \eqref{astructure}. The proof is complete. 
 \end{proof}
...
the form of a direct integral along its fibers as given for a.e. $\t\in\Theta$. 
\vspace{.15in}
...
\subsubsection{Approximation via the two-scale limit operator and an associated two-scale interpolation operator}\label{interpol77}
...
In particular, for $g\in\mathbb{H}_0$, the two-scale limit problem $\left(\mathcal{L}_0+I\right)u_0=g$ is to find such 
...
\end{aligned} \right.
\ee
As a result, Theorem \ref{thm.bivariate} via a routine specialisation to the present setting yields the following. 
...
Then, 
	...
	\end{equation}
	Here  $\mathcal{I}_\ep: L^2(\RR^n) \rightarrow L^2(\RR^n \times \square)$, which we call 
	...
	\ee
	In \eqref{2ScInterp}, $\Gamma_\ep: \, F(x)\mapsto \ep^{n/2}F(\ep x)$ is the rescaling operator and 
	$\Gamma_\ep^{-1}: \, g(x,y)\mapsto \ep^{-n/2}g\left(\ep^{-1} x,\,y\right)$ is its inverse in $x$; 
	...
	$\mathcal{F}^{-1}:h(\xi,y)\mapsto g(x,y)$ is the inverse Fourier transform also in the first variable. 
...
Finally, \eqref{calttstar} immediately follows via \eqref{dpcompe3}, 
...
The latter serves in turn as the input for the two-scale limit problem \eqref{2sclimprob}, whose solution $u_0(x,y)$ is converted by the adjoint 
...
can be derived directly from the above definition of $\mathcal{I}_\ep$ 
(cf. Remark \ref{lastrem} below) 
and is also immediately 
...
%there exists $\ep_0>0$ 
 $0<\ep\le\pi R^{-1}$, $\big(\mathcal{I}_\ep F_\ep\big)(x,y)=\Phi(x+\ep y, y)$.  
...
which property is inherited by the solution $u_0(x,y)$ of the two-scale limit problem \eqref{2sclimprob}. 
...
For such $u_0$ it follows in turn from noticing that  \eqref{7.54-2} is a convolution of $u_0$ (with respect to its first variable) with the rescaled Sinc-function whose 
...
In this respect, 
it would be of interest to compare the new interpolation operator $\mathcal{I}_\ep$ with any 
other candidate interpolators. One of these is the periodic unfolding operator see e.g. \cite{CDG},  
...
which has a track record of establishing operator-type error estimates, see e.g. \cite{griso}, 
...
This means that, for regular enough $F$, both $\mathcal{T}_\ep$ and 
...
We briefly discuss here some similarities and differences between $\mathcal{I}_\ep$ and $\mathcal{T}_\ep$, 
postponing a more detailed discussion for another study.  
Notice that the unfolding operator can be written in a form akin to \eqref{7.54-1}. Namely, 
\begin{equation}
	\label{7.54-1UF}
	\mathcal{T}_\ep F(x,y) \,\,=\,\,  \sum_{m\in \ZZ^n} F\big(\ep y + \ep m\big)\,\,  
	\chi_\square\left( \frac{x}{\ep}  - m\right),  
	\end{equation}
where $\chi_\square$ is the characteristic function of the periodicity cell $\,\square$. 
Comparing then with \eqref{2ScInterp}, one observes that $\mathcal{T}_\ep$ has the following operator form
\be
	\label{2ScInterpUF}
...
extension operator $\chi$ in \eqref{2ScInterp},  is the operator of 
``$\,\square^*$-periodisation'' in the first variable followed by multiplication by a ${\rm Sinc}$ function. Namely, 
\[ %be
%\label{}
...
${\rm Sn}$ is an $L^2$-isometry (and so is $\mathcal{T}_\ep$, as a composition \eqref{2ScInterpUF}); 
and in particular ${\rm Sn}^*\,{\rm Sn}=I$. 

%The key point is that it is 
As we have seen, 
our new two-scale %Whittaker 
interpolation operator $\mathcal{I}_\ep$ %which achieves 
delivers 
a desired approximation with an % strong 
operator norm error estimate 
...
operators \footnote{In the notation of Section \ref{s:resolv}, the difference of the two approximating operators on 
...
the approximations based on $\mathcal{I}_\ep$ and on the unfolding operator $\mathcal{T}_\ep$ 
(i.e. when  $\mathcal{I}_\ep$ is replaced in \eqref{dpcompe3} by $\mathcal{T}_\ep$) 
are $\ep^2$-close, i.e. 
\be
\label{TvsIep2}
\bigl\Vert\,\mathcal{I}_\ep^* \left(\mathcal{L}_0+I\right)^{-1} \mathcal{P} \mathcal{I}_\ep F \,-\,  
...
\ee
%Indeed, 
(We remark that the above estimate holds despite $\mathcal{I}_\ep$ and $\mathcal{T}_\ep$ not being 
...
class of examples. 
Still, we believe that our new two-scale interpolation operator $\mathcal{I}_\ep$ appears here most naturally. 
Indeed, %while 
the extension operator $\chi$ (being the prototype of $\mathcal{I}_\ep$) naturally appears in the abstract 
setting of Theorem \ref{thm.bivariate} for arbitrary $\Theta$. 
...
Remark finally that, like the unfolding operator, the new two-scale interpolation operator $\mathcal{I}_\ep$ provides an equivalence link 
...
$f_0\in L^2(\RR\times \square)$ if and only if $\,\mathcal{I}_\ep F_\ep$ weakly (resp strongly) two-scale converges to 
$f_0$. 
\end{remark}
\begin{remark}\label{remVal}
One potential disadvantage of the above constructed operator $\mathcal{I}_\ep$, suffered in fact also by $\mathcal{T}_\ep$,  
is that for a given $x$ e.g.  
$x\in \ep\,\mathbb{Z}^n$, even for smooth $F(x)$ it produces a discontinuity %in $y$  
on the boundary of $\square$ in the $\square$-periodic extension in $y$ of $(\mathcal{I}_\ep F)(x,y)=F(x+\ep y)$. 
%in the values of 
...
operator $\mathcal{P}$, see \eqref{p-cal-dp}, which in a sense regularises this discontinuity. 
Indeed from \eqref{7.54-1}, % and \eqref{p-cal-dp}, 
for $x=\ep l$, $l\in \mathbb{Z}^n$, and $y\in\square\backslash B$, 
$g(x):=\mathcal{P}\mathcal{I}_\ep F (x,y)=|\square\backslash B|^{-1}\int_{\square\backslash B}F(\ep l +\ep y'){\rm d}y'$ i.e. 
$g(\ep l)$ is the average of $F$ over the matrix part of the cube $\ep l+\ep\square$. 
In other  words, $g(\ep l)$ is (up to a pre-factor) the Steklov smoothing (see e.g. \cite{Zh-Pa2016})  at $x=\ep l$ of 
$\chi_\ep^m F$ where $\chi_\ep^m$ is the characteristic 
function of the matrix phase $M_\ep={\mathbb R}^n\backslash\bigcup_{m\in \mathbb{Z}^n}\ep(B+m)$. 
For $x\notin\ep\mathbb{Z}$, \eqref{7.54-1} implies that 
$g(x)$ is the (classical) Whittaker interpolation: 
$g(x)=\sum_{m\in \mathbb{Z}^n}g(\ep m)\,{\rm Sinc}(x/\ep-m)$. 
On the other hand, we notice that the operator $\mathcal{E}_\t$ could be chosen in a slightly modified form. 
...
Moreover, as directly follows from \eqref{p-cal-dp} and \eqref{I-ep-mod}, for $x\in\mathbb{R}^n$ and 
$y\in \square\backslash B$, 
\[
\mathcal{P}\mathcal{I}_\ep F (x,y)\,\,=\,\,
|\square\backslash B|^{-1}\int_{{\mathbb R}^n\backslash\bigcup_{m\in \mathbb{Z}^n}\ep(B+m)}
F(z)\,\,  
	\ep^{-n}\,{\rm Sinc}\left(\, \frac{x\,-\,z}{\ep}  \, \right). %\,\,=\,\, ,
\]
...
$\mathcal{P}\mathcal{I}_\ep F (x,y)=|\square\backslash B|^{-1}\mathcal{S}_\ep\chi_\ep^m F(x)$ where 
%$\chi_\ep^m$ is the characteristic 
%function of the matrix phase ${\mathbb R}^n\backslash\cup_{m\in \mathbb{Z}^n}\ep(B+m)$ and 
$\mathcal{S}_\ep$ is the smoothing operator \eqref{se2sc}. 
...
$u_0(x,y)=u(x)+v(x,y)$ and $\mathcal{I}_\ep^*u_0(x)=u(x)$ in the matrix phase $M_\ep=\RR^n\backslash B_\ep$ while 
$\mathcal{I}_\ep^*u_0(x)=u_0\Big(x-\ep\{x/\ep\}, \,\{x/\ep\}\Big)$ in the inclusion phase 
$B_\ep= \bigcup_{m\in \ZZ^n} \ep (B+m)$. 

Finally we emphasise again that, for both choices of $\mathcal{I}_\ep$, in the inclusion phase i.e. for $y\in B$ it has to be the same and as in
 \eqref{7.54-1}. As mentioned in Remark \ref{RemShann}, for regular enough two-scale inputs $F_\ep(x)=\Phi(x,x/\ep)$ this yields 
$\big(\mathcal{I}_\ep F_\ep\big)(x,y)=\Phi(x+\ep y, y)$. 
Although the $\ep y$ shift in the $x$-variable might look unnatural from the first sight, in fact it is not. -- 
In the two-scale limit problem \eqref{2sclimprob}, %determined by \eqref{q02sc}, 
...
$\big(\mathcal{I}_\ep F_\ep\big)(x,y)$. 
Any ``true'' inclusion centered at $x_l=\ep l$, $l\in \ZZ^n$, can be parametrised as $x=x_l+\ep y$, $y\in B$, and so for the related 
true right hand side $F_\ep(x)=\Phi(x,x/\ep)=\Phi\left(x_l+\ep y,\, y\right)$. For a fixed $x=x_l\in\ep\ZZ^n$ the latter coincides with the 
``shifted'' $\big(\mathcal{I}_\ep F_\ep\big)(x,y)$ as above, and for $x\notin\ep\ZZ^n$ these are smoothly interpolated. 
...
Notice that, consistently with Proposition \ref{prop:Zfinite}, this can only happen when the weaker hypothesis 
\eqref{KA} holds in 
the absence of its stronger version \eqref{KA2}. 
For that, we consider the  problem which resembles \eqref{nonlocalp} with the term involving derivatives being dropped. Namely, let  $u_\ep \in L^2(\RR)$ be the solution to the difference equation 
\begin{equation}
\label{nonlocalp2}
	\ep^{-2}	
	\int_{\RR} D\left(\tfrac{x}{\ep}\right) \big(  u_\ep(x + \ep) - u_\ep(x) \big)\,
...
and inspecting the proof of Theorem \ref{p.unitaryequiv} shows that on its own it does not actually require  the 
...
$$
The simplification in \eqref{splimSe.56} compared to Theorem \ref{p.unitaryequiv} is due to the facts that 
$\mathcal{E}_\t$ is chosen the identity operator,
%$\mathcal{E}_\t$ is the identity 
%(obviously \eqref{H6} then holds) 
and $\mathcal{H}_0:=\overline{V_0}=L^2(\Box)$ (and therefore  the projector appearing in Theorem \ref{p.unitaryequiv} is also the identity operator).
...
(in fact their proofs are simpler) since $Z=V_0=\mathcal{H}_0=\mathcal{H}$, $b_0=d_0$ and any basis in $V_0$ will serve the purpose.}
	\[
 \left\|\,\mathcal{L}_{\ep,\t}^{-1} \,\,-\,\, 
\big(B_\ep^* \mathcal{L}^{-1} B_\ep\big)(\t)\,\right\|_{L^2(\Box) \rightarrow L^2(\Box)}
\,\,\, \le\,\,  C\,\ep\,  ,	\quad  \quad 
for \,\,a.e.\ \t \in [-\pi,\pi]. 
	\]
Here $B_\ep^* \mathcal{L}^{-1} B_\ep$ is a self-adjoint 
...
\ee 
$B_\ep : L^2\big(\Theta \,; L^2(\Box)\big) \rightarrow L^2\big(\mathbb{R}\, ; L^2(\Box)\big) $ is the composition 
$B_\ep : = \Gamma_\ep^{-1}\mathcal{F}^{-1} \, \chi $ and 
...
\begin{theorem}\label{thm.2scOpResnonloc}
	...
	\begin{equation}
		...
		\mathcal{J}_\ep^* \mathcal{L}^{-1}  \mathcal{J}_\ep\, \bigr\Vert_{L^2(\mathbb{R}) \rightarrow L^2(\mathbb{R})} \,\,\le\,\, C\, \ep
	\end{equation}
for some positive constant $C$ independent of $\ep$.	Here  $\mathcal{J}_\ep: L^2(\mathbb{R}) \rightarrow L^2(\mathbb{R} \times \square)$, which is a modified ``two-scale interpolation operator'' (see  Remark \ref{remVal}, and Remark \ref{lastrem} below),
	...
	\end{equation}
$\mathcal{J}_\ep^*: L^2(\mathbb{R} \times \square) \rightarrow L^2(\mathbb{R}) $ is the adjoint of $\mathcal{J}_\ep$ given by   
...
The key properties of $\mathcal{J}_\ep$ are already encoded in its operator representation \eqref{2ScInterpNL}. 
For sufficiently regular $G\in L^2(\RR\times\square)$, let $\mathcal{R}_\ep$ denote the mapping 
$G(x,y)\mapsto G\big(x,\{x/\ep\}\big)$. We observe from the inversion formula \eqref{gt2} for Gelfand transform that 
$U^{-1}=\mathcal{R}_1\mathcal{F}^{-1}\chi$ which implies $\mathcal{R}_1\mathcal{F}^{-1}\chi U=I$. 
Next, since $\Gamma_\ep^{-1}\mathcal{R}_1=\mathcal{R}_\ep\Gamma_\ep^{-1}$, in combination with \eqref{2ScInterpNL} 
this yields 
\be
\label{2scIntOp}
\mathcal{R}_\ep\mathcal{J}_\ep\,\,=\,\,I,
\ee
 i.e. (for sufficiently regular $F\in L^2(\RR)$) 
$\mathcal{J}_\ep F\big(x,\{x/\ep\}\big)=F(x)$ which is a 
two-scale interpolation property of $\mathcal{J}_\ep$: if $x$ has ``phase'' $y$ i.e. $x=\ep l+y$ for $l\in\ZZ$ and $y\in\square$, 
then  simply $\mathcal{J}_\ep F\big(x,y\big)=F(x)$. 
Further, combining \eqref{2scIntOp} with the second identity in \eqref{calttstarNL}, one obtains 
$\mathcal{J}_\ep^*=\mathcal{R}_\ep\mathcal{S}_\ep$. Applying $\mathcal{J}_\ep$ to both sides and then 
using \eqref{calttstarNL} again results in  
...
for $\ep\le\pi/R$ simply $\mathcal{J}_\ep F_\ep(x)=\Phi(x,y)$. 

This allows a particularly simple specialisation of estimate \eqref{dpcompe3nl} 
for %a particular 
such a class of two-scale right hand sides $F_\ep(x)=\Phi(x,\,x/\ep)$. 
Indeed, 
...
%Hence, by the sampling theorem,   
%Then one can see that 
%for $\ep\le\pi/R$ simply $\mathcal{J}_\ep F_\ep(x)=\Phi(x,y)$, and 
as $\mathcal{L}^{-1}$ commutes with 
$\mathcal{S}_\ep$, for the solution 
$u=\mathcal{L}^{-1}\Phi$ of the two-scale limit problem $\mathcal{J}_\ep^*u(x)=u\big(x,\,x/\ep\big)$. 
...
We finally notice that Theorem \ref{thm.2scOpResnonloc} 
remains valid if we replace $\mathcal{J}_\ep$ by $\,\mathcal{I}_\ep$ as originally 
introduced in \eqref{2ScInterp} and \eqref{7.54-1}. 
Indeed, the only difference is the absence in \eqref{2ScInterpNL} of the operator $\mathcal{E}^{-1}$ of multiplication by 
...
thus arriving at \eqref{dpcompe3nl} with $\mathcal{I}_\ep$ instead of $\mathcal{J}_\ep$. %Of course, i
Notice however that, for the present example, the two resulting approximations coincide. % i.e. 
Indeed, e.g. comparing \eqref{7.54-1NL} and \eqref{7.54-1}, we observe that  
$\,\mathcal{I}_\ep F(x,y)=\mathcal{J}_\ep F(x+\ep y,y)$. 
Similarly, from \eqref{7.54-2NL} and \eqref{7.54-2},  for $\hat u(x,y)=u(x+\ep y,y)$ one has 
$\mathcal{J}_\ep^* \hat u(x)=\mathcal{I}_\ep^* u(x)$. 
...
$\,\mathcal{I}_\ep^* \mathcal{L}^{-1}  \mathcal{I}_\ep=\mathcal{J}_\ep^* \mathcal{L}^{-1}  \mathcal{J}_\ep$. 
We remind that in 
Section \ref{e.dp} %we did not have such a freedom: 
form $b_\theta$ in \eqref{abcdp} was genuinely $\theta$-dependent and so we could not have taken $\mathcal{E}_\t$ to be the identity operator.  This illustrates  that our  choice of the interpolation operator $\mathcal{I}_\ep$ appears more 
...
\bibitem{DGR23}{ Duerinckx, M., Gloria, A. and Ruf, M. (2023). 
\emph{A spectral ansatz for the long-time homogenization of the wave equation}. 
...
\bibitem{griso}{ Griso, G., 2004. Error estimate and unfolding for periodic 
homogenization. {\it Asymptotic Analysis},  40, 269--286. 
}
