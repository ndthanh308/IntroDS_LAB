...
The aim of this work is to develop a general approach for constructing approximations of two-scale type for 
a wide class of spectral problems whose limit behaviour remains intrinsically two-scale (and hence where the 
classical homogenisation fails), 
and to obtain on its basis new results on tight operator-type error estimates for resolvent and spectral convergence 
for such problems. 
With this aim, we study %the uniform asymptotics of solutions to 
an abstract family of asymptotically degenerating variational formulations. %problems. 
Those are natural generalisations of families of problems, in their weak form,  
emerging upon application of  
%in the course of 
%reformulating, via 
a rescaled Floquet-Bloch-Gelfand transform to resolvent problems for high-contrast elliptic PDEs with 
%highly oscillatory 
periodic coefficients.  
%of a `micro-resonant' type.  
An asymptotic analysis of these  models leads us to 
...
a certain `bivariate' operator which appears an abstract generalisation of 
the two-scale limit operators %emerging via two-scale convergence methods 
for highly oscillatory high-contrast PDEs.  
The resulting approximating self-adjoint operator, providing tight operator error estimates, 
is the bivariate operator's resolvent sandwiched %accompanied 
by an isometric connecting operator and its adjoint. 
For a broad class of periodic %high-contrast 
problems, 
the connecting operator specialises to a composition of a problem-specific translation operator 
with a two-scale interpolation operator which is a 
%), which for a broad class of periodic %high-contrast 
%problems specialises to a 
%new 
two-scale version of the classical Whittaker-Shannon interpolation. 
An explicit  description of the limit spectrum in the abstract setting is provided, and new  tight 
error estimates on the distance between the original and limit spectra are established. 
...
allows us to readily consider a wide class of asymptotically degenerating problems including but also going beyond high-contrast %highly oscillatory 
PDEs. 
In particular, for a key model example of high-contrast isolated periodic inclusions, we obtain new results on the rate of uniform 
...
The %obtained 
general results are also illustrated by numerous other %highly oscillatory PDE 
examples,  
...
One of the main broader motivations for the present study comes from the desire for construction of tractable but accurate approximations of 
two-scale type, with quantifiably % controllably 
...
and \cite{ChErKi} for dispersive effects in high-contrast micro-resonant media. 
...
\cite{%Ch-V-2021, 
Ch-V-Z-2023}. 
For a recent review of the high contrast locally resonant materials from point of view of metamaterial modelling see e.g. 
...
{\it Quantifying} %for 
such a convergence however, in a manner {\it uniform} with respect to all possible inputs $F(x)$, typically %often % arguably 
requires applying and developing fundamentally new operator tools, and this is one of the broader ultimate aims of 
%what we ultimately aim at by %in 
the present work. 
Indeed, for any small but fixed $\ep>0$, given an input $F(x)$ with associated solution $u_\ep(x)$, one needs approximating 
the latter in terms of solution $u(x,y)$ of the two-scale limit problem with appropriately chosen two-scale input 
$f(x,y)$. In other words one needs recasting, in a suitable way, any $F(x)$ as a two-scale function $f(x,y)$ i.e. to 
...
to the exact solution $u_\ep(x)$, desirably %which is 
...
approximation, which is in turn (together with ensuring certain asymptotic unitarity % isometry 
properties of  
$\mathcal{J}_\ep$) crucial for {\it quantifying a spectral convergence} 
 of an original operator to the two-scale limit one. 
...
%including 
of its asymptotic spectral properties as $\ep\to 0$ lies in analysing the related resolvent problem  
\begin{equation} 
\label{hom}
...
For {\it uniformly quantifying} the above convergences, i.e. establishing their (uniform) rates, one %first 
...
problem \eqref{hom-2sc} which would be {\it uniformly quantifiable  in $F$}. 
...
$\mathcal{R}_0\mathcal{P}$ sandwiched by such an ($L^2$-isometric and ``asymptotically unitary") operator 
...
%This  estimate 
%which 
can be viewed as that on the {\it rate} of an operator two-scale resolvent convergence and reads 
\begin{equation}
\label{2scresest}
\big\Vert\, \,\mathcal{R}_\ep F\,\,-\,\,\mathcal{J}_\ep^*\mathcal{R}_0\mathcal{P}\mathcal{J}_\ep F\,\big\Vert_{L^2\left(\mathbb{R}^n\right)}
\,\,\,\le\,\,\,C\,\ep\,\big\Vert\,F\,\big\Vert_{L^2\left(\mathbb{R}^n\right)}, \ \ \ \ \forall F\in L^2\left(\mathbb{R}^n\right), 
\end{equation}
with a constant $C$ independent of both $\ep$ and $F$. 
Notice that, importantly, the approximating operator $\mathcal{J}_\ep^*\mathcal{R}_0\mathcal{P}\mathcal{J}_\ep$ in \eqref{2scresest}  %which is 
...
to that of the two-scale limit operator $\mathcal{L}_0$ 
and on the associated band gaps near the resonances, 
Theorem \ref{EVestDP} and Corollary \ref{HempLien-rate}. 
The general spectral approach implies 
%The two-scale resolvent estimates like \eqref{2scresest} imply 
also certain explicit two-scale approximations with estimates on the convergence rates for 
the Floquet-Bloch dispersion relations and for the related 
%the 
eigenfunctions (Bloch waves),   Remark \ref{EFestDP}. 
These uniform estimates appear to %be robust enough to 
also provide new results on the asymptotics of the integrated density of states (Corollary \eqref{denstates}), and 
some information % imply results 
on ranges of validity and failure of classical 
homogenisation for model \eqref{th-hc} on approaching the critical scaling, 
...
discussed example. %, and Remark \ref{H1-2slp}. 
\vspace{.05in} 
...
The two-scale connecting operator $\mathcal{J}_\ep$ appears in \eqref{2scresest} as a specialisation 
...
and is usually 
naturally identified from the problem. It plays a key %an important 
...
transform, see \eqref{2ScInterp}, implying it is an ``asymptotically unitary'' $L^2$-isometry and 
has all kinds of desirable properties. 
In particular, 
$(\mathcal{I}_\ep F)(x, x/\ep)=f(x, x/\ep)=F(x)$ where regular enough $f(x,y)$ is understood as 
...
Moreover, the resolvent problems %like \eqref{hom} 
often also hold keys for analysing related (two-scale) evolution problems, both parabolic and hyperbolic, cf. e.g. \cite{ZhP07,Pas05,IVKVPS13} for related results although only for two-scale convergences without rates. 
...
approximations for a wider class of asymptotically degenerating problems akin to \eqref{hom}, %, with respect to $\ep$,  for 
...
not %even 
attempt to provide a review of all these %different 
methods  here 
except to mention one %approach 
of particular relevance to this work: the so-called {\it spectral method}, see 
for example \cite{Zh89,CoVa97,BiSu,ZhSpectr}, and also \cite{DGR23} and further references therein for most recent developments of the spectral approach for long-time homogenisation problems in both periodic and stochastic settings. 
...
vanishing at %becoming %asymptotically 
%degenerate at 
non-trivial (in fact 
...
variational problems and of related operators near 
%the related key 
certain special degeneracy points of the parameter. % $\t$. % in (an abstract analog of) the `quasi-periodicity' parameter. 
This ultimately allows, in particular, to naturally arrive at 
an approximation to the exact solution operator in terms of abstract analogues of resolvent of the two-scale limit operator 
...
The general nature of our approach implies that it can potentially be used %on its own, i.e. 
...
In \cite{ChErKi} and \cite{ChKiVeZu23} approximations with improved error estimates, although still in terms of 
...
One of the main wider aims of this article is %also  
...
%Now, in this article, 
Next, we observe that the equivalent weak formulation of problem \eqref{hom2} is of the following more abstract type: 
\begin{equation}
...
that is why one can refer to such variational problems as {\it 
...
few generic features of this abstract family of asymptotically degenerating variational problems for which two-scale type approximations with error estimates can be constructed. 
This naturally leads to an idea of investigating \eqref{pr} under minimal abstract assumptions, thereby hoping to cover a 
...
Section \ref{e:idp}), with resulting approximation (accompanied by %new operator 
...
The limit problem appears to be a ``two-phase'' macroscopic (rather than a more general two-scale) one,  
...
demonstrating the additional powers brought in by the generality of the abstract approach. Indeed, in this case %parameter 
$\t$ is %can be 
{\it not} anymore %(only) 
the quasi-periodicity parameter, i.e. $\Theta\ne\square^*$ and contains an additional component accounting for the 
%additional 
parameter of 
...
-- Linear elasticity system with  inclusions which are `partially degenerating', i.e. whose elasticity tensor degenerates on only some of 
...
necessarily be the  Floquet-Bloch parameter (cf. the concentrated perturbation example of Section \ref{sec:concpert})  or even for $H$ to be a function space. This suggests possible far-reaching consequences of the present approach that 
...
see Theorem \ref{thm:contV}. This simple result not only forms the basis for further investigation, it appears applicable to certain physically relevant models, for example,  the stiff inclusion or { inverted } high contrast model of Example \ref{e:idp}  and in the study of certain polarisations of electromagnetic waves in photonic crystal fibers, cf. \cite{Cothesis,CoKaSmPCF}. 
...
in $\xi$ and non-degenerate on $Z$. 
%for $\xi\ne 0$. 
Form $a_\xi^{\rm h}$ appears to  generalise the homogenised tensor in classical homogenisation problems, and $Z$ 
...
Then \eqref{prspec} is a spectral problem for a positive self-adjoint operator $\mathcal{L}_{\ep,\t}$ in $\mathcal{H}$, 
...
or equivalently in terms of an 
...
the norms  $\| \cdot \|_\t : = (\cdot,\cdot)_\t^{1/2}$ 
...
\end{equation}
where %henceforth  
$\mathfrak{b}[v]:=\mathfrak{b}(v,v)$ for a sesquilinear form $\mathfrak{b}$. 
Notice that,  
...
\end{aligned} 
\end{equation}
We emphasise that the above gap condition is generally {\it non-uniform}: in fact, in most of the interesting examples,
 $\inf_{\t\in\Theta}\,\nu_\t=0$. 
...
\begin{remark}
\label{spgap}
To clarify why \eqref{KA} can be regarded a spectral gap condition, notice that 
for every $\t\in\Theta$ the form $a_\t$ defines a non-negative bounded self-adjoint operator in $H$. Condition \eqref{KA} implies together with \eqref{astructure} that the spectrum of this operator is contained in 
...
\begin{remark}
		...
		\end{equation}
In particular, we will see that  \eqref{KA2.1} is self-evident in the context of classical homogenisation, although 
already requires employing certain extension theorems for ``non-classical'' high-contrast models. 
In Section \ref{sec.newKA}, we shall see that \eqref{KA2.1} does not only imply \eqref{KA} but has other important implications. 
Condition \eqref{KA2.1} can be interpreted as the forms $a_\t$ are ``uniformly coercive on $W_\t$ plus compact''.
\end{remark}
...
property \eqref{VtLip} holds. An intuition behind is that the $\t$-continuity property \eqref{ass.alip} of $a_\t$ implies certain regular behaviour 
...
Fix $\t_1, \t_2 \in \Theta$ and 
$v_1 \in V_{\t_1}$. By \eqref{bddspec}, the definition of $P_{W_{\t_2}}$, \eqref{Vkersesa} 
...
$(\cdot,\cdot)_{\t_0}$. We will show that both terms on the right of \eqref{6520e} 
...
\label{section:discV}
Typically, in applications (see Section \ref{sec:examples}) the spaces $V_\t$  violate \eqref{VtLip} and have  isolated discontinuities. 
...
often characterise the key asymptotic properties of the solutions, are possible near those discontinuity points. 
...
\label{sect4.1}
While Theorem \ref{thm:contV} is not anymore %directly 
applicable, the %method of proof 
key idea behind it is. 
 The essential part of Theorem \ref{thm:contV}
was to identify 
$\ep$-independent subspaces $W_\t$ of $H$ on which $a_\t$ is uniformly coercive and to restrict problem \eqref{p1} to 
orthogonal complements of $W_\t$ with respect to $A_{\ep,\t}$, (also $\ep$-independent) $V_\t$. 
...
Staring with the former, as %latter, as 
a consequence of \eqref{KA} held at $\t=\theta_0=0$ together with the continuity of $a_\t$ due to 
\eqref{ass.alip}, we observe that $a_\t$ remains uniformly coercive on $W_0$ 
...
\end{equation}
Problem \eqref{IliaN} is an abstract analog of the cell problem. It is well-posed for $|\theta|\leq \tfrac{1}{2} \nu_0 L_a^{-1}$ by Proposition \ref{Wt2coerc}, 
...
So for proving \eqref{ik43000} one needs to bound $\| w\|_\t$ in terms of $A^{1/2}[r]$. 
...
V_\star \subset V_0.
$
Indeed, for $v_\star\in V_\star$ and $\t\in\Theta$, $\t\neq 0$ and $\t\to 0$, by \eqref{ass.alip}, the definition of $P_{W_\t}$, \eqref{astructure} and  \eqref{contVs2} %(for $\t_1 =0$ and $\t_2=\t$) 
we obtain 
\[
...
via \eqref{as.b1} and \eqref{astructure}, 
...
To maintain the higher accuracy, one would need to replace the simplified approximate problem \eqref{p.trunc1} by a 
more complicated one which we do not pursue here.}:
	\begin{align}
	\label{trunc1.e2}
...
\big\vert a_\t(v ,\, \N \tilde{v}-N_\t \tilde{v})\big\vert \,\,\le\,\,
 L_a |\t|\, \| v\|_0\, 
\big\vert  \N \tilde{v} \,-\, N_\t \tilde{v}\big\vert_0 \,\,\le\,\,\, L_a\, \kappa_2\, |\t|^3\, \| v\|_0\, \| \tilde{v} \|_0. 
...
subspace $V_\t^\star\dot{+}Z$, and replacement of the singular form $a_\t$ by the ``homogenised form'' $a_\t^h$  which has a quadratic dependence on $\t$ (as well as is restricted further to the defect subspace $Z$ only). The dependence of $b_\t$ on 
$\t$ remains so far unspecified, however as we will later see, if it were possible to approximate it for small $\t$ by a 
$\t$-independent $b_0$ (as well as to approximate $V_\t^\star$ by $\t$-independent $V_\star$) that would provide significant additional benefits for properties of the approximate problem. 
...
%The assertions 
Since $b_0[\cdot] = \| \cdot \|_0^2$ on $V_\star \dot{+}Z$, %\eqref{VZorth2}, 
and %the  fact that 
...
Recall \eqref{KA2.1} from Remark \ref{r.oldkafromnewka}, that is  there exists $C >0 $ and a  non-negative sesquilinear form $c$, $\|\cdot\|_\t$-compact\footnote{A form $c$ is $\|\cdot\|_\t$-compact if every sequence $\{u_n\}$ in $H$, bounded in $\|\cdot\|_\t$, has a convergent subsequence $\left\{u_{n_k}\right\}$ with respect to $c$, i.e. $c\left[u_{n_k}-u \right]\to 0$ as $k\to \infty$ for some $u$. Notice that, by the uniform equivalence of the norms $\| \cdot \|_\t$, the  compactness needs only be established for one $\t$ to hold for all $\t$.} for all $\t\in \Theta$, such that
\begin{equation}\tag{H1$^\prime$}
\label{KA2}
\|w\|_\t^2\,\,\, \le\,\,\, C a_\t[w] \,\,+\,\, c[w], \quad \forall w \in W_\t, \; \, \forall \t \in \Theta.
\end{equation}
...
In principle, it can be relaxed, cf Section \ref{sec:nonloc2}. } 
...
quite inexplicit due to the presence of (generally non-isometric) operator $G_\t$, 
and is generally not close to the spectrum of $\mathbb{L}_{\t / \ep}^{-1}$. 
...
spectrum ${\rm Sp}_0$, in particular in its gap, 
...
Indeed, fixing $\lambda\in\mathcal{C}$ and $\eta\in S^{n-1}$, for $\xi =t\eta$ with $t\ge 0$ 
...
problem: 
$\mathcal{B}_{\lambda,\eta}\hat z:=\left(A_\eta^{\rm h}\right)^{-1/2}B_\lambda\left(A_\eta^{\rm h}\right)^{-1/2} \hat z= t^2\hat z$ for eigenvalues $t^2\ge 0$. 
Hence, for every non-negative eigenvalue $k\ge 0$ of $\mathcal{B}_{\lambda,\eta}$ with associated eigenvector $\hat z$, 
...
Floquet-Bloch transform) $U :L^2(\RR^n) \rightarrow L^2(\square^* \times \square)$ and its inverse are unitary maps that can be defined, for example, as the continuous extensions of the ($L^2$-isometric) mappings
\be
\label{gt1}
...
sandwiched by $L^2$-isometric two-scale connecting operators 
(which are in turn key for novel spectral estimates in Section \ref{specestdp}). 
...
apart from losing the self-adjointeness, could not deliver an estimate like \eqref{dpcompe3} either. 
...
Notice that this is exactly what would be consistent with the reduced $H^1$ estimates like \eqref{dp.H1est}; 
...
Namely, $\left(\mathcal{G}_\ep F\right)=\widehat T_\ep^*\mathcal{P}\mathcal{I}_\ep$ where projector $\mathcal{P}$ is given 
...
\left(\mathcal{S}_\ep F\right)(x)\,-\,\int_B\mathcal{I}_\ep\big(x+\ep y',y'\big){\rm d}y'\right\}\,\,+\,\,
\chi_B(y)\,\mathcal{I}_\ep(x+\ep y,y). 
\]
%$(x,y)=h(x)+q(x,y)$ where 
...
We also observe that $u_\ep^{\rm appr}=\mathcal{G}_\ep^*u_0$ where $\mathcal{G}_\ep^*$ is the adjoint of $\mathcal{G}_\ep$. 
...
\subsubsection{Error estimates on the rate of convergence of the spectrum} 
\label{specestdp}
Finally, the following important %new 
results on the rate of convergence %approximation 
of the spectrum hold by adopting our general Theorem \ref{bivariate.spec} to the present example. 
...
with the gap in effect closed in the proof of Theorem \ref{t.collectivespec}.) 
...
for arbitrary Hermitian matrix coefficients as mentioned below \eqref{aepdp}. 
...
problem. 
Namely, cf Remark \ref{remdisprel}, either $\lambda_\xi^{(k)}\in {\rm Sp}\,\left(-\Delta_{H^1_0(B)}\right) $ with 
$\psi_{\xi}^{(k)}$ the associated eigenfunctions of $-\Delta_{H^1_0(B)}$, or 
%$\lambda_\xi^{(k)}$ solves 
...
Notice that, from variational arguments, $\lambda_{\ep,\t}^{(k-1)}\le\lambda_{k-1}\le\mu_k$, 
and for $\ep<\ep_0$, $\lambda_{\ep,\t}^{(k+1)}> \lambda_{\t/\ep}^{(k+1)}-C\ep\ge\mu_{k+1}-C\ep>\lambda_k$. 
...
%and so 
$m_\tau(\lambda)=k-1+(2\pi)^{-n}{meas}\left(S_\ep(\lambda)\right)$ where $S_\ep(\lambda):= 
...
${meas}\left(E_\ep(\lambda)\right)=\omega_n\ep^n\left[\beta_B(\lambda)\right]^{n/2} %$ $
...
Notice that if $\t\in E_\ep(\lambda-C\ep)\subset E_\ep(\lambda)$ then $\lambda^{(k)}_{\t/\ep}\le \lambda-C\ep$, and 
...
{\left({\rm det} A^{\rm hom}_{\rm pd}\right)^{1/2}}\,\,\le\,\,meas\left(S_\ep(\lambda)\right)\,\,\le\,\,
...
If $\ep\ge \pi\left(\lambda^{\rm h}_{\rm min}\right)^{1/2}\beta_B^{-1/2}(\lambda) $ then since $m_\tau(\lambda)\le k$, 
...
classical homogenisation approximations for the model \eqref{th-hc} with an increasingly high contrast 
$\tau=\delta^{-1}$. 
...
For homogenisation of problems with imperfect interfaces, including the spectral problems, see e.g. 
recent works \cite{Donato21} and \cite{Avila.J-24} and further references therein. 
Here we apply our approach to obtain new operator error estimates for such problems. 
...
\bibitem{ArmKuu24}{Armstrong, S., and Kuusi, T., 2024. Renormalization group and elliptic homogenization in high contrast {\it Arxiv preprint } https://arxiv.org/pdf/2405.10732}
...
Arch Mech [Archiwum Mechaniki Stosowanej] 1, 63--90.
...
\bibitem{Avila.J-24} Avila, J.,  Monsurr\`{o}, S. and  Raimondi, F., 2014. 
