...
\begin{assumption}\label{def:ddh} The \emph{decisional Diffie-Hellman assumption} (DDH) holds on a group action $(\G,\Xs,*)$ if, for all QPT adversaries $\As$, there exists a negligible $\lambda$ such that 
	\[\left|\Pr\left[\As(a*x_\lambda,b*x_\lambda,c*x_\lambda)=1: a,b,c\gets\Gs_\lambda\right]-\Pr\left[\As(a*x_\lambda,b*x_\lambda,(a+b)*x_\lambda)=1: a,b\gets\Gs_\lambda\right]\right|\leq\negl(\lambda)\enspace .\]
\end{assumption}
