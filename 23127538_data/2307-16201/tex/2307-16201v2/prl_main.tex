\documentclass[aps,prl,twocolumn,superscriptaddress,showpacs,
...
It is now more than 50 years since Bekenstein analyzed what happens upon throwing a cup of tea into a black hole. Based on this, he proposed that black holes should carry entropy proportional to their horizon area \cite{Bekenstein:1973}. With the constant of proportionality fixed by Hawking's calculation \cite{Hawking:1975}, the Bekenstein area law states that the black hole entropy is given by {\it  one quarter of the horizon area} (using in what follows the natural units where  $G_N=c=\hbar=k_B=1$):
\be\label{Alaw}
S=\frac{\mbox{Area}}{4}\,.
\ee 
This is how the laws of black hole mechanics  \cite{Bardeen:1973} became the laws of black hole thermodynamics.
...
While a change of Wald entropy due to addition of a surface term to the action appears unusual, such a situation is not without a precedent -- it has been known for a long time that by adding the Gauss--Bonnet density in 4D (where it is a surface term) the black hole entropy changes by a universal constant proportional to the Euler characteristic of the horizon \cite{Jacobson:1993xs}. Here the effect just becomes more intricate due to the dependence on the scalar field. Interestingly, the improved action also leads to a modified (different from surface gravity) Hawking temperature via the Euclidean grand canonical ensemble Brown--York procedure \cite{Braden:1990hw, Brown:1992bq}, restoring the validity of the first law.

In what follows we i) show that the area law prevails for black holes in 4D Gauss--Bonnet gravity 
and their temperature gets accordingly modified when the shift symmetry of the theory is preserved at the level of the action, and ii) observe that the area law leads to a simplified thermodynamics of `spherical' black holes in higher-dimensional Lovelock gravity 
...
\end{equation}
Here, $M$ and $Q$ stands for the mass and charge, respectively, $\Lambda$ is the bare cosmological constant, $r_+$ is the horizon radius, and $L$ is an arbitrary integration constant with dimensions of length. In this case, the Smarr formula and first law are, respectively
\ba
