...
%	\preprint{\parbox{3cm}{OU-HET}}
	\begin{document}
		...
		For example, what is the meaning and interpretation of wave function of the universe? Who can observe its consequences, i.e., who are well defined observables? And   what are acceptable wave functions?  
...
%In section \ref{concs}, we end with conclusion and some open questions. Some additional related are presented in appendices.
		...
		\be
		\label{top}
		I_{\text{top}}={\phi_0\over 16 \pi G}  \left(\int d^2\sqrt{-g} R -2 \int_{bdy}\sqrt{-\gamma} K\right)
		\ee
		with 
...
These solutions are characterised by two parameters, $m$ and $A$. 
...
0< x\le 1
\ee
The resulting spacetime consists of a cone in the the upper and lower Milne patches of the black hole geometry, see fig.\ref{dsfig}(b), with an orbifold singularity at $r=0$.
...
As mentioned above the dilaton is then related to the volume of the transverse sphere $S^2$ which cannot become negative. 
  ...
  In the subsequent discussion we take the constant $A$ appearing in the dilaton solution, eq.(\ref{globalds}) and eq.(\ref{dsmilne}) to be positive. 
...
Next we turn to quantise the theory. 
	...
	\end{align}
	The constraints eq.\eqref{hmwfcon} imply that the physical wavefunction is invariant under these residual gauge transformations.  
	...
	\end{align}
	where we used the fact that $U(\phi)=2\phi$ for the case of dS spacetime under consideration. 
...
\label{minquant}
In the previous subsection we have obtained the general wavefunction satisyfing the gauge  constraints. 
...
\be
\label{nearl}
{\hat \Psi}  \simeq  {1\over l} \int _{-M_0}^{\phi^2}(\rho+{\tilde \rho}) dM +\frac{1}{l}\int_{\phi^2}^{M_0} (\rho_1+\rho_2)dM
 -i \int_{-M_0}^{\phi^2}( \rho-{\tilde \rho}) \sqrt{\phi^2-M} dM -\int_{\phi^2}^{M_0} (\rho_1- \rho_2)\sqrt{M-\phi^2} dM 
\ee
...
We also take $\rho$ to be real. From eq.(\ref{conr2})we then have $\rho_1=\rho, \forall 0<M<M_0$ . Finally we also assume that eq.(\ref{condrr})  is met. 
...
For earlier times the norm is not conserved and a key property for a satisfactory notion of physical time is therefore  not being met. 
...
In general  it is quite non-trivial to decide whether $p(l, \phi)$ can be made  positive everywhere, even on one fixed $\phi$ slice.
 ...
 where the integral is at a constant value of $\phi$. 
  ...
  \be
  \label{conjmom}
  \langle \pi_l\rangle = -{1\over 2}\int dl {\hat \Psi}^* (-i  (\overrightarrow{\partial}-\overleftarrow{\partial})) (-i  (\overrightarrow{\partial}-\overleftarrow{\partial})) \Psi,
  \ee
  more generally, 
  \be
  \label{mommom}
...
% ({\bf ** please check this **}). 
  ...
  It is also difficult to carry out the full JT gravity path integral, other than in the asymptotic region, see \cite{Verlinde:2020zld} fore some related work.
...
\subsection{Gaussian Coefficient Function}
\label{gausswf1}
...
\be
	\hat{\Psi}={1\over l}  \int_{-\infty}^{\phi^2} dM \,\rho(M)\,\, e^{-il\sqrt{\phi^2-M}} + \int_{\phi^2}^\infty dM \rho_1(M) e^{-l \sqrt{M-\phi^2}} \label{pshrhsh}
\ee
...
% 
 ...
 We also see that ${\cal C}_N$, eq.\eqref{normccond}, which can be obtained from eq.(\ref{smgapsi}), vanishes. As a result  there is no ``leakage" of the probability current as $l\rightarrow 0$. 
...
\end{align}
 ...
 This integral is  of the form  discussed in subsection \ref{gausswf1}, in terms of the shifted variables, ${\tilde M}, {\tilde\phi}$. 
...
This result is in accord with section \ref{cllimiwf} where  we had mentioned that the solutions $e^{\pm i l \sqrt{\phi^2-M}}$  should be interpreted as being eigenstates of 
 ${\hat M}$ with eigenvalue $M$. Eq.(\ref{bvalm0}) is then expected since the  the coefficient function eq.(\ref{rhomgs})  is a  Gaussian superposition  of various values of $M$ centered around $-M_0$. In appendix \ref{expvalm}, we give more details on the computation of this expectation value for more general coefficient functions. 
...
Finally, note that we could have considered a Gaussian with the opposite sign for the shift, $M_0$. 
...
This is consistent with what we see in the  the quantum theory, where the   classical limit arises  at much later times, for values of $\phi$ which are larger than $\sqrt{\sigma +M_0}$, eq.(\ref{condahs}). 
...
In fact by the time $\phi^2=10 M_0$ we find   that $p(l)$ is positive for all $l$, see Fig \ref{CDSphi}. 
{% Figure environment removed}
...
It would be interesting to explore other possible physical clocks, besides the dilaton, to understand how different is the resulting description. One such possibility is to use the extrinsic curvature, $K$,  of a spatial slice.{ In ADM gauge, eq.\eqref{adm} this is given by 
	\begin{align}
...
We thank Sumit Das, Abhijit Gadde, Alok Laddha, Andrei Linde, Gautam Mandal, Shiraz Minwalla and  Onkar Parrikar for discussions. 
...
Let us begin by computing the symplectic two form in JT gravity. The first step is to compute the pre-symplectic one form, which is the the set of boundary terms that arises when deriving the equations of motion. The next step is to construct a two-form current by taking a second variation of this pre-symplectic one-form. This current is then integrated on a Cauchy slice to give the required symplectic two-form. The JT action, in units of $8\pi G=1$ is given by,
%\begin{equation}
...
\Psi\sim \Tr(e^{-i{\hat P}})
\ee
Note in this context  that for a constant  value of $A$, $P$ and $M$ and proportional to each other, eq.(\ref{admmvsm}),\eqref{psiexexp}.
...
\end{align}
The WKB expansion in our case corresponds to a $G$ expansion of the exponent $S$. Let us consider the following expansion
\begin{align}
...
\label{valdsadap}
We shall now show that the saddle point analysis carried out in the example in subsection \ref{gausswf1} is indeed a good approximation. Consider a wavefunction of the form  
\begin{align}
...
We shall show this is the case in various regions of $l$. For the wavefunction considered in subsection \ref{gausswf1}, the exponent is given by 
\begin{align}
...
\end{equation}
which agrees with eq.\eqref{normdefwf}. Similarly, the mean $\langle \hat{M}\rangle$ and variance $\sqrt{\langle\hat{M}^2\rangle - \langle \hat{M}^2\rangle}$ are given by,
\begin{align}
	& \langle \hat{M}\rangle = -\frac{1}{2} (M_1+M_2) \label{meanM}\\
	& \Delta M = \sqrt{\langle\hat{M}^2\rangle - \langle \hat{M}^2\rangle} = \frac{\abs{M_1-M_2}}{2 \sqrt{3}} \label{varM}.
\end{align}
