%Translation surfaces started to naturally arise in connection with topology, algebraic geometry, dynamics and number theory since the work of Veech \cite{Veech}, Masur \cite{Masur82} and Thurston \cite{Thurston}. Nowadays, the topology of their moduli spaces is an active area of research.  
...
Kontsevich-Zorich showed that each stratum has at most $3$ connected components and in every genus some components are \textit{hyperelliptic} \cite{Kontsevich2003}. Namely, hyperelliptic components consist of translation surfaces $(X,\omega)$ where $X$ is a hyperelliptic Riemann surface and $\iota^*(X,\omega)=(X,-\omega)$ where $\iota$ is the hyperelliptic involution of $X$. Looijenga-Mondello showed that these connected components are orbifold classifying spaces for finite extensions of braid groups \cite[Section 1.4]{Looijenga2014}. Kontsevich-Zorich also conjectured that the other non-hyperelliptic components have orbifold fundamental groups commensurable with some mapping class group \cite{Kontsevich1997}; this conjecture is still open.
...
\textbf{The kernel of the punctured monodromy.} If $\mathcal{C}$ is hyperelliptic then $\operatorname{Im}\rho_\mathcal{C}$ is isomorphic to the symmetric mapping class group $\operatorname{SMod}_{g,n}$, and $\ker\rho_\mathcal{C}$ is finite \cite[Section 2.1]{CalderonConnected2020}. In view of Kontsevich and Zorich's conjecture, it is natural to ask whether or not the topological monodromy is the right homomorphism to look at in order to prove the conjecture. For this reason, we are interested in estimating the size of the kernels of the monodromies $\rho_\mathcal{C}$ for non-hyperelliptic connected components. The first main result of this paper is that in some cases the kernel is large.
...
Wajnryb proved that if the graph $\Gamma$ contains $E_6$ as a subgraph, any geometric homomorphism cannot be an injection \cite{Wajnryb1999}. In particular, Wajnryb found an element $w$ given explicitly in terms of the standard generators in the presentation (\ref{presArt}) and adopted the following strategy: as every inclusion of complete graphs induces a monomorphism of the respective Artin groups \cite{lek}, it is enough to find a non-trivial element $w$ in $A(E_6)$ which can be written in $\operatorname{Mod}_{3,1}$ as a braid relator of Dehn twists. The group $A(E_6)$ is a spherical-type Artin group, a class of groups for which the word problem has been solved by means of their \textit{Garside structure}. Our next result builds on Wajnryb's result and Theorem \ref{ker} can be thought of as a corollary of the following theorem.
...
\textbf{Structure of the paper} The paper is organized into five sections. In Section $1$ we describe the additional length graph associated with the Garside structure of a spherical-type Artin group $A(\Gamma)$, which serves as $\delta$-hyperbolic metric space for the acylindrical action of $A(\Gamma)$. In Section $2$ we define geometric homomorphism and describe the Wajnryb element, while in Section $3$ we prove Theorem \ref{thmb}. In Sections $4$ and $5$ we draw the consequences that the existence of a non-abelian free group of rank $2$ implies for the topological monodromy of strata of abelian differentials. In particular, in Section $5$ we explain how to prove Theorem \ref{ker} from Theorems \ref{thmb} and \ref{main}.

\textbf{Acknowledgments.} I am grateful for the patient guidance and support provided by my supervisors Tara Brendle and Vaibhav Gadre throughout working on this paper. I would also like to thank Aaron Calderon for the many helpful conversations, as well as Matt Bainbridge and Bradley Zykoski for patiently answering my questions. I am also grateful to Aaron Calderon and Nick Salter for pointing out the relation between versal deformation spaces of plane curve singularities and the flat geometry in genus $3$, which is the content of their joint work with Pablo Portilla Cuadrado. Finally, I would like to thank 
Aitor Azemar, Philipp Bader, Jim Belk, Rachael Boyd, Tudur Lewis, Francesco Pagliuca and Franco Rota for their comments and suggestions.