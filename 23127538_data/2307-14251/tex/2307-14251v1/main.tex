...
\title{\Large\textbf{Harmonic Oscillator with a Step and Isospectrality}}
...
\begin{abstract}
We investigate the one-dimensional Schr\"{o}dinger equation with a harmonic oscillator with a finite jump $a$ at the origin.
...
In this context, infinitely many number of isospectral Hamiltonians to the ordinary harmonic oscillator are obtained.
\end{abstract}

\keywords{
Exactly solvable models, Shcr\"{o}dinger equation, mathcing of wavefunctions, Hermite polynomials, isospectral Hamiltonians
}
...
Another major class is analytical potentials.
...
Moreover, the symmetrized Morse-potential short-range interaction~\cite{doi:10.1142/S0217732316500887}, the sheared harmonic oscillator~\cite{doi:10.1021/j100356a004}, the inverse square root potential~\cite{Ishkhanyan_2015}, and other potentials such as $-g^2\exp(-|x|)$-potential~\cite{Sasaki_2016}, a double well potential $\min[(x+d)^2,(x-d)^2]$ and its dual single well potential $\max[(x+d)^2,(x-d)^2]$~\cite{quantum4030022,10.1063/5.0127371} are all defined piecewise and solved by the matching of wavefunctions.

Yet another example exists.
A Dirac delta function at the origin under the harmonic oscillator~\cite{Viana-Gomes_2011} has been discussed.
The energy spectrum of this system is identical to that of the ordinary harmonic oscillator for odd-parity states, while it is not trivial for even-parity states.
Here, for the odd-parity states, the wavefunctions are also the same as the ordinary harmonic oscillator.
This potential can be abstracted as the harmonic oscillator plus a singularity function (For the singularity function, see, \textit{e.g.}, Ref.\,\cite{lighthill_1958}).

In this paper, we discuss the harmonic oscillator with a Heaviside step function, which is a member of the singularity function:
\begin{equation}
V(x) = \begin{cases}
...
On the other hand, for the case of $\omega=0$, the potential is exactly solvable and describes the free electron in metals.
If one takes the limit $a\to -\infty$, the potential $V(x)$ goes to the harmonic oscillator on the positive half line.

The essential isospectrality of our potential to the harmonic oscillator indicates that there is possibility to construct potentials \textit{strictly} isospectral to the harmonic oscillator as many as the choices of such $a$ above.
We shall examine this later in this paper.
Strictly isospectral deformations are of importance not only in its own interest but also in relation to the modelings of physical phenomena with certain energy spectra.
...
Sec.\,\ref{sec:iso} constitutes the starring part of this paper, where several isospectral deformations through the Darboux transformation are calculated.
We show that we can construct infinitely many number of potentials whose spectrum is totally identical to that of the ordinary harmonic oscillator.
...
The $n$-th excited-state energy eigenvalues for Eq.\,\eqref{eq:SE4l}, $E_n=2(n-\ell)$, where $n$ is greater than $\ell$, and the corresponding eigenfunctions are
\begin{equation}
\psi_n(x) = \begin{cases}
...
\end{equation}
Our problem \eqref{eq:SE4l} may be rephrased as finding square-integrable solutions of \eqref{eq:SEa} under the Neumann/Dirichlet boundary condition for $E \geqslant 0$.
...
\end{equation}
This looks a transcendental equation, however, after some algebras,  one finds that it is a degree $\ell$ algebraic equation:
\begin{equation}
...
\end{align}
\end{subequations}
The negative energy eigenvalues are displayed in Tab.\,\ref{tab:ene_l6} (Also see Appendix~\ref{sec:ene_l6} for their explicit form).
...
According to the Crum's theorem~\cite{10.1093/qmath/6.1.121}, there are infinitely many number of associated Hamiltonian systems $\mathcal{H}_{\ell}^{[M]}, M=1,2,\ldots$, which are essentially isospectral to $\mathcal{H}_{\ell}^{[0]}$.
They are
\begin{equation}
...
This Hamiltonian is completely isospectral to the 1-d harmonic oscillator potential $\mathcal{H}_{\rm HO}(x)=x^2-1$.
Since $\ell$ can be any positive integer, we now have infinitely many numbers of the isospectral potentials of the harmonic oscillator in our procedure above.
...
Since the parameter $\ell$ can be any positive integer, we can obtain infinitely many number of potentials that are strictly isospectral to the harmonic oscillator by this procedure.
...
%
Moreover, problems of the harmonic oscillator with other singularity functions might also be interesting to consider.



\appendix
\section{Explicit form of the negative energy eigenvalues for $V(x;24)$}
\label{sec:ene_l6}

We list the explicit form of the negative energy eigenvalues in Example~\ref{eg:l6}.
...
The authors would like to thank Ryu Sasaki for his his careful and useful advice and comments.
