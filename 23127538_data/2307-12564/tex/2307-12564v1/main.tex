...
\IEEEPARstart{T}{opic} modelling is a powerful technique for discovering semantic structures of text corpora in an unsupervised manner. It brings success to various applications, such as information retrieval \cite{blei2003modeling} and natural language processing \cite{boyd2007topic}. Conventional topic models such as Latent Dirichlet allocation (LDA) \cite{blei2003latent} and its extensions \cite{cheng2014btm,hai2017analyzing,guo2013two,soleimani2014parsimonious,zhao2014incorporating,duan2014limtopic,zuo2021topic,zhang2019combine,yao2018topic,wang2016using} are Bayesian probabilistic models that assume generative stories of the data. With the increasing scale of data and the development of modern deep learning techniques, the union of deep learning and topic modelling, namely the Neural Topic Model (NTM), is becoming a popular technique for text analytics. 

Given a collection of documents, a topic model learns a set of latent topics, each describing an interpretable semantic concept. A topic model is usually used in two ways: Using the topics to interpret the content of a corpus and using the topic distribution of a document as the semantic representation. For the latter, the learned topical representations of topic models have shown good performance in downstream applications such as document classification, clustering, and retrieval. In practice, it is important that a trained model yields good representations for new documents. Ideally, these new documents are i.i.d samples from the same distribution of the training documents (e.g. from the same corpus). However, this assumption is usually too strong for real-world applications, where new documents may come from new domains. In this work, given an NTM trained on a source corpus, we are interested in how well its power of learning neural topical representation of documents generalises to an unseen target corpus without retraining. More importantly, we aim to propose a model-agnostic training scheme that can improve the generalisation power of an arbitrary NTM. We believe that the problem studied in this work has not been carefully investigated in the literature.

Our idea is straightforward: If an NTM generalises, it shall yield similar topical representations for documents with similar content. Therefore, to enhance the generalisation power of NTMs, we are inspired by the intuition of Data Augmentation (DA), a widely used technique in computer vision \cite{shorten2019survey} to increase the diversity of data. We assume that a document's topical representation shall be close to its augmentations, motivated by the self-learning idea in contrastive learning \cite{chen2020big,he2020momentum,jaiswal2020survey}. Specifically, a document can be encoded as a latent distribution $\bm{z}$ over topics (i.e. topic distribution/topical representation) by NTMs. To make the model capable of producing quality $\bm{z}$ for unseen documents, we encourage the model to learn similar $\bm{z}$ for similar documents, which can be generated by document augmentations \cite{bayer2022survey,shorten2021text} such as adding, dropping and replacing words or sentences in the documents.
To push the topical representations of similar documents close to each other, we need to measure the distance between topical representations. Here we propose to use the Hierarchical Topic Transport Distance (HOTT) \cite{yurochkin2019hierarchical} that computes the Optimal Transport (OT) distance between $\bm{z}$. Intuitively, HOTT considers the distance between topics as the transport cost when comparing $\bm{z}$. The topic distance is again calculated by OT, where the transport cost is from the distance between word embeddings. It provides a natural way to incorporate semantic information from topics and words into the computation of distance between documents. Finally, with the HOTT distance between the augmentations, we propose to minimise the distance as a regularisation term of training NTMs for better generalisation.
Our generalisation regularisation (Greg) term can be easily plugged into the training procedure of most NTMs. Our main contribution is summarised as followings:
\begin{itemize}
    \item We propose to improve the generalisation capability of NTMs in document representation by introducing a universal regularisation term that encourages a model to produce similar topical representations for documents and their augmentations.
    \item In the proposed regularisation, we leverage hierarchical OT distance to measure the topical similarity between documents' representations, and
    we investigate multiple document augmentation methods.
    \item We examine the generalisation of NTMs trained on a source corpus by testing their topical representations on a target corpus from a different domain as well as noisy corpora, which is a new setup for topic models. Extensive experiments show that our approach consistently improves the generalisation power of most NTMs.
\end{itemize}


\section{Background}\label{background}
This section introduces the background of neural topic models and optimal transport, along with the notations used in this paper, which will facilitate the understanding of our model discussed in section \ref{method}. 
...
Given a document collection, a topic model aims to learn the latent topics and the topical representation of documents. Specifically, a document in a text corpus $\mathcal{D}$ can be represented as a Bag-Of-Words (BOW) vector $\bm{x} \in \mathbb{N}^V$, where $V$ denotes the vocabulary size (i.e., the number of unique words in the corpus).
A topic model learns a set of $K$ topics $\bm{T}\defeq\{\bm{t}_1,...,\bm{t}_K\}$ of the corpus, each of which is $\bm{t}_k \in \Delta^{V}$ is a distribution over the $V$ vocabulary words. In addition, the model also learns a distribution over the $K$ topics $\bm{z} \in \Delta^{K}$ of document $\bm{x}$ by modelling $p(\bm{z}|\bm{x})$. $\bm{z}$ can be viewed as the topical representation of document $\bm{x}$. To train a topic model, one usually needs to ``reconstruct'' the BOW vector by modelling $p(\bm{x}|\bm{z})$.
...
Alternatively, Neural Topic Models (NTMs)~\cite{zhao2021topic} have been recently proposed, which use deep neural networks to model $p(\bm{z}|\bm{x})$ and $p(\bm{x}|\bm{z})$.
Although there have been various frameworks of NTMs, models based on Variational Auto-Encoders (VAEs) \citep{kingma2013auto} and Amortised Variational Inference (AVI) \citep{rezende2014stochastic} are the most popular ones.
...
Optimal Transport (OT) has been widely used in machine learning for its capability of comparing probability distributions~\cite{peyre2019computational}. 
We in this paper focus on the case of OT between discrete distributions. 
...
$\mM \in \mathbb{R}_{\ge 0}^{N \times M}$ is the cost matrix/function of the transport and contains the
pairwise distance/cost between the supports, for which
different distance metrics can be used;
$\mP \in \mathbb{R}_{>0}^{N \times M}$ is the transport matrix/plan;
...
The OT distance can be calculated by finding the optimal transport plan $\bm{P}^*$ and various OT solvers have been proposed \citep{benamou2015iterative,blondel2018smooth,bonneel2011displacement,gozlan2017kantorovich,rakotomamonjy2015generalized}. The direct optimization of Eq.\ (\ref{eq-def-ot}) is computationally expensive, \citep{cuturi2013sinkhorn} introduced the entropy-regularised OT distance, known as the Sinkhorn distance, which is more efficient for large-scale problems. It can be defined as:
\vspace{2mm}
...
\end{equation}
where $\mU_{\lambda}(\bm{a},\bm{b})$ contains transport plans with the constraint that: $h(\bm{P})\geq h(\bm{a})+h(\bm{b})-\lambda$, where $h(\cdot)$ denotes the entropy function and $\lambda \in [0,\infty]$ is the hyperparameter.

\section{Related Work}
\subsection{Neural Topic Models}
For a comprehensive review of NTMs, we refer the readers to \citep{zhao2021topic}. Here we mainly focus on models based on VAE \citep{kingma2013auto} and AVI \citep{rezende2014stochastic}. Early works of NTMs focus on studying the prior distributions for the latent variables, such as Gaussian \citep{miao2017discovering} and various approximations of the Dirichlet prior \cite{srivastava2017autoencoding,zhang2018whai,burkhardt2019decoupling} for its difficulty in reparameterisation \cite{tian2020learning}. Recent NTMs mainly leverage external information such as complementary metadata in \citep{card2017neural} and contextual embeddings in \citep{dieng2020topic,bianchi2020pre,bianchi2020cross,xu2022hyperminer}. In this work, we are interested in generalising NTMs instead of proposing a new NTM. And we believe that our method is general to improve the generalisation of most NTMs not limited to VAE-NTMs. 


\subsection{Topic Models and Optimal Transport}
Recently, a few works have built the connection between
topic modelling and OT, most focusing on developing new OT frameworks for topic modelling, such as non-neural topic model \citep{huynh2020otlda} and NTMs \citep{nan2019topic,zhao2020neural}.
...
the generalisation of topic models, especially NTMs, has not been comprehensively studied. Most existing works focus on generalising topic models to multiple domains with access to the full or partial data of the new domains for retraining/fine-tuning, such as the models on
continual lifelong learning \citep{blum2016generalized,chen2015lifelong,chen2014topic,gupta2020neural,qin2021lifelong,zhang2022lifelong,chen2019affinity,lei2023nmtf} and few-shot learning \citep{duan2022bayesian}. Ours is different from
the above methods in two folds: Our approach needs no access to the data in the new domains nor retraining of the model, and ours is expected to be a general regularisation to improve other NTMs. In
terms of the idea of regularising NTMs with the distance between documents’ topical representations, the Contrastive Neural Topic Model (CLNTM) that recently proposed in \citep{nguyen2021contrastive} is related to ours, which uses the contrastive distance \citep{chen2020big}. There are fundamental differences between CLNTM and ours. Firstly, theirs does not focus on
the generalisation of NTMs. Secondly, the distance between document representations is measured by cosine distance in CLNTM, while we use HOTT, which incorporates semantic information from topics and words.
...
different domains may share similar semantics. However, we argue that such intrinsic generalisation power is not enough to generate high-quality topical representations for unseen documents.
Here we aim to study and improve such intrinsic power of NTMs.


% Figure environment removed

\subsection{Proposed Model}
Here we enhance the generalisation power of NTMs by assuming that a document’s topical representation should be close to the topical representations of its augmentations.
Specifically, let $\bm{x}^s$ be the BOW vector of the document from the source corpus. Suppose a stochastic function $\mathcal{F}$ that can produce a random augmentation $\bm{x}^{aug}$ that is semantically similar to $\bm{x}^s$: $\bm{x}^{aug} \defeq \mathcal{F}(\bm{x}^s)$. As both $\bm{x}^s$ and $\bm{x}^{aug}$ share similar semantics, their topical representations $\bm{z}^s$ and $\bm{z}^{aug}$ should stay close in the learned semantic space. 
...
Straightforward choices of $D$ can be Euclidean, cosine, or Hellinger distances, to name a few.
However, these simple distances cannot sufficiently capture the semantic distance between documents in
terms of their topic distributions. To address this issue, we propose to use a topical OT distance
between documents inspired by the hierarchical OT distance of \citep{yurochkin2019hierarchical}. 
Given the word embedding matrix $\bm{E} \in \mathbb{R}^{V\times L}$ from pre-trained models such as Word2Vec \cite{mikolov2013efficient}, GloVe \cite{pennington2014glove}, BERT \cite{devlin2018bert} and etc, where $V$ is the vocabulary size\footnote{
...
} and $L$ is the embedding dimension.
...
Therefore, each topic $\vt_k$ can be viewed as a discrete distribution whose support points are word embeddings: 
$\mu(\mE, \vt_k)$.
The OT distance between topic $\bm{t}_{k_1}$ and $\bm{t}_{k_2}$ can be computed by:
\vspace{2mm}
...
\end{gathered}
\end{equation} 
where $D_{\bm{M^t}}(\cdot,\cdot)$ denotes the OT distance in Eq.\ (\ref{eq-def-ot}); $\bm{M^t}\in\mathbb{R}_{\ge 0}^{V\times V}$ is the topic cost matrix and $\text{cos}(\cdot,\cdot)$ denotes the cosine similarity; $k_1,k_2 \in [1,...,K]$ are indices of topics and  $v_1,v_2 \in [1,...,V]$ are indices of vocabulary words. Eq.\ (\ref{topic_distance}) can be regarded as the Word Mover's Distance (WMD) \citep{kusner2015word}, measuring the topic distance instead of the document distance. As the document's topical representation $\bm{z}$ can be viewed as a discrete distribution whose support points are topics, the Hierarchical Topic Transport Distance (HOTT) \cite{yurochkin2019hierarchical} between documents $\bm{d}_i$ and $\bm{d}_j$ is defined as:
\vspace{2mm}
\begin{equation} \label{HOTT}
\vspace{2mm}
\begin{gathered}
    %\text{HOTT}(\bm{d}_i,\bm{d}_j) =
    \text{HOTT}(\bm{d}_i,\bm{d}_j) \defeq
    D_{\bm{M^d}}(\mu(\bm{T},\bm{z}_{i}),\mu(\bm{T},\bm{z}_j)) \\
    \text{where }\bm{M^d}_{k_1,k_2} \defeq \text{WMD}(\vt_{k_1},\vt_{k_2}),
\end{gathered}
\end{equation}
where $\bm{M^d}\in\mathbb{R}_{\ge 0}^{K\times K}$ is the document cost matrix and its entries indicate the OT distance between topics as in Eq.\ (\ref{topic_distance}); As for topics, we use the decoder weights: $W\in \mathbb{R}^{V\times K}$ as the representation of topics, like other NTMs. Since OT measures the distance between distributions, we normalise each topic as the topic over word distribution by softmax function: 
\vspace{2mm}
\begin{equation} \label{eq_topic}
\vspace{2mm}
\bm{t}_k\defeq\text{softmax}(W^{\text{T}}_{k,:}), 
\end{equation}
where $\text{T}$ denotes the matrix transpose operation. Then we can construct $\bm{M^d}$ by computing the OT distance between topics as Eq.\ (\ref{topic_distance}). Putting them together, we have:
\begin{subequations}\label{full_form}
\begin{equation}\label{full_form_1}
    \min_{\theta, W}{D_{\bm{M^d}}(\mu(\bm{T},\bm{z}^s),\mu(\bm{T},\bm{z}^{aug}))}, 
\end{equation}
\[
\text{where } \bm{T}\defeq \{\bm{t}_1,...,\bm{t}_K\},\ \bm{z}\defeq\theta(\bm{x}),
\]
\begin{equation}\label{full_form_2}
    \bm{M^d}_{k_1,k_2}\defeq D_{\bm{M^t}}(\mu(\bm{E},\bm{t}_{k_1}),\mu(\bm{E},\bm{t}_{k_2})),
\end{equation}
\begin{equation}\label{full_form_3}
    \bm{M^t}_{v_1,v_2}\defeq 1-\text{cos}{(\bm{e}^{v_1},\bm{e}^{v_2})}.
\end{equation}
\end{subequations}
Intuitively, we want to force the model to produce similar topical representations for documents with similar
semantics, where the semantic similarity between topical representations is captured by HOTT.
Specifically, HOTT measures the OT distance between two topical representations of documents by considering the semantic distances between topics.
The semantic distance between two topics is measured by another OT distance between the topic distributions by considering the semantic distances between words, which are captured by word embeddings.



\subsection{Details of Implementation}
\subsubsection{Efficiently Computing Hierarchical OT}
Directly solving the problem in Eq.\ (\ref{full_form}) during training is computationally expensive for two reasons: (i) To obtain $\bm{M^d}$ in Eq.\ (\ref{full_form_2}), we need to compute the OT distance between $K\times (K-1)/2$ topic pairs, each of them has a problem space of $V\times V$. This will be expensive since a corpus may have a large vocabulary size; (ii) NTMs are usually trained on a large text corpus in batches of documents $\bm{X}\defeq\{\bm{x}_i\}^{B}_{i=1}\in\mathbb{N}^{B\times V}$ where $B$ denotes the batch size, so we need to compute HOTT between $\bm{Z}^s$ and $\bm{Z}^{aug}$ where $\bm{Z}\defeq\{\bm{z}_i\}^{B}_{i=1}\in\mathbb{R}^{B\times K}$. While HOTT leverages the algorithm in \cite{bonneel2011displacement} for the computation of OT, which can not support the computation between $\bm{z}$ pairs of $\bm{Z}^s$ and $\bm{Z}^{aug}$ in parallel, thus bringing an enormous computational cost during training. 
...
\end{equation}
\begin{equation}
    \bm{M}^{\bm{\Tilde{t}}_{k_1},\bm{\Tilde{t}}_{k_2}}_{v_1,v_2} \defeq 1-\text{cos}{(\bm{e}^{v_1},\bm{e}^{v_2})}.
\end{equation}
\end{subequations}
Notably, this time we reduce the size of topic cost matrix to $I \times I$ (i.e., $\ v_1,v_2\in [1,...,I]$). We approximate this way because only a small subset of most important words is helpful for the understanding
of a topic, similar to the consideration when evaluating topic coherence \cite{lau2014machine,newman2010automatic}. 
...
computing HOTT during training:
\begin{subequations}\label{appro_form}
\begin{equation}\label{appro_form_1}
    \min_{\theta, W}{D_{\bm{M^d},\lambda}(\mu(\bm{\Tilde{T}},\bm{Z}^s),\mu(\bm{\Tilde{T}},\bm{Z}^{aug}))},
\end{equation}
\[
\text{where } \bm{\Tilde{T}}\defeq  \{\bm{\Tilde{t}}_1,...,\bm{\Tilde{t}}_K\},\ \bm{Z}\defeq\theta(\bm{X}),
\]
\begin{equation}\label{appro_form_2}
    \bm{M^d}_{k_1,k_2}\defeq D_{\bm{M}^{\bm{\Tilde{t}}_{k_1},\bm{\Tilde{t}}_{k_2}}}(\mu(\bm{E}^{\bm{\Tilde{t}}_{k_1}},\bm{\Tilde{t}}_{k_1}),\mu(\bm{E}^{\bm{\Tilde{t}}_{k_2}},\bm{\Tilde{t}}_{k_2})),
\end{equation}
\begin{equation}\label{appro_form_3}
    \bm{M}^{\bm{\Tilde{t}}_{k_1},\bm{\Tilde{t}}_{k_2}}_{v_1,v_2}\defeq 1-\text{cos}{(\bm{e}^{v_1},\bm{e}^{v_2})}.
\end{equation}
\end{subequations}
% \subsubsection{Computation of Gradients}
\subsubsection{Document Augmentation (DA)} As for the function $\mathcal{F}(\cdot)$ that creates random augmentation $\bm{x}^{aug}$ of the original document $\bm{x}^s$, different text augmentation approaches can be used, such as the approaches summarised in \citep{bayer2022survey} and \citep{shorten2021text}. Here we focus on world-level augmentation \citep{bayer2022survey} for its efficiency during training. Different word-level document augmentation approaches are studied in section \ref{DA_study}. The approaches investigated and their descriptions are summarised in Table \ref{DA_Summary}. As a general form, here we write $\bm{z}^{aug}$ is obtained by: 
...
\begin{gathered}
\min_{\theta, \phi} \left(\mathcal{L}^{\text{NTM}} +\gamma \cdot \mathbb{E}_{q_{\theta}(\bm{z}^s|\bm{x})}[D_{\bm{M^d},\lambda}(\mu(\bm{\Tilde{T}},\bm{z}^s), \mu(\bm{\Tilde{T}},\bm{z}^{aug}))]\right),
\end{gathered}
\end{equation}
where $\mathcal{L}^{\text{NTM}}$ is the original training loss of an NTM, which can be the ELBO in Eq.\ (\ref{eq1}) or other losses; $\bm{z}^s\defeq\text{softmax}(\theta(\bm{x}^s))$; $\bm{z}^{aug}$ is obtained by Eq.\ (\ref{DA}); $\bm{M^d}$ is parameterised by $\phi$\footnote{Precisely, $\bm{M^d}$ is parameterised
by $W$ which is the weight of the linear layer of $\phi$.} and can be obtained by solving Eq.\ (\ref{appro_form_2}) and Eq.\ (\ref{appro_form_3}); $\gamma$ is the hyperparameter that determines the strength of the regularisation; $\lambda$ is the hyperparameter for the Sinkhorn distance. The training algorithm of our generalisation regularisation (Greg) is summarised in Algorithm \ref{alg:1}. Notably, both of the Sinkhorn distance and OT distance support auto differentiation in deep learning frameworks such as PyTorch \cite{bonneel2011displacement,patrini2020sinkhorn}, thus the loss in Eq.\ (\ref{appro_form}) is differentiable in terms of $\theta$ and $\phi$. 
...
\begin{algorithmic}[1]
\Require Dataset $\mathcal{D}^s=\{\bm{x}_{i}\}_{i=1}^{N}$, Pre-trained word embeddings $\bm{E}$, Topic number K, Regularisation weight $\gamma$, Sinkhorn $\lambda$, Augmentation strength $\beta$
\Ensure $\theta,\phi$
...
We conduct our experiments on five widely-used datasets: 20 Newsgroup (20News) \cite{lang1995newsweeder}, R8\footnote{https://www.kaggle.com/datasets/weipengfei/ohr8r52}, Web Snippets (Webs) \cite{phan2008learning}, Tag My News (TMN) \cite{vitale2012classification} and DBpedia \cite{zhang2015character}.
We pre-process the documents as BOW vectors by following steps: We clean the documents by removing special characters and stop words, followed by tokenization. Then we build the vocabulary by considering the words with document frequency greater than five and less than 80\% of the total documents. As we use the pre-trained word embeddings of GloVe \cite{pennington2014glove} pre-trained on Wikipedia\footnote{https://nlp.stanford.edu/projects/glove/}, we filter the vocabulary words by keeping only the words that appear in the vocabulary set of GloVe. Finally, we convert documents to BOW vectors based on the final vocabulary sets. The statistics of the pre-processed datasets are summarised in Table \ref{data_statisc}. These datasets are further randomly split as training and testing sets by 8:2 for our experiments. 
...
To evaluate the quality of learned topics, we compute NPMI \cite{lau2014machine, aletras2013evaluating} based on the top 10 words for each topic. Following \cite{yang2015efficient,zhao2018dirichlet}, we compute the average NPMI score of the top coherent topics with the highest NPMI since not all learned topics are interpretable.
% we consider the top coherent topics that with the highest NPMI score as only part of the learned topics is interpretable. The final score of the set of topics is obtained by the average of the NPMI scores of the selected topics. 
Notably, we choose Wikipedia
as the reference corpus for a more objective evaluation when
computing the NPMI score, done by the Palmetto packages\footnote{http://palmetto.aksw.org} \cite{roder2015exploring}. 

\subsubsection{Backbones and Settings}
Our generalisation regularisation (Greg) framework can be easily applied to most of the NTMs. Here we consider the following popular NTMs as our backbones: (i) Neural Variational Document Model (NVDM) \cite{miao2017discovering}, a pioneer NTM that applies Gaussian prior to $\bm{z}$. (ii) LDA with Products of Experts (ProdLDA) \cite{srivastava2017autoencoding}, an NTM that replaces the mixture of multinomials in LDA  with the product of experts. (iii) Neural Topic Model with
Covariates, Supervision, and Sparsity (SCHOLAR) \cite{card2017neural}, an NTM which applies logistic normal prior and incorporates metadata. (iv) Contrastive Neural Topic Model (CLNTM) \cite{nguyen2021contrastive}, a recent NTM that uses a contrastive learning framework to regularise the document representation. We follow the default settings for all these models, except the learning rates are fine-tuned to fit our own datasets. As for Greg, different DAs can be used; we use ``Highest to Similar'' in Table \ref{DA_Summary} for the source-to-target experiments where the top 20 similar words are considered for replacement. The justification for this choice is described in Section \ref{sec:da_ablation}. As for hyperparameters of Greg, we set $\gamma$ as 300, $\beta$ as 0.5 for all experiments; As for the Sinkhorn algorithm \cite{cuturi2013sinkhorn}, we fix $\lambda$ as 100, the maximum number of iterations as 5,000 and the stop threshold as 0.005; As for the OT distance, we leverage the function with default settings in the POT package\footnote{https://pythonot.github.io/index.html} \cite{bonneel2011displacement} for the calculation.


\subsection{Results and Analysis}
\subsubsection{One Source to Different Targets}
\label{result_one2rest}
% To investigate NTMs'' generalisation power on documents'' topical representations across corpora with or without Greg, we conduct the source-to-target task, where a model is trained only on the source corpus and evaluated on both source and target corpora. 
Here we set 20News as the source corpus and the other datasets as the target corpora for our experiment. The quality of the topical representation is measured by CA, TP and TN. The results for $K=50$ are illustrated in Table \ref{20News_Rest_k50}. We have the following remarks from the results: (i) When applying Greg to different models, the CA, TP and TN on different target corpora are significantly improved in most cases. For example, by integrating NVDM with Greg, we improve CA from 39\% to 63.7\%, 38.8\% to 60.2\%, 31.2\% to 55.8\% and 73.2\% to 80.6\% on average for targets Webs, TMN, DBpedia and R8, respectively. Similarly, a large improvement can also be obtained for TP and TN after applying Greg. (ii) The performance on the source corpus is also improved when applied to different backbones under most settings. For example, when applying Greg to PLDA, the performance is increased from 35.5\% to 37.8\%, 32.4\% to 33.7\% and 21.3\% to 23.8\% on average for the source 20News in terms of CA, TP and TN, respectively. Similar observations can be obtained for cases of $K$ equals 20 and 100 in Table S1 and Table S2 of supplemental material, respectively. The results show that our approach effectively generalises the neural topical representation across corpora in our experiments. 
...
\begin{minipage}[t]{0.48\textwidth}
\vspace{0.1cm}
\footnotesize Notes: The larger value between backbone and backbone with Greg under each setting is underlined. The vocabulary words of TMN are added to different sources' vocabularies for BOW representation during training.
\end{minipage}
\end{table}
...
%When setting a long document corpus R8 as the target domain, which is more challenging than a short document target, Greg can still improve the target''s topical representation in most cases. It further indicates the generalisation capability of our framework.

Meanwhile, the performance in the source domain is summarised in Table \ref{sources_TMN_k50} and Table \ref{sources_R8_k50} for the above experiments, respectively. 
% Notably, when fixing TMN as the target, the vocabulary words of TMN are added to different sources'' vocabularies, respectively, similarly when R8 is the target. 
Other results for $K$ equals 20 and 100 are shown in Table S7-S10. Based on these results, It can be observed that Greg can improve the topical representation quality of the source documents at the same time under most settings.




\subsubsection{Target as Noisy Corpus}\label{sec:noisy_targets}
Here we challenge the models with noisy datasets as the target, where the model is trained on clean source\footnote{Since the target is a noisy version of the source corpus, no extra vocabulary word is needed for BOW representation (i.e. $\mathcal{V}=\mathcal{V}^S=\mathcal{V}^T$), namely, ``pure source''.} datasets but evaluated on noisy versions of the source corpora. The results on the noisy targets and pure sources are illustrated in Table \ref{noisy_corpus_k50} and Table \ref{pure_source_k50}, respectively. Results for other settings, including K equals 20 and 100, are illustrated in Table S11-S14. From this set of experiments, we can observe that: (i) Greg continues showing its benefits in generalising neural topical representation when the target domain contains noise (also illustrated by Figure \ref{fig1}(d)), demonstrating improved robustness at the same time. (ii) Based on the pure source experiments, Greg improves the topical representation quality of the original corpus for most cases, where no extra information from external vocabularies is leveraged.  
...
\begin{table}[!t]
  ...
  \begin{tabular}{ccc|cccc}
    \toprule
    \multirow{3}{*}{Metric} & \multirow{3}{*}{Method}  
    & \multicolumn{1}{c}{Source} & \multicolumn{4}{c}{Target} \\
    \cmidrule(l){3-3} \cmidrule(l){4-7} 
    & & 20News & Webs & TMN & DBpedia & R8 \\
    \midrule
    \multirow{4}{*}{CA} 
    & Random to Similar          & 43.1±0.4 & 62.7±0.4 & 58.3±0.2 & \underline{56.0±0.7} & 80.5±0.5 \\
    & Highest to Similar         & 42.0±0.6 & \underline{63.7±0.3} & \underline{60.2±0.4} & 55.8±0.4 & \underline{80.6±0.5}\\
    & Lowest to Similar          & \underline{43.6±0.7} & 55.5±0.7 & 52.8±0.8 & 49.6±0.8 & 78.9±0.8 \\
    & Both to Similar            & 42.6±0.5 & 60.7±1.2 & 57.3±0.1 & 52.4±1.5 & 79.3±0.9 \\
    \midrule
    \multirow{4}{*}{TP} 
    & Random to Similar          & 18.8±0.5 & 34.7±1.0 & 39.6±1.6 & 25.5±0.9 & 63.5±1.8 \\
    & Highest to Similar         & 18.8±0.4 & \underline{35.8±1.5} & \underline{40.1±1.5} & \underline{26.3±0.9} & \underline{63.7±1.8}\\
    & Lowest to Similar          & \underline{19.1±0.2} & 33.6±0.7 & 37.4±1.2 & 23.9±1.1 & 62.6±1.7 \\
    & Both to Similar            & 18.7±0.3 & 34.8±1.2 & 38.6±1.7 & 25.5±1.2 & 62.4±2.1 \\
    \midrule
    \multirow{4}{*}{TN} 
    & Random to Similar          & 14.4±0.3 & 12.5±1.0 & 9.1±0.7 & 15.6±0.6 & 15.3±1.0  \\
    & Highest to Similar         & 14.2±0.3 & \underline{12.7±0.8} & \underline{9.6±0.8} & \underline{15.7±0.5} & \underline{15.5±0.7}\\
    & Lowest to Similar          & \underline{14.7±0.2} & 10.8±0.3 & 7.4±0.5 & 13.8±0.5 & 14.5±1.3 \\
    & Both to Similar            & 14.4±0.2 & 11.9±0.9 & 8.4±0.9 & 15.0±0.9 & 14.9±0.9 \\
  \bottomrule
\end{tabular}
}
\begin{minipage}[t]{0.48\textwidth}
...
Different DAs can be applied for Greg as long as it creates positive (e.g. similar) documents, which shows the flexibility of our framework. Here we demonstrate our choice of DA by applying different positive DAs to Greg. Their resulting topical representation quality of both source and target domains are illustrated in Table \ref{Ablation_da}, where the backbone is NVDM and $K$ is set as 50. Based on these results, although ``Lowest to Similar'' achieves the highest topical representation quality in the source domain, ``Highest to Similar'' obtains the highest quality in almost all the target domains. Since we focus mainly on the generalisation capability, we apply ``Highest to Similar'' in Greg for our experiments. 
...
\begin{table}[!t]
  ...
  \begin{tabular}{ccc|cccc}
    ...
    \midrule
    \multirow{4}{*}{CA} 
    & Euclidean Distance         & \underline{42.3±0.4} & 41.1±1.1 & 39.7±0.2 & 38.0±0.9 & 69.3±0.6\\
    & Cosine Distance            & 41.8±0.4 & 24.1±0.3 & 26.9±0.2 & 14.6±0.3 & 59.5±0.6 \\
    & Hellinger Distance         & 40.1±0.5 & 38.8±1.0 & 38.8±0.6 & 31.3±0.6 & 73.3±0.9 \\
    & HOTT                       & 42.0±0.6 & \underline{63.7±0.3} & \underline{60.2±0.4} & \underline{55.8±0.4} & \underline{80.6±0.5}\\
    \midrule
    \multirow{4}{*}{TP} 
    & Euclidean Distance         & 14.1±0.3 & 30.0±0.4 & 32.1±0.6 & 21.0±0.5 & 61.5±1.3\\
    & Cosine Distance            & \underline{19.7±0.6} & 23.4±0.1 & 25.8±0.1 & 11.6±0.4 & 55.3±0.6 \\
    & Hellinger Distance         & 18.6±0.4 & 28.8±0.4 & 32.3±0.5 & 19.6±0.2 & 62.0±2.0 \\
    & HOTT                       & 18.8±0.4 & \underline{35.8±1.5} & \underline{40.1±1.5} & \underline{26.3±0.9} & \underline{63.7±1.8}\\
    \midrule
    \multirow{4}{*}{TN} 
    & Euclidean Distance         &  8.5±0.1 & 6.5±0.2 & 3.8±0.2 & 9.9±0.5 & 12.4±1.1\\
    & Cosine Distance            & \underline{16.5±0.6} & 4.3±0.1 & 2.2±0.2 & 5.4±0.3 & 10.0±0.4 \\
    & Hellinger Distance         & 14.3±0.2 & 6.5±0.3 & 3.7±0.2 & 9.0±0.4 & 12.9±1.1 \\
    & HOTT                       & 14.2±0.3 & \underline{12.7±0.8} & \underline{9.6±0.8} & \underline{15.7±0.5} & \underline{15.5±0.7}\\
  \bottomrule
\end{tabular}
}
\begin{minipage}[t]{0.48\textwidth}
...
Here we demonstrate the effectiveness of HOTT in Greg in enhancing NTMs' generalisation power by changing different distance metrics. We consider other standard distances for the experiments, including Euclidean, Cosine and Hellinger distances. The results of this study are illustrated in Table \ref{Ablation_dis}, where the backbone is NVDM and K is set as 50. Based on these results, although the performance in the source corpus by using HOTT is not the best, it is ranked the highest in all target domains with a large margin over the second place. It demonstrates the effectiveness of HOTT within our generalisation regularisation framework. 
