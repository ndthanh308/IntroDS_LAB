...
Assume $\eta(t)$ is a $p$-dimensional Ornstein-Uhlenbeck (OU) stochastic process with a marginal mean of $0$.  From \cite{vatiwutipong_2019}, if we assume that the initial state $\eta(t_0)$ is known, then the cross-covariance function of the OU process at times $s$ and $t$ is 

\begin{equation*}
\begin{aligned}
    Cov\{\eta(t), \eta(s) | \eta(t_0) \} = \int_{0}^{\min(s,t)} e^{-\theta(s-u)} \sigma \sigma^{\top} e^{-\theta^{\top}(t-u)}du
\end{aligned}
\end{equation*}

where $e^A$ is the matrix exponential. Using the identity for matrices $A$, $B$, and $C$ that $vec(ABC) = (C^{\top} \otimes A) vec(B)$, we can re-write the vectorized version of the cross-covariance function as
\begin{equation*}
\begin{aligned}
    vec\{Cov\{\eta(t), \eta(s) | \eta(t_0) \}\} = \int_{0}^{\min(s,t)} e^{-\theta(t-u)} \otimes e^{-\theta(s-u)} \ du \  vec\{\sigma \sigma^{\top}\}
\end{aligned}
\end{equation*}
...
\label{eq:xcov_int_cond}
    vec\{Cov\{\eta(t), \eta(s) | \eta(t_0) \}\} = \int_{0}^{\min(s,t)} e^{[-\theta(t-u)] \oplus [-\theta(s-u)]} du \ vec\{\sigma \sigma^{\top}\}
\end{aligned}
\end{equation}
...
\begin{aligned}
    ...
    =&t (\theta \otimes I) (\theta \otimes I) + t (\theta \otimes I) (I \otimes \theta)  + s(I \otimes \theta) (\theta \otimes I) + s (I \otimes \theta) (I \otimes \theta) \\
    =&t (\theta \otimes I) (\theta \otimes I) + t (I \otimes \theta) (\theta \otimes I)  + s(\theta \otimes I) (I \otimes \theta) + s (I \otimes \theta) (I \otimes \theta) \\
...
\begin{equation*}
\begin{aligned}
    vec\{Cov\{\eta(t), \eta(s) | \eta(t_0) \}\} &= \int_{0}^{\min(s,t)} e^{-(t\theta \oplus s\theta)} e^{u(\theta \oplus \theta)} du \ vec\{\sigma \sigma^{\top}\} \\
...
\begin{equation*}
\begin{aligned}
    vec\{Cov\{\eta(t), \eta(s) | \eta(t_0) \}\} &= (\theta \oplus \theta)^{-1} \big[ e^{min(s,t) (\theta \oplus \theta)} - I \big] e^{-(t \theta \oplus s \theta)} vec\{ \sigma \sigma^{\top} \}
\end{aligned}
\end{equation*}

Note that if $s = t$, then the conditional cross-covariance function simplifies to the conditional covariance function given in \cite{vatiwutipong_2019}.

\section{Derivation of the analytic form of the marginal covariance function of the OU process}\label{a:marginal_xcov}

The analytic form of the conditional covariance function given in Lemma 1 (in the main text) is based on the assumption that the initial state $\eta(t_0)$ is \textit{known}.  We now derive the analytic form of the unconditional cross-covariance function that accounts for the additional uncertainty of an unknown initial state.  From \cite{vatiwutipong_2019}, if $\eta(t_0)$ is known, then
\begin{equation*}
\begin{aligned}
...
\begin{equation*}
\begin{aligned}
    Cov\{ \eta(s), \eta(t) | \eta(t_0) \} = vec^{-1}\Big\{(\theta \oplus \theta)^{-1}\Big[ e^{(\theta \oplus \theta) s} - I \Big] e^{-(\theta s \oplus \theta t)} vec\{\sigma \sigma^{\top} \} \Big\}
\end{aligned}
\end{equation*}

If $\eta(t_0)$ is \textit{unknown}, then using the Law of Total Covariance we can calculate
\begin{equation*}
\begin{aligned}
    Cov\{ \eta(s), \eta(t)\} &= \mathbb{E}\big\{ Cov\big( \eta(s), \eta(t) | \eta(t_0) \big) \big\} + Cov\big\{ \mathbb{E}\big( \eta(t) | \eta(t_0) \big), \mathbb{E}\big( \eta(s) | \eta(t_0) \big) \big\} \\
    &=  vec^{-1}\Big\{(\theta \oplus \theta)^{-1}\Big[ e^{(\theta \oplus \theta) s} - I \Big] e^{-(\theta s \oplus \theta t)} vec\{\sigma \sigma^{\top} \} \Big\} \\ & \ \ + Cov\big\{ e^{-\theta t} \eta(t_0), e^{-\theta s} \eta(t_0) \big\} \\
    &= vec^{-1}\Big\{(\theta \oplus \theta)^{-1}\Big[ e^{(\theta \oplus \theta) s} - I \Big] e^{-(\theta s \oplus \theta t)} vec\{\sigma \sigma^{\top} \} \Big\} \\ & \ \ + e^{-\theta t} Var\big\{  \eta(t_0)\big\} [e^{-\theta s}]^{\top}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    Cov\{ \eta(s), \eta(t)\} =& vec^{-1}\Big\{(\theta \oplus \theta)^{-1}\Big[ e^{(\theta \oplus \theta) s} - I \Big] e^{-(\theta s \oplus \theta t)} vec\{\sigma \sigma^{\top} \} \Big\} \\ &+ e^{-\theta t} vec^{-1}\big\{ (\theta \oplus \theta)^{-1} vec\{\sigma \sigma^{\top} \} \big\}  [e^{-\theta s}]^{\top}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    (\theta \oplus \theta)^{-1}\Big[ e^{(\theta \oplus \theta) s} - I \Big] e^{-(\theta s \oplus \theta t)}
\end{aligned}
\end{equation*}
...
\begin{aligned}
\label{eq:commute1}
    (\theta \oplus \theta)^{-1}\big[ e^{s (\theta \oplus \theta)} - I \big] e^{-(s \theta \oplus t \theta)} = e^{-(s \theta \oplus t \theta)} (\theta \oplus \theta)^{-1}\big[ e^{s (\theta \oplus \theta)} - I \big] 
\end{aligned}
\end{equation}
...
\begin{aligned}
\label{eq:commute2}
    (\theta \oplus \theta)^{-1}\big[ e^{s (\theta \oplus \theta)} - I \big] e^{-s (\theta \oplus t \theta)} = (\theta \oplus \theta)^{-1} e^{-(s \theta \oplus t \theta)} \big[ e^{s (\theta \oplus \theta)} - I \big]
\end{aligned}
\end{equation}
...
\begin{equation*}
\begin{aligned}
    e^{-(s \theta \oplus t \theta)} (\theta \oplus \theta)^{-1} = (\theta \oplus \theta)^{-1} e^{-(s \theta \oplus t \theta)}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    e^{-\theta t} vec^{-1}\big\{ (\theta \oplus \theta)^{-1} vec\{\sigma \sigma^{\top} \} \big\}  [e^{-\theta s}]^{\top}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    vec\big\{e^{-\theta t} vec^{-1}\big\{ (\theta \oplus \theta)^{-1} vec\{\sigma \sigma^{\top} \} \big\}  [e^{-\theta s}]^{\top} \big\} &= e^{-\theta s} \otimes e^{-\theta t} vec\big\{ vec^{-1}\{ (\theta \oplus \theta)^{-1} vec\{ \sigma \sigma^{\top} \} \big\} \\
    &= e^{-\theta s} \otimes e^{-\theta t} (\theta \oplus \theta)^{-1} vec\{ \sigma \sigma^{\top} \} \\
    &= e^{-\theta s \oplus \theta t} (\theta \oplus \theta)^{-1} vec\{ \sigma \sigma^{\top} \}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    e^{-\theta s} vec^{-1}\big\{ (\theta \oplus \theta)^{-1} vec\{\sigma \sigma^{\top} \} \big\}  [e^{-\theta t}]^{\top} &= vec^{-1} \big\{ e^{-(\theta s \oplus \theta t)} (\theta \oplus \theta)^{-1} vec\{ \sigma \sigma^{\top} \} \big\} \\
    &= vec^{-1} \big\{ (\theta \oplus \theta)^{-1} e^{-(\theta s \oplus \theta t)} vec\{ \sigma \sigma^{\top} \} \big\}
\end{aligned}
\end{equation*}
...
\begin{aligned}
\label{eq:xcov}
    Cov\{ \eta(s), \eta(t) \} &= vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} \Big[ e^{(\theta \oplus \theta) s} e^{-(\theta s \oplus \theta t)} - e^{-(\theta s \oplus \theta t)} \Big] vec\{ \sigma \sigma^{\top} \} \Big\} \\ & \ \ \ + vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} e^{-(\theta s \oplus \theta t)} vec\{ \sigma \sigma^{\top} \} \Big\} \\
    &= vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} e^{(\theta \oplus \theta)s} e^{-(\theta s \oplus \theta t)} vec\{ \sigma \sigma^{\top}\} - (\theta \oplus \theta)^{-1} e^{-(\theta s \oplus \theta t)} vec\{ \sigma \sigma^{\top} \} \Big\} \\ & \ \ \ + vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} e^{-(\theta s \oplus \theta t)} vec\{ \sigma \sigma^{\top} \} \Big\}\\
    &= vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} \big[ e^{(\theta \oplus \theta) s} e^{-(\theta s \oplus \theta t)} \big] vec\{ \sigma \sigma^{\top} \} \Big\} \\
    &= vec^{-1} \Big\{ (\theta \oplus \theta)^{-1} \big[ e^{(\theta \oplus \theta)s - (\theta s \oplus \theta t)} \big] vec\{\sigma \sigma^{\top}\} \Big\}
\end{aligned}
\end{equation}

Equation \ref{eq:xcov} is the marginal cross-covariance function of the OU process when the initial state at time $t_0$ is \textit{unknown}.
...
\begin{equation*}
    \begin{aligned}
        Cov\{ \eta(t), \eta(s) \} &= vec^{-1}\big\{ (\theta \oplus \theta)^{-1} e^{s \wedge t (\theta \oplus \theta) - (\theta t) \oplus (\theta s)} vec(\sigma \sigma^{\top})\big\} \\
...
where equality (a) is because these terms commute and equality (b) holds when we assume that $min(s,t) = s$. We can make this assumption without loss of generality because the matrices are symmetric. When $min(s,t) = t$, $Cov\{ \eta(t), \eta(s) \} = e^{-\theta |t - s|}V^{\top} = e^{-\theta|t-s|}V$. %\textbf{check}.
Then, the covariance matrix is given by 
\begin{equation*}
    \Psi =\begin{bmatrix}
    V & e^{-\theta |t_2 - t_1|}V & \dots & e^{-\theta|t_{n-1} - t_1|}V & e^{-\theta|t_n - t_1|}V \\
    Ve^{-\theta^{\top} |t_2 - t_1|} & V & \dots & e^{-\theta|t_{n-1} - t_2|}V & e^{-\theta|t_n - t_2|}V \\
    \vdots & \vdots & \ddots & \vdots & \vdots \\
    Ve^{-\theta^{\top}|t_{n-1} - t_1|} & Ve^{-\theta^{\top}|t_{n-1} - t_2|} & \dots & V & e^{-\theta|t_{n-1} - t_n|}V \\ Ve^{-\theta^{\top}|t_n - t_1|} & Ve^{-\theta^{\top}|t_n - t_2|} & \dots & Ve^{-\theta^{\top}|t_n - t_{n-1}|} & V \end{bmatrix}
\end{equation*}

By the definition of the OU process, we know that the precision matrix, $\Omega = \Psi^{-1}$, is block tri-diagonal. We start by solving for two blocks, $\Omega_{11}$ and $\Omega_{12}$. We assume that $\Omega_{11} = A^{-1}$ and $\Omega_{12} = A^{-1}B$, based on the form of the precision matrix in the case of the univariate OU process.  Based on patterns seen when multiplying the AR(1) precision and covariance matrices, we assume that, for the OU process, the first row of blocks in the precision matrix, $[ \Omega_{11}, \Omega_{12}, 0, \dots, 0 ]$ times the second column of blocks in the covariance matrix, $[ e^{-\theta(t_2 - t_1)}V, V, \dots ]^{\top}$, is equal to 0.  So,
\begin{equation*}
\begin{aligned}
    0 &= \Omega_{11} e^{-\theta(t_2 - t_1)}V + \Omega_{12}V \\ 
    \Longrightarrow 0 &= A^{-1} e^{-\theta(t_2 - t_1)} V + A^{-1} B V \\
    \Longrightarrow 0 &= e^{-\theta (t_2 - t_1)} V + BV \\
    \Longrightarrow B &= -e^{-\theta(t_2 - t_1)}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    I &= \Omega_{11} V + \Omega_{12} V e^{-\theta^{\top}(t_2 - t_1)} \\
    \Longrightarrow I &= A^{-1} V + A^{-1} B V e^{-\theta^{\top} (t_2 - t_1)} \\
    \Longrightarrow A &= V + B V e^{-\theta^{\top}(t_2 - t_1)}
\end{aligned}
\end{equation*}

We know that $B = -e^{-\theta(t_2 - t_1)}$ so
\begin{equation*}
\begin{aligned}
    A &= V - e^{-\theta (t_2 - t_1)} V e^{-\theta^{\top} (t_2 - t_1)} \\
    \Longrightarrow A^{-1} &= [V - e^{-\theta (t_2 - t_1)} V e^{-\theta^{\top} (t_2 - t_1)}]^{-1}
\end{aligned}
\end{equation*}

Now we have $$\Omega_{11} = \big[ V - e^{-\theta (t_2 - t_1)} V e^{-\theta^{\top}(t_2 - t_1)} \big]^{-1} \text{ \ and \ } \Omega_{12} = -\big[ V - e^{-\theta(t_2 - t_1)} V e^{-\theta^{\top} (t_2 - t_1)} \big]^{-1} e^{-\theta (t_2 - t_1)}. $$

Continuing with this logic, we can check the first row of blocks in $\Omega$ against all other columns of $\Psi$ and see that
\begin{equation*}
\begin{aligned}
    0 &= \Omega_{11} e^{-\theta(t_k - t_1)} V + \Omega_{12} e^{-\theta(t_k - t_2)} V \\
    &= A^{-1} e^{-\theta(t_k - t_1)} V + A^{-1} B e^{-\theta(t_k - t_2)} V \\
    &= e^{-\theta(t_k - t_1)} V - e^{-\theta(t_2 - t_1)} e^{-\theta(t_k - t_2)} V \\
    \Longrightarrow & e^{-\theta(t_k - t_1)} - e^{-\theta(t_k - t_1)} = 0
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    \Omega_{23} = - \Big[ V - e^{-\theta (t_3 - t_2)} V e^{-\theta^{\top} (t_3 - t_2)} \Big]^{-1} e^{-\theta (t_3 - t_2)}
\end{aligned}
\end{equation*}
...
\begin{equation*}
    I = \Omega_{21} e^{-\theta(t_2 - t_1)} V + \Omega_{22} V + \Omega_{23} V e^{-\theta^{\top} (t_3 - t_2)}
\end{equation*}

\begin{equation*}
\begin{aligned}
    V^{-1} &= - e^{-\theta^{\top}(t_2 - t_1)} \Big[ V - e^{-\theta(t_2 - t_1)} V e^{-\theta^{\top} (t_2 - t_1)} \Big]^{-1} e^{-\theta(t_2 - t_1)} \\ & \ \ \ \ + \Omega_{22} - \Big[ V - e^{-\theta(t_3 - t_2)} V e^{-\theta^{\top}(t_3 - t_2)} \Big]^{-1} e^{-\theta(t_3 - t_2)} V e^{\-\theta^{\top} (t_3 - t_2)} V^{-1}
\end{aligned}
\end{equation*}

\begin{equation*}
\begin{aligned}
    \Omega_{22} &= V^{-1} + e^{-\theta^{\top}(t_2 - t_1)} \Big[ V - e^{-\theta (t_2 - t_1)} V e^{-\theta^{\top} (t_2 - t_1)} \Big]^{-1} e^{-\theta (t_2 - t_1)} \\
    & \ \ \ \ + \Big[ V - e^{-\theta (t_3 - t_2)} V e^{-\theta^{\top}(t_3 - t_2)} \Big]^{-1} e^{-\theta(t_3 - t_2)} V e^{-\theta^{\top}(t_3 - t_2)} V^{-1}
\end{aligned}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    I &= -e^{-\theta^{\top}(t_n - t_{n-1})} \big[ V - e^{-\theta(t_n - t_{n-1})} V e^{-\theta^{\top}(t_n - t_{n-1})} \big]^{-1} e^{-\theta(t_n - t_{n-1})} V + \Omega_{nn} V \\
    \Longrightarrow \Omega_{nn} &= V^{-1} + e^{-\theta^{\top}(t_n - t_{n-1})} \big[ V - e^{-\theta(t_n - t_{n-1})} V e^{-\theta^{\top}(t_n - t_{n-1})} \big]^{-1} e^{-\theta (t_n - t_{n-1}})
\end{aligned}
\end{equation*}
...
\begin{aligned}
    ...
    & \Omega_{j,j} = \big[ V - e^{-\theta (t_{j+1} - t_j)} V e^{-\theta^{\top} (t_{j+1} - t_{j})} \big]^{-1} + e^{-\theta^{\top}(t_j - t_{j-1})} \big[ V-e^{-\theta (t_{j} - t_{j-1})} V e^{-\theta^{\top} (t_{j} - t_{j-1})} \big]^{-1} e^{-\theta (t_{j} - t_{j-1})} \\
...
\begin{equation*}
\begin{aligned}
    d \eta^*(t) &= - \begin{bmatrix} \theta^*_{11} & \theta^*_{21} \\ \theta^*_{12} & \theta^*_{22} \end{bmatrix} \begin{bmatrix} \eta^*_1(t) \\ \eta^*_2(t) \end{bmatrix} + \begin{bmatrix} \sigma^*_{11} & 0 \\ 0 & \sigma^*_{22} \end{bmatrix} d \begin{bmatrix} W_1(t) \\ W_2(t) \end{bmatrix} \\
    &= - \begin{bmatrix} \theta^*_{11} & \theta^*_{21} \\ \theta^*_{12} & \theta^*_{22} \end{bmatrix} \begin{bmatrix} c_1 & 0 \\ 0 & c_2 \end{bmatrix} \begin{bmatrix} \eta_1(t) \\ \eta_2(t) \end{bmatrix} + \begin{bmatrix} \sigma^*_{11} & 0 \\ 0 & \sigma^*_{22} \end{bmatrix} d \begin{bmatrix} W_1(t) \\ W_2(t) \end{bmatrix} \\
    &= - \begin{bmatrix} c_1\theta^*_{11} & c_2\theta^*_{21} \\ c_1\theta^*_{12} & c_2\theta^*_{22} \end{bmatrix} \begin{bmatrix} \eta_1(t) \\ \eta_2(t) \end{bmatrix} + \begin{bmatrix} \sigma^*_{11} & 0 \\ 0 & \sigma^*_{22} \end{bmatrix} d \begin{bmatrix} W_1(t) \\ W_2(t) \end{bmatrix}
\end{aligned}
\end{equation*}
...
\begin{aligned}
    ...
    &= -\begin{bmatrix} \frac{1}{c_1} & 0 \\ 0 & \frac{1}{c_2} \end{bmatrix} \begin{bmatrix}  c_1 \theta^*_{11} & c_2\theta^*_{12} \\ c_1 \theta^*_{21} & c_2 \theta^*_{22} \end{bmatrix} \eta(t) + \begin{bmatrix} \frac{1}{c_1} \sigma^*_{11} & 0 \\ 0 & \frac{1}{c_2}\sigma^*_{22} \end{bmatrix} dW(t) \\
    &= - \begin{bmatrix} \frac{c_1}{c_1} \theta^*_{11} & \frac{c_2}{c_1} \theta^*_{12} \\ \frac{c_1}{c_2} \theta^*_{21} & \frac{c_2}{c_2} \theta^*_{22} \end{bmatrix} \eta(t) + \begin{bmatrix} \frac{1}{c_1} \theta^*_{11} & 0 \\ 0 & \frac{1}{c_2} \theta^*_{22} \end{bmatrix} dW(t)
\end{aligned}
\end{equation*}
...
The estimation algorithm failed to converge due to numerical issues when applied to a few of the simulated datasets generated in the simulation study described in Section 4.1-4.2. We summarize these issues here.
\begin{itemize}
    \item \textbf{Setting 1:} The estimation algorithm initially failed on 4/1000 datasets generated under this set of true OU process parameters. The failures were caused by a singular V matrix at the start of the first block update of the structural submodel parameters. The algorithm selected default initial values for all four of these datasets; altering the set of initial values used for these four datasets enabled convergence of the algorithm for point estimation.  Point estimates were ultimately calculated for all 1000 datasets.
    \item \textbf{Setting 2:} Point estimates and standard errors were calculated for all 1000 datasets.
    \item \textbf{Setting 3:} Point estimates were calculated for all 1,000 datasets, but invalid variance estimates for the measurement submodel parameter $\sigma_{\epsilon_4}$ were estimated from 2 datasets. For these datasets, the variances estimated for this parameter were negative. We attribute this issue to the  numerical approximation used to calculate the Hessian when applied to these two datasets of size $N = 200$.  We anticipate that a larger dataset would improve the approximation of the numerical Hessian but chose to simulate a dataset of this size in order to assess model performance in a realistic setting similar to that encountered in the real data application.  In practical application, if a negative variance were to be estimated, it could be rounded to 0. In the results presented in the main text, we ignore the variance estimates for these two $\sigma_{\epsilon_4}$s.
\end{itemize}

\paragraph{Simulation study: model selection}

In our simulation studies, we aimed to assess simulated datasets with sample sizes similar to that of our motivating dataset.  For datasets of fixed size ($N = 200$ subjects), we found that convergence speeds decrease and estimation becomes more difficult as the number of factors in the model increases.  We found that point estimates of the diagonal elements of $\theta_{OU}$ hit the lower bound of $1\times10^{-4}$ 1\% of the time.  To improve convergence, we (a) increased the maximum number of block-wise iterations allowed (a value of 200 was used in these simulations) and (b) slightly altered the set of default parameter values considered during the initialization steps of the block-wise estimation algorithm for a subset of datasets.  However, when assessing AIC and BIC as model selection criteria (see Section 4.3), we occasionally encountered numerical issues and so failed to calculate parameter estimates for a subset of models applied to the simulated datasets.  The results reported in the main paper correspond to a comparison of AIC and BIC across datasets for which the algorithm used to fit all three models (the one-factor, two-factor, and three-factor models) either converged or reached the maximum number of iterations prior to convergence.  We assessed whether or not including results in which the maximum number of iterations was reached prior to convergence impacted our model selection results and found no substantial changes.  Web Table \ref{tab:GoF_results_convg_only} shows the equivalent version of Table 1 presented in the main text if only results from datasets that had converged were shown.
...
In this model, we assume that a single latent factor generates all observed emotions of happy, joyful, enthusiastic, active, calm, determined, grateful, proud, attentive, sad, scared, disgusted, angry, ashamed, guilty, irritable, lonely, and nervous.  We plot the point estimates from this model in Web Figure \ref{fig:point_ests_1factor}.  Using these estimated parameters, we calculate the auto-correlation half-life of this latent factor as approximately 27 days.
...
We use behavioral science literature and theory---namely the circumplex model of emotion---to inform the division of the positive affect emotions into groups representing high arousal positive affect and no-to-low arousal positive affect (see \cite{reisenzein_1994}, \cite{remington_2000}, \cite{gilbert_2008}, \cite{mcmanus_2019}).  Literature supporting the placement of each positive affect emotion is summarized in Table \ref{tab:refs_for_3factor_mod}.  Happy and joyful are also commonly placed midway between high and low arousal in the circumplex model of emotion (see \cite{remington_2000}) and so we chose to assess the fit of the OUF model when these emotion items load onto the latent factor representing no-to-low arousal positive affect.
...
We allow the block coordinate descent algorithm to run for a maximum of 1,000 iterations when fitting this model.  Both in simulations and fitting the one- and two-factor models, we defined convergence as a change of less than $1\times10^{-6}$ in the log-likelihood or relative parameter values across consecutive block iterations.  We found that convergence was slow when fitting this three-factor model and that after 1,000 iterations convergence had not been reached according to this rule.  A relaxed convergence rule of $1\times10^{-5}$ would have been achieved after 994 iterations; we present point estimates after 1,000 block iterations in Web Figure \ref{fig:point_ests_3factor}.
...
We generate 100 replicates of the simulated dataset and fit the OU factor model using our proposed estimation algorithm and the algorithm proposed in \cite{tran_2021b}.  \cite{tran_2021b} use a slightly different parameterization of the OU process than we use in this work. 
