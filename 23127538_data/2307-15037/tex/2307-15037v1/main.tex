...
In \cite{Eberhardt:2017uup} Eberhardt introduced eight explicit instances of $\mathcal{N}=(2,2)$ AdS$_3$/CFT$_2$, where the bulk geometry is AdS$_3\times (S^3\times M_4)/G$, and the boundary CFT is (a marginal deformation of) the symmetric orbifold $\mathrm{sym}^N(M_4/G)$. The group $G$ is finite and freely acting in all eight cases. In one case $M_4=K3$ and $G=\mathbb{Z}_2$, such that $M_4/G=K3/\mathbb{Z}_2$ is the Enriques surface denoted $\mathrm{ES}$. In the remaining cases $M_4=T^4$, and each of the seven choices for $G$ corresponds to a complex hyperelliptic surface $\mathrm{HS}=T^4/G$ as reviewed in Appendix~\ref{app:hodge}. Setting $G=1$ recovers of course the standard $\mathcal{N}=(4,4)$ dualities \cite{Maldacena:1997re}. (See \cite{Aharony:1999ti,David:2002wn} for reviews of the $\mathcal{N}=(4,4)$ cases, and \cite{Datta:2017ert,Couzens:2017nnr,Belin:2020nmp,Couzens:2021tnv} for other examples of AdS$_3$ holography with $\mathcal{N}=(2,2)$. See also \cite{Eberhardt:2018ouy,Gaberdiel:2019wjw} for closely related dualities between a tensionless limit of the bulk theories and undeformed symmetric orbifolds on the boundary.)\\

Aspects of these dualities have passed successful checks in \cite{Eberhardt:2017uup,Datta:2017ert,ArabiArdehali:2021iwe}:
\begin{itemize}
    \item the bulk BPS Kaluza-Klein (KK) spectra were matched in \cite{Eberhardt:2017uup} (in the $T^4/\mathbb{Z}_2$ case even earlier in \cite{Datta:2017ert}) with those of the corresponding boundary operators;
    ...
    \item in \cite{ArabiArdehali:2021iwe} the nontrivial boundary elliptic genus in the $\mathrm{ES}$ case was used for microstate counting of the bulk black holes, and the Bekenstein-Hawking entropy ($\mathcal{O}(1/G_N)$) as well as the one-loop correction to it ($\mathcal{O}(\log G_N)$) were matched.
\end{itemize}
...
We compute such helicity-trace indices by taking $\bar{y}$ derivatives of the partition function\footnote{As in \cite{Maldacena:1999bp} we use a non-standard notation where $L_0,\bar{L}_0$ are zero on RR ground states. The reader familiar with the standard notation might want to replace $L_0,\bar{L}_0$ with $L_0-c/24,\bar{L}_0-c/24$ in the following formula.}
\begin{equation}
...
%%%%%%
\subsection{Partition function of the seed theory}

The seed CFTs that we consider in this work have target spaces of the form $M_4=T^4/G$, with $G=\mathbb{Z}_k$. Let $g$ be the fundamental generator of $G.$ The partition function of the $T^4/G$ CFT is given by the standard orbifold formula
\begin{equation}
...
%%%%%%
\subsection{Index of the symmetric orbifold CFT}
...
\end{eqnarray}
Let us restrict attention to a term on the RHS with the power of $q$ a prime number $n$, and the power of $y$ another prime number $j$ (limiting to prime numbers is only for simplicity; we will rectify this limitation in Appendix~\ref{app:fine-grained}).  When then see that it is the $s = 1,\, i =
...
%%%%%%
\subsection{The holomorphic counting function}
...
\end{equation}
These holomorphic functions are the objects of main interest in application to black hole microstate counting.
...
\end{equation}
where we have multiplied the exponential by $\tau$ since this is the behavior that we will get in the $\mathrm{HS}_{2,3,4,6}$ cases below. Then
\begin{equation}
...
\end{equation}
The maximized exponent then turns out to have real part\footnote{The imaginary part is $-i\pi j a_1/a_2$.}
\begin{equation}
...
\begin{equation}
    \mathcal{H}_1[\mathrm{HS}_2]\approx e^{\frac{i\pi}{\tau}[\vartheta(z+1/2)-\vartheta(1/2)+\vartheta(z)]}=\begin{cases}
    \exp[\frac{i\pi}{\tau}(-2z^2+z)],\quad &z\in(0,1/2),\\
    \exp[\frac{i\pi}{\tau}(-2z^2+3z-1)],\quad &z\in(1/2,1).
    \end{cases}
\end{equation}
...
\begin{equation}
\begin{split}
\mathcal{H}_1[\mathrm{HS}_3]&\approx e^{\frac{i\pi}{\tau}[\vartheta(z+1/3)-\vartheta(1/3)+\vartheta(z)]}+e^{\frac{i\pi}{\tau}[\vartheta(z+2/3)-\vartheta(2/3)+\vartheta(z)]}\\
&\approx e^{\frac{i\pi}{\tau}\cdot\mathrm{Max}[\vartheta(z+1/3)-\vartheta(1/3)+\vartheta(z),\vartheta(z+2/3)-\vartheta(2/3)+\vartheta(z)]} \\
&=\begin{cases}
    \exp[\frac{i\pi}{\tau}(-2z^2+\frac{4}{3}z)],\quad &z\in(0,1/2),\\
    \exp[\frac{i\pi}{\tau}(-2z^2+\frac{8}{3}z-\frac{2}{3})],\quad &z\in(1/2,1).
    \end{cases}
\end{split}
\end{equation}
...
near-horizon limit of the geometry is \cite{Maldacena:1997re}
\begin{equation}
\mathrm{d}s_{10}^2=\alpha'(\mathrm{d}s_{\mathrm{AdS}_3}^2+\ell^2
\mathrm{d}\Omega_3^2+\sqrt{\frac{v Q_1}{Q_5}}\mathrm{d}x_{T^4}^2),\label{eq:10dNHD1D5}
\end{equation}
where
\begin{equation}
\mathrm{d}s_{\mathrm{AdS}_3}^2=\frac{U^2}{\ell^2}\mathrm{d}x_{||}^2+\frac{\ell^2}{U^2}\mathrm{d}U^2,\quad
U=\frac{r}{\alpha'},\quad \ell=\left(\frac{g^2 Q_1
Q_5}{v}\right)^{1/4}. \label{eq:nhGeometry}
\end{equation}
Hence
\begin{equation}
G^N_{\mathrm{AdS_3}}=\frac{G^N_{10}}{\mathrm{vol}((S^3 \times
T^4)/G)}=\frac{8\pi^6 g^2 \alpha'^4}{2\pi^2 R_{S^3}^3\times (2\pi)^4
\alpha'^2 v}=\frac{\sqrt{\alpha'}(g^2 Q_1 Q_5/v)^{1/4}}{4Q_1
Q_5},\label{eq:GAdS3}
\end{equation}
where we have used $R_{S^3}=\sqrt{\alpha'}\ell$ from \eqref{eq:10dNHD1D5}.
...
\begin{equation}
\begin{split}
f_1^{-1/2}f_5^{-1/2}\big(-\mathrm{d}t^2+\mathrm{d}x^2+(f_n-1)(\mathrm{d}t-\mathrm{d}x)^2\big)+f_1^{1/2}f_5^{1/2}(\mathrm{d}r^2+r^2\mathrm{d}\Omega_3^2)+f_1^{1/2}f_5^{-1/2}\mathrm{d}x_{T^4}^2,\label{eq:pBraneExcited}
\end{split}
\end{equation}
with 
\begin{equation}
f_i=1+r_i^2/r^2,\ i=1,5,\qquad f_n:=1+r_n^2/r^2,\quad r_n^2=\frac{g^2\alpha'^2
n}{vR_{S^1}^2},\label{eq:rn}
\end{equation}
...
\begin{equation}
\begin{split}
    S_{\mathrm{SV}}=\frac{A}{4G_9}=\frac{(2\pi^2 R_h^3)\mathrm{vol}(T^4)}{4G_9}&=\frac{(2\pi^2 r_1r_5r_n)\mathrm{vol}(T^4)}{4G_9}\\
&=2\pi\sqrt{Q_1Q_5n},\label{eq:Ssv}
\end{split}
\end{equation}
where $G_9=G_{10}/2\pi R_{S^1}=8\pi^6 g^2\alpha'^4/2\pi R_{S^1}$ is the 9d Newton constant.\\
...
\noindent\textbf{II) HS microstate counting via indices}. As we saw in Section~\ref{sec:asy degeneracy}, the 1st helicity-trace index does not capture the full leading entropy in the HS cases. This raises the question whether some other observable (such as defect indices, see \emph{e.g.}~\cite{Chen:2023lzq}) can capture the black hole saddle point. Another question is whether a bulk explanation can be found for the factor of $1-\frac{1}{k}$ mismatch with the Bekenstein-Hawking entropy that we found in the helicity-trace index. It is tempting to speculate that the machinery of \cite{Boruch:2022tno} could be relevant in this context.


%%%%
\begin{acknowledgments}

We are indebted to N.~Benjamin, A.~Gustavsson, M.~Heydeman, C.~Keller, P.~Longhi, V.~Reys, M.~Ro\v{c}ek, and G.~Turiaci for helpful discussions. AA is especially grateful to L.~Eberhardt, J.~Jiang, and W.~Zhao for initial collaboration, and to J.T.~Liu for teaching him to fish with~orbifolds.
