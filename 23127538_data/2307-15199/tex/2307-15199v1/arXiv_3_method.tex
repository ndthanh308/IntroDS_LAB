...
The overall framework of the proposed method is shown in Figure~\ref{fig:fig3}, and details of our PromptStyler are described in Algorithm~\ref{alg:pseudocode_prompt}.
...
At inference time, a pre-trained image encoder $I(\cdot)$ extracts image features from input images, and then the trained linear classifier produces class scores using the image features.
Note that we exploit CLIP as our large-scale pre-trained vision-language model;
...
We choose the former, since it takes much less memory when learning $K$ style word vectors in the proposed framework.
