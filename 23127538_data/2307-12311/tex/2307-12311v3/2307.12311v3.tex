...
\begin{abstract}
In this note, we show that the Ruzsa number $R_m$ is bounded by $192$ for any positive integer $m$, which improve slightly the prior bound $R_m\le 288$ given by Y.--G. Chen in 2008.
\end{abstract}
\maketitle
...
again by the fact that $a_1'+a_2'<2m_1$. If $n-r=a_1'+a_2'$, then $n-r\equiv a_1'+a_2'\pmod{m_2}$. So, we can now assume that $n-r+m_1=a_1'+a_2'$, from which we deduce that $n+m_2=(a_1'+r)+(a_2'+r)$, i.e., $n\equiv (a_1'+r)+(a_2'+r)\pmod{m_2}$. Therefore, we have $\sigma_{B}(n)\ge 1$ for any $m_1\le n\le m_2-1$ too.
...
\begin{remark} In \cite[Lemma3]{Chen1}, Chen showed that $R_{m_1}\le 6R_{m_2}$ in the same condition of $m_1$ and $m_2$. The improvement of the upper bound on the Ruzsa number comes from our refined relation between $m_1$ and $m_2$. The improved relation benefits from the observation that if we use the basis of $\mathbb{Z}_{m_1}$ to generate a basis of $\mathbb{Z}_{m_2}$, then the parameter $k$ in Eq. (\ref{eq2}) can only be $0$ or $1$. Whereas, if we use the basis of $\mathbb{Z}_{m_2}$ to generate a basis of $\mathbb{Z}_{m_1}$, like the ones taken by Chen, then the corresponding parameter $k$ in Eq. (\ref{eq2}) could be $0$ or $1$ or $2$, which makes the relation between $R_{m_1}$ and $R_{m_2}$ less explicit.
\end{remark}
...
In \cite{Chen1}, Chen asked that whether there are infinitely many odd (resp. even) numbers in the sequence $\{R_m\}_{m=1}^{\infty}$. He called it a basic problem on Ruzsa's number. To end up this note, we pose a few conjectures and problems which seems to be of some interests.
...
It is clear that we have $$\liminf_{m\rightarrow\infty}\ell_m\ge 1.5$$
since $\sigma_A(n)\ge 2$ if $n$ is not an `even number' in $\mathbb{Z}_{m}$. By Theorem \ref{thm1}, we have 
$$\limsup_{m\rightarrow\infty}\ell_m\le 192.$$
The author thinks that any improvement of the bounds involving $\liminf_{m\rightarrow\infty}\ell_m$ and $\limsup_{m\rightarrow\infty}\ell_m$ above would be of interest. For positive integer $m$, let $\mathcal{H}^*_m$ be the set of all subsets of $\mathbb{Z}_{m}$ with $1\le \sigma_A(n)\le R_m$ for any $n\in \mathbb{Z}_{m}$.
...
The author shall take this opportunity to express many thanks to Professor Y.--G. Chen in Nanjing Normal University for his suggestion and constant enthusiasm on the Ruzsa number which provide great impetus of the author to work on this topic.
