...
% These are recommended to typeset algorithms but not required. See the subparagraph on algorithms. Remove them if you don't have algorithms in your paper.
\usepackage{algorithm}
\usepackage{algorithmic}

% These are are recommended to typeset listings but not required. See the subparagraph on listing. Remove this block if you don't have listings in your paper.
\usepackage{newfloat}
\usepackage{listings}
...
\affil{\textsuperscript{\rm 1}Université Paris-Saclay, AgroParisTech, INRAE, UMR MIA Paris-Saclay \\}
% \address{}
...
\begin{abstract}
  ...
  present AdaVR, which combines the AdaGrad algorithm with
  variance-reduced gradient estimators such as SAGA or L-SVRG. We
  assess that AdaVR inherits both good convergence properties from VR
  ...
  experiments demonstrate the superiority of AdaVR over
...
% \showthe\font
The most basic gradient method is the Gradient Descent \citep[GD, ][]{cauchy1847methode} which
...
$n$ is the Stochastic Gradient Descent \citep[SGD, ][]{robbins1951stochastic} which replaces
...
About a decade ago, the pioneering work of \citet{roux2012stochastic}, followed by \citet{schmidt2017minimizing},
...
computed gradients to implement the control variates methods~\citep{blatt2007convergent} and reduce its variance. As the iterates converge towards
...
followed, e.g. SVRG~\citep{johnson2013accelerating}, SAGA
\citep{SAGA_article}, SVRG++
\citep{allen2016improved}, L-SVRG~\citep{hofmann2015variance},
JacSketch~\citep{gower2021stochastic},
MISO~\citep{defazio2014finito,mairal2013optimization}, S2GD
\citep{konevcny2017semi}, L2S~\citep{li2020convergence}, etc. Other variance-reduced algorithms computing the convex conjugates of
the functions $f_i$ (instead of gradients) were also developped: SDCA ~\citep{shalev2013stochastic} and its extensions (e.g.\
\citet{lin2014accelerated,shalev2014accelerated}) use a coordinate
...
$O(n+L\sqrt{n}/\varepsilon)$~\citep{reddi2016stochastic},
and the SVRG++ algorithm achieves $O(n\log
(1/\varepsilon)+L/\varepsilon)$~\citep{allen2016improved}. None of those two complexities
dominate the other and are, to the best of our knowledge, the best
that does not involve Catalyst-based~\citep{lin2015universal} or momentum-based acceleration~\citep{allen2017katyusha}.
...
gradients by iteration~\citep{lan2018optimal,lan2019unified,joulani2020simpler,song2020variance,li2021anita}. In particular, the recent DAVIS
algorithm~\citep{liu2022kill} achieves
the optimal $O(n+\sqrt{nL/\varepsilon})$ complexity, which matches the
lower bound from~\citet{woodworth2016tight}.

Many of the above algorithms were also studied for
nonconvex smooth finite-sum optimization, and some others were specifically
developped for that context, e.g.\ SARAH~\citep{nguyen2017sarah},
Natasha~\citep{allen2017natasha}, SPIDER~\citep{fang2018spider} and
STORM~\citep{cutkosky2019momentum}.
...
the problem is high \citep{latorre2020lipschitz}, except in some
...
given condition (e.g.\ the Armijo condition \citep{armijo1966minimization}). This
...
convex problems \citep{nesterov2015universal}.
...
objective function \citep{lemarechal1995new}. They
exhibit good performance in practive and they achieve optimality for smooth convex
optimization, adaptively to an unknown smoothness coefficient \citep{lan2015bundle}.
...
step-size. One popular example is the step-size proposed by \citet{barzilai1988two}: although it has good empirical performance, convergence
guarantee are only known for quadratic objectives.
A similar one is the ALS step-size \citep{armijo1966minimization,vrahatis2000class}, and its YK variant
\citep{malitsky2020adaptive}. The latter adaptively achieves the same
...
e.g. \citet{vaswani2019painless,dvinskikh2019adaptive} for
stochastic versions of the Armijo rule, and
\citet{tan2016barzilai,liu2019class,li2019adaptive,li2020almost,yang2021accelerating}
...
the AdaGrad \citep{mcmahan2010adaptive,Adagrad_article}
...
successful in deep learning optimization (with e.g. RMSprop, Adam \citep{kingma2015adam},
...
variant \citep{levy2017online} was the first such algorithm to be proven adaptive to
smoothness. AdaGrad itself is established to be adaptive to smoothness
and noise in \citet{levy2018online} as well as accelerated
variants AcceleGrad \citep{levy2018online} and UnixGrad \citep{kavis2019unixgrad}. Many subsequent works established similar
adaptivity for other variants for the convex setting \citep{wu2018wngrad,cutkosky2019anytime,joulani2020simpler,ene2021adaptive,ene2022adaptive}, and for the nonconvex setting \citep{wu2018wngrad,ward2020adagrad,levy2021storm+,kavis2022high,faw2022power,attia2023sgd}.
...
estimators in a unified fashion. In the case of convex functions, we
establish a $O(n+(L+\sqrt{nL})/\varepsilon)$ gradient complexity. Importantly,
...
prior knowledge of $L$. Numerical experiments also consider as
...
\citet{dubois2022svrg} and \citet{liu2022adaptive}. The former achieves adaptivity to smoothness by
...
Further related works include \citet{li2022variance} which
combines AdaGrad with SVRG as a heuristic only, \citet{wang2022divergence}
which combines the Adam algorithm with SVRG,
and \citet{kavis2022adaptive} which
...
\citet{cutkosky2018distributed} achieves adaptivity to smoothness by
combining SVRG with abstract adaptive
algorithms (including AdaGrad) in a convex setting,
whereas \citet{li2023convergence} combines Adam with a variance
...
reduction: \citet{xie2022adaptive} considers BB, ALS and YK step-sizes
in conjunction with SAGA and achieves adaptivity to smoothness in the
smooth and strongly convex case; \citet{shi2021ai} proposes the
...
and \citet{gower2016stochastic} which associates the celebrated
...
\section{AdaGrad with loopless variance reduction}\label{sec:AdaVR}
...
finite-sum optimization, we establish in Theorem~\ref{thm:AdaVR} a
...
\citep{mcmahan2010adaptive,Adagrad_article} is a variant of (online)
...
\begin{restatable}[\citet{Adagrad_article}]{Lemma}{AdaGradLemma}\label{lem:convenientBoundAdaGrad}
...
is SAG \citep{roux2012stochastic,schmidt2017minimizing} and was
quickly followed by SVRG \citep{johnson2013accelerating}, SAGA
\citep{SAGA_article} and L-SVRG \citep{hofmann2015variance}, among others.
...
\citep{hofmann2015variance,gorbunov2020unified,condat2022murana}.
 They are sometimes called \emph{loopless}
by contrast to SVRG and many of its variants, which are defined with a
inner and outer loops which make their analysis cumbersome in comparison.
...
We define AdaVR in \cref{alg:AdaVR}, a family of algorithms which allows for four
...
%     the convergence rate of $\frac 1 {\sqrt T} $ is achieved thanks to \citep[Theorem 4.2]{levy2018online}}. However, the decreasing variance of the gradient
% estimator can be leveraged to improve the convergence rate to $\frac 1 T $.
% \end{Remark}

\begin{restatable}{Theorem}{AdaVR}\label{thm:AdaVR}
Let $T \geq 1, \eta >0,  x^{(1)} \in \mR^d$ and $\left(\xt\right)_{1 \leq t
\leq T}$ be a sequence of iterates defined by \Cref{alg:AdaVR} with $p=1/n$,
...
on the Covtype\footnote{http://archive.ics.uci.edu/dataset/31/covertype} and scMARK\footnote{https://zenodo.org/record/5765804} datasets \citep{scMark}.
...
\paragraph{AdaVR against SAGA, L-SVRG and AdaGrad}
Following customary practice, we skip the projection step in AdaGrad and its variants.
More attention will be put on the Diagonal versions of AdaGrad and AdaVR since they outperform
the Norm versions in almost all cases.
We compare AdaVR against SAGA, L-SVRG and AdaGrad in \Cref{fig:AdaptVRvsEach}.
...
gives a faster convergence of the objective function for $10$ out of $12$ plots (all except (b): $L =
0.001, 0.01$). The balanced accuracy is better or equivalent for AdaSAGA-Diagonal in every plot, and
significantly better in $7$ out of $12$ plots ((a): $L = 0.001, 1, 10,
100$, (b): $L = 1, 10, 100$).
In (a), AdaVR-Diagonal gives similar balanced accuracy to SAGA but goes closer to the optimum and converges in more plots.
In (b), the AdaVR-Diagonal is much more robust to the choice of the hyperparameter than SAGA or L-SVRG, giving
good balanced accuracy for $4$ plots out of $6$ ($L= 0.001, 0.01, 0.1, 1$), and
only $2$ for SAGA and L-SVRG ($L = 0.001, 0.01$). The perfomances of AdaVR-Diagonal in terms of objective function are much better than SAGA or L-SVRG, going closer to the optimum in more plots.
...
\paragraph{AdaVR against accelerated and variance-reduced algorithms}
We compare AdaVR against state-of-the-art accelerated algorithms (MiG,
Katyusha, L-Katyusha, VRADA, VARAG, DAVIS) in \Cref{fig:vsAccel}.
We observe that accelerated algorithms are sensitive to the choice of the hyperparameter and converge only when its value is small
enough. By contrast, AdaVR is much more robust to the choice of the
hyperparameter, converging in more plots.
...
% accelerated AdaVR. However, the acceleration does not compensate the cost of
% slow learning induced by a low learning rate, as AdaVR reaches a far better
...
% DELETED In \Cref{sec:AdaVR}, we plugged a VRGE inside the AdaGrad algorithm to improve
...
decrease) which may slow optimization down. RMSprop~\citep{ruder2017overview}
...
\end{equation}
with $0< \gamma <1$. Adam~\citep{kingma2015adam} also adds momentum:
...
$12$ ($L= 0.1, 1,10,100$), while RMSprop struggles to converge when
...
better results for $7$ plots out of $12$ ((a): $L=10,100$ and all
plots of (b) except $L=0.01$) and is sometimes multiplied by $2$
(AdamSAGA in (b): $L=100$, RMSpropSAGA in (b): $L=10,100$).
%%%%%%%%%% END OF EXPERIMENTS %%%%%%%

%%%%%%%%%% CONCLUSION %%%%%%%
\section{Conclusion and perspectives}
In the context of the minimization of a sum of $L$-smooth convex
functions, we introduce AdaVR, combining AdaGrad and SAGA or
L-SVRG-type gradient estimators. We demonstrate that AdaVR takes the
...
guarantees, and an analysis of AdaVR without projection would be
relevant. In addition, more general classes of variance reduced
algorithms---e.g. JacSketch~\citep{gower2021stochastic}, Memorization
Algorithms~\citep{hofmann2015variance}; may be analyzed along with
...
% VR + adaptive to smoothness


% acceleration OK
% without projection OK
% local smoothness
% composite objectives OK
% nonconvex
% strongly convex OK
% rmsprop, adam, etc. OK
% more general class of estimators OK
% importance sampling OK
%%%%%%%%%% END OF CONCLUSION %%%%%%%
...
\begin{align*}
   ...
   \drn _{A_{t-1}} ^2  \right)+ \dln x^{(1)} -\xstar\drn _{A_0}^2 \\
     & - \dln x^{(T+1)} - \xstar \drn _{A_T} ^2  \\
...
\begin{Proposition}\label{prop:adatele}
    Let $T\geqslant 1$ and $\left(\xt\right)_{1 \leq t \leq T}$
        defined by \Cref{alg:AdaVR}. Then,
...
f\left(\xstar\right)\right)},
\end{align}
where we used the tower property at the second line, recognized a telescopic sum at
...
\begin{Lemma}[\citep{Nesterov} Theorem 2.1.5] \label{lem:Lsmoothconseq}
...
\begin{Lemma}\label{lem:normDiffPhiSAGA}
    For \Cref{alg:AdaVR} with \ref{eq:saga} and under
    \Cref{ass:convex-smooth,ass:minimum}, for all $1 \leq t \leq T$, we have
\begin{align*}
...
\begin{Lemma}\label{lem:normDiffPhiLSVRG}
    For \Cref{alg:AdaVR} with \ref{eq:LSVRG} and under
    \Cref{ass:convex-smooth,ass:minimum}, for all $1 \leq t \leq T$, we have
\begin{align*}
    \frac 1 {2L}\esptnd{\nabla f_{i(t)} \left(\wtm\right)
-  \nabla f_{i(t)}\left(\xstar\right)} \leq  f\left(\xt\right) -
...
\begin{Lemma}[\citep{DefazioThese} Lemma 4.5]\label{lem:normgtGen}
...
\end{proof}
The following lemma is adapted from \citet[Theorem 7]{Adagrad_article}.
...
\end{align*}
\end{proof}
The following lemma is adapted from \citet[Lemma 10]{Adagrad_article}.
