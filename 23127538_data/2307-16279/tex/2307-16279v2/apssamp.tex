...
\begin{document}
	...
	\date{July 21, 2024}% It is always \today, today,
	...
	For example, by adapting the fractal Suzuki-Trotter decomposition \cite{fractal_decomposition, Suzuki_book} of order $r$, the following error becomes $\hat{\mathcal{E}}_{\mathrm{ST}}(k\Delta_t)=O(k\Delta_t^{r+1})$ at a circuit depth of $O(k 2^r N_\Gamma)$, \textbf{[[2-S6]]}where $k$ is the repeated number of $\Delta_t$ and $N_\Gamma$ is the number of terms that can be efficiently diagonalized and thus trotterized. % if we set the time step in the trotterization same as that of unit evolution operator for the Krylov ansatz.
	...
	If $\tilde{\bm{H}}$ is assumed to be Toeplitz, the elements in only the first row ($k=0$) are sampled from the distribution in Eq. \eqref{eq:delta_h}, which then determines the other elements.
	...
	\end{equation}
	%For the diagonal one, we only need to measure the real part, which removes the imaginary part from Eq. \eqref{eq:delta_h}.
	
	%Based on the averaged distribution, the variance of $[\bm{\Delta_Z}]_{kl}$ for $\bm{Z}\in\{\bm{S},\bm{H}\}$ in Eqs. \eqref{eq:delta_s} and \eqref{eq:delta_h} is inversely proportional to the number of shots $m_{\bm{Z},kl}$.
	...
	Therefore, instead of using the exact value of $[\bm{S}]_{0l}$, its average is used to estimate the shot allocation, which results in $m_{\bm{S},kl}^{(\mathrm{r})} = m_{\bm{S},kl}^{(\mathrm{i})}$.
	...
	%The variance of diagonal elements are halved because their imaginary parts are determined as zero without sampling as $\bm{H}$ and $\bm{S}$ are Hermitian. 
	%Also, we assume that the same number of shots are allocated for the real and imaginary part in off-diagonals ($k\neq l$).
	...
	The optimal $\epsilon$ will be heuristically found in Section \ref{sec:numerical_truncation}.
	...
	Here, note that $N_\Gamma$ is the number of Hamiltonian fragments whose exact time propagation is known.
	...
	%[5th July]
	If the circuit depth of Eq.~\eqref{eq:min_ckt_depth} is not satisfied while Toeplitz construction is adopted, the GEVP perturbation caused by the sampling error and Trotterization is bounded as [\textbf{2-S6}], [\textbf{3-2}]
	...
	%O&\left(d_0^{-1}\left(\frac{N_\Gamma^2 \|\hat{H}\| \Delta_t^2 n^3}{D^2} + \frac{\|\hat{H}\|_\beta n \sqrt{\log n}}{\sqrt{M_{\bm{H}}}}\right)\right),
	...
	%\sout{Note that our analysis covers the perturbation within the approximated subspace.Therefore, the convergence problem of the approximated subspace to the exact Krylov subspace or full Hilbert space will complete the analysis of the effect of the Trotter error in the QKSD method. We leave this issue as a future work as it is beyond the scope of the current study.}
	...
	By adopting the Pauli error model introduced in \cite{liang2023modeling}, it can be shown that hardware noise induces a decay of the measured matrix:
	\begin{equation}
	...
	%{\color{red}\sout{$(t, u)=(0.1, 0.2)$ and $(0.2, 0.1)$, indicating far $(t/u<1)$ and near $(t/u>1)$ interactions, respectively, and the $(0.1, 0.8)$ case, which is known to be difficult to solve}}.
	...
	It is assumed that we perfectly simulate the time evolution $e^{-i\hat{H}t}$, thereby neglecting %{\color{red}\sout{the effect of Hamiltonian simulation errors, such as}} 
	Suzuki-Trotterization errors and hardware decoherence, 
	%{\color{red}\sout{and focusing}}
	in order to focus on the sampling error.
	...
	\end{align}
	%{\color{red}Note that associated with the condition number bound in Eq. \eqref{eq:cond_bound}, in such assignment of $\epsilon$ produces a bound for perturbation in Eq. \eqref{eq:pert_bound_bigO}:
	%\begin{equation}
	%    |\tilde{E}_0^{(n\rightarrow n_{\epsilon})} - E_0^{(n \rightarrow n_\epsilon)}|\le O\left( \|\hat{H}\| n^{z-1} \right).
	%\end{equation}
	%{[\color{red} This seems a meaningless bound...]}}
	...
	\iffalse
	Unfortunately, even with the optimal truncation, it is shown that the problem is prone to noise and a significant amount of samplings (order of $10^{16} \sim 10^{18}$) are required for the perturbation to coincide to the ideal one by more than half number of original basis.
	\fi
	...
	Many recent quantum-classical hybrid algorithms incorporate the estimation of the overlap to construct matrices and utilize the classical method to solve GEVP or linear equation \cite{PhysRevResearch.3.043212, PhysRevA.104.042418}.
	...
	%{\color{red}\sout{Hence, we replaced the dimensional factor, $q$ %\cite[{Corollary~2.2}]{Theory_QSDK}
	%, with precise definitions and bounds from the numerical radius}}
	...
	Thus, %{\color{red}\sout{assuming identical shot settings for the imaginary and real parts, $m^{(\mathrm{r})}=m^{(\mathrm{i})}=m/2$,}} 
	the sampling variances become
	\begin{equation}
	\begin{split}
	...
	\end{equation}
	%{\color{red}
	%\sout{Adding two then gives the variance of $Z$, owing to the independency, resulting in}
	%\textbf{[REMOVE THE FOLLOWING EQ.]}
	%\begin{equation*}%\label{eq:hadamard_noiseless_variance}
	%\begin{split}
	%    \mathrm{Var}[Z] &= \mathrm{Var}[R] + \mathrm{Var}[I] \\ 
	%                  &=  \frac1 - \mathrm{Re}[\bra{\psi}\hat{U}\ket{\psi}]^2 1 -\mathrm{Im}[\bra{\psi}\hat{U}\ket{\psi}]^2\\
	...
	%{\color{red}\sout{Therefore, the average variance becomes}}
	\begin{equation}
	...
	%\sout{Note that our analysis covers the perturbation within the approximated subspace.
	%Therefore, the convergence problem of the approximated subspace to the exact Krylov subspace or full Hilbert space will complete the analysis of the effect of the Trotter error in the QKSD method.
	%We leave this issue as a future work as it is beyond the scope of the current study.}
