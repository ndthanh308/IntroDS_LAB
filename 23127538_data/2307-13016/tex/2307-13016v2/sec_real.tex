...
the same behavior as in \cref{lem:restrictQ}.
  ...
  \end{lemma}
  \begin{proof}
    For any $i\in [u]$ we have that $\eps i$, the difference
    between $ai, a'i$ satisfies
    $$\eps\cdot i< 1/n,$$ 
    where $1/n$ is the ``bin width", i.e., the size of a
    sub-interval of $(0,1)$ which
    corresponds to a single bin after multiplication by $n$ and
    flooring.
    Thus, $a'i$ and $ai$ either land in the same bin or $a'i$
  ...
  \begin{defin}\label{defn:fclaims}
    For integer $k\in [1,nu]$ we define ${I(k) \subset (0,1)}$ to be the
...
\begin{cor}
The expected maxload of $\RH_u$ is $$\widetilde{\bigO}(n^{1/3}).$$
\end{cor}
