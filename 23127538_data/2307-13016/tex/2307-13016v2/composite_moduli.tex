...
We conclude by demonstrating that the standard $\bigO(\sqrt{n})$ maxload
bound for prime moduli, and even Knudsen's
...
\begin{proof}
  ...
  $n^{-1}\in \Z_m$.  
  Assume $Y\subset X$ is the set of elements mapping to a
  fullest bin under $\blockZm$ with $a=a_0$. Then for
  $a=\modp(n\cdot a_0)$, $\strideZm$ has maxload at least
  $|Y|/2$. 
  Assume $S\subset X$ is the set of elements
  mapping to a fullest bin under $\strideZm$ with
  $a=a_0$. Then for $a=\modp(n^{-1} a_0)$ $\blockZm$ has
  maxload at least  $|S|/2$.

  Multiplication by $n$ or $n^{-1}$ modulo $m$ permutes $\Z_m$.
  The result follows.
\end{proof}
...
\begin{defin}\label{defn:slh}
  ...
  $a\in \Z_m^{\times }$ and place $x\in [m]$ in bin
...
$\ZH_m$ on $X$, and let $M_{\ZH}$ denote the worst-case expected value of
...
\begin{theorem}\label{thm:LHSLH}
  ...
  \begin{proof}
    ...
    We will bound the expected maxload of $\ZH$ by conditioning
    on $G_d$. However, if $\gcd(a,m)$ is very large then  $\ZH$ will
  ...
  $d$ has $\Pr[G_d] \le 1/n^{2.5}$. Thus we have:
  \begin{equation}
    \Pr[\gcd(a,m) \ge m/n^{2.5}] \le \frac{n^{2.5}}{m/n^{2.5}} \le
    \frac{1}{n},
  \end{equation}
  where the final inequality follows by the assumption that $m\ge n^{6}$.
  Thus, the case $\gcd(m,a)\ge m/n^{2.5}$ contributes at most $1$ to
...
&\le \tau(m)\cdot f(n) \label{eqchainSLH8}.
  ...
  \begin{itemize}
    ...
    \item \cref{eqchainSLH4}: Recall that $X_{i,d}\subset I_{i,d}$, where
      ...
      blocks of $m/n$ contiguous elements per bin is
      equivalent to having elements spaced out by $1$
      grouped into blocks of $(m/d)/n$ contiguous elements per
...
specific to Knudsen's proof to translate the
...
\begin{proof}
  ...
  $\FH_p$ where all pairs collide with probability $1/n$.
...
This is tentative evidence that $\LH$ with composite modulus may
