...
In this section we show that $\RH_u$ actually behaves like a
version of $\SLH_m$ with a randomized modulus.
In particular, our lower bound on $\RH_u$'s performance is
relative to the following variant of $\SLH_m$:
\begin{defin}
...
\begin{theorem}
  ...
  $\randH_{\lfloor\sqrt{nu}\rfloor}$ on a worst-case $X\subset
    [\lfloor\sqrt{nu}\rfloor]$, let $M_{\max}$ denote the maximum over
...
\left[0,\frac{1}{nu}\right]} \cap (0,1).\]
      We say that $k$
  \defn{claims} the elements of $I(k)$.
  For $a\in (0,1)$ let $F(a)$ denote be the smallest $k\in \N$ so
  that $k$ claims $a$.
...
bound $M_\R$ we seek to understand $F$.
\begin{lemma}\label{clm:prdistrunderstand}
  For $k\le \lfloor\sqrt{nu}\rfloor$,
...
\begin{proof}
  ...
  For distinct $k_1,k_2\le \lfloor\sqrt{nu}\rfloor$ and appropriate numerators
  ...
  $k$. In other words,
    ...
    This implies that \cref{eq:fakprub} is tight for $k\le
...
\end{cor}
\begin{proof}
  Fix integer ${k\in [\lfloor\sqrt{nu}\rfloor/2, \lfloor\sqrt{nu}\rfloor]}$.
  ...
  \begin{itemize}
    \item Each modulus $k \in [\floor{\sqrt{nu}}/2,
          ...
          $\randH_{\dfloor{\sqrt{nu}}}$ with probability
          $2/\dfloor{\sqrt{nu}}$ which is at most $10 \log\log n$
          times larger than $\Pr[F(a)=k]$. \item Conditional on
          $F(a)=k$ $\RH_u$ achieves expected maxload at least
          $1/2$ of the expected maxload of $\randH_{\dfloor{\sqrt{nu}}}$
...
\begin{proof}
  ...
  \[
  \frac{i}{m} = \frac{\lambda m / \ell - 1/\ell}{m}=
\frac{\lambda}{\ell}-\frac{1}{m \ell}.\]
