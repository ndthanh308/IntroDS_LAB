\section{Two Bins}
\label{sec:twobins} 
In this section we consider an even simpler setting for $\LH$:
hashing $n$ items to $2$-bins. Despite the simple setting,
understanding $\LH$'s behavior remains complicated. In addition
to being of independent interest, we believe the $2$-bin case is
a helpful starting point for analyzing the full $n$ bin case. In
particular, we propose in \cref{conj:reductiontwobins} a tight
concentration bound for the maxload in the $2$-bin setting, and
propose a method \cref{question:reductiontwobins} for using
\cref{conj:reductiontwobins} in the $n$ bin case. We do not
resolve \cref{conj:reductiontwobins}. Instead, the main content
of this section is \cref{thm:dontneedb} in which we
bound the expected maxload of a variant of $\LH$ with no
shift-parameter in the $2$-bin setting.

Formally, our hash functions are defined as follows:
\begin{defin}
  Let $p\in \PRM$.
  ...
  In \defn{Multiplicative Two Bin} $\LH$  ($\MLH$) we hash
  $x\in [p]$ via functions
  parameterized by $a\in\pnozero$ defined as 
  $$h_a(x)  = \floor{\frac{\modp(ax)}{p/2}} \in \set{0,1}.$$
\end{defin}
\begin{rmk}\label{rmk:shiftmatterstwobins}
  ...
  ``$+b$" will not be more than a factor-of-$2$: the shift term
  corresponds to shifting bin boundaries, which at most splits
  the fullest bin in half. Thus, when the number of bins is $n$,
  the difference between these two methods seems insubstantial.
  However, in the $2$-bin case the maxload is between $n/2$ and
  $n$, so a factor-of-$2$ difference would be quite substantial. 
  Thus, analyzing $\ULH$ will not suffice to describe the maxload
  of $\MLH$.
\end{rmk}

We propose the following open question:
\begin{question}[Reduction to Two Bins]
  \label{question:reductiontwobins} Is it possible to obtain a
  bound of $n^{o(1)}$ on $\LH$'s maxload by first establishing a
  strong concentration bound on maxload in the $2$-bin setting
  and then recursively applying this bound to subsets of bins?
\end{question}
...
\end{conj}
 ...
 In the remainder of this section we prove a weaker version of
 \cref{conj:reductiontwobins} for $\ULH$ and $\MLH$.  More
 precisely, we bound the expected maxload of $\ULH, \MLH$, which
 by Markov's inequality gives polynomial strength concentration
 bounds (rather than exponential as in
 \cref{conj:reductiontwobins}) on their maxload.
...
\begin{theorem}\label{thm:dontneedb}
  ...
  $$n/2+\bigO(n^{7/8}).$$
\end{theorem}
  As in \cref{prop:pairwise_concentrate}, our proof is based on
  comparing the expected number of collisions (elements hashing
  to the same bin), with the number of
  collisions given by a certain maxload. However, with $\MLH$
  elements no longer exhibit pairwise independence, so counting
  the expected number of collisions is more challenging. 
  ...
  Instead of working directly with probabilities, it is more
  convenient for combinatorics to work with the following
...
Now we give a bound on $\sum_{x\in X}\lap(x)$. 
Our two key lemmas use the following concept: 
...
\begin{lemma}\label{lem:nasty}
  ...
  \begin{proof}
    ...
    By the continuous pigeon-hole principle there must be
...
$\lap(x)$. \footnote{ This bound is likely quite weak; in fact,
\cref{conj:log2nwouldbenicetoohard} is based on the intuition
that we can actually bound $\lap(x)$ by a term like
$k+p/(mk)$.}
Each cycle contributes at most $\bigO(1)$ to $\lap(x)$, because a
cycle is $\floor{p/k}$ elements every spaced out by $k$.
...
\begin{proof}
  ...
  for any $n$-element set $X\subset [p]$
  ...
  x_n}$. Then
  \begin{align*}
    \E[C] &\le \sum_{i=1}^{n}\sum_{j=1}^{i} \E[C_{i,j}]\\
...
\begin{proof}[Proof of \cref{thm:dontneedb}]
  Comparing the bound on $\E[C]$ from \cref{lhs:complicated7} to
  the expression for $\E[C\mid M]$ given in
  \cref{prop:pairwise_concentrate}'s \eqref{eq:rhs7ez}, and
...
In fact, we believe the following stronger bound:
\begin{conj}
  \label{conj:log2nwouldbenicetoohard}
  $$\sum_{x\in X} \lap(x) \le\bigO( p \cdot \log^2 n).$$
\end{conj}
Proving \cref{conj:log2nwouldbenicetoohard} would allow for a
stronger bound in  \cref{thm:dontneedb}. We leave
\cref{conj:log2nwouldbenicetoohard} as a beautifully elementary
and yet possibly quite complex to solve open problem.
