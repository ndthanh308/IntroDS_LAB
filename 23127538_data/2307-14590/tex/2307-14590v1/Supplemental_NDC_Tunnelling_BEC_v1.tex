\documentclass[notitlepage,superscriptaddress,aps,prl,twocolumn,noeprint]{revtex4-1}% http://ctan.org/pkg/revtex4-1
...
\myexternaldocument{NDC_Tunnelling_BEC_v1}
...
\begin{document}
\title{Supplemental Material for ``Nonequilibrium  Transport in a Josephson Junction Chain: Is There Negative Differential Conductivity?"}
...
Here we provide further details of the simulations, including the choice of parameters for the c-field simulations, and a discussion of the robustness of the results on the initial conditions. We then analyse the emergence of a condensate on the central site, and examine the adiabaticity of the dynamics of the refilling of the system site.

\section{Details of simulations}
\subsection{Simulation parameters} \label{sec:param}
The experiment consists of a 1D chain of 2D harmonic traps, see Fig.~\ref{fig:Schematic} of the main text, with a lattice spacing in the $z$-direction $a = 549$~nm and a radial trap-frequency $\omega_r= 165 \times 2\pi $ rad/s. Traps near the centre of the chain contain approximately $N = 700$ atoms of Rb-87 with mass $m$ and an s-wave scattering length $a_s = 5.8 $ nm. The 3D interaction strength can be calculated as $g = 4\pi\hbar^2 a_s/m$. Since the model Eq.~(\ref{eq:gpe}) considers the $z$-direction to be discrete due to the strong confining potential, we obtain the 2D interaction strength $g_2 = g \int dz ~ w(z)^4 ,$ where $w(z)$ is the Wannier band in the $z$-direction, which is obtained numerically. The chemical potential is obtained via the Thomas-Fermi approximation: $\mu =\sqrt{g_2 m \omega_r^2 N /\pi}$. 
...
The experiment~\cite{Labouvie2015} contained approximately 65 sites, with $\sim$ 700 atoms contained in the sites in the middle of the chain, and comparatively fewer towards the ends of the array due to the weak harmonic potential in the $z$-dimension.
...
To initialize simulations, we first find the system's zero temperature ground-state by evolving  Eq.~(\ref{eq:gpe}) in imaginary time. We then remove almost all atoms on the central site ($i=0$), such that the initial relative atom number is $\sim5\%$, consistent with the experimentally reported values. The removal of the atoms at the central site effectively separates the half-chains on each side into two isolated systems; the relative phase between left and right chains are thus expected to drift due to quantum (or  thermal) fluctuations~\cite{Sinatra2008} while the two subchains are disconnected.  For each trajectory, we multiply the right $R = \{i | i > 0\}$ subset of the chain by random phase $e^{i \theta}$, $\theta \in [0,2\pi]$ for every trajectory, to mimic this phase diffusion. Finally, half a particle per mode of noise is added to all sites, to capture vacuum fluctuations in accordance with the TWA~\cite{Steel1998,Polkovnikov2003a,Blakie2008}.
...
\subsection{Initial Phase Difference Between Half-Chains}
As discussed above, the removal of atoms from the central site during the preparation period separates the system into two half-chains that are essentially disconnected. The relative phase between these half-chains may then drift due to quantum fluctuations. We now demonstrate numerically that this causes rapid dephasing between the two sub-systems, justifying our choice to mimic this effect via a uniform random phase for each respective half-chain. We first initialize the full chain in the zero temperature ground-state, including quantum noise. We then set the tunnelling constant between the central site and the sites +1 and -1 to $J=0$, and
evolve the system in time.
Figure \ref{fig:halfpahse_ev} shows the evolution of the mean of the (absolute) phase difference between the sites 1 and -1, $|\Delta \Phi_0|$, for a large range of $J$ values.  Note that since each half-chain is close to their respective ground-state configurations, $\Phi_1$ and $\Phi_{-1}$ are 
representative of the phase of the half-chains, not just sites $1$ and $-1$ respectively. 
The phase difference is shown to grow over a time of $30 \sim 40 ~~ \omega_r^{-1}$, after which point it has reached approximately $\pi/2$, indicating complete randomness.


We now compare this to the time-scales in the experimental procedure \cite{Labouvie2015}, noting that $1 ~\text{ms} \approx  \omega_r^{-1}$. A detailed breakdown of the experiment can be found in Ref.~\cite{LabouvieThesis}. After the initial cooling process, the lattice height is ramped up so that every site is effectively isolated in a deep well for 9~ms while the central site is depleted via one-body losses provided by an electron beam. The lattice height is then lowered to the final value over a time-scale of 2~ms, with the dissipation still turned on. While this differs from the procedure considered above, we are only interested in the net effect on the phase difference between the two half-chains. Crucially, the two half-chains are effectively uncoupled while the atoms are deleted and the lattice height is lowered. Our results suggest this period  of 11 ms is long enough for significant phase randomness to develop between the half-chains. Furthermore, thermal fluctuations will further contribute to the decoherence. For small values of $J$ the initial part of the filling process is quite slow, and the dephasing should continue even after the dissipation is turned off. In summary, while we cannot rule out the persistence of some partial coherence between the half-chains, substantial decoherence should be present such that a completely random initial phase difference is a reasonable approximation for the initial conditions.
