...
This is typically dealt with through a two-stage ranking setup, where a more lightweight \emph{candidate generator} stage is followed by a \emph{ranking} stage that decides the final personalised order in which we present items to the user~\cite{Covington2016, Ma2020}.
...
Let $\mathcal{A}^{k}$ denote all subsets of $k$ actions: $\mathcal{A}^{k} \subseteq 2^{\mathcal{A}}, |\mathcal{A}^{k}|=k$.
...
Note that this general notation subsumes other common scenarios, such as candidate generation policies that are deterministic, consist of ensembles, or simply yield $\mathcal{A}$ (i.e. \emph{single-stage}).
...
Define $S(a^{k})$ as the set of all possible permutations over $a^{k}$.
Formally, we have:
%\begin{equation}
$
...
Indeed, as has been widely reported~\cite{Jeunen2019DS,Gilotte2018}, online experiments require us to:
\begin{enumerate*}
...
\end{enumerate*}
% Refer to issues w/ A/B-tests if you can, otherwise it's for camera-ready
