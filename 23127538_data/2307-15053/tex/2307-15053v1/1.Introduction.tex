...
Fourth, multiple recent works have reported troubling trends in the reproducibility of widely cited methods~\cite{FerrariDacrema2019,FerrariDacrema2021,Rendle2020,Rendle2022,Rendle2019}---similar issues plagued the adjacent IR field a decade earlier~\cite{Armstrong2009}. 
...
\begin{enumerate}%\vspace{-2ex}
    ...
    \item We formally prove that the widespread practice of \emph{normalising} the DCG metric renders it inconsistent with respect to DCG, in that the ordering given by nDCG can differ from that given by DCG (\S~\ref{sec:ndcg_inconsistent}).
    \item Empirical results from off and on-line experiments on a large-scale recommendation platform show that:
    \begin{enumerate}
    ...
    \end{enumerate}
    \item We revisit the assumptions that are necessary to consider DCG an unbiased estimator: considering when they can be reasonable and how we can lift them, giving rise to a research agenda and ways forward (\S ~\ref{sec:beyond}).
\end{enumerate}

% In what follows, we first provide an overview of related work in the recommender systems field that aims to tackle similar issues related to offline evaluation practices, in Section~\ref{sec:relwork}.
% We formalise our problem setting in Section~\ref{sec:problemsetting}, detailing a probabilistic framework which serves as the backbone to present our core contributions in Sections~\ref{sec:dcg_unbiased}--\ref{sec:beyond}.
% Finally, Section~\ref{sec:conclusion} concludes our work, presenting a scope for future research.