%\documentclass[aip,graphicx]{style/revtex4-1}
...
\begin{quotation}
%Dynamics of coupled oscillators is one of the basic problems in nonlinear dynamics. For a weak coupling, the units remain oscillating but can adjust their phases. This results in synchronization, and many other effects related on the phase dynamics. For the phase dynamics to provide a good quantitative description of the oscillating systems, one needs an accurate reduction from the original equations of motion to the phase dynamics equations. In this paper we provide, for two-dimensional oscillating systems, a theoretical perturbative framework for such an accurate reduction, which begets phase equations as expansions in the orders of a small parameter    describing coupling.
...
phase-amplitude representation~\cite{Shilnikov_Shilnikov_Turaev_Chua-98} of the dynamics near the limit cycle. 
For illustration of the normal form, consider the Stuart-Landau oscillator
\[
...
\begin{align}
    %\dot\bx_1 &= \bG_1(\bx_1) + \e \bS_1(\bx_1, \bx_2) \\
    %\dot\bx_2 &= \bG_2(\bx_2) + \e \bS_2(\bx_1, \bx_2)
    \dot\bx_\mu &= \bG_\mu(\bx_\mu) + \e \bS_\mu(\bx_1, \dots, \bx_M)
    \label{eq:ODE_original}
    \,.
\end{align}
Here $\bG_\mu$ determines the autonomous evolution of oscillator $\mu$, and $\bS_\mu$ encodes the coupling of this unit to all other oscillators. We assume that $\bG_\mu$ and $\bS_\mu$ are sufficiently smooth functions in all arguments. Since all systems exhibit a stable limit cycle, there exist a smooth transformations to coordinates %$\Phi_\mu: \bx_\mu \mapsto \phi_\mu  \in [0,2\pi) $ and $P_\mu: \bx_\mu \mapsto r_\mu \in %\mathbb{R}$ 
$\phi_\mu=\Phi_\mu(\bx_\mu)$ and $r_\mu=P_\mu(\bx_\mu)$ 
%in which the equations for the phase variable $\phi_\mu$ and for the amplitude variable $r_\mu$ 
%(the latter is sometimes called isostable coordinate) are 
which obey linear normal form equations~\cite{Shilnikov_Shilnikov_Turaev_Chua-98} 
 (cf.~Eq.~\eqref{eq:anf}:
\begin{equation}
...
%In terms of theory of dynamical systems representation \eqref{eq:pac} is called normal form~\cite{Shilnikov_Shilnikov_Turaev_Chua-98}.
...
\begin{align}
    ...
    \,.
\end{align}
Thus, the dynamics can be expressed in the phase-amplitude normal form variables as
\begin{align}
    %\dot \phi_\mu &= \w_\mu + \e \mQ_\mu(\phi_1, r_1, \phi_2, r_2) 
    \dot \phi_\mu &= \w_\mu + \e \mQ_\mu(\Vec{\phi}, \Vec{r}) \;,
    \label{eq:ODE_phi} \\
    %\dot r_\mu &= \kappa_\mu r_\mu + \e \mF_\mu(\phi_1, r_1, \phi_2, r_2) \\
    \dot r_\mu &= \kappa_\mu r_\mu + \e \mF_\mu( \Vec{\phi}, \Vec{r})\;,
    \label{eq:ODE_r}
\end{align}
where $\Vec{\phi} = (\phi_1, \dots, \phi_M)^\top$ and $\Vec{r} = (r_1, \dots, r_M)^\top$. Furthermore, $\mQ_\mu$ and $\mF_\mu$ are the coupling functions in terms of the phases and the amplitudes: 
\begin{align}
    %\mQ_\mu(\phi_1, r_1, \phi_2, r_2)
    %=& \nabla_{\bx_\mu} \phi_\mu \cdot \bS_\mu(\bx_1, \bx_2) |_{\phi_1, r_1,\phi_2, r_2} 
    \mQ_\mu(\Vec{\phi}, \Vec{r})
    =& \nabla_{\bx_\mu} \Phi_\mu \cdot \bS_\mu(\bx_1, \dots, \bx_M) |_{\Vec{\phi}, \Vec{r}} \;,
    \label{eq:Q_def} \\
    %\mF_\mu(\phi_1, r_1, \phi_2, r_2)
    %=& \nabla_{\bx_\mu} r_\mu \cdot \bS_\mu(\bx_1, \bx_2) |_{\phi_1, r_1,\phi_2, r_2}
    \mF_\mu(\Vec{\phi}, \Vec{r})
    =& \nabla_{\bx_\mu} P_\mu \cdot \bS_\mu(\bx_1, \dots, \bx_M) |_{\Vec{\phi}, \Vec{r}}
    \,.
    \label{eq:F_def}
\end{align}
We remark that Eqs.~(\ref{eq:ODE_phi}-\ref{eq:F_def}) are equivalent to Eq.~\eqref{eq:ODE_original} as long as all states $\bx_\mu$ are in the domain of validity of transformations $\bx\to (\phi,r)$. One can argue that this domain extends to the whole basin of attraction of the respective limit cycle. However, we do not rely on this since the perturbation technique operates only in close vicinity of the cycle. So far, there has been no reduction, and the new system has the same dimension $2M$. 
...
\begin{align}
    %\dot{\phi}_\mu = \w_\mu + \e \mQ_\mu(\phi_1, R_1(\phi_1, \phi_2), \phi_2, R_2(\phi_1, \phi_2))
...
%Thus, the phase model \eqref{eq:ODE_phi_reduced} is  equivalently rewritten as a series of coupling functions appearing in different orders of $\e$:
...
%The aim of this paragraph is to derive $\mQ_{\mu;n}$ explicitly for $n \in \{0,1,2\}$ and give a route to compute them for $n\geq 3$.
...
%We derive that from \Eq.~\eqref{eq:ODE_r}: 
\begin{align}
    \dot{r}_\mu = \sum_{\nu=1}^M 
    \dot{\phi}_\nu \partial_{\phi_\nu} R_\mu 
    ...
    \,.
\end{align}
%where $\partial_{\phi_\nu}$ is the partial derivative with respect to $\phi_\nu$. 
...
By matching terms of the same power in $\e$ in Eq.~\eqref{eq:PDE_expansion}, we obtain a set of equations determining all $R_{\mu;n}$. First, the terms of  $\mO(\e)$ yield $R_{\mu;0} = 0$, reflecting that the amplitudes vanish asymptotically without coupling. 
Secondly, by writing for clarity the arguments of the unknown terms 
%and their partial derivatives 
explicitly, we obtain
\begin{align}
    \kappa_\mu R_{\mu;n}(\Vec{\phi}) - \sum_{\nu=1}^M \w_\nu \partial_{\phi_\nu} R_{\mu;n}(\Vec{\phi})
    = C_{\mu;n-1}(\Vec{\phi}) %\qquad \forall n \geq 1 \,.
\end{align}
...
%{\tt ----- from here on, I explictly use $M=2$. I try different notations for multiindezes in the following -----}
% \begin{align}
%     \Xi_\mu[f](\phi_1, \phi_2)
%     &=  \iint_0^{2\pi} f(\phi_1-s_1,\phi_2-s_2) \chi_\mu(s_1,s_2)  \dd s_1 \dd s_2 \\
%     \Xi_\mu[f](\phi_1, \dots, \phi_M)
%     &=  \int_0^{2\pi} f(\phi_1-s_1, \dots, \phi_M-s_M) \chi_\mu(s_1,s_2)  \dd s_1 \dots \dd s_M \\
%     \Xi_\mu[f](\bphi)
%     &=  \int_0^{2\pi} f(\bphi - \btheta) \chi_\mu(\btheta) \dd \btheta \\
%     \Xi_\mu[f](\Vec{\phi})
%     &=  \int_0^{2\pi} f(\Vec{\phi} - \Vec{\varphi}) \chi_\mu(\Vec{\varphi}) \dd \Vec{\varphi}
%     \,.
% \end{align}
\begin{align}
    \Xi_\mu[f](\Vec{\phi})
...
the denominator in \eqref{eq:oper} does not vanish for any $\Vec{\alpha}$; thus there are no small divisors in this perturbation technique.

%We provide details in Appendix~\ref{apx:PDE_solution}.

% with
% \begin{align}
%     \chi_\mu(\btheta)
%     &=\frac{1}{(2\pi)^M} \sum_{\alpha \in \mathbb{Z}^M} \frac{\ee^{-\ii \alpha \cdot \btheta}}{\kappa_\mu + \ii \alpha \cdot \omega} \\
%     \chi_\mu(s_1,s_2)&=\frac{1}{4\pi^2} \sum_{\alpha_1, \alpha_2 \in \mathbb{Z}} \frac{\ee^{-\ii (\alpha_1 s_1 + \alpha_2 s_2)}}{\kappa_\mu + \ii (\alpha_1\w_1 + \alpha_2 \w_2)} \\
%     \chi_\mu(\Vec{\phi})&=\frac{1}{(2\pi)^M} \sum_{\Vec{\alpha} \in \mathbb{Z}^M} \frac{\ee^{-\ii \Vec{\alpha}\cdot \Vec{\phi}}}{\kappa_\mu + \ii \Vec{\alpha} \cdot \Vec{\w}}
%     \,.
% \end{align}

Equation~\eqref{eq:R_n_solution} yields an expression for each $R_{\mu;n}$. However, the functions $\mQ_{\nu;m}$ and $\mF_{\nu;m}$ appearing in $C_{\mu;n}$ are not directly available from the definitions of $\mQ$ and $\mF$ given by  Eqs.~(\ref{eq:Q_def}) and (\ref{eq:F_def}). To write an expression for $R_{\mu;n}$ in terms of the original coupling functions, we additionally need to  express $\mQ$ and $\mF$ from Eqs.~(\ref{eq:ODE_phi},\ref{eq:ODE_r}) as expansions in powers of $R_\mu$. We write
\begin{align}
    %\mQ_\mu = \sum_{n=0}^\infty \sum_{k_1=0}^n Q_{\mu; (k_1,n-k_1)} R_1^{k_1} R_2^{n-k_1}
...
By inserting the $\e$-expansions of $R_\mu$ from Eq.~\eqref{eq:Q_expansion_in_eps} into Eq.~\eqref{eq:Q_expansion_in_r} we identify each $\mQ_{\mu;n}$ (or $\mF_{\mu;n}$) with an expression containing  $R_{\nu;m}$ and $Q_{\nu; \Vec{k}}$ (or $F_{\nu; \Vec{k}}$) by collecting terms of the same power in $\e$. Substituting these expressions in \eqref{eq:Cn_definition}, we obtain the r.h.s. for the determining the amplitudes in the next order, etc.
...
%, see Sec.~\ref{sec:example_driven_system} for details.%~\cite{rosenblum2019}

Next, the terms of $\mO(\e)$ yield
\begin{align}
    %\mQ_{\mu;1} = Q_{\mu;(1,0)} R_{1;1} + Q_{\mu;(0,1)} R_{2;1}
    \mQ_{\mu;1} = \sum_{\nu=1}^M Q_{\mu; \Vec{e}_\nu} R_{\nu;1}
    \,,
\end{align}
...
\begin{align}   
    \mQ_{\mu;1} = - \sum_{\nu=1}^M Q_{\mu; \Vec{e}_\nu} \Xi_\nu[F_{\nu;(0, \dots, 0)}] 
    %\quad -&
    %Q_{\mu;(1,0)} \Xi_1[F_{1;(0, \dots, 0)}] 
    %\nonumber \\ -& 
    %Q_{\mu;(0,1)} \Xi_2[F_{2;(0,0)}]
    \label{eq:Q1}
    \,.
\end{align}
%\end{empheq}
Eq.~\eqref{eq:Q1} marks the first non-trivial term of the phase-reduced model of generically coupled two-dimensional oscillators. We demonstrate the advantage of the corresponding $\mO(\e^2)$ phase model over the $\mO(\e)$ model in Sec.~\ref{sec:example_driven_system}. 
...
\begin{align}
    ...
    \,.
\end{align}
We now construct the $\mO(\e^2)$ model
\begin{align}
...
phase-coupling terms remain pair-wise in the leading order, but higher-order terms contain many-body (triplet, quadruplet, etc.) interactions; see discussion in Ref.~\onlinecite{gengel2021}.



%In future work, we will employ the same strategy to obtain a theory for coupled oscillators. We mention the possibility to obtain a second-order accurate Kuramoto model from a network of globally coupled Stuart-Landau oscillators.

%Future work will be attributed to extend this approach to populations of coupled two-dimensional oscillators, limit-cycle oscillators of more than two dimensions and 

%We emphasize that with the given approach we can iteratively compute each $\mQ_n$. However, an explicit formula is still missing 

%An open problem remains to give an explicit expression for the $R_n$ (non-iterative formula).
...
%\section{About the solution of PDE}
%\label{apx:PDE_solution}
...
%The curves mentioned in 
%we need the response curves $Q_0$, $F_0$ and $Q_1$, see Eqs.~\ref{eq:Q0} and \ref{eq:Q1}. In general, the information to compute $\mQ$ and $\mF$ is stored in the 
\begin{align}
    {\bf A} = \begin{pmatrix} 
    ...
    \,.
\end{align}
With that construction, the information for the $n$-th derivative of $\mQ$ and $\mF$ with respect to $r$ evaluated at $r=0$ is contained in the matrices $\bA_1, \dots, \bA_n$. For the purpose of deriving an $\mO(\e^2)$ phase model we need $\bA_0$ and $\bA_1$. We will detail their inference from the dynamical model in the following.
...
\begin{align}
    \bu_1(t) = M(t) \cdot \bu_1(0)\;,
    \label{eq:u1_solution}
\end{align}
where $M$ is the principal fundamental solution, that we find by numerically integrating Eq.~\eqref{eq:linearized_deviation_limit_cycle} in the interval $0 \leq t \leq T$ with initial conditions $\bu_1(0) \in \{(1,0)^\top, (0,1)^\top\}$. The non-unity eigenvalue of the monodromy matrix $M(T)$ is the Floquet multiplier $\ee^{\kappa T}$ from which we derive the real Floquet exponent $\kappa < 0$.

Thus, transforming back to $\bX_1$ yields 
$
    \bX_1(\phi) = \ee^{-\kappa \phi/\omega} M\left(\phi/\omega\right) \bX_1(0)$, $
    \quad \phi \in [0,2\pi)$
Since we require $\bX_1(2\pi) = \bX_1(0)$, the initial condition $\bX_1(0)$ has to be an eigenvector of $M(T)$ corresponding to the non-unity Floquet multiplier. We fix the scaling of the isostable coordinate by choosing $||\bX_1(0)||=-1$ inside the limit cycle, where $||.||$ denotes the standard Euclidean norm.
...
\begin{align}
   ...
   \,.
\end{align}
% Eqs.~\eqref{eq:ODE_X2}, \eqref{eq:Cx_def} and \eqref{eq:Cy_def}
Here and in the following, we omit the notation of argument $\phi$ for conciseness. The term $\bC$ is defined component-wise as
\begin{align}
...
\begin{align}
    ...
    \label{eq:ODE_u2}
\end{align}
where we used Eq.~\eqref{eq:u1_def} to replace $\bX_1$ by $\bu_1$. Since $\bX_0$ and $\bu_1$ are known, this inhomogeneous linear ODE can be solved with Floquet theory. In fact, we employ the principal fundamental solution $M(t)$ from Eq.~\eqref{eq:u1_solution} to write the general solution as
\begin{align}
    \bu_2(t) = M(t) \bu_2(0) + \Tilde{\bu}_2(t)
    \,.
\end{align}
Here, $\Tilde{\bu}_2$ is the special solution to Eq.~\eqref{eq:ODE_u2} with initial condition $\Tilde{\bu}_2=0$. Thus, we obtain
\begin{align}
    \bX_2(\phi) =  \ee^{-2\kappa \phi/\w} \left( M(\phi/\w) \bX_2(0) + \Tilde{\bu}_2(\phi/\w) \right)\;,
\end{align}
where the initial condition $\bX_2(0)=\bX_2(2\pi)$ has to satisfy
\begin{align}
    \bX_2(0) = (\mathbb{1}-\ee^{-2\kappa T} M(T))^{-1}\ee^{-2\kappa T} \Tilde{\bu}_2(T)
    \,
\end{align}
to ensure the $2\pi$-periodicity. With $\bX_0$, $\bX_1$ and $\bX_2$, we construct $\bJ_0$ and $\bJ_1$, and thus compute $\bA_0$ and $\bA_1$.


% Thus, we can now construct initial conditions with given phase $\phi_0$ and small isostable $|r_0| \ll 1$ inside and outside of the limit cycle by
% \begin{align}
%     \bx(0) = \bx_0(\phi_0) + r_0 \bx_1(\phi_0)
% \end{align}
% making an error of $\mO(r_0^2)$. Starting at $\bx(0)$ at $t=0$ we integrate the system backward in time to obtain a series of data points $\bx(t)$ with $t<0$.

% % \phi_0 + \w t

% first-order approximation defined by
% \begin{align}
%     \bx^{(1)}(t) = \bx_0(\phi_0 + \w t) - r_0 \ee^{\kappa t} \bx_1(\phi_0 + \w t)
% \end{align}
% with 
% \begin{align}
%     \bx^{(1)}(0) = \bx_0(\phi_0) + r_0 \bx_1(\phi_0)
% \end{align}
% we can argue that for very small $r_0$, the phase-isostable coordinate of $\bx^{(1)}$ are indeed $\phi_0$ and $r_0$.
% Integrating the system numerically, starting at $\bx(0)=\bx^{(1)}(0)$, we can ...
% We obtain trajectories $\bx(\phi_0, t)$. A family of trajectories distributed in $\phi_0$ will
% \begin{align}
%     \int_0^{2\pi} |\bx(\phi_0, t) - \bx^{(1)}(\phi_0, t)| \dd \phi_0 \propto \ee^{2\kappa t}
% \end{align}
% for a certain small time interval. Using 
% \begin{align}
%     \bx_2(\phi(t)) = \frac{\bx(t) - \bx^{(1)}(t)}{(r_0 \ee^{\kappa t})^2/2} + \mO(r_0 \ee^{\kappa t})
% \end{align}
% we can obtain $\bx_2$ by a least squares fit of data in the linear regime. We iterate the system backward for a number of initial conditions on $r$ in time.
