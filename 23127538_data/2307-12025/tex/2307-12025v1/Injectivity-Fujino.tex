...
\usepackage[pdftex,top=3cm,bottom=3cm,outer=3cm,inner=2cm,marginpar=2.45cm]{geometry}
...
Y.C.~and M.C.~are supported by the National Research Foundation
...
The following notions are used throughout this paper without any mention. 
\begin{itemize}
...
=\set{z_{p(1)} =z_{p(2)} =\dotsm =z_{p(\sigma)}=0}$ when non-empty. 
  ...
  supported smooth function (i.e.~a smooth cut-off function) being
  identically $1$ on $V$, one obtains, 
  ...
  \begin{equation*}
    ...
    =\RTF[\rho]|f|(0),<V>
  \end{equation*}
  for any smooth cut-off function $\rho$ on $V$ (see
  ...
  \begin{prop} \label{prop:res-formula-dbar-exact-dot-harmonic}
    ...
    With the locally finite cover $\cvr V$ and partition of unity
  ...
  \begin{proof}
    ...
    \begin{equation*}
      ...
      \begin{aligned}[t]
        &=\mtidlof<\lcS>{\vphi_L} \cdot
        \res{\defidlof{\lcc+2'}}_{\lcS} \;\;\footnotemark
        \\
        &=\aidlof|1|<\lcS>{\vphi_L}[\psi_{(p)}]
      \end{aligned}
    \end{equation*}%
    \footnotetext{
...

{
  ...
  We define the map $\widehat{\mathcal{R}}_{D_p}:\mathscr
  A_X^{0,q}(X,K_X\otimes L)\rightarrow \mathscr
  A_{D_p}^{0,q-1}(D_p,K_{D_p}\otimes L\vert_{D_p})$ by 
  \begin{equation*}
    \widehat{\mathcal{R}}_{D_p}(u)
    =
    \mathcal{R}_{D_p}\left((\partial\psi_D)^*u\right)\;\;\;\text{for}\;\;\;p\in
    I_D \; ,
  \end{equation*}
  ...
  \begin{equation*}
    \widehat{\mathcal{R}}_{D_p}(u)
    =
    \mathcal{R}_{D_p}\left((\partial\psi_D)^*u\right)
    =
    \mathcal{R}_{D_p}\paren{\paren{\frac{dz^1}{z^1}}^*u}
    =
    \paren{(dz^1)^*\widetilde u_p}\big\vert_{D_p} \; ,
  \end{equation*}
  where $u=dz^1\wedge\widetilde{u}_p$.
  In particular, $\widehat{\mathcal{R}}_{D_p}$ does not depend on the
  choice of the Hermitian metric $\sm\varphi_{D}$. 
  It follows from the above formula that
  $\widehat{\mathcal{R}}_{D_p}(u)$ is actually a $K_{D_p}\otimes
  ...
  $\widehat{\mathcal R}_{D_p}$ on a K\"ahler manifold.
  \begin{prop} \label{prop:harmonic-residue}
    If $u$ is a $\dfadj$-closed $K_X\otimes L$-valued $(0,q)$-form on
    $X$, then $\widehat{\mathcal R}_{D_p}(u)$ is a $\dfadj$-closed
  ...
  \begin{proof}
    ...
    \begin{align*}
      \paren{\dfadj \widehat\PRes_{D_p}(u)}_{\conj\bbeta_{q-2}}
      &=
        -g^{\bar\beta\gamma}
	\nabla_\gamma 
	\paren{
    ...
    completed. \qedhere
    % \begin{align*}
    %   \paren{\dfadj\widehat{\mathcal R}_{D_p}(u)}_{\ov{\boldsymbol\beta}_{q-2}}
    %   &=
    %   -g^{\bar\beta\alpha}
    %   \nabla_\alpha 
    %   \paren{
  ...
  $\widehat{\mathcal{R}}_{D_p}(u)$ is $\dbar$-closed.
  ...
  We see that $\widehat{\mathcal R}_{D_p}(u)$ is $\dbar$-closed by
  ...
  \begin{remark}
    ...
    $\widehat{\mathcal{R}}_{D_p}(u)\in\mathrm{Dom}\;\dbar^*$ with
    ...
    Indeed, $\widehat{\mathcal{R}}_{D_p}(u)$ can be shown to be $L^2$
    ...
    $\widehat{\mathcal{R}}_{D_p}(u)\in\mathrm{Dom}\dbar^*$ follows
...
\begin{equation*}
  ...
  \aidlof|1|* = \smashoperator{\sum_{\substack{p,q = 1 \\ p\neq q}}^3} \defidlof{D_p \cap D_q} \;
  \text{ and } \;
  \aidlof|0|* = \defidlof{D} 
\end{equation*}
in this case.
...
\begin{remark} \label{rem:general-commut-diagram}
  ...
  0$ and $\residlof|0|* =\aidlof|0|*$) which recovers the result in
...
\begin{proof}
  ...
  \item \label{item:pf:use_u-ortho-w}
    ...
    \begin{equation*}
      ...
      \frac{\rs*\gamma_{p;\:\idx 1.q}}{\sect_{(p)}}
      =: (-1)^q \:\frac{\dbar v_{p; (\infty)}'}{\sect_{(p)}}
      \quad\text{ on } \lcS
    \end{equation*}
