...
% \newcommandx{\carol}[2][1=]{\todo[linecolor=magenta,backgroundcolor=blue!25,bordercolor=magenta,#1]{#2}}
% %%%%%%%%
...
Apart from traditional ways of dealing with imbalanced classes (e.g. %labeling more examples, 
under or oversampling %the dataset, 
or applying class weighting), semi-supervised techniques such as self-training can be used to extend the training set with unseen examples that introduce new learning signals without the costly burden of manual data labeling.
...
(Section \ref{ref:related_work_data_aug}). This indicates that there may not be a best method, while some may even negatively impact performance.
...
% a method that draws samples from the original data and combines them convexly. 
...
% Notably, the best method (mixup) does not achieve any increase in accuracy, i.e. the same score as if no augmentations were applied. 
...
\citet{xie2020unsupervised} use six text classification and two image classification benchmark datasets to experiment with different types of noise-inducing techniques %to verify their relevance to 
for self-training. They argue that state-of-the-art augmentations like backtranslation for text classification and RandAugment \citep{randaugment} for image classification, outperform simple noise inducing techniques, such as additive Gaussian noise.
...
% \citet{li-2020-lee} describe a submission to a shared task of identifying hate speech and offensive content in Indo-European languages. Authors participate in the English part of subtask A, which aims to identify hate speech and offensive content in English. They propose an Albert-based model and use self-training and majority voting ensamble, achieving a macro-f1 score of 49.76\%, ranking 20/35.
...
% \carol{and what is the difference between your work and Santos et al.? }
...
% \begin{tabular}{lccc}
% \multicolumn{4}{c}{OLID}                                       \\ \hline
%          & Train           & Dev             & Test            \\ \hline
% Not-Offensive & 8,840 (66.76\%)  & 0 (0\%)               & 620 (72.09\%)   \\
% Offensive & 4,400 (33.24\%)  & 0 (0\%)              & 240 (27.91\%)   \\ \hline
% \multicolumn{4}{c}{ConvAbuse}                                  \\ \hline
% Not-Offensive & 2,163 (86.48\%)  & 719 (86.52\%)   & 725 (84.99\%)   \\
% Offensive & 338 (13.52\%)   & 112 (13.47\%)   & 128 (15.01\%)   \\ \hline
% \end{tabular}%
% }
% \caption{\label{tab:datasets-distributions}Target class distribution for each dataset}
% \end{table}

\subsection{Data Description} \label{sec:data-description}
We use two English binary offensive/hate speech detection datasets in our experiments. Table~\ref{tab:datasets-distributions} presents their target class distributions. %over each split.
...
\end{tabular}%
\caption{\label{tab:datasets-distributions}Target class distribution for OLID and ConvAbuse.}
\end{table}

% \paragraph{Waseem} \citep{waseem-hovy:2016:N16-2} creates a dataset of 16,914 tweets annotated for hate speech, of which 3,383 examples contain sexist content, 1,972 contain racist content and the remaining 11,559 are annotated as neither sexist or racist. Authors do not attempt to balance the dataset aiming towards a realistic distribution of the classes. Their Fleiss' kappa inter-annotator agreement is 0.84 between their two annotators, indicating strong agreement. Their best baseline model is a logistic regression classifier with character n-grams and the gender feature associated with the author of the tweet, obtaining 73.93\% macro F1-score on a 10-fold cross validation. In our experiments, we combine the racist and sexist tweets into a single 'offensive' class, transforming the dataset into a binary version. Also, as the train, development and test splits are not provided, we randomly split the dataset into 70\% for training, 15\% for development and 15\% for testing.
...
% \paragraph{Measuring Hate Speech (MHS)} \citep{mhsdataset} presents a hate speech dataset consisting of more than 50,000 social media comments sourced from YouTube, Twitter, and Reddit and labelled by 11,000 U.S.-based Amazon Mechanical Turk workers. The prime target variable is the ``hate speech score'', a continuous hate speech measure constructed from 10 other labels: sentiment, respect, insult, humiliation, inferior status, violence, dehumanization, genocide, attack/defense, and hate speech benchmark. It also includes eight target identity groups: race/ethnicity, religion, national origin/citizenship, gender, sexual orientation, age, disability, and political ideology. In our experiments, we only used the input text and the prime target variable ``hate speech score''. This dataset is split into 70\% training, 15\% development, and 15\% testing for our experiments. 
...
% \carol{Any intuition why German?}
...
% We find an average target class shift percentage of $23.8\%$, $23.5\%$, and $23.3\%$ for backtranslation, synonym substitution, and word swap, respectively. For backtranslation, $54.7\%$ of the total target shifting is from negative target shift, meaning an offensive example was classified as not-offensive after being backtranslated, and $46.3\%$ is from positive target shift, meaning a non-offensive example was classified as offensive after being backtranslated. For synonym substitution we observe $51.$

% Backtranslation augmentation produces the most amount target class shifting 

% Table \ref{tab:label-shift} presents the average target class shift percentage, either from positive to negative, or negative to positive, when using a classifier trained with one of the human-labelled datasets, then performing inference on both the `clean' and the augmented unlabelled set. Backtranslation augmentation produces the most target class shifting on both datasets, and synonym substitution produces the least average class shifting on OLID, while word swap produces the least average class shifting on ConvAbuse.
...
% \begin{tabular}{@{}llll@{}}
% \toprule
% Trained with & \multicolumn{1}{c}{BT} & \multicolumn{1}{c}{SS} & \multicolumn{1}{c}{WS} \\ \midrule
% OLID         & 29.2\%\footnotesize{ ± 3.1\%}             & \textbf{28.9\%\footnotesize{ ± 3.1\%}}    & 29.3\%\footnotesize{ ± 2.8\%}             \\
% ConvAbuse    & 18.4\%\footnotesize{ ± 6.2\%}             & 18.1\%\footnotesize{ ± 6.1\%}             & \textbf{17.2\%\footnotesize{ ± 5.5\%}}    \\ \bottomrule
% \end{tabular}
% }
% \caption{\label{tab:label-shift}Average target class shift percentage. Lowest values across datasets are in bold.}
% \end{table}
...
% fuck being a gentlemen                           & being a fuck gentlemen                              & WS                         \\
...
\hline
Text                        & Augmented Text                         & Method \\ \hline
% kanye such a fuckin child                       & such kanye fuckin a child                              & WS                         \\
% Is that a dumb blonde question                  & That is dumb a blonde question                         & WS                         \\
...
% Original Text                                 \\ \hline
% what are you? queer?                          \\
% Lol. You're a :clown-face:.                   \\
% Go get the fuckin ball                        \\
% Man this would be cool as fuck if it was true \\
% I would quit for a whole fucking lot less!    \\
% Should have kneed him right in the b*lls.     \\ \hline
% Back-translated Text                          \\ \hline
% what are you? strange?                        \\
% Lol. You are a .                              \\
% Go get the damn ball                          \\
% Man this would be cool if it was true         \\
% I would stop for a lot less!                  \\
% Should have handcuffed him straight.         
% \end{tabular}
% \caption{Examples of target shift from positive to negative after applying back-translation.}
% \label{tab:negative-shift}
% \end{table}

% Table \ref{tab:positive-shift} shows examples that shifted from negative to positive. In those cases, the opposite happens as non-offensive or slightly offensive keywords are swapped for more intensively offensive keywords. 

% \begin{table}[!htb]
% \begin{tabular}{l}
% \hline
% Original Text                                      \\ \hline
% He’s a nut. They’re all nuts!                      \\
% Twitter really snitches on you                     \\
% I can’t stop laughing at this, he’s so goofy :joy: \\
% Awww. Glad you are clean kitties!                  \\
% You damn near got me for a second lmaoo            \\
% This is sad.                                       \\ \hline
% Back-translated Text                               \\ \hline
% He's an idiot. They are all crazy!                 \\
% Twitter is really screwing you                     \\
% I can't help but laugh he's so crazy               \\
% Awww. Glad you are clean pussies!                  \\
% You almost fucking got me for a second lmaoo       \\
% This is pathetic.                                  \\ \hline
% \end{tabular}
% \caption{Examples of target shift from negative to positive after applying back-translation.}
% \label{tab:positive-shift}
% \end{table}
...
\section{Future Work}
% In this work we discuss problems with data augmentation techniques when applied to offensive/hate speech tasks in the context of noisy self-training.

% First, as we discussed previously, there is no dataset annotated for offense/hate speech before and after applying data augmentation, which would enable a more accurate measurement of the semantic variability produced by each method. Thus, we see that one possible future research idea is to produce such dataset and benchmark existing data augmentation methods.
...
% the SoBigData TransNational Access program (EU H2020, grant agreement: 654024).
