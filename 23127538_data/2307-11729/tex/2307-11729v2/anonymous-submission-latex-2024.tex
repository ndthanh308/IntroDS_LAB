...
Experiments in the domain of student essays show that the proposed detector improves the detection performance on the attacker-generated texts by up to +41.3 points in F1-score. 
Furthermore, the proposed detector shows a state-of-the-art detection performance: up to 96.9 points in F1-score, beating existing detectors on non-attacked texts.
Finally, the proposed attacker drastically degrades the performance of detectors by up to -57.0 points F1-score, massively outperforming the baseline paraphrasing method for evading detection.\footnote{Our code and dataset are publicly available at: \url{https://github.com/ryuryukke/OUTFOX}}
\end{abstract}
...
In our analysis, we explore the semantic differences between non-attacked and attacked essays. The analysis reveals that our attacker-generated essays are semantically closer to human-written essays than non-attacked essays, leading to success in such effective attacking.
...
% The detector receives an essay as input and predicts a binary label: \texttt{Human} or \texttt{LM}. The attacker receives an essay-problem statement as input and generates an essay that deceives the detector. In our OUTFOX framework, to improve the robustness of the detector, both the detector and attacker can consider each other's output as examples for in-context learning.
...
To generate non-attacked essays, we instruct the instruction-tuned LMs: ChatGPT (gpt-3.5-turbo-0613), GPT-3.5 (text-davinci-003), and FLAN-T5-XXL\footnote{\url{https://huggingface.co/google/flan-t5-xxl}}.
...
Our OUTFOX detector is based on ChatGPT (gpt-3.5-turbo-0613). We set the $\mathsf{temperature}$ and $\mathsf{top\_p}$ parameters to 0 in order to eliminate the randomness of our detection.
Our detector takes retrieved in-context examples for an essay.
In detection with considering attacks, the in-context examples are 5 human-written and 5 LLM-generated essays.
In detection without considering attacks, the in-context examples are 5 human-written, 3 attacked, and 2 LLM-generated essays.
...
Our OUTFOX attacker is based on ChatGPT (gpt-3.5-turbo-0613).
...
& &\multicolumn{1}{l}{DetectGPT} & 30.0 & 71.6 & 50.8 & 59.3\\
...
\hline
\end{tabular}
\caption{\label{comparison_baseline}
...
\end{tabular}
\caption{\label{unsuccessful-attack}
...
\end{tabular}
\caption{\label{successful-attack}
