...
\author{Sen Fang$^{1}$, Bowen Gao$^{2}$, Yangjian Wu$^{3}$, Teik Toe Teoh$^{4, \star}$ \\
   $^{1, 2}$Victoria University, $^{3}$Hainan University, $^{4}$Nanyang Technological University\\
   \small \texttt{\{sen.fang, bowen.gao\}@live.vu.edu.au, yangjian.wu@hainanu.edu.cn}\\
...
In this paper, we propose a novel universal language representation learning method called UniBriVL, which is based on Bridging-Vision-and-Language (BriVL). 
\textbf{Uni}versal \textbf{BriVL} embeds audio, image, and text into a shared space, enabling the realization of various multimodal applications.  
Our approach addresses major challenges in robust language (both text and audio) representation learning and effectively captures the correlation between audio and image.
...
The entry point for our selection of the new language modality is Microsoft's latest developed model, SpeechLM \cite{zhang2023speechlm}, which is a fusion model of text and audio. It is capable of outputting text and audio as the same representation. This allows us to input text, audio, or both when using the model. Consequently, this significantly enhances the adaptability of the model to various tasks, such as audio-text retrieval, image retrieval, audio recognition, image captioning, and even theoretically enables better perception of real-life scenarios through simultaneous processing of live speech and text. We conducted a comprehensive evaluation of our model in the aforementioned tasks. The experimental results demonstrate its strong generalizability and excellent performance in the main experiments. 
%This confirms the effectiveness of our model and its potential applications in the future.
...
\begin{equation}
    p(c|\mathbf{n}_t)=\frac{\exp(\mathrm{sim}(\mathbf{K}^P\mathbf{n}^{L}_{t},\mathbf{e}_c)/\tau)}{\sum_{c'=1}^{C}\exp(\mathrm{sim}(\mathbf{K}^P\mathbf{n}^{L}_{t},\mathbf{e}_{c'})/\tau)}
\end{equation}
where $\mathbf{K}^P$ is a projection matrix, $\mathbf{n}^{L}_{t}$ is the output hidden state for step $t$, $\mathbf{e}_c$ is the embedding for class $c$, $\mathrm{sim}(a,b)$ means the cosine similarity between $a$ and $b$, and $\tau=0.1$ scales the logit \cite{chen2022wavlm}.
...
Loss function with cross projection defined as $CX Loss = L(f(Image), Language) + L(Image, g(Language))$ ($f, g$: projection functions and $L$: contrastive loss).
For all models, we use grid search to find the best
hyperparameter. 
For other hyperparameters (such as batch size, training steps, learning rate, etc.), we directly use the suggested values in the original papers. 
Note that for per-instance perturbation, we adopt the appropriate quantity compared to the original epochs.
...
& \multicolumn{3}{c}{Classification} & \multicolumn{5}{c}{Retrieval} \\
...
UniBriVL & \textbf{0.9307} & \textbf{0.8722} & \textbf{0.4885} & 0.4111 & \textbf{0.0641} & \textbf{0.0612} \\
\midrule
...
UniBriVL & \textbf{0.434} & \textbf{0.107} & \textbf{0.115} & \textbf{0.268} & \textbf{0.126} \\
%SOTA & 0.619 & 0.439 & 0.313 & 0.220 & 0.186 \\
%Best & 0.619 & 0.439 & 0.313 & 0.220 & 0.186 & 0.417 & 0.521 & 0.129 & 0.325 \\
\bottomrule
\end{tabular}
\caption{Results of audio captionin, ASR, compared with baseline \cite{Drossos_2020_icassp}. We tested some tasks on the test tools we worked on previously\footnote{\url{http://dcase.community/challenge2020/task-automatic-audio-captioning}} and we exclude Bleu2/3, list Bleu1/4 (B1/4), METEOR (M), ROUGEL (RL), CIDEr (Cr).}
...
Our method uses the UniBriVL model to guide the generation of Stable Diffusion.
...
Our research results indicate that the majority of participants believe that the generated images are related to the input sound, thus verifying our method's ability to generate images related to a given sound, and it was a good match.
...
When they faced the input of "Water Sound", our work generated a small stream, WavCLIP generated symbolic images similar to fish fossils, and the other images have similar features.
...
This article introduces a UniBriVL method for generating generic representations.
...
In future research, we will explore a number of interpretable machine learning methods, consider extending to 6 modalities to our work, just like ImageBind \cite{girdhar2023imagebind}.
We will also consider exploring more efficient presentation and using the Consistency Models \cite{song2023consistency} and the NeRF \cite{mildenhall2020nerf} as the next version of the work and method.
%and all are well worth looking forward to.
