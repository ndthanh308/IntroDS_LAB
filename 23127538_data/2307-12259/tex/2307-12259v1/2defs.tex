...
are allowed to be unbounded. For instance, an acute sector
bounded by two rays emanating from the same point
would count as a polygonal region for us.
...
$A$ and $B$.  Figure 3 shows the construction.

\begin{center}
\resizebox{!}{2.8in}{% Figure removed}
\newline
{\bf Figure 3:\/} Symplectic Billiards Defined
\end{center}
...
the point $a_3$ or the point $b_4$ might be
...
Here I will report on some
preliminary experiments I did with
I did not experiment too much with
...
arithmetic, so I will restrict my attention
to that case.

Given
$t \in \Q$ define
\begin{equation}
  z_t=\bigg(\frac{1-t^2}{1+t^2},\frac{2t}{1+t^2}\bigg).
\end{equation}
...
\newline
    {\bf Figure 4:\/} The left half of an orbit on $(A,A_{1/3})$.
\end{center}

The orbit seems to be bounded and aperiodic.  
I did not formally prove the aperiodicity, but
...
an attracting fixed point.  After playing
around for a while, I conjecture that
all orbits on $(A,A_{1/3})$ are aperiodic
and bounded.  They can have a fairly
complicated pre-periodic part but the
limit cycle always seems to look the same.

Figure 5 shows a picture of an unbounded orbit
...
did not write down a formal proof.


\begin{center}
\resizebox{!}{4.5in}{% Figure removed}
\newline
    {\bf Figure 5:\/} A symplectic billiard orbit on $(A,A_{7/11})$.
\end{center}

My preliminary experments lead  me to make the following
conjecture.

\begin{conjecture}
  All symplectic tiling billiards orbits, with respect
  to a pair of rationally rotated square grids,
  are infinite.  That is, there are no periodic orbits
  at all.
\end{conjecture}

Some parameters seem to support both unbounded orbits
and bounded aperiodic orbits.  I intend to write a
program which colors the points in the phase space
of the system according to whether they have a
bounded or unbounded orbit. I bet this will be a
pretty picture.

For irrational
parameters, one can sometimes get periodic orbits.
