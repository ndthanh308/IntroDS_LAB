...
\affiliation{AGH University of Krak\'ow, Faculty of Physics and Applied Computer Science, \\
...
\cite{s,e,gss,eh,gl,kmh,g,wbbj},
mass-transport \cite{mez,emz,emz2}, random trees \cite{bb,j}, 
...
The balls-in-boxes model has also been applied in studies of such diverse problems as wealth condensation \cite{bjjknpz} or diversity of Zipf's populations \cite{maac}. The model 
can also be used to mimic phase separation \cite{klmst,elmm}, 
...
We will describe in detail how the order of the transition depends on the parameters of the model in various ensembles and present some new results on the singularities of the thermodynamic potentials, the relationships between them 
and the (slow) approach to the limiting distribution in the condensed phase. 
We also discuss some subtleties in  a { quasi}-probabilistic normalisation used in \cite{maac} which regularizes an otherwise divergent sum over the weights and elucidate its exact relation to the class of models here. 

The paper is organized as follows. In the next section
we will introduce a canonical ensemble and the basic quantities 
that describe the behaviour of the system in this canonical ensemble.
We then apply the saddle point method to calculate the 
free energy density of the system for low particle densities that are smaller
than some critical density. For densities larger than this critical density the system exhibits a real-space condensation. Whether there is a phase transition at a finite critical density or not depends on the asymptotic behaviour of the weights governing the particle distribution.
We will divide the weight functions into three families
for which the system has only the fluid phase,  
only the condensed phase, or has both phases 
with a phase transition between them at a finite critical density. We will concentrate on power-law weights to analyse the types of possible phase transitions. We will list possible scenarios that depend on the power in the weight function and determine the free energy and its behaviour at the critical point. We will then repeat these steps for the grand-canonical ensemble. As we will see, the canonical system has a continuous phase transition, while the grand-canonical system
may additionally have a discontinuous phase transition. 
We then  move on to discuss other ensembles and the quasi-probabilistic weights of  \cite{maac}. We conclude with 
a short summary.
...
In some conditions, the presence of this constraint leads to a phase transition, as will be seen later. The statistical weights $w(s_i)$ are identical for all boxes. 
The model is thus symmetric under the permutation of box
labels. The phase transition observed in the model is related to spontaneous permutation symmetry breaking,
where one box behaves differently to the others and captures an extensive number of particles.
...
As an example, consider the box-occupation
probability which is the probability that a randomly selected box contains  $q$ particles: 
\begin{equation}
\pi(q) = \frac{1}{N} \sum_{i=1}^N \delta_{q-s_i} . 
\label{pi_definition}
\end{equation}
Because all the boxes are identical, the ensemble average of $\pi(q)$ is the same as 
the ensemble average of the probability that the first box has $q$ particles $\delta_{q-s_1}$. This probability
can be calculated directly from the partition function by observing that if the first 
box contains $q$ particles then the remaining $N-1$ boxes contain $S-q$ particles.
...
\section{Thermodynamic limit} \label{sec:thermolimit}
...
by the radius of convergence of the series (\ref{K})
\begin{equation}
...
In the thermodynamic limit (\ref{dlimit}), we can also derive an asymptotic form of the particle distribution (\ref{piq}) by replacing
the partition functions in the numerator and denominator
by their asymptotic forms (\ref{spZ}):
\begin{equation}
...
\end{equation}
where $r=N/S$ and $\tilde{r}=(N-1)/(S-q)$. In the thermodynamic limit 
(\ref{dlimit})
\begin{equation}
...
\begin{widetext}
\begin{equation}
    \log Z_{S-q,N-1} \propto (S-q) \alpha(r)+(N-1) K\alpha(r)) -  \alpha'(r)\frac{(q r-1)^2}{rS} + o\left(S^{-1}\right)
\end{equation}
\end{widetext}
The calculations are straightforward. In particular, they show
that the $O(1)$ terms cancel out on virtue of the saddle point
equation \eqref{alphastar}.
Dropping the $O(S^{-1})$ terms we finally obtain in the thermodynamic limit (\ref{dlimit})
\begin{equation}
  \frac{Z_{S-q,N-1}}{Z_{S,N}} \propto e^{-q\alpha(r) - K(\alpha(r))}
\end{equation}
and 
\begin{equation}
...
Neglecting the $O(S^{-1})$ terms we have assumed that $q\ll S$. 
This is true for particle densities smaller than 
\begin{equation}
\rho_{\rm c} \equiv \frac{1}{r_{\rm c}}
\label{rhoc}
\end{equation} where this result is expected to hold.  Above $\rho_c$ another {\em condensed} phase appears - the condensed phase where an extensive (proportional to $S$) number of particles condense in one box. In the next section we discuss in detail the phase transition between the two phases. 

\section{Phase structure} \label{sec_phase_t}
...
symmetry (also called the condensed phase) for large particle densities.
The fluid phase is described by the solution of the saddle point equations \eqref{alphastar}. In the condensed phase, the equations are no longer valid. The critical density at which the transition from one phase to another occurs is given by \eqref{rhoc} with $r_c$ defined by \eqref{rcr}. Whether the system has a phase transition or not depends on the weight function $w(s)$. Let us discuss the possible scenarios.
...
$r \in (0,1)$. The saddle point equation \eqref{alphastar} holds for $r \in (r_{\rm c}, 1)$ where $r_{\rm c}$ is given by \eqref{rcr} (with $\alpha_c=0)$. Two situations can 
...
phase, below we will perform a finite size analysis.
...
The excess probability accumulated in the tails
\begin{equation}
    P_{\rm dev} = \sum_{q=1}^{q_{\max}} \left( 
    \langle \pi(q) \rangle_{S,N} - \frac{N-1}{N} \pi_{\rm c}(q) - \frac{1}{N}
    \pi_{\rm cond}(q) \right)
\end{equation}
decreases as $N$ increases. The excess probability is calculated as the area between the curve $\langle \pi(q) \rangle_{S,N}$ and a curve obtained as a weighted sum of the limiting distribution (\ref{pi4}) and the Gaussian peak (\ref{Gp}), to the left of the maximum
of the peak. The weights $(N-1)/N$ and $1/N$ correspond to 
the fractions of boxes occupied by the critical subsystem 
...
$4.2\%$, $2.8\%$ and $1.8\%$, respectively. This means that extra events associated 
with this part of the distribution are suppressed in the thermodynamic limit. The events in
this part of the distribution correspond to the situation where also the number of particles in the second most occupied box is significantly larger than in the remaining boxes, or in other words that the excess of particles is shared by two boxes \cite{gl}.
...
\end{equation}
\end{widetext}
where $H_n=1+1/2+\ldots+1/n$ is the $n$-th harmonic number. The behaviour of $K(\alpha)$
for $\alpha\rightarrow 0^+$ depends on $\beta$. Regarding the critical behaviour of the model, 
which is encoded in the singular behaviour of the free energy
density $\phi(r)$ (\ref{phi}), 
...
\end{itemize}
The free energy density $\phi(r)$ can be calculated using the saddle point equations (\ref{sp_phi}) and (\ref{alphastar}) which lead to a parametric representation of $\phi(r)$
\begin{equation}
    r = -\frac{1}{K'(\alpha)}, \ 
...
infinite for (D). The main difference between (A) and (B) is that the second
derivative $\phi''(r)$ is continuous at $r_{\rm c}$ for (B) and discontinuous
for (A). 

\section{Singularities of free energy} \label{sec:s_fenergy}

In this section we will discuss singularities of the free energy 
...
For the cases (D) and (C), that is for $\beta \in (-\infty,2]$ there is no phases
...
and the borderline case $\beta=3$. Let's begin with $\beta \in (2,3)$.
...
Also for $r\rightarrow r_{\rm c}^-$ the second derivative is equal zero  $\phi''\left(r_{\rm c}^-\right) = 0$ so the second derivative is continuous for $\beta \in (2,3)$. In the same way it can be checked
that the third derivative is continuous for $\beta \in (2,5/2)$ and it is discontinuous 
for $\beta\in [5/2,3)$. So the transition is the third order for $\beta\in [5/2,3)$.
...
\end{equation}
Therefore the transition is of the second order.
...
\end{split}    
\end{equation}
To summarize, the phase transition is of the second order for 
...
of the second order but $\phi''(r)$ has a logarithmic discontinuity 
$\log (r-r_{\rm c})$, as $r$ approaches $r_{\rm c}$ from above. 
For $\beta\in (2,3)$, the phase transition is of the third or higher order. 
...
the saddle point equation breaks down and the average $r=r(\mu)$ (\ref{rho_mu}) drops to zero. In this case $N$ grows sub-linearly with $S$ for $S\rightarrow \infty$.
...
\end{equation}
for $S \rightarrow \infty$ when the particle density in the canonical ensemble
is related to the chemical potential in the grand-canonical ensemble as follows
$N/S =  -K'(K^{-1}(\mu))$ (\ref{rho_mu}).
The equivalence holds in the saddle point regime (fluid phase), that is for $\mu< \mu_{\rm c}$ (\ref{mu_cr}). It breaks down for $\mu\ge \mu_{\rm c}$.

To conclude this section let us draw curves representing 
...
\section{Singularities of grand potential} \label{sec:s_grand_pot}

In this section will determine singularities of grand potential
...
for integer $\beta$), for $\mu \rightarrow \mu_{cr}^-$. 
...
that the second derivative diverges as $\psi''(\mu) \sim (\mu_{\rm c}-\mu)^{-1}$, as $\mu$ tends to $\mu_{\rm c}$,
...
To summarize, in the grand-canonical ensemble 
the phase transition is  first order for $\beta\in (2,+\infty)$. 
For $\beta=2$, it is second order: the second derivative has a logarithmic 
singularity $\psi''(\mu) \sim \log(\mu_{\rm c}-\mu)$ for $\mu\rightarrow \mu_{\rm c}^-$.
For $\beta\in (1,2)$, the order of the transition increases when $\beta$ tends to $1$. 
More precisely, the transition is of $n$-th order for 
$\beta \in [1+1/n,1 + 1/(n-1))$. The transition disappears for $\beta=1$.

\section{Other  ensembles}

One can consider a system with a variable number of particles.
In this case the partition function is
\begin{equation}
    Z_{\alpha,N} = \sum_{S=N}^\infty e^{-\alpha S} Z_{S,N} .  
\end{equation}
It is easy to see that the partition functions factorises 
\begin{equation}
    Z_{\alpha,N} = e^{N K(\alpha)} 
\end{equation}
so it describes $N$ independent boxes, each contributing an energy $-K(\alpha)$
to the total energy of the system. The contributions from different boxes are entirely independent. The average particle density is 
$\rho(\alpha) = -K'(\alpha)$. It becomes infinite when $\alpha$ is equal to or greater than $\alpha_{\rm c}$.
This means that each box absorbs infinitely many
...
\left\{ 
\begin{array}{lll} 
c_1 S + o(S) & \mbox{for} & \beta \in (2,\infty) \\
c_2 S^{\beta-1} +o(S^{\beta-1})& \mbox{for} & \beta \in (1,2) \\
c_3 + o(1)  & \mbox{for} & \beta \in (-\infty,1) 
\end{array}
\right.
\end{equation}
for $S\rightarrow \infty$, with some positive coefficients $c_1$, $c_2$ and $c_3$
depending on $\beta$. The detailed results, also for the borderline cases of
...
the thermodynamic limit (\ref{dlimit}) in the canonical ensemble.
...
The R\'enyi entropy of zeta-urns.
