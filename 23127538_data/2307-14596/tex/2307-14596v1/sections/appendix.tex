...
The batch size is set to 64~(multi-card).
...
% As discussed in Section \ref{sec:exp}, we adopt Mean Absolute Error~(MAE) and Mean Absolute Percentage Error (MAPE) as our evaluation metrics.
...
% \end{aligned}
% \end{equation}
% where $x_i$ denotes the $i$-th ground truth, $\hat{x}_i$ represents the $i$-th predicted values, and $\Omega$ is the indices of observed samples.
% It is obvious that the MAE metric reflects the absolute prediction accuracy but is affected by the data unit, while MAPE can eliminate the influence of data units to reflect the relative prediction accuracy.
