...
A sustained load --and a consequently sustained power consumption-- is needed for simplifying the modeling of the problem as described in \ref{modelling}. To this end,  a containerized version of the \emph{stress-ng} tool was used and controlled from the OpenAI's Gym scenario setup.
...
While interacting with the environment, the agent will explore sub-optimal policies that, in certain situations, may allow the temperature to scale to values that are not suitable for some components. That is why rigid thermal control was imposed that powers-off the processing node (SoC) when its temperature is out of range to keep it into operational temperature range.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Deep Reinforcement Learning}
\subsubsection{Soft Actor-Critic}
Reinforcement learning has been successfully applied to multiple problems in the digital world. However, it is still difficult to use these types of algorithms in real life problems due to the low efficiency in the use of experience. A large number of interactions with the environment are necessary for the algorithms to learn to behave correctly.
Contrary to on-policy algorithms in which the collected samples are discarded after a policy update, off-policy algorithms collect samples in a replay buffer that is used on every policy update making them ideal in cases where gaining experience has a high cost because they can efficiently learn from past events and decisions.
...
In this study, Stable Baselines3 (SB3) \cite{stable-baselines3} implementation of SAC is used. SB3 is a set of implementations of reinforcement learning algorithms in Pytorch that uses the OpenAI's Gym library as an abstraction for the environments.
...
The board used for the experimentation has 9 temperature sensors available to monitor its temperature. Seven of them are included inside the SoC, distributed on different measurement sites, and the other two sensors are located on the top and bottom sides of the board.
...
As in previous equations, \emph{n} represents the number of available CPUs (16 in the SoC used). $C_{high}$, $C_{low}$ and $C_{off}$ are constants used to weigh independently each term and to express how each state (power status and clock speed of the CPUs) conditions the reward.
...
In addition, a modeling of the scenario allowed us to explore the performance of different DRL algorithms on the problem at hand and to tune the hyperparameters of the chosen algorithm.

A stable load produces a stable power consumption which in turn causes a temperature rise that follows a saturation function in time approaching a horizontal asymptote. However, the actions change the consumption profile and with it the temperature function. 

To model this behavior, a family of temperature functions over time conditioned not only by time but also by a variable determined by the state of the CPU was proposed as a naive modeling of the temperature curve followed by the system.
...
A representation of this equation family is shown in Fig.\ref{fig:M1} for different \emph{$P$} parameters and fixed initial and limit temperatures of a scenario.
