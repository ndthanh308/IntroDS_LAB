...
\newcommand{\aor}{\begin{tikzpicture}
\draw[->] (0,0) arc (190:190+340:0.15cm);
\end{tikzpicture}}
\newcommand{\aol}{\begin{tikzpicture}
\draw[->] (0,0) arc (-10:-360:0.15cm);
\end{tikzpicture}}
\newcommand{\cutaol}{\begin{tikzpicture}[scale=0.7]
...
The \GH framework and library, presented in this paper, is an attempt to significantly simplify numerical experiments with graph complexes.
Concretely, \GH contains classes and routines for handling the generation of bases of graph complexes, implementing linear operators on such complexes, managing data files produced, and visualising results.
The \GH library furthermore is an attempt to unify numerical experiments on graph complexes, which have appeared scattered throughout the literature \cite{BarNatanMcKay, Murri2012,BuringKiselevRutten2017}. 

We use \GH to compute the dimensions of the graph homology in low degrees for several common graph complexes. We furthermore test and verify or support numerically some existing results and conjectures on the graph homology, at least in low loop orders.
...
1-vertex irreducible simple graph all of whose vertices have valency $\geq 3$.
...
\end{tikzpicture}
 ...
 For $n$ even, an orientation contains an ordering of the set of vertices and half-edges, up to even permutations.
...
\item In addition, we there is a distinguished forest, that is, a subset $T\subset E\gamma$ of the set of edges, that does not contain any edge incident to a hair vertex, and that does not contain loops. We call the set of edges in the forest $T$ the marked edges.
\end{itemize}
...
\begin{thm}[after Konant, Kassabov, Hatcher, Vogtmann]
We have that 
\begin{align}
\label{equ:FG0 out}
...
First, \eqref{equ:FG0 out} for $r=0$ can be found in \cite{Kontsevich1993,ConantVogtmann2003}.
The general case (i.e., general $r$) of \eqref{equ:FG0 out} is shown in the first half of the proof of \cite[Theorem 11.1]{ConantKassabovVogtmann2013}.
...
The homology $H_k(\Out(F_g);\Q)$ vanishes if $5k< 4n-5$. 
\end{thm}
...
For a forested graph $\Gamma$ let us define the excess of $\Gamma$ as 
  ...
  \begin{prop}
    ...
    \end{itemize}
  \end{prop}
  Note that in the bridgeless version the restriction to loop order $\geq 1$ is necessary, see the discussion of the 0-loop homology below.
  We shall give a proof of this result in Appendix \ref{app:FGC dc bottom proof} below. Also note that ``top degree'' refers to the top degree in the subspace with a fixed number of non-marked edge and fixed genus, not the top degree in the full complex.
...
\item Entries "-" mean that the respective graded piece of the graph complex is trivial, i.e., it contains no graphs and hence the associated homology is trivial. Entries "0" mean that the graph complex is nontrivial, but the homology turned out zero nevertheless. The reader my treat "$0$" and "-" the same, but the distinction indicates where the graph complex is an efficient or not so efficient representation of the underlying homological problem.
\item Where applicable, we have indicated known theoretial vanishing results by shading cells that are known to contain no homology.
\item Some tables also contain reference Euler characteristic data from the literatue. This is included both as confirmation of our results, and an indicator of the expected amount of homology in the respective row of the table.
...
Furthermore, the rank algorithm used by LinBox computes, mathematically speaking, only a (very likely exact) lower bound for the ranks.
...
\begin{align*}
  ...
  \node[int] (v4) at (240:1) {};
  \node[int] (v5) at (300:1) {};
  \draw (v1) edge[bend left] (v2)  edge[line width=1mm, bend right] (v2)
  (v2) edge (v3)
  (v3) edge[bend left] (v4)  edge[line width=1mm, bend right] (v4)
  (v4) edge (v5)
  (v5) edge[bend left] (v0)  edge[line width=1mm, bend right] (v0)
  (v0) edge (v1);
  \end{tikzpicture}
  \in \FGC_1^{4\lp}(0)
  &
  ...
  (v3) edge[bend left] (v4)  edge[line width=1mm, bend right] (v4)
  (v4) edge (v5)
  (v5) edge[bend left] (v6)  edge[line width=1mm, bend right] (v6)
  (v6) edge (v7)
  (v5) edge[bend left] (v6)  edge[line width=1mm, bend right] (v6)
  (v6) edge (v7)
  (v7) edge[bend left] (v8)  edge[line width=1mm, bend right] (v8)
  (v8) edge (v9)
  (v9) edge[bend left] (v0)  edge[line width=1mm, bend right] (v0)
  (v0) edge (v1)
  ;
  \end{tikzpicture}
  \in \FGC_1^{6\lp}(0).
...
Then we have:
\begin{lemma}
The elements $W_{2k}\in \FGC_1^{2k\lp}$ are non-zero and satisfy
\[
  d_c W_{2k} = d_u W_{2k} = 0. 
\]
\end{lemma}
\begin{proof}
The symmetry group of $W_{2k}$ is the dihedral group $D_{2k-1}$, and one readily verifies that the rotation and reflection generator of $D_{2k-1}$ act trivially on a and hence any given orientation of the graph.
Hence $W_{2k}\neq 0$ in $\FGC_1^{2k\lp}$.
...
For each $k=2,3,4,\dots$ the elements $W_{2k}\in \FGC_1^{2k\lp}$ represent non-trivial homology classes.
\end{conj}
...
\begin{prop}\label{prop:FGC dc bottom}
...
Fix integers $r\geq 3$ and $N$ with $r\geq N+1$ and a partition $\underline p=(p_1,\dots,p_N)$ of $r$ with $p_j\geq 1$. 
Then $H^\bullet(V_{r, \underline p})$ is concentrated in the bottom degree $\bullet = 2-r$, corresponding to trivalent trees.
\end{lemma}
...
We claim that the $E^2$ page of the spectral sequence associated to our filtration is concentrated in degree $2-r$, thus showing the Lemma.
...
The differential $d_0$ leaves the rips the same, and just contracts backbone edges. The resulting complex is isomorphic to a direct summand of the (associative) bar complex of a free commutative algebra, with generators corresponding to the rips.
...
\begin{proof}[Proof of Proposition \ref{prop:FGC dc bottom}]
...
The shape of the meta-graph is unaltered by the differential $d_c$ acting only on the decorations as the meta-vertices.
Hence the forested graph complex with bridges decomposes into direct summands of complexes of the form 
\[
\otimes_G^\Lambda \bCom  \cong \left(\Lambda\otimes \otimes_{v\in V\Gamma} (\bCom)(r_v) \right)_{Aut(G)} 
\]
for meta-graphs $G$.
...
\[
\left(\Lambda\otimes \otimes_{v\in V\Gamma} V_{r_v,\underline p_v} \right)_{Aut(G)} ,
\]
