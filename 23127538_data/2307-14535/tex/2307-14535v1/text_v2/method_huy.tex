...
\begin{itemize}[leftmargin=3mm]
      \item We recognize the necessity of task-directed exploration from reinforcement learning, especially where formal models for planning is challenging to specify, but aim to not be constrained by its inefficiency in long-horizon, sparse reward tasks, typical of robotic tasks.
      ...
      \item We are inspired by the effectiveness of robotic planning methods, e.g. TAMP,
            ...
            and non-reliant on ground truth state or state-estimation %
            during policy inference.
      \item We aim to achieve the simplicity and effectiveness of behavior cloning in distilling collected robot experience into a policy for real-world deployment, while side-stepping the requirement for costly human expert demonstrations or play data collection.
            \vspace{-2mm}
\end{itemize}
...
Our framework takes as input a simulation environment, task descriptions, and outputs a multi-task language-conditioned visuo-motor policy, which can be used for real-world deployment.
It does this in two phases.
In the data generation phase (\S\ref{sec:method:datagen}), we use an LLM to recursively decompose (\S\ref{sec:method:datagen:planning}) tasks into a hierachical plan (\ie task tree) for exploration and ground the plan into sampling-based robot utilities and motion primitives (\S\ref{sec:method:datagen:grounding}).
...
% Figure environment removed
\vspace{-2mm}
\subsection{Language-Guided Robot Data Generation}

\label{sec:method:datagen}

\subsubsection{Simplify: Task Planning and Decomposition}\vspace{-2mm}
...
Concretely,  our recursive LLM planner takes as input the task description, the simulation state, and outputs a plan in the form of a task tree (Fig.~\ref{fig:method:datagen}a)\footnote{We provide all prompts in the supplementary material}.
...
\subsubsection{Ground: Compiling a Plan into Robot Utilities}
...
\begin{itemize}[leftmargin=3mm]
      \vspace{-2mm}
      \item \textbf{Flexibility.} It should not be limited to 2D planar actions~\cite{codeaspolicies2022,shridhar2021cliport}.
            For instance, top-down pick-and-places aren't flexible enough to manipulate prismatic (\eg drawers) and revolute (\eg lids) joints.
      ...
      \item \textbf{Language-friendly.} While joint value sequences are flexible enough to encode any action, it is not a suitable interface for a language model.
            \vspace{-2mm}
\end{itemize}


To encompass these properties, we propose to ground the LLM's plan with API calls into a set of robot utility functions, which include a sampling-based motion planner, a geometry-based grasp and placement sampler, and motion primitives for articulated manipulation.
...
The full prompt and implementation details can be found in the appendix.
...
\subsubsection{Verify \& Retry: Robustifying the Data Collection Policy}
...
To address this, we propose a verify \& retry (Fig.~\ref{fig:method:datagen}c) scheme, which uses the LLM to detect failed execution.
...
Our data generation is capable of generating a large and diverse set of robot data, mixed with both successful and failed trajectories.
This data was generated from sampling-based planners, which are highly \textbf{multi-modal} and is a challenge to fit with a vanilla policy formulation.
...
\begin{wrapfigure}{r}{0.5\textwidth}
      ...
      \vspace{-4mm}
\end{wrapfigure}
We chose to extend diffusion policy~\cite{chi2023diffusionpolicy}, the state-of-the-art approach for single-task behavior cloning, to the multi-task domain by adding language-conditioning.
This policy takes as input a task description, robot proprioception history, and visual observations, and outputs a sequence of end effector control commands.
The task description feature is extracted using CLIP~\cite{radford2021learning}'s text encoder.
...
Quantitatively, when using a 10 dimensional action space\footnote{3 for position, 6 for rotation using the upper rows of the rotation matrix, and a gripper close command}, our policy can be run at $\approx 35Hz$ on an NVIDIA A6000.