\section{Inapproximability Proof}
\label{apx: inapprox}
...
\restateinapprox
\begin{proof} 
Using a reduction scheme similar to \citep{chekuri_rg} (who consider the submodular s-t-orienteering problem), we establish the hardness of rooted \sop (\cref{lem: rooted_sop}).
We reduce \subrl to rooted \sop, demonstrating the inapproximability of \subrl.
...
For the sake of completeness, we include Theorem 4.1 from \cite{chekuri_rg} and modify it for the rooted \sop. The reduction scheme is the same as \cite{chekuri_rg}. 
