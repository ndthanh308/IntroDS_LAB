...
\begin{proof}
  ...
  $\sum\limits_{x\in \Sigma_i}\mu(x)h(x)^2\geq \frac{1}{2k}$. Therefore there must be $x\in\Sigma'$
  ...
  Therefore $\card{g(x_i) - g(x_{i+1})}\geq \frac{1}{m^{3/2}\sqrt{2}}$, an it follows that
  \[
  1-\lambda
  ...
  =1-\inner{g}{\mathrm{T}g}
  = \frac{1}{2}\Expect{\substack{z\sim \mu\\ z'\sim \mathrm{T}z}}{\card{g(z)-g(z')}^2}
  \leq \frac{1}{2}\Prob{z\sim\mu\\ z'\sim\mathrm{T}z}{z\neq z'} 4\norm{g}_{\infty}^2,
  \]
  which is at most $\xi \frac{2}{\alpha}\norm{g}_2^2 = \frac{4}{\alpha} \xi$,
...
\begin{lemma}\label{lem:bound_k_in_embed}
  ...
  then $p\leq p_0$ and there is $k'\leq k$ and an Abelian embedding $(\sigma',\gamma',\phi')$ into $(\mathbb{Z}_{p^{k'}},+)$,
...
\begin{proof}
  ...
  Consider an equation $c_i V_i = 0 \pmod{p^k}$ therein, and assume that $k>k'$. Then $p^{k}$ cannot divide $c_i$, so we may write $c_i = p^{a_i} c_i'$
...
\begin{proof}
  ...
  To show that $((z_1,\ldots,z_1),(z_2,\ldots,z_2),(z_3,\ldots,z_3))$ is in the support of $\mu'''$ on $\Phi'''$, take any $y\in \Gamma$ so that
  since the support of $\mu''$ on the set $\Gamma_{{\sf original}}\times \Phi_{{\sf original}}$ is full,
...
\begin{proof}
  ...
  We now argue that $H_1 = H_2 = H_3$. Letting the alphabets of $\mu'$ be $\Sigma'$, $\Gamma'$ and $\Phi'$,
  note that by Lemma~\ref{lem:path_keeps_connectedness} we may apply the path trick on $\mu'$ to get $\mu''$
...
Non-embedding degrees are defined analogously for functions over $y$ and $z$.
\begin{definition}
...
\begin{proof}
  ...
  \end{align*}
  For $\chi_j\in B_{{\sf embed}}$, $\chi_{j}(b) - \chi_{j}(a)=0$ whenever $\sigma(a) = \sigma(b)$, hence such terms give no contribution to the non-embedding
...
\begin{lemma}\label{lem:demonstrate_merge}
  Suppose that $\mu$ is a distribution as in Theorem~\ref{thm:nonembed_deg_must_be_small}, and further suppose that $\card{\Sigma} = \card{\Phi} = H$ and
  that the distribution of $(\sigma(x),\gamma(y),\phi(z))$ is uniform over $\sett{(a,b,c)\in H^3}{a+b+c = 0}$, where $(x,y,z)\sim \mu$. Then
...
\begin{lemma}\label{lem:demonstrate_maximality}
  ...
  and $\card{\Sigma} >  \card{H}$. Then for all $\xi>0$ there is $\eta = \eta(m,\alpha,\xi)>0$ such that
  \[
...
\begin{lemma}\label{lem:pushing_counter_to_extreme}
  ...
  \begin{enumerate}
    ...
    \item The conclusion of Theorem~\ref{thm:nonembed_deg_must_be_small} fails for $\mu'$.
  \end{enumerate}
\end{lemma}
\begin{proof}
...
of the master embedding of $\nu$, hence the operator of $\nu$ does less averaging. We will use this fact in the future (formalized appropriately)
...
that the distribution of $(\sigma_{{\sf master}}(x), \gamma_{{\sf master}}(y), \phi_{{\sf master}}(z))$ where $(x,y,z)\sim \mu$ is uniform over
$\{(a,b,c)\in H^3~|~a+b+c = 0\}$, and then show a direct argument for that case.
...
$(a,b,c)$ uniformly from $\{(a,b,c)\in H^3~|~a+b+c = 0\}$ and then sampling $(x,y,z)\sim \mu$ conditioned on
$\sigma_{{\sf master}}(x) = a$, $\gamma_{{\sf master}}(y) = b$,  $\phi_{{\sf master}}(z) = c$, and note that we may write
...
Let $c' = \alpha c/2$ and let $E$ be the event that ${\sf NEStab}_{1-c'^{-1}\delta}(g_{\overline{I}\rightarrow y''}; \nu_y)\geq \sqrt{\delta}$.
...
Moreover, in $\nu$ we now have the additional property that the master embeddings are uniform, hence the last expectation is at most
...
We now assume that the distribution of $(\sigma_{{\sf master}}(x), \gamma_{{\sf master}}(y), \phi_{{\sf master}}(z))$ where $(x,y,z)\sim \mu$ is uniform over
$\{(a,b,c)\in H^3~|~a+b+c = 0\}$. As $\card{\Sigma} = \card{\Phi} = \card{H}$, we may re-label the symbols in $\Sigma$ to be group elements (and similarly $\Phi$), so that the master
embeddings of $x$ (and similarly $z$) will be the identity. Define $\tilde{g}(y) = \cExpect{(x',y',z')\sim \mu}{y'=y}{f(x')h(z')}$ and note that $\tilde{g}$
...
it suffices to prove Theorem~\ref{thm:nonembed_deg_must_be_small} in the case that non of these results apply,
...
By Lemma~\ref{lem:pushing_counter_to_extreme}, Theorem~\ref{thm:nonembed_deg_must_be_small} is equivalent to the same statement
under the additional assumptions of maximality and that the distribution is fully merged. Letting $\mu$ be a distribution over $\Sigma\times\Gamma\times \Phi$
satisfying these properties, if $\card{\Sigma} = \card{\Phi} = \card{H}$ then the validity of the statement follows from Lemma~\ref{lem:very_base_case}.
...
we know that it satisfies the conclusion of Theorem~\ref{thm:nonembed_deg_must_be_small}. Consider the distribution
...
$\Phi'_{{\sf pre}} = \sett{(z,\ldots,z)}{z\in \Gamma}$ as well as $\Sigma'_{{\sf pre}} = \sett{(x,\ldots,x)}{x\in \Sigma}$.
...
The following lemma applies Theorem~\ref{thm:nonembed_deg_must_be_small} along with a standard replacement argument to
...
\begin{proof}
  ...
  Applying Theorem~\ref{thm:nonembed_deg_must_be_small} we get that
  \[
...
$g^{\sharp}$ is the part of $g''$ of non-embedding degree $0$ and $g^{\sharp}$ is the part of $g''$ of non-embedding degree $0$
...
\]
where we used the fact that $\mathrm{T}_{\text{non-embed}, 0}$ is Hermitian and that
$L\cdot \chi\circ\sigma$ is an embedding function and so it is an eigenfunction of $\mathrm{T}_{\text{non-embed}, 0}$
of eigenvalue $1$ (see Fact~\ref{fact:soft_nonbembed_op}). Thus, we conclude from Claim~\ref{claim:reverse_F_1}
...
Thus, by Cauchy-Schwarz $W_{\leq d}[f'1'']\geq \card{\inner{f'''}{L}}^2\geq \eps'^2$, and we now
...
$\phi_{\mu'}(\overline{f''},\tilde{g},\tilde{h})\leq M\delta^{\eta}$, hence
$\card{\phi_{\mu'}(\tilde{f}_0,\tilde{g},\tilde{h})}^2\leq 2M\delta^{9\eta/10}$
and so $\card{(\rom{6})}\leq \sqrt{2M}\delta^{9\eta/20}$.
...
We will prove the statement for $\eta' = \eta/20$ and $M' = M+3$.
and
define the events:
\begin{enumerate}
...
\leq \rho^{\xi/8},
\]
hence we get that we may apply Theorem~\ref{thm:nonembed_deg_must_be_small} and get that $\card{(\rom{3})}\leq M\rho^{\eta\xi/8}$ for some $M$ and $\eta>0$
