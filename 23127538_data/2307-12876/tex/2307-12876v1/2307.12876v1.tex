...
\newcommand{\version}{v1} %%true version v0.992
%%\newcommand{\version}{v\,;\,\today} %%true version v
...
\begin{widetext}
%\noindent Phys. Rev. D??????  \hfill  arXiv:2301.00724\;(\version)
%\noindent Phys. Rev. D {\bf 101????}, 064061 (2020) \hfill  arXiv:2301.00724
%
%\noindent arXiv:2305.13278 \hfill KA--TP--08--2023\;(\version)
%
...
But recently an alternative has been proposed~\cite{Klinkhamer2023-APPB},
with a follow-up paper in Ref.~\cite{Klinkhamer2023-mirror}
...
%%{Ellis1973,Bronnikov1973,MorrisThorne1988,Visser1996}
%%{Klinkhamer2023-APPB}{Klinkhamer2023-mirror}{Klinkhamer2023-review}
%%{Wang2023}{Ahmed2023}
...
This 4D Schwarzschild-defect-wormhole metric solves the vacuum Einstein equation.
...
place for a point mass (the original Schwarzschild 
...
Ricci flat and therefore, a solution of the vacuum Einstein equation;
\item
add a fifth dimension with radial coordinate $\chi \in (0,\,\infty)$;
\item
...
&&
\end{eqnarray}
with $b>0$, $M < b/2$, and either $(M >0,\, \delta M \geq 0)$
or $(M =0,\, \delta M =0^{+})$, so that the ratio
$M/(M+\delta M)$ takes values $1$ or $0$.
The \textit{raison d'$\,\hat{\text{e}}$tre} of metric %\emph{Ansatz} 
...
For fixed $\chi=b$, $M =0$, and $\delta M =0^{+}$, we recover
from \eqref{eq:5D-metric} 
the 4D vacuum-defect-wormhole metric~\cite{Klinkhamer2023-APPB}:
\begin{eqnarray}
\label{eq:4D-metric-vacuum-defect-wormhole}
&&ds^{2}\,\Big|^{\chi=b}_{M =0,\,\delta M =0^{+}}
=
- dt^{2} +
%\nonumber\\[1mm]
%&&
...
%&&
\end{eqnarray}
The full 5D metric for these mass parameters
...
For fixed $\chi=b$, $M >0$,  and $\delta M = 0$ exactly,
we recover  the 4D Schwarzschild-defect-wormhole metric
...
\end{eqnarray}
The full 5D metric for these mass parameters
is now $\chi$ dependent, so that the
Ricci curvature scalar $R$
is nonvanishing over the whole 5D manifold, also on the
$\chi=b$ slice.

%For fixed $\chi=b$, $M >0$, and $\delta M =0^{+}$,
%we have 4D deformed-defect-wormhole metric:
%\begin{eqnarray}
%\label{eq:4D-metric-deformed-defect-wormhole}
%&&ds^{2}\,\Big|^{\chi=b}_{M >0,\,\delta M =0^{+}}
%=
%- \left(1-\frac{2M}{\sqrt{b^{2}+\xi^{2}}}\,dt\right)\;dt^{2}
%\nonumber\\[1mm]&&
%+
%\left(1-\frac{2M}{\sqrt{b^{2}+\xi^{2}}}\,dt\right)^{-1}\;
%\frac{\xi^{2}}{\xi^{2}+b^{2}}\;d\xi^{2}
%+ \left( \chi^2 + \xi^{2}\right)\,
%  \Big[ d\theta^{2} + \sin^{2}\theta\, d\phi^{2} \Big]\,,
%\end{eqnarray}
%with nonzero Ricci tensor. Here, give two curvature scalars XX

For fixed $\xi=0$, $M >0$, and $\delta M =0^{+}$,
...
from the 5D metric \eqref{eq:5D-metric}
for $b=1$, $M=1/4$, and $\delta M =0^{+}$
can be calculated analytically and are given by
the following expressions:
\bsubeqs\label{eq:R-and-K-b1-Mquarter}
\beqa  %%FRK: later check again latex vs. math
\label{eq:R-b1-Mquarter}
R\,\Big|^{b=1}_{M=1/4,\,\delta M =0^{+}}
&=&
\frac{1}{32\,{\chi}^3\,
...
\\[2mm]  %%FRK: later check again latex vs. math
\label{eq:K-b1-Mquarter}
K\,\Big|^{b=1}_{M=1/4,\,\delta M =0^{+}}
&=&
\frac{1}
...
in App.~\ref{app:Curvature-scalars-for-general-pos-M}).
%%For completeness, 
For the record, we remark that the 5D metric \eqref{eq:5D-metric}
for $M>0$ and $\delta M =0^{+}$ has a nonvanishing
Ricci tensor, so that the vacuum Einstein equation is not solved and
an appropriate matter content is needed.
...
4D Schwarzschild-defect-wormhole metric is.
...
%%\newpage%%tmp
\begin{appendix}
\section{Curvature scalars for general positive $M$}
\label{app:Curvature-scalars-for-general-pos-M}

The curvature scalars
$R\equiv g^{\mu\nu}\,R_{\mu\nu}$
and $K\equiv R^{\mu\nu\rho\sigma}\,R_{\mu\nu\rho\sigma}$
from the 5D metric \eqref{eq:5D-metric}
for $b=1$, $M>0$, and $\delta M =0^{+}$ are given by
\bsubeqs\label{eq:R-and-K-b1-Mpos}
\beqa
&&    %%FRK: later check again latex vs. math
R\,\Big|^{b=1}_{M>0,\,\delta M =0^{+}}
=
...
Download: [[http://arxiv.org/abs/arXiv:2307.04678|arXiv:2307.04678]]
