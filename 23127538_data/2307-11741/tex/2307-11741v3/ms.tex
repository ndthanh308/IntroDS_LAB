...
\label{gauge_transfo}
\end{equation}
where the exponential function is defined via its power series built with the (local) pointwise product. One defines 2 fuzzy plaquettes, their associated star-products and entire functions: $\eta_0 = 1/\Lambda_0^2$, the symbol $\star_0$ and $\vartheta_0(z)$ for the neutral fields; $\eta_e = 1/\Lambda_e^2$, the symbol $\star_e$ and $\vartheta_e(z)$ for the fields creating/annihilating particles of charge $-1$ and antiparticles of charge $+1$. The WNL Lagrangian is
\begin{equation}
\mathcal{L}_\star = - \dfrac{1}{4} \, F_{\mu\nu} \star_0 F^{\mu\nu} + \left( D_\mu \phi \right)^* \star_e \left( D^\mu \phi \right) - V_\star(\phi) \, ,
\end{equation}
...
\left( D_\mu \phi \right)^* \star_e \left( D^\mu \phi \right) &= \left( D_\mu \phi \right)^* \, e^{\vartheta_e \left( \overleftarrow{D_\mu}^* \, \eta^{\mu \nu}_e \,\overrightarrow{D_\nu} \right)} \left( D^\mu \phi \right) \, , \\
\phi^* \star_e \phi &= \phi^* \, e^{\vartheta_e \left( \overleftarrow{D_\mu}^* \, \eta^{\mu \nu}_e \,\overrightarrow{D_\nu} \right)} \phi \, , \\
...
Then, it is important to notice that, in the broken vacuum, the mass terms for the photon and meson fields receive contributions from the interaction terms in the Lagrangian \eqref{U_Lag}. Therefore, the ghost-free condition is realized if one imposes $\eta \equiv \eta_0 = \eta_e$ and $\vartheta(z) \equiv \vartheta_0(z) = \vartheta_e(z)$, thus one can also drop the indices attached to the symbol $\ast$. At the end, the Lagrangian $\mathcal{L}_\star^U$ can be written in terms of a meson and Proca fields as
\begin{align}
