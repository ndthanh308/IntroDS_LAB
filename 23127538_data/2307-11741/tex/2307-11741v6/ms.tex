...
\affiliation[b]{Univ Lyon, Univ Claude Bernard Lyon 1, CNRS/IN2P3, IP2I Lyon, UMR 5822,\\ F-69622, Villeurbanne, France}
...
\end{equation}
One can read the mass of the unique meson mode from the $\sigma^2$-term:
\begin{equation}
...
\label{gauge_transfo}
\end{equation}
where the exponential function is defined via its power series built with the (local) pointwise product. In order to avoid any confusion, one defines 2 different symbols for the star-product between fields with different quantum numbers:
\begin{itemize}[label=$\spadesuit$]
\item For neutral fields, one introduces the fuzzy plaquette $\eta_0 = 1/\Lambda_0^2$; the symbol $\star_0$ defined as (no summation on $i$ and $j$)
\begin{align}
...
\newpage
\item For fields creating/annihilating particles of charge $-1$ and antiparticles of charge $+1$, one introduces the fuzzy plaquette $\eta_e = 1/\Lambda_e^2$; the symbol $\star_e$ defined as (no summation on $i$ and $j$)
\begin{align}
\phi^*(x) \star_e \phi(x)
...
The WNL Lagrangian for this Abelian Higgs model is
\begin{equation}
...
\end{equation}
with the real scalar fields $\sigma(x)$ and $\pi(x)$ for the charged meson and pion, respectively. From the discussions in Sections~\ref{star_product_section} and \ref{star_fuzzy_tachyon_scalar}, the most natural attempt for $V_\star(\phi)$ is to replace all pointwise products in the usual local quartic potential of the Higgs field by star-products $(\mu^2 > 0, \ \lambda > 0)$:
\begin{equation}
...
\end{equation}
One works in unitary gauge, i.e. use the freedom given by the gauge transformations \eqref{gauge_transfo} to remove the explicit dependence on the pion field $\pi(x)$, which becomes the longitudinal polarization of the massive photon field $A_\mu(x)$, i.e.
\begin{equation}
...
Thanks to Raffaele T. D'Agnolo, Hermès Bélusca-Maïto, Pratik Chattopadhyay, Aldo Deandrea, Anish Ghoshal, Ratul Mahanta, Anupam Mazumdar, Leonardo Modesto and Jean-Christophe Wallet for informal and useful discussions. Fig.~\ref{roots} was made with \texttt{Wolfram Mathematica}.
...
% 20 pages (+ references), 2 figures. v4: APPB postprint. v6: include erratum + addendum (Higgs potential with star-products modified + discussion about solution with counter-term)