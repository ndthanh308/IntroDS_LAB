...
\begin{abstract}
  ...
  such that $3$-fold correlations decay exponentially for H\"older
  continuous observables. Given a sequence $(M_k)$ that converges to
  $0$ slowly enough we obtain a strong dynamical Borel--Cantelli result
  for recurrence, i.e., for $\mu$-a.e.\ $x\in X$
  \[
  ...
  holds for Axiom A diffeomorphisms and certain equilibrium states.
\end{abstract}
...
systems. It states that if $(X, \mu, T)$ is a measure-preserving
dynamical system with a Borel probability measure $\mu$, then
$\mu$-a.e.\ point $x \in X$ is recurrent, i.e., there exists an
increasing sequence $n_i$ such that $T^{n_i}x \to x$. Equipping the
system with a metric $d$ such that $(X,d)$ is separable and $d$-open
sets are measurable, we may restate the result as
...
$\mu$--almost all $x$ there exists a constant $c > 0$ such that
\[
...
Hussain, Simmons and Wang~\cite{hussain2022Dynamical},
...
Equip a smooth manifold with the induced metric and let $B(x,r)$ denote
the open ball with centre $x$ and radius $r$.

\begin{theorem}\label{thm:main} 
  Consider a compact smooth $N$-dimensional manifold $M$. Let
  $f \colon M \to M$ be an Axiom A diffeomorphism that preserves a
  mixing equilibrium state $\mu$ given by some H\"older continuous
  potential. Suppose that there exist $c_1>0$ and $s > N - 1$
  satisfying
  \[
    \mu(B(x,r)) \leq c_1r^{s}
  \]
...
Furthermore, if the sets $B_n(x)$ were to not depend on $x$, then,
since $T$ preserves the measure $\mu$, one would obtain that
$\mu(E_n) = M_n$. However, we have to instead settle for estimates on
the measure and correlations of the sets $E_n$. We do so in
Proposition~\ref{prop:main3} using decay of correlations as our main
tool.

\begin{definition}[$r$-fold Decay of Correlations]
  For an m.m.p.s\ $(X,\mu,T,d)$ and $r \in \naturals$ we say that
  $r$-fold correlations decay exponentially for H\"older continuous
  observables if there exists constants $c_1 >0$ and $\tau \in (0,1)$
  such that for all H\"older continuous functions
  $\varphi_k : X \to \reals$, $k=0,\dotsc,r-1$, and integers
  $0 = n_0 < n_1 < \dotsc < n_{r-1}$
  \[
    \Bigl| 
    \int \prod_{k=0}^{r-1} \varphi_k \circ T^{n_k} \diff\mu -
    \prod_{k=0}^{r-1} \int \varphi_k \diff\mu
    \Bigr|
    \leq c_1 e^{-\tau n} \prod_{k=1}^{r} \norm{\varphi_k}_{\theta},
  \]
  where $n = \min \{k_{i+1} - k_{i}\}$, and
  \[
    \norm{\varphi}_{\theta} 
...
It is well known that if $M$ is a compact smooth manifold, $f$ an Axiom
A diffeomorphism and if $\varphi \colon M \to \reals$ is H\"older
continuous, then there exists a unique equilibrium state $\mu_\varphi$
for $\varphi$ such that $2$-fold correlations decay exponentially for
H\"older continuous observables
(see Bowen \cite{bowen2008Equilibrium}).
Kotani and Sunada essentially prove in
\cite[Proposition~3.1]{kotani2001Pressure} that, in fact, $r$-fold
correlations decay exponentially for H\"older continuous observables
for all $r \geq 1$.
...
$(X,\mu,T,d)$.
As it will be of use later, we state the main theorem of
\cite{persson2023Strong}, rewritten to make the assumptions clearer.

\begin{theorem}[Persson \cite{persson2023Strong}]\label{thm:persson}
  Let $(M_n)$ be a sequence in $[0,1]$ satisfying
  Assumption~\ref{assumption:seq} and suppose that $\mu$ satisfies
  Assumption~\ref{assumption:ball}. Suppose further that there exists
  $C,\eta > 0$ such that for all $n,m \in \naturals$
  \[
...
\begin{theorem}\label{thm:general}
  ...
  decay exponentially for H\"older continuous observables and such that
  Assumptions~\ref{assumption:ball}--\ref{assumption:packing} hold.
  Suppose further that $(M_n)$ is a sequence in $[0,1]$ converging to
  $0$ and satisfying Assumption~\ref{assumption:seq}.
  Then
  \[
    \lim_{n \to \infty}\frac{\sum^{n}_{k=1} \charfun_{B_k(x)}(T^{k}x)}
    {\sum^{n}_{k=1} \mu(B_k(x))} = 1
...
remains to show that these systems satisfy $3$-fold decay of
correlations.
...
correlation for a smaller class of observables, i.e., for H\"older
...
H\"older continuous functions in order to apply decay of correlations,
...
$\{h_k \colon X \to [0,1]\}_{k=1}^{n_{\rho}}$ by
\begin{equation}\label{eq:h:definition}
  h_k(x) = \min \{1, \delta^{-1} \dist(x, X \setminus B_{k}(\delta)\}.
\end{equation}  
...
\begin{proposition}\label{prop:main1}
  ...
  H\"older continuous observables. Let $n\in\naturals$ and consider the
  function $F \colon X \times X \to [0,1]$ given by
  \[
    F(x,y) =
    \begin{cases}
...
\begin{proof}
It suffices to prove the proposition for the case when we have decay of
correlations for Lipschitz observables that are bounded by $1$. The
general case is then implied, as for any $\theta$ we can find a
constant $C > 0$ independent of $\varphi$ such that 
$\norm{\varphi}_{\theta} \leq C \lipnorm{\varphi}$.
Finally, we only prove the upper bound part of \eqref{ineq:main1} as
...
where 
$\gamma 
= \frac{1}{2}\bigl(4 + 4K + \frac{4K}{\alpha_0}\bigr)^{-1} \tau$.
...
\begin{proposition}\label{prop:main2}
  ...
  H\"older continuous observables. Let $m,n \in \naturals$ and consider
  the function $F\colon X\times X\times X \to [0,1]$ given by
  \[
    F(x,y,z) =
    \begin{cases}
...
\begin{proof}
Again we only prove the case for when we have decay of correlations for
Lipschitz observables and for $n$ sufficiently large. We also assume
w.l.o.g.\ that $\varepsilon_0,\rho_0 < 1$ and $M_n > 0$.
...
\begin{proposition}\label{prop:main3}
  With the above assumptions on $(X,\mu,T,d)$ and $(M_n)$, suppose
  additionally that $3$-fold correlations decay exponentially for
  H\"older continuous observables. There exists $C,\eta > 0$ such that
  for all $m,n \in \naturals$
  \[
    | \mu(E_n) - M_n | \leq C e^{-\eta n}
  \]
...
\begin{proof}[Proof of Theorem~\ref{thm:main}]
  ...
  $3$-fold decay of correlations against H\"older continuous
  observables. Thus, the theorem follows from Theorem~\ref{thm:general}
  once we verify that Assumptions~\ref{assumption:annulus} and
...
\[
  \liminf_{n \to \infty} (n \log n \log n)^{1/\beta} d(T^{n}x,x) = 0
\]
...
\[
  \mu(A) = \int_{A} h(y) \diff\lambda
  \leq \norm{h}_{L^{p}} \lambda(A)^{\frac{1}{q}},
\]
where $q$ is the harmonic conjugate of $p$.
Since $\lambda(B(x,\rho)) \leq \rho^{s}$ and
$\lambda(B(x,\rho + \varepsilon) \setminus B(x,\rho) )
\leq \varepsilon^{\alpha_0}$ for some $s$ and $\alpha_0$,
...
observables as shown by Young \cite{young1998Statistical}.
...
observables.
For our result to apply to these spaces, one would have to show that
...
and for his guidance in preparing this paper.
...
\bibitem{allen2022Recurrence} D. Allen, S. Baker, B. B\'ar\'any, 
  \emph{Recurrence rates for shifts of finite type},
...
\bibitem{kirsebom2021Shrinking} M. Kirsebom, P. Kunde, T. Persson,
  ...
  \href{https://arxiv.org/abs/2003.01361v3}{arXiv:2003.01361v3}.
