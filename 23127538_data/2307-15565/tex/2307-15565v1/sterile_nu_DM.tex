...
\end{equation}
Note that we take the scalar mass $m_{\phi} \gtrsim 10~ m_s$ throughout. This allows us to neglect the mass of the sterile neutrinos and treat them as relativistic particles during production. In addition, 
...
The evolution of the sterile neutrinos' phase space density follows a Boltzmann equation \cite{Foot:1996qc,DiBari:1999ha,Johns:2019cwc} 
\begin{equation}
...
\begin{equation}
    V_{\text{eff}}= V_a - V_s,
\end{equation}
where $V_{a}$ ($V_s$) is the potential seem by the active (sterile) neutrinos. The active potential reads \cite{Notzold:1987ik}
\begin{equation}
...
expressions for the squared matrix elements and details for the integrals required for the computation of the rate are given in Appx.~\ref{app: Sterile Rates}. 
...
When the mediator is light compare to the temperature 
...
This simplified ansatz for the sterile neutrino production was also put forward in \cite{Bringmann:2022aim}. We come to qualitatively similar conclusions in the regime of parameters where \eqref{eq: Boltzmann equation in low mass limit} is applicable but want the stress that this does not cover the whole range of masses and couplings.
...
\hline 
1 & $15~\text{keV}$ & $150~\text{keV}$ & $2 \times 10^{-4}$ & $1.39 \times 10^{-13}$\\
\hline
2 & $15~\text{keV}$ & $150~\text{keV}$&$7.5 \times 10^{-4}$ & $4.68 \times 10^{-15}$\\
\hline
\end{tabular}
...
Before moving on some further comments on thermalization are in order. Due to the non-linear nature of the problem and the strong dependence of the rates on the temperature, the impact of thermalization can be quite complex. While the previous example featured an increase of the yield, it is also possible that thermalization reduces the production of the steriles. 
The reason is that stronger thermalization is accompanied by lower temperatures. The rates decrease exponentially for large $m_\phi/T_s$ and, therefore, it is possible to end up in a situation where the final production phase does not pick up and the system remains stuck in the state it reached after thermalization. 
Since most of the yield is produced in this production phase, this leads to a marked decrease of the relic. Whether the system remains stuck or picks up again depends sensitively on the couplings and the state of the system before thermalization. 
This is illustrated in the right panel of Fig.~\ref{fig: double solution} where we show the evolution of the yield for three choices of $y$. 
The lowest coupling thermalizes only partially and a second production phase starts immediately afterwards. The intermediate coupling leads to more efficient thermalization and the system remains stuck, markedly decreasing the final yield. 
The highest coupling also thermalizes efficiently, but at an even earlier time. This allows for the sterile self-scattering dominated production phase to pick up thereafter, leading to a higher yield again. The different ingredients scale differently with $y$, and, therefore, the final yield is no longer a monotonous function of the coupling and more than one (or no) solution for the right relic density may be found in this regime, see the left panel of Fig.~\ref{fig: double solution} for illustration.
...
Now we turn to the case $m_\phi \gtrsim 100~\text{MeV}$. Here the production by DW and the one sourced by sterile-sterile interactions will in general happen simultaneously. 
...
From~\eqref{eq: boltzmann equation with resonance} one can see that the rate, which enters the denominator through the quantum damping term, plays an essential role in regulating production here in this case. 
...
\end{equation}
Hereafter, we consider a conservative limit of $m_{WDM} \leq 1.9~\text{keV}$ derived in \cite{Garzilli:2019qki} from Lyman-$\alpha$ observations, which corresponds to $v_{rms} \lesssim 16~\text{m/s}$. For a Maxwell-Boltzmann distribution $v_{rms}$ can be computed easily and we get 
\begin{equation}
    v_{rms}^2 =\left.  \frac{p^2_{rms}}{m_s^2}\right|_{\text{today}}  = \left.
    \frac{1}{3 n_s} \int d^3p \frac{p^2}{m_s^2} f_s(p)~   \right|_{\text{$T_d$}} \times \frac{T_0^2}{T_d^2}
    =  \frac{4}{m^2_s} \left( \frac{T_0}{T_d} T_{s,d}\right)^2,   
\end{equation}
where $T_0$ is the photon temperature today, $T_d$ is the photon temperature at the time where the sterile neutrino production is completed with corresponding temperature $T_{s,d}$. Here we used the fact that the sterile's momentum only redshifts once production is complete, \emph{i.e.} $p \propto a^{-1}$.
...
As is well known, pure DW is excluded, mostly due to $X$-rays. Including interactions in the sterile sector allows to fulfill the relic density requirement for mixing angles well below the DW line and opens up significant new regions of parameter space. However, the unconstrained parameter space (white region in Fig.~\ref{fig: parameter space}) is clearly limited. 
First, constraints from $X$-ray searches bound the parameter space from above and remove the upper right corner in a way that is completely analogous to the Shi-Fuller scenario. Second, on the left, Lyman-$\alpha$ observations set a lower limit on the mass of the sterile neutrinos. These are relatively strong since the bulk of the neutrinos is produced rather late here and, therefore, they inherit the heating of the SM plasma due to the decreasing number of relativistic degrees of freedom. These bounds relax somewhat towards low mixing angles since the associated larger Yukawa couplings allow for partial thermalization. This leads to a decrease of the temperature and, as a consequence, less free-streaming. The onset of partial thermalization can be seen nicely in the dip towards smaller mixing angles of lines with largish $y$, e.g. the line for $y=10^{-3}$ around $m_s=10$ keV. As discussed in Sec.~\ref{sec: light mediator case}, more than one solution may exist if thermalization plays a role. In this case, we plot the solution with the smaller $\sin^2(2 \theta)$ if more than one solution exists. This leads to the jump at $m_s\approx 20$ keV since here the lower solution cannot make up the full relic density anymore. 
If we go towards even smaller $\sin^2(2 \theta)$ the Yukawa couplings increase further. For $y\gtrsim10^{-2}$ the thermalization process is very efficient and we typically end up in the situation where the relic is either too low because the system remains stuck after the thermalization phase, or the relic density is overproduced if the system is able to undergo the second phase of production. Therefore, only highly tuned or no solutions exist in this regime. We highlight this region with lilac shading in the figure.  
...
As briefly discussed in the main text, the interactions of neutrinos with the plasma are responsible for a modification in their dispersion relation through an effective potential, see e.g. \cite{Notzold:1987ik,Quimbay:1995jn,Dasgupta:2013zpn}
\begin{equation}
    E = \abs{\bold{p}} + \frac{m^2}{2\abs{\bold{p}}} + V_{\text{eff}}. 
    \label{eq: modified dispersion relation}
\end{equation}
Indeed, this potential encapsulates thermal corrections to the neutrinos' self-energy. The lowest order of such corrections appear at one loop level in the bubble and tadpole diagrams shown in Fig.~\ref{fig: one-loop diagrams for veff}, where the thick lines represent thermal propagators. One can compute these diagrams in Feynman gauge in the real-time formalism and we proceed in this way in this section, closely following \cite{Quimbay:1995jn}. We also consider ultra-relativistic neutrinos for the calculation. The tadpole diagram gives a contribution that is proportional to the fermion asymmetry and, since we assume this asymmetry to be comparable to the baryon asymmetry, it can be neglected. 
% Figure environment removed
...
\begin{align}
f_f (k \cdot u ) &= \left[e^{\abs{k \cdot u }/T} +1\right]^{-1}  \\
f_b (k \cdot u ) &= \left[e^{\abs{k \cdot u }/T} -1\right]^{-1}, 
\end{align}
...
\end{align}
 ...
 \end{align}
Furthermore, as it is the case with the rate, one can easily compute the heavy and the massless limits for the potential. First, let us consider the high-temperature limit, \emph{i.e.} $\abs{\bold{p}}, T\gg m_{\phi}$. In this case we can drop the sub-leading logarithmic terms and get 
\begin{align}
    V_{\text{eff}}(\abs{\bold{p}},T \gg m_{\phi}) &=   \frac{y^2}{2\abs{\bold{p}}^2} \int_0^{\infty} \frac{\dd{\abs{\bold{k}}}}{8 \pi^2}\left[ 4 \abs{\bold{p}} \abs{\bold{k}} f_f(\abs{\bold{k}}) + \frac{4 \abs{\bold{p}} \abs{\bold{k}}^2}{\epsilon_2}  f_b({\epsilon_2}) \right] \nonumber \\ &= \frac{2 y^2}{\abs{\bold{p}}} \int_0^{\infty} \frac{\dd{\abs{\bold{k}}} \abs{\bold{k}}}{8 \pi^2}\left[  f_f(\abs{\bold{k}}) +   f_b(\abs{\bold{k}}) \right], 
\end{align}
 ...
 \begin{equation}
      V_{\text{eff}}(\abs{\bold{p}},T \gg m_{\phi}) = \frac{y^2}{8 \abs{\bold{p}}} T^2. 
 \end{equation}
...
\begin{align}
    V_{\text{eff}}(\abs{\bold{p}},T \ll m_{\phi}) &= -\frac{y^2}{2 \abs{\bold{p}}^2}
 \int_0^\infty \frac{\dd{\abs{\bold{k}}}}{8 \pi^2} \frac{128}{6} \frac{\abs{\bold{k}}^3 \abs{\bold{p}}^3}{m_{\phi}^4}f_f(\abs{\bold{k}})\nonumber \\ &= -\frac{7 \pi^2 y^2 \abs{\bold{p}} T^4}{45 m_{\phi}^4} + \mathcal{O}(m_{\phi}^{-5}),
 \end{align} 
...
and
\begin{align}
\abs{\mathcal{M}}^2_{\nu_s \nu_s \leftrightarrow \nu_s \nu_s} &= 8 y^4 \left[ \frac{s^2}{(s-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2}  - \frac{ (s^2 +st) \left[(s-m^2_{\phi})(-t-s-m^2_{\phi}) + m_{\phi}^2 \Gamma_{\phi}^2\right] }{\left[(s-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]\left[(s+t+m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]}\right. \nonumber \\ & + \frac{t^2}{(t-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2}- \frac{ (t^2+st) \left[(t-m^2_{\phi})(-t-s-m^2_{\phi}) + m_{\phi}^2 \Gamma_{\phi}^2\right] }{\left[(t-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]\left[(t+s+m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]}   \nonumber\\ &\left.+\frac{(s+t)^2}{(-s-t-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2} + \frac{ st \left[(s-m^2_{\phi})(t-m^2_{\phi}) + m_{\phi}^2 \Gamma_{\phi}^2\right] }{\left[(s-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]\left[(t-m_{\phi}^2 )^2 + m^2_{\phi} \Gamma_{\phi}^2\right]} \right],
\end{align}
...
\begin{align}
    ...
    \sigma_{\nu_s \nu_s \leftrightarrow \phi \phi} =& \frac{y^4}{4 \pi s^2 }\left[\frac{6m_{\phi}^2 - 4m_{\phi}^2 s +s^2}{2m_{\phi}^2 -s} 2 \log\left(\frac{(s(s-4m_{\phi}^2))^{1/2} +s - 2m_{\phi}^2}{(s(s-4m_{\phi}^2))^{1/2} -s + 2m_{\phi}^2}\right) - 6(s(s-4m_{\phi}^2))^{1/2}\right].
\end{align}
...
\Gamma_{\nu_s \nu_s \leftrightarrow \nu_s \nu_s}^{\text{resonant}}(p) = \frac{y^2 T_s m_{\phi}^2}{2 \pi p^2}e^{-\frac{m_{\phi}^2}{4 p T_s}+\frac{\mu}{T_s}}. 
\end{equation}
The $\nu_s \nu_s \leftrightarrow \nu_s \nu_s$ rate is well approximated by a piece-wise function 
\begin{equation}
   \Gamma_{\nu_s \nu_s \leftrightarrow \nu_s \nu_s}(p) =  \begin{cases}
