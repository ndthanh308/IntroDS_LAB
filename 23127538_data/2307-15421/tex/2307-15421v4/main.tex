...
%     xleftmargin=2em,% show line numbers, remove this entire line if you don't want the numbers.
...
\title{MLIC$^{++}$: Linear Complexity Attention-based Multi-Reference Entropy Modeling for Learned Image Compression}

\author{Wei Jiang, Jiayu Yang, Yongqi Zhai, Feng Gao,~\IEEEmembership{Member,~IEEE}, Ronggang Wang,~\IEEEmembership{Member,~IEEE}
        % <-this % stops a space
\thanks{Wei Jiang, Jiayu Yang, Yongqi Zhai, Ronggang Wang are
with Shenzhen Graduate School, Peking University, 518055 Shenzhen, China (email:jiangwei@stu.pku.edu.cn, jiayuyang@pku.edu.cn, zhaiyongqi@stu.pku. edu.cn, rgwang@pkusz.edu.cn).}
\thanks{Yongqi Zhai, Ronggang Wang are with Peng Cheng Laboratory, 518000
Shenzhen, China.}
\thanks{Feng Gao is with School of Arts, Peking University, 100871, Beijing, China (email: gaof@pku.edu.cn).}
% \thanks{This paper was produced by the IEEE Publication Technology Group. They are in Piscataway, NJ.}% <-this % stops a space
\thanks{Ronggang Wang is the corresponding author.}}
...
\begin{abstract}
  Recently, learned image compression has achieved impressive performance.
    ...
    To address these limitations, we propose the Linear Complexity Attention-based Multi-Reference Entropy Model (MEM$^{++}$).
    ...
    Code and pre-trained models are available at \url{https://github.com/JiangWeibeta/MLIC}.
\end{abstract}

\begin{IEEEkeywords}
Entropy model, Learned image compression.
\end{IEEEkeywords}
...
JPEG2000~\cite{maryline1999jpeg2000}, AVC~\cite{schwarz2007overview}, HEVC~\cite{bpg,sullivan2012overview}, and VVC~\cite{bross2021vvc}
...
~\cite{balle2016end,theis2017lossy,balle2018variational,li2018learning,minnen2018joint,hu2020coarse,ma2020iwave}
have emerged, showcasing impressive performance gains.
% Figure environment removed
Notably, certain learned image compression models~\cite{cheng2020learned,minnen2020channel,chen2021nic,guo2021causal,wu2021learned,xie2021enhanced,zou2022the,he2022elic,qian2020learning,gao2021neural,chen2022two,koyuncu2022contextformer,jiang2022mlic,duan2023lossy,jiang2023slic} are already comparable to
...
State-of-the-art learned image compression models~\cite{cheng2020learned,minnen2020channel,xie2021enhanced,zou2022the,zhu2022transformerbased,he2022elic,jiang2022mlic,jiang2023slic}
...
which consumes \textit{huge} GPU memories and have slower encoding and decoding speed,
...
we propose a novel \textit{linear complexity} attention-based multi-reference entropy model.
...
which are extract by a channel-wise context module.
...
further enhance the local context capturing while retaining two-pass decoding.
...
\item We design \textit{linear complexity} attention-based multi-reference entropy model MEM$^{++}$ which
...
while maintaining high-quality compression capabilities.
  ...
  Learned image compression~\cite{balle2016end,theis2017lossy,ma2019image} aims to optimize the trade-off
  ...
  To enhance non-linearity, Generalized Divisive Normalization (GDN)~\cite{balle2015gdn} layers or its variants~\cite{qian2020learning,bao2022nonlinear} are employed.
  Additionally, self-attention~\cite{vas2017attention,liu2021swin,lu2021tic, zou2022the,liu2023learned},
...
\begin{table}[t]
  ...
  \multicolumn{1}{c|}{\multirow{2}{*}{MEM $^{++}$}}                            & \multicolumn{1}{c}{Linear complexity attention-based}   \\
...
motivate us to design a \textit{linear} complexity attention-based multi-reference entropy model.
Our proposed \textit{linear} complexity attention-based multi-reference entropy model effectively
...
The architecture of MLIC$^{++}$, as depicted in Figure 1, incorporates the analysis transform $g_a$, synthesis transform $g_s$,
...
$\hat{\boldsymbol{y}}$.
\subsubsection{MEM$^{++}$}
The proposed linear complexity attention-based multi-reference entropy model effectively captures channel-wise, local spatial, and global spatial correlations with linear complexity.
...
\begin{equation}
  \begin{aligned}
  \begin{split}
  &\mathcal{R}_{\hat{\boldsymbol{y}}^i_{na}} = \\ &\mathbb{E}\left[-\log \int_{-0.5}^{0.5} p\left({\boldsymbol{y}}^i_{na}+\boldsymbol{u}| \Phi_{h}, \Phi_{ch}^i, \Phi_{gc,intra}^i, \Phi_{gc,inter}^i \right) d\boldsymbol{u}\right],
  \end{split}
\end{aligned}
\end{equation}
...
% Figure environment removed
% Figure environment removed
In latent representation $\hat{\boldsymbol{y}} \in \mathbb{R}^{L\times C}$, where $L=H\times W$, $H$ is the height of latent representation,
$W$ is the width of latent representation, $C$ is the channel number,
...
\end{equation}
where $\hat {\boldsymbol{y}}^{i-1}_{na,q}, \hat {\boldsymbol{y}}^{i-1}_{a,k} = \textrm{Embedding}\left(\hat {\boldsymbol{y}}^{i-1}\right)$, $\hat {\boldsymbol{y}}^{i}_{a,v} = \textrm{Embedding}\left(\hat {\boldsymbol{y}}^i_{a}\right)$,
...
\begin{table*}
  ...
  \multicolumn{1}{c|}{Guo'21 (TCSVT'21)~\cite{guo2021causal}}                                        & $-4.45$       & $-45.23$ & $-$       & $-$ & $-$       & $-$       \\\midrule
  ...
  \multicolumn{1}{c|}{Pan'22 (ECCV'22)~\cite{pan2022content}}        & $+7.56$      & $-36.2$ & $+3.97$      & $-$  & $-$      & $-$     \\\midrule
...
\end{table*}
  ...
  Our training datasets contains $10^5$ images\footnote[1]{\url{https://github.com/JiangWeibeta/MLIC/blob/main/train_list.txt}}
...
we conduct performance evaluations on three distinct datasets, including Kodak~\cite{kodak}, Tecnick~\cite{tecnick2014TESTIMAGES},
  ...
  \item CLIC Professional Valid is the validation set of 3rd Challenge on Learned Image Compression
...
% \end{lstlisting}
  ...
  modeling. Our linear complexity attention-based multi-reference entropy models can capture
  ...
  When computing BD-rate, VTM-17.0 Intra under YUV444 is employed as anchor. We use unofficial weights of ELIC\footnote[2]{\url{https://github.com/VincentChandelier/ELiC-ReImplemetation}} to
  evaluate the rate-distortion performance of ELIC on Tecnick~\cite{tecnick2014TESTIMAGES}, CLIC Professional Valid~\cite{CLIC2020}.
  Our MLIC$^{++}$ outperforms our previous MLIC and MLIC$^+$~\cite{jiang2022mlic}.
  Our MLIC$^{++}$ reduces BD-rate by $13.39\%$ over VTM-17.0 Intra on Kodak~\cite{kodak} while
  existing method ELIC~\cite{he2022elic} only reduces $5.95\%$ BD-rate and LIC-TCM~\cite{liu2023learned}
  only reduces $10.14\%$ BD-rate on Kodak. Moreover, our MLIC$^{++}$ performs much better
  on high-resolution datasets, such as Tecnick~\cite{tecnick2014TESTIMAGES}
  and CLIC Professional Valid~\cite{CLIC2020}. Our MLIC$^{++}$ reduces $17.59\%$ on Tecnick~\cite{tecnick2014TESTIMAGES}
  and reduces $13.08\%$ BD-rate on CLIC Professional Valid~\cite{CLIC2020}, which are much better than existing methods.
  ...
  We highlight the BD-rate for MS-SSIM~\cite{wang2003multiscale}, our proposed
  ...
  PSNR value of the image reconstructed by our MLIC$^{++}$ optimized for MSE is $1$dB
    ...
    MS-SSIM~\cite{wang2003multiscale} of the image reconstructed by our MLIC$^{++}$
...
We compared our MLIC$^{++}$ with our prior work MLIC$^{+}$~\cite{jiang2022mlic} which employs \textit{vanilla} attention to illustrate the advantages of
  ...
  $\frac{1}{2}$ of the GPU memory consumed by LIC-TCM~\cite{liu2023learned} when compressing
  $2048\times 2048$ images.
  When compressing $2560\times 2560$ images, our MLIC$^{++}$ only consumes about $\frac{1}{4}$
  of the GPU memory consumed by LIC-TCM~\cite{liu2023learned}.
  When compressing a $3584\times 3584$ image, the GPU memory consumption of our proposed
  MLIC$^{++}$ is still only $12.3$ GB while the LIC-TCM~\cite{liu2023learned} requires $45.95$ GB GPU memory.
  ...
  MLIC$^{++}$ encodes and decodes much faster than Cheng'20~\cite{cheng2020learned},
  Xie'21~\cite{xie2021enhanced}, and Entroformer~\cite{qian2022entroformer}. The linear complexity global context capturing leads to significant computational overhead reductions
  ...
  the time of MLIC$^{+}$. Compared with recent LIC-TCM~\cite{liu2023learned}, MLIC$^{++}$
  ...
  While the context module in Cheng'20~\cite{cheng2020learned} is simpler than that of ELIC~\cite{he2022elic},
  STF~\cite{zou2022the}, and WACNN~\cite{zou2022the}, the overall MACs of Cheng'20~\cite{cheng2020learned} are
  ...
  transform of Cheng'20~\cite{cheng2020learned}, it results in higher MACs compared to ELIC~\cite{he2022elic} and WACNN~\cite{zou2022the}.
  ...
  % Figure environment removed
    \begin{table}[t]
      ...
      \end{tabular}}
      \caption{Influences of Patch Size on Rate-Distortion Performance. BD-rate (\%) is employed as the metric. The anchor is MLIC$^{++}$ trained on $256\times 256$ patches.}
    ...
    \end{tabular}}
      \caption{Parameter number and Forward Macs of each context modules on Kodak dataset.}
  ...
  Based on proposed context modules, we propose linear complexity attention-based 
  ...
  as well as lighter linear complexity attention-based multi-reference entropy model.
