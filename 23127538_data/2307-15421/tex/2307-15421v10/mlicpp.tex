...
WACNN, and Cheng'20—while maintaining competitive image quality. This demonstrates the efficiency and effectiveness of our approach.}
  ...
  We compared our MLIC$^{++}$ with our prior work MLIC$^{+}$ which employs \textit{vanilla} attention to illustrate the advantages of
    ...
    % Figure environment removed
      ...
      {Although $g_{ch}$ has high macs, its inference is fast due to the $3\times 3$ convolution being highly optimized on GPUs with CUDA/cuDNN\footnote[3]{\url{https://github.com/pytorch/pytorch/blob/main/aten/src/ATen/native/Convolution.cpp}}
while the window attention in $g_{lc,attn}$ and linear attention $g_{gc,intra}$ and $g_{gc,inter}$ are implemented in pure Python/Pytorch}\footnote[4]{\url{https://github.com/JiangWeibeta/MLIC/blob/main/MLIC\%2B\%2B/modules/transform/context.py}}.
