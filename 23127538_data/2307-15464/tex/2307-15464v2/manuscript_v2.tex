...
The overall accuracy of $\mathcal{C}$ is defined as a combination of both accuracy levels as shown in Equation \ref{eq:oa}.
...
Completeness refers to the extent to which the necessary information is captured in the ODC. Specifically, for each $O_i$, the conditions from Equation \ref{eq:comp} hold.
...
The goal is to check if a given ODC contains descriptions for all attributes, capturing their essential characteristics. It also provides documentation for relationships, facilitating the understanding of the data and enabling practical data analysis.

\subsubsection{Implementation}
Equation \ref{eq:comp} does not require implementation because its violation will not allow a well-formed ODC. At the same time, Equation \ref{eq:comp2} can be implemented programmatically; Algorithm \ref{alg:complete} iterates through each dataset in the given ODC and checks for the presence of all the fields. If any of these fields are missing, they are added to a list of missing fields. The completeness percentage is then calculated by dividing the count of complete datasets by the total number of datasets and multiplying it by 100. Finally, the algorithm prints the completeness percentage.
...
Checking for inconsistencies in a given $\mathcal{O}$ of $\mathcal{C}$ involves the identification of contradictory pairs of triples $(t_i, t_j) \in T \in \mathcal{O} \in \mathcal{C}$. A pair of triples is contradictory if they have the same $s$ and $p$ but different $o$. Formally, for any $t_i = (s, p, o_i)$ and $t_j = (s, p, o_j)$, the Equation \ref{eq:cons} must hold true.
...
If at least one pair of contradictory triples exist in any $\mathcal{O}$, then that subset is considered inconsistent. The average (in)consistency of the ODC is the proportion of (in)consistent objects in the catalog.
...
Scalability is the ability to efficiently handle an increasing volume of data and adapt to changing data requirements while maintaining acceptable performance levels. Therefore, scalability refers to the ability of an ODC to handle increasing amounts of data without significant degradation in performance. More formally, let $N$ be the number of data records in the ODC, and let $T(N)$ be the time required to perform a specific operation. An ODC $\mathcal{C}$ is considered scalable if the condition in Equation \ref{eq:sca} holds.
...
Timeliness in an ODC ensures that the catalog contains up-to-date information, and can be evaluated from two perspectives: data freshness and data availability.
...
Data availability refers to the responsiveness of the catalog. Let $R(a_k)$ be the response time associated with attribute $a_k$, representing the time required to retrieve that attribute from the catalog. Data Availability ensures that the data in the catalog is readily accessible and can be retrieved within an acceptable time frame. We can formulate this notion as follows: an ODC $\mathcal{C}$ is considered to have \textit{data availability} if the Equation \ref{eq:time2} holds.
...
where $\text{Sim}_{\text{value}}(O_i, O'_i)$ represents the similarity between attribute values of the corresponding attributes in objects $O_i$ and $O'_i$. 
