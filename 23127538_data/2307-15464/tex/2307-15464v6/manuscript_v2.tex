...
In ODCs, licensing is pivotal in shaping the dissemination and utilization of the information. ODCs serve as repositories of diverse datasets, facilitating transparency and collaboration among the data stakeholders. Within this context, implementing suitable licensing mechanisms becomes imperative to ensure equitable access, protect intellectual property rights, and facilitate responsible utilization.

Algorithm \ref{alg:check_license} is designed to detect the presence of licensed items within a given ODC. It would also be possible to operate by iteratively examining each item in the catalog, if necessary. 
...
\section{Use cases}
In order to illustrate how our methods work in practice, we are going to test these methods in a real environment. For this, we are going to select a set of ODCs of public institutions at European level and we are going to test live our quality measures. 

\subsection{Empirical testing}
We are going to see how our proposal works with some ODCs offered officially through the European Union's open data portal\footnote{\url{https://data.europa.eu/}}. The objective is to determine how our proposal performs in a real environment and to compare these results with the quality rating given by the European Union's open data portal itself.

To do that, we have chosen several ODCs, and we are going to study each of the quality dimensions described earlier in this paper. The criteria for the selection of these ODCs have been that a) they did not belong to any specific country of the European Union, but rather to any of its agencies or organizations, b) they were represented using the DCAT standard, and c) they represent a balance combination of catalogs, datasets, and distributions. It is necessary to remark that a small adaptation has had to be made since many ODCs indistinctly combine DCAT tags with Dublin Core \footnote{\url{https://dublincore.org/}} tags, which is a widely used standard in this context.

It should be noted that the experiment was performed on December 21, 2023, so some of the results may not be reproducible. To minimize this risk, we have stored a copy of the ODC available at that date in our repository. However, some aspects such as the accessibility of the sites listed in the ODC cannot be reproduced exactly\footnote{The reason is that, for example, a website might not always have continuous availability}. As a result of our filtering process, we have selected the following ODCs for our evaluation:
...
As can be seen, there is a high degree of correspondence between the EU's own rating on its portal and the one obtained automatically and shelled using our approach.
...
Another approach for future exploration lies in leveraging machine learning to enhance the accuracy of the quality assessment framework. Integrating deep learning and natural language processing techniques can enable the framework to uncover quality patterns and anomalies, entrusting data stakeholders to gain deeper insights into the cataloged data.

Furthermore, validating the framework on a broad range of domains and use cases is crucial. Conducting empirical studies and real-world evaluations across diverse industries and application scenarios will help demonstrate the framework's applicability.
