...
Let $\mathcal{C}$ be an ODC, represented as a set of objects $\mathcal{O} = {O_1, O_2, O_3, \ldots, O_n}$, where each $O_i$ consists of set of attributes $A_i = {a_1, a_2, a_3, \ldots, a_m}$.

Accuracy is about the degree to which $\mathcal{C}$ contains correct information. For the remainder of this paper, and as the DCAT standard is based on RDF, please consider attributes as the objects of the assertions and relations as the predicates.
...
The attribute-level accuracy for $a_k$ can be measured using an error function $E(a_k)$, which quantifies the discrepancy between the actual value $V(a_k)$ and the stored value $D(a_k)$. Therefore, the attribute-level accuracy for an ODC can be defined as shown in Equation \ref{eq:aa}.
...
Relationship-level accuracy refers to the correctness of relationships in the ODC. Let $R(a_k, a_l)$ be the actual relationship between attributes $a_k$ and $a_l$, and $C(a_k, a_l)$ be the corresponding relationship recorded.

The relationship-level accuracy for $a_k$ and $a_l$ can be measured using an error function $E(a_k, a_l)$, which quantifies the discrepancy between the true relationship $R(a_k, a_l)$ and the actual relationship $C(a_k, a_l)$ exactly as shown in Equation \ref{eq:ra}
...
Once again, let $\mathcal{C}$ be an ODC, represented as a set of objects $\mathcal{O} = {O_1, O_2, O_3, \ldots, O_n}$, where each $O_i$ consists of attributes $A_i = {a_1, a_2, a_3, \ldots, a_m}$.

Completeness is about the extent to which the necessary information is captured in the ODC. Specifically, for each $O_i$, the conditions from Equation \ref{eq:comp} hold.
...
where $relationship(a_k, a_l)$ represents an actual relationship between $a_k$ and $a_l$, and $documented\_relationship(a_k, a_l, O_i)$ represents the description for the relationship between $a_k$ and $a_l$ in $O_i$. Therefore, $\mathcal{C}$ is complete if the above conditions hold for all objects $O_i$ in $\mathcal{C}$.
...
Let $\mathcal{C}$ be an ODC represented as a set of objects $\mathcal{O} = {O_1, O_2, O_3, \ldots, O_n}$. Furthermore, let $T$ be a set of triples in a given $\mathcal{O}$ of $\mathcal{C}$, where each triple $t \in T$ is represented as $t = (s, p, o)$, denoting the subject, predicate, and object, respectively. 

Checking for inconsistencies in a given $\mathcal{O}$ of $\mathcal{C}$ involves the identification of contradictory pairs of triples $(t_i, t_j) \in T \in \mathcal{O} \in \mathcal{C}$. Please consider that a pair of triples belonging to the same object is contradictory if they have the same $s$ and $p$ but different $o$. We can express it formally such as for any $t_i = (s, p, o_i)$ and $t_j = (s, p, o_j)$, the Equation \ref{eq:cons} must hold true.
...
Scalability refers to efficiently handling an increasing volume of data and changing data requirements while maintaining acceptable performance levels. Therefore, we are talking about the possibility of handling increasing amounts of data without degradation in performance. More formally, let $N$ be the total number of records, and let $T(N)$ be the time units required to perform a specific operation. An ODC $\mathcal{C}$ is considered scalable concerning that operation if the condition in Equation \ref{eq:sca} holds.
...
Data freshness is about the degree to which the data in the catalog reflects the current state of its corresponding real-world entities. Therefore, let $C(a_k)$ be the timestamp associated with attribute $a_k$, indicating the last update time of that attribute in the catalog. Given a current timestamp $t$, an ODC $\mathcal{C}$ is considered to have \textit{data freshness} if the Equation \ref{eq:time} holds.
...
Data availability is about the responsiveness of the catalog. Therefore, let $R(a_k)$ be the response time associated with attribute $a_k$, representing the time required to retrieve that attribute from the catalog. Data Availability ensures that the data is readily accessible and can be retrieved within an acceptable time frame. 

We can formulate this notion as follows: an ODC $\mathcal{C}$ is considered to have \textit{data availability} if the Equation \ref{eq:time2} holds.
...
It is possible to formalize this notion as follows; let $\mathcal{C}_1$ and $\mathcal{C}_2$ be two ODCs. Their compatibility can be defined as shown in Equation \ref{eq:compat}.
...
where $|\mathcal{C}_1|$ is the cardinality of $\mathcal{C}_1$,and $\mathcal{C}_1 \cap \mathcal{C}_2$ is the intersection of $\mathcal{C}_1$ and $\mathcal{C}_2$.

This means that the compatibility is determined by the cardinality ratio of the intersection between the two ODCs to the cardinality of $\mathcal{C}_1$, i.e., the proportion of elements in $\mathcal{C}_1$ that is also present in $\mathcal{C}_2$.

In order to homogenize with the rest of the quality dimensions, the compatibility score can range from 0 to 100, where a value of 100 indicates that $\mathcal{C}_1$ and $\mathcal{C}_2$ are entirely compatible. In contrast, a value of 0 indicates that both ODCs are not compatible since they have no common elements.
...
Furthermore, similarity assessment can facilitate search and retrieval, enabling users to locate relevant ODCs based on their similarity to a given input \cite{key-martinez-jiis}. This capability enhances data discovery, facilitates reuse, and promotes efficient decision-making processes. Therefore, similarity assessment is instrumental in promoting data interoperability, enabling effective data integration, and facilitating data utilization across diverse environments \cite{key-martinez-mlwa}.
