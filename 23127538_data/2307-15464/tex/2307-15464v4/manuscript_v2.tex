...
Completeness is the quality dimension that aims to quantify the extent to which the necessary information is captured in the ODC. This means that, for each $O_i$, the Equation \ref{eq:comp} must hold.
...
Let $\mathcal{C}$ be an ODC represented as a set of objects $\mathcal{O} = {O_1, O_2, O_3, \ldots, O_n}$. Furthermore, let $T$ be a set of triples in a given $\mathcal{O}$ of $\mathcal{C}$, where each triple $t \in T$ is represented as $t = (s, p, o)$, representing the subject, predicate, and object, respectively. 

Checking for inconsistencies in a given $\mathcal{O}$ of $\mathcal{C}$ involves the identification of contradictory triples $(t_i, t_j) \in T \in \mathcal{O} \in \mathcal{C}$. Two triples belonging to the same $\mathcal{O}$ are contradictory if they have the same $s$ and $p$ but different $o$. We can express it formally through the Equation \ref{eq:cons}.
...
where $|\mathcal{C}_1|$ is the cardinality of $\mathcal{C}_1$,and $\mathcal{C}_1 \cap \mathcal{C}_2$ is the intersection of $\mathcal{C}_1$ and $\mathcal{C}_2$. Therefore, the compatibility estimates the cardinality ratio of the intersection between the two ODCs to the cardinality of $\mathcal{C}_1$.

The compatibility score can range from 0 to 100, where a value of 100 indicates that the catalogs are entirely compatible since all their elements are common. In contrast, a value of 0 indicates that both catalogs are incompatible since they have no common elements.
...
In this way, the attribute-level similarity $\text{Sim}_{\text{attribute}}(\mathcal{C}_1, \mathcal{C}_2)$ can be defined as shown in Equation \ref{eq:sim2}.
