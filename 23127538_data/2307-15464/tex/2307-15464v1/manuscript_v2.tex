...
\begin{itemize}
  ...
  \item We also delineate ODC cross-catalog dimensions and explore potential implementations for cases involving multiple catalogs. In fact, we develop the notions of compatibility and similarity to contribute to a comprehensive understanding and development of new quality metrics in ODC quality assessment.
...
The DCAT vocabulary is an established standard to describe ODCs on the Web effectively. DCAT involves a set of classes, properties, and relationships that facilitate the publication, discovery, and exchange of essential information about datasets \cite{key-Nogueras}. This standardization facilitates interoperability across diverse platforms and systems, thus enabling seamless integration and utilization of data resources \cite{key-Lobe}.
...
The standardized representation of data resource information through DCAT simplifies exposing datasets for data publishers while facilitating OD consumers to find pertinent data efficiently. Furthermore, this standardized vocabulary enables seamless data exchange and integration among diverse data platforms, data portals, and other data-centric tools by employing a shared vocabulary.
...
\subsection{Accuracy}
ODCs should maintain accurate information about the data assets they represent. The metadata, such as descriptions, data types, and so on, should reflect the actual characteristics of the underlying data. Ensuring accuracy helps users make informed decisions and avoid errors when working with the cataloged data. Therefore, in the context of an ODC, data accuracy should be associated with the correctness of the information provided about the data.

\subsubsection{Accuracy in a Data Catalog}
Let $\mathcal{C}$ be an ODC, represented as a set of objects $\mathcal{O} = {O_1, O_2, \ldots, O_n}$, where each $O_i$ consists of attributes $A_i = {a_1, a_2, \ldots, a_m}$.

Accuracy in an ODC refers to the degree to which the data stored in the ODC reflects the actual values and characteristics of the real-world entities it represents. Accuracy can be assessed in terms of attributes and relationships, respectively. For the remainder of this paper, and as the DCAT standard is based on RDF, please consider attributes as the objects of the assertions and relations as the predicates.

\subsubsection{Attribute-level Accuracy}
Attribute-level accuracy refers to the correctness of individual attribute values in the ODC. Let $V(a_k)$ be the actual value associated with attribute $a_k$, and $D(a_k)$ be the corresponding value in the ODC.

The attribute-level accuracy for attribute $a_k$ can be measured using an error function $E(a_k)$, which quantifies the discrepancy between the actual value $V(a_k)$ and the stored value $D(a_k)$. The attribute-level accuracy for $\mathcal{C}$ can be defined as shown in Equation \ref{eq:aa}.
...
Relationship-level accuracy refers to the correctness of relationships between attributes in the ODC. Let $R(a_k, a_l)$ be the actual relationship between attributes $a_k$ and $a_l$, and $C(a_k, a_l)$ be the corresponding relationship recorded.

The relationship-level accuracy for attributes $a_k$ and $a_l$ can be measured using an error function $E(a_k, a_l)$, which quantifies the discrepancy between the true relationship $R(a_k, a_l)$ and the actual relationship $C(a_k, a_l)$ as shown in Equation \ref{eq:ra}

\begin{equation}
\text{Relationship-level Accuracy}(\mathcal{C}) = \frac{1}{\binom{m}{2}} \sum_{k=1}^{m-1} \sum_{l=k+1}^{m} (1 - E(a_k, a_l))
\label{eq:ra}
\end{equation}

where $\binom{m}{2}$ represents the number of unique attribute pairs in the ODC.
...
The overall accuracy of $\mathcal{C}$ can be defined as a combination of attribute-level and relationship-level accuracy as shown in Equation \ref{eq:oa}.

\begin{equation}
\text{Accuracy}(\mathcal{C}) = \alpha \cdot \text{Attr.-level Accuracy}(\mathcal{C}) + (1-\alpha) \cdot \text{Relat.-level Accuracy}(\mathcal{C})
\label{eq:oa}
\end{equation}

where $\alpha$ is weighting factor about the relative importance of the two types of accuracy seen above.
...
Completeness can be defined as the extent to which the necessary information is captured in the ODC. Specifically, for each $O_i$, the conditions from Equation \ref{eq:comp} hold.
...
\begin{equation}
relationship(a_k, a_l) \Rightarrow documented\_relationship(a_k, a_l, O_i)
\label{eq:comp2}
\end{equation}

where $relationship(a_k, a_l)$ represents an actual relationship between the attributes $a_k$ and $a_l$, and $documented\_relationship(a_k, a_l, O_i)$ represents the description for the relationship between attributes $a_k$ and $a_l$ in $O_i$. Therefore, $\mathcal{C}$ is complete if the above conditions hold for all objects $O_i$ in $\mathcal{C}$.

The goal is to check if a given ODC contains descriptions for all attributes, capturing their essential characteristics. It also provides documentation for relationships, facilitating the understanding of the data and enabling practical data analysis, integration, and utilization.

\subsubsection{Implementation}
Equation \ref{eq:comp} does not require implementation because its violation will not allow a well-formed ODC. At the same time, Equation \ref{eq:comp2} can be implemented programmatically; Algorithm \ref{alg:complete} iterates through each dataset in the given ODC and checks for the presence of all the fields. If any of these fields are missing, they are added to a list of missing fields. If the list of missing fields is not empty for a dataset, a counter is increased by one unit. The completeness percentage is then calculated by dividing the count of complete datasets by the total number of datasets and multiplying it by 100. Finally, the algorithm prints the completeness percentage.
...
Let $\mathcal{C}$ be an ODC represented as a set of objects $\mathcal{O} = {O_1, O_2, \ldots, O_n}$. Furthermore, let $T$ be a set of triples in a given $\mathcal{O}$ of $\mathcal{C}$, where each triple $t \in T$ is represented as $t = (s, p, o)$, denoting the subject, predicate, and object, respectively. 

Checking for inconsistencies in $\mathcal{C}$ involves the identification of contradictory pairs of triples $(t_i, t_j) \in T \in \mathcal{O} \in \mathcal{C}$. A pair of triples is considered contradictory if they have the same subject and predicate but different objects. Formally, for any $t_i = (s, p, o_i)$ and $t_j = (s, p, o_j)$, the Equation \ref{eq:cons} must hold true for inconsistency.

\begin{equation}
(t_i, t_j) \text{ is contradictory} \iff o_i \neq o_j
\label{eq:cons}
\end{equation}

If at least one pair of contradictory triples exist in any $\mathcal{O}$, then that subset is considered inconsistent. The average consistency of the ODC is the proportion of consistent objects.

\subsubsection{Implementation}
Algorithm \ref{alg:consistency} assesses the consistency of a given ODC by using a loop to iterate through the triples in each dataset and checks for inconsistencies in the attribute values by comparing objects associated with the same (subject, predicate) pair. The inconsistent pairs are added to the \func{contradictions} set, and then the inconsistency percentage which is then printed. The algorithm in practice can also be applied to other items such as distributions.
...
\For{each \textit{dataset} in \textit{datasets}}		
		\For{each triple $(subj, pred, obj)$ in \textit{dataset}}
...
Scalability in an ODC is defined as the ability to efficiently handle an increasing volume of data and adapt to changing data requirements while maintaining acceptable performance levels. Therefore, scalability refers to the ability of an ODC to handle increasing amounts of data without significant degradation in performance. Let $N$ be the number of data records in the ODC, and let $T(N)$ be the time required to perform a specific operation. An ODC $\mathcal{C}$ is considered scalable if the condition in Equation \ref{eq:sca} holds.
...
This condition indicates that the time required for a specific operation per data record decreases as the data volume increases or at least remains bounded. It implies that the data size does not significantly impact performance. In other words, this ensures that the ODC can efficiently handle increasing data volumes and adapt to changing data requirements while maintaining acceptable performance levels. This will generally be true with exceptions in case a single operation requires the processing of several attributes.

\subsubsection{Implementation}
Algorithm \ref{alg:scalability} aims to evaluate the scalability of a given ODC. It measures the data volume scalability by assessing the time required for a specific operation per data record by examining the effort needed to update the attribute. The resulting score provide insights into the catalog's scalability.
...
Let $\mathcal{C}$ be an ODC, represented as a set of objects $\mathcal{S} = {O_1, O_2, \ldots, O_n}$, where each $O_i$ consists of attributes $A_i = {a_1, a_2, \ldots, a_m}$.

Timeliness in an ODC refers to the freshness and availability of data, ensuring that the catalog contains up-to-date information. Timeliness can be evaluated from two perspectives: data freshness and data availability.

\subsubsection{Data Freshness}
Data freshness refers to the degree to which the data in the catalog reflects the current state of the corresponding real-world entities it represents. Let $C(a_k)$ be the timestamp associated with attribute $a_k$, indicating the last update time of that attribute in the catalog. An ODC $\mathcal{C}$ is considered to have \textit{data freshness} if the Equation \ref{eq:time} holds.
...
This condition states that for every attribute $a_k$ in the catalog, a timestamp $t$ exists such that $t$ is greater than the last update time $C(a_k)$.

\subsubsection{Data Availability}
Data availability refers to the responsiveness of the catalog. Let $R(a_k)$ be the response time associated with attribute $a_k$, representing the time required to retrieve that attribute from the catalog. An ODC $\mathcal{C}$ is considered to have \textit{data availability} if the Equation \ref{eq:time2} holds.
...
This condition states that for every attribute $a_k$ in the ODC, the response time $R(a_k)$ is less than or equal to a predefined threshold $T$. It ensures that the data in the catalog $\mathcal{C}$ is readily accessible and can be retrieved within an acceptable time frame.  
...
The compatibility of two ODCs refers to their respective datasets' seamless integration and complementarity. In order to assess compatibility, several key factors must be considered. Firstly, the format and structure of both catalogs. Secondly, common identifiers across datasets facilitate the merging or replacement process.
...
We can formalize this notion as follows; let $\mathcal{C}_1$ and $\mathcal{C}_2$ be two ODCs. Their compatibility can be defined as shown in Equation \ref{eq:compat}.
...
where $|\mathcal{C}_1|$ denotes the cardinality of $\mathcal{C}_1$,and $\mathcal{C}_1 \cap \mathcal{C}_2$ represents the intersection of $\mathcal{C}_1$ and $\mathcal{C}_2$.

The compatibility is determined by the cardinality ratio of the intersection between the two ODCs to the cardinality of $\mathcal{C}_1$, i.e., the proportion of elements in $\mathcal{C}_1$ that is also present in $\mathcal{C}_2$.

In order to homogenize with the rest of the dimensions, the compatibility score can range from 0 to 100, where a value of 100 indicates that $\mathcal{C}_1$ and $\mathcal{C}_2$ are entirely compatible (i.e., $\mathcal{C}_1$ is a subset of $\mathcal{C}_2$). In contrast, a value of 0 indicates no compatibility at all (i.e., both ODCs have no common elements).
...
Assessing the similarity between ODCs is crucial in various data management and integration tasks. Similarity assessment enables comparing and aligning ODCs from different sources, facilitating data integration, exchange, and collaboration across systems and organizations \cite{key-martinez-eswa2}. Quantifying the degree of similarity between catalogs makes it possible to identify shared attributes, detect redundancies, and match corresponding items \cite{key-martinez-jfis}. 

Furthermore, similarity assessment can facilitate search and retrieval, enabling users to locate relevant ODCs based on their similarity to a given input \cite{key-martinez-jiis}. This capability enhances data discovery, facilitates reuse, and promotes efficient decision-making processes. Overall, similarity assessment is instrumental in promoting data interoperability, enabling effective data integration, and facilitating data utilization across diverse environments \cite{key-martinez-mlwa}.

In this work, we refer to attribute-level similarity that measures the similarity of the attribute values between corresponding attributes in the two ODCs. The idea is to quantify the resemblance between attribute values based on appropriate similarity measures or metrics, such as edit distance, cosine similarity, or Jaccard similarity. The attribute-level similarity $\text{Sim}_{\text{attribute}}(\mathcal{C}_1, \mathcal{C}_2)$ can be defined as shown in Equation \ref{eq:sim2}.
...
where $\text{Sim}_{\text{value}}(O_i, O'_i)$ represents the similarity between attribute values of the corresponding attributes in objects $O_i$ and $O'_i$. The summation is performed over the shared objects between $\mathcal{C}_1$ and $\mathcal{C}_2$.
...
\Procedure{CheckLicensingRights}{catalog}
    ...
    \State \textbf{Print} 'The catalog is ' $\text{percentage\_licensing}$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\section{Discussion}
We have seen how our framework contributes to data management and governance of metadata by providing a practical approach for researchers and practitioners to assess the quality of ODCs. By adopting this framework, data consumers can establish a robust foundation for data-driven initiatives, improving reliability and enabling trust in the cataloged data.
