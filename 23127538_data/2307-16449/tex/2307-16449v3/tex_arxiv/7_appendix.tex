...
\begin{enumerate}
\item We first present schematic diagram of the memory consolidation algorithm of MovieChat in Section~\ref{sec:merge}.

\item We provide detailed supplementary statistical information for MovieChat-1K in Section~\ref{sec:benchmark_stat}.

\item The prompt template we use for LLM-Assisted Evaluation is shown in Section~\ref{sec:gpt-eval}.
...
\item We then list the evaluaotion results with GPT-3.5~\cite{gpt3.5}, Claude~\cite{examplewebpage} and human blind rating in Section~\ref{sec:eva_methods}.
