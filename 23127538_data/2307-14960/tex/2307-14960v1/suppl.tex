...
In this Supplemental Material, we show, in Sec.~\ref{app:Haldane}, the details of the Chern number's computation used in the main text, in relation with the Bloch eigenvectors’ components properties over the BZ. In Sec.~\ref{app:inputoutput}, we show the computation of the coefficient which relates input voltages in a set of probes capacitively coupled to
a bosonic lattice model   and the resulting output voltage in a one of these probes.

\section{Chern number, gauge choices and unique and smooth Berry gauge fields} \label{app:Haldane}

In this section we show a detailed analytical computation of the Chern number, based on an approach introduced by Kohmoto \cite{Kohmoto85}, for the Haldane system with Hamiltonian defined in the Eq.~\eqref{eq:hk}. 
...
$\rho_i({\bf k})$ and $ \lambda_i({\bf k})$ may vanish only at the Dirac points and it depends on the sign of $h_2$, given in Table~\ref{tab:sgnh2} as a function of $M$ and $\phi$. For $|M| <3 \sqrt{3} t_2 |\sin \phi|$ ($|M| >3 \sqrt{3} t_2 |\sin \phi|$), the values of $\rho_i({\bf k})$ and $ \lambda_i({\bf k})$ at the Dirac points are given in Table~\ref{tab:rho1} (Table~\ref{tab:rho3}).
...
As explained in the main text, we then define two non-overlapping domains in the BZ \cite{Kohmoto85} and use a different gauge choice in each domain. To be more specific, for $(-1)^i \, \textrm{sgn}( \sin \phi ) = +1$, we apply $G_I$ for the points contained in $\mathcal{D}_{II}$ and $G_{II}$ for the points contained in $\mathcal{D}_{I}$ while for $(-1)^i \, \textrm{sgn}( \sin \phi ) = -1$, we apply $G_{I}$ for the points contained in $\mathcal{D}_{I}$ and $G_{II}$ for the points contained in $\mathcal{D}_{II}$.
...
\begin{equation} \label{chern_nunununu}
    C_j = \dfrac{1}{2 i \pi} \bigg[ \int_{\mathcal{D}_{\textrm{I}}} d^2k \left( {\bf \nabla}_{{\bf k}} \times {\bf A}_{j,{\bf k},\textrm{I}} \right)\cdot{\bf e}_z  +\int_{\mathcal{D}_{\textrm{II}}} d^2k \left( {\bf \nabla}_{{\bf k}} \times {\bf A}_{j,{\bf k},\textrm{II}} \right)\cdot{\bf e}_z\bigg].
\end{equation}
Using Stokes' theorem and considering the specific choice of path P shown in Fig.~\ref{fig1}(c) leads to
\begin{equation} 
...
\end{equation}
Along P, we have $\ket{u_{j,{\bf k},I}} =\textrm{e}^{i (-1)^j \, \textrm{sgn}( \sin \phi ) \varphi({\bf k})}\ket{u_{j,{\bf k},II}}$. 
...
\subsection{Input-output analysis} Here we use the input-output formalism as reviewed in Ref.~\cite{Clerk10}.
...
\end{equation}
are defined through the components $\alpha_{i }^{1}({\bf k})$ and $\alpha_{i }^{2}({\bf k})$, which are given by
\begin{equation} \label{eq:blockcompo}
\alpha_{i }^{2}({\bf k}) =  \dfrac{h_1({\bf k})^*}{h_2({\bf k})-h_0({\bf k})+E_{i,{\bf k}}} \alpha_{i }^{1}({\bf k}), 
\end{equation}
where $E_{i,{\bf k}} = h_0({\bf k}) + (-1)^i \epsilon({\bf k})$, $\epsilon({\bf k}) = \sqrt{|h_1({\bf k})|^2+h_2({\bf k})^2}$ and the coefficients $\beta_{i }^{j}({\bf k})$ are defined by 
