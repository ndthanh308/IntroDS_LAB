...
Another important aspect of massive higher spins is the complexity of their propagators. Explicit propagators resulting from the actions \cite{Singh:1974qz, Singh:1974rc} were written in \cite{Singh:1981aw}. A different set of propagators were written in \cite{Weinberg:1964cn}, without any reference to an action. They both find spin $s$ propagators which grow like $\sim p^{-2+2s}$ in momentum space. Propagators for spins 1 and higher are not unique, and we demonstrate this non-uniqueness as a manifestation of gauge invariance within this formalism. In this paper, we instead find propagators which grow like $\sim p^{-2}$ and $\sim p^{-1}$ for all integer and half-integer spins, respectively. This discrepancy is understood as a natural extension of the well known fact that massive spin 1 propagators can have different high energy behavior depending on the gauge one picks, e.g. unitarity or $R_{\xi}$ gauges. The improved high energy behavior is achieved at the cost of introducing unphysical polarizations into the propagator, but as in Yang-Mills theory, as long as gauge invariant interactions are present, these unphysical polarizations should not contribute to observables. Furthermore, because the propagators are valid in any spacetime dimension $d$, dimensional regularization can be used readily.

\sm

The expressions for the propagators are compactly written in terms of Gegenbauer polynomials. This is made possible by introducing an auxiliary vector coordinate $s^{\mu}$, and writing propagators for the fields $\frac{1}{n!}\phi_{\mu_1\cdots\mu_n}(X)s^{\mu_1}\cdots s^{\mu_n}$. In particular, this allows us to write the propagators so that the spin $s=n, n+1/2$ is a freely tunable parameter, enabling analytic control over the large spin asymptotic limit $n\to\infty$. This may be particularly helpful in the current research program of using field theory techniques to model black hole binary dynamics with classical values of spin, including but not limited to  \cite{Chung:2018kqs,Guevara:2018wpp,Bern:2020buy,Chiodaroli:2021eug,Bern:2022kto,Cangemi:2022bew}.

\sm

Finally, by virtue of the gauge symmetry inherent in this formulation, finding interactions which preserve the higher spin particle's degrees of freedom from first principles is equivalent to maintaining some form of gauge invariance. Formulating massive higher spins in a way that can be used as a starting point for straightforwardly introducing interactions is a main motivation for the current work. The formulation we arrive at is similar to previous work by Zinoviev \cite{Zinoviev:2001dt} for massive integer spins. Perhaps one virtue of the current work is that the covariant Lagrangians and the gauge symmetries we arrive at do not depend on the spacetime dimension $d$. Furthermore, we find natural objects $\mathcal{F}_{\mu_1\cdots\mu_n}$, $\mathcal{F}_{\mu_1\cdots\mu_{n-1}}$, $\mathcal{F}_{\mu_1\cdots\mu_{n-2}}$, $\mathcal{F}_{\mu_1\cdots\mu_{n-3}}$ for the integer spin case, and $\mathcal{S}_{\mu_1\cdots\mu_n}$, $\mathcal{S}_{\mu_1\cdots\mu_{n-1}}$, $\mathcal{S}_{\mu_1\cdots\mu_{n-2}}$ for the half integer spin case, which are gauge invariant, described in \autoref{sec5} and \autoref{sec6}. We delay a detailed discussion of interactions within this formalism for future work.
...
\subsection{Outline}
In \autoref{sec2}, we introduce the ``hyperfield" formalism, which will enable compact expressions for the covariant actions, propagators, and interactions in subsequent sections. Then in \autoref{sec3} and \autoref{sec4}, we review the formulation of free massless higher spins \cite{Fronsdal:1978rb,Fang:1978wz}, and we compute their propagators in a special gauge, as well as characterize their gauge ambiguities. In \autoref{sec5} and \autoref{sec6}, we do the same for massive higher spins, defining them via ``dimensional reduction" of their massless counterparts as in \cite{Aragone:1987dtt}. Finally in \autoref{sec:disc}, we discuss our results and present some directions for future research.

\section{Hyperfields}
\label{sec2}

In this section, we introduce the notion of a hyperfield, which greatly simplifies the analysis of fields with arbitrary spin. Similar techniques have been used in the high spin literature, including but not limited to \cite{Bekaert:2005in, Ponomarev:2016jqk, Roiban:2017iqg}. We will find in later sections that covariant actions and propagators admit particularly simple closed form expressions within this formalism.
...
\label{sec3}
In this section, we will use the hyperfield formalism of \autoref{sec2} to study massless integer spin fields. We will find compact, closed form expressions for their covariant actions and propagators. First, we review the standard description of massless integer spin fields $\phi_{\mu_1\cdots\mu_n}(X)$.
...
\begin{align}
    \mathcal{F}_{0j_1\cdots j_{n-1}},\quad \mathcal{F}^{\lambda}_{\;\;\lambda j_1\cdots j_{n-2}},
\end{align}
where the $j$ indices run over spatial components, are at most first order in time. These constraints do not affect the counting of $N_0(d,n)$, because in the gauge (\ref{eq:indexdeDonder}), these components turn into dynamical equations. In other gauges the counting of these constraints may become relevant.
...
\end{align}
where $G_0(X-Y)$ is the standard massless spin $0$ propagator. This formula follows directly from the recursive relation $(n+\alpha)C^\alpha_n = \alpha(C^{\alpha+1}_n-C^{\alpha+1}_{n-2})$. This propagator agrees with previous work \cite{Ponomarev:2016jqk}.
...
\begin{align}
    \langle\Phi_n(X,s)\Phi_n(Y,t)\rangle \sim G_0(X-Y)\frac{1}{2^n(n!)^2}(-i\sqrt{s^2t^2})^n\frac{2\cos\big((n+\frac{d-4}{2})\theta-\frac{\pi}{2}\frac{d-4}{2}\big)}{(2\sin\theta)^{\frac{d-4}{2}}}\quad \text{(as }n\to\infty)
\end{align}
where $\cos\theta = s\cdot t/\sqrt{s^2t^2}$, for $0 < \theta < \pi$.
...
\begin{align}
    \gamma^{\lambda}\mathcal{S}_{\lambda j_1\cdots j_{n-1}}
\end{align}
...
To find the propagator, one might proceed as in the integer spin case by gauge fixing $\Psi_n(X,s)$. The natural gauge to pick is (\ref{eq:indexdeDirac})
\begin{align}
\label{eq:deDirac}
    (\fsl{\partial}_s\fsl{\partial}_X-s\cdot\partial_X\partial_s^2)\Psi_n(X,s) = 0
\end{align}
In the case of half integer spins however, this choice does not simplify the analysis, and there does not appear to be such a gauge choice, so we instead proceed directly by finding the simplest solution to (\ref{eq:mlesshalfpropeom}). Working in momentum space, one can classify all possible structures which have a simple pole at $p^2=0$, have a numerator linear in momentum, and are not of the form (\ref{eq:mlesshalfpropamb}), which are triple $\gamma$ traceless in both $s$ and $t$, and make an ansatz that $\langle\Psi_n(X,s)\overline{\Psi}_n(Y,t)\rangle$ is a linear combination of these in momentum space. These structures can be obtained from the three basic structures
\begin{align}
...
\end{align}
where $\Delta_0(X-Y)$ is the standard massless spin $1/2$ propagator. As in the integer spin case, the massless spin $n+1/2$ propagators $\langle\psi_{\mu_1\cdots\mu_n}(X)\overline{\psi}_{\nu_1\cdots\nu_n}(Y)\rangle$ from (\ref{eq:mlesshalfindexaction}) can be obtained from $\langle\Psi_n(X,s)\overline{\Psi}_n(Y,t)\rangle$ straightforwardly by applying derivatives
\begin{align}
...
First, we detail the ambiguity in these correlation functions arising from the St{\"u}ckelberg gauge symmetry (\ref{eq:Stueck}). Coupling $\Phi_n$, $\Phi_{n-1}$, $\Phi_{n-2}$, and $\Phi_{n-3}$ to external sources $\mathcal{J}_n$, $\mathcal{J}_{n-1}$, $\mathcal{J}_{n-2}$, and $\mathcal{J}_{n-3}$, respectively, will lead to inconsistencies unless it is done in a gauge invariant way. The gauge invariant coupling of $\Phi_{n-i}$ to $\mathcal{J}_{n-i}$ is
\begin{align}
...
% \subsubsection{Physical gauge}
% Because there exists a gauge which sets $\Phi_1 = \Phi_2 = 0$ off-shell, it is possible to set all components of $\langle\Phi_i(X,s)\tilde{\Phi}_j(Y,t)\rangle$ to zero except for $\langle\Phi_0(X,s)\tilde{\Phi}_0(Y,t)\rangle$, $\langle\Phi_0(X,s)\tilde{\Phi}_3(Y,t)\rangle$, $\langle\Phi_3(X,s)\tilde{\Phi}_0(Y,t)\rangle$, and $\langle\Phi_3(X,s)\tilde{\Phi}_3(Y,t)\rangle$. 

% \sm

% In fact, it is possible to further constrain the possible forms of the remaining correlation functions by making the following comments. Without loss of generality, we may restrict to the individual spin $n$ level by setting $\Phi_0(X,s) = \frac{1}{n!}i^{-n/2}\phi_{\mu_1\cdots\mu_n}(X)s^{\mu_1}\cdots s^{\mu_n}$ and $\Phi_3(X,s) = \frac{1}{(n-3)!}i^{-(n-3)/2}\phi_{\mu_1\cdots\mu_{n-3}}(X)s^{\mu_1}\cdots s^{\mu_{n-3}}$ for this discussion. By construction, a gauge must exist which makes $\Phi_3(X,s)$ vanish and $\Phi_0(X,s)$ be a Fierz-Pauli system after imposing the equations of motion.
% \begin{align}
% \label{eq:hyperFP}
%     (\partial_X^2-m^2)\Phi_0 &= 0 \nonumber\\
%     \partial_s\cdot\partial_X\Phi_0 &=0 \nonumber\\
%     \partial_s^2\Phi_0 &=0 \nonumber\\
%     \Phi_3 &=0 
% \end{align}
% Indeed, there is a residual gauge symmetry $\epsilon_0 = \frac{1}{6m^3}(s\cdot\partial_X)^2\epsilon_3$, $\epsilon_1 = \frac{i^{-1/2}}{6m^2}s\cdot\partial_X\epsilon_3$, where $\epsilon_3$ itself is a Fierz-Pauli system $(\partial_X^2-m^2)\epsilon_3 = 0$, $\partial_s\cdot\partial_X\epsilon_3 = 0$, and $\partial_s^2\epsilon_3 = 0$, which keeps $\Phi_1$ and $\Phi_2$ zero but transforms $\Phi_0$ and $\Phi_3$
% \begin{align}
%     &\delta\Phi_0 = i^{-1/2}\frac{1}{6m^3}(s\cdot\partial_X)^3\epsilon_3 && \delta\Phi_3 = \epsilon_3
% \end{align}
% This extra redundancy is enough to make $\Phi_0(X,s)$ and $\Phi_3(X,s)$ satisfy (\ref{eq:hyperFP}) after imposing the equations of motion. We show this explicitly for spin 3 in \LL{appendix A}. We refer to this gauge as the Fierz-Pauli gauge. 

% \sm

% Operators that vanish on-shell make correlation functions vanish after insertions except at coincident points. In particular, correlations functions with $\Phi_3(X,s)$, $\partial_s\cdot\partial_X\Phi_0(X,s)$, or $\partial_s^2\Phi_0(X,s)$ inserted vanish except at coincident points in the Fierz-Pauli gauge. In momentum space then, the correlation functions in this gauge must take the form
% \begin{align}
%     \langle\Phi_0(p,s)\tilde{\Phi}_0(-p,t)\rangle &= \frac{-i}{p^2+m^2-i\epsilon}F_n(s^2t^2,s\cdot t,s\cdot p\, t\cdot p, p^2) + \text{polynomial} \\
%      \langle\Phi_0(p,s)\tilde{\Phi}_3(-p,t)\rangle &= \text{polynomial} \\
%       \langle\Phi_3(p,s)\tilde{\Phi}_0(-p,t)\rangle &= \text{polynomial} \\
%       \langle\Phi_3(p,s)\tilde{\Phi}_3(-p,t)\rangle &= \text{polynomial}
% \end{align}
% where ``polynomial" refers to some polynomial in $p$, $s$, and $t$ of a degree consistent with the corresponding correlation function. The function $F_n(s^2t^2,s\cdot t,s\cdot p\, t\cdot p, p^2)$ is also separately a polynomial of degree $n$ in $p$, $s$, and $t$, but with the property that $p\cdot\partial_s F_n$, $p\cdot\partial_t F_n$, $\partial_s^2 F_n$, and $\partial_t^2 F_n$ are proportional to $p^2+m^2$. There is a unique solution to $F_n$ modulo terms proportional to $p^2+m^2$, which is
% \begin{align}
%     F_n = N\frac{1}{(\frac{d-3}{2})_n}\Big(-\frac{i}{2}\sqrt{s\cdot P \cdot s \,t\cdot P\cdot t}\Big)^n C_n^{\frac{d-3}{2}}\Big(\frac{s\cdot P\cdot t}{\sqrt{s\cdot P \cdot s\, t\cdot P\cdot t}}\Big)
% \end{align}
% where $P_{\mu\nu}=\eta_{\mu\nu}+p_{\mu}p_{\nu}/m^2$. The normalization constant $N$ is at this point undetermined, but we will find that $N=1$.


% \subsection{Non-minimal coupling to electromagnetic background}
% \begin{align}
%     &\partial_s\cdot\partial_X\mathcal{F}_0-\frac{1}{2}s\cdot\partial_X\partial_s^2\mathcal{F}_0 + \frac{i}{2}s\cdot\partial_X\mathcal{F}_2 + i^{1/2}m\mathcal{F}_1 = s^2\mathcal{K}_0   \\
%     &\partial_s\cdot\partial_X\mathcal{F}_1 - \frac{1}{2}s\cdot\partial_X\partial_s^2\mathcal{F}_1 + \frac{i}{2}s\cdot\partial_X\mathcal{F}_3 +\frac{1}{2}i^{-1/2}m\partial_s^2\mathcal{F}_0 +\frac{1}{2}i^{1/2}m\mathcal{F}_2 = s^2\mathcal{K}_1 \\
%     &\partial_s\cdot\partial_X\mathcal{F}_2 +\frac{1}{2}s\cdot\partial_X\partial_s^2\mathcal{F}_2 + \frac{i}{2}s\cdot\partial_X\partial_s^4\mathcal{F}_0 + i^{-1/2}m\partial_s^2\mathcal{F}_1 = -is^2\partial_s^2\mathcal{K}_0 + 2i\mathcal{K}_0 \\
%     &\partial_s\cdot\partial_X\mathcal{F}_3 + \frac{1}{2}s\cdot\partial_X\partial_s^2\mathcal{F}_3 + \frac{i}{2}s\cdot\partial_X\partial_s^4\mathcal{F}_1 -\frac{1}{2}i^{1/2}m\partial_s^4\mathcal{F}_0 +\frac{1}{2}i^{-1/2}m\partial_s^2\mathcal{F}_2 = -is^2\partial_s^2\mathcal{K}_1 + 6i\mathcal{K}_1
% \end{align}
% \subsection{Non-minimal coupling to gravitational background}

% $$[\partial_s\cdot\nabla,s\cdot\nabla] = \nabla^2-s^{\mu}R_{\mu\nu}\partial_{s}^{\nu}+s^{\nu}s^{\sigma}R_{\omega\sigma\mu\nu}\partial_s^{\mu}\partial_s^{\omega}$$
% $$[\partial_s\cdot\nabla,\nabla^2] = -\nabla_{\mu}R^{\mu\nu}\partial_{s\nu}-R^{\mu\nu}\partial_{s\mu}\nabla_{\nu}+s^{\lambda}\nabla^{\omega}R^{\mu\;\;\nu}_{\;\;\lambda\;\;\omega}\partial_{s\mu}\partial_{s\nu}+2s^{\lambda}R^{\mu\;\;\nu\omega}_{\;\;\lambda}\partial_{s\mu}\partial_{s\nu}\nabla_{\omega}$$
...
The formulation of massive particles exhibits a gauge symmetry (\ref{eq:Stueck}), (\ref{eq:halfStueck}). Because of this, we were able to arrive at propagators which grow like $\sim p^{-2}$, and $\sim p^{-1}$, respectively, at the cost of including unphysical polarizations. Because of the gauge invariance, there must exist a gauge which only includes physical polarizations. Indeed, the gauge symmetry may be used to set $\Phi_{n-1}=0$, $\Phi_{n-2}=0$, and $\Psi_{n-1}=0$ before imposing the equations of motion, so that in these gauges only $\langle\Phi_n\tilde{\Phi}_n\rangle$, $\langle\Phi_n\tilde{\Phi}_{n-3}\rangle$, $\langle\Phi_{n-3}\tilde{\Phi}_{n-3}\rangle$, and $\langle\Psi_n\overline{\Psi}_n\rangle$, $\langle\Psi_n\overline{\Psi}_{n-2}\rangle$, $\langle\Psi_{n-2}\overline{\Psi}_{n-2}\rangle$ are nonzero. Furthermore, as described in \autoref{sec:eqnint} and \autoref{sec:eqnhalf}, after the equations of motion are imposed, $\Phi_{n-3}$ and $\Psi_{n-2}$ may be set to zero, and $\Phi_n$ and $\Psi_n$ are Fierz-Pauli and Rarita-Schwinger systems (\ref{eq:hyperFP}), (\ref{eq:hyperRaritaSchwinger}), respectively. Operators which vanish on-shell make correlation functions vanish after insertions except at coincident points. There must then be a gauge in which correlations functions with $\Phi_{n-3}$, $(\partial_X^2-m^2)\Phi_n$ , $\partial_s\cdot\partial_X\Phi_n$, $\partial_s^2\Phi_n$, $\Psi_{n-2}$,  $(\fsl{\partial}_X+m)\Psi_n$, or $\fsl{\partial}_s\Psi_n$ inserted vanish except at coincident points. In momentum space, this forces the nonzero correlation functions to be of the form
\begin{align}
\label{eq:unitaryint}
    ...
    &\langle\Phi_n(p,s)\tilde{\Phi}_{n-3}(-p,t)\rangle = \text{polynomial} \\
    &\langle\Phi_{n-3}(p,s)\tilde{\Phi}_{n-3}(-p,t)\rangle = \text{polynomial} \\
...
The author would like to thank Lucile Cangemi, Henrik Johansson, Callum Jones, Paolo Pichini, Massimo Porrati, and Radu Roiban for sharing useful references. The author would also like to thank Amey Gaikwad, Trevor Scheopner, and E.T. Tomboulis for helpful discussions in the preparation of this work. This research is supported in part by the Mani L. Bhaumik Institute for Theoretical Physics.
