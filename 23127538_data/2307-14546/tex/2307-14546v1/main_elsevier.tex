...
\newcommand{\commagenta}[1]{\textcolor{magenta}{#1}}
...
\title{Corrections, Improvements, and Comments on Some \textit{Gradshteyn and Ryzhik} Integrals}
...
\author{Robert C. Elliott\corref{cor1}}
\ead{rce@ualberta.ca}

\author{Witold A. Krzymie\'{n}}
\ead{krzymien@ualberta.ca}

%\author[affil2]{Ivo Maljevi\'{c}}

\address{Department of Electrical and Computer Engineering, University of Alberta, \\Edmonton, AB  T6G~1H9, Canada}

\cortext[cor1]{Corresponding author}

%\affiliation[affil1]{organization={Department of Electrical and Computer Engineering},%Department and Organization
%            addressline={University of Alberta}, 
%            city={Edmonton},
%            postcode={T6G~1H9}, 
%            state={AB},
%            country={Canada}}
...
\begin{keyword}
Table of functions \sep Definite integrals \sep Exponential and trigonometric functions \sep Hypergeometric function ${}_0F_1$
...
\begin{align}\label{eq:sin_int_old}
...
\begin{equation}\label{eq:f_manip}
    f = 2\pi (X^{1/2})^{m\,} (\overline{Y}^{1/2})^{m\,} (\overline{Y}^{1/2})^{-m\,} (Y^{1/2})^{-m\,}  (Y^{1/2})^{m\,} (X^{1/2})^{m\,} \sum_{j = 0}^{\infty} \frac{(X^{1/2})^{2j}\, (Y^{1/2})^{2j}}{j!\,(j+m)!}
\end{equation}
...
The reader may have intuited the source of the error in the derivation by this point. Perhaps surprisingly, the error is not itself in breaking apart terms into products of square roots in (\ref{eq:f_manip}), though the act of doing so is eventually what leads to the error. Rather, the error is in the recombination of terms in (\ref{eq:f_error_combine}). In general, raising a complex number to a power is not well-defined, as the result may be multi-valued\footnote{This is experienced even with purely real values. For instance, it is well known that $x^{1/2}$ in fact has two valid solutions: the positive and negative square roots.}. A unique and/or principal value for $z^{\alpha}$ can be defined in two cases: 1) if $z$ is complex, $\alpha$ is an integer, and $z \neq 0$ if $\alpha \leq 0$, then $z^{\alpha}$ is single-valued; 2) if $z$ is positive real and $\alpha$ is complex, then $z^{\alpha} = e^{\alpha \log z}$. Unfortunately, neither of these cases may apply in general for this problem.
...
\subsection{Case 1: When $X^{m/2\,} \smash{\overline{Y}}^{m/2} \neq (X\overline{Y})^{m/2}$}\label{subsec:case1}
...
\subsection{Case 2: When $X^{1/2\,} {Y}^{1/2} \neq \sqrt{XY}$}\label{subsec:case2}
...
\subsection{Case 3: When $Y^{-m/2\,} \smash{\overline{Y}}^{-m/2} \neq (Y\overline{Y})^{-m/2}$}\label{subsec:case3}
...
\begin{equation}\label{eq:new_result_pt1}
    f = 2\pi X^m \sum_{j = 0}^{\infty} \frac{X^{j}\, Y^{j}}{j!\,\Gamma(j{+}m{+}1)} = \frac{2\pi X^m}{\Gamma(m{+}1)} \sum_{j = 0}^{\infty} \frac{X^{j\,} Y^{j\,} \Gamma(m{+}1)}{j!\,\Gamma(j{+}m{+}1)} = \frac{2\pi X^m}{m!} \sum_{j = 0}^{\infty} \frac{X^{j}\, Y^{j}}{j!\,(m{+}1)_j}
\end{equation}
where $(z)_j = \Gamma(z{+}j)/\Gamma(z)$ denotes the Pochhammer symbol. Since $j$ is a non-negative integer, $X^j$ and $Y^j$ each have a single value, and their product will equal\footnote{As a simple proof, $X^j Y^j = \underbrace{X{\cdot}X{\cdot}X{\cdot}{\ldots}{\cdot}X}_{j\textrm{ copies}} \cdot \underbrace{Y{\cdot}Y{\cdot}Y{\cdot}{\ldots}{\cdot}Y}_{j\textrm{ copies}} = \underbrace{XY{\cdot}XY{\cdot}XY{\cdot}{\ldots}{\cdot}XY}_{j\textrm{ copies}} = (XY)^{\hspace{1pt}j}.$} $(XY)^{\hspace{1pt}j}$. Replacing $X^{j\,} Y^j$ with $(XY)^{\hspace{1pt}j}$ in the last sum of (\ref{eq:new_result_pt1}), it can be seen that the sum is, by definition, the (confluent) hypergeometric function\footnote{This is a special case of the generalized hypergeometric function ${}_pF_q(a_1,a_2,\ldots,a_p;b_1,b_2,\ldots,b_q;z) = \sum\limits_{k=0}^{\infty} \frac{(a_1)_k (a_2)_k \cdots (a_p)_k}{(b_1)_k (b_2)_k \cdots (b_q)_k}\frac{z^k}{k!}$ (NIST \cite[Eq.~16.2.1]{NIST_DLMF}), with $p=0$ parameters in the numerator and $q = 1$ parameters in the denominator. (These are not the same ``$p$'' and ``$q$'' as otherwise used in this paper.)} ${}_0F_1(;m{+}1;XY)$. Therefore, we end up with:
...
\end{equation}
We also note that, from the power series definition of ${}_0F_1$ and the property $\overline{z^{\alpha}} = \overline{z}^{\hspace{1pt}\alpha}$ for $\alpha \in \mathbb{R}$, it follows that ${}_0F_1(;b_1;\overline{z}) = \overline{{}_0F_1(;b_1;z)}$ for $b_1 \in \mathbb{R}$. Using $(f{-}\overline{f})/(2i)$ and $(f{+}\overline{f})/2$, we ultimately arrive at the new expressions for the integrals:
\begin{align}\label{eq:sin_int_new}
&\int_{0}^{2\pi} \exp(p \cos x + q \sin x) \sin(a \cos x + b \sin x - mx) \, dx \nonumber \\
&\quad= \frac{i\pi}{m!} \big[(A^{\prime} - iB^{\prime})^{m\,} {}_0F_1(; m{\,+\,}1; C^{\prime} - iD^{\prime}) - (A^{\prime} + iB^{\prime})^{m\,} {}_0F_1(; m{\,+\,}1; C^{\prime} + iD^{\prime})\big]
\end{align}
\begin{align}\label{eq:cos_int_new}
&\int_{0}^{2\pi} \exp(p \cos x + q \sin x) \cos(a \cos x + b \sin x - mx) \, dx \nonumber \\
&\quad= \frac{\pi}{m!} \big[(A^{\prime} - iB^{\prime})^{m\,} {}_0F_1(; m{\,+\,}1; C^{\prime} - iD^{\prime}) + (A^{\prime} + iB^{\prime})^{m\,} {}_0F_1(; m{\,+\,}1; C^{\prime} + iD^{\prime})\big]
\end{align}
...
& \hspace{90pt} {}- \exp\!\Big({-}i\big[(a_R{+}i\hspace{1pt}a_I) \cos x + (b_R{+}i\hspace{1pt}b_I) \sin x - mx\big]\Big) \Big\} \, dx \nonumber \\
&=\frac{i}{2}\underbrace{\int_{0}^{2\pi} \exp\!\Big(\big[(p_R{+}a_I)+i\hspace{1pt}(p_I{-}a_R)\big] \cos x + \big[(q_R{+}b_I)+i\hspace{1pt}(q_I{-}b_R)\big] \sin x\Big)e^{+imx} \, dx}_{f_1} \nonumber \\ 
&\quad - \frac{i}{2}\underbrace{\int_{0}^{2\pi} \exp\!\Big(\big[(p_R{-}a_I)+i\hspace{1pt}(p_I{+}a_R)\big] \cos x + \big[(q_R{-}b_I)+i\hspace{1pt}(q_I{+}b_R)\big] \sin x\Big)e^{-imx} \, dx}_{f_2} \nonumber \\ 
& = \frac{i}{2} (f_1 - f_2)
\end{align}
...
\begin{align}\label{eq:sin_int_cplx} %note: remove the scalebox in the final version, and change \quad to \qquad
&\int_{0}^{2\pi} \exp(p \cos x + q \sin x) \sin(a \cos x + b \sin x - mx) \, dx \quad \scalebox{0.9}{$[p, q, a, \textrm{and } b \textrm{ complex-valued}]$}\nonumber \\
&\quad= \frac{i\pi}{m!} \big[(A_1 + iB_1)^{m\,} {}_0F_1(; m{\,+\,}1; C_1 + iD_1) - (A_2 + iB_2)^{m\,} {}_0F_1(; m{\,+\,}1; C_2 + iD_2)\big]
\end{align}
\begin{align}\label{eq:cos_int_cplx}
&\int_{0}^{2\pi} \exp(p \cos x + q \sin x) \cos(a \cos x + b \sin x - mx) \, dx \quad \scalebox{0.9}{$[p, q, a, \textrm{and } b \textrm{ complex-valued}]$} \nonumber \\
&\quad= \frac{\pi}{m!} \big[(A_1 + iB_1)^{m\,} {}_0F_1(; m{\,+\,}1; C_1 + iD_1) + (A_2 + iB_2)^{m\,} {}_0F_1(; m{\,+\,}1; C_2 + iD_2)\big]
\end{align}
where
...
\subsection{Integral 1 (Gradshteyn and Ryzhik \cite[Eq.~3.931~4]{gr14}):}\label{subsec:3.931_4}\vskip-\baselineskip
...
\subsection{Integrals 2 and 3 (Gradshteyn and Ryzhik \cite[Eqs.~3.932~1 and 3.931~2]{gr14}):}\label{subsec:3.932_1_2}\vskip-\baselineskip
\begin{equation}\label{eq:3.932_1}
\int_{0}^{\pi} e^{p^{\prime} \cos x} \sin(p^{\prime} \sin x) \sin mx \, dx = \frac{1}{2} \int_{0}^{2\pi} e^{p^{\prime} \cos x} \sin(p^{\prime} \sin x) \sin mx \, dx = \frac{\pi p^{\prime \, m}}{2\,m!}
\end{equation}
\begin{equation}\label{eq:3.932_2}
\int_{0}^{\pi} e^{p^{\prime} \cos x} \cos(p^{\prime} \sin x) \cos mx \, dx = \frac{1}{2} \int_{0}^{2\pi} e^{p^{\prime} \cos x} \cos(p^{\prime} \sin x) \cos mx \, dx = \frac{\pi p^{\prime \, m}}{2\,m!}
\end{equation}
...
\subsection{Integral 4 (Gradshteyn and Ryzhik \cite[Eq.~3.936~1]{gr14}):}\label{subsec:3.936_1}\vskip-\baselineskip
...
\subsection{Integrals 5 and 6 (Gradshteyn and Ryzhik \cite[Eqs.~3.936~2 and 3.936~3]{gr14}):}\label{subsec:3.936_2_3}\vskip-\baselineskip
...
\begin{equation}\label{eq:3.936_2_pt1}
    (-1) \frac{i\pi}{m!} \big[(0+ip^{\prime})^m {}_0F_1(;m{+}1;0) - (0 - ip^{\prime})^m {}_0F_1(;m{+}1;0)\big] = \frac{\pi}{i\,m!} \big[(ip^{\prime})^m - (-ip^{\prime})^m \big]
\end{equation}
...
\begin{aligned}
    (-1) \frac{i\pi}{m!} \big[(-p_{I}^{\prime}+ip_{R}^{\prime})^m  - (p_{I}^{\prime} - ip_{R}^{\prime})^m \big] &= \frac{\pi}{i\,m!} \Big(\big[i(p_{R}^{\prime} + ip_{I}^{\prime})\big]^m - \big[(-i)(p_{R}^{\prime} + ip_{I}^{\prime})\big]^m \Big) \\
    &= \frac{\pi}{i\,m!} \big[(ip^{\prime})^m - (-ip^{\prime})^m \big]
\end{aligned}
\end{equation}
...
\subsection{Integral 7 (Gradshteyn and Ryzhik \cite[Eq.~3.936~4]{gr14}):}\label{subsec:3.936_4}\vskip-\baselineskip
...
\subsection{Integrals 8 and 9 (Gradshteyn and Ryzhik \cite[Eqs.~3.937~3 and 3.937~4]{gr14}):}\label{subsec:3.937_3_4}\vskip-\baselineskip
\begin{equation}\label{eq:3.937_3}
\int_{0}^{2\pi}  \!\exp(p \cos x + q \sin x) \sin(q \cos x - p \sin x + mx) \, dx  = \frac{2\pi}{m!} \big( p^{2} + q^{2}\big)^{m/2} \sin\!\left(\! m \atan \frac{q}{p}\right)
\end{equation}
\begin{equation}\label{eq:3.937_4}
\int_{0}^{2\pi}  \!\exp(p \cos x + q \sin x) \cos(q \cos x - p \sin x + mx) \, dx  = \frac{2\pi}{m!} \big( p^{2} + q^{2}\big)^{m/2} \cos\!\left(\! m \atan \frac{q}{p}\right)
\end{equation}
...
\end{align}
\end{subequations}
Gradshteyn and Ryzhik \cite[Eq.~3.937~4]{gr14} may be expressed correctly in the same way, using $\cos$ instead of $\sin$ in (\ref{eq:3.937_3_correct_v1}) and (\ref{eq:3.937_3_correct_v2}).
...
\begin{subequations}\label{eq:3.937_3_cmplx} %note: remove the scalebox in the final version
\begin{align}
&\int_{0}^{2\pi}  \exp(p \cos x + q \sin x) \sin(q \cos x - p \sin x + mx) \, dx \qquad \scalebox{0.9}{$[p \textrm{ and } q \textrm{ complex-valued}]$} \nonumber \\
&\qquad = \frac{i \pi}{m!} \Big(\big[(p_R{+}q_I){\,+\,}i(p_I{-}q_R) \big]^m - \big[(p_R{-}q_I){\,+\,}i(p_I{+}q_R) \big]^m \Big) \label{eq:3.937_3_cmplx_v1}\\
&\qquad = \frac{i \pi}{m!} \Big(\big[(p_R{+}q_I)^2{\,+\,}(p_I{-}q_R)^2 \big]^{m/2} e^{i\,m \arg[(p_R+q_I)+i(p_I-q_R)]} \nonumber \\[-5pt]
&\hspace{72pt} - \big[(p_R{-}q_I)^2{\,+\,}(p_I{+}q_R)^2 \big]^{m/2} e^{i\,m \arg[(p_R-q_I)+i(p_I+q_R)]} \Big) \label{eq:3.937_3_cmplx_v2} \\
&\qquad = \frac{i \pi}{m!} \big[(p-iq)^m - (p+iq)^m \big] \label{eq:3.937_3_cmplx_v3}
\end{align}
\end{subequations}
...
\begin{subequations}\label{eq:3.937_4_cmplx} %note: remove the scalebox in the final version
\begin{align}
&\int_{0}^{2\pi}  \exp(p \cos x + q \sin x) \cos(q \cos x - p \sin x + mx) \, dx \qquad \scalebox{0.9}{$[p \textrm{ and } q \textrm{ complex-valued}]$} \nonumber \\
&\qquad = \frac{\pi}{m!} \Big(\big[(p_R{+}q_I){\,+\,}i(p_I{-}q_R) \big]^m + \big[(p_R{-}q_I){\,+\,}i(p_I{+}q_R) \big]^m \Big) \label{eq:3.937_4_cmplx_v1}\\
&\qquad = \frac{\pi}{m!} \Big(\big[(p_R{+}q_I)^2{\,+\,}(p_I{-}q_R)^2 \big]^{m/2} e^{i\,m \arg[(p_R+q_I)+i(p_I-q_R)]} \nonumber \\[-5pt]
&\hspace{72pt} + \big[(p_R{-}q_I)^2{\,+\,}(p_I{+}q_R)^2 \big]^{m/2} e^{i\,m \arg[(p_R-q_I)+i(p_I+q_R)]} \Big) \label{eq:3.937_4_cmplx_v2} \\
&\qquad = \frac{\pi}{m!} \big[(p-iq)^m + (p+iq)^m \big] \label{eq:3.937_4_cmplx_v3}
\end{align}
\end{subequations}
...
\begin{itemize}
    ...
    \item \cite[Eq.~3.936~4]{gr14}: The integrated equation may be generalized to $e^{p \cos x} \sin(p\sin x \linebreak[0]\pm mx)$; the same result of 0 will be obtained.
...
%This work was supported in part by the Natural Sciences \& Engineering Research Council (NSERC) of Canada and Huawei Technologies Canada Co. Ltd.
