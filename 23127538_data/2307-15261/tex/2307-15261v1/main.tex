...
\title{
%Fibrational Partition Refinement with Optimisation \`{a} la Hopcroft
Explicit Hopcroft's Trick in Categorical Partition Refinement
}
\author{Takahiro Sanada}{RIMS, Kyoto University, Japan \and \url{https://www.kurims.kyoto-u.ac.jp/~tsanada/}}{tsanada@kurims.kyoto-u.ac.jp}{https://orcid.org/0000-0003-3409-6963}{}%{[funding]}
\author{Ryota Kojima}{RIMS, Kyoto University, Japan}{kojima@kurims.kyoto-u.ac.jp}{[orcid]}{}%{[funding]}
\author{Yuichi Komorida}{National Institute of Informatics \& SOKENDAI, Tokyo, Japan}{komorin@nii.ac.jp}{https://orcid.org/0000-0002-3371-5243}{}%{[funding]}
\author{Koko Muroya}{RIMS, Kyoto University, Japan}{kmuroya@kurims.kyoto-u.ac.jp}{https://orcid.org/0000-0003-1760-9886}{}%{[funding]}
\author{Ichiro Hasuo}{National Institute of Informatics  \& SOKENDAI, Tokyo, Japan}{i.hasuo@acm.org}{https://orcid.org/0000-0002-8300-4650}{}%{[funding]}

\authorrunning{
T. Sanada, R. Kojima, Y. Komorida, K. Muroya and I. Hasuo
} %TODO mandatory. First: Use abbreviated first/middle names. Second (only in severe cases): Use first author plus 'et al.'

\Copyright{Takahiro Sanada, Ryota Kojima, Yuichi Komorida, Koko Muroya and Ichiro Hasuo} %TODO mandatory, please use full first names. LIPIcs license is "CC-BY";  http://creativecommons.org/licenses/by/3.0/
...
\keywords{partition refinement, category theory, coalgebra, fibration, tree algorithm} %TODO mandatory; please add comma-separated list of keywords

% \category{} %optional, e.g. invited paper

% \relatedversion{} %optional, e.g. full version hosted on arXiv, HAL, or other respository/website
\acknowledgements{The authors are supported by ERATO HASUO Metamathematics for Systems Design Project (No. JPMJER1603), JST. Y.K.\ is supported by JSPS KAKENHI Grant Number JP21J13334. T.S.\ is supported by JST Grant Number JPMJFS2123. R.K.\ is supported by JST Grant Number JPMJFS2123.}%optional
...
\begin{abstract}
Algorithms for \emph{partition refinement} are actively studied for a variety of systems,
often with the optimisation called \emph{Hopcroft's trick}.
However, the low-level description of those algorithms  in the literature often obscures the essence of
Hopcroft's trick. 
Our contribution  is twofold.  Firstly, we present  a novel formulation of Hopcroft's trick in terms of general trees with weights. This clean and explicit formulation---we call it \emph{Hopcroft's inequality}---is crucially used in our second contribution, namely a general partition refinement algorithm that is \emph{functor-generic} (i.e.\ it works for a variety of systems such as (non-)deterministic  automata and Markov chains). Here we build on recent works on coalgebraic partition refinement but depart from them with the use of \emph{fibrations}. In particular,
our fibrational notion of \emph{$R$-partitioning} exposes a concrete tree structure
to which Hopcroft's inequality readily applies. It is notable that our fibrational framework accommodates such  algorithmic analysis  on  the categorical level of abstraction.
\end{abstract}
...
\cref{fig:Partitioning-intro} illustrates one iteration of our fibrational partition refinement algorithm $\PRHopcroft$ (\cref{algo:fibrational-block-specified}). In the last step (\cref{subfig:iterRefine}), 
the mono-sink $C_{010}, C_{011}, C_{012}\monoto C_{01}$ arises as the $\pull{(c \circ \kappa)}\lift{F}R$-partitioning of $C_{01}$.
In this manner, a tree structure  explicitly emerges in the base category $\catc$.   Hopcroft's inequality directly applies to this tree, allowing us to systematically present the Hopcroft-type optimisation on the categorical level of abstraction.
...
%This is because of the assumptions we need for the correctness of our algorithm. 
...
\begin{itemize}
    ...
    \item Functor-generic partition refinement algorithms $\PRHopcroftEqRel{\wCard}, \PRHopcroftEqRel{\wPred}, \PRHopcroftEqRel{\wReach}$, obtained as instances of  $\PRHopcroft$ but using different weights in Hopcroft's inequality.
...
It also generalises the trick,  accommodating arbitrary \emph{weights} (\cref{def:weight}) besides the particular one
...
\begin{wrapfigure}[8]{r}{0pt}
    ...
    \begin{tikzpicture}[scale=0.6]
        ...
        \pgfmathsetmacro{\rooty}{5}
        \pgfmathsetmacro{\ya}{0.75}
        \pgfmathsetmacro{\yb}{1.5}
        \pgfmathsetmacro{\yc}{2.25}
        % coordinates of the tree
        \coordinate (root) at (\rootx, \rooty) ;
        \coordinate (n0) at (\rootx - 2, \rooty - \ya) ;
        \coordinate (n1) at (\rootx - 0, \rooty - \ya) ;
        \coordinate (n2) at (\rootx + 2, \rooty - \ya) ;
        \coordinate (n00) at (\rootx - 3, \rooty - \yb) ;
        \coordinate (n01) at (\rootx - 2, \rooty - \yb) ;
        \coordinate (n10) at (\rootx - 1, \rooty - \yb) ;
        \coordinate (n11) at (\rootx - 0, \rooty - \yb) ;
        \coordinate (n12) at (\rootx + 1, \rooty - \yb) ;
        \coordinate (n20) at (\rootx + 2, \rooty - \yb) ;
        \coordinate (n200) at (\rootx + 1.5, \rooty - \yc) ;
        \coordinate (n201) at (\rootx + 2.5, \rooty - \yc) ;
        % edges
        \draw[heavy] (root) -- (n0) ;
        \draw[light] (root) -- (n1) ;
        \draw[light] (root) -- (n2) ;
        ...
        \draw[heavy] (n20) -- (n201) ;
        % weight
        \draw[fill=white] (root) circle[radius=3mm];
        \node[circle] at  (root) {$36$} ;
        \draw[fill=white] (n0) circle[radius=3mm];
        \node[circle] at  (n0) {$14$} ;
        \draw[fill=white] (n1) circle[radius=3mm];
        \node[circle] at  (n1) {$14$} ;
        \draw[fill=white] (n2) circle[radius=3mm];
        \node[circle] at  (n2) {$7$} ;
        \draw[fill=white] (n00) circle[radius=3mm];
        \node[circle] at  (n00) {$5$} ;
        \draw[fill=white] (n01) circle[radius=3mm];
        \node[circle] at  (n01) {$7$} ;
        \draw[fill=white] (n10) circle[radius=3mm];
        \node[circle] at  (n10) {$5$} ;
        \draw[fill=white] (n11) circle[radius=3mm];
        \node[circle] at  (n11) {$2$} ;
        \draw[fill=white] (n12) circle[radius=3mm];
        \node[circle] at  (n12) {$3$} ;
        \draw[fill=white] (n20) circle[radius=3mm];
        \node[circle] at  (n20) {$7$} ;
        \draw[fill=white] (n200) circle[radius=3mm];
        \node[circle] at  (n200) {$2$} ;
        \draw[fill=white] (n201) circle[radius=3mm];
        \node[circle] at  (n201) {$4$} ;
    \end{tikzpicture}
    \begin{tikzpicture}[scale=0.6]
        \small
        \pgfmathsetmacro{\rootx}{0}
        ...
        \node[circle] at  (n200) {$2$} ;
        \draw[fill=white] (n201) circle[radius=3mm];
        \node[circle] at  (n201) {$5$} ;
    \end{tikzpicture}
    \caption{
         Examples of rooted trees, each with a weight function and an hcc.
         The heavy children are indicated by thick edges.
        A thin edge represents a light edge.
    }
    \label{fig:weighted-heavy-chosen-tree}
\end{wrapfigure}
Examples 
are in \cref{fig:weighted-heavy-chosen-tree};
the weight on the left is not tight while the right one is tight.
...
We thus force an equality using the following notion of tightening. 
...
\begin{mytheoremrep}[Hopcroft's inequality]\label{thm:Hopcroft-ineq}
    ...
    \begin{equation}\label{eq:Hopcroft-ineq}\textstyle
        \sum_{v \in \vertex(T)} \sum_{u \in \lch_h(v)} w(u)
        \;\le\;
...
If there exists $K$ with
$t(v) \le K \sum_{u \in \lch(v)} w(u)$---note that the bound only refers to light children---then the time to generate the whole tree is bounded by $K w(r) \log_2 w(r)$.
...
\begin{mydefinition}
    ...
    are defined by the following pullback and pushout, respectively:
    % \begin{equation*}
...
% Our central view in this paper is that
% $\catc$ is the category of ``state sets'' of state-based systems
% and $\cate$ is the category of ``equivalence relations'' on the states.
...
% The rest of~\cref{subsec:prelimFib} introduces 
\begin{mydefinition}[fibration]
    ...
    for each $R \in \cate$ and $u \colon C \to D$ in $\catc$ with $pR = D$,
...
\begin{wrapfigure}[6]{r}{0pt}
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \draw[->] (B) -- node[left] {$v$} (C) ;
    \end{tikzpicture}
\end{wrapfigure}
The category $\cate$ is called \emph{total category}
and the category $\catc$ is called \emph{base category} of the fibration.
For an object $C \in \catc$, the objects in $\cate$ above $C$ forms a category $\cate_C$,
called \emph{fibre category} above $C$.
...
\begin{myexample}[$\eqrel\to\sets$ is a fibration]\label{ex:eqrelIsAFib}
    ...
    The objects of $\eqrel$ are pair $(S, R)$ of a set $S$ and an equivalence relation $R$ on $S$.
    The morphism $f \colon (S, R) \to (S', R')$ in $\eqrel$ is a function $f \colon S \to S'$
...
\begin{mydefinition}[lifting, fibred lifting]\label{def:lifting}
    ...
    $F \colon \catc \to \catc$
    and its lifting $\lift{F} \colon \cate \to \cate$ is \emph{fibred} if
...
\begin{wrapfigure}[4]{r}{0pt}
    ...
    \end{minipage}
\end{wrapfigure}
When $(F \colon \catc \to \catc, \lift{F} \colon \cate \to \cate)$ is fibred, for $f \colon C \to D$ in $\catc$ and $R \in \cate_D$,
...
\begin{mydefinition}[$\clat$-fibration]
    A fibration $p \colon \cate \to \catc$ is a \emph{$\clat$-fibration} if
    each fibre 
    %category
    $\cate_C$ is a complete lattice and
    each inverse image functor $\pull{u} \colon \cate_D \to \cate_{C}$ preserves meets $\sqcap$.
\end{mydefinition}
For a $\clat$-fibration,
there always exists the left adjoint $\push{u} \colon \cate_{C} \to \cate_{D}$
...
\begin{myexample}
    ...
    \begin{enumerate}
        ...
        \item The distribution functor $\distr$ is defined on a set $X$ to be
            ...
            For a state $x$, $c(x) \in \distr C$ is a probabilistic distribution $C \to [0,1]$,
...
\begin{mydefinition}[bisimulations and the bisimilarity]\label{def:fibrational-bisim}
    ...
    that is an object $R \in \cate_C$ with $R \sqsubseteq \pull{c} \circ \lift{F} (R)$.
...
In the above definition, the choice of $\lift{F}$
determines a notion of bisimulation.
The relation lifting $\rellift{F}$ (\cref{def:relation-lifting}) is often used as a lifting of $F$.
For all the functors we consider,
 % (as in (\ref{eq:functorBNF})),
the bisimilarity wrt.\ $\rellift{F}$
% the logical $F$-bisimilarity 
coincides with the \emph{behavioural equivalence},
another well-known notion of bisimilarity~\cite[\S{}4.5]{Jacobs16}.
% See Section 4.5 of \cite{Jacobs16} for more details.
\begin{myexample}[$(F, \rellift{F})$-bisimilarities]
    We illustrate $(F, \rellift{F})$-bisimilarity
    (also called \emph{logical $F$-bisimilarity}~\cite{Jacobs16})
%\cite[Definition~4.5.1]{Jacobs16})
...
\begin{mydefinition}[$R$-partitioning]
    ...
    Let $C\in \catc$ and $R\in \cate_{C}$. An \emph{$R$-partitioning} is a mono-sink 
    $\{ \kappa_i \colon C_i \monoto C \}_{i \in I}$ to $C$ that satisfies the following conditions.
    \begin{enumerate}
    ...
    \end{enumerate}
 % 
    We say a $\clat$-fibration  $p$ \emph{admits partitioning} if
    % \begin{itemize}
    %  \item
(1)	  for each $C\in \catc$ and $R \in \cate_{C}$,
            there is an $R$-partitioning; 
     % \item
and moreover,
(2) the following \emph{monotonicity} holds: for each $C \in \catc$, $R, R' \in \cate_{C}$ 
%such that
s.t.\ $R' \sqsubseteq R$,
            and each $R$-partitioning $\{ \kappa_i \colon C_i \monoto C \}_{i \in I}$,
            we have
            $\textstyle\bigsqcup_{i\in I} \push{(\kappa_i)} (\pull{\kappa_i} R') = R'$.
    % \end{itemize}
\end{mydefinition}

%In the above definition of $R$-partitioning,
...
when restricted to $C_{i}$, should be trivial (which is false when $C_{i}$ includes a pair that is not $R$-equivalent).
...
\begin{myexample}[$\eqrel \to \sets$ admits partitioning]
 ...
 $\{\kappa_{S}\}_{S\in C/R}$),
    ...
    We can easily check that $\eqrel \to \sets$ satisfies monotonicity.
\end{myexample}
...
that make fibrations well compatible with partitioning.
...
\begin{myassumption}\label{assum:well-compatible-fib}
    Assume a $\clat$-fibration  $p \colon \cate \to \catc$ satisfies the following conditions. 
    \begin{enumerate}
        ...
        \item\label{item:good-fib-fibre-modular} For any monomorphisms $\kappa \colon A \monoto C$ and $\lambda \colon B \monoto C$,
            the third diagram in \cref{fig:well-compatible-fib} is a fork.
    \end{enumerate}
\end{myassumption}
...
%\begin{minipage}{\textwidth}
 \begin{mylemmarep}[termination]\label{lem:naive-termination}
    If $\cate_C$ is a well-founded lattice, then \cref{algo:naive} terminates.
 \end{mylemmarep}
 \begin{proof}
...
%\noindent
%\begin{minipage}{\textwidth}
\begin{mypropositionrep}[correctness of the naive algorithm] \label{prop:naive-algorithm-correct}
...
% and are proposed for the first time for coalgebraic settings.
...
\begin{myassumption}\label{assum}
    \begin{enumerate}
        ...
        \item \label{assum:fibred} $\lift{F}$ is a lifting of $F$ along $p$ and $(F, \lift{F})$ is fibred.
        \item \label{assum:F-preserves-mono} $F\colon \catc \to \catc$ preserves monomorphisms whose codomain is not the initial object $0$.
        \item \label{assum:monotone-partitioning} The fibration $p$ admits monotone partitioning.
        ...
        \item \label{assum:weight} For  $C' \monoto C$,
             $R \in \cate_{C'}$,
            and  $R$-partitioning $\{ \kappa_i \colon C_{i} \monoto C' \}$ of $C'$,
            $\sum_{i = 1}^n w(C_{i}) \le w(C')$.
    \end{enumerate}
\end{myassumption}
...
%\begin{minipage}{\textwidth}
\begin{myproposition}
    Consider the class of endofunctors on $\sets$ defined by the BNF below.
...
\end{myproposition}
%\end{minipage}
% Our leading examples satisfy the assumptions.
...
\begin{mypropositionrep}
    ...
    with the relation lifting $\rellift{F}$ of a functor $F$ defined by (\ref{eq:functorBNF})
...
\begin{algorithm}[t]
    % \caption{A fibrational block-specified partition refinement algorithm}
    \caption{An optimised
        %A leaf-selecting
        fibrational partition refinement algorithm $\PRHopcroft_{(F, \lift{F}), w}$.}
    ...
    \begin{algorithmic}[1]
        ...
        \While{there is $\rho \in \leaves(J)$ such that $C^{\clean}_{\rho} \ne C_{\rho}$} \label{line:loop}
            ...
            \State\label{line:choose-an-index}Choose $k_0 \in \{0, \dots, n_\rho \}$
                such that $\displaystyle w(C_{\rho k_0}) = \max_{k \in \{ 0, \dots, n_{\rho}\}} w(C_{\rho k})$
            \Comment \textbf{Relabelling} (Line~\ref{line:choose-an-index}--\ref{line:relabelling-last-line})
...
\vspace{1mm}
% We will later
% analyse the complexity of the algorithm $\PRHopcroft$ (\cref{algo:fibrational-block-specified}) using Hopcroft's inequality (\cref{sec:Hopcroft-trick}). 
The algorithm $\PRHopcroft$ exposes a tree structure to which Hopcroft's inequality applies.
\cref{table:Hopcroft-ineq-and-blockspecified-algorithm} summarises how constructs in $\PRHopcroft$ fit \cref{sec:Hopcroft-trick}. We use
...
Note that, whereas $\PRNaive$ expands
the tree uniformly so that every leaf has the same depth (\cref{fig:naive-tree}),
...
\begin{table}[t]
    ...
    \caption{(Explicit) correspondence between  constructs in $\PRHopcroft_{(F, \lift{F}), w}$ and the theory in \cref{sec:Hopcroft-trick}}
    ...
    \begin{tabular}{ll}
        ...
        A set $V$ of vertices
        \\
        A set $\{ \kappa_{\sigma , k} \colon C_{\sigma k} \monoto C_{\sigma}\}_{\sigma k \in J}$ of monomorphisms
        &
        A set  $E$  of edges
        \\
        A map $w \colon \obj(\sub(\catc)_C) \to \nat$ with \cref{assum}.\itemref{assum:weight}
        &
        A weight function $w \colon V \to \nat$ (\cref{def:weight})
        \\
...
follows from
\cref{assum}.\itemref{assum:well-founded} and \itemref{assum:inj-on-obj} (\cref{prop:termination}).
Combining 
%the loop invariants and termination,
these,
we  prove the correctness of
$\PRHopcroft$ in
 % \cref{algo:fibrational-block-specified} (\cref{thm:correctness}).
...
\begin{mypropositionrep}[termination]\label{prop:termination}
    \cref{algo:fibrational-block-specified} terminates.
\end{mypropositionrep}
\begin{proof}
...
\begin{myproposition}\label{prop:time-markdirty}
    If each call of $\textsc{MarkDirty}$ in \cref{algo:fibrational-block-specified}
    takes $\order(K \sum_{k = 0}^{n_{\rho}} w(C_{\rho k}))$ time for some $K$,
...
we introduce three examples $\wCard, \wPred, \wReach$,
leading to three functor-generic algorithms $\PRHopcroftER_{(F, \lift{F}), \wCard}$, $\PRHopcroftER_{(F, \lift{F}), \wPred}$ and $\PRHopcroftER_{(F, \lift{F}), \wReach}$.
            ...
            \Procedure{MarkDirty}{$\rho, k_0$}
                ...
                \For{$k \in \{0, \dots, n_{\rho}\} \setminus \{ k_0\}$ and $y \in C_{\rho k}$}\label{line:optimised-markdirty-predecessors}
                    \For{$x$: predecessor of $y$}\label{line:optimised-markdirty-one-predecessor}
                        ...
                        \State\label{line:optimised-mark-a-state-dirty}If such $\tau$ exists, then $C^{\clean}_{\tau} := C^{\clean} \setminus \{ x \}$
...
\begin{mydefinition}[predecessor \cite{JacobsWissmann23}]\label{def:pred}
    ...
    % \begin{equation*}
        ...
        \end{tikzcd}. 
        \quad
        \text{Here $i_{y} \colon C \setminus \{ y \} \monoto C$ is the canonical injection.}$
    % \end{equation*}
\end{mydefinition}


For a weight function $w$ as a parameter of $\PRHopcroftEqRel{w}$, we introduce three functions.
\begin{mydefinition}[weight functions $\wCard,\wPred, \wReach$]\label{def:concreteWeightFunc}
    We define $\wCard,\wPred, \wReach \colon \powset(C) \to \nat$---called the \emph{cardinality}, \emph{predecessor} and \emph{reachability} weights, respectively---as follows: 
    $\wCard(A)=|A|$, 
...
\begin{table}[t]
    ...
    \begin{tabular}{llll}
        ...
        \midrule
        $2 \times X^A$ & $\wCard(A)$, $\wReach(A)$ & DFA & A variation of Hopcroft's algorithm \cite{Hopcroft71,Gries73,Knuutila01}\\
        $\powset(A \times X)$ & $\wCard(A)$ & LTS  & A variation of  \cite{Valmari09}\\
        $\distr(X)$ & $\wCard(A)$ & Markov chain & A variation of \cite{ValmariFranceschinis10} \\
        $\powset(\distr(X))$ & $\wCard(A)$ & Markov decision process & A variation of \cite{Groote+18} \\
        \bottomrule
    \end{tabular}
\end{table}
...
We sketch the proof of \itemref{item:complexityWCard} for illustration. It uses  Hopcroft's inequality
(\cref{thm:Hopcroft-ineq}) in its amortised analysis.
\begin{proofsketch} (of \cref{thm:complexityConcrete}.\itemref{item:complexityWCard})
We can check that 
\cref{algo:eqrel-sets-optimized} (with $w=\wCard$)
satisfies the premise of \cref{prop:time-markdirty}.
...
% \section{Conclusion}
% We proved Hopcroft's inequality (\cref{thm:Hopcroft-ineq})
% and proposed the fibrational coalgebraic partition refinement algorithm (\cref{algo:fibrational-block-specified}).
% We proved the correctness of the algorithm fibrationally, and evaluated the time
% complexity using Hopcroft's inequality.
% Although Hopcroft's inequality formalises a combinatorial, not fibrational,
% argument on rooted trees, it is our fibrational view on partition refinement
% that exposes the tree structure of partition refinement and applicability of
% Hopcroft's inequality.
% The fibrational view provided us the tree structure of partition refinement.
% Hence, we could use Hopcroft's inequality evidently and evaluated the time complexity of our algorithm.
\bibliography{ronbun}
...
% LocalWords:  partitionings Hopcroft Hopcroft's FC monomorphisms Caa
% LocalWords:  ckR cR Caba Cabb Cabc subtree hcc pullfR fx fy pushfR pQ
% LocalWords:  pP ph pR uR fibred FCFC Im RelFR Freyd's iff wrt Cn cond
% LocalWords:  Chevalley cate catc Pudots Pddots lamR kapPi Audots Ai
% LocalWords:  Addots Cdots Ddots Dn th inv assum pushouts inj dr C'eq
% LocalWords:  MarkDirty fibredness C'begin Cbegin Ceq mQbegin Qbegin
% LocalWords:  mQ mQeq Qeq C'additional centric complexityWPred yshift
% LocalWords:  complexityWCard lipicsYellow xshift initialisations gray
% LocalWords:  ronbun
