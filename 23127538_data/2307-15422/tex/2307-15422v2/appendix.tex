...
Similarly to what is done on HPOBench in the previous section we perform the same visualization now on LCBench~\cite{zimmer_auto-pytorch_2020}. We present 1,000 randomly selected learning curves colored by their final ranking in Figure~\ref{fig:all-ranking-curves-lcbench},\ref{fig:all-ranking-curves-lcbench-second-part} and the corresponding heatmaps in Figure~\ref{fig:all-heatmap-lcbench},\ref{fig:all-heatmap-lcbench-second-part}. In such visualization, we can identify clearly two groups among learning curves (good and bad candidates). Also we can notice that it is possible to identify good candidates only based on the first epoch.
...
Similarly to what is done on LCBench in the previous section we perform the same visualization now on YAHPO~\cite{pfisterer_yahpo_2022}. We present the same 1,000 randomly selected learning curves as in the previous Section~\ref{sec:appendix-viz-learning-curves-yahpo}, colored by their final ranking in Figure~\ref{fig:all-ranking-curves-yahpo},\ref{fig:all-ranking-curves-yahpo-second-part} and the corresponding heatmaps in Figure~\ref{fig:all-heatmap-yahpo},\ref{fig:all-heatmap-yahpo-second-part}. In such visualization, we can identify clearly two groups among learning curves (good and bad candidates). Also we can notice that it is possible to identify good candidates only based on the first epoch. From this results, it is clear that the 1-Epoch baseline would be performing similarly well on a majority of these problems. Now, LCBench only considers tabular datasets. Therefore, within YAHPO we look at NB301~\cite{siems2020bench} based on Cifar-10 (computer vision dataset) from \cite{krizhevsky2009learning}, the only scenario (other than the LCBench scenario) using epochs as the fidelity. We present the results in Fig.~\ref{fig:nb301-cifar10-yahpo}. From the heatmap, it is clear that the best configuration is already dominating bad configurations from the first epoch. Therefore, 1-Epoch would also perform well on this benchmark.
...
\section{Visualization of Learning Curves from JAHS-BENCH-201}~\label{sec:appendix-viz-learning-curves-JAHSBENCH}

Similarly to was done on HPOBench, LCBench and YAHPO in the previous section we perform the diagnostic visualization now on JAHS-Bench-201~\cite{bansal2022jahs}. We present 1,000 randomly selected learning curves as done before, colored by their final ranking in Figure~\ref{fig:all-ranking-curves-jahsbench} and the corresponding heatmaps in Figure~\ref{fig:all-heatmap-jahsbench}. Same conclusions as previous benchmarks can be derived. The best curves are dominant over the red curves. This can be seen well in the top row of the heatmaps Figure~\ref{fig:all-heatmap-jahsbench} which remain blue from the start to the end.
...
\section{More Details About HPOBench Search Space}
...
% In our experiments, we observed learning curves where steps corresponds to incremental training epochs. Therefore the mapping between steps and budget (i.e., epochs) is the identity function $b(z) = z$ where $b$ is a function modeling the budget allocation. Therefore, we kept only $z$ for simplicity. However, in a setting where at each step the evolving budget is not mapped with identity such as with the number of presented examples, the number of estimators in an ensemble or the number of stochastic gradient steps, $z$ can be replaced by $b(z)$ which is any function mapping each step $z$ to its allocated budget.
 
% We then which to estimate both aleatoric (i.e., noise in the learning curve) and epistemic (i.e., estimation of the model parameters) uncertainty through a Bayesian approach sing Markov Chain Monte Carlo (MCMC) inference. Therefore we are interested in the posterior distribution of parameters $P(\Theta|C) \propto P(C|\Theta) \cdot P(\Theta)$. Then, we set the prior on the parameters $P(\Theta) := \mathcal{N}(\hat{\Theta},1)$ from the estimated parameters returned by least square. Then, the likelihood is defined as $P(C|\Theta) := \prod_{z=0}^n \mathcal{N}(y_z;\mathcal{C}(z;\hat{\Theta}, \sigma^2)$ where $\sigma^2 \sim \text{Exp}(1)$. From this probabilistic definition we then perform MCMC to evaluate the $p_\text{worse} := P(\mathcal{C}(z_\text{max};\hat{\Theta}) < y^* | C)$ (l. 18 in Algorithm~\ref{alg:rober-algorithm}) and discard if $\tau \le p_\text{worse}$ with $\tau$ a threshold we should in practice set quite high to avoid discarding models with performance not certainly worse than the current best. In this sense, the method is quite conservative about model early rejecting because early steps will have high overall uncertainty.

% Now that we explained our probabilistic model we give additional details on how we handle earliest steps where $z < |\Theta|$. Indeed, for these steps no least-square estimate is available has we have more degrees of freedom than the number of collected observations. We could remain in an over-parameterized regime and adopt a different optimization approach to alleviate this issue. Instead, we decide to place some safeguard to help us detect specific cases which are stagnation of the learning curve and unexpectedly bad candidates also commonly named "outliers" in data-science. The stagnation in learning curve is managed through a classic early stopping strategy (l. 7 in Algorithm~\ref{alg:rober-algorithm}). Then, the outliers are detected through the simple inter-quantile range methodology, visually we look at the distribution of performances at a given step and consider them outlier when having a lower value than the lower whisker of a box-plot (l. 12-14 in Algorithm~\ref{alg:rober-algorithm}).
...
\subsection{RoBER is More Robust}~\label{sec:appendix-rober-robust}
...
\section{More Details on the PINNs Case Study}~\label{sec:appendix-pinn}

In PINNs~\cite{RAISSI2019686}, we leverage the governing equation as a regularization term to train the network with minimal or no data. The objective of the neural network in this case study was to learn the solution field of a 1-dimensional Burger's equation solely based on the provided initial and boundary conditions. The 1-dimensional Burger's equation is given by Equation~\ref{eq:burgers}:

\begin{equation}~\label{eq:burgers}
    \frac{\partial u}{\partial t} + u\frac{\partial u}{\partial x} = \nu \frac{\partial^2 u }{\partial x^2}
\end{equation}
where $u$ is the velocity field, $t$ is the time variable, $x$ is the space variable, and $\nu$ is the viscosity. We trained a neural network, $f_{w}(x,t)$, parameterized by $w$, to approximate $u$. During training, we only provided the network with the preset initial condition and boundary conditions such as:
...
The training adopted a loss function comprised of two components such as:

\begin{align}
    L(w) = L_{MSE}(w) + \alpha \cdot L_{f}(w) \\
    L_{MSE}(w) = \frac{1}{N}\sum_{i=1}^N (f_{w} - u)^2 \text{~~and~~} L_f(w) &= \left\lVert \frac{\partial f_{w}}{\partial t} + u\frac{\partial f_{w}}{\partial x} - \nu \frac{\partial^2 f_{w} }{\partial x^2} \right\rVert ^2_2
\end{align}

$L_{MSE}(w)$ measures the difference between the network predictions and true solutions for the $N$ points at the domain boundaries while $L_{f}(w)$ encourages the network to follow the known governing equation for the points inside the domain~(collocation points). $\alpha$ controls the relative importance of the two loss terms and is regarded as a hyperparameter in the search problem. With the help of the built-in automatic differentiation of the deep learning frameworks, it is straightforward to calculate the network's first- and second-order derivatives with respect to the inputs. As a result, the network accurately learns the solution without the input-output pairs inside the domain.

In this case study, we randomly sampled $N=100$ points at the domain boundaries and $M=10000$ collocation points for training. We also provided the true solution at the collocation points for validation in the HPO process. At last, after obtaining the best configuration from HPO, we retrained the network and passed the entire domain for testing. We used \texttt{PhiFlow} to generate the true solution of Burger's equation and implemented the neural network in \texttt{Tensorflow}. The HPO and subsequent retraining were done on an Nvidia A100 GPU from the Swing cluster at Argonne National Laboratory Computing Resource Center.
...
\begin{itemize}
    \item Benchmark definitions: \url{https://anonymous.4open.science/r/benchmark-EF9A}
    \item Library containing the core software and algorithms: \url{https://anonymous.4open.science/r/automl-lib-329B}
    \item Organization, execution and analysis of the experiments of the paper: \url{https://anonymous.4open.science/r/experiments-ECF2}
\end{itemize}

The experiments for this paper are located in the \texttt{scalable-bo/experiments/local/dhb/} folder of the \texttt{experiments-ECF2} repository. The main entry point for the execution is \texttt{scalable-bo/src/scalbo/scalbo/search/dbo.py}. Plots and analysis are all located in the \texttt{scalable-bo/notebooks/learning-curves/} folder.