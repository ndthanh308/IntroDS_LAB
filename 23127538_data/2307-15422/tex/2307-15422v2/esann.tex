...
%In this study, we evaluate the effectiveness of early discarding policies in the {\color{blue}inner loop}. For the {\color{red}outer loop}, we simply use random search. For early discarding, we use representative widely used approaches performing {\em denovo} search, i.e., not exploitin g prior knowledge of learning curve typology nor involving a form of meta-learning.
%Two baselines, at each extreme, are considered. One evaluates each HP configuration candidate at maximum fidelity, i.e., when they are fully trained within the overall time budget (100-Epochs), while the other evaluates them at minimum fidelity (after training only 1-Epoch). We show the {\bf effectiveness of the 1-Epoch approach} and characterize the reason for its success. Our results put in a new perspective the results of previous studies having omitted this baseline. We are aware of only one study~\cite{bohdal2023pasha} that included a similar baseline and reported results similar to ours. We {\bf argue for including the 1-Epoch baseline in further MF-HPO benchlarks}. 

\section{Related Work}~\label{sec:related-work}

% Our study focuses on early discarding with ``vertical'' model selection as described in~\cite{mohr_learning_2022}. Suggested candidates are streamed one by one and one should decide how to grow their learning curves such as in Asynchronous Successive Halving~\cite{li_system_2020} (SHA). Some noticeable methods which can be adapted to this setting are:  Hyperband~\cite{li2017hyperband} (HB) which can explore levels of fidelity to differentiate noisy learning curves; Freeze-Thaw Bayesian optimization~\cite{swersky_freeze-thaw_2014} which can pause/continue the training of a model and optimize the hyperparameter configuration; Learning curve extrapolation~\cite{domhan_speeding_2015} (LCE) which can observe early performances and extrapolate future performances to decide if training should continue; FABOLAS~\cite{klein2017fast} which learns correlations of the candidates ranking between levels of fidelity; Bayesian Optimization Hyperband~\cite{falkner_bohb_2018} which embeds Bayesian optimization in Hyperband to sample candidates more efficiently; Learning Curve with Support Vector Regression~\cite{baker_accelerating_2017} which predicts final performance from the configuration and early observations; Learning Curve with Bayesian neural network~\cite{klein_learning_2017} which is similar to the previous one but switching the model with a Bayesian neural network; and trace aware knowledge-gradient~\cite{wu2020practical} which leverages an observed curve to update the posterior distribution of a Gaussian process more efficiently.

Our study focuses on methods, which train only a single model at a time, but keep all check-points for further reference. Early discarding means switching to training a model with another HP configuration, before attaining the maximum number of epochs allowed. A typical example of such strategy, sometimes referred to as ``vertical'' model selection~\cite{mohr_learning_2022} is Asynchronous Successive Halving~\cite{li_system_2020} (SHA). Several methods can be adapted to this setting, including Hyperband~\cite{li2017hyperband}, which can explore different levels of fidelity to differentiate noisy learning curves; 
% Freeze-Thaw Bayesian optimization~\cite{swersky_freeze-thaw_2014}, which can pause and continue the training of a model and optimize the hyperparameter configuration;\
...
% A first set of limitations in previous works is due to assumptions on the learning-curve. For methods based on SHA, it assumes that discarded learning curves do not cross in the future because only the Top-$K$ are allowed to continue at a given step. Therefore, models with a slow start can be discarded (often related to configurations of the search space with slower convergence). This phenomenon is also refered to as ``short-horizon bias''~\cite{wu2018understanding}. For methods based on LCE, it either assumes a ``biased'' parametric model or it is more generic but require a more expensive initialization~\cite{white_how_2021}. A second limitation is that the returned configuration is is often the lowest validation error observed during the search. However, this is very limiting because it does not consider more sophisticated model selection scheme (cross-validation, Hyperband) which is why we added the {\color{orange}selection} block in Figure~\ref{fig:components-hpo}.

% Benchmarks play a crucial role in the design and development of HPO methods. We surveyed benchmarks for learning curves which are all quite recent at the time of writing and keep evolving: HPOBench~\cite{eggensperger_hpobench_2021,klein_tabular_2019}, LCBench~\cite{zimmer2021auto}, JAHS-Bench-201~\cite{bansal2022jahsbench} and YAHPO-Gym~\cite{pfisterer2022yahpo}.
...
%, which is typically related to configurations of the search space with slower convergence.
...
\begin{wrapfigure}{R}{0.45\textwidth}
    ...
    % Each HPO run is performed with 10 different random states and we present the average results along with their standard error. 
    }
...
% as a way to compare methods independently of the time which can be impacted by many factors that are hard to reproduce (such as hardware, concurrent processes, and flaws in implementation). This follows the idea of using the number of evaluated functions when comparing black-box optimization methods (e.g., Bayesian optimization). 
...
In this study, we investigated the optimization of hyperparameters for machine learning models with minimal training steps using MF-HPO agents. We compared various state-of-the-art multi-fidelity policies and discovered that a new simple baseline method, not considered in prior becnhmarks, namely the 1-Epoch strategy assessing candidate configuration at lowest fidelity, performs surprisingly well. This was traced to the fact that good and bad models can be distinguished early in the training process. This phenomenon occurs frequently in benchmarks commonly used in the literature. Therefore, this point to the need to include 1-Epoch in forthcoming benchmarks and eventually designing new harder (possibly more realistic) benchmarks that will defeat it. Our work is limitated to using ``epoch" as a unit of fidelity. While this is convenient and appealing to conduct studies independent of hardware implementation considerations, practical application settings may require considering wall time or other options as units of fidelity.
...
This material is based upon work supported by ANR Chair of Artificial Intelligence HUMANIA ANR-19-CHIA-0022 and TAILOR EU Horizon 2020 grant 952215.
