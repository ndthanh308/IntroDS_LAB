...
\begin{document}
% The paper headers
%\markboth{IEEE Transactions on Signal and Information Processing over Networks (SUBMITTED)}%
...
In lieu of a maximum likelihood estimator that is computationally infeasible beyond uninterestingly simple problems, here we advocate a least-squares (LS) approach~\cite{scheinerman2010rdpg} to obtain
%
...
Entrywise multiplication with $\bbM=\mathbf{1}_N\mathbf{1}_N^\top-\bbI_N$ effectively discards the residuals corresponding to the diagonal entries of $\bbA$. If suitably redefined, the binary mask $\bbM$ may be used for other purposes, such as modeling unknown edges if data are missing. For instance, in a recommender system we typically have the rating of each user over a very limited number of items. This (dis)information may be captured in \eqref{eq:ase_mask} by zeroing out the entries of $\bbM$ corresponding to the unknown edges. 
...
% %\blue{Somewhere while discussing related work, make a clear case about how the ASE problem is different from multidimensional scaling (MDS). Work in this paper should be also related to the literature of scalable algorithms for spectral clustering (that would conceivable need to efficiently compute spectral embeddings of large similarity matrices). We should review this body of work, and if there is anything relevant cite and position our advances in context. Same for the relevant literature on low-rank factorization-based methods for recommendation systems.}
...
For the RDPG embedding problem dealt with here, it follows that if the initial condition $\bbX_0$ is close to the solution of \eqref{eq:ase_mask}, the GD iteration \eqref{eq:gd} converges linearly to $\hbX$~\cite{Chi2019Review,bhojanapalli2016,vu2021icassp}. 
%
\begin{myproposition}
...
\end{myproposition}
%
%See \cite{Chi2019Review} and references therein for a similar version of this proposition. % \ref{prop:convergence}. 
...
In Fig. \ref{fig:complexity} we compare the execution times of the aforementioned methods (i)-(iv) as a function of $N$. For ASE, we use the \texttt{SciPY} optimized implementation of the eigendecomposition in Python, as in state-of-the-art RDPG inference packages such as \texttt{Graspologic}~\cite{graspy}. Our GD and BCD implementations are in pure Python, not optimized for performance.  
...
On the other hand, inspection of Fig. \ref{fig:embeddings_bipartidismo_no_ortogonal} (right) reveals that GD converges to a solution where laws are not aligned with the corresponding party senators. Accordingly, the affinity of parties to their laws is less evident than before. In fact, it appears as if Party 1 is not as supportive of its laws as in the ASE-based visualization. While the input graph is the same for both methods and the total cost $f(\hbX^l,\hbX^r)$ is smaller for the GD method, interpretability is hindered because of the larger ambiguity set in the absence of additional constraints.
\end{myexample}
...
\end{mylemma}
%
Computing the projection to the normal space requires some work. The result is given in the next lemma. 
%
...
Note that $\widetilde{\bbQ}\in \mathcal{M}$, and this decomposition will serve to define a retraction to the manifold in the next proposition.
...
\begin{algorithmic}[1]
   ...
   \STATE Advance and compute retraction \\$\bbX^l_{k+1} = R_{\bbX^l_k}\left( \bbX^l_k - \alpha \grad f_{\bbX^l}(\bbX^l_k,\bbX^r_k)\right)$,\\
   $\bbX^r_{k+1} = R_{\bbX^r_k}\left( \bbX^r_k - \alpha \grad f_{\bbX^r}(\bbX^l_k,\bbX^r_k)\right)$,\\where $\alpha$ is chosen through the Armijo rule
...
\noindent \textbf{Stability of GD estimates.} Let us illustrate this (desirable) behaviour through a numerical example. We borrow the setting and code from~\cite{gallagher2021spectral}. Consider two graph samples drawn from a dynamic SBM with inter-community probability matrices
$$
	\bbP_1 = \begin{psmallmatrix}
		0.08 & 0.02 & 0.18 & 0.10 \\
		0.02 & 0.20 & 0.04 & 0.10 \\
		0.18 & 0.04 & 0.02 & 0.02 \\
		0.10 & 0.10 & 0.02 & 0.06
	\end{psmallmatrix},\:
		\bbP_2 = \begin{psmallmatrix}
		0.16 & 0.16 & 0.04 & 0.10 \\
		0.16 & 0.16 & 0.04 & 0.10 \\
		0.04 & 0.04 & 0.09 & 0.02 \\
		0.10 & 0.10 & 0.02 & 0.06
...
UASE and GD produce embeddings that fulfill both stability requirements i) and ii). However, GD yields a better overall representation for both graphs. %, actually very similar to the independent ASEs (modulo an orthonormal matrix in the second one). 
...
Our idea is instead to proceed as in Remark \ref{rem:warm_restarts}, and warm-restart the GD iterations with the previous time-step's estimate $\hbX_{t-1}$. % (or $\hbX^l_{t-1}$ and $\hbX^r_{t-1}$ for digraphs). 
%
%\begin{myexample}[Stable and precise tracking of an SBM]\label{Ex:stable_tracking} \normalfont 
%
% Furthermore, the figure also displays on the right the result of applying the method described in~\cite{brand2006fast} to re-compute the ASE at the same time steps. This is a classic iterative method that updates the $d$ most significant eigenpairs based on $\mathbf{\Delta}_t=\bbA_t-\bbA_{t-1}$. Note how, similarly to re-computing the ASE at each time-step, a single node may produce significantly different embeddings (i.e.\ rotations). 
%
%% Figure environment removed
%
%Note that, in addition to warm-starting our GD algorithm, we may further leverage the fact that $\bbX_t$ is typically correlated to $\bbX_{t-1}$ to actually improve the estimated embeddings' precision over time. In what follows, and in order to simplify the exposition, we will discuss the undirected case. A directed case is presented in the example below. 
%
