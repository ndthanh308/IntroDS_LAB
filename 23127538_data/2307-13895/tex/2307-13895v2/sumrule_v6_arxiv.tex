...
so that the PMNS matrix in Eq.~\ref{Eq:PMNS_Definition} is given by,
...
\label{solar}
\end{align} 
This relation is easy to understand if we consider only one charged lepton angle to be non-zero, $\theta^e_{12}$ then the third row of the PMS matrix in Eq.~\ref{Ucorr} is unchanged, so the elements $U_{\tau i}$
...
We will consider $n=2.5$, $3$ and $1+\sqrt{6}\approx 3.45$ and their flipped cases.
...
At this point we notice that, in the diagonal charged lepton mass basis where we work,
the PMNS mixing matrix is fully specified by the choice of $n$ and the parameters $m_a/m_b$ and $\eta$.
...
\begin{align}
	...
	\end{array}\right),
\end{align}
where the vector $(\sqrt{\frac{2}{3}},-\sqrt{\frac{1}{3}},\sqrt{\frac{1}{3}})^T$ is the first column of the TB matrix in Equation \eqref{TB} and is then an eigenvector of the neutrino mass matrix with eigenvalue $0$ and it corresponds to the massless neutrino eigenstate. This means that for a generic $n$ we get a TM1 mixing, Equation \eqref{eq:TM1}, where the first column of the TB matrix is preserved and the other two can change. Therefore we can think of the LS as a
special case of the \textit{atmospheric} sum rules for the TB mixing. Since the \textit{atmospheric} sum rules were derived only using the fact that the first column of the TB matrix is preserved all LS implementations also follow the TM1 sum rules in Equation \eqref{eq:TM1}. Once we have noticed this it is clear that $m_{\nu}$ can be block diagonalised using the TB matrix
