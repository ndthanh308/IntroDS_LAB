...
\abstract{In this work, we revisit neutrino mixing sum rules arising from discrete symmetries, and the class of Littlest Seesaw neutrino models. These theoretical models offer predictions for the leptonic CP phase and mixing angles,
and correlations among them, which can be tested in forthcoming neutrino experiments. In particular we study the \textit{solar} neutrino mixing sum rules, arising from charged lepton corrections to Tri-bimaximal (TB), Bi-maximal (BM), Golden Ratios (GRs) and Hexagonal (HEX) neutrino mixing,
...
arising from a version of neutrino Golden Ratio mixing (GRa1), are already excluded at 3$\sigma$, and determine the remaining models allowed by the data. We also consider the highly predictive Littlest Seesaw models, which are a special case of tri-maximal mixing models (TM1), and discuss their prospects.}
...
As was observed, modular symmetry remarkably suggests CSD($1+\sqrt{6}$) $\approx$ CSD($2.45$)~\cite{Ding:2019gof,Ding:2021zbg,deMedeirosVarzielas:2022fbw,deAnda:2023udh}.

In this paper we shall revisit neutrino \textit{solar} and \textit{atmospheric} mixing sum rules arising from discrete symmetries, and also discuss the class of Littlest Seesaw models. These theoretical models offer predictions for the leptonic CP phase and mixing angles,
and correlations among them, which can be tested in forthcoming neutrino experiments. In particular we study the \textit{solar} neutrino mixing sum rules, arising from charged lepton corrections to TB, BM and GR neutrino mixing,
and \textit{atmospheric} neutrino mixing sum rules, arising from preserving one of the columns of these types of mixing, for example the first or second column of the TB mixing matrix (TM1 or TM2),
...
arising from a Golden Ratio mixings are already excluded at 3$\sigma$ a part from GRa2, and determine the remaining models allowed by the data. We also consider the highly predictive Littlest Seesaw models, which are a special case of tri-maximal mixing models (TM1), and discuss their prospects.
...
\end{align}
where again $\omega=e^{i 2 \pi / 3}$ and the sign of the $U$ matrix corresponds to the two different triplet representation. The group $S_4$ predicts a TB mixing \cite{Harrison:2002er}, see Figure \ref{fig:tbm}. This can be checked by the fact that $S$ and $U$ are diagonalised by $U_{\mathrm{TB}}$, see Equations \eqref{eq:SandU}.
Another commonly used group is $A_4$, which has two generators $S$ and $U$ that follow the same presentation rules as in Equation \eqref{eq:presentation} and in a standard basis \cite{Altarelli:2005yx}, the generators have the same form as in Equation \eqref{eq:generators-S4}.
...
%
%
For the \textit{solar} sum rules we can obtain a prediction for $\cos \delta$ as we shall show. The elements of the PMNS matrix are related by \cite{Antusch:2007rk,Ballett:2014dua}

\begin{align}
	\frac{|U_{\tau1}|}{|U_{\tau2}|}=\frac{s_{12}^{\nu}}{c_{12}^{\nu}}=t_{12}^{\nu}.
\label{solar}
\end{align} 
This relation is easy to understand if we consider only one charged lepton angle to be non-zero, $\theta^e_{12}$ then the third row of the PMS matrix is unchanged even for $\theta_{13}=0$, so the elements $U_{\tau i}$
may be identified with the corresponding elements in the uncorrected mixing matrix in Eq.\ref{GR}.
...
Let us discuss an approximation of the sum rules for the TB mixing as an example, where $\sin \theta_{12}^{\nu} = 1/\sqrt{3}$. We can re-write Equation \eqref{eq:solar-sum-rules} using the parameters $s$, $a$ and $r$ defined in Equation \eqref{eq:parameters} and then expand in them.  The linearised sum rule reads
\cite{King:2007pr}
...
Let us consider again $G=S_4$ and the TB mixing in Equation \eqref{TB}  as an example. %Its two descendent pattern are given by preserving the first column, trimaximal mixing TM1 or the second colum, TM2. 
...
\begin{align}
	\cos \delta= 	\frac{2 a(2+a) \times\left(-1+r^2\right)}{(1+a) \sqrt{1-2 a-a^2} r \sqrt{4-3 r^2}}.
...
\begin{align}\label{key}
	\cos \delta = - \frac{2a}{r}
\end{align}
and in Figure \ref{fig:linearisation} we test this approximation against the exact sum rules using the experimental constraint in \eqref{eq:exp-data}. We can see that given the updated data the linear approximation is now insufficient to describe the exact expression as it was instead in previous studies \cite{Ballett:2013wya}. Similarly for TM1, as seen in Figure \ref{fig:first-order-2}. This is true for the other model we will discuss later and therefore we provide the higher order expansions that agrees with the exact sum rule in Equation \eqref{eq:TM2-sum-rule} given the current data and is
...
\begin{align}
	...
	\label{eq:RH-masse}
\end{align}
that comes from the Lagrangian term 

\begin{align}
	\mathcal{L}_{\mathrm{LS}} \supset -\frac{1}{2} M_{\mathrm{atm}} \bar{N}_R^{\mathrm{atm}} N_R^{\mathrm{atm}}-\frac{1}{2} M_{\mathrm{sol}} \bar{N}_R^{\mathrm{sol}} N_R^{\mathrm{sol}}+h . c . \;.
\end{align}
The Dirac mass matrix is instead a $3 \times 2$ matrix with arbitrary entries
...
\begin{align}
	...
	\label{eq:Dirac-mass}
\end{align}
that is called constrained dominance sequence (CSD) for positive integer $n$ \cite{King:2005bj,Antusch:2011ic,King:2013iva}. Following the literature we will refer to models with $n$ real as LS models \cite{King:2015dvf}. It has been shown that the reactor angle is \cite{King:2015dvf}

\begin{align}
	\theta_{13} \sim(n-1) \frac{\sqrt{2}}{3} \frac{m_2}{m_3},
\end{align}
therefore this can provide non-zero and positive angle for $n>1$ and also excludes already models with $n \geq 5$ since they do not fit the experimental value. 

The littlest seesaw Lagrangian unifies in one triplet of flavour symmetry the three families of electroweak lepton doublets while the two extra right-handed neutrinos, $\nu_R^{\mathrm{atm}}$ and $\nu_R^{\mathrm{sol}}$ are singlets and reads \cite{King:2015dvf}
\begin{align}
	\mathcal{L}=-y_{\mathrm{atm}} \bar{L} \cdot \phi_{\mathrm{atm}} \nu_R^{\mathrm{atm}}-y_{\mathrm{sol}} \bar{L} \cdot \phi_{\mathrm{sol}} \nu_R^{\mathrm{sol}}-\frac{1}{2} M_{\mathrm{atm}} \nu_R^{\bar{c}}{ }^{\mathrm{atm}} \nu_R^{\mathrm{atm}}-\frac{1}{2} M_{\mathrm{sol}} \nu_R^{\mathrm{c}}{ }^{\mathrm{sol}} \nu_R^{\mathrm{sol}}+ h . c . \; ,
\end{align}
...
\begin{align}
	...
	\label{eq:flipped}
\end{align}
We will refer to the first possibility in Equation \eqref{eq:normal} as the normal case and the second, in Equation \eqref{eq:flipped} as the flipped case. The predictions for $n$ in the flipped case are related to the normal one, as we will discuss later, by  

\begin{equation}
	\tan \theta_{23} \rightarrow \cot \theta_{23} \quad \quad \quad \delta \rightarrow \delta + \pi,
	\label{eq:relation}
\end{equation}
...
\begin{align}
	...
	\label{eq:convention2}
\end{align}
that leads to the same results as the previous two cases respectively. In the neutrino mass matrix there will appear a $(-1)$ factor that is only a non-physical phase that can therefore be neglected. In particular the case $n = 1+\sqrt{6}$ that can be obtained with modular symmetry in \cite{deMedeirosVarzielas:2022fbw} is still $n=1+\sqrt{6}$ in our convention using the Equation \eqref{eq:normal}. 


We will compute everything following the derivation in \cite{King:2015dvf} and using Equation \eqref{eq:normal} and deriving the flipped result with Equation \eqref{eq:relation}.
We will consider $n=2.5$, $3$ and $1+\sqrt{6}$ and their flipped cases.
The mass matrix then in the diagonal charged lepton basis is given by
...
\end{align}
where we used Equations \eqref{eq:seesaw}, \eqref{eq:RH-masse} and \eqref{eq:Dirac-mass} 

 \begin{align}
	m_a=\frac{|e|^2}{M_{\mathrm{atm}}} \quad \quad \quad m_b=\frac{|a|^2}{M_{\mathrm{sol}}},
\end{align} 
and the only relevant phase is $\eta= \arg(a/e)$. We can check that 
...
\begin{align}
	...
	\end{array}\right),
\end{align}
where the vector $(\sqrt{\frac{2}{3}},-\sqrt{\frac{1}{3}},\sqrt{\frac{1}{3}})^T$ is the first column of the TB matrix in Equation \eqref{TB} and is then an eigenvector of the neutrino mass matrix with eigenvalue $0$ and it corresponds to the massless neutrino eigenstate. This means that for a generic $n$ we get a TM1 mixing, Equation \eqref{eq:TM1}, where the first column of the TB matrix is preserved and the other two can change. Therefore we can think of the LS as an generalisation of the \textit{atmospheric} sum rules for the TB mixing. Since the \textit{atmospheric} sum rules were derived only using the fact that the first column of the TB matrix is preserved all LS implementations also follow the TM1 sum rules in Equation \eqref{eq:TM1}. Once we have noticed this it is clear that $m_{\nu}$ can be block diagonalised using the TB matrix
...
\end{align}
Finally we diagonalise $	m_{\text {block }}^\nu$ to obtain a matrix $\operatorname{diag}\left(0, m_2, m_3\right)$
...
\begin{align}
	...
	\end{gathered}
\end{align}
and the angle we use to diagonalise
...
\begin{align}
	...
	\end{array}\right),
\end{align}
with the angle being fully specified by the free parameters $m_a/m_b$ and $\eta$
...
\end{align}
The PMNS matrix is therefore the product of the TB and the $U_{\mathrm{block}}$ matrices

\begin{align}
	U_{\mathrm{PMNS}}=U_{\mathrm{block}}^T U_{\mathrm{TB}}^T.
	\label{eq:LS-PMNS}
\end{align}
The PMNS itself is also fully specified by the choice of $n$ and the parameters $m_a/m_b$ and $\eta$, we can see that using the PMNS matrix parametrisation \eqref{eq:parametrisation} together with Equation \eqref{eq:LS-PMNS}, and we obtain the LS prediction for the neutrino oscillation observables.
...
\begin{align}
	...
	\end{array}\right),
\end{align}
and after diagonalisation we are left with
...
\begin{align}
	\delta=-\arg \left(\operatorname{sign}(t) e^{i \eta}\left(4\left(\sqrt{t^2+1}-1\right)+\left(-2+3 e^{2 i B}\right) t^2\right)\right).
	\label{eq:delta}
\end{align}
Therefore we notice that by just specifying two parameters, the phase $\eta$ and the ratio of the masses $r=m_a/m_b$ and choosing the $n$ that eventually is determined by the choice of flavour symmetry, LS predicts all the neutrino oscillation observables, that is why it is called littlest seesaw.\newline
% Figure environment removed
%

Let us consider $n=3$ and the correspondent flipped case, which was realised successfully via $S_4$ symmetry in \cite{King:2015dvf}. We can plot the constraints on the parameter space given by the experimental ranges of $\theta_{13}$ and the mass ratio $m_2^2/m_3^2$. In Figure \ref{fig:LS} we notice that imposing these two experimental constraint leave only two small allowed parameter regions in the plane $r - \eta$. The allowed range we consider in $r$ and $\eta$ are given by the maximal and minimal values of them in the intersection of the blue and orange bands. Once we have the value of $r$ and $\eta$, thanks to the high predictivity of the model we can derive all the physical parameters and we can test them against the observed values. We do this for different values of $n$ in Table \ref{tab:littlestn3} to \ref{tab:littlest2p5}. We do not present the plot for the flipped case since it is exactly the same. In fact it involves only the mass ration and $\theta_{13}$. This fact can be understood easily studying the parameter $t$ for example in the case $n=1+\sqrt{6}$ and the flipped. In this case going from $n$ to the flipped changes sign of t in Equation \eqref{eq:t-parameter}. The prediction for the mass ratio, $\theta_{13}$, $\theta_{12}$ are independent of this sign while $\theta_{23}$ and $\delta$ are affected by it, we can see this in Equations \eqref{eq:angles} and \eqref{eq:delta}. The predictions as anticipated before are related by $\tan \theta_{23} \rightarrow \cot \theta_{23}$ and $\delta \rightarrow \delta + \pi$. 
%
\begin{table}[t]
	\centering
	\begin{tabular}{||c||c|c||c|c||}
		\hline \hline
		&  $n=3$   &  $\eta = 2.1 \pm 0.3$ &
		$\eta = 4.2 \pm 0.3$  & Exp. range \\ \hline
		...
		& $\theta_{12}\; [^{\circ}]$  & $34.3{}^{+0.4}_{-0.7} $ & $34.3{}^{+0.4}_{-0.7}$  &   $31.27 - 35.86$ \\ \hline \hline
		normal & $\theta_{23}\; [^{\circ}]$  & $46{}^{+4}_{-5} $ & $46{}^{+4}_{-5}$  & $39.5 - 52.0$ \\ \hline
		normal & $\delta\; [^{\circ}]$ & $93^{+12}_{-13} $ & $255^{+12}_{-13}$  &  $0 -45 $ \& $105-360$  \\ \hline \hline
		%$\cos \delta: -1 - 1$ & -0.281489 & 0.525607 & 0.281489 & -0.525607 \\ \hline
		%$\sin \delta: -1 - 0.951056$ & -0.959565 & 0.850728 & 0.959565 & -0.850728 \\ \hline
		flipped & $\theta_{23}\; [^{\circ}]$  & $44{}^{+4}_{-5} $ & $44{}^{+4}_{-5}$  & $39.5 - 52.0$ \\ \hline
		flipped & $\delta\; [^{\circ}]$ & $273^{+12}_{-13} $ & $75^{+12}_{-13}$  &  $0 -45 $ \& $105-360$  \\ \hline \hline
	\end{tabular}
	\caption{3$\sigma$ ranges of the predicted parameters and experimental ranges for $  n = 3$.  With $r = 0.10 \pm 0.02$.
	}
...
In Table \ref{tab:littlestn3} we studied the $n=3$ and its flipped case. We present the theoretical prediction and its uncertainty coming from the allowed region in Figure \ref{fig:LS} and the experimental bound. We notice that $\eta = 2.1$ is still allowed only in the lower part of the $\delta$ parameter space for the normal case and similarly $\eta = 4.2$ for the flipped case.
%
In Table \ref{tab:littlestn1s6} for $n = 1 +\sqrt{6}$, which can be realised with a modular symmetry \cite{deMedeirosVarzielas:2022fbw}, we notice that the region with $\eta=2.40$ is excluded thanks to the experimental bounds on $\delta$ while the one with $\eta=3.88$ is well within the $3\sigma$ range. For the flipped case instead $\eta = 3.88$ is close the lower boundary of the $3\sigma$ region in $\delta$. For $n=2.5$ in Table \ref{tab:littlest2p5} we notice that $\eta = 4.7$ is excluded for normal case while for the flipped both values are allowed in the $3\sigma$ range. This case is also known in the literature as $n=-1/2$ using the convention in Equation \eqref{eq:convention2}. But it is more consistent to refer to it as $n=2.5$ in our notation. And in fact we notice that the LS that are still allowed by the data, $n=2.5$ and $n=1+\sqrt{6}$ have $n \sim 3$ which is the original CDS that worked. 
%
\begin{table}[]
	\centering
	\begin{tabular}{|c||c||c|c||c||}
		\hline \hline
		& $  n = 1+ \sqrt{6}$     &  $\eta = 2.40 \pm 0.04$ &
		$\eta = 3.88 \pm 0.04$  & Exp. range \\ \hline
		& $\theta_{12}\; [^{\circ}]$  & $34.4^{+0.4}_{-0.6} $ & $34.4^{+0.4}_{-0.6}$ &  $31.27 - 35.86$  \\ \hline \hline
		normal & $\theta_{23}\; [^{\circ}]$  & $41.7^{+0.19}_{-0.12}$ & $41.6^{+0.19}_{-0.12}$  & $39.5 - 52.0$  \\ \hline
		normal& $\delta\; [^{\circ}]$ & $75\pm 3$ & $285\pm 3$  & $0 -45 $ \& $105-360$   \\ \hline \hline
		flipped & $\theta_{23}\; [^{\circ}]$  & $48.3^{+0.19}_{-0.12}$ & $48.4^{+0.19}_{-0.12}$  & $39.5 - 52.0$  \\ \hline
		flipped& $\delta\; [^{\circ}]$ & $255 \pm 3$ & $103 \pm 3$  & $0 -45 $ \& $105-360$   \\ \hline \hline
		%$\cos \delta: -1 - 1$ & -0.281489 & 0.525607 & 0.281489 & -0.525607 \\ \hline
		%$\sin \delta: -1 - 0.951056$ & -0.959565 & 0.850728 & 0.959565 & -0.850728 \\ \hline
	\end{tabular}
	\caption{3$\sigma$ ranges of the predicted parameters and experimental ranges for $n = 1 + \sqrt{6}$. With $r = 0.072 \pm 0.004$.
	}
...
\begin{table}[h]
	...
	\begin{tabular}{||c||c||c|c||c||}
		\hline \hline
		 &$  n = 2.5$     &  $\eta = 1.5 \pm 0.4$ &
		$\eta = 4.7 \pm 0.4$  & Exp. range w/o SK  \\ \hline
		%$m_3^2/m_2^2$ & 33.8141 & 33.8141 & 33.8141 & 33.8141  \\ \hline
		%$n$ & 1 + $\sqrt{6}$ & 1 + $\sqrt{6}$ &  1 - $\sqrt{6}$ & 1 - $\sqrt{6}$ \\ \hline
		%$\sin \theta_{13}$ & $0.142494$ & $0.14563$ & 0.142494 & 0.14563 \\ \hline \hline
		 & $\theta_{12}\; [^{\circ}]$  & $35.0 \pm 0.1 $ & $35.0 \pm 0.1$  &   $31.27 - 35.86$ \\ \hline
		normal &$\theta_{23}\; [^{\circ}]$  & $47.0{}^{+3}_{-2} $ & $46.5{}^{+4}_{-2}$  & $39.5 - 52.0$  \\ \hline
	    normal &	$\delta\; [^{\circ}]$ & $289 \pm 11 $ & $76 \pm 11$  &  $0 -45 $ \& $105-360$  \\ \hline \hline
		flipped &$\theta_{23}\; [^{\circ}]$  & $43.0{}^{+3}_{-2} $ & $43.5{}^{+4}_{-2}$  & $39.5 - 52.0$  \\ \hline
        flipped &	$\delta\; [^{\circ}]$ & $109 \pm 11 $ & $256 \pm 11$  &  $0 -45 $ \& $105-360$  \\ \hline \hline
		%$\cos \delta: -1 - 1$ & -0.281489 & 0.525607 & 0.281489 & -0.525607 \\ \hline
		%$\sin \delta: -1 - 0.951056$ & -0.959565 & 0.850728 & 0.959565 & -0.850728 \\ \hline
	\end{tabular}
	\caption{3$\sigma$ ranges of the predicted parameters and experimental ranges for $  n = 2.5$.  With $r = 0.15 \pm 0.02$. 
	}
...
In the past decades many attempts have been made to explain the flavour structure of the PMNS matrix by imposing symmetry on the leptonic Lagrangian. These symmetries imply correlations among the parameters that are called sum rules. We have studied two types of sum rules: \textit{solar} and \textit{atmospheric} sum rules. The former breaks the $T$ generator of a given symmetry group in the charged lepton sector in order to generate a non-zero reactor angle $\theta_{13}$. This leads with prediction for $\cos \delta$ that can be tested against the experimental data. These in turn show a preference for GRa and GRb mixing while BM and GRc are constrained to live in a very small window of the parameter space.

The \textit{atmospheric} sum rules instead come from either the breaking of both $S$ and $U$ in the neutrino sector while preserving $SU$ or by breaking $S$ and preserving $U$. In this case we have two relations among the parameters that can be tested. We noticed that only TM1, TM2 and GRa2 are still allowed by the neutrino oscillation data with a preference for GRa2 and with TM2 very close to be excluded. 

We also studied LS models that follow the constrained sequential dominance idea with $n$ allowed to be real. These models are very predictive with only two free real parameters fixing all the neutrino oscillation observables. It is the most economical framework that explains the neutrino masses that is still compatible with data. Despite the great predictivity and the very small parameter space in the $r$ - $\eta$ plane allowed by the data all the LS examples we considered are still allowed in one part of the parameter space.

Finally, we note that the recent global fits to experimental data have provided significantly improved constraints on all of the above 
models and with future improvement in the neutrino oscillation data we will be able to restrict the pool of viable models still further. In particular advancements in the measurement of the leptonic CP violating Dirac phase $\delta$ can strongly constrain more models in the near future. This is particularly true in LS models where already the experimental bounds on the CP phase is removing the degeneracy in the $r$ - $\eta$ parameter space and given the very precise theoretical prediction can strongly constrain these LS models. Future precision neutrino experiments are of great importance to continue to narrow down the possible PMNS flavour models and lead to a deeper understanding of this part of the flavour puzzle of the SM.
