...
\newcommand{\JO}[1]{\textbf{\color{teal}JO: #1}}
...
The problem of aggregate ranking is of central interest to a broad set of problems in economics, political science, psychology, operations research, and computer science. Given a set of (possibly conflicting) pairwise comparisons of alternatives, how can we determine non-conflicting aggregate pairwise comparisons that best summarize the original data? This question is fundamental to topics as diverse as the analysis of democratic voting systems, the design of online search and recommendation algorithms, and the construction of nonparametric statistical tests.

It is well-known that no preference aggregation method satisfies every potential desirable normative property---e.g., non-dictatorship, independence of irrelevant alternatives, and Pareto efficiency \citep{arrow_social_1951,pini2009aggregating}. Further, approaches that are based on maximizing the alignment of an aggregate ranking with a set of pairwise comparisons face issues related to computational complexity that are driven by the exponential size of the set of possible orderings of alternatives \citep{bartholdi_voting_1989}. As a result, there has been significant interest in applying approaches from optimization and machine learning in order to discover new ranking methods that are both well-justified normatively and computationally efficient \citep[see, e.g.,][]{agarwal_mathematics_2010}.

We develop an efficient optimization-based approach for aggregating preferences that always results in a unique partial order that is based on the logic of eliminating cycles of votes. We represent the set of individuals' pairwise preferences over alternatives, or votes, in a vote graph.  The nodes of the vote graph are the alternatives.  For each pair of alternatives $i$ and $j$, we let $q_{ij}$ denote the number of voters who prefer $i$ to $j$.  An arc $(i, j)$ is in the vote graph provided that $q_{ij}>0$.   The collection of votes are said to be \emph{conflicting} if there is a directed cycle in the vote graph.

If a set of votes is non-conflicting, then there is an induced partial order.  That is, alternative $i$ is preferred to alternative $j$ if and only if there is a directed path from $i$ to $j$ in the vote graph.  This is the smallest partial order that is consistent with the votes.  If the set of votes is non-conflicting, then one can also determine a complete ordering of the alternatives (also known as a full ranking) or a weak ordering (also known as a weak ranking) in which ties are permitted.

Our method eliminates  votes that create inconsistencies in the vote graph.  When the original preference data are non-conflicting, our method recognizes the consistency and eliminates no votes.  When the original preference data is conflicting, our method will eliminate certain votes so that the remaining votes are non-conflicting.

The most well-known vote elimination method is due to \citet{kemeny_mathematics_1959}.  Kemeny’s method is to remove the fewest number of votes so that the remaining set of votes is non-conflicting.  While Kemeny's method has some nice theoretical properties, e.g., it can be justified in terms of minimizing the swap distance from the individual rankings \citep{brandt_rationalizations_2016}, it also suffers important drawbacks. First, Kemeny’s optimization model is equivalent to the NP-hard minimum feedback arc set (FAS) problem \citep{bartholdi_voting_1989}.  Second, there may be alternative (conflicting) optimal solutions. That is, one optimal solution may have alternative $i$ preferred to alternative $j$ while a second optimal solution has alternative $j$ preferred to alternative $i$. This is called the \emph{paradox of Kemeny} \citep{muravyov_dealing_2014, yoo_new_2021}. These problems, which also plague similar preference aggregation methods that are justified by a vote-editing logic, also mean that one cannot efficiently determine whether a candidate ranking is optimal, whether a given set of votes has a unique optimal solution, or whether all of the optimal rankings have been enumerated \citep[see, e.g.,][]{caragiannis_dodgsons_2016}.
...
Our method is to remove cycles of votes from the vote graph and determine aggregate preferences from the remainder. In that sense, following \cite{saari_capturing_2003}, our approach is to remove ties and declare the remaining non-conflicting votes as representing the ``voters' will'' (p. 342). For each pair $i, j$ of alternatives, we refer to the number of votes removed in which $i$ is preferred to $j$ as the flow in $(i, j)$. Our method removes votes that correspond to a flow circulation in the vote graph. That is, for each alternative $i$, the flow out of $i$ is equal to the flow into $j$.  Equivalently, the number of votes removed in which $i$ is preferred to some alternative is equal to the number of votes removed in which some alternative is preferred to $i$. It is well known that any circulation can be decomposed into flows around cycles (see, for example, Chapter 3 of \citet{ahuja_network_1993}).

A necessary condition for this approach is that after the removal of a circulation, the remaining votes are non-conflicting; equivalently, the remaining vote graph is acyclic.  We refer to such circulations as \emph{maximal}.  The arc $(i, j)$ is in the remaining vote graph whenever the maximal circulation sends strictly less than $q_{ij}$ units of flow in $(i, j)$. In this case, we say that there is an \emph{aggregate preference} for $i$ over $j$ following the removal of the circulation, i.e., the tied subset of votes.

There are many possible ways of removing maximal circulations from a vote graph, and thus there are many possible vote graphs that remain after their removal. Our method is to remove a \textit{strong maximum circulation}.  A maximum circulation is a circulation with a maximum total number of votes.  A strong maximum circulation is a maximum circulation whose removal leaves as many aggregate preferences as possible. In other words, the remaining vote graph has as many distinct arcs as possible.  We refer to the partial order induced by this acyclic graph as a \emph{strong partial order}.  We establish the following:

\begin{enumerate}
    \item There is a unique strong partial order. Although there may be an infinite number of distinct strong maximum circulations, the deletion of any of them results in the same strong partial order.
    \item The unique strong partial order is the union of the partial orders induced by deleting a maximum circulation. 
    \item One can obtain a strong maximum circulation by solving a single minimum cost flow problem. This permits the definition of optimality conditions that characterize strong maximum circulations.
\end{enumerate}
...
We further investigate the possibility of removing a maximal circulation with a minimum number of votes, which is the same objective as in Kemeny’s model.  We refer to such circulations as \textit{minimum maximal circulations}. The approach of seeking a minimum maximal circulation suffers from the same two drawbacks of Kemeny's method that we described earlier. First, we show that it is NP-hard to find a minimum maximal circulation. Second, we show via example that there may be two minimum maximal circulations that lead to conflicting partial orders.  That is, after removing the first minimum maximal circulation from the vote graph, alternative $i$ is preferred to alternative $j$.  After removing the second one, $j$ is preferred to $i$.

Kemeny's model and our model are quite different vote elimination approaches. In the strong maximum circulation problem, the objective is to maximize the number of eliminated votes.  Kemeny's problem is to minimize the number of eliminated votes. Nevertheless, Hochbaum and Levin's (HL) separation-deviation model \citep[see][]{hochbaum_methodologies_2006}, which generalizes several preference aggregation methods, reveals a surprising and interesting connection between Kemeny’s model and the maximum circulation model. Specifically, Kemeny's model can be transformed into a special case of the HL model with a concave, separable objective function. If this concave objective is approximated by a convex objective, the resulting optimization problem is the dual of the maximum circulation problem.  So, the maximum circulation model can be viewed as the dual of the ``convexification'' of Kemeny's model.

The paper proceeds as follows. We begin the next section by restating the problem of preference aggregation in terms of the isolation of an acyclic subgraph in a capacitated directed network, or vote graph. We use this network-based approach to preference aggregation to understand some of the challenges associated with existing optimization-based approaches, focusing on Kemeny's method (Section \ref{sec:why-cycle-elimination}). We then demonstrate how one can create an acyclic graph by eliminating cycles of votes that satisfy certain optimality criteria, i.e., maximal circulations. We explore the properties of these acyclic graphs, emphasizing the properties of the subsets of votes that remain after eliminating maximum circulations and minimum maximal circulations. 
...
A vote graph is a directed graph $G=(V,A)$ with the set of alternatives $V$ represented by the nodes (or vertices) of the graph. Let $n = |V|$ and $m=|A|$.  Each arc of the graph $(i,j)\in A$ represents the (strict) preference of $i$ over $j$ by at least one voter.  If there are $q_{ij}\geq 1$ voters that express the same preference $(i,j)$, we represent it by associating a capacity $q_{ij}$ with arc $(i,j)$.  Alternatively, one could represent the vote graph as a {\em multi-graph} with $q_{ij}$ arcs going from $i$ to $j$.   An example of a vote graph represented as multi-graph is given in \ref{fig:multi-graph} and represented as a weighted, or capacitated, simple graph is given in \ref{fig:capacitated-graph}. Note that it is possible for both $q_{ij}$ and $q_{ji}$ to be positive, meaning that there are voters that prefer $i$ to $j$ and voters that prefer $j$ to $i$.  In that case we have $\min \{q_{ij},q_{ji}\}$ 2-cycles of votes that contradict each other. A directed graph $G=(V,A)$ is called \textit{acyclic} if there are no directed cycles in $G$.

% Figure environment removed

We denote vectors by boldface characters. Let the vector of capacities of the arcs of $A$ in the vote graph be $\q$, where $\q \in \mathbb{R}_+^m$.  Although the vector $\q$ is initially integer, representing the count of the votes with each preference over pairs of alternatives, our method may leave a fractional number of votes, and therefore capacities can assume rational values.  At times, we  compactly denote the vote graph by $G[V,\q ]$ with the interpretation that this graph is $G=(V,A)$ where $A= A(\q)$ defined as $A(\q)=\{ (i,j): q_{ij}>0 \}$. 
...
For a given vector $\q$ of preferences, the \textit{Borda count} of an alternative $i \in V$ is equal to the number of votes for that alternative, i.e.,  $\sum_{j\in V} q_{ij}$. We refer to $\nabla q_i = \sum_{j\in V} q_{ij}-\sum_{j\in V} q_{ji}$ as the \textit{net Borda count} for alternative $i$.  If $\x$ is a vector such that $0 \le \x \le \q$, we define $\nabla x_i$ similarly.   That is, $\nabla x_i = \sum_{j\in V} x_{ij}-\sum_{j\in V} x_{ji}$. 

A {\em maximal circulation} is a circulation vector $\x'$ for which there is no other circulation vector $\x''$ with $\x'' > \x'$.  
We observe that,
\begin{observation}
...
\end{observation}
That is, the removal of a maximal circulation leaves a graph that is acyclic. This follows since if $G'$ contains a cycle, then $\x'$ can be appended by that cycle, contradicting its being maximal.
...
We will show that for any two strong maximum circulations $\x'$ and $\x''$, $A(\q - \x' ) = A(\q - \x'')$, and thus   $A^{TC}(\q - \x')$  $= A^{TC}(\q - \x'')$.   That is, the arcs that remain following the elimination of a strong maximum circulation $\x^*$ is independent of the strong maximum circulation. 
...
\noindent \textbf{Network Flow.} We next introduce some well-known results from the network flow literature that will be used in the the computational analysis of our algorithm. A more detailed treatment is available in \citet{ahuja_network_1993}.\\
...
The maximum circulation problem is a special case of MCNF in which each supply is equal to zero, the minimization is replaced by maximization, and each cost is equal to $1$.  (This is equivalent to a minimization problem with costs of $-1$.) 
...
\section{Why Cycle Elimination?}\label{sec:why-cycle-elimination}
...
In this section, we analyze preference aggregation methods that are based on eliminating maximum circulations.  As discussed in Section \ref{sec:prelim}, a cycle in a vote graph  $G[V,\q ]$ with a set of arcs $A=A(\q)$ corresponds to a collection of votes $(i_0, i_1), (i_1,i_2) \ldots , (i_k,i_0)$ that can be thought of collectively constituting a tie.  
Recall that an arc $(i, j)$ is called strong if there is a maximum circulation $\x'$ such that $\x'_{ij} < \q_{ij}$. The \emph{strong subgraph} of $A$ is the set $A^* = \{(i, j) : (i, j) \text{ is strong} \}$.


We will show the following results.

\begin{enumerate}
    \item The strong subgraph $A^*$ is acyclic.   This implies that for any two maximum circulations $\x'$ and $\x''$, there are no conflicts between the partial orders induced by $A(\q - \x')$ and $A(\q - \x'')$.  (Subsection \ref{sec:max-circ-non-conflict})

\item  If $\x^*$ is a strong maximum circulation, then $A(\q - \x^*) = A^*$.  (Subsection \ref{sec:max-circ-non-conflict})

\item We provide an algorithm that computes a strong maximum circulation by solving $m+1$ maximum circulation problems.  (Subsection \ref{sec:strong-max-circulations})

\item  We give three alternative methods for obtaining a ranking that is consistent with the strong partial order $A^{SP}$. (Subsection \ref{sec-rankings})  

\item  We provide optimality conditions that characterize strong maximum circulations. (Subsection  \ref{sec:strong})  

\item	 We show how to obtain a strong maximum circulation by solving just one minimum cost flow problem. (Subsection \ref{sec:more-efficient})
\end{enumerate}

\subsection{Maximum Circulations Lead to Non-Conflicting Partial Orders}
\label{sec:max-circ-non-conflict}

Here we show that the strong subset of $A$ is acyclic, and there is at least one strong maximum circulation. 

\begin{lemma}
\label{lem:strong-partial}

Let $G = (V, A(\q))$ be a vote graph. Let $A^*$ denote the strong subgraph of $G$.  Then $G^* = (V, A^*)$ is acyclic.  

\end{lemma}
\begin{proof}
Suppose to the contrary that $G^*$ contains a directed cycle $\cC$ with $\ell$ nodes (and $\ell$  arcs), $ (i_1,i_2), \ldots , (i_{\ell},i_{\ell+1})$ with $i_{\ell +1}=i_1$.  For each $k = 1, \ldots ,\ell$, let $\x ^k$ be a maximum circulation in which the flow in arc $(i_k, i_{k+1})$ is strictly less than the arc's capacity $q_{i_k, i_{k+1}}$.  Let $\x ^* =\frac {\sum_{k=1}^{\ell}\x ^k}{\ell}$. Then $\x ^*$ is also a maximum circulation.  
% Moreover, $A^* = A(\q-\x ^*)$. 
The construction of $\x^*$ implies that  $A(\q-\x ^*)$ contains the cycle $\cC$, which contradicts the fact that $\x ^*$ is a maximum circulation. Therefore, $A^*$ is acyclic, and $\x ^*$ is a strong maximum circulation.  
 \end{proof}

$A^*$ includes each arc that is not at capacity in some maximum circulation. We now show that the arc set $A^*$ is equal to the set of arcs remaining after removing any strong maximum circulation from the graph. 

\begin{lemma}\label{lem:strong-max}
...
\end{lemma}
    ...
    (iii) Finally, suppose that $\x'$ is a strong maximum circulation.  Then 
    $|A(\q-\x')| = |A(\q-\x^*)| = |A^*|$.   Moreover, $A(\q-\x') \subseteq A^*$.  It follows that $A(\q-\x') = A^*$.  
\end{proof}
...
\subsection{Strong Maximum Circulations and Strong Partial Orders} \label{sec:strong-max-circulations}
 
This development of the strong maximum circulation $\x^*$ in Lemma \ref{lem:strong-max} was not an efficient constructive algorithm,  but it immediately suggests an efficient algorithm for constructing the strong subset $A^*$ and the strong partial order $A^{SP}$.  \\
...
04.  \hspace{4 em}  let $z_{ij}$ be the optimal objective value for the max circulation problem\\ 
...
Algorithm 1 determines $A^*$ by solving  $m+1$ maximum circulation problems, each of which is solvable in $O(nm \log^2 n)$ time. We provide a more efficient algorithm in Section \ref{sec:certif}.

Algorithm 1 can easily be modified in order to produce a strong maximum circulation. Suppose that $A^* = \{a_1,\ldots , a_K\}$, and thus $|A^*|=K$.  For each $i = 1$ to $K$, let $\x (a_i)$ be a maximum circulation in which the flow in $a_i$ is less than its capacity. 
Taking the average of these $K$ maximum circulations,

\begin{align}
    \label{eqn:strong-circ}
    \x ^* = \frac{\sum _{i=1}^K \x(a_i)}{K}, 
\end{align}

\noindent yields a strong maximum circulation.

Even though $A^*$ and  $A^{SP}$ are uniquely determined, the construction in Equation \ref{eqn:strong-circ} shows that there may be an infinite number of different strong maximum circulations. This is because the equation could be replaced by 

\begin{align}
    \label{eqn:strong-circ2}
    \x^* = \sum_{i=1}^K \lambda_i \x(a_i) 
\end{align}

\noindent where $\lx$ is any strictly positive vector whose components sum to 1. Note that while there may be an infinite number of strong maximum circulations, there is a unique strong partial order $A^{SP}.$ 
...
Note that the graph $G[V,\q- \x ^*]$ is acyclic, and therefore the set of arcs $A(\q- \x ^*)$ form an acylic graph. That follows since $\x ^*$ is a maximal circulation.  Note, however, that whereas the number of arcs in $A(\q-\x')$ is $3$ and the number of arcs in $A(\q-\x'')$ is $3$, the number of arcs in $A(\q-\x ^*)$ is $4$ and hence strictly greater than the number of arcs in the acyclic graph induced by the integer valued circulations. Following Lemma \ref{lem:strong-max}, $A(\q-\x^*)=A(\q-\x') \cup A(\q-\x'')$. Finally, by adding the transitive closure arcs to $A(\q- \x ^*)$, we get the strong partial order $A^{SP}$.  
...
In the previous section, we introduced the strong partial order, which represents the transitive closure of the arcs remaining after the removal of a strong maximum circulation. Recall that a partial order is transitive and antisymmetric, but not necessarily complete. A ranking is a mapping of the alternatives to the integers in $[1,n]$, so that every alternative is comparable to every other alternative. Recall that a ranking is weak or strict according as ties are or are not permitted.  
...
\section{A More efficient Algorithm for Computing a Strong Circulation}
\label{sec:certif}

In Section \ref{sec:strong-max-circ}, we demonstrated how cycle elimination methods can be used to reveal a unique strong partial order and how obtain this order by solving at most $m+1$ maximum circulation problems. In this section, we present a more efficient method for computing a strong maximum circulation. We begin with optimality conditions that we refer to as ``strong complementary slackness.'' These conditions are more restrictive than the usual complementary slackness conditions, but less restrictive than strict complementary slackness conditions.

\subsection{Strong (Rather than Strict) Complementary Slackness}
\label{sec:strong}
Suppose that $\x'$ is a feasible circulation for (1), and  that $\y'$ is feasible for the dual of (1).   We use the notation $\c$ for the vector of cost coefficients, where $c_{ij} = 1$ for all $(i, j) \in A$.   We say that $\x'$ and $\y'$ satisfy \emph{strong complementary slackness} for the minimum cost flow problem if the following is true:
...
The above conditions are more restrictive than the standard complementary slackness conditions because of (3).    The usual complementary slackness condition is: ``if $x'_{ij} = q_{ij}$, then $c_{ij} - y'_i + y'_j  \ge  0$".  The above conditions are also less restrictive than strict complementary slackness conditions, which require,  in addition to (2) and (3), to replace (1) by:  ``For each $(i,j) \in A$, if $x'_{ij} = 0$, then $c_{ij} - y'_i + y'_j  <  0$". 

If $\x'$ and $\y'$ satisfy strong complementary slackness, then $\y'$  ``certifies'' that $\x'$ is a strongly maximum flow, as stated in the following lemma.

\begin{lemma}
  Suppose that $\x'$ is a feasible circulation, and that $\y'$ is feasible for the dual of (1).  If $\x'$ and $\y'$ satisfy strong complementary slackness, then $\x'$ is a strong maximum circulation.   
\end{lemma}
\begin{proof}
    Assume that $\x'$ and $\y'$ satisfy strong complementary slackness. Then they also satisfy complementary slackness, and thus  $\x'$ must be a maximum circulation.
    Suppose now that $\x''$ is any other maximum circulation. 
    We now claim that if $x'_{ij}=q_{ij}$ for any arc $(i,j) \in A$, then $x''_{ij}=q_{ij}$.  If this claim is true, then $\x'$ is a strong maximum circulation.  We see that the claim is true as follows. By strong complementary slackness, if $x'_{ij}=q_{ij}$, then $c_{ij} - y'_i + y'_j  >  0$.  Since $\x''$ is a maximum circulation, it follows that  $\x''$ and $\y'$ satisfy complementary slackness.  Since $c_{ij} - y'_i + y'_j  >  0$, it follows that $x''_{ij}=q_{ij}$.    
\end{proof}

\subsection {The More Efficient Algorithm}
\label{sec:more-efficient}

Next, we show how to find a strong maximum circulation $\x^*$ as well as the strong partial order $A^{SP}$ by solving a single maximum (or minimum) cost flow problem. We also produce a vector $\y^*$ such that $\x^*$ and $\y^*$ satisfy strong complementary slackness.

The faster method for determining $\x^*$ and $A^{SP}$ relies on solving the following maximum utility (negative cost) flow problem with two sets of decision variables $\w , \v \in \mathbb{R}^m$.  Recall that $m=|A|=|A(\q)|$, the number of arcs in the vote graph $G=(V,A)$.  Our network flow formulation (\ref{eqn:max_xy}) is defined on a graph $G=(V,A^{(2)})$ where $A^{(2)}$ contains 2 copies of each arc $(i,j) \in A$.  The flow in the first copy of $(i, j)$ will be denoted as $w_{ij}$ with capacity upper bound $q_{ij} - \epsilon$, and the flow in the second copy of $(i,j)$ will be denoted as $v_{ij}$ with capacity upper bound $\epsilon$.  If $q_{ij}$ is integral for all arcs $(i,j) \in A$, then we can choose $\epsilon = 1/(m+1)$.  Otherwise, we can choose $\epsilon = 1/( (m+1)\cdot LCD)$, where LCD is the least common denominator of the values in the vector $q$.
...
We let $c_{ij} = 1$ for all arcs $(i,j) \in A$.  
 ...
 \begin{align}
    \label{eqn:max_xy}
      ...
      \text{subject to } & \sum_{j: (i,j) \in A} (w_{ij}+v_{ij})-\sum_{j:  (j,i) \in A}( w_{ji}+v_{ij})=0  \,\,\ \forall i\in V, \nonumber\\     
 %     \nabla w_i \nonumber
...
\begin{lemma}
\label{equivalent}
Let $(\w', \v')$ be an optimal basic solution for the maximum cost network flow problem (\ref{eqn:max_xy}).  Let $\y'$ be optimal for the dual linear program.   Let $\x' = \w' + \v'$.  Then $\x'$ is a strong maximum circulation and the strong partial order 
$A^{SP}$ is the transitive closure of arcs $A'= \{(i, j): v'_{ij} = 0\}$. Moreover, $\x'$ and $\y'$ satisfy strong complementary slackness for the maximum circulation problem  (\ref{eqn:max-circ}).
\end{lemma}
\begin{proof}
...
Let $(\w^*, \v^*)$ be derived from $\x ^*$  as follows.  If  $x^*_{ij}< q_{ij} - \epsilon$, then $w^*_{ij} = x^*_{ij}$ and $v^*_{ij} = 0$.  If  $x^*_{ij}= q_{ij}$, then $w^*_{ij} = x^*_{ij} - \epsilon$ and $v^*_{ij} = \epsilon$.   Note that $(\w^*, \v^*)$ is feasible for the max cost flow problem (\ref{eqn:max_xy}).
Let $z^*$ be the optimal objective for the maximum circulation problem.   Then $f(\w^*, \v^*) = z^* - \frac{(m-K) \epsilon}{m+1} > z^* - \epsilon  $. 

We next establish the optimality of $\x'$ for  (\ref{eqn:max-circ}).
Note that $f(\w', \y') \ge f(\w^*, \y^*) > z^* - \epsilon$.  Therefore, $\c \cdot \x' > z^* - \epsilon$.   Since $(\w', \y')$ is a basic solution, it follows that all flows are integral multiples of $1/\epsilon$, and thus $\c \cdot \x' \ge z^* $, implying that $\x'$ is optimal for the max circulation problem.

We next show that $A' = A^*$, and accordingly $\x'$ is a strong maximum circulation.  $f(\w', \y') = z^* - \frac{(m-|A'|)\epsilon}{m+1} \ge 
f(\w^*, \y^*) = z^* - \frac{(m - |A^*|)\epsilon}{m+1}$.  It follows that $|A'| \ge |A^*|$.  Because $\x'$ is a maximum circulation, and $\x^*$ is a strong maximum circulation, it follows that $A' \subseteq A^*$.  We conclude that $A' = A^*$, and accordingly $\x'$ is a strong maximum circulation.
...
We now claim that $\x'$ and $\y'$ satisfy strong complementary slackness conditions for (\ref{eqn:max-circ}).  We see why as follows.  Because $(\w^*, \v^*)$ is optimal for (\ref{eqn:max-circ}),  $(\w^*, \v^*)$ and $\y'$ satisfy complementary slackness conditions for (\ref{eqn:max_xy}).  Therefore,  the following is true: 
 
\begin{enumerate}
\item  For each $(i,j) \in A$, if $x^*_{ij} = 0$, then $w^*_{ij} = 0$, and $c_{ij} - y_i + y_j  \le  0$.
\item  For each $(i,j) \in A$, if $0 < x^*_{ij} < q_{ij}$, then $0 < w^*_{ij} < q_{ij} - \epsilon$, and  $c_{ij} - y_i + y_j  =  0$.
\item  For each $(i,j) \in A$, if $x^*_{ij} = q_{ij}$, then $v^*_{ij} = \epsilon$, and $(c_{ij} - \frac{1}{m+1}) - y_i + y_j  \ge  0$, which implies that $c_{ij} - y_i + y_j  >  0$.
 \end{enumerate} 
 
 Thus, $\x^*$ and $\y'$ satisfy strong complementary slackness.

Finally, we claim that $\x'$ and $\y'$ satisfy strong complementary slackness, which will complete the proof of this lemma.  We prove the claim as follows.  Clearly, $\x'$ and $\y'$ satisfy the first and third conditions of strong complementary slackness.   As for the second condition, 

$$ \text{If } 0 < x'_{ij} < q_{ij}, \text{ then } 0 < x^*_{ij} < q_{ij} - \epsilon, \text{ and } c_{ij} - y_i + y_j  =  0.$$
\end{proof}
...
\section{Dual Optimal Rankings}\label{sec:HL-and-dual}

 We now discuss rankings that are consistent with the dual linear program of the maximum circulation problem. In Subsection \ref{sec:HL}, we introduce the separation-deviation model of \cite{hochbaum_methodologies_2006}, which links the Kemeny model to maximum cycle elimination methods, and we show that the convex relaxation of Kemeny's model is the dual of the maximum circulation problem (but not the strong maximum circulation problem). In Subsection \ref{sec:dual-optimal-rankings}, we show that all solutions to the convex relaxation of Kemeny's model are consistent with the strong partial order, but the converse is not true.  That is, a solution that is consistent with the strong partial order is not necessarily consistent with the convex relaxation of Kemeny's model. 

\subsection{The Hochbaum-Levin Model}\label{sec:HL}
In this section, we describe the separation-deviation model of \citet{hochbaum_methodologies_2006} which generalizes both the Kemeny model and the (dual of the) maximum circulation model and links them to each other.

The Hochbaum and Levin model, henceforth called the HL model, determines a score for each alternative so that the vector of scores minimizes a loss function.  The voting system permitted in the HL model is more general than the preference votes studied here: Each voter may provide a score for each alternative, as well as a pairwise preference for each pair of alternatives.

Given the more general voting system in the HL model, we will refer to voters in this section as {\em reviewers}.  A reviewer $r$ can submit a score  for some or all of the alternatives, and to submit pairwise preferences that correspond to the intensity of the (additive) difference in scores. For each alternative $i$, each reviewer $r$ can submit a score denoted as $s^r_i$.  For each pair of alternatives $i$ and $j$, reviewer $r$ can submit a preference of $i$ over $j$ with intensity $p^r_{ij}$. 
...
The decision variables for the HL model are the vector $\y$, where $y(i)$ is the score of alternative $i$; the higher the score, the more preferred the alternative. 


For each reviewer $r$ and alternative $i$, there is a loss $g_i^r(y(i) - s^v_i)$ indicating the {\em deviation} of the final score from the score that reviewer $r$ provides for alternative $i$.  For each reviewer $r$ and for each pair $i,j$ of alternatives, there is a loss {\em separation} function $f^r_{i,j}(p^r_{ij} - (y(i)- y(j)))$ which indicates the solution's difference in scores as compared to the intensity of the preference of reviewer $r$ for this pair $i,j$.  Both the separation and deviation functions take the value $0$ for zero valued arguments. That is, if the voter's score or preference is identical to the solution's, then the loss penalty is zero. The HL model is to determine a ranking that minimizes the total deviation and separation loss for all voters.  

As before, we let $V$ be the set of alternatives, and $A$ the set of pairs $(i,j)$ where at least one reviewer expresses a preference of $i$ over $j$. Let $R_{ij}$ be the set of reviewers that express a preference of $i$ over $j$.  Let $R_i$ be the set of reviewers that assigned a score to alternative $i$.  Recall that $q_{ij}=|R_{ij}|$ is the number of votes expressing a preference of $i$ over $j$. The variables
...
Hochbaum and Levin showed how to solve the optimization problem (HL) in polynomial time when the functions $g_i^r()$ and $f_{ij}^r()$  are convex. They also showed how to solve the problem in polynomial time when the variables are constrained to be integers. This problem was shown to be a special case of the {\em convex dual of minimum cost network flow} that is solved by the algorithm of \cite{AHO03} in $O(nm\log {\frac{n^2}{m}}\log nU )$ where $n$ is the number of different $g_i$ functions and $m$ is the number of different $f_{ij}$  functions, and $U$ is the range of the variables. For example, it may be desirable to constrain $\y$ to also fall in the range $[-n,n]$ for certain problems, in which case $U = 2n$.  If the functions are non-convex, then the problem is NP-hard, see e.g., \cite{GT2000}.

The preference voting system considered here is a special case of that in HL in which $R_i = \emptyset$ for all $i\in V$, and where $p^r_{ij} = 1$ for all $r \in R_{ij} $. That is, $p^r_{ij} = 1$ whenever reviewer $r$ prefers alternative $i$ to alternative $j$.  In addition, all decision variables (scores) are required to be integer valued.
...
\begin{align}
    %\label{eqn:max-circ}
     ...
     & 0 \le x_{ij} \le q_{ij} \ \ \  \forall (i,j) \in A. \nonumber
\end{align}
Let $y_i$, $i \in V$, be the dual variables associated with the flow balance constraints, and $z_{ij}$, $(i,j)\in A$ be the dual variables associated with the capacity constraints.


The dual linear programming formulation is then,
...
\end{align}
In an optimal solution, $z_{ij}=\max \{ y_j-y_i +1, 0\}$. It follows that $y(i) = y_i$ minimizes (Relax-KEM).
\end{proof}
...
\subsection{Dual Optimal Rankings Are Consistent}\label{sec:dual-optimal-rankings}

In Section \ref{sec-rankings}, we provided three different ways of obtaining consistent rankings.  In this section, we discuss the integer optimal solutions to Relax-KEM (and thus the negative of the dual solutions to the maximum circulation problem).  We first note that there always will be an integer optimal solution.  This follows from  the total unimodularity of the constraint matrix. We show that negative of an optimal solution vector forms a ranking that is consistent with every maximum circulation, including strong maximum circulations.  We also show that the negative of consistent rankings of Section \ref{sec-rankings} are not necessarily feasible feasible for the dual network flow problem \ref{eq:dual-program}, and thus not necessarily optimal for Relax-KEM.


\begin{theorem}
Suppose that $\y'$ is an optimal integer solution  for Relax-KEM where, for all $i \in V$, $y'_i \in \{-n, ..., -1\}$, and where $\max \{y'_i : i \in V \} = -1$. Suppose that $\x'$ is any maximum circulation.  Then $\r' = -\y'$ is a ranking vector that is consistent with the partial order $A^{TC}(\q - \x')$. 
\end{theorem}

\begin{proof}

Suppose that $\y'$ is an optimal integer solution for Relax-KEM.  Without loss of generality, we may assume that $\max \{y'_i : i \in V \} = -1$.   
We have already shown the following:  if $\y'$ is an optimal integer solution for Relax-KEM, then $(\y', \z')$ is optimal for the dual of the max circulation problem, where $z'_{ij} = \max \{0, 1 - y'_i + y'_j\}$. 

Let $\x'$ be any maximum circulation.  Let $A'$ denote the arc set of $G[V, \q - \x']$.  To complete the proof, we will  show  for all $(i, j) \in A'$, that $r'(j) \ge r'(i) + 1$.   Suppose now that  $(i, j) \in A'$.  By construction, $x'_{ij} < q_{ij}$.  By complementary slackness, $z'_{ij} = 0$.  Therefore, $1 - y'_i + y'_j \le 0$, and thus $r'(j) - r'(i) \ge 1$.  
\end{proof}

The converse is not true.  There may be rankings that are consistent with the strong partial order but whose negative is not optimal for the dual linear program.

The following example shows that the negative of the minimum consistent ranking is not necessarily optimal for Relax-KEM. Consider an instance with votes $(1,2)$, $(2,3)$, $(3,4)$, and $(4,3)$. The unique maximum circulation $\x'$ consists of pushing one unit around the 3-4-3 cycle.  Then $A(\q - \x')$ consists of arcs $(1, 2)$ and $(2, 3)$ only. That is, the minimum consistent rank assigns alternatives their most favorable possible ranks subject to the constraints that $1$ is preferred to $2$, and $2$ is preferred to $3$. The minimum consistent ranking is $\r_1$, where   $r_1(1)$ = 1, $r_1(2)$ = 2, $r_1(3) = 3$, and $r_1(4) = 1$. The objective value of Relax-KEM for $\y' = -\r_1$ is $z'_{3,4} = y'_4 - y'_3 + 1 = 3$.  An optimal solution for Relax-KEM is $\y'' = -\r_2$, where $\r_2$ is defined as follows: $r_2(1)$ = 1, $r_2(2)$ = 2, $r_2(3) = 3$, and $r_2(4) = 2$.  The objective value of Relax-KEM for $\y''$ is $z''_{3,4} = y''_4 - y''_3 + 1 = 2$.

To see why this example arises, note that $x'_{4,3}=q_{4,3} > 0$. 
 In any optimal solution $(\y',\z')$ for the dual, complementary slackness implies that $y'_4 - y'_3 + z'_{4, 3} = 1$.  Equivalently,  $-r'(4) +r'(3) + z'_{4, 3} = 1$.   Therefore, $r'(4) \ge r'(3) - 1$, a condition that is violated for $\r' = \r_1$.
 
 In general, suppose that $\x'$ is any maximum circulation, and that $x'_{ij} > 0$.   If $\y'$ is optimal for Relax-KEM, and if $\r' = -\y'$, then  by complementary slackness, $r'(i) \ge r'(j) - 1$.  This constraint is not necessarily satisfied by other rankings that are consistent with $G[V, \q - \x']$.     
...
% %\path (1) edge [->] node [below] {1} (2);
% %\path (2) edge [->] node [right] {1} (3);
% \path[bend right=15] (4) edge [->] node [below] {1} (3);
% \path[bend right=15] (3) edge [->] node [above] {1} (4);
% %\path (3) edge [->] node [below left] {1} (1);
% \end{tikzpicture}
% }}
% \hspace{0.5in}
% G[V,\q-\x']=\vcenter{\hbox{
% \begin{tikzpicture}[main/.style = {draw, circle}, node distance=2cm] 
% \node[main] (1) {$1$}; 
% \node[main] (2) [right of=1] {$2$}; 
% \node[main] (3) [below of =2] {$3$}; 
...
\section{Cycle Elimination with Minimum Maximal Circulations} \label{sec:minmax}
...
Consider the vote graph $G = (V, A)$, in which $V = \{1, 2, ..., 8\}$, and where $A = \{(1, 2), (2, 3), (2,5), $ $(3, 4), (4, 1), (4, 7), (5, 6), (6, 1), (6, 5), (7, 8), (8, 3), (8, 7)\}$. Each arc in $A$ has a capacity of 1, which is illustrated in the top panel of \autoref{fig:minmax}.
...
\section{Conclusion}
It is self evident that a cycle of preference votes, each requiring that one alternative ranks higher than the next, cannot conform to any partial order, or consistent ranking.  Such a cycle represents a tie or contradictory votes for the alternatives on the cycle and must be disregarded 
in order to obtain a partial order that aggregates the other votes.

Our method removes the votes in a maximum circulation.  Subsequently,  there are no additional cycles remaining and a partial order is immediately derived.  If there is any vote remaining on a preference, it is included in the partial order.  A major contribution of our paper is the introduction of the concept of {\em strong} maximum circulation that guarantees the {\em uniqueness} of the induced {\em strong partial order}. If there is {\em any} maximum circulation that keeps at least one vote for a certain preference, then so does the strong maximum circulation. A second major contribution is that we show how to obtain a strong maximum circulation by solving a single minimum cost flow problem.  This implies that our method runs in strongly polynomial time.  
  
We contrast our method with the well-known Kemeny method. The Kemeny method does not return a unique solution and may result in contradictory rankings among the different solutions. In the case of a single cycle of votes including all alternatives, any alternative can be ranked first in Kemeny's method. Our approach returns a unique partial order.

Even in cases where there is a unique solution for Kemeny's model, the solution can produce a ranking that is unreasonable, as seen in the example in Section \ref{sec:why-cycle-elimination}. In the example, Kemeny's method eliminates five votes in favor of alternative 6 and none against alternative 6 while eliminating no votes in favor of alternative 5 and five votes against alternative 5, leading alternative 5 to be ranked better than alternative 6. In our approach, in contrast, every vote that is eliminated in favor of an alternative is paired with an eliminated vote against that alternative.

Further, there is no polynomial time algorithm to produce an optimal solution for Kemeny's method (unless P = NP). Our approach is solved in strongly polynomial time using combinatorial network flow algorithms.
