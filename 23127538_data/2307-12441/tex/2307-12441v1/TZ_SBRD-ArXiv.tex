...
\noindent
 We discuss a swarm-based descent method for non-convex optimization.
 The swarm consists of $N$ agents, each identified with a time-dependent position and mass.
 \[
 \bxin=\bx_i(t^n) \in \R^d, \ \  m_i^n=m_i(t^n)\in (0,1], \qquad i=1,2,\ldots, N.
 \] 
...
\begin{subequations}\label{eqs:SBD}
...
The fraction of mass transfer,  $\Big(\frac{F(\bxin)-F^n_{\textnormal{min}}}{F^{n}_{\textnormal{max}}-F^{n}_{\textnormal{min}}}\Big)^\mq$,  is determined by the \emph{relative height} of each agent relative to the global extremes\footnote{To prevent vanishing denominator in the extreme case $F_{\textnormal{max}} = F_{\textnormal{min}}$, we adjust \eqref{eq:etai} with a small $\epsilon$-correction, 
 ...
 }, $\displaystyle F^n_{\textnormal{min}}= \min_j F(\bx^n_j)$ and $\displaystyle F^n_{\textnormal{max}}= \max_j F(\bx^n_j)$, and a mass transfer parameter,  $\mq\geq 1$. The higher $\mq$ is, the more tamed is the transfer of mass. A systematic study  reported in section \ref{sec:high-q} below reveals   a dramatic improvement when increasing the mass transfer parameter $\mq=2,4, 8$.\newline
...
This  follows an aggressive ``survival of the fittest'' protocol, \cite[\S3]{LTZ2022}, so that after $N$ iterations the swarm consists of a single agent which should be in the best position to approach the minimum of the space explored so far by the swarm. We note in passing that one   can  adopt a more flexible protocol which allows the worst (highest) agents to survive a few iterations before  elimination; this flexibility would improve the overall success rates of the swarm at the expense of efficiency.
...
\begin{equation}\label{eq:descentclam}
 F(\bxin-h\bp)\leq F(\bxin)-\hf\clam \relmin h |\nabla F(\bxin)|^2, \qquad 
 \relmin=\frac{m_i^{n+1}}{\max_i m_i^{n+1}}, \quad \clam<1.
\end{equation}
Recall that the steepest descent corresponding to the gradient direction, $\bpin=\nabla F(\bxin)$, secures the sharper descent property,  
\[
F(\bxin-h\bp)\leq F(\bxin)-\clam \relmin h|\nabla F(\bxin)|^2,
\]
...
%\medskip\noindent
%{\bf Choosing the descent direction. }. The main new feature of the SBRD method is a stepping  protocol based on a large set of randomly chosen orientations, while still maintaining a descent property. This implies that the swarm, stepping in  other than the  gradient direction, will explore  a larger portion  of the ambient space  which in turn leads to a more effective  search. 

\vspace*{-1cm}
% Figure environment removed

\noindent
To this end, the SBRD choice of the direction $\bpin$ is determined by its orientation $\boin$,
\begin{subequations}\label{eqs:orient-omega}
\begin{equation}
\bpin=|\nabla F(\bxin)|\boin, \qquad \boin \in {\mathbb S}^{d-1},
\end{equation}
relative to the orientation of the gradient, $\bqin=\frac{\nabla F(\bxin)}{|\nabla F(\bxin)|} \in {\mathbb S}^{d-1}$,
so that
...
\be
r\in {\mathcal N}\Big(\frac{1}{2}(1+\relmin), 1\Big).
\ee
\end{subequations}
...
It follows that the new position, $\bx^{n+1}(h)=\bxin-h\bpin$ --- viewed as a function of the step size $h$, satisfies the desired descent property, at least for small enough $h$. Indeed,    \eqref{eq:orient-omega} implies 
  \begin{equation}\label{eq:porient}
  \langle \bpin, \nabla F(\bxin)\rangle= r|\nabla F(\bxin)|^2 \geq \hf(1+\relmin)|\nabla F(\bxin)|^2.
  \end{equation}
 Hence, if $\nabla F$ has a  Lipschitz bound $L\leq \infty$,  then for every $\clam<1$ and  $h<\nicefrac{1}{ L}$,  there holds
  ...
  \frac{h^2}{2}L|\nabla F(\bxin)|^2 \\
   ...
   \end{split}
  \end{equation}
Thus, we recover \eqref{eq:descentclam} for any $h<\nicefrac{1}{L}$.\footnote{In fact,  a slightly larger threshold, $h<\frac{1+\relmin(1-\clam)}{L}$, still allows \eqref{eq:halfdescent} to hold.}\newline
 We note, however, that we have no access to the Lipschitz  bound $L$, and therefore, no access to an effective  step size securing \eqref{eq:halfdescent}, beyond $h$ being `sufficiently small'. Instead, we seek a  step size that identifies the \emph{largest}  $h$ for which \eqref{eq:halfdescent} holds. To this end we use a \emph{backtracking protocol} outlined in \algo \ref{alg:backtracking} below. 
The backtracking algorithm  produces a time step, $\hin$,  depending on the position of the agent,  $\bxin$, and its relative mass $\relmin$,
\[
...
%%%%%%%%%%%%%%%%
\section{Implementation of the \SBD algorithm}\label{sec:construct}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The \SBD method  relies on three procedures: (i) a communication protocol that dictates mass transition factors, 
$\{\etain\}$;  (ii) a random choice of the descent direction, $\bpin$; and (iii) an effective strategy for taking step size, $\hin=h\big(\bxin,\clam\relmin\big)$, in that direction. Both the direction and step size are adjusted to the position and the relative mass of a given agent. These procedures are summarized in the following pseudo-codes.
...
\]
Here $r$ is a randomly chosen parameter from a normal distribution in 
$ \frac{1}{2}(1+\relmin)< r < 1$; thus,  the spherical cap has an opening angle of $\theta=\arccos(r)$,  ranging from $\theta=60^{\circ}$ for lightest agents to  the gradient orientation, $\theta=0^{\circ}$, for the heaviest agent.
...
Recall that by Taylor's expansion \eqref{eq:halfdescent}, the desired  bound holds  as long as the step size is sufficiently small, $\hin\ll 1$. Our aim to choose a step size that is \emph{sufficiently large}, in order to maximize the descent term $\hf \clam \relmin \hin|\nabla F(\bxin)|^2$.  To this end, one employs a
dynamic adjustment, starting with a relatively large $h$ for which one expects 
\[
...
$\displaystyle  
\hin \geq  \frac{\gamma}{L}$ with $L$ denoting the Lipschitz bound of $\nabla F$ which is assumed to exist. Indeed, this can be argued by contradiction: if \mbox{$\displaystyle \frac{\hin}{\gamma} \leq  \frac{1}{L}$} then by \eqref{eq:porient} we would have, 
\[
\begin{split}
...
This contradiction confirms that $\hin \geq \frac{\gamma}{L}$, which in turn enables us to convert the descent bound \eqref{eq:SBDdescent} into a precise descent property,
...
% Unless otherwise specified, all simulations  reported in this paper employ the same thresholds
% \begin{equation}\label{eq:tols}
% tolm = 10^{-4}, \quad tolmerge = 10^{-3}, \quad tolres = 10^{-4}.
% \end{equation}
% Smaller thresholds will improve exploration at the expense of efficiency. 
...
\begin{equation}\label{eq:var}
 \forall n, j: \quad  F(\bx^n_i)-F(\bx^n_j) \leq M, \qquad M:=\max F(\bx_i^0)-F(\bxmin)
 \end{equation}
...
\[
F(\bX\mymax{n})-F(\bXnmin) \geq \frac{\gam}{4L}\clam|\nabla F(\bX\mymax{n})|^2.
\]
...
The desired bound \eqref{eq:summability} follows by a telescoping sum. $\square$
 ...
 The  summability bound \eqref{eq:summability} implies that eventually, for large enough $>N_0$, the minimizers and (parent of) heaviest \SBD agents, $\cdel_n<1$ and hence
\[
...
the  Palais-Smale condition, $F(\bX^{n_\alpha})\leq \max_i F(\bx^0_i)$ while $\nabla F(\bX^{n_\alpha}) \stackrel{\alpha\rightarrow \infty}{\longrightarrow}0$. Arguing along \cite[Theorem 3.3]{LTZ2022} we summarize by stating the following.
...
\end{equation}
In particular, if $F$ admits only distinct local minima in $\Omega$ (i.e., different local minima have different heights), then the whole sequence $\bX^n$ converges to a minimum.
\end{theorem}
...
\begin{theorem}\label{thm:GD-backtracking with PL}
 ...
 $\beta \in(1,2]$, such that the Lip bound \eqref{eq:FisC2} holds.  Let $\{\bXnmin\}_{n\geq0}$ denote the time sequence of   \SBD minimizers,  \eqref{eqs:SBD},\eqref{eq:step length}.  Then, there exists a constant, $C=C(\gamma,\lambda,\mu)$, such that 
...
\left\{\begin{array}{lll}
     ...
     \end{array} \right. \quad \etain := \Big(\frac{F(\bxin)-F^n_{\textnormal{min}}}{F^{n}_{\textnormal{max}}-F^{n}_{\textnormal{min}}}\Big)^\mq \in (0,1],
\ee
We find that  increasing $q$ in the mass transfer protocol
