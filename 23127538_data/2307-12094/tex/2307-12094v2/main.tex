...
\maketitle
\begin{abstract}
    Let $G$ be a multigraph and $L\,:\,E(G) \to 2^\N$ be a list assignment on the edges of $G$.
    % We define $L\,:\,V(G) \to 2^\N$ such that $L(x)$ is the intersection of the lists $\mathcal L(e)$ for all $e$ adjacent to $x$.
    Suppose additionally, for every vertex $x$, the edges incident to $x$ have at least $f(x)$ colors in common.
    We consider a variant of local edge-colorings wherein the color received by an edge $e$ must be contained in $L(e)$.
    The locality appears in the function $f$, i.e., $f(x)$ is some function of the local structure of $x$ in $G$.
    Such a notion is a natural generalization of traditional local edge-coloring.
    Our main results include sufficient conditions on the function $f$ to construct such colorings.
    As corollaries, we obtain local analogs of Vizing and Shannon's theorems,
    recovering a recent result of Conley, Greb\'ik and Pikhurko.
    % as well as earlier results of Borodin, Kostochka and Woodall.
\end{abstract}
...
\begin{defn}\label{defn:edge_coloring}
    % Given a multigraph $G$, we say that two edges $e, f \in E(G)$ are \emphd{adjacent} if $V(e) \cap V(f) \neq \0$.
...
Vizing famously proved $\chi'(G) \leq \Delta(G) + \mu(G)$ \cite{Vizing}.
...
% Borodin, Kostochka and Woodall considered an edge-coloring variant \cite{borodin1997list}.
% \todo{\ldots}
% In fact, they consider the more general list edge-coloring setting.
% Before stating their results, we make the following definition.
...
\begin{defn}[$L$-edge-coloring]\label{defn:local_sha}
    Given a multigraph $G$ and a function $L \,:\,E(G) \to 2^{\N}$, we say $\phi\,:\,E(G) \to \N$ is a \emphd{proper $L$-edge-coloring} if it is a proper edge-coloring such that for each edge $e \in E(G)$, we have $\phi(e) \in L(e)$.
\end{defn}
...
In \cite{borodin1997list}, the authors proved a local list version of K\H{o}nig's theorem, providing an alternate proof of the list edge-coloring conjecture for bipartite multigraphs.
Specifically, they prove that every bipartite multigraph admits a proper $L$-edge-coloring provided that for each edge $e \in E_G(x, y)$, $|L(e)| \geq \max\set{\deg(x), \deg(y)}$.
They also prove an analogous result for Shannon's bound. 
Specifically, there is a proper $L$-edge-coloring for a multigraph $G$ if for each edge $e \in E_G(x, y)$
...
They show there is a proper $L$-edge-coloring whenever $|L(e)| \geq (1+\eps)\max\set{\deg(x), \deg(y)}$ for each $e \in E_G(x, y)$, provided $\Delta(G)$ is sufficiently large and $\delta(G) \geq \ln^{25}\Delta(G)$.



% Their first result is a local list version of K\H{o}nig's theorem for bipartite multigraphs.
% Specifically, they prove that every bipartite multigraph admits a proper $L$-edge-coloring provided that for each edge $e \in E_G(x, y)$, $|L(e)| \geq \max\set{\deg(x), \deg(y)}$.
% They also prove an analogous result for Shannon's bound. 
% Specifically, there is a proper $L$-edge-coloring for a multigraph $G$ if for each edge $e \in E_G(x, y)$
% \[|L(e)| \geq \max\set{\deg(x), \deg(y)} + \lfloor\min\set{\deg(x), \deg(y)}/2\rfloor.\]
% % An asymptotic result in local edge-colorings follows from the work of Bonamy, Delcourt, Lang and Postle \cite{bonamy2020edge}.
% % In particular, there is a proper edge-coloring $\phi$ satisfying $\phi(e) \leq (1+\eps)\max\set{\deg(x), \deg(y)}$ for each $e \in E_G(x, y)$, provided $\Delta(G)$ is sufficiently large and $\delta(G) \geq \ln^{25}\Delta(G)$.
% Bonamy, Delcourt, Lang and Postle provided an asymptotic bound in this setting.
% They show there is a proper $L$-edge-coloring whenever $|L(e)| \geq (1+\eps)\max\set{\deg(x), \deg(y)}$ for each $e \in E_G(x, y)$, provided $\Delta(G)$ is sufficiently large and $\delta(G) \geq \ln^{25}\Delta(G)$.
% for local edge-colorings by constructing a proper edge-coloring $\phi$ satisfying $\phi(e) \leq (1+\eps)\max\set{\deg(x), \deg(y)}$ for each $e \in E_G(x, y)$ (their result is more general as it concerns list edge-coloring) \cite{bonamy2020edge}.
% The result follows from a more general one concerning list edge-coloring.



% We are interested in a more general notion of locality involving lists.

% \begin{defn}[$L$-edge-coloring]\label{defn:local_sha}
%     Given a multigraph $G$ and a function $L \,:\,E(G) \to 2^{\N}$, we say $\phi\,:\,E(G) \to \N$ is a \emphd{proper $L$-edge-coloring} if it is a proper edge-coloring such that for each edge $e \in E(G)$, we have $\phi(e) \in L(e)$.
% \end{defn}

% A list assignment to the edges of $G$ is a function $\mathcal L\,:\,E(G) \to 2^\N$.
% A proper edge-coloring $\phi$ with respect to $L$ is a coloring that satisfies $\phi(e) \in \mathcal L(e)$ for each $e \in E(G)$.
% The list edge-coloring conjecture is a famous open problem posed independently by a number of researchers in the 1970s and 1980s.
% It states that as long as $|L(e)| \geq \chi'(G)$ for each edge $e$, $G$ admits a proper $L$-edge-coloring.
% We will consider a local variant of this problem.
% The setting in which $|L(e)|$ is a function of the local structure of its endpoints has been studied in \cite{bonamy2020edge, borodin1997list}.
% The results mentioned in the previous paragraph follow from these more general results.
Improving the results of \cite{bonamy2020edge} further seems a difficult task as it would result in strong advancements toward the list edge-coloring conjecture.
However, there has been success with certain types of lists, namely sets of the form $[r]$.
Recently, Christiansen \cite{Christ} resolved a conjecture posed in \cite{bonamy2020edge} by proving that all simple graphs admit an $L$-edge-coloring for $L(xy) = [1 + \max\set{\deg(x), \deg(y)}]$.
...
In particular, given a multigraph $G$ and a list assignment $L$ of the edges of $G$, we define a function on the vertices of $G$ as follows:
...
We note that this is a natural abstraction of local edge-colorings considered in \cite{Christ, localvizing} as the constraint $|f(x, L)| \geq c(x)$
% In fact, the requirement that $1 \leq \phi(e) \leq \max\set{c(x), c(y)}$ for some function $c$ (where $V(e) = \set{x,y}$) 
can be achieved by the list assignment $L(e) = [\max\set{c(x), c(y)}]$ (where $V(e) = \set{x, y}$).




With are now ready to state our main result.


\begin{theo}\label{theo:main_theo}
    Let $G$ be an $n$-vertex multigraph of maximum degree $\Delta$ and $L \,:\,E(G) \to 2^{\N}$ be a list assignment on the edges of $G$.
...
\begin{prop}\label{prop:list_size}
    ...
    \end{enumerate}
    Then there exists a list assignment on the edges $L'\,:\,E(G) \to 2^\N$ such that
    \begin{enumerate}
        \setcounter{enumi}{2}
...
\begin{proof}
    ...
    % Let $\ell \in \N$ such that $\ell \geq \max_xc(x)$.
    % As a result of \ref{item:finite_c}, we can choose $\ell < \infty$.
    ...
    % Define $L_v'(x) \subseteq L_v(x)$ by removing any arbitrary $\max\set{|L_v(x)| - \ell, 0}$ elements from $L_v(x)$.
    ...
    % In particular, an $L'$-coloring is also an $L$-coloring.
...
% and by setting $L(e) = [\max\set{\deg(x), \deg(y)}]$, we recover the result on bipartite multigraphs in \cite{borodin1997list}.
% Furthermore, by setting $L(e) = [\max\set{\Shav{x}, \Shav{y}}]$, we prove an analogous local result for Shannon's bound, which is slightly weaker than the one in \cite{borodin1997list}.
...
Throughout the remainder of the paper, $G$ is an $n$-vertex multigraph of maximum degree $\Delta$, $L\,:\, E(G) \to 2^{\N}$ is a list assignment on the edges of $G$, and $f(x, L)$ is as defined in \eqref{eqn:local_list_size}.
...
% If, in addition $\phi$ is local with respect to $L$, we call it a \emphd{partial $L$-edge-coloring} of $G$.
...
% If $e \in E_G(x,y)$ is $\phi$-successful, we can extend the coloring $\phi$ by assigning any color in $M(\phi, x)\cap M(\phi, y)$ to $e$ to obtain a proper partial $\Sha$-edge-coloring.
...
% We also let $E(C) \defeq \set{e_0, \ldots, e_{k-1}}$ be the edge set of $C$ and $V(C)$ be the set of vertices incident to the edges in $E(C)$.
...
% We define \emphd{locally $\phi$-shiftable chains} analogously.
...
% however, the original prooof of Shannon's theorem shows it is always possible to find a $\phi$-successful chain (see \cite[Algorithm~3.1]{ShannonChain} for an explicit construction and Fig.~\ref{fig:shannon_chains} for an example).
...
To this end, we define a \emphd{potential function} of a proper partial coloring $\phi$.
\begin{align*}
...
\end{align}
% In this paper, we will assume $|L(e)| < \infty$ for each edge $e$.
...
% This is enough to prove Theorem~\ref{theo:main_theo}.
...
% We can define \emphd{$\phi$-successful} chains analogously.

% Given two chains $C = (e_0, \ldots, e_{k-1})$ and $C' = (f_0, \ldots, f_{j-1})$ with $e_{k-1} = f_0$, we define their \emphd{sum} as follows:
% \[C+C' \,\defeq\, (e_0, \ldots, e_{k-1} = f_0, \ldots, f_{j-1}).\]
% Note that if $C$ is (locally) $\phi$-shiftable and $C'$ is (locally) $\Shift(\phi, C)$-shiftable, then $C+C'$ is (locally) $\phi$-shiftable.
The first special type of chains we consider are path chains.


\begin{defn}[{\cite[Definition~2.2]{ShannonChain}}]\label{defn:path}
    A chain $P = (e_0, \ldots, e_{k-1})$ is a \emphd{path chain} if the edges $e_1$, \ldots, $e_{k-1}$ form a path in $G$, i.e., if there exist distinct vertices $x_1$, \ldots, $x_k$ such that $e_i \in E_G(x_i,x_{i+1})$ for all $0 \leq i \leq k-1$ (see Fig.~\ref{fig:path} for examples). We let $x_0 \in V(e_0)$ be distinct from $x_1$ and let $\vstart(P) \defeq x_0$, $\vend(P) \defeq x_k$ denote the first and last vertices on the path chain respectively. 
...
\begin{proof}
    ...
    By \ref{item:alpha}, it follows that
    % \ref{item:beta} and since either $\alpha$ or $\beta$ is missing at both $x$ and $y$, 
    $e$ is $\phi$-happy.
    ...
    By our choice of $j$, it follows that
    ...
    This along with \eqref{eqn:avail_sets} implies that $A(G, \psi) \leq A(G, \phi) - 1$, as desired.
\end{proof}
...
\begin{itemize}
    ...
    \item $A(G, \phi) = A(G, \Shift(\phi, C))$ and $\Shift(\phi, C)$ contains a path chain satisfying the conditions of Lemma~\ref{lemma:path}.
\end{itemize}
...
% In the first two cases, we have a happy or content chain $C$.
% In the final case, let $P$ be the path chain satisfying the conditions of Lemma~\ref{lemma:path}.
% If $P$ is $\Shift(\phi, C)$-happy, then the coloring $\Shift(\Shift(\phi, C), P)$ contains an additional colored edge.
% If not, then we have for some $1 \leq j \leq \length(P)$:
% \[A(G, \Shift(\Shift(\phi, C), P|j)) < A(G, \Shift(\phi, C)) = A(G, \phi).\]
% In particular, $\Pot(\Shift(\Shift(\phi, C), P|j)) < \Pot(G, \phi)$ and so the potential decreases.
% As a result of \eqref{eqn:pot_bdd}, this can only happen finitely many times.
% Furthermore
...
\begin{algorithmic}[1]
    ...
    \State Let $f \in E_G(x)$ be such that $\phi(f) = \eta$ and let $z \in V(f)$ be distinct from $x$.
    \State \Return $(e, f)$
\end{algorithmic}
\end{algorithm}
...
\begin{proof}\stepcounter{ForClaims}\renewcommand{\theForClaims}{\ref{lemma:fan_shannon}}
    ...
    Suppose $F = (e, f)$ and let $x,\,y,\,z,\,\eta$ be as defined in Algorithm~\ref{alg:shannon_chain}.
    Note that we have $A(\phi, y) \subseteq U(\phi, x)$.
    Let $\psi \defeq \Shift(\phi, F)$.
    We will consider each case separately.
    \begin{enumerate}[label=\ep{\normalfont{}\textbf{Case\arabic*}},labelindent=0pt,leftmargin=*]
    ...
    \end{enumerate}
    The final case to consider is $A(\phi, y) \cup A(\phi, z) \subseteq U(\phi, x)$, $\eta \in f(z, L)$ and $\deg(z) \geq \deg(y)$.
    ...
    An identical argument to the one in \cite[Lemma~2.11]{ShannonChain} shows that either $P(e; \phi, \alpha\beta)$ or $P(f; \psi, \alpha\beta)$ satisfies the conditions of Lemma~\ref{lemma:path} under the corresponding coloring.
...
Furthermore, an identical argument as that of Algorithm~\ref{alg:shannon_chain} shows that the running time of Algorithm~\ref{alg:vizing_chain} is $O(\Delta\,\ell)$.
...
\begin{algorithmic}[1]
    ...
    \EndFor
    \medskip%\LineComment{Iterations}
...
\begin{Lemma}\label{lemma:fan_vizing}
    Let $(F, \beta, j)$ be the output of Algorithm~\ref{alg:shannon_chain} on input $(\phi, e, x)$. Then for $F' \defeq F|j$, 
    \begin{enumerate}
        \item either $F$ is $\phi$-happy, or
...
\begin{proof}
    ...
    An identical argument to \cite[Lemma~A.1]{ShannonChain} shows that either $P(\End(F); \psi, \alpha\beta)$ or $P(\End(F'); \psi', \alpha\beta)$ satisfies the conditions of Lemma~\ref{lemma:path}.
    ...
    \[A(\psi, z) \subseteq A(\phi, z) \setminus \set{\phi(e_{i+1})\,:\, i \in I(F, z)} \cup \set{\phi(e_{i})\,:\, i \in I(F, z)}.\]
