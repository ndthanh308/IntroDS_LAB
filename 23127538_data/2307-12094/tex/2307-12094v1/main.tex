...
\begin{abstract}
    ...
    Such a notion is a natural generalization of local edge-colorings considered so far in the literature.
    Our main results include sufficient conditions on the function $f$ to construct such colorings.
    As corollaries, we recover a recent result of Conley, Greb\'ik and Pikhurko as well as earlier results of Borodin, Kostochka and Woodall.
\end{abstract}
...
They constructed a proper edge-coloring $\phi$ for bipartite multigraphs such that for each edge $e \in E_G(x, y)$, $\phi(e) \leq \max\set{\deg(x), \deg(y)}$.
They also constructed a local coloring $\phi$ for arbitrary multigraphs matching Shannon's bound, specifically, 
\[\phi(e) \leq \max\set{\deg(x), \deg(y)} + \lfloor\min\set{\deg(x), \deg(y)}/2\rfloor, \quad V(e) = \set{x, y}.\]
Bonamy, Delcourt, Lang and Postle provided an asymptotic bound for local edge-colorings by constructing a proper edge-coloring $\phi$ satisfying $\phi(e) \leq (1+\eps)\max\set{\deg(x), \deg(y)}$ (where $V(e) = \set{x, y}$) \cite{bonamy2020edge}.
Recently, Christiansen \cite{Christ} resolved a conjecture posed in \cite{bonamy2020edge} by proving that all simple graphs admit a local edge-coloring $\phi$ such that $\phi(xy) \leq 1 + \max\set{\deg(x), \deg(y)}$.
Conley, Greb{\'\i}k and Pikhurko extended this result to multigraphs by defining a local edge-coloring $\phi$ such that $\phi(e) \leq \max\set{\deg(x) + \mu(x), \deg(y) + \mu(y)}$, where $V(e) = \set{x, y}$ and $\mu(x) \defeq \max_{z}\mu(x, z)$ \cite{localvizing}.
...
It states that as long as $| L(e)| \geq \chi'(G)$ for each edge $e$, $G$ contains a proper $L$-edge-coloring $\phi$.
...
We note that this is a natural abstraction of local edge-colorings considered so far in the literature (see the previous paragraph).
In fact, the requirement that $\phi(e) \leq \max\set{c(x), c(y)}$ for some function $c$ (where $V(e) = \set{x,y}$) is equivalent to having $L(e) = [\max\set{c(x), c(y)}]$ in our setting.
...
Let $e \in E_G(x, y)$.
We remark that by setting $L(e) = [\max\set{\deg(x) + \mu(x), \deg(y) + \mu(y)}]$, we recover the result of \cite{localvizing}, and by setting $L(e) = [\max\set{\deg(x), \deg(y)}]$, we recover the result on bipartite multigraphs in \cite{borodin1997list}.
...
\begin{corl}[to Theorem~\ref{theo:main_theo}]\label{theo:local_bounds}
    ...
    \begin{enumerate}
        \item There is a proper edge-coloring $\phi\,:\,E(G) \to \N$ such that for each $e \in E(G)$, we have $\phi(e) \leq \max\set{\Shav{x}, \Shav{y}}$, where $V(e) = \set{x, y}$.

        \item There is a proper edge-coloring $\phi\,:\,E(G) \to \N$ such that for each $e \in E(G)$, we have $\phi(e) \leq \max\set{\deg(x) + \mu(x), \deg(y) + \mu(y)}$, where $V(e) = \set{x, y}$.

        \item If $G$ is bipartite, there is a proper edge-coloring $\phi\,:\,E(G) \to \N$ such that for each $e \in E(G)$, we have $\phi(e) \leq \max\set{\deg(x), \deg(y)}$, where $V(e) = \set{x, y}$.
    \end{enumerate}
