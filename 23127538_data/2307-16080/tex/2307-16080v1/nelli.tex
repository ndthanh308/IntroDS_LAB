...
%This issue highlights the need for developing new frontends or adapting existing ones to better align with MLIR's level of abstraction, so that the full potential of MLIR's optimization and transformation capabilities can be harnessed.

% Multi-Level Intermediate Representation (MLIR) is an intermediate representation and associated infrastructure designed to provide modular and extensible components for building domain-specific compilers. 
% MLIR models programs at an intermediate level of abstraction, between high-level source code and low-level machine code, in order to bridge the gap between different representations and optimizations for various hardware targets and thus make it easier to perform optimizations and transformations across different levels of abstraction.
% However, many existing frontend tools used to transform high-level source code into intermediate representations operate at a much higher level of abstraction than MLIR, and thus can obscure the semantics (meaning) and mechanics (functionality) of the fundamental transformations available in MLIR---and therefore make it difficult to utilize fully MLIR's capabilities for code optimization and transformation.
%However, since MLIR models programs at an intermediate level of abstraction, most extant frontends are at a very high level of abstraction, thereby obscuring the semantics and mechanics of the fundamental transformations available in MLIR.
...
However, since MLIR models programs at an intermediate level of abstraction, and most extant frontends are at a very high level of abstraction, the semantics and mechanics of the fundamental transformations available in MLIR are difficult to investigate and employ in and of themselves.
To address these challenges, we have developed \texttt{nelli}, a lightweight, Python-embedded, domain-specific, language for generating MLIR code. 
...
%In approaching MLIR as an existing repository of useful dialects and transformations, one faces the challenge of a lack of a point of ingress that is unencumbered by a particular set of assumptions about the roles of the existing dialects and their mutual relationships.
...
\ref{listing1}; \ian{Should this listing be moved closer?} note that the third loop nest is readily identified as
...
In order to remedy this state of affairs, we present \texttt{nelli}\footnote{\url{https://github.com/makslevental/nelli}},
...
\textit{lightweight}). \ian{Why is being lightweight important?}
...
In the following, we discuss in greater detail \texttt{nelli} design goals, the eDSL implementation (in Python) approaches that we investigated, and the implementation details of our chosen approach.
We also present three use cases: a kernel tuner that uses a black-box,
gradient-free, optimizer \citep{nevergrad} (demonstrating the power and convenience of Python interoperability);
a pipeline for lowering kernels to target GPUs and then evaluating performance on an Raspberry Pi edge device (demonstrating ease
of integration with LLVM downstream of MLIR); and 
a pipeline for translating parallelizable kernels to OpenMP programs (demonstrating value for HPC).
%We also present several use cases that, respectively, demonstrate the power and convenience of Python interoperability by implementing a kernel tuner using a black-box, gradient-free, optimizer \citep{nevergrad}; we also demonstrate ease of integration with LLVM (downstream of MLIR) by implementing a pipeline for lowering kernels to target GPUs and then evaluating performance on an edge device (Raspberry Pi); we also demonstrate value for HPC by exhibiting a pipeline for translating parallelizable kernels to OpenMP programs. 
...
Practically this means that MLIR is comprised by a collection of utilities
for 
\begin{enumerate}
...
MLIR, i.e., such that the source language implementation isn't itself
of research quality (see Section \ref{sec:Related-Work}). In addition,
...
For the purposes of this discussion, an intermediate-level dialect
...
The scope of such an undertaking is slightly improved (narrowed) by
