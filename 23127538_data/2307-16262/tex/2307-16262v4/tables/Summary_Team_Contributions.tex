...
\begin{sidewaystable*}
    \footnotesize
    \centering
   \caption{Summary of the participating teams algorithm for Medico 2020. {Here,  ``Aug." = augmentation used, ``SGD" = Stochastic gradient descent,  ``GAN" = generative adversarial network, \\  ``ASPP" = Atrous Spatial Pyramid Pooling, and ``AP" = Average precision.}}
    \label{table:challenge_summary2020} 
...
AI-TCE~\citep{nathan2020efficient}  & Multi-Supervision Net  & EfficientNetB4 & Encoder-Multi Supervision Decoder & Acc and DSC & Yes & Categorical cross-entropy + Dice loss & Adam \\ \hline
...
{HBKU UNITN SIMULA}~\citep{trinh2020hcmus} & Residual module, Inception module, Adaptive CNN with U-Net and PraNet  & U-Net and ResNet-50 & Cascade of residual blocks and inception module  & mIoU and DSC & Yes &  {Bce + Dsc loss}  & Adam\\ \hline
...
UNITRK~\citep{khadka2020transfer} &	Knowledge transfer using UNet & Pre-trained U-Net model & Encoder-decoder & {mIoU and DSC} & Yes	& Compound loss of DSC and BCE &	Adam \\ \hline
...
{GeorgeBatch}~\citep{batchkala2020real}& U-Net &	None	& Encoder-decoder &	Acc and Speed	& Yes &	Non-Binarized IoU &	Adam \\ \hline %GS-CDT
...
VT~\citep{thambawita2020pyramid} & U-Net coupled with PYRA  &None & Encoder-decoder& mIoU and DSC &	Yes	& {BCEWithLogits} {Loss}	& RMSprop\\ \hline
...
% & {Instrument}	&  EM-Net	&  EfficientNet B3	&  Encoder-decoder & 	F1-Score	& Yes	& Dice Loss & 	Adam	\\ \hline
...
% None & %Ensemble	
% Generator network comprising of encoder-decoder framework & Acc, IoU, Dice, Precision, Recall & 	No	& {BCE and L1 loss}	& {Adam}	 \\\hline																
...
% None & Dual Decoder-Encoder+attention& 	DICE and Mean IoU	& yes	& {binary cross-entropy and dice loss}	& {Adam}		\\\hline															
% leen \citep{ali2021iterative}%Nikhil K Tomar et al. 
% & 	Polyp, Instrument & 	DDANet+FANet & 	Attention based CNN & 	Ensemble Encoder-decoder with feedback attention	& F1-score	& No	& {binary cross-entropy and dice loss}	& {Adam} \\\hline																	
% CV\&Med IIAI \citep{Keprate2021} %Sumit Pandey et al. 
% & 	Polyp, Instrument& 	Unet	& FCN	& Encoder-decoder	& Dice Score	& {Yes}	& Dice & 	Adam\\\hline																	
