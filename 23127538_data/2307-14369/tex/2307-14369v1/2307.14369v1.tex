...
Here, one adopts a simple interpretation for the imaginary velocity: if it could be measured, then there exists a physical mechanism preventing this measure to be greater than $c$.  
...
\begin{eqnarray}\label{eq:integ}
    Z_N^+=\frac{1}{N!}\left(\frac{2\pi m k T}{h^2}\right)^{3N/2}V.
\end{eqnarray}
...
On the other hand, adopting the transformation $(ii)$, there is no change in the partition function given by Eq. (\ref{eq:integ}). To see that is sufficient to replace $v^2$ by $(iv)^2=-v^2$ in Eq. (\ref{eq:integ}). For the $N$-particle system the resulting partition function is just
\begin{eqnarray}
    Z_N^-[v]= Z_N^+.
\end{eqnarray}
...
Despite transformation $(i)$, which leads to an anomalous partition function, the result for $N$ even shown in Eq. (\ref{eq:entropy_0}) is the usual entropy for positive mass particles which is in accordance with the discussion of the preceding subsection. 
...
\begin{eqnarray}\label{eq:entropy_2}
    S^-[T]=-k_B\ln Z_N^+-\ln (-1)^{2n-1}=-k_B\ln Z_N^+- ik_B(2n-1)\pi,
\end{eqnarray}
...
\begin{eqnarray}\label{eq:erim}
    \mathrm{Re}S^-[T]=-k_B\ln Z_N^+ ~~\mathrm{and}~~\mathrm{Im}S^-[T]=- k_B(2n-1)\pi.
\end{eqnarray}
...
\begin{eqnarray}
      S^-[v]=-k\ln Z_N^+,
\end{eqnarray}
\noindent which is in accordance with the assumption that if a negative mass has imaginary velocity, then it has measurable kinetic energy which contributes to the partition function and entropy.
...
On the other hand, using $(ii)$, the results for the partition function and entropy are the expected ones. Nonetheless, even though it results in an imaginary linear momentum, it gives rise to a real kinetic energy, which has actual physical implications on the partition function and entropy. 
...
\begin{eqnarray}
    S_{total}=S_N^+ + S_N^- + (1-q)S_N^+S_N^-=-k_B\bigl[2\ln Z_N^+ + (1-q)\ln^2 Z_N^+\bigr],
\end{eqnarray}
\noindent where the possible interaction between $S_N^+$ and $S_N^-$ is measured by the entropic-index $q$. If the possible measurements of $S_{total}$ tend to linearity in $\ln Z_N^+$, then $q\rightarrow 1$ while for a quadratic behavior, $q<1$. Thus, $q$ may be used to measure the possible interaction between positive and negative mass particles in the system. 
...
%12
\bibitem{C.de.Beauregard.Comptes.Rendus.252.1737.1961}C. de Beauregard, Comptes Rendus \textbf{252}, 737 (1961).
...
%18
\bibitem{R.T.Hammond.arXiv.1308.2683.grqc}R. T. Hammond, arXiv:1308.2683[gr-qc]

%19
\bibitem{W.B.Bonnor.Gen.Relat.Grav.21(11).1143.1989}W. B. Bonnor, Gen. Relat. Grav. \textbf{21}(11), 1143 (1989).
...
%26
\bibitem{P.Medley.D.M.Weld.H.Miyake.D.E.Pritchard.W.Ketterle.Phys.Rev.Lett.106.195301.2011}P. Medley {\it et al.}%, D. M. Weld, H. Miyake, D. E. Pritchard, W. Ketterle
