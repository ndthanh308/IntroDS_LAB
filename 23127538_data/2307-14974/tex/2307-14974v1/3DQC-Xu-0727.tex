...
\affiliation{$^{2}$Department of Physics, Capital Normal University, Beijing 100048, China}
...
and have experienced considerable advancements in recent years~\cite{Taylor2017Science,Taylor2017PRB,Fritz2012PRL,ZhangFan2013PRL,Slager2015PRB,Brouwer2017PRL,FangChen2017PRL,
	...
	Parameswaran2020PRL,Zhao2020PRL,AYang2020PRL,Xu2020NJP}. These phases possess edge states of 
...
characterize the chiral hinge modes when their number is equal to four~\cite{Wang2021PRL}. 
...
time-reversal symmetry (TRS) that support $8$ helical pairs of hinge states, giving rise to 
...
\begin{equation} \label{h_without_trs}
	\hat{H}_c=\sum_{\bm r} [M \hat{c}_{{\bf r}}^{\dagger} \tau_z \sigma_0 \hat{c}_{{\bm r}}
	+ \sum_{{\bm d} } \hat{c}_{{\bm r} +{\bm d}}^{\dagger} T_c(\hat{\bm d}) \hat{c}_{ {\bm r} } ],
\end{equation}
where $\hat{c}_{\bm d}^{\dagger} =  (\hat{c}_{{\bm d}, 1}^{\dagger}, \hat{c}_{{\bm d}, 2}^{\dagger}, 
\hat{c}_{{\bm d}, 3}^{\dagger}, \hat{c}_{{\bm d}, 4}^{\dagger})$
with $\hat{c}_{{\bm d},\alpha}^{\dagger}$ ($\hat{c}_{{\bm d},\alpha}$) creating (annihilating) a 
particle of the $\alpha$th component at the lattice 
site of position ${\bm d}$,
...
results in the quantized conductance. For each chiral mode, there is four-fold degeneracy due
...
hinge modes~\cite{Wang2021PRL}. The quadrupole moment is defined as~\cite{Cho2019PRB,Hughes2019PRB,Xu2021PRB,Shen2020PRL}
\begin{equation}
...
occupied eigenstates of the first-quantization Hamiltonian at $k_z$ under periodic boundary conditions,
...
If we use the original real-space coordinates of the qusicrystal lattice to calculate the quadrupole moment,
...
quadrant (see Ref.~\cite{Tao2023arxiv} or Supplemental Material). 
...
\emph{Higher-order Weyl-like semimetal}.---
...
and a quadrupole insulator as $k_z$ varies, similar to the Dirac point in the regular case~\cite{Hughes2018PRB,Zeng2020PRB}.
...
twofold degenerate points. The split regions develop the quantum anomalous Hall insulating
...
with those of the Fermi arcs in Figure~\ref{Fig4}(d).
