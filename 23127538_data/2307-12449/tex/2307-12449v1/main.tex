...
\begin{table}[!b]
    \vspace{-2mm}
    \centering
    \caption{Performance of AdaP in terms of test loss, plotted against increasing QNN circuit width (qubit), while training on 2000 MNIST-10 samples with a learning rate $\alpha$=0.005. WEPRO consistently performs better than vanilla even with increase in quantum circuit size \& complexity.}
    \label{scalability}
    \vspace{-2mm}
    \begin{tabular}{cccc}
    \cmidrule(lr){1-4}
    QNN Width    & Parameters & Speedup & Shots Savings \\
    \cmidrule(lr){1-4}
    4-Q     & 68        & 1.16      & 1.38$\times$  \\
    8-Q     & 168       & 1.78      & 2.12$\times$   \\
    12-Q    & 300       & 1.54      & 1.53$\times$    \\ 
    \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[!t]
    \vspace{-4mm}
    \centering
    \caption{Performance of WEPRO when limiting the prediction distance $d$ in range (0, 1] for QNN and VQE workloads. We can see that even in the worst case when we are predicting very close by, both AdaP and NaP perform almost similar to vanilla while requiring significantly lesser number of shots.}
    ...
    \cmidrule(lr){1-4}
    Algorithm    & AdaP & NaP & Shots Reduction\\
    \cmidrule(lr){1-4}
...
\subsection{Impact of Choices \& Noise} \label{choices}
