...
\end{align}
where $\gamma = 1.4$ is the ratio of specific heat a diatomic gas. Again, we assume that the width, $a(x)$, does not change over time. 
...
\end{align*}
If the variables and entropy are both scaled by $a(x)$ for the quasi-1D shallow water and compressible Euler equations, we can show the following lemma:
\begin{lemma}
The entropy variables for the quasi-1D shallow water and compressible Euler equations are the same as the entropy variables for the standard shallow water and compressible Euler equations.
\end{lemma}
The proof is given in \ref{appendix:entropy_variables}. We note that it is not generally true that scaling the conservative variables and entropy by $a(x)$ preserves the entropy variables; for example, this is not true of the scalar Burgers' equation with conservative variables $u$ and entropy $S(u) = u^2/2$. 
...
Recall that the entropy variables $v_1, v_2$ for the quasi-1D shallow water equations are
\begin{align*}
...
We modify the numerical fluxes from Ranocha \cite{ranocha2018comparison} to account for spatially varying width $a(x)$ in the quasi-1D Euler equations. These new quasi-1D fluxes can be written as follows:
\begin{align}
...
Recall that the entropy variables for the quasi-1D compressible Euler equations are
\begin{align*}
...
\]
Suppose our domain $\Omega$ is now decomposed into multiple interval subdomains $D^k$, each of which has some size $\LRb{D^k} = h_k$. Transforming the PDE to the reference interval allows us to define a local formulation (similar to local formulations for discontinuous Galerkin (DG) methods discontinuous Galerkin (DG) methods \cite{hesthaven2007nodal}) over each element:
\begin{align}
...
If we multiply Eq.\ (\ref{eq:global_DG_formulation_dis}) by the vector of entropy variables $\bm{v}^T$, we have 
\begin{align}
...
Semi-discrete conservation is derived by testing by $\fnt{1}$ and summing over all elements $D^k$
\[
...
\[
\fnt{1}^T\LRp{2\fnt{Q} \circ \fnt{F}_{\rm nonsym}}\fnt{1} = \fnt{1}^T \fnt{a} \circ \LRp{\fnt{Q} \fnt{p}} \approx \int_{D^k} g a \pd{p}{x}.
\]
...
\begin{theorem}
For the quasi-1D shallow water and compressible Euler equations, (\ref{eq:global_DG_formulation_dis}) is a high order accurate approximation of (\ref{eq:SWE_quasi_1D}) and 
(\ref{eq:Euler_q1D}) under periodic or wall boundary conditions.   
\end{theorem}
\begin{proof}
We use the strong form of the discretization (\ref{eq:global_DG_formulation_strong}) to show high order accuracy. First, we note that the symmetric terms in the entropy conservative flux $\bm{f}_{EC}$ are consistent numerical fluxes for conservative terms in (\ref{eq:SWE_quasi_1D}) and 
(\ref{eq:Euler_q1D}). Since these flux terms are consistent and symmetric, the proof of high order accuracy in \cite{crean2018entropy} holds (see also \cite{fjordholm2012arbitrarily, fisher2013discretely}). All that remains is to show that the non-conservative terms in $\bm{f}_{EC}$ induce a high order accurate approximations to the non-conservative terms in (\ref{eq:SWE_quasi_1D}) and (\ref{eq:Euler_q1D}). 

Let $\fnt{F}_{\rm nc}$ denote the flux matrix corresponding to the scalar non-conservative terms in either the quasi-1D shallow water or compressible Euler equations, and let $\fnt{D} = \fnt{M}^{-1}\fnt{Q}$ denotes the SBP differentiation matrix. For the quasi-1D shallow water equations, the non-conservative terms in (\ref{eq:SWE_quasi_1D}) are $gah\pd{}{x}\LRp{h+b}$. The corresponding non-conservative flux terms are $\frac{1}{2}ga_Lh_L(h_R+b_R)$, and following \cite{gassner2016split}, the flux differencing approximation reduces to 
\begin{align*}
...
\end{align*}
This corresponds to $g \fnt{a} \circ \fnt{h} \circ \LRp{\fnt{D} \LRp{\fnt{h} + \fnt{b}}}$, which is a consistent high order approximation of $gah\pd{}{x}\LRp{h+b}$. 
...
\]
where we have used that $\sum_j \fnt{Q}_{ij} = 0$ for any first order accurate differentiation matrix \cite{gassner2016split}. This corresponds to $\fnt{a}\circ \LRp{\fnt{D}\fnt{p}}$, which is a consistent high order approximation of $a(x)\pd{p}{x}$. 

Finally, under the assumptions $a > 0$ and $h > 0$ (quasi-1D shallow water) or $\rho, p > 0$ (quasi-1D compressible Euler), $\bm{f}_{EC}$ is continuous with respect to its arguments and the boundary term $\LRp{\bm{f}_{EC}(\fnt{u}, \fnt{u}^+) - \bm{f}_{EC}(\fnt{u}, \fnt{u})}$ should be small for $\fnt{u} \approx \fnt{u}^+$. 
\end{proof}
...
We first examine convergence for the quasi-1D shallow water equations by comparing solutions to a fine grid solution on $8000$ elements of degree $N=3$. We follow \cite{qian2018positivity} and utilize the following initial conditions:
\begin{align}
	\begin{aligned}
		a(x) &= \exp(\sin(2\pi x))\\
...
Table~\ref{table:swe_convergence} shows the computed $L^2$ errors for degree $N$ meshes of $\mathrm{K}$ elements. We observe that the rate of convergence appears to approach $O(h^{N+1})$ as the mesh is refined. Optimal rates of convergence are also observed when testing with manufactured solution in Table~\ref{table:swe_convergence_manufactured} using the same initial condition. 
...
\end{equation}
The initial conditions are $h = 2, ahu = 20$, and boundary conditions are taken to be $ahu = 20$ at the inflow $x=0$ and $h = 1.85$ at the outflow $x = 500$. The solution is discretized using 200 uniform elements of degree $N=2$. The water height $h$ and computed Froude number $|u| / c$ where $c = \sqrt{g h}$ are shown in Figure~\ref{fig:swe_conv_div}, along with reference solution values taken from \cite{vazquez1999improved}. 
...
\]
which lie on the left/right of a discontinuity at $x = 0$. The nozzle width is also taken to be a discontinuous function 
\[
a(x) = \begin{cases}
1 	& x < 0\\
1.5 	& x \geq 0
...
\]
The solution is evolved until final time $T_{\rm final} =1 / 10$. Table~\ref{tab:ref_sol} shows computed convergence rates, which approach the optimal rate of $h^{N+1}$.
...
\begin{table}
	\begin{tabular}{|c|c|c|c|c|c|c|c|c|}
		\hline & \multicolumn{2}{|c|}{$N=1$} & \multicolumn{2}{c|}{$N=2$} & \multicolumn{2}{c|}{$N=3$} & \multicolumn{2}{c|}{$N=4$} \\
		\hline $\mathrm{K}$ & $L^2$ error & Rate & $L^2$ error & Rate & $L^2$ error & Rate & $L^2$ error & Rate \\
		\hline 2 & $1.116 \times 10^{-1}$ &-& $1.0274 \times 10^{-1}$ &-& $6.571 \times 10^{-2} $ &-& $3.429 \times 10^{-2}$ &- \\
		4 & $1.061 \times 10^{-1}$ & 0.07&  $4.575 \times 10^{-2}$ & 1.17 &  $ 1.666 \times 10^{-2}$ & 1.98 & $4.349\times 10^{-3}$  & 2.98  \\
		8 & $5.049 \times 10^{-2}$ & 1.07 & $1.475 \times 10^{-2} $  & 1.63  & $2.089 \times 10^{-3}$  & 3.0 & $2.604\times 10^{-4}$ & 4.06  \\
		16 & $2.001 \times 10^{-2} $ & 1.34 & $2.481 \times 10^{-3} $ & 2.57  & $1.416 \times 10^{-4}$ & 3.88 & $8.918  \times 10^{-6}$ & 4.87 \\
		32 & $7.111 \times 10^{-3} $ & 1.49 & $3.201 \times 10^{-4}  $ & 2.95  & $9.006\times 10^{-6}$ &  3.97 & $3.001 \times 10^{-6}$  & 4.89 \\
		\hline
	\end{tabular}
\caption{$L^2$ errors and convergence rates for the quasi-1D compressible Euler equations relative to a fine-grid reference solution.}% computed at $N_{\rm elem} = 2000$ and $N=5$.}
\label{tab:ref_sol}
\end{table}
...
\hline
\end{tabular}
\caption{$L^2$ errors and convergence rates for 1D quasi-Euler equation with a manufacture solution considered as reference solution.}
...
%\section{Proof of the non-symmetric Tadmor condition for the entropy conservative quasi-1D compressible Euler flux}
%\label{appendix:euler_proof}
%
%Recall that formulas for entropy conservative fluxes for the quasi-1D compressible Euler equations are given by
%\begin{align}
%    f_{\rho} &= \avg{\rho}_{\log}\avg{au}, \nonumber \\
%    f_{\rho u} &= \avg{\rho}_{\log}\avg{au}\avg{u} + \boxed{a_L\avg{p}}, \label{eq:Euler_quasi_1D_flux}\\
%    f_{E} &= \frac{1}{2}\avg{\rho}_{\log}\avg{au}\prodmean{u \cdot u} + \frac{1}{\gamma -1}\avg{\rho}_{\log}\avg{ \rho / p }^{-1}_{\log}\avg{au}+\prodmean{p \cdot au},  \nonumber   
...
%with logarithmic and product means
%\[
%\avg{\rho}_{\log} \coloneqq \frac{\jump{\rho}}{\jump{\log \thickspace \rho}} = \frac{\rho_L-\rho_R}{\log(\rho_L)-\log (\rho_R)}, \qquad \prodmean{u \cdot v} \coloneqq \frac{u_L v_R + u_R v_L}{2}.
%\]
...
\end{align}
The entropy variables $\bm{v}$ are then
\begin{align}
