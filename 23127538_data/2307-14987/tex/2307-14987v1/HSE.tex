...
\label{eq_HSE}
\end{align}
which shows that in the background of a time-independent uniform magnetic field ${\bs B}$, the neutral matter develops a helical current along the direction of the magnetic field. The associated helical magnetic conductivity is proportional to the temperature $T$, implying that the higher the temperature, the stronger the helical current. Equation~\eq{eq_HSE} is invariant under time reversal transformation, $t \to -t$, implying that the generated helical current is a dissipationless quantity.\footnote{Notice that magnetic field can also generate dissipationless (persistent) electric current in small metallic rings via the Aharonov-Bohm effect~\cite{Byers:1961zz,Deaver:1961zz,Cotescu2016}. This effect, however, has a different nature than the anomalous transport effects of the kind~\eqref{eq_HSE} since the persistent current in a ring (i) emerges in the transverse direction to the magnetic field and (ii) disappears in thermodynamic limit when the radius of the ring becomes infinite.}



As the helicity of massless fermions is similar to the zilch quantum number for photons~\cite{Lipkin1964}, the HSE~\eq{eq_HSE} is a counterpart of the Zilch Vortical Effect which shows how the polarization of photons is transferred in the rotating photon gas~\cite{Chernodub:2018era}. We have shown in Ref.~\cite{Ambrus:2020oiw} that the helicity and axial vortical effects play a complementary role that elegantly accounts for the discrepancy between the polarization of particles and anti-particles. Similarly, one can expect that the new HSE will also play a complementary role to the influence of the CSE on the polarization of particles in heavy-ion collisions discussed in Ref.~\cite{Buzzegoli:2022qrr}.
...
where
\begin{align}
 \kappa_\beta({\bs \mu}) = \frac{1}{4} \sum_{\substack{a, b, c = \pm 1 \\ abc=1}} \ln \left( 1 + e^{\beta (a\mu_V + b \mu_A + c \mu_H)} \right)\,,
 \label{eq_kappa_beta}
\end{align}
...
This paper is organized as follows. In the first part of the paper, we will be working with free massive fermions for which the helicity charge is a conserved quantity. A disadvantage of considering massive fermions is that, in the addition to the explicit breaking of chirality conservation, they have a frame-dependence in the definition of helicity. Therefore, while discussing the applications of the Helical Separation Effect at the end of the paper, we will turn to massless fermionic limit similarity to the generic tactics applied to the chiral degrees of freedom. We start our discussion from the discussion of the formal definition of the helicity operator in magnetic field, Sec.~\ref{sec_helicity_magnetic}, which leads us to the construction of the common mode solutions for the helicity operator and the Hamiltonian in Sec.~\ref{sec:modes}. Using the spectrum, the finite-temperature field theory at finite helical density is developed in Sec.~\ref{sec_finite_temperature} which brings us to the anomalous transport in Sec.~\ref{sec_anomalous_transport}. We derive not only the Helical Separation Effect~\eq{eq_HSE}, but also the corrections of the helical charges to the chiral magnetic and chiral vortical conductivities as well as to the heat transport. The interplay of the fluctuations of the helical charge density and the energy density -- supported by the Helical Separation Effect~\eq{eq_HSE} and the Helical Magnetic Heat Effect~\eq{eq_heat_flux} -- could, in principle, lead to the appearance of new hydrodynamic excitations. In Sec.~\ref{sec_helical_heat} we analyse this possibility for a {\it  neutral} plasma and show that the helical degree of freedom does not generate a new hydrodynamic wave if all chemical potentials vanish. Our conclusions are summarized in the last section. Throughout this paper, Planck units ($\hbar = c = k_B = 1$) and the $(+---)$ metric signature are adopted.
...
\begin{equation}
 ...
 \label{eq_eigen_H}
\end{equation}
where $\widetilde{U}_j \equiv \widetilde{U}_j(\bx)$ is a time-independent spinor field and the cumulative symbol $j$ labels distinct solutions to be discussed in detail later. Similarly, the anti-particle solutions of Eq.~\eqref{eq_dirac} can be obtained by charge conjugation, 
\begin{equation}
 V_j = i \gamma^2 
...
\end{align}
where the helicity operator and its conjugated form take, respectively, the following forms:
\begin{align}
...
\begin{equation}
 \widetilde{U}_j = \begin{pmatrix}
     \mathfrakE^+_j \\
     2\lambda_j 
     \mathfrakE^-_j
 \end{pmatrix} \otimes \phi_j, \qquad 
 \widetilde{V}_j = \begin{pmatrix}
...
The helicity operator~\eq{eq_h} shares similarity with the zilch quantum number which -- despite a seemingly non-local definition --  is a well-defined quantity associated with polarization of photons~\cite{Lipkin1964}. 
...
In this section, we review the eigensystem of a charged spinor particle in the background of the uniform homogeneous magnetic field with the emphasis on the role of the helical degrees of freedom.
...
Focussing now on the particle modes only, we substitute Eq.~\eqref{eq_eigen_p} into Eq.~\eqref{eq_eigen_h_phij} and we obtain
\begin{multline}
 \begin{pmatrix}
...
\begin{equation}
 ...
 \sigma = {\rm sgn}(qB).
\end{equation}
With the above notation, the functions $f^\pm_j(\xi_j)$ satisfy the following simplified differential equations 
\begin{gather}
...
\end{gather}
As is well known, the above equations have regular solutions given in terms of the Hermite polynomials when $\nu_j^\pm$ is a non-vanishing integer:
\begin{equation}
...
Using now the recurrence relations $H'_\nu(z) = \nu H_{\nu -1}(\xi)$ and $\xi H_\nu(\xi) = H_{\nu + 1}(\xi) + \nu H_{\nu - 1}(\xi)$, it is possible to derive the relation
\begin{multline}
 \left(\frac{\partial}{\partial \xi_j} - \frac{1 \mp \sigma}{2} \right) H_{\nu^\pm_j}(\xi_j) \\
 = \nu^\pm_j \frac{1 \pm \sigma}{2} H_{\nu^\pm_j - 1} - \frac{1 \mp \sigma}{2} H_{\nu_j^\pm + 1},
\end{multline}
which implies that
...
\begin{equation}
 \int_{-\infty}^\infty d\xi_j e^{-\xi_j^2 / 2} H_{n_j}(\xi_j) H_{n_j'}(\xi_j) = \sqrt{2\pi} n! \delta_{n_j,n_j'}.
\end{equation}
...
Normalizing now the modes using the Dirac inner product in Eq.~\eqref{eq_norm_def}, we find the normalization $\mathcal{C}_j^\sigma = (|qB| / 4\pi)^{1/4}$ corresponding to the LLL mode:
\begin{align}
...
which leads to 
\begin{align}
 \mathcal{C}^\sigma_j &= \frac{\mathcal{N}_j}{2\sqrt{n_j!}} \left(\frac{|qB|}{\pi}\right)^{1/4} \mathfrak{p}^{2\sigma \lambda_j}_j, \nonumber\\
 \mathcal{C}^{-\sigma}_j &= \frac{-i \mathcal{N}_j \sqrt{n_j}}{4\lambda_j \sqrt{n_j!}} \left(\frac{|qB|}{\pi}\right)^{1/4} \mathfrak{p}^{-2\sigma \lambda_j}_j,
\end{align}
...
\begin{align} 
 \phi^+_j(\bx) &= 
 \frac{e^{i p_j^y y + i p_j^z z}}{4\pi \sqrt{n_j!}}
 e^{-\xi_j^2/4} \left(\frac{|qB|}{\pi}\right)^{1/4}\nonumber\\
 & \times 
 \begin{pmatrix}
...
% 
 ...
 &\times 
 \begin{pmatrix}
  \mathfrak{p}^{2\lambda}_j \sqrt{n_j} H_{n_j-1}(\xi_j) \\ 
...
% 
 \phi^{c;+}_j(\bx) &= 
 \frac{e^{-i p_j^y y - i p_j^z z}}{4\pi \sqrt{n_j!}}
 e^{-\xi_{j;c}^2/4} \left(\frac{|qB|}{\pi}\right)^{1/4}\nonumber\\
 & \times 
 \begin{pmatrix}
...
% 
 \phi^{c;-}_j(\bx) &= 
 \frac{e^{-i p_j^y y - i p_j^z z}}{4\pi \sqrt{n_j!}}
 e^{-\xi^2_{j;c}/4} \left(\frac{|qB|}{\pi}\right)^{1/4} \nonumber\\
 &\times 
 \begin{pmatrix}
...
\end{equation}
where the $n = 0$ term corresponds to the contribution coming from the lowest Landau level (LLL), which exists only when $2\sigma \lambda_j p^z_j > 0$. 
...
Promoting now $\psi(x)$ to a Fock-space operator, the canonical anti-commutation relation
\begin{equation}
...
Employing the same steps as above for for the derivative of the axial four-current $\partial_\mu J^\mu_A$ reveals the partial conservation of the axial current (PCAC),
\begin{equation}
...
\label{eq_d_JH}
\end{align}
Note however that the conservation of the helicity current is broken perturbatively by helicity-violating pair annihilation processes, as discussed in Ref.~\cite{Ambrus:2019khr}. In a setting of real physical systems, such as quark-gluon plasma, the non-conservation of helicity due to perturbative scattering has the same timescale as the non-conservation of chirality due to dynamical mass generation {\cite{Astrakhantsev:2019zkr}, which allows us to treat the helicity and chirality on equal basis~\cite{Ambrus:2019khr}.
...
Thanks to the helicity eigenmode solutions discussed in Sec.~\ref{sec:modes}, both vector and helicity charge operators $\widehat{Q}_{V/H}$ are diagonal and, together with the Hamiltonian $\widehat{H}$, they admit the following Fock space representation:
\begin{align}
...
\label{eq_bold_mu}
\end{equation}
One should bear in mind that the axial chemical potential corresponds vanishes $\mu_A$ vanishes for a massive particle, since the axial charge is not conserved if the particle mass is nonzero, cf.~\eq{eq_d_QA}.
...
$-1$, & \quad\ & \text{for anti-particles}.
\end{tabular}
\right.
\end{equation}
Equation~\eq{eq_Ecal} takes into account the difference in the positions of the Fermi levels for particles with vector, axial, and helical charges which are encored in the corresponding chemical potentials $\mu_V$, $\mu_A$, and $\mu_H$, respectively. Despite seemly complicated form of this equation, it has a simple meaning consistent with Eq.~\eq{eq_bold_mu}: the vector chemical potential $\mu_V$ controls the excess of particles over antiparticles which, in turn, are distinguished by the quantum number $\varsigma_j$. The axial chemical potential $\mu_A$ applies equally to particles and antiparticles but distinguishes the polarization states described by $\lambda_j$. Finally, the helical chemical potential $\mu_H$ is sensitive to the helicity of the particle which is opposite for particles and antiparticles, hence the product $\varsigma_j \lambda_j$ in the last term of Eq.~\eq{eq_Ecal}.
...
\end{equation}
where $A_0$ corresponds to the lowest Landau level (with $n_j = 0$). The terms $A_0$ and $A_n$ are obtained after performing the $p^y_j$ and $p^z_j$ integrations, as well as the sum over polarization $\lambda_j$:
\begin{equation}
...
In the case of the scalar condensate, we have 
\begin{equation}
...
while for $n_j > 0$, 
\begin{multline}
 \overline{U}_j U_j {\Big|}_{n_j > 0} = \frac{M e^{-\xi_j^2 / 2}}{16 \pi^2 E_j n_j!} \sqrt{\frac{|qB|}{\pi}} \\\times 
 (\mathfrak{p}_{2\sigma\lambda_j} H_{n_j}^2 + n_j \mathfrak{p}_{-2\sigma\lambda_j}^2 H^2_{n_j - 1}),
\end{multline}
where it is understood that the Hermite polynomials take the argument $\xi_j$. In order to evaluate the chiral condensate $\braket{\bar{\psi}\psi}$, the integration with respect to $p^y_j$ can be performed using the orthogonality relation for the Hermite polynomials,
\begin{equation}
...
Specifically, we have 
\begin{equation}
 \overline{U}_j U_j = \frac{M|qB|}{8\pi^2 E_j} 
 \begin{cases}
     2\theta(\sigma\lambda_j p^z_j), & n_j = 0,\\
     1, & n_j > 0.
 \end{cases}
\end{equation}
The final result reads
\begin{equation}
 \bar{\psi} \psi = \frac{M|qB|}{4\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma} \int_0^\infty \frac{dp^z}{E(e^{\beta\mathcal{E}_\varsigma} + 1)},
\end{equation}
where the summation runs over the helicity quantum number $\lambda = \pm 1/2$, particle-antiparticle number $\varsigma = \pm 1$ and $\mathcal{E}_\varsigma = E - \varsigma \mu_V - 2\lambda \mu_A - 2\varsigma\lambda \mu_H$, as introduced in Eq.~\eqref{eq_Ecal}.
...
%\subsection{Lowest Landau level contributions}

On the LLL, an explicit computation shows that 
\begin{equation}
 (\phi^\sigma_{0,p^y,p^z,\lambda})^\dagger \sigma^y \phi^\sigma_{0,p^y,p^z,\lambda} = 
 (\phi^\sigma_{0,p^y,p^z,\lambda})^\dagger \sigma^z \phi^\sigma_{0,p^y,p^z,\lambda} = 0,
\end{equation}
such that 
\begin{equation}
 \mathcal{J}^x_{\ell ;j}\rfloor_{n_j = 0} = \mathcal{J}^y_{\ell ;j}\rfloor_{n_j = 0} = 0.
\end{equation}
The temporal and vertical components can be computed as follows:
\begin{align}
 \mathcal{J}^t_{V;j} \rfloor_{n_j = 0} &= \frac{e^{-\xi_j^2 / 2}}{4\pi^2} \sqrt{\frac{|qB|}{\pi}} \theta(\sigma \lambda_j p_j^z),\nonumber\\ 
% 
 \mathcal{J}^z_{V;j}\rfloor_{n_j = 0} &= \frac{2\lambda_j \sigma |p_j^z| e^{-\xi_j^2 / 2}}{4\pi^2 E_j} \sqrt{\frac{|qB|}{\pi}} \theta(\sigma \lambda_j p_j^z), \nonumber\\
% 
 \mathcal{J}^t_{A;j}\rfloor_{n_j=0} &= \frac{2\lambda_j |p_j^z| e^{-\xi_j^2 / 2}}{4\pi^2 E_j} \sqrt{\frac{|qB|}{\pi}} \theta(\sigma \lambda_j p_j^z),\nonumber\\ 
% 
 \mathcal{J}^z_{A;j}\rfloor_{n_j=0} &= \frac{\sigma e^{-\xi_j^2 / 2}}{4\pi^2}  \sqrt{\frac{|qB|}{\pi}} \theta(\sigma \lambda_j p_j^z). 
 \label{eq_Jmu_sesqui}
\end{align}
...
\begin{align}
 \begin{pmatrix}
  ...
  J^t_{A;0} \\
  J^t_{H;0}
 \end{pmatrix} &= \frac{|qB|}{4\pi^2} \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z}{e^{\beta \mathcal{E}_\varsigma} + 1}
 \begin{pmatrix} 
  \varsigma \\ 
  2\lambda p^z / E \\ 
  2\lambda \varsigma  
...
% 
 ...
 \end{pmatrix},
\end{align}
where $E = \sqrt{M^2 + p_z^2}$ on the LLL.
...
as follows:
\begin{align}
 \mathcal{J}_{V;j}^t &= \frac{e^{-\xi_j^2/2}}{16\pi^2 n_j!} \sqrt{\frac{|qB|}{\pi}} (\mathfrak{p}^2_{2\sigma\lambda_j} H_{n_j}^2 
 + \mathfrak{p}^2_{-2\sigma\lambda_j} n_j H_{n_j-1}^2), \nonumber\\
% 
 \mathcal{J}_{V;j}^y &= -\frac{qB e^{-\xi_j^2/2}}{8\pi^2 E_j(n_j-1)!} \sqrt{\frac{2}{\pi}} H_{n_j} H_{n_j - 1}, \nonumber\\
% 
 \mathcal{J}_{A;j}^z &= \frac{\sigma e^{-\xi_j^2/2}}{16\pi^2 n_j!} \sqrt{\frac{|qB|}{\pi}} (\mathfrak{p}^2_{2\sigma\lambda_j} H_{n_j}^2 - \mathfrak{p}^2_{-2\sigma\lambda_j} n_j H_{n_j-1}^2),\nonumber\\
% 
 \begin{pmatrix}
...
\begin{align}
 ...
 \int_0^\infty \frac{dp_j^z}{e^{\beta \mathcal{E}_j^\varsigma} + 1}
 \begin{pmatrix} 
  \varsigma \\ 
  2\lambda \mathfrakE_+ \mathfrakE_- \\ 
  2\lambda \varsigma  
 \end{pmatrix}, \nonumber\\
 J^z_{\ell ;n_j>0} &= 0.
\end{align}
...
In the case of the energy-momentum tensor $\Theta^{\mu\nu}$, the sesquilinear forms $\mathcal{T}^{\mu\nu}(V_j,V_j)$ corresponding to the anti-particle modes can be related to those pertinent to the particle modes via
\begin{equation}
...
\begin{align}
 ...
 &= \frac{|qB|^{3/2} e^{-\xi_j^2/2}}{8\pi^2 E_j (n_j - 1)! \sqrt{\pi}}W_{\xi_j}(H_{n_j-1}, H_{n_j}),
\end{align}
...
\begin{align}
 \Theta^{tt}_0 &= \frac{|qB|}{4\pi^2} \sum_{\lambda, \varsigma} \int_0^\infty \frac{dp^z E}{e^{\beta \mathcal{E}_\varsigma} + 1}, \nonumber\\
%
 \Theta^{tz}_0 = \Theta^{zt}_0 &= \frac{qB}{4\pi^2} \sum_{\lambda,\varsigma} 2 \varsigma\lambda \int_0^\infty \frac{dp^z p^z}{e^{\beta\mathcal{E}_\varsigma} + 1},\nonumber\\
% 
 \Theta^{zz}_0 &= \frac{|qB|}{4\pi^2} \sum_{\lambda,\varsigma} \int_0^\infty \frac{dp^z p_z^2}{E(e^{\beta\mathcal{E}_\varsigma} + 1)}.
\end{align}
...
\begin{align}
 \int_{-\infty}^\infty dp^y_j \mathcal{T}^{tt}_j{\Bigl|}_{n_j > 0} &= \frac{E_j |qB|}{8\pi^2}, \nonumber\\
 \int_{-\infty}^\infty dp^y_j \mathcal{T}^{tz}_j{\Bigl|}_{n_j > 0} &=  \int_{-\infty}^\infty dp^y_j \mathcal{T}^{zt}_j{\Bigl|}_{n_j > 0} =
 \frac{p^z_j |qB|}{8\pi^2}, \nonumber\\
 \int_{-\infty}^\infty dp^y_j \mathcal{T}^{xx}_j{\Bigl|}_{n_j > 0} &= \int_{-\infty}^\infty dp^y_j \mathcal{T}^{yy}_j{\Bigl|}_{n_j > 0} = \frac{n_j (qB)^2}{8\pi^2 E_j},\nonumber\\
 \int_{-\infty}^\infty dp^y_j \mathcal{T}^{zz}_j{\Bigl|}_{n_j > 0} &= \frac{(p^z_j)^2 |qB|}{8\pi^2 E_j}.
\end{align}
It can be seen that $\mathcal{T}^{tz}_j = \mathcal{T}^{zt}_j$ are odd with respect to $p_j^z \rightarrow -p_j^z$, thus $\Theta^{tz}_n = \Theta^{zt}_n = 0$ for $n > 0$. 
The nonvanishing components of the contribution of the higher Landau levels to the expectation value of the energy-momentum tensor $\Theta^{\mu\nu}_{n > 0}$ are given by
\begin{align}
 \Theta^{tt}_{n>0} &= \frac{|qB|}{4\pi^2} 
 \sum_{\lambda,\varsigma} \int_0^\infty \frac{dp^z E}{e^{\beta \mathcal{E}_\varsigma} + 1}, \nonumber\\
% 
 \Theta^{xx}_{n>0} = \Theta^{yy}_{n>0} &= 
 \frac{n |qB|^2}{4\pi^2} \sum_{\lambda,\varsigma} 
 \int_0^\infty \frac{dp^z}{E(e^{\beta \mathcal{E}_\varsigma} + 1)},\nonumber\\
% 
 \Theta^{zz}_{n>0} &= \frac{|qB|}{4\pi^2} \sum_{\lambda,\varsigma} \int_0^\infty \frac{dp^z\, p_z^2}{E(e^{\beta \mathcal{E}_\varsigma} + 1)}.
\end{align}
...
For a classical (non-quantum) fluid in thermodynamic equilibrium, the charge currents $J^\mu_\ell$ and stress-energy tensor $\Theta^{\mu\nu}$ take the ideal fluid form,
\begin{equation}
...
In the absence of the magnetic field, the Dirac fluid at non-vanishing V/A/H chemical potentials has the following pressure~\cite{Ambrus:2019khr}
\begin{equation}
 P{\Bigl|}_{B = 0} = \frac{1}{6\pi^2} \sum_{\lambda, \varsigma} \int_0^\infty \frac{dp\, p^4}{E(e^{\beta \mathcal{E}_\varsigma}+ 1)}.
\end{equation}
In the vanishing mass limit, the pressure $P$ reduces to
\begin{multline}
 P{\Bigl|}_{B = 0} = \frac{7\pi^2}{180\beta^4} + \frac{\bs{\mu}^2}{6\beta^2} + \frac{4 \mu_V \mu_A \mu_H}{\beta \pi^2} \ln 2 \\
 + \frac{\mu_V^4 + \mu_A^4 + \mu_H^4 + 6(\mu_V^2 \mu_A^2 + \mu_V^2 \mu_H^2 + \mu_A^2 \mu_H^2)}{12\pi^2} \\ 
 + O\left(\beta \mu_V \mu_A \mu_H \bs{\mu}^2\right).
\end{multline}
...
\subsection{Conductivities in thermometer frame}
...
\begin{align}
 \begin{pmatrix} 
  Q_V \\ Q_A \\ Q_H 
 \end{pmatrix} &= \frac{|qB|}{4\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z}{e^{\beta \mathcal{E}_\varsigma} + 1}
 \begin{pmatrix} 
  \varsigma \\ 
  2\lambda \mathfrakE_+ \mathfrakE_- \\ 
  2\lambda \varsigma  
 \end{pmatrix},\nonumber\\
 \epsilon &= \frac{|qB|}{4\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z\, E}{e^{\beta \mathcal{E}_\varsigma} + 1},\nonumber\\
 P + \Pi &= \frac{|qB|}{12\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z (E^2 - M^2)}{E(e^{\beta \mathcal{E}_\varsigma} + 1)},
\end{align}
where $\mathfrakE_\pm = \sqrt{1 \pm M^2 / E^2}$, Eq.~\eq{eq_beautiful_E}, and $E \equiv E_{n,p^z} = \sqrt{p_z^2 + M^2 + 2n |qB|}$ as in Eq.~\eq{eq_Ej_B}, while $\mathcal{E}_\varsigma = E - \varsigma \mu_V - 2\lambda \mu_A - 2\lambda \varsigma \mu_H$ according to Eq.~\eq{eq_Ecal}.
Similarly, $\pi_B = \Theta^{zz} - (P + \Pi)$ can be obtained as
\begin{equation}
 \pi_B = \frac{|qB|}{6\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z (p_z^2 - n |qB|)}{E(e^{\beta \mathcal{E}_\varsigma} + 1)}.
\end{equation}

The above expressions do not explicitly distinguish between LLL and higher energy-level contributions. On the contrary, the charge conductivities $\sigma^B_\ell$ and the heat conductivity $\sigma^B_\epsilon$ are fully determined by the contribution from LLL, receiving vanishing contributions from the $n > 0$ energy levels:
\begin{align}
 \begin{pmatrix}
  \sigma^B_V \\
  \sigma^B_A \\
  \sigma^B_H
 \end{pmatrix} &= \frac{q}{4\pi^2} \sum_{\lambda,\varsigma}
 \int_0^\infty \frac{dp^z}{E(e^{\beta \mathcal{E}_\varsigma} + 1)}
 \begin{pmatrix} 
  2\lambda p^z \\  
  \varsigma E \\
  p^z
 \end{pmatrix},\nonumber\\
 \sigma_B^\epsilon &= \frac{q}{4\pi^2} \sum_{\lambda,\varsigma} 2 \lambda \varsigma \int_0^\infty \frac{dp^z p^z}{e^{\beta\mathcal{E}_\varsigma} + 1}.
\end{align}


It is worth stressing that the property that only the single lowest Landau level determines the value of a current, while the degenerate higher level contributions mutually cancel each other, is the characteristic feature of the anomalous currents. At the level of the charged currents, it is an expected feature for the chiral magnetic conductivity $\sigma^B_V$, responsible for the Chiral Magnetic Effect~\cite{Fukushima:2008xe}, and the axial magnetic conductivity $\sigma^B_A$ which gives us the Chiral Separation Effect~\cite{Son:2004tq}. These conductivities are determined by the axial anomaly~\eq{eq_axial_anomaly}.What is less expected, that the similar statement is also true for the helical conductivity $\sigma^B_H$, which, apparently, is not connected to the axial anomaly but may be related to another form of an anomaly associated with the helical degrees of freedom~\cite{Ambrus:2019ayb,Ambrus:2019khr}.



In the limit of massless fermions, $E = |p^z|$ on the LLL and the $p^z$ integral can be performed analytically, when 
\begin{align}
 \int_0^\infty \frac{dp^z}{e^{\beta \mathcal{E}_\varsigma} + 1} &= \frac{1}{\beta} \ln\left(1 + e^{\beta \mathbf{q}_\varsigma \cdot \bs{\mu}}\right),\nonumber\\
 \int_0^\infty \frac{dp^z\, p^z}{e^{\beta \mathcal{E}_\varsigma} + 1} &= -\frac{1}{\beta^2} {\rm Li}_2(-e^{\beta \bs{q}_\varsigma \cdot \bs{\mu}}),
\end{align}
with $\mathbf{q}_\varsigma \cdot \bs{\mu} = \varsigma \mu_V + 2\lambda \varsigma \mu_H + 2\lambda \mu_A$ and ${\rm Li}_n(z) = \sum_{k = 1}^\infty z^k / k^n$ being the polylogarithm function \cite{DLMF}. Thus, in the high-temperature limit we obtain the following conductivities for massless fermions:
...
Let us now discuss the conductivities~\eq{eq_sigmas} in more detail. The meaning of these formulas is straightforward: in the background of magnetic field ${\bs B}$, the fermionic system produces electric ($\ell = V$), axial ($\ell = A$), and helical ($\ell = H$) currents [cf. Eq.~\eq{eq_j_B}],
\begin{align}
...
In the limit of a strong magnetic field, $|q B| \gg T^2$, the LLL contribution to the other observables becomes dominant, while the contributions with $n> 0$ are exponentially suppressed. Taking as before the limit of negligible masses, we get the following estimates:
\begin{align}
 \Theta^{tt}_0 = \Theta^{zz}_0 &= \frac{|qB|}{12 \beta^2} + \frac{|qB|}{4\pi^2} \bs{\mu}^2 
 + O(\beta \mu_V \mu_A \mu_H),
 \label{eq_Ttt}\\
 % \nonumber\\
 % &+ \frac{\beta |qB|}{4\pi^2} \mu_V \mu_A \mu_H [1 + O(\beta^2)],\nonumber\\
% 
 \Theta^{tz}_0 = \Theta^{zt}_0 &=  \frac{q B \mu_H}{\pi^2 \beta} \ln 2 + 
 \frac{q B}{2\pi^2} \mu_V \mu_A + O(\beta \mu_H \bs{\mu}^2),
 % \frac{q B \mu_H}{2\pi^2 \beta} \Bigg[2\ln 2 \nonumber\\
 % & + \frac{\beta^2}{12} (3 \bs{\mu}^2 - 2\mu_H^2) + O(\beta^4)\Bigg].
\end{align}
where the higher-order corrections to $\Theta^{tt}_0= \Theta^{zz}_0$ vanish when the product $\mu_V \mu_A \mu_H = 0$. Similarly, the corrections to $\Theta^{tz}_0 = \Theta^{zt}_0$ vanish if $\mu_H = 0$.

\subsection{Thermodynamic analysis}

Let us consider the free energy of the system, 
\begin{equation}
 \mathcal{F} = -\frac{|qB| V}{4\pi^2 \beta} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma} \int_{0}^\infty dp^z \ln(1 + e^{-\beta \mathcal{E}_\varsigma}),
\end{equation}
where $V$ is the (infinite) system size.
The free energy $\mathcal{F}$ is related to the partition function $\mathcal{Z} = {\rm tr}(\hat{\rho})$ via
\begin{equation}
 \mathcal{Z} = e^{-\beta \mathcal{F}}.
\end{equation}

Given the thermodynamic relation 
\begin{equation}
 d\mathcal{F} = \beta^{-2} \mathcal{S} d\beta - P dV - \bs{\mathcal{Q}} \cdot \bs{\mu},
\end{equation}
we can identify the total entropy $\mathcal{S}$, thermodynamic pressure $\mathcal{P}$ and total charge $\bs{\mathcal{Q}}$ as 
\begin{equation}
 \mathcal{S} = \beta^2 \frac{\partial \mathcal{F}}{\partial \beta}, \quad 
 \mathcal{P} = -\frac{\partial \mathcal{F}}{\partial V}, \quad 
 \mathcal{Q}_\ell = -\frac{\partial \mathcal{F}}{\partial \mu_\ell}.
\end{equation}
Since the free energy is linearly proportional to the system volume, $\mathcal{F} = f V$ with $f$ being the free energy density, it is clear that 
\begin{equation}
 P = -f = \Theta^{zz}.
\end{equation}
It is easy to check that $Q_{V/H} \equiv \mathcal{Q}_{V/H} / V = \partial \Theta^{zz} / \partial \mu_{V/H}$. A similar relation holds for $Q_A$ only in the case of massless fermions ($M = 0$ and $\mathfrakE_+ = \mathfrakE_- = 1$), confirming that the chiral charge is conserved only for massless fermions. 
...
\end{equation}
Since $\ln \mathcal{Z} = -\beta \mathcal{F}$, it is easy to uncover the Euler relation 
\begin{equation}
...
\end{equation}
Also we are in a position to evaluate the dynamical pressure $\Pi$:
\begin{align}
 \Pi &= \frac{1}{3}(\Theta^{xx} + \Theta^{yy} - 2\Theta^{zz}) = -\pi_B \nonumber\\
 &= \frac{|qB|}{6\pi^2} \sum_{n = 0}^\infty \sum_{\lambda, \varsigma}
 \int_0^\infty \frac{dp^z (n |qB| - p_z^2)}{E(e^{\beta \mathcal{E}_\varsigma} + 1)}. 
 \label{eq_Pi}
\end{align}
...
Let us consider a globally neutral plasma which is characterized by vanishing chemical potentials ${\bar \mu}_\ell = 0$ ($\ell = V,A,H$) at temperature ${\bar T}$, where the bar over the symbol means that we consider a mean global coordinate-independent quantity. We work in hydrodynamic approximation which assumes local thermodynamic equilibrium.
%we consider thermal fluctuations over the mean state which are characterized by inhomogeneous corrections to the mean quantities, $\mu_\ell(x) = {\bar \mu}_\ell + \delta \mu_\ell(x) \equiv \delta \mu_\ell(x)$ and $T(x) = T + \delta T(x)$, where $x = (t, {\bs r})$ is a four-coordinate. We will work 
We also assume the limit of a strong magnetic field, $q B \gg T^2$, where we set $q B > 0$ for convenience.

In strong magnetic field, the excess in vector or axial densities generates the coherent propagation of these charges along the direction of magnetic field in the form of a linear hydrodynamic excitation known as the Chiral Magnetic Wave~\cite{Kharzeev:2010gd}. In this wave, an excess in vector (axial) charge generates an axial (vector) current along the magnetic field which leads to a build-up of an axial (vector) charge and a cyclic repetition of the process.

We look for a 
%Since the heat wave is a 
similar excitation in terms of helical degrees of freedom. To this end we notice that the helical current ${\bs j}_H = \sigma^B_H {\bs B}$ and the heat current ${\bs j}_\epsilon = \sigma^B_\epsilon {\bs B}$, can be expressed, respectively, via Eq.~\eq{eq_sigma_H} for the helical conductivity~$\sigma^B_H$ and Eq.~\eq{eq_heat_conductivity} for the heat conductivity~$\sigma^B_\epsilon$. These relations show that helical fluctuations are coupled to the fluctuations in temperature similarly to the coupling of vector and axial charges (currents) in the Chiral Magnetic Wave. Therefore, one could expect that the helical degrees of freedom may propagate in terms of a hypothetical ``helical heat wave'' which corresponds to a coherent hydrodynamic excitation that combines energy density and helical fluctuations.

To figure out the existence of the helical heat wave as a linear hydrodynamic excitation, we consider the equations of motion derived from the charge (for all $\ell = V,A,H$), energy, and momentum conservation, namely:
\begin{subequations}
\begin{align}
...
We now consider infinitesimal fluctuations in a quiescent, neutral fluid of massless particles, at background temperature $T_0$, in a strong magnetic field $|qB| \gg T^2$. We consider the generic split of a hydrodynamic quantity $a$ into its global average $\bar{a}$ and fluctuation $\delta a$ as follows,
\begin{equation}
 a = \bar{a} +\delta a, \quad 
\delta a = \int d^3k \widetilde{\delta a}_\bk(\omega) e^{-i\omega t + i \bk \cdot \bx},
\label{eq:hydro_Fourier}
\end{equation}
where $\widetilde{\delta a}_\bk(\omega)$ represents the amplitude of its Fourier mode of wavenumber vector $\mathbf{k}$. For brevity, we henceforth suppress the explicit $\bk$ and $\omega$ dependencies of fluctuations.

We take as independent parameters the fluid temperature $T$, chemical potentials $\mu_\ell$, and macroscopic velocity $u^\mu$, with $\overline{T} = T_0$, $\bar{\mu}_\ell = 0$, and $\bar{u}^\mu = \delta^\mu_0$.
The velocity fluctuations lead to a modification $u^\mu = \bar{u}^\mu + \delta u^\mu$ with $\bar{u}^\mu = \delta^\mu_0$ that affects also the covariant magnetic field $B^\mu = \overline{B}^\mu + \delta B^\mu$, with 
\begin{equation}
...
%
 \bar{\sigma}^B_H &= \frac{q T_0 \ln 2}{\pi^2}, & 
...
\begin{equation}
 \omega \widetilde{\delta u}{}^z - k^z \frac{\widetilde{\delta T}}{T_0} + 
 \frac{\sigma \omega}{2\ln 2} \frac{\widetilde{\delta \mu}{}_H}{T_0} = 0.
 \label{eq:hydro_H_Fourier}
\end{equation}

Moving now to the $T^{\mu\nu}$ sector, Eq.~\eqref{eq_Ttt} shows that $\epsilon = P$, while Eq.~\eqref{eq_Pi} indicates that $\Pi = -\pi_B = -\frac{2}{3} P$. Considering that these equalities hold also in the perturbed system, we have 
\begin{align}
 \bar{\epsilon} &= \frac{|qB| T_0^2}{12}, &
 \delta \epsilon &= \frac{|qB| T_0}{6} \delta T,\nonumber\\
 \bar{\sigma}^B_\epsilon &= 0, & 
 \delta \sigma^B_\epsilon &= \frac{q T_0 \ln 2}{\pi^2} \delta \mu_H.
\end{align}

In the case of the shear-stress tensor $\pi^{\mu\nu}$, we ignore dissipative corrections and employ the form in Eq.~\eqref{eq:hydro_pimunu}. Writing $\pi^{\mu\nu} = \bar{\pi}^{\mu\nu} + \delta \pi^{\mu\nu}$, we have $\bar{\pi}^{\mu\nu} = \frac{1}{2} \bar{\pi}_B {\rm diag}(0, -1,-1,2)$ and 
\begin{multline}
...
\begin{equation}
 \partial_\mu j_\epsilon^\mu = B \partial_z \delta \sigma^B_\epsilon \rightarrow \frac{ik \overline{P}}{T_0} \frac{12 \ln 2}{\pi^2} \widetilde{\delta \mu}_H
\end{equation}
to arrive at
\begin{equation}
 2\omega \frac{\widetilde{\delta T}}{T_0} - \bk \cdot \widetilde{\delta \mathbf{u}} - k^z \widetilde{\delta u}{}^z - \frac{12 \sigma \ln 2}{\pi^2} k^z \frac{\widetilde{\delta \mu}{}_H}{T_0} = 0.
 \label{eq:hydro_eps_Fourier}
\end{equation}

Now we move on to the momentum conservation equation, i.e. Eq.~\eqref{eq:hydro_mom}. 
The terms $j_\epsilon^\lambda \nabla_\lambda u^\mu$ and $j_\epsilon^\mu \theta$ are of higher order and can thus be ignored. Furthermore, the projector $\Delta^\mu_\nu$ together with $\dot u^0$ ensures that the $\mu = 0$ relation reduces to a tautological $0 = 0$.
For $\mu = i \in \{x, y\}$ and $\mu = z$, we have
\begin{align}
 \Delta^i_\lambda \partial_\nu \pi^{\nu \lambda} &= \frac{1}{2} \partial^i \delta \pi_B + \frac{\bar{\pi}_B}{2} \partial_t \delta u^i \rightarrow -\frac{i\overline{P}}{3} \left(2 k^i \frac{\widetilde{\delta T}}{T_0} - \omega \widetilde{\delta u}^i\right), \nonumber\\
 \Delta^z_\lambda \partial_\nu \pi^{\nu \lambda} &= \partial_z \delta \pi_B + \bar{\pi}_B \partial_t \delta u^z 
 \rightarrow \frac{2i \overline{P}}{3} \left(2 k^z \frac{\widetilde{\delta T}}{T_0} - \omega \widetilde{\delta u}^z\right).
\end{align}
We thus arrive at:
\begin{align}
 \omega \widetilde{\delta u}{}^i &= 0, & 
 \omega \widetilde{\delta u}{}^z - k^z \frac{\widetilde{\delta T}}{T} + \omega \frac{6 \sigma \ln 2}{\pi^2} \widetilde{\delta \mu}{}_H &= 0. 
...
\begin{pmatrix}
 ...
 {\displaystyle \frac{\widetilde{\delta T}}{T}} \smallskip\\
 {\displaystyle \frac{\widetilde{\delta \mu}{}_H}{T}}
\end{pmatrix} = 0.
\end{align}
...
\end{equation}
where the solution $\omega = 0$ corresponds to the mode that supports helicity chemical potential fluctuations, The gapless modes $\omega = \pm k$ have identically $\widetilde{\delta \mu}_H = 0$ and represent the standard sound modes present in the ideal fluid.
...
Since the helical current generated by the HSE is linearly proportional to background temperature and magnetic field, the helical current~\eq{eq_HSE} and the heat current~\eq{eq_heat_flux} become coupled together via the HSE in a striking similarly with the coupling -- mediated by the CME and CSE -- of the vector and chirality currents that result in the appearance of the chiral magnetic wave~\cite{Kharzeev:2010gd}. However, a detailed analysis shows that the fluctuations of the helical charge in a neutral plasma do not lead to the appearance of a new hydrodynamic mode. The helical degree of freedom may, however, lead to appearance of additional hydrodynamic waves in dense plasma which are left beyond the scope of this paper.

The helicity plays a complementary role to chirality as the total helicity of a particle--anti-particle ensemble is given by the sum of axial charges carried by particles minus the sum of the axial charges possessed by anti-particles. As the helicity is different from chirality, its role in phenomenological applications in Quark-Gluon plasma may lead us to new observational effects which remain to be explored.


\acknowledgments
The authors are grateful to Yago Ferreiros for collaborating on an earlier related project. This work is supported by the European Union - NextGenerationEU through the grant No. 760079/23.05.2023, funded by the Romanian ministry of research, innovation and digitalization through Romania’s National Recovery and Resilience Plan, call no. PNRR-III-C9-2022-I8.
