...
\end{eqnarray}
Where $|z|^{2}=|z_{1}|^{2}+\dots +|z_{d}|^{2}$ and $dz$ is the $2d$-dimensional Lebesgue measure on $\mathbb{C}^{d}$ .\vskip 5mm  
...
\end{eqnarray}
\vskip 5mm  The monomials $\{\frac{z^{n}}{\sqrt{n!t^{n}}}\}$ form an orthonormal basis in $\mathcal{H}L^{2}(\mathbb{C}^{n},\mu_{t})$. The orthonormality condition with $t=d=1$ is 
\begin{eqnarray}
...
|n_{1},n_{2}\rangle=\frac{z_{1}^{n_{1}}}{\sqrt{n_{1}!}} \frac{z_{2}^{n_{2}}}{\sqrt{n_{2}!}}
\end{eqnarray} 
since 
\begin{eqnarray}
	\langle m_{1},m_{2}|n_{1},n_{2}\rangle=\delta_{m_{1},n_{1}} \delta_{m_{2},n_{2}}.
\end{eqnarray}
...
\end{eqnarray}
	with $p_{i}=\frac{|f_{i}\rangle \langle f_{i}|}{\langle {\bf f}|{\bf f}\rangle}$, $q_{i}=\frac{|g_{i}\rangle \langle g_{i}|}{\langle {\bf g}|{\bf g}\rangle}$ and $S(p_{i})=-\sum_{i} p_{i}\log p_{i}$. Consequently, other quantities such as mutual information can be determined in terms of holomorphic functions \cite{Cover}.
...
\begin{eqnarray}
	...
	y_{i}= f(u_{j}+b_{j}),
\end{eqnarray} 
where $\omega_{j1},\omega_{j2},\dots ,\omega_{jm}$ are the synaptic weights, $f$ is the activation function and $b_{k}$ is the bias. The inputs are
\begin{eqnarray}\label{inputs}
	x_{1}=\langle f_{1}|L_{1}|f_{1}\rangle, x_{2}= \langle f_{2}|L_{2}|f_{2}\rangle, \dots, \langle f_{N}|L_{N}|f_{N}\rangle.
\end{eqnarray} 
...
\bibitem{almasri3}
		AlMasri, M.W. and  Wahiddin M. R. B, 2023. `` Integral Transforms and PT-symmetric Hamiltonians''. Chinese Journal of Physics (Taipei), 
				\bibitem{Ahlfors}
