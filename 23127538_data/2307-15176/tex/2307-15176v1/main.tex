...
\textbf{Constructed observational studies} (COSs) start with RCTs and then find non-experimental control samples that come from a similar population \citep{lalonde1986evaluating,hill2004comparison,arceneaux2006comparing,shadish2008can,jaciw2016assessing,gordon2019comparison,eckles2021bias,zeng2022uncovering,gordon2022close}.
...
\begin{tabular}{lllrr}
  \toprule
 &\textbf{Synthetic DGP Setting} & \textbf{Sampling Algorithm} & \textbf{Abs. Bias (std.)} & \textbf{Rel.~Abs.~Bias (std.)} \\
  \toprule 
  \multirow{2}{*}{1} & \multirow{2}{*}{$|C|=1$, $P(T=1)=0.3$} &Algorithm 2 from \citet{gentzel2021and} & 0.222 (0.010) & 0.089 (0.004)\\
  && RCT rejection sampling (This work) & \textbf{0.009 (0.007)} & \textbf{0.004 (0.003)}\\
 \midrule 
 \multirow{2}{*}{2} & \multirow{2}{*}{$|C|=1$, $P(T=1)=0.5$} & Algorithm 2 from \citet{gentzel2021and} & 0.009 (0.006) & 0.003 (0.003)\\ 
 && RCT rejection sampling & 0.007 (0.005) & 0.003 (0.002)\\ 
 \midrule 
 \multirow{2}{*}{3} & \multirow{2}{*}{$|C| =5$, Nonlinear} & Algorithm 2 from \citet{gentzel2021and} & 0.252 (0.010) & 0.979 (0.037)\\ 
 && RCT rejection sampling & \textbf{0.012 (0.009)} &  \textbf{0.046 (0.034)}\\
 \bottomrule
\end{tabular}
}
\caption{
Absolute bias (abs.~bias) between ATE from $D_{\text{RCT}}$ and the estimated ATE via backdoor adjustment on $D_{\text{OBS}}$ created by each sampling algorithm. We also report abs.~bias relative to the RCT ATE (rel.~abs.~bias) and the mean and standard deviation (std.) across samples from 1000 random seeds. The DGPs for Settings 1-3 are given in Appendix~\ref{sec:appendix-dgps}.
...
We also describe our specific approach towards each consideration as we create a proof of concept pipeline for empirical evaluation of high-dimensional backdoor adjustment methods. For this proof of concept, we use a large-scale, real-world RCT dataset with text data as covariates.
...
As we mentioned in Section~\ref{sec:sampling}, a necessary precondition for RCT subsampling in general is the existence of a causal edge between $C$ and $Y$, i.e.~$C \not\ci Y $. 
...
We implement\footnote{We attempted to use the \texttt{EconML} package \citet{econml} but as of this writing it did not support using sparse matrices, which is required for our high-dimensional datasets.}   commonly-used causal estimation methods via two steps: (1) fitting base learners %(i.e.~predictive models with cross-fitting) 
...
measuring confidence interval coverage, measurement error, null causal effects, or moving to more sophisticated natural language processing approaches beyond bag-of-words, e.g.~the CausalBERT model \citep{veitch2020adapting}.
...
\textbf{Setting 2} has the same DGP as Setting 1 but we change $T \sim \text{Binomial}(0.5)$. For Settings 1 and 2, we provide the backdoor adjustment with the true DGP, such that it adjusts for both $C$ and the interaction term, $TC$.
...
\end{align*}
\noindent In this setting, we set $P^{*}(T=1|C) = \sigma(0.5 C_1 + -0.7 C_2 + 1.2 C_3 + 1.5 C_4 + -1.2 C_5 +0.5 C_1 C_2)$. We provide the parametric backdoor with the true adjustment set: $C_4, T C_1 C_2, C_2 C_3, C_5$. 
...
We expand on the details of the RCT described in Section~\ref{ssec:proof-concept-data}.
...
%Katie 2022-01-09 stats from casual-eval/raw_data_processing/semantic_reader_button/select_c.ipynb
 ...
 Intuitively, a positive causal effect is expected since advertising a feature of the website should result in increased click rate. The final ATE was 0.113 as computed by a simple difference in conditional means $\E[Y\mid T=1] - \E[Y \mid T=0]$.
