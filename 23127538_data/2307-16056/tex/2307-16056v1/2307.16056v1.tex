...
The set-theoretic framework for this work is the Zermelo-Fraenkel system $\mathbf{ZF}$. We recall that the axiom of choice $\mathbf{AC}$ is not an axiom of $\mathbf{ZF}$; in consequence, our reasoning in $\mathbf{ZF}$ may require more subtle arguments than in $\mathbf{ZFC}$. 
...
Given a topological space $\mathbf{X}$ with the underlying set $X$, we denote by $\tau[\mathbf{X}]$ the topology of $\mathbf{X}$; hence, $\mathbf{X}=\langle X, \tau[\mathbf{X}]\rangle$. For a set $A\subseteq X$, $\cl_{\mathbf{X}}(A)$ and $\cl_{\tau[\mathbf{X}]}(A)$ denote the closure of $A$ in $\mathbf{X}$. Closed (respectively, open) sets of $\mathbf{X}$ are called $\tau[\mathbf{X}]$-closed (respectively, $\tau[\mathbf{X}]$-open). 
...
\begin{definition}
\label{s2d1}
(Cf. \cite[Definition 1.10, p. 488]{Gr},  \cite[pp. 3, 129]{FL}, \cite{Wils}.)
...
\begin{definition}
\label{s2d2}
(Cf. \cite[Definition 10.1, p. 488]{Gr},  \cite[pp. 3, 129]{FL}, \cite{Wils}.)
\begin{enumerate}
...
\label{s2c09}
$[\mathbf{ZF}]$
For every 4-cover $\mathbf{A}$ of $\mathbb{R}$, the following conditions are all equivalent:
\begin{enumerate}
\item[(i)] $H_4(\mathbf{A})$ is non-archimedeanly quasi-metrizable;
\item[(ii)] $H_4(\mathbf{A})$ is quasi-metrizable;
\item[(iii)] $\mathbf{A}$ satisfies conditions (i)--(ii) of Theorem \ref{s2t5}.
\end{enumerate}
\end{corollary}
...
\end{proposition}
\begin{proof}
Let $C_0, C_1$ be a pair of non-empty disjoint closed sets of $H_{\mathcal{A}}$.  Let $i\in\{0,1\}$ and $c\in C_i$. If  $c\in A_2$, we put $U_i(c)=\{c\}$. Assuming that  $c\in C_i\setminus  A_2$, we define the set $N(c)$ as follows:
\[
...
\noindent We define $V_i=\bigcup\{U(c): c\in C_i\cap A_2\}\cup\bigcup\{ U_i(c): c\in C_i\setminus A_2\}$. Clearly, $V_i\in\tau[H_4(\mathcal{A})]$ and $C_i\subseteq V_i$. Arguing in much the same way as in the proof of Theorem 2.3 in \cite{Rich1}, one can check that $V_0\cap V_1=\emptyset$. This proves that the space $H_4(\mathcal{A})$ is normal.
