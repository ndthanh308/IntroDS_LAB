...
suggested in Ref.~\cite{Oller:2000fj}. Nearly all approaches based on $SU(3)$ chiral effective theory, 
...
Azizi:2023tmw,Xie:2023cej,Lu:2022hwm,Aaij:2017vbb} for some other recent theoretical studies of the 
...
\begin{table}[t]
\caption{
Parameters of the D200 ensemble~\cite{Bruno:2014jqa}. The spatial extent $L$ and
temporal extent $T$ of the lattice are given, as well as the masses $m_{\pi}, m_{\rm K}$
of the pion and kaon, respectively, in units of the lattice spacing $a$.
\label{tab:comp_deets}}
\begin{ruledtabular}
\begin{tabular}{c@{\hskip 12pt}c@{\hskip 12pt}c@{\hskip 12pt}c@{\hskip 12pt}}
 $a [\textup{fm}]$ & $(L/a)^{3} \times T/a$ & $am_{\pi}$ & $am_{\rm K}$ \\ \hline
    0.0633(4)(6) & $64^{3} \times 128$ & 0.06533(25) &  0.15602(16)
\end{tabular}
\end{ruledtabular}
\end{table}
...
\end{gather}
which consists of four free parameters. We also explored a ``multi-exponential'' variant of the 
geometric-exp series, with the replacement $Be^{-Mt}\rightarrow \sum_n B_n e^{-M_nt}$.
...
Four different types of parametrizations of $\widetilde{K}$ are studied here using $\ell_{\rm max}=0$.
In the expressions below, the subscripts $i$ and $j$ denote either of the two scattering channels 
(channel 0 is $\pi\Sigma$ and channel 1  is $\bar KN$), and the quantity
\begin{equation}
...
$\widetilde{A}$, and $\widetilde{B}$ below are real and symmetric.  The four forms of parametrizations are
as follows:
\begin{enumerate}
...
using a simple derivative method, as described in Ref.~\cite{Bulava:2022vpq}. 
...
determined using a bootstrap procedure with 800 samples.  The fit with the lowest AIC value is a 
...
the U.S.~National Science Foundation under awards PHY-1913158 and PHY-2209167 (CJM and SK), 
