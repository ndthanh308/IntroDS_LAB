...
\begin{abstract}
In this paper, we derive a pinching estimate on the traceless Ricci curvature in term of scalar curvature and the $C^{1}$  norm of Weyl tensor under the Laplacian $G_{2}$ flow for closed $G_{2}$ structures. Then we apply this estimate to study the long time existence of the Laplacian $G_{2}$ flow and prove that the $C^{1}$  norm of Weyl tensor has to blow up at least at a certain rate under bounded scalar curvature.
\end{abstract}
...
\end{equation}
where $\triangle_{\varphi(t)}\varphi(t)=dd^{\ast}_{\varphi(t)}\varphi(t)+d^{\ast}_{\varphi(t)}d\varphi(t)$ is the Hodge Laplacian of $g(t)$ and $\varphi$ is an initial closed $G_{2}$-structure. Here $g(t)$ is the associated Riemannian metric of $\varphi(t)$. Since $d\partial_{t}\varphi(t)=d\triangle_{\varphi}\varphi(t)=0$, we see that the closedness of $\varphi(t)$ is preserved along the Laplacian flow $\eqref{The closed Laplacian flow}$. 
...
Note that
    ...
    \end{align}
    Recall the definition of curvature decomposition, which yields
    \begin{align}
