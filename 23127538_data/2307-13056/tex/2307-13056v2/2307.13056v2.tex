...
From the closed-form expressions for ${N=3}$, one can notice the divergent behavior of both parts of the graviton propagator for ${\rho\to0}$,
\begin{equation}
...
\begin{enumerate}[topsep=0pt,itemsep=0pt,label=\textit{\roman*)}]
    ...
    \item Arguably, one of the most interesting lines of research is to study the graviton propagators in the Lorentzian regime either through the analytic extension of our results or by working directly in the Lorentzian signature. Similar to GR, see e.g. \cite{Kleppe:1994ga}, it would be particularly interesting to analyze effects of various boundary conditions, which become even more complex in higher-derivative or non-local theories.
    \item Recently, it turned out that the amplitudes in AdS/CFT take much simpler form when they are expressed in the Mellin space \cite{Fitzpatrick:2011ia,Nandan:2011wc,Bekaert:2014cea}. Following \cite{Balitsky:2011tw}, it would be also interesting to rewrite also the graviton propagators of HDG and IDG we found to the Mellin space representation.
    \item Similar to the analysis performed in \cite{Kolar:2022kgx}, it would be interesting to derive estimates for the graviton propagators based on the estimates of the heat kernels \cite{DaviesMandouvalos:1988}.
    \item Despite using the assumption of $\mathcal{F}_i$-analyticity, some of our intermediary results might be easily extendable to the non-analytic cases and applicable to a wider class of theories, for example, with the logarithmic terms from 1-loop quantum corrections, see, e.g, \cite{Burzilla:2020utr} (and references therein).
\end{enumerate}
