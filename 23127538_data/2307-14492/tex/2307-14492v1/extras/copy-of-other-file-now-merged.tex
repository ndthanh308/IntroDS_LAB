...
\title{Minimization is Undecidable, Primality is Undecidable}
\author[1]{}
...
\section{VASS Minimization is Undecidable}
...
\subsection{1-VASS Regularity}

We prove undecidability using a reduction from a problem we call 1-VASS Regularity, which was shown to be undecidable in \cite{AlmagorY21} as part of the analysis in Section 4.1. In 1-VASS Regularity we ask - given a 1-VASS $\cV$ - is its langage regular?
...
\section{VASS Primality is Undecidable}

The problem we discuss is the following: given a $k$-VASS $\cV$, are there $k'$-VASSes $\cV_1,\ldots \cV_n$ for some $k'<k$ such that $\bigcap_{1 \leq i \leq n}\lang(\cV_i)=\lang(\cA)$. Note that by $\lang(\cV),\lang(\cV')$ we refer to words accepted starting from counter values of $\bar{0}$.

We prove undecidability using a reduction from OCN-containment, shown to be undecidable in~\cite{Valiant73}. Given two OCNS - $\cA,\cB$ - we construct a 2-VASS $\cV$ as illustrated in \cref{fig:primalityReduction}.
