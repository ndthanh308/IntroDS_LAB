...
The need to design optimal estimators can arise for multiple reasons. Such master-field calculations represent a fairly dramatic example, but in general, many modern lattice QCD calculations may face the practical need to rely on a small number of gauge-field configurations. For example, expensive actions such as domain-wall fermions (e.g.\ used extensively by the RBC-UKQCD and JLQCD collaborations), often require the use of a smaller number of fields than other methods.
...
\bottomrule
\end{tabular}
\caption{Pion and nucleon masses from position-space correlators on point sources, compared to zero-momentum projected correlator on the same point sources, with or without smearing.}\label{tab:point_sources_results_1}
\end{table}
...
\end{equation}
We observe that the pion mass obtained with this combined fit is larger but compatible with the value given in eq.~\eqref{eq:pion_result}, obtained from the effective mass plateau of the $\mathring{C}_{PP}(r)$ correlator only and gradient-flow smearing.
...
\begin{table}[tb]
\centering
\begin{tabular}{lS[table-format=1.4(1)]S[table-format=1.3(1)]S[table-format=1.3(1)]S[table-format=1.4(2)]}
\toprule
& {$af_\pi^{\mathrm{bare}}$} & \multicolumn{3}{c}{$\bar{\Pi}^{I=1}_{\mathrm{bare}}(-Q^2)$} \\
& & {\SI{1}{\GeV\squared}} & {\SI{3}{\GeV\squared}} & {ratio} \\
\midrule
position-space & 0.0895(7) & 0.0568(4) & 0.0869(5) & 1.5288(28) \\
\midrule
zero-momentum & 0.0874(8) & 0.0584(7) & 0.0902(7) & 1.544(6) \\
\bottomrule
\end{tabular}
\caption{Pion decay constant and HVP from position-space correlators on point sources, compared to zero-momentum projected correlator on the same point sources. The discrepancies between the two rows may be due to cutoff effects or, in the case of the HVP, due to failing to saturate the estimator in both cases.}\label{tab:point_sources_results_2}
\end{table}
...
With this caveat, we perform a numerical test computing $\bar{\Pi}^{I=1}(-Q^2)$, the $I=1$ part of the HVP function. The computation of the HVP function relies on a genuine vector current, thus we use only the unsmeared meson operators. To eliminate the need for the renormalization factor $Z_V$ of the local vector current, here we quote the ratio between the HVP functions at two different values of $Q^2$
\begin{equation}
\label{eq:subtracted_Pi_ratio}
\frac{\bar{\Pi}^{I=1}(-\SI{3}{\GeV\squared})}{\bar{\Pi}^{I=1}(-\SI{1}{\GeV\squared})} = \num{1.5288(28)} .
\end{equation}
Taking the ratio has also an effect in reducing the relative statistical error to just \SI{0.18}{\percent}, with the unrenormalized value being $\bar{\Pi}^{I=1}_{\mathrm{bare}}=\num{0.0568(4)}$ at $Q^2=\SI{1}{\GeV\squared}$, with a \SI{0.7}{\percent} error. This can be compared with the result obtained from the same correlator data using the TMR method, that amounts to a ratio of \num{1.544(6)}, or $\bar{\Pi}^{I=1}_{\mathrm{bare}}=\num{0.0584(7)}$ for the unrenormalized value. These results are given in table~\ref{tab:point_sources_results_2}. While in both cases the statistical error of the traditional method is roughly twice as large, this analysis does not take into account the systematics associated to the truncation of the integral, which is most likely larger in the CCS case. For reference, in ref.~\cite{Ce:2022eix} the ratio in eq.~\eqref{eq:subtracted_Pi_ratio} has been computed to high precision using an $O(a)$-improved vector current on a CLS ensemble with a similar mass and $a\approx\SI{0.064}{\femto\metre}$ (N200), giving \num{1.5493(19)}, while in the continuum limit the same ratio evaluates to \num{1.484(14)}.
