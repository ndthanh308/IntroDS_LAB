...
\email{aeortiz@mit.edu}
% \date{August 7, 2023} 
\subjclass{42B10}
\keywords{maximal functions, restriction theory}
...
\]
is the unit segment of the cone in $\R^3$, and $w$ is the indicator function of a 1-dimensional disjoint union of unit balls. To state our main theorem, we make a preliminary definition. Let $\gamma(\theta) = \frac{1}{\sqrt 2}(\cos\theta,\sin\theta,1),\theta\in\R$. 
\begin{defn}
...
\end{theorem}
When a disjoint union of unit balls $X$ satisfies \eqref{eqn:1-dimensional}, we will say $X$ is \emph{$1$-dimensional}. The weighted estimate of Theorem \ref{thm:main-l2-sparse} is sharp in the sense that for each $R>1$, and for each $1<T<R$, there is a 1-dimensional disjoint union of unit balls $X$ such that $\mathbf P(X) \sim T$, and a nonzero function $f$ such that
\[
...
In 1987, Mattila \cite{mattila1987spherical} proposed the problem of identifying the optimal polynomial rate of decay of spherical Fourier averages of compactly supported measures with finite $\alpha$-energy.
...
\end{defn}
In \cite{mattila1987spherical}, Mattila established the sharp exponent $\beta_n(\alpha) = \alpha$ when $0<\alpha\le \frac{n-1}{2}$. For the lower bound $\beta_n(\alpha)\ge \alpha$, Mattila used the stationary phase estimate $|\widecheck{d\sigma}(x)|\lesssim|x|^{-(n-1)/2}$, where $\sigma$ is the surface measure of the sphere, by rewriting the Fourier average as a double integral
\begin{equation}\label{eqn:mattila}
\int_{S^{n-1}}|\widehat\mu|^2\,d\sigma = \iint_{\R^n\times\R^n} \widecheck{d\sigma}(x-y)\,d\mu(x)d\mu(y).
\end{equation}
...
Following partial progress on the decay of spherical Fourier means by Mattila \cite{mattila1987spherical}, Sj\"olin \cite{sjolin1993estimates} in 1993, and Wolff \cite{wolff1999decay} in 1999 who found the optimal values of $\beta_2(\alpha)$ for $\alpha\in (1,2)$, as well as updated progress on the Fourier restriction problem due to Tao \cite{tao2003sharp} in 2003, Erdo\u gan established the optimal \emph{conical} Fourier average decay rates for $\alpha$-dimensional measures in $\R^3$ \cite{erdogan2004note}. 
...
holds.
\end{theorem}
The assumption $d\mu=1_Xdx$ for a $1$-dimensional disjoint union of unit balls in Theorem \ref{thm:main-fourier-decay} is technically different but closely related to the assumption that $\mu$ is supported in the unit ball and has normalized $1$-energy in Theorem \ref{thm:main-fourier-decay}. To illustrate the connection between Theorem \ref{thm:erdogan} and our Theorem \ref{thm:main-fourier-decay}, the reader can verify that Theorem \ref{thm:erdogan} immediately implies the following slightly weaker corollary.
\begin{cor}\label{cor:erdogan}
    ...
    \[
    \int_{\mathbb Cone^2} |\widehat\mu|^2d\sigma \le C_\epsilon R^\epsilon R^{3/2}
    \]
    holds.
\end{cor}
...
Heuristically by inequality \eqref{eqn:cone-decay}, the only pairs $(x,x')\in X^2$ which contribute to the double integral in Equation \eqref{eqn:mattila} are those such that $x-x'$ is close to the lightcone $\Gamma_0$. By the rapid decay in inequality \eqref{eqn:cone-decay}, the only pairs $(x,x')\in X^2$ which contribute to the double integral satisfy
\[
...
\end{prob}
We will apply the following recent circular maximal function estimate of Pramanik--Yang--Zahl \cite{pramanik2022furstenberg} which applies to 1-dimensional families of circles as a black box to solve our discretized tangency problem as a key lemma. We use the notation $E(z) = 1_E(z)$ here and elsewhere in the paper.
...
The idea of applying circle tangencies to study $E_{\mathbb Cone^2}f$ is not new. In ``Local smoothing type estimates in $L^p$ for large $p$'' \cite{wolff2000local}, Wolff used the point-circle duality idea to study $E_{\mathbb Cone^2}f$. Part of his goal was to understand the large level sets of $E_{\mathbb Cone^2}f$, and he used estimates for the number of circle tangencies of configurations of circles as an ingredient to prove the first sharp decoupling estimates for the cone in $L^p$ for large $p$.
...
\begin{proof}[Proof of Proposition \ref{prop:root-delta}]
    ...
    \end{equation}
    We need to show that $I\le C\delta$ for a constant that does not depend on $(a_1,a_2)$. Let $x' = (x_1,x_2)$, and $a_1 = 1 + h$ with $|h|\le \delta$. Expanding Equation \eqref{eqn:lightplank-dual-i} and applying the triangle inequality,
    \begin{align*}
    I &= |(x_1 - 1)^2 + x_2^2 - x_3^2 + h^2 -2(x_1-1)h - 2x_2a_2|\\
    &\le \big(||x'-e_1|-x_3|\cdot||x'-e_1|+x_3|\big)+\delta^2 + 2\delta|x_1-1| + 2\sqrt\delta|x_2|.
    \end{align*}
    Now we use that $x\in P^{(o)}$ to make some estimates of this last expression. First, $||x'-e_1|-x_3| = \Delta(x,e_1) \le C\delta$, because $\Delta(x,e_1)$ is nearly the distance from $x$ to the lightcone with vertex $e_1$. The quantity $||x'-e_1|+x_3| < 10$ because $x \in Q$. Similarly, $|x_1-1|\le 2$. Note $|x_2| = |x_m| \le \sqrt\delta$, so $2\sqrt\delta|x_2|\le 2\delta$. Altogether this shows that $I\le C\delta$, as desired.

    Now we prove (ii). Let $\ell = \{e_1 + t(e_3-e_1):t\in\R\}$ be a lightray intersecting $e_1$. Consider the infinite $2\delta\times\sqrt\delta$ rectangular prism $R$ we get by sliding $\Omega^{(o)}$ along $\ell$. (Concretely, $R$ is the Minkowski sum $e_1 + (\ell-e_1) + (\Omega^{(o)}-e_1)$.) If $x\in Q$, but $x$ is not in $CR$, the dilation of $R$ about its central axis $\ell$ by a factor of $C$, and $C$ is sufficiently large, then $\Omega^{(o)} \not\subset C_{10\delta,x}$. This shows that if $x\in \mathbf D_{10\delta}(\Omega^{(o)})$, then $x\in Q\cap CP^{(o)}$ for an appropriate $C>1$.
\end{proof}
...
\begin{defn}
If $E\subset Q$, and $\delta>0$, $\lambda\lessapprox 1$, let $$\mathbf D^*_{\lambda\delta}(E) = \{z\in\R^2:E\subset \Gamma_{\delta,z}\}$$ where $\Gamma_{\delta,z}$ is the $\delta$-neighborhood of $\Gamma_z = z+\Gamma_0$, and we remind that $\Gamma_0 = \{(a,r)\in\R^2\times\R:||a|-|r||=0\}$ is the lightcone with vertex $0$.
\end{defn}
...
\end{itemize}
As the rectangles $\Omega^{(v_1)},\dots,\Omega^{(v_M)}$  are pairwise $A$-incomparable, for each $i \ne j$, at least one of the following inequalities must hold:
\begin{itemize}
...
\[
\sum_{\Omega\in\mathcal R}\mu_{\lambda\delta}(\Omega)\Omega(z) \lesssim \lambda^Cm_{\lambda \delta}(z) = \lambda^C\sum_{x\in X}C_{\lambda\delta,x}(z),\quad z\in\R^2.
\]
\end{prop}
\begin{proof}
...
\[
\lambda^C m_{\lambda \delta}(z)\gtrsim \sum_{\Omega\in\mathcal R}\mu_{\lambda\delta}(\Omega)\Omega(z),\quad z\in\R^2.
\]
...
\begin{proof}
By Proposition \ref{prop:nearly-ll}, there are $\approx 1$-comparable $\delta,\tau_D$-rectangles $\Omega^{(v)},\Omega^{(w)}$ in $C_{\delta,v},C_{\delta,w}$ respectively. By maximality of $\mathcal R$ with respect to $A$-incomparability, there is some $\Omega_0\in\mathcal R$ such that $\Omega^{(v)}$ is comparable to $\Omega_0$, hence $v\in\mathbf D_{CA^C\delta}(\Omega_0)$. Since $\Omega^{(v)}$ and $\Omega^{(w)}$ are comparable, by almost-transitivity (Proposition \ref{almost-transitivity}), $\Omega_0$ and $\Omega^{(w)}$ are $CA^C$-comparable. Hence $w\in\mathbf D_{CA^C\delta}(\Omega_0)$ for a large enough absolute constant $C$, and the claim is proved.
\end{proof}
...
In addition to the estimate of circle tangencies for a 1-dimensional set of circles, we need a pointwise bound for the  Fourier transform of $\sigma$, a smooth surface carried measure on the cone segment. We state the version of the estimate we will use in the proof of Theorem \ref{thm:ortiz-sparse-l1} here. The proof of this lemma is contained in Appendix \ref{app:ft}.
\begin{lemma}\label{lem:fourier-decay}
Let $\sigma$ be a smooth surface measure supported in $\mathbb Cone^2$. For any $\epsilon>0$, there is a constant $C_\epsilon$ so that
\[
...
\]
Then for any measurable function $w\colon\R^3\to[0,1]$ and any $f\in L^2(\mathbb Cone^2)$, the estimate
\[
...
\]
Since $X_\eta$ is a disjoint union of unit balls satisfying $|X_\eta\cap B_r|\le|X\cap B_r|\lesssim r$ for all $r$-balls and $r\ge 1$, we use Pramanik--Yang--Zahl's maximal estimate in the form of Theorem \ref{thm:num-ll-pairs} (after rescaling the ball of radius $R$ to a ball of radius $1$) to bound this by
\[
...
\]
\end{proof}
By taking $w$ identically equal to $1$ in Theorem \ref{thm:ortiz-sparse-l1}, we have the following refinement of $\gamma_3(1)=1/2$ we mentioned in the introduction (recall Definition \ref{def:cone-rate}):
...
\[
\int_X |E_{\mathbb Cone^2}f|^2 \gtrapprox T^{1/2}\|f\|_{L^2(d\sigma)}.
\]
\end{theorem}
\begin{proof}
...
\]
By definition of $f$, $E_{\mathbb Cone^2}f(x)\gtrsim |\theta|\sim T^{-1/2}$ for $x\in P$.
...
\item
As a small variation on the last example, we can also normalize $|X|=R$, with $\mathbf P(X)\sim T$. Let $f$ and $P$ be the same as in the first example, let $X_P$ be any $1\times 1 \times T$ tube contained in the lightplank $P$, and let $X_V$ be a $1\times 1\times (R-T)$-tube with direction $e_3$. Let $X = X_P\cup X_V$. By construction, $|X| \sim R$ and $X$ is $1$-dimensional. Moreover, $\mathbf P(X)\sim T$ as any lightplank intersects $X_V$ in a ball of radius $O(1)$. By the same calculation as in the first example, $\int_X |Ef|^2 \gtrsim T^{1/2}\|f\|_{L^2(d\sigma)}^2$.
\end{enumerate}
\end{proof}
...
\begin{theorem}[Ciprian Demeter, 2023---private communication]\label{thm:ciprian}
    ...
    \end{equation}
\end{theorem}
A sharp example for the estimate \eqref{eqn:cone-ciprian} is  to let $X$  be a $1\times R^{1/2}\times R$-lightplank, and let $f$ be a similar Knapp-type example as in Theorem \ref{thm:sharp}. By covering a lightplank with $R^{1/2}$-many $1\times 1\times R$-tubes, Theorem \ref{thm:ciprian} implies the Mizohata--Takeuchi conjecture for $\mathbb Cone^2$ and general sets $X$ with a $R^{1/3}$-loss, which is slightly worse than the $R^{1/4}$-loss we manage to prove holds for $1$-dimensional sets in Corollary \ref{cor:mt-progress}.

In Carbery--Iliopoulou--Wang's work \cite{carbery2024some}, the authors show how the $R^{1/3}$-loss they get in their Mizohata--Takeuchi-type estimate for the unit circle $S$ in the plane cannot be improved using Guth's decoupling axioms alone because of an enemy scenario which obeys the decoupling axioms, but is not of the form $E_Sf$ \cite{HAPPY}. It would be very interesting to know whether a similar enemy scenario exists for the cone, and which would imply that it is impossible to improve the $R^{1/3}$-loss in the Mizohata--Takeuchi conjecture for the cone in $\R^3$ and general sets $X$ without using properties of the extension operator $E_{\mathbb Cone}$ beyond decoupling axioms.
...
\begin{lemma}[Fourier transform estimate]
    Let $\sigma$ be a smooth surface measure supported in $\mathbb Cone^2$. For any $\epsilon>0$ and any $N>1$, there is a constant $C(\epsilon,N)$ so that
    \[
...
\[
\widecheck\sigma(x) = Ea(x) =  \int a(\xi)e^{2\pi i(x'\cdot \xi+x_3|\xi|)}\,d\xi.
\]
