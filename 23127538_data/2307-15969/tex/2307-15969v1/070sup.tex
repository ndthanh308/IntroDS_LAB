...
\begin{proof}[Proof of Theorem \ref{th:pruning}]
     We set $\mathcal{H}_k (k>0)$ is the remaining graph after k iterations in Algorithm \ref{alg:pruning} and $\mathcal{H}_0$ is the initial whole graph, $\mathcal{H}_{k+1}^{'}$ is the nodeset to be deleted in k+1 iteration. Therefore, $\mathcal{H}_{k+1}=\mathcal{H}_{k} \setminus \mathcal{H}_{k+1}^{'}$ Then:
     \begin{equation*}
         \begin{aligned}
            \rho(\mathcal{H}_{k+1})
