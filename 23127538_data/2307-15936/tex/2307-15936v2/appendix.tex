...
\subsection{Graphs of Emergence Phenomena}
For context, we include some graphs of emergence of capabilities from \cite{wei2022emergent}.
...
The next lemma uses the famous Probabilistic Method~\cite{alonspencer}. In this method, one is trying to show that in a certain probability space, there are no {\em bad} outcomes.  This is done by letting $W$ be an integer random variable denoting the number of bad outcomes that happened, and showing that the  $E[W] \approx 0$. Then it follows that  $W =0$ with probability at least $1 - E[W]$. Concretely, in the next Lemma  $W$ will be the number of ``bad'' set pairs  $(Y,Z)$ of a certain size that violate the lemma. 

\begin{lemma}\label{lem:mixing}
    For every positive integer $k$ and $\theta \in [0,1]$  there are $\alpha, \beta >0$ such that $\alpha \beta \leq 1$ and the following holds with probability almost $1$. For every $Y \subseteq V_1$ of size $\theta N_1$, there are at least   $(1-\alpha)$ fraction of vertices in $V_2$ each of which has  at most $\beta \theta D$ edges going to $Y$, where $D =kN_1/N_2$ is the expected degree of a node in $V_2$. The parameter values for which this occurs are specified by the condition
...
\begin{proof}
For $Y \subseteq V_1, |Y| =\theta N_1$ and $Z \subseteq V_2, |Z|\leq  \alpha N_2$ we say that $(Y, Z)$ are  {\em bad} if $Z$ has at least $\alpha \beta \theta k N_1$ edges to $Y$.  Let $W$ denote the  number of such $Z$'s. The expectation is upper bounded by 
\begin{equation} N_2 N_1 {N_2 \choose \alpha N_2} \times {N_1 \choose \theta N_1} \times {k\theta N_1 \choose \beta \alpha k \theta  N_1} \times \alpha^{\beta \alpha \theta k N_1} \times (1-\alpha)^{(1-\beta \alpha )\theta kN_1} \label{eqn:mix1} \end{equation}
For (\ref{eqn:mix1}) to be $\ll 1$ it suffices for its logarithm to be negative. 
By Stirling's approximation ${N \choose t N} \leq 2^{(H(t) + \epsilon_N)N}$ where $H(t)= - t\log t - (1-t)\log (1-t) $ is the binary entropy function and $\epsilon_N$ goes to zero rapidly as $N \rightarrow \infty$. 
Applying this to (\ref{eqn:mix1}) and  taking logarithms, and assuming $N_2 \ll N_1$, we  arrive at the condition (\ref{eqn:mix2}) for large $N_1$.
...
\end{proof}
\noindent{\bf Note:} Such arguments allow a fair bit of slop. The expectation was  exponentially small, and then we took its logarithm and then divided out by a large number, $N_1$ to reach (\ref{eqn:mix2b}). Thus additional  polynomial factors  in the expectation ---such as $N_1N_2$ above--- have no effect on asymptotics. 
...
$(V_1, V_2, E)$ where each vertex in $V_1$ has edges to $k$ random vertices in $V_2$. We use the shorthand $N_i =|V_i|$. As noted in proof of Theorem~\ref{thm:genmeasure} it suffices to consider the case when $V_2$ has uniform measure and there is a general measure $\mu()$  on vertices of $V_1$, namely
...
The proof will use a discretization of the measure. We conceptually divide $V_1$ (and hence also the set of edges) into {\em classes}, where the $i$th class ${C}_i$ consists of 
$v_1$ such that $\mu(v_1) \in [(1+\epsilon)^{-i -1}, (1+\epsilon)^{-i})$ for $\epsilon$ an arbitrarily small constant. We assume all  $\mu(v_1)$ are sufficiently small (meaning some large-ish $i_0$, class $i$ is empty for $i < i_0$) and the number of nonempty levels is much smaller than $N_1$. Thus each class has reasonable size ---say,  much larger than $N_2$, the number of skills---which allows  the asymptotic arguments appearing below to hold within each class.  
...
\begin{proof}
  ...
  (As explained in the note following Lemma~\ref{lem:mixing}, we can ignore the contribution to the expectation of 
  $Z$'s that have size $< \alpha N$.) 

For any fixed $Y \subseteq V_1$ we denote  $Y \cap  C_i$ as $Y_i$ and let $y_i =|Y_i|$. 
If $\mu(Y) =\theta$ then the $y_i$'s satisfy the following
\begin{equation}
    \sum_i y_i (1+\epsilon)^{-i} \in [\theta, (1+\epsilon)\theta]. \qquad \label{eqn:failm2}
\end{equation} 
...
\begin{equation}
    \label{eqn:mixm3}
    \prod_i {ky_i \choose \alpha \beta_i ky_i}(\alpha)^{-\alpha \beta_i k y_i}(1-\alpha)^{ky_i(1 -\alpha\beta_i)} 
\end{equation}
Since $\sum_i y_i =|Y|$ and ${ky_i \choose \alpha \beta_i ky_i} \approx 2^{H(\alpha \beta_i)ky_i}$, the left hand side is an expression of type 
$$\prod_i 2^{k y_i(H( \alpha \beta_i) -  \alpha\beta_i \log \frac{\alpha}{1-\alpha} + \alpha \log \frac{1}{1-\alpha})}.$$  Using first order optimality wrt $y_i$'s, this is maxiized when all $\beta_i$'s are equal. So for deriving an upper bound it suffices to let all $\beta_i =\beta$,
which simplifies (\ref{eqn:mixm3}) to
\begin{equation}
    \label{eqn:mixm4}
...
Now we finish the proof using reasoning similar to that in Lemma~\ref{lem:mixing}. The number of choices for $y_1, y_2, \ldots, $ is $\prod_i |C_i|$, which is at most $N_1^{P}$ where $P$ is the number of classes. 

For a fixed sequence of $y_i$'s the number of sets  $Y$ consistent with those intersections is 
$$ \prod_i {|C_i| \choose y_i} \approx \prod 2^{H(y_i)} .$$
Since $y_i$'s satisfy \eqref{eqn:failm2} and $H()$ is a concave function, this number is maximised when $y_i/|C_i| \in [\theta, \theta (1+\epsilon)]$, and hence the number of possible $Y$'s is upper bounded by 
\begin{equation}  \prod_i 2^{H(\theta |C_i|)} = 2^{H(\theta N_1)}. \label{eqn:numclasses} \end{equation}

\iffalse show that when we pick a random $(Y, Z)$ then the expected number of bad $(Y, Z)$ pairs is $\ll 1$. %Let the classes for measure $\mu$ be defined as above, and suppose $C_i$ is the set of $v_i$'s in $i$th class. 
...
\fi 
By the union bound, the probability that there exists a $Y$ such that  $(Y, Z)$ is bad is at most $2^{H(\theta) N_1}$ times (\ref{eqn:mixm4}) times  (\ref{eqn:numclasses}). Since $|Y| \approx \theta N_1$ this completes the proof of Lemma~\ref{lem:mixing+measure}.
\end{proof}
