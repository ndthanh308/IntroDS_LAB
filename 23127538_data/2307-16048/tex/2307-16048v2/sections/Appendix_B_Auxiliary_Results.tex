...
\textbf{Part 1:} For a contradiction, let such $h$ exist. Define $\xi:=h(\textbf{X}_S) + f(\textbf{X}_S, \textbf{X}_{\setminus S}) $, which is the left hand side of (\ref{yuiMAIN}). 

Fix $\textbf{a}_0\in\mathbb{R}^s$ in the support of $\textbf{X}_S$ and define
...
Let $X$ be a random variable with strictly increasing distribution function $F_X$. Let $f$ be a continuous injective function such that
...
\end{lemma}
Proof can be found below Remark~\ref{remark_lemma_additivity}. The following remark shows that there exists a periodic continuous function $f$ such that the statement of Lemma~\ref{lemma_additivity} is not valid.

\begin{remark}\label{remark_lemma_additivity}
...
h_1(x) &= 0 \quad \forall x \in \mathbb{R}\\
h_2(y) &= \cos(2\pi F_Y(y)) \quad \forall y \in \mathbb{R}
\end{align*} 
Then 
...
Let $g^{-1}(x, y)$ denote the inverse of $g(x, y)$ with respect to $y$. The existence of this inverse follows from the fact that $f$ is continuous and injective, as stated by the Inverse Function Theorem \citep{Inverse_function_theorem}. 
...
X\indep Y \iff f(X)\indep Y. \tag{\ding{115}} 
\end{equation}
\end{lemma}
\begin{proof}
