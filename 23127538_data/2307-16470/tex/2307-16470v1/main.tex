...
All SM particles also carry $U(1)_{\rm FN}$ charge, and the value of FN charge is generation dependent, which indicates that the masses of different generations of particles get suppressed by different powers of $\lambda$. 
Thus, the hierarchy issue could be well explained by FN mechanism (see Ref. \cite{Altmannshofer:2022aml} for a very recent review).
...
Take $\overline{Q}_{\rm L}^i$ and $d_{\rm R}^j$ for example, the most general form of their FN charge should be $U(1)_{\rm FN}^{\overline{Q}_{\rm L}^i}=\{a,b,c\}$ and $U(1)_{\rm FN}^{d_{\rm R}^j}=\{d,e,f\}$, then we have
\begin{equation}
...
The well-known Wolfenstein parametrization~\cite{Wolfenstein:1983yz} indicates that mixing angles in CKM matrix approximately satisfy $\sin\theta_{12}^{\rm C} \sim \lambda'$, $\sin\theta_{23}^{\rm C}\sim \lambda'^2$ and $\sin\theta_{13}^{\rm C} \sim \lambda'^3$, where $\lambda'\sim 0.2$. It is attempted to assume that $\lambda \sim \mathcal{O}(\lambda')$, and FN charge of quark doublet shall be $U(1)_{\rm FN}^{\overline{Q}_{\rm L}^i}=\{3,2,0\}$ to produce a such mixing pattern~\cite{Fedele:2020fvh, Cornella:2023zme}. 
...
Assuming the FN charge of $u_{\rm R}$ and $d_{\rm R}$ are $\{a,b,0\}$ and $\{c,d,0\}$ respectively. Then we can derive that
\begin{equation}
    n_u= 
    \begin{pmatrix}
        3+a & 3+b & 3 \\
...
\end{equation}
Note that here we use the same $\mathcal{N}$ as Eq.~\eqref{eq:yd} because of the fact that $m_b\sim m_\tau$ at very high energy sale \cite{Huang:2020hdv}. Besides, the $g_{ij}'$ is a symmetric matrix due to the Majorana nature of the neutrino. Similar to the quark sector, the Pontecorvo-Maki-Nakagawa-Sakata (PMNS) matrix is mainly determined by $\overline{\ell}_{\rm L}^i$. The observations tell us that 
the mixing angle $\theta_{12}^{\rm P}$ and $\theta_{23}^{\rm P}$ are relatively larger compared to $\theta_{13}^{\rm P}$, and $\theta_{12}^{\rm P}$ slightly smaller than $\theta_{23}^{\rm P}$. 
...
the FN mechanism naturally prefers normal order (NO).
...
global symmetries must be broken by non-perturbation effects in quantum gravity~\cite{Banks:2010zn}. With the charge assignment in Table~\ref{tab:FN_charges}, we found there is a discrete $\mathbf{Z}_{33}$ symmetry, actually this symmetry is is anomaly-free for $\mathbf{Z}_{33}\times [SU(2)_{\rm L}]^2$ and $\mathbf{Z}_{33}\times [SU(3)_{\rm c}]^2$, and therefore can be gauged. This can be regarded as quite an interesting feature of our model.
...
%Once we fix all FN charges, the theory only leaves one free parameter: the $\lambda$. Besides the free input $\lambda$, we also allow small $\mathcal{O}(1)$ fluctuations on each Yukawa coupling, $g_{ij}$ as explained in the next section. We randomly generate $g_{ij}$ according to the same probability density function, which shall give us a statistical prediction on various quantities in fermion mass ratios and mixings. By comparing with experimental measurements, with FN charges in Table~\ref{tab:FN_charges}, we find the best value of the parameter $\lambda =0.171$. We show in Fig.~\ref{fig:quark_mass},~\ref{fig:ckm},~\ref{fig:lepton_mass} and~\ref{fig:pmns}, the predicted distributions of all masses and mixing angles which fit all observations very well.

%With this success we show, in Fig.~\ref{fig:mee}, the distribution of the key neutrino mass parameter, $m_{ee}$, which determines the magnitude of the neutrinoless double beta decay($0\nu\beta\beta$) amplitudes~\cite{Jones:2021cga}. In our FN model, this quantity has a lower bound $m_{ee}\gtrsim 8\times 10^{-4}\,{\rm eV}$ at $95\%\,{\rm C.L.}$, which is much stronger than the bound in the general normal order (NO) scenario.



%An overall factor $\mathcal{N}$ is introduced to accommodate the overall scale difference between up and down sectors, $m_t \sim \mathcal{N} m_b$, which does not impact the fermion mass ratio between generations. 
%The power of $\lambda = \langle \phi \rangle /M_{\rm Pl} $ is determined by FN charges of quarks as shown in Table~\ref{tab:FN_charges}, and further form matrices $n_u$ and $n_d$, which are
% \begin{equation}
%     n_u= 
%     \begin{pmatrix}
%         7 & 4.5 & 3 \\
...
%     \end{pmatrix}\;.
% \end{equation}
% With the parameter $\lambda<1$, power matrices $n_u$ and $n_d$ indicate the hierarchy between generations. After Higgs developed VEV, $\langle H \rangle = (0,v)^\top$, quark mass matrices are written as $m_u^{ij} = v y_u^{ij}$ and $m_d^{ij} = v y_d^{ij}$. The mixing and hierarchy are all determined by the Yukawa matrices. Hierarchy is represented by mass ratios.
% Naively, one would expect that
% \begin{equation}
%     \frac{m_u}{m_t} \sim \lambda^7 \;,\quad \frac{m_c}{m_t} \sim \lambda^{3.5} \;,\quad \frac{m_d}{m_b} \sim \lambda^4 \;,\quad \frac{m_s}{m_b} \sim \lambda^2\;.
% \end{equation}
% Such estimates could be used to determine FN charges of $u_{\rm R}$ and $d_{\rm R}$. The precise values for mass ratios and mixing should be calculated from matrix decomposition.
...
In order to calculate Eq.~\eqref{eq:chi2} we need the exact distribution of $X_i$. For the quark sector, the Yukawa matrices can be decomposed as
\begin{equation}
    y_u = U_u D_u W_u^\dagger\;,\quad y_d = U_d D_d W_d^\dagger\;,
    \label{eq:quark_decomposition}
\end{equation}
where $U_{u,d}$ and $W_{u,d}$ are unitary matrices, $D_{u,d}$ is a diagonal matrix with all elements are real. The $U_u$ and $W_u$ are obtained from
...
Under the Standard parametrization, the $U_{\rm CKM}$ is 
\begin{equation}
...
% % Figure environment removed
%With the success of our model in the quark sector, we investigate the lepton sector in this section.
%For the lepton sector, we write down the Yukawa matrix for the charged lepton and utilize the dim-5 operator to describe neutrino mass,
% \begin{equation}
%     -\mathcal{L}\supset y_\ell^{ij} \overline{\ell}_{\rm L}^i H e_{\rm R}^j + \frac{1}{M} y_\nu^{ij} \left(\overline{\ell^{\rm c}_{\rm L}}^i \tilde{H}^*\right) \left( \tilde{H}^\dagger \ell^j_{\rm L} \right)\;,
%     \label{eq:lepton_yukawa}
% \end{equation}
% where
% \begin{equation}
%     y_\ell^{ij} = g_{ij} \lambda^{n_\ell^{ij}} \;, \quad y_\nu^{ij} = g_{ij}' \lambda^{n_\nu^{ij}}\;.
%     \label{eq:quark_yukawa}
% \end{equation}
% Here the same factor $\mathcal{N}$ occurs due to the fact that $m_\tau \sim m_b$ at the GUT scale~\cite{Georgi:1979df}.
% Note that $g_{ij}'=g_{ji}'$ is a symmetric matrix due to the Majorana nature of the neutrino, $\nu^{\rm c} = \nu$, which leads to $y_\nu^\top = y_\nu$. The independent elements in $g_{ij}'$ follow the same statistical pattern as $g_{ij}$ in Fig.~\ref{fig:g_ij}. The power matrices are, according to FN charges,
% \begin{equation}
%     n_\ell = \begin{pmatrix}
%         5 & 2 & 1 \\
%         4.5 & 1.5 & 0.5 \\
%         4 & 1 & 0 \\
%     \end{pmatrix}\;,\quad 
...
%     \end{pmatrix}\;.
% \end{equation}
% After electroweak symmetry breaking, one obtains $m_\ell^{ij} = v y_\ell^{ij}$ and $m_\nu^{ij} = v^2y_\nu^{ij}/ M$, where $M$ could be the mass scale of right-handed neutrinos which have been integrated out after Seesaw mechanism. 
...
\end{equation}
where $D_\ell$ and $D_\nu$ are diagonal matrices with all elements are real, $U_\ell$, $W_\ell$, and $U_\nu$ are unitary matrices. The $U_\ell$ and $W_\ell$ are obtained from $y_\ell y_\ell^\dagger = U_\ell (D_\ell)^2 U_\ell^\dagger$ and $y_\ell^\dagger y_\ell = W_\ell (D_\ell)^2 W_\ell^\dagger$. 
Then the mass ratios for charged lepton are
\begin{equation}
    \frac{m_e}{m_\tau}=\frac{D_\ell^{11}}{D_\ell^{33}}\;, \quad \frac{m_\mu}{m_\tau}=\frac{D_\ell^{22}}{D_\ell^{33}}\;.
\end{equation}
%$y_\nu^\dagger y_\nu = \tilde{U}_\nu (D_\nu)^2 \tilde{U}_\nu^\dagger$ and $\Phi$ is a diagonal matrix with only phases. Similar to the quark sector, here , which contain information about charged lepton mass ratio like in Eq.~\eqref{eq:mass_ratio} and the ratio between two neutrino mass observables. 
...
%For a specific input value of $\lambda$, with $g_{ij}$ randomly sampled from distributions in Fig.~\ref{fig:g_ij}, we could calculate the fermion mass ratios using diagonal matrices in Eq.~\eqref{eq:quark_decomposition} and~\eqref{eq:lepton_decomposition}, also mixing parameters in Eq.~\eqref{eq:ckm} and~\eqref{eq:pmns}.
%By sampling random $g_{ij}$ multiple times, we have predictions for those quantities, which are all distributions. This model has only one free input, which is $\lambda$. To find out the best value for it, we construct a reduced $\chi^2$-function to evaluate the goodness of fit for our prediction. Since all measurements of these quantities are much more precise compared with our prediction, we treat them as exact measurements. We define
% \begin{equation}
%     \chi^2(\lambda) = \left( \frac{\overline{X}_i-X_i^{\rm exp}}{{\rm SD}(X_i)}\right)^2\;,
% \end{equation}
% where $X_i^{\rm exp}$ is the experimentally measured value, $\overline{X}_i$ is the mean value of the prediction distribution of $X_i$, and ${\rm SD}(X_i)$ is the standard deviation of the prediction. Here we take
% \begin{align*}
%     X_i&\in \Big\{ \frac{m_u}{m_t},\frac{m_c}{m_t},\frac{m_d}{m_b},\frac{m_s}{m_b},\frac{m_e}{m_\tau},\frac{m_\mu}{m_\tau},\frac{\Delta m_{21}^2}{\Delta m_{32}^2},\\
%     &\qquad \qquad  \theta_{12}^{\rm C},\theta_{23}^{\rm C},\theta_{13}^{\rm C},\delta^{\rm C},\theta_{12}^{\rm P},\theta_{23}^{\rm P},\theta_{13}^{\rm P}\Big\}\;,
% \end{align*}
% where $\delta^{\rm P}$ is not included due to the large uncertainty. With the help of such a definition, we could say that the best value for $\lambda$ is the one that takes the smallest $\chi^2(\lambda)$, which is
Combing with Eq.~\eqref{eq:chi2} we do a parameter scan to get the best value of $\lambda$, that is
\begin{equation}
...
In comparison, we also calculate the $\chi^2$ by using the best three charge assignments in Table. I of Ref.~\cite{Cornella:2023zme}, we find that our model has smaller $\chi^2$.


\section{Predictions on $m_\nu^i$ and $m_{ee}$}\label{sec:neutrino}

In Sec.~\ref{sec:fermion}, we have built a consistent FN model, and its predictions of 15 parameters agree very well with experimental observations. Based on this success, we are more interested in its predictions on the neutrino sector, especially, the $m_\nu^i$ and $m_{ee}$.
As we mentioned in Sec.~\ref{sec:fermion},
the FN mechanism naturally prefers the NO.
As a crosscheck, we randomly generate a neutrino Yukawa matrix, $y_\nu$, whose eigenvalues are $\{D_\nu^i\}$. If the average of $\{D_\nu^1, D_\nu^2, D_\nu^3 \}$ is smaller than the Median, $y_\nu$ is Inverted Order (IO). Vice versa. By sampling $10^6$ times using our FN charge with $\lambda=0.171$ and $\sigma=0.3$, we find that NO takes up $\sim 98\%$. Therefore, we only consider NO in this work.
% Figure environment removed
% Figure environment removed
% Figure environment removed
...
\end{equation}
In the rightmost plot of Fig.~\ref{fig:neutrino_mass} we demonstrate the PDF of $m_{ee}$. We find that most probable value of $m_{ee}$ locates at $\sim 3\times 10^{-3}\,{\rm eV}$, and $m_{ee}$ is bounded by
\begin{equation}
    0.503 \,{\rm meV} \lesssim m_{ee} \lesssim 18.0\,{\rm meV}\;,
\end{equation}
at $95\%$ C.L..  Besides, in Fig.~\ref{fig:mee_m1}, we show the model prediction (blue dots) on the $m_\nu^1$-$m_{ee}$ plane. The green and red regions represent the inverted order (IO) and NO case respectively. The gray region is excluded by cosmological observation~\cite{ParticleDataGroup:2022pth}. Clearly, it shows most of the predictions overlap with the NO region. The near future $0\nu \beta \beta$ experiment LEGEND-1000 can check the validity of our modem to some extent \cite{LEGEND:2021bnm}.
