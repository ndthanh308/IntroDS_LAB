...
\begin{abstract}
    ...
    Lastly, we include some discussion on the constants for the joints of varieties problem. 
\end{abstract}
...
Lastly, we make a brief remark here that \cref{theorem:SharpJoints} gives a genuinely new (though way too long) proof of Lovasz's version of the Krsukal--Katona theorem.
The connection will be made clear in \cref{section:Graph}.
...
Since the tight configuration \cref{example:tight} is determined by several hyperplanes, it is natural to try to generalize the construction and see if it gives good lower bound on the constants $C_{k_1,\ldots, k_r;m_1,\ldots, m_r}$.
This type of constructions has been already considered by Zhao and the second author in \cite{YZ23}, where they call this type of configurations \emph{generically induced}.
...
In \cite{YZ23}, Zhao and the second author conjectured that the answer to this question is the same as the answer to the corresponding joints problem.
This conjecture remains open, but the message behind the conjecture is that it seems hard to come up with constructions with many joints that are not generically induced.
...
On the other hand, in a related work \cite{CY23}, we proved some sharp upper bounds for the number of joints in the set-theoretic setting via entropy method in some cases, some of which are better than the upper bounds obtained from the polynomial method. 
We refer the readers to that paper for a more in-depth discussion.

\subsection{Outline}The paper is structured as follows. 
...
Lastly, in \cref{section:Graph}, we state the set-theoretic version of the joints problem and discuss some special cases.
...
% \begin{proposition}
%     Assume $f\in \FF[x_1,\dots,x_d]$, $\ell$ is a line in $\FF^d$, and $p$ is a point on $\ell$. Let $f|_{\ell}$ be the univariate polynomial obtained from restricting $f$ to $\ell$. It follows that 
%     \[\mult(f,p)\leq \mult(f|_{\ell}, p).\]
% \end{proposition}

% \begin{proposition}
%     Assume $f\in \FF[x]$ is a univariate polynomial with degree at most $d$. If 
%     \[\sum_{p\in\FF}\mult(f,p)> d ,\]
...
\begin{proof}
    ...
    \begin{claim}\label{claim:VolumeSegment}
        Fix any $\beta_3,\dots,\beta_d >0$. For any  $\beta_1,\beta_2,\gamma_3,\dots,\gamma_d,r\in [0,1]$, let $\eta (\beta_1,\beta_2,\gamma_3,\dots,\gamma_d,r)$ be the length of the segment $\cS(\beta_{1},\ldots, \beta_{d})\cap \{(a_1,\dots,a_d)\in\RR_{\geq 0}^d\mid a_3=\gamma_3,\dots,a_d=\gamma_d,\,a_1+a_2=r\}$. It follows that, conditioning on $\beta_1+\beta_2$ being fixed, the length $\eta(\beta_1,\beta_2,\gamma_3,\dots,\gamma_d,r)$ is maximized when $\beta_1=\beta_2$. Moreover, if $\gamma_3=\dots=\gamma_d=0$ and $r=\frac{\beta_1+\beta_2}{2}$, the maximum value is only attended at $\beta_1=\beta_2$.
    \end{claim}
    \begin{proof}
        On the plane $a_3=\gamma_3,\dots,a_d=\gamma_d$, we know that the line $a_1+a_2=r$ is parallel to the line $a_i+\beta_i(\abs{\vec{a}}-a_i)\leq \beta_i$ for all $i\geq 3$.
        If $a_1+a_2=r$ is above $a_i+\beta_i(\abs{\vec{a}}-a_i)\leq \beta_i$ for some $i\geq 3$, then the segment is actually an empty set, and the statement clearly holds.
        
        Assume it is not the case, then $a_1+a_2=r$ is completely under $a_i+\beta_i(\abs{\vec{a}}-a_i)$ on the plane for all $i\geq 3$. For convenience, we will focus on the plane $a_3=\gamma_3,\dots,a_d=\gamma_d$ and drop the $i$-th coordinate for $i\geq 3$. Also, we will write $\eta(\beta_1,\beta_2)$ instead of $\eta(\beta_1,\beta_2,\gamma_3,\dots,\gamma_d,r)$ through out the proof of this claim.
        
        Let $\gamma=\gamma_3+\dots+\gamma_d$. By solving the linear system
        \begin{align*}
        \systeme*{a_1+\beta_1(a_2+\gamma)=\beta_1,a_1+a_2=r}
        \text{ and } 
        \systeme*{a_2+\beta_2(a_1+\gamma)=\beta_2,a_1+a_2=r}
        \end{align*}
        we know the two endpoints of the segment are 
        \[\left(\frac{\beta_1(1-r-\gamma)}{1-\beta_1},\frac{r+\beta_1\gamma-\beta_1}{1-\beta_1}\right)\text{ and }\left(\frac{r+\beta_2\gamma-\beta_2}{1-\beta_2},\frac{\beta_2(1-r-\gamma)}{1-\beta_2}\right)\] 
        whenever the solutions are nonnegative and the second point is on the upper-left of the first point. If second point is not on the upper-left of the first point, then the length is zero and the inequality clearly holds. Assume it is not the case. If $\frac{r+\beta_1\gamma-\beta_1}{1-\beta_1}<0$, we know that the first endpoint is $(r,0)$ instead. Also, if $\frac{r+\beta_2\gamma-\beta_2}{1-\beta_2}<0$, we know that the second endpoint is $(0,r)$ instead.
        
        Assume $\beta_1\leq \beta_2$. We want to show that, when slightly increasing $\beta_1$ and decreasing $\beta_2$ at the same rate, the length is non-decreasing. That is, we want to show that $\frac{d}{d\beta_1^+}\eta(\beta_1,\beta_2)-\frac{d}{d\beta_2^-}\eta(\beta_1,\beta_2)\geq 0$, where $\frac{d}{dx^+}$ means the right derivative and $\frac{d}{dx^-}$ means the left derivative. 
        From the coordinate of the endpoints, We know that $\frac{d}{d\beta_1^+}\eta(\beta_1,\beta_2)$ is either $0$ or $\sqrt{2}\frac{d}{d\beta_1}(\frac{r+\beta_1\gamma-\beta_1}{1-\beta_1})=\sqrt{2}\frac{r+\gamma-1}{(1-\beta_1)^2}$. Also, we know that $\frac{d}{d\beta_2^-}\eta(\beta_1,\beta_2)$ is either $0$ or $\sqrt{2}\frac{d}{d\beta_2}(\frac{r+\beta_2\gamma-\beta_2}{1-\beta_2})=\sqrt{2}\frac{r+\gamma-1}{(1-\beta_2)^2}$. Note that, if $\frac{r+\beta_1\gamma-\beta_1}{1-\beta_1}<0$, then $\frac{r+\beta_2\gamma-\beta_2}{1-\beta_2}<0$. Thus, there are only three cases. Namely, $\frac{d}{d\beta_1^+}\eta(\beta_1,\beta_2)$, $\frac{d}{d\beta_2^-}\eta(\beta_1,\beta_2)\geq 0$ are either both zero, $\sqrt{2}\frac{r+\gamma-1}{(1-\beta_1)^2}$ and zero, or $\sqrt{2}\frac{r+\gamma-1}{(1-\beta_1)^2}$ and $\sqrt{2}\frac{r+\gamma-1}{(1-\beta_2)^2}$. In all of the three cases, we all have $\frac{d}{d\beta_1^+}\eta(\beta_1,\beta_2)-\frac{d}{d\beta_2^-}\eta(\beta_1,\beta_2)\geq 0$, which shows the first part of the claim. 

        When $\gamma_3=\dots=\gamma_d=0$ and $r=\frac{\beta_1+\beta_2}{2}$, the inequality $a_i+\beta_i(\abs{\vec{a}}-a_i)\leq \beta_i$ becomes $a_1+a_2\leq 1$ for any $i\geq 3$, which is clearly above $a_1+a_2=r$.
        Furthermore, we know that the endpoints are $(r,0)$ and $(0,r)$ if $\beta_1=\beta_2$. However, when $\beta_1\neq \beta_2$, at least one of $\frac{r+\beta_1\gamma-\beta_1}{1-\beta_1},\frac{r+\beta_2\gamma-\beta_2}{1-\beta_2}$ is positive, showing that the length is strictly smaller.
    \end{proof}
    We know that $\vol\cS(\beta_{1},\ldots, \beta_{d})$ is the integration of $\eta (\beta_1,\beta_2,\gamma_3,\dots,\gamma_d,r)$ over $\gamma_3,\dots,\gamma_d,r\in [0,1]$. Thus, by using \cref{claim:VolumeSegment}, we know that $\vol\cS(\beta_{1},\ldots, \beta_{d})\leq \vol\cS(\frac{\beta_{1}+\beta_2}{2},\frac{\beta_{1}+\beta_2}{2},\ldots, \beta_{d})$. Moreover, by looking at the values around $\gamma_3=\dots=\gamma_d=0,r=\frac{\beta_1+\beta_2}{2}$, the inequality is strict if $\beta_1\neq \beta_2$ and $\beta_1,\dots,\beta_d>0$. Since $\vol\cS(\beta_{1},\ldots, \beta_{d})$ is symmetric and continuous on the compact region $[0,1]^d$, the first part of the lemma follows.
    ...
    %Note that the $2d$ hyperplanes 
    %\[a_i\geq 0,a_i+\beta_i(\abs{\vec{a}}-a_i)\leq \beta_i,i\in [d]\]
    %that define $\cS(\avgbeta ,\ldots, \avgbeta )$ 
\end{proof}
...
\begin{proof}
    ...
    % The sequence $g(\vec{z}^{(1)}),g(\vec{z}^{(2)}),\ldots$ does not necessarily converge.
    % However, as its range is bounded, we may pass to a subsequence to assume that the sequence converges.
    
\end{proof}

%A joints configuration $(\cJ,\cL)$ is \emph{critical} if for any joints configuration $(\cJ',\cL')$ with $L'<L$, we have $J'/L'<J/L$.
...
% Given the proof of \cref{theorem:SharpJoints} in \cref{section:Sharp}, it is natural to look at the inequalities in the proof and ask when the equalities all hold.
% In this way, we may immediately reach the conclusion that if $J=\binom{x}{d}$ and $L=\binom{x-1}{d}$, then $x=M$ is actually an integer, and each line contains exactly $M-d+1$ joints.
% This will be proved formally later, but for the sake of discussion, we will first assume this.
...
% A crucial observation is that by \cref{lemma:GeneralVanishing}, there must exist $p\in \cJ$ such that $f$ is not $S_p$-vanishing at $p$ while being $S_p(n,T_{\vec{z}},t_{\textup{f}})$-vanishing.
% This allows us to control the nonzero term of smallest order of $f$ expanded at some joint $p$, and we will show soon that this knowledge alone allows us to deduce similar results but for all joints.
% To make the statement more concise, we call a vector $\vec{\alpha}$ \emph{$\varepsilon$-close to the apex} if $\abs{\alpha_i-n/M}\leq \varepsilon n/M$ for all $i\in [d]$.
% The statement below shows a way to deduce $O(\varepsilon)$-closeness from a weaker condition that $\abs{\vec{\alpha}}$ is close to $dn/M$ along with some other conditions.
...
% \begin{lemma}\label{lemma:ExistCloseToApex}
%     Let $(\cJ,\cL)$ be a tight joints configuration with $\binom{M}{d}$ joints and $\binom{M}{d-1}$ lines.
%     Given $\varepsilon\in (0,1)$, suppose $f$ is a nonzero polynomial given by \cref{lemma:StructuralPolynomial}.
%     Then as long as $n$ is sufficiently large with respect to $\varepsilon$, there exists a constant $C=C(\cJ,\cL)$ that does not depend on $n$ and $\varepsilon$ such that the following holds: for every $p\in\cJ$ there exists $\vec{\alpha}\in \ZZ_{\geq 0}^d$ that is $C\varepsilon$-close to the apex with $D^{\vec{\alpha}}_pf(p)\neq 0$.
% \end{lemma}
% \begin{proof}
%     Note that there exists $p^*\in\cJ$ such that $f$ is not $S_{p^*}$-vanishing at $p$ but it is $S_{p^*}(n,T_{\vec{z}},t_{\textup{f}})$-vanishing.
%     This shows that there exists $\vec{\alpha}^*\in\ZZ_{\geq 0}^d\in S_p(n,T_{\vec{z}})$ with $\abs{\vec{\alpha}}\geq (1-\varepsilon)dn/M-1$ and $D^{\vec{\alpha}^*}_{p^*}f(p^*)\neq 0$.
%     By \cref{lemma:ClostToApex}, when $n$ is sufficiently large, $\vec{\alpha}$ is $4d^2\varepsilon$-close to the apex.
%     % Recall that $\vec{\alpha}^*\in\ZZ_{\geq 0}^d\in S_p(n,T_{\vec{z}})$ implies that
%     % \[a_i^*+\frac{1}{M-d+1}(\abs{\vec{a}^*}-a_i^*)\leq\frac{1}{M-d+1}+o(1)\]
%     % for all $i\in[d]$, where $\vec{a}^* = \vec{\alpha}^*/n$.
%     % Plugging in $\abs{\vec{a}^*}\geq (1-\varepsilon)d/M -o(1)$ and rearranging, we get
%     % \[a_i^*\leq \frac{1}{M}+\frac{d}{M(M-d)}\varepsilon+o(1).\]
%     % Therefore
%     % \[(1-\varepsilon)\frac{d}{M}-o(1)\leq a_i^*+(\abs{\vec{a}^*}-a_i^*)\leq a_i^*+(d-1)\left(\frac{1}{M}+\frac{d}{M(M-d)}\varepsilon\right)+o(1),\]
%     % implying that
%     % \[a_i^*\geq \frac{1}{M}-\frac{d(M-1)}{M(M-d)}\varepsilon-o(1).\]
%     % Therefore, as long as $n$ is sufficiently large with respect to $\varepsilon$, we have that $\vec{\alpha}^*$ is, say, $d\varepsilon/M$-close to the apex. 
    
%     Since the joints configuration is connected by \cref{lemma:PrelimStructure}, it suffices to show that there exists a constant $C'=C'(\cJ,\cL)$ such that for every $p,p'\in\cJ$ lying on a line $\ell$, if there exists $\vec{\alpha}\in\ZZ^d_{\geq 0}$ that is $\varepsilon'$-close to the apex for some $\varepsilon'\geq \varepsilon$ with $D^{\vec{\alpha}}_pf(p)\neq 0$, then there exists $\vec{\alpha}'\in\ZZ^d_{\geq 0}$ that is $C'\varepsilon'$-close to the apex with $D^{\vec{\alpha}'}_{p'}f(p')\neq 0$.
%     Upon relabeling, we may assume that $\ell = \ell_{p,1} = \ell_{p',1}$.

%     Let $\vec{\beta} = \vec{\alpha}-\alpha_1\vec{e}_1$.
%     Then $\abs{\abs{\vec{\beta}}-(d-1)n/M}\leq C\varepsilon'(d-1)n/M$ and $\left.D^{\vec{\beta}}_pf\right|_{\ell}$ is not zero.
%     As $D^{\vec{\beta}}_p$ is a linear combination of $\left\{D^{\vec{\beta}'}_{p'}: \abs{\vec{\beta}'}=\abs{\vec{\beta}}, \beta_1'=0\right\}$, there exists one such $\vec{\beta}'$ with $\left.D^{\vec{\beta}'}_{p'}f\right|_{\ell}\not\equiv 0$.
%     Set $g \eqdef \left.D^{\vec{\beta}'}_{p'}f\right|_{\ell}$, and let $\vec{\alpha}' = \vec{\beta}' + \mult\left(g,p'\right)\vec{e}_1$.
%     Then $D^{\vec{\alpha}'}_{p'}f(p')\neq 0$ by definition, and it remains to show that $\vec{\alpha}'$ is close to the apex.
%     By \cref{lemma:ClostToApex}, it suffices to show that $\abs{\vec{\alpha}'}$ is close to $dn/M$.
%     We first note that by \cref{lemma:GeneralVanishing}, for any $p''\in \ell$,
%     \[\mult(g,p'')\geq \mult(f,p'')-\abs{\vec{\beta}'}\geq (1-\varepsilon)\frac{dn}{M}-1-(d-1)\left(1+\varepsilon'\right)\frac{n}{M}\geq (1-2d\varepsilon')\cdot\frac{n}{M}\]
%     when $n$ is sufficiently large with respect to $\varepsilon$.
%     In particular, when we take $p''=p'$, we have that $\alpha_1' = \mult(g,p')\geq (1-2d\varepsilon')n/M$.
%     To upper bound $\alpha_1'$, note that $\sum_{p''\in\ell}\mult(g,p'')\leq \deg g$.
%     Therefore,
%     \[\alpha_1'\leq \deg g - \sum_{p'\neq p''\in\ell}\mult(g,p'')\leq n-\abs{\vec{\beta}'}-(M-d)(1-2d\varepsilon')\cdot\frac{n}{M}< \left(1+2dM\varepsilon'\right)\cdot\frac{n}{M}.\]
%     As a consequence, $\abs{\vec{\alpha}'}=\alpha_1'+\abs{\vec{\beta}'}$ is between $(1-2dM\varepsilon')dn/M$ and $(1+2dM\varepsilon')dn/M$, and we are done by \cref{lemma:ClostToApex}. 
% \end{proof}
...
\begin{proof}[Proof of \cref{lemma:Structural}]
    ...
    By the choice of $\vec{\alpha}^*$, the summand on the right hand side is $0$ if $\min_{i\in[d]}\alpha_i < (1-O_{d,M}(\varepsilon))n$.
...
First, if the number of joints is $1-o(1)$ away from tight, then we can not expect \cref{lemma:Structural} holds on the whole configuration. Here is an example. Consider a tight joints configuration and add a point $p$ on one of the lines $\ell$, then pick $d-1$ lines through $p$ with arbitrary directions that makes $p$ a joint. In this case, we can pick the directions so that $p$ does not share any hyperplane with any other $p'\in\ell$. Note that this example meets the set-theoretic Kruskal--Katona bound. 
This configuration indicates that we should consider a sub-configuration instead.
We make a conjecture in this direction.
\begin{conjecture}
...
We believe that in those cases, the optimum is still achieved by generically induced configurations, which in turn suggests that the Kruskal--Katona bound should be the optimum we expect.
\begin{conjecture}
    Let $(\cJ,\cL)$ be a joints configuration in $\FF^d$.
    If $J=\binom{M_d}{d}+\binom{M_{d-1}}{d-1}+\cdots+\binom{M_k}{k}$ where $M_d>M_{d-1}>\cdots >M_k\geq k$, then $L\geq \binom{M_d}{d-1}+\cdots+\binom{M_k}{k-1}$.
\end{conjecture}
We believe that there is a chance to get the structural results for all $J$'s if we could prove the conjecture using polynomial method.
...
In this section, we will investigate the set-theoretic version of the joints problems.
Using this, we will disprove a conjecture proposed in \cite{TYZ22}, show that $C_{k,k,k;1,1,1}$ grows exponentially in $k$, and also relate the joints problem to a closely related paper of ours \cite{CY23}.
...
\begin{definition}
    Given positive integers $k_1,\dots,k_r,m_1,\dots,m_r$, let $d=k_1m_1+\dots+k_rm_r$ and $s=m_1+\dots+m_r$. Given $\bF_1,\dots,\bF_r$, where $\bF_i$ is a family of $d-k_i$-subsets of $\NN$, we say that $P\subseteq \NN$ is a \emph{$(k_1,\dots,k_r;m_1,\dots,m_r)$-joint set} if there exist sets $F_1,\dots,F_s$, where $m_i$ of them are from $\bF_i$, such that $P\setminus F_1,\dots ,P\setminus F_s$ partition $P$. 

    We say that $(\bJ;\bF_1,\dots,\bF_r)$ is a \emph{$(k_1,\dots,k_r;m_1,\dots,m_r)$-joint set system} if every $P\in\bJ$ is a $(k_1,\dots,k_r;m_1,\dots,m_r)$-joint set of $\bF_1,\dots,\bF_r$.
\end{definition}
For example, $(\bJ,\bF)$ is a $(1;d)$-joint set system if and only if $\bF$ contains $\partial \bJ$, where $\partial \bJ$ is the set of all the $(d-1)$-subsets of $P\in\bJ$. 
The Kruskal--Katona theorem gives a lower bound on the size of $\bF$ given the size of $\bJ$.

We know that every $(k_1,\dots,k_r;m_1,\dots,m_r)$-joint set system gives a $(k_1,\dots,k_r;m_1,\dots,m_r)$-joints configuration. Thus, we are also interested in the upper bound of $|\bJ|$ in terms of $|\bF_1|,\dots,|\bF_r|$. From \cite{TYZ22}, we know that \[|\bJ|=O((|\bF_1|^{m_1}\dots|\bF_r|^{m_r})^{1/(s-1)}).\] 
Hence, we may define $C^G_{k_1,\dots,k_r;m_1,\dots,m_r}$ to be the smallest constant such that
\[|\bJ|\leq C^G_{k_1,\dots,k_r;m_1,\dots,m_r}(|\bF_1|^{m_1}\dots|\bF_r|^{m_r})^{1/(s-1)}.\]
It follows that 
\[C^G_{k_1,\dots,k_r;m_1,\dots,m_r}\leq C_{k_1,\dots,k_r;m_1,\dots,m_r}.\]
