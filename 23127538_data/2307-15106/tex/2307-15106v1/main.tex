...
{Towards non-perturbative BV-theory\\via derived differential cohesive geometry}\vspace{0.3cm}}
...
We investigate two classes of examples of non-perturbative classical BV-theories in the context of derived differential cohesion: scalar field theory and Yang-Mills theory. 
\noindent 
...
\begin{equation}
    \mathbf{dFMfd} \; \coloneqq\; \mathbf{sC^\infty Alg}^\op_{\mathrm{afp}},
\end{equation}
where $\mathbf{sC^\infty Alg}_{\mathrm{afp}}$ is the $(\infty,1)$-category of almost finitely presented $\Coo$-algebras.
...
\paragraph{Derived differential cohesion.} 
in sections \ref{sec:derdiffcohe} we show that the formalism of differential cohesion introduced by Schreiber in \cite{DCCTv2} extends very naturally to the derived smooth setting.
First of all, we show that the $(\infty,1)$-topos of formal derived smooth stacks is cohesive.
Roughly speaking, a cohesive structure provides to an $(\infty,1)$-topos the properties required for a geometry to take place in it and for its objects to be fully-fledged spaces.

\textbf{Theorem 4.2} (Cohesive $(\infty,1)$-topos of formal derived smooth stacks)\textbf{.}
The $(\infty,1)$-topos of formal derived smooth stacks $\mathbf{dFSmoothStack}$ is cohesive.

We will see that the structure of derived cohesion induces the following triplet of adjoint endofunctors:
\begin{equation}
   ({\textstyle\int} \;\dashv\;  \flat \;\dashv\; \sharp):\,\mathbf{dFSmoothStack}\,\longrightarrow\,\mathbf{dFSmoothStack},
\end{equation}
where we respectively have:
\begin{enumerate}[label=(\textit{\roman*})]
    \item \textit{shape modality} ${\textstyle\int}$,
    \item \textit{flat modality} $\flat$,
    \item \textit{sharp modality} $\sharp$.
\end{enumerate}

More specifically, to equip an $(\infty,1)$-topos with a notion of differential geometry, a cohesive structure is not enough: we need differential cohesion.
With the following theorem, we show that formal derived smooth stack come naturally equipped also with a differential cohesive structure, which we will call derived differential cohesion.

\textbf{Theorem 4.7} (Differential cohesive $(\infty,1)$-topos of formal derived smooth stacks)\textbf{.}
The cohesive $(\infty,1)$-topos $\mathbf{dFSmoothStack}$ of formal derived smooth stacks is naturally equipped with a differential cohesive structure.

Such a structure, which we will call derived differential cohesive structure, induces the following triplet of adjoint endofunctors:
\begin{equation}
...
\end{enumerate}
Differential cohesive geometry underpins the definition of the de Rham space $\Im(X)$ of any formal derived smooth stack $X$ by the infinitesimal shape modality. This could be interpreted as an infinitesimal version of the path $\infty$-groupoid of $X$ and its role will be pivotal.
...
\textbf{Proposition 4.43} (Infinitesimal cohesive $(\infty,1)$-topos of formal moduli problems)\textbf{.} 
...
We will make this relation precise in terms of a structure known as relative cohesion,
which induces the following triple of adjoint endofunctors
\begin{equation}
   ({\textstyle\int}^{\mathrm{rel}} \;\dashv\;  \flat^{\mathrm{rel}} \;\dashv\; \sharp^{\mathrm{rel}}):\,\mathbf{dFSmoothStack}\,\longrightarrow\,\mathbf{dFSmoothStack},
\end{equation}
where we respectively have
\begin{enumerate}[label=(\textit{\roman*})]
    \item \textit{relative shape modality} ${\textstyle\int}^{\mathrm{rel}}$,
    \item \textit{relative flat modality} $\flat^{\mathrm{rel}}$,
    \item \textit{relative sharp modality} $\sharp^{\mathrm{rel}}$.
\end{enumerate}
...
\begin{tabular}{||c | c | c | c | c ||} 
 ...
 \hline
 \multirow{2}{*}{Algebras} & Commutative & Homotopy comm. & \multirow{2}{*}{$\Coo$-algebras} & Homotopy \\ 
...
This example can be directly generalised to construct infinitesimally thickened smooth manifolds.
...
We say that a morphism $f:X\rightarrow Y$ of $\Coo$-varieties is \textit{formally \'etale} if we have a pullback diagram
...
\begin{corollary}[Formally \'etale maps generalise local diffeomorphisms]
Let $M$ and $N$ be ordinary smooth manifolds, seen as $\Coo$-varieties. Then, we have that any formally \'etale map $f:M\rightarrow N$ is equivalently a local diffeomorphism in the ordinary differential geometry sense.
\end{corollary}
\begin{proof}
...
\begin{theorem}[Covering family of a $\Coo$-variety]\label{lem:covering_var}
...
\begin{proof}
First, we show that formal \'etale morphisms are stable under pullback.
...
\begin{equation}
    ...
    \end{tikzcd}
\end{equation}
where we assume that $\phi$ is a formally \'etale map.
...
Moreover, $\phi$ being formally \'etale implies that the right square is a pullback too. Then, by applying the pasting law for pullbacks we obtain that the left square is a pullback and thus $\psi$ is formally \'etale. Therefore, formally \'etale maps are stable under pullbacks.
...
\begin{equation}
    ...
    \end{tikzcd}
\end{equation}
Since monomorphisms and formally \'etale monomorphisms are stable under pullbacks, then $\psi_i$ is a formally \'etale monomorphism.
Moreover, we have that $\coprod_{i\in I}Y\times_X U_i \xrightarrow{\;(\psi_i)_{i\in I}\;} Y$ is a formally \'etale epimorphism.
\end{proof}
...
We define a \textit{formal smooth manifold} $M$ as a $\Coo$-variety such that there exist a family $\{\bbR^{n\!}\times \Spec W \,\xhookrightarrow{\;\,\phi_i\,\;}\, M\}_{i\in I}$ of \'etale monomorphisms, where $W$ is Artinian, with the property that the induced map
\begin{equation}
...
Thus, a \textit{homotopy $\Coo$-algebra} is going to be defined as homotopy algebra over the Lawvere theory $\mathsf{CartSp}$.
Accordingly, we can define the model category of {homotopy $\Coo$-algebras} by
\begin{equation}
...
In analogy with the discussion in \cite[Section 2]{Calaque:2017}, we can introduce a weaker notion of being finitely presented for homotopy $\Coo$-algebras as follows.

\begin{definition}[Almost finitely presented $\Coo$-algebra]
An \textit{almost finitely presented $\Coo$-algebra} is defined as a homotopy $\Coo$-algebra $A\in\mathbf{sC^\infty Alg}$ such that $\pi_0A$ is finitely generated as an ordinary $\Coo$-algebra and the $\pi_iA$ are finitely presented modules over $\pi_0A$ for all $i>0$.
The $(\infty,1)$-category of almost finitely presented $\Coo$-algebras $\mathbf{sC^\infty Alg}_\mathrm{afp} \longhookrightarrow \mathbf{sC^\infty Alg}$ is defined as the full sub-category on those objects which are almost finitely presented $\Coo$-algebras.
\end{definition}

\begin{proposition}[Finitely presented $\Coo$-algebras are almost finitely presented]
We have the following full sub-$(\infty,1)$-categories of homotopy $\Coo$-algebras:
\begin{equation}
    \mathbf{sC^\infty Alg}_\mathrm{fp} \,\longhookrightarrow\, \mathbf{sC^\infty Alg}_\mathrm{afp} \,\longhookrightarrow\, \mathbf{sC^\infty Alg}.
\end{equation}
\end{proposition}

\begin{proof}
By \cite[Section 2.1]{toen2007moduli}, any homotopically finitely presented $\Coo$-algebra is equivalent to the retract of a strict finite cell object $\{X_i\}_{i=0,\dots,n}$, characterised by $X_{i+1}\simeq B \widehat{\otimes}_A^\bfL X_i$ for $0\leq i <n$, where $A,B$ and $X_{i>0}$ are all finitely generated free algebras, namely of the form $\Coo(\bbR^n)$, and $X_0=\emptyset$.
Thus, it is sufficient to show that any such $X_n$ is almost finitely presented, which we will do by induction.
First, if $n=0$, the result is trivial. Second, if we assume the result for strict finite cell objects of length $n-1$, then, for any length $n$ strict finite cell object, there is a spectral sequence converging to the homotopy groups of $X_n$ given by $E_{p\bullet}^2=\mathrm{Tor}^A_p(B, \pi_\bullet X_{n-1})$. Since the $\pi_{\bullet} X_{n-1}$ must be finitely presented, then the $\pi_\bullet X_n$ are finitely presented.
\end{proof}
...
\begin{equation}
    \mathbf{dFMfd} \; \coloneqq\; \mathbf{sC^{\infty\!}Alg}^\op_{\mathrm{afp}},
\end{equation}
where $\mathbf{sC^{\infty\!}Alg}^\op_{\mathrm{afp}}$ is the $(\infty,1)$-category of almost finitely presented $\Coo$-algebras.
\end{definition}
...
If we restrict the functor $\pi_0$ to almost finitely presented $\Coo$-algebras, we obtain $\pi_0:\mathbf{sC^\infty Alg}_{\mathrm{afp}}\rightarrow \mathbf{N}(\mathsf{C^\infty Alg}_\mathrm{fg})$, where $\mathsf{C^\infty Alg}_\mathrm{fg}$ is the ordinary category of finitely generated $\Coo$-algebras.
...
Now, given the definition of the $(\infty,1)$-site of formal derived smooth manifolds, we can apply the general discussion above about derived stacks to our case of interest. By \cite[Theorem 3.4.1]{ToenVezzo05} there exists a local projective model structure on the simplicial-category of pre-stacks $[\mathsf{sC^\infty Alg}_\mathrm{afp},\mathsf{sSet}]$ induced by the definition of formally \'etale maps of formal derived smooth manifolds. Thus, by localisation of such a simplicial model structure, one can obtain the simplicial category of formal derived smooth stacks, i.e.
\begin{equation}
    \mathsf{dFSmoothStack} \;\coloneqq\; [\mathsf{sC^\infty Alg}_\mathrm{afp},\mathsf{sSet}]_{\mathrm{proj,loc}}^\circ.
\end{equation}
\end{construction}
...
\begin{aligned}
    ...
    \Big\{U_{i}^2 \,\rightarrow \!\!\!\!\coprod_{j_1,j_2,j_3\in I_1}\!\!\!\! U_{j_1\!}^1\times_{U\!}U_{j_2\!}^1\times_{U\!}U_{j_3}^1\Big\}_{i\in I_2} \\
    \vdots \quad
\end{aligned}
\end{equation}
...
\end{equation}
i.e. by the $(\infty,1)$-category of stacks on the \'etale $(\infty,1)$-site presented by $\mathsf{dFMfd}= \mathsf{sC^\infty Alg}^\op_{\mathrm{afp}}$ of formal derived smooth manifolds.
\end{definition}
...
\begin{proof}
Recall that we have an embedding $\mathbf{dFMfd}\simeq\mathbf{sC^{\infty\!}Alg}_\mathrm{afp}^\op\longhookrightarrow \mathbf{sC^{\infty\!}Alg}^\op$.
...
There is a natural (coreflective) embedding $\mathbf{dFMfd}\simeq\mathbf{sC^{\infty\!} Alg}_\mathrm{afp}^\op\hookrightarrow \mathbf{sC^{\infty\!}Alg}^\op$, since any derived smooth manifold $M$ is immediately equivalent to the spectrum of its homotopy $\Coo$-algebra of functions, i.e. $M\simeq\bfR\Spec\mathcal{O}(M)$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Derived differential cohesive geometry}\label{sec:derdiffcohe}

In the previous section, we constructed the $(\infty,1)$-category $\mathbf{dFSmoothStack}$ of formal derived smooth stacks.

In this section, we show that the formalism of differential cohesion, introduced by Schreiber \cite{DCCTv2} in the setting of formal smooth stacks, extends very naturally to our present setting of formal derived smooth stacks.
...
In subsection \ref{subsec:cohesion}, we show that the $(\infty,1)$-topos of formal derived smooth stacks comes naturally equipped with a cohesive structure.
Roughly speaking, a cohesive structure provides an $(\infty,1)$-topos with the properties required for a geometry to take place in it and for its objects to be fully-fledged spaces.
In subsection \ref{subsec:ddiffcohe}, we prove that the $(\infty,1)$-topos of formal derived smooth stacks comes also naturally equipped with a differential cohesive structure.
In subsection \ref{subsec:FMP}, we will show that the formal moduli problems appearing in BV-theory naturally arise in the context of differential cohesion.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Derived cohesion}\label{subsec:cohesion}

After showing that formal derived smooth stacks constitute an $(\infty,1)$-topos, we will investigate its natural cohesive structure.
The notion of cohesive topos originated in Lawvere's seminal work \cite{Lawvere:1986CategoriesOS, Lawvere:1994, Lawvere:2003, Lawvere:2007}.
The definition of cohesive $(\infty,1)$-topos is given by \cite[Section 4.1]{DCCTv2}.
To facilitate the reading for the broadest possible audience, we will provide now an informal picture of a cohesive $(\infty,1)$-topos. For a detailed and comprehensive discussion of cohesion, we point at the main reference.
The concept of a cohesive topos provides a unifying framework for studying a range of structures, including smooth manifolds, algebraic varieties, and more general spaces that admit some notion of local chart.
At its core, a cohesive topos is a category of sheaves over a site that satisfies certain axioms, which ensure that it has enough structure to capture the basic features of smooth and topological spaces. 
...
Roughly, a cohesive structure is the lift of the geometric morphism $\mathrm{Disc}\dashv\mathit{\Gamma}$ to a quadruple of adjoint functors $(\mathit{\Pi}\dashv\mathrm{Disc}\dashv\mathit{\Gamma}\dashv\mathrm{coDisc})$, which will look as follows:
\begin{equation}
    \begin{tikzcd}[row sep={12ex,between origins}, column sep={12ex}]
    \mathsf{Sh(C)} \arrow[r, "\mathit{\Pi}", shift left=3.4ex]\arrow[r, "\mathit{\Gamma}", shift right=1.1ex] &  \mathsf{Set}  \arrow[l, "\mathrm{Disc}"', shift right=1.1ex, hook]  \arrow[l, "\mathrm{coDisc}"', shift left=3.4ex, hook]  ,
    \end{tikzcd}
\end{equation}
where $\mathrm{Disc}$ and $\mathrm{coDisc}$ must be fully faithful and $\mathit{\Pi}$ must preserve finite products.
As constructed and explained in plenty of detail by \cite{DCCTv2}, this construction can be generalised to a $(\infty,1)$-topos of stacks if we replace the ordinary category of sets with the $(\infty,1)$-category of $\infty$-groupoids. The following concrete definition is precisely \cite[Remark 4.1.9]{DCCTv2}.

\begin{definition}[Cohesive $(\infty,1)$-topos]
A \textit{cohesive structure} on an $(\infty,1)$-topos $\mathbf{H}$ is the datum of a quadruple of adjoint $(\infty,1)$-functors of the following form:
\begin{equation}\label{eq:cohesive_strcuture}
    \begin{tikzcd}[row sep={12ex,between origins}, column sep={12ex}]
    \mathbf{H}\, \arrow[r, "\mathit{\Pi}", shift left=3.4ex]\arrow[r, "\mathit{\Gamma}", shift right=1.1ex] &  \mathbf{\infty Grpd} , \arrow[l, "\mathrm{Disc}"', shift right=1.1ex, hook]  \arrow[l, "\mathrm{coDisc}"', shift left=3.4ex, hook]  
    \end{tikzcd}
\end{equation}
such that:
\begin{enumerate}[label=(\textit{\roman*})]
    \item the $(\infty,1)$-functor $\mathit{\Gamma}$ is the global section functor.
    \item the $(\infty,1)$-functors $\mathrm{Disc}$ and $\mathrm{coDisc}$ are fully faithful,
    \item the $(\infty,1)$-functor $\mathit{\Pi}$ preserves finite products. 
\end{enumerate} 
\end{definition}


We are now ready to show that the $(\infty,1)$-topos $\mathbf{dFSmoothStack}$ of formal derived smooth stacks comes equipped with a natural cohesive structure.


\begin{theorem}[Cohesive $(\infty,1)$-topos of formal derived smooth stacks]
The $(\infty,1)$-topos of formal derived smooth stacks $\mathbf{dFSmoothStack}$ is cohesive.
\end{theorem}

\begin{proof}
Recall that $\bbR^0$ is the terminal object in the category of formal derived smooth manifolds and its $\Coo$-algebra $\bbR=\Coo(\bbR^0)$ is the initial object in the category of homotopy $\Coo$-algebras. 
Let $\ast_\bbR \coloneqq \{\bbR\}$ be its full and faithful sub-category whose only object is initial object $\bbR$.
Then, we have the following co-reflective inclusion:
\begin{equation}
    \begin{tikzcd}[row sep=scriptsize, column sep=8.2ex, row sep=18.0ex]
     \ast_\bbR \arrow[r, "\iota^\bbR", shift left=0.8ex, hook] &  \mathsf{sC^\infty Alg}_\mathrm{afp}. \arrow[l, "\bbR", shift left=0.8ex]
    \end{tikzcd}
\end{equation}
By left and right Kan extension and by essential uniqueness of the adjoint functor, we have the quadruple of simplicially-enriched adjoint functors
\begin{equation}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={15ex}]
     {[\mathsf{sC^\infty Alg}_\mathrm{afp},\,\sSet]} \arrow[r,leftarrow, "\,\,\,\iota^\bbR_\ast\,\,\phantom{,\simeq\,\;\bbR_\ast}", shift left=-4.5ex]\arrow[r,leftarrow, "\,\iota^{\bbR\!}_! \,\,\,\simeq\,\,\bbR^\ast", shift right=-1.5ex] &  {[\ast_\bbR,\sSet]}\simeq\sSet  \arrow[l, leftarrow, "\,\,\,\iota^{\bbR\ast\!}\,\simeq\,{\bbR_\ast\!}\,\,\,"', shift right=-1.5ex]  \arrow[l, leftarrow, "\phantom{\iota^\bbR_!\,\,\simeq}\,\,{\bbR}_!"', shift left=-4.5ex].
    \end{tikzcd}
\end{equation}
Now, we can immediately induce the quadruple of simplicial Quillen adjoint functors for the global projective model structure, i.e.
\begin{equation}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={15ex}]
     {[\mathsf{sC^\infty Alg}_\mathrm{afp},\,\sSet]}_\mathrm{proj} \arrow[r,leftarrow, "\,\,\,\iota^\bbR_\ast\,\,\phantom{\simeq\,\;\bbR_\ast}", shift left=-4.5ex]\arrow[r,leftarrow, "\,\iota^{\bbR\!}_! \,\,\,\simeq\,\,\bbR^\ast", shift right=-1.5ex] &  {[\ast_\bbR,\sSet]}_\mathrm{proj}\simeq\sSet_\mathrm{Quillen}  \arrow[l, leftarrow, "\,\,\,\iota^{\bbR\ast\!}\,\simeq\,{\bbR_\ast\!}"', shift right=-1.5ex]  \arrow[l, leftarrow, "\phantom{\iota^\bbR_!\,\,\simeq}\,\,{\bbR}_!"', shift left=-4.5ex],
    \end{tikzcd}
\end{equation}

Since $\iota^\bbR$ is the natural embedding of the algebra $\bbR$ into the simplicial-category of almost finitely presented $\Coo$-algebras, it is continuous and cocontinuous with respect to the \'etale site structure. Therefore, the adjoint triple $(\iota^{\bbR}_!\dashv\iota^{\bbR\ast}\dashv\iota^{\bbR}_\ast)$ restrict and corestrict to the corresponding simplicial model category of formal derived smooth stacks ${[\mathsf{sC^\infty Alg}_\mathrm{afp},\,\sSet]}_\mathrm{proj,loc}^\circ$.
Thus, we have only left to show that the functor $\bbR_!$ maps locally fibrant objects to Kan-fibrant simplicial sets. By construction, we have $\bbR_!X = \bfL\mathrm{co}\!\lim_{A\!}X(A)$ where all $X(A)\in\sSet_{\mathrm{Quillen}}^\circ$ are Kan-fibrant for any $X\in{[\mathsf{sC^\infty Alg}_\mathrm{afp},\,\sSet]}_\mathrm{proj,loc}^\circ$, which implies that $\bbR_!X$ is always Kan-fibrant.
Thus we have the following quadruple of adjoint $(\infty,1)$-functors:
\begin{equation}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={15ex}]
     {\mathbf{dFSmoothStack}} \arrow[r,leftarrow, "\,\,\,\iota^\bbR_\ast\,\,\phantom{\simeq\,\;\bbR_\ast}", shift left=-4.5ex]\arrow[r,leftarrow, "\,\iota^{\bbR\!}_! \,\,\,\simeq\,\,\bbR^\ast", shift right=-1.5ex] &  \mathbf{\infty Grpd}  \arrow[l, leftarrow, "\,\,\,\iota^{\bbR\ast\!}\,\simeq\,{\bbR_\ast\!}"', shift right=-1.5ex]  \arrow[l, leftarrow, "\phantom{\iota^\bbR_!\,\,\simeq}\,\,{\bbR}_!"', shift left=-4.5ex],
    \end{tikzcd}
\end{equation}
Finally, notice that the terminal functor $\bbR^\op:\mathsf{dFMfd}\rightarrow \ast_\bbR$ preserves finite products, since any finite product of formal derived smooth manifolds $\prod_{i\!}^h M_i$ is sent to $\prod_{i\!}^h\bbR^0 \simeq \bbR^0$. Thus, the functor $\bbR_!$ preserves finite products. Moreover, since the inclusion of the terminal object $(\iota^\bbR)^\op:\ast_\bbR\rightarrow \mathbf{dFMfd}$ is clearly fully faithful, then the functor $\iota^\bbR_!$ is also fully faithful.
\end{proof}
...
Just like any quadruple of adjoint $(\infty,1)$-functors, the derived differential cohesive structure presented by our diagram gives naturally rise to a triplet of adjoint $(\infty,1)$-endofunctors.

\begin{definition}[Modalities of derived cohesion]
...
\begin{enumerate}[label=(\textit{\roman*})]
    \item \textit{shape modality} ${\textstyle\int} \coloneqq \mathrm{Disc}\circ \mathit{\Pi}$,
    \item \textit{flat modality} $\flat \coloneqq \mathrm{Disc}\circ \mathit{\Gamma}$,
    \item \textit{sharp modality} $\sharp \coloneqq \mathrm{coDisc}\circ \mathit{\Gamma}$.
\end{enumerate}
\end{definition}

The term "modality" was imported by \cite{DCCTv2} from type theory and in this context can be read as saying that ${\textstyle\int}$ and $\sharp$ are indempotent monads and $\flat$ is an idempotent comonad.
Morally speaking, the shape ${\textstyle\int}X$ of a formal derived smooth stack $X$ can be interpreted as its path $\infty$-groupoid $\mathit{\Pi}(X)$ equipped with discrete derived formal smooth structure. Similarly, $\flat X$ can be seen as the $\infty$-groupoid $\mathit{\Gamma}(X)$ equipped with discrete derived formal smooth structure and $\sharp X$ with the codiscrete one.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Derived differential cohesion} \label{subsec:ddiffcohe}

Until now, we stressed the fact that we are working not just on the site of derived smooth manifolds, but, slightly more generally, of formal derived smooth manifolds.
Now we will directly exploit the formal aspects of our formal derived smooth sets.

The cohesive $(\infty,1)$-topos $\mathbf{dFSmoothStack}$ is naturally equipped with a differential cohesive structure, as defined in \cite[Section 4.2.1]{DCCTv2}. Such a differential cohesive structure includes a functor $\Im(-)$ which sends a formal derived smooth stack to its de Rham space, which can be thought as its infinitesimal path groupoid (see the reference for more details).

%A differential cohesive topos is a generalization of the concept of a cohesive topos that incorporates a natural notion of differentiation and integration into the framework. It provides a powerful tool for studying differential and geometric structures, and has applications in areas such as algebraic geometry, topology, and mathematical physics.

Like a cohesive topos, a differential cohesive topos is a category of sheaves over a site that satisfies certain axioms, which ensure that it has enough structure to capture the basic features of smooth and topological spaces. However, in addition to the structure of a cohesive topos, a differential cohesive topos includes a natural notion of differentiation and integration that allows us to define differential forms and cohomology on the sheaves.
The following definition appears in \cite[Section 4.2.1]{DCCTv2}.

\begin{definition}[Differential cohesive structure]
A \textit{differential cohesive structure} on a cohesive $(\infty,1)$-topos $\mathbf{H}$ is the datum of a sub-$(\infty,1)$-topos $\mathbf{H}^\mathrm{red}$ which is included via a quadruple of adjoint functors of the form
\begin{equation}\label{eq:diff_cohesive_strcuture}
...
Now, we can restrict everything to almost finitely presented algebras and obtain the following simplicial Quillen adjunction:
\begin{equation}\label{eq:coref_inf}
    \begin{tikzcd}[row sep=scriptsize, column sep=8.2ex, row sep=18.0ex]
     \mathsf{C^\infty Alg}_\mathrm{fg}^\mathrm{red} \arrow[r, "\iota^\mathrm{red}"', shift left=-0.8ex, hook] &  \mathsf{sC^\infty Alg}_\mathrm{afp}. \arrow[l, "(-)^\mathrm{red}"', shift left=-0.8ex]
    \end{tikzcd}
\end{equation}
Since it is a simplicial Quillen adjunction, it gives naturally rise to a reflective embedding of $(\infty,1)$-categories $\mathbf{N}(\mathsf{C^\infty Alg}_\mathrm{fg}^\mathrm{red})\;\begin{matrix}\leftarrow \\[-1.6ex] \hookrightarrow\end{matrix}\; \mathbf{sC^{\infty\!}Alg}_\mathrm{afp}$.
...
\begin{theorem}[Differential cohesive $(\infty,1)$-topos of formal derived smooth stacks]
The cohesive $(\infty,1)$-topos $\mathbf{dFSmoothStack}$ of formal derived smooth stacks is naturally equipped with a differential cohesive structure, i.e. with a quadruplet of adjoint $(\infty,1)$-functors
...
\begin{proof}
Recall that we have an equivalence $\mathsf{sC^\infty Alg}_\mathrm{afp}^\op \simeq \mathsf{dFMfd}$ of the opposite category of almost finitely presented $\Coo$-algebras to the category of formal derived smooth manifolds.
By left and right Kan extension, the reflective embedding \eqref{eq:coref_inf} of simplicial sites induces the quadruple of Quillen adjoint functors
\begin{equation}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={24ex}]
     {[\mathsf{sC^\infty Alg}_\mathrm{afp},\mathsf{sSet}]_\mathrm{proj}} \arrow[r, "\phantom{\,\iota^\mathrm{red}_!\,\,\simeq\,}(-)^\mathrm{red}_\ast", shift left=-4.5ex]\arrow[r, "\,\iota^\mathrm{red\ast\!} \,\simeq\,(-)^{\mathrm{red}}_!", shift right=-1.5ex] &  {[\mathsf{C^\infty Alg}^\mathrm{red}_\mathrm{fg},\mathsf{sSet}]_\mathrm{proj}}  \arrow[l, "\,\,\iota^\mathrm{red}_\ast\,\,\,\simeq\,(-)^{\mathrm{red}\ast\!}"', shift right=-1.5ex, hook]  \arrow[l, "\iota^\mathrm{red}_!\phantom{\,\,\simeq\,(-)^{\mathrm{red}}_!}"', shift left=-4.5ex, hook],
    \end{tikzcd}
\end{equation}
which encodes a quadruple of adjoint $(\infty,1)$-functors between the corresponding $(\infty,1)$-categories of pre-stacks.
Now we must now prove that these functors send, in particular, stacks to stacks.
By \cite[Section 4.8]{ToenVezzo05}, the functor $\iota^\mathrm{red}$ is continuous and cocontinuous, which implies that the adjunction $(\iota^{\mathrm{red}}_!\dashv\iota^{\mathrm{red}\ast}\dashv\iota^{\mathrm{red}}_\ast)$ restricts to an adjunction of stacks.
So, we have left to show that that $(-)^\mathrm{red}_\ast X\in[\mathsf{C^\infty Alg}^\mathrm{red}_\mathrm{fg},\mathsf{sSet}]_\mathrm{proj}$ satisfies descent for any formal derived smooth stack $X$. By the adjoint functor, it is sufficient to see that $\bfL\mathrm{colim}_n\iota^{\mathrm{red}}_\ast H(U)_n\rightarrow \iota^{\mathrm{red}}_\ast U$, but this is verified by noticing that $\iota^{\mathrm{red}}_\ast$ preserves colimits of representable objects and \'etale epimorphisms between them.
Thus we have constructed functors of $(\infty,1)$-categories
\begin{equation}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={24ex}]
     \mathbf{dFSmoothStack} \arrow[r, "\phantom{\,\iota^\mathrm{red}_!\,\,\simeq\,}(-)^\mathrm{red}_\ast", shift left=-4.5ex]\arrow[r, "\,\iota^\mathrm{red\ast\!} \,\simeq\,(-)^{\mathrm{red}}_!", shift right=-1.5ex] &  \mathbf{SmoothStack}^{\pmb{+}}  \arrow[l, "\,\,\iota^\mathrm{red}_\ast\,\,\,\simeq\,(-)^{\mathrm{red}\ast\!}"', shift right=-1.5ex, hook]  \arrow[l, "\iota^\mathrm{red}_!\phantom{\,\,\simeq\,(-)^{\mathrm{red}}_!}"', shift left=-4.5ex, hook],
    \end{tikzcd}
\end{equation}
where $\mathbf{SmoothStack}^{\pmb{+}}=\mathbf{N}_{hc}([\mathsf{C^\infty Alg}^\mathrm{red}_\mathrm{fg},\mathsf{sSet}]_\mathrm{proj,loc}^\circ)$ is by definition the $(\infty,1)$-topos of stacks on the ordinary \'etale site of reduced $\Coo$-varieties $\mathsf{C^\infty Var}_\mathrm{red}=(\mathsf{C^\infty Alg}_\mathrm{fg}^\mathrm{red})^\op$, which we constructed in definition \ref{def:extended_formal_smooth}. 
...
In the terminology of \cite[Definition 4.2.1]{DCCTv2}, the quadruplet of morphisms in diagram \eqref{eq:diffcohesiondiag} characterises the cohesive $(\infty,1)$-topos $\mathbf{dFSmoothStack}$ as an infinitesimal cohesive neighbourhood of the cohesive $(\infty,1)$-topos $\mathbf{SmoothStack}^{\pmb{+}}$.
Intuitively, this tells us that, in some sense, any object of $\mathbf{dFSmoothStack}$ can be thought of as an infinitesimal extension of some object in $\mathbf{SmoothStack}^{\pmb{+}}$. 
Such a structure is, indeed, differential cohesion on $\mathbf{dFSmoothStack}$.

\begin{remark}[Interpretation of reduced and co-reduced objects]
In analogy with non-derived differential cohesion, we could call the functor $\hat{\imath}$ inclusion of \textit{reduced objects} and $\mathrm{Disc}^{\mathrm{dif}}$ inclusion of \textit{co-reduced objects}.
\begin{itemize}
...
in $\mathbf{dC^{\infty\!\!}Aff}$, which corresponds to the homotopy $\Coo$-algebra $\iota^\mathrm{red}(R)\in\mathsf{sC^\infty Alg}$.
    ...
    \end{equation}
where $\Spec (R^\mathrm{red})\in\mathsf{C^\infty Aff}$ is the ordinary affine $\Coo$-scheme corresponding to the reduced $\Coo$-algebra $R^\mathrm{red}\in\mathbf{C^\infty Alg}^\mathrm{red}$.
\end{itemize}
\end{lemma}

\begin{proof}
For any almost finitely presented $\Coo$-algebra $A\in\mathsf{sC^\infty Alg}_\mathrm{afp}$, we have the equivalences
\begin{equation}
    \begin{aligned}
...
Just like any quadruple of adjoint $(\infty,1)$-functors, the derived differential cohesive structure presented by diagram \eqref{eq:diffcohesiondiag} gives naturally rise to a triplet of adjoint $(\infty,1)$-endofunctors.

\begin{definition}[Modalities of derived differential cohesion]\label{def:infinitesimalshape}
We define the following endofunctors:
\begin{equation}
   (\Re \;\dashv\;  \Im \;\dashv\; \&):\,\mathbf{dFSmoothStack}\,\longrightarrow\,\mathbf{dFSmoothStack},
\end{equation}
...
The modalities of derived differential cohesion will constitute our fundamental toolbox in dealing with the geometry of formal derived smooth stacks.
...
The adjoint $(\infty,1)$-functors $(\Re\dashv\Im)$ from derived differential cohesion as described above can be thought of as a smooth version of the adjunction constructed by \cite[Section 2]{Calaque:2017} in the context of derived algebraic geometry.
\end{remark}

\begin{remark}[Modalities of derived cohesion and derived differential cohesion]
\begin{equation*}
    \begin{tikzcd}[row sep={4ex,between origins}, column sep={4ex,between origins}]
    \Re & \dashv & \Im & \dashv & \& &&\\
    && \vee && \vee &&\\
    && {\textstyle\int} & \dashv & \flat & \dashv & \sharp
    \end{tikzcd}
\end{equation*}
where the relations ${\textstyle\int} < \Im$ and $\flat < \&$ respectively mean that we have equivalences ${\textstyle\int}\Im(X) \simeq {\textstyle\int}X$ and $\flat\&(X) \simeq \flat X$ for any formal derived smooth stack $X\in\mathbf{dFSmoothStack}$.
\end{remark}
...
In other words, the formal disk $\bbD_{X,x}$ is the fibre at the point $x:\ast\rightarrow\Im(X)$ of the bundle provided by the canonical morphism $\mathfrak{i}_X:X\longrightarrow\Im(X)$ from $X$ to its de Rham space.

\begin{remark}[Formal disk is infinitesimal]
Notice that the formal disk is an infinitesimal object, in fact we have the equivalence $\Re(\bbD_{X,x})\simeq\ast$.
\end{remark}
...
\begin{construction}[Artinian dg-algebras are almost finitely presented homotopy $\Coo$-algebras]$ $\vspace{-0.7cm}
...
\begin{itemize}
    ...
    \item By definition, the $0$th cohomology $\mathrm{H}^0(\mathcal{A})\cong \pi_0|\mathcal{A}|_\mathrm{DK}$ of a local Artinian dg-algebra $\mathcal{A}$ is an ordinary local Artinian algebra and thus it is canonically a finitely presented $\Coo$-algebra. Moreover, since $\mathcal{A}$ is degree-wise finite dimensional, the $\{\pi_i|\mathcal{A}|_\mathrm{DK}\}_{i>0}$ are finitely presented modules on $\pi_0|\mathcal{A}|_\mathrm{DK}$. Therefore, $|\mathcal{A}|_{\mathrm{DK}}^{\Coo}\in\mathsf{sC^\infty Alg}_\mathrm{afp}$ is canonically an almost finitely presented $\Coo$-algebra.
\end{itemize}
...
\begin{equation}
    ...
    & \mathsf{sC^\infty Alg}_\mathrm{afp} \arrow[d, "(-)^\mathrm{alg}"] \\
...
\begin{equation}
    |-|_{\mathrm{DK}}^{\Coo}\,:\, \mathsf{dgArt}_{\bbR}^{\leq 0} \,\longhookrightarrow\, \mathsf{sC^\infty Alg}_{\mathrm{afp}}.
\end{equation}
...
\end{aligned}
\end{equation}
where $|\mathcal{A}|_{\mathrm{DK}}^{\Coo}\in\mathsf{sC^\infty Alg}_\mathrm{afp}$ is the almost finitely presented simplicial $\Coo$-algebra corresponding to the Artinian dg-algebra $\mathcal{A}\in\mathsf{dgArt}_\bbR^{\leq 0}$.
...
Morally speaking, formal moduli problems in $\mathbf{FMP}$ can be thought of as infinitesimally thickened $\infty$-groupoids, in a derived sense.


\begin{lemma}[Derived relative cohesion]
Formal derived smooth stacks are equipped with a relative cohesive structure over the $(\infty,1)$-topos $\mathbf{FMP}$ of formal moduli problems, i.e. we have a quadruplet of adjoint $(\infty,1)$-functors
\begin{equation}\label{eq:rel_cohesion}
    \begin{tikzcd}[row sep={15ex,between origins}, column sep={19ex}]
     \mathbf{dFSmoothStack} &  \mathbf{FMP} \arrow[l, "\mathrm{Disc}^{\mathrm{rel}}"', hook', shift right=1.5ex] \arrow[l, "\mathit{\Gamma}^{\mathrm{rel}}"', shift right=-1.5ex, leftarrow] \arrow[l, "\mathit{\Pi}^{\mathrm{rel}}"', shift right=+4.5ex, leftarrow] \arrow[l, "\mathrm{coDisc}^{\mathrm{rel}}"', hook', shift left=4.5ex]
    \end{tikzcd}
\end{equation}
such that:
\begin{itemize}
    \item $\mathit{\Pi}^{\mathrm{rel}}$ preserves finite products,
    \item $\mathrm{Disc}^{\mathrm{rel}}$ and $\mathrm{coDisc}^{\mathrm{rel}}$ are fully faithful,
    \item $\mathit{\Gamma}^{\mathrm{rel}} = (-)^\wedge$ is precisely the functor in equation \eqref{eq:stack_to_fmp}.
\end{itemize}
\end{lemma}
...
\begin{equation}
    \begin{tikzcd}[row sep=scriptsize, column sep=12.0ex, row sep=18.0ex]
     \mathsf{sC^\infty Alg}_\mathrm{afp}   &  \arrow[l, "|-|_{\mathrm{DK}}^{\Coo}"', shift left=-0ex, hook'] \mathsf{dgArt}_{\bbR}^{\leq 0} .
    \end{tikzcd}
\end{equation}
...
\begin{equation}
    \begin{tikzcd}[row sep={15ex,between origins}, column sep={19ex}]
    {[ \mathsf{sC^\infty Alg}_\mathrm{afp}, \sSet]_\mathrm{proj}} \arrow[r, "\,\,(|-|_{\mathrm{DK}}^{\Coo})_\ast", shift left=-4.5ex, leftarrow]\arrow[r, "{(|-|_{\mathrm{DK}}^{\Coo})_!}", shift right=-1.5ex, leftarrow] &  {[\mathsf{dgArt}^{\leq 0}_\bbR, \sSet]_\mathrm{proj}}  \arrow[l, "\,(|-|_{\mathrm{DK}}^{\Coo})^{\ast}"', shift right=-1.5ex, hook, leftarrow]  .
    \end{tikzcd}
\end{equation}
The functor $\mathit{\Gamma}^\mathrm{rel}\coloneqq(|-|_{\mathrm{DK}}^{\Coo})^{\ast}$ immediately sends local fibrant objects to fibrant objects, since the simplicial category $[\mathsf{dgArt}^{\leq 0}_\bbR, \sSet]_\mathrm{proj}$ is equipped with a global and not a local model structure. By \cite{ToenVezzo05}, this also implies that its left adjoint $\mathrm{Disc}^{\mathrm{rel}}\coloneqq(|-|_{\mathrm{DK}}^{\Coo})_!$ restricts to stacks.
We now must show that $\mathrm{coDisc}^\mathrm{rel}\coloneqq(|-|_{\mathrm{DK}}^{\Coo})_{\ast}$ sends fibrant objects to local fibrant in the local model structure $[ \mathsf{sC^\infty Alg}_\mathrm{afp}, \sSet]_\mathrm{proj,loc}$.
Notice that, by adjunction, for any formal moduli problem $F\in[\mathsf{dgArt}^{\leq 0}_\bbR, \sSet]_\mathrm{proj}$, we have sections of the form $(\mathrm{coDisc}^\mathrm{rel}F)(U) \simeq \bfR\Hom_{\mathsf{FMP}}(\mathit{\Gamma}^\mathrm{rel}(U),F)$ on any formal derived smooth manifold $U\in\mathbf{dFMfd}$. For $\mathrm{coDisc}^\mathrm{rel}F$ to be a formal derived smooth stack, it must satisfy the descent condition, i.e. one must have an equivalence
$(\mathrm{coDisc}^\mathrm{rel}F)(U) \simeq \bfR\!\lim_n\bfR\Hom_{\mathsf{FMP}}(\mathit{\Gamma}^\mathrm{rel}H(U)_n,F)$ for any \'etale hypercover $H(U)_\bullet\rightarrow U$. This is verified by noticing that we have an equivalence $\bfL\mathrm{colim}_n\mathit{\Gamma}^\mathrm{rel}H(U)_n \simeq \mathit{\Gamma}^\mathrm{rel}U$,
since, for any formal derived smooth stack, we have $\mathit{\Gamma}^\mathrm{rel}X\simeq \coprod_{x:\ast\rightarrow X}X^\wedge_x$, where the $X^\wedge_x$ are pointed formal moduli problems.
Thus, we showed that $(\mathrm{Disc}^\mathrm{rel}\dashv\mathit{\Gamma}^\mathrm{rel})$ and $(\mathit{\Gamma}^\mathrm{rel}\dashv\mathrm{coDisc}^\mathrm{rel})$ are simplicial Quillen adjunctions with respect to the local model structure. 
We now define a further functor
\begin{equation}
    \mathit{\Pi}^\mathrm{rel}X \;\coloneqq\; \mathrm{Disc}^\mathrm{inf}(\mathit{\Pi}X) \sqcup^h_{\mathit{\Gamma}^\mathrm{rel}(\Re X)}\mathit{\Gamma}^\mathrm{rel}(X)
\end{equation}
It is possible to see, by using the limit preserving property of the Hom-space, that the right adjoint of $\mathit{\Pi}^\mathrm{rel}$ is the functor $F\mapsto\mathrm{Disc}(\mathit{\Gamma}^\mathrm{inf}F) \times^h_{ \Im (\mathrm{coDisc}^\mathrm{rel}F)\!} \mathrm{coDisc}^\mathrm{rel}F$ on formal moduli problems $F\in\mathbf{FMP}$. 
However, we also have the following equivalences:
\begin{equation}
\begin{aligned}
    \mathrm{Disc}(\mathit{\Gamma}^\mathrm{inf}F) \times^h_{ \Im (\mathrm{coDisc}^\mathrm{rel}F)} \mathrm{coDisc}^\mathrm{rel}F \;\,&\simeq\; \Big(\coprod_{f:\bbR \rightarrow F}\ast\,\Big)\times^h_{ \Im (\mathrm{coDisc}^\mathrm{rel}F)} \mathrm{coDisc}^\mathrm{rel}F \\
    \;&\simeq\; \coprod_{f:\bbR \rightarrow F}\Big(\ast\times^h_{ \Im (\mathrm{coDisc}^\mathrm{rel}F)} \mathrm{coDisc}^\mathrm{rel}F\Big) \\
        \;&\simeq\; \coprod_{f:\bbR \rightarrow F}\bbD_{\mathrm{coDisc}^\mathrm{rel}F,f} \\
        \;&\simeq\; \coprod_{f:\bbR \rightarrow F}\bbD_{\mathrm{Disc}^\mathrm{rel}F,f} \;\simeq\; \mathrm{Disc}^\mathrm{rel}F,
\end{aligned}
\end{equation}
where we used the equivalences $\mathit{\Gamma}(\mathrm{Disc}^\mathrm{rel}F)\simeq \mathit{\Gamma}(\mathrm{coDisc}^\mathrm{rel}F) \simeq \mathit{\Gamma}^\mathrm{inf}F\simeq F(\bbR)$ of simplicial sets and the equivalence $\bbD_{\mathrm{coDisc}^\mathrm{rel}F,f}\simeq \bbD_{\mathrm{Disc}^\mathrm{rel}F,f}$ at any point $f\in F(\bbR)$. This shows that there is an adjunction $(\mathit{\Pi}^\mathrm{rel}\dashv\mathrm{Disc}^\mathrm{rel})$.
Since $\mathit{\Pi}$ preserves finite products, $\mathit{\Pi}^\mathrm{rel}$ does too.
Moreover, just like in the previous cases, the fact that $|-|_{\mathrm{DK}}^{\Coo}$ is fully faithful, implies that the functors $\mathrm{Disc}^{\mathrm{rel}}$ and $\mathrm{coDisc}^{\mathrm{rel}}$ are fully faithful.
\end{proof}
...
By following \cite[Section 5.3.6]{DCCTv2}, we can also define the following modalities, which will provide a thickened version of the ones provided by cohesion over bare $\infty$-groupoids $({\textstyle\int} \dashv  \flat \dashv \sharp)$.

\begin{definition}[Modalities of derived relative cohesion]
We define the following endofunctors:
\begin{equation}
   ({\textstyle\int}^{\mathrm{rel}} \;\dashv\;  \flat^{\mathrm{rel}} \;\dashv\; \sharp^{\mathrm{rel}}):\,\mathbf{dFSmoothStack}\,\longrightarrow\,\mathbf{dFSmoothStack},
\end{equation}
where we respectively called
\begin{enumerate}[label=(\textit{\roman*})]
    \item \textit{relative shape modality} ${\textstyle\int}^{\mathrm{rel}} \coloneqq \mathrm{Disc}^{\mathrm{rel}}\circ \mathit{\Pi}^{\mathrm{rel}}$,
    \item \textit{relative flat modality} $\flat^{\mathrm{rel}} \coloneqq \mathrm{Disc}^{\mathrm{rel}}\circ \mathit{\Gamma}^{\mathrm{rel}}$,
    \item \textit{relative sharp modality} $\sharp^{\mathrm{rel}} \coloneqq \mathrm{coDisc}^{\mathrm{rel}}\circ \mathit{\Gamma}^{\mathrm{rel}}$.
\end{enumerate}
\end{definition}

\begin{lemma}[Relative shape and flat modalities]
For any given formal derived smooth stack $X\in\mathbf{dFSmoothStack}$, we have the following equivalences:
\begin{equation}
     {\textstyle\int}^\mathrm{rel}X \;\simeq\; {\textstyle\int} X \sqcup_{\Re(X)}^h X, \qquad   \flat^\mathrm{rel}X \;\simeq\; \flat X \times_{\Im(X)}^h X.
\end{equation}
\end{lemma}

\begin{proof}
By unravelling the definition $\flat^\mathrm{rel} = \mathrm{Disc}^{\mathrm{rel}} \circ \mathit{\Gamma}^{\mathrm{rel}}$ of the relative flat modality, we see that a formal derived smooth stack $\flat^\mathrm{rel}X$ can be understood as the coproduct of formal disks $\bbD_{X,x}$ at all points $x:\ast \rightarrow X$. More precisely, one has
\begin{equation}
    \flat^\mathrm{rel}X \;\simeq\, \coprod_{x:\ast\rightarrow X}\!\bbD_{X,x}. \vspace{-0.2cm}
\end{equation}
But this is precisely $\flat^\mathrm{rel}X \;\simeq\; \flat X \times_{\Im(X)}^h X$, since one has that $\flat X \simeq \coprod_{x:\ast\rightarrow X}\ast$ and, as we have seen many times, that the infinitesimal disk is given by $\bbD_{X,x}\simeq\ast\times\times_{\Im(X)}^h X$.

\begin{equation}
\begin{aligned}
    \bfR\Hom( X,\flat^\mathrm{rel}Y) \;&\simeq\; \bfR\Hom(X,\flat Y) \times^h_{\bfR\Hom( X, \Im Y)} \bfR\Hom( X, Y) \\
    \;&\simeq\; \bfR\Hom({\textstyle\int}X, Y) \times^h_{\bfR\Hom( \Re X, Y)} \bfR\Hom( X, Y) \\
    \;&\simeq\; \bfR\Hom({\textstyle\int}X\sqcup^h_{\Re X\!} X, Y),
\end{aligned}
\end{equation}

By unravelling the definition ${\textstyle\int}^{\mathrm{rel}}X = \mathrm{Disc}^{\mathrm{rel}}\mathit{\Pi}^\mathrm{rel}X$ of the relative shape modality, one obtains the equivalence
${\textstyle\int}^{\mathrm{rel}}X \simeq {\textstyle\int}X\sqcup_{\flat^\mathrm{rel}\Re (X)}\flat^\mathrm{rel}X$.
However, notice that we can form the diagram
\begin{equation}\label{diag:inf_cohesion_quadruplet}
    \begin{tikzcd}[row sep={15ex,between origins}, column sep={16.5ex,between origins}]
    \flat^\mathrm{rel}\Re (X) \arrow[r, "\varepsilon^{ \flat^\mathrm{rel}}_{\Re(X)}"] \arrow[d, "\flat^\mathrm{rel}(\mathfrak{r}_X)"] & \Re(X) \arrow[r]\arrow[d, "\mathfrak{r}_X"] & {\textstyle\int}X\arrow[d] \\
    \flat^\mathrm{rel}X \arrow[r, "\varepsilon^{ \flat^\mathrm{rel}}_{X}"] & X \arrow[r] & {\textstyle\int}^\mathrm{rel}X .
    \end{tikzcd}
\end{equation}
Moreover, we have the following chain of equivalences:
\begin{equation}
\begin{aligned}
    \Re(X)\sqcup_{\flat^\mathrm{rel}\Re (X)}^h \flat^\mathrm{rel}X \;&\simeq\; \Re(X)\sqcup_{\flat\Re(X) \times_{\Im\Re(X)}^h \Re(X)}^h (\flat X \times_{\Im(X)}^h X) \\
    \;&\simeq\; \Re(X) \sqcup_{\flat X \times_{\Im(X)}^h \Re(X)}^h (\flat X\times_{\Im(X)}^h X) \\
    \;&\simeq\; \Re(X) \sqcup_{\Re(X)}^h X \;\simeq\; X,
\end{aligned}
\end{equation}
where we used the fact that we have equivalences $\flat\Re(X)\simeq\flat X$ and $\Im\Re(X) \simeq \Im(X)$.
Since the outer square is already a pushout, then we have an equivalence ${\textstyle\int}^\mathrm{rel}X \simeq {\textstyle\int} X \sqcup_{\Re(X)}^h X$.
\end{proof}




\begin{remark}[All the structures in the context of derived differential cohesion]
...
\vspace{-0.2cm}
\begin{equation}
    \begin{tikzcd}[row sep={28ex,between origins}, column sep={20ex}]
     \mathbf{dFSmoothStack}\;\;\;   
        &  \;\;\;\mathbf{FMP}     \arrow[l, "\mathrm{Disc}^{\mathrm{rel}}"', hook', shift right=1.5ex] \arrow[l, "\mathit{\Gamma}^{\mathrm{rel}}"', shift right=-1.5ex, leftarrow] \arrow[l, "\mathit{\Pi}^{\mathrm{rel}}"', shift right=+4.5ex, leftarrow] \arrow[l, "\mathrm{coDisc}^{\mathrm{rel}}"', hook', shift left=4.5ex]                        
      \arrow[d, "", shift right=3.5ex,  shorten=3.5mm,sloped] \arrow[d, "", shift right=-2.5ex,  shorten=3.5mm,sloped]   \\
    \mathbf{SmoothStack}^{\pmb{+}}\;\;\; \arrow[u, "{\mathit{\Pi}^{\mathrm{dif}}}",sloped , shift right=-1.5ex, leftarrow, shorten=3.5mm] \arrow[u, "{\mathit{\Gamma}^{\mathrm{dif}}}",sloped , shift right=4.5ex, leftarrow, shorten=3.5mm]\arrow[u, "\hat{\imath}", shift right=-4.5ex,  shorten=3.5mm, hook, sloped] \arrow[u, "\mathrm{Disc}^{\mathrm{dif}}", shift right=1.5ex,  shorten=3.5mm, hook, sloped] &  \;\;\;\mathbf{\infty Grpd},      \arrow[l, "\mathrm{Disc}"', hook', shift right=1.5ex] \arrow[l, "\mathit{\Gamma}"', shift right=-1.5ex, leftarrow] \arrow[l, "\mathit{\Pi}"', shift right=+4.5ex, leftarrow] \arrow[l, "\mathrm{coDisc}"', hook', shift left=4.5ex]                           \arrow[u, "", shift right=5.5ex,  shorten=3.5mm, hook, "\mathrm{coDisc}^\mathrm{inf}",sloped] \arrow[u, "\mathrm{Disc}^\mathrm{inf}",sloped , shift right=-0.5ex,  shorten=3.5mm, hook] \arrow[u, "{\mathit{\Pi}^\mathrm{inf}}",sloped , shift right=-3.5ex, dash, shorten=3.5mm] \arrow[u, "\mathit{\Gamma}^\mathrm{inf}",sloped , shift right=2.5ex, dash, shorten=3.5mm]
...
\begin{itemize}
    \item the left vertical quadruplet $(\hat{\imath}\dashv\mathit{\Pi}^{\mathrm{dif}}\dashv\mathrm{Disc}^{\mathrm{dif}}\dashv\mathit{\Gamma}^{\mathrm{dif}})$ of $(\infty,1)$-functors presents a differential cohesive structure on the $(\infty,1)$-category $\mathbf{dFSmoothStack}$ of formal derived smooth stacks over the $(\infty,1)$-category $\mathbf{SmoothStack}^{\pmb{+}}$ from diagram \eqref{eq:diffcohesiondiag};
    \item the right vertical quadruplet $(\mathit{\Pi}^{\mathrm{inf}}\dashv\mathrm{Disc}^{\mathrm{inf}}\dashv\mathit{\Gamma}^{\mathrm{inf}}\dashv\mathrm{coDisc}^{\mathrm{inf}})$ of $(\infty,1)$-functors presents an infinitesimal cohesive structure on the $(\infty,1)$-category $\mathbf{FMP}$ of formal moduli problems over the $(\infty,1)$-category $\mathbf{\infty Grpd}$ of $\infty$-groupoids from diagram \eqref{eq:inf_cohesion};
    \item the upper horizontal quadruplet $(\mathit{\Pi}^{\mathrm{rel}}\dashv\mathrm{Disc}^{\mathrm{rel}}\dashv\mathit{\Gamma}^{\mathrm{rel}}\dashv\mathrm{coDisc}^{\mathrm{rel}})$ of $(\infty,1)$-functors presents a relative cohesive structure on the $(\infty,1)$-category $\mathbf{dFSmoothStack}$ of formal derived smooth stacks over the $(\infty,1)$-category $\mathbf{FMP}$ of formal moduli problems from diagram \eqref{eq:rel_cohesion};
    \item the lower horizontal quadruplet $(\mathit{\Pi}\dashv\mathrm{Disc}\dashv\mathit{\Gamma}\dashv\mathrm{coDisc})$ of $(\infty,1)$-functors presents a cohesive structure on the $(\infty,1)$-category $\mathbf{SmoothStack}^{\pmb{+}}$ over the $(\infty,1)$-category $\mathbf{\infty Grpd}$ of $\infty$-groupoids.
\end{itemize}

\end{remark}

Finally, the following proposition is the step-by-step generalisation of \cite[Proposition 6.5.15]{DCCTv2} to derived smooth geometry.

\begin{proposition}[Derived relative cohesion as a pushout]
The diagram
\begin{equation}
    \begin{tikzcd}[row sep={21ex,between origins}, column sep={24ex,between origins}]
    \mathbf{dFSmoothStack} \arrow[r, "\mathit{\Gamma}^\mathrm{rel}"] &  \mathbf{FMP} \\
    \mathbf{SmoothStack}^{\pmb{+}}\arrow[u, hook, "\mathrm{Disc}^{\mathrm{dif}}"] \arrow[r, "\mathit{\Gamma}"] &  \mathbf{\infty Grpd}\arrow[u, hook, "\mathrm{Disc}^{\mathrm{inf}}"'] .
    \end{tikzcd}
\end{equation}
is an $(\infty,1)$-pushout square of $(\infty,1)$-topoi.
\end{proposition}
...
\begin{equation}
    \begin{tikzcd}[row sep={21ex,between origins}, column sep={24ex,between origins}]
    \mathbf{dFSmoothStack} \arrow[r, leftarrow, "\mathrm{Disc}^{\mathrm{rel}}"] &  \mathbf{FMP} \\
    \mathbf{SmoothStack}^{\pmb{+}}\arrow[u, leftarrow, "\mathit{\Pi}^\mathrm{dif}"] \arrow[r, leftarrow, "\mathrm{Disc}"] &  \mathbf{\infty Grpd}\arrow[u, leftarrow, "\mathit{\Pi}^\mathrm{inf}"'] 
    \end{tikzcd}
\end{equation}
is an $(\infty,1)$-pullback of $(\infty,1)$-categories.
Since any stack can be written as the $(\infty,1)$-colimit of representables and the left adjoint preserves colimits, it is enough to show that the following diagram of sites is a pullback square:
\begin{equation}\label{eq:square_sites}
    \begin{tikzcd}[row sep={18ex,between origins}, column sep={20ex,between origins}]
    \mathbf{dFMfd} \arrow[r, leftarrow, "{(|-|_{\mathrm{DK}}^{\Coo})^\op}"] &  \mathbf{dgArt}^{\leq 0}_\bbR \\
    \mathbf{N}(\mathsf{C^\infty Var}^{\mathrm{red}})\arrow[u, leftarrow, "((-)^\mathrm{red})^\op"] \arrow[r, leftarrow, "\ast"] &  \ast\arrow[u, leftarrow, "\ast"'] ,
    \end{tikzcd}
\end{equation}
where the functor $(|-|_{\mathrm{DK}}^{\Coo})^\op$ embeds any dg-Artinian algebra $\mathcal{A}$ into a formal derived smooth manifold $\bfR\Spec|\mathcal{A}|_{\mathrm{DK}}^{\Coo}$ and $((-)^\mathrm{red})^\op$ reduces any formal derived smooth manifold to a reduced $\Coo$-variety. Commutativity of the square \eqref{eq:square_sites} is assured by the fact that there is an equivalence $\big(\bfR\Spec|\mathcal{A}|_{\mathrm{DK}}^{\Coo}\big)^\mathrm{red}\simeq\, \ast$, which is a consequence of $\mathrm{H}^0(\mathcal{A})/\mathfrak{m}_{\mathrm{H}^{0\!}(\mathcal{A})}\cong \bbR$.
However, the converse is also true: given a formal derived smooth manifold $U\in\mathbf{dFMfd}$, the equivalence
$(U)^\mathrm{red} \simeq \ast$ implies that $U\simeq \bfR\Spec|\mathcal{A}|_{\mathrm{DK}}^{\Coo}$ for some dg-Artinian algebra $\mathcal{A}\in\mathbf{dgArt}_\bbR^{\leq 0}$.
This shows that the square \eqref{eq:square_sites} is a $(\infty,1)$-pullback and, therefore, the proposition.
\end{proof}
...
Now, in the formalism of derived differential cohesion we are able to generalise an idea from \cite[Section 6.5.2.2]{DCCTv2} to derived geometry and, thus, provide a very general definition of what we may call derived smooth algebroid. 
...
In this subsection we will provide a definition of jet bundles as formal derived smooth stacks, rooted in differential cohesive geometry, which we delineated in the this section. This will be an application of the framework developed by \cite{khavkine2017synthetic, DCCTv2}.
...
A paper in preparation \cite{AlfYouFuture} will be devolved to the exploitation of the features of derived jet bundles in the context of derived differential cohesion.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Global aspects of classical BV-theory}\label{sec:global_aspects_of_bv_theory}

In this section we finally get our hands dirty: we will use the new toolbox provided by derived differential cohesion to investigate some global-geometric features of classical field theory. 
...
The rest of this section will devoted to the construction of a global version of this formalism in the context of derived differential cohesion.
...
Now, having introduced the smooth stack $\Bun_G(M)$ and its refinement $\Bun_G^\nabla(M)$, we will focus on their infinitesimal properties in the context of differential cohesion.
...
In this subsection, we will proceed with the construction of the derived critical locus $\bfR\crit{S}(M)\rightarrow \Bun_G^\nabla(M)$ of the Yang-Mills action $S$ as a formal derived smooth stack in the context of derived differential cohesion. Finally, we will show that such a geometric object provides a global-geometric version of usual BV-BRST theory.
...
The authors hope that the derived differential cohesive geometry exhibited in the present paper may prove a useful language for addressing various open problems in QFT. In this final section we will point to some of them.
...
Derived differential cohesion provides a formalism which may allow a theoretical interpolation of these approaches and, thus, a global-geometric BV-BRST treatment of double copy. \vspace{-0.3cm}
