...
\emailAdd{r2194@damtp.cam.ac.uk}
...
%It is quite crucial that this construction wouldn't do anything interesting if we assumed that the algebra of the bulk fields was a type $I$ factor --- naively, there is something `purely infinite' in this approach.
...
Second, there is a shared notion of locality between $\mathcal{a}_{\mathrm{r}}$ and $\mathcal{a}_{\mathrm{r}}^{(\delta)}$, such that operators in $\mathcal{a}_{\mathrm{r}}$ smeared over a length scale $\gg \delta$ are well-approximated by operators in $\mathcal{a}_{\mathrm{r}}^{(\delta)}$.\footnote{Local algebras in QFT are believed to all be hyper-finite, which means that they can be approximated by type $I$ algebras in this way. However, this abstract statement is not good enough for my purposes: there is only one type $III_{1}$ algebra up to isomorphism and so there is a `unique' way to approximate it. But this doesn't tell us anything about shared locality properties.}
...
It should be possible to rephrase this notion of an optimal approximation more precisely in terms of conditional expectations.
...
The two most interesting outcomes of the main analysis are:
\begin{enumerate}
...
There are some interesting directions.
\begin{enumerate}
...
I thank Matthew Heydeman, David K. Kolchmeyer, Shiraz Minwalla, Onkar Parrikar, Gabriel Wong and the audience of a talk at Tata Institute of Fundamental Research (TIFR) for discussions.
I thank Chris Akers and Annie Wei for collaboration on a related project \cite{Akers:2023} that led to this work and comments on a very early version; and also David Tong for asking about the possibility of approximating the crossed product.
I also thank Chris Akers for comments on a manuscript.
