...
An excellent problem book on atomic physics \cite{Budker_2008} is aimed  at the development of physical intuition, as opposed to formal mathematics. The downside of this approach is that sometimes the presentation is too informal for inexperienced students to fill in all the necessary details needed for real understanding. In \cite{Budker_2008} the Berry phase is introduced through considering spin-one system in a rotating electric field. According to our observations, students find it quite challenging to understand the solution of this problem as it is given in the book. Therefore, in this note we decided to provide a more detailed solution. Throughout the article, Einstein's summation convention and atomic units $\hbar=1$ are assumed (if not otherwise stated).

\section{Energy levels in a stationary electric field}
\label{ch2}
Let us consider a system with spin one in a constant electric field $\vec{\cal E}$ pointing in the $z$ direction. In the absence of the field, the energy levels of the states $|1,1\rangle$, $|1,0\rangle$ and $|1,-1\rangle$ are degenerate due to the assumed rotational symmetry. The interaction Hamiltonian has the form $\hat{H}_{int}=-\hat{\vec{D}}\cdot\vec{\cal E}=-\hat{D}_z{\cal E}$, where  $\hat{\vec{D}}$ is the electric dipole moment operator. In atomic electric dipole transitions induced by $\hat{D}_z$, the selection rules are $\Delta m=0$, $\Delta l=\pm 1$ \cite{griffiths_2018}. Therefore, we assume that $\hat{H}_{int}$ cannot mix states $|1,m\rangle$, $m=1,0,-1$, among themselves.  In addition, to mimic the $\Delta l=\pm 1$ selection rule when calculating the effect of electric field, we need to extend the system to include the spin-zero state $|0,0\rangle$ in addition to the spin-one states \cite{Vutha_2009}. Since our goal is simply to illustrate the Berry phase, we will consider this simple four-state system and not a real atom with total angular momentum $F=1$. However, we assume that $\hat{H}_{int}$ couples only $|1,0\rangle$ and $|0,0\rangle$ states.

Now we diagonalize the total Hamiltonian $\hat H=\hat H_0+\hat H_{int}$. It is already diagonal in the subspace spanned by the states $|1,1\rangle$ and $|1,-1\rangle$. If $\langle 1,0|\hat{D}_z|0,0\rangle=\langle 0,0|\hat{D}_z|1,0\rangle=d$, where $d$ is some real number, then in the subspace of the $|1,0\rangle$ and $|0,0\rangle$ states $\hat H$ is given by the following matrix
\begin{equation}
...
If $d{\cal E}\ll E_1$, then the mixing angle $\alpha$ is small (and negative). In this case, $|0\rangle$ is mainly the state $|1,0\rangle$, and $|00\rangle$ is mainly the state $|0,0\rangle$.
...
Let $\vec{e}_i$ be unit vectors along coordinate axes in ${\mathbb R}^3$. Then, under an active rotation $\hat{R}_n^{(a)}(\varphi)$ around some axis $\vec{e}_n$, the vector $\vec{r}_1=x_{1i}\vec{e}_i$ changes to the vector $\vec{r}_2=x_{1i}\hat{R}_n^{(a)}(\varphi)\vec{e}_i$. The matrix $R=(R_{ij})$ associated with the rotation $\hat{R}_n^{(a)}(\varphi)$ can be introduced by expanding $\hat{R}_n^{(a)}( \varphi)\vec{e}_i=R_{ji}\vec{e}_j$. Then $x_{2i}=R_{ij}x_{1j}$.
...
It is clear that $\hat{R}_3^{(a)}(\varphi)\vec{e}_1=\cos{\varphi}\,\vec{e}_1+\sin{\varphi}\,\vec{e}_2$ and $\hat{R}_3^{(a)}(\varphi)\vec{e}_2=-\sin{\varphi}\,\vec{e}_1+\cos{\varphi}\,\vec{e}_2$. Therefore, under passive rotation $\hat{R}_3^{(p)}(\varphi)$ we will have
\begin{equation}
...
\begin{equation}
\vec{\cal E}={\cal E}\left (\sin{\theta}\cos{\varphi(t)}\vec{e}_1+\sin{\theta}\sin{\varphi(t)}\vec{e}_2+
\cos{\theta}\vec{e}_3\right ).
\label{eq7}
\end{equation}
If we rotate the coordinate system first by the angle $\varphi(t)$ around the $z$ axis, and then by the angle $\theta$ around the new $y$ axis, the electric field ${\cal E } $ will be along the third axis and will not rotate in the new coordinate system. However, the rotation operator $\hat R^{(a)}=\hat{R}^{(a)}_{2^\prime}(\theta)\hat{R}^{(a)}_3(\varphi)$ mixes the active and passive viewpoints since $\hat{R}^{(a)}_{2^\prime}(\theta)$ is the rotation around rotated axis $y^\prime$. It is useful to rewrite $\hat R$ solely in terms of rotations around fixed axes. This is possible thanks to the 
...
\label{eq8}
\end{equation}
Explicit form of the corresponding matrix, considering $\hat R$ as a passive rotation, can be obtained by using (\ref{eq6}) and remembering that the matrix which corresponds to $\hat{R}_n^{(p)}(\varphi)$ is the transpose of the matrix which corresponds to $\hat{R}_n^{(a)}(\varphi)$:
\begin{equation}
\hspace*{-15mm}
...
\label{eq9}
\end{equation}
Generators of rotations are defined in the usual way \cite{Morrison_1987}: $\hat{R}^{(a)}_3(\varphi)=e^{-i\varphi\hat{J}_3}$, $\hat{R}^{(a)}_2(\varphi)=e^{-i\varphi\hat{J}_2}$. If $\vec{r}_1=x_i\vec{e}_i$, then for $\Delta \varphi\ll 1$, $\vec{r}_2=\hat{R}^{(a)}_3(\Delta \varphi)\vec{r}_1\approx x_1(\vec{e}_1+\Delta \varphi \vec{e}_2)+x_2(-\Delta \varphi \vec{e}_1+\vec{e}_2)+x_3\vec{e}_3$. Comparing with $e^{-i\Delta \varphi\hat{J}_3}\vec{r}_1\approx \vec{r}_1-i\Delta\varphi\left (x_1\hat{J}_3\vec{e}_1+x_2\hat{J}_3\vec{e}_2+x_3\hat{J}_3\vec{e}_3\right )$, we get
\begin{equation}
...
In a given reference frame, we use $|1,1\rangle$,  $|1,0\rangle$, $|1,-1\rangle$ and  $|0,0\rangle$ as basic states for our quantum system. In the rotating frame, where the electric field stays still and aligned with the z axis, according to Section \ref{ch2}, eigenstates of the Hamiltonian have the following components
\begin{equation}
\hspace*{-10mm}
...
Since $\psi_n={\cal R}^+\phi_n$ is an eigenstate of the Hamiltonian, naively one can think that its development in time is described by the usual phase factor $e^{-iE_nt}$.  However, what hinders this is the dependence of the rotation matrix ${\cal R}^+$ on time. As a result, the time-dependent eigenstate $\Psi_n(t)$ contains an extra phase factor $c_n(t)$;
\begin{equation}
\Psi_n(t)=c_n(t)e^{-iE_nt}{\cal R}^+(t)\phi_n.
\label{eq24}
\end{equation}
This additional phase factor is determined from the requirement that (\ref{eq24}) satisfies the Schr\"{o}dinger equation 
\begin{equation}
...
\begin{equation}
\frac{dc_n}{dt}{\cal R}^+\phi_n+c_n\frac{d{\cal R}^+}{dt}\phi_n=0,
\label{eq26}
\end{equation}
which, after multiplying from the left by $\phi_n^+{\cal R}$, takes the form
\begin{equation}
\frac{dc_n}{dt}=-\phi_n^+{\cal R}\frac{d{\cal R}^+}{dt}\phi_n\,c_n.
\label{eq27}
\end{equation}
Since $\psi_n={\cal R}^+(t)\phi_n$ and $\phi_n$ is independent of time, (\ref{eq27}) can also be rewritten in this way
\begin{equation}
...
\label{eq29}
\end{equation}
Combined with (\ref{eq28}), these results imply (we assume that multiplying by $00$ is the same as multiplying by zero)
\begin{equation}
...
\label{eq41}
\end{equation}
However, the Berry phase is invariant under the gauge transformation (\ref {eq41}): as can be easily seen from (\ref{eq28})
\begin{equation}
