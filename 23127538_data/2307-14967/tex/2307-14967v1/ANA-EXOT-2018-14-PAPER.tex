...
is distinct from that of the semi-classical black holes~\cite{Anchordoqui:2001cg}
that decay into a multi-particle final state via Hawking
...
has color so the beam remnant has the corresponding anti-color.
 
A previous search for QBHs in the lepton+jet channel was performed in proton-proton ($pp$)
collisions at a center-of-mass energy of $\sqrt{s}=8~\TeV$ by ATLAS~\cite{EXOT-2013-03}.
The combined 95\% confidence level upper limit on the QBHs production cross-section with threshold mass
...
8~\TeV~\cite{EXOT-2012-22,EXOT-2012-23,EXOT-2013-11,CMS-EXO-12-059,EXOT-2013-13},
and 13~\TeV~\cite{EXOT-2015-02,EXOT-2015-20,CMS-EXO-15-009,CMS-EXO-16-058,EXOT-2019-03}.
...
The results described in this paper use $pp$ collision
...
and multi-jet processes including non-prompt leptons from
...
The multi-jet background is measured directly in data. In this case the events collected
...
is employed for the various jet multiplicities,
...
partons at NLO and up to four (three) additional partons at LO. The $W/Z$+jets
...
\tchan~\cite{Frederix:2012dh} is modeled with \POWHEGBOX
...
correlations in \tchan of the single top background. The \EVTGEN 1.6.0~\cite{Lange:2001uf}
...
Process & ME Generator and ME PDFs & PS, PDFs, non-perturbative effect \\\hline
...
Diboson, semi-leptonic decay & \SHERPA 2.2.1, \nnpdfnlo &\SHERPA 2.2.1, \nnpdfnlo \\
...
The parton showering and hadronization is performed in \PYTHIA 8.205, using
...
The QBH simulation assumes massless partons interaction and conserves total angular momentum.
...
non-perturbative gravity model. The cross-sections predicted by the \textsc{QBH} 3.0
...
at least two orders of magnitude higher cross-sections than those with antiquark pairs
in the initial state.
 
All simulated event samples include the effect of multiple $pp$ interactions in the same
or neighboring bunch crossings. These effects are collectively referred to as pile-up.
The simulation of pile-up collisions is performed with \PYTHIA 8.186 using the ATLAS A3 set
of tuned parameters~\cite{ATL-PHYS-PUB-2016-017} and the \nnpdftwo PDF set
and weighted to reproduce the average number of pile-up interactions per bunch crossing observed in data.
...
For an event to be considered, at least one $pp$ interaction
...
Two identification levels are defined for leptons and jets, referred to as ``{\scshape Baseline}''
and ``{\scshape Signal},'' with {\scshape Signal} objects being a subset of {\scshape Baseline}.
The {\scshape Baseline} requirement provides a higher selection efficiency for leptons and jets
when calculating missing transverse momentum and resolving ambiguities between overlapping physics objects
(see below in this Section).
The leading lepton passing the {\scshape Signal} selection is matched to the lepton that triggered the event.
 
Electron candidates are reconstructed using energy clusters in the EM calorimeter which
are matched to an ID track, and they are calibrated as described in Ref.~\cite{EGAM-2018-01}.
{\scshape Baseline} electron candidates are required to have $|\eta|<2.47$ in order
to pass through the fine-granularity region of the EM calorimeter and
be outside the range $1.37<|\eta|<1.52$ corresponding to the transition
region between the barrel and endcap EM calorimeters.
They should also satisfy {\scshape Loose} identification criteria
and have $\pt>10~\gev$. The trajectory of {\scshape Baseline} electrons must be consistent with
the primary vertex to suppress electrons originating from pile-up.
Therefore, the tracks associated with {\scshape Baseline} electrons must have a longitudinal impact parameter
relative to the primary vertex ($z_0$) such that $|z_0\cdot\mathrm{sin}\theta|<0.5$~mm.
{\scshape Signal} electrons are defined as {\scshape Baseline} candidates that have $\pt>30~\gev$ and satisfy the
{\scshape Tight} identification and \textsc{HighPtCaloOnly} isolation requirements~\cite{EGAM-2018-01}.
The track associated with each {\scshape Signal} electron must have a transverse impact
parameter significance $|d_0 / \sigma(d_0)|\leq5$.
 
{\scshape Baseline} muon candidates are reconstructed in the region $|\eta|<2.5$
by matching ID tracks to tracks reconstructed in the MS, and they are
calibrated in situ using $Z\rightarrow \mu\mu$ decays~\cite{PERF-2015-10}.
{\scshape Baseline} muon candidates are required to have $\pt>10~\gev$.
They have to satisfy a set of requirements on the quality of the tracks defined as
{\scshape Medium}~\cite{PERF-2015-10} and to pass a requirement on the longitudinal
impact parameter $|z_0\cdot\mathrm{sin}\theta|<0.5$~mm.
{\scshape Signal} muons are defined as {\scshape Baseline} candidates that have $\pt>30~\gev$,
pass a requirement on significance of transverse impact parameter $|d_0 / \sigma(d_0)|\leq3$,
and satisfy {\scshape HighPt} muon identification requirements~\cite{PERF-2015-10}
...
the muon candidate track itself, has to be less than 1.25~\gev.
A bad-muon veto for the {\scshape HighPt} muons is applied. An event is rejected when a muon
has a large relative error of charge over momentum ($q/p$) associated with the track.
The veto efficiency is defined as a function of the $\pT$ and $\eta$.
...
Jets are then calibrated as described in Ref.~\cite{PERF-2014-02,PERF-2016-04}.
{\scshape Baseline} jets are required to have $\pt>20~\GeV$ and $\abseta<2.8$.
Events are vetoed if they contain jets induced by calorimeter noise or non-collision background,
according to criteria described in Ref.~\cite{ATLAS-CONF-2015-029}.
Additional jets that arise from pile-up interactions are rejected by
...
The jet candidates passing all the above requirements are called {\scshape Baseline} jets.
{\scshape Signal} jets are defined as {\scshape Baseline} candidates that have $\pt>30~\gev$.
...
procedure is applied to {\scshape Baseline} leptons and jets. First, jet candidates
...
Note, that ``--'' means that this criterion is not applied.
Two same-flavor opposite-sign (SFOS) leptons satisfying the {\scshape Signal} selection
criteria are required in the $Z$+jets control and validation regions;
while {\scshape Signal} and {\scshape Baseline} stand for the corresponding sets of the lepton
...
Leading lepton, \pt[GeV]     & {\scshape Signal}, >130  & {\scshape Signal}, >130      & {\scshape Signal}, >130
& {\scshape Signal}, >130  \\
Subleading leptons, \pt[GeV] & {\scshape Baseline}, <10 & SFOS, >30      & {\scshape Baseline}, <10
& {\scshape Baseline}, <10 \\
Leading jet, \pt[GeV]        & {\scshape Signal}, >130  & {\scshape Signal}, >130      & {\scshape Signal}, >130
& {\scshape Signal}, >130  \\
Subleading jets, \pt[GeV]    & {\scshape Signal}, <130  & {\scshape Signal}, <130      & {\scshape Signal}, <130, N$\ge$3
& {\scshape Signal}, <130  \\
...
background is events with non-prompt and misidentified (fake) leptons. It mostly originates
from multi-jet production processes when one of the jets is misidentified as a lepton.
In the muon channel, this background source is less than 0.5\% of the total background in the SR.
Its contribution is four times smaller than the single-top background and ten times smaller
...
The background yields for $W/Z$+jets and \ttbar processes
in the signal region (SR) are estimated using dedicated
control regions (CRs) and confirmed in validation regions (VRs).
...
to increase the purity of the corresponding background (last 3 rows in Table~\ref{tab:select}).
...
An additional validation region, SVR, is used to verify the agreement of background with data
...
because two {\scshape Signal} leptons are required in ZVR.
 
The multi-jet background for the electron channel is estimated using the data-driven
...
{\scshape Baseline} selection to enrich the selected events with non-prompt electrons
and non-electron objects identified as electrons.
The \textit{Matrix Method} uses \textit{tight} and \textit{loose} selection.
The \textit{tight} selection corresponds to the {\scshape Signal} requirement.
In contrast to \textit{tight}, the \textit{loose} selection uses the {\scshape Loose}
...
as function of lepton $\pT$ and $\eta$, and they cover all regions.
The $r$ and $f$ efficiencies are a part of the fake/non-prompt lepton backgrounds estimation
toolset~\cite{EGAM-2019-01} that is developed using data and MC simulations and is validated in data.
Estimation of the $r$ and $f$ uncertainties is described in Section~\ref{sec:syst}.
...
except the multi-jet are estimated using MC simulated events.
...
muon channels. The normalizations for the multi-jet, diboson and single top backgrounds
...
Systematic uncertainties described in Section 7 are added into the fit as
...
electron and muon samples in the data and in the MC. The sum of the MC events
takes into account weights related to efficiency of trigger, reconstruction,
identification, isolation, pile-up, etc. The combined channel (lepton+jet)
is fitted independently from the electron and muon channels.
Two types of fits are performed as detailed below.
...
In the absence of any significant excess above the SM background prediction, limits are evaluated
with the modified frequentist CL$_\mathrm{S}$ method~\cite{Read:2002hq}
using pseudo-experiments. The background normalization factors and nuisance parameters
are determined simultaneously in the CRs and in the SR. The bin width over \Minv in the SR is optimized
to obtain good fit performance and stability for all QBH threshold masses used in the analysis.
...
of the signal selection do not depend on the QBH threshold mass
...
sub-leading jets with $\pT>130~\GeV$ leads to a better separation between the signal
...
use of the veto on the high-\pT sub-leading jets at LO MC generated events.
...
Multi-jet estimation       & 1.4 & --  & --  & --  \\\hline
...
reconstruction, isolation, and triggering efficiencies as well as in the pile-up jet identification
using the jet vertex tagger algorithm~\cite{PERF-2014-03} are also propagated to the measured
\Minv distributions.
 
The uncertainty in the \Minv spectrum due to pile-up is estimated by varying the average number
of pile-up events in the simulation to account for the differences between the values
of the measured and predicted total inelastic cross-section used in the pile-up simulation~\cite{STDM-2015-05}.
...
The scales are independently varied upwards and downwards by a factor of two, excluding
...
The uncertainties in the multi-jet background are related to the estimate of the $f$ and $r$
...
Multi-jet  & $1670 \pm 200$   & --             & $570  \pm 47$   & --             \\
...
Multi-jet  & $1690 \pm 210$   & --             & $577  \pm 38$   & --             \\
...
The difference between expected and fitted yields of $W$+jet background in the electron channel
is $\sim$10\% in SVR and $\sim$20\% in SR,
with the $W$+jet normalization factor close to unity ($1.01\pm0.02$).
...
is a result of simultaneous pulls of several nuisance parameters (main contributors
are five parameters related to the $W$+jet yield variations in CRs).
...
($\sigma\times Br$) is estimated with pseudo-experiments at 95\%
...
The limits are obtained using pseudo-experiments with a spacing of
...
leads to conservative limit estimates. Future QBH lepton+jet analyses have the potential
to explore higher QBH mass ranges and lower QBH production cross-section values once
hard QCD radiation effects are included in the QBH event generation model.
...
compared to the previous ATLAS result at 8 TeV~\cite{EXOT-2013-03}.
