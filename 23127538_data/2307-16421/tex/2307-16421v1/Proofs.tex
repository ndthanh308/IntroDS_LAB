...
\cref{lem:pmaboundmain} is a Laplace approximation applied to the integral operator $\opV$. A similar estimate can be found in \cite[Lemma 4.2]{berman2020} under different assumptions. On the other hand, \cref{lem:erboundmain} is a triangular approximation argument which is motivated from the proof of \cite[Lemma 4.4]{berman2020}. %We note here that results similar to Lemmas \ref{lem:erboundmain} and \ref{lem:pmaboundmain} can be found in \cite[Section 4]{berman2020} albeit under a different set of assumptions. Note that  \cite{berman2020} assumes that the probability measures are supported on a periodic domain whereas we isolate assumptions on the PMA \eqref{eq:pma} as stated in \cref{asn:solcon}. In view of \cref{rem:bermanver}, the conclusions in Lemmas \ref{lem:erboundmain} and \ref{lem:pmaboundmain} can be viewed as generalizations of \cite[Lemma 4.2 and Proposition 4.3]{berman2020}. 

We will actually prove Lemmas \ref{lem:erboundmain} and \ref{lem:pmaboundmain} in the reverse order. We need to set up some notation first. For $k\ge 0$, consider the Bregman divergence from \eqref{eq:bregdiv} for the convex function $u_{k\vep}(\cdot)$ and $x,y\in\R^d$, 
\[
...
For any $\vep\in (0,1/2)$ and any $y\in\R^d$, define a probability measure with density
 ...
 \begin{align}\label{eq:prestim1}
     ...
     %\left(\vep^2 \omega\big(\sqrt{\vep\log{(1/\vep)}}\big)+\vep^{5/2}\big(-\log{\vep})^{9/2}\right),
 \end{align}
 \end{small}
 where 
