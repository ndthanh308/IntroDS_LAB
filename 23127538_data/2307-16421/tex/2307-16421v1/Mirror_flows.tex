...
\frac{d}{dt} x_t = -\frac{\partial x\hfill}{\partial x^u}(x^u_t) \frac{\partial F}{\partial x}(x_t)= - \frac{\partial}{\partial x^u} F(x_t). 
\end{equation}
...
\]
where $\dot{\rho}= - \div(v\rho)$. The Wasserstein gradient flow is given by the continuity equation \cite[Section 11.1]{ambrosio2005gradient}
\[
...
In this paper we will always take 
\begin{equation}\label{eq:mirror}
U(\rho)=\frac{1}{2}\wass_2^2\left(\rho, e^{-g}\right),
\end{equation}
which is known to be generalized geodesically convex (\cite[Lemma 9.2.1]{ambrosio2005gradient}). 
...
The second equivalent way of describing the Wasserstein mirror gradient flow is to take the time derivative of the curve in the space of measure, i.e., specify a continuity equation for the curve on Wasserstein space.  



    
For a general geodesically convex $U$, it is not immediate how to derive the continuity equation. However, for our special choice of $U(\rho)=\frac{1}{2}\wass_2^2\left(\rho, e^{-g}\right)$, at least formally, one can derive the continuity equation corresponding to the flow. 

Fix an absolutely continuous measure $\rho$. The Hessian matrix $\nabla^2 u$ is defined $\rho$-a.s. and is positive semidefinite. Define a new metric tensor on $\tanspace_{\rho}$ by 
...
\end{equation}
Note that the range of $v_2$ is contained in $\R^d$ and hence $\left(\nabla^2 u\right) v_2$ is well defined as a $d$-dimensional vector. This defines a new Riemannian gradient on the Wasserstein space $\nabla_{\wass}^U$ and a new gradient flow equation
\begin{equation}\label{eq:newgradflow}
\frac{\partial}{\partial t} \rho_t = \div\left( \nabla_{\wass}^U F(\rho_t) \rho_t \right).
\end{equation}
When it exists, we call this flow the Wasserstein mirror gradient flow of $F$ with respect to $U(\rho)=\frac{1}{2}\wass_2^2\left(\rho, e^{-g}\right)$. When $\nabla_{\wass} F(\rho)$ is given by the gradient of the first variation of $F$, $\nabla_x \left(\frac{\delta F}{\delta \rho}\right)$, we get
...
\cite{kitagawa2012parabolic} studies it for bounded domains.
...
F(\rho):=\begin{cases}
\KL{\rho}{e^{-f}},& \text{when $\rho$ is dominated by $e^{-g}$}, \\
+\infty,& \text{otherwise}.  
\end{cases}
\end{equation}
When $f$ is convex, it is well-known that $F$ is geodesically (and generalized geodesically) convex and lower semicontinuous. 

The first variation of $F$ (see \cite[Lemma 10.4.1]{ambrosio2005gradient}) at $\rho_t=e^{-h_t}$ is given by 
\[
\frac{\delta F}{\delta \rho_t}= f(x) - h_t(x), 
\]
...
\]
\item The map $(t,x)\mapsto u_t(x)$  lies in $\diffcont^{1,2}([0,\infty)\times \R^d)$. Further, for every $T>0$, all the corresponding mixed partial derivatives are bounded on $[0,T]\times \R^d$.
...
Sufficient conditions for \cref{asn:solcon} have been studied in the literature under different assumptions on the supports of the probability measures $e^{-f}$ and $e^{-g}$, and the initializer $u_0$ for \eqref{eq:pma}. For example, in \cite[Proposition 4.5]{berman2020}, the author shows that if $u_0,f,g$ satisfy \cref{asn:solcon}, (i) and (iii), and $e^{-f}$, $e^{-g}$ are supported on the torus then \cref{asn:solcon} is satisfied.
\end{remark}
...
The proof of \cref{thm:existlin} depends on the following interesting lemma that will be used multiple times in this paper.
...
for ``smooth" functions $\phi:\R^d\to\R^d$. 
...
Sufficient conditions under which \eqref{eq:uniflb} holds have already been explored in Berman \cite{berman2020}.
...
\end{enumerate}
We refer the reader to \cite{Wang2001,Cattiaux2010,Chen2021} for other conditions under which  the LSI condition can be established.
