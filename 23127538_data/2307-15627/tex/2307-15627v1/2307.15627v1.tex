\documentclass[11pt]{article}\textwidth 160mm\textheight 235mm
...
Section~\ref{local} is devoted to establishing the Q-linear convergence of the primal-dual sequence generated by the proposed inexact ALM. We summarize our results and discuss some open problems
in Section~\ref{con}.  Finally, in Section~\ref{appe}, we present
...
${\cal C}^2$-decomposable functions do not contain    CPLQ functions (see Remark~\ref{notdec}) and hence condition (b) in Theorem~\ref{error}  can't be covered by condition (a) therein. 
...
The next two examples reveal that the semi-stability of second subderivatives also holds for $\C^2$-decomposable functions and spectral functions, respectively.
...
\begin{Example}
Our goal is to show that the second subderivative of spectral functions is semi-stable. 
Let $\S^n$  stand for the space of all real $n\times n$ symmetric  matrices
equipped with the inner product 
$$
\la X, Y\ra=\tr(XY),\quad X,Y\in \S^n.
$$
The induced Frobenius norm of $X\in \S^n$ is defined by $\|X\|=\sqrt{\tr(X^2)}$.
Recall that  $f:\S^n\to \oR$  is a spectral function if it is  orthogonally invariant, meaning that  for any $X \in \S^n$ and any $n\times n$ orthogonal matrix $U$, 
we have $ f (X) = f (U^\top X U )$.
 It follows from \cite[Proposition~4]{l99} that any spectral function $f$ can be expressed in the composite form 
 \begin{equation}\label{spec}
f(X):=(\th\circ\lm)(X), \;\; X\in \S^n,
\end{equation}
where $\th:\R^n\to  \oR$ is a permutation-invariant function on $\R^n$, called symmetric, and $\lm$ is the mapping assigning to each matrix $X\in \S^n$ the vector $\big(\lm_1(X),\ldots,\lm_n(X)\big)$
 of its eigenvalues arranged in nonincreasing order. 
Before proceeding any further, we discuss some instances of eigenvalue functions that can be covered by spectral functions.
To do so, pick $X\in \S^n$ and $x=(x_1,\ldots,x_n)\in \R^n$. If $\th(x)=\max\{x_1,\ldots,x_n\}$, the spectral function $f$ in \eqref{spec} boils down to the largest eigenvalue of $X$. 
If $\th(x)=\sum_{i=1}^n x_i$, then $f$  reduces to the sum of all eigenvalues of $X$. Considering $\th(x)=\max\{x_i+x_j|\, i,j\in \{1,\ldots,n\}, \, i\neq j\}$ allows us to cover the sum of two largest 
eigenvalues of $X$. If $\th=\dd_{\R_+^n}$, then we get $f=\dd_{\S^n_+}$, where ${\S_+^n}$ stands for the 
cone of $n\times n$ positive semidefinite matrices.  The readers can find more examples of eigenvalue functions, covered by \eqref{spec}, in \cite[page~183]{ab}.


Denote by $\Or^n$ the set of all real $n \times n$ orthogonal matrices (those matrices $U$ satisfying $U^\top U=I$).  It is known that  for any $X \in \S^n$, there exists     $U\in \Or$ for which  we have 
\begin{equation}\label{specdocom}
X  =  U \Diag (\lambda (X)) U^\top\quad \mbox{with}\;\;\lm(X):=\big(\lm_1(X),\ldots,\lm_n(X)\big).
\end{equation}
For a given matrix $X\in \S^n$, we denote by $\Or^n (X)$ the set of  all  orthogonal  matrices $U$ satisfying \eqref{specdocom}. 
%We say that two matrices $X,Y\in \S^n$ admit   a simultaneous spectral decomposition if there exists 
%$U\in \Or^n$ such that $U^\top XU$ and $U^\top YU$ are diagonal matrices. The   matrices  $X$ and $Y$ are said to have a simultaneous {\em ordered} spectral decomposition if there exists 
%$U\in \Or^n$ such that $U^\top XU=\Diag (\lambda (X)) $ and $U^\top YU=\Diag (\lambda (Y)) $, where 
%$\Diag (\lambda (X))$ and $\Diag (\lambda (Y))$ are the diagonal matrices whose   diagonal entries  are the vectors $\lm(X)$ and  $\lm(Y)$, respectively.     
 

Suppose that $f:\S^n\to \oR$ is a proper convex spectral function and pick $(\oX,\oY)\in \gph \sub f$. 
Assume that  $\mu_{1} >  \cdots> \mu_{r} $ are distinct eigenvalues of $\oX$ and   define  then  the index sets
\begin{equation*}\label{index}
\alpha_{m} :=\big\{  i \in  \{ 1,\ldots, n\}   | \; \lambda_{i} (\oX)  = \mu_{m}\big\}\quad \mbox{for all}\;\; m=1,\ldots,r.
\end{equation*}\label{XU}
Moreover, define $\ell_i $ for any $i \in \{1, \ldots, n\} $ to be the number of eigenvalues of $\oX$ that are equal to $\lambda_{i} (\oX)$
but are ranked before $\lm_i(\oX)$ including $\lambda_{i} (\oX)$. 
We claim that the second subderivative of $f$ is semi-stable at $\oX$ for $\oY$. To justify it, 
let  $W\in \S^n$, $t>0$, and $Y \in \sub f(\oX)$. Setting $\Delta_{t}\lm(\oX)(W):=\big(\lm(\oX+tW)-\lm(\oX)\big)/t$, we get 
\begin{align*}
\Delta^2_{t} f\big(\oX,Y\big)(W)=&\, \frac{\th\big(\lm(\oX+tW)\big)- \th\big(\lm(\oX)\big)-t\big\la Y,W\big\ra}{\sm t^2}\\
= &\, \frac{\th\big(\lm(\oX)+t\Delta_{t}\lm(\oX)(W)\big)- \th\big(\lm(\oX)\big)-t\big\la \lm(Y),\Delta_{t}\lm(\oX)(W)\big\ra}{\sm t^2}\\
&+ \frac{\big\la \lm(Y),\Delta_{t}\lm(\oX)(W)\big\ra -\big\la Y,W\big\ra}{\sm t}\\
=&\, \Delta^2_{t} \th\big(\lm(\oX),\lm(Y)\big)(\Delta_{t}\lm(\oX)(W)\big)+ \frac{\big\la \lm(Y),\Delta_{t}\lm(\oX)(W)\big\ra -\big\la Y,W\big\ra}{\sm t}.
\end{align*}
It follows from   \cite[Theorem~5.2.4]{bl} that $\lm(Y)\in \sub \th(\lm(\oX))$  and that there is $U\in \Or(\oX)\cap \Or(Y)$ such that  $\oX=U\Lm(\oX){U}^\top$
and  $Y=U\Lm(Y){U}^\top$, where $\Lm(\oX)= \Diag (\lambda (\oX))$ and $\Lm(Y)=\Diag (\lambda (Y))$. Using these yields 
\begin{equation}\label{sdcy}
\big\la Y,W\big\ra= \big\la {U}\Lm(Y){U}^\top,W\big\ra=\big\la \Lm(Y),{U}^\top W {U}\big\ra=  \sum_{m=1}^r \big\la \Lm(Y)_{\al_m\al_m},U_{\al_m}^\top  W U_{\al_m}\big\ra,
\end{equation}
where  $ \Lm(Y)_{\al_m\al_m}$ stands for the submatrix of $ \Lm(Y)$ obtained by removing all the rows and all the columns of $ \Lm(Y)$ not in $\al_m$
and where $U_{\al_m}$ stands for   the submatrix of $U$ obtained by removing  all the columns of $U$ not in $\al_m$. 
On the other hand, it results from  \cite[Proposition~2.3]{ms23} (see also the proof of \cite[Theorem~1.5]{t01} and \cite[Remark~4]{t01} )  and     Fan's inequality (cf. \cite[Theorem~1.2.1]{bl}) that 
\begin{align*}
\big\la \lm(Y),\Delta_{t}\lm(\oX)(W)\big\ra &= \sum_{m=1}^r\sum_{j\in \al_m} \frac{\lm_j(Y) \big( \lm_j(\oX+tW)-\lm_j(\oX)\big)}{t}\\
&=\sum_{m=1}^r\sum_{j\in \al_m} \lm_j(Y) \lm_{l_j}\big(U_{\al_m}^\top WU_{\al_m}+tU_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big) + O(t^2)\\
&\ge  \sum_{m=1}^r\big\la \Lm(Y)_{\al_m\al_m}, U_{\al_m}^\top WU_{\al_m}+tU_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big\ra + O(t^2),
\end{align*}
where  $(\mu_m I-\oX)^\dagger$ stands for the pseudo-inverse of the matrix $\mu_m I-\oX$.
The latter, together  with \eqref{sdcy}, implies that 
\begin{equation*}
\frac{\big\la \lm(Y),\Delta_{t}\lm(\oX)(W)\big\ra -\big\la Y,W\big\ra}{\sm t}\ge  2\sum_{m=1}^r\big\la \Lm(Y)_{\al_m\al_m},  U_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big\ra + O(t).
\end{equation*}
Combining these estimates  leads us to  
\begin{align}
\Delta^2_{t} f\big(\oX,Y\big)(W)\ge &\; \Delta^2_{t} \th\big(\lm(\oX),\lm(Y)\big)(\Delta_{t}\lm(\oX)(W)\big) \nonumber\\
& + 2\sum_{m=1}^r\big\la \Lm(Y)_{\al_m\al_m},  U_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big\ra + O(t).\label{sp1}
\end{align}
Take the sequences $W^k\to W$, $t_k\searrow 0$, and $Y^k\to \oY$ with $Y^k \in \sub f(\oX)$ such that 
$$
\Delta^2_{t_k} f\big(\oX,Y^k\big)(W^k)\to \Hat \d^2 f(\oX,\oY)(W)\quad \mbox{ as} \;\; k\to \infty.
$$
Employing the same argument as the one for \eqref{sp1} tells us that there exist a sequence $U^k\in \Or(\oX)\cap \Or(Y^k)$ such 
that \eqref{sp1} holds when $Y$, $t$, $U$, and $W$ are replaced with $Y^k$, $t_k$, $U^k$, and $W^k$, respectively. 
It follows from $Y^k\to \oY$ and the inequality 
$$
\|\lm(Y^k)-\lm(\oY)\|\le \|Y^k- \oY\|
$$
that  $\lm(Y^k)\to \lm(\oY)$. Moreover, since $\Or$ is compact, we 
can assume without loss of generality that the sequence $\{U^k\b$ converges to some $U\in \Or(\oX)\cap \Or(\oY)$. 
Finally, it is known (cf. see   \cite[equation (4) and Remark~3]{t01}) that $\Delta_{t_k}\lm(\oX)(W^k)\to \lm'(\oX;W)$, where $ \lm'(\oX;W)$ stands for the classical directional derivative of 
the eigenvalue mapping $\lm(\cdot)$ at $\oX$ in the direction of $W$. Passing then to the limit in \eqref{sp1} allows  us to conclude for any $W\in \S^n$ that  
$$
\Hat \d^2 f(\oX,\oY)(W) \ge \Hat \d^2 \th\big(\lm(\oX),\lm(\oY)\big)\big(\lm'(\oX;W)\big)+ 2\sum_{m=1}^r\big\la \Lm(\oY)_{\al_m\al_m},  U_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big\ra.
$$
We should add here that the index sets $\al_m$ are fixed in this limiting process, since they only depend on $\oX$. 
To obtain the opposite inequality, we assume further that the symmetric function $\th$ is polyhedral. Employing now \cite[Corollary~5.8]{ms23} tells us that 
$$
  \d^2 f(\oX,\oY)(W) =   \dd_{K_\th (\lm(\oX),\lm(\oY))} (\lm'(\oX;W)\big)+ 2\sum_{m=1}^r\big\la \Lm(\oY)_{\al_m\al_m},  U_{\al_m}^\top  W( \mu_m I-\oX)^\dagger WU_{\al_m}\big\ra
$$
for any $W\in \S^n$. By \eqref{sspl}, we also have 
$$
\Hat \d^2 \th\big(\lm(\oX),\lm(\oY)\big)(\lm'(\oX;W)\big)=\dd_{K_\th (\lm(\oX),\lm(\oY))} (\lm'(\oX;W)\big).
$$
Combining these and using the fact that the inequality $\Hat \d^2 f(\oX,\oY)(W)\le  \d^2 f(\oX,\oY)(W)$ is always satisfied, we arrive at 
$$
\Hat \d^2 f(\oX,\oY)(W)=  \d^2 f(\oX,\oY)(W)\quad \mbox{for all}\;\; W\in \S^n,
$$
which proves that the second subderivative of $f$ is semi-stable at $\oX$ for $\oY$. We should add that polyhedrality assumption on $\th$ can be significantly  weakened
if one uses \cite[Theorem~5.7]{ms23} to obtain the calculation of $ \d^2 f(\oX,\oY)(W)$.  
\end{Example}
...
When  g in \eqref{comp} is CPLQ, Theorem~\ref{lcon} covers our recent result in \cite[Theorem~5.7]{HaS21}. When $g=\dd_{\S^n_+}$ and $\Th=\S^n$ in \eqref{comp}, which allow us to cover SDPs, the Q-linear convergence of the primal-dual sequence for the exact ALM was justified in \cite{ssz} when the nondegeneracy 
condition and the strong second-order sufficient condition are satisfied. Moreover, it was recently shown in \cite[Theorem~2]{ding} that if the multiplier $\oy$ is taken from the relative interior of the set of Lagrange multipliers  and the SOSC is satisfied, the inexact version 
...
We should also mention that a modified version of the ALM, called the {\em safeguarded}  ALM 
...
$
Parabolic regularity was recently studied extensively in \cite{mms,ms20,ms23} for different classes of sets and functions. Below, we show that ${\cal C}^2$-decomposable  functions
...
\bibitem{ding} {Wang, S., Ding, C.}: {Local convergence analysis of augmented Lagrangian method for nonlinear semidefinite programming}. Arxiv preprint \url{arXiv:2110.10594} (2022)
