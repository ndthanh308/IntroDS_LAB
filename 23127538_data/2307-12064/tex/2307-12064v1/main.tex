...
\emph{Imbalance.}---The main result can be exemplified in Fig.~\ref{fig:imbalance}, where the time evolution of the imbalance is shown for choices of (a) $L_j=n_j$ and (b) $L_j=n_j-\frac{1}{2}$ as the Lindblad operators.
...
In Fig.~\ref{fig:imbalance} (a), for very small $\gamma_p$, such that the effective rate of measurement occurs a long time after the closed system reaches a steady state, we see that the pre-thermal regime is dictated by the coherent dynamics alone. After sufficient time has passed $\sim 1/\gamma_p$ the system begins to lose information and reduces to a $\mathcal{I}=0$ thermal state. 
...
In the supplemental material we provide further details to support the main text. In Sec.~\ref{sec:qta}, we present the quantum trajectory approach employed here to find the time evolution of a quantum system under the influence of measurements. In Sec.~\ref{sec:lrb}, we give the mathematical steps required to derive the Lieb-Robinson bound used to support our numerical study. Finally, in Sec.~\ref{sec:lrb_ana}, we provide a more detailed analysis of the applicability of the Lieb-Robinson bound, particularly in the cases of very small and very large measurement rate outside the regime of enhanced localization.
