...
\title{Higher strangeonium decays into a light flavor baryon pair like $\Lambda\bar{\Lambda}$, $\Sigma\bar{\Sigma}$, and $\Xi\bar{\Xi}$}
...
The unquenched effect is important for us to understand the spectrum and the decay behaviors of hadrons, especially when we face the excited hadrons approaching threshold \cite{Luo:2019qkm,Zhang:2022pxc,Duan:2020tsx,Duan:2021bna,Duan:2021alw,Liu:2015taa,Zhao:2020qpd}. Hadronic loop mechanism is a quantitative description of the unquenched effect, which has been successfully employed to explain the anomalous hidden-charm/bottom decay behaviors of higher charmonium/bottomonium \cite{Cheng:2004ru,Liu:2006dq,Liu:2009dr,Meng:2007tk,Meng:2008dd,Meng:2008bq,Chen:2011qx,Chen:2011zv,Chen:2011pv,Chen:2011jp,Chen:2014ccr,Wang:2015xsa,Wang:2016qmz,Huang:2017kkg,Zhang:2018eeo,Huang:2018cco,Huang:2018pmk,Li:2021jjt,Bai:2022cfz,Li:2022leg}, where the predicted branching ratios are usually close to the experimental measurements. It also has been applied to study the decay behaviors of higher charmonium into a pair of charmed or strange baryon \cite{Qian:2021gby,Qian:2021neg}. In this section, we utilize the hadronic loop mechanism to study the partial decay widths of higher strangeonia to $\Lambda\bar{\Lambda}$, where 
hadronic loops can be as a bridge 
connecting the initial higher strangeonia to final state $\Lambda\bar{\Lambda}$, as depicted in Fig. \ref{fig:Lambda}.
...
\end{split}
\end{equation}
with $\sigma_{\mu\nu}=\frac{i}{2}[\gamma_\mu,\gamma_\nu]$. Here, the $\phi$, $K^{(*)}$, and  $\mathcal{B}_1/\mathcal{B}_2$ denote the fields of the corresponding mesons and baryons. $N$ represent the exchanged proton or neutron. The masses of the charged pion and proton are denoted as $m_{\pi^+}=139.6\,\text{MeV}$ and $m_p=938.3\,\text{MeV}$, respectively. With these preparations, we can now write down the decay amplitudes for higher strangeonia decaying into $\Lambda\bar\Lambda$, i.e.,
\begin{equation}
\begin{split}
...
&\times\frac{-g^{\beta\rho}+q_1^\beta q_1^\rho\slash{m_1^2}}{q_1^2-m_{K^{*}}^{2}}\frac{1}{q_2^2-m_K^{2}}\frac{1}{q^2-m_N^2}\mathcal{F}^{2}(q^2,m_N^2),\\
\end{split}
\end{equation}

\begin{equation}
\begin{split}
\mathcal{M}_{(4)}^{\Lambda\bar{\Lambda}}=&i^3\int\frac{dq^4}{(2\pi)^4}g_{\phi K^*K^*}\epsilon^\mu\Big(g_{\alpha\beta}(q_{1\mu}-q_{2\mu})+g_{\beta\mu}(p_\alpha+q_{2\alpha})\\
...
\end{equation}
where the factor of 2 in the total decay amplitude arises the summation over the isospin doublets  $(K^{(*)},K^{(*)+})$ and $(n,p)$.
...
\begin{equation}
\begin{split}
\Gamma=&\frac{1}{3}\frac{|\vec p_1|}{8\pi m_\phi^2}\sum_{{\rm spin}}|\mathcal{M}_{{\rm total}}^{\Lambda\bar{\Lambda}}|^2,\\
\end{split}
\end{equation}
...
\subsection{Branching ratios of  the $\phi(4S)$ and $\phi(3D)$ decaying into $\Lambda\bar\Lambda$}

In this subsection, we present the calculation result of the $\phi(4S)$ and $\phi(3D)$ decays into $\Lambda\bar{\Lambda}$ final states, focusing specifically on the strangeonia that lie above the $\Lambda\bar{\Lambda}$ threshold. To facilitate our study, we  utilize the resonance parameters of these strangeonium states, which have been obtained from Refs. \cite{Wang:2021gle,Wang:2021abg}.
...
Motivated by these findings, our work aims to provide some insights by employing a simple fit to the experimental data. Specifically, we take into account the contributions from the $\phi(4S)$ and $\phi(3D)$ resonances, in addition to a non-resonance background. Through this analysis, we may shed light on the presence and impact of these resonances in the observed cross section data.
...
\end{equation}
to describe the contribution from the $\phi(4S)$ and $\phi(3D)$ resonances, where $\Phi(s)$ is the two-body phrase space of final states, $m_\phi$, $\Gamma_\phi$, $\Gamma_\phi^{e^+e^-}$ are the mass, width, and di-electron width of the involved higher strangeonia, respectively.



The non-resonance contribution is parameterized by a simple exponential form
\begin{equation}
...
\sigma(s) = |\mathcal{A}|^2,
\end{equation}
where $\theta_{\phi(4S)}$ and $\theta_{\phi(3D)}$ are the phase angles. Since the ratio $R$ in Fig. \ref{fig:br} remains stable, we adopt a typical value of $\mathcal{BR}(\phi(4S)\to\Lambda\bar\Lambda)=0.66\times\,\mathcal{BR}(\phi(3S)\to\Lambda\bar\Lambda)$ in the fitting procedure.
...
Upon comparison with our results from the hadronic loop mechanism, we obtain the corresponding central and maximum values of the $\alpha$ parameter to be 3.55 and 4.15, as shown in Fig. \ref{fig:br}. Notably, the fitted branching ratios of $\phi(4S,3D)\to\Lambda\bar\Lambda$ align well with the corresponding branching ratios predicted by the hadronic loop mechanism. This observation indicates the contribution of the hadronic loop mechanism to the decays of higher strangeonia into strange baryon pairs.
% Figure environment removed


Furthermore, based on these studies, we can explore the decays of other higher strangeonia into $\Lambda\bar\Lambda$, as well as other strange baryon pairs such as $\Sigma\bar\Sigma$ and $\Xi\bar\Xi$, with a reasonable $\alpha$ parameter in the range of $3.0\sim4.0$. Here, the calculated branching ratios of strangeonia above $\Lambda\bar\Lambda$ threshold decaying into $\Lambda\bar\Lambda$ with $\alpha$ parameter from 3.0 to 4.0 are
\begin{equation}
\begin{split}
...
\section{Predicting the branching ratios of higher strangeonia decaying into  $\Sigma\bar\Sigma$ and $\Xi\bar\Xi$}\label{sec3}
...
The predicted branching ratios for strangeonia decaying into $\Sigma\bar\Sigma$ above the threshold, with $\alpha$ parameter ranging from 3.0 to 4.0, are as follows:
 \begin{equation}
\begin{split}
...
By considering the total decay amplitude listed in Appendix, we can obtain the predicted branching ratios for several higher strangeonia decaying into $\Xi\bar\Xi$, with $\alpha$ parameter ranging from 3.0 to 4.0. The results are  \begin{equation}
\begin{split}
...
\end{split}
\end{equation}
These predicted branching ratios can be tested by the future experiments like BESIII.
...
Our initial focus is on the decays of higher strangeonia into $\Lambda\bar{\Lambda}$ pairs. The calculation of the branching ratios for these decays is dependent on a parameter $\alpha$ in the monopole form factor. Recently, the BESIII Collaboration provided data on the cross section of $e^+e^-\to \Lambda\bar{\Lambda}$ \cite{BESIII:2023ioy}, allowing us to constrain the range of $\alpha$ by fitting this data. As demonstrated earlier, our results for higher strangeonium decays into $\Lambda\bar{\Lambda}$ agree well with the experimental data, serving as a robust test for the decay mechanism involved in these processes.

Building on this success, we extend our analysis to predict the decay behavior of other higher strangeonia decaying into baryon pairs such as $\Sigma\bar{\Sigma}$ and $\Xi\bar{\Xi}$, both of which are expected to have sizable branching ratios.
...
\iffalse
The light flavor mesons family is important for us to understand the strong interaction. However, according to theoretical works and experimental measurements \cite{Workman:2022ynf,Wang:2021gle,Wang:2021abg,Godfrey:1985xj,BESIII:2022kzc}, the light flavor mesons around 2.0 GeV has a dense distribution and most of them are relative wide states, it is critical to identify them in experiments. Finding and identifying more higher light flavor mesons are a urgent task at the current situation. Recently, a serious of cross section measurements of $e^+e^-$ decaying to light strange baryon pairs has been performed \cite{BESIII:2023ioy,BaBar:2007fsu,BESIII:2017hyw,BESIII:2019nep,BESIII:2020uqk,BESIII:2021rkn,BESIII:2020ktn,BESIII:2021aer}. We know the $e^+e^-$ annihilation is a ideal process to produce the vector mesons, there may exist signals of higher strangeonium in $e^+e^-\to\Lambda\bar\Lambda, \Sigma\bar\Sigma, \Xi\bar\Xi$ process, so the decay of higher strangeonium to light strange baryon pairs have brought us interest.

For strangeonia above $\Lambda\bar\Lambda$ threshold, the unquenched effect generally has a non-negligible contuibution. In this work, we propose a hadronic loop mechanism to quantitatively describe the unquenched effect, in the framework of hadronic loop mechanism, the excited $\phi$ states decay into a strange meson pair, then reach the final state consisted of strange baryon pair by exchanging a nucleon. With the effect Lagrangian approach, we can obtain the corresponding branching ratios with $\alpha$ parameter dependence. On the other hand, considering the accumulated experimental measurements of $e^+e^-\to\Lambda\bar\Lambda$ process, as well as the strangeonium mass spectrum from
the theoretical predictions, we do a simple fit with the contribution of $\phi(4S)$ and $\phi(3D)$ to describe the cross section date of $e^+e^-\to\Lambda\bar\Lambda$, the fitted branching ratios are agree with our predictions from the hadronic loop calculation, and we restrict the range of $\alpha$ parameter according to the fitted results. With the restricted $\alpha$ parameter, we also predict the branching ratios of more higher strangeonia decaying into $\Lambda\bar\Lambda, \Sigma\bar\Sigma, \Xi\bar\Xi$ process, these results can be tested by future experiments and can help the experimentalists search and identify the higher strangeonia.\fi
...
\begin{equation}
\begin{split}
\mathcal{M}_{(4)}^{\Sigma\bar\Sigma}=&i^3\int\frac{dq^4}{(2\pi)^4}g_{\phi K^*K^*}\epsilon^\mu\Big(g_{\alpha\beta}(q_{1\mu}-q_{2\mu})+g_{\beta\mu}(p_\alpha+q_{2\alpha})\\
...
&\times\frac{-g^{\alpha\eta}+q_1^\alpha q_1^\eta\slash{m_1^2}}{q_1^2-m_{K^{*}}^{2}}\frac{-g^{\beta\rho}+q_2^\beta q_2^\rho\slash{m_2^2}}{q_2^2-m_{K^{*}}^{2}}\frac{1}{q^2-m_N^2}\\
...
&\times\frac{1}{q_1^2-m_K^2}\frac{-g^{\beta\rho}+q_2^\beta q_2^\rho\slash{m_2^2}}{q_2^2-m_{K^{*}}^{2}}\frac{1}{q^2-m_\Lambda^2}\mathcal{F}^{2}(q^2,m_\Lambda^2),\\
\end{split}
\end{equation}
...
&\times\frac{-g^{\beta\rho}+q_1^\beta q_1^\rho\slash{m_1^2}}{q_1^2-m_{K^{*}}^{2}}\frac{1}{q_2^2-m_K^{2}}\frac{1}{q^2-m_\Lambda^2}\mathcal{F}^{2}(q^2,m_\Lambda^2),\\
\end{split}
\end{equation}

\begin{equation}
\begin{split}
\mathcal{M}_{\Lambda(4)}^{\Xi\bar\Xi}=&i^3\int\frac{dq^4}{(2\pi)^4}g_{\phi K^*K^*}\epsilon^\mu\Big(g_{\alpha\beta}(q_{1\mu}-q_{2\mu})+g_{\beta\mu}(p_\alpha+q_{2\alpha})\\
...
&\times\frac{-g^{\alpha\eta}+q_1^\alpha q_1^\eta\slash{m_1^2}}{q_1^2-m_{K^{*}}^{2}}\frac{-g^{\beta\rho}+q_2^\beta q_2^\rho\slash{m_2^2}}{q_2^2-m_{K^{*}}^{2}}\frac{1}{q^2-m_\Lambda^2}\\
...
&\times\frac{1}{q_1^2-m_K^2}\frac{-g^{\beta\rho}+q_2^\beta q_2^\rho\slash{m_2^2}}{q_2^2-m_{K^{*}}^{2}}\frac{1}{q^2-m_\Sigma^2}\mathcal{F}^{2}(q^2,m_\Sigma^2),\\
\end{split}
\end{equation}
...
&\times\frac{-g^{\beta\rho}+q_1^\beta q_1^\rho\slash{m_1^2}}{q_1^2-m_{K^{*}}^2}\frac{1}{q_2^2-m_K^{2}}\frac{1}{q^2-m_\Sigma^2}\mathcal{F}^{2}(q^2,m_\Sigma^2),\\
\end{split}
\end{equation}

\begin{equation}
\begin{split}
\mathcal{M}_{\Sigma(4)}^{\Xi\bar\Xi}=&i^3\int\frac{dq^4}{(2\pi)^4}g_{\phi K^*K^*}\epsilon^\mu\Big(g_{\alpha\beta}(q_{1\mu}-q_{2\mu})+g_{\beta\mu}(p_\alpha+q_{2\alpha})\\
...
&\times\frac{-g^{\alpha\eta}+q_1^\alpha q_1^\eta\slash{m_1^2}}{q_1^2-m_{K^{*}}^{2}}\frac{-g^{\beta\rho}+q_2^\beta q_2^\rho\slash{m_2^2}}{q_2^2-m_{K^{*}}^{2}}\frac{1}{q^2-m_\Sigma^2}\\
...
No. B20063, the National Natural Science Foundation of China under Grant No. 12247101,
the Project for Top-notch Innovative Talents of Gansu Province, and the fundamental Research Funds for the Central Universities.
