...
\begin{abstract}
  ...
  Although structural subtyping is undecidable in this setting, we describe a notion of parametricity for type constructors and then exploit it to define \emph{parametric subtyping}, a conceptually simple, decidable, and expressive fragment of structural subtyping that strictly generalizes nominal subtyping.
  ...
  An implementation of this decision procedure is available in an online repository.
\end{abstract}
...
In prior unpublished work by the present authors~\cite{Das+:arXiv21},
structural subtyping has been proven undecidable in the presence of recursive types and parametric polymorphism.%
\footnote{Independently, a related result was proven by \citet{Padovani:TOPLAS19} for subtyping of context-free session types~\cite{Thiemann+Vasconcelos:ICFP16}.
...
Given this undecidability, our goal is to identify an expressive, practical fragment of structural subtyping that has three key properties:
\begin{enumerate}
...
\item The fragment should \emph{generalize nominal subtyping}, preserving all subtypings that already hold under a nominal treatment of type constructors.
\end{enumerate}
...
\Citet{Solomon:POPL78} showed that structural equality for parametric polymorphism can be reduced to equivalence of deterministic pushdown automata, but it took more than 20 additional years to establish decidability~\cite{Senizergues:TCS01,Stirling:TCS01,Stirling:FSTTCS01}, albeit by an intractable algorithm.
As another example, even \emph{without} recursive types, bounded quantification is undecidable~\cite{Pierce:IC94}.

Nevertheless, in this paper, we are able to achieve our goal:
we propose a notion of parametricity for pairs of type constructors that forms the basis of a suitable fragment of structural subtyping, a fragment that we call \emph{parametric subtyping}.
Parametric pairs of constructors will map subtyping-related arguments to subtyping-related results, echoing \citeauthor{Reynolds:IFIP83}'s characterization of parametric functions~\shortcite{Reynolds:IFIP83}.
...
object-oriented systems; lazy and eager functional languages; imperative languages; mixed inductive/coinductive languages, such as call-by-push-value~\cite{Levy:PhD01}; session-typed languages~\cite{Honda+:ESOP98,Gay+Hole:Acta05,Caires+Pfenning:CONCUR10,Silva+:CONCUR23}; and so on.\footnote{Of course, when applied to a given language, there will be additional language-specific considerations, e.g., interaction with intersection types in TypeScript or type classes in Haskell.
We do not claim to address such considerations here.}

In this paper, we do not address subtypings such as $\dsubtype{{\mathbf{0}}{} < \one{}}$ and $\dsubtype{\one{} < {\mathbf{0} \imp \one}{}}$ that arise when some types are uninhabited~\cite{Ligatti+:TOPLAS17}, instead interpreting all types coinductively, which makes them all (including $\mathbf{0}$) inhabited.
However, the parametric subtyping rules in this paper are sound even in languages where some types are interpreted inductively.
Moreover, we want to examine the fundamental core of parametric subtyping: had we insisted on an inductive treatment of some types, parametric subtyping would be unsound for lazy functional and session-typed languages.
...
\end{alignat*}
  ...
  \begin{equation*}
    ...
    \infer{\dsubtype{{\stack[\alpha]}{} < {\name{qstack_2}[\beta]}{}}}{
      \dsubtype{\alpha{} < {\beta}{}}}
  \end{equation*}
  Specifically, these are valid \emph{parametric} subtyping rules because the premises involve only arguments, $\alpha$ and $\beta$.
  Rules such as ``$\dsubtype{{t[\alpha]}{} < {u[\beta]}{}}$ if $\dsubtype{\alpha{} < \one{}}$'' or ``$\dsubtype{{t[\alpha]}{} < {u[\beta]}{}}$ if $\dsubtype{\beta{} < {t'[\alpha]}{}}$'' would not be parametric because their premises are not subtypings among arguments alone.

  \quad Given such rules, parametric subtyping is conceptually rather straightforward:
  A subtyping between types, such as $\dsubtype{{\stack[\stack[\tau]]}{} < {\name{qstack_2}[\name{qstack_1}[\sigma]]}{}}$, holds if it has a finite derivation from the admissible parametric subtyping rules.
...
\item
 ...
 using the inferred rules.
  ...
  \quad
  We have implemented this decision procedure, and it is available in the supplementary materials (and intended for later submission as an artifact).%
\\[-0.75\baselineskip]

\item
  \emph{Generalizes nominal subtyping.}
  %
  Nominal subtyping is characterized by those parametric rules that relate identical type constructors, such as the above rule for $\dsubtype{{\stack[\alpha]}{} < {\stack[\beta]}{}}$.
  Our notion of parametric subtyping is indeed strictly more general than nominal subtyping, in that it also admits those parametric rules that relate distinct type constructors.
\end{enumerate}
...
Second, \citet[Section 7.4]{Davies:PhD05} defines an algorithm for subsorting parameterized sort constructors that respects parametricity, but requires explicit declarations for the constructors' variance and handles only very limited cases of nested sorts.
(On the other hand, he deals with general pattern matching, module boundaries, and intersections that are beyond the scope of the present work.)
...
For this reason, the user-defined types will be normalized in a preliminary elaboration phase that
inserts additional type constructors, in a manner reminiscent of the conversion of context-free grammars to \citeauthor{Greibach:JACM65} normal form~\shortcite{Greibach:JACM65} and \citeauthor{Huet:MSCS98}'s syntax used in deciding extensional equality of total B\"{o}hm trees~\shortcite{Huet:MSCS98}.
...
The somewhat nonstandard feature of this syntax is that it enforces the first part of the strict alternation between structural and named types that is prescribed by the normal form: here, structural types $A$ have only named types $\tau$ as immediate subformulas.

\subsubsection{Named types and signatures}\label{sec:structural:named}
...
in a signature $\sig$.
The signature provides finitely many definitions of the form $t[\alphas] \defd A$, exactly one for each defined type constructor, where the structural type $A$ may contain free occurrences of the parameters $\alphas$ but must be otherwise closed.
...
Notice that definitions enforce the other part of the strict alternation between structural types and named types that is prescribed by this normal form: here, type constructors $t[\alphas]$ are defined only in terms of structural types $A$, not named types $\tau$.
...
\end{equation*}
The elaboration phase would normalize these types by introducing an auxiliary type name, $\onen$, and revising the definitions of $\nat$ and $\even$ so that structural and named types alternate:
\begin{equation*}
  \onen \defd \one
  \:,\quad
...
To aid intuition, we show both the context-free grammar (in \citeauthor{Greibach:JACM65} normal form~\shortcite{Greibach:JACM65}) for this language and the corresponding, quite parallel, type definitions.
\begin{alignat*}{7}
...
$\dsubtype{\tau{} < \sigma{}}$ for named type $\tau$ a subtype of named type $\sigma$, and $\dsubtype{A{} < B{}}$ for structural type $A$ a subtype of structural type $B$.
...
Subtyping for the explicit polymorphic quantifiers $\forall x.\tau$ and $\exists x.\tau$ is also standard. 
However, we do \emph{not} currently support subtyping for implicit polymorphism~\cite{Odersky+Laeufer:POPL96}, in which $\dsubtype{{\forall x.\tau}{} < {[\sigma/x]\tau}{}}$ and $\dsubtype{{[\sigma/x]\tau}{} < {\exists x.\tau}{}}$ would hold for all $\sigma$.
Neither do we currently support bounded quantification~\cite{Cardelli+Wegner:CSUR85,Cardelli+:IC94}.
Because our interest is in the interaction of subtyping, parametric polymorphism, and recursion, these are outside the scope of this paper and left as future work.
...
\begin{equation*}
  ...
  \infer[\srule{\jrule{UNF-}}]{\dsubtype{\odd{} < \nat{}}}{
    \infer[\mathrlap{\srule{\plus}}]{\dsubtype{{\plus*{\suc\colon \even}}{} < {\plus*{\zero\colon \onen ,\, \suc\colon \nat}}{}}}{
      \infer-{\dsubtype{\even{} < \nat{}}}{}}}
\end{equation*}
We use a dotted line to indicate the coinductive appeals to $\dsubtype{\odd{} < \nat{}}$ and $\dsubtype{\even{} < \nat{}}$, which can also be thought of as admissible structural subtyping rules that can always be unfolded to the corresponding infinite derivations.
...
After all, structural \emph{equality} for coinductively interpreted types is decidable~\cite{Das+:TOPLAS22}, using a reduction from trace equivalence for deterministic first-order grammars, which is itself decidable but intractable~\cite{Jancar:JCSS21}.
However, structural \emph{subtyping} is undecidable in the presence of
recursively defined type constructors, as we will now show.

Our strategy for proving its undecidability will be to give a reduction from simulation of
guarded Basic Process Algebra~(BPA) processes~\cite{Bergstra+Klop:IC84} to structural subtyping.
Because simulation of guarded BPA processes is undecidable~\cite{Groote+Huettel:IC94}, this reduction will witness the undecidability of structural subtyping.
...
\footnote{In a mixed inductive/coinductive setting, where variant record types would be interpreted inductively, we conjecture that structural subtyping would remain undecidable even in the purely inductive fragment (\textit{i.e.}, without record types), and that this could be proved by reducing from BPA language inclusion~\cite{Friedman:TCS76,Groote+Huettel:IC94}, not simulation.
}
...
We call this fragment \emph{parametric subtyping}, for its basis in a notion of parametricity.

Recall the context-free languages example from \cref{sec:structural:examples} in which we were trying to prove that the type $\edyck_0$, corresponding to $\set{\leftn^n\rightn^n \$ \mid n \geq 0}$, is a subtype of the type $\dyck_0$, corresponding to the Dyck language.
...
The key insight behind this quotienting comes in revisiting the coinductive generalization that we used to prove $\dsubtype{{\edyck_0}{} < {\dyck_0}{}}$: in \cref{sec:structural:examples}, we instead proved that $\dsubtype{\kappa{} < {\kappa'}{}}$ implies $\dsubtype{{\edyck[\kappa]}{} < {\dyck[\kappa']}{}}$ for all named types $\kappa$ and $\kappa'$.
This statement could also be viewed as an admissible inference rule:
\begin{equation*}
  \infer-{\dsubtype{{\edyck[\kappa]}{} < {\dyck[\kappa']}{}}}{
    \dsubtype{\kappa{} < {\kappa'}{}}}
\end{equation*}
This rule looks very much like the kind of rules around which nominal subtyping~\cite{Kennedy+Pierce:06} is based, with a key difference: nominal subtyping
requires such rules to use the same type constructor on both sides of the conclusion.
Importantly for our purposes, this admissible rule is parametric, in the sense that the type constructors $\edyck[-]$ and $\dyck[-]$ map related arguments, $\dsubtype{\kappa{} < {\kappa'}{}}$, to related results, $\dsubtype{{\edyck[\kappa]}{} < {\dyck[\kappa']}{}}$.
This echoes \citeauthor{Reynolds:IFIP83}'s characterization of parametric functions as mapping related arguments to related results~\shortcite{Reynolds:IFIP83}.
...
But parameterized type constructors involve some complications, so in this \lcnamecref{sec:decide-mono}, we will provide a gentle introduction to the algorithm by presenting a saturation-based decision procedure for structural subtying of monomorphic types -- that is, recursively defined types that do not take parameters nor use the structural types $\forall x.\tau$ and $\exists x.\tau$.
...
But even if one is uninterested in polymorphic types, this forward-inference procedure offers several distinct advantages over the backward-search algorithm (with one potentially mitigable shortcoming), as we will discuss below.
...
Because the signature contains finitely many type definitions $t \defd A$ and $u \defd B$ and there are therefore finitely many subtyping problems $\dsubtype{t{} < u{}}$, we can check each problem in this way.
...
This last perspective will prove particularly useful in \cref{sec:parametric} and the decision procedure for parametric subtyping of polymorphic types presented there.
...
\intertext{Because $\set{\zero,\suc} \subseteq \set{\zero,\suc}$ as well as $\set{\suc} \subseteq \set{\zero,\suc}$, the $\arule{\plus}$ rule then allows us to infer}
  ...
  \asubtype{\even < \nat} \entails \asubtype{\odd < \nat}
   ;
   \enspace\text{\emph{as well as}}
  \\[-\jot]
   &\hphantom{\asubtype{\even < \nat} \entails \asubtype{\onen < \onen}
  \enspace\text{\emph{and}}\enspace}
...
\end{theorem}
\begin{proof}[Proof sketch]
  Finitely many definitions of the form $t \defd A$ and $u \defd B$ can be drawn from a given signature $\sig$.
...
With respect to the backward-search algorithm, our forward-inference algorithm, as formulated in \cref{fig:decide-mono}, does have one shortcoming: it does not account for structural subtypings that arise from uninhabited types that exist when types' interpretation is inductive or mixed inductive/coinductive, such as those that appear in work by \citet{Ligatti+:TOPLAS17} and \citet{Lakhani+:ESOP22}.
...
Many of the rules are carried over from the decision procedure for structural subtyping of monomorphic types that was described in \cref{fig:decide-mono} of \cref{sec:decide-mono}, with the addition of parameters and variances.
...
Moreover, backward proof construction over these admissible rules can be used to decide other subtying problems.
...
We give only sketches of the proofs here; details can be found in \cref{sec:appendix:proofs} of the supplementary materials.
...
% \begin{restatable}{lemma}{polycompletestructural}\label{lem:complete-structural-poly}
%   If $\dsubtype[i]{{t[\theta]}{\subs} < {u[\phi]}{\subs*}} \vof \xi$ and also $\asubtype[\xi]{t[\alphas] < u[\betas]} \entails \asubtype[\xi']{A < B}$, then $\dsubtype{A{\theta ; \subs} < B{\phi ; \subs*}} \vof \xi'$.
% \end{restatable}
% \begin{proof}[Proof sketch]
%   Directly, using \cref{lem:??}.
% \end{proof}


% \begin{restatable}{lemma}{polycompletenamed}\label{lem:complete-named-poly}
%   If $\dsubtype[i]{{t[\theta]}{\subs} < {u[\phi]}{\subs*}} \vof \xi$ and also $\asubtype[\xi]{t[\alphas] < u[\betas]} \entails \asubtype[\xi']{\tau < \sigma}$, then: $\dsubtype{\tau{\theta ; \subs} < \sigma{\phi ; \subs*}} \vof \xi'$.
% \end{restatable}
% \begin{proof}[Proof sketch]
%   By induction on the finitary derivation of $\asubtype[\xi]{t[\alphas] < u[\betas]} \entails \asubtype[\xi']{\tau < \sigma}$.
% \end{proof}


% \begin{restatable}{lemma}{polycompletebot}\label{lem:complete-bot}
%   If $\dsubtype[i]{{t[\theta]}{\subs} < {u[\phi]}{\subs*}} \vof \xi$, then $\asubtype[\xi]{t[\alphas] < u[\betas]} \nentails \bot$.
% \end{restatable}
% \begin{proof}[Proof sketch]
%   We generalize the \lcnamecref{lem:complete-bot} statement to show that $\dsubtype[i]{{t[\theta]}{\subs} < {u[\phi]}{\subs*}} \vof \xi$ and $\asubtype[\xi]{t[\alphas] < u[\betas]} \entails \bot$ together imply a meta-contradiction.
%   This is proved by induction on the finitary derivation of $\asubtype[\xi]{t[\alphas] < u[\betas]} \entails \bot$.
% \end{proof}
...
\end{theorem}
\begin{proof}[Proof sketch]
  Finitely many definitions $t[\alphas] \defd A$ and $u[\betas*] \defd B$ can be drawn from a given signature and combined with one of two variances.
  For each such triple, the structural types $A$ and $B$ have finitely many subformulas (without unfolding definitions).
  Each of the rules found in \cref{fig:decide-poly}, including the $\arule{\jrule{COMPOSE-}}$ rule, infers a judgment $\asubtype[\xi]{t[\alphas] < u[\betas*]} \entails \asubtype[\xi']{\tau < \sigma}$ only if $\tau$ and $\sigma$ are subformulas of $A$ and $B$, respectively (again, without unfolding definitions).
...
Our implementation in Standard ML is available in an online repository~\cite{Pfenning+:POPL24impl}.  It provides
a syntax for defining types, deriving parametric inference
rules, and checking subtyping.  All examples from this paper are
available in \verb|paper.poly|.  Because of mutual recursion, the
implementation proceeds in two phases: first checking basic
consistency and normalizing types, thereby extending the
signature with additional, internal definitions.  The second
...
definition in the signature $\Sigma$ has the form $t[\alphas] \defd A$
...
process, we may add further (internal) definitions to the signature.  We
show this translation only for the types $t[\theta]$, $\alpha$,
$A_1 \tensor A_2$, $\one$, and $\exists x.A$; all others are analogous.
\begin{alignat*}{2}
  ...
  (\alpha)^\dagger &= \alpha
  \\[-\jot]
  % (\plus*[\ell \in L]{\ell\colon A_\ell})^* &= \plus*[\ell \in L]{\ell\colon A_\ell^\dagger}
  % &
  % (x)^\dagger &= x
  % \\[-\jot]
  (\exists x.A)^* &= \smash{\begin{lgathered}[t]
                       \exists x.\, [x/\alpha]([\alpha/x]A)^\dagger , \\[-\jot]
                       \quad\text{where $\alpha$ fresh}
                     \end{lgathered}}
  &
  (A)^\dagger &= t[\alphas] \smash{\begin{lgathered}[t]
                             \text{ for $A$ structural, where $\alphas = \mathsf{free}(A)$} \\[-\jot]
...
signature $\Sigma$ (modulo renaming of the parameters).
...
repeated and any conjunction with $\bot$ is reduced to $\bot$.  This
...
We can take advantage of the $\forall$ and $\exists$ quantifiers to express module signatures as abstract types.
Here we define two signatures for abstract lists, with constructors $x$ and $\alpha \tensor x \imp x$ (respectively, $\beta \tensor x \imp x$) for the empty list and list concatenation, parameterized by the type $\alpha$ (respectively, $\beta$) of list elements.
...
The most closely related work has been mentioned; we now briefly overview other related work.
...
For session types, the subtyping problem has been shown to be undecidable~\cite{Padovani:TOPLAS19,Silva+:CONCUR23}, even though the corresponding type equality problems are decidable~\cite{Almeida+:ESOP20,Das+:TOPLAS22,Solomon:POPL78}.
