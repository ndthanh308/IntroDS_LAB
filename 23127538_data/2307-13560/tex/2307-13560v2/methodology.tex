...
$$
$$
    \sigma_{\theta}^{{t-1}}=\sqrt{\frac{1-\overline{\alpha_{t-1}}}{1-\overline{\alpha_{t}}}\dot \beta_{t}}.
$$
...
\begin{align*}
    ...
    &\qquad\left\{
    \begin{aligned}
        &v_{t-1}x_t+(1-v^{(1)}_{t-1})q_{\text{noise}}, \quad &&\text{if } x_t = x_0 \\
        &v_{t-1}x_0+(1-v_{t-1}^{(2)})q_{\text{noise}}(x_t), \quad &&\text{if } x_t \neq x_0
    \end{aligned}
    \right.
\end{align*}
...
Which models the joint distribution over both $x$ and $v$, where  $q_{noise}(x_t)=\beta_{t}x_t+(1-\beta_{t})q_{noise}$ and . The sampling process here also takes the reparameterized method, which improves flexibility and expressiveness compared to the vanilla multinomial diffusion process.
