...
\begin{itemize}
\item \texttt{siamonline220329.cls} (required): Main SIAM online-only \LaTeX\ class file.
...
\end{itemize}
To use these files, put \texttt{siamonline220329.cls} and
...
\begin{example}[label={ex:outline},listing only,%
  listing options={style=siamlatex,{morekeywords=[1]{maketitle},
      morekeywords=[2]{siamonline220329}},}]%
  {Document outline}
...
Following the author and title is the abstract, key words listing, and MSC codes, 
designated using the \code{abstract}, \code{keywords}, and \code{MSCcodes}
environments. 
...
\documentclass[supplement]{siamonline220329}
\input{ex_shared}
...
\begin{itemize}
\item Change in file name: \texttt{siamonline190516.cls} is replaced by \texttt{siamonline220329.cls}.
...
\item Added support for the SIAM Journal on Mathematics of Data Science (SIMODS).
...
\cref{ex:numbering} as documentclass options (e.g., \code{\documentclass[review,onefignum,onetabnum]} \code{{siamonline220329}}).
