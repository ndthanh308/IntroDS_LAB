...
%\twcomment{I have ideas, such as the iterative evaluation described in the VQ-VAE paper or systematic image mods to interogate classifiers, but I think they'd be considered a little unfamiliar, so I suggest we drop this line. There is the use of the generative model as a fast surrogate for traditional sims.}
...
%% - Make a generative network for small lartpc images using diffion networks
...
Modeling the processes through which the ionization eventually leads to waveforms
in the readout electronics is an important and challenging task~\cite{adams2018ionization}.  One challenge is that the model can have long execution times. For example, simulating an event in the MicroBooNE LArTPC can take upwards of 10-15 minutes per event when including trajectories from cosmic ray particles and particles produced by neutrino interactions.
...
These models are being used for a large variety of image types.
The types of images can vary widely and include natural images, paintings, or drawings. %Citations needed
...
% Clarify - semantic content 
 ...
 Such images are very different from those for which DMs were developed.
 ...
 What we find is that the method we adapted works well without the need to fine-tune meta-parameters.
...
Qualitatively, the images produced by the DM are quite similar to the training images and are 
 ...
 % DMs capable of producing high-fidelity LArTPC images also provide for new approaches toward the analysis of LArTPC images.
...
The goal of generative models is to learn a probability distribution given samples (data) and to produce, i.e. generate new samples from this learned distribution. In most cases, especially when using deep networks, the model learned is an \emph{implicit} one, in that an explicit characterization of the distribution is not sought but only a means to efficiently sample is desired. For a survey of various methods we refer the reader to \cite{bond2021deep, yang2022diffusion}.
...
Motivated by the recent success of the Score-based Generative Models (SGM) in  \cite{song2021scorebased} that are a part of a wider family of so called diffusion models \cite{yang2022diffusion}, in this paper, we employ them for learning a generative model for the LArTPC data and evaluate its performance. On top of the empirical success, several recent works such as \cite{chen2022improved, chen2022sampling, de2022convergence} have shown that given an accurate score estimator under relatively mild conditions, SGM can provably learn to generate high-dimensional \emph{multi-modal} distributions even when the the data is supported on low-dimensional manifold, a scenario that has plagued past efforts in generative modeling which are usually prone to suffering from mode collapse. However, apart from the requirement to learn good score estimators in high dimensions, diffusion models come with additional computational complexity most notably in generating samples from the learned model \cite{zhang2022fast}. 
...
The reverse SDE is given by the following equation for $s \in [0, T]$ (which, following \cite{chen2022improved}, we write in more transparent forward way):
\begin{multline}
   \label{eq:gen_sde}
...
The data is made by using Geant4 to simulate the transport 
of one or more particles within a volume of liquid argon.
...
We are not aware of such a model that is available outside of one of the LArTPC collaborations.
Instead, images are made by simply projecting the 3D locations of ionizations deposited within a volume onto the XY, XZ, and YZ plane.
However, a simple model of charge diffusion is applied as 
the ionization deposits are projected to the planes.
...
A common bias value is subtracted from all the pixels across the data set and then scaled to keep values between -1 and 1.
Details of the cropping algorithm, normalization, and preparation are recorded in Appendix~\ref{appendix:dataprep}.

A total of 50,000 images were prepared for training. 
An additional 10,000 images are reserved as a validation sample.
The images were compiled into a TensorFlow dataset.
We will post our prepared images as a dataset for reproducibility and comparison purposes on Zenodo.
...
The batch of perturbed images along with the sampled batch of times, $t$, next are passed into the score function estimator, $\bm{s}_{\bm{\theta}}(\vec{\bm{X}}_t, t)$, implemented as a convolution neural network (CNN).
...
\begin{equation}
    ...
    \mathcal{L}(\bm{\theta}; t) 
    = \frac{1}{N}\sum^{N}_{i}|| \bm{s}_{\bm{\theta}}(\vec{\bm{X}^{i}_t}, t) 
    - \frac{-(\vec{\bm{X}}^{i}_t-\vec{\bm{\mu}}^{i}_t)}{\vec{\bm{\sigma^2}}^i_t} ||^2_2,
\end{equation}
where the sum is over the $N$ samples, indexed by $i$, in the training batch. 
...
We will instead refer to training epochs from here onwards, where one epoch is defined as the number of training iterations needed to process a total number of images equal to the number of images in the training dataset.
Since our batch size, $N$, is 128 images and our training dataset is 50,000 images, one epoch is requires 391 training iterations. 
We trained the model for a maximum of 300 epochs. 

The losses for the training and validation set versus epoch for the final training run is shown in Fig.~\ref{fig:loss}.
...
This process can generate any number of images, we selected 50,000 to match the number of training images in order to make comparisons easier. % Formal rephrase 
...
Additional approaches were available within the repository. These approaches include alternate numerical solvers and also the use of ``corrector" functions which aim to modify the sampled images at each iteration to be more like the training data.
...
We believe this is an artifact of the diffusion process struggling to achieve the large regions of zero activation needed for these sparse LArTPC images. 

From our training loss curve in Figure \ref{fig:loss} we can see that the network rapidly reaches a plateau. 
As such we have selected three key epochs: 10, 50, and 150 for which to test quality.
We will demonstrate that 10 epochs is insufficient training and creates low-fidelity images. 
By 50 epochs our generated samples are of high fidelity according to our metrics in the results section. 
Extending training to 150 epochs and beyond produces a minimal reduction in the loss and determine that training for 50 epochs is sufficient. 
Various measurements of image quality will support this choice of training time.
...
In order to quantify this a little further, we conducted a small sample Turing Test where participants we asked to differentiate true LArTPC (training) images from our generated images by eye. 
...
Everyone else was within one $\sigma$ of random chance. 
...
% \textcolor{red}{question to Zev, Shuchin: do we need to do this you think?}
% Incoming particles interact with Argon atoms in the detector to produce ionization along the path. Through the application of a magnetic field charges drift onto a wire plane whereby scintillators [~] record the projected 'image' of charges in the detector. 
...
To measure this, we propose to compare the output of a semantic segmentation network, referred to as SSNet~\cite{collaboration2019deep,abratenko2020semantic}, that has been used by LArTPC experiments as part of their reconstruction and analysis chain~\cite{abratenko2022search}.
...
Electrons and photons typically instigate an electromagnetic cascade which results in a pattern of deposited charge referred to as a ``shower". 
...
%% This details best left to the methods type section
...
% We assume our training data $f_\theta(\mathbf{x})$ is i.i.d. from some unknown data distribution $p_{data}(\mathbf{x})$

% \begin{algorithm}
%     \caption{Generic Initialization}
%     \begin{algorithmic}[1]
%         \For{(each object $i$}
...
% It is worth noting that we apply a lower threshold on the pixel intensities, where any value below 5 (max of 255) is converted to 0. This is simply to compensate for the difficulty in generating exactly 0 in the background. Pixel intensities remain a weak point in the generational method (see charge distribution), but the generated images still surpass our criteria for useful LArTPC images. 

Our first external metric for gauging the quality of generated images is making use of SSNet. This semantic segmentation network labels pixels as background, track, or shower. The most simple analysis is counting the number of pixels each time. We can also look at the scores of each of the labels. These histograms are shown below. We are plotting epochs, 10, 50, and 150 compared against the validation dataset. We clearly see that 10 epochs of training is insufficient. By epoch 50 we have very good results, and see little improvement by training to 150 epochs. 

\subsection{Model Validation Via Comparing High-dimensional Distributions}

% High-Dimensional Goodness of Fit Tests 
% MMD uses sigma_list = [1024, 2048, 4096, 8192, 16384, 32768]
% Sinkhorn Divergence uses eps = 1
% Our data is too high dimensional for Wasserstein-1 or any other metric from Shaoib 

Seeking to perform model selection we propose to compare the pixel-space distributions, i.e. the distributions in the $64 \times 64$ dimensional space, of the generated data, training data, and validation data at each epoch via two high-dimensional goodness-of-fit (GoF) measures: Maximum Mean Discrepancy (MMD) \cite{JMLR:v13:gretton12a}, Sinkhorn Divergence (SD) \cite{genevay2018learning, mena2019statistical}, and the Wasserstein-1 distance \cite{Sriperumbudur_2012}. 
...
Sinkhorn divergences, while not strictly a distance, interpolate between Wasserstein and MMD distances and are numerically faster to compute as well as have faster rates of statistical estimation \cite{pmlr-v89-genevay19a, mena2019statistical}. 
...
This is calculated by getting the deepest layer activations of your model, fitting a Gaussian to your image sample, then finding the 2-Wasserstein distance. 
Typically the activations come from the deepest layer (pool3) of Google's Inception v3. 
However, we do not have a well defined classifier for LArTPC type images. 
Instead, we use SSNet and get the activations of the deepest convolution layer (double\_resnet[2]). 
The architecture of SSNet makes the layer choice difficult as the number of parameters is computationally unwieldy. 
Our activation vector is 8,192 parameters. 
This SSNet-FID  metric, shown in Fig. \ref{fig:FID} supports our previous analyses; the model rapidly improves before performance plateaus with an inflection point around epoch 50. 
The best image activations, i.e. closest to the training and validation datasets, are actually produced by epoch 100, but the differences within the plateau are minimal so we stick with our choices of epochs 10, 50, and 150 for further analysis. % Rephrase 
...
An important characteristic of our LArTPC images is the fact that they encode underlying physical processes. 
As such, we have devised a few simple tests to verify that the generated images follow the expected rules of particle physics. 
Generally, one of the most important features of any particle is its energy, which corresponds to the energy deposited within the detector. % Rephrase   
For track-like events, energy is deposited as it moves through the detector medium (liquid Argon), so we can simply find the length of the track to get an approximate measure of the particle's energy. 
We being by separating the generated images into track or shower according to the dominant SSNet pixel labeling. % Move this earlier? 
...
The majority of track images contain a single particle track, however, many events have more than one track. 
This is from interaction vertices producing multiple track-producing particles or occasionally overlapping events, an example is shown in Fig. \ref{fig:example_tracks}. 
Note that the length of these multiple-track events is the farthest straight line distance between two points, not a trace of the entire event. % Rephrase  
...
We then bin these values into a histogram in Fig. \ref{fig:track_length} displaying the fraction of images for each length. 
...
We see that epoch 50 is a closer match.
...
the track width is sensitive to the frequency of secondary interactions that can occur with pions and protons which scatter off nuclei in the detector. It is also in theory sensitive to the shape and frequency of delta-ray production from muons. The delta rays manifest as shower features in the images and in principle should not be included in the track cluster pixels. However, in practice, the delta ray showers are often small enough to be tagged by the SSNet as track pixels and are then seen as track pixel bumps along the track.
...
As before we have plotted our track width histogram in Fig. \ref{fig:track_width}. 
...
A few epoch 10 tracks are so poor that the image is classified as entirely noise, in which case the track is reported as having zero width.
...
Using the chi-squared values in Table \ref{tab:chi-squared},
we find that for the track width distribution agreement, the time epoch 150 is a significantly closer fit to our validation dataset. 
...
We have plotted a histogram showing the amount of charge deposited per image in Fig. \ref{fig:shower_charge}. 
We find that epoch 10 does not resemble the shape of the validation dataset. 
As expected 10 epochs insufficient amount of training and produces very noisy images, which, due to the larger number of pixels results in a very high charge measurement. 
We have condensed all these outlier events produced by epoch 10 to the final overflow bin. 
Epochs 50 and 150 both more closely resemble the validation dataset, but are not perfect.
Visually it is difficult to determine which charge distribution is more accurate, we can use the mean as a comparison shorthand. 
...
This puts epoch 50 as the closer match, off by only 273 compared to epoch 150 difference of 716. 
...
Both these tests agree that epoch 50 produces at least marginally better shower images than our other key epochs. 
...
\end{tabular}
\caption{Breakdown of track and shower rates. Images are classified as "track-like" or "shower-like" based on which type of pixel label, produced by SSNet, is most frequent in the image. The convergence of the track vs. shower fraction in the generated image data set produced at the different epochs towards the validation set is a measure of the model's fidelity of reproducing the original data set.}
...
An example in our context is that we would want the generator to produce tracks smoothly over a range of lengths and angles and not just generate lengths or angles closely around the neighborhood of examples in the training set. % Rephrase 
...
Several examples of nearest neighbors can be found in Appendix~\ref{appendix:moreimages_mode_check}, Fig. \ref{fig:l2_neighbors}. 
...
Examples of this can be found in Appendix~\ref{appendix:moreimages_mode_check}, Fig. \ref{fig:emd_neighbors}. 
...
This is particularly evident in showers where the overall orientation of the shower and its density is similar but the patterns within the shower envelope vary noticeably.
...
Combing this evidence with the reproduction of various distributions, we conclude that the model does not exhibit significant mode collapse, but is instead generating examples with variations similar to that of the training sample.
...
Future directions for our work would aim at addressing the impact to physics analyses but will also need to explore the large space of choices in the implementation of diffusion models.
...
Such models in order to be useful will have to be capable of generating much larger images than the 64x64 images explored here.
...
Another area of active research is into the theoretical connections of diffusion models to optimal transport~\cite{khrulkov2022understanding} and Schrodinger Bridge formulisms~\cite{de2021diffusion},
...
In order to support efforts to reproduce and compare against our effort, 
...
These materials will be stored on Zenodo.
Scripts and code for training, generating, and calculating our evaluation metrics will be provided on github.
...
According to these metrics, this is the first example for LArTPC experiments where images produced by a generative model are shown to be very similar to the training set. 
...
% Our first test was applying SSNet to provide a semantic analysis of the fine structure of our images through the pixel label histograms of Fig. \ref{fig:SSNet_hists}. 
...
\appendix
%TODO: Replace all arXiv citations with other sources 
...
The code used in our various quality metrics will be made available on GitHub and the associated datasets. 
...
Larger sigma values scaled the results without affecting the shape of the distribution. 
For Sinkhorn Divergence we used eps = 1, varying this value primarily affected the scaling.
...
The values for SSNet-FID are reported in Table~\ref{appendix:SSNet-FID_values}
...
Figures~\ref{fig:training_image_page} and Figures~\ref{fig:generated_image_page} include more examples of training and generated images.
...
We revisited our previous attempt \cite{lutkus2022towards} using a vector quantized variational autoencoder to generate LArTPC images. 
We generated 50,000 new VQ-VAE images to compare performance against our diffusion model and ran the images through SSNet and our physics-based metrics.  
These results can be found in Figures~\ref{fig:SSNet_hists_vqvae} and~\ref{fig:physics_vqvae} respectively.
We see that while VQ-VAE performs better than expected from our previous analysis, it still is noticeably worse than epochs 50 and 150 across all our metrics. 
...
% \begin{itemize}
%     \item page-long figure with generated examples
%     \item page-long figure with training examples
%     \item generated sample and N-top matches using RMS metric (like Fig 9 in {https://arxiv.org/pdf/1907.05600.pdf})
% \end{itemize}
