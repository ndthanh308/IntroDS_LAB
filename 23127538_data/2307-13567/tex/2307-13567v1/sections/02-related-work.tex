...
Templates are generally recognized as a user-friendly way to create charts, especially for beginners. The problem with traditional template-based systems is that the templates are created by system developers, and they usually suffer from limited expressivity and quantity. 
...
Chen~\etal~ \cite{chen_towards_2020} use deep learning to extract timelines from infographics as templates; Chartreuse \cite{cui_mixed-initiative_2022} supports reusing infographics bar chart templates. 

Overall, D3 deconstructor \cite{harper_deconstructing_2014,harper_converting_2017} and Chartreuse \cite{cui_mixed-initiative_2022} are the closest work to Mystique. D3 deconstructor only takes charts created using D3 \cite{bostock_d3_2011} that have the source data embedded; Chartreuse primarily works on Microsoft PowerPoint graphics assets. In contrast, Mystique works on visualizations in the general SVG format, does not require access to underlying data, \revise{and supports more advanced layouts}. 
...
Various automated or semi-automated methods have been proposed to detect marks \cite{ying_glyphcreator_2022,chen_towards_2020}, axes and legends \cite{shukla_recognition_2008, choudhury_scalable_2016}, classify chart types \cite{savva_revision_2011,shukla_recognition_2008}, extract data \cite{jung_chartsense_2017,harper_converting_2017,harper_deconstructing_2014,masson2023chartdetective} and visual encodings \cite{poco_reverse-engineering_2017,poco_extracting_2018,harper_deconstructing_2014,harper_converting_2017,cui_mixed-initiative_2022}. Due to the vast space of visualization examples, these methods typically narrow the scope by focusing on specific glyph or chart types. 

\bpstart{Mark Detection}~Many approaches assume the input visualizations are in a raster image format, where object detection is essential. For example, GlyphCreator \cite{ying_glyphcreator_2022} focuses on circular glyphs, and uses deep learning to perform object and bounding box detection. Similarly, visual elements in timeline infographics can be identified using deep learning \cite{chen_towards_2020}. OCR is typically used to recognize text elements \cite{poco_reverse-engineering_2017}. Since our input format is SVG, mark detection is not necessary.
...
However, these methods can still be error-prone. Since 
% accurate extraction is difficult, and 
it is relatively easy to indicate where the axes and legends are, some tools assume users will provide such information \cite{poco_extracting_2018}. In Mystique, we use heuristics to find axes and legends, and provide a UI for authors to correct potential mistakes.
