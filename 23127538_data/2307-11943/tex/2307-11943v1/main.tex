...
\title{Minimal AC injection into Carbon Nanotubes}
...
Controlled electron injection in an electronic system is an important issue, both for potential applications and for the fundamental study of the many-body properties of the system.\cite{feve07, hermelin11, mcneil11} 
...
These peculiar excitations have been called \textit{Levitons}, and have been realized experimentally in 2d electron gases.\cite{dubois13,bocquillon14,jullien14} 
...
This applies for example for conducting carbon nanotubes\cite{Egger1998,kane97,Yoshioka1999}, semiconductor nanowires,\cite{zaitsev00} edge states of the fractional quantum Hall effect,\cite{wen95} etc.
...
Other remarkable behaviors of 1d interacting systems are the charge fractionalization thanks to the existence of collective mode, spin-charge separation, etc.
When connected to standard, non-interacting electrodes, non trivial processes at the boundaries between the interacting system and the electrodes, similar to Andreev reflection, do occur, creating a complex behavior for the time-dependent current.\cite{maslov95,safi95,ponomarenko95,sandler98,hashisaka21,glidic2023} 
...
To answer this question in the case of a non-chiral system, in this work we consider explicitly AC electron injection into a carbon nanotube (CNT) from a STM tip (see Fig.~\ref{fig:Setup}), and calculate the excess noise induced by the external AC driving.
...
\label{eq:HT} \\
\Gamma_\epsilon(t) &= \Gamma_0 \exp \left[ \frac{i\epsilon e}{\hbar} \int_{-\infty}^{t}V(t')\, dt'\right],
\end{align}
where $\Gamma_0$ and $\Gamma_\epsilon(t)$ are the tunneling amplitudes without and with effect of the time-dependent voltage $V(t)$ and the superscript $\epsilon$ leaves either
...
\begin{align}
\Gamma_-(t) = \sum_{l=-\infty}^\infty p_l e^{-i(\omega_0 + l\Omega)t}, 
\end{align}
...
We note that, contrarily to the case of quasi-particle tunneling into edge states of the fractional quantum Hall effect~\cite{Rech2017}, the excess noise does not show a singular behavior near integer values of $q$.
...
&\qquad \times \left[ \frac{i \theta((l'-l)\Omega)}{(g_{\rm N}+g_{2})v_{2}} \left(\frac{1+b_1 e^{-i\frac{2(l-l')\Omega L_1}{v_N}}}{1-b_1b_2 e^{-i\frac{2(l-l')\Omega}{v_N}L}}e^{-i\frac{(l-l')\Omega L_2}{v_N}}e^{-i\frac{(l-l')\Omega}{v_2}(x-L_2)} -{\rm H.c.}\right) X_{1,(ll')} \right. \nonumber\\
...
\left(\frac{1+b_1 e^{-i\frac{2(l-l')\Omega L_1}{v_N}}}{1-b_1b_2 e^{-i\frac{2(l-l')\Omega}{v_N}L}}(e^{-i\frac{(l-l')\Omega}{v_N}x}-b_2 e^{i\frac{(l-l')\Omega}{v_N}(x-2L_2)})-{\rm H.c.}\right)X_{1,(ll')} \right.  \nonumber\\
