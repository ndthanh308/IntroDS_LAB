...
Let $M$ be a symplectic manifold which is either closed or geometrically bounded and let $K\subset M$ be an arbitrary compact subset. Hamiltonian Floer theory associates to $K\subset M$  the cohomology group $SH^*_K(M)$, the symplectic cohomology of $K$ relative to $M$.  This invariant plays a central role in many recent works.  

As is typical in Floer theory, it is impossible to compute relative $SH$ from its definition. However, one can hope to make progress by analyzing its underlying chain complex $SC^*_M(K)$ as the deformation of something simpler.  The idea that we pursue in the present paper is that $SC^*_M(K)$ is the deformation of \emph{local Floer cohomology} \footnote{There is an unfortunate conflict of terminology, as what is here referred to as relative SH has sometimes been called, including by the author, local SH, whereas the term local Floer cohomology is used in the literature to what we call below the unweighted Floer cohomology of an orbit.}, a version of Floer cohomology which takes into account only Floer trajectories of infinitesimally small energy.  

The main class of examples motivating the present study is the case where $K$ is an invariant neighborhood of a singularity of a Maslov $0$ Lagrangian torus fibration.  However we formulate all our results for compact sets with arbitrary contact boundary.  This of course contains a lot more than singularities of Lagrangian torus fibrations.  On the other hand,  the contact hypothesis is a bit restrictive for the case of Lagrangian torus fibrations. For example, the symplectic cluster manifolds studied in \cite{GromanVarolgunes2021} are not generally exact. The removal of the contact type hypothesis will be taken up in forthcoming work \cite{GromanToAppear2}. 
...
\begin{itemize}
\item any path of Reeb orbits has vanishing flux, and,
\item the Chern class evaluates to zero on any closed path of Reeb orbits.
\end{itemize}
Note that we do not assume these components are Morse-Bott, or even that they are smooth manifolds. To each Reeb component $\gamma$ of $\partial K$ we can associate an Abelian group,  $SH^*_{uw}(\gamma)$, the \emph{local Floer cohomology} of the Reeb orbit as studied by \cite{Mclean2012}.  A precise definition in the present context is given below.  Before proceeding we point out some basic facts.
\begin{itemize}
\item The group $SH^*_{uw}(\gamma)$ depends only on the multiplicity of $\gamma$ and the germ  of $M$ and $\partial K$ up to symplectomorphism near $\gamma$.   In fact, it depends on the germ only up to uniformly isolating smooth isotopy.  
\item If $\gamma$ is Morse-Bott, the group $SH^*_{uw}(\gamma)$ over $\bZ$ is, up to a shift of grading,  the integral singular homology of $\gamma$ twisted by a local system $\cL$ coming from Floer theory.  
\item When $\gamma$ is a Maslov $0$ Lagrangian torus and $\partial K$ is integral affine convex at $\gamma$ the local system $\cL$ is trivial. 
\end{itemize}
...
\end{equation}
We can think of the component $H^*(K;R)$ is the local Floer cohomology of the component of constant orbits inside $K$. 

We first give a formulation of our result  under the assumption that \emph{the period spectrum is gapped} this means that there is a $\delta>0$ so that the absolute value of the difference of any two distinct periods of orbits representing a given homology class in $H_1(\partial K;\bZ)$ is at least $\delta$. We later relax this assumption. 

The underlying complex $SC^*_M(K)$ of relative symplectic cohomology is defined over the universal Novikov ring, and as such comes equipped with a $T$-adic filtration.  In particular, for any real number $\hbar$ we have a $\bZ$ filtration of $SC^*_M(K)$ defined by $F^pSC^*_M(K):=T^{p\hbar}SC^*_M(K)$. This gives rise to a spectral sequence $E_*^{*,*}=E_*^{*,*}(M,K)\Rightarrow SH^*_M(K) $ associated with the filtration. 
...
\item
The restriction of $\omega$ to $\partial K_1\cup \partial K_2$ is exact.
\end{enumerate}

\begin{rem}
Crucially, we \emph{do not} require the primitive on $\partial K_1\cup\partial  K_2$ to be of contact type on both of them. See Figure \ref{FigAdmInc}. This is essential for studying restriction maps for non-exact embeddings.  One of the main advantages of relative $SH$ over other approaches to symplectic cohomology is that it can handle non-exact embeddings. 
\end{rem}
...
\begin{equation}
\xymatrix{E_1^{p,q}(K_2,M)\ar[r]\ar[d]&RH^*(K_2)\otimes \Lambda_{[-(p+1)\hbar,-p\hbar)}\ar[d]\\
E_1^{p,q}(K_1,M)\ar[r]& RH^*(K_1)\otimes\Lambda_{[-(p+1),\hbar,-p\hbar)}}
\end{equation}
...
There is a natural isomorphism $SH^*_{K,t^+}(M)\simeq RH^*(K)$. This isomorphism commutes with  restriction maps for admissible inclusions. 
\end{theorem}



An important notion which comes out of this discussion is the \emph{spectral symbol} $\sigma:SH^*_K(M)\to RH^*(K)$ defined when $SH^*_K(M)$ and which associates to an element its leading term. For a precise definition see \S\ref{SecSpectral}
...
\end{equation}
for admissible inclusions $K_1\subset K_2$ and $\delta>0$ small enough. This can be seen as a weak version of Viterbo functoriality. Note that for general ambient manifold $M$, we generally don't have a natural map $H^*(K)\to SH^*_K(M)$. 
...
\item  there is a subset $\Delta$ which deformation retracts to a codimension $2$ hypergraph so that the fibers over $B\setminus \Delta$ are smooth Maslov $0$ Lagrangian tori.  
\end{enumerate}

\begin{pblm}\label{pblmNonAdHoc}
Construct in a non-ad hoc way a $K$-analytic space $M^{\vee}$ over the universal Novikov field together with a Non-Archimedean torus fibration $\pi^{\vee}:M^{\vee}\to B$ which is mirror to  $\pi:M\to B$ in the sense of, say, homological mirror symmetry. 
\end{pblm}
...
Let $P_1\subset P_2$ be admissible. We say that the inclusion is \emph{admissible} if $\partial P_1\cap\partial P_2$ is a  connected codimension $1$ subset whose boundary points are interior points of the $1$ dimensional strata of $\partial P_2$. See Figure \ref{FigAdm}. It is easy to see this property is  transitive. Note that $\partial P_1\cap\partial P_2$  is a union of successive edges of $\partial P_1$. 
...
Suppose the regular fibers of $\pi$ are undeformed.   Suppose the inclusion of a regular fiber into $\pi^{-1}(P)$ induces an injection of $H^*(\pi^{-1}(P);\bZ)$ into $H^*(\bT^n;\bZ)$.  Then
\begin{enumerate}
\item \label{tmLocSHn2:pt1} $\cF^*(P)$ is torsion free over the Novikov ring.
\item \label{tmLocSHn2:pt2} For $i$ indexing the edges of $\partial P$,  let $z_i\in \cF_P$ be a choice of element whose spectral symbol is the generator associated with the $i$th edge.   Then $\cF_P$ is generated as a Banach algebra by the collection $z_i$. In particular, $\cF_P$ is affinoid. 
...
\item $\cF^*(P)$ injects into the ring of polyvector fields over $\cF^0(P)$. 
\end{enumerate}
\end{theorem}
\begin{rem}
It follows in particular that $\cF^0(P)$ is an integral domain and that $
...
Suppose $\cF^0(P)$ is a smooth algebra and that  Suppose $\pi$ admits a section over $P$. Then
\begin{enumerate}
\item  the dual cohomology class maps to a generator $\sigma$ of $\cF^2$ over $\cF_0$ corresponding to a nowhere vanishing bi vector field. 
\item
The injection of  $\cF^*(P)$  into the polyvector fields over $\cF^0(P)$ is an isomorphism
\item 
The BV operator on $\cF^*(P)$  is the divergence operator associated with the isomorphism $\cF^0\simeq\cF^2$ induced by $\sigma$. 
\end{enumerate}
\end{theorem}

The question of smoothness for symplectic cluster manifolds is discussed in forthcoming work. We point out that that this holds for neighborhoods of the nodal singularity in a symplectic cluster manifold. A simple example where $\cF^0(P)$ is not smooth occurs when $P$ contains a node with multiplicity greater than $1$. In forthcoming work it will be shown that this the only way in which non-smoothness occurs for symplectic cluster manifolds. 
...
\end{cases}
\end{equation}
The main Theorems relate Floer cohomology of $H_K$ in small action windows with local Floer cohomology groups $SH^*_{uw}(\gamma)$ associated with the Reeb orbits. To explain what goes in to the proof of this relation we first discuss the case of smooth Hamiltonians. 

When $H$ has degenerate $1$-periodic orbits, the Floer complex $CF^*(H)$ is defined up to homotopy equivalence as a colimit over a monotone sequence of non-degenerate Hamiltonians $H_i$ approximating it from below
$$
...
\[
\Lambda_{b}:=\left\{\sum a_iT^{\lambda_i}|\lambda_i\in (-b,\infty),a_i\in R,\lim_{i\to\infty}\lambda_i=\infty\right\}.
\]
...
$$
Here  $hoColim$ denotes some unspecified model for the homotopy colimit, a popular choice being the telescope construction. The widehat denotes completion with respect to the adic norm. The complex $CF^*(H)$ is well defined up to contractible choice. Within the text below we shall have occasion to refer to the Floer complex $CF^*(H)$ for $H$ which is not smooth, or even, as in the case of indicator functions, discontinuous.  In Appendix \ref{SecAcceleration} we recall the construction for indicator functions.
...
Recall that given an arbitrary proper Hamiltonian $H$, with no assumptions regarding non-degeneracy or dissipativity, we define the truncated Hamiltonian Floer cohomology 
\begin{equation}
HF^*_{[a,b)}(H):=\varinjlim_{H'}HF^*_{[a,b)}(H'), 
\end{equation}
where the colimit is taken over all dissipative non-degenerate Hamiltonians. This can of course by constructed at the chain level by an appropriate homotopy colimit.
\begin{rem}
...
\end{rem}
The following lemma expresses the sense in which $HF^*_{[a,b)}(H)$, for small enough intervals, is about the dynamics of $H$, not the sequence approximating it. For simplicity of presentation, assume in the following $M$ is closed. The adjustment for $M$ geometrically bounded is completely straightforward.
...
\begin{df}
Let $H$ be a Hamiltonian. A family $\gamma$ of $1$- periodic orbits of $H$ is said to be a \emph{well isolated periodic component} if 
\begin{enumerate}
...
Fix a trivialization of the canonical bundle over $\gamma$ and denote by $\iota_{CZ}(\gamma)$ the Conley-Zehnder index of $\gamma$ according to this trivialization.  The family $\gamma$ considered as a manifold whose points are $1$-periodic orbits of $H$ carries a local system $\cL$ of $\bZ$-coefficients whose stalk over each point $x\in\gamma$ is the orientation line of the Fredholm operator associated with $\gamma$. This operator is defined up to homotopy \cite{Seidel} and the corresponding orientation line is defined up to isomorphism. 
...
The map is constructed by considering configurations consisting of a Floer trajectory $u$ with input a periodic orbit $x$ of $H'$ and output a periodic orbit $y'$ in the family $\gamma$ followed a a gradient trajectory $\alpha$ flowing int a critical point $y$ of $f$.  The virtual dimension is $0$ exactly when the dimension of the unstable manifold of $y$ complements $n-i_{CZ}(x)$ which is exactly when $i_{morse}(y)=i_{CZ}(x)$. The map is given as follows. Denote by $D_u$ the Fredholm operator associated with Floer solutions whose input is $x$ and whose output is anywhere on $\gamma$. Then $\delta_x=|D_u|\otimes \cL_{y'}$. Given an orientation of $\gamma$ the latter is isomorphic to $T_{y'}W^u(y)\otimes\cL_{y'}$. The gradient trajectory $\alpha$ then gives an isomorphism 
$$
...
An \emph{isotopy of isolated isotropic components} is family $\tau\mapsto\gamma^{\tau}$ of isolated compact isotropic connected components of $1$-periodic orbits of $H^{\tau}$ such that there exists a homotopy of embeddings $f:\tau\times U\to M$ where $U$ is a manifold with boundary and $U^{\tau}:=f({\tau}\times U)$ is an exact isolating compact neighborhood of $\gamma^{\tau}$. Moreover, we assume we can choose the primitives of $\omega$ on $U^{\tau}$ so that they are locally constant in $\tau$.  Given such an isotopy define a function $\cA_t:[0,1]\to\bR$ by $t\mapsto \cA_{H^{t}}(\gamma^{t})$ where the action is defined using primitive which is locally constant in $t$.  
\end{df}
\begin{lm}\label{lmIstopyInfMap}
An isotopy of isolated isotropic components induces an isomorphism 
...
Let $K\subset M$ be a compact set with smooth boundary of positive contact type.  The aim of this section and the next two is develop analogous results to those of the previous section in the case of a Hamiltonian which is the indicator function $H_K$
\begin{equation}\label{eqHK}
H_K(x):=\begin{cases}
0&\quad x\in K,\\
\infty&\quad x\not\in K.\notag
...
\subsection{Symplectic cohomology of Reeb orbits}
...
\subsection{Proofs}
\subsubsection{Construction of the Floer diagrams}


We would like to approximate the continuous functions $H_{K,I}$ by smooth functions. 

 Fix any $\eta\ll \min\{\epsilon,\delta(a)\}$ and choose a function  $H=H_{K,I,\eta}$ such that
...
\end{enumerate}
We denote by $\cK(K,I,\eta)$ the set of all such Hamiltonians. 
 




For each $\eta\ll\epsilon$ fix once and for all an element $H_{K,I,\eta}\in\cK(K,I,\delta)$. 

We refer to $\eta$ as the \emph{smoothing parameter}. If $\eta_i\to 0$ then  $H_{K,I,\eta_i}$  converges uniformly to $H_{K,I}$. Fix once and for all a  sequence $\eta_i\to 0$ such that $K_{K,I,\eta_i}$ is monotone. 
...
If $\eta$ is small enough and $\cN$ is a small enough  neighborhood of $\cK(K,I,\eta)$ in $C^2$ then  for $\cH=\cH(\cN,\cB)$ we have that $S_{\cH}(\cU(\gamma,r))$ is isolated. 
\end{prp}

Note this does not follow from the argument of Lemma \ref{lmEpsilonSeparation} since $\cK(K,I,\eta)$ does not consist of Hamiltonians that are $C^k$ close to each other on the neighborhood of some $1$ periodic orbit of a fixed Hamiltonian. The proof of Proposition \ref{prpActionWindowComponent} will require the following observation. Fix as before a symplectization neighborhood $V\simeq\partial K\times (-\epsilon,\epsilon)$ and let $t$ be the coordinate in the $\bR$ direction. Denote by $H_1(x,t)=e^t$ the Hamiltonian whose flow is the Reeb flow in a symplectization neighborhood. Let $H_2=h\circ H_1(x,t)$ for some monotone function $h$. Let $\gamma=(\alpha,a):[0,T]\to V$ be a smooth path.  Consider the function $\tau(t):=\int_0^th'\circ a$. It is monotone, and so, invertible. For any $H$ and $t$ denote by $E_H(\gamma;t)$ the energy $\int_0^t\|\gamma'(x)-X_H\circ\gamma(x)\|dx.$
\begin{lm}\label{lmReparam}
We have
\begin{equation}
E_{H_2}(\gamma;t)=E_{H_1}(\gamma\circ \tau^{-1},\tau(t)).
\end{equation}
\end{lm}
\begin{proof}
Changing variable in the integral and using $X_{H_2}=h' X_{H_1}$, we obtain
$$
\int_0^t\|\gamma'-X_{H_2}\circ\gamma\|=\int_0^{\tau(t)}\|\gamma'\circ \tau^{-1}-s'X_{H_1}\circ\gamma\circ \tau^{-1}\|.
$$
Rearranging, the right hand side becomes $E_{H_1}(\gamma\circ \tau^{-1};\tau(t))$. 
\end{proof}
...
The proof is similar to that of Lemma \ref{lmEpsilonSeparation} with some adjustments which we spell out.  First observe can find open sets $V_0\subset V_1\subset V_2\subset V_3\subset \partial K$ so that
...
In symplectization coordinates we let $U_i=V_i\times[0,\delta]$. Denote by $\pi_i$ for $i=1,2$ the projections to $\partial K$ and to $\bR$ respectively in symplectization coordinates. By making the neighborhood $\cN$ and $\eta$ small enough, we guarantee that all the Floer trajectories in  $S_{\cH}(\cU(\gamma,r))$  have arbitrarily small energy, and have ends contained in arbitrarily small neighborhoods of $\gamma$. 

The reasoning of Hein's Lemma immediately allows to make the oscillation of $\pi_2\circ u$ arbitrarily small for $u\in S_{\cH}(\cU(\gamma,r))$. Indeed, for any fixed $\delta'>0$ monotonicity applies with uniform constants in the region $\pi_2^{-1}([\delta',\delta])$ for all Floer data in $\cH(U,r)$. The energy distance inequality applies since $a_I$ is not in the period spectrum. 

It thus remains to confine the projection $\pi_1\circ u$. The pre-image $u^{-1}\circ\pi^{-1}(V_2\setminus V_1)$ contains a connected component $S\subset\bR\times S^1$. 



Denote by $\psi_t$ the time $t$ flow under $H_1$. For any $t\in[0,a_I]$ let $d_t$ be a bound from below on distance $d(x,\psi_t(x))$ of any $x\in V_3\setminus V_0$. By our assumptions, for any $t_0>0$, $d_t$ is bounded away from $0$ on the interval $[t_0,a_I]$ Let $\delta$ be a bound from below on the distance $d(\psi_t(x),\partial (V_3\setminus V_0))$ for $x\in V_2\setminus V_1$ and for $t\in[0,a_I]$. All bounds are assumed to hold for metrics determined by any almost complex structure in $\cB$. Let $[a,b]$ be the image of $S\subset\bR\times S^1$ under projection to the first factor. We now find a constant $\delta'$ such that 
$$
E_H(u(s,\cdot))dt>\delta'. 
$$
for all $s\in[a,b]$.
For this observe that if $u_s$ does not remain inside $V_3\setminus V_0$ then using our assumptions on the flow of $H_1$,  Lemma \ref{lmReparam} and Proposition \ref{PrpDistEn} we obtain a $\delta'$ as required. It remains to deal with the case $u_s$ does remain inside. Fix a real number $t_0$ to be determined momentarily. If $\tau(1)>t_0$, we derive a uniform estimate from below on  $E_H(u(s,\cdot))$ , depending on $t_0$, using  Lemma \ref{lmReparam} and Proposition \ref{PrpDistEn} again. Suppose on the other hand that $\tau(1)<t_0$. We have an estimate
$$
E_H(\gamma)=E_{H_1}(\gamma;t_0)\geq\ell^2(\gamma)-t_0\sup\|X_{H_1}\|. 
$$
...
We conclude that $E^{geo}(u)\geq (b-a)\delta'$. To continue we observe that the monotonicity constant of Lemma \ref{lmMonEst++} is sensitive only to the $C^0$ norm of the Hamiltonian vector field. And thus, as in \ref{lmReparam} we get estimate $E^{geo}+(b-a)\geq C\delta''$. Combining the two estimates we get an a priori estimate on the energy of any trajectory associated with $\gamma$ and leaving $V_3$. 
...
\item
Given $r_1\leq r_2$ and $\cB_2\subset\cB_1$ let $\cN_1,\cN_2$ be choices of neighborhoods as in the previous part.  Then $\cN_1\cap\cN_2$ is $I$-thick since it contains  the set $\cK(K,I,\eta)$.   For the equality $S_{\cH'_2}(\cU_1)=S_{\cH'_2}(\cU_2)$ note that by choice of $\cN_2$ all Floer solutions arising from the diagram  $\cH_2$ and connecting loops in $\cU_2$ are paths in $\cU_2$ regardless of the fact that we do not impose this as a requirement. 
...
Our confinement estimates build on the monotonicity estimate of Lemma \ref{lmMonEst++} and on the energy distance estimate of Proposition \ref{EnergyDistanceSShaped}, both of which are robust in the sense that the constants are unaffected by the behaviour outside if the region under consideration. The claim follows as in the proof of Lemma \ref{lmInfSHGermDep}. 
...
\subsubsection{Behavior under isotopy}
...
Given a path $K^{\tau}$, an isotopy $\gamma^{\tau}$ of Reeb components of $K^{\tau}$ is \emph{isolated} if for each $T>0$ there is an isotopy of  compact domains $V^{T,\tau}$ isolating $\gamma^{\tau}$ in the sense that for each $\tau$ the  only periodic points of period $\leq T$ that are in the neighborhood $V^{T,\tau}$ lie on $\gamma^{\tau}$ and $\gamma^{\tau}$ is contained in the interior of $V^{T,\tau}$. 

\begin{lm}\label{lmIsotopUW}
There is an induced isomorphism $SH^*_{uw}(\gamma^0)\to SH^*_{uw}(\gamma^1)$.
\end{lm}
\begin{rem}
...
\begin{enumerate}
\item $CF^*(H_{K,I})$  filtered by the  action $\cA_{H_{K,I}}$ and continuation maps respect this filtration.
\item 
...
For the following Proposition fix a positive real number $K$.  Let $M$ be a symplectic manifold with boundary. Let $S\subset\bR\times S^1$ be a compact Riemann surface with boundary.  Let $J_z$ be an $S$-parametrized family of  almost complex structures on $M$ with absolute value of sectional curvature bounded from above by $K$ and with radius of injectivity bounded from below by $\frac1{\sqrt{K}}$. Let $H:S\times M\to\bR$ be a Hamiltonian. For a solution $u$ to Floer's equation denote by $E^{geo}(u):=\int_S\|\partial_su\|^2dtds.$

\begin{prp}\label{prpSmallDiamEst}
 Let $\gamma:[0,1]\to S$ be a geodesic. Let $u:(S,\partial S)\to (M,\partial M)$  be a solution to Floer's equation. Let $d=dist(u(\gamma(0)),u(\gamma(1))$. Let $d_0=dist(u(\gamma),\partial M)$.  Then for  $\delta=\min\left\{\frac1{d_0},\frac1{\sqrt{K}}\right\}$ we have 
\begin{equation}
d<\inf_{t\in\bR}\left(1+\frac{\|X_H\|^2}{t^2}\right)^3 \frac{10}{\delta t}\left(E^{geo}(u)+t^2\ell(\gamma)\right).
\end{equation}
\end{prp}
...
\begin{proof}
We refer to Appendix \ref{SecEstimates} for the notions introduced during the proof. For each $t$ let $\tilde{\omega}_t:=t^2\pi_1^*\omega_{\Sigma}+\pi_2^*\omega_M+dH\wedge dt$. Applying the Gromov trick to the space $S\times M$ we obtain a family of (Cauchy-)complete metrics $\tilde{g}_t:=g_{\tilde{J}_t}$. For each $t$ let $C_t:=\left(1+\frac{\|X_H\|^2}{t^2}\right)$. Then the geometry of $\tilde{g}_t$ is $C_t$ equivalent to the product metric $t^2\pi_1^*g_{\Sigma}+\pi_2^*g_M$. The product metric in turn has geometry bounded by $K_t:=\max\left \{\sqrt{K},\frac{1}{t}\right\}$. Denote by $\tilde{u}\subset S\times M$ the graph of $u$. Consider the lift $\tilde{\gamma}$ of $\gamma$. 
For any integer $N$ let $r_N=\frac{d}{2N}$.  Then for each $t$ there are at least $\lfloor N/t\rfloor$ points $s_i\in[0,1]$ such that $d_t(\tilde{u}(\gamma(s_i)), \tilde{u}(\gamma(s_{j})))\geq2tr_N$ whenever $i\neq j$. 
Let $B_i=B_{r_{N}}(\tilde{u}(\gamma(s_i)))$. Suppose now that $N$ is so that $r_N\ll \min\left \{\frac1{\sqrt{K}},\frac1{d_0}\right \}$. The monotonicity equality then gives 
\begin{equation}
\sum_i \tilde{\omega}_t(B_i) \geq \frac1{C^3_t} \lfloor N/t\rfloor t^2 r_N^2=\frac1{2C^3_t}tr_N{d}.
\end{equation}
On the other hand
\begin{equation}
\tilde{\omega}_t(B_i)= E^{geo}(u;u^{-1}(\pi_1(B_i)))+Area_t(\pi_1(B_i)).
\end{equation}
So,
\begin{equation}
\sum_i \tilde{\omega}_t(B_i) \leq E^{geo}(u)+2t\ell_t=E^{geo}(u)+2t^2\ell.
\end{equation}

Combining these, we obtain
\begin{equation}
d\leq  \frac{C_t^3}{r_N}\left(\frac1{t}E^{geo}(u)+2t\ell\right).
\end{equation}
Taking $r_N=\frac15\delta$ gives the claim. 
\end{proof}
...
If $H|_{M\setminus V}$ is sufficiently $C^1$ small there is an $\hbar>0$ depending only on the geometry of $J$ such that if $E^{rel}(u)\neq 0$ then $E^{geo}(u)>\hbar$. 
\end{prp}
\begin{proof}
...
\begin{equation}
\epsilon<\inf_t\left(1+\frac{\|X_{H_s}\|}{t}\right)^3 \frac{10}{t}\left(E^{geo}+\pi t^2\right).
\end{equation}
Taking $t_0:=\frac{\epsilon}{40\pi}$ and  $h_s$ so that $\frac{\|X_{H_s}\|}{t_0}\leq 1$ we obtain the estimate
\begin{equation}
E^{geo}(u)>\frac{t_0}{40}\epsilon. 
\end{equation} 
...
Any monotone Floer trajectory involving Hamiltonians as in the hypothesis of Lemma \ref{prpRelEn} whose input is concave and whose output is convex has energy $\leq\hbar$. 
\end{lm}
\begin{proof}
...
We use Hamiltonians satisfying the hypotheses of Lemma \ref{prpRelEn}. The claim is then immediate from  Lemmas \ref{prpRelEn} and \ref{lmBorgOanc}. 
\end{proof}
...
\item each Morse-Bott component of either $\partial K_1$ or $\partial K_2$ is either contained in $\partial K_1\cap\partial K_2$ or is disjoint of  $\partial K_1\cap\partial K_2$, 
\item there is a contact type primitive of $\omega$ near $\partial K_2$ which extends to a primitive of $\omega$ on a neighborhood of $\partial K_1$.
\end{enumerate}
\end{df}
\begin{ex}
...
The proof of Theorem \ref{tmInfSHn2} requires a version of Theorem \ref{mainThmC} for certain domains with corners. In this subsection we develop a very basic approach which builds on Theorems \ref{tmFilteredMainTm} and \ref{tmInfintsmlSHFunct}. A more complete approach, which would suffer from less limitations, is to prove versions of these theorems from the start for appropriate domains with corners. This will be pursued elsewhere. 

\begin{df}\label{dfAdmSm}
Consider an inclusion $K^0\subset K^1$ of compact sets with contact boundary.  Suppose there is an interpolating family $\tau\mapsto K^\tau$  and a choice $\alpha$ of a primitive of $\omega$ in a connected open neighborhood containing $K^{\tau}$ for all $\tau\in[0,1]$  such that the following hold
\begin{enumerate}
...
\item The set of primitive periodic Reeb orbits  decomposes for each $\tau$ into a disjoint union of path connected isotropic components.
\item For each $T$ the set of Reeb components of period $\leq T$ is \emph{isolated uniformly in $\tau$}. This means  there is a continuous  isotopy $\tau\mapsto U^{T,\tau}$ of embedded domains decomposing into a disjoint union of isotopies of connected components  $U^{T,\tau}(\gamma^{\tau})$ with the property that for each $\tau$ the interior of $U^{T,\tau}(\gamma^{\tau})$  is an isolating neighborhood of a unique primitive isotropic connected Reeb  component $\gamma^{\tau}$ and moreover, each Reeb component of period $\leq T$ is contained in  $U^{T,\tau}$. 
\end{enumerate} 
\end{df}



Denote by $\cR$ a set parametrizing the Reeb components, and for each $\gamma\in\cR$ denote by $\gamma^{\tau}$ the corresponding Reeb component of  $\partial K^{\tau}$. 
...
Suppose the limiting periods are all distinct and let $\delta(a)$ be the infimum. Then
\begin{equation}
...
To discuss functoriality we need the notion of compatible smoothings. Given $K_1\subset K_2$ we say that a pair $\{K_1^{\tau}\}\subset \{K_2^{\tau}\}$ of smoothings is \emph{compatible} if for each $\tau$  the inclusion $K_1^{\tau}\subset K_2^{\tau}$ is admissible, and for each action $a$ with respect to some fixed primitive, the set of common components stabilizes as $\tau\to\infty$. We call the intersection over $\tau$ of the common components for $K_1^{\tau}\subset K_2^{\tau}$ the common components for $K_1\subset K_2$. 
...
\begin{lm}
Let $\partial K$ carry a Lagrangian torus fibration, and let the Reeb orbit $\gamma$ be a torus fiber. Assume $TM|_{\gamma}$ is trivialized so that the Robin Salamon index of $\gamma$ is $0$. Then $SH^*_0(\gamma)=H^*(T^*\bT^n;R)$. 
\end{lm}
\begin{proof}
...
We define a category $\cP$ of compact sets $P\subset B$ with morphisms inclusions. We also consider the category $PMon$ of \emph{partial monoids} over $\bZ$. We define a contravariant functor $Trop: \cP\to PMon$  as follows. 
...
\end{equation}
where we identify the generator of $C_{e_i}\subset  C_{i-1,i}$ with $C_{e_i}\subset C_{i,i+1}$. Given an admissible inclusion $Q\subset P$  we define a map $f:Edges(P)\to Edges (Q)\cup\{*\}$ by $f(i)=*$ if the $i$th edge of $P$ does not contain any edge of $Q$, and by $f(i)=j$ for $\partial_jQ$ the unique edge contained in $\partial_i P$ otherwise. We then get an induced map of partial monoids $P_{trop}\to Q_{trop}$ by mapping the generator associated with $\partial_iP$ to the generator associated to $\partial_{f(i)}Q$ if $f(i)\neq *$ and to $0$ otherwise.
...
\begin{lm}
Compatible good convex smoothings exist. 
\end{lm}
...
The symplectic cohomology $SH^*_M(K)$ is a module over the Novikov ring $\Lambda_{\geq 0}$. As such it carries a valuation defined by $\val(a)=\sup\{r\in\bR_{\geq 0}:a\in T^rSH^*_M(K)$. We define a semi-norm by $|a|=e^{-\val(a)}$. Note the semi-norm makes sense even for torsion elements though it is only submultiplicative with respect to scalar multiplication for torsion elements. 

For $a\in SH^*_M(K)$ the statement $|a|\leq t$ implies that $a$ is in the image of $SH^*_{M,[-\infty,t+\epsilon)}\to SH^*_M(K)$ for all $\epsilon>0$. 

Suppose now that  $SH^*_M(K)$ is torsion free. It then follows that there is a unique element in $SH^*_{M,t^+}(K)$ that maps to the image of $a$ in the quotient $SH^*_{M,[t,\infty)}$.  We thus get for each $t\in\bR$ a map
\begin{equation}
\tilde{\sigma}_t: SH^*_{M,[-\infty,t]}(K)\to SH^*_{M,t^+}(K),
\end{equation} 
where
$$
 SH^*_{M,[-\infty,t]}(K):=\varprojlim_{\epsilon\to 0} SH^*_{M,[-\infty,t+\epsilon)}(K).
$$
Combining this with the map from Theorem \ref{mainThmC} we obtain the 
\begin{equation}
\sigma_t: SH^*_{M,[-\infty,t]}(K)\to RH^*(K).
\end{equation} 
We call $\sigma_t(x)$ the \emph{$t$-symbol} of $x$. 

From now on we assume $SH^*_M(K)$ is torsion free.  
...
Let $\epsilon>0$ and let $B\subset SH^*_M(K)$ be a collection of $\epsilon$ good elements. Suppose the set $\{\sigma(x):x\in B\}$ generates $RH^*(K)$. Then $B$ generates $SH^*_M(K)$ as a module over the Novikov field. 
\end{lm}
\begin{proof}
Since we are working over the Novikov field, we may assume without loss of generality that the elements of $B$ are normalized. For any $a$ denote by $V_a=\{x\in SH^*_M(K):|x|<e^a\}$. Then we claim that torsion freedom implies
\begin{equation}
V_b/V_a=SH^*_{M,[a,b)}(K). 
\end{equation}
First we verify that we have a well defined map. Given an element $x$ of $V_b$ we get an element on the right hand side by picking a representative $y$ of $x$ of norm $<b$. For any other representative $y'$ we must have that $y-y'$ can be killed by an element of norm $<b$ or otherwise we could produce a non-zero torsion element.  So we get a well defined map from $V_b$.  This map is norm preserving by the same reasoning.  In particular, the submodule $V_a$ maps to $0$. The map is thus well defined. 
...
For this we observe that for any action $a$ we can construct a non-canonical isomorphism the group $F^aSH^*_{M,[0,\hbar)}(K)\simeq F^aRH^*(K)\otimes\Lambda_{[0,\hbar)}$ in a filtration and leading term preserving manner.  More precisely,   there is an isomorphism 
...
\begin{df}
A Lagrangian sub-manifold $L\subset M$ is called \emph{undeformed} if there is a  Weinstein neighbourhood $K\subset M$ of $L$ such that, denoting by $\overline{K}$ the Liouville completion, $SH_{\overline{K}}(K)$ is torsion free and  there exists an isomorphism of BV algebras $SH^*_M(K)\simeq SH_{\overline{K}}(K)$ which respects spectral symbols. 
\end{df}
\begin{rem}
...
Let $\pi:M\to B$ and $P\subset B$ be as in Theorem \ref{tmInfShPtrop}. For $P\subset B_{reg}$ a convex polygon denote by $\cF^*_{loc}(P)$ the relative $SH$ of $\pi^{-1}(P)$ inside its Liouville completion. Explicitly, fix a base-point $p$ in $P$ and  $L_p:=\pi^{-1}(p)\simeq \bT^2$. We can canonically identify $P$ with a subset of $H^1(L_p;\bR)$ so that $p$ maps to the origin. There we can lift this to an embedding of $\pi^{-1}(P)$ into $T^*L_p$ and, abbreviating $B_{loc}:=H^1(L_p;\bR)=(H_1(L_p;\bZ)\otimes\bR)^*$, extend $\pi$ to a map $\pi_{loc}:T^*L_p\to B_{loc}$. Picking a basis $\{e_1,e_2\}$ for $H_1(L_p;\bZ)$ we get an integral affine isomorphism $B_{loc}\simeq\bR^2$. Consider the algebraic torus $mSpec(\Lambda[ H_1(L_p;\bZ)])\simeq {(\Lambda^*)^2}$ over the Novikov field. It comes equipped with a log map to $B_{loc}$ which upon picking a basis is the map $f:(\Lambda^*)^2\to\bR^2$ given by $(x,y)\mapsto(\val(x),\val(y))$. 

Picking a basis,  $\cF^0_{loc}$ is isomorphic as a sheaf over the Novikov field to the push-forward $f_*\cO_{(\Lambda^*)^2}$ \cite[\S5]{GromanVarolgunes2022}. Let us describe this geometrically. Say the polygon $P$ is cut out by rational half plains $P_i$. For each $i$, let $\ell_i=\langle a,\cdot\rangle +b$ be the defining affine function of $P_i$, where $a\in H_1(L_p;\bZ)$ is a primitive generator and $b\in\bR$. Denote the corresponding monomial by $T^bu^a$ there is a monomial $u_i\in\Lambda[ H_1(L_p;\bZ)])$ which corresponds to the primitive lattice element in  $H_1(L_p;\bZ)]$. Then for each $i$, the pre-image $f^{-1}(P_i)$ is the set of points where $|T^bu^a|\leq1$, or equivalently $|u^a|<e^b$. Thus $\cF^0_{loc}(P)$ is the algebra of analytic function on the non-Archimedean toric domain cut out by the inequalities $|u^{a_i}|<e^{b_i}$. Similarly, it is shown  in \cite[\S5]{GromanVarolgunes2022}  that $\cF^*_{loc}$ is canonically isomorphic to the sheaf of polyvector fields on the analytification of the algebraic torus over the Novikov field. Note that this identification of $\cF^0(P)$ involves a choice of base-point. This corresponds to choosing a primitive of the symplectic form on the completion of $\pi^{-1}(P)$. We generally work with not with the generators $u^a$ but rather with the generators $z^a:=T^bu^a$. That is our generators always have norm $1$. This corresponds to the fact that our construction of Hamiltonian Floer cohomology over the Novikov field assigns to each generator the norm $1$ since primitives exist only in special situations. 

\begin{tm}\label{tmAffinoidPolygon}
Let $\pi:M\to B$ and $P\subset B$ be as in Theorem \ref{tmInfShPtrop}. Suppose the fibers of $\pi$ are undeformed. For each $i$ let $\xi_i\in P_{trop}(\bZ)$ be the generator associated with the $i$th face. Let  $z_i\in \cF_P$ be a normalized element whose leading symbol is $\xi_i$. Then $\cF_P$ is generated as a Banach algebra by the $z_i$. In particular, $\cF_P$ is affinoid and of dimension $n$.  Moreover, for an admissible inclusion $Q\subset P$  the restriction map $\cF_P\to\cF_Q$ is the inclusion of a Laurent domain provided $Q$ is contained in a sufficiently small neighborhood of the boundary. 

\end{tm}
\begin{proof}
First observe $\cF^0(P)$ is torsion free by Lemma \ref{lmTorsCrit}. By finiteness there is an $\epsilon\in(0,\hbar)$ so that the $z_i$ are all $\epsilon$-good.  Since the product preserves the action filtration modulo $T^{\epsilon}$ it follows that all monomials in  the $z_i$ are $\epsilon$-good. We now show that for each $i,j$ the monomial $z_i^j$  has leading symbol $\xi_i^j$ and that for adjacent edges $i_1,i_2$ and integers $j_1,j_2\geq 0$ the product $z_{i_1}^{j_1}z_{i_2}^{j_2}$ has leading symbol $\xi_{i_1}^{j_1}\xi_{i_2}^{j_2}$.  
...
We need to show that $h$ is an isomorphism. Injectivity we prove in Lemma \ref{lmUniqueContinuation}. For surjectivity,  let $u_1,u_2,u_3,u_4$ be the images under $h$ of the generators corresponding to the three common edges of $Q$ and $P$ , and  of $X$ respectively. We take the generators to be cyclically ordered in the obvious way. Let $B$ the set of monomials in these elements with nonnegative powers where we only consider products of adjacent generators. Using Lemma \ref{lmEpsGoodGenCrit} we see that the submodule $V$ generated by $B$ is dense in $\cF(Q)$. To conclude surjectivity it suffices to observe that on the set $B$, the map $h$ precisely preserves norms. In particular the closure of $V$ is in the image of $h$. 

Fix now a neighborhood of $\partial P$ which the union of the polygons $Q_i$ cut out by lines which are parallel and sufficiently close to $e_i$. Then the claim for polygons in such a neighborhood follows by our knowledge of $\cF_{loc}$ and by transitivity of Laurent domains. 
...
defined by 
$$
\{v,f\}:=\Delta(v*f)+(\Delta v)*f-v*\Delta .
$$
Here $\Delta$ denotes the BV operator and $*$ the pair of pants product. 
In the cases we care about $SH^*$ is supported in positive degrees, so the formula simplifies to 
$$
...
Theorem \ref{tmLocSHn2} follows from Theorem \ref{tmAffinoidPolygon} and Lemma \ref{lmInjPoly}. Theorem \ref{tmLocSHn3} follows from Lemma \ref{lmPolyvectPolygon}.
\end{proof}
...
In this section we recall $C^0$ estimates on Floer trajectories in the small regime. These are enhancements of estimates by \cite{Usher09} and \cite{Hein}. 
