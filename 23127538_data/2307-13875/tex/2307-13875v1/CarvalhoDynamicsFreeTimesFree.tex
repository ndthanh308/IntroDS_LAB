...
Center for Mathematics and Applications (NOVA Math), NOVA SST
...
\newtheorem*{uccmp}{Corollary \ref{uccmp}}
\begin{uccmp}
...
In \cite{[Car22a]}, something stronger was proven for automorphisms of $\Z^m\times F_n$: for a  uniformly continuous automorphism, every point in the completion must be either periodic or wandering. This shows that non-periodic points, when iterated long enough wander away from some neighborhood \emph{carrying} the neighborhood with them. In particular, since $\omega$-limit sets are nonempty, they must be periodic orbits.

In this paper, we prove the same result for automorphisms of $F_n\times F_m$:
\newtheorem*{wandper}{Theorem \ref{wandper}}
\begin{wandper}
Let $\Phi\in \Aut(F_n\times F_m)$, $d$ be the product metric given by taking the prefix metric in each factor and let $\widehat\Phi$ be its continuous extension to the completion with respect to $d$. Then every point in $\widehat{F_n\times F_m}$ is either wandering or periodic. 
\end{wandper}

It follows in particular that the dynamics of automorphisms of $F_n\times F_m$ is asymptotically periodic.
...
\subsection{Endomorphisms of $F_n\times F_m$} 
 ...
 For $(i,j)\in[n]\times [m]$, we define   $\lambda_i:F_n\to \mathbb Z$ as the endomorphism given by $a_k\mapsto \delta_{ik}$ and $\tau_j:F_m\to \mathbb Z$ given by $b_k\mapsto \delta_{jk}$, where $\delta_{ij}$ is the Kronecker symbol.
...
We will often use the above theorem for \emph{affine} transformations. It is seen in \cite{[CD23]} that this is not a problem, as an affine transformation can be seen as a restriction of a linear one. 
\end{remark}
...
When we consider a hyperbolic group $G$ endowed with a visual metric $d$ and take its completion, we obtained the \emph{Gromov completion} of $G$. As said above, the endomorphisms of $G$ admitting a continuous extension to the completion are precisely the uniformly continuous ones (with respect to $d$). It is proved in \cite{[Car22c]} that uniformly continuous endomorphisms of a hyperbolic group satisfy a \emph{H\"older condition}. An endomorphism $\phi$ satisfies a H\"older condition with constants $(K,r)$ if 
$$d(x\phi,y\phi)\leq Kd(x,y)^r.$$ If $\phi$ satisfies a H\"older condition, then so does its continuous extension to the completion, by \cite{[AS16]}.



We now present some standard dynamical definitions that will be useful to the classification of infinite points. A point $x\in G$ is said to be a \emph{$\varphi$-wandering point} if there is a neighbourhood $U$ of $x$ and a positive integer $N$ such that for all $n>N$, we have that $U\varphi^n\cap U=\emptyset.$ A point $x\in G$ is said to be a \emph{$\varphi$-recurrent point} if, for every neighbourhood $U$ of $x$, there exists $n>0$ such that $x\varphi^n\in U$. 
Now, let $f$ be a homeomorphism of a compact space $K$. Given $y\in K$, the \emph{$\omega$-limit set} $\omega(y,f)$, or simply $\omega(y)$, is the set of limit points of the sequence $f^n(y)$ as $n\to + \infty$. We say that the dynamics is \emph{asymptotically periodic} if every $\omega$-limit set is a periodic orbit (see \cite{[LL08]}). A point is recurrent if it belongs to its own $\omega$-limit set.
...
We have that 
\begin{align}\label{implicas}
\text{periodic}\implies \text{recurrent} \implies \text{nonwandering}.
\end{align}

It is proved in \cite[Proposition 5.10]{[Car22a]} that, if $\phi$ is a uniformly continuous endomorphism of a free group, every nonwandering point of $\widehat \phi$ is recurrent. In fact, the proof only uses the fact that $\widehat\phi$ satisfies a H\"older condition. So the following more general statement, which can be found in \cite[Proposition 8.1.29]{[Car23e]}, holds:

\begin{proposition}\label{holder nonwandering recurrent}
Let $(X,d)$ be a metric space and $\phi:(X,d)\to (X,d)$ be a H\"older mapping. Then every nonwandering point is recurrent.
\end{proposition}


If the dynamics is asymptotically periodic, since recurrent points belong to their own $\omega$-limit set and these are periodic orbits, then recurrent points are periodic. So if the dynamics is asymptotically periodic and the mapping is H\"older, all implications in (\ref{implicas}) are in fact equivalences. Conversely, if all implications in (\ref{implicas}) are equivalences, since every point in a $\omega$-limit set must be nonwandering, it must be periodic and we have that the dynamics is asymptotically periodic. This dichotomy wandering/periodic holds for automorphisms of free-abelian times free groups (see \cite{[Car22a]}) and \emph{some} (defined in a precise way) uniformly continuous endomorphisms of free-abelian times free groups. It is not known to the author if this holds for general uniformly continuous endomorphisms (injective and not surjective) of free groups.
...
Since the word problem and the conjugacy problem are decidable in $F_n\times F_m$, we will prove that we can decide the existence of a positive $k$ and that is equivalent to deciding if there is a nonnegative value of $k$ such that $(x,y)\Phi^k=(z,w)$ (or $(x,y)\Phi^k\sim(z,w)$, in the conjugacy case). 
...
which can be done by \cite{[KL86]}. 

In the conjugacy case, we start by checking if $k=0$ is a solution to our problem by solving the conjugacy problem. If not, using Lemma \ref{decide  conjugate power}, we check if $z$ is conjugate to a power of $u$ and $w$ is conjugate to a power of $v$.  If in any of the cases the answer is no, then we will not have a positive solution to our problem; if both answer yes, we compute the unique exponents $(a,b)\in \N^2$ such that $(z,w)\sim(u^a, b^v)$. Then we simply check if there is some $k$ such that $(x,y)\Phi^k=(u^a,v^b)$ using the equality algorithm above.
...
By \cite{[KL86]}, we can decide if there is some $k$ such that   
...
We will show that TCP($F_n\times F_m$) and $\tsBrCP(F_n\times F_m)$ are decidable with respect to monomorphisms, which, using Logan's techniques implies that the conjugacy problem is decidable for ascending HNN-extensions of the direct product of two free groups. Notice that the free-abelian times free case was already dealt with in \cite{[CD23]}. We recall that it is proved in \cite[Proposition 3.2]{[Car23b]} that injective endomorphisms of $F_n\times F_m$ are precisely the types VI and VII endomorphisms having injective component homomorphisms $\phi$ and $\psi$.
...
We start by using \cite[Proposition 4.4]{[Log22]} to check whether there is a pair $(r_1,s_1)$ such that $x\phi^{r_1}\sim z\phi^{s_1}$. If there is none, then we answer \texttt{NO}. If there is such a pair, we compute the minimal $r_1$ for which there is some $k\in \N$ such that $x\phi^{r_1}\sim z\phi^{k}$. This is possible by \cite[Lemma 4.1]{[Log22]}: we keep increasing the candidate values $r_1$ until the algorithm in \cite[Lemma 4.1]{[Log22]} answers \texttt{YES}. When it does, we compute the minimal $s_1$ such that $x\phi^{r_1}\sim z\phi^{s_1}$, which can be done by iteratively solving the conjugacy problem.
 ...
 We claim that the set $\mc S$ of all pairs $(p,q)\in \N^2$ such that $x\phi^{p}=z\phi^{q}$ is precisely
 \small{
$$\begin{cases}
...
Proceeding analogously, we compute the set $\mc S'\subseteq \N^2$ of pairs $(p,q)\in \N^2$ such that $y\phi^{p}=w\phi^{q}$. These are semilinear sets, so their intersection can be computed (in particular its emptyness can be decided) and the solution set for our problem is precisely $\mc S\cap \mc S'$.
...
\end{proposition}
\noindent\textit{Proof.} Suppose that $\varphi$ is not trivial nor injective. Then there are some $x,y\in F_n$ such that $x\varphi=1$ and $y\varphi\neq 1$. For all $C\in \N$, we have that 
\begin{align*}
...
Coarse-median preserving endomorphisms of $F_n\times F_m$ are precisely the uniformly continuous ones.
\end{theorem}
\noindent\textit{Proof.} Let $\Phi\in \End(F_n\times F_m)$ be a coarse-median preserving endomorphism  with constant $C$.
...
>\;&C
\end{align*}
which contradicts the fact that $\Phi$ is coarse-median preserving with constant $C$. So, $1\in X\implies X=\{1\}$. The same holds for $Y, \,Z$ and $W$, and that can be seen analogously.
...
Finally, if $\Phi$ is a type VII endomorphism defined as $(x,y)\Phi=(y\psi,x\phi)$, then, for all $(x_1,x_2),(y_1,y_2),(z_1,z_2)\in F_n\times F_m$ and $C>0$, we have that 
\begin{align*}
&d(\mu((x_1,x_2),(y_1,y_2),(z_1,z_2))\Phi,\mu((x_1,x_2)\Phi, (y_1,y_2)\Phi,(z_1,z_2)\Phi))\leq C\\
\iff\; &d((\mu_1(x_1,y_1,z_1),\mu_2(x_2,y_2,z_2))\Phi,\mu((x_2\psi,x_1\phi), (y_2\psi,y_1\phi),(z_2\psi,z_1\phi)))\leq C\\
\iff\; & d((\mu_2(x_2,y_2,z_2)\psi,\mu_1(x_1,y_1,z_1)\phi),(\mu_1(x_2\psi,y_2\psi,z_2\psi),\mu_2(x_1\phi,y_1\phi,z_1\phi)))\leq C\\
\iff\; & \begin{cases}
d((\mu_2(x_2,y_2,z_2)\psi,\mu_1(x_2\psi,y_2\psi,z_2\psi))\leq C\\
d(\mu_1(x_1,y_1,z_1)\phi,\mu_2(x_1\phi,y_1\phi,z_1\phi))\leq C
\end{cases}.
\end{align*}
It follows, as in the type IV case, that $\Phi$ is coarse-median preserving if and only if both $\phi$ and $\psi$ are. Since, in view of Proposition \ref{cmphomo} this is equivalent to having  injective or trivial $\phi$ and $\psi$. These are precisely the uniformly continuous type VII endomorphisms of $F_n\times F_m$ (see \cite[Proposition 6.2]{[Car23b]}).
\qed\\
...
\begin{theorem}\label{wandper}
Let $\Phi\in \Aut(F_n\times F_m)$, $d$ be the product metric given by taking the prefix metric in each factor and let $\widehat\Phi$ be its continuous extension to the completion with respect to $d$. Then every point in $\widehat{F_n\times F_m}$ is either wandering or periodic. 
\end{theorem}
\noindent\textit{Proof.} Let $\Phi\in \Aut(F_n\times F_m)$ be a type VI automorphism of $F_n\times F_m$. Then $\Phi$ is of the form $(x,y)\mapsto (x\phi,y\psi)$, for some $\phi\in \Aut(F_n)$ and $\psi\in \Aut(F_m)$. By \cite[Proposition 6.2]{[Car23b]}, $\Phi$ is uniformly continuous with respect to the metric $d$ and, by uniqueness of the extension $\widehat\Phi$ is given by $(x,y)\mapsto (x\widehat\phi, y\widehat\psi)$. Let $(x,y)\in  \widehat{F_n}\times \widehat{F_m}$ be a nonwandering point. Then, 
$$\forall \varepsilon >0 \; \forall N\in \N\; \exists n>N:\;\; B((x,y), \varepsilon)\widehat\Phi^n \cap B((x,y), \varepsilon) \neq \emptyset.$$
Rewriting, we obtain that 
$$\forall \varepsilon >0 \; \forall N\in \N\; \exists n>N \;\exists (z,w)\in \widehat{F_n}\times \widehat{F_m}:\;\; \begin{cases} d((x,y),(z,w))<\varepsilon \\ d((x,y),(z,w)\widehat\Phi^n)<\varepsilon\end{cases},$$
which is the same as saying that 
$$\forall \varepsilon >0 \; \forall N\in \N\; \exists n>N \;\exists (z,w)\in \widehat{F_n}\times \widehat{F_m}:\;\; \begin{cases} d_1(x,z)<\varepsilon \\ d_1(x,z\widehat\phi^n)<\varepsilon \\ d_2(y,w)<\varepsilon \\ d_2(y,w\widehat\psi^n)<\varepsilon\end{cases}.$$

In particular, the above condition yields that $x$ is $\widehat\phi$-nonwandering and $y$ is $\widehat\psi$-nonwandering. By \cite[Corollary 5.11]{[Car22a]}, $x$ must be $\widehat\phi$-periodic and $y$ must be $\widehat\psi$-periodic, and so $(x,y)$ is $\widehat\Phi$-periodic.
...
We start by proving that $\widehat \Phi$ satisfies a H\"older condition. Notice that, since $\phi$ and $\psi$ are uniformly continuous with respect to $d_1$, then $\widehat\phi$ and $\widehat\psi$  satisfy a H\"older condition. Let $(K_1,r_1)$ and $(K_2,r_2)$ be the constants for $\widehat\phi$ and $\widehat\psi$, respectively.

 We have that, for all $(x,y), (z,w)\in \widehat{F_n}\times\widehat{F_n}$, 
\begin{align*}
d((x,y)\widehat\Phi,(z,w)\widehat\Phi)&=d((y\widehat\psi,x\widehat\phi), (w\widehat\psi,z\widehat\phi))\\
&=\max\{d_1(y\widehat\psi,w\widehat\psi),d_1(x\widehat\phi,z\widehat\phi)\}\\
&\leq \max\{K_2d_1(y,w)^{r_2},K_1d_1(x,z)^{r_1}\}\\
&\leq \max\{K_1,K_2\}\max\{d_1(y,w),d_1(x,z)\}^{\min\{r_1,r_2\}}\\
&= \max\{K_1,K_2\}d((x,y),(z,w))^{\min\{r_1,r_2\}}.
\end{align*}
We remark that the last inequality holds since $d_1(x,z), d_1(y,w)\leq 1$, by definition of the prefix metric.

  Let $(x,y)\in  \widehat{F_n}\times \widehat{F_m}$ be a nonwandering point. Then, by Proposition \ref{holder nonwandering recurrent}, $(x,y)$ must be recurrent and so there is a sequence $(n_i)_{i\in \N}$ such that $(x,y)\widehat\Phi^{n_i}\to (x,y)$. If $(n_i)$ has infinitely many even numbers, then consider the subsequence $(n_{k_i})_{i\in \N}$ of even numbers such that  $(x,y)\widehat\Phi^{n_{k_i}}\to (x,y)$. This means that 
  $$\left( x(\widehat\phi\widehat\psi)^{\frac{n_{k_i}}{2}},y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}}{2}}\right)\to (x,y),$$ and so 
 $x(\widehat\phi\widehat\psi)^{\frac{n_{k_i}}{2}}\to x$ and $y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}}{2}}\to y$, which implies that $x$ is $\widehat\phi\widehat\psi$-recurrent and $y$ is $\widehat\psi\widehat\phi$-recurrent. Since recurrent elements of extensions of free group endomorphisms must be periodic by \cite[Corollary 5.11]{[Car22a]}, then there are $p_1,p_2\in \N$ such that $x(\widehat\phi\widehat\psi)^{p_1}=x$ and $y(\widehat\psi\widehat\phi)^{p_2}=y$. Hence, $$(x,y)\widehat\Phi^{2p_1p_2}=(x(\widehat\phi\widehat\psi)^{p_1p_2},y(\widehat\psi\widehat\phi)^{p_2})=(x,y),$$
that is $(x,y)$ is $\widehat\Phi$-periodic.  
  
  If $(n_i)$ has only finitely many even numbers, then it must have infinitely many odd numbers and we consider the subsequence $(n_{k_i})_{i\in \N}$ of odd numbers such that  $(x,y)\widehat\Phi^{n_{k_i}}\to (x,y)$.
  This means that 
  $$\left( y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}-1}{2}}\widehat\psi,x(\widehat\phi\widehat\psi)^{\frac{n_{k_i}-1}{2}}\widehat\phi\right)\to (x,y),$$ and so 
$ y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}-1}{2}}\widehat\psi\to x$ and 
$ x(\widehat\phi\widehat\psi)^{\frac{n_{k_i}-1}{2}}\widehat\phi \to y$.
Let $\varepsilon>0$. 
It follows that 
\begin{align}\label{c1}
\exists N\in \N \; \forall i\geq N\;\; d_1(x(\widehat\phi\widehat\psi)^{\frac{n_{k_i}-1}{2}}\widehat\phi ,y)\leq \frac \varepsilon 2
\end{align}
and since $(\widehat\phi\widehat\psi)^{\frac{n_{k_N}-1}{2}}\widehat\phi$ is uniformly continuous, there is some $\delta$ such that 
\begin{align}\label{c2}
\forall z\in \widehat{F_n}\;\; d_1(x,z)\leq \delta \implies d_1(x(\widehat\phi\widehat\psi)^{\frac{n_{k_N}-1}{2}}\widehat\phi,z(\widehat\phi\widehat\psi)^{\frac{n_{k_N}-1}{2}}\widehat\phi)\leq \frac\varepsilon 2
\end{align}
Since $ y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}-1}{2}}\widehat\psi\to x$,
\begin{align}
\exists N_2\in \N \; \forall i\geq N_2\;\; d_1(y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}-1}{2}}\widehat\psi ,x)\leq  \delta,
\end{align}
which, by (\ref{c2}) implies that 
\begin{align}\label{c3}
\forall i\geq N_2\;\; d_1(y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}-1}{2}}\widehat\psi(\widehat\phi\widehat\psi)^{\frac{n_{k_N}-1}{2}}\widehat\phi ,x(\widehat\phi\widehat\psi)^{\frac{n_{k_N}-1}{2}}\widehat\phi)\leq \frac \varepsilon 2.
\end{align}
Combining (\ref{c1}) and (\ref{c3}), we get that 
\begin{align*}
\forall i\geq N_2\;\; d_1(y(\widehat\psi\widehat\phi)^{\frac{n_{k_i}+n_{k_N}}{2}} ,y)\leq  \varepsilon,
\end{align*}
thus $y$ is $(\widehat\psi\widehat\phi)$-recurrent and so $(\widehat\psi\widehat\phi)$-periodic. Analogously, we deduce that  $x$ is $(\widehat\phi\widehat\psi)$-periodic. Proceeding as above, we obtain that $(x,y)$ is $\widehat\Phi$-periodic.
\qed\\
