...
In Sec.\ \ref{sec:covariant-ADM} we present the spacetime covariant formulation and the spatially covariant ADM formulation of Maxwell's equations. In Sec.\ \ref{sec:Maxwell} we present the main result, presenting Maxwell's equations in four different definitions of the EM fields. Section \ref{sec:generic} presents the equations in the generic frame and proves the inevitable nature of the gravity-modified homogeneous part. In Sec.\ \ref{sec:Discussion}, we discuss the result. %Appendix \ref{sec:potential} present the relations between the EM fields and the potential in the normal frame.
Appendix \ref{sec:FNLE}, presents the ADM metric variables in terms of variables associated with $\delta_{ij}$ and its inverse using the FNLE formulation. Appendix \ref{sec:linear} presents relations to the linear order metric perturbation which will be useful to handle electrodynamics coupled with gravitational waves and the weak gravity.
...
We use spatially covariant formulation of the curved spacetime. The ADM metric and its inverse are \cite{ADM}
\bea
...
\bea
   & & \widetilde \eta_{0ijk} = - N \overline \eta_{ijk}
...
The two non-covariant definitions are not based on the observer's four-vector. The EM fields are directly read as components of $F_{ab}$ or $F^*_{ab}$, both with covariant indices. It turns out that, in this way, one cannot identify the frame four-vectors allowing such definitions of the EM fields. The situations are troublesome because without the observer's four-vectors specified, one cannot introduce the charge and current densities associated with the respective EM fields. Without the associated four-vector identified, the EM variables introduced in non-covariant manner are not the EM fields that can be measured by any observer.
...
In the literature, we often find a frame choice with $u^i \equiv 0$ \cite{Wilson-Mathews-2003, Baumgarte-Shapiro-2010, deFelice-1971}. This corresponds to the coordinate frame where the observer is attached with the coordinate. In the coordinate frame, we have
\bea
...
\bea
   ...
   \label{Fab-breve}
\eea
where indices of ${\hat E}_i$, ${\hat B}_i$, ${\breve E}_i$, and ${\breve B}_i$ are raised and lowered using $\delta_{ij}$ and its inverse. Notice that these definitions of EM fields, directly associating the physical EM fields to components of tensors, are {\it not} covariant; definition in Eq.\ (\ref{Fab-hat}) is also used in \cite{Cooperstock-1968, Baroni-1985}. The merit of these two definitions, however, noticed by Einstein \cite{Einstein-1916} is that in these ways, the second part of Eq.\ (\ref{Maxwell-tensor-eqs}) using Eq.\ (\ref{Fab-hat}) and Eq.\ (\ref{Hab*-eq}) using Eq.\ (\ref{Fab-breve}), in the absence of the external current four-vector, naturally gives the homogeneous part of Maxwell's equations in exactly special relativistic form as
\bea
   & & {\breve E}^i_{\;\;,i}
       = {1 \over c} \sqrt{-g} J^0,
...
\eea
This set is a combination of parts of two sets of Maxwell's equations associated with two different definitions of the EM fields in Eqs.\ (\ref{Fab-hat}) and (\ref{Fab-breve}). The more complicated complementary parts are not written, see below.
...
\bea
   ...
   \label{Maxwell-hat-4}
\eea
where $\hat {\bf P}_{\rm E}$ and $\hat {\bf M}_{\rm E}$ are defined in Eq.\ (\ref{EB-relation}). The effective ${\bf P}$ and ${\bf M}$ appear only in the inhomogeneous part of Maxwell's equations \cite{Cooperstock-1968, Baroni-1985}, and in such a case the effect of gravity can be interpreted as the effective medium property.
...
The above two non-covariant definitions are widely used in the literature of interpreting the effect of gravity as a medium property \cite{Skrotskii-1957, Balazs-1958, Plebanski-1960, deFelice-1971, Leonhardt-2006}. For a similar non-covariant definition of the EM fields in a spatially covariant form, using an alternative spacetime decomposition compared with Eq.\ (\ref{ADM-metric}), see Sec.\ 115 of \cite{Moller-1952} and Sec.\ 90 of \cite{Landau-Lifshitz-1975}. In these non-covariant definitions one cannot identify the four-vectors of corresponding observers who may measure such EM fields. Consequently, one cannot introduce the corresponding charge and current densities.
...
We will show that in the curved spacetime, the non-covariant definitions of the EM fields in Eqs.\ (\ref{Fab-hat}) or (\ref{Fab-breve}) is not possible for any observer. As a consequence, we will show that gravity inevitably causes modifications in both the homogeneous and inhomogeneous equations. From Eqs.\ (\ref{Fab-cov}) and (\ref{Hab-cov}), we have
\bea
   & & \hskip -.8cm
       F_{0i}
...
To the linear order in the metric perturbations and $V_i$, we have
\bea
   & & F_{0i} = - N \overline E_i
...
\eea
Thus, even to the linear order in metric perturbations, one {\it cannot} remove the metric dependence of $F_{ab}$ by choosing $V_i$. %This conclusion is independent of the gauge conditions. For example, synchronous gauge conditions with $N = 1$ and $N_i = 0$, still leads to dependence on $\sqrt{\overline h}$ and $\overline h{}^{ij}$ through $\overline \eta_{ijk}$ and $\overline B{}^i$.

Therefore, the non-covariant definitions of the EM fields introduced in \cite{Moller-1952, Cooperstock-1968, Landau-Lifshitz-1975, Baroni-1985, Berlin-2022, Domcke-2022}, na\"ively assigning the EM fields to components of the field strength tensor as in Eq.\ (\ref{Fab-hat}) cannot be regarded as any measurable EM fields. That is, as these variables cannot be measured as EM fields by any observer with their own four-vector, these are disqualified as the EM field variables. We can similarly show the non-covariant definition in Eq.\ (\ref{Fab-breve}) is not allowed.
...
The two non-covariant definitions of EM fields lead to trouble as these fail to identify the observer's four-vectors, who may measure such fields, thus failing to identify the corresponding external charge and current densities, which also depend on the same four-vector (observer). This trouble was noticed even in the weak gravity case \cite{Hwang-Noh-2023-EM-definition}. Here, we prove that such definitions are {\it not} possible for the EM fields using the analysis for a generic observer. As a consequence, the gravity causes the homogeneous part of Maxwell's equations modified in addition to the inhomogeneous part. We propose the normal frame as the one to use as it corresponds to the Eulerian observer. Relations to the other definitions of the EM fields are derived in this work.

Effect of axion in flat spacetime is well known in the literature \cite{Sikivie-1983, Wilczek-1987, Sikivie-2021}. Effects of gravity observed by different observers, even without the axion, is new in the following sense. There are numerous works on the gravity-medium analogy, but the modifications in the homogeneous part of Maxwell's equations are new in the literature. To our knowledge, {\it all} the previous literature on the subject are, in fact, based on the two non-covariant identifications of the EM fields \cite{Moller-1952, Skrotskii-1957, Balazs-1958, Plebanski-1960, deFelice-1971, Landau-Lifshitz-1975, Leonhardt-2006}, and thus are in error. The point is that gravity causes modification of Maxwell's equations in both the homogeneous and inhomogeneous parts as proved in Sec.\ \ref{sec:generic}. In this way, the medium interpretation of gravity is not available.

There are two {\it wrong} arguments \cite{Berlin-2022, Domcke-2022} used to support the absence of metric in the EM fields definition using $F_{ab}$ with two covariant indices; equivalently the absence of metric in the homogeneous Maxwell's equations. Authors of \cite{Berlin-2022} argued for metric independence of the homogeneous Maxwell's equation using the topological nature of the equation in terms of form algebra. On this regards, however, the inhomogeneous equation is also topological, with no relation to metric. Thus, ``neither equation makes any reference whatsoever to {\it metric}" \cite{MTW-1973}. The metric appears when we related the two parts, and also in the decomposition of field strength tensor into the EM fields.

Authors of \cite{Domcke-2022} similarly argued for metric independence of $F_{ab}$ using the absence of the metric in the relation between $F_{ab}$ and the four-potential $A_a$, i.e., $F_{ab} \equiv A_{b,a} - A_{a,b}$. This, however, does not imply the absence of metric between $F_{ab}$ and the EM fields. In the normal-frame, Eq.\ (\ref{Fab-normal}) gives
\bea
...
The presence of metric in the decomposition of $F_{ab}$ and $F^*_{ab}$ into the EM fields is related to the presence of metric in the four-vector in gravitational environments. The generic four-vector in Eq.\ (\ref{u_a-ADM}) shows that for any choice of $V_i$, it is not possible to have the special relativistic form of the normal four-vector $n_a = (-1, 0, 0, 0)$. The presence of metric in the EM fields decomposition of $F_{ab}$ directly leads to the presence of metric in the homogeneous part of Maxwell's equations. Therefore, proposals for gravitational wave detection using the axion haloscope in \cite{Berlin-2022, Domcke-2022} are based on {\it wrong} Maxwell's equations. Although the effect of axion can be treated as an effective medium, the effect of gravity is more complicated, as expounded in this work. Proper study should use Maxwell's equations in the normal frame.

In Laboratory situations, like
%in the Earth's gravitational field \cite{Spengler-2023},
in measuring passing gravitational waves using electromagnetic means \cite{Cooperstock-1968, Baroni-1985, Berlin-2022, Domcke-2022}, in inverse designing medium property for desired optical path using curved geometry \cite{Leonhardt-2006}, and in given weak gravitational fields, Maxwell's equations are enough with the gravity encoded in a given spacetime metric. These previous literature, however, rely on the gravity-medium analogy based on defining the EM fields in non-covariant manner. Here we showed that the EM fields defined in non-covariant manner are {\it not} the EM fields measured by any observer. Consequently, gravity causes modifications in both homogeneous and inhomogeneous parts, and the gravity-medium analogy is broken. We will clarify the errors in previous literature in detail in a subsequent work \cite{Hwang-Noh-EM-medium-2023}. In the presence of gravity, for optical properties we should analyze Maxwell's equations directly. In the normal-frame, these are Eqs.\ (\ref{Maxwell-normal-1-1})-(\ref{Maxwell-normal-1-4}) or Eqs.\ (\ref{Maxwell-normal-1})-(\ref{PM-normal}) in general curved spacetime.
...
We thank Anton Sokolov for useful discussion. H.N.\ was supported by the National Research Foundation (NRF) of Korea funded by the Korean Government (No.\ 2018R1A2B6002466 and No.\ 2021R1F1A1045515). J.H.\ was supported by IBS under the project code, IBS-R018-D1, and by the NRF of Korea funded by the Korean Government (No.\ NRF-2019R1A2C1003031).
...
The FNLE formulation is designed to handle cosmological perturbations to fully nonlinear order \cite{Hwang-Noh-2013, Gong-2017}. The final equations are exact and the metric variables are directly visible in the equations. The essential point is that we can derive exact inverse metric for the most general perturbations of the Friedmann background without imposing the gauge condition. In Minkowski background, simply setting the scale factor to unity in \cite{Hwang-Noh-2013, Gong-2017}, the FNLE metric convention is
\bea
   & &
...
\bibitem{Minkowski-1910}
         H. Minkowski, Die Grundgleichungen f\"ur die elektromagnetischen Vorg\"ange in bewegten K\"orpern, Nachrichten der Kgl. Ges. d. Wiss. zu G\"ttingen, Math.-Phys. KI., 21 Dec. (1907), reprinted in Math. Ann. 68, 472 (1910).
...
\bibitem{Moller-1952}
         C. M$\o$ller, {\it The Theory of Relativity} (Clarendon Press, Oxford, 1952), Sections 73.
...
\bibitem{Landau-Lifshitz-1975}
         L.D. Landau and E.M. Lifshitz, {\it The Classical Theory of Fields}, 4th edn. (Pergmon Press, Oxford, 1975).
\bibitem{Berlin-2022}
...
\bibitem{Hwang-Noh-EM-medium-2023}
         J. Hwang and H. Noh, On gravitation as a medium property in Maxwell equations, in preparation (2023).
