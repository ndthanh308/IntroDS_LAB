...
relies only on partial solutions~$F(u',v')$ where $u' \in V(T_{i'})$ and $v' \in V(T_{j'})$ for some~$i'<j' \leq i$. 
...
Algorithm~\ref{alg:DISP} computes two minimum-cost flows of constant value,
and performs a call of Algorithm~\ref{alg:PermDisj}. 
The former can be carried out in $O(nm \log n)$ time using, e.g., the Successive Shortest Path algorithm~\cite{Jewell62,Iri60,BG60}).
The bottleneck is therefore Algorithm~\ref{alg:PermDisj}: for at most~$\binom{n}{2}$ pairs of vertices~$(u,v)$
it performs a call to Algorithm~\ref{alg:PartSol}, which in turn iterates over all possible values for vertices~$u'$ and~$v'$, and 
for each pair~$(u',v')$ computes a shortest $(u',v)$-path in the auxiliary graph $G \langle T[v',u],u',v \rangle$. 
Note that $G \langle T[v',u],u',v \rangle$ implicitly depends also on the path~$X$ (via the definition of the trees $T_i$, $i \in [r]$). Since there are $O(n^2)$ possibilities for choosing~$X$, we obtain that the number of shortest-path computations we need to perform is 
$O(n^6)$ in total.

Let us now consider an implementation where we compute all necessary shortest paths in all possible auxiliary graphs as a preprocessing step: since we can compute a shortest path between two given vertices in an undirected conservative graph in $O(n^3)$ time (see e.g.,~\cite[Section 29.2]{schrijver-book}), this takes $O(n^9)$ time in total. Assuming that we already have the results of this preprocessing step, a single call to Algorithm~\ref{alg:PartSol} (assuming that all partial solutions called for on line~\ref{line:PS-callF} are available) takes $O(n^3)$ time, because there are $O(n^2)$ possibilities for choosing $u'$ and~$v'$, and an iteration for fixed values of~$u'$ and~$v'$ takes linear time. Therefore, running Algorithm~\ref{alg:PermDisj} for fixed values of~$a_1,b_1,a_2$ and $b_2$ takes $n^2 O(n^3)=O(n^5)$ time (as it calls Algorithm~\ref{alg:PartSol} at most $n^2$ times), which yields a running time of $O(n^9)$ in total after the preprocessing phase. Hence, the overall running time of our algorithm (including all necessary preprocessing steps) is $O(n^9)$.
\hfill$\qedsymbol$
...
spanned by all negative-weight edges is a fixed constant~$c$. Is it possible to give a fixed-parameter tractable algorithm for \DISP{} on undirected conservative graphs 
when parameterized by~$c$?

Finally, is it possible to find in polynomial time $k$ openly disjoint $(s,t)$-paths with minimum total weight for some fixed~$k \geq 3$ in undirected conservative graphs?
