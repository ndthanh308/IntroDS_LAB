...
\noindent \textbf{Language Encoder} \ In order to relate known and novel concepts, GraspGPT necessitates the ability to digest a large variety of linguistic elements in language descriptions. For instance, in the case of an affordance description of the task \textit{``pour"}:
\begin{center}
...
\noindent \textbf{Loss Function} \ We compute the binary cross-entropy loss between $S$ and the ground truth label $S_{gt}$:
\begin{align*}
