...
In this scenario present-day dwarfs would have not yet have merged
...
(\citealt{Marinacci2018, Naiman2018, Nelson2018, Pillepich2018b, Springel2018}). 
The IllustrisTNG project consists of a suite of 
gravo-magnetohydrodynamic simulations which follow the evolution
...
(\citealt{Springel2001, Dolag2009}).
...
their satellites, having selected those which meet a series of 
criteria, namely: 
\begin{itemize}
...
by \texttt{SUBFIND} which are clumps of baryonic matter rather 
...
(\citealt{Bullock2017}). We label `dSph' those dwarfs whose gas 
...
and that completed 90 per cent of their star formation at $z \geq 0.5$.
 The remaining dwarfs are labeled `non-dSphs' and can be a mixture 
...
proper motions, and distances measured from the centre of the host.
...
to the observer (the centre of the host), $\rho_i$ the angular 
distance to the centre of the satellite, and $\phi_i$ the position 
...
where $\sigma_S$ is the standard deviation of $v_{S,i}$ and $N_*$ the 
...
1 to 5) than non-dSphs (between 0 to 3), and that their 
...
host in systems which are more sensitive to perturbation, owing 
to their lower mass. 
...
Our analysis reveals that the reduction of the ratios is  
related to the intrinsic evolution of the kinematics of the 
satellites. However, we note that other effects may also have 
...
As satellites orbit their host, there is a progressive mass 
loss and thus a reduction in the number of stellar particles, 
...
followed by a quick drop once pericentre is reached (see, 
for example, \texttt{subhaloID} = 428192). 
...
we show the median $|V_T|$ and $A_{\rm grad}^{v_z}$ for the satellites 
at $z = 0$.   
...
a continuous contribution of dwarfs which show $A_{\rm grad}^{v_z} \neq 0$ 
...
values taken from \citet{McConnachie2020a, MartinezGarcia2021, MartinezGarcia2022}.}
    ...
    \hline
         ...
         \hline
         \hline
        %TNG50 satellites & $0.93 \pm 0.05$ & $0.97 \pm 0.04$ \\
        TNG50 satellites  & $0.96 \pm 0.05$ & $0.99 \pm 0.04$ & $ 14.23 \pm 0.15$\\
        %TNG50 dSphs & $0.91 \pm 0.05$ & $0.96 \pm 0.03$ \\
%        TNG50 dSphs & $0.96 \pm 0.05$ & $0.97 \pm 0.04$ \\
        %TNG50 non-dSphs & $1.22 \pm 0.18$ & $1.21 \pm 0.17$ \\
%        TNG50 non-dSphs & $0.93 \pm 0.17$ & $1.19 \pm 0.16$ \\
        %MW dSph analogues & $1.06 \pm 0.09$ & $0.91 \pm 0.05$\\
        MW dSph analogues & $1.08 \pm 0.07$ & $0.91 \pm 0.07$ & $10.43  \pm  0.15$\\
        %MW dSph analogues (restricted)$^{\dag}$ & $3.12 \pm 0.19$ & $1.92 \pm 0.15$\\
        %MW dSph analogues (restricted)$^{\dag}$ & $3.19 \pm 0.28$ & $2.09 \pm 0.28$ & $10.43  \pm  0.15$\\
        MW dSph analogues (MC)$^{\dag}$ & $2.00^{+1.72}_{-0.90}$ & $1.45^{+1.01}_{-0.60}$ & $10.43  \pm  0.15$\\
        %MW dSphs$^{\dag\dag}$ & $1.85 \pm 1.64$ & $2.03 \pm 0.75$\\
...
is the distance of the satellite to the host, $v_R^{\rm GC}$ the radial 
velocity of the satellite with respect to the host, $d_a$ the apocentric 
distance, and $d_p$ the pericentric distance. $f_{\mathrm{peri}} = 0$ 
...
$A_{\rm grad}^{v_z}$ of one of the observed MW dSphs \citet{MartinezGarcia2022} 
...
We include blue vertical lines and blue-shaded areas which 
...
with those that that we observed in the internal kinematics of 
