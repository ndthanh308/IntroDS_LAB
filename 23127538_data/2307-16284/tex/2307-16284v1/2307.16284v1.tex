...
\date{July 30, 2023}
\subjclass[2010]{37P05, 11R32, 14G25}
...
field extensions of $K$ naturally embed into the automorphism group
...
We have $\deg(f^n)=d^n$, and hence $f^{-n}(y)$ has $d^n$ elements,
counted with multiplicity.

For each $n\geq 0$, define $K_n:=K(f^{-n}(x_0))\subseteq\Kbar$.
...
(i.e., ramification points) of $f$ in the backward orbit of $x_0$,
then we may consider $\Orb_f^-(x_0)$ as forming
...
See
\cite{ABetal,BosJon,BHL,FPC,Hindes,JonMan,Swam}
...
$[\Aut(T_{\infty}):G_{\infty}]$ is known to be infinite.
...
$[\Aut(T_{\infty}):G_{\infty}]=\infty$ for PCF maps,
...
which forces $[\Aut(T_{\infty}):G_{\infty}]=\infty$, as follows.
...
of quantities $\kappa_1,\kappa_2,\ldots\in L:=K(\sqrt{\delta})$,
...
\end{enumerate}
\end{thm}
We define the quantities $\kappa_n\in K$ of Theorem~\ref{thm:main2}
in Definition~\ref{def:kappa}.



%In the theorem above and throughout this paper,
%when we say that two groups that act on the tree $T_{\infty}$ or $T_n$
%are isomorphic, we mean that the isomorphism between the groups
%is equivariant with respect to the action on the tree.

The outline of the paper is as follows.
In Section~\ref{sec:prelim}, we set notation, describe tree labelings,
and define the groups $M_{\ell}$ and $\tilde{M}_{\ell}$ in terms of
...
the groups $M_{\ell}$ and $\tilde{M}_{\ell}$, in particular tree automorphisms
...
whether a given Galois automorphism is an odd cousins map or not.
...
two injective tree morphisms $a,b:T_{n-1}\to T_n$
...
(We will generally assume that there are no critical points in
the backward orbit of $x_0$.)
...
in particular permuting the $2^\ell$ nodes that are $\ell$ levels above $y$
with sign $\sgn_{\ell}(\tau,y)$.
Then $\sigma$ permutes those same nodes with sign
...
given by the basis $\{ U_n \, | \, n\geq 1\}$ of open neighborhoods of $e$,
where for each $n\geq 1$, the normal subgroup $U_n$ consisting of those
$\sigma\in\Aut(T_{\infty})$ that are trivial on the finite subtree $T_n$.
...
that $\sgn_{\ell}(\sigma,y)$ is even introduces an index~2 restriction
on $M_{\ell,n}$, and these restrictions are independent of one another.
...
A short computation shows that $D_{P,Q}$ is
a homogeneous polynomial of degree $2m-2$ in $K[X,Y]$,
...
and define $R(X,Y):=\beta P(X,Y) - \alpha Q(X,Y)$. Then
\begin{enumerate}
    \item $\dsps \Res(R,D) = (-1)^{m(m-1)/2} \Res(P,Q) \Delta(R)$.
    \item Let $\tilde{\alpha},\tilde{\beta}\in\Kbar$, not both zero,
and define $\tilde{R}(X,Y):=\tilde{\beta} P(X,Y) - \tilde{\alpha} Q(X,Y)$.
...
and let $H(X,Y)=J(P(X,Y),Q(X,Y))$. Then
...
where $P(X,Y)=Y^d p(X/Y)$, and $Q(X,Y)=Y^d q(X,Y)$,
...
Let $[a,b]\in\PKbar$
be the point $F^{\ell}([\eta_1,\theta_1]) = F^{\ell}([\eta_2,\theta_2])$.
...
For each $i=1,2$, define $\alpha_i,\beta_i\in L$
by $(\alpha_i,\beta_i)=F^{\ell-1}(\eta_i,\theta_i)$.
...
$F^{n-1}(\eta_2,\theta_2) = \mu F^{n-1}(\eta_1,\theta_1)$. It follows that
...
such that $F^{n-1}(\eta_1,\theta_1)=\mu (a_{n-1},b_{n-1})$
...
Writing $P=a_0 X^2 + a_1 XY + a_2 Y^2$
and $Q=b_0 X^2 + b_1 XY + b_2 Y^2$, direct computation
...
\cite[Proposition~3.2]{AHM}, and version similar to~\eqref{eq:polyiter}
...
%The following result is a more precise version of Theorem~\ref{thm:main1}.
%We will prove it by applying Theorem~\ref{thm:discsquare} in the context of the
%fields $K_n$ and $K_{\infty}$, Galois groups $G_n$ and $G_{\infty}$,
%and tree automorphism groups $M_{\infty}$ and $\widetilde{M}_{\infty}$
%from Section~\ref{sec:prelim}.
%
%\begin{thm}
%\label{thm:GsubsetM}
%Let $K$ be a field with $\charact K\neq 2$, let $f\in K(z)$
%be a rational function with $\deg f = 2$,
%and let $\xi_1,\xi_2\in\PKbar$ be the two critical points of $f$.
%Suppose that $\xi_1$ and $\xi_2$ collide at the $\ell$-th iterate,
%for some integer $\ell\geq 2$.
%Let $x_0\in\PK$, let $G_{\infty}=\Gal(K_{\infty}/K)$ be the associated
%arboreal Galois group, and identify $G_{\infty}$
%with its image in $\Aut(T_{\infty})$ induced by its action on $\Orb_f^-(x_0)$.
%Then there is a labeling of $T_{\infty}$ such that:
%\begin{enumerate}
%\item if $\xi_1,\xi_2$ are $K$-rational, then $G_{\infty}\subseteq M_{\ell}$.
%\item otherwise, we have $G_{\infty}\not\subseteq M_{\ell}$,
%but $G_{\infty}\subseteq\widetilde{M}_{\ell}$.
%\end{enumerate}
%\end{thm}
...
define $P,Q\in K[X,Y]$ by $P(X,Y):=Y^2 p(X/Y)$
and $Q(X,Y):=Y^2 q(X/Y)$. Thus, $F:=(P,Q)$ is a homogenization of $f$,
...
of the tree, and we also call this node $x$. Writing
$x=[s,t]\in\PKbar$ with $s,t\in K(x)$, define
$H_{x,\ell}:=t P_\ell - s Q_\ell\in K(x)[X,Y]$, where $F^\ell=(P_\ell,Q_\ell)$.

Choose any labeling of the tree $T_{\infty}$;
in the rest of the proof, we will make changes to produce the desired labeling.

\smallskip

\textbf{Case 1}. If $\delta$ is a square in $K$,
then $\xi_1,\xi_2\in\PK$, and hence we may assume that
...
In particular, since every $\sigma\in G_{\infty}$ fixes $x_0\in\PK$,
implication~\eqref{eq:sigmaeven} shows that $\sgn_\ell(\sigma,x_0)=+1$.
...
$\lambda:=\sigma_z^{-1}\circ\rho\circ\sigma_w$ fixes $y$
...
and hence so is $\sgn_{\ell}(\sigma_w^{-1},w)$. Therefore, by
...
and adjust the labels above each $w\in W_y$. Having completed this
...
In particular, $\sqrt{\delta}\in K(x_0)_{\ell}=K_{\ell}\subseteq K_{\infty}$.
...
For any $m\geq 1$,
suppose that we have already verified that for all nodes $x$ up to level $m-1$,
we have $\sgn_\ell(\sigma,x)=\sgn_\ell(\sigma,x_0)$ for all $\sigma\in G_{\infty}$,
...
\end{align*}
Thus, for every $\sigma$ in either of the two cosets of $G'_{\infty}$ in $G_{\infty}$,
...
gives us $\sgn_{\ell}(\rho^{-1},\rho(w)) = \sgn_{\ell}(\rho^{-1},x_0)$.
...
Let $\sigma\in M_{\ell}$.
\begin{enumerate}
\item Let $y_1,y_2$ be the two children of a node $w$,
i.e., the two nodes connected to $w$
on the level above $w$.
We say that $\sigma$
\[ \text{\emph{acts $\ell$-positively above $w$} if }
\sgn_{\ell -1}(\sigma,y_1) = \sgn_{\ell -1}(\sigma,y_2) = +1, \]
or that $\sigma$
...
Note that the two signs $\sgn_{\ell -1}(\sigma,y_1)$
and $\sgn_{\ell -1}(\sigma,y_2)$ in Definition~\ref{def:cousins}(1) must
be equal, since $\sigma\in M_{\ell}$ satisfies $\sgn_{\ell}(\sigma,x)=+1$.
As for part~(2) of the definition, observe 
...
are naturally partitioned into $N:=2^{n-\ell}$ sets of $2^{\ell}$ nodes, with one
such set above each of the nodes $w_1,\ldots,w_N$
in Definition~\ref{def:cousins}(2); and $\sigma\in M_{\ell}$ must act with even
parity on each of these $N$ sets.

An example of a $(2,3)$-odd cousins map $\sigma$ above a node $x$ appears
...
$N=2^{3-2}=2$ nodes $w=0,1$, it is indeed
a $(2,3)$-odd cousins map.


% Figure environment removed

The following result is essentially \cite[Lemma~1.6]{Stoll}, but we include
a proof for the convenience of the reader, especially given our slightly different context.
...
that is, $w(\sigma)_y:=(1-\sgn_1(\sigma,y))/2\in \FF_2$.
...
Observe that $V$ is an $\FF_2$-subspace of $\FF_2^S$, and because it is
carved out of $\FF_2^S$ by a set of $\dim W$ linearly independent equations,
we have $\dim V + \dim W = |S|$.
Moreover, $V$ is also
an $\FF_2[G']$-module, because for any $\tau\in G'$ and $v\in V$,
we have $\tau(v)\in V$. Indeed, lifting $\tau$ to $G$, for any $\sigma\in E$
and $y\in S$, equation~\eqref{eq:Parident} gives
\[ \sgn_1\big(\tau^{-1}\sigma\tau,\tau^{-1}(y)\big)
=\sgn_1(\tau^{-1}, \sigma(y)) \sgn_1(\sigma,y) \sgn_1(\tau,\tau^{-1}(y))
= \sgn_1(\sigma,y), \]
since $\sigma(y)=y$, and hence
...
which is trivial for all $\sigma\in E$ if and only if $v\in V$.

We claim that $V$ is trivial. To prove the claim, observe that for every $v\in V$,
...
for every $\sigma\in E$, we have $\sgn_n(\sigma,x_0)=0$,
...
our hypothesis say that  satisfies $G'\cong M_{\ell,n-1}$.
...
As before, for every $[v]=X+v \in \FF_2^S / X$, the orbit $G'[v]$
has cardinality dividing $|G'| = |M_{\ell,n-1}|$, which is a power of $2$.
The $G'$-orbit of the zero vector $[0]$ clearly has only one element.
We claim that there is exactly one other one-element orbit.

To prove the claim, we define a particular vector $\tilde{v}\in \FF_2^S$ as follows.
...
Define $\tilde{v}$ by setting $\tilde{v}_y$ to be $i$ if $y$ belongs to some $S_{ui}$,
for $i=0,1$.
...
--- and hence applying any $\sigma\in G'$ --- yields a vector $\sigma v$ of the same
form. Thus, we have verified that $\sigma [\tilde{v}] = [\tilde{v}]$,
and hence that the orbit $G'[v]$ does indeed have only one element.
Clearly $\tilde{v}\not\in X$, and hence $[\tilde{v}]\neq [0]$.

Now suppose $v'\in\FF_2^S$ has the property that the orbit $G'[v']$ has only one element.
Since $G'$ acts transitively on the nodes $u$ at level $n-\ell$, then after replacing $v'$
by $v'+x\in [v']$ for some $x\in X$, we have that $v'$ looks the same on each block $S_u$.
Pick any node $u$ at level $n-\ell$, and pick a node $y\in S_{u0}$.
Possibly after adding $(1,\ldots, 1)\in X$ to $v'$, we may assume that $(v')_y=0$.
...
in particular, $v'-\tau v' \in X$ is constant on $S_u$.
But $(\tau v')_y=0 = (v')_y$, so this constant difference is $0$.
Therefore, 
\[ (v')_{z_1} = (\tau v')_{z_1} = (v')_{z_2} .\]
Since this identity holds for all $z_1,z_2\in S_{u1}$, it follows that $v'$ is constant
on $S_{u1}$. By a similar argument, $v'$ is also constant on $S_{u2}$.
Because $v'$ looks the same on each block $S_u$ of $2^{\ell-1}$ nodes, it follows
that either $[v']=[0]$ or $[v']=[\tilde{v}]$,
proving our claim.

Having proven our first claim, we make a second claim: that $V=X$,
i.e., that $V/X$ is trivial. To see this, suppose not. Then $|V/X|$ is even,
...
Our second claim follows: we have $V=X$.
Hence,
...
$\psi(\sigma):= (e_1, \ldots, e_n)$, where for each $i=1,\ldots,n$, we have
\[ e_i:= \begin{cases}
...
For $n\leq\ell-1$, note that half of the elements of $E_n$ are even, and half are odd.
Similarly, for $n\geq \ell$, half of the elements of $E_n$ are even-cousins maps,
and the other half are odd-cousins maps.
We also have
...
some $\sigma\in M_{\ell,n-1}$ such that $\psi_{n-1}(\sigma)=(e_1,\ldots, e_{n-1})$.
Lift $\sigma$ to some $\tilde{\sigma}\in M_{\ell,n}$,
so that $\psi_n(\sigma)=(e_1,\ldots,e_{n-1}, \tilde{e}_n)$ for some $\tilde{e}_n\in\{\pm 1\}$.
...
Then $\psi_n(\tau\tilde{\sigma}) = (e_1,\ldots, e_n)$, as desired.

It remains to show,
given $\sigma\in\ker(\psi_n)$, that $\sigma$ belongs to
the commutator subgroup of $M_{\ell,n}$. Restricting $\sigma$ to $T_{n-1}$
yields $\sigma' \in M_{\ell,n-1}$, which by our inductive hypothesis is a product
of commutators. Lifting each of the factors of these commutators to $M_{\ell,n}$,
we are reduced to the case that $\sigma\in E_n$.
Recall that the two children of $x_0$ are the nodes labeled $0$ and $1$.
Write $\sigma=(\sigma_0,\sigma_1)\in E_{n-1}\times E_{n-1}$,
...
Each factor $\tau\in M_{\ell,n-1}$ in each commutator for $\sigma_0$
extends to $(\tau,e)\in M_{\ell,n}$,
where $e$ denotes the identity element of $M_{\ell,n-1}$.
Taking the product of the resulting commutators
yields $(\sigma_0,e)$ as a product of commutators in $M_{\ell,n}$.
Similarly, $(e,\sigma_1)$ can be written as a product of commutators in $M_{\ell,n}$.
Therefore, $\sigma = (\sigma_0,e)(e,\sigma_1)$
can also be written as such a product.

Second, suppose that $n=\ell$,
in which case $\sigma_0$ and $\sigma_1$ are both necessarily even,
since we assumed that $e_\ell=0$, and hence that
...
Fix $\rho\in E_{\ell-1}$ that is odd. Each factor $\tau$ in one
of the commutators for $\sigma_0$ extends to $(\tau,e)\in M_{\ell,\ell}$ if $\tau$ is even,
or to $(\tau,\rho)\in M_{\ell,\ell}$ if $\tau$ is odd.
As in the first case above, it follows that
$(\sigma_0,e)$ is a product of commutators in $M_{\ell,\ell}$,
and similarly for $(e,\sigma_1)$.
Once again, then $\sigma = (\sigma_0,e)(e,\sigma_1)$
can also be written as a product of commutators.
...
As in the first case above, it follows that $(\sigma_0,e)$
and $(e,\sigma_1)$ are products of commutators in $M_{\ell,n}$.
Therefore, so is $\sigma = (\sigma_0,e)(e,\sigma_1)$.
...
Let $\lambda\in\Aut(T_n)=M_{\ell,n}$ be the automorphism given by
...
Thus, $\sigma\mu^{-1} = (\sigma_0\rho, \sigma_1 \rho^{-1}) \in E_n$ is
even (respectively, an even-cousins map) on each half of the tree.
...
with the usual understanding of what this expression means if any of the four points
is $\infty$. (For example, if $c=\infty$, the cross ratio above is $-(a-b)/(b-d)$.)
...
$\xi_1 =f^n(\xi_1)=f^n(\xi_2)=\xi_2$, a contradiction.
...
Let $\delta\in K^{\times}$ be the discriminant
of the minimal polynomial of $\xi_1$ over $K$, or $\delta=1$ if $\xi_1\in\PK$.
Let $F=(P,Q)$ be a homogeneous lift of $f$;
that is, $P,Q\in K[X,Y]$ are relatively prime homogeneous polynomials of degree $2$
such that $f(z) = P(z,1)/Q(z,1)$.
Let $(s_0,t_0)$ be a lift of the point $x_0\in\PK$ to $K\times K \smallsetminus \{(0,0)\}$,
so that $x_0=s_0/t_0$, with the usual understanding of $1/0$ as the point $\infty$.
Similarly lift each critical point $\xi_i$ to
$(\eta_i,\theta_i)\in L \times L \smallsetminus \{(0,0)\}$,
where $L:= K(\sqrt{\delta})$.
For each integer $n\geq 1$, write $(P_n,Q_n)=F^n$, and define
$H_n:= t_0 P_n - s_0 Q_n$, as in Corollary~\ref{cor:iterdisc}.
...
$\nu\in\PGL(2,K)$, let $g:=\nu\circ f \circ \nu^{-1}$, and let $y_0:=\nu(x_0)$.
Let $\tilde{\kappa}_1,\tilde{\kappa}_2,\ldots\in K(\sqrt{\delta})$ be the associated quantities
...
Lift $f$ to $F=(P,Q)$ and $x_0$ to $(s_0,t_0)$ as described
at the start of this section.
Lift $\nu$ to $N=(R,S)$ where $R=aX+bY$ and $S=cX+dY$
with $a,b,c,d\in K$ and $\eps:=ad-bc\in K^{\times}$.
...
Let $x\in\PKbar$ not in the forward orbit of $f(\infty)$,
...
\begin{equation}
\label{eq:lemmaclaim}
\prod_{i=1}^{2^{n-1}} \beta_i^2 = q_n \bigg( \frac{w-f^n(0)}{w- f^n(\infty)}\bigg),
\quad\text{where} \quad
...
\bigg[ q_{n-1} \bigg( \frac{u-f^{n-1}(0)}{u - f^{n-1}(\infty)} \bigg) \bigg]
\bigg[ q_{n-1}  \bigg( \frac{-u-f^{n-1}(0)}{-u - f^{n-1}(\infty)} \bigg) \bigg]  \\
& = q_{n-1}^2 \bigg( \frac{u^2 - (f^{n-1}(0))^2}{u^2 - (f^{n-1}(\infty))^2} \bigg)
= q_{n-1}^2 \CR\big(w,f^n(0),f^n(\infty),f(\infty) \big)
\\
&= q_{n-1}^2 \bigg(\frac{f(\infty) - f^n(\infty)}{f(\infty) - f^n(0)} \bigg)
\bigg( \frac{w-f^n(0)}{w-f^n(\infty)} \bigg)
%& = q_{n-1}^2 \bigg( \frac{Cw - B}{A-w} - \frac{Cf^n(0)-B}{A-f^n(0)} \bigg)
%\bigg( \frac{Cw - B}{A-w} - \frac{Cf^n(\infty)-B}{A-f^n(\infty)} \bigg)^{-1} \\
%& = q_{n-1}^2 \bigg( \frac{A - f^n(\infty)}{A-f^n(0)} \bigg)
%\bigg( \frac{ (Cw-B)(A-f^n(0)) - (A-w)(Cf^n(0))-B)}
%{ (Cw-B)(A-f^n(\infty)) - (A-w)(Cf^n(\infty))-B)} \bigg) \\
%& = q_{n-1}^2 \bigg( \frac{f(\infty) - f^n(\infty)}{f(\infty)-f^n(0)} \bigg)
%\bigg( \frac{ ACw + Bf^n(0) - ACf^n(0) - Bw }{ ACw + Bf^n(\infty) - ACf^n(\infty) - Bw } \bigg) \\
%& = q_n \bigg( \frac{ (AC-B)(w-f^n(0))}{ (AC-B)(w-f^n(\infty))} \bigg)
= q_n \bigg( \frac{w-f^n(0)}{w-f^n(\infty)} \bigg),
\end{align*}
where the fourth equality is by equation~\eqref{eq:CRrewrite}
and the fact that $f(u)=w$,
% the sixth uses the fact that $f(\infty)=A$,
...
& = q_{\ell - 1} \bigg[ \bigg( \frac{ y - f^{\ell-1}(0) }{ y - f^{\ell-1}(\infty) } \bigg)
...
& = q_{\ell-1} \bigg( \frac{4 y^2 }{y^2 - (f^{\ell-1}(\infty))^2} \bigg)
= 4q_{\ell-1} \bigg( \frac{y^2 - 0^2}{y^2 - (f^{\ell-1}(\infty))^2} \bigg)
\\
...
let $n\geq\ell+1$, and define $m:= 2^{\ell -2}$ and $N:=2^{n-\ell}$.
Write the $2N$ points of $f^{-(n-\ell+1)}(x)$ as $\pm\beta_1,\ldots,\pm\beta_N$.
For each $j=1,\ldots, N$, write
...
Then
\[ \prod_{j=1}^N \bigg( \prod_{i=1}^m \alpha_{j,i} + \prod_{i=1}^m \alpha'_{j,i} \bigg)^2
...
Note that the lemma is only started for $n\geq\ell+1$, but the formula for $r_n$
...
$f^{-(n-\ell)}(y)=\{\pm\beta_1,\ldots,\pm\beta_{N/2} \}$
and $f^{-(n-\ell)}(-y)=\{\pm\beta_{1+N/2},\ldots,\pm\beta_{N} \}$;
see Figure~\ref{fig:RnLemma}.
Thus,
\begin{align*}
\prod_{j=1}^N \bigg( \prod_{i=1}^m & \alpha_{j,i} + \prod_{i=1}^m \alpha'_{j,i} \bigg)^2
=
\prod_{j=1}^{N/2} \bigg( \prod_{i=1}^m \alpha_{j,i} + \prod_{i=1}^m \alpha'_{j,i} \bigg)^2
\cdot
\prod_{j=1+N/2}^N \bigg( \prod_{i=1}^m \alpha_{j,i} + \prod_{i=1}^m \alpha'_{j,i} \bigg)^2
\\
...
then Definition~\ref{def:kappa} yields $\kappa_n=0$, forcing both versions
of statement~(a) of the theorem to fail.
...
it follows that both versions of statement~(b) also fail.
...
$K_n$ or $K_{\infty}$, and hence it also does not change the Galois group $G_{\infty}$
or its action on the tree of preimages.
Thus, condition (b) is also unaffected by this coordinate change.

Hence, we may assume without loss of generality that
$\xi_1=0$ and $\xi_2=\infty$. Equivalently, $f(z)$ is a function of $z^2$.
...
\noindent
\textbf{Claim}: 
For each $n\geq 1$, the quantity $\kappa_n\in K$ is a square in $K_n$.
...
For this implication, it suffices to show that $G_n\cong M_n$ for each $n\geq 0$.
We proceed by induction on $n$.
The desired isomorphism holds trivially for $n=0$.

For arbitrary $n\geq 1$, assume $G_{n-1}\cong M_{\ell,n-1}$.
...
$\sigma_n\in\Gal(K_n/K_{n-1})$ with 
...
as in Lemma~\ref{lem:Qn}, we have that $\pi_n:=\prod \alpha_i + \prod \alpha'_i$
is $\sqrt{\kappa_n}$ time an element of $K^{\times}\subseteq K_{n-1}^{\times}$.
...
\[ f^{-n}(x_0)=\bigcup_{j=1}^N \Big( \{\pm\alpha_{j,1},\ldots,\pm\alpha_{j,m}\} \cup
\{\pm\alpha'_{j,1},\ldots,\pm\alpha'_{j,m}\} \Big )\]
as in Lemma~\ref{lem:Rn}, we have that
$\pi_n:=\prod_j \big( \prod_i \alpha_{j,i} + \prod_i \alpha'_{j,i} \big)$
is $\sqrt{\kappa_n}$ time an element of $K^{\times}\subseteq K_{n-1}^{\times}$.
...
with $1\leq i_1 < \cdots < i_m$ and $m\geq 1$.
...
the argument from the previous implication shows that $G_{n-1}\cong M_{n-1}$.
...
$G_n \subsetneq M_{\ell,n}$, and hence that $G_{\infty}\not\cong M_\ell$.
...
Thus, we may define $G'_n:=\Gal(K_n/L)$ for each $n\geq \ell$, and
...
it follows that $G_{\ell}\cong\Aut(T_\ell) = \tilde{M}_{\ell,\ell}$.

In addition, as noted above, we have $L\subseteq K_{\ell}$.
...
level of the tree. Thus, $G'_{\ell}$ must be precisely the set of elements of $\Aut(T_{\ell})$
that are even at the $\ell$-th level; that is, $G'_{\ell}=M_{\ell,\ell}$.
...
By Theorem~\ref{thm:main1}, then, $G_{\infty}$ is a subgroup of $\tilde{M}_{\ell}$
that contains $G'_{\infty}\cong M_{\ell}$ as a subgroup of index $[L:K]=2$.
Hence, $G_{\infty}$ is isomorphic to the whole group $\tilde{M}_{\ell}$.
...
leaving a nontrivial product not involving $\kappa_{\ell}$ that is a square in $L$.
By Case~1, it must be that $G'_{\infty}$ is \emph{not} isomorphic to $M_{\ell}$.
That is, $G'_{\infty}$ is a proper subgroup of $M_{\ell}$, so that
$[M_{\ell}:G'_{\infty}]\geq 2$.
Since $[G_{\infty}:G'_{\infty}] = [L:K]=2$, and $[\tilde{M}_{\ell}:M_{\ell}]=2$,
it follows that $[\tilde{M}_{\ell}:G_{\infty}]\geq 2$.
In particular, $G_{\infty} \not\cong \tilde{M}_{\ell}$.
\end{proof}
...
\emph{Bull.\ Lond.\ Math.\ Soc.}\ \textbf{51} (2019), 237--350.
% no. 2
