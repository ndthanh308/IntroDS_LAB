...
topological algebras do not arise in this way.
 Furthermore, the dual vector space to an infinite-dimensional
$k$\+algebra usually does not have a coalgebra structure.
...
on the category $\sE$ (which we call \emph{left strict local
...
version is much more involved and depends on the assumption of left
strict local finiteness of the $k$\+linear category~$\sE$.
...
left $\C_\sE$\+contramodules for a left strictly locally finite
...
\C^*\Modl$ is fully faithful~\cite[Theorem~1.2]{Psm},
...
$\Modr\sE$ for any left strictly locally finite $k$\+linear
category~$\sE$.
...
the left strict local finiteness assumption is \emph{not} sufficient
to make the whole category of (locally infinite) left
$\C_\sE$\+contramodules closed under extensions in $\C_\sE\Modl$.
...
locally finite $k$\+linear categories or left strictly
locally finite $k$\+linear categories $\sE$ considered in the main
...
sum of left\/ $\C_\xi$\+comodules\/ $\M_x$ over\/ $\xi\in\Xi$,
$$
...
a direct product of left\/ $\C_\xi$\+con\-tra\-mod\-ules\/ $\fP^x$
over\/ $\xi\in\Xi$,
$$
...
of~\cite[Lemma~8.1(a)]{Pproperf}, where the applicability of
topological algebra language to comodules over coalgebras over
fields is established in~\cite[Section~2.1]{Swe} (see the discussion
in the beginning of Section~\ref{change-of-scalars-subsecn} below,
...
computed as follows.
 For any left $C$\+module $N$, one has $f^*(M)=N/DN$.
...
\begin{itemize}
\item $x\preceq y$ if and only if there exists an integer $n\ge1$ and
...
not full.
 ...
 Let $R\subset\C^*$ be a dense subring in the natural
...
\begin{defn} \label{left-strict-locally-finite}
 Let $\sE$ be a locally finite $k$\+linear category.
 We will say that $\sE$ is \emph{left strictly locally finite} if
...
Example~\ref{contra-ff-counterex} is \emph{not} left strictly
locally finite (take $y=-1$).

\begin{thm} \label{main-full-and-faithfulness-theorem}
 Let\/ $\sE$ be a left strictly locally finite $k$\+linear category.
...
$V\rarrow\Hom_k(\C_\sE,V)$ representing $V$ as space of free
...
the composition $\rho_\fQ\:\Hom_k(\D_\sE^\prec,\fQ)\rarrow\fQ$ of
the injective map $\Hom_k(\D_\sE^\prec,\fQ)\rarrow\Hom_k(\C_\sE,\fQ)$
induced by the surjective map $\C_\sE\rarrow\D_\sE^\prec$ with
the $\C_\sE$\+contraaction map $\pi_\fQ\:\Hom_k(\C_\sE,\fQ)\rarrow\fQ$
\emph{cannot} be surjective.
 The image of the $\D_\sE^\prec$\+contraaction map~$\rho_\fQ$
is a $\C_\sE$\+subcontramodule in $\fQ$; in fact, it is the maximal
$\C_\sE$\+subcontramodule $\fS\subset\fQ$ with the property that
the $\C_\sE$\+contramodule structure on the quotient contramodule
$\fQ/\fS$ arises from a $\C_\sE^\sim$\+contramodule structure.
...
vanishes for every $y\in\sE$.
 ...
 We still have not used the assumption that $\sE$ is left strictly
locally finite so far in this proof.
 Now we are using it.
 Consider the finite set of objects $X_y\subset\sE$ corresponding to
the object~$y$ as per Definition~\ref{left-strict-locally-finite}.
 Put
$$
...
Definition~\ref{left-strict-locally-finite} implies injectivity
...
the dual vector space to $\Hom_\sE(z,y)$.
 The condition in Definition~\ref{left-strict-locally-finite}
tells precisely that the composition/multiplication map
in the category~$\sE$,
$$
...
the map~\eqref{Hom-from-U-cogeneration-map} with the contraaction map
\begin{equation} \label{long-contraaction-map}
 \prod\nolimits_{z\in\sE}^{z\prec y}
 \Hom_k(\C_\sE^{z,y},\fP^z)\overset{\rho_\fP^y}\lrarrow\fP^y
\end{equation}
as the composition of two maps induced by
the contraaction map~$\pi_\fP$.
...
projection~$q_y$.
 ...
 Informally speaking, we have shown that \emph{the image of
the contraaction of the long morphisms in\/ $\fP$, projected
to\/ $\fP^y$, is contained in the image of the contraaction
...
\simeq P$ is unique if it exists.
 Let us also emphasize that the left strict local finiteness
...
Theorem~\ref{locally-finite-contramodules-described}]
 This implication requires \emph{neither} the left strict local
...
\end{defn}
 ...
 In particular, Definition~\ref{left-strict-locally-finite} can be
restated by saying that a locally finite $k$\+linear category $\sE$
is left strictly locally finite if and only every object $y\in\sE$
admits a frontier.
 To be more precise, Definition~\ref{left-strict-locally-finite}
...
object $y\in\sE$ as per Defintion~\ref{left-strict-locally-finite}.
...
Defintion~\ref{left-strict-locally-finite}(ii), the set $X^\sim$
is finite.
...
$$
where $g_i\in\Hom_\sE(u,x_i)$ and $h_x\in\Hom_\sE(x_i,y)$, \
...
Q(x)\rarrow Q(y)$, as we have seen.
...
\begin{qst}
 Let $\sE$ be a left strictly locally finite $k$\+linear category and
...
\begin{cor} \label{anti-equivalence-corollary}
 For any left strictly locally finite $k$\+linear category\/ $\sE$,
...
corresponding to a left/right strictly locally finite $k$\+linear
...
the left strict local finiteness condition on the category $\sE$
...
$\sE$\+modules $0\rarrow N^*\rarrow M^*\rarrow L^*\rarrow0$.
 If the right $\sE$\+modules $L$ and $N$ are cofinite, the left
...
\end{proof}
 ...
 It is not very original (cf.~\cite[Theorem~4.8]{Iov}), but we include
it in this paper, together with a full proof, for the sake of
...
the left strict local finiteness assumption on~$\sE$; they were valid
over any locally finite $k$\+linear category.
 But the resulf of Proposition~\ref{comodules-extension-closed}
already depends on the left strict local finiteness.
...
a left strictly locally finite $k$\+linear category $\sE$ in general.
...
left modules over a left strictly locally finite
$k$\+linear category~$\sE$.
...
\Hom_\sE(l,n)$ are isomorphisms for all $l\le m\le n$.
 One can easily see that $\sE$ is a left (and right) strictly
...
locally finite $k$\+linear categories that are not left strictly
locally finite, the category $\sE$ from
Example~\ref{noncomodule-noncontramodule-counterex} is a source of
simple counterexamples to statements about left and right strictly
...
left strictly locally finite in the sense of
Definition~\ref{left-strict-locally-finite}.
...
Construction~\ref{from-comodule-construction} and
...
\begin{proof}
 Using the notation of
Construction~\ref{from-contramodule-construction} and
...
\textit{Bulletin of the London Math.\ Society}, published online at
\texttt{https://doi.org/10.1112/blms.12797} in January~2023, 90~pp.
\texttt{arXiv:2207.07063 [math.CT]}
