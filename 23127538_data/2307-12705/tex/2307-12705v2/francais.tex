...
\title{Amortissement des phonons dans un superfluide 2D : insuffisance de la règle d'or de Fermi à basse température / {\it Phonon damping in a 2D superfluid: insufficiency of Fermi's golden rule at low temperature}}
...
\ee
où le chemin d'intégration $C_+$ court dans le demi-plan {\yc complexe} supérieur parallèlement à l'axe réel de $+\infty$ à $-\infty$ et où nous avons, par commodité, retiré l'évolution libre due à l'hamiltonien $H_2$. Dans la suite, il sera avantageux de prendre comme nouvelle variable d'énergie complexe l'écart à l'énergie de l'évolution libre, toute confusion étant évitée par mise en indice de la dépendance en vecteur d'onde:
\be
\label{eq013}
\Sigma(\qq,z)=\Sigma_\qq(\zeta)\quad\mbox{avec}\quad\zeta\equiv z-\veps_\qq\quad{\ycd\mbox{d'où}\quad \mathcal{G}(\qq,z)=\frac{1}{\zeta-\Sigma_\qq(\zeta)}}
\ee
...
\paragraph{Étape (ii)} Pour savoir s'il suffit de calculer la fonction énergie propre à l'ordre dominant en le couplage entre phonons, gardons pour simplifier seulement le couplage cubique $H_3$ (on peut vérifier que le couplage quartique $H_4$ ne change rien qualitativement), omettons les processus non résonnants qu'elle contient (termes en $\hat{b}^\dagger\hat{b}^\dagger\hat{b}^\dagger$ et $\hat{b}\hat{b}\hat{b}$) de contribution sous-dominante dans la limite $\epsilon\to 0$, et estimons l'ordre de grandeur de sa contribution d'ordre $2n$ à $\Sigma_\qq{\ycd(\ii 0^+)}$ comme suit:\footnote{Comme $H_3$ change la parité du nombre de phonons, les contributions d'ordre impair $2n+1$ sont nulles.}
\be
\label{eq024}
...
\end{multline}
avec $\zeta(2)=\pi^2/6$. À suffisamment basse température, $\Sigma^{(2)}_\qq(\ii 0^+)$ devient donc imaginaire pur et se réduit à $-\ii\hbar\Gamma_q/2$ où $\Gamma_q$ est le taux d'amortissement de la règle d'or à l'ordre dominant en $\epsilon$ (l'hamiltonien quartique $H_4$ ne fait pas apparaître ici de partie réelle car il est sous-dominant en $\epsilon$, voir la section \ref{sec2}). L'équation (\ref{eq033}) est à notre connaissance originale. Dans les limites $\qb\to 0$ et $\qb\to +\infty$, elle redonne les équations (30) et (22) de la référence \cite{CB} (fois $-\ii\hbar$ compte tenu des conventions de \cite{CB}) qui traite du gaz bidimensionnel de bosons en interaction faible pour lequel $\gamma=1$ (comme dans le spectre de Bogolioubov) et $\Lambda=0$ (comme pour l'équation d'état de champ moyen à température nulle $\mu=\rho g$ utilisée dans \cite{CB}). Dans la suite, nous irons en dimension $d=2$ au-delà de la règle d'or de Fermi avec des simulations numériques de champ classique dans la section \ref{sec1} et avec la méthode des fonctions de Green à des ordres en $H_3$ de plus en plus élevés dans les sections ultérieures: à l'ordre deux dans la section \ref{sec2}, à l'ordre quatre dans la section \ref{sec3} et de manière non perturbative dans la section \ref{sec4} ; à notre surprise, nous y sommes contraints par l'échec de l'estimation (\ref{eq028}) pour $n\geq 2$. {\ycd Nous établissons en effet, par un calcul explicite de l'ordre quatre en $H_3$ dans la section \ref{sec3}, un résultat troublant qui dément (\ref{eq028}) en dimension $d=2$: $\Sigma_{\qq}^{(4)}(\ii 0^+)$ ne tend pas vers zéro comme $k_B T \epsilon^2$ mais comme $k_B T\epsilon^0$ à basse température $\epsilon\to 0$, voir l'équation (\ref{eq296}). Ceci rend en fait inutilisable le développement perturbatif en puissances de $H_3$ dans cette limite, puisque l'ordre quatre devient beaucoup plus grand que l'ordre deux à force des interactions $(\rho\xi^2)^{-1}$ fixée dans le superfluide. Pour obtenir une description opérationnelle de l'amortissement, la section \ref{sec4}, au contraire de l'équation (\ref{eq028}), prend la limite $\epsilon\to 0$ à $\zetat\neq 0$ fixé dans $\tilde{\Sigma}_{\qb}^{(4)}(\zetat)$, régularise le résultat (qui diverge en $\zetat=0$) de manière non perturbative en donnant à la main une durée de vie finie aux phonons intermédiaires des processus collisionnels à quatre phonons et calcule l'intégrale sur $\zetat$ dans l'équation (\ref{eq023}), pour aboutir à un taux d'amortissement variant en $k_B T\epsilon^2$ comme attendu.} Nous concluons dans la section \ref{sec5}.
...
\be
\label{eq100}
H_3 = {\ycd H_3^{(+)} + H_3^{(-)}} + H_3^{\mbox{\scriptsize (non rés.)}}
\ee
avec
\be
\label{eq101}
...
\ee
et l'amplitude de couplage\footnote{Par choix de notation, notre amplitude $\mA$  diffère d'un facteur deux de celle de la référence \cite{Annalen} {\yct(plus précisément d'un facteur $2mc^2/\rho^{1/2}$)}.}
\be
\label{eq102}
...
\ee
Il se trouve que ce signal est en pratique très proche de celui de l'équation (\ref{eq012}), {\yc ce qui autorise dans les simulations à l'identifier à $s(t)$, comme nous le ferons dans la légende des figures:}
\be
\label{eq111}
s_*(t) \simeq s(t)
\ee
En effet, même pour l'interaction la plus forte considérée dans nos simulations, on dispose, pour toute valeur de l'énergie $\veps$, de la propriété $|\im\Sigma(\qq,\veps+\ii 0^+)|\ll \veps_\qq$, si bien que la différence entre les fonctions de Green $\mG(\qq,\veps\pm\ii\delta)$ entre crochets dans l'équation (\ref{eq110}), qui ne peut résulter que d'une partie imaginaire non nulle de la fonction énergie propre dans la limite $\delta\to 0^+$, est très petite sauf dans la largeur de la \g{lorentzienne} $\mathcal{G}(\qq,\veps+\ii 0^+)$ où $|\veps-\veps_\qq|\lesssim |\im\Sigma(\qq,\veps+\ii 0^+)|\ll \veps_\qq, k_B T$. C'est donc une excellente approximation que de remplacer $\veps$ par $\veps_\qq$ dans la loi de Bose au numérateur de l'intégrande dans (\ref{eq110}). Par analyticité dans le demi-plan complexe inférieur, le terme proportionnel à $\mathcal{G}(\qq,z-\ii\delta){\ycd\,\exp(-\ii zt/\hbar)}$ donne alors une contribution nulle à l'intégrale {\ycd pour $t>0$} (on le voit en refermant le chemin d'intégration par un demi-cercle à l'infini et en utilisant le théorème de Cauchy) et on aboutit à (\ref{eq111}). La même conclusion vaut dans le modèle de champ classique, où les deux fonctions exponentielles dans (\ref{eq110}) sont linéarisées.
...
\ee
où $\mH_4$ est l'hamiltonien volumique tel que $H_4=\mH_4/L^{d}$, la constante de couplage $\Lambda_*$ est celle de l'équation (\ref{eq032}), nous avons déduit l'élément de matrice dans l'état à deux phonons de l'expression (\ref{eq104}) de $H_4$ {\yc et} (\ref{eq052}) de $\delta\hat{\rho}(\rr)$, et nous avons obtenu le comportement d'échelle dans la limite (\ref{eq020}) au quatrième membre de l'équation (\ref{eq120}) comme dans la section \ref{sec0}. Comme nous le voyons sur la contribution (\ref{eq028}) de $H_3$ écrite pour $n=1$, la contribution de $H_4$ en dimension $d=2$ (mais pas en dimension trois) est sous-dominante et peut donc être négligée lorsque $\epsilon\to 0$. C'est ce que nous ferons désormais.
...
\Bigg\}
\ee
{\yc dans la limite thermodynamique avec $\mathcal{H}_3=H_3 L^{d/2}$, $|0\rangle$ le vide de phonons et,} pour chaque contribution, un domaine d'intégration en $\kk$ assurant que les vecteurs d'onde mis en jeu $\kk$, $\qq-\kk$, etc., satisfont à la coupure (\ref{eq103}). Sont inclus aussi les facteurs de symétrie $1/2$ lorsque les deux branches de la boucle ont la même orientation et sont donc physiquement identiques. Les sommes de Matsubara se calculent au moyen de l'expression générale \footnote{\label{notedemo} La démonstration se fait très bien par décomposition du sommande en éléments simples vis-à-vis de la variable $n$ et utilisation de l'identité $\lim_{N\to +\infty} \sum_{n=-N}^{N} 1/(2\ii\pi n-\vepsb)=-(\bar{n}_\veps+1/2)\ \forall \veps\in{\yc\mathbb{C}\setminus 2\ii\pi\mathbb{Z}}$ résultant des propriétés de la fonction digamma $\psi(z)$ {\yc comme la formule de réflexion}.}
\be
\label{eq122}
\sum_{n\in\mathbb{Z}} \frac{1}{(2\ii\pi n-\vepsb_1)(2\ii\pi n-\vepsb_2)} = -\frac{\bar{n}_{\veps_1}-\bar{n}_{\veps_2}}{\vepsb_1-\vepsb_2}
\ee
où les énergies réelles {\ycd ou même complexes} $\veps_j$ sont deux à deux distinctes, on a posé $\vepsb=\veps/k_B T$ et $\bar{n}_\veps=1/(\exp\vepsb-1)$. On utilise ensuite le fait que $\Omega/k_B T\in 2\ii\pi\mathbb{Z}$ pour faire disparaître cette variable de la loi de Bose, et la relation $\bar{n}_{-\veps}=-(1+\bar{n}_\veps)$ pour se ramener à de vrais nombres d'occupation des modes (à une énergie positive). Ainsi, par exemple:
\be
\label{eq123}
...
\ee
de façon que le point de branchement sur la figure \ref{fig5} reste fixe au lieu d'être rejeté à l'infini (ce qui ferait effectivement disparaître la contribution du lacet). On trouve alors une correction d'ordre $u^{-1/2}$ {\yc au signal de} la règle d'or de Fermi ; comme $u^{-1}\ll u^{-1/2}$ il semble entendu que l'on puisse garder cette correction sans avoir à calculer la fonction énergie propre à l'ordre suivant (l'ordre quatre) en $H_3$, {\yc même si nous verrons} dans la section \ref{sec4} qu'il en va autrement. Reportons les détails techniques dans l'\ref{app2} et donnons directement le résultat {\yc dans la théorie quantique}:
\be
\label{eq151}
...
\begin{SCfigure}
% Figure removed
\caption{Dans la théorie quantique et pour une fonction énergie propre calculée à l'ordre deux en $H_3$ et à l'ordre deux en $\epsilon=k_B T/mc^2$, écart du signal (\ref{eq012}) à {\yc l'exponentielle $\exp(-\Gamma_qt/2)$ de} la règle d'or de Fermi multiplié par $u^{1/2}$ (le paramètre universel $u$ est donné par (\ref{eq112}), {\yc le taux $\Gamma_q$ par (\ref{eq033})}) et représenté en fonction du temps réduit (\ref{eq130}) en partie réelle et partie imaginaire, pour des valeurs croissantes de $u$ (le superfluide bidimensionnel sous-jacent est en interaction de plus en plus faible) mais pour un nombre d'onde mis à l'échelle comme dans (\ref{eq150}) {\yc constant}, $Q=1$. Courbes pour des valeurs finies de $u$: obtenues par intégration numérique de l'expression (\ref{eq132}). Courbes pour $u=+\infty$: tirées de la prédiction analytique (\ref{eq151}) (elles correspondent simplement à $\re S(\check{t})$ et $\im S(\check{t})$).}
\label{fig4}
\end{SCfigure}
...
En conclusion, le calcul du signal avec la fonction énergie propre $\tilde{\Sigma}^{(2,2)}_{\yc\qb}(\zetat)$, par la seule faute de la partie réelle, échoue à rendre compte du premier écart à la règle d'or dans la limite $u\to +\infty$ où il avait toutes les chances d'exceller.
...
\ee
avec {\yc les nombres d'occupation (\ref{eq029}) étendus aux nombres d'onde négatifs et} l'intégrale angulaire\footnote{Dans l'intégrale, avant de prendre la limite $\epsilon\to 0$, on a posé $\tilde{\theta}=\theta/\epsilon$ et $\tilde{\phi}=(\theta-\theta')/\epsilon$, où $\theta$ est l'angle entre $\qq$ et $\kk$, $\theta'$ l'angle entre $\qq$ et $\kk'$.}
\be
\label{eq221}
...
\ee
que la variable déplacée $\zeta=z-\veps_\qq$ joue le rôle de $\ii\delta$ dans l'approximation du pôle (\ref{eq014}) {\yc et qu'une fonction énergie propre diffère d'un taux par un facteur $-\ii\hbar/2$}, nous arrivons sans hésitation à la forme
\begin{multline}
\label{eq257}
...
\end{multline}
Le seul piège réside dans la construction de la dépendance en $\zeta$ de l'amplitude de transition {\yc$\mA_{\ii\to\ff}$}. Le choix de $E_\ii$ ou $E_\ff$ comme énergie de référence n'est maintenant plus indifférent. Choisir $E_\ii$ comme au premier membre de (\ref{eq253}), c'est prendre le point de vue d'une transition $\ii\to\ff$ donc parcourir le temps dans le sens ordinaire ; le bon chemin d'intégration est précisément $C_+$, celui décrit par la variable $\zeta$ dans (\ref{eq012}) ; il faut donc remplacer $E_\ii$ par $\zeta+E_\ii$ dans le dénominateur d'énergie interne à {\yc$\mA_{\ii\to\ff}$}. En revanche, choisir $E_\ff$ comme au second membre de (\ref{eq253}) revient à prendre le point de vue inverse d'une transition $\ff\to\ii$ donc à remonter le temps; le bon chemin d'intégration de la fonction de Green est alors $C_-$ \cite{CCTbordeaux}, qui court dans le demi-plan inférieur parallèlement à l'axe réel de $-\infty$ à $+\infty$ et qui est décrit par la variable $-\zeta$ dans (\ref{eq012}) ; il faut donc remplacer $E_\ff$ par $-\zeta+E_\ff$ dans le dénominateur d'énergie interne à {\yc$\mA_{\ii\to\ff}$}. 

Pour trancher selon l'état intermédiaire $|\lambda\rangle$, il faut s'appuyer sur la propriété suivante. Par construction, la fonction $\Sigma^{(4)}(\qq,z)$ dépend du mode au travers de son vecteur d'onde $\qq$ mais pas de son énergie $\veps_\qq$ (lorsqu'on utilise comme variable l'énergie complexe originelle $z$), et ne peut donc pas comporter de dénominateur d'énergie contenant $\veps_\qq$. C'est le contraire qui se produit si l'on utilise la variable déplacée $\zeta$. Ceci veut dire que, si dans le sens direct de l'amplitude de transition, l'action sur $|\ii\rangle$ de l'opérateur $\mH_3^{(\pm)}$ ne fait pas disparaître le phonon de vecteur d'onde $\qq$, {\ycd son énergie $\veps_\qq$ apparaît dans $E_\lambda$ donc disparaît dans $\zeta+E_\ii-E_\lambda$, et} il faut utiliser le sens inverse de l'amplitude de transition, cette règle étant à appliquer état intermédiaire par état intermédiaire. D'où notre prescription finale sur l'amplitude de transition à inclure dans (\ref{eq257}):\footnote{{\ycd En réalité, les processus collisionnels $n\to n'$ à $n$ phonons entrants $\qq_i$ et $n'$ phonons sortants $\qq'_j$ se produisent dans un bain thermique, et les modes des phonons virtuels $\kk$ (ni entrants ni sortants) ne sont pas initialement vides. Il faudrait donc tenir compte des facteurs d'amplification bosoniques correspondants $(n_\kk+1)^{1/2}$ et $n_\kk^{1/2}$ dans l'expression (\ref{eq258}) de l'amplitude de transition, et inclure la contribution des nouveaux processus autorisés, dans lesquels un phonon virtuel $\kk$ est annihilé avant d'être recréé (ce qui est interdit dans le vide). Cependant, ces deux modifications se compensent exactement et l'expression (\ref{eq258}) reste inchangée. Vérifions-le sur un terme de la première somme dans (\ref{eq258}). Pour cela, décrivons plus finement la transition correspondante $|\ii\rangle\to|\lambda\rangle\to|\ff\rangle$ en notant $|\ii\rangle=|\ii_1\rangle\otimes|\ii_2\rangle\otimes|n_\kk:\kk\rangle$ et $|\ff\rangle=|\ff_1\rangle\otimes|\ff_2\rangle\otimes|n_\kk:\kk\rangle$, où $|\ii_1\rangle$ ($|\ii_2\rangle$) sont les photons entrants annihilés par la première (la seconde) action de $\mathcal{H}_3$, $|\ff_1\rangle$ ($|\ff_2\rangle$) sont les phonons sortants créés par la première (la seconde) action de $\mathcal{H}_3$ et $|n_\kk:\kk\rangle$ l'état de Fock à $n_\kk$ quanta dans le mode du phonon virtuel, si bien que l'état intermédiaire s'écrit $|\lambda\rangle=|\ff_1\rangle\otimes|\ii_2\rangle\otimes|n_\kk+1:\kk\rangle$
(nous ne tenons pas compte des autres modes, spectateurs, ni de l'occupation thermique des modes entrants et sortants puisque leurs facteurs bosoniques sont inclus dans (\ref{eq250},\ref{eq257})). Dans l'exemple pris, $\qq\notin|\lambda\rangle$ donc $\qq\in|\ii_1\rangle$.
Les composantes de $\mathcal{H}_3$ assurant les transitions $|\ii\rangle\to|\lambda\rangle$ et $|\lambda\rangle\to|\ff\rangle$ s'écrivent respectivement $W_1=\mathcal{A}_1\hat{b}^\dagger_\kk(\prod_{\qq'_j\in|\ff_1\rangle} \hat{b}^\dagger_{\qq'_j})(\prod_{\qq_i\in|\ii_1\rangle} \hat{b}_{\qq_i})$ et $W_2=\mathcal{A}_2(\prod_{\qq'_j\in|\ff_2\rangle} \hat{b}^\dagger_{\qq'_j})(\prod_{\qq_i\in|\ii_2\rangle} \hat{b}_{\qq_i})\hat{b}_\kk$. 
En présence du bain thermique, (i) le numérateur $\langle\ff|\mH_3^{(\pm)}|\lambda\rangle\langle\lambda|\mH_3^{(\pm)}|\ii\rangle=\langle\ff|W_2|\lambda\rangle\langle\lambda|W_1|\ii\rangle$ devient $\mathcal{A}_1\mathcal{A}_2(1+n_\kk)$, (ii) le nouveau processus associé, correspondant à l'ordre d'action inverse de $W_1$ et $W_2$, est de numérateur $\langle\ff|W_1|\lambda'\rangle\langle\lambda'|W_2|\ii\rangle=\mathcal{A}_1\mathcal{A}_2n _\kk$, (iii) le nouvel état intermédiaire $|\lambda'\rangle=|\ii_1\rangle\otimes|\ff_2\rangle\otimes|n_\kk-1:\kk\rangle$ contient le phonon $\qq$ donc son dénominateur d'énergie doit être pris dans la seconde somme de (\ref{eq258}), et se trouve être l'exact opposé du dénominateur de l'ancien processus, le calcul donnant $-\zeta+E_\ff-E_{\lambda'}=-(\zeta+E_\ii-E_\lambda)$. En définitive, les contributions linéaires en $n_\kk$ de l'ancien et du nouveau processus se compensent exactement dans l'amplitude de transition $\mathcal{A}_{\ii\to\ff}(\qq,z)$, ce qui autorise à oublier le fond thermique comme il est fait dans (\ref{eq258}). Un raisonnement similaire, mais limité aux processus $2\to 2$ sur la couche d'énergie, figure déjà dans la référence \cite{Annalen}.}}
\be
\label{eq258}
...
Notre prescription (\ref{eq257},{\yc\ref{eq258}}) donne des résultats simples mais en général différents de ceux, exacts, de la théorie diagrammatique à l'ordre quatre, compte tenu des expressions des sommes doubles de Matsubara dans l'\ref{app3}. Cependant, de façon remarquable, 
une fois que ces sommes ont été remplacées par leurs expressions simplifiées (\ref{eq213},\ref{eq233}), ce qui est légitime aux petits angles, elle est en accord parfait avec la valeur de $\Sigma^{(4)}$ prédite par les diagrammes de la figure \ref{fig8}; on le vérifie à l'aide de la table de correspondance \ref{table1}, obtenue au vu des amplitudes de couplage dans (\ref{eq212},\ref{eq232}) et des dénominateurs d'énergie dans (\ref{eq213},\ref{eq233}), et qui permet d'associer à chaque ligne interne sur cette figure un phonon entrant $\qq_i$, sortant $\qq'_j$ ou virtuel (combinaison linéaire des précédents) d'un processus $n\to n'$ donc de donner un sens physique clair au diagramme correspondant.\footnote{{\ycd La vérification est facilitée par le résultat suivant: selon notre prescription à la Landau, chaque contribution diagrammatique aux sommes de Matsubara (\ref{eq213},\ref{eq233}) vaut $\sigma/\prod_j (\zeta-\zeta_j)$ avec $\sigma=s(f_1 f_2-f_3f_4)$, où (i) le signe global $s$ est le produit des orientations $s_i$ des lignes de phonons virtuels (en effet, si un phonon virtuel remonte le temps, son énergie apparaît avec un signe plus au dénominateur dans le facteur $(\zeta-\zeta_j)$ correspondant, donc il contribue au travers de la seconde somme de (\ref{eq258}) et il faut inclure un signe moins global), (ii) chaque $f_k$ est le produit des facteurs d'amplification bosoniques des lignes reliées au sommet $k$ (la ligne $i$ contribue à $f_k$ par un facteur $\bar{n}_{\veps_i}$ si elle va vers le sommet, ou un facteur $1+\bar{n}_{\veps_i}$ si elle s'en éloigne, ce qui s'écrit $s_i(1+\bar{n}_{s_i\veps_i})$ si elle est à droite du sommet et $s_i\bar{n}_{s_i\veps_i}$ si elle est à gauche), à l'exclusion cependant des lignes externes et des lignes de phonons virtuels, dont les nombres d'occupation n'apparaissent pas dans la prescription (\ref{eq257},\ref{eq258}). Les sommets des diagrammes sont numérotés de gauche à droite ; dans le cas des diagrammes à pont, il faut lever la dégénérescence de numérotation des sommets internes en faisant tourner la ligne de phonon verticale d'un petit angle vers la gauche (type I, orientation effective $s_5$) ou vers la droite (type II, orientation effective $-s_5$). On trouve ainsi, en sortant tous les signes, $\sigma_B=s_1s_2s_3s_4s_5[(1+\bar{n}_{s_1\veps_1})(1+\bar{n}_{s_3\veps_3})(1+\bar{n}_{s_4\veps_4})-\bar{n}_{s_1\veps_1}\bar{n}_{s_3\veps_3}\bar{n}_{s_4\veps_4}]$ pour les diagrammes à boucle interne ($s=s_2s_5$), $\sigma_{P^{\rm I}}=s_1s_2s_3s_4s_5[(1+\bar{n}_{s_2\veps_2})(1+\bar{n}_{s_3\veps_3})(1+\bar{n}_{s_5\veps_5})-\bar{n}_{s_2\veps_2} \bar{n}_{s_3\veps_3} \bar{n}_{s_5\veps_5}]$ et $\sigma_{P^{\rm II}}=s_1s_2s_3s_4(-s_5)[(1+\bar{n}_{s_1\veps_1})(1+\bar{n}_{s_4\veps_4})(1+\bar{n}_{-s_5\veps_5})-\bar{n}_{s_1\veps_1} \bar{n}_{s_4\veps_4} \bar{n}_{-s_5\veps_5}]$ pour les diagrammes à pont ($s=s_1s_4$ pour le type I, $s=s_2s_3$ pour le type II). Dans la limite $\epsilon\to 0$, on retrouve alors exactement les formes simplifiées (\ref{eq213},\ref{eq233}) si l'on utilise la conservation de l'énergie au deuxième sommet, respectivement $s_2\veps_2=s_3\veps_3+s_4\veps_4$, $s_1\veps_1=s_3\veps_3+s_5\veps_5$ et $s_2\veps_2=s_4\veps_4+(-s_5)\veps_5$, au travers de l'identité $\bar{n}_{a+b}=\bar{n}_a\bar{n}_b/(1+\bar{n}_a+\bar{n}_b)$, ainsi que l'égalité $s_2=s_5$ dans les diagrammes à boucle interne.}} En définitive, la prescription (\ref{eq257},{\yc\ref{eq258}}) conduit en $d=2$ à une expression rigoureusement équivalente mais beaucoup plus compacte de $\Sigma^{(4)}$ à l'ordre dominant en $\epsilon$:
\be
\label{eq269}
\boxed{
...
\ee
et conclut à l'absence de divergence ({\yc pour avoir $J_{\rm div}\neq 0$,} il faudrait que la branche supérieure gauche de la boucle à pont soit orientée vers la gauche, et que les deux branches inférieures soient d'orientations opposées, ce qui ne se produit pas sur la figure \ref{fig8}).
...
\be
\label{eq294}
\zetac^{n\phi}_{\qb} = \check{\Sigma}_{\qb}^{(2,2)}(\ii 0^+)-\frac{2}{\qb^{2}}\int_{\qb}^{\eta}\kb\,\dd\kb \int_0^{\eta-\kb}\kb'\dd\kb' \frac{(\kb-\qb)(\kb'+\qb) {\yc(\bar{n}_{k-q}^{\rm lin}-\bar{n}_{k}^{\rm lin})(\bar{n}_{k'+q}^{\rm lin}-\bar{n}_{k'+k}^{\rm lin})}}{\zetac^{n\phi}_{\qb}-\frac{9}{8}\gamma u \epsilon^2\kb\kb'(\kb-\qb)(\kb'+\qb)}
\ee
cesse d'admettre une solution dans le demi-plan supérieur lorsque $\gamma u\epsilon^2>h(\qb)$, où $u$ est le paramètre universel (\ref{eq112}) et $h(\qb)$ est fonction exclusivement de $\qb$ (et de la coupure $\eta$ si on ne la fait pas tendre vers l'infini dans la théorie quantique), comme on le voit en prenant formellement la limite $\gamma u \epsilon^2\to +\infty$ dans (\ref{eq294}). Pour le champ classique, nous trouvons numériquement que $h(\qb)$ est une fonction décroissante de $\qb$; le problème d'applicabilité de l'ordre quatre pour de faibles valeurs de $\epsilon$ subsiste donc dans la limite $\qb\to 0$.\footnote{{\ycd Pour le champ classique ($\eta=1$), on a l'équivalent $h(\qb)\sim \frac{8}{9}(\pi^2/2\qb)^{4/3}$ lorsque $\qb\to 0$ (en pratique $\qb^{1/3}\ll 1$). Ceci s'obtient en écrivant le second membre de (\ref{eq294}) à l'ordre dominant en $\qb$ (en particulier, on remplace $\qb$ par zéro dans la première borne d'intégration et dans le dénominateur de l'intégrande, et on remplace le numérateur par son approximation linéaire en $\qb$) puis en montrant que l'intégrale double est dominée par un voisinage de $(\kb,\kb')=(0,0)$ de rayon $h(\qb)^{-1/4}\ll 1$ lorsque $\qb\to 0$, ce qui autorise à remplacer les bornes supérieures d'intégration par $+\infty$. On aboutit à une intégrale double calculable analytiquement et à l'équation $\zetac^{n\phi}_{\qb}=-\ii+(1+\ii)\pi^2/[2\qb(\frac{9}{8}\gamma u\epsilon^2)^{3/4}(\zetac^{n\phi}_{\qb})^{1/4}]$. Il reste à traduire le fait que le pôle non physique est de partie imaginaire nulle au seuil de disparition.}}  {\ycd Pour le voir plus simplement, nous pouvons évaluer  $\check{\Sigma}_{\qb}^{(4)}(\ii 0^+)$ (contribution d'ordre 4 en $H_3$ considérée à tous les ordres en $\epsilon$) dans la limite $\epsilon\to 0$. Il suffit pour cela de remplacer $\check{\zeta}_{\qb}^{n\phi}$ par $\ii 0^+$ dans l'intégrale double au second membre de (\ref{eq294}), que l'on sait alors calculer explicitement pour le champ quantique de coupure $\eta\to+\infty$:
\be
\label{eq296}
\check{\Sigma}_{\qb}^{(4)}(\ii 0^+)\underset{\epsilon\to 0}{\sim} \frac{16 g_2(\eee^{-\qb})}{9\gamma u \epsilon^2 \qb^2} \quad\mbox{d'où}\quad \Sigma_{\qb}^{(4)}(\ii 0^+)\underset{\epsilon\to 0}{\sim} k_B T \left[\frac{(1+\Lambda)^2}{\gamma^{3/2}\rho\xi^2}\right]^2 \frac{2 g_2(\eee^{-\qb})}{\pi^2\qb}
\ee
À basse température $\epsilon\ll 1$, l'ordre quatre en $H_3$ (\ref{eq296}) n'est donc une petite correction à l'ordre deux (\ref{eq141}) que si $\gamma u\epsilon^2$ est beaucoup plus grand qu'une certaine fonction de $\qb$, à savoir $1/\qb^2$ à faible $\qb$.\footnote{{\yc Le résultat (\ref{eq296}) n'est pas en contradiction avec l'équation (\ref{eq280}) car l'ordre des limites $\zetat\to 0$ et $\epsilon\to 0$ n'est pas le même.}}}

\paragraph{Conclusion} À nombre d'onde réduit $\qb$ fixé du mode de phonon {\yc dont on étudie l'amortissement}, la condition d'applicabilité de la théorie diagrammatique à l'ordre quatre en $H_3$ n'est pas seulement que le paramètre universel $u$ soit assez grand (régime d'interaction faible attendu) mais aussi que le produit ${\yc\gamma}u\epsilon^2$ soit assez élevé. Elle présente donc une incompatibilité de fond avec la limite étudiée dans cette article, celle de basse température $\epsilon\to 0$ à force des interactions fixée. Ainsi, pour les simulations numériques de la figure \ref{fig2}, où $\qb=1/4$ et $\gamma=1$, nous trouvons, en suivant par la méthode de Newton jusqu'à disparition la racine de l'équation (\ref{eq294}) écrite en champ classique, qu'il faudrait avoir $u\epsilon^2>10,\!6$ soit $\rho\xi^2>23,\!4$ pour $\epsilon=1/2$ et $\rho\xi^2>52,\!6$ pour $\epsilon=1/3$. Or $\rho\xi^2$ vaut au plus $10$ sur la figure \ref{fig2} ; la théorie à l'ordre quatre développée dans cette section est donc inapplicable.


\section{Théories non perturbatives en $H_3$}
\label{sec4}

Comme nous l'avons vu dans la section \ref{sec3}, la théorie à l'ordre quatre en $H_3$ n'est pas utilisable en dimension $d=2$ dans la limite de basse température $\epsilon=k_B T/mc^2\to 0$. Pour rendre compte de l'écart entre la théorie à l'ordre deux en $H_3$ et les simulations sur le temps de décroissance à $1/\eee$ du mode de phonon $\qq$ et sur la partie réelle du signal associé (\ref{eq012}) dans le régime d'interaction faible $(\rho\xi^2)^{-1}\ll 1$, voir les figures \ref{fig1}d et \ref{fig2}d, nous proposons ici une théorie non perturbative guidée par des considérations physiques. En bref, nous {\yc omettons la partie finie mais} nous régularisons la partie divergente de l'ordre quatre de la fonction énergie propre en donnant une durée de vie finie aux phonons dits virtuels (ou intermédiaires) des processus collisionnels à quatre phonons ; ceci fait apparaître une contribution qui se révèle être d'ordre deux en $H_3$ et qui vient donc corriger l'expression de l'énergie propre à l'ordre dominant en la force des interactions. Nous présentons une régularisation de type physique atomique dans la section \ref{sec4.1} (les phonons virtuels $\KK$ acquièrent une énergie complexe $\veps_\KK\to \veps_\KK +\Sigma_\KK^{(2,2)}(\ii 0^+)=\veps_\KK-(\ii\hbar/2)\Gamma_\KK$) et une régularisation de type matière condensée dans la section \ref{sec4.2} (on garde dans l'énergie complexe des phonons virtuels la dépendance en fréquence de la fonction énergie propre $\Sigma_\KK^{(2,2)}(\zeta)$). Enfin, dans la section \ref{sec4.3}, nous donnons la nouvelle forme de la théorie dans la limite $(\rho\xi^2)^{-1}\to 0$, commune aux deux régularisations et qui remplace avec succès la règle d'or de Fermi.
...
\ee
l'ordre des facteurs au dénominateur de (\ref{eq243}) respectant celui de (\ref{eq233}) ; ici $\tilde{\Gamma}_{\bar{K}}=\hbar\Gamma_{\KK}/k_BT\epsilon^2$ dans l'esprit de l'équation (\ref{eq022}). {\yc En revanche, la quantité $\zeta_3$ ne met en jeu aucun phonon virtuel et n'est pas modifiée.} L'ancienne partie divergente de l'intégrale angulaire (\ref{eq282}) devient pour $\im\zetat>0$:\footnote{On continue à linéariser autour du même point de triple intersection des lieux d'annulation en regroupant les corrections complexes avec la variable $\zetat$ si bien qu'au second membre de (\ref{eq282}), on remplace $\zetat$ par $\zetat+(\ii/2)\tilde{\Gamma}_{\kb-\qb}$ dans le premier facteur et par $\zetat+(\ii/2)\tilde{\Gamma}_{\kb'}$ dans le deuxième facteur au dénominateur.}
\be
\label{eq302}
...
\right.
\ee
Si l'on quitte le temps adimensionné $\check{t}$ (\ref{eq130}), ce qui fait apparaître la quantité $\Gamma_u$ déja introduite dans (\ref{eq113}) {\yc puisque $\check{t}=\qb\Gamma_u t/2$}, on trouve que ceci correspond à un taux d'amortissement corrigé tendant vers zéro linéairement en $\qb$ :
\be
\label{eq313}
\boxed{
\Gamma_q^{{\rm corr}\,\infty} \underset{\qb\to 0}{\sim} A \Gamma_u \qb = A \frac{9k_B T\epsilon^2(1+\Lambda)^2}{4\pi\hbar(3\gamma)^{1/2}\rho\xi^2} \qb \sim \left\{ \begin{array}{ll} \displaystyle\frac{A}{\eta} \Gamma_q^{{\yct\rm clas}} \simeq 0,\!4\,\Gamma_q^{{\yct\rm clas}} & \mbox{(champ classique)} \\ \displaystyle\frac{A}{2\zeta(2)} \Gamma_q \simeq 0,\!3\,\Gamma_q & \mbox{(champ quantique)}\end{array} \right. }
\ee
représenté en pointillé sur la figure \ref{fig9} pour le champ quantique et sur la figure \ref{fig3}a pour le champ classique. Ici, $\Gamma_q^{{\yct\rm clas}}$ et $\Gamma_q$ sont les taux d'amortissement de la règle d'or pour le champ classique (\ref{eq113}) et pour le champ quantique (\ref{eq033}). On voit donc que la pente à l'origine du taux corrigé est considérablement plus faible que celle de la règle d'or. Ensuite, dans la limite $\qb\to +\infty$, bien entendu pour le champ quantique de coupure infinie, l'intégrale au second membre de (\ref{eq311}) tend uniformément vers zéro dans le demi-plan supérieur et la décroissance du signal devient purement exponentielle, avec un taux équivalent à celui de la règle d'or: \footnote{Ce résultat est aussi celui de la température nulle, ce qui explique pourquoi le troisième membre de (\ref{eq315}) est en fait indépendant de la température lorsqu'on revient au nombre d'onde $q$ non adimensionné. Comme il résulte de l'hamiltonien effectif de l'hydrodynamique quantique, il ne vaut alors qu'à l'ordre dominant en $q\xi$ c'est-à-dire dans la partie linéaire {\yc de la branche acoustique}.}
\be
\label{eq315}
...
\paragraph{Se ramener à une seule intégrale} Dans la contribution de Landau à (\ref{eq131}), c'est-à-dire {\yc dans} la seconde intégrale, nous effectuons le changement de variable $x=\kb(\kb+\qb)$ soit $\kb=(x+\qb^2/4)^{1/2}-\qb/2$, où $x\in\mathbb{R}^+$. Dans la contribution de {\yc Belyaev}, l'intégrande est invariant par changement de $\kb$ en $\qb-\kb$, ce qui autorise à limiter l'intégration sur $\kb$ à l'intervalle $[0,\qb/2]$ au prix d'une multiplication par un facteur $2$; nous effectuons alors le changement de variable devenu injectif $x=\kb(\kb-\qb)$ soit $\kb=\qb/2-(\qb^2/4+x)^{1/2}$, où $x\in[-\qb^2/4,0]$; nous utilisons au préalable l'identité $\bar{n}_k^{\rm lin}=-(1+\bar{n}_{-k}^{\rm lin})$ pour donner au facteur d'amplification bosonique la même structure de différence de nombres d'occupation que dans Landau. Nous {\yc arrivons} dans les deux cas au même intégrande, si ce n'est que {\yc Belyaev} fait apparaître un facteur global $(-x)^{1/2}/(\zetac/u-x)^{1/2}$ là où Landau contient un facteur $x^{1/2}/(x-\zetac/u)^{1/2}$ ; on peut leur donner cependant l'expression commune $(x-\ii 0^+)^{1/2}/(x-\zetac/u)^{1/2}$ au moyen des relations $(-x)^{1/2}=\ii(x-\ii 0^+)^{1/2} \forall x<0$, $x^{1/2}=(x-\ii 0^+)^{1/2} \forall x>0$, $(\zetac/u-x)^{1/2}=\ii(x-\zetac/u)^{1/2}\forall x\in\mathbb{R}$ sachant que $\im(\zetac/u)>0$. Nous aboutissons à l'intégrale unique cherchée:
\be
\label{eq1001}
...
\ee
sachant que $z=\eee^{\ii(\phi+\pi)}x$, $x>0$. L'intégrande dans (\ref{eq2004}) est non nul seulement pour $K<x^{1/2}$, ce qui incite à poser $K=x^{1/2}\sin\alpha$, $\alpha\in [0,\pi/2]$, d'autant plus que ceci fait disparaître les racines carrées au dénominateur: nous retrouvons $S_{\rm lacet}(\check{t})$ dans (\ref{eq152}). Dans l'équation implicite définissant le pôle, réécrite en mettant à part son approximation d'ordre zéro, $\zetac_{\qb}-\check{\Sigma}^{(2,2)}_{\qb\downarrow}(0)=\check{\Sigma}^{(2,2)}_{\qb\downarrow}(\zetac_{\qb})-\check{\Sigma}^{(2,2)}_{\qb\downarrow}(0)$, il suffit au second membre d'utiliser (\ref{eq2003}) avec $\zetac=-\ii\pi^2/3$; le choix commode $\phi={\ycd(-\pi)}^{+}$ (dont le résultat ne dépend pas) redonne $\delta\zetac_{\qb}$ dans (\ref{eq154}). Enfin, pour obtenir le premier écart à un du résidu, il suffit de dériver (\ref{eq2003}) par rapport à $\zetac$ (sous le signe intégral au second membre), de remplacer $\zetac$ par $-\ii\pi^2/3$ puis de prendre $\phi={\ycd(-\pi)}^+$~: {\yct on retrouve} $\delta\check{Z}$ dans (\ref{eq154}).
...
\eea
avec les énergies algébriques des états intermédiaires repérés par les droites rouges sur la partie droite de la figure \ref{fig7}, $z_1=s_1\veps_1+s_2\veps_2$ et $z_2=s_3\veps_3+s_4\veps_4$ communes aux deux types, $z_4=s_2\veps_2+s_3\veps_3+s_5\veps_5$ de type I et enfin $z_3=s_1\veps_1+s_4\veps_4-s_5\veps_5$ de type II. Ces variables ne sont pas indépendantes puisque $z_1+z_2=z_3+z_4$. Il reste à prendre la limite des petits angles $\epsilon\to 0$, dans laquelle {\yc $z=\veps_q+O(k_B T \epsilon^2)$ et} $z_j=\veps_q+O(k_B T \epsilon^2)$, $1\leq j\leq 4$. Alors $s_3\veps_3+s_5\veps_5=s_1\veps_1+O(k_BT\epsilon^2)$ et l'on peut, à l'ordre dominant, remplacer $\bar{n}_{s_3\veps_3+s_5\veps_5}$ par $\bar{n}_{s_1\veps_1}$ dans (\ref{eq3012}) ; en regroupant par nombre d'occupation et en simplifiant, on retrouve la première contribution au second membre de (\ref{eq233}). De même, on remplace $\bar{n}_{s_5\veps_5-s_4\veps_4}$ par $\bar{n}_{-s_2\veps_2}$ à l'ordre dominant dans (\ref{eq3013}), ce qui reproduit la seconde contribution à (\ref{eq233}).
...
\ee
Ceci résulte de deux phénomènes. (i) À nombre d'onde réduit $\kb$ fixé, l'intégrale angulaire (\ref{eq221}) dans les diagrammes à boucle interne diverge en $\zetat=A_1^B$ avec un exposant $3/2$ car, pour cette valeur de $\zetat$, le dénominateur de l'intégrande admet $\thetat=0$ comme racine quadruple. Pour le voir quantitativement, on peut utiliser l'identité $\int_{\mathbb{R}}\dd\thetat/(z-\thetat^2)^2=-\ii\pi\,{\ycd\sig(\im z)}/(2 z^{3/2}), \forall z\in\mathbb{C}\setminus\mathbb{R}$, avec $z=(\zetat-A_1^B)/B_1^B$. (ii) Cette divergence {\yc en} $(\zetat-A_1^B)^{-3/2}$ se produit à une position $A_1^B$ dépendant de $\kb$, comme le montre l'équation (\ref{eq222}), donc elle est en général effacée par l'intégration sur $\kb$ à $\zetat$ fixé dans (\ref{eq220}), sauf si $\zetat$ coïncide avec un extrémum parabolique de $A_1^B(\kb)$ sur le domaine d'intégration $\mathcal{D}_1^B$. Pour le voir, choisissons un nombre d'onde réduit $\kb_0$ arbitraire dans $\mathcal{D}_1^B$, posons $\zetat=w+A_1^B(\kb_0)$ et passons à la limite $w\to 0$ par parties imaginaires positives dans (\ref{eq220}): le résultat ne peut diverger que si la valeur de l'intégrale 
\be
\label{eq4005}
...
\ee
ce que l'on peut reporter dans l'intégrande de (\ref{eq220}), en remplaçant de plus $\kb$ par $\kb_0=\qb/2$ dans tous les autres facteurs et dans la borne supérieure d'intégration sur $\kb'$. {\yc On change ensuite l'ordre d'intégration.} Dans l'intégrale {\yc maintenant intérieure} sur $\kb$ à $\kb'$ fixé, il ne reste plus que
\be
\label{eq4003}
...
Pour terminer, expliquons pourquoi les diagrammes à pont ne contribuent pas à la divergence linéaire (\ref{eq4000}) sur l'exemple du diagramme $P_1$ (tous les $s_i=1$, $\mathcal{D}_1^P=[0,\qb]$ et $\mathcal{D}_2^P(\kb)=[0,\kb]$, type II). Comme l'avait remarqué la section \ref{sec3.3}, l'intégrale angulaire (\ref{eq243}) diverge linéairement en {\yc un certain} $\zetat=X_0(\kb,\kb')$ si les lieux d'annulation des trois facteurs au dénominateur de (\ref{eq243}) se croisent en un même point dans le plan $(\tilde{\theta},\tilde{\theta}')$. La condition de triple intersection {\ycd dans le cas général $X_0\neq 0$} s'obtient en remplaçant tous les coefficients $A_i^P$ par $A_i^P-X_0$ dans (\ref{eq281}); l'équation du second degré sur $X_0$ qui en résulte admet la solution nulle évidente, à laquelle s'était implicitement limitée la section \ref{sec3.3}, et une deuxième racine réelle, $X_0(\kb,\kb')=-3\gamma \kb(\kb-\qb)\kb'(\kb'-\qb)/(2\qb)$ ; cette dernière conduit à une triple intersection dans le plan $(\tilde{\theta},\tilde{\theta}')$, aux points de coordonnées $\pm[\sqrt{3\gamma}/(2\qb)]((\qb-\kb)(\qb-2\kb'),-(\qb-2\kb)(\qb-\kb'))$, si on ajoute les contraintes $\kb>\qb/2$ et $\kb'<\qb/2$ pour avoir les bons signes dans les coefficients des petits écarts $(\tau,\tau')$ à ces points (sans quoi, comme dans (\ref{eq282}), il n'y a pas de divergence). Alors, en procédant comme dans (\ref{eq282}), on obtient la partie divergente de l'intégrale angulaire (pour $\zetat\to X_0$ par parties imaginaires positives),
\be
\label{eq4006}
...
\bibitem{SJ} R. Saint-Jalm, \href{https://theses.hal.science/tel-03116129}{\it Exploration de la physique à deux dimensions avec des gaz de Bose dans des potentiels à fond plat: ordre en phase et symétrie dynamique}, thèse de doctorat de {\yct l'}École normale supérieure et de l'Université PSL (Paris, octobre 2019).
\bibitem{broui} H. Kurkjian, Y. Castin, A. Sinatra, \href{https://doi.org/10.1016/j.crhy.2016.02.005}{\g{Brouillage thermique d'un gaz cohérent de fermions}}, Comptes Rendus Physique {\bf 17}, 789 (2016).
