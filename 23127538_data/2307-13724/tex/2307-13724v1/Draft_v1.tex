...
The corresponding deformation parameter $\langle \phi_a\rangle$ is the vacuum expectation value (VEV) for the lowest component of the background vector multiplet $\mathcal{V}_I$ for the SO$(3)_I$ symmetry acting on the Higgs branch chiral ring. Selecting the third component $\langle\phi_3\rangle$, we can identify the inverse gauge coupling $\frac{1}{g^2}$ of the IR free gauge theory as $\langle \phi_3\rangle\equiv h\sim  \frac{1}{g^2}$. %can be thought of as the vacuum expectation value (VEV) for the lowest component of the background vector multiplet $\mathcal{V}_I$
This deformation explicitly breaks the SO$(3)_I$ symmetry to its Cartan, which at low energies and at the origin of the Coulomb branch (CB) describes the topological U$(1)_I$ symmetry of the gauge theory with conserved current 
...
This follows because this operator carries an index $i$ in the adjoint of the SU$(2)_R$ R-symmetry, breaking it down to its U$(1)_R$ Cartan. Both the gaugino and the scalar gaugino acquire a mass proportional to $d/h$ and can be integrated out in the weak coupling regime $|d|\ll h^2$, hence the theory flows to pure SU(2) Yang-Mills. 
The $E_1$ theory enjoys a UV dual description, which can be interpreted as the continuation past infinite coupling of the weakly coupled gauge theory description. 
If we start from the SCFT point, this phase is reached by flowing in the IR after having turned on a negative value for $h$. 
The resulting phase is however related to the weakly coupled description by a $\mathbb{Z}_2$ reflection symmetry, which sends $h\to -h$. 
This symmetry actually belongs to the global symmetry group of the fixed point 
and it is the residual Weyl group of the SO$(3)_I$ topological symmetry.
...
This changes both the sign of the Chern-Simons level associated with the background vector field of the U$(1)_I$ global symmetry and of the residual U$(1)_R$ symmetry, which can both be shown to be non-zero in the pure YM phase. 
...
The string theory analysis revealed an instability on the Higgs branch as one turns on the SUSY breaking deformation $d$, which was confirmed by analyzing the effect of the deformation on a generic point of the Higgs branch of the SCFT. 
...
A crucial ingredient in determining the CS levels for background instantonic symmetry of the $E_1$ theory was the existence of UV self-duality relating the weakly coupled description of the $E_1$ theory with its continuation past infinite coupling. 
...
In section \ref{Weak coupling analysis}, we study the effect of a generic SUSY breaking deformation involving the background vector multiplet of the flavor symmetry on a generic SYM theory coupled to $F$ hypermultiplets. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
Consider a 5d $\mathcal{N}=1$ gauge theory with a simple gauge algebra $\mathfrak{g}$, assumed to be simple, coupled to $n_H$ hypermultiplets in representation $R_i$ of $\mathfrak{g}$, with $i\in\{1,2,\cdots,n_H\}$. 
The Lagrangian for this system is comprised of three terms: the supersymmetric YM Lagrangian $\lag_{\text{SYM}}$, a CS term together with its SUSY completion $\lag_{SCS}$, and the hypermultiplet Lagrangian $\lag_{\text{matter}}$:
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
An important tool to understand the dynamics of 5d supersymmetric comes from the prepotential. 
In the context of 5d SUSY gauge theories, the prepotential describes the perturbative dynamics of the theory. The moduli space of vacua of a 5d gauge theory consists of a Coulomb and a Higgs branch. 
...
\end{equation}
The low energy dynamics of type IIB $(p,q)$ webs, composed of junctions of $(p,q)$ 5-branes, can reproduce the dynamics of 5d gauge theories. 
...
In doing so, we will generate integration constants. 
These do not change either the metric on the CB of the theory nor the masses of the monopole strings, so they are often ignored in the literature. However, such integration constants carry important physical information on phase structures as discussed in \cite{BenettiGenolini:2019zth}. 
...
Moreover, although the perturbative prepotential can, in principle, get modified by non-perturbative hypermultiplet contributions, the $(p,q)$ web construction still allows us to access the value of the tensions of the monopole strings. 
...
Firstly, at strong coupling, their symmetry enhances SO$(3)_I$. 
...
The theory in question admits a brane web construction, that on a generic point of the Coulomb branch phase is given by
\begin{equation}
    \begin{array}{c}
    \begin{scriptsize}
...
In order to write down the prepotential invariant under the UV duality eq. \eqref{duality map SU(3) level 3}, we can restore this integration constant into the IMS prepotential
\begin{equation}
...
\subsubsection*{Complete prepotential}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The Weyl group of the global symmetry of the fixed point is implemented in the $(p,q)$-web description by S-duality, since the S-dual webs host the same low energy field theory. 
...
When this symmetry is broken to its Cartan by the deformation, we obtain $N^2-1$ BPS particles charged under the U$(1)_R$ global symmetry, being the gauginos in the adjoint\footnote{For this reason, there is no shift for the CS level of the dynamical gauge field, since $d_{abc}=0$.} of SU$(N)$. 
...
There are some possibilities on interpretations of the 't Hooft anomaly at the superconformal point \cite{BenettiGenolini:2020doj,Apruzzi:2021vcu} involving two-group symmetries \cite{Apruzzi:2021vcu,Genolini:2022mpi,DelZotto:2022joo}.
...
Also, the R-symmetry level jumps between the two weakly coupled phases. Integrating out the gaugino induces a shift on the CS level for the background gauge field $A_R$ of the U$(1)_R$ symmetry, which in the G$_2$ side reads
\begin{equation}
    k_R=-7\, \text{sgn}(d)\; ,
\end{equation}
...
Firstly, the global symmetry at the fixed point is SO$(3)_I\times \text{SU}(2)_A$, where the first group is associated with the enhancement of the instantonic U$(1)_I$ symmetry of the Sp$(N)$ theory and the second with the symmetry under which the anti-symmetric hypermultiplet transforms. 
...
Starting from the perturbative prepotential in eq.~\eqref{Sp(N)+1AS}, we need to introduce two invariant CB parameters, since the theory enjoys an U$(1)_I\times\text{SU}(2)_A$ global symmetry, that it is known to enhance to SO$(3)_I\times\text{SU}(2)_A$ at the fixed point. 
...
three choices: giving a VEV to the D-term for SU$(2)_A$, breaking SUSY and SU$(2)_A\rightarrow \text{U(1)}_A$, giving a VEV to the D-term of U$(1)_I$ or giving a VEV to both. However, as follows from the analysis in section \ref{Weak coupling analysis}, only the first option is stable at tree level, while the other two choices need to be supplemented by adding a supersymmetric mass, in order to avoid tachyonic instabilities. In the following, we will deform the theory only by turning on a deformation for U$(1)_I$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%
Having analyzed the phase diagram of the $E_1^{(N)}$ SCFT, it is natural to generalize the previous analyses to the SU$(2N)_0$+2\textbf{AS} SCFT point. These theories reduce to Sp$(N)+$1\textbf{AS} if we go on a point of its Higgs branch and to SU$(2N-1)_{2N-1}$ when we go at infinite distance on one of the CB directions. For these reasons, these theories can be considered as a sort of "mother theory" for many of the cases we analyzed above.
...
The inverse gauge coupling squared $h$ is associated with the global symmetry, while the two independent masses $m_T,m$ are respectively the masses associated with the U$(1)$ and the SU$(2)$ part of the U$(2)$ flavor group. In the following, we will study the case $N=2$. This is particularly interesting, since $\mathfrak{su}(4)\cong \mathfrak{so}(6)$ and this will have consequences on the global symmetry of the theory. In the generic $N$ case, it is still not completely clear how we can rich the CFT phase from the perturbative one. Therefore, we leave the discussion on this generalisation for a future analysis.


For $N=2$, the gauge symmetry is SU$(4)\cong \text{Spin}(6)$, so the global symmetry is Sp$(2)/\mathbb{Z}_2$, since the anti-symmetric of SU$(4)$ is the vector of SO$(6)$, which has an Sp$(2)$ global symmetry. This can be directly seen from the perturbative prepotential
\begin{equation}
...
In the case of supersymmetric fixed points, much of the insight into their dynamics is obtained through their holographic dual AdS$_6$ geometries \cite{Legramandi:2021uds, Akhond:2022awd, Akhond:2022oaf}. Recently, a study of non-SUSY AdS$_6$ solutions of type IIB appeared in \cite{Apruzzi:2021nle}. Moreover the aforementioned class of solutions are obtained by RG flows from the SUSY solutions dual to SCFTs living at the intersection of $(p,q)$ 5-brane junctions. It would be interesting to explore potential similarities between our field theory deformations, and known deformations of these AdS$_6$ solutions.
