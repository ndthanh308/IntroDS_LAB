...
\begin{document}
	...
	\author{Xuezhang Chen\thanks{X. Chen is partially supported by NSFC (No.12271244). Email: xuezhangchen@nju.edu.cn.}~
		and Wei Wei\thanks{W. Wei is partially supported by NSFC (No.12201288, No.12271244) and BK20220755. W. Wei: wei\_wei@nju.edu.cn.}\\
	...
	\begin{abstract}
		...
		%		We first establish local pointwise a priori $C^2$-estimate of solutions to the $\sigma_2$-curvature equation with nonlinear boundary condition. Then, on three- or four-dimensional manifolds with totally non-umbilic boundary and non-negative boundary mean curvature, we prove the existence of a conformal metric having a prescribed positive $\sigma_2$-curvature function and a prescribed non-negative boundary mean curvature function. The local estimate plays a crucial role in blow-up analysis in the latter existence result.
		\medskip{}
	...
	\begin{convention} 
		The letters like $1\leq i,j,k \leq n$ stand for the full indices; the Greek letters like $1 \leq \alpha,\beta,\gamma\leq n-1$ stand for tangential indices. 
	...
	We are interested in the following $\sigma_2$-curvature equation with nonlinear boundary condition:
	...
	\end{align}
As first pointed out by S. Chen in  \cite[Lemma 5]{Chen}, under the assumptions that $0<f \in C^\infty(\overline M)$ and $c=0$, on compact manifolds with umbilic boundary the above equation \eqref{PDE:prescribing_sigma_2} serves as the Euler-Lagrange equation of a geometric functional $\mathcal{F}_2[g]$ (see \eqref{def:F_2} for the definition). We leave an overview of this issue to Section \ref{Sect:degree_theory}.	
	...
	A more general fully nonlinear PDE with Neumann boundary condition including \eqref{PDE:sigma_2_with_bdry} has been studied by Q. Jin, A. Li and Y. Y. Li \cite{Jin-Li-Li}, where the authors \cite[Theorem 1.4]{Jin-Li-Li} established local $C^{1}$ estimates of solutions assuming a local lower $C^0$ bound. 
	
	Once the local $C^2$ estimates were achieved for $c\ge 0$,  the compactness and existence of solutions to (\ref{PDE:prescribing_sigma_2}) come out with proper $C^0$ estimates. There exist several existence theorems  for \eqref{PDE:prescribing_sigma_2} when $(M,g)$  is assumed to be locally conformal flat  with umbilic boundary. In \cite{Li-Luc3} Li and Nguyen established the compactness of solutions to $\sigma_k$-curvature equation on manifolds with multiply connected boundaries for nonnegative prescribed mean curvature.  He and Sheng \cite{HS} obtained the compactness of solutions $\sigma_k$-curvature equation on manifolds with totally geodesic boundary for $c=0$.  On four-dimensional compact manifolds with umbilic boundary, which is not conformally equivalent to the standard hemi-sphere $(\Sp^4_+,g_{\Sp^4})$, S. Chen \cite[Theorem  2]{Chen} proved the existence of solutions to \eqref{PDE:prescribing_sigma_2} with $c=0$   provided that the boundary Yamabe constant $Y(M,\pa M,[g])$ and $\mathcal{F}_2[g]$ are both positive, where
	...
	\end{equation}
	With the local estimates in Theorem \ref{Thm:local estimates}, the same proof of S. Chen for the existence result of \eqref{PDE:prescribing_sigma_2} is applicable to $c\geq 0$, in other words, \cite[Theorem  2]{Chen} is true for $c\geq 0$.    The existence of solutions
	to \eqref{PDE:prescribing_sigma_2} on manifolds with non-umbilic boundary is largely open. 
	
	Besides the local $C^1$ estimates, the boundary local $C^2$ estimates for non-umbilic boundary  are difficult obstructions to the existence. Our first purpose is to derive local $C^2$ estimates  of solutions to \eqref{PDE:sigma_2_with_bdry} on compact manifolds with \emph{generic} boundary.
	...
	\end{theorem}
	The boundary local  $C^2$ estimates for \eqref{PDE:sigma_2_with_bdry} are even new for Euclidean domains and compact manifolds with umbilic boundary for $c>0$. Jin, Li and Li \cite{Jin-Li-Li} obtained local  $C^2$ estimates when $\partial M$ is umbilic and $(M,g)$ is locally conformally flat near $\pa M$. Meanwhile, with different technique, Chen \cite{Chen3} obtained the boundary local  $C^2$ estimates under the same assumption except for $c=0$. Jin \cite{Jin} obtained the boundary local  $C^2$ estimates for manifolds with totally geodesic boundary.  We emphasize that the dependence of the above constant $C$ on $\sup_{\mathcal{O}}\frac{1}{f(x,u)}$ only originates in double normal derivative estimates. This is a different phenomenon from the local $C^1$ estimates obtained in \cite{Jin-Li-Li}. 
	
	 The condition that the prescribed mean curvature is nonnegative, that is, $c \geq 0$, is \emph{necessary} to boundary local $C^2$ estimates in some sense. For umbilic boundary case, it can be seen from the counter-examples constructed by Li and Nguyen \cite{Li-Luc4}; for non-umbilic boundary case, this necessity is ensured by our counter-examples (see Proposition \ref{prop:counterexample_new}), which is an adaptation of Li-Nguyen's counter-examples.  The  counter-examples for $c<0$  in \cite{Li-Luc4} are a sequence of radially symmetric solutions $\{u_j\}$ on an annulus, which demonstrates that  approaching  the boundary for a prescribed negative mean curvature $c$, the double normal derivative $\pa_r^2 u_j$  on the inner boundary goes to infinity. It is no doubt that the sign of $c$ is essential in the estimates of double normal derivative on the boundary.  Jiang and Trudinger \cite{JT} introduced the so-called $(\Gamma_k, A,G)$-convex boundary condition, which is equivalent to the condition $c e^{-u}>0$ in $\Gamma_2^+$ for $k=2$ (cf. \cite[p.401]{JT}), and obtained the global $C^2$ estimates for a Euclidean domain $\Omega$ with non-umbilic boundary $\pa \Omega$, in particular $\inf_{\partial \Omega} (c e^{-u})>0$ plays a crucial role in their proof. 
...
For non-umbilic boundary case, one of missing elements in the existence of solutions to \eqref{PDE:sigma_2_with_bdry} is exactly the local  $C^2$ estimates in Theorem \ref{Thm:local estimates}, although  the Neumann boundary problem for fully nonlinear PDEs has been developed a lot, see  Lions, Trudinger and Urbas \cite{LTU} and recent work of X. N. Ma and G. Qiu \cite{MQ} etc. Generally, the curvature of boundary plays an essential role on the boundary $C^2$ estimates for the most fully nonlinear equations. For example, in \cite{CMW, CW, LTU, MQ} etc., the boundary $C^2$ estimates only work for strictly convex domains. Without the convexity of the boundary, Dong and Wei \cite{DW} proved  $C^2$ estimates for certain type of fully nonlinear equations, which highly depend on the strictly elliptic property of these equations. For most types of fully nonlinear elliptic equations we would not expect  $C^2$ estimates without any condition on the equations or manifolds.  The classical existing method consists of two steps: The first step is to reduce second  tangential derivative estimates to double normal derivative estimates; the second step is to obtain double normal derivative estimates.  However, this reduction argument does not work well to obtain the local $C^2$ estimates for $\sigma_2$-curvature equation on manifolds with non-umbilic boundary. Our strategy to derive the boundary local $C^2$ estimates is quite different from these works just described.
	
	For boundary local $C^2$ estimates, we split the proof into two steps.  The first step is to derive the trace of second tangential derivative estimates. We first realize that an inaccuracy occurs in computations of second derivatives of the barrier function in  S. Chen \cite{Chen}.  This brings us some new `\emph{hard}' terms involving third covariant derivate terms. To get a good control of these `\emph{hard}' terms, we use a complete expansion of the term $\frac{\pa^2 \sigma_2(W)}{\pa W_i^j \pa W_r^s}$ with $W=A_u$ instead of just using the concavity of $\sigma_2^{1/2}$ as the traditional way for $\sigma_2$-Yamabe equation, where some `\emph{positive}' terms are in general thrown away. Readers are referred to \eqref{key-est:3rd_derivatives} for details. In fact, the $\sigma_2$-curvature equation has interior $C^2$ estimates, which are rare in the fully nonlinear elliptic equations and make second tangential  derivative estimates reasonable. In the second step, we construct a delicate barrier function to obtain double normal derivative estimates and then obtain the trace of second derivative estimates.  To derive the local $C^2$ estimates, we take advantage of a beautiful structure hidden behind two geometric PDEs: The interior $\sigma_{2}$-curvature equation and boundary mean curvature equation. Moreover, the nonnegativity of prescribed boundary mean curvature function is crucial in the proof.


%	
%		As first pointed out by S. Chen in  \cite[Lemma 5]{Chen}, under the assumptions that $0<f \in C^\infty(\overline M)$ and $c=0$, on compact manifolds with umbilic boundary the above equation \eqref{PDE:prescribing_sigma_2} serves as the Euler-Lagrange equation of a certain geometric functional. We leave an overview of this issue to Section \ref{Sect:degree_theory}.	
%	

	
	Based on the local $C^1$ and $C^2$ estimates and the blow-up analysis, we are able to obtain the $C^0$ estimates, as well as the following existence result.
	\begin{theorem}\label{Thm:existence}
		Let $(M,g)$ be a compact Riemannian manifold of dimension $n\geq 3$ with totally non-umbilic boundary $\pa M$. Assume that the Schouten tensor satisfies $\lambda(A_g) \in \Gamma_2^+$ and the mean curvature $h_g\geq 0$  on $\pa M$. Suppose $n=3,4$ and $0<f \in C^\infty(\overline M), 0\leq c \in C^\infty(\pa M)$. Then the boundary $\sigma_2$-curvature equation \eqref{PDE:prescribing_sigma_2} admits at least one solution $u \in C^\infty(\overline M)$.
	\end{theorem}
	
	Here `\emph{totally non-umbilic boundary}' means every boundary point is non-umbilic, which is essentially used to exclude the occurrence of boundary isolated blow-up points as in Definition \ref{def:bdry_blowup_pts}. Guan, Viaclovsky and Wang \cite{GVW} established that  for $k\ge n/2$, $\lambda(A_g)\in \Gamma_k^+$ implies the nonnegativity of  Ricci curvature, which is crucial in the study of $\sigma_k$-Yamabe problem on closed manifolds; see \cite{Gursky-Viaclovsky,Li-Luc2,TW} etc. Due to this reason, we limit ourselves to dimensions three and four. These are two geometric significances of our assumption. Moreover, the assumption of $\lambda(A_g) \in \Gamma_2^+$ and $h_g\geq 0$  directly implies $Y(M,\partial M,[g])>0$.
	
		
	
	
	
	In all dimensions $n\geq 3$, some basic results  are  developed for  the analogue of Schoen's blow-up theory  of the boundary $\sigma_2$-curvature equation \eqref{PDE:prescribing_sigma_2}, which are of independent interest. Such a blow-up theory of boundary Yamabe problem has already been delicately explored  in Han and Li \cite{han-li2}.  However, the strategy is technically quite different from that of Schoen \cite{Schoen} on prescribing scalar curvature  problem. On the basis of local estimates in Theorem \ref{Thm:local estimates} and Liouville-type theorems by Y. Y. Li and collaborators \cite{Li-Li1,Li-Li2,Han-Li-Tei} etc, we are able to complete the blow up analysis in dimensions three and four, then the existence result follows. Another advantage in dimensions three and four is that for any sequence of solutions $u_i$ to \eqref{PDE:prescribing_sigma_2} with $\min_{\overline M} u_i \to -\infty$ (such a $\{u_i\}$ is called a blow-up sequence), we define a rescaled sequence of  functions $v_i=u_i-u_i(y_0)$ for some interior `\emph{regular}' point $y_0$ (cf.  Section \ref{Subsect:compactness}), then the sequence of conformal metrics $g_{v_i}$ has nonnegative Ricci curvature by virtue of Guan, Viaclovsky and Wang \cite{GVW}. This plays a central role in excluding interior isolated blow-up points as in Definitions \ref{def:isolated blow up} and \ref{def:interior_isolated_simple} for the above blow-up sequence $\{u_i\}$ similar to the case of closed manifolds.
		 
	
	The following is the organization  of the paper, as well as a sketch of our strategy. 
	
	Section \ref{Sect:local_est} is devoted to the a priori $C^2$ estimates for solutions to \eqref{PDE:sigma_2_with_bdry}, which is accomplished by the combination of estimates of trace of  second tangential derivatives and double normal derivatives on the boundary. In Subsection \ref{subsect:counter-examples}, counter-examples to local boundary $C^2$ estimates on \emph{generic} boundary are presented. In Section \ref{Sect:first_bdry_eigenvalue}, we study first boundary $\sigma_2$-eigenvalue problem, which parallels  first boundary Yamabe/$\sigma_1$-curvature eigenvalue problem. As a byproduct, this reduces the assumption that $h_g\geq 0$ to $h_g=0$ in the study of existence result of \eqref{PDE:prescribing_sigma_2} in the next two sections. In Section \ref{Sect:degree_theory}, we adopt the degree theory to prove the existence of solutions to \eqref{PDE:prescribing_sigma_2} and encounter great difficulty obtaining the uniform $C^0$ estimates of solutions to a $1$-parameter family of boundary $\sigma_2$-curvature equations. The uniform $C^0$ bounds are obtained via the blow-up analysis in Section \ref{Sect:blowup_analysis}. 
	
	 In Section \ref{Sect:blowup_analysis}, we build the blow-up analysis for a slightly different boundary $\sigma_2$-curvature equation \eqref{PDE:sigma_2_new}, which enjoys a similar structure of each equation in the $1$-parameter family \eqref{eq:path}.  In Subsection \ref{Subsect:Basic facts in all dimensions}, we introduce relevant definitions, such as (interior and boundary)  isolated and isolated simple blow-up points etc, and establish some common facts in the blow-up procedure for all dimensions $n\geq 3$.  Starting from Subsection \ref{Subsect:isolated_singularity}, we restrict our attention to lower dimensions $n=3,4$. Given any blow-up sequence $\{u_i\}$ for \eqref{PDE:sigma_2_new} (if exists), we describe the isolated singularity behavior near each interior and boundary isolated blow-up point. We next  establish that every isolated blow-up point is indeed simple and the singular set $\mathcal S:=\mathcal{S}[\{u_i\}]$, consisting of only isolated blow-up points, is a finite set. Keep in mind that boundary isolated blow-up points do not occur since the boundary is totally non-umbilic by assumption. Then we adapt some insights from Gursky and Viaclovsky's proof \cite{Gursky-Viaclovsky}  to show that $\mathcal{S}$ has exactly one interior isolated blow-up point.  Using the volume estimate in Perales \cite{Perales} for manifolds with boundary under nonnegative Ricci curvature and a lower  mean curvature bound, with some extra efforts we are able to demonstrate that the unique interior isolated blow-up point could not happen either. In summary, such a blow-up sequence actually does not exist.
	
	For readers' convenience, we leave two proofs involving degenerate $\sigma_2$-curvature equations with Neumann data to Appendix \ref{Appendix}: One concerns the limit of a  blow-up sequence of solutions to the above family of boundary $\sigma_2$-curvature equations; the other is about an even reflection of a viscosity solution to a degenerate $\sigma_2$-curvature equation in a Euclidean half-ball with zero Neumann boundary condition.
	
	\vskip 8pt
	
	\noindent{\bf Acknowledgements.} The second named author would like to thank Professor Hao Fang and Professor Xinan Ma for enlighten discussions and constant support. The authors are grateful to Professors Yan Yan Li and Xinan Ma for bringing their attentions to Jiang-Trudinger's paper \cite{JT}. They are greatly appreciate Yan Yan Li's encouragement to construct the counter-examples for boundary local $C^2$ estimates. They thank their colleague at Nanjing University, Professor Yalong Shi for fruitful discussions about elementary properties of ellipsoids. They also thank Professor Feida Jiang for private communications about their results in \cite{JT} relating to those in this paper.
...
It sounds more natural and convenient to adopt these notations above in the study of fully nonlinear partial differential equations on manifolds. Throughout this section, we adopt the Einstein summation.	
	...
	The proof of Theorem \ref{Thm:local estimates} is complete through a series of technical lemmas, mainly Lemmas \ref{lem:tangential trace estimate} and \ref{lem:double_normal_derivatives}.
...
calculations to derive that $(\pa_{j}\pa_{i}G)$ is negative semi-definite at some local maximum point of $G$, where $G$ is the barrier function therein. We first need a correction of such an inaccuracy. 
\iffalse
\begin{align*}
...
\begin{proof}[Proof of Theorem \protect\ref{Thm:local estimates}]
		...
		function in $B_r^+$ as in the proof of Lemma \ref{lem:tangential trace estimate}. The maximum point $x_0$ of $F$ happens either on the boundary or in the interior.
...
\subsection{Counter-examples to boundary local $C^2$ estimates}\label{subsect:counter-examples}
	As mentioned in the introduction, the boundary local $C^2$ estimates for boundary $\sigma_2$-curvature equation \eqref{PDE:prescribing_sigma_2} fail in general if $c$ is allowed to be negative somewhere. To be consistent with our notations, we first state Li-Nguyen's counter-examples in \cite{Li-Luc4}.
\begin{proposition}\label{prop:counterexample_Li-Luc}
For any $2\leq k\leq n$ and $c<0$, there exist constants $C_{0}>0,R_{0}>1$
...
The following basic facts about ellipsoids are of independent interest.
...
The inward unit normal at $p \in \Sigma$ is
 $$\vec n=-\frac{\nabla F}{|\nabla F|}$$
and for an orthonormal basis $\{e_\alpha; 1 \leq \alpha \leq n-1\}$ of $T_p \Sigma$, the second fundamental form is
 ...
 Further assume $a_1<a_2<\cdots<a_n$. Then, $p=(x_1,\cdots, x_n) \in \Sigma$ is umbilic if and only if for all 
 $(u_1,\cdots,u_n) \in T_p \Sigma$, that is, $\sum_{i=1}^n a_i^{-2}x_iu_i=0$, there exists $\lambda \in \R$ such that
...
Inserting it into $\eqref{eqn:umbilic}$ to show
 \begin{align*}
  \sum_{\alpha=1}^{n-1} \frac{u_\alpha^2}{a_\alpha^2}+\frac{a_n^2}{x_n^2}\left(\sum_{\alpha=1}^{n-1}\frac{x_\alpha}{a_\alpha^2}u_\alpha\right)^2=\lambda \left[\sum_{\alpha=1}^n u_\alpha^2+\left(\sum_{\alpha=1}^{n-1}\frac{x_\alpha}{a_\alpha^2}u_\alpha\right)^2\right]
 \end{align*}
...
\begin{remark}
For an ellipsoid with generic $a_i \in \R_+$, we further can show that $p=(x_1,\cdots,x_n) \in \Sigma$ is umbilic if and only if the following equation holds:
...
at two points $x_{0}=(\pm1,0,\cdots,0)$, and $B_{R_{0}}\supset\Omega\supset B_{1}$. Moreover, it follows from Lemma \ref{lem:ellipsoid} that  the mean curvature $h_\ve$ on $\Sigma \subset \pa(B_{R_0}\backslash \overline\Omega)$ is 
...
\]
  where $\vec n$ is the inward normal vector on $\partial(B_{R_0}\backslash \overline\Omega)$ and $\vec n(x_{0})=x_0$.

To this end, notice that at $x_{0}$, $\frac{\partial u_{j}}{\partial \vec n}=\frac{\partial u_{j}}{\partial r}$
and $\nabla^2 u_{j}(\vec n,\vec n)=\frac{\partial^{2}u_{j}}{\partial r^{2}}$, furthermore
\begin{align*}
...
This produces counter-examples with $c$ being negative somewhere for boundary local $C^2$ estimates.
...
\end{remark}
	...
	\end{cases}
	\]
	It is known in \cite{escobar4} that the sign of $\lambda_{1}$
	coincides with the sign of boundary Yamabe constant $Y(M,\pa M,[g])$ as defined in \eqref{def:bdry_Yamabe_constant}.
	
	Motived by  eigenvalue problem above, we are interested in the
	...
	In this section, we  write a conformal metric in the form of $g_{u}=u^{4/(n-2)}g$. For a positive $C^2$ function $u$, by an abuse of notation we define 
	...
	In order to show the existence part of boundary  $\sigma_{2}$-curvature equation \eqref{PDE:prescribing_sigma_2}, we adopt the method of Leray-Schauder degree. To this end, we first study  the solvability of positive solutions to  a slightly more general boundary $\sigma_2$-curvature equation:
	\begin{equation}
		\begin{cases}
	...
	simplicity, we just take $f_{0}$ as a constant function, which is enough to our use. Obviously, 
	the estimates for $w$ could be regarded as the ones of $\log u$ due to $w=-2\log u/(n-2)$.
	
	In the history, Schoen built a blow-up theory for the scalar curvature equation on closed manifolds, see Schoen and Zhang \cite[Lemma 3.1]{Schoen-Zhang}; in studying compactness of boundary Yamabe problem on locally conformally flat manifolds with umbilic boundary,  Han and Li \cite[Proposition 1.2]{han-li2} gave a  delicate chacterization. Our goal is to establish a generalization of the above blow-up theory for \eqref{PDE:sigma_2_new}. More precisely,  any sequence of positive solutions $\{u_i\}$ to \eqref{PDE:sigma_2_new} with $\max_{\overline{M}}u_i \to \infty$ only admits a finite set of distinct isolated blow-up points; see Lemma \ref{lem:pairwize_disjoint_sigular_pts}. 
	...
	\begin{theorem}[\cite{Li-Li2}] \label{Thm:Acta}
		...
		$\lambda(2b^{2}a^{-2}I)\in \Gamma_2^+$ and $F\left(2b^{2}a^{-2}I\right)=1$, 
		\[
		u(x)=\left(\frac{a}{1+b^{2}|x-\bar{x}|^{2}}\right)^{(n-2)/2},\quad x\in\mathbb{R}^{n}.
		\]
	...
	\begin{theorem}[\cite{Li-Li3}]\label{half liouville theorem}
		...
		is a superharmonic solution of $F(\lambda(g_u^{-1}A_{u}))=1$.
		Then 
		\[ u(x) =\left(\frac{a}{1+b\left|x-\bar{x}\right|^{2}}\right)^{(n-2) / 2} \quad \mathrm{~~for~~} x=(x^{\prime}, x_{n}) \in \mathbb{R}_{+}^{n},\]
		where $\bar{x}=\left(\bar{x}^{\prime}, \bar{x}_{n}\right) \in \mathbb{R}^{n}, a>0$ and $b+\left(\min \bar{x}_{n}, 0\right)^{2}>0$ are two constants satisfying $\lambda(2b^{2}a^{-2}I)\in \Gamma_2^+, F\left(2 a^{-2} b I\right)=1$ and $(n-2) a^{-1} b \bar{x}_{n}=c$.
	\end{theorem}
	...
	\begin{definition} \label{vis-definition boundary}
		...
		\[\lambda(A_{\psi})(\bar{x})\in\overline{\Gamma_2^+},~(\mathrm{resp.}~~ \lambda(A_{\psi})\in \mathbb{R}^n\backslash \Gamma_2^+ ).\]
	...
	As indicated in Y. Y. Li \cite[Proposition 5.1]{LiYY}, we know that
	an  even reflection of a positive Lipschitz viscosity solution $u^+$ to  $\lambda(A_{u^+}) \in \pa \Gamma_2^+$ in $B_1^+$ with vanishing Neumann boundary condition on $\pa' B_1^+$ keeps the equation invariant. For our purposes, we intend to show that the same result still holds for a viscosity solution as in Definition \ref{vis-definition boundary}, which is slightly different from that of \cite{LiYY}. For clarity, we restate it in the following proposition, whose proof in the spirit we present in Appendix \ref{Appendix} is very similar to that of   \cite[Proposition 5.1]{LiYY}.
	...
	Our perspective is to develop the blow-up theory for every blow-up sequence of solutions to boundary $\sigma_2$-curvature equation \eqref{PDE:sigma_2_new} in all dimensions $n\geq 3$, which is the counterpart corresponding to those of Schoen in scalar curvature equation and Han-Li in boundary Yamabe problem. 
	
	As a first step, we present some common facts for dimensions $n\geq 3$. 
	%These results might be known to experts in this field, although it seems not stated or proven in the literature. 
	\begin{lemma}
		\label{lem:preparation lemma}Let $(M,g)$ be a smooth compact Riemannian
		manifold of dimension $n\geq3$ with boundary $\pa M$. For any $R\geq1$ and $0<\ve<1$,
		there exists a positive constant $C_{0}$ depending on $M,g,f,c,R$ and $\ve$, such
		that for any compact set $K\subset\overline{M}$ and any positive solution
		$u$ to $(\ref{PDE:sigma_2_new})$ with 
		\[
		\max_{q\in\overline{M\backslash K}}d_{g}^{\frac{n-2}{2}}(q,K)u(q)\geq C_{0},
		\]
		where $d_{g}(q,K):=1$ for $K=\emptyset$, and the following statements hold. 
		...
		\begin{itemize}
			...
			\[
			\left\Vert u\left(q_{0}\right)^{-1}u\circ\exp_{q_{0}}\left(u(q_{0})^{-\frac{2}{n-2}}y\right)-\left(1+\frac{f(q_{0})}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}(0)\right)}<\ve.
			\]
			
			\item[($a_2$)] If $q_{0}\in \overline{M}$ and $d_g(q_0,\partial M)<r_*$, then 
			\[
			\left\Vert u\left(q_{0}\right)^{-1}u\circ G_{q_0'}\left(u^{-\frac{2}{n-2}}(q_{0})y+{d}_{0}\mathbf{e}_{n}\right)-\left(1+\frac{f(q_{0})}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}\right)}<\ve,
			\]	
			where $d_0:=d_g(q_0, \partial M)=d_g(q_0, q_0')$ for some $q_0'\in \partial M$.	
			
			\item[($a_3$)] If $q_{0}\in\partial M$, then 
			\[
			\left\Vert u\left(q_{0}\right)^{-1}u\circ G_{q_{0}}\left(u(q_{0})^{-\frac{2}{n-2}}y\right)-(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(q_0)})^{\frac{n-2}{4}}\left(1+b|y+{T_c}\mathbf{e}_{n}|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}\right)}<\ve,
			\]
			where 
			\[
			b=\frac{f(q_{0})}{2(C_{n}^{2})^{1/2}}+\frac{c(q_{0})}{(n-2)^{2}}\quad\mathrm{~~and~~}\quad {T_c}=\frac{c(q_{0})}{f^{1/2}(q_{0})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b^{1/2}}.
			\]
		\end{itemize}
	\end{lemma} 
	\begin{proof}
		...
		\]
		and there exists no $q_{0}\in\overline{M}\backslash K_i$, which is a local maximum point of $u_i$ in $\overline M$, satisfying any of $(a_1)$-$(a_3)$.
		...
		For convenience, we abbreviate $\lambda_{i}=u_{i}^{-2/(n-2)}(q_{i})$, $d_{i}:=d_{g}(q_{i},q_{i}')=d_{g}(q_{i},\pa M)$ for some $q_{i}'\in\pa M$
		and $T_{i}=\lambda_{i}^{-1}d_{i}$.
		
		Fix a small constant $\delta>0$ and assume that up to a subsequence, $d_{i}<\delta$
		...
		and define 
		\[
		v_{i}(y):=\frac{1}{u_{i}(q_{i})}u_{i}\circ\Psi_{i}(y)\quad\mathrm{~~and~~}\quad g_{i}=\lambda_i^{-2}\Psi_{i}^{\ast}(g).
		\]
		In other words,
		...
		\begin{equation}
			d_{g}(x,K_{i})\geq\frac{1}{2}d_{g}(q_{i},K_{i}) \quad \mathrm{~~and~~thus~~} \quad x\notin K_{i}.\label{est:distance lower}
		\end{equation}
		...
		It is not hard to see that $v_{i}$ satisfies 
		\begin{align*}
		...
		\[
		\frac{\partial v_{i}}{\partial\vec{n}_{g_{i}}}=-c\circ\Psi_{i}v_{i}^{\frac{n}{n-2}}+\frac{n-2}{2}h_{g_{i}}v_{i}\qquad\mathrm{~~on~~}\quad y_{n}=-T_{i}.
		\]
		
		Up to a further subsequence, we may assume $\lim_{i\rightarrow\infty}T_{i}:=T\in[0,\infty]$.
		It suffices to consider $T<\infty$, since the case of $T=\infty$
		can be similarly handled as in \emph{Case 1} below.
		
		By  the gradient estimate in \cite{Jin-Li-Li} and Theorem \ref{Thm:local estimates} we have
		\[
		\|v_{i}\|_{C^{2}(\tilde{K})}\le C_{\tilde{K}}\quad\mathrm{~~for~~any~~compact~~set~~}\tilde{K}\subset\Omega_{i},
		\]
		...
		Therefore, there exists a subsequence $v_{i}$ converging in $C_{\mathrm{loc}}^{2}(\overline{\mathbb{R}_{-T}^{n}})$
		to $v_{\infty}$ with $v_{\infty}(0)=1$ by virtue of $v_i(0)=1, ~\forall~i \in \N$, and $v_{\infty}$ satisfies
		\[
		\begin{cases}
			\sigma_{2}^{1/2}(-\frac{2}{n-2}\frac{\nabla_{g_{\mathrm{E}}}^{2}v_{\infty}}{v_{\infty}}+\frac{2n}{(n-2)^{2}}\frac{\ud v_{\infty}\otimes\ud v_{\infty}}{v_{\infty}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla v_{\infty}|_{g_{\mathrm{E}}}^{2}}{v_{\infty}^{2}}g_{\mathrm{E}})=f(q_{\infty})v_{\infty}^{\frac{4}{n-2}} & \mathrm{in~~}\mathbb{R}_{-T}^{n},\\
			\frac{\partial v_{\infty}}{\partial y_{n}}=-c(q_{\infty})v_{\infty}^{\frac{n}{n-2}} & \mathrm{on~~}y_{n}=-T.
		\end{cases}
		\]
		By the classification theorem in A. Li-Y.Y. Li \cite{Li-Li1}, we
		know that up to horizontal translations, the unique solution of the
		above problem is 
		...
		\end{equation}
		where $b\in\R_{+}$ and $\bar{y}_{n}\in\R$ is fixed, coupled with
		\begin{equation}
			\bar{y}_{n}+T=-\sqrt{\frac{2}{f(q_{\infty})}}\frac{c(q_{\infty})}{n-2}\frac{(C_{n}^{2})^{1/4}}{\sqrt{b}}\label{eq:bdry_T}
		\end{equation}
		and 
		\[
		(\bar{y}_{n})^{2}b-\sqrt{\frac{2}{f(q_{\infty})}}(C_{n}^{2})^{1/4}\sqrt{b}+1=0.
		\]
		...
		\[
		\max_{\overline{\R_{-T}^{n}}}v_{\infty}=v_{\infty}(\hat{y})=\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(q_{\infty})}\right)^{\frac{n-2}{4}}.
		\]
		...
		\begin{equation}\label{bdry_T-1}
			\bar{y}_{n}=-\sqrt{\frac{2}{f(p_{\infty})}}\frac{c(p_{\infty})}{n-2}\frac{(C_{n}^{2})^{1/4}}{\sqrt{\tilde b}}
		\end{equation}
		and
		...
		local maximum point of $u_{i}$. Noticing that $d(p_{i},\partial M)u_{i}^{2/(n-2)}(p_{i})=0$,
		...
		large $i$ we obtain 
		\[
		\left\Vert u_{i}^{-1}\left(p_{i}\right)u\circ G_{p_{i}}\left(u_{i}^{-\frac{2}{n-2}}(p_{i})y\right)-\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(p_{\infty})}\right)^{\frac{n-2}{4}}(1+b|y+{T_c}\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}\right)}<\ve,
		\]
		where $p_{i}\to p_{\infty}\in\pa M$ and 
		\[
		{T_c}=\frac{c(p_{\infty})}{f^{1/2}(p_{\infty})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b^{1/2}}
		\]
		with $b\in\R_{+}$ being determined by 
		\[
		\frac{1}{f(p_{\infty})^{\frac{n-2}{4}}}\left(\frac{(2b(C_{n}^{2})^{\frac{1}{2}})^{\frac{1}{2}}}{1+b{T_c}^{2}}\right)^{(n-2)/2}=1.
		\]
		...
		\[
		\left\Vert u_{i}^{-1}\left(p_{i}\right)u\circ G_{p_{i}}\left(u_{i}^{-\frac{2}{n-2}}(p_{i})y\right)-(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(p_{i})})^{\frac{n-2}{4}}(1+b_{i}|y+T_c^{(i)}\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}(0)\right)}<\ve.
		\]
		Here 
		\[
		T_c^{(i)}=\frac{c(p_{i})}{f^{1/2}(p_{i})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b_{i}^{1/2}}
		\]
		with $b_{i}\in\R_{+}$ being determined by 
		\[
		\left(\frac{2b_i(C_{n}^{2})^{\frac{1}{2}}}{f(p_{i})}\right)^{\frac{n-2}{4}}(1+b_{i}(T_c^{(i)})^{2})^{\frac{2-n}{2}}=1.
		\]
		It is not hard to see that 
		\begin{align*}
			b_{i}=\frac{f(p_{i})}{2(C_{n}^{2})^{1/2}}+\frac{c(p_{i})}{(n-2)^{2}}\quad\mathrm{~~and~~}\quad T_c^{(i)}=\frac{c(p_{i})}{f^{1/2}(p_{i})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b_{i}^{1/2}}.
		\end{align*}
		However, it again contradicts the contradiction hypothesis $(a_3)$. 
	\end{proof}
	...
	Now we give a lemma which only relies on the Liouville theorems.
	...
	\begin{lemma}\label{lem fudament-1-1}
		Let $(M,g)$ be a smooth compact Riemannian
		manifold of dimension $n\geq3$ with boundary $\pa M$. For any $R\geq1$ and $0<\ve<1$, there exist two positive constants $C_{0}, C_{1}$ depending on $M,g,f,c,R$ and $\ve$, such that for all positive solutions $u$
		to (\ref{PDE:sigma_2_new}) with $\max_{\overline{M}}u\geq C_{0}$,
		and there exists a finite set $\left\{ q_{1},\ldots,q_{N}\right\} \subset\overline{M}$,
		with $N\geq1$ such that the following statements are true.
		...
		\begin{itemize}
			\item[($a_1$)] $q_{i}\in M$, 
			\[
			\left\Vert u^{-1}\left(q_{i}\right)u\circ\exp_{q_{i}}\left(u_{i}^{-\frac{2}{n-2}}\left(q_{i}\right)y\right)-\left(1+\frac{f(q_{i})}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}\right)}<\varepsilon.
			\]
			\item[($a_2$)] $q_{i}\in \overline{M}$, 
			\[
			\left\Vert u^{-1}\left(q_{i}\right)u\circ G_{q_i'}\left(u^{-\frac{2}{n-2}}(q_{i})y+{d}_{i}\mathbf{e}_{n}\right)-\left(1+\frac{f(q_{i})}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}\right)}<\ve,
			\]	
			...
			\begin{align*}
				\left\Vert u(q_{i})^{-1}u\circ G_{q_{i}}\left(u(q_{i})^{-\frac{2}{n-2}}y\right)-(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(q_{i})})^{\frac{n-2}{4}}\left(1+b_i|y+{T_c^{(i)}}\mathbf{e}_{n}|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{2R}^{+}\right)}<\ve,
			\end{align*}
			where 
			\[
			b_i=\frac{f(q_{i})}{2(C_{n}^{2})^{1/2}}+\frac{c(q_{i})}{(n-2)^{2}}\quad\mathrm{~~and~~}\quad {T_c^{(i)}}=\frac{c(q_{i})}{f^{1/2}(q_{i})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b^{1/2}}.
			\]
		\end{itemize}
	...
	\begin{proof}
		...
		Taking $r_{1}=Ru^{-2/(n-2)}(q_{1})$ and $K_{1}=\overline{B_{r_{1}}(q_{1})}$.
		Now if 
		\[
		\max_{q\in\overline{M\backslash K_{1}}}d_{g}^{\frac{n-2}{2}}(q,K_{1})u(q)<C_{0},
		\]
		then we shall show that \textit{(iii)} holds, which will be included in the proof
		...
		a local maximum point  of $u$,  $q_{2} \in \overline{M}\backslash K_{1}$ satisfying \textit{(ii)}. Define $r_{2}=Ru^{-2(n-2)}(q_{2})$.
		
		For any fixed $\ve\in(0,1)$, it follows from \textit{(ii)} that there exists
		a positive constant $c_{1}=c_{1}(\ve)\to0$ as $\ve\searrow0$
		such that one of three cases happen corresponding to $(a_1),(a_2),(a_3)$:
		\begin{itemize}
			...
			has no  local maximum point in $|y+T_c^{(2)} \mathbf{e}_n|\ge T_c^{(2)}+ c_{1}$.
		\end{itemize}
		...
		For cases $(b_1)$ and $(b_2)$, otherwise $d_{g}(q_{1},q_{2})\le2Ru^{-2/(n-2)}(q_{2})$. Then, by $(b_1)$ and $(b_2)$, with the fact that $q_{1}$ is also a local maximum point of $u$, we have $d_{g}(q_{1},q_{2})\le c_{1}u^{-2/(n-2)}(q_{2})$ . This together with \textit{(ii)} (that is $(a_1)$ and $(a_2)$)
		implies that $u(q_{2})\le u(q_{1})\le2u(q_{2})$.		Therefore,  by choosing
		$\ve$ sufficiently small we obtain $d_{g}(q_{1},q_{2})\le c_{1}u^{-2/(n-2)}(q_{2})\le 2^{2/(n-2)}c_1u^{-2/(n-2)}(q_{1})<r_{1}$,
		which contradicts $q_{2}\notin\overline{B_{r_{1}}(q_{1})}:=K_1$.
		
		For case $(b_3)$, otherwise  $0<d_{g}(q_{1},q_{2})\le2Ru^{-2/(n-2)}(q_{2})$ and $q_2\in \partial M$. By choosing $\ve$ small enough we have $|y(q_1)+T_c^{(2)} \mathbf{e}_n|>T_c^{(2)}+c_1$. However, by $(b_3)$ we know that $q_1$ is not a local maximum of $u$. This is again a contradiction.
		
		By induction we can find other points $q_{i+1}$ and the argument is almost the same. 
		
		Suppose that we have found a finite set of disjoint geodesic balls $\{\overline{B_{r_j}(q_j)}; 1 \leq j \leq i\}$ with each $q_j$ satisfying \textit{(ii)}, and let $K_i=\cup_{j=1}^{i}\overline{B_{r_j}(q_j)}$ and assume that 
		$$
		\max_{q\in\overline{M\backslash K_{i}}}d_{g}^{\frac{n-2}{2}}(q,K_{i})u(q)\geq C_{0},
		$$ 
		then we can apply Lemma \ref{lem:preparation lemma} to find a local maximum point of $u$, $q_{i+1} \in \overline M \backslash K_i$ satisfying \textit{(ii)} and define  $r_{i+1}=Ru^{-2/(n-2)}(q_{i+1})$. 	To see $K_i \cap \overline{B_{r_{i+1}}(q_{i+1})}=\emptyset$. If not, there exists some $1 \leq j_0\leq i$ such that
		$$B_{r_{i+1}}(q_{i+1})\cap B_{r_{j_0}}(q_{j_0})\neq \emptyset.$$
		Now we choose 
		$$u(q_{i_0})=\min\{u(q_{j_0}),u(q_{i+1})\}.$$
		It is enough to replace $q_2,q_1$ in the above argument by $q_{i_0}$ and the other point, respectively. Thus, we are able to obtain a contradiction.
		...
		\]
		By \textit{(ii)}, by choosing $\ve$ sufficiently small we know that 
		\[
	...
	%	\begin{lemma}
		%		\label{lem fudament-1-1-boundary}Let $(M,g)$ be a smooth compact Riemannian
		%		manifold of dimension $n\geq3$ with boundary. For any $c>0$ and
		%		any $R\geq1,0<\ve<1$, there exist positive constants $\delta_{0}=\delta_{0}(M,g,R,\ve),C_{0}=C_{0}(M,g,R,\ve)$,
		%		and $C_{1}=C_{1}(M,g,R,\ve)$ such that, for all the solution $u$
		%		to (\ref{PDE:sigma_2_new}) with $\max_{\partial{M}}u\geq C_{0}$,
		%		there exists $\left\{ q_{1},\ldots,q_{N}\right\} \subset\partial{M}$,
		%		with $N\geq1$ such that the following statements are true.
		%		
		%		(1) Each $q_{i}$ is a local maximum point of $u$ on $\partial{M}$
		%		and 
		%		\[
		%		\overline{D_{\bar{r}_{i}}\left(q_{i}\right)}\cap\overline{D_{\bar{r}_{j}}\left(q_{j}\right)}=\emptyset,\quad\mathrm{~~for~~}i\neq j,
		%		\]
		%		where $\bar{r}_{i}=Ru^{-2/(n-2)}(q_{i})$.
		%		
		%		(2)
		%		$q_{i}\in\partial M$,
		%		
		%		\[
		%		\left\Vert u\left(q_{i}\right)^{-1}u\circ G_{q_{0}}\left(u(q_{i})^{-\frac{2}{n-2}}y\right)-\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(q_{i})}\right)^{\frac{n-2}{4}}\left(1+b(|y+T\mathbf{e}_{n}|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}\left(B_{3R}^{+}\right)}<\ve,
		%		\]
		%		where 
		%		\[
		%		b=\frac{f(q_{i})}{2(C_{n}^{2})^{1/2}}+\frac{c(q_{i})}{(n-2)^{2}}\quad\mathrm{~~and~~}\quad T=\frac{c(q_{i})}{f^{1/2}(q_{i})}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b^{1/2}}.
		%		\]
		%		
		%		(3) $d_{g,\partial M}^{(n-2)/2}(q_{i},q_{j})u(q_{j})\ge C_{0}$ for $j>i$, while
		%		for $q\in\partial{M}$, 
		%		\[
		%		u(q)\le C_{1}d_{g,\partial M}(q,\{q_{1},\cdots,q_{N}\})^{-\frac{n-2}{2}}.
		%		\]
		%	\end{lemma} 
	%	
	%	The proof of Lemma \ref{lem fudament-1-1-boundary} is similar as Lemma \ref{lem fudament-1-1}.
	%	
	%	
	Suppose $\{u_{i}\}$ is a sequence of positive solutions to \eqref{PDE:sigma_2_new}.
	\begin{definition}\label{def:isolated blow up} 
		...
		\[
		u_{i}(x)\leq Cd_{g}\left(x,x_{i}\right)^{-\frac{n-2}{2}},\quad\mathrm{~~for~~all~~}x\in\overline{M},~d_{g}\left(x,x_{i}\right)<\delta,
		\]
	...
	\begin{lemma} \label{lem fudament} Let $(M,g)$ be a smooth compact
		Riemannian manifold of dimension $n\geq3$ with boundary $\pa M$. Let $x_{i}\rightarrow x_0$
		be an isolated blow-up point for ${u_{i}}$. For any sequences of positive real
		numbers $R_{i}\rightarrow\infty$, $\varepsilon_{i}\rightarrow0$,
		there exists a subsequence of  $u_{i}$ to (\ref{PDE:sigma_2_new}) such that
		\[
		r_{i}:=R_{i}u_{i}^{-\frac{2}{n-2}}\left(x_{i}\right)\rightarrow0,
		\]
		and either Case 1: for $x_{i}\in M$, 
		\[
		\left\Vert u_{i}^{-1}\left(x_{i}\right)u_{i}\circ \exp_{x_{i}}(u_{i}^{-\frac{2}{n-2}}\left(x_{i})y\right)-\left(1+\frac{f(x_0)}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}(B_{2R_{i}})}<\varepsilon_{i}
		\]
		or Case 2: for $x_{i}$ near  $\partial M$,
		\[
		\left\Vert u_{i}^{-1}\left(x_{i}\right)u_{i}\circ G_{x_{i}'}(\frac{y}{u_{i}^{\frac{2}{n-2}}\left(x_{i}\right)}+d_i\mathbf{e}_n)-(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_0)})^{\frac{n-2}{4}}\left(1+b|y+{T_c}\mathbf{e}_{n}|^{2}\right)^{\frac{2-n}{2}}\right\Vert _{C^{2}(B^+_{2R_{i}})}<\varepsilon_{i},
		\]
		where 
		
		\[
		b=\frac{f(x_0)}{2(C_{n}^{2})^{1/2}}+\frac{c(x_0)}{(n-2)^{2}}\quad\mathrm{~~and~~}\quad {T_c}=\frac{c(x_0)}{f^{1/2}(x_0)}\frac{\sqrt{2}}{n-2}\frac{(C_{n}^{2})^{1/4}}{b^{1/2}},
		\] and  $d_i=d_g(x_i, \partial M)=d_g(x_i,x_i')$ for some $x_i' \in \pa M$. If $T_i=d_iu_i(x_i)^{2/(n-2)}\rightarrow \infty$, then Case 1 happens. If $T_i=d_iu_i(x_i)^{2/(n-2)}$ is bounded, then Case 2 happens.
	\end{lemma}
	
	The proof of Lemma \ref{lem fudament} is a standard argument of limitation as before and we omit it here.
	
	
	
	
	
	We next give the definition of an isolated simple blow-up point.
	...
	\begin{definition} \label{def:interior_isolated_simple}
		...
		\begin{equation}
			\overline{w}_{i}'(r)<0\quad\mathrm{~~for~~all~~}Cu_{i}(x_{i})^{-\frac{2}{n-2}}\le r\le\tilde{r},\label{eq:isolated blow up-1}
		\end{equation}
		where $\overline{w}_{i}(r):=r^{\frac{n-2}{2}}\bar{u}_{i}(r)$ and
		\[
		\bar{u}_{i}(r)=\frac{1}{r^{n-1}}\int_{\exp_{x_{i}}(\partial B_{r})}u_{i}\ud\sigma_{g},
		\]
		with $\partial B_{r}=\{z\in\Rn;|z|=r\}$.
		
		%(anther definition: or in $0<r<1$, $\overline{w}_{i}(r)$ has only one critical point. This is equivalent when using.) 
	\end{definition}
	
	It is not hard to see that if we let $v_{i}(z)=s^{(n-2)/2}u_{i}(\exp_{x_{i}}(sz)):=s^{(n-2)/2}u_i\circ\mathcal{T}_{i}(z)$
	for $s\in\R_{+}$, then 
	\[
	...
	%\textcolor{red}{\emph{In the follwing, we will discuss the blow up
			%of $u_{i}$ with boundary condition $\frac{\partial u_{i}}{\partial n_{g}}=-cu_{i}^{\frac{n}{n-2}}+h_{g}u_{i}=h_{g}u_{i}$
			%with $c=0$. (Because I find no liouville theorem for singular solution
			%about non-vanish $c$. If $c=0$, then the Liouville theorem for half
			%space may be proved by even reflection. Anyway, I didn't find the
			%reference, but I think it is right if $\frac{\partial u}{\partial x_{n}}=0$
			%on $\{x_{n}=0\}$.)}} 
	
	Now we turn to the boundary isolated blow-up point.
	%	point. Given any $x_{i}$ near $\pa M$, let $G_{i}=G_{x_{i}}:z\in B_{\rho}^{+}\to\overline{M}$
	%	be the map inducing Fermi coordinates $z=(z',z_{n})$ around some
	%	$x_{i}'\in\pa M$ with $d_{g}(x_{i},x_{i}')=d_{g}(x_{i},\pa M)$.
	
	\begin{lemma} \label{lem:harnack inequality} 
	...
	\begin{proof}
		...
		\[
		\frac{\partial\tilde{u}_{i}}{\partial\vec{n}_{g_{i}}}=-c\circ\Psi_{i}\tilde{u}_{i}^{\frac{n}{n-2}}+\frac{n-2}{2}h_{g_{i}}\tilde{u}_{i}.
		\]
		Here $\vec{n}_{g_{i}}=r\vec{n}\circ\Psi_{i}$.
		
		By definition of isolated blow-up point in (\ref{def:isolated blow up}),
		...
		The boundary gradient estimate in \cite[Corollary 3.1]{Chen} shows
		that 
		\[
		...
		We may assume 
		\[
		\max_{A_i^{+}(1/2,2)}\tilde{u}_{i}=\tilde{u}_{i}(z_{0})\quad\mathrm{~~and~~}\quad\min_{A_i^{+}(1/2,2)}\tilde{u}_{i}=\tilde{u}_{i}(z_{1}).
		\]
		...
		Observe that 
		\begin{align*}
			\log\max_{A_i^{+}(1/2,2)}\tilde{u}_{i}-\log\min_{A_i^{+}(1/2,2)}\tilde{u}_{i}= & \int_{0}^{1}\langle\nabla\log\tilde{u}_{i},\gamma'(t)\rangle\ud t\le CC_{1}.
		\end{align*}
		This means 
		\[
	...
	\end{proof}
	%\begin{lemma}
	%\label{lem:boundary bounded limit} For $n=3$. Let $x_{i}\rightarrow x_{0}\in\partial M$
	%be an isolated blow-up point of $u_{i}$ with $\sigma_{2}^{1/2}(A_{u_{i}})=u_i^{\frac{4}{n-2}}f+f_0$.
	%Then 
	%\[
	...
	%Let 
	%\[
	%T_{i}=d_{g}(x_{i},\partial M)^{\frac{n-2}{2}}u_{i}(x_{i}).
	%\]
	%Suppose $T_{i}\rightarrow\infty$. Let $d_g(x_i,x_i')=d_g(x_i,\pa M)$ for some $x_i' \in \pa M$ and define
	%$$\Psi_i(z)=G_i(d_{g}(x_{i},\partial M)z) \quad \mathrm{~~and~~}\quad g_i=d_{g}(x_{i},\partial M)^{-2} \Psi_i^\ast(g).$$ 
	%We define
	%\[
	%v_{i}(z)=d_{g}(x_{i},\partial M)^{\frac{n-2}{2}}u_{i}\circ \Psi_i(z),
	%\]
	%then
	%$$g_{v_{i}}=v_{i}^{\frac{4}{n-2}}g_{i}=\Psi_i^\ast(u_i^{\frac{4}{n-2}}g)=\Psi_i^\ast(g_{u_i}).$$
	%Then 
	%\[
	%\sigma_{2}^{1/2}(A_{v_i})=f \circ \Psi_i v_i^{\frac{4}{n-2}}+d_g(x_i, \partial M)^2f_0\quad\mathrm{~~in~~}\quad B_{\frac{\rho}{d_{g}(x_{i},\partial M)}}^+\backslash\{\mathbf{e}_n\}
	%\]
	%and 
	%\[
	%\frac{\partial v_{i}}{\partial n_{g_{i}}}=-c\circ \Psi_{i} v_{i}^{\frac{n}{n-2}}+\frac{n-2}{2}h_{g_{i}}\circ \Psi_{i} v_{i}\quad\mathrm{~~on~~} \{x_n=0\}.
	%\]
	%Since $x_0$ is an isolated blow-up point, there holds 
	%\[
	%|z-\mathbf{e}_n|^{\frac{n-2}{2}}v_{i}(z)\le C
	%\]
	%for any compact $K\subset \overline{\mathbb{R}_+^{n}}$. 
	%Notice that $\mathbf{e}_n$ is a local maximum point of $v_{i}$ and $v_{i}(\mathbf{e}_n)=T_{i}\rightarrow\infty.$
	%By the Harnack inequality, we know 
	%\[
	%0\le\sup_{K_{1}}v_{i}\le c\inf_{K_{1}}v_{i},
	%\]
	%where $c$ is independent of $i$ and $K_{1}$ is a compact set in $\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}.$
	%By Lemma \ref{lem:interior blow up is simple}, $\mathbf{e}_n$ is a simple
	%blow-up point of $v_{i}$ and thus there exists some $\tilde{r} \in \R$
	%such that $\big(s^{\frac{n-2}{2}}\bar{v}_{i}(s)\big)'<0$ in $(\tilde{r_{i}},\tilde{r}),$ where
	%$s=|z-\mathbf{e}_n|, \tilde{r}_i:=R_{i}v_{i}(\mathbf{e}_n)^{-\frac{2}{n-2}}\rightarrow0$ and $R_{i}$
	%is the given sequence.
	%%
	%%\emph{Case 1:} If for any $y_{0}\in\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}$
	%%such that $\lim_{i\rightarrow\infty}v_{i}(y_{0})\neq0,$ then in any
	%%compact set $K\subset\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}$
	%%containing $y_0$, 
	%%\[
	%%\|\log v_{i}\|_{C^{2}(K)}\le C_{K},v_{i}\ge c_{K}.
	%%\]
	%%
	%%Moreover, there exists a subsequence $v_{i}$ converging to $v_{\infty}$ in $C_{\mathrm{loc}}^{3}(\mathbb{R}_+^{n}\backslash\{\mathbf{e}_n\})$ satisfying 
	%%\[
	%%\begin{cases}
	%%\sigma_{2}^{1/2}(A_{v_\infty})=f(x_0) & \mathrm{~~in~~} \mathbb{R}_+^{n}\backslash\{\mathbf{e}_n\}\\
	%%\frac{\partial v_{\infty}}{\partial z_{n}}=-c(x_{0})v_{\infty}^{\frac{n}{n-2}} & \mathrm{~~on~~}  \{z_n=0\},\\
	%%\big(s^{\frac{n-2}{2}}\bar{v}_{\infty}(s)\big)'\le0 &\mathrm{~~for~~} 0<s<\tilde{r}.
	%%\end{cases}
	%%\]
	%%However, it follows from Theorem \ref{thm:Han-Li-Texi} that $v_{\infty}$ is bounded near $\mathbf{e}_n$,
	%%contradicting the fact that $\big(s^{\frac{n-2}{2}}\bar{v}_{\infty}(s)\big)'\le0$
	%%and the positivity of $v_{\infty}.$
	%By Theorem \ref{isolated behavior}, 
	%then $v_{i}(z)\rightarrow0$ in $C_{\mathrm{loc}}^{0}(\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\})$.
	%For any compact set $K\subset\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}$ we have
	%\[
	%\|\nabla \log v_{i}\|_{C^{0}(K)}\le C(K).
	%\]
	%For any compact set $K\subset{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}$, by the classical interior estimate (Guan-Wang, chen. S. Li, Wang etc),  we have
	%\[
	%\|\nabla \log v_{i}\|_{C^{0}(K)}+\|\nabla^2 \log v_{i}\|_{C^{0}(K)}\le C(K).
	%\]
	%Actually we have $v_{i}(z)\rightarrow0$ in $C_{\mathrm{loc}}^{1,1}(\mathbb{R}_+^{n}\backslash\{\mathbf{e}_n\})\cap C_{\mathrm{loc}}^{0,1}(\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}).$
	%We define $\omega_{i}(z)=v_{i}(z)/v_{i}(y_{0})$ with $\omega_{i}(y_{0})=1$. Then $\omega_{i}$ satisfies 
	%\begin{align*}
	%&\sigma_{2}^{1/2}(-\frac{2}{n-2}\frac{\nabla_{g_i}^{2}}{\omega_{i}}\omega_{i}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{i}\otimes_{g_i} \ud\omega_{i}}{\omega_{i}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{i}|_{g_i}^{2}}{\omega_{i}^{2}}g_i+A_{g_{i}})\\
	%=&\omega_{i}^{\frac{4}{n-2}}(z)v_{i}^{\frac{4}{n-2}}(y_{0})f \circ \Psi_i +d_g(x_i, \partial M)^2f_0\qquad\mathrm{~~in~~}\quad B_{\frac{\rho}{d_{g}(x_{i},\partial M)}}^+\backslash\{\mathbf{e}_n\}
	%\end{align*}
	%and $\frac{\partial\omega_{i}}{\partial z_{n}}=-c(x_{0})\omega_{i}^{\frac{n}{n-2}}v_{i}(y_{0})^{\frac{2}{n-2}}$ on $\{z_n=0\}$.
	%By Lemma \ref{lem:harnack inequality}, we actually know $w_{i}(z)\le C_{K}$
	%in every compact set $K\subset\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\}$. Also by the Harnack inequality, we have $w_{i}\geq c_{K}$
	%in $K$ and thus $\|\log w_{i}\|_{C^{1}(K)}\le C$. Now there exists a subsequence
	%$w_{i}$ converging to $w_{\infty}\in C_{\mathrm{loc}}^{1,1}(\mathbb{R}_+^{n}\backslash\{\mathbf{e}_n\})\cap C_{\mathrm{loc}}^{0,1}(\overline{\mathbb{R}_+^{n}}\backslash\{\mathbf{e}_n\})$
	%such that $w_{\infty}$ satisfying the following equation in viscocity sense
	%\[
	%\sigma_{2}(-\frac{2}{n-2}\frac{\nabla_{g_{\mathrm{E}}}^{2}\omega_{\infty}}{\omega_{\infty}}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{\infty}\otimes_{g_E} \ud\omega_{\infty}}{\omega_{\infty}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{\infty}|_{g_E}^{2}}{\omega_{\infty}^{2}}g_{\mathrm{E}})=0\quad\mathrm{in~~}\mathbb{R}_+^{n}\backslash\{\mathbf{e}_n\}
	%\]
	%and $\frac{\partial w_{\infty}}{\partial z_{n}}=0$ on $z_{n}=0.$ Moreover,
	%$\big(s^{\frac{n-2}{2}}\bar{w}_{\infty}(s)\big)'\leq 0$ near zero. (Note that $\bar{w}_{\infty}$ near zero is $C^{1,1}$. ) By the same reason as before, we know that $w_{\infty}$ is singular at $\mathbf{e}_n$. 
	%On the other hand, it follows from Theorem \ref{thm:(Bocher-Type-theorems--Li-Nguyen} then
	%for $n=3$, $w_{\infty}(z)=b|z-\mathbf{e}_n|^{2-n}$ for some $b\in \R_+$. This yields
	%\[
	%\frac{\partial\omega_{\infty}}{\partial z_{n}}\bigg|_{z_{n}=0}=[(2-n)b|z-\mathbf{e}_n|^{-n}(z_{n}-\mathbf{e}_n)]\big|_{z_n=0}=(n-2)b>0,
	%\]
	%which contradicts the boundary condition that $\frac{\partial\omega_{\infty}}{\partial z_{n}}=0$
	%on $\{z_{n}=0\}.$
	%\end{proof}
	
	
	
	%	Thanks to the boundary Harnack inequality in Lemma \ref{lem:harnack inequality},
	%	we can introduce a refined definition of boundary isolated simple
	%	blow-up point. \begin{definition} Suppose $\{x_{i}\}\subset\pa M$.
		%		A blow-up point $x_{i}\rightarrow x_{0}\in\pa M$ for $u_{i}$ is
		%		isolated, if there exist constants $\rho,C>0$ such that 
		%		\[
		%		u_{i}(x)\leq Cd_{g}\left(x,x_{i}\right)^{-\frac{n-2}{2}},\quad\mathrm{~~for~~all~~}x\in\overline{M},~d_{g}\left(x,x_{i}\right)<\rho,
		%		\]
		%		where $x_{i}$ is a local maximum point of $u_{i}|_{\pa M}$ and $u_{i}(x_{i})\rightarrow+\infty.$
		%		
		%		An isolated blow-up point $x_{i}\rightarrow x_{0}\in\partial M$ for
		%		$u_{i}$ is simple, if there exist $\tilde{r},C>0$ such that 
		%		\[
		%		\overline{w}_{i}'(r)<0,\quad\mathrm{~~for~~all~~}r\mathrm{~~satisfying~~}Cu_{i}(x_{i})^{-\frac{n-2}{2}}\le r\le\tilde{r},
		%		\]
		%		where $\overline{w}_{i}(r):=r^{\frac{n-2}{2}}\bar{u}_{i}(r)$ and
		%		\[
		%		\bar{u}_{i}(r)=\frac{1}{|\partial^{+}B_{r}^{+}|}\int_{G_{i}(\partial^{+}B_{r}^{+})}u_{i}\ud\sigma_{g},
		%		\]
		%		where $\partial^{+}B_{r}^{+}=\{|z|=r,z_{n}>0\}$ and $G_{i}:B_{\rho}^{+}\to\overline{M}$
		%		is Fermi coordinates around $x_{i}$. \end{definition}
	%	
	\begin{definition}[New boundary  integral average]\label{def:bdry_blowup_pts}
		...
		\[
		\overline{w}_{i}'(r)<0,\quad\mathrm{~~for~~all~~}Cu_{i}(x_{i})^{-\frac{2}{n-2}}\le r\le\tilde{r},
		\]
		...
		\]
		where $\partial^{+}B_{r,\theta}^{+}=\{z\in\Rn;|z|=r,\mathrm{dist}_{\Sp^{n-1}}(N,z/r)<\theta\}$
	...
	and
	$$\bar{v}_{i}(r)=\frac{1}{r^{n-1}}\int_{\pa^{+}B_{r,\theta}^{+}}v_i(z)\ud \sigma_{s^{-2}\Psi_{i}^\ast(g)}$$
	for $s\in\R_{+}$, then 
	\begin{equation}
		r^{\frac{n-2}{2}}\bar{v}_{i}(r)=(sr)^{\frac{n-2}{2}}\bar{u}_{i}(sr).\label{property_bdry_blowup_pt}
	\end{equation}
	Indeed, this follows from
	\begin{align*}
	...
	By Lemma \ref{lem fudament} we are able to prove an elementary property of the isolated blow-up point.
	...
	\end{lemma} 
	%		\begin{proof}
		%			We only present the proof for the case that $c=0$, which is enough
		%			to our use. For simplicity, we may assume $f(q_{i})=2(C_{n}^{2})^{1/2}$
		%			and first consider $q_{0}\in\pa M$. We define 
		%			\[
		%			\eta_{i}(y):=u_{i}^{-1}\left(q_{i}\right)u_{i}\circ G_{i}\left(u_{i}^{-\frac{2}{n-2}}(q_{i})y+d_i\mathbf{e}_n\right)\quad\mathrm{and}\quad\eta(|y|):=\left(1+|y|^{2}\right)^{(2-n)/2}.
		%			\]
		%			By Lemma \ref{lem fudament}, we can choose $\varepsilon_{i}=o(R_{i}^{1-n})$
		%			such that 
		%			\[
		%			\|\eta_{i}-\eta\|_{C^{2}(B_{R_{i}}(0))}\le\varepsilon_{i}.
		%			\]
		%			
		%			A direct computation yields 
		%			\[
		%			(s^{\frac{n-2}{2}}\eta(s))'=\frac{n-2}{2}s^{\frac{n-2}{2}-1}\eta(s)+s^{\frac{n-2}{2}}\eta'(s)=\frac{n-2}{2}\left(\frac{s}{1+s^{2}}\right)^{\frac{n}{2}}\frac{1-s^{2}}{s^{2}}
		%			\]
		%			and 
		%			\begin{align*}
			%				(s^{\frac{n-2}{2}}\eta(s))''=\frac{n-2}{4}\frac{s^{\frac{n}{2}-3}}{(1+s^{2})^{\frac{n+2}{2}}}\left[ns^{4}-2(n+2)s^{2}+(n-4)\right].
			%			\end{align*}
		%			Then with a positive dimensional constant $C$, for $\sqrt{(n+2)/n}\le s\le3R_{i}$
		%			we have 
		%			\begin{equation}
			%				(s^{\frac{n-2}{2}}\eta(s))'\le-Cs^{-\frac{n}{2}}\qquad\mathrm{~~for~~}\sqrt{\frac{n+2}{n}}\le s\le3R_{i}.\label{negative_first_derivative}
			%			\end{equation}
		%			
		%			Observe that 
		%			\begin{align*}
			%				\overline{\eta}_{i}(s)=\frac{1}{s^{n-1}}\int_{\partial^{+}B_{s,\pi/4}^{+}}\eta_{i}(y)\ud\sigma_{G_{i}^{\ast}(g)}=\int_{\partial^{+}B_{1,\pi/4}^{+}}\eta_{i}(sz)\ud\sigma_{g_{i}},
			%			\end{align*}
		%			where $g_{i}=s^{-2}u_i^{\frac{4}{n-2}}(q_i)\Psi_{i}^{\ast}(g)$ with $\Psi_{i}(z)=G_{i}(sz):=G_{q_i'}\left(u_{i}^{-\frac{2}{n-2}}(q_{i})y+d_i\mathbf{e}_n\right)$.
		%			The $\pi/4$ is for example and  we can choose $\theta\in (0,\pi/2)$.
		%			By choosing $\ve_{i}$ sufficiently small, we have 
		%			\begin{align}
			%				& (s^{\frac{n-2}{2}}\overline{\eta}_{i}(s))'\no\label{eq:first derivative}\\
			%				= & \frac{n-2}{2}s^{\frac{n-2}{2}-1}\overline{\eta}_{i}(s)+s^{\frac{n-2}{2}}\overline{\eta}_{i}'(s)\no\\
			%				= & \frac{n-2}{2}s^{\frac{n-2}{2}-1}\overline{\eta}_{i}(s)+s^{\frac{n-2}{2}}\int_{\partial^{+}B_{1,\pi/4}^{+}}\langle(\nabla\eta_{i})(sz),z\rangle\ud\sigma_{g_{i}}\nonumber \\
			%				\le & \frac{n-2}{2}s^{\frac{n-2}{2}-1}(\varepsilon_{i}+\eta)\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})+s^{\frac{n-2}{2}}\int_{\partial^{+}B_{1,\pi/4}^{+}}(\varepsilon_{i}+\langle(\nabla\eta)(sz),z\rangle)\ud\sigma_{g_{i}}\nonumber \\
			%				\le & C_{1}(\frac{n-2}{2}s^{\frac{n-2}{2}-1}\varepsilon_{i}+s^{\frac{n-2}{2}}\varepsilon_{i})+[\frac{n-2}{2}s^{\frac{n-2}{2}-1}\eta(s)+s^{\frac{n-2}{2}}\eta'(s)]\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})\nonumber \\
			%				\le & (s^{\frac{n-2}{2}}\eta(s))'\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})+C_{1}\varepsilon_{i}s^{n-1}s^{-\frac{n}{2}}\nonumber \\
			%				\le & -Cs^{-\frac{n}{2}}\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})+C_{1}\varepsilon_{i}(3R_{i})^{n-1}s^{-\frac{n}{2}}\nonumber \\
			%				\le & -\frac{C}{2}s^{-\frac{n}{2}}\qquad\mathrm{~~for~~}\quad\sqrt{\frac{n+2}{n}}\le s<3R_{i}.
			%			\end{align}
		%			
		%			\iffalse This in turn implies 
		%			\[
		%			\overline{w}_{i}'(r)<0,\quad\mathrm{~~for~~all~~}r\mathrm{~~satisfying~~}\quad\sqrt{\frac{n+2}{n}}u_{i}(q_{i})^{-\frac{2}{n-2}}\le r\le r_{i}=R_{i}u_{i}(q_{i})^{-\frac{2}{n-2}}.
		%			\]
		%			\fi
		%			
		%			Fix $\e_{0}\in(0,1)$ for $n=3,4$ and 
		%			\[
		%			\e_{0}\in(0,1-\left(\frac{n+2-2\sqrt{2n+1}}{n}\right)^{1/2})\qquad\mathrm{when~~}n\geq5.
		%			\]
		%			Similar as (\ref{eq:first derivative}), for $0<s\le1-\e_{0}$
		%			we have 
		%			\begin{align}
			%				& (s^{\frac{n-2}{2}}\overline{\eta}_{i}(s))'\no\label{eq:first derivative lower positive}\\
			%				\geq & -C_{2}(\frac{n-2}{2}s^{\frac{n-2}{2}-1}\varepsilon_{i}+s^{\frac{n-2}{2}}\varepsilon_{i})+(s^{\frac{n-2}{2}}\eta(s))'\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})\no\\
			%				\ge & \frac{n-2}{2}\left(\frac{s}{1+s^{2}}\right)^{\frac{n}{2}}\frac{1-s^{2}}{s^{2}}\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})-C_{2}(\frac{n-2}{2}s^{\frac{n-2}{2}-1}\varepsilon_{i}+s^{\frac{n-2}{2}}\varepsilon_{i})\nonumber \\
			%				\geq & s^{\frac{n}{2}-2}\left(\frac{(n-2)(1-s^2)}{2(1+s^2)^{\frac{n}{2}}}\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})-C_{2}(\frac{n-2}{2}\varepsilon_{i}+s\varepsilon_{i})\right)>0.
			%			\end{align}
		%			%			where 
		%			%			\[
		%			%			m_{1}(n,\e_{0}):=\min_{[0,1-\e_{0}]}(s^{\frac{n-2}{2}}\eta(s))'>0.
		%			%			\]
		%			%			
		%			Next we choose $\ve_{i}$ sufficiently large such that for $n=3, 4$
		%			and $1-\e_{0}\leq s\leq\sqrt{(n+2)/n}$, 
		%			\begin{align}
			%				& (s^{\frac{n-2}{2}}\overline{\eta}_{i}(s))''\no\label{eq:second derivative of standard function}\\
			%				= & \frac{n-2}{2}(\frac{n-2}{2}-1)s^{\frac{n-2}{2}-2}\overline{\eta}_{i}(s)+(n-2)s^{\frac{n-2}{2}-1}\int_{\partial^{+}B_{1,\pi/4}^{+}}\langle(\nabla\eta_{i})(sz),z\rangle\ud\sigma_{g_{i}}\no\\
			%				& +s^{\frac{n-2}{2}}\int_{\partial^{+}B_{1,\pi/4}^{+}}\langle z(\nabla^{2}\eta_{i})(sz),z\rangle\ud\sigma_{g_{i}}\nonumber \\
			%				\le & \frac{n-2}{2}(\frac{n-2}{2}-1)s^{\frac{n-2}{2}-2}(\eta+\mathrm{sign}(n-4)\varepsilon_{i})\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})\nonumber \\
			%				& +(n-2)s^{\frac{n-2}{2}-1}\int_{\partial^{+}B_{1,\pi/4}^{+}}(\varepsilon_{i}+\langle(\nabla\eta)(sz),z\rangle)\ud\sigma_{g_{i}}\nonumber \\
			%				& +s^{\frac{n-2}{2}}\int_{\partial^{+}B_{1,\pi/4}^{+}}(\langle z(\nabla^{2}\eta)(sz),z\rangle+\varepsilon_{i})\ud\sigma_{g_{i}}\nonumber \\
			%				\le & (s^{\frac{n-2}{2}}\eta(s))''\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})+C_{1}\varepsilon_{i}(s^{\frac{n-2}{2}-2}+s^{\frac{n-2}{2}-1}+s^{\frac{n-2}{2}})\no\\
			%				\leq & s^{\frac{n}{2}-3}\left(-m_{2}(n,\e_{0})\mathrm{Vol}(\partial^{+}B_{1,\pi/4}^{+},g_{i})+C_{1}\varepsilon_{i}(1+s+s^{2})\right)<0.
			%			\end{align}
		%			%		
		...
		%			$1$ if $n\geq5$ and 
		%			\[
		%			-m_{2}(n,\e_{0}):=\max_{[1-\e_{0},\sqrt{\frac{n+2}{n}}]}\frac{(s^{\frac{n-2}{2}}\eta(s))''}{s^{\frac{n}{2}-3}}<0.
		%			\]
		%			
		%			Therefore, we combine \eqref{negative_first_derivative}, \eqref{eq:first derivative}
		%			and \eqref{eq:second derivative of standard function} to conclude
		%			that for $n\geq3$, there exists a unique point $s_{0}\in(1-\e_{0},\sqrt{(n+2)/n})$
		%			such that $(s^{(n-2)/2}\overline{\eta}_{i}(s))'|_{s=s_{0}}=0$, which
		%			also indicates that $s_{0}$ is the unique critical point of $s^{(n-2)/2}\overline{\eta}_{i}(s)$
		%			in $(0,R_{i})$.
		%			
		%			Finally, by letting $r=u_{i}^{-2/(n-2)}(q_{i})s$ and the property
		%			\eqref{property_bdry_blowup_pt} we obtain 
		%			\[
		%			(s^{\frac{n-2}{2}}\overline{\eta}_{i}(s))'=\frac{\ud}{\ud r}[r^{\frac{n-2}{2}}\overline{u}_{i}(r)]\frac{\ud r}{\ud s}=u_{i}^{-\frac{2}{n-2}}\left(q_{i}\right)\frac{\ud}{\ud r}[r^{\frac{n-2}{2}}\overline{u}_{i}(r)].
		%			\]
		%			This implies that $r^{(n-2)/2}\overline{u}_{i}(r)$ has only one critical
		%			point in $(0,r_{i})$.
		%			
		%			Indeed, the above same idea also works well for interior isolated
		%			blow-up point $q_{0}\in M$. 
		%		\end{proof}
	...
	\begin{proof}
		...
		\begin{equation}\label{key estimate}
			\left\Vert \frac{1}{u_{i}(x_{i})}u_{i}\circ G_{x_{i}'}\big(\frac{y}{u_{i}^{\frac{2}{n-2}}\left(x_{i}\right)}+d_i\mathbf{e}_n\big)-\big(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_0)}\big)^{\frac{n-2}{4}}(1+b|y+T_c\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\right\Vert _{C^{2}(B^+_{3R_{i}})}<\varepsilon_{i}.
		\end{equation}
		
		For simplicity, we let 
		$$\eta_i(y)= \frac{1}{u_{i}(x_{i})}u_{i}\circ G_{x_{i}'}\left(u_{i}^{-\frac{2}{n-2}}\left(x_{i}\right)y+d_i\mathbf{e}_n\right)$$
		and 
		$$\eta(y)=\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_0)}\right)^{\frac{n-2}{4}}(1+b|y+T_c\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}.$$
		...
		\begin{align*}
			...
			& =\bigg(\frac{2b\left(C_{n}^{2}\right)^{\frac{1}{2}}}{f\left(x_{0}\right)}\bigg)^{\frac{n-2}{4}}\int_{\partial^+ B_{1,\theta }^{+}}\left(1+b\left(\left|sz^{\prime}\right|^{2}+\left|sz_{n}+T_c\right|^{2}\right)\right)^{\frac{2-n}{2}}\ud\sigma.
		\end{align*}
		
		A direct computation yields
		\begin{align*}
			&\bigg(\frac{2b\left(C_{n}^{2}\right)^{\frac{1}{2}}}{f\left(x_{0}\right)}\bigg)^{\frac{2-n}{4}} \big(s^{\frac{n-2}{2}}\bar{\eta}(s)\big)'\\
			= & \frac{n-2}{2}s^{\frac{n-2}{2}-1}\int_{\partial^+ B_{1,\theta }^{+}}\bigg(1+b\left(\left|sz^{\prime}\right|^{2}+\left|sz_{n}+T_c\right|^{2}\right)\bigg)^{-\frac{n}{2}}\times\\
			& \qquad\qquad\qquad\bigg(1+b(\left|sz^{\prime}\right|^{2}+\left|sz_{n}+T_c\right|^{2})-2bs\big(s|z'|^{2}+(sz_{n}+T_c)z_{n}\big)\bigg)\ud\sigma\\
			= & \frac{n-2}{2}s^{\frac{n-2}{2}-1}\big(1-bs^{2}+bT_c^{2}\big)\int_{\partial^+ B_{1,\theta }^{+}}\big(1+b(s^{2}+2sz_{n}T_c+T_c^{2})\big)^{-\frac{n}{2}}\ud\sigma
		\end{align*}
		and
		\begin{align}
			&\bigg(\frac{2b\left(C_{n}^{2}\right)^{\frac{1}{2}}}{f\left(x_{0}\right)}\bigg)^{\frac{2-n}{4}} \big(s^{\frac{n-2}{2}}\bar{\eta}(s)\big)''\nonumber\\
			= & \frac{n-2}{2}s^{\frac{n-2}{2}-2}\int_{\partial^+ B_{1,\theta }^{+}}\bigg(1+b\left(\left|sz^{\prime}\right|^{2}+\left|sz_{n}+T_c\right|^{2}\right)\bigg)^{-\frac{n}{2}-1}\cdot A(s,z) \ud \sigma. \label{second derivatives of standard function}
		\end{align}
		where 
		\begin{align*}
			A(s,z):=&\bigg(1+b(s^{2}+2sz_{n}T_c+T_c^{2})\bigg)\bigg((1-bs^{2}+bT_c^{2})(\frac{n}{2}-2)-2bs^{2}\bigg)\\
			&-nbs\big(s+T_cz_{n}\big)\big(1-bs^{2}+bT_c^{2}\big).
		\end{align*}
		
		Keep in mind that the sign of $1-bs^{2}+bT_c^{2}$ coincides with $\big(s^{\frac{n-2}{2}}\bar{\eta}(s)\big)'$. 
		
		Let $s_{0}:=\sqrt{\frac{1+bT_c^{2}}{b}}$, then $1-bs^{2}+bT_c^{2}\ge0$
		for $0< s\le s_{0}$. 
		
		Fix some $\alpha \in (0,1)$ to be determined later and let $\varepsilon_{0}=\alpha s_0$, then
		\begin{align*}
			1-bs^{2}+bT_c^{2} \ge bs_0^2(2\alpha-\alpha^2)>0&\quad\mathrm{for~~}\quad 0<s\le s_{0}(1-\alpha);\\
			1-bs^{2}+bT_c^{2} \leq -bs_0^2(2\alpha+\alpha^2)<0&\quad\mathrm{for~~}\quad s\ge s_{0}(1+\alpha);\\
			-bs_0^2(2\alpha+\alpha^2)< 1-bs^{2}+bT_c^{2}< bs_0^2(2\alpha-\alpha^2) &\quad\mathrm{for~~}\quad s_0(1-\alpha)<s<s_0(1+\alpha).
		\end{align*}
		
		For $n=3, 4$ and $s_0(1-\alpha)<s<s_0(1+\alpha)$, there holds 
		\[(1-bs^{2}+bT_c^{2})(\frac{n}{2}-2)-2bs^{2}\le -(\frac{n}{2}-2)bs_0^2(2\alpha+\alpha^2)-2b(1-\alpha)^2s_0^2; \]
		for $n\ge 5$ and $s_0(1-\alpha)<s<s_0(1+\alpha)$, there holds
		\[(1-bs^{2}+bT_c^{2})(\frac{n}{2}-2)-2bs^{2}\le (\frac{n}{2}-2)bs_0^2(2\alpha-\alpha^2)-2b(1-\alpha)^2s_0^2.\]
		Meanwhile, for $n\ge 3$ and $s_0(1-\alpha)<s<s_0(1+\alpha)$, 
		\[-nbs(s+T_cz_{n})(1-bs^{2}+bT_c^{2})\le n bs_0(1+\alpha)(s_0(1+\alpha)+T_c)bs_0^2(2\alpha+\alpha^2).\]
		Thus we may choose $\alpha$ small enough such that for $s_0(1-\alpha)<s<s_0(1+\alpha)$,
		\begin{align*}
			A\le & \big(1+bT_c^{2}\big)\big((2-\frac{n}{2})(2\alpha+\alpha^2)-2(1-\alpha)^2\big)bs_0^2\\
			&+nbs_0(1+\alpha)\left(s_0(1+\alpha)+T_c\right)bs_0^2(2\alpha+\alpha^2):=-c_2<0\quad 
			\mathrm{for}\quad  n=3,4;
		\end{align*}
		and 
		\begin{align*}
			A \le &\big(1+bT_c^{2}\big)\big((\frac{n}{2}-2)(2\alpha-\alpha^2)-2(1-\alpha)^2\big)bs_0^2\\
			&+nbs_0(1+\alpha)\left(s_0(1+\alpha)+T_c\right)bs_0^2(2\alpha+\alpha^2):=-c_3<0\quad \mathrm{for} \quad n\ge 5.
		\end{align*}
		...
		\[
		s^{2}+2sz_{n}T_c+T_c^{2}\le4s^{2},\quad1-bs^{2}+bT_c^{2}=b(s_0^2-s^2)<-bs^{2}\frac{2\alpha+\alpha^2}{(1+\alpha)^2}
		\]
		and 
		...
		\begin{equation}\label{first lower derivative_1}
			\big(s^{\frac{n-2}{2}}\bar{\eta}(s)\big)'\ge\bigg(\frac{2b\left(C_{n}^{2}\right)^{\frac{1}{2}}}{f\left(x_{0}\right)}\bigg)^{\frac{n-2}{4}}\frac{n-2}{2}s^{\frac{n-2}{2}-1}c_{0}\ge c_1 s^{\frac{n-2}{2}-1}.
		\end{equation}
	...
	%	\begin{remark} We also can relax to define $\bar{u}_{i}(r)$ in the
		%		following way 
		%		\[
		%		\bar{u}_{i}(r)=\frac{1}{r^{n-1}}\int_{G_{i}(\pa^{+}B_{r,\theta}^{+})}u_{i}\ud\sigma_{g},
		%		\]
		%		where $\partial^{+}B_{r,\theta}^{+}=\{z\in\Rn;|z|=r,\mathrm{dist}_{\Sp^{n-1}}(N,z)<\theta\}$
		%		for some $\theta\in(0,\pi/2)$. Furthermore, the property \eqref{property_bdry_blowup_pt}
		%		still holds. This is enough to our later use. \end{remark} (\textcolor{red}{This
		%		definition ensures that we only use the estimates away from boundary
		%		and we would like to adopt this definition})
	...
	\begin{lemma}[\protect{Li-Nguyen \cite[Lemma 3.4]{Li-Luc2}}]
		...
		\]
	\end{lemma}
	We  next give boundary singularity behavior near a boundary blow-up point under the boundary condition that $\frac{2}{n-2}\frac{\partial u}{\partial\vec{n}}=-cu^{\frac{n}{n-2}}+h_g u$, which is more subtle due to  the effect of the second fundamental form.
	
	\begin{lemma} \label{lem:four-dim lower bound-boundary}
		%	Assume that $\Gamma$ satisfies (1), (2) and that $\mu_{\Gamma}^{+}=1$. 
		...
		satisfies
		\[
		\lambda\left(g^{-1}A_{\underline{v}_{\delta}^{\frac{4}{n-2}}g}\right)\in\mathbb{R}^{n}\backslash\overline{\Gamma_2^{+}}\quad\mathrm{~~in~~}\left\{ 0<r(x)<r_{1}\right\} 
		\]
		...
		Furthermore,   for sufficiently small $\delta$ there holds  $C^{-1}\delta \le r_{1}\le C\sqrt{\delta}$ for some positive constant $C$ depending on $g$ and $L_{\alpha \beta}(x_0)$.
	...
	\begin{proof}
		...
		\begin{align*}
			(\widetilde{L}_{ij})=\left(\begin{array}{cc}
				L_{\alpha\beta} & 0\\
				0 & 0
			\end{array}\right),
		\end{align*}
		whence,
		\[
		g_{ij}=\delta_{ij}-2\widetilde{L}_{ij}x_{n}+O(|x|^{2}),
		\]
		\[ g^{ij}=\delta_{ij}+2\widetilde{L}_{ij}x_{n}+O(|x|^{2}).\]
		Write $g=\sqrt{g}\sqrt{g}$ as before, then the inverse of $\sqrt{g}$ has
		\[
		\sqrt{g}^{ij}=\delta_{ij}+\tilde{L}_{ij}x_{n}+O(|x|^{2}).
		\]
		
		Suppose $v(x)=v(r)$.  A direct computation shows 
		\begin{align*}
			v_{,\alpha\beta}= & \pa_{\beta}\pa_{\alpha}v-L_{\alpha\beta}v_{,n}+O(r)|v'|,\qquad v_{,nn}=  \pa_{n}\pa_{n}v,\\
		...
		Using
		\[
		|\nabla v|_{g}^{2}=g^{ij}v_{i}v_{j}=(\delta^{ij}+2\widetilde{L}_{ij}x_{n}+O(|x|^{2}))v_{i}v_{j}
		\]
		we have
		\begin{align*}
			...
			= & -\frac{2}{n-2}\frac{1}{v}\bigg(\frac{v''}{r^{2}}x_{i}x_{j}+\frac{v'}{r}\delta_{ij}-\frac{v'}{r^{3}}x_{i}x_{j}\bigg)+\frac{2n}{(n-2)^{2}}\frac{(v')^{2}}{v^{2}}\frac{x_{i}x_{j}}{r^{2}}-\frac{2}{(n-2)^{2}}\frac{(v')^{2}}{v^{2}}g_{ij}\\
			& +O(r^{2})\frac{(v')^{2}}{v^{2}}g_{ij}-\frac{4}{(n-2)^{2}}(\frac{v'}{v})^{2}L_{\alpha\beta}x_{\alpha}x_{\beta}x_{n}g_{ij}+A_{ij}\\
		...
		\begin{align*}
			...
			\end{array}\bigg)\\
			=&-\frac{2}{n-2}\frac{v'}{rv}\bigg(\begin{array}{cc}
				L_{\alpha\beta}x_n+O(r^2) &L_{\alpha\beta}x_{\beta}\\
				(L_{\alpha\beta}x_{\beta})^{\top} & 0
			\end{array}\bigg)=O(\big|\frac{v'}{v}\big|).
		\end{align*}
		This enables us to decompose
		\begin{align*}
			\sigma_2(g^{-1}A_\delta)=& \sigma_{2}(g^{-1}({D}+B))\\
			= & \sigma_{2}\big((\sqrt{g})^{-1}({D}+B)(\sqrt{g})^{-1}\big).
		\end{align*}
		
		We now choose the auxiliary function $v$ as
		$$v(r)=r^{-(n-2-\delta)}f(r),$$
		...
		\begin{align*}
			...
			= & (\delta_{kp}+\tilde{L}_{kp}x_{n}+O(|x|^{2}))\frac{x_{p}x_{q}}{r^{2}}(\delta_{qj}+\tilde{L}_{qj}x_{n}+O(|x|^{2}))\\
			= & \frac{x_{k}x_{j}}{r^{2}}+\frac{x_{k}x_{q}x_{n}}{r^{2}}\widetilde{L}_{qj}+\widetilde{L}_{kp}\frac{x_{n}x_{p}x_{j}}{r^{2}}+O(|x|^{2}).
		\end{align*}		
		Then we arrange
		\begin{align*}
			...
			& +|\chi_{2}|O(|x|^2)-\frac{4}{(n-2)^{2}}(\frac{v'}{v})^{2}L_{\alpha\beta}x_{\alpha}x_{\beta}x_{n}\delta_{kj}+O(|x|^{2})(\frac{v'}{v})^{2}\delta_{kj}+\sqrt{g}^{kp}A_{pq}\sqrt{g}^{qj}\\
		...
		\begin{align*}
			...
			(D_3)_{kj} =&|\chi_{2}|O(|x|^2)-\frac{4}{(n-2)^{2}}(\frac{v'}{v})^{2}L_{\alpha\beta}x_{\alpha}x_{\beta}x_{n}\delta_{kj}\\
			&+O(|x|^{2})(\frac{v'}{v})^{2}\delta_{kj}+\sqrt{g}^{kp}A_{pq}\sqrt{g}^{qj},\\
		...
		\end{align}
		Here we have used the definition of mixed symmetric functions (see \cite[Definition 1]{Chen}).
		...
		When $0<r<(1-\delta)\sqrt{\delta}$ for any $0<\delta<1/2$, we know that $|\frac{v'}{v}|\le \frac{C}{r}$, $|\chi_1|\le \frac{C\delta}{r^2}$ and $|\chi_2|\le \frac{C\delta}{r^2}$ for some postive constant $C$.  Thus, $|D_1|\le \frac{C\delta}{r^2} $,  $|D_2|\le \frac{C\delta}{r}$,  $|D_3|\le C$,  $|D_4|\le \frac{C}{r} $. 
		...
		\end{align*}
		Then, for $0<r<r_1:=\min\{\frac{\delta(1-\delta)^2}{4C},(1-\delta)\sqrt{\delta}\}$ we have
		...
		Now taking $f=e^{br}$ with $b \in \R_+$ to be determined later, we have
		\[
		...
		\fi			
		%			Now for simplicity, we assume that $L_{\alpha\beta}$ is diagnoal
		%			and $TrL_{\alpha\beta}=0.$
		This implies	
		...
		By (\ref{eq:summation}),\eqref{D1-D2-D4} and \eqref{extra summation} we have 
		\[
	...
	\begin{remark}
		For $x_0$ close to the boundary, we may take $r=r_{x_i'}$ with $d_g(x_i, x_i')=d_g(x_i, \partial M)$ for some $x_i' \in \pa M$, then Lemma \ref{lem:four-dim lower bound-boundary} still holds.
	\end{remark}
	...
	\begin{lemma}\label{lem:superharmonic}
		...
		on $\Omega\cap\partial M$ (if nonempty) away from $p$, then with $r(x)=d_{g}(x,p)$ when $p\in M$, there holds
		\[
	...
	\begin{proof}
		...
		a direct computation yields 
		\[
		L_{g}G=\frac{1}{2}K(n-\frac{5}{2})r^{\frac{1}{2}-n}+O(r^{1-n})>0, ~~G>0  \qquad \mathrm{~~in~~} B_{\delta}^{+}\backslash\{0\}.
		\]
		Moreover, we notice that
		\begin{align*}
			G=0 & \quad\mathrm{~~on~~}\quad\pa^{+}B_{\delta}^{+};\\
			\frac{\underline{w}(\rho)}{G(\rho)}\frac{\partial G(r)}{\partial x_{n}}=0\geq\frac{\partial w}{\partial x_{n}}=-cw^{\frac{n}{n-2}}+\frac{n-2}{2}h_g w & \quad\mathrm{~~on~~}\quad D_\delta \backslash\{0\}.
		\end{align*}
		...
		nonnegative in $(0,\delta)$. Thus, we know that $\lim_{r\rightarrow0}\underline{w}(r)/G(r)$
	...
	\begin{theorem}\label{Thm:isolated behavior}
		For $n=3,4$, let $u_{i}$ be a sequence of positive solutions to \eqref{PDE:sigma_2_new}
		and $x_{i}\rightarrow x_{0}$ be an isolated blow-up
		point for $u_i$. Assume that $c\ge 0$ and $h_g\le 0$ on $\partial M$ and whenever $x_{0}\in\partial M$, 
		\[
		\frac{2}{n-2}\frac{\partial u_{i}}{\partial\vec{n}}=-cu_{i}^{\frac{n}{n-2}}+h_{g}u_{i}\quad\mathrm{~~on~~}\quad\partial M\cap B_{r_{0}}(x_{0}).
		\]
		Then there exists $v_{\infty}\in C_{\mathrm{loc}}^{1,1}(B_{r_{0}}(x_{0})\backslash\{x_{0}\})$
		...
		For $n=3$ and $x_0 \in \overline M$,	 there holds $\lim_{x\rightarrow x_{0}}v_{\infty}(x)r_{x_0}^{n-2}(x)=a$ for some $a \in \R_+$.
		
		For $n=4$, 	if $x_0\in \partial M$, then with a positive constant $N=N(\delta)$ there holds
		\[v_{\infty}(x)\geq  \frac{N}{r_{x_0}^{n-2-\delta}(x)}
		\qquad\mathrm{~~in~~} \left\{ 0<r_{x_0}(x)\leq C_3\delta\right\} \mathrm{~~for~~any~~}
		0<\delta<1;\]
		if $x_0\in M$, then $$\lim_{x\rightarrow x_{0}}v_{\infty}(x)r_{x_0}^{n-2}(x)=a.$$
	\end{theorem} 
	\begin{proof}
		...
		By contradiction,  using the local $C^1$ estimates we know that up to a subsequence, $u_{i}\rightarrow u_{\infty}>0$ in $C_{\mathrm{loc}}^{0,\alpha}(B_{r_{0}}(x_{0})\backslash\{x_{0}\}\cap\overline{M})$ for any $0<\alpha<1$
		and thus
		\begin{equation}\label{limit decay}
			u_{\infty}(x)\le C_{1}d(x,x_{0})^{-\frac{n-2}{2}},~\forall~x\in\overline{M},~~d_{g}(x,x_0)<\rho.
		\end{equation}
		\begin{claim}
			(1) If $x_{0}\in M$, then for any $0<\delta<(n-2)/2$, there
			exist positive constants $N$ and $r_{1}$ independent of $i$, and $K=K(\delta)$
			and  $i_0=i_0(\delta)\in \N$ such that for all $i\geq i_0$, we have 
			\[
			u_{i}(x)\geq\frac{N}{d_{g}^{n-2-\delta}(x,x_{i})}\quad\mathrm{~~in~~}\left\{ Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}\leq d_{g}(x,x_{i})\leq r_{1}\right\} ,
			\]
			where $r_{1}$ is given in Lemma \ref{lem:four-dim lower bound-boundary}.
			
			(2) If $x_{0}\in\partial M$, then for any $0<\delta<(n-2)/2$, there
			exist positive  constants $N=N(\delta)$ and $r_{1}$ independent of $i$, $K=K(\delta)>0$
			and $i_0=i_0(\delta)\in \N$ such that for all $i\geq i_0$, we have 
			\[
			u_{i}(x)\geq\frac{N}{r_{x_i'}^{n-2-\delta}(x)}\quad\mathrm{~~in~~}\left\{ Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}\leq r_{x'_i}(x)\leq r_{1},z_{n}+d_i\ge 0, x=G_{x_i'}(z+d_i\mathbf{e}_n)\right\} ,
			\]
		...
		It suffices to prove Claim \emph{(2)} as the proof of Claim \emph{(1)} is identical to Li-Nguyen's. 
		
		We can find a positive constant $N$ independent of $i$ such that
		\[
		u_{i}\geq N\underline{v}_{\delta}\quad\mathrm{~~on~~}\left\{ r_{x_i'}=r_{1}\right\},
		\]
		where 	
		$$
		...
		\[
		\bar{\beta}=\sup\left\{ 0<\beta<N;u_{i}(x)\geq\beta\underline{v}_{\delta}(x)\quad\mathrm{in~~}\left\{ r_{i}:=Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}<r_{x_i'}(x)<r_{1},x_{n}\ge0\right\} \right\} .
		\]
		
		Our goal is to show that for all sufficiently large $i$, there exists a large $K=K(\delta) \in \R_+$ such that  $\bar{\beta}=N$.
		
		By contradiction, suppose $\bar{\beta}<N$. We can find
		...
		First notice that $\hat x_{i}$ can
		not be on $\{r_{x_i'}=r_{1},x_n\geq 0\}$ due to the definition of $\bar{\beta}$. The following argument is to show that $r_{x_i'}(\hat x_i)=r_{i}$. 
		
		To this end, we may assume $r_{i}< r_{x_i'}(\hat x_i)< r_{1}$ and need to exclude two possibilities:
		\begin{itemize}
		...
		\[
		\frac{1}{u_{i}\left(x_{i}\right)}u_{i}\circ \exp_{x_i}(\frac{y}{u_{i}\left(x_{i}\right)^{\frac{2}{n-2}}})\rightarrow\left(1+\frac{f(x_{0})}{2(C_{n}^{2})^{\frac{1}{2}}}|y|^{2}\right)^{\frac{2-n}{2}}\quad\mathrm{~~in~~}\quad C_{\mathrm{loc}}^{2,\alpha}(\mathbb{R}^{n});
		\]
		\\
		or if $\lim_{i\rightarrow \infty}d_iu_i(x_i)^{2/(n-2)}$ is bounded, then
		\[
		\frac{1}{u_{i}(x_{i})}u_{i}\circ G_{x_i'}(\frac{y}{u_{i}(x_{i})^{\frac{2}{n-2}}}+d_i\mathbf{e}_n)\rightarrow\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_{0})}\right)^{\frac{n-2}{4}}(1+b|y+{T_c}\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\quad\mathrm{~~in~~}\quad C_{\mathrm{loc}}^{2,\alpha}(\overline{\mathbb{R}_{+}^{n}}).
		\]
		...
		This means that given $R>0,\varepsilon>0$, for all sufficiently large $i$ there holds 
		\[	
		\|\frac{1}{u_{i}\left(x_{i}\right)}u_{i}\circ G_{x_i'}(\frac{y}{u_{i}\left(x_{i}\right)^{\frac{2}{n-2}}}+d_i\mathbf{e}_n)-\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_{0})}\right)^{\frac{n-2}{4}}(1+b|y+{T_c}\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\|_{C^{2}(B_{R}^{+})}<\varepsilon.
		\]
		
		It is enough to consider the latter case that $\lim_{i\rightarrow \infty }d_iu_i(x_i)^{2/(n-2)}$ is bounded, since the front case is relatively easier.
		...
		and 
		\[
		\varphi(y)=\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_{0})}\right)^{\frac{n-2}{4}}(1+b|y+T_c\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}.
		\]
		
		For any $\ve_{0}\in(0,1)$,
		there exists a positive constant $K\gg 1$ such that for all $|y|>K/2$, 
		\[
		\frac{b(|y|^{2}+T_cy_{n})}{1+b(|y'|^{2}+(y_{n}+T_c)^{2})}\ge1-\ve_{0}
		\]
		and \[ 	\frac{2b(|y'|+|y_{n}+T_c|)}{1+b(|y'|^{2}+(y_{n}+T_c)^{2})}\le \frac{4}{K}.
		\]
		...
		\begin{align}\label{est2:D_log_bubble}
			...
			=&- \frac{n-2}{2}(y_{i}\partial_{y_{i}})\log[1+b(|y'|^{2}+(y_{n}+{T_c})^{2})]\no\\
			= & -\frac{n-2}{2}\frac{2b(|y|^{2}+{T_c}y_{n})}{1+b(|y'|^{2}+(y_{n}+{T_c})^{2})}\no\\
			\le & -(n-2)(1-\ve_{0}),
		\end{align}
		...
		Therefore, we conclude that $\bar \beta=N$ and $u_{i}\rightarrow0$ in $C^0_{\mathrm{loc}}(B_{r_{0}}(x_{0})\backslash\{x_{0}\}\cap \overline{M}).$
		\vskip 8pt
		\emph{Step 2:} By local $C^{1}$- estimate,  we know 
		\[
		...
		\begin{align*}
			\begin{cases}
				...
				\frac{\partial v_{\infty}}{\partial\vec{n}}=\frac{n-2}{2}h_{g}v_{\infty} & \quad\mathrm{~~on~~}\partial M\backslash \{x_0\},\quad\mathrm{in~~viscosity~~sense}.
			\end{cases}
		\end{align*}
		...
		\end{align*}
		Then it follows from Harnack inequality that 
		$A:=\lim\sup_{x\rightarrow0}r(x)^{n-2}v_{\infty}(x)$ is finite.
		\vskip 8pt
		
		Now it remains to show that $A=a$. Since  the case of interior blow-up
		point has been well settled in Li-Nguyen \cite[P. 3753]{Li-Luc2},
		we may assume $x_{0}\in\pa M$.
		...
		\]
		Then $g_i \to g_E$ in $C_{\mathrm{loc}}^3(\overline{\Rn_+}\backslash\{0\})$ and $\hat{v}_{i}\in C_{\mathrm{loc}}^{1,1}(B^{+}_{\delta r_{i}^{-1}}\backslash\{0\})$
		satisfying 
		\begin{align*}
		...
		$C_{\mathrm{loc}}^{1,1}({\mathbb{R}_{+}^{n}}\backslash\{0\})\cap C^{0,1}_{\mathrm{loc}}(\overline{\mathbb{R}_{+}^{n}}\backslash\{0\})$
		satisfying 
		\begin{align}
		...
		To show $a>0$, we just repeat the proof of \emph{Step 1} and find some postive
		constants $N$ and $r_{1}$ independent of $i$, such that for any
		$\delta \in (0,1)$,  with some $K=K(\delta)\in \R_+$ and $i_0=i_0(\delta)\in \N$
		there holds that for all $i\geq i_0$, 
		\[
		v_{i}(x)\geq\frac{N}{r_{x_i'}^{n-2-\delta}(x)}\quad\mathrm{~~in~~}\left\{ Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}\leq r_{x_i'}(x)\leq r_{1},x_{n}\ge0\right\} .
		\]
		Clearly this implies that $v_{\infty}(x)\geq\frac{N}{r_{x_0}^{n-2-\delta}(x)}$
		...
		For $n=4$,  we only have
		$v_{\infty}(x)\geq\frac{N}{r_{x_0}^{n-2-\delta}(x)}$
		in $\left\{ 0<r_{x_0}(x)\leq C\sqrt{\delta}\right\} $ for some positive constant $C$ as described in Lemma \ref{lem:four-dim lower bound-boundary}. 
	\end{proof}
	...
	For future citation, we may apply the same strategy in Theorem \ref{Thm:isolated behavior} to boundary blow-up points happening at a Euclidean domain and eventually make an improvement in dimension four.
	
	\begin{theorem}\label{isolated behavior-1} 
		Suppose $n=3,4$. Assume that two sequences of constants $\ve_i \to 0$ and $D_i \rightarrow D<+\infty$. Let $B_{-D,r_0}^+:=\{x\in \Rn;|x-x_0|<r_0,(x-x_0)_{n}>-D\}$. Suppose a sequence of smooth metrics $g_{i}$ in $\overline {B_{-D_i,r_0}^+}$ converges to $g_{\mathrm{E}}$ in $C_{\mathrm{loc}}^{3}(\overline{B_{-D,r_{0}}^{+}}\backslash\{x_{0}\})$ and $h_{g_i}\leq 0$ on $\{(x-x_0)_n=-D_i\}$. Let $u_{i}$ be a sequence
		...
		\begin{equation*}
			\begin{cases}
				...
				\frac{2}{n-2}\frac{\pa u_{i}}{\pa\vec{n}}=-c_i(x)u_{i}^{\frac{n}{n-2}}+h_{g_{i}}u_{i}, & \qquad\mathrm{~~on~~}(x-x_0)_{n}=-D_{i},
			\end{cases}
		\end{equation*}
		and $x_{i}\rightarrow x_{0}$ be an
		isolated blow-up point, where $f_i,c_i$ are smooth functions and $f_i \to f(x_0)$ and $c_i \to c(x_0)$ in $C^3_{\mathrm{loc}}(B_{-D,r_{0}}^{+})$.
		Then there exist $a\in\R_{+}$ and $v_{\infty}\in C_{\mathrm{loc}}^{0,1}(\overline{B_{-D,r_{0}}^{+}}\backslash\{x_{0}\})\cap C_{\mathrm{loc}}^{1,1}(B_{-D,r_{0}}^{+}\backslash\{x_{0}\})$
		such that for any fixed $y_{0}\in B_{-D,r_{0}}^{+}\backslash\{x_{0}\}$,
		we have $\text{for any \ensuremath{0<\gamma<1}}$,
		\begin{align*}
			u_{i}(x)\rightarrow0 & \quad\mathrm{~~in~~}\quad C_{\mathrm{loc}}^{\gamma}(\overline{B_{-D,r_{0}}^{+}}\backslash\{x_{0}\})\cap C_{\mathrm{loc}}^{1,\gamma}(B_{-D,r_{0}}^{+}\backslash\{x_{0}\}),\\
			\frac{u_{i}(x)}{u_{i}(y_{0})}\rightarrow v_{\infty} & \quad\mathrm{~~in~~}\quad C_{\mathrm{loc}}^{\gamma}(\overline{B_{-D,r_{0}}^{+}}\backslash\{x_{0}\})\cap C_{\mathrm{loc}}^{1,\gamma}(B_{-D,r_{0}}^{+}\backslash\{x_{0}\}),\\
		...
		\begin{equation}
			\begin{cases}
				\sigma_{2}(A_{v_{\infty}^{\frac{4}{n-2}}g_{\mathrm{E}}})=0 & \quad\mathrm{~~in~~}\quad B_{-D,r_{0}}^{+}\backslash\{x_{0}\},\\
				\frac{\partial v_{\infty}}{\pa\vec{n}}=0 & \quad\mathrm{~~on~~}\quad\{x; (x-x_0)_{n}=-D\}\backslash\{x_{0}\}.
	...
	\begin{proof}
		...
		Since the proof is in spirit similar to that of Theorem \ref{Thm:isolated behavior}, we write it as concisely as possible. 
		...
		\emph{Step 1:} There holds $u_{i}\rightarrow0$ in $C^{0}(\overline{B_{-D,r_{0}}^{+}}\backslash\{x_{0}\}).$
		
		Without loss of generality, we assume that $D_{i}=0,D=0$ and $x_{0}=0.$
		...
		\begin{claim}
			For any $0<\delta<(n-2)/2$, there exist positive constants $N$ and $r_{1}$
			independent of $i$, $K=K(\delta)$ and $i_0=i_0(\delta)\in \N$ such that
			for all $i\geq i_0$, we have 
			\[
			u_{i}(x)\geq\frac{N}{|x|^{n-2-\delta}}\quad\mathrm{~~in~~}\left\{ Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}\leq|x|\leq r_{1},x_{n}>0\right\} ,
			\]
			where $r_{1}$ is given in Lemma \ref{lem:four-dim lower bound-boundary}.
		\end{claim}
		...
		There exists a positive constant $N$ independent of $i$ such that
		\[
		u_{i}\geq N \underline{v}_{\delta}\quad\mathrm{~~on~~}\left\{ r(x):= |x|=r_{1},x_n>0\right\},
		\]
		where $\underline v_\delta$ is defined in  Lemma \ref{lem:four-dim lower bound-boundary}.
		
		Fix a $K \in \R_+$, define 
		\[
		\bar{\beta}=\sup\left\{ 0<\beta<N; u_{i}\geq\beta\underline{v}_{\delta}\quad\mathrm{~~in~~}\left\{ r_{i}:=Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}<|x|<r_{1},x_{n}\ge0\right\} \right\} .
		\]
		
		Our purpose is to show that for all sufficiently large $i$, there exists a large $K=K(\delta) \in \R_+$ such that  $\bar{\beta}=N$.
		...
		We argue by contradiction. Suppose $\bar{\beta}<N$. With the help of Lemma \ref{lem:four-dim lower bound-boundary}, we also can establish that the first touch point of $u_{i}$ and $\bar \beta\underline{v}_{\delta}$ only occurs at some $\hat{x}_{i}$ with $\left|\hat{x}_{i}\right|=r_{i}$.
		
		The inner unit  normal with respect to $g_i$ at $\hat{x}_{i}$ is given by
		$$\vec n_{g_i}(\hat x_i)=\frac{g_{i}^{jk}x_{k}\partial_{x_{i}}}{\|x\|^{2}}|_{x=\hat{x}_{i}},$$
		where
		...
		exists a positive constant $C$ such that 
		\begin{equation}
			\partial_{r}\log u_{i}|_{\hat{x}_i}\ge\partial_{r}\log(\bar \beta\underline{v}_{\delta})|_{\hat{x}_i}+C>-\frac{n-2-\delta}{r_{i}}.\label{eq:lower bound-1-limit case}
		\end{equation}
		
		Write $\hat{x}_{i}:=u_{i}\left(x_{i}\right)^{-2/(n-2)}\tilde{y}_{i}$
		with $|\hat x_i|=r_{i}$, which means $|\tilde{y}_{i}|=K$.
		
		Without loss of generality, we just consider the case:$|x_i|u_i(x_i)^{\frac{2}{n-2}}<\infty$. 			Given $R,\varepsilon>0$, there holds
		\[
		\|\frac{1}{u_{i}\left(x_{i}\right)}u_{i}(u_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}y)-\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_0)}\right)^{\frac{n-2}{4}}(1+b|y+T_c\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}\|_{C^{2}(B_{R}^{+})}<\varepsilon,
		\]
		where $b,T_c$ are given in Lemma \ref{lem fudament}.
		
		For any $\ve_0 \in (0,1)$, there exists  a large $K$ such that for all $|y|=K$,
		\[
		\frac{b(|y|^{2}+T_{i}y_{n})}{1+b(|y'|^{2}+(y_{n}+T_{i})^{2})}\ge1-\ve_{0}.
		\]
		...
		and 
		\[
		\varphi(y)=\left(\frac{2b(C_{n}^{2})^{\frac{1}{2}}}{f(x_0)}\right)^{\frac{n-2}{4}}(1+b|y+T_{i}\mathbf{e}_{n}|^{2})^{\frac{2-n}{2}}.
		\]
		...
		\begin{align*}
			...
			= & \frac{n-2}{2}(|y|^{-1}y_{i}\partial_{y_{i}})\big|_{y=\tilde{y}_{i}}\log[1+b_{i}(|y'|^{2}+(y_{n}+T_{i})^{2})]\\
		...
		\]
		and $v_{\infty}\in C_{\mathrm{loc}}^{0,1}(\overline{B_{r_{0}}^{+}}\backslash\{0\})\cap C_{\mathrm{loc}}^{1,1}(B_{r_0}^+\backslash\{0\})$ is the viscosity solution
		to  
		\begin{align*}
			\begin{cases}
				\sigma_{2}(A_{v_{\infty}^{\frac{4}{n-2}}g_{\mathrm{E}}})=0 & \quad\mathrm{~~in~~}B_{r_{0}}^{+}\backslash\{0\},\\
		...
		\end{align*}
		Then it follows from Harnack inequality that for any $x_{0}$, $A:=\lim\sup_{r\rightarrow0}r(x_{0})^{n-2}v_{\infty}(x_{0})$
		is finite.
		...
		\]
		Then $\hat{v}_{i}\in C_{\mathrm{loc}}^{1,1}(B^{+}(r_{0}r_{i}^{-1})\backslash\{0\})$
		satisfying 
		\begin{align*}
			\sigma_{2}(A_{\hat{v}_{i}^{\frac{4}{n-2}}g_{\mathrm{E}}})=0 & \quad\mathrm{~~in~~}\quad B_{r_{0}r_{i}^{-1}}^{+}\backslash\{0\},\\
		...
		\end{align*}
		Thus, up to a subsequence, $\hat{v}_{i}$ converges to $v_{*}$ in
		$C_{\mathrm{loc}}^{1,1}({\mathbb{R}_{+}^{n}}\backslash\{0\})\cap C^{\gamma}_{\text{loc}}(\overline{\mathbb{R}_{+}^{n}}\backslash\{0\})$ for any $0<\gamma<1$
		...
		\end{align}
		By an even reflection, we know from Proposition \ref{even reflection in Appendix} and Theorem \ref{thm:(Li--CPAM09}
		...
		positive constant $N$ and $r_{1}$ independent of $i$ such that, for any 
		$0<\delta<(n-2)/2$, there holds for some large $K=K(\delta)\in \R_+$ and all sufficiently large $i$, 
		\[
		v_{i}(x)\geq\frac{N}{|x|^{n-2-\delta}}\quad\mathrm{~~in~~}\left\{ Ku_{i}\left(x_{i}\right)^{-\frac{2}{n-2}}\leq |x|\leq r_{1},x_{n}\ge0\right\} .
		\]
		Clearly this implies that $v_{\infty}(x)\geq\frac{N}{|x|^{n-2-\delta}}$
		in $\left\{ 0<|x|\leq r_{1}\right\} $.
		
		For $n=3$, again by Li-Nguyen \cite[Theorem 1.3]{Li-Luc1} or Theorem \ref{thm:(Bocher-Type-theorems--Li-Nguyen}  we have
		\[
	...
	%		below with boundary condition. \begin{theorem}\label{isolated behavior}
		...
		%			such that for any fixed $y_{0}\in B_{r_{0}}(x_{0})\backslash\{x_{0}\}$,
		%			we have for any $\ensuremath{0<\gamma<1}$,
		%			\begin{align*}
			...
			%				\frac{u_{i}(x)}{u_{i}(y_{0})}\rightarrow v_{\infty} & \quad\mathrm{~~in~~}\quad C_{\mathrm{loc}}^{1,\gamma}(M\cap B_{r_{0}}(x_{0})\backslash\{x_{0}\})\cap C_{loc}^{0,\gamma}(\bar{M}\cap B_{r_{0}}(x_{0})\backslash\{x_{0}\})\\
	...
	%		\begin{theorem}\label{isolated behavior-1} Let $u_{i}$ be a sequence
		...
		%			\]
		%			with $g_{i}\rightarrow g_{\mathrm{E}}$ in $C_{\mathrm{loc}}^{3}.$ Assume that $\lim_{i\rightarrow\infty}D_{i}:=D$
		%			is bounded. Then there exist $a\in\R_{+}$ and $v_{\infty}\in C_{\mathrm{loc}}^{0,1}(\overline{B_{-D,r_{0}}^{+}(x_{0})}\backslash\{x_{0}\})\cap C_{\mathrm{loc}}^{1,1}(B_{-D,r_{0}}^{+}(x_{0})\backslash\{x_{0}\})$
	...
	Suppose that $\{u_{i}\}$ is a sequence of positive solutions to \eqref{PDE:sigma_2_new}.
	
	
	First we would like to prove that interior isolated blow-up points
	are simple. \begin{lemma} \label{lem:interior blow up is simple}
		Let $x_{i}\rightarrow x_{0}\in M$ be an isolated blow-up point for
		$u_{i}$, then $x_{0}$ is an isolated simple blow-up point . \end{lemma} 
	\begin{proof}
		%			From Lemma \ref{lem fudament}, we know that $r^{(n-2)/2}\overline{u}_{i}(r)$
		%			has only one critical point in $(0,r_{i})$, where $r_{i}=R_{i}u_{i}(y_{i})^{-2/(n-2)}$
		%			with $R_{i}\to\infty$ as $i\to\infty$.
		%			
		%			By Lemma \ref{lem fudament}, 
		%			\[
		%			\eta_{i}(y):=2^{\frac{n-2}{2}}u_{i}^{-1}\left(q_{i}\right)u_{i}\left(\exp_{q_{i}}(2u_{i}^{-\frac{2}{n-2}}\left(q_{i}\right)y)\right)\rightarrow\left(\frac{1}{1+|y|^{2}}\right)^{(n-2)/2}:=\eta(|y|)
		%			\]
		%			in $C_{\mathrm{loc}}^{2}(\mathbb{R}^{n})$. Since it is not hard to
		%			see that $(s^{\frac{n-2}{2}}\eta(s))'\le-Cs^{-\frac{n}{2}}$ for $2\le s\le3R_{i}$,
		Let 
		$$\eta_i(z)=u_{i}^{-1}\left(x_{i}\right)u_{i}\circ \exp_{x_{i}}(u_{i}^{-\frac{2}{n-2}}\left(x_{i}\right)z)$$
		and 
		$$\eta(z)=\left(1+\frac{f(x_0)}{2(C_{n}^{2})^{\frac{1}{2}}}|z|^{2}\right)^{\frac{2-n}{2}}.$$
		...
		\]
		where $s_0=\sqrt{2(C_n^2)^{1/2}/f(x_0)}$.
		This means 
		\[
		...
		\begin{align*}
			...
			= & \omega_{i}^{\frac{4}{n-2}}(z)\tilde{u}_{i}^{\frac{4}{n-2}}(y_{0})f\circ\mathcal{T}_{i}(z)+\mu_{i}^{2}f_{0}\qquad\mathrm{~~in~~}|z|<\delta/\mu_{i}.
		\end{align*}
	...
	\begin{lemma} \label{lem:boundary bounded limit} Let  $x_{i}\rightarrow x_{0} \in \pa M$
		be an isolated blow-up point for $u_{i}$ with $\sigma_{2}^{1/2}(g^{-1}A_{u_{i}})=u_{i}^{\frac{4}{n-2}}f+f_{0}$, then 
		\[
		d_{g}(x_{i},\partial M)u_{i}(x_{i})^{\frac{2}{n-2}}\mathrm{~~is~~bounded.}
		\]
	\end{lemma} 
	\begin{proof}
		...
		By contradiction, suppose $T_{i}:=d_i^{(n-2)/2}u_{i}(x_{i})\rightarrow\infty$. We define 
		\[
		...
		and 
		\[
		\frac{2}{n-2}\frac{\partial v_{i}}{\partial \vec n_{g_{i}}}=-c\circ\Psi_{i}v_{i}^{\frac{n}{n-2}}+h_{g_{i}}\circ\Psi_{i}v_{i}\quad\mathrm{~~on~~}\quad\{z_{n}=-1\}.
		\]
		...
		and $R_{i} \to \infty$ is the given sequence of real numbers.
		
		
		For any compact set $K\subset{\mathbb{R}_{-1}^{n}}\backslash\{0\}$, for large $i$,
		by the classical interior estimate (see \cite{Guan-Wang0,Wang,Chen2,LiYY}),
		we have 
		\[
		\|\nabla\log v_{i}\|_{C^{0}(K)}+\|\nabla^{2}\log v_{i}\|_{C^{0}(K)}\le C(K).
		\]
		Actually we have $v_{i}(z)\rightarrow0$ in $C_{\mathrm{loc}}^{1,1}({\mathbb{R}_{-1}^{n}}\backslash\{0\})\cap C_{\mathrm{loc}}^{0,1}(\overline{\mathbb{R}_{-1}^{n}}\backslash\{0\})$.
		
		By Theorem \ref{isolated behavior-1}, $v_{i}\rightarrow0$ in $C_{\mathrm{loc}}^{0}(\overline{\mathbb{R}_{-1}^{n}}\backslash\{0\})$. We define $\omega_{i}(z)=v_{i}(z)/v_{i}(y_{0})$  for any fixed $y_0\neq 0$ with $\omega_{i}(y_{0})=1$.
		Then $\omega_{i}$ satisfies 
		\begin{align*}
			& \sigma_{2}^{1/2}(-\frac{2}{n-2}\frac{\nabla_{g_{i}}^{2}\omega_{i}}{\omega_{i}}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{i}\otimes_{g_{i}}\ud\omega_{i}}{\omega_{i}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{i}|_{g_{i}}^{2}}{\omega_{i}^{2}}g_{i}+A_{g_{i}})\\
		...
		\end{align*}
		and 
		$$\frac{2}{n-2}\frac{\partial\omega_{i}}{\partial \vec n_{g_i}}=-c(x_{0})\omega_{i}^{\frac{n}{n-2}}v_{i}(y_{0})^{\frac{2}{n-2}}+h_{g_i}\omega_i
		\qquad \mathrm{~~on~~} \quad z_{n}=-1.$$
		
		By the local gradient estimate for $v_i$, we actually know $\omega_{i}(z)\le C_{K}$
		...
		satisfying the following equation in viscosity
		sense 
		\[
		\sigma_{2}(-\frac{2}{n-2}\frac{\nabla_{g_{\mathrm{E}}}^{2}\omega_{\infty}}{\omega_{\infty}}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{\infty}\otimes_{g_{\mathrm{E}}}\ud\omega_{\infty}}{\omega_{\infty}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{\infty}|_{g_{\mathrm{E}}}^{2}}{\omega_{\infty}^{2}}g_{\mathrm{E}})=0\quad\mathrm{in~~}\mathbb{R}_{-1}^{n}\backslash\{0\}
		\]
		and $\frac{\partial w_{\infty}}{\partial z_{n}}=0$ on $z_{n}=-1$. 
	...
	\begin{remark} As a direct consequence of Lemma \ref{lem:boundary bounded limit}, we know that if
	...
	Now let us see that every boundary isolated blow-up point of $\{u_{i}\}$
	is simple.
	\begin{lemma} \label{lem:boundary blow up is simple-1}
		Let $x_{i}\rightarrow x_{0}\in\partial M$ be an isolated blow-up
		point for $u_{i}$, then $x_{0}$ is an isolated simple blow-up point
		. \end{lemma} 
	\begin{proof}
		%			From Lemma \ref{lem fudament}, we know that $r^{\frac{n-2}{2}}\overline{u}_{i}(r)$
		%			has only one critical point in $(0,r_{i})$,where $r_{i}=R_{i}u_{i}(y_{i})^{-2/(n-2)}$
		%			with $R_{i}\to\infty$ as $i\to\infty$.
		%			
		%			By Lemma \ref{lem fudament}, 
		%			\[
		%			\eta_{i}(y):=2^{\frac{n-2}{2}}u_{i}^{-1}\left(q_{i}\right)u_{i}\left(G_{q_{i}}(u_{i}^{-\frac{2}{n-2}}\left(q_{i}\right)2y)\right)\rightarrow\left(\frac{1}{1+|y|^{2}}\right)^{(n-2)/2}:=\eta(|y|)
		%			\]
		%			in $C_{\mathrm{loc}}^{2}(\mathbb{R}_{+}^{n})$.
		From (\ref{eq:first derivative negative upper bound}) in the proof of Lemma \ref{one critical point},   we have 
		\[
		...
		be the second critical point of $r^{(n-2)/2}\overline{u}_{i}(r)$,
		we know that $\mu_{i}>r_{i}$ and $\lim_{i\rightarrow\infty}\mu_{i}=0.$
		%			Let $G_{i}=\exp_{q_{i}}\{z_{n}\vec{n}(q_{i})\}:B_{\rho}^{+}\to\overline{M}$
		%			be Fermi coordinates around $q_{i}$ and
		Define 
		\[
		\Psi_{i}(z)=G_{x_i'}(\mu_{i}z+d_i \mathbf{e}_n)\quad\mathrm{~~and~~}\quad g_{i}=\mu_{i}^{-2}\Psi_{i}^{\ast}(g)\rightarrow g_E.
		\]
		We define 
		\[
		...
		\begin{cases}
			...
			\frac{2}{n-2}\frac{\partial\tilde{u}_{i}}{\partial \vec n_{g_{i}}}=-c\circ\Psi_{i}\tilde{u}_{i}^{\frac{n}{n-2}}+h_{g_{i}}\circ\Psi_{i}\tilde{u}_{i} &\mathrm{~~on~~}B_{\rho/\mu_{i}}\cap \{z_n= -\frac{d_i}{\mu_i}\},\\
		...
		the boundary local estimate yields 
		\begin{align}\label{gradient estimate 5.22}
			\|\nabla\log\tilde{u}_{i}\|_{C^{0}(K)}\le C(K) 
		\end{align}
		Actually we have $\tilde{u}_{i}(z)\rightarrow0$ in $C_{\mathrm{loc}}^{\gamma}(\overline{\mathbb{R}_{+}^{n}}\backslash\{0\})\cap C_{\mathrm{loc}}^{1,\gamma}({\mathbb{R}_{+}^{n}}\backslash\{0\})$
		for any $0<\gamma<1$.
		...
		\end{align*}
		and 
		$$\frac{2}{n-2}\frac{\partial\omega_{i}}{\partial z_{n}}=-c\circ \Psi_i\omega_{i}^{\frac{n}{n-2}}\tilde{u}_{i}(y_{0})^{\frac{2}{n-2}}+h_{g_i}\omega_i \qquad \mathrm{~~on~~} D_{\rho/\mu_{i}}.$$
		...
		exists a subsequence $\omega_{i}$ converging to $\omega_{\infty}\in C_{\mathrm{loc}}^{0,1}(\overline{\mathbb{R}_{+}^{n}}\backslash\{0\})\cap C_{\mathrm{loc}}^{1,1}({\mathbb{R}_{+}^{n}}\backslash\{0\})$ satisfying the following equation
		\[\sigma_{2}^{1/2}(-\frac{2}{n-2}\frac{\nabla_{g_{\mathrm{E}}}^{2}\omega_{\infty}}{\omega_{\infty}}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{\infty}\otimes\ud\omega_{\infty}}{\omega_{\infty}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{\infty}|_{g_{\mathrm{E}}}^{2}}{\omega_{\infty}^{2}}g_{\mathrm{E}})=0\quad\mathrm{in~~}\overline{\mathbb{R}_{+}^{n}}\backslash\{0\}
		\]
		in viscosity
		...
		Arguing as before, we know that $\omega_{\infty}$  should  only be a
		radial solution with singularity at the origin.   Again thanks to Theorem \ref{isolated behavior-1}, we actually
		have $\lim_{z\rightarrow0}\omega_{\infty}(z)|z|^{n-2}=a$ for some $a\in \R_+$. Hence, via an even reflection, we can apply Theorem
		\ref{thm:(Bocher-Type-theorems--Li-Nguyen} to conclude that when $n=3,4$, $\omega_{\infty}(z)=a|z|^{2-n}$, which contradicts the fact that $(r^{(n-2)/2}\overline{\omega}_{\infty}(r))'|_{r=1}=0.$ 
	\end{proof}
	...
	\begin{lemma}\label{lem:pairwize_disjoint_sigular_pts}
		Let $(M,g)$ be a smooth compact Riemannian manifold of dimension
		$n=3,4$ with boundary $\pa M$. For any $0\le c \in C^\infty(\pa M)$, any given large $R\geq1$ and $0<\ve<1$,
		there exist positive constants $\delta_{0}=\delta_{0}(M,g,c,R,\ve)$
		and $C_{0}=C_{0}(M,g,c,R,\ve)$ such that for all positive solutions $u$ to
	...
	\begin{proof}
		...
		We only focus on $q_{0}\in\partial M$, since the other case
		is relatively simpler and handled by the same idea.
		...
		We can derive from (\ref{eq:lower bound}) that 
		\begin{equation}
		...
		\begin{align*}
			\begin{cases}
				...
				\frac{2}{n-2}\frac{\partial w_{i}}{\partial\vec{n}_{g_{i}}}(y)=-c\circ\Psi_{i}w_{i}^{\frac{n}{n-2}}+h_{g_{i}}\circ\Psi_{i}w_{i} & \quad\mathrm{~~on~~}y_{n}=-T_{i}.
			\end{cases}
		\end{align*}
		
		Up to a subsequence, we denote $T=\lim_{i\rightarrow\infty}T_{i}\in[0,\infty]$. Though $T$ might be $T=\infty$ or $T\in\R_{+}$ or $T=0$, we manage to deal with them in a unified way.
		...
		\end{cases}
		\]
		However, by  Theorems \ref{half liouville theorem} and \ref{Thm:Acta}, the solution can not have two local maximum
		...
		\|\nabla\log w_{i}\|_{C^{0}(K)}+\|\nabla^{2}\log w_{i}\|_{C^{0}(K)}\le C(K).
		\]
		By boundary estimates, we have $w_{i}(z)\rightarrow0$ in $C_{\mathrm{loc}}^{\gamma}(\overline{\mathbb{R}_{-T}^{n}}\backslash\mathcal{S})\cap C_{\mathrm{loc}}^{1,\gamma}({\mathbb{R}_{-T}^{n}}\backslash\mathcal{S})$
		for all $\gamma\in(0,1)$.
		...
		\end{align*}
		together with 
		$$\frac{2}{n-2}\frac{\partial\omega_{i}}{\partial z_{n}}=-c\circ\Psi_{i}\omega_{i}^{\frac{n}{n-2}}w_{i}(y_{0})^{\frac{2}{n-2}}+h_{g_i}\omega_i.$$
		
		By gradient estimate and $\omega_{i}(y_{0})=1$, we actually know $\omega_{i}(z)\le C_{K}$
		on every compact set $K\subset\overline{\mathbb{R}_{-T}^{n}}\backslash\mathcal{S}$.
		This together with gradient estimate implies $\omega_{i}\geq c_{K}$ in $K$ and $\|\log\omega_{i}\|_{C^{1}(K)}\le C$. Now there exists a subsequence
		$\omega_{i}$ converging to $\omega_{\infty}\in C_{\mathrm{loc}}^{1,1}(\mathbb{R}_{-T}^{n}\backslash\mathcal{S})\cap C_{\mathrm{loc}}^{0,1}(\overline{\mathbb{R}_{-T}^{n}}\backslash\mathcal{S})$
		...
		\begin{align*}
			...
			\quad\mathrm{~~in~~}\mathbb{R}_{-T}^{n}\backslash\mathcal{S}
		\end{align*}
		and $\frac{\partial\omega_{\infty}}{\partial z_{n}}=0$ on $\{z;z_{n}=-T\}\backslash\mathcal{S}.$
		...
		%								Then $\omega_{i}$ satisfies 
		%								\[
		%								\sigma_{2}^{1/2}(-\frac{2}{n-2}\frac{\nabla_{g_{i}}^{2}\omega_{i}}{\omega_{i}}+\frac{2n}{(n-2)^{2}}\frac{\ud\omega_{i}\otimes\ud\omega_{i}}{\omega_{i}^{2}}-\frac{2}{(n-2)^{2}}\frac{|\nabla\omega_{i}|_{g_{i}}^{2}}{\omega_{i}^{2}}g_{i}+A_{g_{i}})=\omega_{i}^{\frac{4}{n-2}}(z)w_{i}^{\frac{4}{n-2}}(y_{0}).
		%								\]
		%								On boundary, $\frac{\partial\omega_{i}}{\partial z_{n}}=-c(q_{0})\omega_{i}^{\frac{n}{n-2}}w_{i}(y_{0})^{\frac{2}{n-2}}.$
		%								
		%								By Lemma \ref{lem:harnack inequality}, we actually know $\omega_{i}(z)\le C_{K}$
		%								on every compact set $K\subset\overline{\mathbb{R}_{-T}^{n}}\backslash\mathcal{S}$.
		%								Thus also by the gradient estimate, $\omega_{i}\geq c_{K}$ in $K$
		...
		%								and 
		%								\[
		%								\bar{\omega}_{\infty,1}(r)=\frac{1}{|\partial B_{r}(\lim_{i\rightarrow\infty}A_{i,2})|}\int_{\pa^{+}B^+_{r,\theta}(\lim_{i\rightarrow\infty}A_{i,2})}\omega_{\infty}d\sigma.
		%								\]
		%								Also $\big(s^{\frac{n-2}{2}}\overline{\omega}_{\infty}(s)\big)'\le0$ and
		%								$\big(s^{\frac{n-2}{2}}\bar{\omega}_{\infty,1}(s)\big)'\le0$ near
		%								zero, and then $\omega_{\infty}$ is singular at zero and $\lim_{i\rightarrow\infty}A_{i,2}$.
		%								And by Theorem \ref{isolated behavior-1}, we actually have $\lim_{z\rightarrow0}w_{\infty}|z|^{n-2}=a_{1},$
		%								and $\lim_{z\rightarrow\lim_{i\rightarrow\infty}A_{i,2}}w_{\infty}|z-\lim_{i\rightarrow\infty}A_{i,2}|^{n-2}=a_{2}$
		%								for some positive constant $a_{1},a_{2}>0$. By Theorem \ref{thm:(Bocher-Type-theorems--Li-Nguyen}
		%								and Proposition \ref{Li-cpam2009PROPOSITION 5.1}, for $n=3,4$, $\omega_{\infty}$
		%								can not have two singular points. 
	\end{proof}
	...
	%	%\noindent\begin{minipage}[t]{1\columnwidth}
		...
		\[
		A_{n,\bar{H}}(\bar \delta)=\begin{cases}
			(-\bar{H}\bar \delta+n-1)^{n-1}/(n-1)^{n-1} & \mathrm{~~if~~}-\bar{H}\bar \delta+n-1\geq0,\\
			0 & \mathrm{~~otherwise.}
		\end{cases}
		\]
		
		Fix a nonnegative constant $\bar\delta$, let $M^{\bar \delta}:=\{x\in M;d_{g}(x,\pa M)>\bar \delta\}$
		and $\partial M^{\bar \delta}$ be the boundary (as a metric subspace of
		$M$) of $M^{\bar \delta}$.
		...
		\begin{theorem}[\protect{Perales,  \cite[Theorem 1.3]{Perales}}]\label{Perales}
			...
			Then, for $\mathcal{L}^{1}$-almost everywhere $\bar \delta>0$, 
			\[
			\mathrm{Vol}\left(\partial M^{\bar \delta},g\right)\leq\mathrm{Vol}(\partial M,g)A_{n,\bar{H}}(\bar \delta),
			\]
			where $\mathcal{L}^{1}$ denotes the 1-dimensional Lebesgue measure.
		\end{theorem}			
		
		Suppose $\{u_i\}$ is a sequence of positive solutions to \eqref{PDE:sigma_2_new} with $\max_{\overline M} u_i \to +\infty$ for dimensions $n=3,4$. It follows from Lemma \ref{lem:pairwize_disjoint_sigular_pts} that the singular set $\mathcal{S}=\mathcal{S}[\{u_i\}]$ is a finite set of isolated blow-up points, denoted by $\mathcal{S}=\{x_1,\cdots,x_N\} \subset \overline M$. 
		...
		As we shall see in Section \ref{Sect:degree_theory}, if the boundary is totally non-umbilic,   then $\mathcal{S}$ consists of only interior isolated blow-up points.
		
		\begin{proof}
			The same proof of Theorem \ref{Thm:isolated behavior} can be used to show that for any `\emph{regular}' point $y_{0}\in \overline M \backslash \mathcal{S}$ and $\gamma \in (0,1)$,
			\begin{align*}
			...
			and $\frac{\partial v_{\infty}}{\pa\vec{n}}=h_{g}v_{\infty}$
			on $\partial M\backslash \mathcal{S}$.
			
			%The combination of Lemma \ref{lem:boundary bounded limit}
			%and Theorem \ref{Thm:totally_nonumbilic} demonstrates that the singular set $\mathcal{S}$ %consists of only interior isolated blow-up points. 
			...
			Let $\{y\}$ be the geodesic normal coordinates around $x_k$, then $r_{x_k}(y)=|y|$ near $x_k$. Denote by $\{z=y/|y|^2\}$ the inverted coordinates in a component $N_k:=U_{x_k} \setminus\{x_k\}$. We will call each $N_k$ an \emph{end}.  Notice that
			...
			$$M_1 \approx \sqcup_{k=1}^N (\Rn \backslash B_{R_0}(0)).$$
			...
			\end{equation}
			is nonincreasing with respect to $r$, where $B_r^{g_{v_\infty}}(y_0)$ denotes a geodesic ball in the metric $g_{v_\infty}$ and $|B_1(0)|$ stands for the volume of the unit Euclidean ball. In the third step, using the technique of tangent cone analysis developed  in \cite{Gursky-Viaclovsky}, we can prove the volume growth on each end: For each end $N_k$ and the above base point $y_0 \in M_0$, there holds
			...
			\[
			M_{g_{i}}^{\bar \delta}:=\{x\in M;d_{g_{i}}(x,\pa M)>\bar \delta\}
			\]
			for clarity. Since $A_{u_{i}}\in\Gamma_{2}^{+}$, by Guan-Viaclovsky-Wang
			...
			By definition we have $A_{n,0}(\bar \delta)=1$ with $\bar H=0$. Then, on each fixed $(M,g_{i})$, we may apply Theorem \ref{Perales} to obtain
			\[
			\mathrm{Vol}\left(\partial M_{g_{i}}^{\bar \delta},g_{i}\right)\leq\mathrm{Vol}(\partial M,g_{i}).
			\]
			...
			such that for $\mathcal{L}^{1}$-almost everywhere $\bar \delta>0$, 
			\begin{equation}
				\mathrm{Vol}(\partial M,g_{i})\leq C_{\ast}.\label{est:area_bdd}
			\end{equation}
			
			On the other hand, we always assume $\partial M_{g_{i}}^{\bar \delta}\neq\emptyset$.
			For any $x\in\pa M_{g_{i}}^{\bar \delta}$, let $\gamma(t)$ be a unit-speed
			...
			\]
			With the choice of $\bar \delta>2C_{r_{0}}^{2/(n-2)}\mathrm{Diam}(M)$, we
			claim that $x\in B_{r_{0}}(x_{0})\backslash\{x_{0}\}$. 
			
			Otherwise, if $x\notin B_{r_{0}}(x_{0})$, then we have 
			\begin{align*}
				\bar \delta=d_{g_{i}}(x,\pa M)\leq d_{g_{i}}(x,x')\leq & \int_{0}^{d_{g}(x,\pa M)}|\gamma'(t)|_{g_{i}}\ud t\\
				\leq & C_{r_{0}}^{\frac{2}{n-2}}\mathrm{Diam}(M),
			\end{align*}
			which contradicts the choice of $\bar \delta$.
			...
			\begin{align*}
				\bar \delta\leq & \int_{0}^{d_{g}(x,\pa M)}|\gamma'(t)|_{g_{i}}\ud t\\
				...
				\leq & a^{\frac{2}{n-2}}d_{g}(x,x_{0})^{-2}d_{g}(x_{0},\pa M)+\frac{\bar \delta}{2}.
			\end{align*}
			This indicates that 
			\begin{equation}
				d_{g}(x,x_{0})\leq a^{\frac{1}{n-2}}\sqrt{\frac{2d_{g}(x_{0},\pa M)}{\bar \delta}}.\label{est:r(x)}
			\end{equation}
			
			It is not hard to see that $M_{g_{i}}^{\bar \delta}$ is star-shaped with
			...
			If choosing anther larger constant $\bar \delta$, by \eqref{est:r(x)}
			we have 
			\begin{align*}
				\mathrm{Vol}(\partial M_{g_{i}}^{\bar \delta},g_{i})= & \int_{\partial M_{g_{i}}^{\bar \delta}}v_{i}^{\frac{2(n-1)}{n-2}}\ud\sigma_{g}\\
				\overset{i\to\infty}{\longrightarrow} & a^{\frac{2(n-1)}{n-2}}\int_{\Sp^{n-1}}d_{g}(x,x_{0})^{-2(n-1)}\sqrt{\det h(r,\theta)}\ud\theta_{1}\wedge\cdots\wedge\ud\theta_{n-1}\\
				\geq & C_{0}\bar \delta^{\frac{n-1}{2}}
			\end{align*}
			for some positive constant $C_{0}$ independent of $i$ and $\bar \delta$.
			This yields a contradiction with \eqref{est:area_bdd} provided that
			$\bar \delta$ is large enough.
		\end{proof}
		...
		We adopt the method of Leray-Schauder degree theory to solve the boundary $\sigma_2$-curvature equation \eqref{PDE:prescribing_sigma_2} and complete the proof of Theorem \ref{Thm:existence} as well.
		
		
		We first wander a litter into the variational aspect of boundary $\sigma_2$-curvature equation. A variational characterization of the $\sigma_2$-curvature equation \eqref{PDE:prescribing_sigma_2} with $c=0$ on a compact manifold with umbilic boundary was described in \cite{Chen}, where the following functional
	was introduced:
	...
	\end{equation}
	see \cite[formula $(2)$ on p.1030]{Chen}  for the definition of $\mathcal{B}_{\tilde g}^2$.\footnote{For $3 \leq k\leq n/2$, $\mathcal{B}_g^k$ is only known in locally conformally flat manifolds with boundary; see \cite{Chen} as well.}  The above $\mathcal{F}_2$ appears in the  Gauss-Bonnet formula \cite{Branson-Gilkey} on a four-dimensional compact manifold with boundary:
	...
	$\mathcal{F}_2[\tilde g]$ under constraint that $\int_M  f \ud \mu_{\tilde g}=1$ for all $\tilde g \in [g]$. 
		...
		\begin{proof}[Proof of Theorem \ref{Thm:existence}]					
			...
			and $\zeta(t)=1$ for $1/2\le t\le1$. It is noticed that the above equation at $t=1$ is exactly \eqref{PDE:prescribing_sigma_2}.
			
			Under assumption that $\lambda(A_g) \in \Gamma_2^+$ and $h_g\ge 0$,  thanks to Proposition \ref{prop:sigma_2_1st_bdry_eigenvalue} it suffices to consider a  background metric (conformal to the original one) with vanishing boundary mean curvature. The resulting equation  \eqref{eq:path} at $t=1$ is exactly \eqref{PDE:prescribing_sigma_2}. As discussed above, we assume the background metric $g$ satisfies $\lambda(A_g)\in \Gamma_2^+$ and $h_g=0$ on $\pa M$.
			...
			\begin{proof}
				...
				$$\frac{\partial\tilde{u}_{i}}{\partial \vec n_{g_i}}=\zeta(t_i)(c\circ \Psi_ie^{-\tilde{u}_{i}}-h_{g_i}\circ \Psi_i) \qquad \mathrm{on~~} \quad D_{\rho/\ve_i}.$$
				...
				$$\frac{\partial\tilde{u}_{i}(x)}{\partial \vec n_{g_{i}}}=\zeta(t_i)(c(\Psi_i(x))e^{-\tilde{u}_{i}}-h_{g_i}(\Psi_i(x))) \quad \mathrm{~~on~~} D_{\frac{\rho}{\ve_{i}}}.$$
...
%			\]
		 and 	by assumption that $\sigma_2(A_{v_{i}^{4/(n-2)}g_{i}})=f_i\rightarrow0$,
		 taking
		 $i\rightarrow\infty$,
			\[
...
The proof is divided into three steps. 
			...
			\iffalse which means that
			let $\varphi\in C^{2}(\overline{B_{1}^{+}})$, $u_{\varepsilon}^{+}-\varphi(x_{0})=0$
			and $u_{\varepsilon}^{+}\ge\varphi$ in $\overline{B_{1}^{+}}$. If $x_{0}\in\partial'B_{1}^{+}$,
			then $\max\{F(A_{\varphi})(x_{0}),-\frac{\partial\varphi}{\partial x_{n}}(x_{0})\}>0.$
			Here $\varphi^{+}$ is the function constructed in Lemma 5.3 in Li. 
			\fi	
