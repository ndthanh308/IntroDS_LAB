...
\begin{equation}
f_{k}(m) = \begin{cases}
(m \times i - j)\mod D, \hspace{0.86cm} \text{if} \ m< D, \\\hspace{1.5cm}i \hspace{2.4cm} \text{if} \  m=D,
\end{cases}
\end{equation}
...
\section*{Acknowledgments}The authors would like to thank Alicia Sit for her valuable discussions, as well as Manuel Ferrer for his help in the current version of the analysis code. This work was supported by the Canada Research Chairs (CRC), the High Throughput and Secure Networks (HTSN) Challenge Program at the National Research Council of Canada, and the Joint Centre for Extreme Photonics (JCEP). 
