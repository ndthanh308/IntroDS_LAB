...
\NewDocumentCommand{\Fib}{t' t" O{n} O{n} o}{
	% cart clov discr(d)-op(o)-discop(b)-twoset(t)-clov(c)-split(s) cat base
	\ensuremath{\ifx#3t{2\mbox{-}\Set\mbox{-}}\fi\catfont{\ifx#3d{D}\else{\ifx#3o{Op}\else{\ifx#3b{DOp}\else{\ifx#3t{Op}\else{\ifx#3c{Cl\h[-3]}\else{\ifx#3s{Sp}\fi}\fi}\fi}\fi}\fi}\fi{Fib}}\IfBooleanTF{#1}{_{\h[0.4]\opn{cart}\ifx#4n{}\else{\h[-1.4],\h[0.4]{#4}}\fi}}{\IfBooleanTF{#2}{_{\h[0.4]\opn{clov}\ifx#4n{}\else{\h[-1.4],\h[0.4]{#4}}\fi}}{\ifx#4n{}\else{_{\h[0.4]{#4}}}\fi}}\IfNoValueF{#5}{\h[-1]\left({#5}\right)}}
}
...
The Grothendieck construction, or category of elements construction, is a fundamental tool in category theory that regulates the equivalence between indexed categories and Grothendieck fibrations. It allows to reorganize the data of an indexed family of categories in a total category equipped with a functor that tells which index each object came from. And it abstractly captures the concept of change of base as well.

The Grothendieck construction was born in geometry (Grothendieck's~\cite{grothendieck_revetsetalegroupefond}) to better handle categories naturally associated to every point of a space, bringing to the concept of bundle. But it then had tons of applications in logic and algebra as well. In algebra, it brought for example to the total category of all modules, collecting together $R$-modules for every ring $R$. While logicians have particularly used the restriction of the Grothendieck construction to the equivalence between families of sets indexed over a category and discrete fibrations. This allowed to consider such families of sets without mentioning morphisms that land into the universe of all sets. We believe that the general setting of the Grothendieck construction could find as well new applications in logic, when one moves to dimension $2$. This could actually pave the way towards $2$-dimensional logic. For this aim, it would be probably best to consider the $2$-$\Set$-enriched Grothendieck construction (which is a natural extension of the usual one), studied more in detail in the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits}.

The equivalence between indexed categories and Grothendieck fibrations gives the possibility to exploit the advantages of both worlds. But also the process itself, which is the Grothendieck construction, has important and useful consequences. Notably, it allows to conicalize all weighted $\Set$-enriched (i.e.\ ordinary) limits, and to almost conicalize weighted $2$-limits (see Street's~\cite{street_limitsindexedbycatvalued} and the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits}). This also presents every presheaf as a colimit of representables and gives the famous explicit formula for the ordinary Kan extension.

In this paper, we produce an indexed version of the Grothendieck construction, that does not seem to appear in the literature. Our main results are condensed in the following theorem. Op and non-split variations are also considered in \remx\ref{remopvariations} and \remx\ref{remtwocatvariations}.
...
between slices of (co)presheaves and (co)presheaves on slices. Our theorem also guarantees its pseudonaturality in $A$, which does not seem to be stated in the literature. We will show in \exax\ref{exhofstreichuniv} that this can be applied to get a nice candidate for a Hofmann-Streicher universe (see~\cite{hofmannstreicher_liftinggrothuniverses}) in 2-presheaves. The second author will then show in a forthcoming paper that such candidate is indeed a 2-dimensional classifier in $\m{\A\op}{\CAT}$, towards a $2$-dimensional elementary topos structure on $\m{\A\op}{\CAT}$. This was actually the starting motivation for the second author to produce the indexed Grothendieck construction.
...
$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
gives a $2$-dimensional generalization of this. It is indeed understood that the correct analogue of slices in dimension $2$ is an (op)fibrational slice, given by (op)fibrations over an element rather than all maps into that element. And our equivalence can be thought as saying that every (op)fibrational slice of a Grothendieck $2$-topos is again a Grothendieck $2$-topos.
...
We will also give an explicit description of the indexed Grothendieck construction, so that it can be applied more easily.
...
In Section~\ref{sectionopfibintwopresh}, after recalling the notion of opfibration in a $2$-category, we show an equivalent characterization of the opfibrations in $\m{\A}{\CAT}$. We produce a pseudofunctor $F\mto \Fib[o][\m{\A}{\CAT}][F]$.\v[0.2]

In Section~\ref{sectionindexedgrothconstr}, we present our main theorems, proving an equivalence of categories between opfibrations in $\m{\A}{\CAT}$ over $F$ and $2$-copresheaves on $\Groth{F}$. We also show that such equivalence is pseudonatural in $F$. We present the explicit indexed Grothendieck construction.
...
We will denote as $\CAT$ the $2$-category of small categories, functors and natural transformations, and as $\Cat$ its underlying category. 

It is known that the Grothendieck construction $\Groth{F}$ of a 2-functor $F\:\C\to \CAT$ with $\C$ a category is equivalently the oplax colimit of $F$. As we could not find a proof of this in the literature, we show a proof below (\thex\ref{teorgrothisoplaxcolim}). Another useful characterization is that $\Groth{F}$ is equivalently the lax comma object from $\1\:\1\to \CAT$ to $F$ in $\twoCATlax$. This is explored in detail in the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits} and recalled below (\thex\ref{teorgrothconstrislaxcomma}).

\begin{rem}
	In this paper, we focus on the Grothendieck construction of (strict) $2$-functors $F\:\C\to \CAT$ with $\C$ a $1$-category, which correspond with split opfibrations over $\C$. We will consider variations of this setting in \remx\ref{remopvariations} (fibrations) and \remx\ref{remtwocatvariations} ($\C$ a $2$-category and non-split opfibrations).
\end{rem}

\begin{rec}\label{recgrothopfib}
	A functor $p\:\E\to \C$ is called a \dfn{\pteor{Grothendieck} opfibration} (in $\CAT$) (over $\C$) if for every object $E\in \E$ and every morphism $f\:p(E)\to C$ in $\C$, there exists a cartesian lifting $\ov{f}^E\:E\to f\stb E$ of $f$ to $E$
	\fibsq{E}{f\stb E}{p(E)}{C}{\ov{f}^E}{p}{f}
	\dfn{Cartesian} means that for every $E'\in \E$, every morphism $e\:E\to E'$ in $\E$ and every morphism $w\:C\to p(E')$ in $\C$, there exists a unique morphism $v\:f\stb E\to E'$ such that $p(v)=w$ and $v\c \ov{f}^E = e$.
...
\begin{rec}\label{recgrothconstr}
	...
	The \dfn{Grothendieck construction} of $F$ is the functor $\groth{F}\:\Groth{F}\to \C$ of projection on the first component from the category $\Groth{F}$, which is defined as follows:
		\begin{description}
			...
			\item[a morphism $(C,X)\to (D,X')$ in $\Grothop{F}$] is a pair $(f,\alpha)$ with $f\:C\to D$ a morphism in $\C$ and $\alpha\:F(f)(X)\to X'$ a morphism in $F(D)$.
		\end{description}
...
\begin{teor}\label{usualgrothconstr}
...
\begin{teor}\label{teorgrothisoplaxcolim}
	Let $\C$ be a category and let $F\:\C\to \CAT$ be a $2$-functor. The Grothendieck construction $\Groth{F}$ of $F$ is equivalently the oplax (conical) colimit of the $2$-diagram $F$:
...
\begin{teor}\label{teorgrothconstrislaxcomma}
	...
	\begin{eqD*}
		\sq*[l][6][6][\h[-1.9]\lax \opn{comma}][2.7][2.2][0.55]{\Grothop{F}}{\1}{\C}{\CAT}{}{\groth{F}}{\1}{F}
	\end{eqD*}
	in $\twoCATlax$ \pteor{the $2$-category of $2$-categories, $2$-functors, lax natural transformations and modifications}.
	...
	\begin{eqD*}
		\begin{cd}*[5.85][5]
			\Grothop{F}\PB{rd} \arrow[d,"\groth{F}"'] \arrow[r] \& \CAT\bl \arrow[d,"{\tau}"'] \arrow[r]\& \1 \arrow[d,"{\1}"]\arrow[ld,Rightarrow,shorten <=2.7ex,shorten >=2.2ex,"{\h[-2.3]\lax \opn{comma}}"{pos=0.57}] \\
...
\begin{proof}
	...
	This is shown by the universal property of the lax comma object (or of the pullback) $\Groth{F}$. For this we use the fact that the chosen cleavage on $\groth{F}\:\Groth{F}\to \C$ (with $\ov{f}^{(C,X)}=(f,\id{})$, see \recx\ref{recgrothconstr}) makes the square
...
In this section, after recalling the notion of opfibration in a $2$-category, we characterize the opfibrations in the functor $2$-category $\m{\A}{\CAT}$ with $\A$ a category. We will show that such characterization restricts to one of discrete opfibrations as well.

The definition of (op)fibration in a $2$-category is due to Street~\cite{street_fibandyonlemma}, in terms of algebras for a $2$-monad. It is known that we can equivalently define (op)fibrations in a $2$-category by representability, as appears in Weber's~\cite{weber_yonfromtwotop}.
...
\begin{rem}
	By definition, a (split) opfibration $\phi\:G\to F$ in $\L$ is required to lift every $2$-cell $\theta\:\phi\c \alpha\aR{}\beta$ to a cartesian $2$-cell $\ov{\theta}^\alpha\:\alpha\aR{}\theta\stb \alpha$. We can draw the following diagram to say that $\phi\c \theta\stb \alpha=\beta$ and $\phi\star\ov{\theta}^\alpha=\theta$.
		...
		\end{cd}
	$\ov{\theta}^\alpha$ cartesian means that for every $2$-cell $\rho\:\alpha\aR{}\alpha'$ and $2$-cell $\sigma\:\beta\aR{}\phi\c \alpha'$, there exists a unique $2$-cell $\nu\:\theta\stb\alpha\aR{}\alpha'$ such that $\phi\star \nu=\sigma$ and $\nu\c \ov{\theta}^\alpha=\rho$. Analogously, we can express being split in these terms.
	
	The second condition of \defx\ref{defopfibinatwocat} then requires the chosen lifting of $\theta\star f$ to be $\ov{\theta}^\alpha\star f$ (i.e.\ the chosen lifting of $\theta$ whiskered with $f$).
...
\begin{prop}\label{propcharactopfibacat}
	Let $\A$ be a category and consider a morphism $\phi\:G\to F$ in $\m{\A}{\CAT}$ \pteor{i.e.\ a natural transformation}. The following facts are equivalent:
	\begin{enumT}
...
\begin{prop}\label{propcharactmoropfibacat}
	Let $\A$ be a category and consider $\phi,\psi\in \Fib[o][\m{\A}{\CAT}][F]$. Let then $\xi\:\phi\to \psi$ be a morphism in $\slice{\m{\A}{\CAT}}{F}$. The following facts are equivalent:
	\begin{enumT}
...
\begin{prop}\label{propopfib-}
	Let $\L$ be a $2$-category. The assignment $F\in \L\mto \Fib[o][\L][F]\in \CAT$ extends to a pseudofunctor 
	$$\Fib[o][\L][-]\:\L\op\to \CAT.$$
...
\begin{proof}
	...
	We are also using \remx\ref{rempullbackopfib}. We immediately get also the isomorphisms that regulates the image of identities and compositions. 
...
In this section, we present our main results. We prove an equivalence of categories between opfibrations in $\m{\A}{\CAT}$ over $F$ and $2$-copresheaves on $\Groth{F}$. This equivalence restricts to one between discrete opfibrations in $\m{\A}{\CAT}$ over $F$ and $\Set$-valued copresheaves on $\Groth{F}$. We also show that both such equivalences are pseudonatural in $F$.
...
Let $\A$ be a category and consider the functor $2$-category $\m{\A}{\CAT}$.

\begin{rem}
	We aim at proving that for every $2$-functor $F\:\A\to \CAT$, there is an equivalence of categories
	$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
	between opfibrations in $\m{\A}{\CAT}$ over $F$ (see \prox\ref{propcharactopfibacat}) and $2$-copresheaves on the Grothendieck construction $\Groth{F}$ of $F$.
	
	Our strategy will be to use \thex\ref{teorgrothisoplaxcolim}, that states that the Grothendieck construction $\Groth{F}$ of $F$ is equivalently the oplax colimit of the $2$-diagram $F\:\A\to \CAT$. Notice that a (strict) $2$-functor from a category to $\CAT$ is the same thing as a functor into the $1$-category $\Cat$. In \remx\ref{remtwocatvariations}, we will say what we could do to extend our results to $\A$ a $2$-category or $F$ a pseudofunctor.
\end{rem}

\begin{prop}\label{propapplyoplaxcolim}
	There is an isomorphism of categories
	$$\m{\Grothdiag{F}}{\Cat} \iso \HomC{\moplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}$$
...
\begin{rem}
	Thanks to \prox\ref{propapplyoplaxcolim}, we can reduce to apply the usual Grothendieck construction on every index. For this we also need the pseudonaturality of the Grothendieck construction (\prox\ref{propgrothconstrispseudonat}).
\end{rem}

\begin{prop}\label{propgrothconstroneveryindex}
	There is an equivalence of categories
	$$\HomC{\moplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}\simeq \HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][F(-)]}$$
	which is pseudonatural in $F$, where $\mPsoplax{\A\op}{\CAT}$ is the $2$-category of pseudofunctors, oplax natural transformations and modifications.
\end{prop}
\begin{proof}
	Notice that
	$$\HomC{\moplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}\iso \HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}$$
	So it suffices to exhibit an equivalence
	$$\m{F(-)}{\Cat}\simeq \Fib[o][n][F(-)]$$
	in the $2$-category $\mPsoplax{\A\op}{\CAT}$. Indeed, for a general $2$-category, postcomposing with a morphism that is an equivalence in the $2$-category gives a functor between hom-categories that is an equivalence of categories. The left hand side is certainly a $2$-functor, while the right hand side is a pseudofunctor by \prox\ref{propopfib-}. We have that the Grothendieck construction gives a pseudonatural adjoint equivalence
	$$\G{-}\:\m{-}{\Cat}\simeq\Fib[o][n][-],$$
	by \prox\ref{propgrothconstrispseudonat}. Whiskering it with $F\op$ on the left gives another pseudonatural adjoint equivalence, that is then also an equivalence in the $2$-category $\mPsoplax{\A\op}{\CAT}$ as needed. The quasi-inverse is given by extending to a pseudonatural transformation the quasi-inverses of the Grothendieck construction on every component. This can always be done by choosing as structure $2$-cells the pasting of the inverse of the structure $2$-cells of the Grothendieck construction with unit and counit of the adjoint equivalences on components. The triangular equalities then guarantee that we have an equivalence in $\mPsoplax{\A\op}{\CAT}$ (we have that the two composites are isomorphic to the identity).
	
	We now prove that the equivalence of categories 
	$$\G{F(-)}\c -\:\HomC{\moplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}\simeq \HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][F(-)]}$$
	that we have produced is pseudonatural in $F$.
	
	We show that $\HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][+(-)]}\:{\m{\A}{\CAT}}\op\to \CAT$ is a pseudofunctor. Given $\alpha\:F'\to F$ in $\m{\A}{\CAT}$, we define the image on $\alpha$ to be $\alpha_{-}\st\c -$, where
	$$\alpha_{-}\st\:\Fib[o][n][F(-)]\apseudo{}\Fib[o][n][F'(-)]\:\A\op\to \CAT$$
	...
	\end{cd}
	where the two isomorphisms are the ones given by the pseudofunctoriality of $\Fib[o][n][-]$ (see \prox\ref{propopfib-}).	We are using that $F(h)\c \alpha_A=\alpha_B\c F'(h)$ by naturality of $\alpha$. Then $\alpha_{-}\st$ is a pseudonatural transformation because $\Fib[o][n][-]$ is a pseudofunctor. As $\alpha_{-}\st$ is a morphism in the $2$-category $\mPsoplax{\A\op}{\CAT}$, we have that $\alpha_{-}\st\c -$ is a functor. Considering $F''\ar{\alpha'}F'\ar{\alpha}F$ in $\m{\A}{\CAT}$, there is a an invertible modification
	...
	\begin{cd}[5.6][5.6]
		\m{F(-)}{\Cat} \arrow[r,simeq,"{\G{F(-)}}"]\arrow[d,"{-\c \alpha}"']\& \Fib[o][n][F(-)] \arrow[d,"{\alpha_{-}\st}"]\\
		\m{F'(-)}{\Cat} \arrow[ru,iso,"{\G{\alpha_{-}}}"{inner sep=0.5ex}]\arrow[r,simeq,"{\G{F'(-)}}"']\& \Fib[o][n][F'(-)]
	\end{cd} 
...
\begin{rem}
	An object of $\HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][F(-)]}$ is essentially a collection of opfibrations on every index $A\in \A$ together with a compact information on how to move between different indexes. The last ingredient that we need in order to prove our main result is that we can pack these data in terms of an opfibration in $\m{\A}{\CAT}$ over $F$.
\end{rem}

\begin{prop}\label{propopfibsonindexes}
	There is an isomorphism of categories
	$$\HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][F(-)]}\iso \Fib[o][\m{\A}{\CAT}][F]$$
	which is \pteor{strictly} $2$-natural in $F$.
\end{prop}
\begin{proof}
	Given $\phi\:G\to F$ an opfibration in $\m{\A}{\CAT}$, we produce an oplax natural
	$$[\phi]\:\Delta 1\aoplax{}{\Fib[o][\CAT][F(-)]}\:\A\op\to \CAT.$$
	For every $A\in \A$, we define $[\phi]_A\deq \phi_A$, thanks to \prox\ref{propcharactopfibacat}. For every $h\:A\to B$ in $\A$, the structure $2$-cell $[\phi]_h$ is the functor $\phi_A\to F(h)\st (\phi_B)$ is defined by the universal property of the pullback in $\CAT$:
	...
	\end{cd}
	$G$ is a functor because $\gamma$ is oplax natural. For every $A\in \A$, we define $\widehat{\gamma}_A\deq\gamma_A$. Then $\widehat{\gamma}$ is a natural transformation by construction of $G$. And the naturality squares of $\widehat{\gamma}$ are cleavage preserving because every $\gamma_h$ and every universal square that exhibit a pullback are cleavage preserving. By \prox\ref{propcharactopfibacat}, we conclude that $\widehat{\gamma}$ is a split opfibration in $\m{\A}{\CAT}$.
	...
	$$\Fib[o][\m{\A}{\CAT}][+] \iso \HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][+(-)]}\:{\m{\A}{\CAT}}\op\to \CAT$$
...
\begin{teor}\label{teormain}
	Let $\A$ be a category and consider the functor $2$-category $\m{\A}{\CAT}$. For every $2$-functor $F\:\A\to \CAT$, there is an equivalence of categories
	$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
	between opfibrations in $\m{\A}{\CAT}$ over $F$ and $2$-copresheaves on the Grothendieck construction $\Groth{F}$ of $F$. Moreover this equivalence is pseudonatural in $F$.
\end{teor}
\begin{proof}
	...
	\begin{center}
		\linesep{1.9}
		\begin{tabular}{LL}
			\m{\Grothdiag{F}}{\Cat} \iso \HomC{\moplax{\A\op}{\CAT}}{\Delta 1}{\m{F(-)}{\Cat}}\simeq\\
			\simeq \HomC{\mPsoplax{\A\op}{\CAT}}{\Delta 1}{\Fib[o][\CAT][F(-)]}\iso \Fib[o][\m{\A}{\CAT}][F]
		\end{tabular}
	\end{center}
...
\begin{cons}[Indexed Grothendieck construction]\label{consindexedgroth}
	...
	\begin{cd}[5.9][5.9]
		\1 \arrow[r,"{\phi_B}"]\arrow[d,equal,"{}"]\& \Fib[o][n][F(B)]\arrow[d,"{F(h)\st}"]\arrow[r,"{\Gprime{F(B)}}"] \&[-0.4ex] \m{F(B)}{\Cat} \arrow[d,"{-\c F(h)}"]\\
		\1 \arrow[ru,Rightarrow,"{[\phi]_h}"{pos=0.585,inner sep=0.35ex},shorten <=3.5ex,shorten >=3ex]\arrow[r,"{\phi_A}"']\& \Fib[o][n][F(A)]\arrow[r,"{\Gprime{F(A)}}"']\arrow[ru,iso,"{\Gprime{F(h)}}"'] \& \m{F(A)}{\Cat}
	\end{cd}
	...
	\noindent Finally, following the proof of \thex\ref{teorgrothisoplaxcolim}, we get the $2$-functor\v[0.5]
	\begin{fun}
		\grothprime{\phi} & \: & \Grothdiag{F} \hphantom{CCC}& \too &\hphantom{CC} \CAT \\[1.4ex]
		&&\begin{cd}*[3][3]
			(A,X) \arrow[dd,bend right=70,"{(h,\alpha)}"']\arrow[d,"{(h,\id{})}"]\\
			(B,F(f)(X))\arrow[d,"{(\id{},\alpha)}"]\\
			(B,X')
	...
	\begin{cd}[5.9][5.9]
		\1 \arrow[r,"{Z\c \opn{inc}_{\h[1]B}}"]\arrow[d,equal,"{}"]\& \m{F(B)}{\Cat} \arrow[d,"{-\c F(h)}"] \arrow[r,"{\G{F(B)}}"]\&[-0.4ex]
		...
		\m{F(A)}{\Cat}\arrow[r,"{\G{F(A)}}"']\arrow[ru,iso,"{\G{F(h)}}"']\& \Fib[o][n][F(A)] 
	\end{cd}
...
\begin{teor}
	Let $\A$ be a category. For every $2$-functor $F\:\A\to \CAT$, the equivalence of categories
	$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
	of \thex\ref{teormain} restricts to an equivalence of categories
	$$\Fib[b][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\Set}$$
	between discrete opfibrations in $\m{\A}{\CAT}$ over $F$ and $\Set$-valued copresheaves on $\Groth{F}$. Moreover this equivalence is pseudonatural in $F$.
\end{teor}
\begin{proof}
	The isomorphism of \prox\ref{propapplyoplaxcolim} restricts to one with $\Set$ on both sides in the place of $\Cat$ by $2$-naturality in $U$ of the isomorphism given by an oplax colimit (see \defx\ref{defoplaxcolim}). Pseudonaturality in $F$ still holds by the same general argument that guaranteed it with $\Cat$ on both sides.
	
	The equivalence of \prox\ref{propgrothconstroneveryindex} restricts to one with $\Set$ in the place of $\Cat$ on the left hand side and discrete opfibrations in the place of opfibrations in the right hand side. Indeed the following is a commutative square of pseudonatural transformations:
	\sq[n][5][5.5]{\m{F(-)}{\Set}}{\Fib[b][n][F(-)]}{\m{F(-)}{\Cat}}{\Fib[o][n][F(-)]}{\G{F(-)}}{}{}{\G{F(-)}}
	...
	The isomorphism of \prox\ref{propopfibsonindexes} restricts to one with discrete opfibrations on both sides in the place of opfibrations, because it suffices to look at the components. Then pseudonaturality in $F$ holds as well, precomposing the pseudonatural transformation produced in the proof of \prox\ref{propopfibsonindexes} with the inclusion of discrete opfibrations in opfibrations.
\end{proof}
...
\begin{rem}
	...
	$$\slice{\m{\A}{\Set}}{F}\simeq \m{\Grothdiag{F}}{\Set}.$$
	Indeed any discrete opfibration $\phi\:G\to F$ in $\m{\A}{\CAT}$ over $F\:\A\to \Set$ needs to have $G\:\A\to \Set$, and all functors $G\to F$ in $\m{\A}{\Set}$ are discrete opfibrations. Our theorem guarantees that this equivalence is pseudonatural in $F$, which does not seem to appear in the literature.
...
\begin{rem}
	...
	We can interpret our main theorem as a $2$-dimensional generalization of this. Indeed it is understood that, in dimension $2$, the correct analogue of the slice is an (op)fibrational slice. Our equivalence
	$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
	says that every opfibrational slice of a Grothendieck $2$-topos is again a Grothendieck $2$-topos.	
\end{rem}
...
\begin{rem}\label{remopvariations}
	...
	The most natural Grothendieck construction $\Groth{F}$ of a contravariant $2$-functor $F\:\A\op\to \CAT$ appears instead to handle fibrations in the place of opfibrations. Such Grothendieck construction $\Groth{F}$ is the lax colimit of $F$. Then $-\op\:\CAT\to\CAT\co$ preserves this colimit and we obtain that $\left(\Groth{F}\right)\op$ is the lax colimit in $\CAT\co$ of $F(-)\op$, which means that
	$${\p{\Grothdiag{F}}}\op=\oplaxcolim{\left(F(-)\op\right)}$$
	in $\CAT$. Then we have the following chain of equivalences of categories:
	\begin{center}
		\linesep{1.9}
		\begin{tabular}{LL}
			\m{{\p{\Grothdiag{F}}}\op}{\Cat} \iso \HomC{\moplax{\A}{\CAT}}{\Delta 1}{\m{{F(-)}\op}{\Cat}}\simeq\\
			\simeq \HomC{\mPsoplax{\A}{\CAT}}{\Delta 1}{\Fib[n][\CAT][F(-)]}\iso \Fib[n][\m{\A\op}{\CAT}][F]
		\end{tabular}
	\end{center}
\end{rem}
...
\begin{rem}\label{remtwocatvariations}
	...
	$$\Fib[o][\m{\A}{\CAT}][F]\simeq \m{\Grothdiag{F}}{\CAT}$$
	where $\Groth{F}$ is now the $2$-$\Set$-enriched Grothendieck construction (introduced by Street's~\cite{street_limitsindexedbycatvalued} and explored more in detail in the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits}). In order to adapt our proof of $\thex\ref{teormain}$ to this setting, one would need $\Groth{F}$ to be a kind of oplax colimit in $\twoCAT$. We believe that $F$ is the $2$-oplax colimit of $F$ followed by the inclusion $i$ of $\CAT$ into $\twoCAT$, where a $2$-oplax natural transformation is a Crans's~\cite{crans_tensorprodforgraycat} oplax $1$-transfor. Such transformations have the same $1$-dimensional conditions of an oplax natural transformation but now also have structure $3$-cells on every $2$-cell in $\A$. Having as codomain $\twoCAT$, they compose well. The added structure $3$-cells are precisely what one needs to encode the $2$-cells
	$$\underline{\delta}^X\:(f,F(\delta)_X)\aR{}(g,\id{})\:(A,X)\to (B,F(g)(X))$$
	in $\Groth{F}$ for every $\delta\:f\aR{}g\:A\to B$ in $\A$. As explained in the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits}, every $2$-cell in $\Groth{F}$ is a whiskering of such particular $2$-cells (in some sense, these $2$-cells are the only ones we need). The middle equivalence of the chain that proves our $\thex\ref{teormain}$ would then be given by the $2$-$\Set$-enriched Grothendieck construction. Finally, the last part of the chain would probably work as well, with the structure $3$-cells managing to encode the action of $G$ on $2$-cells. However, we have not checked these details.
	
	Such generalization would be helpful also to handle non-split opfibrations and pseudofunctors from $\Groth{F}$ into $\CAT$, for which we cannot reduce to functors into $\Cat$. Of course, for this, one could also extend the explicit indexed Grothendieck construction.
...
\begin{rem}[$\A=\1$]\label{exa=1}
	When $\A=\1$, the $2$-category $\m{\A}{\CAT}$ reduces to $\CAT$. A $2$-functor $F\:\1\to \CAT$ is just a category $\C$ and $\Groth{F}=\C$. So \thex\ref{teormain} gives the classical
	$$\Fib[o][n][\C]\simeq \m{\C}{\CAT}.$$
...
\begin{exampl}[$F=\Delta 1$]
	Given any category $\A$, we can consider $F=\Delta 1\:\A\to \CAT$ the functor constant at the terminal $\1$. We have that $\Groth{\Delta 1}=\A$. So \thex\ref{teormain} gives an equivalence of categories
...
\begin{exampl}[Hofmann-Streicher universe in $2$-presheaves]\label{exhofstreichuniv}
	We apply \thex\ref{teormain} to get a nice candidate for a Hofmann-Streicher universe (see~\cite{hofmannstreicher_liftinggrothuniverses}) in the $2$-category $\m{\A\op}{\CAT}$ of $2$-presheaves. The second author will then show in a forthcoming paper that such candidate is indeed a $2$-dimensional classifier in $\m{\A}{\CAT}$, towards a $2$-dimensional elementary topos structure on $\m{\A}{\CAT}$. This was actually the starting motivation for the second author to produce the indexed Grothendieck construction.
	...
	Moving to dimension $2$, $\CAT$ becomes the archetypal elementary $2$-topos. And we now want to classify morphisms with fibres of dimension $1$, i.e.\ fibres that are general sets. As proposed by Weber's~\cite{weber_yonfromtwotop}, the correct $2$-categorical generalization of subobject classifiers are classifiers of discrete opfibrations. The archetypal classifying process becomes, in $\CAT$, the construction of the category of elements (i.e.\ the Grothendieck construction restricted to functors into $\Set$). This can be captured as a comma object from $\1\:\1\to \Set$ or as a pullback of the forgetful $\Set\b\to \Set$ from pointed sets to sets, on the line of \thex\ref{teorgrothconstrislaxcomma}. And doing either of the two provides an equivalence of categories
	$$\groth{-}\:\m{\C}{\Set}\aequi \Fib[b][n][\C]$$
	for every $\C\in \CAT$, exhibiting $\Set$ as the $2$-dimensional universe of the elementary $2$-topos $\CAT$.
	...
	$$\groth{-}\:\m{F}{\O}\aequii \Fib[b][\m{\A\op}{\CAT}][F]$$
	 ...
	 Trying to define $\O$ to send $A\in \A$ precisely to $\Fib[b][\m{\A\op}{\CAT}][\y{A}]$, we only get a pseudofunctor $\O'$, defined by \prox\ref{propopfib-}. \thex\ref{teormain} (together with \remx\ref{remopvariations}) offers a nice way to replace such pseudofunctor $\O'$ with a strict $2$-functor $\O$. Indeed it gives an equivalence of categories 
	...
	$\O$ acts on morphisms by postcomposition. Notice that in this case $\y{A}\:\A\op\to \Set$ and so the left hand side of the equivalence above simplifies to $\slice{\m{\A\op}{\Set}}{\y{A}}$, but we still need the pseudonaturality in $A$, that does not seem to appear in the literature. Our \thex\ref{teormain} guarantees such pseudonaturality and therefore that we get a strict $2$-functor $\O$ pseudonaturally equivalent to $\O'$. This is a Hofmann-Streicher universe, in line with the ideas of~\cite{hofmannstreicher_liftinggrothuniverses} and with Awodey's recent work~\cite{awodey_onhofmannstreicheruniverses}. The second author will show in a forthcoming paper that $\O$ is indeed a $2$-classifier in $\m{\A\op}{\CAT}$, by an argument of reduction of the study of $2$-classifiers to dense generators. He will then restrict this $2$-classifier to one in stacks.
	
	When $\A$ is a $2$-category, the $2$-$\Set$-enriched Grothendieck construction (introduced in Street's~\cite{street_limitsindexedbycatvalued} and explored in the second author's~\cite{mesiti_twosetenrgrothconstrlaxnlimits}) gives
