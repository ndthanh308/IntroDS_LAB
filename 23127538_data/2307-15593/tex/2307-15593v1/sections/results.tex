...
Let $\Xi$ denote the space in which lie the elements of the watermark key sequence.
...
Assuming the conditions of the lemma are met, the only material difference between an LM provider using $\generate$ versus sampling directly from the language model 
...
\begin{lemma}\label{lemma:dist-free-equiv}
    ...
    Let $\decode$ be distortion free with respect to $\nu \in \Delta(\Xi)$
...
\begin{algorithm}[h]
    ...
    \Input{watermark key sequence $\xi \in \Xi^*$}
...
% \pl{haven't defined $\xi$ yet either}
        $y_i \leftarrow \decode(\xi_i,p(\cdot \mid y_{:i-1}))$\label{line:generate} \tcp{assume $\len{\xi} \geq m$} \label{line:generate}
    }
    \Return $y$\;
\end{algorithm}
...
then each $(\wt{Y},\xi^{(t)})$ constitutes an independent copy of $(\wt{Y},\xi)$ and therefore by symmetry $\est{p}$ is 
uniformly distributed over $\{1/(T+1),2/(T+1),\dots,1\}$ for any (non-atomic) test statistic.\footnote{
    ...
    have to break ties when computing $\est{p}$.
...
\begin{algorithm}[h]
    ...
    \Input{string $y \in \mc{V}^*$, watermark key sequence $\xi \in \Xi^*$}
...
\begin{algorithm}[h]
    ...
    \For{$i \in 1,\dots,\len{y}-k + 1$}{
        \For{$j \in 1,\dots,\len{\xi}$}{
            $y^i \leftarrow \{y_{i+\ell}\}_{\ell = 0}^{k-1}$,
            $\xi^j \leftarrow \{\xi_{(j+\ell) \% \len{\xi}}\}_{\ell=0}^{k-1}$\;
...
The so-called birthday paradox \cite{elfving1966selected} implies the LM provider can typically expect to call $\shuffle$ on the order of ${n}^{1/2}$ times,
...
\begin{algorithm}[h]
    ...
    \Params{generation length $m$, language model $p$, decoder $\decode$}
    \Output{string $y \in \mc{V}^m$}
    $\tau \sim \txt{Unif}([\len{\xi}])$,
    $\xi' \leftarrow \{\xi_{(i+\tau)\% \len{\xi}}\}_{i=1}^{m}$\;
...
Observe the watermark potential of text from a deterministic language model is always zero,
...
Type I and II error rates of $h$ will be close to one if the watermark potential is close to zero.
...
\begin{proof}
    ...
    \begin{align*}
        \left[\mu(\{y':\pi(y')<\pi(y)\}),\mu(\{y':\pi_i(y')\leq\pi_i(y)\})\right).
    \end{align*}
    % \pl{technically upper bound should be open ')'}
    As the width of this interval is exactly $\mu(y)$, the result follows immediately.
\end{proof}

Having shown that the ITS decoder is distortion-free, we now proceed to analyze the detectability of the watermark.
...
which in practice may be a more relevant limitation than the statistical power of the watermark.\footnote{
    Note that both $\detect$ and the test statistic (Algorithm~\ref{algorithm:test}) are easily parallizeable.
}
...
we can again upper bound the $p$-value output by $\detect$ in Lemma~\ref{lemma:perm-test} 
using a standard concentration argument and taking a union bound over $j \in [n]$.
We defer the proof of Lemma~\ref{lemma:perm-test} to Appendix~\ref{app:gumbel}.
