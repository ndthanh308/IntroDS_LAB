...
We consider a setting where a (untrusted) third party user queries a language model (LM) by sending prompts 
...
a $\generate$ method that deterministically maps a sequence $\xi$ of random numbers encoded by a (secret)
...
watermark key\footnote{
    Whether the watermark key is secret or not (e.g., if the LM provider publishes the key to allow 
    anyone to detect watermarked text) is an implementation choice that does not affect the main parts of our analysis.
}---which we call the watermark key sequence---to a sample from the language model, and a $\detect$ method that aligns a putative watermarked text
...
call to $\generate$ is equal in distribution to a sample from the original language model, i.e.,
$P(\textbf{text}) = \int_{\xi} \indic{\textbf{text} = \generate(\xi,\textbf{prompt})} d \nu(\xi)$ is 
equal to the original language model's sampling distribution. %\pl{new sentence, detectability is a different point; define $\phi$ before using it - I find the '[claim] where [definitions]' constructions harder to process}
...
We resolve this limitation by designing a wrapper around $\generate$ that calls $\generate$ using a randomly chosen subsequence of $\xi$
...
To remark briefly on the work most closely related to ours, we contrast the distortion-free property with the watermarks 
of \citet{kirchenbauer2023watermark} and \citet{aaronson2023openai},
both of which bias the distribution of watermarked text towards certain $(k+1)$-grams by hashing 
the previous $k$ tokens to determine the next token; 
...
Our work is similar in spirit to the watermark of \citet{christ2023undetectable}, which is also distortion-free;
the key distinguishing feature of our watermark in comparison to theirs and related steganographic techniques \cite{ziegler2019neural}
is that our watermark is not only distortion-free but also robust to substantial corruption of the text,
...
are again a hash of the previous $k-1$ tokens.
...
as an adversary can break the signal from a particular token by
replacing any one of the previous $k-1$ tokens.
Also concurrent to our work, \citet{christ2023undetectable} propose watermarking text from a language model
by generating the initial few tokens truly at random (i.e., sampling directly from the language model) 
and subsequently using these tokens to seed a random number generator to watermark the remaining tokens.
Their work is similar to ours in that it is distortion-free;
however, the resulting watermark is not robust (e.g., replacing any one of the initial seed tokens breaks the watermark)
and they do not empirically evaluate the effectiveness of their watermark.
