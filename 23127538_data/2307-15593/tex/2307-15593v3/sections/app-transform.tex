...
We first introduce the following supporting lemma.
Recall $C_0 = \txt{Var}(\eta(\txt{Unif}([N])))$ for $\eta(i) = (i-1)/(N-1)$.
...
\begin{proof}
    ...
    \begin{align*}
        I(Y,\pi) \defeq \left[\mu(\{y : \pi(y) < \pi(Y)\}),\mu(\{y : \pi(y) \leq \pi(Y)\})\right],
    \end{align*}
