...
We also prove the Morava K-theory version of the above result, which permits to construct plenty of new points for the Balmer spectrum of the Morel-Voevodsky ${\Bbb{A}}^1$-stable homotopic category.
...
$DM(k)$ with respect to the localising subcategory generated by
...
spectrum of Morel-Voevodsky stable homotopic category $SH(k)^c$. Here one needs
...
$BP$-theory.  It allows to represent our numerically trivial element by a class of a regular embedding $[Y\row X]$, such that all the
$Q$-Chern classes of the normal bundle of it come from $X$ and are $Q$-numerically trivial. Passing to the flag variety, one
...
Thus, we get plenty of new ``isotropic'' points of the Balmer spectrum of Morel-Voevdosky category. These points are analogues to and complement the ``classical'' Morava-points 
...
\noindent
{\bf Acknowledgements:}
The support of the EPSRC standard grant EP/T012625/1 is gratefully acknowledged.
...
In the case when the source theory is $\Omega^*$, a multiplicative operation $G$ of invertible type is completely determined by $\gamma_G$ (due to universality of the 
...
$\gamma(x)=\sum_{i\geq 0}^{BP}t_ix^{p^i}$ and $t_0=1$ (note the formal summation!).
...
$t_i$ to $d_i\in B$, where $\gamma_G(x)=\sum_{i\geq 0}^{B}d_ix^{p^i}$ -
...
Our main result compares isotropic and numerical properties of the $P(m)^*$ theory. This plays the crucial role in applications below, and implies the respective facts about Chow groups and Morava K-theories.  

Let $P(m)^*_{Num}$ be the numerical version of the theory (we mod-out the 
kernel of the numerical pairing on $P(m)^*$) - see \cite[Definition 4.3]{Iso}. 

\begin{proposition}
 \label{L-N-Pm-Num}
 The Landweber-Novikov operations descend to $P(m)^*_{Num}$.
\end{proposition}

This is a particular case of the following general result.
Let $J\subset BP$ be an invariant ideal, $Q=BP/J$ and $Q^*$ is the free theory given by: $Q^*(X)=BP^*(X)\otimes_{BP}Q$.

\begin{proposition}
 \label{L-N-Pm-Num-Gen}
 The Landweber-Novikov operations descend to $Q^*_{Num}$.
\end{proposition}

\begin{proof} We need to show that Landweber-Novikov operations preserve
...
$$
This can be extended to a multiplicative endomorphism of the $Q^*[\ov{t}]$-theory
$$
G=\wt{S}_{Q}^{Tot}: Q^*[\ov{t}]\row Q^*[\ov{t}],
$$
with the morphism  $(\ffi_G,\gamma_G)$ of the FGLs, where
$\ffi_G$ acts on $Q$ as $\ffi_{S^{Tot}_{Q}}$ and maps $t_i$ to $t_i$,
while $\gamma_G(x)=\sum_{i\geq 0}^{Q}t_ix^{p^i}$. 
Since the constant term of $\ffi_{S^{Tot}_{Q}}$ is the identity map on 
$Q$, and the terms at higher monomials in $\ov{t}$ decrease the dimension of elements, it follows that $\ffi_G:Q[\ov{t}]\row Q[\ov{t}]$ is invertible. Also, the power series $\gamma_G$ is invertible (with respect to composition). Then the pair $(\ffi_H,\gamma_H)$ given by: $\ffi_H=\ffi_G^{-1}$ and $\gamma_H(x)=\ffi_G^{-1}(\gamma_G^{-1})(x)$
defines an endomorphism of the FGL $(Q[\ov{t}],F_{Q})$ inverse to
$(\ffi_G,\gamma_G)$. 
Recall, that the composition of FGLs is defined as: 
...
$\op{Td}(X)\in Q^*(X)[\ov{t}]$ be the
{\it Todd genus} of $X$ corresponding to the operation $G$ (multiplicative of invertible type). In other words,
$\op{Td}(X)=\prod_j\left(\frac{x}{\gamma_G(x)}\right)(\lambda_j)$, where $\lambda_j$ runs over all {\it $Q$-roots} of the Tangent bundle $T_X$.
This is an invertible element in $Q^*(X)[\ov{t}]$

Let $x\in Q^*(X)[\ov{t}]$ be a numerically trivial element.
Since $G=\wt{S}^{Tot}_{Q}$ is surjective and $\op{Td}(X)$ is invertible, for any 
$z\in Q^*(X)[\ov{t}]$, there exists $y\in Q^*(X)[\ov{t}]$, such 
that $z=\wt{S}^{Tot}_{Q}(y)\cdot\op{Td}(X)$. Then
$\pi_*(\wt{S}^{Tot}_{Q}(x)\cdot z)=\pi_*(\wt{S}^{Tot}_{Q}(x)\wt{S}^{Tot}_{Q}(y)\cdot\op{Td}(X))$. By the Riemann-Roch theorem for multiplicative operations
(of invertible type) - Panin \cite[Theorem 2.5.4]{P-RR}, this is equal to
$\wt{S}^{Tot}_{Q}(\pi_*(x\cdot y))$. So, it is zero, since $x$ was numerically trivial.
...
\begin{remark}
 ...
 This result applies, in particular, to the theories
 $Q^*=BP^*\otimes_{BP}BP/I(m)^r$, for any $m$ and $r$.
 \Red
\end{remark}
...
\begin{proposition}
 \label{killing-SLN}
 Let $X/k$ be a smooth projective variety and $x\in P(m)^*(X)$ be a 
 numerically trivial class, where $m>0$. Then it may be represented by 
 $\wt{x}\in\Omega^*(X)$, such that the projection of $S^{Tot}_{L-N}(\wt{x})$ to $P(m)^*$ is numerically trivial.
\end{proposition}
...
\begin{proof}
 Let $\rho:\Omega^*_{\zz_{(p)}}\row\Omega^*_{\zz_{(p)}}$ be the multiplicative projector defining the theory $BP^*$. It can be 
 ...
 $$
 It is a multiplicative operation $\delta: BP^*\row BP^*[\ov{b}]$ between $p$-typical theories and so, is a specialization of 
 ...
 $\mu[\ov{b}]\circ S^{Tot}_{L-N}(\wt{x})=\mu[\ov{b}]\circ S^{Tot}_{L-N}\circ\eta\circ\mu(\ov{x})=\delta\circ\mu(\ov{x})$. We know that the projection of $\ov{x}$ to $Q^*$ is numerically trivial and that $\delta$ is a specialization of $S^{Tot}_{BP}$. 
 ...
 Hence, the projection of $\wt{x}$ to $Q^*$ coincides with $x$.
 Finally, since $J$ is non-zero, it contains $p^r$, for some $r$ (as it is invariant). Then multiplying $\wt{x}$ by an appropriate integer $q\equiv 1\,(mod\,p^r)$, we may assume that the class $\wt{x}$ is integral (and so, represented by some $[Y\stackrel{y}{\row}X]$ with $Y$-smooth projective). 
 \Qed
\end{proof}
...
\begin{proof}
 ...
 $Z\stackrel{\pi}{\low}Y\stackrel{\beta}{\row}\pp^N$, where $\pi$ is a projective bundle. Let $\xi=c_1^A(O(1))$ on it. 
 ...
 elements of $A^*(W)$ are numerically trivial, which means that $W$ is 
 anisotropic. Thus, $u$ becomes anisotropic over some purely transcendental extension. Since $k$ is {\it flexible}, that is, 
 $k=k_0(t_1,t_2,\ldots)$ and the class $u$ and the variety $W$ are defined
 on some finite level: over some $k_n=k_0(t_1,\ldots,t_n)$ and
 $k_n(\pp)$, respectively, and one can embed $k_n(\pp)$ into $k$ over
 $k_n$, it follows that $u$ is anisotropic already over $k$.
 \Qed
\end{proof}

\begin{remark}
 \label{compl-int-rem}
  Note that here we don't have any restrictions on $A^*$ (aside from being free). In particular, we see that if $A^*$ is not torsion, then complete intersections can't be numerically trivial (since there are no non-zero anisotropic classes, in this case). 
...
The operation $S^{Tot}_{L-N}$ commutes with pull-backs. There is the homological version of it $S^{L-N}_{Tot}$, which on $X$ is equal to
$S^{Tot}_{L-N}\cdot\op{Td}(X)$, where the {\it Todd genus}
$\op{Td}(X)$ is equal to $\prod_j\left(\frac{x}{\gamma(x)}\right)(\lambda_j)$, where $\lambda_j$ runs over $\Omega^*$-roots of the
tangent bundle $T_X$ of $X$, and $\gamma=\gamma_{S^{Tot}_{L-N}}$
is the ``generic'' stable power series $x+b_1x^2+b_2x^3+\ldots$. 
Due to the Riemann-Roch theorem for multiplicative operations - Panin \cite[Theorem 2.5.4]{P-RR},
$S^{L-N}_{Tot}$ commutes with (proper) push-forwards.
Since $S^{Tot}_{L-N}(u)$ is $Q^*$-numerically trivial, so is
$S^{L-N}_{Tot}(u)$. 

The individual homological Landweber-Novikov operations 
$S^{L-N}_{b^{\vec{r}}}(u)$ are just polynomials in various Chern classes
$c_{s}^{\Omega}(-T_Y)[Y]$ of the minus tangent bundle of $Y$. More precisely, $S^{L-N}_{b^{\vec{r}}}(u)$ is the coefficient
at the monomial $b^{\vec{r}}$ in $\op{Td}(Y)$. Since all these classes
are $Q^*$-numerically trivial on $X$, arbitrary polynomials in Chern
classes of the tangent bundle of $Y$ are also $Q^*$-numerically 
trivial on $X$.

Suppose, $\ddim(Y)=d$.
Let $L$ be a very ample line bundle on $Y$. Then, for sufficiently large
$k$, $T_Y\otimes L^{\otimes p^k}$ is generated by global sections and 
...
$T_Y\otimes L^{\otimes p^k}=f^*(Tav)$ is the pull-back of the
tautological vector bundle.

Thus, our class $u\in\Omega^*(X)$ is the push-forward of the class of the
regular embedding 
$$
v=[Y\stackrel{(y,f)}{\lrow}X\times Gr(d,N)]\in\Omega^*(X\times Gr(d,N)).
$$ 
Note that $\Omega^*(X\times Gr(d,N))$
as an $\Omega^*(X)$-algebra is generated by the Chern classes $c_i^{\Omega}(Tav)$.
Since $Q^*$ is $p^r$-torsion, for some $r$ (as $J$ is invariant and non-zero), for $k$ sufficiently large in comparison to the $\ddim(Y)=d$, for any line bundle $L$ on $Y$, the first Chern 
class $c_1^{Q}(L^{\otimes p^k})$ is zero. 
Thus, for sufficiently large $k$, the Chern classes of the pull-back $f^*(Tav)$ are just the Chern classes of $T_Y$. But we know that any polynomial in such Chern classes is $Q^*$-numerically trivial on
$X$. Hence, for such $k$, the class $v\in\Omega^*(X\otimes Gr(d,N))$
is $Q^*$-numerically trivial.

Replacing $X$ by $X\times Gr(d,N)$ and $u$ by $v$, we reduce to the case,
where $u=[Y\stackrel{y}{\row}X]$ is a class of a regular embedding and
the total Chern class of the normal bundle $c_{\bullet}^{Q}(N_{Y\subset X})$ is $y^*c_{\bullet}^{Q}([V]-[U])$, for some
vector bundles $V$ and $U$ on $X$ (namely, $T_{X\times Gr(d,N)}$ and $Tav$, in the above notations). By construction, 
$\ddim(N_{Y\subset X})=\ddim([V]-[U])$. Replacing $X$ by $\pp_X(O\oplus U)$ and $Y\stackrel{y}{\row}X$ by $Y\stackrel{y}{\row}X\row\pp_X(O\oplus U)$ (which keeps the $Q^*$-numerical triviality of the element),
we can assume that $c_{\bullet}^{Q}(N_{Y\subset X})=y^*c_{\bullet}^{Q}(V)$. Let $\eps: Fl_X(V)\row X$ - the variety of complete flags
on $V$. Replacing $X$ by $Fl_X(V)$ and $Y$ by $\eps^{-1}(Y)$, we may 
assume that $[V]=[O(a_1)]+\ldots+[O(a_n)]\in K_0(X)$. Note that anisotropy of $[y]$ is equivalent to the anisotropy of $[\eps^*(y)]$,
...
$[\eps^*(y)]$ by the latter two operations). Similarly, $Q^*$-numerical triviality of $[y]$ implies that of $[\eps^*(y)]$.
...
is $O\oplus N_{Y\subset X}$, by \cite[Proposition 5.27]{so2}, 
$\pi^*(\ov{u})=\xi(\xi+_{Q}a_1)\cdot\ldots\cdot(\xi+_{Q}a_n)-
0\cdot a_1\cdot\ldots\cdot a_n=\xi(\xi+_{Q}a_1)\cdot\ldots\cdot(\xi+_{Q}a_n)$,
where $\xi=c_1^{Q}(O(1))$. 
Tensoring by $M^{\otimes p^k}$ for a very ample line bundle $M$ on $W$,
for sufficiently large $k$, we may assume that the $Q^*$-divisor
...
But the push-forward of this class to $Q^*(X)$ is equal to $u$.
...
Theorem \ref{Main} permits to prove a similar result for {\it isotropic Morava K-theory}.
...
\begin{proof}
 ...
 $CK(m)=\ff_p[v_m]$ (the ``connected Morava K-theory'').
 ...
 $x\in P(m)^*(X)$. I claim that (again, modulo multiplication by some power of $v_m$) $x$ can be chosen in such a way that
 ...
 Clearly, for a given choice of $x$, the statement is true for $d>\ddim(x)$, and the case $d=0$ gives the claim above (note that $P(m)^*(X)$ is generated as a $P(m)$-module by elements of non-negative codimension, since it is so for $\Omega^*$). 
 ...
 Note that since $x$ satisfies the statement for $d+1$, for any
 $\ov{s}$ and any $z\in P(m)^*(X)$, $\pi_*(\ov{S_{P(m)}^{\ov{s}}(x)}\cdot\ov{z})\in CK(m)$ is then divisible by $v_m^a$.
 By the Riemann-Roch theorem (for multiplicative operations -
 Panin \cite[Theorem 2.5.4]{P-RR}), for any $\ov{q}$,
 $$
 \ov{S_{P(m)}^{\ov{q}}(u)}=\sum_{\ov{q}_1+\ov{q}_2+\ov{q}_3=\ov{q}}\pi_*(\ov{c_{\ov{q}_1}(-T_X)}\cdot\ov{S_{P(m)}^{\ov{q}_2}(x)}\cdot\ov{S_{P(m)}^{\ov{q}_3}(y)})
 $$ 
 is divisible by $v_m^a$ as well.
 Consider
 $$
 ...
 where $b=a$, respectively, $b=a+1$, since
 $x$ satisfies the statement for $d+1$, respectively, for any $\ov{s}'\leq\ov{s}_1$, $\ov{s}'\not\in R(x)$. Since
 $S_{P(m)}^{\ov{s}_2}S_{P(m)}^{\ov{r}}$ is a linear combination of Landweber-Novikov operations and $x$ satisfies the
 statement for $d+1$, our expression is divisible by $v_m^{a+b}$ and so, is zero (since the dimension of the result is less, respectively, equal to that
 of $v_m^{2a}$). 
 ...
 Finally, since $\ov{r}$ is a minimal element of $R(x)$, $\ov{S_{P(m)}^{\ov{r}}(u)}=\lambda\cdot v_m^a$, while
$\ov{S_{P(m)}^{\ov{r}'}(u)}$ is divisible by $v_m^{a+1}$, for every other $\ov{r}'\leq\ov{r}$, by degree consideration. Hence, for $|\ov{r}|+\op{codim}(z)=d$, we have: 
$$
...
$$ 
for some $\ov{w}_{\ov{q}}\in CK(m)^*(X)$. The latter summand is divisible by $v_m^{2a+1}$, since $x$
satisfies the statement for $d+1$, and so, is zero by degree considerations. The first summand is equal to
...
condition for $d$. The induction step and the claim is proven. So, we may assume that
$\ov{S_{P(m)}^{Tot}(x)}\in CK(m)^*[\ov{t}](X)$ is numerically trivial.
...
$P(m)$-Landweber-Novikov operations. On the other hand, we know that the projection of it to $CK(m)$ is zero.
...
This is an invariant ideal in $BP$ which is finitely generated, since $BP^*(X)$ is generated by elements of non-negative codimension by the result of Levine-Morel. By the result of Landweber \cite[Theorem 2.7, Proposition 3.4]{La73b}, $\sqrt{I(X)}$ is either $I(n)$, for some $1\leq n<\infty$, or the whole $BP$.
...
Considering the pseudo-abelian (Karoubian) envelope of $Corr^A(k)$
...
Let $A^*$ be a free theory and $A^*_{iso}$ and $A^*_{Num}$ are {\it isotropic} and {\it numerical} versions of it. We have a natural
...
choice of a prime number $p$ and an equivalence class of an extensions of the ground field. 
...
as the Verdier localisation of $DM(L)$ by the localising subcategory
...
\begin{proof}
 ...
 \begin{lemma}
  ...
  decompositions $U=W\oplus\wt{U}$ and $V=W\oplus\wt{V}$, such that $f=id_W\oplus 0$.
 \end{lemma}
 ...
 Let $X$ be an object of $DM(k/k;\ff_p)^c$. Since our weight structure is bounded, $X$ is supported on some segment $[m,n]$, that is, 
 ...
 $X_{m+1}[-1]\row X_{>m}[-1]\row X_m$. Present it as 
 $(id_W\oplus 0)[m]$ as above, for an appropriate $W\in Chow(k/k;\ff_p)$.
...
\begin{remark}
 This can be also observed directly (without the use of weight complex functor, or \cite{BQ}). Namely, it is easy to show that
 if $X_{\leq i}$ and $Y_{\leq j}$ are weight filtrations on $X$ and $Y$, respectively, with inclusion maps $X_{\leq i-1}\stackrel{\iota_X}{\lrow}X_{\leq i}$, $Y_{\leq j-1}\stackrel{\iota_Y}{\lrow}Y_{\leq j}$ and zero differentials, then
 $$
 (X\otimes Y)_{\leq k}:=\op{Cone}(\oplus_{i+j=k-1}X_{\leq i}\otimes Y_{\leq j}\stackrel{g}{\lrow}\oplus_{i+j=k}X_{\leq i}\otimes Y_{\leq j}),
 $$
 where $g=\iota_X\otimes id-id\otimes\iota_Y$, with the inclusion maps $\iota_{X\otimes Y}$ induced by $\iota_X\otimes id$, provides a weight filtration on $X\otimes Y$ with graded pieces
 $(X\otimes Y)_k=\oplus_{i+j=k}X_i\otimes Y_j$ and zero differentials.
 \Red
\end{remark}
...
cardinality, such that there are no correspondences of odd degree between $X_i$ and $X_j$, for $i\neq j$, and the respective elliptic curves are not isogenous. Moreover, if $Y$ is the product of finitely many $X_i$, where
all $i$ are different from a given $j$, then there is no odd degree correspondence from $Y$ to $X_j$ (as the respective map of Albanese varieties would have been dominant). For any $J\subset I$, let us introduce the extension
...
\begin{remark}
 ...
 ${\frak a}_{p,F}$ for non-$p$-equivalent extensions $E/k$ and $F/k$ are not comparable in the sense of topology of 
 ...
 on the Balmer spectrum, a structure not detected by topology.
 \Red
\end{remark}
...
Let $SH(k)$ be the $\aaa^1$-stable homotopic category of Morel-Voevodsky \cite{MV}.
...
$$
Define the
{\it $K(p,n)$-isotropic stable homotopic category} 
...
We can introduce the partial $K(p,n)$-order on the set of finitely generated extensions of $k$. Let $E=\operatorname{colim}_{\alpha}E_{\alpha}$ and $F=\operatorname{colim}_{\beta}F_{\beta}$, where $E_{\alpha}=k(Q_{\alpha})$ and $F_{\beta}=k(P_{\beta})$
...
Our Main Theorem implies that these realisations provide points of the
...
\bibitem{BsMV}
  P.\,Du,\,A.\,Vishik, {\it On the Balmer spectrum of Morel-Voevodsky category}, in preparation.
...
\bibitem{Tan-COIMC}  
  F.\,Tanania, {\it Cellular objects in isotropic motivic categories}, to appear in Geometry and Topology. 
