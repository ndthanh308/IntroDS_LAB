...
\label{subsec:setup}
We carried out our experiments using the ROS-Noetic framework on a computer with core-i7 12700H processor, 32GB RAM and RTX3060 GPU. 
...
\begin{itemize}
	...
	\item \textbf{Scenario 2} \figref{fig:scenario_2}: Dual arm system where the Lite6 is controlled by the IK solver and an Xarm7 arm \cite{ufactory2023xArmCollaborative} controlled by MoveIt
	\item \textbf{Scenario 3} \figref{fig:scenario_3}: HortiBot system, where the Lite6 arm is controlled by the IK solver and the two Xarm7 arms controlled by MoveIt
	...
	We do not compare with CollisionIK for Scenario 4 as it was not possible to adapt the same for controlling multiple arms.
\end{itemize}
...
\begin{table*}[t] \centering
	...
	\caption{DawnIK versus CollisionIK end-effector errors, and number of collisions for the three different scenarios. S1, S2 and S3 indicate Scenarios 1, 2 and 3 respectively. The results demonstrate the ability of DawnIK to avoid collisions in all the three scenarios while being superior at trajectory tracking in Scenarios 1 and 2. Figures shown in bold indicate statistically significantly lower errors for p $<$ 0.05 using the Mann-Whitney U test.}
	\label{tab:results}
\end{table*}
...
The squares are of width 40 cm and the circles are of radius 18 cm. The loops for the eight are circles of radius 10 cm. 
...
% Figure environment removed
In the third scenario, we omit the square paths as a major portion of the square paths intersect with the stationary arms. 
...
With the conflicting constraints of trajectory following and collision avoidance, DawnIK successfully avoids collisions in all the trials. However, this comes at the cost of increased trajectory tracking error. CollisionIK, on the other hand, demonstrates trajectory tracking error similar to that in Scenario 2. 
...
This shows, that DawnIK is better suited to avoiding collisions with multiple arms. 
...
In \figref{fig:s4_square_yz}, the two arms have an extremely challenging task of following the same path while avoiding collisions with each other. However, no collision has been detected and both arms are able to follow the square path where feasible. In all the four scenarios, the controlled arm does not get stuck in any singularity for both the solvers. 