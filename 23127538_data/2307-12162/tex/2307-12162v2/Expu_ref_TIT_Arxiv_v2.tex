...
\usepackage{Expu_ref_TIT_ArXiv_v2}

\usepackage[utf8]{inputenc}

\usepackage{url}
%\title{Typical Codewords and Expurgated Exponent: Most Codes are Expurgable}
%\title{Expurgated Exponent: Most Codes are Good Mother Codes}
%\title{Most Codes Can Be Reliably Expurgated}
%\title{Most Codes Can Be Expurgated with High Probabiltity}
...
%The work of G. Cocco was supported by the Ramon y Cajal fellowship program (grant RYC2021-033908-I) funded by MCIN/AEI/10.13039/501100011033 and by the European Union ``NextGenerationEU'' Recovery Plan for Europe.
...
% change wrt v6: included comments from telco on 5.6.2023
...
%This result has practical relevance, since it implies that finding a good code to expurgate is easy, the difficult part is how to perform a good expurgation, since almost any code is just as good as a starting point.
...
&=\PP\bigl[\Cnall \text{ is typical}, \Cnall \text{ is a good mother code}\bigr]+ \PP\bigl[\Cnall \text{ is typical}, \Cnall \text{ is a bad mother code}\bigr] \notag\\\label{eq:trc_good}
...
&\leq\frac{2M_n^2(1+\epsilon)^2}{\gamn}.\label{eq:var_bound}
\end{align}
...
&\leq \frac{2(1+\epsilon)^2}{\epsilon_2^2\gamn}\label{eq:chain_last}
...
\bibliography{IEEEabrv,Expu_ref_TIT_ArXiv_v2}
