...
\title[Alder-type partition inequality]{Alder-type partition inequality of levels 2 and 3}
...
\keywords{Alder-type inequality, Rogers-Ramanujan identity, gap condition, congruence condition}
...
In this paper, we show that the most general form of Alder type inequality of  levels $2$ and $3$ holds  for all but a finite number of $d$, without the need to exclude any parts:
...
\[
	q_{d}^{(2)} (n) \geq Q_{d}^{(2)} (n)
\]
holds for non-negative integers $n$, except for $n = d+1$, $d+3$, or $d+5$ when $d$ is odd.
\end{thm}


\begin{thm}\label{main2} 
For all $d \geq 3  (2^{12}-1)-2$,
\[
	q_{d}^{(3)} (n) \geq Q_{d}^{(3)} (n)
\]
holds for non-negative integers $n$, except for $n = d$, $d+3$, or $d+6$ when $d$ is a multiple of $3$.
\end{thm}

\begin{rmk}
When $d \equiv 0 \pmod{3}$, we show that Theorem~\ref{main2} holds for $d \geq 3  (2^7 -1)$.
%$d \geq 3 (2^{12}-1)-2$ may not be the best bound. For example, during the proof, we show that Theorem~\ref{main2} holds for $d \geq 3  (2^7 -1)$ when $d \equiv 0 \pmod{3}$.
\end{rmk}




The rest of the paper is organized as follows. In Section 2, we collect key ingredients from the literature and we give a new interpretation for $Q_{d}^{(a)}(n)$, which enables us to include the previously eliminated part $d+3-a$. In Sections 3 and 4, we construct an injection mapping to prove that Theorem~\ref{main} holds for all but finitely many cases when $d$ is even.  In Section 5, we examine the first few values of the corresponding partition functions to complete the proof of Theorem~\ref{main} when $d$ is even.  In Section 6, we complete the proof of Theorem~\ref{main} by proving the case when $d$ is odd. In Section 7,  we cutline the proof of Theorem~\ref{main2}. We conclude the paper with conjectures on general leval $a$ in Section 8.
...
$3d+3\sim 4d+2^r$ & $1+\lfloor \frac{n-d}{2} \rfloor + p_3 (n-3d-2)$ & $(n-y, y)$ and partitions with three parts\\
\bottomrule
\end{tabular}
...
$1\sim d-1$ & 1 & $(1,\dots,1)$ \\
$d\sim d+1$ & 2 & $(d,1,\dots,1)$ \\
$d+2\sim 2d-1$ & 3 &  $(d+2,1,\dots,1)$\\
$2d$ & 4 & $(d,d)$ \\ 
$2d+1$ & 5 & $(2d+1)$ \\
$2d+2$ & 6 & $(d+2,d)$ \\
$2d+3$ & 7 & $(2d+3)$ \\
$2d+4\sim 3d-1$ & 8 & $(d+2,d+2,1\dots,1)$ \\
$3d$ & 9 & $(d,d,d)$ \\
$3d+1$ & 10 & $(2d+1,d)$ \\
...
$3d+5$ & 17 & $(2d+3, d+2)$ \\
$3d+6\sim 4d-1$  &18 & $(d+2,d+2,d+2,1,\dots,1)$ \\
$4d$ & 19 & $(d,d,d,d)$ \\
$4d+1$ & 20 & $(2d+1,d,d)$ \\
...
$4d+8\sim 5d-1$ & 38 & $(d+2,d+2,d+2,d+2,1,\dots,1)$  \\
\bottomrule
\end{tabular}
...
We can also see that $q_d (n) \geq d/2 > 12\ge Q_{d-2}^{(1)} (n)$ holds from $2d$ to $3d+2$ and $q_d (n) \geq 3d/2 > 38\ge Q_{d-2}^{(1)} (n)$ from $3d+1$ to $4d+2^r$. Since $d>127$, both are true.
...
Since the flow of the proof of Theorem~\ref{main2} is similar with that of level 2 case, we only provide an outline of the proof. We again begin by improving the shift Alder-type inequality \cite[Proposition 3.1]{IT} by removing the elimination condition, which is also a strengthened version of Proposition~\ref{ourshift}.


\begin{prop}\label{ourshift3}
For $d \geq 2^{12} - 1$ and $n \geq d+4$,
\[
	q_{d}^{(1)} (n) \geq Q_{d-3}^{(1)} (n).
\]
\end{prop}
...
\[
	g_{d} (n) \geq Q_{d-3}^{(1)} (n)
\] 
by constructing an injection $\psi$ from the set of partitions counted by $Q_{d-3}^{(1)} (n)$ to that counted by $g_{d} (n)$.
%
Set
\[
\begin{aligned}
	S &= \{x \mid x \equiv 1, d-1 \pmod{d}\} \cup \{2d-2, 2d+2\} \setminus \{d-1, d+1\}, \\
...
\[
	\frac{1}{(1-q) (1-q^{2d-2})(1-q^{2d+2}) (q^{2d-1},q^{2d+1} ; q^{d} )_\i},
\]
while 
\[
\sum_{n=0}^\i Q_{d-3}^{(1)} (n)q^n=\frac{(1+q^{d-1})(1+q^{d+1})}{(1-q) (1-q^{2d-2})(1-q^{2d+2}) (q^{2d-1},q^{2d+1} ; q^{d} )_\i}.
\]

As before, we interpret $Q_{d-3}^{(1)}(n)$ as the number of pairs of partitions $(\pi, \mu)$, where $\mu$ is a partition into parts from $S$ and $\pi$ is a partition $\emptyset,  \{d-1\}, \{d+1\}, $ or $\{ d-1, d+1 \}$ satisfying $|\pi| + |\mu| = n$.
%
We arrange $S$ and $T_r$ in increasing order and let $x_i$ and $y_{r,i}$ be the $i^{th}$ elements of $S$ and $T_r$, respectively, it can be easily verified that $y_{r,i} \leq x_i$ for each $i$ from
\begin{align*}
	S    &=\{1, 2d-2, 2d-1,  2d+1, 2d+2,   3d-1,   3d+1,    4d-1, 4d+1, \ldots \}, \\
...
\begin{enumerate}
\item[] (Case 1). If $\pi=\emptyset$, then we just take the map so that $\psi=\varphi$. 
\item[] (Case 2). If $\pi = (d-1)$ or $(d+1)$, we divide into two cases. For convenience, we let $\l_1 = d+2$ (resp. $d+4$) and $\l_2 = d+8$ (resp. $d+16$) when $\pi = (d-1)$ (resp. $(d+1)$). 
\begin{enumerate}
\item Suppose $\mu$ has only a part of size $1$. Then  we define $\psi$ by
\[\psi(d \pm 1,1,1,\ldots, 1)=(\l_1, 1, \ldots, 1).\]
This is well-defined, because we have enough copies of the part $1$ in $\mu$, as $n \geq d+4$.
\item Suppose $\mu$ has a part other than $1$.  We  let $\mu_m$ be the smallest part $>1$ in $\mu$.  Then $\mu_m=2d-2$, $2d-1$, $2d+1$, $2d+2$, $(k+3)d -1$, or $(k+3)d+1$  for a non-negative integer $k$. We note that $\mu_m = (k+3)d- 1$ is the $(2k+6)^{th}$ element and $\mu_m = (k+3)d+1$ is the $(2k+7)^{th}$ element in $S$. 
For a positive integer $m$, let  $\ell$ be the largest integer satisfying $\displaystyle{\frac{\ell (\ell +1)}{2} \leq m}$ and $\displaystyle{m = \frac{\ell (\ell +1)}{2} +j}$ with $0\leq j\le\ell$.
%
%
We define $\psi$ when $\mu_m$ is the $m^{th}$ element in $S$ by
$$\psi(d\pm1,\mu_m,\overline{\mu_m})=(\l_2,\dots,\l_2,\l_1,\dots,\l_1,1,\dots1,\varphi(\overline{\mu_m})),$$
...
\end{enumerate}
\item[] (Case 3). Finally, suppose $\pi = (d-1, d+1)$.
\begin{enumerate}
\item If $\mu$ has only a part of size $1$, then  we define $\psi$ by
\[
	\psi(\pi,1,1,\ldots, 1)=(d+2^{r-1}, 1, \ldots, 1).
\]
\item If $\mu$ has a part other than $1$, then we define $\psi$ by
\[
\qquad\quad	\psi(\pi,\mu_m,\overline{\mu_m})=(d+2^{r-1}, d+8,\dots,d+8,d+2,\dots,d+2,1,\dots1,\varphi(\overline{\mu_m})),
\]
where the part $d+8$ appears $j$ times and  the part $d+2$ appears $\ell-j$ times.  Here, $\mu_m$, $\ell$, $j$ are as in the Case 2 (b). This is also well-defined, because 
\[d+2^{r-1}+j(d+8) + (\ell-j)(d+2)   \leq 2d + \ell(d+8)  \leq 2d + \mu_m.
\] 
\end{enumerate}
\end{enumerate}
 
In summary, we have established that for $n \geq 4d+2^r$ and $d > 2^{12}-1$,
\[
	q_{d}^{(1)} (n) \geq Q_{d-3}^{(1)} (n).
\]
To complete the proof, the remaining values of both sides up to $n = 4d + 2^r - 1$ are verified by direct computation.
\end{proof}

Now we are ready to give the proof of Theorem~\ref{main2}. 

\begin{proof}[Proof of Theorem~\ref{main2}]
For the case $d = 3d' \geq 3 (2^7 -1)$, $Q_{d}^{(3)}(n) = 0$ if $n \equiv \pm 1 \pmod{3}$. Thus, we assume that $n = 3n' \geq 3(d'+4) = d+12$, then applying Lemma \ref{lem:change_a} and Propositions \ref{ourshift} and \ref{shift2} as in the proof of Theorem~\ref{main}, we find that 
\[
	q_{d}^{(3)} (n) \geq q_{d'}^{(1)} (n') \geq Q_{d'-2}^{(1)} (n') = Q_{d}^{(3)} (n).
\] 
%n \geq d+6 for 1st, n' \geq d'+4, i.e. 3 \geq 3d'+12=d+12
Now we assume $d \equiv \pm 1 \pmod{3}$. For this case, we will employ Proposition~\ref{ourshift3} instead of Propositions~\ref{ourshift} and \ref{shift2}. Let $d'=\lceil\frac{d}{3}\rceil \geq 2^{12}-1$. Then applying Lemma~\ref{lem:change_a}, Proposition~\ref{ourshift3} and Lemma~\ref{lem:base_inj}, we find that for $n \geq d+12$,
% n=3n'  \geq d+6 -> n' \geq d'+4 -> n \geq 3d'+12 = d+13 or d+14 
% n=3n'-1 \geq d+ 6 -> n' \geq d'+4 -> n \geq 3d'  + 11 = d+12, d+13
% n=3n'-2 \geq d+6 -> n' \geq d'+4 -> n \geq 3d' + 10 = d+11,d+12
\[
	q_{d}^{(3)} (n) \geq q_{\lceil \frac{d}{3} \rceil}^{(1)} \left( \left\lceil \frac{n}{3} \right\rceil \right)\geq Q_{\lceil \frac{d}{3} \rceil-3}^{(1)} \left( \left\lceil \frac{n}{3} \right\rceil \right)=Q_{3d'-3}^{(3)} \left( 3\left\lceil \frac{n}{3} \right\rceil \right)\ge Q_{d}^{(3)} (n),
\]
where the identity is deduced from the obvious bijection resulting from multiplying or dividing each part by $3$, and the last inequality comes from the same argument as in \eqref{holly1}.



By examining the values of $q_{d}^{(3)}(n)$ and $Q_{d}^{(3)}(n)$ for small values of $n$  in the table~\ref{lv3table}, we complete the proof. In the table, $\delta_{condition}$ is $1$ if the {\it condition} holds, otherwise it is $0$.
\end{proof}
...
If $d \ge 4$, it follows from \cite{KK} that
\[
	\lim_{n\to\i}(q_d^{(a)}(n)-Q_d^{(a)}(n))=\i
\]
for any positive integer $a$.
Hence the inequality $q_d^{(a)}(n)\ge Q_d^{(a)}(n)$ should hold for sufficiently large $n$. There may exist a general method to find a lower bound on $n$, depending on $a$, for which the inequality holds.  Based on numerical experiments, we propose the conjecture that the lower bound of $n$ for the validity of Alder-type inequality of level $a$ is at most $d+a+4$ when $d$ is large enough. More specifically, numerics support the follwing conjectures:
\begin{itemize}
\item[(a)] Theorem~\ref{main} holds for all even $d$ and all odd $d \geq 9$. 
\item[(b)] Theorem~\ref{main2} holds for all $d \geq 4$ except for $d=6$ or $9$.
\item[(c)] For $a \geq 4$, $q_{d}^{(a)} (n) \geq Q_{d}^{(a)} (n)$ holds for all $n \geq d+a+4$ when $d \geq 4a-2$.
\end{itemize}
...
Generalized Alder-type partition inequalities}, https://arxiv.org/abs/2210.04070, 2022. 
...
\bibitem{KK}
S.-Y. Kang and Y.~H. Kim, {\it Bound for $d$-distinct partitions}, Hardy-Ramanujan Journal
(special commemorative volume in honor of Srinivasa Ramanujan) {\bf 43} (2021), 83--90.
