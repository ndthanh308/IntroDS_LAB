...
%To prove that matrices group \eqref{ESL} is a presentation of $SL_2(\mathbb{F}_p)\rtimes \,{{\mathcal{C}}_{2}}$
It is obvious that $\texttt{ESL}_2(\mathbb{F}_p)$ possess presentation in $GL_2(\mathbb{F}_p)$ by subgroup defined in (\ref{ESL}) to show it
...
have degree 1, then $A$  is called split semisimple or diagonalizable \cite{Linear, LinearShpringer}.
...
Let $p=3\sqrt[3]{\det A}$, $q=trA$  then we have 1 root in this field if \[D=\frac{{{p}^{3}}}{3}+\frac{{{q}^{2}}}{2}=-\frac{27\det A}{3}+\frac{{{(trA)}^{2}}}{2}>0.\]
