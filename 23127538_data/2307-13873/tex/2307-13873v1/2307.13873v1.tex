...
\section{Abstract }
We introduce Extended special linear group $ESL_2(k)$, which is generalization of matrix group $SL_2(k)$ over perfect field $k$.
...
The the analytical formula of square roots of 2-nd and 4-th power in $ SL_2(\mathbb{F}_p)$, $ ESL_2({\mathbb{F}_p})$, for any prime $p$,  as well as in $ SL_2(\mathbb{Z})$,  $ ESL_2(\mathbb{Z})$ and in $ SL_2({k}), ESL_2({k})$, where $k$ is arbitrary perfect field, is found by us.
...
We consider more general case \cite{SkSq} consisting in whole group $G= SL_2(F_q)$. We do not  provide additional condition of  splitting.  Also the authors considered separetely  conjugacy classes in $SL_2(F_q )$  \cite{Amit}  such as:  central classes, split regular semisimple classes, non-semisimple classes, anisotropic regular semisimple classes. For each case the criterion of solvability of equation is provided. In the last two cases Bruhat decomposition is applied.
...
We research the conditions of solvability of matrix equation $X^2= A$ in $SL_2(\mathbb{F}_p), GL_2(\mathbb{F}_p)$ and one of splitting extension of $SL_2(\mathbb{F}_p)$ that is $ESL_2(\mathbb{F}_p)$  \cite{SkSq, \cite{Square}}. %New matrix group $ESL_2(\mathbb{F}_p)$ is presented.
...
The new group $ESL_2(\mathbb{F}_p)$  introduced here also arose without description of group structure in the topology.
...
In hyperbolic geometry there are groups in particular group projective special linear group preserves hyperbolic length \cite{Khan} and orientation as well as change orientation. As well known $S{{L}_{2}}(R)$ possess such a property too, but action of $S{{L}_{2}}(R)$ is non-faithful because of $PSL_2(R)$ is a homomorphic image of $SL_2(R)$.

Action of such a group appears in leaf preserving diffeomorphism group diffeomorphism groups which is called
foliated  leaf preserving in Morse-Bott foliation on the solid torus \cite{MaksTor} of simplest Morse-Bott foliations.
Also geometrical transformations from the similar group to $S{{L}_{2}}\left( R \right) \rtimes {{C}_{2}} $  over $Z$ occur in leaf preserving diffeomorphism group in Morse-Bott foliation on the solid Klein bottle, with the complementary circle, which are homeomorphisms.
...
In hyperbolic geometry there are groups in particular group projective special linear group preserves hyperbolic length \cite{Khan} and orientation as well as change orientation. As well known $S{{L}_{2}}(\mathbb{R})$ possess such a property too besides besides property of changing orientation, because of action of $S{{L}_{2}}(R)$ is non-faithful because of $PSL_2(R)$ is a homomorphic image of $SL_2(R)$. A proposed by us group $ES{{L}_{2}}(\mathbb{R})$ also  preserves hyperbolic length \cite{Khan} and orientation as well as change space orientation.

Also such a group  over $R$, i.e. $S{{L}_{2}}\left( R \right) \rtimes {{C}_{2}} $ occurs in continuous transformations of the Klein bottle with the complementary circle, which are homeomorphisms. Action of such a group appears in  diffeomorphisms groups of simplest Morse-Bott foliations which were first presented in geometrical and topological forms as shift and symmetries relatively meridian and parallel by S. I. Maksymenko.

Also Morse-Bott foliation on solid torus \cite{MaksTor} $T = S^{-1} \times D^2$ into 2- tori parallel to the boundary and one singular circle. The diffeomorphisms group of this manifold  posses the subgroup  $\mathcal{G}$ described in geometrical terms,  where the actions of shifts, symmetries relative to a parallel and symmetries relative to a meridian appear, where shift is generated by $\left(\begin{array}{rr}
   1\,\,&0 \\
  1 \,\,&1
...
Define the algebraic properties and structures of $ESL_2(\mathbb{F}_p)$ in the following item.

\textbf{\bf Definition 1.}
{\sl The set of matrices \[\left\{ {{M}_{i}}:Det({{M}_{i}})=\pm 1,  {M}_{i} \in GL_2(\mathbb{F}_p) \right\} \] form \textbf{extended special linear group}  in $GL_2(\mathbb{F}_p)$ and is denoted by  $\texttt{ESL}_2(\mathbb{F}_p)$.
...
As it was found in our study of the roots of matrices from group matrices from defined above group $ESL_2(F_p)$ arise.
 ...
 Justification of such $ SL_2(F_p),  SL_2(\mathbb{Z}),  SL_2(\mathbb{Q}),  SL_2(\mathbb{R})$ extensions existence based on description $Aut\left( SL_2((\mathbb{F_p}) \right), Aut\left( SL_2(\mathbb{Z}) \right), \ldots,  Aut\left(SL_2(\mathbb{R})\right)$ and its subgroups of order 2.
...
\end{array}
\right)$
    as $t$  they generate $SL_2\left( 2,\,\,Z \right)$, new generator
$\left(
\begin{array}{cc}
-1 & 0 \\
0 & 1 \\
...
But $\mathcal{G}$ is proper subgroup of $ESL(2,\mathbb{Z})$ that is more special whole $G{{L}_{2}}\left( \mathbb{Z} \right)$  then $ES{{L}_{2}}\left( \mathbb{Z} \right)$ which has as kernel a proper subgroup of $S{{L}_{2}}\left( \mathbb{Z} \right)$, because of  $det\left( \mathcal{G} \right)=\pm 1$.
...
We take in consideration involution which generate symmetry of Torus symmetries with respect to the parallel are represented by matrix
...
Now using concept of new group ESL(2,Z) allow us to give exact and structural characterization of group $\mathcal{G}$ which contains in ${{D}^{lp}}\left( F \right)$. For this goal we consider subgroup of ESL(2,Z) with kernel $K\simeq \left\langle \left( \begin{array}{rr}
   1\,\,\,&0 \\
  1\,\,\,\,&1 \\
...
If  $A \in ESL_2(\mathbb{F}_p)$ then the matrix equation  \eqref{Eq} has a solution iff
...
there exists such  $c\in {{F}_{p}}$  for which  ${{c}^{2}}=tr(A)+2$. For such $c$ we get, that the roots  ${{\mu }_{1}},{{\mu }_{2}}$  of the polynomial  ${{x}^{2}}-cx+1$ satisfy the following property:  $\mu _{1}^{2}$  and  $\mu _{2}^{2}$  are equal to e.v. of matrix $ $.  Hence any matrix $B$ with  $tr(B)=c$  and  $\det (B)=1$ satisfies the property:  ${{B}^{2}}$ satisfies the property: ${{B}^{2}}$ has the same e.v. as $A$.
...
% & 1\,\,c \\
%\end{align} \right)$,
 therefore this matrix is over ${{\text{F}}_{p}}$.
...
Let us express the square by trace and determinant. Since $ \mu_1^2 + \mu_2^2 = (\mu_1 + \mu_2)^2  - 2\mu_1\mu_2$ then $(\mu_1 + \mu_2)^2  = \mu_1^2 + \mu_2^2  + 2\mu_1\mu_2 = tr(A) + 2$. A value in the left part is the quadratic residue so $tr(A) + 2$ is too.
 Since in $PSL_2(F_p)$ and $A$ and $-A$
 are identified according to the factorization by the center of the $SL_2(F_p)$ then in case of $PSL_2(F_p)$ the both signs $"\pm"$ before the $tr(A)$ fit. In the case of $SL_2(F_p)$ this conditions transforms in the form $tr(A) \pm2$ due to possible root $B$ with $det(B)=-1$. But such case $B \notin SL_2(F_p)$. If $tr(A)+2$ is multiple to $p$ then characteristic polynomial of $A$ ($\chi_A(x)$) takes form $x^2+1$, because of general formula is such $\chi_A(x)$ has form  $x^2 -(tr(A))x+detA$.

\textbf{Define} the \textit{polynomial squared}  if there is such ${{\chi }_{B}}(x) = x^2 - cx + d$, that $d=\mu_1 \mu_2, \, c= \mu_1 + \mu_2, \,  c,d \in F_p $  and $\mu^2_1 = \lambda_1$,  $\mu^2_2 =\lambda_2$, where  $\lambda_2$, $\lambda_2$ are e. v. of  another polynomial   ${{\chi }_{A}}(x)$.
We can formulate the similar criterion for a polynomial.
In particular  characteristic polynomials of $A$ and $B$  could play such  a role.
Let be ${{\chi }_{A}}(x) = x^2 - ax + 1$ $(a \in F_p)$.
To such squared polynomial ${\chi}_{A}(x)$ exists we investigate relation between their roots. We take into account that ${\chi }_B(x)$ for matrix for $SL_2(F_p)$, so $\mu_1 \mu_2=1$.  But roots of ${\chi }_{B}(x)$ and even ${\chi }_{A}(x)$ can belong to $F_{p^2}$ but not to $F_p$. So we will consider symmetric functions from $\lambda_1$ and  $\lambda_2$. $(\mu_1 +\mu_2)^2= \lambda_1 + \lambda_2 +2 \sqrt{\lambda_1 \lambda_2} = \lambda_1 + \lambda_2 +2$ because of $\lambda_1 \lambda_2 =1$.
...
In cases when $A\in ES{{L}_{2}}({{F}_{p}})$ or $A\in G{{L}_{2}}({{F}_{p}})$ the proof is very similar.
\end{proof}
...
$\sqrt[3]{A}=\frac{A-tr\left( \sqrt[3]{A} \right)\det (\sqrt[3]{A})}{t{{r}^{2}}\left( \sqrt[3]{A} \right)-\det \left( A \right)}$
...
\bibitem{SkAbst} Skuratovskii R.V. On the verbal width in the alternating group $A_n$ and Matieu groups1 // Modern problems in mathematics and its applications International (53rd National) Youth School-Conference, 2022, Yekaterinburg, 2022 from January 31 to February 4 section Group theory, pp. 1-2. https://sopromat.imm.uran.ru.
