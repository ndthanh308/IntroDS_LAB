...
\begin{center}
	{\LARGE\LARGE \bf
Extended Special Linear group $ESL_2(\mathbb{F})$ and roots in $SL_2(\mathbb{F})$, $SL_2(\mathbb{Z})$, $GL_2(\mathbb{F}_p)$,
 root computation in $M_2(\mathbb{F}_p)$
}
...
The analytical formula of square roots of 2-nd, 3-rd and 4-th power in $SL_2(\mathbb{F}_p)$, $SL_2({\mathbb{F}_p})$. Moreover
...
\textbf{Key words}: extended special linear group, equation in matrix group,  splittable  extension, formula of square roots in linear groups, set of squares in matrix group, criterion of  square root existing in $SL_2(\mathbb{F}_p)$.\\
...
discrete subgroups of $PSL_2(\mathbb{R})$ found by the GM algorithm. But we solve existing root problem for arbitrary element of $PSL_2(\mathbb{F}_p)$.

Also such criterion of root existing for $SL_2(\mathbb{F}_p)$, $PSL_2(\mathbb{F}_p)$ and $GL_2(\mathbb{F}_p)$ are established.
...
As it was found in our study of the roots in matrix groups, solutions of $X^2=A$ arise in defined above group $ESL_2(\mathbb{F}_p)$, where $A \in SL_2(\mathbb{F}_p)$.
 ...
 $\mathbb{F}$ as well as over the ring $\mathbb{Z}$.

 Justification of existence of $ SL_2(\mathbb{F}_p),  SL_2(\mathbb{Z})$ extensions is based on  the description $Aut\left( SL_2((\mathbb{F_p}) \right), Aut\left( SL_2(\mathbb{Z}) \right) $ and its subgroups of order 2.
...
The existence of a non-trivial homomorphism $\varphi :\,\,{{\mathbb{Z}}_{2}}\to Aut\left( S{{L}_{2}}(\mathbb{Z}) \right)$, as well as $\phi :\,\,{{\mathbb{Z}}_{2}}\to Aut\left( S{{L}_{2}}({{\mathbb{F}}_{p}}) \right)$ and $\phi :\,\,{{\mathbb{Z}}_{2}}\to Aut\left( S{{L}_{2}}({{\mathbb{Z}}}) \right)$, can be proved by indicating an element of order 2 in the automorphisms of base group that is the kernel of the semidirect product we want to construct.
...
Furthermore the concept of new group  $\widehat{ESL}_{2}\left( \mathbb{Z} \right)$ admits us to obtain a structural characterization and set of generators with relations for $\mathcal{G}$.
...
We take in consideration involutions which generate symmetry of torus symmetries with respect to the parallel are represented by matrix
 $t=\left(\begin{array}{rr}
   1\,\,&0 \\
  0 \,\,&-1
\end{array} \right)$ and generators of the top subgroup of $\widehat{ ESL}_{2}\left( \mathbb{Z} \right)$  $i=\left(\begin{array}{rr}
  -1\,\,\,&0 \\
  0 \,\,\,&1
\end{array} \right)$ and $D$ which are also is symmetries with respect to the meridian of the torus in paper \cite{MaksTor}.
Now using concept of new group $ESL_2(\mathbb{Z)}$ allows us to give exact and structural characterization of group $\mathcal{G}$ which contains in ${{D}^{lp}}\left( F \right)$. For this goal we consider subgroup of $\widehat{ESL}_2(\mathbb{Z)}$ with kernel $K\simeq \left\langle \left( \begin{array}{rr}
   1\,\,\,&0 \\
  1\,\,\,\,&1 \\
...
Thus, we find a structure of $\mathcal{G}$ which, up to a way to define a semidirect product, is $\mathcal{G}\,\simeq K\ltimes \left\langle t,i \right\rangle $.
 An important fact that $K\ltimes \left\langle t,i \right\rangle $
  is a subgroup  in $\widehat{ESL}_{2}(\mathbb{Z})$. Top subgroup of $\mathcal{G}$ has 2 generators but kernel subgroup $K$ is one generated, unlike the  kernel in $\widehat{ESL}ESL_{2}(\mathbb{Z})$ having 2 generators. If we denote $\left( \begin{array}{rr}
   \,1\,\,\,& 0 \\
     \,1\,\,\,\,& 1
...
Geometrical transformations corresponding to matrices that form the subgroup of the introduced here $S{{L}_{2}}\left( \mathbb{R} \right) \rtimes {{C}_{2}} $ group, occur in leaf preserving diffeomorphism group in Morse-Bott foliation on the solid Klein bottle \cite{MaksB}, with the complementary circle, which are homeomorphisms.
...
of diffeomorphism $D\left( {{L}_{p,q}} \right)$ of $L_{p,q}$ is under consideration in \cite{MaksTor},
 whence a group closure of $D\left( {{L}_{p,q}} \right)$ is just $ES{{L}_{2}}(\mathbb{Z})$ but algebraic structure of set was not investigated before so it was clasified in \cite{MaksTor}
  as the matrix subset of $G{{L}_{2}}\text{ }\left( \mathbb{Z} \right)$  with determinant -1 also there is transformation $T$  in that item with $\det (T)=1$.
...
This solution $X \in ESL_2(\mathbb{F}) \setminus SL_2(\mathbb{F})$ iff $\left({tr A - 2}{}\right)$ is quadratic element or 0 in $\mathbb{F}$ but $\left({\tr A + 2}{}\right)$ is not. Conversely $X \in SL_2(\mathbb{F}_p)$ iff $\left({\tr A + 2}{}\right)$ is quadratic element. Solutions belong to $ ESL_2(\mathbb{F})$ and $SL_2(\mathbb{F})$ iff $\left({\tr A + 2}{}\right)$ and $\left({\tr A - 2}{}\right)$ are quadratic elements.
...
Thus, the coefficients $c,$ $d \in {\text{F}}_{p}$, so such $B$ exists in $SL_2({\text{F}}_{p})$. In case $A \in \mathbb{S}{{L}_{2}}\left( {{\text{F}}_{p}} \right)$  our expression takes form  $\tr A-2\sqrt[{}]{\det A}=2 \pm2$ whose values are always squares.
...
\end{prop}
\begin{proof}
If matrix $A$ do not admits diagonal form over ${\F}_{2}$ then $A$ is not square in $G{L_{2}}\left( {{\F}_{2}} \right)$ over ${\F_{2}}$.
...
%Вставлял начиная отсюда в новый Архив
...
\begin{thm}
%(Theorem 2.)
 If a matrix $A\in S{{L}_{2}}({{F}_{}})$ is semisimple and diagonalizable over ${{\text{F}}_{p}}$ and $(\frac{{{\lambda }_{1}}}{p})=(\frac{{{\lambda }_{2}}}{p})=-1$, then  for the existing $\sqrt{A}$, it is necessary and sufficient, to $A$ be similar to scalar matrix $D$.
 \end{thm}
...
\item If $B^3=0$, then it's minimal canceling polynomial is $X^2$ or $X$. By Kelly Hamilton equation (C.H.E) $B^2 - tr{B} \cdot B + \det{B} \cdot I = 0$, which leads us to $tr{B}=0, \det{B}=0$.
\end{itemize}
...
$X^n = X \cdot X^{n-1} \underset{c(X)}  \equiv X \cdot (s_{n-1}X + t_{n-1} E) = s_{n-1}X^2 + t_{n-1} X \underset{c(X)}  \equiv s_{n-1}(\tr{B} X -\\- \det{B} \cdot E) + t_{n-1} X = (s_{n-1} \tr{B} + t_{n-1}) X - s_{n-1} \det{B} \cdot I$
