...
\begin{center}
	{\LARGE\LARGE \bf
Extended Special Linear group $ESL_2(\mathbb{F})$ and square roots in matrix groups $SL_2(\mathbb{F})$, $SL_2(\mathbb{Z})$, $GL_2(\mathbb{F}_p)$    \RomanNumeralCaps{1}.
}
...
The criterion of roots existing for different classes of matrix --- simple and semisimple matrixes from $ SL_2({\mathbb{F}})$, $ SL_2({\mathbb{Z}})$  and $ GL_2({\mathbb{F}})$ are established.
...
The analytical formula of square roots of 2-nd, 3-rd and 4-th power in $ SL_2(\mathbb{F}_p)$, $ ESL_2({\mathbb{F}_p})$, for any prime $p$,  as well as in $ SL_2(\mathbb{Z})$,  $ ESL_2(\mathbb{Z})$ and in $ SL_2({k}), ESL_2({k})$, over perfect field $k$, is found by us. We managed to find solutions for all cases of roots existing for the root formula from \cite{Sull} the equation $X^2=A$, in contrast to the formula proposed in \cite{Sull} for which the authors did not find roots in the singular case, when 0 appears in the denominator of the formula.
...
\cite{Sull} but under unsolved by him condition which $\sqrt{A}$  exists.
Moreover, the author did not find solutions in the limiting case when the denominator $trA  \pm 2 \sqrt{detA}$ tends to 0 so we describe this cases. Also formulas for roots of  3-rd and forth powers were established by us.


But we find not only all such conditions but the method of square root computation for $n \times n$ matrix. Moreover we consider this question in groups over finite fields. Also we indicate in which group $\sqrt{A}$  lies.
...
In hyperbolic geometry there are groups in particular projective special linear group preserve hyperbolic length \cite{Khan} and orientation as well as changes orientation. As well known $S{{L}_{2}}(\mathbb{R})$ that possess such a property too besides property of changing orientation, because of action of $S{{L}_{2}}(\mathbb{R})$ is non-faithful because of $PSL_2(R)$ is a homomorphic image of $SL_2(R)$. A proposed by us group $ES{{L}_{2}}(\mathbb{R})$ also  preserves hyperbolic length \cite{Khan} but its action can change space orientation. Furthermore a half of its elements change orientation and a half do not change.

%Повтор предыдущего абзаца в
...
In this research we continue our previous investigation \cite{SkAbst, SkAbst21,  SkMahn, Square}.
%
...
But this action was defined geometrically by symmetry with respect to meridian and parallel of torus and infinite shift on torus also corresponding 3 matrix to these elements were given.



% Ooo iu eni?aaeee yoi ii?ii ia?anoaaeou a I?eeeieia?eac eee i?inoi inoaaeou.
...
0 & 1 \\
\end{array}
\right)$. The involution from the top-subgroup $\,{{\mathcal{C}}_{2}} \simeq   \left\langle \left(
\begin{array}{cc}
-1 & 0 \\
0 & 1 \\
\end{array}
\right) \right\rangle  $ induces the sign of automorphism in $Aut\left( SL_2(F_p) \right)$.
}

Matrices  with determinant -1 correspond to the elements changing Euclidean space orientation.
As it was found in our study of the roots in matrix groups solutions from defined above group $ESL_2(F_p)$ arise.
...
\begin{rem}
Subgroup $\mathcal{C}_{2}$ is $\mathbf{TI-subgroup}$ and antinormal subgroup.
\end{rem}

\begin{proof}
In view of ${\mathcal{C}}_{2}$ is one generated then its centralizer coincides with its normalizer. One easy can verify that centralizer consists of all diagonal matrices  from $ESL_2(\mathbb{F}_p)$.
Let us find a structure of such normalizer $N_{ESL_2(\mathbb{F}_p)} ({\mathcal{C}}_{2})$.
In view of e.v. is  invariant under conjugation by non-singular matrix over field the normalizer of top subgroup ${\mathcal{C}}_{2}$ in $ESL_2(\mathbb{F}_p)$ consists of  all diagonal matrices  from $ESL_2(\mathbb{F}_p)$ and permutational matrix ${{\mathcal{P}}}=\left( \begin{array}{rr}
   0\,\,\, &1 \\
  1\,\, & 0 \\
\end{array} \right)$.
We assume that $N_{ESL_2(\mathbb{F}_p)} ({\mathcal{C}}_{2}) \simeq   D(SL_2(\mathbb{F}_p))\rtimes \,{\mathcal{P}}$, where $D(SL_2(\mathbb{F}_p))$ diagonal subgroup of $ESL_2(\mathbb{F}_p)$.


For the rest of elements condition of $A \cap A^g = e$ for each $g \in ESL_2(\mathbb{F}_p) \setminus N_{ESL_2(\mathbb{F}_p)} ({\mathcal{C}}_{2})$ holds. Thus, $\mathcal{C}_{2}$ is $\mathbf{TI-subgroup}$, hence $\mathcal{C}_{2}$ is antinormal subgroup.
\end{proof}
...
We briefly introduce the minimal set of generators and new relations in   $ESL_2\left( \mathbb{Z} \right)$ \cite{AutSLp} i.e. this group over  integer ring. Each relation of $SL_2\left(  \mathbb{Z} \right)$ holds.
We denote
$\left(
\begin{array}{cc}
1 & 1 \\
...
${{i}}si^{-1}=s^{-1}$. The second relation is ${{i}}ti^{-1}=t^{-1}$ and the rest of them are $t^4=i^2=e$. The order of $s$ is $\infty$ because $s$ is a shift. Note, that $\mathcal{C}_{2}= <i>$.
...
We additionaly denote this subgroup by $\left\langle\rho \right\rangle\ltimes \left\langle i  \right\rangle$. Note that group of the diffeomorphisms $h$ coinciding with some vector bundle morphism also function ${{\lambda }_{h}}:\,\,\mathbb{R}\to \mathbb{R}$ is described in \cite{MaksB}  in item 3), there are subgroup $h'(w,s)=\left( {{e}^{2\pi i{{\lambda }_{h}}(s)}},s \right),\,\,\,\,\,{{\lambda }_{h}}\left( s+1 \right)=-{{\lambda }_{h}}\left( s \right)$ presented in form of functions.  Now we can describe its structure as semidirect product. We establish a homomorphism from this group to $\left\langle\rho \right\rangle\ltimes \left\langle i  \right\rangle$.
...
This solution $X \in ESL_2(\mathbb{F}) \setminus SL_2(\mathbb{F})$ iff $\left({tr A - 2}{}\right)$ is quadratic element or 0 in $\mathbb{F}$ but $\left({trA + 2}{}\right)$ is not. Conversely $X \in SL_2(\mathbb{F}_p)$ iff $\left({trA + 2}{}\right)$ is quadratic element. Solutions belong to $ ESL_2(\mathbb{F})$ and $SL_2(\mathbb{F})$ iff $\left({trA + 2}{}\right)$ and $\left({trA - 2}{}\right)$ are quadratic elements.
...
Let us prove the sufficiency of the condition $(\frac{trA+2}{p})=1$.
According to Viet Theorem ${{\mu }_{1}}+{{\mu }_{2}}=c$ and ${{\mu }_{1}}+{{\mu }_{2}}=Tr(B)$, also ${{c}^{2}}=trA+2$ by construction of ${{\chi }_{B}}(x)$.
...
Consider the sum $\mu _{1}^{2}+\mu _{2}^{2}={{\left( {{\mu }_{1}}+{{\mu }_{2}} \right)}^{2}}-2{{\mu }_{1}}{{\mu }_{2}}={{c}^{2}}+2-2{{\mu }_{1}}{{\mu }_{2}}={{c}^{2}}+2-2=trA=a$, according to Viet theorem $\mu _{1}^{2}+\mu _{2}^{2}$ is coefficient of linear term in ${{\chi }_{{{B}^{2}}}}$. The free term of ${{\chi }_{{{B}^{2}}}}\left( x \right)$ as well as of ${{\chi }_{A}}\left( x \right)$ equals to 1 as products of e.v. $\mu _{1}^{2}\mu _{2}^{2}=Det\left( B ^2 \right)$ and ${{\lambda }_{1}}{{\lambda }_{2}}=1$ because of ${{B}^{2}},  A\in S{{L}_{2}}\left( \mathbf{F} \right)$.  Thus coefficients of ${{\chi }_{{{B}^{2}}}}\left( x \right)$ and ${{\chi }_{A}}\left( x \right)$ coincide providing an equality of these polynomials.
...
% & 4\lambda ,\,\,\mbox{iff}\,\,\sqrt[{}]{\det A}=\lambda. \\
%\end{align} \right. \]
The value $4\lambda =trA+2\sqrt[{}]{\det A}$ is declaimed in the condition \eqref{trGL} as quadratic residue, therefore $4\lambda \in {\text{F}}_{p}$. Also absolute term $d$ is $\sqrt[{}]{\det A}=\sqrt[{}]{{{\lambda }^{2}}}=\pm \lambda \in F_p $
because of  both elements $\lambda$  on diagonal and rest of elements is 0 moreover all conjugated matrices to scalar matrix $A$ coincide with $A$ because $A$ in centre, that’s why $\lambda \in F_p$.
Thus, the coefficients $c,$ $d \in {\text{F}}_{p}$, so such $B$ exists in $SL_2({\text{F}}_{p})$. In case $A \in \mathbb{S}{{L}_{2}}\left( {{\text{F}}_{p}} \right)$  our expression takes form  $trA-2\sqrt[{}]{\det A}=2 \pm2$ whose values are always squares.

In case of diagonal matrix which is not scalar (case of simple matrix) we get $d =\pm \sqrt{\det A}$ but under additional condition to \eqref{trGL} $\det A$ is quadratic residue, hence we have $\pm \sqrt[{}]{\det A} \in F_p$.



%(under additional condition ($(\frac{\lambda }{p})=1$ or $(\frac{-\lambda }{p})=1$)) belongs to ${{\text{F}}_{p}}$ because of \[trA=2\lambda \in {{\text{F}}_{p}}\] and that’s why \[\lambda \in {{\text{F}}_{p}}.\]


% So such ${{\chi }_{B}}(x)={{x}^{2}}-cx+\sqrt[{}]{\det A}$ exists over ${{\text{F}}_{p}}$.
The structure of matrix roots $B_i$ of \emph{\textbf{exceptional limiting case}}, when  $trA+2=0$ corresponds to a scalar matrix $A= -E$  in   $S{{L}_{2}}\left( \mathbb{F} \right)$,
 then
$$
B_1=
\left(
...
\right)
,$$
where  $\lambda^2=-1$.  It is obviously that this root exists if -1 is quadratic element in ${\mathbb{F}}_{}$, whence we see $ B_1, B_2$ are elements of $ESL_2{\mathbb{F}_p}$. If  $trA-2=0$ then we construct the same roots  but with condition  $\lambda^2=1$.
...
then the case  $trA-2\sqrt{detA}=0$,  where  $A= \lambda E$  implies that  $trA =2 \lambda$,  and its  roots
$$
\sqrt{A}=
\left(
\begin{array}{rr}
...
The case  $trA-2=0$  implies that  $A= E$ , so its  roots
$$
\sqrt{A}=
\left(
\begin{array}{rr}
...
%The condition $(\frac{trA+2}{p})=1$ is necessary and sufficient for existing a solution of ${{X}^{2}}=A$  in $S{{L}_{2}}({{\text{F}}_{p}})$.



Let us construct the solution of equation $X^2=A$ in $SL_2(F_p)$. In general case we obtain the solution \[{{B}^{2}}=A',\] where $A'\sim A$ with eigenvalues ${{\lambda }_{1}}=\mu _{1}^{2},\,\,\,{{\lambda }_{2}}=\mu _{2}^{2}.$     Since $c \in {{\text{F}}_{p}}$ then we can construct in normal Frobenius form a matrix
\[\left( \begin{array}{rr}
  0\,\,& -1 \,  \\
  1\,\, &c \, \\
\end{array} \right)={{B}_{}}\]
...
Since ${{\lambda }_{1}}+{{\lambda }_{2}}={{(\mu _{1}^{}+\mu _{2}^{})}^{2}}-2=trA$ and  that is why ${{\left( \mu _{1}^{{}}+\,\mu _{2}^{{}} \right)}^{2}}=trA+2$ this equality holds iff  $(\frac{trA+2}{p})=1$. Thus, the condition $(\frac{trA+2}{p})=1$ is  sufficient for existing of ${{\chi }_{b}}(x)$.
...
\begin{cor}
All solutions of equation $X^2=A$, where $A \in SL_2(\mathbb{F})$ contain in $ESL_2(\mathbb{F})$ and $\mathbb{F}$ is arbitrary field.
\end{cor}
\begin{proof} Let $B$ is solution of $X^2=A$.
Since $1=detA= detB detB$ then $detB=\pm1$. Thus, $B \in ESL_2(\mathbb{F})$.
\end{proof}
...
Consider the characteristic equation for $A$:  ${{x}^{2}}-tr(A)x+\det (A) E=0$.  According to Cayley Hamilton theorem we have
\begin{align*}
  & {{A}^{2}}-tr(A)A+E=0,  \\
  & {{A}^{2}}+E=tr(A)A.    
\end{align*}
...
Note the in analytical formula of root in $GL_2 (\mathbb{R})$ founded in \cite{DON} the case of finite field $\mathbb{F}_p$ was not considered. Furthermore, case when $ tr(A) + \epsilon_1 2 \sqrt{det(A)} E= 0$ was not provided not in their formula nor in their work \cite{DON}.
...
If a simple matrix  $A\in G{{L}_{2}}({\mathbb{F}_{p}})$ and $\left( Tr(A) \pm 2A \sqrt{det (A)}  \right)$ is quadratic residue or 0 in ${\mathbb{F}_{p}}$, then $\sqrt{A}\in G{{L}_{2}}({\mathbb{F}_{p}})$
$$\sqrt{A}=\frac{ \pm 1}{ \sqrt{trA \pm 2 \sqrt{det A}}}\left( A \pm E \sqrt{det A} \right),$$
where  sign $'\pm'$ in $\left( A \pm E \sqrt{det A} \right)$  coincides with  sign $'\pm'$  in denominator $\sqrt{trA \pm 2 \sqrt{det A}}$, whereas sign before 1 in the nominator is independent.
\end{cor}

\begin{proof}
Consider the characteristic equation for $A \in GL(\mathbb{F})$:  ${{x}^{2}}-tr(A)x+\det (A) E=0$.  Then using Cayley Hamilton theorem we derive
\begin{align*}
...
\begin{align*}
  & {{A}^{2}}+2A(\pm \sqrt{detA})+E=-tr(A)A+2A\sqrt{detA}, \\
  & {{\left( A \pm E{detA} \right)}^{2}}=A\left( trA+2\sqrt{detA} \right).
\end{align*}
This lead us to solution in similar way described above. But the exceptional limiting case $c=trA-2\sqrt[{}]{\det A}= 0$ was not founded in \cite{Sull} so we describe it in the prove of Theorem \ref{CriterionPSL}, where we investigate two possible subcases in this situation $0= trA-2\sqrt[{}]{\det A} = 2\lambda -2\sqrt{{{\lambda }^{2}}} = 2\lambda \pm 2\lambda$ where in particular the solutions $B_1, B_2 \in ESL_2 (\mathbb{F})$ appear. Also roots of third and fourth power were not founded in \cite{Sull, Nort, Arslan}. The investigation \cite{Nort} claims that there are class of matrices in $S{{L}_{2}}\left( {{\text{F}}_{2}} \right)$ having not square root but we make group classification of roots distribution in which root always exists in splшtable group extension of $S{{L}_{2}}\left( {{\text{F}}_{p}} \right)$ by the same field viz it is in $ES{{L}_{2}}\left( {{\text{F}}_{p}} \right)$.
\end{proof}
...
\textbf{Proposition}.  If $B\in S{{L}_{2}}({{\mathbb{F}}_{p}})$ is root of equation ${{X}^{3}}=A$, then
...
%$$B=\frac{A+tr(\sqrt[3]{A})\sqrt[3]{\det (A)}}{tr{{\left( \sqrt{A} \right)}^{2}}-\sqrt[3]{\det (A)}},$$
 where $A\in S{{L}_{2}}({{\mathbb{F}}_{p}})$.

\begin{proof}
Proof.  If $\sqrt[3]{A}\in S{{L}_{2}}({{\mathbb{F}}_{p}})$ then we consider Cayley-Hamilton equation (C.H.E.)${{A}^{3}}-tr\left( A \right){{A}^{2}}+\left( {{\lambda }_{1}}{{\lambda }_{2}}+{{\lambda }_{1}}{{\lambda }_{3}}+{{\lambda }_{2}}{{\lambda }_{3}} \right)A-\det \left( A \right)=0$.
...
In that case from \eqref{2} we obtain $$A=B^3 =  - tr{B} \det{B} \cdot E = -(tr{B})^3 \cdot E.$$
...
The criterions of solvability of matrix equation $X^2=A$ over different linear groups with respect to matrix classification by its $tr(A)$ and type of space contracting is found and proved in this paper.
...
dedicated to the 70-th anniversary of A.A. Makhnev. July 9 - 15, 2023.
