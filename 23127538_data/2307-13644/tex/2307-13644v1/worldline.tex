...
Gorbunov:1998id, Kuzenko:1995mg, Bastianelli:2007pv, Bastianelli:2008nm, Bonezzi:2012cf, Bastianelli:2014lia}).
...
see e.g. \cite{Gomis:2006xw,Gomis:2006wu,Gomis:2007zz}
...
groups $O(n)$. Finally, we collect in Appendix \ref{app:tables}
...
\ee
where $\dot x^\mu := \tfrac{{\rm d} x^\mu}{{\rm d} t}$, and the einbein $e$ plays the r\^ole of a Lagrange multiplier which
...
to various different particles
...
Kirillov--Kostant--Souriau symplectic structure on
a coadjoint orbit. Then, we discuss several issues
arising in interpreting the coadjoint orbit action \eqref{MC action}
...
In other words, the coadjoint vectors with rescaled
parameters belong to the same coadjoint orbit.
As we shall review shortly below,
each coadjoint orbit is an even dimensional subspace 
of $\LieAlg^*$ with $G$-invariant symplectic form.


Various properties of a coadjoint orbit can be
captured by the quotient Lie algebra,
\be 
    \check{\mathfrak{g}}_\phi=\mathfrak{g}_\phi/
\hat{\mathfrak{g}}_\phi\,,
\label{check g}
\ee
where $\hat{\mathfrak{g}}_\phi$
is the ideal of $\mathfrak{g}_\phi$ satisfying
\be 
    \hat{\mathfrak{g}}_\phi=\{
    \xi\in \mathfrak{g}_\phi\,|\,
    \la\phi,\xi\ra=0\}\,.
\ee
Since $\hat{\mathfrak{g}}_\phi
=[\mathfrak{g}_\phi,
\mathfrak{g}_\phi]$,
the quotient $\check{\mathfrak{g}}_\phi$
is in fact Abelian.
For a nilpotent orbit, $\hat{\mathfrak{g}}_\phi=\mathfrak{g}_\phi$
and hence $\check{\mathfrak{g}}_\phi=\emptyset$.
A semisimple orbit has
non-trivial $\check{\mathfrak{g}}_\phi$
and it
is \emph{elliptic} if $\check{\mathfrak{g}}_\phi$
is compact, i.e.
its coadjoint representation has only purely imaginary eigenvalues.
...
$O_N$ or $Sp_N$ which can be compact or non-compact,
...
are regular, which is to say that their centralizers
...
and can be described as a surface in $\mathfrak{g}^*$
defined as the common level set of the functions
dual to the Casimir operators.\footnote{In the sense
that the space of polynomial functions on $\LieAlg^*$
is isomorphic to $S(\LieAlg)$, to which the Casimir
operators of $\LieAlg$ belong.} Hence, their dimension
...
\ee
The closeness of the second term,
...
belongs to the first de Rham cohomology group $H^1(\check G_\phi)$. 
Here, $\check G_\phi$ denotes the Lie group associated with
the Lie algebra  $\check{\mathfrak{g}}_\phi$
defined in \eqref{check g}.
Since $\la \phi, [\xi,\zeta]\ra=0$
for any $\xi, \zeta\in \mathfrak{g}_\phi$,
it is sufficient to consider $\check{\mathfrak{g}}_\phi$
instead of $\mathfrak{g}_\phi$.
The group $\check G_\phi$ is Abelian, and
we can parameterise an element $\t_{ij}\in \check G_\phi$ as
\be
	\t_{ij}=e^{\z^1_{ij}\,J_1}\,e^{\z^2_{ij}\,J_2}\cdots e^{\z^{k}_{ij}\,J_{k}}\,,
	\qquad k={\rm dim}\check G_\phi\,,
\ee
where $J_a$'s are the generators of  the Lie algebra $\check{\mathfrak{g}}_\phi$.
...
Clearly, the change of sections by a transition map $\t_{ij}$ can be interpreted as gauge transformation. 
...
and the degrees of freedom --- mathematically speaking
the Gelfand--Kirillov dimension --- is reduced from 1 to 0.
...
For example,
the spin degrees of freedom of the four dimensional spinning particle
...
is proportional to the symplectic form $\Omega$.
...
groups $SO(3)$, $SO(2,1)$, $ISO(2)$, $ISO(1,1)$
...
$\so(3)$ or $\so(2,1)$ by contracting $J_3$ generator,
whereas $\mathfrak{iso}(1,1)$ can be obtained
from $\so(2,1)$ by contracting $J_1$ generator.
...
$\check{\mathfrak{g}}_\phi=\mathfrak{g}_\phi = \uu(1)$
...
$\check{\mathfrak{g}}_\phi={\rm span}\{J_3\}\simeq
\mathfrak{u}(1)$.
...
$\check{\mathfrak{g}}_\phi={\rm span}\{J_1\}\simeq
\mathbb R$.
The geometry is a two-dimensional hyperbolic hyperboloid $H^2(-j^2)\cong SO^+(2,1)/SO^+(1,1)$.
The last case has the entire $SO_0(2,1)$ as its stabiliser and 
...
For $\mathfrak{iso}(2)^*$ case, any coadjoint vector $\phi$ can be
transformed into
\be
	\phi=\left\{ 
	\begin{array}{ccc}
...
cylinder $S^1 \times \R \cong ISO(2)/\R$
of radius $\sqrt{p^2}$.
...
(which is subdivided by four pieces of two half-planes) and a continuum of points on $j$-axis.
...
and can be described respectively by two-dimensional
...
\be
	...
	2\pi\,\mathbb Z & \qquad [SO(3)\ {\rm or}\ SO_0(2,1)]\\
...
\ee
The above quantity should be $2\pi\,n$ for an integer $n$ in order that the path integral is invariant under such a transformation,
...
\be
	...
	\end{array}\right..
\ee
The other $\mathfrak{iso}(1,1)^*$ obits with $p^2<0$ are isomorphic to the ones with $p^2>0$.
...
\ea
is a Lie algebra homomorphism from $(\mathfrak{g}, [\cdot,\cdot])$
...
\ee
is a co-isotropic submanifold of $\cM$:
...
satisfying $\pi^*\o=\i_\phi^*\Omega$ because the fiber $G_\phi$ 
is the isotropic complement of $\mu^{-1}(\phi)$
inside the symplectic manifold $\cM$\,. In other words,
...
is the physical phase space reduced by the action of the gauge symmetry generated by
...
\end{equation}
where $I \subset \R$ is an interval of the real line,
...
Note that   $y^\mu(t) \equiv y^\mu(\g(t))$ where $t \in I$
is the worldline parameter and $\g\,:\,I\to \cM$ is 
the worldline trajectory.
...
requires that the second term be proportional to $2\pi\,\hbar$ times an integer.
...
An important class of examples of the above discussion is 
the cotangent bundle $\cM=T^*G$ of a Lie group $G$,
which is a symplectic manifold as any cotangent
bundle: Locally, the symplectic form reads
\begin{equation}
...
\begin{equation}
	...
	- A^a\left(\rho_a{}^\mu(x)\,p_\mu - \phi_a\right)\,.
\end{equation}
The constraints
\begin{equation}
	\chi_a(x,p) = \rho_a{}^\mu(x)\,p_\mu - \phi_a\approx 0\,,
\end{equation}
...
\begin{equation}
 ...
 \label{g inv g act}
\end{equation}
Remark that the constraint surface 
is 
\be
 \mu^{-1}(\phi)
...
that the cotangent bundle of a Lie group is trivial,
$T^*G\cong G\times \mathfrak{g}^*$.
Further
quotienting by the gauge symmetry generated by
the first class constraints, which is given by
the action of the isotropy group $G_\phi$,
leads to
\begin{equation}
    \cN_\phi= \mu^{-1}(\phi)/G_\phi
    =\{(g,{\rm Ad}_g^*\phi)\in T^*G\,|\,
    g\in G/G_\phi \}\cong \cO^G_\phi\,,
\end{equation}
...
$\pi_\phi: G \twoheadrightarrow \cO^G_\phi: g \mapsto {\rm Ad}_g^*\phi$,
...
to $b_{\sst (N)}=\eta^{\sst (p,N-p)}$ with a $p$
where $\eta^{\sst (p,N-p)}$ is
...
$B(\O_{\sst(N)},\mathbb H)$
 ...
 because here the $\mathbf{i}$,
 the first element of $\mathbb H$,
...
is necessary for the $2(2N)^2$ and $2(4N)^2$ dimensional
...
and this result in a constant factor. Discarding this factor, 
...
\be 
    ...
    -\tilde\phi_{\alpha\beta}\right)\,.
 	\label{O action}
\ee 
The constraints are given through
...
\ee 
Here, it is worth to emphasize that the $\tilde\phi^\a{}_\b$
is determined by $\phi^a{}_b$,
up to a $Sp(2M,\R)$ transformation.
The choice of a coadjoint element $\phi^a{}_b$ itself is fixed up to a $O(p,N-p)$ transformation.
...
The coadjoint orbits of inhomogeneous orthogonal algebra can be classified as follows.
Let $P_a$ and $J_{ab}$ the generators of the Lie algebra
and $\cP^a$ and $\cJ^{ab}$ their duals. A coadjoint  vector is given by $\phi=\phi_{I\,a}\,\cP^a+
\phi_{H\,ab}\,\cJ^{ab}$.
If a coadjoint orbit has $\phi_I=0$, then it reduces to that of the Lorentz subgroup, 
which might be interpreted as the dS group of one lower dimensions.
...
coadjoint orbits of the little group algebra.


Let us apply the general method to the 
inhomogeneous Lorentz group, i.e. the Poincar\'e group. 
We take $H$ as the orthogonal group $O(p,N-p)$ and $I$ as the translation group, 
...
\ba
	...
	+ \phi_H{}^{ab}\,\Sigma_{bc}\,{\rm d}\Sigma^c{}_a
...
then skew-diagonalize and normalize $\phi_H$ as $\phi_H{}^{ab}=T^{a}{}_\a\,T^{b}{}_\b\,\Omega^{\a\b}$.
Here again, $\a, \b =1,\ldots, 2M$ where $2M$ is the rank of $\phi_H$. 
Since $\phi_H\in \mathfrak{h}_{\phi_I}^*$, we can do it without touching the vector $\phi_I$\,:\, $\phi_{I\,b}\,T^b{}_a\,=\phi_{I\,a}$\,. Finally, 
by substituting
$(B,B^a,B^{ab})$ with
$(A,A^a,A^{ab})$ again, we 
reduce the action as 
\ba
	S[x, p, \Sigma, A] & = & \int \Big[\,p_a\,{\rm d}x^b
	+ \Omega^{\a\b}\,\Sigma_{c\b}\,{\rm d}\Sigma^c{}_\a
	+ A\,(p_a\,p^a-\phi_{I\,a}\,\phi_I{}^a) \nn 
	&& \hspace{40pt} +\,A^\a\,(p_a\,\Sigma^a{}_\a-\phi_{I\,\a})
	+ A^{\a\b}\,(\Sigma_{c\b}\,\Sigma^c{}_\a
    -\tilde\phi_{H\,\b\a})\Big]\,,
	\label{inhomo Lorentz}
\ea
where $p_a=\Sigma_a{}^b\,\phi_I{}_b$
and $\tilde\phi_{H\,\a\b}=T^a{}_\a\,T^b{}_\b\,\eta_{ab}$\,.
...
\ee
and $\mu$ is the center. This Lie algebra is isomorphic to
$\heis_{2N}\niplus\mathfrak{sp}(2N,\mathbb R)$
with dimension $(N+1)(2N+1)$,
the semi-direct sum of the Heisenberg and the symplectic 
algebra.
...
\ba
	...
	+A\,(p_a\,p^a-\phi_{I\,a}\,\phi_I{}^a)\nn 
	&&\hspace{45pt}	+\,A^\a\,(p_a\,\S^a{}_\a-\phi_{I\,\a})
...
is given by
\be
	\tilde\phi = \phi_I{}^2\,\cT + \phi_{I\,\alpha}\,\cM^\alpha
...
\be
    ...
    +\l^{\a\b}\,(\S_{a\a}\,\S^{a}{}_\b-{\tilde\phi}_{H\b\a})\,,
\ee
...
\ba
	...
	-\frac{e}2 \left(\phi_I{}^2-\phi_{I\a}\,\D^{\a\b}\,\phi_{I\b}\right)
	\nn
	&&+\,
	\S_{a\b}\left(\O^{\a\b}\,\dot\S^a{}_\a+\dot x^a\,\D^{\a\b}\,\phi_{I\b}\right)
	+\l^{\a\b}\,(\S_{a\a}\,\S^{a}{}_\b-{\tilde\phi}_{H\b\a})\,.
\ea
In the case where $\lambda^\alpha$ cannot be solved,
it is associated with a first class constraint.
Finally, when $\phi_I{}^2-\phi_{I\a}\,\D^{\a\b}\,\phi_{I\b}\neq0$,
we can solve $e$ out to find the Nambu-type action,
\ba
	L\eq -\sqrt{\left(\phi_I{}^2-\phi_{I\a}\,\D^{\a\b}\,\phi_{I\b}\right)     \left(\dot x^a{}^2
	-\dot x^a\,\S_{a\a}\,\D^{\a\b}\,\S_{b\b}\,\dot x^b\right)}
	\nn
	&&+\,
	\S_{a\b}\left(\O^{\a\b}\,\dot\S^a{}_\a+\dot x^a\,\D^{\a\b}\,\phi_{I\b}\right)
	+\l^{\a\b}\,(\S_{a\a}\,\S^{a}{}_\b-{\tilde\phi}_{H\b\a})\,.	
\ea
Even though the above action contains all the parameters $\phi_{I}{}^2, \phi_{I\a}$ and ${\tilde\phi}_{H\a\b}$,
the dependence on $\phi_{I\a}$ is in fact irrelevant because $\phi_{I\a}$ are non-trivial
...
and the action gets simplified as the familiar form,
\be
...
The dual coadjoint orbit is given by ${\tilde\phi}=\phi_I{}^2\,\cT$
and the stabiliser is the dual algebra itself
$\tilde{\LieAlg}_{\tilde\phi}=\tilde{\LieAlg} = \R$.
Depending on the signature of the vector $\phi_{I\,a}$,
...
we have $\dim\,\cM = 2d$ (here $\cM=T^*\R^d$), $\dim\,{\cO}^{\tilde G}_{\tilde\phi}=0$ and $\dim\,\tilde{\LieAlg}_{\tilde\phi}=1$,
...
\ba
	...
	+ \pi \cdot {\rm d}\chi + A\,(p^2-\phi_{I}^2)
	+ A^\chi\,(p\cdot \chi-\phi_{I\chi})
	+ A^\pi\,(p\cdot \pi-\phi_{I\pi}) \nn 
	&&\hspace{45pt}	
...
\be
	{\tilde\phi}=\phi_I{}^2\ \cT+
	\phi_{I\,\chi}\,\cM^\chi +\phi_{I\,\pi}\,\cM^\pi+ 
...
contributing again only some discrete labels. In the following,
we present the classification of coadjoint orbits with $M=1$.

\begin{itemize}
\item 
A massive spinning particle corresponds to the coadjoint orbit
$\cO^G_\phi$ with representative and stabiliser given by
\begin{equation}
	\phi = m\,\cP^0 + s\,\cJ^{12}\,,
    \qquad 
    \LieAlg_\phi = \R_{P_0} \oplus \uu(1)_{J_{12}}
    \oplus \so(d-3)_{[3, \dots, d-1]}\,.
\end{equation}
The coadjoint orbit $\cO^G_\phi \simeq \frac{IO(1,d-1)}{\R \times 
O(2)\times O(d-3)}$
can be viewed as a fiber bundle
with the massive scalar coadjoint orbit $\frac{IO(1,d-1)}{{\mathbb R}\times O(d-1)}$ (which is the cotangent bundle of the momentum orbit)
as the base space and 
the spin-orbit $\frac{O(d-1)}{O(2)\times O(d-3)}$ as the fiber.
The latter, the real Grassmanian ${\rm Gr}_{\mathbb R}(2,d-1)$, is a compact manifold,
and hence contributes only discrete degrees of freedom, when quantised.
...
\begin{equation}
    ...
    + s^2\,\cS^{\pi\pi}\,,
    \qquad 
    \tilde{\LieAlg}_{\tilde\phi} =\R_{T} \oplus \uu(1)_{S_{\pi\pi}+s^2\, S_{\chi\chi}}\,.
\end{equation}
See \cite{Lyakhovich:1998ij}
for the derivation of a related worldline
action for a massive spinning particle in flat spacetime.
\item
...
but only the combination $\varepsilon=E\,\epsilon$ is.
Hence, the particle action involves only one parameter $\varepsilon$. 
...
are $\varepsilon\,\cJ^{-1}$ and $\cS^{\pi\pi}$ are null-vectors,
...
\begin{equation}
    ...
    \qquad
    \LieAlg_\phi = \R_{P_{d-1}} \oplus \uu(1)_{J_{12}}
...
\end{equation}
% is generated by $T$ and $S_{\pi\pi}+s^2\,S_{\chi\chi}$.
...
\begin{equation}
	...
	\label{Mink st}
    \qquad 
    \LieAlg_\phi = \R_{P_{d-1}} \oplus \R_{J_{02}}
    \oplus \so(d-3)_{[2, \dots, d-2]}\,.
\end{equation}
...
\ea
Massive, massless and tachyonic particles of spin $s$ are 
described by $\CasimirSpin=s^2$ and positive, zero and negative values of $\CasimirMass$, respectively.
The tachyonic particles of time-like and light-like spins
are described by a negative and zero $\CasimirSpin$, respectively, and a negative $\CasimirMass$.
...
The indices $A, B$ take values $0,1,\ldots, d-1$
and $\bullet$ ($\bullet=d$ for dS
and $\bullet=0'$ for AdS).
The metric $\eta_{AB}$ is ${\rm diag}(-1,+1,\ldots,+1,\s)$
...
\ba
	...
	\label{Ham act AdS}
\ea
where $X^a$ and $P_a$ will play the role
...
are associated with the dual algebra, 
\begin{equation}
	\tilde{\mathfrak{g}} = \mathfrak{sp}(2(M+1),\mathbb R)\,,
\end{equation}
generated by
\begin{subequations}
\begin{equation}
...
Similarly to the Minkowski case, we can integrate out $P_a$
...
of (A)dS algebra using the notation of transvection, 
\be
	P_a=\ell^{-1}\,J_{\bullet a}\,,
	\qquad 
	[P_a,P_b]=\s\,\ell^{-2}\,J_{ab}\,,
    \qquad
    [a,b=0,1,\dots,d-1]\,,
\ee
and its dual $\cP^a=\ell\,\cJ^{\bullet a}$\,,
in order to make the analogy with the Minkowski case manifest. From now on, we set $\ell=1$ for simplicity.

As we shall see below,
many cases can be viewed as the (A)dS counterparts of the Minkowski particles.
However, there do exist additional cases which do not have  Minkowski counterparts.
In fact, we find even more cases in AdS than dS.
For explicit characterisation of (A)dS
coadjoint orbits, see e.g. 
\cite{Kosinski:2022qrf,Enayati:2023lld}.
...
\begin{equation}
    \phi = m\,\cP^0\,,
    \qquad
    \LieAlg_\phi = \mathfrak{i}(-\sigma)_{P_0}
...
\begin{equation}
    \phi = E\,\cP^+\,,
    \qquad 
    \LieAlg_\phi = \R_{P_-} \oplus \iso(d-2)_{[-;1,\dots,d-2]}\,.
\end{equation}
...
\begin{equation}
    \phi=\mu\,\cP^1\,,
    \qquad 
    \LieAlg_\phi = \mathfrak{i}(\sigma)_{P_1}
...
$({\tilde\phi}_{PP},{\tilde\phi}_{XX},{\tilde\phi}_{XP})=(0,1,m)$ or $(0,-1,\mu)$.
 ...
 However, if we insist it naively, they could be given by the complex vectors 
$({\tilde\phi}_{PP},{\tilde\phi}_{XX},{\tilde\phi}_{XP})=(0,-1,i\,m)$ or $(0,1,i\,\mu)$\,.
This seemingly ill-defined choice of coadjoint vector
makes a good sense after quantisation: since $P_{\amb A}=-i\,\hbar\,\partial/\partial X^{\amb A}$,
...
$T-\s\,m^2\,U$ and $s^2\,S_{\chi\chi}+S_{\pi\pi}$.
This case can be interpreted as a massive spinning particle.
See \cite{Kuzenko:1994vh, Kuzenko:1995aq}
for the derivation of a
related worldline action of 
massive spinning particle in AdS.
Note also 
that the above set of constraints is the same as the ones
identified and used in the treatment of massive
and (partially-)massless mixed-symmetry fields in AdS$_d$ using BRST techniques and the ambient space
approach \cite{Alkalaev:2009vm, Alkalaev:2011zv}.
...
\begin{equation}
    ...
    \oplus \so(d-3)_{[3,\dots,d-1]}\,,
    \qquad 
    \tilde{\LieAlg}_{\tilde\phi} = \mathfrak{u}(1,1)\,,
\end{equation}
...
\oplus (\cD^\pm_{j_{\rm\sst L}}\otimes \cD^\pm_{j_{\rm\sst L}})$.
At the level of representations, we find the decomposition,
\be 
	\cD^\pm_{-\frac t2}=\cD_{\frac t2}\oplus \cD^\pm_{\frac t2+1}\,,
\ee
when $2j$ becomes a non-positive integer $-t$.
Here, $\cD_{\frac t2}$ is the $(2t+1)$-dimensional representation.
And, for $m=s-t$, the quotient representations 
\be
	(\cD^{\pm}_{s-\frac t2}\otimes \cD_{\frac t2})\oplus (\cD_{\frac t2}
	\oplus \cD^{\pm}_{s-\frac t2})\,,
\ee
correspond to partially massless representations of depth $t+1$,
which are non-unitary. 
Here, non-unitary is due to the finite representation $\cD_{t}$ of $\so(1,2)$ algebra.
Therefore, one might confirm once again that the orbits with $m=1,\ldots,s-1$ lead
to non-unitary representations.
However, this is not correct because the non-unitary representation $\cD_{\frac t2}$
should arise from  $S^2$,
while the orbits with $m=s-t$ is given by
the product space,
\be 
    \left[H^2_\pm\big((s-\tfrac t2)^2\big)\times H^2_\mp\big((\tfrac {t}2)^2\big)\right]\cup \left[H^2_\mp\big((\tfrac t2)^2
    \big)\times H^2_\pm\big((s-\tfrac t2)^2\big)\right].
\ee 
This coadjoint orbit would correspond to the \emph{unitary} representation,
\be
	(\cD^{\pm}_{s-\frac t2}\otimes \cD^{\mp}_{\frac t2})\oplus (\cD^{\mp}_{\frac t2}
	\oplus \cD^{\pm}_{s-\frac t2})\,,
\ee
whose particle interpretation is unclear
for the moment.
In Section \ref{sec: BdS}, we propose an interpretation for
this type of orbits.
...
\begin{equation}
    \phi = E\,\cP^+ + s\,\cJ^{12}\,,
    \qquad 
	\LieAlg_\phi = \R_{P_-} \oplus \uu(1)_{J_{12}}
    \oplus \iso(d-4)_{[-;3,\dots,d-2]}\,,
\end{equation}
...
$\tilde{\LieAlg}_{\tilde\phi} = \R \oplus \uu(1)$
...
From the analogy to the Minkowski case,
...
\end{equation}
Note here that only
when the components $\bullet$ 
and 1 have the same signature,
that is only in dS,
we can perform a $\pi/2$-rotation
in the $\bullet$--$1$ plane to get
\ba
    \phi\eq \frac\ve{\sqrt{2}}
    \left(
    \cJ^{\bullet 0}+\cJ^{1\,d-1}\right)\nn
    &\simeq&
    \frac\ve{\sqrt{2}}
    \left(\cP^0
    +\cJ^{12}\right),
...
with the coordinate 2 by a rotation
to get a canonical form.
In AdS, this cannot be done,
so the coadjoint orbit 
given by \eqref{conti spin AdS}
...
\begin{equation}
	\LieAlg_\phi = \uu(1)_{P_0+J_{1\,d-1}}
    \oplus \R_{P_{d-1} + J_{01}}
    \oplus \so(d-3)_{[2,\dots,d-2]}\,,
\end{equation}
and with the indices $\chi=1, \pi=-$, we find
...
\end{equation} 
generated by $T-\,S_{\p\p}+2\,\ve^2\, N_\c$
and $2\,M_{\c}+\ve^2\,(U-S_{\c\c})$.
Note that the stabiliser
contains $\mathfrak{u}(1)$
...
\end{equation}
and taking the non-trivial indices as $\chi=0$ and $\pi=1$,
we find for the dual orbit
\begin{equation}
	\tilde\phi = \mu^2\,\cT - \cU + \cS^{\chi\chi}
    - \nu^2\,\cS^{\pi\pi}\,.
\end{equation}
Again, the above case corresponds to a new case only in AdS,
because it is the same as the massive spinning case with $m=\nu$
and $s=\mu$ in dS. The stabiliser is given by
\begin{equation}
	\tilde{\LieAlg}_{\tilde\phi}
    = \R_{T - \mu^2\,U}
...
of the stabilisers,
\begin{equation}
	\LieAlg_\phi = \mathfrak{gl}(2,\R)
    \oplus \so(d-3)_{[2,\dots,d-2]}\,,
    \qquad 
    \tilde{\LieAlg}_{\tilde\phi} = \mathfrak{gl}(2,\R)\,,
\end{equation}
...
\end{equation}
respectively. This exotic case can be better understood
...
and space-like spins. The stabiliser is
\begin{equation}
	\LieAlg_\phi = \R_{P_{d-1}} \oplus \R_{J_{+2}}
    \oplus \iso(d-4)_{[+;3,\dots,d-2]}\,,
\end{equation}
...
\begin{equation}
	\tilde\phi = \mu^2\,\cT - \cU + \cS^{\chi\chi}\,,
    \qquad 
	\tilde{\LieAlg}_{\tilde\phi} = \R_{T - \mu^2\,U}
    \oplus \R_{S_{\pi\pi}}\,.
\end{equation}
This is the anti-de Sitter analogue of the tachyonic particle
with light-like spin \eqref{Mink sl} in Minkowski.
\end{itemize}
...
For $\CasimirMass=0$, the gauge symmetry is enhanced
for $\CasimirSpin>0$ in both AdS and dS
...
We can add to the original representative vector $\phi$
...
because taking other spins from the latter part
will be interpreted as mixed symmetry ones.
It turns out the dS short tachyon
(or equivalently the massless particle) becomes
...
but the ones equivalent to non-short coadjoint orbits
which we already considered. Similarly, taking 
the `spin' vector from the center will end up
changing the label of the short orbits.
...
with stabiliser,
\begin{equation}
    \tilde\LieAlg_{\tilde\phi} = \mathfrak{u}(1) \oplus \R\,,
\end{equation}
generated by 
\begin{equation}
    \begin{aligned}
...
the corresponding particle could be interpreted either massive ($s>\n$),
massless ($s=\n$) or tachyonic ($s<\n$),
...
as massive, massless and tachyonic particles of continuous spin.
...
\ba 
    \tilde\phi\eq -\cU-\cN^\c-4\,s^2\,(\cS^{\p\p}
    +\cM^\p)\,,\nn 
    \tilde{\mathfrak{g}}_{\tilde\phi}\eq 
    \R_{M_\p +4\,s^2\, N_\c}\oplus
    \uu(1)_{T+4\,s^2\, S_{\c\c}}\,.
\ea 
...
has the dual orbit given by
\ba 
    \tilde\phi\eq -\cU-\cN^\c+4\,\nu^2\,(\cS^{\p\p}
    +\cM^\p)\,,\nn 
    \tilde{\mathfrak{g}}_{\tilde\phi}\eq \R_{M_\p -4\,\nu^2\, N_\c}\oplus \R_{T-4\,\nu^2 \,S_{\c\c}}\,.
\ea 
...
\cite{Metsaev:2016lhs} (see also \cite{Metsaev:2017cuz, Metsaev:2017ytk, Metsaev:2018moa, Metsaev:2019opn, Metsaev:2021zdg} for further developments).
...
This means that the corresponding particle leaves in a spacetime with two temporal directions.
...
\end{equation}
For $m=s$,
the stabiliser and the dual stabiliser are enhanced to 
\begin{equation}
	\mathfrak{g}_\phi = \uu(2)
    \oplus \so(2,d-5)_{[0',0,5,\dots,d-1]}\,,
    \qquad \text{and} \qquad
...
\end{equation}
respectively.
The orbit of massless space-like spin particle
in BdS is determined by the coadjoint vector,
\begin{equation}
    \phi = E\,\cJ^{1+} + s\,\cJ^{23}\,,
    \qquad
    \LieAlg_\phi =\R_{J_{-1}}
...
\be 
 ...
 \mathbb R_{J_{1-}-J_{2-'}}
 \oplus 
    \mathfrak{so}(2)_{J_{12}-2J_{-+'}-2J_{+-'}}\inplus 
    \mathfrak{heis}_{2(d-4)}
    \niplus 
    \mathfrak{so}(d-5)_{[3,\ldots,d-3]}\,,
     \ee
where $\mathfrak{heis}_{2(d-4)}$
is generated by
\be
    J_{1-}+J_{2-'},\quad 
    J_{1-'}+J_{2-},\quad 
    ...
    \quad J_{--'}\,.
\ee
Here, we take the non-trivial indices as $\c = 2$, $\p = +$ to find 
the dual coadjoint orbit is given by
\be
	\tilde\phi =\cU+\cS^{\c\c}\,,
    \qquad 
...
\begin{equation}
 ...
 +\cU\,,
	...
	\tilde{\LieAlg}_{\tilde\phi}
	= \uu(1)_{U+m^2\,T}
    \oplus \sp(2,\R)_{S_{\p\p}, S_{\p\c}, S_{\c\c}}\,.
\end{equation}
...
because $E$ and $\epsilon$
can be rescaled independently. 
...
\begin{equation}
    ...
    = \sp(2,\R)_{T,U,V}\,.
\end{equation}
Here, we take $X^a{}_+=X^a$ and $X^a{}_{+'}=P^a$
to find the action
\be
...
The dual stabilisers $\tilde{\LieAlg}_{\tilde\phi}$
...
\end{equation}
where $m \in \N$
and $s_1 \ge s_2 \ge \cdots \ge s_M$\,.
 $M \leq [\tfrac{d-1}2]$ is the integer part
of half of the dimension minus one.
...
\begin{equation}
    \LieAlg_\phi = \uu(h_1) \oplus \cdots
...
with 
\be 
    \LieAlg_\phi = \R \oplus \uu(m_1)
    \oplus \cdots \oplus \uu(m_p) \oplus \iso(d-2-2M)\,,
\ee
...
\be 
    \mathfrak{g}_\phi = \R \oplus \uu(m_1)
    \oplus \cdots \oplus \uu(m_p) \oplus \so(d-1-2M)\,.
\ee 
...
\eqref{mixed T}
with $m \in \N$ contain (partially-)massless spinning particles
 of mixed symmetry.
...
a box from the \emph{last} block. The mass parameter
...
with $\phi=s(\cP^0+\cJ^{12}+\cdots+\cJ^{D-2\,D-1})$\,.
...
some of the well-known results of the inclusion 
...
$\cP^+$ can be scaled down to $0$ under the infinite boost along $\pm$ direction in dS and Poincar\'e case.
...
to the nilpotent coadjoint orbits:
its representative vector can be obtained 
from that of a nilpotent coadjoint orbit
by adding a representative vector of a 
semisimple coadjoint orbit of the stabiliser algebra.
...
\ee 
with an inclusion structure, say,
\be 
    \cO_{\phi^{(0)}_{\rm N}}
    \subset \bar\cO_{\phi^{ (1)}_{\rm N}}
    \subset \cdots \subset
    \bar \cO_{\phi^{(m)}_{\rm N}}\,.
    \label{simple is}
\ee 
...
\be 
    \cO_{\phi^{(0)}_{\rm S}}
    \subset \bar\cO_{\phi^{ (1)}_{\rm S}}
    \subset \cdots \subset
    \bar \cO_{\phi^{(m)}_{\rm S}}\,,
\ee 
...
representation, so would become an interior point
of massive/tachyonic spectrum (see figure \ref{fig:spinning_AdS}).
...
It is intriguing that in dS, a massive spin $s$ particle
with a specially tuned mass would contain a tachyonic scalar
in its boundary of phase space. It is also intriguing
that in AdS,  a tachyonic spin $s$  contains a BdS scalar,
though we have already seen that in AdS tachyonic particles
can live both AdS and BdS.

One can also think about whether the orbit of a spinning
conformal particle can be included in a larger orbit.
The former is given by
$\phi=s\,(\cP^0+\cJ^{12}+\cdots+\cJ^{d-2\,d-1})$
for $\so(2,d-1)$ with odd $d$. The simplest case of $d=3$
leads to the orbit of $\phi=s\,(\cP^0+\cJ^{12})
+ \epsilon\,(\cP^0+\cP^1-\cJ^{12}-\cJ^{02})$ 
which has been treated as one of the cases
where mass and spin are entangled.
The physical interpretation of this limit is not clear
as the particle corresponding to the larger orbit
is an exotic one, though it still can be defined in AdS.
...
such as spins are quantised.
%
Focusing on the classical Lie group, we reformulate
the coadjoint orbit actions into constrained Hamiltonian ones, 
where the definition of the group give rise to a mixture
...
manifolds $\cP_1$ and $\cP_2$ commute with one another in $\cM$.
For more details, the interested reader may consult
\cite[Chap. 9]{CannasDaSilva1999} or \cite[Chap. 11]{Ortega2013}.}
...
\begin{equation}
    \begin{tikzcd}
        & (\cM,\Omega) \ar[dl, swap, "\mu"] \ar[dr, "\tilde\mu"] & \\
        \LieAlg^* && \tilde{\LieAlg}^*
    \end{tikzcd}
\end{equation}
...
is a single
$\tilde G$-orbit, and vice-versa. Such actions are called
`mutually transitive' \cite{Skerritt2019}, and establish a 
one-to-one correspondence between $G$-orbits 
in $\mu(\cM)$
and  $\tilde G$-orbits in $\tilde\mu(\cM)$.
More precisely,
the reduced phase space at $\phi\in\LieAlg^*$
is symplectomorphic to a coadjoint orbit of $\tilde G$,
\begin{equation}
    \mu^{-1}(\phi)/G_\phi \cong \cO^{\tilde G}_{\tilde\phi}\,,
\end{equation}
where the representative $\tilde\phi \in \tilde\LieAlg^*$
...
dual pairs, wherein one considers the cotangent bundle 
$T^*Mat_{N \times M}(\mathbb F) \cong \mathbb F^{2(N \times M)}$
...
of reductive groups $G$ and $\tilde G$ (as suggested by the examples
worked out in \cite{Skerritt2019}, which seem to fall
in the class of models considered in this paper).

This correspondence is
the classical analog of the
reductive dual
pair correspondence \cite{Howe1989i, Howe1989ii},
which consists in a bijection between irreducible
representations of two subgroups of $Sp(2n,\R)$,
say $G$ and $\tilde G$, appearing in the oscillator
representation
(i.e. the metaplectic representation) of $Sp(2n,\R)$. This 
representation is known to arise as the quantisation
of the minimal nilpotent orbit of $Sp(2n,\R)$,
which is simply the flat symplectic manifold
$\R^{2n}\backslash\{0\}/\Z_2$ \cite{Brylinski1998}.
Therefore, the irreps
...
the manifest covariance is realised in the ambient space coordinates.
...
can be interpreted in either AdS or BdS particles, and hence
...
spin particles, which is even part of a two-parameter 
...
the path integral, but once it is done we expect that the constrained Hamiltonian system
can be easily quantisable because all the constraints are quadratic. As mentioned earlier, 
...
a few important issues to be clarified.
One important such issue is how the degrees of freedom associated to the spin are projected to a finite dimensional space. As we discussed with the example of $SU(2)$, this can happen for a compact coadjoint orbit, but understanding the same mechanism for a non-compact orbit will be an important task.
In certain cases, such as massive spinning orbits where the coadjoint orbits have a bundle structure where the fiber is a compact coadjoint orbit, it is easy to understand 
the mechanism.
However, in other cases, such as massless coadjoint orbits, it is not easy to grasp
the precise mechanism. Especially in the dS partially massless cases, the ``spin orbits'' are non-compact while we still expect the projection to takes place.

Another important issue is  how the classical equations of motion for
the first-quantised fields arise
from the Hamiltonian constraints
that we derived.
We already see that the constraints ought to be associated with a set of equations 
similar to that of Fierz equations.
By properly quantising the system with BRST symmetry, we may get
a gauge invariant equation
from the BRST charge (see  e.g. \cite{Alkalaev:2009vm, Alkalaev:2011zv}
and references therein).
When the spin degrees of freedom are to be projected, we should be able to get the equations which define tensor fields of a finite rank.


Finally, we would like to remark about the unitary irreducible representations which would arise by quantising the coadjoint orbits.
We have seen that some labels need to be quantised for a well-defined path integral.
These labels will get some $d$-dependent constant shift
due to the ordering of
the quantised variables:
e.g. the quadratic Casimir of $SU(2)$ gets shift from $s^2$ to $s(s+1)$.
...
The latter arises
in the excluded interval 
bounded by the end point of  a continuous family of orbits (such as massive dS and tachyonic AdS).
The representative vector of this end point orbit contains a nilpotent part,
and it will be tempting to obtain
the complementary series representations
by quantising this end point orbits
with non-trivial deformation parameters.
...
%
E.J. is grateful to 
Karapet Mkrtchyan and Junggi Yoon for discussions
...
\begin{itemize}
    ...
    subject to
    \begin{equation}
        [J_{\amb{AB}}, J_{\amb{CD}}] = \eta_{\amb{BC}}\,J_{\amb{AD}}
        - \eta_{\amb{AC}}\,J_{\amb{BD}} - \eta_{\amb{BD}}\,J_{\amb{AC}}
        + \eta_{\amb{AD}}\,J_{\amb{BC}}\,,
    \end{equation}
...
where $\mathsf t_i$ denotes a basis of $\mathfrak g$.
    ...
    \item We often omit the word ``coadjoint'' and simply say 
    ``orbit'' to refer to a coadjoint orbit.
    \item By the notation $\bar\cO$, we
    mean the Zariski closure of an orbit $\cO$.
...
first class ones, based on the `shifting trick' used
in symplectic reduction \cite{Ortega2013, Dwivedi2019}.

First, let us point out that the dimension of the reduce
phase space, given in \eqref{dim count}, can be also expressed as
\be
	\dim \cN_\phi=\dim\cM+\dim\cO^G_\phi-2\,\dim\mathfrak{g}\,,
\ee
...
to $\cM \times \cO^G_\phi$ with the symplectic structure $\O(y) - \o(\varphi)$
with the potential
\be
...
\ee
The Poisson bracket on $\cO^G_\phi$ can be obtained from the symplectic structure $\o$,
but also deduced directly from that of $\mathfrak{g}^*$.
The entire coadjoint space $\mathfrak{g}^*$ 
is not a symplectic manifold but is  endowed only
with a Poisson structure: for any two functions $f, g$ on $\mathfrak{g}^*$,
\begin{equation}
...
Pullback of the above by the inclusion $\i_{\phi}\,:\,\cO^G_\phi\hookrightarrow \mathfrak{g}^*$\footnote{Note that the inclusion of $\cO^G_\phi$ in $\mathfrak g^*$ is a moment map for the coadjoint action of $G$.} 
...
we find the constraints $\chi_a(y,\varphi)$ is of the first class as $\{\chi_a,\chi_b\}_{\cM\times \cO^G_\phi}\approx 0$.
The particle action
...
to first class ones, in the class of constrained Hamiltonian system 
discussed here, is an application of what is known as
...
The representative coadjoint vector can be written as 
$\phi =\phi_{ab}\,\cJ^{ab}$\,.
...
\be
    ...
    M := h_1 + \dots + h_p\,.
\ee
The derived algebra $\hat \LieAlg_\phi$ in this case is 
\begin{equation}
    \hat \LieAlg_\phi =  [\LieAlg_\phi, \LieAlg_\phi]
    = \su(h_1) \oplus \dots \su(h_p) \oplus \so(n-2M)\,,
\end{equation}
and therefore
\begin{equation}
    \check\LieAlg_\phi = \underbrace{\uu(1)
...
of $SO(2,1)$ orbits, for which we have a clear picture
of their geometry.
...
\ee
In Figure \ref{tab:so(2,2)},  we have collected
