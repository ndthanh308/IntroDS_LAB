...
%\tableofcontents
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Introduction\footnote{The opinions expressed in this research are those of the authors and are not meant to represent the opinions or official positions of BPCE S.A.}}
...
\end{enumerate}
Then there exists a unique $(\Cc_{t},\Yy_{t})$ satisfying~\eqref{eq:Y_it_C_it}.
...
\begin{align} \label{eq main technical ass}
    \rho := \max_{1 \le n \le N} \Big\{\mathfrak{a}^{n\cdot} \overline \mu+ \frac12 \sigma_{\bb_n}^2 + \frac{\varepsilon^2}2 | \mathfrak{a}^{n\cdot} |^2 | \sqrt{\Sigma}|^2(1-|\Gamma|)^{-2}\Big\} < r.
\end{align}
...
\begin{align}\label{eq the case of b}
    ...
    = \exp \left( \frac{p^2}2  s \sigma_{\bb_n}^2\right),
\end{align}
we compute 
\begin{equation}
    \EE\left[ \exp \left( 
