...
\[X_{ij}\equiv x_i^{j-1}\; (\bmod\; p).\]

\noindent
By~\eqref{eq:det_vand}
\[\det(X)\equiv \prod_{1\leq i<j\leq n}(x_j-x_i)\quad (\bmod\; p).\]
...
\begin{lemma}
\label{lem:key}
    Let $p$ be a prime, and let $x_1, x_2,\dots, x_n$ be $n$ distinct integers less than $p$. For the $n\times n$ matrix $X$, with $0\leq X_{ij}<p$ and $X_{ij}\equiv x_i^{j-1}\; (\bmod\; p)$, and a column vector  $\vectorbold{y}\in\mathbb{R}^n$,  
