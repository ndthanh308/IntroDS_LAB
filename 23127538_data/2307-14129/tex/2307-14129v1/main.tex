...
\usepackage{geometry}
\geometry{top=3.2cm, bottom=3.2cm, left=2.5cm, right=2.5cm}
\usepackage{amsfonts}
\usepackage{hyperref}
\usepackage{amsmath}
\usepackage{xcolor}
...
\author[1,2]{Ivan Guo}
\author[1]{Shijia Jin}
\author[1,2]{Kihun Nam}

\affil[1]{\small School of Mathematical Sciences, Monash University, Melbourne, Australia}
\affil[2]{\small Centre for Quantitative Finance and Investment Strategies, Monash University, Australia\thanks{\textbf{Acknowledgements\quad}This work was partially supported by the Australian Research Council (Grant DP220103106) and CSIRO Data61 Risklab.}}
...
\begin{gather*}
    ...
    \mathbb{H}^p:=\Big\{X=(X_t)_{0\leq t\leq T}: X \text{ is } \mathbb{F} \text{-progressively measurable and } \mathbb{E}\Big[\big(\int_0^T |X_t|^2\,dt\big)^{p/2}\Big]<\infty\, \Big\};\\
    \mathbb{S}^p:=\Big\{X\in \mathbb{H}^p: \mathbb{E}\big[\sup_{0\leq t\leq T} |X_t|^p\big]<\infty \Big\};\\
    \mathcal{M}:= \big\{ M = (M_t)_{0 \leq t\leq T}: M_t \in L^2(\Omega, \mathcal{F}_t) \text{ for a.e. } t \in [0, T] \text{ and } \{M_t, \mathcal{F}_t\}_{0\leq t\leq T}\text{ is a martingale} \big\};\\
    \mathbb{M}:= \big\{ M \in \mathcal{M}: \mathbb{E} [\langle M\rangle _T]<\infty \big\},
\end{gather*}
where the notation $\langle M\rangle_T$ denotes the quadratic variation of $M$ over the interval $[0, T]$. These spaces are equipped with the norm $\|\cdot\|_{L^p(\Omega, \mathcal{G})}$, $\|\cdot\|_{\mathbb{H}^p}$, $\|\cdot\|_{\mathbb{S}^p}$ and $\|\cdot\|_{\mathbb{M}}$ defined naturally.\\
...
\begin{remark}
The first assumption is common in optimal execution and market making literatures, where the mid-price $S$ has usually no effect on the optimization problem. In this case, the emphasis is placed on how the market maker deals with the order flows. The last condition on $\Sigma$ is added for technical reasons.
\end{remark}

\vspace{0.2cm}


A market maker keeps trading such asset to profit from the bid-ask spread by posting bid and ask quotes during the time interval $[0, T]$. More precisely, she posts the ask orders and bid orders at the price levels
\begin{equation}
...
\begin{equation}
    \mathbb{E}\big[X_T+S_T\,Q_T-\int_0^T \phi_t\,(Q_t)^2\,dt-A\,(Q_T)^2 \big] \quad \rightarrow \quad \max_{\boldsymbol{\delta}\in\mathbb{H}^2\times\mathbb{H}^2},
    \label{obj_functional}
\end{equation}
where $A:=A(L_T)$ and $\phi_t:=\phi(L_t)$ for some non-negative bounded functions $A, \phi:\mathbb{R}^d\to\mathbb{R}_+$ with the upper bounds $\bar{A}, \bar{\phi}>0$, accordingly.

\vspace{0.2cm}

\begin{assumption}
Function A is a continuous non-negative function with the upper bound $\bar{A}$. Functions $a, b, \phi$ are non-negative functions bounded above by $\bar{a}, \bar{b}, \bar{\phi}$ accordingly. Further, functions $a, b, \phi$ are all Lipschitz continuous in any bounded domain of $\mathbb{R}^d$.
\label{lip_ord_flow}
\end{assumption}
...
\begin{equation}
    ...
    \label{lin_pde_2}
\end{equation}
subjecting to the boundaries $h_2(T,l)=-A(l)$,  $h_1(T,l)=0$ and $h_0(T,l)=0$. One can see that the main challenges lies in the nonlinear PDE \eqref{non_lin_pde}, while the others are all linear and the well-posedness is direct given $h_2$ with sufficient regularity. We summarize this result in the next theorem with the verification step. Hence, the remaining of this section is devoted to \eqref{non_lin_pde}.
...
\indent (1) Regarding the optimal execution problems that take the order flow into account (see \cite{cartea2015algorithmic} section 7.3), in the PDE for $h_2$, the quadratic term of $h_2$ has a constant coefficient, that is,
    \begin{equation}
        ...
        \nonumber
    \end{equation}
where $k, \phi>0$ represent the permanent price impact and risk parameter accordingly. As there is no source term of $a$ and $b$, they deduce that $h_2$ is independent of the order flow and solve a Riccati equation. However, there is a source term of order flows in our case.
...
(2) With respect to execution problems with stochastic  parameters (\cite{barger2019optimal}, \cite{fouque2022optimal} and \cite{souza2022regularized}), they all consider PDEs for $h_2$ that are similar to ours. While \cite{barger2019optimal} and \cite{fouque2022optimal} study the approximation solutions, the existence and uniqueness of the viscosity solution is proven by \cite{souza2022regularized}. Our contribution lies in the well-posedness of the classical solution. Let us look at the equation from \cite{barger2019optimal} (equation (2.17) with no permanent price impact):
    \begin{equation}
        ...
        \nonumber
    \end{equation}
where $\varphi>0$ stands for the risk and $f(l)$ is the stochastic impact parameter. Comparing $1/f(l)$ with $\gamma(a(l)+b(l))$ of our equation, it is interesting to see that the lower friction as in optimal executions is equivalent to the higher market activity as in market makings. Furthermore, our method can be applied to prove the well-posedness of (2.17) by a simple change of variable.
\end{remark}
...
\begin{theorem}
\label{verification}
(1) Suppose \eqref{non_lin_pde} admits a bounded classical solution denoted by $h_2$, then
both \eqref{lin_pde_1} and \eqref{lin_pde_2} also possess bounded classical solutions written as $h_1$ and $h_0$ accordingly;\\
\indent (2) Define $\tilde{H}:[0,T]\times\mathbb{R}^d\times\mathbb{R}\to\mathbb{R}$ as
\begin{equation*}
    \tilde{H}(t,l,q)=h_0(t,l)+h_1(t,l)\,q+h_2(t,l)\,(q)^2,
\end{equation*}
then $H=\tilde{H}$ on $[0,T]\times\mathbb{R}^d\times\mathbb{R}$.\\
\indent (3) Let $\boldsymbol{\delta}^*=(\delta^{a,*}, \delta^{b,*})$ be defined as
\begin{equation}
    \delta_t^{b,*}=\frac{\zeta}{2\gamma}-\frac{1}{2}\frac{\partial \tilde{H}}{\partial q}, \hspace{1cm} \delta_t^{a,*}=\frac{\zeta}{2\gamma}+\frac{1}{2}\frac{\partial \tilde{H}}{\partial q},
...
\begin{aligned}
    ...
    &\hspace{1.8cm}-\frac{\partial \tilde{H}}{\partial q}\,a(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^a)+\frac{\partial \tilde{H}}{\partial q}\,b(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^b)\big]\,du.
\end{aligned}
\end{equation*}
If we choose
\begin{equation*}
...
\begin{equation*}
\begin{aligned}
    \mathbb{E}\big[\tilde{H}(s\wedge\tau_n, L_{s\wedge\tau_n}^{t,l}, Q_{s\wedge\tau_n}^{t,l,q,\boldsymbol{\alpha}})\big]&=\tilde{H}(t,l,q)+\mathbb{E}\int_t^{s\wedge\tau_n}\big[\frac{\partial \tilde{H}}{\partial t}(u, L_{u}^{t,l}, Q_{u}^{t,l,q,\boldsymbol{\alpha}})+\mathcal{L}\tilde{H}(u, L_{u}^{t,l}, Q_{u}^{t,l,q,\boldsymbol{\alpha}})\\
...
\begin{aligned}
    ...
    &\hspace{2cm}\leq \int_t^{T}\big|\phi(L_{u}^{t,l})\,(Q_{u}^{t,l,q,\boldsymbol{\alpha}})^2-\alpha_u^a\,a(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^a)-\alpha_u^b\,b(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^b)\big|\,du
\end{aligned}
\end{equation*}
and
\begin{equation*}
...
\begin{equation*}
\begin{aligned}
    -A(L_{T}^{t,l})\,(Q_{T}^{t,l,q,\boldsymbol{\alpha}})^2&\leq \tilde{H}(t,l,q)\\
    &+\mathbb{E}\int_t^{T}\Big[\phi(L_{u}^{t,l})\,(Q_{u}^{t,l,q,\boldsymbol{\alpha}})^2-\alpha_u^a\,a(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^a)-\alpha_u^b\,b(L_{u}^{t,l})\,(\zeta-\gamma\,\alpha_u^b)\big]\,du.
\end{aligned}
\end{equation*}
...
\begin{aligned}
    ...
    &\hspace{2.2cm}-\frac{\partial \tilde{H}}{\partial q}\,a(L_{u}^{t,l})\,(\zeta-\gamma\,\delta_u^{a,*})+\frac{\partial \tilde{H}}{\partial q}\,b(L_{u}^{t,l})\,(\zeta-\gamma\,\delta_u^{b,*})\big]\,du\\
...
(3) It suffices to prove the uniqueness of the optimal control, which is true if the objective functional \eqref{equ_obj_functional} is concave. In succession, the concavity of \eqref{equ_obj_functional} can be deduced from the concavity of $\delta_u^a\,a_u(\zeta-\gamma\delta_u^a)+\delta_u^b\,b_u(\zeta-\gamma\delta_u^b)$ and the convexity of $Q_t^2$ with respect to the control. The one of $\delta_u^a\,a_u(\zeta-\gamma\delta_u^a)+\delta_u^b\,b_u(\zeta-\gamma\delta_u^b)$ is clear because they are second order polynomials with negative signs for the leading terms. Let $\boldsymbol{\delta}, \boldsymbol{\beta}\in\mathbb{H}^2\times\mathbb{H}^2$ be two generic controls, and set $\lambda\in[0,1]$. We check the convexity of the quadratic inventory as follows:
\begin{equation*}
\begin{aligned}
    ...
    &=\lambda\,(\lambda-1)\,(Q_u^{\boldsymbol{\delta}}-Q_u^{\boldsymbol{\beta}})^2\\
    &\leq0,
\end{aligned}
\end{equation*}
...
We intend to prove the existence and uniqueness of the classical solution to \eqref{non_lin_pde} on the basis of \cite{becherer2005classical}, the result of which cannot be applied directedly because of the quadratic term $h_2(t,l)^2$.  To begin with, we introduce two auxiliary independent linear PDEs
\begin{gather*}
...
\begin{equation*}
    ...
    \end{cases}
\end{equation*}
and $g$ is a bounded continuous function to be specified later. If the function $g$ and the solution of the linear PDE are considered as input and output accordingly, we can define two mappings $F_1$ and $F_2$ for two equations by the Feynman-Kac formula:
\begin{gather*}
...
\begin{equation}
    ...
    &\hspace{3cm}+\bar{\phi}\int_t^T|e^{\int_t^s \gamma (a_u+b_u)\,v(u, L_u^{t,l})\,du}-e^{\int_t^s \gamma (a_u+b_u)\,w(u, L_u^{t,l})\,du}|\,ds\Big]\\
    &\leq e^{-\zeta_1\,(T-t)}\, \mathbb{E}\Big[A\,\big|\int_t^T \gamma (a_s+b_s)\,v(s, L_s^{t,l})\,ds-\int_t^T \gamma (a_s+b_s)\,w(s, L_s^{t,l})\,ds\big|\\
    &\hspace{3cm}+\bar{\phi}\int_t^T\big|\int_t^s \gamma (a_u+b_u)\,v(u, L_u^{t,l})\,du-\int_t^s \gamma (a_u+b_u)\,v(u, L_u^{t,l})\,du\big|\,ds\Big]\\
...
Similarly, we look at the second statement and compute that
\begin{equation}
    \begin{aligned}
...
\begin{equation}
\begin{aligned}
    A-\mathbb{E}\big[A\,e^{\int_t^T\gamma (a_s+b_s)\,v(s,L_s^{t,l})\,ds}\big]&=\mathbb{E}\int_t^T\gamma (a_s+b_s)\,v(s,L_s^{t,l})\\
    &\hspace{1.5cm}\times\mathbb{E}_s\big[(-A)\,e^{\int_s^T\gamma (a_u+b_u)\,v(u,L_u^{t,l})\,du}\big]\,ds,\\
    v(t, l)+\mathbb{E}\Big[\int_t^T\phi_s\,e^{\int_t^s \gamma\,(a_u+b_u)\,v(u,L_u^{t,l})\,du}\,ds\Big]&=\mathbb{E}\int_t^T\gamma (a_s+b_s)\,v(s,L_s^{t,l})\\
    &\hspace{1.5cm}\times\mathbb{E}_s\big[(-A)\,e^{\int_s^T\gamma (a_u+b_u)\,v(u,L_u^{t,l})\,du}\big]\,ds-A.
    \label{fix_1}
\end{aligned}
\end{equation}
If one notes that
...
\noindent The other reason is that the mapping $F_2$ has been studied by \cite{becherer2005classical} under some relative general assumptions. We can now extend its result to our equation \eqref{non_lin_pde}.
...
\begin{equation}
\begin{aligned}
    |\iota(t', l', v')-\iota(t, l, v)|&\leq\gamma\,(a(l')+b(l'))\,|\psi(v')^2-\psi(v)^2|+\gamma\,\psi(v)^2\,|a(l')+b(l')-a(l)-b(l)|\\
    &\hspace{1.8cm}+|\phi(l')-\phi(l)|\\
...
\begin{equation}
    ...
    \nonumber
\end{equation}
Note that both $\hat{v}$ and $u$ solves the following `regular' PDE:
\begin{equation}
...
\end{equation*}
for $n\in \mathbb{N}$. A sequence of solutions $(h_n)_{n\in\mathbb{N}}$, being uniformly bounded, is guaranteed by the Theorem \ref{pde_thm} and is non-decreasing by the comparison result. If we denote by $g$ as the limit of the sequence, it turns out that $g$ satisfy the conditional representation
\begin{equation*}
...
\begin{example}
We conclude this section with a particular example of \eqref{non_lin_pde} that has the solution of an explicit form. Some economic intuition is also helpful in a later proof. First, the boundedness of $\partial h_2/\partial l$ needs to be derived. For this purpose, several additional conditions are imposed:
\begin{itemize}
...
\end{itemize}
While other conditions can be checked in a similar way, one can again learn from \cite{becherer2005classical} that the PDE \eqref{linear_pde_diff} accepts a unique solution $g\in C_b^{1,2}$. The solution $g$ possesses the representation
\begin{equation*}
\begin{aligned}
    ...
    &\hspace{1cm} +\int_t^T (\tilde{a}(L_s^{t,l})+\tilde{b}(L_s^{t,l}))\,h_2(s,L_s^{t,l})\,e^{\int_t^s \gamma(a(L_u^{t,l}+\epsilon\,e^i)+b(L_u^{t,l}+\epsilon\,e^i))\,(h_2(u,L_u^{t,l}+\epsilon\,e^i)+h_2(u,L_u^{t,l}))\,du}\Big],
\end{aligned}
\end{equation*}
...
Let $Y_t:=h_2(t,L_t)$, $Z_t:=(\partial h_2/\partial l)(t,L_t)\,\Sigma(t,L_t)$, and then apply the It\^o's formula to obtain
\begin{equation*}
    Y_t=-A+\int_t^T \gamma\,(a_s+b_s)\,Y_s^2\,ds-\int_t^TZ_s\,dW_s.
\end{equation*}
Because $Y_t<0$, to observe the following we use the It\^o's formula again:
\begin{equation}
\begin{aligned}
...
\label{chang_measur}
\end{equation}
where $\tilde{W}$ is a standard Brownian motion with respect to some probability measure $\mathbb{Q}$, guaranteed by the Girsanov theorem. Indeed, the Girsanov transform can be conducted since $Z_t$ is bounded and $Y_t<-A\,\exp{(-\gamma\,(\bar{a}+\bar{b})\,A\,T)}$ by the mapping $F_1$. An explicit form of $h_2$ is obtained via taking the conditional expectation of \eqref{chang_measur}:
\begin{equation*}
...
\noindent In this section, while the same market making problem will be investigated, we extend the problem in two directions. First, the order flows and penalty parameters are not necessarily Markovian.

\vspace{0.2cm}

\begin{assumption}
Let $a$, $b$ and $\phi\in\mathbb{H}^2$ be non-negative processes and are bounded by constants $\bar{a}$, $\bar{b},$ $\bar{\phi}>0$ respectively. Let $A\in L^2(\Omega,\mathcal{F}_T)$ be a non-negative random variable, bounded by $\bar{A}>0$.
\end{assumption}
...
\noindent Denote by $(\Lambda^a,\Lambda^b)\in\boldsymbol{\Lambda}\times\boldsymbol{\Lambda}$ the intensity functions of the ask and bid side. Additionally, a strategy $\boldsymbol{\delta}:=(\delta^a,\delta^b)$ is admissible if two entries are bounded by $\xi$ for some large $\xi>0$, i.e., the admissible space is defined as
\begin{equation*}
...
\begin{aligned}
   ...
   &=\mathbb{E}\Big[\int_0^T\delta_t^a\,a_t\,\Lambda^a(\delta_t^a)\,dt+\int_0^T\delta_t^b\,b_t\,\Lambda^b(\delta_t^b)\,dt-\int_0^T \phi_t\,(Q_t)^2\,dt-A\,(Q_T)^2 \Big]\\
    &\rightarrow\max_{\boldsymbol{\delta}\in\mathbb{A}\times\mathbb{A}}.
\end{aligned}
\end{equation*}
To apply the Pontryagin maximum principle, the Hamiltonian of the agent with respect to the objective functional reads
\begin{equation}
    ...
    \nonumber
\end{equation}
While the function $\mathscr{H}$ is concave in the state variable $Q$, the concavity with respect to the control $\boldsymbol{\delta}$ is not guaranteed, which is required by the usual stochastic maximum principle (for example \cite{carmona2016lectures}). However, the separation between the state variable and control ensures us to still apply this principle, the proof of which is placed in the appendix. Denote by $(\delta^{a*},\delta^{b*})$ the maximisers associated with the intensity functions $(\Lambda^a, \Lambda^b)$ according to Lemma \ref{inten_fun}, and further define $\tilde{\delta}^{i*}(p):=\delta^{i*}(p)\wedge\xi\vee(-\xi)$ for $i\in\{a,b\}$. Then the optimal feedback controls on the ask and bid sides are given by 
\begin{equation}
    \tilde{\delta}^{a*}(Y_t) \quad\text{and}\quad \tilde{\delta}^{b*}(-Y_t).
    \nonumber
\end{equation}
Indeed, concerning the ask side, we look for the maximizer of 
\begin{equation*}
...
\end{equation*}
and $\delta_t^a+\Lambda^a(\delta_t^a)/\Lambda^{a'}(\delta_t^a)$ is an increasing function because
\begin{equation*}
...
\end{equation*}
one can see that function $g$ first increases then decreases. Hence, the maximiser is $\tilde{\delta}^{a*}(Y_t)$ under the constraint stated in $\mathbb{A}$. The bid side can be derived in the same way. Combined with the stochastic maximum principle, the above discussion yields the next result.
...
\begin{equation}
    \mathscr{H}(t,Q_t,Y_t,\boldsymbol{\delta}_t)=\sup_{\boldsymbol{\alpha}\in [-\xi,\infty)^2}\mathscr{H}(t,Q_t,Y_t,\boldsymbol{\alpha}),  \qquad \text{a.e. in } t\in[0,T],
    \nonumber
\end{equation}
or equivalently
\begin{equation}
...
\end{theorem}
\begin{proof}
    It suffices to see the appendix for a proof of the stochastic maximum principle.
\end{proof}
...
\begin{proof}
Motivated by the linearity of \eqref{Lin_noncont_FBSDE}, we adopt the affine ansatz $Y_t=P_t\cdot Q_t$ for some $P:=(P_t)_{0\leq t\leq T}$ to be determined. Through applying the It\^o's formula and matching the coefficients, one can see that $P$ satisfies the BSDE
\begin{equation}
...
\end{gather*}
for some constant $\tilde{\xi}>0$ and bounded process $G\in\mathbb{H}^2$ to be specified. As before, $F_3$ is a contraction mappings on the space $\mathbb{S}^2$ with respect to the norm
\begin{equation*}
    \|G\|_{\zeta_3}:=\mathbb{E}\big[\sup_{0\leq t\leq T}e^{-\zeta_3\,(T-t)}\cdot |G_t|^2\,\big],
\end{equation*}
and $F_4$ is a contraction mappings on the space of non-positive processes in $\mathbb{S}^2$ with respect to the norm
\begin{equation*}
    \|G\|_{\zeta_4}:=\mathbb{E}\big[\sup_{0\leq t\leq T}e^{-\zeta_4\,(T-t)}\cdot |G_t|^2\,\big],
\end{equation*}
...
\right.
\end{equation}
While the It\^o's formula implies
...
\end{equation*}
we can see the $\mathcal{Q}=0$ a.s. and thus $\mathcal{Y}=\mathcal{Z}=0$ a.s. follows immediately. 
\end{proof}
\end{lemma}
...
\noindent Since the FBSDE \eqref{Lip_den_FBSDE} is Lipschitz, denote by $\mathscr{L}>0$ the Lipschitz constant for the coefficient functions of forward and backward equations of $\eqref{Lip_den_FBSDE}$ (excluding the terminal condition), i.e., it holds $\mathbb{P}$-a.s. for any $y, \Tilde{y}, q, \Tilde{q}$ that
\begin{equation*}
\begin{aligned}
    \Big|-a_t\,\Lambda^a\big(\tilde{\delta}^{a*}(y)\big)+b_t\,\Lambda^b\big(\tilde{\delta}^{b*}(-y)\big)+a_t\,\Lambda^a\big(\tilde{\delta}^{a*}(\tilde{y})\big)-b_t\,\Lambda^b\big(\tilde{\delta}^{b*}(-\tilde{y})\big)\Big|&\leq \mathscr{L}\,|\tilde{y}-y|,\\
    \big|2\phi_t\,q-2\phi_t\,\tilde{q}\big|&\leq  \mathscr{L}\,|\tilde{q}-q|,\\
    \big|2A\,q-2A\,\tilde{q}\big|&\leq  \mathscr{L}\,|\tilde{q}-q|.
\end{aligned}
\end{equation*}
To prove the well-posedness, we apply the decoupling approach introduced by \cite{ma2015well}. Actually, the Lemma \ref{growth_lip_fbsde} studies the growth of Lipschitz coefficient---a critical step for such technique.
...
\begin{equation}
\begin{aligned}
    |\tilde{Q}'_t-Q'_t|&\leq \mathscr{L}T \sup_{0\leq s \leq T}|\tilde{Y}_s-Y_s|\\ \mathbb{E}\sup_{0\leq s \leq T}|\tilde{Q}'_s-Q'_s|^2&\leq (\mathscr{L}T)^2\,\mathbb{E}\sup_{0\leq s \leq T}|\tilde{Y}_s-Y_s|^2.
...
\begin{equation}
    \begin{aligned}
    |\tilde{Y}_t-Y_t|^2&\leq (2\bar{A}+2\mathscr{L}T)^2\,\mathbb{E}_t\big[\sup_{0\leq s\leq T}|\tilde{Q}_s-Q_s|\big]^2,\\
    \mathbb{E}\sup_{0\leq s\leq T}|\tilde{Y}_s-Y_s|^2&\leq (2\bar{A}+2\mathscr{L}T)^2\,\mathbb{E}\Big[\sup_{0\leq t\leq T}\mathbb{E}_t\big[\sup_{0\leq s\leq T}|\tilde{Q}_s-Q_s|\big]^2\Big],\\
    &\leq 4\,(2\bar{A}+2\mathscr{L}T)^2\,\mathbb{E}\sup_{0\leq s\leq T}|\tilde{Q}_s-Q_s|^2,
    \end{aligned}
...
\begin{equation}
    \mathbb{E}\sup_{0\leq s \leq T}|\tilde{Q}'_s-Q'_s|^2\leq 4\,(\mathscr{L}T)^2\,(2\bar{A}+2\mathscr{L}T)^2\,\mathbb{E}\sup_{0\leq s\leq T}|\tilde{Q}_s-Q_s|^2
    \nonumber   
\end{equation}
and it suffices to pick $T$ satisfying
\begin{equation}
    4\,(\mathscr{L}T)^2\,(2\bar{A}+2\mathscr{L}T)^2<1
    \nonumber
\end{equation}
...
\begin{equation}
    \Delta=\frac{1}{\sqrt{2}}\cdot \big[4\mathscr{L}^2\,(2\bar{A}+2\bar{\phi}T+2\mathscr{L}T)^2\big]^{-1/2}.
    \label{control_time_step}
\end{equation}
Note that 
\begin{equation*}
    4\,(\mathscr{L}\Delta)^2\,(2\bar{A}+2\mathscr{L}\Delta)^2\leq4\,(\mathscr{L}\Delta)^2\,(2\bar{A}+2\bar{\phi}T+2\mathscr{L}T)^2=\frac{1}{2}
\end{equation*}
...
\end{aligned}
    \nonumber
\end{equation}
Naturally, for those $t$ such that $Y_t^1= Y_t^2$, we let $\mathcal{U}_t=0$. Note that $\mathcal{U}_t\in \mathbb{H}^2$ is bounded by $\mathscr{L}$ due to the Lipschitz continuity. It is further non-negative since the expression
\begin{equation}
    -a\,\Lambda^a\big(\tilde{\delta}^{a*}(y)\big)+b\,\Lambda^b\big(\tilde{\delta}^{b*}(-y)\big)
    \nonumber   
\end{equation}
is monotone increasing with respect to $y$, which can be deduced from the monotone increasing property of $(\delta^{a*},\delta^{b*})$ and decreasing property of $(\Lambda^a,\Lambda^b)$. One can then observe that $(Q^1-Q^2, Y^1-Y^2, Z^1-Z^2)$ satisfies the following FBSDE
\begin{equation}
    \left\{
\begin{aligned}
...
\begin{equation*}
    4\,(\mathscr{L}\Delta)^2\,(2\bar{A}+2\bar{\phi}\Delta+2\mathscr{L}\Delta)^2<1,
\end{equation*}
and our choice \eqref{control_time_step} is clearly qualified. Referring to Lemma \ref{growth_lip_fbsde}, we study the variational FBSDE and observe the linear growth of the Lipschitz coefficient of the decoupling field. Because the potential Lipschitz coefficient of the decoupling field can not exceed $2\bar{A}+2\bar{\phi}T$, the time step $\Delta$ will be always qualified, and hence the field $u$ can be extended to the whole time horizon $[0,T]$ by a finite number of iterations. The global existence of the solution then can be inferred from the (global) decoupling field $u$. While the uniqueness of solution can be proved in the same fashion as Lemma \ref{growth_lip_fbsde}, the well-posedness of \eqref{Lip_den_FBSDE} is established.
...
\nonumber
\end{equation}
In light of this, the lower bound for $\tilde{Y}_t$ can also be revealed by
\begin{equation}
...
\begin{gather}
    \hat{Y}_t=\mathbb{E}_t\Big[-2A\,\hat{Q}_T- 2\int_t^T\phi_s\,\hat{Q}_s\,ds\Big]\geq\mathbb{E}_t\Big[-2A\, \hat{Q}_0- 2\int_t^T\phi_s\,\hat{Q}_0\,ds\Big]\geq(-2)\,|q_0|\,(\bar{A}+\bar{\phi}\,T),\nonumber\\
...
\label{q^hat_bound}\\
    \hat{Y}_t=\mathbb{E}_t\Big[-2A\,\hat{Q}_T- 2\int_t^T\phi_s\,\hat{Q}_s\,ds\Big]\leq 2\,(\bar{A}+\bar{\phi}\, T)\cdot\bigg[|q_0|+\bar{a}\,T\,\Lambda^a\Big(\delta^{a*}\big((-2)\,|q_0|\,(\bar{A}+\bar{\phi}\,T)\big)\Big)\bigg],
\end{gather}
...
\begin{equation}
    ...
    \nonumber
\end{equation}
when $\tilde{Y}_t-Y_t\neq0$, and $\pi_t=0$ in the case of $\tilde{Y}_t-Y_t=0$. Due to the monotonicity of $(\Lambda^a,\Lambda^b)$ and $(\delta^{a*},\delta^{b*})$, we know that $\pi\in\mathbb{H}^2$ is a bounded non-negative process. Afterwards, the triplet $(\tilde{Q}-Q,\tilde{Y}-Y,\tilde{Z}-Z)$ solves the following linear FBSDE:
\begin{equation}
\left\{
\begin{aligned}
...
\end{aligned}
\end{equation*}
The uniform boundedness of $(\tilde{Q},\tilde{Y})$ implies that $Q$ is lower bounded by a constant that is independent of $\xi$. On the other hand, to search for an upper bound for $Q$, we study the relation between $(Q,Y,Z)$ and  $(\hat{Q},\hat{Y},\hat{Z})$. Taking the difference of \eqref{nonLip_den_FBSDE} and \eqref{nonLip_dom_FBSDE_2}, we apply a similar transform to see
\begin{equation}
\begin{aligned}
    ...
    &=\iota_t\,(\hat{Y}_t-Y_t)\,dt-b_t\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_t)\big)\,dt,
\end{aligned}
\nonumber
\end{equation}
where we let
\begin{equation*}
    \iota_t=\frac{b_t\,\Big[\Lambda^b\big(\tilde{\delta}^{b*}(-\hat{Y}_t)\big)-\Lambda^b\big(\tilde{\delta}^{b*}(-Y_t)\big)\Big]+a_t\,\Big[\Lambda^a\big(\tilde{\delta}^{a*}(Y_t)\big)-\Lambda^a\big(\tilde{\delta}^{a*}(\hat{Y}_t)\big)\Big]}{\hat{Y}_t-Y_t}
\end{equation*}
when $\hat{Y}_t-Y_t\neq0$, and $\iota_t=0$ in the case of $\hat{Y}_t-Y_t=0$. One can see that $\iota\in\mathbb{H}^2$ is non-negative and bounded as well. Similarly, the triplet $(\hat{Q}_t-Q_t,\hat{Y}_t-Y_t,\hat{Z}_t-Z_t)$ solves the following (well-posed) linear FBSDE:
\begin{equation}
\left\{
\begin{aligned}
\;& d\mathcal{Q}_t  = \iota_t\,\mathcal{Y}_t dt-b_t\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_t)\big)\,dt, \\
& d\mathcal{Y}_t=2\phi_t\,\mathcal{Q}_t\,dt+\mathcal{Z}_t\,dW_t,\\
& \mathcal{Q}_0=0,\quad \mathcal{Y}_T=-2A\,\mathcal{Q}_T.
\nonumber
\end{aligned}
...
\begin{gather*}
    d\mathscr{P}_t=(-\iota_t\,\mathscr{P}_t^2+2\phi_t)\,dt+\mathscr{Z}^{\mathscr{P}}_t\,dW_t, \quad\text{with}\quad \mathscr{P}_T=-2A;\\
    d\mathscr{H}_t=-\Big[\iota_t\,\mathscr{P}_t\,\mathscr{H}_t-\mathscr{P}_t\,b_t\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_t)\big)\Big]\,dt+\mathscr{Z}^{\mathscr{H}}_t\,dW_t, \quad\text{with}\quad \mathscr{H}_T=0.
\nonumber
\end{gather*}
One knows the first BSDE accepts a solution $\mathscr{P}\in\mathbb{H}^2$ that is negative and bounded. In turn, the solution of the second linear BSDE is given explicitly by
\begin{equation}
    \mathscr{H}_t=-\,\mathbb{E}_t\Big[\int_t^T\mathscr{P}_s\,b_s\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_s)\big)\,e^{\int_t^s\iota_u\,\mathscr{P}_u\,du}\,ds\Big]\geq 0.
\nonumber
\end{equation}
...
\begin{equation*}
    \mathcal{Q}_t=\int_0^t \Big[\iota_s\,\mathscr{H}_s-b_s\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_s)\big)\Big]\,e^{\int_s^t\iota_u\,\mathscr{P}_u\,du}\,ds,
\end{equation*}
by definition we observe
\begin{equation*}
\begin{aligned}
    \hat{Q}_t-Q_t&=\int_0^t \Big[\iota_s\,\mathscr{H}_s-b_s\,\Lambda^b\big(\tilde{\delta}^{b*}(\hat{Y}_s)\big)\Big]\,e^{\int_s^t\iota_u\,\mathscr{P}_u\,du}\,ds\\
...
\end{aligned}
\end{equation*}
The uniform boundedness of $(\hat{Q},\hat{Y})$ suggests that $Q$ is upper bounded by a constant that is independent of $\xi$. While now we know $Q$ is uniformly lower bounded, it leads to the uniform boundedness of $Y$ since
\begin{equation*}
...
\noindent While in the previous two cases, the coefficients---order flows $(a, b)$ and penalisation parameters $(\phi, A)$---are all bounded. Since we have mentioned the unbounded order flows in Remark \ref{unbound_order_flow}, this section is devoted to such situation. For convenience of notation, we will represent the result in the case of linear intensity function, i.e., $\Lambda(\delta)=\zeta-\gamma\,\delta$ for some constants $\zeta, \gamma>0$. Regarding the general intensity function, it suffices to add a polynomial growth condition in Assumption \ref{general_inten}, while the exponential function will be excluded.
...
\begin{gather*}
    ...
    \nonumber
\end{gather*}
where $\delta^a,\delta^b\in\mathbb{H}^8$ represent the control. Note that here we seek strategy in the space $\mathbb{H}^8$ to make sure the associated inventory $Q$ is still in $\mathbb{H}^4$ (indeed in $\mathbb{S}^4$). Hence, the following objective functional is well-defined and can be simplified as before:
\begin{equation*}
\begin{aligned}
    J(\boldsymbol{\delta}):&=\mathbb{E}\big[X_T+S_T\,Q_T-\int_0^T \phi_t\,(Q_t)^2\,dt-A\,(Q_T)^2 \big]\\
   &=\mathbb{E}\Big[\int_0^T\delta_t^a\,a_t(\zeta-\gamma\delta_t^a)\,dt+\int_0^T\delta_t^b\,b_t(\zeta-\gamma\delta_t^b)\,dt-\int_0^T \phi_t\,(Q_t)^2\,dt-A\,(Q_T)^2 \Big]\\
   &\rightarrow\max_{\boldsymbol{\delta}\in\mathbb{H}^8\times\mathbb{H}^8}.
\end{aligned}
\end{equation*}
...
\end{aligned}
\end{equation*}
to justify the integrability. Since the convex nature of the objective functional $J$ is revealed in the proof of Theorem \ref{verification}, the convex-analytic method helps characterize the (unique) optimal control as the solution of a FBSDE.

\vspace{0.2cm}

\begin{theorem}
    The functional $\boldsymbol{\delta}\hookrightarrow J(\boldsymbol{\delta})$ is G\^ateaux differentiable and the derivative in the direction of $\boldsymbol{\beta}$ reads
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $V_t=\gamma\int_0^t(a_s\,\beta_s^a-b_s\,\beta_s^b)\,ds$. A control $(\delta^{*,a},\delta^{*,b})\in\mathbb{H}^8\times\mathbb{H}^8$ maximises the functional $J$ if and only if it can be represented by
\begin{equation}
...
\label{compute_deriva}
    \frac{1}{\epsilon}\,\big[J(\boldsymbol{\delta}+\epsilon\boldsymbol{\beta})-J(\boldsymbol{\delta})\big]=\mathbb{E}&\Big[\int_0^Ta_t\,\beta_t^a\,(\zeta-2\,\gamma\,\delta_t^a-\epsilon\,\gamma\,\beta_t^a)\,dt\Big]+\mathbb{E}\Big[\int_0^Tb_t\,\beta_t^b\,(\zeta-2\,\gamma\,\delta_t^b-\epsilon\,\gamma\,\beta_t^b)\,dt\Big]\\
    & -\mathbb{E}\Big[\int_0^T \phi_t\,(Q_t^\epsilon+Q_t)\,V_t\,dt\Big]-\mathbb{E}\Big[A\,(Q_T^\epsilon+Q_T)\,V_T\Big].
\end{aligned}
\end{equation}
We look at the first term on the right hand side of \eqref{compute_deriva}. To perform the limiting procedure, note that
\begin{equation*}
\begin{aligned}
    \int_0^Ta_t\,\beta_t^a\,(\zeta-2\,\gamma\,\delta_t^a-\epsilon\,\gamma\,\beta_t^a)\,dt&\leq \int_0^Ta_t\,|\beta_t^a|\,\big(\zeta+2\,\gamma\,|\delta_t^a|+\gamma\,|\beta_t^a|\big)\,dt,\\
    a_t\,\beta_t^a\,(\zeta-2\,\gamma\,\delta_t^a-\epsilon\,\gamma\,\beta_t^a)&\leq a_t\,|\beta_t^a|\,\big(\zeta+2\,\gamma\,|\delta_t^a|+\gamma\,|\beta_t^a|\big),
\end{aligned}
\end{equation*}
the right hand sides of which either have a finite expectation or almost surely integrable on the time horizon interval. Therefore, it follows from the dominated convergence theorem that
\begin{equation*}
...
\begin{aligned}
    ...
    &\leq C\sup_{t\in[0,T]}|Q_t|^2+C\,\Big(\int_0^T (a_t+b_t)\,dt\Big)^2+C\,\Big(\int_0^T \phi_t^2\,dt\Big)^2+C\,\Big(\int_0^T V_t^2\,dt\Big)^2\\
...
\begin{aligned}
    ...
    &=\mathbb{E}\bigg\{\int_0^T\Big[a_t\,\beta_t^a\,\big(\zeta-2\gamma\,\delta_t^a-2\gamma\int_t^T\phi_s\,Q_s\,ds-2\gamma\, A\big)\Big]\,dt\bigg\}\\
    &\hspace{3cm}+\mathbb{E}\bigg\{\int_0^T\Big[b_t\,\beta_t^b\,\big(\zeta-2\gamma\,\delta_t^b-2\gamma\int_t^T\phi_s\,Q_s\,ds+2\gamma\, A\big)\Big]\,dt\bigg\}.
    \label{derive_fbsde}
\end{aligned}
\end{equation}
...
\begin{equation*}
\begin{aligned}
    \gamma\,Y_t=\zeta-2\gamma\,\delta_t^{*,a}&=\mathbb{E}_t\Big[2\gamma\int_t^T\phi_s\,Q_s\,ds+2\gamma\, A\Big],\\
    \gamma\,Y_t=2\gamma\,\delta_t^{*,b}-\zeta&=\mathbb{E}_t\Big[2\gamma\int_t^T\phi_s\,Q_s\,ds+2\gamma\, A\Big]
\end{aligned}
\end{equation*}
$d\mathbb{P}\otimes dt$-almost everywhere. The FBSDE can be obtained by `differentiating' $Y$.
\end{proof}
...
\end{equation}
the coefficients $a, b, \phi, A$ of which are not bounded. Note that the $(Y,Z)$ here is different from the one in the FBSDE, and here we only focus on the existence of solutions. We regularize such equation via the truncation:
\begin{equation}
...
\end{equation*}
Note that $-U_t(\omega)$ may be infinity for some $t$ and $\omega$. Thanks to the monotonicity of $(Y^{(n)})_{n\in\mathbb{N}}$, we can observe that
\begin{equation*}
    \lim_{n\to\infty}|Y_t^{(n)}|^{8\mathscr{D}}\leq \lim_{n\to\infty}\,\sup_{0\leq u\leq T}|Y_u^{(n)}|^{8\mathscr{D}}, \qquad 0 \leq t \leq T,
\end{equation*}
the right hand side of which is well-defined (possibly infinity). It further reveals the integrability of $U$ as follows
\begin{equation}
\begin{aligned}
    ...
    &= \lim_{n\to\infty}\,\mathbb{E}\Big[\sup_{0\leq u\leq T}\Big|\,\mathbb{E}_u\big[-2\,(A\wedge n)-2\int_t^T (\phi_s\wedge n)\,ds+\int_t^T\gamma\,\big((a_u+b_u)\wedge m\big)\,(Y_u)^2\,du\big]\,\Big|^{8\mathscr{D}}\Big]\\
...
\begin{equation*}
\begin{aligned}
    U_t=\lim_{n\to\infty}Y_t^{(n)}&=\lim_{n\to\infty}\mathbb{E}_t\Big[-2\,(A\wedge n)-2\int_t^T (\phi_s\wedge n)\,ds+\gamma\int_t^T \big((a_s+b_s)\wedge m\big)\,\big(Y_s^{(n)}\big)^2\,ds\Big]\\
...
\begin{aligned}
    ...
    &\hspace{1.5cm}-\gamma\,\big((a_t+b_t)\wedge (m+1)\big)\,(Y_t^{(m)})^2+\gamma\,\big((a_t+b_t)\wedge m\big)\,(Y_t^{(m)})^2\Big)\,dt+\Delta Z_t\,dW_t\\
    &=\Big(-\gamma\,\big((a_t+b_t)\wedge (m+1)\big)\,(Y_t^{(m+1)}+Y_t^{(m)})\,\Delta Y_t\\
    &\hspace{1.5cm}-\gamma\, (Y_t^{(m)})^2\,\big[(a_t+b_t)\wedge (m+1)-(a_t+b_t)\wedge m\big]\Big)\,dt+\Delta Z_t\,dW_t.
\end{aligned}
\end{equation*}
...
\end{theorem}
\begin{proof}
\noindent Denote by $(Y^{(m)}, Z^{(m)})\in\mathbb{S}^{8\mathscr{D}}\times\mathbb{H}^2$ the solution of equation \eqref{half_trun_bsde_unbound_coeff} for each $m\in\mathbb{N}$. Given the monotonicity of $(Y^{(m)})_{m\in\mathbb{N}}$, for almost every $\omega\in\Omega$, we define the limit similarly as
\begin{equation*}
    Y_t(\omega):=\lim_{n\to\infty}Y_t^{(n)}(\omega)\leq 0,\qquad 0 \leq t \leq T.
\end{equation*}
...
\begin{theorem}
The FBSDE \eqref{linear_fbsde_unbound} accepts a unique solution $(Q, Y, Z)\in \mathbb{S}^{2\mathscr{D}}\times\mathbb{S}^{\mathscr{D}}\times\mathbb{H}^{2}$. The admissibility of the control requires $\mathscr{D}=8$.
\end{theorem}
\begin{proof}
...
\begin{equation*}
\begin{aligned}
    \int_0^Ta_t\,\delta_t^a\,\frac{1}{\epsilon}\,\big(\Lambda^a(\delta_t^a+\epsilon\,\beta_t^a)-\Lambda^a(\delta_t^a)\big)\,dt&\leq C\,\int_0^Ta_t\,|\delta_t^a|\,dt,\\
    \int_0^Ta_t\,\beta_t^a\,\Lambda^a(\delta_t^a+\epsilon\,\beta_t^a)\,dt &\leq C\,\int_0^T a_t\,dt,
\end{aligned}
\end{equation*}
the right hand sides of which have finite expectations. Also, for any $t\in[0,T]$ one can observe
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
the right hand sides of which are $\mathbb{P}$-a.s. integrable on the time horizon interval. Therefore, it follows by the dominated convergence theorem that
\begin{equation*}
\begin{aligned}
...
\end{aligned}
\end{equation*}
Being aware of the boundedness of $\phi$, $Q$, $Q^\epsilon$, $a$,  $b$ as well as
\begin{equation*}
...
processes, then
\begin{equation*}
    \mathscr{H}(t,Q_t,Y_t,\boldsymbol{\delta}_t)\geq\mathscr{H}(t,Q_t,Y_t,\boldsymbol{\beta}), \qquad \text{a.e. in } t\in[0,T],\; \mathbb{P}-a.s.,
\end{equation*}
for any $\boldsymbol{\beta}\in[-\xi, \xi]\times[-\xi, \xi]$.
\end{theorem}
\begin{proof}
...
\begin{equation}
    ...
    \label{smp_gener}
\end{equation}
for all $\boldsymbol{\beta}\in[-\xi, \infty)\times[-\xi, \infty)$. Regarding the condition \eqref{smp_gener}, it can only happen at time $t$ if one of the following cases holds:
\begin{gather*}
...
\begin{equation}
    \begin{aligned}
        \mathbb{E}[-A\,(Q_T)^2 + A\,(Q'_T)^2]&\geq \mathbb{E}\big[-2 A\, Q_T\,(Q_T-Q'_T)\big]\\
...
\begin{equation}
    \begin{aligned}
    \mathbb{E}\int_0^T\Big[a_t\,&\delta_t^a\,\Lambda^a(\delta_t^a)+b_t\,\delta_t^b\,\Lambda^b(\delta_t^b)-\phi_t\,(Q_t)^2-\big(a_t\,\alpha_t^a\,\Lambda^a(\alpha_t^a)+b_t\,\alpha_t^b\,\Lambda^a(\alpha_t^a)-\phi_t\,(Q'_t)^2\big)\Big]\,dt\\
...
\begin{equation}
\begin{aligned}
    J(\boldsymbol{\delta})-J(\boldsymbol{\alpha})&=\mathbb{E}\int_0^T\Big[a_t\,\delta_t^a\,\Lambda^a(\delta_t^a)+b_t\,\delta_t^b\,\Lambda^b(\delta_t^b)-\phi_t\,(Q_t)^2\\
