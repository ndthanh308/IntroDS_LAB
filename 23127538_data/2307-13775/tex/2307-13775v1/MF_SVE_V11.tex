...
\noindent\textbf{Acknowledgments:} D. Scheffels gratefully acknowledges financial support by the Research Training Group ``Statistical Modeling of Complex Systems'' (RTG 1953) funded by the German Science Foundation (DFG).


\section{Main results: well-posedness and propagation of chaos}\label{sec:main result}

Let $T\in (0,\infty)$, $d,m\in\N$, and let $(\Omega,\mathcal{F},(\mathcal{F}_t)_{t\in [0,T]},\mathbb{P})$ be a filtered probability space, which satisfies the usual conditions. Suppose $B=(B_t)_{t\in [0,T]}$ is an $m$-dimensional Brownian motion with respect to $(\mathcal{F}_t)_{t\in [0,T]}$. The law of a random variable~$X$ is denoted by $\mathcal{L}(X)$ and, for $p\geq 1$, the space of probability measures on $\R^d$ with finite $p$-th moments by $\mathcal{P}_{p}(\R^d)$. For $\rho,\tilde{\rho}\in\mathcal{P}_p(\R^d)$, we write $W_p(\rho,\tilde{\rho})$ for the $p$-Wasserstein distance between $\rho$ and $\tilde{\rho}$, see \cite[Chapter~5]{Carmona2018} for its definition. The space $\R^d$ is always equipped with the Euclidean norm~$| \cdot |$. Moreover, we set $\Delta_T:=\lbrace (s,t)\in [0,T]\times [0,T]\colon \, 0\leq s\leq t\leq T \rbrace$ and use the notation $A_{\eta}\lesssim B_{\eta}$ for a generic parameter~$\eta$, meaning that $A_{\eta}\le CB_{\eta}$ for some constant $C>0$ independent of~$\eta$.
...
Our second result is propagation of chaos for mean-field stochastic Volterra equations, i.e. we show that the unique $L^p$-solution to the mean-field stochastic Volterra equation~\eqref{eq:MVSVE} is the limit $N\to\infty$ of the solutions to the following symmetric system of $N$ mean-field stochastic Volterra equations
...
\begin{lemma}\label{lem:rates}
  Supposing the assumptions and notation of Theorem~\ref{thm:propagation}, it holds that
...
First, we establish the well-posedness of the mean-field stochastic Volterra equation~\eqref{eq:MVSVE2} with H{\"o}lder continuous diffusion coefficients. Its proof is based on a Yamada--Watanabe type approach~\cite{Yamada1971}, which requires essentially a one-dimensional setting and leads to the stronger assumptions on the kernels.

\begin{theorem}\label{thm:well-posedness2}
  Suppose Assumption~\ref{ass:coefficients2}, and the kernels $K_{\mu},K_{\sigma}$ and the initial condition~$X_0$ satisfy Assumption~\ref{ass:kernels2} or Assumption~\ref{ass:kernels3} with $p$ given as therein. Then, the mean-field stochastic Volterra equation~\eqref{eq:MVSVE2} is well-posed in $L^p$. Moreover, the unique solution $X$ of~\eqref{eq:MVSVE2} satisfies
  \begin{equation*}
    \sup\limits_{t\in[0,T]}\E[|X_t|^{q}]<\infty\quad \text{for all } q\geq 1.
  \end{equation*}
\end{theorem}
...
\begin{remark}\label{rem:multi-dimVolterra}
  ...
  and
  \begin{align*}
    \sigma(s,X_s)=\left( \begin{array}{rrr}
    \sigma_1(s,X_s^1) &\cdots & 0 \\
    \vdots & \ddots & \vdots \\
    0 & \cdots& \sigma_N(s,X_s^N) \\
    \end{array}\right).
  \end{align*}
%  Suppose at least one of the following two conditions
%  \begin{itemize}
%    \item[(i)] the kernels $K_\mu,K_\sigma$ fulfill Assumption~\ref{ass:kernels2} and the initial value $X_0$ is in $L^p(\Omega;\R^d)$ for some $p>\max\lbrace  \frac{1}{\gamma}, 1+\frac{2}{\epsilon}\rbrace$,
%    \item[(ii)] the kernels are of the form $K_\mu(s,t)=K_\sigma(s,t)=\tilde{K}(t-s)$ for some $\tilde{K}\in C^1([0,T];\R)$ and $X_0\in L^p(\Omega;\R)$ for some $p>2$,
%  \end{itemize}
...
\begin{remark}
  ...
  \end{equation*}
  one needs to assume that both solutions $X,Y$ are in $L^p(\Omega\times[0,T];\R^d)$ to guarantee finiteness of the expectations $\sup_{s\in[0,t]}\E[|X_s|^p]$ and $\sup_{s\in[0,t]}\E[|Y_s|^p]$ by standard estimates, as in \cite[Lemma~3.4]{Promel2023}. %Consequently, there might be further processes that solve the SVE, but are not in $L^p(\Omega\times [0,T])$. %As an example, consider the (deterministic) SVE
  %\begin{equation}\label{eq:detSVE}
  %X_t=\int_0^t(\alpha+1)t^{-\frac{3}{2}-\alpha}s^\alpha\dd s,\quad t\in[0,T],
  %\end{equation}
  %for some $\alpha\in(-1/2,0)$.
\end{remark}
...
\begin{proof}[Proof of Theorem~\ref{thm:well-posedness}]
  ...
  \begin{align}\label{eq:L1bound}
    ...
    &\lesssim \Big( \int_0^t|K_\mu(s,t)|^{\frac{4+2\epsilon}{4+\epsilon}}\dd s \Big)^{\frac{4+\epsilon}{1+\epsilon}} \int_0^t \E\big[ \big|\mu(s,X^\rho_s,\rho_s)-\mu(s,X^{\tilde{\rho}}_s,\tilde{\rho}_s) \big|^{\delta}\big]\dd s \notag\\
  ...
  \end{align}
  for $t\in[0,T]$, where we used H{\"o}lder's inequality in the drift integral with $\frac{4+2\epsilon}{4+\epsilon}<1+\epsilon$ such that by the choice of $\delta$ in \eqref{eq:delta}, $\frac{4+\epsilon}{4+2\epsilon}+\frac{1}{\delta}=1$ and in the diffusion integral with $\frac{2+\epsilon}{2}$ such that \eqref{eq:delta_Holder} holds, Burkholder--Davis--Gundy's inequality, Fubini's theorem, the integrability of the kernels from Assumption~\ref{ass:kernels1} and the Lipschitz continuity of $\mu$ and $\sigma$ from Assumption~\ref{ass:coefficients1}. Since we have that
  \begin{equation*}
...
\begin{proof}[Proof of Theorem~\ref{thm:well-posedness2}]
  ...
  To get an analogue estimate as \eqref{ineq:Gronwall}, we use the semimartingale property of a solution $(X_t^\rho)_{t\in[0,T]}$ to \eqref{eq:MVSVE} with fixed $\rho\in\mathcal{P}_1(\R)$ (cf. \cite[Lemma~3.6]{Promel2023} or \cite[Theorem~3.3]{Protter1985}),
  \begin{align*}
    X_t^\rho - X_0 &= \int_0^t K_\sigma(s,s)\sigma(s,X_s^\rho,\rho_s)\dd B_s+ \int_0^t K_\mu(s,s)\mu(s,X_s^\rho,\rho_s) \dd s \\
    &\qquad +  \int_0^t \bigg(\int_0^s \partial_2 K_\mu(u,s)\mu(u,X_u^\rho,\rho_u) \dd u + \int_0^s \partial_2 K_\sigma(u,s)\sigma(u,X_u^\rho,\rho_u)\dd B_u\bigg) \dd s,
  \end{align*}
...
\begin{proof}[Proof of Theorem~\ref{thm:propagation}]
  ...
  \begin{align}
    ...
    &\quad\lesssim \Big( \int_0^t \big|K_\mu(s,t)\big|^{\frac{4+2\epsilon}{4+\epsilon}}\dd s\Big)^{\frac{4+\epsilon}{1+\epsilon}}\int_0^t \E\Big[\Big| \mu(s,X_s^{N,i},\bar{\rho}_s^N)-\mu\big(s,\underbar{X}_s^i,\mathcal{L}(\underbar{X}_s^i)\big) \Big|^\delta\Big]\dd s\notag\\
    ...
    &\quad\qquad +\Big( \int_0^t |K_\sigma(s,t)|^{2+\epsilon}\dd s \Big)^{\frac{4+2\epsilon}{\epsilon(2+\epsilon)}} \int_0^t\E\Big[\big| \sigma(s,X_s^{N,i},\bar{\rho}_s^N)-\sigma\big(s,\underbar{X}_s^i,\mathcal{L}(\underbar{X}_s^i)\big) \big|^\delta\Big]\dd s\notag\\
  ...
  \end{equation*}
  In the same way, there is a measurable map $G_N\colon \R^{N\times d}\times C([0,T];\R^m)^N\to C([0,T];\R^d)^N$, such that
  \begin{equation*}
  ...
  \end{equation*}
  Hence, the processes $\big((X^{N,i},\underbar{X}^i)\big)_{1\leq i\leq N}$ are identically distributed, and we can for $i=1$ insert \eqref{ineq2} into \eqref{ineq1} and conclude by Jensen's inequality that
  \begin{align*}
  ...
  \begin{equation}
    \E[|X_t^{N,1}-\underbar{X}_t^1|^\delta] \lesssim \int_0^t \E\bigg[ W_\delta\bigg(\frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1)\bigg)^\delta \bigg]\dd s,\label{ineq:show02}
  \end{equation}
 ...
 for any $s\in[0,T]$ by showing convergence in probability and uniform integrability. By the Glivenko--Cantelli theorem (see \cite[Chapter~26, Theorem~1]{Shorack2009} for a general version) and since the $\underbar{X}^j$ are i.i.d., we get the convergence
  ...
  \end{equation*}
  almost surely and hence in probability. Furthermore, using again the notation $\bar{\rho}^N_s= \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j}$ we can bound using the boundedness of all moments of $\underbar{X}_s^i$, $1\leq i\leq N$, in \eqref{fin_moments}, that
  \begin{align*}
    ...
    &\qquad \lesssim K^{-1} \sup\limits_{N\in\N}\E\big[ W_\delta\big( \bar{\rho}^N_s,\delta_0 \big)^{\delta+1} + W_\delta\big( \delta_0,\mathcal{L}(\underbar{X}_s^1) \big)^{\delta+1} \big]\\
    &\qquad \lesssim K^{-1} \sup\limits_{N\in\N}\E\Big[ \frac{1}{N^{\frac{\delta+1}{\delta}}}\big(\sum\limits_{i=1}^N |\underbar{X}_s^i|^\delta\big)^{\frac{\delta+1}{\delta}} + |\underbar{X}_s^1|^{\delta+1} \Big]\\
    &\qquad \lesssim K^{-1}\to 0,
  \end{align*}
  ...
  %_---------------OLD_------------------------
  %We show that the term under the integral in \eqref{ineq:show0} converges to $0$ with $N\to\infty$, uniformly in $s$. First, we get by \cite[(5.18)]{Carmona2018} that
  %\begin{equation}
  %\lim\limits_{N\to\infty}\E\Big[ W_2\Big(\frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1)\Big)^\delta \Big]=0,\label{convergence_uniform}
  %\end{equation}
  %for any $s\in[0,T]$.
  %OLD----------------------------------------------------------------------------------------
  %by showing convergence in probability and uniform integrability. By the Glivenko--Cantelli theorem (see \cite[Theorem 5.23]{Klenke2014}) and since the $\underbar{X}^j$ are i.i.d., we get the convergence \[\frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j}\to \mathcal{L}(\underbar{X}_s^1)\] almost surely and hence in probability. Further, using again the notation $\bar{\rho}^N_s= \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j}$, we can bound using the boundedness of {\color{blue}the $\delta+1$-}moments of $\underbar{X}_s^i$, $1\leq i\leq N$, in \eqref{fin_moments}, that
  %\begin{align*}
  %&\sup\limits_{N\in\N}\E\big[ W_2\big( \bar{\rho}^N_s,\mathcal{L}(\underbar{X}_s^1) \big)^\delta \mathbbm{1}_{\lbrace W_\delta(\bar{\rho}^N_s,\mathcal{L}(\underbar{X}_s^1))>K \rbrace} \big]\\
  %&\qquad \leq \sup\limits_{N\in\N}\frac{\E\big[ W_\delta\big( \bar{\rho}^N_s,\mathcal{L}(\underbar{X}_s^1) \big)^{\delta+1} \big]}{K}\\
  %&\qquad \lesssim \sup\limits_{N\in\N}\frac{\E\big[ W_\delta\big( \bar{\rho}^N_s,\delta_0 \big)^{\delta+1} + W_\delta\big( \delta_0,\mathcal{L}(\underbar{X}_s^1) \big)^{\delta+1} \big]}{K}\\
  %&\qquad \lesssim \sup\limits_{N\in\N}\frac{\E\Big[ \frac{1}{N^{\frac{\delta+1}{\delta}}}\big(\sum\limits_{i=1}^N |\underbar{X}_s^i|^\delta\big)^{\frac{\delta+1}{\delta}} + |\underbar{X}_s^1|^{\delta+1} \Big]}{K}\\
  %&\qquad \lesssim \frac{1}{K}\to 0,
  %\end{align*}
  %with $K\to\infty$, which shows uniform $\delta$-integrability of the family of random variables \[\bigg(W_\delta\Big(\frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1)\Big)\bigg)_{N\in\N}.\]
  %Hence, the Vitali convergence theorem (see \cite[Theorem~4.5.4]{Bogachev}) implies the $L^\delta$-convergence \eqref{convergence_uniform}.
  %--------------------------------------------------------------------------

  To conclude \textit{Step 3}, it remains to show that the convergence \eqref{convergence_uniform} is uniform in $s$. Therefore, we first notice, for any $s\in[0,T]$, the finiteness of
  \begin{align}
    &\E\bigg[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big)^{p} \bigg]\notag\\
    &\qquad \lesssim \E\bigg[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\delta_0 \Big)^{p} \bigg] +\E\Big[ W_\delta\big( \delta_0,\mathcal{L}(\underbar{X}_s^1) \big)^{p} \Big]\notag\\
    &\qquad \leq \frac{1}{N^{\frac{p}{\delta}}}\E\Big[ \Big(\sum\limits_{j=1}^N |\underbar{X}_s^j|^{\delta}\Big)^{\frac{p}{\delta}} \Big] + \E[|\underbar{X}_s^1|^{p}]\notag\\
    &\qquad =2\E[|\underbar{X}_s^1|^{p}] <\infty,\label{ineq:boundEW}
  \end{align}
  for any $p\geq 1$, by \eqref{fin_moments}. Let $k :=\lceil \delta\rceil\geq\delta$ denote the smallest integer greater or equal to $\delta$. We use the factorization
  \begin{equation}
    a^{k} -b^{k} =(a-b)(a^{k-1}+a^{k-2}b+a^{k-3}b^2+\dots +ab^{k-2}+b^{k-1}),\label{factorization_d}
  \end{equation}
  the inequality
  \begin{equation}
    a^\delta-b^\delta\leq \max\lbrace a^k-b^k,a-b \rbrace,\quad \text{for all }a,b\in\R_+,\label{ineq:abdelta}
  \end{equation}
  and \eqref{ineq:boundEW}, which shows that the expectation of any power of the second summand in \eqref{factorization_d} is finite, such that if $a^\delta$ and $b^\delta$ in \eqref{ineq:abdelta} are of the form of \eqref{ineq:boundEW}, we get the bound
  \begin{align*}
    a^\delta-b^\delta\leq \max\lbrace C_k(a-b),a-b \rbrace\leq (C_k+1)(a-b).
  \end{align*}
  Hence, we can bound, for any $s,t\in[0,T]$,
  \begin{align*}
    &\bigg| \E\Big[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_t^j},\mathcal{L}(\underbar{X}_t^1) \Big)^{\delta} \Big] - \E\Big[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big)^{\delta} \Big] \bigg|\\
    %&\qquad\leq \bigg| \E\bigg[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_t^j},\mathcal{L}(\underbar{X}_t^1) \Big)^k -  W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big)^k \bigg] \bigg|\\
    &\qquad\leq (C_k+1) \E\bigg[\bigg| W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_t^j},\mathcal{L}(\underbar{X}_t^1) \Big) -  W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big) \bigg|\bigg] \\
    &\qquad\lesssim \E\bigg[ \bigg| W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_t^j},\mathcal{L}(\underbar{X}_t^1) \Big) -  W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big)\bigg|^\delta \bigg]^{\frac{1}{\delta}}.
  \end{align*}
  Using the triangle inequality, we continue with
  \begin{align*}
    &\bigg| \E\Big[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_t^j},\mathcal{L}(\underbar{X}_t^1) \Big)^{\delta} \Big] - \E\Big[ W_\delta\Big( \frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1) \Big)^{\delta} \Big] \bigg|\\
  ...
  \end{align*}
  where the last line holds by %\cite[proof of Theorem~1.3]{Wang2008} (see also \cite[Corollary~3.5]{Promel2023} in case of a one-dimensional setting)
  Remark~\ref{rem:2} for any $\beta\in (0,\gamma-1/p)$ with $\gamma\in(0,\frac{1}{2}]$ from Assumption~\ref{ass:kernels1}. Hence, the convergence in \eqref{convergence_uniform} is uniform in $s\in[0,T]$, meaning that
  \begin{equation}
    \lim\limits_{N\to\infty}\sup\limits_{ s\in [0,T]}\E\Big[ W_\delta\Big(\frac{1}{N}\sum\limits_{j=1}^N \delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1)\Big)^\delta \Big]=0,\label{limit0}
  \end{equation}
  which shows together with \eqref{ineq:show0} that
  \begin{equation}
  ...
  \end{align}
  which also tends to $0$ as $N\to\infty$, by \eqref{limit0} and \eqref{limit0_2}.
\end{proof}
...
\begin{proof}[Proof of Theorem~\ref{thm:propagation2}]
  ...
  \end{align*}
  to perform a Yamada--Watanabe approach exactly as we did around equality~\eqref{eqmean}, and obtain for fixed $i\in\lbrace1,\dots,N\rbrace$ with the notation $M^{N,i}(t):=\E[|X_t^{N,i}-\underbar{X}_t^i|]+\E[|\tilde{Y}_t|]$, where $\tilde{Y}_t:=\int_0^t\sigma(s,X_s^{N,i})\dd B_s^i-\int_0^t \sigma(s,\underbar{X}_s^n)\dd B_s^i$, that
  \begin{equation*}
  ...
  \begin{equation}
    \E\big[ W_1(\frac{1}{N}\sum\limits_{j=1}^N\delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1)) \big]\leq \E\big[ W_2(\frac{1}{N}\sum\limits_{j=1}^N\delta_{\underbar{X}_s^j},\mathcal{L}(\underbar{X}_s^1))^2 \big]^{\frac{1}{2}},\label{eq:bound1_propa}
  \end{equation}
...
\begin{lemma}\label{lem1}
  Let $\bar{\rho}^N:=\frac{1}{N}\sum\limits_{i=1}^N \delta_{X^i}$ be the empirical distribution of i.i.d. random variables $(X^i)_{i=1,\dots,N}$ with common distribution $\rho$ such that $\rho\in\mathcal{P}_p(\R^d)$ for every $p\geq 1$. Then, we have
  \begin{equation*}
