...
\begin{abstract} 
In this paper, we introduce Wick's quantization on groups and discuss its links with Kohn-Nirenberg's one. By  quantization, we mean     an operation that associates an operator to a symbol. 
...
studied in \cite{FJFA2015} and  briefly recalled in Section \ref{subsec_pseudoC_compact}. We obtain the following G\aa rding inequality.
...
We also discuss below what the method of proof gives regarding  $(\rho,\delta)$-generalisation of the latter  G\aa rding inequality (see Theorem~\ref{thm_Gardingcomp_rhodelta}).
\smallskip
...
The Sobolev space $L^2_s(G)$ will be the ones adapted to this setting \cite{FR,FRSob}. We obtain the following G\aa rding inequality.
...
We also discuss below what the method of proof gives regarding  $(\rho,\delta)$-generalisation of the latter  G\aa rding inequality (see Theorem~\ref{thm_Gardingnilp_rhodelta}).
\smallskip
...
In Appendix~\ref{app}, we develop the same strategy of proof in the Euclidean case; to our knowledge, the  proof which is the closest to ours is Folland's one~\cite[Chapter 2, Section 6]{folland}.

\medskip

\noindent{\bf Acknowledgements}. Clotilde Fermanian Kammerer thanks the Erwin Schr\"odinger Institut for its hospitality when writing this paper and acknowledge the support of the Connect Talent  Pays de la Loire project {\it HiFrAn} 2022-27. Clotilde Fermanian Kammerer and Veronique Fischer gladly acknowledge the support of  The Leverhulme Trust via Research Project Grant RPG 2020-037. 
...
Let $m\in \bR$ and $1\geq \rho\geq \delta\geq 0$.
If $x_0\in G$, then for any $\sigma\in S_{\rho,\delta}^m(G)$, the symbol $L_{x_0}\sigma  = \{\sigma(x_0x,\pi) : (x,\pi)\in G\times \Ghat)\}$ is in $S_{\rho,\delta}^m(G)$ with 
$$
	...
	Moreover, 
	 	the map
 $(x_0,\sigma) \mapsto L_{x_0} \sigma $ is  continuous $G\times S_{\rho,\delta}^m(G)\to S_{\rho,\delta}^{m+\delta} (G)$. 
\end{lemma}
\begin{proof} Taylor estimate yields for any $(x,\pi)\in G\times \Ghat$,
$$
\| \sigma(xx_0,\pi) - \sigma (x,\pi))\|_{\sL(\cH_\pi)}
\lesssim_G |x_0| \sup_{x'\in G} \| X_{x'} \sigma(x',\pi)\|_{\sL(\cH_\pi)},
$$
where $|x_0|$ denotes the Riemann distance of $x_0\in G$ to the neutral element $e_G$.
More generally for any semi-norm $\|\cdot \|_{S^{m}_{\rho,\delta},a_0,b_0}$, we have
$$
\|L_{x_0}\sigma  - \sigma\|_{S^{m+\delta}_{\rho,\delta},a_0,b_0}
\lesssim_G |x_0|\|\sigma\|_{S^{m}_{\rho,\delta},a_0,b_0+1}.
$$
\end{proof}


This  implies readily the following property of convolution in the $x$-variable of a symbol:
\begin{corollary}
...
denoting by $\varphi *\sigma$ the symbol
	 ...
	 $$
	 we have $\varphi *\sigma \in S_{\rho,\delta}^{m}(G)$ with 
	 $$
	\|\varphi*\sigma \|_{S_{\rho,\delta}^m,a,b} 
	\leq \|\varphi\|_{L^1(G)}\| \sigma\|_{S_{\rho,\delta}^m,a,b}.
	$$
 \end{corollary}
...
When $\delta=0$ and $\rho=1$, the second part of Theorem~\ref{thm_Gardingcomp_rhodelta} is Theorem \ref{thm_Gardingcomp}. Note that when $\delta=0$ and $\rho\in (0,1)$ the term involving $ \| f\| _{H^{\frac{m-\delta}2}(G)}^2$ in the right hand-side of the weak G\aa rding inequality~\eqref{eq:w_g} disappears.
\smallskip 
...
$$
for any continuous function $\psi:G\to \bC$; this extends to  continuous functions $\psi$ that are  Banach valued. 
From this, we  obtain the  properties regarding the approximation of the identity that we will use in our proof of G\aa rding inequalities below and  summarised in the following lemma:

\begin{lemma}
  \label{lem_control}
  Let $\varphi_t$, $t>0$, be an approximation of the identity on $G$. There exists $C>0$ such that for all $t>0$, 
$$
\forall \sigma\in S^{m}_{\rho,\delta}(G),\qquad 
\|\sigma -  \varphi_t*\sigma\|_{S^{m+\delta}_{\rho,\delta}(G),a_0,b_0}
\leq C t \|\sigma\|_{S^{m}_{\rho,\delta},a_0,b_0+1},
$$
for any $a_0,b_0>0$, $m\in \bR$, $1\geq \rho\geq \delta\geq 0$.
\end{lemma}
...
\leq 
\int_G 
\|L_{y^{-1}} \sigma -\sigma\|_{S^{m+\delta }_{\rho,\delta},a_0,b_0} |\varphi(y)|
dy
\lesssim_G \int_G |y||\varphi(y)|dy 
\|\sigma\|_{S^{m}_{\rho,\delta},a_0,b_0+1},$$
by Lemma \ref{lem_translationSm} and its proof. 
Hence, 
we have
$$
\|\varphi_t * \sigma  - \sigma\|
_{S^{m}_{\rho,\delta},a_0,b_0}
\lesssim_G \int_G |y||\varphi_t(y)|dy \ 
\|\sigma\|_{S^{m}_{\rho,\delta},a_0,b_0+1},$$
for any $\sigma\in S^{m }_{\rho,\delta}$. 
\end{proof}
...
-\left| \left(\Op^{\rm KN} (\sigma -  |a|^2*\sigma)  f,f \right)_{L^2(G)}\right|
...
$$
	...
	\geq 0
$$
while the properties of the pseudo-differential calculus (Theorem \ref{thm_calculus_comp}) imply that the operator norm 
$
...
By Lemma~\ref{lem_control}, one can find $t=t(\eta)$ such that 
$$
\left|\left(\Op^{\rm KN} (\sigma -  |a|^2*\sigma )\|_{\sL(L^2(G))} f,f\right)_{L^2(G)} \right|\leq \eta \|f\|_{H^{\frac \delta 2}(G)}.
$$
...
$$
	...
	c \|f\|^2_{L^2(G)}.
$$
One then choose $\eta\leq c$ so that the term $ \eta \|f\|_{H^{\frac \delta 2}(G)}= \eta \|f\|_{L^2(G)}$ is absorbed by $c \|f\|^2_{L^2(G)}$. 
\end{proof}


In the proof above, we observe  that $\|\Op^{\rm KN}(\rho)\|_{\sL(H^{-\frac {\rho-\delta}2}(G),H^{\frac {\rho-\delta}2}(G))}$ depends on our choice of $a:=\sqrt{\varphi_t}$, and thus of $\eta$. 
...
\Re ((\id+\cL)^{-\frac{m}4} \Op^{\rm KN}(\sigma) \,(\id+\cL)^{-\frac{m}4}
 ...
 &\geq -\eta \| f_1\|_{ H^{\frac\delta 2}(G)} -C _\eta \|f_1\|_{H^{-\frac {\rho-\delta}2}(G)},
\end{align*}
...
Before this,  we recall some definitions and notations about this class of groups and the associated pseudo-differential calculus. 
...
Generalising the definition in the compact setting,  the difference operator $\Delta_q$ associated to $q\in \cC^\infty(G)$ is defined as in~ \eqref{eq_RTdiff} via
...
The statement of Lemma \ref{lem_contDelta_q} for difference operators  also holds in the context of graded Lie groups if $a\in \cD(G)$ is replaced with $a\in \cS(G)$. 
...
Let $m\in \bR$ and $1\geq \rho>\delta\geq 0$.
	...
	\Re \left (\Op^{\rm KN}(\sigma) f, f\right )_{L^2(G)} \geq -\eta  \|f\|_{L^2_{\frac{m+\delta}2}(G)}^2 -C_\eta  \|f\|_{L^2_{\frac{m-(\rho-\delta)}2}(G)}^2 .
$$	 
Moreover, if $\delta=0$  and  $\sigma\in S_{\rho,\delta}^{m}(G)$ satisfies the elliptic condition $\sigma \geq c (\id  +\widehat \cR)^{\frac {m}\nu}$, 
		for some constant $c>0$ where $\cR$ is a positive Rockland operator of homogeneous degree $\nu$.
	Then there exists a constant $C>0$ such that 
	$$
	\forall f\in \cS(G),\qquad 
...
$$
\item  There exists $C>0$ such that for all $T>0$
$$
\forall \sigma\in S^m_{\rho,\delta}(G),\qquad 
\|\sigma -  \varphi_t*\sigma\|_{S^{m+\delta}_{\rho,\delta}(G),a_0,b_0}\leq C \, t\, 
\| \sigma\|_{S^{m}_{\rho,\delta}(G),a_0,b_0+1},
$$
for any  $a_0,b_0>0$, $m\in \bR$, $1\geq \rho> \delta\geq 0$.
	\end{enumerate}
\end{lemma}
...
$$	
\|\sigma -  \varphi*\sigma \|_{S^{m+\delta}_{\rho,\delta},a_0,b_0,c_0}
=
...
$$
\sigma_{\alpha,\beta,\gamma}(x,\pi):=\pi(\id+\cR)^{-\frac {m+\delta -\rho [\alpha]+\delta[\beta]+\gamma}{\nu}} X^\beta \Delta^\alpha \sigma(x,\pi) \pi(\id+\cR)^{\frac {\gamma}{\nu}}.
$$
This and   the second estimate in Part (2) extended to Banach valued functions $\psi$ imply 
$$
\|\varphi_t * \sigma  - \sigma\|
_{S^{m+\delta }_{\rho,\delta},a_0,b_0}
\lesssim_{G,\varphi_1} t  
\|\sigma\|_{S^{m}_{\rho,\delta},a_0,b_0+1}.
$$ 
for any $\sigma\in S^{m}_{\rho,\delta}$. 
\end{proof}
...
by  Lemma \ref{lem_appIGnilp} (3),
$$
\left|\left(\Op^{\rm KN} (\sigma -  |a|^2*\sigma ) f, f\right)_{L^2(G)}\right|
\geq \eta   \|f\|_{L^2_{\frac \delta 2}(G)}^2.
$$
 This shows the case of $\sigma\in S_{\rho,\delta}^0(G)$ and  one can conclude the proof of Theorem \ref{thm_Gardingnilp_rhodelta} in a similar manner as for Theorem \ref{thm_Gardingcomp_rhodelta}. 
\end{proof}
...
In this section, we show the semi-classical inequality stated in Theorem \ref{thm:garding_sc}. The proof is inspired by Lemma~1.2 in~\cite{GerLeich93}.
...
&=
\int_G a ( \delta_{\sqrt{\eps}} w^{-1} y)\bar a (y) \kappa_{\delta_{\sqrt{\eps}}{y}^{-1} x}(w) dy,
\end{align*}
\end{lemma}
...
$$
This defines the generalised Bargmann transform $\cB_a = \cB$. The function $a$ is usually chosen as the Gaussian function $a(x)=\pi^{-\frac d4}{\rm e}^{-\frac{|x|^2}{2}} $ \cite{corobook}.  The map $\mathcal B$  is a unitary transformation $L^2(\bR^n)\to L^2(\bR^n\times \bR^n)$.
...
\end{definition}
The space $S^m_{\rho,\delta}(\bR^n)$ is naturally equipped with a structure of Fr\'echet space, with semi-norms:
$\|\sigma\|_{S^m_{\rho,\delta},a,b}=\max_{|\alpha|\leq a, |\beta|\leq b} C_{\alpha,\beta}$ with $C_{\alpha, \beta}$ the best constant as above. 
The resulting class of operators
 $\Psi_{\rho,\delta}^m(\bR^n):=\Op^{\rm KN}(S_{\rho,\delta}^m(\bR^n))$ inherits the structure of Fr\'echet space. 
 Moreover, 
...
then 
$$
\varphi*\sigma:(x,\xi) \longmapsto \int_{\bR^n} \varphi(z)\sigma(x-z,\xi) dz = \varphi*\sigma (\, \cdot\, ,\xi)\, (x), 
$$
...
\label{thm_gardingRn}
Let $m\in \bR$ and $1\geq \rho>\delta\geq 0$.
	Assume that the symbol $\sigma\in S_{\rho,\delta}^{m}(\bR^n)$ satisfies the positivity condition $\sigma(x,\xi) \geq 0$, 
	Then, for all $\eta>0$,  there exists a constant $C_\eta>0$ such that 
	$$
	\forall f\in \cS(\bR^n),\qquad 
	\Re \left (\Op^{\rm KN}(\sigma) f, f\right )_{L^2} \geq -\eta \|f\|_{ H^{\frac {m+\delta}2}} ^2-C_\eta \|f\|_{H^{\frac{m-(\rho-\delta)}2}}^2 .
$$	 
Moreover, if $\delta=0$ and $\sigma\in S_{\rho,0}^{m}(\bR^n)$
satisfies the elliptic condition $\sigma(x,\xi) \geq c$, 
	for some constant $c>0$,
	then, there exists a constant $C>0$ such that 
	$$
	\forall f\in \cS(\bR^n),\qquad 
...
\end{theorem}
 ...
 We will use the following property of approximation of the identity. The proof relies in crucial ways on the fact that the convolution is in $x$ only.
\begin{lemma}
\label{lem_appRn}
	Let $\varphi_1\in \cS(\bR^n)$. Set $\varphi_t (x) := t^{-n} \varphi_1(t^{-1}x)$.
Let $m\in \bR^n$ and $1\geq \rho> \delta\geq 0$. 
	For any $\sigma\in S^m_{\rho,\delta}(\bR^n)$,
	 $ \varphi_t * \sigma$ converges to $\sigma$ in $S^m_{\rho,\delta}(\bR^n)$ as $t\to 0$. This means that given any semi-norm $\|\cdot\|_{S^m_{\rho,\delta},a,b}$, we have
	 $$
	 \lim_{t\to 0 }\|\sigma - \varphi_t * \sigma\|_{S^m_{\rho,\delta},a,b} =0.
	 $$
\end{lemma}
\begin{proof}
	We observe that 
\begin{align*}
|\sigma(x,\xi)-	\varphi_t*\sigma(x,\xi)|
&\leq \int_{\bR^n} |\sigma(x,\xi) -\sigma(x-ty,\xi)| \, |\varphi_1|(y) dy
\\&\leq t \sup_{x'\in \bR^n}|\nabla_x \sigma(x',\xi)| \int_{\bR^n} 	|y| \, |\varphi_1(y)|dy.
\end{align*}
Since
$$
\partial_x^\beta \partial_\xi^\alpha \left(\sigma-	\varphi_t*\sigma\right)(x,\xi) 
=
 \partial_x^\beta \partial_\xi^\alpha\sigma(x,\xi)-	\varphi_t*\partial_x^\beta \partial_\xi^\alpha \sigma(x,\xi),
$$
we have a similar result applying $\partial_x^\beta \partial_\xi^\alpha$.
Consequently, 
if $\sigma\in S^m_{\rho,\delta}(\bR^n)$
$$
\|\sigma-	\varphi_t*\sigma\|_{S^{m+\delta}_{\rho,\delta},a,b} 
\leq C_{\varphi_1} t \|\sigma\|_{S^{m}_{\rho,\delta},a,b+1}, \quad \mbox{with} \ C_{\varphi_1}:=\int_{\bR^n} 	|y||\varphi_1(y)|dy.
$$
\end{proof}
...
&\geq  c \|f\|^2_{L^2} 
- \left|\left(\Op^{\rm KN} (\sigma -  |a|^2*\sigma ) f,f\right)_{L^2}\right|
 -  C
\|f\|^2_{H^{\frac{\rho-\delta}2}},
\end{align*}
for some constant $C>0$. 
We fix  a  function $a_1\in \cS(\bR^n)$ with $\|a_1\|_{L^2(\bR^n)}=1$
and set $a_t(x) :=t^{-n/2} a_1(t^{-1} x)$. 
Applying Lemma \ref{lem_appRn} to $|a_t|^2 = t^{-n}|a_1 (t^{-1}\cdot)|^2$, $t>0$, we  then choose $a=a_t$ with $t>0$ small enough so that  
$$
\left| \left(\Op^{\rm KN} (\sigma -  |a|^2*\sigma )f,f \right)_{L^2(G)}\right|
\leq \eta \| f\| _{H^{\frac \delta 2}}.
$$
The second part of the Theorem is proved as for Theorem~\ref{thm_Gardingcomp_rhodelta}.
\end{proof}
...
\newblock \emph{Duke Math. J.}, \textbf{71}, {1993}, No 2, pp 559--607.
