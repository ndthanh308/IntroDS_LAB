...
In this work, similar to~\cite{Hong2022bridging}, we pre-train our model based on the proposed GridMM in discrete environments
...
Recently, BEVBert\ \cite{Dong2022bevbert}  introduced topo-metric maps from robotics into VLN, which uses topological maps for long-term planning and applies hybrid metric maps for short-term reasoning. Our GridMM aims to perceive more space-time relations with the dynamically growing grid map, which
leverages both temporal and spatial information to depict
the globally visited environment.
%Its metric map divides the local environment around the agent into $21$$\times$$21$ cells, and each cell represents a square region with a side length of $0.5$$m$. Moreover, the short-term visual observations within two steps are mapped into these cells. Our approach differs from BEVBert in three ways. First, the metric map in BEVBert is only a small-size map, which only does short-term reasoning limited by local spatial perception. Our GridMM expands with the expansion of the visited environment, which enables better long-term planning with global spatial perception. Second, the local metric map only memorizes the short-term historical environment, our approach stores all historical observations to better handle time-dependent tasks. Third, BEVBert obtains a grid region representation via average pooling features within  this region, which might introduce noise. Our aggregation method evaluates the relevance of each feature to the instruction, filters out irrelevant features, and captures critical clues.
...
We predict a local navigation score for each candidate views
...
WS-MGridMM\textcolor{black}{${\color{red}^{{\color{black}\ast}}}$}\ \cite{chen2022weakly} & 10.12 & 5.65 & 51.7 & 46.9 & 43.4 & 10.00 & 6.28 & 47.6 & 38.9 & 34.3 & 12.30 & 7.11 & 45 & 35 & 28\tabularnewline
...
We compare the performance of different maps representing the visited environments on the val unseen split of the R2R-CE dataset. % and some analyses of our approach are given.
\vspace{-3ex}
...
Grants 62125207, 62102400, 62272436, U1936203, and
U20B2052, in part by the National Postdoctoral Program
for Innovative Talents under Grant BX20200338, and in
part by the Peng Cheng Laboratory Research Project under
Grant PCL2021A07.
...
\subsection{Datasets}\label{sec:sup_datasets}
We extensively evaluate our approach in discrete environments (\textit{e.g.}, R2R~\cite{VLN-2018vision}, REVERIE~\cite{2020reverie}, SOON~\cite{zhu2021soon}), and further analyze many characteristics of our approach in continuous environments (\textit{e.g.}, R2R-CE~\cite{Krantz2020r2r-ce}).
...
R2R-CE~\cite{Krantz2020r2r-ce} transfers the discrete paths in R2R dataset to continuous trajectories
based on the Habitat simulator~\cite{2019habitat}. Our splits in R2R-CE are consistent with the ones adopted by Hong \textit{et al.}~\cite{Hong2022bridging}.
...
As illustrated in Fig.~\ref{fig:model_details}, we present the detailed architecture for action prediction. We present the following descriptions:\ \textbf{a)} For the symbol definition in Fig.~\ref{fig:framework}, $\mathcal{M}_{t}$ is the set of extracted features, and $\mathcal{M}_{t}^{rel}$ is the projected features with relative coordinates. $\mathcal{M}_{t,m,n}^{rel}$  is a subset of $\mathcal{M}_{t}^{rel}$ within a cell, and ${M}_{t}$ is the obtained map features after aggregation. 
\ \textbf{b)} For loss functions, $MLM$ and $MVM$ are employed in the same ways as previous works~\cite{chen2022think-GL} (which are omitted in Fig.~\ref{fig:framework}). The $SAP$ loss and $HER$ loss are clearly described in Sec.~\ref{sec:cross-modal_reasoning} and Sec.~\ref{sec:action_prediction}. \ \textbf{c)} The candidate views are part of the agent's current panoramic observation as candidates for local action prediction. But the candidate waypoints are candidate locations in the global grid map for global action prediction. %Obviously, they are not the same thing. 
\textbf{d)} We use ``Dynamic Fusion" to gated fuse these two action scores following DUET~\cite{chen2022think-GL}. 


\subsection{Experimental Details}\label{sec:sup_experimental_setups}
...
\subsubsection{Ablation Details}
...
\begin{tabular}{cccc}
\hline 
R2R & REVERIE & SOON & R2R-CE\tabularnewline
\hline 
41.51\% & 42.65\% & 47.22\% & 51.62\%\tabularnewline
\hline
\end{tabular}
\vspace{2pt}
\caption{The ratio of cell regions that contain grid features on different datasets.}\label{cell_ratio}
\end{table}


As shown in Table~\ref{cell_ratio}, almost half of the cell regions in GridMM do not contain any grid features, due to the visual occlusion of indoor objects and irregular room structure. Therefore, we only use cell regions that contain grid features for less computational cost.

\vspace{0.4in}
\subsection{Analysis of Computational Cost}\label{sec:computational_cost_analyses}
...
\subsection{Inherent differences between our GridMM and BEVBert}
Based on visual features from CLIP~\cite{radford2021learning}, BEVBert~\cite{Dong2022bevbert} proposes a hybrid semantic topo-metric map scheme for VLN, where a metric map is employed for short-term reasoning. However, our GridMM is completely different in terms of: \textbf{(1)} 
...
We visualize a sample from the R2R dataset to illustrate the dynamic growth of the Grid Memory Map. Each image contains the instruction, the forward-facing observation of the agent, and the visualization of the Grid Memory Map.
\vspace{-0.1in}
% Figure environment removed
