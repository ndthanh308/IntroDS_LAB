...
\begin{proof}[Proof of Lemma \ref{lem:complete-description-property-main_text}]
    ...
    \end{enumerate}
    Therefore, the lemma is true in all the cases. \qed{ Lemma \ref{lem:complete-description-property-main_text}}
...
\begin{proof}
    ...
    Therefore we have
     $$-|g_i(x)-f^*(x)|\leq \hat{f}_S(x)-f^*(x) \leq |g_{i+1}(x)-f^*(x)| \leq |g_{i-1}(x)-f^*(x)|,$$
     and the lemma follows. The proof is exactly symmetric for when $\curv(x_i)=\curv(x_i)=+1$; the only difference is that we apply the same argument on $-g_i,-g_{i-1},-g_{i+1}$ and $-\hat{f}_S$ and add the function $f^*(x)$ instead of subtracting. \qed{ Lemma \ref{lem:bound on |f^-f*|}}
