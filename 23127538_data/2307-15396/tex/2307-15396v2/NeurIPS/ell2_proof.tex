...
Finally, if $\mu<0$, then apply the same argument to $-\mu$ giving the desired result.\qed{ Claim \ref{clm:minimum}}
This concludes the proof of the lemma.
\qed{ Lemma \ref{lem: Lp>Rp}}
...
Then by doing the above analysis as in \eqref{eq:lb-one-interval-risk-p(1,2)} and \eqref{eq:lb-one-interval-risk-p>2} for each $i \in N$, along with the observation that the risk in any $[x_{i,3},x_{i,4}]$ is non-negative, we obtain the desired result. In particular, for $1\leq p <2:$
 $$\mathcal{R}_p(\hat{f}_S)\geq \sum_{i=1}^{N}\frac{\ell_{i,3}^{p+1}}{(\ell_{i,2}+\ell_{i,4})^{p}} \mathbbm{1}[A_i];$$
    And, for $p \geq 2:$
        $$\mathcal{R}_p(\hat{f}_S) \geq \sum_{i=1}^{N} \frac{\ell_{i,3}^3}{(\ell_{i,2}+\ell_{i,4})^{2}} \mathbbm{1}[A_i]. \qed{ \text{ Lemma \ref{lem:risk-when-Ai-happens}}}$$
...
\end{lemma}
\begin{proof}
    The events $A_1,\dots, A_N$ are independent since the noises are i.i.d. Also, the event about the interval lengths (that are dependent random variables) in the above $\{ \ell_{i,3} \geq \ell_{i,2} \} \cap \{ \ell_{i,3} \geq \ell_{i,4} \}$ can be expressed in terms of the underlying exponential random variables (which are independent) as $\{ X_{i,3} \geq X_{i,2} \} \cap \{ X_{i,3} \geq X_{i,4} \}$. Finally, it is easy to observe that for any $i\in [N]$, $\prob{A_i}$ is some universal constant $c_0$. \qed{ Lemma \ref{lem:A_i-are-iid}}
...
$$\lim_{n\rightarrow \infty} \Pr_S \left[ \mathcal{L}_p(\hat{f}_S) \geq \frac{c}{(2-p)} \, \mathcal{L}_p(f^*)\right ]=1. \qed{\text{ Theorem } \ref{thm:tempered-for-ell_1-lower_bound}}$$
...
$$\lim_{n \rightarrow \infty} \Pr_S [ \mathcal{L}_p(\hat{f}_S)> b]=1. \qed{ \text{ Theorem \ref{thm:ell2-catastrophic}}}$$
