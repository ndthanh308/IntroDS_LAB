...
which is solved to show that the size distributions are close to log-normal size distributions 
...
Likewise it is essential for technological purposes where time dependent 
kernels have been suggested to engineer 3d printing for in medicine 
...
The conditions define the aggregation kernels. 
The methods derived here should be applicable to some of those, although 
the general situation is not analyzed in detail. 
The approach is that of mean field theory where, at a given time, a single 
concentration for each particle size describes the growth. 
...
themselves to exact analytical solutions.
Of special interest here is the solution for the size independent kernel used 
by Smoluchowski, yielding a single exponential decay.
...
in any case assume without any further justification that they are symmetric 
...
\cite{AldousBernoulli1999}, and the theory is therefore a mean field theory.  
...
\ee
The geometric capture cross section assumed here is calculated with the sum of
the radii of the two colliding particles.
The radii are proportional to the cube root of the particle number, reflecting a 
constant density for all sizes.
The relative speed is the average thermal speed of the particles in thermal 
equilibrium at temperature $T$.
This is calculated as the average value of a single particle with the reduced 
mass \cite{L&Lstatphys}.
...
\ee
For the right hand side derivatives, the aggregation Smoluchowski equation, 
Eq. (\ref{smo-gain}) is used.
Inserting it gives 
\begin{alignat}{2}
&~~~~~~~~\frac{\der \olsi{N}}{\der \tau}= \nonumber\\
...
Clearly, different kernels with similar properties can be analyzed similarly.
...
The approximate calculation of this result suggests that a check with a numerical
...
The main difference from the predicted value in Eq. (\ref{smo-mean-approx}) 
is a deviation from the predicted multiplicative factor of only 16 \%.
This must be considered to be a surprisingly good agreement given the 
level of approximation in the derivation.
...
{f\(\frac{N}{\olsi{N}} \)},
\end{equation}
The left hand side of this equation does not depend on $N$ and the right hand 
side not on time. 
Hence a separation of variables has been achieved, making both sides equal to 
the same constant. 
We have already calculated this constant to be $2\sqrt{2}$ in 
Eq. (\ref{meanvalue}). 
It is of interest to note that the specific kernel is manifested only in the power
1/6.
...
This demonstrates that there are solutions to the aggregation-only Smoluchowski
equation that scale according to the Ansatz in Eq. (\ref{n-scaled}), and that the 
mean value varies as in Eq. (\ref{smo-mean-approx}) when the kernels obey 
Eq. (\ref{a-coeff}). 

\section{Solution of the scaled equation}
...
The analysis so far does not provide the criterion for choosing either
of these two forms of solutions.
This is accomplished by considering the time development of the monomer
which, from Eq. \ref{c-deriv} we have 
\be
\label{monomerderiv}
...
\ee
Clearly this is not consistent with the pure exponential behavior and we
can assign the log-normal distributions to the physical kernels of interest here.

The constants of integration $a, s$ and $N_0$ have relations determined by 
mass conservation and the known time dependence of the mean size.
...
\ee
The relation between mean and peak values is known for a log-normal 
distribution and allows us to express the loss term as 
\be
\label{lossterm}
&&-c_{N_0} \sum_{i=1}^{\infty} a_{N_0,i} c_i \approx\nonumber\\
...
The spectra were calculated numerically with a brute force solution of the
equations after discretization of the time.
