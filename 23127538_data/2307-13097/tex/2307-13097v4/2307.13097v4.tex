...
\]
for arbitrary $ H, $ where $ A $ is positive definite. Note that $ \|\cdot\|_q $ is a norm if $ q>0, $ and that $ \Upsilon_{p,q}(A) $ is positively homogeneous of degree $ q. $ Suppose that  $ \Upsilon_{p,q}(A) $ is convex (respectively concave). It then follows from the theory of convexity, see for example \cite[Proposition 1]{hansen:2017:2}, that if $ q>0 $ then $ \Upsilon_{p,q}(A)^{1/q} $ is also convex (respectively concave). If on the other hand $ q<0, $ then  $ \Upsilon_{p,q}(A)^{1/q} $ is concave (respectively convex). By a general amplification argument we obtain that the function
\begin{equation}
\label{the trace function Phi}
\Phi_{p,q}(A_1,\dots,A_k)=\left\lVert\left(\sum_{i=1}^k A_i^p\right)^{1/p} \right\rVert_q
\end{equation}
is convex/concave for the same parameter values $ p,q $ as  $ \Upsilon_{p,q}(A)^{1/q}\,. $  Indeed,  consider $ k\times k $ block matrices
\[
\hat A=\begin{pmatrix}
     A_1     & 0     & \cdots   & 0\\
     0         & A_2 &             & 0\\
...
\[
\begin{array}{rl}
\Upsilon_{p,q}(\hat A)^{1/q}&=\displaystyle\Bigl(\tr\bigl(\hat H^* \hat A^p \hat H\bigr)^{q/p}\Bigr)^{1/q}=\left(\tr\left(\sum_{i=1}^k A_i^p\right)^{q/p}\right)^{1/q}\\[2ex]
&=\Phi_{p,q}(A_1,\dots,A_k),
\end{array}
\]
the statement follows. Bekjan \cite[Lemma 1]{Bekjan:2004} proved that $ \Phi_{p,q}(A_1,\dots,A_k) $ is concave for $ -1\le p\le 0 $ and $ q=1. $ We may summerise these results as follows.

\begin{theorem}

The trace function $  \Phi_{p,q} $ defined in (\ref{the trace function Phi}) has the following geometric properties depending on the parameters $ p $ and $ q. $

\begin{enumerate}[(i)]

\item $ \Phi_{p,q}(A) $ is concave in positive definite $ A $ for the parameter values
\[
\begin{array}{rlllll}

 -1&\le p\le 1\quad &\text{and}\quad  &0\le q\le 1,\\[1ex] 
 
  -1&\le p\le 0  \quad &\text{and}\quad  &q\le p\text{\hskip 0.8em or \hskip 0.8em} 2q\le p\le q, \\[1ex]
0&\le p\le 1 \quad &\text{and}\quad  &q\le 0.
...
\item $ \Phi_{p,q}(A) $ is convex in positive definite $ A $ for the parameter values
\[
1\le p\le 2  \quad \text{and}\quad  q\ge 1.
\]
...
\bibitem{hansen:2017:2}
Frank Hansen, Jin Liang, and Guanghua Shi.
\newblock Peierls-\uppercase{B}ogolyubov's inequality for deformed
  exponentials.
\newblock {\em Entropy}, 19(271):e19060271, 2017.
