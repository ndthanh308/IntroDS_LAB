...
\section{Proof Theorem \ref{th:biased_AC_SA}} \label{Appendix:proof_th1}
    ...
    \begin{algorithm}
        ...
        \hspace*{\algorithmicindent} Let stepsize $ \eta_k > 0$, parameters $\beta_k, \gamma > 0$, batch size $B \in \mathbb{Z}_+$. 
        \begin{algorithmic}[1]
        \For{$k=0,...,N-1$}
        ...
        \State $\tilde{x}_{k+1} = x_k - \eta_k \gg_k^B(x_k^{md})$, where $\gg_k^B(x_k^{md})$ is defined from \eqref{eq:batched_gradient_oracle}
...
\allowdisplaybreaks
    ...
    \begin{align}
        \norms{\expect{\gg(x_k,\xi,e)} - \nabla f(x_k)} &= \norms{\expect{\frac{d}{2 \tau}\left( f_\delta(x_k + \tau e, \xi) - f_\delta(x_k - \tau e) \right) e} - \nabla f(x_k)}
        \nonumber \\
