...
\begin{equation} \label{eq:gradient_approximation}
    \gg(x,\xi,e) = \frac{d}{2 \tau}\left( f_\delta(x + \tau e, \xi) - f_\delta(x - \tau e) \right) e,
\end{equation}
...
\begin{algorithm}
        ...
        \hspace*{\algorithmicindent} Let stepsize $ \eta_k > 0$, parameters $\beta_k, \tau > 0$, batch size $B \in \mathbb{Z}_+$. 
        \begin{algorithmic}[1]
        \For{$k=0,...,N-1$}
        \State $\beta_k = 1 + \frac{k}{6}$ and $\eta_k = \eta (k+1)$ for $\gamma = \min \left\{ \frac{1}{12 L}, \frac{B}{24 L(N+1)}, \sqrt{\frac{B R^2}{Lf^* N^3}} \right\}$  
        ...
        \State $\tilde{x}_{k+1} = x_k - \eta_k \gg_k$
        \State $x_{k+1} = \min \left\{ 1, \frac{R}{\norms{\tilde{x}_{k+1}}} \right\} \tilde{x}_{k+1}$
        \State $x_{k+1}^{ag} = \beta^{-1}_k x_{k+1} + (1 - \beta^{-1}_k) x_{k+1}^{ag}$
        \EndFor
        \end{algorithmic}
        \textbf{Output}: $x_N^{ag}$.
    \end{algorithm}
...
(smoothing parameter), $\eta = 0.0001$ (step size),
