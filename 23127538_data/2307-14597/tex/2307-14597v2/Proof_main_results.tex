...
$\sigma_n$, $\tau_n$ are defined as in \eqref{eq:stopping_times}{, but for the process $(X_t^\e,\xi_t^\e)$ on $M\times\mathbb T^m$}, and 
...
\end{align*}
One can easily check that this is consistent with the definitions of $\bar A$ and $\bar B_c$ in \eqref{eq:definition_of_operator_AB}, which are the generalizations of the coefficients defined in \eqref{def:llimit_operator} and, moreover,
\begin{equation}
...
\end{lemma}
\begin{proof}
    {This is the place where the gluing condition \eqref{eq2:gluing_condition} plays a role.}
