\subsection{Averaging principle starting from \texorpdfstring{$\gamma(\e^\alpha)$}{}}
Fix $0<\alpha<\alpha_1<\alpha_2<1/2$, $r>0$ small enough. 
Recall $Q(h)$ is the rotation time of $\bm x_t$ on $\gamma(h)$.
%In this section, we consider the behavior in the region $U(0,2r)$.
Our first lemma in this section concerns the typical deviation during one rotation.
\begin{lemma}
...
\end{lemma}
\begin{proof}
    It suffices to prove the result for $\delta<1/2-\alpha_1$. Fix $0<\delta'<\delta''<1/2-\alpha_1-\delta$. Recall the definition of $q$ in Section~\ref{sec:Averaging principle before} and consider the coordinates $H$ and $q$ in $U(\e^{\alpha_2},2r)$. As in \eqref{eq:H} and \eqref{eq:phi}, let $q_0=q(x)$, $u_h=u\cdot\nabla H$, $u_q=u\cdot\nabla q$, and $q_t=q_0+t$, and write the equations with $\tau^0=\inf\{t:|\tbh_t^\e-h_0|>\e^{1/2-\delta}~\mathrm{or}~|\tbq_t^\e-q_t|>\e^{\delta''}\}\wedge Q(h_0)$:
    %-----------------------------------------------------------------------------------
    % Here q must be understood continouously beyond one rotation. The bounds on q and gradient is still true even after one rotation.
    %-----------------------------------------------------------------------------------
    \begin{align}
...
%The proof follows from that of Lemma~\ref{lem:throughsaddlepoint} and $G^\e_t$ is of order $\e^\alpha$.
    %The previous proof says that during one rotation, change in $\varphi$ cannot be more than $1$. But that was $\sqrt{\e}$ close. Now if we consider $\e^\alpha$ close with $\alpha<1/2$, we should be able to make a stronger claim. ( Directly change the order of G in the variance expression)
\end{proof}
