...
A technical result is needed to verify \eqref{eq:mg_problem_M} for the original process $(X_t^\e,\xi_t^\e)$ on $M\times\mathbb T^m$.
\begin{lemma}
...
\end{align}
 ...
 %    we have
 %   \begin{equation}
 %       \E_{(x,y)}|\sum_{n=0}^\infty\chi_{\{\sigma_n<{\sigma'}\}}[f(h((X_{\tau_{n+1}'}^\e))-f(h((X_{\sigma_n}^\e))|\leq 2\sup |f'(O)|\e^\alpha\sum_{n=0}^\infty\Prob_{(x,y)}(\sigma_n<\rho)
 %   \end{equation}
...
%($\sigma'$ is the last one before $\rho-\kappa$, $\kappa$ depends on the previous arrival)
\begin{equation}
\label{eq:secondsum3}
