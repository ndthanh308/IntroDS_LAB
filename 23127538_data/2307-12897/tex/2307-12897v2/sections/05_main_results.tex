...
Similar treatment can be applied to cases where the oracle uses other (sublinear) polices for solving linear or contextual bandits \citep[e.g.,][]{thompson1933likelihood,kaufmann2012bayesian,agarwal2014taming}. In either case, agent $j^\star$ calculates a ridge estimate of the coefficients vector based on the history $H_{t}$
 \vspaceequation
\begin{align*}
 \vspaceequation
...
% We use the word hallucinate, since for misspecified agents, $u_{t,j}$ may not be a uniformly valid upper bound. However, for any agent that uses a correct model, e.g., $j^\star$, 
...
\end{equation}
which is greater than zero under the conditions specified in our problem setting. Prior works in the sparse bandit literature all require a similar or stronger assumption of this kind, and \cref{tab:sparse_bandits} gives an overview. 
...
\end{theorem}
% ADD SOMETHING ABOUT THE FACT THAT THE THEOREM ABOVE DOESN"T CARE WHICH UCB?GREEDY COMBINATION WE USE, HAS SAME RATE EITHERWAY
\looseness-1 
% The first three terms in the bound correspond to the regret due to selecting sub-optimal models: 
...
In \cref{cor:alexp_unif}, we provide a regret bound independent of $\cmin$, for orthogonal feature maps, and show that the same $\calO(\max\{ \sqrt{n\log^3 M}, n^{3/4}\sqrt{\log M}\})$ rate may be achieved even with the simple choice $\pi = \mathrm{Unif}(\calX)$.%, instead of the minimizer of \eqref{eq:cmin_def}.
...
% , \cref{fig:virtual_regret} compares virtual regret with regret of the agent when run in isolation and regret of \alexp.
...
% Since \smash{$\hat r_{t,j} = \E_{\vx \sim p_{t,j}} \hat \vtheta_t^\top\vphi_j(\vx)$}, 
...
% {\color{blue}The last term (IV), we notice that it is a bounded Martingale difference sequence and apply anytime azuma-hoeffding (\cref{lem:anytime_azuma}), this term is $\calO(\sqrt{n\log 1/\delta})$.}
...
% \begin{lemma}(Model-Selection Regret, Informal)\label{thm:anytime_regret_MS} Let $\delta \in (0, 1]$.
% Suppose $\eta_t = \calO(\cmin/\sqrt{t}C(M,\delta,d))$ and $\gamma_t = \calO(t^{-1/4})$ and 
%         $\lambda_t  = \calO(\cmin C(M, \delta, d)/\sqrt{t})$,
%     then for all $j = 1, \dots, M$,
% \begin{align*}
% \sP \left(\forall n \geq 1:  R(n,j)  = \calO\left( n^{3/4}(B + C(M,\delta,d))+\cmin^{-1}\sqrt{n}\log M \right)\right) \geq 1-\delta
% \end{align*}
% where $C(M,\delta,d)$ is as defined in \cref{thm:regret_main}.
% \end{lemma}
...
\begin{theorem}[Anytime Lasso Confidence Sequences] Consider the data model $y_t = \vtheta^\top \vphi(\vx_t) + \varepsilon_t$ for all $ t \geq 1$, where $\varepsilon_t$ is i.i.d.~zero-mean sub-Gaussian noise, and $(\vx_t)_{t\geq 1}$ is $(\calF_{t})_{t\geq 1}$-predictable, where $\calF_{t}\coloneqq \left(\vx_1, \dots, \vx_t, \varepsilon_1, \dots, \varepsilon_{t-1}\right)$. \label{thm:anytime_lasso}
    % Suppose \cref{cond:betamin} holds. 
...
% \citet{kim2019doubly} employ the bound of \citet{bastani2020online}. \citet{hao2020high} use explore-then-commit, which results in i.i.d.~data for the LASSO estimator.
...
% \cref{app:exp4_background} serves as a primer on the exponential weights algorithm and its rich history. 
...
% This is enabled by our novel time-uniform analysis of the Lasso.
% Achieving horizon-independent high-probability bounds for \textsc{EXP4} requires a novel time-uniform analysis of the Lasso, presented in \cref{thm:anytime_lasso}.
...
% Instead of relying on a forced exploration stage, as it is common to sparse bandits \citep{kim2019doubly, hao2020high}, \alexp has adaptive exploration which vanishes at a $\gamma_t$ rate.
% Sparse bandits need to acquire sufficient evidence which allows them to commit to a model, in contrast, we do not settle with a model and always randomly select them according to our exponential weighting scheme.
% Moreover, sparse bandits commonly emulate Lasso analysis fixed design, given i.i.d.~data or a martingale sequence with known length \citep{ hao2020high, bastani2020online}. Instead, we employ the ``stitched'' uniform martingale bounds of \citet{howard2020time} to present the first time-uniform analysis of online Lasso regression (\cref{thm:anytime_lasso}). This allows us to explore adaptively, and not have to rely on existence of a certain number of i.i.d.~samples. 

%  In contrast to previous work on sparse bandits, or model selection for bandits, \alexp can be stopped at any time with valid guarantees. We owe this again, to our anytime Lasso analysis, and horizon-independent analysis of the exponential weights algorithm.
% In short, standard analysis of sparse bandits, requires 1) knowledge of horizon 2) a long enough sequence of random samples. \alexp removes both of this constraints, and presents a fully adaptive algorithm. 
% Explore-Then-Commit algorithms are horizon-dependent in nature, since the number of the exploratory samples has to be determined based on the horizon for achieving sublinear regret.
...
% However, the Lasso estimator requires diverse data for the aforementioned rate of bias and variance, particularly in time-uniform setting, which comes at the cost of sub-optimal dependency in $n$.


%=============================== More properties, but probably too niche/irrelevant to mention

% \textbf{High Probability Guarantee}
% % Another benefit of the algorithm structure is that for model selection we benefit from bandit to full-information conversion, which allows for high-probability guarantee. With bandit feedback this is generally tough to show.
% High probability regret bounds in adversarial setting are more challenging that expected regret bounds \citep{}. Due to complex dependencies and shared history among the agents, our online optimization problem is in adversarial setting, connecting us to this literature.
% \citet{} considers finite domain and bandit feedback, and proposes a biased reward estimator with low variance, combined with \textsc{EXP3.P}. More related work: 

% In contrast, previous work use Lasso for implicit model selection \citep{hao2020high, kim2019doubly, bastani2020online, oh2021sparsity, li2022simple}. Based on the sparsity parameter of the Lasso estimator, and once enough samples are gathered $t \geq t_0$, these methods can guarantee that with a high-probability a model is correct, but no distribution over the model class can be inferred from this. From a practical perspective, a distribution is a more flexible and interpretable solution to the model selection problem.
