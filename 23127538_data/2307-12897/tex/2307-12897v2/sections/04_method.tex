...
% Many algorithms have been proposed for this classic construction, e.g. Multiplicative Weights or Exponential Weights which sets \smash{$q_{t,j} \propto \exp(\sum_{s=1}^t r(\vx_{s,j}))$} \citep{cesa2006prediction} and achieves a $\sqrt{T}\log M$ model selection regret.
...
We propose the \textsc{\textbf{A}}nytime \textsc{\textbf{Exp}}onential weighting algorithm based on \textsc{\textbf{L}}asso reward estimates (\alexp),  summarized in \cref{alg:lassoexp}.
  % To obtain an any-time sample efficient algorithm, we design a two-part decision-making procedure.
  At step $t$ we first sample an agent $j_t$, and then sample an action $\vx_t$ according to the agent's policy $p_{t, j_t}$. % (action-selection module). 
  % Therefore, actions are overall selected according to the distribution $\vx_t \sim \sum_{j=1}^M q_{t,j}p_{t,j}$. 
  %can remove last line.
...
%with exponential weighting, proportional to $\hat r_{t,j}$
\[
...
This rule allows us to imitate the full-information example that we mentioned above. By utilizing $\hat r_{t,j}$ and hallucinating feedback from all agents, we can reduce the probability of selecting a badly performing agent, without ever having sampled them (c.f. \cref{fig:prob_curves}). 
 ...
 We concatenate all feature maps, and, knowing that many features are redundant, use a sparsity inducing estimator over the resulting coefficients vector. Mainly, let $\vtheta = (\vtheta_1, \dots, \vtheta_M) \in \sR^{Md}$ be the concatenated coefficients vector. We then solve
...
Problem \eqref{eq:glasso} is the online variant of the group Lasso \citep{lounici2011oracle}. 
The second term is the loss is the mixed $(2,1)$-norm of $\vtheta$, which can be seen as the $\ell_1$-norm of the vector $(\norm{\vtheta_1}, \dots, \norm{\vtheta_M}) \in \sR^M$. This norm induces sparsity at the group level, and therefore, the sub-vectors \smash{$\hat\vtheta_{t,j} \in \sR^{d}$} that correspond to redundant feature maps are expected to be $\boldsymbol{0}$, i.e. the null vector. 
...
% As an example, substituting the Lasso with a ridge or least squares regression, will lead to a $\sqrt{M}$ dependency. 
% This is due to the fact that while $\hat\vtheta_t$ is $Md$-dimensional, its bias$^2$ and variance scale with $\sqrt{d\log M}$. 
...
We mix $p_{t,j}$ with $\pi$, to collect sufficiently diverse data for model selection. We are not restricting the agents' policy, and therefore can not rely on them to explore adequately. % balance exploration and exploitation. 
