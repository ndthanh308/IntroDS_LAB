...
\hline
% \makecell{MS for online\\Supervised Learning\\e.g. \citet{foster2017parameter}}& $\log M$ & \cmark  & \cmark & EXP4 on SE&\xmark & \cmark \\
% \hline
{\small\makecell{MS for Black-\\Box Bandits}}& {\small \makecell{\textsc{OMD} with\\bandit feedback}} & $\mathrm{poly} M$ & \cmark  &  \cmark & \xmark \\
\hline
% \makecell{Meta-Learning for High-dimensional Bandits} & \makecell{$\log M$\\$\mathrm{Poly} M$} & \xmark & \xmark & 
% \makecell{Learning with\\Expert Advice\\e.g. \citet{auer2002nonstochastic}} & $\log M$ & \cmark & \xmark & \makecell{\small full-information\\ EXP4} & \cmark & \cmark\\
% \hline
...
% \alexp is an algorithm for simultaneous online model selection and bandit optimization, designed for linearly reward functions. \cref{tab:general_overview} shows how we connect the two prior areas of research.

% every agent that is not chosen at a step receives no data at that step.
...
These works assume knowledge of the horizon, however, we suspect that this may be lifted with a finer analysis of OMD. % would result in time-uniform extensions.
...
In contrast to prior literature, we perform model selection with an online optimizer (\textsc{EXP4}), which hallucinates full-information feedback using a low-variance Lasso estimator instead of importance-weighted estimates. %, which results in exponential improvement in the regret.
...
% The focus is on regret with respect to the performance of the best agent in hindsight, and not the optimal action.
...
% (Has weaker adaptation of EXP4 than us: uses LS but variance grows with $\min(K, Md)$, the assume $K$ is small and give a $\sqrt{K}$ bounds \dots
% These have the $\log M$, however, they don't implicitly perform model selection and down use a sparsity inducing regression oracle, they only consider finite-armed case/action set that can be reduced to finite arm.
%  % \citet{moradipari2022feature} repeat the result of \citet{foster2020beyond}, with an emphasis of feature and parameter selection.
