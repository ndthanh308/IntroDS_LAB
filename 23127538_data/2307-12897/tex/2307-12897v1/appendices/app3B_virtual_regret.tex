...
\end{equation}
therefore, there exists $C_3$ such that
\begin{align*}
...
\begin{align*}
    \omega(t, \delta, d) = C_4\sqrt{\tfrac{\sigma^2d\log\left(\frac{t}{\tilde\lambda d}+1\right) + 2\sigma^2\log(1/\delta) + \tilde\lambda B^2}{\tilde \lambda + \cmin t^{3/4}}}\left( 1+ \cmin^{-1}t^{-3/8} \sqrt{\log(Md/\delta) + (\log\log t)_+}\right)
\end{align*}
where $C_4$ is an absolute constant. The upper confidence bound then simply follows: for any $\vx\in \calX$
\begin{align*}
