...
Equivalent to \cref{cond:compatibility}, we write $\kappa(\Phi_t, s)$ as \smash{$\kappa(\Phi_t, s) = \inf_{b \in \Xi_s} \|\Phi_t \vb\|_2/\sqrt{t}$} where 
...
\resizebox{0.91\hsize}{!}{%
    ...
    $}
\end{equation}
Using the set $\Xi_s$ we define
\begin{equation} \label{eq:def_tilde_kappa}
    \tilde \kappa(\Sigma, s) \coloneqq \min_{b \in \Xi_s} b^\top \Sigma b.
\end{equation}
and use in this section for simplicity.

\textbf{Step \textsc{I}.} Consider the exploratory steps at which $\alpha_t = 1$.
Let $\Phi_{\pi,t}$ be a sub-matrix of $\Phi_t$ where only rows from exploratory steps are included. Note that $\Phi_{\pi,t} \in \sR^{t'\times dM}$ is a random matrix, where $t'$ the number of rows are also random. We show that $\kappa^2(\Phi_t, s)$ is lower bounded by $\kappa^2(\Phi_{t,\pi}, s)$.
...
\begin{lemma}\label{lem:kappa_exp_to_kappa}
        ...
        \]
\end{lemma}
 While the number of rows of $\Phi_{\pi,t}$ is a random variable, we condition on the event that $\Phi_{\pi,t}$ has $t'$ rows, and investigate the distribution of its restricted eigenvalues.
 
\textbf{Step \textsc{II}.} The restricted eigenvalues of the {\em exploratory} sub-matrix are well bounded away from zero. 
...
\[
\sP \left(\forall t':\,  \kappa^2(\Phi_{\pi, t},s)  \geq \tilde \kappa(\Sigma, s) - \frac{80s}{\sqrt{t'}} \sqrt{(\log \log 4t')_+ + \log(2Md/ \delta)} \right) \geq 1-\delta
\]
where $\tilde \kappa(\Sigma, s)$ is defined in \eqref{eq:def_tilde_kappa}.
\end{lemma}

\textbf{Step \textsc{III}.} 
Remains to combine the two lemmas above and incorporate a high probability bound on $t'$, showing that it is close to $\sum_{s=1}^t \gamma_s$.

\begin{lemma}\label{lem:kappa_true_emp}
Let $\pi$ be the solution to \eqref{eq:cmin_def}. There exists absolute constants $C_1, C_2$ which satisfy,
    \[
        ...
        \]
if $\gamma_t = \calO(t^{-1/4})$, as prescribed by \cref{thm:regret_main}. It further holds that,
    \[
...
For the last lemma in this section we show that the empirical sub-matrices $\Phi_{t,j}$ are also bounded away from zero. This will be required later for proving \cref{thm:virtual_regret_rigorous}.
\begin{lemma}[Base Model $\lambda_{\mathrm{min}}$ Bound]\label{lem:lambda_min_virtual}
    Assume $\pi$ is the maximizer of \cref{eq:cmin_def}.  Then, with probability greater than $1-\delta$, simultaneously for all $j =1, \dots, M$ and $t \geq 1$, 
        ...
        \]
if $\gamma_t = \calO(t^{-1/4})$, as prescribed by \cref{thm:regret_main}.
\end{lemma}

\subsection{\alexp with Uniform Exploration} \label{app:cmin_bound}

We present our main regret bound (\cref{thm:regret_main}) in terms of $\cmin$, which only depends on properties of the feature maps and the action domain. 
...
\end{proposition}
We expect this proposition to be applicable to any action set which span $\sR^{d}$, and to non-linear invertible maps.
...
\end{lemma}
We are now ready to present the proofs to Lemmas in \cref{app:explore}.
...
\end{align*}
where the set $T_\pi$ contains the indices of the steps at which the action is selected according to $\pi$. We use $\alpha_t$ to indicate whether $s \in T_\pi$ or not. Therefore, ,
\begin{align*}
...
We observe that the restricted minimum eigenvalue of $\hat \Sigma_{t'}$ is the $\kappa^2$ constant of $\Phi_{\pi, t'}$:
\begin{equation*}
...
\begin{align*}
 \sP\left(\forall t \geq1:\, \abs{ t' -  \left(\sum_{s=1}^t\gamma_s \right)} \leq \Delta_{t} \right) \geq 1-\delta/2
\end{align*}
where
\[
\Delta_t \coloneqq \frac{5}{2}\sqrt{ \frac{\left( (\log \log t)_+ + \log(8/\delta)\right)}{t}}.
\] 
    For the rest of the proof, to keep the calculations simple, we ignore the values of the absolute constants, and use the notation $g(t) = o(f(t))$ if there exists an absolute constant $c$ where
    $g(t) \leq c \abs{f(t)}$ for all $t \geq 1$.
    Let $\Gamma_t = \sum_{s=1}^t\gamma_s$, then since $\gamma_s = \calO(s^{-1/4})$ there exists $C$ such that $\Gamma_t = C t^{3/4}$.

    Due to \cref{lem:kappa_com}, with probability greater than $1-\delta/2$ the following holds for all $t\geq 1$
    \begin{align*}
         \kappa^2(\Phi_{t},2) &  \geq \frac{t'}{t} \cmin - \frac{160\sqrt{t'}}{t}  \sqrt{(\log \log 4t')_+ + \log(4Md/ \delta)} \\
         & \geq  \frac{\Gamma_t-\Delta_t}{t}\cmin - 160 \sqrt{\frac{\Gamma_t+\Delta_t}{t^2}}\sqrt{\left(\log \log \left(4\Gamma_t + \Delta_t\right)\right)_+ + \log(4Md/ \delta)} 
         \end{align*}
         It is straightforward to observe that there exists absolute constants $\tilde C_i$ which satisfy,
         \begin{align*}
         \kappa^2(\Phi_{t},2) &  \geq \tilde C_1 t^{-1/4}\cmin - \frac{5t^{-3/2}\cmin}{2} \sqrt{\left( (\log \log t)_+ + \log(8/\delta)\right)}\\
         & \quad -\tilde C_2 t^{-5/8} \sqrt{\log(Md/\delta) + (\log\log t)_+} - o\left(t^{-5/8} \sqrt{\log(Md/\delta) + (\log\log t)_+}\right)\\
         & \geq \tilde C_1 t^{-1/4}\cmin - \tilde C_3 t^{-5/8} \sqrt{\log(Md/\delta) + (\log\log t)_+}
    \end{align*}
        The last inequality holds since $t^{-3/2}\sqrt{\log\log t} = o(t^{-5/8}\sqrt{\log\log t})$.
        The above chain of inequalities imply that there exist absolute constants $C_1, C_2$ (w.r.t the parameters of the problem), for which
        \[
        \sP\left(\forall t\geq 1: \,\, \kappa^2(\Phi_t,2) \geq C_1\cmin t^{-1/4} - C_2t^{-5/8} \sqrt{\log(Md/\delta) + (\log\log t)_+} \right) \geq 1-\delta.
        \]
\end{proof}
...
% \begin{proof}[Proof of \cref{cor:alexp_unif}]
