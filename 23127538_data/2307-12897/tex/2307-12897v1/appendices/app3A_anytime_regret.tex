...
$\lambda_t  = \calO(C(M, \delta, d)/\sqrt{t})$,
     then exists absolute constants $C_i$ for which \algo attains the regret 
\begin{align*}
...
\begin{lemma} [Anytime Exponential Weights Guarantee]\label{lem:exp3_vanilla}
    Assume $\eta_t \hat r_{t,j} \leq 1$ for all $1\leq j\leq M$ and $t \geq 1$. If the sequence $\eta_1, \dots, \eta_t,\dots$ is non-increasing, then for all $n\geq 1$,
    \[
...
\begin{equation*}
    \sum_{t=1}^n J_{t}(\eta_t) - J_{t-1}(\eta_t) = \sum_{t=1}^{n-1} (J_{t}(\eta_t) - J_{t}(\eta_{t+1})) + J(\eta_n)
\end{equation*}
...
\begin{align}
    \sum_{t=1}^n J_{t}(\eta_t) - J_{t-1}(\eta_t) &\leq J_n(\eta_n) \leq \frac{\log(M)}{\eta_n} - \frac{1}{\eta_n}\log\left(\sum_{i=1}^K \exp(\eta_n \hat R_{n,i})\right) \nonumber \\
...
\begin{proof}[Proof of \cref{lem:anytime_mother_exp4}]   
    At each step $t$, if $\alpha_t= 1$, then \alexp accumulates a regret of at most $2B$, since $\norm{\vtheta}_\infty \leq B$ and $\norm{\vphi(\cdot)} \leq 1$. We can decompose the regret as,
    \begin{align*}
...
\begin{proof}[Proof of \cref{lem:anytime_CI_main}] By the definition of the expected reward and its estimate,
    \begin{align*}
        ...
        =\sum_{t=1}^n \norm{\vtheta - \hat \vtheta_t}_2.
         \end{align*}
From \cref{thm:anytime_lasso}, with probability greater than $1-\delta/2$ simultaneously for all $n\geq 1$,
    \begin{align*}
...
\begin{lemma}[Anytime Bound on Variance Term] \label{lem:anytime_variance_bound}
    Suppose $\lambda_t$ is chosen according to \cref{lem:anytime_CI_main}, $\gamma_t = \calO(t^{-1/4})$ and $\eta_t = \calO(\cmin t^{-1/2}/C(M,\delta,d))$. Then with probability greater than $1-\delta$, the following holds simultaneously for all $n \geq 1$
    \begin{align*}
...
We start by conditioning on the event $E$ that $\eta_t$ is picked such that $\eta_t \hat r_{t,j}$ for all $t\geq 1$ and $j = 1, \dots, M$.
...
First, suppose this condition is met with probability greater than $1-2\delta$, we will later verify this.
Now let $\calB = \calB(\tilde \delta, M, d, n, B, \sigma, \cmin)$ denote the right-hand-side of the regret inequality above. By the chain rule we may write,
\begin{align*}
