...
\begin{abstract}
An aperiodic binary sequence of length $\ell$ is a doubly infinite sequence $f=\ldots,f_{-1},f_0,f_1,\ldots$ with $f_j \in \{-1,1\}$ when $0 \leq j < \ell$ and and $f_j=0$ otherwise.
...
The autocorrelation of $f$ at shift $s$ is the dot product of $f$ with the sequence obtained by translating $f$ by $s$ places.
...
In a previous paper, the authors developed a combinatorial theory for determining the $p$th central and standardized moments of the demerit factor, which enabled them to obtain an explicit calculation of the skewness \cite[Thm.\ 1.3]{Katz-Ramirez}.
\begin{theorem}[Katz--Ramirez, 2023]\label{Barbara}
For every positive integer $\ell$, we have 
\[
\mom{3} \ADF(f)=
\begin{cases}
...
\]
\end{theorem}
The previous paper also had a computer-assisted calculation of the kurtosis of the distribution of demerit factors \cite[Thm.\ 1.4]{Katz-Ramirez}.
...
\begin{theorem}[Katz--Ramirez, 2023]
...
\end{definition}
One of the most important properties of the action of $\cWp$ on partitions is that it preserves global evenness, local oddness, satisfiability, number of assignments, and contributoriness, as shown in \cite[Lem.\ 4.7--Cor.\ 4.9]{Katz-Ramirez}, which we summarize here.
...
\end{definition}
We most often apply \cref{Sonia} when $\fP \in \Isom(p)$, and this leads to our basic formula for the central moments of the sum of squares of autocorrelation in \cite[Prop.\ 4.12]{Katz-Ramirez}, which we restate here.
...
To help us count assignments, we also need two elementary counting results, which are proved in Corollary C.2 and Lemma C.6 of \cite{Katz-Ramirez}.
...
Moreover, if $p$ is odd then $\Con(p)\cap\Sep(p)$ has no principal partitions.
\end{corollary}
\begin{proof}
The first statement follows immediately from \cref{water}\ref{potato}.
If $ \cP \in \Con(p)\cap\Sep(p)$ is principal and $ \fP $ is the quotient of $ \cP $, then by \cref{water} $ |\fP| = p/2 $.
...
If we let $\tau\in\As([p])$ where $\tau_{e,0,0}=\tau_{e+p/2,0,0}=4 e$, $\tau_{e,0,1}=t_{e+p/2,0,1}=4 e+3$, $\tau_{e,1,0}=\tau_{e+p/2,1,0}=4 e+1$, and $\tau_{e,1,1}=\tau_{e+p/2,1,1}=4 e+2$ for every $(e,s,v) \in [p/2]\times\lindexset$, then one sees that $\tau\in\As(\cQ,=)$, so that $\cQ\in\Sat(p)$, and so $\cQ\in\Con(p)$.
...
\]
Then one can check that for each $(e,s,v) \in [p/2] \times\lindexset$, we obtain $\pi(Q_{i,s,v})=C_{i,s,v}$, so that $\pi(\cQ)=\cP$, and thus $\cP$ is isomorphic to $\cQ$.
\end{proof}
...
Therefore, since $|\cWp|=p! 8^p$ by \cite[eq.\ (5)]{Katz-Ramirez}, the orbit of $\cQ$ (which is the set of all principal partitions in $\Part(p)$) has cardinality
\begin{align*}
...
\end{itemize}
Since $\As(\succeq\cP,=,\ell)=\As(\succeq\cP,=)\cap\As(\succeq\cP,\ell)$, we can restrict our map $\phi$ to have domain $\As(\succeq\cP,=,\ell)$ and codomain to $\{U \in [\ell]^\cP: f_0(U)=\cdots=f_{p-1}(U)=0\}$, and we obtain a bijection which proves claim \ref{Lilian}.
Since $\As(\cP,=,\ell)=\As(\succeq\cP,=)\cap\As(\succeq\cP,\ell)\cap\As(\cP)$, we can restrict our map $\phi$ to have domain $\As(\cP,=,\ell)$ and codomain to $\{U \in \Inj(\cP,[\ell]): f_0(U)=\cdots=f_{p-1}(U)=0\}$, and we obtain a bijection which proves claim \ref{Marisol}.
\end{proof}
...
Our choices make the linear forms $f_{e_1}(X), \dots, f_{e_m}(X) $ a $\Q$-linearly independent list of polynomials.
...
By \cref{Nancy}\ref{Lilian}, it then suffices to show that
\[
