...
As in the Bethe-Heitler calculation, a suitable set of approximations is applied to calculate the polarized contribution to the cross section.
...
The ``no-recoil" approximation \cite{Berestetskii:1982qgu} is applied, where the energy transferred by the exchange photon is neglected: $q = (0,\textbf{q})$.
Since the photon polar-angle distribution of the Bethe-Heitler expression
is sharply peaked near zero, it follows that the virtuality $q^2$ of the exchange photon is also very small, $q^2 \sim m_e^2$ (see section 97 of Ref.~\cite{Berestetskii:1982qgu}). 
The structure of the nucleus is neglected, which is justified at low $q^2$. 
...
$\sum\limits_{pol} \varepsilon^*_a (k) \varepsilon_b (k) \rightarrow - g_{ab}$, $\sum\limits_{spin} u(p') \bar{u}(p') = \slashed{p'} + m_e$, $\sum\limits_{spin} u(r') \bar{u}(r') = \slashed{r'} + m_p$, where $m_p$ is the proton mass.
...
In the target (proton) rest frame, $a^{(e)} = 2 \mathbb{P}_e \frac{ \varepsilon \, \varepsilon_p }{ m_e m_p } \left( -1, 0, 0, +1 \right)$.
The parameter $\mathbb{P}_e$ is the electron beam polarization, which is more conveniently defined as that measured in the laboratory frame (like $\varepsilon$ and $\varepsilon_p$) along the electron's momentum.
...
\mathfrak{p}_{\mu\alpha}^{(3)} &=& 8i \frac{ m_e a^{\lambda,(e)}}{ (pk)(p'k) }  \Bigg[ \frac{q^2}{2} p'^a \varepsilon_{\lambda a \mu \alpha} + p'^a p^\mu (k^b - p^b) \varepsilon_{\lambda a b \alpha} \nonumber \\
&& \hspace{52pt} + ( p'^\alpha k^a (p^b-p'^b) \nonumber \\ 
&& \hspace{52pt} \; + p^b (p^\alpha - k^\alpha)(p'^a + k^a))\varepsilon_{\lambda a b \mu} \Bigg], \\
...
\begin{equation}
d\sigma_{\mathcal{P}} = \frac{ -\alpha \, r_e^2 \, m_e^2 }{ 4(4\pi)^2 q^4 \, m_p^2 } \mathcal{P}^{\mu\alpha} \mathfrak{p}_{\mu\alpha} \frac{ |\textbf{p}'| \omega d\omega }{ |\textbf{p}| } d\Omega' d\Omega_k .
\end{equation}
...
{\small
\begin{eqnarray}
\mathcal{P}^{\mu \nu} \mathfrak{p}_{\mu \nu} &=& -32 m_e m_p \Biggl[  \frac{1}{(pk)^2} \Big( \frac{q^2}{2} ka^{(e)}(qa^{(p)}-2p'a^{(p)} ) \Big) \label{eq:fullyDif} \\ 
...
In the small-angle limit that characterizes the Bethe-Heitler expression and for ultra-relativistic particles, the Lorentz transformation from the lab frame (LF) to the target rest frame (TRF) is especially simple: $E_{TRF} = \frac{2\epsilon_p}{m_p} E_{LF}$ for $E \in \{\varepsilon, \varepsilon', \omega\}$.
...
\begin{eqnarray}
\frac{ d\sigma_{\mathcal{P}} }{ d\omega d\delta } &=& \mathbb{P}_e \mathbb{P}_p \frac{ \alpha \, r_e^2 \, m_e^2 \, \delta }{\omega \varepsilon^3 \varepsilon' \varepsilon_p (1+\delta^2)^2} \Biggl[ 2\omega^3  + 2 (-1+2 L_2) \varepsilon'^3 \nonumber \\
    && + (8 + 6L_2) \varepsilon'^2 \omega + 2 (1+L_2) \varepsilon' \omega^2  \nonumber \\ 
    && - 2 (L_1 + L_{2} - L_{\theta} ) (\varepsilon^2+\varepsilon'^2 - \varepsilon \omega \delta^2) \varepsilon \nonumber \\ 
    && -\left(4\varepsilon'^3+2 (-1+L_2) \varepsilon'^2 \omega + 5\varepsilon' \omega^2 + 2 \omega^3\right) \delta^2 \nonumber \\ 
    && + \frac{2\varepsilon \varepsilon' \omega }{1+\delta^2} + 2L_1
    \varepsilon  \varepsilon' (\varepsilon + \varepsilon') (1+\delta^2)
    \Biggr],
\end{eqnarray}
...
where
\begin{eqnarray}
F_1 &=& \frac{1}{8} \left( 5 + L_1(-2 + 4L_3) -4L_3(-3+L_2) + 2L_2 \right), \nonumber \\
F_2 &=& L_1 +(-1+L_3)(3 + 2L_3 - 2L_2), \nonumber \\
F_3 &=& -1 - 2L_2 +2L_3(-9 + 2L_2), \nonumber \\
...
It was also observed in a numerical calculation at larger $q^2$ \cite{Akushevich:1998dz}.
...
In regards to the general luminosity program at the EIC, where high $Z$ heavy-ions will be accelerated, it should be noted that the Born approximation used in the Bethe-Heitler expression is expected to be inadequate.
Thus, the Bethe-Heitler expression should be replaced by the Bethe-Maximon expression \cite{Bethe:1954zz,Davies:1954zz,Berestetskii:1982qgu}:
{\footnotesize
\begin{eqnarray}
...
The polarized component of the bremsstrahlung cross section has been calculated.
...
The suppression is linked to the low $q^2$ that characterizes the bremsstrahlung process, which is $\sim m_e^2$.
