...
\end{equation}
This formula is also expected to exhibit the integrality of Gromov-Witten invariants of Calabi-Yau manifolds of higher dimensions. Klemm and Pandharipande \cite{KP08} conjectured that for a Calabi-Yau 4-fold $Z$ and insertions $\gamma_1, \dots, \gamma_n \in H^*(Z;\bZ)$, the BPS invariants $n^{Z}_{0, \beta}(\gamma_1, \dots, \gamma_n)$ defined from the Gromov-Witten invariants $N^{Z}_{0, \beta}(\gamma_1, \dots, \gamma_n)$ by the formula
...
\end{equation}
are integers. For a compact Calabi-Yau symplectic manifold $Z$ of real dimension at least 6, this statement was proved by Ionel-Parker \cite{IP18}.

On the other hand, on the level of Gromov-Witten theory, closed invariants of 4-folds are related to open invariants of 3-folds. In more detail, given an open geometry on a toric Calabi-Yau 3-fold $X$ relative to a single framed outer brane $(L,f)$, we consider its disk invariants $N^{X, L, f}_{0, \beta, (d)}$ where for an effective class $\beta \in H_2(X,L;\bZ)$, $d \in \bZ_{\ge 1}$ denotes the winding number at the unique boundary component of the curve and is determined by $\beta$. The \emph{open/closed correspondence}, conjectured by Mayr \cite{Mayr01} and proved by the author and Liu \cite{LY21,LY22}, produces a toric Calabi-Yau 4-fold $\tX$ together with an isomorphism $\iota: H_2(X, L;\bZ) \to H_2(\tX;\bZ)$ and a class $\tgamma \in H^4(\tX;\bZ)$ such that
$$
...
Combining this result with Theorem \ref{thm:IntroOpenBPS}, we obtain the integrality of the closed BPS invariants. This gives an additional class of examples for the conjecture of Klemm-Pandharipande \cite{KP08}.
...
We refer to \cite{PSW08} and the references therein for discussions on the case where $X$ is the quintic 3-fold and $L \subset X$ is the real quintic.
...
This work originates from the author's joint project with Chiu-Chu Melissa Liu \cite{LY21,LY22} and I would like to thank Melissa for many valuable discussions and suggestions. I would like to thank Bohan Fang and Zhengyu Zong for helpful discussions and comments. I would also like to thank the authors of \cite{BBvG20} for their inspiring results on the open and closed BPS invariants. 