...
%%%%%%%%% TITLE
% \title{3D Shifter: A Semantic Generative Model Using Subspaces for Shape Editing}
\title{3D Semantic Subspace Traverser: Empowering 3D Generative Model \\
with Shape Editing Capability}


\author{
Ruowei Wang \qquad Yu Liu \qquad Pei Su \qquad Jianwei Zhang \qquad Qijun Zhao\\
Sichuan University\\
...
Building a generative model to synthesize 3D shapes is a predominant topic in computer graphics and 3D vision, since it is widely used in applications such as 3D content creation \cite{mmvad, sketch2mesh}, autonomous driving \cite{autodrive1, autodrive2, autodrive3}, AR/VR \cite{ar_vr}, and robotics \cite{robotics1, robotics2}. Typically, A 3D shape generative model takes a random vector as input and produces a 3D shape as output. To evaluate the effectiveness of generative models, researchers consider three aspects: quality, diversity, and controllability. The ideal 3D shape generative model should produce visually realistic and diverse 3D shapes while also enabling local structure manipulation to achieve controllability.

% Figure environment removed

Previous 3D shape generative models like VAE \cite{atlas-net, GRASS, editvae, dsg-net, pq-net}, GAN \cite{3d-gan, pc-gan, spgan, im-gan, grid-im-gan, shapegan, sdfstylegan}, flow-based models \cite{pointflow, dpf, softflow}, and diffusion models \cite{luo2021diffusion, lion, luo2021diffusion, pvd} have been applied to various 3D representations, including point clouds \cite{pc-gan}, voxels \cite{3d-gan}, mesh \cite{atlas-net} and so on. By adopting the rapidly developed implicit function-based shape representation \cite{im-gan, deepls, deepsdf}, the generated shapes show promising quality and diversity \cite{sdfstylegan}. However, the ability to control and manipulate the local structures of generated shapes during the generation process remains a challenging task.
It is crucial to develop methods for effective shape control that allow for the modification of local structures during the generation process.
% Ideally, controlling the shapes is to manipulate their local structures. 

Previous methods that edit local shapes can be classified into two groups: interpolation-based methods \cite{g2l-gan, pq-net, spgan, StructureNet, SPAGHETTI, editvae} and optimization-based methods \cite{dualsdf, dif-net, shapecrafter, implicit_text_3d, mmvad, sketch2mesh}. The interpolation-based ones utilize part-level interpolation to edit shapes. However, since it does not consider the semantic relation between shape parts, it produces inconsistent results (see in the first row of \cref{fig:consistency}). The optimization-based methods take the editing inputs, which are sketches, point moving operations, and so on, to optimize shapes. But its results focus on matching the editing inputs while ignoring other shape parts (see in the second row of \cref{fig:consistency}). In light of this, we propose a new paradigm that utilizing the learned semantic information in the subspace (see in the last row of \cref{fig:consistency}). During the generation process, we can edit shapes by traversing semantic dimensions in the subspace to produce diverse results. More details are illustrated as follows:
...
% Some methods \cite{g2l-gan, pq-net, spgan, StructureNet, SPAGHETTI, editvae} decompose the shape into parts to learn the structure. By this means, they are able to edit shapes using part-level shape interpolation. However, handcrafted part labels are usually required, and those methods suffer from capturing the semantic information of 3D shapes. As a result, they can not create new instances and may produce edited shapes with inconsistent parts (see \cref{fig:consistency}). Other methods \cite{dualsdf, dif-net, shapecrafter, implicit_text_3d, mmvad, sketch2mesh} edit shapes by using an extra optimization  (see \cref{fig:consistency}), which costs more time and computing resources.


% . Some 3D generation methods \cite{3d-gan, pc-gan, im-gan, sdfstylegan} learn the shape distribution and directly generate the whole shapes. Due to the complexity of the 3D shape space, those methods can not take control of the structures. Other 3D generation methods \cite{g2l-gan, pq-net, spgan, StructureNet} decompose the shape into parts to learn the structure. Instead of generating the whole shape, they generate the shape parts respectively and assemble those parts into an integrated shape. It is possible for them to manipulate the shape's local structures by feature interpolation. But they require handcrafted part labels and do not capture the semantic information of 3D shapes. As a result, they can not create new instances and may produce edited shapes with inconsistent parts (see \cref{fig:consistency}).


% 记得结合 fig:consistency ，提一下我们的方法的有点。
% To further improve the editability of 3D shape generation, we adopt the implicit function as the 3D representation and propose a category-specific GAN-based semantic generative model, called 3D Semantic Subspace Traverser, to effectively utilize the semantic information for shape generation and shape edit. 
% We present 3D Semantic Subspace Traverser, a novel category-specific generative model for 3D shape generation and editing by leveraging an implicit function as the 3D representation and GAN architecture. 
We present 3D Semantic Subspace Traverser, a novel semantic generative model for 3D shape generation and editing by leveraging the propose local linear subsapce models and the latent-space generative adversarial network (GAN). We adopt deep implicit function as the 3D representation and apply our GAN to the latent shape space, which is produced by a variational autoencoder (VAE). The GAN generates shape features in the form of a shape code grid, which can be decoded into 3D shapes by the VAE decoder. To empower our 3D generative model with shape editing capability, we embed the local linear subspace models into the GAN. Those local linear subspace models can discover semantic dimensions from feature maps in an unsupervised manner. By traversing along those semantic dimensions, our GAN can control the semantic attributes of the generated shapes, as shown in \cref{fig:consistency}. The proposed 3D Semantic Subspace Traverser generates plausible 3D shapes and enables semantically editing of 3D shape, as evidenced by quantitative and qualitative results.

%Our model effectively leverages semantic information for shape generation and editing, using a combination of a variational autoencoder (VAE) and a generative adversarial network (GAN) inspired by the concept of latent-space GAN (l-GAN). The GAN module generates shape features in the form of a shape code grid, which can be decoded into 3D shapes by the VAE decoder. The shape features have been designed to follow the Gaussian distribution, making them easier for the GAN to learn. By combining latent-space GAN and shape code grid, our generative model can produce 3D shapes with complex structures. To extract semantic information, we devise a local linear subspace model that enables the generator to extract semantic information from feature maps in an unsupervised manner. The subspace models can control the semantic attributes of the generated shapes, as shown in \cref{fig:consistency}. The proposed 3D Semantic Subspace Traverser generates plausible 3D shapes and enables semantically editing of 3D shape, as evidenced by quantitative and qualitative results.
...
% In the five categories of the ShapeNet Core (v1) dataset \cite{shapenet2015}, we show that 3D Semantic Subspace Traverser can produce plausible 3D shapes and edit the semantic attributes of 3D shapes without part labels.
...
\subsection{Overview}
We design 3D Semantic Subspace Traverser, a novel semantic 3D generative model for 3D shape generation and editing, as illustrated in \cref{fig:generator}. In the following sections, we first introduce our local linear subspace model and how embed it into the local regions of the feature map by differentiable embedding in \cref{sec:subspace}. Second, we present the details of our 3D Semantic Subspace Traverser in \cref{sec:generator}. Then, we talk about how we encode shapes into the latent shape code grids in \cref{sec:vae}. Lastly, we present the loss function and implementation details in \cref{sec:loss} and \cref{sec:impl_detail} respectively.




% 3D Semantic Subspace Traverser consists of a VAE and a GAN. The local linear subspace models are embedded into the GAN to capture the semantics of the shapes. We first train the VAE to produce the reliable shape code grid in a 3D reconstruction manner. Then we employ GAN on those shape code grids to learn its distribution and the semantic attributes. VAE and GAN are trained separately. After the training process, the generator produces and edits a shape code grid, which is decoded into a 3D shape by the implicit decoder of VAE. 

% 列举不同章节写什么。

\subsection{Local Linear Subspace Model}
% 先介绍下我们的子空间模型，从坐标轴等部分开始解释。然后解释它如何嵌入到feature中，特别是要提一下feature和shape code的嵌入方式的区别（好像也没啥大区别）
\label{sec:subspace}



% 首先先说图中大致展示了子空间模型是如何嵌入到某个固定大小的feature（左边是子空间嵌入，右边是可微嵌入）。然后简单说一下嵌入的流程。
% 接下来引入到我们子空间如何构建。如何做可微分的嵌入。最后提一下我们的这个框架是很容易融入到其他框架的。


% . It would be better to mention the new trait (compared to EigenGAN) in the method section. .

Different from the linear subspace model in EigenGAN \cite{eigengan}, our local linear subspace models can be embedde into local regions to learn the semantics. In the forward propagation, we first take a random vector $C=[c_1,\cdots,c_n]^T$ as the coordinates to sample a point $\phi \in R^{c \times d\times h\times w}$ from the local linear subspace:
\begin{equation}
...
In other words, they decide which region of the feature map $m$ we embed the $\phi$ into. By this means, we replace the indexing with the transform matrices, which make the embedding differentiable. \cref{fig:subspace} shows a 2D exmaple to illustrate the differentiable embedding. 
...
After trainning, the local linear subspace model captures the semantic attributes of the local region in the feature map $m$. By changing the coordinates $C$ of the sample point in the subpace, we can semantically manipulate the feature map. 
...
% 实际上我们是如何嵌入的，包括选位置、追加的处理方式 % 输入是什么，z和c。先介绍z，然后介绍c。介绍完c，说说中间我们是如何嵌入的。
\begin{equation}
...
% Such an embedding layout, in our opinion, is generally effective and can reveal the nature of our editing method, which we will talk about in \cref{sec:shape edit}. 
% It is also free to design different embedding layouts for other tasks. 
% We can select different embedding locations for various generation tasks. But in our experiments, we use the same embedding locations for convenience. 

Our generator take a random noise $z \sim N(0, 1)$ and a set of random vectors $\left\{ C_i | C_i \sim N(0, 1) , i=1, \dots, 4 \right\}$ as input. $C_i$ is the coordinates for the $i^{th}$ local linear subspace to sample a point.  $G(z, C_i)$ is the generated shape code grid.
% The random noise $z \sim N(0, 1)$ intends to provide the rest information for shape.

We design a 3D PatchGAN discriminator $D$ to focus on the patches of the shape code grid. The discriminator follows the basic idea of the 2D PatchGAN discriminator from Pix2Pix \cite{pix2pix}. It classifies some overlapping $N \times N \times N$ patch in the shape code grid as real or fake. By this means, we penalize the shape code grid at the scale of patches to improve the diversity of the generated shapes. More network details of the GAN are provided in the supplementary material.

At the inference time, the generated shape code grid are decoded into occupancy values by a pretrained implicit decoder, which we will talk about in \cref{sec:vae}. And we can extract the mesh from the occupancy values by marching cubes algorithm \cite{marchingcubes}.
...
Once trained, the shape code grid $s$ produced by the encoder are used to train our 3D Semantic Subspace Traverser. More network details of the VAE are provided in the supplementary material.

  % Figure environment removed


% In the training stage, we use standard cross-entropy loss to optimize the VAE weights. After the training, we use marching cubes algorithm \cite{marchingcubes} to extract the mesh from the occupancy values. More details of the VAE network are provided in the supplementary material.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Here $s$ is the shape code grid produced by the encoder of VAE. $\Theta_D$ is the parameters of the discriminator $D$. $\beta$ is the weight of $ \mathcal{R}_{1}$ and $\beta = 1$.
...
\begin{equation}
	...
	\end{aligned}
\end{equation}
Here $\mathcal{L}_{BCE}$ is the binary cross entropy loss. $a$ is the input voxel. $V(\cdot)$ is the predicted occupancy values of VAE. $o$ is the ground truth occupancy values. $KL$ is the KL loss.$\mu$ and $\sigma$ are the mean $\mu$ and the standard deviation produced by the encoder of VAE.

%\int p(x) \log\frac {p(x)} {q(x)} \,dx \\

\subsection{Implementation Details}
\label{sec:impl_detail}
In 3D Semantic Subspace Traverser, we set $n=6$, which is the number of dimension of all subspace models. In experience, when $n \leq 3$, the subspace models have difficulty in learning semantics. When $n > 3$, the subspace model is able to capture semantics. $n=6$ is chosen as a trade-off between the learned semantics and the number of parameters. 
...
We use the ShapeNet Core (v1) dataset \cite{shapenet2015} to train our network. For comparison, we adopt the data split of  \cite{sdfstylegan} \cite{hsp}: 70\% of the data for training, 20\% of the data for testing, the rest for validating which is not used in our expeiments. For data preprocessing, we follow the steps in  \cite{if-net}. To be specific, we first convert the meshes to the watertight ones. Second, we normalize those meshes to make the lengths of their largest bounding box edges equal to one. Third, we sample some points and compute their occupancy values for each mesh. Finally, we voxelize meshes to produce $64^3$ voxels. Our VAE is trained with five categories of the ShapeNet Core (v1) dataset, including chair, airplane, table, car and rifle. Different from the VAE, we train one specific GAN for each specific category. In other words, our GAN is category-specific.
% 用什么数据，数据划分是什么。用了什么预处理的方法。
% 我们的实验设置（在什么上面训练，用了什么显卡，batch——size的数目，local region的选择（可以随机，也可以自己固定））
...
We show various shape edit results in \cref{fig:edit1}, where ``Dim=i” is the $\rm i^{th}$ dimension of all local linear subspace models. Different kinds of semantic attributes are learned by different dimensions. As shown in \cref{fig:edit1}, moving along those dimensions, some semantic attributes of the generated shapes change. We can edit both local attributes, e.g., ``seat" in Dim = 4, 5, 6, ``back" in Dim = 8, 19, and ``leg" in Dim = 12, 14; and global attributes, e.g., ``type" in Dim = 0, 2, and ``width" in Dim = 18. Specifically, when we traverse along the $\rm 4^{th}$ dimension and the $\rm 6^{th}$ dimension, the same attributes are edited but with opposite results. The $\rm 12^{th}$ dimension and the $\rm 14^{th}$ dimension show similar results. The $\rm 12^{th}$ dimension translates all chairs to the ones with star-shaped legs, while the $\rm 14^{th}$ dimension changes all legs to two types. In addition, our method maintain right semantic structures without inconsistency after editing shape parts.
% 列举我们能改变那些部件、以及修改整体的什么。4和6有什么特殊的。12和14有什么特殊的。局部变化时，也会带来一些语义变化。



\noindent\textbf{Comparision with VAE+PCA.} We provide a meaningful baseline for comparision, which is to apply principal components analysis (PCA) to the VAE encoder output space to discover semantic dimensions. In \cref{fig:pca}, we show some editing results of our method and VAE+PCA. As illustrated, VAE+PCA typically generates unreasonable float artifacts, whereas our results preserve the proper semantic structures. In conclusion, our approach captures semantic information more effectively than PCA.

% Figure environment removed

% \noindent\textbf{Semantic editing consistency.} Since \cref{fig:edit1} show

% \noindent\textbf{Traversing the semantic dimension.} 

% . In summary, one of the layers learns most of semantic attributes related to the geometry variance. The dimensions in this layer learn the local attributes or the global attributes. There are also some entanglements of attributes in some dimensions, but those entanglements are interpretable. By utilizing those semantic attributes, we can produce various edited shapes with reasonable semantic structure.
...
To evaluate our shape generation quality, we compare our method to other representative methods that use implicit function for 3D generation: IM-GAN \cite{im-gan}, ShapeGAN \cite{shapegan}, Implicit-Grid \cite{grid-im-gan}, and SDF-StyleGAN \cite{sdfstylegan}. Among these methods, IM-GAN, Implicit-Grid, and our method use occupancy values to represent 3D shapes. ShapeGAN and SDF-StyleGAN use SDF values to represent 3D shapes. 

For a fair comparison, we take ECD \cite{grid-im-gan} and Shading-image-based FID (S-FID) \cite{sdfstylegan} as our evaluation metrics, and also report COV \cite{pc-gan} and MMD \cite{pc-gan} values as the reference. Following the experiments setting in \cite{sdfstylegan}, ECD, MMD and COV are all based on the light-field-distance (LFD) \cite{lfd, sdfstylegan} to measure the distance between two samples. LFD is based on the silhouette images and measures the structure similarity between two shapes. When computing COV, MMD, and ECD, we take the testing dataset as the reference to compare with our generated shapes, which is five times of the number of the reference. When computing S-FID, we use the training dataset as the reference, and the number of generated shapes is the same as the reference. We use the clean-fid algorithm \cite{clean-fid} to compute the FID.

% 这个地方要根据之后的改动，重写以下。
We show the quantitative results in \cref{tab:metrics}. COV is listed for reference only as mentioned before. In all categories, we have the best performance in MMD. And for three classes with complex topological structures, i.e., chair, airplane, and table, we also achieve the best results in ECD. It shows that our method is good at generating shapes with complex topological structures. But in S-FID, we are not the best due to the unsmooth surfaces.

% In conclusion, our capability of generating shapes with diverse structures 

% In all categories, we have the best performance in MMD, which means that our best shapes in $A$ are the closest to the shapes in $B$ from the perspective of the structure. For LFD-based ECD, the smaller values mean that the structures of $a \in A$ are more similar to the reference shapes $b \in B$. In the category of chair, airplane and table, our method has the best ECD values. But for the rifles and the cars, our results are not the best in ECD. The reason is that our method is better at learning the topological structure variance in the chairs, airplanes, and tables than the geometry details of the rifles and cars. In terms of FID, SDF-styleGAN achieves the best visual performance. However, since FID can only be used for the Gaussian distributions as reported in  \cite{grid-im-gan}, it can not faithfully measure the visual appearance of the rendered images, which are different from the natural images. Furthermore, the low MMD and ECD values reveal its limitation of producing flawed topological structures, as evidenced in Fig.6 (supp).
% However, since the rendered images of S-FID are significantly different from the natural images that are used to train Inception

% fid 的问题在于 FID提取的特征，可能不一定适用于三维数据（白色背景+三维投影）。

% In summary, thanks to the combination of the shape code grid and the latent-space GAN, our method is good at generating shapes with various topological structures. Some synthesized shapes of different methods are provided in the supplementary material.
...
\label{sec:recon}
This section evaluate our shape compression ability by the voxel super-resolution experiment. We compare our VAE with IM-NET \cite{im-gan} and IF-NET \cite{if-net} (see in \cref{tab:recon}). For fair comparison, we use five classes in the data split of IF-NET, which are chairs, airplanes, tables, cars, and rifles. We train all models for 200 epochs. All methods take $32^{3}$ voxels as inputs, and the output meshes are sampled at $128^3$. To measure the quality, we employ three recognized measures \cite{if-net}: volumetric intersection over union (IoU) evaluates how well the defined volumes overlap, Chamfer-L2 evaluates the accuracy and completeness of the shape surface, and normal consistency evaluates the accuracy and completeness of the shape normals. \cref{tab:recon} shows that our method achieves second-best results, which is reasonable. In contrast to IM-NET, our method utilizes a grid to represent the shape code rather than a vector, which improves the quality of local regions' reconstruction. And IF-NET outperforms us in terms of metrics since it utilizes multi-scale features. Our method make a balance between quality and network complexity. Additionally, our main contribution is in the GAN part. From \cref{tab:recon}, we believe that the current VAE is good enough to support our method.
...
\noindent\textbf{Conclusion.} In this paper, we introduce a novel semantic 3D generative model, 3D Semantic Subspace Traverser, which can edit semantic attributes of 3D shapes. We apply 3D Semantic Subspace Traverser to the latent shape code, and use our local linear subspace models to capture the semantic information. Once trained, each dimension of subspaces corresponds to a specific semantic attributes, and we can edits shapes by traversing along those dimensions.
...
Research was sponsored by the National Natural Science Foundation of China (No. 62176170).
