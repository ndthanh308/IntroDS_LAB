...
Eventually, at low energy, the nucleon and  electron EDMs are induced.
...
$\mathcal{O}(10^{-27})\,e\,$cm, with a null electron EDM signal.
...
In the near future, the sensitivities of precision measurements for the elementary particles, particularly $B$ physics and the electric dipole moments (EDMs), are expected to be improved by an order of magnitude. 
...
has been reported 
by the BaBar, LHCb, Belle, and Belle II experiments \cite{Lees:2012xj,Lees:2013uzd,Huschle:2015rga,Hirose:2016wfn,Hirose:2017dxl,Abdesselam:2019dgh,Belle:2019rba,Aaij:2015yra,Aaij:2017uff,Aaij:2017deq,LHCb:2023zxo,LHCb:2023cjr,Belle2},
...
The up-to-date world average of the data \cite{HeavyFlavorAveragingGroup:2022wzx,HFLAV2023summer} is  
\beq
R_D^{\rm exp}=0.357 \pm 0.029\,,  \quad R_{D^\ast}^{\rm exp}  = 0.284 \pm 0.012\,,
\eeq
...
A new physics candidate is an $SU(2)_L$-singlet  vector leptoquark (LQ), dubbed as $U_1$ LQ.
...
%%%%%%%%%%%%%%%%%%%%%
The LQ model naturally brings a $CP$-violating (CPV) phase, which comes from the rotation matrices to the mass bases of the left- and right-handed quark and lepton fields that are not aligned in general.
In this Letter, it will be clarified that the CPV phase is necessary to accommodate the \rdst anomaly, 
...
Although Refs.~\cite{Panico:2018hal,Altmannshofer:2020ywf} investigated the EDMs in the vector-LQ model in light of the \rdst anomaly, they focused on the parameter benchmark points and the necessity of the CPV phase was unclear.
On the other hand, it is known that 
there is no robust correlation between the \rdst anomaly and EDMs in scalar LQ models
\cite{Fuyuto:2018scmz,Dekens:2018bci,Panico:2018hal,Crivellin:2019qnh,Babu:2020hun}.
...
We consider a simplified $U_1$ LQ scenario with a $U(2)$ flavor symmetry.
...
In this setup, 
$\phi_d$ is an arbitrary CPV phase; the other CPV phases can be absorbed by a redefinition of $\phi_d$ \cite{Iguro:2018vqb}.
Therefore, the relative phase between $\beta_L$ and $\beta_R$ interactions plays an important role in the CPV observables.


In our analysis, only three parameters are relevant to the phenomenology:  $\beta_L^{33}/m_{U_1}$, $\beta_L^{23}/\beta_L^{33} (=c_d s_{q_2} s_\chi/c_\chi) $, and $\phi_R$.
...
\begin{align}
    d_\tau &=-\frac{3e}{8\pi^2} \frac{m_b}{m_{U_1}^2} \textrm{Im}\left[\beta_L^{33} (\beta_R^{33})^*\right]\,,\\
    d_b(\Lambda_{\rm LQ})&= 
...
Note that the Weinberg operator ($GG\tilde{G}$) would be induced at two-loop level, but it is suppressed by $m_b m_\tau/m_{U_1}^4$, and we discarded it \cite{Dekens:2018bci}.
While there is no QCD renormalization-group (RG) evolution for $d_\tau$,
we have taken into account  the RG evolutions  from $\Lambda_{\rm LQ} $ to $\mu_b (=m_b)$ \cite{Crivellin:2019qnh,Haisch:2021hcg},
which are known to relax the EDM bound \cite{Dekens:2013zca}.
For example, the RG evolutions provide the following corrections:
\beq
d_b (\mu_b) &= 0.84\,d_b(\Lambda_{\rm LQ}) - 0.42e\,\tilde{d}_b(\Lambda_{\rm LQ}) \,,\\
\tilde{d}_b (\mu_b) &= 0.91\,\tilde{d}_b(\Lambda_{\rm LQ}) \,,
\eeq
...
Furthermore, a semi-leptonic $CP$-odd operator, $(\ov{e}i\gamma_5 e)(\ov{p}p + \ov{n}n)$, is also induced by them from QED two-loop diagrams \cite{Ema:2021jds,Ema:2022pmo}, 
...
By using a result of the improved analysis for the  QED three-loop calculation \cite{Ema:2022wxd}, 
we obtain
\begin{align}
    d_e &=  \left[4.7\times 10^{-13}+8.8\left(1\pm0.1\right)\times 10^{-12}\right]\, d_b (\mu_b) \nonumber \\
...
By a similar but more involved process, the nucleon (neutron and proton) EDMs are induced from the bottom-quark EDM and chromo-EDM.
...
For the neutron and proton EDMs, numerically we have
\begin{align}
   d_N&=d_N^{\rm light}+d_N^{\rm W} + d_N^{\tilde{F}G^3} \qquad (\textrm{for~}N=n,\,p)\,,\\
%%%%%%%%%   
   d_n^{\rm light}&=  4.3 \times 10^{-7} e\, \tilde{d}_b (\mu_b) + 4.2\times 10^{-8} \, d_b(\mu_b) \,,\\
%%%%%%%%%   
   d_p^{\rm light}&=  -3.6 \times 10^{-7} e\, \tilde{d}_b (\mu_b) + 9.5\times 10^{-9} \, d_b(\mu_b) \,,\\   
%%%%%%%%%
   d_n^{\rm W}& = -5.9\left(1\pm0.5\right)\times 10^{-5} e\, \tilde{d}_b (\mu_b)\,,
   \label{eq:dnW}\\
%%%%%%%%%
    d_p^{\rm W}& = 8.5\left(1\pm0.5\right)\times 10^{-5} e\, \tilde{d}_b (\mu_b)\,,
    \label{eq:dpW}\\
%%%%%%%%%
...
\end{align}
For $d_N^{\rm light}$, 
the QCD sum-rule estimate is used \cite{Pospelov:2000bw,Pospelov:2005pr,Hisano:2012sc,Fuyuto:2013gla,Kaneta:2023wrl} (where the Peccei-Quinn mechanism is assumed to suppress a $\bar{\theta}$ term), whose overall normalization is determined by the lattice result \cite{Alexandrou:2019brg}.
...
%
Note that although all the above terms have $10\%$--$30\%$ theoretical uncertainties, we suppressed them except for the leading one.
...
The current bounds and the future prospects for the electron, neutron, and proton EDMs are summarized in Table~\ref{Tab:EDM}.
...
% \begin{adjustbox}{width=\columnwidth,center}
 ...
 & & 
  ...
  ~~$|d_p|$ & $\le 2.1\times 10^{-25}$ \cite{Sahoo:2016zvr} & $ \mathcal{O}(10^{-29})$ \cite{CPEDM:2019nwp,Alexander:2022rmq}\\ 
   \end{tabularx}
%\end{adjustbox}
...
\!\!\!\!\! O_{V_L} \! = (\ov{c} \gamma^\mu P_L b)(\ov{\tau} \gamma_\mu P_L \nu_{\tau})\,, ~
 O_{S_R} \! = (\ov{c}  P_R b)(\ov{\tau} P_L \nu_{\tau})\,. \label{eq:operator}
\end{align}
...
The black contour denotes the values of $|\phi_R|$ with varying $\beta_L^{33}/m_{U_1}$. We use $\beta_L^{23}/\beta_L^{33} = \lambda \simeq 0.225$ as a typical reference value \cite{Cornella:2019hct}. 
The gray-shaded region cannot be predicted within the $U_1$ LQ model.
...
\end{align}
It is noted that the effect from the CPV phase $\phi_R$ is mild due to 
the smallness of the SM contribution.
...
It is known that although loop-induced LQ contributions to \bbms~mixing 
give a severe constraint, 
...
The estimated sensitivities based on upcoming Run\,3 data are shown by the dashed blue and green lines in Fig.~\ref{ref:param}.
...
through the popular $SU(2)_L$-singlet vector LQ
coupled primarily to third-generation fermions.
In the LQ interactions, 
there is one $CP$-violating phase which is required to accommodate the $R_{D^{(*)}}$ anomaly, and hence
...
within reach of the sensitivities of future experiments.
...
In this appendix, 
other related observables in $b \to c \tau \ov\nu$ are discussed in the simplified $U_1$ LQ model.
%%%
...
in $\Bb \to D^{(\ast)}\tau \overline{\nu}$, $P_\tau^D$ and $P_\tau^{D^\ast}$  \cite{Tanaka:2012nw,Asadi:2018sym}, and the LFU violation 
 in $\Lambda_b \to \Lambda_c l \ov\nu$, $R_{\Lambda_c} \equiv \mathcal{B}(\Lambda_b\rightarrow \Lambda_c \tau \ov\nu_\tau)/ \mathcal{B}(\Lambda_b\rightarrow \Lambda_c \ell\ov\nu_\ell)$, are shown  by the green, magenta, and purple contours, respectively.

 It is found that $P_\tau^{D^\ast}$ cannot deviate from the SM prediction $P^{D^\ast}_{\tau, \text{SM}}\simeq -0.50$, while $P_\tau^{D}$ can deviate from $P^{D}_{\tau, \text{SM}}\simeq 0.33$ which will be probed by the Belle II with good accuracy \cite{Alonso:2017ktd}.
...
This behavior is consistent with a sum rule prediction \cite{Blanke:2018yud,Blanke:2019qrx,Fedele:2022iib}, and it should also be a smoking-gun signal in the LHCb \cite{LHCb:2022piu}.
...
$\Delta F_L^{D^*}=0.01$ \cite{Iguro:2018vqb,Fuentes-Martin:2019mun},
