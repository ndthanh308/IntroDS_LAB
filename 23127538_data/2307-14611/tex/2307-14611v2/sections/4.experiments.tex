...
We evaluate our method in various cases presenting sparse data with 
% two
different tasks: long-tail classification in \Sref{sec4.1}, scarce data classification in \Sref{sec4.2}, and few-shot object detection (FSOD) in \Sref{sec4.3}.
We also conduct additional experiments demonstrating the compatibility of $\TextMani$ and ablation study on attributes
...
\resizebox{1.0\linewidth}{!}{
    ...
    \begin{tabular}{@{}l C{16mm}C{16mm}C{16mm}@{}}
        ...
        Cutout + $\TextMani$    & 40.35 (\blue{+2.84}) & 45.48 (\blue{+3.20}) & \textbf{61.31 (\blue{+2.05})} \\
        \cmidrule{1-4}
        Cutmix~\cite{yun2019cutmix}      & 37.93 & 43.34 & 59.30\\
        Cutmix + $\TextMani$    & 40.22 (\blue{+2.29}) & 45.36 (\blue{+2.02}) & 61.30 (\blue{+2.00})\\
        \cmidrule{1-4}
        Mixup~\cite{zhang2017mixup}       & 36.75 & 40.77 & 57.50 \\
        Mixup + $\TextMani$     & 38.40 (\blue{+1.65}) & 43.33 (\blue{+2.56}) & 59.80 (\blue{+2.30})\\
        \cmidrule{1-4}
        ManiMixup~\cite{verma2019manifold}   & 35.72 & 40.51 & 55.26 \\ 
        ManiMixup + $\TextMani$ & 38.60 (\blue{+2.88}) & 43.22 (\blue{+2.71}) & 59.35 (\blue{+4.09})\\
        \midrule
        \midrule
        ...
        Cutout + $\TextMani$ & 71.94 (\blue{+0.83}) & \textbf{40.97 (\blue{+2.55})} & 4.03 (\blue{+3.03}) \\
        \cmidrule{1-4}
        Cutmix                & 72.02 & 37.17 & 0.90 \\
        Cutmix + $\TextMani$  & 72.37 (\blue{+0.35}) & 40.80 (\blue{+3.63}) & 3.90 (\blue{+3.00}) \\
        \cmidrule{1-4}
        Mixup               & 71.97 & 33.62 & 0.36 \\
        Mixup + $\TextMani$ & 71.97 (+0.00) & 36.77 (\blue{+3.15}) & 1.83 (\blue{+1.47})\\
        \cmidrule{1-4}
        ManiMixup               & 72.97 & 29.51 & 0.70 \\
        ManiMixup + $\TextMani$ & \textbf{73.20 (\blue{+0.23})} & 36.80 (\blue{+7.29})& 0.76 (\blue{+0.06})\\
        \bottomrule
    \end{tabular}
    }
...
\begin{table}
    ...
    \begin{tabular}{@{\,}l@{}c@{\,\,\,}c@{\quad}c@{\quad}c@{\quad}c@{\,}}
         \toprule
...
\begin{table}
    ...
    \begin{tabular}{@{\,}l@{\quad\quad}C{9mm}C{9mm}C{9mm}C{9mm}@{\,}}
         ...
         \midrule
         LWS~\cite{kang2019decoupling} & \textcolor{RedViolet}{63.34} & \textcolor{CarnationPink}{48.08} & \textcolor{CarnationPink}{27.19}& \textcolor{CarnationPink}{51.14}  \\
         cRT~\cite{kang2019decoupling} & \textcolor{gray}{61.80} & \textcolor{gray}{46.20} & \textcolor{gray}{27.40} &\textcolor{gray}{49.60} \\
         cRT+\TextMani & \textcolor{CarnationPink}{62.74} & \textcolor{RedViolet}{48.60} & \textcolor{RedViolet}{29.67} & \textcolor{RedViolet}{51.47} \\
         \bottomrule
    \end{tabular}
    }
    % \vspace{-3mm}
    \caption{Long-tail classification accuracy(\%) on ImageNet-LT with ResNext50. 
    We compare with LWS, cRT, and $\TextMani$ on cRT, and color the value as \textcolor{RedViolet}{best}, \textcolor{CarnationPink}{second best}, and \textcolor{gray}{third best}.
...
In \Tref{tab:ImageNetLT_512} for ImageNet-LT, we compare with LWS~\cite{kang2019decoupling}, cRT~\cite{kang2019decoupling}, and $\TextMani$ on cRT. 
% This is the same experiment with Table~\textcolor{blue}{3} in the main paper but with a different batch size of 512.
The result shows that our $\TextMani$ on cRT achieves the best results compared to the counterparts in all classes except for the Many class, wherefrom ours achieves second best.
This is consistent with the result in Table~\textcolor{blue}{3} in the main paper regardless of different batch sizes.
The overall results show that our $\TextMani$ is capable of boosting well-established works, \eg, LWS, and cRT.
...
\footnotesize
    ...
    Baseline represents random horizontal flip, random crop, and normalization, which are basically applied in all experiments.
...
\subsection{Additional Experiments}\label{sec4.4}
