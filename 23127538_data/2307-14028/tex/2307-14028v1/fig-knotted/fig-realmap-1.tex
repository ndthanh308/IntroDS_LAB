...
\begin{document}
   \begin{tikzpicture}[scale=0.73,every node/.style={scale=1.1},even odd rule]
        ...
        \draw[ thick]
            (1,-0.01) circle (1.6pt) node[below]{\small$\mathsf{u}(-1)$}
            (11,-0.01) circle (1.6pt) node[below]{\small$\mathsf{u}(1)$};
        % \draw[red, thick]
        ...
        \draw[thick,postaction = decorate,decoration = {markings, mark = at position 0.76 with {\arrowreversed{>}} }] (5.6,3) circle (0.65cm) node[left=0.36cm]{$g$};
