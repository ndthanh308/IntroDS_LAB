...
\begin{equation}
ds^2=a^2(\eta)
\left[ -\left( 1+2\Phi \right)d\eta^2 +(1-2\Psi)\left( dr^2+r^2 (d\theta^2 + \sin^2\theta d\varphi^2 \right) \right]\,
\end{equation}
...
=\mathcal{W}(z)dzd\Omega\,,
\end{equation}
where $d\Omega$ is the infinitesimal solid angle, $d^{(0)}_A$ is the background angular distance, $\rho^{(0)}$ is the background density which can be expressed as $\rho^{(0)}(z)=3H^2(z)$ thanks to the Friedmann equation, the denominator comes from the background expansion of the number counts, and we have defined $\mathcal{W}(z)\equiv-\left[\frac{\rho(z)\,d^2_A(z)}{(1+z)H(z)}\right]^{(0)}$. We are then left with reporting the linear order galaxy number counts~\cite{Yoo:2009au,Yoo:2010ni,Challinor:2011bk,Bonvin:2011bg}. The leading terms in the weak field expansion are given by \cite{DiDio:2014lka}
...
We first provide some general preliminaries needed to follow our derivations. Technical details are reported in the Appendix~\ref{app:analytic}. In order to compute the different contribution to the third moment defined in Eqs.~\eqref{eq:skewness_label} is convenient to introduce the generalized Hankel transform of the matter power spectrum at two given times $\eta_1$ and $\eta_2$, namely  $P(k,\eta_1,\eta_2)\equiv P(k)D_1(\eta_1)D_1(\eta_2)$, as
\beq
...
&=&\overline{\Delta_2\psi\left( r_1,\bn \right)\pa_a\Delta_2\psi\left( r_2,\bn \right)}=0\,.
\label{eq:2point}
\eea
We remark that 
\beq
...
To evaluate this correlation, we use again the Poisson equation to relate $\Psi^{(2)}$ to the second order matter fluctuations which can be express in perturbation theory~\cite{Goroff:1986ep,Bernardeau:2001qr} as
\beq
\delta^{(2)}(\eta,\bk)=\int\frac{d^3k_1d^3k_2}{(2\pi)^{3}}
\delta_{D}\left( \bk-\bk_1-\bk_2 \right)
F_2\left( \bk_1,\bk_2 \right)
\delta\left( \eta,\bk_1 \right)\delta\left( \eta,\bk_2 \right)\,,
\label{eq:delta2}
\eeq
with
\beq
F_2\left( \bk_1,\bk_2 \right)=\frac{5}{7}
+\frac{1}{2} \frac{\bk_1\cdot\bk_2}{k_1 k_2} \left( \frac{k_1}{k_2} +\frac{k_2}{k_1} \right)
+\frac{2}{7} \left( \frac{\bk_1\cdot\bk_2}{k_1 k_2} \right)^2\,.
\label{eq:F2}
\eeq
From Eq.~\eqref{eq:delta2} we see directly that the stochastic average of $\delta^{(2)}$ vanishes. Indeed, we have that $\overline{\delta^{(2)}}\propto\overline{\delta\left( \eta,\bk_1 \right)\delta\left( \eta,\bk_2 \right)}\sim \delta_D(\bk_1+\bk_2)$. Hence, because of the Dirac-delta, this automatically selects only the monopole at $\bk =0$, with $\bk_1=-\bk_2$. However, $F_2\left( \bk_1,-\bk_1 \right)=0$.
Given these features we then have that
\bea
&&\overline{\Delta_2 \psi \left( r_1 ,\bn \right) \Delta_2 \psi \left( r_2, \bn \right) \Delta_2 \Psi^{(2)} \left( r_3, \bn \right)}
=-2\,C \int  \frac{d^3k_1\,d^3k_2\,d^3k_3}{\left( 2 \pi \right)^{6}}
F_2\left( k_1 , k_2,k_3 \right)
\nonumber\\
&&\times
\delta_D \left( \bk_1 + \bk_2 + \bk_3 \right)
\frac{P \left( k_1,\eta_1,\eta_3\right)
P \left( k_2,\eta_2,\eta_3 \right)}{k_1^2 k_2^2 k_3^2 }
\Delta_2 e^{i \bk_1 \cdot \bn r_1} 
\Delta_2 e^{i \bk_2 \cdot \bn r_2}  
\Delta_2 e^{i \bk_3 \cdot \bn r_3}\,,
\label{eq:3point}
\eea
where\footnote{This factor follows from the fact that the transfer function for $\delta^{(2)}$ is linked to the transfer function of $\Psi^{(2)}$ by the Poisson equation, which preserves the form as the linear one reported in Eq.~\eqref{eq:transf_dict}.} $C\equiv\frac{27}{8}\frac{\HH_0^6\,\Omega_{m0}^3}{a(r_1)a(r_2)a(r_3)}$
and, following \cite{DiDio:2014lka}, the kernel $F_2$ has been rewritten in terms of a third vector $\bk_3$ rather than the angle ${\hat\bk}_1\cdot{\hat\bk}_2$ as
\bea
F_2\left( k_1 , k_2 ,k_3 \right) &=&
-\frac{5 \left(k_1^2-k_2^2\right)^2}{28\,k_1^2 k_2^2}
+\frac{3}{28} \left(\frac{1}{k_1^2}+\frac{1}{k_2^2}\right)k^2_3
+\frac{k^4_3}{14\,k_1^2 k_2^2}\,.
\label{eq:redF}
\eea
At this point, a comment about the factor 2 in Eq.~\eqref{eq:3point} is in order. In fact, due to Wick theorem, the stochastic average on the r.h.s. of Eq.~\eqref{eq:3point} accounts for all the permuted 2-point functions of the linear matter field and this should contain three different terms. However, since $\overline{\delta^{(2)}}=0$, as discussed after Eq.~\eqref{eq:F2}, only two permutations survive. Moreover, these two terms are equal due to the symmetry under the exchange $r_1\leftrightarrow r_2$. This, then, leads to the factor 2 mentioned above.

From Eq.~\eqref{eq:3point}, we can write the 3-point function as
\bea
&&\overline{\Delta_2 \psi \left( r_1 ,\bn \right) \Delta_2 \psi \left( r_2, \bn \right) \Delta_2 \Psi^{(2)} \left( r_3, \bn \right)}
=2\frac{C(r_1,r_2,r_3)}{2\pi^5}  \sum_{\ell_1\ell_2\ell_3} \ell_1 \left( \ell_1 +1 \right) \ell_2 \left( \ell_2 +1 \right) \ell_3 \left( \ell_3 +1 \right)\nonumber\\
&&\times\left( \begin{array}{ccc}
\ell_1 & \ell_2 & \ell_3 \\ 0 & 0 & 0 
\end{array} \right)^2 
\left( 2\ell_1 +1 \right)\left( 2\ell_2 +1 \right)\left( 2\ell_3 +1 \right)
\int  dk_1\,dk_2\,dk_3\,dx\,x^2
P \left( k_1,\eta_1,\eta_3 \right)
P \left( k_2,\eta_2,\eta_3 \right)
\nonumber\\
&&\times F_2\left( k_1 , k_2,k_3 \right)
...
\bea
&&\overline{\Delta_2 \psi \left( r_1 ,\bn \right) \Delta_2 \psi \left( r_2, \bn \right) \Delta_2 \Psi^{(2)} \left( r_3, \bn \right)}
\approx 2\frac{C}{\left(4\pi\right)^2}  \sum_{\ell_1\ell_2\ell_3} \ell_1 \left( \ell_1 +1 \right) \ell_2 \left( \ell_2 +1 \right) \ell_3 \left( \ell_3 +1 \right)\nonumber\\
&&\times\left( \begin{array}{ccc}
\ell_1 & \ell_2 & \ell_3 \\ 0 & 0 & 0 
\end{array} \right)^2 
\left( 2\ell_1 +1 \right)\left( 2\ell_2 +1 \right)\left( 2\ell_3 +1 \right)
...
\nonumber\\
&&\times \frac{P \left( \widetilde{k_1},\eta_1,\eta_3 \right) P \left( \widetilde{k_2},\eta_2,\eta_3 \right)}{\widetilde{k_1}^2\widetilde{k_2}^2\widetilde{k_3}^2}
F_2\left( \widetilde{k_1},\widetilde{k_2},\widetilde{k_3} \right)\,,
\label{eq:bis2}
\eea
...
\bea
\mu^{LSS}_3&=&
6 \sum_{\ell_1\ell_2\ell_3} \ell_1 \left( \ell_1 +1 \right) \ell_2 \left( \ell_2 +1 \right) \ell_3 \left( \ell_3 +1 \right)\left( \begin{array}{ccc}
\ell_1 & \ell_2 & \ell_3 \\ 0 & 0 & 0 
\end{array} \right)^2\frac{\left( 2\ell_1 +1 \right)\left( 2\ell_2 +1 \right)\left( 2\ell_3 +1 \right)}{\left( 4\pi \right)^2}\nonumber\\
&&\times\int_0^{r_s}dx\, \frac{C(x,x,x)}{x^4}
\left(\frac{x-r_s}{x\,r_s}\right)^3
\frac{P \left( \widetilde{k_1},\eta_x,\eta_x \right)
P \left( \widetilde{k_2},\eta_x,\eta_x \right)}{\widetilde{k_1}^2\widetilde{k_2}^2\widetilde{k_3}^2}
F_2\left( \widetilde{k_1},\widetilde{k_2},\widetilde{k_3} \right)\,,
\label{eq:bis3}
\eea
...
With such smoothing scale, all the integrals over the momentum $k$ converge quickly and the choice of $k_{\rm max}$ become numerically irrelevant. Since the time required to evaluate $\mu_3^{LSS}$ scales with $k_{\rm max}^3$, we set $k_{\rm max}$ to achieve the percent level precision.

\subsection{Quadratic and Post-Born terms}
We start our numerical investigation by looking at the contributions to the skewness arising from the linear gravitational potential, derived in Sect.~\ref{sec:quad} and \ref{sec:PB}. For this discussion, we will use the cutoff in real space on small scales as prescribed by Eq.~\eqref{eq:windowed_PS}. Results are shown in Fig.~\ref{fig:lin} respectively for the choices of $\rho = 5,\,10,\,20$ Mpc/$h$. For each case, we consider linear matter power spectrum (left panels) and HaloFit model for the matter power spectrum (right panels).
...
Once again, we perform numerical investigations by using linear matter power spectrum and HaloFit model. For different values of the coarse-graining scale $\rho$, results are shown in Fig.~\ref{fig:LSS_total}.
The numerical results are reported in Fig.~\ref{fig:LSS_total}, where we have adopted linear power spectrum in the left panel and HaloFit model in the right one.
% Figure environment removed
...
A similar behavior occurs also for the case $\rho = 10$ Mpc/$h$. Even in this case, the value of $\rho$ is still such that non-linear features in the power spectrum are marginally relevant, and then the HaloFit model returns results that are almost alongside the ones obtained by the linear power spectrum. However, we notice that the overall amplitude increases quite a lot when we lower the coarse-graining scale from 20 to 10 Mpc/h. As a quantitative instance, the comparison between red and blue curves in Fig.~\ref{fig:LSS_total} at $z=0.1$ exhibits an increasing in the absolute value of the skewness that is of $\sim 40\%$. This confirms that a significant amount of information is encoded in the skewness within the scales of 10 to 20 Mpc/h. However, as the prediction from linear theory does not deviate significantly from the estimation using HaloFit, we are still within a regime where perturbation theory has not yet failed.

Finally, with orange curves in Fig.~\ref{fig:LSS_total} we report the numerical results for $\rho = 5$ Mpc/$h$. For this case study, we have new emerging features. First of all, we start to appreciate a more prominent difference between the linear power spectrum and the non-linear one, and this difference is more evident at smaller redshifts. In fact, here we have an enhancement when the HaloFit model is considered of $\sim 10\%$, as can be appreciated also in Fig.~\ref{fig:LSS_relative} where we show the analogous of Fig.~\ref{fig:relative} but only for $\kappa^{LSS}_3$.
% Figure environment removed
...
Hence, by considering the discrepancy between the linear power spectrum and HaloFit as an indicator of the validity range of perturbation theory, we observe that for $\rho=5$ Mpc/h, the accuracy of perturbation theory cannot be trusted beyond approximately $10\%$.
