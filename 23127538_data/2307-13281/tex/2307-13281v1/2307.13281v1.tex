...
\subsection{Notation} We write $\bV(k)$ for the category of smooth projective varieties over a field $k$, and $\Sm(S)$ for the category of smooth separated schemes of finite type over a base scheme $S$. An \emph{arithmetic scheme} is an object of $\Sm(\Spec \Z)$.
...
\begin{thm}\label{t0} For any $n\ge 0$, the image of $\tilde\cl^n_X$, and a fortiori that of $\cl^n_X$, is a finitely generated $\Z_l$-module.\qed
\end{thm}
...
\end{CD}\]
where $\sX$ is an arithmetic model of $X$, using the finite generation of $H^{2n}_\cont(\sX,\Z_l(n))$.)

\section{Motivic cohomology}\label{s2.1} 

\subsection{Refined motivic cycle class maps} In the sequel we shall use a generalisation of \eqref{eq0bis} to all motivic cohomology:
...
\Z(n)_\et\otimes \Z/l^\nu\simeq \mu_{l^\nu}^{\otimes n};
\end{equation}
extended to arithmetic schemes in \cite[Th. 1.2 4]{geisser3}. Since the étale cohomology of $\sX$ with finite coefficients is finite, $\beta^{n,i}_\sX$ may simply be seen as $l$-adic completion. To pass to smooth schemes over a field, we use the continuity of motivic and étale motivic cohomology (commutation with filtering inverse limits of schemes with affine transition morphisms): this follows from
\begin{itemize}
\item continuity of the cycle complexes themselves, 
\item continuity of Zariski and étale cohomology (see \cite[Th. 5.7]{sga4} for the latter\footnote{Strictly speaking, this reference concerns étale cohomology with coefficients in a system of sheaves, while we consider here a system of complexes of sheaves which are not [known to be] bounded below. One can proceed as in \cite[\S 2C]{cycletale}, by using \eqref{eq8} and \cite[Prop. 3.6]{geisser3}.}).
\end{itemize}
...
This example seems basic; it already appears in \cite{sc-suz} and \cite{ct-sc}, and will reappear in Sections \ref{s4.4} and \ref{s4.5}.


\section{The case of positive characteristic}\label{s2}

\subsection{The Tate-Beilinson conjecture} We start with a smooth projective variety $X$ over $k=\F_p$. Let $\bar k$ be an algebraic closure of $k$, $G=Gal(\bar k/k)$ and $\bar X=X\otimes_k \bar k$. There are two basic conjectures:
...
But the left hand side is $0$ by (Deligne's proof of) the Weil conjectures, so the cohomological Tate conjecture is equivalent to the \emph{surjectivity} of $\cl^n_X\otimes \Q$.
...
\begin{rque} I don't know if Theorem \ref{p1} should hold when replacing $\tilde H^{2n}_\cont$ by $H^{2n}_\cont$. However, c) is sufficient to imply the Bloch-Beilinson--Murre conjectures \cite{jann3}, as noted in \cite[Th. 8.32]{sheaf} (which also shows that the Tate-Beilinson conjecture implies the standard conjectures and the cohomological Tate conjecture over $K$).
\end{rque}
...
\begin{thm}\label{t3} Assume that $CH_0(X_{k(X)})\otimes \Q\by{\deg\otimes \Q} \Q$ is an isomorphism. Let $\delta=|\Coker \deg|$. Consider the following two cases:
\begin{itemize}
\item $\delta=1$, or
\item $k$ is of Kronecker dimension $\le 3$.
\end{itemize}
Then\\
a) If $\car k=0$,  $\Ker \cl^2_X$ is finite and $CH^2(X)$ is  finitely generated. In general, $CH^2(X)$ is an extension of a finitely generated group by a group of exponent $\delta$ and $\Ker \cl^2_X$ is killed by $\delta$; the group $\Ker \beta^2_X$ is torsion.\\
...
1) The general conclusion is true with $\delta$ replaced by $n$.

2) The case $X(k)\neq \emptyset$.

3) Refining $n$ to $\delta$ in 1).

4) The case of Kronecker dimension $\le 3$.
\bigskip
...
In particular, the two maps $\rho$ in the diagram are surjective (especially the bottom one), and we reduce to see that the middle map $a'$ is surjective. 
...
\begin{rques} \label{r1} a) The only new thing in Theorem \ref{t3} is the case of Kronecker dimension $3$: indeed, in characteristic $0$ the finite generation of $CH^2(X)$ goes back to \cite[th. 4.3.10]{colliot} when $\delta=1$ or $k$ is a number field, and the injectivity of $\cl^2_X$ on torsion when $\delta=1$ goes back to \cite[Theorem]{saito} under the weaker hypotheses $H^1(X,\sO_X)=H^2(X,\sO_X)=0$. (The statement for general $\delta$ in Theorem \ref{t3} is mainly for decoration.) The proofs given here are different.\\
b) The case of Kronecker dimension $4$ fails because we cannot prove that $CH^2(\sV)=0$ for $S$ small enough: this would follow from the finite generation of $CH^2$ for arithmetic $3$-folds, which is wide open in general.\\ 
...
\subsection{\protect{\cite[Th. 2.3]{sc-suz}}}\label{s4.1} In this reference, a counterexample to the injectivity of $CH^3(X)\allowbreak \otimes \Z_2\to H^6_\cont(X,\Z_2(3))$ is given for a smooth projective variety $X$ over a field $k_0$, which is a Godeaux-Serre variety associated to a finite $2$-group $G$; the culprit comes from a class $\alpha\in CH^3(BG)$ such that $\alpha_{\bar k_0}\ne 0$ but $\cl^3_{BG}(\alpha)_k=0$ for some finite extension $k$ of $k_0$. In fact,  we already have $\alpha^3_{BG}(\alpha)_{\bar k_0}=0$, hence $\alpha^3_{BG}(\alpha)_k=0$ for suitable $k$ since étale motivic cohomology commutes with filtering limits of schemes (this trivialises the delicate descent argument in the proof of loc. cit., Lemma 2.1 (b)). Indeed, by \cite[Th. 7.1]{BG}, we have
\[H^i_\et(BG_{\bar k_0},\Z(n))\simeq H^i_\et(\bar k_0,\Z(n))\oplus H^i(G,\Z)(n)\]
so we are reduced to a computation in the cohomology of $G$, which is done in \cite[\S 5]{totaro}.
...
factors through $CH^3_\et(X_{\bar k_0})\otimes \Z_2$.
...
Here $X$ is a rational surface over $k=\Q(\sqrt{-p})(t)$, where $p$ is a prime number $\equiv -1\pmod{3}$. More generally, Colliot-Thélène and Scavia consider smooth projective surfaces satisfiying strong conditions  in the spirit of \cite{saito}. Here is a version of their results in this generality, using $CH^2_\et(X)$ and $\beta^2_X$. For simplicity, we neglect $p$-torsion if $k$ is of characteristic $p>0$.


\begin{thm}\label{t4} Let $X$ be a surface over a field $k$, verifying Hypothesis (H4) of \cite[\S 3]{ct-sc}.\\
...
E_2^{p,q}=H^p(k,\bar H^q_\et(\bar X,\Z(2)))\Rightarrow \bar H^{p+q}_\et(X,\Z(2))
\end{equation}
where $\bar H^*_\et(\bar X,\Z(2))$ is defined similarly, for $\bar X$. Here, we can use Proposition \ref{l5} to write
\[\bar R\Gamma_\et(\bar X,\Z(2)) = R\Gamma_\et(\NS_X\otimes \L \oplus t_2,\Z(2)).\]

We now note that the idempotent in $CH^2(\bar X\times \bar X)$ defining $\NS_X\otimes \L$ is Galois-invariant by \cite[Lemma 14.2.2]{kmp}, which works for the $\Z$-perfect pairing on $\NS(\bar X)$. Accordingly, the direct sum decomposition
\begin{equation}\label{eq14}
\bar R\Gamma_\et(\bar X,\Z(2))  = R\Gamma_\et(t_2,\Z(2))\oplus R\Gamma_\et(\NS_X\otimes \L,\Z(2))
\end{equation}
...
\bar H^q_\et(\bar X,\Z/l^n(2))=H^q_\et(\bar k,\Z/l^n(2))\oplus H^q_\et(t_2,\Z/l^n(2))\oplus H^{q-4}_\et(\bar k,\Z/l^n)
\end{equation}
 for any integer $n>0$, which already gives $H^q_\et(t_2,\Z/l^n(2))=0$ for $q\notin [1,3]$. Then, \cite[(H1) and (H3)]{ct-sc} also yield $H^q_\et(t_2,\Z/l^n(2))=0$ for $q=1,3$. Finally, Hypothesis (H1) implies that $\Br(\bar X)=0$ and that $H^2_\cont(\bar X,\Z_l(2))\to H^2_\et(\bar X,\Z/l^n(2))$ is surjective; since the image of the projector defining $t_2$ acting on $H^2_\cont(\bar X,\Z_l(2))$ equals $T_l(\Br(\bar X))(1)$, we also get $H^2_\et(t_2,\Z/l^n(2))\allowbreak=0$. The unique divisibility follows from the long cohomology exact sequence.
 \end{proof}
...
\bibitem{tatediv} B. Kahn {\it Sur la conjecture de Tate pour les diviseurs},  \url{https://arxiv.org/abs/2205.05287}, to appear in Ess. Numb. Theory.
