...
\begin{itemize}
    ...
    \item A  disparity compensation algorithm (DCA) to identify the optimal value of compensatory bonus points to minimize disparity for a top-$k$ selection set. DCA runs in sub-linear time\cut{We also propose a variation over our main algorithm (DCA/CMA-ES) that focuses on reducing disparity when intervention over some attributes is limited, because of legal requirements or lack of information.} (Section~\ref{sec:algo}). We propose a modification of DCA that takes into account the whole ranking, using logarithmic discounting techniques~\cite{yang2017measuring}, to adapt to different selection sizes (Section~\ref{sec:log_discount}).
