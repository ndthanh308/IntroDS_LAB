...
In HeteFedRec, we present a heterogeneous recommendation model aggregation strategy, including a unified dual-task learning mechanism and a dimensional decorrelation regularization, to allow knowledge aggregation among recommender models of different sizes.
...
Recently, there has been a significant surge in demand for recommender systems in various online services, including e-commerce~\cite{chen2020try,wei2007survey}, social media~\cite{yin2016spatio}, and online news~\cite{wu2020mind},
since recommender systems can effectively alleviate information overload by actively discovering users' potential interests.
...
This setting conflicts with the real applications where clients often possess heterogeneous resources, such as varying amounts of training data and computing power. 
...
In the field of federated learning, several endeavours have been undertaken to achieve model heterogeneity~\cite{ma2022state}.
These endeavours can generally be classified into two categories.
The first category~\cite{chang2019cronus,li2019fedmd,sun2020federated} involves leveraging knowledge distillation~\cite{hinton2015distilling} to exchange knowledge over heterogeneous models via a public reference dataset.
...
Some works combine elements from both of these solutions to achieve their goals~\cite{liu2022no,kim2023depthfl}.
However, all existing heterogeneous federated learning methods cannot be directly used in FedRecs because of the following inherent disparities between general federated learning and FedRecs. 
\begin{itemize}
\item \textbf{Difference in Data Structures}. 
In FedRecs, a data sample is typically formed as $(user, item, rating)$ which is highly associated with specific users. Consequently, constructing a public reference dataset to facilitate knowledge distillation while ensuring privacy protection becomes impractical.
...
\end{itemize}
As a result, implementing model heterogeneity is non-trivial in federated recommendations.
...
Since item embeddings significantly influence the entire recommendation model and user embeddings are typically private parameters not shared among clients, HeteFedRec primarily focuses on heterogeneous item embeddings. The conventional idea is to employ the padding method to aggregate item embeddings of different sizes. However, this aggregation strategy suffers from a severe mismatch problem because parameters with the same indexes in different embedding tables may represent different latent factors and meanings. 
...
The experimental results demonstrate the superiority of HeteFedRec over the baselines.
...
Related work is reviewed in Section~\ref{sec_relatedwork}, followed by the introduction of FedRecs' preliminaries in Section~\ref{sec_preliminaries}.
Section~\ref{sec_methodology} describes the technical details of HeteFedRec.
Then, in Section~\ref{sec_experiments} we present extensive experimental results with comprehensive analysis.
Finally, Section~\ref{sec_conclusion} briefly summarizes this paper.



\section{Related Work}\label{sec_relatedwork}
In this section, we briefly review the literature on three related topics: federated recommender systems, heterogeneous federated learning, and knowledge distillation. Other involved topics such as general recommender systems and federated learning can be referred to corresponding surveys~\cite{hung2017computing,nguyen2017argument,yu2023self,zheng2023automl,zhang2021survey}.

\subsection{Federated Recommender Systems} 
Due to the privacy-preserving ability, FedRecs have received remarkable attention recently and have been widely deployed in many recommendation scenarios such as news recommendation~\cite{qi2020privacy}, social recommendation~\cite{liu2022federated}, POI recommendation~\cite{guo2021prefer}, and so on~\cite{imran2023refrs}.
...
Building upon this basic framework, extensive research has been proposed to improve the performance of FedRecs in a short time.
...
\cite{zhang2022comprehensive,yuan2023interaction,yuan2023federated} reveal that FedRecs still suffer potential privacy issues since adversaries can steal user-sensitive information and the system cannot forget quit clients.
\cite{wu2022fedattack,yu2023untargeted} show that it is easy to destroy a federated recommender system.
...
Data heterogeneity research focuses on learning a global model with clients whose training data is non-IID~\cite{sun2021heterogeneous,ma2022state}.
...
On the other hand, research on system heterogeneity considers variations in computing power among different participants~\cite{jiang2022fedmp,wang2022fedadmm}.
...
For example, ~\cite{diao2020heterofl,horvath2021fjord,zhu2022resilient} design width-level aggregation for parameters with different channel scales.
...
Based on layer-level aggregation, \cite{jiang2022fedmp} applies neural pruning techniques to create heterogeneous models.
...
In this section, we provide a formal introduction to the fundamental framework of current federated recommender systems followed by the base recommendation models.
...
In the context of FedRecs, each user $u_{i}$ possesses a local dataset $\mathcal{D}_{i}$ with the size $\left|\mathcal{D}_{i}\right|$ varying among users as shown in Fig.~\ref{fig_distribution}.
...
\begin{equation}
    ...
    \end{aligned}
  \end{equation}
where $FFN(\cdot)$ is a feedforward network, and its parameters are included in $\mathbf{\Theta}$, $[\cdot]$ denotes concatenation operation, $\sigma(\cdot)$ is a sigmoid function.
...
Subsequently, user and item feature vectors are computed by propagating the embeddings of neighbouring nodes:
\begin{equation}
  \label{eq_lightgcn}
  \begin{aligned}
    \mathbf{u}_{i}^{l} = \sum\limits_{j\in \mathcal{N}_{u_{i}}}\frac{1}{\sqrt{\left| \mathcal{N}_{u_{i}} \right|} \sqrt{\left| \mathcal{N}_{v_{j}} \right|}}\mathbf{v}_{j}^{l-1}\\
    \mathbf{v}_{j}^{l} = \sum\limits_{i\in \mathcal{N}_{v_{j}}}\frac{1}{\sqrt{\left| \mathcal{N}_{v_{j}} \right|} \sqrt{\left| \mathcal{N}_{u_{i}} \right|}}\mathbf{u}_{i}^{l-1}
  \end{aligned}
\end{equation}
where $\mathcal{N}_{u_{i}}$ and $\mathcal{N}_{v_{j}}$ represent the neighbors of $u_{i}$ and $v_{j}$ respectively.
$l$ is the number of propagation layers.
Note that in FedRecs' settings, the propagation of LightGCN is limited on each client's private dataset $\mathcal{D}_{i}$ for privacy concerns.
After $L$ layers' propagation, we obtain the final user and item embeddings:
\begin{equation}
  \mathbf{u}_{i} = \sum\limits_{l=0}^{L} \mathbf{u}_{i}^{l}, \quad \mathbf{v}_{j} = \sum\limits_{l=0}^{L} \mathbf{v}_{j}^{l} 
\end{equation}
...
In this paper, without loss of generality, we categorize clients into three groups based on the scale of user-item interactions: small clients $\mathcal{U}_{s}$, medium clients $\mathcal{U}_{m}$, and large clients $\mathcal{U}_{l}$.
...
In general, $\mathbf{V}$ dominates the number of parameters in the entire recommender system.
...
According to the above aggregation, when HeteFedRec initializes $\mathbf{V}_{s}^{0}$, $\mathbf{V}_{m_{[:N_{s}]}}^{0}$ and $\mathbf{V}_{l_{[:N_{s}]}}^{0}$, $\mathbf{V}_{m_{[N_{s}:N_{m}]}}^{0}$ and $\mathbf{V}_{l_{[N_{s}:N_{m}]}}^{0}$ from the same point respectively, we can get following relationship:
...
To address the issue of update mismatch, HeteFedRec employs a unified dual-task learning mechanism to ensure submatrices within larger item embeddings share the same objective as small item embeddings. 
...
Fig.~\ref{fig_dual_task} illustrates the process of unified dual-task learning.
...
Our distillation method is based on the idea that item embeddings of different sizes can transfer knowledge through spatial information and the ensemble spatial information derived from $\{\mathbf{V}\}_{s,m,l}$ is more reliable than that from a single embedding table. 
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The calculated knowledge distillation updates $\{\nabla_{kd} \mathbf{V}\}_{s,m,l}$ are used to update corresponding embedding tables.
Note that to avoid too many hyperparameters, we use the same learning rate $lr$ here.


Algorithm~\ref{alg_hetefedrec} summarizes the framework of HeteFedRec using pseudocode.
...
Since this is the first work on the heterogeneous federated recommender system, we construct the following baselines for comparison:
\begin{itemize}
  ...
  \item \textbf{Clustered FedRec.} Following the clustered scheme in federated learning~\cite{ghosh2020efficient,meng2022improving}, we construct Clustered FedRec. It conducts aggregation for the same type of model.
...
The regularization factor $\alpha$ is set to $1.0$ by default.
...
This simple training rule can make sure all clients evenly and timely participate in the training process.
...
All differences between our framework and others are statistically significant ($p<0.01$). 
...
However, when the model size continued to increase to $32$ and $128$, FedRecs' performance dropped to about $0.040$ NDCG scores, suggesting that some clients' data are not sufficient to support training with large models.
...
In both Fed-NCF and Fed-LightGCN, the trends indicate that HeteFedRec's performance initially increases to a peak point ($\alpha=1.5$ and $\alpha=1.0$ in Fed-NCF and Fed-LightGCN, respectively) and then decreases with a further increase in $\alpha$. 
