...
With the assumption $\La_2\gg\La_1\gg v$, both the $\mathcal{S}_1, \mathcal{A}_1$ mass splitting $(m^2_{\mathcal{S}_1}-m^2_{\mathcal{A}_1})/m^2_{\mathcal{S}_1,\mathcal{A}_1}$ and the mixing angles $\theta^2_{R,I}$ are proportional to $\la^2v^2/\La^2_{2}$. Hence, the observed neutrinos obtain small Majorana masses, $m_\nu\sim (h^\nu)^2\la^2v^2\La_1/32\pi^2\La_{2}^2$, as expected. For example, if $h^\nu\sim 0.01$, $\la\sim 0.02$, the observed value $m_\nu\sim 0.1$ eV requires $\La_1\sim 1$ TeV and $\La_2\sim 10$ TeV.
...
\subsection{LHC}
At the LHC experiment, the new gauge boson $Z'$ can be resonantly produced via the fusion processes $\bar{q}q\to Z'$ in which $q$ indicates the SM quarks, and this boson subsequently decays into the SM fermions as well as the exotic particles if kinetically allowed. Furthermore, the most significant decay channel of $Z'$ is given by $Z'\to l^+l^-$ with $l=e,\mu,\tau$ because of well-understood backgrounds \cite{ATLAS:2017fih} and also that it signifies a $Z'$ having both couplings to lepton and quark like ours. Using the narrow width approximation, the cross section for the relevant process takes the form, 
\be
\sigma(pp\rightarrow Z'\rightarrow l^+l^-) \simeq \frac{\pi}{3}\sum_{q}L_{q\bar{q}}(m^2_{Z'})(C^{Z'}_q)^2 \frac{\Gamma(Z'\rightarrow l^+l^-)}{\Gamma_{Z'}},
\ee
where the parton luminosity $L_{q\bar{q}}$ is given by \cite{Martin:2009iq}
\begin{eqnarray}
L_{q\bar{q}}(m^2_{Z'})=\int^1_{\frac{m^2_{Z'}}{s}}\frac{dx}{xs}\left[f_q(x,m^2_{Z'})f_{\bar{q}}\left(\frac{m^2_{Z'}}{xs},m^2_{Z'}\right)+f_q\left(\frac{m^2_{Z'}}{xs},m^2_{Z'}\right)f_{\bar{q}}(x,m^2_{Z'})\right]
\end{eqnarray}
where $\sqrt{s}$ is the collider center-of-mass energy, and $f_{q(\bar{q})}(x,m^2_{Z'})$ is the parton distribution function of the quark $q$ (antiquark $\bar{q}$), evaluated at the scale $m_{Z'}$. Additionally, the partial decay width of $Z'$ boson is approximately given by 
\be \Gamma(Z'\rightarrow l^+l^-)\simeq \frac{m_{Z'}}{12\pi}(C_l^{Z'})^2, \ee
while the total $Z'$ decay width is estimated as
\bea
...
From the upper limits set for the cross section of this process at $95\%$ credibility level using $36.1$ fb$^{-1}$ of proton-proton collision at $\sqrt{s}=13$ TeV by ATLAS experiment \cite{ATLAS:2017fih}, we obtain the corresponding lower limits for the new physics scale, which depends on the $|z|g_X$ product, as described by the purple curves in Fig. \ref{fig2}. The left panel corresponds to the convention $U(1)_X$ model, assuming $m_{\nu_{iR}}=2m_\xi/3=4m_\eta/3=m_{Z'}/3$, while the right panel corresponds to the alternative $U(1)_X$ model, given that $m_{\nu_{3R}}= m_{Z'}/2$. 

For comparison, in each of panels in Fig. \ref{fig2}, we add the lower bounds on the new physics scale which come from the FCNCs (red curve) and the LEP-II (blue curve), besides the purple curve from ATLAS. The available regions for the new physics scale lie above these three curves. It is easy to see that the constraint from the LHC (FCNCs) is the strongest if the coupling strength $|z|g_X$ is smaller (larger) than about $0.14$ for the convention $U(1)_X$ model and $0.12$ for the alternative $U(1)_X$ model.
...
Let $\La=15$ TeV satisfy the limits from the left panel in Fig. \ref{fig2}, which requires $|z|g_X\gtrsim 0.13$, and take $m_{\nu_{iR}}=m_{Z'}/3$. We make the blue contours of the correct dark matter relic density, i.e. $\Om_\xi h^2\simeq 0.12$ \cite{Planck:2018vyg}, as a function of the fermion dark matter mass $m_\xi$ and the coupling strength $|z|g_X$, as shown in Fig. \ref{fig3}. The dashed red line corresponds to $|z|g_X= 0.13$, and the region that lies below this line is excluded. From this figure, we obtain the lower bound for fermion dark matter mass to be $m_\xi\gtrsim 1.7$ TeV.
% Figure environment removed 
...
\be \sigma^{\text{SI}}_\xi=\frac{9\mu^2_{\xi\mathcal{N}}}{4\pi}\frac{1}{m^4_{Z'}}(C^{Z'}_\xi)^2(C^{Z'}_u)^2\simeq 1.744\times 10^{-45} \left(\frac{m_\mathcal{N}}{1\text{ GeV}}\right)^2\left(\frac{14.14\text{ TeV}}{\La}\right)^4 \text{ cm}^2,\ee
...
Taking $m_{\mathcal{N}}\simeq 1$ GeV and $\Lambda\gtrsim 14.14$ TeV as obtained from the left panel in Fig. \ref{fig2} for $|z|g_X\gtrsim 0.14$, the model predicts $\sigma^{\text{SI}}_\xi\lesssim 1.744\times 10^{-45} \text{ cm}^2$, in agreement with the XENON1T experiment if dark fermion mass satisfy $m_\xi\gtrsim 3.9$ TeV \cite{XENON:2018voc}. This lower bound for the fermion dark matter mass is stricter than that obtained from the correct density constraint.
