...
\author[tud]{Junzi Sun\corref{cor}}
\ead{j.sun-1@tudelft.nl}

\author[tud]{Esther Roosenbrand}
\ead{e.j.roosenbrand@tudelft.nl}

\cortext[cor]{Corresponding author}

\address[tud]{Faculty of Aerospace Engineering, Delft University of Technology,\\Kluyverweg 1, 2629 HS, Delft, the Netherlands}
...
\begin{keyword}
Contrail Detection \sep Hough Transformation \sep Image Augmentation \sep Machine Learning \sep SR Loss \sep Transfer Learning
\end{keyword}
...
Image augmentation provides a new way to generate training data using a small labeled dataset. We apply a set of transformations, also known as image augmentations \citep{buslaev2020albumentations} to the image dataset. This way, we can create a large range of scenarios with a small number of manually labeled images, which includes:
...
Generating an adequate amount of training data has always been a challenging topic in supervised machine learning studies. Labeling image data can be a tedious task for researchers. In this study, we took advantage of image augmentation to generate diverse transformations and qualities of satellite images based on a very small number of carefully labeled data. In total, only around 30 images were labeled, of which 10 were reserved for evaluating the performance of the model.

The labeled images contain GOES satellite data within a few days at two locations. This still poses a limitation of this study. However, given how well the model can be trained with such a small amount of data and perform with completely new sources of image data as shown in Section \ref{sec:other_source}, we can expect that the model could be further improved when more diverse image data sources are considered.

Hence, a recommendation for further improvement of the model is to create a dataset containing a small number of carefully labeled image sources from different satellites. This way, our models can be further trained on this richer dataset using the same image augmentation approach.
...
\section{Source Code and Additional Results} \label{sec:source_code}
