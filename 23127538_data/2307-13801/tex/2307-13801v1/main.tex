...
\title{Energy preserving evolutions over Bosonic systems} % Just an idea
...
% Motivation: Relevance of bosonic systems for quantum information, modelling of a quantum computer, laser/maser systems.
...
Given the significant technological and experimental relevance of CV systems, there is a pressing need for a rigorous mathematical theory of quantum dynamical semigroups over such systems.
...
\end{equation*}
In the latter, we refer to these spaces as Sobolev spaces and denote them by $W^{k,1}$ in analogy with their classical analogues (see also \cite{Becker.2021}). Next, we call operators $\cL$ that satisfy both conditions (i) and (ii) generators of Sobolev preserving quantum dynamical semigroups. Indeed, in our first main result, we show that such operators generate QMS with the extra property that the latter preserve Sobolev spaces. More precisely:

\begin{thm*}[Generation of bosonic semigroups, see \Cref{thm:generation-theorem}]
    Let $(\cL, \cD(\cL))$ be an operator defined on the Banach space $\cT_{1,\operatorname{sa}}$ of self-adjoint, trace-class operators. If $(\cL, \cD(\cL))$ satisfies conditions (i) and (ii) above, then its closure $\overline{\cL}$ generates a strongly continuous, positivity preserving semigroup $(\cP_t)_{t\ge 0}$ on $W^{k, 1}$ for all $k\geq 0$ with 
    \begin{equation*}
        \norm{\cP_t}_{W^{k, 1} \to W^{k, 1}} \le e^{\omega_k t} \,\quad \forall t\ge 0\, . 
    \end{equation*}
   where $\omega_k \coloneqq \frac{k}{k_0} \omega_{k_0}$ for $k_0 \ge k \ge 0$.
...
% \tm{Both assumption of our generation theorem are physical motivated and satisfied by a rich class of applications. The so called bosonic CAT code is claimed to be a universal gate set in quantum error correction \cite{Azouit.2016}. The rough idea is that the codespace is an invariant subspace of an evolution and thereby stabilized if the evolution is exponentially converging to the codespace. Especially, Azouit, Sarlette and Rouchon consider as evolution the $l$-photon dissipation evolution, i.e.
% \begin{equation*}
%     \frac{\partial}{\partial t}\rho_t=\cL[a^l-\alpha^l](\rho_t)
% \end{equation*}
% where $\cL[L] = L (\cdot) L^\dagger - \frac{1}{2}\{L^\dagger L, \cdot\}$, and manage to prove the well-posedness as well as the exponential convergence of states to the code-space spanned by phases of covariant state in $\alpha$ \cites{Azouit.2015}{Azouit.2016}. This is done via the generation theorem by Hille and Yosida and a Banach space constructed via $L$, compactly embedded in the self-adjoint trace class operators so that they circumvent the problems Davies, Fagnola and others encountered when trying to take limits of the minimal semigroups. However, this structure depends on the generator jump-operator $L$, which is generalized in our setting to the natural candidate at hand: the number operator.}\par
...
We begin with a short review of useful tools from Banach space theory in \Cref{subsec:banach-space} and build upon them to prove a compact embedding theorem for a class of weighted spaces in \Cref{subsec:weighted-norms-compact-embedding}. We then recall standard results from the theory of strongly continuous semigroups as well as evolution systems in \Cref{subsec:c0-semigroups}. These will play an important role in \Cref{sec:polynomial-generators}. Finally, we introduce continuous variable quantum systems, provide some useful properties of polynomials of annihilation and creation operators, and introduce the notion of a \textit{Sobolev preserving semigroup}. These semigroups are the main objects of study in the remainder of the paper.
...
\end{defi}
% \begin{rmk}
%     Another direct generalization could involve permitting sums of weighted norms and relaxing the invertibility assumption at the same time.
% \end{rmk}
...
\begin{proof}
    ...
    \end{equation*}
    shows that the graph norm of $\cW$ is equivalent to $\|\cdot\|_{\cW}$. By definition, the graph is a closed operator on the Banach space $\cX\cross\cX$ such that the vector space $\cD(\cX)$ equipped with the graph norm is a Banach space. The statement hence follows.
\end{proof}
...
\begin{itemize}
    ...
    \item[$(2)$\hspace{1ex}] There exists a subspace $\cY\subset \cX$ and a norm $\|.\|_\cY$ on $\cY$ endowing $\cY$ with a Banach space structure, such that for all $ s\ge 0$, $(\cY,\|.\|_\cY)$ is \textit{$\cL_s$-admissible} and $(\cL_s)_{s\ge 0}$ is stable on $(\cY,\|.\|_\cY)$\,;
    \item[$(3)$\hspace{1ex}] Finally, the map $s\mapsto\cL_s\in\cB((\cY,\|.\|_\cY),(\cX,\|.\|_\cX))$ is uniformly continuous.
\end{itemize}
...
\end{equation}
It counts the number of photons in a mode. All of $a$, $\ad$, and $N$, although linear, are unbounded operators, hence are only defined on (dense) subset of $\cH$, namely
\begin{equation}
...
\begin{equation}\label{eq:domain-ccr-polynomial}
    \cD(p(a,\ad))=\cD(N^{d/2}).
\end{equation}
Next, we add a power of the number operator of leading order to the polynomial, i.e.
...
%\begin{lem}[Adjoint and core of polynomials of $a, \ad$]\label{lem:formal-polynomial-ccr-adjoint-core}
%    Let $p\in\C[X,Y]$ be a polynomial on $\C$. Then there exist $q \in \C[X,Y]$ satisfying $\deg(p) = \deg(q)$ such that $(p(a,\ad))^\dagger = q(a, \ad)$ on $\cD(N^{\frac{\deg(p)}{2}})$ which is the domain of $p(a,\ad)^\dagger$, i.e. $\cD(N^{\frac{\deg(p)}{2}}) = \cD(p(a, \ad)^\dagger)$. We further have that $p(a,\ad)$ is closed and satisfies the following bound
%    \begin{equation*}
%        \|p(a,\ad)\ket{\phi}\|\leq c\|(\1+N)^{\frac{\deg(p)}{2}}\ket{\phi}\|\,.
%    \end{equation*}
%    Lastly, $\cH_f$ is a core of every polynomial $p(a,\ad)$ and hence also for $p(a, \ad)^\dagger$.
%\end{lem}
...
The following theorem will become useful later and has an analogue in the theory of commutative $L_p$ spaces, which inspired its name. Although it can be proved by interpolation theory, we provide a more rudimentary approach using only Hirschmann's strengthening of Hadamard's three-line theorem.

% Another reason: I don't understand IntPol theory enough to use results from that theory :D
%%%%
% Frederic Chopin - Simplicity is the final achievement. After one has played a vast quantity of notes and more notes, it is simplicity that emerges as the crowning reward of art. [and math] :D
%%%%

\begin{thm}[Stein-Weiss theorem for Bosonic Sobolev spaces]\label{thm:stein-weiss}
    Let $0 \le k_0 < k_1$ and $T: W^{k_j, 1} \to W^{k_j, 1}$, be a linear map with $\norm{T}_{W^{k_j, 1} \to W^{k_j, 1}} \le M_j$ for some $M_j \ge 0$, $j = 1, 2$. Then for $\theta \in [0, 1]$, $T: W^{k_\theta, 1} \to W^{k_\theta, 1}$ with $k_\theta = (1-\theta) k_0 + \theta  k_1$ obtained by restriction of the input $T:W^{k_0, 1} \to W^{k_0, 1}$ to $W^{k_\theta, 1} \cap W^{k_0, 1}$ is a well defined bounded linear map with
    \begin{equation}
...
\begin{proof}
    ...
    \begin{equation}\label{eq:g(it + 1)}
        ...
        \end{aligned}
    \end{equation}
    An application of the theorem by Hirschmann (\cite[Lem.~D.1]{Sutter.2016}) now gives us that for $G_0 \coloneqq \sup\limits_{t \in \R} |g(it)|$ and $G_1 \coloneqq \sup\limits_{t \in \R} |g(it + 1)|$
    \begin{equation}
...
With the Sobolev embedding at hand, we introduce the notion of a \textit{Sobolev preserving semigroup} as a semigroup defined on any Bosonic Sobolev space.

\begin{defi}[Sobolev preserving evolution system]\label{defi:sobolev-preserving-semigroups}
    Let $(\cP_t)_{t\ge 0}$ be a $C_0$-semigroup on $\cT_{1,\operatorname{sa}}$. We then call $(\cP_t)_{t\ge 0}$ \textit{Sobolev preserving} if, for all $k \ge 0$, $W^{k, 1}$ is an admissible subspace for $(\cP_t)_{t\ge 0}$. Similarly, let $(\cP_{t,s})_{0\leq s\leq t}$ be a evolution system on $\cT_{1,\operatorname{sa}}$. We call $(\cP_{t,s})_{0\leq s\leq t}$ \textit{Sobolev preserving} if, for all $k \ge 0$, $W^{k, 1}$ is admissible for $(\cP_{t,s})_{0\leq s\leq t}$ to $W^{k, 1}$.
\end{defi}
...
Note that with the Stein-Weiss theorem for Bosonic Sobolev spaces, \Cref{thm:stein-weiss}, one can immediately interpolate a semigroup defined on $W^{k_0, 1}$ and $W^{k_1, 1}$ to $W^{k_\theta, 1}$ with $k_\theta = (1 - \theta)k_0  + \theta k_1$, $\theta \in [0, 1]$ (q.v.~\Cref{lem:interpolation-lemma}).
...
\begin{assum}\label{assum:finite-degree}
    The operator $(\cL,\cT_f)$ has $\operatorname{GKSL}$ form, i.e.~for $\rho\in\cT_f$ 
    \begin{equation}\label{eq:lindblad}
        \begin{aligned}
            \cL:\cT_f \to \cT_f \quad \rho \mapsto\cL(\rho) &= - i [H, \rho] + \sum\limits_{j = 1}^K L_j \rho L_j^\dagger  - \frac{1}{2}\{L_j^\dagger L_j, \rho\} \\
            &\coloneqq G\rho + \rho G^\dagger + \sum\limits_{j = 1}^K L_j \rho L_j^\dagger\, , 
        \end{aligned}
    \end{equation}
...
There exists a $k_0 \in \N$ s.t. for all $k \in \N$ with $k \ge k_0$, there exist $\omega_k \ge 0$ such that for all positive semi-definite $\rho \in \cT_f$
    \begin{equation}\label{eq:Assumptionsobolevstability}
        \tr[\cL(\rho) (N + \1)^{k/2}] \le \omega_k \tr[\rho (N + \1)^{k/2}] \, .
    \end{equation}
\end{assum}
...
% \begin{rmk}
%     Note that the assumptions stated above are both necessary and sufficient in the sense that if $(\cL,\cD(\cL))$ is not generating a Sobolev preserving semigroup, at least one of the assumptions is not satisfied.
% \end{rmk}
...
\begin{ex}[Pure birth process \texorpdfstring{\cite[Ex.~3.3.]{Davies.1977}}{[9,Ex.~3.3.]}]\label{eq:no-sobolev-stability}
    ...
    \begin{equation*}
        \cL(\rho)=G\rho+\rho G^\dagger+L\rho L^\dagger\,.
    \end{equation*}
...
\begin{proof}
    ...
    \end{align*}
    By the requirement of \Cref{assum:finite-degree}, it is clear that $\tr[\cL(\rho)] = 0$ for $\rho \in \cT_f$, using the cyclicity of the trace. For the explicit case of a pure state $\rho = \ketbra{\psi}{\psi} \in \cT_f$, we get
    \begin{equation*}
    ...
    \textit{Step 2)} That the implemented semigroup\footnote{A discussion on implemented semigroups can be found in \cites{Alber.2001}.} $(\cP_t)_{t \ge 0}$ is a strongly continuous, positivity-preserving contractive semigroup can be easily checked. We further get from \cite[Prop.~2.1]{Davies.1977} that it is generated by the closure of the operator
    \begin{equation*}
        \widetilde{\cG}_\varepsilon: \cD(\widetilde \cG_\varepsilon) = \{R(1,\overline{G}_\varepsilon) \rho R(1,\overline{G}_\varepsilon)^\dagger \; : \; \rho \in \cT_{1,\operatorname{sa}}\}  \to \cT_{1,\operatorname{sa}}, \quad \rho \mapsto \overline{G}_\varepsilon \rho + \rho \overline{G}_\varepsilon^\dagger
    \end{equation*}
    where $\overline{G}_\varepsilon$ is the closure of $G_\varepsilon$, $\overline{G}_\varepsilon^\dagger$ its adjoint, and $R( 1,\overline{G}_\varepsilon)$ its resolvent on $\cH$, respectively. Since $\cH_f$ is a core for the generator $\overline{G}_\varepsilon$, the set $O \coloneqq (\1 - \overline{G}_\varepsilon)\cH_f = (\1 - G_\varepsilon)\cH_f$ is dense in $\cH$, which in turn means $\cO = \text{span}\{\ketbra{\psi}{\varphi} \;:\; \ket{\psi}, \ket{\varphi} \in O\}$ is dense in $\cT_{1,\operatorname{sa}}$. A simple calculation further shows that $R(1,\overline{G}_\varepsilon)\cO R(1,\overline{G}_\varepsilon)^\dagger = \cT_f$. Hence for $\sigma \in \cD(\widetilde \cG_\varepsilon)$, we find $\rho \in \cT_{1,\operatorname{sa}}$ and a sequence $\{\rho_n\}_{n \in \N} \subseteq \cO$ with $\rho_n \to \rho$ for $n \to \infty$ such that
    \begin{align*}
        \widetilde \cG_\varepsilon(\sigma) &= \widetilde \cG_\varepsilon(R(1,\overline{G}_\varepsilon) \rho R(1,\overline{G}_\varepsilon)) = \widetilde \cG_\varepsilon(R(1,\overline{G}_\varepsilon) \lim\limits_{n \to \infty}\rho_n R(1,\overline{G}_\varepsilon)^\dagger)\\
        &= \lim\limits_{n \to \infty} \widetilde\cG_\varepsilon(R(1,\overline{G}_\varepsilon)\rho_n R(1,\overline{G}_\varepsilon)^\dagger)\\
        &= \lim\limits_{n \to \infty} \cG_\varepsilon(\sigma_n)
    \end{align*}
    where we used the continuity of the map $\widetilde \cG_\varepsilon(R(1,\overline{G}_\varepsilon) \cdot R(1,\overline{G}_\varepsilon)^\dagger)$ and set $\sigma_n = R(1,\overline{G}_\varepsilon) \rho_n R(1,\overline{G}_\varepsilon)^\dagger$. Note that $\{\sigma_n\}_{n \in \N}$ is by construction a convergent sequence on $\cT_f$. In the last equality, we used that $\widetilde \cG_\varepsilon$ and $\cG_\varepsilon$ agree on $\cT_f$. This shows not only that $\cG_\varepsilon$ is closable but further that its closure is the closure of $\widetilde \cG_\varepsilon$. Hence the closure of $\cG_\varepsilon$ is the generator of $(\cP_t^\varepsilon)_{t \ge 0}$.
\end{proof}
Using the above lemma, we are now able to prove the following.

\begin{lem}\label{lem:semigroup-of-G-for-sobolev-stability}
    For any $k \in \N$ and $\varepsilon > 0$, the closure of the operator $\cG_\varepsilon$ from \Cref{lem:semigroup-of-G-for-positivity} generates a strongly continuous, positivity preserving semigroup $(\cP^\varepsilon_t)_{t \ge 0}$ on $W^{k, 1}$ such that, for all $t\ge 0$, 
    \begin{equation*}
...
\medskip
    ...
    \noindent Proof of step 2. Let $\lambda > \omega_k$ be in the resolvent set of $\overline{\cG}_\varepsilon$. From the compact embedding of $W^{k, 1}$ in $\cT_{1,\operatorname{sa}}$, we immediately get that $R(\lambda,\overline{\cG}_\varepsilon):W^{k, 1} \to W^{k, 1}$ agrees with the respective restricted resolvent of the closure $\widehat{\cG}_\varepsilon$ of $\cG_\varepsilon$ on $\cT_{1,\operatorname{sa}}$ that we obtained in \Cref{lem:semigroup-of-G-for-positivity}. We know that the latter resolvent is positivity preserving, as the semigroup is. This is due to the following integral representation for strongly continuous semigroups \cite[Thm.~II.1.10 (i)]{Engel.2000}: for all $x\in \rho(\widehat{\cG}_\varepsilon)$,
    \begin{equation}
R(\lambda, \widehat{\cG}_\varepsilon)(x)=\int_0^\infty\,e^{-\lambda s}\,e^{s\widehat{\cG}_\varepsilon}(x)\,ds\,.
    \end{equation}
     Hence $R(\lambda,\overline{\cG}_\varepsilon)$ is positivity preserving as well. Using \Cref{assum:sobolev-stability}, we have that for $\rho \in \cT_f$, $\rho$ positive semi-definite,
    \begin{equation*}
        \tr[\cL(\rho) (N + \1)^{k/2}] \le \omega_k \tr[\rho (N + \1)^{k/2}] \, .
    \end{equation*}
    Adding non-negative terms, using the cyclicity of the trace and splitting up $\cL$ gives us
    \begin{align*}
        &\sum\limits_{j = 1}^K\tr[(N+\1)^{k/4}L_j \rho L_j^\dagger(N+\1)^{k/4}] + (\lambda - \omega_k)\tr[(N + \1)^{k/4} \rho (N + \1)^{k/4}] \\
        &\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad\qquad \le \tr[(N + \1)^{k/4}(\lambda - \cG_\varepsilon)(\rho)(N + \1)^{k/4}]\,,
    \end{align*}
    and therefore
    \begin{align*}
        (\lambda - \omega_k) \norm{\rho}_{W^{k, 1}} \le \norm{(\lambda - \cG_\varepsilon)(\rho)}_{W^{k, 1}} \, 
    \end{align*}
    where we have just dropped non-negative terms and used $\tr[\cdot] \le \norm{\cdot}_1$ with equality if the argument is positive semi-definite. Since $\overline{\cG}_\varepsilon$ is the closure of $\cG_\varepsilon$, the above inequality extends to $\rho \in \cD( \overline{\cG}_\varepsilon)$, $\rho$ positive semi-definite and $\overline{\cG}_\varepsilon$ instead of $\cG_\varepsilon$. Together with the positivity preserving property of the resolvent, this gives us that for all $\rho \in W^{k, 1}$, $\rho$ positive semi-definite
    \begin{equation}\label{eq:resolvent-bound-G-positive-semidefinite}
        \norm{R(\lambda,\overline{\cG}_\varepsilon)\rho}_{W^{k, 1}} \le \frac{1}{\lambda - \omega_k} \norm{\rho}_{W^{k, 1}} \, . 
    \end{equation}
    For a general $\rho \in W^{k, 1}$, we set $\rho_\pm = \frac{1}{(N + \1)^{k/4}}\,[(N + 1)^{k/4} \rho (N + \1)^{k/4}]_\pm \,\frac{1}{(N + \1)^{k/4}} \in W^{k, 1}$, where $[\cdot]_\pm$ denotes the positive, resp. the negative part of a self-adjoint trace-class operator. We clearly have that $\rho = \rho_+ - \rho_-$ and further that $\rho_+, \rho_-$ are positive semi-definite by construction. Hence
    \begin{align*}
        \norm{R(\lambda,\overline{\cG}_\varepsilon)\rho}_{W^{k, 1}} &\le \norm{R(\lambda,\overline{\cG}_\varepsilon )\rho_+}_{W^{k, 1}} + \norm{R(\lambda,\overline{\cG}_\varepsilon)\rho_-}_{W^{k, 1}}\\
        &\le \frac{1}{\lambda - \omega_k}(\norm{\rho_+}_{W^{k, 1}} + \norm{\rho_-}_{W^{k, 1}}) = \frac{1}{\lambda - \omega_k} \norm{\rho_+ - \rho_-}_{W^{k, 1}}\\
        &= \frac{1}{\lambda - \omega_k}\, \norm{\rho}_{W^{k, 1}}
    \end{align*}
    where we used \Cref{eq:resolvent-bound-G-positive-semidefinite} and the construction of $\rho_+$ and $\rho_-$. We, therefore, conclude step 2.
\medskip
...
\begin{lem}\label{lem:eliminating-perturbation-G}
    The closure of 
    \begin{equation*}
        \cG:\cT_f \to \cT_f, \quad \rho \mapsto \cG(\rho) = G\rho + \rho G^\dagger\,,
    \end{equation*}
    ...
    \end{equation*}
where $\omega_k = \frac{k}{k_0} \omega_{k_0}$ for $0 \le k \le k_0$.
\end{lem}
\begin{proof}
...
\begin{proof}[Proof of \Cref{thm:generation-theorem}]
    The proof strategy is inspired by \cite[Thm.~2.5]{Davies.1977}. It however makes use of \Cref{lem:approximation-lemma} to avoid the issues discussed in \cite[§3]{Davies.1977}. Let $k \in \mathbb{N}$ and either $k \ge k_0$, or $k = 0$ for the moment. Note that from \Cref{assum:finite-degree}, we can conclude $\omega_k = 0$ for $k = 0$ in \Cref{eq:Assumptionsobolevstability}. We first define for $r \in (0, 1)$ the map
    \begin{align*}
        \cL_r:\cT_f \to \cT_f, \quad \rho \mapsto \cL_r(\rho) &= G \rho + \rho G^\dagger + r\sum\limits_{j = 1}^K L_j \rho L_j^\dagger =: \cG(\rho) + r\Sigma(\rho) , 
    \end{align*}
    and show that its closure defines a strongly continuous, positivity preserving semigroup $(\cP_t^r)_{t \ge 0}$ on $W^{k, 1}$ which further satisfies
    \begin{equation*}
        \norm{\cP_t^r}_{W^{k, 1} \to W^{k, 1}} \le e^{\omega_k t} \, . 
    \end{equation*}
    ...
    \begin{equation*}
        \norm{\Sigma(\rho)}_{W^{k, 1}} \le \norm{(\widetilde \lambda + \omega_k - \cG)(\rho)}_{W^{k, 1}} 
    \end{equation*}
    for $\rho \in \cT_f$ and $\rho\ge 0$. Now using that $\cG$ is closable (\Cref{lem:eliminating-perturbation-G}) and its resolvent positivity preserving we can conclude  for $\lambda \coloneqq \widetilde \lambda + \omega_k > \omega_k$, $\rho \in (\lambda - \cG)\cT_f$ and $\rho \ge 0$, 
    \begin{equation*}
        \norm{\Sigma \circ R(\lambda,\overline{\cG}) (\rho)}_{W^{k, 1}} \le \norm{\rho}_{W^{k, 1}}\,.
    \end{equation*}
    Applying similar methods as in step 2. of the proof of \Cref{lem:semigroup-of-G-for-sobolev-stability}, we can extend the above inequality to general $\rho \in (\lambda - \cG) \cT_f$. Hence, $\Sigma \circ R(\lambda,\overline{\cG})$ is contractive on the dense set $(\lambda - \cG)\cT_f$ and positivity preserving, since both $\Sigma$ and $R(\lambda,\overline{\cG})$ are. It can therefore be uniquely extended to a positivity preserving contractive map on all of $W^{k, 1}$ which we will call $\cA_\lambda$ in the following. As a consequence $(\cL_r, \cD(\cL_r))$ is closable and $\lambda > \omega_k$ in the resolvent set of the closure. Both facts follow from the representation
    \begin{equation*}
        (\lambda - \cL_r) = (\1 - r\Sigma \circ R(\lambda,\overline{\cG})) \circ (\lambda - \cG)
    \end{equation*}
    which decomposes $\lambda - \cL_r$ into a composition of a closable map with a dense range and a map that is bounded on that range. We further get for the resolvent of the closure
    \begin{equation*}
        R(\lambda,\overline{\cL}_r) = R(\lambda,\overline{\cG}) \sum\limits_{n = 0}^\infty r^n \cA_\lambda^n \,,  
    \end{equation*}
    ...
    \begin{equation}\label{eq:bound-resolvent-L_r}
        \norm{R(\lambda,\overline{\cL}_r)}_{W^{k, 1} \to W^{k, 1}} \le \frac{1}{\lambda - \omega_k} \, . 
    \end{equation}
    To obtain this inequality we again rearrange \Cref{assum:sobolev-stability}, add non-negative terms, use cyclicity of the trace and that $\tr[\cdot] \le \norm{\cdot}_1$ with equality if the argument is positive semi-definite, to conclude that for $\rho \in (\lambda - \cL_r)\cT_f$, $\rho$ positive semi-definite,
    \begin{equation*}
        \norm{R(\lambda,\overline{\cL}_r) \rho}_{W^{k, 1}} \le \frac{1}{\lambda - \omega_k}\norm{\rho}_{W^{k, 1}}\,.
    \end{equation*}
    We again extend the above bound to all $\rho \in (\lambda - \cL_r)\cT_f$ analogously to step 2 in the proof of \Cref{lem:eliminating-perturbation-G}. Using that $(\lambda - \cL_r)\cT_f$ is dense then gives \Cref{eq:bound-resolvent-L_r}. Employing \Cref{thm:lumer-phillips}, we get that indeed for all $r \in (0, 1)$ the closure of $(\cL_r, \cD(\cL_r))$ generates a strongly continuous semigroup which is positivity preserving since the resolvent is and satisfies the claimed bound. To now fill the gap between $0$ and $k_0$, we interpolate between the semigroups at $0$ and $k_0$ (q.v.~\Cref{lem:interpolation-lemma}), obtaining $e^{t\omega_k k/k_0}$ as the bound of the interpolated semigroups. Now that we have the result for all $k \in \N$ we can employ \Cref{lem:approximation-lemma} and take the limit $r \to 0$ to obtain the assertion. The contractivity and trace-preserving property of the semigroup in the case $k = 0$  just follows from the GKSL form of $(\cL, \cD(\cL))$, i.e.~$\tr[\cL(\rho)] = 0$ for $\rho \in \cT_f$, or put differently \Cref{assum:finite-degree}.
\end{proof}
...
\begin{assum}\label{assum:finite-degree-time-dep}
    The operator $(\cL_s,\cT_f)$ has $\operatorname{GKSL}$ form, i.e.~for $\rho\in\cT_f$ and $s\in[0,\infty)$ 
    \begin{equation}\label{eq:lindblad-time-dep}
        \begin{aligned}
            \cL_s:\cT_f \to \cT_f \quad \rho \mapsto\cL_s(\rho) &= - i [H(s), \rho] + \sum\limits_{j = 1}^K L_j(s) \rho L_j^\dagger(s)  - \frac{1}{2}\{L_j^\dagger(s) L_j(s), \rho\} \\
            &\coloneqq G(s)\rho + \rho G^\dagger(s) + \sum\limits_{j = 1}^K L_j(s) \rho L_j^\dagger(s)\, , 
        \end{aligned}
    \end{equation}
...
There exists $k_0 \in N$, s.t. for all $k \in \N$ with $k \ge k_0$ there exists $\omega_k \ge 0$ such that for all $s \in [0, \infty)$ and $\rho \in \cT_f$ positive semi-definite,
    \begin{equation}
        \tr[\cL_s(\rho) (N + \1)^{k/2}] \le \omega_k \tr[\rho (N + \1)^{k/2}] \, .
    \end{equation}
    Note that the coefficients $\omega_k$ are independent of $s$.
\end{assum}
...
\begin{thm}[Generation of bosonic evolution systems]\label{thm:timedep-generation-theorem}
    Let $(\cL_s, \cD(\cL_s))_{s\in[ 0,\infty)}$ be a family of operators that fulfill \Cref{assum:finite-degree-time-dep} and \Cref{assum:sobolev-stability-time-dep}. Then $(\overline{\cL}_s, \cD(\overline{\cL}_s))_{s \in [0, \infty)}$ gives rise to a unique evolution system $(\cP_{t,s})_{0\le s\le t}$ on $W^{k, 1}$ for all $k\ge 0$ with the following properties
    \begin{enumerate}
        \item $\cP_{t,s} (W^{k + 4d, 1}) \subseteq W^{k + 4d, 1}$ for all $0\le s\le t$
        \item For any $\rho \in W^{k + 4d, 1}$, the family $( \cP_{t,s}(\rho))_{0\le s\le t}$ is the unique solution to the initial value problem
        \begin{equation}\label{eq:time-dep-init-value-problem}
            \frac{d}{dt} \rho(t) = \overline{\cL}_t(\rho(t)) \qquad t \in [s, \infty), \; \rho(s) = \rho\,.
        \end{equation}
    \end{enumerate}
...
\begin{proof}
    ...
    %Due to the $C_0$ condition on the coefficients comprising $\cL(s)$ and the requirement that all degrees are below or equal to $d$ we can conclude that 
    \begin{equation*}
    ...
    %for every $s$ is a bounded map from $\cT_{1, sa.}(\cH)$ to $\cT_{1, sa.}(\cH)$. This is because it can be decomposed as $\sum\limits_{\text{finite}} A_{i}(s) \cdot B_{i}(s)$ where $A_i$ and $B_i$ are polynomials of $a$ and $a^\dagger$ with degree at most $d$ sandwiched in between $(N + \1)^{k/4}$ and $(N + \1)^{-k/4 + d}$. Now \Cref{lem:boundedness-polynomials} gives us that those $A_i$ and $B_i$ are bounded on $\cH$. We further know that the $s$ dependence is in the coefficients of the polynomials which are by assumption $C_0([0, 1], \C)$.
    %We hence can conclude that $s \mapsto B_i(s)$ or $s \mapsto A_i(s)$ are continuous in $\cB(\cH)$, which in turn means $s \mapsto \cA(s)$ is continuous in $\cB(\cT_{1, sa.}(\cH), \cT_{1, sa.}(\cH))$ or equivalently $s \mapsto \cL_s$ is continuous in $\cB(W^{k + 4d, 1}, W^{k, 1})$.
...
In this section, we discuss the extension of \Cref{sec:polynomial-generators} to the multi-mode setting. Since the details are almost completely analogous to the single-mode situation, we choose to elaborate further only at places where there might remain some ambiguities. Let us first fix the notations for this setting. We consider the Hilbert space of an $m$-mode system, $m\in\N$, whose Hilbert space we conveniently denote by $\cH_m=L^2(\mathbb{R}^m)$. We further use $\cB(\cH_m)$ for the bounded, $\cT_1$ for the trace class, and $\cT_{1, \operatorname{sa}}$ for the self-adjoint trace class operators. Now we define $\cT_f$ to be
\begin{equation*}
    \cT_f \coloneqq \{\rho = \sum\limits_{\text{finite}} f_{\n, \p}  \ketbra{n_{1}}{p_{1}} \otimes \hdots \otimes \ketbra{n_m}{p_m} \;:\; f_{\n, \p} \in \C, \; \rho = \rho^\dagger\} \, ,
\end{equation*}
where $\n = (n_1, \hdots, n_m) \in \N^m$ and $\p$ analogously function as an index in $f_{\n, \p}$. For $\k = (k_1, \hdots, k_m) \in \N^m$, we define $\k \prec \k'$ for $\k, \k' \in \N^m$ if $k_j < k_j'$ for all $j = 1, \hdots, m$. Analogously, we define $\preceq$. We set for $\k \in \N^m$
\begin{equation*}
...
\begin{thm}[Stein-Weiss theorem for multimode Bosonic Sobolev spaces]
    Let $\k_0, \k_1 \in \N^m$ $\k_0 \prec \k_1$ and $T: W^{\k_j, 1} \to W^{\k_j, 1}$ a linear map with $\norm{T}_{W^{\k_j, 1} \to W^{\k_j, 1}} \le M_j$, bounded by $M_j \ge 0$ for $j = 0, 1$ respectively. Then for $\theta \in [0, 1]$, $T: W^{\k_\theta, 1} \to W^{\k_\theta, 1}$ with $\k_\theta = (1 - \theta) \k_0 + \theta \k_1$ obtained by restriction of the input of  $T:W^{\k_0, 1} \to W^{\k_0, 1}$ to $W^{\k_\theta, 1} \cap W^{\k_0, 1}$, is a well defined bounded linear map with
    \begin{equation}
        \norm{T}_{W^{\k_\theta, 1} \to W^{\k_\theta, 1}} \le M_0^{(1 - \theta)} M_1^\theta \, . 
    \end{equation}
\end{thm}

In the multimode setting, the property of being Sobolev preserving is not defined for all $0 \prec \k$ but only for a ray $\{l\k : l\in [0, \infty)\}$ where $0 \prec \k$. Note that the Bosonic Sobolev spaces for higher $l$ are compactly embedded into the ones with lower $l$ due to \Cref{lem:multimode-sobolev-embedding}.

\begin{defi}[Sobolev preserving semigroup/evolution system in multimode Sobolev spaces]
    Let $(\cP_t)_{t\ge 0}$ be a $C_0$-semigroup on $\cT_{1,\operatorname{sa}}$. We then call $(\cP_t)_{t \ge 0}$ \textit{Sobolev preserving} if there exists $\k \in \N^m$ with $0 \prec \k$, s.t. for all $l \ge 0$, $W^{l \k, 1}$ is an admissible subspace for $(\cP_t)_{t\ge 0}$. Similarly, let $(\cP_{t,s})_{0\leq s\leq t}$ be an evolution system on $\cT_{1,\operatorname{sa}}$. We call $(\cP_{t,s})_{0\leq s\leq t}$ \textit{Sobolev preserving} if there exists $\k \in \N^m$, s.t. for all $l \ge 0$, $W^{l \k, 1}$ is admissible for $(\cP_{t,s})_{0\leq s\leq t}$ to $W^{l \k, 1}$.
\end{defi}
...
\Cref{assum:sobolev-stability} is generalized using the concept of rays, already mentioned in the definition of the Sobolev preserving semigroups and evolution systems. 

\begin{assum}\label{assum:multimode-sobolev-preserving}
     There exists $\k \in \N^m$, s.t. $0 \prec \k$ and there exists $l_0 \in \N$, s.t. for all $l \in \N$, there exist $\omega_l \ge 0$ such that for all positive semi-definite $\rho \in \cT_f$
    \begin{equation}\label{eq:Multimodeassumptionsobolevstability}
        \tr[\cL(\rho) (N + \1)^{l\k/2}] \le \omega_l \tr[\rho (N + \1)^{l \k/2}] \, .
    \end{equation}
\end{assum}
...
\begin{thm}[Generation of multimode bosonic semigroups]\label{thm:multimode-generation-theorem}
    Let $(\cL, \cD(\cL))$ be an operator defined on $\cT_{1,\operatorname{sa}}$. If $(\cL, \cD(\cL))$ satisfies \Cref{assum:multimode-finite-degree} and \Cref{assum:multimode-sobolev-preserving}, then the closure $\overline{\cL}$ generates a strongly continuous, positivity preserving semigroup $(\cP_t)_{t\ge 0}$ on $W^{l \k, 1}$ for all $l \ge 0$ with $\k \in \N^m$ from \Cref{assum:multimode-sobolev-preserving}. We further find that the semigroup satisfies the bound
    \begin{equation}
        \norm{\cP_t}_{W^{l \k, 1} \to W^{l \k, 1}} \le e^{\omega_l t} \,\quad \forall t\ge 0\, ,
    \end{equation}
    where $\omega_l = \frac{l}{l_0} \omega_{l_0}$ for $l_0 \ge l \ge 0$. Note that for $l = 0$, the semigroup is contractive and trace-preserving.
\end{thm}
...
\begin{assum}\label{assum:multimode-sobolev-stability-time-dep}
  There exists $\k \in \N^m$ with $0 \prec \k$ and $l_0  \in \N$, s.t. for all $l \in \N$ with $l \ge l_0$, there exists $\omega_l \ge 0$ such that for all $s \in [0, \infty)$ and $\rho \in \cT_f$ positive semi-definite,
    \begin{equation}
        \tr[\cL_s(\rho) (N + \1)^{l \k/2}] \le \omega_l \tr[\rho (N + \1)^{l \k/2}] \, .
    \end{equation}
    Note that the coefficients $\omega_l$ are independent of $s$.
\end{assum}
...
\begin{thm}[Generation of multimode bosonic evolution systems]\label{thm:multimode-timedep-generation-theorem}
    ...
    $(\overline{\cL}_s, \cD(\overline{\cL}_s))_{s \in [0, \infty)}$ gives rise to a unique evolution system $(\cP_{t,s})_{0\le s\le t}$ on $W^{l \k, 1}$ for all $l \in N$ with the following properties
    \begin{enumerate}
        \item $\cP_{t,s} (W^{(l + 4d) \k, 1}) \subseteq W^{(l + 4d)\k, 1}$ for all $0\le s\le t$;
        \item For any $\rho \in W^{(l + 4d)\k, 1}$, the family $(\cP_{t,s}(\rho))_{0\le s\le t}$ is the unique solution to the initial value problem
        \begin{equation}\label{eq:multimode-time-dep-init-value-problem}
            \frac{d}{dt} \rho(t) = \overline{\cL}_t(\rho(t)) \qquad t \in [s, \infty), \; \rho(s) = \rho\,.
        \end{equation}
    \end{enumerate}
    For $l = 0$, the evolution system is contractive and trace-preserving.
\end{thm}


\section{Examples of Sobolev preserving semigroups}\label{sec:examples-sobolev-preserving-semigroup}

In this section, we consider two classes of examples of practical relevance in quantum information processing for which \Cref{assum:finite-degree} (or \ref{assum:finite-degree-time-dep},\ref{assum:multimode-finite-degree},\ref{assum:multimode-finite-degree-time-dep}) trivially holds and derive \Cref{assum:sobolev-stability} (or \ref{assum:sobolev-stability-time-dep}, \ref{assum:multimode-sobolev-preserving}, \ref{assum:multimode-sobolev-stability-time-dep}). Particular care will be given to finding time-independent upper bounds on the $W^{k,1}\to W^{k,1}$ norm of the semigroup. For this, the overall strategy is as follows: given the generator $(\cL,\cT_f)$, we prove that there are coefficients $\mu_k\ge 0, c_k>0$ for all $k\in\N$ such that for any state $\rho\in\cT_f$
\begin{align}
    \tr[\cL(\rho) (\Nind+\1)^{k/2}] &\leq - c_k\tr[\rho (\Nind+\1)^{k/2}] + \mu_k \label{eq:examples-assum2-step1}\\
    & \leq (\mu_k- c_k)\tr[\rho (\Nind+\1)^{k/2}]\,,\nonumber
\end{align}
where we have used $\tr[\rho (\Nind+\1)^{k/2}]\geq \tr[\rho] = 1$ in the second inequality.
Then, \Cref{thm:generation-theorem} can be applied, which shows that for all $k\in\N$, the closure of $(\cL,\cT_f)$ generates a positivity preserving $C_0$-semigroup $(\cP_t)_{t\ge 0}$ on $W^{k,1}$ which satisfies for all $x\in W^{k,1}$:
\begin{equation*}
    \|\cP_t(x)\|_{W^{k,1}}\leq e^{|\mu_k- c_k|\,t}\|x\|_{W^{k,1}}\,.
\end{equation*}
This bound can then be improved by better exploiting \Cref{eq:examples-assum2-step1}:
\begin{prop}\label{prop-ex:uniformly-bounded-semigroup}
    Let $(\cL,\cT_f)$ be an operator satisfying \Cref{assum:finite-degree} and \Cref{eq:examples-assum2-step1}. Then, for all $k\in\N$, the closure of $(\cL,\cT_f)$ generates a positivity preserving $C_0$-semigroup $(\cP_t)_{t\ge 0}$ on $W^{k,1}$ which satisfies for all states $\rho\in W^{k,1}$  (here state is still $\rho \ge 0$, $\tr[\rho] = 1$),
    \begin{equation*}
        \|\cP_t(\rho)\|_{W^{k,1}}\leq \max\left\{\|\rho\|_{W^{k,1}},\,\frac{\mu_{k}}{c_{k}}\right\} \, .
    \end{equation*}
    Further, there is a sequence $(t_n)_{n\in\N}$ such that
    \begin{equation*}
        \lim_{t_n\rightarrow\infty}\cP_{t_n}(\rho)=\overline{\rho}
    \end{equation*}
    for $\overline{\rho}\in W^{k,1}$\,.
    Similar conclusions hold in multi-mode as well as time-dependent settings.
\end{prop}
\begin{proof}
    By assumption, \Cref{thm:generation-theorem} shows that the closure of $(\cL,\cT_f)$ defines a positivity preserving, quasi-contractive semigroup $(\cP_t)_{t\ge 0}$. Moreover, for $k \ge k_0$, $\rho(t)\coloneqq \cP_t(\rho)$
    \begin{equation*}
        \begin{aligned}
        ...
        \end{aligned}
    \end{equation*}
    Thus, for $\|\rho(t)\|_{W^{k,1}}\geq\frac{\mu_k}{c_k}$, we have $\frac{d}{dt}\|\rho(t)\|_{W^{k,1}}\leq0$, which concludes the bound. Finally, for every $k\geq1$, every sequence $n\rightarrow \cP_{t_n}(\rho)$ is uniformly bounded in $W^{k,1}$ so that the compact embedding shows that there exists a converging subsequence in $W^{k-1,1}$, which is also converging in $W^{k,1}$. This finishes the proof.
\end{proof}
...
\begin{lem}[$l$-photon dissipation]\label{lem:l-diss}
    ...
    \end{align*}
    where $\mu_k^{(l)}=\Delta_l^\nu\left(\frac{(\nu-1)^{\nu-1}}{\nu^\nu}\right)$ with $\nu=l+\frac{k}{2}-1$ and $\Delta_l=(l+1)l+2|\alpha|^lkl^{k/2}\sqrt{l!}$. Therefore, $\cL_l\coloneqq \cL[a^l-\alpha^l]$ generates a Sobolev and positivity preserving quantum Markov semigroup satisfying for all states $\rho\in W^{k,1}$
    \begin{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        \begin{aligned}
            \frac{1}{2}\tr\big[\rho(\overline{\alpha}^la^lg_l(N)+{\alpha}^lg_l(N)(\ad)^l)\big]&{\leq}\,(l+1)\,{|\alpha|^l}\tr[\rho\, g_l(N+l\1)\sqrt{N[1:l]}] \\
            &\overset{(1)}{\leq}|\alpha|^l \frac{kl^{k/2}(l+1)}{2}\,\tr[\rho\, (N+\1)^{k/2-1}\sqrt{N[1:l]}]\\
            &\leq|\alpha|^lkl^{k/2+1}\sqrt{l!}\,\tr[\rho\, (N+\1 )^{k/2-1+\frac{l}{2}}]\,.
        \end{aligned}
    \end{equation*}
    ...
    \begin{equation}\label{eq-ex:l-dissipation-upper-bound}
        ...
        \end{aligned}
    \end{equation}
    where we used again that $l\ge 2$ in the last inequality. A half of the leading order term can be used to control the second term by a constant. For that, we use the spectral decomposition of the operator $N$ so that the above problem can be reduced to the following simple optimization: 
    ...
    %and choosing $K$ such that $(K+1)\frac{l}{2}= \Delta_l$
    %\begin{align*}
    %    &\tr\big[\rho\cL[a^l-\alpha^l]^\dagger(f(N))\big]\\
    %    & \le \sum_{\n=(n,\dots, n_m)}\,f(n_2)\dots f(n_m)\, \braket{\n,\rho\,\n}\, \left(-l(n+1)^{l+k-1}+\Delta_l (n+1)^{k+l-2}\right)\\
    %    &\le -\frac{l}{2}\sum_\n\,f(n_2)\dots f(n_m)\, \braket{\n, \rho \,\n}\, (n+1)^{l+k-1}+\sum_{\n|n< K}f(n_2)\dots f(n_m)\, \braket{\n, \rho\,\n}\,\Delta_l(n+1)^{l+k-2}\\
    %    &\le -\frac{l}{2} \tr\big[\rho\,(N+\1)^{l+k-1}\big]+\Delta_l\,\left(\frac{2\Delta_l}{l}\right)^{l+k-1}\tr\big[\rho\,  \big]\,.
    %\end{align*}
\end{proof}
...
\begin{cor}[$2$-photon dissipation]\label{lem:cat-identity}
    ...
    \end{align*}
    where $\mu^{(2)}_k=(\Delta^{(2)}_k)^\nu\left(\frac{(\nu-1)^{\nu-1}}{\nu^\nu}\right)$ with $\nu=\frac{k}{2}+1$ and $\Delta_2=6+2\sqrt{2!}|\alpha|^2k2^{k/2}$. Therefore, $\cL_2\coloneqq \cL[a^2-\alpha^2 ]$ generates a Sobolev and positivity preserving quantum Markov semigroup which satisfies for all states $\rho$ in $W^{k,1}$
    \begin{equation*}
...
\begin{lem}\label{lem:l-diss-hamiltonian}
    ...
    for $\mu_k,\nu\geq1$ defined by 
    \begin{equation*}
        \mu_k=c^\nu\left(\frac{(\nu-1)^{\nu-1}}{\nu^\nu}\right)\quad\text{with}\quad c={(l+1)l}+2|\alpha|^lkl^{k/2}\sqrt{l!}+\Lambda(2l)^{k/2+1}\sqrt{(2l)!}\,,\quad\nu=l+\frac{k}{2}-1\,.
    \end{equation*}
...
\begin{proof}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    where $f(x)=(x+1)^{k/2} 1_{x\ge -1}$ and $\Delta_l={(l+1)l}+2|\alpha|^lkl^{k/2}\sqrt{l!}$\,. To upper bound
    \begin{equation*}
        \begin{aligned}
    ...
    \begin{equation*}
        ...
        \end{aligned}
    \end{equation*}
    where we used \Cref{lem:upper-lower-bound-gl} in $(3)$. As the above function is montone in $d_H$ we can w.l.o.g assume $d_H=2(l-1)$ and conclude
    \begin{equation*}
        \begin{aligned}
...
% \begin{proof}
%     Following the proof of  \Cref{lem:upper-bound-diff-energy-unperturbed}, in particular \Cref{eq-ex:l-dissipation-upper-bound}, we have 
%     \begin{equation}\label{eq-ex:cat-identity-upper-bound}
%         \begin{aligned}
%             \tr[\rho\cL[a^2-\alpha^2]^\dagger(f(N))]&\leq-k\tr[\rho(N+\1)^{k/2+1}]+2k\left(\left(\frac{k}{4}+3\right)+\frac{|\alpha|^2k}{\sqrt{2}}\right)\tr[\rho(N+\1)^{k/2}]\\
%             &\leq-k\tr[\rho(N+\1)^{k/2+1}]+2k^2\left(4+|\alpha|^2\right)\tr[\rho(N+\1)^{k/2}].
%         \end{aligned}
%     \end{equation}
%     So that \Cref{eq-ex:optimization} shows 
%     \begin{equation*}
%         \tr[\rho\cL[a^2-\alpha^2]^\dagger(f(N))]\leq-\frac{k}{2}\tr[\rho(N+\1)^{k/2+1}]+\frac{k^2}{8}\left(4+|\alpha|^2\right)^{k/2}.
%     \end{equation*}
% \end{proof}
...
% \begin{proof}
%      By \Cref{lem:cat-identity},
%      \begin{equation*}
%         \begin{aligned}
%             \tr[\rho\cL[a^2-\alpha^2]^\dagger(f(N))]\leq-\tr[\rho(N+\1)^{k+1}]+\Delta_2(\alpha)
%         \end{aligned}
%     \end{equation*}
%     where $f(x)=(x+1)^{k} 1_{x\ge -1}$. Then, \Cref{lem:two-point-hamiltonian-bound} and \ref{lem:upper-lower-bound-gl} show
%     \begin{equation*}
%         \begin{aligned}
%             \tr[\cH[a+\ad](\rho)f(N)]&=i\tr[\rho\left(f(N)(a+\ad)-(a+\ad)f(N)\right)]\\
%             &=\tr[\rho\left(-iag_1(N)+ig_1(N)\ad \right)]\\
%             &\overset{\ref{lem:two-point-hamiltonian-bound}}{\leq}\tr[\rho g_1(N)\sqrt{N+\1}]\\
%             &\overset{\ref{lem:upper-lower-bound-gl}}{\leq}k\tr[\rho(N+\1)^{\frac{k-1}{2}}]
%         \end{aligned}
%     \end{equation*}
%     so that
%     \begin{equation*}
%         \begin{aligned}
%             \tr[(\varepsilon\cH[a+\ad]+\cL[a^2+\alpha^2])(\rho)f(N)]\leq-k\tr[\rho(N+\1)^{k/2+1}]+2k^2\left(4+\varepsilon+|\alpha|^2\right)\tr[\rho(N+\1)^{k/2}]\,.
%         \end{aligned}
%     \end{equation*}
%     Finally, \Cref{eq-ex:optimization} shows 
%     \begin{equation*}
%         \tr[(\varepsilon\cH[a+\ad]+\cL[a^2+\alpha^2])(\rho)f(N)]\leq-\frac{k}{2}\tr[\rho(N+\1)^{k/2+1}]+\frac{k^2}{8}\left(4+\varepsilon+|\alpha|^2\right)^{k/2}
%     \end{equation*}
...
\begin{prop}[CNOT-gate]\label{lem:assum2-CNOT}
    ...
    \begin{equation*}
        |\alpha|k_12^{k_1/2-1/2}\leq\frac{1}{32}k_2\,,
    \end{equation*}
...
We denote $f(x_1,x_2)=f_1(x_1)f(x_2)$ with $f_{1}(x_{1})=(x_{1}+1)^{k_{1}/2} 1_{x_{1}\ge -1}$, $f_{2}(x_{2})=(x_{2}+1)^{k_{2}/2} 1_{x_{2}\ge -1}$, and rewrite the CNOT-generator as
  ...
  We first focus on the second Lindbladian $\cL[b^2+za+w]$: we first consider, for $n\coloneqq (n_1,n_2)\in\N^2$, 
    \begin{equation*}
        \begin{aligned}
            &\cL[b^2+za+w]^\dagger(\ket{n}\bra{n})\\
            &\quad =((\bd)^2+\overline{z}\ad+\overline{w})\ket{n}\bra{n}(b^2+za+w)-\frac{1}{2}\left\{((\bd)^2+\overline{z}\ad+\overline{w})(b^2+za+w),\ket{n}\bra{n}\right\}\\
&\quad =F_1(n)\,|n\rangle\langle n|+F_2(n)|n_1,n_2+2\rangle\langle n_1,n_2+2|+F_3(n)|n_1+1,n_2\rangle\langle n_1+1,n_2|\\
&\quad \qquad + \big(F_4(n)\,|n_1,n_2+2\rangle\langle n_1+1,n_2|+h.c.\big)+\big(F_5(n) \,|n_1+1,n_2-2\rangle\langle n|  +h.c.\big) \\
&\quad\qquad +\big(F_6(n)\,|n_1,n_2-2\rangle\langle n|+h.c.\big)
+\big( F_7(n)\,|n_1-1,n_2\rangle\langle n|+h.c.\big)\\
&\quad\qquad +\big(F_8(n)\,|n_1,n_2+2\rangle\langle n|+h.c. \big)+\big(F_9(n)\,|n_1+1,n_2\rangle\langle n|+h.c. \big)  \\
&\quad\qquad +\big(F_{10}(n)|n_1-1,n_2+2\rangle\langle n|+h.c.\big)\,,
            % &=\Bigl(\sqrt{(n_2+1)(n_2+2)}\ket{n_1,n_2+2}+\overline{z}\sqrt{n_1+1}\ket{n_1+1,n_2}+\overline{w}\ket{n}\Bigr)\bra{n}(b^2+za+w)\\
            % &\qquad-\Bigl({n_2(n_2-1)}+|z|^2 {n_1}+|w|^2\Bigr)\ket{n}\bra{n}\\
            % &\qquad-\frac{1}{2}\biggl(\sqrt{n_2(n_2-1)}\Bigl(\overline{z}\sqrt{n_1+1}\ket{n_1+1,n_2-2}+\overline{w}\ket{n_1,n_2-2}\Bigr)\bra{n}\\
            % &\qquad\qquad\quad+\sqrt{n_1}z\Bigl(\sqrt{(n_2+1)(n_2+2)}\ket{n_1-1,n_2+2}+\overline{w}\ket{n_1-1,n_2}\Bigr)\bra{n}\\
            % &\qquad\qquad\quad+w\Bigl(\sqrt{(n_2+1)(n_2+2)}\ket{n_1,n_2+2}+\overline{z}\sqrt{n_1+1}\ket{n_1+1,n_2}\Bigr)\bra{n}+h.c.\biggr)
        \end{aligned}
    \end{equation*}
...
\noindent\textit{Case 0:} Diagonal terms, involving $F_1$, $F_2$ and $F_3$,
    \begin{equation*}
        \begin{aligned}
            C_0(n)\coloneqq F_1(n)|n\rangle\langle n|+F_2(n)|n_1,n_2+2\rangle\langle n_1,n_2+2|+F_3(n)|n_1+1,n_2\rangle\langle n_1+1,n_2|\,.
        \end{aligned}
    \end{equation*}
    
    
   \noindent \textit{Case 1:} Terms of the form $\ket{k_1+1,k_2-2}\bra{k_1,k_2}$, involving $\overline{F}_4$, $F_5$ and $\overline{F}_{10}$,
    \begin{equation*}
        \begin{aligned}
            C_1(n)\coloneqq \overline{F}_4(n)|n_1+1,n_2\rangle\langle n_1,n_2+2|+F_5(n)|n_1+1,n_2-2\rangle\langle n|+\overline{F}_{10}(n)|n\rangle\langle n_1-1,n_2+2| \,.
        \end{aligned}
    \end{equation*}

    
\noindent    \textit{Case 1':} Terms of the form $\ket{k_1,k_2}\bra{k_1+1,k_2-2}$, involving $F4$, $\overline{F}_5$ and $F_{10}$,
    \begin{equation*}
        \begin{aligned}
         C_{1'}(n)&\coloneqq {F}_4(n)|n_1,n_2+2\rangle\langle n_1+1,n_2|+\overline{F}_5(n)|n\rangle\langle n_1+1,n_2-2|+{F}_{10}(n)|n_1-1,n_2+2\rangle\langle n| \,.
        \end{aligned}
    \end{equation*}

    \noindent\textit{Case 2:} Terms of the form $\ket{k_1,k_2-2}\bra{k_1,k_2}$, involving $F_6$ and $\overline{F}_8$,
        \begin{equation*}
        \begin{aligned}
         C_{2}(n)\coloneqq {F}_6(n)|n_1,n_2-2\rangle\langle n|+\overline{F}_8(n)|n\rangle\langle n_1,n_2+2| \,.\\
        \end{aligned}
    \end{equation*}
    
    
   \noindent \textit{Case 2':} Terms of the form $\ket{k_1,k_2}\bra{k_1,k_2-2}$, involving $\overline{F}_6$ and ${F}_8$,
     \begin{equation*}
        \begin{aligned}
         C_{2'}(n)\coloneqq \overline{F}_6(n)|n\rangle\langle n_1,n_2-2|+{F}_8(n)|n_1,n_2+2\rangle\langle n| \,.\\
        \end{aligned}
    \end{equation*}

   \noindent  \textit{Case 3:} Terms of the form $\ket{k_1-1,k_2}\bra{k_1,k_2}$, involving $F_7$ and $\overline{F}_9$,
      \begin{equation*}
        \begin{aligned}
         C_{3}(n)\coloneqq {F}_7(n)|n_1-1,n_2\rangle\langle n|+\overline{F}_9(n)|n\rangle\langle n_1+1,n_2| \,.\\
        \end{aligned}
    \end{equation*}



   \noindent  \textit{Case 3':} Terms of the form $\ket{k_1,k_2}\bra{k_1-1,k_2}$, involving $\overline{F}_7$ and ${F}_9$,
      \begin{equation*}
        \begin{aligned}
         C_{3'}(n)\coloneqq \overline{F}_7(n)|n\rangle\langle n_1-1,n_2|+{F}_9(n)|n_1+1,n_2\rangle\langle n| \,.\\
        \end{aligned}
    \end{equation*}
To summarize, we have decomposed $\cL[b^2+za+w]^\dagger(|n\rangle\langle n|)$ into the sum 
...
\begin{align*}
&\hat{C}_0\coloneqq \sum_{n}\,C_0(n)=-\sum_n\,\Big(f_1(n_1)g_{2,2}(n_2)n_2(n_2-1)+f_2(n_2)g_{1,1}(n_1)|z|^2n_1\Big) |n\rangle\langle n|\\
&\hat{C}_1\coloneqq \sum_{n}\,C_1(n)\\
&\qquad =\sum_{n}\,-\frac{\overline{z}}{2}\sqrt{(n_1+1)(n_2-1)n_2}\Bigl(f_1(n_1)g_{2,2}(n_2)+g_{1,1}(n_1+1)f_2(n_2-2)\Bigr)\ket{n_1+1,n_2-2}\bra{n}\\
&\hat{C}_{1'}\coloneqq \hat{C}_1^\dagger \\
&\qquad =\sum_n\,-\frac{{z}}{2}\sqrt{(n_1+1)(n_2-1)n_2}\Bigl(f_1(n_1)g_{2,2}(n_2)+g_{1,1}(n_1+1)f_2(n_2-2)\Bigr)\ket{n}\bra{n_1+1,n_2-2}\\
&\hat{C}_2\coloneqq \sum_n\,C_2(n) = \sum_n-\frac{\overline{w}}{2}\sqrt{(n_2-1)n_2}f_1(n_1)g_{2,2}(n_2)\ket{n_1,n_2-2}\bra{n}\\
&\hat{C}_{2'}\coloneqq \hat{C}_2^\dagger= \sum_n-\frac{{w}}{2}\sqrt{(n_2-1)n_2}f_1(n_1)g_{2,2}(n_2)\ket{n}\bra{n_1,n_2-2}\\
&\hat{C}_3\coloneqq \sum_n\,C_3(n) = \sum_n-\frac{\overline{w}z}{2}\sqrt{n_1}g_{1,1}(n_1)f_2(n_2)\ket{n_1-1,n_2}\bra{n}\\
&\hat{C}_{3'}\coloneqq \hat{C}_3^\dagger =\sum_n-\frac{{w}\overline{z}}{2}\sqrt{n_1}g_{1,1}(n_1)f_2(n_2)\ket{n}\bra{n_1-1,n_2}\,.
\end{align*}
% Here, we extended the domain of definition of any function $F$ involved above to $\mathbb{Z}^2$, and fixed $F(n)=0$ for $n\notin\mathbb{N}$.
% Notice that we also incorporated the action of $\cL[a^2-\alpha^2]^\dagger$ on $f(N)$ into the operator $\hat{C}_0$.
 % In the next step, we consider the output coefficients of $\cL^\dagger(f(N))$ w.r.t.~the above Fock basis elements: \textit{Case 0:} $\ket{n_1,n_2}\bra{n_1,n_2}$:
 %    \begin{equation*}
 %        \begin{aligned}
 %            &f(n_1,n_2-2)(n_2-1)n_2\ket{n}\bra{n}+f(n_1-1,n_2)|z|^2n_1\ket{n}\bra{n}-f(n)\Bigl({n_2(n_2-1)}+|z|^2 {n_1}\Bigr)\ket{n}\bra{n}\\
 %            &\qquad=-\left(f_1(n_1)g_{2,2}(n_2)(n_2-1)n_2+|z|^2g_{1,1}(n_1)f_2(n_2)n_1\right)\ket{n}\bra{n}
 %        \end{aligned}
 %    \end{equation*}
    % \textit{Case 1:} $\ket{n_1+1,n_2-2}\bra{n_1,n_2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         &\frac{\overline{z}}{2}\sqrt{(n_1+1)(n_2-1)n_2}\biggl(2f(n_1,n_2-2)-\Bigl(f(n)+f(n_1+1,n_2-2)\Bigr)\biggr)\ket{n_1+1,n_2-2}\bra{n}\\
    %         &\qquad=-\frac{\overline{z}}{2}\sqrt{(n_1+1)(n_2-1)n_2}\Bigl(f_1(n_1)g_{2,2}(n_2)+g_{1,1}(n_1+1)f_2(n_2-2)\Bigr)\ket{n_1+1,n_2-2}\bra{n}
    %     \end{aligned}
    % \end{equation*}
    % \textit{Case 1:} $\ket{n_1,n_2}\bra{n_1+1,n_2-2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         &-\frac{{z}}{2}\sqrt{(n_1+1)(n_2-1)n_2}\Bigl(f_1(n_1)g_{2,2}(n_2)+g_{1,1}(n_1+1)f_2(n_2-2)\Bigr)\ket{n}\bra{n_1+1,n_2-2}
    %     \end{aligned}
    % \end{equation*}
    % \textit{Case 2:} $\ket{n_1,n_2-2}\bra{n_1,n_2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         &\frac{\overline{w}}{2}\sqrt{(n_2-1)n_2}\Bigl(f(n_1,n_2-2)-f(n)\Bigr)\ket{n_1,n_2-2}\bra{n}\\
    %         &\qquad=-\frac{\overline{w}}{2}\sqrt{(n_2-1)n_2}f_1(n_1)g_{2,2}(n_2)\ket{n_1,n_2-2}\bra{n}
    %     \end{aligned}
    % \end{equation*}
    % \textit{Case 2:} $\ket{n_1,n_2}\bra{n_1,n_2-2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         -\frac{{w}}{2}\sqrt{(n_2-1)n_2}f_1(n_1)g_{2,2}(n_2)\ket{n}\bra{n_1,n_2-2}
    %     \end{aligned}
    % \end{equation*}
    % \textit{Case 3:} $\ket{n_1-1,n_2}\bra{n_1,n_2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         &\frac{\overline{w}z}{2}\sqrt{n_1}\Bigl(f(n_1-1,n_2)-f(n)\Bigr)\ket{n_1-1,n_2}\bra{n}=-\frac{\overline{w}z}{2}\sqrt{n_1}g_{1,1}(n_1)f_2(n_2)\ket{n_1-1,n_2}\bra{n}
    %     \end{aligned}
    % \end{equation*}
    % \textit{Case 3:} $\ket{n_1,n_2}\bra{n_1-1,n_2}$:
    % \begin{equation*}
    %     \begin{aligned}
    %         &-\frac{{w}z}{2}\sqrt{n_1}g_{1,1}(n_1)f_2(n_2)\ket{n}\bra{n_1-1,n_2}
    %     \end{aligned}
    % \end{equation*}
In fact, we will only use the following upper bound on $\hat{C}_0(n)$ in what follows:    
   \begin{equation*}
        \begin{aligned}
           &C_0(n)\\
           &=- \Big(f_1(n_1)g_{2,2}(n_2)(n_2-1)n_2+|z|^2g_{1,1}(n_1)f_2(n_2)n_1\Big)\\
            &\le -\Big( f_1(n_1)g_{2,2}(n_2)1_{n_2\geq2}((n_2+1)^{2}-3(n_2+1))+|z|^2g_{1,1}(n_1)f_2(n_2)n_1\Big)\\
            &\overset{(1)}{\le} -\left\{ k_2f_1(n_1)(n_2+1)^{k_2/2}1_{n_2\geq2}\Big((n_2+1)-1_{k_2\geq3}\frac{k_2}{2}-6\Big)   +|z|^21_{n_1\geq1}(n_1+1)^{k_1/2-1}f_2(n_2)n_1
            \right\}\\
&\equiv C_{0'}(n)\,,
        \end{aligned}
    \end{equation*}
    where $(1)$ follows from \Cref{lem:upper-lower-bound-gl}. In what follows, we will hence denote $\hat{C}_{0'}=\sum_n C_{0'}(n)|n\rangle\langle n|$. We also recall that, by \Cref{lem:l-diss} and \Cref{rmk:l-diss-multimode},
    \begin{equation}\label{eq-ex:CNOT-diss-rep}
        \begin{aligned}
            \cL[a^2-\alpha^2]^\dagger(f(N))&\leq - \,(N_1+\1)^{k_1/2+1}(N_2+\1)^{k_2/2}
            +\mu_{k_1}^{(2)}\, (N_2+\1)^{k_2/2} \\
            &=\sum_{n} \Big(-(n_1+1)f(n)+\mu_{k_1}^{(2)}\,f_2(n_2)\Big)|n\rangle\langle n|=:\hat{C}_{4}\,,
        \end{aligned}
    \end{equation}
    where $\mu_{k_1}^{(2)}=\Delta_2^\nu\left(\frac{(\nu-1)^{\nu-1}}{\nu^\nu}\right)$ with $\nu=\frac{k_1}{2}+1$ and $\Delta_2=6+2|\alpha|^2k_12^{k_1/2}\sqrt{2}$. Therefore, the diagonal contribution of $\cL^\dagger(f(N))$ can be controlled by 
    \begin{equation*}
        \begin{aligned}
    ...
    %     \tikzmark{left}\ast &0 &\ast &0 &0 &0 \\               
    %     0 &\textcolor{gray!40!white}{\ast} &0 &\ast &0 &0 \\      
    %     \ast &0 &\ast\tikzmark{right} &0 &\ast &0 \\\DrawBox[thick]
    %     0 &\ast &0 &\tikzmark{left}\ast &0 &\ast \\
    %     0 &0 &\ast &0 &\textcolor{gray!40!white}{\ast} &0 \\
    %     0 &0 &0 &\ast &0 &\ast\tikzmark{right} \\\DrawBox[thick]
    % \end{array}\right)\,.\]
   The operator \ref{eq:2x2} has the following eigenvalues:
    ...
    \end{equation*}
   Using this bound, we control each of the off-diagonal operators $\hat{C}_i+\hat{C}_{i'}$, $i\in\{1,2,3\}$, in terms of $\frac{1}{4}X$, where $X=\sum_n x(n)|n\rangle\langle n|$. 
    % Note that the diagonal term given by \Cref{eq-ex:CNOT-diss-rep} is added after the following upper bound. Since, we have three cases, we split the leading order on the diagonal in four pieces to use three to bound the other terms by a constant. 
    \begin{equation*}
        \begin{aligned}
          -  \frac{1}{4}X+\hat{C}_1+\hat{C}_{1'}&\coloneqq \sum_n-\frac{1}{4}x(n)\ketbra{n}{n}+y\ketbra{n}{n_1+1,n_2-2}+\overline{y}\ketbra{n_1+1,n_2-2}{n}\\
&      \le     \sum_{n|n_2\ge 2}-\frac{1}{8}x(n)\ketbra{n}{n}-\frac{1}{8}x(n_1+1,n_2-2)\ketbra{n_1+1,n_2-2}{n_1+1,n_2-2}\\
    ...
    so that
    \begin{equation}
         -  \frac{1}{4}X+\hat{C}_1+\hat{C}_{1'}\leq \sum_{n|n_2\ge 2}\left(-\min\{x_1,x_2\}+|y_1|\right)\sum_{m_1=n_1}^{n_1+1}\sum_{m_2=n_2-2}^{n_2}\ketbra{m_1,m_2}{m_1,m_2}\label{eqcaseC1C1'}
    \end{equation}
...
\medskip
    ...
    \begin{equation*}
        \begin{aligned}
           -\min\{x_1,x_2\}+|y_1|&=-\frac{1}{16}\biggl(k_2f_1(n_1)(n_2+1)^{k_2/2+1}+(n_1+1)^{k_1/2+1}f_2(n_2)\biggr)+\frac{1}{8}\Delta_0\\
    ...
    \end{equation}
    where we recall the definition $z=-\frac{\alpha}{2}(1-e^{2i\pi t/T})$. We have therefore proved the existence of a constant $\Tilde{\Delta}_1$ such that 
    \begin{equation*}
        \begin{aligned}
           -\min\{x_1,x_2\}+|y_1|\leq\Tilde{\Delta}_1\,.
        \end{aligned}
    \end{equation*}
...
\noindent Case (ii): $x_2\leq x_1$. In that case $-\min_{x_1,x_2}+|y_1|=-x_2+|y_1|$, and therefore
    \begin{equation*}
        \begin{aligned}
        ...
        \end{aligned}
    \end{equation*}
    To upper bound the above, we use again the assumption \eqref{assumptionequationok}, which implies the existence of a constant $\Delta_1$ such that
    \begin{equation*}
        \begin{aligned}
           -\min\{x_1,x_2\}+|y_1|&\leq\Tilde{\Delta}'_1\,.
        \end{aligned}
   ...
   \begin{align}\label{lastC1}
       -\frac{1}{4}X+\hat{C}_1+\hat{C}_{1'}\le \Delta_1\sum_{n|n_2\ge 2}\sum_{m_1=n_1}^{n_1+1}\sum_{m_2=n_2-2}^{n_2}|m_1,m_2\rangle\langle m_1,m_2|\,.
    \end{align}
...
-\frac{1}{4}X+\hat{C}_2+\hat{C}_{2'}\le \Delta_2\,\sum_{n|n_2\ge 2}\sum_{m_2=n_2-2}^{n_2}|n_1,m_2\rangle\langle n_1,m_2|\,.
   \end{align}
...
-\frac{1}{4}X+\hat{C}_3+\hat{C}_{3'}\le \Delta_3\,\sum_{n|n_1\ge 1}\sum_{m_1=n_1-1}^{n_1}|m_1,n_2\rangle\langle m_1,n_2|\,.
   \end{align}
    Combining \eqref{lastC1}, \eqref{lastC2} and \eqref{lastC3}, we have shown that
    \begin{align*}
        \cL^\dagger(f(N))&\leq -\frac{X}{4}+\Delta_1\sum_{n|n_2\ge 2}\sum_{m_1=n_1}^{n_1+1}\sum_{m_2=n_2-2}^{n_2}|m_1,m_2\rangle\langle m_1,m_2|\\
        &+\Delta_2\sum_{n|n_2\ge 2}\sum_{m_2=n_2-2}^{n_2}|n_1,m_2\rangle\langle n_1,m_2|+\Delta_3 \sum_{n|n_1\ge 1}\sum_{m_1=n_1-1}^{n_1}|m_1,n_2\rangle\langle m_1,n_2|\\
...
\begin{rmk}\label{rmk:Sobolev-preserving-CNOT}
    Following the multi-mode extension in \Cref{sec:multi-mode-extension} of the time-dependent generation theorem (see \Cref{thm:multimode-generation-theorem}), the main ingredient for its proof consists in the construction of a sequence of compactly embedded Sobolev spaces $W^{\k_j,1}$ which are used to make operators $\cL$ defined via polynomials of annihilation and creation operators bounded, i.e. $\|\cL\|_{W^{\k_i,1}\rightarrow W^{\k_j,1}}<\infty$ for some $i,j\in\N$. Since the above powers of the number operator in each modes can grow to infinity, one can follow the same strategy as in \Cref{sec:multi-mode-extension} to show that the $\operatorname{CNOT}$-gate is a Sobolev and positivity preserving quantum evolution system $\cP_{t,t_0}^{\operatorname{CNOT}}$ which satisfies for all states $\rho\in W^{\k,1}$  
    ...
    \end{equation}
    Note that the evolution system is Sobolev preserving w.r.t.~a sequence of compactly embedded Sobolev spaces that satisfies the conditions of \Cref{lem:assum2-CNOT}. 
\end{rmk}
...
% For this, we consider the operator $\cL_{G}^\dagger(N+\1)$ which can be rewritten as
%      \begin{equation*}
%          \begin{aligned}
%              \cL_{G}^\dagger(N+\1)&=|\gamma|^2N^2 +\gamma\overline{\eta} a^2 N+\overline{\gamma}\eta(\ad)^2(N+2)+|\eta|^2(N+\1)(N+2)\\
%              &\quad-\frac{1}{2}\left\{(|\gamma|^2+|\eta|^2)N+|\eta|^2\1+\overline{\eta}\gamma a^2+\overline{\gamma}\eta(\ad)^2,(N+\1)\right\}\\
%              &=N\left(|\eta|^2((N+2)-(N+\1))+|\gamma|^2(N-(N+\1))\right)+|\eta|^2((N+2)-(N+\1))\\
%              &\quad-\frac{1}{2}\left(\overline{\eta}\gamma a^2((N+\1)-(N-1))+\overline{\gamma}\eta((N+\1)-(N-1))(\ad)^2\right)\,.
%          \end{aligned}
%      \end{equation*}
%      Next, we apply the Cauchy-Schwarz inequality to the last two terms so that
%      \begin{equation*}
%          \begin{aligned}
%              |\tr[\rho a^2g_2(N)]|&= |\tr[\sqrt{\rho}\sqrt{g_2(\Nind-2\1)}a a\sqrt{g_2(\Nind)}\sqrt{\rho}]|\\
%              &\leq\|\ad \sqrt{g_2(N-2\1)}\sqrt{\rho}\|_2\,\|\sqrt{\rho}\sqrt{g_2(N)}a\|_2\\
%              &=\sqrt{\tr[\rho g_2(N-2\1)(N+\1)]\,\tr[\rho g_2(N)(N+\1)]}
%              \leq \tr[\rho g_2(N)(N+\1)]
%          \end{aligned}
%      \end{equation*}
%      and similarly
%      \begin{equation*}
%          \begin{aligned}
%              |\tr[\rho g_2(N)(\ad)^2]|&= |\tr[\sqrt{\rho}\sqrt{g_2(\Nind)}\ad \ad\sqrt{g_2(\Nind-2\1)}\sqrt{\rho}]|\leq \tr[\rho g_2(N)N]
%          \end{aligned}
%      \end{equation*}
%      where we used that $g_2(x-2)\leq g_2(x)$ for all $x\geq0$. Summarizing the above equations show 
%      \begin{equation*}
%          \begin{aligned}
%              \tr[\rho\cL^\dagger(f(N))]\leq\tr[\rho\left(N\left(|\eta|^2g_1(N+\1)-|\gamma|^2g_1(N)\right)+|\eta|^2g_1(N+\1)+|\eta\gamma|g_2(N)(N+\1)\right)].
%          \end{aligned}
%      \end{equation*}
%      As before, we can reduce the problem of finding an upper bound to the spectrum and use the binomial theorem (see \Cref{eq:binomial-rep} \textcolor{red}{???}) with the additional representation 
%      \begin{equation}\label{eq-ex:binomial-rep-2}
%          x^{k}-(x-2)^k=-\sum_{j=1}^k\binom{k}{j}x^{k-j}(-2)^{j}=2kx^{k-1}-\sum_{j=2}^k\binom{k}{j}x^{k-j}(-2)^{j}
%      \end{equation}
%      to identify the leading order by $-k(|\gamma|^2-|\eta|^2-2|\eta\gamma|)n^k$, i.e.
%      \begin{equation}\label{eq-ex:gaussian-leading-order}
%          \begin{aligned}
%          	             n\biggl(&|\eta|^2g_1(n+1)-|\gamma|^2g_1(n)\biggr)+|\eta|^2g_1(n+1)+|\eta\gamma|g_2(n)(n+1)\\
%              &=n\biggl(|\eta|^2\sum_{j=1}^k\binom{k}{j}n^{k-j}-|\gamma|^2\sum_{j=1}^k\binom{k}{j}n^{k-j}(-1)^{j+1}\biggr)\\
%              &\qquad+|\eta|^2\sum_{j=1}^k\binom{k}{j}n^{k-j}-|\eta\gamma|\sum_{j=1}^k\binom{k}{j}x^{k-j}(-2)^{j}(n+1)\\
%              &=\sum_{j=1}^k\binom{k}{j}\left(n^{k-j+1}(|\eta|^2-|\gamma|^2(-1)^{j+1}-|\eta\gamma|(-2)^{j})+n^{k-j}(|\eta|^2-|\eta\gamma|(-2)^{j})\right)\\
%              &=n^{k}(|\eta|^2-|\gamma|^2+2|\eta\gamma|)+\sum_{j=2}^k\binom{k}{j}n^{k-j+1}(|\eta|^2-|\gamma|^2(-1)^{j+1}-|\eta\gamma|(-2)^{j})\\
%              &\qquad+\sum_{j=1}^k\binom{k}{j}n^{k-j}(|\eta|^2-|\eta\gamma|(-2)^{j})\,.
%          \end{aligned}
%      \end{equation}
%      By assumption the leading order is negative  so that the following function admits a global maximum on $x\in[0,\infty)$.
%      \begin{equation}\label{eq-ex:gaussian-optimization-muk}
%          \begin{aligned}
%              h_k^G(x)&\coloneqq x\biggl(|\eta|^2g_1(x+1)-|\gamma|^2g_1(x)+|\eta\gamma|g_2(x)\biggr)+|\eta|^2g_1(x+1)+|\eta\gamma|g_2(x)\\
%              &+(k-1)(|\gamma|^2-|\eta|^2-2|\eta\gamma|)x^k
%          \end{aligned}
%      \end{equation}
%      The global maximum $\mu_k=\max_{x\in[0,\infty)}h_k^G(x)$ shows
%      \begin{equation*}
%          \tr[\cL_{G}(\rho)(N+\1)^k]\leq -(k-1)(|\gamma|^2-|\eta|^2-2|\eta\gamma|)\tr[\rho (N+\1)^k]+\mu_k
%      \end{equation*}
%      and finishes the proof. With help of $\sum_{j=0}^k\binom{k}{j}=2^k$ and \Cref{eq-ex:gaussian-leading-order}
%      \begin{equation*}
%          \begin{aligned}
%              h_k^G(x)\leq-(|\gamma|^2-|\eta|^2-2|\eta\gamma|)n^k+(2|\eta|^2+2|\eta\gamma|+|\gamma|^2)4^kn^{k-1}\,.
%          \end{aligned}
%      \end{equation*}
%      Therefore, a rough upper bound for $\mu_k$ is given the maximum 
%      \begin{equation}\label{eq-ex:gaussian-rough-upper-bound-muk}
%          -(|\gamma|^2-|\eta|^2-2|\eta\gamma|)x_{\max}^k+(2|\eta|^2+2|\eta\gamma|+|\gamma|^2)4^kx_{\max}^{k-1}
%      \end{equation}
%      where $x_{\max}=\frac{(k-1)(2|\eta|^2+2|\eta\gamma|+|\gamma|^2)4^k}{k(|\gamma|^2-|\eta|^2-2|\eta\gamma|)}$.
    ...
    \end{align*}
    Next, we use \Cref{regularization}, so that
    \begin{align*}
...
\end{equation}
Additionally, it is shown that there is a unique limit $\overline{\rho}$ of $e^{t\cL_l}(\rho)$ for $t\rightarrow\infty$. Combining this bound with our established generation theory for Sobolev and positivity preserving Markov semigroups, we show large time perturbation bounds. We start with the $l$-photon dissipation perturbed by the Hamiltonian introduced in \Cref{lem:l-diss}, i.e.~$H=p_H(a,\ad)$ with $d_H\leq2(l-1)$.
\begin{thm}\label{thm:l-diss-hamiltonian-perturbation}
    Let $\cL_l$ be the generator of the $l$-photon dissipation and $p_H\in\C[X,Y]$ with $\deg(p_H)=d_H\leq2(l-1)$ such that $H=p_H(a,\ad)$ is a symmetric operator. Then, there exist explicit constants $c,\gamma>0$ such that for $\varepsilon \ge 0$ and all states $\rho\in W^{6l-4,1}$
    \begin{equation*}
        \begin{aligned}
            \Big|\tr[L\left(e^{t\cL_l}(\rho)-e^{t(\cL_l+\varepsilon\cH[H])}(\rho)\right)L^\dagger]\Big|\leq\varepsilon c\left(1-e^{-l!t}\right)\max\{\gamma,\|\rho\|_{W^{6l-4,1}}\}\,.
        \end{aligned}
...
\end{thm}
\begin{proof}
    The proof consists in applying \Cref{thm:semigroup-perturbation} in combination with \Cref{eq:exponential-convergence}. For that, we show that
    \begin{equation*}
        \|\cW^{2l}\cH[H]\cW^{-6l+4}\|_{1\rightarrow1}<\infty\,,
    \end{equation*}
where we recall that $\cW^k(\cdot) \coloneqq (N+\1)^{k/4}(\cdot)(N+\1)^{k/4}$. For a state $\rho\in\cT_f$ and $Y\coloneqq\cW^{-6l+4}(\rho)$, we have by Hölder's inequality
    ...
    where
    \begin{equation*}
        \gamma_\varepsilon=\tilde{c}^\nu\left(\frac{(\nu-1)^{\nu-1}}{\nu^\nu}\right)\quad\text{with}\quad \tilde{c}={(l+1)l}+2|\alpha|^lkl^{k/2}\sqrt{l!}+\varepsilon\Lambda(2l)^{k/2+1}\sqrt{(2l)!}\,,\quad\nu=l+\frac{k}{2}-1\,.
    \end{equation*}
...
Next, we bound 
    \begin{equation}\label{eq:L-bound}
        \begin{aligned}
            \|L(N+1)^{-l}L^\dagger\|_1\leq1+|\alpha|^l\,(l+1)\,\sqrt{l!}+|\alpha|^{2l}
        \end{aligned}
    \end{equation}
    which follows from
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            &\tr[L\biggl(e^{t\cL_l}(\rho)-e^{t(\cL_l+\varepsilon\cH[H])}(\rho)\biggr)L^\dagger\tr]\\
            &\qquad=\varepsilon\tr[L\int_{0}^te^{s\cL_l}\cW^{-2l}\cW^{2l}\cH[H]e^{(t-s)(\cL_l+\varepsilon\cH[H])}(\rho)dsL^\dagger]\\
            &\qquad\overset{(1)}{\leq}\varepsilon\tr[L\int_{0}^te^{s\cL_l}\cW^{-2l}(\1)dsL^\dagger]2\Lambda d_H^2\sqrt{d_H!}\max\left\{\gamma_\varepsilon,\,\|\rho\|_{W^{6l-4,1}}\right\}\\
            ...
            &\qquad\eqqcolon\varepsilon c(1-e^{-l!t})\max\left\{\gamma_\varepsilon,\,\|\rho\|_{W^{6l-4,1}}\right\}
        \end{aligned}
    \end{equation*}
...
A special case of the above result is that of the $Z(\theta)$ gate. The explicit structure and small number $l\in\N$ allow us to improve the perturbation bound in the following way:
\begin{lem}[$Z(\theta)$-gate]
    Let $\cL_2$ be the $2$-photon dissipation. Then, for all $\varepsilon\in[0,1]$ and all states $\rho\in W^{6,1}$
    \begin{equation*}
        \begin{aligned}
            \Big|\tr[L\left(e^{t\cL_2}(\rho)-e^{t(\cL_2+\varepsilon\cH[a+\ad])}(\rho)\right)L^\dagger]\Big|\leq\varepsilon8(1+6|\alpha|^2+|\alpha|^{4})(1-e^{-2t})\max\left\{\gamma_\varepsilon,\|\rho\|_{W^{6,1}}\right\}\,,
        \end{aligned}
    \end{equation*}
    where $\gamma_\varepsilon=\frac{3^7}{4^2}\left(1+\sqrt{2}2^4\,|\alpha|^2+\varepsilon8\right)^4$
\end{lem}
\begin{proof}
    As in the proof of \Cref{thm:l-diss-hamiltonian-perturbation}, we first proove the bound 
    \begin{equation*}
    ...
    % \begin{equation*}
    %     \tr[(\varepsilon\cH[a+\ad]+\cL[a^2+\alpha^2])(\rho)(N+\1)^3]\leq-3\tr[\rho(N+\1)^{k/2+1}]+\frac{9}{2}\left(4+\varepsilon+|\alpha|^2\right)^{3}.
    % \end{equation*}
    % so that \Cref{prop-ex:uniformly-bounded-semigroup} shows 
    \begin{equation*}
        \|e^{t(\cL_2+\varepsilon\cH[a+\ad])}(\rho)\|_{W^{6,1}}\leq\max\left\{\frac{3^7}{4^2}\left(1+\sqrt{2}2^4\,|\alpha|^2+\varepsilon8\right)^4,\|\rho\|_{W^{6,1}}\right\}\,.
    \end{equation*}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            &\qquad\leq\varepsilon\tr[\int_0^tLe^{s\cL_2}\cW^{-4}(\1)L^\dagger ds]16\max\left\{\frac{3^7}{4^2}\left(1+\sqrt{2}2^4\,|\alpha|^2+\varepsilon8\right)^4,\|\rho\|_{W^{6,1}}\right\}\\
            &\qquad\leq\varepsilon8(1+6|\alpha|^2+|\alpha|^{4})(1-e^{-2t})\max\left\{\frac{3^7}{4^2}\left(1+\sqrt{2}2^4\,|\alpha|^2+\varepsilon8\right)^4,\|\rho\|_{W^{6,1}}\right\}
        \end{aligned}
    \end{equation*}
...
\end{align*}
for $\cH_{m'}\cong \cH_m$, and where the difference with \eqref{ECnorm} lies in the fact that the input states in the above optimization are energy constrained in both their inputs. Clearly, from the definitions we have that 
...
\emph{Acknowledgments:} We would like to thank Yu-Jie Liu for the fruitful discussions which initiated this project. Moreover, we would like to thank Marius Lemm, Angela Capel-Cuevas, Simone Warzel, Michael M. Wolf, Libor Caha, Vjosa Blakaj, Shin Ho Choe, Robert Salzmann, Simon Becker, Lauritz van Luijk, and Niklas Galke for the valuable feedback and discussions on the topic. Moreover, we would like to thank Jochen Schmid for his help with questions on evolution systems. T.M. and C.R. acknowledge the support of the Munich Center for Quantum Sciences and Technology, C.R. that of the Humboldt Foundation, and P.G. that of the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) – Project-ID 470903074 – TRR 352.
...
In this section, we prove some basic properties of polynomials of annihilation and creation operators. We shortly repeat the normla form of our polynomials in $a$ and $\ad$ given in \Cref{eq:ccr-polynomial-representation}: 
\begin{equation*}
    p(a\,,\ad)=\sum_{i + 2j \le \deg(p)}\lambda_{ij}(\ad)^iN^j+\,\sum_{k + 2l\le \deg(p)}\mu_{kl}N^la^k
\end{equation*}
...
\begin{equation*}
    ...
    \end{aligned}
\end{equation*}
Then, the leading order of the summands in $n$ is maximally of order $d/2$ so that $\cD(N^{d/2})$ is a domain of $p(a,\ad)$. For the modified polynomial $\Tilde{p}$, there is sequence of functions $R_n:\cH \rightarrow\R$ with asymptotics strictly smaller than $n^{4d}$ such that
...
\begin{proof}
    ...
    %\begin{equation}
    %    P_M\Tilde{p}(a,\ad)=P_{M}\left(\sum_{i,j,k,l=0}^{d+1}\lambda_{ij}P_{M-j}N^ia^j+\mu_{kl}P_{M+l}(\ad)^lN^k\right)P_{M+d}
    %\end{equation}
    \begin{equation*}
...
Now, by definition of $\Tilde{p}$ and denoting by $\varphi_n$ the coefficients of $|\varphi\rangle$ in the Fock basis,
    ...
    \end{equation*}
    where $p^\dagger$ is defined similarly to $\tilde{p}^\dagger$. By assumption $\phi\notin\cD(N^{2d})$, so that the above sequence is diverging for $M\rightarrow\infty$ to infinity (see \Cref{eq:leading-order-poly}). This contradicts the assumption and shows $\cD(\tilde{p}(a,\ad)^\dagger)=\cD(N^{2d})$ as well as $\tilde{p}(a,\ad)^\dagger=\tilde{p}^\dagger(a,\ad)$. Moreover, $p(a,\ad)$ is by Theorem 7.1.1 in \cite{Simon2015} a closed operator.


    
    % In the second part of the lemma, we start proving that $-(N+\1)$ is a generator of the $C_0$-semigroup. By the spectral decomposition of $N$,
    % \begin{equation*}
    %     \|(\lambda+(1+N)^k)\ket{\phi}\|^2=\sum_{j\in \mathbb{N}}(\lambda (1+j)^k)^2|\phi_j|^2\geq\sum_{j\in \mathbb{N}}\lambda^2|\phi_j|^2=\lambda^2\|\phi\|
    % \end{equation*}
    % shows that $-(N+\1)$ is dissipative. Since $-(N+\1)^k$ is self-adjoint and closed by the argument above, \Cref{cor:lumer-phillips} shows that $-(\1+N)^k$ is a generator of a contraction $C_0$-semigroup. Since the generator uniquely determines the semigroup
    % \begin{equation*}
    %     e^{-t(\1+N)^k}=\sum_{n\in\N}e^{-t(1+n)^k}\ketbra{n}{n}.
    % \end{equation*}
    % This representation shows that the semigroup is invariant under $\cH_f$, i.e.~$e^{tN}\cH_f\subset\cH_f$. 
    Since $\{\ket{n}\}_{n \in \N}$ is an orthonormal basis and $N$ a multiplication operator on that basis, we can immediately conclude that $\cH_f$ is a core for $N$ and further for all $(\1 + N)^k$, $k \ge 0$. Since $\Tilde{p}(a,\ad)$ is closed w.r.t.~$\cD(N^{p/2+1})$, $\cH_f$ is also a core of $\Tilde{p}(a,\ad)$.
\end{proof}
...
\end{proof}
% A trick, we use in the example section is the following bound on the simple Hamiltonian $H=\overline{z}(\ad)^lh(N)+zh(N)a^l$, where g is any real-valued function, $l\in\N$, and $z\in\C$.
% \begin{lem}\label{lem:two-point-hamiltonian-bound}
%     Let $l\in\N$, $z\in\C$, and $h$ a real-valued function. Then,
%     \begin{equation*}
%         H\coloneqq za^lh(N)+\overline{z}h(N)(\ad)^l\leq |z|\,l\,h(N)\sqrt{(N+l\1)\cdots (N+\1)}
%     \end{equation*}
% \end{lem}
% \begin{proof}
%     To prove the statement, we present the Hamiltonian $H$ in the Fock basis:
%     \begin{equation*}
%         \begin{aligned}
%             H&=\sum_{j\in\N}zh(j)\sqrt{(j+l)\cdots (j+1)}\ket{j+l}\bra{j}+\overline{z}h(j)\sqrt{(j+l)\cdots (j+1)}\ket{j}\bra{j+l}\\
%             &=\sum_{j\in\N}\Tilde{h}_l(j)(z\ket{j+l}\bra{j}+\overline{z}\ket{j}\bra{j+l})
%         \end{aligned}
%     \end{equation*}
%     where $\tilde{h}_l(j)\coloneqq h(j)\sqrt{(j+l)\cdots (j+1)}$. Then, we split the sum into sums of blocks with only two non-zero elements:
%     \begin{equation*}
%         \begin{aligned}
%             H=\sum_{k=0}^{l-1}\sum_{j\in\N}\Tilde{h}(jl+k)(z\ket{(j+1)l+k}\bra{jl+k}+\overline{z}\ket{jl+k}\bra{(j+1)l+k}).
%         \end{aligned}
%     \end{equation*}
%     For fixed $k\in\{0,...,l-1\}$,
%     \begin{equation*}
%         \sum_{j\in\N}\Tilde{h}(jl+k)(z\ket{(j+1)l+k}\bra{jl+k}+\overline{z}\ket{jl+k}\bra{(j+1)l+k})
%     \end{equation*}
%     has the eigenvalue $0$ w.r.t.~the eigenvectors $\ket{n}$ for $n\in\N\backslash(\N l+k)$ and the eigenvalues $|z|\Tilde{h}(jl+k)$ w.r.t.~$|z|\ket{jl+k}+z\ket{(j+1)l+k}$ and $-|z|\Tilde{h}(jl+k)$ w.r.t.~$|z|\ket{jl+k}-z\ket{(j+1)l+k}$ for all $j\in\N$. Therefore, 
%     \begin{equation*}
%         \begin{aligned}
%             \sum_{j\in\N}\Tilde{h}(jl+k)(z\ket{(j+1)l+k}\bra{jl+k}&+\overline{z}\ket{jl+k}\bra{(j+1)l+k})\\
%             &\leq\sum_{j\in\N}|z|\Tilde{h}(jl+k)\sum_{i=0}^{l-1}\ket{jl+k+i}\bra{jl+k+i}~.
%         \end{aligned}
%     \end{equation*}
%     All in all we achieve, 
%     \begin{equation*}
%         \begin{aligned}
%             H&=\sum_{k=0}^{l-1}\sum_{j\in\N}\Tilde{h}(jl+k)(z\ket{(j+1)l+k}\bra{jl+k}+\overline{z}\ket{jl+k}\bra{(j+1)l+k})\leq|z|l\tilde{h}(N)
%         \end{aligned}
%     \end{equation*}
%     which proves the statement.
% \end{proof}
% The above result can be extended to two modes. 
% \begin{lem}\label{lem:two-point-hamiltonian-bound}
%     Let $l_1,l_2\in\N$, $z\in\C$, and $h$ a real-valued function. Then,
%     \begin{equation*}
%         \begin{aligned}
%             H&\coloneqq za^{l_1}h(N_1,N_2)(\bd)^{l_2}+\overline{z}b^{l_2}h(N_1,N_2)(\ad)^l_1\\
%             &\leq |z|\,l_1l_2\,h(N_1,N_2)\sqrt{(N_1+l_1\1)\cdots (N_1+\1)(N_2+l_2\1)\cdots (N_2+\1)}
%         \end{aligned}
%     \end{equation*}
%         \begin{equation*}
%         H\coloneqq za^lh(N)+\overline{z}h(N)(\ad)^l\leq |z|\,l\,h(N)\sqrt{(N+l\1)\cdots (N+\1)}
...
% \end{lem}
% \begin{proof}
%     As in the proof of \Cref{lem:two-point-hamiltonian-bound}, we present the Hamiltonian $H$ in the Fock basis:
%     \begin{equation*}
%         \begin{aligned}
%             H
%             &=\sum_{j_1,j_2\in\N}\Tilde{h}_{l_1,l_2}(j_1,j_2)\left(z\ket{j_1+l_1,j_2}\bra{j_1,j_2+l_2}+\overline{z}\ket{j_1,j_2+l_2}\bra{j_1+l_1,j_2}\right)
%         \end{aligned}
%     \end{equation*}
%     where $\tilde{h}_{l_1,l_2}(j_1,j_2)\coloneqq h(j_1,j_2)\sqrt{(j_1+l_1)\cdots (j_1+1)}\sqrt{(j_2+l_2)\cdots (j_2+1)}$. Then, we split the sum into sums of blocks with only two non-zero elements:
%     \begin{equation*}
%         \begin{aligned}
%             \sum_{k_1,k_2=0}^{l_1-1,l_2-1}\sum_{j_1,j_2\in\N}\Tilde{h}_{l_1,l_2}(j_1l_1+k_1,j_2l_2+k_2)\biggl(&z\ket{(j_1+1)l_1+k_1,j_2l_2+k_2}\bra{j_1l_1+k_1,(j_2+1)l_2+k_2}\\
%             &+\overline{z}\ket{j_1l_1+k_1,j_2(l_2+1)+k_2}\bra{(j_1+1)l_1+k_1,j_2l_2+k_2}\biggr).
%         \end{aligned}
%     \end{equation*}
%     For fixed $k_1\in\{0,...,l_1-1\}$ and $k_2\in\{0,...,l_2-1\}$,
%     \begin{equation*}
%         \begin{aligned}
%             \sum_{j_1,j_2\in\N}\Tilde{h}_{l_1,l_2}(j_1l_1+k_1,j_2l_2+k_2)\biggl(&z\ket{(j_1+1)l_1+k_1,j_2l_2+k_2}\bra{j_1l_1+k_1,(j_2+1)l_2+k_2}\\
%             &+\overline{z}\ket{j_1l_1+k_1,j_2(l_2+1)+k_2}\bra{(j_1+1)l_1+k_1,j_2l_2+k_2}\biggr).
%         \end{aligned}
%     \end{equation*}
%     has the eigenvalue $0$ w.r.t.~the eigenvectors $\ket{n_1,n_2}$ for $n_1\in\N\backslash(\N l_1+k_1)$ or $n_2\in\N\backslash(\N l_2+k_2)$. The eigenvalues $|z|\Tilde{h}(j_1l_1+k_1,j_2l_2+k_2)$ w.r.t.~$|z|\ket{j_1l_1+k_1,j_2l_2+k_2}+z\ket{(j_1+1)l_1+k_1,(j_1+1)l_1+k_1}$ and $-|z|\Tilde{h}(j_1l_1+k_1,j_2l_2+k_2)$ w.r.t.~$|z|\ket{j_1l_1+k_1,j_2l_2+k_2}-z\ket{(j_1+1)l_1+k_1,(j_2+1)l_2+k_2}$ for all $j_1,j_2\in\N$. This proves the statement.
% \end{proof}
...
\begin{lem}\label{lem:two-point-hamiltonian-bound}
    ...
    \begin{equation*}
        \begin{aligned}
            ...
            &\qquad\qquad\qquad\qquad \leq (m_1+1)(m_2+1)|z|\widetilde{h}_{m_1,m_2}(N_1+m_1I,N_2+m_2I)\,,
        \end{aligned}
    \end{equation*}
...
\section{Real power inequalities}
...
For $k\geq2$,
    ...
    \end{equation*}
    which finishes the proof of the first upper bound. The other two bounds are obvious by definition. Next, we consider the lower bounds. The case $x<l-1$ is trivial, so that we are left with proving
    \begin{equation*}
    ...
    %\begin{equation*}
    %    \begin{aligned}
    %        g_l(x)&=(x+1)^{1/2}\frac{3l}{2}\int_{0}^1\left(1-s\frac{l}{x+1}\right)^{1/2}ds\\
    %        &\geq (x+1)^{1/2}\frac{3l}{2}\int_{0}^1\left(1-s\right)^{1/2}ds\\
    %        &\geq (x+1)^{1/2}l
    %    \end{aligned}
    %\end{equation*}
...
\begin{lem}[Differentiability of G(z)]\label{lem:differentiability-G}
    Let $0 \le k_0 < k_1$, $T: W^{k_j, 1} \to W^{k_j, 1}$, be a linear map with $\norm{T}_{W^{k_j, 1} \to W^{k_j, 1}} \le M_j$, bounded by $M_j \ge 0$ for $j = 1, 2$ respectively. Further let $\theta \in [0, 1]$ and $k_\theta = (1-\theta) k_0 +  \theta k_1$ and $x \in \cT_f$, then the map
    \begin{align*}
        G: S &\coloneqq \{ z \in \C \::\; 0 \le \Re(z) \le 1\} \to \cT_{1, \operatorname{sa}} \\
...
\begin{lem}\label{lem:(n+1)-(n+1)-properties}
    ...
    \begin{equation*}
        \cI_{d, \varepsilon}: \cT_f \to \cT_f, \qquad \rho \mapsto \cI_{d, \varepsilon}(\rho) \coloneqq - \varepsilon\{(N + \1)^{4d}, \rho\} \, . 
    \end{equation*}
    For $\lambda \ge 0$, we have that $\lambda - \cI_{d, \varepsilon}:\cT_f \to \cT_f$ is bijective. While for all $k \ge 0$ and $\rho \in \cT_f$ one further has
        % \item  
        % \begin{equation}\label{eq:inequality-(n+1)2d-(n+1)4d}
        %     \varepsilon\norm{(N + \1)^{2d} \rho (N + \1)^{2d}}_{W^{k, 1}} \le \norm{\cI_{d, \varepsilon}(\rho)}_{W^{k, 1}} \, ;
        % \end{equation}
        \begin{align}
& \norm{(\lambda - \cI_{d, \varepsilon})^{-1} \rho}_{W^{k, 1} } \le \frac{1}{\lambda + 2 \varepsilon} \norm{\rho}_{W^{k, 1}}\,;\tag{1}\label{item:(1)}\\
       &     \norm{\cI_{d, \varepsilon} \circ (\lambda - \cI_{d, \varepsilon})^{-1}(\rho)}_{W^{k, 1}} \le {2}\norm{\rho}_{W^{k, 1}} \tag{2}\label{eq:inequality-resolvent-(n+1)4d}\, . 
        \end{align}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \rho = \sum\limits_{\text{finite}} \rho_{nm} \ketbra{n}{m} &\mapsto (\lambda - \cI_{d, \varepsilon})^{-1}(\rho) \coloneqq \sum\limits_{\text{finite}} \rho_{nm} \frac{1}{\varepsilon(n + 1)^{4d} + \varepsilon(m + 1)^{4d} + \lambda} \ketbra{n}{m} \, .
    \end{align*}
    or alternatively 
    \begin{align*}
        (\lambda - \cI_{d, \varepsilon})^{-1}(\rho) &= \int\limits_{0}^\infty e^{-(\varepsilon(N + \1)^{4d} + \lambda/2) s} \rho e^{-(\varepsilon(N + \1)^{4d} + \lambda/2) s} ds\\
        &=  \int\limits_{0}^\infty  \sum\limits_{\text{finite}} e^{-(\varepsilon(n + \1)^{4d} + \lambda/2)s} \rho_{nm} e^{-(\varepsilon(m + \1)^{4d} + \lambda/2)s} \ketbra{n}{m} ds \, .
    \end{align*}
    The integral representation allows us to deduce that $(\lambda - \cI_{d, \varepsilon})^{-1}$ preserves positivity. Using the first expression it is straightforward to show that this map is indeed the inverse to $\lambda - \cI_{d, \varepsilon}:\cT_f \to \cT_f$. The bound $\norm{(\lambda - \cI_{d, \varepsilon})^{-1} \rho}_{W^{k, 1} } \le \frac{1}{\lambda + 2 \varepsilon} \norm{\rho}_{W^{k, 1}}$ can be shown, using the integral representation and Hölder inequality:
    \begin{align*}
        \norm{(\lambda - \cI_{d, \varepsilon})^{-1}(\rho)}_{W^{k, 1}} &\le \int\limits_{0}^\infty \norm{e^{-\varepsilon(N + \1)^{4d} + \lambda/2)s} (N + \1)^{k/4} \rho (N + \1)^{k/4} e^{-\varepsilon(N + \1)^{4d} + \lambda/2)s}}_1\\
        &\le \int\limits_{0}^\infty \norm{e^{-\varepsilon(N + \1)^{4d} + \lambda/2)s}}_\infty^2 ds \norm{\rho}_{W^{k, 1}}\\
        &= \int\limits_{0}^\infty e^{-(2\varepsilon + \lambda)s} \norm{\rho}_{W^{k, 1}} = \frac{1}{\lambda + 2\varepsilon} \norm{\rho}_{W^{k, 1}} \, .
    \end{align*}
    % For \Cref{eq:inequality-(n+1)2d-(n+1)4d} let first $\rho \ge 0$, $\rho \in \cT_f$, then we find
    % \begin{align*}
    %     \norm{(N + \1)^{2d} \rho (N + \1)^{2d}}_{W^{k, 1}} &= \tr[\rho (N + \1)^{4d + k/2}] \\
    %     &\le \tr[\rho (N + \1)^{4d + k/2}] + \tr[(N + \1)^{4d + k/2}\rho ] \\
    %     &= \tr[(N + \1)^{k/4}\{(N + \1)^{4d}, \rho\} (N + \1)^{k/4}] \\
    %     &\le \norm{\cI_{d, 1}(\rho)}_{W^{k, 1}}
    % \end{align*}
    % For general $\rho \in \cT_f$ we can decompose $\sigma =  (N + \1)^{k/4} (-\cI_{d, 1}(\rho)) (N + \1)^{k/4} \in \cT_f$ into $\sigma = \sigma_+ - \sigma_-$ into its positive and negative part respectively and then define $\rho_\pm \coloneqq (-\cI_{d, 1})^{-1} \{(N + \1)^{-k/4} \sigma_\pm (N + \1)^{-k/4}\} \in \cT_f$. Since $(N + \1)^{-k/4} \cdot (N + \1)^{-k/4}$ and $(-\cI_{d, 1})^{-1}$ are both positivitiy preserving, we get that $\rho_\pm \ge 0$ and clearly $\rho = \rho_+ - \rho_-$. Hence with the above result and triangle inequality 
    % \begin{align*}
    %     \norm{(N + \1)^{2d} \rho (N + \1)^{2d}}_{W^{k, 1}} &\le \norm{(N + \1)^{2d} \rho_+ (N + \1)^{2d}}_{W^{k, 1}} +   \norm{(N + \1)^{2d} \rho_- (N + \1)^{2d}}_{W^{k, 1}}\\
    %     &\le \norm{\cI_{d, 1}(\rho_+)}_{W^{k, 1}} + \norm{\cI_{d, 1}(\rho_-)}_{W^{k, 1}} \\
    %     &= \norm{\sigma_+}_1  + \norm{\sigma_-}_1 = \norm{\sigma_+ - \sigma_-}_1 = \norm{\cI_{d, 1}(\rho)}_{W^{k, 1}} \, . 
    % \end{align*}
    % Multiplying both sides with $\varepsilon > 0$ then gives \Cref{eq:inequality-(n+1)2d-(n+1)4d}. 
    Issues with arising from the unbounded nature of $N$ can be ignored in the above estimations, as we can take a finite cut-off in the Fock basis due to $\rho \in \cT_f$. For \eqref{eq:inequality-resolvent-(n+1)4d}, we have that on $\cT_f$, $-\cI_{d, \varepsilon}\circ (\lambda - \cI_{d, \varepsilon})^{-1} = \1 - \lambda (\lambda - \cI_{d, \varepsilon})^{-1}$, i.e. for $\rho \in \cT_f$
    \begin{equation}
        \norm{(\1 - \lambda (\lambda - \cI_{d, \varepsilon})^{-1})\rho}_{W^{k, 1}} = \norm{-\cI_{d, \varepsilon}\circ (\lambda - \cI_{d, \varepsilon})^{-1} (\rho)}_{W^{k, 1}}
    \end{equation}
    where the LHS can be upper bounded by $(1 + \frac{\lambda}{\lambda + 2\varepsilon})\norm{\rho}_{W^{k, 1}} \le 2 \norm{\rho}_{W^{k, 1}}$ using \eqref{item:(1)}. This proves the last claim.
...
\begin{lem}\label{lem:boundedness-polynomials}
    ...
    \end{equation*}
    we get that for all $k \ge 0$ and $d' \ge d$
    \begin{align*}
...
\begin{proof}
    ...
    \end{equation*}
    are bounded, and hence can be uniquely extended to a bounded map on $\cH$. We only give the argument for the first map, since it is almost completely analogous for the second one. Let $\ket\psi = \sum\limits_{n = 0}^M\psi_n \ket{n} \in \cH_f$, then
    \begin{align*}
...
\begin{lem}\label{lem:infinitesimal-boundedness-W-k-1}
    ...
    \begin{equation*}
        \cA: \cT_f \to \cT_f, \qquad \rho \mapsto \cA(\rho) = \sum\limits_{i = 1}^KA_{i, 1}\rho A_{i, 2} = \sum\limits_{i = 1}^K p_{i, 1}(a, a^\dagger) \rho \, p_{i, 2}(a, a^\dagger)
    \end{equation*}
    where the action of $p_{1/2}(a, a^\dagger)$ on $\rho$ is defined via the action of $a$ and $a^\dagger$ on $\ketbra{n}{m}$. We then have that for all $k \ge 0$, $d \ge \max\limits_{i = 1, \hdots, K}\max\{d_{i, 1}, d_{i, 2}\}$ there exists $C_k \ge 0$, s.t. for all $\varepsilon \ge 0$ and $\forall \rho \in \cT_f$
 
    \begin{equation*}
        \norm{\cA(\rho)}_{W^{k, 1}} \le \varepsilon\norm{\{(N + \1)^{4d}, \rho\}}_{W^{k, 1}} + \frac{C_k}{\varepsilon} \norm{\rho}_{W^{k, 1}} \, . 
    \end{equation*}
\end{lem}
\begin{proof}
    Let $k \ge 0$. The first step is to show that there exists $c_k \ge 0$, s.t. for all $\rho \in \cT_f$
    \begin{equation}\label{eq:proof-first-stage-inequality}
        \norm{\cA(\rho)}_{W^{k, 1}} \le c_k \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}} \, . 
    \end{equation}
    The argument reduces to showing that for $i = 1, \hdots, K$ and $\rho \in \cT_f$
    \begin{equation*}\label{eq:proof-first-stage-inequality-substage}
        \norm{(N + \1)^{k/4} A_{i, 1} \rho A_{i, 2} (N + \1)^{k/4}}_1 \le c_{i, k} \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}}
    \end{equation*}
    since the sum comprising $\cA$ is finite. Note that the trace norm on the LHS is the one on the trace-class operators since the argument might not necessarily be self-adjoint. For $\rho \in \cT_f$, we have
    \begin{align*}
        &\norm{(N + \1)^{k/4} A_{i, 1} \rho A_{i, 2} (N + \1)^{k/4}}_1\\
        &\hspace{0.5cm} = \norm{(N + \1)^{k/4} A_{i, 1} (N + \1)^{-k/4 - d} (N + \1)^{k/4 + d}\rho  (N + \1)^{k/4 + d}  (N + \1)^{-k/4 - d} A_{i, 2} (N + \1)^{k/4}}_1\\
        &\hspace{0.5cm}\le \norm{(N + \1)^{k/4} A_{i, 1} (N + \1)^{-k/4 - d}}_\infty \norm{\rho}_{W^{k+4d, 1}} \norm{(N + \1)^{-k/4 - d} A_{i, 2} (N + \1)^{k/4}}_\infty\\
        &\hspace{0.5cm}\le c_{i, k} \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}} \, 
    \end{align*}
    where we used \Cref{lem:boundedness-polynomials} to argue that the operators involved are bounded and we can employ Hölder's inequality to split them off. Subsequently, we replaced the operator norms with the constant $c_{i, k}$. Now in the second step we show that for all $\varepsilon > 0$ and $\rho \in \cT_f$
    \begin{equation*}
        \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}} \le \varepsilon \norm{\{(N + \1)^{4d}, \rho\}}_{W^{k, 1}} + \frac{1}{4\varepsilon} \norm{\rho}_{W^{k, 1}} \, . 
    \end{equation*}
    Combining this with \Cref{eq:proof-first-stage-inequality} then immediately provides the claim.
    Therefore, let $\lambda > 0$ and $\rho \in \cT_f$ with $\rho \ge 0$. We find
    \begin{align*}
        &\norm{(N + 1)^d(\lambda - \cI_{d, 1})^{-1}(\rho) (N + \1)^d}_{W^{k, 1}}\\
        &\hspace{2cm} = \tr[(\lambda - \cI_{d, 1})^{-1}\{(N + \1)^{k/4 + d} \rho (N + \1)^{k/4 + d}\}]\\
        &\hspace{2cm} = \int\limits_{0}^\infty \tr[e^{-(2(N + \1)^{4d} + \lambda) s}(N + \1)^{2d} (N + \1)^{k/4} \rho (N + \1)^{k/4}] ds\\
        &\hspace{2cm} = \tr\Big[\int\limits_{0}^\infty e^{-(2(N + \1)^{4d} + \lambda) s}(N + \1)^{2d} ds\,  (N + \1)^{k/4} \rho (N + \1)^{k/4}\Big]\\
        &\hspace{2cm} = \tr[\frac{(N + \1)^{2d}}{(N + \1)^{4d} + \lambda}(N + \1)^{k/4} \rho (N + \1)^{k/4}] \, ,
    \end{align*}
    where we used the map $\cI_{d, 1}$ from \Cref{lem:(n+1)-(n+1)-properties}, the integral representation of its resolvent $(\lambda - \cI_{d, 1})^{-1}$ and that this resolvent preserves positivity. Further, we applied the cyclicity of the trace and conveniently suppressed issues that might arise from the unbounded nature of $N$ by taking a cut-off in the Fock basis. This is justified by $\rho \in \cT_f$. Now we can use that $(N + \1)^{k/4} \rho (N + \1)^{k/4} \ge 0$ to bound the RHS of the above chain of inequalities to get
    \begin{align*}
        \norm{(N + 1)^d(\lambda - \cI_{d, 1})^{-1}(\rho) (N + \1)^d}_{W^{k, 1}} &\le \sup\limits_{x,y \ge 1} \frac{x}{x^2 + \lambda} \tr[(N + \1)^{k/4} \rho (N + \1)^{k/4}]\\
        &= \sup\limits_{x,y \ge 1} \frac{x}{x^2 + \lambda} \norm{\rho}_{W^{k, 1}} \, .
    \end{align*}
    Now for a general $\rho \in \cT_f$, we can set $\sigma = (N + \1)^{k/4} \rho (N + \1)^{k/4}$ decompose into $\sigma = \sigma_+ - \sigma_-$ the positive and negative part of $\sigma$ respectively and then set $\rho_\pm = (N + \1)^{-k/4} \sigma_\pm (N + \1)^{-k/4}$. We clearly have that $\rho_\pm \in \cT_f$, $\rho = \rho_+ - \rho_-$ and $\rho_\pm \ge 0$ as $(N + \1)^{-k/4} \cdot (N + \1)^{-k/4}$ preserves positivity. This allows us to apply what we have shown above to obtain
    \begin{align*}
        \norm{(N + 1)^d(\lambda - \cI_{d, 1})^{-1}(\rho) (N + \1)^d}_{W^{k, 1}} &\le \norm{(N + 1)^d(\lambda - \cI_{d, 1})^{-1}(\rho_+) (N + \1)^d}_{W^{k, 1}}\\
        &\hspace{2cm} + \norm{(N + 1)^d(\lambda - \cI_{d, 1})^{-1}(\rho_-) (N + \1)^d}_{W^{k, 1}}\\
        &\le \sup\limits_{x \ge 1} \frac{x}{x^2 +\lambda}(\norm{\rho_+}_{W^{k, 1}} + \norm{\rho_-}_{W^{k, 1}})\\
        &= \sup\limits_{x \ge 1} \frac{x}{x^2 +\lambda} (\norm{\sigma_+}_1 + \norm{\sigma_-}_1)\\
        &= \sup\limits_{x \ge 1} \frac{x}{x^2 +\lambda} \norm{\sigma}_1\\
        &= \sup\limits_{x \ge 1} \frac{x}{x^2 + \lambda} \norm{\rho}_{W^{k, 1}}
    \end{align*}
    ...
    \begin{equation}
        \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}} \le \sup\limits \frac{x}{x^2 + \lambda} \norm{\cI_{d, 1}(\rho)}_{W^{k, 1}} + \lambda \sup\limits \frac{x}{x^2 + \lambda} \norm{\rho}_{W^{k, 1}}\,.
    \end{equation}
    Choosing $\lambda = \frac{1}{4\varepsilon^2}$, we find that $\sup\limits_{x \ge 1} \frac{x}{x^2 + \lambda} < \varepsilon$, hence
    \begin{equation}
        \norm{(N + \1)^d \rho (N + \1)^d}_{W^{k, 1}} \le \varepsilon \norm{\{(N +\1)^{4d}, \rho\}}_{W^{k, 1}} + \frac{1}{4\varepsilon} \norm{\rho}_{W^{k, 1}}
    \end{equation}
...
\begin{lem}[Interpolation Lemma]\label{lem:interpolation-lemma}
    Let $0 \le k_0 < k_1$ and $(\cL, \cD(\cL))$ an operator on $W^{k_j, 1}$, $j = 0, 1$ respectively. Further, assume that the closure of $(\cL, \cD(\cL))$ defines a strongly continuous semigroup $(\cP_t^j)_{t \ge 0}$ with 
    \begin{equation}
...
\end{lem}
\begin{proof}
    In the following assume that w.l.o.g. $k_0 < k_1$. We begin with the second claim and only cover $k_j = k_0$ as the other case only requires minor changes that are left to the reader. Let $\theta \in (0, 1)$ and the closure of $(\cL, \cD(\cL))$ the generator of $(\cP_t^0)_{t \ge 0}$ and $(\cP_t^\theta)_{t \ge 0}$ on the respective spaces. Since $k_0 < k_\theta$ and hence $W^{k_\theta, 1} \Subset W^{k_0, 1}$, we have that the closure of $(\cL, \cD(\cL))$ in $W^{k_\theta, 1}$ agrees with the restriction of the closure in $W^{k_0, 1}$. As the semigroup is completely determined by its generator, we find that the semigroups agree on $W^{k_\theta, 1}$.\newline
    For the first claim, note that the semigroups $(\cP_t^j)_{t \ge 0}$, $j = 1, 2$ agree on $W^{k_1, 1}$ by \Cref{item:int-pol-2}, which allows us to employ the Stein-Weiss theorem for Bosonic Sobolev spaces (\Cref{thm:stein-weiss}) to conclude \Cref{eq:interpolation-semigroup-bound}. It remains to check that the families of bounded maps $(\cP_t^\theta)_{t \ge 0}$ are strongly continuous semigroups generated by the closure of $(\cL, \cD(\cL))$ on $W^{k_\theta, 1}$. We have that $\cP_0^\theta = \1$  and $\cP_t^\theta \cP_s^\theta = \cP_{t + 1}^\theta$ $\forall t, s \ge 0$ as a consequence of $\cP_t^0 |_{W^{k_\theta, 1}} = \cP_t^\theta$ $\forall t$ (this equality holds by \Cref{thm:stein-weiss}). The strong continuity follows, due to $\cP^1_t = \cP_t^\theta|_{W^{k_1, 1}}$, as for $\rho \in W^{k_1, 1}$
    \begin{equation*}
        \lim\limits_{t \to 0} \norm{\cP_t^\theta(\rho)  - \rho}_{W^{k_\theta, 1}} = \lim\limits_{t \to 0} \norm{\cP^1_t(\rho) - \rho}_{W^{k_\theta, 1}} \le \lim\limits_{t \to 0} \norm{\cP^1_t(\rho) - \rho}_{W^{k_1, 1}} = 0
    \end{equation*}
    where we used $W^{k_1, 1} \Subset W^{k_\theta, 1}$ and that $\cP^1_t$ is a strongly continous semigroup on $W^{k_1, 1}$. For general $\rho \in W^{k_\theta, 1}$, we find $(\rho_n)_{n \in \N} \subset W^{k_\theta, 1}$ converging to $\rho$ in $W^{k_\theta, 1}$ and for all $n \in \N$
    \begin{align*}
         \lim\limits_{t \to 0} \norm{\cP_t^\theta(\rho)  - \rho}_{W^{k_\theta, 1}} &\le \lim\limits_{t \to 0}\left[ (1 + M_0^{1 - \theta} M_1^\theta e^{(\omega_{k_0}(1 - \theta) + \omega_{k_1} \theta) t}) \norm{\rho - \rho_n}_{W^{k_\theta, 1}} + \norm{\cP_t^\theta(\rho_n)  - \rho_n}\right]\\
...
\begin{lem}[Approximation Lemma]\label{lem:approximation-lemma}
    ...
    \begin{equation}
       \cA_n :\cT_f \to \cT_f, \quad \rho \mapsto \cA_n(\rho) \coloneqq \sum\limits_{i = 1}^K a_{i, n} A_{i, 1} \,\rho\, A_{i, 2} \coloneqq \sum\limits_{i = 1}^K a_{i, n} p_{i, 1}(a, a^\dagger) \,\rho\, p_{i, 2}(a, a^\dagger) \, . 
    \end{equation}
    If for all $k \ge 0$ there exists $M_k, \omega_k$ such that for all $n \in \N$ the closure of $(\cA_n, \cD(\cA_n))$ generates a strongly continuous semigroup $(\cP_t^n)_{t \ge 0}$ on $W^{k, 1}$ with
...
\end{lem}
\begin{proof}
    Let $k \in \N$. To prove the lemma, we first note that $(\cA, \cD(\cA))$ is densely defined and the pointwise limit of $\{(\cA_n, \cD(\cA_n))\}_{n \in \N}$. To employ the second Trotter-Kato approximation theorem, which implies the claim (see the version in \cite[Thm.~III.4.9]{Engel.2000}), we simply need to show that there exists $\lambda > 0$ such that $(\lambda - \cA, \cD(\cA))$ has dense range in $W^{k, 1}$. We will do so by showing that the closure of the range contains $\cT_f$ which is a dense subset of $W^{k, 1}$. Therefore let $\lambda > \max\{\omega_k, \omega_{k + 4d}\}$ (with $\omega_\cdot$ from \Cref{eq:uniform-bound-semigroups} and $d$ the maximal degree of the polynomials but at least one, i.e.~$d = \max\limits_{i = 1, \hdots, K} \max\{d_{i, 1}, d_{i, 2}, 1\}$). Let $\xi \in \cT_f$ arbitrary, we then have that for all $n \in \N$ the operator $(\lambda - \cA_n, \cD(\cA_n))$ has dense range in $W^{k + 4d, 1}$, meaning in particular we find a sequence $\{\rho_{n, m}\}_{m \in \N}$ which is convergent in $W^{k + 4d, 1}$ and further
    \begin{equation}
        \lim\limits_{m \to \infty} \norm{(\lambda - \cA_n)(\rho_{n, m}) - \xi}_{W^{k + 4d, 1}}  = 0 \, .
    \end{equation}
    In addition, we can choose the sequence such that for all $m \in \N$, $\norm{(\lambda - \cA_n)(\rho_{n, m})}_{W^{k + 4d, 1}} \le \norm{\xi}_{W^{k + 4d,1}} + 1$. Due to the $\omega_{k + 4d}$-quasi dissipativity of $\cA_n$ (it is a generator of a strongly continuous semigroup with a bound given in \Cref{eq:uniform-bound-semigroups}) this immediately implies $\norm{\rho_{n, m}}_{W^{k + 4d, 1}} \le M_{k + 4d}\frac{\norm{\xi}_{W^{k + 4d, 1}} + 1}{\lambda - \omega_{k + 4d}} =: c_{\xi}$, i.e.~the set $\{\rho_{n, m} \;:\; n, m \in \N\}$ is bounded in $W^{k + 4d, 1}$. We now have that for $n, m \in \N$
    \begin{equation}\label{eq:dense-range-argument-inequality}
        \begin{aligned}
            \norm{(\lambda - \cA)(\rho_{n, m}) - \xi}_{W^{k, 1}} &\le \norm{(\lambda - \cA_n)(\rho_{n, m}) - \xi}_{W^{k, 1}} + \norm{(\cA - \cA_n)(\rho_{n, m})}_{W^{k, 1}}\\
            &\le \norm{(\lambda - \cA_n)(\rho_{n, m}) - \xi}_{W^{k, 1}} + \sum\limits_{i = 1}^K c_{i, k} |a_i - a_{i, n}|  \norm{\rho_{n, m}}_{W^{k + 4d, 1}}\\
            &\le \norm{(\lambda - \cA_n)(\rho_{n, m}) - \xi}_{W^{k, 1}} + \sum\limits_{i = 1}^K |a_i - a_{i, n}| c_{i, k} c_\xi\\
            &\le  \norm{(\lambda - \cA_n)(\rho_{n, m}) - \xi}_{W^{k, 1}} + C\sum\limits_{i = 1}^K |a_i - a_{i, n}| \, .
        \end{aligned}
    \end{equation}
 ...
 \begin{align*}
    \norm{(N + \1)^{k/4}A_{i, 1} \rho_{n, m} A_{i, 2} (N + \1)^{k/4}}_1\le c_{k, i} \norm{(N + \1)^d \rho_{n, m} (N + \1)^d}_{W^{k, 1}}
    \end{align*}
    as in the proof of \Cref{lem:infinitesimal-boundedness-W-k-1}. Lastly we used the uniform bound $c_\xi$ and set $C = \max_{i = 1, \hdots, K} c_{i,k} c_\xi$. By a proper choice of a subsequence of $\{\rho_{n, m}\}_{n, m \in \N}$, we get that the RHS of \Cref{eq:dense-range-argument-inequality} vanishes. Since $\{\rho_{n, m} \;:\; n, m \in \N\}$ is bounded in $W^{k + 4d, 1}$ it is in particular precompact in $W^{k, 1}$ (as of the compact embedding of the Sobolev spaces), meaning we can further choose the aforementioned sequence to be convergent in $W^{k, 1}$. Let us call it $\{\sigma_n\}_{n \in \N} \subset \cT_f$. To summarise, for the chosen $\lambda$ and $\xi \in \cT_f$ arbitrary we have constructed a sequence $\{\sigma_n\}_{n \in \N} \subset \cD(\cA)$ which is convergent in $W^{k, 1}$ and further $\{(\lambda - \cA)(\sigma_n)\}_{n \in \N}$ converges to $\xi$ in $W^{k, 1}$. Hence the closure of the range of $(\lambda - \cA, \cD(\cA))$ contains $\cT_f$ a dense subset of $W^{k, 1}$, which concludes the proof. 
\end{proof}
\end{document}