...
A (total, deterministic) \emph{macro tree transducer with look-ahead} (``mttr'') 
...
We us a notation that is slightly different from the one used in the Introduction:
...
Since $M$ is total and $\Rightarrow_M$ is confluent and terminating,
...
We define \emph{the extension $\hat{M}$ of $M$} which can also process
look-ahead states at leaves of input trees.
Let $\hat{M}=(Q,P,\hat{\Sigma},\hat{\Delta},q_0,R\cup \hat{R},h)$ where
$\hat{\Sigma}=\Sigma\cup \{p^{(0)}\mid p\in P\}$ and
$\hat{\Delta}=\Delta\cup \{\< q,p>^{(m)} \mid q \in Q^{(m)}, p \in P, m\geq 0\}$.
For every $q\in Q$ and $p\in P$ we let the rule
...
\emph{reachable} if there is an input tree $s\in T_{\hat{\Sigma}}$ such
that $\<q,p>$ occurs in $\hat{M}(s)$.
...
$\text{rhs}_M(q,\sigma,\< p_1,\dots,p_k>)$.
The next proposition if proved in Lemma~6.7 of~\cite{DBLP:journals/iandc/EngelfrietM99}
(for mttrs that do not copy parameters, but the proof works analogous for
arbitrary mtts).
...
\begin{proposition}(Theorem~6.2 of~\cite{DBLP:journals/iandc/DrewesE98})
\label{prop:finite}
