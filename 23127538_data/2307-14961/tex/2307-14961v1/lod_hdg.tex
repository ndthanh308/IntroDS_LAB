...
% \newcommand{\dt}{\ensuremath{\, \textup dt}}
% \newcommand{\dy}{\ensuremath{\, \textup dy}}
...
\newcommand{\tri}{\mathcal{T}}%{\mathcal{Q}}%
\newcommand{\id}{\mathbb{1}}%{\mathtt{id}}
\newcommand{\idm}{I}
\newcommand{\opid}{\mathtt{id}}
...
\def\calT{\mathcal{T}}
\def\VHp{V_H^p}
\def\Vhp{V_{h,p^\prime}}
\def\tVHp{\tilde{V}_H^p}
\def\tVHpl{\tilde{V}_H^{p,\ell}}
\def\tVHhp{\tilde{V}_{H,h}^p}
\def\tVHhpl{\tilde{V}_{H,h}^{p,\ell}}
...
\newcommand{\Vho}{V_h^0}
\newcommand{\VH}{V_H}
\newcommand{\VpH}{\bar V_H}
\newcommand{\VHG}{V_{\Gamma,H}}
\newcommand{\Vh}{V_h}
\newcommand{\Vph}{\bar V_h}
\newcommand{\VC}{\tilde{V}_H}
\newcommand{\QC}{\tilde{Q}_H}
\newcommand{\VhC}{\tilde{V}_{H,h}}
\newcommand{\VhCl}{\tilde{V}_{H,h}^\ell}
\newcommand{\RVH}{\calR\VH}
\newcommand{\RsVH}{\calR^*\VH}

\newcommand{\RlVH}{\calR^{\ell}\VH}
\newcommand{\RlsVH}{\calR^{*,\ell}\VH}
\newcommand{\RhVH}{\calR_h\VH}
\newcommand{\RhsVH}{\calR_h^*\VH}
\newcommand{\RhlVH}{\calR_h^\ell\VH}
\newcommand{\RhlsVH}{\calR_h^{*,\ell}\VH}

\newcommand{\W}{\calW}
\newcommand{\Wh}{W_h}
\newcommand{\V}{\mathcal{V}}
\newcommand\Q{\mathcal{Q}} 
\newcommand{\Ve}{H^1_0(D)} % explicit choice
\newcommand{\Vp}{\mathcal{\bar V}}%^+}
\newcommand{\Vs}{\mathcal{V}^*}
\newcommand{\HV}{\mathcal{H}}

\newcommand{\Hv}{{\mathcal{H}_{\V}}}
...
\newcommand{\IH}{\calI_H}
\newcommand{\Ih}{\calI_h}
\newcommand{\IhK}{\calI_h^K}
\newcommand{\PipH}{\Pi_H^p}
\newcommand{\Nb}{\mathsf{N}}
\newcommand{\WlK}{\W_{\ell,K}}
\newcommand{\WlKs}[1]{\W_{#1,K}}
\newcommand{\WlKc}{\W^\mathrm{c}_{\ell,K}} % its complement
\newcommand{\WlKcc}{\W^\mathrm{c}_{\ell+1,K}} % its complement

\newcommand{\vC}{\tilde{v}_H}
\newcommand{\wC}{\tilde{w}_H}
\newcommand{\qC}{\tilde{q}_H}
\newcommand{\uC}{\tilde{u}_H}
\newcommand{\zC}{\tilde{z}_H}

\newcommand{\vhC}{\tilde{v}_{H,h}}
\newcommand{\uhC}{\tilde{u}_{H,h}}
\newcommand{\uhH}{u_{H,h}}
\newcommand{\uo}{u_0}
\newcommand{\uHo}{u_{H,0}}
...
\newcommand{\vB}{\bar{v}_H}
\newcommand{\uB}{\bar{u}_H}
\newcommand{\uH}{u_H}
\newcommand{\vH}{v_H}
\newcommand{\vK}{v_K}
\newcommand{\vT}{v_T}
...
\newcommand{\lamKl}{\lambda^\ell_K}
\newcommand{\lamHs}{\lambda_H^*}
\newcommand{\lamvH}{\lambda_{\vH}}
\newcommand{\lamvHs}{\lambda_{\vH}^*}
\newcommand{\muH}{\mu_H}
\newcommand{\wH}{w_H}
\newcommand{\qH}{q_H}
\newcommand{\vh}{v_h}
...
% \newcommand\RM[1]{{\color{blue}RM: #1}} 
...
\definecolor{rmcol}{rgb}{0.85000,0.32500,0.09800}
...
\address{School of Engineering Science, Lappeenranta--Lahti University of Technology, P.O. Box 20, 53851 Lappeenranta, Finland}
\email{andreas.rupp@fau.de}

\thanks{A.\ Rupp has been supported by the Academy of Finland's grant number 350101 \emph{Mathematical models and numerical methods for water management in soils}.}
...
\section{Problem formulation and numerical methods}\label{s:probform}
% 
...
\end{equation}
% 
where $\fine < \eps < \coarse$, and we again assume that the corresponding meshes are refinements of each other. In particular, $A$ is also element-wise constant on the mesh $\mesh_\fine$. In the present setting, $\eps$ is relatively small indicating that $A$ contains heavy oscillations. Therefore, $\mesh_\fine$ possibly includes many elements such that a direct global finite element computation for it is unfeasible. That is why one would like to conduct numerical simulations concerning the coarser mesh $\mesh_\coarse$. Note that throughout this work, we denote mesh elements of $\mesh_\fine$ and $\mesh_\coarse$ by $\elem$. If we want to highlight that an element is in the fine mesh $\mesh_\fine$, we might write $e$ instead.
% 
% ----------------------------------------------------------------------------------------------------
\subsection{HDG methods}\label{ss:hdg}
% ----------------------------------------------------------------------------------------------------
% 
...
\end{equation*}
% 
and they are defined element-wise. For a given element $E\in\mesh_\fine$, they map the skeletal unknown $m$ to $u_\elem$ and $\vec q_\elem$ satisfying
% 
\begin{subequations}\label{EQ:hdg_scheme}
\begin{align}
 \int_\elem A^{-1} \vec q_\elem \cdot \vec p_\elem \dx - \int_\elem u_\elem \ddiv \vec p_\elem \dx
 & = - \int_{\partial \elem} m \vec p_\elem \cdot \Nu \ds\label{EQ:hdg_primary} \\
 \int_{\partial \elem} ( \vec q_\elem \cdot \Nu + \tau u_\elem ) v_\elem \ds - \int_\elem \vec q_\elem \cdot \nabla v_\elem \dx
 & = \tau \int_{\partial \elem} m v_\elem \ds \label{EQ:hdg_flux}
\end{align}
\end{subequations}
% 
for all $v_\elem \in V_\elem$, $\vec p_\elem \in \vec W_\elem$ and some $\tau \ge 0$ (which will be set later). In this sense $\localU \colon m \mapsto u_\elem$, $\localQ: m \mapsto \vec q_\elem$ for all $\elem \in \mesh_\fine$. Importantly, \eqref{EQ:hdg_flux} can be rewritten as
\begin{equation}\label{EQ:get_penalty}
 \int_{\partial \elem} \vec q_\elem \cdot \Nu v_\elem \ds - \int_\elem \vec q_\elem \cdot \nabla v_\elem \dx = - \underbrace{ \int_{\partial \elem} \tau (u_\elem - m) v_\elem \ds }_{ = s_\elem(m, v) }
\end{equation}
%
...
\begin{equation}\label{EQ:bilinear}
 a(m,\mu) = \int_\Omega A^{-1} \localQ m \cdot \localQ \mu \dx + {\underbrace {\sum_{\elem \in \mesh_\fine} \tau \int_{\partial \elem} (\localU m - m) (\localU \mu - \mu) \ds }_{ = s(m,\mu) }}
\end{equation}
% 
...
% 
\begin{itemize}[leftmargin=*]
 \item For the \emph{hybridized local discontinuous Galerkin} (\textbf{LDG-H}) method, we choose all local spaces to consist of polynomials of degree at most $p$, i.e.,  $M_\face = \mathbb P_p(\face)$, $V_\elem = \mathbb P_p(\elem)$, and $\vec W_\elem = \mathbb P^d_p(\elem)$. In our work, the parameter $\tau > 0$ is chosen as a face-wise constant such that $\tau h \lesssim 1$. Common choices for such $\tau$ are $\tau = \tfrac1h$ and $\tau = 1$.
...
\end{itemize}
% 
Note that we write $b \lesssim c$ if there exists a constant $C > 0$ (that might depend on the dimension and the upper and lower bounds of $A$) such that $b \leq C\, c$.  
 
\section{Preliminaries on HDG methods}\label{s:prelim}
% 
...
\end{equation}
% 
with $\star \in \{\fine,\coarse\}$ as above. This scalar product readily extends to the case in which $\mu \in V_\star$ instead of $\mu \in \skeletalSpace_\star$ and is commensurate to the $L^2$-scalar product in the bulk if $m,\mu \in \linElementSpace_\star$. Further, we define the mesh-dependent norm $\|\cdot\|_\star = \sqrt{\langle \cdot,\cdot \rangle_\star}$ and denote with $\| \cdot \|_0$ the standard $L^2$-norm for bulk functions.	Moreover, we define for any union of (either coarse or fine) elements $\omega \subset \Omega$, the local norm 
% 
\begin{equation*}
...
% 
\begin{equation}\label{EQ:vertex_patch}
 \omega_\elem = \operatorname{int}\left\{ x \in \Omega \mid x \in \elem' \in \mesh_\star \text{ with } \overline \elem' \cap \overline \elem \neq \emptyset \right\}.
\end{equation}
% 
We also write $\Nb^1(E):= \omega_E$ and iteratively define for $\ell \in \N,\,\ell \geq 2$ the $\ell$th-order patch by $\Nb^\ell(E) := \Nb^1(\Nb^{\ell-1}(E))$.
%  
...
\begin{lemma}\label{LEM:rewrite_prod}
 ...
 \begin{equation*}
  \int_\elem A^{-1} \localQ \xi \cdot \localQ \rho \dx = - \int_{\partial \elem} (\localQ \rho \cdot \Nu) \, \xi + \tau (\localU \rho - \rho) \,\localU \xi \ds
 \end{equation*}
 for any $\elem \in \mesh_\fine$.
\end{lemma}
 % 
\begin{proof}
 Given $\elem \in \mesh_\fine$, we have that
 \begin{align*}
  \int_\elem A^{-1} \localQ \xi \cdot \localQ \rho \dx 
  & = \int_\elem \localU \xi (\ddiv \localQ \rho) \dx - \int_{\partial \elem} \xi\, (\localQ \rho \cdot \Nu) \ds \\
  & = - \int_\elem \nabla \localU \xi \cdot \localQ \rho \dx + \int_{\partial \elem} (\localU \xi - \xi) \localQ \rho \cdot \Nu \ds \\
  & = - \int_{\partial \elem} (\localQ \rho  \cdot \Nu)\, \xi \ds - s_\elem (\rho, \localU \xi),
 \end{align*}
 where the first equality is \eqref{EQ:hdg_primary} with $\vec p = \localQ \rho$ and the second one is integration by parts. The third equation combines the two terms containing $\localU \xi$ to the penalty term by exploiting \eqref{EQ:get_penalty} with $v_\elem = \localU \xi$.
\end{proof}

The local solvers of all HDG methods of Section~\ref{ss:hdg} also fulfill the following essential conditions for any $\mu \in \skeletalSpace_\fine$ and $\elem \in \mesh_\fine$. These properties are shown in~\cite{LuRK22a} and will be heavily used in the following sections.
% 
\begin{itemize}
 \item The trace of the bulk unknown approximates the skeletal unknown, i.e.,
 \begin{equation}
  \| \localU \mu - \mu \|_\fine \lesssim \fine \| \localQ \mu \|_0. \tag{LS1}\label{EQ:LS1}
 \end{equation}
 \item The operators $\localQ \mu$ and $\localU \mu$ are continuous. That is,
 \begin{equation}
  \| \localQ \mu \|_{0,\elem} \lesssim \fine^{-1} \| \mu \|_{\fine,\elem} \quad \text{ and } \quad \| \localU \mu \|_0 \lesssim \| \mu \|_\fine. \tag{LS2}\label{EQ:LS2}
 \end{equation}
 \item The dual approximation fulfills $\localQ \mu \sim -\nabla_\fine \localU \mu$. More precisely, 
 \begin{equation}
  \| \localQ \mu + A \nabla_\fine \localU \mu \|_0 \lesssim \fine^{-1} \| \localU \mu - \mu \|_\fine. \tag{LS3}\label{EQ:LS3}
 \end{equation}
 ...
 \begin{equation}
  C_1 \| \mu \|^2_\fine \le a(\mu,\mu) \le C_2 \fine^{-2} \| \mu \|^2_\fine. \tag{LS5}\label{EQ:LS6}
 \end{equation}
\end{itemize}
...
\begin{align}
 \injectionOp(\traceSpace_\coarse) &\subset \traceSpace_\fine, \label{EQ:injection_space}\\
 ...
 \| m_\coarse \|_{\coarse,\elem} &\eqsim \| \injectionOp m_\coarse \|_{\fine,\elem} && \text{ for } m_\coarse \in {\traceSpace_\coarse}. \label{EQ:norm_equiv_inj}
\end{align}
\end{subequations}
%
...
The linear interpolations can now be used in conjunction with the coarse element-wise $L^2$-projection $\Pi^{L^2}_\coarse$ onto $\discElementSpace_{\coarse}$ 
to define a projection via
% 
\begin{equation}\label{EQ:def_projectionOp}
 \projectionOp\colon \skeletalSpace_\fine \ni \mu \mapsto {\gamma_\coarse}[\averagingOp_\coarse \, \Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] \mu \in \traceSpace_\coarse \subset \skeletalSpace_\coarse.
\end{equation}
%
Clearly, $\injectionOp \projectionOp$ is a projection since both $\injectionOp$ and $\projectionOp$ are projections by construction. Further, the following result holds.

\begin{lemma}\label{lem:stab}
 We have for all $m \in \skeletalSpace_\fine$ and all $\elem \in \mesh_\coarse$ that
 \begin{align}
  \| (\injectionOp \projectionOp) m \|_{a} & \lesssim \| m \|_{a} , \label{eq:stab_ah}\\
  \| (\injectionOp \projectionOp) m \|_{\fine,\elem} &\lesssim \| m \|_{\fine, \elem} + \| m \|_{\coarse,\omega_\elem} + \coarse \| \localQ m \|_{0,\omega_\elem}. \label{eq:stab_h}
 \end{align}
...
\begin{proof}
 ...
 \begin{enumerate}
  ...
  \item For all $v \in V_\star, \; \star \in \{\fine,\coarse\}$ and $\elem \in \mesh_\star$, we have that
  \begin{equation}\label{EQ:Lu22-510a}
   \star \| \nabla_\star \averagingOp_\star v - \nabla_\star v \|_{0,\elem} \lesssim \| \averagingOp_\star v - v \|_{0,\elem} \lesssim \star \| \nabla_\star v \|_{0,\omega_\elem} + \| \jump{v} \|_{\star,\omega_\elem}.
  \end{equation}
  Again, the first inequality is a standard scaling argument, and the second inequality can be shown similarly to \cite[(5.10)]{LuRK22a}.
  \item For all $\mu \in \skeletalSpace_\fine$, we have that
  \begin{equation}\label{EQ:Lu22-52}
   \| \nabla_\fine (\averagingOp_\fine \, \localU) \mu \|_0 \lesssim \| \localQ \mu \|_0.
  \end{equation}
 \end{enumerate}
 ...
 %
 \begin{align*}
  \| (\injectionOp \projectionOp) m \|_{a} & \overset{\circlearound{A}}= \| A^{-1/2} \localQ (\injectionOp \projectionOp) m \|_0 \overset{\circlearound{B}}\eqsim \| \nabla [\averagingOp_\coarse \, \Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m \|_0 \\
  & \overset{\circlearound{C}}\lesssim \underbrace{ \| \nabla_H [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m \|_0 }_{ = \Xi_1 } + \coarse^{-1} \underbrace{ \| [\Pi^{L^2}_\coarse \averagingOp_\fine \,\localU] m - \localU m\|_\coarse}_{ = \Xi_2 } \\
  & \qquad + \underbrace{ \coarse^{-1} \| \localU m - m \|_\coarse }_{ = \Xi_3 }.
 \end{align*}
 %
 Here \circlearound{A} uses $(\injectionOp \projectionOp) m \in \traceSpace_\fine$ and the first equality in \eqref{EQ:LS4}, which imply that $\gamma_\fine \localU (\injectionOp \projectionOp) m = (\injectionOp \projectionOp) m$. Relation \circlearound{B} uses the second equality in \eqref{EQ:LS4} as well as~\eqref{eq:admissibleA} and~\eqref{EQ:a_const}. To obtain \circlearound{C}, we first use \eqref{EQ:Lu22-510} (for $\star = \coarse$) and the fact that
 \begin{equation*}
  \| \jump{ [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m } \|_\coarse \le 2 \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - m \|_\coarse,
 \end{equation*}
 ...
 %
 \begin{align*}
  \Xi_1 & \le \| \nabla_\coarse [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - \nabla_\fine [\averagingOp_\fine \, \localU] m \|_0 + \| \nabla_\fine [\averagingOp_\fine \, \localU] m \|_0 \\
  & \lesssim \| \nabla_\fine \averagingOp_\fine \, \localU m \|_0 \overset{\eqref{EQ:Lu22-52}}\lesssim \| \localQ m \|_0,
 \end{align*}
 %
 where the second inequality is \cite[Lem.\ 1.58]{PietroErn12} stating that on a single element $\elem \in \mesh_\coarse$, $\| \nabla (\Pi^{L^2}_\coarse v - v) \|_{0,E} \lesssim \| \nabla v \|_{0,E}$ for all $v \in H^1(\elem)$. Additionally, using the approximation properties of $\averagingOp_\fine$ (see~\eqref{EQ:Lu22-510}) and $\Pi^{L^2}_\coarse$ we estimate
 %
 \begin{align*}
  \Xi_2 & \lesssim \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - [\averagingOp_\fine \, \localU] m\|_0 + \| [\averagingOp_\fine \, \localU] m - \localU m\|_0 \\
  & \lesssim \coarse \| \nabla_\fine [\averagingOp_\fine \, \localU] m \|_0 + \fine \| \nabla_\fine \localU m \|_0 + \| \localU m - m \|_\fine,
 \end{align*}
 %
 which, in turn, can be bounded by $\coarse \| \localQ m \|_0$ if one applies \eqref{EQ:Lu22-52}, \eqref{EQ:LS3}, and \eqref{EQ:LS1}.
 %
 Last, we use the definitions of $\|\cdot\|_\fine$ and $\|\cdot\|_\coarse$ to write
 %
 \begin{equation*}
  \Xi_3 \le (\coarse\fine)^{-1/2} \; \| \localU m - m \|_\fine \overset{\eqref{EQ:LS1}}\lesssim \sqrt{\tfrac\fine\coarse} \; \| \localQ m \|_0 \overset{\fine < \coarse}\lesssim \| \localQ m \|_0.
 \end{equation*}
 %
 Altogether, this proves~\eqref{eq:stab_ah}.
 
 Next, we prove the inequality~\eqref{eq:stab_h}. 
 %
 Analogously to \circlearound{C}, we now employ~\eqref{EQ:Lu22-510a} and obtain
 % 
 \begin{align*}
  \| (\injectionOp & \projectionOp) m \|_{\fine,\elem} =  \| [\averagingOp_\coarse \, \Pi^{L^2}_\coarse \averagingOp_\fine \ \localU] m \|_{0,\elem} \\
  & \le \| [\averagingOp_\coarse \, \Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - [\Pi^{L^2}_\coarse \averagingOp_\fine \ \localU] m \|_{0,\elem} + \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m \|_{0,\elem} \\
  &\lesssim \coarse \| \nabla_\coarse [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m \|_{0,\omega_\elem} + \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - \localU m\|_{\coarse,\omega_\elem} \\
  &\qquad\qquad+ \| \localU m - m \|_{\coarse,\omega_\elem} + \| m \|_{\coarse,\omega_\elem} + \| [\Pi^{L^2}_\coarse \averagingOp_\fine \,\localU] m \|_{0,\elem},
 \end{align*}
 % 
 where we need to take into account that (i) the operator $\averagingOp_\coarse$ generates a dependency on the vertex patches $\omega_\elem$ and that (ii) the term $\| m \|_{\coarse,\omega_\elem}$ needs to be added since we do not necessarily have $m|_{\partial \elem} = 0$. The first three summands can be estimated as in the proof of \eqref{eq:stab_ah}, but they have an additional factor $\coarse$.
 %
 The triangle inequality bounds the last summand by
 % 
 \begin{equation*}
  \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m \|_{0,\elem} \le \underbrace{ \| [\Pi^{L^2}_\coarse \averagingOp_\fine \, \localU] m - \localU m\|_{0,\elem} }_{=\Psi_1} + \underbrace{ \| \localU m \|_{0,\elem} }_{=\Psi_2},
 \end{equation*}
 where $\Psi_1$ can be bounded analogously to $\Xi_2$, and $\Psi_2 \lesssim \| m \|_{\fine, \elem}$ by \eqref{EQ:LS2}.
\end{proof}
...
\begin{lemma}\label{LEM:uih_error}
  We have for all $m \in \skeletalSpace_\fine$ and all $\elem \in \mesh_\coarse$ that
  \begin{equation*}
   \| \localU m - \localU \injectionOp \projectionOp m \|_{0, \elem} \lesssim \| m - \injectionOp \projectionOp m \|_{h, \elem} \lesssim \coarse \| \localQ m \|_{0,\omega_\elem}.
  \end{equation*}
\end{lemma}
% 
...
\begin{subequations}
\begin{align}
 \mu & = \faceProj \gamma_\fine \liftingOp \mu \label{EQ:proj_prop},\\ %&& \text{ for all } \mu \in \skeletalSpace_\fine, \label{EQ:proj_prop}\\
 \| \liftingOp \mu \|_{0,\elem} & \eqsim \| \mu \|_{\fine, \elem} \label{EQ:s_norms}, \\ %&& \text{ for all } \mu \in \skeletalSpace_\fine, \; \elem \in \mesh_\coarse, \label{EQ:s_norms} \\
 \| \nabla \liftingOp \mu \|_{0,\elem} & \eqsim \| \localQ \mu \|_{0,\elem} \label{EQ:norm_equiv}. %&& \text{ for all } \mu \in \skeletalSpace_\fine, \; \elem \in \mesh_\coarse. \label{EQ:norm_equiv}
\end{align}
\end{subequations}
%
...
\begin{proof}[Proof of Lemma \ref{LEM:uih_error}]
  ...
  \end{equation*}
  Notably, the definition of $\projectionOp$ and \eqref{EQ:proj_prop} imply
  \begin{equation*}
  ...
  \end{equation*}
  %
  where the second equality follows from the property of $\localU$ in~\eqref{EQ:def_local_solver_l2} and the definition of $\projectionOp$ in~\eqref{EQ:def_projectionOp}. Next, we set $z = \liftingOp m\in H^1(\Omega)$ and consider a coarse element $\elem \in \mesh_\coarse$ with vertex patch $\omega_\elem$. We want to use the standard scaling argument in a non-standard way. That is, we want to prove that
  %
  ...
  %
  \begin{equation}\label{eq:proof_stab}
   \| \liftingOp \injectionOp \projectionOp \gamma_\fine  z \|_{0,\elem} \hspace{-.1cm}\overset{\eqref{EQ:s_norms}}\eqsim\hspace{-.1cm} \| \injectionOp \projectionOp \gamma_\fine  z \|_{\fine,\elem} \hspace{-.1cm}\overset{\eqref{eq:stab_h}}\lesssim\hspace{-.1cm} \| \gamma_\fine  z \|_{{\fine,\elem}} + \| \gamma_\fine  z \|_{{\coarse,\omega_\elem}} + \coarse \| \localQ \gamma_\fine  z \|_{0,{\omega_\elem}}
  \end{equation}
  %
  with $z = \hat z \circ \Phi$. For the third term on the right-hand side of~\eqref{eq:proof_stab}, we have that
  %
  \begin{align*}
   \| \localQ \gamma_\fine z \|_{0,\omega_\elem} \overset{\eqref{EQ:def_local_solver_l2}} = \| \localQ \faceProj \gamma_\fine z \|_{0,\omega_\elem} \overset{\eqref{EQ:proj_prop}}= \| \localQ m  \|_{0,\omega_\elem} 
   \overset{\eqref{EQ:norm_equiv}}\lesssim |S m |_{1,\omega_\elem} \lesssim H^{d/2-1}\| \hat z \|_{H^1(\hat \omega_{\hat \elem})}
  \end{align*}
  %
  with a standard scaling argument. For the second term on the right-hand side of~\eqref{eq:proof_stab}, we obtain
  %
  \begin{equation*}
    \| \gamma_\fine  z \|_{\coarse,\omega_\elem} \lesssim \coarse^{d/2} \| \hat \gamma_\fine \hat z \|_{L^2({\partial \hat \omega_{\hat \elem}})} \lesssim \coarse^{d/2} \| \hat z \|_{H^1(\hat \omega_{\hat \elem})}
  \end{equation*}
  %
  using once again a standard scaling argument and the continuity of the trace operator~$\gamma_\fine$. The first term can be bounded analogously.
\end{proof}
...
\end{equation*}
%
where $\projectionOp$ and $\injectionOp$ are the projection and injection operator from Section~\ref{SEC:mesh_transfer}, respectively. We define a so-called \emph{correction operator} $\corOp\colon \skeletalSpace_\fine \to \kerSpace$ as follows: for any $\xi \in \skeletalSpace_\fine$, the function $\corOp\xi \in \kerSpace$ solves 
%
...
Therefore, $\corOp\xi = \corOp\injectionOp\projectionOp\xi$ and~\eqref{eq:saddlepointCor}
equivalently reads
%
\begin{equation}\label{eq:saddlepointCor2}
\begin{aligned}
 a(\corOp \xi, \eta)\qquad\, &+& \langle\lambda_\coarse, \projectionOp\eta\rangle_\coarse \quad&=\quad a(\injectionOp\projectionOp\xi,\eta),\\
 \langle \projectionOp\corOp \xi,\mu_\coarse\rangle_\coarse\; && &=\quad 0.
\end{aligned}
\end{equation}
%
...
%
$
 (\idOp - \corOp)\xi = (\idOp - \corOp)\injectionOp\projectionOp\xi,
$
%
such that 
%
...
\end{equation*}
%
is generated by the coarse space $\traceSpace_\coarse$ only. In the following, we abbreviate $\msOp = (\idOp - \corOp)\injectionOp\colon \traceSpace_\coarse \to \skeletalSpace_{\fine}$. Note that, if $\{b_{\coarse,i}\}_i$ is a (nodal) basis of the coarse space~$\traceSpace_{\coarse}$, then $\{\tilde b_{\coarse,i}\}_i$ with
%
\begin{equation}\label{eq:msBasisFunctions}
 \tilde b_{\coarse,i} := \msOp b_{\coarse,i}
\end{equation}
%
defines a basis of $\msSpace$. For the practical localization of the correction (cf.~Section~\ref{ss:loc} below), an equivalent formulation based on so-called \emph{element corrections} is beneficial. Therefore, we decompose the correction operator in~\eqref{eq:saddlepointCor2} into element-wise contributions. Let $\xi_\coarse \in \traceSpace_\coarse$ and $\elem \in \mesh_\coarse$ a coarse mesh element. The pair $(\corOp_\elem \xi_\coarse, \lambda_{\coarse,\elem}) \in \skeletalSpace_{\fine} \times \traceSpace_{\coarse}$ solves
%
...
\begin{equation}\label{eq:stabTm}
	\|\tilde m_\coarse\|_{a} \lesssim \|f\|_0
\end{equation}
%
...
\begin{theorem}[Error of the prototypical method]\label{t:errIdeal}
 ...
 \begin{equation}\label{eq:errIdeal}
  \| \localU m - \localU \tilde m_\coarse \|_0 + \| \localQ m - \localQ \tilde m_\coarse \|_0 \lesssim \|m - \tilde m_\coarse\|_a \lesssim \coarse\, \|f\|_0.
 \end{equation}
\end{theorem}
% 
...
\begin{proof}[Proof of Theorem~\ref{t:errIdeal}]
 ...
 \begin{equation*}
  \| \localU m - \localU \tilde m_\coarse \|_0 = \| \localU (m - \tilde m_\coarse ) \|_0 \overset{\eqref{EQ:LS2}}\lesssim \| m - \tilde m_\coarse \|_\fine \overset{\eqref{EQ:LS6}}\lesssim \| m - \tilde m_\coarse \|_a
 \end{equation*}
 and the the definition of $\|\cdot\|_a$, i.e., 
 \begin{equation*}
  \| \localQ m - \localQ \tilde m_\coarse \|^2_0 = \| \localQ (m - \tilde m_\coarse) \|^2_0 = \int_\Omega [\localQ (m - \tilde m_\coarse)]^2 \dx \le \| m - \tilde m_\coarse \|^2_a.
 \end{equation*}
 ...
 \begin{align*}
  ...
  \lesssim \|f\|_0\, H\,\|m - \tilde m_\coarse \|_a,
 \end{align*}
 where we use that $m$ solves~\eqref{EQ:hdg_approx} to get the third equality. The fourth equality uses that $\injectionOp \projectionOp (m - \tilde m_\coarse) = 0$ (cf.~Remark~\ref{rem:zeroProj}), and the final inequality is Lemma~\ref{LEM:uih_error}. We obtain the assertion by dividing both sides of the inequality by $\| m - \tilde m_\coarse \|_a$.
\end{proof}
...
\begin{lemma}[Local pre-image]\label{lem:local_pre_image}
 ...
 \begin{equation*}
  \projectionOp\bubble = \projectionOp\xi\quad\text{ and } \quad \|\bubble\|_{a,\omega} \lesssim \|\xi\|_{a,\omega}.
 \end{equation*}  
\end{lemma}
...
\begin{proof}
 ...
 \begin{aligned}
  ...
  \langle \projectionOp\bubble,\mu_\coarse\rangle  && &=\quad \langle\projectionOp\xi, \mu_\coarse\rangle_{\coarse}
 \end{aligned}
 \end{equation}
 ...
 \begin{equation*}
  \| \injectionOp \mu_\coarse \|_a \eqsim \| \nabla v \|_0 =: (\bigstar).
 \end{equation*}
 Now, by the standard scaling argument on the coarse mesh, we have
 \begin{equation*}
  (\bigstar) \lesssim \coarse^{-1} \| v \|_0 \eqsim \coarse^{-1} \| \injectionOp \mu_\coarse \|_\fine,
 \end{equation*}
 where the second estimate follows from the equivalence of $\| \cdot \|_0$ and $\| \cdot \|_\fine$ on $\linElementSpace_\fine$. Going back to~\eqref{eq:proofInfSup}, we obtain the inf-sup condition~\eqref{eq:infsup} with $\gamma(\coarse) \sim \coarse > 0$.
 ...
 \begin{align*}
  \|\bubble\|_{a,\omega} \leq \frac{2\beta^{1/2}}{\alpha^{1/2}\gamma(\coarse)}\,\|\projectionOp\xi\|_{\coarse,\omega}
 \end{align*}
 holds. Using \eqref{EQ:norm_equiv_inj},	we obtain
 \begin{align*}
  \|\bubble\|_{a,\omega} %= \|\bubble\|_{a,\omega} 
  & \lesssim \coarse^{-1}\,\|\projectionOp\xi\|_{\coarse,\omega} \lesssim \coarse^{-1} \| \injectionOp \projectionOp \xi \|_{\fine,\omega} \\
  &\le H^{-1} \| \injectionOp \projectionOp \xi - \xi \|_{\fine,\omega} + H^{-1} \| \xi \|_{\fine,\omega}.
 \end{align*}
 ...
 \begin{equation*}
  \|\bubble\|_{a,\omega} \lesssim \|\localQ\xi\|_{0,\omega} + \|\xi\|_{a,\omega} \lesssim \|\xi\|_{a,\omega}.
 \end{equation*}
...
\begin{theorem}[Decay of element corrections]\label{t:decay}
 Let $\xi_{\coarse} \in \skeletalSpace_{\coarse}$ and $\elem \in \mesh_\coarse$ a coarse element. Further, let $\ell \in \N$. Then there exists $0 < \vartheta < 1$ such that
...
\begin{lemma}\label{LEM:q_bound}
 ...
 \begin{equation*}
   \sum_{\fineElem \in \mesh_{\elem'}} \int_{\fineElem} A^{-1} \localQ m\cdot \left[ \localQ(m\eta) -  \eta \localQ m \right] \dx \lesssim \| A^{-1/2} \localQ m \|^2_{0,\omega_{\elem'}}.
 \end{equation*}
...
\begin{proof}
 ...
 \begin{equation*}
  | \eqref{EQ:T1} | \lesssim h^{-1} \| \localQ m \|_{0,\fineElem} \| m \|_{\fine,\fineElem} \| \eta - \bar \eta \|_{L^\infty(\partial \fineElem)} \lesssim H^{-1} \| \localQ m \|_{0,\fineElem} \| m \|_{h,\fineElem},
 \end{equation*}
 ...
 \begin{align*}
  ...
  & \lesssim \tau_\mathrm{max} \| \localQ m \|_{0,\fineElem} \| \faceProj( m\eta ) - m\bar \eta \|_{h,\fineElem}  = \tau_\mathrm{max} \| \localQ m \|_{0,\fineElem} \| \faceProj( m\eta - m\bar \eta ) \|_{h,\fineElem} \\
  & \le \tau_\mathrm{max} \| \localQ m \|_{0,\fineElem} \| m\eta - m\bar \eta \|_{h,\fineElem} \lesssim \tau_\mathrm{max} \| \localQ m \|_{0,\fineElem} \| m \|_{h,\fineElem} \| \eta - \bar \eta \|_{L^\infty(\partial \fineElem)} \\
  & \lesssim \frac{\tau_\mathrm{max} \fine}{\coarse} \| \localQ m \|_{0,\fineElem} \| m \|_{h,\fineElem},
 \end{align*}
 where $\tau_\mathrm{max} = \max \{ \tau(x)\colon x \in \partial \fineElem \}$ and the second inequality uses~\eqref{EQ:def_local_solver_l2} and~\eqref{EQ:LS1}. If~$\tau_\mathrm{max} \fine \lesssim 1$, this implies that
 \begin{equation*}
  | \Xi_1 | \lesssim \| \localQ m \|^2_{0,\fineElem} + H^{-2} \| m \|^2_{h,\fineElem}.
 \end{equation*}
 We also have that
 \begin{equation*}
  | \Xi_2 | \le \| \localQ m \|^2_{0,\fineElem} \| \bar \eta - \eta \|_{L^\infty(\fineElem)} \lesssim \frac{\fine}{\coarse}\| \localQ m \|^2_{0,\fineElem}.
 \end{equation*}
 %
 Finally, since $\injectionOp \projectionOp m = 0$ we observe that with Lemma \ref{LEM:uih_error}
 \begin{equation*}
  \sum_{\fineElem \subset \elem'} \| m \|^2_{\fine,\fineElem} = \| m \|^2_{\fine,\elem'} = \| m -\injectionOp\projectionOp m \|^2_{\fine,\elem'} \lesssim H^2 \| \localQ m \|^2_{0,\omega_{\elem'}},
 \end{equation*}
...
\begin{lemma}\label{LEM:u_bound}
 ...
 \begin{equation*}
   \sum_{\fineElem \in \mesh_{\elem'}} \int_{\partial \fineElem} \tau (\localU m - m) \left[ \localU(m\eta) -  \eta \localU m \right] \ds \lesssim \| A^{-1/2} \localQ m \|^2_{0,\omega_{\elem'}}
  \end{equation*}
 for any $m \in \skeletalSpace_\fine$ with $(\injectionOp \projectionOp m)\vert_{\mesh_{\elem'}} = 0$.
\end{lemma}
% 
\begin{proof}
 ...
 \begin{align*}
  | \Psi_2 | & \lesssim \frac{\tau_\mathrm{max} h}{H} \| \localQ m \|_{0,\fineElem} \| \localU m \|_{h,\fineElem} \lesssim \frac{1}{H} \| \localQ m \|_{0,\fineElem} \| m \|_{h,\fineElem}.
 \end{align*}
 The second inequality follows directly from~\eqref{EQ:LS2} and the fact that $\| p \|_{h,\fineElem} \lesssim \| p \|_{0,\fineElem}$ for any polynomial $p$. The remainder of the proof is based on the same arguments as in the proof of Lemma~\ref{LEM:q_bound}.
\end{proof}

\begin{lemma}\label{LEM:bound_eta_no_eta}
 Let $\eta$ be as defined in \eqref{EQ:eta}. Then, for $\elem' \in \mesh_\coarse$ 
 \begin{equation*}
  \| A^{-1/2} \localQ (m\eta) \|_{0,\elem'} \lesssim \| A^{-1/2} \localQ m \|_{0,\omega_{\elem'}}
 \end{equation*}
 for any $m \in \skeletalSpace_\fine$ with $(\injectionOp \projectionOp m)\vert_{\mesh_{\elem'}} = 0$.
\end{lemma}
% 
...
\begin{proof}[Proof of Theorem \ref{t:decay}]
 ...
 \begin{equation}\label{EQ:proofDec}
  \|m\|_{a,\Omega \setminus \Nb^{\ell}(\elem)}^2 \lesssim a(m,\faceProj(m\eta)) + \|m\|_{a,\Nb^1(R)}^2,
 \end{equation}
 ...
 \begin{equation*}
  \|\bubble\|_{a,\Nb^1(R)} \lesssim \|\faceProj(m\eta)\|_{a,\Nb^1(R)}\qquad\text{and}\qquad\projectionOp(m\eta) = \projectionOp\bubble. 
 \end{equation*}	
 We further have
 \begin{equation*}%\label{EQ:proofDec2}
 \begin{aligned}
  a(m,\faceProj(m\eta)) &= a(m,\faceProj(m\eta) - \bubble) + a(m,\bubble)
  \leq \|m\|_{a,\Nb^1(R)}\|\faceProj(m\eta)\|_{a,\Nb^1(R)},
 \end{aligned}
 \end{equation*}
 ...
 \end{equation*}
 by the first line of~\eqref{eq:saddlepointCorEle} and the fact that $\supp(\faceProj(m \eta) - \bubble) \cap \elem = \emptyset$. By~\eqref{EQ:skeletalBilForm} and~\eqref{EQ:LS1} and the fact that $\localQ(m\eta) = \localQ(\faceProj(m\eta))$, we have that
 \begin{equation*}
  \|\faceProj(m\eta)\|_{a,\Nb^1(R)} \lesssim \|A^{-1/2}\localQ (m\eta)\|_{0,\Nb^{\ell-1}(\elem)\setminus \Nb^{\ell-3}(\elem)} \lesssim \|A^{-1/2}\localQ m\|_{0,\Nb^{\ell}(\elem)\setminus \Nb^{\ell-4}(\elem)},
 \end{equation*}
 where we use Lemma~\ref{LEM:bound_eta_no_eta} for all $\elem' \in \mesh_\coarse$ with $\operatorname{int}\elem' \subset \Nb^{\ell-1}(\elem)\setminus \Nb^{\ell-3}(\elem)$ in the last step. Going back to~\eqref{EQ:proofDec} and using once more~\eqref{EQ:Qvsah}, we conclude that there exists a constant $C >0$ with 
 \begin{align*}
  \|m\|_{a,\Omega \setminus \Nb^{\ell}(\elem)}^2 &\leq C\,\|m\|_{a,\Nb^{\ell}(\elem)\setminus \Nb^{\ell-4}(\elem)}^2 \leq C\,\|m\|_{a,\Omega \setminus \Nb^{\ell-4}(\elem)}^2 - C\,\|m\|_{a,\Omega \setminus \Nb^{\ell}(\elem)}^2.
 \end{align*}
Therefore, we obtain
 \begin{equation*}
  \| m \|^2_{a,\Omega \setminus \Nb^{\ell}(\elem)} \leq \tfrac{C}{C+1}\| m \|^2_{a,\Omega \setminus \Nb^{\ell-4}(\elem)} \leq \big(\tfrac{C}{C+1}\big)^{\lfloor\ell/4\rfloor}\| m \|^2_a.
 \end{equation*}
 That is, the estimate~\eqref{eq:decay} holds with $\vartheta = \big(C/(C+1)\big)^{1/8}$. For $\ell \leq 3$, the results holds by a slight adjustment of the constant.
\end{proof}

% --------------------------------------------------------------------------------------------------
\section{A practical localized multiscale method}\label{ss:loc}\label{SEC:localized}
% 
As mentioned before, a major drawback of the above multiscale approach is that it evolves around globally solving auxiliary corrections. Based on the decaying behavior of these corrections that has been proved in the previous section, we now introduce a localized---and thus practical---method that achieves similar convergence properties as its non-localized prototypical version. 
...
\begin{theorem}[Error of the localized multiscale method]\label{t:errloc}
 ...
 \begin{equation*}
  \|m - \tilde m^\ell_\coarse\|_a \lesssim H\,\|f\|_0 + \theta^\ell\,\|f\|_0
 \end{equation*}
...
\begin{proof}
 ...
 \end{align*}
 Using the continuity of $(\idOp - \injectionOp\projectionOp)$ with respect to the $a$-norm (cf.~Lemma~\ref{lem:stab}), Lemma~\ref{LEM:bound_eta_no_eta} (with $m = \corOp_\elem\xi_\coarse$, $\eta = 1-\chi$) and~\eqref{EQ:LS1} locally, we get
 \begin{equation*}
  \|(\corOp_\elem - \corOp_\elem^\ell)\xi_\coarse\|_a \lesssim \|\corOp_\elem\xi_\coarse\|_{a,\Nb^\ell(\elem) \setminus \Nb^{\ell-3}(\elem)} \lesssim \|\corOp_\elem\xi_\coarse\|_{a,\Omega \setminus \Nb^{\ell-3}(\elem)}.
 \end{equation*}
 Going back to~\eqref{eq:locproof} and using Theorem~\ref{t:decay}, we arrive at
 \begin{align*}
  \|(\msOp - \msOp^\ell)\xi_\coarse\|_a &\leq \sum_{\elem \in \mesh_\coarse} \|(\corOp_\elem - \corOp_\elem^\ell)\xi_\coarse\|_a \lesssim \sum_{\elem \in \mesh_\coarse} \theta^{\ell-3} \|\corOp_\elem\xi_\coarse\|_a.
 \end{align*}
 As a last step, we use the stability of $\corOp_\elem$, i.e., 
 \begin{equation*}
  \|\corOp_\elem\xi_\coarse\|_a \lesssim \|\injectionOp\xi_\coarse\|_{a,\elem} = \|\injectionOp\projectionOp\tilde m_\coarse\|_{a,\elem} \lesssim \|\tilde m_\coarse\|_{a,\omega_\elem}
 \end{equation*}
 ...
 \begin{equation*} 
  \|(\msOp - \msOp^\ell)\projectionOp\tilde m_\coarse\|_a \lesssim \theta^{\ell} \|f\|_0.
 \end{equation*}	
...
% 
We investigate the behavior of the practical multiscale method~\eqref{eq:locMethod} to approximate the solution to~\eqref{eq:PDEell} with $A$ defined as 
% 
\begin{equation*}
 A = \begin{cases} 1 & \text{ in black cells,} \\ 10 & \text{ in white cells,} \end{cases} \quad \text{ and } \qquad A = \begin{cases} \tfrac1{100} & \text{ in black cells,} \\ 1 & \text{ in white cells} \end{cases}
\end{equation*}
% 
in the checkerboard and random cases of Figure \ref{FIG:matrix_a}, respectively. Both coefficients are piecewise constant on a domain partition into $2^6 \times 2^6$ small squares, and each cell is either black or white. In the checkerboard case, we set $f(x) = 5 \pi^2 \sin(2 \pi x_1) \cos(2 \pi x_2)$, while $f=1$ is chosen in the random case.

The theoretical results of our work quantify the error of the multiscale solution compared to a very fine reference HDG solution based on a mesh with mesh size $\fine$ that resolves all the oscillations in the coefficient. For our numerical examples, we set $\fine = 2^{-8}$ and work with the 
linear LDG-H method with $\tau = \tfrac{100}\fine$. In Figure~\ref{FIG:results}, we plot the errors of solutions to~\eqref{eq:locMethod} for different coarse mesh sizes $\coarse$ and $\ell=2,3$. We present the errors in the energy norm (crosses) and in the $L^2$-norm (circles). We observe that for $\ell=2$, the error curve stagnates at some point if $\coarse$ is decreased, which is in line with the theoretical result in Theorem~\ref{t:errloc} that states that $\ell$ needs to be suitably increased with decreasing $\coarse$ to keep the convergence rate. In our experiments, $\ell=3$ appeared to be sufficient in that sense.
% 
% Figure environment removed
...
We believe that the presented analysis is only a first step towards the reliable combination of hybrid discontinuous Galerkin methods and the localized orthogonal decomposition strategy. In particular, the aim is to extend the scheme and the corresponding analysis to enable higher-order convergence rates.
% 
% --------------------------------------------------------------------------------------------------
...
% 
A possible operator $\liftingOp\colon\skeletalSpace_{\fine} \to \contdElementSpace \cap H^1(\Omega)$ has been constructed in \cite[Sect.\ 5.2]{LuRK22a} and is motivated by \cite{TanPhD}, \cite[Lem.\ A.3]{GiraultR86}, and \cite[Def.\ 5.46]{Monk03}. We follow their construction and characterize $\liftingOp \mu$ in two spatial dimensions via
% 
\begin{subequations}
\begin{align}
...
\begin{lemma}[Properties of $\liftingOp$]\label{LEM:lifting}
 ...
 \begin{align*}
  ...
  |\liftingOp \mu|_{1,\elem} & \eqsim \| \localQ \mu \|_{0,\elem} &&  \text{(lifting estimate).}
 \end{align*}
...
\begin{theorem}\label{TH:poincare_smooth}
 Consider a domain $\Omega \subset \IR^d$ that can be decomposed into $n \in \IN$ %\setminus \{ 0 \}$ 
...
% 
\begin{equation}
 \| \localU m - m \|^{2}_\fine \lesssim \sum_{\elem \in \mesh_\fine} \fine_\elem^2 \| \localQ m \|_{L^2(\elem)}^2, \tag{LS1*}\label{EQ:LS1*}
\end{equation}
% 
...
\begin{theorem}[broken Poincar\'e--Friedrichs inequality]\label{TH:skeleton_spaces}
 ...
 \begin{equation*}
  \| m \|_\fine + \| \localU m \|_{L^2(\Omega)} \lesssim \big(r + \fine + \sqrt{\delta r \fine}\big) \| \localQ m \|_{L^2(\Omega)} + \sqrt{\delta r} \| m \|_{L^2(\Gamma)}
 \end{equation*}
...
\begin{proof}
 ...
 \begin{equation*}
  \| m \|_\fine \le \| m - \localU m \|_\fine + \| \localU m \|_\fine \lesssim \fine \| \localQ m \|_{L^2(\Omega)} + \| \localU m \|_{L^2(\Omega)}.
 \end{equation*}
 ...
 \begin{align*}
  ...
  & \lesssim \big(\fine^2 + r^2 + \delta r \fine \big) \|\localQ m\|^2_{L^2(\mesh_\fine)} + \delta r \min\left\{ \| m \|^2_{L^2(\Gamma)}, \| \localU m \|^2_{L^2(\Gamma)} \right\}  \\
...
% 
\begin{equation*}
 a(m,\mu) = \int_\Omega \localQ m \cdot \localQ \mu \dx + s(m,\mu)
\end{equation*}
% 
with a symmetric positive semi-definite stabilizer/penalty term $s$, the above results clearly show that also
% 
\begin{equation*}
 \| m \|^2_\fine + \| \localU m \|^2_{L^2(\Omega)} \lesssim \big(r + \fine + \sqrt{\delta r \fine}\big)^2 a(m,m) + \delta r \| m \|^2_{L^2(\Gamma)}
\end{equation*}
% 
