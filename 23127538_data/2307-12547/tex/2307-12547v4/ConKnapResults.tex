...
\begin{proof}
    Clearly, \pa $\in$ \NP. We reduce \vc to \pa to prove NP-hardness. Let $(\GG=(\VV=\{v_i: i\in[n]\},\EE),$ $k)$ be an arbitrary instance of \vc where \GG is $3$-regular. We construct the following instance $(\GG^\pr=(\VV^\pr,\EE^\pr),(w(u))_{u\in\VV^\pr},(\alpha(u))_{u\in\VV^\pr},s,d)$ of \pa.
    \begin{align*}
