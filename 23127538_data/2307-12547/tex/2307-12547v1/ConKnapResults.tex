\section{Results for \pa}
We present our results for \pa in this section. We show that \pa is strongly \NPC by reducing it from \vc.
...
We know that \vc is \NPC even for $3$-regular graphs.

\begin{theorem}\label{thm:pa-gen-npc}
\pa is strongly \NPC even if the maximum degree of the input graph is four.
\end{theorem}
...
Hence, we do not expect a pseudo-polynomial time algorithm for \pa unlike \kp. \Cref{thm:pa-gen-npc} also implies the following corollary in the framework of parameterized complexity.

\begin{corollary}\label{cor:pa-max-deg}
    \pa is \PNPH parameterized by the maximum degree of the input graph.
\end{corollary}

We next show that \pa in \NPC even for trees. For that, we reduce from the \NPC problem \kp.
\longversion{
\begin{definition}[\kp]
...
\begin{theorem}\shortversion{[$\star$]}\label{thm:pa-star-npc}
...
We complement the hardness result in \Cref{thm:pa-star-npc} by designing a pseudo-polynomial-time algorithm for \pa for trees. In fact, we have designed an algorithm with runtime $2^{k\log k}\cdot n^{\mathcal{O}(1)}\cdot {\sf min}\{s^2,d^2\}$ where the treewidth of the input graph is $k$. We present this algorithm next.
...
&=(1-\eps)\OPT(\II)
\end{align*}
Hence, the approximation factor of our algorithm is $(1-\eps)$. We now analyze the runtime of our algorithm.
...
Hence, the runtime of our algorithm is $2^{\tw\log \tw}\cdot \text{poly}(n,1/\eps)$.
\end{proof}

\subsection{Other Parameters}

We next consider $vcs$, the size of a minimum vertex cover of a sub-graph induced by any solution of \pa, as our parameter. We already know 
from \Cref{thm:pa-star-npc} that \pa is \NPC for star graphs. We note that $vsc$ is one for star graphs. Hence, \pa is \PNPH with respect to $vcs$, that is, there is no algorithm for \pa which runs in polynomial-time for even constant values of $vsc$. However, whether there exists any algorithm with runtime $\OO(f(vcs).\text{poly}(n,s,d))$, remains a valid question. We answer this question negatively in \Cref{thm:vcs-woh}. For that, we exhibit an \FPT-reduction from \pvc which is known to be \WOH parameterized by the size of the partial vertex cover we are looking for.
...
\begin{theorem}\shortversion{[$\star$]}\label{thm:vcs-woh}
