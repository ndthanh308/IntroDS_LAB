...
Given an undirected edge-weighted graph $\GG=(\VV,\EE,c:\EE\longrightarrow\RB_{\ge0})$, non-negative weights of vertices $(w(u))_{u\in\VV}$, non-negative values of vertices $(\alpha(u))_{u\in\VV}$, size $s$ of knapsack, target value $d$, two vertices $x$ and $y$, compute if there exists a subset $\UU\subseteq\VV$ of vertices such that:
\begin{enumerate}
