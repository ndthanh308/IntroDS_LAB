...
\newpage
% PSYCHSCI: Five to seven relevant keywords should be listed directly under the abstract on the same page.

%\authornote{
%\addORCIDlink{Lisa Schwetlick}{0000-0003-3356-8324}, \addORCIDlink{Ralf Engbert}{0000-0002-2909-5811}
%\textcolor{blue}{<Optional Funding Statement>}
%This work was funded by the Bundesdruckerei and executed in collaboration with the University of Potsdam.
%Correspondence should be addressed to Lisa Schwetlick (email: lisa.schwetlick@uni-potsdam.de).
%}
%PSYCHSCI:The introduction should explain the rationale behind the current study, placing the research topic and study within the context of the current research landscape. Authors should summarize and cite previous research relevant to the current study and highlight the gap in knowledge being filled by the present research. The introduction should clearly pose the research question, describe the experimental design, and outline the authors’ hypothesis.
...
%Previous exposure to stimuli, such as in working memory or recognition tasks, can lead to larger P300 responses for repeated stimuli compared to novel stimuli. The hypothesis suggests that attentional allocation and the updating of the current neural representation play a role in producing the P300. %Doyle and Rugg, 1992; Guo et al., 2007; McEvoy et al., 2001

Using an oddball task, further electrophysiological correlates have also been found the human Locus Coeruleus using fmri \citep{Murphy2014}, an area which is generally associated with attention modulation. The authors also report a high correlation of Locus Coeruleus activation with pupil dilation. Effects of the oddball task on pupil dilation  \citep{Kamp2015, Strauch2020} are consistent with this finding. Further, the oddball task also affects microsaccade rate \citep{Valsecchi2006}. 
...
%In relation to the oddball paradigm, reflexive response components correspond to perceptual novelty while the cognitive and emotional components are comparable to task relevance effects. 
...
% it would be so nice to understand if there is a sequence effect in ms too!
...
% The cognitive recognition response may be modulated by an emotional response which is also known to influence pupil dilation.

The aim of the present study is to investigate whether pupil size and microsaccade rates can indicate whether a participant is looking at an image of her own face compared to the image of a friend's or a stranger's face. A potential application for this research includes biometric identification procedures. If robust differences between self recognition and viewing of other faces can be found in our measures derived from pupil dynamics and microsaccade statistics, then there is a potential use in multi-factor authentication. 

%We expect that one's own face is perceived as a special category and therefore act as an oddball target between distractors. Particularly, strangers' faces are likely to be perceived in a comparatively undifferentiated way (like the oddball paradigm's distractors), while one's own face is more likely to be perceived as the oddball (or target). 

% The Plan
%Here we focus on two miniature processes which occur involuntarily and unconsciously: the modulation of pupil size and the generation of microsaccades as part of the complex behavior of fixational eye movements. Both processes are typically looked upon as basic physiological responses to visual input. We show that pupil size and microsaccades also respond to high-level cognition in face perception. The present study investigates whether these subconscious responses, i.e., pupil dilation and microsaccades, can reveal whether participants were seeing a picture of their own- or someone else's face.

%% We know
% that arousal, surprisal, cognition modulate involuntary eye movement
% we know that the oddball task generates surprisal/effects
% "hidden oddball task"- the oddball is seeing yourself. Follow up q: if the study is all pics of you, mixed with some others, is it reversed?
% 



\section{Methods}
%For this study, we recorded the eye movements and pupil diameter from human participants looking at pictures of faces. Participants were recruited from two local high schools. 
...
%\textbf{\textcolor{purple}{Discuss Pupil difference\\}}
% Main Effect Face
...
%\textbf{\textcolor{purple}{Discuss MS difference\\}}
...
%\textbf{\textcolor{purple}{Discuss Origin of difference\\}}
Our findings are consistent with existing research concerning the oddball paradigm. The oddball stimulus (or target) has been found to increase dilation \citep{Strauch2020} and inhibit microsaccades \citep{Valsecchi2006}. In the context of this study we consider the Self condition to act as an oddball stimulus. We assume that one's own face is seen as qualitatively different from the faces of other individuals. Particularly the faces of strangers are likely to be less salient and capture less attention and to be grouped together. The greater dilation of the pupil and greater inhibition of microsaccades in response to one's own face can be interpreted as a marker of greater attention, consistent with previous research.  

%\textbf{\textcolor{purple}{Discuss Application Implications\\}}
At the outset of this study we posed the question of whether it is possible to distinguish self- and other-recognition using involuntary eye movement measures. We find strong differential effects of face recognition on pupil size and microsaccades. 
Such reaction differences may present a good basis for applications, such as biometric security. One plausible scenario is continuous authentication, i.e., data collection over a period of time.
For this particular biometric trait, we can imagine implementation in an application where users observe faces of themselves and others already, e.g., video conferencing or video-based identification.
Another potential application is the use in a presentation attack detection (PAD).
Malicious actors may use face masks or live video deepfakes to impersonate another user in face-recognition authentication or interaction with other user, e.g., in video conferences.
Measureable self-recognition effects can contribute to confirm or a legitimate user or convict a malicious user of a presentation attack.
...
%\color{green}
%Anm.:
%Das könnte ja widerum ein Feature in einem multi-class identification scenario sein, was wir hier ja bisher nicht betrachten. Vielleicht nehmen wir das noch auf?
%}
...
%\textcolor{red}{In order to explore this connexion further it would be necessary to analyze the microsaccades in the over repetitions in the same way as pupil sizes, an analysis which is less feasible for such a low probability point process.}

%\color{red}
%\textbf{\textcolor{purple}{Future Analyses\\}}
%The differences between participants are considerable, with individual participants showing a reversal of the main trend (i.e., reacting less to themselves). We speculate that the reasons for these interindividual differences may be related to questions of self-perception and self-image. Particularly, pupil size is known to be influences by emotional reactions at a longer timescale.
%Future work will include exploratory analyses using the self-image questions that were asked at the end of the study. We hypothesize that particularly strongly negative self image would be a factor that can influence pupil size in response to one's own face. 
