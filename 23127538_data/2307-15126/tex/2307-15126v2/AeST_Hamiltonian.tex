...
We find the presence of four first class and four second class constraints and show that the theory propagates six  degrees of freedom on general backgrounds.
We determine the case of small perturbations around Minkowski space as an example stemming from our analysis.
\end{abstract}
...
gravitational waves \cite{TheLIGOScientific:2017qsa} are in accord with its predictions. Although there are expected to be
corrections to the theory that become important at extremely high energy scales/short length scales (for instance in the early universe \cite{Starobinsky:1980te}), 
a question mark hangs over whether corrections or additional structure in the gravitational sector may also become important on cosmological and certain astrophysical scales. 
The reason for this is the considerable evidence that matter in many astrophysical and cosmological systems moves as if under the gravitational pull of a greater amount of matter 
than seems to exist in those systems.
 A notable early example of this was the observation of the motion of stars orbiting in the outskirts of spiral galaxies \cite{RubinEtAl1980}, 
the rotational velocity of the stars being too large to be due to the gravity of visible matter interior to their orbits. 
Similar `mass discrepancies' have since been consistently seen in other galaxies \cite{LelliEtAl2019}, 
clusters of galaxies \cite{Hahn:2006mk}, and on the largest cosmological scales \cite{Aghanim:2018eyx}.
...
by a factor of over five \cite{Aghanim:2018eyx}. Despite there being many proposed candidates for what this matter may be, see \cite{Jungman:1995df,Bertone:2004pz,Marsh:2015xka,Green:2020jor} for reviews, evidence 
for it remains comprised entirely of its gravitational effect on known matter. As such, 
there remains the possibility that what is being observed 
may not be the effect of the presence of additional (dark) matter, but that of additional gravitational degrees of freedom leading to
a modification of the way known matter affects the gravitational field. For this to manifest, an extension of GR must be at play.
...
This has prompted the construction of a variety of extensions of GR~\cite{BekensteinMilgrom1984,Bekenstein1988,Sanders1997,Bekenstein2004,NavarroVanAcoleyen2005,ZlosnikFerreiraStarkman2006,Sanders2005,Milgrom2009,BabichevDeffayetEsposito-Farese2011,DeffayetEsposito-FareseWoodard2011,BlanchetMarsat2011,Sanders2011,Mendoza:2012hu,Woodard2014,Khoury2014,Blanchet:2015sra,Hossenfelder2017,Burrage:2018zuj,Milgrom:2019rtd,DAmbrosio:2020nev,Kading:2023hdb},
which lead to  MOND behaviour at the quasistatic weak-field limit\footnote{
Extended dark matter models~\cite{Blanchet:2006yt,Blanchet:2009zu,Berezhiani:2015pia,Berezhiani:2015bqa,Kaplinghat:2015aga,Kamada:2016euw} have also
been proposed to accommodate for MOND behaviour, keeping GR as the description of gravity so that only two massless tensor-mode gravitational waves are present.
 These models automatically have a  $\Lambda$CDM cosmology in accordance with observations.}.
However, none of these extensions have been shown to fit the observations of the Cosmic Microwave Background radiation as most recently reported by the Planck Surveyor satellite~\cite{Aghanim:2018eyx}. 
Moreover, they usually do not lead to a gravitational wave tensor mode speed equalling the speed of light, although exceptions can be found, as required by data~\cite{TheLIGOScientific:2017qsa,Savchenko:2017ffs}.
...
It was further required to have a Friedman-Lemaitre-Robertson-Walker (FLRW) behaviour extremely close to that of $\Lambda$CDM by
...
It was further shown that linear fluctuations on  Minkowski space
...
and do not cause quantum vacuum instability at low momenta~\cite{GumrukcuogluMukohyamaSotiriou2016}.  Further studies of the AeST theory 
...
Despite these promising features of the theory, it is crucial that it can match the success of GR in all cases where it has been tested, while fitting observations in the regimes where GR fails 
without the addition of dark matter.  This will require finding solutions to the theory in systems that might not be describable by linear perturbations propagating on highly symmetric backgrounds. 
Towards these ends, an important first step will be to cast the equations as first order evolution equations in time  - this will enable both analytical and numerical solutions for more complicated 
to be more easily found.

In this paper we develop the canonical/Hamiltonian formulation of the theory and this will allow us to put the theory's equations of motion in the form of Hamilton's first-order equations of motion. 
...
of an irregular system, that is, a theory where the canonical structure varies throughout phase space. A manifestation of the latter can be that perturbations around some backgrounds describe 
 different number of degrees of freedom than perturbations around other backgrounds, see \cite{Alexandrov:2012yv,Alexandrov:2021qry} for examples.

The outline of the paper is as follows: In Section \ref{The_Theory} we introduce the theory; 
in Section \ref{ADM_formalism} we introduce the so-called 3+1 formalism and its associated decomposition of fields; 
...
around a Minkowski space solution and in doing so demonstrate the recovery of the results previously found in~\cite{SkordisZlosnik2021}.
...
symbol $A^\mu$ (without the `hat') for the projected vector field on the 3-dimensional hypersurface  introduced in section \ref{ADM_formalism}, see equation \eqref{A_mu_decomposition},
 as this will feature more prominently  than $\Ah^\mu$ in the present work.}. The action is 
\begin{widetext}
\begin{align}
S 
=& 
...
%
As a necessary first step towards constructing the Hamiltonian formalism for the theory, we must make a distinction between space and time. Specifically, we assume that for the region of spacetime of interest, there exists a global time coordinate $t(x^{\mu})$.
...
\end{align}
With the above relations and again using \eqref{J_0} and \eqref{J_i} from appendix \ref{UsefulResults} we find
\begin{align}
...
\label{J_idev}
\end{align}
so that the 3+1 form of  \eqref{NT_A_action} as
\begin{widetext}
\begin{align}
...
\end{subequations}
%
where $\approx$ denotes that these phase space functions need only vanish on the submanifold of phase space that they define (`weakly vanish'). 
...
To satisfy \eqref{eq_f_dot} there are ten Poisson brackets to be evaluated among the smeared constraints $\Constr_I$. Of these, three are trivial, namely,
...
\end{align}
%
Once again, this Poisson bracket is identical to the ones for GR albeit for an enlarged phase-space. 
...
\section{An example: Small perturbations around Minkowski space}
...
We consider a background Minkowski space and the linear evolution of small perturbations to this background.
...
\end{align}
Moreover, for Minkowski space we have $\bar{q}_{ij} = \delta_{ij}$ and then \eqref{q_ij_dot} trivially gives $\Pib^{ij} = 0$ (and so $\bar{\Bcal}_{(\Pi^{ij})}=0$). Now, the background solution should not
violate the isometries of Minkowski space, including rotational invariance, hence, all three dimensional vector fields should vanish: 
\begin{align}
...
We now compute the diffeomorphism constraint \eqref{Hcal_i} which very easily leads to
\begin{align}
...
We thus perform a canonical transformation to new canonical pairs $\{\chi,P_\chi\}$ and $\{\alphat,P_\alphat\}$ defined through 
 $\alphat = \alpha$, $P_\chi = P_\varphi$  and $P_\alphat =  P_\alpha - \Qcal_0  P_\varphi$ to get
\begin{widetext}
\begin{align}
 H^{(2)} =&
  \int d^3x\bigg\{  
...
\end{align}
are two constraints imposed by the Lagrange multipliers $\Psi$ and $\zeta$.
...
We subtract the four second class constraints and twice the number of first class constraints which remove the gauge redundant degrees of freedom, that is, we subtract $12$ degrees of freedom because of the 
constraints. We finally divide by two to find six physical degrees of freedom.
%
...
Marie Sk\l{}odowska-Curie grant agreement No. 945339. CS was partially supported by the European Structural and Investment Funds
