...
\newtheorem*{que}{Question}
	...
	\title{Kodaira type conjecture on almost complex 4 manifolds}
	\date{}
	\author{Dexie Lin}
	\maketitle
...
E-mail: lindexie@126.com	
	...
	\begin{abstract}
		...
		In this paper, we define  a refined Dolbeault cohomology on almost complex manifolds. We show that the condition $h^{1,0}=\tilde h^{0,1}$ implies  a symplectic structure on a compact almost complex $4$ manifold, where $\tilde h^{0,1}$ is the Hodge number of the refined Dolbeault cohomology and $h^{1,0}$ is the Hodge number of the   Dolbeault cohomology  defined by Cirici and Wilson \cite{CW21}. Moreover, we prove that the condition $h^{1,0}=\tilde h^{0,1}$ is equivalent to $\partial\bar\partial$-lemma, which is similar to the case of  compact complex surfaces. Meanwhile, unlike  compact complex surfaces, we show that on compact almost complex $4$ manifolds   the equality $b_1=h^{0,1}+h^{1,0}$ does not hold in general. 
	\end{abstract}
	\noindent
 ...
 \noindent
	...
	A complex manifold is a real $2n$ dimensional manifold $M$   whose coordinate charts are open subsets of $\mathbb C^n$ and the transition functions between charts are holomorphic, c.f.,   \cite[Section 2-Chapter 0]{GH} and \cite[Chapter 2]{Huy}.  The holomorphic structure induces a complex structure $J$ on the tangent bundle $TM$. On   complex manifolds, the deRham differential has the decomposition	
	$d=\partial+\bar\partial,$
	where $\partial$ and $\bar\partial$ are first order differential operators with   bidegree $(1,0)$ and $(0,1)$ respectively, c.f., \cite[Definition 2.6.9 and Proposition 2.6.15]{Huy}. Expanding $d^2=0$, one gets that 
	\[\partial^2=0,~\bar\partial^2=0,\mbox{ and }\partial\bar\partial+\bar\partial\partial=0.\]
	The Dolbeault cohomology groups $H^{*,*}_{Dol}$ of a complex
	manifold   are defined as the cohomology groups associated to the operator $\bar\partial$, c.f.,  \cite[Section 2 and 3-Chapter 0]{GH}. On compact complex manifolds, it turns out that all dimensions of $H^{*,*}_{Dol}$ are finite. 
    They provide  complex invariants
	of complex manifolds.
	A complex manifold $M$ equipped with a $J$-invariant metric is called an Hermitian manifold. On a compact complex manifold, the Hodge decomposition shows that $H^{*,*}_{Dol}$ are isomorphic to  $\Delta_{\bar\partial}$-harmonic spaces with respect to the same bi-degrees, c.f., \cite[Section 6-Chapter 0]{GH}. 
	A K\"ahler manifold  is an Hermitian manifold of complex dimension $n$ such that for every point, there is a holomorphic coordinate chart around this point in which the metric agrees with the standard metric on $\mathbb C^n$ up to order $2$  near this point, c.f., \cite[Section 6-Chapter 0]{GH} and \cite[Chapter 2 and Chapter 3]{Huy}. On the compact K\"ahler manifold, one has the K\"ahler identities, c.f., \cite[Proposition 3.1.12]{Huy} and 
    \cite[Section 7-Chapter 0]{GH}. These identities  induce  
	\[\Delta_d=2\Delta_{\partial}=\Delta_{\bar\partial}.\]
	Hence, by Hodge-decomposition  one gets
	\[ b^r =\sum_{j+k=r}h^{j,k}\mbox{ and }
	h^{j,k}=h^{k,j},
	\]
	on any compact K\"ahler manifold, where $h^{j,k}=\dim_{\mathbb C}H^{j,k}_{Dol}$, c.f., \cite[Section 7-Chapter 0]{GH}. Consequently,  all
	Betti numbers with odd degree   must be  even. 
 ...
 \end{emp}
	By Enrique-Kodaira's   classification \cite[Chapter VI]{BHPV} on compact complex surfaces,
	only the
	cases of elliptic surfaces and $K3$ surfaces leave open.
	\begin{itemize}
		...
		\item[(3)] Buchdahl \cite{Buch} and Lamari \cite{Lamari} independently proved the Kodaira's conjecture without using Enrique-Kodaira's   classification.
	\end{itemize} 
	In a big picture of   $4$-manifolds, Kodaira's  conjecture gives us the following diagram.
	\[\begin{tikzcd}	    
	...
	\end{tikzcd}\]
	Here an almost complex manifold is  a manifold whose tangent bundle admits a complex structure. For a compact oriented smooth $4$ manifold without  boundary, it is well-known that  the existence of   almost complex structure is a  topological obstruction, c.f., {\cite[Proposition 9.3-Chapter IV]{BHPV}}:
	\begin{emp}
		Let $X$ be an oriented compact smooth $4$ manifold. Then, $X$ admits an almost complex structure if and only if there is a class $c\in H^2(X;\mathbb Z)$ such that 
		\[c\equiv w_2(X) \bmod2\mbox{ and }
		c^2=2\chi(X)+3\tau(X),\]
		where $\chi(X)$ denotes the Euler characteristic number of $X$ and $\tau(X)$ denotes the signature of $X$. 
	\end{emp}
	From the above diagram,
	it is nature to ask the obstruction for compact almost complex $4$ manifolds admitting a symplectic structure. 
    However,	on  compact symplectic manifolds or almost K\"ahler manifolds,  Gompf  \cite{Gom95} showed that:
    \begin{emp}
     	Every finitely presented group $G$ can be realized as  the fundamental group of some compact symplectic $4$-manifold.
    \end{emp}
%	\end{thm}
	On the other hand, one can find many   almost complex $4$-manifolds with first Betti number even, but does not admit any symplectic structure, e.g., $(2k+1)\mathbb{C}P^2\# l\overline{\mathbb{C}P^2}$ with integers $k>0$ and $l\geq0$. In fact, on a compact complex surface, the existence of K\"ahler structure is equivalent to one of the following conditions,  c.f., \cite[Section 2 and 3-Chapter IV]{BHPV} and Gauduchon's work\cite{Gau76}:
	\begin{equation}
	(1)~b_1\mbox{ is even};~~~~(2)~\mbox{conditions on }h^{1,0}\mbox{ and } h^{0,1};~~~~(3)~\partial\bar\partial-\mbox{lemma},\label{eqn-kahler-condition}%b_1=2h^{1,0}\mbox{ or }b_1=2h^{0,1}
	\end{equation}
	where $\mbox{conditions on }h^{1,0}\mbox{ and } h^{0,1}$ means one of the following equivalent identities:
	\begin{equation}
	...
	Gompf's result excludes the first choice of \eqref{eqn-kahler-condition} for almost complex manifolds admitting  symplectic structure. 
	For an almost complex manifold, we have  two additional terms for the decomposition, 
	\[d=\partial+\bar\partial+\mu+\bar\mu,\]
	where $\bar\mu$  and $\mu$  arise from the Nijenhuis tensor.
	Not long ago, to answer Hirzebruch's $20$-th. problem \cite{Hir54}, Cirici and Wilson \cite{CW21} defined the Dolbeault cohomology  for almost complex manifolds by using the relation between $\bar\partial$ and $\bar\mu$. In Section 2, we will review  their definition of the Dolbeault cohomology   for almost complex manifolds.  We still denote the Dolbeault cohomology groups   by $H^{*,*}_{Dol}$ and their dimensions by $h^{*,*}$. By using  the Dolbeault cohomology groups to study the existence of  symplectic structure, we define another Dolbeault cohomology $\tilde{H}^{*,*}_{Dol}$, see Definition \ref{defi-tilde-Dolbeault}. We set $\tilde{h}^{*,*}=\dim_{\mathbb C}\tilde{H}^{*.*}_{Dol}$ and  show that,
	\begin{equation}
	\tilde{h}^{1,0}=h^{1,0}\leq\tilde{h}^{0,1} \leq h^{0,1} ,	\label{eqn-tilde-Dolbeault-ineq}
	\end{equation}
 \mbox{on compact almost complex $4$ manifolds, }
	 see Lemma \ref{lemma-hodge-inequality} and Lemma \ref{lemma-h1} for more details.
    One purpose of this paper is to give a necessary condition on Dolbeault cohomology groups for compact almost complex $4$  admitting a symplectic structure. 
	\begin{thm}\label{thm-main-1}
		Let $(X,J)$ be a compact almost complex $4$ manifold. Suppose that $h^{1,0}=\tilde h^{0,1}$. Then, $X$ admits a symplectic structure.
	\end{thm}
	Similar to compact complex manifolds, we get the following.
	\begin{thm}\label{thm-b1-inequality}
		Let $(X,J)$ be a compact almost complex $4$ manifold. Then, it holds that $b_1(X)\leq h^{0,1}+\hat h^{0,1}\leq  h^{0,1}+  h^{0,1}$. 
		Moreover, when the equality holds $b_1=h^{1,0}+h^{0,1}$, 
		the differential map $\delta_1: H^{0,1}_{Dol}\to H^{1,1}_{Dol}$ is trivial. 
	\end{thm}
	% the Fr\"ohlicher-type inequality holds $$b_1\leq h^{1,0}+ \hat h^{0,1}\leq h^{1,0}+h^{0,1},$$	for compact almost complex manifolds, see Proposition \ref{thm-b1-inequality} and Definition \ref{defi-hat-Dolbeault} of $\hat h^{0,1}$.%
	 In  Definition \ref{defi-hat-Dolbeault}, we will give  the definition of $\hat h^{0,1}$.
    On compact almost complex $4$ manifolds, we give the following relations,
    \begin{equation}
    	 2h^{1,0}\leq b_1 \mbox{ and }\tilde{h}^{0,1}\leq\hat{h}^{0,1}\leq h^{0,1},
    	 \label{eqn-b1-ineq-4mfld}
    \end{equation}  
	...
	\begin{equation}
		b_1=h^{1,0}+h^{0,1},\label{eqn-b1-hodge-complex-surface}
	\end{equation}
	always holds on compact complex surfaces by Atiyah-Singer-Hirzebruch index theorem \cite[Theorem 2.7-Chapter IV]{BHPV}. However, by Theorem \ref{thm-main-1} and the inequality \eqref{eqn-b1-ineq-4mfld}, we only have the inequality on compact  almost complex $4$  manifolds. Moreover, we show that the three identities in \eqref{eqn-three-identities} are not equivalent on almost complex $4$ manifolds, e.g.,   manifolds $(2k+1)\mathbb{C}P^2\#l\overline{\mathbb{C}P^2}$ satisfy the relation $b_1=2h^{1,0}$, but violate the equality \eqref{eqn-b1-hodge-complex-surface} for $k>0$ and $l\geq0$. 
	
	On compact complex surfaces, Gauduchon \cite{Gau76} proved  that $\partial\bar\partial$-lemma is equivalent to any one identity in \eqref{eqn-three-identities}. Hence, $\partial\bar\partial$-lemma is equivalent to the existence of K\"ahler structure on compact complex surfaces. On compact almost complex $4$ manifolds, we also obtain an equivalent condition for holding $\partial\bar\partial$-lemma. 
	\begin{lemma}\label{lemma-ddbar}
		Let $(X,J)$ be a compact almost complex $4$ manifold. Then, for any  $d$-exact $(1,1)$ form $\psi$ can be written as $\psi=\bar\partial\partial f$ for some    function $f$, if and only if the equality  $h^{1,0}=\tilde h^{0,1}$ holds.
	\end{lemma}
	For the  convenience of understanding, we give the following diagram to illustrate the conditions on compact almost complex $4$ manifolds admitting symplectic structure.
		...
		\begin{tikzcd}[column sep=0.02, row sep=small] 
			...
			\end{array}} &                                                   &                                                    \\
			{h^{0,1}=\tilde h^{0,1}} \arrow[rr, Leftrightarrow,"\begin{array}{c}
				~
				\\
				\mbox{Almost Complex }4 \mbox{ Manifolds}
		...
		\end{tikzcd}
	\end{center} 
  On compact complex surfaces, these conditions are equivalent,  here we pose a question. 
...
\begin{que}
	Let $(X,\omega)$ be a compact symplectic $4$ manifold. Can we find an almost complex structure $J$, such that  the identity
	\[
	h^{1,0}=\tilde{h}^{0,1},\]
    holds	on $(X,J)$?
\end{que}
In summary, we give a comparison between the conditions for an almost complex $4$ manifold  admitting a  symplectic structure and the ones for a complex surface admitting a K\"ahler structure. 
\begin{center}
...
%\begin{center}    \begin{tabular}{|c||c|}\hline    compact complex surfaces& compact almost complex $4$ manifolds\\\hline   {\small  \begin{tabular}{l|l|l}       $b_1$ even  & equivalent  & K\"ahler structure \\\hline       $b_1=2h^{1,0}$&equivalent  & K\"ahler structure \\\hline       $h^{1,0}=h^{0,1}$&equivalent  & K\"ahler structure \\\hline       $b_1=2h^{0,1}$&equivalent  & K\"ahler structure \\\hline       $\partial\bar\partial$-lemma&equivalent  & K\"ahler structure \\\hline    \end{tabular}    } & {\small \begin{tabular}{l|l|l}       $b_1$ even  & unrelated  & symplectic structure \\\hline       $b_1=2h^{1,0}$&unrelated & symplectic structure \\\hline       $h^{1,0}=\tilde h^{0,1}$&induces  & symplectic structure\\\hline       $b_1=\tilde h^{0,1}+\hat h^{0,1}$&induces  & symplectic structure\\\hline       $\partial\bar\partial$-lemma&induces  & symplectic structure \\\hline    \end{tabular} }       \end{tabular}\end{center}


 \noindent 	
	 In the study  field  of $4$ manifolds, they are  constrains on $b^+$ for a closed  oriented smooth $4$ manifold admitting a  symplectic structure by using Seiberg-Witten gauge theory, e.g.,  Bauer's work \cite{Bau06} and Li's work \cite{Li06}. On the other hand,  by the exotic phenomenon in smooth $4$ manifolds, the existence of symplectic structure can not be a topological obstruction for almost complex $4$ manifolds, e.g., Fintushel and Stern \cite{FS98} proved that there are infinitely many exotic $K3$
 surfaces which do not admit any symplectic structure. In this paper, we want to use the  Dolbeault cohomology and the first Betti number to study the symplectic structure on compact almost complex $4$ manifolds. 
  
	 The organization of this paper is as follows: In Section 2, we review the related differential operators on almost complex manifolds, and definition of Dolbeault cohomology; In Section 3, we define another Dolbeault cohomology, give some partial relations between  these two Dolbeault cohomologies; In Section 4.1, we give  some  relations  between the dimensions of Dolbeault cohomology groups and the first Betti number on compact almost complex $4$ manifolds and prove Theorem \ref{thm-b1-inequality}, In Section 4.2, we give the proofs to Theorem \ref{thm-main-1} and Lemma \ref{lemma-ddbar}.
	\begin{ack}
		The author would like to express the gratitude to  Xiangdong Yang for introducing the Dolbeault cohomology groups on almost complex manifold  and sharing   the work \cite{CW21} and \cite{ST23}, to Feng Wang for sharing \cite{Gom95} and Masaya Kwamura for explaining his work and Gauduchon's resultu \cite{Gau77}.  Part of the writing of this paper was completed at  Zhuhai Campus of Sun Yat-sen University, the author also thanks Yuan Wei for the hospitality.
	\end{ack}
	
	\section{Operators on almost complex manifolds}
	
	In this section, we give some necessary notions on almost complex  manifold, and introduce the related differential operators associated with almost complex manifolds. We begin from the definition of almost complex manifold. 
	\begin{defi}
		For a smooth manifold $M$ of real dimension $2m$. If there exists  a smooth section $J\in\Gamma(End(TM))$ on $TM$  satisfying $J^2=-1$, then we call
 ...
 $$(J^b \psi)\left(v_1, \cdots, v_p\right)=(-1)^p \psi\left(J v_1, \cdots, J v_p\right).$$
	...
	Denoting $ \mathcal A^*(M)$ by the sections of  $\bigwedge^*T^*_{\mathbb C}M$, we have the decomposition 
	\[ \mathcal A^r(M)=\bigoplus_{p+q=r} \mathcal A^{p,q}(M),\]
    where $\mathcal{A}^{p,q}(M)$ denotes the space  of complex valued  forms of $\bigwedge^{p,q}T^*_{\mathbb C}M$.
	By 
	...
	we  decompose the deRham differential as 
	\[
	d=\bar{\mu}+\bar{\partial}+\partial+\mu \mbox{ on } \mathcal{A}^{p,q},
	\]
	where   each  component is a derivation  with bi-degrees given by
	\[
	|\bar{\mu}|=(-1,2),~|\bar{\partial}|=(0,1),~|\partial|=(1,0),  ~|\mu|=(2,-1).
	\]
	By observation, the operators $\partial,~\bar\partial$ are of the first order and the operators $\mu,~\bar\mu$   are of the zero order. 
	Expanding  the identity $d^2=0$  gives the following: 
	\begin{equation}
		\begin{aligned}
			\mu^{2} &=0, \\
	...
	\begin{defi}
		...
		Moreover, when $\omega$ is $d$-closed, we call the quadruple   $(M,J,g,\omega)$   
		\textit{almost  K\"ahler} manifold. 
	\end{defi}
	 % i.e. a Riemannian metric satisfying $g(J-,J-)=g(-,-)$,
	Given an Hermitian metric $g$ on $(M,J)$, we set
	$\{Z_r\}$ as a local $(1,0)$-frame with respect to  $g$ and  $\left\{\theta^r\right\}$ as the local associated coframe, i.e., $\theta^j(Z_k)=\delta^{j}_{k}$. Similar to the case of complex manifolds, we locally write $g_{k \bar j}=g\left(Z_k, \bar Z_j\right)$. The imaginary component $\omega$ of $g$   can be  locally written as $\omega=i\sum_{k,j}g_{k \bar j}/2\theta^k \wedge \theta^{\bar{j}}$, c.f., \cite[Chapter 0-Section 6]{GH}. %Since $g$ is   Hermitian, one has $g_{i j}=g_{i j}=0$ and $g_{i j}=g_{j i}=\bar{g}_{i j}$.
	We locally write the coefficients of   Nijenhuis tensor $N$ by
	$$
	N\left(Z_{\bar{j}}, Z_{\bar{k}}\right)=-\left[Z_{\bar{j}}, Z_k\right]^{(1,0)}=  N_{j k}^t Z_t,~ N\left(Z_j, Z_k\right)=-\left[Z_j, Z_k\right]^{(0,1)}=\overline{N_{jk}^t} Z_{\bar{t}},
	$$
	 and write the structure coefficients of Lie bracket by $$[Z_j,\bar Z_{k}]=\sum_k C^r_{j\bar k}Z_r+
	\sum_lC^{\bar l}_{i\bar j}Z_{\bar l},$$
 c.f.,   \cite[2.8 and 2.9]{Kaw22}. Hence, we get the local expression of the Nijenhuis tensor
	$$
	N=\frac{1}{2} \overline{N_{i j}^k} Z_{\bar{k}} \otimes\left(\theta^i \wedge \theta^j\right)+\frac{1}{2} N_{i j}^k Z_k \otimes\left(\theta^{\bar{i}} \wedge \theta^{\bar{j}}\right) .
	$$
    One can locally write the actions of the operators $\bar\mu,~\bar\partial$ and $\partial$ on $\theta^s$ as follows:
	$$\bar\mu \theta^s=\frac14\sum_{k,j}N^s_{\bar k\bar j}\bar\theta^k\wedge\bar\theta^j,~\bar\partial \theta^s=-\sum_{k,j}C^{s}_{k\bar j}\theta^k\wedge \bar\theta^j,~
	\partial \theta^s=-\frac{1}{2}\sum_{k,j}C^s_{kj}\theta^k\wedge\theta^j.$$
	By  conjugation, one has the similar expressions for $\mu\bar\theta^s,~\partial\theta^s$ and $\bar\partial\bar\theta^s$. By \cite[Lemma 2.1]{CW21}, one obtains 
	\[
 ...
 $$(\varphi_1,\varphi_2)=\int_M\varphi_1\wedge *\bar\varphi_2.$$  
	Let  $\delta^*$ denote the formal-$L^2$-adjoint of $\delta$ for $\delta\in\{\mu,\partial,\bar\partial,\bar\mu,d\}$. 
	By integral part, one has the identities 
	...
	Its adjoint operator is $\Lambda=L^*=\star^{-1} L \star$. It is well known that the triple $\{L, \Lambda,  [L, \Lambda]\}$ defines a representation of $\mathfrak{s l}(2, \mathbb{C})$, with Lefschetz decomposition on complex $k$-forms
	$$
	\mathcal{A}^k=
	\bigoplus_{j \geq 0} 
	L^jP^{k-2j},
	$$
	where $P^j=\ker(\Lambda) \cap \mathcal{A}^j$, c.f., \cite[Section 6, Chapter 0]{GH}.  On almost  K\"ahler manifolds, Cirici and Wilson proved the  Hard Lefschetz Duality \cite[Theorem 5.1]{CW20}. In general, for almost K\"ahler manifolds, one has the following proposition and theorem. 
	...
	\begin{prop}[Cirici and Wilson {\cite{CW20}}]\label{prop-kahler-identities}
		For any   almost K\"ahler manifold $(M,J,g,\omega)$, i.e. $d\omega=0$, the following identities hold:
		\begin{itemize}
...
%	By taking the conjugates and duality,	the above identities imply the following isomorphism. 
	...
	\begin{thm}[Cirici and Wilson {\cite{CW20}}]\label{thm-CW-02}
		On any compact almost K\"ahler manifold with dimension $2 n$, we obtain identities
		$$
		\mathcal{H}_d^{p, q}=\mathcal{H}_{\bar\partial}^{p, q} \cap \mathcal{H}_\mu^{p, q}=\mathcal{H}_{\partial}^{p, q} \cap \mathcal{H}_{\bar{\mu}}^{p, q},
		$$
     for all $(p, q)$.
		We also have the following dualities:
		\begin{itemize}
			\item[$(1)$](Complex conjugation). We get equalities
			$$
			\mathcal{H}_{\bar{\partial}}^{p, q} \cap \mathcal{H}_\mu^{p, q}=\mathcal{H}_{\bar{\partial}}^{q, p} \cap \mathcal{H}_{\bar{\mu}}^{q, p} .
			$$
			\item[$(2)$]  The Hodge $*$-operator induces isomorphisms
			$$
			*: \mathcal{H}_{\bar{\partial}}^{p, q} \cap \mathcal{H}_\mu^{p, q} \rightarrow \mathcal{H}_{\bar{\partial}}^{m-q, m-p} \cap \mathcal{H}_\mu^{m-q, m-p} .
			$$
			\item[$(3)$] (Serre duality). There are isomorphisms
			$$
			\mathcal{H}_{\bar\partial}^{p, q} \cap \mathcal{H}_\mu^{p, q} \cong \mathcal{H}_{\bar\partial}^{m-p, m-q} \cap \mathcal{H}_\mu^{m-p, m-q} .
			$$
		\end{itemize}
	\end{thm}
 ...
 for $\delta,\delta'\in\{\partial,\bar\partial,\mu,\bar\mu\}$ and $\ell^{p,q}=\dim_{\mathbb C} \mathcal{H}^{p,q}_{\bar\partial,\mu}$. 
	Similar to the case of complex manifolds, we define an operator
	\[d^c=i\frac{\bar\partial+\mu-\partial-\bar\mu}{2}.\]
	On  almost complex manifolds, one has the identities \cite{ST23}:
	\[d^2=0=(d^c)^2\mbox{ and }\frac{i}{2}(dd^c+d^cd)=\partial^2-\bar\partial^2.\]
	In particular, for  dimension $4$, we have the following identity on $2$ forms,
    \begin{equation}
    dd^c|_{\mathcal{A}^{1,1}}=i\partial\bar\partial|_{\mathcal{A}^{1,1}}.
    	\label{eqn-partial-barpartial}
    \end{equation}
	...
	First, we review the Dolbeault cohomology $H^{*,*}_{Dol}$ defined by Cirici and Wilson on   almost complex manifolds. Later, we  introduce the refined  Dolbeault cohomology  $\tilde H^{*,*}_{Dol}$ and give some relations between these two Dolbeault cohomologies.
	
	Recall that 
	the  last three formulas in \eqref{eqn-bar-operator} imply  the following identities:
	\[
	\bar{\mu} \bar{\partial}+\bar{\partial} \bar{\mu}=0, ~
	\bar{\mu}^{2}=0, \mbox{ and }\bar \partial^2|_{\ker(\bar\mu)}\equiv0(\bmod im(\bar\mu)). \]
	These identities induce a variant Fr\"ohlicher-type spectral sequence on $\mathcal{A}^{*,*}$. 
	Hence, we can define the Dolbeault cohomology for an  almost  complex manifold
	$(M,J)$.
	\begin{defi}[Cirici and Wilson {\cite[Definition 3.1]{CW21}}]
		We define the Dolbeault cohomology of an almost  complex $2n$-dimensional manifold
		$(M,J)$ by
		\[
		H^{p,q}_{Dol}=H^q(H^{p,*}_{\bar\mu},\bar\partial)=
		\frac{\ker(\bar\partial:H^{p,q}_{\bar\mu}(M)\to H^{p,q+1}_{\bar\mu}(M))}
		{im(\bar\partial:H^{p,q-1}_{\bar\mu}(M)\to H^{p,q}_{\bar\mu}(M))},
		\]
		where $H^{p,q}_{\bar\mu}(M)=\frac{\ker(\bar\mu:\mathcal{A}^{p,q}\to \mathcal{A}^{p-1,q+2})}{im(\bar\mu:\mathcal{A}^{p+1,q-2}\to \mathcal{A}^{p,q})}$.
	\end{defi}
  If there is no ambiguity, we omit the underline manifold and almost complex structure and  simply write  the cohomology groups by  $H^{p,q}_{Dol}$ with bi-degree $(p,q)$. % of the above sequence \eqref{eqn-tilde-complex} by $\tilde H^{p,q}_{Dol}$. Similarly, we also  omit the manifolds and almost complex structures for the cohomology groups  $\tilde H^{p,q}_{Dol}$	and $H^{p,q}_{Dol}$. 	
	By taking
	into account the presence of the operator $\bar\mu$,
	Cirici and Wilson gave the definition of the Hodge-type filtration.
	
	\begin{defi}[Cirici and Wilson {\cite[Definition 3.2]{CW21}}]
		We define the Hodge filtration of $\mathcal{A}$ of an almost complex manifold $(M,J)$ by the following decreasing filtration 
		$$
		F^p \mathcal{A}^n:=\operatorname{Ker}(\bar{\mu}) \cap \mathcal{A}^{p, n-p} \oplus \bigoplus_{i>p} \mathcal{A}^{i, n-i}.
		$$
	\end{defi}
	Cirici and Wilson  showed that the Dolbeault cohomology groups are isomorphic to the first page of the above Hodge filtration, and all $(\Delta_{\bar\partial}+\Delta_{\bar\mu})$-harmonic forms  can be embedded into  the Dolbeault cohomology groups with respect to the same bidegree. 
	
	\begin{thm}[Cirici and Wilson  {\cite[Theorem 3.8 and Proposition 4.10]{CW21}}]
		...
		\begin{itemize}
			...
			{\{im({\bar\partial})  \cap\ker(\bar\mu)+im(\bar\mu)\}\cap\mathcal{A}^{p,q}}, ~0\leq p,q\leq n;
			\]
			\item[$(2)$] \[
			\mathcal{H}_{\bar{\partial}}^{p, q} \cap \mathcal{H}_{\bar\mu}^{p, q}\subseteq H^{p,q}_{Dol},
			\]
			where the equality holds for $p\in\{0,...,n\}$ and $q\in\{0,n\}$.
		\end{itemize}
	\end{thm}
	Inspired by the work of Sillari and  Tomassini \cite{ST23}, we introduce the sub-complex and the associated Dolbeault type cohomology. 
	\begin{defi}\label{defi-tilde-Dolbeault}
		On an almost complex manifold $(M,J)$,	we define the subspace of $\mathcal{A}^{*,*}$ by
		\[
		\mathcal{A}^{*,*}_{Dol}=\ker(\mu)
		\cap \ker((\bar\partial+\mu)\bar\partial)\cap
		\mathcal{A}^{*,*},
		\]
		and the refined Dolbeault cohomology groups $\tilde{H}^{*,*}_{Dol}$ by
		\[\tilde{H}^{*,*}_{Dol}=\frac{\ker(\bar\partial)\cap\mathcal{A}^{*,*}_{Dol}}{\bar\partial(\mathcal{A}^{*,*-1}_{Dol})}.\]
	\end{defi}
    Here, we also omit the underline manifold and the almost complex and just write $\tilde{H}^{*,*}_{Dol}$. 
	One can check that the identities $\partial^2|_{\mathcal{A}^{*,*}_{Dol}}=0,~
	...
	By observation, we have some  relations among $\mathcal{H}^{p,q}_{\bar\partial,\mu}$, $\tilde H^{p,q}_{Dol}$
	and $H^{p,q}_{Dol}$ for almost Hermitian manifolds. 
	\begin{lemma}   For a  compact almost Hermitian manifold $(M,J,g,\omega)$ with dimension $2n$, we have the following:
		\begin{itemize}
			\item[$(1)$] $\tilde H^{p,0}_{Dol}\cong H^{p,0}_{Dol}$, for $1\leq p\leq n$;
			\item[$(2)$] $\mathcal{H}^{0,1}_{\bar\partial,\mu}\subseteq \tilde H^{0,1}_{Dol} \subseteq H^{0,1}_{Dol}$; 
			\item[$(3)$] $\tilde H^{n,0}_{Dol}\cong\mathcal{H}^{n,0}_{\bar\partial,\mu}\cong H^{n,0}_{Dol}$. 
		\end{itemize}
	\end{lemma}
	\begin{pf}  
   ...
   the linear map $\alpha\in \ker(\mu)\cap \ker((\bar\partial+\mu)\bar\partial)\cap\ker(\bar\partial)\cap\mathcal{A}^{p,q} \mapsto [\alpha] \in H^{p,q}_{Dol}$   induces a well-defined  linear map $\tilde H^{p,q}_{Dol}\to H^{p,q}_{Dol}$ for all $p,q$. 
  \begin{itemize}
      \item[$(1) $] The isomorphism $\tilde H^{p,0}_{Dol}\cong H^{p,0}_{Dol}$ follows from 
		the facts that the operator $\mu$ acts trivially on $\mathcal{A}^{p,0}$ and the intersection $im(\bar\partial)\cap\mathcal{A}^{p,0}$ is trivial.
     \item[$(2)$] When a class $[\alpha]\in\tilde H^{0,1}_{Dol}$ is trivial in $H^{0,1}_{Dol}$,  we write $\alpha=\bar\partial f$ for some function. By the definition of $\alpha$,  one gets that $f\in\mathcal{A}^0_{Dol}$, i.e., $[\alpha]$ is trivial in $\tilde H^{0,1}_{Dol}$. Similarly, if a form $\alpha\in\mathcal{H}^{0,1}_{\bar\partial,\mu}$ is trivial in $\tilde H^{0,1}_{Dol}$, then $\alpha=\bar\partial f$.
		Applying the maximum principle to the equation $\bar\partial^*\bar\partial f=0$,   $f$ is constant and $\alpha=0$.
    \item[$(3)$] The proof follows from the facts
		that $\ker(\mu^*)\cap\mathcal{A}^{n,0}=\ker(\bar\mu)\cap\mathcal{A}^{n,0}$, and the operators $\mu$, $\bar\partial^*$ act trivially on $\mathcal{A}^{n,0}$. 
  \end{itemize}	
	...
	For the convenience to compare the dimensions of  Dolbeault cohomology groups with the first Betti number,   we introduce the following new  terms.
	\begin{defi}\label{defi-hat-Dolbeault}
		We consider the short complex, 
		\[0\to
		\mathcal{A}^{0}\overset{d}{\to}
		\mathcal{A}^{1}_{}\overset{d^{0,2}\oplus d^{2,0}}{\to}\mathcal{A}^{0,2}\oplus\mathcal{A}^{2,0},
		\]
		whose middle cohomology is defined by
		$\hat H^{1}:=\frac{\ker(d^{0,2}\oplus d^{2,0})\cap\mathcal{A}^{1}}{im(d)\cap \mathcal{A}^{1}}$.
	    We define  \[\hat H^{0,1}=\frac{\{u''\in\mathcal{A}^{0,1}|~d^{2,0}(u'+u'')=0=d^{0,2}(u'+u'')\mbox{ for some }u'\in\mathcal{A}^{1,0}\}}{im(\bar\partial)\cap\mathcal{A}^0}.\]
	\end{defi} 
	It is clear that $H^1_{dR}\subseteq\hat H^1$ and $H^{0,1}_{Dol}\subseteq\hat H^{0,1}$. 
	We set their dimensions by 
	...
	\] 	
   For the convenience of the later arguments in the next section, we end this section by the following lemma.
 
	\begin{lemma}\label{lemma-b1-h1}
		 One has the  formula
		\begin{equation} \hat h^{1}= \hat h^{0,1}+\tilde h^{0,1} .\label{eqn-b1-h1}
		\end{equation}		on any compact almost complex manifold.
	\end{lemma}
	\begin{pf}
	 ...
	 One can check that the above map induces a well-defined surjective linear map $$\pi:\hat H^1\to \hat H^{0,1},$$ 
    $\mbox{defined by }[u'+u'']\mapsto[u'']$. 
	 If a class $[u'+u'']$ belongs to the kernel of $\pi$, we   write 
		...
		\]
		Substituting  the identities $\partial^2+\mu\bar\partial=0$ and $\bar\partial^2+\bar\mu\partial=0$  into the above two equations, we obtain 
		\[u'-\partial f\in\ker(\bar\mu)\cap\ker(\partial),
		\]
		i.e., $[{\bar u'-\bar \partial \bar f}]\in \tilde H^{0,1}_{Dol}$.
		On the other hand,  there is an injective map 
		\[
		0\to\tilde H^{0,1}_{Dol}\to \hat H^1,\]
  $\mbox{defined by }
		[u]\mapsto[\bar u+0]$.
		It is not hard to see that for any class $[u]\in\tilde H^{0,1}$, the class $[\bar u+\bar u'-\bar\partial \bar f]$ belongs to $\ker(\pi)$. Therefore, we get the exact sequence,
		\[0\to\overline{\tilde H^{0,1}}\to \hat H^1\to \hat H^{0,1}\to 0,
		\]
	...
	Throughout this section, all almost complex manifolds are compact and $4$-dimensional, unless otherwise specified.   Recall that for a random Hermitian metric $g$ on an almost complex $4$ manifold, one has the identities, c.f., \cite[Lemma 2.1.57]{Don86},
	\[\Lambda^+=\Lambda^{2,0}\oplus\Lambda^{0,2}\oplus\mathbb C\langle\omega\rangle,~\Lambda^-=\ker(L)\cap\Lambda^{1,1},\]
	where $\omega$ is the imaginary part of $g$ and $\Lambda^\pm$ denotes the $\pm1$-eigenspace of the Hodge operator $*$ with respect to $g$. 
	
	\subsection{First Betti number on almost complex $4$ manifolds}
	In this subsection, we show some inequalities about the relations between the dimensions of Dolbeault cohomology groups and the first Betti number on compact almost complex $4$ manifolds. 
	
	

	
	First, we show some similar properties to the case of compact complex surfaces. 
	\begin{lemma}\label{lemma-barpartial}
		Let $(X,J)$ be  a compact  almost complex $4$ manifold. Then, we have the following:
		\[\ker(\bar\partial)\cap \mathcal{A}^{1,0}=\ker(d)\cap \mathcal{A}^{1,0}.\]
	\end{lemma}
	
	\begin{pf}
		For any form $\alpha\in \mathcal{A}^{1,0}(X)$, one has
		\[d\alpha=\partial\alpha+\bar\partial\alpha+\bar\mu\alpha.\]
		It is clear that $\ker(d)\cap \mathcal{A}^{1,0}\subset \ker(\bar\partial)\cap \mathcal{A}^{1,0}$.
		By the identities $$*|_{\mathcal{A}^{2,0}}=Id \mbox{ and }*|_{\mathcal{A}^{0,2}}=Id,$$ 
        we compute 
		...
		\]
		By taking the above formula  integral  over $X$, one has $\ker(\bar\partial)\cap \mathcal{A}^{1,0}\subset \ker(d)\cap \mathcal{A}^{1,0}$, which finishes the proof.
	\end{pf}
	
	Consequently, the above lemma implies the following corollary.
	...
	\begin{lemma}\label{lemma-h1}
	 We have an injective map  $$H^{1,0}_{Dol}(X)\to H^{0,1}_{Dol},$$
		on any compact almost complex $4$ manifold.
	\end{lemma}
	
	\begin{pf}
		If an  element $u\in\ker(\bar\partial)\cap \mathcal{A}^{1,0}$  can be written as $u=\partial f$ for some  function $f$, then it holds  $\bar\partial \partial f=0$. By applying the maximum principle for the operator $i\bar\partial\partial $,    $f$ is constant and $u=0$. 
	\end{pf}

 \begin{lemma}\label{lemma-b1-equiality}
		Let $(X,J)$ be a compact almost complex $4$ manifold. Then, it holds that $b_1(X)\geq 2h^{1,0}$. % Moreover, if $(M,J,g)$ is a compact almost K\"ahler $4$ manifold, then we have that $b^1(X)=2h^{1,0}$.
	\end{lemma}
	
	\begin{pf}
		Since $H^{1,0}_{Dol}\cap \overline{H^{1,0}_{Dol}}=\{0\}$ and $H^{1,0}_{Dol}=\ker(d)\cap\mathcal{A}^{1,0}$. The  fact  $$H^{1,0}_{Dol}\cap im(\partial)=H^{1,0}_{Dol}\cap im(d)=0,$$ 
        implies  the    desired inequality.
	\end{pf}
	
	Similarly, one establishes the following lemma.
	
	\begin{lemma}
		Let $(X,J)$ be  a compact almost complex $4$ manifold.
		If $\sigma\in H^{2,0}_{Dol}$ with  $\sigma=\partial \alpha+\mu\beta$ for some $\alpha\in \mathcal{A}^{1,0}$ and $\beta\in\mathcal{A}^{0,1}$, then $\sigma=0$.
	\end{lemma}
	
	\begin{pf}
		Clearly, one has the identities $$\bar\partial\alpha\wedge\bar\sigma=0,~\bar\mu\alpha\wedge\bar\sigma=0,$$ and
		\[\bar\partial\beta\wedge\bar\sigma=0, ~\partial\beta\wedge\bar\sigma=0.\] 
		These show that
		$$\sigma\wedge\bar\sigma=d(\alpha+\beta)\wedge\bar\sigma.$$ By applying the Stokes lemma and identity $d\sigma=0$, one obtains
  \[
...
%	The above induces a linear map	$H^{1,0}_{Dol}(X)\to H^{0,1}_{Dol}$.	
	...
	Together with  the isomorphisms $$\ker(\Delta_{\bar\partial}+\Delta_{\bar\mu})\cap\mathcal{A}^{2,0}\cong H^{2,0}_{Dol}\mbox{ and }
	\ker(\Delta_{\bar\partial}+\Delta_{\bar\mu})\cap\mathcal{A}^{0,2}\cong H^{0,2}_{Dol},$$ 
    one has the following lemma.
	
	\begin{lemma}
		The conjugate map $$H^{2,0}_{Dol}\to H^{0,2}_{Dol},$$
     is an  isomorphism on any compact almost complex $4$ manifold.
	\end{lemma}
	...
	\begin{pf}
		...
		By the relation $$H^{2,0}_{Dol}(X)\cong
		\ker(\Delta_{\bar\partial})\cap \ker(\Delta_{\bar\mu})\cap\mathcal{A}^{2,0},$$
         and self-duality of $\mathcal{A}^{2,0}$, one has   that
		for any $(2,0)$-form $\sigma$ belongs to $H^{2,0}_{Dol}(X)$ if and only if $\bar\partial\sigma=0$ and $\bar\mu\sigma=0$. This implies the identities
		$$H^{2,0}_{Dol}(X)\cong
		\ker(\Delta_{\bar\partial})\cap \ker(\Delta_{\mu})\cap\mathcal{A}^{2,0}\cong \ker(\Delta_{\partial})\cap \ker(\Delta_{\mu})\cap\mathcal{A}^{2,0}.$$
		Clearly, the conjugate map
		\[H^{2,0}_{Dol}(X)\cong\ker(\Delta_{\bar\partial})\cap \ker(\Delta_{\bar\mu})\cap\mathcal{A}^{2,0}\to
		\ker(\Delta_{\partial})\cap \ker(\Delta_{\mu})\cap\mathcal{A}^{0,2}
		\cong H^{0,2}_{Dol}(X),\]
		is an isomorphism, which proves the lemma.
	\end{pf}
	
	
	In section 3, we define another Dolbeault cohomology on almost complex manifolds.  On real dimension  $4$, we have the following relations among the dimensions of different Dolbeault cohomology groups.
	%Similar to the proofs of Lemma \ref{lemma-barpartial} and Proposition \ref{lemma-h1}, one has the following.
	
	
	\begin{lemma}\label{lemma-hodge-inequality}
	  It holds $\tilde h^{1,0}=h^{1,0}\leq \tilde h^{0,1}\leq h^{0,1}$. 
	\end{lemma}
	
	Here we skip the proof, since it is  not essentially different to Lemma \ref{lemma-h1}. As a consequence, one proves the inequality \eqref{eqn-tilde-Dolbeault-ineq}
	
	\begin{lemma}
		We have the following formula
		\[H^{2,0}_{Dol}=\tilde H^{2,0}_{Dol}=\mathcal{H}^{2,0}_{\bar\partial,\mu}
		\cong \tilde H^{0,2}_{Dol}=\mathcal{H}^{0,2}_{\bar\partial,\mu}=H^{0,2}_{Dol},\]
		on any compact almost Hermitian $4$ manifold.
	\end{lemma}
	\begin{pf}
     Clearly, $\partial$ acts trivially on $\mathcal{A}^{2,0}$ and $\bar\partial$ acts trivially on $\mathcal{A}^{0,2}$. 
		The  identities  \[\ker(\bar\partial^*)\cap\mathcal{A}^{2,0}=\ker(\partial)\cap\mathcal{A}^{2,0}\mbox{ and }\ker(\Delta_{\bar\mu})\cap\mathcal{A}^{2,0}=\ker(\Delta_\mu)\cap\mathcal{A}^{2,0}=\ker(\bar\mu),
		\]
		 show that $\mathcal{H}^{2,0}_{\bar\partial,\mu}=\ker(\partial)\cap\ker(\bar\mu)\cap\mathcal{A}^{2,0}=\tilde H^{2,0}_{Dol}=H^{2,0}_{Dol}$. By taking the conjugate, we finish the proof. 
	\end{pf}
 ...
 We summarize the results of the above lemmas by the following corollary.
	...
	\begin{cor}\label{coro-betti-inequality}
		...
		The following identities establish
		\begin{itemize}
			\item[$(1)$] $\ell^{1,0}= h^{1,0}=\tilde h^{1,0}\leq h^{0,1}
			$, and $ \ell^{0,1},~h^{1,0} \leq \tilde h^{0,1} \leq h^{0,1}$.
			\item[$(2)$] $\ell^{2,0}=h^{2,0}=\ell^{2,0}=
			h^{0,2} $.
		\end{itemize}
	\end{cor}
	\begin{pf}
		Here we only give the proof for $\ell^{0,1}\leq h^{0,1}$. 
		We define  
        $$\mathcal{H}^{0,1}_{\bar\partial}\cap \mathcal{H}^{0,1}_{\mu}\to H^{0,1}_{Dol},$$ 
        by $u\mapsto [u]$. 
		When $u$ maps to a trivial class, it holds that $u=\bar\partial f$ for some function $f$. By applying the maximum principle to the  equation $\bar\partial^*\bar\partial f=0$,   $f$ is constant and $u=0$.  
	\end{pf}
 
	Now we show the relation between $b_1$ and $h^{1,0}+\hat h^{0,1}$. 
	...
	\begin{pf} \textbf{of Theorem \ref{thm-b1-inequality}} By the relation $\hat H^{0,1}_{Dol}\subseteq H^{0,1}$, we only need to prove the first inequality
  \[
  b_1\leq h^{1,0}+\hat h^{0,1}. 
  \]
		...
		Similar to the first paragraph of the proof to Lemma \ref{lemma-b1-h1}, we define a linear map 
		\begin{equation}
			\ker(d)\cap\mathcal{A}^1 \to \mathcal{A}^{0,1},\label{eqn-b1-map}
		\end{equation} 
  $ \mbox{by } u'+u''\mapsto u''$, where $u'\in\mathcal{A}^{1,0}$ and $u''\in\mathcal{A}^{0,1}$. One can check that the above map  \eqref{eqn-b1-map} gives a   well-defined map  $$p:H^1_{dR}\to \hat H^{0,1}.$$ 
		If an image $[u'']$ is trivial in $\hat H^{0,1}$, then we write $u''=\bar\partial f$ for some function $f$. Combing the equation
		\[0=d(u'+u'')=\partial u'+\mu u''+\bar\partial u''+\bar\mu u'+\bar\partial u'+\partial u'',\] 
		for some $u'\in\mathcal{A}^{1,0}$ with the two identities $$\bar\partial^2+\bar\mu\partial=0\mbox{ and }\partial\bar\partial+\bar\partial\partial=0,$$ on functions, we get the formulas,
		\[\bar\mu(u'-\partial f)=0\mbox{ and }\bar\partial (u'-\partial f)=0\]
		i.e., $u'-\partial f\in H^{1,0}_{Dol}$.
		Hence, we establish the short exact sequence $$0\to H^{1,0}_{Dol}(X)\to H^1(X,\mathbb C)\to \hat H^{0,1},$$
		via the identification $\ker(p)\cong H^{1,0}_{Dol}$, which yields the inequality.
		%We set $p:H^1(X,\mathbb C)\to H^{0,1}_{Dol}$ by $[u'+u'']\mapsto [u]$, where $u'\in\mathcal{A}^{1,0}$ and $u''\in\mathcal{A}^{0,1}$. One can check that the map is well-defined. We   show that  $\ker(p)\cong H^{1,0}_{Dol}$.    If an image $[u'']$ is trivial, then we have $u''=\bar\partial f$ for some function $f$. We write  $0=du=\partial u'+\mu u''+\bar\partial u''+\bar\mu u'+\bar\partial u'+\partial u''$, where $u'\in\mathcal{A}^{1,0}$ and $u''\in\mathcal{A}^{0,1}$. By the identities $\bar\partial^2+\bar\mu\partial=0$ and $\partial\bar\partial+\bar\partial\partial=0$ on $\mathcal{A}^0$, one deduces that  $\bar\mu(u'-\partial f)=0$ and $\bar\partial (u'-\partial f)=0$. This gives $u'-\partial f\in H^{1,0}_{Dol}$, and establishes the short exact sequence $$0\to H^{1,0}_{Dol}(X)\to H^1(X,\mathbb C)\to H^{0,1}_{Dol},$$ via the identification $\ker(p)\cong H^{1,0}_{Dol}$, which gives the inequality $b^1\leq h^{1,0}+h^{0,1}$. 
		%
		By the same arguments of the above paragraph, one obtains the exact sequence
		$$0\to H^{1,0}_{Dol}\to H^1_{dR}\overset{p'}{\to }H^{0,1}_{Dol}.$$
		When the equality $b_1=h^{1,0}+h^{0,1}$ holds, one gets   the exact sequence 
		\begin{equation}
			0\to H^{1,0}_{Dol}\to H^1_{dR}\to H^{0,1}_{Dol}\to0.
			\label{eqn-exact-b1}
		\end{equation}
		Recall that the differential map  between  the stages in \cite[Appendix]{CW21}, 
		\[\delta_1:  E^{p,q}_{1}\to E^{p+1,q}_1,\]
		is defined by 
		$[w]\mapsto [\partial w+\bar\partial w_1]$, where $\bar\mu w_1+\bar\partial w=0$ for some $w_1$. 
		By the exact sequence \eqref{eqn-exact-b1}, there is some $v\in\mathcal{A}^{1,0}$ satisfying $d(u+v)=0$ for any $[u]\in H^{0,1}_{Dol}$. This means, 
        \[\partial u+\bar\partial v=0\mbox{ and }\bar\partial u+\bar\mu v=0.\]
        Therefore, $\delta_1([u])=[\partial u+\bar\partial v]=0$.
	\end{pf}
	
	\begin{rmk}
		 When $(X,J)$ is a compact  complex surface,  one establish the equality $b_1=h^{1,0}+h^{0,1}$   by using the sheaf theory,  c.f., \cite[Lemma 2.4-Chapter IV]{BHPV}. %Proposition  \ref{thm-b1-inequality} just %and the following Theorem \ref{thm-betti-equality}  		can prove the inequality.  In the next section, we give an example that the equality does not hold in general. 
...
% On dimension $4$,  one has the estimate about the first Betti number.
	...
	\begin{cor}\label{cor-b1-ineq-4-mfld}
	 The inequalities 
	 \[2h^{1,0}\leq b^1\leq h^{1,0}+\hat h^{0,1}\leq  h^{1,0}+h^{0,1},\]
	 hold on compact almost complex $4$ manifold. Moreover, one has the following: 
       \begin{itemize}
           \item[$(1)$] the equality $h^{1,0}=\hat{h}^{0,1}$ implies 
	 \[
	 	b^1=\hat h^1=h^{1,0}+\hat h^{0,1};
	 \]
           \item[$(2)$]  the equality $b_1=\tilde h^{0,1}+\hat h^{0,1}$ implies 
           \[ h^{1,0}=\tilde h^{0,1}.\]
       \end{itemize}
	...
	\begin{pf}
		The first inequality follows from Theorem \ref{thm-b1-inequality} and Lemma \ref{lemma-b1-equiality}. 
       \begin{itemize}
           \item[$(1)$]  The equality $h^{1,0}=\hat h^{0,1}$ is equivalent to the  condition $\hat h^1=\tilde h^{0,1}+\hat h^{0,1}$.  Thus,  the equality follows from the inequalities
		$$2h^{1,0}\leq b^1\leq h^{1,0}+\hat h^{0,1}\leq \tilde h^{0,1}+\hat h^{0,1}=\hat h^1.$$
       \item[$(2)  $]   The equality follows from the  equality follows from the inequalities $b_1\leq h^{1,0}+\hat{h}^{0,1}$ and $h^{1,0}\leq\tilde h^{0,1}$. 
       \end{itemize}
	\end{pf}
%	For any compact almst complex manifold, we have the following lemma.
	...
	%\begin{pf}  The identity $\ker(\Delta_{\bar\partial})\cap\ker(\Delta_{\bar\mu})\cap\mathcal{A}^{0,1}=\ker(\Delta_{\bar\partial})\cap\mathcal{A}^{0,1}$ implies that  $\ker(\Delta_{\bar\partial}+\Delta_\mu)\cap\mathcal{A}^{0,1}=\mathcal{H}_{\bar\partial}^{0, 1} \cap \mathcal{H}_\mu^{0, 1}\subseteq \ker(\Delta_{\bar\partial})\cap\ker(\Delta_{\bar\mu})\cap\mathcal{A}^{0,1}$, which proves the lemma.\end{pf}
	
	%We set $h^{p,q}=\dim_{\mathbb C}H^{p,q}_{Dol}(X)$.
 
	Corollary \ref{coro-betti-inequality} tells that
	 the condition  $h^{1,0}=h^{0,1}$ implies the identities  
     $$h^{1,0}=\ell^{1,0}=\ell^{0,1}=h^{0,1}.$$
	For almost K\"ahler manifold, Cirici and Wilson \cite{CW20} showed  the equality $\ell^{p,q}=\ell^{q,p}$ via  the identity $\Delta_{\bar\partial}+\Delta_{\mu}=\Delta_\partial+\Delta_{\bar\mu}$ and the  first relation in Theorem \ref{thm-CW-02}. 
	For almost K\"ahler $4$-manifolds, one can prove the equality $\ell^{1,0}=\ell^{0,1}$ via a simple method rather than using $\bar\partial,\mu$-Laplacian operators. 
	\begin{lemma}
		On compact almost K\"ahler $4$ manifolds, % $(X,J,g,\omega)$, 
        it holds that
		$h^{1,0}=\ell^{1,0}=\ell^{0,1}$.
	\end{lemma}
	\begin{pf}
		The formula $J u=*(\omega\wedge u)$ induces that
		$\bar\partial^* u=\Lambda(\partial u)$ for any $u\in \mathcal{A}^{0,1}$. 
		For any $u\in\mathcal{H}^{0,1}_{\bar\partial}\cap\mathcal{H}^{0,1}_\mu$, the conditions $\bar\partial^*u=0$ and $\bar\partial u=\mu u=0$  imply that $d^+u=0$. On a closed oriented $4$ smooth manifolds, one has the  formulas 
		\[0=\int_Xdu\wedge d\bar u=\int_Xd^+ u\wedge d^+\bar u+\int_Xd^-u\wedge d^-\bar u,\]
		and $\|du\|^2_{L^2}=\int_X|d^+u|^2+|d^-u|^2$.  %Similar to the  argument of Lemma \ref{lemma-(1,0)}, we have $u\cap im(\bar\partial)=0$, thus $\bar\partial^*u=d^*u=0$.
		Hence,  we get that  $du=0$ and  the conjugate map $\mathcal{H}^{1,0}_{\bar\partial}\cap\mathcal{H}^{1,0}_{\mu}\to \mathcal{H}^{0,1}_{\bar\partial}\cap \mathcal{H}^{0,1}_{\mu}$ is an isomorphism. 
	\end{pf}
	...
	For a compact symplectic $4$ manifold, its first Betti number might not be even  by Gompf's theorem, i.e., the equality $b_1=2h^{1,0}$ would not hold for a compact  almost K\"ahler $4$ manifold. At the end of this subsection, we give an analytic condition for compact symplectic $4$ manifolds admitting an even first Betti number.
	
	\begin{lemma}\label{lemma-b1-almost-kahler}
		 Suppose that   the inequality
		\[
		\lambda_1>4\|\Delta_{\bar\mu}|_{\mathcal{A}^1}\|,\]
  holds on a compact almost K\"ahler $4$ manifold $(M,J,g,\omega)$,
		where $\lambda_1$ denotes the first non-zero eigenvalue of the Hodge-Laplacian operator $\Delta_d$ restricted on $1$-forms and 
		$\|\Delta_{\bar\mu}|_{\mathcal{A}^1}\|$ denotes the operator norm of the linear map $\Delta_{\bar \partial}:L^2(\mathcal{A}^1)\to L^2(\mathcal{A}^1)$.
		Then, it holds  that $b^1=2\ell^{0,1}=2h^{1,0}$.
	\end{lemma}
	\begin{pf}
  %Since $\ker(\Delta_d)$ for $\alpha=\alpha'+\alpha''\in \ker(d^*)$, we have that $\Lambda(\bar\partial\alpha'-\partial \alpha'')=0$.
	Recall that $\Delta_d$ is a real operator, and each form in $\mathcal{A}^1_{\mathbb R}$ can be written as $u+\bar u$ for a unique element $u\in\mathcal{A}^{1,0}$.  By Lemma \ref{lemma-b1-equiality}, it suffices to show that each element $u+\bar u\in\ker(d)\cap\ker(d^*)\cap\mathcal{A}^1_{\mathbb R}$ induces a unique element $u\in H^{1,0}_{Dol}$ on $(M,J,g,\omega)$.  	Combining  the condition \[d^*(u+\bar u)=0 ,\]   
 $\mbox{with the K\"ahler identities  }[\Lambda,\partial]=-i\bar\partial^*,~[\Lambda,\bar\partial]=-i\partial^*$
 of Proposition \ref{prop-kahler-identities}, one obtains  $\Lambda(\bar\partial u-\partial\bar u)=0$. 
	Together with the equation  $(\bar\partial u+\partial \bar u)=0$, we get 
 \[\Lambda\bar\partial u=\Lambda\partial \bar u=0,\]
 i.e., $\partial\bar u$ is an imaginary-valued anti-self-dual $2$ form. 
	...
	\end{eqnarray*}   
	where we use  the identity $\bar\partial\partial u=-\partial\bar\partial u-\bar\mu\mu u-\mu\bar\mu u=    \partial^2\bar u+\mu\bar\partial\bar u$ for the second equality and the identity $\partial u+\mu\bar u=0$ for the last equality.   
	By using the above formula and the anti-self-duality of $\bar\partial u$, it holds that 	\begin{equation}
		...
		\end{equation}  
	By   the identity\[    \Delta_d=2\left(\Delta_{\bar{\partial}}+\Delta_\mu+\left[\bar{\mu}, \partial^*\right]+\left[\mu, \bar{\partial}^*\right]+\left[\partial, \bar{\partial}^*\right]+\left[\bar{\partial}, \partial^*\right]\right),    \]  in \cite[Proposition 3.4]{CW20}, we compute  
		\begin{eqnarray*}      \int_X(\Delta_du,\bar u)=2\int_X([\bar{\partial}, \partial^*]u,\bar u)=2\int_X(\partial^*\bar\partial u,\bar u)        =2\int_X(\bar\partial u,\partial\bar u)  = 4\int_X(\Delta_{\bar\mu} u,u), 
		\end{eqnarray*}    
		where we use  the equation $\bar\partial \bar u+\bar\mu u=0$ and the formula \eqref{eqn-parital-d} for the last equality. Therefore, by the hypothesis of $\lambda_1$, it holds that $u\in\ker(\bar\partial)$, i.e., $u\in H^{1,0}_{Dol}$.
		\end{pf}
		
	\subsection{Symplectic condition on almost complex $4$ manifolds}

Recall that on almost complex $4$ manifolds, we have the identity \eqref{eqn-partial-barpartial}. %$$dd^c|_{\mathcal{A}^{1,1}}=i\partial\bar\partial|_{\mathcal{A}^{1,1}}.$$
 First, we show that the equality $h^{1,0}=\tilde h^{0,1}$ is equivalent to the condition that each $dd^c$-closed $(1,1)$ associates a unique  $d$-closed $2$ form. 
...
%	By the parallelable argument as in \cite[Lemma 3]{Buch}, one obtains  the following lemma. 	\begin{lemma}		Let $(X,J,g)$ be a closed almost Hermitian $4$ manifold. 		If $\psi\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ is weakly $\bar\partial\partial$-closed, then there exists a sequence of  smooth $\bar\partial\partial$-closed real $(1,1)$-forms $\{\psi_i\}$ converging to $\psi$ in $L^2$ norm. 	\end{lemma}
...
%	\begin{lemma}\label{lemma-weak-partial}		Let $X$ be a compact complex surface, and let $\omega$ be its K\"ahler form. Suppose that 		$\psi\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ is a weakly $\bar\partial\partial$-closed. Then, we have the  inequality 		$(\int_X\omega\wedge\psi)^2\geq \int_X\psi^2\cdot \int_X\omega^2$. Moreover, the equality holds if and only if there exists a function satisfying $\psi=c\omega+i\bar\partial\partial f$ for some $c\in\mathbb R$. 	\end{lemma}	\begin{pf}		We define $c:=\int_X\psi\wedge \omega/(\int_X\omega^2)$. Assume that  $\psi$ is smooth. We consider the equation for  a smooth function $f$ such that 		\[\omega\wedge(\psi-c\omega+i\bar\partial\partial f)=0.\]		The equation is equivalent to $\Lambda\psi-2c-Pf=0$, where $P:=i\Lambda \bar\partial\partial$. The hypothesis on $\psi$ implies that 		$\Lambda\psi-2c\perp \ker(P^*)=const$. Hence, there exists  a smooth  function $f$ satisfying the above equation. 		\[\|\psi-c\omega -i\bar\partial\partial f\|^2=-\int_X\psi^2+c.\]		When the equality holds, we  choose a sequence of smooth $\bar\partial\partial$-		closed forms $\{\psi_j\}$ converging to $\psi$ in $L^2$. Let  $f_j$ satisfy		$\omega\wedge(\psi_j-c\omega+i\bar\partial\partial f_j)=0$, where $c_j$ is defined as above. 			\end{pf}
	...
	For a $dd^c$-closed $(1,1)$-form $\psi$, we want to find some  $u\in\mathcal{A}^{0,1}$  satisfying the equation,
	$$d(\psi+\bar\partial u+\partial\bar u)=0.$$ 
    This is equivalent to solving $\bar\partial\psi=\partial\bar\partial u+\bar\mu\partial\bar u+\bar\mu\partial \bar u+\bar\mu\mu u$. The existence of such $u$   is provided by the following proposition. 
	...
	\begin{prop}\label{prop-partial-solution}
		Let $(X,J,g,\omega)$ be a compact almost Hermitian $4$-manifold. Then, $h^{1,0}=\tilde h^{0,1}$ if and only if for any $\psi\in\mathcal{A}^{1,1}_{\mathbb R}$ satisfying $dd^c\psi=0$, there exists a solution to the equation
        \begin{equation}
      \bar\partial \psi=\partial\bar\partial u+\bar\mu\partial\bar u+\partial\bar\mu \bar u+\bar\mu\mu u,\label{eqn-ddbar}
         \end{equation}
		  for some $u\in\mathcal{A}^{0,1}$.
	\end{prop}
	\begin{pf}
		% We  solve the equation  $\bar\partial\psi=\partial \bar\partial u+\bar\mu\mu u+\bar\mu\partial \bar u$ for some  $u\in\mathcal{A}^{0,1}$ with a given $\bar\partial\partial$-closed $\psi\in\mathcal{A}^{1,1}$. 
  ...
  where $c$ denotes the conjugate map.
  ...
  holds  
		for all $w\in\mathcal{A}^{1,2}$ with $*\partial\bar\partial*w+*{\bar\mu\mu *w}+c\comp(\bar\partial\mu+\mu\bar\partial)*w=0$.
		Setting $v=*  w$, 
		we get that 
       \begin{equation}
    ...
    By the Stokes lemma, the above equation \eqref{eqn-ddbar-c} implies the following:
		\begin{eqnarray*}
			...
			&=&-(\bar\partial v,\bar\partial v)-(\bar\mu \bar v,\mu v)-(\bar\partial  v,\bar\mu \bar v)-(\bar\mu \bar v,\bar\partial v)\\
			&=& 
			-\|\bar\partial v+\bar\mu\bar v\|^2_{L^2},
		\end{eqnarray*}
      i.e.,  $\bar\partial v=0$ and $\mu v=0$.
		...
		The condition $h^{0,1}=\tilde h^{0,1}$ induces the  exact sequence $$0\to  H^{1,0}_{Dol}\to\hat H^1\to\hat H^{0,1}\to0.$$
        Hence, we get that $v+\bar v\in \ker(d^{0,2})\cap\ker(d^{2,0})$ and  
		the $(1,0)$-component of $v+\bar v-d f$ belongs to $\ker(d)$ for some function $f$, i.e., $\bar v-\partial  f\in\ker(d)$.
		%exists a smooth function $f$.  $\bar\partial f\in(\mathcal{H}^{0,1}_{\bar\partial,\mu})^\perp\cap\ker(\bar\partial)\cap\ker(\mu)$ 
		Therefore, it holds that 
		\[\int_X \bar v \wedge \bar\partial \psi=-\int_Xd(\bar v-\partial f) \wedge \psi=0,\]
  i.e., $\int_X \bar w\wedge *\bar\partial\psi=0$. 
		%Recall that $(\bar\partial+\bar\mu)^2=0$ on $\mathcal{A}^1$
		
		
		
		Conversely, we assume that for any $dd^c$-closed form $\psi\in\mathcal{A}^{1,1}_{\mathbb R}$, the equation $\bar\partial \psi=\partial\bar\partial u+\bar\mu\partial\bar u+\bar\mu\partial \bar u+\bar\mu\mu u$ can be solved for some $u\in\mathcal{A}^{0,1}$. Recall that for any $dd^c$-closed $(1,1)$-form $\psi$, we can write $\psi=\psi'+\psi''$, where $\psi'$ is the real part of $\psi$ and $\psi''$ is the imaginary part of $\psi$.
		Since the operator $dd^c$ is real, the $dd^c$-closeness of $\psi$ is equivalent to the $dd^c$-closeness of $\psi'$ and $\psi''$.
		By taking the sum of the solutions, the assumption implies that for any complex valued $dd^c$-closed $(1,1)$-form $\psi$, we have the solutions for the equation,
		\[\bar\partial\psi=\partial\bar\partial u_1+\bar\mu\mu u_1+\bar\mu\partial u_2+\partial\bar\mu u_2,\]
		where $u_1\in\mathcal{A}^{0,1}$ and $u_2\in\mathcal{A}^{1,0}$. 
		We need to show that each solution to the equation $\partial v=0$ for $v\in\mathcal{A}^{1,0}_{Dol}$ generates a unique form in $\ker(\bar\partial)\cap\mathcal{A}^{1,0}$. %up to an element in $im(\bar\partial)$. 
		It is clear that 
		the equation $$\bar\partial v=\partial\bar\partial f,$$
  can be solved for some function $f$, if and only if the formula $\int_X \bar\partial v\wedge\bar \psi=0$ holds for any $dd^c$-closed $(1,1)$ form $\psi$. 
		By the Stokes lemma, we rewrite the above formula as 
		\begin{eqnarray*}
		...
		\end{eqnarray*}
		Consequently, by the conjugate map,   each element in $\ker(\bar\partial)\cap\mathcal{A}^{0,1}_{Dol}$  is $\partial$-closed up to some element in $im(\bar\partial)\cap\mathcal{A}^{0,1}_{Dol}$, i.e.,  $\tilde h^{0,1}=h^{1,0}$. 
	\end{pf}
...
For the  convenience of later arguments, we verify the following  identities for almost complex $4$ manifolds.
	\begin{lemma}\label{lemma-ddbar-zero}
		Let  $(X,J)$ be an almost complex $4$ manifold. We have the following identities: 
		\begin{itemize}
	...
	\end{lemma}
	\begin{pf}
		From the identity \[\partial\bar\partial f=\partial(\bar\partial f/2)+\bar\partial(-\partial f/2),\] we see that (1) implies (2). Therefore, it suffices  to show (1). 
		We compute 
...
Combining the above Lemma \ref{lemma-ddbar-zero} with Proposition \ref{prop-partial-solution}, one obtains the following. 
\begin{lemma}
	Let $(X,J,g,\omega)$ be a compact almost Hermitian $4$ manifold.     
	If $h^{1,0}=\tilde h^{0,1}$, then there exists a linear map $s:Z^{1,1}_{dd^c}(X,\mathbb R)\to Z^{2}_{d}(X,\mathbb R)$, such that
	\begin{enumerate}
	...
	\end{enumerate}
	where $Z^{1,1}_{dd^c}$ denotes the space of all $\partial\bar\partial$-closed real $(1,1)$-forms and $Z^2_d$ denotes the space of all $d$-closed real $2$-forms. 
\end{lemma}
\begin{pf}
	We only need to show that the map $s$ is well-defined. It suffices to prove that the equation  \eqref{eqn-ddbar} has a unique solution. 
	We assume that there are two forms $u_1,u_2\in\mathcal{A}^{0,1}$ satisfying  
    \[\bar\partial \psi=\partial\bar\partial u_i+\bar\mu\partial\bar u_i+\partial\bar\mu \bar u_i+\bar\mu\mu u_i,~i=1,,2\]
    for some $dd^c$-closed $(1,1)$-form $\psi$. 
     Setting $\gamma=u_1-u_2$, one has the equation 
     ...
     \end{equation}
	By the basic identity $Re(a+b)=Re(a+\bar b)$ for any complex numbers $a$ and $b$, 
	one obtains
	\begin{eqnarray*}
	...
	\end{eqnarray*}
	where we use the equation \eqref{eqn-ddbar-zero} for the last equality. This means 
 $\bar\partial\gamma=0,\mbox{ and }\mu\gamma=0$.
  Therefore, we get $$
 \bar\partial u_1+\partial\bar u_1+\mu u_1+\bar\mu\bar u_1=\bar\partial u_2+\partial\bar u_2+\mu u_2+\bar\mu\bar u_2,
 $$ i.e., the map $s$ is well-defined. 
...
By Lemma \ref{lemma-ddbar-zero},
one  also has the following corollary.
\begin{cor}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Suppose that $h^{1,0}=\tilde h^{0,1}$. Then, we have an injective map
	\[
	S: H^{1,1}_{dd^c}\to H^2_{dR},
	\]
...
\begin{pf}\textbf{ of Theorem \ref{thm-main-1}}
    By Gauduchon's result  \cite{Gau77}, one can find an Hermitian metric $g$ on $(X,J)$, whose imaginary part $\omega$  is  $dd^c$-closed. Proposition \ref{prop-partial-solution} provides that there is a form $u\in\mathcal{A}^{0,1}$ such that the $2$-form $\omega'=\omega+\bar\partial u++\partial\bar u \mu u+\bar\mu\bar u$ is $d$-closed. 
    By the following Lemma \ref{lemma-nondegenerate},  we  have that  $\omega'$ is non-degenerate. 
\end{pf}
...
\begin{lemma}\label{lemma-nondegenerate}
	Let $\psi$ be a real valued positive $(1,1)$-form of $(X,J)$ and let $\sigma$ be a $(2,0)$ form. Then, the real form 
...
\begin{pf}
    ...
    where $(\theta^1,\theta^2)$ is dual to $(Z_1,Z_2)$ near $x$.
	It suffices to show that the contraction $\iota_v\psi'$ is zero at $x$ if and only if $v$ is zero at $x$ for any  real-valued vector field $v\in\Gamma(TX)$. Again, we have the local expressions \[v=v_1Z_1+v_2Z_2+\bar v_1\bar Z_1+\bar v_2\bar Z_2,\]
	and 
	...
	\]
	Since the matrix $\left(\begin{array}{cc}
		i&-\bar a  \\
		a & -i
	\end{array}\right)$ is invertible, we get that $v_1=\bar v_2=v_2=0$, i.e., $v$ is zero at $x$. %the form $\psi'$ is nondegenerate at each point. 
\end{pf}
...
At last, we give the proof to Lemma \ref{lemma-ddbar} to ending this paper. 
...
\begin{pf} {\bf of Lemma \ref{lemma-ddbar}}
	First, we assume that the equality $h^{1,0}=\tilde h^{0,1}$ holds, which is equivalent to establish an exact sequence\[0\to H^{1,0}_{Dol}=\tilde H^{0,1}_{Dol}\to \hat H^1\to \hat H^{0,1}\to0.	\] 
	For any $d$-exact $(1,1)$-form $\psi$,   we write $\psi=d\alpha$ for some $\alpha\in \mathcal{A}^1$.  
	The condition that $d\alpha$ is a $(1,1)$-form shows that \[\bar\partial \alpha''+\bar\mu\alpha'=0\mbox{ and }\partial\alpha'+\mu\alpha''=0,\]
	where $\alpha'$ denotes the $(1,0)$-component of $\alpha$ and $\alpha''$ denotes the $(0,1)$-component of $\alpha$. 
%	Since the condition is equivalent to the exact sequence	\[0\to H^{1,0}\to\hat H^1\to \hat H^{0,1}\to0.	\] %$H^1_{dR}=\bar H^1\to \bar H^{0,1}$ is surjective and
	Since
	$[\alpha'+\alpha'']\in\hat H^{1}$,  there is a function  $ f''$ satisfying 
	\begin{equation}
		\alpha'-\partial f'' \in\ker(d).\label{eqn-d-1}
	\end{equation}% for some $(1,0)$-form $v'$.
	Combining the equations \[\bar\partial (\alpha''-\bar\partial f'')+\bar\mu (\alpha'-\partial f'')=0\mbox{ and }\partial (\alpha'-\partial f'')+\mu (\alpha''-\bar\partial f'')=0,\] we get
	\[\alpha''-\bar\partial f''\in\ker(\mu)\cap\ker(\bar\partial).\]
	Again by the condition $h^{1,0}=\tilde h^{0,1}$, we can find a function $f'\in\ker(\partial^2)\cap\ker(\bar\partial^2)$ such that 
	\begin{equation}
		\alpha''-\bar\partial f''-\bar\partial f'\in\ker(d). \label{eqn-d-2}
	\end{equation}
	Hence, by \eqref{eqn-d-1} and \eqref{eqn-d-2} we obtain
	...
	\begin{eqnarray*}
		\psi=d(\alpha''-\bar\partial f''-\bar\partial f')+d(\alpha'-\partial f'')+d(\bar\partial f''+\partial f''+\partial f')=-\partial \bar\partial f',
	\end{eqnarray*}
	where we use the identity $d=\partial+\bar\partial $ on functions for the last equality. 
	% i.e., .
	...
	Conversely,  we assume that each $d$-exact  $(1,1)$ form $\psi $ can be  written as $\psi=\bar\partial\partial f$ for some   function $f$. We consider a representative form $u$ in a class $[u]\in \tilde H^{0,1}_{Dol}$. Clearly, the form $\partial u=du$ is a $d$-exact $(1,1)$-form. By the hypothesis, we can find a function $f$ such that 
	$$\partial (u-\bar\partial f)=0.$$
 We obtain $u-\bar\partial f\in \ker(d)$, which implies that $f\in\ker(\partial^2)\cap\ker(\bar\partial^2)$. 
	Therefore,  the conjugate map 
	\[ 0\to  H^{1,0}_{Dol}=\tilde H^{1,0}_{Dol}\to \tilde H^{0,1}_{Dol},\]
	is an isomorphism, i.e., $h^{1,0}=\tilde h^{0,1}$.  
...
\begin{rmk}
	On K\"ahler manifolds, it is well-known that each $d$-exact form is $\bar\partial\partial$-exact, which is called the $\bar\partial\partial$-lemma.  Gauduchon's work \cite{Gau76} tells us that for a compact complex surface, the $\bar\partial\partial$-lemma is equivalent to the condition $h^{1,0}=h^{0,1}$. Hence, by the proof of Kodaira conjecture, the $\bar\partial\partial$-lemma is equivalent to the K\"ahler condition on  compact complex surfaces. %Since it holds that 	$\tilde h^{0,1}=h^{0,1}=\bar h^{0,1}$ for compact complex surfaces, the above lemma can be regarded as an alternative proof of Gauduchon's work.
\end{rmk}  

The above lemma  induces the following. 
\begin{cor}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Then, the condition $h^{1,0}=\tilde h^{0,1}$ is equivalent to the 
    condition
	$H^{1,1}_{dR}(X)\cong H^{1,1}_{BC}(X)$, where 
	$H^{1,1}_{dR}=\frac{\ker(d)\cap\mathcal{A}^{1,1}}{
		im(d)\cap\mathcal{A}^{1,1}
	}$ and 
	$H^{1,1}_{BC}(X)=\frac{\ker(d)\cap\mathcal{A}^{1,1}}{
		im(dd^c)\cap\mathcal{A}^{1,1}
	}$.
\end{cor}


%\begin{prop}\label{prop-5}    Let $\psi_1,~\psi_2\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ be weakly     $\bar\partial\partial$-closed and satisfy $\int_X\psi^2_i\geq0$ and    $\int_X\psi_i\wedge\omega\geq0$, for $i=1,2$.    Then, $\int_X\psi_1\wedge\psi_2\geq (\int_X\psi^2_1)^{1/2}     (\int_X\psi^2_2)^{1/2}$,  with equality if and only if $\psi_1$ and $\psi_2$     are linearly dependent modulo the image of $i\bar\partial\partial$. \end{prop}
...
%\begin{cor}\label{cor-6}    If $\phi\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ is weakly $\bar\partial\partial$-closed, and satisfies $\int_X\phi^2>0$ and $\int_X\phi\wedge\chi>0$ for any form $\chi$ with the condition, $\int_X\chi^2\geq0$ and $\int_X\chi\wedge\omega>0$. \end{cor}

%\begin{lemma}\label{lemma-7}    Let $\chi\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ be $\bar\partial\partial$-closed, and satisfy     $\int_X\chi^2\geq0$ and $\int_X\chi\wedge \omega\geq0$. Then,     for each $\epsilon>0$, there is a positive $(1,1)$-form $p_\epsilon$ and a function $f_\epsilon$ such that     \[\|\chi-p_\epsilon+i\bar\partial\partial f_\epsilon\|_{L^2}<\epsilon.\]\end{lemma}
%\begin{pf}    If $\int_X\chi\wedge \omega=0$, by Lemma \ref{lemma-weak-partial}, $\chi$ is $\bar\partial\partial$-exact.     We assume $\int_X\chi\wedge \omega=1$ by scaling.     Let     \[\mathcal P=\{p\in L^2(\mathcal{A}^{1,1}_{\mathbb R})|~p\geq 0,~a.e.,~    \int_X\omega\wedge p>0\},\]    \[\mathcal{P}_\epsilon=\{p'\in  L^2(\mathcal{A}^{1,1}_{\mathbb R})|~    \|p'-p\|_{L^2}<\epsilon,~\mbox{for some }p\in\mathcal{P}\},\]    and\[    \mathcal{H}:=\{\chi+i\bar\partial\partial f|~f\in L^2_2(X)\}.    \]    $\mathcal P_\epsilon$ is an open convex subset of $L^2(\mathcal{A}^{1,1}_{\mathbb R})$ and $\mathcal H$ is a closed convex subset.     If $\mathcal{P}_\epsilon\cap \mathcal{H}=\emptyset$, the Hahn-Banach Theorem implies that there exists $\phi\in L^2(\mathcal{A}^{1,1}_{\mathbb R})$ and a constant $c\in \mathbb R$ such that $\int_X\phi\wedge h\leq c$ and $\int_X\phi\wedge p>c$ for each $h\in\mathcal{H}$ and each $p\in\mathcal{P}_\epsilon$. We have $\phi$ is weakly $\bar\partial\partial$-closed.     We set $\phi_0=\phi-c\omega$, which is weakly $\bar\partial\partial$-closed.     We have that     $\int_X\wedge_0\wedge \chi\leq 0$, and $\int_X\phi_0\wedge p_0>0$, i.e. $\phi_0$ is positive almost everywhere. Hence, $\int_X\phi^2_0>0$ and     $\int_X\phi_0\wedge \omega>0$, i.e $\int_X\phi_0\wedge\chi>0$ by Corollary \ref{cor-6}. This is a contradiction.     Therefore, $\mathcal{P}_\epsilon\cap \mathcal{H}\neq\emptyset$, which implies the existence of $p_\epsilon$ and $f_\epsilon$.\end{pf}
...
\begin{thebibliography}{99}
 ...
 Bauer, Stefan: Almost complex 4-manifolds with vanishing first Chern class, J. Differential Geom.,  \textbf{79} (2008), no.1, 25–32.
	...
	\bibitem{CW20}
	Cirici, Joana; Wilson, Scott O.: Topology and geometric aspects of almost K\"ahler manifolds via harmonic theory, preprint available at https://arxiv.org/pdf/1809.01414.pdf
 ...
 \bibitem{FS98}
  Fintushel, Ronald; Stern, Ronald J.: Knots, links, and $4$-manifolds, Invent. Math., \textbf{134} (1998), 363-400.
	...
	Mathematics, \textbf{142}  (1995), 527-95.
	...
	\bibitem{Hir54}
	Hirzebruch, F.: Some problems on differentiable and complex manifolds,  Ann. of Math., \textbf{2} (1954) 60, 213–236.
 ...
 \bibitem{Li06}
  Li, Tian-Jun:  Quaternionic bundles and Betti numbers of symplectic $4$-manifolds with Kodaira dimension zero, Int. Math. Res. Not. (2006), Art. ID 37385, 28 pp.
	...
	\bibitem{NN}
	 Newlander, A.; Nireburg, L.: Complex analytic coordinates in almsot complex manifolds,
