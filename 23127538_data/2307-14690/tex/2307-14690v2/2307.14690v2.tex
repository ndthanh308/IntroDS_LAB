...
\begin{document}
	...
	where $\partial$ and $\bar\partial$ are first order differential operators with   bi-degree $(1,0)$ and $(0,1)$ respectively, c.f., \cite[Definition 2.6.9 and Proposition 2.6.15]{Huy}. %Expanding $d^2=0$, one gets that 	\[\partial^2=0,~\bar\partial^2=0,\mbox{ and }\partial\bar\partial+\bar\partial\partial=0.\]
	By $\bar\partial^2=0$, the Dolbeault cohomology  groups $H^{*,*}_{Dol}$ of a complex
	manifold   are defined as the cohomology groups associated with the operator $\bar\partial$ and the space of complex-valued forms, c.f.,  \cite[Section 2 and 3-Chapter 0]{GH}. % On compact complex manifolds, it turns out that all dimensions of $H^{*,*}_{Dol}$ are finite. 
	They provide  complex invariants
	of complex manifolds. Similar to Riemannian manifolds,
	a  complex manifold $M$ equipped with a $J$-invariant metric is called an Hermitian manifold. On a compact complex manifold, the Hodge decomposition shows that $H^{*,*}_{Dol}$ are isomorphic to  ${\bar\partial}$-harmonic spaces with respect to the same bi-degrees, c.f., \cite[Section 6-Chapter 0]{GH}. From the viewpoint of metrics,
	a K\"ahler manifold  is an Hermitian manifold of complex dimension $n$ such that for every point, there is a holomorphic coordinate chart around this point in which the metric agrees with the standard metric on $\mathbb C^n$ up to order $2$  near this point, c.f.,  \cite[Chapter 2 and Chapter 3]{Huy}. On   compact K\"ahler manifolds,  by the K\"ahler identities(\cite[Proposition 3.1.12]{Huy}), one obtains   
	%\[\Delta_d=2\Delta_{\partial}=2\Delta_{\bar\partial}.\]
	%Hence, by Hodge-decomposition  one gets
	\[ b_r =\sum_{j+k=r}h^{j,k}\mbox{ and }
	h^{j,k}=h^{k,j},
	\]
	where $h^{j,k}=\dim_{\mathbb C}H^{j,k}_{Dol}$ is the Hodge number of bi-degree $(j,k)$, c.f., \cite[Section 7-Chapter 0]{GH}. Consequently,  all
	...
	\end{itemize} 
	In this paper, we want to consider a variant  Kodaira's conjecture on the case of almost complex $4$ manifolds.
	...
	\end{tikzcd}\]
	Here an almost complex manifold is  a manifold whose tangent bundle admits a complex structure. 
	From the above diagram,
	it is natural to ask: what is a proper condition for a compact almost complex $4$ manifold   to admit  a symplectic structure? In other words, what should be a proper generalization of the Kodaira's conjecture on almost complex $4$ manifolds? To start with, one can observe that the naive generalization is not valid. 
	...
	\end{emp} 
	On the other hand, there are many   almost complex $4$ manifolds with  even first Betti numbers, but do not admit any symplectic structure, e.g., $(2k+1)\mathbb{C}P^2\# l\overline{\mathbb{C}P^2}$ with integers $k>0$ and $l\geq0$.
	...
	\end{equation}
	where $\mbox{conditions on }h^{1,0}\mbox{ and } h^{0,1}$ means one of the following equivalent equalitties:
	\begin{equation}
...
%Not long ago, to answer Hirzebruch's $20$-th. problem \cite{Hir54},  %In Section 2, we will review  their definition of the Dolbeault cohomology   for almost complex manifolds.  We still denote the Dolbeault cohomology groups   by $H^{*,*}_{Dol}$ and their dimensions by $h^{*,*}$. 
...
We begin from the Hodge numbers to define a refined Dolbeault cohomology $\tilde{H}^{*,*}_{Dol}$ on an almost complex manifold. Roughly speaking, the  refined Dolbeault cohomology is the cohomology of a  complex $$\cdots\mathcal{A}^{*,*-1}_{Dol}\overset{\bar\partial}{\to}\mathcal{A}^{*,*}_{Dol}
\overset{\bar\partial}{\to}\mathcal{A}^{*,*+1}_{Dol}\cdots,$$
where $\mathcal{A}^{*,*}_{Dol}$ is a  certain subspace of the complex-valued forms, c.f, Definition \ref{defi-tilde-Dolbeault}. In particular, they   satisfy  $\partial^2|_{\mathcal{A}^{*,*}_{Dol}}=0=\bar\partial^2|_{\mathcal{A}^{*,*}_{Dol}}$.    We set their dimensions as $\tilde{h}^{*,*}=\dim_{\mathbb C}\tilde{H}^{*.*}_{Dol}$. To answer  Hirzebruch's $20$-th. problem \cite{Hir54}, Cirici and Wilson \cite{CW21} defined the Dolbeault cohomology $H^{*,*}_{Dol}$ by using the relations between $\bar\partial$ and $\bar\mu$ from the decomposition  
\[d=\partial+\bar\partial+\mu+\bar\mu,\]
on almost complex manifolds.   On compact almost complex $4$ manifolds, we have the following inequalities
\begin{equation}
...
We  give a sufficient condition for a compact almost complex $4$ manifold admitting a symplectic structure in terms of the refined Dolbeault cohomology. 
\begin{thm}\label{thm-main-1}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Suppose that $\tilde h^{1,0}=\tilde h^{0,1}$. Then, $X$ admits a symplectic structure.
\end{thm}
Next, we consider the third condition in \eqref{eqn-kahler-condition}. 
Recall that on compact complex surfaces, Gauduchon \cite{Gau76} proved  that the  $\partial\bar\partial$-lemma is equivalent to any one condition in \eqref{eqn-three-identities}. Hence, the  $\partial\bar\partial$-lemma is equivalent to the existence of K\"ahler structure on compact complex surfaces. By importing   new terms  $\hat h^{0,1}$ and $\hat h^1$, c.f., Definition \ref{defi-hat-Dolbeault}, we  obtain an equivalent condition to the  $\partial\bar\partial$-lemma  on compact almost complex $4$ manifolds. 
\begin{thm}\label{thm-ddbar}
...
\begin{center}
	...
	\end{tikzcd}
\end{center} 
On compact complex surfaces, the similar conditions are equivalent, hence it is natural to pose the following question. 
...
\begin{que}
	Let $(X,\omega)$ be a compact symplectic $4$ manifold. Can we find an almost complex structure $J$, such that  the equality
	\[
...
\begin{center}
	\begin{tikzcd}	    
		& \mbox{symplectic 4-manifold}        &              \\
		&    \mbox{ almost complex 4-manifold  }   \arrow[u]    &  \\
		&\mbox{oriented compact smooth  4 manifold}\arrow[u,"\boxed{\mbox{purelly topological obstruction}}"',]&      
	\end{tikzcd}
\end{center}
...
Here, this topological obstruction is well-known, c.f., {\cite[Proposition 9.3-Chapter:  IV]{BHPV}}:
\begin{emp}
	Let $X$ be a compact  oriented smooth $4$ manifold without boundary. Then, $X$ admits an almost complex structure if and only if there is a class $c\in H^2(X;\mathbb Z)$ such that 
	\[c\equiv w_2(X) \bmod2\mbox{ and }
	c^2=2\chi(X)+3\tau(X),\]
	where $\chi(X)$  and $\tau(X)$ are the Euler number and the signature of $X$ respectively. 
\end{emp}
In the study   of $4$ manifolds, there are topological constrains   for a certain kinds of almost complex $4$ manifolds admitting a  symplectic structure by using the Seiberg-Witten gauge theory, see  Bauer's work \cite{Bau06} and Li's work \cite{Li06}. On the other hand, the exotic phenomenon  in smooth $4$ manifolds imply that there are also non-topological obstructions for the existence of a symplectic structure on an almost complex $4$ manifold, see   Fintushel and Stern's work \cite{FS98} on exotic $K3$
surfaces and Taubes' work \cite{Taub94}, \cite{Taub95}. A consequence of Theorem \ref{thm-main-1} and Theorem \ref{thm-ddbar}  is a sufficient condition for a compact  oriented smooth  $4$ manifold admitting a symplectic structure.
...
By using the term $\hat h^{0,1}$, we get a Fr\"ohlicher type inequality on $b_1$. %, which is similar to compact complex surfaces.
\begin{thm}\label{thm-b1-inequality}
	...
	Moreover, when the equality holds $b_1=\tilde h^{1,0}+h^{0,1}$, 
	the differential map $\delta_1: H^{0,1}_{Dol}\to H^{1,1}_{Dol}$ between the Dolbeault cohomology groups is trivial. 
\end{thm}
...
On compact almost complex $4$ manifolds, we show the following relations, c.f., Corollary \ref{cor-b1-ineq-4-mfld}:
\begin{equation}
	2\tilde{h}^{1,0}=2h^{1,0}\leq b_1 \mbox{ and }\tilde{h}^{1,0}\leq\tilde{h}^{0,1}\leq\hat{h}^{0,1}\leq h^{0,1}.
...
b_1=h^{1,0}+h^{0,1}
$
always holds by using the Hodge decomposition and the Atiyah-Singer-Hirzebruch index theory, c.f., \cite[Theorem 2.7-Chapter IV]{BHPV}, and the equalities $\tilde h^{0,1}=\hat{h}^{0,1}=h^{0,1}$ trivially hold. Hence, it is natural to wonder  whether a similar equality holds for almost complex $4$ manifolds. However, by Theorem \ref{thm-main-1} and Inequalities \eqref{eqn-b1-ineq-4mfld}, the equality 
\begin{equation}
	b_1=\tilde h^{1,0}+ \tilde h^{0,1},~b_1=\tilde h^{1,0}+ \hat h^{0,1}\mbox{ or }b_1=\tilde  h^{1,0}+{h}^{0,1}, \label{eqn-b1-hodge-complex-surface}
\end{equation} on the case of compact  almost complex $4$  manifolds does not hold in general. Moreover, we show that the three identities in \eqref{eqn-three-identities} are not equivalent to each other on almost complex $4$ manifolds. % and the equality \eqref{eqn-b1-hodge-complex-surface} does not hold, 
For example, all  manifolds $(2k+1)\mathbb{C}P^2\#l\overline{\mathbb{C}P^2}$ with $k>0$,  $l\geq0$ and  all almost complex structures satisfy the relations $$b_1=2h^{1,0}=2\tilde{h}^{1,0}=0\mbox{ and  }h^{0,1}\geq \hat{h}^{0,1}\geq\tilde h^{0,1}>0,$$
...
% In the case of almost complex $4$ manifolds, the non-integrablity of $\bar\partial$ raises a main difficulty to study this parallel problem coming from compact complex surfaces.   In this paper, we   use the refined  Dolbeault cohomology and the method of solving a certain partial differential equation    to give a  sufficient condition for a compact almost complex $4$ manifold admitting a symplectic structure. By importing two term $\hat{h}^1$ and $\hat h^{0,1}$, we establish the Fr\"ohlicher type inequality on $b_1$ and give the $\partial\bar\partial$-lemma. 
\noindent
This paper is organized as follows: In Section 2, we review the related differential operators on almost complex manifolds, and the Dolbeault cohomology defined by Cirici and Wilson \cite{CW21}; In Section 3, we give the definition of  the refined Dolbeault cohomology and some   relations between  the refined Dolbeault cohomology and the ones defined by Cirici and Wilson; In Subsection 4.1, we show  some analogous  relations about the dimensions of the Dolbeault cohomology groups  on compact almost complex $4$ manifolds and prove Theorem \ref{thm-b1-inequality}; In Subsection 4.2, we give the proofs to Theorem \ref{thm-main-1} and Theorem \ref{thm-ddbar}.
\begin{ack}
	The author would like to express his gratitude to  Xiangdong Yang for introducing the Dolbeault cohomology  on almost complex manifold  and sharing   the work \cite{CW21} and \cite{ST23}, to Feng Wang for sharing \cite{Gom95} and Masaya Kwamura for explaining his work and Gauduchon's result \cite{Gau77}.  Part of the writing of this paper was completed at  Zhuhai Campus of Sun Yat-sen University, the author also thanks Yuan Wei for the hospitality.
\end{ack}
...
\end{defi}
A celebrated theorem of  Newlander and Nirenburg' work \cite{NN} states that an almost structure comes from a complex structure if and only if the associated Nijenhuis tensor 
...
Denoting $ \mathcal A^*(M)$ by the space of sections of  $\bigwedge^*T^*_{\mathbb C}M$, we have the decomposition 
\[ \mathcal A^r(M)=\bigoplus_{p+q=r} \mathcal A^{p,q}(M),\]
where $\mathcal{A}^{p,q}(M)$ denotes the space  of complex valued  forms of $\bigwedge^{p,q}T^*_{\mathbb C}M:=\bigwedge^pT^{*,(1,0)}_{\mathbb C}M\otimes\bigwedge^qT^{*,(0,1)}_{\mathbb C}M$.
By 
...
\begin{defi}
	...
	Moreover, when $\omega$ is $d$-closed, we call the quadruple   $(M,J,g,\omega)$   an
	\textit{almost  K\"ahler} manifold. 
\end{defi}
% i.e. a Riemannian metric satisfying $g(J-,J-)=g(-,-)$,
We give some local calculation for helping  understanding. 
...
$$
and write the structure coefficients of the Lie bracket by $$[Z_j,\bar Z_{k}]=\sum_k C^r_{j\bar k}Z_r+
...
L^jP^{k-2j},
$$
where $P^j=\ker(\Lambda) \cap \mathcal{A}^j$, c.f., \cite[Section 6, Chapter 0]{GH}.  On almost  K\"ahler manifolds, Cirici and Wilson proved the  Hard Lefschetz Duality \cite[Theorem 5.1]{CW20}. In general, on almost K\"ahler manifolds, they gave the following proposition and theorem. 
...
In particular, for  dimension $4$, we have the following identity on $(1,1)$-forms,
\begin{equation}
...
$(M,J)$. If there is no ambiguity, we omit the underline manifold and almost complex structure and  simply write  the cohomology groups by  $H^{p,q}_{Dol}$ with bi-degree $(p,q)$. 
...
\begin{thm}[Cirici and Wilson  {\cite[Theorem 3.8 and Proposition 4.10]{CW21}}]\label{thm-CW-21}
	...
	\begin{itemize}
		...
		\]
		\item[$(2)$] \[
		\ker(\Delta_{\bar\partial}+\Delta_{\bar\mu})\cap\mathcal{A}^{p,q}\subseteq H^{p,q}_{Dol},
		\]
		where the equality holds for $p\in\{0,...,n\}$ and $q\in\{0,n\}$.
	\end{itemize}
\end{thm}
Inspired by the work of Sillari and  Tomassini \cite{ST23}, we define the certain subspaces of $\mathcal{A}^{*,*}$ and the associated Dolbeault type cohomology. 
...
and $H^{p,q}_{Dol}$ on compact almost Hermitian manifolds. 
...
\begin{pf}  
	...
	the linear map $\alpha\in \ker(\mu)\cap\ker(\bar\mu)\cap \ker((\bar\partial+\mu)\bar\partial)\cap\ker(\bar\partial)\cap\mathcal{A}^{p,q} \mapsto [\alpha] \in H^{p,q}_{Dol}$   gives a well-defined  linear map $\tilde H^{p,q}_{Dol}\to H^{p,q}_{Dol}$ for all $(p,q)$. 
	\begin{itemize}
...
\noindent
To compare the dimensions of  Dolbeault cohomology groups with the first Betti number,   we import the following  two terms.
...
\begin{lemma}
	On a compact almost complex manifold, we have an inclusion \[
	H^{1}_{dR}\hookrightarrow \hat{H}^1.\]
\end{lemma}
\begin{pf}
	First,  we notice that the  inclusion $$\ker(d)\cap(\mathcal{A}^{1,0}\oplus\mathcal{A}^{0,1})\to\ker(d^{2,0})\cap \ker(d^{0,2})\cap(\mathcal{A}^{1,0}\oplus\mathcal{A}^{0,1}),$$ 
	gives a canonical map $H^{1}_{dR}\to \hat{H}^1$, where $d^{2,0}$ and $d^{0,2}$ are  $(2,0)$ and $(0,2)$-components of $d$ respectively. Now, we  need to show that this map is an injection. 
	If  a class $[\alpha]\in H^{1}_{dR}$ maps to a trivial class in $\hat{H}^1$. Then, we write $\alpha=\partial f_1+\bar\partial f_2$ for   two functions $f_1$ and $f_2$. Since $d\alpha=0$, one gets that 
	\[\bar\partial\partial f_1+\partial\bar\partial f_2=0, \]
	hence $\partial\bar\partial(f_1-f_2)=0$ by the identity $\partial\bar\partial+\bar\partial\partial=0$ on functions. Applying the maximum principle to the equation $i\partial\bar\partial(f_1-f_2)=0$,  one obtains that $f_1-f_2$ is a constant. Therefore, we write $\partial f_1+\bar\partial f_2=\partial f_1+\bar\partial f_1=df_1$, which is trivial in $H^{1}_{dR}$.
\end{pf}
%Recall the conjugate map $\Lambda^{1,0}\to \Lambda^{0,1}$ is an involution map  coming from the map $i\mapsto -i$, which is $\mathbb R$-linear the anti-$\mathbb C$ linear. Similarly, we define a map $J\mapsto -J$. This induces a $\mathbb{C}$-linear map $\Lambda^{0,1}\to \Lambda^{1,0}$, which is denoted by In particular, This involution map is identity on the space of complex-valued functions. 
...
\begin{pf}
	...
	For any class $[v]\in {\tilde H^{0,1}_{\partial}}$, the class $[ v+ u'+\bar\partial  f]$ belongs to $\ker(\pi)$. Therefore, we get the exact sequence
	\[0\to\tilde{H}^{1,0}_{\partial}\to \hat H^1\to \hat H^{0,1}\to 0,
	\]
...
In this subsection, we show some analogous inequalities about   the dimensions of Dolbeault cohomology groups  on compact almost complex $4$ manifolds. 
...
\begin{lemma}\label{lemma-barpartial}
	Let $(X,J)$ be  a compact  almost complex $4$ manifold. Then, we have 
...
\begin{pf}
	...
	The inclusion $\ker(d)\cap \mathcal{A}^{1,0}\subset \ker(\bar\partial)\cap \mathcal{A}^{1,0}$ is clear.
...
\begin{lemma}\label{lemma-h1}
	We have  $$0\to\tilde  H^{1,0}_{Dol}=H^{1,0}_{Dol}\to\tilde  H^{0,1}_{Dol}\subseteq\hat{H}^{0,1}\subseteq H^{0,1}_{Dol},$$
...
\begin{pf}
	...
	If an  element $u\in\ker(\bar\partial)\cap \mathcal{A}^{1,0}$  can be written as $u=\partial f$ for some  function $f\in\mathcal{A}^0_{Dol}$, then it holds  $\bar\partial \partial f=0$. By applying the maximum principle to the equation $i\bar\partial\partial f=0 $,    $f$ is constant and $u=0$. By the conjugation map, the first map is injective.
\end{pf}

\begin{lemma}\label{lemma-b1-equiality}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Then, it holds that $b_1\geq 2\tilde h^{1,0}$. % Moreover, if $(M,J,g)$ is a compact almost K\"ahler $4$ manifold, then we have that $b_1(X)=2h^{1,0}$.
\end{lemma}
...
\begin{pf}
	...
	These show that
	$$\sigma\wedge\bar\sigma=d(\alpha+\beta)\wedge\bar\sigma.$$ By applying the Stokes lemma and using the identity $d\sigma=\bar\partial\sigma+\bar\mu\sigma=0$, one obtains
	\[
...
\noindent
%Together with  
We show the following lemma, which is similar to the case of compact complex surfaces.
...
%	Since solving  equation $\Delta_{\delta}\alpha=0$ is equivalent to solving $\delta\alpha=0$ and $\delta^*\alpha=0$ for any symbol $\delta\in\{\mu,\partial,\bar\partial,\bar\mu\}$ and any form $\alpha\in\mathcal{A}^{p,q}$.
    We recall	the isomorphisms in Theorem \ref{thm-CW-21} $$\ker(\Delta_{\bar\partial}+\Delta_{\bar\mu})\cap\mathcal{A}^{2,0}\cong H^{2,0}_{Dol}\mbox{ and }
    ...
    By using % the identification $H^{0,2}_{Dol}\cong\ker(\Delta_{\bar{\partial}}+\Delta_{\bar \mu})\cap\mathcal{A}^{0,2}$ and
      the facts that the operators $\bar\partial$ and $\bar\mu$ act trivially on $\mathcal{A}^{0,2}$,  one has that 
    $\sigma\in\ker(\Delta_{\bar{\partial}}+\Delta_{\bar \mu})\cap\mathcal{A}^{0,2}$
...
%	c.f., \cite[Lemma 2.3]{CW20} and, one has   that
   ...
   By using the similar arguments  to $(2,0)$-forms, % and the formulas in \cite[Lemma 2.3]{CW20}, 
   one also gets 
   $$H^{2,0}_{Dol}\cong
   \ker(\Delta_{\bar\partial}+\Delta_{\bar\mu})\cap\mathcal{A}^{2,0}=\ker(\bar\partial)\cap\ker(\bar\mu)\cap\mathcal{A}^{2,0}.$$
...
\begin{pf}
	...
	$H^{0,2}_{Dol}\cong \ker(\partial)\cap\ker(\mu)\cap\mathcal{A}^{0,2}=\mathcal{H}^{0,2}_{\bar\partial,\mu}$. Similarly, we also get $\mathcal{H}^{0,2}_{\bar\partial,\mu}=\ker(\partial)\cap\ker(\mu)\cap\mathcal{A}^{0,2}$, which implies the isomorphism 
	...
	For any $\bar{\partial},\mu$-harmonic $(0,2)$-form $\sigma$, if it is trivial in $\tilde{H}^{0,2}_{Dol}$, then we write $\sigma=\bar\partial\alpha$ for some $(0,1)$-form $\alpha$.  By taking the conjugation on $\sigma$ and using Lemma \ref{lemma-(2,0)-trivial}, we get $\sigma=0$, i.e. $\mathcal{H}^{0,2}_{\bar{\partial},\mu}\to \tilde{H}^{0,2}_{Dol}$ is an injection. 
...
\begin{pf}
	...
	by $u\mapsto [u]$. 
	When $u$ maps to a trivial class, we write $u=\bar\partial f$ for some function $f\in\mathcal{A}^0_{Dol}$. By applying the maximum principle to the  equation $\bar\partial^*\bar\partial f=0$,   $f$ is constant and $u=0$.  
\end{pf}
...
\begin{pf} \textbf{of Theorem \ref{thm-b1-inequality}} By the relation $\hat H^{0,1}\subseteq H^{0,1}_{Dol}$, we only need to prove the first inequality
	$
...
$
	...
	\end{equation} 
	$ \mbox{by } u'+u''\mapsto u''$, where $u'\in\mathcal{A}^{1,0}$ and $u''\in\mathcal{A}^{0,1}$. One can check that the above map  \eqref{eqn-b1-map} induces a   well-defined map  $$p:H^1_{dR}\to \hat H^{0,1}.$$ 
	...
	By the exact sequence \eqref{eqn-exact-b1}, for any $[u]\in H^{0,1}_{Dol}$, there is some $v\in\mathcal{A}^{1,0}$ satisfying $d(u+v)=0$. This means, 
	\[\partial u+\bar\partial v=0\mbox{ and }\bar\partial u+\bar\mu v=0,\]
	for any  $[u]\in H^{0,1}_{Dol}$. 
	Therefore, $\delta_1([u])=[\partial u+\bar\partial v]=0$ is trivial.
\end{pf}

\begin{rmk}
	When $(X,J)$ is a compact  complex surface,  one can establish the similar inequality   by using the sheaf theory,  c.f., \cite[Lemma 2.4-Chapter IV]{BHPV}. %Proposition  \ref{thm-b1-inequality} just %and the following Theorem \ref{thm-betti-equality}  		can prove the inequality.  In the next section, we give an example that the equality does not hold in general. 
...
\begin{pf}
	The first three inequalities follows from Theorem \ref{thm-b1-inequality} and Lemma \ref{lemma-b1-equiality}. 
	\begin{itemize}
		...
		\item[$(2)  $]   Recall that we have two inequalities
...
\noindent
On almost K\"ahler manifolds, Cirici and Wilson \cite{CW20} gave  the equality $\ell^{p,q}=\ell^{q,p}$ by using  the identity $$\Delta_{\bar\partial}+\Delta_{\mu}=\Delta_\partial+\Delta_{\bar\mu},$$ and the  first relation in Theorem \ref{thm-CW-02}. 
For the completeness of this paper, we  show the equality $\ell^{1,0}=\ell^{0,1}$ on almost K\"ahler $4$ manifolds via a simple method rather than using $\bar\partial,\mu$-Laplacian operators. 
\begin{lemma}
...
\end{lemma}
\begin{pf} 
	The first equality is provided by Corollary \ref{cor-ell-h(1,0)}. We just show the second one here. The identity 	$[\Lambda, \bar{\partial}]=-i \partial^*$ in Proposition \ref{prop-kahler-identities} implies $\partial^*v=0$ for $v\in\tilde{H}^{1,0}_{Dol}$, which gives an injection $0\to\tilde{H}^{1,0}_{Dol}\to \mathcal{H}^{0,1}_{\bar\partial,\mu}$ by the conjugation map.
%	The formulas   $J u=*(\omega\wedge u)$ and 
	Similarly, the identity $[\Lambda, \partial]=i \bar{\partial}^*$ in Proposition \ref{prop-kahler-identities} induces  that
	...
	So,  we get that  $du=0$ and  the conjugation map $\tilde{H}^{1,0}_{Dol}\to \mathcal{H}^{0,1}_{\bar\partial,\mu}$ is an isomorphism. 
\end{pf}
...
\begin{pf}
	...
	\end{eqnarray*}   
	where we use  the identity $\bar\partial\partial u=-\partial\bar\partial u-\bar\mu\mu u-\mu\bar\mu u=    \partial^2\bar u+\mu\bar\partial\bar u$ for the second equality and the identity $\partial u+\mu\bar u=0$ for the last one.   
	...
	\end{eqnarray*}    
	where we use the formulas $[\Lambda, \bar{\partial}]=-i \partial^*$, \eqref{eqn-asd} for the second equality and  the formulas $\bar\partial \bar u+\bar\mu u=0$, \eqref{eqn-parital-d} for the last one. Therefore, by the hypothesis of $\lambda_1$, it holds that $u\in\ker(\bar\partial)$, i.e., $u\in\tilde  H^{1,0}_{Dol}$.
\end{pf}
...
\begin{lemma}\label{lemma-partial-solution}
	...
	manifold. Then, $\tilde h^{1,0}=\tilde h^{0,1}$ if and only if for any $\psi\in\mathcal{A}^{1,1}_{\mathbb R}$ satisfying $dd^c\psi=0$, there exists a solution to the equation
	\begin{equation}
		\bar\partial \psi=\partial\bar\partial u+\bar\mu\partial\bar u+\partial\bar\mu \bar u+\bar\mu\mu u,\label{eqn-ddbar}
	\end{equation}
	for some $u\in\mathcal{A}^{0,1}$, where $\mathcal{A}^{1,1}_{\mathbb R}$ denotes the space of real-valued $(1,1)$-forms.
\end{lemma}
\begin{pf}
	...
	The condition $\tilde h^{1,0}=\tilde h^{0,1}$ gives an  exact sequence $$0\to\tilde  H^{1,0}_{\partial}\cong\tilde{H}^{1,0}_{Dol}\to\hat H^1\to\hat H^{0,1}\to0.$$
	By the above exact sequence and the condition $v+\bar v\in \ker(d^{0,2})\cap\ker(d^{2,0})$, we can find some functions $f$ and $f'$ with the property $$d^{2,0}(\partial f+\bar\partial f')=d^{0,2}(\partial f+\bar\partial f')=0,$$
	 such that
	...
	Since $\psi$ is $dd^c$-closed, it holds
	...
	Conversely, we assume that for any $dd^c$-closed form $\psi\in\mathcal{A}^{1,1}_{\mathbb R}$  the equation $$\bar\partial \psi=\partial\bar\partial u+\bar\mu\partial\bar u+\bar\mu\partial \bar u+\bar\mu\mu u,$$
	can be solved for some $u\in\mathcal{A}^{0,1}$. Recall that for any $dd^c$-closed $(1,1)$-form $\psi$, we can write $\psi=\psi'+i\psi''$, where $\psi'$ and $i\psi''$ are the real  and the imaginary parts of $\psi$ respectively.
	Since the operator $dd^c$ is real, the $dd^c$-closeness of $\psi$ is equivalent to the $dd^c$-closeness of both $\psi'$ and $\psi''$.
	Taking the sum of the solutions $u'$ and $u''$ to the equations $$	\bar\partial \psi'=\partial\bar\partial u'+\bar\mu\partial\bar u'+\partial\bar\mu \bar u'+\bar\mu\mu u'\mbox{ and }	\bar\partial \psi''=\partial\bar\partial u''+\bar\mu\partial\bar u''+\partial\bar\mu \bar u''+\bar\mu\mu u''\mbox{ respectively},$$ 
	the assumption implies that for any $dd^c$-closed  $(1,1)$-form $\psi$, we have the solutions $u_1\in\mathcal{A}^{0,1}$ and $u_2\in\mathcal{A}^{1,0}$ to the equation 
...
%	where  
	...
	Again by the Stokes lemma, we rewrite the above formula as 
	\begin{eqnarray*}
	...
	\end{eqnarray*}
	Consequently,    each element in $\ker(\bar\partial)\cap\ker(\mu)\cap\mathcal{A}^{0,1}_{Dol}$  is $\partial$-closed up to some element in $im(\bar\partial)\cap\mathcal{A}^{0,1}_{Dol}$, i.e.,  $\tilde h^{1,0}=\tilde{h}^{0,1}$ by the conjugation map. 
\end{pf}
...
Before proceeding, we verify the following  two identities on almost complex $4$ manifolds.
...
\begin{lemma}
Let $(X,J,g,\omega)$ be a compact almost Hermitian $4$ manifold.     
If $\tilde h^{1,0}=\tilde h^{0,1}$, then there exists a linear map $s:Z^{1,1}_{dd^c}(X,\mathbb R)\to Z^{2}_{d}(X,\mathbb R)$, such that
\begin{enumerate}
...
\end{enumerate}
where $Z^{1,1}_{dd^c}(X,\mathbb R)$ and $Z^2_d(X,\mathbb R)$ are the space of all $dd^c$-closed real-valued $(1,1)$-forms and the space of all $d$-closed real-valued $2$-forms respectively. 
\end{lemma}
\begin{pf}
...
It suffices to prove that for two solutions to the equation  \eqref{eqn-ddbar} induce the same $d$-closed $2$-form.
...
for some $dd^c$-closed $(1,1)$-form $\psi$ with $i=1,2$. 
...
\end{eqnarray*}
i.e., $\bar\partial\gamma+\bar\mu\bar\gamma=0$. Substituting $\gamma=u_1-u_2$ into this equation, it is clear that \[
\bar\partial u_1+\bar\mu \bar u_1=\bar\partial u_2+\bar \mu\bar u_2.
\] 
By taking the conjugation, 
one also gets 
\[
...
\begin{cor}
Let $(X,J)$ be a compact almost complex $4$ manifold. Suppose that $\tilde h^{1,0}=\tilde h^{0,1}$. Then, we have an injective map
\[
...
By using the following Lemma \ref{lemma-nondegenerate},    $\omega'$ is non-degenerate, i.e., $X$ admits a symplectic structure. 
\end{pf}
...
Since the matrix $\left(\begin{array}{cc}
	a&-i  \\
	i & -\bar a
\end{array}\right)$ is invertible, we get that $v_1=\bar v_1=v_2=\bar v_2=0$, i.e., $v$ is zero at $x$. %the form $\psi'$ is nondegenerate at each point. 
\end{pf}
...
\noindent
Now, we show the $\partial\bar\partial$-lemma on compact almost complex $4$ manifolds. 
...
First, we assume that the equality $\tilde h^{1,0}=\tilde h^{0,1}$ holds. Recall that this establishes the exact sequence
...
Since
$[\alpha'+\alpha'']\in\hat H^{1}$,  there are function  $ f'$ and $f''$ satisfying 
\begin{equation}
	d^{2,0}(\partial f'+\bar\partial f'')=0=d^{0,2}(\partial f'+\bar\partial f''),\label{eqn-d-0}
\end{equation}
such that
\begin{equation}
...
and
\[\partial (\alpha'-\partial f')+\mu (\alpha''-\bar\partial f'')=0,\] we get
\[\alpha''-\bar\partial f''\in\ker(\mu)\cap\ker(\bar\partial),\]
i.e., $[\alpha''-\bar\partial f'']\in\tilde H^{0,1}_{Dol}$.
Again by the condition $\tilde h^{1,0}=\tilde{h}^{0,1}$, we can find a function 
\begin{equation}
...
Conversely,  we assume that each $d$-exact  $(1,1)$-form $\psi $ can be  written as $\psi=\partial\bar\partial f$ for some   function $f$. We consider a representative form $u$ in a class $[u]\in \tilde H^{0,1}_{Dol}$. Clearly, the form $\partial u=du$ is a $d$-exact $(1,1)$-form. By the assumption, there is a function $f$ such that 
$$\partial (u-\bar\partial f)=0.$$
We obtain $u-\bar\partial f\in \ker(d)\cap\mathcal{A}^{0,1}$, which implies that $f\in\ker(\partial^2)\cap\ker(\bar\partial^2)$ by $\mu(u)=\bar\partial(u)=0$. 
...
\noindent
At last, we give the following corollary to end this paper.  
\begin{cor}
