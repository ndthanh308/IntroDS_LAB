...
\begin{document}
	...
	\begin{abstract}
		...
		In this paper, we define  a refined Dolbeault cohomology on   almost complex manifolds. We show that the condition $\tilde h^{1,0}=\tilde h^{0,1}$ implies  a symplectic structure on a compact almost complex $4$ manifold, where $\tilde h^{1,0}$ and  $\tilde h^{0,1}$ are the dimensions of the refined Dolbeault cohomology groups with bi-degrees $(1,0)$ and $(0,1)$ respectively. We also show that the refined Hodge number $\tilde{h}^{0,1}$ is finite on any compact almost complex $4$ manifold, which is quite different to the Dolbeault cohomology defined by Cirici and Wilson.  Moreover, we prove that the condition $\tilde{h}^{1,0}=\tilde h^{0,1}$ is equivalent to the $\partial\bar\partial$-lemma. This can be regarded as an analogue of the Kodaira's conjecture on almost complex $4$ manifolds. Meanwhile, we show that   the Fr\"olicher-type equality % $b_1=\tilde h^{0,1}+\tilde h^{1,0}$
		 does not hold on a  general almost complex $4$ manifold, which is  different to the case of  compact complex surfaces.  The main results of this paper can be regarded as the symplectic proof of the Kodaira's conjecture. 
	\end{abstract}
	\noindent
	...
	of  compact complex manifolds, c.f.,  \cite[Section 2 and 3-Chapter 0]{GH}. % On compact complex manifolds, it turns out that all dimensions of $H^{*,*}_{Dol}$ are finite. 
...
We start from the condition (2) in \eqref{eqn-kahler-condition}.  We define a refined Dolbeault cohomology $\tilde{H}^{*,*}_{Dol}$ on an almost complex manifold. Roughly speaking, the  refined Dolbeault cohomology is the cohomology of a  complex $$\cdots\mathcal{A}^{*,*-1}_{Dol}\overset{\bar\partial}{\to}\mathcal{A}^{*,*}_{Dol}
\overset{\bar\partial}{\to}\mathcal{A}^{*,*+1}_{Dol}\cdots,$$
where $\mathcal{A}^{*,*}_{Dol}$ is a  certain subspace of the complex-valued forms, c.f, Definition \ref{defi-tilde-Dolbeault}. In particular, we have $\partial^2|_{\mathcal{A}^{*,*}_{Dol}}=0=\bar\partial^2|_{\mathcal{A}^{*,*}_{Dol}}$.    We set $\tilde{h}^{*,*}=\dim_{\mathbb C}\tilde{H}^{*.*}_{Dol}$.   
...
\begin{thm}\label{thm-main-1}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Suppose that $\tilde h^{1,0}=\tilde h^{0,1}$. Then, $X$ admits a symplectic structure $\omega$.
\end{thm}
 Note that  $J$ is $\omega$-tame by the proof of the above theorem. Combining the Tan, Wang, Zhou and Zhu'work \cite{TWZZ22} on Donaldson's conjecture \cite{Don06}, one can find another symplectic form $\omega'$ such that $J$ is $\omega'$-compactible under the condition $h^-_J=b^+-1$, where $h^-_J$ is the dimension of $J$-anti-invariant second cohomology. 
To answer  Hirzebruch's $20$-th  problem \cite{Hir54}, Cirici and Wilson \cite{CW21} defined the Dolbeault cohomology $H^{*,*}_{Dol}$ by using the relations between $\bar\partial$ and $\bar\mu$ from the decomposition  
\[d=\mu+\partial+\bar{\mu}+\bar{\partial},\]
on almost complex manifolds. On compact almost complex $4$ manifolds, we show the following inequalities
\begin{equation}
...
We  obtain an equivalent condition to the  $\partial\bar\partial$-lemma  on compact almost complex $4$ manifolds. 
...
\begin{center}
	...
	\end{tikzcd}
\end{center} 
On compact complex surfaces, the three conditions are equivalent to each other.  Hence, it is natural to pose the following question. 
...
\begin{emp}
	...
	c^2=2\chi(X)+3\tau(X),\]
	where $w_2(X)$ is the second Stiefel-Whitney class of $X$,  $\chi(X)$  and $\tau(X)$ are the Euler number and the signature of $X$ respectively. 
\end{emp}
...
Similar to the case of compact complex manifolds, we give a Fr\"olicher type inequality on $b_1$. %, which is similar to compact complex surfaces.
...
Very recently, Coelho,  Placini and Stelzig \cite{CPS} showed that the Hodge number $h^{0,1}$ is  finite if and only if such a compact almost complex $4$-manifold is a compact complex surface. However, we can show that $\tilde{h}^{0,1}$, $\hat{h}^{0,1}$ and $\hat h^1$ are finite on compact almost complex $4$ manifolds, see Proposition \ref{prop-finite}. Such a result is quite different to the Dolbeault cohomology defined by Ciric and Wilson . 
...
Moreover, on compact almost complex $4$ manifolds, we show the following relations, c.f., Corollary \ref{cor-b1-ineq-4-mfld}:
\begin{equation}
...
For example, the  manifold $(2k+1)\mathbb{C}P^2\#l\overline{\mathbb{C}P^2}$ with $k>0$,  $l\geq0$ and  any almost complex structure satisfies the relations $$b_1=2h^{1,0}=2\tilde{h}^{1,0}=0\mbox{ and  }\infty=h^{0,1}> \hat{h}^{0,1}\geq\tilde h^{0,1}>0,$$
...
\noindent
This paper is organized as follows: In Section 2, we review the related differential operators on almost complex manifolds, and the Dolbeault cohomology defined by Cirici and Wilson \cite{CW21}; In Section 3, we give the definition of  the refined Dolbeault cohomology and discuss some   relations between  the refined Dolbeault cohomology and the ones defined by Cirici and Wilson; In Subsection 4.1, we show  some analogous  relations about the dimensions of the Dolbeault cohomology groups  on compact almost complex $4$ manifolds and prove Theorem \ref{thm-b1-inequality}; In Subsection 4.2, we give the proofs to Theorem \ref{thm-main-1} and Theorem \ref{thm-ddbar}.
\begin{ack}
...
On  almost complex manifolds,  the following identities \cite{ST23} hold:
\[d^2=0=(d^c)^2\mbox{ and }\frac{i}{2}(dd^c+d^cd)=\partial^2-\bar\partial^2.\]
In particular, in the case of  dimension $4$, we have the following identity on $(1,1)$-forms,
\begin{equation}
...
$(M,J)$. If there is no ambiguity, we omit the underline manifold and almost complex structure and  simply write  the cohomology as $H^{*,*}_{Dol}$.% with bi-degree $(p,q)$. 
...
\begin{prop}\label{prop-finite}
	Let $(X,J)$ be a compact almost complex $4$ manifold. Then,  $\tilde{h}^{0,1}$ and $\hat{h}^{0,1}$ are finite. Moreover,  $\tilde{h}^{0,1}=h^{0,1}$ if and only if $(M,J)$ is a compact complex surface. 
\end{prop}
%In Section 3,  one will see the proof of the above theorem. 

\begin{pf} %{\bf of Theorem \ref{thm-finite}}
	By the direct calculus of  \cite[Appendix]{CW21}, it is known that $\hat{H}^{0,1}=E^{0,1}_2=E^{0,1}_\infty$ in real dimension $4$, which is finite by the first statement of Theorem \ref{thm-CW-21}. 
	By the relation 
	\[\tilde{H}^{0,1}_{Dol}\subseteq \hat H^{0,1},\]
	and Lemma \ref{lemma-b1-h1}, $\tilde{h}^{0,1}$, $\hat{h}^{0,1}$ and $\hat{h}^1$ are finite. 
	Since the condition $h^{0,1}<\infty$ is equivalent to  $(X,J)$ is a compact complex surface, this also gives that  $\tilde{H}^{0,1}_{Dol}=H^{0,1}_{Dol}$  is equivalent to  $(X,J)$ is a compact complex surface. Thus, we proved the theorem. 
\end{pf} 
...
We end this section by showing that $\tilde{H}^{1,0}_{Dol}$ and $\tilde{H}^{0,1}_{Dol}$ are  trivial on  some compact almost complex manifolds with $\dim_{\mathbb R}\geq6$. 

\begin{lemma}\label{lemma-trivial}
	Let $(M,J)$ be a compact   almost complex  $2n(\geq6)$-manifold   such that the associated  Nijenhuis tensor takes the maximal rank at some point. Then, $H^{1,0}_{Dol}(M,J')=\tilde{H}^{1,0}_{Dol}=\{0\}$. Moreover, if the Nijenhuis tensor takes the maximal rank anywhere, then $\tilde{H}^{0,1}_{Dol}=\{0\}$.  
\end{lemma}
\begin{pf}
	...
	When $n\geq3$, it is clear that $\bar\mu|_U:\mathcal{A}^{1,0}(U)\to\mathcal{A}^{0,2}(U)$ is either  isomorphic($n=3$) or injective($n>3$) by the algebraic calculus. The equation $\bar\mu(u)=0$ gives that $u|_U=0$ for any $u\in\mathcal{A}^{1,0}$.
	Hence,    the unique-continuity-property for harmonic forms shows that $u=0$ for any $u\in\tilde{H}^{1,0}_{Dol}$. 
	
	Similarly, 
	when $N$ is nowhere vanishing,  $\mu:\mathcal{A}^{0,1}\to\mathcal{A}^{2,0}$ is either  isomorphic($n=3$) or injective($n>3$). Therefore, $\mu(v)=0$ implies $v=0$ for any $v\in\mathcal{A}^{0,1}$, which proves the last statement. 
\end{pf}

% New results in almost Kahler
%Set $H_1=\{f\in L^2_1(M)\mid~\mu\bar\partial f=\bar\mu\partial f=0\}$ as a closed subspace of $L^2_2$.  The continuity of $\mu\bar\partial:L^2_1(M)\to L^2(\mathcal{A}^{2,0})$ shows that $\mathcal{A}^0_{Dol}$ is dense in $H_1$. Similarly, set$H_2=\{a\in L^2(\mathcal{A}^{0,1})\mid~\mu(a)=0\}$ as a closed subspace of $L^2(\mathcal{A}^{0,1})$  and $H_3=L^2(\mathcal{A}^{0,2})$.  $\mathcal{A}^{0,1}_{Dol}$ is also dense in $H_2$ by the continuity of $\mu:L^2(\mathcal{A}^{0,1})\to L^2(\mathcal{A}^{2,0})$. Consider the complex \[H_1\overset{T}{\longrightarrow}H_2\overset{S}{\longrightarrow}H_3,\]where $T=\bar\partial$ and $S=\bar\partial$, whose domains $D(T)$ and $D(S)$  are $\mathcal{A}^0_{Dol}$ and $\mathcal{A}^{0,1}$ respectively. It is clear that $S\comp T=0$ and $\frac{\ker(S)}{Im(T)}$ is of finite dimension.  By the toy model theorem of ...., it is known that $T$ has a closed range, i.e., $im(T)=\overline{im(T)}$. One can find a representative $a$ in each class $\mathfrak{a}\in\tilde{H}^{0,1}_{Dol}$ such that $a\perp im(T)$.  By \cite[Lemma 5.4]{Taub96}, for each $p\in X$ there exists a neighborhood $U$ and a $J$-fiber-diffeomorphism $f:D\times D\to U$ such that $f(0\times D)$ and $f(D\times w)$ are pseudo-holomorphic discs for each $w\in D$, where $D\subset \mathbb C$ is a small disc.  For a pump function $\rho$ on $D$, consider $h(z,w)=(f^{-1})^*\rho(z)\rho(w)$, which is supported in $U$. We claim $h\in\mathcal{A}^0_{Dol}$ by the direct  calculation \begin{eqnarray*}  \bar\partial^2h=(\bar\partial^2\rho(z))\rho(w)+\rho(z) 	(\bar\partial^2\rho(w)) \end{eqnarray*} \begin{itemize} 	\item For all $w'\in D'$, $f(D'_{w'})$ is a $J_1$-holomorphic submanifold containing $(0,w')$. 	\item For all $w'\in D'$, $dist((\xi',w'),f'(\xi',w'))\leq z\cdot\rho'\cdot|\xi'|$.  	\item For all $w'\in D'$, the derivative of order $m$ of $f'$ are bounded by  	$z_m\cdot\rho'$. \end{itemize} In particular, all the discs $f'(D'_{w'})$ are transverse to $f(D_0)=f'(0\times D')=0\times D'$. 
...
\begin{lemma}\label{lemma-partial-solution}
	...
	manifold. Then, the condition $\tilde h^{1,0}=\tilde h^{0,1}$ holds  if and only if for any $\psi\in\mathcal{A}^{1,1}_{\mathbb R}$ satisfying $dd^c\psi=0$, there exists a solution  $u\in\mathcal{A}^{0,1}$ to the equation
	\begin{equation}
...
\begin{pf}
	...
	By the above exact sequence and the condition $v+\bar v\in \ker(d^{0,2})\cap\ker(d^{2,0})$, there are two functions $f$ and $f'$ with the property $$d^{2,0}(\partial f+\bar\partial f')=d^{0,2}(\partial f+\bar\partial f')=0,$$
	such that
	...
	The $dd^c$-closeness of $\psi$ implies that
	...
	can be solved for some $u\in\mathcal{A}^{0,1}$. Recall that for any $dd^c$-closed $(1,1)$-form $\psi$, it can be written as $\psi=\psi'+i\psi''$, where $\psi'$ and $i\psi''$ are the real  and the imaginary parts of $\psi$ respectively.
	Since the operator $dd^c$ is real, the $dd^c$-closeness of $\psi$ is equivalent to  those  of both $\psi'$ and $\psi''$.
...
Since
$[\alpha'+\alpha'']\in\hat H^{1}$,  there are two function  $ f'$ and $f''$ satisfying 
\begin{equation}
...
Again by the condition $\tilde h^{1,0}=\tilde{h}^{0,1}$, there is a function 
\begin{equation}
...
\bibitem{Taub96}
Taubes, Clifford Henry: SW$\Rightarrow$Gr: From the Seiberg–Witten equations to pseudoholomorphic
curves. Journ. Amer. Math. Soc., \textbf{9}(1996), 845-918.
