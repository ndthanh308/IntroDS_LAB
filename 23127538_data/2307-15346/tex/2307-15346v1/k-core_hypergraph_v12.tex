...
These definitions pertain to specific multi-node interactions in which the removal or damage of one interacting agent disrupts the interaction among the remaining agents (e.g., supply chain, protein interaction networks, networks of chemical reactions). 
In the present work 
...
These phase diagrams are significantly more rich than for the $k$-cores in ordinary graphs, where the phase transition for the $2$-core is continuous, while the phase transitions for $(k\geq3)$-cores are hybrid. 
In particular, for the $(2,2)$-core we observe two transition lines on the phase diagram---the continuous transition line and the hybrid transition one. These lines converge at the tricritical point.  
Furthermore, we introduce novel $k$-core problems, in which the pruning process accounts not only for the closest neighborhood of a node (i.e., it accounts not only for hyperedges adjacent to the node but also for all their nodes). 
The point is that 
...
We show that the second-neighbor $k$-core percolation involving exclusively pruning of the nodes or involving exclusively pruning of the hyperedges are distinct, and we relate these models to both hypergraph $(k,n)$-cores depending on the closest neighborood and to factor graphs $(k,n)$-core.  In particular, we show that the percolation threshold for the second-neighbor $k$-core problems with pruning of the nodes coincides with the percolation threshold for the first-neighbor hypergraph $k$-core problems, however in the liming case in which only hyperedges are initially damaged, the second-neighbor $k$-core problems with pruning of the hyperedges coincides with the percolation threshold of the factor graph $k$-cores.

These results are obtained within a message-passing theory \cite{newman2023message,weigt2006message,bianconi2018multilayer,cantwell2023heterogeneous} exact for locally tree-like hypergraphs and within the theory of critical phenomena on networks supported by simulations. 
...
to random hypegraphs \cite{bianconi2022statistical,newman2009random,yoon2011belief} with given cardinality and degree distributions.
...
Section~\ref{s7} introduces the basic definitions and notations for hypergraphs, in particular, the definition of the  subhypergrapha of a hypergraph. 
In Sec.~\ref{s4p} we explore the $(k,n)$-core problem for hypergraphs (the first-neighbor version in the sense of nodes and factor nodes in factor graphs). 
In Sec.~\ref{second_neighbor} we describe the second-neighbor version of the $(k,n)$-core problem for hypergraphs.  
...
\eea
Note that the message $\hat\sigma_{i\to j}$ indicates whether node $j$ connects node $i$ to the giant component ($\hat\sigma_{i\to j}=1$) or not ($\hat\sigma_{i\to j}=0$) and it is defined assuming that node $j$ is already in the giant component (see for instance discussion of the message-passing algorithm in multilayer networks with link overlap \cite{cellai2016message}).
...
Under the assumption that $j$ is connected to the giant component,i.e. $\hat{\sigma}_{j\to i}=1$, substituting the above expression for $s_i$ into Eq.~(\ref{per_mess})  for $\hat{\sigma}_{i\to j}$ we obtain then that at stationarity the messages  $\sigma_{i\to j}$ satisfy
%%
\bea
...
\eea
%%
while $\sigma_i=s_i$ is given by Eq.~(\ref{0100}).
...
The hypegraph can be always represented as a factor graph which is a bipartite networks having two types of nodes, namely, the nodes corresponding to the nodes of the hypegraphs and the factor nodes corresponding to the hyperedges of the hypergraph.
Accordingly, we can choose to iteratively prune only nodes, only  hyperedges, or both nodes and hyperegdes. 
...
%%
We note that node and hyperedge percolation are distinct as outlined in Ref.~\cite{bianconi2023theory} and that a hyperedge is in the giant component only if (i) it is not initially damaged; (ii) none of its nodes are initially damaged; (iii) at least one of its nodes is connected to the giant component. For this reason the considered hypergraph percolation algorithm is distinct from the factor graph pruning percolation  (see Fig.~\ref{f1}). 
%An important consequence of the hypergraph pruning process is that for any hyperedge within the $(k,n)$-core, having cardinality $m\geq n$, it is unnecessary to check whether its nodes are undamaged as if they are not all undamaged, the hyperedge will not belong to the giant component at step (1) and hence will not belong to  the $(k,n)$-core. 
...
\eea
%%
see Ref.~\cite{bianconi2023theory}. 
The indicator functions $\hat\sigma_i\in \{0,1\}$ and $\hat{r}_{\alpha}\in \{0,1\}$ indicating whether nodes and hyperedges are in the giant component  are expressed in terms of $s_i$ and $s_{\alpha}$ and are  given respectively by the equations \cite{bianconi2023theory}
%%
\bea
...
\eea 
%%
The pruning of the nodes determines the indicator function $s_i$ which is  only non-zero if the node $i$ receives at least $k$ positive messages from its neighbors, i.e. 
%%
\bea
...
\eea
%%
The pruning of the hyperedges determines $s_{\alpha}$ which is  simply given by 
%%
\bea
...
\eea
%%
where $\theta(x)=1$ if $x\geq 0$ and $\theta(x)=0$ otherwise.
Inserting these expressions of $s_i$ and $s_{\alpha}$ into  Eq.~(\ref{hv}), considering that the messages $\hat{v}_{\alpha\to i}$ is defined under the assumption that node $i$ is in the giant component and $\hat{w}_{i\to \alpha}$ is defined under the assumption that hyperedge $\alpha$ is in the giant component, and exploiting the fact that the messages take only the $0,1$ values, we get
%%
\bea
...
\eea
%%
Exploiting furthermore the fact that both $s_i,s_{\alpha}$ and the messages $\hat{w}_{i\to \alpha},\hat{v}_{\alpha\to i}$ are taking values $0,1$, it is also immediate to show that 
%%
\bea
...
%%
\bea
{w}_{i\to \alpha}&=&p_N\sum_{\substack{\Theta \subseteq N(i)\setminus\alpha \\ |\Theta| \geq k-1}}\ \ \prod_{\beta \in \Theta} \hat{v}_{\beta \to i}\prod_{\gamma \in N(i)\setminus (\Theta\cup \alpha)} (1 - \hat{v}_{\gamma \to i}),
\nonumber 
\\
...
{r_{\alpha}}&=&p_H\theta(|\alpha|-n)\prod_{j\in N(\alpha)}\!{w}_{j\rightarrow \alpha}.
\eea
...
Here we consider the ensembles of random hypergraphs $\mathcal{H}=(V,H)$ of $N=|V|$ nodes and $M=|H|$ hyperedges whose node degree distribution  is $P(q)$ and whose distribution of hyperegde cardinalities is $Q(m)$. In this ensemble the probability of a hypergraph $\mathcal{H}$ of incidence matrix ${\bf b}$ is given by 
\bea
...
\eea
%%
Similarly  the fractions of vertices, $S_{kn}$, and hyperedges, $R_{kn}$, belonging to the $(k,n)$-core, can be obtained by Eq.~(\ref{MSkn}) and Eq.~(\ref{MRkn}) by averaging over $P(\mathcal{H})$ giving 
%%
\bea
&&
...
also displaying the tricritical point at which the $(2,2)$-core percolation transition changes from continuous  to discontinuous (hybrid). 
...
\\[3pt]
&&
= \avg{q}\frac{\avg{q} + 1 + \sqrt{(\avg{q} - 1)^2 -4}}{\avg{q} + 1 - \sqrt{(\avg{q} - 1)^2 -4}}
.
\label{44060}
\eea
%%
...
This is due to the fact that in   hypegraph percolation hyperedges  in order to be belong to the giant component must have  all their nodes  undamaged while no corresponding constraint holds for the nodes. 
...
\eea 
%%
The pruning of the hyperedge determines the indicator function $s_\alpha$ which is  only non-zero if the hyperedges is connected to al least $n$ nodes, each connected to at least $k$ undamaged hyperedges (belonging to  the $(k,n)$-core/giant component). Let us define a node $i$ to be active if it receives at least $k$ positive messages from its neighbors, i.e. 
%%
\bea
...
\eea
%%
where $\tilde{y}_{\alpha}$ is given by 
%%
\bea
...
the all intact  nodes will belong to the giant component unless both  (i) and (ii) are satisfied. These two conditions are: (i) the node is  connected to no hyperedge including at least to $n$ other active nodes; (ii) the nodes belongs to less than  $k$ hyperedges linked to  $n-1$ other active nodes. It follows from this that 
...
Intuitively a active node will be always part of the 2nd neighbor node $(k,n)$ core. An intact inactive node with be part of the $(k,n)$ core  only if it belongs to at least on hyperedge that belongs to the $(k,n)$ core. It follows that a node will be always in 2nd neighbor node $(k,n)$ core unless (i) none of its hyperegdes is connected to at least $n$ other active nodes- which occurs with probability $\hat{V}$- and (ii) there are less than $k$ hyperedges connected to $n-1$ active nodes-which occurs with probability $\tilde{V}$. Note that condition (ii) together with (i) ensures that the node is not active.
...
The $k$-core decomposition is a widely used tool for the discovery of highly connected substructures within complex networks, which essentially determine the character of  cooperative and spreading phenomena in networks. We demonstrate that the set of $k$-core problems for hypergraphs is much richer than for ordinary graphs.   
...
highlight the important differences between hypergraphs and factor graphs and will contribute to a better understanding of specific critical phenomena in higher-order networks. 
