...
\begin{abstract}
The year 2021 marked a new era in the study of fission dynamics, as both new experimental and theoretical 
results made it acutely clear that more than 80 years after the discovery of nuclear fission we do not understand the generation and dynamics of fission fragment intrinsic spins well, in particular their magnitudes, their spatial orientation, 
and their correlations. The magnitude and orientation of the primary fission fragments have a crucial role in defining 
the angular distribution and correlation between the emitted prompt neutrons, and subsequent emission of statistical (predominantly E1)
and stretched E2 $\gamma$-rays, and their correlations with the final fission fragments. Here we present detailed microscopic  
evaluations of the fission fragment intrinsic spins, for both even- and odd-mass fission fragments,
and of their spatial correlations, which point to well-defined 3D dynamics and
characteristics predicted only in recent theoretical studies, and absent in 
semi-phenomenological studies used so far in literature. Our results indicate that bending and 
wriggling relative collective rotational modes of the two fission fragments are equally 
probable and twisting modes are also clearly present.
...
studies followed ~\cite{Marevic:2021,Randrup:2021,Stetcu:2021,Bulgac:2022b,Bulgac:2022e,Randrup:2022,Randrup:2022a,Scamps:2022,Scamps:2023}, 
 ...
 than the light FF (LFF).  At the same time, the average intrinsic spin of the HFF is smaller compared to the LFF, demonstrated in a recent publication of the first fully microscopic study on the FF intrinsic spin distributions~\cite{Bulgac:2021}. This was opposite to the prior consensus in literature, namely that the HFF 
...
FREYA model~\cite{Vogt:2021,Randrup:2021,Randrup:2022}.
The event-by-event orientation of the FF intrinsic spins has important 
consequences, as it will control, to a large degree, the direction of emission for prompt neutrons and gammas, which is correlated with the direction the FFs travel post scission. This is one of the most pressing questions experiment 
should now address~\cite{Sobotka:2023}. In the present microscopic study, we will specifically address this aspect and make a clear statement about
where the most advanced microscopic theory today stands, with a result starkly different from 
...
clear information available currently in literature.
 ...
 The first indication that the angular distribution between the FF intrinsic spins is likely very different from previous models was reported in 
Refs.~\cite{Bulgac:2022b,Bulgac:2022e}. This opposed what phenomenology at the time pointed to~\cite{Vogt:2021,Randrup:2021,Randrup:2022}. 
In FREYA~\cite{Vogt:2021,Randrup:2021,Randrup:2022} the angular distribution was almost uniform, while the 
microscopic results showed a clear non-uniformity. 
...
At the time, when this study~\cite{Bulgac:2022b}  
was performed, an angular momentum projection on several angles was out of question. 
Today, two new technical  developments have made such a full study possible. 
The analysis of the real-time microscopic time-dependent density functional theory extended to superfluid systems~\cite{Bulgac:2019} 
...
step allows us to represent various observables with a significantly smaller set of quasi-particle wave functions. The second development 
was the realization that one can easily evaluate the triple distribution $P(\Lambda,S_H,S_L)$ exactly, without any additional assumptions or
approximations. 
...
&P(S, K) = \langle \Psi | \hat P_{K K}^S | \Psi \rangle, \label{eq:Prob_JK}
\end{align}
with $\Omega=(\alpha,\beta,\gamma)$ representing a separate set of the three Euler angles  for each FF, and
$|\Phi\rangle $ representing the many-body wavefunction. The angular momenta $\hat S_{x,y,z}$ are defined 
...
$P(S, K) $ is  probability distribution for $K$.
 
Our goal is to calculate  the triple angular momentum distribution 
\begin{align}
P(\Lambda,S_H,S_L)&= \sum_{k_H k_L} \langle \Psi | \hat P^{\Lambda}_{0,0} \hat P^{S_H}_{k_Hk_H} \hat P^{S_L}_{k_Lk_L} |\Psi\rangle, \label{eq:Triple_proj}
\end{align}
where $\hat{\bm \Lambda} = - \hat{\bm S}_H - \hat{\bm S}_L$.
The above triple distribution can be shown to be given exactly by the expression 
\begin{widetext}
\begin{align}
P(\Lambda,S_H,S_L) &= \sum_{ k_H k_L k_H' k_L'}  (-1)^{k_H'-k_H+k_L'-k_L}  
C_{S_H,-k_H,S_L,-k_L}^{\Lambda,0}  
C_{S_H,-k_H',S_L,-k_L'}^{\Lambda,0}  \langle \Psi | \hat P^{S_H}_{k_H k_H'} \hat P^{S_L}_{k_L k_L'} | \Psi \rangle. \label{eq:Triple_proj2}
\end{align}
\end{widetext}
with $C_{j_1,m_1,j_2,m_2}^{J,M}$ the well-known Clebsch-Gordan coefficients. 
...
One should appreciate the complexity of the triple projection formula in Eq.~\eqref{eq:Triple_proj2}, which at first sight requires the 
evaluation of a 6-dimensional overlap,  as there are 3 Euler angles for each FF. One can easily show that in the case of $K_H+K_L=0$, 
only 4 angles are needed. Moreover, since the commuting
operators $\hat P^{S_H}_{k_H k_H'} \hat P^{S_L}_{k_L k_L'}$ act on a generalized Slater determinant, each of these projectors rotates quasiparticle 
wave-functions, each one of which has a well defined and non-vanishing angular momentum $k$ along the fission axis. 
The presence of the Clebsch-Gordan 
coefficients ensure that the triangle constraint Eq.~\eqref{equ:3} is automatically satisfied. These Clebsch-Gordan coefficients 
naturally emerge when evaluating the overlap in Eq.~\eqref{eq:Triple_proj}. Numerical evaluations were performed using the 
...
The distributions for the projections of each FF spin on the fission axis are shown in Fig.~\ref{fig:fig1}. Here one sees the first new aspects 
...
the corresponding FF is either an even-even (both $Z_F$ and $N_F$ are even) or an odd-odd nucleus  (both $Z_F$ and  $N_F$ are odd), 
and the corresponding $A_F=Z_F+N_F$ is even. In the case of half-integer FF spins and $K$-values, the corresponding $A_F$ is odd. One should also note that the presence 
of non-zero $K$-values is at odds with FREYA model, where only $K=0$ is allowed.
Additionally, as expected, the range of LFF intrinsic spins $S_L$ is wider than the range of the HFF intrinsic spins $S_H$, 
...
the probabilities to find even and odd mass FFs are almost equal. This is consistent with little or no odd-even staggering observed in experimental mass yields. Note that pre-neutron emission mass yields are corrected for neutron emission, correction that is subject to model dependence. %\textcolor{blue}{Could say here: in agreement with experiment.} 

 % Figure environment removed  

Related to the data shown in Fig.~\ref{fig:fig1} is the data shown in Fig.~\ref{fig:fig2}, where the probability distributions 
$P(K)= P_H(K)=P_L(K)=\sum_{S_{L,H}}P(S_{L,H},K_{L,H})$ %only for  $K\ge 0$ 
are shown.
%, since $K_H+K_L=0$ and $P(K)=P(-K)$. 
For both fissioning nuclei $^{252}$Cf and $^{236}$U 
one observes a very large peak corresponding to $|K| \le 1/2$, followed by quite long tails. In the cases of $^{252}$Cf and $^{236}$U, the
sum $P(-1/2)+P(0)+P(1/2) \approx 0.33$ and  $\approx 0.49$ respectively. Correspondingly, this implies that the probability to 
find a FF with $|K| \le 1$ is 0.67 for $^{252}$Cf and $0.51$ for $^{236}$U. This is particularly important, as it points to the fact 
that the FF intrinsic spins are, with very large probability, not perpendicular to the fission axis. Instead, they are most likely to be found opposite to each other with respect to the fission axis (as  $K_H+K_L=0$). As a result, the plane defined by the triangle formed via the three angular momenta $\hat{\bm S}_H +\hat{\bm S}_L + \hat{\bm \Lambda} = {\bm 0}$ forms an angle 
significantly different than 90$^\circ$ with the fission axis for very large fraction ($\ge 1/2$) of fission events. The lower panel of Fig.~\ref{fig:fig2} 
...
The wide range of $K_{H,L}$ values is particularly important, as they 
control the so-called FF twisting degrees of freedom, whose role was basically ignored in the 
semi-phenomenological FREYA model~\cite{Vogt:2009,Vogt:2013,Vogt:2021,Randrup:2021}, based on an argumentation 
in the treatment of transfer-induced transport in nuclear collisions~\cite{Randrup:1979,Randrup:1982}. In this respect 
one should also notice the role played by Coulomb re-orientation effects of the separated FFs~\cite{Scamps:2022,Scamps:2023},
which can lead to the increase of the FF intrinsic spins by 1-2 $\hbar$.  Additionally, it contributes to the wriggling motion of the FFs, which otherwise 
...
In Fig.~\ref{fig:fig3} we show the distribution of the opening angle between the FF intrinsic spins. This distribution, as reported in 
...
at the fission workshop~\cite{workshop:2022}. As mentioned above, at the time, due to technical difficulties 
Refs.~\cite{Bulgac:2022b,Bulgac:2022e}, we were not able to perform a full momentum projection and had to rely 
on the one-angle formulae.
%assumption that the probability to find any triangle orientation in space $\hat{\bm S}_H +\hat{\bm S}_L + \hat{\bm \Lambda} = {\bm 0}$
%is equiprobable.
Presently, this has been overcome and 
the exact triple angular momentum distribution is shown in Fig.~\ref{fig:fig3}. As intuited by T. D\o{}ssing during 
the workshop~\cite{workshop:2022}, there was a need for a Clebsch-Gordan coefficient to enforce the triangle constraint. %, an aspect somewhat
%at odds with the (reasonable) assumption used in Refs.~\cite{Bulgac:2022b,Bulgac:2022e}, that all triangle orientations in space are equiprobable. 
The final results, shown in the upper panel of Fig.~\ref{fig:fig3}, are  closer to the  FREYA almost uniform 
predictions~\cite{Vogt:2021,Randrup:2021,Randrup:2022} for angles $(50^\circ,150^\circ)$.  However, unlike FREYA predictions, the probabilities vanish at $\varphi_{HL} = 0^{\circ}$ and 180$^\circ$,
and obtain a prominent peak slightly above $\varphi_{HL}\approx 20^\circ$, and a smaller peak at $\approx 165^\circ$. These two peaks originate from the Clebsch-Gordan coefficients appearing Eq.~\eqref{eq:Triple_proj2} favoring angles $\varphi_{HL}$ close to $0^\circ$ 
and to a lesser extent $180^\circ$. In the lower panel of Fig.~\ref{fig:fig3}, we compare the distribution obtained with the exact triple distribution 
from Eq.~\eqref{eq:Triple_proj2} with a pure 2D distribution when only $K_{H,L} \equiv 0$ are allowed, which should be equivalent to the FREYA predictions, apart from the 
behaviors at  $\varphi_{HL}=0^\circ$ and $180^\circ$.  We also compare it to a 3D distribution and a 3D classic distribution, 
equivalent to the distribution in the limit $\hbar\rightarrow 0$ in Refs.~\cite{Bulgac:2022b,Bulgac:2022e}.  The classical version of the triple projection is also shown using the classical limit of the Clebsch-Gordan coefficients \cite{BRUSSAARD:1957}.

The emerging lesson from our microscopic calculations 
is that the FF intrinsic spins dynamics is indeed of a 3D character. It is not restricted to the plane perpendicular to the fission axis, 
as in the classical treatment of the FF intrinsic spins of \textcite{Randrup:2022}. It is also not fully unrestricted in 3D, as initially assumed in Ref.~\cite{Bulgac:2022b}.
The distribution of the angle between  the  FF intrinsic spins reported initially in
Ref.~\cite{Bulgac:2022b}, with details clarified and reported here in Figs.~\ref{fig:fig1}, \ref{fig:fig2} and \ref{fig:fig3}, 
...
the $nn$ and $pp$ short-range correlations (SRCs) never vanished. 
...
can be accompanied by the formation of new nucleon pairs with non-zero total spin.  Nevertheless, the SRCs obviously survive in $L=0$.
...
FREYA model~\cite{Vogt:2021,Randrup:2021,Randrup:2022}, which is based on a number of fitting parameters, and 
the microscopic  treatment of fission dynamics, both lead to starkly different predictions.  It is hopefully this difference will be addressed by 
future experiments, which will be hopefully interpreted in fully parameter-free theoretical treatments.  The microscopic framework adopted in this study is based on a nuclear energy density functional, which depends only 8 parameters (saturation density and energy of symmetric nuclear matter, spin-orbit and pairing couplings, proton charge, nuclear surface tension (related to the nucleon-nucleon interaction range), symmetry energy and to a less extent its density dependence~\cite{Shi:2018,Bulgac:2022f}, whose values are well-known for decades. 
...
very wide fluctuations. In the present exact treatment of the FF intrinsic spin 
evaluations, the distribution $P(\varphi_{HL})$ 
shows an almost uniform distribution in the interval 
...
We thank K. Godbey for a number of discussions of preliminary results.  We also thank L. Sobotka for his comments on and reading of the manuscript.
