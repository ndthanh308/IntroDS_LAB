...
\title[Boundary Harnack in the slit domain]{The $C^{1,\alpha}$ boundary Harnack in slit domains and its application to the Signorini problem}
\author{Chilin Zhang}
...
\end{equation*}
in an domain $D\subseteq\mathbb{R}^{n+1}$ under the further constrain that $U\geq\psi$ on a $n$-dimensional hypersurface $M\subseteq D$ dividing $D$ into $D_{1}$ and $D_{2}$. One can imagine a scenario of a carpet hanged over a string. The minimizing solution satisfies
...
\begin{definition}
    Let $U\geq\psi$ on $M$ and satisfies \eqref{general Signorini}. The free boundary $\Gamma\subseteq M$ is defined as $\Gamma:=\partial\Big|_{M}\{U=\psi\}$.
\end{definition}
...
Let's first look at the constant coefficient model, or called the ``model problem S" in \cite{PSU12}. We assume $A=\delta^{ij}$, $F,\psi=0$, $M=\mathbb{R}^{n}=\{x_{n+1}=0\}$, and $U$ is an even function in the $x_{n+1}$-direction. In this case, \eqref{general Signorini} becomes
\begin{equation*}
...
\end{equation*}
This is an increasing function. If $\ds N=\lim_{r\to0}N_{x_{0}}(U,r)\in(1,2)$, then the only possibility is $N=3/2$. In this situation, up to a subsequence, the rescale function
\begin{equation*}
...
Let's briefly explain the idea in \cite{ACS08}. First, one need to show that the free boundary is Lipschitz near a regular point. If the rescaling $U_{r}$ converges to $C_{n}\cdot Re((x_{n}+ix_{n+1})^{3/2})$, then for every unit vector $\vec{e}$ with
\begin{equation*}
    e_{n+1}=0,\quad e_{n}>0,
\end{equation*}
we have that for any $\epsilon$, there is a sufficiently small $r$ (for simplicity, let $r=1$), so that
\begin{equation*}
    \partial_{\vec{e}}U_{1}(x)\geq C_{n}\partial_{\vec{e}}Re((x_{n}+ix_{n+1})^{3/2})-\epsilon\mbox{ in }B_{1}.
\end{equation*}
Notice that $\partial_{\vec{e}}U_{1}(x)$ vanishes on $\{x_{n+1}=0,U(x)=0\}$ and is harmonic elsewhere. One can then show
\begin{equation*}
    \partial_{\vec{e}}U_{1}(x)\geq0\mbox{ in }B_{1/2}.
\end{equation*}
It shows that the free boundary is a Lipschitz graph
\begin{equation*}
    \Gamma=\{(x_{1},\cdots,x_{n},0):x_{n}=\gamma(x_{1},\cdots,x_{n-1})\}
\end{equation*}
near the origin.

After that, one apply the boundary Harnack principle to the ratios $\partial_{i}U/\partial_{n}U(i<n)$ to show the $C^{1,\alpha}$-regularity of the free boundary. More precisely, one first straighten the free boundary using the coordinate change
\begin{equation*}
    (x_{1},\cdots,x_{n+1})\rightarrow(x_{1},\cdots,x_{n-1},x_{n}-\gamma(x'),x_{n+1}),
\end{equation*}
and then apply the following Lipschitz coordinate change to open the ``slit"
\begin{equation*}
    (x_{1},\cdots,x_{n-1},\rho\cos{\theta},\rho\sin{\theta})\rightarrow(x_{1},\cdots,x_{n-1},\rho\cos{\frac{\theta}{2}},\rho\sin{\frac{\theta}{2}}).
\end{equation*}
It turns out that the original partial derivatives $u_{i}=\partial_{i}U$ in the original coordinate satisfy an uniformly elliptic equation
\begin{equation*}
    \mathrm{div}(A\nabla u_{i})=0,\quad\lambda I\leq A\leq\Lambda I
\end{equation*}
in the half space $\mathbb{R}^{n+1}_{+}$ and vanish at the boundary. By a version of the boundary Harnack principle in \cite{CFMS81}, one gets that the ratios $\partial_{i}U/\partial_{n}U(i<n)$ are $C^{\alpha}$.

Let's now consider the more general Signorini problem \eqref{general Signorini} where we allow variable coefficients. It was shown in \cite{G09} that when $M,\psi$ are $C^{1,\beta}$ for some $\beta>1/2$, $A$ is $C^{1,\gamma}$ for all $\gamma>0$ and $F$ is just H\"older continuous, then $U$ is $C^{1,1/2}$ on both sides of $M$. Later, in \cite{GSVG14,KRS16,RS17}, the optimal regularity of $U$ was improved subsequently to Lipschitz, $W^{1,p}$ and $C^{\alpha}$ coefficients. 
...
We plan to use the higher order boundary Harnack principle to provide an alternative proof the analyticity of the free boundary for the analytic coefficients Signorini problem. In this paper however, we only work on the first step, meaning that we allow variable coefficients and use a $C^{1,\alpha}$ boundary Harnack principle to show that $\Gamma\in C^{2,\alpha}$ near a regular point. The main difficulty is that the blow up differs along the free boundary $\Gamma$ due to the changing coefficient matrix. This will be discussed soon in the next subsection.
...
\begin{itemize}
    \item[(a)] $\lambda I\leq A\leq\Lambda I$ and $[A]_{C^{\alpha}}$($\alpha<1/2$) is bounded,
...
We can do a scaling, replacing $U(x)$ by $R^{-3/2}U(Rx)$ near the origin. This keeps the blow-up of $U(x)$ at the regular boundary invariant, but we can assume $\|D^{i}A\|_{C^{\alpha}}(i\leq n)$ and $[F]_{W^{1,\infty}}$ are small by sending $R\to0$.
...
\begin{equation*}
    U_{r}(x):=\frac{U(rx+x_{0})}{(\fint_{\partial B_{r}(x_{0})}U^{2})^{1/2}}
\end{equation*}
converges as $r\to0$ in $C^{1,\alpha}$ sense to a $3/2$-homogeneous solution like the previous mentioned
\begin{equation*}
...
\end{equation*}
in $C^{\alpha}$ sense near the regular point. In fact, the blow-up limit of $U_{x_{i}}$ is more complicated, in the sense the expression depends on the coefficient matrix $A$. Therefore, let's now get ourselves familiar with the homogeneous solution.
...
\end{equation}
where $\xi$ here is the same as \eqref{rho and xi}. In the $(x_{1},\cdots,x_{n-1},\xi,\eta)$-coordinate, let's fix the first $(n-1)$ coordinates $(x_{1},\cdots,x_{n-1})$ and obtain a Poincare estimate for the $(\xi,\eta)$-coordinates.
\begin{lemma}
    Assume that $w\in C^{1}_{loc}$ in the two-dimensional half space $\{(\xi,\eta):\xi>0\}$. If $w\Big|_{|x|>r}=0$ for some $r$, then
...
Consequently, we have the following Poincare inequality for a slit domain.
...
\end{align*}
\end{proof}
It is then natural for us to define a weighted Sobolev space $H^{1}(B_{r}\setminus S,\xi^{2}dx)$. It is a subspace of $H^{1}_{loc}(B_{r}\setminus S)$ functions which locally has weak derivatives in $B_{r}\setminus S$, such that its weighted energy is bounded.
\begin{definition}
...
The reason is quite simple, and let's just argue with a specific $r=1$. We can use the complex coordinate $(x^{T},\xi,\eta)$ mentioned in \eqref{complex cordinate}. It turns $B_{1}\setminus S$ to $\Omega_{1}=\{(x^{T},\xi,\eta):\xi>0,|x^{T}|^{2}+(\xi^{2}+\eta^{2})^{2}\leq1\}$, and the slit $S$ becomes the plane $\{\xi=0\}$, which is now a $C^{1}$ boundary. The corresponding weighted Sobolev semi-norm $\ds\int_{B_{1}\setminus S}\xi^{2}|\nabla w|^{2}dx$ becomes
\begin{equation*}
...
As a simple consequence of the density of smooth functions inside $H^{1}(B_{r}\setminus S,\xi^{2}dx)$, we see that Proposition \ref{thin poincare} not only hold for $C^{1}_{loc}$ functions, but also functions in $H^{1}(\mathbb{R}^{n+1}\setminus S,\xi^{2}dx)$ with compact support.
...
\begin{remark}\label{absorb remark}
    If the right-hand-side of \eqref{main again} has a term $\ds\frac{\xi^{2}}{\rho}h$, where $h\in C^{\alpha}$ at the origin, then it could be absorbed into $\mathrm{div}(\xi^{2}\vec{f})$ by setting
\begin{equation}\label{absorb}
    \vec{f}:=\int_{0}^{1}s\cdot h(sx+(1-s)x^{T})ds\nabla\rho=\frac{h(0)}{2}\nabla\rho+O(|x|^{\alpha}).
\end{equation}
To check $\ds \mathrm{div}(\xi^{2}\vec{f})=\frac{\xi^{2}}{\rho}h$, we write $y=\gamma(s):=sx+(1-s)x^{T}$, then we have
\begin{equation*}
    \Psi(x):=\int_{0}^{1}s\cdot h(sx+(1-s)x^{T})ds=\rho(x)^{-2}\int_{\gamma}\rho(y)h(y)dy,
\end{equation*}
so $\ds\partial_{\rho}(\rho^{2}\Psi)=\rho h$. Since $\mathrm{div}(\frac{\nabla\rho}{\rho})=\Delta(\log{\rho})=0$, we have
\begin{equation*}
    \mathrm{div}(\xi^{2}\vec{f})=\mathrm{div}(\rho\xi^{2}\Psi(x)\frac{\nabla\rho}{\rho})=\frac{\nabla\rho}{\rho}\cdot\nabla\Big(\frac{\xi^{2}}{\rho}\rho^{2}\Psi\Big)=\frac{\xi^{2}}{\rho^{2}}\partial_{\rho}(\rho^{2}\Psi)=\frac{\xi^{2}}{\rho}h.
\end{equation*}
\end{remark}
...
\begin{proof}
    ...
    As $u_{k}$ vanishes at $S$, there is a Hardy-like inequality (will be given later)
...
\end{proof}
The missing proof of \eqref{hardy} and \eqref{induction} are as follows.
...
\begin{itemize}
    ...
    \begin{equation}\label{phi also absorb}
        \mathrm{div}(\vec{f}_{i})+\phi_{i}(x)=div\Big(\vec{f}_{i}+\int_{0}^{x_{n+1}}\phi_{i}(x_{1},\cdots,x_{n},h)dh\cdot e_{n+1}\Big)=:\mathrm{div}(\vec{f}_{i}').
    \end{equation}
...
Let's prove the ``equivalence" of properties $(\mathcal{F})$, $(\mathcal{F}_{1})$, $(\mathcal{F}_{2})$, $(\mathcal{F}_{3})$. The most obvious relation is clearly $(\mathcal{F}_{3})\Rightarrow(\mathcal{F}_{2})\Leftrightarrow(\mathcal{F}_{1})$ up to a shrinking of radius.
