...
Based on their strategy, we describe  in Section~\ref{sec:Algorithm-computations} 
...
in cases where this was not previously possible
(see~\S\ref{subsec:pbsd} and~\S\ref{pbsd}).
\end{itemize}

We now sketch our method.
Assume, for simplicity, that $f$ is monic and denote by $\infty_-$ and
$\infty_+$ the two points at infinity, 
  $\infty_{\pm} = (1:\pm 1:0)$ on the model of $X$ given by the closure of~$y^2=f(x)$ in the weighted
  projective plane $\PP_{1,3,1}$.
...
We show in \cref{subsec:Even-two-infinities} and \cref{subsec:affine}  that the computation of $h_p(P-Q,R-S)$ can be reduced essentially to the 
computation of Coleman integrals of the differentials 
$$\omega_i\colonequals \frac{x^idx}{y},\quad i=0,\ldots,2g\,. $$
For $P-Q=D_\infty$, this reduction is straightforward, since the residue divisor of $\omega_g$ is $D_\infty$. 
Using a change of variables and further reductions to divisors of a specific shape,
we also reduce the computation of the relevant integrals $\int \omega_{P-Q}$ for most other divisors $P-Q$ to the computation of $\int \omega_g$.
...
in~\cref{subsec:implementation} and can be obtained from
from~{\url{https://github.com/StevanGajovic/heights_above_p}}. 
We give a precision analysis in~\cref{subsec:prec-p-adic-heights}.
For our algorithm, we assume that the points $P,Q,R,S$ satisfy a mild condition
(see Condition \ref{cond2}, and see~\S\ref{subsec:weakening} on how this 
...
$r<g$.
  ...
  applied to several explicit examples~\cite{QC13, BBBLMTV19, AABCCKW}.
...
The first condition is also satisfied for $p=7$, but the second one is not. We use the methods of Section~\ref{sec:Algorithm-computations} to compute  $X_0^+(107)(\Q)$ via a combination of the quadratic Chabauty method and the Mordell-Weil sieve as in~\cite{Jen-Netan-QCRP1, QC13, BDMTV2} for $p=7$. 
On 
...
Another
application is the numerical verification of Conjecture~\ref{pbsd}, a $p$-adic Birch and Swinnerton-Dyer
...
We thank 
Jennifer Balakrishnan, Alex Best, Francesca Bianchi and Michael Stoll for many valuable comments and suggestions, and Amnon Besser, Bas Edixhoven, Sachi Hashimoto, Enis Kaya, Kiran Kedlaya, Guido Lido, David Rohrlich, and Jaap Top for helpful discussions. 
...
Condition (a) means that the local character $\ell_{\p}$ is ramified,
...
$h_v(D_1, D_2)\in\Q_p$ for divisors $D_1,D_2\in \Div^0(X_v)$ with disjoint support. These have the property that for $D_1,D_2\in \Div^0(X)$,
only finitely many $h_v(D_1, D_2)\colonequals h_v(D_1\otimes K_v, D_2\otimes K_v)$ are nonzero.
Hence it makes sense to define 
$$
h(D_1,D_2)\colonequals \sum_vh_v(D_1,D_2)\,.
$$
...
$$
The local height pairings $h_v$ for $v\nmid p$ are symmetric, whereas the local height pairings $h_{\p}$ for $\p\mid p$ are symmetric if and only if all $W_\p$ are isotropic with respect to the cup product
...
\end{equation*}
Up to a constant, this is equal to the local N\'eron symbol at $v$, discussed in~\cite[Sections~2,~3]{Gross-local-heights}. It does not depend on the choice of model or on the choices of $\mathcal{D}_1,\mathcal{D}_2$.
...
A meromorphic differential on $C/k$ is called 
  \begin{itemize}
    ...
    \item of the \textit{third kind} if it is holomorphic except possibly at finitely many
      points and if it has at most simple poles with residues in $\mathbb{Z}$\,.
  \end{itemize}
\end{definition}
...
When $C$ has good ordinary reduction at $v$, there is a canonical choice of
  ...
  the cup product. 
  See \cite[Theorem 3.1(v)]{Noriko-g-eigenvalues-Frobenius} for the hyperelliptic case. 
\end{remark}
...
$$\Res\colon T(k)\rightarrow \Div^0(C), \hspace{5mm} \Res(\omega)=\sum_{P\in C}\Res_P(\omega)P$$
is surjective and induces an exact sequence 
$$0\rightarrow \hdrh(C/k)\rightarrow T(k)/T_l(k)\rightarrow J(k)\rightarrow 0\,.$$

\begin{lemma}\label{L:psi}{\cite[Section~2]{BBHeights}}
...
\end{tikzcd}$$
\end{lemma}
We extend $\psi$ to a linear map on all differentials $\omega$ on $C/k$ as follows: Write 
$$\omega = \sum_{i=1}^n
...
$$h_v(D_1,D_2)\colonequals t_v\left(\int_{D_2}\omega_{D_1}\right),$$ 
  where $t_v$ is the linear map introduced in~\eqref{trace-on-O_K}. 
\end{definition} 
...
where the sum is taken over all points in $C(\bar{k})$ where $\omega$ or $\rho$ has a singularity.
\end{definition} 
We note that the definition of the global symbol does not depend on a point $Z$.  To compute $\psi$ in practice  (see \cref{subsec:affine}, Step (vi)), we use the following statement. 
...
\begin{corollary}\label{cor:height-independence}
  ...
  (respectively $\hdr(C'/k)$) such that $\tau(W) = W'$.
...
Let $C\colon y^2=f(x)$ be a hyperelliptic curve over $\Q_p$ with good reduction. In \cite{BBHeights},  Balakrishnan and Besser
present an algorithm to compute the local height $h_p$ on $C$ with respect to
...
speed up the algorithm to compute $h_p$ when $\deg(f)$ is even. 
We assume that there exists a point $P_0\in C(\Q_p)$ such that $\ord_p(y(P_0))=0$. In this case, we can apply a transformation (see Step \eqref{ints} in \S\ref{subsec:affine}) that does not
change the local height by Corollary~\ref{cor:height-independence} and
results in a model $C\colon y^2=f(x)$ where $f$ is monic of even degree. We
will assume that we are in this situation from now on.
In particular, the points at infinity, denoted
$\infty_+$, $\infty_-$, are in $C(\Q_p)$. For each $i\geq 0$, we denote
$\omega_i\colonequals \frac{x^idx}{2y}$; recall that  $\omega_0,\ldots,\omega_{g-1}$ form a  basis of
the holomorphic differentials on $C$. 
...
compute $p$-adic heights, it suffices to compute $h_p(P-Q,R-S)$ for fixed distinct points $P,Q,R,S\in C(\Q_p)$. 
...
\begin{alg}{\bf (Computation of the local height $h_p(P-Q,R-S)$)} \label{alg:hp}  
\begin{enumerate}[(i)]
  ...
  \item\label{W} Compute a basis of $W$.
...
\item\label{coeffs}  Compute the unique coefficients $u_0,\ldots,u_{g-1}\in
  \Q_p$ 
    such that
  $\omega\colonequals \omega' -\sum^{g-1}_{i=0}u_i\omega_i$ satisfies $\Res(\omega)=P-Q$ and $\psi(\omega)\in W$.
\item\label{ints}  Compute the Coleman integrals $\int_S ^R \omega'$ and $\int_S ^R \omega_i$ for
  $i=0,\ldots,g-1$ and return $h_p(P-Q,R-S) =t_\p(\int^R_S\omega)$. 
\end{enumerate}
\end{alg}
\begin{remark}\label{rmk:trace-scalar}
Recall that the linear map $t_\p$ in Step~\eqref{ints} is determined by
  $\chi$ via Equation~\eqref{trace}. Since we assume that $k=\Q_p$, the same equation also induces a branch $\log_p$ of the $p$-adic logarithm. In this situation, $\chi=\log_p$ is the Iwasawa branch of the logarithm, determined by $\log_p(p)=0$, and $t_\p$ is a scalar multiple of the identity map. In the following, we assume for ease of notation that it is the identity map. 
\end{remark}
...
In contrast, Steps~\eqref{om'} --\eqref{ints} depend on the points we plug in to compute heights. We
will distinguish two cases: first, we assume
in~\cref{subsec:Even-two-infinities} that 
...
affine to it; see~\cref{subsec:affine}, especially Remark~\ref{R:aff_inf}.


We will not consider the case when the points at infinity
are in the support of two distinct divisors, see Conditions
\ref{cond2} and \ref{cond1}. When there is only one point at infinity among
...
\end{itemize} 
   ...
   So $R$ and $S$ need to satisfy Condition~\ref{cond3} if we want to carry out Step~\eqref{ints} directly. 
However, it is possible to remove this condition, and our implementation
   does not assume it; see~\S\ref{subsec:weierstrass}. 
Nevertheless, we choose to assume Condition \ref{cond3} in our description
...
\subsection{Precomputations}\label{subsec:Curve_Part}
We consider each Step~\eqref{basis}--\eqref{W} in turn.

\textit{Step \eqref{basis}.} We determine a basis of $\hdr(C/\Q_p)$ in
Lemma \ref{lm:dR-from-MW} below; first we briefly discuss the cohomological background.
Denote the Weierstrass points on the reduction $\overline{C}$ of $C$ modulo $p$ by $W_1,\ldots,W_{2g+2}\in \overline{C}(\overline{\F_p})$ and let
\[
...
\overline{C}(\overline{\F}_p),\;\;\; \mathcal{V}\colonequals
\{\mathcal{W}_1,\ldots,\mathcal{W}_{2g+2}\}\subset C(\overline{\Q}_p)\,.
\]

Let $U=\Spec\F_p[x,y,1/y]/(y^2-f(x))$ be the affine curve $\overline{C}\setminus V$. By~\cite[Theorem 3.11]{Tuitman-P1-two}, there is an exact sequence:
\begin{equation}\label{eq:Cohomology-Exact-Sequence}
\begin{tikzcd}
\displaystyle  0 \arrow[r] & H^1 _{\mathrm{rig}}(\overline{C}) \arrow[r] &
  H^1 _{\mathrm{rig}}(U) \arrow[r, "\Res"] & \bigoplus_{\substack{P\in
  \mathcal{V}}}F\oplus \Q_p \oplus \Q_p\,,
\end{tikzcd}
\end{equation} 
where 
\begin{enumerate}[(i)]
\item $H^1 _{\mathrm{rig}}(U)$ is the first rigid cohomology of $U$. By
  \cite[Proposition 1.10]{Berthelot-isomorphism-rigid-MW}, $H^1
    _{\mathrm{rig}}(U)$ is isomorphic to  the first Monsky-Washnitzer
    cohomology group $\hmw(U)$ of $U$. Therefore, by \cite[\S3.2]{Harrison-Even-Deg-MW}, we have
\begin{equation}\label{eq:H1MW-even-basis}
H^1
  _{\mathrm{rig}}(U)\simeq\langle\omega_0,\ldots,\omega_{2g}\rangle\oplus\langle\mu_0,\ldots,\mu_{2g+1}\rangle\,,
\end{equation}
where $\mu_i \colonequals \frac{x^idx}{2y^2}$, for $i\ge 0$.
\item Similarly, $H^1 _{\mathrm{rig}}(\overline{C})\simeq \hdr(C/\Q_p)$ by the work of Baldassarri and Chiarellotto \cite{Baldassarri-Chiarellotto}. Our goal is to compute $\hdr(C/\Q_p)$. 
\item  The map $\Res\colon H^1 _{\mathrm{rig}}(U)\to \bigoplus_{\substack{P\in \mathcal{V}}}F\oplus \Q_p \oplus \Q_p$ is defined by
\[
...
\item $\Res(\omega_g) =  (0,\ldots,0,1,-1)$, as we will see in \cref{subsec:Even-two-infinities};
...
2g+1$, we have
\[
\Res_{\mathcal{W}_j}(\mu_i)=\Res_{\mathcal{W}_j}\left(\dfrac{x^i}{f'(x)}\dfrac{dy}{y}
\right)= \dfrac{a_j^i}{f'(a_j)}\,.
\]

\begin{lemma}\label{lm:positive-cohomology-misses-kernel}
We have $\ker(\Res)\cap \langle\mu_0,\ldots,\mu_{2g+1}\rangle = \langle0\rangle$.
\end{lemma}
\begin{proof}
...
The exact sequence \eqref{eq:Cohomology-Exact-Sequence} and Lemma
\ref{lm:positive-cohomology-misses-kernel} imply:
...
\displaystyle    0 \arrow[r] & \hdr(C/\Q_p) \arrow[r] &
  \langle\omega_0,\ldots,\omega_{2g}\rangle_{\Q_p} \arrow[r, "\Res"] &
...
$\eta_i=\omega_i$ and for $g\leq i \leq 2g-1$, let $\eta_i
\colonequals\omega_{i+1} -
c_i\omega_g$, with $c_i$ as in (iii). Denote the sets $\omeg\colonequals \{\omega_0,\ldots,\omega_{2g}\}$, $\et\colonequals \{\eta_0,\ldots,\eta_{2g-1}\}$.

\begin{lemma}\label{lm:dR-from-MW}
The classes of the differentials in $\et$ form a basis of $\hdr(C/\Q_p)$.
\end{lemma}
\begin{proof}
From the above, it follows that $\et\subset \ker(\Res)$. Since the classes of the $\eta_i$ are independent in $\hdr(C/\Q_p)$ by \eqref{eq:H1MW-even-basis}, the claim follows by comparing dimensions. 
\end{proof}


\textit{Step \eqref{frob}. }
An algorithm for the computation of the matrix $\Phi=(f_{i,j})\in
\Q_p^{(2g+1)\times(2g+1)}$ of Frobenius acting on $\langle \omeg \rangle$
is given in~\cite{Harrison-Even-Deg-MW}.
By~\cite[\S3.2]{Harrison-Even-Deg-MW}, $\langle \omeg
\rangle$ is a basis of the $-$ component $\hmw(U)^-$ of $\hmw(U)$ with
respect to the hyperelliptic involution. 
The matrix of Frobenius on $\hdr(C/\Q_p)$ with respect to the basis $\et$, denoted by $\Frob$, is 
$$\Frob=\begin{pmatrix} 
...
\end{pmatrix}.$$
\textit{Step \eqref{cup}.} 
Since all $\eta_i$ are holomorphic on $U$, 
the cup product matrix $M=(m_{ij})_{i,j}$ is given by 
\[
...
forms a symplectic basis. 
Alternatively, if $p$ is ordinary, and we want to use the unit root
subspace $W$, we can compute it to any desired precision using the
following result. We denote by $\phi$ a lift of Frobenius to (a certain wide open
subspace of) $C$, as constructed by
...
The elements ${\phi^*}^n(\eta_g), \ldots , {\phi^*}^n(\eta_{2g-1})$
...
\subsection{One divisor supported at $\infty$}\label{subsec:Even-two-infinities}
Suppose that $R,S\in C(\Q_p)$ are affine such that 
Condition \ref{cond2} is satisfied for $\infty_-,\infty_+, R,S$ or we have $D(R)=D(S)$. We explain in \S\ref{subsec:weierstrass} how to remove the assumption that $R$ or $S$ belongs to a Weierstrass disc.
...
we see that  $\Res_{\infty_+}\omega'=-1$ and $\Res_{\infty_-}\omega'=1$.
\end{proof}

{\it Step \eqref{psiom'}.}
In the spirit of \cite[Algorithm 4.8]{BBHeights}, we define the differential  $$\alpha=\phi^*(\omega')-p\omega'\,.$$
...
\item[(b)] The differential $\alpha$ is of the second kind.
\end{itemize} 
\end{proposition}
...
It follows that
\[
\alpha=(pt^{t-1}A(t^p)-pA(t))dt
\]
is holomorphic at $\infty_-$. 

For (b), we first show that $\alpha$ is holomorphic at non-Weierstrass
  points. We already know that $\omega'$ is holomorphic at every point
...
\phi^*(\omega')=\phi^*\left(\dfrac{x^gdx}{y}\right)=\dfrac{px^{pg+p-1}dx}{y^p}\displaystyle\sum_{i\geq
  0}\dbinom{-\frac{1}{2}}{k}\dfrac{(f(x^p)-f(x)^p)^i}{y^{2pi}}\,,    
\end{equation}
so that $\ord_P\frac{\phi^*(\omega')}{dt}=\ord_P\frac{\phi^*(\omega')}{dx}\geq 0$.
...
from which we see that all powers of $y$ are even. Hence there are no odd powers of $t$ in the expansion of $\alpha$ and (b) follows.
\end{proof}

\begin{corollary}
Let $[\alpha]\in \hdr(C/\Q_p)$ be the class of $\alpha$. Recall that $\phi^*\omega_g=\sum_{i=0}^{2g}f_{0,i}\omega_i$. Then we have $[\alpha]=\begin{pmatrix}
2f_{0,0} & \cdots & 2f_{0,g-1} & 2f_{0,g+1} \cdots & 2f_{0,2g}
\end{pmatrix}^t$ and
...
\[
\alpha=\phi^*(\omega')-p\omega=2\sum_{0\leq i\leq 2g, i\neq g}f_{0,i}\omega_i + 2(f_{0,g}-p)\omega_g + dv\,.
\]
Since $\alpha$ is of the second kind by Proposition~\ref{P:alphasecond}, we obtain
$\alpha=2\sum_{0\leq i< 2g}f_{0,i}\eta_i+dv$ and Lemma~\ref{L:psi} implies
...
If $p$ is good ordinary and $W$ is the unit root subspace, we use the following for {\it Step \eqref{coeffs}}.
...
\begin{corollary} 
Suppose that $p$ is a good ordinary prime and $W$ is the unit root
  subspace. Then the local  height pairing $h_p$ with respect to $\ell_p$ and $W$ satisfies
\[
h_p(\infty_- - \infty_+,R-S) = 2\int_S^R \omega_g -
  (u_0\int_S^R\omega_0+\cdots+u_{g-1}\int_S^R\omega_{g-1})\,,
\]
...
\[
\int_S^R \omega_g,\;\; u_0\int_S^R\omega_0+\cdots+u_{g-1}\int_S^R\omega_{g-1}\,.
\]
Note that for the computation of the integrals above we needed to assume
that $R$ and $S$ belong to the same residue disc or to affine discs.
Also note that we do not need the cup product matrix to compute
  $h_p(\infty_--\infty_+, R-S)$.



\subsection{The affine case}\label{subsec:affine}

Our goal is to compute $h_p(P-Q,R-S)$ for four distinct affine points $P,Q,R,S\in
C(\Q_p)$. We assume that $P,Q,R,S$ satisfy Condition  \ref{cond2}. For
simplicity, we also assume
that the pairs $(P,Q)$ and $(R,S)$ both satisfy Condition \ref{cond3}. This
assumption is not strictly necessary, but it simplifies
the exposition of the algorithm. We discuss in~\S\ref{subsec:weierstrass} how
to
proceed when Condition~\ref{cond3} is not satisfied. 
As explained in~\S\ref{subsec:implementation}, our implementation in
\texttt{SageMath} assumes only Condition \ref{cond2} for $P,Q,R,S$. 
...
\begin{align*} 
  h_p(P-Q,R-S) = &\dfrac{1}{2}\chi\left(\dfrac{x(R)-x(P)}{x(R)-x(Q)}\dfrac{x(S)-x(Q)}{x(S)-x(R)}\right)\\ 
...
h_p\left(\dv\left(\dfrac{x-x(P)}{x-x(Q)}\right),
  R-S\right)=\log_p\left(\dfrac{x(R)-x(P)}{x(R)-x(Q)}\dfrac{x(S)-x(Q)}{x(S)-x(P)}\right)\,.
\]
The claim follows from additivity.
\end{proof}
Condition~\ref{cond2} for  $P,Q,R,S$ implies that the right hand side in
Lemma \ref{L:symm-antisymm} is defined. 
 However, the much weaker condition $\{x(R),x(S)\}\cap \{x(P), x(Q)\} =
\emptyset$ actually suffices. This is precisely Condition \ref{cond1}, which
we will introduce later. We will explain in~\S\ref{subsec:weakening} how to
weaken Condition \ref{cond2} to Condition \ref{cond1} in such a way that
Lemma \ref{L:symm-antisymm} continues to hold (though we did not implement
this). 
In fact, Lemma \ref{L:symm-antisymm} is the reason why we cannot weaken Condition
\ref{cond1} any further. 
...
Condition~\ref{cond3} is satisfied for $P$ and, by symmetry, $Q$.
We can simplify this
further as follows:

\begin{lemma}\label{L:zero-residue-even}
We have $\Res_{\infty_+}\left(\omega'\int\eta_j\right)=\Res_{\infty_-}\left(\omega'\int\eta_j\right)=0$ for $0\leq j\leq 2g-1$.
\end{lemma}
\begin{proof}
...
We can prove a similar statement in the odd degree case. Namely, in the
  ...
  $\Res_{\infty}\left(\omega'\int\eta_j\right)=0$ for $0\leq j\leq 2g-1$.
  This can be used to simplify the computation of the global symbols
  in~\cite[\S5.2]{BBHeights}.

\end{remark}

We compute $u_0,\ldots,u_{g-1}$ in {\it Step \eqref{coeffs}} as
in~\cref{subsec:Even-two-infinities}.
...
considered in~\cref{subsec:Even-two-infinities}.
Namely, consider
\begin{align*}
...
\[
  h_p(P-\iota(P),R-S) = \int_{S'}^{R'} \omega_g - u_0\int_S^R \omega_0 - \cdots - u_{g-1}\int_S^R \omega_{g-1}.
\]
\end{lemma}
\begin{proof}
  This follows from 
...
from~\cref{subsec:Even-two-infinities}. In Lemma \ref{L:change}, the points $R, S\in
C(\Q_p)$, and $R',S'\in C'(\Q_p)$ are endpoints of Coleman integrals. 
Hence we
require Condition \ref{cond3} for both pairs $(P,Q)$ and $(R,S)$, and we require 
Condition
\ref{cond2} for $P,Q,R,S$. The division by $y$ in the
...
We can also completely reduce the affine case
  to the infinite case discussed in \cref{subsec:Even-two-infinities}. 
  Recall that it suffices to compute heights of the form $h_p(P-\iota(P), R-S)$.
  Using the same map $\tau\colon C\rightarrow C'$ as in Lemma
  ...
  on $C'$, where we use the same notation as in Lemma \ref{L:change}.
\end{remark}



\subsection{Precision analysis}\label{subsec:prec-p-adic-heights}
Steps \eqref{basis} and \eqref{cup} depend on the precision of local
coordinates. In both cases, we need to compute residues of differentials at
points at infinity. Step \eqref{cup} clearly requires more precision than
Step \eqref{basis}. We want to compute
...
compute the matrix of Frobenius acting on $\omeg$ to any desired precision $p^n$. We assume
that we have already computed the basis $\underline{\eta}$ in Step \eqref{basis} to the same
precision $p^n$. Then it is clear that the results of Steps \eqref{frob} and \eqref{W} are correct to
precision $p^n$ as well. Step \eqref{om'} is exact, so there is no loss of precision.

Now we need to distinguish cases. We first deal with the approach from \cref{subsec:Even-two-infinities}.
For Step \eqref{psiom'}, suppose that the matrix of Frobenius and the basis $\underline{\eta}$ of
$\hdr$ are correct modulo $p^n$. Denoting  $m\colonequals \ord_p\det(\Frob-pI)$, we compute $\psi(\omega')$ in terms of $\underline{\eta}$ correctly modulo $p^{n-m}$. Step \eqref{coeffs} is accurate to the same precision as Step \eqref{psiom'}. 
...
It remains to analyze the precision for \cref{subsec:affine}. In addition to the above,
Step \eqref{om'} requires an evaluation of $\log_p$, but  since $P,Q,R,S$ satisfy Condition
\ref{cond2}, this does not lead to any loss of precision. 
Furthermore, Steps \eqref{coeffs} and
\eqref{ints} are analogous to Steps~\eqref{coeffs} and~\eqref{ints} in~\cref{subsec:Even-two-infinities}, so we only focus on Step \eqref{psiom'}.
...
\begin{remark}\label{R:precspecial}
  ...
  In the precision analysis above, we assume Conditions~\ref{cond2}
  and~\ref{cond3}. As discussed
  in~\S\ref{subsec:weierstrass} and \S\ref{subsec:inf},
  Condition~\ref{cond3} is not necessary, and in order to reduce 
...
$\int_S^R\omega_0,\ldots,\int_S^R\omega_{2g}$ efficiently if $R$ and $S$ are $\Q_p$-points in
affine discs. 
This made it easy to implement the algorithm outlined 
in~\cref{subsec:Even-two-infinities} and~\cref{subsec:affine} 
for points $P,Q,R,S\in C(\Q_p)$
that satisfy Conditions~\ref{cond2} and \ref{cond3}. 
Recall that by~\S\ref{subsec:weierstrass} we can drop Condition~\ref{cond3} provided
Condition~\ref{cond2} is satisfied. 


We require Condition~\ref{cond2} because we use Lemma~\ref{L:change} for
Step \eqref{ints}
of~\cref{subsec:affine}, since it is much simpler than the one based on
...
for points satisfying Condition~\ref{cond2} usually suffices. 



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Timings}\label{subsec:timings}
...
We now explain how to compute $h_p(P-Q, R-S)$ when Condition \ref{cond3} is not
satisfied. Recall that we still assume that $P,Q,R,S\in C(\Q_p)$ satisfy Condition \ref{cond2}.
...
$\int_S^R \omega= \int_S^T \omega + \int_T^R \omega$. The second integral is tiny,
 ...
 We could therefore replace 
  Condition~\ref{cond3} by the weaker condition that
...
the  type $h_p(P-\iota(P),R-\iota(R))$. Condition \ref{cond2} implies that
...
\cref{subsec:Even-two-infinities}. The integrals that show up in Step
...
assume Condition \ref{cond2} when the points $P,Q,R,S$ are affine. 
We have implemented this approach in \texttt{SageMath}. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Points at infinity}\label{subsec:inf}
When exactly one of the points $P,Q,R,S$ (satisfying Condition \ref{cond2}) is a point at infinity, we compute
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{Weakening Condition~\ref{cond2}}\label{subsec:weakening}
We now explain how Condition~\ref{cond2} may be weakened. However, we have
not implemented this, so in our implementation 
we assume Condition \ref{cond2}. 


Assume the points $P,Q,R,S$ do not satisfy Condition~\ref{cond2}, but do
...
\cref{subsec:Even-two-infinities}, where Condition~\ref{cond2} is required to perform Step~\eqref{ints}. 
...
curves due to Balakrishnan and
    ...
    to compute integrals of differentials that are not of the second kind; in particular,
    we cannot compute integrals of the form $\int^R_S\omega_g$ directly.  Instead, we could
    follow the strategy from~\cite{BBHeights} to carry out Step~\eqref{ints}, by using the second kind
    differential $\alpha=\phi^{*}(\omega')-\omega'$. Hence
    ...
    \[
    \int_S^R\omega'=\dfrac{1}{1-p} \left(\int_S^R\alpha - \int_{\phi(S)}^S\omega' - \int_R^{\phi(R)}\omega'\right).
    \]
The integrals $ \int_{\phi(S)}^S\omega'$ and $\int_R^{\phi(R)}\omega'$ are tiny integrals,
  hence they can be computed easily. 

  \begin{remark}\label{R:}
As remarked in {\it Step \eqref{ints}} of \cref{subsec:affine}, if the
...
Condition~\ref{cond2} by allowing that $R$ and $S$ (or by symmetry, $P$ and
$Q$) are in the same residue disc. But as this is a minor improvement
comparing to weakening Condition~\ref{cond2} to Condition~\ref{cond1}, and
...
currently, it works for primes $p$ that split in $K$. 
...
The Tamagawa numbers can be computed using work of van Bommel~\cite{vB22}.
 In~\cite{KS22} Keller and Stoll announce algorithms to compute
...
verified by Keller and Stoll~\cite{KS22}.
...
ordinary primes $p<100$ such that $X$ has a quintic model over $\Q_p$.
  ...
  normalization factor $\delta^+$, which is independent of $p$. In this
  example, we have $\delta^+=1/4$ by~\cite[Table~4.4]{BMS16}. We find that
...
that have representatives
  ...
  \begin{itemize}
    ...
    \item the tuples $(Q_i,R_i, Q'_j, R'_j)$ satisfy Condition~\ref{cond2}.
      for all $i,j\in \{1,2\}$.
  \end{itemize}
