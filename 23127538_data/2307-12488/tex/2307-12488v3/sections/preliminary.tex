    ...
    inline void bubble_sort(It begin, It end, Pred pred=Pred()){
        ...
        while ( !finished ){ //loop stop when no adjacent elements are not in required order. 
            ...
            for (auto it = begin; it! = it_end; ++ it ){
                ...
                // compare two adjacent elements and exchange them if not in required order.
                if (pred( * next, * it)){ 
                    std::swap( * it, * next);
                    finished = false;
                }
...
label={code:badname},caption={The same code without meaningful variable/function names.}, captionpos=b]{}
    template<typename It, typename Fun2=std::less<typename std::iterator_traits<It>::value_type>>
