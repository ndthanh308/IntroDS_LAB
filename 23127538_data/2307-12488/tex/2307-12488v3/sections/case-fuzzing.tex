\subsection{Dynamic Vulnerability Discovery: Fuzzing}
In this section, we assess ChatGPT's ability in vulnerability discovery, particularly in the context of Fuzz testing.
Our goal is to determine whether ChatGPT can offer valuable and effective seed inputs after understanding the source code.
By examining its performance in this scenario, we aim to gain insights into ChatGPT's potential to assist in the discovery of vulnerabilities.


\colorlet{shadecolor}{red!5}
\begin{shaded*}
\noindent
{\bf Q11:} \textit{Could you define the seeds to fuzz the following program?}
  {
  \begin{lstlisting}[language=C++,
    emph={uridecode,strlen,main,printf},
    belowskip=-1.2 \baselineskip,
    caption={{\bf AFL fuzz tester demonstration program~\textendash~URI decoder.} It translates \%xx (xx = two hex digits) to the character with the appropriate ASCII code. Translates '+' into space. Leaves all other characters unchanged.},label=code:seedfuzz,captionpos=b]
	const char * uridecode(const char *s) {
		static char ret[100];
		for(auto * p=ret; *s; ++s) {
			if ( * s=='%') {
				auto const a = *++s;
				auto const b = *++s;
				*p++ = (a<='9' ? a-'0' : a-'a') * 16 + (b<='9' ? b-'0' : b-'a');
			} else if ( *s=='+') {
				*p++ = ' ';
			} else {
				*p++ = *s;
			}
		}
		return ret;
	}
\end{lstlisting}
  }
  \end{shaded*}
\vspace{-2.5em}
\colorlet{shadecolor}{green!5}
\begin{shaded*}
\setlength{\parindent}{0em}
{\footnotesize 
  \noindent{\bf Reply from ChatGPT:}
Sure, here are some example seeds that you can use to fuzz the uridecode function:

Empty input: ``''

Single character input: ``a''

Input with one percent-encoded character: 
\begin{lstlisting}[language=C,captionpos=b, numbers=none, frame=none]
"hello%20world"
\end{lstlisting} 
Input with multiple percent-encoded characters: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz1,captionpos=b, numbers=none, frame=none]
"hello%20world%21%22"
\end{lstlisting}

Input with plus sign: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz2,captionpos=b, numbers=none, frame=none]
"hello+world"
\end{lstlisting}


Input with invalid percent-encoded character: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz3,captionpos=b, numbers=none, frame=none]
"hello%xxworld"
\end{lstlisting}

Input that is too long: a long string of random characters (\eg, `x' $* 10000$)

You can use these seeds as a starting point for your fuzzing campaign. Keep in mind that the more diverse and complex your input seeds are, the more likely you are to find edge cases and bugs in the program.
  }\end{shaded*} 



% \noindent{\bf Q11:} \textit{Could you define the seeds to fuzz the following program?}
% \begin{lstlisting}[language=C++,
%     emph={uridecode,strlen,main,printf},
%     caption={{\bf AFL fuzz tester demonstration program~\textendash~URI decoder.} It translates \%xx (xx = two hex digits) to the character with the appropriate ASCII code. Translates '+' into space. Leaves all other characters unchanged.},label=code:bit,captionpos=b]
% 	const char * uridecode(const char *s) {
% 		static char ret[100];
% 		for(auto * p=ret; *s; ++s) {
% 			if ( * s=='%') {
% 				auto const a = *++s;
% 				auto const b = *++s;
% 				*p++ = (a<='9' ? a-'0' : a-'a') * 16 + (b<='9' ? b-'0' : b-'a');
% 			} else if ( *s=='+') {
% 				*p++ = ' ';
% 			} else {
% 				*p++ = *s;
% 			}
% 		}
% 		return ret;
% 	}
% \end{lstlisting}

% \begin{shaded*}
% 	\footnotesize
% \noindent{\bf Reply from ChatGPT:}
% Sure, here are some example seeds that you can use to fuzz the uridecode function:

% Empty input: ``''

% Single character input: ``a''

% Input with one percent-encoded character: 
% \begin{lstlisting}[language=C,captionpos=b, numbers=none, frame=none]
% "hello%20world"
% \end{lstlisting} 
% Input with multiple percent-encoded characters: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%20world%21%22"
% \end{lstlisting}

% Input with plus sign: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello+world"
% \end{lstlisting}


% Input with invalid percent-encoded character: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%xxworld"
% \end{lstlisting}

% Input that is too long: a long string of random characters (\eg, `x' $* 10000$)

% You can use these seeds as a starting point for your fuzzing campaign. Keep in mind that the more diverse and complex your input seeds are, the more likely you are to find edge cases and bugs in the program.
% \end{shaded*}

\colorlet{shadecolor}{red!5}
\begin{shaded*}
\noindent
{\bf Q12:} \textit{Could you mutate the following seed? ``hello\%20worlw''}
  { }
  \end{shaded*}
\vspace{-2.5em}
\colorlet{shadecolor}{green!5}
\begin{shaded*}
\setlength{\parindent}{0em}
{\footnotesize 
  \noindent{\bf Reply from ChatGPT:}
Sure, here are some examples of how you could mutate the seed 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz5,captionpos=b, numbers=none, frame=none]
"hello%20world"
\end{lstlisting}
 to generate different test cases:

Remove the percent-encoded character: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz6,captionpos=b, numbers=none, frame=none]
"helloworld"
\end{lstlisting}
Change the percent-encoded character: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz7,captionpos=b, numbers=none, frame=none]
"hello%22world"
\end{lstlisting}
Add more percent-encoded characters:     
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz8,captionpos=b, numbers=none, frame=none]
"hello%20%21%22world"
\end{lstlisting}

Add non-ASCII characters: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz9,captionpos=b, numbers=none, frame=none]
"hello%C3%A9world"
\end{lstlisting}

Add special characters: 
\begin{lstlisting}[language=C++,
	emph={strcpy,strlen,main,printf},
	label=code:seedfuzz10,captionpos=b, numbers=none, frame=none]
"hello%00world"
\end{lstlisting}

These mutations should produce different test cases that exercise different parts of the uridecode function. Remember to apply these mutations systematically and to analyze the results of your fuzzing campaign carefully. Good luck!
  }\end{shaded*} 

% \noindent{\bf Q12:} \textit{Could you mutate the following seed? ``hello\%20worlw''}

% \begin{shaded*}
% 	\footnotesize
% \noindent{\bf Reply from ChatGPT:}
% Sure, here are some examples of how you could mutate the seed 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%20world"
% \end{lstlisting}
%  to generate different test cases:

% Remove the percent-encoded character: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "helloworld"
% \end{lstlisting}
% Change the percent-encoded character: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%22world"
% \end{lstlisting}
% Add more percent-encoded characters:     
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%20%21%22world"
% \end{lstlisting}

% Add non-ASCII characters: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%C3%A9world"
% \end{lstlisting}

% Add special characters: 
% \begin{lstlisting}[language=C++,
% 	emph={strcpy,strlen,main,printf},
% 	label=code:bit,captionpos=b, numbers=none, frame=none]
% "hello%00world"
% \end{lstlisting}

% These mutations should produce different test cases that exercise different parts of the uridecode function. Remember to apply these mutations systematically and to analyze the results of your fuzzing campaign carefully. Good luck!
% \end{shaded*}

\colorlet{shadecolor}{red!5}
\begin{shaded*}
\noindent
{\bf Q13:} \textit{What kind of vulnerability will be found if we use the seed ``hello\%20\%21\%22world''?}
  { }
  \end{shaded*}
\vspace{-2.5em}
\colorlet{shadecolor}{green!5}
\begin{shaded*}
\setlength{\parindent}{0em}
{\footnotesize 
  \noindent{\bf Reply from ChatGPT:}
Using the seed ``hello\%20\%21\%22world'' can help to uncover vulnerabilities related to the decoding of percent-encoded characters. Specifically, the seed contains three percent-encoded characters: \%20 (space), \%21 (exclamation mark), and \%22 (quotation mark).

When this seed is passed to the uridecode function, it will attempt to decode each percent-encoded character to its corresponding ASCII value. If the function is not implemented correctly or does not handle edge cases properly, it could potentially result in various types of vulnerabilities, such as:

Buffer overflow: If the input string is too long, the function may attempt to write past the end of the output buffer, potentially overwriting important data or causing a segmentation fault.

Integer overflow: If the percent-encoded values are interpreted incorrectly as integers, an integer overflow vulnerability could occur, leading to unexpected behavior or potentially allowing an attacker to execute arbitrary code.

Injection attacks: If the uridecode function is used to decode user-supplied input that is subsequently used in a vulnerable application, it could potentially allow for injection attacks such as cross-site scripting (XSS) or SQL injection.

By using the seed ``hello\%20\%21\%22world'', you can help to uncover these types of vulnerabilities and improve the security of the uridecode function.
  }\end{shaded*} 


% \noindent{\bf Q13:} \textit{What kind of vulnerability will be found if we use the seed ``hello\%20\%21\%22world''?}

% \begin{shaded*}
% 	\footnotesize
% \noindent{\bf Reply from ChatGPT:}
% Using the seed ``hello\%20\%21\%22world'' can help to uncover vulnerabilities related to the decoding of percent-encoded characters. Specifically, the seed contains three percent-encoded characters: \%20 (space), \%21 (exclamation mark), and \%22 (quotation mark).

% When this seed is passed to the uridecode function, it will attempt to decode each percent-encoded character to its corresponding ASCII value. If the function is not implemented correctly or does not handle edge cases properly, it could potentially result in various types of vulnerabilities, such as:

% Buffer overflow: If the input string is too long, the function may attempt to write past the end of the output buffer, potentially overwriting important data or causing a segmentation fault.

% Integer overflow: If the percent-encoded values are interpreted incorrectly as integers, an integer overflow vulnerability could occur, leading to unexpected behavior or potentially allowing an attacker to execute arbitrary code.

% Injection attacks: If the uridecode function is used to decode user-supplied input that is subsequently used in a vulnerable application, it could potentially allow for injection attacks such as cross-site scripting (XSS) or SQL injection.

% By using the seed ``hello\%20\%21\%22world'', you can help to uncover these types of vulnerabilities and improve the security of the uridecode function.
% \end{shaded*}
 

Upon analyzing the responses from ChatGPT, we observed that the provided seeds were comparable to those generated by AFL (American Fuzzy Lop) \cite{aflrefurl} using seed mutation strategies.
However, it became apparent that ChatGPT did not propose highly effective seeds (capable of triggering the vulnerability).
This observation suggests that ChatGPT may face challenges in leveraging the program semantics it learned from the code to generate such effective seeds.
There could be two possible reasons for this:

\begin{enumerate}
\item The prompts we use may not have been designed to specifically encourage ChatGPT to produce the most useful and targeted solutions.
\item ChatGPT's responses might be too general, primarily relying on the information present in the corpus it was trained on, rather than effectively applying the learned program semantics to new situations.
\end{enumerate}

Understanding these limitations is crucial for utilizing ChatGPT effectively in vulnerability discovery and other security-oriented tasks.
Further research is needed to refine the prompts and explore strategies to enhance ChatGPT's ability to provide more precise and context-aware responses in security-related scenarios.

