...
These pre-trained models are usually first learned on a large unsupervised text corpus and then fine-tuned on different downstream tasks, and widely used in many domains~\cite{zhang2023qualigpt,zhang2023redefining}. 
However, we observe that no existing work has investigated the use of ChatGPT in security domains. 
As security researchers, our primary focus centers on exploring the potential and limitations of LLMs (\ie, ChatGPT and CodeBERT) in program analysis tasks, with a particular emphasis on addressing software security challenges. 
...
Firstly, as ChatGPT is a conversational large language model, the use of a prompt style
% ~\footnote{A prompt is a set of instructions provided to an LLM that programs the LLM by customizing it and/or enhancing or refining its capabilities~\cite{white2023prompt}.} 
plays a crucial role in effectively conversing with it.
...
Secondly, considering that CodeBert is a pre-trained model applicable to downstream tasks, we conduct a focused set of experiments to quantitatively assess its performance using datasets specially created for this purpose.

Specifically, our case study on the ChatGPT focuses on 4 problems: code semantic inference, vulnerability analysis, input generation, and generalization ability. 
Through empirical analysis, we demonstrate that ChatGPT shows capabilities of catpuring program data flow and high-level semantics of the source code. 

Despite these strengths, it is crucial to acknowledge certain limitations when utilizing ChatGPT for security-oriented program analysis.
%
Instances such as code generated from decompilation~\cite{katz2018using} or non-conventional code naming~\cite{butler2015investigating} might yield reduced accuracy, as ChatGPT's generalization ability is limited to the patterns and examples present in its training data. 
%
Furthermore, our experiments on CodeBert and GraphCodeBert reveal the extent to which literal features and logic features impact their performance. While these models exhibit remarkable capabilities, their effectiveness can be influenced by unreliable features in the analyzed code.
%
