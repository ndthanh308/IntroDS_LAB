...
In this part, we give proof of the equivalence between depth modeling and height modeling in 2D to 3D mapping, as shown in \cref{fig:main}. Meanwhile, the proof shows the feasibility of constructing high-quality BEV features by modeling heights accurately. 


\paragraph{Precondition} We use two coordinate systems in our work: the image coordinate system and the BEV coordinate system. In the image frame, points are noted as $(u,v)$, while in the BEV frame$\footnote{Theoretically, there is a camera frame other than the BEV frame. Without losing generality, this manuscript does not distinguish between the camera frame and the BEV frame, because we assume that the extrinsic parameter matrix %which does transformation between the two frames 
is an identity matrix.}$ points are noted as $(x,y,z)$. We use $(x,z)$ to denote a point or grid in the 2D BEV feature space for convenience. Given an object in the BEV frame at $(x_{gt}, y_{gt}, z_{gt})$, its coordinate in the image frame is $(u_{gt},v_{gt})$, and its depth is defined as $d_{gt}\triangleq z_{gt}$. These variables satisfy:
\begin{equation}
    \begin{aligned}
        \left[
...
\paragraph{Assumption} We assume that an object will be correctly detected if its feature is placed close to its ground truth position in the 2D BEV feature space. Mathematically, ``close" is defined as being in the $\epsilon$-neighbourhood of $(x_{gt},z_{gt})$ grid. Here $\epsilon$ is the tolerance of the feature misplacement error. That is to say,
\begin{equation}
    | x - x_{gt} | + | z - z_{gt}| \le \epsilon,\quad \st \epsilon > 0,
    \label{eq:assumption}
\end{equation}
where $(x,z)$ is the coordinate of the object's BEV feature. We simply use the Manhattan distance here to measure the feature misplacement error. 
...
\begin{equation}
    ...
    \label{eq:depth}
\end{equation}
In this way, the feature of the object will fall at $(x,z)$ in the 2D BEV feature space. Solving \cref{eq:depth} and \cref{eq:assumption},
...
\paragraph{Height error} Given a BEV grid $x,z$, the height error is $\delta_y=|y-y_{gt}|$. For all $(x,z)$ in the $\epsilon$-neighbourhood, we can calculate the corresponding sampling location set in the image space. We note this set as $S_{\epsilon}$, which follows:
\begin{equation}
    \begin{aligned}
...
The previous analysis shows the equivalence between two solutions to the ill-posed 2D to 3D mapping problem as well as the feasibility of constructing BEV space by modeling heights. However, although these two solutions are equivalent, height estimation has advantages over depth estimation. Height estimation is done in BEV space which is a unified space, while depth estimation needs to handle the manufacturing differences of cameras. The training process of modeling heights requires no extra data but learning to model the depths of images needs LiDAR information. % Besides, there has been no work to explicitly model heights, and this work will fill in this gap to further improve the performance.

\subsection{Constructing BEV space by height modeling}\label{sec:a2b}

% Figure environment removed

Constructing BEV space from image features requires an elaborate feature sampling method. As shown in \cref{fig:main}(b), for a BEV grid, its position and heights can derive several 3D reference points. To acquire the features at these points, they are projected into images, and this step produces 2D reference points. %The features around 2D reference points are exactly what we want. 
...
The initial height embeddings are designed to make initial anchor heights uniformly distributed from the ground to the sky. 


\paragraph{Segmentation-based query mask} After statistical analysis, it is found that most queries do not contain any object. For these queries, there is no proper definition of heights. As a result, the predicted heights in these queries are meaningless. To avoid gathering irrelevant features from the background, we introduce the segmentation-based query mask. The self-recursive height predictor will predict an extra binary segmentation map, which indicates the probability of covering an object. The gathered features in queries that have low probability will be filtered.

\paragraph{Other modules} Temporal fusion modules fuse information from previous frames, and spatial cross-attention samples image features at given heights with deformable attention to construct the BEV representation. The design of these two modules mainly follows BEVFormer\cite{li2022bevformer}. In addition, DETR-style decoders are adopted to detect objects based on BEV features.
