...
We can note the extra LiDAR supervision brings little improvement to the performance of the proposed method compared with the improvement brought by explicit height modeling. A potential reason is that the LiDAR points are sparse. When they are projected into the BEV space, only a few grids contain the points, which is not sufficient enough to supervise the whole BEV space. As a result, experiments in other sections are conducted without LiDAR supervision. }
...
\begin{table}[htb]
    ...
    \begin{tabular}{lccc}
        ...
        BEVFormer & \checkmark & {\bf 0.421}         & {\bf 0.299}         \\
        \midrule
        BEVDepth  &            & 0.366         & {\bf 0.273}             \\
        % \midrule
        BEVDepth  & \checkmark & {\bf 0.369}         & {\bf 0.273}             \\
        \bottomrule
    \end{tabular}
    \label{tab:expgen}
\end{table}
