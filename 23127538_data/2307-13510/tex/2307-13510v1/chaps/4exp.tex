...
As a challenging dataset for autonomous driving, NuScenes provides data of 1k scenes, from a sensor suite that consists of 6 cameras, 1 LiDAR, 5 RADAR, 1 GPS, and 1 IMU. The {\it train} set contains 28,130 samples and the {\it val} set contains 6,019 samples.
...
As shown in \cref{tab:expa0}, HeightFormer-gt exceeds BEVFormer by 0.208 in NDS and 0.374 in mAP, which can be considered as the performance upper bound. If heights could be accurately modeled, there would be a giant leap in performance. {{\color{blue}}In the meanwhile, Lift-Splat\cite{philion2020lift} in \cref{tab:expa0} is a method that constructs the BEV space with depth modeling and according to BEVDepth\cite{li2022bevdepth} introducing ground truth depth improves mAP of the Lift-Splat style detector by 0.188. The experiments in \cref{tab:expa0} show that there is much room for improvement whether in depth modeling or height modeling.


Besides, it should be noticed that the two types of methods, LSS and BEVFormer, cannot be compared directly as they have different network architectures. We list these results here for showing the performance upper bound of Lift-Splat and BEVFormer on the condition of accurate depth or height modeling.}

\paragraph{Effectiveness of explicit height modeling} We present the vanilla HeightFormer which turns fixed reference points into adaptive reference points. The adaptive reference points are generated with predicted anchor heights. In this way, height information is explicitly learned by the height predictor. To show the effectiveness of explicit height modeling, it is compared with BEVFormer which encodes height information into attention weights of spatial cross-attention in an implicit way. The vanilla HeightFormer simply has a multilayer perceptron as the height predictor. Results are shown in \cref{tab:expa1}. Explicit height modeling brings a gain of 0.5 percentage points in both NDS and mAP.

\begin{table}[htb]
    \centering
    \caption{Ablation study on height modeling. *: Vanilla HeightFormer which predicts heights in a standalone way.}
    \begin{tabular}{lccc}
        \toprule
        Model      & Height   & NDS$\uparrow$ & mAP$\uparrow$ \\
        \midrule
        BEVFormer  & implicit & 0.517         & 0.416         \\
        % \midrule
        HeightFormer* & explicit & {\bf 0.522}         & {\bf 0.421}     \\
...
As described in Introduction, although both depth and height modeling can provide an extra condition for 2D to 3D mapping, height modeling has a unique advantage in that it can process any camera rig. This is because estimating depth in the image space is likely to be influenced by the camera rigs, but estimating height is simple and robust since all information has been mapped to the unified BEV space. To verify this property, we test our method with depth modeling specifically with the back camera, which has a different focal length from other cameras. We use BEVDepth\cite{li2022bevdepth} as the depth modeling method. The results are shown in \cref{tab:expa4}. 
...
From \cref{tab:expa4} we can see that although BEVDepth has higher overall performance (due to extra LiDAR data training), height modeling's performance on the back camera is higher. This means height modeling is more robust to different camera rigs, even when the cameras' focal lengths are different. 
