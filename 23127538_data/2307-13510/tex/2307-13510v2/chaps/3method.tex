...
\uwave{As a result, considering two models, one models the height and the other models the depth. Their height or depth estimation error is proportional, and they yield features with similar misplacement error. Therefore, in terms of error modeling, they are considered equivalent.}
...
\paragraph{Other modules} \uwave{Temporal fusion modules\cite{li2022bevformer} fuse information from previous frames with Self Attention. In this module, the query is current BEV query, and the value is the concatenated (previous and current) BEV query.} DETR-style decoders are adopted to detect objects based on BEV features.