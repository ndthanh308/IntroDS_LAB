...
\title{Levitated optomechanics: A tutorial and perspective}


\author{George Winstone,  \authormark{1} Mishkat Bhattacharya,\authormark{2} Andrew A. Geraci,\authormark{1,*}  Tongcang Li,\authormark{3}  Peter J. Pauzauskie,\authormark{4,5} and Nick Vamivakas\authormark{6}} 
...
In this section we describe the basic features of optical trapping that allow the levitation of nanoparticles and their subsequent manipulation.
We will begin by identifying the various regimes of optical trapping in terms of the length scales involved.
For each regime we will identify the forces that are responsible for the dynamics. Subsequently, a variety of trapping configurations used in the field will be identified along with particle sources and materials for optical levitation. Next the basic optomechanics of 
levitated nanoparticles will be characterized, with reference to laser cooling, quantum ground state occupation, regenerative amplification, etc. Finally, mention will be made of other levitation schemes which use electric fields, magnetic fields and hybrid systems using field types to define the potential.
...
As with many fields of physics, the characteristic behaviour of the system depends greatly on the length scales it presents. In the case of levitated optomechanics, the ratio between the wavelength of the trapping light and the size of the levitated object is critical. 

As such the size parameter: 

\begin{equation}
x=\frac{2 \pi R}{\lambda_0}
\end{equation}

is commonly defined, in which $\lambda_0$ is the wavelength of the incident light and $R$ is the radius of the spherical object.
The case where the wavelength is much greater than the trapped object's circumference is commonly referred to as the Rayleigh regime. The case in which the object is approximately equal to the wavelength (i.e. $x \sim 1$) is known as the Mie-Lorentz regime. The case in which the object is much larger than the wavelength is the geometric optics regime. 
...
The complete motional dynamics of the levitated object can be obtained from the self consistent EM field configuration in and around the particle, in which in which scattered photons represent transient fluctuations.

%general form for the gradient and dipole force go here
%hadrada has a good formulation



Within the Rayleigh regime, the gradient and scattering forces can be considered to be separable, with the gradient force producing a force proportional to the derivative of the intensity of the EM field and the scattering force one in proportion to the product of the intensity and the particle's scattering cross section. It is important to note that the gradient force typically overpowers the scattering force within the Rayleigh regime for focused beams - and creates a restoring force about the focus. The scattering and gradient forces in the Rayleigh limit along the axis of beam propagation for a single beam are given
by \cite{li2012fundamental,harada1996radiation}:

\begin{equation}
F_{\mathrm{scat}}(r)=\hat{z}\frac{n_{\mathrm{med}}}{c}C_{\mathrm{scat}}I(r)=\hat{z}\frac{128 \pi^5 R^6}{3 c \lambda_{0}^4}\Big(\frac{m_{\mathrm{obj}}^2 -1}{m_{\mathrm{obj}}^2 +2}\Big)^2 n_{\mathrm{med}}^5 I(r),\label{eq:fscat}
\end{equation}
and
\begin{eqnarray}
F_{\mathrm{grad}}(r)&=& \nabla [p(r,t) \cdot E(r,t)]\\
&=& \frac{4 \pi n_{\mathrm{med}}^2 \epsilon_0 R^3}{c} (\frac{m_{\mathrm{obj}}^2 -1}{m_{\mathrm{obj}}^2 +2}) \cdot \frac{1}{2}\nabla E^2(r,t) = \frac{2 \pi n_{\mathrm{med}} R^3}{c} (\frac{m_{\mathrm{obj}}^2 -1}{m_{\mathrm{obj}}^2 +2}) \nabla I(r) \label{eq:fgrad}
\end{eqnarray}
where $\hat{z}$ is the displacement vector along the optical axis, $r$ describes the out of plane distance from from the optical axis. $n_{\mathrm{med}}$ is the refractive index of the medium surrounding the object while $m_{\mathrm{obj}} \equiv n/n_{\mathrm{med}}$ is the refractive index
of the object relative to the surrounding medium. $p(r,t)$ is the dipole induced on the object from the electric field
$E(r,t)$ of the incident light, while $I(r)$ is the intensity of the lasers light field. $C_{\mathrm{scat}}$ is the scattering cross section of the object and $\lambda_0 $ is the wavelength of the incident light generating the optical potential. $R$ is the radius of the particle. Note that both the dipole response and forces are time averaged. The characteristic size scalings (w.r.t the levitated object) of the two forces are $F_{\mathrm{scat}}(r) \propto R^6$ and $F_{\mathrm{grad}}(r) \propto R^3$ in the Rayleigh limit only $(2 \pi R<<\lambda_0)$, this usually sets the requirement for the scattering force to be counterbalanced by another field (optical, gravitational etc) in the direction of beam propagation when the particle radius is a large fraction of the wavelength. A commonly employed trapping configuration for a Rayleigh-size particle is a single beam optical tweezer and is illustrated in Fig. \ref{canonical}.  The center of mass motion of the particle can be described by an asymmetric three-dimensional harmonic potential in the limit of small motional amplitudes. 

We also note that the gradient force described by Eq. (\ref{eq:fgrad}) is not universally applicable. In particular, dielectric particles sufficiently large compared to the trap wavelength and with large enough refractive index (e.g. silicon) can also exhibit strong magnetic dipole responses, with different force coefficients \cite{opticaltrapmag1,opticaltrapmag2}. Further, the force induced by the imaginary component of the Poynting vector generally needs to be considered \cite{Poynting1,Poynting2}. However for the low-optical-loss silica particles typically used in levitated optomechanics experiments, we expect Eqs. (\ref{eq:fgrad}) and (\ref{eq:fscat}) to be a reasonable approximation. 

For most setups, the beam profile is typically Gaussian, or a superposition of counter - propagating Gaussian beams:

\begin{equation}
I_{Gaussian} (x,y,z) = \frac{ I_{0}}{ \left(\frac{z^{2}}{z_{R}^{2}} + 1\right)} e^{\frac{- 2 x^{2} - 2 y^{2}}{w_{0}^{2} \left(\frac{z^{2}}{z_{R}^{2}} + 1\right)}}
\end{equation}

For the case of the gradient force, taking the derivative of such a function leads to a series of high order terms for each axis, providing coupling between the degrees of freedom for even a perfect sphere. In the limit of small displacement, these terms are negligible and a Gaussian potential approximates an $x^2$ or linear harmonic oscillator. Since $U = \frac{\chi I}{c}
$, $F(r) = -\frac{dU}{dr}$ and $F(r)=k \cdot r$ quantifying the coupling rates as a function of displacement is possible.

% Figure environment removed

In the Mie-Lorentz regime, in which the optical size parameter is roughly equal to the wavelength of the trapping light, this approximation breaks down and the computation of optical forces is achieved by a variety of other means. Fig. \ref{all_regimes} illustrates how the scattering cross section of a dielectric sphere varies with size parameter across the Rayleigh, Mie-Lorentz, and geometric optics regimes. 
...
In the general case, the force apparent upon the particle may be obtained by solving the Maxwell Stress Tensor :
...
The force and torque exerted on the object by the optical field can then be calculated as:

\begin{equation}
\overrightarrow{F} = \oint \overleftrightarrow{T} \dot \hat{n} dS
\end{equation}

\begin{equation}
\overrightarrow{\tau} = \oint \overleftrightarrow{M} \dot \hat{n} dS
\end{equation}
where the tensor $M$ is given by $\overrightarrow{M} = - \overleftrightarrow{T} \times  \overrightarrow{r}$ and $\overrightarrow{r}$ is the displacement vector. \cite{seberson2020stability}


This naturally requires the user to know the configuration of optical fields within the object, for complex objects not matching the cases above often obtained with an iterative solver. The Discrete Dipole Approximation (DDA)\cite{draine1994discrete} or Green's Dyad Method (GDM)\cite{wiecha2018pygdm} being two popular examples. Several other popular solvers exist in including MEEP (a Finite-Difference Time-Domain (FDTD) method)\cite{oskooi2010meep} and COMSOL\cite{comsol}.

While the Maxwell stress tensor is often quoted as the canonical method, equivalent integrals - for example over volume rather than area are often used\cite{wiecha2022pygdm,chaumet2000coupled}, with different scaling of numerical convergence and scaling of required computations resources in each case.

%give a specific example in the case of Dave's sphere's


It should be noted that while the restoring optical potential in the Mie-Lorentz regime is still defined by the gradient force, momentum carried in the propagation direction of the light field in the Mie-Lorentz regime is significant and creating a stable trap usually requires this force to be balanced against another optical beam, field, or gravity. 


%distinguish between mie lorentz and MST better here 

In the geometric optics limit, in which the optical size parameter is much larger than the wavelength, forces are generally given by conservation of momentum and 4-matrix formalism. Due to practical limitations of transferring adequate power to compete against gravity, few levitated optomechanics experiments take place within this regime\cite{PhysRevA.101.053857,PhysRevLett.105.213602}.
...
\subsection{Motion defined by optical forces}
...
At higher pressures (>~50mbar depending on object size and geometry) the motion is completely dominated by random momentum interactions with the background gases and undergoes a random walk around the optical potential.%, this manifests as $1/f$ behaviour in frequency space.

At lower pressures (<10mbar) - where the damping is linear and no longer the leading term in the equation of motion, the levitated object behaves like a driven damped harmonic oscillator, manifesting as a Lorentzian peak in frequency space for each degree of freedom with a width defined by the sum of its damping processes described in Fig. \ref{damping_lorentz}. A more detailed discussion of gas damping and Brownian motion is also included in Sec. \ref{sec:thermo}.

In the intermediate regime, where the mean free path of the gas molecules is of order the size of the dielectric particle to be trapped, additional instabilities are possible due to radiometric forces, which can dominate over the confining forces due to radiation pressure, as was first studied by Ashkin and coworkers in the late 1970s \cite{Ashkin:1977}. Thus one of the challenges of reaching high vacuum is to overcome radiometric forces. Of prime importance is to minimize heating of the particle surface from optical absorption, as temperature gradients across the surface of the levitated object can drive drive currents of gas molecules which are de-stabilizing over a range of pressure spanning orders of magntitude. A detailed study of these effects was performed in Ref. \cite{Atherton:2015} for trapped silica microspheres. Heating from optical absorption can be mitigated by choice of material as well as trapping laser wavelength. As described later in Sec. \ref{sec:thermo}, solid-state laser cooling may enable negligible heating of the particle surface, or cooling substantially below room temperaure.
To compensate for radiometric forces, laser feedback cooling at a rate $\Gamma_{\mathrm{fb}}$ can be utilized to restore damping that is no longer provided by the background gas being evacuated from the trapping chamber.

For each degree of freedom, the temperature or phonon occupation number is defined by the ratio of the heating and cooling processes in the system:

\begin{equation}
n_{\infty} = \frac{\Gamma_{\mathrm{th}} + \Gamma_{\mathrm{fb}}+\Gamma_{\mathrm{photon}} +\Gamma_{\mathrm{other}} }{\gamma_{\mathrm{th}} + \gamma_{\mathrm{fb}} + \gamma_{\mathrm{photon}}+ \gamma_{\mathrm{other}}},
\end{equation}

where $\Gamma_{\mathrm{th}}$ describes the heating corresponding to interactions with background gases, $\Gamma_{\mathrm{photon}}$ photon recoil scattering events, $\Gamma_{\mathrm{fb}}$ heating events corresponding to phase imprecision in the feedback loop and the lower case gammas $\gamma$ correspond to cooling events from the same physical interaction. Each of these can be thought as the levitated object's connection to a heat bath. At higher pressures the dominating term is the collisions with background gases, and at lower pressures various random processes defined by the laser dominate (photon recoil, laser phase noise etc). 

In the limit of negligible other technical heating and noise sources, i.e. $\Gamma_{\mathrm{other}}=\gamma_{\mathrm{other}}=0$, as well as negligible heating from feedback $\Gamma_{\mathrm{fb}} \approx 0$ and  $\gamma_{\mathrm{photon}}=0$, and where $\Gamma_{\mathrm{photon}}$ is dominiated by photon recoil heating via e.g. Rayleigh scattering, we arrive at the theoretical mean steady state phonon number for this limited case
\begin{equation}
n_{\infty} \approx \frac{\Gamma_{\mathrm{th}}+\Gamma_{\mathrm{photon}}}{\gamma_{\mathrm{th}}+\gamma_{\mathrm{fb}}} \approx \frac{n_i \gamma_g + \gamma_{\mathrm{sc}}}{\gamma_g+\gamma_{\mathrm{fb}}}
\end{equation}
where in the last equation we specialize to the case of a nanosphere optically levitated with trap frequency $\omega_0/2\pi$ in contact with a thermal gas at temperature $T$ with damping rate $\gamma_g$ where $n_i=k_B T / \hbar \omega_0$ with laser cooling at rate $\gamma_{\mathrm{fb}}$ and photon recoil heating at rate $\gamma_{\mathrm{sc}}$. 

The time required to enter equilibrium (thermalise) with any given heat bath is related to the heating rate of the process (in Hz) and the frequency of the degree of freedom of the oscillator, and can be on the order of milliseconds to seconds depending on the process and frequency, providing an accessible  platform for thermodynamics, as well as slightly non Markovian system noise.  
...
\subsection{Trapping configurations}
...
For low-optical-loss silica nanoparticles, with refractive index $n=1.45$, we expect Eqs. (\ref{eq:fscat}) and (\ref{eq:fgrad}) to be a reasonable approximation, and expanding about the equilbrium position for a focused Gaussian beam, it can be seen that a nanosphere will be confined in an approximately harmonic potential, and attracted to the point of maximal intensity with a nearly linear restoring force \cite{tweezerreview}. In vacuum, the motion of the trapped particle largely corresponds to a 3-dimensional simple harmonic oscillator in the under-damped regime, damped by background has collisions and driven by thermal noise. %with differing frequencies of oscillations in the transverse and axial directions.} 
...
Control over the specific material properties is especially and increasingly important given that such attributes of objects at this scale frequently deviate from those of the bulk, such as the (lower) density of silicon dioxide spheres made from the stober process for example \cite{blakemore2019precision}. This has spurred efforts in generating/creating/engineering new levitatable nanomaterials with bespoke desirable properties. Experiments to date include vaderite spheres \cite{ghosh2019fabrication}, and species with potentially useful internally accessible states: nanodiamonds\cite{rahman2016burning} for a spin degree of freedom, 
 NaYF\cite{Luntz-Martin:21,winstone2022optical} and YLF\cite{rahman2017laser} crystals for optically accessible internal state cooling.
...
+\omega_{m}^{2}q(t)=A\eta(t)+u_{\mathrm{fb}}(t),    
\end{equation}
where $q(t),\Gamma_{m},$ and $\omega_{q}$ are the position, center-of-mass damping, and oscillation frequency of the nanoparticle, respectively. The stochastic noise due to sources such as Brownian fluctuations from the background gas, optical scattering and feedback backaction is represented by the random variable $\eta(t)$ of strength $A$. The feedback applied to the nanoparticle is accounted for by $u_{\mathrm{fb}}(t)$. Various kinds of feedback have been used. Linear damping proportional to the velocity, with $u_{fb}(t)=C_{l}\dot{q}(t)$, has been used to cool the particle motion [], as well as nonlinear damping, with $u_{fb}(t)=C_{nl}q(t)\dot{q}(t)$. The terms linear and nonlinear are used here with the significance that the particle damping rate is independent of the energy for linear damping and depends on energy for nonlinear damping. A comparison of the efficacy of the different types of feedback for cooling has been carried out \cite{PhysRevA.104.023502}. State base Bayesian models from control theory (such as Kalman filters), have also been demonstrated experimentally for feedback \cite{magrini2021real}.
...
The central observational modality for levitated optomechanics is the detection of the nanoparticle's position. This information can be processed, while making some reasonable assumptions, to obtain the energy (both transient as well as steady-state) of the particle. In experiments with cavities, the mechanical displacement information can be found from the transmission of the optical cavity \cite{RevModPhys.86.1391}. In experiments without cavities, oscillator displacement may be obtained by interfering the light scattered by the nanoparticle with a local reference field \cite{PhysRevLett.109.103603}, or by directly observing the scattered light of the particle in a spatially sensitive detection system \cite{ranjit2016zeptonewton}, as illustrated in Fig.  \ref{detection_mechanisms_figure}.
...
Each frequency band of information propagates into space via a its own set of Bessel functions, meaning that the position of the detector on the optics bench determines the ratio of information it sees about each degree of freedom and its resonances. Solutions for optimal placement of the detector with respect to the trapping location can be found in Ref.  \cite{tebbenjohanns2019optimal} for Rayleigh sized particles, and in Ref.  \cite{maurer2022quantum} for Mie-Lorentz regime sized spheres. The computation of these parameters for non spherical particles of arbitrary geometries is an ongoing element of research. 

In terms of practical experiment construction, phase bound information is recovered by interfering the scattered light from the levitated object with a local reference field - conditioned for maximum interference and spatial mode overlap, while in the case of intensity modulated detection the objects motion is usually literally imaged on a spatially sensitive detector. A review of the ultimate sensitivity limits of both methods (for Rayleigh regime sized spheres) may be found in Ref. \cite{tebbenjohanns2019optimal} - as well as a discussion on optimum detector placement with respect to the trapping region. An extension of this discussion on optimum detector placement to particles in the Mie-Lorentz regime may be found in Ref. \cite{maurer2022quantum}.

\subsubsection{Interpreting data - volts to meters and SI unit conversions}

Given the complex set of systematic errors which are coupled to the conversion of photons to volts to nanometers (in terms of particle motion), we stress the importance of calibrating the sensor under its final operating conditions. For an example if the placement of the detector upon on the tabletop affects the proportional of frequency band information available at each detector (important for the final volts-meters conversion), then thermal drift and other positional systematics will affect the measurement. Indeed, it is possible to accidentally fool oneself by pathological placement of a detector optimising for certain frequency bands at the expense of others. An experimentalist might trap a nonspherical particle and place a detector in such a position as to ignore or de-weight the frequency peaks containing rotational information, and conclude they have trapped a sphere. For microscopic parameters; experimental pressure and COM temperature dramatically affect the volts-meters conversion. COM temperature especially - many papers in the field assuming a linear conversion of volts to meters, it has been demonstrated that this is a deterministically nonlinear function \cite{rondin2017direct}.
While a complete discussion is beyond the scope of this Tutorial article, references for the interpretation of detector signals and transforming then into physical displacement units can be found in Refs. \cite{hebestreit2018calibration, dawson2019spectral}.
...
\section{Hybrid and coupled systems with multiple degrees of freedom}
...
Recent work has demonstrated a light-mediated coupling between the spins of cold atoms and the mechanical motion of a membrane in a cavity \cite{treutlein2022}. By utilizing a cryogenically precooled membrane,this method is predicted to allow cooling of the mechanical oscillator close to its quantum mechanical ground state and permit the preparation of non-classical states. Similar experients using the spins degrees of freedom of cold atoms could also be applied to levitated optomechanical systems.

\subsection{Cold and degenerate atomic gases and optical cavities}
%\textcolor{blue}{George please add the refs below to the bib file; also, please supply a figure. Thanks! - Mishkat}
The ready availability of samples of cold as well as degenerate samples of atoms have led to levitated optomechanical studies of such gases coupled to optical modes of electromagnetic cavities. 
...
\begin{equation}
	S(\omega)=\frac{2k_B T}{m \omega^2_m} \frac{\omega^2_m \Gamma_{gas}}{(\omega^2_m-\omega^2)^2+\omega^2 \Gamma_{gas}^2},
\end{equation} 
where $\Gamma_{gas}$ is the damping rate of the levitated particle due to air molecules. This equation is often used to calibrate the detection system in levitated optomechanical experiments. For a spherical particle, the damping rate at thermal equilibrium is \cite{Li:2011}
\begin{equation}
\Gamma_{gas}=\frac{6\pi \eta R}{m} \frac{0.619}{0.619+Kn}(1+c_k),
\end{equation} 
...
During experiments in high vacuum a levitated particle no longer thermalizes with the surrounding gas environment due to an increase in the mean-free-path between particle collisions.   Consequently, the power from optical absorption no longer can be carried away by the reduced number of molecular collisions, leading to an increase in the particle’s internal absolute temperature (T$_{int}$).  The molecules that do collide with a trapped particle leave the particle’s surface with a temperature (T$_{em}$) greater than the surrounding gas temperature (T$_{gas}$), leading to a new damping term $\Gamma_{em}/2\pi = 1/16 \cdot \sqrt{(T_{em}/T_{gas})} \cdot \Gamma_{gas}$.   If optical absorption is high enough, additional effects become significant including photophoretic forces \cite{RN296}, changes in the trapped particle’s physical properties (density, refractive index), particle melting, or even complete particle destruction through evaporation \cite{Chang:2010} or burning \cite{Rahman2016}.
...
Levitated optomechanical experiments are naturally highly sensitive to momentum impulses \cite{moore2021searching}. Experiments have therefore been designed such that study of nuclear decays may be possible by directly observing the motion of a levitated optomechanical sensor embedded with a decaying radioactive species. As the species decays, the momentum from the outbound high energy ejecta is offset by an equal and opposite momentum impulse on the levitated sensor.

This, in principle might allow for the study of nuclear processes without having to transform the outbound nuclear ejecta directly into electrons and may therefore be sensitive to parameter spaces not covered by existing detector technologies. For example, heavy sterile neutrinos \cite{carney2023searches}.
...
%\subsection{Entanglement \textcolor{blue}{(Andy/George)}}
% Markus A pretty keen on entangling two particels

%\subsection{CSL bounds \textcolor{blue}{(Andy)}}
%CSL parameter space almost gone - hopefuly all ruled out next ten years

%\subsection{Tests of quantum gravity \textcolor{blue}{(Andy)}}
...
We thank Alexey Grinin and Andrew Dana for useful discussions and for assistance with Visualization 1.
