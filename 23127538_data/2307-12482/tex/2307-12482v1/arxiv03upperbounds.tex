...
The hardness of achieving optimal envy even on simple classes of graphs (e.g. disjoint unions of paths, cycles, or stars) \citep{canon} immediately gives rise to the question of whether we can approximate optimal solutions. As stated before, we need to assume connectivity in general.
...
In what follows, we first discuss how to improve this bound for bounded-degree trees and then generalize this result to graphs based on a structural parameter called the \emph{cutwidth}.
...
We will use the following folklore fact\footnote{ For a proof of this fact, see, for instance, \cite{mlatrees}, who attributes this as a folklore result to \cite{seidvasser}, who claims the fact is well-known, but proves it anyway.} without a proof.

\begin{fact}[Folklore]\label{fact:folklore}
    Every $n$-vertex tree $T$ has a \emph{center of gravity}: i.e., a vertex $v$ such that all connected components of $T - v$ have at most $n/2$ vertices. Furthermore, this vertex $v$ can be found in $O(n)$ time.
\end{fact}

\begin{algorithm}[t]
    \caption{Recursive Algorithm $\mathsf{TrickleDown}(T, H)$}
    \hspace*{\algorithmicindent} \textbf{Input:} {A {\GHA} instance on a tree $T$ and a set of values $H = \{h_1, \ldots, h_n\}$.} \\
    \hspace*{\algorithmicindent} \textbf{Output:} {An $O(\Delta\log n)$-approximate allocation.}
    \begin{algorithmic}[1]
        \If{$|T| = 1$}
        \State \textbf{Allocate} the only house in the instance to the only vertex in $T$.
        \Comment{Base case}
        \Else
        \State Find a center of gravity $v$ of $T$.
        \State Let $T - v$ consist of subtrees $T_1, \ldots, T_k$, with size $n_1, \ldots, n_k$. 
        \Comment{$k \leq \Delta$, and $n_i \leq n/2$ for each $i$.}
...
\begin{theorem}\label{thm:treelogn}
There is an $O(n\log n)$-time algorithm that, given any instance of {\GHA} on a tree with maximum degree $\Delta$, returns an allocation whose envy is at most $\Delta\log n$ times the optimal envy.
%Algorithm \ref{alg:treelogn} runs in time $\tilde{O}(n)$, and returns an $O(\Delta\log n)$-approximation to the optimal envy.
\end{theorem}
%\andrew{I think with a more careful analysis this can be made $(\Delta+1)/2+\Delta/2 \times \log_2 n$}
%\rik{I agree, $\Delta$ at level $1$, $\Delta - 1$ in each of the next $\log n - 1$ levels. Oh, I see your updated one now too, yes, put $v$ on the middle value, nice.}
%\andrew{i don't know whether it's worth changing -- it just occurred to me when i was commenting on the next corollary.}
\begin{proof}
We will show that Algorithm \ref{alg:treelogn} provides the desired guarantee. The algorithm starts by locating the center of gravity $v$ of the given tree $T$ (which is guaranteed to exist by Fact \ref{fact:folklore}). Then it assigns the largest (i.e., rightmost) value to node $v$, and recursively constructs the assignment for each of the disjoint subtrees in $T - v$.
      ...
      It is easiest to visualize the allocation resulting from Algorithm \ref{alg:treelogn} as in Figure \ref{fig:treelogn}. All recursive calls in line 9 occur in a single ``level'' of the figure, and all subsequent recursive calls from the subtrees $T_1, \ldots, T_k$ can also be packed into a single level, as the relevant edges do not interact. The crucial point is that the envy incurred strictly within disjoint subtrees $T_i$ and $T_j$ cannot involve the same smallest subintervals of the original instance.
...
We remark that with a slightly more careful analysis, we can improve the approximation ratio to $(1/2)\cdot(1 + \Delta +\Delta\log n)$.\footnote{ Technically this involves tweaking the algorithm such that the center of gravity is assigned slightly differently in line 7, and the partition of $H$ is consistent with this.} In particular, for any instance of {\GHA} on a binary tree, the optimal envy can be $(2\log n)$-approximated in $O(n\log n)$ time. % The following observation readily follows from \Cref{thm:treelogn}. \andrew{personally, i'd skip this corollary. but if we keep it, note that $\Delta=3$ for a binary tree so while a $2\log n$ approximation is possible, you'd only get $3\log n$ from the above theorem.}
...
In this section, we generalize the results from Section \ref{sec:boundeddegreetrees} using the structural graph theoretic property of \emph{cutwidth}. This will enable us to have a black-box process to obtain envy approximations parameterized by the cutwidth. All of these algorithms will be value-agnostic. Let us first define the cutwidth of a graph.

%\hadi{consider moving all below  Section 2.2.}\rik{we decided this was better}

\begin{definition}\label{def:cutwidth}
    For a graph $G = (V, E)$ on $n$ vertices, let $\sigma = (v_1, \ldots, v_n)$ be any ordering of its vertices. The \emph{width} of the ordering $\sigma$ is defined as
    \begin{equation*}
...
The ordering $\sigma$ of a graph $G$ is often called a \emph{layout}. A (nearly) \emph{optimal layout} is an ordering that (nearly) achieves the cutwidth of $G$. The cutwidth is closely related to the other standard notions of width defined in Section \ref{sec:prelims}. In particular, we can extend the chain of inequalities at the end of that section as follows (from \citet{korach1993cutwidth}):
\begin{equation}
    \tw(G) \leq \pw(G) \leq \cw(G) \leq O(\Delta\cdot\pw(G)) \leq O(\Delta\cdot\tw(G)\cdot\log n)
    \label{eq:twpwcw}
\end{equation}
...
% There is a dynamic $O(n\cdot 2^n)$ algorithm known, and a quantum algorithm that achieves a better runtime of $O(1.817^n)$.

% \hadi{up to here..........................}

% Approximating the cutwidth is a slightly easier problem. 
If $G$ is sufficiently dense, there is a polynomial-time approximation scheme for the cutwidth \citep{frieze1996cutwidthdense}. In general, there is an efficient $O(\log^{1.5} n)$-approximation of the cutwidth known \citep{leighton1999cutwidthapprox}, which also returns a layout achieving this ratio. We can use this algorithm to obtain approximate allocations for {\GHA}.


\begin{theorem}\label{thm:cwgeneric}
Let $G = (V, E)$ be a connected graph, and $(G, H)$ be a {\GHA} instance on this graph. Given a layout $\sigma$ that $\beta$-approximates the cutwidth of $G$, we can efficiently construct an allocation $\pi$ that is a $(\beta \cdot \cw(G))$-approximation to the optimal envy.
\end{theorem}
\begin{proof}
...
\begin{enumerate}[(i)]
    ...
    \item An $O(\sqrt{n \Delta \log^{3} n})$-approximation algorithm on planar graphs,\label{cor:cwplanar}
...
%\rik{I think me followed by Andrew already took care of the subsection above, except for the corollar(ies) at the end. I actually don't remember the specific corollaries we talked about -- so can someone populate that part? In particular, the previous subsection might be subsumed by this result.}
%\vignesh{Yes it does. I'm not sure what we want to do about it. Depends on what SODA reviewers will like.}