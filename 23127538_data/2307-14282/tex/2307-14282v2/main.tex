...
Such discontinuities arise from unpredictable admission cutoffs that characterize the matching of students to schools.
...
Controlling for local preferences that equal a pair of schools, e.g., $(a,b)$, allows the RD to identify the causal effect of a change in the school assignment from $b$ to $a$, averaged over individuals who prefer $a$ over $b$.
A natural question to ask, then, is how often students submit their true preferences and what the consequences are of controlling for misreported preferences in the RD strategy.
...
Although a powerful identification strategy, this paper shows that the RD strategy that controls for reported preferences raises two important concerns, unless we assume everyone is a truth-teller.
First, we argue that an average effect over students that truly prefer $a$ over $b$ is more externally valid than an average effect over students that only report to prefer $a$ over $b$
because preference reports could easily change in counterfactuals with different incentives.
Second, even if interest lies on the second type of average effect, we show that controlling for reported preferences may invalidate the fundamental continuity assumption of the RD identification strategy. 
...
%{\color{red}Add a sentence: Using results on random set theory developped in Molinari and Chesher and Rosen (2017), this property allows us to provide a tractable characterization of sharp bounds on the distribution of the true local preference.}
...
The reason is that, when we select individuals with the same (strategically) reported local preferences near a cutoff, our local preference sets change discontinuously at that cutoff. 
...
The bounds that we compute are informative about the sign of average treatment effects in 33--67\% of the university--major pairs, depending on the outcome.
...
Thus, we explicitly assume every one is a truth-teller. 
...
This step features high-level assumptions imposed on the local preference sets such that the researcher is not restricted to the methods in Section \ref{sec:id_r:qj_id}.
...
This example illustrates why an RD that controls only for $P_4=(4,2)$ may fail to identify the treatment effect of a change in assignment from school $2$ to school $4$.
...
This does not necessarily require that these agents are certain ex ante about the cutoff value $c_4$ but simply that they have expectations about that cutoff that change as $S_1$ crosses $c_4$.
\end{example*}
...
The expressions for the bounds in the case of binary $Y$ change relative to the case of continuous $Y$,   
...
Although intuitive, these bounds are not necessarily sharp because $\lbar{\delta}^+_{j,k}$ is not exogenously given as considered by \cite{horowitz1995};
...
Indeed, Part III of our SD example (Section \ref{sec:id_r:qj_id}, right after Proposition \ref{result:partial_id:nextbest}), shows that the distribution of the potential outcomes conditional on reported local preferences and the priority score can change discontinuously at the cutoff in the presence of strategic behavior. 
