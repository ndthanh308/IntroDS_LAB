...
extending the result to skew shapes \cite{sun2018dimer}. {These techniques lead to some natural gradient variational problems in $\R^2$ whose solutions are explicitly parameterized by $\kappa$-harmonic functions, as show in \cite{KenyonPrause2022variational}.}
...
see the left-hand side of \cref{fig:interlacing1}. We call this graphical representation the \emph{Young diagram} of shape $\lambda$.
...
A \emph{Young tableau} of shape $\la$ is a filling of the boxes
...
is a finite subset of $A\times[0,1]$ where $A$ is a finite sub-interval of $\mathbb{Z}$, while in the infinite case a bead configuration is a locally finite subset of $\mathbb{Z} \times \R$.
\medskip
...
\begin{equation}\label{eq:boundary_conditions}
	\begin{cases}
		H_{\la,T}(a_0,t)=H_{\la,T}(x,0)=H_{\la,T}(a_m,t)=0,\quad&\text{for all }t\in[0,1],\\
...
\begin{definition}[Liquid region]\label{defn:liquid_region}
	We let $L$ be the set of pairs $(x,t)$ in $[\eta\, a_0, \eta\, a_m] \times [0,1]$ such that \eqref{eq:critical_intro} has two non-real solutions and we call it {\em liquid region}.
...
\end{equation}
where $\Im$ and $\Re$ denote the imaginary and real part of a complex number.
...
\begin{remark}\label{rem:cont_points}
	...
	$D^{\mathrm{reg}}_{\lambda^0}$ for all but countably many $y$. As a consequence, $D_{\lambda^0} \setminus D^{\mathrm{reg}}_{\lambda^0}$ has zero (two-dimensional) Lebesgue measure. Moreover, if $(x,y)\in D_{\lambda^0} \setminus D^{\mathrm{reg}}_{\lambda^0}$ and $x-\eps,x+\eps \in D^{\mathrm{reg}}_{\lambda^0}$, then
	$T^\infty(x-\eps,y)$ and $T^\infty(x+\eps,y)$ converge respectively
	to $T^\infty_-(x,y)$ and $T^\infty_+(x,y)$ as $\eps$ tends to $0$ with $\eps>0$.
	Therefore the limiting surface $T_\infty$ is discontinuous at such points $(x,y)$
...
In this section, we consider a rectangular diagram $\la_0$.
...
An essential difference between the two examples is that the heart example, the interlacing coordinates satisfy Condition~\eqref{eq:cont_cond},
while this is not the case in the heart example.
...
as soon as $x_0 \sqrt{|\la^0|}$ is a rational number, it is possible to replace $\la^0$ with a rescaled version of $\la^0$ so that $x_0 \sqrt{|\la^0|}$ is an integer.}
...
Let $M_{\be}$ be the infinite bead process of intensity $1$ and skewness $\be\in (-1,1)$. There is a natural bijection between the set of beads in $M_{\be}$ and the set $\set{(x,y)\in\Z^2 | x+y \text{ is odd}}$: we label by $(0,1)$ the first bead in the zero-thread with positive height, and then we label all the other beads as shown in the left-hand side of \cref{fig:beads_to_SYT2}.


% Figure environment removed

Given a bead $(x,y)$ in $M_{\be}$, we denote by $H_{\be}(x,y)$ the height of the bead $(x,y)$. 
...
\begin{corollary}[Local limit for the uniform Poissonized Young tableau; corollary of \cref{thm:cv_bead_process}]\label{corol:local_lim_standard Young tableau}
	...
	We denote $\oblong_N$ the box corresponding to
...
as local limit of finite random Young tableaux, they should have an interesting invariant re-rooting property.
\end{enumerate}
...
\cref{sec:KernelRenormalized} is devoted to the analysis of the kernel of the random bead process $\widetilde{M}_{\la_N}^{(x_0,t_0)} $ in \eqref{eq:defn_of_Mla}. The main goal of this section will be to identify the \emph{critical points of the action} (\cref{ssec:localization-critical-points}).
...
\subsection{Gorin--Rahman's determinantal formula}
...
\subsubsection{The Gorin--Rahman's correlation kernel}
...
\begin{proof}
	...
	\begin{multline}
		...
		\label{eq:Tech2}
	\end{multline}
	We now integrate the right-hand side over the close contour $\gamma_w$.
...
The integration contour are different in the case $t_1 \ge t_2$
and $t_1<t_2$.
...
\end{equation}
so that, for $N$ large enough, we can take (for instance) $\gw=\partial D(0,3L)$ and $\gz=\partial D(0,4L)$ (both followed in counterclockwise order). For convenience, we will often write $\Int_N(W,Z)$ for the integrand in \eqref{eq:double_int_integr}.
...
In the following, for two sequence $A=(A_N)$ and $B=(B_N)$, we write $A \simeq B$ when $A=B(1+O(N^{-1/2}))$.
Moreover, unless stated otherwise, when $A$ and $B$ depends on $Z$ and $W$,
...
\end{align}
We now estimate the quotients involving $F_{\la_N}$ and $\Gamma$ in \eqref{eq:double_int_integr}. In what follows we use the principal determination of the logarithm, defined and continuous on $\mathbb C \setminus \mathbb R_-$. 
...
\end{equation}
Let also introduce a specific subset $\DS$ of the complex plane.
...
\end{equation}
Indeed, the terms of order $\Theta(U\log U)$ cancel out.
...
applied to negative real number.
...
Moreover, the gamma factors can be controlled as above using Stirling formula (all their arguments
...
$|\sin(z)| \le \exp(|\Im(z)|)$, and using Stirling formula for the $\Gamma$ factors.
\end{proof}


\subsection{The critical points of the action and the shape of the liquid region} 
...
Then for each $i$ in $\{1,\dots,i_0-1\}$,
the critical equation $\frac{\partial S}{\partial U}(U) = 0$ has 
at least one real root in the interval ${[}\eta\, b_{i} -x_0, \eta\, a_i -x_0)$,
while, for each $i$ in $\{i_0+1,\dots,m\}$, it has at least one real root
in $(\eta \,a_{i-1} -x_0, \eta\, b_{i} -x_0{]}$.
\end{lemma}
\begin{proof}
The critical equation  writes as
\begin{equation}
...
\end{equation} 
{If $t_0=1$ then the lemma statement immediately follows, hence we assume that $t_0\in[0,1)$ (so that the factor $(1-t_0)$ is non-zero).}
The lemma is then easily obtained by looking at the signs of the left and right-hand sides
\begin{equation}\label{eq:left_right_fct}
	L(U) := U\prod_{i=1}^m(x_0-\eta\,b_i + U)
	\qquad \text{and}\qquad 
	R(U) := (1-t_0)\,\prod_{i=0}^m(x_0-\eta\,a_i+U)
\end{equation}
at the boundary points of the intervals introduced above. For instance, if $1 \leq i \leq i_0-1$, then
\begin{itemize}
	\item $L(\eta\,b_i-x_0)=0$ and $R(\eta\,b_i-x_0)$ {is non-zero and} has sign $(-1)^{m-i+1}$;
	\item $L(\eta\,a_i-x_0)$ {is non-zero and} has sign $(-1)^{m-i+1}$ {(because $0 \in  (\eta \,a_{i_0-1} -x_0, \eta\, a_{i_0} -x_0)$)} and $R(\eta\,a_i-x_0)=0$;
\end{itemize}   so the difference $L(U)-R(U)$ has to vanish between these two boundary points.
\end{proof}
...
\item {Finally, if $t_0\in(t_-(x_0),t_+(x_0))$ and the critical equation~\eqref{eq:critical_intro} has only real solutions, then the two extra real solutions\footnote{in addition to the ones determined by \cref{lem:critical_points_general}.} (counted with multiplicities) are either both inside a negative interval $(\eta\,b_{j_0} -x_0,\eta\,a_{j_0} -x_0)$ for some $j_0<i_0$,
  or both inside a positive interval $(\eta\,a_{j_0}-x_0,\eta\,b_{{j_0}+1} -x_0)$ for some $j_0\geq i_0$.}
    \end{itemize}
\end{proposition}
...
An illustrative examples of the various results obtained in this section is given in \cref{fig:liquid_region_and_landscape}.
...
\subsection{The imaginary and real part of the action on the real line}
...
In what follows, we keep working with the principal determination of the logarithm,
...
\Re F(0)\}=\{U=x+iy\in\mathbb C:(x+y)(x-y)=0\}$.} there are \emph{eight special  curves} leaving $U_c$ (see for instance the left-hand side of \cref{fig:landscape_ReS}): four curves corresponding to the level sets $\{\Re S(U) = \Re S(U_c)\}$
 ...
 each of this region containing one imaginary level line.
...
\end{proof}
 ...
 (in particular larger than $K |\Im(U)|$ for come constant $K>0$),
...
and for some values of $Z$ in the interval $I_Z=[o(1),\eta\, a_m -x_0-1]$. We also recall from \eqref{eq:bound_L} that we chose the contours $\gw=\partial D(0,3L)$ and $\gz=\partial D(0,4L)$ (both followed in counterclockwise order), with $L=\eta \max(|a_0|,a_m)$. (Recall that we restricted our analysis to the case $t_1 \ge t_2$, i.e.\ when $\gamma_W$ is inside $\gamma_Z$.)
...
\end{equation}
Note that $\gamma$ is indeed a path form $U_c$ to $\overline{U_c}$ passing on the left of $0$, as required in the case $t_1 \ge t_2$.
...
\end{equation*}
with $g(y,\eps)=\exp(\frac{\eps R\cos\theta}{1-t_0}) \,(\frac{R}{1-t_0})^{x_2-x_1}$.
...
we integrate over the following path $\gamma$ going from $\overline{U_c}$ to $U_c$
 ...
 or go to infinity in the negative real direction.
  ...
  by the conditions $\bx>0$ and  $\Im(S(\bx))=-\pi x_0$.
    \item 
      ...
      the uniqueness by our assumption that the critical equations has two solutions in $(-\infty,\eta \,a_0-x_0)$).
  \end{itemize}
\smallskip
 ...
 The case when $y=\eta a_i-x_0$ or $y=b_i-x_0$ is a bit more delicate. Locally around $y$, the function $S$
 ...
 point $y=\eta a_i-x_0$ or $y=b_i-x_0$ , there is a direction along the real line in which $\Im S(u)$ is constant).
...
Propositions \ref{prop:convergence_kernel_frozen1bis}, \ref{prop:convergence_kernel_frozen2}, and \ref{prop:convergence_kernel_frozen3} completes the proof of the second item of \cref{thm:cv_bead_process}.
...
\begin{proof}[Proof of \cref{thm:limiting_surface_formula}]
	...
	Recall now that $\a(x,s):=\frac{\Im U_c}{1-s}\mathds{1}_{(x,s)\in L}$, where $L$ denotes the liquid region. If $(x,s)$ is in the liquid region, \cref{eq:Kinfty,eq:asymp_Ktilde} implies that
...
we can use Birkhoff ergodic theorem and conclude that, a.s.,
...
Using Skorohod representation theorem,
...
in the probability space constructed by Skorohod representation theorem.
