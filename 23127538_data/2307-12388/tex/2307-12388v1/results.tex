...
$\bullet{\textbf{RQ1:}}$ Does performance gap exist in TSC?\\  
$\bullet{\textbf{RQ2:}}$ Can \ours effectively mitigate the performance gap?\\  
$\bullet{\textbf{RQ3:}}$ How do dynamic grounding rate $\alpha$, uncertainty quantification, and action grounding influence the performance of \ours?\\ 
$\bullet{\textbf{RQ4:}}$ Does \ours method perform stably with various uncertainty quantification methods?
...
\toprule
Setting & \begin{tabular}[c]{@{}c@{}}accel \\ (m/$s^2$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}decel \\ (m/$s^2$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}eDecel \\ (m/$s^2$)\end{tabular} & \begin{tabular}[c]{@{}c@{}}sDelay \\ (s)\end{tabular} & Description        \\ \midrule
% v1 ==> v3, v2 ==> v4
...
We consider the places under special weather. In Table \ref{param}, $V3$ and $V4$ represent rainy and snowy conditions respectively. In these two settings, the vehicles' corresponding accelerating, decelerating, and emergency decelerating rates are smaller than the default, and the startup delays are larger. For snowy weather, the first three rates are smaller than rainy ones, and the startup delay discrepancy in snowy conditions is extended to mimic the tire slip.



\subsubsection{\textbf{Evaluation Metrics}} 
Following the literatures in TSC~\cite{zhao2011computational, wei2021recent}, we adopt commonly used traffic signal control metrics as below: \\
...
$\bullet$ \textit{Throughput (TP)} is the number of vehicles that have reached their destinations in a given amount of time. The larger $TP$, the better.\\
$\bullet$ \textit{Reward} is an RL term that measures the return by taking action $a_t$ under state $s_t$. We use the total number of the waiting vehicles as the reward, aligned with Preliminaries~\ref{pre:rewarddefine}. The larger reward, the fewer waiting vehicles, the better.\\
$\bullet$ \textit{Queue} is the number of vehicles waiting to pass through a certain intersection in the road network. The smaller queue, the better.\\
$\bullet$ \textit{Delay} is the average delay per vehicle in seconds and measures the amount of time that a vehicle spends waiting in the network. The smaller delay, the better.

In this work, since our goal is to mitigate the performance gap of trained policy $\pi_{\theta}$  between $E_{sim}$ and $E_{real}$, we additionally calculate the gap $\Delta$ for each referred metric as $ATT_{\Delta}$, $TP_{\Delta}$, $Reward_{\Delta}$, $Queue_{\Delta}$, and $Delay_{\Delta}$. 
...
To guarantee the reproducibility of the work, we provide the details of important hyperparameters in \ours method.
We adopted the DQN implementation in~\cite{mei2022libsignal} and keep the same hyperparameters. 
The forward and inverse models are implemented with the feed-forward neural network with three hidden layers, which have 64, 128, and 20 hidden neurons respectively. The forward and inverse models are optimized with Adam optimizer~\cite{kingma2014adam}, using $Betas=(0.9, 0.99)$ and $batch size = 64$, with specific $lr=1e-4$ for the forward model and $lr=1e-5$ for the inverse model. For uncertainty quantification, we follow the implementation in~\cite{sensoy2018evidential} and add an extra $L2$ regularization in the last layer.
The total training iteration $I$ for UGAT is 300, and the RL policy is trained in total $E=100$ epochs with each epoch having $T=360$ steps. 
...
\subsubsection{\textbf{Gap between real-world and simulator (RQ1)}}
...
\subsubsection{\textbf{Gap mitigating by uncertainty-aware \ours (RQ2)}}
...
The ($\cdot$) in Table~\ref{tab:main} describes the performance gap from $E_{real}$ to $E_{sim}$ using the results above and calculated as Equation \eqref{eq:delta}. The gap directly reflects the methods' ability to generalize from $E_{sim}$ to $E_{real}$. We have the following observations:

$\bullet$ Compared with the Direct-Transfer method, applying \ours is able to mitigate the performance gap $\Delta$. It is observable that the $ATT_{\Delta}$, $Queue_{\Delta}$, and $Delay_{\Delta}$ in \ours are smaller than in Direct-Transfer column, and $TP_{\Delta}$, $Reward_{\Delta}$ are the larger, which means \ours effectively mitigates the performance gaps.

$\bullet$ On the original metrics of traffic signal control, \ours could improve the performance of policy $\pi_{\phi}$ performance as well. When comparing the two methods, \ours could reach much lower $ATT$ and higher $TP$ than Direct-Transfer.

$\bullet$ In Table \ref{tab:main}, the experiments include 5 different metrics, and the whole processes are conducted across 4 various real-world settings. Based on the reported results, we can conclude that \ours is both robust and effective to multiple metrics and complicated environmental settings.
...
\subsubsection{\textbf{Ablation Study (RQ3)}}
...
\subsubsection{\textbf{Different Uncertainty Methods in \ours (RQ4)}}
