...
\bottomrule
\end{tabular}
\caption{Percentage of outputs preferred by GPT-4 when comparing \rlaif{}-Few to \ours{} on the harmlessness prompt set. The comparison is somewhat unfair to \ours{} since \ours{}'s prompts during preference data simulation are zero-shot rather than few-shot, but \ours{} still performs much better at 7B scale for preference data simulation, and does not lose heavily at 30B scale either.%\todo{}
}
