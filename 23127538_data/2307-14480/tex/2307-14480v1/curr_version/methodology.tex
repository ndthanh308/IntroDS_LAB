...
Next, we use the positions $P^T$ to generate the initial set of seed $tests$ (line 7). 
Then, the main fuzzing loop starts, which iterates until either the target coverage ($tc$) is achieved or the runtime exceeds the time limit, $t_{limit}$ (line 8). 
In each iteration, we first simulate the DUT with the generated $tests$ to obtain the function $\mathcal{F}^M$ to calculate the fitness of the particles and the coverage achieved so far, $cov$ (line 9). This $\mathcal{F}^M$ is used to calculate the local and global bests for the particles for mutation operators and to decide which particles need to be reset according to $\beta^M$ following Algorithm~\ref{alg:reset} (line 10). 
Then, the velocities and the positions of the particles for mutation operators are updated (line 15), and the new $tests$ are generated for both particles that are reset (line 16) and not reset (line 17). 

Additionally, if any of the particles for the mutation operators needs to be reset, i.e., $rst\_P^M \neq \phi$ (line 11), we calculate the number of iterations those particles survived (i.e., fitness) and return $\mathcal{F}^T$ (line 12). This $\mathcal{F}^T$ is used to calculate the local and global bests for the particles for seed generation and to decide which ones to reset according to $\beta^T$ (line 13). 
Finally, the velocities and the positions of the particles for seed generation are updated (line 14).