...
Following~\cite{kande2022thehuzz}, we set the number of simultaneous threads and hence the number of particles, $|N|$, as $10$ and the number of instructions in each program, $|O|$, as $20$. To have a balanced impact of exploration (due to velocity) and exploitation (due to local and global bests), we set $k$ (Eq.~(\ref{eq:up_v})) as $0.5$. Based on empirical observations, we set $\beta^M = \beta^T = 3$, as a larger $\beta$ will cause runtime overhead, while a smaller $\beta$ will make \ourtool{} generate test cases similar to random exploration, as discussed in Sec.~\ref{sec:solution1}.
