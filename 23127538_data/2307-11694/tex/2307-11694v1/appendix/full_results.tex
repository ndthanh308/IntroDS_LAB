...
We consider three BERT training variations: the original BERT \cite{devlin2019bert}, SciBERT \cite{beltagy2019scibert}, and BioLinkBERT \cite{yasunaga2022linkbert}. SciBERT was trained on a corpus of scientific documents which would be considerably more focused on drugs than a general corpus. BioLinkBERT is a biomedical BERT model additionally trained using document relation prediction (e.g. citation links).

\paragraph{ChemicalX Results} We report full results on the subset of DrugCombDB \cite{liu2020drugcombdb} used by ChemicalX \cite{rozemberczki2022chemicalx} in Table \ref{tab:full_transductive_results}.
...
\paragraph{Protonets}
As another meta-learning baseline, we consider Protonets \cite{snell2017prototypical}. We use the same meta-learning framework as for MAML. Because we don't have drug task meta-data, we only consider the few-shot setting. We find that the Protonet's 
