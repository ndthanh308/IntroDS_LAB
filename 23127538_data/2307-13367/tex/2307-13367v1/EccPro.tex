...
\author[M. Garg et al.]{Mudit Garg,$^{1}$\thanks{E-mail: mudit.garg@ics.uzh.ch} Shubhanshu Tiwari,$^2$ Andrea Derdzinski,$^1$ John Baker,$^3$ Sylvain Marsat,$^4$ \newauthor and Lucio Mayer$^1$\\
...
We generate eccentric waveforms $\tilde{h}_{\rm ecc}(f)$ for our systems of interest using the \texttt{TaylorF2Ecc} template over these parameter grids to optimally cover the intrinsic parameter space:
\begin{align}\label{eq:parameters}
    M_z\in&\{10^{4.5},10^5,10^{5.5},10^6,10^{6.5},10^{7},10^{7.5}\}~\MSun,\\
    q\in&\{1.2,2.0,4.0,8.0\},\nonumber\\
    e_{1{\rm yr}}\in&\{10^{-3.5},10^{-3.25},10^{-3},10^{-2.75},10^{-2.5},10^{-2},\nonumber\\
    &10^{-1.5},0.1\}\nonumber.
\end{align}
...
In this work, we only work with Newtonian amplitudes and study binaries until they reach their innermost stable circular orbit (ISCO), i.e. at binary separation $r_{\rm ISCO}\equiv 3r_{\rm s}$, where $r_{\rm s} \equiv2GM_z/c^2$ is the Schwarzschild radius of the total mass BH,\footnote{$G$ is the gravitational constant and $c$ is the speed of light in vacuum.} at which point binaries are expected to circularize.\footnote{We perform circularization test of \texttt{TaylorF2Ecc} in Appendix~\ref{AppA}.} We find the starting frequency ($f_{1{\rm yr}}$) such that the system reaches the ISCO at frequency $f_{\rm ISCO}$ in exactly one year by using the Peters' time-scale \citep{Peters1964}. 

In Fig.~\ref{fig:waveforms}, we show the characteristic strain $h_{\rm c}(f)\equiv 2f\tilde{h}(f)$, a visual aid to represent how signal adds up in the detector, the LISA noise curve $S_n(f)$ including a confusion noise due to galactic binaries taken from \citet{Marsat2021}, and the accumulated phase for an $M_z=10^5~\MSun$, $q=8.0$, and $e_{1{\rm yr}}=0.1$ system at $z=1$ for \texttt{TaylorF2Ecc} and the quasicircular inspiral-merger-ringdown waveform model \texttt{IMRPhenomD} \citep{Husa2016,Khan2016}. Since the inspiral part of the \texttt{IMRPhenomD} comes from the \texttt{TaylorF2} template, phasings are almost identical until the system is close to the ISCO. The initial phase difference is due to non-zero eccentricity in \texttt{TaylorF2Ecc}, and later deviations come from the merger part of the \texttt{IMRPhenomD}, which are beyond the scope of the inspiral-only model \texttt{TaylorF2Ecc}.

% Figure environment removed

To account for LISA motion and to project the waveform into TDI channels, namely A, E, and T, we modify the \texttt{lisabeta} software (see \citealt{Marsat2021} for details and subsequent notations) by including support for \texttt{TaylorF2Ecc}. Therefore, a waveform $\tilde{h}(f)$ will have strain projections $\tilde{h}_{\rm A,E,T}(f)$ and noise power spectral densities $S_n^{\rm A,E,T}$ corresponding to A, E, and T channels, respectively.
...
One can always scale these errors ($\sim1/$SNR) at higher redshift by using SNR values in Fig.~\ref{fig:SNR}. The error estimates of the parameters from the Fisher matrix procedure are simplistic but its quite useful in understanding the upper-limit (Cram\'{e}r-Rao Bound). In the next section, we perform Bayesian inference to find error estimates on eccentricity recovery and the minimum measurable eccentricity.
...
AD, MG, and LM acknowledge support from the Swiss National Science Foundation (SNSF) under the grant 200020\_192092. ST is supported by the SNSF Ambizione Grant Number: PZ00P2-202204. We acknowledge Stanislav Babak, Pedro R. Capelo, and Jonathan Gair for insightful discussions. The authors also acknowledge use of the Mathematica software \citep{Mathematica}, NumPy  \citep{harris2020array}, and inspiration drawn from the \texttt{GWFAST} package \citep{Iacovelli2022} regarding the python implementation of \texttt{TaylorF2Ecc}.  
