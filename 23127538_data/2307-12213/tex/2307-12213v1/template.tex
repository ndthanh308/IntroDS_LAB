...
\authorfooter{
  ...
  \item Yuchen Wu, Yuansong Xu, Shenghan Gao, and Quan Li are with the School of Information Science and Technology, ShanghaiTech University, and Shanghai Engineering Research Center of Intelligent Vision and Imaging, China. Wenkai Song and Xiaomeng Fan are with the School of Entrepreneurship and Management, ShanghaiTech University. Quan Li is the corresponding author. E-mail: \{wuych3,xuys1,gaoshh1,liquan,songwk,fanxm\}@shanghaitech.edu.cn.
  \item Xingbo Wang is with Weill Cornell Medical College, Cornell University. E-mail: xingbo.wang@\{med.cornell.edu, connect.ust.hk\}.
  \item Zhiheng Nie is with Be Friends Holding Limited. E-mail: 815194445@qq.com.
}
...
\teaser{
  ...
  % Figure removed
  \vspace{-6mm}
  \caption{In Case I, the strategic retrospect for streamers involves several steps. \protect\casebox{1} The session with the highest transaction amount per thousand views (GPM) is selected. \protect\casebox{2} poor sales performance of the ``Marker Pen'' is identified in the \protect\teasercircled{A} \textit{Session View}. \protect\casebox{3} 
  It is discovered that the \textit{atmosphere pitch} dominates the \textit{Text} channel during that period in the \protect\teasercircled{B} \textit{Segment View}. \protect\casebox{4} A low \textit{GPM} value is observed in the \protect\teasercircled{C} \textit{Exploration View}. \protect\casebox{5} Negative contributions from the \textit{Text} and \textit{Frame} channels in the merchandise-level summary are observed. \protect\casebox{6} The \textit{Performance Inspection} reveals a scarcity of facial expressions and a high number of \textit{close-up}s. \protect\casebox{7} The live replay indicates that the streamer was drawing a small rabbit on paper. \protect\casebox{8} A peak in the \textit{Like} value is observed during that time period. \protect\casebox{9} 
  The positive contribution of the \textit{atmosphere pitch} to the target is discovered, along with \protect\casebox{10} low \textit{Departure} and \protect\casebox{11} negative contribution. \protect\casebox{12} Abundance of semantically similar comments is found in the \textit{Comment Reference} part. \protect\casebox{13} The conclusion is drawn that streamers should maintain a balance in the utilization of various sales pitches, which is appended as \textit{Drawbacks} in the \protect\teasercircled{D} \textit{Record View}. \protect\casebox{14} A period is identified where the \textit{GPM} value initially decreased but quickly rebounded. \protect\casebox{15} This period involves a transition from the \textit{atmosphere pitch} to the \textit{selling pitch} and \textit{order pitch}. \protect\casebox{16} The review of video content reveals the streamer providing historical tidbits, and \protect\casebox{17} all three sales pitches are found to have a positive impact. \protect\casebox{18} The combination of these three sales pitches is summarized as a wise choice and added as \textit{Highlights} to the \textit{Record View}.}
  \label{fig:teaser}
}
...
\par The \textit{Exploration View} consists of three parts: \textit{Performance Inspection}, \textit{Correlation Modeling}, and \textit{Comment Reference} (\textbf{R1}-\textbf{R4}, \textbf{R6}-\textbf{R8}), arranged in vertical alignment with a synchronized cursor for concurrent analysis (\dpbox{\textcolor{white}{\small{\texttt{DP2}}}}).
