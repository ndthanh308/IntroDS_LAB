...
\author{Piotr B. Mucha, \v{S}\'{a}rka Ne\v{c}asov\'{a}, Maja Szlenk}
\date{}
...
\begin{abstract}
   ...
   It is shown that there exists a global weak solution, satisfying the $L^\infty$ bounds for all the components. Moreover, despite the lack of the regularity on the gradients, we obtain strong compactness of densities in $L^p$ spaces. The applied method captures the properties of the models of high generality, which admit an arbitrary number of components. Furthermore, the framework we develop can handle models that contain both diffusing and non-diffusing elements.
\end{abstract}
...
In the paper we analyze the system of $N$ compressible, reacting fluids with densities $$\varrho_1, \dots, \varrho_N\colon \Omega \; \subset \; \mathbb{R}^d \to \mathbb{R},$$ moving along a common velocity vector $u\colon\Omega\to \mathbb{R}^d$.  We assume the slow motion,
...
%
The interactions between the particular components are described by the production rates $\omega_1,\dots,\omega_N\colon\mathbb{R}^N_+\to\mathbb{R}$, $\omega_1,\dots,\omega_N\in C^1(\overline{\mathbb{R}_+^N})$ and dissipation is prescribed by fluxes $F_i$. 
...
\begin{equation}\label{slip}
    ...
    n\cdot \mathbb{T}(u,p)\cdot\tau_k + fu\cdot\tau_k &= 0, \quad k=1,\dots,d-1, & \mbox{at \ } \partial \Omega
    \times (0,T),\\
    \quad F_i\cdot n & = 0, \quad i=1,...,N,& \mbox{at \ } \partial \Omega \times (0,T),
    \end{aligned}
\end{equation}
where $n$ and $\tau_k$ are the normal and tangent vectors respectively, $f$ describes the friction and the stress tensor $\mathbb{T}$ is given by
...
The kernel of the system is the constitutive relation giving us the form of the pressure. We assume the total pressure $p$ is in the form
\[ p(\vec\varrho\,)=\sum_{i=1}^N p_i(\varrho_i), \mbox{ \ with \ }
p_i(\varrho_i\,)=\frac{1}{m_i}\varrho_i^{\gamma_i}, \] 
where $m_i$ are molar masses of the components and $\gamma_i>1$. 
%
The diffusion fluxes are depending on the pressure. Following \cite{Gi}, we assume that the fluxes have the form 
\begin{equation}\label{flux}
 F_i = \nabla p_i -\frac{\varrho_i}{\varrho}\nabla p. 
 \end{equation}
A such form guarantees the fundamental feature of fluxes, namely
$\sum_{i=1}^N F_i=0$, and also non-negativity of densities $\rho_i$.
%
...
we  focus on {\it the irreversible reactions} in the form
...
Moreover, the latter is satisfied if $\sum_{i=1}^N\omega_i(\vec\varrho\,)=0$. This, together with the fact that $\sum_{i=1}^N F_i=0$, ensures us that the sum of the densities $\varrho=\sum_{i=1}^N\varrho_i$ satisfies the continuity equation
...
Let $\vec\varrho_0\in L^\infty(\Omega)$. In addition, let us assume that all $\gamma_i=2$.
Then there exist
\[ \varrho_1,\dots,\varrho_N \in L^\infty([0,\infty)\times\Omega), \]
...
\begin{equation}\label{F_bar}
   ...
   -\mu\Delta u -\nabla((\mu+\lambda)\ddiv u) + \nabla p(\vec\varrho\,) &= 0
    \end{aligned}
\end{equation}
in $(0,\infty)\times\Omega$ in the sense of distributions with the boundary conditions (\ref{slip}). Moreover, assuming  $\varrho_i\in L^2(0,T;H^1)$, $i=1,\dots,N$, it {gives} us that the fluxes $\bar{F}_i$ satisfy the relation
\begin{equation}\label{fl}
\frac{1}{2}\sum_{i=1}^N\frac{1}{\varrho_i}|\bar{F}_i|^2 \leq \sum_{i=1}^N\bar{F}_i\cdot\frac{1}{m_i}\nabla\varrho_i \quad \text{a. e. in} \quad (0,\infty)\times\Omega. \end{equation}
\end{tw}

The inequality (\ref{fl}) can be also rewritten in the following way: if $F_i=\nabla p_i-\frac{\varrho_i}{\varrho}\nabla p$, then (\ref{fl}) is equivalent to
\[ \sum_{i=1}^N\frac{1}{\varrho_i}\bar{F}_i\cdot(\bar{F}_i-F_i) \leq 0. \]
...
It opens rather a lot of questions, then it delivers answers. The first question concerns the meaning of obtained fluxes $\bar F_i$. The ground regularity of solutions is too weak to determine $\bar F_i$ in terms of $\rho_i$. 
...
A hint to resolve this problem seems to be in (\ref{fl}), but on the other hand it looks that it is not enough. Note that in the literature we find some positive answers to similar questions, but for slightly or significantly different systems. In \cite{Za_1,zatorska2013}
the author were able to control the fluxes $\bar F_i$ in terms of gradients of species, but in that framework 
...
obtains the solutions to an approximate system to (\ref{diff_react}). Here the exponents $\gamma_i$ can already be different from $2$:
...
Note that the above result delivers well defined objects. Gradients of species are in $L^2$, so $F_i$ are well defined in terms of formula (\ref{fl}). The proof of Theorem \ref{appr_th} is based on the classical techniques. First, we consider a suitable regularization, which makes our fluxes less degenerate, and densities better integrable. Then starting from the Galerkin methods we prove existence of each step of approximation scheme, obtaining the solutions to (\ref{approx}) finally. As Theorem \ref{appr_th}, one can say, belongs to the classical theory of weak solution to compressible flows, the step to Theorem \ref{th:1.1} seems to be not. We need to take $\varepsilon \to 0$, and then determine the limits in the equation. What is unexpected, is the strong convergence of all densities. We prove
\begin{equation}\label{bb4}
    \varrho^{(\varepsilon)}_i \to \rho_i \mbox{ \ \ strongly in \ \ }
...
The main difficulty in Theorem \ref{th:1.1} is a possible lack of strict positivity of the particular densities. It turns out that even though we still cannot control the gradients of the densities, assuming that all components are bounded away from zero we can rewrite the fluxes $F_i$ in terms of new variables, which are already bounded in $L^2(0,T;H^1)$. We summarize this result below:
...
\begin{itemize}
    \item In Sections 2--5, we consider the case when diffusion occurs for all the components and the most general diffusive -- non-diffusive case is shortly commented in Section 6.
    ...
    The next step towards the proofs of Theorems \ref{th:1.1} and \ref{positive_th} is the limit passage with $\varepsilon\to 0$. This final step is done Sections 4 and 5 under the assumptions of Theorem \ref{th:1.1} and \ref{positive_th} respectively.

    \item Section 4 contains the end of the proof of Theorem \ref{th:1.1}. Due to the presence of the additional terms in the continuity equations, the standard Lions--Feireisl approach \cite{lions, feireisl} is not well-suited for this framework. Instead, we are able to adapt the method of Bresch and Jabin \cite{bresch-jabin,BJ2}, based on the Kolmogorov compatness criterion, which allows us to show the strong convergence of the densities. However, due to the limitations of this method we need to restrict to the case $\gamma_1=\dots,\gamma_N=2$.
    \item In Section 5 we perform the final limit passage in the framework of Theorem \ref{positive_th}, namely assuming that the sequence of approximate solutions is strictly positive. In this case we again do not need any additional assumptions on the exponents $\gamma_i$ besides (\ref{gammas}).
...
j_i^{\rm mol}= - D_i \nabla c_i.
\end{equation}
It was realized that such a relation is less realistic and it leads to introduce the general setting of the Theory of irreversible processes. We can refer to De Groot and Mazur 1984, where the deficiencies of Fick's law are described together with a new law - the Fick-Onsager form of diffusion fluxes. The diffusion coefficients $D_i$ are replaced by a matrix phenomenological coefficients $L_{ij}$ called Osanger coefficients for diffusion. Another approach is the Maxwell-Stefan equations - in the so-called diffusive approximation, which ignore the acceleration of the relative motion, see Dreyer, Druet, Gajewski and  Guhlke (2020) or Standart, Taylor and Krishna (1975). For more details concerning modelling, we refer to paper by Bothe, Druet \cite{BD}.

Maxwell - Stefan type models  were studied from the engineering point of view but there are only a few results concerning the mathematical theory. Recently such problems attract a lot of attention, see works of \cite{B,BGS,JS}. The mass-based Maxwell-Stefan approach to one-phase multicomponent reactive mixture is analyzed \cite{HMPW}. It is shown the local well-posedness in $Lp$ setting and generate a local semiflow on its natural state space. A self-contained introduction to this approach can be find in \cite{J}.
Analysis of cross-diffusion systems for fluid mixtures driven by a pressure gradient  was analysis for weak and strong solution by Druet, Jungel \cite{DJ}.

Many mathematical results have been proved in direction of multicomponent diffusion systems where the component share the same velocity and the Maxwell-Stefan system is coupled with the Navier-Stokes equations.
The model for the compressible chemically reacting mixtures  developed by Giovangigli in \cite{Gi} in the context of the Fick law were considered in \cite{FPT}; for non-diagonal mobility matrix in \cite{Za_1}. 
    The paper \cite{MPZ_SIMA} includes the case when the mean viscosity depends on the total density and fulfils the Bresch - Desjardin relation.   The steady problems (with non-diagonal mobility matrix) were studied in \cite{Zatorska_2011,GPZ,PiPo1}. More complex situation is considered in \cite{DDGG}. Strong solution of such type of model was investigated by Piasecki, Shibata, Zatorska \cite{PSZ1,PSZ2}.
...
The above results in principle concern the situation, where the chemical reaction is reversible. Concerning the irreversible reaction, from the mathematical point of view not much is known. A fast irreversible reaction of type (\ref{abc}) was investigated by Bothe \& Pierre in the case of reaction-diffusion system \cite{BP}, where it appears between species $A$ and $B$ with similar concentrations. Purely numerical and experimental results, involving irreversible reactions, were obtained for example in \cite{bothe-kroeger-warnecke,bubbles1,bubbles2}. 

\section{A priori estimates}\label{a_priori_sect}

Now we will proceed with a priori estimates. 
...
{\bf Proof.} The proof is splited into four steps.
\smallskip
...
We will now show that $ \varrho_i\geq 0$  for all $ i=1,\dots,N.$
% \begin{lem}
...
\[ -\frac{\dd}{\dd t}\int_{\Omega}\varrho_i^-\dd x \geq 0, \]
...
%  \end{lem}
 ...
 {\bf Step II.} In this part we construct the basic energy estimate, in terms of the point-wise bound on the density and the production rates $\omega_i$'s. The goal is to show that
%  \begin{lem}
%  A sufficiently smooth solution to (\ref{diff_react}) satisfies 
 \begin{multline}\label{energy} \int_0^T\int_\Omega 
\mu|\nabla u|^2 + (\lambda+\mu) (\ddiv u)^2\;\dd x\dd t + \sup_{t\in [0,T]}\sum_{i=1}^N\frac{1}{\gamma_i-1}\int_\Omega p_i(\varrho_i)\dd x + \sum_{i=1}^N \int_0^T\int_\Omega \frac{1}{\varrho_i}|F_i|^2 \dd x\dd t \\
\leq \sum_{i=1}^N\frac{1}{\gamma_i-1}\int_\Omega p_i(\varrho_{0,i})\dd x + C\|\varrho\|_{L^\infty([0,T]\times\Omega)}^{\gamma_3-1}. \end{multline}
%  \end{lem}
%  \begin{proof}
Note that in the general case (\ref{irreversible}), instead of $\gamma_3$ we need to put $\gamma_S$, which is the maximum of exponents corresponding to $C_1,\dots,C_L$.

 Testing the momentum equation by $u$, we get
 \[ \mu\int_\Omega |\nabla u|^2\dd x + (\lambda+\mu)\int_\Omega (\ddiv u)^2\dd x + \int_\Omega \nabla p\cdot u =0. \]
 To obtain the desired estimate, we need to deal with the last term. We have
 \[ \int_\Omega \nabla p\cdot u \dd x  =  \frac{\dd}{\dd t}\sum_{i=1}^N\frac{1}{\gamma_i-1}\frac{1}{m_i}\int_\Omega \varrho_i^{\gamma_i} \;\dd x +2\sum_{i=1}^N\frac{\gamma_i}{\gamma_i-1}\frac{1}{m_i}\int_\Omega\nabla\varrho_i^{\gamma_i-1}\cdot F_i\;\dd x -2\sum_{i=1}^N\frac{\gamma_i}{m_i}\int_\Omega \varrho_i^{\gamma_i-1}\omega_i \;\dd x.  \]
...
\end{aligned} \end{equation}
 ...
 \begin{multline*}
 \mu\int_\Omega |\nabla u|^2 \;\dd x + (\lambda+\mu)\int_\Omega (\ddiv u)^2 \;\dd x + \frac{\dd}{\dd t}\sum_{i=1}^N\frac{1}{\gamma_i-1}\int_\Omega p_i(\varrho_i)\;\dd x \\
...
\begin{array}{ll}
    ...
    \frac{(\alpha \cdot n)}{\partial n}=-(\alpha\cdot \tau_1)_{\tau_1} - (\alpha\cdot \tau_2)_{\tau_2}
...
\begin{equation}\label{p_rho} \frac{1}{Nm_{\max}}\varrho^{\gamma_{\min}}\leq p(\vec\varrho\,) \leq \varrho^{\gamma_{\max}}\sum_{i=1}^N\frac{1}{m_i}, \end{equation}
where $\gamma_{\min}$ and $\gamma_{\max}$ are respectively the minimum and maximum of $\{\gamma_1,\dots,\gamma_N\}$. In consequence, since by the interpolation of $L^p$ spaces
\[ \int_\Omega |p(\vec\varrho\,)|^p\dd x \leq \|p(\vec\varrho\,)\|_{L^\infty(\Omega)}^{p-1}\int_\Omega p(\vec\varrho\,)\dd x \leq C\|\varrho\|_{L^\infty(\Omega)}^{\gamma_{\max}(p-1)}(1+\|\varrho\|_{L^\infty([0,T]\times\Omega)}^{\gamma_3-1}), \]
we get that 
...
&\geq C\varrho(t,x)^{\gamma_{\min}} - C\|\varrho\|_{L^\infty((0,T)\times\Omega)}^{\gamma_3-1}-\|d\|_{L^\infty((0,T)\times\Omega)}^{\tilde\gamma} \\
&\geq C\left(\left(\|\varrho\|_{L^\infty((0,T)\times\Omega)}-1\right)^{\gamma_{\min}} - \|\varrho\|_{L^\infty((0,T)\times\Omega)}^{\tilde\gamma}-\|\varrho\|_{L^\infty((0,T)\times\Omega)}^{\gamma_3-1}\right). 
\end{aligned}\]
As $\gamma_{\min}>\max(\tilde\gamma,\gamma_3-1)$, the function $ z\mapsto (z-1)^{\gamma_{\min}}-z^{\tilde\gamma}-z^{\gamma_3-1}$
...
\[\begin{aligned} \frac{1}{2}\int_\Omega (\varrho-k)_+^2(t,x)\dd x - \frac{1}{2}\int_\Omega (\varrho_0-k)_+^2\dd x &=  \int_0^t\int_\Omega \varrho u\cdot\nabla(\varrho-k)_+\dd x\dd s \\
...
\section{Existence of an approximated system}\label{existence_sect}
...
% \end{tw}
%
Note that from the Neumann condition on $\varrho_i$ it also follows that $F_{i}\cdot n=0$ for $i=1,\dots,N$.

\smallskip 

\paragraph{Proof of Theorem \ref{appr_th}:} Let us introduce another level of approximation, depending on a small parameter $\delta$. We improve the integrability of the densities, by adding the terms $\delta\tilde\varrho^{\beta-2}\varrho_i$ for
$\displaystyle\tilde\varrho = \sum_{i=1}^N |\varrho_i|$ and $\beta$ sufficiently large. We also introduce the truncation of $\varrho_i$, defined as 
...
%\end{equation}
\mbox{ \ where \ \ }
 \tilde \varrho^\delta = \sum_{k=1}^N \left|\varrho_k^\delta\right| \quad \text{and} \quad p^\delta = \sum_{k=1}^N p_k^\delta(\varrho_i). 
\end{equation}
...
We will obtain the solutions to (\ref{approx2}) using the Galerkin method. 
\begin{lem}
    The equation (\ref{approx2}) admits a global weak solution in $[0,T]\times\Omega$.
\end{lem}

\begin{proof} Let $\{w_k\}_{k\in\mathbb{N}}\in C^\infty(\Omega)$ and $\{v_k\}_{k\in\mathbb{N}}\in (C^\infty(\Omega))^d$ be the suitable orthogonal bases of $H^1(\Omega)$ and $(H^1(\Omega))^d$ respectively. Define
...
\[\begin{aligned} \mu\int_0^T\int_\Omega |\nabla u_n|^2\dd x\dd t + \sup_{t\in [0,T]}\sum_{i=1}^N\frac{1}{m_i}\int_\Omega\varrho_{i,n}^2\dd x \\
...
\end{aligned}\]
As $\omega_i^\delta\in L^\infty((0,T)\times\Omega)$, from the Cauchy inequality we have
\[ \int_0^T\int_\Omega \varrho_{i,n}\omega_i^\delta\dd x \leq \eta\|\varrho_{i,n}\|_{L^\infty(0,T;L^2)}^2 + \frac{CT}{\eta}\|\omega_i^\delta\|_{L^\infty((0,T)\times\Omega)}^2. \]
Therefore choosing $\eta$ sufficiently small, we get the estimates

\begin{multline}\label{ap_energy} \|\nabla u_n\|_{L^2((0,T)\times\Omega)}^2 + \sum_{i=1}^N\left(\|\varrho_{i,n}\|_{L^\infty(0,T;L^2)}^2 + \varepsilon\|\nabla\varrho_{i,n}\|_{L^2((0,T)\times\Omega)}^2+\delta\|\varrho_{i,n}\|_{L^\beta((0,T)\times\Omega)}^\beta\right) \\
...
\[ \|\nabla p_n\|_{L^{\beta/2}(0,T;W^{-1,\beta/2})}+\|u_n\|_{L^{\beta/2}(0,T;W^{1,\beta/2})} \leq C. \]
In particular, if $\beta$ is large enough, then
...
Note that the low regularity of $\varrho_i$ does not allow us to replicate the argument from Section \ref{a_priori_sect}. Instead, we will obtain the result by choosing a suitable test function. Let $\varrho_i = \varrho_i^+-\varrho_i^-$, where $\varrho_i^+,\varrho_i^-\geq 0$ denote the positive and negative part of $\varrho_i$. We test the equation on $\varrho_i$ by the function $\psi_i = (l+\varrho_i^-)^\alpha$ for $l,\alpha>0$ small. Since $\varrho_i$ is in $L^2(0,T;W^{1,2})$, $\psi_i\in L^2(0,T;W^{1,2})$ as well. First, observe that if $\varphi_\eta(t,x)$ is the standard mollifier over time and space, then for $(\varrho_i^-)_\eta = \varrho_i^-\ast\varphi_\eta$ we have $(l+(\varrho_i^-)_\eta)^\alpha\to (l+\varrho_i^-)^\alpha$ in $L^2(0,T;H^1)$ and $\partial_t(\varrho_i^-)_\eta \rightharpoonup^* \partial_t\varrho_i^-$ in $L^2(0,T;H^{-1})$. Thus
...
&-\alpha\int_0^t\int_\Omega (l+\alpha_i^-)^{\alpha-1}\frac{\min(\varrho_i^-,\frac{1}{\delta})}{\tilde\varrho^\delta}\nabla p^\delta\cdot\nabla\varrho_i^-\dd x\dd s.
\end{aligned}\]
...
% + l^\alpha\int_\Omega \varrho_i^+(t,\cdot)\dd x - l^\alpha\int_\Omega \varrho_{0,i,\varepsilon}\dd x \\
% -\varepsilon\alpha\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}|\nabla\varrho_i^-|^2\dd x\dd s \\
% - \alpha\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}\tilde\varrho_i^Mu\cdot\nabla\varrho_i^-\dd x\dd s \\
% -\frac{\alpha}{m_i}\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}\tilde\varrho_i^M|\nabla\varrho_i^-|^2\dd x\dd s \\
% -\alpha\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}\frac{\min(\varrho_i^-,M)}{\tilde\varrho^M}\nabla p^M\cdot\nabla\varrho_i^-\dd x\dd s &= \int_0^t\int_\Omega \omega_i^M(\vec\varrho)(l+\varrho_i^-)^\alpha\dd x\dd s.
% \end{aligned}\]
...
% \frac{-1}{\alpha+1}\int_\Omega (l+\varrho_i^-(t,\cdot))^{\alpha+1}\dd x + \frac{l^{\alpha+1}|\Omega|}{\alpha+1} \\
% + l^\alpha\int_\Omega \varrho_i^+(t,\cdot)\dd x - l^\alpha\int_\Omega \varrho_{0,i,\varepsilon}\dd x \\
% - \alpha\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}\min(\varrho_i^-,M)u\cdot\nabla\varrho_i^-\dd x\dd s \\
% -\alpha\int_0^t\int_\Omega (l+\varrho_i^-)^{\alpha-1}\frac{\min(\varrho_i^-,M)}{\tilde\varrho^M}\nabla p^M\cdot\nabla\varrho_i^-\dd x\dd s \geq & \int_0^t\int_\Omega \omega_i^M(l-\varrho_i)^\alpha \mathbbm{1}_{\varrho_i\leq 0} \;\dd x\dd s \\
% &+ l^\alpha\int_0^t\int_\Omega \omega_i^M\mathbbm{1}_{\varrho_i\geq 0} \dd x\dd s.
% \end{aligned}\]

First, we want to pass to the limit with $l\to 0$. For small $\alpha$ we have
\[ (l+\varrho_i^-)^{\alpha-1}\varrho_i^- = \frac{\varrho_i^-}{l+\varrho_i^-}(\varrho_i^-)^\alpha \leq \varrho_i^- \]
and
...
First, let us show that $\varrho\in L^\infty((0,T)\times\Omega)$. Recall the following result:

\begin{prop}\label{parabolic_th}
Let $h\in L^p(0,T;L^q)$ for $1<p,q<\infty$. Then the solution to
\[ \partial_t\varrho -\varepsilon\Delta\varrho = h, \quad \varrho_{|_{t=0}}=\varrho_0 \]
...
Note that if $\beta$ is large enough, then in particular $\varrho u\in L^p(0,T;L^q)$ for some suitable $p\geq 2$ and $q>3$. Therefore from Proposition \ref{parabolic_th}, we have
\[ \varrho \in L^\infty(0,T;L^q). \]
Then, using the property (\ref{p_rho})
from the elliptic estimates we have $p\in L^\infty(0,T;L^{q/2})$ and thus
we get that
\[ u\in L^\infty(0,T;W^{1,q/2}). \]
In consequence, 
...
We will now prove that in fact the $L^\infty$ bound on $\varrho$ depends only on $\|\varrho_0\|_{L^\infty(\Omega)}$, using the reasoning analogous as in Section \ref{a_priori_sect}. Similarly as for the a priori estimates, we have
...
=& -\frac{1}{m_i}\iint_{\Omega\times\Omega} F_i^x\cdot\nabla \overline{K}_h(x-y)(\varrho_i^x-\varrho_i^y)\dd x\dd y -\frac{1}{m_i}\iint_{\Omega\times\Omega}
\overline{K}_h(x-y)F_i^x\cdot\nabla\varrho_i^x\dd x\dd y \\
&- \frac{1}{m_i}\iint_{\Omega\times\Omega}F_i^y\cdot\nabla \overline{K}_h(x-y)(\varrho_i^x-\varrho_i^y)\dd x\dd y -\frac{1}{m_i}\iint_{\Omega\times\Omega} \overline{K}_h(x-y)F_i^y\cdot\nabla\varrho_i^y\dd x\dd y \\
...
\[ \int_{\mathbb{R}^d} f(x)g(x)\dd x\leq C\|f\|_{BMO}\|g\|_{L^1}\big(|\log\|g\|_{L^1}+\log(1+\|g\|_{L^\infty})\big) \]
...
However, it is now easy to see that $\displaystyle \sup_{t\in [0,T]}z_\varepsilon(t)\to 0$ as $\varepsilon\to 0$. Indeed, as $z_\varepsilon(t)\geq z_\varepsilon(0)$, we get $|\ln z_\varepsilon(t)|\leq |\ln z_\varepsilon(0)|$. Then from Gronwall's lemma
...
for some functions $\bar{F}_i\in L^2((0,T)\times\Omega)$. Moreover, for any $\varphi\in C_0^\infty((0,T)\times\Omega)$,
...
In the end, let us show that if the solution to (\ref{F_bar}) has higher regularity, then it satisfies the relation (\ref{fl}). At the level of approximated equation (\ref{approx}), testing the continuity by $\frac{1}{m_i}\varrho_{i,\varepsilon}\varphi$ for some smooth, positive $\varphi\in C_0^\infty((0,T)\times\Omega)$ yields
\[\begin{aligned}
...
\end{aligned}\end{equation}
Using the strong convergence of $\varrho_{i,\varepsilon}$, weak convergences of $F_{i,\varepsilon}, u_\varepsilon$ and weakly lower semicontinuity of convex functions, after passing to the limit in (\ref{suma}) we get
...
&+ \sum_{i=1}^N\int_0^T\int_\Omega \frac{1}{\varrho_i}|\bar{F}_i|^2\varphi \;\dd x\dd t + \sum_{i=1}^N\frac{1}{m_i}\int_0^T\int_\Omega \varrho_i\bar F_i\cdot\nabla\varphi \;\dd x\dd t \leq \sum_{i=1}^N\frac{1}{m_i}\int_0^T\int_\Omega \varrho_i\omega_i\varphi \;\dd x\dd t.  \end{aligned}\end{equation}
...
After substracting (\ref{suma2}) from (\ref{suma1}), we get 
\[ \int_0^T\int_\Omega \sum_{i=1}^N\left(\frac{1}{2}\frac{1}{\varrho_i}|\bar{F}_i|^2-\bar{F}_i\cdot\frac{1}{m_i}\nabla\varrho_i \;\dd x\dd t\right)\varphi \;\dd x\dd t \leq 0 \]
...
\section{End of the proof of Theorem \ref{positive_th}} 
...
In consequence, the matrix $B$ is invertible whenever $\varrho_1\dots\varrho_N\neq 0$ and if $\varrho_1\dots\varrho_N\geq c$ for some $c>0$, then 
...
We will use the above information to first show the compactness of the total density again via Bresch \& Jabin method (note that in this case $(\partial_t\varrho_{i,\varepsilon})$ is also bounded in $L^2(0,T;W^{-1,p})$ for each $i$). Let $K_h$ be as in Section 4 and define
\[ \mathcal{R}_h(t) = \iint_{\Omega\times\Omega}\overline{K}_h(x-y)|\varrho(t,x)-\varrho(t,y)|^2\dd x\dd y. \]
By analogous calculations as in the previous section, we have
...
and
\[ A_2 \leq C\iint_{\Omega\times\Omega} \overline{K}_h(x-y)(|d^x-d^y|+|p^x-p^y|)|\varrho^x-\varrho^y|\dd x\dd y. \]
Using (\ref{p_lip}) and Young inequality, we can further estimate $A_2$ by
\[ A_2 \leq C\iint_{\Omega\times\Omega}\overline{K}_h(x-y)\left(|d^x-d^y|^2+\sum_{j=1}^{N-1}|q_j^x-q_j^y|^2\right)\dd x\dd y + C\mathcal{R}_h(t). \]
We have
\[\begin{aligned} \iint_{\Omega\times\Omega} \overline{K}_h(x-y)|d^x-d^y|^2\dd x\dd y \leq \frac{\|\nabla d\|_{L^2}^2}{|\log h|}\int_{\mathbb{R}^3} K_h(z)|z|^2\dd z
\end{aligned} \]
and similarly
...
\end{aligned} \]
Then, since the integral $\int_{\mathbb{R}^3} K_h(z)|z|^2\dd z$ is bounded independently of $h$, we get
...
and after applying the logarithmic inequality from \cite{mucha-rusin} and integrating over time, we end up with
...
\end{aligned}\]
Then, proceeding in the same way as in Section 4, we arrive at
\[ \limsup_{k\to\infty}\sup_t R_h(t) \to 0 \quad \text{as} \quad h\to 0. \]
