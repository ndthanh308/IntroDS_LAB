...
\begin{proof}
    As $\bZ$ contains no descendents of $X$, it cannot block the directed path between $X$ and $Y$ and hence the Causal Markov Condition does not declare $X$ and $Y$ independent. Faithfulness stipulates that $X$ and $Y$ are therefore dependent given $\bz$, so there exist $x,x'$ such that
    \[
