...
The contributions proportional to $N_f^2 N_{f,V}$ vanish by Furry's theorem. 
...
\begin{align}
    L_{f_1 f_2}^\gamma &= - e_{f_1} \delta_{f_1 f_2}\,,\\
...
Once the helicity projectors have been applied, all Feynman diagrams are expressed in terms of scalar integrals, which can be written in terms of a single planar auxilliary topology of the form
\begin{equation}
...
directly in terms of the canonical basis for the three-loop planar topology defined in~\cite{Canko:2023yoe}.
...
Therefore, 
 ...
 we also devised a hybrid method involving finite field reconstruction, in parallel to a standard fully analytic approach. In particular, the IBP reduction was obtained with the aid of \code{Kira2}'s internal finite-field implementation. Next we performed an insertion of the reduction and MI solutions for a handful of rational numerical values of the variables $y$ and $z$ in the coefficients of the MPLs. Having established which polylogarithms appear in the final expression for a helicity coefficient, we reconstructed the functions of $y$ and $z$ in the rational prefactors of the relevant polylogarithms leveraging the implementation provided by \code{FiniteFlow}~\cite{Peraro:2019svx}. This approach lends itself to parallelization and gives us access to the full bare amplitude in various poles schemes and greater versatility in recycling the symbolic reductions in future calculations. We verified that the reconstructed results exactly matched those obtained through a symbolic insertion.