...
% This usually does not hold for the right and left eigenvectors of  $\K$.
 ...
 Another easy implication is that $(\B \V )^* =\V \B^*$, so $\B \V$ is a
 symmetric matrix that also follows by~\eqref{B}. Indeed,
 $$
 \B \V =\begin{pmatrix}
 \B_{12}  & \B_{11} \\
 \B_{22}  & \B_{21} 
...
Consequently, $\B \V$  is diagonalizable in an ortogonal basis, and so,
the singular vaues of $\B$ are the absolute values of its real eigevalues,
...
for $a=1,\dots ,k$.
The trivial fixed point $\0$ is an unstable fixed point of the above
system if there is at least one eigenvalue of the matrix
...
they are the same as the eigenvalues of the symmetric matrix
