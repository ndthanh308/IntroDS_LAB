...
\title{Percolated stochastic block model via EM algorithm and
...
College, 405 Main St. Davidson NC 28035, USA. E-mail: dzhou@davidson.edu} 
  }
...
coming into existence depend on the cluster memberships of the endpoints.
%that are fixed during the expansion. 
...
This concept can be extended to non-symmetric matrices, like the
...
of a random graph coming from the $SBM_k$
...
This seems to contradict to the laws of large numbers, but in the percolated 
...
$$
 b_{i \leftarrow j, \, k \leftarrow l } = \delta_{jk } (1-\delta_{il} ) ,
$$
so $b_{i \leftarrow j, \, k \leftarrow l } =1$ if and only if for the quadruple
in the lower indices, $i \leftarrow j=k \leftarrow l$ holds, where $l\ne i$. 
...
because a walk starting at a node cannot turn around and return to it
...
non-backtracking random walk is forced to a leaf of the tree, 
from where it has nowhere to go. 
...
$m-n$ eigenvalues equal to $-1$, whereas its other eigenvalues are those of 
...
$$
b^*_{i \leftarrow j, \, k \leftarrow l } =
b_{k \leftarrow l, \, i \leftarrow j } = b_{j \leftarrow i, \, l \leftarrow k }
$$
for each directed pair of edges and every entry $b^*$ of the transpose 
...
adjoint of a matrix or a vector (in case of real entries, this is the usual
transposition).
...
$$
for the coordinates of the $2m$-dimensional vector
$\x \in \R^{E^{\rightarrow }}$ ($E^{\rightarrow }$ is the set of bidirected edges),
where for $e=\{ i\to j \}$ the reversely directed
edge is denoted $e^{-1}=\{ j\to i \}$.
In partitioned form (into two $m$-dimensional vectors),
...
Let $\M$ denote the $n\times m$ node--edge (0-1) incidence matrix of
the simple graph $G$ (except for trees, $n\le m$). Then $\M \M^*$ and
$\M^* \M$ are Gramians (covariance matrices), so they are positive
semidefinite, have the same rank and the same set of positive eigenvalues. 
$\M \M^* =\DD + \A$ is called signless Laplacian (whereas, the Laplacian
matrix is $\DD -\A$, see~\cite{Bolla13}). On the other hand,
$\M^* \M - 2 \I_m$ is the adjacency matrix of the line-graph of $G$
(because the valency of the edges is 2). Consequently, its positive
eigenvalues are those of $\DD +\A$ decreased by 2, and the others
are equal to -2, see~\cite{Haemers} for more explanation.
Therefore, it is not surprising that the relevant eigenvalues of $\B$,
which are also the eigenvalues of $\K$, can be related to $\DD$ and $\A$.
However, $\B$ is not a symmetric matrix, but considering the edges in both
directions makes it possible to perform non-backtracking random walks and
belief propagation along them.

\begin{proof}
Let us label the rows (and columns) of $\B$ with the edges, ordered 
lexicographically: the first $m$ rows correspond to the $i\to j$ edges 
$(1\le i<j \le n)$, 
 whereas the second $m$ ones to the inverse edges $j\to i$, in the same order.
...
matching $e\to f$ and $f\to e$ edges in both directions (here $e\to f$ is the
...
It is also obvious that the undirected $\{ i,j \}$ and $\{ k,j \}$ edges can
be joined at $j$ in four possible ways: 
$i \to j\to k$, $k \rightarrow j \leftarrow i$, 
$i \leftarrow j\rightarrow k$, and $k \to j\to i$. Only one of them
results in the true relation $\{ i\to j \} \to \{ j \to k\}$.
...
\end{equation}
 ...
 In~\cite{Krzakala}, the following relation between
  the eigenvectors corresponding to the leading eigenvalues of $\B$ (the same
  as those of $\K$) is proved. For any vector  $\x \in \R^{2m}$  define
 $$
  x_i^{out} := \sum_{j\sim i} x_{i\to j}  \quad \textrm{and}  \quad
  x_i^{in} := \sum_{j\sim i} x_{j\to i}, \quad i=1,\dots ,n.
$$
If we put these coordinates in the $n$-dimensional
(column) vectors $\x^{in}$ and $\x^{out} $,  then obviously,
...
\begin{equation}\label{rel}
  (\B \x)^{out}_i = (d_i -1) x^{in}_{i} \quad \textrm{and} \quad
  (\B \x)^{in}_{i} = \sum_{j \sim i}  x^{in}_j -x^{out}_i , \quad i=1,\dots, n, 
\end{equation}
...
Summarizing, with matrix notation:
$$
 \begin{pmatrix}
 (\B \x)^{out} \\
 (\B \x)^{in}
...
\end{pmatrix} .
$$
Furthermore, $ \x^{out} =\frac1{\mu} (\DD -\I_n ) \x^{in}$.
...
The parameters of the $SBM_k$ model are as follows.
...
$n\to\infty$. An edge between $i<j$ comes into existence with probability
$p_{ab}$ if $i\in V_a$ and $j\in V_b$. This will produce the upper-diagonal
...
$\bar \A$ (which is the inflated $\PP$) with respect to the the diagonal,
...
$\psi_{i \leftarrow j }$ to $i$, which is an estimate of $j$'s marginal
...
\psi_{i \leftarrow j}^a = C_a^{ij} r_a \prod_{l\sim j , \, l\ne i} 
 \sum_{b=1}^k \psi_{j\leftarrow l}^b \, p_{ab}  , \quad a=1,\dots ,k ,
\end{equation}
where $C_a^{ij}$ is a normalizing factor.
Here $\psi_{j\leftarrow l}^b$ is the conditional probability that node $l$
...
when belief propagation has a trivial fixed point 
$\psi_{i\leftarrow j}^a =\frac1{k}$, for $a=1,\dots ,k$. If it gets stuck there,
then belief propagation does no better than chance. It happens when this
...
$$
 \psi_{i \leftarrow j}^a := r_a + \ep_{i \leftarrow j}^a .
$$ 
If we substitute this in~\eqref{psik} and expand to first order in $\epv$
(vector of $2mk$ coordinates $\ep_{i \leftarrow j}^a$'s), the updated system of
equations is
\begin{equation}\label{hosszu}
\begin{aligned}
 \ep_{i \leftarrow j}^a &= \psi_{i \leftarrow j}^a  -  r_a = r_a \left\{ C_a^{ij}
 \prod_{l\sim j , \, l\ne i} \left[ \sum_{b=1}^k \psi_{j\leftarrow l}^b \, 
 p_{ab} \right] -1 \right\}  \\
 &= r_a \left\{ C_a^{ij} \prod_{l\sim j , \, l\ne i} \left[ \sum_{b=1}^k (r_b +
 \ep_{j\leftarrow l}^b ) \, p_{ab} \right] -1 \right\} \\
&= r_a \left\{ C_a^{ij} \prod_{l\sim j , \, l\ne i} \left[ \sum_{b=1}^k r_b p_{ab}   +\sum_{b=1}^k \ep_{j \leftarrow l}^b \, p_{ab} \right] -1 \right\} \\
&= r_a \left\{ C_a^{ij} \prod_{l\sim j , \, l\ne i} \left[ \frac{c_a}{n} 
  +\sum_{b=1}^k \ep_{j \leftarrow l}^b \, \frac{c_{ab}}{n} \right] -1 \right\} \\
 &= r_a \left\{ C_a^{ij} (\frac1{n} )^{s_j -1 } \left[ \sum_{b=1}^k 
 \sum_{l\sim j , \, l\ne i}
 \ep_{j\leftarrow l}^b \, c_{ab} \, c_a^{s_j -2} +c_a^{s_j -1} \right] -1 
  \right\} +O (\epv^2 ) ,
\end{aligned}
\end{equation}
...
the $(i \leftarrow j, \, a ) \, , \, ( k \leftarrow l ,\, b )$ entry of 
$\M$ is $\partial \ep_{i\leftarrow j}^a / \partial \ep_{k\leftarrow l}^b $.
It is not zero only for $i \leftarrow j$, $k \leftarrow l$ pairs with $k=j$
and $l\ne i$, i.e., when the corresponding entry of $\B$ is 1.
If it is not zero, then the partial derivative of $\ep_{i \leftarrow j}^a$ with
respect to $\ep_{j\leftarrow l}^b$ is $\frac{r_a}{c_a} c_{ab}$ (the second
...
for $a=1,\dots ,k$.
The trivial 0 solution of Equation~\eqref{lin} is unstable if there are
eigenvalues of
...
$\bar \A$, then  $\vvv_i =\sqrt{n}\RR^{\frac12} {\tilde \uuu}_i$ $(i=1,\dots ,k )$  
...
with unit norm eigenvectors ($\uuu$'s) trivially satisfy Equation~\eqref{u},
where $\uuu$ is the inflated vector of 
${\tilde \uuu } = (u (1) ,\dots ,u(k ))^*$ with block-sizes $n_1 ,\dots ,n_k$.
...
$$
that in turn is equivalent to Equation~\eqref{v},
...
The (random) adjacency matrix $\A$ of (the random graph) $G_n$ is 
$\A ={\bar \A } +\W$, where
...
than from $\B$. The same is true for the relation of the $k$ leading
...
eigenvectors of $\B$ have an inner product with the $\bm{End}$-transforms
of the analogous eigenvectors
...
we know that $\uuu$ is a step-vector with $k$ different coordinates.  
...
$$
and, as ${\bm{End}}^*{\bm{End}} =\DD $ holds by~\eqref{es},
$$ 
 \left \| \x^{in} - \DD \frac{\uuu }{\| {\bm{End} } \,
\uuu \|} \right \|^2 =
...
the random graph has $k$ structural eigenvalues with eigenvectors
...
\textit{Expectation-Maximization}, briefly EM algorithm,
...
The above is also in accord with Equation~\eqref{bet}, since the threshold for 
$\beta$ is
$$
...
is more dangerous than the uniform one. Indeed, as below $\beta_1$ the graph 
is not connected, there are many small components,  among
...
no between-cluster bonds, but those are negligible to the within-cluster ones.
...
We also thank Katalin Friedl and Daniel Kelinger for their useful remarks.
...
\, textit{The Annals of Probability} \textbf{46} (1), 1--71.
...
%\bibitem{Kelinger}
%  Kelinger, D., Lov\'asz, L., M\'ori, T., \'Odor, G., Switchover phenomenon
