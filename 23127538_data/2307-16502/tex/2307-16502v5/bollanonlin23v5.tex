...
two senses: the within-
and between-cluster edge probabilities
...
$o(1)$).
%In case of expanding regular graphs,
...
theoretical considerations (see, e.g.,~\cite{Capitaine,Stephan}). 
...
$1,2,\dots ,k$ clusters can be detected, see Section~\ref{betaperc}.
...
$\lambda (\B)$ which is positive real, it is the spectral radius $\rho (\B )$
of $\B$,
and it can also have some other so-called structural
...
of the underlying graph (see Proposition 5.2 of~\cite{Glover}). 
...
\item
Proposition 5.5 of~\cite{Glover} states that if $G$ is a connected graph which
...
$\B$ is irreducible. Therefore, the Perron--Frobenius theory is applicable
to $\B$, and under the above conditions, its largest absolute value
eigenvalue is a single positive real one, with corresponding
eigenvector having positive real coordinates. 
...
$\V^2 =\I$, $\V$ is an orthogonal and symmetric matrix):
$$
 \V =\begin{pmatrix}
 \bm O  & \I_m \\
 \I_m  & \bm O
...
Note that this $\W$  is approximately 
$\frac1{\sqrt{n}}$ times a Wigner noise, and a ``semicircle law'' is also valid
with radius of  constant order: 
$2\sigma =2\max_{a,b} \sqrt{p_{ab} (1-p_{ab})} \le 1$, see~\cite{Capitaine}.
Now $\bar \A$ is the finite rank ($k$) perturbation on this
``deformed Wigner noise'', and if 
$\lambda_{max} ({\bar \A })\sim \lambda_{max} (\B ) 
>1$, then the spectrum of $\A$ is out of the semicircle.
...
In~\cite{Odor}, the effect of different seeding patterns is investigated
...
quantum chemistry, for example. In the estimation,
discrepancy techniques, see~\cite{Bolla16,Bollakk} may help.
...
This research was partly done under the auspices of the Budapest Semesters in 
...
the second author and Josephine Walk who helped to organize the early simulation
results of the second author into digestible form.
...
We also thank Tam\'as M\'ori, Katalin Friedl and Daniel Keliger for their
useful remarks.
