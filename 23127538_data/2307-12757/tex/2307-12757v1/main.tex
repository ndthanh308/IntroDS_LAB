...
\begin{document}
 ...
 \email{shariqamu90@gmail.com, maithilpraveen@gmail.com,  jitenderarora09@gmail.com}
	...
	%\date{...}
\begin{abstract}
  Let $R$ be a commutative ring with unity. The weakly zero-divisor graph $W\Gamma(R)$ of the ring $R$ is the simple undirected  graph whose vertices are nonzero zero-divisors of $R$ and two vertices $x$, $y$ are adjacent if and only if there exists $r\in {\rm ann}(x)$ and $s \in {\rm ann}(y)$ such that $rs =0$. The zero-divisor graph of a ring is a sp{\rm ann}ing subgraph of the weakly zero-divisor graph. It is known that the zero-divisor graph of the ring $\mathbb{Z}_{{p^t}}$, where $p$ is a prime, is the Laplacian integral. In this paper, we obtain the Laplacian spectrum of the weakly zero-divisor graph $W\Gamma(\mathbb{Z}_{n})$ of the ring $\mathbb{Z}_{n}$ and show that  $W\Gamma(\mathbb{Z}_{n})$ is Laplacian integral for arbitrary $n$. 
\end{abstract}
...
%In Section 5, the Laplacian spectral radius and the algebraic connectivity of $W\Gamma(\mathbb{Z}_n)$ have been investigated. 
...
For $x, y \in \mathcal{A}_{d_{i}}$, we have $ \langle x\rangle = \langle y\rangle= \langle d_i \rangle$. Further, note that the sets $\mathcal{A}_{d_{1}}, \mathcal{A}_{d_{2}}, \ldots, \mathcal{A}_{d_{k}}$ form a partition of the vertex set of the graph $W\Gamma(\mathbb{Z}_n)$. Thus, $V(W\Gamma(\mathbb{Z}_n)) = \mathcal{A}_{d_{1}} \cup \mathcal{A}_{d_{2}} \cup \cdots \cup \mathcal{A}_{d_{k}}$. 
\end{remark}
...
It follows that
$D_p = \phi(p^{k})+\phi(p^{k-2}q)+\phi(p^{k-3}q)+\cdots+\phi(q)+\phi(p^{k-1})+\phi(p^{k-2})+\cdots +\phi(p)=|V|-\phi(p^{k-1}q)$, $D_{p^2}=|V|-\phi(p^{k-2}q)$, $D_{p^{k_i}} =| V|-\phi(p^{k-i}q)$,  $D_{pq} = | V|-\phi(p^{k-1})$,$\ldots,$ $D_{p^rq} = | V|-\phi(p^{k-r})$, $D_{p^{k-1}q} =  |V|-\phi(p)$, $D_q=|V|-\phi(p^{k})$ 
...
\[ = \begin{bmatrix}
V-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) &  \cdots &- \phi(q )& -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots &-\phi(p) &-\phi(p^k)\\
-\phi(p^{k-1}q) &  V-\phi(p^{k-2}q) &   \cdots & -\phi(q ) & -\phi(p^{k-1}) & -\phi(p^{k-2}) & \cdots &-\phi(p) &-\phi(p^k) \\
%\cdots & \cdots &  \cdots & \cdots & \cdots & \cdots & \cdots & \cdots & \cdots\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &   -\phi(p^{k-2}q) &   \cdots & V-\phi(p^{k-i}q) & -\phi(p^{k-1}) & -\phi(p^{k-2}q) & \cdots  &-\phi(p) &-\phi(p^k)\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
 -\phi(p^{k-1}q) &  -\phi(p^{k-2}q) &   \cdots &- \phi(q )&V -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots& -\phi(p) &-\phi(p^k)\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) &  \cdots &- \phi(q )& -\phi(p^{k-1})&\cdots &V-\phi(p^{k-r)})&\cdots &-\phi(p^k)\\
%\cdots & \cdots &  \cdots & \cdots & \cdots & \cdots & \cdots & \cdots & \cdots\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) &  \cdots &- \phi(q )& -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots &-\phi(p) &V-\phi(p^k)
\end{bmatrix},\] 
\vspace{.1cm}
where the matrix $L(\Upsilon_{p^{k}q})$ is obtained by indexing the rows and columns as $p, p^2, \ldots, p^{k},  p^{k-1}q, p^{k-2}q,\ldots, pq, q$. The matrix $ L(\Upsilon_{p^{k}q})$ can be decomposed into $A + B$, where $A = |V|I$  with the identity matrix $I$ of order $2k$ and

  \[B = \displaystyle\begin{bmatrix}
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots &- \phi(q )& -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots &-\phi(p) &-\phi(p^k)\\
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots & -\phi(q ) & -\phi(p^{k-1}) & -\phi(p^{k-2}) & \cdots &-\phi(p) &-\phi(p^k) \\
%\cdots & \cdots &  \cdots & \cdots & \cdots & \cdots & \cdots & \cdots & \cdots\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &   -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots & -\phi(q) & -\phi(p^{k-1}) & -\phi(p^{k-2}q) & \cdots  &-\phi(p) &-\phi(p^k)\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
 -\phi(p^{k-1}q) &  -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots &- \phi(q )& -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots& -\phi(p) &-\phi(p^k)\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots &- \phi(q )& -\phi(p^{k-1})&\cdots &-\phi(p^{k-r)})&\cdots &-\phi(p^k)\\
%\cdots & \cdots &  \cdots & \cdots & \cdots & \cdots & \cdots & \cdots & \cdots\\
\vdots & \vdots &  \vdots & \vdots & \vdots & \vdots & \vdots & \vdots & \vdots \\
-\phi(p^{k-1}q) &  -\phi(p^{k-2}q) & -\phi(p^{k-3}q) & \cdots &- \phi(q )& -\phi(p^{k-1})&-\phi(p^{k-2}) & \cdots &-\phi(p) &-\phi(p^k)\\
\end{bmatrix}.\]
...
\newblock {\em Linear Multilinear Algebra}, pages 1--16, 2020.
...
\newblock On the planarity, genus and crosscap of weakly zero-divisor graph of
  commutative rings.
\newblock {\em Rev. Un. Mat. Argentina}, 08 2022.
