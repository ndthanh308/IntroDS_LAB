...
This motivates the study of {\bf output-sensitive} algorithms for $k$-clique listing: algorithms whose runtime depends on the number of $k$-cliques in the output. An even more desirable version of an output-sensitive algorithm is one that can also take as input some parameter $t$, and can list up to $t$ $k$-cliques in the graph. When $t$ is much smaller than the number of $k$-cliques in the graph, such an algorithm could potentially be more efficient. These two versions are actually runtime-equivalent up to logarithmic factors for most natural running times (we provide a proof in  Section~\ref{sec:prelim} for completeness). We thus use these  two notions interchangeably. 


Bj{\"o}rklund, Pagh, Vassilevska W. and Zwick \cite{bjorklund2014listing} designed such output-sensitive algorithms for triangle listing with runtime $\tilde{O}(n^{\omega}+n^{\frac{3(\omega-1)}{5-\omega}}t^{\frac{2(3-\omega)}{5-\omega}})$ and $\tilde{O}(m^{\frac{2\omega}{\omega+1}}+m^{\frac{3(\omega-1)}{\omega+1}}t^{\frac{3-\omega}{\omega+1}})$\footnote{We use $\tilde{O}$ to hide polylog factors.}, where $\omega<2.373$~\cite{alman2021refined} is the exponent of matrix multiplication and $t$ is the number of triangles listed. If $\omega=2$, the runtimes simplify to $\tilde{O}(n^2+nt^{2/3})$ and $\tilde{O}(m^{4/3}+mt^{1/3})$, and these are shown to be conditionally optimal for any $t= \Omega(n^{1.5})$ and $t= \Omega(m)$ respectively under the popular $3$SUM hypothesis \cite{patrascu2010towards,kopelowitz2016higher} and the even more believable Exact Triangle hypothesis \cite{williams2020monochromatic}.

While the output-sensitive question for triangle listing is well-understood by now, no similar conditionally optimal results are known for $k$-clique listing when $k>3$.
...
We are not the first to suggest this natural parameterization of the input. In fact, small $\Delta_\ell$ values are exploited to obtain faster $k$-clique algorithms in experimental algorithmics: e.g., \cite{count5via3} and \cite{osti_1141233} count $k$-cliques faster in graphs with a small number of triangles. Motivated by these practical results, we are the first to consider the following question within theoretical computer science:
...
We present a systematic study of clique finding and listing, and provide answers to both Questions \ref{q2} and \ref{q3}.
We give the first general algorithms for detecting and listing $k$-cliques, and the first fine-grained lower bounds for the listing problem, showing that our algorithms are tight for a non-trivial range of the number of $k$-cliques to output. We summarize our contributions in Table~\ref{tab:contributions}.
...
\renewcommand{\arraystretch}{1.2} 
    \begin{table}[h]
        ...
        \hline 
             ...
             \hline 
             \textbf{Lower bounds} & Conditional lower bounds for $\cliquelist{k,\ell}$ & Theorem~\ref{thm:lb_intro}\\
             \hline 
             ...
             & Optimal $\cliquelist{k,\ell}$ algorithms for large $t$ & Theorem~\ref{thm:intro-k-l-large-t-listing}\\
             & Generalized $\cliquelist{k,\ell}$ algorithm for all $t$ & Theorem~\ref{thm:reduction_from_small_t}\\
...
\paragraph{A general algorithm for finding $k$-cliques in terms of the number of $\ell$-cliques.} 

Let $\cliquedet{k, \ell}$ be the problem of detecting a $k$-clique in a graph $G$, given the list of all $\ell$-cliques in the graph for some $\ell\in \{1,\ldots,k-1\}$. 

We propose a general framework to solve $\cliquedet{k, \ell}$. 
Our framework is a vast generalization of the previous $k$-clique detection algorithms which were only in terms of $n=\Delta_1$ and $m=\Delta_2$. 

While we refer only to the clique detection problem, our algorithm can also find a $k$-clique if one exists.
In fact, the detection and finding problem for $k$-clique for any constant $k$ are runtime-equivalent.
For $\ell=1$, and for $k=3$ and $\ell=2$ this was shown within a constant factor (see e.g.~\cite{focsyj,JafargholiV16}).
More generally, for any constant $k$, the detection and finding problems for $k$-clique in terms of $\Delta_\ell$ (for any $\ell<k$) are equivalent up to a log factor (see Section~\ref{sec:prelim:basic}). Our algorithm can also be used to count the number of cliques with the same runtime. 
...
Let us compare our $\cliquedet{k, 1}$ runtime with previous results. 
For $k$ not divisible by $3$,
Eisenbrand and Grandoni \cite{eisenbrand2004complexity} extended Ne\v{s}etril and Poljak's reduction to obtain a $k$-clique runtime of
 $O(n^{\beta(k)})$ where $\beta(k)=\omega(\lceil{k/3}\rceil, \lceil{(k-1)/3}\rceil, \lfloor{k/3}\rfloor)$, and $\omega(a,b,c)$ is the exponent of multiplying an $n^a\times n^b$ matrix by an $n^b\times n^c$ matrix. As the runtime of $k$-clique detection has remained unchallenged for several decades, the hypothesis that these algorithms are optimal has been used to provide conditional lower bounds in several works (e.g. \cite{AbboudBW18,BackursT17,BringmannW17}). 
...
For $\cliquedet{k, 1}$, our runtime indeed matches the best runtime for $k$-clique detection in terms of $n$, as in \cite{nesetril1985complexity,eisenbrand2004complexity} and Hypothesis~\ref{hyp:k_clique}.

Our general framework is actually quite powerful, and allows us to obtain \emph{the first improvement in almost 20 years} over
the runtime of Eisenbrand and Grandoni \cite{eisenbrand2004complexity} for $4$ and $5$-clique detection in terms of $m$ (we show this in Examples~\ref{ex:clique-det-4-2} and \ref{ex:clique-det-5-2} in Section~\ref{sec:detection_examples}).


 \begin{theorem}\label{intro:detection-m}
     There is an $O(m^{1.657})$ time algorithm for $\cliquedet{4,2}$ and an $O(m^{2.058})$ time algorithm for $\cliquedet{5,2}$ in $m$-edge graphs.
 \end{theorem}
We compare the explicit values of~\cite{eisenbrand2004complexity}'s exponent and our improved exponents in Table~\ref{table:improved_det_4_5} in terms of the current bounds for rectangular matrix multiplication. 
    ...
    \begin{table}[ht]
        ...
        \begin{tabular}{c|c|c}
         ...
         5 & 2.097 & 2.058
        \end{tabular}
        \caption{The table contains exponents $c$ such that $4$ and $5$-clique detection is in $O(m^c)$ time. For $\cliquedet{4, 2}$ and $\cliquedet{5, 2}$, the previous exponent was given by $\beta(k) \cdot \beta(k-1)/(\beta(k) + \beta(k-1) - 1)$ \cite{eisenbrand2004complexity}, where $\beta(k)$ is the exponent of $\cliquedet{k, 1}$. We give the runtime of their algorithm with the current bounds on rectangular matrix multiplication \cite{LU18}.}\label{table:improved_det_4_5}
    \end{table}
    


    For any $\ell \leq \lfloor k/3 \rfloor$, the runtime of our general algorithm is simple to state:  
    
    \begin{theorem}
         For any positive integers $k \ge 3$ and $\ell \le \lfloor k/3\rfloor$, $\cliquedet{k, \ell}$ can be solved in $\tilde{O}(\Delta_\ell^{\beta(k)/\ell})$ time.
    \end{theorem}
    

For every $k$ and $\ell \le \lfloor k/3\rfloor$ the runtime above is essentially optimal
under Hypothesis~\ref{hyp:k_clique}.
    
\begin{proposition}
        For any positive integers $k \ge 3$ and $\ell \le \lfloor k/3\rfloor$, if $\cliquedet{k, 1}$ requires $n^{\beta(k) - o(1)}$ time, then
        $\cliquedet{k, \ell}$ requires $\Delta_\ell^{\beta(k)/\ell-o(1)}$ time. 
\end{proposition}

Analyzing the runtime of our algorithm for $\ell> \lfloor k/3\rfloor$ becomes significantly more complex.
In Section~\ref{sec:detection}, we provide two types of analyses for our detection framework: (1) when $\ell$ is additively close to $k$, i.e. for small $h$ we give bounds for  $\cliquedet{k, k-h}$, and (2) when $\ell$ is multiplicative close to $k$, i.e. $\cliquedet{k, k/\alpha}$ for constants $\alpha > 1$.


\paragraph{Conditional lower bounds for $k$-clique listing.} 
...
Prior works \cite{patrascu2010towards,kopelowitz2016higher,williams2020monochromatic} give fine-grained lower bounds for listing triangles in an $n$-node, $m$-edge graph: $\cliquelist{3, 1}$ requires $n^{1-o(1)}t^{2/3}$ time and $\cliquelist{3, 2}$ requires $m^{1-o(1)}t^{1/3}$ time. The lower bounds imply tightness of the known algorithms \cite{bjorklund2014listing}  if $t$ is large enough: $t= \Omega(n^{1.5})$ or $t= \Omega(m)$ respectively.
...
Under the Exact-$k$-Clique hypothesis we prove  lower bounds for $\cliquelist{k, \ell}$ for \emph{all} $k\geq 3, 1\leq \ell<k$.
These are the first fine-grained lower bounds for any $k>3$. This is Theorem~\ref{thm:lower_bound} in the main body. 
...
$$\left(\Delta_\ell^{\frac{2}{\ell(k-\ell)}} t^{1 - \frac{2}{k(k-\ell)}}\right)^{1-o(1)}$$ time in  the word-RAM model with $O(\log n)$ bit words, under the  Exact-$k$-Clique hypothesis.
\end{theorem}

For $k=3$ this is the same lower bound as previously proven \cite{patrascu2010towards,kopelowitz2016higher}.
Shortly, we will present algorithms that  match our lower bound for all $k,\ell$ and for large $t$ if $\omega =2$, implying that our lower bound is tight in this case.

    
    \paragraph{Optimal algorithms for 4 and 5-clique listing.} For the special cases of $\cliquelist{4,1}$ and $\cliquelist{5,1}$, we give algorithms which are indeed conditionally optimal if $\omega = 2$. We prove these results in Corollary~\ref{cor:4-1-opt} and Corollary~\ref{cor:5_1_opt} in the main body.

    Similar to \cite{bjorklund2014listing}, we state our runtimes in terms of $\omega$. We note that if $\omega>2$, the bounds can be slightly improved via rectangular matrix multiplication.
    ...
    Recall that the $4$-Clique hypothesis, which is a special case of  Hypothesis~\ref{hyp:k_clique} when $k = 4$, gives a lower bound of $n^{3-o(1)}$ if $\omega = 2$. Moreover, Theorem~\ref{thm:lb_intro} gives a lower bound of $(n^{2/3}t^{5/6})^{1-o(1)}$. Therefore, our $\cliquelist{4, 1}$ algorithm is indeed conditionally optimal.
    ...
    \end{theorem}
    Recall that the 5-Clique hypothesis from Hypothesis~\ref{hyp:k_clique} gives us a lower bound of $n^{4-o(1)}$ if $\omega = 2$. Moreover,  Theorem~\ref{thm:lb_intro} gives a lower bound of $(n^{1/2}t^{9/10})^{1-o(1)}$. Therefore, our $\cliquelist{5, 1}$ algorithm is also conditionally optimal. 
    
    \paragraph{Nearly-everywhere optimal algorithms for 4 and 5-clique listing in sparse graphs.} In the case of $\cliquelist{4,2}$ and $\cliquelist{5,2}$, we obtain conditionally optimal runtimes for almost all values of $t$ except a small range if $\omega = 2.$ We prove this in Section~\ref{sec:4_5_l_listing}.
    We pictorially depict the runtimes in Figure~\ref{fig:4_5_sparse_runtime}.
    ...
    \begin{theorem}\label{thm:4_2}
        ...
        \end{align*}
    \end{theorem}
    This algorithm matches the lower bound in Hypothesis~\ref{hyp:k_clique} when $t \leq m^{5/4}$, and it matches our lower bound of Theorem~\ref{thm:lb_intro} when $t \geq m^{10/7}$.
    ...
    \end{theorem} 
     ...
     This algorithm matches the runtime of the lower bound in Hypothesis~\ref{hyp:k_clique} when $t \leq m^{19/10}$, and it matches our lower bound from Theorem~\ref{thm:lb_intro} when $t \geq m^{55/28}$.
    
    \paragraph{Optimal algorithms for listing many $k$-cliques.} More generally, we consider the problem of $\cliquelist{k, \ell}$ for all $k > \ell$. For instance, consider the problem of $\cliquelist{6, 2}$. If we adapt the existing approach for $k$-clique detection~\cite{nesetril1985complexity, eisenbrand2004complexity} and directly reduce it to triangle listing in a graph with $m$ nodes and then use \cite{bjorklund2014listing}, we get an $\tO(m^2+mt^{2/3})$ runtime when $\omega = 2$. In comparison, the lower bound from Theorem~\ref{thm:lb_intro} is $(m^{1/4}t^{11/12})^{1-o(1)}$. When $t$ is close to maximum (as $t \to O(m^3)$), the $\tO(m^2+mt^{2/3})$ runtime is polynomially higher than the lower bound. Therefore, we cannot only rely on such reductions. 
    
    Nevertheless, we give a conditionally {\em tight} algorithm for graphs with many $k$-cliques, provided that $\omega=2$. In particular, the runtime of the algorithm in the theorem below matches the lower bound of Theorem~\ref{thm:lb_intro}. 
    ...
    We give more explicit bounds in terms of $\omega$ in Sections~\ref{sec:k_1_opt} and \ref{sec:k_l_opt}. In other words, we have an algorithm which matches the lower bound in Theorem~\ref{thm:lb_intro} for graphs with many $k$-cliques. At a high level, this algorithm requires a highly rectangular matrix multiplication, which has an inherent cost that makes it sub-optimal for graphs with fewer $k$-cliques.
    
    \paragraph{General listing algorithm for all $t$.} 
    In Section~\ref{sec:general-list}, we give a general black-box approach (by adapting previous reductions \cite{nesetril1985complexity, eisenbrand2004complexity}) that uses our (conditionally) optimal algorithm for a large number of $k$-cliques $t$ to obtain a fast algorithm that works for {\em all} $t$. The main advantage of this approach is its simplicity and generality. In particular, we obtain an intuitive and simple analysis of the runtime for all $k,\ell,t$. In Section~\ref{sec:general-list}, we show a comparison of our lower bounds and the runtime of our general algorithm in some examples. We illustrate the runtime of the general algorithm for some specific cases in Figure~\ref{fig:general_examples}.

    % Figure environment removed
    
    
    \paragraph{Improved algorithm for 6-clique listing.} We note that our generic algorithm trades simplicity for optimality, and it is not always the best algorithm one can obtain for fixed $k,\ell$. 
    
    In Section~\ref{sec:6clique}, we give a more refined algorithm for 6-clique listing if $\omega = 2$ to illustrate how one might obtain a tighter runtime bound for  $\cliquelist{k, \ell}$ for specific $k$ and $\ell$. In Figure~\ref{fig:6_1}, we compare our ``general'' bound, our best bound and our lower bounds to illustrate the improvement in the algorithm. However, since the number of terms and parameters in the runtime increases significantly with $k$, we do not do this refined analysis for all $k$.
     % Figure environment removed
...
For $a < \ell$, we set a parameter $D$ and consider two types of $a$-cliques: ``low-degree" ones that are contained in $<D$ $\ell$-cliques, and ``high-degree'' ones that are contained in $\geq D$ $\ell$-cliques. There are at most $O(\Delta_\ell/D)$ high-degree $a$-cliques.
...
We can bound the recursion runtime using the fact that $K$ has low degree. Since we have handled all low-degree $a$-cliques, we can set $A$ to be only the $O(\Delta_\ell/D)$ high-degree $a$-cliques. Similarly, we can get bounds on $|B|$ and $|C|$.
...
Bj\"{o}rklund et al. \cite{bjorklund2014listing} gave an algorithm for triangle listing using a  \textit{dense-sparse} paradigm. We generalize this algorithm to  $k \geq 4$. Let $t$ be the number of $k$-cliques in the graph which we want to list.
