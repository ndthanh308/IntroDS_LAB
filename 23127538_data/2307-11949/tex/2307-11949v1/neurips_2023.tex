...
\author{%
  ...
  $^{1}$University of California, Berkeley \quad $^{2}$Carnegie Mellon University \\
...
and hierarchical RL~\citep{option_sutton1999,option_stolle2002,oc_bacon2017,eigen_machado2017,ho2_wulfmeier2021,mo2_salter2022},
...
with no need for complex graph-based planning procedures.
...
Amongst goal-conditioned approaches, hierarchical imitation learning \citep{play_lynch2019,ril_gupta2019}
...
Finally, our method is similar to POR \citep{por_xu2022},
...
Specifically, \methodname trains
%
...
J_{\pi^\ell}(\theta_\ell) &= \E_{(s_t, a_t, s_{t+1}, s_{t+k})}[\exp(\beta \cdot \tilde A^\ell(s_t, a_t, g))
...
%
as they are (mostly) constants or can be subsumed into the temperature $\beta$ (see Appendix for further discussion).
%
%
%
...
the high-level policy $\pi^h(z_{t+k} \mid s_{t+k}, g)$ produces $z_{t+k} = \phi(s_{t+k})$ instead of $s_{t+k}$,
%
...
\end{align*}
\end{proposition}
We provide a pseudocode for \methodname in \Cref{alg:algo} and the full training details in Appendix.
%
...
\begin{algorithm}[t!]
    ...
    \begin{algorithmic}[1]
        ...
        high-level policy $\pi_{\theta_h}^h(z_{t+k}|s_{t+k}, g)$, low-level policy $\pi_{\theta_\ell}^\ell(a|s_t, z_{t+k})$,
...
For offline goal-conditioned RL methods, we evaluate a goal-conditioned variant of IQL \citep{iql_kostrikov2021} (\Cref{sec:prelim}),
which does not use hierarchy,
and POR \citep{por_xu2022}, which uses hierarchy but does not use temporal abstraction (\ie, similar to $k=1$ in \methodname) nor representation learning. In AntMaze, we additionally compare \methodname with two model-based approaches that studied this domain in prior work:
...
We provide full details of environments and baselines in Appendix.
%
...
\begin{table}[t!]
    ...
    %
    `gc-' denotes goal-conditioned variants.
    We show the standard deviations across $8$ random seeds and refer to Appendix
    %
    for the full training curves.
    Baselines: GCBC~\citep{gcsl_ghosh2019}, HGCBC~\citep{ril_gupta2019}, IQL~\citep{iql_kostrikov2021}, POR~\citep{por_xu2022}, TAP~\citep{tap_jiang2022}, TT~\citep{tt_janner2021}.
    }
    ...
    \toprule
    \textbf{Task} & \textbf{GCBC} & \textbf{HGCBC} & \textbf{IQL} & \textbf{POR} & \textbf{TAP} & \textbf{TT} & \textbf{\methodname} (\textbf{ours}) & \textbf{\methodname} (w/o repr.) \\
    \midrule
    gc-antmaze-medium-diverse & $67.3$ {\tiny $\pm 10.1$} & $71.6$ {\tiny $\pm 8.9$} & $63.5$ {\tiny $\pm 14.6$} & $74.8$ {\tiny $\pm 11.9$} & $85.0$ & $\mathbf{100.0}$ & $86.8$ {\tiny $\pm 4.6$} & $89.9$ {\tiny $\pm 3.5$} \\
    gc-antmaze-medium-play & $71.9$ {\tiny $\pm 16.2$} & $66.3$ {\tiny $\pm 9.2$} & $70.9$ {\tiny $\pm 11.2$} & $71.4$ {\tiny $\pm 10.9$} & $78.0$ & $\mathbf{93.3}$ & $84.1$ {\tiny $\pm 10.8$} & $87.0$ {\tiny $\pm 8.4$} \\
    gc-antmaze-large-diverse & $20.2$ {\tiny $\pm 9.1$} & $63.9$ {\tiny $\pm 10.4$} & $50.7$ {\tiny $\pm 18.8$} & $49.0$ {\tiny $\pm 17.2$} & $82.0$ & $60.0$ & $\mathbf{88.2}$ {\tiny $\pm 5.3$} & $87.3$ {\tiny $\pm 3.7$} \\
    gc-antmaze-large-play & $23.1$ {\tiny $\pm 15.6$} & $64.7$ {\tiny $\pm 14.5$} & $56.5$ {\tiny $\pm 14.4$} & $63.2$ {\tiny $\pm 16.1$} & $74.0$ & $66.7$ & $\mathbf{86.1}$ {\tiny $\pm 7.5$} & $81.2$ {\tiny $\pm 6.6$} \\
    gc-antmaze-ultra-diverse & $14.4$ {\tiny $\pm 9.7$} & $39.4$ {\tiny $\pm 20.6$} & $21.6$ {\tiny $\pm 15.2$} & $29.8$ {\tiny $\pm 13.6$} & $26.0$ & $33.3$ & $\mathbf{52.9}$ {\tiny $\pm 17.4$} & $52.6$ {\tiny $\pm 8.7$} \\
    gc-antmaze-ultra-play & $20.7$ {\tiny $\pm 9.7$} & $38.2$ {\tiny $\pm 18.1$} & $29.8$ {\tiny $\pm 12.4$} & $31.0$ {\tiny $\pm 19.4$} & $22.0$ & $20.0$ & $39.2$ {\tiny $\pm 14.8$} & $\mathbf{56.0}$ {\tiny $\pm 12.4$} \\
    \midrule
    gc-kitchen-partial & $38.5$ {\tiny $\pm 11.8$} & $32.0$ {\tiny $\pm 16.7$} & $39.2$ {\tiny $\pm 13.5$} & $18.4$ {\tiny $\pm 14.3$} & - & - & $\mathbf{65.0}$ {\tiny $\pm 9.2$} & $46.3$ {\tiny $\pm 8.6$} \\
    gc-kitchen-mixed & $46.7$ {\tiny $\pm 20.1$} & $46.8$ {\tiny $\pm 17.6$} & $51.3$ {\tiny $\pm 12.8$} & $27.9$ {\tiny $\pm 17.9$} & - & - & $\mathbf{67.7}$ {\tiny $\pm 6.8$} & $36.8$ {\tiny $\pm 20.1$} \\
    \midrule
    gc-calvin & $17.3$ {\tiny $\pm 14.8$} & $3.1$ {\tiny $\pm 8.8$} & $7.8$ {\tiny $\pm 17.6$} & $12.4$ {\tiny $\pm 18.6$} & - & - & $\mathbf{43.8}$ {\tiny $\pm 39.5$} & $23.4$ {\tiny $\pm 27.1$} \\
    \bottomrule
    \end{tabular}
    }
\end{table}
\begin{table}
    ...
    We refer to Appendix for the full training curves.
    %
    }
    %
        \centering
        ...
        \toprule
        \textbf{Task} & \textbf{GCBC} & \textbf{HGCBC} {\scriptsize (+ repr.)} & \textbf{IQL} & \textbf{POR} {\scriptsize (+ repr.)} & \textbf{\methodname} (\textbf{ours}) \\
        \midrule
        gc-procgen-maze-500-train & $16.8$ {\tiny $\pm 2.8$} & $14.3$ {\tiny $\pm 4.1$} & $72.5$ {\tiny $\pm 10.0$} & $75.8$ {\tiny $\pm 12.1$} & $\mathbf{82.5}$ {\tiny $\pm 6.0$} \\
        gc-procgen-maze-500-test & $14.5$ {\tiny $\pm 5.0$} & $11.2$ {\tiny $\pm 3.7$} & $49.5$ {\tiny $\pm 9.8$} & $53.8$ {\tiny $\pm 14.5$} & $\mathbf{64.5}$ {\tiny $\pm 13.2$} \\
        gc-procgen-maze-1000-train & $27.2$ {\tiny $\pm 8.9$} & $15.0$ {\tiny $\pm 5.7$} & $78.2$ {\tiny $\pm 7.2$} & $82.0$ {\tiny $\pm 6.5$} & $\mathbf{87.0}$ {\tiny $\pm 13.9$} \\
        gc-procgen-maze-1000-test & $12.0$ {\tiny $\pm 5.9$} & $14.5$ {\tiny $\pm 5.0$} & $60.0$ {\tiny $\pm 10.6$} & $69.8$ {\tiny $\pm 7.4$} & $\mathbf{78.2}$ {\tiny $\pm 17.9$} \\
        \midrule
        gc-visual-antmaze-diverse & $71.4$ {\tiny $\pm 6.0$} & $35.1$ {\tiny $\pm 12.0$} & $72.6$ {\tiny $\pm 5.9$} & $47.4$ {\tiny $\pm 17.6$} & $\mathbf{80.5}$ {\tiny $\pm 9.4$} \\
        gc-visual-antmaze-play & $64.4$ {\tiny $\pm 6.3$} & $23.8$ {\tiny $\pm 8.5$} & $70.4$ {\tiny $\pm 26.6$} & $57.0$ {\tiny $\pm 8.1$} & $\mathbf{78.4}$ {\tiny $\pm 4.6$} \\
        gc-visual-antmaze-navigate & $33.2$ {\tiny $\pm 7.9$} & $21.4$ {\tiny $\pm 4.6$} & $22.1$ {\tiny $\pm 14.1$} & $16.1$ {\tiny $\pm 15.2$} & $\mathbf{45.7}$ {\tiny $\pm 18.1$} \\
        \midrule
...
%
%
For the prior hierarchical approaches that generate raw subgoals (HGCBC and POR),
...
Notably, action-limited \methodname still outperforms previous offline RL methods (IQL and POR) trained with the full action-labeled data.
...
\begin{table}[t!]
    ...
    \toprule
    \textbf{Task} & \textbf{IQL} (full) & \textbf{POR} (full) & \textbf{\methodname} (full) & \textbf{\methodname} (\textbf{action-limited}) & {\color{gray}\textbf{vs. HIQL} (full)} & {\color{gray}\textbf{vs. Prev. best} (full)} \\
    \midrule
    gc-antmaze-large-diverse & $50.7$ {\tiny $\pm 18.8$} & $49.0$ {\tiny $\pm 17.2$} & $88.2$ {\tiny $\pm 5.3$} & $88.9$ {\tiny $\pm 6.4$} & {\color{myblue}$+0.7 $} & {\color{myblue}$+38.2 $} \\
    gc-antmaze-ultra-diverse & $21.6$ {\tiny $\pm 15.2$} & $29.8$ {\tiny $\pm 13.6$} & $52.9$ {\tiny $\pm 17.4$} & $38.2$ {\tiny $\pm 15.4$} & {\color{myred}$-14.7 $} & {\color{myblue}$+8.4 $} \\
    gc-kitchen-mixed & $51.3$ {\tiny $\pm 12.8$} & $27.9$ {\tiny $\pm 17.9$} & $67.7$ {\tiny $\pm 6.8$} & $59.1$ {\tiny $\pm 9.6$} & {\color{myred}$-8.6 $} & {\color{myblue}$+7.8 $} \\
    gc-calvin & $7.8$ {\tiny $\pm 17.6$} & $12.4$ {\tiny $\pm 18.6$} & $43.8$ {\tiny $\pm 39.5$} & $35.8$ {\tiny $\pm 30.7$} & {\color{myred}$-8.0 $} & {\color{myblue}$+23.4 $} \\
    gc-procgen-maze-500-train & $72.5$ {\tiny $\pm 10.0$} & $75.8$ {\tiny $\pm 12.1$} & $82.5$ {\tiny $\pm 6.0$} & $77.0$ {\tiny $\pm 12.5$} & {\color{myred}$-5.5 $} & {\color{myblue}$+1.2 $} \\
    gc-procgen-maze-500-test & $49.5$ {\tiny $\pm 9.8$} & $53.8$ {\tiny $\pm 14.5$} & $64.5$ {\tiny $\pm 13.2$} & $65.5$ {\tiny $\pm 16.4$} & {\color{myblue}$+1.0 $} & {\color{myblue}$+11.7 $} \\
    \bottomrule
    \end{tabular}
    }
\end{table}
...
we compare the policy accuracies of IQL (flat policy), POR (hierarchy without temporal abstraction),
...
We refer to Appendix for further analyses, including
...
Empirically, we found that concatenating $s$ to the input (\ie, using $\phi([g, s])$ instead of $\phi(g)$),
...
we use the same architecture for \methodname and all the baselines implemented in this work (\ie, GCBC, HGCBC, IQL, and POR).
...
For HIQL, IQL, and POR, in state-based environments and Roboverse,
%
%
