...
\title{Spectral Independence Lecture Notes}
...
Our aim is to present the analysis from a
...
Our starting point for these notes were the summer school lectures and the associated lecture notes which were initially prepared by some of the students in attendance.  The lectures were given by Nima Anari, Pietro Caputo, Zongchen Chen, Heng Guo, Tali Kaufman, and Kuikui Liu.  Lecture notes were prepared by Yuzhou Gu, Tushant Mittal, Amanda Priestley, and Juspreet Singh Sandhu.
 For more information on the summer school, including lecture videos and notes, see:
...
For a pinning $\tau\in\Pinning$ where $\tau:S\rightarrow\{0,1\}$ for $S\subset V$, for $i\neq j$ where $i,j\notin S$, let 
\begin{align*}
...
\end{align*}
Note the emptyset is always a valid pinning and hence $\Psi=\Psi_\emptyset$.

The matrix $\Psi$ can be asymmetric and the entries of $\Psi$ can be positive or negative.  Nevertheless all of its eigenvalues are real, see \cref{DL1}.  Intuitively the fact that $\Psi$ is positive semidefinite is because it is closely related to the covariance matrix $\Cov$.  In particular, the influence matrix $\Psi$ is equal to a diagonal matrix times the covariance matrix $\Cov$ and then the fact follows from the fact that the covariance matrix is positive semidefinite.
...
Suppose there exists $\eta>0$ and $b>0$ such that
...
\[
  ...
  \]
    ...
    \begin{align*}
        \Psi_{\mu} = D^{-1}\Cov_{\mu}.
    \end{align*}
    Since $\Cov_{\mu}$ is symmetric positive semidefinite and $D$ is a diagonal matrix with positive diagonal entries, it follows that $\Psi_{\mu}$ has non-negative real eigenvalues.
\end{proof}
...
\[
\Exp_\mu[f] = \sum_{\eta\in\Omega}\mu(\eta)f(\eta).
\]
...
\[ \Var(f) := \Exp_\mu[(f - \mu(f))^2] = \sum_{\sigma\in\Omega}\mu(\sigma)\left(f(\sigma)-\mu(f)^2\right)
...
If $P$ is positive semidefinite then the absolute spectral gap is the same as the spectral gap and the bounds in \cref{eq:relax-gap} hold for $P$ in place of $P_{zz}$.  Moreover, Dyer, Greenhill, and Ullrich~\cite{DGU} proved that the heat-bath block dynamics is positive semidefinite and the Glauber dynamics is a special case of the heat-bath block dynamics where the blocks are single vertices.
...
We will partition the pinnings based on the number of vertices.  Recall, 
...
Let us first extend the definition of the influence matrix in \cref{defn:inf-matrix} to a fixed pinning.  For a subset $S\subset V$ where $|S|=k$, for a pinning $\tau:S\rightarrow\{0,1\}$ where $\tau\in\Pinning$, let the influence matrix $\Psi_\tau$ be defined as:
  \[
\Psi_\tau(i\rightarrow j) = \Psi_\tau(i,j) := \mu\left[\sigma(j)=1 \mid \sigma(i)=1,\sigma(S)=\tau\right]
- 
\mu\left[\sigma(j)=1 \mid \sigma(i)=0,\sigma(S)=\tau\right].
\]
...
We will ``zero-out'' these trivial eigenvalues of $1$ and $-1/(n-1)$ in the following manner:
\[
...
We can now utilize~\cref{lem:QandPsi} with \eqref{eqn:RW-thm} to conclude $\poly(n)$ relaxation time for the Glauber dynamics and thereby prove \cref{thm:SI-mixing}.

\begin{proof}[Proof of \cref{thm:SI-mixing}]
Recall, the definition of spectral independence in~\cref{defn:SI}; it states that the maximum eigenvalue of the influence matrix $\Psi$  is at most $\eta$.  Hence, from~\cref{lem:QandPsi}, we get that 
...
When $k\geq n-\eta-1$ then the local walk $Q_\tau$ for $\tau\in\Pinning_k$ is a walk on $2(n-k)\leq 2(\eta+1)$ states.  Since $\eta$ is a constant, then this a walk on a constant number of states and hence the spectral gap is lower bounded by a constant $C=C(\eta)$.  Therefore, the following holds:
\begin{equation}
    \label{eqn:gap-bigk}
\mbox{ if } k\geq n - \eta-1, \mbox{ then } \gamma_k\geq C \mbox{ for some constant $C>0$.}
\end{equation}

Now we can apply \eqref{eqn:RW-thm-simplified} in the Random Walk Theorem and we obtain the following:
\begin{align*}
 \gamma(P_{\mathrm{Glauber}}) &\geq \frac{1}{n}\prod_{k=0}^{n-2}\gamma_k
 \\
 &\geq (1-C)^{\eta-1}\times\frac{1}{n}\prod_{k=0}^{n-\eta-1}\gamma_k
 & \mbox{by \cref{eqn:gap-bigk} }
 \\
 &\geq (1-C)^{\eta-1}\times\frac{1}{n}\prod_{k=0}^{n-\eta-1} \left(1-\frac{\eta}{n-k-1}\right)
 & \mbox{by \cref{eqn:gammak} }
 \\
&\geq \frac{C'}{n}\prod_{k=0}^{n-\eta-1} \left(1-\frac{\eta}{n-k-1}\right) 
%&\mbox{by breaking into $k\leq n-\eta-1$ and $k>n-\eta-1$}
\\
&\geq
\frac{C'}{n}\exp\left(-\sum_{k=0}^{n-\eta-1}\frac{\eta}{n-k-1-\eta}\right)
& \mbox{since $1-x\geq\exp\left(-\frac{x}{1-x}\right)$} \\
&\geq \frac{C''}{n}\exp(-\eta\ln(n-\eta)) 
\\
&= \frac{C''}{n}(n-\eta)^{-\eta}
\\
&\geq  \frac{C''}{n^{1+\eta}}.
...
We generalize the above definition to a fixed pinning $\eta\in\Pinning$. For $0\leq\ell\leq n$, consider $\eta\in\Pinning_\ell$ where $\eta$ is on $S$ (i.e., for $S\subset V$, $\eta:S\rightarrow\{0,1\}$).  We define the probability distribution for $k$ levels above $\eta$ with respect to the conditional Gibbs distribution $\mu_\eta$.  Hence, for $k$ where $\ell\leq k\leq n-\ell$, define $\pi_{\eta,k}$ as follows:
\[ \mbox{for }\tau\in\Pinning_{\eta,k} \mbox{ where } \tau:S'\rightarrow\{0,1\} \mbox{ with } S\subset S', \ \ \ \ \pi_{\eta,k}(\tau) = \frac{1}{ {n-\ell \choose k}}\mu(\sigma(S')=\tau\mid\sigma(S)=\eta).
\]


For every $1\leq k\leq n$, we define the following random walks which are known as {\em Up and Down Operators}.
\begin{description}
...
\]
Since this transition probability is independent of the pair of states we denote it as:
\[ \down{k} = 1/k.
\]
\item[Up Walk:] The up-walk walk is denoted by $\up{k}$ and corresponds to the following stochastic process from 
...
\]
	\item[Up-Down Walk:] This walk corresponds to the following Markov chain from $\Pinning_k$ to $\Pinning_k$. Start from $\tau \in \Pinning_k$. In the up-step, we sample a random $(j,s_j)$ such that $\tau\cup(j,s_j) \in \Pinning_{k+1}$; the probability of picking $(j,s_j)$ is proportional to $\pi_{k+1}(\tau\cup(j,s_j))$. Then in the down step, we remove an element $(i,s_i)$ chosen uniformly at random from $\tau\cup(j,s_j)$: 
		\[ \updown{k} = \up{k}\down{k+1}.\]

	\item[Down-Up Walk:] This chain is denoted by $\downup{k}$.  From $\tau\in\Pinning_k$, we first remove a uniformly random element $(i,s_i)$ from $\tau$ and then add an element $(j,s_j)$ with probability proportional to the weight of the resulting set $\pi_k(\tau\cup(j,s_j)\setminus(i,s_i))$,
	\[ \downup{k} = \down{k}\up{k-1}.\]
\end{description}

\begin{remark}[{\bf Glauber dynamics}]\label{rem:glauber} For a spin system, the down-up chain $\downup{n-1}$  is equivalent to the Glauber dynamics.  
\end{remark}
...
Notice that the local walk $Q$ defined in \cref{sec:RW} is similar to the down-up chain.  For simplicity let us first consider the case without a pinning.  Observe that for the down-up chain starting from $(i,s_i)\in\Pinning_1$ we have the following transition probability:
\begin{align*}
\updown{1,2}((i,s_i),(j,s_j)) & = 
...
More generally, for any $1\leq k<n$, for $\eta\in\Pinning_{k-1}$, for $(i,s_i),(j,s_j)\in V\setminus S\times\{0,1\}$ where $\eta$ is an assignment on $S$, we have the following:
\begin{equation}
...
For $1\leq k\leq n$, and $\eta\in\Pinning_{k-1}$ we know that     $\gamma(Q_\eta) \geq \gamma_{k-1}$,
    and hence for any $f$,  we have that 
    \begin{equation}
        \label{eq:DDD}
    \Dirichlet_{Q_{\eta}}(f_\eta)\geq \gamma_{k-1}\Var_{\pi_{\eta,1}}(f_\eta).
    \end{equation}
...
&=
   ...
   & \mbox{by \cref{claim:AAA}} \\
    &\geq 
    \frac{k}{k+1}\gamma_{k-1} \sum_\eta \pi_{k-1}(\eta) \Var_{\pi_{\eta,1}}(f_\eta)
       & \mbox{by \cref{eq:DDD}}
       \\
      &=
...
\begin{proof}[Proof of \cref{claim:AAA}]
    As a warm-up let us consider the simpler case where $k=1$.  Recall, \cref{rem:local-downup} where we showed that for $(i,s_i),(j,s_j)\in V\times\{0,1\}$ where $i\neq j$, \[ \updown{1}((i,s_i),(j,s_j))=\frac{1}{2}Q((i,s_i),(j,s_j)).
    \]
       \begin{align*}
           ...
           \end{align*}
           where the summation is over $(i,s_i),(j,s_j)\in V\times\{0,1\}$.  Note we do not need to exclude the case $i=j$ since $f((i,s_i))=f((j,s_j))$ when $i=j$.
...
In general, for $k\geq 1$ we have the following:
      \begin{align*}
           \Dirichlet_{\updown{k}}(f)  & =
           \frac{1}{2}\sum_{\eta\in\Pinning_{k-1}}\sum_{\substack{a,b\in V\setminus S\times\{0,1\}: \\ \eta:S\rightarrow S}} \pi_k(\eta\cup a)\updown{k}(\eta\cup a,\eta\cup b)(f(\eta\cup a)- f(\eta\cup b))^2 \\
        & =
           ...
           \\
           & = \frac{k}{k+1}\sum_{\eta}\pi_{k-1}(\eta)\Dirichlet_{Q_\eta}(f_\eta).
           \end{align*}
\end{proof}
...
Our goal is to prove by induction that for all $0\leq\ell\leq n$:
\begin{equation}
...
\end{equation}
Recall $\gamma(\updown{\ell-1})=\gamma(\downup{\ell})$ was established in \cref{lem:updown-downup}.  Hence, \cref{eqn:ind-hyp-Dir-downup} is equivalent to the following statement:
\begin{equation}
...
%      = \sum_{\sigma\in\Omega} \mu(\sigma)f(\sigma)\log(f(\sigma)) - \sum_{\sigma\in\Omega}\mu(\sigma)f(\sigma)\log[\sum_{\eta\in\Omega}\mu(\eta)f(\eta)].
% \]
% where \end{definition}


% The log-Sobolev inequality is an entropy analog of the Poincare constant.
% \begin{lemma}[Log-Sobolev Inequality]
%     The LSI constant $\delta_{LSI}$ is the largest (universal constant), such that, given any function $f:\Omega\rightarrow\R_{\geq0}$,
%     \begin{equation}
%         \delta_{LSI}\cdot\mathsf{Ent}_\mu[f] \leq \Dirichlet_P(\sqrt{f}, \sqrt{f})\, .
%     \end{equation}
% \end{lemma}
% An alternative version is the modified log-Sobolev inequality.  
% \begin{lemma}[Log-Sobolev Inequality]
%     The MLSI constant $\delta_{MLSI}$ is the largest (universal constant), such that, given any function $f:\Omega\rightarrow\R_{\geq0}$,
%     \begin{equation}
%         \delta_{MLSI}\cdot\mathsf{Ent}_\mu[f] \leq \Dirichlet_P(f, \log(f))\, .
%     \end{equation}
% \end{lemma}
% Whereas LSI has a nicer form on the RHS which often makes it easier to analyze, MLSI more directly captures the decay rate of the relative entropy and hence provides a tighter bound on the mixing time.

% \subsection{Variance/Entropy Tensorization}

% To bound the convergence rate of the Glauber dynamics we
% consider the approximate tensorization of variance/entropy as introduced in~\cite{CMT}.

% We begin with the definition of approximate tensorization of variance.

% \begin{definition}[Variance Tensorization]\label{def:var-tensorization}
% A distribution $\mu$ with support $\Omega \subseteq \{\pm 1\}^V$ satisfies the approximate tensorisation of 
% Variance with constant $C>0$, denoted using the predicate $VT(C)$, if for 
% all $f:\Omega\to\mathbb{R}_{\geq 0}$ we have that
% \begin{align}\nonumber
% \mathrm{Var}(f)\leq C\cdot \sum_{v\in V}\mu\left( \mathrm{Var}_v(f)\right) \enspace.
% \end{align}
% \end{definition}
% For a vertex $x$, recall that  $\mathrm{Var}_{x}[f]=  \sum_{\sigma}\mu_{V\setminus \{x\}}(\sigma)\mathrm{Var}^{\sigma}_{x}[f_{\sigma}]$.
% % is the average (over $\mu$ on all possible pinnings on $V\setminus\{x\}$) conditional variance of $f$.  
% %
% Variance tensorization $VT(C)$  yields the following bound on the
% relaxation time and mixing time of the Glauber dynamics~\cite{CMT,Caputo-notes}:  
% \begin{equation}\label{eq:relax-VT}
%     \Trelax \leq  Cn \ \ \ \mbox{ and }
%     \Tmix \leq C'n\log(1/\mu^*),
% \end{equation}
% where $\mu^*=\min_{x\in\Omega}\mu(x)$.
...
% \begin{definition}[Entropy Tensorization]\label{def:ent-tensorization}
% A distribution $\mu$ with support $\Omega \subseteq \{\pm 1\}^V$ satisfies the approximate tensorisation of 
% Entropy with constant $C>0$, denoted using the predicate $ET(C)$, if for 
% all $f:\Omega\to\mathbb{R}_{\geq 0}$ we have that
% \begin{align}\nonumber
% \mathrm{Ent}(f)\leq C\cdot \sum_{v\in V}\mu\left( \mathrm{Ent}_v(f)\right) \enspace.
% \end{align}
% \end{definition}
% For a vertex $x$, recall that $\mathrm{Ent}_{x}[f]=  \sum_{\sigma}\mu_{V\setminus\{x\}}(\sigma)\mathrm{Ent}^{\sigma}_{x}[f_{\sigma}]$.
% % is the average (over $\mu$ on all possible pinnings on $V\setminus\{x\}$) conditional entropy of $f$. 
% %
% Entropy tensorization $ET(C)$ immediately yields the following mixing time bound for the Glauber dynamics~\cite{CMT,Caputo-notes}:
% \begin{equation}\label{eq:mix-ET}
%    \Tmix \leq  C n\log(\log(1/\mu^*)).  
% \end{equation}       
