...
Notice that the Glauber dynamics is irreducible (since all states can reach the empty set) and aperiodic (since there is a self-loop of not changing the state at $v$) and hence the chain is ergodic where the unique stationary distribution is the Gibbs distribution.
...
\begin{enumerate}
    ...
    \item Choose $X_{t+1}(v)$ from the conditional Gibbs distribution $\mu(\sigma(v)|\sigma(w)=X_{t}(w) \mbox{ for all }w\neq v\})$, i.e., fix the spin at all vertices except $v$ and resample the spin/label at $v$ conditional on the fixed configuration on the rest of the vertices.
    \end{enumerate}

    The {\em mixing time} of the Glauber dynamics, denoted as $T_{\mathrm{mix}}(\eps)$ for $\eps>0$, is the minimum number of steps $t$ such that, for the worst initial state $X_0$, the distribution of $X_t$ is within total variation distance $\leq \eps$ of the stationary distribution $\pi$.
   We will refer to $T_{\mathrm{mix}}=T_{\mathrm{mix}}(1/4)$ as the mixing time.  The choice of constant $1/4$ is somewhat arbitrary as one can ``boost'' the mixing time to obtain, for any $\eps>0$, $T_{\mathrm{mix}}(\eps)\leq T_{\mathrm{mix}}(1/4)\times\lceil\log_2(1/\eps)\rceil$.  Oftentimes, the constant $1/2e$ is used in place of $1/4$ in the definition of the mixing time and then the corresponding boosting has $\ln(1/\eps)$ in place of $\log_2(1/\eps)$, see~\cite[Section 4.5]{LevinPeresWilmer}.

   We say the mixing time is optimal if $T_{\mathrm{mix}}=O(n\log{n})$ as this matches the lower bound established by Hayes and Sinclair~\cite{HayesSinclair} for any graph of constant maximum degree $\Delta$.  The proofs of optimal mixing in these notes will yield an $O(n\log(n/\eps))$ mixing time bound for any $\eps>0$, which is better than obtained from the standard boosting technique.

In contrast to the mixing time, a weaker notion of convergence is the relaxation time.  Let $\lambda_1=1> \lambda_2\geq\lambda_3\geq \cdots \geq \lambda_N>-1$ denote the eigenvalues of the Glauber dynamics.  Note, $\lambda_1$ corresponds to the principal eigenvector which is the stationary distribution $\pi$.  Moreover, $\lambda_2<1$ since the Glauber dynamics is an ergodic Markov chain and hence the stationary distribution is unique.  In addition, $\lambda_N>-1$ since the Markov chain is aperiodic.  The {\em relaxation time} is defined as $T_{\mathrm{relax}}:=(1-\lambda_*)^{-1}$ where $\lambda_*=\max\{\lambda_2,|\lambda_N|\}$. 
The relaxation time measures the decay rate of the variance of the chain with respect to the stationary distribution, see~\cref{sec:MC} for a more detailed discussion.

The relaxation time is often the critical quantity for approximate counting algorithms in which the aim is to estimate the partition function $Z$ using random samples from the Gibbs distribution.  In particular, an optimal mixing time of $O(n\log(n/\eps))$ for the Glauber dynamics implies an $O((n/\delta)^2\log(n)\log(n/\delta))$ time algorithm for estimating the partition function $Z$ within an $(1\pm\delta)$-multiplicative factor, for any $\delta>0$, see~\cite{SVV:annealing,Huber,Kolmogorov}.
...
Throughout these notes we use the notation $\sigma(S)$ to refer to the restriction of $\sigma$ to the set $S$.
...
\]
where 
\[ \mu(\tau) = \sum_{\eta\in\Omega: \atop \tau(S)=\eta(S)} \mu(\eta).
\]
...
\end{equation}
  Moreover, Dyer, Greenhill, and Ullrich~\cite{DGU} proved that the heat-bath block dynamics is positive semidefinite (that is, all the eigenvalues of $P$ are non-negative) and the Glauber dynamics is a special case of the heat-bath block dynamics where the blocks are single vertices.
...
\mu(\tau) := \sum_{\eta\in\{0,1\}^n:\atop \tau(S)=\eta(S)} \mu(\eta).
\end{equation}
...
The authors are thankful to Zongchen Chen for many helpful conversations, and to 
Catherine Greenhill and Daniel Zhang for  many corrections and clarifications.
