...
%We present an encoding technique that maps a timeline-based planning problem to a \emph{deterministic} finite state automaton (DFA). 
...
%todo spiegare che questo rispetta la complessità migliore che abbiamo dato
...
$a_i[x_i=v_i]$ of $\E$.
%Moreover, if there is more than one constraint on $T - T'$, then $n$ will be theminimum of all the right-hand sides of these constraints.
...
On top of DBMs, we define the concept of \emph{matching structure}, 
a data structure that allows us to monitor and update the fulfillment of atomic temporal relations among terms throughout the execution of the plan. More precisely, it allows us to manipulate and reason about partially matched existential statements of which only a portion of the requests has been satisfied by the word read so far, while the rest is potentially matchable in the future. %chiarire cosa vogliamo intendere per match 

\begin{defi}[Matching Structure] %TODO NELLA DEFINIZIONE n \in Z + infinity quindi non ha senso
  ...
  \item %$D \in \Z_{+\infty}^{|V|^2}$ is a DBM indexed by terms of $V$, 
    $D$ is DBM of size $|V| \times |V|$, indexed by terms of $V$, whose
...
The set $M$ contains the terms of set $V$ correctly matched %in the event sequence
by the matching structure. We use $\overline{M} = V \setminus M$ to refer to terms yet to be matched. We say a matching structure $\M$ to be \emph{closed} if $M = V$, \emph{initial} if $M = \emptyset$, and \emph{active} if $\tokstart(a_0) \in M$ and it is not closed. The component $t$ represents the time elapsed since matching $\tokstart(a_0)$. As time progresses, we update a matching structure as follows.
...
\begin{defi}[Time shifting]
  ...
  \end{itemize}
\end{defi}
\Cref{def:time-shift} specifies how to update the entries of $D$ and how to update $t$ to the trigger occurrence of an active matching structure. The bounds between any pair of terms $T$ and $T'$, with one in $M$ while the other not, are tightened by the elapsing of time. When $T\in M$ and $T'\in\overline{M}$, $D[T,T']$ is a lower bound loosened by adding the elapsed time $\delta$. When $T\in\overline{M}$ and $T'\in M$, $D[T,T']$ is an upper bound tightened by subtracting $\delta$.
Consider the DBM in \Cref{fig:dbm} and the pair of terms $\tokstart(a_1)$ and $\tokend(a_0)$. We have $D[\tokstart(a_1),\tokend(a_0)]=-4$, implying that $\tokstart(a_1)-\tokend(a_0)\le -4$ must hold. Suppose that $\tokstart(a_1)\in M$ (it has been matched), and that $\tokend(a_0)\in\overline{M}$ (it needs to be matched). Now, in a time step, the entry in the DBM is incremented and updated to $-4+1=-3$ reflecting the fact that we now have $3$ time steps left to match $\tokend(a_0)$.
A similar analysis leads us to the conclusion that the entry $D[\tokend(a_0),\tokstart(a_1)]=14$ has to be decremented by $1$ and updated to $14-1=13$.
...
A \emph{matching function}~\cite{Gigante19} maps \emph{terms} of an existential
statement $\E$ to events within an event sequence, ensuring that all temporal
relations are satisfied. In contrast to a matching structure, which only records
the relative temporal distance between terms, a matching function associates
terms with absolute time points in an event
sequence.
% As a result, matching functions simplify considerations regarding the temporal
% distances between terms and enable comprehensive reasoning about an event
% sequence when analyzing a solution plan.
In \cite{Gigante19}, matching functions are defined in terms of \emph{rule
  graphs}, a data structure that we do not use here. Hence, we present a
reformulated definition of matching function and establish a strong
correspondence between such functions and the runs of matching structures.

\begin{defi}[Matching functions~\cite{Gigante19}(Relaxed definition)]
  \label{def:matching-function}
  %
  Let $\evseq=\seq{\event_1,\ldots,\event_n}$ be a (possibly open) event
  sequence, and let $\E\equiv \exists a_1[x_1=v_1]\ldots
  a_k[x_k=v_k]\suchdot\clause$ be one of the existential statements of a
  synchronization rule $\Rule\equiv a_0[x_0=v_0]\implies
  \E_1\lor\ldots\lor\E_m$. Let $V$ be a set of terms such that $\tokstart(a)\in
  V$ or $\tokend(a) \in V$ only if $a \in \set{a_0,\ldots, a_k}$. A
  \emph{matching function} is a function $\gamma:V\to[1,\ldots,n]$, mapping each
  term in $T\in V$ with an event $\event_{\gamma(T)}$ in $\evseq$, such that:
  \begin{enumerate}
  \item \label{def:matching-function:nodes}
        for each term $T\in V$, with $T=\tokstart(a)$ (resp., $T=\tokend(a)$),
      ...
      \item \label{def:matching-function:tokens} if both
        ...
        endpoints of the same token for $x$;
  \end{enumerate}
\end{defi}

We can now state how matching functions and runs are related.
