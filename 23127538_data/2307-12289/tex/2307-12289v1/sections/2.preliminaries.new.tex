...
our work. We begin with the general features of timeline-based planning, and then we
discuss timeline-based games. Next, we introduce the reactive synthesis problem.
Finally, we introduce the concept of \emph{difference bound matrices}
...
The first basic concept in timeline-based planning is that of \emph{state variable}.
...
A state variable, denoted by $x$, takes its values from a finite domain and represents a finite state machine with a transition function $T_x$. The behavior over time of a set of state variables $\SV$ is defined by a set of \emph{timelines}, with one timeline for each variable. We specify the value of a state variable over a time interval with \emph{tokens}.

\begin{defi}[Token]
    A token for $x$ is a tuple $\tau = (x,v,d),$ where $x$ is a state variable, $v \in V_x$, and $d \in \mathbb{N}_+$ is the duration of the token, with $d^{x=v}_{min} \leq d \leq d^{x=v}_{max}$.
\end{defi}

\begin{defi}[Timeline]
    A timeline for a state variable $x = (V_x, T_x, D_x, \gamma)$ is a finite sequence $\mathrm{T} = \langle \tau_1 \dots \tau_2 \rangle$ of tokens for $x$.
\end{defi}
In this paper, we follow the approach outlined in~\cite{GiganteMOCR20} that represents the execution of a system modeled by a set of state variables with a single word, called an \emph{event sequence}. This way, we can avoid reasoning about timelines directly.
\begin{defi}[Event sequence~\cite{GiganteMOCR20}]
  ...
  the terms, called \emph{actions}, of the forms $\tokstart(x,v)$ and
  $\tokend(x,v)$, where $x\in\SV$ and $v\in V_x$. An \emph{event sequence} over $\SV$ is a sequence
...
Given an event sequence $\evseq=\seq{\event_1,\ldots,\event_n}$ over a set of state variables $\SV$, with $\event_i=(A_i,\delta_i)$, we define $\delta(\evseq)$ as $\sum_{1<i\le n}\delta_i$, that is, $\delta(\evseq)$ is the duration of the event sequence or, equivalently, the time elapsed from the start to the end of the sequence. If we have a subsequence of $\evseq$, we use $\delta_{i,j}$ to denote the amount of time spanning that subsequence. It is worth noting that $\delta(\slice\evseq_{i,j})$ is defined as $\sum_{i<k\le j}\delta_k$.
Finally, for each $1<i\le n$ in an event sequence $\evseq=\seq{\event_1,\ldots,\event_n}$, we define $\evseq_{<i}$ as $\seq{\event_1,\ldots,\event_{i-1}}$.

In timeline-based planning, the objective is to satisfy a set of \emph{synchronization rules}, which specify the desired behavior of the system. These rules relate tokens, possibly belonging to different timelines, through temporal relations among token endpoints. Let \SV be a set of state variables and $\toknames = \set{a,b,\ldots}$ be an arbitrary set of \emph{token names}. 

\begin{defi}[Atom]
    \label{def:atom}
    An atom is a temporal relation between tokens' endpoints of the form $\production{term} \before_{l,u} \production{term}$, where $l\in\N$, $u\in\N\cup\set{+\infty}$, and a \emph{term} is either $\tokstart(a)$ or $\tokend(a)$, for some $a\in\toknames$.
\end{defi}

\begin{defi}[Synchronization rule]
    A synchronization rule \Rule can be of two forms:
\begin{equation*}\label{eq:synchronisation-rules}
  \begin{array}{rcl}
    a_0[x_0=v_0]&\implies& \E_1 \lor \E_2 \lor \dots \lor \E_k\quad \\
     \top&\implies& \E_1 \lor \E_2 \lor \dots \lor \E_k\quad
    with\\[0.4em]
    \E_j&\bydef&\exists a_1[x_1=v_1] a_2[x_2=v_2]\ldots a_n[x_n=v_n] \suchdot \clause_j \ (for \ 1 \leq j \leq k)
  \end{array}
\end{equation*}
where, for $0 \leq i\leq n$, $a_i \in \toknames$, $x_i \in \SV$, and $v_i \in V_{x_i}$, and $\clause_j$ is a conjunction of atoms.
...
\end{defi}
The terms $a_i[x_i=v_i]$ are referred to as \emph{quantifiers}, and $a_0[v_0=x_0]$ is called the \emph{trigger}. The disjuncts in the body are called \emph{existential statements}.
A token $\tau = (x, v, d)$ \emph{satisfies} a quantifier $a_i[x_i=v_i]$ if $x = x_i$ and $v = v_i$. The semantics of a synchronization rule \Rule is that for every token satisfying the trigger, at least one of the existential statements must be satisfied as well. Each existential statement $\E_j$ requires the existence of tokens that satisfy the quantifiers in its prefix and the clause $\clause_j$. A token that satisfies the trigger of a rule is said to \emph{trigger} that rule. The trigger of a rule can be empty ($\top$). In such a case, the rule is referred to as \emph{triggerless} and requires the satisfaction of its body without any precondition.

Let $a$ and $b$ be token names. Examples of atoms include $\tokstart(a) \before_{3,7} \tokend(b)$ and $\tokstart(a) \before_{0,+\infty} \tokstart(b)$. In this context, $a$ refers to a specific token, which is a pair of $\tokstart(x,v)$ and $\tokend(x,v)$ actions in an event sequence, and $\tokstart(a)$ and $\tokend(a)$ refer to its endpoints. An atom of the form $\tokstart(a) \before_{l,u} \tokend(b)$ constrains token $a$ to start before the end of token $b$, with the distance between the two endpoints being at least $l$ and at most $u$.
Here are some examples of synchronization rules. Note that the relations $=$ and $\before$ are syntactic sugar for $\before_{0,0}$ and $\before_{0,+\infty}$ respectively:
\begin{align*}
...
\emph{Timeline-based planning problems} are formally defined as follows.
...
We are now ready to introduce the notion of \emph{timeline-based game}, that subsumes the notion of \emph{timeline-based planning with
uncertainty}~\cite{CialdeaMayerOU16}.
...
\begin{defi}
  Given a set of state variables $\SV$, an \emph{action} is a term of the form
  $\tokstart(x,v)$ or $\tokend(x,v)$, where $x\in\SV$ and $v\in V_x$.
\end{defi}

Actions of the form $\tokstart(x,v)$ are \emph{starting} actions, and those of
the form $\tokend(x,v)$ are \emph{ending} actions. We denote as $\actions_\SV$
the set of all the actions definable over a set of state variables $\SV$. We
partition all the available actions into those that are playable by either of
the two players.\fitpar
...
Hence, players can start tokens for owned variables and end them for values
which they control. Let $d=\max(L, U)+1$, where $L$ and $U$ are the maximum
lower and (finite) upper bounds appearing in any rule of $G$. Actions combine
into \emph{moves} starting (resp., ending) multiple tokens simultaneously. 
...
\begin{defi}[Outcome of rounds]
  ...
  $\event_n=(A_n,\delta_n)$ ($\event_n=(\emptyset,0)$ if $\evseq=\epsilon$).
  Let $\round=(\move_C,\move_E)$ be a round, $A_E$ and $A_C$ 
  be the sets of actions of the two moves ($A_C$ is empty if $\move_C$ is a 
  $\wait$ move), and $\delta_E$ and $\delta_C$ be the time increments 
  of the moves, with $\delta_C=1$ (resp., $\delta_E=1$) for $\play(A_C)$  
  (resp., $\play(A_E)$) moves.
