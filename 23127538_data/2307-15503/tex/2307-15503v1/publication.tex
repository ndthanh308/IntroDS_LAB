...
\begin{document}
	...
	\author{Joshua Stock\inst{1} \and Oliver Hauke\inst{2} \and Julius Wei\ss{}mann\inst{2} \and Hannes Federrath\inst{1}} % fuer Entwurf und spaetere Veroeffentlichung
	...
	\begin{abstract} 
		...
		At the same time, its utilization can safeguard the privacy of data holders, thus facilitating access to a broader range of data and ultimately enhancing official statistics.
	...
	The aim of national statistical offices (NSOs) 
	%\todo{Destatis erwähnen? -- JS: Vielleicht für die Reviewphase noch nicht, da das im weiteren Sinne auf die Identität von zwei der Autoren hinweist? Aber in einer nicht-anonymen Version gerne, dann steht Destatis ohnehin oben in eurer affiliation.}
	%such as the Federal Statistical Office of Germany (Destatis)
	is to develop, produce and disseminate high-quality official statistics that can be considered a reliable portrayal of reality~\cite{yung2022quality}.
	...
	%To supply the increasing demand of both society and policymakers for more timely and comprehensive data, 
	...
	%web scraped texts\footnote{\url{https://link.springer.com/article/10.1007/s11943-021-00280-5}} 
	%or satellite images\footnote{\url{https://www.destatis.de/EN/Service/EXSTAT/Datensaetze/satellite-data.html}}.
	...
	\url{https://digital-strategy.ec.europa.eu/en/policies/private-sector-data-sharing}, accessed on July 17, 2023} 
	% \href{https://ec.europa.eu/eurostat/web/european-statistical-system/-/access-to-privately-held-data}{ESS 2017. Position paper on access to privately held data which are of public interest.}
	or data from certain official entities, remains largely untapped.
	...
	Cooperation with potential data donors %, particularly private companies, 
	faces restrictions due to concerns about privacy, confidentiality, or disclosing individual business interests.
	...
	%\todo{@DESTATIS: Den zweiten Teil des Satzes habe ich etwas umformuliert, bitte prüfen (JS). Passt so nicht (OH). Das neue Framework definiert eher, was "quality requirements" fuer ML in NSOs bedeutet}.
	%
	Within official statistics, ML tools have proven effective in processing new data sources, such as text and images, or enabling the automation of statistical production tasks, including classifying information or predicting not (yet) available data. % ...using regression
	...
	to regulatory or strategic % (politically or business interests)
	considerations, but still aim to analyze or disseminate global insights in their field of application, NSOs can offer trustworthy solutions utilizing FL.
	...
	%by integrating new information into statistical production and improving existing ML applications.
	%by broadening the scope of features, harnessing the potential of predictive modeling, improving machine learning (ML) applications such as classification and automation based on widely accepted industry-standard classifications like NACE/COICOP. FL also facilitates imputation and nowcasting techniques, paves the way for innovative data publication methods, enables the evaluation of emerging technologies, and supports the ongoing development of NSOs' analytical infrastructure.
	%

	This work investigates the methodological and practical potential of FL for NSOs to leverage new data sources. 
	The primary contribution of this work involves a comprehensive investigation through three simulations that address current data needs representative for official statistics. 
	%
	In the first simulation related to \textbf{health}, individual healthcare costs are predicted utilizing tools for regression.
	%
	...
	%These simulations are based on a medical insurance data set regarding health, a mobile radio coverage data set regarding mobility and a fine dust pollution data set – all of which are from domains close to official statistics (health, mobility and sustainability). 
	...
	When training data for a ML model is distributed among multiple parties, the data traditionally needs to be combined on a central server prior to applying ML training.
	...
	%This is especially feasible if the records are very similar to each other, as shown in~\cite{shokri2017membership}.
	\emph{Membership inference}~\cite{shokri2017membership}
	% attack, which has evolved to one of the most important ML privacy attacks.
	%This is because membership inference is a simple attack 
	...
	%Due to its simplicity, it is often used as a benchmark for general privacy leakage.
	...
	%\todo{Gibt es Angriffe die durch FL anstatt ML schwerer gehen? Der Abschnitt bringt fuer mich nicht deutlich genug rueber, dass FL Vorteile in Privacy bietet. (OH)}	
	...
	% nonetheless we want to stress the importance of awareness towards these privacy issues, which persist in many ML (including FL) scenarios.
	%In a FL process, the least one can do is communicating potential privacy risks to clients.
	...
	%, although most of them are not yet computationally feasible enough for practical applications~\cite{yin2021comprehensive}.
	...
	%For data processing, we use scikit-learn\footnote{scikit-learn, Machine Learning in Python \url{https://scikit-learn.org/}, accessed on July 17, 2023} 
	%%(for data preprocessing, linear regression and random forests) for  the centralized benchmark models.
	%and PyCaret\footnote{PyCaret \url{https://pycaret.org/}, accessed on July 17, 2023} for automizing benchmark experiments in the centralized settings.  
	...
	%\todo{OH: Ist es nötig, dass wir im cross-silo setting sind? Kann das ggf. raus? -- JS: Kann ich nicht gut einschätzen, aber ich kann mir nicht vorstellen, dass es ein großer Kritikpunkt wäre, es nicht zu erwähnen. Zumal es durch die Use Cases auch offensichtlich wird, dass wir nur cross-silo FL untersuchen.}
	% \todo{JW: Für den Leser könnte es an dieser Stelle interessant sein zu erwähnen, dass unsere Experimente alle "cross-silo" sind}
	...
	%The simulated use cases are in domains which are close to official statistics.
	...
	\paragraph{Data set}
	% \todo[color=green!40]{specified}
	The first data set links medical insurance premium \emph{charges} 
to related individual attributes\footnote{US health insurance dataset \url{https://www.kaggle.com/datasets/teertha/ushealthinsurancedataset}, accessed on July 17, 2023}. 
	...
	%\todo{OH: Update und kurze Aussage zu Datenalyse/Link zu Repo mit genauerer Analyse}

	\paragraph{Data preprocessing}
	%\todo{Kuerzen: Standard preprocessing: convert binaries to numeric, scale numeric, one-hot encode factors}
	...
	In the centralized benchmarks, we use the attribute \emph{region} one-hot-encoded.
	...
	\paragraph{Setup}
        ...
        A basic fully connected neural network architecture, that takes five input features, was utilized. The network consists of three hidden layers with 40, 40, and 20 units in each respective layer. Following each layer, a Rectified Linear Unit (ReLU) activation function is applied. The final output layer comprises a single neuron. To optimize the network, the Adam optimizer with a learning rate of 0.05 was employed.
        In the federated task, we utilized the same initial model but integrated FedAdam for server updates. This decision was based on previous research~\cite{reddi2020adaptive},  which emphasized the benefits of adaptive server optimization techniques for achieving improved convergence.
		%\todo{@JW: Konvergenz und Stabilität auch. Checke nochmal ob es gegen Heterogenität hilft. AW: Ja Heterogenität ist richtig, allerdings würde ich es an dieser Stelle nicht erwähnen, weil man sich meiner Meinung nach auch die Heterogenität unserer Daten aufführen müsste, damit es sinnhaft erscheint. Daher würde ich es ganz allgemin halten mit Konvergenz...}
        
        In the centralized approach, we allocate a training budget of 100 epochs. In contrast, the federated approach incorporates 50 rounds of communication between the client and server during training. Each round involves clients individually training the model for 50 epochs. To evaluate our results in both scenarios, we employ 5-fold cross-validation for testing. To track the running training, $10\%$ evaluation data is used by each client in the FL setting and $20\%$ is used in the centralized scenario. It is neglected in calculating the final test performance.
        The remaining shallow learning models underwent hyperparameter optimization using a random search approach with a budget of 100 iterations. Similar to the neural networks, we conducted evaluation using 5-fold cross-validation.
    ...
    \begin{table}[htbp]
      ...
      \begin{tabular}{lcc} % Added vertical line after the first column
        ...
        %\multirow{2}{*}{Model} & \multicolumn{3}{c}{Statistics} \\
        %\cmidrule{2-4} % Partial horizontal line
        \multirow{1}{3.1cm}{\textbf{Model}} & 
		\multirow{1}{2.2cm}{\centering\textbf{\rsquared ($\pm$ std)}} & 
		\multirow{1}{*}{\centering\textbf{Rel. loss (\%)}} \\
        \midrule
        neural network    & $\textbf{81.5 (4.01)}$  & 3.5 \\
        neural network (federated)   & $\textbf{78.4 (3.13)}$  & 7.2 \\
        %\specialrule{0.02em}{0em}{0em} % Thin line after the third row
        random forest    & $\textbf{84.5 (4.73)}$ & 0.0\\
        %gbr   & 0.841 (3.62) & x\%   \\
        %cat   & 84.5 (3.71) & x\%   \\
    ...
    \end{table}
        ...
        \paragraph{Results}
		%\todo{OH: Ich dachte wir haetten uns auf Abweichung in Prozentpunkten festgelegt, aber ist jetzt egal. Warten wir den Review ab.}
        We conducted a performance comparison of the models based on their 5-fold cross-validation \rsquared{} scores and considered their standard deviation (see \autoref{tab:med-perf}). The random forest model achieved the highest performance with an \rsquared{} of 84.5~\%, closely followed by XGBoost and Decision Tree, which scored 0.2 and 0.5 percentage points lower, respectively.

        The neural network model achieved an \rsquared{} of 81.5~\%, indicating a performance 3.5~\% worse than the best model. However, it still provides a reasonable result compared to K-Nearest Neighbors~(KNN) and Linear Regression, which obtained significantly lower \rsquared{} scores of 12~\% and 13.8~\%, respectively.
        
        The Federated neural network demonstrated an \rsquared{} of 78.4~\%, slightly lower than the centralized neural network but 7.2~\% worse than the random forest model. Notably, the Federated neural network exhibited a lower standard deviation of 3.99 compared to the centralized neural network (4.92) and also outperformed the random forest model (4.73) in this regard.
	...
	In our second use case, we additionally provide a comparison between centralized and FL models (without personalization) and make the developed code and methods accessible. 
	...
	The threshold for each class are chosen in a way such that the samples of the whole data set are distributed evenly among the three classes.
	...
	We use one-hot encoding for the wind direction attribute.
	...
	The total training budget we have allocated is 10 epochs for centralized learning and 200 epochs for FL.
	%\todo{Wieviele rounds oder client epochs?} AW (JS): Nur eine. Weil die Datensätze so groß waren, haben wir sie nicht auch noch dupliziert für die Clients.´
	...
	\begin{table}[]
		...
		\begin{tabular}{lcccc} 
			...
			\midrule
			%neural network & \textbf{72.4}\% (0.68\%)   & \textbf{72.8\%} (11.9\%) & \textbf{72.3\%} (11.2\%)    \\ 
			%neural network*   & \textbf{68.0\%} (3.35\%) & \textbf{67.9\%} (14.8\%) & \textbf{68.0\%} (14.1\%)    \\
			neural network & \textbf{72.4}\% (4.92)   & \textbf{72.8\%} (8.66) & \textbf{72.3\%} (8.10) & 0.0    \\ 
			neural network (fed.)   & \textbf{68.0\%} (7.59) & \textbf{67.9\%} (10.05) & \textbf{68.0\%} (9.59) & 5.9-6.7  \\
			\bottomrule
		\addlinespace[4pt]
		\end{tabular}
	...
	\begin{table}
		...
		\begin{tabular}{lccc}
			...
			\midrule
			low & \textbf{114\,450}  & \num{23712} & \num{5769} \\ 
			medium & \num{23635} & \textbf{80\,718} & \num{33754} \\
			high & \num{1944} & \num{24629} & \textbf{111\,581} \\
			\bottomrule
			\addlinespace[4pt]
		\end{tabular}
	...
	%\todo{@JS: Abgleichen mit Satz aus 1. Use Case}
	%Although this 4.4 percentage points decrease may be significant in some scenarios, we stress that this was only a simulation:
	...
	%\todo{pruefen: ist das konsistent zum Anfang von 3.2?}
	%\citeauthor{hu2018federated} have also worked with this data set, introducing \enquote{Federated Region-Learning}~\cite{hu2018federated}.
	...
	%Different from standard FL, the result of this personalized FL strategy  consists of multiple specialized models, i.e., one model per predefined region of the city of Beijing, capturing the area of multiple meteorological stations.
	%For the grouping of regions, see \autoref{fig:pl-beijing-stations}.
	
	Furthermore, \citeauthor{hu2018federated} have not provided sufficient information to properly retrace their experiments.
	...
	%\todo{D.h. LTE Qualität wird genutzt um den Bewegungsradius zu schaetzen? Da würde ich auch einen schwachen Zusammenhang erwarten. Warum ist die Home Base (PLZ) kein Feature? (OH)}
...
%	depicted in \autoref{tab:lte-dataset}\todo{JS: Tabelle in Text nehmen, jeweils mean variance pro Variable erwähnen.}.
%	
%	\begin{table}
%		\small
%		\centering
%		\begin{tabular}{ll}
%			\toprule
%			\textbf{Feature}           & \textbf{Example value} \\
%			\midrule
%			User ID           & 360a02ef...  \\
%			radius of action in meters*          & 2300     \\
%			calendar week       & 20   \\
%			day of the week  & 3     \\
%			month & 5     \\
%			weekend       & True \\
%			number of data records* & 30     \\
%			variance LTE-RSRQ* & 0.84     \\
%			mean LTE-RSRQ* & -11.3 \\
%			variance LTE-RSRP* & 12.3     \\
%			mean LTE-RSRP* & -90.8 \\
%			variance LTE-RSSNR* & 7.54     \\
%			mean LTE-RSSNR* & 6.25 \\
%			variance LTE-RSSI* & 12.4     \\
%			mean LTE-RSSI* & -84.5 \\
%			WiFi connection record share* & 0.1 \\
%			\bottomrule
%			\addlinespace[4pt]
%		\end{tabular}
%		\caption{Aggregated features of the LTE data set by the company umlaut SE. The \emph{user} attribute is not used as training data, it is merely used to aggregate the features per user. Features marked with * are computed over the aggregated data for one particular day.}
%		\label{tab:lte-dataset}
%	\end{table}
	...
	%todo{JW: würde ich deutlicher schreiben. Für mich (ich kenne das Netzwerk nicht) klingt das wie "?in:? | h1:28 | h2:14 | out:1 + RelU" => Interessant wäre zu wissen wie der input layer ausschaut, wo RelU überall ist und ob der final output Layer auch ne Aktivierungsfunktion hat? Bzw. ist y standartisiert?}
	%AW (JS): Input ergibt sich aus Beschreibung der Features. Die Architektur ist hier detaillierter beschrieben als in den anderen Use Cases, ich würde nicht noch weiter in die Tiefe gehen. y ist nicht standardisiert, soweit ich weiß.
	...
	\paragraph{Discussion} 
	%\todo{JW: Wie schon im Meeting von Oli angesprochen braucht es hier einen Referenzwert (NIR). AW (JS): Den Wert haben wir leider nicht.}
	...
	%In the real world, FL can often be an enabling technology, allowing the use of privacy preserving machine learning algorithms where laws and other concerns might make their application (or the centralized merging of training data) otherwise impossible.
	...
	% JS: hier vielleicht eher out of scope: Automatisierte HPO braucht offene Schnittstellen, vielleicht nicht gut aus Privacy Perspektive
	...
	%(simulation 3 was performed on a machine of the company umlaut).
	...
	%Possible mitigation strategies might be found in privacy-preserving machine learning (PPML) techniques~\cite{yin2021comprehensive}.
	
	\section{Implications for Official Statistics}
	\label{sec:implications-off-statistics}
	
	%\todo{Bitte pruefen. Ggf. kuerzen? Anmerkung JW: Ja kürzen finde ich insbesondere für die späteren Abschnitte sinnvoll}
	% intro
	...
	%
	%\todo{JW: ein einleitender Satz könnte hilfreich sein was advantages in der amtlichen Statistik sind: In official statistics, unbiased models with high accuracy are of particular importance because they can influence policy and the economy. Inhalte aus Yun+22 (OH)}
	%Quality of ML in official statistics can be measured by the framework QF4SA \citeauthor{yung2022quality}, 2022 with five quality dimensions: accuracy, explainabilitiy, reproducibility, timeliness and cost effectiveness.
	...
	% also best case 'efficientyly' but not nessesary
	...
	%The successful implementation of FL requires establishing a framework that supports the coordination, knowledge-sharing, and collaborative efforts of all involved partners. 
	...
	%This collaborative approach ensures that the benefits of FL are maximized and that all parties can actively participate.
