...
\hspace{\parindent}Throughout, let $\ind(\mathcal{A})$ denote the indicator function of an event $\mathcal{A}$, i.e., $\ind(\mathcal{A})=1$ if $\mathcal{A}$ is satisfied and $\ind(\mathcal{A})=0$ otherwise. Let $\R$ be the set of real numbers. Let $V_1 \indep V_2 \cond V_3$ denote that $V_1$ and $V_2$ are conditionally independent given $V_3$. Conversely, we use $V_1 \nindep V_2 \cond V_3$ to denote that $V_1$ and $V_2$ are conditionally dependent given $V_3$. Let $0_{p \times d}$, $1_{p \times d}$,  and $I_{p \times p}$ denote the $(p \times d)$-dimensional zero matrix, $(p \times d)$-dimensional matrix with ones,  and $(p \times p)$-dimensional identity matrix, respectively. 
...
\tau_{t}^* A_t
+
\potY{t}{0}$ where the ATT is represented as $\tau_t^* = \potY{t}{1}-\potY{t}{0}$ for $t=T_0+1,\ldots,T$; note that $\potY{t}{1}-\potY{t}{0}$ is non-random under model \eqref{eq-IFEM}. In addition, if there were a donor whose factor loading matched that of the treated unit, i.e., $\bm{\mu}_i=\bm{\mu}_0$ for some $i=1,\ldots,N$, then $W_{it}$ would be unbiased for $\potY{t}{0}$ and, therefore, $\potY{t}{1} - W_{it}$ would be unbiased for the ATT. This suggests that confounding bias of the treatment effect on the treated unit's outcome reflects the extent to which donors' factor loadings differ from the treated unit’s.
...
t=1,\ldots,T_0 \ .
\end{align}
A reasonable candidate for $\bZ_{t}$ maybe the outcome of units excluded from the donor pool; see \citet{Shi2023SC} for alternative choices of proxies. Then, under Assumptions \ref{assumption:consistency} and \ref{assumption:noitf}, the IFEM \eqref{eq-IFEM}, condition \eqref{eq-ExistSC}, and the existence of proxies satisfying \eqref{eq-Shi-proxy}, the synthetic control weights $\bgamma^\dagger$ in \eqref{eq-ExistSC} satisfy $\EXP ( Y_t - \bW_{ t}\T \bgamma^\dagger \cond \bZ_{ t} ) = 0$ and $ \EXP \{ \Psi_{\PI}(\bO_t \con \bgamma^\dagger) \} = 0$ for $t=1,\ldots,T_0$  where $\Psi_{\PI}(\bO_t \con \bgamma) = \bg  ( \bZ_{ t} ) \big( Y_t - \bW_{ t}\T \bgamma \big)$; here, $\bg$ is a user-specified function of $\bZ_{t}$ with $\text{dim}(\bg) \geq d$. Based on this second result, one can estimate the synthetic control weights as the solution to the generalized method of moments (GMM) \citep{GMM1982}, i.e., $\widehat{\bgamma}_{\PI} 
...
\widehat{\Omega}
\big\{ \Psi_{\PI}(\bO_t \con \bgamma) \big\} $ where $\widehat{\Omega}$ is a user-specified symmetric and positive-definite weight matrix. Importantly, in contrast with the OLS-based estimator $\widehat{\bgamma}_{\text{PLS}}$ in \eqref{eq-OLS}, the proximal estimator $\widehat{\bgamma}_{\PI}$ is consistent for $\bgamma^\dagger$. Under certain regularity conditions, \citet{Shi2023SC} established that the resulting GMM estimator of the ATT is consistent and asymptotically normal. For instance, in the special case of constant ATT, i.e., $\tau_t^* = \tau^*$ for all $t=T_0+1,\ldots,T$, the estimator $T_1^{-1} \sum_{t=T_0+1}^{T} (Y_t - \bW_{ t}\T \widehat{\bgamma}_{\PI})$ is consistent for $\tau^*$; see Section 3.2 of \citet{Shi2023SC} for details.
...
\overline{e}_{t}
\ , \quad
\EXP \big\{ \overline{e}_{t} \cond \potY{t}{0} \big\} = 0 \text{ almost surely for  all $t=1,\ldots,T$} \ .
\end{align}
...
\subsection{A Single Proxy Generative Model} \label{sec:IFEM discussion}
...
For all $t=1,\ldots,T$, there exists a function $h^*: \R^N \rightarrow \R$ that satisfies $\potY{t}{0} = f_0(\blambda_t , e_{0t}) = \EXP \big\{
h^*(\bW_{ t})
\cond
\blambda_t, e_{0t}
\big\}$ almost surely.
...
Under Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy}, and \ref{assumption:SC}, the synthetic control bridge function $h^*$ satisfy $\EXP \big\{ h^* ( \bW_{ t} ) \cond Y_t \big\} = Y_t $ almost surely for $t=1,\ldots,T_0$.
\end{theorem}
...
Under Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy}, and \ref{assumption:SC}, we have that $\EXP \big\{ \potY{t}{0} \big\} = \EXP \big\{ h^*(\bW_{ t}) \big\}$ for any $t = 1,\ldots,T$. Additionally, the ATT is identified as $	\tau_t^*
=
\EXP
\big\{
Y_t - h^*(\bW_{ t})
...
\sum_{t=1}^{T_0} 
\Psi_{\pre} (\bO_t \con \Beta , \bgamma) $ is the empirical mean of the estimating function over the pre-treatment periods evaluated at $(\Beta, \bgamma)$. Also, $\widehat{\Omega}_{\pre}\in \R^{(2d+p)\times(2d+p)}$  is a user-specified symmetric positive definite block-diagonal matrix as $\widehat{\Omega}_{\pre} =  \text{diag}(I_{d \times d} , \widehat{\Omega}_{\bg} )$, which can simply be set to the identity matrix. Since the first block of $\widehat{\Omega}_{\pre}$ is the identity matrix, 
...
=
\tau ( t \con \bbeta^* )$ for $t=1,\ldots,T$. This parametrization allows us to pool information over time in the post-treatment period to infer $\bbeta^*$ and the ATT. Possible forms for $\tau(t \con \bbeta)$ are given below:
...
This includes a quadratic model $\tau(t \con \bbeta) = \beta_0 + \beta_1 (t- T_0)_+/T_1 + \beta_2 (t - T_0)_+^2/T_1$, or an exponentially time-varying treatment model $\tau(t \con \bbeta) = \exp \big\{ \beta_0 + \beta_1 (t-T_0)_+/T_1 \big\}$, or a model spanned by nonlinear basis functions, e.g., $\tau(t \con \bbeta) = \mathcal{B}_b\T(t) \bm{\beta} $ where $\mathcal{B}_b\T(t)$ is the $b$-dimensional cubic B-spline function; this model is appropriate if the treatment yields a nonlinear effect over time.
\end{example}
...
Let $\epsilon_t = Y_t - \bW_{ t} \T \bgamma_0^* - \tau(t \con \bbeta^*)$ for $t=1,\ldots,T$. Then, the error process $\big\{ \epsilon_t \cond t=1,\ldots,T \big\}$ is weakly dependent, i.e., $\text{corr}(\epsilon_{t}, \epsilon_{t+t'})$ converges to 0 as $t' \rightarrow \pm \infty$.
\end{assumption}
...
\end{bmatrix} 
\ , 
\quad t=1,\ldots,T_0+1 \ .
\end{align*}
At the minimum-norm synthetic control weights $\bgamma_0^*$, the redefined estimating function is mean-zero for $t = 1,\ldots, {T_0+1}$ under $H_{0,T_0+1}$. Therefore, a GMM estimator $\widehat{\bgamma}(\xi_{0,T_0+1})$ can be obtained by solving the following minimization problem, which is similar to \eqref{eq-GMM-Formula}: 
\begin{align*}
...
With these generated variables, $\potY{t}{0}$ and $W_{it}$ at $t=1,\ldots,T$ were generated as $\potY{t}{0} = \bm{\mu}_0\T \bm{\lambda}_t + e_{0t}$ and $W_{it} = \bm{\mu}_i\T \bm{\lambda}_t + e_{it}$ for $i=1,\ldots,N$, respectively. Note that the latter eight untreated units $(W_{9t},\cdots,W_{16t})\T$ were independent of $\potY{t}{0}$, which resulted in multiple synthetic control bridge functions satisfying Assumption \ref{assumption:SC}. The potential outcomes under treatment at $t=1,\ldots,T$ were generated as $\potY{t}{1} = \potY{t}{0} + 3 A_t + \epsilon_{t}$ where $\epsilon_t$ were generated independently across time periods from $N(0,0.25)$. Note that the ATT was $\tau_t^* = 3$ for $t=T_0+1,\ldots,T$.
...
\hline
\multicolumn{1}{|c|}{\multirow{3}{*}{$\blambda_t$}} &  \multicolumn{1}{c|}{\multirow{3}{*}{Statistics}} & \multicolumn{12}{c|}{Estimators and $T_0$} \\ \cline{3-14} 

\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{}  & \multicolumn{2}{c|}{OLS-NoReg} & \multicolumn{2}{c|}{OLS-Standard} & \multicolumn{2}{c|}{ASC}       & \multicolumn{2}{c|}{SCPI}      & \multicolumn{2}{c|}{SPSC-NoDT} & \multicolumn{2}{c|}{SPSC-DT}   \\ \cline{3-14} 
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{}  & \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500} & \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}  \\ \hline

\multicolumn{1}{|c|}{\multirow{7}{*}{$\!\!\!\begin{array}{c}\text{No trend}\end{array}\!\!\!$}} & \multicolumn{1}{c|}{Bias $(\times10)$} & \multicolumn{1}{c|}{0.02} & {-0.01} & \multicolumn{1}{c|}{0.04} & {0.00} & \multicolumn{1}{c|}{0.03} & {-0.01} & \multicolumn{1}{c|}{0.03} & {-0.01} & \multicolumn{1}{c|}{0.03} & {-0.01} & \multicolumn{1}{c|}{0.03} & \multicolumn{1}{c|}{-0.01}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{ASE $(\times10)$} & \multicolumn{1}{c|}{0.89} & {0.37} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.84} & {0.37} & \multicolumn{1}{c|}{0.84} & \multicolumn{1}{c|}{0.37}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{BSE $(\times10)$} & \multicolumn{1}{c|}{0.97} & {0.38} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.84} & {0.37} & \multicolumn{1}{c|}{0.84} & \multicolumn{1}{c|}{0.37}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{ESE $(\times10)$} & \multicolumn{1}{c|}{0.92} & {0.37} & \multicolumn{1}{c|}{1.25} & {0.51} & \multicolumn{1}{c|}{0.90} & {0.39} & \multicolumn{1}{c|}{0.92} & {0.39} & \multicolumn{1}{c|}{0.89} & {0.37} & \multicolumn{1}{c|}{0.89} & \multicolumn{1}{c|}{0.37}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{MSE $(\times100)$} & \multicolumn{1}{c|}{0.85} & {0.14} & \multicolumn{1}{c|}{1.56} & {0.26} & \multicolumn{1}{c|}{0.82} & {0.15} & \multicolumn{1}{c|}{0.84} & {0.15} & \multicolumn{1}{c|}{0.79} & {0.14} & \multicolumn{1}{c|}{0.79} & \multicolumn{1}{c|}{0.14}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Coverage (ASE)} & \multicolumn{1}{c|}{0.95} & {0.96} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.93} & {0.96} & \multicolumn{1}{c|}{0.93} & \multicolumn{1}{c|}{0.96}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Coverage (BSE)} & \multicolumn{1}{c|}{0.96} & {0.96} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.93} & {0.96} & \multicolumn{1}{c|}{0.93} & \multicolumn{1}{c|}{0.96}\\ \hline
\multicolumn{1}{|c|}{\multirow{7}{*}{$\!\!\!\begin{array}{c}\text{Linear trend}\end{array}\!\!\!$}} & \multicolumn{1}{c|}{Bias $(\times10)$} & \multicolumn{1}{c|}{1.30} & {1.37} & \multicolumn{1}{c|}{10.70} & {10.51} & \multicolumn{1}{c|}{8.26} & {5.70} & \multicolumn{1}{c|}{11.93} & {11.85} & \multicolumn{1}{c|}{-1.42} & {-1.61} & \multicolumn{1}{c|}{0.07} & \multicolumn{1}{c|}{0.08}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{ASE $(\times10)$} & \multicolumn{1}{c|}{1.76} & {0.79} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{1.85} & {0.84} & \multicolumn{1}{c|}{1.79} & \multicolumn{1}{c|}{0.81}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{BSE $(\times10)$} & \multicolumn{1}{c|}{2.02} & {0.82} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{2.24} & {0.99} & \multicolumn{1}{c|}{1.94} & \multicolumn{1}{c|}{0.86}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{ESE $(\times10)$} & \multicolumn{1}{c|}{1.82} & {0.77} & \multicolumn{1}{c|}{1.16} & {0.41} & \multicolumn{1}{c|}{1.70} & {0.95} & \multicolumn{1}{c|}{0.96} & {0.43} & \multicolumn{1}{c|}{2.08} & {0.97} & \multicolumn{1}{c|}{1.94} & \multicolumn{1}{c|}{0.85}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{MSE $(\times100)$} & \multicolumn{1}{c|}{5.01} & {2.45} & \multicolumn{1}{c|}{115.72} & {110.54} & \multicolumn{1}{c|}{71.16} & {33.37} & \multicolumn{1}{c|}{143.18} & {140.58} & \multicolumn{1}{c|}{6.36} & {3.54} & \multicolumn{1}{c|}{3.75} & \multicolumn{1}{c|}{0.72}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Coverage (ASE)} & \multicolumn{1}{c|}{0.87} & {0.58} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.84} & {0.49} & \multicolumn{1}{c|}{0.93} & \multicolumn{1}{c|}{0.94}\\ \cline{2-14}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Coverage (BSE)} & \multicolumn{1}{c|}{0.93} & {0.61} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{-} & {-} & \multicolumn{1}{c|}{0.91} & {0.60} & \multicolumn{1}{c|}{0.94} & \multicolumn{1}{c|}{0.96}\\ \hline
...
\multicolumn{8}{c|}{Estimators and $T_0$}
\\ \cline{3-10} 

\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & \multicolumn{2}{c|}{ASC}       & \multicolumn{2}{c|}{SCPI}      & \multicolumn{2}{c|}{SPSC-NoDT} & \multicolumn{2}{c|}{SPSC-DT}   \\ \cline{3-10} 


\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{}   & \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500} & \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500}& \multicolumn{1}{c|}{100} & \multicolumn{1}{c|}{500} \\ \hline


\multicolumn{1}{|c|}{\multirow{3}{*}{$\!\!\!\begin{array}{c}\text{No trend}\end{array}\!\!\!$}} & \multicolumn{1}{c|}{Independent errors} & \multicolumn{1}{c|}{0.933} & \multicolumn{1}{c|}{0.925} & \multicolumn{1}{c|}{0.935} & \multicolumn{1}{c|}{0.912} & \multicolumn{1}{c|}{0.959} & \multicolumn{1}{c|}{0.948} & \multicolumn{1}{c|}{0.961} & \multicolumn{1}{c|}{0.948}\\ \cline{2-10}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Correlated errors} & \multicolumn{1}{c|}{0.904} & \multicolumn{1}{c|}{0.906} & \multicolumn{1}{c|}{0.925} & \multicolumn{1}{c|}{0.913} & \multicolumn{1}{c|}{0.962} & \multicolumn{1}{c|}{0.952} & \multicolumn{1}{c|}{0.963} & \multicolumn{1}{c|}{0.953}\\ \cline{2-10}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{No $Y$ error} & \multicolumn{1}{c|}{0.922} & \multicolumn{1}{c|}{0.910} & \multicolumn{1}{c|}{0.938} & \multicolumn{1}{c|}{0.925} & \multicolumn{1}{c|}{0.964} & \multicolumn{1}{c|}{0.951} & \multicolumn{1}{c|}{0.964} & \multicolumn{1}{c|}{0.953}\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{$\!\!\!\begin{array}{c}\text{Linear trend}\end{array}\!\!\!$}} & \multicolumn{1}{c|}{Independent errors} & \multicolumn{1}{c|}{0.795} & \multicolumn{1}{c|}{0.846} & \multicolumn{1}{c|}{0.938} & \multicolumn{1}{c|}{0.889} & \multicolumn{1}{c|}{0.959} & \multicolumn{1}{c|}{0.944} & \multicolumn{1}{c|}{0.962} & \multicolumn{1}{c|}{0.949}\\ \cline{2-10}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{Correlated errors} & \multicolumn{1}{c|}{0.844} & \multicolumn{1}{c|}{0.883} & \multicolumn{1}{c|}{0.907} & \multicolumn{1}{c|}{0.843} & \multicolumn{1}{c|}{0.957} & \multicolumn{1}{c|}{0.944} & \multicolumn{1}{c|}{0.967} & \multicolumn{1}{c|}{0.948}\\ \cline{2-10}
\multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{No $Y$ error} & \multicolumn{1}{c|}{0.728} & \multicolumn{1}{c|}{0.808} & \multicolumn{1}{c|}{0.920} & \multicolumn{1}{c|}{0.852} & \multicolumn{1}{c|}{0.957} & \multicolumn{1}{c|}{0.953} & \multicolumn{1}{c|}{0.957} & \multicolumn{1}{c|}{0.946}\\ \hline
...
Lincoln's stock price showed a strong downward trend over the pre-treatment period. Therefore, we defined the average of the log stock prices of the first two trust companies as $Y_t$, the outcome of the treated units at time $t=1,\ldots,384$. As for potential donors, \citet{Dataset1907} identified $N=49$ trust companies that had weak financial connections with the aforementioned three severely affected trust companies. 
%However, some of these trust companies seem to violate the relevance condition \eqref{eq-relevant}. Therefore, we chose donors based on the procedure proposed in Section \ref{sec:supp:Donors} of the Supplementary Material, which resulted in $d=24$ trust companies. 
...
\hline
\multicolumn{1}{|c|}{Estimator} & { OLS-NoReg} & { OLS-Standard} & {  ASC} & { SCPI} & { SPSC-NoDT } & {  SPSC-DT} \\ \hline

\multicolumn{1}{|c|}{Estimate} & {-1.021}  & {-0.873}  & {-0.912}  & {-0.876}  & {-0.813}  & {-0.816}  \\ \hline
\multicolumn{1}{|c|}{ASE} & {0.139}  & {-}  & {-}  & {-}  & {0.084}  & {0.066}  \\ \hline
\multicolumn{1}{|c|}{95\% CI} & {(-1.295,-0.748)}  & {-}  & {-}  & {-}  & {(-0.978,-0.648)}  & {(-0.945,-0.688)}  \\ \hline
