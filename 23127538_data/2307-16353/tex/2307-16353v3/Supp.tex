...
\FOR{$s=1,\ldots,\dim( \Psi_{\post} )$}

\STATE Fit AR(1) model for the $s$th component of the time series $\big\{ \Psi_{\post}(\bO_t \con \widehat{\Beta}, \widehat{\bgamma}_{\rho}, \widehat{\bbeta} ) \big\}_{t=1,\ldots,T}$. 
...
\end{align}
Again, note that $\sigma_{0i}=0$ for all $i=1,\ldots,N$ when errors are independent.
...
\ ,
\end{align*}
where $\Beta^*$ satisfies $\EXP(Y_t) = \bm{\mu}_0\T \bm{\nu}_t = \bD_t\T \Beta^*$ for $t=1,\ldots,T_0$. Note that $\bm{\eta}^* = \mathcal{E}^* \bm{\mu}_0$. For simplicity, we choose $\bh(y)=y$. Then, it is straightforward to show that the regularized GMM estimator of $\bgamma$ is given by
\begin{align*}
\widehat{\bgamma}_{\rho}
=
\big(
...
In practice, a rich collection of measured exogenous covariates may be available. One may want to incorporate these covariates in the synthetic control analysis because using these covariates may improve efficiency. In this Section, we provide details on the SPSC framework by incorporating measured covariates. Specifically, we denote $q$-dimensional measured exogenous covariates for unit $i = 0,\ldots,N$ at time $t=1,\ldots,T$ as $\bX_{it} \in \R^{q}$; we remind the readers that $i=0$ is the treated unit and $i=1,\ldots,N$ are the untreated units. Let $\bX_{t} = ( \bX_{1 t}\T,\ldots,\bX_{N t}\T)\T \in \R^{Nq}$ be the collection of all measured covariates of donors at time $t$. To account for covariates, we modify Assumptions \ref{assumption:valid proxy} and \ref{assumption:SC} as follows:
...
Suppose that Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov} are satisfied. Then, for $t=1,\ldots,T_0$, the synthetic control bridge function $h^*$ satisfies 
\begin{align}
...
With these generated variables, $\potY{t}{a}$ and $W_{it}$ at $t=1,\ldots,T$ were generated as 
\begin{align*}
\potY{t}{0} 
&
= \bm{\mu}_0\T \bm{\lambda}_t + e_{0t}
...
Lastly, we study the width of 95\% prediction intervals obtained from the SPSC-DT estimator. We use the same simulation setup as before, with the modification that $T_0=100$ and $T_1=400$, focusing only on cases where $\blambda_t$ does not have an intercept. We calculate the 95\% prediction intervals at $t=120, 140, \ldots, 480, 500$ and compute the average prediction interval width over 500 simulation repetitions. As shown in Figure \ref{fig:supp:PI width wider}, the prediction intervals may or may not widen as the post-treatment period progresses. Specifically, the width of the SPSC prediction intervals remains stable or shows only a slight increase when there is no systematic drift in the outcomes, but it tends to widen when such drift is present. Note that such systematic drift can be empirically verified by examining the trend of the estimated synthetic control, $\bW_t\T \widehat{\bgamma}_{\rho}$. 
...
\\
&
W_{i , T_0+1} = \kappa W_{i T_0} + \eta_{i , T_0+1} \ , \quad i=2,\ldots,N \ ,
\end{align*}
...
%\EXP \big\{ Y_t - h^*(\bW_{ t}) \cond Y_t \big\} = 0 \  \text{ almost surely} \ , \quad t=1,\ldots,T_0 \ .
%\end{align}
%Moreover, we have $
%\EXP \big\{ \potY{t}{0} - h^*(\bW_{ t}) \big\} = 0$ for any $t=1,\ldots,T$. 
...
%\begin{theorem}	\label{thm:Extension NP Cov}
%
%Suppose Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, and \ref{assumption:valid proxy Cov} are satisfied. Then, the synthetic control bridge function $h^*$ satisfies $\EXP \big\{ Y_t - h^*(\bW_{ t}, \bX_{0t}, \bX_{ t}) \cond Y_t, \bX_{0t}, \bX_{ t} \big\} = 0$ for $t=1,\ldots,T_0$. 	
%Moreover, we have $
%\EXP \big\{ \potY{t}{0} - h^*(\bW_{ t}, \bX_{0t}, \bX_{ t}) \big\} = 0$ for any $t=1,\ldots,T$. 
%Lastly, the ATT at time $t = T_0+1,\ldots,T$ is identified as $\tau_t^*
%=
%\EXP
%\big\{
...
For $t=1,\ldots,T$, suppose $\EXP \big\{ q(\bW_{ t}) \cond \potY{t}{0} \big\} = 0$ almost surely for a square integrable function $q$. Then, $q(\bW_{ t} ) = 0$ almost surely for $t=1,\ldots,T$. 	
\end{assumption}
...
Y_t = \EXP \big\{ h(\bW_{ t}) \cond Y_t \big\}, \ t=1,\ldots,T_0
\Big\}
\end{align*}
...
We consider a sieve approach as follows. First, we choose a sequence of approximating bases functions of $\bW_{ t}$, denoted by $\big\{ \varphi_k(\bw) \big\}_{k=1,2,\ldots}$. For this sequence, we define an approximating function space for $\mathcal{H}$ by using the first $k_T$ bases functions, i.e.,
\begin{align*}
\mathcal{H}_T
=
\bigg\{
...
\ .
\end{align*}
For a given function $h$, a sieve estimator of the conditional expectation $\EXP \big\{ h(\bW_{ t}) \cond Y_t \big\}$ for $t=1,\ldots,T_0$ can be obtained by regressing $h(\bW_{ t})$ on $ \bphi(Y_t) $, i.e., 
\begin{align*}
...
\begin{itemize}[leftmargin=1cm, itemsep=0cm]
\item[1.] For any $h \in \overline{\mathcal{H}}$, there exists a function $g_{0,h} \in \mathcal{H}$ satisfying $\langle g_{0,h}, h \rangle_w = \EXP \big\{ h(\bW_{ t}) \big\}$ for $t=T_0+1,\ldots,T$. 
...
\ .
\end{align*}
We consider additional sieve estimators of ${\EXP} \big\{ \widehat{g} (\bW_{ t}) \cond \potY{t}{0} \big\} $ and ${\EXP} \big\{ \widehat{h}_0(\bW_{ t}) \cond \potY{t}{0} \big\}$ for $t=T_0+1,\ldots,T$:
\begin{align*}
...
In addition, for any $T = 1,2,\ldots$, we find $\widehat{\btheta}_{\rho} \rightarrow \widehat{\btheta}_0 $ as $\rho \downarrow 0$ where
\begin{align*} 
&
\widehat{\btheta}_0
=
