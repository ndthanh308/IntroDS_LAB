...
\end{align*}
\end{theorem}
We remark that similar results are established in \citet{FuKnight2000, Fu2003, Caner2009}. In addition, we can establish a similar result when covariates are included. Following  Theorem \ref{thm:AN}, we may use $\widehat{\Sigma}_1
=
\big( \widehat{G}\T \widehat{\Omega} \widehat{G} \big)^{-1} \widehat{G}\T \widehat{\Omega}$ as an estimator of $\Sigma_1^*$. 
% Alternatively, to incorporate ridge regularization, one can use $ \widehat{\Sigma}_1
% =
...
For all $t=1,\ldots,T$, there exists a function $h^*: \R^{d+q+dq} \rightarrow \R$ that satisfies 
\begin{align*}
\potY{t}{0} = \EXP \big\{
...
Suppose Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, and \ref{assumption:SC NP Cov} are satisfied. Then, for $t=1,\ldots,T_0$, the synthetic control bridge function $h^*$ satisfies 
\begin{align}
\EXP \big\{ Y_t - h^*(\bW_{\D t}, \bX_{0t}, \bX_{\D t}) \cond Y_t, \bX_{0t}, \bX_{\D t} \big\} = 0 \ .
\label{eq-Fredholm}
\end{align}
Moreover, for $t=1,\ldots,T$, we have 
\begin{align*}
\EXP \big\{ \potY{t}{0} - h^*(\bW_{\D t}, \bX_{0t}, \bX_{\D t}) \big\} = 0 \ .
\end{align*}
Lastly, the ATT at time $t = T_0+1,\ldots,T$ is identified as 
\begin{align*}
\tau_t^*
=
\EXP
...
%0$.
%\end{assumption}
%Similar to the result established under the absence of covariates, we establish the following identification results under Assumptions \ref{assumption:valid proxy Cov} and \ref{assumption:SC NP Cov} when covariates are available:
%\begin{theorem} \label{thm:identification cov}
%Under Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, and \ref{assumption:SC NP Cov}, the synthetic control weights $\bgamma^*$ satisfy $\EXP \big \{ (Y_t - \bX_{0t}\T \bdelta_0^*) - ( \bW_{it}\T \bgamma^* - \bW_{\D t}\T \bdelta_\D^* ) \cond Y_t, \bX_{0t} , 
...
\bX_{\D t} \T \bdelta_{\D}^* \ .
\end{align*}
First, we define the following estimating function:
...
Suppose Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, \ref{assumption:SC NP Cov}, and \ref{assumption:weakdep Cov} hold, $(\bgamma^*,\bbeta^*,\bdelta_0^*,\bdelta_{\D}^*)$ are unique, and Regularity Conditions in Section \ref{sec:supp:AN} hold. Let $( \widehat{\bgamma}, \widehat{\bbeta}, \widehat{\bdelta}_0, \widehat{\bdelta}_\D ) $ be the GMM estimators where the estimating function \eqref{eq-Moment-Cov} is used, i.e., 
\begin{align*}
\big(
\widehat{\bgamma}
,
...
\begin{align*}
\potY{t}{0} = 0.2 \potY{t-1}{0} + 0.1 \potY{t-2}{0} + t/T_0 + 0.25 \delta X_{0t} + \epsilon_{y,t} \ .
\end{align*}
...
\begin{align*}
\potY{t}{1} = \potY{t}{0} + \tau_t^* A_t + \epsilon_{\tau,t} \ , \ 
t=1,\ldots,T \ .
\end{align*}
where $\tau_t^*$ for $t=T_0+1,\ldots,T$ is specified as
\begin{itemize}[leftmargin=1cm]
\item[] (\textit{Constant Effect}) $\tau_t^* = 3$;
\item[] (\textit{Linear Effect}) $\tau_t^* = 3 + (t-T_0)/T_1$.
\end{itemize}
Therefore, the random ATT is given by $\xi_t^* = \potY{t}{1} - \potY{t}{0} = \tau_t^* \ind(t > T_0) + \epsilon_{\tau,t}$. Lastly, we generated $\bW_{\D t} \in \R^{d}$ so that it satisfies Assumptions \ref{assumption:valid proxy} and \ref{assumption:SC} if $\delta=0$, and the corresponding assumptions made for incorporating covariates if $\delta=1$. Specifically, we considered the following data generating process for $\bW_{\D t}$ according to the number of donors $d \in \{2,5,9\}$:
...
\bepsilon_{\D t}
\end{align*} 
The true synthetic control weights are $\bgamma^* = (0.8,1.2)$.
...
\end{bmatrix}
\begin{bmatrix}
1 \\ \potY{t}{0}  - 0.25 \delta X_{0 t} \\ 0.5 \big\{ \potY{t}{0} \big\}^2 \\ \ind \big\{ \potY{t}{0} > 3 \big\} \\ \ind \big\{ \potY{t}{0} < 0 \big\}
\end{bmatrix}
+
0.25
\delta 
...
\bepsilon_{\D t}
\end{align*} 
The true synthetic control weights are $\bgamma^* \simeq ( 0.51, 0.77, 0.15, 0.23, 0.34)$. 
...
\end{bmatrix}
\begin{bmatrix}
1 \\ \potY{t}{0}  - 0.25 \delta X_{0 t} \\ 0.5 \big\{ \potY{t}{0} \big\}^2 \\ 
...
\bepsilon_{\D t}
\end{align*} 
The true synthetic control weights are $\bgamma^* \simeq (0.46 , 0.68 ,  0.03 , 0.04 , 0.06 , 0.09 , 0.14 , 0.20 , 0.30)$.
\end{itemize} 
...
\item[] (\textit{SPSC-NoReg}) 
The single proxy synthetic control approach with no regularization.
\item[] (\textit{SPSC-Ridge}) 
The single proxy synthetic control approach with ridge regularization.
\end{itemize}
For the two SPSC estimators, we set $\bg_t(\potY{t}{0})$ as time-invariant cubic B-spline bases functions with dimensions equal to twice the number of donors, i.e., $\bg_t(y) = \mathfrak{b}_{2d}( \cdot )$ where $\mathfrak{b}_{k}( \cdot )$ the $k$-dimensional cubic B-spline bases function. The knots of the spline functions were chosen based on the empirical quantiles of the pre-treatment outcomes. 

We first estimated the aggregated ATT over the post-treatment periods, i.e., $T_1^{-1} \sum_{t=T_0+1}^{T} \tau_t^*$. Figures \ref{Fig:Supp:Sim:XnT0}-\ref{Fig:Supp:Sim:XyT1} summarize the empirical distribution of the estimators graphically. Each figure is drawn in the following format:
\begin{itemize}[itemsep=0cm,leftmargin=1cm]
...
\end{itemize}
We find that the results are similar to those in Figure \ref{fig:Sim:Constant} of the main paper. 
...
\end{itemize}
We remark that the results in Tables \ref{Tab:Supp:Sim:T0} and \ref{Tab:Supp:Sim:T1} are similar to those in Table \ref{tab:Sim:Constant d9d0} of the main paper.
...
\hline
\multicolumn{3}{|c|}{Estimator}                                   & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}OLS\\ -NoReg\end{tabular}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}OLS\\ -Standard\end{tabular}} & \multicolumn{2}{c|}{ASC}       & \multicolumn{2}{c|}{SCPI}      & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}SPSC\\ -NoReg\end{tabular}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}SPSC\\ -Ridge\end{tabular}} \\ \hline
\multicolumn{1}{|c|}{$\delta$}           & \multicolumn{1}{c|}{$d$}               & $T_0$ & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 \\ \hline

\multicolumn{1}{|c|}{\multirow{21}{*}{0}} & \multicolumn{1}{c|}{\multirow{7}{*}{2}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{8.636} & 8.650 & \multicolumn{1}{c|}{22.109} & 24.215 & \multicolumn{1}{c|}{7.815} & 7.831 & \multicolumn{1}{c|}{7.815} & 7.831 & \multicolumn{1}{c|}{-0.194} & 0.019 & \multicolumn{1}{c|}{0.417} & 0.204\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.684} & 0.822 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.940} & 1.327 & \multicolumn{1}{c|}{2.863} & 1.317\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.756} & 0.892 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.326} & 1.421 & \multicolumn{1}{c|}{3.002} & 1.387\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.733} & 0.819 & \multicolumn{1}{c|}{7.547} & 3.580 & \multicolumn{1}{c|}{1.446} & 0.679 & \multicolumn{1}{c|}{1.446} & 0.679 & \multicolumn{1}{c|}{3.278} & 1.330 & \multicolumn{1}{c|}{3.257} & 1.321\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{77.584} & 75.495 & \multicolumn{1}{c|}{545.646} & 599.139 & \multicolumn{1}{c|}{63.165} & 61.786 & \multicolumn{1}{c|}{63.166} & 61.785 & \multicolumn{1}{c|}{10.762} & 1.765 & \multicolumn{1}{c|}{10.764} & 1.784\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.000} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.922} & 0.938 & \multicolumn{1}{c|}{0.894} & 0.926\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.000} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.932} & 0.952 & \multicolumn{1}{c|}{0.896} & 0.942\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{5}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.798} & 2.653 & \multicolumn{1}{c|}{10.509} & 10.514 & \multicolumn{1}{c|}{2.786} & 2.618 & \multicolumn{1}{c|}{3.395} & 3.243 & \multicolumn{1}{c|}{0.260} & 0.609 & \multicolumn{1}{c|}{0.124} & 0.138\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.514} & 0.727 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.980} & 1.382 & \multicolumn{1}{c|}{2.404} & 1.143\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.707} & 0.796 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.534} & 1.545 & \multicolumn{1}{c|}{2.262} & 1.031\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.866} & 0.705 & \multicolumn{1}{c|}{2.094} & 0.886 & \multicolumn{1}{c|}{1.490} & 0.636 & \multicolumn{1}{c|}{1.303} & 0.594 & \multicolumn{1}{c|}{3.014} & 1.269 & \multicolumn{1}{c|}{2.080} & 0.982\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{11.301} & 7.535 & \multicolumn{1}{c|}{114.818} & 111.330 & \multicolumn{1}{c|}{9.979} & 7.259 & \multicolumn{1}{c|}{13.223} & 10.868 & \multicolumn{1}{c|}{9.136} & 1.978 & \multicolumn{1}{c|}{4.335} & 0.983\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.550} & 0.032 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.952} & 0.946 & \multicolumn{1}{c|}{0.960} & 0.980\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.630} & 0.058 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.986} & 0.990 & \multicolumn{1}{c|}{0.966} & 0.978\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{9}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.450} & 2.335 & \multicolumn{1}{c|}{17.639} & 17.716 & \multicolumn{1}{c|}{3.674} & 2.591 & \multicolumn{1}{c|}{5.954} & 5.811 & \multicolumn{1}{c|}{0.285} & 0.411 & \multicolumn{1}{c|}{0.240} & 0.369\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.506} & 0.706 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.269} & 1.622 & \multicolumn{1}{c|}{2.764} & 1.322\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.784} & 0.772 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.678} & 1.648 & \multicolumn{1}{c|}{2.740} & 1.224\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.897} & 0.767 & \multicolumn{1}{c|}{2.918} & 1.025 & \multicolumn{1}{c|}{2.037} & 0.848 & \multicolumn{1}{c|}{1.685} & 0.748 & \multicolumn{1}{c|}{3.200} & 1.398 & \multicolumn{1}{c|}{2.601} & 1.119\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{9.595} & 6.041 & \multicolumn{1}{c|}{319.641} & 314.900 & \multicolumn{1}{c|}{17.640} & 7.430 & \multicolumn{1}{c|}{38.283} & 34.326 & \multicolumn{1}{c|}{10.301} & 2.120 & \multicolumn{1}{c|}{6.811} & 1.385\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.580} & 0.114 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.958} & 0.958 & \multicolumn{1}{c|}{0.950} & 0.966\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.688} & 0.150 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.978} & 0.978 & \multicolumn{1}{c|}{0.956} & 0.964\\ \hline

 \multicolumn{1}{|c|}{\multirow{21}{*}{1}} & \multicolumn{1}{c|}{\multirow{7}{*}{2}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{8.726} & 8.574 & \multicolumn{1}{c|}{14.617} & 15.756 & \multicolumn{1}{c|}{7.908} & 7.734 & \multicolumn{1}{c|}{7.823} & 7.768 & \multicolumn{1}{c|}{-0.301} & -0.147 & \multicolumn{1}{c|}{0.290} & 0.062\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.713} & 0.820 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.028} & 1.336 & \multicolumn{1}{c|}{2.946} & 1.325\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.830} & 0.909 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{4.198} & 2.109 & \multicolumn{1}{c|}{4.083} & 2.079\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.933} & 0.813 & \multicolumn{1}{c|}{8.262} & 8.063 & \multicolumn{1}{c|}{2.332} & 0.838 & \multicolumn{1}{c|}{1.592} & 0.724 & \multicolumn{1}{c|}{3.442} & 1.438 & \multicolumn{1}{c|}{3.365} & 1.415\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{79.868} & 74.166 & \multicolumn{1}{c|}{281.787} & 313.137 & \multicolumn{1}{c|}{67.965} & 60.514 & \multicolumn{1}{c|}{63.726} & 60.859 & \multicolumn{1}{c|}{11.914} & 2.085 & \multicolumn{1}{c|}{11.384} & 2.003\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.004} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.924} & 0.932 & \multicolumn{1}{c|}{0.916} & 0.934\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.002} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.970} & 0.988 & \multicolumn{1}{c|}{0.970} & 0.984\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{5}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.979} & 2.718 & \multicolumn{1}{c|}{9.128} & 8.799 & \multicolumn{1}{c|}{2.645} & 2.456 & \multicolumn{1}{c|}{3.256} & 3.040 & \multicolumn{1}{c|}{0.315} & 0.663 & \multicolumn{1}{c|}{0.116} & 0.118\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.516} & 0.731 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.787} & 1.366 & \multicolumn{1}{c|}{2.358} & 1.134\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.744} & 0.821 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.834} & 1.971 & \multicolumn{1}{c|}{3.389} & 1.746\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.744} & 0.786 & \multicolumn{1}{c|}{4.684} & 3.621 & \multicolumn{1}{c|}{1.452} & 0.703 & \multicolumn{1}{c|}{1.264} & 0.635 & \multicolumn{1}{c|}{2.749} & 1.294 & \multicolumn{1}{c|}{2.091} & 1.120\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{11.909} & 8.003 & \multicolumn{1}{c|}{105.227} & 90.504 & \multicolumn{1}{c|}{9.099} & 6.523 & \multicolumn{1}{c|}{12.196} & 9.646 & \multicolumn{1}{c|}{7.642} & 2.111 & \multicolumn{1}{c|}{4.379} & 1.266\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.476} & 0.038 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.948} & 0.946 & \multicolumn{1}{c|}{0.962} & 0.950\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.582} & 0.070 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.984} & 0.990 & \multicolumn{1}{c|}{1.000} & 0.994\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{9}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.725} & 2.352 & \multicolumn{1}{c|}{14.191} & 15.108 & \multicolumn{1}{c|}{3.540} & 2.435 & \multicolumn{1}{c|}{5.806} & 5.786 & \multicolumn{1}{c|}{0.321} & 0.398 & \multicolumn{1}{c|}{0.223} & 0.280\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.542} & 0.708 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.303} & 1.645 & \multicolumn{1}{c|}{2.838} & 1.352\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{1.828} & 0.797 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{4.309} & 2.355 & \multicolumn{1}{c|}{4.159} & 2.268\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.887} & 0.794 & \multicolumn{1}{c|}{6.606} & 6.145 & \multicolumn{1}{c|}{1.981} & 0.859 & \multicolumn{1}{c|}{1.604} & 0.788 & \multicolumn{1}{c|}{3.206} & 1.363 & \multicolumn{1}{c|}{2.760} & 1.096\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{10.981} & 6.159 & \multicolumn{1}{c|}{244.942} & 265.940 & \multicolumn{1}{c|}{16.445} & 6.668 & \multicolumn{1}{c|}{36.281} & 34.093 & \multicolumn{1}{c|}{10.360} & 2.011 & \multicolumn{1}{c|}{7.650} & 1.277\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.576} & 0.100 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.972} & 0.982 & \multicolumn{1}{c|}{0.968} & 0.978\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.676} & 0.154 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.990} & 0.994 & \multicolumn{1}{c|}{0.990} & 1.000\\ \hline

\end{tabular}
\caption{Summary Statistics of the Estimation Results Under the Constant Effect.} 
...
\multicolumn{1}{|c|}{$\delta$}           & \multicolumn{1}{c|}{$d$}               & $T_0$ & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 \\ \hline
 ...
 \multicolumn{1}{|c|}{\multirow{21}{*}{0}} & \multicolumn{1}{c|}{\multirow{7}{*}{2}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{8.679} & 8.526 & \multicolumn{1}{c|}{22.431} & 24.693 & \multicolumn{1}{c|}{7.907} & 7.783 & \multicolumn{1}{c|}{7.907} & 7.783 & \multicolumn{1}{c|}{-0.214} & 0.039 & \multicolumn{1}{c|}{0.388} & 0.197\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.648} & 0.802 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.904} & 1.309 & \multicolumn{1}{c|}{2.827} & 1.301\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.567} & 1.470 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.592} & 1.687 & \multicolumn{1}{c|}{3.329} & 1.665\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.726} & 0.850 & \multicolumn{1}{c|}{6.994} & 3.451 & \multicolumn{1}{c|}{1.490} & 0.732 & \multicolumn{1}{c|}{1.490} & 0.732 & \multicolumn{1}{c|}{3.300} & 1.436 & \multicolumn{1}{c|}{3.262} & 1.426\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{78.293} & 73.413 & \multicolumn{1}{c|}{551.985} & 621.628 & \multicolumn{1}{c|}{64.733} & 61.112 & \multicolumn{1}{c|}{64.733} & 61.112 & \multicolumn{1}{c|}{10.914} & 2.058 & \multicolumn{1}{c|}{10.770} & 2.069\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.000} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.924} & 0.926 & \multicolumn{1}{c|}{0.896} & 0.924\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.032} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.974} & 0.982 & \multicolumn{1}{c|}{0.956} & 0.970\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{5}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.787} & 2.710 & \multicolumn{1}{c|}{10.564} & 10.627 & \multicolumn{1}{c|}{2.765} & 2.679 & \multicolumn{1}{c|}{3.408} & 3.301 & \multicolumn{1}{c|}{0.303} & 0.606 & \multicolumn{1}{c|}{0.064} & 0.111\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.492} & 0.735 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.916} & 1.426 & \multicolumn{1}{c|}{2.411} & 1.178\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.394} & 1.313 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.755} & 1.819 & \multicolumn{1}{c|}{2.707} & 1.443\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.708} & 0.771 & \multicolumn{1}{c|}{1.925} & 0.871 & \multicolumn{1}{c|}{1.422} & 0.675 & \multicolumn{1}{c|}{1.278} & 0.630 & \multicolumn{1}{c|}{3.001} & 1.375 & \multicolumn{1}{c|}{2.097} & 1.062\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{10.678} & 7.935 & \multicolumn{1}{c|}{115.299} & 113.691 & \multicolumn{1}{c|}{9.664} & 7.632 & \multicolumn{1}{c|}{13.248} & 11.294 & \multicolumn{1}{c|}{9.082} & 2.253 & \multicolumn{1}{c|}{4.393} & 1.137\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.504} & 0.046 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.944} & 0.946 & \multicolumn{1}{c|}{0.956} & 0.952\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.878} & 0.444 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.996} & 0.994 & \multicolumn{1}{c|}{0.984} & 0.994\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{9}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.414} & 2.278 & \multicolumn{1}{c|}{17.577} & 17.618 & \multicolumn{1}{c|}{3.704} & 2.565 & \multicolumn{1}{c|}{5.984} & 5.780 & \multicolumn{1}{c|}{0.291} & 0.418 & \multicolumn{1}{c|}{0.270} & 0.330\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.498} & 0.707 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.273} & 1.604 & \multicolumn{1}{c|}{2.829} & 1.336\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.359} & 1.278 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.972} & 1.861 & \multicolumn{1}{c|}{3.199} & 1.574\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.777} & 0.730 & \multicolumn{1}{c|}{2.737} & 0.995 & \multicolumn{1}{c|}{1.919} & 0.797 & \multicolumn{1}{c|}{1.641} & 0.710 & \multicolumn{1}{c|}{3.036} & 1.320 & \multicolumn{1}{c|}{2.532} & 1.118\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{8.976} & 5.722 & \multicolumn{1}{c|}{316.425} & 311.365 & \multicolumn{1}{c|}{17.392} & 7.213 & \multicolumn{1}{c|}{38.502} & 33.907 & \multicolumn{1}{c|}{9.285} & 1.914 & \multicolumn{1}{c|}{6.473} & 1.357\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.610} & 0.104 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.962} & 0.982 & \multicolumn{1}{c|}{0.958} & 0.976\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.884} & 0.610 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.994} & 0.994 & \multicolumn{1}{c|}{0.990} & 0.998\\ \hline



 \multicolumn{1}{|c|}{\multirow{21}{*}{1}} & \multicolumn{1}{c|}{\multirow{7}{*}{2}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{8.649} & 8.628 & \multicolumn{1}{c|}{14.668} & 15.062 & \multicolumn{1}{c|}{7.903} & 7.779 & \multicolumn{1}{c|}{7.802} & 7.840 & \multicolumn{1}{c|}{-0.416} & -0.010 & \multicolumn{1}{c|}{0.165} & 0.178\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.650} & 0.807 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{2.970} & 1.336 & \multicolumn{1}{c|}{2.893} & 1.326\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.611} & 1.496 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{4.394} & 2.274 & \multicolumn{1}{c|}{4.289} & 2.259\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.830} & 0.861 & \multicolumn{1}{c|}{8.334} & 8.122 & \multicolumn{1}{c|}{2.786} & 0.917 & \multicolumn{1}{c|}{1.611} & 0.768 & \multicolumn{1}{c|}{3.468} & 1.454 & \multicolumn{1}{c|}{3.412} & 1.445\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{78.142} & 75.185 & \multicolumn{1}{c|}{284.461} & 292.698 & \multicolumn{1}{c|}{70.204} & 61.351 & \multicolumn{1}{c|}{63.461} & 62.051 & \multicolumn{1}{c|}{12.176} & 2.109 & \multicolumn{1}{c|}{11.644} & 2.117\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.004} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.904} & 0.922 & \multicolumn{1}{c|}{0.874} & 0.914\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.026} & 0.000 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.986} & 0.998 & \multicolumn{1}{c|}{0.978} & 0.990\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{5}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.866} & 2.738 & \multicolumn{1}{c|}{9.156} & 9.101 & \multicolumn{1}{c|}{2.615} & 2.455 & \multicolumn{1}{c|}{3.309} & 3.048 & \multicolumn{1}{c|}{0.181} & 0.611 & \multicolumn{1}{c|}{-0.030} & 0.080\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.503} & 0.727 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.031} & 1.312 & \multicolumn{1}{c|}{2.467} & 1.105\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.402} & 1.325 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{4.035} & 2.041 & \multicolumn{1}{c|}{3.665} & 1.892\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.872} & 0.776 & \multicolumn{1}{c|}{4.987} & 3.674 & \multicolumn{1}{c|}{1.591} & 0.704 & \multicolumn{1}{c|}{1.382} & 0.631 & \multicolumn{1}{c|}{3.089} & 1.247 & \multicolumn{1}{c|}{2.249} & 1.118\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{11.714} & 8.096 & \multicolumn{1}{c|}{108.662} & 96.304 & \multicolumn{1}{c|}{9.363} & 6.522 & \multicolumn{1}{c|}{12.853} & 9.691 & \multicolumn{1}{c|}{9.556} & 1.925 & \multicolumn{1}{c|}{5.050} & 1.254\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.512} & 0.030 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.958} & 0.944 & \multicolumn{1}{c|}{0.954} & 0.940\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.836} & 0.418 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.984} & 0.996 & \multicolumn{1}{c|}{0.992} & 0.998\\ \cline{2-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{7}{*}{9}} & Bias $(\times 10)$ & \multicolumn{1}{c|}{2.875} & 2.354 & \multicolumn{1}{c|}{14.220} & 14.726 & \multicolumn{1}{c|}{3.541} & 2.438 & \multicolumn{1}{c|}{5.842} & 5.728 & \multicolumn{1}{c|}{0.279} & 0.328 & \multicolumn{1}{c|}{0.293} & 0.220\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ASE $(\times 10)$ & \multicolumn{1}{c|}{1.502} & 0.699 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{3.171} & 1.571 & \multicolumn{1}{c|}{2.754} & 1.303\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & BSE $(\times 10)$ & \multicolumn{1}{c|}{2.406} & 1.259 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{4.411} & 2.352 & \multicolumn{1}{c|}{4.295} & 2.279\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & ESE $(\times 10)$ & \multicolumn{1}{c|}{1.846} & 0.726 & \multicolumn{1}{c|}{6.096} & 6.232 & \multicolumn{1}{c|}{2.135} & 0.839 & \multicolumn{1}{c|}{1.726} & 0.751 & \multicolumn{1}{c|}{3.060} & 1.322 & \multicolumn{1}{c|}{2.622} & 1.064\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & MSE $(\times 100)$ & \multicolumn{1}{c|}{11.664} & 6.069 & \multicolumn{1}{c|}{239.309} & 255.607 & \multicolumn{1}{c|}{17.088} & 6.645 & \multicolumn{1}{c|}{37.100} & 33.373 & \multicolumn{1}{c|}{9.420} & 1.852 & \multicolumn{1}{c|}{6.947} & 1.178\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (ASE) & \multicolumn{1}{c|}{0.506} & 0.078 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.966} & 0.976 & \multicolumn{1}{c|}{0.960} & 0.986\\ \cline{3-15}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & Coverage (BSE) & \multicolumn{1}{c|}{0.852} & 0.566 & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{-} & - & \multicolumn{1}{c|}{0.992} & 0.998 & \multicolumn{1}{c|}{0.994} & 1.000\\ \hline

\end{tabular}
\caption{Summary Statistics of the Estimation Results Under the Linear Effect.} 
...
We report the performance of the conformal inference in Section \ref{sec:Conformal} of the main paper under the simulation scenarios in Section \ref{sec:Sim} of the main paper. First, we obtain the pointwise 95\% pointwise prediction interval for the random treatment effect at 10 post-treatment times $t \in \mathcal{T} = \{ T_0 + 0.1 T_1, T_0 + 0.2 T_1, \ldots T_0 + 0.9 T_1 , T\}$, which are
\begin{align*}
&
\text{Constant Effect} :
&&
\xi_{t}^* = 3 + \epsilon_{\tau,t} \ 
, \
&&
\text{Linear Effect} :
&&
\xi_{t}^* = 3 + 3(t-T_0)/T_1 + \epsilon_{\tau,t}
\ .
\end{align*}
...
Table \ref{tab:supp:Table3} shows the empirical coverage rates obtained from 500 repetitions for each simulation scenario. We find that the conformal inference approach for the SPSC achieves the nominal coverage rate across all simulation scenarios in general. However, we find that the ASC approach fails to so for all reported scenarios. Likewise, the SCPI approach appears to struggle in attaining the desired nominal coverage rate, especially when the number of donors is small or the time periods are long, say $d=2$ and $T_0=500$. This finding demonstrates that existing inference methods may be invalid when applied to a dataset compatible with the SPSC framework.


Next, we calculate the average length of the 95\% prediction intervals for $t \in \mathcal{T}$ across 500 repetitions. 
...
We remark that prediction intervals obtained from the SCPI approach are generally narrower than those obtained from the SPSC. 
...
\hline
\multicolumn{3}{|c|}{Estimator}                                                                          & \multicolumn{2}{c|}{ASC}       & \multicolumn{2}{c|}{SCPI}      & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}SPSC-NoReg\end{tabular}} & \multicolumn{2}{c|}{\begin{tabular}[c]{@{}c@{}}SPSC-Ridge\end{tabular}} \\ \hline
\multicolumn{3}{|c|}{$T_0$}                                                                              & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100}                       & 500                       & \multicolumn{1}{c|}{100}                       & 500                       \\ \hline

 \multicolumn{1}{|c|}{\multirow{6}{*}{\begin{tabular}[c]{@{}c@{}}Constant\\ Effect\end{tabular}}} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=0$}} & $d=2$ & \multicolumn{1}{c|}{0.789} & 0.800 & \multicolumn{1}{c|}{0.853} & 0.818 & \multicolumn{1}{c|}{0.943} & 0.950 & \multicolumn{1}{c|}{0.941} & 0.951\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{0.893} & 0.895 & \multicolumn{1}{c|}{0.944} & 0.930 & \multicolumn{1}{c|}{0.936} & 0.937 & \multicolumn{1}{c|}{0.938} & 0.944\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{0.870} & 0.896 & \multicolumn{1}{c|}{0.958} & 0.927 & \multicolumn{1}{c|}{0.941} & 0.942 & \multicolumn{1}{c|}{0.939} & 0.942\\ \cline{2-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=1$}} & $d=2$ & \multicolumn{1}{c|}{0.814} & 0.811 & \multicolumn{1}{c|}{0.891} & 0.848 & \multicolumn{1}{c|}{0.940} & 0.948 & \multicolumn{1}{c|}{0.939} & 0.948\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{0.894} & 0.905 & \multicolumn{1}{c|}{0.959} & 0.951 & \multicolumn{1}{c|}{0.929} & 0.931 & \multicolumn{1}{c|}{0.934} & 0.938\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{0.876} & 0.905 & \multicolumn{1}{c|}{0.974} & 0.941 & \multicolumn{1}{c|}{0.931} & 0.938 & \multicolumn{1}{c|}{0.929} & 0.937\\ \hline
 \multicolumn{1}{|c|}{\multirow{6}{*}{\begin{tabular}[c]{@{}c@{}}Linear\\ Effect\end{tabular}}} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=0$}} & $d=2$ & \multicolumn{1}{c|}{0.755} & 0.729 & \multicolumn{1}{c|}{0.857} & 0.814 & \multicolumn{1}{c|}{0.952} & 0.953 & \multicolumn{1}{c|}{0.949} & 0.953\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{0.859} & 0.851 & \multicolumn{1}{c|}{0.945} & 0.935 & \multicolumn{1}{c|}{0.945} & 0.937 & \multicolumn{1}{c|}{0.938} & 0.939\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{0.816} & 0.829 & \multicolumn{1}{c|}{0.958} & 0.928 & \multicolumn{1}{c|}{0.934} & 0.940 & \multicolumn{1}{c|}{0.936} & 0.942\\ \cline{2-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=1$}} & $d=2$ & \multicolumn{1}{c|}{0.767} & 0.757 & \multicolumn{1}{c|}{0.885} & 0.851 & \multicolumn{1}{c|}{0.938} & 0.955 & \multicolumn{1}{c|}{0.937} & 0.957\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{0.879} & 0.895 & \multicolumn{1}{c|}{0.968} & 0.949 & \multicolumn{1}{c|}{0.929} & 0.938 & \multicolumn{1}{c|}{0.931} & 0.935\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{0.845} & 0.846 & \multicolumn{1}{c|}{0.974} & 0.934 & \multicolumn{1}{c|}{0.940} & 0.935 & \multicolumn{1}{c|}{0.940} & 0.936\\ \hline
...
\multicolumn{3}{|c|}{$T_0$}                                                                              & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100} & 500 & \multicolumn{1}{c|}{100}                       & 500                       & \multicolumn{1}{c|}{100}                       & 500                       \\ \hline
 ...
 \multicolumn{1}{|c|}{\multirow{6}{*}{\begin{tabular}[c]{@{}c@{}}Constant\\ Effect\end{tabular}}} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=0$}} & $d=2$ & \multicolumn{1}{c|}{2.711} & 2.689 & \multicolumn{1}{c|}{1.488} & 1.292 & \multicolumn{1}{c|}{4.487} & 4.364 & \multicolumn{1}{c|}{4.321} & 4.313\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{1.990} & 1.863 & \multicolumn{1}{c|}{2.039} & 1.946 & \multicolumn{1}{c|}{2.980} & 2.858 & \multicolumn{1}{c|}{2.246} & 2.188\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{1.994} & 1.780 & \multicolumn{1}{c|}{2.286} & 1.760 & \multicolumn{1}{c|}{3.216} & 3.197 & \multicolumn{1}{c|}{2.568} & 2.467\\ \cline{2-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=1$}} & $d=2$ & \multicolumn{1}{c|}{2.865} & 2.751 & \multicolumn{1}{c|}{1.772} & 1.459 & \multicolumn{1}{c|}{4.537} & 4.386 & \multicolumn{1}{c|}{4.424} & 4.356\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{2.072} & 1.928 & \multicolumn{1}{c|}{2.321} & 2.113 & \multicolumn{1}{c|}{2.594} & 2.556 & \multicolumn{1}{c|}{2.235} & 2.175\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{2.062} & 1.846 & \multicolumn{1}{c|}{2.564} & 2.002 & \multicolumn{1}{c|}{2.807} & 2.707 & \multicolumn{1}{c|}{2.561} & 2.395\\ \hline
 \multicolumn{1}{|c|}{\multirow{6}{*}{\begin{tabular}[c]{@{}c@{}}Linear\\ Effect\end{tabular}}} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=0$}} & $d=2$ & \multicolumn{1}{c|}{2.555} & 2.393 & \multicolumn{1}{c|}{1.502} & 1.280 & \multicolumn{1}{c|}{4.536} & 4.379 & \multicolumn{1}{c|}{4.381} & 4.334\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{1.865} & 1.758 & \multicolumn{1}{c|}{2.004} & 1.934 & \multicolumn{1}{c|}{3.049} & 2.935 & \multicolumn{1}{c|}{2.257} & 2.199\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{1.857} & 1.559 & \multicolumn{1}{c|}{2.314} & 1.781 & \multicolumn{1}{c|}{3.281} & 3.171 & \multicolumn{1}{c|}{2.652} & 2.457\\ \cline{2-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{\multirow{3}{*}{$\delta=1$}} & $d=2$ & \multicolumn{1}{c|}{2.704} & 2.527 & \multicolumn{1}{c|}{1.767} & 1.463 & \multicolumn{1}{c|}{4.579} & 4.403 & \multicolumn{1}{c|}{4.470} & 4.374\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=5$ & \multicolumn{1}{c|}{1.959} & 1.919 & \multicolumn{1}{c|}{2.361} & 2.165 & \multicolumn{1}{c|}{2.652} & 2.529 & \multicolumn{1}{c|}{2.254} & 2.188\\ \cline{3-11}
 \multicolumn{1}{|c|}{} & \multicolumn{1}{c|}{} & $d=9$ & \multicolumn{1}{c|}{1.956} & 1.703 & \multicolumn{1}{c|}{2.565} & 1.988 & \multicolumn{1}{c|}{2.852} & 2.699 & \multicolumn{1}{c|}{2.590} & 2.393\\ \hline

\end{tabular}
\caption{Average Lengths of 95\% Pointwise Prediction Intervals at $t \in \mathcal{T} = \{ T_0+0.1T_1,\ldots,T_0+0.9T_1, T\}$.}
\label{tab:supp:Table4} 
\end{table}


Lastly, we revisit Tables \ref{Tab:Supp:Sim:T0} and \ref{Tab:Supp:Sim:T1} to compare the bias and the empirical standard error of the ATT estimators. Compared with the two estimators obtained from the SPSC, we find the estimators obtained from the ASC and SCPI yield significant magnitudes of biases even under a large sample size. Moreover, these biases are not negligible based on the size of the empirical standard errors. Consequently, we can deduce that the failure of the ASC and SCPI approaches to attain the nominal coverage rate can be attributed to the non-diminishing bias.
...
\multicolumn{1}{|c|}{\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SPSC\\ -Ridge\end{tabular}}} & \begin{tabular}[c]{@{}c@{}}Time-invariant $\bg$\end{tabular} & \multicolumn{1}{c|}{0.942} & \multicolumn{1}{c|}{0.946} & \multicolumn{1}{c|}{0.946} & \multicolumn{1}{c|}{0.944} & 0.934 & \multicolumn{1}{c|}{0.920} & \multicolumn{1}{c|}{0.942} & \multicolumn{1}{c|}{0.956} & \multicolumn{1}{c|}{0.956} & 0.946 & \multicolumn{1}{c|}{0.808} & \multicolumn{1}{c|}{0.852} & \multicolumn{1}{c|}{0.858} & \multicolumn{1}{c|}{0.856} & 0.824 \\ \cline{2-17} 
\multicolumn{1}{|c|}{}                                                                       & \begin{tabular}[c]{@{}c@{}}Time-varying $\bg_t$\end{tabular} & \multicolumn{1}{c|}{0.942} & \multicolumn{1}{c|}{0.946} & \multicolumn{1}{c|}{0.946} & \multicolumn{1}{c|}{0.944} & 0.934 & \multicolumn{1}{c|}{0.946} & \multicolumn{1}{c|}{0.954} & \multicolumn{1}{c|}{0.960} & \multicolumn{1}{c|}{0.958} & 0.952 & \multicolumn{1}{c|}{0.914} & \multicolumn{1}{c|}{0.932} & \multicolumn{1}{c|}{0.942} & \multicolumn{1}{c|}{0.952} & 0.948 \\ \hline
\multicolumn{2}{|c|}{SCPI}                                                                                                                                    & \multicolumn{1}{c|}{0.974} & \multicolumn{1}{c|}{0.976} & \multicolumn{1}{c|}{0.974} & \multicolumn{1}{c|}{0.976} & 0.978 & \multicolumn{1}{c|}{0.974} & \multicolumn{1}{c|}{0.970} & \multicolumn{1}{c|}{0.970} & \multicolumn{1}{c|}{0.978} & 0.980 & \multicolumn{1}{c|}{0.984} & \multicolumn{1}{c|}{0.980} & \multicolumn{1}{c|}{0.978} & \multicolumn{1}{c|}{0.982} & 0.988 \\ \hline
\end{tabular}
\caption{Empirical Coverage Rates of 95\% Pointwise Prediction Intervals  of the Three ATT Estimators.}
...
We find that the prediction intervals obtained from the SCPI approach are generally narrower than those obtained from the SPSC, except for the cases where $\rho=1$ and $\zeta=\pm1$. 
...
\multicolumn{2}{|c|}{$\rho$}                                                                                                                                   & \multicolumn{5}{c|}{0}                                                                                                    & \multicolumn{5}{c|}{0.5}                                                                                                  & \multicolumn{5}{c|}{1}                                                                                                    \\ \hline
\multicolumn{2}{|c|}{$\zeta$}                                                                                                                                  & \multicolumn{1}{c|}{-1}    & \multicolumn{1}{c|}{-0.5}  & \multicolumn{1}{c|}{0}     & \multicolumn{1}{c|}{0.5}   & 1     & \multicolumn{1}{c|}{-1}    & \multicolumn{1}{c|}{-0.5}  & \multicolumn{1}{c|}{0}     & \multicolumn{1}{c|}{0.5}   & 1     & \multicolumn{1}{c|}{-1}    & \multicolumn{1}{c|}{-0.5}  & \multicolumn{1}{c|}{0}     & \multicolumn{1}{c|}{0.5}   & 1     \\ \hline
\multicolumn{1}{|c|}{\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SPSC\\ -Ridge\end{tabular}}} & \begin{tabular}[c]{@{}c@{}}Time-invariant $\bg$\end{tabular} & \multicolumn{1}{c|}{3.018} & \multicolumn{1}{c|}{3.035} & \multicolumn{1}{c|}{3.040} & \multicolumn{1}{c|}{3.046} & 3.052 & \multicolumn{1}{c|}{2.963} & \multicolumn{1}{c|}{2.967} & \multicolumn{1}{c|}{2.970} & \multicolumn{1}{c|}{2.991} & 2.971 & \multicolumn{1}{c|}{3.262} & \multicolumn{1}{c|}{3.288} & \multicolumn{1}{c|}{3.272} & \multicolumn{1}{c|}{3.285} & 3.281 \\ \cline{2-17} 
\multicolumn{1}{|c|}{}                                                                       & \begin{tabular}[c]{@{}c@{}}Time-varying $\bg_t$\end{tabular} & \multicolumn{1}{c|}{3.018} & \multicolumn{1}{c|}{3.035} & \multicolumn{1}{c|}{3.039} & \multicolumn{1}{c|}{3.045} & 3.052 & \multicolumn{1}{c|}{2.904} & \multicolumn{1}{c|}{2.903} & \multicolumn{1}{c|}{2.902} & \multicolumn{1}{c|}{2.921} & 2.931 & \multicolumn{1}{c|}{2.926} & \multicolumn{1}{c|}{2.900} & \multicolumn{1}{c|}{2.904} & \multicolumn{1}{c|}{2.931} & 2.979 \\ \hline
\multicolumn{2}{|c|}{SCPI}                                                                                                                                     & \multicolumn{1}{c|}{2.657} & \multicolumn{1}{c|}{2.596} & \multicolumn{1}{c|}{2.576} & \multicolumn{1}{c|}{2.597} & 2.658 & \multicolumn{1}{c|}{2.654} & \multicolumn{1}{c|}{2.593} & \multicolumn{1}{c|}{2.574} & \multicolumn{1}{c|}{2.598} & 2.662 & \multicolumn{1}{c|}{3.154} & \multicolumn{1}{c|}{3.024} & \multicolumn{1}{c|}{2.985} & \multicolumn{1}{c|}{3.035} & 3.165 \\ \hline
\end{tabular}
\caption{Average Lengths of 95\% Pointwise Prediction Intervals at $t=T_0+1$ of the Three ATT Estimators.}
\label{tab:supp:Table11}
\vspace*{-0.3cm}
\end{table}


Lastly, we report the bias and the empirical standard error of the estimators for the ATT obtained from the SCPI approach. Specifically, the estimator is obtained as $\widehat{\tau}_{\text{ATT}}
=
Y_t - \widehat{Y}_{t}^{(0)}$ where $\widehat{Y}_{t}^{(0)} $ is a predicted value of the treatment-free potential outcome at time $t=T_0+1$. Comparing the estimator derived from the SCPI approach to the SPSC estimator, we observe that the latter leads to larger biases. Nevertheless, when considering the empirical standard errors, these biases become negligible. As a result, we deduce that the ATT estimator from the SPSC approach exhibits a negligible bias, whereas the conformal inference approach can be anticonservative in the presence of nonstationarity.
...
\multicolumn{2}{|c|}{$\rho$} & \multicolumn{5}{c|}{0} & \multicolumn{5}{c|}{0.5} & \multicolumn{5}{c|}{1} \\ \hline
\multicolumn{2}{|c|}{$\zeta$} & \multicolumn{1}{c|}{-1} & \multicolumn{1}{c|}{-0.5} & \multicolumn{1}{c|}{0} & \multicolumn{1}{c|}{0.5} & 1 & \multicolumn{1}{c|}{-1} & \multicolumn{1}{c|}{-0.5} & \multicolumn{1}{c|}{0} & \multicolumn{1}{c|}{0.5} & 1 & \multicolumn{1}{c|}{-1} & \multicolumn{1}{c|}{-0.5} & \multicolumn{1}{c|}{0} & \multicolumn{1}{c|}{0.5} & 1 \\ \hline
\multicolumn{1}{|c|}{\multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}SPSC\\ -Ridge\end{tabular}}} & Time-invariant $\bg$ & \multicolumn{1}{c|}{-0.030} & \multicolumn{1}{c|}{-0.034} & \multicolumn{1}{c|}{-0.038} & \multicolumn{1}{c|}{-0.041} & \multicolumn{1}{c|}{-0.045} & \multicolumn{1}{c|}{-0.084} & \multicolumn{1}{c|}{-0.085} & \multicolumn{1}{c|}{-0.085} & \multicolumn{1}{c|}{-0.086} & \multicolumn{1}{c|}{-0.086} & \multicolumn{1}{c|}{-0.181} & \multicolumn{1}{c|}{-0.102} & \multicolumn{1}{c|}{-0.023} & \multicolumn{1}{c|}{0.055} & 0.134 \\ \cline{2-17}
...
\caption{Biases (top table) and Empirical Standard Errors (bottom table) of the Three ATT Estimators.}
...
We review the six approaches employed for the analysis:
\begin{itemize}[leftmargin=1cm]
\item[] (\textit{OLS-NoReg}) OLS-based approach based on \eqref{eq-ExistSC-Abadie} with no regularization.
\item[] (\textit{OLS-Standard}) The standard synthetic control approach proposed by \citet{Abadie2010}; we used \texttt{synth} R-package \citep{Synth2011}.
\item[] (\textit{ASC}) The augmented synthetic control approach proposed by \citet{ASCM2021}; we used , \texttt{augsynth} \citep{ASCM2023package} R-package.
\item[] (\textit{SCPI}) The synthetic control prediction interval approach proposed by \citet{Cattaneo2021}; we used \texttt{scpi} \citep{scpiPackage2023} R-package. Of note, we included a constant term by specifying \texttt{constant=T} in \text{scdata} function, and increased \texttt{sims} parameter in \texttt{scpi} function from its default value of 200 to 2000, i.e., \texttt{scdata(..., constant = T)};  \texttt{scpi(..., sims = 2000)}. The adjustments were implemented to enhance the overall fit and minimize variability in the outcomes.		
\item[] (\textit{SPSC-NoReg}) The single proxy synthetic control approach with no regularization.
\item[] (\textit{SPSC-Ridge}) The single proxy synthetic control approach with ridge regularization.
\end{itemize}

Using these six choices for the donor pool and the two choices for $\bg_t$, we focus on the estimation of the ATT. Table \ref{tab:supp:ATT} summarizes the result. We find that the results are similar to each other except the two OLS-Standard ATT estimators under Group 1 and Group 5. In terms of the length of the 95\% confidence intervals, the SPSC with ridge regularization results in the narrowest confidence intervals in general, except for Group 2 with time-invariant $\bg$. These additional analyses for the ATT corroborate the results in Table \ref{tab:data:ATT} of the main paper.
...
\hline
\multirow{2}{*}{Donor Pool} & \multirow{2}{*}{Statistic} & \multirow{2}{*}{\begin{tabular}[c]{@{}c@{}}OLS\\ -Standard\end{tabular}} & \multirow{2}{*}{ASC} & \multirow{2}{*}{SCPI} & \multicolumn{3}{c|}{Time-invariant $\bg$}                                     & \multicolumn{3}{c|}{Time-varying $\bg_t$}                                     \\ \cline{6-11} 
       &                            &                                                                          &                      &                       & \multicolumn{1}{c|}{OLS-NoReg} & \multicolumn{1}{c|}{SPSC-NoReg} & SPSC-Ridge & \multicolumn{1}{c|}{OLS-NoReg} & \multicolumn{1}{c|}{SPSC-NoReg} & SPSC-Ridge \\ \hline

\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 1\\ (12)\end{tabular}}}} & Estimate& -0.120& -0.719& -0.925& -0.906& -0.855& -0.855& -0.906& -0.883& -0.850\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.101& 0.115& 0.114& 0.101& 0.124& 0.120\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.103,-0.709)& (-1.081,-0.629)& (-1.079,-0.632)& (-1.103,-0.709)& (-1.127,-0.639)& (-1.085,-0.615)\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 2\\ (12)\end{tabular}}}} & Estimate& -1.016& -0.878& -0.886& -0.840& -0.787& -0.887& -0.840& -0.790& -0.892\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.089& 0.104& 0.178& 0.089& 0.107& 0.175\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.014,-0.667)& (-0.991,-0.583)& (-1.236,-0.539)& (-1.014,-0.667)& (-0.999,-0.580)& (-1.236,-0.549)\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 3\\ (13)\end{tabular}}}} & Estimate& -0.857& -0.970& -0.823& -0.875& -0.520& -0.798& -0.875& -0.614& -0.798\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.154& 0.235& 0.350& 0.154& 0.212& 0.299\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.177,-0.574)& (-0.982,-0.059)& (-1.484,-0.111)& (-1.177,-0.574)& (-1.030,-0.198)& (-1.384,-0.212)\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 4\\ (12)\end{tabular}}}} & Estimate& -0.863& -0.877& -0.801& -0.922& -0.852& -0.713& -0.922& -0.879& -0.710\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.095& 0.138& 0.264& 0.095& 0.137& 0.286\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.109,-0.735)& (-1.122,-0.581)& (-1.229,-0.196)& (-1.109,-0.735)& (-1.147,-0.610)& (-1.271,-0.150)\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 5\\ (5)\end{tabular}}}} & Estimate& 0.704& -1.203& -0.892& -0.852& -0.887& -0.886& -0.852& -0.868& -0.635\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.120& 0.111& 0.111& 0.120& 0.106& 0.197\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.087,-0.616)& (-1.105,-0.669)& (-1.105,-0.668)& (-1.087,-0.616)& (-1.077,-0.660)& (-1.021,-0.249)\\ \hline
\multicolumn{1}{|c|}{\multirow{3}{*}{{\begin{tabular}[c]{@{}c@{}}Group 6\\ (24)\end{tabular}}}} & Estimate& -0.878& -0.959& -0.874& -0.966& -0.975& -0.830& -0.966& -0.910& -0.797\\ \cline{2-11}
\multicolumn{1}{|c|}{} & ASE& -& -& -& 0.108& 0.101& 0.091& 0.108& 0.096& 0.099\\ \cline{2-11}
\multicolumn{1}{|c|}{} & 95\% CI& -& -& -& (-1.177,-0.755)& (-1.172,-0.777)& (-1.008,-0.651)& (-1.177,-0.755)& (-1.098,-0.722)& (-0.991,-0.602)\\ \hline

\end{tabular}
\caption{Bias and Asymptotic Standard Errors of the Six ATT Estimators under time-invariant $\bg$ and time-varying $\bg_t$. 
...
Next, we conduct the conformal inference on the SPSC with ridge regularization. Following the main paper, we compare the proposed approaches to the recent works by \cite{ASCM2021} and \citet{Cattaneo2021}. Figure \ref{fig:supp:Conformal} visually summarizes the predicted treatment-free outcome $\widehat{Y}_{t}^{(0)}$ and the 95\% prediction intervals for all six donor pool groups. In general, all three approaches produce similar $\widehat{Y}_{t}^{(0)}$ in terms of the shape. However, we find the average width of the prediction intervals over the post-treatment periods are significantly different, which is summarized in Table \ref{tab:supp:PIwidth}. Except for Groups 2 and 4, our approach produces narrower prediction intervals compared to the competing approaches. These additional results from the conformal inference approaches certify the results in the main paper are robust to the choice of the donor pool.
...
\multicolumn{2}{|c|}{ASC}                                                 & 0.388        & 0.198        & 0.210        & 0.066        & 0.582       & 0.263        \\ \hline
...
\end{tabular}
\caption{Average Width of the 95\% Prediction Intervals over the Post-treatment Periods. The numbers in the parentheses show the number of donors in each group. We remark that the results of Group 6 are the same as those in Figure \ref{fig:data:1} of the main paper.}
...
\hline
\multirow{2}{*}{Donor Pool} & \multirow{2}{*}{Statistic} & \multicolumn{6}{c|}{Estimator} \\ \cline{3-8} 
& & \multicolumn{1}{c|}{\makebox[1.8cm][c]{OLS-Standard}} & \multicolumn{1}{c|}{\makebox[1.8cm][c]{SCPI}} & \multicolumn{1}{c|}{\makebox[1.8cm][c]{ASC}} & \multicolumn{1}{c|}{\makebox[1.8cm][c]{OLS-NoReg}} & \multicolumn{1}{c|}{\makebox[1.8cm][c]{SPSC-NoReg}} & \makebox[1.8cm][c]{SPSC-Ridge} \\ \hline


 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 1 (12)}} & Estimate & 0.771 & -0.058 & 0.195 & -0.058 & -0.085 & -0.014 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.025 & 0.058 & 0.053 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.107,-0.010) & (-0.199,0.030) & (-0.119,0.090) \\ \hline
 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 2 (12)}} & Estimate & -0.049 & -0.028 & -0.048 & -0.023 & -0.016 & -0.008 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.015 & 0.025 & 0.048 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.053,0.006) & (-0.065,0.034) & (-0.103,0.086) \\ \hline
 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 3 (13)}} & Estimate & 0.054 & -0.038 & -0.086 & -0.100 & -0.090 & 0.005 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.021 & 0.038 & 0.111 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.142,-0.058) & (-0.165,-0.015) & (-0.212,0.222) \\ \hline
 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 4 (12)}} & Estimate & -0.009 & 0.057 & -0.009 & -0.016 & 0.029 & 0.040 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.014 & 0.122 & 0.243 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.044,0.011) & (-0.210,0.268) & (-0.436,0.517) \\ \hline
 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 5 (5)}} & Estimate & 1.576 & -0.015 & -0.102 & -0.002 & -0.002 & 0.027 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.016 & 0.023 & 0.016 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.034,0.029) & (-0.046,0.043) & (-0.005,0.059) \\ \hline
 \multicolumn{1}{|c|}{\multirow{3}{*}{Group 6 (24)}} & Estimate & -0.013 & -0.010 & -0.008 & -0.003 & 0.030 & 0.000 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & ASE & - & - & - & 0.009 & 0.036 & 0.060 \\ \cline{2-8}
 \multicolumn{1}{|c|}{} & 95\% CI & - & - & - & (-0.020,0.014) & (-0.041,0.102) & (-0.117,0.118) \\ \hline
...
Figure \ref{fig:supp:Conformal Placebo} visually shows the synthetic controls under the placebo treatment. For the proposed SPSC approach, we find 95\% prediction intervals for $\potY{t}{0}$ include the true treatment-free potential outcome $\potY{t}{0}$ except for Group 1. These results suggest that our SPSC approach seems reasonable for analyzing the effect of the 1907 panic on the stock price of the two trust companies. The results from the SCPI estimator largely agree with those of the SPSC approach, whereas the results from the ASC estimator are somewhat different. 
...
%
%Under Assumptions \ref{assumption:consistency}--\ref{assumption:valid proxy} and \ref{assumption:SC NP Cov}, the synthetic control bridge function $h^*$ satisfies the following equation:
...
In this Section, we provide sufficient conditions for the existence of the synthetic control bridge function $h^*$ satisfying Assumption \ref{assumption:SC NP Cov}. We restate the assumption for readability after suppressing covariates:
\renewcommand{\theassumptionNew}{\ref{assumption:SC NP Cov}}
\begin{assumptionNew}[Existence of Bridge Function in the Presence of Covariates] 
For all $t=1,\ldots,T$, there exists a function $h^*: \R^{d} \rightarrow \R$ that satisfies 
\begin{align*}
\potY{t}{0} = \EXP \big\{
h^*(\bW_{\D t} )
\cond
...
\end{align*}
Additionally, as shown in page 5659 of \citet{Carrasco2007}, $\mathcal{K}$ and $\mathcal{K}^{\text{adjoint}}$ are compact operators under Condition \HL{NPSC-2}. Moreover, by Theorem 15.16 of \citet{Kress2014}, there exists a singular value decomposition of $\mathcal{K}$ as $\big\{ \mu_n,\phi_n,g_n \big\}_{n=1,2,\ldots}$. 
...
%In Section \ref{sec:Cov}, we showed that \eqref{eq-Fredholm} is satisfied Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, and \ref{assumption:SC NP Cov}; for readability, we restate Assumption \ref{assumption:SC NP Cov} and \eqref{eq-Fredholm} below:
%\renewcommand{\theassumptionNew}{\ref{assumption:SC NP Cov}}
%\begin{assumptionNew}[Existence of Bridge Function in the Presence of Covariates] 
%For all $t=1,\ldots,T$, there exists a function $h^*: \R^{d} \rightarrow \R$ that satisfies 
%\begin{align*}
...
%and
%\begin{align}
%\EXP \big\{ Y_t - h^*(\bW_{\D t} ) \cond Y_t  \big\} = 0 \ .
%\tag{\ref{eq-Fredholm}}
%\end{align}
%
...
%Therefore, $h^*$ satisfies Assumption \ref{assumption:SC NP Cov}. 

We provide a sufficient condition for the uniqueness of the bridge function. Consider the following the completeness assumption:
\begin{assumption}[Completeness] \label{assumption-complete}
For $t=1,\ldots,T$, suppose $\EXP \big\{ q(\bW_{\D t}) \cond \potY{t}{0} \big\} = 0$ almost surely for a square integrable function $q$. Then, $q(\bW_{\D t} ) = 0$ almost surely for $t=1,\ldots,T$. 	
\end{assumption}
The assumption states that $\potY{t}{0}$ should be $\bW_{\D t}$-relevant for all time periods in the sense that any variation in $\bW_{\D t}$ is captured by variation in $\potY{t}{0}$. 

Let $h_1^*$ and $h_2^*$ be the synthetic control bridge functions satisfying Assumptions \ref{assumption:consistency}, \ref{assumption:noitf}, \ref{assumption:valid proxy Cov}, and \ref{assumption:SC NP Cov}, and \ref{assumption-complete}. We then find $\EXP \big\{ h_1^*(\bW_{\D t}) - h_2^*(\bW_{\D t}) \cond \potY{t}{0} \big\} = 0$, implying $h_1^*(\bW_{\D t})$ and $h_2^*(\bW_{\D t}) = 0$ for all $t$, implying that a function $h^*$ satisfying \ref{assumption:SC NP Cov} is unique. 

We remark that Assumption \ref{assumption-complete} may not be satisfied if the cardinality of the support of $\bW_{\D t}$ is strictly larger than that of $\potY{t}{0}$. For instance, suppose that the outcomes are binary and two donors are available, i.e., $\bW_{\D t} \in \{0,1\}^2$ and $\potY{t}{0} \in \{0,1\}$. Then, the equation in Assumption \ref{assumption-complete} reduces to
...
\subsection{Proof of Theorems \ref{thm:SC}, \ref{thm:ATT}, \ref{thm:Extension NP Cov}, and \ref{thm:Extension NP Cov}}
...
\begin{REG}[Sufficiently Long Pre- and Post-treatment Periods]
\label{assumption-reg-11}
