...
\newcommand{\zhomology}[2]{\tilde{H}_{#1}\left(#2\right)}
\newcommand{\fhomology}[2]{\tilde{H}_{#1}\left(#2 ;\mathbb{F}\right)}
\newcommand{\rhomology}[2]{\tilde{H}_{#1}\left(#2; R \right)}
\newcommand{\homology}[3]{\tilde{H}_{#1}\left(#2; #3 \right)}
\newcommand{\cohomology}[3]{\tilde{H}^{#1}\left(#2;#3\right)}
\newcommand{\fcohomology}[2]{\tilde{H}^{#1}\left(#2;\mathbb{F}\right)}
\newcommand{\zcohomology}[2]{\tilde{H}^{#1}\left(#2\right)}
\newcommand{\rcohomology}[2]{\tilde{H}^{#1}\left(#2;R\right)}
\newcommand{\nrhomology}[2]{H_{#1}\left(#2;R\right)}
\newcommand{\doublehomology}[2]{H^h _{#1}H^v(#2)}
\newcommand{\totalhomology}[2]{H^{\text{tot}} _{#1}(#2)}
\newcommand{\nerve}{N}

\DeclareMathOperator{\rep}{\text{rep}}
\DeclareMathOperator{\boxx}{\text{box}}

\DeclareMathOperator{\lk}{lk}
\DeclareMathOperator{\st}{st}
\DeclareMathOperator{\cost}{cost}
\newcommand{\degree}[1]{\deg_{#1}}
...
\DeclareMathOperator{\intcomplex}{Int}
\DeclareMathOperator{\hcov}{Cov}
\DeclareMathOperator{\hmat}{Mat}
...
\begin{document}
    ...
    \title{Laplacian eigenvalues of independence complexes via additive compound matrices}


    \author{Alan Lew\thanks{\href{mailto:alanlew@andrew.cmu.edu}{alanlew@andrew.cmu.edu}.}}
    \affil{Dept. Math. Sciences, Carnegie Mellon University, Pittsburgh, PA 15213, USA}
...
The simplicial Hodge theorem states that for any $w:V\to\Rea_{>0}$, $\Ker(\lapw{k}{I(G)}{w})$ is isomorphic to the $k$-th homology group $\tilde{H}_k(I(G);\Rea)$. The definition of $\lapw{k}{I(G)}{w}$ can be extended to the case of non-negative weight functions $w$, in which case we have $\dim(\tilde{H}_k(I(G);\Rea))\leq \dim(\Ker(\lapw{k}{I(G)}{w}))$ (see Lemma \ref{lemma:homology_from_eigenvalues_degenerate_case}).

For any matrix $M\in \Rea^{n\times n}$ with real eigenvalues and $1\leq i\leq n$, we denote by $\eigsmallest{i}{M}$ the $i$-th smallest eigenvalue of $M$ and by $\eiglargest{i}{M}$ its $i$-th largest eigenvalue (so that $\eiglargest{i}{M}=\eigsmallest{n+1-i}{M}$). For any $k$, let
\[
...
Here, we continue the study of the spectra of high dimensional Laplacian operators on independence complexes.
%relation between the spectrum of the graph Laplacian on $G$ and that of higher dimensional Laplacians on its independence complex.
Our main result is the following extension of Theorem \ref{thm:ABM_main}.
...
%The following bound on the homological connectivity of an independence complex follows from Theorem \ref{thm:bettinumbers_indepedence_complex}.
...
\]
%In the special case where $w(\sigma)=1$ for all $\sigma\in X$, we denote $\lapw{k}{X}{w}=\lap{k}{X}$.
...
Let $G=(V,E)$ be a graph. The \emph{clique complex} (also know as the flag complex) of $G$ is the simplicial complex $X(G)$ on vertex set $V$ whose simplices are the cliques of $G$ (i.e. vertex subsets forming a complete subgraph). Note that for any graph $G$, $I(G)=X(\bar{G})$, where $\bar{G}$ is the graph complement of $G$.
...
%Next, we prove Theorem \ref{thm:bettinumbers_indepedence_complex} and its Corollary \ref{cor:connectivity}.
...
\end{align*}
Since this holds for any fractional quadratic packing of $G$, we obtain $\eta(I(G))\geq \lceil \rho^*_q(G)\rceil\geq \rho^*_q(G)$, as wanted.
\end{proof}
...
Then, for any $I\in\binom{V}{k+1}$, we have 
\[
...
\label{sec:conc}
\todo[inline]{Write here questions about whether we can use this method to relate $k$ and $k+1$ spectral gaps of clique complex as in ABM, and also whether we can extend these results to generalized clique complexes, or to the original garland's method (perhaps cite the paper with garland bounds on betti numbers from \cite[Theorem 2.5]{hino2019asymptotic} for a result in a similar flavour)}
\end{comment}
