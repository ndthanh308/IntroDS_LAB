...
\end{align*}
where $\bar{i}(k)=\max\Bp{i\in[i_0 - 1]\mid \Delta_k\leq 2^{-i}}$ and it is well-defined since $i_0 = \ceil{\log_2(1/\Delta_{1})}+1$. Here, the last inequality use the same simple fact about $t\mapsto\frac{t^2}{at+b}$.
...
\begin{align*}
    ...
    &\leq 2i_0 KT \min_{\ve{a}\in\mc{D}} \max_{k\in[K]}\exp\Sp{-\frac{3Ta_{i(k)}^2\Delta_k^2}{48m\sum_{i'=1}^{i(k)}(a_{i'}-a_{i'-1})f(\mc{A}_{i'-2})+8(m\sqrt{d f(\mc{X})}+1)a_{i(k)}\Delta_k}},
\end{align*}
...
\end{theorem}
\begin{proof}
    For $i\in\Bp{1, \dots, i_0-1}$, we take $a_i=\frac{\Delta_1}{\Delta_{\bar{k}(i)}}$, where $\bar{k}(i)=\min\Bp{k\in[K]\mid \Delta_k\geq\frac{2^{-i}}{2}}$. Then, since $\bar{i}(k)=\max\Bp{i\in[i_0]\mid \Delta_k\leq 2^{-i}}$, for any $k\in[K]$, we have $\frac{2^{-\bar{i}(k)}}{2}\leq\Delta_{\bar{k}(\bar{i}(k))}\leq\Delta_k$, which further implies 
...
In the following corollary, we show that the above simplified complexity is in an same order (up to logarithmic factors) of $H_{\mathrm{BOB}}$ proposed in \citet{abbasi2018best}.
\begin{corollary}
...
\begin{proof}
    ...
    Since $\lambda_t=\frac{\bar{\lambda}_t+\lambda^*}{2}$ and $\lambda\mapsto\Norm{x}^2_{A(\lambda)^{-1}}$ is convex in $\lambda$, to use the Berstein's inequality for martingale differences\citet{freedman1975tail}, we have
