...
\subsection{A Modified Parameter-free Robust Algorithm}
\label{sec:modified_algo}

In this section, we briefly explain how we modify \textsf{P1-RAGE} to get a parameter-free and computationally efficient algorithm \textsf{P1-Peace}. First, to compute the allocation strategy in a parameter-free way, we propose another subroutine, called \textsf{Peace-Elimination}, based on the elimination strategy in \textsf{Peace} \citet{katz2020empirical}, which has the same spirit as \textsf{RAGE}. Similar to \textsf{RAGE-Elimination}, \textsf{Peace-Elimination} also repeatedly computes $\mc{XY}$-allocation, but (virtually) eliminate arms so that the value of the remaining arms' optimal $\mc{XY}$-design is halved. In addition, in \textsf{P1-Peace}, we only update the sampling distribution $\lambda_t$ after a period of time. The intuition is that if the environment is stationary, then we do not need to update our allocation probability frequently just like \textsf{RAGE} and \textsf{Peace}; if the environment is non-stationary, then the non-stationarity is handled by the mixed G-optimal design $\lambda^*$, which is fixed from the very beginning. Therefore, updating $\lambda_t$ in a low frequency should not severely harm the performance. The new algorithm and elimination subroutine are summarized in Algorithm \ref{algo:p1_peace}.
...
\begin{algorithm}[ht]
    ...
    \myproc{\proc{$\htheta_t$}}{\label{algo:peace_elimination}
        ...
        \While{$|\X_{t}^{(i)}|> 1$}{
            Compute $\bar{\lambda}^{(i)}_t\leftarrow\arginf_{\lambda\in\triangle_{\X}}\rho(\X_t^{(i)}, \lambda)$\\
...
%     \SetAlgoLined
%     \KwIn{estimated parameter $\htheta_t$}
