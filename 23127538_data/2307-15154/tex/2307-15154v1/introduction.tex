...
As the use of these methods has increased, low-hanging fruit is more rare and these technologies are increasingly being utilized to determine problems with smaller effect sizes, while also targeting smaller audiences. These two competing trends of smaller effect sizes and smaller sample sizes make it increasingly challenging to obtain  statistical significance and correct inferences since the absolute number of observations is limited. 
This tension is leading to the increased use of \emph{adaptive} sampling schemes like multi-armed bandits that aim to obtain the same statistical insights using fewer total observations.
...
Under such an environment, it's also possible for the underlying performance of treatements to wildly change, and as a result, the treatment that is best performing on any given day may change. %As a result, it 
...
That is, the treatment that would have received the \emph{highest total reward had it received all the samples}. 
...
Fortunately, randomization is a powerful tool to provide the next best thing: un-biased \emph{estimates} of how a treatment would behave as if it had been used at every time in the past. 
...
However, if the treatment performances are time-varying, it is challenging to determine how one might do this optimally.
...
Given $n$ possibilities, this results in $\binom{n}{6}$ total distinct treatments to A/B test!
...
Note that this these sums can be succinctly written as $\langle x, \theta \rangle$ for $\theta = (\theta^{(0)},\theta^{(1)},\theta^{(2)})^\top \in \R^{1+n+\binom{n}{2}}$ and an appropriate $x \in \{0,1\}^{1+n+\binom{n}{2}}$. 
...
A similar situation arises if we have features or covariates describing each possible treatment. 
...
In these kinds of scenarios--whether it be multivariate testing or items with feature descriptions--we would like to perform adaptive experimentation in the presence of time-variation. 
