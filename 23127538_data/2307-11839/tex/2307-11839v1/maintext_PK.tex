%Preprint
%\documentclass[aps,pre,onecolumn,amsmath,amssymb,superscriptaddress,preprint,longbibliography]{revtex4-2}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\usepackage{changes}
%\definechangesauthor[name=PK, color=blue]{pk}
%\definechangesauthor[name=HM, color=red]{hm}
%
%\newcommand{\peter}[1]{{\color{blue}#1}}
%\newcommand{\hamid}[1]{{\color{red}#1}}
...
due to their intrinsic electric dipole moment which, among other consequences, 
triggers Rashba band splitting. 
...
For example, 
Rashba band splitting occurs if a potential gradient arises normal to the monolayer. 
In the conventional TMDCs, this could for example be achieved by an applied electric field normal to the monolayer~\cite{nanoscale_cheng_16-rashbafeild} or by assembling a bilayer of different TMDC monolayers~\cite{prb_lin_2019_rashbatmdc}. 
...
The numerous structural variants, differing both in their chalcogen atom sequence and in the registry between the upper and lower  layer, offer many more possibilities to tune the band alignment and local electric field, and thus the Rashba parameter. 
...
Eventually, the intrinsic electric field strength gets modified due to the change in the amount of negative charge accumulated on the chalcogen atom, which leads to a change in the Rashba parameter. 
...
have been observed~\cite{apl-Plechinger-12}. Puretzky {\it et al.}~\cite{acsnano-puretzky-15} detected shear and layer-breathing rigid vibrations of monolayers in homo- and hetero-bilayer TMDCs. A strong dependence of vibration frequencies to the types of junction and stacking orientation was observed and supported by several follow-up studies~\cite{apl-elis-11,apl-holler-20,apl-ufuk-22} on similar bilayer structures that may even include Janus monolayers.
...
The BFGS~\cite{Fletcher87} method is employed to relax the atomic positions, and the maximum residual force component per atom was set at $5.0 \times 10^{-3} \text{eV/\AA}$. 
...
\end{equation}
%
where $E_{\text{tot}}$ is the total energy of the heterostructure, and $E_{\text{MoXY}}$ and $E_{\text{WXY}}$ are total energies of the isolated Janus MoXY and WXY monolayers, respectively. The contribution of the van der Waals (vdW) interaction ($E^{\text{vdW}}$) in forming the bilayer heterostructure is quantified by 
re-computing the binding energy disregarding the van der Waals interaction. The binding energy difference of the bilayers relaxed with and without vdW interaction gives an estimation of $E^{\text{vdW}}$.
...
From a theoretical perspective, the 1T$'$ phase of an isolated WSeTe monolayer can be constructed starting from 2H by rotating the Te atomic plane by $60^{\circ}$ along the $z$ direction, see Fig~\ref{fig-bilayer-different-phases}.
...
{TeMo{\bf SeTe}WSe} & {$d_\text{inter}~(\text{\AA}$) ($\%$)} & {$4.06~ (+4.6)$} & {$3.30~ (+2.4)$} & {$3.36~ (+0.1)$} & {$3.30~ (+1.4)$} & {$3.49~ (+0.0)$} & {$4.01~ (+4.5)$} \\
  ...
  \\\bottomrule
\end{tabular}
\caption{Thermodynamic stability of Janus MoSeTe/WSeTe bilayer with different chalcogen atom sequences (CAS) and stacking patterns. The chalcogen species forming the junction  inside the bilayer are printed in boldface. The binding energy $E_\text{b}$ (in eV) using different dispersion methods (TS and MBD) and the distance $d_\text{inter}$ (in $\text{\AA}$) between Janus monolayer of each bilayer structure are presented for the MBD method. The changes (in percent) in inter-layer distances compared to the TS method are given in parentheses. The distance between monolayers after subtraction of the radii of atoms at the junction $d^{\text{sub}}_\text{inter}$ are also shown.}
    \label{table_thermal}
    \end{table*}
 
\par
As pointed out above, the AA and AB-M stacking patterns are the least stable (AA is the least stable and the AB-M the second least stable) and they have the largest inter-layer distance $d_{\text{inter}}$, placing the Janus monolayers further apart. For AA stacking, chalcogen atoms, as well as transition metal atoms (of the two Janus monolayers), are at the same registry on top of each other, while for AB-M stacking the chalcogens share the same position in the horizontal plane, see Fig.~\ref{fig-sketch}. 
Given the same in-plane positions of atoms, a larger space between the MoSeTe and WSeTe layers is required. 
This is because the electron lone-pair orbitals of the chalcogens, pointing normal to the plane, repel each other. 
...
As seen from Table~\ref{table_thermal}, the inter-layer distances in the MBD treatment are often larger than in the TS methods; this becomes particularly clear for the AA and AB-M stacking. 
...
This is corroborated by the pronounced distance increase upon using the MBD for CAS-3 (Se1-Se2 junction) where the intrinsic electric dipole moment vectors point toward each other. The larger negative charges accumulated at the junction Se atoms 
give rise to an enhanced screening of vdW dipolar interactions, which is accounted for in the MBD method. 
We recall that in the TS treatment, the Se1-Se2 junction for some stackings was found to be the most stable junction; in hindsight, we judge this to be a faulty overestimation of binding energies by the TS method. 
...
\subsection{Lattice Vibrations of MoSeTe/WSeTe Bilayers}
...
\par
For every MoSeTe/WSeTe bilayer geometry, the phonon energy band dispersion was calculated. Since there are no negative frequencies in our calculated vibration spectra of the bilayer geometries, one can conclude that the relaxed geometries are dynamically stable. 
...
%
Due to C$_{\text{3v}}$ structural symmetry of Janus monolayer, the in-plane modes are doubly degenerate. For the MoSeTe/WSeTe bilayer, there are two extra low-frequency vibrations assigned to the gliding and vertical rigid motions of MLs with respect to each other due to coupling caused by the dipole-dipole interactions. These two modes are the shear (S) and layer-breathing (LB) modes which emerge in the low-frequency range below the acoustic region of the vibration spectrum of multi-layer TMDCs. Since rotational C$_{\text{3v}}$ symmetry is preserved in the MoSeTe/WSeTe bilayer, the shearing mode is double degenerate. Also, the frequency of the shear mode is calculated to be lower than that of the layer-breathing mode since the dipole-dipole potential energy surface has greater curvature normal than parallel to the bilayer junction. In cases where the bilayer distance is very large (or the bilayer is less commensurate), the S modes may not be detectable 
in the vibration spectrum. From an experimental perspective~\cite{apl-holler-20}, the S mode is considered a signature of commensurate bilayer stacking. Hence, its absence in the vibration spectrum indicates clearly the lack of restoring force and thus a weak ML binding or a twist in the stacking pattern. Low-frequency Raman spectroscopy is one option to detect such low-energy vibrations~\cite{apl-holler-20,apl-ufuk-22,acsnano-puretzky-15}. 
...
For every atomic sequence considered in the present work, AA and AB-M stacking orientations appear to have only LB modes, see Table~\ref{table_freqallstacking}. This resonates well with thermodynamic stability (see Table~\ref{table_thermal}) analyses pointing to thermodynamically less stable AA and AB-M stackings among the six possible staking orientations for every atomic sequence. This is also in good agreement with low-energy Raman spectroscopy measurements~\cite{apl-holler-20}, which suggest that the S mode is missing for AA stacking (R-type stacking) of a hetero-bilayer, while there is a non-zero frequency (above $10~\text{cm}^{-1}$) assigned to the LB mode, see Table~\ref{table_freqallstacking}. 
This indicates that the restoring force of rigid gliding vibration is weaker at the AA stacking, which is a clear manifestation of less strong binding of MLs, see Table~\ref{table_thermal}. Also, except for CAS-2, for every atomic sequence, the S mode of AA-M (3R type) occurs at lower frequencies compared to the AB (2H type) S mode. This is similar to the experimental results obtained for the homo-bilayer of MoSe$_2$~\cite{acsnano-puretzky-15}. 

\par
The systematic dependence on the chalcogen atom sequence (CAS) is shown in Figure~\ref{fig-vib-s-mode-vs-d-diff-cas} for both the change in inter-layer distance and the energy of the shearing (S) mode. Only four stacking patterns are included (panels (a) to (d)), since the remaining two stackings, AA and AB-M, do not show any S modes since the parallel component of the restoring (dispersion) force is too weak for those stackings,  as pointed out earlier. 
We want to stress that the 
inter-layer distance $d$ and the S mode frequency $\omega_{\text{S}}$ are anti-correlated when varying the CAS. For CAS-1 and CAS-4 bilayers, both having non-zero net electric dipole moments and thus strong attraction, $d$ becomes shorter. Since the MLs are coupled stronger by this  attraction caused by the stronger dispersion forces, stiffer horizontal rigid motions of the MLs are expected, and thus the S mode frequency increases. This line of argument rationalizes the trends visible in Fig.~\ref{fig-vib-s-mode-vs-d-diff-cas}. 
...
% Using the many-body dispersion (MBD) to treat the dispersion forces, sometimes the coupling between the A$_1$ modes of the monolayers is very weak, such that at the $\Gamma$ point the vibrations appear to be degenerate. This is the case for the CAS-1:AA-X bilayer, for which the out-of-plane A$_1$ modes both have the frequency of $198.0~\text{cm}^{-1}$. However, using the Tkatchenko-Scheffler (TS) method, the corresponding A$_1$ frequencies for MoSeTe and WSeTe are $197.6$, and $199.3~\text{cm}^{-1}$, respectively (not shown in Table~\ref{table_freqallstacking}).
...
\begin{tabular}{SSSS} \toprule
 {Atomic sequence} & {Stacking} & {High-frequency Raman modes  $(\text{cm}^{-1})$ } & {Low-frequency Raman modes  $(\text{cm}^{-1})$ } \\
   {}  &   {} & { $E^{1}_1$, $A^{1}_1$, $E^{2}_1$, $A^{2}_1$} & {S, LB}\\\midrule\midrule

{Isolated MoSeTe} & {} & {$134.8$, \hspace{1.00cm} $197.1$, \hspace{1.00cm} $257.1$, \hspace{1.00cm} $321.3$ \hspace{0.9cm} } & {$-$, $-$} \\
{Isolated WSeTe} & {}  & {\hspace{0.50cm} $(137.1)$, \hspace{0.8cm} $(198.9)$, \hspace{0.80cm} $(221.1)$, \hspace{0.80cm} $(277.7)$}  & {$-$, $-$} \\
{} & {AA} & {$134.7~(137.1)$, $196.3~(199.3)$, $256.9 ~(221.0)$, $319.2~ (277.1)$}   & {$-$,    $18.3$}  \\
...
{}         & {AB-M}   & {$134.7~ (137.2)$, $196.3~ (199.1)$, $257.2~ (221.2)$, $319.8~ (277.8)$} & {$-$, $17.4$}    \\ 
\midrule

{}         & {AA}     & {$134.7~ (137.0)$, $196.2 ~(198.4)$, $257.0~ (221.2)$, $319.4~ (277.8)$} &  {$-$,    $17.9$}  \\
...
{}         & {AB-X}   & {$134.7~ (137.1)$, $196.1~ (198.7)$, $257.0~ (221.1)$, $320.1~ (278.0)$} &  {$-$,    $17.3$}  \\

{}         & {AB-M}   & {$134.7~ (137.0)$, $195.1~ (198.1)$, $257.2~ (221.3)$, $319.8~ (277.9)$} &  {$-$,    $22.4$}  \\ \midrule
{}         & {AA}     & {$134.7~ (137.1)$, $196.7~ (199.2)$, $256.8~ (220.9)$, $319.7~ (277.2)$} &  {$-$,       $17.9$}  \\
...
{}         & {AB-M}   & {$134.8~ (137.1)$, $197.4~ (201.0)$, $257.1~ (221.0)$, $318.5~ (277.6)$} &  {$-$,       $18.3$}  \\
\midrule
{}         & {AA}     & {$135.4~ (137.1)$, $197.0~ (201.4)$, $260.7~ (223.9)$, $324.1~ (282.3)$} &  {$-$,    $24.4$}  \\
...
{}         & {AB-M}   & {$134.9~(137.0)$, $196.9~(198.7)$, $257.2~ (221.2)$, $319.4~ (277.3)$} &  {$-$,    $18.6$}  \\ \bottomrule
...
%   {structure} & \multicolumn{4}{c|}{Raman modes  $(\text{cm}^{-1})$, TS}  & \multicolumn{4}{c}{Raman modes  $(\text{cm}^{-1})$, MBD}\\
%   {} & $\text{E}^1_1$ & $\text{A}^1_1$ & $\text{E}^2_1$ & $\text{A}^2_1$  & $\text{E}^1_1$ & $\text{A}^1_1$ & $\text{E}^2_1$ & $\text{A}^2_1$      \\ \midrule\midrule
%{MoSeTe} & 136.1 & 195.3 & 259.5 & 320.1 & 134.8 & 197.1 & 257.1 & 321.3 \\
%{WSeTe}  & 138.3 & 198.6 & 223.0 & 278.1 & 137.1 & 198.9 & 221.1 & 277.7  \\\bottomrule
%\end{tabular}
%\caption{Frequencies of the Raman-active modes for isolated Janus MoSeTe and WSeTe MLs obtained with pair-wise additive Tkatchenko-Scheffler (TS) and many-body-dispersion (MBD) treatments of dipole-dipole atomic interactions.}
...
%PK
To illustrate this point, we discuss one example, the MoSeTe/WSeTe bilayer (CAS-1: AB). In Fig.~\ref{fig-ele-bands}, we show first the band structures of the two monolayers MoSeTe and WSeTe on their own, 
...
\end{tabular}
\caption{Electronic properties of Janus MoSeTe/WSeTe bilayer with different chalcogen atom sequences (CAS) and stacking patterns. The chalcogen species forming the interface inside the bilayer are printed in boldface. The electric dipole moment density $D_\text{tot}$ (in $e/\text{\AA}$), band gap $E_\text{g}$ (in eV), and offset of the conduction band minimum $\Delta E_\text{CBM}$ (in eV) at the high-symmetry point $K$ are listed. The values for $E_\text{g}$ are underlined for direct band gaps.}
    \label{table_electronics}
    \end{table*}

%PK
\par
Next, we turn to a general discussion of the electronic structure of the bilayers for all possible chalcogen atom sequences and stackings. An overview of the data is given in Tab.~\ref{table_electronics}, which also contains the total  areal electronic dipole densities of the MoSeTe/WSeTe bilayers studied in this work. 
Since the isolated Janus MoSeTe and WSeTe monolayers lack out-of-plane inversion symmetry and thus polar, they possess already dipole moments densities of $-2.51$ and $-2.40~10^{-2}\text{e}/\text{\AA}$, respectively. If the interface inside the bilayer consists of different atomic species Se and Te, as for CAS-1 and CAS-4, the intrinsic dipole vectors of the MLs point in the same direction, adding up to nearly the sum of individual dipole moments, 
...
\end{tabular}
\caption{Valence band splitting at the high-symmetry point $K$ $\Delta E^{\text{SOC}}_V$ for Mo and W atoms. The numbers for W are given in parentheses.}
...
The $E^{\text{SOC}}_V$ values are larger than the conduction band splitting since the d$_{x^2 -y^2}$ orbitals with the large magnetic quantum number of $m_l = \pm 2$ form the  valence band edge at K~\cite{PhysRevB-kosmider-14}. The spin-orbit term in the  Hamiltonian is proportional to $Z^4/n^3$, where $Z$ and $n$ are atomic and valence shell numbers, respectively. As the W atom has a greater atomic number than Mo, $\Delta E^{\text{SOC}}_V$ is larger for W than Mo. Also, the splitting is almost insensitive to the atomic sequence (CAS) and stacking. This suggests that such a band splitting originates to a large extent from the potential near the nuclei and the orbital angular momentum of the electrons, with just a very slight contribution from the electrostatic potential gradient across the bilayers. However, for AB-stacked MoSeTe/WSeTe bilayer, notable (few tens of meV) deviations of the valence splitting are observed for both Mo and W atoms. For example,  $\Delta E^{\text{SOC}}_V$ for W is nearly $~50~\text{meV}$ larger when the CAS-2 bilayer has AB stacking.

\par
Larger band splitting alongside with large enough band gap ($\Delta$ in Hamiltonian $H_{\text {Dirac }}=\tau k_x \sigma_x + k_y \sigma_y + \Delta \sigma_z$ describing a 2D Dirac system) may trigger a valley Hall effect (VHE) and thus an effective valley polarization of electrons of opposite spins. 
Such an imbalance of charges can be triggered by applying an electric field or shining a circularly polarized light. Therefore, in the Janus bilayer, due to the existence an electric field and a non-zero band gap, with their magnitude depending strongly on the atomic sequence and stacking orientation, the VHE can be well modulated by changing the type of atomic sequence (junction) and stacking orientation. This suggests that the heterostructuring with specific stacking (such as AB stacking orientation) can be used to modulate the VHE, thus paving the way toward making more efficient valleytronic devices.
...
Table~\ref{table_electronics} shows $\Delta E_{\text{CBM}}$ for the full collection of bilayers studied in the present work. Depending on the sequence of chalcogen atoms in the bilayer and the ML stacking, the magnitude of the CBM offset changes. Since the transition metal atoms have major contributions to the conduction band edges of the bilayer, the CBM offset can to a large extent be determined by the potential gradient between the transition metal atoms. The CBM offset becomes notable for CAS-4, in which {\em two different}  chalcogen species form the junction. This can be explained by the fact that for CAS-4, the interface 
produces a potential drop over a short distance (almost $3.3$ to $4.0~\text{\AA}$), thus a notable dipole moment pointing in the $-z$ direction is generated, see Fig.~\ref{fig-dipole-aa-ab-stackings}. This enhances the impact of the polarization of the transition metal atom,  pushing up further the W d orbitals contributing to the CBM, and thus the offset becomes larger. The CAS effect becomes slightly more pronounced when the chalcogen atoms at the interface sit on top of each other (AA and AB-M stacking, see Fig.~\ref{fig-sketch}), where the potential gradient is oriented normal to the interface. For sufficiently large CBM offset, efficient  charge separation can take place, yielding long-lived interlayer excitons. 
...
{TeMoSeTeWSe} & {$\alpha~(\text{meV \AA})$} & {620.2}  & {212.0} & {230.2} & {223.8} & {212.6} & {337.7}  \\
 \bottomrule
\end{tabular}
\caption{Rashba energy $\Delta E$ (in eV), inverse vector offset $\Delta k$ (in $\text{\AA}^{-1}$), and Rashba parameter $\alpha$ (in eV $\text{\AA}$) due to Rashba band splitting at high-symmetry point $\Gamma$.}
...
%
In general, this effect may occur in a two-dimensional system in the presence of SOC if inversion symmetry is broken. 
The latter condition is often realized by applying an electric field to a 2D layer. 
 In our present study, the inversion symmetry is broken by the Janus structure itself, and the dipole density provides an intrinsic electric field that drives the Rashba splitting.
 
While the effect of SOC is fully included in our first-principles treatment, it can be instructive to consider simple toy models where one starts from an electronic band structure disregarding SOC and then adds it in a second step by a Hamiltonian term 
\begin{equation}
H_{\mathrm{R}}=\alpha\left(k \times \sigma\right)_{z},
\label{eq:RashbaHam}
\end{equation}
where $\alpha$ is the so-called Rashba constant. When the above Bychkov-Rashba Hamiltonian is added to the Hamiltonian of a free particle with effective mass $m$, the electronic bands take the form 
\begin{equation}
...
From the fit parameters, we calculate the Rashba constant 
...
As a result, we have the $\left<\sigma_{x,y,z} \right> = (\cos\varphi, -\sin \varphi, 0)$. 
...
\par
Depending on the direction of the electric field of the monolayer, in-plane rotation of the spin vector can change around the high-symmetry point $\Gamma$. This can be well observed by looking at the non-zero in-plane components of the spin vector of spin-up and spin-down bands shifted by the Rashba SOC to the right and left, respectively. Figure~\ref{fig-ele-spin-texture} displays the expectation of the $\sigma_x$ Pauli matrix around the $\Gamma$ point for the MoSeTe/WSeTe bilayers with different chalcogen atom sequence. In CAS-2, CAS-4, and CAS-4 bilayers, the VM bands are localized on the WSeTe ML, while the VM band in CAS-1 belongs to the MoSeTe ML. In CAS-2 and CAS-3 bilayers, the electric dipole moment of WSeTe ML is directed upward, so the spin vectors of spin-up electrons (right-shifted bands) undergo the same rotations around the $\Gamma$ point. In contrast, the spin vector rotates (of spin-up electrons) in a different direction for CAS-3, where the electric dipole moment vector of the WSeTe ML is pointing down. 


\par
As pointed out earlier, SOC, being a relativistic effect, strongly increases with the atomic number of the atom ($\Delta E_{\text{SOC}}\propto Z^4/n^3$). 
This effect can be seen clearly in the large splitting at the high symmetry point K, see Table~\ref{table_electronics_splitting}. Since both transition metal and Se atomic orbitals contribute to the valence band edge at the $\Gamma$ point, the $d_{z^2}$ and $p_z$ orbitals (or their relativistic counterparts) play important roles in the splitting. 
A previous theoretical study suggests that the contribution to the SOC is larger from $d_{z^2}$ than $p_z$~\cite{prb-yao-17}. Also, a stronger Rashba-type splitting (greater Rashba parameter $\alpha$) is expected for the monolayer containing the heavier atoms. 
...
Indeed, the Rashba $k$-offset and Rashba energy differences are $0.146~\text{\AA}^{-1}$ and $42.2~\text{meV}$ ($\alpha = 577.9~\text{meV}\text{\AA}$) for an isolated Janus WSeTe ML. %, respectively. 
...
The Rashba $k$-offset, energy difference, and $\alpha$ of the bilayers studied in the present work, obtained from fitting eq.~(\ref{eq:eps_rashba}) to the DFT+SOC band structures, are listed in Table~\ref{table_electronics_rashba}. When the electric dipole moments are lined up, like in the AA and AB-M stacking patterns, the generated electric fields are uniform and the energy band dispersion is nearly symmetric around the $\Gamma$ point. Hence, the Rashba parameters are not very sensitive to the directions along a specific $k$-path for bilayers with the same registry of the chalcogen atoms. For the other stacking patterns, a slight anisotropy is observed, and we report averages of the Rashba parameters for both bands. 

\par
It is seen that the values of $k_{\text{R}}$ and $E_{\text{R}}$ depend strongly on the chalcogen atomic sequence as well as on the stacking. For the stacking patterns AA and AB-M when dipole moments are aligned, the resulting electric field vectors are extremal 
and the Rashba effect becomes strongest, i.e. larger $k$-offsets and energy differences occur. In Table~\ref{table_electronics_rashba}, the special role played by the AA and AB-M stacking patterns is clearly seen, and the variation of the chalcogen atom sequence is shown. 
Given that the total electric dipole moment vanishes almost to zero for CAS-2 and CAS-3 geometries and grows to the extremes for CAS-1 and CAS-4, it is expected that the Rashba parameters become larger for CAS-1 and CAS-4 than for CAS-2 and CAS-3. One can find such expectation fulfilled to a large extent by larger $E_{\text{R}}$ and $k_{\text{R}}$ for CAS-1 and CAS-4 throughout the stackings studied in this work, see Table~\ref{table_electronics_rashba}. 
%
The few deviations from this trend can be attributed to the non-uniform electric fields (cf. Fig.~\ref{fig-dipole-aa-ab-stackings}) generated due to shifted layers of chalcogen and transition atoms of the two MLs. 
For the CAS-3 bilayer, where the electric fields of the MLs cancel each other, the band splitting at the $\Gamma$ point almost disappears and thus the degeneracy is restored, see Fig.~\ref{fig-bands-cas-3}. For a few stacking patterns (CAS1:AB and CAS-4:AA), the Rashba parameters can be close to or even larger than the parameters of isolated monolayers. Therefore, a well-selected stacking pattern may provide a bilayer structure with suitable Rashba parameters for specific applications that require the generation of a well-tuned spin current. One may find merit in the variation of atomic order and stacking as suitable tools for fine-tuning the Rashba parameter. 
...
% This figures shows the spin texture (expectation value of Pauli matrix along x) for four sequences
...
\par
Funded by the Deutsche Forschungsgemeinschaft (DFG, German Research Foundation) through project B02 of Collaborative Research Center SFB1242 'Nonequilibrium dynamics of condensed matter in the time domain' (Project-ID 278162697). 
The authors also  gratefully acknowledge the funding of this project by computing time provided by the Paderborn Center for Parallel Computing (PC2). 
The authors would like to thank Gabrielle Koknat from Duke University for helping with a part of the coding.
...
\bibitem [{\citenamefont {Holler}\ \emph {et~al.}(2020)\citenamefont {Holler},
  ...
  and\ \citenamefont {Sch{\"u}ller}}]{apl-holler-20}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {J.}~\bibnamefont
...
\bibitem [{\citenamefont {Erkılıç}\ \emph {et~al.}(2022)\citenamefont
  {Erkılıç}, \citenamefont {Wang}, \citenamefont {Sekine},\ and\
  \citenamefont {Taniyasu}}]{apl-ufuk-22}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {U.}~\bibnamefont
  {Erkılıç}}, \bibinfo {author} {\bibfnamefont {S.}~\bibnamefont {Wang}},
...
\bibitem [{\citenamefont {Togo}\ and\ \citenamefont {Tanaka}(2015)}]{phonopy}%
  ...
  in materials science},\ }\href@noop {} {\bibfield  {journal} {\bibinfo
  {journal} {Scr. Mater.}\ }\textbf {\bibinfo {volume} {108}},\ \bibinfo
  {pages} {1 } (\bibinfo {year} {2015})}\BibitemShut {NoStop}%
\bibitem [{\citenamefont {Fang}\ \emph {et~al.}(2015)\citenamefont {Fang},
  \citenamefont {Defo}, \citenamefont {Shirodkar}, \citenamefont {Lieu},
  \citenamefont {Tritsaris},\ and\ \citenamefont {Kaxiras}}]{Fang2015}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {S.}~\bibnamefont
  {Fang}}, \bibinfo {author} {\bibfnamefont {R.~K.}\ \bibnamefont {Defo}},
...
\bibitem [{\citenamefont {Zhang}\ \emph {et~al.}(2015)\citenamefont {Zhang},
  ...
  {\bibinfo  {journal} {Nano Letters}\ }\textbf {\bibinfo {volume} {15}},\
...
\bibitem [{\citenamefont {Terrones}\ \emph {et~al.}(2013)\citenamefont
  ...
  {volume} {3}},\ \bibinfo {pages} {1549} (\bibinfo {year} {2013})}\BibitemShut
  {NoStop}%
...
\bibitem [{\citenamefont {Feng}\ \emph {et~al.}(2019)\citenamefont {Feng},
  ...
  {Arita}, \citenamefont {Schwier}, \citenamefont {Shimada} \emph
  {et~al.}}]{natcomm-feng-2019-rashba}%
  \BibitemOpen
  \bibfield  {author} {\bibinfo {author} {\bibfnamefont {Y.}~\bibnamefont
  ...
  {\bibfnamefont {K.}~\bibnamefont {Shimada}}, \emph {et~al.},\ }\bibfield
  {title} {\bibinfo {title} {Rashba-like spin splitting along three momentum
  directions in trigonal layered {PtBi}$_2$},\ }\href@noop {} {\bibfield
  {journal} {\bibinfo  {journal} {Nature communications}\ }\textbf {\bibinfo
  {volume} {10}},\ \bibinfo {pages} {4765} (\bibinfo {year}
