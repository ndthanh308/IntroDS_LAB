...
it still requires large amounts of data to induce correct generalizations~\citep{lake_ullman_tenenbaum_gershman_2017}.
...
Research in compositional generalization has two main aspects: evaluating the current models' compositional abilities as well as improving them. In terms of evaluation, some studies use artificially created test sets that mimic arithmetic-like compositionality~\citep{LakeB18}, while others evaluate compositionality in a more natural way~\citep{KeysersSSBFKMSS20,KimL20,DankersBH22}. In terms of improvement, earlier work aimed to enhance the models' compositional abilities on tasks such as semantic parsing datasets~\citep{QiuSPNLST22}, math word problem solving~\citep{abs-2209-01352}, data-to-text generation~\citep{MehtaRTKPS22}, and classification~\citep{KimRAO20}.
...
To exemplify, given the De-En sentence pair <\emph{Sie hat Rom besucht, She visited Rome}>, we replace nouns with variables: <\emph{$X_1$ hat $X_2$ besucht, $Y_1$ visited $Y_2$}>. 
...
\fi
We discuss different criteria to replace phrases with variables in \S\ref{section3.2}.
...
Finally, we add the variable-induced corpus to the original training set, effectively doubling its size.\footnote{The code will be released on GitHub upon acceptance.}
...
For the preliminary experiments, we use the TED data from the IWSLT 2014 German-English~(De-En) shared translation task~\citep{CettoloNSBF14} and randomly sample 
 %5k, 10k, 20k, and 40k sentence pairs 
from the training data to represent various low-resource settings. 
...
In order to evaluate the compositional ability of JD, following~\citet{DankersBH22} we use an English-Dutch~(En-Nl) training set from OPUS and randomly sample to create low-resource sets.

We use Transformer-base with its default hyper-parameters~\citep{VaswaniSPUJGKP17} and Transformer-optimized~\citep{AraabiM20} which is tailored to low-resource NMT as baselines. We use the Fairseq library~\citep{OttEBFGNGA19} for our experiments and average sacreBLEU\footnote{sacreBLEU parameters:

nrefs:1$|$case:mixed$|$eff:no$|$tok:13a$|$smooth:exp$|$version:2.0.0}~\citep{Post18} over three runs as the evaluation metric. For more details, see App.~\ref{AppendixG}.


% Figure environment removed

\header{Joint Dropout parameters}
\label{section3.2}
...
While phrases can have different lengths, we consider all phrases as candidates to be replaced with variables, regardless of their length.
%We limit the number of variables in a single sentence. 
Based on preliminary experiments, we set the maximum number of variables in each sentence to $10$.
...
Therefore, in a set of experiments we investigate the choice of phrase types. As shown
in App.~\ref{appendixB}, there is no significant difference between selecting different phrase types. Thus, our method does not require a constituency parser which is not available for many languages. For the rest of the experiments, we substitute phrases regardless of their types.

%For the rest of the experiments, we substitute the phrases regardless of their types. 

To control the number of variables throughout the entire training corpus, we propose the use of a \emph{Joint Dropout rate}, which is calculated as the proportion of dropped tokens (from within phrases) with respect to the total length of the source and target sentences.
...
\small
	...
	\begin{tabular}{l rrrr}
		...
		\midrule
		T-base &	$ 4.6 $ &	$13.4$  & $8.9$   & $24.0$ \\
		T-base + JD & $\mathbf{6.5}$ &   $\mathbf{15.8}$  & $\mathbf{10.2}$  & $\mathbf{25.0}$  \\
		\midrule
		T-opt. &	$8.0$   &	$21.8$  & $15.2$  & $28.9$ \\
		T-opt. + JD & $\mathbf{9.9}$ &   $\mathbf{22.8}$  & $\mathbf{16.1}$  & $\mathbf{29.8}$  \\
...
%\VN{Is there a risk that ``extremely low-resource'' is too strong for En-Sk 55k and could upset readers who work on even more low-resource languages?}{} 
	...
	\label{tab1}
\end{table}
Fig~\ref{fig1} illustrates the improvements obtained by two different systems with increasing Joint Dropout rate.
JD consistently improves Transformer-base and Transformer optimized by $+2.4$ and $+1.8$ BLEU points on 10k samples, respectively, and also by $+3.1$ and $+1.4$ BLEU points on 20k samples.
...
To make JD independent of a phrase translation table, we consider not-aligned phrases in both or either translation sides. As shown in App.~\ref{AppendixC}, using aligned phrases is necessary, and using not-aligned phrases degrades the performance by $2.5$ BLEU points.
\begin{table}[t]
\small
	\centering
...
\header{Compositional generalization}
...
%However, since 
JD concentrates on improving compositionality at the local level. In this section, we aim to evaluate our method on local compositionality. Here, we take advantage of the most relevant theoretically grounded test from~\citet{HupkesDMB20} which is \emph{systematicity}, a notion frequently used in the context of compositionality. This attribute of the model concerns the recombination of known parts and rules, ensuring that the model's ability to grasp novel inputs is systematically tied to their aptitude to comprehend related inputs. For instance, understanding ``smallpox killed billions of people on this planet'' and ``tuberculosis'', also implies understanding ``tuberculosis killed billions of people on this planet''. Given that there are an infinite number of potential novel combinations that can be derived from known parts in natural data, we concentrate on a sentence-level, context-free rule: S~$\rightarrow$ NP VP, as proposed by~\citet{dankers-etal-2022-paradox}, where a noun from the NP in the subject position is replaced with a different noun, while maintaining number agreement with the VP. Additionally, they highlight that a systematic system necessitates consistency. After replacement, translations are deemed consistent if there is only one word difference between them. Table~\ref{tab9} illustrates that JD consistently enhances the consistency scores for various low-resource data conditions. 




\header{Translation performance}
We evaluate translation quality across various language pairs. Table~\ref{tab1} shows that JD improves translation quality for actual low-resource language pairs, which holds for the reverse language direction as well, as demonstrated in App.~\ref{AppendixD}.
...
As shown in Table~\ref{tab2}, Zero-Out yields only minor improvements. In addition, while Token Drop and SwitchOut methods prove to be ineffective in low-resource situations, JD consistently yields the largest improvements, especially for extreme low-resource cases.
...
\if0
\section{Joint Dropout algorithm}

\begin{algorithm}[]
    \caption{Joint Dropout with custom rate
    %\VN{%Redundant. Say just ``Joint Dropout'', or ``Joint Dropout with custom rate''.}{}
    }\label{alg:cap}
    \hspace*{\algorithmicindent} \textbf{Input:} source sentence S, target sentence T, phrase translation table, and Joint Dropout rate  \\
    \hspace*{\algorithmicindent} \textbf{Output:} set of phrase pairs P ready to get replaced with variables
    
    \begin{algorithmic}
    \State $\textbf{A} \gets $ all phrases from phrase translation table of S and T with no overlap between them. 
    %If $|\textbf{C}| > 10$, select 10 phrases from $\textbf{C}$ randomly.
    \State \textbf{if} $|\textbf{A}| \leq 10$ \textbf{then}
        \State \hspace*{\algorithmicindent}  $\textbf{C} \gets  \textbf{A}$
    \State \textbf{else}
        \State  \hspace*{\algorithmicindent}  $\textbf{C} \gets$  10 random phrases from $\textbf{A}$.
    \State \textbf{end if}
    
    \State $\textbf{N} \gets |\textbf{C}|\times\textnormal{Joint Dropout rate}$
    \State $\textbf{P} \gets $ randomly select $\textbf{N}$ phrase pairs from set $\textbf{C}$
    \label{appendixD}
\end{algorithmic}
\end{algorithm}
\label{appendixA}
...
\label{appendixB}
We consider four different scenarios: replacing 1)~only Noun Phrases~(NP), 2)~only Prepositional Phrases~(PP), 3)~only Verb Phrases~(VP), and 4)~mixtures of all the above.
...
Note that we choose phrase pairs in both languages only based on English constituency parse trees.
As shown in Table~\ref{tab6}, there is no significant difference between selecting different phrase types.
Thus, our method does not require any constituency parser, which is also not available for most of the low-resource languages.
...
\begin{table*}[t]
	...
	\begin{tabular}{l@{~}l@{~}l}
		...
		\midrule
		\small{Src} &	\makecell[l]{\scriptsize{[\textbf{ein Kind in Indien}] sagt: ``heute habe ich einen Affen} \scriptsize{  gesehen''.}} & \makecell[l]{\scriptsize{\{\textbf{meine Oma in China}\} sagt: ``heute habe} \scriptsize{ ich einen Affen gesehen''.}} \\
		\small{Ref.} & \scriptsize{[a child in India] says ,``\uline{I saw a monkey} today .''}	  &	\scriptsize{\{my grandmother in China\} says,``\uline{I saw a monkey} today .''}   \\
		\midrule
		
		\small{T-opt.} &	\scriptsize{[a child in India] says, ``today \uline{I've seen a monkey}s.''} &	\makecell[l]{\scriptsize{\{my grandmother's mother in China\} says,} \scriptsize{``\uwave{\textcolor{red}{Look}} today.''}}\\
		
		\small{T-opt. + JD} & \scriptsize{[a kid in India] says, ``\uline{I've seen a monkey}s today.''} & \scriptsize{\{my grandmother in China\} \scriptsize{says, ``today} \uline{\textcolor{OliveGreen}{I've seen a monkey}}s.''}  \\
		\bottomrule			
	\end{tabular}
	\vspace{-2pt}
