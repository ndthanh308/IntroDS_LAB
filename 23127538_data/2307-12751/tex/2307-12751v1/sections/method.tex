...
%as shown in Figure~\ref{fig:framework_a}.
Figure~\ref{fig:framework_a} illustrates the concept of our ICF.
%
...
%
As shown in Figure~\ref{fig:framework_b}, our method can simultaneously super-resolve and down-sample the given LR image $X$ with different scale conditions $s$ and $\nicefrac{1}{s}$, without requiring any paired/unpaired LR-HR training samples.
%
...
%
%For an arbitrary scale factor $s$, we first 
%
%For a given input LR image $X$ we employ a learnable IMF $f_{\theta}$ with learnable parameters $\theta$ to generate two images $X_s=f_{\theta}(X,s)$ and $X_{1/s}=f_{\theta}(X,1/s)$ with different scales $s$ and $1/s$, respectively where $s\geq1$.
%
%Accordingly, we apply the same function $f_{\theta}$ on these generated images $X_s$ and $X_{1/s}$ with the inverse scaling factors $1/s$ and $s$, respectively. 
%
%Then $f_{\theta}$ must generate the same outputs as the input $X$ as follows:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
\begin{equation}
    \begin{split}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%where \textcolor{blue}{$X_{s} \in \mathbb{R}^{sH\times sW\times 3}$  and $X_{\nicefrac{1}{s}} \in \mathbb{R}^{\nicefrac{H}{s} \times \nicefrac{W}{s} \times 3}$ are SR and low-low-resolution~(LLR) images, respecively.}
...
For an ideal ICF $f_\theta$, both $\check{X}$ and $\hat{X}$ in \eqref{eq:icf} should be the same as the original LR image $X$. 
%
Therefore, we train $f_\theta$ in a self-supervised manner by reducing the distance between $X$ and the generated images $\check{X}$ and $\hat{X}$ in two stages simultaneously, as shown in Figure~\ref{fig:framework_b}.
%
In the first~(up-down) stage, we minimize the distance between $\check{X}$ and $X$.
%
...
%
On the other hand, in the second~(down-up) stage, we aim to approximate the original input $X$ by reducing the distance between $\hat{X}$ and $X$.
%
...
%\begin{aligned}
\begin{split}
\mathcal{L}^{\text{Color}} &= %||\textbf{Pool}({X_s},4s,4s)-\textbf{Pool}(X,4,4)||\\+
%||\textbf{Pool}({X_{1/s}},4,4)-\textbf{Pool}(X,4s,4s)||.
