...
it is universal with respect to the sparsity pattern. In contrast, 
delocalization of \emph{exact} eigenvectors is sensitive to the 
specific sparsity pattern, whose understanding is a long-standing open  
problem even in the concrete example of random band matrices.
\end{abstract}
...
on the band width. The state of the art on this topic is surveyed in 
\cite{bourgade-survey}.
...
\begin{ex}
\label{ex:exactnono}
Consider the two examples illustrated in Figure \ref{fig:mat}: an $N\times 
N$ band matrix or block-diagonal matrix with $d$ nonzero entries per row.
...
localized when $d\ll N^{5/6}$.\footnote{%
	Thoughout this paper,  we use $A\ll B$ to mean $A/B\to0$, and 
	$A\gg B$ to mean $A/B\to\infty$.}
These conjectures remain largely open to date.

On the other hand, it is clear that \emph{every} eigenvector of a random 
block matrix will be supported in the coordinates associated to one of the 
blocks. Thus the block matrix model has localized eigenvectors whenever 
$d=o(N)$. In fact, \cite[Theorem 2.9]{bandsparse} shows that the 
eigenvectors in this model are nearly maximally localized (the latter 
result states that for any model of the kind considered in this paper, the 
mass of eigenvectors near the edge must be spread over at least $\sim 
\frac{d}{\log N}$ coordinates).
\end{ex}

The aim of this paper is to exhibit a different type of (de)localization 
phenomenon that is universal with respect to sparsity pattern. Let us
first recall that the macroscopic behavior of the 
\emph{eigenvalues} is in fact universal in this sense.
...
these entries. Our main result will show that this phenomonon is captured 
...
could hope for. In section \ref{sec:unitsphere}, we will show that a 
...
section~\ref{sec:unitsphere} by elucidating the delocalization properties 
of uniform random vectors in $\mathbb{S}^{N-1}$. In section~\ref{sec:loc}, 
...
Then we can apply Lemma \ref{lem:locest}
with $\nu=\nu(N):=\frac{d}{\beta^2\log N}=o(1)$ to estimate
\begin{multline*}
	\mathbf{P}[
	\text{there exists a }(1-\varepsilon)\text{-approximate
	top eigenvector of }X_N\text{ in }D_{\nu(N) N,\kappa}]
...
section \ref{sec:unitsphere} for a uniformly chosen random vector in the 
entire sphere $\mathbb{S}^{N-1}$: here we show that a uniformly chosen 
unit vector in a subspace $E$ is still delocalized when $E$ statisfies the 
requisite assumption. (We do not develop high probablity results as in 
section \ref{sec:unitsphere}, as these are not needed in the sequel.)
...
zero mean and covariance matrix $P_E$. Then $V:=\frac{Z}{\|Z\|_2}$ is 
...
a random matrix $X_N$ with those of a certain determinstic operator
$X_{N,\textrm{free}}$ that arises from free probability theory. In 
particular, \cite[Theorem 2.8]{free} states that
$$
	\| \mathbf{E} [(d^{-1/2}X_N-z)^{-1}] -
	(\mathrm{id}\otimes\tau)[(d^{-1/2}X_{N,\textrm{free}}-z)^{-1}]\|
	\le
	\frac{\tilde v(d^{-1/2}X_N)^4}{(\Im z)^5},
$$
where we adopt the notation of \cite{free}. For the present model,
$\tilde v(d^{-1/2}X_N)^4 \le \frac{2}{d}$
by \cite[Lemma 3.1]{free}. It remains to compute
...
LS was supported in part by NSF Graduate Research Fellowship under Grant No. DGE-2039656, and Simons Foundation grant 563916, SM.
