...
Overall, these QCD sum-rule diquark constituent mass predictions are in good agreement with the values used in the diquark various models, providing  QCD evidence supporting the tetraquark and pentaquark mass predictions emerging from these models.
...
Doubly-strange $\left[ss\right]$ $J^P=1^+$ diquarks are also considered.\footnote{The $[Qq]$ and $[ss]$ notation is used only to denote the diquark flavour content and not the flavour symmetry properties.   }
...
%$x=\frac{m^2}{t}=\frac{m^2}{q^2}$  
given in Table~\ref{Im_pert_coeff_fi} and Table~\ref{imag_part_coeff_fcns_tab}. The coefficients $f_0$ to $f_7$ in Table~\ref{Im_pert_coeff_fi}  agree with Table~3 of Ref.~\cite{Kleiv:2013dta}, providing a  consistency check on the extraction of the imaginary parts.  The new results in \eqref{imag_part_result_eq} and  are the strange-quark mass corrections  $g_0$ to $g_7$ in Table~\ref{imag_part_coeff_fcns_tab}. Similarly, the new results in 
\eqref{renorm_pert_result_eq} are the the strange-quark mass corrections $d_0$ to $d_7$ in Table~\ref{renorm_pert_coeff_fcns_tab}.
...
\label{continuum_eq}
\end{gather}
leading to a family of Laplace sum-rule relating the QCD prediction ${\cal R}_k\left(\tau,s_0\right)$ to resonance contributions $\rho^{\rm res}_{_\Gamma}(t)$
\begin{gather}
...
\label{ms_mu_tom}
\end{equation}
The result \eqref{ms_mu_tom} is needed for the $[ss]$ LSRs~(\ref{lsr_ss_0}) and~(\ref{lsr_ss_1}).
...
suffices to cover the acceptable Borel window of any LSRs analysis of
light- or strange-quark systems. None of the plots have a local minimum, and so
it can be concluded that the LSRs analysis of $J^P = 1^+$ $[ss]$ diquarks fails to stabilize.
...
LSRs analysis of $J^P = 1^+$ $[ss]$ diquark masses failed to stabilize. 
