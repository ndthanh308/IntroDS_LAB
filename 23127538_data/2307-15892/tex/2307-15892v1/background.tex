...
\begin{align}
\bold{NEU}(\theta) &= \EE[\delta\phi]^\tr \EE[\delta\phi] \nonumber\\
...
Note that here just putting them together by multiplication does not work, because the expectation of the two terms cannot be taken individually: they are dependent on each other.  
...
E(\theta) = \EE[\delta\phi] ^\top U^{-1}\EE[\delta\phi]$, where $U$ is any S.P.D matrix.
...
\citet*{xu2021sample_twotimescale} analyzed the convergence rate of linear and nonlinear TDC with constant step-sizes. \citet*{xu2020finite_q_learning} analyzed the convergence rate of a Q-learning algorithm with a deep ReLU network. Their algorithm also has a projection ball applied to the TD update. 
...
Some methods are based on the importance sampling over the stationary distributions of behavior and target policies \citet*{hallak2017consistent,liu2018breaking_IS_stationary,xie2019towards_IS,gelada2019off_IS_stationary}, instead of the product of policy ratios. 

Emphatic TD (ETD) \citep*{etd,yu2015convergence_etd}, a non-gradient-based method, has only one step-size in its update rule. However, ETD does not converge to the TD solution and it suffers from high variances. One has to use small step-sizes for ETD, which results in slow convergence \citep*{ghiassian2017first_etd,ghiassian2021empirical_etd}. ETD is still not a practical problem due to high variances \citep{sutton2018reinforcement}.
