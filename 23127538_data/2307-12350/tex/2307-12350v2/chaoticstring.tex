...
However, the dynamics of such a geodesic becomes chaotic and  non-integrable if we consider more complicated geometries like in \cite{hanan_chaotic_2007,chen_chaotic_2016,chervonyi_non-integrability_2014}. In fact, how a string behaves in a given spacetime is also not fully understood \cite{basuAnalyticNonintegrabilityString2011a,basuChaosHolographicRegge2012a,basu_chaos_2011, basu_chaotic_2017,ishii_fate_2017, ma_chaotic_2020, ma_chaotic_2022}. %Due to the extended nature of the string, 
Even in the simplest scenario of the Schwarzchild black hole, the string dynamics is non-integrable\cite{frolov_chaotic_1999,zayas_chaos_2010}. However, there are certain selected class of backgrounds - e.g, $AdS_5 \times S^5$ in which string dynamics tunrs out to be integrable, see \cite{beisertReviewAdSCFT2012a,vantongerenIntegrabilityAdSSuperstring2014,ricciTDualityIntegrabilityStrings2007,benaHiddenSymmetriesAdS2004}. Nevertheless, in any generic curved background, the string motion is chaotic\cite{rigatosNonintegrabilityaC2020,basuIntegrabilityLost2011, basu_chaotic_2017,asanoChaoticStringsPenrose2015,hashimotoChaosWilsonLoop2018,nunezIntegrabilityNonIntegrabilitySCFTs2018,roychowdhuryAnalyticIntegrabilityStrings2017,banerjeeProbingAnalyticalNumerical2018a,maChaosRingString2014a,giataganasNonintegrabilityNonrelativisticTheories2014}. The (non)-integrability aspect can also be considered when examining the behavior of string dynamics in Dp-branes \cite{stepanchuk_nonintegrability_2013,giataganasNonintegrabilityChaosUnquenched2017}. Previous studies \cite{stepanchuk_nonintegrability_2013} have shown that the classically extended string in Dp-brane in extremal scenarios having charge as the interpolation parameter between flat space and $AdS_5 \times S^5$ are non-integrable. In this paper, our aim is to explore this concept and conduct a numerical analysis of the circular string dynamics in a black p-brane. Specifically, we will focus on two cases: p=5 and p=6. We shall begin with the non-extremal scenarios first and then eventually examine the extremal situation as well by means of varying the suitable parameter.
...
First, we consider the charged p=5 brane (figure \ref{winding_charged_p=5}). When $r(0) = 11$ (figure \ref{winding_charged_p=5}(a)) and Q = 0.1, the string escapes to infinity for $n > 1$. The rate of escape  slightly increases with $n$. However, the string falls very quickly when $n$ = 1 (see also figure \ref{fig: FLI_P=5}(a)). With increasing charge (Q = 1.5), we 
...
\vspace{2.5mm}
 % Figure environment removed
Finally, we describe the role of $n$ in uncharged p=6 brane. Figure \ref{winding_uncharged_p=6} (a)-(d) depicts the plots showing the string modes at different E (control parameter) and n for r(0) = 12. For E = 5, we observe the exponential growth of FLI curve for $n \geq 1$ and the growth increases with n. By increasing energy (E = 7.5 and E = 9), we observe the capture mode for n = 2,3 and the capture time seems to decrease with both E and n. Finally, at E = 10, we observe very quick capture of the string for all $n \geq 1$. However, capture time is largest for n = 2. For r(0) = 300 (figure \ref{winding_uncharged_p=6}(e),(f)), the string  with any non-zero $n$ escapes to infinity for all E, the rate of escape increases with both $n$ and E. 
% Figure environment removed
...
\item For the uncharged 6-brane and when r(0) = 12, we see a transition from escape to capture mode for $n\geq 1$ as we keep increasing control parameter(E). By increasing the initial radial position (r(0) = 300), we observe only the escape of the string with the rate of escape increasing with $n$.
 \end{itemize}
\vspace{4mm}
