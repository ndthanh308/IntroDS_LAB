...
\begin{abstract}
The main goal of this paper is to determine for which prime numbers~$r\geq 3$ can an elliptic curve~$E$ defined over $\Q$ have an $r$-isogeny over $\Q(\zeta_r)$. We study this question under various assumptions on the 2-torsion of $E$. Apart from being a natural question itself, the mod~$r$ representations attached to such $E$ arise in the Darmon program for the generalized Fermat equation of signature~\((r,r,p)\), playing a key role in the proof of modularity of certain Frey varieties in the recent work of Billerey, Chen, Dieulefait and Freitas
\end{abstract}


\thanks{I gratefully acknowledge support by the QuantiXLie Centre of Excellence, a  project co-financed by the Croatian Government and European Union
...
Let~\(r\geq 3\) be a prime number. For an elliptic curve~\(E\) defined over a number field~\(K\), we denote by~\(\rhobar_{E,r} : G_K \to \GL_2(\F_r)\) its mod~$r$ Galois representation (after fixing a basis for the $r$-torsion module~$E[r]$). The present paper is concerned with the (ir)reducibility of the representation~\(\rhobar_{E,r}\) where~\(K = \Q(\zeta_r)\) and~\(E\) is a base change to~\(K\) of an elliptic curve defined over~\(\Q\).
...
I thank Nicolas Billerey and Nuno Freitas for asking questions motivating this paper, and for many helpful comments. 
...
For a subgroup $G\leq \GL_2(\F_r)$, define $S(G):=G\cap \SL_2(\F_r)$. Denote by $C_s(r)$ the subgroup of diagonal matrices in $\GL_2(\F_r)$ and by $C_s^+(r)$ its normalizer in $\GL_2(\F_r)$. Let $\epsilon=-1$ if $r\equiv 3 \pmod 4$ and otherwise let $\epsilon\geq 2$  be the smallest integer which in not a quadratic residue modulo $r$. Denote by $C_{ns}(r)$ the group
...
\end{lemma}
\begin{proof}
It is straightforward to check that $S(C_s^+(r))$ and $S(C_{ns}^+(r))$ are of the claimed form and that $S(C_s^+(r))$ acts freely on $\F_r^2 \backslash \{(0,0)\}$, from which it follows that the length of each orbit is $2(r-1)$, so we see that there are no fixed $1$-dimensional subspaces of $\F_r^2$.
...
\begin{proposition}
\label{prop1}
Let $r\geq 17$, $E/\Q(\zeta_r)$ with $j(E)\in \Q$ be an elliptic curve without CM, such that
\begin{multline*}
(r,j(E))\notin\left\{ (17,-17\cdot 373^3/2^{17}), (17,-17^2\cdot 101^3/2), \right. \\
\left.(37,-7\cdot 11^3), (37,-7\cdot 137^3\cdot 2083^3)\right\}.
\end{multline*}
Then $E$ has no $r$-isogeny over $\Q(\zeta_r)$.
\end{proposition}
\begin{proof}
As explained above we can assume that $E$ is defined over $\Q$. We have shown in Lemma \ref{lem:subgroups} that $S(C_{ns}^+(r))$ does not fix any $1$-dimensional subspace of $\F_r^2$.
...
Now the result follows from Proposition \ref{prop-zyw}.
\end{proof}

\begin{proposition}
\label{prop:ired}
Suppose that $r\geq 5$, that $E/\Q(\zeta_r)$ has $CM$ and either $j(E)\in \Q\backslash \{0,1728\}$ or if $j(E)\in\{0,1728\}$ that $E$ is a base change of an elliptic curve defined over $\Q$. Then $E$ has an $r$-isogeny over $\Q(\zeta_r)$ if and only if any elliptic curve $E'/\Q$ with $j(E')=j(E)$ has an $r$-isogeny over $\Q$ which occurs if and only if $r$ divides $D$, where $-D$ is the discriminant of the imaginary quadratic number field containing the endomorphism ring of $E$.
\end{proposition}
\begin{proof}
...
%%%%%%%%%%%%%%%%%%%%%
\section{Elliptic curves with some $2$-torsion and reducible mod $r$ Galois representations}
%%%%%%%%%%%%%%%%%%%%%
...
\begin{enumerate}
%\item\label{item:SL2} \(G_{E,13} = \rhobar_{E,13}(G_\Q)\) contains~\(\SL_2(\F_{13})\);
\item\label{item:Borel} \(G_{E,13}\) is included in a Borel subgroup of~\(\GL_2(\F_{13})\);
\item\label{item:NormC} \(G_{E,13}\) is included in the normalizer of a Cartan subgroup of~\(\GL_2(\F_{13})\);
\item\label{item:excep} the projective image~\(H_{E,13}\) of~\(G_{E,13}\) is isomorphic to~\(A_4\), \(S_4\), or~\(A_5\).
\end{enumerate}
...
If $\rhobar_{E,2}(G_\Q)=\GL_2(\F_2)$, then obviously $E$ does not have full $2$-torsion over $\Q(\zeta_r)$ (which is abelian over $\Q$). By \cite[Proposition 1.15]{zyw} and \Cref{prop:ired}, we need only consider elliptic curves with $j(E)=-3^3\cdot5^3$ and $3^3\cdot5^3\cdot17^3$, as these are the only ones with both an $r$-isogeny over $\Q$, for $r\geq 5$, and non-surjective $\rhobar_{E,2}$. It turns out we necessarily have $r=7$ and now the result follows from \Cref{prop7}.
...
$$G_7:=\left\langle\begin{pmatrix}
             ...
             $$
We compute in Magma that the orbits of $S(G_7)$ acting on $\F_{13}^2 \backslash \{(0,0)\}$ are all of length 24, so elliptic curves with such image do not have $13$-isogenies over $\Q(\zeta_{13})$. We conclude that if a non-CM elliptic curve has a $13$-isogeny over $\Q(\zeta_{13})$, it has one already over $\Q$. As there are no $26$-isogenies over $\Q$ \cite{MazurVelu}, an elliptic curve with a $13$-isogeny over $\Q$ cannot have a 2-torsion point over $\Q$. Since if $E$ gains full $2$-torsion over $\Q(\zeta_{13})$, it follows that $\Q(E[2])$ is a subfield of $\Q(\zeta_{13})$ and hence $\rhobar_{E,2}(G_\Q)$ is cyclic of order 3. But there are no such curves, which can be seen from \cite[Table 8]{DGJ} in the line [2Cn,13B] and \cite[Table 11]{DGJ}.
\end{proof}
...
\end{proposition}
\begin{proof}
For $r=7$, by \cite[Theorem C]{Morrow}, if $\rhobar_{E,7}(G_\Q)\neq\GL_2(\F_{7})$, then $\rhobar_{E,7}(G_\Q)$ is contained in a Borel subgroup, which would imply that $E$ has a $14$-isogenous over $\Q$, which is impossible since it does not have CM.
...
The projective image of the group $G_{9,5}$ is isomorphic to $S_4$, and is denoted by $5S4$ in \cite{DGJ}.
 ...
 From \cite[Table 13]{DGJ}, in the line $[2Cs,5S4]$ one can see that the only elliptic curves $E$ over~$\Q$ with $\rhobar_{E,2}(G_\Q)=\{I\}$ (which our hypothetical curve satisfies) and $\rhobar_{E,5}(G_\Q)\leq G_{9,5}$ are the ones with $j(E)=2^6\cdot3^3$, and such curves have CM.
\end{proof}
