...
I thank Nicolas Billerey and Nuno Freitas for asking questions motivating this paper and for many helpful comments, and Davide Lombardo for pointing out the proof of \Cref{prop1} for $r<17$.
...
For $r=5$ the only candidate group we find is conjugate in $\GL_2(\F_5)$ to the group denoted $G_3$ in \cite[Section 1.3]{zyw}. By \cite[Section 1.4. (ii)]{zyw}, $\rhobar_{E,3}\subseteq G_3$ if and only if $j(E)$ is of the form given in \eqref{eq:execptions}.
\begin{comment}
...
\begin{remark}
  Recall that for $r\geq 17$, $E/\Q$ without CM has an $r$-isogeny if and only if $(r,j(E))\in\left\{ (17,-17\cdot 373^3/2^{17}), (17,-17^2\cdot 101^3/2), \right.
\left.(37,-7\cdot 11^3), (37,-7\cdot 137^3\cdot 2083^3)\right\}$, so by \Cref{prop1} these are the only instances when an elliptic curve defined over $\Q$ can have an $r$-isogeny over $\Q(\zeta_r)$ for $r\geq 17$.
\end{remark}
