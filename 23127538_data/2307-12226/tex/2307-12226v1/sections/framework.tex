...
We introduce our framework, called $\loki$. 
...
Finally, we show how $\loki$ relates to standard classification.%> in the absence of relational structure between the classes. 
...
We model unobserved classes $\mathcal{Y} \setminus \Lambda$ using the Fréchet mean among observed classes weighted by their \textit{prediction probabilities} $P(y=\lambda_i \mid x \text{ and } y \in \Lambda)$. 
We denote the vector of model outputs as $\mathbf{P}_{y|x} := [\mathbf{P}_{\lambda_i|x}]_{i=1}^K = [P(y=\lambda_i \mid x \text{ and } y \in \Lambda)]_{i=1}^K$. 
...
This equation is given by setting the weights in Equation~\ref{def:fm} to values that are produced by a model. 
Since our goal is to adapt pretrained models without any additional training, we choose to set the Fréchet mean weights to the prediction probabilities produced by the model: $\bm{w_i} = \mathbf{P}_{\lambda_i|x}$. 
