...
In addition to our dataset, we have access to a relational structure on $\Y$. We assume that $\Y$ is a metric space with metric (distance) $d : \Y^2 \rightarrow \mathbb{R}$; $d$ encodes the relational structure of the label space. In other words, $d$ is such that related elements of the space are closer and unrelated elements farther.  Additionally, we model this metric space using a graph, $G = (\set{Y}, \set{E})$ where $\set{E} \subseteq \set{Y}^2 \times \mathbb{R}_{+}$ is a set of edges relating the labels and $d: \set{Y}^2 \to \mathbb{R}_{\geq 0}$ is the standard shortest-path distance. 
In addition to its use in prediction, the metric $d$ can be used for evaluating a model by measuring, for example, $\frac{1}{n} \sum_{i=1}^n d^2(f(x_i), y)$. This is the analogue of the empirical square loss.
...
Often, ZSL methods that use such information rely on the use of graph neural network architectures \cite{KG_zsl_1, Kampffmeyer2018RethinkingKG, kipf2017semisupervised}.
\textit{However, we note that these architectures can be heavyweight and can be challenging to scale up to extremely large graphs, whereas $\loki$ does not have architectural requirements and its time complexity scales linearly in the size of the graph when $K$ is small.} 
...
For example, in \cite{Ciliberto2016}, 
$ \hat{f}(x) =$  $\arg\min_{y \in \Y}$  $\frac{1}{n}\sum_{i=1}^n \alpha_i(x)d^2(y, \bm{y}_i)$, 
where $\alpha(x) = (K + n \nu I)^{-1} K_x$. 
Here, $K$ is the kernel matrix for a kernel 
