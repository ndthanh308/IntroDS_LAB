...
Later, we shall see that pairwise decomposability is useful in efficiently computing the locus. 
...
Then, we will discuss analogous results for different metric spaces (whose formal statements and proofs can be found in the appendix). 
\begin{theorem}[Minimum locus cover for trees]
\label{thm:min_locus_trees_main}
...
This follows from the construction of the weights in the proof of Theorem~\ref{thm:min_locus_trees_main} and the property that all paths in trees are unique. 
Finally, we note that any locus in any tree is pairwise decomposable---the proof of this is given in the Appendix. 
...
Later, we give algorithms for computing loci in arbitrary graphs---if the locus is pairwise decomposable, the algorithm for doing so is efficient, and if not, it has time complexity exponential in $K$. 
Due to the pairwise decomposability of phylogenetic trees, this polynomial-time algorithm to compute $\Pi(\Lambda)$ applies. 
%\fred{a lot of these hanging lines with just a few words should be removed with a tiny bit of rewriting.}
...
While the set of vertices given by Theorem~\ref{thm:min_loc_grid} (found in the Appendix) satisfies Definition~\ref{def:min_loc_cover}, this set does not satisfy Definition~\ref{def:identifying_loc_cover}. 
...
In contrast, the following example set of vertices satisfies Definition~\ref{def:identifying_loc_cover} but it clearly does not satisfy Definition~\ref{def:min_loc_cover}.
...
%\fred{Let's put the results back into this section since we have space.}
