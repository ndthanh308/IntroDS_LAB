...
appear as a phase. As a consequence, while the 't Hooft matching for chiral anomalies, i.e. the constancy 
...
the detailed analytic structure of the anomalous correlators using strongly the local 
nature of trace anomalies.
...
type A and B trace anomalies (and also the local chiral anomalies).
...
determined by the (derivatives of the) functions $f^r$ in the change of basis \eqref{eq609}. 
...
one-point functions, which could have given a semi local contribution,  are zero in the unbroken phase.
So depending on the sources, semi-local contributions could be present. We do not, however, have an intrinsic definition for
...
the transformation of the metric. The semi-local contributions do not have an intrinsic (universal) meaning, unless 
...
\be
C_{\mu}(k_1,k_2)=\int \frac{d^d q}{\pi^{d/2}}\frac{q_\mu}{((l^2-M^2)((l+k_1)^2-M^2)((l-k_2)^2-M^2)}
\ee
and 
\be
C_{\mu\nu}(k_1,k_2)=\int \frac{d^d l}{\pi^{d/2}}\frac{q_\mu\,q_\nu}{((l^2-M^2)((l+k_1)^2-M^2)((l-k_2)^2-M^2)}
\ee
...
\noalign{\vskip.3cm}
\lambda^4\,C&=-2\,\qr\,\lambda^2-\frac{1}{4}\big[3(q^2-r^2)^2-4\,\lambda^2\big]C_0+3\,q^2\,\qr\,(q^2-r^2)B_0^f(q^2)-4\,\qr\,\lambda^2\,M^2\,C_0\\
\noalign{\vskip.2cm}
&+\frac{1}{2}\big[\big((\qr)^2(r^2-3\,q^2)+3\qr\,q^2(q^2-r^2)+2\,q^2\,r^4\big)B_0^f(k_1^2)-(k_1\leftrightarrow k_2)\big]\\
\noalign{\vskip.3cm}
\lambda^4\,D&=2\,q^2\,\lambda^2+\frac{1}{4}q^4\big[3(q^2-r^2)-4\,\lambda^2\big]C_0-3\,q^4(q^2-r^2)B_0^f(q^2)+4\,q^2\,\lambda^2\,M^2\,C_0\\
\noalign{\vskip.2cm}
...
massive flow \cite{KS,Komargodski,Luty}.\footnote{For a bootstrsap approach see \cite{Penedones}.}
