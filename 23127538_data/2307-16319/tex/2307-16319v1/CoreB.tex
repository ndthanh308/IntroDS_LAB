...
Let $\h=\h_{r,n}(q,{\bf Q})$ denote an Ariki-Koike algebra over a field $\mathbb{F}$ of characteristic $p$ and with quantum characteristic $e \in \{2,3,\ldots\} \cup \{\infty\}$. Each of these algebras decomposes into a direct sum of indecomposable two-sided ideals, its blocks, and so in order to try to understand the algebras, one may study certain types of blocks in the hope that they are more manageable.
...
Now fix $e \in \{2,3,\ldots\} \cup \{\infty\}$. If $e$ is finite, take $I=\{0,1,\ldots,e-1\}$ which we identify with $\Z/e\Z$; otherwise take $I=\Z$. Suppose that $\mc \in \Z^r$. 
...
For $\bmu \in \La^{r}$ we define the $i$-signature of $\bmu$ by looking at all the addable and removable $i$-nodes of $[\bmu]$ from top left to bottom right and writing $\ab$ for an addable $i$-node and $\rr$ for a removable $i$-node. We then construct the reduced $i$-signature of $\bmu$ by removing all adjacent $(\rr\ab)$-pairs. If there are any $\rr$ terms in the reduced $i$-signature of $\bmu$, the first $\rr$ term corresponds to a removable node of $[\bmu]$ which is called a good $i$-node.
...
It is usual to write $+$ instead of $\ab$ and $-$ instead of $\rr$, however we want use those symbols for other purposes in this paper. The set $\La^{\mc}$ is defined recursively. 
  ...
  \item Otherwise, if $\bmu$ does not contain a good $i$-node for any $i \in I$ then $\bmu \notin \La^{\mc}$.    
    ...
    \end{itemize}
We set $\La^{\mc}_n = \La^{\mc} \cap \La^r_n$.

Given a multipartition, it is convenient to represent it on an abacus. 
If $e$ is finite, an $e$-abacus is an abacus with $e$ runners which are infinite in both directions indexed from left to right by the elements of $I=\{0,1,\ldots,e-1\}$. The possible bead positions are indexed by the elements of $\Z$ such that bead position $b$ on the abacus is in row $l$ of runner $i$ where $b=le+i$ and $i \in I$. If $e=\infty$, the abacus has runners and bead positions indexed by the elements of $I=\Z$, so that runner $i\in \Z$ contains either one bead or no beads.  
...
For $\bla \in \La^{r}$ and $\mc \in \Z^r$ we define the abacus configuration of $\bla$ with respect to $\mc$ to be an $r$-tuple of abacuses, the $k^{\text{th}}$ one of which is the abacus of $\la^{(k)}$ with respect to $a_k$. 

Operations on the Young diagram of $\bla$ can be translated into operations on the abacus of $\bla$. 

\begin{lemma}
Let $\bla \in \La^r$ and $\mc \in \Z^r$. All residues below are with repect to $\mc$ as are all abacuses.  
\begin{itemize}
...
\item If $e \ne \infty$ then adding (resp. removing) an $e$-rim hook to component $k$ of $[\bla]$ corresponds to pushing a bead on the abacus of $\la^{(k)}$ up one position (resp. down by one position). 
\end{itemize}
\end{lemma}
...
\end{array}$$
Define $e \geq 2$ to be minimal such that $1+q+\dots+q^{e-1}=0$, or set $e=\infty$ if no such value exists. Set $I=\{0, 1,\ldots,e-1\}$ if $e$ is finite and $I=\Z$ otherwise. If $e$ is finite, we identify $I$ with $\Z/e\Z$.    
...
From the properties of cellular algebras, we know that there is a bilinear form on each cell module. If we define $\rad(S^{\bmu})$ to be the radical of the Specht module $S^{\bmu}$ with respect to this bilinar form then if $\mc$ is a multicharge for $\h$ then $\rad(S^{\bmu}) \neq \{0\}$ if and only if $\bmu \in \La^{\mc}_n$ and so
...
We fix $e \in \{2,3,\ldots\} \cup \{\infty\}$ and set $I=\{0,1,\ldots,e-1\}$ if $e$ is finite and $I=\Z$ otherwise. In each case, we take $<$ to be the usual total order on $I$. If $e$ is finite, we may identify $I$ with $\Z/e\Z$. We also fix $\ms =(s_1,s_2) \in I^2$. We shall take a block to mean an $\ms$-equivalence class of $\La^2$ and a core block to be a block $B$ in which each $\bla \in B$ is a bicore. 

Let $\mathbb{F}$ be a field of characteristic $p \geq 0$. Fix $q \in \mathbb{F}$ where $q$ is a primitive $e^{\text{th}}$ root of unity if $e$ is finite and $q^l \neq 1$ for any $l \in \Z$ otherwise. Let ${\bf Q}=(q^{s_1},q^{s_2})$. 
...
If $I=\{i_0,i_1,\ldots,i_{e-1}\}$ with $i_0 \prec i_1 \prec \ldots \prec i_{e-1}$ we take $\pi=\pi_{\bb}(B)$ be the permutation that sends $j$ to $i_j$ for $j \in I$. 
 If $e=\infty$, there is a unique reduced pair for $B$, namely $\re=(\ms,\bz)$ where $\bz=(\ldots,0,0,0,\ldots)$. We then take $\prec$ to be the usual total order $<$ on $\Z$ and $\pi$ to be the identity permutation on $\Z$. 
...
l^{\mc}_{mk}(\bla) - 1, & m=i \text{ and } k=1 \text{ or } m=j \text{ and } k=2, \\
l^{\mc}_{mk}(\bla)+1, & m=j \text{ and } k=1 \text{ or } m=i \text{ and } k=2,\\
...
\begin{corollary}
Let $B$ be a core block. Then \[0 \leq \wt(B) \leq \left\lfloor \frac{e}{2} \right \rfloor.\] 
\end{corollary}
...
Suppose that $B$ is a core block with $|B|>0$ and $(\mc,\bz)$ is a reduced pair for $B$. Let $\bmu \in B$. There is a bipartition $\bnu$ which satisfies the following conditions. 
\begin{itemize}
...
Let $\F^{\ms}$ be the $\mathbb{Q}(v)$-vector space with basis $\{\Cr{\bla} \mid \bla \in \Lambda^2\}$. This becomes a $\mathcal{U}$-module under the action described in~\cite{LLT}; we call $\mathcal{F}^{\ms}$ the Fock space representation of $\mathcal{U}$. The $\mathcal{U}$-submodule $M^{\ms}$ generated by $s_{\emp^2}$ (where $\emp^2$ is the unique $2$-bipartition of $0$) is isomorphic to the irreducible highest weight module $V(\Upsilon)$ for some dominant integral weight $\Upsilon$ of $\mathcal{U}$. This module has a canonical basis (in the sense of Lusztig and Kashiwara) which is indexed by the elements of $\La^{\ms}$; we write $P^{\bmu}$ for the canonical basis element indexed by $\bmu \in \La^{\ms}$. We define the transition coefficients $d_{\bla\bmu}(v)$ so that they satisfy the equation 
...
If $\mathfrak{n}_1=(x_1,y_1,k_1), \mathfrak{n}_2=(x_2,y_2,k_2) \in \Z_{>0} \times \Z_{>0} \times \{1,2\}$ we say that $\mathfrak{n}_2$ is below $\mathfrak{n}_1$ if $k_2>k_1$ or $k_2=k_1$ and $x_2>x_1$. If $d>0$ and $\bnu,\bla \in \La^2$, write $\bnu \xrightarrow{d:i} \bla$ if $[\bla]$ is formed from $[\bnu]$ by adding $d$ nodes all of residue $i \in I$. 
If $\bnu \xrightarrow{d:i} \bla$ set
\begin{multline*}
N(\bnu,\bla) =\sum_{\mathfrak{n} \in [\bla] \setminus [\bnu]} \#\{\mathfrak{m} \text{ an addable $i$-node of } \bnu \text{ with } \mathfrak{m} \text{ below } \mathfrak{n}\}\\
-\#\{\mathfrak{m}\text{ a removable $i$-node of } \bla \text{ with } \mathfrak{m} \text{ below } \mathfrak{n}\}. 
\end{multline*}
...
Suppose that $B$ and $B'$ are any two core blocks with $n_B=n_{B'}$ and $p_{B}=p_{B'}$. Then we have a bijection between $\Phi:B \rightarrow B'$ determined by $\delta_{\bla}=\delta_{\Phi(\bla)}$ for $\bla \in B$. If $\bmu \in B$ then by Lemma~\ref{P:Klesh2}, we have that $\bmu \in \La^{\ms}$ if and only if $\Phi(\bmu) \in \La^{\ms}$. 
...
For $\bla,\bnu$ in a core block $B$, let $\dim(\Hom(S^{\bla},S^{\bnu}))_v$ denote the graded dimension of the homomorphism space between $S^{\bla}$ and $S^{\bnu}$. The next result appears in George Witty's 2020 thesis~\cite{WittyThesis}. Although Witty only proves the case where $(\mc,\bz)$ is a base pair for $B$, he conjectures that the result holds for an arbitrary base pair~\cite[Conjecture~4.32]{WittyThesis}. It is possible that one could prove this conjecture using Scopes equivalence~\cite{Webster}. 
...
{\em Quiver presentations and isomorphisms of Hecke categories and Khovanov arc algebras}, In preparation. 
...
{\sc M.~Fayers},
{\em Weights of itions and representations of Ariki-Koike algebras}, Adv. Math., {\bf 06} (2006), 112--133. 
...
\bibitem{WittyThesis}
{\sc G.~Witty}, 
{\sc Homomorphisms between Specht modules of KLR algebras},
