...
\subjclass[2020]{20G25, 14G35}


\begin{abstract}
We introduce a natural analogue of Steinberg's cross-section in the loop group of an unramified reductive group $\bG$. We show that this loop Steinberg's cross-section provides a simple geometric model for the poset $B(\bG)$ of Frobenius-twisted conjugacy classes (referred to as Newton strata) of the loop group. As an application, we comfirm a conjecture by Ivanov on loop Delgine-Lusztig varieties of Coxeter type. This geometric model also leads to new and direct proofs of several classical results, including the converse to Mazur's inequality, Chai's length formula on $B(\bG)$, and a key combinatorial identity in the study affine Deligne-Lusztig varieties with finite Coxeter parts.
\end{abstract}
...
(1) $N$ consists of regular elements of $G$;
...
\subsection{The main result} In this note we focus on a loop group version of Steinberg's result mentioned above. On the one hand, the Steinberg's cross-section has a straightforward analogue in a loop group. On the other hand, a loop group is stratified by its Frobenius-twisted conjugacy classes. It is natural to ask how the loop Steinberg's cross-section intersects various Frobenius-twisted conjugacy classes. To our surprise, the answer is simple and of purely combinatorial. This suggests that the loop Steinberg's cross-section serves as a ``transversal slice'' to Frobenius-twisted conjugacy classes.

To formulate the main results, we introduce more notations. Let $\bG$ be an unramified reductive group over a non-archimedean local field $k$. Let $\brk$ be the completion of a maximal unramified extension of $k$. Fix a uniformizer $\varpi \in k$. Denote by $\s$ the Frobenius automorphism on the loop group $\bG(\brk)$. We fix two opposite Borel subgroups $\bB$ and $\bB^-$, and let $\bT = \bB \cap \bB^-$ be a maximal torus. Let $\bN$ be the normalizer of $\bT$ in $\bG$. The Weyl group of $\bG$ is defined by \[W = \bN(\COk) / \bT(\COk) \cong \bN(\brk) / \bT(\brk),\] where $\COk$ is the integer ring of $\brk$. For $w \in W$ we set \[\bU_w = \bU \cap {}^{w\i}\bU^-,\] where $\bU, \bU^-$ are the unipotent radicals of $\bB, \bB^-$ respectively.

Fix a representative set $\{\a_1, \dots, \a_r\}$ for the $\s$-orbits of simple roots appearing in $\bB$. Denote by $s_{\a_i} \in W$ the simple reflection corresponding to $\a_i$. Set $c = s_{\a_1} \cdots s_{\a_r}$ which is a $\s$-twisted Coxeter element. Let $\dc \in \bN(\brk)$ be lift of $c$. Then there exist a unique cocharacter $\mu \in X_*(\bT)$ and some lifts $\ts_{\a_i} \in \bN(\COk)$ of $s_{\a_i}$ such that $\dc = \varpi^\mu \ts_{\a_1} \cdots \ts_{\a_r}$, where $\varpi^\mu = \mu(\varpi) \in \bG(\brk)$. Following \cite[Theorem 1.4]{S}, we define the loop Steinberg's cross-section attached to $\dc$ by \[\dc \bU_c(\brk) = \varpi^\mu \ts_{\a_1} \bU_{\a_1}(\brk) \cdots \ts_{\a_r} \bU_{\a_r}(\brk),\] where $\bU_{\a_i}$ denotes the root subgroup of $\a_i$.

For $b \in \bG(\brk)$ the corresponding $\s$-twisted conjugacy class is defined by \[[b] = \{g\i b \s(g); g \in \bG(\brk)\}.\] Thanks to Kottwitz, $[b]$ is determined by two invariants: the Kottwitz point $\k(b) = \pi_1(\bG)_\s$ and the (dominant) Newton point $\nu(b) \in X_*(\bT)_\BQ$. We denote by $B(\bG)$ the set of $\s$-twisted conjugacy classes of $\bG(\brk)$.

The main result of this note is a combinatorial description of the intersections $[b] \cap \dc \bU_c(\brk)$ for $[b] \in B(\bG)$.
\begin{theorem} \label{main}
    Let notation be as above. Then $[b] \cap \dc\bU_c(\brk) \neq \emptyset$ if and only if $\k(\dc) = \k(b)$. In this case, \[[b] \cap \dc\bU_c(\brk) = \varpi^\mu \ts_{\a_1} H_{\mu, b, \a_1} \cdots \ts_r H_{\mu, b, \a_r},\] where \[H_{\mu, b, \a_i} = \begin{cases} \bU_{\a_i}(\varpi^{\<\mu-\nu(b),  \o_{\CO_i}\>} \CO_\brk^\times); &\text{ if } \<\nu(b), \a_i\> \neq 0 \\ \bU_{\a_i} (\varpi^{\lceil \<\mu-\nu(b), \o_{\CO_i}\> \rceil} \COk), &\text{ otherwise.} \end{cases}\] Here $\o_{\CO_i}$ is the sum of fundamental weights corresponding simple roots in the $\s$-orbits of $\a_i$, and $\< , \>: X_*(\bT)_\BQ \times X^*(\bT)_\BQ \to \BQ$ is the natural paring.
\end{theorem}

\begin{remark}
    Note that $[b]$ is an admissible set in the sense of \cite{HV} and \cite{He16}. In particular, it makes sense to define topological invariants of $[b]$, such as the closure $\overline{[b]}$ of $[b]$ in $\bG(\brk)$, the irreducible/connected components of $[b]$, and the relative dimension $\dim [b]$ of $[b]$.

    By construction, the loop Steinberg's cross-section $\dc\bU_c(\brk)$ can be viewed as an infinite dimensional affine space. By Theorem \ref{main}, the intersections $[b] \cap \dc\bU_c(\brk)$ are admissible subsets of $\dc\bU_c(\brk)$. So it also makes sense to study the topological properties of $[b] \cap \dc\bU_c(\brk)$.
\end{remark}
...
The above results indicate that the Newton stratification of the loop Steinberg's cross-section provides a simple geometric model of the poset $(\bG, \leq)$. This lead to new interpretations of several classical results. Here are two immediate applications.
...
\end{theorem}
The implication ($\Rightarrow$) is again due to Rapoport and Richartz \cite{RR}. The implication ($\Leftarrow$) is prove by Gashi \cite{Ga} in a purely combinatorial way. Now we give a new proof. First we can assume that $\mu$ is dominant. Then the Newton point $\nu(\varpi^\mu)$ equals the $\s$-average of $\mu$. By definition, for any $[b] \in B(G)$ we have $\<\mu-\nu(b), \o_{\CO_i}\> \ge 0$ for $1 \le i \le r$. Hence it follows from Theorem \ref{main} that \[ \emptyset \neq [b] \cap \dc\bU_c(\brk) \subseteq \varpi^\mu \ts_{\a_1} \bU_{\a_1}(\COk) \cdots \ts_{\a_r} \bU_{\a_r}(\COk) \subseteq \varpi^\mu \CK.\] So the implication ($\Leftarrow$) follows.

\

\subsection{Further consequences} We continue to discuss some further consequence of the main results.

In \cite{Chai}, Chai carried out a combinatorial study of the poset $(B(\bG), \leq)$. We will give a geometric proof of Chai's length formula.
\begin{theorem} \label{poset}
    The poset $(B(\bG), \leq)$ is ranked. Moreover, the length between two elements $[b'] \leq [b] \in B(\bG)$ is given by \[\leng([b], [b']) = \sum_{i=1}^r \lceil \<\mu-\nu(b'), \o_{\CO_i}\>\rceil - \lceil \<\mu-\nu(b), \o_{\CO_i}\>\rceil,\] where $\mu \in X_*(\bT)$ is any cocharacter such that $\k(\varpi^\mu) = \k(b) =\k(b')$.
\end{theorem}

\

In \cite{HNY}, He, Yu and the author shows that an affine Deligne-Lusztig varieties with a finite Coxeter part has a simple geometric structure. A key ingredient of our approach is a combinatorical identity, whose proof of the identity is indirect and quite involved. This motivates us to search a better interpretation. As a result, we will give a new and direct proof of the identity by counting $k$-rational points on the loop Steinberg's cross-section. The same idea also guides us to discover a new combinatorical identity.
...
\begin{remark}
    Theorem \ref{dec} is also proved in \cite{IN} using a different method. The last statement of theorem confirms a conjecture of Ivanov \cite[Conjecture 1.1]{Iv1}. If $\bG$ is of classical type, the theorem is proved by Ivanov \cite{Iv2}.
\end{remark}


\subsection{Outline} The note is organized as follows. In the first section, we introduce basic notions/constructions, and reduce Theorem \ref{main} to certain translation case, see Theorem \ref{st-cross}. In the second section, we study the intersection $[b] \cap \dc \bT(\COk) \bU_c(\brk)$ when $b$ is a translation, and show it equals the image of certain explicit map, see Proposition \ref{red}. In the third section, we make a digression to introduce an algorithm to compute the image. In the fourth section, we finish the proof of Theorem \ref{main} using the algorithm. In the last section, we prove Theorem 0.6 and Theorem 0.7 as applications of Theorem \ref{main}.

\subsection*{Acknowledgement} We would like to thank Miaofen Chen, Xuhua He, Alexander Ivanov and Qingchao Yu for helpful comments and suggestions.
...
We keep the notations $\bG, \bB, \bU, \bT, \bN, W$ introduced in the introduction. Let $\Phi^+ \subseteq \Phi$ be the set of (positive) roots of $\bT$ that appears in $\bB$. We have $\Phi = \Phi^+ \sqcup \Phi^-$ with $\Phi^- = - \Phi^+$. Let $\Pi \subseteq \Phi^+$ be the set of simple roots. Denote by $\BS \subseteq W$ the corresponding set of simple reflections of $W$. Then $(W, \BS)$ is a Coxeter system.

Let $\a \in \Phi$. We denote by $\a^\vee$ the coroot of $\a$, and let $s_\a \in W$ be the reflection sending $\l$ to $\l - \<\l, \a\> \a^\vee$ for $\l \in X_*(\bT)$. Choose root subgroups $\bU_\a: \BG_a \to \bG$ such that for $z \in \brk^\times$ we have \begin{align*} \bU_{-\a}(-z\i) \bU_\a(z) \bU_{-\a}(-z\i) = z^{\a^\vee} \tilde s_{-\a} \in \bN_\bT(\brk),\end{align*} where $\tilde s_{-\a} = \bU_{-\a}(-1) \bU_\a(1) \bU_{-\a}(-1) \in \bN(\CO_{\brk})$.



\subsection{} \label{cox-subsec}
Let $\{\a_1, \dots, \a_r\}$ be a representative set for the $\s$-orbits of $\Pi$. Let $c = s_{\a_1} s_{\a_2} \cdots s_{\a_r} \in W$, which is a (minimal) $\s$-twisted Coxeter element.
...
\end{lemma}
\begin{proof}
    We argue by induction on $r$. If $r = 0$, the statement is trivial. Let $z_1, \dots, z_r \in \brk^\times$ such that \[\tag{a} \varpi^\eta \bU_{-\a_1}(z_1) \cdots \bU_{-\a_r}(z_r) \in \bU(\brk) \dc \bT(\COk) \bU(\brk).\] Suppose that $z_r \in \varpi^{-n_r} \CO_\brk^\times$ for some $n_r \in \BZ$. By \S \ref{setup-subsec} (*) we have \begin{align*}
        &\quad\ \varpi^\eta \bU_{-\a_1}(z_1) \cdots \bU_{-\a_r}(z_r) \\ &= \varpi^\eta \bU_{-\a_1}(z_1) \cdots \bU_{-\a_{r-1}}(z_r) \bU_{\a_r}(z_r\i) z_r^{-\a_r^\vee} \tilde s_{\a_r} \bU_{\a_r}(z_r\i) \\ &\subseteq \bU_{\a_r}(\brk) \varpi^{\eta + n_r \a_r^\vee} \bT(\COk) \bU_{-\a_1}(z_r^{-\<\a_r^\vee, \a_1\>} z_1) \cdots \bU_{-\a_{r-1}}(z_r^{-\<\a_r^\vee, \a_{r-1}\> }z_{r-1}) \tilde s_{\a_r} \bU_{\a_r}(\brk).
    \end{align*} Thus (a) is equivalent to the following inclusion \[\varpi^{\eta + n_r \a_r^\vee} \bU_{-\a_1}(z_r^{-\<\a_r^\vee, \a_1\>} z_1) \cdots \bU_{-\a_{r-1}}(z_r^{-\<\a_r^\vee, \a_{r-1}\> }z_{r-1}) \in \bU(\brk) \dc \tilde s_{\a_r} \bT(\COk) \bU(\brk).\] Now the statement follows by induction hypothesis.
\end{proof}



\subsection{} \label{H-subsec}
Let $\Phi_{c\s}^+ = \Phi^+ \cap (c\s)\i(\Phi^-)$. Then $\Phi_{c\s}^+  = \{\b_1, \dots, \b_r\}$, where $\b_i = \s\i s_{\a_r} \cdots s_{\a_{i+1}}(\a_i) \in \Phi^+$ for $1 \le i \le r$. Note that $\bU_c = \prod_{i=1}^r \bU_{\s(\b_i)}$.

\begin{lemma} \label{commute}
    The root subgroups $\bU_{\b_i}$ for $1 \le i \le r$ commutes with each other.
\end{lemma}
\begin{proof}
Assume otherwise. Then $\b_i + \b_{i'} \in \Phi_{c\s}^+$ for some $1 \le i < i' \le r$. So $\b_i + \b_{i'} = \b_{i''}$ for some $1 \le i'' \le r$. As $c$ is product of distinct simple reflections, it follows that \[\s(\b_k) \in \a_k + \sum_{k < j \le r} \BZ_{\ge 0} \a_j.\] Thus $i'' = i$, which is impossible.
\end{proof}
...
For a simple root $\a \in \Pi$ let $\o_\a \in \BR\Phi$ be the fundamental weights corresponding to $\a$. For $1 \le i \le r$ let $\o_{\CO_i} = \sum_\a \o_\a$, where $\a$ ranges over the $\s$-orbit of $\a_i$. Let $w_0$ denote the unique longest element of $W$.
\begin{lemma} \label{eq}
    Let $\mu \in X_*(\bT)$ and $b \in G(\brk)$ such that $\k(\mu) = \k(b)$. Then there exists a unique vector $\l \in X_*(\bT)_\BQ$ modulo $X_*(\bZ)_\BQ^\s$ such that $\mu - \nu(b) = \l - c\s(\l)$. Here $\bZ$ denotes the center of $\bG$. Moreover,

    (1) $\<\l, \g\> + \<\mu, c\s(\g)\>  - \<\l, c\s(\g)\> = \<\nu(b), c\s(\g)\>$ for $\g \in \Phi$;

    (2) $\<\l, \b_i\> = \<\mu - \nu(b), \o_{\CO_i}\>$ for $1 \le i \le r$;

    (3) $\<\mu, \a_1\> - \<\l, \b_1\> - \<\l, \a_1\> = \<\nu_b, \a_1\>$.
\end{lemma}
\begin{proof}
    The existence of $\l$ follows from that $\k(\mu) = \k(b)$ and that $1 - c\s$ restricts to an isomorphism of the linear space $\BQ\Phi^\vee$.

    Now (1) follows directly from the equality $\mu- \nu(b) = \l - c\s(\l)$. Notice that $c\s(\l) = \s(\l) - \sum_{k=1}^r \<\b_k, \l\> \a_k^\vee$. Thus (2) follows from that \[ \<\mu - \nu(b), \o_{\CO_i}\> = \<\l - \s(\l), \o_{\CO_i}\> + \sum_{k=1}^r \<\l, \b_i\> \<\a_k^\vee, \o_{\CO_i}\> = \<\l, \b_i\>.\] Finally, (3) follows from (1) by noticing that $-\a_1 = c\s(\b_1)$.
\end{proof}

\begin{definition}
Let $\mu, b, \l$ be as in Lemma \ref{eq}. Define $J_{\nu(b)} = \{1 \le i \le r; \<\nu(b), \a_i\> \neq 0\}$. By \cite[Lemma 3.5]{HN}, \[\<\l, \b_i\> = \<\mu - \nu(b), \o_{\CO_i}\> \in \BZ \text{ for } i \in J_{\nu(b)}.\] We define \[H_{c, \mu, b} = {}^\s H_{c\s, J_{\nu(b)}}^\l \subseteq \bU_c(\brk),\] which is independent of the choice of $\l$.
\end{definition}

Now Theorem \ref{main} amounts to the following result.
\begin{theorem} \label{main'}
    Let $\mu \in X_*(\bT)$ and $b \in \bG(\brk)$ such that $\k(\mu) = \k(b)$. Let $\dc \in \varpi^\mu \bN(\COk)$ be a lift of $c$. Then \[[b] \cap \dc \bU_c(\brk) = \dc H_{c, \mu, b} = \dc \ts_{\a_1} H_{\mu, b, \a_1} \cdots \ts_{\a_r} H_{\mu, b, \a_r},\] where \[H_{\mu, b, \a_i} = \begin{cases} \bU_{\a_i}(\varpi^{\<\mu-\nu(b),  \o_{\CO_i}\>} \CO_\brk^\times); &\text{ if } i \in J_{\nu(b)}\\ \bU_{\a_r}(\varpi^{\lceil \<\mu-\nu(b), \o_{\CO_i}\> \rceil} \COk), &\text{ otherwise.} \end{cases}\]
\end{theorem}



\subsection{}
Let $\mu, \eta \in X_*(\bT)$ such that $\k(\mu) = \k(\eta)$ and the $\s$-average $\nu$ of $\eta$ is dominant. Let $\l \in X_*(\bT)_\BQ$ such that $\mu - \nu = \l - c\s(\l)$.



The following result is a special case of Theorem \ref{main'}.
\begin{theorem} \label{st-cross}
    Let $\mu, \eta, \l$ be as above. Let $b = [\varpi^\eta]$ and let $\dc \in \varpi^\mu \bN(\COk)$ be a lift of $c$. Suppose that $\l \in X_*(\bT)$. Then \[ [b] \cap \dc \bT(\COk) \bU_c(\brk) = \dc \bT(\COk) H_{c, \mu, b},\] that is, $[b] \cap \dc \bU_c(\brk) = \dc H_{c, \mu, b}$.
\end{theorem}

Now we prove Theorem \ref{main'} using Theorem \ref{st-cross}.
\begin{proof}[Proof of Theorem \ref{main'}]
Let $\l \in X_*(\bT)_\BQ$ such that $\mu - \nu(b) = \l - c\s(\l)$. Choose $N \in \BZ_{\ge 1}$ such that $\l \in \frac{1}{N} X_*(\bT)$. In particular, $\nu(b) \in \frac{1}{N} X_*(\bT)$ and hence $[b]_N = [\varpi^{\nu(b)}]_N$, where $[b]_N$ denotes the $\bG(\brk_N)$-$\s$-conjugacy class of $b$ with $\brk_N = \brk[\varpi^{1/N}]$. Applying Proposition \ref{st-cross} to the field $\brk_N$ we have \[[b]_N \cap \dc \bU_c(\brk_N) = \dc H_{c, \mu, b, N},\] where $H_{c, \mu, b, N}$ is defined similarly as $H_{c, \mu, b}$ by replacing $\brk$ with $\brk_N$. Thus \[[b] \cap \dc \bU_c(\brk) \subseteq [b]_N \cap \dc \bU_c(\brk_N) \cap \dc \bU_c(\brk)  = \dc H_{c, \mu, b, N} \cap \dc \bU_c(\brk)  = \dc H_{c, \mu, b}.\] On the other hand, for any $x \in \dc H_{c, \mu, b} \subseteq [b]_N$ we have $\nu(x) = \nu(b)$ and $\k(x) = \k(\dc) = \k(b)$. So $x \in [b] \cap \dc \bU_c(\brk)$, and the equality $[b] \cap \dc \bU_c(\brk) = \dc H_{c, \mu, b}$ follows.
\end{proof}
...
The aim of this section is to study the intersection $[b] \cap \dc\bT(\COk)\bU_c(\brk)$, where $b = \varpi^\chi$ is translation for some $\chi \in X_*(\bT)$.


First we recall a result which is essentially proved in Proposition (2.2) and Corollary (2.5) of \cite{L76} respectively.
...
Let $c \in W$ be a full $\s$-Coxeter element.  Then there is an isomorphism \[ \Psi_\dc:  \bU(\brk) \times \dc \bT(\COk)\bU_c(\brk) \overset \sim \to \bU(\brk) \dc \bT(\COk) \bU(\brk), \quad (x, y) \mapsto x\i y \s(x).\]
\end{theorem}

\begin{lemma} \label{unique}
    Let $\mu, \chi \in X_*(\bT)$ such that $\k(\mu) = \k(\chi)$. Then there exists a unique cocharacter $\eta \in X_*(\bT)$ such that $\mu - \eta \in \sum_{i=1}^r \BZ \a_i^\vee$ and $\chi - \eta \in (1-\s) X_*(\bT)$.
\end{lemma}
\begin{proof}
...
\begin{proposition} \label{red}
    Let $b = \varpi^\chi$ for some $\chi \in X_*(\bT)$ and let $\dc \in \varpi^\mu \bN(\COk)$ be a lift of $c$ for some $\mu \in X_*(\bT)$. Then $[b] \cap \dc \bU_c(\brk) \neq \emptyset$ if and only if $\k(\dc) = \k(b)$. In this case we have \[\tag{a}[b] \cap \dc \bT(\COk) \bU_c(\brk) = \pr_2 \Psi_{\dc}\i(\varpi^\eta \bT(\COk) K_{\mu-\eta}),\] where $\eta \in X_*(\bT)$ is the unique cocharacter such that $\mu - \eta \in \sum_{i=1}^r \BZ \a_i^\vee$ and $\eta - w_0(\chi) \in (1-\s)(X_*(\bT))$ as in Lemma \ref{unique}.
\end{proposition}
\begin{proof}
    It suffices to deal with the case that $\k(\chi) = \k(b) = \k(\dc) = \k(\mu)$.

    Let $g \in \bG(\brk)$ such that $g\i b \s(g) \in \dc \bT(\brk) \bU_c(\brk)$. By Proposition \ref{cox} (2), $g \in \bB(\brk) w_0 \bB(\brk)$, where $w_0 \in W$ is the longest element. Write $g = u \dw_0 u'$ for some $u, u' \in \bU(\brk)$ and some lift $\dw_0 \in \bN(\brk)$ of $w_0$. Then \[h := \dw_0\i u\i b \s(u) \s(\dw_0) \in \bU(\brk) \dc \bT(\COk) \bU(\brk).\] As $b = \varpi^\chi$, $h \in \varpi^\eta \bT(\COk) {}^{w_0} \bU(\brk)$ for some $\eta \in w_0(\chi) + (1 - \s)(X_*(\bT))$. In view of Proposition \ref{cox} (1) we have \[h \in \varpi^\eta \bT(\COk) \bU_{\a_1}(\brk^\times) \cdots \bU_{\a_r}(\brk^\times).\] By Lemma \ref{mu-eta} we have $\mu - \eta \in \sum_{i=1}^r \BZ \a_i^\vee$ and $h \in \varpi^\eta \bT(\COk) K_{\mu - \eta}$. In particular, $\eta$ is uniquely determined as in Lemma \ref{unique}.
    ...
    On the other hand, by Lemma \ref{mu-eta} we have \[\varpi^\eta \bT(\COk) K_{\mu-\eta} \subseteq [\varpi^\eta] \cap \bU(\brk) \dc \bT(\COk) \bU(\brk)  = [b] \cap  \bU(\brk) \dc \bT(\COk) \bU(\brk).\] So $[b] \cap \dc \bT(\COk) \bU_c(\brk) \supseteq  \pr_2 \Psi_{\dc}\i(\varpi^\eta \bT(\COk) K_{\mu-\eta})$ and the (a) follows.
\end{proof}



\section{A digression}
In this section, we digress to introduce a method to compute the right hand side of Proposition \ref{red} (a). For simplicity we assume that $\brk = \overline \BF_q((\varpi))$, where $\BF_q$ is a finite field with $q$ elements.


\subsection{} \label{R-sec}
Let $\l \in X_*(\bT)$ and $J \subseteq \{1, \dots, r\}$. Set $ H_J^\l := H_{c\s, J}^\l$

By definition, each point $x = (x_i^k)_{i, k}$ in $H_J^\l$ can be uniquely written as \[ x = \prod_{i=1}^r \bU_{\b_i}(\varpi^{\<\l, \b_i\>} \sum_{k=0}^\infty x_i^k \varpi^k),\] where $x_i^k \in \overline \BF_q^\times$ if $k=0$ and $i \in J$, and $x_i^k \in \overline \BF_q$ otherwise. Let $(X_i^k)_{i, k}$ be the coordinates of the points $x = (x_i^k)_{i, k} \in H_J^\l$. Set $\CR = R((\varpi))$, where \[R = \overline \BF_q[X_i^k; k \in \BZ_{\ge 0}, \ 1 \le i \le r] [(X_j^0)\i; j \in J].\]

Let $\e \in \BR$. Let $\CR^{\ge \e}$ be the linear space of elements $\sum_k f^k \varpi^k \in \CR$ such that
\begin{itemize}
    \item $f^k = 0$ if $k < \e$;

    \item $f^k \in \overline \BF_q[X_i^{k'}; 0 \le k' \le k-\e, 1 \le i \le r][(X_j^0)\i; j \in J]$ for $k \ge \e$.
\end{itemize}
 Similarly, let $\CR^{> \e}$ be the linear space of elements $\sum_k f^k \varpi^k$ such that
 \begin{itemize}
     \item $f^k = 0$ if $k \le \e$;

     \item $f^k \in \overline \BF_q[X_i^{k'}; 0 \le k' < k-\e, 1 \le i \le r][(X_j^0)\i; j \in J]$ for $k > \e$.
 \end{itemize}
...
The following results are immediate by definition.
 \begin{lemma} \label{CR}
     ...
     (2) $d \CR^{\ge \e} = \CR^{\ge \e+k}$ and $d \CR^{>\e} = \CR^{> \e+k}$ for $d \in \varpi^k \CO_\brk^\times$ with $k \in \BZ$.

     (3)  $f_j\i \in \CR^{\ge -\<\l, \b_j\>}$ for $j \in J$, where $f_j = \varpi^{\<\l, \b_j\>} \sum_{k=0}^\infty X_j^k \varpi^k \in \CR^{\ge \<\l, \b_j\>}$.
\end{lemma}

Let $\l \in X_*(\bT)$. We set $\bU_\a^{\ge \l} = \bU_\a(\CR^{\ge \<\l, \a\>})$ for $\a \in \Phi$ . It follows from Lemma \ref{CR} that \[ [\bU_\a^{\ge \l}, \bU_\b^{\ge \l}] \subseteq \prod_{i, j \ge 1} \bU_{i\a + j\b}^{\ge \l} \ \text{ for } -\a \neq \b \in \Phi.\] So we can define the following group \[ \bU^{\ge \l} = \prod_{\g \in \Phi^+} \bU_\g^{\ge \l}.\] Moreover, we can define $\bU_\g^{>\l}$ and $\bU^{>\l}$ in a similarly way. Note that $\bU^{>\l}$ is a normal subgroup of $\bU^{\ge \l}$.


\begin{lemma} \label{surj}
    Let $g \in h \bU_{c\s}^{>\l}$ with $h = \prod_{i=1}^r \bU_{\b_i}(f_i)$, where \[f_i = \varpi^{\<\l, \b_i\>} \sum_{k=0}^\infty X_i^k \varpi^k \text{ for } 1 \le i \le r.\]  Then $g$ gives a surjective endomorphism of $H_J^\l$.
\end{lemma}
\begin{proof}
    Write $g = \prod_{i=1}^r \bU_{\b_i}(g_i)$ with $g_i = \varpi^{\<\l, \b_i\>} \sum_{k=0}^\infty g_i^k \varpi^k \in \CR$. By assumption we have

    (a) $g_i^0 = X_i^0$;

    (b) $g_i^k = X_i^k + \d_i^k$ with $\d_i^k \in \overline \BF_q[X_{i'}^{k'}; 0 \le k' < k, \ 1 \le i' \le r][(X_j^0)\i; j \in J]$.
    By (a) we have $h(H_J^\l) \subseteq h(H_J^\l)$. It remains to show that $h(H_J^\l) = H_J^\l$.

    Let $y = (y_i^k) \in H_J^\l$. We construct a point $x = (x_i^k) \in H_J^\l$ inductively as follows. If $k = 0$, we set $x_i^0 = y_i^0$. Suppose that the point $(x_i^{k'})_{k' < k}$ is already constructed. In view of (b) we set \[x_i^k = y_i^k - \d_i^k((x_i^{k'})_{k' < k})\] and this finishes the construction of $x$. It follows from (a) and (b) that $y_i^k = g_i^k(x)$ and hence $g(x) = y$. So $g(H_J^\l) = H_J^\l$ as desired.
\end{proof}



\subsection{} \label{cross-subsec}
Let $\g \in \Phi^+$. Following \cite{M} we define \[\Cross_{c\s}(\g) = \Phi^+ \cap c\s(\BZ_{\ge 1} \g + \BZ_{\ge 0} \Phi_{c\s}^+).\] For $D \subseteq \Phi^+$ we set $\Cross_{c\s}(D) = \cup_{\g \in D} \Cross_{c\s}(\g)$.

The following result is proved in Theorem B and Lemma 2.37 of \cite{M}.
\begin{theorem} \label{cross}
    We have $\Cross_{\dc\s}^d(\Phi^+) = \emptyset$ for $d \gg 0$.
\end{theorem}
...
\begin{corollary} \label{conj}
      The image of $\pr_1 \circ \Xi_{\dc\s}^d$ is trivial for $d \gg 0$.
\end{corollary}
\begin{proof}
    For $d \in \BZ_{\ge 0}$ let $\bU_d \subseteq \bU$ be the subgroup generated by $\bU_\g$ for $\g \in \Cross_{c\s}^d(\Phi^+)$. By Theorem \ref{cross}, it suffices to show that \[\Xi_{\dc\s}(\bU_d(\CR) \times \bU_{c\s}(\CR)) \subseteq \bU_{d+1}(\CR) \times \bU_{c\s}(\CR),\] that is, $\dc\s \bU_{c\s}(\CR) \bU_\a(\CR) \subseteq \bU_{d+1}(\CR) \dc\s \bU_{c\s}(\CR)$ for $\a \in \Cross_{c\s}^d(\Phi^+)$. Let \[\G = \BZ_{\ge 1} \a + \BZ_{\ge 0}\Phi_{c\s}^+ \subseteq \Phi^+.\] Note that the following products \[\prod_{\g \in \G} \bU_\g = \prod_{\g \in \G \cap \Phi_{w_0 c\s}^+} \bU_\g  \prod_{\g \in \G \cap \Phi_{c\s}^+} \bU_\g\] are unipotent subgroups of $\bG$, which are independent of the orders in the products. For $x \in \bU_\g(\CR)$ and $y \in \bU_{c\s}(\CR)$ we have \[y x y\i \in \prod_{\g \in \G} \bU_\g(\CR).\] Write $y x y\i= x' y'$, where $x' \in \prod_{\g \in \G \cap \Phi_{w_0 c\s}^+} \bU_\g(\CR)$ and $y' \in \prod_{\g \in \G \cap \Phi_{c\s}^+} \bU_\g(\CR)$. Thus \[\dc\s y x = {}^{\dc\s} (x') \dc\s y' y,\] where $y' y \in \bU_{c\s}(\CR)$ and \[{}^{\dc\s} (x') \in \prod_{\g \in \Phi^+ \cap {}^{c\s} \G} \bU_\g(\CR) = \prod_{\g \in \Cross_{c\s}(\a)} \bU_\g(\CR) \subseteq \bU_{d+1}(\CR)\] as desired.
\end{proof}
...
First we show that the main result is independent of the choice of $\s$-twisted elements.

Recall that two element $w, w' \in W$ are equivalent by $\s$-cyclic shifts if there exists a sequence \[w=w_0, w_1, \dots, w_n = w'\] such that for each $1 \le i \le n$ the elements $w_{i-1}, w_i$ are $\s$-conjugate by a simple reflection, and have the same length.
\begin{lemma} \label{shift}
    Let $c'$ be another full $\s$-Coxeter element. Then Theorem \ref{st-cross} holds for $c$ if and only if it holds for $c'$.
\end{lemma}
\begin{proof}
    It is well-Known that all $\s$-Coxeter elements are equivalent by $\s$-cyclic shifts. So we can assume that $c' = s_{\a_1} c \s(s_{\a_1})$. Suppose that $\dc \in \varpi^\mu \bN(\COk)$ and let $\dc' = \tilde s_{\a_1}\i \dc \s(\tilde s_{\a_1}) \in \varpi^{s_{\a_1}(\mu)} \bN(\COk)$. Note that there is a natural isomorphism \begin{align*}\dc \bT(\COk) \bU_c(\brk) &\cong \varpi^\mu \bT(\COk) \times \tilde s_{\a_1} \bU_{\a_1}(\brk) \times \cdots \times \tilde s_{\a_r} \bU_{\a_r}(\brk) \\ &\cong \tilde s_{\a_1} \bU_{\a_1}(\brk) \times \varpi^{s_{\a_1}(\mu)} \bT(\COk) \times \tilde s_{\a_2} \bU_{\a_2}(\brk) \times \cdots \times \tilde s_{\a_r} \bU_{\a_r}(\brk). \end{align*} For $z \in \varpi^{s_{\a_1}(\mu)} \bT(\COk)$ and $u_i \in \bU_{\a_i}(\brk)$ the map \[\tilde s_{\a_1} u_1 z \tilde s_{\a_2} u_2  \cdots \tilde s_{\a_r} u_r \mapsto z \tilde s_{\a_2} u_2 \cdots \tilde s_{\a_r} u_r {}^\s(\tilde s_{\a_1} u_1)\] induces a bijection \[\psi: \dc \bT(\COk) \bU_c(\brk) \cong \dc' \bT(\COk) \bU_{c'}(\brk).\] Thus $\psi([b] \cap \dc \bT(\COk) \bU_c(\brk)) = [b] \cap \dc' \bT(\COk) \bU_{c'}(\brk)$. Now the statement follows from the following equality \begin{align*}
        ...
        &= \dc' \bT(\COk) H_{c', s_{\a_1}(\mu), b}, \end{align*} where the third equality follows from that \begin{align*} \<s_{\a_1}(\mu) - \nu(b), \o_{\CO_i}\> = \begin{cases} \<\mu - \nu(b), \o_{\CO_i}\>  - \<\mu, \a_i\>, &\text{ if } i = 1; \\ \<\mu -\nu(b), \o_{\CO_i}\>, &\text{ otherwise.} \end{cases} \end{align*} The proof is finished.
\end{proof}


\begin{lemma} \label{preserve}
     Let $\mu, b, \l$ be as in Theorem \ref{st-cross}. Let $\CR$ be as in \S \ref{R-sec} associated to the pair $(\l, J_{\nu(b)})$. Then the map $\Xi_{\dc\s}$ in \S \ref{cross-subsec} induces endomorphism of (1) $\bU^{\ge \l} \times \bU_{c\s}^{\ge \l}$ and (2) $\bU^{>\l} \times h_2 \bU_{c\s}^{>\l}$ for $h_2 \in \bU_{c\s}^{\ge \l}$.
\end{lemma}
\begin{proof}
    By the proof of Corollary \ref{conj} and that $\bU_{c\s}^{\ge \l}$ normalizes $\bU_{c\s}^{>\l}$, it suffices to show that \[{}^{\dc\s} \bU_{w_0 c\s}^{>\l} \subseteq \bU^{>\l} \text{ and } {}^{\dc\s} \bU_{w_0 c\s}^{\ge \l} \subseteq \bU^{\ge \l},\] where $w_0$ is the longest element of $W$. We only show the first inclusion. Indeed, for $\g \in \Phi_{w_0 c \s}^+$, that is, $c\s(\g) \in \Phi^+$, we have \begin{align*}{}^{\dc\s} \bU_\g(\CR^{> \<\l, \g\>}) &= \bU_{c\s(\g)}(\CR^{>\<\mu, c\s(\g)\> + \<\l, \g\>}) = \bU_{c\s(\g)}(\CR^{>\<\l, c\s(\g)\> + \<\nu_b, c\s(\g)\>}) \\ &\subseteq \bU_{c\s(\g)}(\CR^{>\<\l, c\s(\g)\>}),\end{align*} where the second equality follows from Lemma \ref{eq} (1), and the inclusion follows from that $\<\nu(b), c\s(\g)\> \ge 0$ since $c\s(\g) \in \Phi^+$. So the statement follows.
\end{proof}
...
\begin{proof}[Proof Theorem \ref{st-cross}]
    For simplicity we can assume that $\brk = \overline \BF_q((\varpi))$. Set $\nu = \nu(b)$ and $J = J_{\nu(b)}$. By Lemma \ref{shift} we may assume $\<\nu, \a_1\> \neq 0$ if $\nu$ is non-central.

     By Lemma \ref{unique} we may assume that $\mu - \eta = \sum_{i=1}^r n_i \a_i^\vee$ with $n_i \in \BZ$ for $1 \le i \le r$. By Lemma \ref{red} it suffices to show  \[\varpi^\eta \bT(\COk) K_{\mu - \eta} \s \sim_{\bU(\brk)} \dc\s \bT(\COk) H_{c\s, J}^\l.\] We argue by induction on $r$. If $r = 0$, the statement is trivial. Suppose $r \ge 1$. Let $\bM$ be the standard Levi subgroup generated by $\bT$ and the root subgroup $\bU_{\pm \a}$ for $\a \in \Pi - \CO_1$. Let $c_\bM = s_{\a_1} c$ and let $\dc_{\bM} \in \varpi^{\mu - n_1\a_1^\vee} \bN(\COk)$ be a lift of $c_\bM$. It follows from Lemma \ref{eq} that $\<\l, \b_i\> = \<\mu - \nu, \o_{\CO_i}\> = n_i$ for $1 \le i \le r$, and \[\mu-n_1\a^\vee - \nu = \l - c_\bM\s(\l).\] By induction hypothesis, we have \[\tag{a} \varpi^\eta  \bT(\COk) K_{\mu-n_1\a_1^\vee - \eta}^{\bM} \s \sim_{\bU_\bM(\brk)} \dc_\bM \s \bT(\COk) H_{c_{\bM}\s, J^\bM}^{\l, \bM},\] where $\bU_\bM = \bU \cap \bM$, $J^\bM = J - \{1\}$, and $K_{\mu-n_1\a_1^\vee - \eta}^{\bM}$, $ H_{c_{\bM}\s, J^M}^{\l, \bM}$ are defined similarly for $\bM$. Note that \[K_{\mu-\eta} = \bU_{-\a_1}(\varpi^{m_1} \CO_\brk^\times) K_{\mu-n_1\a_1^\vee-\eta}^\bM,\] where $m_1 = -n_1 - \sum_{i=2}^r n_i \<\a_i^\vee, \a_1\>$. Thus \begin{align*}
        ...
        &=\varpi^{\mu - n_1\a_1^\vee} \bT(\CO) \bU_{-\a_1}(\varpi^{-n_1} \CO_\brk^\times) \tilde s_{\a_2} \cdots \tilde s_{\a_r} \s H_{c_\bM\s, J^M}^{\l, \bM} \\
    ...
    \end{align*} where the relation $\sim_{\bU_M(\brk)}$ follows from (a) and the observation that $\bU_\bM$ commutes with $\bU_{-\a_1}$, and $d_z \in \CO_\brk^\times$
    is certain constant depending on $z \in \bT(\COk)$. Therefore, it suffices to show
    ...
    Set \[V_{c\s, J}^\l = \bU_{\b_1}(\varpi^{n_1} \CO_\brk^\times) H_{c_\bM\s, J^\bM}^{\l, \bM} \subseteq H_{c\s, J}^\l.\] Let $\CR$ be as in \S \ref{R-sec} associated to the pair $(J, \l)$. As $n_1 = \<\l, \b_1\>$, the right hand side of ($\ast$) is the image of map \[h^0 = (h_1^0, h_2^0) : V_{c\s, J}^\l \to \bU(\brk) \times \bU_{c\s}(\brk),\] where $h_1^0 = \bU_{\a_1}(d \varpi^{\<\mu, \a_1\>} f_1\i)$, $h_2^0 = \prod_{i=1}^r \bU_{\b_i}(f_i)$, and \[f_i = \varpi^{\<\l, \b_i\>} \sum_{k=0}^\infty X_i^k \varpi^k \in \CR^{\ge \<\l, \b_i\>}.\] Let $h^d = (h_1^d, h_2^d) = \Xi_{\dc\s}^d \circ h^0$ for $d \in \BZ$. By Corollary \ref{conj}, ($\ast$) is equivalent to that the image $\Im(h_2^d)$ of $h_2^d: V_{c\s, J}^\l \to \bU_{c\s}(\brk)$ (for $d \gg 0$) equals $H_{c\s, J}^\l$.

    Note that $f_1\i \in \CR^{\ge -\<\l, \b_1\>}$ by Lemma \ref{CR} (3). Moreover, It follows from Lemma \ref{eq} (3) that \[\<\mu, \a_1\> - \<\l, \b_1\> - \<\l, \a_1\> = \<\nu, \a_1\> \ge 0.\] So $h_1^0 \in \bU^{\ge \l}$, and $h_1^0 \in \bU^{>\l}$ if $\<\nu, \a_1\> > 0$.

    \

    First we assume that $\<\nu, \a_1\> \neq 0$. In this case, $h^0 \in \bU^{>\l} \times h_2^0 \bU_{c\s}^{> \l}$ and $V_{c\s, J}^\l = H_{c\s, J}^\l$. It follows from Lemma \ref{preserve} (2) that $h_2^d \in h_2^0 \bU_{c\s}^{>\l}$. By Lemma \ref{surj} we have $\Im(h_2^d) = H_{c\s, J}^\l$ as desired.


    Now we assume that $\<\nu, \a_1\> = 0$. Then $\nu$ is central by assumption. It follows from Lemma \ref{preserve} (1) that $h_2^d \in \bU_{c\s}^{\ge \l}$. Hence \[\Im(h_2^d) \subseteq {}^{\varpi^\l} \bU_{c\s}(\COk) = H_{c\s, J}^\l.\] By Lemma \ref{red} this means that \[[b] \cap \dc \bU_c(\brk) \subseteq \dc H_{c, \mu, b}.\] On the other hand, as $\nu$ is central and $\mu + c\s(\l) = \l + \nu$, we see that $\dc \s$ fixes the Moy-Prasard group $G_{\ge \l}$ generated by $\bT(\COk)$ and $\bU_\a(\varpi^{\<\l, \a\>} \COk)$ for $\a \in \Phi$. Since ${}^{\s\i} H_{c, \mu, b} = H_{c\s, J}^\l \subseteq G_{\ge \l}$, it follows from Lang's theorem that \[\dc H_{c, \mu, b} \subseteq [\dc] \cap \dc \bU_c(\brk) = [b] \cap \dc \bU_c(\brk).\] So $[b] \cap \dc \bU_c(\brk) = \dc H_{c, \mu, b}$ and the theorem also holds.
\end{proof}



\section{Applications}
Let $c = s_{\a_1} \cdots s_r$ and let $\dc \in \varpi^\mu \bN(\COk)$ with $\mu \in X_*(\bT)$ be a lift of $c$.


\subsection{The closure relation}  \label{subsec:closure}
First we recall the dominance order $\leq$ on $B(\bG)$. Namely, $[b'] \le [b] \in B(\bG)$ if $\k(b) = \k(b')$ and $\nu(b) - \nu(b') \in \sum_{\a \in \Pi} \BR_{\ge 0} \a^\vee$.

Now we prove Theorem \ref{closure}. By Theorem \ref{main} we have \[ \tag{a} \overline{[b] \cap \dc\bU_c(\brk)} = \varpi^\mu \prod_{i=1}^r \ts_{\a_i} \bU_{\a_i}(\varpi^{\lceil\<\mu - \nu(b), \o_{\CO_i}\> \rceil } \COk).\] Then it follows that $\overline{[b] \cap \dc \bU(\brk)} - ([b] \cap \dc \bU(\brk))$ is pure of codimension one with $\sharp J_{\nu(b)}$ irreducible components.


Let $[b'] \le [b]$. Then $\<\mu-\nu(b), \o_{\CO_i}\> \le \<\mu-\nu(b'), \o_{\CO_i}\>$ for $1 \le i \le r$. By (a) we have \[\overline{[b'] \cap \dc \bU(\brk)} \subseteq \overline{[b] \cap \dc \bU(\brk)}.\] On the other hand, Let $[b''] \in B(\bG)$ which intersects $\overline{[b] \cap \dc \bU(\brk)}$. It follows from Theorem \ref{main} that \[ \tag{a} \<\mu-\nu(b), \o_{\CO_i}\> \le \<\mu-\nu(b''), \o_{\CO_i}\> \text{ for } i \in J_{\nu(b'')}.\] Let $\Pi_1, \Pi_2 \subseteq \Pi$ be $\s$-stable subsets such that $\Pi = \Pi_1 \sqcup \Pi_2$ and \[\nu(b) - \nu(b'') = v_1 - v_2,\] where $v_i \in \BR_{\ge 0} \Pi_i^\vee$ for $i \in \{1, 2\}$. By (a) we can assume that $\<\nu(b''), \a\> = 0$ for $\a \in \Pi_2$. Let $\rho_2 \in \BR \Pi_2$ be the half sum of roots in $\Phi^+$ spanned by $\Pi_2$. Then $\<\nu(b''), \rho_2\> = 0$ and $\<\a^\vee, \rho_2\> > 0$ for $\a \in \Pi_2$. We have \[0 \le \<v_2, \rho_2) = \<v_1, \rho_2\> - \<\nu(b), \rho_2\> + \<\nu(b''), \rho_2\> \le 0.\] Thus $v_2 = 0$ and hence $[b''] \le [b]$ as desired.


\subsection{The poset} Now we prove Theorem \ref{poset}. Let \[[b'] = [b_0] < [b_1] < \cdots < [b_n] = [b']\] be a maximal chain in the poset $(B(\bG), \leq)$. We claim that

(a) $[b_{i-1}] \cap \dc \bU(\brk)$ is of codimension one in $\overline{[b_i] \cap \dc \bU(\brk)}$ for $1\le i \le n$

By Theorem \ref{closure}, there is $[b_i'] < [b]$ such that $\overline {[b_i'] \cap \dc \bU(\brk)}$ (of codimension one in $\overline{[b_i] \cap \dc \bU(\brk)}$) is an irreducible component of $\overline{[b_i] \cap \dc \bU(\brk)} - ([b_i] \cap \dc \bU(\brk))$ containing $[b_{i-1}] \cap \dc \bU(\brk)$. In particular, $[b_{i-1}] \leq [b_i'] < [b_i]$. This means that $[b_i'] = [b_{i-1}]$ since the chain $[b_{i-1}] \leq [b_i]$ is maximal. So (a) is proved.

By (a) it follows that the length of any maximal chain between $[b']$ and $[b]$ equals the codimension of $[b'] \cap \dc \bU(\brk)$ in $\overline{[b'] \cap \dc \bU(\brk)}$. So the poset $(B(\bG), \leq)$ is ranked, whose length function is given by \begin{align*}\leng([b], [b']) &= \codim ([b'] \cap \dc \bU(\brk), \overline{[b'] \cap \dc \bU(\brk)}) \\ &=  \sum_{i=1}^r \lceil \<\mu-\nu(b'), \o_{\CO_i}\>\rceil - \lceil \<\mu-\nu(b), \o_{\CO_i}\>\rceil,\end{align*} as desired.





\subsection{The combinatorial identity} \label{subsec:identity}
For $\mu \in X_*(\bT)$ recall that \[B(\bG, \mu)_\irr = \{[b] \in B(\bG, \mu); \nu(\varpi^\mu)-\nu(b) \in \sum_{\a \in \Pi} \BR_{>0} \a^\vee\}.\] First we show the following result.
\begin{lemma} \label{indec}
    Let $\dc \in \varpi^\mu \bN(\COk)$  be a lift of $c$ for some $\mu \in X_*(\bT)^+$ such that $B(\bG, \mu)_\irr \neq \emptyset$. Then \[
    \dc \bU_c(\varpi \COk) = \bigsqcup_{[b] \in B(\bG, \mu)_\irr} [b] \cap \dc\bU_c(\brk).\]
\end{lemma}
\begin{proof}
    We may assume $\mu$ is dominant. Then $\nu(\varpi^\mu)$ equal the $\s$-average $\mu^\diamond$ of $\mu$. Note that $B(\bG, \mu)_\irr \neq \emptyset$ if and only if $\mu^\diamond$ is non-central on each $\s$-orbit of connected components of $\BS$.

    If $[b] \in B(\bG, \mu)_\irr$, by definition \[\<\mu-\nu(b), \o_{\CO_i}\> = \<\mu^\diamond - \nu(b), \o_{\CO_i}\> > 0 \text{ for } 1 \le i \le r.\] So it follows from Theorem \ref{main} that $[b] \cap \dc\bU_c(\brk) \subseteq \dc\bU(\varpi\COk)$.

    On the other hand, let $[b'] \in B(\bG, \mu) - B(\bG, \mu)_\irr$ which intersects $\dc\bU_c(\varpi\COk)$. Then there exists a proper subset $\s$-stable $\Pi_1 \subsetneq \Pi$ and $v \in \sum_{\a \in \Pi_1} \BR_{>0} \a^\vee$ such that $v = \mu^\diamond - \nu(b')$. By Theorem \ref{main} we have $\<\nu(b'), \a\> = 0$ for $\a \in \Pi - \Pi_1$. Let $\rho_2$ be the half sum of roots in $\Phi^+$ spanned by $\Pi - \Pi_1$. Then \[ 0 \le \<\mu^\diamond, \rho_2\> = \<\nu(b'), \rho_2\> + \<v, \rho_2\> = \<v, \rho_2\> \le 0.\] Thus $\<\mu^\diamond, \rho_2\> = \<v, \rho_2\> = 0$. This means that $\<\a^\vee, \b\> = 0$ for $\a \in \Pi_1$ and $\b \in \Pi - \Pi_1$, and $\mu^\diamond$ is central on $\Pi - \Pi_1$. Therefore, $\Pi-\Pi_1$ is a union of $\s$-orbits of connected components of $\BS$. This contradicts that $\mu^\diamond$ is non-central on any $\s$-orbit of connected components of $\BS$. The proof is finished.
\end{proof}




Now we are ready to prove Theorem \ref{identity}. We can assume  $\mu$ is dominant. Let $m$ be an integer such that $m > \<\mu, \o_{\CO_i}\>$ for all $1 \le i \le r$. Suppose that the residue field of $k$ has $q$ elements. Then for any $[b] \in B(\bG, \mu)$ it follows from Theorem \ref{main} that \[\tag{a} \sharp ( ([b] \cap \dc \bU_c(k)) / \bU_c(\varpi^m \CO_k)) = (q-1)^{\sharp J_{\nu(b)}} q^{mr - \sharp J_{\nu(b)} - \leng([b], [\varpi^\mu])}.\]
...
Note that $\<\mu - \nu(\varpi^\mu), \o_{\CO_i}\> = \<\mu - \mu^\diamond, \o_{\CO_i}\> = 0$ for $1 \le i \le r$. We have the following decomposition \[\dc\bU_c(\COk) = \overline{[\varpi^\mu] \cap \dc\bU_c(\brk)} = \bigsqcup_{[b] \in B(\bG, \mu)} [b] \cap \dc\bU_c(\brk).\] Now the first identity follows in a similar way as above.
...
\bibitem{CI}
C.~Chan, A.~Ivanov, \emph{On loop Deligne--Lusztig varieties of Coxeter type for inner forms of $\GL_n$}, to appear in Camb. J. Math.
...
\bibitem{HNY}
X.~He, S.~Nie, Q.~Yu, \emph{Affine Deligne-Lusztig varieties wiht finite Coxeter parts}, arXiv:2208.14058.
