...
\end{aligned}
\end{equation}
In detail, the first line always exists because $D_{\mathrm{KL}}(\mathbb{P} \| \mathbb{Q})$ is a constant. The second line comes from Lemma~\ref{lem:kl}. And because $\left(u, v_1\right)$ and $\left(u, v_{2: n}\right)$ can be interchangeable when they are all sampled from $\mathbb{Q}$, we can obtain the result in the third line. In conclusion, since the inequality works for $\forall f$, we can obtain Lemma~\ref{lem:kl_long}
...
\section{Online Clustering\label{app:online}}
To avoid the trivial solution, we add the constraint that the prototype assignments must be equally partitioned following:
\begin{equation}
