...
\end{aligned}
\end{equation}
where $d_{\mathcal{A}}\left(G_1, G_2\right):=\inf _{\tau_1, \tau_2 \in \mathcal{T}} d\left(\tau_1\left(G_1\right), \tau_2\left(G_2\right)\right)$ for some distance $d(\cdot, \cdot)$.
...
\begin{equation}
\underset{G \in C_k}{\mathbb{E}}\left\|g\left(G_1\right)-\mu_k\right\| \leq c \sqrt{\frac{1}{t_k}} \mathcal{L}_{\text {align }}^{\frac{1}{4}}(g)+\tau(\sigma, \delta)
\end{equation}
...
\begin{equation}
\begin{aligned}
& c\left(\sum_{k=1}^K \sqrt{t_k}\right) \mathcal{L}_{\mathrm{align}}^{\frac{1}{4}}(f)+\tau(\sigma, \delta) \\
...
& \geq \sum_{k=1}^K \frac{t_k}{\|p\|} \underset{G \in C_k}{\mathbb{E}} \underset{G}{\mathbb{E} \in \tau(G)}\left\|p \circ g\left(G_1\right)-e_k\right\| \\ 
