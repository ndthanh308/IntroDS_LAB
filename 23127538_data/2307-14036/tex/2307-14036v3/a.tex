...
\newcommand{\subterm}{\trianglelefteqslant}
\newcommand{\TF}[2][]{{\triangledown_{\!#1}}(#2)}
\newcommand{\msn}{\RV{\m{s}(\m{n})}}
\newcommand{\msm}{\m{s}(\m{m})}
...
parent of $h$ gets multiplied, where the number of copies 
\RV{corresponds to} the
...
In this article we present yet another
...
\RV{A preliminary version of this article appeared in the proceedings of the
8th International Conference on Formal Structures for Computation and
Deduction~\cite{HM23}. AC-MPO is a new result. New examples
provide further illustration of the simulation of the Battle of Hercules
and Hydra.}
...
\RV{We write $s \subterm t$ if $t = C[s]$ for some context $C$.  We write
$s \prsubterm t$ if $s \subterm t$ and $s \neq t$.}
...
\RV{It has the \emph{subterm property} if the inclusion
${\prsuperterm} \subseteq {\to}$ holds.}
Moreover, the relation $\to$ is said to be a \emph{rewrite relation}
if it is closed under contexts and substitutions.
\RV{\emph{Rewrite orders} are rewrite relations that are strict orders,
and \emph{reduction orders} are rewrite orders that are well-founded.}
...
are non-collapsing.
\RV{A TRS $\xR$ is \emph{terminating} if $\Rb[\xR]$ is well-founded.}
...
\RV{Since equations in $\AC$ are rewrite
rules, we can view $\AC$ as a TRS. Using this fact, we
define the relation $\ac$
as the reflexive, transitive, and symmetric closure of $\Rb[\AC]$.}
...
A reduction order $>$ is \emph{AC-compatible} if the inclusion
...
exist multisets $X$ and $Y$ such that $\RV{N} = (\RV{M} - X) \uplus Y$, 
...
\RV{First we give a formal account of the Hydra Battle.}
...
\RV{We write $\xTH$ for the set of ground terms over $\SET{\h,\I,{\si}}$.
\emph{Encodings} of Hydras are terms $t$ in $\xTH$ with 
$\RT{t} \in \SET{\h,\I}$.}
\end{defi}

\RV{To improve readability we omit parentheses in terms with nested
$\si$ symbols in examples.}
...
\I(\I(\h) \si \h)) \,\si\, \h) \\
H_3 &= \I(\h \,\si\, \h \,\si\, \h \,\si\, \I(\I(\I(\h) \si \h) \si
\I(\I(\h)
\si \h) \si \I(\I(\h) \si \h)) \,\si\, \h \,\si\, \h) \\
H_4 &= \I(\h \,\si\, \h \,\si\, \I(\I(\I(\h) \si \h) \si \I(\I(\h) \si \h)
\si \I(\I(\h) \si \h)) \,\si\, \h \,\si\, \h)
\end{alignat*}
\RV{and they are encodings of Hydras. The term $\h \si \h$ is included in
$\xTH$ but not regarded as an encoding of a Hydra.}
\end{exa}
...
\end{cases}
\end{align*}
\RV{The transition relation $\Rightarrow_n$ on encodings of Hydras is
defined as follows: $H \Rightarrow_n H'$ if}
\begin{enumerate}
\Item
$H = \I(\h)$ and $H' = \h$, or
\Item
...
\RV{That $H$ and $H'$ are encodings of successive Hydras at stages $n$ and
$n+1$ in a battle is expressed as $H \Rightarrow_n H'$. So fights with
Hydras are represented by finite or infinite sequences of the form
$H_0 \Rightarrow_0 H_1 \Rightarrow_1 H_2 \Rightarrow_2 \cdots$.}

\begin{exa}[continued from \exaref{hydras}]
\RV{We have the following sequence:
\[
H_0 \,\Rightarrow_0\, H_1 \,\Rightarrow_1\, \cdots \,\Rightarrow_3\, H_4
\]
For instance, the first step is verified as follows: Let $\ell \to r$
be the third rule in $\xR_0$, and let
$C = \I(\I(\h) \,\si\, \I(\Box) \,\si\, \h)$ and
$\sigma = \SET{y \mapsto \I(\h)}$. Since $H_0 = C[\ell\sigma]$ and
$H_1 =_\AC C[r\sigma]$ hold, we have $H_0 \RRAC[0] H_1$ and so
$H_0 \Rightarrow_0 H_1$ is obtained.}
\end{exa}

\RV{Now we present our TRS encoding of the Hydra Battle. We represent
natural numbers $n$ by $\m{s}^n(\m{0})$, which is abbreviated to $\m{n}$.}

\begin{defi}
\RV{Let $\xF$ be the signature consisting of the constant $\m{0}$, the
unary symbol $\m{s}$, the five binary symbols $\m{A}\,\text{--}\,\m{E}$ as
well as the three symbols $\h$, $\I$, and $\si$ in \defref{hydra}.}
The \textup{TRS} $\xH$ \RV{over $\xF$} consists of the following 14
rewrite rules:
\begin{align*}
\m{A}(n,\I(\h)) &\nR{1} \m{A}(\m{s}(n),\h) &
\m{D}(n,\I(\I(x))) &\nR{8} \I(\m{D}(n,\I(x))) \\
...
The Battle is started with the term $\m{A}(\m{0},\RV{H})$ where $\RV{H}$
is the \RV{encoding}
of the initial Hydra. Rule 1 takes care of the dying Hydra
\btkz[ld=7,level 2/.style={sd},rotate=90]
\node[inner] at (0,0) {}
 child { node[leaf] {} };
\etkz\,. 
\RV{An application of the rule ends the battle with a term of
the form $\m{A}(n,\h)$. 
The $\h$ denotes here a dead Hydra; a Hydra with
only one head is represented by the term $\I(\h)$.}
Rule 2 cuts a head without grandparent node, and so no
...
\RV{Let $n$ be a natural number. If $H \Rightarrow_n H'$} then
$\m{A}(\RV{\m{n}},H) \RHAC[+] \m{A}(\RV{\msn},H')$\RV{.}
\end{thm}
...
step 
$H_0 \Rightarrow_0 H_1$
in the example fight in the introduction. The step is simulated by the
...
\emph{unsorted} and we establish in this article the result that it is AC
...
\begin{lem}
\label{lem:cut}
\RV{Let $n$ be a natural number. If $H \Rightarrow_n H'$}
then $\m{D}(\msn,H) \RHAC \m{E}(H')$.
\end{lem}
...
$\m{D}(\msn,H) \nR{13} \I(\m{C}(\msn,\h))$. Using
\lemref{copy} we obtain
\begin{align*}
\I(\m{C}(\msn,\h)) &\RHAC \I(\h^{n+1} \si \m{E}(\h))
...
$\m{D}(\msn,H) \ac \cdot \nR{11} \I(\m{C}(\msn,\I(t)))$. Using
\lemref{copy} we obtain
\[
\I(\m{C}(\msn,\I(t))) \RHAC \I(\I(t)^{n+1} \si \m{E}(\I(t)))
...
$\m{D}(\msn,H_1) \RHAC \m{E}(H_1')$ from the induction hypothesis.
Note that $\m{root}(H_1) = \I$. If $m = 1$ then
\begin{align*}
\m{D}(\msn,H) &\ac \m{D}(\msn,\I(H_1))
\nR{8} \I(\m{D}(\msn,H_1)) \RHAC \I(\m{E}(H_1'))
\nR{7} \m{E}(\I(H_1')) \\
&\ac \m{E}(\RV{H'})
\end{align*}
...
Our task is to show
\[
\m{A}(\m{n},H) \RHAC \m{A}(\msn,H')
\]
\RV{If $H \Rightarrow_n H'$ is derived from condition (1) or (2) in
\defref{Rn},} the claim is immediate by rules 1 and 2 of $\xH$.
\RV{Otherwise,}
$H \RRAC H'$. This implies $\m{root}(H) = \I$. Using rules $3$ and
$14$ together with \lemref{cut} yields
\[
\m{A}(\m{n},H) \nR{3} \m{B}(\m{n},\m{D}(\msn,H))
\RHAC \m{B}(\m{n},\m{E}(H')) \nR{14} \m{A}(\msn,H')
\qedhere
\]
\end{proof}

In the remaining part of this section we prove the converse of
\thmref{simulation}.

\begin{thm}
\label{thm:converse}
Let $H, H' \in \xTH$ be encodings of Hydras and let $n$ be a natural
number. If $\m{A}(\m{n},H) \RHAC \m{A}(\msn,H')$ then
\RV{$H \Rightarrow_n H'$.}
\end{thm}

In order to show the claim we need a few auxiliary lemmata.
\RV{Let $\xCH$ be the set of ground contexts over $\SET{\h,\I,{\si}}$.}

\begin{defi}
We define $U$ as the set consisting of all terms of the forms
$\m{A}(\m{n},t)$, 
$\m{B}(\m{n},C[\m{C}(\m{m},t)])$,
$\m{B}(\m{n},C[\m{D}(\msn,t)])$, and
$\m{B}(\m{n},C[\m{E}(t)])$,
...
\begin{lem}
\label{lem:U}
If $t \in U$ and $t \Rab[\xH\hh\cup\hh\AC]{*} u$ then $u \in U$.
\qed
\end{lem}
\begin{proof}
\RV{The claim is easily shown by induction on the length of 
$t \Rab[\xH\hh\cup\hh\AC]{*} u$.}
\end{proof}

In order to analyze the rewrite sequence
$\m{A}(\m{n},H) \RHAC \m{A}(\msn,H')$ we define three subsets
of $\xH$: $\xH_1 = \SET{1,2}$, $\xH_2 = \SET{\RV{3\,\text{--}\,9},14}$, and
$\xH_3 = \SET{\RV{10\,\text{--}\,13}}$. The second rewrite sequence in
...
The following statements hold for all terms $s\RV{, t} \in U$.
\begin{enumerate}
\item
...
$t = \m{B}(\m{n},\m{D}(\msn,\I(u)))$ for some $n \geqslant 0$ and
$u \in \xTH$. We have
$\pi(s) = \I(u) = \pi(\m{D}(\msn,\I(u))) = \pi(t)$ by the definition
of $t$.
\item
...
\item
If $s \nR{5} t$ then $s = \m{B}(\m{n},C[\m{C}(\RV{\msm},u)])$ and
$t = \m{B}(\m{n},C[u \si \m{C}(\RV{\m{m}},u)])$ for some $n \geqslant 0$,
$m \RVr{\geqslant} 0$,
$C \in \xCH$ and $u \in \xTH$. We have $\pi(s) = C[u^{m+\RV{2}}] \ac
C[u \si u^{\RV{m+1}}] = C[\pi(u \si u^{\RV{m+1}})] = 
\pi(C[u \si \m{C}(\RV{\m{m}},u)]) = \pi(t)$.
\item
...
\item
If $s \nR{8} t$ then $s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(u)))])$ and
$t = \m{B}(\m{n},C[\I(\m{D}(\msn,\I(u)))])$ for some
$n \geqslant 0$, $C \in \xCH$ and $u \in \xTH$. We have
$\pi(s) = C[\I(\I(u))] = \pi(t)$.
\item
If $s \nR{9} t$ then $s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(u) \si v))])$
and $t = \m{B}(\m{n},C[\I(\m{D}(\msn,\I(u)) \si v)])$ for some
$n \geqslant 0$, $C \in \xCH$ and $u, v \in \xTH$. In this case we
obtain $\pi(s) = C[\I(\I(u) \si v)] = \pi(t)$.
\item
If $s \nR{14} t$ then $s = \m{B}(\m{n},\m{E}(u))$ and
$t = \m{A}(\msn,u)$ for some $n \geqslant 0$ and $u \in \xTH$. In this
...
\item
If $s \nR{10} t$ then
$s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(\h \si u) \si v))])$ and
$t = \m{B}(\m{n},C[\I(\m{C}(\msn,\I(u)) \si v)])$ for some
...
\item
If $s \nR{11} t$ then
$s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(\h \si u)))])$ and
$t = \m{B}(\m{n},C[\I(\m{C}(\msn,\I(u)))])$ for some $n \geqslant 0$,
...
\item
If $s \nR{12} t$ then
$s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(\h) \si v))])$ and
$t = \m{B}(\m{n},C[\I(\m{C}(\msn,\h) \si v)])$ for some
...
\item
If $s \nR{13} t$ then $s = \m{B}(\m{n},C[\m{D}(\msn,\I(\I(\h)))])$
and $t = \m{B}(\m{n},C[\I(\m{C}(\msn,\h))])$ for some $n \geqslant 0$
...
Suppose $s = \m{A}(\m{n},H) \RHAC[+] \m{A}(\msn,H') = t$.
...
\end{enumerate}
\smallskip
We first consider \RV{\ref{a}.}
If $s \RHnAC[]{1} t$ is a root step using rule 1 then
$H = \I(\h)$ and $H' = \h$.
If $s \RHnAC[]{1} t$ is a root step using rule 2 then
$H \ac \I(\h \si u)$ and $H' \ac \I(u)$ for some term $u$.
Next we consider \RV{\ref{b}.} We have
...
\]
is obtained by \lemref{projection} and thus %also
$H \RRAC H'$. 
\RV{Hence, we conclude $H \Rightarrow_n H'$.}
\end{proof}
...
by associating ordinal numbers to Hydras \RV{(see~\cite{KM76,DM07} for
notions and notations for ordinal numbers).} Consider, for example, the
...
natural addition on ordinals, respectively,
...
functions are \RV{weakly monotone. Here a function 
$\phi$ of type $A_1 \times \cdots \times A_n \to B$ is
\emph{weakly monotone} if $\phi(a_1, \dots, a_i, \dots, a_n) \geqslant_B
\phi(a_1, \dots, b, \dots, a_n)$ whenever $a_i \geqslant_{A_i} b$. We call}
...
\end{cases}
\end{gather*}
where $\RV{a = {}} \m{lab}_f(\EVALA{t_1},\dotsc,\EVALA{t_n})$.
...
\end{gather*}
\RV{Since there is no need to label the AC symbol $\si$ in the encoding of
the Hydra Battle,} we assume
...
\RV{So in a quasi-model, AC symbols are interpreted as associative
and commutative functions.}
...
\Item
\RV{if $t \Rb[\AC] u$ then $\lab{t} \ac \lab{u}$}
\Item
...
$\alpha \in \xA^\xV$. 
\RV{The claim follows from the first and third statements.}
First suppose $t \Rb[\xR] u$ is a root step
...
\RV{The second statement is shown in the same way, but since} 
AC symbols are not labeled the rules of $\DEC$ do not come into play.
\RV{The third statement is obtained from the
the second statement together with the fact that $t \ac u$ implies
$t \Rab[\AC]{*} u$.}
\end{proof}
...
\m{C}_\xA(n,x) &= \RV{x \otimes n} + 1 \\
\m{A}_\xA(n,x) &= \ML{$\m{B}_\xA(n,x) = \m{D}_\xA(n,x) = x$}
\end{align*}
Here $\oplus$ denote natural addition on ordinals \RV{and $\otimes$
denote natural product characterized by
$x \otimes 0 = 0$ and
$x \otimes (n+1) = (x \otimes n) \oplus x$.
Both satisfy strict monotonicity.}
...
\tag{4}
\\
\m{C}_\xA(\m{s}_\xA(n),x) = \RV{(x \otimes n) \oplus x} + 1
&\EO \RV{(x \otimes n) \oplus x} + 1 = x \si_\xA \m{C}_\xA(n,x)
\tag{5}
\\ 
...
\tag{9}
\\
\makebox[15em][r]{$\m{D}_\xA(n,\I_\xA(\I_\xA(\h_\xA \si_\xA x) \si_\xA y))
= \omega^{\omega^{x+1} \Oplus y}$}
&\GO \omega^{\RV{(\omega^x \Otimes n)} \Oplus y \plus 1} =
\I_\xA(\m{C}_\xA(n,\I_\xA(x)) \si_\xA y)
\tag{10}
\\
\m{D}_\xA(n,\I_\xA(\I_\xA(\h_\xA \si_\xA x))) = \omega^{\omega^{x+1}}
&\GO \omega^{\RV{(\omega^x \Otimes n)} \plus 1} =
\I_\xA(\m{C}_\xA(n,\I_\xA(x)))
\tag{11}
\\
...
\,\R_{\DEC}\, \m{B}_{\RV{v+1}}(\m{0},\I(\I(\m{C}(\m{s}(\m{0}),\h)))) \\
\nR{5} {} &\m{B}_{\RV{v+1}}(\m{0},\I(\I(\h \si \m{C}(\m{0},\h)))) \\
\nR{4} {} &\m{B}_{\RV{v+1}}(\m{0},\I(\I(\h \si \m{E}(\h))))
\,\ac\, \m{B}_{\RV{v+1}}(\m{0},\I(\I(\m{E}(\h) \si \h))) \\
\nR{6} {} &\m{B}_{\RV{v+1}}(\m{0},\I(\m{E}(\I(\h \si \h)))) \\
\nR{7} {} &\m{B}_{\RV{v+1}}(\m{0},\m{E}(\I(\I(\h \si \h)))) \\
...
\begin{defi}
\label{def:acmpo}
\RV{\emph{Precedences} are strict orders on function symbols.}
...
\smallskip
\end{enumerate}
\RV{In the third condition $=_\AC$ is used instead of $=$ in the
definition of multiset extension.}
...
the conjunction of $\RT{s} > \RT{t}$ and
$s \ACMPO t'$ for all $t' \in \TF{t}$. These equivalences will be used
freely in the sequel.
\RV{The multiset comparison in the third condition is spelled out as
follows:}
...
original recursive path order of Dershowitz\RV{~\cite{D82}}, nowadays known
as the \emph{multiset path order}.
\RV{Moreover, if AC symbols are minimal
in a precedence, AC-RPO with the multiset status reduces to AC-MPO.} 
Hence the simplified AC-RPO will be called AC-MPO.  

The proof of the following result can be found in the appendix.
\RV{\emph{Incrementality} of AC-MPO 
means that for precedences $>$ and $\sqsupset$ the inclusion 
${\ACMPO} \subseteq {\ACMPO[\sqsupset]}$ holds
whenever ${>} \subseteq {\sqsupset}$.}
...
\RV{This is the reason that our termination proof employs semantic
labeling.}
...
Unlike earlier encodings, it makes use of AC symbols. This allows \RV{us}
...
by semantic labe\RV{l}ing was proved terminating by suitably instantiating
AC-RPO.

\RV{One of the reviewers for this article pointed out that the Hydra
battle can still be simulated even if rule $6$ in $\xH$ is replaced
by the simpler $\m{E}(x) \si y \R \m{E}(x \si y)$. While we expect
that this variant also has the termination property, the presented
termination methods are not applicable. In fact, the rule cannot be
ordered by AC-MPO and the AC symbol $\si$ cannot be labeled.}
\RV{The reviewer also suggested an alternative encoding of Hydras that
omits $\I$ from $\I(t_1 \si \cdots \si t_n)$.  For instance, $H_0$ in
\exaref{hydras} is written as $\I(\h) \si \I(\I(\h \si \h)) \si \h$ in
this encoding. While this simplifies representations of Hydras, it seems
difficult to construct an ordinal interpretation of $\si$ for semantic
labeling. Further investigations of AC termination techniques are
required.}

The finite TRS $\xH$ poses an interesting challenge for automatic
termination tools. None of the tools (\AProVE~\cite{APROVE},
\muterm~\cite{MUTERM}) competing in the ``TRS Equational'' category of 
\RV{the} Termination Competition
\RV{2024}\footnote{\RV{https://termcomp.github.io/Y2024/}}
succeeds on $\xH/\AC$.
...
Formalizing the techniques used in this article in a proof assistant is an
...
\section*{\RV{Acknowledgements}}

\RV{
We are grateful to the anonymous reviewers for their pertinent comments,
which helped to improve the presentation. We thank Teppei Saito for his
thorough feedback on the proofs for AC-MPO.}
...
\item
Suppose $\RT{s} > \RT{t}$ and $s \ACMPO t'$ \RV{for} all $t' \in \TF{t}$.
...
induction hypothesis yields $s \ACMPO u'$. Hence
$\RV{\SET{s}} \ACMPOm \TF{u}$ and thus $s \ACMPO u$ by case (2).
\item
Suppose $\RT{s} = \RT{t}$ and $\TF{s} \ACMPOm \TF{t}$. From $t =_\AC u$ we
derive $\RT{t} = \RT{u}$ and $\TF{t} \ACm \TF{u}$. As
$\TF{s} \ACMPOm \TF{t}$,
...
\end{itemize}
\item
Suppose $\RT{s} = \RT{t}$ and $\TF{s} \ACMPOm \TF{t}$. Also in this
...
\begin{proof}
Assume to the contrary $t \ACMPO t$. We derive a
...
AC compatibility (\lemref{AC compatibility}) of $\ACMPO$.
The induction hypothesis yields the desired contraction.
\item
If $t \ACMPO t$ is derived by case (2) then $\RT{t} > \RT{t}$, which
contradicts the irreflexivity of the precedence $>$.
\item
Suppose $\TF{t} \ACMPOm \TF{t}$. Let $U$ be the set of all 
\RV{proper subterms of}
$t$, and $\succ$ the restriction of $\ACMPO$ to $U \times U$. The
...
\RV{for an $f$-rooted term $t$ and a substitution $\sigma$ the multiset
$\TF{t\sigma}$ is the multiset sum of $\TF[f]{t'\sigma}$ for all 
$t' \in \TF{t}$.}
...
\item
Suppose $\RT{s} > \RT{t}$ and $\SET{s} \ACMPOm \TF{t}$. Clearly
$\RT{s\sigma} = \RT{s} > \RT{t} = \RT{t\sigma}$. \RV{Consider an arbitrary
term $u \in \TF{t\sigma}$. Let $f = \RT{t}$. There exists a term
$t' \in \TF{t}$ such that $u \in \TF[f]{t'\sigma}$. The
induction hypothesis yields $s\sigma \ACMPO t'\sigma$.}
Since
$u \in \TF[f]{t'\sigma}$ satisfies $u \subterm t'\sigma$,
we have
$t'\sigma \ACMPO[\geqslant] u$ by the subterm property
(\lemref{subterm property})
and thus $s\sigma \ACMPO u$ by transitivity (\lemref{transitivity}).
Hence $\SET{s\sigma} \ACMPOm \TF{t\sigma}$ and thus
$s\sigma \ACMPO t\sigma$ by case (2).
\item
Suppose $\RT{s} = \RT{t}$ and $\TF{s} \ACMPOm \TF{t}$.
\RV{We write $f$ for $\RT{s}$.}
...
\RV{From $s' \ACMPO t'$ we infer $s' \notin \xV$ and thus
$s'\sigma \notin \xV$.  Because of $s' \in \TF{s}$, we have
$\RT{s'} \neq f$ and thus $\RT{s'\sigma} \neq f$. Hence
$\TF[f]{s'\sigma} = \SET{s'\sigma}$. Consider a term 
$u \in \TF[f]{t'\sigma}$. Since $u \subterm t'\sigma$, we
obtain} $t'\sigma \ACMPO[\geqslant] u$ by the subterm property
(\lemref{subterm property}) \RV{and thus} $s'\sigma \ACMPO u$ by
transiti\RV{vi}ty
(\lemref{transitivity}). Hence
$\TF[f]{s'\sigma} \ACMPOm \TF[f]{t'\sigma}$ follows.
\Item
Suppose $s' =_\AC t'$. Since $=_\AC$ is closed under substitutions, we
have $s'\sigma =_\AC t'\sigma$ and thus
$\TF[f]{s'\sigma} \ACm \TF[f]{t'\sigma}$.
\smallskip
\end{itemize}
It follows that \RV{the derivation of} $\TF{s} \ACMPOm \TF{t}$ 
can be simulated, resulting in $\TF{s\sigma} \ACMPOm \TF{t\sigma}$.
...
\begin{proof}
We have $\TF{s} = \TF[f]{s} = \TF[f]{s_1} \uplus \TF[f]{s_2}$.
We distinguish three cases.
\begin{enumerate}
\item
Suppose $s' \ACMPO[\geqslant] t$ for some $s' \in \TF{s}$.
We obtain $\RT{s'} \neq f$ from $s' \in \TF{s} = \TF[f]{s}$
and thus $\TF[f]{s'} = \SET{s'}$.
If also $\RT{t} \neq f$, then $\TF[f]{t} = \SET{t}$ and thus
$s' \ACMPO[\geqslant] t$ leads to 
$\TF[f]{s} \supsetneq \TF[f]{s'} \ACMPOm[\geqslant] \TF[f]{t}$
and hence $\TF[f]{s} \ACMPOm \TF[f]{t}$. Suppose $\RT{t} = f$.
...
\begin{proof}
Suppose $s \ACMPO t$ and consider a 
context \RV{of the form} $C = f(\dots,\Box,\dots)$.
If $f \notin \xF_\AC$ then
...
Let $t' \in \TF[f]{t}$. We have $t \superterm t'$ and thus
...
$s' \ACMPO[\sqsupset] t'$. Thus, \RV{the derivation of}
