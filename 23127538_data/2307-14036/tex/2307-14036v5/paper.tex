...
\author{Nao Hirokawa\lmcsorcid{0000-0002-8499-0501}}[a]
\address{School of Information Science, JAIST, Japan}
\email{hirokawa@jaist.ac.jp}

\author{Aart Middeldorp\lmcsorcid{0000-0001-7366-8464}}[b]
\address{Department of Computer Science, University of Innsbruck, Austria}
...
A preliminary version of this article appeared in the proceedings of the
...
We write $s \subterm t$ if $t = C[s]$ for some context $C$.  We write
...
It has the \emph{subterm property} if the inclusion
...
\emph{Rewrite orders} are rewrite relations that are strict orders,
...
are non-collapsing.
A TRS $\xR$ is \emph{terminating} if $\Rb[\xR]$ is well-founded.
...
Since equations in $\AC$ are rewrite
...
exist multisets $X$ and $Y$ such that $N = (M - X) \uplus Y$, 
...
We write $\xTH$ for the set of ground terms over $\SET{\h,\I,{\si}}$.
\emph{Encodings} of Hydras are terms $t$ in $\xTH$ with 
$\RT{t} \in \SET{\h,\I}$.
\end{defi}

To improve readability we omit parentheses in terms with nested
...
\end{alignat*}
and they are encodings of Hydras. The term $\h \si \h$ is included in
...
\end{cases}
\end{align*}
The transition relation $\Rightarrow_n$ on encodings of Hydras is
...
That $H$ and $H'$ are encodings of successive Hydras at stages $n$ and
...
Now we present our TRS encoding of the Hydra Battle. We represent
natural numbers $n$ by $\m{s}^n(\m{0})$, which is abbreviated to $\m{n}$.

\begin{defi}
Let $\xF$ be the signature consisting of the constant $\m{0}$, the
unary symbol $\m{s}$, the five binary symbols $\m{A}\,\text{--}\,\m{E}$ as
well as the three symbols $\h$, $\I$, and $\si$ in \defref{hydra}.
The \textup{TRS} $\xH$ over $\xF$ consists of the following 14
rewrite rules:
\begin{align*}
\m{A}(n,\I(\h)) &\nR{1} \m{A}(\m{s}(n),\h) &
\m{D}(n,\I(\I(x))) &\nR{8} \I(\m{D}(n,\I(x))) \\
...
The Battle is started with the term $\m{A}(\m{0},H)$ where $H$
is the encoding of the initial Hydra. Rule 1 takes care of the dying Hydra
\btkz[ld=7,level 2/.style={sd},rotate=90]
\node[inner] at (0,0) {}
 child { node[leaf] {} };
\etkz\,. 
An application of the rule ends the battle with a term of
the form $\m{A}(n,\h)$. 
...
Let $n$ be a natural number. If $H \Rightarrow_n H'$ then
...
\begin{lem}
\label{lem:cut}
Let $n$ be a natural number. If $H \Rightarrow_n H'$
...
\m{A}(\m{n},H) \RHAC \m{A}(\msn,H')
\]
If $H \Rightarrow_n H'$ is derived from condition (1) or (2) in
\defref{Rn}, the claim is immediate by rules 1 and 2 of $\xH$. Otherwise,
...
Let $\xCH$ be the set of ground contexts over $\SET{\h,\I,{\si}}$.
...
\end{lem}
\begin{proof}
The claim is easily shown by induction on the length of 
$t \Rab[\xH\hh\cup\hh\AC]{*} u$.
\end{proof}

In order to analyze the rewrite sequence
$\m{A}(\m{n},H) \RHAC \m{A}(\msn,H')$ we define three subsets
of $\xH$: $\xH_1 = \SET{1,2}$, $\xH_2 = \SET{3\,\text{--}\,9,14}$, and
$\xH_3 = \SET{10\,\text{--}\,13}$. The second rewrite sequence in
...
$\pi(C[t]) = C[\pi(t)] = C[t] \ac D[t] \ac D[u] = D[\pi(u)] = \pi(D[u])$.
\end{proof}
...
The following statements hold for all terms $s, t \in U$.
\begin{enumerate}
\item
...
\item
If $s \nR{5} t$ then $s = \m{B}(\m{n},C[\m{C}(\msm,u)])$ and
$t = \m{B}(\m{n},C[u \si \m{C}(\m{m},u)])$ for some $n \geqslant 0$,
$m \geqslant 0$,
$C \in \xCH$ and $u \in \xTH$. We have $\pi(s) = C[u^{m+2}] \ac
C[u \si u^{m+1}] = C[\pi(u \si u^{m+1})] = 
\pi(C[u \si \m{C}(\m{m},u)]) = \pi(t)$.
\item
...
Next we consider \ref{b}. We have
...
\]
is obtained by \lemref{projection} and thus 
$H \RRAC H'$. 
Hence, we conclude $H \Rightarrow_n H'$.
\end{proof}
...
by associating ordinal numbers to Hydras (see~\cite{KM76,DM07} for
...
functions are weakly monotone. Here a function 
...
$\m{lab}_f\colon (S_1)_\xA \times \cdots \times (S_n)_\xA \to L_f$ for
...
\end{cases}
\end{gather*}
where $a = \m{lab}_f(\EVALA{t_1},\dotsc,\EVALA{t_n})$.
...
\end{gather*}
Since there is no need to label the AC symbol $\si$ in the encoding of
...
So in a quasi-model, AC symbols are interpreted as associative
...
\Item
if $t \Rb[\AC] u$ then $\lab{t} \ac \lab{u}$
\Item
...
$\alpha \in \xA^\xV$. 
The claim follows from the first and third statements.
...
The second statement is shown in the same way, but since
AC symbols are not labeled the rules of $\DEC$ do not come into play.
The third statement is obtained from the
...
\m{C}_\xA(n,x) &= x \otimes n + 1 \\
\m{A}_\xA(n,x) &= \ML{$\m{B}_\xA(n,x) = \m{D}_\xA(n,x) = x$}
\end{align*}
Here $\oplus$ denotes natural addition on ordinals and $\otimes$
denotes natural product characterized by
$x \otimes 0 = 0$ and
...
\tag{4}
\\
\m{C}_\xA(\m{s}_\xA(n),x) = (x \otimes n) \oplus x + 1
&\EO (x \otimes n) \oplus x + 1 = x \si_\xA \m{C}_\xA(n,x)
\tag{5}
\\ 
...
\,\R_{\DEC}\, \m{B}_{v+1}(\m{0},\I(\I(\m{C}(\m{s}(\m{0}),\h)))) \\
\nR{5} {} &\m{B}_{v+1}(\m{0},\I(\I(\h \si \m{C}(\m{0},\h)))) \\
\nR{4} {} &\m{B}_{v+1}(\m{0},\I(\I(\h \si \m{E}(\h))))
\,\ac\, \m{B}_{v+1}(\m{0},\I(\I(\m{E}(\h) \si \h))) \\
\nR{6} {} &\m{B}_{v+1}(\m{0},\I(\m{E}(\I(\h \si \h)))) \\
\nR{7} {} &\m{B}_{v+1}(\m{0},\m{E}(\I(\I(\h \si \h)))) \\
...
If $\xHl \cup \DEC$ is \textup{AC} terminating,
$\xH$ is \textup{AC} terminating on sorted terms.
\qed
\end{cor}
...
\begin{defi}
\label{def:acmpo}
\emph{Precedences} are strict orders on function symbols.
...
\smallskip
\end{enumerate}
In the third condition $=_\AC$ is used instead of $=$ in the
...
The multiset comparison in the third condition is spelled out as
follows:
...
original recursive path order of Dershowitz~\cite{D82}, nowadays known
as the \emph{multiset path order}.
Moreover, if AC symbols are minimal
...
This is the reason that our termination proof employs semantic
labeling.
...
Unlike earlier encodings, it makes use of AC symbols. This allows us
...
by semantic labeling was proved terminating by suitably instantiating
AC-RPO.

One of the reviewers for this article pointed out that the Hydra
...
The reviewer also suggested an alternative encoding of Hydras that
...
2024\footnote{https://termcomp.github.io/Y2024/} succeeds on $\xH/\AC$.
...
\item
Suppose $\RT{s} > \RT{t}$ and $s \ACMPO t'$ for all $t' \in \TF{t}$.
...
$\SET{s} \ACMPOm \TF{u}$ and thus $s \ACMPO u$ by case (2).
\item
...
for an $f$-rooted term $t$ and a substitution $\sigma$ the multiset
...
$\RT{s\sigma} = \RT{s} > \RT{t} = \RT{t\sigma}$. Consider an arbitrary
...
(\lemref{subterm property}) and thus $s'\sigma \ACMPO u$ by
transitivity
...
\smallskip
\end{itemize}
It follows that the derivation of $\TF{s} \ACMPOm \TF{t}$ 
...
context of the form $C = f(\dots,\Box,\dots)$.
If $f \notin \xF_\AC$ then
...
$s' \ACMPO[\sqsupset] t'$. Thus, the derivation of
