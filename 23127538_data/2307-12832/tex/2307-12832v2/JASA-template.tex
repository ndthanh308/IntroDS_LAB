...
\bigskip
\begin{abstract}
	We consider testing invariance of a distribution under an algebraic group of transformations, which includes permutations.
	In this context, it is commonly believed that one should strive to construct a test based on the entire group.
	We find that one can sometimes obtain dramatically more power by replacing the entire group with a tiny subgroup.
	Surprisingly, this allows us to obtain much more power at a much lower computational cost.
	We examine this finding in the popular group invariance-based Westfall \& Young MaxT multiple testing method.
...
\section{Introduction}\label{sec:intro}
	...
	Examples range from traditional permutation tests that date back to \citet{fisher1935}, to more recent developments in large-scale permutation-based multiple testing methods \citep{westfall1993resampling, tusher2001significance}, conformal prediction \citep{shafer2008tutorial}, and many others.
	Moreover, even standard statistical tools such as the $t$-test can be interpreted as group invariance tests \citep{lehmann1949theory, koning2022efficient}.
	
	Fundamentally, these methods test the null hypothesis that the law of a random variable $X$ is invariant under a compact algebraic \emph{group} $\mathcal{G}$: 
	\begin{align}\label{eq:null}
		H_0 : X \overset{d}{=} \overline{G} X, 
	\end{align}
	where $\overline{G}$ is uniformly (Haar) distributed on $\mathcal{G}$ and independent of $X$, against the alternative that it is not invariant under $\mathcal{G}$.
	In practice, one often has a null hypothesis that implies invariance, so that rejecting invariance also rejects the null hypothesis of interest.
	...
	Such a test is exact up to discreteness of the reference distribution, and uniformly most powerful against simple alternatives if $T$ is chosen to be equal to the density of the alternative \citep{lehmann1949theory}.
	...
	Recently, \citet{koning2022efficient} proposed an alternative approach in which they use a reference distribution based on a sub\emph{group} of $\mathcal{G}$, which is strategically selected based on the choice of test statistic and the alternative of interest.
	...
	The main contribution of this paper is the surprising observation that a tiny subgroup can sometimes yield a method that dramatically outperforms the entire group.
	This is illustrated in the right panel of Figure \ref{fig:intro}, where a method based on a subgroup with just 16 to 64 elements substantially outperforms both the methods based on larger subgroups which nest the smaller subgroups, as well as the Monte Carlo method based on a large number of draws.
	This implies that, contrary to popular belief, one should not necessarily aspire to use or approximate the reference distribution that is based on the entire group.
	Indeed, if we would want to test invariance under the group with 1024 elements, one would be better off using a subgroup with 64 elements, or even just 16 elements.
	...
	From this perspective, it can perhaps even be viewed as surprising that this higher power was not observed by \citet{koning2022efficient}.
	
	% Additional contributions
	We analyze the impact of our findings in a high-dimensional location model for the group invariance-based \citet{westfall1993resampling} MaxT multiple testing method, which is perhaps the most popular multiple testing method for controlling the familywise error rate.
	...
	The ideal subgroups rely on an orthogonality condition that can only hold if the subgroup has at most $n$ (for a two-sided hypothesis) or $2n$ (for a one-sided hypothesis) elements, which explains the drop in power in Figure \ref{fig:intro} as the subgroup grows beyond $2n = 64$ elements.
...
\section{The group invariance based maxT method}	
	...
	The goal is to test the $p$ hypotheses $H_0^j : \mu_j = 0$ against the $p$ alternatives $H_1^j : \mu_j > 0$, with $j = 1, \dots, p$, with a method that has high \emph{power}: the expected proportion of correctly rejected hypotheses.
