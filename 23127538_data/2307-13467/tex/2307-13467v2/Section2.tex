...
The generation of the $n$th physical signal that is to be transmitted is modeled by a voltage source, with complex envelope $v_{{\rm G},n}$, in series with the impedance $Z_{{\rm G}}=R_{{\rm G}}+jX_{{\rm G}}$. The average available power of the voltage generator is $P_{{\rm a},n}=\frac{\Exp\{ |{v}_{{\rm G},n}|^{2}\}}{4R_{{\rm G}}}$
...
\begin{equation}
\label{eq:ZMT}
{\bf Z}_{\rm MT} = \left[\begin{array}{cc} {\bf Z}_{{\rm MT},11} & {\bf Z}_{{\rm MT},12} \\ {\bf Z}_{{\rm MT},21} & {\bf Z}_{{\rm MT},22} \end{array}\right] \quad \quad {\bf Z}_{\rm MR} = \left[\begin{array}{cc} {\bf Z}_{{\rm MR},11} & {\bf Z}_{{\rm MR},12} \\ {\bf Z}_{{\rm MR},21} & {\bf Z}_{{\rm MR},22} \end{array}\right]
\end{equation}
...
\end{equation}
where $R_{\rm N}$ is the so-called \textit{noise resistance} of the LNAs, usually indicated in the manufacturer data sheets. The complex parameter $\rho=\dfrac{\Exp\{{v}_{{\rm LNA},m}{i}^{\ast}_{{\rm LNA},m}\}}{\sqrt{\Exp\{|{v}_{{\rm LNA},m}|^{2}\}\Exp\{|{i}_{{\rm LNA},m}|^{2}\}}}$ accounts for the correlation between voltage and current noise generators at each port.
% \begin{equation}
% \label{ro}
...
\subsection{Transmit power and noise covariance matrix} The \textit{transmit power} is defined as the average active power at the output of the transmit matching network, or equivalently, at the input of the transmit antenna array, i.e., $P_{\rm T}=\Exp\{\re({\bf v}^{\Htran}_{\rm AT}{\bf i}_{\rm AT}) \}$.
% \begin{equation}
% \label{eq:P_T}
...
\begin{equation}
\label{eq:P_T_new}
P_{\rm T}=\frac{1}{4R_{\rm G}}\Exp\{\re({\bf v}_{\rm G}^{\Htran}{\bf B}{\bf v}_{\rm G}) \}
\end{equation}
with 
\begin{equation}
\label{matB2}
{\bf B}=4R_{\rm G} (Z_{\rm G}{\bf I}_{N}+{\bf Z}_{{\rm T}})^{-\Htran} \re\{{\bf Z}_{{\rm T}}\}  (Z_{\rm G}{\bf I}_{N}+{\bf Z}_{{\rm T}})^{-1}.
\end{equation}
...
%and ${\bf d}$ reduces to
%\begin{align}
%{\bf d}&=\frac{1}{2}\frac{Z_{L}}{Z_{G}(Z_{L}+Z_{\rm opt})}{\bf z}_{{\rm RT}}
%\end{align}
%with 
%\begin{align}
%{\bf z}_{{\rm RT}}&=\frac{ \re\{{Z}_{{\rm AT}}\}^{-1/2}}{\sqrt{\re\{Z_{\rm opt}\}\re\{Z_{\rm G}\}}}\re\{{\bf Z}_{{\rm AR}}\}^{-1/2}{\bf z}_{{\rm ART}} .
%\end{align}
%The power coupling matrix reduces to ${\bf B}= {R_{G}^2}/{|Z_{G}|^2}  {\bf I}_N$ such that the transmit power becomes
%\begin{equation}
%\label{TxPow2}
%P_{T}= \dfrac{R_{G}}{4|Z_{G}|^2}\Exp\{ {\bf v}^{\Htran}_{G} {\bf v}_{G}\}.
%\end{equation}
The covariance matrix ${\bf R}_{\eta} = \dfrac{|Z_{\rm L}|^2}{|Z_{\rm L}+Z_{\rm opt}|^2} \sigma^2 {\bf I}_M$
% \begin{align}
...
%\hline
% %Variance of the voltage noise source & $\sigma^2_u = 2 k_B B_W T_{\rm A}R_{\mathrm N}$  \\
% \hline
% Variance of the current noise source & $\sigma^2_i = 2 k_B B_W T_{\rm A}/R_{\mathrm N}$  \\
% \hline
% LNA noise resistance & $R_{\mathrm N} = 5\,\si{\ohm}$  \\
% \hline
% Complex correlation coefficient& $\rho =0.1$  \\
% \hline
% Dissipation resistance   & $R_{\mathrm d} = 10^{-3}R_{\mathrm r}$ \si{\ohm} \\
\hline
...
Next, we show how to compute the mutual coupling impedance matrix ${\bf Z}_{\rm A}$. In particular, we assume that the antennas at both sides are half-wavelength dipoles of length $l_{d}=\lambda/2$ and radius $a_{d} \ll l_{d}$. Moreover, we assume that the receiver is equipped with a uniform linear array.
...
\end{equation}
where $e_{qp}(s)$ is the component (along the direction of dipole $q$) of the electric field produced by a current $I_{p}(s')$ flowing in dipole $p$, $I_{q}(s)$ is the current flowing in dipole $q$, and finally $I_{p}$ and $I_{q}$ are the currents at the input terminals of dipoles $p$ and $q$, respectively. The current distributions $I_{p}(s')$ and $I_{q}(s)$, for $p,q=1,2,\ldots,N$,  can be found by solving a system of Hall\'en integral equations~\cite[\S 25.7]{orfanidis}. 
% Once, $I_{p}(s')$ and $I_{q}(s)$ are available, $e_{qp}(s)$ can easily be determined by standard electromagnetic formulas. 
The solutions of the Hall\'en integral equations is found by numerical methods (e.g. the method of moments discussed in~\cite[\S 24.8]{orfanidis}). A very good approximation of the current distributions $I_{n}(s)$ for \textit{center-fed} dipoles is represented by the \textit{sinusoidal current model}, i.e.,
\begin{equation}
\label{Insin}
I_{n}(s)=I_{n} \dfrac{\sin \left[k\left({l_{d}}/{2}-|s|\right)\right]}{ \sin(kl_{d}/2)}.
\end{equation}
where $k=2 \pi/\lambda$ is the wavenumber. Based on~\eqref{Insin}, closed form expressions for the mutual impedance can be found in~\cite[Eqs. (8.69) and (8.71a-b)]{balanis} for dipoles in \textit{side-by-side configuration}.
% \begin{subequations}
% \label{Mutual}
% \begin{equation}
% \label{ReMutual}
% R_{\rm mut}=\dfrac{Z_{0}}{4 \pi \sin^{2}(kl_{d}/2)}[2C_{i} (u_{0}) - C_{i} (u_{1}) - C_{i} (u_{2})]
% \end{equation}
% \begin{equation}
% \label{ImMutual}
% X_{\rm mut}=-Z_{0}\dfrac{Z_{0}}{4 \pi \sin^{2}(kl_{d}/2)}[2S_{i} (u_{0}) - S_{i} (u_{1}) - S_{i} (u_{2})]
% \end{equation}
% \end{subequations}
% where $Z_{0}$ is the intrinsic impedance of vacuum, $S_{i} (x)=\int_{0}^{x} \frac{\sin t} {t} {\rm d}t$ and $C_{i} (x)=-\int_{x}^{\infty} \frac{\cos t} {t} {\rm d}t$ are the sine and cosine integral functions. Also, we have that $u_{0}=kd$ and
% \begin{equation}
% \label{u1}
% u_{1}=k\Big(\sqrt{d^{2}+l_{d}^{2}}+l_{d}\Big) \quad\quad
% u_{2}=k\Big(\sqrt{d^{2}+l_{d}^{2}}-l_{d}\Big)
% \end{equation}
% with $d$ being the distance between dipoles and $l_{d}$ being their common length.
...
In the case of Hertzian dipoles, a uniform current distribution is typically assumed, which approximates well the current distribution of an \textit{infinitesimal} linear wire $(l_{d} \le \lambda/50)$ with plates at its endpoints~\cite[\S 4.2]{balanis}. The sinusoidal model is a more accurate representation of the current distribution of any wire antenna~\cite[\S 4.3]{balanis}.\vspace{-0.3cm} 
\end{remark}
...
The impedance matrix ${\bf Z}_{{\rm ART}}$ is needed to obtain the \textit{open-circuit} voltage array response represented by ${\bf v}_{\rm OC}={\bf Z}_{{\rm ART}} {\bf i}_{\rm AT}={\bf D}_{\rm OC} {\bf v}_{\rm G}$, which requires the computation of ${\bf D}_{\rm OC}$. In general, ${\bf v}_{{\rm OC}}$ is influenced by various factors, including the type of antennas, array configuration, polarization and transmission medium. Assuming that the receive array is in the far-field region of the transmitter and possible scatterers, ${\bf v}_{{\rm OC}}$ is produced by the superposition of a (possibly large) number $L$ of plane waves, each reaching the receiver from a particular azimuth angle $\phi_i \in [-\pi/2, \pi/2)$ and elevation angle $\theta_i\in [-\pi/2, \pi/2)$. In the Appendix, we assume a single transmitting antenna (i.e., ${\bf Z}_{{\rm ART}}$ and ${\bf D}_{\rm OC}$ reduce to the vectors ${\bf z}_{{\rm ART}}$ and ${\bf d}_{\rm OC}$, respectively) and show that
\begin{equation}
\label{multipath}
