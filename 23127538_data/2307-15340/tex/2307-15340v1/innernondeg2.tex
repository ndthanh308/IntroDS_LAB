...
A link type $L$ arises as the link of a weakly isolated singularity of an inner non-degenerate mixed function with nice Newton boundary if and only if $L$ is as in Theorem~\ref{thm:main1} or it has a representative $L([L_1,L_2,\ldots,L_{N-1}],L_N)$ with $N>1$, where
\begin{itemize}
...
\end{itemize}
\end{theorem}
Note that the different $L_i$, $i\in\{2,3,\ldots,N-1\}$, can satisfy different symmetry requirements. $L_2$ could be odd, for example, while $L_3$ is fixed-point-free even.
...
Although Oka's notion of Newton non-degeneracy in \cite{oka} is not equivalent to inner non-degeneracy (see Example~3.4 in \cite{AraujoBodeSanchez}), the two definitions are closely related. In particular, we find that the set of links of singularities of convenient, Newton non-degenerate mixed functions with nice Newton boundary is almost identical to the set of links associated with inner non-degenerate mixed functions with nice Newton boundary. The only difference is that the set of fixed points of $\tau_u$ or $\tau_v$ can no longer appear as a component of $L_1$ or $L_N$ if the other components of $L_1$ or $L_N$ are $v$-even or $u$-even, respectively.
...
\begin{proposition}\label{cor:Oka}
A link type $L$ arises as the link of a weakly isolated singularity of a convenient, Newton non-degenerate mixed function with nice Newton boundary if and only if it is fixed-point-free even or odd or it has a representative $L([L_1,L_2,\ldots,L_{N-1}],L_N)$ with $N>1$ for a sequence of links $L_1\subset\mathbb{C}\times S^1$, $L_i\subset(\mathbb{C}\backslash\{0\})\times S^1$ for all $i\in\{2,3,\ldots,N-1\}$, $L_N\subset S^1\times\mathbb{C}$, all of which are fixed-point-free even or odd.
\end{proposition}
...
Let $f:\mathbb{C}^2\to\mathbb{C}$ be an inner non-degenerate, mixed function with nice Newton boundary with $N$ compact 1-faces. Then the link of its weakly isolated singularity is of the form $L([L_1,L_2,\ldots,L_{N-1}],L_N)$ with each $L_i$ is as specified in Theorem~\ref{thm:main2}.
\end{lemma}
\begin{proof}
The case of $N=1$ is covered by Theorem~\ref{thm:main1}. Now assume $N>1$.
As usual we write $P_i$, $i=1,2,\ldots,N$, for the weight vectors associated to the 1-faces of $f$.
We know from \cite{AraujoBodeSanchez} that $f$ has a weakly isolated singularity, whose link is of the form $L([L_1,L_2,\ldots,L_{N-1}],L_N)$, where each $L_i\subset(\mathbb{C}\backslash\{0\}\times S^1)$, $i=2,\ldots,N-1$, is given by the roots with non-zero $u$-coordinate of the $g$-polynomial associated with the face function $f_{P_i}$, $L_1\subset\mathbb{C}\times S^1$ is given by the roots of the $g$-polynomial of $f_{P_1}$ and $L_N\subset S^1\times\mathbb{C}$ is given by the zeros of the $h$-polynomial associated with the face function $f_{P_N}$. As pointed out in \cite{AraujoBodeSanchez}, some of the $L_i$ might be the empty set, which of course possesses all the symmetries discussed in this article.

Since every face function is radially weighted homogeneous, all links must be even or odd by Lemma~\ref{lem1mix}. Write $P_i=(p_{i,1},p_{i,2})$ with $\gcd(p_{i,1},p_{i,2})$ and $i\in\{1,2,\ldots,N\}$. If $p_{1,1}$ is even, then $L_1$ is $u$-even and it must be fixed-point-free, since there are no fixed points of $\tau_u$ in $\mathbb{C}\times S^1$. The analogous statement holds for $L_N$ if $p_{N,2}$ is even.

If $p_{1,2}$ is even, then $L_1$ is $v$-even. By the same arguments as in Lemma~\ref{lem:convcases} it is fixed-point-free if $f$ is $v$-convenient and it is the union of a fixed-point-free $v$-even link and the set of fixed points of $\tau_v$ if $f$ is not $v$-convenient. The analogous statement holds for $L_N$ if $p_{N,1}$ is even.
...
Together with Lemma~\ref{lem:miximplies} this implies Proposition~\ref{cor:Oka}, since convenient, Newton non-degenerate polynomials are inner non-degenerate \cite{AraujoBodeSanchez}.
...
This proves Theorem~\ref{thm:main2}. The construction in Lemma~\ref{lem:constrmix} also illustrates what was stated in \cite{AraujoBodeSanchez} for mixed polynomials whose face functions corresponding to 1-faces have non-empty zero sets in $(\mathbb{C}^*)^2$. If $L_1,L_2,\ldots,L_N$ is a sequence of non-empty links in the usual ambient 3-manifolds, then $L([L_1,L_2,\ldots,L_{N-1}],L_N)$ is the link of the singularity of an inner non-degenerate mixed polynomial with nice Newton boundary if and only if it is the link of the singularity of such a polynomial with exactly $N$ compact 1-faces in its Newton boundary. In particular, the knot $8_{16}$ from Example~\ref{ex:knot} cannot be realised as the link of the singularity of any inner non-degenerate mixed function with nice Newton boundary, nor as the link of the singularity of a convenient, Newton non-degenerate mixed function with nice Newton boundary. Since $8_{16}$ is fibered, this also shows that in order to prove the Benedetti-Shiota conjecture \cite{benedetti}, degenerate mixed polynomials must be considered.
...
\cup_{j=1}^s\cup{t\in[0,2\pi]}(u_j(t),\rme^{\rmi t})\subset\mathbb{C}\times S^1
\end{equation} 
...
\bibitem{bode:sat} B. Bode. \textit{Twisting and satellite operations on P-fibered braids}. Communications in Analysis and Geometry (In press, 2021).
