...
\keywords{}
\address{J.~Dianetti: Center for Mathematical Economics (IMW), Bielefeld University, Universit\"atsstrasse 25, 33615, Bielefeld, Germany}
...
The rest of the paper is organized as follows. In Section \ref{Section 2}, we introduce the probabilistic setting and the MFG under study. Next, in Section \ref{Chapter 3}, for a given and fixed mean-field parameter, we solve the ergodic stochastic control problem faced by the representative player. In Section \ref{Mean field analysis} we then prove the existence and uniqueness of the mean-field equilibrium, while in Section \ref{Approximation} we provide the approximation result for a related $N$-player symmetric game. Finally, technical proofs are collected in the Appendices \ref{Appendix A} and \ref{Appendix B}.
...
Let $(\Omega,\mathcal{F},\mathbb{P})$ be a probability space which satisfies the usual conditions, on which it is defined a one-dimensional Brownian motion $\{W_{t}\}_{t\geq 0}$ and an independent irreducible continuous-time Markov chain $\{Y_{t}\}_{t\geq 0}$. Denote by $\mathbb{F}:=\{\mathcal{F}^{W,Y}_{t}\}_{t\geq 0}$ the filtration which is generated by $W$ and $Y$, as usual augmented by $\mathbb{P}$-null sets of $\mathcal{F}$. The Markov chain $Y$ has state space $\mathbb{Y}:=\{1,...,d\}$ and transition matrix $\mathbb{Q}:=\{q_{ij}\}_{1\leq i,j\leq d}$. The transition rates are such that $\kappa_{i}:=-q_{ii}>0$ and the condition $\sum_{j\in\mathbb{Y}}q_{ij}=0$ holds for every $i\in\mathbb{Y}$. Accordingly, the transition probabilities of $Y$ are defined as
\begin{equation*}
    \mathbb{P}(Y_{t+\Delta t}=j|Y_{t}=i):=\begin{cases}
                                               q_{ij}\Delta t+o(\Delta t), \qquad \ \; j\neq i, \\
                                               1+q_{ii}\Delta t+o(\Delta t),\quad j=i.
                                         \end{cases}
\end{equation*}
For future frequent use, we denote by $p(i):=\frac{\kappa_{i}}{\sum_{j=1}^{d}\kappa_{j}}$ the $i$-th component of the stationary distribution of $Y$.
Let
\begin{equation}
    \mathcal{A}:=\{\;\{\xi_{t}\}_{t\geq 0},\;\mathbb{F}\text{-adapted, with bounded-variation,  left-continuous, }\xi_{0}=0,\text{ a.s.}\;\}
\end{equation}
and notice that any $\xi\in\mathcal{A}$ admits the Jordan decomposition $\xi=\xi^{+}-\xi^{-}$, for $\xi^{\pm}$ nondecreasing. Also, let $\{ |\xi|_{t} \}_{t\geq0}:=\xi^{+}+\xi^{-}$ denote the variation of $\xi\in\mathcal{A}$. Then, for given $\xi\in\mathcal{A}$ and Borel-measurable functions $b:\mathbb{R}\times \mathbb{Y}\to \mathbb{R}$, $\sigma:\mathbb{R}\times \mathbb{Y}\to (0,\infty)$, we introduce the process $X^{\xi}$ with state space $\mathcal{I}:=(\underline{x},\overline{x})\subseteq \mathbb{R}$ and dynamics
\begin{equation}
\label{eq:(2.2)}
...
The following assumption in particular ensures that there exists a unique strong solution to (\ref{eq:(2.2)}), for every $\xi\in\mathcal{A}$ and $(x,i)\in\mathcal{I}\times\mathbb{Y}$ (see Theorem 7 Chapter V in \cite{protter2005stochastic}). In the following, we shall denote such a strong solution by $(X^{x,\xi},Y^{i})$, when needed.
...
The following hold:
        \begin{enumerate}
            ...
            \item \label{eq:(2.1-2)} There exists $C>0$,such that
                    \[
                        |b(x,i)|+|\sigma(x,i)|\leq C(1+|x|),\quad \text{for any } (x,i)\in\mathcal{I}\times \mathbb{Y}.
                    \]
            \item \label{eq:(2.1-3)} There exists $c>0$, such that
            \begin{equation*}
                b_{x}(x,i)\leq -c,\quad \text{for any } (x,i)\in\mathcal{I}\times \mathbb{Y}.
            \end{equation*}
...
\newline
For $f:\mathbb{R}\times \mathbb{Y}\to \mathbb{R}$ such that $f(\cdot,i)\in C^{2}(\mathbb{R}),\text{ for any }i\in\mathbb{Y}$, the infinitesimal generator of the  uncontrolled process $(X^{0},Y)$ is denoted by $\mathcal{L}_{(X,Y)}$ and it is such that
\begin{equation}
    \label{eq:(2.4)}
    \big(\mathcal{L}_{(X,Y)}f\big)(x,i)=\frac{1}{2}\sigma^{2}(x,i)f_{xx}(x,i)+b(x,i)f_{x}(x,i)+\sum_{j\neq i}q_{ij}(f(x,j)-f(x,i)),
\end{equation}
while the infinitesimal generator $\mathcal{L}_{(\widehat{X},Y)}$ for the process $(\widehat{X},Y)$ is such that
\begin{equation}
    \label{eq:(2.5)}
    \big(\mathcal{L}_{(\widehat{X},Y)}f\big)(x,i)=\frac{1}{2}\sigma^{2}(x,i)f_{xx}(x,i)+(b(x,i)+\sigma\sigma_{x}(x,i))f_{x}(x,i)+\sum_{j\neq i}q_{ij}(f(x,j)-f(x,i)).
\end{equation}
...
% Finally, we note that from Section 2.6 in \cite{yin2009hybrid} the processes $(X^{0},Y)$ and $(\widehat{X},Y)$ are \textit{strong Feller}.

%\textcolor{orange}{Ioannis: Later we use the strong Markov property of $(X^{\xi},Y)$ (cf. Proposition \ref{Prop 4.1}) and $(\widehat{X},Y)$ (cf. Theorem \ref{eq:Thm 3.1}).}

\subsection{The Ergodic Mean-Field Game}

Within the previous probabilistic setting, we now introduce the ergodic mean-field game (ergodic MFG for short) which will be the main object of our study. For $(x,i)\in\mathcal{I}\times \mathbb{Y}=:\mathcal{O}$, $\xi\in\mathcal{A}$ and $\theta\in\mathbb{R}_{+}$, we introduce the ergodic profit functional 
\begin{equation}
    \label{eq:(2.6)}
...
\end{equation}
where $0<k_{2}<k_{1}$. In (\ref{eq:(2.6)}), $\pi$ is the instantaneous profit function satisfying Assumption \ref{eq:Ass 2.2} below, and $\theta$ is (for the moment) a fixed nonnegative number. We will see later, that $\theta$ drives the mean-field interaction (cf. Definition \ref{eq: Def 1} below). 
\newline
...
\begin{assumption}
    ...
    \begin{enumerate}
        ...
        \begin{multline*}
            \widehat{\mathbb{E}}\bigg[ \int_{0}^{\infty}e^{-ct}\sup_{x'\in (x,x+\epsilon_{0})}\Big(|\pi_{xx}(\widehat{X}^{x'}_{t},\theta)|\partial_{x}\widehat{X}^{x}_{s}\big|_{x=x'}\Big)dt\bigg]+ \\
            +\widehat{\mathbb{E}}\bigg[ \int_{0}^{\infty}e^{-ct}|\pi_{x}(\widehat{X}^{x}_{t},\theta)|\sup_{x'\in(x,x+\epsilon_{0})}\bigg(\int_{0}^{t}|b_{xx}(\widehat{X}^{x'}_{s},Y^{i}_{s})|\partial_{x}\widehat{X}^{x}_{s}\big|_{x=x'}ds\bigg)dt\bigg]<\infty,
        \end{multline*}
        where
        $\partial_{x}\widehat{X}^{x}_{t}:=\exp{\big( \int_{0}^{t}\big(b_{x}+\partial_{x}(\sigma\sigma_{x})-\frac{1}{2}\sigma^{2}_{x}\big)(\widehat{X}_{s}^{x},Y_{s})ds+\int_{0}^{t}\sigma_{x}(\widehat{X}^{x}_{s},Y_{s})d\widehat{W}_{s} \big)}$ and $c>0$ is the same constant as in Assumption \ref{eq:Ass 2.1}-(\ref{eq:(2.1-3)}),
...
We are now ready to introduce the notion of stationary mean-field equilibrium. To that end, we provide the following assumption for the functions $F$ and $f$ that will appear in Definition \ref{eq: Def 1} below. Furthermore, we restrict to those $\xi\in\mathcal{A}$ belonging to
\begin{equation}
    \label{admissible controls} \mathcal{A}_{e}:=\bigg\{\xi\in\mathcal{A}:\mathbb{E}\big[|\xi|_{T}\big]<\infty\text{ for any }T<\infty,\,\limsup_{T\uparrow\infty}\frac{1}{T}\mathbb{E}\big[|X^{\xi}_{T}|\big]=0\bigg\}.
\end{equation}
%The aforementioned set of admissible controls is not empty. As a %matter of fact, $\alpha,\beta \in \mathcal{I}$ with $\alpha<\beta$, %one has that the process
%\[
%   \xi_{t}:=\max\bigg\{\bigg[(x-\beta)^{+}\wedge \inf_{s\in[0,t]}( %X^{0}_{s}-\alpha)\bigg],\bigg[\sup_{s\in [0,t]}(X^{0}_{s}-%\beta)^{+}\wedge \inf_{u\in[s,t]}( X^{0}_{u}-\alpha)\bigg]\bigg\},
%\]
%is such that $\mathbb{E}\big[|\xi_{t}|\big]<\infty$, for all $t\geq %0$, and $\alpha\leq|X^{\xi}_{t}|\leq \beta,\;\mathbb{P}$-a.s..
\begin{assumption}
    \label{eq:Ass 2.3}
...
\begin{definition}[Ergodic MFG Equilibrium]
        \label{eq: Def 1}
        For $(x,i)\in \mathcal{O}$, a couple $(\xi^{*}(\theta^{*}),\theta^{*})\in\mathcal{A}_{e}\times\mathbb{R}_{+}$ is said to be an \textbf{equilibrium of the ergodic MFG} for the initial condition $(x,i)$ if
        \begin{enumerate}
...
\begin{enumerate}
    ...
    \item We then impose the consistency condition (\ref{eq:Def 1-2}) in Definition \ref{eq: Def 1} and we prove the existence and uniqueness of the mean-field parameter $\theta^{*}$ via a fixed-point argument.
    \item We finally show that the ergodic MFG equilibrium realizes an $\epsilon_{N}$-Nash equilibrium for a suitable ergodic $N$-player game of singular control with regime-switching.



%    \item We apply the results of previous steps in a case-study and we investigate how %the equilibrium is affected by the model's parameters.
\end{enumerate}
...
\end{equation}
Notice that the value $\bar{\lambda}$ is independent of $(x,i)\in\mathcal{O}$, since an initial jump does not alter the value of the limit in (\ref{eq:(2.6)}). In order to solve (\ref{eq:(3.1)}), we let $V:\mathcal{O}\times \mathbb{R}_{+}\to \mathbb{R}$ and $\lambda:\mathbb{R}_{+}\times \mathbb{Y}\to \mathbb{R}$ to be determined such that $V(\cdot,i;\theta)\in C^{2}(\mathcal{I}),\text{ for any } (i,\theta)\in\mathbb{Y}\times \mathbb{R}_{+}$ and the pair $(V,\lambda)$ solves the variational inequality
\begin{equation}
\label{eq:(3.2)}
...
\begin{assumption}
    \label{eq:Ass 3.1}
    For $x_{-}(\theta):=(x_{-}(1,\theta),...,x_{-}(d,\theta)),\,x_{+}(\theta):=(x_{+}(1,\theta),...,x_{+}(d,\theta))$, with $x_{-}(i,\theta)<x_{+}(i,\theta),\,i\in\mathbb{Y}$, it holds:
        \begin{equation*}
...
\end{assumption}
Assumption \ref{eq:Ass 3.1} together with Assumption \ref{eq:Ass 2.2}-(\ref{eq:2.2-4}) guarantee that a solution to (\ref{eq:(3.2)}) will be of threshold type, meaning that there shall exist $\alpha(i,\theta)<\beta(i,\theta),\;i\in\mathbb{Y}$, such that $V_{x}(x,i;\theta)<k_{1}$ on $\{(x,i)\in\mathcal{O}:x<\alpha(i,\theta)\}$ and $V_{x}(x,i;\theta)>k_{2}$ on $\{(x,i)\in\mathcal{O}:x>\beta(i,\theta)\}$. 
\subsection{An auxiliary optimal stopping game}
\label{Section 3.1}
To deal with (\ref{eq:(3.2)}), we introduce the auxiliary optimal stopping game (Dynkin game) with stopping functional
\[
...
\end{equation}
where $(\widehat{X}_{t},Y_{t})_{t\geq 0}$ is the unique strong solution to (\ref{eq:(2.3)}) and $\tau,\sigma\in\mathcal{T}$, where
...
By Proposition \ref{eq:Prop 3.3} and arguments based on the semi-harmonic characterization of $v$ (see \cite{doi:10.1137/S0040585X97983821}), we are allowed to conclude that $(v(x,i;\theta),\alpha_{i}(\theta),\beta_{i}(\theta))_{i\in\mathbb{Y}}$ solves the free-boundary problem:
\begin{equation}
    \label{eq:(3.10)}
    \begin{cases}
...
\label{Section 3.2}
In this section we establish the relation between the Dynkin game of Section \ref{Section 3.1} and the ergodic singular control problem (\ref{eq:(3.1)}). 

\begin{proposition}
\label{eq:Prop 3.4}
        Recall $v$ as in (\ref{eq:(3.6)}), the free boundaries $(\alpha_{i}(\theta))_{i\in \mathbb{Y}}$ and $(\beta_{i}(\theta))_{i\in\mathbb{Y}}$ as in (\ref{eq:(3.7)}), and define 
        \begin{equation}
            \label{eq:(3.11)}
...
%            \label{eq:(3.13)}
%            \begin{cases}
%                \mathcal{L}_{(X,Y)}\tilde{U}(x,i;\theta)+\pi(x,\theta)=\tilde{\lambda}%(\theta,i),\quad x\in(\alpha_{i}(\theta),\beta_{i}(\theta)) \\
%                \mathcal{L}_{(X,Y)}\tilde{U}(x,i;\theta)+\pi(x,\theta)\leq \tilde{\lambda}(\theta,i),\quad x\notin (\alpha_{i}(\theta),\beta_{i}(\theta)) \\
%                \tilde{U}_{x}(x,i;\theta)=k_{1},\quad x\in(\underline{x},\alpha_{i}(\theta)] \\
%                \tilde{U}_{x}(x,i;\theta)\leq k_{1},\quad x \in (\alpha_{i}(\theta),\beta_{i}(\theta))  \\
%                \tilde{U}_{x}(x,i;\theta)\geq k_{2},\quad x \in (\alpha_{i}(\theta),\beta_{i}(\theta)) \\
%                \tilde{U}_{x}(x,i;\theta)=k_{2},\quad x\in [\beta_{i}(\theta),\overline{x}),
%            \end{cases}
%        \end{equation}
        where
        \begin{equation}
        ...
        \end{equation}
%        \begin{equation}
%            +\sum_{j\neq i}q_{ij}\bigg( \int_{\alpha_{i}(\theta)}^{\beta_{i}(\theta)}v(y,i;\theta)dy-

%\int_{\alpha_{j}(\theta)}^{\beta_{j}(\theta)}v(y,j;\theta)dy\bigg)
%        \end{equation}
...
%        \begin{equation}
%            \label{eq:(3.15)}
%            \tilde{\lambda}(\theta,i)=\sum_{j\neq i}q_{ij}\bigg(\int_{\beta_{j}%(\theta)}^{\beta_{i}(\theta)}v(y,i;\theta)dy\bigg)+\big( b(\beta_{i}(\theta),i)k_{2}+\pi_{x}(\beta_{i}(\theta),\theta)\big).
%        \end{equation}
\end{proposition}

\begin{proof}
        By Proposition \ref{eq:Prop 3.3} and the definition of $U$, we obtain that $U(\cdot,i;\theta)\in C^{2}(\mathcal{I})$ for $i\in\mathbb{Y}$. Then,
        \[
            \mathcal{L}_{(X,Y)}U(x,i;\theta)+\pi(x,\theta)=\frac{1}{2}\sigma^{2}(x,i)U_{xx}(x,i;\theta)+b(x,i)U_{x}(x,i;\theta)
        \]
        \[
            +\sum_{j\neq i}q_{ij}(U(x,j;\theta)-U(x,i;\theta))+\pi(x,\theta),
        \]
        so that, by (\ref{eq:(3.11)}),
        \[
        ...
        \[
            ...
            b(y,i)+\sigma\sigma_{x}(y,i))v_{x}(y,i;\theta)+b_{x}(y,i)v(y,i;\theta)+\pi_{x}(y,\theta)\bigg)dy
        \]
        
        \[
        	+\big(b(\alpha_{i}(\theta),i)k_{1}+\pi(\alpha_{i}(\theta),\theta) \big).
        \]
        ...
        \[
            \hspace{-8cm} \mathcal{L}_{(X,Y)}U(x,i;\theta)+\pi(x,\theta)\leq 
        \]
%        \[
%            \frac{1}{2}\sigma^{2}(x,i)U_{xx}(x,i;\theta)+\mu(x,i)U_{x}(x,i;
%            \theta)
%        \]
%        \[
%            +\sum_{j\neq i}q_{ij}\bigg(\kappa_{j}+\int_{\alpha_{j}(\theta)}^{x}v(y,j;\theta)dy-\kappa_{i}-\int_%{\alpha_{i}(\theta)}^{x}v(y,i;\theta)dy\bigg)+\pi(x,\theta(i)) 
%        \]
        \[
            \leq -\int_{\alpha_{i}}^{x}\sum_{j\neq i}q_{ij}(v(y,j;\theta)-v(y,i;\theta))dy+\big( b(\alpha_{i}(\theta),i)k_{1}+\pi_{x}(\alpha_{i}(\theta),\theta) \big)
        \]
        \[
        	+\sum_{j\neq i}q_{ij}\bigg(K_{j}(\theta)+\int_{\alpha_{j}(\theta)}^{x}v(y,j;\theta)dy-K_{i}(\theta)-\int_{\alpha_{i}(\theta)}^{x}v(y,i;\theta)dy\bigg)
        \]
        \[
            =\sum_{j\neq i}q_{ij}\bigg( \int_{\alpha_{i}(\theta)}^{x}v(y,j;\theta)dy-\int_{\alpha_{j}(\theta)}^{x}v(y,j;\theta)dy \bigg)+\big( b(\alpha_{i}(\theta),i)k_{1}+\pi(\alpha_{i}(\theta),\theta) \big)
        \]
        \[
            =\sum_{j\neq i}q_{ij}\bigg( \int_{\alpha_{j}(\theta)}^{\alpha_{i}(\theta)}v(y,j;\theta)dy\bigg)+\big( b(\alpha_{i}(\theta),i)k_{1}+\pi(\alpha_{i}(\theta),\theta) \big).
        \]
        Thus, defining
        \begin{equation*}
            \lambda(\theta,i):=\sum_{j\neq i}q_{ij}\bigg( \int_{\alpha_{j}(\theta)}^{\alpha_{i}(\theta)}v(y,j;\theta)dy\bigg)+\big(b(\alpha_{i}(\theta),i)k_{1}+\pi(\alpha_{i}(\theta),\theta) \big)
        ...
        \end{equation}
        \newline
        we see that $(U,\lambda)$ solves the variational inequality (\ref{eq:(3.12)}). %Analogous arguments allow to conclude that $\tilde{U}$ solves (\ref{eq:(3.13)}) %for $\tilde{\lambda}$ defined as follows
        %\[
        %    \tilde{\lambda}(\theta,i)=\sum_{j\neq i}q_{ij} \int_{\beta_{j}%(\theta)}^{\beta_{i}(\theta)}v(y,i;\theta)dy +\big( b(\beta_{i}(\theta),i)k_{2}+%%\pi_{x}(\beta_{i}(\theta),\theta)\big).
        %\]
\end{proof}


%\begin{lemma}
%    One has that $\lambda(i,\theta)=\tilde{\lambda}(i,\theta),\;\forall i\in\mathbb{Y}$.
%\end{lemma}
%\begin{proof}
% See Appendix \ref{Proof of Lemma 3.1}.
%\end{proof}

%%To complete our verification result we need to find the optimal policy $\xi^{*}$ which %%belongs to $\mathcal{A}_{e}$ and attains the supremum. During this part we will verify %%that indeed the value function $\bar{\lambda}(\theta),\; \theta\in\mathbb{R}^{d}_{+}$ %%satisfies $(3.3)$.
 In order to derive the optimal control rule we introduce the following definition.
\begin{definition}[Skorokhod reflection regime-switching problem]
    \label{eq:Def 3.1}
   Let $\mathcal{D}[0,\infty)$ be the space of càdlàg processes on $[0,\infty)$. 
   ...
   $\alpha_{j}<\beta_{j},\text{ for any }j\in\mathbb{Y}$, the process $(X^{\xi},\xi)\in\mathcal{D}[0,\infty)\times \mathcal{A}$ is said to be a solution to the Skorokhod refection problem $\mathbf{SP}(\alpha,\beta,i,x)$ for the noise $(W,Y)$ if it satisfies the following properties:
   \begin{enumerate}
...
\begin{theorem}
    \label{Thm 3.2}
    There exists $\xi^{*}(\theta)\in\mathcal{A}_{e}$ which is the unique solution to $\mathbf{SP}(\alpha(
    \theta),\beta(\theta),x,i)$ with decomposition $\xi^{*}(\theta)=\xi^{*,+}(\theta)-\xi^{*,-}(\theta)$, where
    \begin{gather}
        \xi^{*,+}(\theta):=\sup_{0\leq s\leq t}\bigg( \alpha_{Y_{s}}(\theta)-I(X^{\xi^{*}(\theta)})_{s}+\xi^{*,-}_{s}(\theta) \bigg)^{+},\nonumber \\ 
        \label{eq:(3.20)} \xi^{*,-}(\theta):=\sup_{0\leq s\leq t}\bigg( I(X^{\xi^{*}(\theta)})_{s}+\xi^{*,+}_{s}(\theta)-\beta_{Y_{s}}(\theta) \bigg)^{+}
    \end{gather}
\end{theorem}
\begin{proof}
...
\begin{proposition}
\label{Prop  3.5}
    For every $(x,i)\in \mathcal{O}$, $\xi^{*}(\theta)=\xi^{*,+}(\theta)-\xi^{*,-}(\theta)$  solving $\mathbf{SP}(\alpha( \theta),\beta(\theta),x,i)$ is optimal for (\ref{eq:(3.1)}). Moreover, (\ref{eq:(3.3)}) holds with $\lambda$ as in (\ref{eq:(3.13)}).
\end{proposition}

\begin{proof}
     Let $T>0$ and $(x,i)\in\mathcal{O}$. Recall that $U$ as in (\ref{eq:(3.11)}) is such that $U(\cdot,i)\in C^{2}(\mathcal{I}),\; i\in\mathbb{Y}$, and $(U,\lambda)$ solves (\ref{eq:(3.12)}), with $\lambda$ as in (\ref{eq:(3.13)}). Let $\{\eta_{n}\}_{n\in\mathbb{N}}$ be an increasing sequence of $\mathbb{F}$-stopping times such that $\eta_{n}\uparrow \infty,n\to \infty$. Fixing $\xi\in\mathcal{A}_{e}$ and applying Itô-Meyer's formula (in the sense of \cite{Bjork}) to $(U(X_{T\wedge \eta_{n}},Y_{T\wedge \eta_{n}};\theta))_{T\geq 0}$, we have:
     \begin{equation}
     \label{eq:(3.21)}
     ...
     \end{equation}
     is an $\mathbb{F}$-local martingale, where $\nu(dt,dz):=\wp(dt,dz)-dt\otimes m(dz)$. In particular, $\wp(dt,dz)$ denotes the Poisson random measure with intensity $dt\otimes m(dz)$ for Lebesgue measure $m$, and the function $h:\mathbb{Y}\times \mathbb{R}_{+}$ has the form
     \[
            h(i,z)=\sum_{j=1}^{d}(j-i)\boldsymbol{1}_{\{z\in\Delta_{i,j}\}}, 
     \]
     with $\Delta_{i,j}$ being the partition of the $\mathbb{R}_{+}$ where each interval has length $q_{ij}$ (for further details see Chapter 2 in \cite{yin2009hybrid}). We then define the $\mathbb{F}$-local martingale $\{\tilde{M}_{t}\}_{t\geq0}$ as
     \begin{equation}
         \tilde{M}_{t}:=M_{t}+\int_{0}^{t}U_{x}(X^{\xi}_{s},Y_{s};\theta)\sigma(X^{\xi}_{s},Y_{s})dW_{s}
     \end{equation}
     and notice that $\mathbb{E}_{(x,i)}\big[ \tilde{M}_{T\wedge \eta_{n}} \big]=0,\forall T\geq 0,n\in\mathbb{N}$. Then, taking expectations in (\ref{eq:(3.21)}), we obtain
     \begin{align*}
     ...
     & -\boldsymbol{1}_{\{\Delta \xi^{-}_{s}>0\}}\int_{0}^{\Delta \xi^{-}_{s}}U_{x}(X^{\xi}_{s}-z,Y_{s};\theta)dz,
     \end{align*}
     then (\ref{eq:(3.24)}), together with the fourth and sixth conditions in (\ref{eq:(3.12)}), give
     \begin{align}
        \label{eq:(3.25)}
        \mathbb{E}_{(x,i)}\big[ U(X_{T\wedge \eta_{n}}^{\xi},Y_{T\wedge \eta_{n}};\theta) \big]\leq
        & U(x,i;\theta)+\mathbb{E}_{(x,i)}\bigg[ \int_{0}^{T\wedge \eta_{n}}\big(\lambda(\theta,Y_{s})-\pi(X_{s}^{\xi},\theta)\big)ds \bigg] \\ \notag
    ...
    Hence, using the inequality 
     \[
        \liminf_{n\to\infty}(f_{n})+\limsup_{n\to\infty}(g_{n})\leq \limsup_{n\to\infty}(f_{n}+g_{n}),
     \]
     ...
     \begin{equation}
         ...
         \leq \limsup_{T\uparrow \infty}\frac{1}{T} U(x,i;\theta)+\limsup_{T\uparrow \infty}\frac{1}{T}\mathbb{E}_{(x,i)}\bigg[ \int_{0}^{T}\lambda(\theta,Y_{s})ds\bigg].
     \end{equation}
    Since $U_{x}\in [k_{2},k_{1}]$ by (\ref{eq:(3.12)}), we have that exists $C>0$ such that for every $(x,i,\theta)\in\mathcal{O}\times \mathbb{R}_{+}$ one has
    \begin{equation}
        \label{eq:(3.30)}
...
%%    We now show that
%%    \[
%        \limsup_{T\uparrow \infty}\frac{1}{T} \mathbb{E}_{(x,i)}\big[|X^{\xi}_{T}|\big]=0
%    \]
%    which implies through (3.27) that
%    \begin{equation}
%        \liminf_{T\uparrow \infty}\frac{1}{T} \mathbb{E}_{(x,i)}\big[ U(X_{T}^{\xi},Y_{T};\theta) %\big]\geq 0
%    \end{equation}
%    We prove that by contradiction, and assume that there exists $\epsilon>0$ such that
%    \[
%        \liminf_{T\uparrow \infty}\frac{1}{T} \mathbb{E}_{(x,i)}\big[|X^{\xi}_{T}|\big]>\epsilon.
%    \]
%    Then, there exists $T_{1}>0$, such that, for every $s\geq T_{1}$, we have
%    \[
%        \mathbb{E}_{(x,i)}\big[|X^{\xi}_{s}|\big]>\frac{\epsilon s}{2},
%    \]
%    which gives
%    \[
%        \limsup_{T\uparrow \infty}\frac{1}{T} %\mathbb{E}_{(x,i)}\bigg[\int_{0}^{T}|X^{\xi}_{t}|dt\bigg]>\limsup_{T\uparrow \infty}\frac{1}{T} %\mathbb{E}_{(x,i)}\bigg[\int_{T_{1}}^{T}\frac{\epsilon s}{2}ds\bigg]=\infty,
%    \]
%   thus contradicting the fact that $\xi\in\mathcal{A}_{e}$ (cf. (2.10)). 
   Hence, using (\ref{eq:(3.31)}) and (\ref{eq:(3.32)}) in (\ref{eq:(3.29)}), we obtain
    ...
    \begin{equation}
        \label{eq:(3.34)}
        \overline{\lambda}(\theta)\leq \sum_{i\in\mathbb{Y}}\lambda(\theta,i)p(i).
    \end{equation}
    ...
    \[
        \limsup_{T\uparrow\infty}\frac{1}{T}\mathbb{E}_{(x,i)}\big[ U(X_{T}^{\xi^{*}(\theta)},Y_{T};\theta) \big]+\limsup_{T\uparrow\infty}\frac{1}{T}\mathbb{E}_{(x,i)}\bigg[ \int_{0}^{T}\pi(X_{s}^{\xi^{*}(\theta)},\theta)ds-k_{1}\xi^{*,+}_{T}(\theta)+k_{2}\xi^{*,-}_{T}(\theta) \bigg]
    \]
    \begin{equation}
        \label{eq:(3.35)}
    ...
    \begin{equation}
        \limsup_{T\uparrow\infty}\frac{1}{T}\mathbb{E}_{(x,i)}\bigg[ \int_{0}^{T}\pi(X_{s}^{\xi^{*}(\theta)},\theta)ds-k_{1}\xi^{*,+}_{T}(\theta)+k_{2}\xi^{*,-}_{T}(\theta) \bigg]\geq \limsup_{T\uparrow \infty}\frac{1}{T}\mathbb{E}_{(x,i)}\bigg[ \int_{0}^{T}\lambda(\theta,Y_{s})ds\bigg].
    \end{equation}
    ...
    By (\ref{eq:(3.30)}),
    \begin{equation*}
        |U(X^{\xi}_{T\wedge \eta_{n}},Y_{T\wedge \eta_{n}};\theta)|\leq C(1+|X^{\xi}_{T\wedge\eta_{n}}|)\leq C(1+\sup_{t\leq T}|X^{\xi}_{t}|).
    \end{equation*}
...
\begin{assumption}
    ...
    The following hold.
    \begin{enumerate}
        \item \label{Ass 4.1-1} There exists $\kappa:\mathbb{R}_{+}\to\mathbb{R}_{+}$ such that
        \[
...
\begin{lemma}
    ...
    \begin{enumerate}
        ...
        \item \label{Strictly neg} The function $v_{x}(x,i;\theta)$ is nonpositive for any $(x,i,\theta)\in\mathcal{O}\times\mathbb{R}_{+}$ and for fixed $\theta\in\mathbb{R}_{+}$ is strictly negative for any $(x,i)\in\mathcal{C}^{\theta}$, where $\mathcal{C}^{\theta}$ is the continuation region as in Theorem \ref{eq:Thm 3.1}.
    \end{enumerate}
...
First of all, we show that a stationary distribution for the process $(X^{\xi^{*}(\theta)}_{t},Y_{t})_{t\geq 0}$ exists.

\begin{proposition}
\label{Prop 4.1}
     For any $\theta\in\mathbb{R}_{+}$, there exists a unique stationary distribution $\nu^{\theta}\in\mathcal{P}(\mathcal{O})$ for the hybrid-reflected process $(X_{t}^{\xi^{*}(\theta)},Y_{t})_{t\geq 0}$.
\end{proposition}
...
If $Y_{0}=i$ by Proposition \ref{Prop B.2}  we have that $X^{x,\xi^{*}(\theta)}_{t}\leq X^{\beta_{i}(\theta),\xi^{*}(\theta)}_{t},\;\mathbb{P}_{i}\text{-a.s.}$.
Thus, we have
\begin{equation}
...
\end{equation}
for $\tilde \eta:=\inf\{t>0:Z_{t}= \alpha_{i}(\theta) \}$, by a comparison principle (a slightly different version of Proposition \ref{Prop B.2}) we also have $ X^{\beta_{i}(\theta),\xi^{*}(\theta)}_{t} \leq Z_t,\text{ for any }t\in [0,\tau\wedge \tilde{\eta}),\;\mathbb{P}_{i}\text{-a.s.}$,
...
\begin{align}
    ...
    $$
        \sup_{(x,i)\in\mathcal{O}}\mathbb{E}_{(x,i)}\big[\eta(\alpha_{i}(\theta))\big]\leq \mathbb{E}_{i}\big[ \tau\big]+(1-\rho)\sup_{(x,i)\in\mathcal{O}}\mathbb{E}_{(x,i)}\big[\eta(\alpha_{i}(\theta))\big].
    $$
    Equivalently, we write
    \begin{equation}
        \label{finite stopping time}
        \sup_{(x,i)\in\mathcal{O}}\mathbb{E}_{(x,i)}\big[\eta(\alpha_{i}(\theta))\big]\leq \frac{1}{\rho}\mathbb{E}_{i}\big[ \tau\big]<\infty,
    \end{equation}
...
Next, we characterize the stationary distribution and we study its stability with respect to changes of the boundary. 
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        For any $i\in\mathbb{Y}$, the cumulative distribution function $\mu^{\theta}(x,i):=\nu^{\theta}((\underline{x},x],i),$ $x \in \mathcal{I}$ has regularity $\mu^{\theta}(\cdot,i)\in C^{1}([\alpha_{i}(\theta),\beta_{i}(\theta)])\cap C^{2}((\alpha_{i}(\theta),\beta_{i}(\theta))\setminus \bigcup_{j\in\mathbb{Y}}\{\alpha_{j}(\theta),\beta_{j}(\theta)\})$ and it is the unique nondecreasing solution of the equation
    \begin{equation}
    \label{eq:(4.1)}
        \frac{1}{2}\sigma^{2}(x,i)\,\mu^{\theta}_{xx}(x,i)-(b(x,i)-\sigma\sigma_{x}(x,i))\mu^{\theta}_{x}(x,i)+\sum_{j\in\mathbb{Y}}q_{ji}\mu^{\theta}(x,j)=0,\  x\in (\alpha_{i}(\theta),\beta_{i}(\theta)),
    \end{equation}
...
\begin{proof}
The proof of the first item follows closely the proof of Theorem 1 in \cite{DAURIA20121566}, and it is proved in Appenidx \ref{Appendix C}. 
Therefore, we only provide a proof of Claim \ref{Continuity of a->mu}.
...
Since $\mu$ and $\mu^{n}$ are solutions to the linear system (\ref{eq:(4.1)}), for each $n\in\mathbb{N}$, $v^{n}$ is a solution to the boundary value problem
     \begin{equation}
     \label{n-BVP}
     \begin{cases}
        \mathcal L^{*} _{(\widehat X,i)}v^n (x,i) +\sum_{j\in\mathbb{Y}}q_{ji}v^{n}(x,j)=0,\; x\in(\alpha_{i}^{n},\beta^{n}_{i}), \ i \in \mathbb Y, \\
         v^{n}(\alpha^{n}_{i},i)=\mu(\alpha^{n}_{i},i),\quad v^{n}(\beta^{n}_{i},i)=\mu(\beta^{n}_{i},i)-p(i),
     \end{cases}
     \end{equation}
     where $\mathcal L^{*} _{(\widehat X,i)}v^n (x,i):= \frac{1}{2}\sigma^{2}(x,i)\,v^{n}_{xx}(x,i)-(b(x,i)-\sigma\sigma_{x}(x,i))v^{n}_{x}(x,i)$.
    Now, thanks to Assumption \ref{eq:Ass 2.1} and Assumption \ref{Ass 4.1}-(\ref{Ass 4.1-3}), we can apply Theorem 1 in \cite{maxprinc} to (\ref{n-BVP}) which gives us
     \begin{equation*}
         \sup_{x\in(\alpha^{n}_{i},\beta^{n}_{i})}|v^{n}(x,i)|\leq C\max\big\{ |\mu(\alpha^{n}_{i})|,|\mu(\beta^{n}_{i},i)-p(i)| \big\}.
     \end{equation*}
     Therefore, for any $x\in(\alpha_{i},\beta_{i})$, we deduce that $\mu^n (x,i) \to \mu(x,i)$ as $n \to \infty$. 
  Hence, by an application of Theorem 5.25 in \cite{kallenberg2002foundations} we obtain that $\nu^{n}\rightharpoonup \nu^{\theta}$ as $n\to\infty$ thus completing the proof of Claim \ref{Continuity of a->mu}.
\end{proof}
...
\begin{proof}
    ...
    \end{equation}
    where $\underline{\nu}$ is the unique stationary distribution of the process $(\underline{X}_{t},Y_{t})_{t\geq 0}$ and  $\underline{X}$ is the solution to the $\mathbf{SP}((\underline{\alpha}_{i},\underline{\beta}_{i})_{i\in\mathbb{Y}}; \underline X _0, Y_0)$. Then, by Proposition \ref{Prop B.2}, since $f$ is increasing, we obtain
    \begin{equation}
        \mathbb{E}\big[ f(\underline{X}_{t})\big]\leq\mathbb{E}\big[ f(X^{\xi^{*}(\theta)}_{t})\big],\;\text{ for any }t\geq 0.
    \end{equation}
    Hence, by Proposition \ref{Prop 4.1}, we can employ the ergodic theorem (see pg.274 in \cite{Sigman}), in order to deduce that 
    $$
    \langle f, \underline{\nu} \rangle = 
   ...
   \langle f, {\nu^\theta} \rangle, 
    ...
    \end{equation}
    To find an upper bound for $\mathcal{T}\theta$, we proceed as follows. Since, $X^{\xi^{*}(\theta)}_{t}\in [\alpha_{Y_{t}}(\theta),\beta_{Y_{t}}(\theta)],\text{ for any }t\geq 0,\;\mathbb{P}$-a.s. for any $\theta\in\mathbb{R}_{+}$, thanks to the monotonicity of $F$ and $f$ (see Condition (\ref{eq:Ass 2.3-1}) Assumption \ref{eq:Ass 2.3}), we have
    \begin{equation*}
    ...
    In the last inequality above we have used the fact that the map $\theta \mapsto (\beta_{i}(\theta))_{i\in\mathbb{Y}}$ is nonincreasing (cf. Lemma \ref{Lemma 4.1}).
    Hence, we define $\overline{\theta}:=F\big(\sum_{i=1}^{d}p(i)f(\beta_{i}(\underline{\theta}))\big)$ and for any $\underline{\theta}\leq \theta$ we find that
    \begin{equation}
        \label{eq:(4.10)}
        \mathcal{T}\theta\leq \overline{\theta}.
    \end{equation}
 ...
 \end{equation}
   ...
   Next, we denote by $\mathcal{T}_{2}:\mathcal{P}(\widehat K \times \mathbb Y) \to K$ the map 
    \begin{equation*}
        \mathcal{T}_{2}(\nu):=F\bigg( \sum_{i=1}^{d}\int_{\mathcal{I}}f(x)\nu(dx,i)\bigg).
    \end{equation*}
...
Concluding, the map $\mathcal{T}:=\mathcal{T}_{2}\circ\mathcal{T}_{1}:K \to K$ is continuous in the convex compact set $K$ and, by Schauder-Tychonof fixed point theorem ( Corollary 17.56 in \cite{aliprantis1999infinite}), there exist $\theta^{*}\in K$, such that $\mathcal{T}\theta^{*}=\theta^{*}$.
    ...
    \begin{equation*}
    	\alpha_{i}(\theta^{*})\geq \alpha_{i}(\theta'),\quad \beta_{i}(\theta^{*})\geq \beta_{i}(\theta'),\;\forall i\in\mathbb{Y},
    \end{equation*}
...
\label{Approximation}
%In the previous subsection we established the solution to the ergodic MFG with regime switching. Our natural question is, how could we use this limiting model in order to approximate the Nash equilibria of N-player game?
...
%In this subsection we provide an approximation result in the spirit of \cite{https://doi.org/10.48550/arxiv.2105.07213} and \cite{CAO2022995}.We first introduce the $N$-player game with idiosyncratic noises. 
...
In particular, for $\widehat K$ being the compact set as in \eqref{remark compact set}, defining the set of restricted barrier strategies
\begin{equation}
    \label{eq restricted strategies}
    \widehat {\mathcal A}_b := \{ \text{$(\alpha^{n},\beta^{n})$-barrier-type strategy with $(\alpha^n_j,\beta^n_j)_{j\in\mathbb{Y}} \in \widehat{K}^{2d}$}\},
\end{equation}
 we give the following definition of $\epsilon$-Nash equilibrium. 
...
%(cf.\ Theorem 1.2.2 and Exercise 1.2.2 in \cite{Pilipenko2014}), 
...
\begin{proposition}
    ...
    For any $N\geq 1$ and $\{\xi^{n}\}_{n\leq N}\in\widehat{\mathcal{A}}_{b}^{N}$, the ergodic limit\begin{equation*}
                    ...
                    \nu^{\ell}(dx_{\ell},i_{\ell}),\quad \mathbb{P}\text{-a.s.}
        \end{equation*}
        holds for any bounded function $G:\mathcal{O}^{N}\to\mathbb{R}$.        
\end{proposition}
...
the stationary distribution for $(X^{1,\xi^{1}},Y^{1},...,X^{N,\xi^{N}},Y^{N})$, denoted by $\bar{\nu}^{N}\in\mathcal{P}(\mathcal{O}^{N})$, exists and it is given by
    ...
    \end{equation*}
Clearly, this distribution is unique. 
Thus, by Theorems 3.2.6 and 3.3.1 in \cite{daprato_zabczyk_1996}, the ergodic limit holds.
\end{proof}
\subsection{Approximation result}

We enforce the following condition.
\begin{assumption}
	\label{Ass 4.2}
...
In the spirit of \cite{CaoFerrariDianetti} and \cite{CAO2022995}, we can now state and prove the main result of this section. 
\begin{theorem}
...
\beta_{i})_{i\in\mathbb{Y}}\in \widehat{K}^{2d}$, with $\widehat{K}$ as in (\ref{remark compact set}).
 ...
 By uniqueness in law of the solution to the Skorokhod problem, the processes $(X^{n,\xi^{n}},\xi^n)$ and $(\tilde X ^{n,\xi^{n}}, \tilde \xi ^n)$ have the same law, hence
 \begin{align}
...
%        \]
    ...
    \begin{proposition}
        ...
        \begin{enumerate}
            \item $\bigg( \int_{0}^{t\wedge \tau^{*}(\theta)}\pi_{x}(\widehat{X}_{t},\theta)dt+e^{\int_{0}^{t\wedge \tau^{*}(\theta)}b_{x}(\widehat{X}_{t},Y_{t})dt}v(\widehat{X}_{t\wedge \tau^{*}(\theta)},Y_{t\wedge \tau^{*}(\theta)}) \bigg)_{t\geq 0}$ is an $\mathbb{F}$-submartingale;
            \item $\bigg( \int_{0}^{t\wedge \sigma^{*}(\theta)}\pi_{x}(\widehat{X}_{t},\theta)dt+e^{\int_{0}^{t\wedge \sigma^{*}(\theta)}b_{x}(\widehat{X}_{t},Y_{t})dt}v(\widehat{X}_{t\wedge \sigma^{*}(\theta)},Y_{t\wedge \sigma^{*}(\theta)}) \bigg)_{t\geq 0}$ is an $\mathbb{F}$-supermartingale;
            \item \label{Prop A.1-3} $\bigg( \int_{0}^{t\wedge \tau^{*}(\theta)\wedge \sigma^{*}(\theta)}\pi_{x}(\widehat{X}_{t},\theta)dt+e^{\int_{0}^{t\wedge \tau^{*}(\theta)\wedge\sigma^{*}(\theta)}b_{x}(\widehat{X}_{t},Y_{t})dt}v(\widehat{X}_{t\wedge \tau^{*}(\theta)\wedge \sigma^{*}(\theta)},Y_{t\wedge \tau^{*}(\theta)\wedge \sigma^{*}(\theta)}) \bigg)_{t\geq 0}$ is an $\mathbb{F}$-martingale.
        \end{enumerate}
    \end{proposition}
