...
\newcommand{\meanEMF}{\overline{\mbox{
\boldmath ${\cal E}$}}{}}{}
\newcommand{\meanmu}{\overline{\mu}}
...
\title{Chiral magnetic anomaly and dynamos from 
spatial chemical potential fluctuations}
...
\begin{abstract}
Using direct numerical simulations, we show that 
a chiral magnetic anomaly can be produced just from 
initial spatially inhomogeneous fluctuations of the chemical potential, provided there is a 
small mean magnetic flux through the domain.
The produced chiral asymmetry in the number densities
of left- and right-handed fermions causes a chiral magnetic effect, the excitation of 
a chiral dynamo instability, the production of 
magnetically driven turbulence, and the generation of a large-scale magnetic field via the 
magnetic $\alpha$ effect from fluctuations of current helicity.
\end{abstract}
...
``chiral magnetic effect'' (CME) \citep{Vilenkin:80a} in plasmas with chiral, electrically charged particles.
The CME has been derived using different approaches
\citep{RW85,Tsokos:85,AlekseevEtAl1998,Frohlich:2000en,Frohlich:2002fg,Fukushima:08,Son:2009tf}.
...
It can be incorporated in the framework of 
magnetohydrodynamics 
\citep{AlekseevEtAl1998,GI13,RogachevskiiEtAl2017,HattoriEtAl2019}, 
and leads to chiral dynamo instabilities
\citep{JS97,BFR12,BFR15,GKR15,KH14,K16,Schober2017,SBR19,SBR20},
which can amplify magnetic energy by many orders of magnitude. 
...
neutron stars \citep{Dvornikov:2015lea,YA16,Sigl:2015xva}, quark-gluon
...
\citep{KH14,K16}.
We present a series of DNS, in which 
sufficiently large values of $\mu_5$ are generated
such that the CME causes a chiral dynamo instability
within the numerical domain, 
which, in turn, leads to magnetically driven
...
To study the chiral magnetic anomaly and dynamos from
spatial chemical potential fluctuations,
we solve the following set of
equations for an effective description of a plasma 
composed of chiral electrically charged fermions 
with CME and CSE:
\begin{eqnarray}
...
\delta_{ij} ({\bm \nabla}{\bm \cdot} \UU)/3$,
and $\lambda$ is the chiral feedback parameter.
For numerical stability, the evolution equations for 
$\mu_5$ and $\mu$ also include (hyper-)diffusion terms 
with the diffusion coefficients $\mathcal{D}_5$ and
$\mathcal{D}_\mu$, respectively \citep{SchoberEtAl2022b}. 
The chiral separation effect (CSE) is described by 
the second terms on the RHS of
Eqs.~(\ref{mu-DNS_CSE})--(\ref{mu5-DNS_CSE}).
The coupling between $\mu_5$ and $\mu$, determined by the value 
of the constants $C_5$ and $C_\mu$, leads to chiral magnetic waves (CMW) with the frequency
$\omega_{\rm CMW} \approx  \pm (C_5 \,C_\mu)^{1/2}
k_\mu |\BB_\mathrm{ex}|$ \citep{KY11}, 
where $k_\mu$ is the inverse length scale 
over which $\mu$ changes along a large-scale magnetic field $\BB_\mathrm{ex}$.

For our numerical analysis, we use the \textsc{Pencil Code} \cite{PencilCode2021} to
solve Eqs.~(\ref{ind-DNS_CSE})--(\ref{mu5-DNS_CSE})
in a three-dimensional periodic domain of size 
$L^3$ with a resolution of up to $720^3$ mesh points.
...
The smallest wave number in the domain 
is $k_1 = 2\pi/L$.
We use an isothermal equation of state with $p=\rho\cs^2$, where $\cs$ is the sound speed.
The density is initially uniform and, because of mass conservation, its value is always equal to the mean density $\meanrho$, where the angle brackets denote a full volume average.
...
at the Nyquist wave number.
In the following analysis, time is either normalized by the diffusion time 
$t_\eta = (\eta k_1^2)^{-1}$ or by the 
period of the CMW, $t_\mathrm{CMW} = 2 \pi / \omega_\mathrm{CMW}$.
...
\hline
\hline
Res.            &   $672^3$             &  $672^3$            &  $672^3$            & $720^3$   \\
\hline
$E_\mu(k,t_0)$  &   ---                 & $\propto k^{-4}$    & $\propto k^{-4}$    & $\propto k^{-4}$  \\
$E_5(k,t_0)$    &   $\propto k^{-2}$    & $0$                 & $0$                 & $0$       \\
$\mu_{\mathrm{max}}(t_0)$ &  ---        &  $45$               & $44$                & $48$  \\
\hline
$B_\mathrm{ex}$            &  $0$                  &  $10^{-4}$          & $10^{-4}$           & $10^{-4}$  \\
$C_5 = C_\mu$    &  $0$                  &  $1$                & $10$                & $10$\\
$\eta$          & $2 \times 10^{-4}$    & $10^{-4}$           & $10^{-4}$           & $2 \times 10^{-4}$ \\
$\lambda$       & $4 \times 10^{2}$     & $4 \times 10^{2}$   & $4 \times 10^{4}$   & $4 \times 10^{4}$ \\
\hline
$\mathrm{max}(\mu_{\mathrm{5,max}})$ &  $50$   & $59$         & $38$   &  $47$\\
$\mathrm{max}(B_\mathrm{rms})$   &  $0.18$  & $0.041$         & $0.024$ & $0.013$\\
$\mathrm{max}(\Rm)$  &  $140$  &  $58$          & $36$     & $5.5$\\
$\mathrm{max}(\mathrm{Lu})$  &  $460$  &  $200$  &  $98$ & $21$\\
\hline
\end{tabular}
...
In this Letter, we perform a detailed comparison 
with the results obtained in our previous study 
\cite{SchoberEtAl2022a} (see Run~$R$--$2$),
where chiral dynamo instabilities were found
for an initial $\mu_5$ with zero mean but spatial fluctuations.
For this comparison, the spatial maximum value of the 
chemical potential at the initial time $t_0$,
$\mu_{\rm max} (t_0)$, and its spectrum $E_\mu$ have been chosen to 
obtain the same initial conditions in the run $R$--$2$ discussed in Ref.~\cite{SchoberEtAl2022a}.
In particular, $\mu_{5,\mathrm{max}}(t_0)$ in $R$--$2$ 
was $\approx50$ and the initial spectrum was 
$E_5(k,t_0)\propto k^{-2}$. 
$E_5$ and $E_\mu$ are normalized such that
$\int E_5(k)\,\mathrm{d}k=\bra{\mu_5^2}$ and 
$\int E_\mu(k)\,\mathrm{d}k=\bra{\mu^2}$.
To allow for chiral magnetic waves in the new simulations, we apply an
external magnetic field $\BB_\mathrm{ex}=(B_\mathrm{ex}, 0, 0)$, 
which effectively produces the chiral magnetic anomaly.
A summary of all runs presented in this Letter
is given in Table~\ref{tab_DNSoverview},
where ${\rm Re}_{\rm M}=u_{\rm rms}/(k_{\rm int}\eta)$ is the magnetic Reynolds number, ${\rm Lu}=u_{\rm A, rms}/(k_{\rm int}\eta)$ is the Lundquist number, 
...
Whether in such setups, enough $\mu_5$ can be produced to trigger 
...
The different stages of chiral asymmetry production and magnetic 
field amplification 
are shown in Fig.~\ref{fig_sketch}.
In Phase~1, a chiral asymmetry 
is generated via the second term on the RHS in Eq.~(\ref{mu5-DNS_CSE}).
For times less than the period of the CMW, i.e., 
$t \ll t_\mathrm{CMW}$,
the evolution of $\mu_5$ is described as
\begin{eqnarray}
  \mu_5(t) \approx C_5 ({\BB} {\bm \cdot} \nab) \mu(t) \, t
          \approx C_5 B_{\mathrm{ex}} k_{\mu,\mathrm{eff}}(t_0)
 \mu(t) \, t,
\label{eq_mu5_init_gen}
\end{eqnarray}
where $k_{\mu,\mathrm{eff}}(t_0)$ is the characteristic wave number of the initial $\mu$.
Note that, even for $t \ll t_\mathrm{CMW}$, 
$\mu$ can be a function of time due to the dissipation 
term in Eq.~(\ref{mu-DNS_CSE}).
Regeneration of $\mu$ occurs due to the term 
$C_\mu ({\BB} {\bm \cdot} \nab) \mu_5$ in Eq.~(\ref{mu-DNS_CSE}).
If the initial chemical potential has a spectrum
$E_\mu \propto k^{\beta}$,
we have $\mu^2 \approx E_\mu(k_{\mu,\mathrm{eff}}) k_{\mu,\mathrm{eff}} = k_{\mu,\mathrm{eff}}^{1+\beta}$,
so that $\mu(k) \approx  k^{(1+\beta)/2}$.
Inserting $\mu(k)$ in Eq.~(\ref{eq_mu5_init_gen}), yields $\mu_5(k,t) \approx  C_5 B_\mathrm{ex} k^{(3+\beta)/2} ~ t $.
The produced spectrum of $\mu_5$ is then
\begin{eqnarray}
   E_5(k) = 
   \frac{\mu_5^2(k)}{k}
   \approx  C_5^2 B_\mathrm{ex}^2 k^{2+\beta} ~t^2 .
\label{eq_E_5k}
\end{eqnarray}
The time evolution determined by Eq.~(\ref{eq_mu5_init_gen}) 
is seen in the initial times of Runs $A$, $B1$, and $B2$ (see Fig.~\ref{fig_ts_m2_turb}a).
Moreover, the obtained spectrum $E_5$ in 
Phase~1 of Run $B1$ is $\propto k^{-2}$ 
[see Fig.~\ref{fig_spec_gamma}b], as expected from 
Eq.~(\ref{eq_E_5k}), where the initial $\mu$ spectrum 
has $\beta =-4$ [see Fig.~\ref{fig_spec_gamma}a].

Phase~2 starts when $\mu_5$ exceeds a critical value
for the excitation of the chiral dynamo instability with the growth rate
[see also Fig.~\ref{fig_ts_m2_turb}b]
\begin{eqnarray}
...
\citep{SchoberEtAl2022a,SchoberEtAl2022b}. 
The maximum possible growth rate is estimated as 
...
\begin{eqnarray}
  k_{\mu_5,\mathrm{eff}}^{-1}(t) = \bra{\mu_5^2}^{-1} \int  k^{-1} E_5(k)\,\mathrm{d}k.
\label{eq_kmu5eff}
\end{eqnarray}
The latter condition applies only to systems
with initial spatial fluctuations of $\mu$ or $\mu_5$
\citep{SchoberEtAl2022a,SchoberEtAl2022b} and vanishing mean $\mu_5$.
In Fig.~\ref{fig_ts_m2_turb}d, the ratio $k_5/k_{\mu_5,\mathrm{eff}}$ 
is shown for all four runs.
We find that $k_5/k_{\mu_5,\mathrm{eff}}\approx 10$ is needed for the
excitation of the chiral dynamo instability, as can be seen when comparing
Figs.~\ref{fig_ts_m2_turb}c and~\ref{fig_ts_m2_turb}d.
In Fig.~\ref{fig_spec_gamma}d, we show the measured growth rate of the scale-dependent magnetic field strength and compare it with the
theoretically expected growth rate given by Eq.~(\ref{eq_gammak})
...
according to Eq.~(\ref{eq_mu5_init_gen}), and the field
$B_{\mathrm{ex}}$ is being replaced by $B_{\mathrm{ex}}+b(t)$, 
once $b(t)\gtrsim B_{\mathrm{ex}}$, where $b$ are 
magnetic fluctuations.
Therefore, the $\mu_5$ production phase becomes nonlinear. 
For Run A, the transition to the nonlinear $\mu_5$ production phase
occurs at $t\approx 0.22\,t_\eta$; see Fig.~\ref{fig_ts_m2_turb}a.

% Figure environment removed

Eventually, the dynamo instability reaches its nonlinear stage
since the Lorentz force [the first term on the RHS of 
Eq.~(\ref{UU-DNS_CSE})] increases with time due to the 
nonlinear evolution of the magnetic field and $\Rm$ becomes 
larger than unity [see the growth of the
rms velocity in Fig.~\ref{fig_ts_m2_turb}b].
Then Phase~3 
begins and a mean-field dynamo instability is excited with 
the growth rate $\gamma_\alpha=(\eta \overline{\mu}_{5} 
+ \alpha_\mathrm{M}) k - (\eta+\eta_\mathrm{T}) k^2$.
It attains the maximum value
\begin{eqnarray}
  \gamma_\alpha^{\rm max}= \frac{(\eta 
  \overline{\mu}_{5} 
        + \alpha_\mathrm{M})^2}{4(\eta+\eta_\mathrm{T})},
\end{eqnarray}
where $\alpha_\mathrm{M} = 2(q-1)/(q+1) \, \tau_{\rm c} \, \chi_{\rm c}$
is the magnetic $\alpha$ effect, which is determined by the current helicity
$\chi_{\rm c} = \overline{{\bm b} {\bm \cdot} ({\bm \nabla} \times{\bm b})} \approx \overline{ {\bm a}\cdot {\bm b}} \, k_\mathrm{int}^2$.
In the steady-state of the evolution of the current helicity,
the magnetic $\alpha$ effect reaches the value
$\alpha_\mathrm{M}^{\rm sat} = \eta \, \meanmu_{5} \, {\overline{{\bm b}^2} / \meanBB^2}$
\citep{SchoberEtAl2022a,SchoberEtAl2022b},
where $\tau_{\rm c} \approx (v_\mathrm{A} k_\mathrm{int})^{-1}$ 
is the correlation time of 
the magnetically driven turbulence,
and $q$ is the exponent of the magnetic energy spectrum $E_{\rm M}\propto k^{-q}$.
As follows from Fig.~\ref{fig_spec_gamma}c, 
the exponent is $q\approx3$.
We present a mean-field analysis for Runs A and B1
in Fig.~\ref{fig_ts_m2_meanfield}. 
For Run B2, a mean-field dynamo stage cannot be observed.
This is probably due to a large value of the product $\eta \lambda$ that
leads to saturation of the magnetic field at a low value and therefore
to a low maximum value of the magnetic Reynolds number of $\Rm \approx 6$.
The mean quantities used in Fig.~\ref{fig_ts_m2_meanfield} are calculated in the simulations as
$\langle B \rangle_{\mathrm{int}} \equiv ({\cal E}_{\rm M}^{-1}\int E_\mathrm{M}(k)^{2}\,\mathrm{d}k)^{1/2}$,
where $v_{5,\mathrm{int}}\equiv \eta \langle \mu_5 \rangle_{\mathrm{int}}$
with $\langle \mu_5 \rangle_{\mathrm{int}} \equiv ({\cal E}_{\rm M}^{-1}\int E_5(k) E_\mathrm{M}(k) \,\mathrm{d}k)^{1/2}$.
In the initial turbulent phase, the dynamo generation term
$\alpha_\mathrm{M} k$ is larger than the turbulent diffusion term
$\eta_\mathrm{T} k^2$, but $\eta_\mathrm{T}$ is increasing constantly, leading finally to
the end of the mean-field dynamo instability.
Here we take into account that $\eta \langle \mu_5 \rangle_{\mathrm{int}} \ll \alpha_\mathrm{M}$
and $\eta \ll \eta_\mathrm{T}$. 
...
This study has been initiated several years ago through very productive discussions with Dmitry Kharzeev.
...
\bibitem [{\citenamefont {{Schober}}\ \emph {et~al.}(2023)\citenamefont
  ...
  {{Schober}}}, \bibinfo {author} {\bibfnamefont {I.}~\bibnamefont
  {{Rogachevskii}}},\ and\ \bibinfo {author} {\bibfnamefont {A.}~\bibnamefont
  {{Brandenburg}}},\ }\bibfield  {title} {\bibinfo {title} {{Conditions for
  magnetic field amplification in plasma with autonomous generation of chiral
  asymmetry}},\ }\href@noop {} {\bibfield  {journal} {\bibinfo  {journal}
  {unpublished}\ } (\bibinfo {year} {2023})}\BibitemShut {NoStop}%
