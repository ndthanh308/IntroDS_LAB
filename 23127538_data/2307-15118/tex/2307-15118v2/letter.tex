...
\title{\blue{Chiral anomaly and dynamos from 
...
\blue{from the current helicity of the small-scale field.}
\end{abstract}
...
\blue{Producing chiral asymmetry from an initially vanishing 
...
\blue{\citep{KH14,K16}, resulting in a production of 
...
\blue{To study these effects, we consider
...
\blue{$\lambda=3 \hbar c (8 \alphaem / \kB T)^2$
...
\blue{artificial (hyper-) diffusion terms with 
...
\blue{with a resolution of up to $N^3=672^3$ mesh points.}
...
\blue{up to the Nyquist wave number $k_\mathrm{Ny} = N k_1/2$}.
...
$\int E_\mu(k)\,\mathrm{d}k=\blue{\bra{\mu^2}}$.
...
we apply 
\blue{
a small external magnetic field $\BB_\mathrm{ex}=(B_\mathrm{ex}, 0, 0)$,} which effectively produces 
the chiral anomaly.
\blue{
...
\blue{Introducing the spectrum of the chiral chemical 
...
\blue{with the rough estimate for the
...
magnetic field. 
\blue{The Lorentz force produces velocity fluctuations
...
\label{Eq10a}
\end{eqnarray}
\blue{Here, $\alpha_\mathrm{M} = 2(q-1)/(q+1) \, 
\tau_{\rm c} \, \chi_{\rm c}$ 
...
\blue{Since $\alpha_\mathrm{M}$ and $\eta_\mathrm{T}$ change in time in the simulations, Eqs.~(\ref{Eq10a}) and~(\ref{Eq10b}) serve only as very
...
\blue{The dynamo instability can,
...
\blue{This scenario can be relevant for the early Universe if chirality is not produced by other mechanisms. 
