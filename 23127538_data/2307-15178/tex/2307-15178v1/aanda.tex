...
\newcommand{\var}[2]{\langle \vec{#1^2_{#2}} \rangle} % variance command
...
% Bibliography source and style
   ...
   \title{Integrated turbulence parameters estimation from NAOMI AO telemetry data \thanks{Based on observations collected at the European Southern Observatory under ESO programme 60.A-9278(D).}}
   ...
   \institute{Faculdade de Engenharia da Universidade do Porto, Rua Dr. Roberto Frias, s/n, 4200-465 Porto, Portugal\\
            \and
            CENTRA – Centro de Astrofísica e Gravitação, IST, Universidade
...
\begin{itemize}
    ...
    \item in AO pre-compensation for free-space optical communications \citep{Griffiths2023,Osborn2021}.
\end{itemize}
...
The use of adaptive optics telemetry data for the estimation of integrated turbulence parameters is one such alternative \citep{Jolissaint2018,Fusco2004}. Adaptive optics systems are ubiquitous in current and future-generation telescopes. Estimating turbulence parameters from telemetry data has the advantage of sourcing data from already-built systems. Additionally, since the processed AO telemetry is sourced directly from the observing telescopes, we have a line-of-sight seeing estimation for each telescope. Finally, since the telemetry encompasses the full light path from the guide star to the telescope, we can include the dome turbulence contribution to the seeing. The former is particularly interesting to high-resolution imaging since it has been shown to greatly impact image quality \citep{Munro2023,Lai2019}.



Testing the algorithm on simulated and on-sky data is crucial to validate the estimation method. This study will utilise the telemetry data produced by the new adaptive optics module of the Very Large Telescope Interferometer (VLTI), NAOMI \citep{Woillez2019}, to achieve this validation. NAOMI is installed on all four ATs of the VLTI and can provide up to four concurrent telemetry samples. However, as a low-resolution system with a $4\times4$ Shack-Hartmann wavefront sensor, it is a challenging scenario for estimating turbulence parameters from AO telemetry since only 
low-order mode variances are available for the fit. These additional complications will affect the estimation of the turbulence parameters, and their impact is not yet understood. 
...
\noindent where $M$ is the total number of modes, $a_i$\footnote{If not explicitly stated in this article, a modal coefficient is implicitly represented at a fixed time, $t$} are the modal coefficients, $Z_i(\vec{\rho})$ is the Zernike polynomial of Noll order $i$, and $\vec{\rho} = (\rho/R,\theta)$ represents the position vector in polar coordinates  \citep{Noll1976}. The piston contribution, $i=1$, is discarded in the representation since it plays no role in imaging through a single pupil.


The NAOMI system tracks wavefront aberrations using a Shack-Hartmann sensor. Conventionally \citep{Southwell1980}, a set of slope measurements, $\vec{s}$, is related to the modal coefficients of the Zernike wavefront expansion through 
...
For modelling purposes we truncate the phase representation to a finite set of Zernike coefficients and split it into two subsets of the reconstructed $\vec{a_f}$ and remaining modes $\vec{a_r}$. Equation (\ref{eq: zernike coeff rel SH}) can be rewritten as 

\begin{equation}
    \label{eq: truncated a eq}
    \vec{s} = \mat{G} \begin{bmatrix} \vec{a_f} \\ \vec{a_r} \end{bmatrix} + \vec{e} = \mat{H}\vec{a_f} + \mat{G_r}\vec{a_r} + \vec{e},
\end{equation}

\noindent where $\mat{H}$ are the first $L$ columns of the $\mat{G}$ matrix, and $\mat{G_r}$ is constructed from the remaining higher-order columns. The least-squares solution of equation (\ref{eq: truncated a eq}) is given as
...
The former discussion is valid for an AO system operating in open loop. Instead, the adaptive optics systems we are interested in operate in a negative feedback closed loop \citep{hardy1998adaptive}. To retrieve the complete phase coefficients, $\vec{b}$, the control loop must be undone. Undoing the control loop will correspond to the summation of deformable mirror voltages, $\vec{v}_{\mathrm{DM}}$, with the corresponding Shack-Hartmann slope measurements, $\vec{s}_{\mathrm{SH}}$, in the modal space.

The modal coefficients for the deformable mirror, $\vec{c}_\mathrm{DM}$, are obtained from the mirror voltages and the matrix that translates the response of the deformable mirror into mode coefficients, $\mat{DM2M}$, through 
...
The residual modes, $\vec{b}_\mathrm{SH}$, are obtained from the residual slopes, $\vec{s}_{\mathrm{SH}}$, through the control matrix, $\mat{CM}$, that translates Shack-Hartmann slopes into Zernike modes using
...
\noindent A delay, $\tau$, is introduced in the slope reconstruction to account for the system's response time. In the case of NAOMI the total loop delay is \SI{4.6}{\ms} \citep{Woillez2019}.

The pseudo-open-loop coefficients are then 
...
The relation between the true coefficients, $\vec{a}$, and reconstructed coefficients, $\vec{b}$, is obtained by substituting equation (\ref{eq: truncated a eq}) into equation (\ref{eq: ls solution of b}), such that

\begin{equation}
    \label{eq: finite rec of modes}
    \vec{b} = \vec{a_f} + \mat{C}\,\vec{a_r} + \mat{H^+}\,\vec{e},
\end{equation}
...
The $\mat{C}$ matrix introduces a dependence on the remaining modal coefficients in two ways: i) a geometric coupling due to the non-orthogonality of the Zernike polynomial derivatives \citep{Herrmann1981} and ii) the aliasing term. 

\iffalse
    \begin{itemize}
    \item Due to the working principle of the SH-WFS, the $\mat{G}$ matrix contains the derivatives of the Zernike polynomials. Since these are non-orthogonal \citep{Herrmann1981}, the columns of $\mat{G}$ are non-orthogonal and introduce an error to the reconstructed coefficients;
    \item the $\mat{G}$ matrix also reflects the finite resolution of the sensor, and as such, the cross-talk matrix will also include aliasing effects. 
\end{itemize}
\fi


Although these are two different effects, they are treated as one - the remaining error, $\vec{R}$. 

\subsection{Zernike coefficient variances}


Estimating the Zernike modal variances requires the computation of the remaining error, which requires knowledge of the remaining modal variances. These are estimated through analytical expressions. The theoretical values for the von Kármán (co)-variance, $\cov{a_r}{a_r}$\footnote{ $\langle .\rangle$ indicates the ensemble average of a parameter.}, are obtained through the expressions derived by \cite{takato1995spatial} and are themselves a function of the Fried parameter, $r_0$, and the outer scale, $\mathcal{L}_0$.

The reconstruction also assumes that the measurement errors are equal and uncorrelated between measurements \citep{Southwell1980}. As a consequence the variance of the measurement noise, $\var{\vec{e}}{}$, can be treated as a constant variance across slopes, $i$, such that
...
The Zernike coefficient variances are evaluated from equation (\ref{eq: finite rec of modes}) through

\begin{equation}
    \var{\vec{b}}{} = \var{\vec{a_f}}{} + \mat{C}\,\cov{a_r}{a_r}\,\mat{C}^\mathrm{T} + 2\mat{C}\cov{a_r}{a_f} + \mat{H^+}\,\var{\vec{e}}{}\,(\mat{H^+})^\mathrm{T},
\end{equation}

\noindent where $\var{\vec{b}}{}$ represents the variance of the reconstructed modal coefficients and $\cov{\vec{a_i}}{\vec{a_j}}$ the (co-)variances of the exact modal coefficients. The measurement noise is assumed to be uncorrelated from the modal coefficients (i.e. $\cov{e}{a_r} = \cov{e}{a_f} = 0$). The variance of the reconstructed modes is then
\begin{equation}
    \label{eq: modal variances}
    \var{\vec{b}}{} = \var{\vec{a}}{f} + \vec{R} + \vec{\sigma^2_{n}},
\end{equation}

\noindent where $\vec{\sigma^2_{n}}$ is associated with the measurement error in the Zernike space and is expressed as

\begin{equation}
    \label{eq: measurement error}
    {\vec{\sigma^2_{n}}}= \sigma_0^2 (\mat{H}^\mathrm{T}\mat{H})^{-1},
\end{equation}
...
\begin{equation}
    \label{eq: reconst ct}
    {R}_i = \sum^{K=M-L}_{j=L+1}\sum^{K}_{j'=L+1}C_{j, i} \covnv{a_{r, j}}{a_{r, j'}} C_{j', i} + 2\sum^{K}_{j=L+1}C_{i, j} \covnv{a_{f, i}}{a_{r, j}},
\end{equation}




\noindent where $L$ is the number of fitted modes, $K = M - L$ is the number of remaining modes, $i$ represents the Zernike mode and $C$ are terms of the cross-talk matrix. Since Zernike polynomials are used in the real-time control of the NAOMI system, we opt to use them in favour of Karhunen-Loève modes. The Zernike basis is not statistically independent. This introduces the additional cross-term, $\cov{a_r}{a_f}$, in the remaining error. There is then an additional explicit dependence on the remaining modes. As a result of this dependence, the $\vec{R}$ is larger when compared to statistically independent modes (the K-L modes) \citep{Dai1996}. 



\subsection{Turbulence parameter estimation algorithm}

The integrated turbulence parameters, $\vec{p} = \left[ r_0, \mathcal{L}_0 \right]$, are estimated through the minimisation of the norm of the error function
\begin{equation}
    \label{eq: minimisation of p}
    \mathbf{\vec{p} = \arg \min_{\vec{p}}{\{||\vec{\epsilon}||^2\}}.}
\end{equation}
...
\begin{equation}
    ...
    \vec{\epsilon} = \var{\vec{b}}{} - \left( \var{\vec{a}}{f}_\mathrm{vk}(\vec{p}) + \vec{R}(\vec{p}) + \vec{\sigma^2_{n}} \right).
\end{equation}

This function is a vector in the Zernike modes that includes the analytical models for the von Kármán variances, $\var{a}{}_\mathrm{vk}(\vec{p})$, remaining error, $\vec{R}(\vec{p})$, and measurement error, $\vec{\sigma^2_{n}}$.

A $\chi^2$ approach is employed \citep{bevington2003data} to solve equation (\ref{eq: minimisation of p}), thus expanding on previous work by \cite{Andrade2019}. The $\chi^2$ approach requires knowledge of the squared uncertainty of each data point, in our case we assume that the uncertainty of each radial order is constant. This constant value, $\vec{\sigma^2_{\var{b}{}}}$, is estimated by taking the variance of the reconstructed modes, $\var{b}{}$, in each radial order.
...
The measurement and remaining error contributions to the $\epsilon^2$ function are much smaller in magnitude compared to $\var{a}{f}_\mathrm{vk}$. Therefore we choose to separate the estimation of the former from the global fit performed in equation (\ref{eq: minimisation of p}):

\begin{itemize}
    \item The measurement error is estimated through the use of the autocorrelation of the Zernike coefficients \citep{Fusco2004} (c.f. Appendix \ref{annex: Fusco noise}). A method based on the precomputation of the measurement noise allows the noise to be treated as a vector. Due to the reconstruction matrix, this is the case for the noise expressed in equation (\ref{eq: measurement error}). Finally, this new approach was shown in simulation to perform better then a global fitting approach \citep{Andrade2019} at $\mathrm{SNR} < 1$, while performing equally as well at higher $\mathrm{SNR}$ (c.f. Appendix \ref{annex: Fusco noise comparison}). We define the $\mathrm{SNR}$ as
    \begin{equation}
        \label{eq: SNR}
        \mathrm{SNR} = \frac{\var{\vec{s}}{}}{\sigma^2_0}.
    \end{equation}
...
\begin{equation}
    f(\vec{p^{k}}) =
    \begin{cases}
          \left(\varnv{a}{f,i}_\mathrm{vk} \right)(\vec{p^k}) + \sigma^2_{n,i}, & \text{if}\ k=0 \\
          \, \\
          \left(\varnv{a}{f,i}_\mathrm{vk} \right)(\vec{p^k}) + {R}_i(\vec{p^{k-1}}) + \sigma^2_{n,i}, & \text{if}\ k>0
    \end{cases},
\end{equation}
...
The tip/tilt modes are removed from the range of the fit (i.e. the minimisation starts at $i=4$) as they are still largely affected by wind shake and telescope vibrations \citep{gluck2017investigations}. While the defocus mode is affected also, we choose to use it in the reconstruction due to the small number of available modes. The maximum reconstructed modal variance included in the fit, $L$, is dictated by the number of modes controlled by the NAOMI system $(L=15)$ \citep{Woillez2019}.
...
This confirms an expected result: the minimum measurable spatial frequency will determine the maximum spatial scale detectable by the sensor. In the case of the ATs, the minimum measurable spatial frequency is $ f_\mathrm{m} = 1/D =  \SI{0.556}{\m^{-1}}$, where $D$ is the telescope diameter. As such, NAOMI is insensitive to spatial scales larger than its $D=\SI{1.8}{\m}$. The outer scale has been shown to vary between 
...
The following method estimates statistical uncertainty of the fit, $u(\mathbf{p})|_\mathrm{stat}$. The statistical uncertainty is added to the systematic uncertainty of the algorithm to serve as a confidence interval for the estimated $r_0$ when the algorithm is applied to on-sky telemetry, where the turbulence parameters are unknown.  
...
\noindent where the $\overline{\var{b}{i}}$ is the mean modal variance of the radial order, $n$, for the reconstructed variances, $\var{\vec{b}}{}$, and $\sigma_{\var{b}{},i}$ is the standard deviation around the mean. A new random sample, $\var{\vec{b}}{MC}$, is obtained by sweeping across the modes in Eq. (\ref{eq: full algorithm}) (i.e. $i=4$ to $i=15$). When applying the estimation algorithm to the new sample, $\var{\vec{b}}{\mathrm{MC}}$, we obtain a new set of turbulence parameters from which we evaluate the uncertainty.

The Monte Carlo algorithm generates $H=50$ random samples. The sample size was a good compromise between computational load and the performance of the estimation. Since the study of the uncertainty is done by a repeated generation of new values of turbulence parameters, we are interested in calculating the error of the mean estimation. From the $H$ turbulence parameters, the uncertainty of the mean is given by the standard deviation of the mean \citep{bevington2003data}. For the Fried parameter, the statistical uncertainty is given by
...
\noindent at a wavelength, $\lambda$, of $\SI{500}{\nm}$. The uncertainty of the seeing, $u(\alpha)$, is obtained directly from the uncertainty of the $r_0$ through error propagation analysis. $u(\alpha)$ is then given by
...
\begin{table}[h]
    \centering
    \caption{Features of the NAOMI adaptive optics system.}
    \vspace{0.3 cm}
    \begin{tabular}{c c}
    \hline\hline  
    Feature & \\
...
The estimation algorithm was optimised entirely in \textit{python} using the \textit{OOPAO} (Object Oriented Python Adaptive Optics) package \citep{Heritier2023}, which inherits the structure and functionality of its Matlab precursor OOMAO \citep{conan14}. The generation of von Kármán atmospheric phase screens was done using the algorithm proposed by \cite{assemat2006method} using the \textit{AOtools} package \citep{townson2019aotools}.

The atmospheric parameters were kept constant throughout the simulation due to the assumed stationarity. Turbulence parameters vary in time, but we will admit them to be stationary in the range of 1-2 minutes of acquisition \citep{Ziad2004}. 

The simulation is performed in an open loop, implemented by propagating the generated phase screens through a telescope object and a diffractive model of the WFS with a lenslet field-of-view of $\SI{7.33}{\arcsecond}$ to ensure linearity (Figure \ref{fig: block diagram}). The slopes obtained from the SH-WFS are converted into Zernike modal coefficients using a synthetic control matrix. Finally, the variances of the reconstructed coefficients are introduced to the estimation algorithm to obtain the $r_0$. The simulation is done in open-loop since the von Kármán model exists for uncorrected variances.

% Figure environment removed

The telescope and WFS were configured per the prescription in \cite{Woillez2019} and summarised in Table \ref{tab:features of NAOMI}. The propagating phase screens are correlated and reflect the typical turbulence parameters of the Paranal Observatory (Table \ref{tab: simulation conditions}). Nine layers were used for the atmospheric model with the wind speed and altitude profiles following the values presented by \cite{Masciadri2013}. Screens were generated at a $\SI{500}{\Hz}$ loop frequency. SH-WFS measurements feature a Gaussian additive white noise with known variance ($\sigma = [10^{-6}-10^{-7}] $, c.f. \ref{eq: noise def}). In the simulation, this corresponds to a 0.1 - 25 SNR regime. The average noise magnitude is taken from a $N=50$ telemetry data sample representative of the entire set.

The telemetry samples already contain a gradient matrix, $\mathbf{G}_\mathrm{system}$, obtained from the inverted control matrix that accounts for the first 14 controlled modes. A much higher number of modes is needed for the estimation of the remaining error (c.f. equation (\ref{eq: reconst ct})). The estimation algorithm used a synthetic matrix, $\mat{G}_\mathrm{synthetic}$, truncated between the $2^\mathrm{nd}$ and $13^\mathrm{th}$ radial order (i.e. 90 modes) to calculate these terms. The synthetic matrix was obtained by registering the response of the Shack-Hartmann model to individual Zernike modes introduced at the telescope pupil. The matrix was validated against the $\mat{G}_\mathrm{system}$ available in the telemetry samples and extended to additional modes once validation was achieved. The synthetic and system matrices were compared, and a median error of 1.3\% was found between the two. This mismatch adds systematic error to our estimation algorithm.  
...
\begin{itemize}
    ...
    \item The maximum radial order of the remaining error corresponds to the maximum number of modes included in the calculation of the cross-talk matrix. The matrix is constructed from the corrected terms, $\mat{H^+}$, and the higher non-corrected terms, $\mat{G_r}$. Letting the $\mat{G_r}$ matrix vary in size makes it possible to control the number of radial orders present in the estimation of the remaining error. Thus the estimation of turbulence parameters can be made to vary as a function of the maximum radial order included in the calculation of $\vec{R}$. $\widetilde{\Delta r_0}$ was found to stabilise once the error estimation included the 8$^\mathrm{th}$ radial order (Figure \ref{fig: evol with Gr}). The optimal radial order does not coincide with the maximum available radial order. This is attributed to a mismatch between the models used for the simulation of the optical system and the one used for the estimation of $f(\vec{p})$; 
    \item The minimum number of iterations, $k$, for which the estimation algorithm converges to a set of fitting parameters, $\vec{p^k}$, was shown to be small ($k = 2$). Once the threshold of $k=2$ iterations is crossed, only sub-\% changes to the $\widetilde{\Delta r_0}$ are registered;
...
\noindent The temporal evolution for a $\SI{20}{\second}$ time horizon was retrieved from the fit of the model and was found to add an uncertainty of $\SI{0.2(2)}{\%}$. The error of the uncertainty is calculated from the variance of the fitted parameters of the fit. 



The convergence bias is assumed to be independent from the mismatch between synthetic and system matrices. The total systematic error will then be the square sum of the two errors ($u(r_0)|_\mathrm{sys} = \SI{1.2}{\%}$).
...
Figure \ref{fig: simulation results} indicates the algorithm's performance in estimating the Fried parameter as a function of the time horizon of the sample. The effects of the remaining error estimation were tested by comparing the $k = 0$ and $k = 2$ iteration of the algorithm. The algorithm's $k=0$ iteration does not estimate the remaining error, while any iteration above the second was shown to be unnecessary. The optimised algorithm shows that the correction of the remaining error is necessary. Between the two iterations of the algorithm, an improvement in convergence bias from $\SI{17}{\%}$ to sub-$\SI{}{\%}$ is verified.

The algorithm's performance answers the feasibility question posed in Section \ref{sec: intro}. Sub-\% convergence bias estimations can be obtained in intervals of $\SI{20}{\second}$ non-overlapping telemetry samples once the remaining error estimation is included in the fit for a $4\times4$ Shack-Hartmann sensor.
...
\begin{table}[h]
\caption{Features of the NAOMI telemetry data set}
    \centering
    \begin{tabular}{c c}
    \hline\hline  
    Feature & \\
...
The on-sky data was packaged following the telemetry data format proposed by \cite{Gomes2022}. This data was curated following several criteria summarised in Table \ref{tab: features of cluster}:

\begin{itemize}
    \item Data with a $\SI{500}{\Hz}$ loop rate are selected. This loop ratio ensures a sufficient Strehl ratio in the samples. Lower loop frequencies are only active for high R magnitude AO reference star samples with low H and K-band Strehl ratios \citep{Woillez2019};
     \item A minimum time horizon of $\SI{20}{\s}$ was selected. $\SI{88.50}{\%}$ of the available data satisfy this condition;
     \item $N = 166$ telemetry samples possess unviable data (e.g. Figure \ref{fig: temp Zern coeff outlier}). We consider samples with a defocus $\var{b}{}$ below a $\SI{1e-10}{\radian^2}$ threshold to be incorrectly stored and remove them from the sample. 
\end{itemize}


We ensure the correct confidence levels of the algorithm through an analysis of the uncertainty of the estimation. The per cent statistical uncertainty of the algorithm estimation is shown in Figure \ref{fig: uncertainty estimates}. The median statistical uncertainty is 1.2\%. High uncertainty points are removed from the sample. We chose $\SI{10}{\%}$ as a threshold for acceptance. $N = 73$ data points (less than 1\%) were removed from the data sample this way. The total error of the estimation must include the systematic error. As such, the systematic error is added to the median uncertainty to obtain the median confidence level of ${(\alpha \pm 1.2|_\mathrm{stat} \% \pm 1.2|_\mathrm{sys} \%)}$.
...
Samples are gathered from the four Auxiliary Telescopes of the VLTI. These are spread across the Observatory and monitor the same star. The four telescopes will provide concurrent AO telemetry samples for the same observation. We can sample the spatial profile of the Paranal Observatory through the former with up to four concurrent seeing estimations. Table \ref{tab: comp SH} summarises the difference in seeing estimates for concurrent telemetry samples by presenting the mean difference between telescopes. The error shown is the error of the mean for the data points. From Table \ref{tab: comp SH}, we conclude that:

\begin{itemize}
    \item the average difference between telescopes is $\SI{0.011}{\arcsecond}$. This difference will account for a \SI{1.4}{\%} difference in average seeing across ATs. Since the median statistical uncertainty is \SI{1.2}{\%}, and the systematic uncertainty is \SI{1.2}{\%} for a 20 seconds time horizon, this points to an absence of a spatial profile of the seeing across Paranal within our uncertainty.
...
\subsection{Comparison with the DIMM}
...
\noindent where $N$ is the number of telemetry samples, $X_i$ is the observation of the MASS-DIMM and $Y_i$ is the estimation of our algorithm. We obtain $\mathrm{BIAS} = \SI{0.03}{\arcsecond}$ and $\mathrm{RMSE} = \SI{0.22}{\arcsecond}$. These results are consistent with those reported in \cite{Masciadri2023} for the comparison of values between Stereo-SCIDAR and MASS-DIMM, which are $\mathrm{BIAS} = \SI{0.08}{\arcsecond}$ and $\mathrm{RMSE} = \SI{0.25}{\arcsecond}$. 
...
The ATs and the DIMM tower are located at different positions in the Observatory, with the ATs occupying a central position while the DIMM is installed at the edge of the Observatory. The dome turbulence is an additional component in the telemetry data. The dome turbulence has been shown to play an important role in the total value of the seeing in larger telescopes than then ATs \citep{Lai2019}. The DIMM uses an open telescope design \citep{Tokovinin2007}, which does not share a dome with the ATs. Coupling these factors and discarding systematic effects between the DIMM and NAOMI turbulence measurements, the local turbulence at the ATs seems to be larger than the one seen at the edge of the Observatory.
         

\section{Conclusions} 
\label{sec: conclusion}

This paper aimed to answer a main question: Can a low order ($4\times 4$) Shack-Hartmann wavefront sensor telemetry be used to estimate integrated turbulence parameters?

To answer this question, an iterative estimation algorithm was first validated on simulated data and then applied to telemetry data produced from the operation of the adaptive optics system of the Auxiliary Telescopes of the VLT, NAOMI.

The estimation algorithm was optimised using a simulation of the NAOMI system. Simulation results show the ideal number of iterations, minimum time horizon and the number of modes to be included in the fitted variances and remaining error estimation. The $\chi^2$ map of the algorithm was shown to converge to the expected value without bias, and through its analysis, the ideal initial guess for our algorithm was established. The correction of remaining and measurement noise was shown to be necessary for the correct estimation of the Fried parameter. The uncorrected variances were shown to incorrectly estimate the Fried parameter, with an error of 17\%. By compensating for the remaining error, we were able to achieve sub-per cent convergence bias in the estimation of the parameter. 
...
It was shown that the Fried parameter could be estimated from a low-order Shack-Hartmann system in a \SI{1.8}{m} diameter telescope but not the outer scale. The next immediate step is to apply the algorithm to a higher-order system in a larger-diameter telescope. Larger telescope diameters imply a larger sensitivity to the outer scale. On the other hand, higher-order systems translate into less contamination of lower orders (more sensitive to the outer scale) by aliasing and cross-talk. The ideal test bench is the CIAO system installed in the UTs.
...
We acknowledge and thanks discussions of the telemetry data standard with Tiago Gomes.
...
In this appendix we address the non-constancy of the weighting factors in the chi-squared fit.
Figure \ref{fig: reconstructed coefficients} illustrates:
\begin{itemize}
    \item On the top graph: (i) dashed - the reconstructed variances, $\var{b}{}$, for a simulated telemetry samples; (ii) dotted - the theoretical von Kármán variances of the phase screen turbulence parameters; (iii) solid - the corrected variances, $\var{b}{}_\mathrm{corr}$, which we will define as the subtraction of the estimation noise from the reconstructed variances, such that
    
    \begin{equation}
        \var{b}{}_\mathrm{corr}  = \var{b}{} - \vec{R} - \vec{\sigma^2_n}.
    \end{equation}
    
    \item On the bottom graph - the weight factors, $\sigma_{\varnv{b}{},i}$, obtained from the variance of radial order of the reconstructed variances presented in the preceding graph.
\end{itemize}
...
\section{Noise estimation from temporal autocorrelation of reconstructed coefficients}
...
\begin{equation}
    C_i(\tau)  = \langle {b_i(t)}{b_i(t+\tau)}\rangle_t = C_{\mathrm{turb},i}(\tau) + \sigma_{n,i}^2\,\delta\tau,
\end{equation}
 
\noindent where $C_{\mathrm{turb},i}(\tau)$ is the temporal autocorrelation of the atmospheric contributions and $ \sigma_{n,i}^2$ is the measurement noise. The measurement noise is retrieved from $C_i(0)$ through the difference between $C_i(0)$ and $C_{\mathrm{turb},i}(0)$. 

Since the measurement noise is only present at $\tau = 0$, we can estimate $C_{\mathrm{turb},i}(0)$ through the polynomial fit of the first points of autocorrelation (excluding $\tau=0$). $C_{\mathrm{turb},i}(0)$ is estimated through the intercept at $\tau=0$.


\section{$r_0$ estimation performance comparison for different measurement noise estimation techniques} 
\label{annex: Fusco noise comparison}

The measurement noise is introduced as white noise in the slopes, following a known standard deviation, $\sigma$ (c.f. equation (\ref{eq: noise def})). Since the noise is obtained from the reconstructed coefficients, we need to take into account the effects of the reconstruction matrix, $\mat{H}$, into the noise vector, ${\vec{\sigma^2_{n}}}$, through equation (\ref{eq: measurement error}). As such, the noise can't be considered constant across Zernike modes. The SNR of individual Zernike modes is defined as

\begin{equation}
    SNR_i = \frac{\varnv{b}{i}}{ \sigma^2_{n,i}},
\end{equation}


\noindent and is represented in Figure \ref{fig: SNR modes} for modes of distinct radial orders, $n$. 

% Figure environment removed


Empirically, the noise can't be taken as constant across radial orders. An approach that allows the noise to act as a vector is then more in line with reality, which is the case for the estimation of noise through autocorrelation of the Zernike coefficients.

To illustrate the choice of our estimation method, we test both measurement noise estimations: 
...
To illustrate their effect on the convergence of the algorithm, we apply both methods to simulated telemetry samples at different SNR regimes, following the same atmospheric conditions expressed in the Section \ref{subsec: simulation results}. The results are illustrated in Figure \ref{fig: SNR relation}. We find that the Fried parameter estimation by the autocorrelation method is more stable in the low SNR regime. As such, the estimation of the measurement noise in the low SNR regime is improved, while performing equally as well in the high SNR regime.


% Figure environment removed

Additionally, by removing the noise from the chi-squared minimisation we reduce the number of fitting variables from $\vec{p} = (r_0, \mathcal{L}_0, \sigma_0$) to $\vec{p} = (r_0, \mathcal{L}_0)$.
