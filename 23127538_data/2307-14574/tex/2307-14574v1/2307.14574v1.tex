...
\address{School of Mathematics and Statistics, University of Melbourne, Parkville, Australia, 3010}
...
\address{Department of Mathematical and Statistical Sciences, University of Alberta, 632 CAB, Edmonton, Alberta, Canada T6G 2G1}
...
Let $\g$ be a simple Lie superalgebra. To any nilpotent element $f\in\g$ of even parity and any complex number $k\in\C$, referred as the level, one associates the $\W$-(super)algebra $\WA$. It is obtained by applying the quantum Drinfeld--Sokolov reduction (a.k.a BRST reduction) to the universal affine vertex superalgebra $\VA$ \cite{FF1,KRW03,KW} and can be regarded as an affinization of the finite $\W$-algebra $U(\g,f)$ introduced by Premet \cite{Pr}. Indeed, $U(\g,f)$ can be obtained from $\WA$ by taking its Zhu's algebra \cite{FZ,Z}.

The $\W$-algebras were firstly constructed by Feigin and Frenkel \cite{FF1} for principal nilpotent elements $f\in\g$. The corresponding principal $\W$-algebras $\W^k(\g,\fpr)$ -- usually denoted by $\W^k(\g)$ -- and their representation theories have been studied intensively \cite{A1,A2,AF,D,DR,FKW92,R}.
...
When the affine vertex subalgebras are non-super, these isomorphisms can be proven by using certain universal $\W_\infty$-algebras \cite{CL1,CL2}.
...
The $\W$-superalgebra $\W^\ell(\sll_{2|1})$ is realized in terms of $V^k(\sll_2)$ \cite{KaSu} and vise versa \cite{FST}, which amounts to a block-wise equivalence of their weight module categories, including the natural isomorphisms of the spaces of intertwining operators and the fusion rules \cite{CGNS,FSST}.
...
In this paper, we continue the study for the orthosymplectic analogue between
\begin{align*}
...
In the literature, we find type $A$ case and some simply-laced cases \cite{AvE,CGNS}. Our result is a generalization of $\W^k(\symp_{4},\fsub)$ established by one of the author \cite{Fa} and seems to be the first result on the orthosymplectic type in the super side beyond the rank one case \cite{Ad3}.

\subsection{Results}

In the present paper, we start with proving an embedding between subregular and principal $\W$-algebras of type $B$.
...
the diagram below commutes:
    \begin{center}
		\begin{tikzcd}[row sep=huge, column sep = huge]
            ...
            \hspace{-3cm}\otimes \mathcal{F}_1 %V_\Z\otimes\pi^{k+h^\vee_+}_{\epsilon_0}
			\\
			\W^\ell(\osp_{2|2n})
			\arrow[r, hook, "\text{Wakimoto real.}"]&
			\W^\ell(\symp_{2n})&
            \hspace{-3.4cm}\otimes \mathcal{F}_2. %V_\Z\otimes\pi^{\ell+h^\vee_-}_{t_0}.
		\end{tikzcd}
	\end{center}
Here $\mathcal{F}_i$ $(i=1,2)$ are free field vertex superalgebras.
% Here, $\mu_{\mathrm{iHR}}\colon \subW\hookrightarrow \Ker S \cap \left(\Pi(0)\otimes\Wpr\right)$ is an embedding of vertex algebras in general and an isomorphism at generic levels.
\end{MainThm} 
...
\subsection{Outlook}
As we classify the simple $\W_k(\so_{2n+1},\fsub)$-modules in the exceptional case, a next natural and desirable step is to describe the fusion rules. In this view, and based on the construction of the simple modules, we formulate a precise conjecture when $k$ is principal admissible, i.e. denominator $q=2n-1$ (see Conjecture~\ref{conj:fusion_rules}). In that case, the description is similar to the type $A$ case \cite{AvE,CGNS}.
...
We hope to come back to these topics in a close future.
...
After we recall the Feigin--Semikhatov duality, we relate it with the Feigin--Frenkel duality in Sect.~\ref{sec:FF-FS}.
In order to do so, we establish the inverse Hamiltonian reduction between the subregular and the principal $\W$-algebras associated to $\so_{2n+1}$ (see Theorem~\ref{inverse HR}).
...
on $\so_{2n+1}$.
The Fig.~\ref{good grading of so} below gives the good grading in terms of weighted Dynkin diagram. 
...
Its structure is very similar to the one of $\Wsub$.
...
By using the coset construction, we can compare the multiplicities of the Fock modules over $\pi^{J^\pm}$. 
We decompose 
...
This happens iff $k$ is one of 
$$k_1=-h^\vee_++\frac{n}{n-1},\qquad k_2=-h^\vee_++\frac{2n+1}{2n},$$
which correspond to the dual level $\ell$ to be
...
We postpone the proof in the case $i=2$ to Theorem~\ref{unitary case1}. Indeed, the level $k_2$ is known to be admissible exceptional. We study these levels in more details in Sect.~\ref{sec:unitarity}.
...
The first case is of particular interest as $\W^{\ell_1}(\so_{2n-1})$ is isomorphic to the $\mathrm{Sp}_{2n-2}$-orbifold $\mathcal{A}(n-1)^{\mathrm{Sp}_{2n-2}}$ of the rank $n-1$ symplectic fermion algebra $\mathcal{A}(n-1)$ \cite[Theorem~5.1]{KL19}, which is simple by \cite{DLM}.
We conjecture the following:
\begin{conjecture}
    For $n\geq2$, we have an isomorphism of vertex algebras
    $$\W_{-h^\vee_{n}+1}^{D^+}(n,1)\simeq \W^{-h^\vee_{n-1}+1}(\so_{2n-1})\ (\simeq \mathcal{A}(n-1)^{\mathrm{Sp}_{2n-2}}).$$
\end{conjecture}
The case $n=2$ is established by the first-named author \cite{Fas21}
$$\W_{-2}^{D^+}(2,1)\simeq \mathrm{Vir}_{-2}.$$ 
For higher ranks we show that $\W_{-h^\vee_n+1}^{D^+}(n,1)$ is weakly generated by the strong generators $W_2$, $W_4$, of conformal weights $2$ and $4$ respectively, and that the parameter $\lambda$ appearing in the OPE of $W_4(z)W_4(w)$ agrees with that of $\W_{-h^\vee_{n-1}+1}(\so_{2n-1})$  \cite{FLN}.
...
Hence, both $\W$-algebras are realized as subalgebras of $\Pi(0)\otimes\pi^{k+h^\vee_{+}}_{\h_+}$, which are characterized as joint kernel of screening operators at generic levels. Moreover, most of these operators coincide: $Q_i^+=Q_i^{\mathrm{pr}}$ for $i=2,\ldots,n$. 
...
\end{align*}
In particular, if a category of $V$-module is a braided tensor (super)category closed under the spectral flow twists $S_{\theta H}$ $(\theta\in\Z)$,
...
\noindent
As we could not find an appropriate reference for the proof, we provide it in Appendix~\ref{appx:proof_simple_current}. 
...
\proof  
It remains to show the second isomorphism. Note that $ \bigwedge{}^{\frac{\infty}{2}+\bullet}(\g_{>0})$ is a tensor product of the bc-systemx $\bigwedge^{\bullet}$ generated by $\varphi_\alpha^*(z)$ and $\varphi_\alpha(z)$ for each $\alpha\in\Delta_{>0}$. 
...
To motivate this approach, notice that Lemma~\ref{isom for the multiplicities} implies that $\subW$ and $\sprW$ resemble except for the norms of $J^+(z)$ and $J^-(z)$.
...
The functors $\mathrm{H}_{\lambda_\theta^+}^+$ are actually related to each other through spectral flow twits.
...
%We deduce a compatibility between $\mathrm{H}_{\lambda^+_{\theta}}^+$ and spectral flow twists.
...
Applying $H_f^0$, we obtain the embedding 
...
J^+ &\mapsto -y+{\varpi}_{1} & J^-& \mapsto x+\check{\varpi}_0\\
\end{array}
\end{align}
...
We start collecting some useful data to describe the representation theory of the affine Kac--Moody algebra associated with ${\so}_{2n+1}$.
...
The level $k$ is said admissible if $k\Lambda$ is an admissible weight.
...
\subsection{Zhu's algebra}
To any conformal vertex algebra $V$ is attached a unital associative $\C$-algebra $A(V)$, called the Zhu's algebra \cite{Z}. 
In particular, the Zhu's algebra $A(V^k(\g))$ of the universal affine vertex algebra $V^k(\g)$ is isomorphic to the enveloping algebra $U(\g)$. 
...
The Zhu's algebra of the universal $\W$-algebra $\W^k(\g,f)$ is isomorphic to the corresponding finite $\W$-algebra $U(\g,f)$ \cite{Pr}, which is isomorphic to the finite analogue of the BRST reduction $H_f^0(U(\g))$ (see \cite{A1,DK}). Moreover, we have an isomorphism of $\C$-algebras
\begin{align*}
...
By \cite[Theorem 7.2]{AvE} we have an isomorphism of vertex algebras
\begin{align}\label{simple exceptional W-algebra}
    \ssubW\simeq H_f^0(L_k(\g)),
\end{align}
which can be used to describe the Zhu's algebra of the simple $\W$-algebras $\ssubW$ \cite[Theorem 4.5]{AvE}
...
\end{align*}
Moreover, the $\Z_2$-action in the right-hand side is fixed-point free. 
\end{proposition}
...
\begin{align*}
J_{m}^+\mapsto J_m^++\delta_{m,0}\tfrac{p-q}{q}\theta,\quad L_m\mapsto L_m+\theta J_n^++\tfrac{1}{2}\tfrac{p-q}{q}\theta^2 \delta_{m,0}.
\end{align*}
Accordingly, the character $\mathrm{ch}_{M}(z,q):=\mathrm{tr}_{M}z^{J^+_0}q^{L_0}$ and the set $\mathrm{wt}(M)$ transform into%
%\footnote{In general for a conformal vector $L$ of central charge $c$ and a primary Heisenberg vector $H$ such that $[H_\lambda H]=a \lambda$, 
%$$\mathrm{ch}_{S_\theta M} (z,q)=q^{\half a \theta^2}z^{a\theta}\mathrm{ch}_M(zq^\theta,q).$$
%} 
...
\proof
Since $J^+$ correspondes to $\check{\varpi}_1$, $\mathrm{wt}(\mathbf{L}_k(\lambda))=\Z$ or $\half+\Z$ for $\lambda \in \Pr_\Z^k$.
...
In the case $q=2n-1$, $D(\lambda)$ agrees with the quantum dimension of the module $L_{p-h^\vee}(\lambda)$ of the simple affine vertex algebra $L_{p-h\vee}(\g)$. Hence, the assertion follows from Proposition~\ref{simple current action}. For $q=2n$, we analyze $D(\lambda)$ with classic tools of analysis, adapting an argument of \cite{Fu}. Details are provided in Appendix \ref{app: proof of lemma}.
\endproof
...
%where the tensor product identifies $\mathbf{L}_{k}(\sigma\circ\lambda)\otimes [\theta]\simeq \mathbf{L}_{k}(\lambda)\otimes [\theta+\bar{q}]$.
\end{enumerate}
\end{theorem}
...
The simple $L(\half,0)$-modules are $L(\half,h)$ ($h=0,\tfrac{1}{16}, \tfrac{1}{2}$). The fusions rules can be computed directly or find in \cite{dFMS}:
\begin{align*}
...
In this case, $V$ is also unitary as a module over the Virasoro subalgebra generated by its conformal vector. In particular, the central charge is positive real.
...
Since $\W_{-5/4}^{D_+}(2,1)$ is self-dual and $\widehat{L}(\tfrac{6}{7},\tfrac{4}{3})^\pm$ are contragredient to each other, both $\widehat{L}(\tfrac{6}{7},\tfrac{4}{3})^\pm$ must appear in the decomposition. By replacing $J^+$ with $-J^+$ if necessary, we obtain the desired decomposition. 
\end{proof} 
...
The spectral flow twists $\mathcal{E}^s(a)\simeq S_{a}\ssprW$ are simple currents %, i.e. invertible objects in the tensor category
...
When $\mathcal{C}$ is realized a module category of vertex superalgebras as in our case, the monodromy for a simple module $M$ is expresses as 
...
%The $(L_0,\h_+)$-eigenspace decomposition of the complex $\mathscr{C}_\mu^k$ gives a direct sum decomposition into finite-dimensional subcomplexes.
The convergence of the spectral sequences associated with double complexes implies that 
\begin{equation*}
    H^n(\mathscr{C}_{\mu,f}^k)\simeq \delta_{n,0}H_f^n(L_k(\mu))\simeq \mathbf{L}_k(\mu),%\quad \mathbf{L}_k(\mu):=H_f^0(L_k(\mu))
\end{equation*}
...
\\
%&=q^{-\frac{q}{2p}(|\rho|^2+(\mu,\varpi_1)^2)+n(\mu+\rho,\varpi_1)+(x^+_\Gamma,\rho)}z^{\frac{q}{p}(\mu+\rho,\varpi_1)}\frac{(-zq^{n+\half},-z^{-1}q^{-n+\half};q)_\infty}{(q;q)_\infty^{n+1}}\\
%&\quad\sum_{\substack{w\in W\\\lambda\in \check{Q}}}(-1)^{\ell(w)}\frac{q^{-(x^+_\Gamma+n\varpi_1,w(\mu+\rho)+p\lambda)+\frac{q}{2p}|w(\mu+\rho)+p\lambda|^2}z^{-\frac{q}{p}(\varpi_1,w(\mu+\rho)+p\lambda)}}{(1+z^{-1}q^{-(w(\mu+\rho)-\mu+p\lambda,\varpi_1)})}\\
...
\appendix
\section{On simple current modules}
\subsection{Proof of Proposition \ref{simple current action}}\label{appx:proof_simple_current}
\begin{proof}[Proof of Proposition \ref{simple current action}]
Under the spectral flow twist $S_{\check{\varpi}_i}$, the Chevalley generators $E_{j}$, $H_{j}$ ($j=0,\ldots, n$) of the affine Lie algebra $\widehat{\g}$ on $L_k(s_i\Lambda)$ act as
\begin{align}\label{modified action}
E_j\mapsto \begin{cases}f_{\theta,0} & (i=0)\\ e_{\alpha_j,0}& (j\neq i)\\ e_{\alpha_j,1}& (i=j)\end{cases},\quad 
H_j\mapsto \begin{cases}-\theta_0 & (i= 0) \\ \check{\alpha}_{j, 0}& (j\neq i)\\\check{\alpha}_{i,0}+k& (i=j).\end{cases}
\end{align}
Here we use the following notation: $\Delta$ the root system; $\Delta=\Delta_+\sqcup \Delta_-$ the standard polarization with simple roots $\Pi_+=\{\alpha_1,\ldots, \alpha_n\}$; $\theta$ the highest root, $e_{\alpha}$ (resp.\ $f_\alpha$) the standard positive (resp.\ negative) root vector with $\check{\alpha}_j=[e_{\alpha_j},f_{\alpha_j}]$ ($1\leq j\leq n$). 

Notice that under the action (\eqref{modified action} the simple $\g$-module $L(\lambda)$, realized as the lowest conformal weight subspace of $L_k(\Lambda)$, still contains an affine highest weight vector, say $v_{\mathrm{new}}$. Indeed, it is a highest weight vector of $\g$-module with respect to the different polarization, namely $\{\alpha_j,-\theta\mid j\neq i\}$.
Therefore, the affine weight of $v_{\mathrm{new}}$ under \eqref{modified action} gives $s_i\Lambda$. 

We compute $v_{\mathrm{new}}$ for the basic representation $L(\varpi_j)$ ($j=1,\ldots,n$) of $\g$ by using an explicit realization \cite{FH}.
First, $L(\varpi_1)$ is realized as the standard representation $\C^{n+1}$, $\C^{2n+1}$, $\C^{2n}$, $\C^{2n}$ for $A_n$, $B_n$, $C_n$, $D_n$, respectively. Let $e_i$ denote the standard basis of these vector spaces with index $I_X$ given by
$$I_A=\{1,\ldots, n\},\ I_B=\left\{0,1,\ldots, n,\parity{1},\ldots,\parity{n}\right\},\ I_C=I_D=\left\{1,\ldots, n,\parity{1},\ldots,\parity{n}\right\}.$$
Then $L(\varpi_i)$ is realized as the subspace generated by $e_1 \wedge\cdots \wedge e_j$ inside $\bigwedge^j L(\varpi_1)$ for all $i$ in the case $A_n$, $C_n$, for $i\neq n$ in the case $B_n$ and for $i\neq n-1,n$ in the case $D_n$.
The remaining cases $L(\varpi_n)$ for $B_n$ and $L(\varpi_{n-1})$, $L(\varpi_n)$ for $D_n$ are given by the spin representations:
\begin{align*}
\begin{array}{cl}
B_n\colon & L(\varpi_n)\simeq \bigwedge^\bullet \bigoplus_{p=1}^n \C e_p \\
D_n\colon & \begin{cases} L(\varpi_{n-1})\simeq \bigwedge^{\mathrm{odd}} \bigoplus_{p=1}^n \C e_p,\quad L(\varpi_{n})\simeq \bigwedge^{\mathrm{even}} \bigoplus_{p=1}^n \C e_p &(n\ \text{even})\\
L(\varpi_{n-1})\simeq \bigwedge^{\mathrm{even}} \bigoplus_{p=1}^n \C e_p,\quad L(\varpi_{n})\simeq \bigwedge^{\mathrm{odd}} \bigoplus_{p=1}^n \C e_p &(n\ \text{odd}). \end{cases}
\end{array}
\end{align*}
It is straightforward to show that $v_{\mathrm{new}}$ can be taken as in the table below.
{\tiny
\begin{center}
\begin{tabular}{llll || llll } \hline
 $X_n$ &\hspace{-2mm} $L(\varpi_j)$ & $s_i$ & $v_{\mathrm{new}}$ &  $X_n$ &\hspace{-2mm} $L(\varpi_j)$ & $s_i$ & $v_{\mathrm{new}}$\\ \hline
 $A_n$ &\hspace{-2mm} $j\colon$ all & $s_1$ & $e_2 \wedge\cdots \wedge e_{j+1}$ & $D_n$ & \hspace{-2mm} $j=n-1$ & $s_1$& $e_2 \wedge\cdots \wedge e_{n}$\\
 $B_n$ &\hspace{-2mm} $j\neq n$ & $s_1$ & $e_{\parity{1}}\wedge e_2 \wedge\cdots \wedge e_{j}$ & & & $s_{n-1}$&  $\begin{cases} e_n &\hspace{-2mm} (n\colon \text{even}) \\ e_1\wedge e_n &\hspace{-2mm} (n\colon \text{odd}) \end{cases}$\\ 
       &\hspace{-2mm} $j=n$ & $s_1$ & $e_2 \wedge\cdots \wedge e_{n}$ &  & & $s_n$ & $\begin{cases} e_1 &\hspace{-2mm} (n\colon \text{even}) \\ 1 & (n\colon \text{odd})\end{cases}$\\
 $C_n$ &$j\colon$ all & $s_n$ & $e_{\parity{n+1-j}} \wedge\cdots \wedge e_{\parity{n}}$ &&\hspace{-2mm} $j=n$ & $s_1$& $e_2 \wedge\cdots \wedge e_{n-1}$ \\
  & & $s_{n-1}$&  $\begin{cases} e_1 \wedge e_n &\hspace{-2mm} (n\colon \text{even}) \\ e_n &\hspace{-2mm} (n\colon \text{odd}) \end{cases}$  & & & $s_{n-1}$&  $\begin{cases} e_1 \wedge e_n &\hspace{-2mm} (n\colon \text{even}) \\ e_n &\hspace{-2mm} (n\colon \text{odd}) \end{cases}$\\
 $D_n$ &\hspace{-2mm}$j\neq n-1,n$ & $s_1$& $e_{\parity{1}}\wedge e_2 \wedge\cdots \wedge e_{j}$ &  & & $s_n$ & $\begin{cases} 1 & (n\colon \text{even}) \\ e_1 & (n\colon \text{odd}) \end{cases} $\\
  & & $s_{n-1}$&  $e_{\parity{n+1-j}} \wedge\cdots \wedge e_{\parity{n-1}}\wedge e_{n}$\\
  & & $s_n$ &  $e_{\parity{n+1-j}} \wedge\cdots \wedge e_{\parity{n-1}}\wedge e_{\parity{n}}$ \\\hline
\end{tabular}
\end{center}}
Since $L(\lambda)$ with $\lambda=\sum_{i=1}^n\lambda_j \varpi_j$ is realized inside $\otimes_{j=1}^n L(\varpi_j)^{\otimes \lambda_j}$, $v_{\mathrm{new}}$ for $L(\lambda)$ is obtained as the tensor product of the $v_{\mathrm{new}}$'s of each factor. Now, by looking at the eigenvalue of the modified action \eqref{modified action} of $H_j$, we obtain the assertion for type $B_n$, $C_n$, $D_n$ and $s_1$ for $A_n$. The assertion of $s_i$ for $A_n$ immediately follows by noticing $L_k(k \Lambda_1)\boxtimes L_k(k \Lambda_i)\simeq L_k(k \Lambda_{i+1})$ and thus $S_{\check{\varpi}_i}=S_{\check{\varpi}_1}^i$ by induction. This completes the proof.
\end{proof}

\subsection{Proof of Lemma \ref{classification of simple currents}}\label{app: proof of lemma}
\begin{proof}[Proof of Lemma \ref{classification of simple currents}]
The proof is similar to the principal case and obtained by a slight modification of \cite{Fu}.
The $\ssubW$-module $\mathbf{L}_k(\lambda)$ ($\lambda\in \Pr_\Z^k$) is a simple current iff  the quantum dimension \eqref{qdim for coprincial case} is one, that is
\begin{align*}
    D(\lambda)=\prod_{\alpha\in \Delta_+}\frac{\sin(\tfrac{(\lambda+\rho,\check{\alpha})}p\pi)}{\sin(\tfrac{(\rho,\check{\alpha})}p\pi)}=1.
\end{align*}
Let us regard $D(\lambda)$ as an analytic function on the (compact) region with boundary
\begin{align*}
    R=\left\{\lambda=\sum \lambda_i \varpi_i\in \bigoplus \R \varpi_i\left | \lambda_i \geq0,\ (\lambda,\check{\theta}_s)\leq p-h\right.\right\}.
\end{align*}
Note that $D(\lambda)>0$ on $R$ and $\mathrm{Pr}_\Z^k$ is the set of $\Z$-valued points in $R$.
We first show that $D(\lambda)$ has minimal values on $R$ only on the corners, i.e.
$$C:=\{0,\tfrac{p-h}{2}\varpi_1,\ldots, \tfrac{p-h}{2}\varpi_{n-1},(p-h)\varpi_n\}.$$
For this, we use the following stratification of $R$ by subsets $R_I^\circ$ without boundary indexed by subsets $I\subset \{0,1,\ldots,n\}$ (including the empty set $\emptyset$)
$$ R=\bigcup_{I}R_I^\circ,\quad R_I^\circ=\left(R \cap \bigcap_{i\in I} P_i \right)\backslash \partial \left(R \cap \bigcap_{i\in I} P_i \right),$$
where $P_i$ is the hyperplane $\{\lambda_i=0\}$.
Introduce 
\begin{align*}
    D_\ell(\lambda):=
    \begin{cases}
        D(\lambda_1,\ldots,\lambda_{n-1},p-h-2(\lambda_1+\ldots+\lambda_{n-1}))& (\ell=0),\\
        D(\lambda_1,\ldots,\lambda_{\ell-1},0,\lambda_{\ell+1},\ldots,\lambda_{n})& (\ell\neq0).
    \end{cases}
\end{align*}
If $I\neq \emptyset$, $D(\lambda)=D_\ell(\lambda)$ on $R_I^\circ$ for $\ell\in I$. We take $\ell=0$ if $0\in I$ and set $D_\phi(\lambda)=D(\lambda)$ when $I=\emptyset$. 
By setting
\begin{align*}
    \mathscr{E}_\ell(\lambda)=\sum_{\alpha\in \Delta_+}(\varpi_\ell,\check{\alpha})\cot(\tfrac{(\lambda+\rho,\check{\alpha})}{p}\pi),
\end{align*}
we have
\begin{align*}
    &\partial_{\lambda_i} D_\ell(\lambda)=\frac{\pi}{p}(\mathscr{E}_i(\lambda)-2\delta_{\ell,0}\mathscr{E}_n(\lambda))D_\ell(\lambda).
\end{align*}
Let $\lambda^\circ\in R_I^\circ$ be a point where the gradient $\nabla D_\ell(\lambda^\circ)=(\partial_{\lambda_i}D_\ell)(\lambda^\circ)=0$. The Hessian $H^\ell(\lambda)=(\partial_{\lambda_i}\partial_{\lambda_j}D_\ell)(\lambda^\circ)$  satisfies
$H^\ell_{i,i}(\lambda^\circ)<0$
on the diagonal entries since we have 
\begin{align*}
H^\ell_{i,i}(\lambda^\circ)
=\frac{\pi}{p}\left(\partial_{\lambda_i}\mathscr{E}_i(\lambda^\circ)-\frac{\pi}{p}\mathscr{E}_i(\lambda^\circ)^2\right)D_{\ell}(\lambda^\circ)
=\frac{\pi}{p}\partial_{\lambda_i}\mathscr{E}_i(\lambda^\circ)D_{\ell}(\lambda^\circ)<0
\end{align*}
for $\ell\neq 0$ by using the inequality
\begin{align*}
    &\partial_{\lambda_i}\mathscr{E}_i(\lambda^\circ)=-\frac{\pi}{p}\sum_{\alpha\in \Delta_+}\frac{(\varpi_\ell,\check{\alpha})^2}{\sin^2(\frac{(\rho+\lambda^\circ,\check{\alpha})}{p}\pi)}<0,
\end{align*}
and similarly for $\ell=0$
\begin{align*}
H^0_{i,i}(\lambda^\circ)
&=\frac{\pi}{p}\left(\partial_{\lambda_i}(\mathscr{E}_i(\lambda^\circ)-2\mathscr{E}_n(\lambda^\circ))-\frac{\pi}{p}(\mathscr{E}_i(\lambda^\circ)-2\mathscr{E}_n(\lambda^\circ))^2\right)D_{0}(\lambda^\circ)\\
&=\frac{\pi}{p}\partial_{\lambda_i}(\mathscr{E}_i(\lambda^\circ)-2\mathscr{E}_n(\lambda^\circ))D_{0}(\lambda^\circ)<0
\end{align*}
by using 
\begin{align*}
    \partial_{\lambda_i}(\mathscr{E}_i(\lambda^\circ)-2\mathscr{E}_n(\lambda^\circ))
    =-\frac{\pi}{p}\sum_{\alpha\in \Delta_+}\frac{(\varpi_i-\varpi_n,\check{\alpha})^2+(\varpi_n,\check{\alpha})^2}{\sin^2(\frac{(\rho+\lambda^\circ,\check{\alpha})}{p}\pi)}<0.
\end{align*}
Now, as the trace of $H^\ell(\lambda^\circ)$ is less than zero, $H^\ell(\lambda^\circ)$ has a negative eigenvalue.
Thus $D_\ell$ does not have a local minimal value at $\lambda=\lambda^\circ$, which implies that $D(\lambda)$ has a local minimal value only at the corners $C$ of $R$.

Next, we consider the values $D(\lambda)$ ($\lambda\in C$). 
Note that $p>h=2n$ and $p$ is coprime to $q=2n$. 
It is to show that
\begin{align*}
    D(0)=1,\quad D((p-h)\varpi_n)=1,\quad D(\tfrac{p-h}{2}\varpi_i)=D(\tfrac{p-h}{2}\varpi_{n-i})\quad (1\leq i <n).
\end{align*}
For $p=2n+1$, we have $\Pr_\Z^k=\{0,\varpi_n\}$ and thus obtain the assertion. We consider the case $p\geq 2n+3$ and show
$$1<D(\tfrac{p-h}{2}\varpi_1)<D(\tfrac{p-h}{2}\varpi_2)<\cdots <D(\tfrac{p-h}{2}\varpi_N)\quad (N= \lfloor\tfrac{p}{2}\rfloor-1).$$
As the ratio $E_\ell=D(\tfrac{p-h}{2}\varpi_{\ell+1})/D(\tfrac{p-h}{2}\varpi_\ell)$ is expressed as 
\begin{align*}
    E_\ell=\prod_{i=2\ell+2}^{2n-2\ell-1}\frac{\cos(\tfrac{-n+i}{p}\pi)}{\sin(\tfrac{i}{p}\pi)},
\end{align*}
it suffices to show
\begin{align*}
    \frac{E_\ell}{E_{\ell+1}}=\underbrace{\frac{\cos(\tfrac{-n+2\ell+2}{p}\pi)^2}{\sin(\tfrac{2\ell+2}{p}\pi)\sin(\tfrac{-2n+2\ell+2}{p}\pi)}}_{A} \underbrace{\frac{\cos(\tfrac{-n+2\ell+3}{p}\pi)\cos(\tfrac{-n+2\ell+1}{p}\pi)}{\sin(\tfrac{2\ell+3}{p}\pi)\sin(\tfrac{-2n+2\ell+1}{p}\pi)}}_{B}>1,\quad E_N>1.
\end{align*}
By using the inequality
\begin{align*}
    \sin (z+\varepsilon_1)\sin (z-\varepsilon_1)>\sin (z+\varepsilon_2)\sin (z-\varepsilon_2),\quad (0\leq \varepsilon_1 <\varepsilon_2<\tfrac{\pi}{2}),
\end{align*}
and addition formulae of  trigonometric functions, we obtain straightforwardly  
$$A>1,\quad B>1, \quad E_N>1,$$
respectively. Thus $D(\tfrac{p-h}{2}\varpi_i)>1$ for $1\leq i<n$ and $p\geq 2n+3$. This completes the proof.
\end{proof}
\begin{comment}
The central charge of the $\W$-superalgebra $c(\g,k,f)$ is given by 
\begin{align*}
c(\g,k,f)=\dim \g_0-\half\dim \g_{1/2}-\frac{12}{k+h^\vee}|\rho-(k+h^\vee)x_\circ|^2
\end{align*} 
where $x_\circ\in \mathfrak{h}$ such that it defines the \emph{symmetric} good grading.
...
\hline
& central charge & non-deg adm. levels & Irr($\W_k$) \\  \hline  \hline
$\W^k(\sll_n)$& $(n-1)(1-n(n+1)\tfrac{(k+n-1)^2}{k+n})$ &$k=-n+\frac{p+n}{q+n}$ & $\frac{P_+^{p}(\sll_n)\times P_+^q(\sll_n)}{\Z_n}$\\ \hline
$\W^k(\so_{2n+1})$& $-n\frac{(-3+2k+2kn+4n^2)(k-2n+2kn+4n^2)}{k+2n-1}$ & &\\ \hline
$\W^k(\symp_{2n})$& $-n\frac{(k+2n+2kn+2n^2)(-3-2k+4kn+4n^2)}{k+n+1}$ & &\\ \hline
$\W^k(\so_{2n})$& $-n\frac{(5 - 2 k - 10 n + 2 k n + 4 n^2) (4 - k - 8 n + 2 k n + 4 n^2)}{k+2n-2}$& &\\ \hline
$\W^k(\osp_{1|2n})$& $-\frac{(1+2n)(1+4kn+4n^2)(-1-2k-2n+4kn+4n^2)}{2(1+2k+2n)}$ & &\\ \hline\hline
$\W^k(\so_{2n+1},\fsub)$& $kn\left(\frac{2n+1}{k+2n-1}-2(n-1)(2n-1)\right)$ & &\\ 
&$-2(n-1)(2n-1)(2n^2-3n-1)$& &\\ \hline
$\W^k(\symp_{2n},\fsub)$& $kn\left(\frac{2n+1}{k+n+1}-2(2n-3)^2-4\right)$ & &\\
& $-2(n-1)(4(n-2)^3+45-53n+16n^2)$& &\\ \hline
$\W^k(\so_{2n},\fsub)$& $kn\left(\frac{2n-1}{k+2n-2}-2(13-9n+2n^2)\right)$ & &\\ 
& $-2(n^2(4n^2-26n+57)-(35n+13))$& &\\ \hline
\end{tabular}
%\end{adjustwidth}
\end{table}
\end{comment}
...
\bibitem{ACG} D.~Adamović, T.~Creutzig, N.~Genra, 
 ...
 \doi{10.1007/s00208-023-02634-6}{{\it Math. Ann.}, 2023}. 
...
\bibitem{AMP} D.~Adamovi\'{c}, P.~Möseneder Frajria, P.~Papi,
 ...
 \doi{10.1093/imrn/rnad138}{{\it Int. Math. Res. Not.}, 2023}.
...
%\bibitem{CKM2} T.~Creutzig, S.~Kanade, R.~McRae,
% Gluing vertex algebras,
% \doi{10.1016/j.aim.2021.108174}{{\it Adv. Math.}, {\bf 396} 108174, 2022}.

\bibitem{CKM1} T.~Creutzig, S.~Kanade, R.~McRae,
 Tensor categories for vertex operator superalgebra extensions,
 to appear in {\it Mem. Am. Math. Soc.}, \arxiv{1705.05017}{arXiv:1705.05017 [math.QA]}.

%$\bibitem{CG} T.~Creutzig, D.~Gaiotto, 
% Vertex Algebras for S-duality, 
% \doi{10.1007/s00220-020-03870-6}{{\it Commun. Math. Phys.}, \textbf{379}(3), 785-845, 2020}.

%\bibitem{CL5} T.~Creutzig, P.~Gao, A.~R.~Linshaw,
% A commutant realization of $W^{(2)}_n$ at critical level
% \doi{10.1093/imrn/rns229}{\it{Int. Math. Res. Not.}, \textbf{3} 577--609, 2014}.

%\bibitem{CGL} T.~Creutzig, N.~Genra, A.~R.~Linshaw,
% Category $\mathcal O$ for vertex algebras of $\mathfrak{osp}_{1|2n}$,
% \arxiv{2203.08188}{arXiv:2203.08188 [math.RT]}.
...
%\bibitem{CL4} T.~Creutzig, A.~R.~Linshaw,
% The super $W_{1+\infty}$ algebra with integral central charge,
% \doi{10.1090/S0002-9947-2015-06214-X}{\it{Trans. Am. Math. Soc.}, \textbf{367}(8) 5521--5551, 2015}.
...
\bibitem{Fas21} J.~Fasquel,
 ...
 \arxiv{2210.15728}{arXiv:2210.15728 [math-ph]}.
...
\bibitem{Fe23} Z.~Fehily,
 ...
 \arxiv{2306.14673}{arXiv:2306.14673 [math.QA]}.
...
\bibitem{FF1} B.~Feigin, E.~Frenkel,
 ...
 \doi{10.1155/S1073792891000119}{{\it Int.~Math.~Res.~Not., IMRN}, (6) 75--82, 1991}.
...
%\bibitem{FG} E.~Frenkel, D.~Gaiotto,
% Quantum Langlands dualities of boundary conditions, $D$-modules, and conformal blocks,
% \doi{10.4310/CNTP.2020.v14.n2.a1}{{\it Commun. Num. Theor. Phys.}, \textbf{14}(2) 199--313, 2020}.
...
%\bibitem{Hua} Y.~Z.~Huang,
% Rigidity and modularity of vertex tensor categories,
% \doi{10.1142/S0219199708003083}{{\it Commun. Contemp. Math.}, {\bf 10}(1) 871--911, 2008}.

\bibitem{HL} Y.~Z.~Huang, J.~Lepowsky, 
 A theory of tensor products for module categories for a vertex operator algebra I, II, III,
 \doi{10.1007/BF01587908}{{\it Selecta Math. (N.S.)}, {\bf1}(4) 699--756, 1995}, \doi{10.1007/BF01587909}{{\it Selecta Math. (N.S.)}, {\bf1},(4) 757--786, 1995}, \doi{10.1016/0022-4049(95)00049-3}{{\it J. Pure Appl. Algebra}, {\bf100}(1-3) 141--171, 1995}.
