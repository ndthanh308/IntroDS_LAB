...
\institute{C. Coelho \and M. Fernanda P. Costa \and L.L. Ferrás \at Centre of Mathematics (CMAT), University of Minho, Braga, 4710-057, Portugal \\
cmartins@cmat.uminho.pt, mfc@math.uminho.pt, lferras@fe.up.pt
\and
L.L. Ferrás \at Department of Mechanical Engineering (Section of Mathematics) - FEUP, University of Porto, Porto, 4200-465, Portugal \\
}
...
\keywords{Neural ODEs \and Constrained Optimisation \and Natural Systems.}
\subclass{68T07 \and 90C99}
...
To address the issue of selecting an appropriate $\mu$, adaptive penalty functions have been proposed \textcolor{blue}{in the field of constrained optimisation}. These functions are designed to dynamically adjust penalty parameters during the optimisation process \cite{aliPenaltyFunctionbasedDifferential2013,costaTheoreticalPracticalConvergence2017}. 

\textcolor{blue}{In \cite{aliPenaltyFunctionbasedDifferential2013} the authors introduce an innovative adaptive penalty function-based differential evolution algorithm aimed at tackling constrained global optimisation problems. In \cite{costaTheoreticalPracticalConvergence2017} the authors develop a self-adaptive penalty function and integrate it with a population-based meta-heuristic, resulting in the hybrid self-adaptive penalty firefly algorithm. This hybrid algorithm is designed specifically to address challenges posed by nonsmooth and nonconvex constrained optimisation problems.}
...
\textcolor{blue}{To the best of our knowledge this is the first time a self-adaptive penalty function is proposed to incorporate prior knowledge constraints into a NN architecture, more specifically into a Neural ODE.}

This paper is organised as follows. Section \ref{sec:background} provides a brief introduction to Neural ODEs and penalty methods. Section \ref{sec:method} presents the new self-adaptive penalty function and Section \ref{sec:algo} outlines the self-adaptive penalty algorithm for Neural ODEs. Section \ref{sec:experiments} presents the results obtained for some numerical experiments. The conclusions and future work are presented in Section \ref{sec:conclusion}. \textcolor{blue}{Additionally, a detailed description of a dataset developed and introduced for the first time in this work is included in Appendix.}
...
To define $\phi$, the objective function $l$ and the constraint violation values at each point $\boldsymbol{\theta}$ are normalised using a bounded function $\psi$, to ensure that all values have the same order of magnitude. In this work, we propose the following normalisation function 
...
\noindent where $0\leq \psi(x) \leq 1$, for all $x \in \mathbb{R}_0^+$. \textcolor{blue}{The design and selection of this function was carefully conducted to ensure a range of values that fosters a stable training process. During the design of this self-adaptive function, various well-known functions were considered, such as sigmoid, hyperbolic tangent, and softmax. Nevertheless, these functions have a tendency to saturate when confronted with extremely large or small input values, causing their derivatives to approach zero. Consequently, this saturation behaviour can impede effective learning from such inputs and potentially lead to slower or stalled learning. In contrast, \eqref{eq:normalise} exhibits dissimilar saturation tendencies for both large and small input values. This distinctive characteristic allows its derivative to maintain a non-zero value, enabling the network to continue learning from these inputs without obstruction. As a result, opting for this function as your loss term could potentially yield enhanced performance, particularly when dealing with substantial or widely varying input values.}
...
\noindent In this work we consider $l=|\boldsymbol{y}_t - \hat{\boldsymbol{y}}_t|_{t=t_1,\dots,t_N}.$
...
$$\boldsymbol{v}^i = |c_t^i(\boldsymbol{\hat{y}}_t)|_{t=t_1\dots t_N}.$$
...
$$\boldsymbol{v}^j = |[c_t^j(\boldsymbol{\hat{y}}_t)]^+|_{t=t_1,\dots, t_N}.$$
...
\paragraph{\textcolor{blue}{Proof of equivalence}}
...
\paragraph{\textcolor{blue}{Enhancement of Interpretability}}
...
This work proposes a self-adaptive penalty algorithm for Neural ODEs to enable modelling of constrained natural systems. The proposed self-adaptive penalty function can dynamically adjust the penalty parameters
 ...
 Our approach is an improvement over traditional penalty methods which require an appropriate initialisation and tuning of penalty parameters $\mu$ during the optimisation process. This selection is challenging and time-consuming. In general, in the context of NNs, a fixed penalty parameter is used, and consequently an optimal solution is not guaranteed to be found.
...
The authors acknowledge the funding by Fundação para a Ciência e Tecnologia (FCT) (Portuguese Foundation for Science
...
\section*{Appendix: Developed Dataset} \label{app:DHO}

\paragraph{\textcolor{blue}{Development of the Damped Harmonic Oscillator Dataset}}
...
\begin{equation}
    \begin{cases}
        m \frac{d^2x(t)}{dt^2} + c \frac{dx(t_n)}{dt} + kx(t) = 0, \\
        x(0) = x_0 = 1, \\
        \dfrac{dx(0)}{dt} = \dfrac{dx_0}{dt} = 0.
    \end{cases}
\end{equation}
}\textcolor{blue}{
\noindent where $x(t)$ is the displacement, $v(t)=\frac{dx(t)}{dt}$ is the velocity and $a(t)=\frac{d^2x(t)}{dt^2}$ the acceleration, at time step $t=t_n$ with $n \in 1,\dots,N$, of a mass, $m$, of 1kg in a mass-spring-damper system with a spring constant $k$ of 1N/m and a damping coefficient $c$ of 0.1 Ns/m.
The oscillator is initially displaced by 1m and given an initial velocity of 0m/s.
}\textcolor{blue}{
This dataset translates a physical system governed by physical laws thus it possesses two prior knowledge constraints that must be satisfied at all time steps. The energy $E(t_n)$ of the system decreases through time until the mass comes to rest,
}\textcolor{blue}{
$$E(t) > E(t+1),$$
}\textcolor{blue}{
\noindent with $E(t)= \dfrac{1}{2} *m* (v(t))^2 + \dfrac{1}{2} * k * (x(t))^2$.
}\textcolor{blue}{
The rate of dissipation of energy is the same through time,
}\textcolor{blue}{
$$\Delta P = 0,$$
}\textcolor{blue}{
\noindent with $P(t) = -c * v(t) * x(t).$
}\textcolor{blue}{
The time series data in the DHO dataset includes various parameters characterising the oscillator's motion: time-step $t=t_n$, displacement $x(t)$, velocity $v(t)$ and acceleration $a(t)$. To generate the dataset, we solved the IVP using the Runge-Kutta method of order 5 of Dormand-Prince-Shampine with varying time intervals and sampling frequencies to accommodate the three different experiments. The details of the available data for each experiment are as follows:
\begin{itemize}
