...
\begin{abstract}
In this work we introduce a new method for manufacturing minimal submanifolds in Riemannian geometry. For this we employ the so called {\it complex-valued eigenfunctions}. This is particularly interesting in the cases when the  Riemannian ambient manifold is compact.
\end{abstract}
	
%\dedicatory{version 2.033 - \today \ - current editor: SG+TM}
...
In Riemannian geometry the study of minimal submanifolds plays a central role.  In many cases these submanifolds are either surfaces of dimension two or hypersurfaces of codimension one.  In this work we are interested in constructing minimal submanifolds of codimension two. For this we employ our new method of {\it complex-valued eigenfunctions}. It has recently been shown that these exist on all the classical Riemannian symmetric spaces, see \cite{Gud-Sak-1}, \cite{Gud-Sak-2}, \cite{Gud-Sif-Sob-2}, \cite{Gha-Gud-4} and \cite{Gha-Gud-5}.
\smallskip

Complex-valued harmonic morphisms from Riemannian manifolds are a special case of eigenfunctions.  They have successfully been utilised for the construction of minimal foliations of codimension two.  In this work we extend the investigation to complex-valued eigenfunctions and provide a new method for constructing multi-dimenional families of minimal submanifolds. 
...
The case of $(M,g)$ being a compact Riemannian symmetric spaces is of particular interest.  We construct multi-dimensional families of compact minimal submanifolds in several important cases.
...
Let $\cn P^n$ be the standard $n$-dimensional complex projective space. For a fixed integer $1\le\alpha < n+1$ and $1\le j\le\alpha < k\le n+1$  define the function $\phi_{jk}:\cn P^n\to\cn$ by
...
In the spirit of Theorem \ref{theorem:Bai-Eel}, Baird and Gudmundsson later established the following result of Theorem \ref{theorem-Bai-Gud-1}.  This plays a central role in this work.  In the next section we will explain the notions and background and provide a new proof avoiding the {\it stress-energy tensor} employed in \cite{Bai-Gud-1}.
...
From now on, we will turn our focus on the complex-valued eigenfunctions $\phi:(M,g)\to\cn$ with $(\lambda,\mu)\neq(0,0)$. The existence of these objects has been proven in several important cases, for example when $(M,g)$ is a classical Riemannian symmetric space, see the articles \cite{Gud-Sak-1}, \cite{Gud-Sak-2}, \cite{Gud-Sif-Sob-2}, \cite{Gha-Gud-4} and \cite{Gha-Gud-5}.
\smallskip

\begin{definition}
Let $\phi:(M,g) \to (N,h)$ be a map between Riemannian manifolds. We say that $\phi$ is a \emph{$p$-harmonic map} if it is a critical point of the $p$-energy functional,
...
A map $\phi: (M,g) \to (N,h)$ between Riemannian manifolds is $p$-harmonic if and only if it satisfies the Euler-Lagrange equation
...
\begin{eqnarray*}
|d\phi|^2
&=& \sum_{i=1}^n h(\phi (\lambda \hat{Z}_i), \phi (\lambda \hat{Z}_i))\\
...
\begin{example}
Let $S^{2n-1}$ be the odd-dimensional unit sphere in the standard Euclidean $\cn^n\cong\rn^{2n}$ and $A\in\cn^{n\times n}$ be a complex matrix which is invertible i.e. $\det A\neq 0$.  Further define the function $\Phi:S^{2n-1}\to\cn$ with
$$\Phi:z=(z_1,z_2,\dots,z_n)\mapsto \frac 1{|z|^2}\cdot\Big(\sum_{k\neq l}^na_{kl}\, z_kz_l+\tfrac 12\cdot\sum_{k=1}^na_{kk}\,z_k^2\Big).$$
If $p\in S^{2n-1}$ is a element of the unit sphere then for $k=1,2,\dots,n$ we have 
\begin{equation}\label{equation-sphere-1}
	\frac{\partial\hat\Phi}{\partial z_k}(p)= \frac{\partial P}{\partial z_k}(p)
	-\bar p_k\cdot P(p)
\end{equation}
and
\begin{equation}\label{equation-sphere-2}
	\frac{\partial\hat\Phi}{\partial \bar z_k}(p)= 
	\frac{\partial P}{\partial \bar z_k}(p)
	-p_k\cdot P(p)=-p_k\cdot P(p).
\end{equation}

Let us now assume that $p\in S^{2n-1}$ is a critical point of $\Phi$.  If $p$ is not in the fibre $\Phi^{-1}(\{0\})$ then  equation (\ref{equation-sphere-2}) shows that $p=0\notin S^{2n-1}$.  On the other hand if the critical point $p\in\Phi^{-1}(\{0\})$ then we see from equation (\ref{equation-sphere-1}) that for $k=1,2,\dots ,n$, we have 
$$\frac{\partial \hat\Phi}{\partial z_k}(p)=\frac{\partial P}{\partial z_k}(p)=a_{k1}\, p_1+\cdots +a_{kn}\, p_n.$$
This shows that $p$ is a critical point for $\Phi$ if and only if $A\cdot p=0$.  Since $\det A\neq 0$ this implies that $p=0\notin S^{2n-1}$. 

These considerations show that the compact fibres of the function $\Phi:S^{2n-1}\to\cn$ form a foliation on the unit sphere of codimension two. From Example \ref{example-basic-sphere} and Theorem \ref{theorem-polynomials}, $\Phi$ is an eigenfunction on $S^{2n-1}$.  According to Theorem \ref{theorem-main} the compact submanifold $\Phi^{-1}(\{0\})$ is minimal in $S^{2n-1}$.
\end{example}
...
Let $a_1,a_2,\dots,a_n\in\cn$ be non-zero complex numbers. 
It then follows from Theorem \ref{theorem-polynomials} and Example \ref{example-basic-projective-space} that $\Phi:\cn P^{2n-1}\to\cn$ with 
$$\Phi:[z_1,z_2,\dots,z_{2n}]\mapsto \frac 1{|z|^2}
\cdot\big( a_1\cdot z_1\bar z_{n+1}+\dots +a_n\cdot z_n\bar z_{2n}\big) .$$
is an eigenfunction on the complex projective space $\cn P^{2n-1}$. This clearly lifts to $\hat\Phi=\pi\circ\Phi$, on the $(4n-1)$-dimensional unit sphere $S^{4n}$ in $\cn^{2n-1}\cong\rn^{4n}$, via the standard submersive Hopf fibration $\pi:S^{4n-1}\to\cn P^{2n-1}$.  Then we can use the machinery developed earlier in this section for the lift $\hat\Phi$.

If $p\in S^{2n-1}$ is a element of the unit sphere then for $k=1,2,\dots,n$ we have 
\begin{equation}\label{equation-sphere-3}
\frac{\partial\hat\Phi}{\partial z_k}(p)= \frac{\partial P}{\partial z_k}(p)	-\bar p_{k}\cdot P(p)
=a_k\cdot\bar p_{n+k}-\bar p_{k}\cdot P(p)
\end{equation}
and
\begin{equation}\label{equation-sphere-4}
\frac{\partial\hat\Phi}{\partial \bar z_{n+k}}(p)= 
\frac{\partial P}{\partial \bar z_{n+k}}(p)
-p_{n+k}\cdot P(p)=a_k\cdot p_k-p_{n+k}\cdot P(p).
\end{equation}
...
For a point $p\in S^{4n-1}$ contained in the compact fibre $\hat\Phi^{-1}(\{0\})$ we then have, for $k=1,2,\dots ,n$, that 
$$\frac{\partial \hat\Phi}{\partial z_k}(p)=\frac{\partial P}{\partial z_k}(p)=a_k\cdot\bar p_{n+k}\ \ \text{and}\ \ 
\frac{\partial\hat \Phi}{\partial \bar z_{n+k}}(p)=\frac{\partial P}{\partial \bar z_{n+k}}(p)=a_k\cdot p_k.$$
This implies that $\nabla\Phi(p)\neq 0$ along the fibre $\Phi^{-1}(\{0\})$ over the origin $0\in\cn$.  According to the implicit function theorem this fibre is a submanifold of $\cn P^{2n-1}$ of codimension two.  Furthermore Theorem \ref{theorem-main} tells us that it is minimal.
\end{example}
...
\begin{example}
For a positive integer $d\in\zn^+$ and non-zero complex numbers $a_{1},\dots, a_{n}\in\cn$, let $\Phi:\U n\to\cn$ be the complex-valued function with 
$$\Phi:z\mapsto a_1\cdot z_{11}^d+a_2\cdot z_{12}^d+\cdots +a_n\cdot z_{1n}^d.$$
Then it follows from Proposition \ref{proposition-U(n)-eigen} and Theorem \ref{theorem-polynomials} that the function $\Phi$ is an eigenfunction on $\U n$.  For a point $z\in\U n$ the following derivatives satisfy 
$$
iD_{1}(\Phi)=i\,a_1\, d \cdot z_{11}^d,\,\dots\, ,
iD_{n}(\Phi)=i\,a_n\, d \cdot z_{1n}^d.
$$
Since the first row $(z_{11},z_{12},\dots,z_{1n})$ can not vanish, at least one of these derivatives, and hence the gradient $\nabla\Phi$, does not vanish along the unitary group $\U n$.  This implies that the compact fibres of $\Phi:\U n\to\cn$ form a foliation $\F$ of codimension two.  According to Theorem \ref{theorem-main} the fibre $\Phi^{-1}(\{0\})$ is a minimal submanifold of $\U n$.
\end{example}
...
$$(z+jw)\mapsto q=\begin{pmatrix}z & w \\ -\bar w & \bar
	z\end{pmatrix}.$$ 
...
Adv. Geom. (2023).\newline  https://doi.org/10.1515/advgeom-2023-0015
