...
Kaon physics is not as reach as $B$ physics but still has a number of stars
...
on the next-to-leading (NLO) strong interaction (QCD) corrections to rare decay branching ratios for $\kpn$, $\klpn$,
...
most recent
review with many anecdotes that will  be soon  published in Physics Reports
...
are in a good shape. I refer to FLAG reports \cite{FlavourLatticeAveragingGroupFLAG:2021npn} and in particular to the article of Aida El-Kadra in this volume, where the references to the reach literature can be found. Some references will be given in the context of our presentation.
...
the article of Aida El-Kadra in this volume.
...
\mathcal{B}(\klpn)_\text{SM}\le 3.0\times 10^{-9}, 
\ee
...
other papers, but also the most recent formfactors from HPQCD lattice collaboration \cite{Parrott:2022rgu,Parrott:2022zte}.
...
A detailed analysis of $K$ physics without NP physics in $\varepsilon_K$   in a simple $Z^\prime$ model has been presented
...
This is not possible with a single $Z^\prime$ gauge boson with only flavour-violating left-handed couplings. At present it appears that that there are three possible strategies to solve this problem.
\begin{itemize}
\item
...
transitions. Using our strategies together with $B\to K$ formfactor from HPQCD \cite{Parrott:2022zte,Parrott:2022rgu,Parrott:2022smq} we find first \cite{Buras:2022qip}
\be\label{BVNEW}
...
does not include $10\%$ upward shift from a tree-level contribution pointed out in \cite{Kamenik:2009kc}. It should be included when the data improves.
...
\begin{align}
{\mathcal{B}}(B^+\to K^+\nu\bar\nu) &\leq 1.3\times 10^{-5}\quad \text{@ 90\% CL}~,\\
...
\cite{Bause:2021cna,He:2021yoz,Bause:2022rrs}.
...
in the present case, one finds  anti-correlation between $\nu\bar\nu$ and $\mu\mu$ channels. The observed suppresion of  $\mathcal{R}_{K\mu\mu}$ and $\mathcal{R}_{K^*\mu\mu}$ below unity implies then the enhancements of  $\mathcal{R}_{K\nu\nu}$ and $\mathcal{R}_{K^*\nu\nu}$  above unity. A detailed updated numerical analysis
of these correlations should appear soon \cite{Buras:2023yyy}.



\subsection{Cabibbo Anomaly and the Unitarity of the CKM Matrix}
There was recently a large activity in connection with the so-called Cabibbo
anomaly (CAA) which is nicely reviewed in  \cite{Crivellin:2022ctt}.
...
unitarity in the CKM matrix. There exist also tensions between determinations of $\vus$ from leptonic $K\mu2$ and semileptonic $Kl3$ kaon decays.
In the end a $3\,\sigma$ deficit in the CKM unitarity
 ...
 Let us illustrate this problematic repeating the 1974 Gaillard-Lee
...
\begin{itemize}
 \item While the vector contribution follows the shape of the SM contribution, the scalar contribution features a distinct distribution, clearly different from the vector one, in particular for larger $s$. Even within the current possible range for NP  (limited by the current experimental upper limit $\mathcal{B}(\kpn)=1.55 \times 10^{-10}$), the difference is strong enough to be visible in the distribution.
  \item Interestingly, we find that the scalar differential distribution $\Gamma(\kpn)/ds$ turns out to be independent of the scalar phase $\phi_S$, while $\mathcal{B}(\klpn)$ is. Hence, for different  $\phi_S$, $\Gamma(\kpn)/ds$ does not change, while $\mathcal{B}(\klpn)$ does. While it is maximal for $\phi_S=0$, it follows the SM line for $\phi_S=\pi/2$. Therefore, a combined analysis of the differential distributions for $\Gamma(\kpn)$ and $\mathcal{B}(\klpn)$ is very powerful. In case $\Gamma(\kpn)$ would give a hint towards a new scalar contribution, a comparison with $\Gamma(\klpn)$ could tell us if it features a non-zero scalar phase $\phi_S$. 
\end{itemize}
...
Soon after the discovery of asymptotic freedom  in 19973 Altarelli and Maiani \cite{Altarelli:1974exa} and Gaillard and Lee \cite{Gaillard:1974nj} made a first unsuccessful attempt to explain this  huge enhancement 
...
the
short-distance  RG-evolution of WCs from scales ${\cal O}(M_W)$
...
freedom and related non-factorizable QCD interactions are primarly responsible for this rule. 
 It is simply the
...
We have just mentioned  WET and SMEFT in the context of $\epe$ but it should
...
%
\item
  The one-loop ADMs  relevant for the RG in WET \cite{Jenkins:2017dyc,Aebischer:2017gaw} and SMEFT \cite{Jenkins:2013zja,Jenkins:2013wua,Alonso:2013hga} are
  also known.
\item
...
In view of many operators involved, it was crucial to develop sophisticates computing tools
...
will be available. For recent updates see \cite{Greljo:2022jac,Alguero:2023jeh}.
...
list for exciting time we spent together.  
