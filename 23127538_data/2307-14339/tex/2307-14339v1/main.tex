...
\author{Marco Bonici$^{\star1}$}
\author{Federico Bianchini$^{\dagger2, 3, 4}$}
\author{Jaime Ruiz-Zapatero$^{\ddagger 5}$}
\affiliation{$^1$ INAF-IASF Milano IT}
\affiliation{$^2$ Kavli Institute for Particle Astrophysics and Cosmology, Stanford University, 452 Lomita Mall, Stanford, CA, 94305, USA}
\affiliation{$^3$SLAC National Accelerator Laboratory, 2575 Sand Hill Road, Menlo Park, CA 94025}
\affiliation{$^4$Department of Physics, Stanford University, 382 Via Pueblo Mall, Stanford, CA 94305}
\affiliation{$^5$Astrophysics, University of Oxford, DWB, Keble Road, Oxford OX1 3RH, UK}
\thanks{$^\star$ E-mail: \nolinkurl{marco.bonici@inaf.it} \\
$\dagger$ E-mail: \nolinkurl{fbianc@stanford.edu} \\ $\ddagger$ E-mail: \nolinkurl{jaime.ruiz-zapatero@physics.ox.ac.uk}}

\begin{abstract}
We present \capse{}, a novel emulator that utilizes neural networks to predict Cosmic Microwave Background (CMB) temperature, polarization and lensing angular power spectra. The emulator computes predictions in just a few microseconds with emulation errors below $0.1\sigma$ for all the scales relevant for the planned CMB-S4 survey. \capse{} can also be trained in an hour's time on a CPU.
As a test case, we use \capse{} to analyze \planck{} 2018 data and ACT DR4 data. We obtain the same result as standard analysis methods with a computational efficiency 3 to 6 order of magnitude higher. We take advantage of the differentiability of our emulators to use gradients-based methods, such as Pathfinder and Hamiltonian Monte Carlo (HMC), which speed up the convergence and increase sampling efficiency. 
...
Depending on the dimensionality and the topology of the parameter space, as well as the desired numerical accuracy of the Boltzmann solver, this operation is typically repeated $10^4$-$10^6$ times before convergence thresholds are reached, therefore representing a main hurdle in standard Monte Carlo Markov Chain (MCMC) analyses.
...
Additionally, \cite{nygaard_connect_2022} adopted an iterative approach for emulator training to reduce the overall computational resources required.
Currently, the most advanced CMB emulator, covering the widest parameter volume with the best performance is \cosmopower{}~\citep{mancini_itcosmopower_2021, bolliet_high-accuracy_2023}, which directly emulates the CMB angular power spectra.

The cosmological community also developed emulators tailored for LSS analysis, with emulators for the linear matter power spectrum~\citep{mancini_itcosmopower_2021, donald-mccann_textttmatryoshka_2021, arico_accelerating_2021}, nonlinear corrections~\citep{Lawrence_2017, DeRose_2019, euclid_collaboration_euclid_2019, euclidcollaboration_euclid_2021, angulo_bacco_2021}, power spectrum multipoles~\citep{arico_accelerating_2021, eggemeier_comet_2022, donald-mccann_textttmatryoshka_2022} and galaxy survey angular power spectrum~\citep{manrique-yus_euclid-era_2019, mootoovaloo_parameter_2020, bonici_fast_2022}. The amount of papers employing emulators is rapidly growing, demonstrating an increasing interest of the cosmological community toward this technique.

In this paper, we present a novel emulator of the CMB temperature, polarization, and lensing potential anisotropies angular power spectra, \capse{}, which differs from previously developed emulators in two fundamental ways. On the one hand, we propose and implement more efficient preprocessing, \textit{i.e.} the operations performed on the training data before being fed to the NN for the training, and  test a different technique for data compression. On the other hand, we employ a specialized, performant NN framework in \julia{}.
Moreover, \capse{} can be trained in an hour's time on a CPU to cover a wide range of cosmological parameters, and down to scales that will be mapped by upcoming experiments. This makes \capse{} the fastest CMB power spectra emulator currently in the market as well as extremely cheap to train. In addition, we also implement fully differentiable CMB likelihoods. Coupled to our emulators, these methods enable the use of novel gradient-based inference methods. The combination of these state-of-the-art inference methods with \capse{} allows us to analyze \planck{} data in less than one second and ACT data in  $\sim10$ seconds.
...
Another crucial step is the preprocessing of the training data. In this work, we perform a min-max normalization on each input and output feature, mapping them to the range of $[0,1]$ interval, a procedure known to improve the emulation performance~\citep{nygaard_connect_2022}. 
Furthermore, we leverage domain knowledge, exploiting the dominant linear dependence of the $C_\ell$'s on $A_s$. 
Hence, we divide each output feature for the corresponding value of $A_s$, while leaving it as a free parameter of the NN.
In this way, we account for the linear rescaling in an analytical way, while letting the NN to learn only the (small) nonlinear feature.\footnote{A similar rescaling for $\tau$ could potentially be employed. However, as its effect varies across different power spectra and scales~\citep{Alvarez:2020gvl} and since the desired accuracy was achieved with our current procedure, further optimization for $\tau$ is left for future work.} 
We found that this enhances the emulation accuracy given the same amount of training resources. 
This improvement can be easily explained by the fact that, given the $\ln 10^{10}A_s$ emulation range, these preprocessing steps reduce the range of the output features by almost a factor of 3, hence improving the emulation performance. 
...
The training is performed on a standard laptop with an 8-core CPU using the \texttt{ADAM} optimizer~\citep{kingma_adam_2017} with a mean square error loss function. 
...
\begin{equation}
    ...
    \label{eq:chebyshev}
\end{equation}
where $a_n$ represents the $n$-th coefficient that carries the dependence on the cosmological parameters $\theta$, while $T_n$ denotes the $n$-th Chebyshev polynomial.\footnote{Note that we use a rescaled version of the Chebyshev polynomials since they are defined on the interval $\left[-1, 1\right]$. The rescaling is automatically handled by the Chebyshev projection library employed in our analysis, \href{https://github.com/JuliaMath/FastChebInterp.jl}{\texttt{FastChebInterp.jl}}}
...
The variance can be computed, under the assumption that both $\left[X,\,Y\right]\in\left[{T,\,E,\,\phi}\right]$ behave as gaussian random fields, as:
\begin{equation}
    \begin{aligned}
\sigma_{\ell, \mathrm{CMB}}^{\mathrm{XY}}= & \sqrt{\frac{1}{f_{\mathrm{sky}}(2 \ell+1)}} \\
& \times \sqrt{(C_{\ell}^{\mathrm{XY}} )^2+\left(C_{\ell}^{\mathrm{XX}}+N_{\ell}^{\mathrm{XX}}\right)\left(C_{\ell}^{\mathrm{YY}}+N_{\ell}^{\mathrm{YY}}\right)},
\end{aligned}
...
First and foremost, we only consider cosmic variance over this multipole range since  S4 will not be able to image the largest angular scales at $\ell \lesssim 40$. 
...
If we exclude the $C_\ell$'s with $\ell<10$, the results are even better, with an emulation error that is below $0.08\,\sigma$ for $99\%$ of the points in the validation dataset.

%We leave for a future work a further optimization that would enable use to improve on these results. The current mean square error loss function used to optimize the NN  gives the same weight to all angular scales. However, this is not optimal since the measurement variance is actually an $\ell$-dependent quantity. Using a \textit{weighted} mean square error loss function as in \citet{nygaard_connect_2022}, where each multipole $\ell$ is weighted with the corresponding $\sigma_\ell$, should enable us to reach a more homogeneous emulation error at no additional computational cost. 
...
After providing a detailed description of the \capse{} emulator (refer to Sec.~\ref{sec:capse_description}), our focus shifts to leveraging its CMB power spectra predictions for cosmological inference.
To this end, we use the \julia{} Bayesian statistical inference library \turing{}~\citep{ge2018t}. 

The key feature of Bayesian inference is that it  allows for a principled way to update and revise our beliefs about the parameters or variables of interest based on new evidence.
Therefore, within a Bayesian framework, the statistical distribution of the parameters of a given model given some observations (known as posteriors) is proportional to the product of what we believe the distribution of the parameters might be prior to observations (known as priors) and the so-called likelihood, a function that captures how well our theoretical predictions fit the data for a given set of parameter values. 
Mathematically, this is expressed through the Bayes theorem: 
\begin{equation} \label{eq. Bayes}
    \mathcal{P}(\boldsymbol{\theta}|\boldsymbol{d}) \propto \mathcal{L}(\boldsymbol{d}|\boldsymbol{\theta}) \Pi(\boldsymbol{\theta}) \, ,
\end{equation}
where $P(\boldsymbol{\theta}|\boldsymbol{d})$ represents the posterior distribution of the model parameters $\boldsymbol{\theta}$ given the data $\boldsymbol{d}$, $\mathcal{L}(\boldsymbol{d}|\boldsymbol{\theta})$ denotes the likelihood function and $\Pi(\boldsymbol{\theta})$ the priors. 
...
In this work, we use the distributions listed in Tab. \ref{tab:emulation_space} as priors for the input parameters of the \capse{} emulator. The only exception is $\tau$, for which we employ a gaussian prior, stemming from the analysis of low-$\ell$ $EE$ angular spectrum.
Moreover, since both \planck{} and ACT measurements are Gaussian-distributed (see Sec.~\ref{sec:planck} and \ref{sec:ACT}), we use \turing{} to draw a Gaussian likelihood of the shape:
\begin{equation}
...
\section{Samplers}
\label{sec:samplers}
After presenting the likelihoods in Sec.~\ref{sec:likelihood}, we will now outline the methods employed to explore these likelihoods and derive parameter constraints for the \planck{} and ACT datasets. 
...
As advertised in Sect. \ref{sec:accuracy_tests}, we start by testing the accuracy of our emulators performing the MCMC analysis of two different datasets, the \planck{}~\citep{aghanim_planck_2020} and ACT DR4~\citep{aiola_atacama_2020} datasets. The analyses are performed using both \camb{} and \capse{} and the obtained posteriors are compared in order to assess the emulator accuracy.
...
In this section, we show the results from the chains based on the ACT  Data Release 4 (DR4) likelihood \citep{aiola_atacama_2020}.\footnote{\url{https://github.com/ACTCollaboration/pyactlike}}
...
The only nuisance parameter that is allowed to freely float is the overall polarization efficiency $y_p$, which linearly and quadratically rescales the $C_{\ell}^{TE}$ and $C_{\ell}^{EE}$ spectra, respectively. Also in this case we have coded a pure \julia{} version of this likelihood, \texttt{ACTPolLite.jl}.\footnote{The code for the \julia{} ACTPol lite likelihood can be found \href{https://github.com/JuliaCosmologicalLikelihoods/ACTPolLite.jl}{here}.}
...
\hline
\end{tabular}
\caption{Mean and 95\% limits of the 1D marginal posteriors of the cosmological parameters obtained from the \planck{} (first half of the table) and ACT (second half of the table) analyses. Different columns show the constraints obtained performing the same analyses using the sampling algorithms MH, NUTS, MCHMC and Pathfinder. The mean and limits have been computed using \texttt{GetDist}.}
...
\begin{tabular}{|c|c|c|c|c|}
\hline
& \multicolumn{2}{c}{\planck{}} & \multicolumn{2}{|c|}{ACT}\\
\hline
& ESS/s & Comp. (s) & ESS/s & Comp. (s) \\
\hline
\camb{}+\cobaya{} & 0.004  & 345,600 & 0.0003 & 1,728,000 \\
\capse{}+NUTS  & 1.597  & 7,014  & 1.914  & 11,813 \\
\capse{}+MCHMC  & 3.686  & 4,374  & 4.136  & 7,792 \\
\capse{}+Pathfinder  & N/A  & 15 & N/A  & 16 \\
\hline
\end{tabular}
\caption{Shows the ESS per second and computation time achieved by the \cobaya{}, NUTS, MCHMC and Pathfinder inference algorithms when performing the \planck{} (2nd and 3rd columns) and ACT (4th and 5th columns) analyses. Note that \cobaya{}  used the cosmological code \camb{} while the other samplers were paired with the \capse{} emulator.}
\label{tab:performance}
\end{table}

After validating our emulators, let us now focus on the different inference algorithms at our disposal and their impact in the speed, accuracy and robustness of the analyses.

The compatibility of the \capse{} emulating framework with AD methods make it extremely synergistic with a plethora of gradient-based samplers. In this section we compare the performance of three gradient-based samplers (see Sec.~\ref{sec:samplers} for details). First, we compare the state-of-the-art NUTS sampler used to obtain our fiducial results in Sects.~\ref{sec:planck} and \ref{sec:ACT} with the recently developed MCHMC sampler. Second, we compare NUTS against a VI method, Pathfinder\footnote{We use the publicly available \julia{} implementations of \href{https://github.com/TuringLang/AdvancedHMC.jl}{NUTS}, \href{https://github.com/JaimeRZP/MicroCanonicalHMC.jl}{\texttt{MicroCanonicalHMC.jl}} and \href{https://github.com/mlcolab/Pathfinder.jl}{\texttt{Pathfinder.jl}}.}. In all cases we use the same likelihoods described in Sects.~\ref{sec:planck} and \ref{sec:ACT} to draw the comparisons. This is possible thanks to the modularity of the \julia{} ecosystem, which allows us to write a likelihood in \turing{} and then sample it using different libraries and algorithms.

\subsubsection{NUTS vs MCHMC}
\label{sec:nuts_vs_mchmc}
Before comparing the performance of the two samplers, it is essential to verify that NUTS and MCHMC return the same posterior distributions. As MCMC methods, both NUTS and MCHMC should asymptotically converge to the same target distribution.  The upper half of Figs.~\ref{fig:contour_planck} and ~\ref{fig:contour_act} show the comparison of the NUTS and MCHMC posteriors for \planck{} and ACT analyses respectively. The corresponding numerical values for the quoted constraints can be found in Tab.~\ref{tab:constraints}. In this table, we observe that the constraints obtained from both NUTS and MCHMC are within a 0.1$\sigma$ distance of their Metropolis-Hastings (MH) counterpart. The excellent agreement between the two samplers indicates that differences are well within the sampling noise.
...
We compute the ESS of both chains using the \texttt{Python} library \texttt{GetDist}. We find an ESS/s of 1.6 for the NUTS sampler, while MCHMC returns an ESS/s of 3.7, as can be seen in Tab.~\ref{tab:performance}. In the case of ACT, NUTS achieves 1.9 ESS/s while MCHMC reaches 4.1. Thus in both analyses we see that MCHMC is twice as fast as NUTS, as expected on the findings of  \cite{robnik2022microcanonical}.\footnote{We would like to emphasize that \texttt{MicroCanonicalHMC.jl} is still an alpha-software which is being actively developed: performance might improve with a more stable and refined implementation of MCHMC within this library.}
...
Despite these caveats, we observe excellent agreement between the posteriors obtained with Pathfinder and NUTS for both the \planck{} and ACT analyses. 
...
In this section we compare the emulators presented in this work to two closely related alternatives in the literature, \classnet{}~\citep{albers_cosmicnet_2019} and the recently developed \cosmopower{}~\citep{mancini_itcosmopower_2021}. \classnet{} emulates the transfer functions required to compute the CMB angular power spetrum using \texttt{Python} based neural network emulators. The result of the emulation is then passed to \texttt{CLASS}, which computes the line-of-sight integral. The main advantage of this approach is that \classnet{} does not need to include $A_s$ and $n_s$ in the emulation space, since the primordial power spectrum is analitycally computed and integrated against the emulated transfer functions. \cosmopower{} is a  \texttt{TensorFlow} based emulator that follows a design philosophy closer to \capse{}, as it directly emulates the CMB angular power spectra. Initially it focused on the \planck{} analysis  but  it was later extended to $\ell_\mathrm{max}=10000$ and the same high-accuracy settings employed here~\citep{bolliet_high-accuracy_2023}. 
...
We report the speedup obtained by the three emulation frameworks when compared to a traditional Boltzmann solver in the first row of Tab.~\ref{tab:comp}. In this table, we can see that \classnet{}, \cosmopower{}, and \capse{} emulators report a speedup of a factor of 3\footnote{We report that in \cite{gunther_cosmicnet_2022} the authors performed a \planck{} data analysis, without focusing on high-precision settings in which they could likely achieve a higher speed-up.}, 1,000, and 1,000,000 respectively. Thus \capse{} is by far the fastest emulator, being 3 orders of magnitudes faster than the second fastest one, \cosmopower{}.
...
\hline
 $\ln 10 A_{\mathrm{s}}$          & N/A        & [2.5, 2.5] & [2.5, 3.5] \\
$n_{\mathrm{s}}$                  & N/A        & [0.88, 1.06] & [0.88, 1.06] \\
$\tau$                            & $N(0.0561, 0.0071)$ & [0.02, 0.12] & [0.02, 0.12] \\
$H_0[\mathrm{km}/s/\mathrm{Mpc}]$ & $N(67.66, 0.42)$ & [40, 100] & [40, 100] \\
$\omega_{\mathrm{b}}$             & $N(0.02242, 0.00014)$ & [0.0193, 0.02533] & [0.0193, 0.02533] \\
$\omega_{\mathrm{c}}$             & $N(0.11933, 0.00091)$ & [0.08, 0.2] & [0.08, 0.2] \\
$\ell_\mathrm{max}$ & 2,500 & 5,000& 10,000\\ 
\hline
\end{tabular}
\caption{A comparison between the emulating frameworks \classnet{}, \cosmopower{}, and \capse{}. Note$^*$ that the architecture of \classnet{} involves 7 different NN with 3 to 4 layers each adding to a total 5366 neurons.}
\label{tab:comp}
\end{table*}
...
Regarding the number of learned features, \classnet{} learns transfer functions on a $k$-grid with $\mathcal{O}(700)$ elements, chosen to accurately compute the \planck{} $C_\ell$'s. In contrast, \capse{} computes all $C_\ell$'s up to $\ell_\mathrm{max}=5,000$, making it compatible with all current CMB experiments, while \cosmopower{} reaches an $\ell_\mathrm{max}$ of 10,000, making it suitable for next-generation CMB experiments.

Considering the aforementioned caveats, training \capse{} requires $10^4$ training points, which is of the same order of magnitude as \classnet{}, and an order of magnitude less than \cosmopower{}~\cite{bolliet_high-accuracy_2023}. In terms of network architecture, \cosmopower{} employs a one-size-fits-all approach with 4 fully connected hidden layers, each containing 512 neurons. \classnet{}, on the other hand, tailors its networks for each emulator, utilizing different architectures with 2-4 hidden layers and 100-500 neurons. \capse{} employs a consistent architecture across all networks, consisting of 5 hidden layers with 64 neurons, making it the emulator framework with the least complicated architecture. In terms of training time, \capse{} takes slightly less than one hour using a CPU, while the training of \cosmopower{} lasted one hour using a GPU. After training, \capse{} is slightly more precise than \cosmopower{} for the $\phi\phi$ coefficients, while \cosmopower{} is 2-3 times more precise for the $TT$, $TE$, $EE$ coefficients.




Regarding flexibility, \classnet{} is potentially the most versatile of the emulators. As previously described, it only emulates the transfer functions and computes the line-of-sight integrals using \texttt{CLASS}. 
This allows the user to change the computation of the primordial power spectrum or the method to compute the line-of-sight integrals without having to retrain the emulator, as it would be the case for \capse{} and \cosmopower{}. The question is then whether such design is ultimately advantageous given that \capse{} and \cosmopower{} are 6 and 3 orders of magnitude faster. The answer to this question lays on how many times the user is expected to have to retrain the emulator. We argue that for CMB analyses such as the one we concern ourselves in this work, directly emulating the $C_l$'s is an optimal strategy since the analysis is unlikely to change. This is specially poignant given the vast trade off in speed a more flexible strategy requires. %However, this does not mean that emulating lower level functions is always sub-optimal. For example, when performing photometric analyses of LSS data we expect the photometric distributions of galaxies as well as the different nuisance parameters to frequently change. In said scenarios emulating more fundamental quantities such as the matter power spectrum can be become a more advantageous strategy given the number of times users might have to retrain the emulators otherwise. 

Finally, we studied the efficiency of sampling algorithms compatible with each emulator. The wall time users will experience depends on both the speed of the emulator and the sampling algorithms available for Bayesian inference.  \classnet{} employs traditional gradient-free sampling methods while \capse{} and \cosmopower{} are compatible with state-of-the-art gradient-based algorithms. The original \cosmopower{}  was written in \texttt{TensorFlow} and, coupled to \texttt{TensorFlowProbability}\footnote{\href{https://www.tensorflow.org/probability}{https://www.tensorflow.org/probability}}, has in principle access to a rich variaty of gradient-based samplers. Moreover, \cosmopower{} was recently re-written in \texttt{JAX}~\citep{Piras_2023} and embedded in the \texttt{JAX-COSMO} framework, explicitly designed for gradient-based sampling methods.
...
When comparing the wall time required to compute converging chains, \classnet{} reports a speedup by a factor of 3. In \cite{mancini_itcosmopower_2021}, a \planck{} analysis was performed in $\mathcal{O}(10\,s)$ using a GPU; however, as can be seen on the official notebook prepared by the authors, the chains were initialized on the typical set, using the knowledge coming from previous analyses. \capse{} is able to perform a \planck{} analysis in around a minute with MCMC methods or $\mathcal{O}(1\,s)$ with variational methods. We emphasize that \capse{}'s runs were performed initializing our algorithms from the prior.

%\label{sec:comparison}
%In this section we compare the emulators presented in this work to two closely related alternatives in the literature, ~\cite{albers_cosmicnet_2019} and the recently developed~\cite{mancini_itcosmopower_2021}. In order to do so we will use several metrics:
%\begin{itemize}
%    \item Computational performance of each emulator in terms of speed and the efficiency of the samplers that have (or might have) been employed
%    \item Training efficiency, the amount of resources employed in the training phase
%    \item the precision and the flexibility of the emulator
%\end{itemize}

%Regarding the computational performance, the \classnet{}, \cosmopower{}, and \capse{} emulators report a speedup, respectively, by a factor of 3\footnote{We report that in \cite{gunther_cosmicnet_2022} the authors performed a \planck{} data analysis, without focusing on high-precision settings, when they could likely achieve a higher speed-up.} , 1,000, and 1,000,000: \capse{} is by far the fastest emulator, being order of magnitues faster than \cosmopower{}. On the sampler side, \classnet{} and \cosmopower{} employ more traditional gradient-free methods, while we use state-of-the-art gradient-based algorithms. Using these methods with \classnet{} would be technically very challenging, since this package is a mixture of \texttt{C} and \texttt{Python}. 

%Using these methods with \cosmopower{}, such as those present in \texttt{TensorFlowProbability}\footnote{\href{https://www.tensorflow.org/probability}{https://www.tensorflow.org/probability}}, should be easily achievable.In fact, in \cite{Piras_2023}, \cosmopower{} was embedded in the \texttt{JAX-COSMO} framework, employing gradient-based methods. However, we point out that being \cosmopower{} and \texttt{JAX-COSMO} written respectively using \texttt{TensorFlow} and \texttt{JAX}, they could not directly operate together: this required to rewrite \cosmopower{} using \texttt{JAX}. Contrary to this \textit{balkanization} of the \texttt{Python} ecosystem, with different DSL which have difficulty in working together, we emphasize that the \texttt{Julia} ecosystem is way more interoperable: although we are using \texttt{SimpleChains.jl}, we might as well use other NN libraries such as \texttt{Flux.jl} or \texttt{Lux.jl}.

%Regarding the training, performing the comparison with \classnet{} is complicated, since it has been trained on an hyperellypsoid, rather than an hypercube; anyway, the amount of training points is 10,000, the same we employed. The comparison with \cosmopower{} is more direct, since we precisely used the same training hypercube and Boltzmann solver accuracy settings. However, we covered the same hypercube with just 10,000 points, a requirement 12 times lower than the one of~\cite{bolliet_high-accuracy_2023}. The training for \capse{} lasted for slightly less than one hour, using a CPU, while the training of \cosmopower{} lasted one hour using a GPU. Regarding the flexibility of the emulators, since \classnet{} emulates the transfer functions and then computes the line-of-sight integrals, it does not include $A_s$ and $n_s$ in the emulator: this represents an advantage compared to \capse{} and \cosmopower{}, since those two parameters do not have any emulation space limit. Regarding the multipole range, \classnet{} has the narrowest one, while \cosmopower{} has the widest, reaching $\ell_\mathrm{max}=10,000$. Regarding the precision of the emulators, \capse{} is slightly more precise than \cosmopower{} for the $\mathrm{PP}$ coefficients, while \cosmopower{} is 2-3 times more precise for the $\mathrm{TT}$, $\mathrm{TE}$, $\mathrm{EE}$ coefficients.
...
While \camb{} paired with \cobaya{}  required around 100 and 480 CPU hours to analyze \planck{} and ACT data respectively, \capse{} combined with the NUTS algorithm required only 0.5 CPU hours for the \planck{} analysis and one CPU hour for the ACT analysis. Thus our analyses were at least two orders of magnitude more efficient.
When comparing NUTS to MCHMC, we found the latter to be more efficient by a factor of 2-3, while producing virtually identical posteriors. Pathfinder was able to perform the previously mentioned analysis in 1-10 s, with a very good precision. However, this approach is less robust when compared to MCMC methods as it lacks asymptotic convergence guarantees.
...
Furthermore, we compared \capse{} to two closely related emulators in the literature, namely \classnet{} and \cosmopower{}. \capse{} outperformed both in terms of speed, being two orders of magnitude faster, while maintaining comparable precision to \cosmopower{}, despite its simpler architecture. Additionally, \capse{} required the same amount of training data points as \classnet{} but covered a significantly larger emulation domain. Moreover, \capse{} trained on a CPU in one hour, whereas \cosmopower{} required a GPU for the same training time.


While our emulators and methods exhibited superior performance compared to existing literature, there remain opportunities for future improvements. 
...
MB is grateful to Martin White for the initial suggestion on employing the Chebyshev polynomial decomposition and to Uros Seljak, Marius Millea, Simone Ferraro, Giulio Fabbian and Emanuele Castorina for useful discussions. MB thanks University of California Berkeley for hospitality during his visit, during which this project was started.
