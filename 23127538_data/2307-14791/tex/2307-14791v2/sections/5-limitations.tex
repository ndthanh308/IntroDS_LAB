...
and iii) no pointer arithmetic is allowed outside the data-structures. These constraints are already enforced for safety reasons in commonly used packet processing framework like eBPF\footnote{NFs developed in eBPF store their state in kernel-maintained maps~\cite{ebpf-maps}.}~\cite{ebpf}, a widely used framework in both academia and industry\footnote{There is an ongoing effort in adapting \maestro to also accept eBPF NFs as input (an effort already set in motion by PIX~\cite{pix}).}~\cite{katran,xdp,cilium,crab,hxdp,xdp-netdev,hda}.
