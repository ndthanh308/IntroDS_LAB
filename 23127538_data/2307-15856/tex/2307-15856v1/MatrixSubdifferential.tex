...
\cite{Papageorgiou,Thera,KusraevKutateladze}, cannot be applied to convex matrix-valued function, since the space of
...
the cooresponding norm $\| \cdot \|_F$, which is called the Frobenius norm. Here $\trace(A)$ is the trace of a square
matrix $A$.

Denote by $\preceq$ the L\"{o}wner partial order on the space $\mathbb{S}^{\ell}$, which as defined as follows:
...
(with respect to the L\"{o}wnder partial order), if
\[
...
\]
Let us check that the function $F$ is convex despite the fact that its nondiagonal elements are nonconvex functions.
...
\]
for some matrices $V_i \in \mathbb{S}^{\ell}$, $i \in \{ 1, \ldots, d \}$ (namely, $V_i = \mathcal{A} e_i$, where $e_i$
is the $i$-th vector from the canonical basis of $\mathbb{R}^d$). Therefore, hereinafter we identify linear operators
...
Let now a sequence $\{ V_n \} \subset \partial F(x)$ converge to some $V \in \mathbb{S}^{\ell}$. By the definition of
...
With the use of the previous lemma we can show that the subdifferential of a differentiable convex matrix-valued
function is, as one can expect, a singleton.
...
\section{Computing subdifferentials: one-dimensional case}
\label{sect:OneDimCase}



In this section we study subdifferential of convex matrix-valued functions in the one-dimensional case, that is, in the
case of functions defined on the real line $\mathbb{R}$. Let a function $F \colon \mathbb{R} \to \mathbb{S}^{\ell}$ be
given. First, we provide a convenient (from the theoretical point of view) characterisation of the convexity of $F$.
...
\]
Hence with the use of \eqref{eq:LoewnerOrderProp} one gets $F(x) - F(x_1) \preceq \alpha ( F(x_2) - F(x_1) )$ or,
equivalently, inequality \eqref{eq:IncreasingSecant} holds true.
...
from the right one obtains that inequalities \eqref{eq:DDInequal} are valid. In turn, passing to limit in
...
any $y > x$ one has $(y - x) F'_+(x) \succeq (y - x) V$. Hence with the of the definition of subgradient and the fact
that $F'_+(x) \in \partial F(x)$ one gets that for any $y > x$ the following inequality holds true:
...
Thus, the points of nonsmoothness of the diagonal elements of a convex matrix-valued function must be consistent
...
convex function and for all $i \in \{ 1, \ldots, \ell \}$ the functions $F_{ii}$ be differentiable in $x_i$ at a point
$x \in \mathbb{R}^d$. Then applying Corollary~\ref{crlr:Smoothness} to the mapping $x_i \mapsto F(x)$ one obtains that
$F$ is differentiable in $x_i$ as well.
\end{remark}



\section{Computing subdifferentials: multi-dimensional case}
...
is locally Lipschitz continuous. Therefore, by Rademacher's theorem $F$ is almost everywhere differentiable and one can
define its Clarke subdifferential (generalised Jacobian) as follows:
\[
...
\]
where $\Omega_F$ is the set of points $x \in \mathbb{R}^d$ at which $F$ is differentiable. From the general properties
of the Clarke subdifferentials/generalised Jacobians of locally Lipschitz mappings (see \cite{Clarke}) it follows that
the Clarke subdifferential of a convex matrix-valued function possesses the following properties. 

\begin{proposition}
For any $x \in \mathbb{R}^d$ the Clarke subdifferential is correctly defined, nonempty, convex and compact.
\end{proposition}
...
to $x$ and such that $F$ is G\^{a}teaux differentiable at each point $x_n$ and $F'(x_n) \to V$ as $n \to \infty$. By
...
\begin{equation} \label{eq:MatrixTransformSubdiff}
  ...
  := \Big\{ (M V^{(1)} M^T, \ldots M V^{(d)} M^T) \in (\mathbb{S}^m)^d \Bigm| V \in \partial F(x) \Big\}
...
\[
  \langle z, \Big(\alpha F(x_1) + (1 - \alpha) F(x_2) - F(\alpha x_1 + (1 - \alpha) x_2)\Big) z \rangle \ge 0 \quad
\forall
  z \in \mathbb{R}^{\ell}.
\]
...
and $\partial F_1(\cdot) + \partial F_2(\cdot) \subseteq \partial (F_1 + F_2)(\cdot)$.
\end{proposition}
...
\[
  ...
  \quad \forall z \in \mathbb{R}^{\ell} \: \forall y \in \mathbb{R}^d,
\]
which means that $V_1 + V_2 \in \partial F(x)$.
\end{proof}
...
$b \in \mathbb{R}^m$. Then the matrix function $G$ is convex and for any $y \in \mathbb{R}^s$ one has
\begin{multline*}
...
$z_k \in \mathbb{R}^m$ one has
\[
  \langle z_k, \Big( F_k(y) - F_k(x) + \sum_{i = 1}^d (y^{(i)} - x^{(i)}) V_k^{(i)} \Big) z_k \rangle \ge 0 
  \quad \forall y \in \mathbb{R}^d, \: k \in \{ 1, 2 \}.
\]
Summing up these two inequalities one obtains for any $z \in \mathbb{R}^{\ell + m}$ and $\forall y \in \mathbb{R}^d$
\[
...
\begin{multline*}
  ...
  + \langle z_2, V_{21}^{(1)} z_1 \rangle + \langle z_1, V_{11}^{(1)} z_1 \rangle, \ldots,
  \\
  \langle z_1, V_{11}^{(d)} z_1 \rangle + \langle z_1, V_{12}^{(d)} z_2 \rangle 
  + \langle z_2, V_{21}^{(d)} z_1 \rangle + \langle z_1, V_{11}^{(d)} z_1 \rangle \Big)
  \\
...
$F'_+(0) \succeq V_0 \succeq F'_-(0)$, which by Theorem~\ref{thrm:OneDimCase} implies that $V_0 \in \partial F(0)$. Note
that the matrix $V_0$ is not diagonal, despite the fact that the function $F$ is diagonal matrix-valued.
