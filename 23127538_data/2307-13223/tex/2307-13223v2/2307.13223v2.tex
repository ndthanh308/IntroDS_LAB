...
Therefore, we have to consider the mixed type of discrete conformal structures, i.e., the discrete hyperbolic metrics $l_{ij}, l_{jk}, l_{ki}$ of a spherical triangle $\{ijk\}\in F$ are induced by 
(\ref{Eq: F32}) and (\ref{Eq: F34}), (\ref{Eq: F33}) and (\ref{Eq: F35}), 
or $\mathbf{(c2)}$ and $\mathbf{(c4)}$, such that the condition (\ref{Eq: spherical compatible condition}) holds.
\end{remark}
...
+\frac{\eta_{ij}e^{f_i+f_j}}{\sinh l_{ij}}.
\end{equation*}
It is easy to check that the $\alpha_i$ and $\eta_{ij}$ derived in any two adjacent hyperbolic triangles must be equal.
...
Note that the discrete conformal structure (\ref{Eq: spherical edge}) for $\varepsilon\equiv 1$ corresponds to  the cosine law for a generalized hyperbolic triangle with two hyper-ideal vertices and one ideal vertex and
...
For the discrete conformal structure (\ref{Eq: F23}), 
set $e^{f_i}=\cosh r_i$, then it corresponds to the cosine law for a generalized hyperbolic triangle with two hyperbolic vertices and one hyper-ideal vertex.
For the discrete conformal structure $\mathbf{(b2)}$ in Theorem \ref{Thm: classification of DCS},
