...
\begin{remark}
Glickenstein-Thomas' classification theorem in \cite{GT} contains only the discrete conformal structures $\mathbf{(A)}$, $\mathbf{(b1)}$ and $\mathbf{(c1)}$ in Theorem \ref{Thm: classification of DCS}.
We add the discrete conformal structures $\mathbf{(b2)}$, $\mathbf{(c2)}$, $\mathbf{(c3)}$ and $\mathbf{(c4)}$ in Theorem \ref{Thm: classification of DCS} to complete the classification of discrete conformal structure on a triangulated surface with $\mathbb{G}$ background geometry.
...
one can reparameterize discrete conformal structures $d=d(f)$ in $\mathbf{(A)}$, $\mathbf{(b1)}$ and $\mathbf{(c1)}$ of Theorem \ref{Thm: classification of DCS} so that $\alpha_i\in \{-1,0,1\}$ while keeping the induced $\mathbb{G}$ polyhedral metrics invariant.
Similar parameterization also applies to the discrete conformal structure $\mathbf{(c3)}$.
...
For the case $\mathbf{(B)}$, since Glickenstein-Thomas \cite{GT} and Thomas \cite{Thomas} gave detailed calculations, we only choose some key formulas to avoid repetitions.
...
It should be mentioned that Glickenstein-Thomas \cite{GT} only considered the case that $\frac{\partial H}{\partial f_i}=\frac{2a_{ij}e^{2f_i}}{1+a_{ij}e^{2f_i}}$ and $\frac{\partial H}{\partial f_j}=-\frac{2a_{ji}e^{2f_j}}{1+a_{ji}e^{2f_j}}$.
...
The rest of the proof in this case has been given in detials by Glickenstein-Thomas \cite{GT} and Thomas \cite{Thomas}, so we omit the proof here.
...
H=2f_i-2f_j+c_{ij},
\end{equation}
where $c_{ij}$ is a constant with $c_{ij}+c_{ji}=0$.
Then
\begin{equation*}
...
H=2f_i-2f_j+c_{ij},
\end{equation} where $c_{ij}$ is a constant with $c_{ij}+c_{ji}=0$.
Then
\begin{equation*}
\frac{\cos^2 d_{ij}}{\cos^2 d_{ji}}
=e^H=c_2e^{2f_i-2f_j},
...
\end{cases}
\end{eqnarray*}
for the discrete conformal structure $\mathbf{(b1)}$ in Theorem \ref{Thm: classification of DCS} and set
\begin{eqnarray*}
e^{u_i}=
\begin{cases}
...
then it is easy to check that the discrete conformal structure $\mathbf{(b1)}$ in Theorem \ref{Thm: classification of DCS} is equivalent to (\ref{Eq: hyperbolic edge}) in Definition \ref{Def: ZGZLYG}.
...
\end{cases}
\end{eqnarray*}
for the discrete conformal structure $\mathbf{(c1)}$ in Theorem \ref{Thm: classification of DCS} and set
\begin{eqnarray*}
e^{u_i}=
\begin{cases}
r_i  &{\text{if $\varepsilon_i=0$}},\\
...
then it is easy to check that the discrete conformal structure $\mathbf{(c1)}$ in Theorem \ref{Thm: classification of DCS} is equivalent to (\ref{Eq: spherical edge}) in Definition \ref{Def: ZGZLYG}.
For example, if we set $\alpha\equiv 1$ and $e^f=\sin r$, then the discrete conformal structure $\mathbf{(c1)}$ can be reduced to
\begin{equation*}
\cos l_{ij}
...
Note that the discrete conformal (\ref{Eq: spherical edge}) for $\varepsilon\equiv 1$ satisfies $(-1,-1,0)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2} and $u$ represents the edge length of $(-1,-1,0)$ type generalized hyperbolic triangle.
Therefore, the discrete conformal structure $\mathbf{(c1)}$ in Theorem \ref{Thm: classification of DCS} takes the edge lengths as its variables.

While the discrete conformal structure $\mathbf{(c3)}$ in Theorem \ref{Thm: classification of DCS} takes the generalized angles as its variables, which can be taken as the dual form of $\mathbf{(c1)}$.
Set
\begin{eqnarray*}
e^{f_i}=
\begin{cases}
...
In the case that $\alpha\equiv0$, the discrete conformal structure $\mathbf{(c3)}$ can be reduced to
\begin{equation*}
\cos l_{ij}=-1+\eta_{ij}r_ir_j,
\end{equation*}
which satisfies $(0,0,1)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2}.

In the case that $\alpha\equiv1$, the discrete conformal structure $\mathbf{(c3)}$ can be reduced to
\begin{equation*}
\cos l_{ij}=-\cos r_i\cos r_j+\eta_{ij}\sin r_i\sin r_j,
\end{equation*}
which satisfies $(1,1,1)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2} if $\eta_{ij}>1$.

In the case that $\alpha\equiv-1$, the discrete conformal structure $\mathbf{(c3)}$ can be reduced to
\begin{equation*}
\cos l_{ij}=-\cosh r_i\cosh r_j+\eta_{ij}\sinh r_i\sinh r_j,
\end{equation*}
which satisfies $(-1,-1,1)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2} if $\eta_{ij}>1$.

In the case that $\alpha_i=0$ and $\alpha_j=1$, the discrete conformal structure $\mathbf{(c3)}$ can be reduced to
\begin{equation*}
\cos l_{ij}=-\cos r_j+\eta_{ij} r_i\sin r_j,
\end{equation*}
which satisfies $(1,1,0)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2} if $\eta_{ij}>0$.
...
In the case that $\alpha_i=1$ and $\alpha_j=-1$, the discrete conformal structure $\mathbf{(c3)}$ can be reduced to
\begin{equation*}
\cos l_{ij}=-\cos r_i\cosh r_j+\eta_{ij}\sin r_i\sinh r_j,
\end{equation*}
which satisfies $(1,1,-1)$ type generalized hyperbolic cosine law in Guo-Luo \cite{GL2} if $\eta_{ij}>0$.

In the six cases above, $r$ represents the generalized angle of generalized hyperbolic triangle.

For the discrete conformal structure $\mathbf{(b2)}$ in Theorem \ref{Thm: classification of DCS}, it is easy to check that it is exactly the $(1,1,0)$ type generalized circle packing metric introduced by Guo-Luo \cite{GL2}.
However, the relationships between the hyperbolic geometry and the discrete conformal structures $\mathbf{(c2)}$ and $\mathbf{(c4)}$ are not clear.
...
%\bibitem{Glickenstein arxiv} D. Glickenstein, \emph{Geometric triangulations and discrete Laplacians on manifolds}, \href{https://arxiv.org/abs/math/0508188} {arXiv:math/0508188 [math.MG].}
