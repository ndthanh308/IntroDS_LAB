...
\author{S.\ Alama, L.\ Bronsard, and S.\ Vriend}
...
\begin{document}
 ...
 \subsection*{Clusters}
    ...
    \begin{defn}\label{clusterdef}
        An \textit{$(N, M)$-cluster} $\X = (\E, \F)$ in $\R^n$ is a pair of finite families of sets of locally finite perimeter 
        \begin{equation*}
            \E = \{\E(h)\}_{h = 1}^N, \quad \F = \{\F(i)\}_{i = 1}^M, \quad N, M \in \N_0,
        \end{equation*}
        which satisfies the following list of properties:
        \begin{enumerate}[label=(\roman*)]
            \item \textit{Proper chambers:} the sets $\E(h)$, $1 \leq h \leq N$, satisfy
            \begin{align*}
            ...
            \item \textit{Full measure:}  following Maggi Section 30.3 \cite{maggi2012sets}, we have 
            \[
    ...
    We remark that as a consequence of the full measure axiom, any $(N,M)$-cluster necessarily has $M \geq 1$. Proper $N$-clusters, having $N$ disjoint chambers $\E=(\E(h))_{h=1,\dots,N}$ of finite measure satisfying the above conditions, are defined in \cite{maggi2012sets}. Given such an $N$-cluster we may always append the exterior chamber to obtain an $(N,1)$-cluster consistent with this definition. Conversely, any $(N,1)$-cluster must be an $N$-cluster as defined in \cite{maggi2012sets}, together with its exterior chamber (up to a null set). Note that Maggi implicitly includes the complement of an $N$-cluster as an improper chamber by denoting it $\E(0)$. Note that Definition~\ref{clusterdef} with $N=0$ extends the notion of an improper $M$-cluster from \cite{maggi2012sets}.
    
    It will often be convenient to denote the proper and improper chambers of a cluster with a single index, and so we also write an $(N,M)$-cluster as $\X=(\X(j))_{1\le j\le N+M}$ by setting
            \begin{equation*}
                \X(j) = \begin{cases}
...
So, for example, a double-bubble cluster in $\R^2$ is described as a $(2,1)$-cluster, with two disjoint chambers $\E(1),\E(2)$ of prescribed areas $|\E(1)|=\mathbf{m}(1)=A_1$, $|\E(2)|=\mathbf{m}(2)=A_2$, and the exterior chamber 
...
%    \end{remark}
    ...
    \begin{defn}
        An \textit{$(N,M)$-partitioning problem in $\R^n$} is any variational problem of the form
        \begin{equation*}
            \inf \{P(\X; B_r) : \mathbf{m}(\X) = \mathbf{m}\} \quad \text{for each open ball } B_r \text{ centered at 0 with radius } r > 0
        \end{equation*}
        corresponding to some choice of $\mathbf{m} \in \overline{\R}^{N + M}$ with 
        \begin{align*}
            \begin{cases}
                0 < \mathbf{m}(j) < \infty & \text{if } 1 \leq j \leq N, \\
                \mathbf{m}(j) = \infty & \text{if } N+1 \leq j \leq N+M,
            \end{cases}
        \end{align*}
        where the infimum is taken among all $(N,M)$-clusters $\X$ with volume vector $\mathbf{m}$. 
    \end{defn}
...
%    \end{remark}
    ...
    \begin{lem}\label{lem:approximating-double-bubble}
        For $A > 0$, consider the minimizing double bubble $\D_A = \{\D_A(1), \D_A(2), \D_A(3)\}$, $\D_A(3)=\R^2\setminus\overline{\D_A(1)\cup\D_A(2)}$, with area vector $(1,A, \infty)$, oriented so that the circle supporting the interface  $\D_A(2,3)$ is tangent to the $x$-axis at the origin; see Figure \ref{fig:approximating-double-bubble}. Then as $A \to \infty$,
        \begin{equation*}
    ...
    \begin{proof}
        ...
        Let $r' \geq r$ be arbitrary. We aim to derive estimates on $d_{B_r'}(\D_A, \X_\infty)$ via geometrical information about $\D_A$ (see \cite{isenberg}).
...
Assume toward a contradiction that $\X=\X_\infty$ is \textit{not} a locally minimizing $(1,2)$-cluster for the area vector $(1, \infty, \infty)$. From now on,  we will refer to the standard double bubble $X_\infty$ as $\X$. Then there exists an open ball $B_r$ of radius $r > 0$ centered at $0$ and a  $(1,2)$-cluster $\X'$ such that the area vectors of $\X=\X_\infty$ and $\X'$ agree, 
        \[
        ...
        We may assume by expanding $B_r$ that $\X(1), \X'(1) \subset \subset B_r$. Note that we make no \textit{a priori} assumptions about the regularity of the interfaces of $\X'$ within $B_r$. Denote by $K$ a compact set such that
        \begin{equation*}
        ...
        Since $\mathbf{m}(\X') = \mathbf{m}(\X)$, we have that $\abs{\X'(1)} = 1$. Upon modification, one of $\X(2)$ or $\X(3)$ must diminish (i.e.\ weakly decrease) in relative area within $B_r$, so by reflecting we can assume that the upper region $\X'(2) \cap B_r^+$ has area at least that of $\X(2) \cap B_r^+=\X(2) \cap B_r$, where $B_r^+ = B_r \cap \{y \geq 0\}$. Since $\X$ and $\X'$ only differ in $K$, we can assume,
       ...
       % \begin{equation*}
        ...
        \begin{enumerate}
            ...
            \item {\it The transitions $\M(2,3)$, between phase domains $\M(2)$ and $\M(3)$ are straight line segments or rays.}  As there is no mass constraint for $\M(2),\M(3)$, there is no Lagrange multiplier in the Euler-Lagrange equation, and so minimizers must have zero curvature at points of $\M(2,3)$.
