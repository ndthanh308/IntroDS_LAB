...
\emailAdd{
raffer06@ucm.es, m.jaarsma@uva.nl, ignazios@ucm.es, w.j.waalewijn@uva.nl}

\abstract{
In view of the increasing precision of theoretical calculations and experimental measurements, power corrections to transverse-momentum-dependent observables are highly important. We study the next-to-leading power corrections for transverse momentum measurements in $e^+e^- \to 2$ jets. We obtain a factorized expression for the cross section, identify the new jet functions that appear in it, and provide the corresponding anomalous dimensions at leading order in $\alpha_s$.  The factorization of this process up to next-to-leading power involves twist-2 and twist-3 operators, which exhibit non-trivial behavior. By working with jets, everything is perturbatively calculable and there are substantial simplifications compared to the general next-to-leading power framework. Importantly, our analysis with jets can be extended to semi-inclusive deep-inelastic scattering, with the future Electron-Ion Collider as key application.
}
...
In this work, we address two fundamental questions: First, we want to identify the new jet functions that arise from the additional operators at NLP. Second, we want to obtain the factorized expression for the cross section in terms of jet functions and hard matching coefficients. The answers to these questions that we obtain,  enable us to study how NLP corrections impact transverse-momentum-dependent measurements, including angular asymmetries.
We find that in order to get a non-vanishing contribution from NLP operators, one has to break the symmetry between the energetic radiation going into the two different directions. For the case of di-jet production,  this can be achieved by e.g.~using two different (recoil-free) recombination schemes.  

Despite the  complexity of the subject we have made an effort to present our material in a didactic way. 
The choice of the process and the jet definitions that we use greatly reduces the number of independent distributions and simplifies the evolution with respect to the original papers~\cite{Vladimirov:2021hdn,Rodini:2022wic,Rodini:2023plb},  see \eq{ga_J_21}.
In this sense, the present work can also be viewed as an introductory overview to the study of NLP effects in TMD factorization. 

The outline of this paper is as follows: We start in sec.~\ref{sec:Kinematics} by introducing the kinematic variables needed to describe $e^+e^-\rightarrow 2$ jets, and writing the cross section in terms of the hadronic and leptonic tensor.  In sec.~\ref{sec:HadronicTensor},  the hadronic tensor is factorized and rewritten, with Fierz identities relegated to app.~\ref{sec:Fierz}. Our final expression in terms of jet functions is given in \eq{W_final}. We calculate the NLP jet functions in sec.~\ref{sec:JetFunction}.
The final cross section is then presented in sec.~\ref{sec:NLP}, highlighting the effect of NLP contributions. Our conclusions are given in sec.~\ref{sec:Conclusions}.
...
In order to introduce our observables, we define two planes, which we will refer to as the transverse and perpendicular plane. The transverse plane is orthogonal to the two outgoing jets and the perpendicular plane is orthogonal to the intermediate photon $q$ and one of the outgoing jets. These planes can be described in terms of the metric tensors that project onto the corresponding subspaces: 
%%%
...
If instead of jets two hadrons are considered, this has been studied before, especially in relation to Belle experiment, see the reviews in refs.~\cite{Boussarie:2023izj,USBelleIIGroup:2022qro,Accardi:2022oog,Belle:2008fdv}. The distributions in this case were classified in ref.~\cite{Boer:1997mf}, and mass corrections to kinematics have been included in \cite{Mulders:2019mqo}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Hadronic tensor}
\label{sec:HadronicTensor}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The target of this section is to factorize the hadronic tensor that appears in the cross section of sec.~\ref{sec:Kinematics} into a set of products of jet functions accompanied by different Lorentz structures up to NLP. 
As a start, we consider the complete and un-expanded hadronic tensor  for the process under consideration,
%%%
...
\end{align}
%%%
Since we assume a large jet radius, all energetic final-state particles are clustered into one of the two jets and $X$ only contains soft radiation.

The explicit  notation for the   power expansion of the hadronic tensor is
%%%
\begin{align}
    W^{\mu\nu}(q)&=W^{\mu\nu}_\text{LP}(q)+W^{\mu\nu}_\text{NLP}(q)
...
\end{align}
%%%
In order to arrive at our final expressions for $W_{\text{LP},\text{NLP}}$, we proceed in several steps, starting from the TMD operator expansion method and the basis of operators in sec.~\ref{sec:OB}.
 In particular, we list a set of  operator blocks that have definite twist, which allow for a clean separation in terms of power counting. We use these definitions in sec.~\ref{sec:EHT}, to obtain the power expansion of the product of currents in \eq{had_tensor} in position space (indicated by the tilde)
%%%
\begin{align}
    \tilde{\mathcal{W}}^{\mu\nu}(y)&=J^{\mu \dagger}(y)\, J^\nu(0)
...
\end{align}
%%%
Next, we include the external jet states and Fourier transform back to momentum space, to obtain an expression for the hadronic tensor in terms of a set of jet functions with open spinor indices. In sec.~\ref{sec:JF}, we apply Fierz relations to obtain a set of jet functions with closed spinor indices, and use discrete symmetries and Lorentz decomposition to simplify and reduce the basis of jet functions, resulting in just one new jet function at NLP! This section contains the  final definitions  for jets, the first result of this paper.
To obtain the physical jet functions, we need to remove the overlapping soft contributions and renormalize, which we discuss in sec.~\ref{sec:OS} and \ref{sec:evolution}. In particular, we discuss how the cancellation of rapidity divergences is realized at LP and NLP. In \sec{evolution}, we arrive at the  evolution equations for jets. The evolution equation for the twist-3 jet function takes on a particularly simple form compared to earlier results in the literature, which is another relevant result of this work.
The final expression for the hadronic tensor at LP and NLP is then presented in sec.~\ref{sec:HT}.
...
\end{align}
%%%
such that each field has a definite power counting ($\xi\sim\lambda$, $\eta\sim \lambda^2$). 
Using these background fields one can construct a set of building-block operators of definite twist (dimension minus spin), 
%%%
...
\begin{align}\nn
    ...
    f(z)
    &=
    -\img\int_{-\infty}^{\infty}\df x\,
    \frac{1}{\pm\bar{x} - \img\delta_E^\pm}
    \int\frac{\df z}{2\pi}\,e^{\pm \img \bar{x} q^\pm z}\,
    f(z)\,,
\end{align}
%%%
where a positive $\delta_E^\pm=\delta^\pm/q^\pm$ ensures that the integral is well defined and shows how $\delta$ regulates rapidity divergences. Note that in this identity the sign of
%%%
\begin{align}\nn
\bar{x} \equiv 1-x
\,,\end{align}
%%%
may be chosen freely as $x$ is integrated over all values. It turns out that it is convenient to choose $+\bar{x}$ for the $\mathbb{O}_{21}$ operators and $-\bar{x}$ for the $\mathbb{O}_{12}$ operators, as the jet functions that will be built from these operators then have support for $x\in[0,1]$. This convention differs from earlier work, and allows for several simplifications in our case.
...
\begin{align}\nn
    ...
    &=
    -\img\int\df x\,\frac{1}{\bar{x} - \img\delta_E^+}
    \int\frac{\df z^-}{2\pi}\,e^{\img \bar{x} (z^- - y^-) q^+}\,
    ...
    &=
    \img\int\df x\,\frac{1}{\bar{x} + \img\delta_E^+}
    \int\frac{\df z^-}{2\pi}\,e^{-\img \bar{x} z^- q^+}\,
    \bigl[\overline{U}_{1,\bar{n}}(y^-,y_T)\bigr]_j^{(-)}
...
\begin{align} \label{eq:W_intermediate}
    ...
    \delta_{ij} (\gamma_T^\nu)_{kl}
    \int_0^1\df x\, C_2^*(x;q^+ q^-) C_1(q^+ q^-)
    \nonumber
    \\
    &\quad\qquad\times
    \int\frac{\df^2 b}{(2\pi)^2}\,e^{\img  b\cdot q}
    ...
    \biggl[\frac{\bar{n}^\nu}{q^-} - \frac{n^\nu}{q^+}\biggr]
    (\gamma_T^\mu)_{ij} \delta_{kl}
    \int_0^1\df x\, C_1^*(q^+ q^-) C_2(x;q^+ q^-)
    \nonumber
    \\
    &\quad\qquad\times
    \int\frac{\df^2 b}{(2\pi)^2}\,e^{\img  b\cdot q}
...
\end{align}
%%%
Here, $C_1$ and $C_2$ are matching coefficients that can be calculated in perturbation theory and the symbol $\mathscr{J}$  denotes jet functions with open spinor indices. At leading order in perturbation theory, these matching coefficients are 1, consistent with eqs.~(\ref{eq:WLP}) and (\ref{eq:WNLP}). The bounds of the $x$ integral arise from the range over which the jet functions have support.
...
\begin{align}  \label{eq:J_21_indices}
    \bigl[\mathscr{J}^q_{21,\bar{n}}(x,b)\bigr]^{jk}
    &=
    \frac{1}{\bar{x} - \img\delta_E^+}
    \,\tr_c \int\frac{\df y_1^-}{2\pi}\frac{\df y_2^-}{2\pi}\,
    e^{ \img  (x y_1^- + \bar{x} y_2^-) q^+}\,
    \nn \\ \nn
    &\qquad\times
    \bra{0}
...
\end{align}
%%%
Note that whether the operator appears in the amplitude or conjugate amplitude is determined by whether the operator is causal or anti-causal. From now on we will drop these superscripts on the operators, as it is no longer necessary to make the distinction.
...
\begin{align}
    ...
    \bigl[\mathscr{J}^q_{21,\bar{n}}(x,b)\bigr]^{jk}
    &=
    2N_c\, J^{\mu,q}_{21,\bar{n}}(x,b)\,,
    &
    +\epsilon_T^{\mu\alpha}(\sigma^{\alpha+} \gamma^5)_{il}
    \bigl[\mathscr{J}^{\bar{q}}_{21,\bar{n}}(x,b)\bigr]^{li}
    &=
    2N_c\, J^{\mu,\bar{q}}_{21,\bar{n}}(x,b)\,,
    \nonumber
    \\[1ex]
    -\epsilon_T^{\mu\alpha}(\sigma^{\alpha-} \gamma^5)_{kj}
    \bigl[\mathscr{J}^q_{21,n}(x,b)\bigr]^{jk}
    &=
    2N_c\, J^{\mu,q}_{21,n}(x,b)\,,
    &
    -\epsilon_T^{\mu\alpha}(\sigma^{\alpha-} \gamma^5)_{il}
    \bigl[\mathscr{J}^{\bar{q}}_{21,n}(x,b)\bigr]^{li}
    &=
    2N_c\, J^{\mu,\bar{q}}_{21,n}(x,b)\,,
    \nonumber
    \\[3ex]
    -\epsilon_T^{\mu\alpha}(\sigma^{\alpha+} \gamma^5)_{kj}
    \bigl[\mathscr{J}^q_{12,\bar{n}}(x,b)\bigr]^{jk}
    &=
    2N_c\, J^{\mu,q}_{12,\bar{n}}(x,b)\,,
    &
    -\epsilon_T^{\mu\alpha}(\sigma^{\alpha+} \gamma^5)_{il}
    \bigl[\mathscr{J}^{\bar{q}}_{12,\bar{n}}(x,b)\bigr]^{li}
    &=
    2N_c\, J^{\mu,\bar{q}}_{12,\bar{n}}(x,b)\,,
    \nonumber
    \\[1ex]
    +\epsilon_T^{\mu\alpha}(\sigma^{\alpha-} \gamma^5)_{kj}
    \bigl[\mathscr{J}^q_{12,n}(x,b)\bigr]^{jk}
    &=
    2N_c\, J^{\mu,q}_{12,n}(x,b)\,,
    &
    +\epsilon_T^{\mu\alpha}(\sigma^{\alpha-} \gamma^5)_{il}
    \bigl[\mathscr{J}^{\bar{q}}_{12,n}(x,b)\bigr]^{li}
    &=
    2N_c\, J^{\mu,\bar{q}}_{12,n}(x,b)\,.
    \nn\\
    \label{eq:J8}
\end{align}
%%%
...
\begin{align} \label{eq:J_rel}
    J^{\rho,q}_{12,\bar{n}}(x,b)
    &=
    -\bigl[J^{\rho,q}_{21,\bar{n}}(x,-b)\bigr]^*\,,
    &
    J^{\rho,\bar{q}}_{21,\bar{n}}(x,b)
    &=
    J^{\rho,q}_{21,\bar{n}}(x,b)\,,
    \nn \\
    J^{\rho,q}_{12,n}(x,b)
    &=
    -\bigl[J^{\rho,q}_{21,n}(x,-b)\bigr]^*\,,
    &
    J^{\rho,\bar{q}}_{21,n}(x,b)
    &=
    J^{\rho,q}_{21,n}(x,b)\,,
\end{align}
%%%
...
\begin{align} \label{eq:J_ld}
    J^{\rho}_{21,\bar{n}}(x,b)
    &=\frac{b^\rho}{b^2} J_{21,\bar{n}}(x,\mathbf{b}^2)\,,
    &
    \partial_\rho J_{11,\bar{n}}(b)
    &=\frac{b_\rho}{b^2} J^{\prime}_{11,\bar{n}}(\mathbf{b}^2)\,,
    \nn \\ 
    J^{\rho}_{21,n}(x,b)
    &=\frac{b^\rho}{b^2} J_{21,n}(x,\mathbf{b}^2)\,,
    &
    \partial_\rho J^q_{11,n}(b)
    &=\frac{b_\rho}{b^2} J^{\prime\,q}_{11,n}(\mathbf{b}^2)\,.
\end{align}
%%%
In the next step, we insert the jet functions into the hadronic tensor in \eq{W_intermediate} and integrate over the angle of $\mathbf{b}$,
%%%
\begin{align}
    \int\frac{\df^2 b}{(2\pi)^2}\,e^{\img b\cdot q}\,f(\mathbf{b}^2)
    &=
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{|\mathbf{b}|J_0(|\mathbf{b}||\mathbf{q}|)}{2\pi}\,
    f(\mathbf{b}^2)\,,
    \nn \\ 
    \int\frac{\df^2 b}{(2\pi)^2}\,e^{\img b\cdot q}\,
    \frac{\img b^\rho}{b^2}\,f(\mathbf{b}^2)
    &=
    q_T^\rho\int_0^\infty\df|\mathbf{b}|\,
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    f(\mathbf{b}^2)\,.
\end{align}
%%%
The latter equation is needed to take the explicit $b^\rho$ dependence in \eq{J_ld} into account.
...
The two collinear regions overlap in the soft region, which is doubly counted. 
The presence of this overlap  has been extensively discussed in the literature about TMD factorization~\cite{Collins:2011zzd, Echevarria:2011epo,Chiu:2012ir},
and it has a similar treatment in background field method~\cite{Vladimirov:2021hdn}.
In this section we discuss the corresponding subtraction, which treats the rapidity divergences. 

Long story short, the overlap can be removed for bare operators by making the following replacements
%%%
\begin{align} \label{eq:overlap}
    \mathcal{O}_{11,\bar{n}}\mathcal{O}_{11,n}\rightarrow
    \frac{\mathcal{O}_{11,\bar{n}}\mathcal{O}_{11,n}}{\mathcal{S}_\text{LP}}\,,\quad
    \partial_\rho\mathcal{O}_{21,\bar{n}}
    \mathcal{O}_{11,n}\rightarrow
    \frac{\partial_\rho\mathcal{O}_{21,\bar{n}}
    \mathcal{O}_{11,n}}{\mathcal{S}_\text{LP}}\,,\quad
    \mathcal{O}_{21,\bar{n}}\mathcal{O}_{11,n}\rightarrow
    \frac{\mathcal{O}_{21,\bar{n}}\mathcal{O}_{11,n}}{\mathcal{S}_\text{LP}}\,,
\end{align}
%%%
...
\label{eq:soft}
   \mathcal{S}_\text{LP}^{\rm bare} 
    (\mathbf{b}^2,\epsilon,\delta^+\delta^-)
    &=
    \frac{1}{N_c}\tr_c\bigl\{
...
The overlap subtraction as presented in \eq{overlap} happens at the level of the product of two operators from the different collinear sectors. However, it is convenient to implement this subtraction by absorbing a square root of the overlap factor into each of the jet functions. To do this we use the fact that the soft function can be separated as
%%%
\begin{align}
    \mathcal{S}_\text{LP}^{\rm bare}
    (\mathbf{b}^2,\epsilon,\delta^+\delta^-)
    ...
    \bigl(\mathbf{b}^2,\epsilon,\tfrac{1}{2}(\delta_E^+)^2\zeta\bigr)}
    \sqrt{\mathcal{S}_\text{LP}^{\rm bare}
    \bigl(\mathbf{b}^2,\epsilon,\tfrac{1}{2}(\delta_E^-)^2\zeta\bigr)}\,,
\end{align}
%%%
...
\end{align}
%%%
and $\zeta$ is referred to as the rapidity scale. Using the above identity, we now define the (bare) subtracted jet functions as
%%%
\begin{align}
\label{eq:Jbaresub}
    ...
    (\mathbf{b}^2,\epsilon,\tfrac{1}{2}(\delta_E^+)^2\zeta)}}\,,
    \nn \\ \nn
    J^{\prime\,\text{bare,sub.}}_{11,\bar{n}}
    (\mathbf{b}^2,\zeta,\epsilon,\delta_E^+)
    &=
    \frac{J^{\prime
    \,\text{bare}}_{11,\bar{n}}(\mathbf{b}^2,\epsilon,\delta_E^+)}
    {\sqrt{\mathcal{S}_\text{LP}
    (\mathbf{b}^2,\epsilon,\tfrac{1}{2}(\delta_E^+)^2\zeta)}}\,,
    \\ 
    J^\text{bare,sub.}_{21,\bar{n}}
    (x,\mathbf{b}^2,\zeta,\epsilon,\delta_E^+)
    &=
    \frac{J^\text{bare}_{21,\bar{n}}(x,\mathbf{b}^2,\epsilon,\delta_E^+)}
    {\sqrt{\mathcal{S}_\text{LP}
    (\mathbf{b}^2,\epsilon,\tfrac{1}{2}(\delta_E^+)^2\zeta)}}\,.
\end{align}
%%%
Note that since the overlap factor depends on $b$, the definition of $J^\prime_{11}$ as the derivative of $J_{11}$ only holds at the unsubtracted level. 

Both the overlap factor of eq.~(\ref{eq:soft}) and the jet functions that appear in the hadronic tensor contain rapidity divergences. The rapidity divergences  cancel out in the hadronic tensor, although this cancellation is not trivial. For the twist-2 jet functions, the rapidity divergences cancel after the overlap subtraction is performed and the rapidity scale dependence of the jet functions cancels when the product of the jet functions from the two collinear sectors is taken. This is not the case for the twist-3 jet functions, where special rapidity divergences remain even after the overlap subtraction is performed. The special rapidity divergences eventually cancel  in the linear combination of $J_{21}$ and $J^\prime_{11}$ that  appears in our final result in eq.~(\ref{eq:finalLW}). 
...
The subtracted jet functions defined above still contain ultraviolet divergences and require renormalization.
Since renormalization and evolution takes place at the operator level, it is independent of the recoil-free jet algorithm that is employed.
...
\end{align}
%%%
The same equations apply to $J^\prime_{11}$ as the renormalization and evolution do not involve $b$. 

The renormalization and evolution of the twist-3 operators is discussed in ref.~\cite{Rodini:2022wki}. Here it was found that the evolution involves a convolution in momentum fractions and these anomalous dimension appeared before in ref.~\cite{Braun:2009mi}. The precise form of the evolution for our twist-3 jet functions can be derived from ref.~\cite{Rodini:2022wki} by identifying the following relation between our jet function and their TMD distributions denoted by $\Phi$
%%%
\begin{align}
    J_{21,\bar{n}}(x,\mathbf{b}^2)
    =
    \frac{1}{N_c}
    \frac{1}{\bar{x}-\img \delta^+}
    \Phi_{21,\rho}^{[b_\mu\epsilon_T^{\mu\alpha}\gamma_T^\rho (\sigma^{\alpha+}\gamma^5)]}
    (x,\bar{x},-1;b)\,.
\end{align}
%%%
Using this relation, the evolution of the twist-3 jet functions then follows from eq~(3.11) of ref.~\cite{Rodini:2022wki}\footnote{We thank Alexey Vladimirov for pointing out a few typos (wrong signs) in eq.~(3.11), which have been corrected in their recent paper~\cite{Rodini:2023plb}.}. We observe that the term proportional to $\mathbb{P}^{B}$ has no contribution for the present case. 

A few simplifications allow us to recast the evolution presented in ref.~\cite{Rodini:2022wki} in a more elegant form. First, our jet functions only have support for $x\in[0,1]$. This can be shown by inserting a complete set of states in between each field that enters the matrix element definition of the jet function. Second, by changing variables in such a way that all functions that appear in the convolution  of eq.~(3.11) of ref.~\cite{Rodini:2022wki} have the same argument, the convolutions can be written in terms of a single integral. Third, we can explicitly extract the plus distributions in the anomalous dimension.
Following these manipulations, the renormalized twist-3 jet functions can be expressed as
%%%
\begin{align}
    J_{21,\bar{n}}(x,\mathbf{b}^2,\mu,\zeta,\delta_E^+)
    &=
    \int_0^1\df x'\,Z_{J_{21}}(x,x',\mu,\zeta,\epsilon)\,
    J^\text{bare,sub}_{21,\bar{n}}
    (x',\mathbf{b}^2,\zeta,\epsilon,\delta_E^+)\,,
\end{align}
%%%
...
\begin{align}
    ...
    J_{21,\bar{n}}(x,\mathbf{b}^2,\mu,\zeta,\delta_E^+)
    &=
    \int_0^1\df x'\,\gamma_{J_{21}}(x,x',\mu,\zeta)\,
    J_{21,\bar{n}}(x',\mathbf{b}^2,\mu,\zeta,\delta_E^+)\,.
\end{align}
%%%
The residual $\de_E^+$ is due to the special rapidity divergence and cancels against $J_{11}'$.
The anomalous dimension for the twist-3 jet functions is given to one-loop order by 
\\[-1ex]
...
\begin{align} \label{eq:ga_J_21}
    \gamma_{J_{21}}(x,x',\mu,\zeta)
    &=
    a_s\biggl\{\delta(x-x')
    \biggl[2C_F\ln\biggl(\frac{\mu^2}{\zeta}\biggr)
    +3C_F-2C_A\ln\bar{x} - 2C_F \ln x\biggr]
    \nn \\
    &\quad
    +C_A\biggl(\biggl[\frac{\Theta(x\!-\!x')}{x\!-\!x'}\biggr]_+
    \!\!-\! \biggl[\frac{\Theta(x'\!-\!x)}{x\!-\!x'}\biggr]_+\!
    \!\!+\! \Theta(x\!-\!x')\frac{\bar{x}\bar{x}'\!-\!x}{\bar{x}'}
    \!-\! \Theta(x'\!-\!x)\frac{x(x'\!-\!\bar{x}\bar{x}')\!+\!\bar{x}}{\bar{x}x'}
    \biggr)
    \nonumber
    \\
    &\quad
    -(2C_F-C_A)\biggl(\Theta(\bar{x}\!-\!x')\frac{x[2\bar{x}-(1+\bar{x})x']}{\bar{x}\bar{x}'}
    +\Theta(x'\!-\!\bar{x})\frac{\bar{x}\bar{x}'}{x'}\biggr)
    \biggr\}\,.    
\end{align}
%%%
\vspace{-2ex}}}\\[2ex]} 

The subtracted and renormalized jet functions also depend on the rapidity scale $\zeta$, in addition to their renormalization scale dependence. For all  jet functions the rapidity scale dependence is described by the Collins-Soper equation
%%%
\begin{align}
    \frac{{\df }}{{\df }\ln\zeta}\ln J_{11}(\mathbf{b}^2,\mu,\zeta)=\frac{{\df }}{{\df }\ln\zeta}\ln J_{11}^\prime(\mathbf{b}^2,\mu,\zeta,\delta_E^+)=\frac{{\df }}{{\df }\ln\zeta}\ln J_{12}(x,\mathbf{b}^2,\mu,\zeta,\delta_E^+)=-{\cal D}(\mathbf{b}^2,\mu)\,,
\end{align}
%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Hadronic tensor at LP and NLP}
\label{sec:HT}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

We now combine all ingredients to write the hadronic tensor in a decomposed and factorized form. The LP and NLP contributions to the cross section can be written as 
\\[-2ex]
...
&\hspace{1.3ex} W_{\text{LP}}^{\mu\nu}(q)
    =
    2N_c\, (-g_T^{\mu\nu})
    \bigl|C_1(q^+ q^-,\mu^2)\bigr|^2
    \int\frac{\df^2 b}{(2\pi)^2}\,e^{\img b\cdot q_T}
    J_{11,\bar{n}}(\mathbf{b}^2,\mu,\zeta)\,J_{11,n}(\mathbf{b}^2,\mu,\zeta)\,,    
    \\[2ex]
    &W_{\text{NLP}}^{\mu\nu}(q) =
    \nn \\
    & \quad
    ...
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    J^\prime_{11,\bar{n}}(\mathbf{b}^2,\mu,\zeta)\,J_{11,n}(\mathbf{b}^2,\mu,\zeta)
    \nonumber
    \\
    &\quad
    +2N_c\, \frac{1}{q^-}
    ...
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    J_{11,\bar{n}}(\mathbf{b}^2,\mu,\zeta)\,J^\prime_{11,n}(\mathbf{b}^2,\mu,\zeta)
    \nonumber
    \\
    &\quad
    +2N_c\,\,
    \biggl[\frac{\bar{n}^\mu q_T^\nu}{q^-} - \frac{n^\mu q_T^\nu}{q^+}\biggr]
    \int_0^1\df x\, C_2^*(x;q^+ q^-,\mu^2) C_1(q^+ q^-,\mu^2)
    \nonumber
    \\
    &\quad\qquad\times
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    \Bigl\{
    J_{21,\bar{n}}(x,\mathbf{b}^2,\mu,\zeta) J_{11,n}(\mathbf{b}^2,\mu,\zeta)
    -J_{11,\bar{n}}(\mathbf{b}^2,\mu,\zeta) J_{21,n}(x,\mathbf{b}^2,\mu,\zeta)
    \Bigr\}
    \nonumber
    \\
    &\quad
    +2N_c\,\,
    \biggl[\frac{\bar{n}^\nu q_T^\mu}{q^-} - \frac{n^\nu q_T^\mu}{q^+}\biggr]
    \int_0^1\df x\, C_1^*(q^+ q^-,\mu^2) C_2(x;q^+ q^-,\mu^2)
    \nonumber
    \\
    &\quad\qquad\times
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    \Bigl\{
    J^*_{21,\bar{n}}(x,\mathbf{b}^2,\mu,\zeta) J_{11,n}(\mathbf{b}^2,\mu,\zeta)
    -J_{11,\bar{n}}(\mathbf{b}^2,\mu,\zeta) J^*_{21,n}(x,\mathbf{b}^2,\mu,\zeta)
    \Bigr\}\,.
\nn \end{align}
%%%
\vspace{-2ex}}}\\[2ex]} 
\noindent Due to the relative signs that appear in \eq{W_final}, the only way a nontrivial NLP contribution appears is by choosing different final states, for instance a hadron and a jet, for the two back-to-back directions. This also happens for Drell-Yan, establishing that the LHC is sensitive to NLP corrections, while the Tevatron is not.

For completeness we give the (bare) jet functions, which were derived in the previous sections. For twist-2 they are given by the following matrix elements of the leading-twist building block operators in \eq{U1}, 
%%%
\begin{align}
    ...
    &=
    \frac{1}{2N_c}\, \int\frac{\df y^+}{2\pi}\,e^{\img y^+ q^-}\,
    \tr\bigl[\gamma^-\bra{0}U_{1,n}(y^+,b)
    \ket{J^n_\text{alg}}
    \bra{J^n_\text{alg}}
    \overline{U}_{1,n}(0,0)
    \ket{0}\bigr]\,,
\end{align}
%%%
where the trace is over both the color and Dirac indices.
At NLP, derivatives of the twist-2 jet function also appear in the hadronic tensor,
%%%
\begin{align}
    ...
    J^{\prime\,\text{bare}}_{11,\bar{n}}(b)
    =
    b^\mu\frac{\partial}{\partial b^\mu} J^\text{bare}_{11,n}(b)\,,
\end{align}
%%%
Finally, the twist-3 jet functions that enter the hadronic tensor at NLP are defined as
%%%
\begin{align}
    J^\text{bare}_{21,\bar{n}}(x,\mathbf{b}^2)
    &=
    +b^\mu\epsilon_{T,\mu\alpha}\,
    \frac{1}{\bar{x} - \img \delta_E^+}\,
    \frac{1}{2N_c}
    \int\frac{\df y_1^-}{2\pi}\frac{\df y_2^-}{2\pi}\,
    e^{ \img  (x y_1^- + \bar{x} y_2^-) q^+}\,
    ...
    \ket{0}\Bigr]\,,
    \nn \\[2ex]
    J^\text{bare}_{21,n}(x,\mathbf{b}^2)
    &=
    -b^\mu\epsilon_{T,\mu\alpha}\,
    \frac{1}{\bar{x} - \img \delta_E^-}\,
    \frac{1}{2N_c}
...
\end{align}
%%%
with $U_2$ given in \eq{U2}.


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Jet function calculation}
\label{sec:JetFunction}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In the previous section we obtained a factorization formula for the hadronic tensor in terms of a set of jet functions. For convenience we repeat their (unsubtracted, bare) definition here,
%%%
...
We now calculate the jet functions in \eq{jet_repeat} to first order in $a_s = \alpha_s/(4\pi)$. The (leading-order) matrix elements that are required in this calculation are given by
%%%
...
\nn\end{align}
%%%
with the relevant diagrams shown in \fig{diagrams}. We have omitted the virtual corrections, as the virtual correction to $J_{11}$ vanish in dimensional regularization, and the virtual correction to $J_{21}$ is exactly zero. Inserting the expressions in \eq{me} in \eq{jet_repeat}, we find to order $a_s$ 
%%%
\begin{align}
    J_{11,\bar{n}}^{\rm bare}
    &=
    ...
    J_{21,\bar{n}}^{\rm bare}
    &=
    -4\img g^2C_F\,\frac{1}{\bar{x} - \img \delta_E^+}
    \int\frac{\df^d p}{(2\pi)^d}\frac{\df^d k}{(2\pi)^d}\,
    (2\pi)\delta_+[k^2]\,(2\pi)\delta_+[(p-k)^2]\,
    (2\pi)^{d-1}\delta^{(d-2)}(\mathbf{P}_J^{\rm alg})
    \nonumber
    ...
    \frac{(q^+)^2}{p^2}\,
    (x+\epsilon\bar{x})
    \bigl[x k^\mu - \bar{x} (p-k)^\mu\bigr]b_\mu\,.
\end{align}
%%%
...
%%%
\begin{align}
  p^2 = \frac{ x \bar x (x^n + {\bar x}^n)^2}{({\bar x}^n x - \bar x x^n)^2} \mathbf{p}^2
\,,\end{align}
%%%
leading to
%%%
\begin{align}
    ...
    &\qquad\times
    \int_0^1\df x\,
    \biggl|\frac{x^n + \bar{x}^n}{\bar{x}^{n-1}-x^{n-1}}\biggr|^{d-4}
    \biggl[\frac{\bar{x}(1+x^2)}{\bar{x}^2 + (\delta_E^+)^2}
        -\epsilon\bar{x}\biggr]\,,
    \\[2ex]
    J_{21,\bar{n}}
    &=
    ...
    \Theta(x)\Theta(\bar{x})
    \frac{x}{\bar{x} - \img\delta_E^+}\,
    \frac{x^n + \bar{x}^n}{\bar{x}^{n-1}-x^{n-1}}
    \,.
\end{align}
%%%
...
\end{align}
%%%
and the integral that appears for $J_{21}$ can be obtained by differentiating with respect to $\mathbf{b}$. 

The result of these steps is the bare unsubtracted jet functions. To obtain the jet functions that enter in the factorization formula, we divide out a square root of the soft factor and remove the UV divergences, as discussed in \secs{OS}{evolution}. At one-loop the overlap factor is given by~\cite{Echevarria:2011epo,Echevarria:2015byo}
%%%
\begin{align}
    \mathcal{S}_\text{LP}(\mathbf{b}^2,\epsilon,\delta^+\delta^-)
    &=
    1-4a_s C_F
...
\end{align}
%%%
and the renormalization factor is given by
%%%
\begin{align}
...
J^{E^n}_{11,\bar{n}}(\mathbf{b}^2)
    ...
    &=
    2a_s C_F
    \bigl(4\ln\de_E \!+\!3\bigr),
    \nn \\
    J^{E^n}_{21,\bar{n}}(x,\mathbf{b}^2)
    &=
    4 a_s C_F\,
    \biggl[
    \Theta(x)
    \Bigl[\frac{\Theta(\bar{x})}{\bar{x}}\Bigr]_+\,
    \frac{x(x^n + \bar{x}^n)}{\bar{x}^{n-1}-x^{n-1}} +
    \de(\bar x) \Bigl(\ln \de_E - \frac{\img\pi}2\Bigr)
    \biggr]
\,,\end{align}
%%%
where
%%%
\begin{align}
  J_{C_F} = 1+ 4 \int_0^1\! \df x\, \frac{1+x^2}{1-x}\, \ln\biggl|\frac{x^n + (1-x)^n}{x^{n-1}-(1-x)^{n-1}}\biggr|
\,.\end{align}
%%%
Note that since $J_{21}$ and $J^\prime_{11}$ only start at order $a_s$, the soft factor subtraction and renormalization does not effect them at this order in perturbation theory. 
The WTA axis corresponds to the $n \to \infty$ limit. In this case, 
%%%
\begin{align}
   \lim_{n \to \infty} J_{C_F} &=
    7-\frac{2\pi^2}{3}-6\log (2)
   \,,\nn \\
   \lim_{n \to \infty} \frac{x(x^n + \bar{x}^n)}{\bar{x}^{n-1}-x^{n-1}}  &=
   \bar{x}x\Theta(\bar{x}-x)-x^2\Theta(x-\bar{x})
\,.\end{align}
%%%

The twist-3 jet function $J_{21}$ is shown in \fig{J21} for $n=2,3$ and $\infty$ (which corresponds to WTA). By dividing $J_{21}$ by $a_s$, we removed dependence on the renormalization scale. Note that these jet functions have a discontinuity at $x=\tfrac12$ arising from the $E^n$ scheme.

% Figure environment removed


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{NLP cross section factorization for $e^+e^-$}
\label{sec:NLP}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

In this section we will combine the NLP factorization of the hadronic tensor in \sec{HT} with the leptonic tensor in \eq{leptonic} to obtain the cross section  for $e^+ e^-\to 2$ jets.
The jet functions that enter were calculated at order $a_s$ in \sec{calculation}, the anomalous dimensions were given in \sec{evolution}, and the hard matching coefficients at order $\alpha_s$ are given here. 

The hadronic tensor was decomposed into several Lorentz structures, see \eq{W_final}. As an intermediate step, we record the following results for the contractions between these Lorentz structures and the leptonic tensor, 
%%%
\begin{alignat}{2}
    (-g_T^{\mu\nu})L_{\mu\nu}
...
\label{eq:finalLW}
    ...
    \nn \\
    &\quad 
    \frac{\pi \alpha_{\rm em}^2Q_q^2 N_c}{Q^4} \Biggl(
    2N_c
    \Bigl(2Q^2\,(1-2y+2y^2)\Bigr)
    \int_0^\infty\df|\mathbf{b}|\,
    \frac{|\mathbf{b}|J_0(|\mathbf{b}||\mathbf{q}|)}{2\pi}\,
    |C_1|^2\,J_{11,\bar{n}}\,J_{11,n}
    \nn \\
    &\qquad
    +2N_c
    \Bigl(4Q_T Q\,\cos(\phi_1)\,(1-2y)\sqrt{y(1-y)}\Bigr)
    \int_0^\infty\df|\mathbf{b}|\,
    \biggr\{
    -\frac{|\mathbf{b}|J_0(|\mathbf{b}||\mathbf{q}|)}{2\pi}\,
    |C_1|^2\,J_{11,\bar{n}}\,J_{11,n}
    ...
    \frac{J_1(|\mathbf{b}||\mathbf{q}|)}{2\pi|\mathbf{q}|}\,
    \biggl[
    \biggl(\int_0^1\df x\, 2C_2^*\,J_{21,\bar{n}}
        - C_1^*\,J^\prime_{11,\bar{n}}\biggr) C_1\,J_{11,n}
    +\text{c.c.}\,
...
\vspace{-2ex}}}\\[2ex]} 
\noindent
The arguments are suppressed for brevity but are the same as in \eq{W_final}. Note that the special rapidity divergences $\ln \de_E$ in \eq{J_res} cancel in the combination of $J_{21}$ and $J_{11}'$ that appear in this expression.

The above expression depends on a combination of jet functions and matching coefficients. For completeness, we provide the expressions for the one-loop matching coefficients here~\cite{Vladimirov:2021hdn}
%%%
\begin{align}
    C_1(q^+q^-,\mu^2)
    &=
    1+a_s C_F\biggl(-\mathbf{L}_Q^2+3\mathbf{L}_Q-8+\frac{\pi^2}{6}\biggr)\,,
    \nn \\[1ex]
    C_2(x,q^+q^-,\mu^2)
    &=
    1+a_s\biggl[
    C_F\biggl(-\mathbf{L}_Q^2+\mathbf{L}_Q-3+\frac{\pi^2}{6}\biggr)
    -C_A\frac{\ln \bar{x} }{x}
    \nn \\
    &\quad\qquad\quad
    -\bigl(C_F-\tfrac{1}{2}C_A\bigr)\bigl(2\mathbf{L}_Q + \ln x - 4\bigr)
        \frac{\ln x}{\bar{x}}\biggr]\,,
\end{align}
%%%
where
%%%
\begin{align}
   \mathbf{L}_Q = \ln \Bigl(\frac{- 2q^+ q^- - \img 0}{\mu^2}\Bigr)
\,.\end{align}
%%%
Both the matching coefficients and the jet functions depend on a renormalization or factorization scale $\mu$, and we have explicitly checked that this dependence cancels out in the cross section 
%%%
\begin{align}
   \frac{\df C_2(x,q^+q^-,\mu)}{\df \ln \mu^2}
   = - \int_0^1\! \df x'\, \ga_{J_{21}}(x',x,\mu,2q^+q^-) + \mathcal{O}(a_s^2)
\,,\end{align}
%%%
with $\ga_{J_{21}}$ given in \eq{ga_J_21}.
This factorization enables the resummation of large logarithms.
...
It is natural to examine how higher twist operators that appear at NLP enter in the description of jets and generate new measurable effects. 
We illustrate all these aspects on a prototypical process, $e^+e^-\rightarrow 2$ jets, which is theoretically simple and experimentally relevant.
The final factorized hadronic tensor  in sec.~\ref{sec:HT} was obtained by starting from its power expansion, decomposing in spin structures and applying discrete symmetries. It involves only one independent twist-2 and twist-3 jet operator matrix element for each collinear direction.  To our knowledge, this is the first time that the contribution of a twist-3 operator to a jet has been determined. 
We have found that  NLP effects exist in $e^+e^-\rightarrow 2$ jets only when the two jets are different, e.g.~when two different recoil-free jet algorithms are used or a jet is replaced by a hadron. 

Elaborating on this idea, we have computed the twist-3 jet functions using WTA and E$^n$ recombination schemes at leading order in perturbation theory.
We have also discussed their overlap subtraction and renormalization.
Consistently with the literature, the overlap is described  by the (leading-power) soft factor. The remaining so-called special rapidity divergences cancel in the combination of the twist-3 and the derivative of twist-2 operator appearing in the cross section in sec.~\ref{sec:NLP}.
We have reduced the evolution equation first obtained at operator level in refs.~\cite{Braun:2009mi,Vladimirov:2021hdn,Rodini:2022wic} to a much simpler expression for our case in sec.~\ref{sec:evolution}.

There are several aspects of this study that we consider relevant for future applications. First of all, the measurement we consider allows for a substantial simplification of the NLP formalism, making it more palatable. The process that we describe can be measured at the Belle experiment, but we leave a detailed phenomenological analysis to future work. It would be intriguing to consider how the nonperturbative effects that we study impact QCD research at the Future Circular Collider (FCC).
More importantly, measurements of hadron structure at the EIC will benefit from our NLP jet analysis, which we consider a priority. Our framework can be naturally extended to also take into account polarized targets at the EIC.


\acknowledgments

We thank Lorenzo Zoppi for collaborating in early stages of this project, and Alexey Vladimirov for discussions on TMDs at next-to-leading power.
This project is supported by the Spanish Ministry grant PID2019-106080GB-C21, European Union Horizon 2020 research and innovation program under grant agreement num.~824093 (STRONG-2020), the NWO projectruimte 680-91-122, and the D-ITP consortium, a program of NWO that is funded by the Dutch Ministry of Education, Culture and Science (OCW).
