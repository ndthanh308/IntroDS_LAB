...
\author{Seunghun Lee\footnote{%
		Einstein Institute of Mathematics, Hebrew University, Jerusalem, Israel. \texttt{seunghun.lee@mail.huji.ac.il}} %
...
In this manuscript, we are interested in colorings of (finite) $k$-uniform hypergraphs embeddable in $\RR^d$. We consider two types of embedding: \textit{geometric (or linear)} and \textit{PL (piecewise linear) embedding}. 
...
Note that Main theorem D follows from Main theorems A and B and the following PL extension theorem by Adiprasito, with a simpler proof and a generalization by Venturello and Yashfe. It generalizes a classical result by Bing for the $d=3$ case~\cite[Theorem I.2.A]{Bing_geometric-topology-3-manifolds}. 
\begin{thm}[\cite{PL-extension-karim-geva}] \label{theorem_PL_extension_karim-geva}
...
The simplicial direction is explored for general simplcial manifolds by Lutz and M\o ller \cite{coloring_surface_lutz}. They showed that (i) $\chi_2(M_g)$, where $M_g$ is a genus $g$ surface, tends to infinity as the genus $g$ tends to infinity, and (ii) $\chi_s(M)=\infty$ when $M$ is a triangulable $d$-dimensional manifold with $d\geq 3$ and $s \leq \lceil d/2\rceil$. Note that Main theorem D 
...
studied in 
\cite{transversals_spheres_joseph_michael, novik_transversal_open_problem, cho2023transversal} for simplicial spheres, and in \cite{nevo_stable_flag}, equivalently, for stable sets in flag spheres.
...
We will need the assertion below, which we suspect to be known. As we failed to find a reference we include its proof.
...
\begin{proof}
	Let
	\begin{align*}	
	P_1\setminus P_2 =&\{a_1, a_2, \dots, a_k \},\;
	P_2\setminus P_1 =\{b_1, b_2, \dots, b_l \},\textrm{ and}\\
	P_1\cap P_2=&\{c_1, c_2, \dots, c_m\}.
	\end{align*}
	By the condition, there is a point $p \in (\conv(P_1)\cap \conv(P_2)) \setminus \conv(P_1\cap P_2)$. That is, we can represent $p$ in two convex combinations
	\begin{align}
	p=\sum_{a_i \in P_1\setminus P_2} \kappa_i a_i+\sum_{c_i \in P_1\cap P_2} \lambda_i c_i = \sum_{b_i \in P_2\setminus P_1} \mu_i b_i+\sum_{c_i \in P_1\cap P_2} \nu_i c_i, \label{eqn_first_convex_comb}
	\end{align}
	where
	\begin{itemize}
		\item[(i)] $\kappa_i$, $\lambda_i$, $\mu_i$ and $\nu_i$ are all nonnegative,
		\item[(ii)] $\sum_{i=1}^k \kappa_i+\sum_{i=1}^m \lambda_i=1$ and $\sum_{i=1}^l \mu_i+\sum_{i=1}^m \nu_i=1$, and
		\item[(iii)] there exist $i\in [k]$ and $j \in [l]$ such that $\kappa_i >0$ and $\mu_j>0$. 
	\end{itemize}
	Now, we split the index set $[m]$ for $P_1\cap P_2$ into two subsets $I_\lambda$ and $I_\nu$, where 
	\begin{align*}
	I_\lambda=\{i \in [m]: \lambda_i\geq \nu_i\}, \textrm{ and }
	I_\nu=\{i \in [m]: \lambda_i< \nu_i\}.
	\end{align*}
	By moving terms in (\ref{eqn_first_convex_comb}) and scaling, we have another pair of convex combinations
	\begin{align}
	\sum_{a_i \in P_1\setminus P_2} \frac{\kappa_i}{K} a_i+\sum_{i \in I_\lambda} \frac{\lambda_i-\nu_i}{K} c_i = \sum_{b_i \in P_2\setminus P_1} \frac{\mu_i}{K} b_i+\sum_{i \in I_\nu} \frac{\nu_i-\lambda_i}{K} c_i, \label{eqn_second_convex_comb}
	\end{align}
	where 
 \[K=\sum_{i=1}^k \kappa_i+\sum_{i \in I_\lambda} (\lambda_i-\nu_i)=\sum_{i=1}^l \mu_i+\sum_{i \in I_\nu} (\nu_i-\lambda_i).\] Note that there are no points appearing in both sides of (\ref{eqn_second_convex_comb}), and at least one of the coefficients should be positive in both sides by (iii). (\ref{eqn_second_convex_comb}) can be reduced into a minimal affine dependency where the points remain at the original side (see \cite[Chapter 6]{lectures_on_polytopes_book}). In that minimal affine dependency, both sides should involve at least one nontrivial term. Let $Q_1$ and $Q_2$ be the subsets of $P_1$ and $P_2$ which have positive coordinates in the left- and the right-hand side, respectively. By the general position assumption on $P_1\cup P_2$, we have $|Q_1\cup Q_2|=d+2$.
\end{proof}
...
We thank Karim Adiprasito and Geva Yashfe for their illustrations of Theorem \ref{theorem_PL_extension_karim-geva}.
