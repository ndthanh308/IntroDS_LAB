...
\usepackage[
 ...
 pdfauthor={Nuprl},
 pdftitle={Open Bars},
 pdfsubject={Open Bars},
 pdfkeywords={Nuprl, Coq, Semantics},
 bookmarks=false
]{hyperref}
\usepackage{cleveref}
...
\newcommand{\Nnoreadmod}[1]{#1_{\not{r}}}
\newcommand{\Nnowritemod}[1]{#1_{\not{w}}}
\newcommand{\Npuremod}[1]{#1_{\not{c}}}
\newcommand{\Nnoreadwritemod}[1]{#1_{\not{r}\not{w}}}
...
\newcommand{\Mch}{c}
\newcommand{\modaa}{\MbarmodSYMB_{\boldsymbol{1}}}
\newcommand{\modab}{\MbarmodSYMB_{\boldsymbol{2}}}
\newcommand{\modad}{\MbarmodSYMB_{\boldsymbol{3}}}
\newcommand{\modae}{\MbarmodSYMB_{\boldsymbol{4}}}
\newcommand{\modaf}{\MbarmodSYMB_{\boldsymbol{5}}}
...
\author[L.~Cohen]{Liron Cohen\lmcsorcid{https://orcid.org/0000-0002-6608-3000}}[a]
\author[V.~Rahli]{Vincent Rahli\lmcsorcid{https://orcid.org/0000-0002-5914-8224}}[b]
...
here:~\myurl{https://github.com/vrahli/opentt/}.
...
We use induction-recursion to define the forcing interpretation
in~\Cref{sec:forcing}, where we use function extensionality to interpret
universes.
We do not discuss this further here and the interested
reader is referred
to~\myhref{https://github.com/vrahli/opentt/blob/master/forcing.lagda}{forcing.lagda}
in the Agda code for further details.
...
a partial function
$\MgetchoiceSYMB\in\Mfun{\Mworld}{\Mfun{\Mcname}{\Mctype}}$.
...
contains two values $\Mca$ and $\Mcb$, which are syntactically different.
\end{defi}
...
modality as follows: let $\Nnowritemod{T}\DEF\Nisect{T}{\NnowriteSYMB}$.
...
(see~\myhref{https://github.com/vrahli/opentt/blob/master/worldInstanceRef.lagda}{worldInstanceRef.lagda}
for details):
\vspace*{-0.07in}\begin{description}
...
$\MextSYMB$:
\begin{lem}[Computations respect $\MextSYMB$]
...
\item $\modae$:
  ...
  {\Mfun
    {\Mallw{\Mw}{P}}
    {\Mbarmod{\Mw}{P}}}$
\item $\modaf$ ($T$, reflexivity axiom):
...
\label{ex:coverings}
\label{def:chains}
The Kripke covering is defined as follows, i.e., $\Mcovn{K}{\Mw}{\Mopen}$
whenever $\Mopen$ contains all the extensions of $\Mw$:
$$\Mcovn{K}{\Mw}{\Mopen}\DEF\Mallww{\Mw}{\Mw'}{\Mapppar{\Mopen}{\Mw'}}$$

The Beth covering is defined as follows, i.e., $\Mcovn{B}{\Mw}{\Mopen}$
...
The Open covering is defined as follows, i.e., $\Mcovn{O}{\Mw}{\Mopen}$
...
$\modaa$ is used to prove monotonicity when $\Meqtypes{\Mw}{T_1}{T_2}$
is derived by closing under $\MbarmodM{\Mw}$; 
$\modab$ and $\modae$ are used, e.g., to prove the symmetry and transitivity of
$\Mintype{\Mw}{t}{t'}{\NUPRLnat}$;
$\modad$ is used to prove locality; and 
$\modaf$ is used to prove consistency.
See~\myhref{https://github.com/vrahli/opentt/blob/master/props3.lagda}{props3.lagda}
for further details.
\end{proof}
...
\item Most types also satisfy
  ...
  except types involving $\Npure$ because it could happen that $a'$ is
  pure but $a$ is not, for example if $A\DEF\Npure$ or
  $A\DEF\Nisect{\Npure}{\NUPRLnat}$,
  $a\DEF\Nsq{\Nread{\Mcn}}{\metanat{0}}$, and $a'\DEF\metanat{0}$.
\end{itemize}
...
Let us now provide a sample of \Btt's key inference rules.
...
An hypothesis $h$ is of the form $\NUPRLhyp{x}{A}$, where the variable
$x$ stands for the name of the hypothesis and $A$ its type.
...
Note that the $\beta$-computation rule is not true for all types $T$. For example
it could be that $T$ is $\Npure$ and~$s$ is~$\Nread{\Mcn}$, in which
case $\NUPRLsuba{t}{x}{s}$ might be pure, while
$\NUPRLapp{(\NUPRLlam{x}{t})}{s}$ would not be. It is however true for
$\NUPRLnat$, $\Nproduct{x}{A}{B}$, $\Nsum{x}{A}{B}$, and
$\NUPRLunion{A}{B}$.%
...
$$\begin{scriptsize}\begin{array}{l}\infer[]
{\Nsequentext{\NUPRL{H},\NUPRLhyp{p}{\Nsum{x}{A}{B}},\NUPRL{J}}{C}{\NUPRLspread{p}{a}{b}{e}}}
{\Nsequentext{\NUPRL{H},\NUPRLhyp{p}{\Nsum{x}{A}{B}},\NUPRLhyp{a}{A},\NUPRLhyp{b}{\NUPRLsuba{B}{x}{a}},\NUPRLsuba{\NUPRL{J}}{p}{\NUPRLpair{a}{b}}}{\NUPRLsuba{C}{p}{\NUPRLpair{a}{b}}}{e}}
\hspace{0.2in}
\infer[]
...
\end{array}\end{scriptsize}$$
As for the $\beta$-computation rule, the above computation rule is
only true for some types $T$, such as the ones listed
above.%
...
$$\begin{scriptsize}\begin{array}{l}\infer[]
{\Nsequentext{\NUPRL{H},\NUPRLhyp{d}{\NUPRLunion{A}{B}},\NUPRL{J}}{C}{\NUPRLdecide{d}{x}{t}{y}{u}}}
{
  \begin{array}{l}
    \Nsequentext{\NUPRL{H},\NUPRLhyp{d}{\NUPRLunion{A}{B}},\NUPRLhyp{x}{A},\NUPRLsuba{\NUPRL{J}}{d}{\NUPRLinl{x}}}{\NUPRLsuba{C}{d}{\NUPRLinl{x}}}{t}
    \hspace{0.2in}
    \Nsequentext{\NUPRL{H},\NUPRLhyp{d}{\NUPRLunion{A}{B}},\NUPRLhyp{y}{B},\NUPRLsuba{\NUPRL{J}}{d}{\NUPRLinr{y}}}{\NUPRLsuba{C}{d}{\NUPRLinr{y}}}{u}
  \end{array}
}
\\ \\
\infer[]
...
\end{array}\end{scriptsize}$$
As for the $\beta$-computation rule, the above computation rules are
only true for some types $T$, such as the ones listed
above.%
...
$$\begin{scriptsize}\begin{array}{l}\infer[]
{\Nsequentext{\NUPRL{H},\NUPRLhyp{x}{\NUPRLnat},\NUPRL{J}}{C}{\Nnatrec{x}{b}{\NUPRLlam{y}{\NUPRLlam{r}{s}}}}}
{
  \Nsequentext{\NUPRL{H},\NUPRLhyp{x}{\NUPRLnat},\NUPRL{J}}{\NUPRLsuba{C}{x}{\metanat{0}}}{b}
  &
  \Nsequentext{\NUPRL{H},\NUPRLhyp{x}{\NUPRLnat},\NUPRL{J},\NUPRLhyp{y}{\NUPRLnat},\NUPRLhyp{r}{\NUPRLsuba{C}{x}{y}}}{\NUPRLsuba{C}{x}{\Nsucc{y}}}{s}
}
\\ \\
\infer[]
...
they are consistent with the Law of Excluded Middle (LEM)
(see~\myhref{https://github.com/vrahli/opentt/blob/master/lem.lagda}{lem.lagda}),
and other instances that are incompatible with classical reasoning, in
that they allow validating the \emph{negation} of LEM
(see~\myhref{https://github.com/vrahli/opentt/blob/master/not_lem.lagda}{not\_lem.lagda}),
the Limited Principle of Omniscience
(LPO)~\cite[p.9]{bishop1967foundations}
(see~\myhref{https://github.com/vrahli/opentt/blob/master/not_lpo.lagda}{not\_lpo.lagda}),
and Markov's Principle (MP)
(see~\myhref{https://github.com/vrahli/opentt/blob/master/not_mp.lagda}{not\_mp.lagda}).%
...
(see~\mylemma{continuity}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity7.lagda}{continuity7.lagda}
for details).
...
(see~\myhref{https://github.com/vrahli/opentt/blob/master/continuity-conds.lagda}{continuity-conds.lagda}
for details):
$$\begin{array}{l@{\hspace{0.1in}}l}
(\assa) & \Mallb{\Mw}{\Mworld}{P}{\Mwpred{\Mw}}
{\Mfun
  {\Mbarmod{\Mw}{P}}
  {\Mexw{\Mw}{P}}
}
...
{\Mfun
  {\Mcompatible{\Mcn}{\Mw}{\Mr}}
  {\Mgetchoice{\Mchoose{\Mw}{\Mcn}{\metanat{k}}}{\Mcn}=k}
}
\end{array}$$
...
$\Mcov{\Mw}{\Mopen}$ then $\Mexww{\Mw}{\Mw'}{\Mapppar{\Mopen}{\Mw'}}$
-- see~\mylemma{${\rightarrow}{\exists}\mathbb{W}$}
in~\myhref{https://github.com/vrahli/opentt/blob/master/mod.lagda}{mod.lagda}),
...
(see for example~\myhref{https://github.com/vrahli/opentt/blob/master/contInstanceKripkeRef.lagda}{contInstanceKripkeRef.lagda}
for the proof that \Btt instantiated with a Kriple modality and
references satisfies these properties).
...
in~\cite{Cohen+Rahli:fscd:2022} (see for
example~\myhref{https://github.com/vrahli/opentt/blob/master/contInstanceKripkeCS.lagda}{contInstanceKripkeCS.lagda}
for the proof that \Btt instantiated with a Kriple modality and choice
sequences satisfies these properties).
...
precisely, we prove the following (see~\mylemma{testM-NAT}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity1.lagda}{continuity1.lagda}
for details):
\begin{thm}[The Modulus is a Number]
...
example, using $\modae$ from \Cref{label:mod-props} on \Cref{equ:concl-mod-nat-a}), then we need to
...
\begin{itemize}
\item (A) first have to find a number $n$
...
(see~\mylemma{differNF${\Downarrow}$APPLY-upd}
in~\myhref{https://github.com/vrahli/opentt/blob/master/terms7.lagda}{terms7.lagda}).
...
Let us now define the simulation mentioned above (see~\mylemma{differ}
in~\myhref{https://github.com/vrahli/opentt/blob/master/terms3.lagda}{terms3.lagda}
for details):
\begin{defi}
\label{def:sim1}
...
(see~\mylemma{differ$\Downarrow$}
in~\myhref{https://github.com/vrahli/opentt/blob/master/terms6.lagda}{terms6.lagda}
for details):
\begin{lem}
...
(see~\mylemma{steps-sat-isHighest$\nat$}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity3.lagda}{continuity3.lagda}
for details):%
\begin{thm}[The Modulus is the Highest Number]
If
$\NUPRLcomputestopp
{\Mw}
{\Mw'}
...
(see~\myhref{https://github.com/vrahli/opentt/blob/master/continuity3.lagda}{continuity3.lagda}):
\begin{lem}
\label{lem:highest}
...
\subsection{The Modulus is the Modulus}

We now prove the crux of continuity, namely that $F$ returns the same
number on functions that agree up to $\Nmod{F}{\Mba}$
(see~\mylemma{eqfg}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity6.lagda}{continuity6.lagda}
for details):
\begin{thm}[The Modulus is the Modulus]
...
$\NUPRLapppar{F}{\beta}$ allows getting access to the values that
...
computation, which are applications of $\beta$ in the second, and that
these applications reduce to the same numbers because the arguments,
recorded using $\Mcn$, are less than $\Nmod{F}{\Mba}$.
...
(see~\mylemma{updRel}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity4.lagda}{continuity4.lagda}
for details):
\begin{defi}
\label{def:sim2}
...
$\NUPRLcomputestopp{\Mw}{\Mw'}{t_1}{t'_1}$ (see~\mylemma{steps-updRel}
in~\myhref{https://github.com/vrahli/opentt/blob/master/continuity5.lagda}{continuity5.lagda}
for details):
\begin{lem}
