...
\end{align*}
where infimum is taken over all stochastic plans $\pi$ satisfying (\ref{eq:marginal1}) and (\ref{eq:marginal2}).  
...
\end{align}
where identity (\ref{eq:green}) is used.

The score functions $S_i ({\bm{x}}, {\bm{\theta}})$  give a set of estimating functions from (\ref{eq:escore}), which are used to obtain an estimator $\hat{\bm{\theta}}$.  Let $\hat{p}_{\rm{emp}}({\bm{x}})$ be the empirical distribution given by
\begin{align*}
	\hat{p}_{\rm{emp}}({\bm{x}}) = \frac 1n
	\sum^n_{j=1} \delta
	\left({\bm{x}}-{\bm{x}}_j \right),
\end{align*}
where ${\bm{x}}_1, \cdots, {\bm{x}}_n$ are $n$ independent observations.  Then, replacing expectation ${\rm{E}}$ in (\ref{eq:escore}) by the expectation with respect to the empirical distribution, we have estimating equations, 
\begin{align*}
	{\rm{E}}_{\rm{emp}} 
...
\begin{theorem}\upshape
    ...
    For the elliptically symmetric deformation model \eqref{eq:elliptic}, the Wasserstein score functions are quadratic. 
...
\begin{corollary}\upshape \label{cor_ellip}
    For the elliptically symmetric deformation model \eqref{eq:elliptic}, the $W$-estimator $\hat{\bm{\theta}}_W$ is the second-order moment estimator irrespective of the waveform $f(\bm{z})=g(\| \bm{z} \|)$.  
\end{corollary}
\begin{proof}
...
Note that \cite{Gelbrich} showed that the $L^2$-Wasserstein divergence for the elliptically symmetric deformation model \eqref{eq:elliptic} does not depend on the waveform and is given by
\[
...
Regarding the geometric structure of the elliptically symmetric deformation model \eqref{eq:elliptic}, we obtain the following.
See Figure~\ref{fig1}.
