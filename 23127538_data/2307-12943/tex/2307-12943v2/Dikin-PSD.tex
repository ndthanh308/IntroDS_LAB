...
$i\in[m]$, 
\begin{align}
\text{uniformly sample \ensuremath{X} subject to } & \inner{A_{i},X}\geq b_{i}\ \forall i\in[m]\text{ and }X\succeq0.\label{eq:PSDcone}
\end{align}
\end{problem*}
...
\vec(A_{1}) & \cdots & \vec(A_{m})\end{array}\right]^{\top},\,S_{X}:=\Diag\Par{\inner{A_{i},X}-b_{i}},\,A_{X}:=S_{X}^{-1}A.
\]
The metric below comes from the Hessian of the following function:
\[
-2n\log\det X-2\sum_{i=1}^{m}\log\Par{\inner{A_{i},X}-b_{i}}.
\]
...
i.e., $\sigma(A_{X})_{i}:=\Par{P_{X}}_{ii}$ for $i=1,\dots,m$. We
...
weight of $A_{X}$. By setting $p=O(\log m)$, the $\dw$ with a Lewis
weights metric achieves poly-logarithmic dependence on $m$.
...
metric is significantly faster than the state-of-the-art (the $\bw$)
for every value of $m$, the number of affine constraints. 
...
metrics (e.g., log-barriers to Vaidya metrics to Lewis weights metrics)
can also be realized for PSD sampling, albeit with considerably more
technical challenges. The $\dw$ with the Vaidya metric and Lewis
weights metric (along with $\hess\phi_{1}$) mixes in $\otilde{(n+\sqrt{m})n^{3}}$
...
in Section~\ref{sec:LS-psdSampling}. Notably, by setting $g_{1}$
to $0$, our framework recovers the mixing time of the \textsf{Vaidya walk}
...
(Lemma~\ref{lem:paramsBarrier}), and $(\log m)^{O(1)}$ for Lewis
weights metrics (Lemma~\ref{lem:LSmetricStrongandSymmetry}). For
lower trace self-concordance, $\sqrt{m/n}$-scaling is sufficient
for Vaidya metrics (Lemma~\ref{lem:hybridLowerSCTrace}), while Lewis
weights metrics require an additional scaling of $\sqrt{n}$ along
with $(\log m)^{O(1)}$ (Lemma~\ref{lem:LSLowerSCTrace}).

In analyzing the Vaidya and Lewis weights metrics, a main technical
...
self-concordance of $g$. For the Lewis weights metric, the situation
is more complicated due to numerous terms appearing in $D^{2}W_{x}[h,h]$.
In order to avoid dealing with each of the terms, we employ existing
bounds on derivatives of $W_{x}$ and other relevant matrices. This
strategy significantly simplifies the computation but comes at the
cost of an additional scaling of $\sqrt{n}$, which as far as we can
tell might be unavoidable.
