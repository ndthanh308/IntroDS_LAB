...
PSD constraint $X\succeq0$, and $-\sum_{i}\log\Par{\inner{A_{i},X}-b_{i}}$
(the standard log barrier) for linear constraints $\inner{A_{i},X}\geq b_{i}$
...
the barrier $n\phi(X)=-n\log\det X$ is $n^{2}$-symmetric.
\end{lem}
...
To handle both constraints of $X\succeq0$ and $\inner{A_{i},X}\geq b_{i}$
in (\ref{eq:PSDcone}), we use the metric $g(X)$ induced by a barrier
$\phi(X):=-2\Par{n\log\det X+\sum_{i=1}^{m}\log\Par{\inner{A_{i},X}-b_{i}}}$.
To be precise, 
\begin{align*}
...
\end{align*}
where $S_{X}=\Diag\Par{\inner{A_{i},X}-b_{i}}\in\R^{m\times m}$,
...
of $\psi(X):=-\sum_{i=1}^{m}\log\Par{\inner{A_{i},X}-b_{i}}$, since
...
barrier for $P=\{x\in\R^{d}:Ax\geq b\}$ with $A\in\R^{m\times d}$
...
same with $K=\{X\succeq0\}\cap\{\inner{A_{i},X}\geq b_{i}\}$. Hence,
...
\subsection{Implementation of each step \label{subsec:oracleImplementation}}
