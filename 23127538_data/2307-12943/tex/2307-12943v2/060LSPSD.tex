...
\section{Poly-logarithmic dependence on $m$ via the Lewis weights metric
\label{sec:LS-psdSampling}}
...
and symmetry of the Lewis weights metric in Section~\ref{subsec:sc-and-symm-LS}
...
be the Lewis weights metric, and $h\in\Rn$.
\begin{itemize}
...
Lewis weights metric with the $\ell_{p}$-Lewis weight $w_{x}$ with
...
of $A_{x}$.
\begin{lem}
[\cite{lee2019solving}, Lemma 24] The directional derivative of
the $\ell_{p}$-Lewis weight $W_{x}$ in direction $h\in\Rn$ is
\[
...
\]
\end{proof}
We recall bounds on the derivatives of matrices relevant to Lewis
weights.
\begin{lem}
[\cite{lee2019solving}] \label{lem:LS-comp-tool} For $P=\Brace{x\in\Rn:Ax\geq b}$
with $A\in\R^{m\times n}$ and $b\in\R^{m}$, let $x\in\text{int}(P)$
...
We can check lower trace self-concordance by using the above bounds.
\begin{lem}
...
\end{align}
where we used the Cauchy-Schwartz inequality in (i) and Lemma~\ref{lem:usefulFactLewis}-3
in (ii).
...
\begin{align*}
\norm{\text{I}}_{W_{x}^{-1}} & =\norm{W_{x}^{-1}W_{x,h}'N_{x}W_{x}^{\half}s_{x,h}}_{2}\\
...
For the third term, 
\begin{align*}
\norm{\text{III}}_{W_{x}^{-1}} & =\norm{N_{x}W_{x}^{-\half}W_{x,h}'s_{x,h}}_{2}\\
