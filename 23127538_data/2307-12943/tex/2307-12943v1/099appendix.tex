...
the logarithmic barrier, $\phi_{\log}(X):=-\sum_{i=1}^{m}\log\Par{b_{i}-\inner{A_{i},X}}$.
...
\end{array}\right]M\\
 & =M^{\top}A^{\top}S_{X}^{-2}AM,
\end{align*}
where $S_{X}=\Diag(b_{i}-\inner{A_{i},X})\in\R^{m\times m}$ and $A^{\top}=\left[\begin{array}{ccc}
...
\begin{proof}
The first is obvious from differentiation of $S_{x}=\Diag(Ax-h)$
...
a convex region $Ax\leq b$ defined by 
\[
