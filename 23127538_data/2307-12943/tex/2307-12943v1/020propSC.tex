...
and thus $\onu=O(\nu^{2})$.
\end{proof}
For a polytope ($K=\{x:Ax\leq b\}$), the first inclusion above has
a useful equivalent description as follows:
\begin{lem}
\label{lem:symmforPolytope} Let $x\in K=\{Ax\leq b\}$ be a polytope.
It holds that $y\in K\cap(2x-K)$ if and only if $\norm{A_{x}(y-x)}_{\infty}\leq1$.
\end{lem}

\begin{proof}
For $y\in K$, we have $Ay\leq b$ and thus $A(y-x)\leq b-Ax=s_{x}$
(elementwise inequality). As $s_{x}\geq0$, we have $A_{x}(y-x)\leq1$.
When $y\in(2x-K)$, we can write $y=2x-z$ for some $z\in K$. Note
that
\[
A(y-x)=A(x-z)\geq Ax-b=-s_{x},
\]
and thus $A_{x}(y-x)\geq-1$. Therefore, $\norm{A_{x}(y-x)}_{\infty}\leq1$.
\end{proof}
...
We finish this section by providing sufficient conditions under which
the sum of metrics satisfies lower trace self-concordance.
\begin{lem}
\label{lem:additiveCondition}Let $g_{1}$ and $g_{2}$ be matrix
functions from $\R^{n}$ to $\R^{n\times n}$. If $D^{2}g_{1}[h,h]\succeq0$
and $\tr\Par{(g_{1}+g_{2})^{-1}D^{2}g_{2}[h,h]}\geq-\norm h_{g_{1}+g_{2}}^{2}$,
then $g_{1}+g_{2}$ is lower trace self-concordant.
\end{lem}

We note that the first condition is \emph{additive. }Also, a special
case of the lemma is that if $D^{2}g_{1}[h,h]\succeq0$ and $D^{2}g_{2}[h,h]\succeq0$,
then $g_{1}+g_{2}$ is lower trace self-concordant.\emph{ }
