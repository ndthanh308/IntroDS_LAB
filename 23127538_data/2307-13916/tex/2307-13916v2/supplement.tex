...
%\title{Appendices: Supplementary materials for \\
...
\hline
\hline
0.01 & 0.1 & 0.047 (0.0015) & 0.046 (0.0015) & 0.027 (0.0011) & 0.038 (0.0013) \\
0.1  & 0.1 & 0.047 (0.0015) & 0.047 (0.0015) & 0.026 (0.0011) & 0.039 (0.0013) \\
1.0  & 0.1 & 0.048 (0.0015) & 0.048 (0.0015) & 0.027 (0.0011) & 0.038 (0.0013) \\
0.01 & 1.0 & 0.757 (0.0164) & 0.647 (0.0145) & 0.198 (0.0079) & 0.371 (0.0107) \\
0.1  & 1.0 & 0.769 (0.0160) & 0.721 (0.0156) & 0.205 (0.0080) & 0.392 (0.0110) \\
1.0  & 1.0 & 0.714 (0.0155) & 0.697 (0.0150) & 0.218 (0.0083) & 0.404 (0.0112) \\
0.01 & 2.0 & 1.492 (0.0281) & 1.504 (0.0283) & 0.358 (0.0129) & 0.616 (0.0169) \\
0.1  & 2.0 & 1.195 (0.0244) & 1.333 (0.0260) & 0.368 (0.0131) & 0.584 (0.0164) \\
1.0  & 2.0 & 1.299 (0.0257) & 1.476 (0.0277) & 0.416 (0.0139) & 0.625 (0.0170)\\
\hline
\hline
\end{tabular}
\end{subtable}
...
\hline
\hline
0.05 & 0.1 & 0.027 (0.0067) & 0.027 (0.0070) & 0.022 (0.0057) & 0.024 (0.0058) \\
0.1  & 0.1 & 0.026 (0.0057) & 0.024 (0.0053) & 0.020 (0.0046) & 0.020 (0.0046) \\
5.0  & 0.1 & 1.030 (0.0287) & 0.743 (0.0262) & 0.831 (0.0267) & 1.173 (0.0343) \\
0.05 & 1.0 & 0.412 (0.0355) & 0.408 (0.0351) & 0.117 (0.0148) & 0.112 (0.0143) \\
0.1  & 1.0 & 0.309 (0.0293) & 0.316 (0.0299) & 0.085 (0.0112) & 0.087 (0.0116) \\
5.0  & 1.0 & 1.321 (0.0417) & 0.918 (0.0304) & 1.458 (0.0422) & 1.322 (0.0388) \\
0.05 & 2.0 & 0.660 (0.0343) & 0.634 (0.0322) & 0.144 (0.0129) & 0.148 (0.0133) \\
0.1  & 2.0 & 0.740 (0.0505) & 0.704 (0.0489) & 0.151 (0.0145) & 0.155 (0.0149) \\
5.0  & 2.0 & 1.585 (0.0454) & 2.415 (0.0816) & 1.577 (0.0508) & 1.436 (0.0462)
\end{tabular}
\end{subtable}
...
For any $t\in[T]$, let $q_t = \inf_{\tau\in[t], a\in\cA}\pi_\tau(a|\tilde\xb_\tau, \cH_{\tau-1})$. Then under Assumption \ref{ass:boundedness} and \ref{ass:min-signal}, there exist constants $C$ and $C_1$ such that as long as $q_t\geq \frac{C_1}{\lambda_0\wedge\lambda_0^2}\frac{d+\log t}{t}$, with probability at least $1-\frac{4K}{t^2}$,
$$
...
(i) For the standard setting, $\forall T\geq C'\max\{1,$ $ (\frac{d+\log T}{1\wedge\lambda_0\wedge\lambda_0^2})^{\frac94}\}$, with probability at least $1\!-\!\frac{8K}{\sqrt{T}}$, Algorithm \ref{alg1-multiple-actions} with the choice of $T_0 = \lceil 2T^{\frac23}\rceil$, $p_0^{(t)} = \min\{\frac12, t^{-\frac13}\}$ outputs a policy with $\text{Regret}(T; \pi^*)$ no more than
$$
C T^{\frac23}\bigg\{\frac{(K-1)R_\theta}{1\!-\!\rho} + \frac{R\!+\!R_{\theta}}{(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\}.
$$

(ii) For the clipped policy setting, $\forall T\geq C'\max\{1, \!\frac{d^2\!+\log^2({T})}{p_0^2}, \!\frac{d^2\!+\log^2({T})}{\lambda_{0}^2(1\wedge \lambda_{0}^2)p_0^2}\}$, with probability at least $1\!-\!\frac{8K}{\sqrt{T}}$, Algorithm \ref{alg1-multiple-actions} with the choice of $T_0 = \lceil2\sqrt{T}\rceil$ and  $p_0^{(t)}\equiv p_0$ outputs a policy with $\text{Regret}(T; \bar\pi^*)$ no more than 
$$
CT^{\frac12}\bigg\{R_\theta + \frac{(1\!-\!Kp_0)(R\!+\!R_\theta)}{\sqrt{p_0}(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\}.
$$
\end{corollary}
...
\begin{itemize}
    ...
    \item $\{\xb_\tau\}$ is a weakly-dependent stationary time series (a common example is the multivariate ARMA process under regularity conditions, see e.g. \citep{banna2016bernstein}). The stationary distribution $P$ satisfies $\EE_{\xb\sim P}\xb\xb^\top\succeq \lambda_2I_d$, $\lambda_2>0$;
...
\begin{align}
\|\hat\btheta_a^{(t-1)}-\btheta_a^*\|_2
&\leq \frac{C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log (t-1)}{(t-1)^{\frac23}}, \sqrt{\frac{d+\log (t-1)}{(t-1)^{\frac23}}}\right\}\nonumber\\
&\leq \frac{C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log t}{(t/2)^{\frac23}}, \sqrt{\frac{d+\log t}{(t/2)^{\frac23}}}\right\}\nonumber\\
&\leq \frac{2C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log t}{t^{\frac23}}, \sqrt{\frac{d+\log t}{t^{\frac23}}}\right\}.\label{eq:standard-benchmark-estimation-error}
\end{align}

If in addition, for all $t\geq T_0$,
\begin{equation}\label{eq:standard-benchmark-condition2}
\frac{d+\log t}{t^{\frac23}}\leq 1,
\end{equation}
then with probability at least $1-\frac{8}{t^2}$, for $a\in\{0, 1\}$,
$$
\|\hat\btheta_a^{(t-1)}-\btheta_a^*\|_2\leq \frac{2C(R+R_\theta)}{\lambda_0}\sqrt{\frac{d+\log t}{t^{\frac23}}}.
$$
Plugging in the above to Theorem \ref{thm-regret}, we have that with high probability, 
\begin{align*}
\text{Regret}(T;\pi^*)
&\leq 2R_\theta\cdot \lceil 2T^{2/3}\rceil + \frac2{1-\rho}\sum_{t>T_0}\left(\frac{R_\theta}{t^{1/3}} + \frac{2C(R+R_\theta)}{\lambda_0} \sqrt{\frac{d+\log t}{t^{\frac23}}}\right)\\
&\leq C_1\left\{\frac{R_\theta}{1-\rho}T^{2/3} + \frac{R+R_\theta}{(1-\rho)\lambda_0}T^{2/3}\sqrt{d+\log T}\right\}
\end{align*}
for a universal constant $C_1$. Finally, conditions (\ref{eq:standard-benchmark-condition1}) and (\ref{eq:standard-benchmark-condition2}) can be guaranteed by $T\geq C_2\max\{1, (d+\log T)^{\frac94}, \big(\frac{d+\log T}{\lambda_0\wedge\lambda_0^2}\big)^{\frac94}\}$ for some universal constant $C_2$. We conclude the proof.

\noindent\textbf{Clipped policy setting.} Under the assumptions of the Corollary \ref{cor:alg1-with-theta-estimation}, we first apply a union bound to Theorem \ref{thm:theta-estimation} to obtain that with probability at least $1-8/\sqrt{T}$, for all $t\geq \max\{\sqrt{T}, \frac{C_1}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}\}$, for all $a\in\{0, 1\}$,
...
\end{equation}
At the same time, it's easy to verify that as long as $T\geq \max\{10, \frac{C'[d+\log(\sqrt{T})]^2}{\min\{\lambda_{0}^2, \lambda_{0}^4\}p_0^2}\}$ with $C'=C_1^2$ as in Theorem \ref{thm:theta-estimation}, $\forall t\geq \sqrt{T}$, $t\geq \frac{C_1}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}$. This implies that $\forall t>T_0$, $t/2\geq\max\{\sqrt{T}, \frac{C_1}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}\}$. Then, with high probability, for all $t>T_0$,
\begin{align*}
\|\hat\btheta_a^{(t-1)}-\btheta_a^*\|_2
&\leq \frac{C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log (t-1)}{p_0(t-1)}, \sqrt{\frac{d+\log (t-1)}{p_0(t-1)}}\right\}\\
&\leq \frac{C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log t}{p_0t/2}, \sqrt{\frac{d+\log t}{p_0t/2}}\right\}\\
&\leq \frac{2C(R+R_\theta)}{\lambda_0}\max\left\{\frac{d+\log t}{p_0t}, \sqrt{\frac{d+\log t}{p_0t}}\right\}.
\end{align*}
Moreover, one can also verify that as long as $T\geq \max\{10, \frac{[d+\log(\sqrt{T})]^2}{p_0^2}\}$, we have $\forall t>2\sqrt{T}$, $\frac{d+\log t}{p_0\cdot t}\leq 1$, which indicates that 
$$
\|\hat\btheta_a^{(t-1)}-\btheta_a^*\|_2
\leq\frac{2C(R+R_\theta)}{\lambda_0}\sqrt{\frac{d+\log t}{p_0t}}.
$$
Plugging in this result to Theorem \ref{thm-regret}, we obtain that with high probability, 
\begin{align*}
\text{Regret}(T; \bar\pi^*)
&\leq 2T_0R_\theta + \frac{2(1-2p_0)}{1-\rho}\sum_{t>T_0}\frac{2C(R+R_\theta)}{\lambda_0}\sqrt{\frac{d+\log t}{p_0t}}\\
&\leq C_1\sqrt{T}\left\{ R_\theta + \frac{(1-2p_0)(R+R_\theta)}{\sqrt{p_0}(1-\rho)\lambda_0}\sqrt{d+\log T}\right\}
\end{align*}
for a universal constant $C_1$.

\noindent\textbf{Results with a high-probability version of Assumption \ref{ass:small-error}.} Recall that Assumption \ref{ass:small-error-high-probability} is a weakened version of Assumption \ref{ass:small-error} with a high-probability statement. Given Assumptions \ref{ass:boundedness}, \ref{ass:min-signal}, and \ref{ass:small-error-high-probability} (instead of \ref{ass:small-error}), in the standard setting, the results of Theorem \ref{cor:alg1-with-theta-estimation} hold as long as (\ref{eq:thm:theta-estimation-union}) for all $t>T_0$, $a\in\{0, 1\}$ and under the event $\cap_{t = T_0+1}^TA_t$. Thus, we deduce that the regret upper bound in (i) hold with probability at least $1-16/\sqrt{T} - c_e$. Similarly, given Assumptions \ref{ass:boundedness}, \ref{ass:min-signal}, and \ref{ass:small-error-high-probability}, in the clipped benchmark setting, the regret upper bound in (ii) hold with probability at least $1-16/\sqrt{T} - c_e$.
...
\begin{itemize}
    \item[(i)] In the standard setting, as long as the set of model update times $\cS$ satisfies: (a) $s_{\min}\leq T^{2/3}$; (b) $\forall t\in(T^{2/3}, T]$, $s_t = \max\{s\in\cS: s<t\} \geq \alpha t$ for some constant $\alpha\in(e^{-d}, 1)$, then $\forall T\geq C'\max\{1, (\frac{d+\log T}{1\wedge\lambda_0\wedge\lambda_0^2})^{9/4}\}$, with probability at least $1-\frac{16}{\sqrt{T}}$, Algorithm \ref{alg2} with the choice of $p_0^{(t)} = \min\{\frac12, t^{-1/3}\}$ achieves $\text{Regret}(T; \pi^*)\leq CT^{\frac23}\left\{\frac{R_\theta}{\alpha}+\frac{R_\theta}{1-\rho}+\frac{R+R_\theta}{\alpha^{1/3}\lambda_{0}(1-\rho)}\sqrt{{d}+{\log T}}\right\}$.
    \item[(ii)] In the clipped policy setting, as long as the set of model update times $\cS$ satisfies: (a) $s_{\min}\leq \sqrt{T}$; (b) $\forall t\in(\sqrt{T}, T]$, $s_t = \max\{s\in\cS: s<t\} \geq \alpha t$ for some constant $\alpha\in(e^{-d}, 1)$, then for any $T$ s.t. $T\geq C'\max\{1, \frac{d^2+\log^2({T})}{p_0^2}, \frac{d^2+\log^2({T})}{\min\{\lambda_{0}^2, \lambda_{0}^4\}p_0^2}\}$, with probability at least $1-\frac{16}{\sqrt{T}}$, Algorithm \ref{alg2} with the choice of $p_0^{(t)}\equiv p_0$ achieves: $\text{Regret}(T; \bar\pi^*)\leq C\sqrt{T}\left\{\frac{R_\theta}{\alpha}+\frac{(1-2p_0)(R+R_\theta)}{\lambda_{0}(1-\rho)\sqrt{\alpha p_0}}\sqrt{{d}+{\log T}}\right\}$.
\end{itemize}
\end{corollary}

The proof of Corollary \ref{cor:alg1-with-theta-estimation-S} can be directly obtained by combining Theorem \ref{thm:theta-estimation} and \ref{thm-regret-S} with $T_0 = T^{2/3}/\alpha$ in the standard setting and $T_0 = T^{1/2}/\alpha$ in the clipped benchmark setting. Thus, the proof is omitted here.
...
\begin{itemize}
    \item[(i)] In the standard setting, as long as $\max_{t\in[T^{2/3}, T]}\max_{a\in\{0, 1\}}\|\bDelta_t(a)\|_2\leq \frac{\lambda_0}{4}$, $T\geq C''\max\{1, (\frac{d+\log T}{1\wedge\lambda_0\wedge\lambda_0^2})^{\frac94}\}$, by choosing $T_0 = \lceil2T^{\frac23}\rceil$, $p_0^{(t)} = \min\{\frac12, t^{-\frac13}\}$, and (\ref{eq::proposed-estimator-estimated-Sigma}) instead of (\ref{eq:proposed-estimator}) in Algorithm \ref{alg1}, with probability at least $1-\frac{16}{\sqrt{T}}$,
$$
\text{Regret}(T;\pi^*)\leq C T^{\frac23}\bigg\{\frac{R_\theta}{1\!-\!\rho} + \frac{R\!+\!R_{\theta}}{(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\} + C\frac{R_\theta}{(1-\rho)\lambda_0}\sum\limits_{t\in[2T^{2/3}-1, T)\cap \mathbb N}\max\limits_{a\in\{0, 1\}}\|\bDelta_{t}(a)\|_2.
$$
    
    \item[(ii)] In the clipped policy setting, as long as $\max_{t\in[\sqrt{T}, T]}\max_{a\in\{0, 1\}}\|\bDelta_t(a)\|_2\leq \frac{\lambda_0}{4}$,\\$T\geq C''\max\{1, \frac{d^2+\log^2({T})}{p_0^2}, \frac{d^2+\log^2({T})}{\lambda_{0}^2(1\wedge \lambda_{0}^2)p_0^2}\}$, by choosing $T_0 = \lceil2\sqrt{T}\rceil$, $p_0^{(t)}\equiv p_0$, and (\ref{eq::proposed-estimator-estimated-Sigma}) instead of (\ref{eq:proposed-estimator}) in Algorithm \ref{alg1}, with probability at least $1-\frac{16}{\sqrt{T}}$, 
$$
\text{Regret}(T;\bar\pi^*)\leq CT^{\frac12}\bigg\{R_\theta + \frac{(1\!-\!2p_0)(R\!+\!R_\theta)}{\sqrt{p_0}(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\}+C\frac{(1\!-\!2p_0)R_\theta}{(1-\rho)\lambda_0}\sum\limits_{t\in[2\sqrt{T}-1, T)\cap \mathbb N}\max\limits_{a\in\{0, 1\}}\|\bDelta_{t}(a)\|_2.
$$
\end{itemize}
\end{corollary}
...
\noindent\textbf{Standard setting.} 
We first apply a union bound to Theorem \ref{thm:theta-estimation-estimated-Sigma} to obtain that with probability at least $1-8/\sqrt{T}$, for all $t\geq \max\{T^{2/3}, \frac{C_1'}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0^{(t)}}\}$, for all $a\in\{0, 1\}$,
...
\end{equation}
Meanwhile, it's easy to verify that as long as $T\geq C''\max\{1, (\frac{d+\log T}{1\wedge\lambda_0\wedge\lambda_0^2})^{\frac94}\}$ for an appropriate constant $C''$, $\forall t\geq T^{2/3}$, $t\geq \frac{C_1'}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0^{(t)}}$. Combining the results in Theorem \ref{thm-regret} (with (\ref{eq::proposed-estimator-estimated-Sigma}) instead of (\ref{eq:proposed-estimator})) and \ref{thm:theta-estimation-estimated-Sigma}, we obtain that the regret of Algorithm \ref{alg1} with $(\tilde\btheta_a^{(t)})_{a\in\{0, 1\}, t\in[T]}$ as plug-in estimators:
\begin{align*}
\text{Regret}(T;\pi^*)&\leq 2T_0R_\theta + \frac{2}{1-\rho}\sum_{t = T_0+1}^T\bigg[t^{-\frac13}R_\theta + 
\frac{C(R+R_\theta)}{\lambda_0}\max\bigg\{\frac{d+\log(t-1)}{p_0^{(t-1)}(t-1)}, \sqrt{\frac{d+\log(t-1)}{p_0^{(t-1)}(t-1)}}\bigg\} \\
&\quad + \frac{2R_\theta}{\lambda_0}\max_a\|\bDelta_{t-1}(a)\|_2\bigg]\\
&\leq C_1 T^{\frac23}\bigg\{\frac{R_\theta}{1\!-\!\rho} + \frac{R\!+\!R_{\theta}}{(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\} + C_1\frac{R_\theta}{(1-\rho)\lambda_0}\sum\limits_{t\in[2T^{2/3}-1, T)\cap \mathbb N}\max\limits_{a\in\{0, 1\}}\|\bDelta_{t}(a)\|_2.
\end{align*}
for a universal constant $C_1$. Here we simplify the first terms using proof of Corollary \ref{cor:alg1-with-theta-estimation}.

\noindent\textbf{Clipped policy setting.} Similar to the standard setting, we apply a union bound to Theorem \ref{thm:theta-estimation-estimated-Sigma} to obtain that with probability at least $1-8/\sqrt{T}$, for all $t\geq \max\{\sqrt{T}, \frac{C_1'}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}\}$, for all $a\in\{0, 1\}$,
\begin{equation}
% \label{eq:thm:theta-estimation-union-estimated-Sigma}
    \|\tilde\btheta_a^{(t)}-\btheta_a^*\|_2\leq \frac{C(R+R_{\btheta})}{\lambda_{0}}\max\left\{\frac{d+\log t}{p_{0}t}, \sqrt{\frac{d+\log t}{p_{0}t}}\right\} + \frac{2R_\theta}{\lambda_0}\|\bDelta_t(a)\|_2.
\end{equation}
Meanwhile, it's easy to verify that as long as $T\geq \max\{10, \frac{C''[d+\log(\sqrt{T})]^2}{\min\{\lambda_{0}^2, \lambda_{0}^4\}p_0^2}\}$ with $C''=C_1'^2$ as in Theorem \ref{thm:theta-estimation-estimated-Sigma}, $\forall t\geq \sqrt{T}$, $t\geq \frac{C_1'}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}$. This implies that $\forall t>T_0$, $t/2\geq\max\{\sqrt{T}, \frac{C_1'}{\min\{\lambda_{0}, \lambda_{0}^2\}}\frac{d+\log t}{p_0}\}$. Combining the results in Theorems \ref{thm-regret} (with (\ref{eq::proposed-estimator-estimated-Sigma}) instead of (\ref{eq:proposed-estimator})) and \ref{thm:theta-estimation-estimated-Sigma}, we obtain that the regret of Algorithm \ref{alg1} with $(\tilde\btheta_a^{(t)})_{a\in\{0, 1\}, t\in[T]}$ as plug-in estimators:
\begin{align*}
\text{Regret}(T;\bar\pi^*)&\leq 2T_0R_\theta + \frac{2(1-2p_0)}{1-\rho}\sum_{t = T_0+1}^T\bigg[
\frac{C(R+R_\theta)}{\lambda_0}\max\bigg\{\frac{d+\log(t-1)}{p_0(t-1)}, \sqrt{\frac{d+\log(t-1)}{p_0(t-1)}}\bigg\}\\
&\quad + \frac{2R_\theta}{\lambda_0}\max_a\|\bDelta_{t-1}(a)\|_2\bigg]\\
&\leq C_1T^{\frac12}\bigg\{R_\theta + \frac{(1\!-\!2p_0)(R\!+\!R_\theta)}{\sqrt{p_0}(1\!-\!\rho)\lambda_0}\sqrt{d\!+\!\log T}\bigg\} + 
C_1\frac{(1-2p_0)R_\theta}{(1-\rho)\lambda_0}\sum_{t\in[2\sqrt{T}-1, T)\cup\mathbb N}\max_a\|\bDelta_{t-1}(a)\|_2.
\end{align*}
for a universal constant $C_1$. Here we simplify the first terms using proof of Corollary \ref{cor:alg1-with-theta-estimation}.
