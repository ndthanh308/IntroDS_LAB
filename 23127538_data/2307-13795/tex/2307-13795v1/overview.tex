...
operation calls. Crucially however, these outgoing signals differ from conventional algebraic operation calls in that no additional variables 
are bound in the continuation $M$, making it naturally possible to continue executing $M$ straight after the signal has been issued, as depicted below:
\vspace{-2ex}
\[
...
\[
  \tmopout{\opsym{request}}{\mathit{cachedSize} + 1}{\client}
\]
...
variable $p$ in $N$, as captured by the following reduction rule:
\[
  \tmopin{op}{V}{\tmwith{op}{x}{M}{p}{N}} \reduces \tmlet{p}{M[V/x]}{\tmopin{op}{V}{N}}
\]

It is worth noting that the interrupt itself \emph{keeps propagating inwards} into the sub-computation $N$.
In order to skip certain interrupt handlers for some $\opsym{op}$, one 
...
\autoref{sec:extensions:reinstallable-interrupt-handlers}), which have the form
\[
...
In examples, we often find it useful to also pass some data between 
...
to be fulfilled makes the meta-theory of the underlying calculus cleaner, in programming we prefer to use 
\emph{generic} versions of them, i.e., ones with trivial continuations~\cite{Plotkin:GenericEffects}.
In particular, we define and use the following syntactic sugar:
\begin{align*}
...
it uses the \ls$requestNewData$ function to 
...
to display a message asking the user to wait. The client then simply reinvokes itself
by reinstalling the interrupt handler for \ls$nextItem$ interrupts. Observe that the 
