...
While \lambdaAEff, as introduced in our earlier work \cite{Ahman:POPL} and summarised in the previous
...
We recall from the reduction rules given in \autoref{fig:small-step-semantics-of-computations} that
...
of the same kind. One option to achieve this is through (general) recursive functions~\cite{Ahman:POPL}.
...
calculus, despite it being an orthogonal concern to modelling asynchrony.
...
\[
    \tmwith{op}{x}{M}{p}{N}
\]
with an additional variable $r$ bound to a function through which $M$ can reinstall the handler:\footnote{We 
  highlight the parts of \lambdaAEff~that change in this section's extensions with a grey background.}
\[
...
\end{multline*}
while all other reduction rules remain the same, except that interrupt handlers are extended with the additional variables $r$.
...
$N$ assigns to $\op$, i.e.,  $({\o'} , {\i'}) \mathrel{\order{O \times I}} \i\, (\op)$. The reason lies in the 
...
specifies that at the top level the interrupt handler does not issue any signals,
...
In our previous work~\cite{Ahman:POPL}, such state-passing behaviour was achieved by
...
Finally, we illustrate the use of stateful reinstallable interrupt handlers via the example
...
More examples of stateful interrupt handlers can be 
...
\[
  \tmwithre{op}{x}{r}{M}{p}{\tmopout{op'}{V}{N}}
  \reduces \tmopout{op'}{V}{\tmwithre{op}{x}{r}{M}{p}{N}}
\]
...
Restricting $V$ to ground values is a simple way to ensure this~\cite{Ahman:POPL}, 
but as a result, e.g., one can only send the arguments needed for the execution of 
remote function calls but not the functions themselves.

To address this limitation, we adopt a Fitch-style \emph{modal type system}~\cite{Clouston:FitchStyle}, 
where the typing context $\Gamma$ can contain (lock) tokens $\ctxlock$,
which delimit the extent to which variables are allowed to be used in terms.
...
\]
Since this prevents us to construct boxed values that would refer to a promise-typed variable,
we can safely extend operation payloads from ground to mobile types. Crucially however, when
...
\]
Here, we first use Fitch-style modal typing to ensure that $M$ cannot refer to promise-typed 
variables bound by any enveloping interrupt handlers, making it safe to propagate it outwards past them.
What is perhaps more surprising is that the type of the whole computation depends only
...
\[\Gamma \types \E[M] : \tycomp{Y}{(\o',\i')} 
\,\Longleftrightarrow\, 
\exists\, \Gamma', X, \o, \i .~ 
\Gamma \types\!\![\, \Gamma' \,\vert\, \tycomp{X}{(\o,\i)} \,]~ \E : \tycomp{Y}{(\o',\i')}
~\wedge~
\Gamma,\Gamma' \types M : \tycomp{X}{(\o,\i)}
\]
\end{lem}
...
\quad
\\[1ex]
\hspace{13.25cm} {\color{rulenameColor}(\op \neq \op')}
\end{array}
\]
...
Finally, in the evaluation context case, we use the induction hypothesis with 
\sref{Lemma}{lem:eval-ctx-typing}.
\end{proof}
...
For any process type $\tyC$ and interrupt $\op$, we have $\mathsf{signals\text{-}of}(\tyC) \order O \pi_1\, (\opincomp{op}{\tyC})$.
\end{lem}
...
\srefcase{Lemma}{lem:type-reduction}{1}.
...
in combination with the induction hypothesis. Finally, in order to discharge effect annotations-related side-conditions 
