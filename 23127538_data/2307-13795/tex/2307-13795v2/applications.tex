...
from a timer process that we run in parallel.
...
\end{lstlisting}
with the triggering of the guarded interrupt handlers being based on the received IDs.
...
\ls$remoteCall$ wrapper with. However, in order to be able to communicate 
the remote function's result back to the caller, $\o$ should include at 
...
refer the reader to our previous work~\cite{Ahman:POPL} for an example of remote function 
calls being triggered by passing a particular call's arguments to the callee. 
%
However, it is important to highlight that whereas in our previous work we were limited to only 
...
that each \ls$cancel$ interrupt affects only one remote function call. In our previous
...
$\overline{\op}_{\mathcal{R}} : A_\op \to R \Rightarrow B_\op \times R$, 
...
co-operation call by reinstalling the interrupt with the updated seed value.
...
\begin{lstlisting}
promise (op _ |-> let _ = await p in let y = comp in return <<y>>)
\end{lstlisting}
%
...
Returning to post-processing a list of integers promised by some interrupt handler, 
below is an example showing the use of the \ls{process$_{\op}$} combinator and how to \emph{chain multiple 
post-processing computations together} (filtering, folding, and issuing a signal), 
in the same spirit as how one is taught to program compositionally with futures and promises~\cite{Haller:Futures}:
\begin{lstlisting}
let p = promise (op x |-> original_interrupt_handler) in 
...
