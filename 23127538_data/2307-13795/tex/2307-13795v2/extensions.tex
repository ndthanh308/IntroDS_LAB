...
operation that it is handling, e.g., an exception handler handling an exception and then re-raising it at the end for other external handlers.
...
integer payload, and $\op' : \typromise{\tyint}$ carrying an integer-typed promise.
...
\]
which would again be ill-typed due to $p$ escaping its interrupt handler's binding scope.
