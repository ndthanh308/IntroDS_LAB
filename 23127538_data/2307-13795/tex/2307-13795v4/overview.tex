...
\]
to request a new batch of data starting from the given $\mathit{offset}$, or a different signal
\[
...
\]
%CHANGED Indent
\noindent In our running example, there are two interrupts of interest that $\client$ might receive:
\[
...
We use 
the $\tmkw{\downarrow}$-notation to indicate that interrupts propagate inwards into subcomputations, 
...
\]
For brevity, we often omit the $S$-annotation in examples when it is clear from
the context.
...
\begin{align*}
  ...
  \tmwithregenst{op}{x}{r}{s}{M}{W} &~\defeq~ \tmwithrest{op}{x}{r}{s}{M}{W}{p}{\tmreturn p} \\
...
issuing a signal and then immediately reinstalling the handler, and it is tempting to avoid the
repetition. A dual shared pattern can be found in
\autoref{sec:overview:runningexample:client}, where issuing a request signal is
immediately followed by installing an interrupt handler for its response. However, proper user-defined
