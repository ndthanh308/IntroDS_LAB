...
\title{Modelling intermittent anomalous diffusion with switching fractional
Brownian motion}
...
occurrence of heterogeneous complex systems in nature, their analysis is
...
fractional Brownian motion are analysed, with transitions arising from a
Markovian stochastic process and scale-free intermittent processes. We obtain
simple formulas for classical statistics of the processes, namely the mean
squared displacement and the power spectral density. Further, a method to
identify switching fractional Brownian motion based on the distribution of
displacements is described. A validation of the model is given for experimental
measurements of the motion of quantum dots in the cytoplasm of live mammalian
cells that were obtained by single-particle tracking.
\end{abstract}
...
approaches \cite{burrage,thapa,munoz2021objective,henrik,henrik1,janusz,
...
he2016dynamic,jeon2016protein,sabri2020elucidating,Balcerek2023}. Further, in other
...
\cite{loverdo2009quantifying}. \textcolor{blue}{In Fig.~\ref{Trajectories}a
we show three trajectories of quantum dots recorded within live HeLa cells
\cite{sabri2020elucidating}, as a visual example for experimental trajectories,
in which the state changes within individual trajectories.
On top of these examples, other fields, in which} regime
...
% \subsection{Scale-free sojourn times}
% %
% In the continuous time random walk (CTRW), one of the most useful models in the analysis of anomalous diffusion processes, a particle undergoes independent and identically distributed jumps Gaussian steps, similar to the increments of Brownian motion, but the time between steps is a random variable. When the time probability density has a power law tail with infinite mean, the process is not-stationary \cite{montroll1965random,shlesinger2017origins}. Such distributions have power law tails such that 
% \begin{equation}
% \label{dwellPowerLaw}
% \psi(t) \sim \frac{\beta}{\Gamma(1-\beta)} 
% \frac{t_0^\beta}{t^{1+\beta}} , 
% \end{equation}
% with $0<\beta<1$.
% Such dynamics resemble a Brownian particle with scale-free immobilisations. In this case, both the MSD and the PSD depend on the observation time. The MSD exhibits non-stationary subdiffusion with anomalous exponent $\beta$ \cite{Klafter:2011,meroz2010subdiffusion},
% %
% \begin{equation}
%    \langle \overline{\delta^2(\Delta,T)} \rangle \sim \frac{t_{\rm {lag}}^\beta}{T^{1-\beta}},
% \label{MSDctrw}
% \end{equation}
% %
% and a PSD that resembles that of Brownian motion, albeit with a dependence on observation time \cite{fox2021aging},
% %
% \begin{equation} \label{bm_Expansion}
%     \langle S(\omega, T)  \rangle  \sim  
%     \frac{1}{T^{1-\beta}\omega^2},
% \end{equation}
% %
% that is the PSD scales as $\omega^{-2}$.

% Despite the usefulness of the CTRW, its applicability is limited to systems with uncorrelated increments. On the other hand, the strength of the FBM lies in the spatial correlations. One way to overcome these limitations was formulated in terms of a subordinated process where the increments have correlations of the type of the FBM, but the particle remains immobilised during scale free times. The MSD and PSD of the subordinated process are found as \cite{meroz2010subdiffusion, fox2021aging}
% %
% \begin{equation}
%    \langle \overline{\delta^2(\Delta,T)} \rangle \sim \frac{\tau^{1-\beta+2\beta H}}{T^{1-\beta}} .
% \label{MSDsubord}
% \end{equation}
% and
% %
% \begin{equation} \label{psdsub}
%     \langle S(\omega, T) \rangle \sim 
%     \begin{cases}
%     T^{-(1-\beta)} \omega^{-2+\beta-2\beta H} & \text{if } H<1/2\\
%     T^{2\beta H - 1} \omega^{-2} & \text{if } H>1/2
% \end{cases},
% \end{equation}
% respectively.
...
of the process, Fig.~\ref{Trajectories}b shows the first $300$ points of
...
state is considered to have exponentially distributed dwell times. 
\textcolor{blue}{Such dichotomous processes, in which one of the states exhibits 
a dwell time distribution with an exponential tail and the second state has 
a power-law distribution, have received attention in diverse physical systems
\cite{sadegh20141,sikora2017elucidating,kurilovich2020complex,kurilovich2022non}.}
The first 2,000 points of a representative trajectory and its corresponding $H$
and $D$ time series are shown in Fig.~\ref{Trajectories}c.
...
\textcolor{blue}{When the first state is subdiffusive ($H_1<0.5$,
data not shown) the obtained results do not exhibit any difference from
those when both processes are superdiffusive. Namely for the mixed sub-
and superdiffusive case, the PSD and MSD asymptotically converge to those
of the state with power-law dwell times.}
...
An interesting case is that of superdiffusive FBM, 
\textcolor{blue}{based on persistent active stochastic dynamics, such as intracellular
motion driven by molecular motors in living cells \cite{reverey2015superdiffusion} or animal motion
\cite{vilk2022unravelling,vilk2022ergodicity}. This should not be taken to indicate that active motion will always
lead to superdiffusion; as soon as there exists a finite persistence time,
the motion will be Brownian at times longer than this time scale \cite{romanczuk2012active,lemaitre2023non}.
Moreover, transient superdiffusion may arise in passive systems, such as
bulk-mediated diffusion \cite{campagnola2015superdiffusive}.}
%given that this can
...
%animal motion \cite{vilk2022unravelling}. 
Superdiffusive FBM has a PSD that
...
\textcolor{blue}{We foresee that switching FBM with different Hurst exponents 
can have multiple direct applications in cell biology, 
such as the heterogeneous dynamics of intracellular endosomes \cite{fedotov}.} 
...
grant ME 1535/12-1). A.W. acknowledges  National Center of Science (Poland)
- Opus Grant 2020/37/B/HS4/00120.
...
Barkai E, Garini Y and Metzler R 2012 {\em Physics Today\/} {\bf 65(8)} 29
...
Krapf D and Metzler R 2019 {\em Physics Today\/} {\bf 72(9)} 48--54
...
\bibitem{henrik1}
Seckler H, Szwabi{\'n}ski J and Metzler R 2023 {\em Journal of Physical
  Chemistry Letters\/} {\bf 14} 7910
...
\bibitem{Balcerek2023}
Balcerek M, Burnecki K, Thapa S, Wy{\l}oma{\'n}ska A and Chechkin A 2022 {\em
  Chaos: An Interdisciplinary Journal of Nonlinear Science\/} {\bf 32} 093114
...
Wang W, Balcerek M, Burnecki K, Chechkin A~V, Janu\v{s}onis S, {\'S}l\k{e}zak
  J, Vojta T, Wy{\l}oma{\'n}ska A and Metzler R 2023 {\em Phys. Rev. Res.\/}
  {\bf 5} L032025
...
\bibitem{sadegh20141}
Sadegh S, Barkai E and Krapf D 2014 {\em New Journal of Physics\/} {\bf 16}
  113054

\bibitem{kurilovich2020complex}
Kurilovich A~A, Mantsevich V~N, Stevenson K~J, Chechkin A~V and Palyulin V~V
  2020 {\em Physical Chemistry Chemical Physics\/} {\bf 22} 24686--24696

\bibitem{kurilovich2022non}
Kurilovich A~A, Mantsevich V~N, Mardoukhi Y, Stevenson K~J, Chechkin A~V and
  Palyulin V~V 2022 {\em Physical Chemistry Chemical Physics\/} {\bf 24}
  13941--13950
...
Romanczuk P, B{\"a}r M, Ebeling W, Lindner B and Schimansky-Geier L 2012 {\em
  The European Physical Journal Special Topics\/} {\bf 202} 1--162

\bibitem{lemaitre2023non}
Lemaitre E, Sokolov I~M, Metzler R and Chechkin A~V 2023 {\em New Journal of
  Physics\/} {\bf 25} 013010
...
% For more details on the available numerical methods see \cite{kloeden1992stochastic}.
