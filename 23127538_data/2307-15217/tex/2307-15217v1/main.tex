...
% RLHF is not all you need: fundamental limitations and open problems
% Fundamental Limitations and Open Problems with RLHF
...
% CAS NOTES TO SELF -- TODOS
% PublicizingL FLI (meia), haist, maia, lab slack, fli slack, ai-x-risk slack, tom woodside, sam bowman, Richard ngo, max tegmark, the acknowledgements, twitter
...
Here, we present a simple formal framework for RLHF based, in part, on the formalism from \citet{christiano2017deep}. However, as will be discussed in \Cref{sec:challenges_with_rlhf} and \Cref{app:improved_model}, \emph{there are several ways in which this framework fails to reflect reality.}
...
As a result, these fundamental challenges must be either avoided by not using RLHF or compensated for by other safety measures.
We note that many of the problems RLHF faces are not new and represent broader challenges in ML, a point which we discuss further in \Cref{sec:conclusion}.
...
\textbf{\tractable Data collection can introduce biases.}
...
\textbf{\fundamental RLHF suffers from an unavoidable tradeoff between the richness and efficiency of feedback.} Below, we discuss challenges with the most prominent forms of feedback used in practice.
...
Even modeling human preferences with a reward at all, implicitly accepting the reward hypothesis~\citep{silver2021reward}, might be unwarranted~\citep{skalsereward, vamplew2022scalar, bobu2023aligning}.
...
In particular, modeling human irrationalities can make reward learning difficult~\citep{shah2019feasibility, mindermann2018occam}, leading to a trade-off between efficiency and accuracy.
...
\textbf{\tractable Reward models can misgeneralize to be poor reward proxies, even from correctly-labeled training data.}
...
The deployment distribution can always differ from the training and evaluation distributions \citep{christiano2019worst}. 
...
As discussed in \Cref{sec:challenges_with_rlhf}, human desires are difficult to express with a reward function \citep{skalsereward,vamplew2022scalar}.
...
There are several notable examples of AI-generated language feedback \citep{bai2022constitutional, saunders2022self, ye2023selfee, kim2023aligning, akyurek2023rl4f, madaan2023self, chen2023improving, gilardi2023chatgpt} with research agendas like Recursive Reward Modeling \citep{leike2018scalable} and AI Safety via debate \citep{irving2018ai, du2023improving}.
However, AI-generated feedback has drawbacks. Humans often disagree with AI feedback. 
The rate of human/AI disagreement will vary by task, but \citet{perez2022discovering} and \citet{casper2023explore} found this to happen up to 10\% and 46\% of the time respectively in different experiments. 
...
\textbf{Using direct human oversight.} Although learning a reward model is efficient, it might be necessary to directly provide rewards \citep{knox2008tamer, knox2009interactively, macglashan2017interactive} for RL training in certain safety-critical situations.
...
But this is overly simplistic \citep{skalsereward}. 
...
Several techniques for aligning LLMs with human preferences obtain results competitive with RLHF by using supervised learning instead of RL. 
...
The simplest variant of this is to perform standard supervised learning on well-curated data. Curation can mean filtering out bad demonstrations \citep{gehman-etal-2020-realtoxicityprompts,weibl2021}, compiling a small set of good demonstrations \citep{palms,sanh_t0, ibarz2018reward, stiennon2020learning, chung2022_scaling_instruction, biyik2022learning, zhou2023lima},
or generating good demonstrations using an LLM, e.g., after conditioning human feedback provided in natural language \citep{scheurer2022training, scheurer2023training, chen2023improving}.
A different family of methods augments the language modeling objective to utilize feedback provided by the reward model \citep{korbak2023pretraining,yuan2023rrhf,rafailov2023direct}. This last setting shares similarities with offline RL, which focuses on training an optimal policy using demonstrations annotated with rewards \citep{levine2020offline,snell_ilql}. 
...
How these types of risks should
 be documented remains an area of active work in AI governance. 
...
\textbf{Many problems with RLHF are \orange{fundamental}.} 
...
However, other problems with RLHF cannot fully be solved and instead must be avoided or compensated for with non-RLHF approaches.
...
We thank Sam Bowman, Adam Jermyn, Ethan Perez, Alan Chan, Gabriel Recchia, and Robert Kirk for their helpful feedback. 
