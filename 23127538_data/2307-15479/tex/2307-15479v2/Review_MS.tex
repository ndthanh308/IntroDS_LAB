...
Similarly to the case of a vanishing magnetic field, we can define the (first) transition temperature $T_{tr}^{(1)}=1/\beta_{tr}^{(1)}$ as the temperature at which the probability of the ferromagnetic configuration becomes equal to the total probability of single spin-flip configurations, $P_{m=N} = P_{m= N-2}$, leading to $Ne^{-4\beta J - 2h} = 1$ and therefore 
\begin{equation}
...
When calculating residence times, we select spin configurations having prescribed values of the quantity of interest and follow the time evolution of those configurations.
