%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% P a p e r V _ P o s t N O V I S S I M A  _ R E V
%
% Uwe Trittmann
%
% 27 April 2023 
...
likely important, and thus sectors of different parton\footnote{ A
  ...
  theory is bosonized or not.} number are coupled. Lately, there have
been discussions \cite{Jacobson,Smilga21,Kleb21} about the
...
its spectrum is much richer. Two Regge trajectories
...
%%%%%%%%%%%%%%%%
%My understanding is that the authors of [2] have now accepted that what
%we always meant by "massless adjoint QCD_2," i.e. your eq. (2), is NOT
%confining. They instead consider a more general theory with an
%dditional 4-fermion term  [tr(\bar \Psi\Psi)]^2, and they think that
%this theory will confine. 
%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%% Changes for XXX Version 2 %%%%%%%%%%%%

An argument in \cite{Jacobson} corroborated in Ref.~\cite{Smilga21},
based on index theorems (``mod 2 argument'') that both massive {\em
  and} massless $QCD_{2A}$ are strongly confining\footnote{States are
  color singlets {\em and} Wilson loop has area law.} theories for odd
$N$ has recently been shown to apply only to the theory with an {\em
  additiona}l four-fermion term $[\Tr(\bar \Psi\Psi)]^2$.  In fact,
the augmented model is no longer UV finite but rather has logarithmic
flow in the UV \cite{KlebPrivate}.  The current consensus seems to be
that massless QCD$_{2A}$ proper (without the additional term) is
screening, cf.~Ref.~\cite{Seifnashri}, where it is argued that the
existence of newly found super-selection sectors of the massless
theory is evidence for its {\em screening} behavior.
%There is no problem at large (even) $N$,
Also \cite{Anton,Kleb22} show that the spectrum of QCD$_{2A}$ is
largely insensitive to $N$ (which includes $N=3$), suggesting a
screening massless theory at odd $N$.
...
%%%% NEW
Another issue is the appearance of vacua. For instance, in \cite{Gomis} the flow of UV operators 
is mapped for generic two-dimensional field theories, and QCD$_{2A}$ in particular. It is argued that
a subset of UV operators ends up as vacua in the extreme infrared where the theory is describable 
as a topological field theory, and thus would not show
up as massive excitations in a Hamiltonian approach. 
Due to the (naive) trivial vacuum in light-cone quantization,
one has a hard time dealing with multiple vacua in this approach. 
%%%%%%%%%%%
Most often DLCQ-based work \cite{BDK,GHK,UT1,Kleb21,Kleb22} assumes light-cone gauge to be
principally sound even in the continuum limit. 
...
%However, 
One might be worried that gauge or fermionic zero-modes, absent %trivially
...
by the authors of Ref.~\cite{CartorRobPin97} for SU(2).
...
projects out only the {\em exact} multi-particle states.
It is unclear what the role of the {\em approximate} multi-particle states
...
believe we can show that discretized or compactified approaches are {\em not}
...
$A^+=0$, as is customary --- but see Sec.~\ref{SecIntro} and
...
we express the operators in terms of the dynamic fields, namely the
right-moving adjoint fermions $\psi_{ij}$ quantized by imposing anticommutation 
...
\eeas
where we introduced the right-moving components $J^+_{ij}=\psi_{ik}\psi_{kj}$
of the SU(N) current.
...
P^+=\sum_{i=1}^r x_i=1,
\eeq
which defines the {\em physical Hilbert space}, i.e.~the hyperplane of the naive Hilbert space $[0,1]^r$ on which
...
in Eq.~(\ref{TheStates}).
The upper integral limits, e.g.~in the unique Hilbert space cell volume
...
completely determine the asymptotic eigenfunctions.
It is often useful to think of them as operating in excitation number space:
\begin{eqnarray}
...
momenta on the physical hyperplane of the naive Hilbert space. If nothing else,
...
\item {\bf The massive bosonic $T=+1$ sector} with $T_{state}=(-1)^{r/2}$ states
  ...
  \]
  with $R:=
  %\frac{r}{2}\left(r-\left(\frac{r}{2}-1\right)\right)=
...
under momentum order reversal\footnote{Actually, the question is more
  ...
  additional symmetries associated with the
  lower-dimensional inversions ${\cal S}_i$.}. For states with three fermionic
...
momentum reversal, and hence a relative sign in
...
is massless, and acquires a mass (squared) of
about $r(r-1)\frac{g^2N}{\pi}\approx 5.7\frac{g^2N}{\pi}$ via the non-singular
...
the lowest four- and five-parton states are $M^2\approx 36\frac{g^2N}{\pi}$
and $M^2\approx 55\frac{g^2N}{\pi}$, so about three- and
...
eLCQ: the lowest three-parton asymptotic state is at
$M^2\approx 50\frac{g^2N}{\pi}$,
...
for one of the two bosonic
${\cal T}$ sectors were presented in Ref.~\cite{Kutasov94},
...
of states separately in each parton sector so that $\sum_r N_{\phi,r}=N_{\phi}$.
...
of modest dimensions (a few hundred rows and columns at most), and if we
...
Fig.~\ref{FigDLCQr6mu4Tm}(a). At the supersymmetric
...
parton-number violation parameter $\varepsilon$, where the
asymptotic(full) theory
has $\varepsilon=0(1)$, Fig.~\ref{eLCQvseps},
we see that some states' masses are
quite insensitive, while others depend substantially on $\varepsilon$. In fact,
there seems to be a cascade of $\varepsilon$-dependent states. 
These states are impure in parton number, and intriguingly
...
generator\footnote{Note that $G_0$ acts on operators not the
  ...
  partner sectors.}  $G_0$. Nonetheless, it is amusing to watch this unfold
...
In the {\em massless} theory we find states whose asymptotic masses are
...
the masses, even though the asymptotic approximation assumes {\em high}
excitation numbers.

Overall, our results are in fair agreement with previous DLCQ-based
work \cite{GHK,Kleb21}.
This shows that discretization or compactification of the theory yields
qualitatively correct results, even if accuracy of bound-state masses
...
All matrix elements are linear combinations of integrals of these types,
e.g. part of the ``singular'' matrix element 
\beq\label{Integral9}
\int^1_0 dx_1\, x_1^{n_1}\int^{1-x_1}_0 dx_2\, x_2^{n_2}\sin_s(\pi (k_1 x_1+k_2 x_2)){\rm Si}_S(\pi q x_2).
%\eeq
...
since $j$ has to be such that the exponent of $x$ is zero.
...
{\small
\beas
\int_{\epsilon}^{x_1}\!\!\!I^{(n_2 = n_3,n_1 \neq n_3)}_{PNV,1c}dy_1\!\!\!
%&=:&J_{PNV,1c}(x_1)=
&=&\frac{1}{\pi(n_1-n_3)}\frac{\sin\pi n_3x_1-\sin \pi n_1x_1}{x_1}
-\sin\pi n_3x_1{\rm Si}(\pi(n_1-n_3)x_1)\\
&&+\cos\pi n_3x_1[{\rm Ci}(\pi(n_1-n_3)x_1)-{\rm Ci}(\pi(n_1-n_3)\epsilon)]
  \\
%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\delta^{n_1}_{n_3}\cos\pi n_3x_1[\ln x_1 -\ln\epsilon]-\bar{\delta}^{n_1}_{n_3}
\Big\{\sin(\pi n_3 x_1){\rm Si}(\pi (n_1-n_3)x_1)\\
&&\qquad\qquad\qquad-\cos(\pi n_3 x_1)[{\rm Ci}(\pi (n_1-n_3)x_1)-{\rm Ci}(\pi (n_1-n_3)\epsilon)]\Big\}\\
...
%%%%%%%% Similar thing with sin <-> cos, Si <-> Ci plus signs
    &&\qquad\qquad\,\,\,+\frac{{\rm Ci}(\pi(n_2-n_3)x_1)}{\pi(n_2-n_1)}[\cos(\pi (n_2\!-\!n_1\!-\!n_3)x_1)-\cos(\pi n_3x_1)]\\
...
\]
The contributions from the second terms,
Eqs.~(\ref{SecondTerm1})\&(\ref{SecondTerm2}),
are similar, but only contribute when $n_1\neq n_3$.
...
Note that we are assuming $k,q\in\mathbb{Z}$ and that expressions for identical arguments have to be worked out separately.
}
\beas
...
\[
\langle \vec{\mu}|\hat{H}_m|\vec{\nu}\rangle=\frac{m^2}{2}\int_0^1 \frac{dx_1}{x_1}\int_0^{1-x_1} dx_2
\cdots\int_0^{1-\sum_i^{r-1} x_i} dx_{r-1} \sin_I(\pi \vec{\mu}\cdot \vec{x})
  \sin_I(\pi \vec{\nu}\cdot \vec{x}), 
\]
where $I$ is the ${\cal I}$ quantum number of the sector, $\sin_{-1}=\sin$,
and $\sin_{+1}=\cos$.    
...
\bibitem{KlebPrivate} I.R.~Klebanov, private communication.
\bibitem{Gomis} D.~Delmastro, J.~Gomis and M.~Yu,
%``Infrared phases of 2d QCD,''
JHEP \textbf{02} (2023), 157;
%doi:10.1007/JHEP02(2023)157
%[arXiv:2108.02202 [hep-th]];
%\bibitem{Delmastro:2022prj}
D.~Delmastro and J.~Gomis,
%``RG flows in 2d QCD,''
[arXiv:2211.09036 [hep-th]].
