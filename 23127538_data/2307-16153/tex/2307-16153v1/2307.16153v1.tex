...
By introducing a general framework involving the so-called \textit{semivirial-vanishing geometry}, the author was able to prove the sharp bifurcation of the scattering and blowing-up solutions for the focusing NLS in the mass-supercritical regime \cite{Luo_inter,Luo_energy_crit,luo2021sharp}. One of the main purposes of the present paper is to show that the theory of the semivirial-vanishing geometry will continue to work in the mass-critical setting and help us to formulate sharp scattering and blow-up results, which will ultimately improve the previous ones established in \cite{Luo_Waveguide_MassCritical}. In turn, this also gives a complete variational framework for focusing NLS problems posed on $\R^d\times\T$, up to a remaining energy-critical endpoint open case\footnote{We note that the single issue arose in \cite{Luo_energy_crit}, where the author was unable to prove the existence of normalized ground states with small mass. Nonetheless, we believe that small mass ground states do in fact exist.}. This single open case, together with the doubly mass- and energy-critical model ($m=2$), will be studied in a companion paper.
...
\end{proposition}
Proposition \ref{prop iden of gamma and beta} thus enables us to manipulate the analysis solely for the variational problem $\gamma_\omega$, which will be the case in many of the upcoming compuations.
...
\item[(iii)] If $u_1$ and $u_2$ are positive optimizers of $\gamma_\omega$ which also solve \eqref{nls2} with the given $\omega$, then $u_1=u_2$.
\end{itemize}
\end{theorem}

\begin{theorem}[$y$-dependence of ground states of $\gamma_\omega$]\label{thm threshold frequency}
Let $\alpha\in [\tas,\tbs)$. Then there exists some $\omega_*\in(0,\infty)$ such that
\begin{itemize}
...
B_c&:=\{\omega>0:\text{ there exists $u\in H^1(\R^d\times\T^m)$ with $u\in\mathrm{argmin}\,\beta_\omega$ and $M(u)=c$}\},
\end{align*}
where $\mathrm{argmin}\,\kappa_\omega$ denotes the set of minimizers of a variational problem $\kappa_\omega\in\{\gamma_\omega,\beta_\omega\}$. Denote also by $Q$ the unique radial and positive solution of \[-\Delta_x Q+Q=Q^{1+\frac{4}{d}}\quad\text{on $\R^d$.}\]
Then for
\begin{align}\label{range of c}
c\in
\left\{
\begin{array}{cl}
(0,\infty),&\text{if $\alpha\in (\tas,\tbs)$,}\\
\\
(0,(2\pi)^m\widehat M(Q)),&\text{if $\alpha=\tas$ and $m=1$,}
\end{array}
\right.
\end{align}
we have $A_c=B_c\neq \varnothing$ and
\begin{align}\label{lengendre identity}
m_c=\inf_{\omega\in A_c}(\gamma_\omega-\frac{1}{2}c\omega)=\inf_{\omega\in B_c}(\beta_\omega-\frac{1}{2}c\omega).
\end{align}
\end{theorem}

\begin{remark}
Notice that the statements involving the set $B_c$ and the second identity in \eqref{lengendre identity} in Theorem \ref{thm legendre} will be immediate consequences of Proposition \ref{prop iden of gamma and beta}, as long as the claims for the set $A_c$ and the first identity in \eqref{lengendre identity} are proved. We have included these statements on purpose, as the variational problem $\beta_\omega$ will play an important role in proving the large data scattering and blow-up results later.
\end{remark}

We briefly elaborate on the idea for proving Theorem \ref{thm legendre}. In fact, \eqref{lengendre identity} will immediately follow from the definitions of the variational problems and the Pohozaev's identity (Lemma \ref{lemma of poho}) as long as we can prove that $A_c$ and $B_c$ are not empty. We will then use Theorem \ref{thm threshold frequency} and suitable test function arguments to prove the latter claim.

\begin{remark}
Notice that by definition, the quantity on the r.h.s. of \eqref{lf identity rd} will not exceed the second quantity on the r.h.s. of \eqref{lengendre identity}. One may naturally ask whether \eqref{lf identity rd} will indeed hold in place of \eqref{lengendre identity} for $m_c$ with the additional constraint $K(u)=0$. We note that this can not be the case: as a consequence of Lemma \ref{lemma 4.1} we have $\lim_{\omega\to \infty}(\gamma_\omega-\frac12 c\omega)=\lim_{\omega\to \infty}(\beta_\omega-\frac12 c\omega)=-\infty$, while by definition we always have $m_c\geq 0$. This reveals the fact that \eqref{lengendre identity} improves \eqref{lf identity rd} in a strict sense.
\end{remark}
\begin{remark}
In the case $\alpha=\tas$ and $m=0$, we have a more precise description on the quantities given in Theorem \ref{thm legendre} which may be directly deduced from the classical theories for the mass-critical NLS (see e.g. \cite{weinstein,Kwong_uniqueness}): In fact, we have $A_c=B_c=\varnothing$ for $c<\widehat M(Q)$,
\begin{align*}
m_c=
\left\{
\begin{array}{cl}
...
\begin{remark}
Theorem \ref{thm threshold mass 2} implies particularly that the focusing quintic NLS on $\R\times\T$ and focusing cubic NLS on $\R^2\times\T$, which being physically relevant cases, possess ground state solutions of arbitrary mass not exceeding $2\pi\widehat M(Q)$ that also have non-trivial $y$-portion.
\end{remark}

Notice again that due to the failure of the scaling arguments, it is rather difficult to find for a given number $c\in (0,2\pi\widehat M(Q))$ a candidate $u\in H^1(\R^d\times\T)$ satisfying $M(u)=c$ and $K(u)=0$. Theorem \ref{thm threshold mass 2} thus guarantees the existence of such a candidate. In conjunction with suitable surgery arguments, we are then able to prove the following dynamical properties of the mapping $c\mapsto m_c$ which play a central role in proving the large data scattering results.
...
As a final application of Theorem \ref{thm legendre}, we establish the following large data scattering and blow-up results which sharpen the ones deduced in \cite{Luo_Waveguide_MassCritical}.
...
Basically, we invoke almost the same arguments as the ones given in \cite{Luo_JFA_2022,Luo_DoubleCritical,Luo_inter,Luo_energy_crit} to prove Theorem \ref{thm large data} and \ref{thm blow up}. In the mass-critical setting, however, we face a new difficulty that we lose control on the gradient energy $\|\nabla_x u\|_{L^2(\R^d\times\T^m)}^2$ by merely using the condition $K(u)\neq 0$. It is in this situation that Theorem \ref{thm legendre} kicks into play again, where we are able to use the constraint $N_\omega(u)\neq 0$ to overcome this difficulty, see Lemma \ref{lem 7.6} for details.
...
The author is grateful to Rowan Killip, Changxing Miao, Monica Visan and Jiqiang Zheng for some stimulating discussions.
...
\end{align*}
which is again a contradiction. Hence $\mK(u_\omega)=0$ and $u_\omega$ is an optimizer of $\gamma_{\omega}$. That $u_\omega$ is a solution (up to symmetries) of \eqref{nls2} follows from Lemma \ref{vc implies sc} and the positivity of $u_\omega$ follows from the strong maximum principle. Finally, in the case $p=\tas$, that the function $v^t$ defined by \eqref{1.4} is also an optimizer of $\gamma_\omega$ follows from the fact that the energies $S_\omega(u)$ and $K(u)$ are stable under the transformation $u\mapsto v^t$ for any $t>0$.

The previous steps finish the existence part of the proof. It still remains to show that deduced positive optimizer $u_\omega$, which also solves \eqref{nls2} with the given $\omega$, is unique. Assume therefore that $\gamma_\omega$ has two different positive optimizers $u_1$ and $u_2$ with the required properties. W.l.o.g assume that $u_1(z_0)>u_2(z_0)$ for some $z_0\in\R^{d+m}$. Then by maximum principle we know that $u_1(z)>u_2(z)$ for all $z\in\R^{d+m}$. Inserting $N_\omega(u_1)=N_\omega(u_2)=0$ (deduced from testing \eqref{nls2} with $\bar u_i$) into $\gamma_\omega=S_\omega(u_i)$ we thus obtain
$\gamma_\omega=\frac{\alpha}{2(\alpha+2)}\|u_i\|_{\alpha+2}^{\alpha+2}$ for $i=1,2$. Combining with $u_1>u_2>0$ and the continuity of $u_i$ (deduced from local elliptic regularity) this can only happen if $u_1=u_2$ on $\R^{d+m}$, a contradiction. This implies the uniqueness of $u_\omega$ and in turn completes the desired proof.
\end{proof}
...
\begin{align*}
\lim_{\omega\to \infty}\gamma_{1,\omega}<2\pi\wm_{1}.
\end{align*}
...
\item[(ii)]Let $u_\omega$ be the positive optimizer of $\gamma_\omega$ which also solves \eqref{nls2} with the given $\omega$. Then
\begin{align}\label{4.0}
\lim_{\omega\to 0}M(u_\omega)=
\left\{
\begin{array}{cl}
\infty,&\text{if $\alpha\in(\tas,\tbs)$},\\
\\
2\pi \widehat M(Q),&\text{if $\alpha=\tas$ and $m=1$}
\end{array}
\right.
\end{align}
and $\lim_{\omega\to\infty}M(u_\omega)=0$.
\end{itemize}
\end{lemma}
...
\gamma_\omega=(2\pi)^m\widehat \gamma_\omega=\omega^{1+\frac2\alpha-\frac{d}{2}}\wm_{1}
\end{align}
...
Now we prove (ii) and begin with proving $\lim_{\omega\to\infty}M(u_\omega)=0$. Notice that
...
as $\omega\to\infty$, since $\frac{2}{\alpha}-\frac{d+m}{2}<0$.
...
By Proposition \ref{prop iden of gamma and beta} it suffices to show that $A_c\neq\varnothing$ and the first identity in \eqref{lengendre identity}. It suffices also to only consider the case $\alpha\in(\tas,\tbs)$, where the case $\alpha=\tas$ and $m=1$ can be shown similarly. Arguing as in the end of the proof of Theorem \ref{thm threshold frequency}, we are able to show that the mapping $\omega\mapsto M(u_\omega)$, where $u_\omega$ is the unique positive optimizer of $\gamma_\omega$ which also solves \eqref{nls2} with the given $\omega$, is continuous. Using \eqref{4.0} and the intermediate theorem we infer that for any $c>0$ there exists some $\omega>0$ with $M(u_\omega)=c$, from which the claim follows.

Having proved that $A_c\neq\varnothing$ we are now able to show \eqref{lengendre identity}. By definition and $A_c\neq\varnothing$ we already know that $m_c\leq \inf_{\omega\in A_c}(\gamma_\omega-\frac{1}{2}c\omega)$. Now let $(u_n)_n\subset V(c)$ be a minimizing sequence of $m_c$. For $\omega\in A_c$ we deduce by definition that
\begin{align}\label{abc}
E(u_n)=S_\omega(u_n)-\frac{1}{2}\omega c\geq \gamma_{\omega}-\frac{1}{2}c\omega \geq \inf_{\omega\in A_c}(\gamma_\omega-\frac{1}{2}c\omega).
\end{align}
...
As a first application of Theorem \ref{thm legendre}, we establish the existence of normalized ground states for the mass-critical variational problem $m_c$.

\begin{proof}[Proof of Theorem \ref{thm threshold mass 2}]
By Proposition \ref{prop iden of gamma and beta} it suffices to show the claim for $\gamma_\omega$. In fact, we will show that a minimizer $u_c$ of $m_c$ can be characterized as a minimizer of $\gamma_\omega$ with some $\omega>0$ using Theorem \ref{thm legendre}.

By Theorem \ref{thm legendre}, choose $(\omega_n)_n\subset A_c$ and $(u_n)_n\subset H_{x,y}^1\setminus\{0\}$ with $K(u_n)=0$ such that
$$m_c=
\gamma_\omega-\frac{1}{2}c\omega+o_n(1)=S_\omega(u_\omega)-\frac{1}{2}c\omega+o_n(1).$$
We first claim that $(w_n)_n$ is bounded from above. Indeed, by Lemma \ref{lemma 4.1} we know that $\gamma_\omega\lesssim \omega^{1+\frac{2}{\alpha}-\frac{d+m}{2}}$. Combining with $\frac{2}{\alpha}-\frac{d+m}{2}<0$ we infer that
\[\lim_{\omega\to\infty}\gamma_\omega-\frac{1}{2}c\omega=-\infty,\]
which is incompatible with the fact that $m_c\geq 0$. Thus up to a subsequence, $w_n$ converges to some $\omega\in[0,\infty)$. Again, if $\omega_n<\omega_*$, where $\omega_*$ is defined by Theorem \ref{thm threshold frequency}, then it is necessary that $M(u_n)=2\pi\widehat M(Q)$, which contradicts the fact that $M(u_n)=c<2\pi \widehat M(Q)$. Thus $\omega\in[\omega_*,\infty)$. Now arguing as in the proof of Corollary \ref{cor lower bound}, in conjunction with the boundedness of $(w_n)_n$, we know that $(u_n)_n$ is a bounded sequence in $H_{x,y}^1$ with $\liminf_{n\to\infty}\|u_n\|_{\tas+2}>0$ and by Lemma \ref{lemma non vanishing limit} we may also assume that $u\neq 0$. On the other hand, by the continuity of the mapping $\omega\mapsto \gamma_\omega$ we actually see that $(u_n)_n$ is in fact a minimizing sequence of $\gamma_\omega$. Thus arguing as in the proof of Theorem \ref{thm existence ground states 1} we infer that $u$ is a minimizer of $\gamma_\omega$.

Notice that in the proof of Theorem \ref{thm existence ground states 1}, all inequalities deduced from the weakly lower semicontinuity of norms will indeed hold as equalities, thus it actually holds $\lim_{n\to\infty} M(u_n)=M(u)$, from which we conclude that $M(u)=c$. Up to rescaling (see Lemma \ref{vc implies sc}) we know that $u$ is a solution of \eqref{nls2}, hence by Lemma \ref{lemma of poho} we know that $K(u)=0$. Moreover, the identities of converging norms given in the proof of Theorem \ref{thm existence ground states 1} also imply that $$E(u_n)=\frac{1}{2}\|\nabla_y u_n\|_2^2=\frac{1}{2}\|\nabla_y u\|_2^2+o_n(1)=E(u)+o_n(1).$$
Thus
\[m_c=E(u_n)+o_n(1)=E(u)+o_n(1).\]
Taking $n\to\infty$ and combining $K(u)=0$ shows that $u$ is an optimizer of $m_c$ which also solves \eqref{nls2} with $\omega\geq \omega_*$. From this the statements (i), (ii) and (iv) already follow. Notice now that if $\nabla_y u=0$, then we necessarily have $M(u)=(2\pi)^m \widehat M(Q)>c$, a contradiction. This completes the proof of (iii) and in turn also the proof of Theorem \ref{thm threshold mass 2}.
\end{proof}
...
The mapping $c\mapsto m_c$ is monotone decreasing on $(0,(2\pi)^m\widehat M(Q))$.
\end{lemma}

\begin{proof}
For $2\pi \widehat M(Q)>a>b>0$ we need to show $m_{a}<m_b$. Let $u_b$ be an optimizer of $m_b$ whose existence is guaranteed by Theorem \ref{thm threshold mass 2}. Let $\phi\in C_c^\infty(\R^d)$ satisfy $\widehat M(\phi)=(2\pi)^{-1}(a-b)<\widehat M(Q)$. Since $\widehat M(\phi)<\widehat M(Q)$, using the Gagliardo-Nirenberg inequality on $\R^d$ we infer that
\[K(\phi)=:\eta>0.\]
Let $\vare>0$ be some small number to be chosen later. For $t>1$ we know that $K(t u_b)<0$. Choose now $t=t(\vare)>1$ such that
\begin{align}\label{6.1}
m_b+\vare\geq E(tu_b),\quad b+\vare\geq M(tu_b)\geq b+\frac{\vare}{2} \quad \text{and}\quad K(tu_b)\in(-2\vare,-\vare).
\end{align}
Let $\eta\in C_c^\infty(\R^d;[0,1])$ be a radially decreasing cut-off function such that $\eta\equiv 1$ in $B_1(0)$ and $\eta\equiv 0$ in $B_2^c(0)$. We then choose $R=R(\vare)\gg 1$ such that
\begin{gather}\label{6.2}
\begin{array}{cc}
E(tu_b)+\vare\geq E(\eta(\cdot/R)tu_b)
,\quad
M(tu_b)\geq M(\eta(\cdot/R)tu_b)\geq M(tu_b)-\frac{\vare}{4},\\
\\
K(\eta(\cdot/R)tu_b)\in(-3\vare,-\frac{\vare}{2}).
\end{array}
\end{gather}
Defining $u_1:=\eta(\cdot/R)tu_b$, \eqref{6.1} and \eqref{6.2} imply that
\begin{align}\label{6.3}
m_1+2\vare\geq E(u_1),\quad b+\vare\geq M(u_1)\geq b+\frac{\vare}{4} \quad \text{and}\quad K(u_1)\in(-3\vare,-\frac{\vare}{2}).
\end{align}
...
\begin{proof}
Since $c\mapsto m_c$ is monotone decreasing, it suffices to prove the right continuity of $c\mapsto m_c$. Let thus $c_n\searrow c$. By Theorem \ref{thm threshold mass 2} we denote by $u_n$ the optimizers of $m_{c_n}$ which are also optimizers of $\gamma_{\omega_n}$ with $\omega_n\geq \omega_*$. Arguing as in the proof of Theorem \ref{thm threshold mass 2}, the fact $\gamma_\omega\lesssim \omega^{1+\frac{2}{\alpha}-\frac{d+m}{2}}$ and the boundedness of $(c_n)$ from below (as they converge to $c>0$) imply the boundedness of $(\omega_n)$. Thus we are again in the same situation in the proof of Theorem \ref{thm threshold mass 2}, and we conclude that there exists some $u\in V(c)$ and some $\omega=\lim_{n\to\infty}\omega_n\in[\omega_*,\infty)$ such that $u$ is both a minimizer of $m_c$ and $\gamma_{\omega}$. By definition we already know that $E(u)\geq m_c$. Moreover, using the monotonicity of $c\mapsto m_c$ deduced from Lemma \ref{lem 6.1} and \eqref{lengendre identity} we have
\[E(u)=\gamma_\omega-\frac{1}{2}c\omega=\lim_{n\to\infty}(\gamma_{\omega_n}-\frac{1}{2}c_n\omega_n)=
\lim_{n\to\infty}m_{c_n}\leq m_c\]
...
\section{Scattering vs. blow-up: Proof of Theorem \ref{thm large data} and \ref{thm blow up}} \label{sec 6}
...
As already pointed out in the introduction, the variational problem $\beta_\omega$ plays an important role in the upcoming proofs. Its significance is given by the following lemma.

\begin{lemma}[Equivalent formulations of the admissible set]\label{lem 7.6}
Let $c\in(0,2\pi\widehat M(Q))$, $u\in H_{x,y}^1$ and let $\omega_c^\beta$ be the number defined through Theorem \ref{thm threshold mass 2}. Then the following statements are equivalent:
\begin{itemize}
\item[(i)]$M(u)< c$, $E(u)<m_c$ and $K(u)>0$.
\item[(ii)]$M(u)< c$, $E(u)<m_c$ and $N_{\omega_c^\beta}(u)>0$.
\item[(iii)]$M(u)< c$, $E(u)<m_c$, $K(u)>0$ and $N_{\omega_c^\beta}(u)>0$.
\end{itemize}
The equivalence will still hold if the condition $M(u)< c$ is replaced by $M(u)\leq c$
\end{lemma}

\begin{proof}
We only prove the equivalence of (i) and (iii), the equivalence of (ii) and (iii) can be deduced similarly. Since also the proofs for the cases $M(u)< c$ and $M(u)\leq c$ are identical, we shall only give the proof in the case $M(u)< c$.

Clearly (iii) implies (i). Thus assume that (i) is satisfied and assume on the contrary that $N_{\omega_c^\beta}(u)\leq 0$. If $N_{\omega_c^\beta}(u)=0$, then using \eqref{lengendre identity} and Theorem \ref{thm threshold mass 2} we infer that
\[\beta_{\omega_c^\beta}=\frac{1}{2}c\omega_c^\beta+m_c
>\frac{1}{2}\omega_c^\beta M(u)+E(u)=S_{\omega_c^\beta}(u)\geq \beta_{\omega_c^\beta},\]
a contradiction and thus $N_{\omega_c^\beta}(u)<0$. Next, notice that
\[0>N_{\omega_c^\beta}(u)=\|\nabla_x u\|_2^2-\|u\|_{\tas+2}^{\tas+2}+\|\nabla_y u\|_2^2+\omega_c^\beta M(u).\]
Since $\|\nabla_y u\|_2^2+\omega_c^\beta M(u)>0$, it is necessary that $\|\nabla_x u\|_2^2-\|u\|_{\tas+2}^{\tas+2}<0$. Set
$u^t(x,y):=t^{\frac{d}{2}}u(tx,y)$. Then
\begin{align*}
\|\nabla_x u^t\|_2^2-\|u^t\|_{\tas+2}^{\tas+2}&=t^{2}(\|\nabla_x u\|_2^2-\|u\|_{\tas+2}^{\tas+2}),\\
\|\nabla_y u^t\|_2^2+\omega_c^\beta M(u^t)&=\|\nabla_y u\|_2^2+\omega_c^\beta M(u).
\end{align*}
Thus we may choose some $t\in(0,1)$ such that $N_{\omega_c^\beta}(u^t)=0$. Similarly, since $K(u)>0$, we know that for $t<1$ it holds
$E(u^t)<E(u)<m_c$ and $K(u^t)=t^2 K(u)>0$. We thus get back to the first case and reach the contradiction again. This completes the proof.
\end{proof}

As a consequence of Lemma \ref{lem 7.6}, we have the following coerciveness result for elements from the set
\begin{align}\label{7.26}
\mathcal{A}:=\{u\in H_{x,y}^1:M(u)< c,\, E(u)<m_c,\,K(u)>0\}.
\end{align}

\begin{lemma}\label{lemma coercivity}
Let $u\in \mathcal{A}$. Then
\[\frac{1}{d+2}(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2)\leq S_{\omega_c^\beta}(u)\leq \frac{1}{2}(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2).\]
\end{lemma}

\begin{proof}
The upper bound is trivial. Next, recall from Lemma \ref{lem 7.6} that $N_{\omega_c^\beta}(u)>0$, which implies $\|u\|_{\tas}^{\tas}<\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2$. Then
\begin{align*}
S_{\omega_c^\beta}(u)&=\frac{1}{2}(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2)-\frac{d}{2(d+2)}\|u\|_{\tas}^{\tas}\nonumber\\
&>\bg(\frac{1}{2}-\frac{d}{2(d+2)}\bg)(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2)\nonumber\\
&=\frac{1}{d+2}(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2),
\end{align*}
as desired.
\end{proof}
...
\begin{proof}
Assume that this is not the case. By conservation of mass and energy this can only happen if there is some $t$ with $K(u(t))\leq 0$. By continuity we can then find some $t_0$ lying between $0$ and $t$ such that $K(u(t_0))=0$. By Proposition \ref{prop monotone} we also know that $E(u(t_0))<m_c\leq m_{M(u(t_0))}$, which however contradicts the minimality of $m_{M(u(t_0))}$.
\end{proof}
...
We now construct the mass-energy-indicator (MEI) functional $\mathcal{D}$. For $c\in(0,2\pi\widehat M(Q))$ let $\omega_c^\beta$ be the number corresponding to $m_c$ defined in Theorem \ref{thm threshold mass 2}. As we will fix the mass number $c$ in the upcoming computations, we simply write $\omega=\omega_c^\beta$.

Next, let $\Omega:=[0,c)\times [0,m_c)$. We then define MEI-functional $\mathcal{D}:\Omega\to(0,\infty)$ by
\[\mathcal{D}(a,b)=\frac{a}{c-a}+\frac{b}{m_c-b}.\]
...
\item[(iii)] For all $u\in \mA$ it holds $S_\omega(u)< \beta_\omega\mD(u)$.
\end{itemize}
\end{lemma}

\begin{proof}
(i) and (ii) follow immediately from the definition of the MEI-functional and the fact that $E(u)\in (0,m_c)$ for $u\in\mA$. For (iii), direct computation and Theorem \ref{thm threshold mass 2} yield
\begin{align*}
S_\omega(u)&=\frac{1}{2}\omega M(u)+E(u)\nonumber\\
&=\frac{1}{2}\omega(c-M(u))\frac{M(u)}{c-M(u)}+(m_c-E(u))\frac{E}{m_c-E(u)}\nonumber\\
&<\frac{1}{2}c\omega \mD(u)+m_c\mD(u)=\beta_\omega \mD(u).
\end{align*}
...
For $c\in(0,\pi\widehat M(Q))$ let $\mA=\mA_c$ be the set defined through \eqref{7.26}. Thanks to the monotonicity and lower semicontinuity of the mapping $c\mapsto m_c$ deduced in Proposition \ref{prop monotone}, the scattering result given in Theorem \ref{thm large data} will follow as long as for each given $c\in(0,\pi\widehat M(Q))$ we can prove the same statement for all initial data lying in $\mA_c$, which will be proved in the rest of the paper. As the mass number $c$ will be seen as a fixed number, we simply drop the index $c$ from the set $\mA_c$.
...
More precisely, we need to show that, under the assumption that there exist at least two nonzero linear profiles $\phi^j$, we have
\begin{alignat}{2}
...
We next assume that \eqref{pos of K} does not hold. Then up to a subsequence we may assume that there exists some $\phi^j$ such that $K(T_n^j\phi^j)\leq 0$ for all $n\in\N$. By the mass decomposition \eqref{orthog L2} we know that $M(T_n^j)<c$ for all $n\gg 1$ (here we also used the assumption that there exist at least two nonzero linear profiles in order to get a strict mass gap between $M(T_n^j\phi^j)$ and $c$). Notice that using \eqref{orthog L2} and \eqref{cnls conv of h} we also have the energy decomposition of $I(u_n)$:
\begin{align}
...
\end{align}
Let $u_0$ satisfy \eqref{gwp condition} and set $c:=M(u_0)$. By Lemma \ref{lem 7.6}, \ref{lemma coercivity} and conservation of mass and energy we obtain
\[\frac{1}{d+2}(\omega_c^\beta M(u)+\|\nabla_{x,y}u\|_2^2)\leq S_{\omega_c^\beta}(u)=S_{\omega_c^\beta}(u_0),\]
...
Next, define the set
\[\mathcal{B}:=\{\phi\in H_{x,y}^1: M(\phi)<2\pi \widehat{M}(Q),\, E(\phi)<m_{M(\phi)},\,K(\phi)<0\}.\]
Let $\phi\in \mathcal{B}$ and let $\omega=\omega_c^\beta$ be the number defined through Theorem \ref{thm threshold mass 2} according to the mass number $c=M(\phi)$. By Lemma \ref{lem 7.6} we infer that $N_\omega(\phi)<0$. Define also $\phi^t(x,y):= t^{\frac{d}{2}}\phi(tx,y)$ for $t>0$. Then using $N_\omega(\phi)<0$ and $\|\nabla_y \phi\|_2^2+\omega M(\phi)>0$ we can find some $t^*\in(0,1)$ such that $N_\omega(\phi^{t^*})=0$. Hence by the definition of $\beta_\omega$
\begin{align*}
S_\omega(\phi)&=S_\omega(\phi^{t^*})+\int_{t^*}^1 \frac{d}{ds}S_\omega(\phi^{s})\,ds\\
&=S_\omega(\phi^{t^*})+K(\phi)\int_{t^*}^1 s\,ds\\
&=S_\omega(\phi^{t^*})+\frac{1-(t^*)^2}{2}K(\phi)
\geq \beta_\omega+\frac12K(\phi),
\end{align*}
which in turn implies $K(\phi)\leq 2(S_\omega(\phi)-\beta_\omega)$. Using Theorem \ref{thm threshold mass 2} we know however that $S_\omega(\phi)-\beta_\omega=E(\phi)-m_c$, thus
\begin{align}\label{blow up upper}
K(\phi)\leq 2(E(\phi)-m_c)<0.
\end{align}
...
\begin{align*}
\pt_{t}^2 J(t)=8\mK(u(t))\leq 4(m_{\mM(u)}-\mH(u))<0.
\end{align*}
