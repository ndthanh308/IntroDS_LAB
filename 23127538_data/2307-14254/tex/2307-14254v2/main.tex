...
% since its inception because it proposes a concrete theoretical scenario 
% of a phase transition that is 
...
-- N\'eel and VBS -- for spin-$\frac{1}{2}$
moments %degrees of freedom 
in $2+1d$. 
% Numerical hints of such a transition were seen 
% in an earlier work~\cite{Sandvik_Scalapino_etal_PRL_2002}.
%The 
% Berry phase interference effects~\cite{Haldane_prl1988,Read_Sachdev_prl1989,Read_Sachdev_prb1990,Read_Sachdev_nucphysB}  
% of the quantum degrees of freedom is crucial for the beyond-LGWF nature.
DQC is 
described %often described in terms of 
as a gauge theory of fractionalized 
...
Evidence for many 
%proposed 
features of DQC have been numerically
seen 
%at the transition 
in these studies,
...
Zhang_Levin_PRL2023},
and fermionic models~\cite{DaLiao_etal_prbAug2022,DaLiao_etal_prbOct2022,Sato_etal_prl2017,Li_natcomm2017,Li_arxiv2019}. 
%argued to be in the same universality class. 
%The issue is contentious though due to the 
However scaling violations 
...
% physics~\cite{Desai_Kaul_prl2019}.


Our focus will be on spin-$1$ here.
% for which there are only a handful of papers discussing possible DQC
% and none which have shown DQC behavior 
%without known microscopic model realizations.
...
% Wang, Kivelson and Lee 
%A later work
Ref.~\cite{Wang_Kivelson_Lee_NatPhys_2015} 
%has 
similarly 
%proposed 
conjectured a possible DQC from
...
and from N\'eel to cVBS 
as a ``doubled" 
%version of the 
spin-$\frac{1}{2}$
DQC theory~\cite{doubled_theory_footnote}.
We %are motivated to 
investigate this latter scenario 
due to the following heuristic. %explained below.
%The class of 
$JQ$ models~\cite{Sandvik_prl2007} 
%originated by Sandvik~\cite{Sandvik_prl2007} 
have been crucial in probing DQC physics 
%on bipartite lattices
in large-scale simulations. The basic units 
%of these models 
are 
%the 
...
%A nice aspect of the projector formulation is that 
...
%in terms of biquadratic exchanges
%due to the exact mapping 
% between the $SU(3)$ singlet projector
...
%As a spin-$1$ Hamiltonian, this maps to 
% biquadratic
...
%in the form of the  spin-$1$ Heisenberg exchange which also 
...
\label{eq:SU2Ham}
\end{equation}
%We study the N\'eel-cVBS quantum phase transtion 
by varying 
$g \equiv \frac{Q_B}{J_B}$ for different %values of 
$J_H$ using  
QMC~\cite{Desai_Kaul_prl2019,Sandvik_1991,Sandvik_1992,Sandvik_2010_review}.
% With the recent 
...
% $S=1/2$ models. 
% Briefly, this is achieved by working in an expanded Hilbert 
...
% subspace~\cite{Kawashima_1995,Todo_2001}. 
%in the QMC ensemble.
We note for later discussion that 
%the QMC ensemble in 
our QMC simulations work with 
...
had 
%instead 
studied the $J_{B}=0$ case and found 
...
%In presence of the $SU(2)$-symmetric perturbation, 
...
%are plotted as a function of the coupling 
versus $g=\frac{Q_B}{J_B}$
for a representative value of $J_H=0.05$.
%The ratios plotted for different system sizes show 
We see a clear crossing at the 
transition 
%as shown  
...
%Having established a direct transition, 
We now 
%take a 
look at 
order parameter histograms to probe the nature of this
direct transition. 
No signature of two-peak behavior is 
seen
%in the histograms of the order parameters 
...
are %will be 
those of the doubled-DQC theory~\cite{monopole_footnote}.
...
%of Lou \textit{et al}~\cite{Lou_Sandvik_Kawashima_PRB_2009}
...
studied, 
%in the $SU(3)$ language, 
%Kaul had 
it had been noted that this dip grows 
sub-extensively with system size and interpreted as evidence for a 
continuous transition. We similarly find 
%that this negative dip grows slower than 
sub-extensively growing dips 
%in the regime of 
for small $J_H$
...
but the spinons get weakly confined at much larger length scales (dependent
on $J_H$)
...
%by Nakayama and Ohtsuki~\cite{Nakayama_Ohtsuki_PRL_2016} 
pointed out strong constraints on the scaling dimensions
that apparently rule out emergent symmetry in $2+1d$ DQC.
%Ma, Wang~\cite{Ma_Wang_PRB_2020}, and Nahum~\cite{Nahum_PRB_2020} had
...
Recent quantum entanglement based results~\cite{Zhao_etal_PRL2022,yuan_etal_arxiv2023,Song_etal_Arxiv2023} 
% by Zhao \emph{et al}~\cite{Zhao_etal_PRL2022}, 
% Diao \emph{et al}~\cite{yuan_etal_arxiv2023} 
% and Song \emph{et al}~\cite{Song_etal_Arxiv2023} 
have significantly 
...
%Bartosch~\cite{Bartosch_PRB_2013} 
...
%by Shao, Guo and Sandvik 
%as another possible resolution~\cite{Shao_Guo_Sandvik_Science2016}. 
...
Near the apparent transition, the confinement length scale 
...
scenario. 
%as yet not seen in the literature.
