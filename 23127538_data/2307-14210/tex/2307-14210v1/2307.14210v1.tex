...
\end{eqnarray}
where $I_{E}(g)$ represents the Euclidean action that is under investigation, $\xi$ is the Lagrange multiplier and $C(g)-\mu=0$ is the constraint equation. We shall consider the three-dimensional gravity introduced in \cite{Tekin1}. In $2+1$ dimensions, Einstein's gravity with or without a cosmological constant has no local degrees of freedom. Hence, it has no resemblance to the four-dimensional Einstein's theory. In \cite{NMG}, a massive three-dimensional gravity theory was introduced which has two propagating degrees of freedom, just like the four-dimensional Einstein's gravity. The action of this new massive gravity theory (NMG) is 
\begin{eqnarray}
...
\begin{enumerate}
\item For $\lambda_0 = 0$, unlike any generic finite order theory besides the cosmological Einstein's gravity, it has a {\it unique} maximally symmetric vacuum with an effective cosmological constant $\Lambda=m^2\lambda$ given as \cite{Tekin2,Nam}
\begin{equation}
...
\item The theory has a unitary massive spin-2 degree of freedom (with $\pm 2$ modes) in flat space with mass $m_g=m$; and with mass $m_g = m \sqrt{1 + \lambda}$ around the AdS backgrounds. This provides an infinite-order extension of the quadratic NMG which has the same perturbative properties.
\item In three dimensions, the Riemann and the Ricci tensors both carry 6 independent components, hence a generic gravity theory built from the contractions and the powers of the Ricci curvature is of the form \cite{Gurses}
\begin{equation}
...
\end{equation}
where $i$ in $\left(R_\nu^\mu\right)_j^i$ represents the number of Ricci tensors and $j$ represents the number of possible ways to contract the $i$-number of Ricci tensors. Generically, each higher curvature combination has a different dimensionful coupling constant denoted by $a_{n i}^j$. This theory generically has many maximally symmetric vacua, many massive degrees of freedom, and is non-unitary at the tree level about its constant curvature vacua. However, due to the determinantal nature of the Lagrangian, BINMG (\ref{BINMG}) as a very specific example of the generic theory (\ref{Gen}) is consistent at infinite order or at any finite truncation in powers of curvature. By consistency, we mean: it reproduces, up to desired order in the curvature expansion, the extended NMG theories that are consistent with the AdS/CFT duality and that have a c-function \cite{Tekin2, Sinha, Paulos}. See \cite{Sisman} for details of these calculations. 
...
Given this $h(r)$ in this saddle point metric one can determine $N(r)$ but the O.D.E. is still highly nonlinear and it pays to discuss the boundary conditions first.  We expect the lapse function $N(r)$ to vanish on the surface of the constraint volume, $N(R_V)=0$, where $R_V$ can be found by using the constraint equation, $\int d^2x \sqrt{\gamma}=V$. In analogous to the discussion given in \cite{Visser1} to remove the canonical singularity at the surface of the constraint boundary we impose 
\begin{equation}
...
For the flat spacetime, $\lambda_0=0$, $\Lambda = 0$ and $h(r)=1$. When we insert (\ref{metric}) to (\ref{field}), one finds that in order to obtain $\mathcal{E}_{\phi\phi}=0$ since $T_{\phi\phi}=0$, the lapse function should be of the form $N(r) = \alpha r^2 + \beta $, where $\alpha$ and $\beta$ are to be found from the boundary conditions. The first boundary condition, namely the lapse function should vanish on the surface of the volume constraint, gives $N(r) = \alpha \left(r^2-R_V^2\right)$. For the second boundary condition, we have 
\begin{equation}
...
\end{equation}
Assuming $N(\chi) = \alpha \cos\chi + \beta$. The first boundary condition on the lapse function discussed in the previous section gives $N(\chi) = \alpha \left(\cos\chi-\cos\chi_V\right)$. From the second boundary condition, one determines $\alpha=\frac{1}{\sin\chi_V}$.
...
\end{equation}
which is the same result obtained in \cite{Tekin2}.  The next order of business is to evaluate the action of the constrained instanton metric (\ref{metric2}). In the $u$-cordinate the action looks a little cumbersome, but in the $\chi$ coordinate, it becomes
\begin{eqnarray}
...
Following Jacobson and Visser, \cite {Visser1}, who generalized the notion of the gravitational entropy of horizons to the constrained finite spatial volume of $n$-ball topologies in (cosmological) Einstein's gravity, we computed the quantum gravity partition function at the saddle point approximation in an infinite order gravity theory of the  Born-Infeld form in $2+1$ dimensions. The theory has two propagating degrees of freedom at the perturbative level and is locally non-trivial, unlike the three-dimensional Einstein's gravity. Our computations support the conclusions of \cite{Visser1}: the dimension of the Hilbert space of all states of a given disk with a boundary is equal to the exponential of the Bekenstein-Hawking, Gibbons-Hawking or Wald entropy of it. These results are encouraging: In a coming work, we shall give a similar analysis in the $n$-dimensional Born-Infeld gravity theory that has the same perturbative and vacuum structure as Einstein's theory \cite{Tekin4, Tekin5, Tekin6}. 
...
\begin{thebibliography}{10}
	...
	Phys. Rev. Lett. \textbf{130}, no.22, 221501 (2023).	
	...
	Phys. Rev. D \textbf{15}, no.10, 2752 (1977).
	
\bibitem{Bala}
V.~Balasubramanian, P.~Horava and D.~Minic,
``Deconstructing de Sitter,''
JHEP \textbf{05}, 043 (2001).
	
	
	\bibitem{Fischler}
	W.~Fischler,
	``Taking de Sitter seriously,''
	Talk given at "Role of Scaling Laws in Physics and Biology" (celebrating the 60th birthday of Geoffrey West), Santa Fe (2000).
	...
	SciPost Phys. \textbf{14}, no.2, 022 (2023).
	
	\bibitem{Affleck}
	I.~Affleck,
	``On Constrained Instantons,''
	Nucl. Phys. B \textbf{191}, 429 (1981).
	...
	JHEP \textbf{08}, 149 (2014).

	\bibitem{Wald}
	R.~M.~Wald,
	``Black hole entropy is the Noether charge,''
	Phys. Rev. D \textbf{48}, no.8, R3427-R3431 (1993)
	
	\bibitem{Tekin3}
	D.~G.~Ozen, S.~Kurekci and B.~Tekin,
	``Entropy in Born-Infeld gravity,''
	Phys. Rev. D \textbf{96}, no.12, 124038 (2017).
