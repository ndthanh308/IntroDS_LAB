...
Extensive experiments are conducted on two large-scale datasets, the interaction subsets of NTU RGB+D 60 and NTU RGB+D 120. The results show that our network outperforms state-of-the-art approaches across all standard evaluation settings.
...
Interactions between two people, such as handshaking and hugging (see Fig.~\ref{fig_example}), represent a fundamental aspect of human life. 
...
For skeleton-based approaches, they are common in single-person action recognition, but there are still challenges in human-to-human interaction action recognition \cite{rgbdhm}.
There are more body overlaps and occlusions in human-human interaction actions.
In the process of obtaining skeleton data from RGB-D data for further action recognition, errors in skeleton estimation can accumulate mistakes in action recognition and lead to performance degradation \cite{3dv}.
% A more intuitive idea is to perform action recognition directly from the raw data, avoiding the accumulated errors in the skeleton estimation process.
...
In interaction learning, frame sampling plays a critical role due to the high frame rate of videos, typically containing a dozen frames per second, with many adjacent images containing redundant information. 
...
\item 
Our network outperforms state-of-the-art approaches across all four standard evaluation settings of the interaction subsets of NTU RGB+D 60 and NTU RGB+D 120 datasets. This remarkable performance improvement validates the effectiveness of our proposed TMDPT model and its potential for practical applications.
...
However, a significant challenge with using skeleton data lies in the robustness of skeleton estimation, which is still not trivial. Inaccurate and incomplete estimated data can adversely impact recognition performance, posing a crucial limitation to the effectiveness of skeleton-based methods.
...
While features from multiple modalities can offer valuable information for recognition, these methods often require large data sizes, which can impact the efficiency of the networks. 
Additionally, many of these approaches involve using RGB images as part of the input, which may expose excessive personal privacy information. 
Consequently, they might not be suitable for certain real-world applications where users prefer unobtrusive monitoring.
...
The overview of our Two-stream Multi-level Dynamic Point Transformer (TMDPT) is depicted in Fig.~\ref{fig_overview}. TMDPT comprises four main components: a novel frame sampling scheme, a frame features learning module, a feature aggregation module, and a transformer classification module. 
...
To begin, we convert depth videos into point cloud videos by mapping depth values to coordinates \cite{seq}. 
...
We introduce a novel frame sampling technique called Interval Frame Sampling (IFS) (refer to Fig.~\ref{fig_ifs}). 
...
The frame features learning module outputs three types of features: local-region features, frame spatial features, and frame temporal features, which are obtained through a series of feature extraction steps (refer to Fig.~\ref{fig_feat_learn}).
...
Where $ FS_t $ is the frame spatial feature of the $ t\text{-th} $ point cloud frame ($ PC_t $). $ e^t_j $ is the abstracted feature of the $ j\text{-th} $ local region from the $ t\text{-th} $ frame. SA is the set abstraction operation.
...
Where $ t $ is the temporal position. $ 2j $ and $ 2j+1 $ represent the dimension serial number. $ m3 $ is the size of a frame spatial feature. Each dimension of the sinusoidal positional vector corresponds to a sinusoid with a period of $10000^{2j/m3} \times 2\pi$. 
...
Where $ FT_t $, $ FS_t $ and $ TP_t $ are $ t\text{-th} $ frame temporal feature, frame spatial feature, and temporal positional feature, respectively.
...
With frame-level features generated, we propose a two-stream module to aggregate them into global and partial representations (the whole architecture with the following transformer classification module can be seen in Fig.~\ref{fig_two_multi}).
...
Where $ S_g $ is the feature representation for the whole video, $ S_i $ is the feature representation for the $ i\text{-th} $ temporal segment, and $ S $ is an integrated feature vector containing both global and partial information. 
...
\resizebox{0.5\columnwidth}{!}{
    ...
    \hline
    TMDPT (Ours) &     2022 & \textbf{96.6} & \textbf{99.0} \\
    \hline
    \end{tabular}
}
\label{tab1}
...
We compare our network’s performance to the state-of-the-art methods on the two large-scale 3D two-person interaction datasets, the interaction subsets of NTU RGB+D 60 and NTU RGB+D 120. The comparison results can be seen in Table.~\ref{tab1} and Table.~\ref{tab2}. Impressively, our network achieves remarkable results. It consistently outperforms all the other methods on all four standard evaluation settings of these two challenging datasets.
...
\resizebox{0.5\columnwidth}{!}{
    ...
    SequentialPointNet \cite{seq} & 2022 & 91.8 & 92.2 \\
    \hline
    TMDPT (Ours) & 2022 & \textbf{92.7} & \textbf{93.3} \\
    \hline
    \end{tabular}
}
\label{tab2}
...
\subsubsection{NTU RGB+D 120} Compared with NTU RGB+D 60, it is even harder to conduct interaction analysis on NTU RGB+D 120 (see point cloud interaction examples in Fig.~\ref{fig_vis}). It is not only because NTU RGB+D 120 contains a larger amount of data than NTU RGB+D 60, but also because it has much higher variability in many different aspects, including action categories, subjects, camera views, and environments. As can be seen in Table.~\ref{tab2}, despite facing more challenges, our network still achieves excellent results. With achieving an accuracy of 92.7\% on the cross-subject setting and 93.3\% on the cross-view setting, our network outperforms all the state-of-the-art approaches again. This essentially demonstrates the effectiveness of our network for handling two-person interaction recognition tasks under complex subjects, environments, and viewpoints conditions.
...
\subsubsection{Effectiveness of Two-stream feature aggregation structure} In our network, a two-stream feature aggregation structure is proposed to aggregate global and partial information simultaneously. To evaluate its effectiveness, we create another network by removing the two-stream component with the following transformer and only using a global feature for the final classification. 
...
As shown in Fig.~\ref{fig_comp2} (A), the accuracy is first improved when there are more blocks, reaching a peak when the transformer contains 5 blocks. 
...
Then, we analyze the influence of the number of heads on the performance. Fig.~\ref{fig_comp2} (B) shows that the multi-head design can improve TMDPT’s learning ability, as long as not too many heads are used. One main reason for this result is that a transformer with a reasonable number of heads can jointly attend to information from different representation subspaces at different positions to improve performance.
