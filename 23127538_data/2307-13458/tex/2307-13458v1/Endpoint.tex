...
\begin{abstract}
We calculate two-point massless QCD correlator of nonlocal (composite) vector quark currents with chains of fermion one-loop radiative corrections inserted into gluon lines.
...
The paper is organized as follows. In Section~\ref{sec:2}, we start with the results of calculating the two-point correlators $\Pi_{n}(x,\underline{0};L)$ of nonlocal vector quark currents within the $n$-bubble approximation
...
which have an immediate application in the area of QCD SRs, as discussed in Section~\ref{sec:SRs}.
...
Within the approach of QCD SR, the Borel transform $\hat{\mathbf{B}}$ (see Appendix \ref{sec:Borel}) of the correlator~\eqref{eq:cor-def} determines the perturbative contributions to meson DAs,
\begin{gather}\label{eq:DA-def}
\!\!\!\!\text{DA}(x;L_\text{B})\! \sim \!\frac{ N_c}{\pi^2}\Big[ \frac12 x \bar{x}+ a_s C_F\hat{\mathbf{B}}\sum_{n \geqslant 0} A^n \Pi_{n}(x,\underline{0};L)\Big],
\\
\Pi_n(x,\underline{0};L)=\int_0^1 \Pi_n(x,y;L) dy,
\end{gather}
where $\bar{x} =1-x$ and $L_\text{B} = \ln(M^2/\mu^2)$ is the logarithm of the Borel parameter $M$ appearing in the Borel transform $\hat{\mathbf{B}}[L^m]$ of
...
\end{align}
Further, we will consider two quantities derived from $\Pi_{n}$ --- its Borel image \eqref{eq:borel}, $\hat{\mathbf{B}}[\Pi_{n}]$,
and the derivative $\displaystyle \dot{\Pi}_{n}\equiv \frac{d}{dL}\Pi_{n}$,
...
\end{align}
\begin{gather}
\sum_{n \geqslant 0} u^n \hat{\mathbf{B}}[\Pi''_{n}(\underline{-1},\underline{0};L)] = \frac{1}{u} \int_0^u \mathrm{d}a F(\underline{-1},a),
\end{gather}
where
\begin{align*}
...
+ \left(f_{A_1}\right)^2\varphi_{A_1}(x) e^{-m^2_{A_1}/M}
\notag \\
&{}= \Phi_\text{PT}(x;M;s_0^A) + \Phi_S(x;M) + \Delta_C \stackrel{\text{def}}{=} \Phi_\pi(x,M),
\label{eq:sr-pi} \\
& \left(f^{\|}_{\rho}\right)^2 \varphi^{\|}_{\rho}(x)e^{-m^2_\rho/M}
 + \left(f^{\|}_{\rho'}\right)^2\varphi^{\|}_{\rho'}(x) e^{-m^2_{\rho'}/M}
\notag \\
...
\begin{eqnarray}
\Phi_\text{PT}(x;M;s_0^A) = \int_{0}^{s_0^A} \rho_\text{pt}(x;s)e^{-s/M}ds,\, \label{eq:PT-rho}
\end{eqnarray}
...
Remarkably, the r.h.s.\ $\Phi_{\pi}$ and $\Phi_{\rho}$ of QCD SRs (\ref{eq:sr}) for these two channels differ only in sign of the scalar-condensate contribution $\Phi_S$.
The reason for that was discussed in \cite{Mikhailov:2021kty}.

The purely perturbative contributions $\Phi_\text{PT}(x;M;s_0^A)$ and $\Phi_\text{PT}(x;M;s_0^V)$ in the r.h.s.\ of Eqs.~\eqref{eq:sr-pi} and \eqref{eq:sr-rho} can be obtained from higher order corrections to $\Pi(x;L)$ by integrating the spectral density $\rho_\text{pt}(x;s)= \mathbf{\hat{B}^2}_{(s \to P^2)}\Pi(x;L)$ in Eq.~(\ref{eq:PT-rho}), see Appendix~\ref{sec:rho}.
These perturbative terms dominate in the r.h.s.\ of Eqs.~\eqref{eq:sr-pi} and \eqref{eq:sr-rho} in accordance with the standard practice of processing QCD SR \cite{Shifman:1978bx}.
...
\end{align}
In the vicinity of the endpoints $x=0$ and 1, the scalar condensate $\Phi_S(x;M)$ dominates the nonlocal condensate (NLC) contributions that include condensates $\Delta\Phi_{G,V,T_i}(x;M)$ \cite{Bakulev:1998pf,Bakulev:2001pa,Bakulev:2004mc} collected in the term $\Delta_C(x;M)$ in Eq.~(\ref{eq:Delta}). To estimate the behavior near the endpoints, we take into account only these \textit{two dominant terms}, $\Phi_\text{PT}$ and $\Phi_S$, in the r.h.s.\ of Eqs.~(\ref{eq:sr-pi}) and \eqref{eq:sr-rho}, which is represented diagrammatically in Fig.~\ref{fig:renorm+cond}.
...
Let us call ``reduced NLC SRs''  those that contain in their r.h.s.\ of (\ref{eq:sr}) only the dominant
...
With growing  powers of $\beta_0 a_s$, the bubble-chain corrections lead to ``swelling'' of the perturbative part $\Phi_\text{PT}$ of DA $\varphi_\pi$ at the endpoints, which is shown in the top panel of Fig.~\ref{fig:qcdsr} in comparison with the leading-order contribution --- the asymptotic $\Phi_\text{As} = 6 x \bar{x}$. We restrict ourselves to considering orders up to $a_s(a_s\beta_0)^3$ (the next order $a_s (a_s\beta_0)^4$ does not change the result significantly) for which the series convergence stays good enough and the series does not yet succumb to factorial growth, see the discussion in Subsections~\ref{sec:2.2}--\ref{sec:2.3}.
% Figure environment removed
At the same time, the corrections to the NLC part $\Phi_S(x;M^2)$ have the opposite effect, see left panel of Fig.~\ref{fig:qcdsr-rho} --- they alleviate the swelling of the perturbative part $\Phi_\text{PT}$.
The final result of this mutual compensation in the sum $\Phi_\text{PT}(x;M)+\Phi_S(x;M^2)$ is illustrated in Fig.~\ref{fig:qcdsr} (bottom panel).
We should mention here that Eqs.~(\ref{eq:sr}) should be considered as equalities \textit{in a weak sense}, i.e., for smooth convolutions of both sides of equations within the stability domain in $M$.
Usually, such convolutions are chosen as moments $ (\xi=2x-1)^N$ or $x^{-1}$, but, in general,
it can be any appropriate function of $x$.
In addition, the r.h.s.\ of the QCD SR for DA should not be a smooth function of $x$, the smoothness of its behavior depends on a certain model for NLC, see, e.g.,
...
This model ignores any (still speculative) details of vacuum quark-gluon distributions at the cost of finite discontinuous contributions to the r.h.s.\ of NLC SR, see the behaviour of solid blue/red curves for $\Phi_S(x;M^2)$ in Fig.~\ref{fig:qcdsr-rho} (top). The contribution of $\Phi_S(x;M^2)$ is comparable to $\Phi_\text{PT}(x;M)$ near the
%left endpoint
lower bound $M_-$ (blue curve) of the stability interval and significantly decreases at the %right endpoint
upper bound $M_+$ (red curve). Let us briefly clarify the calculation of $\Phi_S(x)$ presented as a right diagram in Fig.~\ref{fig:renorm+cond}.
...
C(M) = \frac{ m_{\rho}^2}{M} + \ln \left(f_{\pi}^2/f_{\rho}^2\right),
\end{align}
\begin{align}\label{eq:Delta-A1}
\Delta_{A_{1}\rho'}(x, M)
= {}&\left(\frac{f_{A_1}}{f_\pi} \right)^2 e^{-m_{A_1}^2/M} \varphi_{A_1}(x)
\notag\\
&{}- \left(\frac{f_{\rho'}}{f_\pi} \right)^2 e^{-m_{\rho'}^2/M} \varphi_{\rho'}(x).
\end{align}
...
Keeping only the contribution $\Phi_\text{PT}(x;M)-\Phi_S(x;M)$ (reduced NLC SR) in the r.h.s.\ of (\ref{eq:sr-rho}), one gets the profile of $\varphi_{\rho}^{\|}(x)$ that becomes wider near the endpoints. This endpoint ``swelling'' of the $\varphi_{\rho}^{\|}(x)$ profile is seen clearly in Fig.~\ref{fig:qcdsr-rho} (bottom panel). This effect can be traced back also in the representation (\ref{eq:rho-pi-DAs}) for $\varphi_{\rho}^{\|}(x)$ through $\varphi_\pi(x)$.
% Figure environment removed
...
Since the omitted contribution $\Delta_C$ in the r.h.s.\ of the NLC SR (\ref{eq:sr}) is the same for both channels, it does not violate the inequalities.
...
\\
a^{\pi}_2 \text{ (LQCD \cite{Bali:2019dqc}}) \approx a^{\pi}_2 \text{ (SR \cite{Mikhailov:2021znq}}) \notag
\end{gather}
at $\mu_\text{Latt}^2=4$ GeV$^2$, $\lambda^2_q \approx 0.45$ GeV$^2$.
Note that the previous versions of NLC SR for DAs of $\pi$ and $\rho^{\|}$ \cite{Mikhailov:2021kty,Pimikov:2013usa} yielded an opposite hierarchy of the moments, $a^{\rho^{\|}}_2 = 0.032(46) < a^{\pi}_2 = 0.149^{+0.052}_{-0.043}$.
...
$$
Let us emphasize that we insist on the validity of inequality \eqref{eq:ineq-a2} for $a_2^\text{M}$, $\text{M}= \pi$ and $\rho_{\|}$, per se, rather than on the precise values of the moments which serve only as an illustration here.
...
$\varphi_\pi(x)$, do not change the behavior of $\varphi_\pi(x)$ at the endpoints $x=0$ and 1.
This result is in accord with the previous calculations and phenomenological analysis of the transition form factor
of the process $\gamma+\gamma^* \to \pi^0$ \cite{Bakulev:2003cs,Mikhailov:2021znq}.
...
The Borel transform $\hat{\mathbf{B}}[f(t)]$ of the function $f(t)$ reads
\begin{gather}
\label{eq:borel}
\mathbf{\hat{B}} \left[ f(t) \right](\mu) = \lim_{\begin{subarray}{c} t=n\mu \\ n\to \infty\end{subarray}} \frac{(-t)^n}{\Gamma(n)} \frac{\mathrm d^n }{\mathrm d t^n}f(t),
\\
\text{so,}~\mathbf{\hat{B}} e^{A L} = - \frac{A e^{A l}}{\Gamma(1-A)}.
\end{gather}
Here $L = \ln t$, $L_c = \ln t-\frac{5}{3}$, $l=\ln\mu$, $l_B=\ln(\mu e^{-\gamma_\text{E}})$.
The useful equations for $\mathbf{\hat{B}}$ are presented below,
\begin{equation}
...
\begin{subequations}
\begin{align}
\!\!\!\!\mathbf{\hat{B}} \left[ \ln^{m}(t) \right](\mu)\! =& m(-1)^m
...
\section{\label{sec:rho}Transform  to spectral density $\rho(s)$}
Let us define a ``double'' Borel transform $\mathbf{\hat{B}^2}_{(s)} \equiv \mathbf{\hat{B}^2}_{(s \to P^2)}$ to obtain the spectral density $\rho_\text{pt}(s)$ of $\Pi(L)$,
\begin{subequations}
\begin{gather}
\Pi(L) = \int^\infty_0 \frac{\rho_\text{pt}(s) ds}{s+P^2}-\text{subtracted terms},
\\
M\mathbf{\hat{B}}_{(M\to P^2)}\Pi(L) = \int^\infty_0 \rho_\text{pt}(s) e^{-s/M} ds,
\\
\mathbf{\hat{B}^2}_{(s)} \Pi(L) \equiv \frac{1}{s}\mathbf{\hat{B}_{(\frac{1}{s}\to \mu)}}\left[\frac{1}{\mu}\mathbf{\hat{B}_{(\frac{1}{\mu}\to P^2)}}\Pi(L)\right]=\rho_\text{pt}(s)
\end{gather}
%\begin{eqnarray}
%\Pi(L)&=& \int^\infty_0 ds \frac{\rho_\text{pt}(s)}{s+P^2}-(\text{subtracted terms}),\\
%M\mathbf{\hat{B}}_{(M\to P^2)}\Pi(L)&=&\int^\infty_0 \rho_\text{pt}(s) e^{-s/M}\,ds, \\
%\left[\mathbf{\hat{B}^2}_{(s)}\equiv\mathbf{\hat{B}^2}_{(s \to P^2)}\right]\Pi(L)&\equiv&\frac{1}{s}\mathbf{\hat{B}_{(\frac{1}{s}\to \mu)}}\left[\frac{1}{\mu}\mathbf{\hat{B}_{(\frac{1}{\mu}\to P^2)}}\Pi(L)\right]=\rho_\text{pt}(s)\,.
%\end{eqnarray}
\end{subequations}
For every power $L^n$ in $\Pi(L)$ one obtains a polynomial in $l_s=\ln(s)$:
\begin{eqnarray}
...
%\end{table*}
\end{widetext}
The key element of the perturbative contribution in the ``theoretical part" (r.h.s.) of the SR is the integration of $\ln^j(s)$ from (\ref{eq:B2}), $l_\text{B}=\ln(M)$,
\begin{align}
\int^{s_{0}}_0 \rho_\text{pt}(s) e^{-s/M}ds \Rightarrow M \int_0^{s_{0}/M}\ln^j(Mt) e^{-t}dt\,.
\end{align}
Taking into account the main terms of the structure of the correlator after summation,
i.e., the terms of the generating functions for $\Pi'$ and $\Pi''$ in Section~\ref{sec:2}
(see the terms in  braces below),
we present the results for these functions and their different derivatives,
\begin{align}
\rho_\text{pt}(s)
&\propto \mathbf{\hat{B}^2}_{(s)} \Big\{ \frac{\exp{(A L_c)}}{A}, L_c  \Big \}
\notag\\
&= \Big\{ - \exp{(A l_c)} \frac{\sin(\pi A)}{\pi A},\,-1\Big\}
\\
\mathbf{\hat{B}}_{(M)}\Pi(L)
&\propto \mathbf{\hat{B}}_{(M)} \Big\{ \frac{\exp{(A L)}}{A},\, L  \Big \}
\notag\\
&= \Big\{ - \frac{\exp{(A l_B)}}{\Gamma(1-A)},\,-1 \Big\}
 \\
\dot{\Pi}(L)
...
\begin{widetext}
\begin{eqnarray}
\mathbf{\hat{B}}_{(M)}\Pi^{(n)}_S(x,y) &\sim & (a_s \beta_0)^n \mathop{\hat{\mathbf{S}}} f(x,y)
\left[ \ln\left(\frac{\bar{\Delta}}{y-x}-1\right)-\ln\left(\frac{\delta^2}{\mu_0^2} \right) -c \right]^n ; \\
         &&\Delta=\frac{\delta^2}{M};~ \bar{\Delta}=1-\Delta;~ \delta^2= \frac{\lambda_q^2}2;~\mu_0^2=\mu^2e^{\gamma_E}; ~c=-\frac{5}{3}; \\
...
\bibinfo{author}{\bibfnamefont{G.~S.} \bibnamefont{Bali}},
  ...
  (\bibinfo{year}{2019}), \bibinfo{note}{[Addendum: JHEP 11, 037 (2020)]},
  \eprint{1903.08038}.
