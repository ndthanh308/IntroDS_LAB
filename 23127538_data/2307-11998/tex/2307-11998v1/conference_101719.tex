...
\begin{IEEEkeywords}
Odometry, LiDAR, Deep learning
\end{IEEEkeywords}

\section{Introduction}
Localization is one of the most essential modules in the autonomous robots for their real-world deployment. In recent years, LiDAR (Light Detection and Ranging) and vision-based \cite{qin2018vins, cao2022gvins, cvivsic2022soft2} localization methods have gained substantial interest because the localization quality of global navigation satellite system (GNSS) deteriorates in urban canyons due to the signal multi-path and weak signal strength \cite{kamat2018survey, sun2020robust}.
...
This method of representing 3D data in 2D does not fully reflect the characteristics of 3D data and has difficulty considering the sparse data characteristics. As a result, research has emerged to directly process the sparse, unordered set of 3D points as in \cite{qi2017pointnet, qi2017pointnet++}.
...
Recently, 3D data Transformer backbone is studied to increase the receptive field using self-attention algorithm \cite{misra2021end, mao2021voxel}. 
This research aims to improve performance by applying feature extraction and self-attention mechanism used in 3D detector research to LO task.
...
Deep point-cloud registration has been previously proposed as a solution to mitigate the substantial noise that can pervade real-world data \cite{horn2020deepclr}. However, conventional geometric concepts such as the \textit{KNN} algorithm are still utilized for motion embedding part \cite{Liu_2019_CVPR}.
...
\label{sec:implement}
% Inspired by \cite{shi2020pv}, we implement VSA module to encode the multi-scale semantic features into the $n_{key}$ of keypoints.
Raw points are consecutively sampled using FPS methods, and aggregated into feature abstracted set $\mathcal{S}_{sa, i}^{\mathcal{P}}$ and $\mathcal{S}_{sa, i}^{\mathcal{Q}}$.
\par
...
\par
The evaluation results with classic and learning-based methods are shown in Table \ref{tab:result}.
In addition, we have visualized the trajectories generated by our proposed method on KITTI odometry sequences 07 through 10 in Fig. \ref{fig:kitti_07_10}.
We have also compared the trajectories from our proposed method with those generated by traditional techniques, as well as with our reference learning-based studies. 
These comparisons further illustrate the effectiveness and improvements offered by our approach.

% \subsubsection{Mulran}


% \subsection{Ablation study}
% An ablation study on the KITTI dataset is performed to verify how efficiently the proposed Transformer block operated in the Motion flow module.
% We examine the results of replacing the transformer encoder-decoder with an MLP layer.
% As \cite{naseer2021intriguing} found transformers are extremely strong in handling occlusions in the field of vision detection leading to the advantage of better generalization, we found that the transformer encoder-decoder based self-attention module has a strong ability to generalize in the process of embedding flow.
% However, there is a trade-off where runtime time increases when the simple MLP layer is replaced with a transformer encoder-decoder block as shown in Table #.
...
\par
Through various experiments, we has demonstrated effectiveness of proposed method. 
While its current performance is close to traditional methods, its innovative approach suggests a strong potential for further improvement and refinement.
\par
One of the key strengths is its adaptability and scalability. 
It is designed to offer a versatile solution to a wide range of point cloud processing tasks. 
This versatility makes proposed framework not only a powerful tool for current applications, but also a promising platform for future advancements in the field.
