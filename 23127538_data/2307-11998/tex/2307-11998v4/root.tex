...
Our end-to-end approach forgoes conventional geometric concepts and leverages positional embedding networks to identify motion flow and emphasize key features, achieving geometric motion in a seamless manner.
...
Despite these advancements, geometry-based methods may not adequately account for memory considerations, whereas learning-based methods leverage data-driven approaches, as demonstrated in the work of Xue \textit{et al} \cite{xue2020deep}.
\par
...
Lidar odometry algorithms can be categorized into the following two types: geometry-based methods and learning-based methods.
...
Owing to registration algorithm's large computing consumption, feature-based LO methods \cite{shan2020lio, shan2018lego,xu2021fast} implement handcrafted feature extraction and constraint for solution space by utilizing inertial measurement unit (IMU) values. 
...
Geometry-based methods are intuitive for robot pose prediction and have shown high accuracy in benchmarks \cite{geiger2012we}. 
\par
...
In the wake of the PointNet proposals \cite{qi2017pointnet, qi2017pointnet++}, 3D CNN-based methods have been proposed for end-to-end registration \cite{Yew_2018_ECCV, Lu_2019_ICCV,wang2019deeppco}.
...
We adopt and extend the framework of the 3D Transformer \cite{misra2021end} to perform the LO task.
\par
...
\section{Conclusion}
\label{sec:conclusion}
In this work, we presented \textbf{ELiOT}, a novel architecture that seamlessly integrates point feature abstraction with a motion flow self-attentioning 3D transformer framework. 
% Our end-to-end LiDAR odometry approach unlocks new potential for efficient and accurate data interpretation without relying on conventional geometric methods.
\par
...
\par
Nonetheless, our deep learning-based LiDAR odometry method holds significant promise. Leveraging the potential of positional embedding networks to identify motion flow and emphasizing key features, our approach achieves geometric motion through an end-to-end framework. This makes it a valuable contribution to the field, offering a unique and promising perspective for advancements in autonomous navigation systems.
\par
To address the generalization issue, we plan to expand our training dataset with diverse sequences. Additionally, exploring the integration of novel techniques and architectural enhancements will further improve \textbf{ELiOT}'s capabilities and push the boundaries of LiDAR odometry accuracy and efficiency.
