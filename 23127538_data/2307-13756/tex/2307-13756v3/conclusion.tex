...
In spite of the state-of-the-art performance, there are still some limitations in PlaneRecTR++. For example, the rotational accuracy on scenes from Matterport3D still suffers from large outliers, which could be partly addressed using a robust kernal function on top of existing objective functions. In addition, it still remains unexplored how to extend PlaneRecTR++ to efficiently process sequential images for a larger scale of plane recontruction in an online fashion. An exciting future venue would be to leverage the learned correspondences for long-term plane-level tracking, possibly drawing recent advancements in Transformer-based video panoptic segmentation.
