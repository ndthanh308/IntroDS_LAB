...
\subsection{Hierarchical Text Generation}

\revise{\methodshort{} can be regarded as being ``hierarchical'' since it has high-level answer structure planning. Prior studies in hierarchical text generation~\citep{li2015hierarchical,shao2019long,puduppully2019data,fan2018hierarchical} all focus on enhancing the answer quality, including improving the long-range coherence, relevance to the topic, or reducing redundancy. These methods craft hierarchical neural architectures that contain different modules to model high-level (sentence-level or document-level) and low-level (word-level) dependencies~\citep{li2015hierarchical,shao2019long,fan2018hierarchical}. They still employ sequential word-by-word generation without parallelization between sentences.}

\revise{Note that the sentence-level representations in previous work~\citep{li2015hierarchical,shao2019long} are ``implicit'' latent variables instead of ``explicit'' language descriptions. Some previous studies~\citep{shao2019long,puduppully2019data} train a dedicated planning module to execute explicit content planning in advance. Nevertheless, these methods all conduct ``closed-form'' planning that only reorders and groups the input keywords, rather than producing ``free-form'' plans on ``what to say'' and ``how to say''. All the hierarchical architectures and planning modules require training or even special data processing~\citep{puduppully2019data}.}

\revise{To summarize, in terms of the objective, the primary focus of \methodshort{} -- efficient generation -- is different from previous hierarchical text generation literature. In terms of the methodology, instead of designing new hierarchical architectures or planning modules, \methodshort{} exploits the emerging planning and instruction-following abilities of LLMs to do explicit (which means the plan is described by interpretable language) and free-form planning. This allows \methodshort{} to be applied to off-the-shelf LLMs for producing structured answers.}

\revise{As the hierarchical text generation literature focuses on enhancing answer quality, they could provide inspiration for future expansions of \methodshort{} to generate high-quality answers for broader types of questions.}
