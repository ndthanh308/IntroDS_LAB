...
\def\svnVersion{9d70032}\def\svnDate{2023/12/30}\def\cmsCernNoTag{CERN-EP-2023-124}\def\cmsCernDate{\today}\def\cmsMessage{Published in the Journal of High Energy Physics as \href{http://dx.doi.org/10.1007/JHEP12(2023)068}{\doi{10.1007/JHEP12(2023)068}.}}
...
\newcommand{\twolss}{\ensuremath{2\Pell\text{ss}}\xspace}
\newcommand{\twolsstwob}{\ensuremath{2\Pell\text{ss }2\PQb}\xspace}
\newcommand{\twolssthreeb}{\ensuremath{2\Pell\text{ss }3\PQb}\xspace}
\newcommand{\threel}{\ensuremath{3\Pell}\xspace}
\newcommand{\threeloffZ}{\ensuremath{3\Pell\text{ off-}\PZ}\xspace}
\newcommand{\threeloffZoneb}{\ensuremath{3\Pell\text{ off-}\PZ\text{ }1\PQb}\xspace}
\newcommand{\threeloffZtwob}{\ensuremath{3\Pell\text{ off-}\PZ\text{ }2\PQb}\xspace}
\newcommand{\threelonZ}{\ensuremath{3\Pell\text{ on-}\PZ}\xspace}
\newcommand{\threelonZoneb}{\ensuremath{3\Pell\text{ on-}\PZ\text{ }1\PQb}\xspace}
\newcommand{\threelonZtwob}{\ensuremath{3\Pell\text{ on-}\PZ\text{ }2\PQb}\xspace}
...
In addition to these SM contributions, the analysis also aims to probe new physics effects that may impact these final state signatures without an intermediate boson (via a four-fermion EFT vertex).
...
\begin{table}[!htp]
\centering
\topcaption{Theoretical cross sections at next-to-LO (NLO) used for normalization of simulated signal samples. The items are ordered by cross section. The uncertainties are broken into normalization components due to modeling the parton distribution functions (PDFs) and QCD order. Entries without a value are negligible.}
...
\cmsTable{\begin{tabular}{lrll}
    ...
    \ttll ($\mll>10\GeV$) & $0.113\,^{+12\%}_{-10\%}\QCD$ & NLO (QCD + EWK) & \cite{LHCHiggsCrossSectionWorkingGroup:2016ypw} \\
...
Photons 
are identified as ECAL energy clusters not
...
Expressed mathematically, the \ptljz variable can be described as follows: 
\begin{linenomath}
\begin{equation}
\label{eq:ptljz_def}
   \ptljz = \max\left( \max \left[ \pt(\ell,\ell')\right], \, \max \left[ \pt(\PQj,\PQj') \right], \, \max \left[ \pt(\ell,\PQj) \right] \right),
\end{equation}
\end{linenomath}
where $\max[\pt(\ell,\ell')]$ indicates the \pt of the pair of unique leptons with the largest \pt, $\max[\pt(\PQj,\PQj')]$ indicates the \pt of the pair of unique jets with the largest \pt, and $\max[\pt(\ell,\PQj)]$ indicates the \pt of the lepton-jet pair with the largest \pt.
...
The SFs for the heavy-flavor (\PQb and \PQc quark) jets are varied together, and the SFs for the light-flavor (gluon and \PQu, \PQd, and \PQs quark) jets are also together, independently from the heavy-flavor SFs.
...
We have quantified the level of agreement between data and the SM hypothesis using a saturated model. We obtain a p-value of 0.18, showing no significant discrepancies between the two.
...
\cleardoublepage \appendix\section{The CMS Collaboration \label{app:collab}}\begin{sloppypar}\hyphenpenalty=5000\widowpenalty=500\clubpenalty=5000\input{TOP-22-006-public-authorlist.tex}\end{sloppypar}
%%% END EDITABLE REGION %%%
% skeleton_end
\end{document}
