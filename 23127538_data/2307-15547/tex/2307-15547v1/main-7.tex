\documentclass[aps,preprint,prd,nofootinbib]{revtex4}
...
\author{Jun-Qian Jiang$^{1}$\footnote{jiangjq2000@gmail.com}}
\author{Yong Cai$^{2}$\footnote{caiyong@zzu.edu.cn}}
\author{Gen Ye$^{3}$\footnote{ye@lorentz.leidenuniv.nl}}
\author{Yun-Song Piao$^{1,4,5,6}$\footnote{yspiao@ucas.ac.cn}}

\affiliation{$^1$ School of Physical Sciences, University of
Chinese Academy of Sciences, Beijing 100049, China}

\affiliation{$^2$ School of Physics and Microelectronics, Zhengzhou University, Zhengzhou, Henan 450001, China}

\affiliation{$^3$ Leiden University, Instituut-Lorentz for Theoretical Physics, 2333CA, Leiden, Netherlands}

\affiliation{$^4$ International Center for Theoretical Physics
    Asia-Pacific, Beijing/Hangzhou, China}

\affiliation{$^5$ School of Fundamental Physics and Mathematical
    Sciences, Hangzhou Institute for Advanced Study, UCAS, Hangzhou
    310024, China}

\affiliation{$^6$ Institute of Theoretical Physics, Chinese
    Academy of Sciences, P.O. Box 2735, Beijing 100190, China}



%\author{author$^{1}$\footnote{email@example.com}}

%\affiliation{$^1$ some affiliations}
...
implications of current PTA and CMB data on IGW. In this paper, we
perform a joint Markov Chain Monte Carlo analysis for a broken
...
\cite{NANOGrav:2023hvm}. Though such a SGWB is compatible with
that brought by inspiralling supermassive black holes binaries
\cite{NANOGrav:2023hvm}, see also
\cite{Huang:2023chx,Depta:2023qst} for the supermassive primordial
black holes, it might be just IGW but with $n_\text{T}>0$
\cite{NANOGrav:2023hvm,Vagnozzi:2020gtf,Vagnozzi:2023lwo}\footnote{In
addition, see also recent other possibilities
e.g.\cite{Ghosh:2023aum,Niu:2023bsr,Konoplya:2023fmh,Li:2023bxy,DiBari:2023upq,Zhu:2023faa,Du:2023qvj,Ye:2023xyr,Balaji:2023ehk,Zhang:2023nrs}.}.
...
scale. It is possible for inflation to yield a blue-tilted SGWB with
$n_\text{T}>0$
\cite{Piao:2004tq,Piao:2003ty,Piao:2004jg,Baldi:2005gk,Liu:2010dh,Kobayashi:2010cm,Kobayashi:2011nu,Fujita:2018ehq,Endlich:2012pz,Cannone:2014uqa,Giare:2020plo,Cai:2016ldn,Cai:2015yza},
...
\cite{Liu:2011ns,Liu:2012ww,Nishi:2015pta,Nishi:2016ljg}, it might
be more reliable to consider a broken power-law IGWB at $f\sim
10^{-18}-10^{-8}$Hz,
\begin{equation}
P_\text{T}(k) = r A_s
...
, \label{PT2}\end{equation} where $k=2\pi f$, and
$k_\text{break}\ll 10^{-8}$Hz is the scale at which power-law is
broken. In
Refs.\cite{Kuroyanagi:2014nba,Kuroyanagi:2020sfw,Benetti:2021uea}
such a broken SGWB has been studied but with
$k_\text{break}>10^{-7}$Hz. According to \autoref{PT2}, we have
$P_\text{T}= r A_s
\left(\frac{k}{k_\text{pivot}}\right)^{n^\text{CMB}_\text{T}}$
...
such a IGW spectrum will be conflicted with the BBN bound on
...
NANOGrav data favors $n_\text{T}\simeq 2$. As a result, IGW at CMB band
is negligible. Thus recent CMB data has not been included in the
analysis of NANOGrav \cite{NANOGrav:2023hvm} (a hard prior
...
data with full exact likelihood for exploring the underlying
impact of current data on IGW.

However, a joint Markov Chain Monte Carlo (MCMC) analysis of
NANOGrav 15-year and BICEP/Keck 2018 data has been still open.
Here, we perform the first such analysis, and find that the
...
$n^\text{CMB}_\text{T} \simeq 0$ is also compatible.
...
($n_\text{T}^\text{PTA}\simeq 2$) tilt must be cut off at certain
$k_{cutoff}=2\pi f_{cutoff}$ to avoid the confliction with the BBN
bound. In \autoref{fig:future}, what is $P_\text{T}$ at
...
\cite{Ashoorioon:2014nta,Choudhury:2023kam} and the collisions of
bubbles nucleated \cite{Li:2020cjj,Wang:2018caj}. In light of our
MCMC results on $r$, $n_\text{T}^\text{CMB}$ and
$n_\text{T}^\text{PTA}$, and also bestfit $\Omega_{GW}(f)$ in
\autoref{fig:future}, it is interesting to resurvey the relevant
models.
