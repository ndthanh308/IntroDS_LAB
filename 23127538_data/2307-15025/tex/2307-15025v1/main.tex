...
%
\begin{titlepage}
%\title{{\huge The Generalised Born Oscillator and the Berry-Keating Hamiltonian Hilbert-P\'{o}lya Conjecture}}
\title{{\huge  The Generalised Born Oscillator
...
\begin{abstract}
%
In this study, we introduce and investigate a family of quantum mechanical models in 0+1 dimensions, named generalised Born quantum oscillators. These models constitute a one-parameter deformation of a particular system obtained by dimensionally reducing the Nambu-Goto theory to $0+1$ dimensions. While these models bear a close resemblance to the $\TTB$-type perturbations of two-dimensional relativistic models, our analysis reveals that they can be seen as regularizations of the Berry-Keating theory.
Furthermore, by utilizing the Weyl quantisation method, we demonstrate the emergence of a clear relationship between the generalised Born quantum oscillators and the smooth part of the counting function for the non-trivial zeroes of the Riemann zeta function.
%
\end{abstract}
\end{titlepage}
\newpage
...
\label{sec:intro}
%
In the early 20th century, Hilbert and Pólya speculated about the imaginary parts of the complex zeros of the Riemann zeta function, $\zeta(s)$. They proposed that a self-adjoint operator  $H$ could have these imaginary parts as eigenvalues. The existence of such an operator would confirm the famous Riemann hypothesis, which asserts that all complex $\zeta(s)$ zeros lie on the critical line, where the real part of $s$ equals $1/2$. The potentiality of the spectral approach to the Riemann hypothesis fully emerged after Selberg discovered a trace formula connecting the eigenvalues of the Laplacian on a compact hyperbolic surface to its geodesics. Notably, this formula resembles the Riemann-Weil explicit formula, which relates the Riemann zeros to prime numbers.
Further progress,  was made by Montgomery and Odlyzko. Assuming the Riemann hypothesis, they deduced that the local statistical behaviour of the Riemann roots could describe the Gaussian unitary ensemble (GUE) of random matrix theory. Later, Berry in \cite{BerryChaos}  put forward a conjecture, drawing inspiration from analogies between trace formulas in number theory and the Gutzwiller formula for chaotic dynamical systems \cite{gutzwiller1983stochastic}. According to \cite{BerryChaos}, a particular classical Hamiltonian system with chaotic behaviour and isolated periodic prime-number-related orbits is likely to exist. The spectrum of the corresponding quantum theory would yield the complex Riemann roots. It is important to note that the roots' GUE statistics necessitate the time-reversal symmetry violation by the Hamiltonian.

In 1999, Connes \cite{Connes1998TraceFI}, Berry and Keating \cite{Berry1999TheRZ}, provided support for Berry's conjecture. They explored a semiclassical model involving a one-dimensional particle with a classical Hamiltonian $H_{\mathrm{BK}} = p q$. This Hamiltonian breaks time-reversal symmetry, $H_{\mathrm{BK}}(p,q)= - H_{\mathrm{BK}}(-p,q)$ and the classical orbits in this model are unbounded hyperbolas in phase space. By imposing boundary conditions, the Hamiltonian $H_{\mathrm{BK}}$ is ``regularized'' to a well-defined Hermitian operator with discrete spectrum. In this setting, the count of  states with positive energy less than $E = \hbar T$ is related to the leading asymptotic contributions to the function $\overline{N(T)}$, measuring the average number of complex zeroes of the Riemann zeta function with positive imaginary part less than $T$. The approaches of Berry and Keating and of Connes differ in the way the classical Hamiltonian is regularized and, consequently, on how the asymptotics of $\overline{N(T)}$ is recovered: respectively as the number of states present in the spectrum \cite{Berry1999TheRZ} and as those missing from the continuum \cite{Connes1998TraceFI}.
...
Various variants of the Berry-Keating Hamiltonian have been introduced and studied \cite{Sierra2011, Sierra_2019}, including one that incorporates broken time-reversal symmetry and closed orbits \cite{Berry2011}. Finally, it is worth mentioning that an intriguing $\cal{PT}$-symmetric quantum Hamiltonian has been shown to be directly related to the Hilbert-Pólya conjecture \cite{Bender_2017}.

While the findings in \cite{Bender_2017} are highly suggestive, we believe that there is still potential for interesting discoveries by following the line of thought presented in the original papers \cite{Connes1998TraceFI, Berry1999TheRZ}.

This work introduces a novel two-parameter deformation of the Berry-Keating model -- the \emph{generalized Born oscillator} -- which incorporates naturally the regularization prescription of Connes. The Hamiltonian of this model is symmetric under time reversal, hence we do not expect to see a naive direct link to prime numbers. Instead, our primary goal is to observe the emergence in the spectrum of the smooth part $\overline{N(T)}$ of the counting function. The generalized Born oscillator is a system with naturally closed trajectories, and its quantisation does not necessitate a regularization prescription. Thus, by using the Weyl's quantisation technique, we are able to extract the number of states $N(E)$ beyond the leading semiclassical approximation to an --in principle-- arbitrary order in $\hbar$. What we will see is that this expansion coincides exactly with that of $\overline{N(T)}$, up to correction terms that vanish as one of the system's parameters is sent to $\infty$.



This work is structured as follows. In \S\ref{sec:RZF} we quickly recall the necessary lore about the Riemann zeta function, the counting function $N(T)$, and the Hilbert-P\'{o}lya conjecture, while \S\ref{sec:BK} will present a short review of the known results on the Berry-Keating theory. The \emph{Born oscillator}, a special case of the generalized Born oscillator, will be introduced in \S\ref{sec:BO} as a dimensional reduction of relativistic  free scalar theory in $1+1$ dimensions, deformed with the $\mathrm{T}\overline{\mathrm{T}}$ operator. In this same section, we will describe the properties of the Born oscillator and study the spectrum arising from its Weyl quantisation, comparing the state-counting function $N(E)$ to the function $\overline{N(T)}$. 
We will see that for large $E = \hbar T$ the latter is indeed reproduced order by order, although it is sided by a number of spurious terms that cannot be eliminated naturally. To solve this problem, in \S\ref{sec:GBO} we introduce an additional parameter $u$ in the system, defining the generalized Born oscillator. Studying its properties and its spectrum, we find that this system constitutes a regularization of Connes' regularization for the Berry-Keating theory, and it is such that in the $u\rightarrow\infty$ limit, the large $E$ behaviours of $N(E)$ and $\overline{N(T)}$ coincide exactly, up to a term linear in $E$. In \S\ref{sec:conclusions} we conclude and give some outlook. We collected some technical details about the Weyl quantisation in Appendix \ref{app:Weyl_quant}, while Appendix \ref{app:iterative} describes the procedure that we employed to determine the spectrum. Appendix \ref{app:Weyl_WKB} contains a comparison of the Weyl quantisation with the more familiar WKB approach. Finally, in Appendix \ref{app:poly} we collect some lengthy expressions pertaining to the $\hbar\rightarrow 0$ expansions of the quantisation conditions for the Born oscillator and for the generalised one.
...
\label{sec:RZF}
%
Let us briefly review some of the properties of the Riemann zeta function  that are relevant for our purposes. More details can be found in any of the many books and reviews on the subject; we suggest the beautiful work \cite{edwards2001riemann}.\\
The Riemann zeta $\zeta(s)$ is a function of the complex variable $s$ that, in the half-plane $\Re s>1$, can be expressed as a convergent sum
%
\beq
...
\eeq
%
or, equivalently, as the Euler product
%
\beq
    \zeta (s)=\prod _{p{\text{ prime}}}{\frac {1}{1-p^{-s}}}\;,\qquad \Re s>1\;.
\label{eq:zR_p}
\eeq
%
This representation constitutes the essence of the relation between the Riemann zeta function and the properties of the prime numbers.

The identity between \eqref{eq:zR} and \eqref{eq:zR_p} was proven by Euler in 1737, more than a hundred years prior to the publication of Riemann's momentous paper \emph{On the Number of Primes Less than a Given Magnitude} (see the Appendix of \cite{edwards2001riemann} for a translation in English).

In this work, Riemann derives an analytic continuation of $\zeta(s)$ in the form of the following integral representation
%
\beq
    \zeta(s) = \frac{\Gamma(1-s)}{2\pi i}\int_{+\infty^+}^{+\infty^-} \frac{dx}{x}\,\frac{(-x)^s}{e^x - 1}\;,
\label{eq:zR_i}
\eeq
%
where the limits of integrations are meant to indicate a path in the complex $x$ plane that issues from $+\infty$ slightly above the real axis, moves towards $0$, circle it in a counterclockwise fashion and returns to $+\infty$, this time slightly below the real axis. Thanks to this representation, he furthermore proved the validity of the reflection formula
%
\beq
...
\eeq
%
This functional equation allows the extraction of information on the position of some zeroes of $\zeta(s)$. In particular, owing to the presence of the sine function, there exist a zero at each negative even integer $s_k = -2k$, $k\in \mathbb Z$. These are called \emph{trivial zeroes}. For $\Re s>1$, the zeta function has no zero (the zero of the sine balances the pole of the $\Gamma$ function), while at $s=1$ there is a simple pole.

The only zeroes of $\zeta(s)$ for $\Re s<0$ and $\Re s>1$ are the trivial ones $s_k$. This leaves open the possibility of \emph{non-trivial zeroes} existing inside the so-called \emph{critical strip} $0 < \Re s < 1$. The functional relation \eqref{eq:zR_refl} yields no information about them, apart from the fact that they need to be symmetrically placed about the real axis and the \emph{critical line} $\Re s = 1/2$. The determination of the position of these zeroes and of their distribution constitutes one of the most important open problems in mathematics. In particular the Riemann hypothesis, claiming that\\[-.55cm]
\begin{center}
    {\bf the only non-trivial zeroes of $\displaystyle\zeta(s)$ are located on the critical line $\displaystyle\Re s = \frac{1}{2}$} ,
\end{center}~\\[-.5cm]
is both a part of Hilbert's eight problem and one of Clay Mathematics Institute's Millennium Prize Problems.
%
\subsection{Number of zeros}
In order to gain a better understanding of the distribution of zeroes inside the critical strip, we employ the argument principle. Let $\gamma$ be a closed path (counterclockwise oriented) and $f(z)$ a meromorphic function with $Z$ zeros and $P$ poles inside $\gamma$.\footnote{Counted with their multiplicity and order.} Then
%
\beq
    Z-P=\frac{1}{2\pi i}\oint_\gamma dz \, \frac{f'(z)}{f(z)} \,.
\eeq
%
In our case we are interested in the critical strip $0 \le \Re s \le 1$, so we take the path $\gamma$ to be the boundary of the following region
\beq
    \mathcal{S}_{\epsilon,T} = \left\lbrace s \in \mathbb C\;\Big\vert\; -\e \le \Re s \le 1+\e \;,\quad 0 \le \Im s \le T \right\rbrace\;.
\eeq
%
The Riemann zeta has no poles inside the critical strip therefore
%
\beq
N(T)=\frac{1}{2\pi i}\oint_\gamma ds \, \frac{\zeta'(s)}{\zeta(s)} \,.
\eeq
%
Thanks to the functional equation \eqref{eq:zR_refl},  $N(T)$ can be manipulated into  the following form
%
\beq
...
\eeq
%
can be interpreted as the average number of zeroes. A comparison of the graphs of these functions is plotted in \ref{fig:zeta}. The remaining term $N_{\mathrm{fl}}(T) = \pi^{-1}\Im \log \zeta(1/2+i T)$ corresponds then to the fluctuations of $N(T)$ around its mean value $\overline{N(T)}$. Using the well known asymptotic expansion for the $\Gamma$ function, from \eqref{eq:sieg} we readily obtain the large $T$ behaviour of the average
%
\beq
    \begin{split}
...
\eeq
%
This expansion is one of the central expressions for this work. As we will see in later sections, the number of states of certain quantum-mechanical systems exhibit the same behaviour \eqref{eq:sieg_asy}. In particular, in \S\ref{sec:GBO} we present a system that reproduces \eqref{eq:sieg_asy} \emph{exactly}.

%
\subsection{Hilbert-P\'{o}lya conjecture}
%
The idea that the imaginary part of the Riemann zeta function non-trivial zeroes could correspond to the energy eigenvalues of a quantum-mechanical system dates back to the beginning of the 20\textsuperscript{th} century, when G. P\'{o}lya was asked by L. Landau about the physical reasons for which the Riemann hypothesis should hold \cite{Polya_Odlyzko}. His answer to the question was that, if there existed a connection between the non-trivial zeroes and a physical system, then the Riemann hypothesis would correspond to the statement that all the eigenvalues of the physical problem are real. In other words,
%
\beq
    \zeta\left(\frac{1}{2} + i T_n\right) = 0 \; \longleftrightarrow \; T_n \textrm{ are the eigenvalues of a self-adjoint operator}\;.
\label{eq:HP_conjecture}
\eeq
%
This statement came to be known as the \emph{Hilbert-P\'{o}lya conjecture}\footnote{The name of D. Hilbert came to be associated with this conjecture due to his claim, during a seminar in the early 1900s, that a proof of the Riemann Hypothesis would follow from Fredholm's work on integral equations with a symmetric kernel. See the comment of A. Weyl (included in his collected works \cite{Weil_collected_II}) to his paper on the explicit formulae of number theory \cite{weil1952formules}.} and ties together the Riemann hypothesis with the theory of quantum mechanics (which, in general, deals with the spectrum of self-adjoint operators). One of the most promising candidates for the ``Hilbert-P\'{o}lya'' operator was proposed by M. Berry and J. Keating right at the end of the last century \cite{Berry1999TheRZ}. In the following section, we will give a brief overview of their proposal, mainly following \cite{Berry1999TheRZ, Sierra_2019}.
%
...
% Figure environment removed
%
In \cite{Berry1999TheRZ}, M. Berry and J. Keating conjectured that a self-adjoint operator satisfying Hilbert-P\'{o}lya conjecture \eqref{eq:HP_conjecture} can be obtained by appropriately quantising the strikingly simple classical Hamiltonian
%
\beq
...
\eeq
%
hereafter referred to as the \emph{Berry-Keating Hamiltonian}.
This function generates a flow on the $2$-dimensional phase space $(p,q)$ whose classical trajectories are branches of hyperbolas (see fig. \ref{fig:pq})
%
\beq
...
\eeq
%
Since the trajectories are open, the associated quantum mechanical system will have a continuum spectrum. This of course is problematic if one's goal is to identify the spectrum with the distribution of the Riemann zeta non-trivial zeroes. In order to overcome this problem, we can proceed in two complementary ways. The first, proposed by Berry and Keating \cite{Berry1999TheRZ}, is to impose the following constraints
%
\beq
...
\eeq
%
Here too, the trajectories become bounded (see fig. \ref{fig:pq_lambda}) and the number of states $N_{\mathrm{C}}$ in semi-classical quantisation reads
%
\beq
...
The derivation of the spectra $N_{\mathrm{BK}}$ and $N_{\mathrm{C}}$ are quite simple and far from being formal. However, as remarked by Berry and Keating \cite{8e50c801912c4798a2696cbe087fdcf6}, the fact that they reproduce the leading asymptotic of $\overline{N(T)}$ is unlikely to be a coincidence. They also stress the need to replace the semi-classical regularization \eqref{eq:BK_constraints} -- or \eqref{eq:C_constraints} for that matter -- with some procedure that generates a discrete spectrum in a natural way. The main result of this work is to introduce such a procedure, in the form of a family of deformations of the Berry-Keating Hamiltonian \eqref{eq:BK_hamiltonian}. As we will see, the parameters controlling these deformations serve as natural regulators of the trajectories \eqref{eq:BK_trajectories} and the quantum spectrum, obtained with the Weyl quantisation of the classical Hamiltonian, is such that the asymptotic expansion \eqref{eq:sieg_asy} of $\overline{N(T)}$ is \underline{exactly reproduced} in a limiting regime.
%
%
...
\eeq
%
which obeys the following limiting behaviours
%
\beq
...
\eeq
%
We see that the  $\mathrm{T} \overline{\mathrm{T}}$ deformation \eqref{eq:TTbar_deformation} determines a one-parameter family of models that interpolates -- at least at the classical level -- from a theory of a free massless scalar to one with vanishing Lagrangian. The latter is suggestively similar to the Berry-Keating theory \eqref{eq:BK_hamiltonian}, albeit in $1+1$ dimensions instead of $1+0$.
...
% Figure environment removed
%
Another important fact that we notice is that the hyperbolic classical trajectories \eqref{eq:BK_trajectories} of the Berry-Keating theory are also approached as the energy grows, in accordance with the limiting behaviours
%
\beq
    \begin{split}
...
\eeq
%
Finally, from both the expression \eqref{eq:BO_Hamiltonian} and the figures \ref{fig:Born} and \ref{fig:Born_vs_BK} it is clear that our systems possesses a $D_4$ dihedral symmetry. Thus, we can consider $H_{\mathrm{BO}}$ as a ``$D_4$ regularisation'' of the Berry-Keating model.
...
\eeq
%
where $q_t = \sqrt{(\lambda^2 E^2-1)/\lambda}$ is the positive turning point of the trajectory at energy $E$ and $\mathsf{K}(x)$, $\mathsf{E}(x)$ are the Legendre complete elliptic integrals (see \S 19 in \cite{NIST:DLMF}). For large values of $E$ we find the behaviour
%
\beq
...
While the expansion \eqref{eq:BO_semiclass_sieg} is an encouraging signal that the Born oscillator might indeed provide the sought after P\'{o}lya operator, regularizing the trajectories of the Berry-Keating system naturally, we need to devise a way to check that the sub-leading terms in the semi-classical expansion of the number of states $N_{\mathrm{BO}}$ agree with the expansion of the Riemann-Siegel $\theta$ function \eqref{eq:sieg_asy}. This task requires us to properly quantise the Hamiltonian $H_{\mathrm{BO}}$. The canonical procedure of promoting $(p,q)$ to operators, say in position space representation $(\hat{p},\hat{q}) = (-i\hbar\frac{d}{dq},q)$, is severely complicated by the fact that $H_{\mathrm{BO}}$ is non-polynomial in $p$ and $q$. One might think that expanding the square roots simplifies the problem, however the resulting stationary Schr\"{o}dinger equation is a differential eigenvalue problem of infinite order, whose study is, to put it mildly, unwieldy. For this reason, we are going to employ a different approach, called Weyl quantisation, to the evaluation of the number of states of the Born oscillator. This procedure is detailed in Appendix \ref{app:Weyl_quant}.



\subsection{Quantisation of the Born Oscillator}
\label{subsec:QuantisationBO}
Using the Weyl quantisation procedure to quantise the Born oscillator Hamiltonian \eqref{eq:BO_Hamiltonian} brings us to the following expansion (see Appendix \ref{app:iterative} for more details)
%
\beq
...
We are interested in the behaviour of \eqref{eq:quant_cond_for_BO} for large $E$, equivalently large $\tqt$. For the elliptic integrals, we have
%
\beq
    \begin{split}
...
\eeq
%
from which we can estimate the asymptotic behaviour
%
\beq
    \Sigma_m(E) \underset{E\rightarrow\infty}{\sim} c\,E^{1-2m}\;.
\eeq
%
Having explicit expressions for $m<6$, we can determine the large $E$ behaviour of \eqref{eq:quant_cond_for_BO} up to order $\mathcal{O}\left(E^{-11}\right)$. Using the identifications \eqref{eq:parameter_identifications_1}, this takes the following form
%
\beq
    \begin{split}
...
\eeq
%
So we see that, while the Weyl quantisation of the Born oscillator \eqref{eq:BO_Hamiltonian} does indeed seem to produce correctly all the terms in the asymptotic expansion of the mean number of zeroes \eqref{eq:average_numb_zeroes}, it also generates several spurious terms. In principle, it is possible to eliminate at least part of them by performing a redefinition of $T$. For example, setting $T = T + \sum_{l=1}^{\infty} \alpha_{2l-1} T^{-2l+1}$ and carefully choosing the coefficients $\alpha_{2l-1}$ it is possible to eliminate all the terms $T^{-2l+1}\log T/(2\pi)$ in \eqref{eq:BO_spurious_terms}. However, this redefinition looks very unnatural and, what's more, fails to eliminate completely $\mathrm{U.T.}$ In order to obtain a spectrum whose large energy expansion exactly reproduces the behaviour of the Riemann-Siegel $\theta$ function, we need to do something more radical: deform the Born oscillator by introducing an additional parameter $u$ that we can then tune appropriately to eliminate the spurious terms. Before doing so, we wish to close this section by mentioning the fact that the results obtained here with the iterative procedure detailed in Appendix \ref{app:iterative} can actually be reproduced by applying the more familiar WKB quantisation. This has to be applied to an operator $\hat{H}_{\mathrm{BO}}$ obtained from the Hamiltonian $H_{\mathrm{BO}}$ by imposing a very specific ordering that follows from the Weyl quantisation prescription. See Appendix \ref{app:Weyl_WKB} for more details on this point.


\section{The Generalized Born Oscillator}
\label{sec:GBO}
%
In this section we are going to propose a $1$-parameter generalization of the Born oscillator. This modified theory, named \emph{generalized Born oscillator}, can be interpreted as a natural implementation of the Connes cutoff \eqref{eq:C_constraints} on the Berry-Keating Hamiltonian, as it will be clear momentarily. Just as for the Born oscillator, the Weyl quantisation of this modified theory produces a spectrum that reproduces the asymptotic behaviour of the mean number of Riemann zeroes $\overline{N(T)}$, together with a number of spurious terms. The difference is that, in the present case, we have an additional parameter $u$. As it turns out, sending $u\rightarrow\infty$ completely cancels out the spurious term, leaving us purely with the terms reproducing the asymptotic behaviour of $\overline{N(T)}$.
...
% Figure environment removed
%
The trajectories of this system, depicted in Fig. \ref{fig:GBO_trajectories_energy}, approach the ones of the Berry-Keating Hamiltonian as the energy grows
%
\beq
...
\eeq
%
Another interesting fact that we can note about the generalized Born oscillator is that, as $u$ grows towards $\infty$, its trajectories resemble more and more the one obtained with a Connes cutoff from the Berry-Keating ones (see Fig. \ref{fig:pq_lambda}). Being more precise, we can say that in the limit $u\rightarrow\infty$, the trajectories become the following
%
\beq
...
\eeq
%
The identifications $N_{\mathrm{GBO}} = 4N_{\mathrm{C}}$ and $\lambda = 1/(2\pi\hbar)$ yield precisely \eqref{eq:C_spectrum_no_lambda} (with the addition of the Maslov index $-1/8$), as expected from the limiting behaviour of the trajectories \eqref{eq:GBO_trajectory_limit}. Hence, the generalized Born oscillator reproduces the first order in the semi-classical expansion of the Berry Keating spectrum, with the Connes regularization prescription implemented. The difference is that the Hamiltonian \eqref{eq:GBO_Hamiltonian}, having naturally bounded trajectories, allows pushing the semi-classical expansion of the spectrum beyond first approximation to an in principle, arbitrary order in $\hbar$.
%
\subsection{Quantisation of the generalised Born oscillator}
\label{subsec:QuantisationGBO}
We are going to perform the same analysis we carried on in \S \ref{subsec:QuantisationBO} for the Born oscillator. The quantisation condition is expanded in $\hbar$ as in \eqref{eq:quant_cond_for_BO}, where now the coefficients $\Sigma_0(E)$ and $\Sigma_m(E)$ take the following form
%
\beq
...
Fortunately, for the purposes of this work the explicit expression of the polynomials $P_m$ and $Q_m$ is largely irrelevant. In fact, we are interested in the large $E$ -- equivalently, large $\tqt$ -- behaviour of $\Sigma_m$. We see that
%
\beq
...
\eeq
%
The correctness of this asymptotic behaviour can be verified for larger values of $m$ by fixing $u$ to be a large integer and investigating the leading $E\rightarrow\infty$ behaviour of $\Sigma_m$. This additionally allows determining the rational numbers $\mathfrak{q}_m$:
%
\beq
	\begin{split}
...
\eeq
%
The final result is that, in the $u\rightarrow\infty$ limit, the large $E$ expansion of quantisation condition \eqref{eq:quant_cond_for_BO} for the generalized Born oscillator is
%
\beq
    \begin{split}
...
%
The main result of this article is the introduction of a specific regularization scheme for the Berry-Keating Hamiltonian and the investigation of its quantum correction at very high perturbative orders using Weyl quantisation. This allowed us to observe the emergence of contributions from the smooth part of the counting function for the Riemann zeros. This finding represents a crucial conceptual advancement in the original Berry-Keating program, which aims to establish a connection between this simple Hamiltonian and the Riemann-Pólya conjecture.

Although this represents a significant conceptual leap forward, unfortunately, as with previous attempts, there is no direct evidence of a straightforward relationship with prime numbers. This point is noteworthy and warrants some speculation. In the spectral theory of Sturm-Liouville-type problems, we typically encounter two classes of spectral problems. On the one hand, we have the \emph{lateral problems}, which involve imposing boundary conditions in two different sectors of the complex-$x$ plane (Stokes sectors) as $|x|$ approaches infinity. Alternatively, one can impose boundary conditions at the origin and infinity (in some specific sector), obtaining the so-called \emph{central problem}. The analysis conducted in this work is akin to a lateral problem. On the other hand in \cite{Berry1999TheRZ} (\S6, page 262, penultimate paragraph), the zeta function explicitly appears as a spectral determinant in what appears to be a central problem (compare e.g. with \cite{ABCD} or the Appendix B.2 of \cite{Dorey:2006an}). Still, as tantalizing as these speculations are, without some additional guiding ideas there is limited progress that can be made in this direction.

From the base of our results, an important avenue to explore is establishing a link with integrable quantum field theories, exact S-matrices, and Bethe Ansatz equations in the spirit of the ODE/IM correspondence. In particular, it would be intriguing to find the integrable system associated with the generalized Born quantum mechanical model, where local conserved charges on the integrable model  side are  related to the high-order WKB-Weyl analysis performed in the current article.

Finally, it is worth mentioning that the paper \cite{LeClair:2023keb} by LeClair and Mussardo, which appeared while we were writing this concluding section, seems to align with our direction. They suggested a scattering model with defects, leading to a Bethe Ansatz-type interpretation for the quantisation condition $N(T) = n$. A similar final objective, i.e.  the search for an ODE/IM -type correspondence,  motivated our investigation. We consider the current paper as the first step in this direction, and the results  of \cite{LeClair:2023keb} seem to  suggest that we are working in a potentially successful framework.
...
%
We would like to express our gratitude to Gianni Coppa for fruitful discussions, during the initial stages of this work and in the period associated with his Master thesis project \cite{CoppaThesis}. Additionally, we are grateful to him for sharing a draft of his article \cite{CoppaArticle}. We also thank Michael Berry and Zoltan Bajnok for  useful discussions.
...
\eeq
%
For $E = E_n$ this turns into a quantisation condition for the energy eigenvalues
%
\beq
...
The main idea underlying the Weyl quantisation is to express the trace on the left-hand side of \eqref{eq:quant_cond} as an integral over the classical phase space of the system. In order to do so, one can follow the procedure used by E. Wigner in \cite{wigner1997quantum} and define the classical function
%
\beq
...
\eeq
%
and, consequently, we can recast the quantisation condition \eqref{eq:quant_cond} as a phase space integral
%
\beq
...
\eeq
%
Let us now rewrite the trace in the rightmost side above by using a version of Baker-Campbell-Hausdorff formula
%
\beq
...
\label{app:iterative}
In order to solve \eqref{eq:Weyl_diff_eq_in_z} and, consequently, explicitly determine the exact quantisation condition \eqref{eq:quant_cond_phase_space} through \eqref{eq:S_integral_rep}, we can expand the function $\mathcal{E}_c$ in powers of $\hbar$. In particular, it is convenient to use the following expansion
%
\beq
...
Collecting everything we have derived, we can write the quantisation condition \eqref{eq:quant_cond_phase_space} as
%
\beq
    n+\frac{1}{2} = \frac{1}{\hbar}\Sigma_0(E) + \sum_{m=1}^\infty \Sigma_m(E) \hbar^{2m-1}\;,
...
\eeq
%
To recapitulate, we can expand the quantisation condition around $\hbar = 0$ and compute the coefficients $\Sigma_m(E)$ using \eqref{eq:Sigma_eq_final}. This requires us to extract the functions $g_{m,\ell}(p,q)$, which are the coefficients of the polynomials $G_m(p,q\vert z)$ \eqref{eq:G_as_poly}. These have to be determined from the recursion equation \eqref{eq:recursion_relation}. All these steps can be automated in a Mathematica\textsuperscript{\textcopyright} script, providing us with an iterative routine to determine the quantisation condition \eqref{eq:quant_cond_phase_space} up to, in principle, any desired order $\mathcal{O}\left(\hbar^k\right)$. In practice, for the Hamiltonians considered in the main text, the computations involved in the routine become excessively taxing for large $k$, so we limited ourselves to $k=11$ for the Born oscillator and $k=5$ for the modified one. For the latter, however, it is not necessary to determine the exact form of $\Sigma_{m}(E)$ to determine the order $\hbar^{2m-1}$ contribution to the quantisation condition in the large $E$ and $u$ limits. In fact, as explained in the main text \eqref{eq:GBO_Sigma_limit_form}, this limit is controlled by a set of rational numbers $\mathfrak{q}_m$, whose value is independent of $u$. For this reason, they can be computed by fixing $u$ to be some natural number, which greatly improves the speed of the procedure presented here and allowed us to obtain the large $E$ and $u$ behaviour of the quantisation condition up to order $\mathcal{O}\left(\hbar^{11}\right)$.

\section{Weyl vs WKB quantisations}
\label{app:Weyl_WKB}
In this Appendix, we are going to show that the Weyl quantisation amounts to the choice of a very specific ordering prescription for the quantum Hamiltonian. As such, we expect the spectrum extracted via the procedure detailed in Appendix \ref{app:iterative} to coincide with the one obtained from the usual WKB expansion. This fact is easily proven true for Hamiltonians of the form $H = \frac{p^2}{2m} + V(q)$, where no ordering issue arises. We are going to provide evidence that the identity of the spectra holds true also for more complicated classical Hamiltonians of product form $H = F(p)G(q)$, by computing the WKB quantisation condition up to order $\mathcal{O}\left(\hbar^5\right)$ for the Born oscillator \eqref{eq:BO_Hamiltonian} and comparing it to the one found with the Weyl quantisation procedure (\ref{eq:app_BO_Sigma} -- \ref{eq:app_BO_poly_last}).

\subsection{Operator ordering from Weyl quantisation}
...
%
    ...
    \eeq
    %
    which naturally agrees with the canonical quantisation prescription.
%
    ...
    \eeq
    %
    which, again, is consistent with the canonical quantisation.
%
    ...
    \eeq
    %
    The Weyl correspondence produces the symmetrized version of the naive quantisation $pq\,\to\,\hat{p}\hat{q}$.
%
    ...
    \eeq
    %
    Again, we see that the Weyl quantisation yields a quantum operator with a very specific ordering of the operators $\hat{p}$ and $\hat{q}$.
\end{enumerate}
...
\eeq
%
According to the analysis above, the corresponding operator obtained via the Weyl quantisation acts of functions of $q$ as
%
\beq
...
\beq
    \begin{split}
        ...
        S_4'(q) &= -\frac{\lambda^{7/2}}{128 (1 + \tqt^2)^3} \frac{1}{(\tqt^2 - \lambda q^2)^{11/2}(1 + \lambda q^2)^{3/2}} \times \\
    ...
    \end{split}
\eeq
The functions $S_{2l-1}'(q)$ are total derivatives, as expected from the form of the quantisation condition \eqref{eq:quant_exp_in_h}
\beq
    \begin{split}
...
\section{Expansion of the quantisation condition}\label{app:poly}
