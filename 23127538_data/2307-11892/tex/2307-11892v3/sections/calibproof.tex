...
\begin{enumerate}
\item Assume that group A has $1-\alpha$ of the probability mass i.e. $P_{(x,y) \sim \calD}[x \in A] \geq 1-\alpha$ and thus $P_{(x,y) \sim \calD}[x \in B] \leq \alpha$.
\item The positive fraction for each group under distribution $\mathcal{D}$ is $P_{(x,y) \sim \DA}[y=1]=P_{(x,y) \sim D_B}[y=1]=\frac{1}{2}$
\item Since $P_{(x,y) \sim \calD}[x \in B] \leq \alpha$, the adversary has sufficient corruption budget such that they can a duplicate copy of each example in B but with the opposite label.   
...
\[  P_{(x,y) \sim \hat{\mathcal{D}}_{B}}[y=1 | h(x)=1 ] = P_{(x,y) \sim   \hat{\mathcal{D}}_{B}}[y=0  | h(x)=0]=\frac{1}{2} \]
...
That is;
For each group $z$ (i.e $z=A$ or $z=B$), for each bin $r \in [R]$ (i.e., ${x: h^{*}(x)=r}$), we create a new bin if there is no bin in $[R]$ with value  $\hat{r} = E_{(x,y) \sim \mathcal{D}_z}[y | h^{*}(x)=r]$.  
