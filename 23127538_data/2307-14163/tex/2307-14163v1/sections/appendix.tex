...
\begin{proof}[Proof of Proposition \ref{proprox}]
	...
	\end{multline}
	which gives the first part of the statement. For the second part, by the second order expansion \eqref{eq:K1K2},
	$$
	...
	\end{align*}
	Finally,  write  \eqref{eq_gamma1} with $\gamma_{\Tt}$ replaced by $\alpha_{\Tt}$, and derive the representation for $\overline H(\Tt)-\underline H (\Tt)$. \end{proof}

\smallskip

\begin{proof}[Proof of Proposition \ref{prop_Lest}]
	Similar to that of Proposition \ref{proprox}, the details are thus omitted. 
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{propCH}]
	For simpler writings,  we write $\varrho(\Delta)$ instead of $\varrho(\Delta,\mathfrak m)$. The proof has  several steps. 
First, using Assumptions (H\ref{ass_H1}), (H\ref{ass_H2}) and (H\ref{ass_H3}), 	combined with Bernstein's inequality, a constant $\mathfrak u >0$ exists such that, for any $i=1,2$ and $\Delta>0$~:
    \begin{equation}\label{eq:assumption-theta-hat_main}
	\max\left\{
	\PP\left(\widehat{\theta_{\Tt}^{(i)}}(\Delta)-\theta_{\Tt}^{(i)}(\Delta)\geq \varepsilon\right),
	\PP\left(\widehat{\theta_{\Tt}^{(i)}}(\Delta)-\theta_{\Tt}^{(i)}(\Delta)\leq -\varepsilon\right)
	\right\}
	\leq \exp\left( -\mathfrak{u}N\varepsilon^2\varrho(\Delta)\right),
\end{equation}
with $\widehat{\theta_{\Tt}^{(i)}}(\Delta)$ defined in \eqref{eq_theta_hat}. 	The proof of \eqref{eq:assumption-theta-hat_main} is provided in the Supplementary Material. 

\noindent
	\textbf{\textit{Step 1 : proof of equation \eqref{eq:conc-Hhat-around-H_main}.}}
	Set $\varepsilon \in (0,1)$ and define
	\begin{equation*}
		A_\varepsilon = \mathbb{P}\left[|\underline{\widehat{H}}(\Tt)-\underline{H}(\Tt)|\geq \varepsilon \right].
	\end{equation*}
	Using the definition of  $\underline{\widehat{H}}(\Tt)$ and $\underline{H}(\Tt)$, we have
	\begin{align*}
		A_\varepsilon
		&=  \mathbb{P}\left[\left|\log\left(\frac{\widehat{\gamma_{\Tt}}(2\Delta)\gamma_{\Tt}(\Delta)}{\gamma_{\Tt}(2\Delta)\widehat{\gamma_{\Tt}}(\Delta)}\right)\right|\geq 2\varepsilon\log2 \right]\\
		&\leq \mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(2\Delta)\gamma_{\Tt}(\Delta)}{\gamma_{\Tt}(2\Delta)\widehat{\gamma_{\Tt}}(\Delta)}\geq 2^{2\varepsilon}\right]+\mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(2\Delta)\gamma_{\Tt}(\Delta)}{\gamma_{\Tt}(2\Delta)\widehat{\gamma_{\Tt}}(\Delta)}\leq 2^{-2\varepsilon}\right]\\
		&\leq  \mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(2\Delta)}{\gamma_{\Tt}(2\Delta)}\geq 2^\varepsilon\right]+\mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(2\Delta)}{\gamma_{\Tt}(2\Delta)}\leq 2^{-\varepsilon}\right] +\mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(\Delta)}{\gamma_{\Tt}(\Delta)}\geq 2^\varepsilon\right]+\mathbb{P}\left[\frac{\widehat{\gamma_{\Tt}}(\Delta)}{{\gamma_{\Tt}}(\Delta)}\leq 2^{-\varepsilon}\right]\\
...
By elementary algebra, combined with the fact that, for small $\Delta$,  we have $\gamma_{\Tt}(\Delta) = K_1(\Tt)\Delta^{2\underline H (\Tt)}\{1+o(1)\}$,  we deduce that   positive constants  $C_1$ and $C_2$  exist such that 
	...
	\begin{multline*}
		...
		+ \PP[\bar A_N].
	\end{multline*}
Here, for a set $A$, $\overline{A}$ denotes its complement. We now remark that
	\begin{multline}
\hspace{-.2cm} \PP\left[\left|\widehat D(\Tt)-D(\Tt)\right|\geq \varepsilon/2\right]
=\PP\left[\left|\log\left(\frac{\widehat{\alpha}_{\Tt}(2\Delta)\alpha_{\Tt}(\Delta)}{\widehat{\alpha}_{\Tt}(\Delta)\alpha_{\Tt}(2\Delta)}\right)\right|\geq \varepsilon\log(2)\right]\\
%&\leq \PP\left[\frac{\widehat{\alpha}_{\Tt}(2\Delta)\alpha_{\Tt}(\Delta)}{\widehat{\alpha}_{\Tt}(\Delta)\alpha_{\Tt}(2\Delta)}\geq 2^{\varepsilon}\right]+\PP\left[\frac{\widehat{\alpha}_{\Tt}(2\Delta)\alpha_{\Tt}(\Delta)}{\widehat{\alpha}_{\Tt}(\Delta)\alpha_{\Tt}(2\Delta)}\leq 2^{-\varepsilon}\right]\\
\leq \PP\!\left[\frac{\widehat{\alpha}_{\Tt}(2\Delta)}{\alpha_{\Tt}(2\Delta)}\geq2^{\varepsilon/2}\right]+\PP\!\left[\frac{\widehat{\alpha}_{\Tt}(\Delta)}{\alpha_{\Tt}(\Delta)}\leq2^{-\varepsilon/2}\right]+\PP\!\left[\frac{\widehat{\alpha}_{\Tt}(2\Delta)}{\alpha_{\Tt}(2\Delta)}\leq2^{-\varepsilon/2}\right]+\PP\!\left[\frac{\widehat{\alpha}_{\Tt}(\Delta)}{\alpha_{\Tt}(\Delta)}\geq 2^{\varepsilon/2}\right].
 ...
 We focus on the first term, the other three can be bounded similarly.  By~\eqref{eq:bound-alpha_main}, for small $\Delta$,
	\begin{align*}
		...
		&=\PP\left[\widehat{\alpha}_{\Tt}(2\Delta)-\alpha_{\Tt}(2\Delta)\geq (2^{\frac{\varepsilon}{2}}-1)\alpha_{\Tt}(2\Delta)\right]\\
		&\leq \tilde L_5
		\exp\left[
		-\tilde L_6N\left((2^{\frac{\varepsilon}{2}}-1)\alpha_{\Tt}(2\Delta)\right)^2\frac{\Delta^{4\overline{H}(\Tt)}\varrho(\Delta)}{\log^2(\Delta)}
		\right].
	\end{align*}
	...
	\begin{equation*}
		...
		\right],
	\end{equation*}
	for some positive constant $\tilde L_7$. The same inequality, with possibly different constants, remains valid for the three other terms. Therefore, a positive constant $\tilde L_8$ exists such that
	...
	\end{equation*}
	Using \eqref{eq:concentration-Difference} with $\varepsilon= 2\tau$ leads to 
...
We now have to bound $\PP[A_N]$, instead of  $\PP[\Bar A_N]$. For this, we can simply write 
	...
	\end{equation*}
	Using \eqref{eq:concentration-Difference} we then obtain 
...
\begin{proof}[Proof of Proposition \ref{conc_Lest}] 
	...
	\textbf{\textit{Proof of  \eqref{eq:conc_L1_main}}.} Here is  the anisotropic case,  and we consider $H_1(\Tt)=\underline H (\Tt)$. Set $\varepsilon \in (0,1)$ and define  
\begin{equation*}
\mathfrak A_{\varepsilon}^{(i)}= \PP\left(\left|\widehat{L_1^{(i)}}(\Tt)-L_1^{(i)}(\Tt)\right|\geq \varepsilon  \right),\qquad i=1,2.
\end{equation*}
...
\end{equation}
To bound these two terms, we first notice that a constant $K$ exists such that, for any $\varepsilon \in (0,1)$,
\begin{equation*}
...
\geq 1+\varepsilon K.
\end{equation*}
Then, by  \eqref{eq:assumption-theta-hat_main}, 
\begin{equation}
    \PP\left(\frac{\widehat{\theta}_{\Tt}^{(i)}(\Delta)}{\theta_{\Tt}^{(i)}(\Delta)}\geq\left(1+ \varepsilon\frac{\Delta^{2\underline{H}(\Tt)}}{\theta_{\Tt}^{(i)}(\Delta)}\right)^{\frac{1}{2}}\right) 
...
\begin{equation}\label{eq:first-term-L}
 ...
 \leq\exp \left(
	...
	\right),
\end{equation}
for some positive constant $\mathcal L$.  For the second term, since $\Delta$ is assumed to be sufficiently small, and $\log(x)\leq x-1$, $x>0$,  we have 
\begin{equation*}
...
\end{equation*}
Using \eqref{eq:concentration-Hhat-around-H}, we now obtain
...
\leq   C_1\exp\left(
	...
	\right). 
\end{equation}
Finally, combining \eqref{eq:decom-holder-const}, \eqref{eq:first-term-L} and \eqref{eq:second-term-L}, and considering without loss of generality that $\Delta_0\leq e^{-1}$ in Definition \ref{def}, positive constants $ \mathfrak C _1$ and  $ \mathfrak C_2$ exist such that for any $\varepsilon\in(0,1) $, we have~:
...
%%%%%%%%%%%%%%%
%\medskip
\noindent \textbf{\textit{Proof of \eqref{eq:conc_L2_main}:}} Recall that 
\begin{equation*}
...
%
%
%For any $\varepsilon\in (0,1)$ and $i=1,2$, 
%we have to bound
%\begin{equation*}
%\mathfrak G _{\varepsilon}^{(i)}=\PP\left(\left|\widehat{L_2^{(i)}}(\Tt)-L_2^{(i)}(\Tt)\right|\geq \varepsilon\right).
%\end{equation*}
%
%
%
...
\end{equation*}
For any $\varepsilon\in (0,1)$ and $i=1,2$, we decompose as follows~:
\begin{equation}\label{eq:decompose-L2}
 \PP\left(\widehat{L_2^{(i)}}(\Tt)-L_2^{(i)}(\Tt)\geq \varepsilon\right)\leq B^{(i)}_1+B^{(i)}_2+B^{(i)}_3+B^{(i)}_4,
\end{equation}
 where 
 \begin{equation}
 B^{(i)}_1=\PP\!\left(\widehat{\alpha}_{\Tt}^{(i)}(\Delta)-\alpha_{\Tt}^{(i)}(\Delta)\geq \sqrt{\varepsilon/3}\right),\qquad 
 ...
 B^{(i)}_3&=\PP\left(\alpha_{\Tt}^{(i)}(\Delta)\left(\frac{1}{\{4^{\widehat{D}(\Tt)}-1\}\Delta^{2\widehat{D}(\Tt)}} -\frac{1}{\{4^{D(\Tt)}-1\}\Delta^{2D(\Tt)}}\right)\geq \varepsilon/3\right),\\
 B^{(i)}_4&=\PP\left(\frac{1}{\{4^{\widehat{D}(\Tt)}-1\}\Delta^{2\widehat{D}(\Tt)}} -\frac{1}{\{4^{D(\Tt)}-1\}\Delta^{2D(\Tt)}}\geq \sqrt{ \varepsilon/3}\right).
 \end{align}
The quantities   $B^{(i)}_1$ and $B^{(i)}_2$ can be bounded by the arguments used for \eqref{eq:bound-alpha_main}. More precisely, two positive constants $\tilde C_1$ and $\tilde C_2$ such that, for any $\varepsilon \in (0,1)$, we have 
...
B_2^{(i)}\! \leq \! \tilde C_1\!\exp\!\left[\!
	...
	\! \right]\!.
\end{equation}
To bound $B^{(i)}_3$ and $B^{(i)}_4$ in \eqref{B3B4_main}, we  use the fact that $\alpha_{\Tt}^{(i)}(\Delta)\sim L_2^{(i)}(\Tt)(4^{D(\Tt)}-1)\Delta^{2D(\Tt)}$,  Lemma SM.\ref{lemma_L2_use} in the Supplementary Material, and the fact that  $L_2^{(1)}$, $L_2^{(2)}$ are bounded functions.
%
%
%
%and obtain:
...
%	\right).   
%\end{equation}
%Since $L_2^{(i)}$ is a bounded function, and combining \eqref{eq:bound-B1}, %\eqref{eq:bound-B2}, 
%\eqref{eq:bound-B3} and \eqref{eq:bound-B4}, 
%
%
%We deduce that positive constants $C_3$ and $C_4$ exist such that for any $\varepsilon\in (0,1)$ we have 
%\begin{equation}
%\mathfrak G^{(i)}_\varepsilon \leq C_3\exp\left(
%	-C_4N\varepsilon\min\{\varepsilon,\Delta^{4D(\Tt)}\}(4^{D(\Tt)}-1)^2\frac{\Delta^{4\overline H (\Tt)}\varrho(\Delta)}{\log^4(\Delta)}\Delta^{4D(\Tt)}
%	\right),\qquad i=1,2.
%\end{equation} 
The details are given in the Supplementary Material. From that and  \eqref{eq:bound-B1}, the proof follows. \end{proof}

%%%%%%%%%%%%%
\smallskip

\begin{proof}[Proof of Proposition \ref{prop5_simple}]
	The statement is a direct consequence of \eqref{A_N_aniso}  and \eqref{A_N_iso}.
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{mprop}]
	First, let us denote  $B(\Tt,\Ss) =2D(H_1(\Tt),H_1(\Ss))D(H_2(\Tt),H_2(\Ss))$, for any $\Tt,\Ss\in \cT$, with $D(x,y)$ defined in \eqref{def_D_func}. By construction the function $B(\cdot,\cdot)$ is symmetric. Moreover, we show in the Supplementary Material that 
\begin{equation}\label{eq:B-diag}
	B(\Tt,\Ss)= \frac{1}{2}+O(\|\Tt-\Ss\|^2).
\end{equation}
for $\Tt$ and $\Ss$ sufficiently close.		
...
\begin{align*}
	\EE[(X(\Tt)-X(\Ss))^2]=& \frac{1}{2}\left(|A_1(\Tt)|^{H_1(\Tt)+H_1(\Ss)}+|A_1(\Ss)|^{H_1(\Tt)+H_1(\Ss)}\right)|A_2(\Tt)-A_2(\Ss)|^{H_2(\Tt)+H_2(\Ss)}\\
	+&\frac{1}{2}\left(|A_2(\Tt)|^{H_1(\Tt)+H_1(\Ss)}+|A_2(\Ss)|^{H_1(\Tt)+H_1(\Ss)}\right)|A_1(\Tt)-A_1(\Ss)|^{H_1(\Tt)+H_1(\Ss)}\\
	+&\mathfrak B_4(\Tt,\Ss)+O(\|\Tt-\Ss\|^2).
\end{align*}
The last expression and the Taylor expansion imply
\begin{align*}
...
By \eqref{simpl_bet},
\begin{equation}
f_1^2(\Tt)= \left(\frac{L_1^{(1)}(\Tt)}{v(\Tt)}\right)^{\frac{1}{H_1(\Tt)}}\leq \left(\frac{\overline \beta}{\underline v}\right)^{\frac{1}{\underline \beta}}.
\end{equation}
Moreover,
\begin{equation}
\widehat f_1^2(\Tt)=\left(\frac{\widehat L_1^{(1)}(\Tt)}{\widehat v(\Tt)} \right) ^{\frac{1}{\widehat H_1(\Tt)}} \leq \left(\frac{\overline \beta}{\underline v}\right)^{\frac{1}{\underline \beta}}\times \left(\frac{v(\Tt)}{\widehat v(\Tt)}\right)^{\frac{1}{\underline \beta}}.
\end{equation}
Therefore, by  \eqref{simpl_v} we obtain 
\begin{equation}
\EE |\widehat f_1(\Tt)|^2 \leq  \left(\frac{\overline \beta}{\underline v}\right)^{\frac{1}{\underline \beta}}\times\EE\left[ \left(\frac{v(\Tt)}{\widehat v(\Tt)}\right)^{\frac{1}{\underline \beta}}\right].
\end{equation}
and the last expectation is finite. The second part of \eqref{eq_f1_g1} can be obtained with similar arguments. 

Next,  since  $|e^a-1|\leq |a-1| $,  we  have 
	\begin{multline*}
		\left|A_1(\Tt)-\widehat A_1(\Tt)\right|
		\leq A_1(\Tt)\left|1- \exp\left(\int_{t_0}^{t_1} \widehat f_1(s,t_2)-f_1(s,t_2){\D} s+\int_{s_0}^{t_2}\widehat g_1(t_0,s)-g_1(t_0,s)\D s\right)\right|\\
		\leq A_1(\Tt)\left|\int_{t_0}^{t_1} \widehat f_1(s,t_2)-f_1(s,t_2){\D} s+\int_{s_0}^{t_2}\widehat g_1(t_0,s)-g_1(t_0,s)\D s\right|\\
		\leq A_1(\Tt)\int_{t_0}^{t_1} |\widehat f_1(s,t_2)-f_1(s,t_2)|{\D} s+\int_{s_0}^{t_2}|\widehat g_1(t_0,s)-g_1(t_0,s)|\D s.
	\end{multline*}
	Taking  expectation and using Fubini's Theorem we obtain 
	\begin{equation}\label{deux_int}
		\EE\left[|A_1(\Tt)-\widehat A_1(\Tt)|\right]\leq A_1(\Tt)\left[  \int_{t_0}^{t_1} \EE|\widehat f_1(s,t_2)-f_1(s,t_2)|{\D} s  + \int_{s_0}^{t_2}\EE|\widehat g_1(t_0,s)-g_1(t_0,s)|\D s \right].
	\end{equation}
We next bound the two integrals in \eqref{deux_int}. For $\lambda \in (0,1)$, we define the set 
	$$
...
$$
and $\mathfrak C= \overline \beta/\underline v$.  For this, let us first note that, by Proposition \ref{conc_Lest} with $i=1$, for any $\varepsilon \in (0,1)$, we have 
$$
...
$$
Moreover, by Lemma SM.\ref{concentration:variance}, there exists a constant $\mathfrak e$ such that 
$$
\PP(|\widehat v(\Tt)-v(\Tt)|\geq \varepsilon)\leq 2\exp(- \mathfrak  e N\varepsilon ^2). 
$$
...
Finally, by Lemma SM.\ref{lem} we obtain 
	...
	$$
	for some positive constants $q_1$ and $q_2$. We now use \eqref{exp} and write 
	\begin{multline}
...
%		\\		\leq  \int_{t_0}^{t_1}\left(\lambda+\EE\left[ |\hat f_1(s,t_2)-f_1(s,t_2)|^2\right]^\frac{1}{2}q_1\exp\left\{ -q_2N\lambda^2\frac{\Delta^{2H_1(s,t_2)}\varrho(\Delta,\mathfrak m)}{\log^2(\Delta)} \right\}\right){\D} s
	...
	\end{multline}
A simple choice of $\lambda$ can be defined as follows~: for some arbitrarily fixed   $\ell\in (0,1/2)$, let
	$$
	...
	$$
	for some constant $C$. Up to a change of the constants $C$, $q_1$, $q_2$, a similar bound holds true for the second integral on the RHS on \eqref{deux_int}. The proof is now complete. 
\end{proof}
...
\begin{proof}[Proof of Proposition \ref{prop_risk1}.]
 ...
 $$ 
 \EE\!\left[\!\left(\sum_{m=1}^{M_0}\{X^{new}(\Tt^{new}_m;\!\boldsymbol B)\!-X^{new}(\Tt)\} W_m(\Tt)\right)^2\! \Big{|}M_0\! \right] \!\leq 2\left\{\!L_1(\Tt) h_1^{2H_1(\Tt)}\!+L_2(\Tt) h_2^{2H_2(\Tt)}\!\right\}\!\{1+o(1)\},
 $$
and, for some $a_1>1$,
 $$
 \EE\left[\left(\sum_{m=1}^{M_0}\varepsilon^{new}_m W_m(\Tt)\right)^2 \Big{|}M_0\right]\leq \frac{\kappa^2\sigma^2}{c\pi}\frac{1}{M_0 h_1h_2}\left\{1+a_1 M_0^{-1/4}\right\}.
 $$
%The following decomposition then allows to complete the proof of Proposition \ref{prop_risk1}~: 
%\begin{multline}
%\mathcal R(\Tt; \boldsymbol B ,M_0)= \EE\left[\left(\sum_{m=1}^{M_0}\left(X^{new}(\Tt^{new}_m)-X^{new}(\Tt)\right) W_m(\Tt)\right)^2 \Big{|}M_0 \right]\\ + \EE\left[\left(\sum_{m=1}^{M_0}\varepsilon^{new}_mW_m(\Tt)\right)^2\Big{|}M_0\right].
%\end{multline}
\end{proof}
...
\widehat {\Upsilon}(\Tt)=
	M_0^{ \widehat \alpha_1(\Tt)+\widehat\alpha_2(\Tt)-\alpha_1(\Tt)-\alpha_2(\Tt)}\frac{\widehat \Lambda_1^{-\widehat\alpha_1(\Tt)}(\Tt)\widehat \Lambda_2^{-\widehat\alpha_2(\Tt)}(\Tt)}{ \Lambda_1^{-\alpha_1(\Tt)}(\Tt)\Lambda_2^{-\alpha_2(\Tt)}(\Tt)}.
		\end{equation}
		Then, on the event $\mathcal F\cap \mathcal E$ and using Cauchy-Schwarz inequality, we obtain
\begin{multline*}
\EE\left[ \frac{\kappa^2}{c\pi} \frac{\sigma^2}{M_0 \hat h_1 \hat h_2}  \mathbf{1}_{\mathcal{F}\cap\mathcal{E} } \big{|} M_0 \right]\leq \frac{\kappa^2\sigma^2}{c\pi} M_0^{ \alpha_1(\Tt)+\alpha_2(\Tt)-1}\Lambda_1^{-\alpha_1(\Tt)}(\Tt)\Lambda_2^{-\alpha_2(\Tt)}(\Tt) \EE\left[  \widehat {\Upsilon}(\Tt)\mathbf{1}_{\mathcal{F}\cap\mathcal{E} } \big{|}M_0\right]
...
\\ \leq\frac{\kappa^2\sigma^2}{c\pi} M_0^{ \alpha_1(\Tt)+\alpha_2(\Tt)-1+2\log^{-a}(\mathfrak m)}\Lambda_1^{-\alpha_1(\Tt)}(\Tt)\Lambda_2^{-\alpha_2(\Tt)}(\Tt)\EE\left[\frac{\hat \Lambda_1^{-2\hat\alpha_1(\Tt)}(\Tt)\hat \Lambda_2^{-2\hat\alpha_2(\Tt)}(\Tt)}{ \Lambda_1^{-2\alpha_1(\Tt)}(\Tt)\Lambda_2^{-2\alpha_2(\Tt)}(\Tt)}\mathbf{1}_{\mathcal{F}\cap\mathcal{E} } \Big{|}M_0\right].
\end{multline*}
	...
	Note that, by definition, $2\alpha_i(\Tt)<1$, $i=1,2$. 	It follows that on the event $\mathcal F \cap \mathcal E$ we have  
		...
		$$
		\EE\left[ \frac{\kappa^2}{c\pi} \frac{\sigma^2}{M_0 \widehat h_1 \widehat h_2} \mathbf{1}_{\mathcal{F}\cap\mathcal{E} }\big{|} M_0 \right]\leq \frac{\kappa^2 \sigma^2}{c\pi\Lambda_1^{\alpha_1(\Tt)}(\Tt)\Lambda_2^{\alpha_2(\Tt)}(\Tt)} M_0^{ -\frac{\omega(\Tt)}{2\omega(\Tt)+1}+2\log^{-a}(\mathfrak m)}  \{1+O(\log^{-a}(\mathfrak m))\}.
		$$
		By similar argument, we can also show that on the event $\mathcal F \cap \mathcal E$ we have 
		$$
		\EE\!\left[ L_1(\Tt)\widehat h_1^{2H_1(\Tt)}\mathbf{1}_{\mathcal{F}\cap\mathcal{E} }\big{|}M_0\right]\!\leq L_1(\Tt)\!\left[\frac{\Lambda_1(\Tt)^{2H_1(\Tt)+1}}{\Lambda_2(\Tt)}\right]^{\!\alpha_1(\Tt)}\!\!M_0^{-\frac{\omega(\Tt)}{2\omega(\Tt)+1}+2\log^{-a}(\mathfrak m)}\!\times \{1+O(\log^{-a}(\mathfrak m))\},
		$$
		and 
		$$
		\EE\!\left[ L_2(\Tt)\widehat h_2^{2H_2(\Tt)}\mathbf{1}_{\mathcal{F}\cap\mathcal{E} }\big{|}M_0\right]\leq L_2(\Tt)\!\left[\frac{\Lambda_2(\Tt)^{2H_2(\Tt)+1}}{\Lambda_1(\Tt)}\right]^{\!\alpha_2(\Tt)}\!M_0^{-\frac{\omega(\Tt)}{2\omega(\Tt)+1}+2\log^{-a}(\mathfrak m)}\times \{1+O(\log^{-a}(\mathfrak m))\}.
		$$
	Since
		\begin{multline*}
