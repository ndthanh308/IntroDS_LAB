...
We now have to bound $\PP[A_N]$, instead of  $\PP[\Bar A_N]$. For this, we can simply write 
	...
	\begin{multline}\label{eq:isotropic}
		...
		\right] .
	\end{multline}
	Combining  \eqref{eq:anisotropic} and \eqref{eq:isotropic}, three positive constants $L_3,L_4$ and $L_5$ exist such that  
...
Next, by the condition \eqref{simpl_v} and Fubini's Theorem, a constant $\mathfrak C_v$ exists such that 
...
\end{multline*}
	...
	Since
		...
		\end{multline*}
	and given the facts above, it remains to investigate the last two expectations in the last display. 
