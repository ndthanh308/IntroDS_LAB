...
For instance, Nvidia's DGX H100 server~\cite{dgxh100} consists of eight H100 GPUs interconnected with NVSwitches, providing 7.2~Tbps of non-blocking bandwidth internally. The GB200 NVL72 computer announced recently connects 36 GB200 Superchips with fifth-generation NVLink within a rack at 14.4~Tbps per GPU~\cite{gb200nvl72}. The AMD MI300X platform, on the other hand, employs AMD's Infinity Fabric to connect eight MI300X accelerators in a full-mesh topology with 7.2~Tbps of bandwidth per GPU~\cite{mi300x}. Similar platforms such as Nvidia's DGX GH200 Super Computer have utilized multi-tiered NVSwich topologies to scale the platform's size up to 256 GPUs while maintaining 7.2~Tbps full-bisection intra-GPU bandwidth~\cite{gh200}.
