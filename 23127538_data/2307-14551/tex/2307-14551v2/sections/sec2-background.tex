...
Finally, the \emph{watch history page}\footnote{\url{https://www.youtube.com/feed/history}} (or \emph{watch history} for short) is the page that displays a log of the user's previously-watched videos. This page is only available to users who are logged in. What relevant to our study is the option for users to delete specific videos from their watch history. Specifically, the ``Delete'' button is ``X'' located on the upper-right hand corner of each video in the log. 
