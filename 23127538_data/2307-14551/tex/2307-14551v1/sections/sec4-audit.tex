...
\begin{itemize}
    ...
    \item \emph{Political Left}: Collected by \citet{wu_cross-partisan_2021}, they include local news, talk shows, and magazines. We use the US political left, which takes similar views among various issues of political significance, such as climate change.
    ...
    \item \emph{Political Right}: Same as above, but with the political right.
\end{itemize}
...
\item \emph{Watch neutral}: Load and watch a video from mainstream, politically neutral news outlets as defined by the fact-checking organization Media Bias/Fact Check.\footnote{https://mediabiasfactcheck.com/}
...
Because the video being watched when performing videopage collection may have an effect on the recommendations themselves, each agent loads the same video at all three collection points: The first video in the stain video list.
...
Altogether, Algorithm \ref{alg:sock_puppet} provides an overview of a sock puppet agent's interactions with the platform.
...
\begin{algorithm}
    ...
    \begin{algorithmic}
        ...
        \State \sw{Clear YouTube activity (cancel revertible actions)}
...
\al{
%\marginnote{R1-4}
We now describe the configurations of agents for the overall experiment. For each topic we tested seven strategies, each five times, resulting in (7*5=) 35 agents. All 35 agents of a given topic were run in parallel in order to deal with recommendation noise that may arise from having agents make queries on different days.}

For a given topic, we also drew five stain video lists, and assigned each list to exactly one agent within every strategy tested. Doing so assures that agents of the same strategy watch different sets of staining videos, boosting generalizability of strategy effects, while simultaneously assuring that agents of different strategies watch, overall, the same videos, enabling comparability between strategies. 
...
The two raters then went on and labeled all the remaining channels. The final IRR kappa scores were 0.660, 0.822, 0.854, and 0.945, respectively. The raters also discussed all disagreed cases and resolved disagreement. The final annotation codebook is attached in~\Cref{app:codebook}. The annotation results and IRR calculation can be previewed via this link.\footnote{Anonymous link: \url{https://tinyurl.com/45t2rhrs}}
}
...
\al{
%\marginnote{SPC-1}
In contrast, the videopage demonstrated significant changes in stain only on \emph{Political Right} and \emph{Antitheist}.} 
%Some topics actually experienced a slight decrease between P1 and P2. 
\al{One topic actually experienced a slight decrease between P1 and P2.}
...
\al{Comparing average relative changes between P2 and P3, \emph{Not interested} clearly wins out (-88\%). 
%\marginnote{SPC-1}
}
...
We wanted to know whether our scrubbing strategies removed stain in general, or if they only removed the subset of channels for which the the agent had explicitly scrubbed up to that point.
