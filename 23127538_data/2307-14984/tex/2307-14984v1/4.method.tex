...
It is worth mentioning that we did not consider social bots and zombie users in this work.
...
Next, we predict occupations only using pre-trained LLMs. In this scenario, we do not rely on additional external information, implying a zero-shot approach. Instead, we directly feed users' posts and personal profile descriptions to the tuned LLM for prediction. By examining the content of these inputs, the model showcased its capacity to comprehend and infer users' occupations, further enhancing our demographic prediction capabilities. The rationale behind these prediction tasks is twofold. Firstly, gender prediction aimed to showcase the impressive capacity of the language model for understanding and inferring information from personal descriptions. Secondly, age prediction highlighted the language model's ability to grasp age-related expressions within posts. Lastly, occupation prediction demonstrated how, even in the absence of explicit ground truth information, leveraging the language model's extensive knowledge and contextual understanding can enable us to expand our demographic predictions effectively.


\noindent \textbf{Prediction Result Evaluation}

The outcomes of our age and gender prediction analysis are presented in Table \ref{tab:dem_pre}. Our gender predictor, which relies on a fine-tuned Language and Linguistic Model (LLM), achieves satisfactory results. Despite the absence of explicit gender information in all personal descriptions, the predictor successfully generates valid predictions. Moving on to age, we select English blogs from \cite{schler2006effects} and ensured similar age distribution across the training and testing process. The results show that the mean squared error (MSE) was 128, while the mean absolute error (MAE) was around 7.53. These values indicate a 21.5\% unified percentage error (see Table \ref{tab:dem_pre}). Note that when we examine language patterns and usage within a relatively small age range (such as this dataset), it does not deviate significantly. This means that individuals within the same age range tend to use similar vocabulary, sentence structures, and linguistic expressions. Considering the context of language and the relatively small age difference within the same age range, the performance we obtain can be considered acceptable.


As for the occupations, we initially include the posts and personal descriptions of the combined user dataset in the prompt. We then feed the prompt to pre-trained ChatGLM to obtain the occupation of each user since we do not have an explicit occupation label. Thus, we leave the supervised prompt tuning for occupation as future work. It results in a total of 1,016 different occupations being identified from all users. However, utilizing all occupations is not essential since some occupations are very close. Thus, we group all occupations into 10 distinct occupation categories using the LLM, of which the categories can be found in Table \ref{tab:gpt_occ}. By condensing the number of occupations into a smaller set, we are able to simplify the simulation. The details can be found in Appendix.
...
User profile information serves as a reference point for the LLMs, furnishing essential context to shape content responses. The current emotional or attitudinal state symbolizes the user's mindset before reacting to the event, thereby playing a vital role in the LLM's generation of potential responses.
...
We first sample positive samples by extracting instances of observed interaction behaviors, while negative samples are sampled from others.
However, we encountered an inherent imbalance in the real training sample, where negative samples tend to dominate the positive samples in terms of numbers. This makes the absolute performance relatively low. It is important to note that this disparity between positive and negative samples is not a significant concern for our task since we focus more on the positive samples (the observed behaviors in simulation). 
Despite the exceedingly high tendency to predict as negative (due to the imbalanced distribution of real data),  the precision of the algorithm remains still consistently higher than 0.5 (0.61 on Nuclear Energy Scenario and 0.55 on Gender Discrimination Scenario), indicating an acceptable prediction ability. We leave overcoming the imbalanced data distribution as future work.
Specifically, to leverage the ability of LLMs to simulate a real user's interaction behavior, we prompt the model with information regarding the user's demographic properties, \textit{i.e.} gender, age, and occupation, in addition to the specific posts received. By incorporating these contexts, we enable LLM to make predictions regarding the user's inclination to repost the message or post new content.
...
For prompting-driven methods, we use either GPT-3.5 API provided by OpenAI\footnote{https://platform.openai.com/overview} or a ChatGLM-6B model~\cite{duetal2022glm}.
For prompt-tuning methods, we conduct the tuning based on the open-source ChatGLM model in a single GPU card. To reduce computational resources, 4-bit quantization was chosen throughout the experiments. Any other hyper-parameters remained the default.
