...
\author[a]{J. I. McDonald,}
\author[b]{B. Garbrecht,}
\author[c]{P. Millington}



\emailAdd{jamie.mcdonald@uclouvain.be}
...
Reference~\cite{Millar:2022peq} approached the calculation of axion-photon conversion in 3D magnetised plasmas by directly solving the equations of axion electrodynamics,
...
In this work, rather than attempting to solve classical field equations directly, we show instead that kinetic theory provides a simple, elegant and powerful framework, that offers a consistent language to address all the issues mentioned above. In this formalism, the calculation can be carried out in a controlled way by deriving a Boltzmann-like transport equation (see, e.g.,  Eqs.~\eqref{eq:BoltzmannExplicit} and \eqref{eq:BoltzmannEMFields}) for the phase-space distribution of photons. This describes their production due to axions and subsequent propagation along photon worldlines. Not only does this make clear the physical interpretation at every step of the calculation, but the final result is well-behaved when integrated over the photon emission surface, and free from divergences in a formal sense, so long as perturbation theory remains valid.
...
To make the physical properties of our transport equation more manifest, it is informative to integrate Eq.~\eqref{Eq:BoltzmannFull} over positive frequencies by acting with $\int_0^\infty dk_0 $. This leads to an explicit form
...
\end{equation}
which, using the temporal gauge identity $\mathcal{E}^2 = -  \left|\textbf{E}\right|^2$, can be written as
\begin{equation}\label{eq:EigenvalueDerivaive}
   \mathcal{E}^2 \partial_{k_0} \mathcal{H}  = -\frac{ \omega \, \left|\textbf{E}\right|^2}{R}.
\end{equation}
...
\end{equation}
Here, we have used $\omega^2\mathcal{E}^*_i \mathcal{E}_i =\left|\textbf{E}\right|^2$ and defined
\begin{equation}
...
\begin{equation}
\frac{ \varepsilon^*_\mu \varepsilon_\nu \Pi^{\lessgtr\, \mu \nu}  }{  \varepsilon^2 \partial_{k_0} \mathcal{H} }  =  - \frac{\omega U_E}{U} \hat{\boldsymbol{\varepsilon}}^*_i \hat{\boldsymbol{\varepsilon}}_j  \Pi^{<\, ij}.
\end{equation}
...
\partial_t f_\gamma + \textbf{v}_g \cdot \nabla_\textbf{x} f_\gamma - \nabla_\textbf{x} E_\gamma \cdot \nabla_\textbf{k} f_\gamma - \partial_t E_\gamma \partial_{E} f_\gamma = -  \frac{\omega U_E}{U} \hat{\boldsymbol{\varepsilon}}^*_i \hat{\boldsymbol{\varepsilon}}_j \Big( (1+ f_\gamma) \Pi^{<\, ij}  - \Pi^{>\, i j} f_\gamma \Big),
\end{equation}
Note that the appearance of the stored energy in the plasma, as captured by the factor $R = U_E/U$ defined in Eq.~\eqref{eq:R}, has also been identified in scattering processes in plasmas \cite{Melrose1981} and was discussed in Ref.~\cite{Millar2021}. 
...
\end{equation}
We reiterate that the eigenlabel $c$ on $f^c$, $\varepsilon^c$ and $\mathcal{H}^c$ has been suppressed; it is understand that we deal with production of a particular photon eigenmode, whose distribution function we label simply $f_\gamma$. 

We recall that the object $\mathcal{H}=\mathcal{H}(k,x)$ is the Hamiltonian of the mode, with $k_\mu$ and $x^\mu$ giving the 4-momentum and coordinates of the photon, respectively. Setting $\mathcal{H}(k,x) =0$ gives the dispersion relation for that mode. For example, in an unmagnetised cold plasma, one has $\mathcal{H}= k_\mu k^\mu - \omega_p^2$. The object $\Pi_{\rm ax}^{ < \mu \nu}(x,k)$ is the Wigner transform of the photon Wightman self-energy, and corresponds to production due to axions. It is defined by
\begin{equation}\label{eq:Piax}
	\Pi^{< \mu \nu}_{\rm ax}(x,x') =\braket{j^\mu_\phi(x) j^\nu_\phi(x')} .
\end{equation}
...
Furthermore, we should remember that $k_0$ appearing in Eq.~\eqref{eq:SelfEnergy} is on-shell with respect to the photon dispersion relation, i.e., $k_0 = E_\gamma(\textbf{k},x)$. Since the axion energy satisfies $E_\phi(\textbf{k})^2 = \left|\textbf{k}\right|^2 + m_\phi^2$, it follows that the delta function can be written as
...
With this in mind, we construct solutions using a method of characteristics, which consists of identifying those integral curves $(x^\mu (\lambda),k_\mu(x))$ in phase space that satisfy
...
\end{equation}
By solving this equation and propagating the solutions out to infinity, one immediately obtains the asymptotic distribution of photons measured by an external observer. This situation is illustrated in Fig.~\ref{fig:Rays}. Note also that $E_{\gamma, a} ( \lambda)$ is a shorthand for $E_{\gamma, a}\left(\textbf{k}(\lambda),x(\lambda)\right)$ and that the axion energy function is evaluated along the \textit{photon} worldline. 
% Figure environment removed
...
A^\mu(x) = \sum_c \int \frac{{\rm d}^3 \textbf{k}}{(2 \pi)^3 } \left[ \varepsilon^\mu_c c^c_\textbf{k} e^{ - i k\cdot x} + \varepsilon^{* \, \mu}_c c^{c \, \dagger}_\textbf{k} e^{i k\cdot x} \right],
\end{equation}
...
\begin{equation}
   \left\{ f_\gamma, \mathcal{H} \right\} =
	\int \frac{{\rm d}^3 \textbf{k}_a}{(2\pi)^3 2 E^a_\textbf{k}} \left| \mathcal{M}_{a \rightarrow \gamma}\right|^2 (2 \pi)^4 \delta^{(4)}(k - k_a) f_\phi,
\end{equation}
...
\begin{equation}
	\left| \mathcal{M}_{a \rightarrow \gamma}\right|^2  =\left|\bra{\gamma} \mathcal{L}_{\rm int} \ket{a}\right|^2 =g_{a \gamma \gamma}^2 \big| k \cdot \tilde{F}_{\rm ext} \cdot \varepsilon\big|^2.
\end{equation}
...
\begin{align}
   \frac{d \, f_\gamma}{d \lambda}  & =  \pi   g_{a \gamma \gamma}^2 \big| k \cdot \tilde{F}_{\rm ext} \cdot \varepsilon\big|^2 \frac{ \delta\left(E_\gamma(\lambda) - E_\phi(\lambda) \right)}{ E_\gamma}  f_\phi.
\end{align}
...
\end{align}
Next, we can integrate this equation over a \textit{finite} spatial 3-volume $\mathcal{V}$, whose bounding surface has area element $dA$, and a region in 4-momentum space $\int d^4 k$ running over all momenta. Upon performing these integrations, we arrive at
...
\end{equation}
where we have used $\partial_\textbf{k}  \mathcal{H} /\partial_{k_0} \mathcal{H}  = \textbf{v}_g$  and $\partial_\textbf{x} \mathcal{H}/\partial_{k_0} \mathcal{H} = \nabla_\textbf{x} E_\gamma$. To derive the second term, we applied the divergence theorem with respect to the spatial integral. We also discarded the surface terms arising from the $d^4 k$ integration of $k_0 \partial_k \left[  \partial_x \mathcal{H}_c  f^c \delta(\mathcal{H}_c) \right]$. The source term $Q$ is defined as
\begin{equation}
...
\end{equation}
where $d\Sigma$ is the area element on the surface defined by $G(\textbf{x}) =0$. Applying this to the spatial integration on the right-hand side of Eq.~\eqref{eq:EnergyConservation}, by treating $\delta \left(E_\gamma ( \textbf{k},\textbf{x})^2 - E_\phi(\textbf{k})^2 \right)$ as a function of $\textbf{x}$ for fixed $\textbf{k}$, we arrive at
...
\end{equation}
In addition, $\textbf{v}_p\cdot \nabla E_\gamma= (\omega_p/E_\gamma) \textbf{v}_p \cdot \nabla \omega_p$, and the polarisation vectors are transverse to the direction of propagation, so that $ (\textbf{B}_{\rm ext}\cdot \boldsymbol{\varepsilon})^2 = \sin^2 \theta |\textbf{B}_{\rm ext}|^2$, where $\theta$ is the angle between $\textbf{k}$ and $\textbf{B}_{\rm ext}$. The ratio of the total and electric energy in a weakly magnetised plasma turns out to be $U/U_E = 2$. Altogether, using Eq.~\eqref{eq:ProbabilityPhysical}, this yields
...
\end{equation}
We choose coordinates in which $\textbf{k}=(0,0,k)$ points in the z-direction, and $\textbf{B}_{\rm ext}$ lies in the $z$-$y$ plane, with $\textbf{B}_{\rm ext} = \left| \textbf{B}_{\rm ext} \right|( - \sin \theta \hat{\textbf{y}} +  \cos \theta \hat{\textbf{z}})$, so that $\theta$ is the angle between $\textbf{k}$ and $\textbf{B}$.  For a strongly magnetised plasma, neglecting relativistic effects\footnote{The generalisation to the relativistic limit is straightforward, see, e.g., Ref.~\cite{Witte:2021arp}.}, we find the permittivity 3-tensor is given by \cite{gurevich_beskin_istomin_1993}
...
These are the magnetosonic-t, Alfv{\'e}n, and Langmuir-O (LO) modes, respectively. To leading order in $g_{a \gamma \gamma}$, the LO mode is the only one which can both be produced on resonance and propagate out of the magnetosphere as $\omega_{\rm p} \rightarrow 0$. The LO mode has electric polarisation unit 3-vector
 \begin{equation}
...
\begin{equation}
\nabla_\textbf{x} E_\gamma (k,x) =\frac{\omega  \omega _p \left( \cos \theta  \omega _p \left(\omega _p^2-\omega ^2\right) \nabla_\textbf{x} \cos \theta  +\omega ^2  \sin ^2\theta
...
\begin{itemize}
	...
	The answer is that it is \textit{already} contained in the factor $1/|\nabla_{x} E_\gamma|$ appearing in the probability (see Eq.~\eqref{eq:PStrongB}). On the one hand, this term can be thought of as capturing the physical width of the resonance, in that it quantifies the size of spatial gradients in the background. In this sense, it defines an effective conversion length $L_c \propto /|\nabla_{x} E_\gamma|^{1/2}$ over which the resonance persists. However, this term also completely captures the refraction of the photon, and occurs directly from the force term $\partial_\textbf{x}\mathcal{H}$ in the Boltzmann equation. Thus, $ 1/|\nabla_{x} E_\gamma|^{1/2}$ can also be thought of as capturing a generalised notion of a radius of curvature $\mathcal{R} \propto 1/|\nabla_{x} E_\gamma|^{1/2}$ of the photon due to refraction which quantifies the degree to which the photon is accelerated as it passes through the resonance. This is true whether the photon is accelerated parallel or transverse to the sourcing axion, so that it is more helpful to think of the conversion probability as scaling as $P_{a \gamma} \propto \mathcal{R}^2$ with $1/\mathcal{R}$ proportional to the acceleration experienced by the photon.
...
\newpage
	...
	\begin{equation}
   	\left| \nabla_\textbf{x} E_\gamma \right| \ll  g_{a \gamma \gamma}^2  (  \boldsymbol{\epsilon} \cdot \textbf{B}_{\rm ext} )^2.
	\end{equation}
...
J.I.M.~is indebted to Sam Witte for many useful discussions and thanks Richard Battye and the University of Manchester for hospitality during the development of this work. He also acknowledges support from an FSR Fellowship. P.M.~is supported by a United Kingdom Research and Innovation (UKRI)
Future Leaders Fellowship [Grant No.~MR/V021974/2].
...
\end{equation}
where we made use of the the Poisson bracket defined in Eq.~\eqref{eq:PB}, which we will use frequently in the subsequent approximations. We further note that the Wigner transform of the kinetic operator $\mathcal{D}_0$ in Eq.~\eqref{eq:DmunuSpatial} can be written as
...
\begin{align}\label{eq:Dkin:momentumspace}
    \mathcal{D}_0^{\mu\nu}(k)=-k^2 \eta^{\mu\nu}+k^\mu k^\nu -\frac1\xi P^{\mu\alpha}P^{\nu\beta}k_\alpha k_\beta,
\end{align}
where the subscript $X$ in Eq.~\eqref{eq:AppWigKinetic} indicates the presence of $X$-derivatives which act on $D^{<,>}(k,X)$.
...
\end{align}
For general $D^{<,>}$, i.e., with possible correlations between the physical polarization states, the commutators do not vanish. They only do so when $D$ is polarization-diagonal, as we shall assume eventually.
...
\end{equation}
where, in this appendix, we allow ourselves to use $a$ as a polarisation label, since here it cannot be confused with axions. Expanding the Poisson bracket by the product rule and using $\varepsilon^a \cdot \varepsilon^c = \delta^{ac}$,  we find
\begin{align}\label{eq:PoissonBraketKinetic}
   ...
   &+  \left[ \,f_a \delta(\mathcal{H}_a) (\varepsilon^{a \, *} \cdot \partial_{[k } \mathcal{D} \cdot \partial_{x]} e^a)   + \sum_c  f_c\delta(\mathcal{H}_c) (\varepsilon^{a \, *} \cdot \partial_{[k } \mathcal{D} \cdot \varepsilon^c) \, (  e^{a } \cdot \partial_{x]} e^{c \, *}) \right].
\end{align}
...
\begin{align}
  \textbf{I}:\quad & \varepsilon^{a \, *}  \cdot \partial_k  \mathcal{D}\cdot \varepsilon^a  =  \partial_k \mathcal{H}_a,
 	\\
 	\nonumber \\
  \textbf{II}:\quad  &\delta (\mathcal{H}_c) ( \varepsilon^{a\, *}  \cdot \partial_{[ k}  \mathcal{D} \cdot \varepsilon^c)\,  ( e^a \cdot \partial_{x]} e^{c \, *})   =0,\label{eq:app:polderII}
 	\\
	\nonumber \\
...
\end{equation}
Finally, we can multiply this by $\varepsilon^a \cdot \partial_x e^{c \, *}$ to arrive at
\begin{equation}\label{eq:IdII}
\delta (\mathcal{H}_c) (\varepsilon^{a\, *}  \cdot \partial_{ k}  \mathcal{D} \cdot \varepsilon^c )(e^a \cdot \partial_{x} e^{c\, *} )   = \delta^{ac} \partial_{k} \mathcal{H}_a \delta (\mathcal{H}_a) (e^a \cdot \partial_{x} e^{a \, *})  +  \mathcal{H}_a \delta(\mathcal{H}_c) (\varepsilon^{c \, *} \cdot \partial_{k} \varepsilon^a )  (e^a \cdot \partial_{x} \varepsilon^{c \, *} ),
\end{equation}
where, in the first term on the right-hand side, we used $\delta^{a c}$ to set $a = c$ in $\varepsilon^a\cdot \partial_x e^{c\,*}$. The first term on the right-hand side of Eq.~\eqref{eq:IdII} vanishes, because $\varepsilon^a\cdot \partial_x \varepsilon^{a\, *}  = \partial_x(\varepsilon^a \cdot \varepsilon^{a \, *})/2 =0$, since $\varepsilon^a\cdot \varepsilon^{a\,*} = 1$. This leaves
\begin{equation}
\delta(\mathcal{H}_c) (\varepsilon^{a\, *} \cdot \partial_k \mathcal{D} \cdot \varepsilon^c) ( e^a \cdot \partial_x e^{c\, *} )  = \mathcal{H}_a \delta(\mathcal{H}_c) (\varepsilon^c \cdot \partial_k \varepsilon^{a \, *}) (\varepsilon^a \cdot \partial_x e^{c \, *}).
\end{equation}
However, the second term on the right-hand side is clearly symmetric under interchange of $k$ and $x$. As such, it is killed by virtue of antisymmetrisation, so that
\begin{equation}
\delta (\mathcal{H}_c) ( \varepsilon^{a\, *}  \cdot \partial_{[ k}  \mathcal{D} \cdot \varepsilon^c )( e^a \cdot \partial_{x]} e^{c\, *} )   =  0.
\end{equation}
...
\begin{equation}
	\mathcal{E}' = \mathcal{E}_\mu - i k_\mu \tilde{\Lambda}.
\end{equation}
...
\end{equation}
The derivatives $\partial_X$ are by assumption sub-dominant in the gradient expansion and can be neglected. Similarly, by Taylor expanding about $r=0$, we have
\begin{align}
...
\begin{equation}
	\Pi^{ < \, \mu \nu}_{\rm ax}(k,X) = g_{a \gamma \gamma}^2 k_\rho k_\sigma \tilde{F}_{\rm ext}^{\mu \rho}(X) \tilde{F}_{\rm ext}^{\nu \sigma}(X)  2 \pi \delta(k^2 - m_\phi^2) \left\{f_\phi(k,X) \theta(k_0) +  \theta(-k_0) \left[1 + f_\phi (k,X) \right] \right\}.
\end{equation}
...
\end{equation}
where $\hat{\varepsilon}^{\rm LO}_\nu$ is the unit 4-vector defined by $\hat{\varepsilon}^{\rm LO}_\nu = \varepsilon^{\rm LO}_\nu/(\varepsilon^{\rm LO\, *}_\mu \eta^{\mu \nu} \varepsilon^{\rm LO}_\nu)^{1/2}$. Next, we require the expression for $\partial_{k_0} \mathcal{H}$, which can be written on-shell as
