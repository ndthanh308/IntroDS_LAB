...
\begin{document}
	...
	\author{Deepankar Basu\thanks{Department of Economics, University of Massachusetts Amherst. Email: \texttt{dbasu@econs.umass.edu}. Without implicating them in any way in the contents of this paper, I would like to thank Michael Ash, William Greene and James MacKinnon for helpful comments on an earlier version of this paper.} }
	...
	%One use of the YFWL theorem is that if researchers are interested in only a subset of coefficients in a multiple regression equation, they need not estimate the full multiple regression. They can estimate the relevant partial regressions to recover estimates of the desired coefficients. In contexts of empirical analyses with large numbers of variables and observations, this can significantly reduce computational time and resources, e.g. models with large numbers of fixed effects. Will researchers also be able to conduct inference about the subset of coefficients in a multiple regression using the standard errors (or the covariance matrix) estimated from the relevant partial regression? \citet{udny-1907} and \citet{frisch-waugh-1933} did not pose, and therefore did not address, this question about standard errors (or the covariance matrix). 
	...
	The rest of the paper is organized as follows: in section~\ref{sec:setup}, I introduce the set-up and pose the main question; in section~\ref{yfwl-iv}, I present the YFWL for generalized instrumental variables estimation; in section~\ref{sec:history}, I present a narrative account of the analytical development of the theorem through the decades; I conclude in section~\ref{sec:conclusion} by highlighting the usefulness of, and highlighting a puzzle about the intellectual history of, the YFWL theorem. Proofs are collected together in the appendix.  
	...
	\end{equation}
	where $\beta_1$ and $\beta_2$ are $k_1 \times 1$ and $k_2 \times 1$ vectors of parameters. I would like to ensure that the model satisfies the identification condition \citep[p.~19, 223]{greene} for both sets of parameters, $\beta_1$ and $\beta_2$, using the following assumption.
	...
	\begin{assumption}\label{assmp:iv}
		...
		\begin{enumerate}
			...
			\item Relevance: The instrumental variables are correlated with the regressors. In particular, $ \left( Z'_2 W_1\right)  $ has rank  $ k_1 $, and $ \left( Z'_2 W_2\right)  $ has rank  $ k_2 $.\footnote{Note that both the exogeneity and relevance conditions can also be stated in asymptotic terms using probability limits.}
		\end{enumerate}
	\end{assumption}
	
	Given her interest and the set-up, can the researcher avoid estimating the full model in (\ref{model-full-iv})? Can she work with the partial model (\ref{model-partial-iv}) and yet consistently estimate and conduct proper statistical inference on $\beta_2$? The YFWL theorem provides an answer in the affirmative.
	
	
	\section{The YFWL theorem for linear instrumental variables estimation}\label{yfwl-iv}
	I will now derive a YFWL theorem for instrumental variables estimation, i.e. I will show: (a) the instrumental variables estimator of $\beta_2$ from the full regression model is equal to the the instrumental variables estimator of $\beta_2$ from the partial model; and (b) I will be show that the residuals from (\ref{model-full-iv}) are equal to the residuals from (\ref{model-partial-iv}). Since instrumental variables estimators are method of moment estimators \citep[p.~469]{greene}, I will work with the moment conditions defining the estimators to develop the proof.
	
	\subsection{The full regression}\label{full-reg}
	Let $b_{1}$ and $b_{2}$ be the instrumental variables estimators of $\beta_1$ and $\beta_2$, respectively, in the full regression model (\ref{model-full-iv}). By construction, the set of exogenous variables, $W_1$, and the set of instrumental variables, $Z_2$, are both orthogonal to the regression residual vector $u = Y - W_1 b_{1} - W_2 b_{2}$. Hence, we have the following $k_1+k_3$ moment condition equations,
	\begin{equation}\label{moment-cond-full}
		Z' \left( Y - W_1 b_{1} - W_2 b_{2} \right) = 0,
	\end{equation}
	where
	\begin{equation}
		\underset{\left( n \times \left( k_1+k_3\right)\right)  }{Z} = \left[ \underset{\left( n \times k_1\right) }{W_1} : \underset{\left( n \times k_3\right) }{Z_2} \right] 
	\end{equation}
	is the matrix of instrumental variables. Writing out the equations for the two components of $Z$ separately, we have,
	\begin{equation}
	W'_1 \left( Y - W_1 b_{1} - W_2 b_{2} \right) = 0.
	\end{equation}
	and
	\begin{equation}
		Z'_2 \left( Y - W_1 b_{1} - W_2 b_{2} \right) = 0
	\end{equation}		
	From the first equation, we get
	\[
	b_1 = \left( W'_1 W_1\right)^{-1} W'_1 Y -  \left( W'_1 W_1\right)^{-1} W'_1 W_2 b_2,
	\]
	where $\left( W'_1 W_1\right)^{-1}$ exists because $W_1$ is of full column rank (assumption~\ref{reg-col-rank}). This expression can be plugged into the second equation and rearranged to give us a linear system of equation for determining the $k_2 \times 1$ vector $b_2$ as
	\begin{equation}\label{eq-sys-m}
		Z'_2 M_{W_1} W_2 b_2 = Z'_2 M_{W_1} Y.
	\end{equation}
	Thus, $b_{2}$, the instrumental variables estimators of $\beta_2$ in the full regression model (\ref{model-full-iv}) is the solution of the linear equation system in (\ref{eq-sys-m}). Note that $Z'_2 M_{W_1} W_2$ is a $k_3 \times k_2$ matrix, where $k_3 \geq k_2$. Hence, in general (\ref{eq-sys-m}) cannot be inverted to give us the solution for $b_2$. 
	
	
	\subsection{The partial regression}
	Let $\tilde{b}_2$ denote the instrumental variables estimator of $\tilde{\beta}_2$ in the the partial regression model (\ref{model-partial-iv}). Since $Z_2$ is a set of valid instrumental variables for $W_2$, the residualized part of $Z_2$, i.e. $\widetilde{Z}_2=M_{W_1} Z_2$, is a valid instrumental variable for $\widetilde{W}_2$, the residualized part of $W_2$, appearing as the regressors in the partial regression (\ref{model-partial-iv}). Thus, $\widetilde{Z}_2$ is orthogonal to the residual vector of the partial regression (\ref{model-partial-iv}) $\tilde{u}=\widetilde{Y} - \widetilde{W}_2 \tilde{b}_{2}$, and we have the following moment conditions,
	\begin{equation}
		{\widetilde{Z}_2}' \left( \widetilde{Y} - \widetilde{W}_2 \tilde{b}_{2}\right) =0,
	\end{equation}
	which can be rearranged to give the following system of linear equations for determining the $k_2 \times 1$ vector $\tilde{b}_{2}$:
	\begin{equation}\label{eq-sys-p}
		{\widetilde{Z}_2}' \widetilde{W}_2 \tilde{b}_2 = {\widetilde{Z}_2}' \widetilde{Y}.
	\end{equation}
	
	%Let me point out that a straightforward way to construct $\widetilde{Z}_2$ is to discard the first $k_1$ columns of the matrix $M_{W_1}Z$, i.e. residualize the full set of instrumental variables $Z$ with respect to the exogenous variables, $W_1$ by pre-multiplying $Z$ with $M_{W_1}$, and then discard the first $k_1$ columns. This general account for constructing $\widetilde{Z}_2$ will be useful when we look at some special cases below.


	\subsection{Coefficients and residuals are equal}\label{proof-iv-coef}
	
	\begin{theorem}\label{thm:coef-resid}
		For the full regression model in (\ref{model-full-iv}) and the partial regression model (\ref{model-partial-iv}), we have
		\begin{enumerate}
			\item the estimated coefficient vectors $b_2$ from the full model (\ref{model-full-iv}) and $\tilde{b}_2$ from the partial model (\ref{model-partial-iv}) are identical, and
			\item the residual vectors from the full model (\ref{model-full-iv}) and from the partial model (\ref{model-partial-iv}) are identical when $b_2$ and $\tilde{b}_2$ are unique.
		\end{enumerate}
	\end{theorem}
	
	 
	\subsection{Special cases}
	There are three cases of interest with a unique solution of (\ref{moment-cond-full}), and hence of (\ref{eq-sys-m}): the IV estimator for an exactly identified model, the two stage least squared (2SLS) estimator for an overidentified model, and the ordinary least squares estimator (when all regressors are exogenous).
	
	\subsubsection{IV estimator} 
	In this case, the number of instruments is exactly equal to the number of endogenous variables, i.e. $k_3=k_2$. Thus $Z=\left[ W_1:Z_2\right] $ is a $n \times \left( k_1+k_2\right) $ matrix, and $Z' W$ is an invertible $\left( k_1+k_2\right) \times \left( k_1+k_2\right) $ matrix. Thus, the IV estimator of $b$ for the full regression model (\ref{model-full-iv}) is \citep[p.~186]{cameron-trivedi-2005} given by
	\begin{equation}\label{bfull-iv}
		b = 
		\begin{pmatrix}
			b_1 \\
			b_2
		\end{pmatrix} =
		\left( Z' W\right) ^{-1} Z'Y,
	\end{equation}
	so that $b_2$ is the ($2,1$) block of the right hand side (RHS) of (\ref{bfull-iv}). 
	
	Turning to the partial model (\ref{model-partial-iv}), we should first note that $\widetilde{Z}_2 = M_{W_1}Z_2$ is a $n \times k_2$ matrix and $\widetilde{W}_2 = M_{W_1}W_2$ is a $n \times k_2$ matrix as well. Hence, ${\widetilde{Z}_2}' \widetilde{W_2}$ is an invertible $k_2 \times k_2$ matrix. Thus, the IV estimator for the partial model (\ref{model-partial-iv}) using $\widetilde{Z}_2$ as the set of instrumental variables, is  
	\begin{equation}
		\tilde{b}_2 = \left( {\widetilde{Z}_2}' \widetilde{W_2}\right) ^{-1} {\widetilde{Z}_2}' \widetilde{Y},
	\end{equation}
	and by the general result proved in theorem~\ref{thm:coef-resid}, we have $b_2 = \tilde{b}_2$.
	
	
	\subsubsection{2SLS estimator} 
	In this case, the number of instrumental variables is strictly larger than the number of endogenous variables and we choose from among all possible linear combination of the set of instruments that one which minimizes the asymptotic covariance matrix of the instrumental variables estimator \citep{brundy-jorgenson-1971}. In this case \citep[p.~231]{greene}, the estimator for the full regression model (\ref{model-full-iv}) is given by 
		\begin{equation}
			b = 
			\begin{pmatrix}
				b_1 \\
				b_2
			\end{pmatrix} =
		\left[ W' Z \left( Z' Z\right)^{-1} Z' W\right]^{-1} W' Z \left( Z' Z\right)^{-1} Z' Y,
		\end{equation}
	where $Z = \left[ W_1 : Z_2\right] $ is the full set of instruments, including the exogenous variables, $W_1$. But this is just a special case of the IV estimator in (\ref{bfull-iv}) with the set of instrumental variables given by the $n \times \left( k_1+k_2\right) $ matrix 
	\begin{equation}\label{instrument-2sls}
		P_Z W = Z \left( Z' Z\right)^{-1} Z' W.
	\end{equation}
	Thus, $\left( P_Z W\right) ' W$ is an invertible $\left( k_1+k_2\right) \times \left( k_1+k_2\right) $ matrix and we have the 2SLS estimator
	...
	\end{equation}
	so that $b_2$ is the ($2,1$) block of the right hand side (RHS) of (\ref{bfull-2sls}). 
	...
	Turning to the partial model (\ref{model-partial-iv}), we estimate it with the instrumental variables formed by the  residualized set of instrumental variables, $\widetilde{Z}_2=M_{W_1}Z_2$, which is a $n \times k_3$ matrix. Thus, the 2SLS estimator for the partial regression model (\ref{model-partial-iv}) is given by
		\begin{equation}\label{bpartial-2sls}
			\tilde{b}_2 = \left[ \left( P_{\widetilde{Z}_2} \widetilde{W}_2 \right)' \widetilde{W}_2 \right]^{-1} \left( P_{\widetilde{Z}_2} \widetilde{W}_2 \right)' Y,
		\end{equation}
	and, once again, by the general result established in theorem~\ref{thm:coef-resid}, we have $b_2 = \tilde{b}_2$ for the 2SLS estimator.
	
	
%	\subsubsection{Linear GMM estimator}
%	In this case also, we have an overidentified model and the number of instrumental variables is strictly larger than the number of endogenous variables. Suppose the researcher has the same set of instrumental variables as in the 2SLS case, given by $Z = \left[ W_1:Z_2\right] $. Let $\Sigma$ denote a $(k_1+k_3) \times (k_1 + k_3)$ symmetric weighting matrix of full rank. Then the general linear GMM estimator of $\beta_1$ and $\beta_2$ in (\ref{model-full-iv}) is given by \citep[p.~185]{cameron-trivedi-2005}:
%	\begin{equation}
%		b = 
%		\begin{pmatrix}
%			b_1 \\
%			b_2
%		\end{pmatrix} =
%		\left( W' Z \Sigma Z W\right) ^{-1} W' Z \Sigma Z' Y.
%	\end{equation}
%	But this is just an IV estimator (\ref{bfull-iv}) where the set of instrumental variables is given by the $n \times \left( k_1+k_2\right) $ matrix 
%	\begin{equation}\label{instrument-gmm}
%		U=Z \Sigma Z' W.
%	\end{equation} 
%	Hence, $U' W$ is an invertible $\left( k_1+k_2\right) \times \left( k_1+k_2\right) $ matrix and we have the liner GMM estimator of the full model (\ref{model-full-iv}) given by
%	\begin{equation}\label{bfull-gmm}
%		b = 
%		\begin{pmatrix}
%			b_1 \\
%			b_2
%		\end{pmatrix} =
%		\left[ U' W\right]^{-1} U' Y,
%	\end{equation}
%	so that $b_2$ is the ($2,1$) block of the right hand side (RHS) of (\ref{bfull-gmm}).
%	
%	Let us now turn to the partial model (\ref{model-partial-iv}). Note that $\widetilde{Z}_2$, the residualized set of instrumental variables, is constructed in this case as follows: we take the full set of instrumental variables $U$ in (\ref{instrument-gmm}) and pre-multiply it by $M_{W_1}$ and then discard the first $k_1$ columns. We are left with a $n \times k_2$ matrix, $\widetilde{Z}_2$. Hence, ${\widetilde{Z}_2}' \widetilde{W_2}$ is an invertible $k_2 \times k_2$ matrix. Thus, the linear GMM estimator for the partial regression model (\ref{model-partial-iv}) is given by
%	\begin{equation}\label{bpartial-2sls}
%		\tilde{b}_2 = \left( \widetilde{Z}'_2 \widetilde{W_2}\right) ^{-1} \widetilde{Z}'_2 \widetilde{Y},
%	\end{equation}
%	and, once again, by the general result established in theorem~\ref{thm:coef-resid}, we have $b_2 = \tilde{b}_2$ for the linear GMM estimator.
	
	\subsubsection{OLS estimator}
	In this case, both sets of regressors in (\ref{model-full}), $W_1$ and $W_2$, are exogenous. Thus $Z = W = \left[ W_1: W_2\right] $ and the OLS estimator of the parameters in the full model is 	
	\begin{equation}\label{bfull-ols}
		b = 
		\begin{pmatrix}
			b_1 \\
			b_2
		\end{pmatrix} =
		\left[ W' W\right]^{-1} W' Y,
	\end{equation}
	so that $b_2$ is the ($2,1$) block of the right hand side (RHS) of (\ref{bfull-ols}), and the OLS estimator from the partial model is 
	\begin{equation}\label{bpartial-ols}
		\tilde{b}_2 = \left( \widetilde{W}'_2 \widetilde{W_2}\right) ^{-1} \widetilde{W}'_2 \widetilde{Y},
	\end{equation}
	and, once again, by the general result established in theorem~\ref{thm:coef-resid}, we have $b_2 = \tilde{b}_2$ for the OLS estimator.

	
	\subsection{Relationship between estimated covariance matrices}
	I will now use the result in theorem~\ref{thm:coef-resid} that the vector of residuals from the full and partial models are identical to investigate the relationship among the \textit{estimators} of the covariance matrices of $b_2$ (instrumental variables estimator of $\beta_2$ in (\ref{model-full}) from the full regression model) and $\tilde{b}_2$ (instrumental variables estimator of $\beta_2$ in (\ref{model-full}) from the partial regression model).
	...
	\begin{theorem}\label{thm:cov}
		Consider $b_2$, the instrumental variables estimators of $\beta_2$ in (\ref{model-full}) from the full regression model (\ref{model-full-iv}), and $\tilde{b}_2$, the instrumental variables estimators of $\beta_2$ in (\ref{model-full}) from the partial regression models in (\ref{model-partial-iv}). If the estimators of the error vectors in the full and partial regression models are equal, $\widehat{\Omega}_f=\widehat{\Omega}_p$  (after making degrees of freedom corrections, if necessary), then
		\begin{equation}
			\widehat{\Var} \left( b_2\right) = \widehat{\Var} \left( \tilde{b}_2\right) \text{ (after making degrees of freedom corrections).}
		\end{equation}
	\end{theorem}
	
	\begin{remark}
		The proof of the theorem can be found in appendix~\ref{app:cov}. At this point, let us return to (\ref{eq-sys-p}) to note an important point. Using the definitions of the residualized vectors and matrices, we see that (\ref{eq-sys-p}) is
		\begin{equation}
			\left( {M_{W_1} Z_2}\right) ' M_{W_1} W_2 \tilde{b}_2 = \left( {M_{W_1} Z_2}\right) ' M_{W_1} Y,
		\end{equation}
		which becomes
		\begin{equation}
			Z'_2 M_{W_1} W_2 \tilde{b}_2 = Z'_2 M_{W_1} Y,
		\end{equation}
		because $M_{W_1}$ is symmetric and idempotent. Thus, (\ref{eq-sys-p}) is equivalent to
		\begin{equation}\label{partial-eqiv-1}
			Z'_2 \widetilde{W}_2 \tilde{b}_2 = Z'_2 \widetilde{Y},
		\end{equation}
		so that using $Z_2$ (instead of $\widetilde{Z}_2$) as the set of instrumental variables for estimating the partial model (\ref{model-partial-iv}) would give us an identical value of $\tilde{b}_2$. Similarly, 
		(\ref{eq-sys-p}) is also equivalent to
		\begin{equation}\label{partial-equiv-2}
			{\widetilde{Z}_2}' W_2 \tilde{b}_2 = {\widetilde{Z}_2}' Y,
		\end{equation}
		so that estimating a linear regression of $Y$ on $W_2$ using $\widetilde{Z}_2$ as the set of instrumental variables would also give us $\tilde{b}_2$ as the value of the coefficient vector on $W_2$. I recommended that neither of these models, (\ref{partial-eqiv-1}) or (\ref{partial-equiv-2}), be estimated in place of estimating (\ref{model-partial-iv}) with $\widetilde{Z}_2$ as the set of instrumental variables. This is because of reasons pertaining to the relationship between estimators of the covariance matrix of $b_2$ and $\tilde{b}_2$. 
		
		If (\ref{partial-eqiv-1}) is estimated, then the covariance matrix of the IV estimator would, instead of the expression in (\ref{estvar-model-partial}), become
		\begin{equation}
			\widehat{\Var} \left( \tilde{b}_2\right) = \left[{\left(P_{Z_2} \widetilde{W}_2\right)}^{'} \left(P_{Z_2} \widetilde{W}_2\right)\right]^{-1} \left(P_{Z_2} \widetilde{W}_2\right)' \widehat{\Omega}_p \left[{\left(P_{Z_2} \widetilde{W}_2\right)}^{'} \left(P_{Z_2} \widetilde{W}_2\right)\right]^{-1},
		\end{equation}
		and the proof of theorem~\ref{thm:cov} would not go through. In fact, we would not be able to complete step~2 of the proof. This is because $P_Z \widetilde{W}_2 \neq P_{Z_2} \widetilde{W}_2$. If, on the other hand, (\ref{partial-equiv-2}) is estimated, then the residual vector from the partial regression becomes $Y - \tilde{b}_2 W_2$ and this is not equal to the residual from the full regression model. This can be seen from the argument in (\ref{resid-equal}). Hence, $\widehat{\Omega}_f \neq \widehat{\Omega}_p$, and theorem~\ref{thm:cov} will not apply.
	\end{remark}
	 

	Let me summarize what we get from theorem~\ref{thm:cov}: (a) if the estimate of the error covariance matrix depends only on the vector of regression residuals, then (\ref{estvar-model-full}) and (\ref{estvar-model-partial}) are exactly equal; (b) if some degree of freedom correction is applied to generate the estimate of the error covariance matrix, then (\ref{estvar-model-full}) and (\ref{estvar-model-partial}) are equal up to the relevant degree of freedom correction; (c) if the estimate of the error covariance matrix depends, in addition to the residual vector, on the elements of the matrix of regressors, then (\ref{estvar-model-full}) and (\ref{estvar-model-partial}) are not, in general, equal even after making degree of freedom adjustments.
	
	In particular, theorem~\ref{thm:cov} in this paper extends \citet[Theorem~2, 3, 4]{ding-2021} to linear instrumental variables estimation and we have the following results: 
	\begin{enumerate}
...
%	\subsection{The YFWL for ordinary least squares estimation}
%	
%	The YFWL theorem for instrumental variables estimation immediately carry over to ordinary least squares (OLS) estimation because OLS estimation is a special case of instrumental variables estimation where the full set of regressors are its own instruments. That is why OLS estimators can are also method of moments estimators \citep[p.~469]{greene}. Thus, if both sets of regressors in (\ref{model-full}), $W_1$ and $W_2$ are exogenous, we can use these to generate moment conditions for estimating the parameters of the model. All we need to do is rewrite the moment conditions for the full regression model (\ref{model-full-iv}) using orthogonality of the two sets of regressors as
%	\begin{equation}\label{mom:ols-f1}
%		W'_2 \left( Y - W_1 b_{1} - W_2 b_{2} \right) = 0,
%	\end{equation}		
%	and
%	\begin{equation}\label{mom:ols-f2}
%		W'_1 \left( Y - W_1 b_{1} - W_2 b_{2} \right) = 0,
%	\end{equation}
%	and use the orthogonality of the residualized regressor in the partial regression model (\ref{model-partial-iv}) as
%	\begin{equation}\label{mom:ols-p1}
%		{\widetilde{W}_2}' \left( \widetilde{Y} - \widetilde{W}_2 \tilde{b}_{2}\right) =0.
%	\end{equation}
%	The rest of the analysis proceeds exactly as in the case of instrumental variables estimation and give us the following results: (a) the estimated coefficient vectors $b_2$ and $\tilde{b}_2$ are identical; (b) the vector of residuals from the full and partial regressions are identical; (c) if estimators of the covariance of the error vectors are only functions of the regression residual vectors, then the estimated covariance matrices of $b_2$ and $\tilde{b}_2$ are equal up to a degree of freedom correction.
	...
	%The YFWL is a remarkable result for linear regression models estimated with the method of instrumental variables or (ordinary or generalized) least squares. Not only does it make computation faster by allowing researchers to work with partial regression models, but it also provides the intuitive content of multiple regression. For instrumental variables estimation of a linear regression model, the YFWL shows that if researchers are interested in the coefficients on the endogenous variables in the model, then they can estimate a partial regression model by residualizing the outcome variable, the endogenous variables and the set of instrumental variables on the exognenous variables in the model. By estimating a linear regression of the residualized outcome on the residualized endogenous variables using the residualized instrumental variables as the set of instruments, the researcher can recover consistent estimates of the endogenous variables in the original model. In situations where the estimator of the error vector is a function only of the regression residuals, she can also conduct proper statistical inference by using the covariance matrix (with a degrees of freedom correction in some cases) of the coefficient vector estimated from the partial regression. 
	...
	\begin{appendices}
		...
		\section{Proof of Theorem~\ref{thm:coef-resid}}
		
			\begin{proof}
			
			\textit{Coefficients are identical.} The instrumental variables estimator of $\beta_2$ in the full regression model (\ref{model-full-iv}) satisfies the linear equation system (\ref{eq-sys-m}), and the instrumental variables estimator of $\tilde{\beta}_2$ in the partial regression model (\ref{model-partial-iv}) satisfies the system of linear equations in (\ref{eq-sys-p}). Since $\widetilde{Y} = M_{W_1} Y$, $\widetilde{W}_2 = M_{W_1}W_2$, $\widetilde{Z}_2=M_{W_1} Z_2$, and $M_{W_1}$ is idempotent, we can immediately see that (\ref{eq-sys-m}) and (\ref{eq-sys-p}) are the \textit{same} equation systems. Hence, the solutions, when they exist, will also be identically the same. 
			
			In general, an infinite number of solutions of (\ref{eq-sys-m}) and (\ref{eq-sys-p}) will exist whenever the equation system is consistent, i.e. whenever the $k_3 \times 1$ vector $\widetilde{Z}_2 \widetilde{Y}$ belongs to the column space of the $k_3 \times k_2 $ matrix $Z'_2 \widetilde{W}_2$. If, in addition, the rank of $\widetilde{Z}_2 \widetilde{W}_2$ is equal to $k_2$, then there will exist a unique solution. In the case where there are an infinite number of solutions, the \textit{sets of solutions} of the moment equations for the full and partial regressions will be equal; in the case where there is a unique solution, we will have $b_2 = \tilde{b}_2$, i.e. coefficients from the full and partial regression models will be identical.
			
			\textit{Residuals are identical}. Let $u_M$ and $u_P$ denote the vectors of residuals from the instrumental variables estimation of the multiple and partial regression models (\ref{model-full-iv}) and (\ref{model-partial-iv}), respectively. Note two properties of the residual maker matrix $M_{W_1}$: first, that $M_{W_1}u_M = \left( I - W_1 \left(W'_1 W_1 \right)^{-1} W'_1\right) u_M = u_M$ because $W'_1 u_M=0$ (by the exogeneity of $W_1$); and, second, that $M_{W_1}W_1= \left( I - W_1 \left(W'_1 W_1 \right)^{-1} W'_1\right) W_1=0$ (because the vector of residuals from projecting each column of $W_1$ onto the column space of $W_1$ is the vector of zeros). 
			
			Consider the case when the system of equations (\ref{eq-sys-m}) and (\ref{eq-sys-p}) have unique solutions, so that $b_2 = \tilde{b}_2$. We can see immediately that residuals from the instrumental variables estimation of the multiple and partial regression models are the same because,
		...
		\section{Proof of Theoerm~\ref{thm:cov}}\label{app:cov}
		...
		\begin{proof}
			...
			\end{pmatrix}
			\] 
			where $\widetilde{X}_2 = \left[  I - X_1 \left( X'_1 X_1\right)^{-1} X'_1\right] X_2 $, and the ($1,1$) block on the RHS, $*$, is left unspecified because it is not needed for the computations. Now, let $X_1 = P_Z W_1$ and $X_2 = P_Z W_2$, so that $P_Z W = \left[P_Z W_1 : P_Z Z_2\right] $. I will now show that $\widetilde{X}_2 = P_{\widetilde{Z}_2} \widetilde{W}_2$ in two steps and that will complete the proof. In the first step, I will show that $\widetilde{X}_2=P_Z \widetilde{W}_2$; in the second step I will show that $P_Z \widetilde{W}_2 = P_{\widetilde{Z}_2} \widetilde{W}_2$. 
			...
			\begin{align*}
				...
				& = P_Z \left[  I - W_1 \left( W'_1 P_Z W_1\right)^{-1} W'_1 P_Z \right]  W_2  = P_Z \left[  I - W_1 \left( W'_1 W_1\right)^{-1} W'_1 \right]  W_2 = P_Z M_{W_1} W_2 = P_Z \widetilde{W}_2, 
			\end{align*}
		...
		%The main advantage of this procedure is computational. If a researcher were to estimate (\ref{model-full-sample}) and compute the covariance matrix of the full coefficient vector $b$, then she would have to program (\ref{hc2-full}), which would involve computing the inverse of a $k \times k$ matrix, $W'W$. If, instead, the partial regression-based procedure is used instead, then the researcher would: (a) compute $h_{ii}$ using (\ref{inv-part}), which involves inverses of $k_1 \times k_1$ and $k_2 \times k_2$ only; and (b) implement (\ref{hc2}), which involves computing the inverse of the $k_1 \times k_1$ matrix, ${W^*_1}^{'} W^*_1$. If $k_1$ and $k_2$ are much smaller than $k=k_1+k_2$, then there might be a significant reduction in computational burden. 
...
%		For instance, for HC3 and HC4 variants of heteroskedasticity consistent covariance matrices, steps~1, 2 and 3 would remain unchanged; in step~4, she would use either
%			\begin{equation}\label{hc3}
%					\left( {W^*_1}^{'} W^*_1\right)^{-1} {W^*_1}^{'} \frac{\text{diag}\left[ \tilde{u}_i^2\right]}{\left( 1-h_{ii}\right)^2 } {W^*_1} \left( {W^*_1}^{'} W^*_1\right)^{-1};
%				\end{equation} 
%		for HC3, or,
%			\begin{equation}\label{hc4}
%					\left( {W^*_1}^{'} W^*_1\right)^{-1} {W^*_1}^{'} \frac{\text{diag}\left[ \tilde{u}_i^2\right]}{\left( 1-h_{ii}\right)^{\delta_i} } {W^*_1} \left( {W^*_1}^{'} W^*_1\right)^{-1},
%				\end{equation} 
%		for HC4, where $\delta_i = \max \left( 4, N h_{ii}/k\right) $ and $k$ is the total number of regressors in (\ref{model-full-sample}), including the constant.
