...
\begin{equation}\label{eq:mainresult}
    ...
    \begin{aligned}
    &\Big[\frac{1}{\sqrt{2}}+O\left(\frac{k}{e^{S(E)}}\right)\Big]&  (k&\ll e^{S(E)})\\
    &\Big[\frac{1}{\pi}\sqrt{\frac{3}{2}+\log \frac{\Delta E}{a}}+O\left(\frac{e^{S(E)}}{k}\right)\Big]&  (k&\gg e^{S(E)})
    \end{aligned}
    \right..
\end{equation}
...
The black hole Hilbert space dimension controls the $k\gg S(E)$ regime of the Page curve. Our result above implies that \emph{its} fluctuation dominates over the intrinsic fluctuations in the Page curve in this regime. Specifically, the lower part of Eq.~\eqref{eq:mainresult} indicates that $\delta[e^{S(E)}]=\sqrt{\frac{3}{2}+\log \frac{\Delta E}{a}}/\pi$ to leading order, which depends logarithmically on the width $\Delta E$ of the microcanonical window. 
...
\begin{equation}
    ...
    \begin{aligned}
    &~\,0 ~~ +O(k^{-1})&  (k&<e^{S(E)})\\
    & \frac{1}{\pi}\sqrt{\frac{3}{2}+\log \frac{\Delta E}{a}}+O\left(\frac{e^{S(E)}}{k}\right)+O(e^{-S(E)})&  (k&>e^{S(E)})~.
    \end{aligned}
    \right.
\end{equation}
...
\end{equation}
This is a toy model for the semiclassical state of a black hole whose interior is entangled with Hawking radiation. Note that the state (\ref{eq:state}) is normalized only in the ensemble-averaged sense: $\mathbb{E}[\langle\Psi|\Psi\rangle]=1$, but $\mathbb{E}[\langle\Psi|\Psi\rangle^2]\neq 1$. The fluctuation of the normalization is not relevant here, but we will need to take it into account in the next section.
...
\begin{equation}
    ...
    h(E,\mu):=\frac{|\Gamma(\mu-1/2+i\sqrt{2E})|^2}{2^{2\mu-2}}.
\end{equation}
...
\begin{equation}\label{eq:Pagecurve}
    S_{\mathbf{R}}=\left\{
    \begin{aligned}
    &\log k-\frac{k}{2e^{S(E)}}+O(k^{-2}) &  (k&<e^{S(E)})\\
    &S(E)-\frac{e^{S(E)}}{2k}+O(e^{-2S(E)}) &  (k&>e^{S(E)}).
    \end{aligned}
    \right.
\end{equation}
Note that (\ref{eq:Pagecurve}) is identical to the entanglement entropy of random state at large dimensions \cite{Page:1993df}\cite{Foong:}. The rank of the reduced density matrix is
...
In the following, we will evaluate $\hat{R}(\lambda_1,\lambda_2)$ diagrammatically. To avoid overcounting, it is convenient to consider the quantity
\begin{equation}
...
Non-tubular wormholes consist of re-connections of EOW branes without containing double trumpet geometries nor handles. We will evaluate the contributions to the entropy fluctuation from these non-tubular wormholes. We denote contributions from non-tubular wormholes by a superscript, such as $D(\lambda_1,\lambda_2)^{\text{Non-tubular}},~(\delta S_{\mathbf{R}})_{\text{Non-Tubular}}$.
...
\end{eqnarray}
The integral of 
$D(\lambda_1\lambda_2)_{\text{unnormalized}}$ in (\ref{eq:nontubularexp}) needs to be performed analytically in order to regulate a divergence coming from $1/(x_1-x_2)^2$. When $n=n_1=n_2$, this integral is an integral of a symmetric polynomial with Selberg density. 

We are able to evaluate this integral in complete generality for certain values of $n$.  For example by explicitly expanding (\ref{eq:relationtorandomstate}), we find the Renyi entropy fluctuations
\begin{equation}
...
\end{equation}
Using the equivalence to the random state, we will show in section \ref{section:random state} that the rank of $\rho_{\mathbf{R}}$ has no fluctuations from non-tubular wormholes:
\begin{equation}\label{eq:rank}
    (\delta\text{Tr}[\hat{\rho}_{\mathbf{R}}^{0+}])^{\text{Non-Tubular}}
    =0~\text{for all}~k.
\end{equation}

To compute the von Neumann entropy fluctuation $\delta S_{\mathbf{R}}$, we need to consider arbitrary $n$, which we are able to do for small and large values of $k/e^{S(E)}$.\footnote{A simpler but related integral was studied in \cite{Forrester:2012} near $k\sim e^{S(E)}$ (see also \cite{Forrester:2004}). Perhaps a similar method can be used to study the behavior of the entropy fluctuation near $k\sim e^{S(E)}$.} 
...
\label{eq:UUtubular}
    ...
    -
    R(\lambda_1)
    \frac{1}{\bar{\lambda}}\sum_{m=1}^{\infty}\frac{kZ^{(1,m)}_{\text{Double Trumpet}}R(\lambda_2)^{m}}{m(kZ_{\text{Disk}}^{(1)})^{n_2+1}}.\nonumber\\
\end{eqnarray}
...
\begin{equation}\label{eq:renyifluctuationtubular}
    ...
    \begin{aligned}
    &\log (e^{\frac{3}{2}}\frac{\Delta E}{a})\frac{(n-1)(m-1)}{4\pi^2 e^{4S(E)}k^{n+m-4}}+O(k^{-(n+m)})&  (k&<e^{S(E)})\\
...
\begin{equation}\label{eq:entropyfluctuationtubular}
    ...
    \begin{aligned}
    &\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}\frac{k}{2\pi e^{2S(E)}}+O(k^{-2})&  (k&<e^{S(E)})\\
    &\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}\frac{1-\frac{e^{S(E)}}{2k}}{\pi e^{S(E)}}+O(e^{-2S(E)})&  (k&>e^{S(E)}).
    \end{aligned}
    \right.
\end{equation}
...
\begin{equation}\label{eq:rankfluctuationtubular}
    ...
    \begin{aligned}
    &0+O(k^{-1})&  (k&<e^{S(E)})\\
    &\frac{\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}}{\pi}+O(e^{-S(E)})&  (k&>e^{S(E)}).
    \end{aligned}
    \right.
\end{equation}
Thus the fluctuation of the rank undergoes a phase transition and becomes non-zero only after the Page time.
...
Summing up the two kinds of wormhole contributions, we obtain the total fluctuations of the entropy and of the rank of the Hawking radiation state. 
\begin{equation}\label{eq:summary1}
    \delta S_{\mathbf{R}}
    =\left\{
    \begin{aligned}
    &\frac{1}{\sqrt{2}e^{S(E)}}\Big(1+O(ke^{-S(E)})\Big)&  (k&\ll e^{S(E)})\\
    &\frac{\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}}{\pi e^{S(E)}}\Big(1+O(e^{S(E)}k^{-1})\Big)&  (k&\gg e^{S(E)})
    \end{aligned}
    \right.,
\end{equation}
\begin{equation}\label{eq:summary2}
    ...
    \right.
\end{equation}
The fluctuation of $\text{Tr}[\rho_{\mathbf{R}}^n]$ is
\begin{equation}\label{eq:summaryforrenyi}
    \delta \text{Tr}[\rho_{\mathbf{R}}^n]
    =\left\{
    \begin{aligned}
    &\frac{n(n-1)}{\sqrt{2}k^{n-1}e^{S(E)}}\Big(1+O(k^{-1})\Big)
    &  (k&\ll e^{S(E)})\\
    &\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}\frac{n-1}{\pi e^{nS(E)}}\Big(1+O(e^{-S(E)})\Big)&  (k&\gg e^{S(E)})
    \end{aligned}
    \right..
\end{equation}

As noted in the introduction, the entropy fluctuation (\ref{eq:summary1}) is exponentially suppressed in the black hole entropy at all times. At first this is surprising: like the Page curve itself, the fluctuations should be symmetric under exchange of the Hilbert space dimension of the two subsystems. 
...
Once the black hole becomes the smaller subsystem --- after the Page time --- its Hilbert space size controls both the Page curve and its fluctuations. The entropy is approximately given by the log of the rank in this regime. The nonzero rank fluctuation (\ref{eq:summary2}) in the microcanonical ensemble dominates over the much smaller entropy fluctuation expected if both Hilbert space dimensions were fixed.

In the next section, we compare this result with the case of a random state of a bipartite quantum system with both subsystem dimensions fixed. We will find an answer that is symmetric under interchange of subsystems, and which agrees with (\ref{eq:summary1}) before the Page time.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Average Entropy Fluctuation in a Fixed Bipartite System}\label{section:random state}

In this section, we consider a nongravitational, standard quantum-mechanical bipartite system, consisting of subsystems with fixed Hilbert space dimensions $m$ and $n$. In Sec.~\ref{subsection:matrixintegral}, we will compute the Haar average, over pure bipartite states, of the fluctuations of the subsystem entropy. We will find that they agree with the radiation entropy fluctuations computed in the previous section, before the Page time. We will find a discrepancy after the Page time, which we discuss in Sec.~\ref{comparison}. It can be understood in terms of fluctuations in the black hole state preparation. In Sec.~\ref{oldbound}, we compare our result to a known upper bound on entropy fluctuations; we find that they are much smaller than allowed by the bound.

\subsection{Calculation}\label{subsection:matrixintegral}
...
\end{equation}
which is identical to (\ref{eq:relationtorandomstate}) of the microcanonical PSSY model restricted to non-tubular wormholes after identifying $n=e^{S(E)}$ and $m=k$. We thus can conclude that the random state model in this subsection has the identical entanglement spectrum and fluctuation as the microcanonical PSSY model without tubular wormholes. Thus, we have
\begin{equation}
    \delta \text{Tr}[\rho^r]
    =\left\{
    \begin{aligned}
    &\frac{r^2(r-1)^2}{2m^{2r-2}n^{2}}\Big(1+O(mn^{-1})\Big)
    &  (m&\ll n)\\
    &\frac{r^2(r-1)^2}{2n^{2r-2}m^2}\Big(1+O(nm^{-1})\Big)&  (m&\gg n)
    \end{aligned}
    \right..
\end{equation}
Finally, we find
\begin{equation}\label{eq:ntu}
    ...
    =\left\{
    \begin{aligned}
    &\frac{1}{\sqrt{2}n}\Big(1+O(mn^{-1})\Big)&  (m&\ll n)\\
    &\frac{1}{\sqrt{2}m}\Big(1+O(nm^{-1})\Big)&  (m&\gg n).
    \end{aligned}
    \right.
\end{equation}

A byproduct of the equivalence above is that the fluctuation of the rank must be zero in the PSSY case without tubular wormhole at large $e^{S(E)}$, $k$ limit, thus proving (\ref{eq:rank}). To show (\ref{eq:rank}), it is sufficient to note that the rank fluctuation is zero in the random state, by the equivalence.

The absence of the rank fluctuation in the random state is a general feature not restricted to the Gaussian approximation employed here. This is simply because, in the space of pure states on $H=\mathbb{C}^m\otimes\mathbb{C}^n$, the subspace of states with rank strictly smaller than $m$ has lower dimensionality; thus its measure is zero and therefore the fluctuation is also zero.
...
\subsection{Comparison to the PSSY Model}
\label{comparison}
...
The nonzero rank fluctuation after the Page time tells us that the number of states in the given energy window fluctuates by an additive constant of order one, between different theories. (This is hardly surprising.) In the entropy fluctuation, after the Page time, the rank fluctuation contributes and dominates over the much smaller contribution from the random state. To see this, substitute (\ref{eq:summary2}) into (\ref{eq:Pagecurve}); this yields (\ref{eq:summary1}).
...
\subsection{Comparison to a Known Bound}\label{oldbound}

As shown by Page~\cite{Page:1993df, Foong:}, the Haar average of the bipartite entanglement is nearly maximal. Moreover, for $3\leq m\leq n$, the probability for a fluctuation by more than $\delta$ from the Page curve is exponentially suppressed in the total Hilbert space dimension $mn$ \cite{Patrick2006, Popescu:2006}:
\begin{equation}\label{eq:probability}
    \text{Pr}\left(|S(\rho)-\mathbb{E}[S(\rho)]|\geq \delta\right)\leq 2\exp\left(-\frac{mn\delta^2}{36\pi^3(\log m)^2}\right),
\end{equation}
This result implies an upper bound for the Haar average of the entropy fluctuation of a pure state. This can be shown by taking
\begin{eqnarray}
    \mathbb{E}\Big[|S(\rho)-\mathbb{E}[S(\rho)]|^2\Big]
    &\leq&
    \sum_{a=0}^{\infty}\text{Pr}
    \Big[
    \delta_k\leq|S(\rho)-\mathbb{E}[S(\rho)]|\leq\delta_{k+1}\Big]
    \delta_{k+1}^2
    \nonumber\\&\leq& 
    \sum_{k=0}^{\infty}\text{Pr}\Big[\delta_k\leq|S(\rho)-\mathbb{E}[S(\rho)]|\Big]\delta_{k+1}^2,
\end{eqnarray}
 for $\delta_k^2:=kmn/(36\pi^2(\log m)^2)$, and bounding the last line from above by an integral. One finds 
\begin{equation}\label{eq:Hayden}    
    \delta S(\rho)\leq\frac{(6\pi)^{3/2}\log m}{\sqrt{mn}}.
\end{equation}

Surprisingly, our result (\ref{eq:ntu}) is far smaller than this upper bound, though it is restricted to the case with large Hilbert space dimensions. If the connected parts of higher point correlation function of the entropy are sufficiently suppressed, so that we can approximate in terms of a Gaussian distribution, by applying the Chernov's bound, we could obtain $\text{Pr}(|S(\rho)-\mathbb{E}[S]|>u)\lesssim \exp(-u^2n^2)$ for $m\ll n$ and $\text{Pr}(|S(\rho)-\mathbb{E}[S]|>u)\lesssim \exp(-u^2m^2)$ for $n\ll m$. Such inequality could significantly tightens the upper bound (\ref{eq:probability}), if it were to hold. We leave this question for future investigation.

Since we now know that the non-tubular wormhole contribution is identical to the random pure state, we can establish a bound of the entropy fluctuation of the PSSY model for arbitrary but large $k$ and $e^{S_0}$ as
\begin{eqnarray}
    \delta S(\rho)=\left\{
    \begin{aligned}
    &\frac{(6\pi)^{3/2}\log k}{\sqrt{ke^{S_0}}}+\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}\frac{k}{2\pi e^{2S(E)}}&  (k&<e^{S(E)})\\
    &\frac{(6\pi)^{3/2}S_0}{\sqrt{ke^{S_0}}}+\frac{\sqrt{\log (e^{\frac{3}{2}}\frac{\Delta E}{a})}}{\pi e^{S(E)}}&  (k&>e^{S(E)}).
    \end{aligned}
    \right.
\end{eqnarray}
This inequality is weaker than (\ref{eq:summary1}) for $k\ll e^{S_0}$ or $k\gg e^{S_0}$, but it is applicable to intermediate $k$. It would be interesting to understand whether there is similar improvement of the bound for intermediate $k$.

%\subsection{Canonical Typicality}

We close with some implications and speculative remarks. (\ref{eq:ntu}) can be regarded as a manifestation of canonical typicality\cite{canonical,Popescu:2006}: the reduced density matrix of a random pure state of a small subsystem is close to the averaged reduced density matrix. The latter is nearly maximally mixed due to its entanglement. 

Our result (\ref{eq:ntu}) implies that the typical state $|\psi\rangle$ on $H_A\otimes H_B$ with $1\ll d_A\ll d_B$ fluctuates as
\begin{equation}\label{eq:ETHforentropy}
    S(\text{Tr}_B|\psi\rangle\langle \psi|)=\mathbb{E}[S_{A}]+\frac{1}{d_B}R_i,
\end{equation}
where $R_i$ is a real random variable with zero average with $O(1)$ variance. It quantifies the fluctuation of the random state in terms of entanglement entropy. We note that this result is not a trivial consequence of the canonical typicality, because the combination of the Fannes inequality and $\mathbb{E}[|\rho_A-\mathbb{E}[\rho_A]|_1]\leq\sqrt{d_A/d_B}$ \cite{Popescu:2006} yields an estimate for the upper bound of the entropy fluctuation proportional to $\frac{d_A}{d_B}\log d_A$. We can also obtain an independent upper bound for the probability of the entropy fluctuation using a method given in the appendix of \cite{Cotler:2020jtk}. Indeed, since we have $-\log d_A\leq-\log\text{Tr}[\rho_A^2]\leq S_A$ in general, we can apply Markov's inequality to the non-negative variable $d_A\text{Tr}[\rho_A^2]-1$ to obtain $\text{Pr}[S\leq\log d_A-\delta]\leq \text{Pr}[e^{\delta}-1\leq d_A\text{Tr}\rho_A^2-1]\leq(d_A\langle\text{Tr}[\rho_A^2]\rangle-1)/(e^{\delta}-1)\approx(d_A/d_B)/(e^{\delta}-1)$, where we assumed large $d_A,d_B$ at the last approximate equality. Thus we obtain $\mathbb{E}[(S_A-\log d_A)^2]\lesssim 2\zeta(3)\,d_A^2/d_B^2$. This bound constrains the fluctuation around the average when $d_A\ll d_B$.

It would be interesting to relate our result (\ref{eq:ETHforentropy}) to the properties of energy eigenstates, as in the case of the eigenstate thermalization hypothesis (ETH) \cite{Deutsch:1991, Srednicki:1994, Rigol, singleigenstate, Dymarsky:,Lashkari:2016vgj}. The ETH states that any few-body operator $O$ and the majority of energy eigenstates $|E_i\rangle$ satisfy,  
\begin{equation}
    \langle E_i|O|E_j\rangle=O(E)^{\text{micro}}\delta_{ij}+e^{-S(\bar{E})/2}f(\bar{E},E_i-E_j)R_{ij},
\end{equation}
where $\bar{E}=(E_i+E_j)/2$, $f$ is a smooth real $O(1)$ function, and $R_{ij}$ is a random variable with unit variance and zero mean. This hypothesis is strong enough to guarantee the thermalization of the few-body operator for any initial state. The ETH for few-body operators reduces to the random state prediction when we take a sufficiently small energy window, whose width is given by the Thouless energy \cite{DAlessio:2015qtq}. This is accomplished by assuming the unitary transformation matrix between energy eigenstates and eigenstates of $O$ as a random matrix. The ETH for small subsystem was proposed in \cite{singleigenstate,Lashkari:2016vgj, Dymarsky:}. Although our result does not apply at all to energy eigenstates, it is amusing to make a formal comparison and to find stronger suppression in the entropy fluctuation compared to that of the few-body operator. Such stronger suppression were considered previously in \cite{Behemoths}. It would be interesting to study the subsystem entropy fluctuation in realistic systems and compare it with (\ref{eq:ETHforentropy}).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\paragraph{Acknowledgements}
We are grateful to Douglas Stanford for initial collaboration, and for generously sharing results on which subsection \ref{subsection:matrixintegral} is based. We thank L.~Iliesiu, Y.~Nakata, G.~Penington, and T.~Takayanagi for discussions and correspondence. RB was supported in part by the Berkeley Center for Theoretical Physics; by the Department of Energy, Office of Science, Office of High Energy Physics under QuantISED Award DE-SC0019380 and under contract DE-AC02-05CH11231; and by the National Science Foundation under Award Number 2112880.
...
\begin{equation}
    \int_{-\infty}^{\infty}dle^{l}~\psi_{\text{Disk}}(\beta_1,l)\psi_{\text{Disk}}(\beta_2,l)
    =
...
where we used
\begin{equation}
    \int_{-\infty}^{\infty}dl~K_{i\sqrt{8E}}(4e^{-l/2})K_{i\sqrt{8E'}}(4e^{-l/2})=\frac{\delta(E-E')}{16D_{\text{Disk}}(E)}.
\end{equation}
The path integral of a disk with $n$ geodesic boundaries with lengths $l_1,\cdots,l_n$ is
\begin{equation}
    I_{n}(l_1,\cdots,l_n)=2^{2n}\int_0^{\infty}dED_{\text{Disk}}(E)K_{i\sqrt{8E}}(4e^{-l_1/2})\cdots K_{i\sqrt{8E}}(4e^{-l_n/2}).
\end{equation}
This satisfies
\begin{equation}
    \int_{-\infty}^{\infty}dl~I_{n+1}(l_1,...,l_n,l)I_{m+1}(l_1',...,l_m',l)=I_{n+m}(l_1,...,l_n,l_1',...,l_m'),
\end{equation}
\begin{eqnarray}
    &&    
    e^{l/2}\psi_{\text{Disk}}(x_1+\cdots+x_{n-1},l)
    \nonumber\\&=&
    \int_{-\infty}^{\infty}dl_1...dl_{n-1}~e^{\frac{l_1+...+l_{n-1}}{2}}I_{n}(l_1,...,l_{n-1},l)\psi_{\text{Disk}}(x_1,l_1)...\psi_{\text{Disk}}(x_{n-1},l_{n-1}).
\end{eqnarray}
...
\begin{eqnarray}
    &&\psi_{\text{Disk}}(l_1,...,l_n:x_1,...,x_n)
    \nonumber\\&=&
    \int_{-\infty}^{\infty}dl_1'...dl_n'~e^{(l_1'+\cdots+l_n')/2}I_{2n}(l_1,...,l_1',....)\psi_{\text{Disk}}(l_1',x_1)...\psi_{\text{Disk}}(l_n',x_n)
    \nonumber\\&=&
...
\begin{eqnarray}
    Z^{(n)}_{\text{Disk}}\left[\text{canonical}, \text{boundary lengths}=x_i\right]=e^{S_0}\int_0^{\infty}dED_{\text{Disk}}(E)h(E,\mu)^ne^{-(x_1+....+x_n)E},\nonumber\\
\end{eqnarray}
where
\begin{eqnarray}
    h(E,\mu):=4\int_{-\infty}^{\infty} dle^{l/2}K_{i\sqrt{8E}}(4e^{-l/2})e^{-\mu l}
    =\frac{|\Gamma(\mu-1/2+i\sqrt{2E})|^2}{2^{2\mu-2}},
\end{eqnarray}
...
\begin{eqnarray}
    Z(x:y)&=&\underset{d\rightarrow 2}{\lim}\int_0^{\infty}dEdE'D_{\text{Double Trumpet}}(E,E':d)e^{-xE-yE'}\nonumber\\
    &=&\frac{\sqrt{xy}}{2\pi(x+y)}.
\end{eqnarray}

Next we generalize to the partition function with an arbitrary number of AdS boundaries and geodesic boundaries anchored from them. The path integral for a trumpet with $n$ geodesic boundaries with length $l_i~(i=1,\cdots, n)$ and AdS boundary segments of length $x_a~(a=1,...,n)$ can be written as
\begin{eqnarray}
    &&T(l_1,...,l_n:x_1,...,x_n:b)\nonumber\\
    &=&
    \int_{-\infty}^{\infty}
...
\begin{eqnarray}
    Z_{\text{Trumpet}}^{(n)}(x_1,...,x_n:b)=\int_0^{\infty}dED_{\text{Trumpet}}(E,b)h(E,\mu)^ne^{-(x_1+...+x_n)E}.
\end{eqnarray}
