...
Much has been learned by investigating these distributions one parameter at a time, however, we can extract additional information by studying the covariance between pairs of parameters.
...
In this study, we test for the existence of this anti-correlation using updated data from the third gravitational wave transient catalogue (GWTC-3), and improving our copula-based framework to employ a more robust model for black-hole spins.
We find evidence for an anti-correlation in $(q, \chieff)$ with 99.8\% credibility.
...
\end{equation}
Here, $m_1$ and $m_2$ are the heavier and lighter component masses respectively, $\chi_{1,2}$ are their corresponding dimensionless spin magnitudes, and $\cos t_{1,2}$ are the tilts of each spin vector measured about the orbital angular momentum.
Using BBH events from the second gravitational wave transient catalogue \citep[GWTC-2;][]{GWTC2}, \cite{Callister_2021} measured such an anti-correlation with 98.7\% credibility.
...
\cite{Adamcewicz_2022} addresses this concern using a copula density function: a statistical tool that imposes a variable correlation $\kappa$ between two parameters, but keeps their corresponding marginal distributions fixed.
In spite of this concern, \cite{Adamcewicz_2022} corroborates the results of \cite{Callister_2021}, inferring an anti-correlation in $(q,\chieff)$ with 98.7\% credibility with GWTC-2 events, and demonstrating that the evidence for anti-correlation is not due merely to a better model for $\chieff$.\footnote{
...
Binary black hole spins are defined by six parameters, three for each black hole: the spin magnitudes of each black hole $\chi_{1,2}$, the spin tilts of each black hole $\cos t_{1,2}$, and the the azimuthal angles of each component $\phi_{1,2}$.
...
\begin{align}\label{eq:skew_norm}
    ...
    \left[1 + \text{erf}\left(a \frac{x - \xi}{\omega\sqrt{2}}\right)\right] \times \nonumber\\
    & \Theta(x+1) \Theta(1-x) .
\end{align}
...
\begin{equation}\label{eq:rho_dist}
    \pi(\rho_1, \rho_2|\Lambda) = \text{Beta}(\rho_1|\mu_\rho, \sigma_\rho^2) \text{Beta}(\rho_2|\mu_\rho, \sigma_\rho^2),
\end{equation}
...
With all of the marginal distributions defined, we employ a Frank copula density function in order to introduce a correlation between $q$ and $\chieff$:
...
With 90\% credibility, we find $\kappa = 26^{+21}_{-19}$ when GW191109 is excluded, and $\kappa = 29^{+19}_{-20}$ when it is included.
We rule out the uncorrelated hypothesis ($\kappa = 0$) with 99.8\% credibility.
This number varies only slightly (by $\lesssim 0.1\%$) when we vary the prior bounds on $\kappa$ or include GW191109.

The correlation hypothesis is preferred over the no-correlation hypothesis with a natural log Bayes factor of  $\ln \text{BF} = 2.8$ excluding GW191109 or $\ln \text{BF} = 3.1$ including GW191109.
Moreover, the correlation hypothesis yields a maximum natural log likelihood that is $\Delta \ln \mathcal{L}_{\max} = 5.9$ (8.6) higher than the no-correlation hypothesis including (excluding) GW191109, suggesting that the data are better fit by the correlation model.
...
Superimposed over this distribution, is a set of weighted delta functions \sout{$\pi$}, which maximise the population likelihood, taken from \cite{Payne_2023}.
...
\cite{Ma_2023} predict that tidal spin-up rate should scale with $q^2$.
...
We thank Maya Fishbach, Charlie Hoy, and Christopher Berry for their helpful comments on this manuscript.
...
Mass ratio is then power-law distributed with a dependence on $m_1$:
\begin{equation}
...
\begin{equation}
    \mathcal{L}(d|\Lambda,\kappa,\lambda_0) = \frac{1}{p_\text{det}(\Lambda)} \prod_i^N \Big[
    \lambda_0 \mathcal{L}_0(d^i|\Lambda) +
    (1 - \lambda_0) \mathcal{L}_s(d^i|\Lambda, \kappa) 
    \Big].
\end{equation}
Here, the factor of $p_\text{det}(\Lambda)^{-1}$ accounts for mass and redshift-based effects on BBH detection efficiency \citep[see][]{Messenger_2013, Thrane_2019, Mandel_2019, GWTC1_analysis, GWTC2_analysis, GWTC3_analysis}.
...
These are the Frank copula density funtion, Gaussian copula density function, and Farlie-Gumbel-Morgenstern (FGM) copula density function.
...
A Gaussian copula density function is given as
\begin{equation}
...
With a Gaussian copula, we find anti-correlation ($\kappa < 0$) with 97.8\% credibility.
Compared to the Frank copula model, the Gaussian copula model is disfavoured by a natural log Bayes factor of $\ln \text{BF} = 1.2$.
Alternatively, the maximum natural log likelihood of the Frank copula model is $\Delta \ln \mathcal{L}_{\max} = 2.5$ higher.
This implies that the Gaussian copula model is still favoured over the no-correlation hypothesis by a natural log Bayes factor of $\ln \text{BF} = 1.6$ ($\Delta \ln \mathcal{L}_{\max} = 3.4$).
...
This feature likely explains the data's preference for the Frank copula model.
...
With an FGM copula, we find anti-correlation ($\kappa < 0$) with 82.1\% credibility.
Again comparing to the Frank copula model, the FGM model is disfavoured by a natural log Bayes factor of $\ln \text{BF} = 2.6$ (difference in maximum natural log likelihood of $\Delta \ln \mathcal{L}_{\max} = 3.1$).
Hence, the FGM model is less distinguishable from the uncorrelated model, favoured by a natural log Bayes factor of only $\ln \text{BF} = 0.1$, but with a notable improvement in maximum natural log likelihood $\Delta \ln \mathcal{L}_{\max} = 2.8$.
