...
The MHD system with only magnetic diffusion models many significant phenomenon such as the magnetic reconnection in astrophysics and geomagnetic dynamo in geophysics (see, e.g., \cite{EPTF}). For more physical background, we refer to \cite{HC}, \cite{TGCDP}, \cite{DL}, \cite{LDEM}, \cite{ST}.
...
Let  $\alpha>0, \beta>0$ be  two arbitrarily fixed constants.
  For any  ${N}\ge (2\beta+3)r+\alpha+2\beta+5$ with $r>1$, and $(\u_0,\b_0)\in H^{N}(\T^2)$ with
...
\end{align}
For any ${N}\ge (2\beta+3)r+\alpha+2\beta+5$, we invoke the interpolation inequality
