...
\newcommand{\marcomm}[1]{\marginpar{\begin{flushright}#1\end{flushright}}}%
...
\newcommand{\Addresses}{{%
 ...
 \textit{E-mail address}, V.~Amato: \texttt{vincenzo.amato@unina.it} 
...
\]
as shown in \cite{HM1}, \cite{buchenmic}, and \cite{girkarlag}.
...
\end{equation}
This choice of $\mathcal{K}_c$ avoids shapes that could make $F$ degenerate, and precisely it could be shown, as in \cite{HM1}, that there exist two constants $c,C>0$ such that
\[
...
\begin{conj}[Henrot, Michetti \cite{HM1}]
\label{conje}
...
Indeed, following in the footsteps of \cite{HM1}, for such a family, 
...
\]
which represents the limit of a family of thinning triangles. Consequently, familiarizing oneself with the properties of $\mu_1(h)$ and $\sigma_1(h)$ can offer advantages when it comes to analyzing the eigenvalues $\mu_1(\Omega)$ and $\sigma_1(\Omega)$. It is worth mentioning that the quantities $\mu_1(h)$ and $\sigma_1(h)$ are in a way related to a weighted Hardy constant (see \cite{hardy}, \cite{opickuf}, \cite{payne}, and \autoref{prop: sigma=hardy}). 

Following this path, we refer to \cite{payne}, \cite{HM1} and \cite{troesch} for the proof of the subsequent properties: let 
\[
...
\label{prop: mu=sigma}
    ...
    \left(\int_0^1 h(t)\,dt\right)^2 \mu(h)=\sigma\left(\mathcal{G}(h)\right),
    \end{equation}
    and
    \begin{equation}
    \label{eq: sigmatoneum}
    \left(\int_0^1 \frac{1}{\sqrt{k(t)}}\,dt\right)^{2}\sigma(k)=\mu(\mathcal{G}^{-1}(k)).
    \end{equation}
\end{teor}
...
We refer to \cite{HM1} for the proof of the lemmas in this section.
...
Here we recall some other tools that will be useful in the next pages. We refer to \cite{maggi2012sets,ambrosio2000functions}.
...
\end{split}\]
admit solutions. In particular, the solution to the maximization problem (see for instance \cite{troesch}) is given by the parabola $p(x)=6x(1-x)$, with corresponding eigenvalue $\sigma_1(p)=12$. In this section, we aim to prove \autoref{teor: minstek}, namely that the problem
\[\min\Set{\sigma_1(h)\colon h\in\prof_1},\]
...
\begin{proof}
    ...
    \end{split}\]
    On the other hand, by the minimality of $\sigma_1(\Omega)$, we have 
\begin{align*}
...
for every $\varphi\in H^1(0,1)$, which yields $h=h_0$, since, for every $\psi\in L^2(0,1)$, we can choose 
\[\varphi(x)=\int_0^x \psi(t)\,dt.\] 
\end{proof}
In order to study the minimum problem \eqref{eq:minstek}, we need to evaluate $\sigma_1$ on triangles, and we will need the following result, whose proof can be found in \cite{HM1}. 
\begin{lemma}
\label{lem: defsigma}
...
Following classical arguments (see for instance \cite{ferone}) we have that $v$ vanishes in one and only one point $x_0\in(0,1)$, so that $w'$ vanishes only in $x_0$. Without loss of generality, we can assume that $w'$ is positive in $[0,x_0)$ and it is negative in $(x_0,1]$. Let now $\varphi\in H^1_0(0,1)$ be such that
...
\[
\mu_1(h)\ge \left(\int_0^1 h(t)\,dt\right)^{-2}\sigma_1(h).
\]
On the other hand, choosing $w=w_\sigma$ to be the eigenfunction of $\sigma_1(\mathcal{G}(h))$, we get 
\[
\mu_1(h)\le\left(\int_0^1 h(t)\,dt\right)^{-2}\sigma(\mathcal{G}(h)),
\]
