...
Robust optimization (RO) has become increasingly popular as a method to account for parameter uncertainty. Compared to more conventional methods such as stochastic optimization, which can be computationally intensive in high dimensions, RO offers a significant computational advantage \citep{bental_nemirov_ro, Bertsimasrobusttext, robustreview, robusttext}. 
...
Adaptive robust optimization (ARO) is an important extension of RO that allows certain decision variables, referred to as the wait-and-see variables, to be determined after the uncertainty is revealed. In ARO, the wait-and-see decisions are mathematically modeled as functions of uncertain parameters, enabling them to adapt to the realization of those parameters. ARO is particularly useful in multi-stage decision-making problems, where decision-makers may be uncertain about future parameter values, and where decisions may need to be made sequentially over time. Compared to RO, ARO provides greater modeling flexibility and often results in superior solutions that are better able to adapt to changing conditions \citep{Bertsimasrobusttext,survey,adaptivefirst}. Application areas include energy \citep{Sun2014Wind, BertsimasSun, energy}, inventory management \citep{inventory1, inventory2} , portfolio management \citep{portfolio} among many others \citep{survey}. 
 
 Despite its many benefits, ARO poses significant computational challenges that distinguish it from RO. One of the primary challenges arises from the fact that ARO is an infinite-dimensional optimization problem, as the wait-and-see variables are functions of the uncertain parameters. To overcome this issue, approximation methods have been proposed that restrict the wait-and-see variables to a limited set of functions, such as affine functions \citep{Bertsimasrobusttext, adaptivefirst}. However, while these methods may be able to reformulate ARO into RO, there is no guarantee that the resulting approximation will be near-optimal or even feasible \citep{robusttext, adaptivefirst}. Other methods have been developed that can ensure near-optimal or even optimal solutions for ARO, including Benders Decomposition \citep{BertsimasSun}, Column and Constraint Generation (CCG) \citep{CCG}, and Fourier-Motzkin Elimination \citep{adaptive}. These methods, however, may not scale well in high dimensions, limiting their practical use. For more information on the solution methods and their limitations see  \citep{Bertsimasrobusttext, robustreview, robusttext, survey, adaptivefirst, affine}. Given the substantial computational burden of ARO, the application of ARO may be limited particularly in real-time settings. To address this issue, we propose a novel method to significantly reduce the computational burden associated with ARO. 
 
 \citet{Voice, Prune, OPT} proposed a machine learning approach to solve mixed integer convex optimization (MICO) problems, exploiting the fact that similar optimization instances are solved repeatedly in many real life applications. Their approach is to train a machine learning model that predicts the optimal strategy of a MICO problem (we define optimal strategy for MICO in Section \ref{subsec:mlopt}). \citet{Voice, Prune} use classification algorithms, while \citet{OPT} use a prescriptive machine learning algorithm, Optimal Policy Trees (OPT) \citep{Policy}. When used to solve large scale MICO problems, this machine learning approach can substantially reduce the solve time compared to commercial solvers. We extend this approach to two-stage linear ARO with binary here-and-now variables and polyhedral uncertainty sets. 
 

The contributions of the paper are as follows.

\begin{enumerate}
    \item First, we propose a machine learning approach to solve two-stage linear ARO with binary here-and-now variables and polyhedral uncertainty sets. To generate a training set, we solve multiple ARO instances in advance using the CCG algorithm. Then, we train machine learning models to predict high-quality strategies for ARO problems, which results in significantly faster solve times compared to the state-of-the-art algorithms. The major difference of our approach from \citep{Voice, Prune, OPT} is that unlike MICO, ARO consists of infinite dimensional problems. Hence, it requires a different solution algorithm to generate training data, and also requires a different definition of the optimal strategies. Furthermore, in addition to predicting the here-and-now decisions, our approach also predicts the worst-case scenarios associated with the here-and-now decisions and the wait-and-see decisions. 
    \item Second, we propose a new method to reduce the number of distinct target classes the machine learning model is trained on. Depending on the sample generation scheme or the size of the uncertainty sets, the number of distinct target classes can be prohibitively large, making the prediction task difficult. Our proposed method helps overcome this challenge.
    \item Finally, we test our algorithm on synthetic and real-world examples. The examples we test on include the facility location, the multi-item inventory control and the unit commitment problems. We demonstrate that we can obtain high-quality solutions using the proposed method. Furthermore, although training may take hours to days, the on-line application speed is drastically faster than the state-of-the-art methods. 
\end{enumerate}

  


The structure of this paper is as follows. In Section 2, we briefly introduce ARO and explain how we solve a two-stage linear ARO problem with polyhedral uncertainty sets. We also demonstrate that this method may not scale to problems with high dimension. In Section 3, we first review the approach by \citet{Voice, Prune} to solve MICO problems using machine learning. Then we review \citep{OPT}, a prescriptive machine learning approach to MICO. In Section 4, we develop a machine learning approach to solve two-stage ARO with binary here-and-now variables and polyhedral uncertainty sets. In Section 5, we present an algorithm to reduce the number of different target classes the machine learning algorithm needs to be trained on. In Section 6, we present the results of numerical experiments. 

\paragraph{Notational Conventions} Throughout this paper, we use boldface letters to denote vectors and matrices. The $i_{th} $ entry of a vector $\bm{x}$ is denoted $x_i$, unless otherwise noted. 
...
&s.t.\quad &&  \bm{A}(\bm{d})\bm{x} + \bm{B}\bm{y} (\bm{d})\leq \bm{g},\quad \forall {\bm{d}\in\mathcal{D}}, \nonumber
\end{alignat}



 \noindent where $\bm{d}$ is a vector of uncertain parameters and $\mathcal{D}$ is a polyhedral uncertainty set. We also use the term scenario to refer to a specific realization of the uncertain parameter. $\bm{x}$ is the vector of here-and-now variables that represents the decisions that have to be made before the uncertainty is revealed.  $\bm{y}$ is the vector of wait-and-see variables, which is a function of $\bm{d}$. $\bm{A}(\bm{d})$ and $\bm{c}(\bm{d})$ are affine in $\bm{d}$. We assume $\bm{B}$ and $\bm{b}$ do not involve uncertainty.
 
  The wait-and-see variables represent the decisions that can be made after the uncertainty is revealed.  This flexibility leads to less conservative and more realistic solutions compared to RO.  ARO results in better objective values because the wait-and-see variables can be decided based on actual realizations of the uncertain parameters, whereas in RO, conservative decisions must be made in advance. Moreover, ARO tolerates larger uncertainty levels than RO. In some cases, a RO problem can be infeasible if the uncertainty set is too large. However, by switching some of the decision variables to wait-and-see variables, the resulting ARO problem may become feasible \citep{Bertsimasrobusttext, robusttext, survey, adaptivefirst}.
...
Since a worst-case scenario is one of the extreme points of the uncertainty set, problem \eqref{2} is equivalent to the following problem:
\begin{alignat}{2}
&\underset{\bm{x}, \bm{\alpha}, \{\bm{y_{\bm{d}}}\}} \min\quad && \alpha \label{4} \\
&s.t. && \alpha \geq \bm{c}(\bm{d})^\top \bm{x} + \bm{b}^\top \bm{y}_{\bm{d}} , \quad \forall  \bm{d} \in \mathcal{E}, \nonumber \\
...
Without using the entire set $\mathcal{E}$, CCG solves \eqref{4} by iteratively adding a new extreme point $\bm{d}$ and the associated wait-and-see variable $\bm{y}_{\bm{d}}$ until the quality of the solution is good enough.  Iteration $i$ of the CCG algorithm involves $i$ extreme points identified so far. Denoting the set of extreme points at iteration $i$ as $\mathcal{E}_i$, the so-called restricted master problem at iteration $i$ is
\begin{alignat}{2}
&\underset{\bm{x}, \bm{\alpha}, \{\bm{y}_\ell\}}\min\quad &&\alpha  \label{5}\\
&s.t. && \alpha \geq \bm{c}(\bm{d})^\top \bm{x} + \bm{b}^\top \bm{y}_{\bm{d}}, \quad \forall  {\bm{d} \in\ \mathcal{E}_i}, \nonumber\\
...
&s.t. && -\bm{\pi}^\top \bm{B} = \bm{b}^\top, \nonumber\\
&\ && \bm{\pi} \geq \bm{0}, \nonumber\\
& \ && \bm{d} \in \mathcal{D}. \nonumber
\end{alignat}

% Note that the constraints on $\bm{d}$ and $\bm{\pi}$ are disjoint because of the fixed-recourse property. 
The Alternating Direction method to solve problem \eqref{6} is described in Algorithm \ref{alg2}. For conciseness, we use $\mathcal{T} = \{\bm{\pi} \mid \-\bm{\pi}^\top \bm{B} = \bm{b}^\top, \bm{\pi} \geq 0\}$ in the algorithm description. Theoretically, CCG can output suboptimal solutions precisely because problem \eqref{3} is non-convex. Hence, in each iteration, we are in fact computing an approximation of $\mathcal{Q}(\bm{x}_i)$, which we denote as $\tilde{\mathcal{Q}}(\bm{x}_i)$. To ensure the quality of the solution, in our implementation we try three different initial points for problem \eqref{3} and choose the best solution found. Empirical evidence also shows that the solutions found by Algorithm \ref{alg2} are very often near-optimal \citep{Sun2014Daily}. For more detail on CCG method and the Alternating Direction method see \citep{Sun2014Wind, Sun2014Daily}. Algorithms \ref{alg1} and \ref{alg2} describe the CCG and the Alternating Direction method, respectively. 

\begin{algorithm}
 \KwInput{Problem \eqref{1}, $\epsilon_1$}
\KwOutput{$\tilde{\bm{x}}^*$,$\tilde{\bm{d}}^*$} 
 \textbf{Initialization}: $i = 0$, $\mathcal{E}_0 = \emptyset$, UB = $\infty$, LB = $-\infty$\\
 \While{UB -LB $\geq \epsilon_1$}{
  Solve \eqref{5} with the extreme points in $\mathcal{E}_i$. Denote the solutions as $\bm{x}_i$ and $\bm{\alpha}_i$. \\
...
\section{A Machine Learning Approach to Deterministic Optimization}
\label{sec:3}

In this section, we review the approach by \citet{Voice, Prune} to solve MICO problems using machine learning. We also review an extension of this approach by \citet{OPT} to use a prescriptive machine learning algorithm, OPT, instead of classification algorithms.

\subsection{Solving MICO problems with Classification Algorithms}
\label{subsec:mlopt}

Consider the MICO problem
\begin{alignat*}{3}
&\min\quad &&  f(\bm{x},\bm{\theta}) \\
&s.t.\quad && {g}_1(\bm{x},\bm{\theta}) \leq \bm{0}, \nonumber \\
&\quad && \vdots \nonumber \\
&\quad && {g}_m(\bm{x},\bm{\theta}) \leq \bm{0}, \nonumber \\
&\quad && \bm{x}_{\mathcal{I}} \in \{0,1 \}^{d} , \nonumber
\end{alignat*}

 \noindent where $\bm{x} \in \mathbb{R}^n$ is the vector of decision variables and $\bm{\theta}$ is the vector of key parameters. $\mathcal{I}$ is the set of indices for the decision variables constrained to take binary values, where $|\mathcal{I}| = d$. The functions $f$ and ${g}_j, j \in [m]$ are assumed to be convex in $\bm{x}$. 
 
The approach consists of three phases. In Phase 1, we generate a training set by solving $N$ instances each associated with parameter $\bm{\theta}_i, i \in [N]$. Every time we solve the instance associated with a parameter ${\bm{\theta}}_i$, we identify the optimal solution $\bm{x}^*({\bm{\theta}}_i)$ and the set of constraints that are satisfied as equality at optimality. We call these constraints the tight constraints, denoted as $\tau({\bm{\theta}}_i)$, with the following formal definition.
\[ \tau({\bm{\theta}}_i) = \{j \in [m] \mid g_j(\bm{x^*}(\bm{{\theta}}_i),{\bm{{\theta}}_i}) = 0\}. \]

\noindent The optimal integer variable and the tight constraints define the optimal strategy $s({\bm{\theta}}_i) =(\bm{x}_\mathcal{I}^*(\bm{{\theta}}_i), \tau({\bm{\theta}}_i))$. 


In Phase 2, we train a classification model using the vectors $\{\bm{\theta}_i\}_{i=1}^{N}$ as the features and the optimal strategies $\{s(\bm{\theta}_i)\}_{i=1}^{N}$ as the prediction targets. 

In Phase 3, given a new parameter $\hat{\bm{\theta}}$, the trained model predicts a strategy $\hat{s}(\hat{\bm{\theta}})$. Using this strategy, we solve the instance associated with $\hat{\bm{\theta}}$ and get a solution $\hat{\bm{x}}(\hat{\bm{\theta}})$. The computational burden is substantially reduced since integrality and redundant constraints are no longer imposed. \citet{Prune} show that large dimensional MICO problems can be solved in just milliseconds.

To evaluate the quality of the strategy $\hat{s}(\hat{\bm{\theta}})$ applied to the instance associated with $\hat{\bm{\theta}}$, we assess feasibility of $\hat{\bm{x}}(\hat{\bm{\theta}})$ and also compare it with the optimal solution $\bm{x}^*(\hat{\bm{\theta}})$. If $\hat{\bm{x}}(\hat{\bm{\theta}})$ is feasible, we define its suboptimality as
 \begin{equation*}
sub \Big(\hat{s}(\hat{\bm{\theta}}) \Big) = \Bigg(f\Big(\hat{\bm{x}}(\hat{\bm{\theta}}), \hat{\bm{\theta}} \Big) - f\Big(\bm{x}^*(\hat{\bm{\theta}}), \hat{\bm{\theta}} \Big) \Bigg) / \abs{f\Big(\bm{x}^*(\hat{\bm{\theta}}), \hat{\bm{\theta}} \Big)},
\end{equation*}
% \subsection{Strategy Exploration}

% Strategy exploration enables us to estimate the probability of encountering an unseen strategy. This approach can help us decide how many samples to generate. By using Good-Turing estimator \citep{good},   \citep{Voice} 
%  estimate the probability of encountering an unseen strategy given the current number of samples and the number of strategies to which only a single sample is assigned. 

\subsection{Strategy Pruning}

As the problem dimension gets higher, the number of distinct strategies in the training set can get prohibitively large, making the classification task difficult. Pruning algorithm described in \citep{Prune} enables us to reduce this number with only a small compromise of performance. The idea of pruning stems from the fact that many strategies are in fact redundant, either because of the existence of multiple global optima or the existence of near-optimal strategies that can replace the rarely occurring strategies. In addition, the majority of the instances are usually paired with relatively small number of strategies. The pruning algorithm aims to replace the rarely occurring strategies with the strategies that occur most frequently. It tries to assign all the instances to a small set of strategies, denoted as $M(\alpha)$, where $0 < \alpha < 1$. $M(\alpha)$ is the set of most frequent strategies that account for at least $(1-\alpha)$ fraction of the instances in the data set. If there exists an instance that cannot be reassigned to $M(\alpha)$, we decrease $\alpha$ and try to reassign the instance again. We iterate until we find a $\alpha$ 
so that all the instances can be assigned to $M(\alpha)$.




\subsection{A Prescriptive Machine Learning Approach} \label{sec3.5}

 
 \citet{OPT} use a prescriptive machine learning algorithm, OPT, instead of classification algorithms to solve MICO problems. OPT, an optimal-tree based prescriptive algorithm, learns a decision tree that assigns a decision to a feature vector to optimize certain outcome. OPT requires a training set  $\{(\bm{\theta}_i, s_i, y_i)\}_i^{N}$, where $\bm{\theta}_i$ is the feature, $s_i$ is the decision applied and ${y}_i$ is the outcome related to point $i$. Using this data, OPT aims to find a globally optimal decision tree that assigns the same decision to the points in the same leaf. Mathematically, OPT solves the following optimization problem
\begin{align} \label{opt}
&\underset{v(\cdot),\bm{z}} \min \quad  \sum_{i=1}^{N}\sum_{\ell}\mathbbm{1}\{v(\bm{\theta}_i) = \ell\}{\cdot}R_{iz_{\ell}},
\end{align}
\noindent where $v(\bm{\theta}_i)$ is the leaf of the tree $\bm{\theta}_i$ is assigned to, $\bm{z}_{\ell}$ is the decision made to the points in leaf $\ell$, and $R_{iz_{\ell}}$ is the outcome for the data point $i$ under the decision $z_{\ell}$. Notice that solving this problem requires knowledge of $R_{iz_{\ell}}$ for every combination of $i$ and $z_\ell$. In the usual prescriptive settings, this information has to be estimated as the counterfactuals are unknown. 

In the context of solving MICO problems, $\bm{\theta}_i$ is the key parameter vector, $s_i$ is the optimal strategy, and $y_i$ is the optimal objective value for the $i_{th}$ optimization instance. The goal is to train a decision tree that assigns a strategy to an optimization instance, so that its objective value is minimized. Notice that we do not need to estimate the counterfactuals in this setting. Instead, the true counterfactuals can be computed by assessing feasibility and computing suboptimality of the available strategies for each instance in our data set. If applying a strategy to a MICO instance leads to an infeasible solution, we assign a large penalty number to its outcome.
 
 
 \citet{Voice} demonstrates that a classification algorithm, Optimal Classification Trees (OCT) \citep{OCT, MLOPT} predicts high quality solutions even comparable to black box algorithms like deep neural networks. \citet{OPT} show that the performance of OPT is better than OCT, especially in the sense that OPT is better at avoiding infeasible or highly suboptimal solutions. For a more detailed explanation on this approach, see \citep{OPT}. 
...
In this section, we develop a machine learning approach to two-stage ARO with binary here-and-now variables and polyhedral uncertainty sets. 
...
&s.t.\quad &&  \bm{A}(\bm{d}, \bm{\theta})\bm{x} + \bm{B}(\bm{\theta})\bm{y} (\bm{d})\leq \bm{g}(\bm{\theta}),\quad \forall {\bm{d}\in\mathcal{D}}, \nonumber \\
...
Given $\bm{x}^*$ and the scenario $\bar{\bm{d}}$,  we compute the tight constraints of $Det(\bar{\bm{\theta}},\bm{x}^*, \bar{\bm{d}})$, following the explanation in Section \ref{subsec:mlopt}. We denote the tight constraints as $\tau_{\bm{y}}(\bar{\bm{\theta}},\bar{\bm{d}})$, and define the optimal strategy as $s_{\bm{y}}(\bar{\bm{\theta}}, \bar{\bm{d}}) = (\bm{x}^*, \tau_{\bm{y}}(\bar{\bm{\theta}},\bar{\bm{d}}))$. Given a new parameter $\hat{\bm{\theta}}$ and a scenario $\hat{\bm{d}}$, a model predicts a here-and-now decision $\hat{\bm{x}}$ and a set of tight constraints. A wait-and-see decision for the scenario $\hat{\bm{d}}$ can be computed by solving $Det(\hat{\bm{\theta}}, \hat{\bm{x}}, \hat{\bm{d}})$ imposing only the predicted tight constraints. 
...
Suboptimality and infeasibility of the strategy $\hat{\bm{x}}$ are measured by computing $\tilde{\mathcal{Q}}(\hat{\bm{x}})$ of the instance associated with $\hat{\bm{\theta}}$. We consider $\hat{\bm{x}}$ infeasible if $\tilde{\mathcal{Q}}(\hat{\bm{x}}) = \infty$. If it is feasible, we define its suboptimality as 
...
\[sub(\hat{\bm{x}}, \hat{\bm{d}}) = max  \Bigg\{ sub(\hat{\bm{x}}), \bigg(\tilde{\mathcal{Q}}(\hat{\bm{x}}) - \Big(\bm{c}(\hat{\bm{d}}, \hat{\bm{\theta}})^\top\hat{\bm{x}} +\bm{b}(\hat{\bm{\theta}})^\top{\bm{y}}^* \Big)\bigg) / \abs{\tilde{\mathcal{Q}}(\hat{\bm{x}})} \Bigg\}.\] 
...
Measuring the quality of the strategy $(\hat{\bm{x}}, \hat{\tau}_{\bm{y}})$ also consists of two stages. First, we evaluate the $\hat{\bm{x}}$ part following the procedure described above for the here-and-now decisions. If it is infeasible, then $(\hat{\bm{x}}, \hat{\tau}_{\bm{y}})$ is considered infeasible in the first place. If it is feasible, then we evaluate the $\hat{\tau}_{\bm{y}}$ part. We solve $Det(\hat{\bm{\theta}}, \hat{\bm{x}}, \hat{\bm{d}})$ imposing only the constraints included in $\hat{\tau}_{\bm{y}}$, and compute suboptimality and infeasibility following the description for the deterministic case in Section \ref{subsec:mlopt}. If this leads to an infeasible solution for $Det(\hat{\bm{\theta}}, \hat{\bm{x}}, \hat{\bm{d}})$, then again $(\hat{\bm{x}}, \hat{\tau}_{\bm{y}})$ is considered infeasible. If not, we compute the suboptimality of the $\hat{\tau}_{\bm{y}}$ part, denoted as $sub(\hat{\tau}_{\bm{y}})$. For a feasible $(\hat{\bm{x}}, \hat{\tau}_{\bm{y}})$, we define its suboptimality as
...
&s.t.\quad && \sum_{i =1}^{n} y_{ij}(\bm{d}) \geq d_j, \quad &&\forall {\bm{d}\in\mathcal{D}}, \; \forall {j\in [m]}, \nonumber \\
...
In the computational experiments, we have observed that as the size of the uncertainty sets gets large, the number of distinct strategies for the wait-and-see variables in the training set can get prohibitively large. The pruning algorithm described in \citep{Prune} is often not sufficient to deal with such large number of strategies. However, the algorithm we develop in this section can reduce the number effectively. 
...
\section{Computational Experiments}\label{sec6}
...
& s.t. && l_{i}x_{i}^1 + l_{i}x_i^2 + y_i(\bm{d}) - d_i \geq 0, && \quad \forall {\bm{d}\in\mathcal{D}}, \quad \forall {i\in [n]},  \nonumber \\
...
\paragraph{Results}
\begin{itemize}
    \item Both OPT and XGBoost show excellent accuracy regardless of the problem size or the size of the uncertainty sets.
    \item Even when the solutions are not exactly accurate, the maximum suboptimalities are still very low, indicating that the solutions are of high quality.
    ...
    \item  The solve time using OPT and XGBoost is significantly faster than Algorithm \ref{alg1}. Furthermore, OPT is often much faster than XGBoost. This is because the time it takes for a decision tree to compute its prediction is usually less than a millisecond, while XGBoost generally takes slightly longer.  
    \item The speed-up of our approach to compute the wait-and-see decisions is less drastic compared to here-and-now decisions or worst-case scenarios. To compute a here-and-now decision or a worst-case scenario, the only computation needed is to determine the output of the trained model on an input. In order to compute a wait-and-see decision, however, we still need to solve a linear optimization problem, leading to longer computation time.
...
\toprule
  $n$ & $m$ & $\Gamma$ & $N$ & $\abs{\bm{\tau}}$ & $K$   & $|\bar{u}| - |\tau_{\bm{y}}|$       \\ 
\midrule
 7& 7 & 38& 20000 &22 &  1 &  9  \\
\hline
 80& 60 & 241 & 20000 &65&  13 &  9  \\
\hline 
200& 150& 751& 25000&17498& 1& 148\\
\hline 
200& 150 & 601& 25000 &115& 1&12 \\
\bottomrule
\end{tabular}
...
\toprule
  $n$ & $\Gamma$ & $N$ &$\abs{\bm{\tau}}$ & $K$   & $|\bar{u}| - |\tau_{\bm{y}}|$      \\ 
\midrule
25 & 10 & 40000&47&  7 &  9 \\
\hline
600 & 10& 60000&90&  13 & 13  \\
\hline 
1000 & 10 &60000&48& 5& 8\\
\hline 
1000 & 45& 60000&5550& 1&24 \\
\bottomrule
\end{tabular}
...
\toprule
 $n$ & $m$ & $\Gamma$ & $N$ &$\abs{\bm{\tau}}$ & $K$   & $|\bar{u}| - |\tau_{\bm{y}}|$       \\ 
\midrule
 10&24 & 0.1& 20000 &1492&  30 & 430 \\
\hline
10&24 & 2& 15000 &10482&  1 & 446  \\
\bottomrule
\end{tabular}
...
Before presenting the main experiment results, we first compare the solve times of Algorithm \ref{alg1} under the relaxed tolerances versus the tolerances used in Section \ref{sec:scale}. Other experimental settings are identical to the description in Section \ref{sec:scale}. We provide the results in Table \ref{table:suboptimal}. We observe that the solve time of Algorithm \ref{alg1} becomes manageable with the relaxed tolerances, although it may come at the cost of the quality of the generated strategies.

\begin{table*}\centering
\ra{1.3}
\begin{tabular}{cccc}
\toprule
 $\epsilon_1$ & $\epsilon_2$ & Gurobi Optimality Gap & Solve Time(s)        \\ 
\midrule
 0.001 & 0.001 & 0.0001 & 849.12 \\
\hline
0.05 & 0.01 &  0.005 & 172.78   \\
\bottomrule
\end{tabular}
\caption{Solve time of the unit commitment problem with $n = 100, m = 24$ under different tolerances (in seconds).}
\label{table:suboptimal}
\end{table*}

Table \ref{table:UC_largescale} contains the main experiment results. The notations are identical to the previous sections.

\paragraph{Results}

\begin{itemize}
    \item As in Section \ref{sec:numerical_acc}, the predictions are always feasible.
    \item The accuracies are much lower compared to the previous results. However, the maximum suboptimalities are still around 0.02, indicating that the predictions are of reasonable quality.
...
&s.t.\quad && \sum_{j \in J}p_j(k) \geq D(k) \label{c1}  \\
