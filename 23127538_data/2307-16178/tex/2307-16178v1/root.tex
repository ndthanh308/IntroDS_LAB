...
\author{MirSaleh Bahavarnia$^\dagger$ and Ahmad F. Taha$^\dagger$
	\thanks{$^\dagger$The authors are with the Department of Civil and Environmental Engineering, Vanderbilt University, 2201 West End Avenue, TN 37235, USA. Ahmad F. Taha is also affiliated with the Department of Electrical and Computer Engineering. Emails: mirsaleh.bahavarnia@vanderbilt.edu, ahmad.taha@vanderbilt.edu.This work is supported by National Science Foundation under Grants 2152450 and 2151571.}
}
...
In order to improve the stability robustness of the perturbed state-space \eqref{PerturbedSD}, let us consider the updated stabilizing SOF controller, as $F + G$, with the following state-space model:
\begin{align}
...
\end{align}
\end{subequations}
For the special case of a diagonalizable $A+BFC$ with the eigendecomposition $A+BFC = V\Lambda V^{-1}$, since $\sigma^{\min}(A+BFC) = -\alpha(A+BFC)$ holds, \eqref{UpBoG} reduces to
\begin{align} \label{UpBoDiag}
& \beta_{\mathbb{R}}(A+BFC) \le -\alpha(A+BFC),
\end{align}
which is a tighter bound compared to the upper bound in \eqref{UpBo}. Specifically, for the case of symmetric $A + BFC$, according to Corollary $3.5.$ in \cite{hinrichsen1986stability}, the equality in \eqref{UpBoDiag} holds.
 
\subsubsection{Exact value} Unfortunately, computing the exact value of $\beta_{\mathbb{R}}(A+BFC)$ is not theoretically possible \cite{van1984near}. Also, there is no systematic tractable way to compute the exact value of the lower bound $\delta_{\sup}$ in \eqref{LoBo} due to the fact that we only know about the existence of $\delta(\epsilon)$ and nothing more. However, taking advantage of the upper bounds on $\beta_{\mathbb{R}}(A+BFC)$ (derived in \eqref{UpBoG} and \eqref{UpBoDiag}), we may utilize heuristics to obtain an appropriate approximate value of $\beta_{\mathbb{R}}(A+BFC)$ in a reasonable computational time. It is remarkable that if the equality in \eqref{UpBoG} becomes active (i.e., the case of a tight upper bound), then the proposed updated stabilizing SOF controller in this paper becomes efficient as it only requires the value of $\beta_{\mathbb{R}}^{u}$ which can efficiently be computed (e.g., the case of a symmetric $A+BFC$ for which $\beta_{\mathbb{R}}(A+BFC) = -\alpha(A+BFC)$ holds). For the special case of structured perturbation, i.e., $\Delta = B M C$ for a matrix $M \in \mathbb{R}^{m \times p}$, one may compute MDRP via (frequency domain)-based algorithms detailed by \cite{hinrichsen1990real}.
 
\subsection{Main idea}

Since \eqref{ImpIneq} provides a sufficient condition on the stability of $A + \Delta + B(F + G)C$, our main idea to propose an efficient updated stabilizing SOF controller $F + G$ is to compute $G$ via minimizing $\|BGC + \Delta\|_F^2$ and verifying that under which conditions, the minimized value of $\|BGC + \Delta\|_F^2$ would be less than $\beta_{\mathbb{R}}(A+BFC)^2$. It is noteworthy that if the most optimistic scenario occurs, (i.e., the scenario in which for a known $\Delta$, equation $\|BGC + \Delta\|_F = 0$ has a solution $G$), then one can completely cancel out the effect of the hitting perturbation $\Delta$ and retrieve the primary unperturbed $A + BFC$ as detailed later on.

With that in mind and to find a reasonable answer to the question stated in Section \ref{sec:ProFor} (\textit{Q1}), we consider the following optimization problem:
...
\end{align}
Assuming that $B$ and $C$ are respectively full-column rank and full-row rank and noting that identity $(C^T \otimes B)^{+} = {C}^{T+} \otimes B^{+}$ holds, optimization problem \eqref{EOP} can analytically be solved as
...
The items presented by Proposition \ref{Prop4} can be simplified into the following items:
\begin{enumerate}[label=\roman*]
...
Fig. \ref{FigCor} illustrates the guaranteed stability region $\mathcal{S}$ in $2$-dimensional parametric space of $(\eta,\tau)$ for various cases itemized by Corollary \ref{CorFig}. It is remarkable that item \textit{v} in Corollary \ref{CorFig} can only occur for the case of $\rho < \beta$ as $1 < 1+s_{\hat{\tau}}(s_{\hat{\theta}}+1)$ should be satisfied. Also, the non-trivial boundary points with $\eta = 0$ ($(0,\tau_l^0)$ and $(0,\tau_u^0)$) or $\eta = 1$ ($(0,\tau_u^1)$) can be computed via the following formulas:
\begin{align*}
...
In order to compute $\Xi_{\tau_{\hat{\Delta}},\theta_{\hat{\Delta}};\frac{\beta}{\rho},n}~(\%)$ (defined by \eqref{XiM}), we need to compute $\mathcal{V}_{n^2}(\texttt{D}(\mathcal{S}))$ and $\mathcal{V}_{n^2}\big(\texttt{S}_{\rho}^{n^2}\big)$. We compute both volumes via integral computation techniques similarly utilized by \cite{li2011concise}. First, $\mathcal{V}_{n^2}\big(\texttt{S}_{\rho}^{n^2}\big)$ can simply be computed as follows:
...
The following corollary highlights that since $G^{\ast}_{\hat{\Delta}_{\mathrm{NF}}}$ in \eqref{GDelCC}, \eqref{GDel}, and \eqref{WGDel} all lie inside the guaranteed stability region $\hat{S}$, the corresponding $F + G^{\ast}_{\hat{\Delta}_{\mathrm{NF}}}$ is a robust updated stabilizing SOF controller.
\begin{mycor}
Given an unknown norm-bounded perturbation $\Delta$ with an upper bound $\rho$ on its Frobenius norm, then $F + G^{\ast}_{\hat{\Delta}_{\mathrm{NF}}}$ with $G^{\ast}_{\hat{\Delta}_{\mathrm{NF}}}$ in \eqref{GDelCC}, \eqref{GDel}, and \eqref{WGDel} is a robust updated stabilizing SOF controller. 
\end{mycor}
...
Given $(\rho,n)$, computing $(\beta_{\mathbb{R}}(A+BFC),\hat{\tau}_{\mathrm{NF}},\hat{\theta}_{\mathrm{NF}})$, and having access to sufficiently accurate estimates $(\hat{\phi}_c,\hat{\phi}_s)$ of $(\phi_c,\phi_s)$, we can utilize \eqref{GDelCC}, \eqref{GDel}, and \eqref{WGDel} to propose robust updated stabilizing SOF controllers.
...
It is remarkable that the accurate computing of $\beta$ plays a significant role in accurately identifying the stability regions. As Fig. \ref{FigKS} (Right) depicts, choosing $\rho$ equal to $2 \times 0.1931$ (as chosen for Fig. \ref{FigKS} (Left)) and $\beta$ equal to $0.3230$ (an inaccurate value), leads to the misleading stability regions. First, the guaranteed (conservative) stability region has erroneously been enlarged. Second, the guaranteed (conservative) stability region has erroneously become the superset of the exact one.

\subsection{Unknown norm-bounded perturbation}

Let us consider the autopilot control problem for an air-to-air missile ($AC4$) \cite{leibfritz2006compleib}. The number of states for such a control problem is $n = 4$. For $\frac{\beta}{\rho} = \frac{1}{2}$ and $n = 4$, we get the following NF-based designs:
\begin{align*}
& (\hat{\tau}_{\mathrm{NF}}^{\mathrm{Cheb.~center}}, \hat{\theta}_{\mathrm{NF}}^{\mathrm{Cheb.~center}}) = \bigg(\frac{2-\sqrt{2}}{2},\frac{2-\sqrt{2}}{2}\bigg),\\
...
\end{align*}
where attain $\Xi_{\frac{\beta}{\rho},\tau_{\hat{\Delta}},\theta_{\hat{\Delta}}}^{\mathrm{Cheb.~center}} = 5.0077 \times 10^{-7}\%$, $\Xi_{\frac{\beta}{\rho},\tau_{\hat{\Delta}},\theta_{\hat{\Delta}}}^{\mathrm{Centroid}} = 2.0450 \times 10^{-10}\%$, and $\Xi_{\frac{\beta}{\rho},\tau_{\hat{\Delta}},\theta_{\hat{\Delta}}}^{\mathrm{W-centroid}} = 7.0944 \times 10^{-5}\%$, respectively. 
...
To empirically verify the relative performance of the weighted centroid update compared to the centroid update, the Chebyshev center update, and the update based on a point close to the origin $(\hat{\tau},\hat{\theta}) = (0.01,0)$, we generate uniformly random samples of an unknown norm-bounded perturbation $\Delta$ with $0 < \|\Delta \|_F \le \rho$ \cite{harman2010decompositional} and check if $\| \Delta - \hat{\Delta} \|_F < \upsilon$ holds. To be more precise, we generate the uniformly random samples as follows:
\begin{align*}
...
Note that given $\vartheta$, we can compute $\phi_c$, $\phi_s$, and $\theta$ via \eqref{ExtractedF}. In order to compute $G^{\ast}_{\hat{\Delta}}$ in \eqref{AYGDel}, we need to choose $(\hat{\phi}_c,\hat{\phi}_s)$ given $(\gamma_c,\gamma_s)$. The more accurate $(\gamma_c,\gamma_s)$ (i.e., the larger values of $\gamma_c$ and/or $\gamma_s$) and/or $(\hat{\tau},\hat{\theta})$ (i.e., the smaller values of $\hat{\tau} -\tau$ and/or $\hat{\theta}-\theta$), the more accurate estimate $\hat{\Delta}$ we have. Given $(\gamma_c,\gamma_s)$ and computing $(\phi_c,\phi_s)$, we solve the following equations:
...
Fig. \ref{FigBPsRE} visualizes the relative performance (both guaranteed (conservative) and exact scenarios) of the weighted centroid update compared to the centroid update, the Chebyshev center update, and the update based on a randomly generated point $(\hat{\tau},\hat{\theta}) = (0.4081,0.3969)$ for a randomly generated choice of $(\gamma_c,\gamma_s) = (0.9212,0.8315)$. As Fig. \ref{FigBPsRE} (Left) shows, the weighted centroid update is the only successful update among all the updates. Fig. \ref{FigBPsRE} (Right) similarly depicts the outperformance of the weighted centroid update compared to the other updates. Also, it depicts that in the exact scenario, the other updates have attained some positive results. The descending order of the performance according to Fig. \ref{FigBPsRE} (Right) is the W-centroid update, the Cheb. center update, the centroid update, and the random update. Interestingly, we observe that the corresponding values of the geometric metric $\Xi_{\tau_{\hat{\Delta}},\theta_{\hat{\Delta}};\frac{\beta}{\rho},n}~(\%)$ have the same order $(7.0944 \times 10^{-5} \%, 5.0077 \times 10^{-7} \%, 2.0450 \times 10^{-10} \%, 7.1872 \times 10^{-12} \%)$. 
...
\end{align*}
Then, defining $\chi := U_H^T \delta$ and noting that $U_H^T U_H = I_{n^2}$ holds, we get $\delta = U_H \chi$. Since $\delta^T \delta = \chi^T U_H^T U_H \chi$, $U_H^T U_H = I_{n^2}$, and $\delta^T \delta = \|\Delta\|_F^2 = r^2$ hold, we get $\chi^T \chi = r^2$ that inspires us to define $\psi := \frac{\chi}{\| \chi \|} = \frac{\chi}{r}$. Note that $\psi \in \mathbb{R}^{n^2}$ and $\|\psi\| = 1$ hold. Then, we have $\chi = r \psi$ and subsequently we get $\delta = U_H \chi = r U_H \psi$. Defining $\mu \in \mathbb{R}^{mp}$ and $\nu \in \mathbb{R}^{n^2-mp}$ as follows:
\begin{align*}
