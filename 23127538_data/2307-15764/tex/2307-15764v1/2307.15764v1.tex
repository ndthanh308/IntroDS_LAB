...
\title{Geometric Ergodicity, 
Unique Ergodicity and Wasserstein Continuity of 
Non-Linear Filters with Compact State Space}
...
\begin{abstract}   
    ...
    Furthermore, we provide additional results on 
    the unique ergodicity of filter processes 
    associated with ergodic hidden Markov models, 
    generalizing existing results to compact state spaces.
...
The study of asymptotic properties of hidden Markov models dates back to at least a 1957 paper by Blackwell\cite{blackwell1959entropy}, who was interested in understanding the entropy rate of a stochastic process $Y_n = h(X_n)$, where $h$ is a non-invertible function and $(X_n)_{n\geq 0}$ is a stationary Markov chain. To do this, Blackwell introduced the concept of a filtering process $(\pi_n)_{n\geq 0}$, which is a measure-valued Markov process defined as $\pi_n := P(X_n \in \cdot | Y_1,..., Y_n)$. Blackwell's work raised the question of whether the filtering process has a unique stationary measure, or in other words, whether it is uniquely ergodic. Blackwell conjectured that this was the case for irreducible underlying Markov chains, but a counterexample by Kaijser\cite{Kaijser} showed that this conjecture was incorrect. The problem of fully characterizing the unique ergodicity of the filtering process has since then received significant attention. On the other hand, 
there is interest in the filter stability problem, 
which refers to the property that the filtering process 
becomes nearly independent of an incorrect initialization
 as the number
of steps $n$ approaches infinity. It has been proved 
...
Previous studies on unique ergodicity of the filter process have 
generally been limited to the case where the hidden Markov model 
lives in a finite or countable state space. While some studies 
have allowed for the observation space to be a Polish space, 
in general the observation space is also assumed to be finite 
or countable. 
In our study, we extend the state space of the hidden Markov model 
to a compact space and assume that the observation space 
is countable. 
A crucial step in our analysis is to establish 
Wasserstein continuity and equi-continuity 
of non-linear filters, which we present as 
a separate contribution; this generalizes 
recent studies on weak Feller continuity of non-linear filters.
Additionally, we establish conditions that ensure geometric ergodicity, 
which is a novel contribution to the literature.
...
\begin{thm}[\cite{Bil99}, Prokhorov Theorem]Let $(X, d)$ be a Polish space. 
    $K \subset \mathcal{P}(X)$ is tight if and only if the closure 
    of $K$ in $\left(\mathcal{P}(X), \rho_{BL}\right)$ is compact.
\end{thm}
\begin{corollary}\label{CorCompact}
Assume that $(X, d)$ is compact metric space. We have $P(X)$ is compact metric space with respect to the bounded Lipschitz metric. 
\end{corollary}
...
We build upon and generalize the approaches 
presented in \cite{kara2020near} 
(see also \cite{KSYWeakFellerSysCont}) 
for establishing Wasserstein continuity and 
geometric ergodicity, as well as by Kochman 
and Reeds \cite{kochman2006simple} for 
proving unique ergodicity. 

\begin{asm} \label{asm1}
\begin{enumerate}[label=(\roman*)]
\item  $(\mathbb{X},d)$ is a bounded compact metric space, 
with $D$ being the diameter of $\mathbb{X}$.
...
\end{thm}
Indeed, this result is novel in the literature as 
previous works have primarily focused on unique 
ergodicity or strong ergodicity, without specifically 
addressing geometric ergodicity. The concept of 
...
{\bf Wasserstein Continuity and the L-chain Property}
...
Lemma \ref{Talpha} establishes the property of 
Wasserstein continuity, ensuring the smoothness and 
continuity of the Wasserstein distance for the filter process. This complements recent results on the weak Feller property of non-linear filters \cite{FeKaZg14} \cite{KSYWeakFellerSysCont}, in addition to \cite[Theorem 7]{kara2020near} .

{\bf Implications for Controlled Models.} 

As a corollary of our analysis, 
specifically with reference to Lemma \ref{Talpha}, 
we will present a 
...
is new to the literature, such an implication on optimal stochastic control with partial information was not available earlier in the literature.
...
For all these models, we assume that $T(x|.)$, $Q(y|.)$ 
...
then filter stability and unique ergodicity become equivalent 
...
Van Handel (\cite{Handel} Theorem $5.2$) showed that
...
\cite{mcdonald2020exponential} 
provides another condition for 
filter stability, and unique ergodicity. 
Recall that for a kernel $K$, $\delta(K)$ is the 
Dobrushin coefficient defined in Definition \ref{Dobrushincoefficient}.
If the inequality $(1-\delta(T))(2-\delta(Q))<1$ 
is satisfied, then the filter is exponentially stable and, therefore,
uniquely ergodic. Our analysis also relies on results from \cite{kara2020near}, 
which we will discuss in detail in the following section.
...
as presented in Lemma \ref{Talpha}.
...
\subsection{Wasserstein Continuity of the Filtering Kernel}
...
\begin{proof}[Proof of Lemma \ref{Talpha}]
...
by Lemma \ref{Talpha}. As a result, $g/\beta \in \Lip(\mathcal{Z})$.
...
we know that the weak Feller property holds \cite{WeakFeller}.
...
\begin{proof}[Proof of Theorem \ref{Main2}]    
    ...
    $\mathcal{Z}$ is compact (Corollary \ref{CorCompact}). 
...
$$
for every $x,x' \in \mathbb{X}$, $u \in \mathbb{U}$ and for some $\alpha<\infty$. We have
$$
...
$$
for all $z_0,z_0' \in \mathcal{Z}$, $u \in \mathbb{U}$, $\beta\in R^+$.
\end{lemma}
The proof is the same as the proof of Lemma \ref{Talpha} 
because we choose a fixed control action.
...
For the uncontrolled case, we first proved Lemma \ref{Talpha} 
and then used this lemma to show that the filter process is an L-chain. 
We can also prove Lemma \ref{Calpha} in a similar way to Lemma \ref{Talpha}. 
However, Lemma \ref{Calpha} does not give us the desired result because 
...
rigorous study on the conditions ensuring 
geometric ergodicity and unique ergodicity 
of non-linear filter processes. This work 
broadens the understanding of these topics, 
which have received limited attention in the 
literature, especially in the context of compact 
state spaces.
...
equality. This exploration shows optimal 
costs are constants and robust to incorrect 
initializations under Lipschitz regularity.
Our research expands the 
understanding of unique and geometric ergodicity 
in non-linear filter processes and provides 
new insights into the average cost optimality 
