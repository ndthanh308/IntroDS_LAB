...
This task takes a single image as input and regresses human model parameters such as SMPL~\cite{SMPL} as output.
...
These \enquote{local} contents (\ie, human joints) occupy only a small portion of the 3D space, while the majority of voxels are often occupied by occlusions and background.
Consequently, due to the lack of explicit supervision for the entire 3D space, it becomes difficult to differentiate target humans from other semantically similar voxels, resulting in ambiguous 3D representations.
