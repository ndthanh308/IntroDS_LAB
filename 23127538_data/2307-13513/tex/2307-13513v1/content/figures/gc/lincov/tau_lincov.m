...
bias_tau_sec = [0.25,1, 2, 4] * 60 * 60;
nH = length(bias_tau_sec);

% sweep over gyro bias
...
for idx = 1:2
        ...
        for hdx = 1:nH
            ...
            MGNC_params.Nav.Filter.Config.imu.gyro_bias_Qpsd   = (MGNC_params.Nav.Filter.Config.imu.gyro_bias_repeat)^2 * 2 / bias_tau_sec(hdx);
            ...
            MGNC_params.Nav.Filter.Config.imu.acc_bias_Qpsd    = (MGNC_params.Nav.Filter.Config.imu.acc_bias_repeat)^2 * 2 / bias_tau_sec(hdx);
            ...
            P0(params.OrnIdx,params.OrnIdx) = eye(3)*deg2rad(heading_p0)^2;
            ...
            for tdx = 1:Nt
                % propagate
                [Fk, Qk] = ComputeSTMLinCov(params, MGNC_params, dt, bias_tau_sec(hdx));
                Pk       = Fk * Pk * Fk' + Qk;
...
figure('name','Heading'); hold on; grid on; grid minor;
 ...
 fig_comps.p1 = annotation(fig,'rectangle',...
                           [0.1296875 0.381606765327696 0.776 0.542283298097252],'FaceColor',[1 0 0],...
                           'FaceAlpha',0.25);

 hdx = 1;
 fig_comps.p2 = plot(time_sec./60, 3*rad2deg(sqrt(squeeze(Phdg(hdx,2,:)))), '-', 'Color', PS.MyBlack,'LineWidth', 4.0, 'DisplayName', sprintf('tau = 0.25 hour'));

 hdx = 2;
 fig_comps.p3 = plot(time_sec./60, 3*rad2deg(sqrt(squeeze(Phdg(hdx,2,:)))), '--', 'Color', PS.MyBlack,'LineWidth', 4.0, 'DisplayName', sprintf('tau = %i hour', bias_tau_sec(hdx)/3600));

 hdx = 3;
 fig_comps.p4 = plot(time_sec./60, 3*rad2deg(sqrt(squeeze(Phdg(hdx,2,:)))),':', 'Color', PS.MyBlack,'LineWidth', 4.0, 'DisplayName', sprintf('tau = %i hour', bias_tau_sec(hdx)/3600));

 hdx = 4;
 fig_comps.p5 = plot(time_sec./60, 3*rad2deg(sqrt(squeeze(Phdg(hdx,2,:)))), '-.', 'Color', PS.MyBlack,'LineWidth', 4.0, 'DisplayName', sprintf('tau = %i hour', bias_tau_sec(hdx)/3600));


xlabel('Initialization Time (minutes)', 'FontSize', 24);
ylabel('Heading Uncertainty (deg, 3\sigma)', 'FontSize', 24);
legend('show');
legend('AutoUpdate','off')
fig_comps.p6 = plot([time_sec(1), 7200]./60, [1.2, 1.2], '-.r', 'LineWidth', 4.0);
...
% legend('show');
% legend('AutoUpdate','off')
% plot([time_sec(1), 7200]./60, [1, 1], '-.r', 'LineWidth', 2.0);
% plot([120, 120], [0, 1], '-.r', 'LineWidth', 2.0);
...
function [Fk, Qk] = ComputeSTMLinCov(params, MGNC_params, stm_dt_sec, tau)
    ...
    Fcon(params.Bgy1Idx,params.Bgy2Idx) = -1/tau * I;
