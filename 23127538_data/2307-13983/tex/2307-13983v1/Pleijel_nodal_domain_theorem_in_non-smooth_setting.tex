...
\begin{abstract}
	We prove an asymptotic upper bound on the number of nodal domains of Laplacian eigenfunctions in  non-collapsed RCD spaces. As a consequence, we obtain that the Courant nodal domain theorem holds except at most for a finite number of eigenvalues. More in general, we show that the same bound is valid for Neumann eigenfunctions on uniform domains. This result is new even in the Euclidean space, by improving on the regularity assumptions of previous statements.
\end{abstract}		
...
Before stating our main result, let us first introduce the setting and some notations. Our investigation deals with eigenfunctions of the Neumann Laplacian $\Delta_\cN$ (see Definition \ref{def:neum lapl}) of a uniform domain $\Omega\subset \X$ (see Definition \ref{def: unif dom}) in a non-collapsed $\rcd(K,N)$ space $(\X,\sfd,\hau^N)$,  where $\hau^N$ denotes the $N$-dimensional Hausdorff measure in $(\X,\sfd)$. We will clarify below why we need to restrict our attention to non-collapsed ambient spaces instead of considering the full $\RCD(K,N)$ class.  Here we limit ourselves to mention that these assumptions are sufficient for the Neumann Laplacian in $\Omega$ to have discrete spectrum and for the Neumann eigenfunctions to be continuous (see Corollary \ref{cor:spectrum compatilbity}). We list the Neumann eigenvalues by
$$0=\lambda_1^\cN(\Omega)\le \lambda_2^\cN(\Omega)\le \ldots\le \lambda_k^\cN(\Omega)\le\ldots\rightarrow +\infty\, ,$$
counted with multiplicity. Moreover thanks to the continuity of a Neumann eigenfunction $u$, it makes sense to define its \emph{nodal domains}, which are the connected components of $\Omega \setminus \{u=0\}$.
For any $k\in \N$ we can now define
$$M_\Omega(k):=\sup\left\{\# \text{ of nodal domains of } u : u \ \textrm{Neumann eigenfunction of eigenvalue } \ \lambda_k^\cN(\Omega)\right\}.$$ We refer to Def. \ref{def:counting} for the precise definition of  $M_\Omega$.
...
Let $(\X,\sfd,\hau^N)$ be an $\rcd(K,N)$ space, with $K\in \R$ and $N\ge 2$, and let $\Omega\subset \X$ be a uniform domain. Then
\begin{equation}\label{eq: main result}
\limsup_{k\to +\infty} \frac{M_\Omega(k)}{k} \le \frac{(2\pi)^N}{\omega^2_N j^N_{\frac{(N-2)}{N}}}<1\, .
\end{equation}
In particular for every $k \in \nn$ large enough every Neumann eigenfunction of eigenvalue $\lambda_k^\cN(\Omega)$  has less than $k$ nodal domains.
\end{theorem}
There has been recently a growing interest in the study of  eigenvalues and eigenfunctions of the Laplacian and their zero set in the setting of RCD spaces (see  \cite{AHPT21,AH18,H23,HM21,DePF22,BF22,CF21,DZ23,ZZ,AHT,DZsec}). However, to the best of our knowledge, this is the first non-trivial result related to nodal domains.
...
Additionally, thanks to the recent \cite{Raj20}, we know that every $\rcd(K,N)$ space contains a rich class of non-trivial uniform domains, hence it is possible to find many sets that satisfy the assumptions of our result.
...
\end{equation}
\end{cor} 
In the case of Riemannian manifolds Corollary \ref{cor: mainRCD} goes back to the work of B\'{e}rard and Meyer in \cite{BM82}.  

The main result in Theorem \ref{main th} above is interesting already when taking $(\X,\sfd,\hau^N)$ to be the $N$-dimensional Euclidean space endowed with the Lebesgue measure. We extract this version below in a self-contained statement, for the convenience of the reader.
\begin{cor}\label{cor: mainEucl}
Let $\Omega\subset \rr^N$, $N\ge2,$ be a uniform domain. For every $k\in \nn$ denote by $\lambda_k^\cN(\Omega)$ the Neumann Laplacian eigenvalues in $\Omega$ and by $M_\Omega(k)$ the maximal  number of nodal domains of a Neumann eigenfunction of eigenvalue $\lambda_k^\cN(\Omega).$   Then
...
Our work introduces a different strategy and avoids any reflection argument, allowing us to handle more general domains without imposing any restriction on the dimension.  The basic idea of our method is to consider the domain $\Omega$ as a metric measure space in its own right, and infer from the uniform condition some good analytical properties of this space. 
With this point of view, we bypass some of the major difficulties faced in the previous Euclidean literature. In this regard we also prove two technical results on uniform domains in the Euclidean space that we believe interesting on their own, namely
a Faber-Krahn-tpye inequality (see Corollary \ref{cor:FKeucl}) and a Green's formula for eigenfunctions (see Corollary \ref{cor:rayleig-on-nodal}).


\vspace{3mm}
We now comment further on the assumptions and the proof of Theorem \ref{main th}.

The uniformity hypothesis on the domain   guarantees the discreteness of the spectrum of the Neumann Laplacian, a fact even needed to state the theorem. Additionally we will make a crucial use of analytical properties of uniform domains in metric measures spaces, such as Sobolev extension properties, the validity of a Poincar\'e inequality and a Sobolev embedding  (see Section \ref{sec:unif domain} for more details).
...
The authors thank Asma Hassannezhad for helpful discussions.  The third author is supported by the Academy of Finland project Incidences on Fractals, Grant No.321896.
...
$$ |{\bf D}f|(A)\coloneqq \inf \{|{\bf D}f|(A) \ : \ U\subset \X \text{ open, } A\subset U\}, \quad \forall\, \text{$A\subset \X$ Borel} $$
...
\begin{theorem}[\hspace{1sp}{\cite[Theorem 8.3]{HK00}}]\label{thm:compact embedding}
    Let $\Xdm$ be a bounded PI space. Then the embedding $\W(\X)\hookrightarrow L^2(\mm)$ is compact.
\end{theorem}
...
\begin{equation}\label{eq:PI discrete sepctrum}
    \parbox{12cm}{The Laplacian has discrete spectrum on any bounded and infinitesimally Hilbertian PI space.}
\end{equation}
...
\begin{theorem}[Continuity of eigenfunctions]\label{thm:continuous eigen}
    Let $\Xdm$ be a bounded  PI space  and $u$ be an eigenfunction of the Laplacian in $\Xdm$, i.e.\ $u \in \dom(\Delta) $ and $\Delta u=-\lambda u$ for some $\lambda \in \rr.$ Then $u$ is H\"older continuous in $\X.$ 
\end{theorem}
...
\begin{prop}\label{prop: neum bound dir}
		Let $\Xdm$ be a bounded infinitesimally Hilbertian PI space  and let  $f$ be an eigenfunction of the Laplacian of eigenvalue $\lambda$. Then for any $\Omega$ nodal domain of (the continuous representative of) $f$ it holds
  \begin{equation}\label{eq:NDineq}
      \lambda_1(\Omega)\leq\lambda=\frac{\int_{\Omega} |D f|^2\, \d \mm}{\int_{\Omega} f^2\,\d\mm}.
  \end{equation}
	\end{prop}
 In \cite{BM82} (in Riemannian setting) it is  shown that the first in \eqref{eq:NDineq} is  actually an equality, however we do not know whether the same is true also in this more general setting. Nevertheless \eqref{eq:NDineq} will be sufficient for our purposes.
...
\end{proof}
	...
	\begin{proof}[Proof of Proposition \ref{prop: neum bound dir}]
		Let $f$ be an eigenfunction of the Laplacian of eigenvalue $\lambda$ and let $\Omega$ be one of its nodal domains. Since $\Xdm$ is a PI space, the metric space $(\X,\sfd)$ is  locally connected, hence by Lemma \ref{lem:basic nodal} we have that $\Omega$ is open and that either $f>0$ or $f<0$ in $\Omega.$ Assume without loss of generality that $f$ is positive in $\Omega$.
		...
		\end{equation}
		To see this   by definition of eigenfunction and since $\supp(\psi_n)\subset \Omega$ one has
		\begin{equation*}
...
\begin{definition}[Nodal domains counting function]\label{def:counting}
    Let $\Xdm$ be an infinitesimally Hilbertian PI space and $\Omega\subset \X$ be a uniform domain (see Definition \ref{def: unif dom}). The \emph{nodal domains counting function} $M_\Omega:\nn \to \nn\cup\{+\infty\} $ is given by
    $$M_\Omega(k):=\sup\{M(u) : u \ \textrm{Neumann eigenfunction in $\Omega$ of eigenvalue} \ \lambda_k^\cN(\Omega)\},\quad k \in \nn,$$ 
    where $\{\lambda_k^\cN(\Omega)\}_k$ are the eigenvalues of the Neumann Laplacian in  $\Omega$ (which has discrete spectrum by Corollary \ref{cor:spectrum compatilbity}) and
    where $M(u)$ is the number of nodal domains (see Definition \ref{def:nodal domains}) of the continuous representative of $u$ in $\Omega$ (which exists again by Corollary \ref{cor:spectrum compatilbity}).
\end{definition}}

We are finally  ready to prove the main result of the note.
...
\begin{equation*}
    \frac{M_\Omega(k)}{k}\le \frac{\mm(\overline{\Omega})}{k}\frac{1}{\delta(   \Omega, \eps,N,\eta)}+ 
...
\end{equation*}
where $M_\Omega(k)$ is as in Definition \ref{def:counting}.
Passing to the $\limsup$ as $k \to +\infty$, using \eqref{eq:weyl},

\begin{equation}\label{eq:fine}
\limsup_{k \to +\infty }\frac{M_\Omega(k)}{k}\le  \frac{(2\pi)^N}{\omega_N\mm(\overline \Omega)}\frac{1}{C(\overline{\Omega})^{\frac{N}{2}}}\frac{\eta}{\delta'(\Omega,\eps,N)}+ \frac{(2\pi)^N}{\omega_N} \frac{1}{\left((1-\eps) \alpha_N\right)^{\frac N2}},
\end{equation}
sending first $\eta \to 0$ and then $\eps \to 0$ (note that both $\delta'$ and $\eps$ are independent of $\eta$) we conclude that 
\begin{equation*}
\limsup_{k \to +\infty }\frac{M_\Omega(k)}{k}\le  \frac{(2\pi)^N}{\omega_N\alpha_N^{\frac N2}}=\frac{(2\pi)^N}{\omega^2_N j^N_{\frac{(N-2)}{N}}}<1,
\end{equation*}
