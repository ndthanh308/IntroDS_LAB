...
%\title{\boldmath Mass spectrum of 2-flavor Schwinger model by DMRG method}
%\title{\boldmath Three ways of calculating mass spectra for 2-flavor Schwinger model in the Hamiltonian formalism}
% \title{\boldmath Calculating mass spectra in the Hamiltonian formalism and demonstration in the 2-flavor Schwinger model}
...
The first one corresponds to the conventional Euclidean method in the Monte Carlo simulations. 
...
We note that the multi-flavor Schwinger model at small mass $m$ is a strongly-coupled field theory even after the bosonizations, and thus it deserves to perform the first-principle numerical calculations. 
...
These methods rely on different disciplines from that of Monte Carlo simulations for the conventional lattice gauge theories, and thus they are expected to give complimentary frameworks. 
...
\begin{itemize}
    ...
    \item one-point-function scheme, and 
    \item dispersion-relation scheme. 
\end{itemize}
The first one corresponds to the conventional method using the Euclidean (or spatial) correlators as in the Monte Carlo simulations. 
...
It is physically nontrivial if those analytic predictions are reproduced by the first-principle numerical computations when we go into the details beyond qualitative aspects. 

We performed the DMRG computations using the C++ library of ITensor \cite{itensor} to obtain the mass spectrum with the above three methods. 
...
This is consistent with the analytic prediction based on the WKB approximation of the Abelian bosonized description, and our numerical results are so close to the WKB-based formula between the pion and sigma-meson masses, $M_\sigma/M_\pi=\sqrt{3}$. 
...
In Section~\ref{sec:method}, we briefly explain our simulation method, the DMRG algorithm.
Section~\ref{sec:setup} shows our setup of the simulation.
In Section~\ref{sec:result}, we present our simulation results for the three methods.
Section~\ref{sec:summary} is devoted to conclusion and discussion.
...
%With the $\theta$ term, the Schwinger model suffers from the sign problem in the conventional Monte Carlo method.
...
The massive 2-flavor Schwinger model \eqref{eq:Lagrangian} no longer has the chiral symmetry, but it maintains the vector-like symmetry, 
\begin{equation}
    \left\{
    \begin{array}{cc}
    [\SU(2)_V/\mathbb{Z}_2]\times (\mathbb{Z}_2)_G     &  \quad (\theta\in \pi\mathbb{Z}),\\
    \SU(2)_V/\mathbb{Z}_2     & \quad (\text{else}), 
    \end{array} 
    \right.
\end{equation}
and we call $\SU(2)_V/\mathbb{Z}_2$ as the isospin symmetry. 
...
\end{equation}
%where $J_{y}$ is the $y$-component of the isospin operator (\ref{eq:J_cont}), 
...
We would like to clarify what kinds of approximations are used in the previous studies and give the justification of this work on physical aspects. 
...
Thus, it is worth studying the mass spectrum of the 2-flavor model by first-principle numerical methods.
...
the bosonic degrees of freedom having an infinite dimensional Hilbert space. The Gauss law (\ref{eq:Gauss_law_continuum}) is also discretize as
\begin{equation}
...
Let us consider the meson operators (\ref{eq:pi_meson}), (\ref{eq:eta_meson}),
and (\ref{eq:sigma_meson}) on the lattice. 
...
We can also effectively calculate the expectation values or correlation functions of local operators by rewriting those operators in the form of matrix product operators (MPOs) and then taking contractions with the ground state $\ket{\Psi}$.  

We can use DMRG to obtain the low-energy excited states in a recursive way. 
Assume that we already find the energy eigenstates $\ket{\Psi_{k'}}$ with $k'=0,1,\ldots, k-1$ from below. 
Then, we apply the same technique to find the $k$-th state $\ket{\Psi_k}$ by changing the Hamiltonian for the cost function as  
\begin{equation}
H_{k}=H+W\sum_{k^{\prime}=0}^{k-1}\ket{\Psi_{k^{\prime}}}\bra{\Psi_{k^{\prime}}},  
\end{equation}
...
we generate many excited states up to $k=23$, which require a lot of computational cost.
...
\begin{itemize}
    ...
    \item the one-point-function scheme, and 
...
In the relativistic quantum field theories, the Hilbert space only plays the secondary role,
...
Thus, the leading contribution to the imaginary-time correlation function
$\bra{\mathrm{Vac}}Oe^{-H|x|}\ket{\mathrm{Bdry}}$ should come from the lightest particle with the zero-momentum projection, giving $e^{-Mx}$. 
...
\end{equation}
with $C=0.203(9)$,
which do not depend on the cutoff $\varepsilon$ as before.
...
\subsection{Disparsion-relation shceme}
...
For example, the zero modes of the lightest meson, namely the pion, is expected to be obtained as the first excited state. 
...
The energy gap $\Delta E_{k}=E_{k}-E_{0}$ of the $k$-th excited state is shown
...
We note that its ground-state expectation value $\Braket{K^{2}}_{0}\simeq 0.46$ is nonzero because of the boundary effect and maybe also due to lattice artifacts, and thus we subtract $\Braket{K^{2}}_{0}$ from $\Braket{K^{2}}_{k}$ of the excited states. 
...
To identify the states, we measure the expectation values of the isospin operators, $\bm{J}^{2}$ and $J_{z}$, the parity $P$ and the $G$-parity $G=Ce^{i\pi J_{y}}$ in Section~\ref{subsec:global_obs}.
...
$\Braket{\psi_{k_{1}}|J_{z}|\psi_{k_{2}}}$ in each triplet to compute expectation values in the $J_z$ basis.\footnote{In computing the expectation value of the $G$-parity, we find it easier to do it in the $J_y$ basis instead of the $J_z$ basis because $G=Ce^{i\pi J_y}$, and we thus performed it in the $J_y$ basis.}\textsuperscript{,}\footnote{It is possible that triplets and singlets are also mixed in the DMRG if their energies are close. In fact, the states for $k=19,\cdots,23$
are mostly degenerated. We separated one triplet and two singlets out of them
by diagonalization of $\Braket{\psi_{k_1}|\bm{J}^{2}|\psi_{k_2}}$ and of $\Braket{\psi_{k_1}|C|\psi_{k_2}}$.}

\begin{table}[tb]
\centering
\include{table_triplet}\vspace{-1.3em}
\caption{\label{tab:triplet}
The quantum numbers of the isospin triplet states.
The index $k$ comes from the level of each state in the original
basis. The rows of the table are separated into each triplet.}
\end{table}
\begin{table}[tb]
\centering
\include{table_singlet}\vspace{-1.3em}
...
We identify the lowest triplet $k=1,2,3$ as the lowest modes of
...
we find that the $k=13$ state has the quantum number consistent with the sigma meson,
namely $J^{PG}=0^{++}$ and $J_{z}=0$.
The $k=18$ state is consistent with the eta meson
...
$\Delta E_{k}=E_{k}-E_{0}$ against the momentum square
$\Delta K_{k}^{2}=\Braket{K^{2}}_{k}-\Braket{K^{2}}_{0}$
...
The triplet for $k=19,20,21$ is not shown in this plot since it is
...
In addition, all the results show the relation $M_{\pi}<M_{\sigma}<M_{\eta}$,
which agrees with the analytic prediction by the bosonization technique.
The mass of the eta meson $M_{\eta}\sim 0.9$ is consistent with $M_{\eta}=\mu+O(m)$ since $\mu\sim 0.8$ and $m=0.1$ in the current setup.
We also find that the masses of the pion and sigma mesons satisfy the WKB-based formula \eqref{eq:sqrt3-Schwinger}, $M_{\sigma}/M_{\pi}=\sqrt{3}$, within not more than a 5\% deviation. 
...
We found the scattering state unexpectedly by this method.
...
For example, in our setup, we encounter the $3\times 4$ states of the pion before obtaining the sigma meson at $k=13$.
...
For example, if we are interested in a specific meson, it is more effective generating excited states with a constraint on the quantum number to skip mesons out of interest.
...
\end{equation}
Then it is straightforward to construct these operators as MPO.
...
does not commute with the Hamiltonian with the open boundary condition. 
...
We generate the ground state and the excited states up to the level $k=4$.
The four excited states turned out to be degenerated.
Thus, we compute $\Braket{C}_{k,k^{\prime}}$ including the off-diagonal elements,
