...
%\icmltitlerunning{Modified Spectrum Kernels}
\icmltitlerunning{Controlling the Inductive Bias of Wide Neural Nets}
\begin{document}

\twocolumn[
%\icmltitle{Modified Spectrum Kernels: Controlling the Inductive Bias of Wide Neural Networks}

\icmltitle{Controlling the Inductive Bias of Wide Neural Networks by Modifying the Kernel’s Spectrum}
...
\icmlauthor{Amnon Geifman}{equal,sch}
\icmlauthor{Daniel Barzilai}{equal,sch}
\icmlauthor{Ronen Basri}{sch,meta}
\icmlauthor{Meirav Galun}{sch}
%\icmlauthor{}{sch}
%\icmlauthor{}{sch}
\end{icmlauthorlist}

\icmlaffiliation{sch}{Department of Computer Science, Weizmann Institute of Science, Rehovot, Israel}
\icmlaffiliation{meta}{Meta AI}

\icmlcorrespondingauthor{Amnon Geifman}{amnon.geifman@weizmann.ac.il}
\icmlcorrespondingauthor{Daniel Barzilai}{daniel.barzilai@weizmann.ac.il}

% You may provide any keywords that you
% find helpful for describing your paper; these are used to populate
% the "keywords" metadata in the PDF but will not be shown in the document
\icmlkeywords{Neural Networks, Neural Tangent Kernels, Kernel Methods, Preconditioning, Theory}
...
% \begin{keywords}
%   Neural Networks, Neural Tangent Kernels, Kernel Methods, Preconditioning
% \end{keywords}
...
% It has been shown that for small initialization, as the width of a network tends to infinity, the trajectory of gradient descent for a neural network is close to the trajectory of a kernel gradient descent for a family of kernels called the Neural Tangent Kernels (NTK). This equivalence usually refer to as the NTK regime. 



% This equivalence raises two questions: (1) can we leverage the NTK analysis to improve Neural Network Training? (2) Can we design a data-dependent kernel that can utilize the data distribution similar to what feature learning does in Neural Networks? \rb{Point (2) is unclear}


% Regarding the first question, accelerating the training of Neural Networks is still an open research area. Most of the efforts in this direction aim to find an optimizer that will reduce the number of iterations of an iterative optimizer such that Gradient Descent. When training in the NTK regime, the number of iterations required for convergence is inversely proportional to the condition number of the kernel matrix of the NTK. The spectrum of this matrix might decay exponentially and therefore, some specific target functions might require an exponential number of iterations. To the best of our knowledge, all existing preconditioneirs for Neural Networks overlook the spectrum of the Neural Tangent kernel. \rb{It's not clear at this point why they should look at this spectrum} 

% Regarding the second question, finding a family of kernels with desirable properties is a long-standing problem in Reproducing Kernel Hilbert Space (RKHS) theory. Classical machine learning literature provided several ways to generate new kernels, e.g., using multiplication, addition, and subtraction of two kernels. There are also more complex ways to generate kernels such as applying a mapping on the RKHS or applying a function on the kernel. Nonetheless, there is still a finite number of known kernels,  and practitioners are still restricted to using one of these finite kernels. \rb{There are parameterized kernels, such as $\gamma$-exponential, so the set of known kernels is not finite. Also, I don't think that the small number of existing kernels is a good motivation. Focus on what can be achieved with new kernels.}


% In this work, we investigate the interplay between these two questions \rb{Rephrase. Interplay between questions sounds weird} by presenting two types of manipulations. The first is a Tangent Kernel manipulation where we present a simple preconditioned gradient descent for deep neural networks. We show that under the Lazy Training assumptions, it is possible to manipulate the gradient of the network by a linear transformation to change the dynamics of the loss during gradient descent. We further show for a specific choice of a linear transformation that the convergence of the Neural Network in the Lazy Training regime becomes independent of the condition number of the NTK. This enables accelerating the convergence of gradient descent and to learn high frequencies in the target function. We also show that in the case of linear models, our manipulation does not change the underlying predictor.

% The second manipulation is kernel spectrum manipulation. In this part, we build upon our the Neural Tangent preconditioner, but in contrast, we show that this manipulation does change the underlying predictor. Specifically, our manipulation enables us to generate any Mercer kernel that shares its eigenfunctions with some known Mercer kernel. We prove theoretically that with sufficiently many samples, our manipulated kernel converges to the desired kernel. We also derive a novel algorithm to optimize the spectrum of the kernel in order to find a new kernel with the best predictive properties.

% \rb{The last two paragraphs are not clear enough}

% We back our theory with experiments showing that \ag{TBD}


% \ag{\begin{enumerate}
%     \item Convergence of gradient descent with preconditioning
%     \item Spectral bias manipulation/ convergence acceleration (with easy to manipulate)
%     \item Kernel engineering for spectral and for general kernels
%     \item Robustness of all of the above from representer Thm
% \end{enumerate}}
...
We next provide a proof sketch. The full proof is given in Appendix \ref{appendix:engineering}. For an eigenfunction $\Phi$ of $\kr(\x,\z)$, we let $\Phi(X):=(\Phi(\x_1),\ldots,\Phi(\x_n))^T\in\Real^n$ be the evaluation of the eigenfunctions on the data. By the definition of the kernels, $K=\sum_{k=1}^\infty\lambda_k\Phi_k(X)\Phi_k(X)^T$,  and similarly, $K_g$ can be written as $K_g=\sum_{k=1}^\infty g(\lambda_k)\Phi_k(X)\Phi_k(X)^T$. Since $\tilde K_g$ has the eigenvectors of $K$ with the eigenvalues $g(\lambda_k)$, we would like to say that the eigenvalues of $K_g$ are close to $g(\lambda_k)$ and that the eigenvectors of $K$ are close to those of $K_g$. It is already known that the eigenvalues of a kernel matrix converge to those of its integral operator (with the suitable normalization) \citep{rosasco2010learning}, and as such, we know that the eigenvalues of $\tilde K_g$ should be close to those of $K_g$. The challenge is that the eigenvectors $\vv_k$ do not have to be close to $\Phi_k(X)$, the evaluations of the eigenfunctions on the data (for example when there is an eigenvalue of multiplicity greater than 1). This means that the eigenvectors of $\tilde K_g$ can be very different from those of $K_g$. We work around this by showing that for large $n$, if $\vv_l,..,\vv_d$ are the eigenvectors of $K$ corresponding to eigenvalue $\lambda_k$ and $\Phi$ is some eigenfunction, then
\begin{align*}
\sum_{i=l}^d(\vv_i^T\Phi(X))^2\rightarrow 
\begin{cases} 1~ \text{ if } \Phi \text{ is an eigenfunction of } \lambda_k\\
0 ~ \text{ else,}
\end{cases}
\end{align*}
allowing us to show that the norm between the eigenspaces of $\tilde K_g$ and $K_g$ tends to 0. \\
...
% This extends known techniques  such as kernel arithmetic by enabling the construction of new families of kernels. In order to construct a kernel with some eigenvalue decay, one only needs to know how to construct a different kernel with the same eigenfunctions, and then use theorem \ref{thm:norm_conv}. For example, shift invariant kernels and dot product kernels on the unit sphere always have the same eigenfunctions \db{Citation}. Using known kernels, one can use MSK's in order to approximate any shift invariant kernel or dot product kernel using existing ones.\\

% \noindent \textbf{Accelerated Kernel Regression}\label{sec:accelerate_regression}
% To see the utility of MSKs, consider a simple case where one wishes to perform kernel ridge regression on a large dataset. Specifically, given training data $X=\{\x_1,...,\x_n\}$, corresponding labels  $\{y_i\}_{i=1}^n$, $\x_i \in \calx$, $y_i \in \Real$, and a regularization parameter $\lambda > 0$, the problem of kernel ridge regression is formulated as
% \begin{equation}  \label{eq:regression}
%     \min_{f \in \hh}\sum_{i=1}^n(f(\x_i)-y_i)^2+ \gamma\|f\|_\hh.
% \end{equation}
% % where if $\lambda\rightarrow 0$ we call the problem minimum norm interpolant, and the object becomes
% % \begin{equation}  
% %     \min_{f \in \hh}\norm{f}_\hh ~~~ \mathrm{s.t.} ~~~~ \forall i, ~ f(\x_i)=y_i.
% % \end{equation}
% The solution satisfies $f(\x)=\kr_\x^T (K+\gamma I)^{-1}\y$, where the entries of $\kr_\x \in \Real^n$ are $\kr(\x,\x_i)$, $K$ is the $n \times n$ kernel matrix with $K_{ij}=\frac{1}{n}\kr(\x_i,\x_j)$ and $\y=(y_1,...,y_n)^T$. 

% % Solving kernel ridge regression generally has cubic complexity in the size of the dataset (due to the matrix inversion) \mg{In general, the computational complexity of naive iterative methods, is comparable to the complexity of matrix inversion.}. One popular approach is to learn the kernel via gradient descent (which we will often call kernel gradient descent) to cope with this computational hurdle. Specifically, the sought function is parametrized as  $f(\x)=\sum_{i=1}^n\alpha_i\kr(\x,\x_i)$ and the optimization is over the parameters $\alpha_i\in \Real$. The Representer Theorem guarantees  that any solution of the kernel ridge regression problem must be of this form \cite{scholkopf2001generalized}.
% % \mg{I think that the terminology "gradient kernel regression" is more appropriate. By the way, how does the ridge is taken into account?}

% % The rate at which kernel gradient descent converges is inversely proportional to the condition number of the kernel matrix. \mg{I think that in the case of "gradient kernel regression," the convergence rate is inversely proportional to the squared condition number of the kernel matrix. In addition, can we introduce arguments related to "better / good" kernels? I am not sure; maybe it is too early in this context.} 

% For simplicity, suppose $\gamma \approx 0$ and $K$ is invertible. In this case, the minimizer becomes  $f(\x)=\kr_\x^T \alpha$ where $\alpha$ is the solution to the linear system $K\alpha = \y$. Such linear systems are often solved via iterative methods, where the number of iterations depends on the ratio between the largest and smallest eigenvalues of $K$ (a.k.a.\ the condition number). \db{Can we be more explicit about how the efficiency changes with the condition number?}

% Since the eigenvalue decay of $K$ is an approximation to that of the kernel $\kr$ \citep{rosasco2010learning}, given a kernel with Mercer decomposition as in \eqref{eq:mercer}, one would wish to work with a modified kernel of the form:
% \begin{align}\label{eq:eigpro_changed}
%      \hat \kr(\x,\z)=\sum_{i=1}^q \lambda_q \Phi_i(\x) \Phi_i(\z)+\sum_{i=q+1}^{\infty} \lambda_i \Phi_i(\x) \Phi_i(\z)
% \end{align}
% for some predefined $q\in\mathbb{N}$. This modified kernel squashes the top $q$ eigenvalues to be equal, and thus its kernel matrix will have eigenvalues that are approximately $\underset{\text{q times}}{\underbrace{\lambda_q,\ldots,\lambda_q}},\lambda_{q+1},\ldots,\lambda_n$, and a condition number of $\approx \frac{\lambda_q}{\lambda_n}$. Note that the functions in the RKHS (i.e the functions which can be approximated by the kernel) depend only on the asymptotic decay of the eigenvalues, and thus  remain the same. 

% The EigenPro algorithm \citep{ma2017diving,ma2019kernel} approximated the kernel in \eqref{eq:eigpro_changed} by creating a preconditioner that modifies the feature map directly. Despite gaining success and being implemented in a few open-source packages (e.g Scikit-Learn \citep{pedregosa2011scikit}), this approach suffers from two major limitations. First, it is typically intractable to work directly with the feature map as it is often infinite-dimensional or, in the case of wide neural networks, significantly larger than the number of data points. The second limitation is that it is unproven theoretically that their feature map modification indeed results in the Mercer decomposition in \eqref{eq:eigpro_changed} as they desired. 

% By contrast, the kernel in $\eqref{eq:eigpro_changed}$ can be well approximated as a MSK by taking $g(\hat{\lambda}_1),\ldots,g(\hat{\lambda}_q)=\hat{\lambda}_q$ and for $i\geq q+1$ taking $g(\hat{\lambda}_i)=\hat{\lambda}_i$. Theorem \eqref{thm:norm_conv} ensures us that the MSK indeed approximates the kernel in \eqref{eq:eigpro_changed}. Moreover, this approach does not require to work explicitly with the feature map, making the construction computationally efficient. We thus obtain a sound method for reducing the condition number of the kernel as desired. Such choice of $g$ will be further analyzed in the following section, where we will show how this sort of manipulation can be used in the context of wide neural networks.

\section{Provable NTK Based Preconditioning for Neural Networks}\label{sec:pgd_intro}

In this section, we develop and analyze a preconditioning scheme for accelerating the convergence rate of gradient descent in the training phase of neural networks while minimizing the Mean Squares Error (MSE) loss. The acceleration is achieved by manipulating the spectrum of the NTK, overcoming the spectral bias of neural networks. We begin by explaining how the convergence rate of neural networks is related to the spectrum of the NTK. Then, we introduce a preconditioning scheme for wide neural networks and prove that it attains a global optimum. We further prove that in the infinite width limit and when training the network to completion, preconditioned and standard gradient descent converge to the same global minimum. 
...
\end{align*}
where $\w\in \Real^p$ is the set of all the network parameters. We select a simple architecture and note that our results can easily extend to many other architectures. We denote by $m$ the width of the network and assume that $d_1=d_2=..=d_L=m$. The activation function is denoted by $\rho$ and the following quantities  $|\rho(0)|$, $\norm{\rho'}_\infty$, and $\sup_{x\neq x'}|\rho'(x)-\rho'(x')|/|x-x'|$ should be finite. We use standard initialization that assures that the network is trained in the NTK regime (see more details in Appendix \ref{appendix:convergence}).
...
In contrast to standard preconditioning techniques, which multiply the complete gradient with a preconditioning matrix, we manipulate only the network's gradient, hence reducing the cost per iteration for a generic preconditioner from $p^2$ (where $p$, the number of parameters in the network, is usually huge) to $n^2$, where in the over-parameterized case $n \ll p$. 


We next show an analogous result to the linear dynamics of the form of \eqref{eq:linear_dynamics} for PGD. 
The key difference, is that carefully choosing $S$ enables to manipulate the dynamics of gradient descent almost arbitrarily.

\begin{theorem}\label{Thm:precond_dynamic}
Let $\eta_0, \delta_0, \epsilon > 0$,  $S \in \Real^{n \times n}$ such that $S \succ 0$ and $\eta_0 < \frac{2}{\lambda_{min}(KS)+\lambda_{max}(KS)}$. Then, there
...
\end{align}
where now, in contrast to \eqref{eq:linear_dynamics}, $\hat \lambda_i,\hat \vv_i$ are the eigenvalues and eigenvectors of $KS$. This enables to manipulate the convergence behaviour in two ways. The first is to reduce the condition number of $K$ and therefore achieve a significant improvement in the convergence rate. The second is to change certain eigenvalues and therefore manipulate the relative convergence of each eigenfunction of the NTK.



where now, in contrast to \eqref{eq:linear_dynamics}, $\hat \lambda_i,\hat \vv_i$ are the eigenvalues and eigenvectors of $KS$. 
As such, the choice of $S$ determines the rate at which the network converges to any specific target function.
...
\end{align*}
with $F$ being the Fisher information matrix associated with the network's predictive distribution. Their derivation leads to similar convergence results as shown in \eqref{eq:linear_dynamics_PGD} but is limited to the case of $S=K_t^{-1}$.
...
\end{algorithmic}
\end{algorithm}
As an example, we may set $g(\lambda_i)=\lambda_{k+1}$, $1 \le i \le k$, making the top $k+1$ eigenvalues equal to each other. Then, due to \eqref{eq:linear_dynamics_PGD}, by picking the learning rate $\eta=\frac{2}{\lambda_{k+1}+\lambda_{n}}$, we get that for every $i\leq k+1$, $\vv_i$ can be learnt in $O(1)$ time (number of iterations). This starkly contrasts the typical spectral bias of neural networks, under which for large $i$, learning $\vv_i$ may be infeasible in polynomial time. For example, when the activation function $\rho$ is tanh, \citet{murray2022characterizing} showed that $\lambda_k=O\left(k^{-d+1/2}e^{-\sqrt{k}}\right)$. Applying \eqref{eq:linear_dynamics}, vanilla GD requires $O\left(k^{d+1/2}e^{\sqrt{k}}\right)$ iterations to converge, exponentially slower in $k$ compared to PGD.

We leave the choice of $k$, the number of modified eigenvalues, to the practitioners. Typically, the NTK matrix will be ill-conditioned with the smallest eigenvalues very close to 0. Thus, $k$ may be chosen in a way that helps avoid these numeric instabilities. Furthermore, since only the top $k$ eigenvalues and eigenvectors need to be calculated, choosing a small $k$ allows for a very efficient calculation of $S$. By contrast, choosing $k=n$ implies that $S=K^{-1}$.

Previous work showed that the networks' width in the NTK regime should generally exceed $m\sim n^4$ \citep{song2019quadratic}. Therefore, in this regime inverting $K$ or calculating its eigenvalues ($O(n^3)$ operations) is cheaper than computing the gradient ($O(n^4)$ operations). Our method is also significantly more efficient than a standard preconditioner matrix whose size is $\sim L^2m^2\sim L^2n^8$. Furthermore, in practice, one may choose $k$, the number of eigenvalues to modify, to be small, leading to further speedups.
...
Research at the Weizmann Institute was partially supported by the Israel Science Foundation, grant No. 1639/19, by the Israeli Council for Higher Education (CHE) via the Weizmann Data Science Research Center and by research grants from the Estate of Tully and Michele Plesser and the Anita James Rosen Foundation.
...
\begin{enumerate}
    \item The width of the hidden layers are identical and equal to $m$
    \item The analytic NTK $K$ is full rank with $\lambda_{min}(K)>0$. %We set $\eta_{critical}=\frac{2}{\lambda_{min}(K)+\lambda_{max}(K)}$
    \item The training set $\{x_i,y_i\}_{i=1}^n$ is contained in some compact set.
    \item The activation function $\rho$ satisfies
    \begin{align*}
...
\end{equation}
Throughout the proofs we assume that $\norm{S}$ is bounded
...
\begin{align*}
    ...
    \leq & ^{(1)} \norm{S}_F \tilde\kappa\norm{\w-\tilde \w}=: \kappa \norm{\w-\tilde \w}
\end{align*}
where $^{(1)}$ is given by Lemma (1) of \citet{lee2019wide} and $\kappa:=\tilde \kappa \norm{S}$ which is constant since $\norm{S}$ is bounded. The second part of the theorem can be shown using the same two arguments.  
\end{proof}
...
\begin{align*}
    \norm{\w_{t+1}-\w_t}\leq \eta \norm{J(\w_t)S}_{op}\norm{\res_t}_2\leq \left(1-\frac{\eta_0\bar \lambda_{min}}{3}\right)^tR_0 \kappa \eta_0 m^{-1/2}
\end{align*}
Implying that 
\begin{align*}
    \sum_{j=0}^{t-1}\norm{\w_{j+1}-\w_j}\leq  R_0 \kappa \eta_0 m^{-1/2} \sum_{j=0}^{t-1}\left(1-\frac{\eta_0\bar \lambda_{min}}{3}\right)^{j}\leq \frac{3\kappa R_0}{\bar \lambda_{min}}m^{-1/2}
\end{align*}
...
\begin{align*}
    ...
    \leq& \norm{I-\eta J(\tilde \w_t)J(  \w_t)^T S}_{2} \left(1-\frac{\eta_0\bar \lambda_{min}}{3}\right)^tR_0
\end{align*}
...
\begin{align*}
    \norm{I-\eta J(\tilde \w_t)J(  \w_t)^T S}_{2}\leq \left(1-\frac{\eta_0\bar \lambda_{min}}{3}\right)
\end{align*}
...
\begin{align*}
    \norm{K S-K_0 S}_F\leq \frac{\eta_0 \bar \lambda_{min}}{3}
\end{align*}
Therefore we get
\begin{align*}
    &\norm{I-\eta J(\tilde \w_t)J(  \w_t)^T S}_{2}\\
    ...
    &\leq (1-\eta_0\bar \lambda_{min}+\frac{\eta_0\bar \lambda_{min}}{3} +2\eta_0\kappa^2\frac{3\kappa R_0}{\bar \lambda_{min}}m^{-1/2}\leq 1-\frac{\eta_0\bar \lambda_{min}}{3}
\end{align*}
...
\begin{align}\label{eq:prec_proof_transition}
    ...
    &\leq 2\kappa^2\norm{\w_0-\w_t}_2\leq \frac{6\kappa^3R_0}{\bar \lambda_{min}}{m^{-1/2}}
\end{align}
\end{proof}
...
\begin{align*}
    ...
    \leq^{(1)} &\eta_0(\frac{6\kappa^3R_0}{\bar \lambda_{min}}m^{-1/2}+\frac{\bar \lambda_{min}\epsilon}{2R_0})R_0=\frac{6\kappa^3R_0}{\bar \lambda_{min}}m^{-1/2}+\frac{\epsilon\bar \lambda_{min}\eta_0}{2}
\end{align*}
where $(1)$ is based on Lemma \ref{lemma:conv_proof} using the fact that $K_0 S \rightarrow K  S$ in probability \citet{yang2019scaling} so we can set $m$ large enough for the inequality to hold. 

For term $B$ we have that 
\begin{align}\label{eq:jac_conv}
    \norm{\eta(J(\tilde \w )-J(\w_t))J(\w_t)^TS\res_t}& \leq \frac{\eta_0}{m}\norm{J(\tilde \w )-J(\w_t)}_{2}\norm{J(\w_t)S\res_t}_2 \nonumber\\
...
\begin{align*}
    ...
    \leq& \norm{\sum_{i=1}^t (I-\eta_0 K S)^i}(\frac{\eta_0^2 \kappa^3 R_0^3}{\sqrt{m}}+\frac{6\kappa^3R_0}{\bar \lambda_{min}\sqrt{m}} +\frac{\epsilon \bar \lambda_{min}\eta_0}{2})
    +\norm{(I-\eta K S)^tf(X,\w_0)}\\
    \leq& (\frac{\eta_0^2 \kappa^3 R_0^3}{\sqrt{m}}+\frac{6\kappa^3R_0}{\bar \lambda_{min}\sqrt{m}} +\frac{\epsilon\bar \lambda_{min}\eta_0}{2}) \sum_{i=1}^t(1-\eta_0\bar \lambda_{min})^i+(1-\eta_0 \bar\lambda_{min})^t\nu C_0\\
    &\leq (\frac{\eta_0^2 \kappa^3 R_0^3}{\sqrt{m}}+\frac{6\kappa^3R_0}{\bar \lambda_{min}\sqrt{m}} +\frac{\epsilon\bar \lambda_{min}\eta_0}{2})\frac{1}{\eta_0\bar \lambda_{min}}+(1-\eta_0 \bar\lambda_{min})^t\nu C_0\\
    =&\epsilon/2 +\frac{\eta_0^2 \kappa^3 R_0^3}{\sqrt{m}}+\frac{6\kappa^3R_0}{\bar \lambda_{min}\sqrt{m}}+(1-\eta_0 \bar\lambda_{min})^t\nu C_0
\end{align*}
...
\begin{lemma}\label{lem:krr_s}
    Let $\kr(\x,\z) = \langle \phi(\x), \phi(\z) \rangle$ be any kernel and $[K]_ij=\frac{1}{n}\kr(\x_i,\x_j)$ be its kernel matrix. Given $S\succ 0$, let  $L_S(\w)=\frac{1}{2}\norm{S^\frac{1}{2}(f(X,\w)-\y)}_2^2 + \frac{\gamma}{2}\norm{f(\cdot,\w)}_2^2$ (where $\gamma > 0$ is some regularization term).
...
\end{lemma}
\begin{proof}
We are looking to find $f(\x,\w)=\langle(\w,\phi(\x)\rangle\in\mathcal{H}$ (since every function in the RKHS can be written this way) s.t the evaluations on the training set $f(X,\w)=(f(\x_1,\w),\ldots,f(\x_n,\w))$ minimize the loss $L_S(\w)=\frac{1}{2}\norm{S^\frac{1}{2}(f(X,\w)-\y)}_2^2 + \frac{\gamma}{2}\norm{f(\cdot,\w)}_2^2$ (where $\gamma > 0$ is some regularization term).
The Representer Theorem \citep{scholkopf2001generalized} states that any minimizer of the kernel ridge regression problem $f^* = \arg\min_\w L_S(\w)$ is of the form $f^*(\x)=\sum_{i=1}^n\alpha^*_i\langle \phi(\x), \phi(\x_i))$.

Then $f^*(X)$ can be written as $K\alpha^*$ and $\norm{f^*}_2^2={\alpha^*}^TK\alpha^*$. So we can equivalently solve for:
\[
...
\]
\[
= \arg\min_\alpha \frac{1}{2}\alpha^T \left(KSK + \gamma I \right)\alpha - \alpha^TKS\y.
\]
Now deriving with respect to $\alpha$ and setting to $0$ we obtain:
\[
0=\left(KSK + \gamma I \right)\alpha - KS\y = KS\left(\left(K + \gamma S^{-1}\right)\alpha - \y\right).
\]
...
Let $h(\x,\w) = \langle \w, \nabla f(\x,\w_0) \rangle$. Observe that by the triangle inequality, 
\[
\abs{h(\x,\w'_T) - f(\x,\w_T)} \leq  \abs{f^{lin}(\x,\w'_{T}) - f(\x,\w_T)} + \abs{f(\x,\w_0)}
\]
The weights of the last layer are initialized $\mathcal{N}(0,\nu)$ with some constant $\nu$ which we may choose. Thus, by taking small enough $\nu$, $\abs{f(\x,\w_0)}<\frac{\epsilon}{3}$ with the desired probability. Notice that $\tilde \res^{lin}_{t}-\res_{t} =f^{lin}(\x,\w'_{t}) - f(\x,\w_T)$ and as such, it remains to show that $\abs{\tilde \res^{lin}_{t}-\res_{t}}<\frac{2\epsilon}{3}$
...
Now each $\xi(i)$ tends to 0 as $m$ tends to infinity. Formally, by Lemma \ref{lemma:conv_proof} there exists some $N_0\in \mathbb{N}$ s.t for all $m>N_0$, $\max_{0\leq i\leq T-1}\norm{\xi(i)}<\frac{\epsilon}{3T\norm{\eta_0K_0(\x,X)S}}$ so that the entire left hand side is at most $\frac{\epsilon}{3}$.
...
\end{align*}
Next we use \citet{braun2005spectral} (Theorem 4.10) to get that 
\begin{align*}
...
% \begin{theorem}\label{thm:eigenvector_conv}\cite{braun2005spectral}
% Let $k$ be a mercer kernel with measure $\mu$, eigenvalues $\{\lambda_i\}$ and eigenfunctions $\{\phi_i\}$. Let $\mathcal F$ be set of measurable functions on $\mathcal X$ with square integrable majorant $F$ such that for each $i\in \mathbb{N}$
% \begin{align*}
% \mathcal{F} \phi_i =\{ f \phi_i | f \in \cal{F}\}\in GC(\mu)
% \end{align*}
% Then for all $r\in \mathbb{N}$ and $\epsilon=\epsilon(r)$ it holds that
% \begin{align*}
% sup_{f,g\in\mathcal F}|\langle \pi_r^\epsilon(K)f,g\rangle_{\mu_n}-\langle \pi_r(L)f,g\rangle_\mu|\rightarrow_{a.s.}0
% \end{align*}
% where $ \pi_r^\epsilon(K)$ is the projection to the eigenspace of all eigenvalues $\lambda_i$ such that $|\lambda_i-\lambda_r|<\epsilon$. $\langle f,g\rangle_{\mu_n}=f(X)^Tg(X)$
% \end{theorem}
% \begin{corollary}\label{cor:vec_conv}
% Let $\vv_1,..,\vv_d$ be the eigenvectors of $K$ correspond to eigenvalue $\lambda_k$. Let $\phi$ be an eigenfunction of $L$ such that $\forall x, \phi(x)\leq M$. Then
% \begin{align*}
% \sum_{i=1}^d(\vv_i^T\phi(X))^2\rightarrow \begin{cases} 1~ if ~\phi ~ is ~eigenfunction~of~\lambda_k\\
% 0 ~ o.w\end{cases}
% \end{align*}
% \end{corollary}
