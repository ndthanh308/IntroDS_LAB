...
We recall that every coupling between $X^1$ and $X^2$ can be written as such, see Theorem \ref{thm:sde_coupling} in the Appendix.
...
\hline
Black-Scholes $d_1=1$ & 0.029190 & 0.016994 & -- & 0.016994 & -- \\
Heston $d_1'=1$ & 0.009507 & 0.005220 & 0.004341 & 0.004341 & 0.004341 \\
Black-Scholes $d_1=2$ & 0.014537 & 0.0084426 & 0.018756 & 0.0084426 & 0.0048761 \\
Black-Scholes $d_1=5$ & 0.0058034 & 0.003375 & -- & 0.003375 & 0.002713 \\
\hline
\end{tabular}
\vspace*{1ex}
\caption{Variance values obtained for reference and trained estimators for the different models.}
\label{tab:all}
\end{table*}
...
\end{align*}
where $Z^1, Z^3 \sim \mathcal{N}(0,1)$ are independent and $g(x) = f \circ \exp((b-\sigma^2/2)T + \sigma \sqrt{T} x)$ is non-decreasing. Defining $\bar{\rho}:= \sqrt{1-\rho^2}$, we obtain
\begin{align*}
& \varphi'(\rho) = \psi(\rho)/\bar{\rho}, \\
& \psi(\rho) := \bar{\rho} \mathbb{E}\big[ g(Z^1) g'(\rho Z^1 + \bar{\rho}Z^3) Z^1 \big] \\
& \qquad \qquad - \rho \mathbb{E} \big[ g(Z^1) g'(\rho Z^1 + \bar{\rho} Z^3) Z^3 \big].
%=: \bar{\rho} \psi_1(\rho) + \rho \psi_2(\rho).
\end{align*}
...
\end{align*}
where we used the Stein formula, stating that for every differentiable function $\phi:\mathbb{R}\to \mathbb{R}$ and $Z \sim \mathcal{N}(0,1)$ we have $\mathbb{E}[\phi'(Z)] = \mathbb{E}[Z \phi(Z)]$. We thus obtain
$$ \psi(\rho) = \bar{\rho} \mathbb{E} \big[ g'(Z^1) g'(\rho Z^1 + \bar{\rho} Z^3) \big] $$
...
\begin{remark}
With the same computations we also get
$$ \varphi''(\rho) = \mathbb{E} \big[ g''(Z^1) g''(\rho Z^1 + \bar{\rho} Z^3) \big] $$
so that if $f$ is convex but not necessarily non-decreasing, then $\varphi$ is also convex and its minimum may not be reached in $1$ neither $-1$. More precisely, its minimum is not reached in $-1$ under the condition
$$ \varphi'(-1) = \mathbb{E}[ g'(Z) g'(-Z) ] < 0 .$$
\end{remark}
