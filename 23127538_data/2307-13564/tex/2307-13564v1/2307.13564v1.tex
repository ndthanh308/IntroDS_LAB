...
(see e.g. \cite{BK}). On anisotropic problems many results in different directions have been obtained, here we quote a list of references that is obviously not exhaustive and we refer the reader to references therein to extend it: \cite{ACC,AdBF1,AC,BGM,BaCri,Barbara,C2,GGG,FVV,P,Dic1,Fragala,LI,Str}.
...
To take into account the terms $\mathcal{A}_i(x,\nabla u_n)$ we need the following lemma, which is the anisotropic version of Lemma  3.3 of \cite{LL}. Note that if $A_i$ is strongly monotone, the claim is obvious.
...
as in Lemma  3.3 of \cite{LL}. For the convenience of the reader we give some details.  Let us denote  for $i=1,\cdots,N$
