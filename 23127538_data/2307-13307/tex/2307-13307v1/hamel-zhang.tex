...
\author{Fran\c cois Hamel\thanks{Aix-Marseille Univ, CNRS, I2M, Marseille, France (\texttt{francois.hamel@univ-amu.fr}). This work has received funding from Excellence Initiative of Aix-Marseille Universit\'e~-~A*MIDEX, a French ``Investissements d'Avenir'' programme, and from the French ANR RESISTE (ANR-18-CE45-0019) project.} \ \& Mingmin Zhang\thanks{CNRS, UMR 5219, Institut de Math\'ematiques de Toulouse, Universit\'{e} de Toulouse, UPS IMT, F-31062 Toulouse Cedex 9, France (\texttt{mingmin.zhang.math@gmail.com}). M.Z. acknowledges support from the ANR via the project Indyana under grant agreement ANR-21-CE40-0008.}}
...
for $i=1,2$, $t\in[0,T]$ and $x\in I_i$, with any positive integers $p_i$ and $q_i$. Here, the functions $\overline{u}_{j,i}$ and $\underline{u}_{k,i}$, for $i=1,2$, $j\in\llbracket1,p_i\rrbracket$ and $k\in\llbracket1,q_i\rrbracket$, are all assumed to defined, bounded and continuous in $[0,T]\times\overline{I_i}$, and of class $C^{1;2}_{t;x}((0,T]\times\overline{I_i})$. One also assumes that:
\begin{itemize}
...
The main result of this paper is the following theorem on the existence of transition fronts connecting $V$ and $0$ for problem~\eqref{model}.
...
which is possible thanks to~\eqref{sub}-\eqref{underoveru}, and $\hat\phi_1(\xi)=\phi_1(\xi)\to K_1$ as $\xi\to-\infty$. Denote by $w$ the solution of the following initial-boundary value problem:
\begin{equation}
\label{w-Dirichlet}
\begin{aligned}
\begin{cases}
...
Consider now the solution of the ODE $\zeta'(t)=f_1(\zeta(t))$ for $t\ge T^*$ associated with the initial condition $\zeta(T^*)=\min(\eta,K_1-2\delta)\in(0,K_1-2\delta]$. One has $\zeta(t)\nearrow K_1$ as $t\to+\infty$ by~\eqref{hypkpp}, and there is a unique $\overline{T}\in[T^*,+\infty)$ such that $\zeta(\overline{T})=K_2-2\delta$. Using $\zeta$ as a subsolution to~\eqref{model} for~$t\in[T^*,\overline{T}]$ and $x\le x_2$, the comparison principle asserts that $\zeta(t)\le u(t,x)$ for all $t\in[T^*,\overline{T}]$ and $x\le x_2$. In particular, $u(\overline{T},x)\ge K_2-2\delta$ for all $x\le x_2$, and then
