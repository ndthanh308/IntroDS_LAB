...
\section{Theoratical understanding of the model}
\subsection{A brief discussion about causal sets}
CST is a discrete theory of gravity that is proposed to solve the problem of quantum gravity. The main principles are causality and discreteness, which are implemented at the kinematic level of the theory. Unlike some other approaches to quantum gravity where time is either non-existent [?][?] or its passage requires a foliation of spacetime [?][?], in CST it is very much an objective notion that corresponds to the birth of spacetime events in a partial order. It is not the metric that plays an essential role, but the causal relation between spacetime events. There doesn't exist any preferred frame or spacelike hypersurface that is evolving through time, nor does the whole causal set exist at once as a block universe to be approximated by a GR manifold. Instead, the elements are born in a partial order. It may be said they are born \textit{one at a time}, but that is a misnomer, since two causal set elements that are spacelike separated cannot be in any physically meaningful way be put into an order. Any such external view of the dynamics of a causal set is a gauge artifact. Yet, for the purpose of a mathematical formulation, it is indeed very useful to think of an external, gauge dependent \textit{time} variable that keeps track of the birth of causal set elements \textit{one at a time}. Such a clock ticks once every time a new element is born, and puts all spacetime events into a fiducial total order. The total order should also respect the physical partial order of causal set elements. Yet we always keep in mind that such a \textit{natural labeling} is a gauge artifact. These ideas are put together in a family of classical dynamical models for causal sets called \textit{Classical Sequential Growth} (CSG) \cite{rideout1999classical}. We believe that CSG models present a novel way for thinking about the dynamics of a physical system, without relying on any action principle or equation of motion. For the purpose of the present paper, the main takeaway lesson is that a proper time variable for quantum gravity would be the total number of causal set elements, which corresponds to spacetime volume in the continuum (number-to-volume correspondence).\\
...
Now imagine we want to perform a path integral for CST. The causal set action is known \cite{benincasa2010scalar}\cite{dowker2013causal}, and we can write down the path integral as \cite{loomis2017suppression}
\begin{equation}
    \mathcal{Z}(N)=\sum_{|C|=N}e^{iS[C]}
\end{equation}
Setting a fixed $N$ as the total number of elements when summing over different causal sets is somehow analogous to fixing two time slices when we perform path integral for a single particle. So in fact, different partial orders are being summed over, while the number of elements is kept fixed. So in the low energy limit, we expect to have a theory of continuum where in its path integral, the total volume of the spacetime is kept fixed. In such a theory of gravity, if we are to sum over all topologies and geometries, we have to be careful to keep the total volume constant. This is the global version of Unimodular Gravity.\\

\section{A brief overview of Unimodular Gravity}
Unimodular gravity was originally proposed as a mild modification of GR with the potential to solve the old cosmological constant problem, namely to address why the vacuum energy does not seem to gravitate \cite{buchmuller1989gauge}\cite{weinberg1989cosmological}\cite{sorkin1997forks}. In its \textit{global} version, one would only sum over those geometries in the path integral that keep the total volume of spacetime equal to some constant $V$, in accord with our discussion on causal sets. In the analysis of its saddle point, we should be careful not to perform a variational principle with respect to $\delta g_{\mu\nu}$ because it is (globally) constrained:
\begin{equation}
    \int d^4x \sqrt{-g}=V\ \ \Rightarrow\ \ \int d^4x\sqrt{-g}\ \delta g_{\mu\nu}g^{\mu\nu}=0
\end{equation}
Therefore, when we perform a variation of the metric in the Einstein-Hilbert action, we cannot any longer infer the full Einstein equations. Instead, what we do is to write the metric $g_{\mu\nu}$ in terms of another unconstrained metric $\hat{g}_{\mu\nu}$ as follows
\begin{equation}
\label{gghat}
...
\end{equation}
where $E_{\mu\nu}=G_{\mu\nu}+\Lambda g_{\mu\nu}-8\pi GT_{\mu\nu}$. Then for finding the classical equations of motion, we merely set the coefficient of $\delta\hat{g}_{\mu\nu}(x)$ equal to zero. A bit of algebra reveals 
\begin{equation}
\label{EEE}
...
\begin{equation}
    \langle E\rangle=\frac{\int d^4y\sqrt{-g}\ E(y)}{\int d^4y\sqrt{-g}(y)}
\end{equation}
...
\end{equation}
which is the trace-free form of the Einstein equations. The full Einstein equations can be recovered by taking the covariant derivative of \ref{EE}, as it's done in \cite{weinberg1989cosmological} for the \textit{local} version of UG. Bianchi identity and the conservation of energy-momentum tensor force $R+8\pi GT$ to be a constant as well, which we call $4\lambda$. Then 
\begin{equation}
    G_{\mu\nu}+\lambda g_{\mu\nu}=8\pi GT_{\mu\nu}
\end{equation}
This $\lambda$ is just an integration constant that should be set as initial conditions for the universe, and is totally unrelated to any coupling constant in the action or the vacuum energy (Note that we initially separate any vacuum energy from $T_{\mu\nu}$ and add its contribution to $\Lambda$, so that $T$ doesn't include vacuum energy.) As it's evident from the above discussion, UG doesn't provide any insight on the new cosmological constant problem, or on the coincidence problem. In the context of UG, it remains a totally open problem why the observed dark energy density is of the order of the present critical density. The Everpresent $\Lambda$ model adds insights from CST to UG in order to fill this gap and provide a viable explanation for the observed value of dark energy density.\\
...
\section{The Everpresent $\Lambda$ model}
The original argument for the Everpresent $\Lambda$ goes as follows. The path integral of global UG can be formally written as
\begin{equation}
\label{Zglobal}
...
\begin{equation}
    \int d^4x\sqrt{-g}\ \nabla^\mu\xi_\mu=0
\end{equation}
Having the causal set picture in mind, the total causal set elements are fixed at each instance, so $V$ cannot be fixed. But then if we insist on a continuum picture, heuristically we have to describe the universe as being in a superposition of different volumes. The distribution of $V$ should be a Poisson with mean $N$, $N$ being the total number of causal set elements. For $1\ll N$ (which is indeed the case for all cases of interest), the distribution is very well approximated by a normal distribution $\mathcal{N}(N,\sqrt{N})$. We reinterpret this distribution, which comes from a classically stochastic process, as a quantum wave function distribution. The deeper nature of this correspondence is yet to be understood. Nevertheless, it hints toward an exciting possibility to be explored. Now instead of working in the “volume space", we can perform a Fourier transform and go to the conjugate space.
\begin{equation}
...
\end{equation}
This is just the usual path integral with a cosmological constant $\Lambda$. We conclude from this piece of calculation that $V$ and $\Lambda$ are quantum mechanically conjugate. This is not a rigorous derivation, as is required from a full theory of quantum gravity. However, we see it as a well motivated conjecture to expect from discrete quantum gravities. Therefore, if our universe is in a Gaussian state in $V$-space, it should also be described with a Gaussian wave function in $\Lambda$-space. Gaussian wave functions saturate the Heisenberg uncertainty principle. Therefore we expect the cosmological state of the universe to be in a superposition of states that each have a definite $\Lambda$, with a spread equal to $\delta\Lambda$ so that
\begin{equation}
...
inferred velocity dispersion $\sigma$ of the ionized gas (e.g. Hβ, Hα, [OIII] emission lines) in extreme starburst galaxies (i.e. containing a population of O and/or B stars) may be used as a cosmological tracer to constrain cosmological model parameters~\cite{Ch_vez_2012}. Apart from these other measurements of hubble parameter comes from observation of tip of the red-giant branch (TRGB)~\cite{M_ller_2018,1999IAUS..183...48S}, Cepheid Amplitudes\cite{Riess_2020,Sabour2014} etc. Intensity mapping is a net field in astonomy which promises to provide 3D map of the universe at different redshifts. 
