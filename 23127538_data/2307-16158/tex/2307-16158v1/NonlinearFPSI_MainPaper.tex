...
\title{\sc{Fluid-poroviscoelastic structure interaction problem with nonlinear coupling}}
...
Bulk poroviscoelasticity means that the dimensions of the ``free fluid flow'' domain and the poroviscoelastic medium domain are the same. In particular, in this manuscript we consider the 2D case, see Fig.~\ref{domain}, which captures the main mathematical difficulties of such coupling. 
The free fluid flow and the Biot poro(visco)elastic medium are coupled across the current location of the interface, which has inertia and elastic energy, modeled by
the reticular plate equation. A reticular plate is a lattice-type structure characterized by
...
To get around these difficulties, we take the following approaches. First, the reticular plate at the interface associates mass and elastic energy to the interface, and regularizes the boundary of the fluid domain. This is, however, not sufficient to deal with the fact that in the nonlinearly coupled problem, the Biot domain is also moving, and as a result certain integrals in the weak formulation over the moving Biot domain are not well defined. This is why we introduce a ``consistent {\bf{regularized}} weak formulation'' of the coupled problem by introducing a suitably constructed convolution in spatial variables and regularizing only the problematic terms in the weak formulation of the coupled problem. We then prove the existence of a weak solution to the regularized problem and show that as the regularization parameter tends to zero, this  solution converges to the solution of 
the original nonregularized problem in the case when the original problem has a {{classical}} solution and the Biot poroelastic matrix is viscoelastic, {{where a classical solution is a solution that is smooth both temporally and spatially that hence satisfies the original system of PDEs for the original FPSI problem pointwise}}. 
{{We call this type of result a {\bf{weak-classical consistency}} result.}} {Namely, we prove that if a classical solution for the fluid-poroviscoelastic structure interaction (FPSI) problem without regularization exists on time-interval [0,T], then a sequence of weak solutions to the regularized FPSI problem constructed here, converges to the classical solution on $[0,T]$ as the regularization parameter converges to $0$.}
...
We mention that the existence of a weak solution to the regularized problem was considered by the authors of this manuscript in \cite{NonlinearFPSI_CRM23}, where only the main 
steps in the proof were outlined. Here we present details of that proof, and show the {{weak-classical consistency}} result. Therefore, in this manuscript 
{\emph{we prove
...
Navier-Stokes equations, and a structure consisting of two solids -- a thick poroviscoelastic medium modeled by the Biot equations, and a thin interface with mass modeled by a reticular plate equation.}}
 ...
 to the regularized problem holds in the purely poroelastic case (and in the viscoelstic case). Viscoelasticity of the Biot poroelastic matrix is needed only in the proof of {{weak-classical}} consistency. 

One of the interesting features of this work is that the proof of the existence of a weak solution is constructive.
The main steps of the proof can be used to construct a numerical scheme to capture the physical solution to the problem \cite{AndrewPartitionedSchemeFPSI}. The main idea of the proof is based on semidiscretizing the regularized FPSI problem in time by subdividing the time interval into $N$ subintervals of width $\Delta t$. 
...
physical domain. We switch between the reference domain formulation and moving domain formulation in the proof as needed. 
{{For each $\Delta t$,}} approximate solutions are constructed by ``solving'' the sequence of semidiscretized (linearized) problems defined on the current (approximate) moving domain for each $t_n = n \Delta t, n = 1,\dots,N$. 
{{For each fixed $\Delta t$,}} we obtain energy estimates uniform in $\Delta t$, which allow us to deduce the existence of 
weakly and weakly* convergent subsequences. Since the problem is highly nonlinear, these are not sufficient to pass to the limit in the weak formulations
of {{the approximate problems}}. Strong convergence of approximate sequences is then obtained by using
...
and for which we can show converge to a test function of the limiting, continuous problem. Luckily, in contrast with the classical fluid-elastic structure interaction problems, in our case the 
...
filtrating flow through the poroelastic medium only locally in time. However, using the approaches presented in \cite[Section 5]{CDEM} the time of existence can be extended to
the maximal time until either (1) the {{moving fluid domain or Biot domain}} degenerates (e.g., the interface touches the bottom of the fluid domain {{or the top of the Biot domain}}), (2) the pores in the poroelastic matrix
denegerate in the sense that the Lagrangian mapping stops being injective, or (3) $T = \infty$.

We finish this manuscript by addressing the {{weak-classical consistency}} of the regularized problem, namely, we prove, using a Gronwall-type estimate, that the energy of the difference
...
There is extensive past work on fluid-structure interaction studying fully coupled systems involving incompressible, Newtonian fluids interacting with elastic structures. In many fluid-structure interaction models considered in the literature, the solid structure, which is elastic or deformable, is modeled by equations of elasticity. The models first considered in the literature are linearly coupled fluid-structure interaction models \cite{barGruLasTuff2, BarGruLasTuff, KukavicaTuffahaZiane}, which pose the fluid equations on a fixed reference fluid domain, as a linearization that approximates real-life dynamics well when structure displacements 
and deformations are small. In cases when displacements and deformations of the structure are large, they can significantly affect the dynamics of the fluid 
in which case time-dependent moving fluid domains that depend on the displacement itself must be taken into account. 
There has been extensive work on studying such nonlinearly coupled models \cite{BdV1,CDEM,ChengShkollerCoutand,ChenShkoller,CSS1,CSS2,CG,Grandmont16,FSIforBIO_Lukacova,IgnatovaKukavica,ignatova2014well,Kuk,LengererRuzicka,Lequeurre,MuhaCanic13, BorSun3d,BorSunMultiLayered,BorSunNonLinearKoiter,BorSunSlip,Raymond}, in which the time-dependent and a priori unknown fluid domain evolves according to the displacement of the structure, giving rise to a fully coupled problem with two-way coupling between the fluid and structure that has significant geometric nonlinearities arising from the moving boundary. 
As a result of past work in fluid-structure interaction, significant progress has been made within the past 20 years in the mathematical analysis of fluid-structure systems involving incompressible fluids and elastic structures. 
...
in which case poroelasticity of the material needs to be taken into account. The study of poroelasticity was initiated in studies by Biot modeling soil consolidation \cite{Biot1, Biot2}, but has since been extended to broader applications. Such porous media and poroelastic materials are found in applications to geoscience, including the study of fractures in porous and poroelastic materials \cite{GWG15, LDQ11} and more recently, applications to biomedical science, including the study of the ocular poroelastic tissue known as the lamina cribrosa, which is related to understanding the onset of glaucoma \cite{CGH14}, and the modeling of intestinal walls by equations of poroelasticity \cite{YRC14}. In addition to modeling, the mathematical study of poroelastic materials and the Biot equations in terms of well-posedness has also been an active area of research \cite{Biotwell1, Biotwell2, Biotwell3, Biotwell4, Biotwell5, Biotwell6, Biotwell7, Biotwell8,BiotWell22,BiotWell23}. 
...
These coupled problems are referred to as fluid-poroelastic structure interaction (FPSI) problems, and have been analyzed, for instance, in \cite{AEN19, Ces17, Sho05}, where linear coupling between the free fluid equations and poroelastic medium was assumed. 
Recent progress in the design of bioartificial organs, see e.g., \cite{FluidsCanic}, sparked the need to study FPSI problems in which the fluid-structure interface has mass and elastic or poroelastic properties itself. Namely, in the recent work on the design of 
...
The resulting mathematical problem in \cite{FluidsCanic} is a 
fluid-poroelastic structure interaction problem in which the structure consists of two layers: a thin poroelastic plate located at the interface between
the free fluid flow and a thick poroelastic medium modeled by the linear and nonlinear Biot equations, coupled over a fixed interface (linearized coupling).
The well-posedness for this problem was studied in \cite{BCMW21} for both the linear and nonlinear Biot equations, where the nonlinearity refers to 
the dependence of the permeability tensor in the Biot equations on the fluid content. 
In this work the fluid-structure interface with mass serves as a regularizing mechanism and 
provides sufficient information about the regularity of the interface and the free fluid domain to allow, for the first time, the proof of the existence of 
 ...
 nonlinear coupling over the moving interface. Such problems, however, are of importance in many applications, including the flow of blood in coronary arteries 
...
We study fluid-structure interaction between the flow of an incompressible, viscous fluid and a multilayered poro(visco)elastic structure
...
principle of the filtrating fluid. See, e.g., \cite{NonlinearFPSI1, NonlinearFPSI2}.
...
%
Here, $\bd{D}$ denotes the symmetrized gradient, $\mu_{e}$ and $\lambda_{e}$ are the Lam\'{e} parameters related to the elastic stress, $\mu_{v}$ and $\lambda_{v}$ are the corresponding parameters related to the viscoelastic stress, and $\hat{\bd{\Phi}}^{\eta}_{b}$ is the Lagrangian map defined above. 
...
\end{equation*}
We remark that in the last term of the Piola-Kirchhoff stress tensor \eqref{Biotstress}, we have used the Piola transform, which is a transformation that maps tensors in Lagrangian coordinates to corresponding tensors in Eulerian coordinates in such a way that divergence-free tensors in Lagrangian coordinates remain divergence free in Eulerian coordinates \cite{Ciarlet}. 
...
are governed by a  plate-type equation, defined on the equilibrium middle surface of the homogenized plate or shell $\hat\Gamma$. 
...
In the present work, we will assume that it is equal to $1$. 
...
\begin{itemize}
%
\item The Biot Cauchy stress tensor, defined on the physical domain, is obtained by applying the Piola transform to the Biot Cauchy stress tensor 
$\hat{S}_{b}(\nabla \boldsymbol{\eta}, p)$ on the reference domain:
\begin{align}
...
\subsection{Preview of the main results}
 Our first main result is the existence of a weak solution to a regularized FPSI problem, introduced in Sec.~\ref{regularized}. 
 The existence result holds for both elastic and {{viscoelastic}} Biot material. Here we state the theorem informally and refer the reader to Theorem \ref{MainThm1} for the precise statement.
\begin{theorem}[Existence of a weak solution to the regularized problem]
	...
	\begin{itemize}
	\item the fluid or Biot domain degenerates so that the moving interface collides with the bottom boundary of $\hat{\Omega}_{b}$ or the top boundary of $\hat{\Omega}_{b}$) or
...
Next, we multiply the structure equation by a test function $\hat{\bd{\psi}}$ to obtain
\begin{align*}
...
Finally, we test the second equation corresponding to the evolution of the pore pressure for the Biot poroviscoelastic medium with a test function $r$, and  recall the definition of the Darcy velocity $\bd{q}$ from \eqref{darcy},  keeping in mind that $\bd{n}$ is the \textit{inward} normal vector to $\Omega_{b}(t)$:
\begin{align*}
...
\end{align*}
where we used the coupling conditions \eqref{bjs} and \eqref{pressurebalance} in the last step. 

The weak formulation then follows by summing everything together. 
...
\begin{itemize}
\item The regularized Biot displacement obtained by extending $\hat{\boldsymbol{\eta}}$ to $\tilde{\Omega}_{b}$ by odd extension and defining:
\begin{equation}\label{etadelta}
\hat{\boldsymbol{\eta}}^{\delta} = \hat{\boldsymbol{\eta}} * \sigma_{\delta}, \qquad \text{ on } \hat{\Omega}_{b},
\end{equation}
...
Before we state the definition of a weak solution to the regularized problem, we introduce the following notation.
Define the {\bf{transverse velocity}} of the plate by the variable $\hat{\zeta}$, so that 
...
To transform the first term in the weak formulation  \eqref{deltaweakphysical} above, we use  \eqref{womega} to transform the time derivatives  and  assume that $|\hat{\omega}| < R$ so that there is no domain degeneracy. After using \eqref{womega} and \eqref{transformedgradient} we get
...
Here we show that the regularized problem is defined in a way that preserves the variational structure of the problem. More precisely, we formally prove that a weak solution to the regularized problem satisfies the following energy equality.
\begin{lemma}
...
\end{equation*}
For the splitting scheme we will work on the fixed reference domain and hence, we will semi-discretize the regularized weak formulation \eqref{regularweakref} on the fixed reference domain. Backwards Euler discretization will be used to approximate time derivatives, with the following shorthand notation:
\begin{equation*}
...
\end{equation}
so  that $\omega^{n + \frac{1}{2}}_{N} \in H_{0}^{2}(\Gamma)$ above satisfies the following weak formulation:
\begin{equation*}
...
To deal with the first difficulty and recover the coercive structure of the problem, the test functions can be rescaled so that
...
To deal with the second difficulty, one can show by explicit calculation that the following Korn's inequality holds for this problem:
%
...
the weak formulation involves the following coercive and continuous bilinear form:
\begin{align*}
&B[\bd{u}, \bd{v}, \bd{\eta}, \bd{\psi}, p, r] := (\Delta t)^{2} \int_{\Omega_{f}} \left(1 + \frac{\omega^{n}_{N}}{R}\right) \boldsymbol{u} \cdot \boldsymbol{v} \\
...
B[\bd{u}^{n + 1}_{N}, \bd{v}, \bd{\eta}^{n + 1}_{N}, \bd{\psi}, p^{n + 1}_{N}, r] = (\Delta t)^{2} \int_{\Omega_{f}} \left(1 + \frac{\omega^{n}_{N}}{R}\right) \boldsymbol{u}^{n}_{N} \cdot \boldsymbol{v} - \frac{1}{2} (\Delta t)^{2} \int_{\Gamma} \boldsymbol{\eta}^{n}_{N} \cdot \boldsymbol{n}^{\omega^{n}_{N}} (\boldsymbol{u}^{n}_{N} \cdot \boldsymbol{v}) \\
...
We now show that the bilinear form $B[\bd{u}, \bd{v}, \bd{\eta}, \bd{\psi}, p, r]$ is coercive and continuous as a bilinear form on the Hilbert space 
$
\mathcal{V}^{\omega^{n}_{N}}_{f} \times V_{d} \times V_{p},
$
...
To show coercivity we calculate $B[\boldsymbol{u}, \boldsymbol{u}, \boldsymbol{\eta}, \boldsymbol{\eta}, p, p]$.
...
\begin{multline*}
B[\boldsymbol{u}, \boldsymbol{u}, \boldsymbol{\eta}, \boldsymbol{\eta}, p, p] := (\Delta t)^{2} \int_{\Omega_{f}} \left(1 + \frac{\omega^{n}_{N} + \omega^{n + 1}_{N}}{2R}\right) |\boldsymbol{u}|^{2} + 2\nu (\Delta t)^{3} \int_{\Omega_{f}} \left(1 + \frac{\omega^{n}_{N}}{R}\right) \left|\boldsymbol{D}^{\omega^{n}_{N}}_{f} (\boldsymbol{u})\right|^{2} \\
...
This  formulation implies {uniform energy estimates} for the 
 following discrete energy and discrete dissipation:
...
\end{equation}
\item {\sc{Assumption 2B: Uniform invertibility of the ALE mapping (Jacobian).}} There exists a positive constant $c_{0}$ such that for all $N$,
\begin{equation}\label{2B}
\det(\bd{I} + \nabla(\bd{\eta}^{n}_{N})^{\delta}) \ge c_{0} > 0, \qquad \text{ for all } n = 0, 1, ..., N. 
\end{equation}
\item {\sc{Assumption 2C: Uniform boundedness of the ALE mapping (matrix norm).}} There exists positive constants $c_{1}$ and $c_{2}$ such that for all $N$,
...
The uniform boundedness of $\bd{\eta}_{N}$ in $L^{\infty}(0, T; H^{1}(\Omega_{b}))$ follows from combining the uniform energy estimates with Korn's inequality, stated in Proposition \ref{Korn}. To establish the uniform boundedness of $p_{N}$ in $L^{2}(0, T; H^{1}(\Omega_{b}))$, we recall that by the uniform dissipation estimate,
\begin{equation*}
\sum_{n = 1}^{N} \kappa (\Delta t) \int_{\Omega_{b}} \mathcal{J}^{(\eta^{n}_{N})^{\delta}}_{b} |\nabla^{(\eta^{n}_{N})^{\delta}}_{b} p^{n + 1}_{N}|^{2} \le C,
\end{equation*}
...
\begin{equation*}
(\Delta t) \sum_{n = 1}^{N} \int_{\Omega_{b}} |\nabla^{(\eta^{n}_{N})^{\delta}}_{b} p^{n + 1}_{N}|^{2} \le C.
\end{equation*}
...
\begin{equation*}
(\Delta t) \sum_{n = 1}^{N} \int_{\Omega_{b}} |\nabla p^{n + 1}_{N}|^{2} \le |\bd{I} + \nabla (\bd{\eta}^{n}_{N})^{\delta}|^{2} \cdot (\Delta t) \sum_{n = 1}^{N} \int_{\Omega_{b}} |\nabla^{(\eta^{n}_{N})^{\delta}}_{b} p^{n + 1}_{N}|^{2} \le C,
\end{equation*}
for a constant $C$ independent of $N$. Thus,  $p_{N}$ is uniformly bounded in $L^{2}(0, T; H^{1}(\Omega_{b}))$. 
...
\textbf{Step 1.} We first find a condition on $T$ such that Assumption 1B \eqref{1Anew}  is satisfied. Suppose that the linear interpolation $\overline{\omega}_{N}$ is defined up to time $(k + 1)\Delta t$. Then, by \eqref{energybound2} and \eqref{energybound3}, we have
\begin{equation}\label{Lipschitzomega}
||\overline{\omega}_{N}||_{W^{1, \infty}(0, (k + 1)\Delta t; L^{2}(\Gamma))} \le C, \qquad ||\overline{\omega}_{N}||_{L^{\infty}(0, (k + 1)\Delta t; H_{0}^{2}(\Gamma))} \le C,
\end{equation}
where $C$ depends only on $E_{0}$ and is independent of $N$. Thus, following the method in \cite{MuhaCanic13}, we obtain by an interpolation inequality that for all $t, t + \tau \in [0, (k + 1)\Delta t]$ with $\tau > 0$, 
...
\end{equation}
Here, we used a Sobolev interpolation inequality, see for example Theorem 4.17 (pg.~79) of \cite{Adams}. By the Lipschitz continuity of $\overline{\omega}_{N}$ taking values in $L^{2}(\Gamma)$ and by the boundedness of $\overline{\omega}_{N}$ in $H_{0}^{2}(\Gamma)$, \begin{equation}\label{holderplate}
||\overline{\omega}_{N}(t + \tau) - \overline{\omega}_{N}(t)||_{H^{1}(\Gamma)} \le C \cdot \tau^{1/2} 
\end{equation}
for a constant $C$ depending only on $E_{0}$ (and in particular, not depending on $k$ or $N$). Therefore, setting $t = 0$ and $\tau = (k + 1)\Delta t$ and using the continuous embedding of $H^{1}(\Gamma)$ into $C(\Gamma)$,
\begin{equation}\label{estT1}
||\omega^{k + 1}_{N} - \omega_{0}||_{C(\Gamma)} \le C \cdot [(k + 1)t]^{1/2} \le C \cdot T^{1/2},
\end{equation}
...
\textbf{Step 2.} Next, we find a condition on $T$ so that the remaining assumptions \eqref{1B}, \eqref{2B}, and \eqref{2C} are satisfied. 
...
\end{equation*}
for $C$ depending only on $E_{0}$. By the odd extension defined in Definition \ref{extension},
\begin{equation*}
...
\end{equation*}
for a constant $C$ depending only on $E_{0}$, where the estimate $||\omega^{k + 1}_{N} - \omega_{0}||_{L^{2}(\Gamma)} \le CT$ follows from the bound \eqref{Lipschitzomega}. By regularization, we then have that for a constant depending only on $\delta$ and $E_{0}$,
\begin{equation*}
...
The uniform boundedness of the linear interpolation of the plate displacement $\overline{\omega}_{N}$ 
 in $W^{1, \infty}(0, T; L^{2}(\Gamma))$ and $L^{\infty}(0, T; H_{0}^{2}(\Gamma))$ implies the following strong convergence result:
...
\end{equation*}
for arbitrary $0 < s < 2$. Once we observe that $\overline{\omega}_{N}(n\Delta t) = \omega_{N}(t)$ for $n\Delta t \le t < (n + 1)\Delta t$, this follows immediately from the above H\"{o}lder continuity estimate, as
\begin{equation*}
...
\left|\left|\frac{\tau_{\Delta t}(\bd{\xi}_{N}, \zeta_{N}) - (\bd{\xi}_{N}, \zeta_{N})}{\Delta t}\right|\right|_{L^{1}(\tau, T; \mathcal{Q}_{v}')} + ||(\bd{\xi}_{N}, \zeta_{N})||_{L^{\infty}(0, T; L^{2}(\Omega_{b}) \times L^{2}(\Gamma))} \le C.
\end{equation}
...
We will obtain convergence of the fluid velocity along a subsequence by using a {\emph{generalized Aubin-Lions compactness theorem}} for functions defined on moving domains \cite{AubinLions}. The reason we must use a {generalized Aubin-Lions compactness theorem} is that the approximate fluid velocities are defined on different time-dependent fluid domains. To prepare for an application of the generalized Aubin-Lions compactness argument we will 
...
\subsubsection{Velocity convergence via a generalized Aubin-Lions compactness argument}
...
\begin{proof}
The proof is based on using the generalized Aubin-Lions compactness theorem, Theorem 3.1 in \cite{AubinLions}, for problems on moving domains. 
...
The sequence $\bd{u}_{N}$ constructed in this manuscript, however, {\emph{does not satisfy this property}}. Never the less, $\bd{u}_{N}$ satisfy the following
...
First notice that because $||\bd{v}||_{Q^{n}_{N}} \le 1$, and by the definition of $Q^{n}_{N}$ in \eqref{VnN}, we have that $\bd{v}$ is bounded in $H^{3}(\Omega^{n}_{f, N})$, and hence, $\bd{v}$ and $\nabla \bd{v}$ are bounded pointwise. Furthermore, by the boundedness of the fluid velocity $\bd{u}^{n}_{N}$ on the reference domain due to the uniform energy estimate, and by the uniform boundedness of the Jacobian of the ALE map $\Phi^{\omega^{n}_{N}}_{f}$, we obtain the following bound:
\begin{align*}
&\frac{1}{2} \left|\int_{\Omega^{n}_{f, N}} \left[\left(\left(\tilde{\boldsymbol{u}}^{n}_{N} - \zeta^{n + \frac{1}{2}}_{N} \frac{R + y}{R + \omega^{n}_{N}} \boldsymbol{e}_{y}\right) \cdot \nabla \boldsymbol{u}^{n + 1}_{N}\right) \cdot \boldsymbol{v} - \left(\left(\tilde{\boldsymbol{u}}^{n}_{N} - \zeta^{n + \frac{1}{2}}_{N} \frac{R + y}{R + \omega^{n}_{N}} \boldsymbol{e}_{y} \right) \cdot \nabla \boldsymbol{v}\right) \cdot \boldsymbol{u}^{n + 1}_{N}\right]\right| \\
...
&\le C\left(||\bd{u}^{n + 1}_{N}||_{H^{3/4}(\Omega_{f})} \cdot ||\bd{u}^{n}_{N}||_{H^{3/4}(\Omega_{f})}  + ||\bd{\dot{\eta}}^{n + 1}_{N}||_{L^{2}(\Gamma)} \cdot ||\bd{u}^{n}_{N}||_{H^{3/4}(\Omega_{f})} \right) \cdot ||\bd{v}||_{H^{3}(\Omega_{f})} \\
...
\end{align*}
The second to last term in \eqref{aux} is estimated as follows:
\begin{align*}
...
with the fact that $\bd{u}_{N}$ and $\bd{u}$ are all uniformly bounded in $L^{2}(0, T; H^{1}(\Omega_{f}(t)))$ for all $N$,
 to deduce strong convergence of the traces of the fluid velocities using the previous elementary lemma. We do this in the following steps.
...
&\le \int_{0}^{T} \int_{0}^{L} \int_{-R}^{\min(0, y^{*}(t, x))} \left(\int_{y + \left(1 + \frac{y}{R}\right)\tau_{\Delta t}\omega_{N}}^{y + \left(1 + \frac{y}{R}\right)\omega} |\partial_{r} \bd{u}(t, x, y')|^{2} dy' \right) \cdot \left(1 + \frac{y}{R}\right) \cdot |\omega - \tau_{\Delta t} \omega_{N}|.
\end{align*}
We note that $\tau_{\Delta t} \omega_{N} \to \omega$ pointwise uniformly on $[0, T] \times \Gamma$ as $N \to \infty$
by Proposition \ref{platedisplacement}, which implies $\overline{\omega}_{N} \to \omega$ in $C(0, T; H^{s}(\Gamma))$ for $0 < s < 2$, and 
by the estimate \eqref{holderplate}.
Combining this with the fact that $||\nabla \bd{u}||_{L^{2}(0, T; L^{2}(\Omega^{\omega}_{f}(t)))}$ is bounded, we have that $I_{2, 1} \to 0$ as $N \to \infty$. 

Next, by Poincare's inequality,
\begin{align*}
I_{2, 2} &\le \int_{0}^{T} \int_{0}^{L} |\min(0, y^{*}(t, x))| \cdot \max_{w \in [-R, \omega(t, x)]} |\bd{u}(t, x, w)|^{2} \\
&\le \int_{0}^{T} \int_{0}^{L} |\min(0, y^{*}(t, x))| \cdot \int_{-R}^{\omega(t, x)} |\partial_{r} \bd{u}(t, x, y')|^{2} dy',
\end{align*}
so we conclude that $I_{2, 2} \to 0$ as $N \to \infty$ by the fact that $|\min(0, y^{*}(t, x))| \to 0$ uniformly on $[0, T] \times \Gamma$, and by the boundedness of $||\nabla \bd{u}||_{L^{2}(0, T; L^{2}(\Omega^{\omega}_{f}(t))}$. Thus, we have that $||\hat{\bd{u}}_{N} - \hat{\bd{u}}||_{L^{2}(0, T; L^{2}(\Omega_{f})))} \to 0$. 
...
We are now in a position to pass to the limit in the semidiscrete formulation. From \eqref{semi1} we obtain that for all $(\tilde{\bd{v}}_{N}, \varphi, \bd{\psi}, r)$ in the test space with $\bd{v} \in \mathcal{X}$, the following holds:
\begin{align*}
...
\end{align*}
{{By using the weak and weak-star convergences of the approximate solutions, stated in Proposition \ref{prop:weak} and lower semicontinuity, we can pass to the limit in the energy inequality, one recovers the energy inequality \eqref{energyinequality}.}}
\end{proof}
...
\item We will use $(\bd{u}, \bd{\eta}, p, \omega)$ to denote a spatially and temporally smooth solution $(\bd{u}, \bd{\eta}, p, \omega)$ to the original FPSI problem \eqref{Biot1}, \eqref{Biot2}, \eqref{plate}, \eqref{NS1}, \eqref{mass}-\eqref{pressurebalance};
...
\nonumber \\
&- \alpha \int_{0}^{t} \int_{\Omega_{b}(s)} p \nabla \cdot \boldsymbol{\psi} - c_{0} \int_{0}^{t} \int_{\Omega_{b}} p \partial_{t}r - \alpha \int_{0}^{t} \int_{\Omega_{b}(s)} \frac{D}{Dt} \boldsymbol{\eta}_{1} \cdot \nabla r - \alpha \int_{0}^{t} \int_{\Gamma(s)} (\boldsymbol{\xi}_{1} \cdot \boldsymbol{n}) r 
\nonumber \\
&+ \kappa \int_{0}^{t} \int_{\Omega_{b}(s)} \nabla p \cdot \nabla r - \int_{0}^{t} \int_{\Gamma_{1}(s)} ((\boldsymbol{u} - \boldsymbol{\xi}_{1}) \cdot \boldsymbol{n}) r 
\nonumber \\
...
$\bd{u}$ and $\bd{u}_{\delta}$ and test formally with the difference of the two solutions $\bd{v} = \bd{u} - \bd{u}_{\delta}$. 
 However, the functions $\bd{u}$ and $\bd{u}_{\delta}$ are defined on different domains, and hence, the difference $\bd{u} - \bd{u}_{\delta}$ is not well-defined. Therefore, we will have to use a transformation to bring a divergence-free function defined on one fluid domain to a divergence-free function on another fluid domain. 
...
\begin{enumerate}
\item $(\bd{\eta}_{\delta}, \omega_{\delta}, p_{\delta}, \bd{u}_{\delta})$ is uniformly defined on the time interval $[0, T]$ for all $\delta > 0$;
...
E_{\delta}(t) &:= ||(\widehat{\bd{u}}_{1} - \bd{u}_{\delta})(t)||^{2}_{L^{2}(\Omega_{f,\delta}(t))} + \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}}_{1} - \bd{u}_{\delta})(s)||^{2}_{L^{2}(\Omega_{f,\delta}(s))} ds 
\nonumber \\
&+ ||(\bd{\xi} - \bd{\xi}_{\delta})(t)||_{L^{2}(\Gamma)}^{2} + ||(\omega - \omega_{\delta})(t)||^{2}_{H^{2}(\Gamma)} + ||(\bd{\xi} - \bd{\xi}_{\delta})(t)||^{2}_{L^{2}(\Omega_{b})} 
\nonumber \\
&+ ||\bd{D}(\bd{\eta} - \bd{\eta}_{\delta})(t)||^{2}_{L^{2}(\Omega_{b})} + ||(\nabla \cdot (\bd{\eta} - \bd{\eta}_{\delta}))(t)||_{L^{2}(\Omega_{b})} + \int_{0}^{t} ||\bd{D}(\bd{\xi} - \bd{\xi}_{\delta})(s)||^{2}_{L^{2}(\Omega_{b})} ds 
\nonumber \\
&+ \int_{0}^{t} ||\nabla \cdot (\bd{\xi} - \bd{\xi}_{\delta})(s)||_{L^{2}(\Omega_{b})} + ||(p - p_{\delta})(t)||^{2}_{L^{2}(\Omega_{b})} + \int_{0}^{t} ||\nabla(p - p_{\delta})(s)||^{2}_{L^{2}({\Omega}^\delta_{b,\delta}(s))}.
\end{align}
\end{enumerate}
\end{theorem}
...
E_{\delta}(t) \le C\left(\int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}^\delta||^{2}_{L^{2}(\Omega_{b})} + \int_{0}^{t} E_{\delta}(t) ds\right)
\end{equation}
...
To do this, we will test the weak formulations for $\bd{u}$ and $\bd{u}_{\delta}$ with appropriate test functions and use the energy inequality \eqref{energyestimate}.
...
\eqref{Gronwall} for the energy difference $E_{\delta}(t)$. Details will be presented in Section~\ref{weakstrongestimate};
\item Construct a bootstrap argument. Namely, at a first pass, Gronwall's inequality is proven locally in time, namely, on the interval $[0,T_\delta]$ along which the assumptions on the determinant of the Jacobian of the transformation  from the moving domain to the fixed domain is bounded for the solution of the regularized problem. However, we need the Gronwall's inequality to hold along the entire time interval 
$[0,T]$, along which the {{classical solution}} is defined. This will be done by a construction of a bootstrap argument, see Section~\ref{bootstrap}.
\item Apply Gronwall's inequality to \eqref{Gronwall} holding on $[0,T]$ to obtain the following bound for  $E_{\delta}(t)$:
\begin{equation*}
E_{\delta}(t) \le C\delta^{3}e^{Ct},
\end{equation*}
...
Let \eqref{det}, \eqref{norm}, and \eqref{grad} hold for almost all $t \in [0, T_{\delta}]$.
Furthermore, let $\bd{\eta}$ and ${\bd{\eta}}^\delta$ denote the smooth solution and its regularization, defined on $[0,T]$, and 
$E_\delta$ be the energy norm difference \eqref{energydiff}.
Then the following inequality hold:
\begin{equation}\label{Gronwall}
E_{\delta}(t) \le C\left(\int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}^\delta||^{2}_{L^{2}(\Omega_{b})} ds + \int_{0}^{t} E_{\delta}(t) ds\right),
\end{equation}
...
The transformation of fluid velocities from one domain to another, while preserving the divergence-free condition, will be performed using the following matrix:
...
\end{pmatrix} \cdot \bd{u}\left(x, \frac{R + \omega_{\delta}(t, x)}{R + \omega(t, x)}(R + y) - R\right).
\end{equation*}
...
\begin{align}
T_{1} = &-\int_{0}^{t} \int_{\Omega_{f}(t)} \bd{u} \cdot \partial_{t} \left[\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}\right] - \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}] + \int_{\Omega_{f}(t)} \bd{u}(s) \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}](s) 
\\
& - \int_{\Omega_{f}(0)} \bd{u}(0) \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}](0) - \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \bd{u}_{\delta} \cdot \partial_{t} \widehat{\bd{u}} - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \bd{u}_{\delta} \cdot \widehat{\bd{u}} 
\nonumber\\
&+ \int_{\Omega_{f,\delta}(t)} \bd{u}_{\delta}(s) \cdot \widehat{\bd{u}}(s) - \int_{\Omega_{f}(0)} \bd{u}_{\delta}(0) \cdot \widehat{\bd{u}}(0) + \frac{1}{2} \int_{\Omega_{f,\delta}(t)} |\bd{u}_{\delta}(s)|^{2} - \frac{1}{2} \int_{\Omega_{f,\delta}(0)} |\bd{u}_{0}|^{2}.
\label{T1}
\end{align}
...
where
\begin{align*}
{{|R_{1}|}} \le &\epsilon \int_{0}^{T} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{H^{1}(\Omega_{f,\delta}(t))} \\
&+ C(\epsilon) \left(\int_{0}^{T} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{T} ||\partial_{t}\omega - \partial_{t}\omega_{\delta}||_{L^{2}(\Gamma)}^{2} + \int_{0}^{T} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{align*}
...
\begin{align}
T_{2} = &\frac{1}{2} \int_{0}^{t} \int_{\Omega_{f}(t)} ((\bd{u} \cdot \nabla) \bd{u}) \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}] - \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f}(t)} (\bd{u} \cdot \nabla) [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}] \cdot \bd{u} 
\nonumber\\
&- \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} ((\bd{u}_{\delta} \cdot \nabla) \bd{u}_{\delta}) \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) + \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} ((\bd{u}_{\delta} \cdot \nabla) (\widehat{\bd{u}} - \bd{u}_{\delta})) \cdot \bd{u}_{\delta}.
\label{T2}
\end{align}
...
\begin{equation*}
|T_{2}| \le \epsilon \int_{0}^{t} ||\nabla(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{equation*}
...
\begin{align}
T_{3} = &\frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{u} \cdot \bd{n} - \bd{\xi} \cdot \bd{n}) \bd{u} \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}] - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{u}_{\delta} \cdot \bd{n}_{\delta} - \bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \bd{u}_{\delta} \cdot \widehat{\bd{u}} 
\nonumber \\
&+ \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} |\bd{u}|^{2} (\bd{\xi} \cdot \bd{n} - \bd{u} \cdot \bd{n}) - \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} |\bd{u}|^{2} [(\bd{\xi}_{\delta})_{\ttt} \cdot \bd{n} - (\widecheck{\bd{u}}_{\delta})_{\ttt} \cdot \bd{n}] - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |\bd{u}_{\delta}|^{2} (\bd{\xi} \cdot \bd{n}_{\delta} - \widehat{\bd{u}} \cdot \bd{n}_{\delta}) 
\nonumber \\
&+ \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{u}_{\delta} \cdot \bd{n} - \bd{\xi}_{\delta} \cdot \bd{n}) |\bd{u}_{\delta}|^{2} + \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |\bd{u}_{\delta}|^{2} (\bd{\xi}_{\delta} \cdot \bd{n} - \bd{u}_{\delta} \cdot \bd{n}).
\label{T3}
\end{align}

After taking the limit $\nu\to 0$, term $T3$ can be estimated as follows:
\begin{align*}
|T_{3}| \le &\epsilon \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{H^{1}(\Omega_{f,\delta}(t))}^{2} \\
&+ C(\epsilon) \left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||^{2}_{L^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{align*}
...
\begin{align}
T_{4} = & 2\nu \int_{0}^{t} \int_{\Omega_{f}(t)} \bd{D}(\bd{u}) : \bd{D}(\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt}) - 2\nu \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \bd{D}(\bd{u}_{\delta}) : \bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta}).
\label{T4}
\end{align}

After taking the limit $\nu\to 0$, term $T4$ can be estimated as follows:
\begin{equation*}
T_{4} = 2\nu \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} |\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})|^{2} + {{R_{4}}},
\end{equation*}
where
\begin{equation*}
{{|R_{4}|}} \le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||^{2}_{L^{2}(\Omega_{f,\delta}(t))} + C(\epsilon) \left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right). 
\end{equation*}
...
\begin{align*}
T_{5} = &\beta\int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} - \bd{u})_{t}[(\bd{\xi} - (\bd{\xi}_{\delta})_{\ttt})_{t} - (\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\ttt})_{t}] - \beta\int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} - \bd{u}_{\delta})_{t}[(\bd{\xi} - \bd{\xi}_{\delta})_{t} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{t}]
\end{align*}

After taking the limit $\nu\to 0$, term $T_5$ can be estimated as follows:
\begin{equation*}
T_{5} = \beta \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |(\bd{\xi} - \bd{\xi}_{\delta})_{t} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{t}|^{2} + R_{5},
\end{equation*}
where
\begin{equation*}
|R_{5}| \le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)}^{2}\right).
\end{equation*}
...
\begin{align}
T_{6} = &-\rho_{p} \int_{0}^{t} \int_{\Gamma} \zeta \cdot \partial_{t}\left[\zeta - (\zeta_{\delta})_{\ttt}\right] + \rho_{p} \int_{\Gamma} \zeta(s) \cdot  \left[\zeta(s) - (\zeta_{\delta})_{\ttt}(s)\right] - \rho_{p} \int_{\Gamma} \zeta(0) \cdot \left[\zeta(0) - (\zeta_{\delta})_{\ttt}(0)\right] 
\nonumber \\
&+ \rho_{p} \int_{0}^{t} \int_{\Gamma} \zeta_{\delta} \cdot \partial_{t} \zeta - \rho_{p} \int_{\Gamma} \zeta_{\delta}(s) \cdot \zeta(s) + \rho_{p} \int_{\Gamma} \zeta_{\delta}(0) \cdot \zeta(0) + \frac{1}{2}\rho_{p} \int_{\Gamma} |\zeta_{\delta}(s)|^{2} - \frac{1}{2} \rho_{p} \int_{\Gamma} |\zeta_{0}|^{2}.
\label{T6}
\\
T_{7} = &\int_{0}^{t} \int_{\Gamma} \Delta \omega \cdot \Delta \left[\zeta - (\zeta_{\delta})_{\ttt}\right] - \int_{0}^{t} \int_{\Gamma} \Delta \omega_{\delta} \cdot \Delta \zeta + \frac{1}{2} \int_{\Gamma} |\Delta \omega_{\delta}(s)|^{2} - \frac{1}{2} \int_{\Gamma} |\Delta \omega_{0}|^{2}.
\\
T_{8} = &-\rho_{b} \int_{0}^{t} \int_{\Omega_{b}} \partial_{t}\bd{\eta} \cdot \partial_{t}\left[\bd{\xi} - (\bd{\xi}_{\delta})_{\ttt}\right] + \rho_{b} \int_{\Omega_{b}} \bd{\xi}(s) \cdot \left[\bd{\xi}(s) - (\bd{\xi}_{\delta})_{\ttt}(s)\right] - \rho_{b} \int_{\Omega_{b}} \bd{\xi}(0) \cdot \left[\bd{\xi}(0) - (\bd{\xi}_{\delta})_{\ttt}(0)\right] 
\nonumber \\
&+ \rho_{b} \int_{0}^{t} \int_{\Omega_{b}} \partial_{t}\bd{\eta}_{\delta} \cdot \partial_{t} \bd{\xi} - \rho_{b} \int_{\Omega_{b}} \bd{\xi}_{\delta}(s) \cdot \bd{\xi}(s) + \rho_{b} \int_{\Omega_{b}} \bd{\xi}_{\delta}(0) \cdot \bd{\xi}(0) + \frac{1}{2} \rho_{b} \int_{\Omega_{b}} |\bd{\xi}_{\delta}(s)|^{2} - \frac{1}{2} \rho_{b} \int_{\Omega_{b}} |\bd{\xi}_{0}|^{2}.
\end{align}
...
\begin{equation*}
T_{6} = \frac{1}{2} \rho_{p} \int_{\Gamma} |(\zeta - \zeta_{2})(t)|^{2}, \ \ \ 
T_{7} = \frac{1}{2} \int_{\Gamma} |\Delta (\omega - \omega_{2})(t)|^{2}, \ \ \ T_{8} = \frac{1}{2} \rho_{b} \int_{\Omega_{b}} |(\bd{\xi} - \bd{\xi}_{2})(t)|^{2}.
\end{equation*}
...
\begin{align}
T_{9} = &2\mu_{e} \int_{0}^{t} \int_{\Omega_{b}} \bd{D}(\bd{\eta}) : \bd{D}\left[\bd{\xi} - (\bd{\xi}_{\delta})_{\ttt}\right] - 2\mu_{e} \int_{0}^{t} \int_{\Omega_{b}} \bd{D}(\bd{\eta}_{\delta}) : \bd{D}(\bd{\xi}) + \mu_{e} \int_{\Omega_{b}} |\bd{D}(\bd{\eta}_{\delta})(s)|^{2} - \mu_{e} \int_{\Omega_{b}} |\bd{D}(\bd{\eta}_{0})|^{2}.
\nonumber \\
T_{10} = &\lambda_{e} \int_{0}^{t} \int_{\Omega_{b}} (\nabla \cdot \bd{\eta}) \left(\nabla \cdot \left[\bd{\xi} - (\bd{\xi}_{\delta})_{\ttt}\right]\right) - \lambda_{e} \int_{0}^{t} \int_{\Omega_{b}} (\nabla \cdot \bd{\eta}_{\delta}) (\nabla \cdot \bd{\xi}) + \frac{1}{2} \lambda_{e} \int_{\Omega_{b}} |\nabla \cdot \bd{\eta}_{\delta}(s)|^{2} - \frac{1}{2} \lambda_{e} \int_{\Omega_{b}} |\nabla \cdot \bd{\eta}_{0}|^{2}.
\nonumber\\
...
\begin{align*}
T_{13} = &-\aalpha \int_{0}^{t} \int_{\Omega_{b}(t)} p \left(\nabla \cdot \left[\bd{\xi} - (\bd{\xi}_{\delta})_{\ttt}\right]\right) + \aalpha \int_{0}^{t} \int_{{\Omega}^\delta_{b,\delta}(t)} p_{\delta} \left(\nabla \cdot (\bd{\xi} - \bd{\xi}_{\delta})\right).
\end{align*}


After taking the limit $\nu\to 0$, term {{$T_{13}$}} can be estimated as follows:
\begin{align*}
|T_{13}| \le &C(\epsilon)\int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}_\delta ||_{L^{2}(\Omega_{b})}^{2} + \epsilon \int_{0}^{t} ||\nabla(\bd{\xi} - \bd{\xi}_{\delta})||_{L^{2}(\Omega_{b})}^{2} \\
...
\begin{align}
T_{14} = &-c_{0} \int_{0}^{t} \int_{\Omega_{b}} p \cdot \partial_{t} \left[p - (p_{\delta})_{\ttt}\right] + c_{0} \int_{\Omega_{b}} p(s) \cdot \left[p(s) - (p_{\delta})_{\ttt}(s)\right] - c_{0} \int_{\Omega_{b}} p_{0} \cdot \left[p(0) - (p_{\delta})_{\ttt}(0)\right] 
\nonumber \\
&+ c_{0} \int_{0}^{t} \int_{\Omega_{b}} p_{\delta} \cdot \partial_{t} p - c_{0} \int_{\Omega_{b}} p_{\delta}(s) \cdot p(s) + c_{0} \int_{\Omega_{b}} |p_{0}|^{2} + \frac{1}{2} c_{0} \int_{\Omega_{b}} |p_{\delta}(s)|^{2} - \frac{1}{2} c_{0} \int_{\Omega_{b}} |p_{0}|^{2}.
\label{T14}
\end{align}

This term can be handled in the same way as Terms 6-8. In the limit as $\nu \to 0$, the contribution from this term is
\begin{equation*}
T_{14} = \frac{1}{2} c_{0} \int_{\Omega_{b}} |(p - p_{\delta})(\tau)|^{2}.
\end{equation*}
...
\begin{align}
T_{15} = &-\aalpha \int_{0}^{t} \int_{\Omega_{b}(t)} \bd{\xi} \cdot \nabla\left[p - (p_{\delta})_{\ttt}\right] + \aalpha \int_{0}^{t} \int_{{\Omega}^\delta_{b,\delta}(t)} \bd{\xi}_{\delta} \cdot \nabla (p - p_{\delta}).
\label{T15}
\end{align}

After taking the limit $\nu\to 0$, term {{$T_{15}$}} can be estimated as follows:
\begin{align*}
|T_{15}| \le &\epsilon \int_{0}^{t} ||\nabla(p - p_{\delta})||_{L^{2}(\tilde{\Omega}_{b, 2, \delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}_\delta ||_{L^{2}(\Omega_{b})}^{2} \\
...
\begin{align}
T_{16} = &-\aalpha \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \left[p - (p_{\delta})_{\ttt}\right] + \aalpha \int_{0}^{t} \int_{{\Gamma}^\delta_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}) (p - p_{\delta}).
\label{T16}
\end{align}

After passing to the limit as $\nu \to 0$, this term can be estimated as follows:
\begin{multline*}
|T_{16}| \le \epsilon\left(\int_{0}^{t} ||\nabla \bd{\xi} - \nabla \bd{\xi}_{\delta}||^{2}_{L^{2}(\Omega_{b})} + \int_{0}^{t} ||\nabla p - \nabla p_{\delta}||_{L^{2}(\tilde{\Omega}_{b, 2, \delta}(t))}^{2}\right) + C(\epsilon)\left(\int_{0}^{t} ||p - p_{\delta}||^{2}_{L^{2}(\Omega_{b})} \right. \\
\left. + \int_{0}^{t} ||\nabla \bd{\eta} - \nabla \tilde{\bd{\eta}}_{1}||_{L^{2}(\Omega_{b})}^{2} + \int_{0}^{t} ||\nabla \bd{\eta} - \nabla \bd{\eta}_{\delta}||_{L^{2}(\Omega_{b})}^{2} + \int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||^{2}_{L^{2}(\Omega_{b})}\right).
\end{multline*}
...
\begin{align}
T_{17} = &\kappa \int_{0}^{t} \int_{\Omega_{b}(t)} \nabla p \cdot \nabla \left[p - (p_{\delta})_{\ttt}\right] - \kappa \int_{0}^{t} \int_{{\Omega}^\delta_{b,\delta}(t)} \nabla p_{\delta} \cdot \nabla (p - p_{\delta}).
\label{T17}
\end{align}

This term can be estimated as follows:
\begin{equation*}
T_{17} \le \kappa \int_{0}^{t} \int_{{\Omega}^\delta_{b,\delta}(t)} |\nabla(p - p_{\delta})|^{2} + {{R_{17}}},
\end{equation*}
where the remainder is bounded by 
\begin{align*}
{{|R_{17}|}} \le &\epsilon \int_{0}^{t} ||\nabla(p - p_{\delta})||^{2}_{L^{2}({\Omega}^\delta_{b,\delta}(t))} \\
...
\begin{align*}
T_{18} &= \int_{0}^{t} \int_{\Gamma(t)} p (\bd{u} - \bd{\xi}) \cdot \bd{n} - \int_{0}^{t} \int_{\Gamma(t)} p [(\bd{u}_{\delta})_{\ttt} - (\bd{\xi}_{\delta})_{\ttt}] \cdot \bd{n} - \int_{0}^{t} \int_{\Gamma_{\delta}(t)} p_{\delta}(\bd{u} - \bd{\xi}) \cdot \bd{n} 
\nonumber \\
&+ \int_{0}^{t} \int_{\Gamma_{\delta}(t)} p_{\delta}(\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n} - \int_{0}^{t} \int_{\Gamma(t)} ((\bd{u} - \bd{\xi}) \cdot \bd{n}) [p - (p_{\delta})_{\ttt}] + \int_{0}^{t} \int_{\Gamma_{\delta}(t)} ((\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n}) (p - p_{\delta}).
\end{align*}

This term can be estimated as follows:
\begin{align*}
|T_{18}| \le &\epsilon \left(\int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||^{2}_{L^{2}(\Omega_{f,\delta}(t))} + \int_{0}^{t} ||\nabla (\bd{\xi} - \bd{\xi}_{\delta})||_{L^{2}(\Omega_{b})} + \int_{0}^{t} ||\nabla(p - p_{\delta})||^{2}_{L^{2}({\Omega}^\delta_{b,\delta}(t))}\right) \\
&+ C(\epsilon) \int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)}.
\end{align*}

The combined estimates for the terms $T_1$-$T_{18}$ give the estimate \eqref{Gronwall}. This finishes the proof of the Gronwall's estimate presented in 
...
To obtain the desired Gronwall estimate, we need the following uniform bounds on the factor $\det(\bd{I} + \nabla {\bd{\eta}}_{\delta}^\delta)$,
...
for each $\delta > 0$. 
In fact, similarly, the following three estimates hold locally,  for $t \in [0,T_\delta]$, where {\emph{$T_\delta$ may depend on $\delta$}},
...
We will now show how to use a bootstrap argument to deduce that the time interval on which 
 estimates \eqref{local}-\eqref{inverse} hold for the regularized weak solution of the regularized problem ${\bd{\eta}}_{\delta}^\delta$ can, in fact, be extended to the entire 
 interval $[0,T]$, namely, that estimates \eqref{local}-\eqref{inverse} hold globally, uniformly in $\delta$, where $T$ is independent of $\delta$.
...
the domain $\Omega_b$.
Furthermore, we recall that the convolution is defined using odd extensions as in Definition \ref{extension}.  
...
Now we use a bootstrap argument on {{$\det(\bd{I} + \nabla \bd{\eta}^{\delta}_{\delta})$}} by continuity, since we have that $\det(\bd{I} + \nabla \bd{\eta}) \ge c > 0$ up to a final time $T > 0$. Similarly, for all sufficiently small $\delta$, the assumptions \eqref{norm} and \eqref{grad} will also hold up to the final time $T > 0$, as we can also bootstrap these two conditions \eqref{norm} and \eqref{grad} similarly. This closes the bootstrap argument, and so we obtain that the estimate \eqref{det}, and similarly the estimates \eqref{norm} and \eqref{grad}, hold uniformly up to the final time $T > 0$ uniformly in $\delta$.
...
We end this section by proving the following lemma, which establishes convergence of the spatial convolution of the classical solution $\bd{\eta}$ in $H^{1}(\Omega_{b})$, which is needed for the bootstrap argument described above.
...
An interesting extension of this work is to consider the singular limit as the thin interface thickness converges to zero, and investigate the existence  of a weak solution to the FSI problem between the Navier-Stokes equations for an incompressible, viscous fluid and the Biot equations of poroviscoelasticity, 
nonlinearly coupled over the moving interface. Preliminary results indicate that this will be possible under certain assumptions. 
...
\subsection{Weak continuity of solutions to the regularized PFSI problem}\label{appendix1}
...
to obtain the terms:
\begin{align*}
T_{1, 1} &= -\int_{0}^{t} \int_{\Omega_{f}(t)} \bd{u} \cdot \partial_{t} \left[\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}\right] - \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}] \\
...
\begin{equation*}
T_{1, 1} = \int_{0}^{t} \int_{\Omega_{f}(t)} \partial_{t}\bd{u} \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}] + \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot [\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}].
\end{equation*}
...
\begin{equation*}
T_{1, 1} = \int_{0}^{t} \int_{\Omega_{f}(t)} \partial_{t}\bd{u} \cdot [\bd{u} - \widecheck{\bd{u}}_{\delta}] + \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot [\bd{u} - \widecheck{\bd{u}}_{\delta}] + K_{1, 1, \nu},
\end{equation*}
where $K_{1, 1, \nu} \to 0$ as $\nu \to 0$. Using estimates as found in \cite{WeakStrongFSI}, we can transfer the first integral from $\Omega_{1}(t)$ to $\Omega_{f,\delta}(t)$ at the cost of an additional term, so that 
\begin{equation*}
T_{1, 1} = \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \partial_{t} \widehat{\bd{u}} \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) + \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot (\bd{u} - \widecheck{\bd{u}}_{\delta}) + {{\tilde{R}_{1}}} + K_{1, 1, \nu},
\end{equation*}
where
\begin{multline*}
{{|\tilde{R}_{1}|}} \le \epsilon \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{H^{1}(\Omega_{f,\delta}(t))} \\
+ C(\epsilon) \left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\partial_{t}\omega - \partial_{t}\omega_{\delta}||_{L^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{multline*}
Thus, by using Proposition \ref{alphaconvprop} again,
\begin{equation}\label{T1-1}
T_{1, 1} = \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \partial_{t} \widehat{\bd{u}} \cdot (\widehat{\bd{u}} - (\bd{u}_{\delta})_{\nu}) + \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot (\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}) + {{\tilde{R}_{1} + K_{1, 1, \nu}}},
\end{equation}
where {{$K_{1, 1, \nu} \to 0$}} as $\nu \to 0$. 

Next, we test the regularized weak formulation for $\bd{u}_{\delta}$ with $\widehat{\bd{u}}$ and obtain the following terms:
\begin{align*}
T_{1, 2} = -\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \bd{u}_{\delta} \cdot \partial_{t} \widehat{\bd{u}} - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \bd{u}_{\delta} \cdot \widehat{\bd{u}} 
...
T_{1, 2} = \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \partial_{t}\left[(\bd{u}_{\delta})_{\nu}\right] \cdot \widehat{\bd{u}} + \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) (\bd{u}_{\delta})_{\nu} \cdot \widehat{\bd{u}} + K_{1, 2, \nu},
\end{equation}
where $K_{1, 2, \nu} \to 0$ as $\nu \to 0$. 
...
&+ \frac{1}{2} \int_{\Omega_{f,\delta}(t)} |\bd{u}_{\delta}(t)|^{2} - \frac{1}{2} \int_{\Omega_{f,\delta}(0)} |\bd{u}_{\delta}(0)|^{2} - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \widehat{\bd{u}} \cdot (\widehat{\bd{u}} - (\bd{u}_{\delta})_{\nu}) \\
&+ \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot (\bd{u} - (\widecheck{\bd{u}}_{\delta})_{\nu}) + {{\tilde{R}_{1} + K_{1, 1, \nu}}} + K_{1, 2, \nu}. 
\end{align*}
...
\begin{align*}
T_{1} &= \frac{1}{2} \int_{\Omega_{f,\delta}(t)} |(\widehat{\bd{u}} - \bd{u}_{\delta})(t)|^{2} - \frac{1}{2} \int_{\Omega_{f,\delta}(0)} |(\widehat{\bd{u}} - \bd{u}_{\delta})(0)|^{2} \\
&- \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \widehat{\bd{u}} \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) + \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) \bd{u} \cdot (\bd{u} - \widecheck{\bd{u}}_{\delta}) + {{\tilde{R}_{1}}}.
\end{align*}
...
where
\begin{align*}
{{|R_{1}|}} &\le \epsilon \int_{0}^{T} ||\widehat{\bd{u}} - \bd{u}_{2. \delta}||^{2}_{H^{1}(\Omega_{f,\delta}(t))} \\
&+ C(\epsilon) \left(\int_{0}^{T} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{T} ||\partial_{t}\omega - \partial_{t}\omega_{\delta}||_{L^{2}(\Gamma)}^{2} + \int_{0}^{T} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{align*}
...
\begin{align*}
T_{2} &:= \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f}(t)} ((\bd{u} \cdot \nabla) \bd{u}) \cdot (\bd{u} - \widecheck{\bd{u}}_{\delta}) - \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f}(t)} ((\bd{u} \cdot \nabla) (\bd{u} - \widecheck{\bd{u}}_{\delta})) \cdot \bd{u} \\
&- \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} ((\bd{u}_{\delta} \cdot \nabla) \bd{u}_{\delta}) \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) + \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} ((\bd{u}_{\delta} \cdot \nabla) (\widehat{\bd{u}} - \bd{u}_{\delta})) \cdot \bd{u}_{\delta}.
\end{align*}
...
\int_{0}^{t} \int_{\Omega_{f}(t)} ((\bd{u} \cdot \nabla) \bd{u}) \cdot (\bd{u} - \widecheck{\bd{u}}_{\delta}), \qquad \int_{0}^{t} \int_{\Omega_{f}(t)} ((\bd{u} \cdot \nabla) (\bd{u} - \widecheck{\bd{u}}_{\delta})) \cdot \bd{u},
\end{equation}
to integrals on $\Omega_{f,\delta}(t)$ by using the map $\psi_{\delta}: \Omega_{f,\delta}(t) \to \Omega_{f}(t)$ defined by \eqref{psi}. We use 
\begin{equation*}
...
\begin{align*}
&\left|\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} (\nabla ((I - \gamma_{\delta} J_{\delta}^{-1})(\bd{u} \circ \psi_{\delta})) \widehat{\bd{u}}) \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) \right| \\
&\le C \int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))} \le C\left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{L^{2}(\Omega_{f,\delta}(t))}\right).
\end{align*}
...
\begin{equation*}
\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [(\nabla(\bd{u} \circ \psi_{\delta})) \widehat{\bd{u}}] \cdot [(I - \gamma_{\delta}^{-1}J_{\delta})(\widehat{\bd{u}} - \bd{u}_{\delta})] \le C\left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{L^{2}(\Omega_{f,\delta}(t))}\right).
\end{equation*}
Thus, we obtain
\begin{equation}\label{T2R1}
{{|R_{1}|}} \le C\left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{L^{2}(\Omega_{f,\delta}(t))}\right).
\end{equation}
...
where
\begin{equation*}
{{R_{2, 2}}} := \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} (\nabla [(I - \gamma_{\delta} J_{\delta}^{-1})((\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta})] \widehat{\bd{u}}) \cdot \widehat{\bd{u}} - \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [(\nabla((\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta})) \widehat{\bd{u}}] \cdot [(I - \gamma_{\delta}^{-1}J_{\delta})\widehat{\bd{u}}].
\end{equation*}
...
{{|R_{2, 2}|}} &\le C\left(\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} |\nabla(\gamma_{\delta} J_{\delta}^{-1})| \cdot |(\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta}| + \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} |I - \gamma_{\delta} J_{\delta}^{-1}| \cdot |\nabla((\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta})| \right.
\nonumber \\
&\left. + \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} |I - \gamma_{\delta}^{-1}J_{\delta}| \cdot |\nabla((\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta})|\right) 
\nonumber\\
&\le C\left(\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \left(|\nabla(\gamma_{\delta}J_{\delta}^{-1})| + |\nabla(\gamma_{\delta}^{-1}J_{\delta})|\right) \cdot |\widehat{\bd{u}} - \bd{u}_{\delta}| + \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)} \cdot |\nabla(\widehat{\bd{u}} - \bd{u}_{\delta})|\right) 
\nonumber\\
&\le \epsilon \int_{0}^{t} ||\nabla(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{align}
...
\begin{align*}
&\frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [(\widehat{\bd{u}} \cdot \nabla) \widehat{\bd{u}}] \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) - [(\widehat{\bd{u}} \cdot \nabla)(\widehat{\bd{u}} - \bd{u}_{\delta})] \cdot \widehat{\bd{u}} \\
&- \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t))} [(\bd{u}_{\delta} \cdot \nabla) \bd{u}_{\delta}] \cdot (\widehat{\bd{u}} - \bd{u}_{\delta}) - [(\bd{u}_{\delta} \cdot \nabla)(\widehat{\bd{u}} - \bd{u}_{\delta})] \cdot \bd{u}_{\delta} \\
&= \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [((\widehat{\bd{u}} - \bd{u}_{\delta}) \cdot \nabla) \bd{u}_{\delta}] \cdot \widehat{\bd{u}} - \frac{1}{2} \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [((\widehat{\bd{u}} - \bd{u}_{\delta}) \cdot \nabla) \widehat{\bd{u}}] \cdot \bd{u}_{\delta}.
\end{align*}
In absolute values, the right hand-side can be bounded as follows:
\begin{equation*}
\le \epsilon \int_{0}^{t} ||\nabla(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}.
\end{equation*}
...
\begin{align*}
T_{3} &= \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{u} \cdot \bd{n} - \bd{\xi} \cdot \bd{n}) \bd{u} \cdot (\bd{u} - \widecheck{\bd{u}}_{\delta}) - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{u}_{\delta} \cdot \bd{n}_{\delta} - \bd{\xi}_{\delta} \cdot \bd{n}_{\delta}) \bd{u}_{\delta} \cdot \widehat{\bd{u}} \\
&+ \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} |\bd{u}|^{2} (\bd{\xi} \cdot \bd{n} - \bd{u} \cdot \bd{n}) - \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} |\bd{u}|^{2} (\bd{\xi}_{\delta} \cdot \bd{n} - \widecheck{\bd{u}}_{\delta} \cdot \bd{n}) \\
&- \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |\bd{u}_{\delta}|^{2} (\bd{\xi} \cdot \bd{n}_{\delta} - \widehat{\bd{u}} \cdot \bd{n}_{\delta}) = \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n} - \bd{u} \cdot \bd{n}) \bd{u} \cdot \widecheck{\bd{u}}_{\delta} \\
&- \frac{1}{2} \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi}_{\delta} \cdot \bd{n} - \widecheck{\bd{u}}_{\delta} \cdot \bd{n}) |\bd{u}|^{2} - \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi} \cdot \bd{n}_{\delta} - \widehat{\bd{u}} \cdot \bd{n}_{\delta}) |\bd{u}_{\delta}|^{2} \\
&+ \frac{1}{2} \int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} \cdot \bd{n}_{\delta} - \bd{u}_{\delta} \cdot \bd{n}_{\delta}) \bd{u}_{\delta} \cdot \widehat{\bd{u}} = {{R_{3, 1} + R_{3, 2}}},
\end{align*}
where
\begin{align*}
{{R_{3, 1}}} &= \frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{u})_{y} \bd{u}_{\delta} \cdot (\bd{u} - \bd{u}_{\delta}) - \frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{\xi}_{\delta} - \bd{u}_{\delta})_{y} \bd{u} \cdot (\bd{u} - \bd{u}_{\delta}),
\\
{{R_{3, 2}}} &= \frac{1}{2} \int_{0}^{t} \int_{\Gamma} \partial_{x}\omega (\bd{u})_{x} \bd{u} \cdot \bd{u}_{\delta} - \frac{1}{2} \int_{0}^{t} \int_{\Gamma} \partial_{x}\omega (\bd{u}_{\delta})_{x} |\bd{u}|^{2} 
- \frac{1}{2} \int_{0}^{t} \int_{\Gamma} \partial_{x}\omega_{\delta} (\bd{u})_{x} |\bd{u}_{\delta}|^{2} + \frac{1}{2} \int_{0}^{t} \int_{\Gamma} \partial_{x}\omega_{\delta} (\bd{u}_{\delta})_{x} \bd{u} \cdot \bd{u}_{\delta}.
\end{align*}

We estimate {{$R_{3, 1}$}} as follows: decompose {{$R_{3, 1}$ as $R_{3, 1} = R_{3, 1, 1} + R_{3, 1, 2}$}}, where
\begin{equation*}
{{R_{3, 1, 1}}} = -\frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{\xi})_{y} (\bd{u} - \bd{u}_{\delta}) \cdot (\bd{u} - \bd{u}_{\delta}) + \frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{\xi}_{\delta})_{y} \bd{u} \cdot (\bd{u} - \bd{u}_{\delta}),
\end{equation*}
\begin{equation*}
{{R_{3, 1, 2}}} = \frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{u})_{y} (\bd{u} - \bd{u}_{\delta}) \cdot (\bd{u} - \bd{u}_{\delta}) - \frac{1}{2} \int_{0}^{t} \int_{\Gamma} (\bd{u} - \bd{u}_{\delta})_{y} \bd{u} \cdot (\bd{u} - \bd{u}_{\delta}).
\end{equation*}
By interpolation, 
\begin{multline*}
{{|R_{3, 1, 1}|}} \le C\left(\int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{1/2}_{L^{2}(\Omega_{f,\delta}(t))} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{3/2}_{H^{1}(\Omega_{f,\delta}(t))} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{H^{1}(\Omega_{f,\delta}(t))}\right) \\
\le \epsilon \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{H^{1}(\Omega_{f,\delta}(t))} + C(\epsilon) \left(\int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)}^{2}\right).
\end{multline*}
By using the same interpolation inequality, we obtain the following estimate for {{$R_{3, 1, 2}$}}.
\begin{equation*}
{{|R_{3, 1, 2}|}} \le \epsilon \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||^{2}_{H^{1}(\Omega_{f,\delta}(t))} + C(\epsilon) \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}.
\end{equation*}
...
\begin{equation*}
{{|R_{3, 2}|}} \le \epsilon \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{H^{1}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon) \left(\int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{equation*}
...
T_{4} := 2\nu \int_{0}^{t} \int_{\Omega_{f}(t)} \bd{D}(\bd{u}) : \bd{D}(\bd{u} - \widecheck{\bd{u}}_{\delta}) - 2\nu \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \bd{D}(\bd{u}_{\delta}) : \bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta}).
\end{equation}
We want to transfer the integral on $\Omega_{1}(t)$ to $\Omega_{f,\delta}(t)$. Recalling \eqref{gradidentity}, we have that
\begin{equation*}
\int_{0}^{t} \int_{\Omega_{f}(t)} \bd{D}(\bd{u}) : \bd{D}(\bd{u} - \widecheck{\bd{u}}_{\delta}) = \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \gamma_{\delta} [\nabla(\bd{u} \circ \psi_{\delta}) J_{\delta}^{-1}]^{s} : [\nabla((\bd{u} - \widecheck{\bd{u}}_{\delta}) \circ \psi_{\delta}) J_{\delta}^{-1}]^{s},
\end{equation*}
where the superscript `s' notation denotes a symmetrization. Following the procedure in \cite{WeakStrongFSI}, we break up the integral as
\begin{multline}\label{T4equality2}
\int_{0}^{t} \int_{\Omega_{f}(t)} \bd{D}(\bd{u}) : \bd{D}(\bd{u} - \widecheck{\bd{u}}_{\delta}) = \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} \bd{D}(\widehat{\bd{u}}) : \bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta}) + {{R_{4, 1} + R_{4, 2} + R_{4, 3} + R_{4, 4}}},
\end{multline}
where
\begin{align*}
{{R_{4, 1}}} &= \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} (\nabla (\bd{u} \circ \psi_{\delta}) J_{\delta}^{-1})^{s} : [\nabla (\widehat{\bd{u}} - \bd{u}_{\delta}) (J_{\delta}^{-1} - I) + (J_{\delta} - I) \nabla (\widehat{\bd{u}} - \bd{u}_{\delta}) J_{\delta}^{-1}]^{s},
\\
{{R_{4, 2}}} &= \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [(I - \gamma_{\delta} J_{\delta}^{-1})\nabla (\bd{u} \circ \psi_{\delta}) + \nabla (\bd{u} \circ \psi_{\delta}) (J_{\delta}^{-1} - I)]^{s} : \bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta}),
\\
{{R_{4, 3}}} &= \int_{0}^{t} \int_{\Omega_{f,\delta}(t)} (\nabla (\bd{u} \circ \psi_{\delta}) J_{\delta}^{-1})^{s} : (\gamma_{\delta} \nabla(\gamma_{\delta}^{-1}J_{\delta})(\widehat{\bd{u}} - \bd{u}_{\delta})J_{\delta}^{-1})^{s},
\\
{{R_{4, 4}}} &= -\int_{0}^{t} \int_{\Omega_{f,\delta}(t)} [(\nabla(\gamma_{\delta} J_{\delta}^{-1})) \bd{u} \circ \psi_{\delta}]^{s} : \bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta}).
\end{align*}
...
\begin{align*}
{{|R_{4, 1}|}} &\le C\int_{0}^{t} ||\nabla (\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} (||\gamma_{\delta}^{-1} - 1||_{L^{2}(\Omega_{f,\delta}(t))} + ||\gamma_{\delta} - 1||_{L^{2}(\Omega_{f,\delta}(t))} + ||\partial_{x} \gamma_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}) \\
&\le \epsilon \int_{0}^{t} ||\nabla (\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)}.
\end{align*}
We also have that
\begin{equation*}
{{|R_{4, 2}|}} \le \epsilon \int_{0}^{t} ||\bd{D} (\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)}.
\end{equation*}
%
...
\begin{align*}
{{|R_{4, 3}|}} &\le C \int_{0}^{t} ||\nabla(\gamma_{\delta}^{-1}J_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))} \le C\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))} \\
&\le C\left(\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\widehat{\bd{u}} - \bd{u}_{\delta}||_{L^{2}(\Omega_{f,\delta}(t))}^{2}\right).
\end{align*}
...
\begin{align*}
{{|R_{4, 4}|}} &\le C \int_{0}^{t} ||\nabla(\gamma_{\delta} J_{\delta}^{-1})||_{L^{2}(\Omega_{f,\delta}(t))} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} \\
&\le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\omega - \omega_{\delta}||^{2}_{H^{2}(\Gamma)}. 
\end{align*}
%
...
T_{5} = \beta\int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} - \bd{u})_{\tau}[(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\bd{u} - \widecheck{\bd{u}}_{\delta})_{\tau}] - \beta\int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi}_{\delta} - \bd{u}_{\delta})_{\tau}[(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{\tau}].
\end{equation} 
...
\begin{equation*}
T_{5} = \beta \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{\tau}|^{2} + R_{5},
\end{equation*}
where
\begin{equation*}
R_{5} = \beta\int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} - \bd{u})_{\tau}[(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\bd{u} - \widecheck{\bd{u}}_{\delta})_{\tau}] - \beta\int_{0}^{t} \int_{\Gamma_{\delta}(t)} (\bd{\xi} - \widehat{\bd{u}})_{\tau}[(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{\tau}].
\end{equation*}
%
Denote the arc length elements of $\Gamma_{t}(t)$ and $\Gamma_{\delta}(t)$ respectively by $\mathcal{J}^{\omega}_{\Gamma} = \sqrt{1 + |\partial_{x}\omega|^{2}}$ and $\mathcal{J}^{\omega_{\delta}}_{\Gamma} = \sqrt{1 + |\partial_{x}\omega_{\delta}|^{2}}$, 
and we denote the tangent vectors to $\Gamma(t)$ and $\Gamma_{\delta}(t)$ respectively by $\bd{\tau}_{1} = \frac{1}{\mathcal{J}^{\omega}_{\Gamma}}(1, \partial_{x}\omega)$ and $\bd{\tau}_{\delta} = \frac{1}{\mathcal{J}^{\omega_{\delta}}_{\Gamma}}(1, \partial_{x}\omega_{\delta})$.
%
...
\begin{align*}
R_{5} &= \beta\int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{u}) \cdot (1, \partial_{x}\omega) [(\bd{\xi} - \bd{\xi}_{\delta}) - (\bd{u} - \bd{u}_{\delta})] \cdot \bd{\tau}_{1} \\
&- \beta\int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{u}) \cdot (1, \partial_{x}\omega_{\delta}) [(\bd{\xi} - \bd{\xi}_{\delta}) - (\bd{u} - \bd{u}_{\delta})] \cdot \bd{\tau}_{\delta}.
\end{align*}
...
\begin{align*}
R_{5, 1} &= \beta \int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{u})_{y} (\partial_{x}\omega - \partial_{x}\omega_{\delta}) [(\bd{\xi} - \bd{\xi}_{\delta}) - (\bd{u} - \bd{u}_{\delta})] \cdot \bd{\tau}_{1},
\\
R_{5, 2} &= \beta \int_{0}^{t} \int_{\Gamma} (\bd{\xi} - \bd{u}) \cdot (1, \partial_{x}\omega_{\delta}) [(\bd{\xi} - \bd{\xi}_{\delta}) - (\bd{u} - \bd{u}_{\delta})] \cdot (\bd{\tau}_{1} - \bd{\tau}_{\delta}).
\end{align*}
...
\begin{equation*}
|R_{5, 1}| \le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)}^{2}\right),
\end{equation*}
where we used the trace inequality, Poincare's inequality, and Korn's inequality for the fluid. For the second term $R_{6, 2}$, we use the estimate $|\bd{\tau}^{\omega} - \bd{\tau}^{\omega_{\delta}}| \le C|\partial_{x}\omega - \partial_{x}\omega_{\delta}|$ to obtain
\begin{equation*}
|R_{5, 2}| \le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)}^{2}\right).
\end{equation*}
Hence,
\begin{equation*}
T_{5} = \beta \int_{0}^{t} \int_{\Gamma_{\delta}(t)} |(\bd{\xi} - \bd{\xi}_{\delta})_{\tau} - (\widehat{\bd{u}} - \bd{u}_{\delta})_{\tau}|^{2} + R_{6},
\end{equation*}
where
\begin{equation*}
|R_{5}| \le \epsilon \int_{0}^{t} ||\bd{D}(\widehat{\bd{u}} - \bd{u}_{\delta})||_{L^{2}(\Omega_{f,\delta}(t))} + C(\epsilon)\left(\int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||_{L^{2}(\Gamma)}^{2}\right).
...
Furthermore, because $\zeta(0) = \zeta_{\delta}(0) = \zeta_{0}$ weak continuity of $\zeta_{\delta}$ at $t = 0$ implies
 ...
 Similarly, $\displaystyle \int_{\Gamma} \zeta(\tau) \cdot [\zeta(\tau) - (\zeta_{\delta})_{\nu}(\tau)] \to 0$ as $\nu \to 0$ for almost every $t \in [0, T]$. 
 Hence, as $\nu \to 0$, the contribution from $T_{6}$ is
\begin{equation*}
T_{6} = \frac{1}{2} \rho_{p} \int_{\Gamma} |(\zeta - \zeta_{2})(\tau)|^{2}.
\end{equation*}
Similarly, the contributions from $T_7$ and $T_8$ as $\nu \to 0$ are
\begin{equation*}
T_{7} = \frac{1}{2} \int_{\Gamma} |\Delta (\omega - \omega_{2})(\tau)|^{2}, \qquad T_{8} = \frac{1}{2} \rho_{b} \int_{\Omega_{b}} |(\bd{\xi} - \bd{\xi}_{2})(\tau)|^{2}.
\end{equation*}
...
\begin{equation*}
T_{13} = -\aalpha \int_{0}^{t} \int_{\Omega_{b, 1}(t)} p \nabla \cdot (\bd{\xi} - \bd{\xi}_{\delta}) + \aalpha \int_{0}^{t} \int_{{\Omega}^\delta_{b, \delta}(t)} p_{\delta} \nabla \cdot (\bd{\xi} - \bd{\xi}_{\delta}).
\end{equation*}
...
\begin{equation*}
T_{15} = -\aalpha \int_{0}^{t} \int_{\Omega_{b, 1}(t)} \frac{D}{Dt} \bd{\eta} \cdot \nabla (p - p_{\delta}) + \alpha \int_{0}^{t} \int_{{\Omega}^\delta_{b, \delta}(t)} \frac{{D}^\delta}{Dt} \bd{\eta}_{\delta} \cdot \nabla (p - p_{\delta}).  
\end{equation*}
...
\begin{align*}
{{R_{15, 1}}} &\le \epsilon \int_{0}^{t} ||\nabla(p - p_{\delta})||_{L^{2}({\Omega}^\delta_{b, \delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}_\delta||_{L^{2}(\Omega_{b})}^{2} \\
...
\begin{equation*}
{{R_{15, 2}}} \le \epsilon \int_{0}^{t} ||\nabla(p - p_{\delta})||_{L^{2}({\Omega}^\delta_{b, \delta}(t))}^{2} + C(\epsilon) \int_{0}^{t} ||\partial_{t} \bd{\eta} - \partial_{t} \bd{\eta}_{\delta}||^{2}_{L^{2}(\Omega_{b})}.
\end{equation*}
...
\begin{equation*}
T_{16} = -\aalpha \int_{0}^{t} \int_{\Gamma(t)} (\bd{\xi} \cdot \bd{n}) (p - p_{\delta}) + \aalpha \int_{0}^{t} \int_{{\Gamma}^\delta_{\delta}(t)} (\bd{\xi}_{\delta} \cdot {\bd{n}}^\delta_{\delta}) (p - p_{\delta}),
\end{equation*}
...
\begin{equation*}
{{R_{16, 1}}} := \aalpha \left|\int_{0}^{t} \int_{\Omega_{b, 1}(t)} (\nabla \cdot \bd{\xi}) (p - p_{\delta}) - \int_{0}^{t} \int_{{\Omega}^\delta_{b, \delta}(t)} (\nabla \cdot \bd{\xi}_{\delta})(p - p_{\delta})\right|,
\end{equation*}
\begin{equation*}
{{R_{16, 2}}} := \aalpha \left|\int_{0}^{t} \int_{\Omega_{b, 1}(t)} \bd{\xi} \cdot \nabla(p - p_{\delta}) - \int_{0}^{t} \int_{{\Omega}^\delta_{b, \delta}(t)} \bd{\xi}_{\delta} \cdot \nabla(p - p_{\delta})\right|.
\end{equation*}
...
\begin{multline*}
|T_{16}| \le \epsilon\left(\int_{0}^{t} ||\nabla \bd{\xi} - \nabla \bd{\xi}_{\delta}||^{2}_{L^{2}(\Omega_{b})} + \int_{0}^{t} ||\nabla p - \nabla p_{\delta}||_{L^{2}({\Omega}^\delta_{b, \delta}(t))}^{2}\right) + C(\epsilon)\left(\int_{0}^{t} ||p - p_{\delta}||^{2}_{L^{2}(\Omega_{b})} \right. \\
\left. + \int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}^\delta||_{L^{2}(\Omega_{b})}^{2} + \int_{0}^{t} ||\nabla \bd{\eta} - \nabla \bd{\eta}_{\delta}||_{L^{2}(\Omega_{b})}^{2} + \int_{0}^{t} ||\omega - \omega_{\delta}||_{H^{2}(\Gamma)}^{2} + \int_{0}^{t} ||\bd{\xi} - \bd{\xi}_{\delta}||^{2}_{L^{2}(\Omega_{b})}\right).
\end{multline*}
...
\begin{equation*}
{{R_{17, 2}}} \le \epsilon \int_{0}^{t} ||\nabla (p - p_{\delta})||^{2}_{L^{2}({\Omega}^\delta_{b, \delta}(t))} + C(\epsilon) \int_{0}^{t} ||\nabla \bd{\eta} - \nabla {\bd{\eta}}^\delta_{\delta}||^{2}_{L^{2}(\Omega_{b})}.
\end{equation*}
...
\begin{align*}
T_{18} &= \int_{0}^{t} \int_{\Gamma(t)} p (\bd{u} - \bd{\xi}) \cdot \bd{n} - \int_{0}^{t} \int_{\Gamma(t)} p (\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n} 
- \int_{0}^{t} \int_{\Gamma_{\delta}(t)} p_{\delta}(\bd{u} - \bd{\xi}) \cdot \bd{n}_{\delta} + \int_{0}^{t} \int_{\Gamma_{\delta}(t)} p_{\delta}(\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n}_{\delta} \\
&- \int_{0}^{t} \int_{\Gamma(t)} ((\bd{u} - \bd{\xi}) \cdot \bd{n}) (p - p_{\delta}) + \int_{0}^{t} \int_{\Gamma_{\delta}(t)} ((\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n}_{\delta}) (p - p_{\delta}) \\
&= -\int_{0}^{t} \int_{\Gamma(t)} p (\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n} - \int_{0}^{t} \int_{\Gamma_{\delta}(t)} p_{\delta}(\bd{u} - \bd{\xi}) \cdot \bd{n}_{\delta} 
+ \int_{0}^{t} \int_{\Gamma(t)} ((\bd{u} - \bd{\xi}) \cdot \bd{n}) p_{\delta} + \int_{0}^{t} \int_{\Gamma_{\delta}(t)} ((\bd{u}_{\delta} - \bd{\xi}_{\delta}) \cdot \bd{n}_{\delta}) p.
\end{align*}
...
&= \int_{0}^{t} \int_{\Gamma} p (\bd{u}_{\delta} - \bd{\xi}_{\delta})_{x} \cdot (\partial_{x}\omega - \partial_{x}\omega_{\delta}) - \int_{0}^{t} \int_{\Gamma} p_{\delta}(\bd{u} - \bd{\xi})_{x} \cdot (\partial_{x} \omega - \partial_{x}\omega_{\delta}) \\
&= -\int_{0}^{t} \int_{\Gamma} p[(\bd{u} - \bd{\xi})_{x} - (\bd{u}_{\delta} - \bd{\xi}_{\delta})_{x}] \cdot (\partial_{x} \omega - \partial_{x} \omega_{\delta}) + \int_{0}^{t} \int_{\Gamma} (p - p_{\delta}) (\bd{u} - \bd{\xi})_{x} \cdot (\partial_{x}\omega - \partial_{x}\omega_{\delta}).
\end{align*}
The absolute value is bounded as follows:
\begin{multline*}
\left|\int_{0}^{t} \int_{\Gamma} p[(\bd{u} - \bd{\xi})_{x} - (\bd{u}_{\delta} - \bd{\xi}_{\delta})_{x}] \cdot (\partial_{x} \omega - \partial_{x} \omega_{\delta})\right| + \left|\int_{0}^{t} \int_{\Gamma} (p - p_{\delta}) (\bd{u} - \bd{\xi})_{x} \cdot (\partial_{x}\omega - \partial_{x}\omega_{\delta})\right| \\
\le C\left(\int_{0}^{t} ||(\bd{u} - \bd{\xi})_{x} - (\bd{u}_{\delta} - \bd{\xi}_{\delta})_{x}||_{L^{2}(\Gamma)} ||\partial_{x}\omega - \partial_{x}\omega_{\delta}||_{L^{2}(\Gamma)} + \int_{0}^{t} ||p - p_{\delta}||_{L^{2}(\Gamma)} ||\partial_{x}\omega - \partial_{x}\omega_{\delta}||_{L^{2}(\Gamma)}\right).
\end{multline*}
