...
Understanding the hierarchical structure of the fermion masses remains a puzzle for the Standard Model (SM) of particle physics. This hierarchical structure of the Yukawa sector of the SM can potentially be attributed to new symmetries that unify the lepton and quark sectors. One popular setup to explain this hierarchical structure is the Froggatt-Nielsen (FN) mechanism~\cite{Froggatt:1978nt}. In this mechanism, the SM Yukawa structure is dynamically generated through the spontaneous breaking of a $U(1)$ symmetry by a scalar field known as the `flavon,' which acquires a vacuum expectation value ({\it vev}). All SM fermions are charged under that new symmetry, leading to an interaction of the flavon field with them and thus enhancing the detectability of the flavon field.  Interestingly, the pseudoscalar component of the flavon field can serve as a QCD axion field. This arises when the global FN symmetry is identified with the global Peccei-Quinn symmetry~\cite{Peccei:1977hh}, introduced to solve the strong $CP$ problem~\cite{Weinberg:1977ma}. 
However, in the absence of such an identification, the pseudoscalar flavon may populate a heavier mass range and exhibit rich phenomenology. Some studies in these directions can be found in~\cite{Ema:2016ops,Bauer:2016rxs}.

Some of the demanding reasons for extending the SM include the observation of nonzero neutrino masses and astrophysical evidence of dark matter (DM). In this work, we combine these two limitations of the SM together with the dynamical reasons for the hierarchical flavor structure. We use flavon as a portal between the dark sector and the SM. We add three generations of heavy right-handed neutrinos (RHNs), where the lightest of them is a DM candidate, and others are responsible for the neutrino mass generation via the Type-I seesaw mechanism.

The model setup considered in this work is economic and spans a quite wide range of the mass spectrum. Interestingly, the interaction strength of the flavon with the DM candidate can be such that the DM may or may not be in thermal equilibrium with the SM. Therefore, the genesis of DM is realized separately through both freeze-out and freeze-in mechanisms. In such a flavon model as the SM fermion mass hierarchy is achieved in powers of small parameter proportional to the {\it vev} of the flavon field divided by the cutoff of the theory, the model parameter space is very restrictive. This in turn increases the predictive power of the theory. We find that as the interaction strength between new physics and SM particles constrained by the observed mass and mixing spectrum, the single parameter, that is, the {\it vev} of the flavon field $v_\phi$, mainly determines the nature of DM genesis to be achieved.  

Our analysis focuses on relatively heavy mediators and the DM mass spectrum compared to previous work~\cite{Babu:2023zni} where the freeze-in mechanism is studied through the scalar flavon portal which mixes with the SM Higgs field. Flavon theories inherently induce the flavor-changing neutral current (FCNC) at the tree level. From the outcomes of current experimental searches in both quark- and lepton-level FCNC processes, the FN symmetry breaking scale requires to be advanced at least to the TeV range. In light of a naturalness perspective, the core of these theories rely on the dynamical generation of mass hierarchy invalidating small bare coupling parameters at the Lagrangian level. This makes the mass of the scalar component of the flavon at the FN symmetry breaking scale, whereas the mass term for the pseudoscalar part (the pseudo-Goldstone boson of spontaneous breaking of FN symmetry)  arises only from the soft-breaking term, which is supposed to be much lower than its scalar counterpart. We focus mostly on the pseudoscalar portal due to the discovery prospects in recent experiments. We explore both the freeze-out and freeze-in mechanisms for the DM productions and identify the parameter space explaining the observed relic density of the Universe. Our findings show that for a $\mathcal{O}(\gev)$ mediator freeze-out mechanism works with the DM candidate in the $\mathcal{O}(\tev)$ range, and the framework provides spin-independent and spin-dependent scattering cross sections at direct detection experiments. However, for the freeze-in production of a DM below 1 TeV, a significantly higher value of $v_\phi$, $\mathcal{O}(10^7-10^9)\,$GeV is required. The choice of reheating temperature of the Universe also plays an important role in identifying the processes contributing to the DM production. It is worth mention that the entire framework can produce the observed mass and mixing texture for all the SM fermions including nonzero masses for the light neutrinos. 
...
\label{eq:Lag}
	...
	&+c^{ik}_{\nu}\left(\frac{\phi}{M}\right)^{n^{i k}_{\nu}} \bar{L}^{i}\, i \sigma_2 H^* N_{R }^k +\frac{1}{2} c^{ij}_{N}\left(\frac{\phi}{M}\right)^{n^{ij}_{N}} M \overline{N_{R}^{c\,i}} N_{R}^j \ + \ \text{h.c.} \,,
\end{align}
where $c_x^{ij}$ are all $\mathcal{O}(1)$ numbers with $i,j=1,2,3$ and $k=2,3$. By invoking an extra $Z_2$ symmetry, we consider the lightest RHN, $N^1$, as a viable candidate for DM that is odd under the $Z_2$ symmetry, implying $c^{1k}_{N} = c^{k1}_{N} = 0$, for $k=2,3$. Here the differences of the $U(1)_\text{FN}$ charges of the fermions are defined as
\begin{equation}
\begin{aligned}
...
\end{align}
The resultant contribution to the mass difference of neutral meson can then be written as
%
\begin{align}
\label{eq:DeltaMq}
...
\end{align}
Current average of the theoretical predictions compared to the measurements is found to be~\cite{DiLuzio:2019jyq}
\begin{align}
...
Therefore, it is clear from the above estimates that to achieve thermal freeze-in of $N^1$, by producing the pseudoscalar that remains in thermal equilibrium with the thermal bath of SM, requires $10^4 < v_\phi < (10^{8}- 10^9)\,$GeV (similar observation was made in  Fig.~5 in Ref.~\cite{Bharucha:2022lty}).
...
\end{equation}
where $g_{*\rho}$ is the effective degrees of freedom in energy \cite{Husdal:2016haj}. As a DM candidate, $N_1$ relic density today can be derived from the its yield $\YIR$,
\begin{align}
...
In order to obtain the observed relic density of the Universe, the result of the scan in the $\mdm$ vs. $v_\phi$ plane is shown in Fig.~\ref{fig:IR-Freeze-in}.
We choose $v_\phi$ starting from $10^7\,$GeV, such that $N^1$ does not thermalize with the SM thermal bath and then extends to $v_\phi = 10^9\,$GeV to ensure that the pseudoscalar is in the thermal bath. In this range, the observed relic abundance $\omg = 0.120 \pm 0.001$ can be achieved with $\mdm$ ranging from 10 GeV to 800 GeV, as depicted by the black curve in Fig.~\ref{fig:IR-Freeze-in}. The regions on the left and right sides of the black curve represent over and under abundance, respectively. The light gray region to the left of the dashed curve is excluded because it indicates $c_N^{11} > 10$, invalidating the FN framework. Our results show that the IR freeze-in is almost 100\% dominated by the contribution of the $2 \to 2$ process $a \, a \to N^1 N^1$ as long as $\TR \leq 10\,$TeV and high enough to produce the DM from the thermal bath.  On the other hand, if $\TR \geq 100\,$TeV, then the UV freeze-in process starts to become important, which we discuss next.
...
\begin{alignat}{2}
    \text{tree-level:}& &\quad \frac{g_{DM} \times g_+^f}{m_s^2} \quad &\sim \quad \frac{1}{v_\phi^4} \,, \nn\\[1ex]
    \text{box diagram:}& &\quad g_{DM}^2 \times (g_+^f)^2 \quad &\sim \quad \frac{1}{v_\phi^4} \,,
\end{alignat}
where both $g_{DM}$ and $g_+^f$ are suppressed by one power of $v_\phi$, and $m_s = 2 \sqrt{\lambda_\phi} \, v_\phi$. Consequently, spin-independent amplitudes are suppressed by $v_\phi^4$, compare with $v_\phi^2$ for the spin-dependent amplitudes which we will see in the next section. Moreover, spin-independent scatterings do not suffer from momentum suppression. On the contrary, due to the coherent cross section, an enhancement proportional to the square of the atomic mass $A^2$ further increases the sensitivity of spin-independent direct detection experiments. \\

We first calculate the tree-level scalar-mediated scattering. In the non-relativistic approximation, it can be described by an effective operator as
\begin{align}
...
The amplitude generated by this operator is proportional to the sum of nuclear matrix element
\begin{align}
...
In order to compute the box diagram contribution, we adapt the similar formalism as discussed in Refs.~\cite{Arcadi:2017wqi,Freytsis:2010ne}. We take the non-relativistic approach and start with an effective Lagrangian given by \footnote{Another detailed computation can be found in~\cite{Hisano:2010ct}, where the top-mass dependency of the DM-gluon effective interaction is considered.}
\begin{align}
...
\begin{align}
    g_n &= \sum_{q=u,d,s} \left[ g_+^q - \bar{m} \, \xi \right] \, \frac{\Delta_q^n}{m_q} \,, \\[1ex]
    {\rm where} \quad \bar{m} &= \left( m_u^{-1} + m_d^{-1} + m_s^{-1} \right)^{-1} \quad {\rm and} \quad \xi = \sum_{q\,=\,\text{all\,quarks}} \frac{g_+^q}{m_q} \,.
\end{align}
...
The mass of the DM depends on the FN charge of $N^1$, as
\begin{align}
