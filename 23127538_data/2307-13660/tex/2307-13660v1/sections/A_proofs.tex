...
\cthreshold*
\begin{proof}
    Recall that $\VV - \RR\UU\RR^T + \RR\WW - \WW^T\RR^T$ for any $\RR\in\RRR$ is symmetric with zero main diagonal by construction, and $\dis \RR$ is the largest magnitude among its entries. Let     $\RR, \RR' \in \RRR$ be such that $\dis \RR > \dis \RR'$ and therefore $\dis \RR \geq \dis\RR' + \rho$.
    ...
    \begin{align*}
        ...
        &\quad> 0.
    \end{align*}
    It follows that the 1-norm relaxation monotonically decreases with $\dis \RR$, and therefore $$\argmin_{\RR\in\RRR}\left\|c^{\VV - \RR\UU\RR^T + \RR\WW - \WW^T\RR^T} + c^{\RR\UU\RR^T - \VV + \WW^T\RR^T - \RR\WW}\right\|_1 \subseteq \argmin_{\RR\in\RRR}\dis\RR.$$
    % It follows that the 1-norm relaxation always decreases with the distortion, and therefore
    % \begin{align*}
    % \argmin_{\RR\in\RRR}\left\|c^{\VV - \RR\UU\RR^T + \RR\WW - \WW^T\RR^T} + c^{\RR\UU\RR^T - \VV + \WW^T\RR^T - \RR\WW}\right\|_1 \subseteq \argmin_{\RR\in\RRR}\left\|\VV - \RR\UU\RR^T + \RR\WW - \WW^T\RR^T\right\|_\infty.
    % \end{align*}
\end{proof}
...
\solutiononinterior*
\begin{proof}
The statement trivially holds for $\SS^* \in \RRR$ (recall that a vertex is its own interior). Otherwise, $\exists h,k,l$ s.t. $0 < S^*_{hk}, S^*_{hl} < 1$ meaning that $\SS^*$ lies on the open segment whose endpoints $\SS', \SS''$ are given by $S'_{ij} = \begin{cases}
...
\begin{cases}
    ...
    S^*_{ij} & \text{otherwise}
\end{cases}$. First, note that we can derive the following identity by applying the trace trick:
\begin{align*}
    \left\langle \SS', \nabla \obj(\SS'') \right\rangle &= 2\left\langle \SS', c^\VV\SS'' c^{-\UU} + c^{-\VV}\SS''c^\UU + \big(c^\WW\SS'' c^{-\WW} + c^{-\WW}\SS'' c^\WW\big)^T \right\rangle \\
    &= 2\left\langle \SS'', c^\VV\SS' c^{-\UU} + c^{-\VV}\SS'c^\UU + \big(c^{-\WW}\SS' c^{\WW} + c^{\WW}\SS' c^{-\WW}\big)^T \right\rangle \\
    &= \left\langle \SS'', \nabla \obj(\SS') \right\rangle.
\end{align*}
Denoting $\alpha \defeq \frac{S^*_{hk}}{S^*_{hk} + S^*_{hl}} \in (0, 1)$,
\begin{align*}
    \obj(\SS^*) &= \obj\left(\alpha \SS' + (1-\alpha)\SS''\right) \\
    &= \Big\langle \alpha \SS' + (1-\alpha)\SS'', \frac{1}{2}\nabla\obj\left(\alpha \SS' + (1-\alpha)\SS''\right)\Big\rangle \\
    &= \alpha^2\obj(\SS') + \alpha(1-\alpha)\left\langle \SS'', \nabla \obj(\SS') \right\rangle + (1-\alpha)^2\obj(\SS'') \\
    &\leq (1 - 2\alpha(1-\alpha)\obj(\SS^*) + \alpha(1-\alpha)\left\langle \SS'', \nabla \obj(\SS') \right\rangle,
\end{align*} which implies $$\left\langle \SS'', \nabla \obj(\SS') \right\rangle \leq 2 \obj(\SS^*) \leq \left\langle \SS', \nabla \obj(\SS')\right\rangle$$ and therefore
\begin{equation*}
    \nabla\obj(\SS')_{hl} \leq \nabla\obj(\SS')_{hk}. \tag{1}
\end{equation*}
By the analogous reasoning using $\left\langle \SS', \nabla \obj(\SS'') \right\rangle$ in place of $\left\langle \SS'', \nabla \obj(\SS') \right\rangle$,
\begin{equation*}
    \nabla\obj(\SS'')_{hk} \leq \nabla\obj(\SS'')_{hl}.    \tag{2}
\end{equation*}
...
\begin{equation*}
\begin{aligned}
    \nabla\obj(\SS')_{hk} - \nabla\obj(\SS'')_{hk} &= 2(S^*_{hk} + S^*_{hl})\big(2 - c^{-U_{lk}} - c^{U_{lk}} + 2 - c^{W_{kh}}c^{-W_{lh}} - c^{-W_{kh}}c^{W_{lh}}\big) \\ &\leq 0,
\end{aligned} \tag{3}
\end{equation*}
\begin{equation*}
\begin{aligned}
        \nabla\obj(\SS')_{hl} - \nabla\obj(\SS'')_{hl} &= 2(S^*_{hk} + S^*_{hl})\big(c^{-U_{kl}} + c^{U_{kl}} - 2 + c^{W_{lh}}c^{-W_{kh}} + c^{-W_{lh}}c^{W_{kh}} - 2\big) \\ &\geq 0. 
\end{aligned}\tag{4}
\end{equation*}
Combining inequalities (1), (2), (3), and (4) gives $\nabla\obj(\SS')_{hk} = \nabla\obj(\SS')_{hl}$. Then $$\left\langle \SS'', \nabla \obj(\SS')\right\rangle = \left\langle \SS', \nabla \obj(\SS') \right\rangle = 2 \obj(\SS^*)$$ and therefore $\SS' \in \argmin_{\SS \in \SSS}\obj(\SS)$.
...
Since $\obj$ is quadratic, attaining the same value at any three points on the same line renders it constant on the entire line. In particular, any face $\Psi$ is included in $\argmin_{\SS\in\SSS}\obj(\SS)$ provided that $$\partial\Psi \cup \{\SS^\mathrm{o}\} \subseteq \argmin_{\SS\in\SSS}\obj(\SS)$$ for some interior point $\SS^\mathrm{o} \in \Psi\setminus\partial\Psi$. It follows that any positive-dimension face of $\Phi$ must be a part of the solution set if all the faces of $\Phi$ of dimension smaller by 1 are. Starting from the vertices of $\Phi$ and applying induction yields $\Phi \subseteq \argmin_{\SS\in\SSS}\obj(\SS)$.
\end{proof}

\voronoiface*
\begin{proof}
    %Let $\SS = \sum_{i=1}^k\alpha_i \RR_i$ for some $\sum_{i=1}^k=1$.
    For any $\RR \in \RRR$, $$\|\SS - \RR\|_2^2 = \|\SS\|_2^2 - 2\langle \SS, \RR \rangle + n+m,$$ and therefore $$\argmin_{\RR \in \RRR}\,\|\SS - \RR\|_2 = \argmax_{\RR \in \RRR} \;\langle \SS, \RR \rangle.$$
    Let $s_i$ denote the maximum entry in the $i$-th row of $\SS$ for $i=1,\ldots,n+m$. Then $$\max_{\RR \in \RRR} \;\langle \SS, \RR \rangle = \sum_{i=1}^n s_i$$ and $\RR^* \in \argmax_{\RR \in \RRR} \;\langle \SS, \RR \rangle$ means that $R^*_{ij} \neq 0$ only if $S_{ij} = s_i$. Because $\SS$ is a convex combination of the vertices of $\Phi$, $S_{ij} = s_i > 0$ in turn implies the existence of $\RR \in \Phi \cap \RRR$ s.t. $R_{ij} \neq 0$. By Lemma \ref{lem:polytope_geometry}, $\RR^*$ must be in $\Phi$.
\end{proof}

% Proof of Theorem \ref{thm:time_complexity}:
% \begin{proof}
%     The time complexity of a single Frank--Wolfe iteration is defined by the calculations of gradient at the current point, the direction of smallest correlation with it, and the step size to take along said direction. Computing the gradient $\nabla \obj(\SS_i)$ is comprised of multiplying and summing $(n+m)\times(n+m)$ matrices which has the time complexity of $O(n^3)$. Finding the descent direction $\DD_i$ amounts to solving $\min_{\RR\in\RRR}\langle \RR, \nabla\obj(\SS_i) \rangle$, which can be done in $O(n^2)$ time by locating the smallest entry in each row of $\nabla\obj(\SS_i)$. Finally, the quadratic minimization step $\min_{\gamma \in [0, 1]} \obj(\gamma\SS_i + (1-\gamma)\DD_i)$ admits a closed-form solution that requires $O(n^3)$-time computations including of $\ob2j(\DD_i)$.
% \end{proof}

\oneminimizer*
\begin{proof}
    $\left|\argmin_{\SS\in\SSS} \left\langle \SS, \nabla\obj(\SS^*) \right\rangle\right| = 1$ if and only if the smallest entry in each row of $\nabla\obj(\SS^*)$ is unique.  In order for $\nabla\obj(\SS^*)_{hk} = \nabla\obj(\SS^*)_{hl}$ for some $h$ and $k\neq l$, the realizations $d_1, \ldots, d_\frac{n(n-1)}{2}$ of the distances in $X$ must satisfy
    ...
    which can be rewritten as $$\sum_{i=0}^\frac{n(n-1)}{2}\sum_{j=0}^\frac{n(n-1)}{2}a_{ij}c^{d_i - d_j} = 0$$ for some $a_{ij} \in \R$ and $d_0 \defeq 0$. Because the above can be cast as a generalized polynomial through a change of variables, it must have a finite number of solutions. The probability of the distances in $X$ to form a particular solution $d_1^*, \ldots, d_\frac{n(n-1)}{2}^*$ is
    \begin{align*}
        &\mathbb{P}_\XX\left[D_1=d^*_1,\ldots,D_\frac{n(n-1)}{2}=d_\frac{n(n-1)}{2}^*\right] \\ &\hspace{2cm} = \mathbb{P}_\XX\left[D_1=d^*_1|D_2=d_2^*,\ldots,D_\frac{n(n-1)}{2}=d_\frac{n(n-1)}{2}^*\right]\mathbb{P}_\XX\left[D_2=d_2^*,\ldots,D_\frac{n(n-1)}{2}=d_\frac{n(n-1)}{2}^*\right] \\
        &\hspace{2cm} = 0\cdot\mathbb{P}_\XX\left[D_2=d_2^*,\ldots,D_\frac{n(n-1)}{2}=d_\frac{n(n-1)}{2}^*\right]
        \\
        &\hspace{2cm} = 0,
    \end{align*} and therefore
    $\mathbb{P}_\XX\left[\nabla\obj(\SS^*)_{hk} = \nabla\obj(\SS^*)_{hl}\right] = 0.$
    Then
    \begin{align*}
        \mathbb{P}_\XX\left[\Big|\argmin_{\SS\in\SSS} \langle \SS, \nabla\obj(\SS^*) \rangle\Big| > 1\right] &\leq \sum_{h=1}^{n+m}\sum_{k=1}^{n+m}\sum_{l=k+1}^{n+m} \mathbb{P}_\XX\left[\nabla\obj(\SS^*)_{hk} = \nabla\obj(\SS^*)_{hl}\right] \\ &= 0.
    \end{align*}
\end{proof}
...
\begin{proof}
    ...
    \end{align*}
    To bound $\lambda_{-}$ from above%the total magnitude of eigenvalues
    , we will first introduce two new notations for convenience. For any two matrices $\mathbf{A}$ and $\mathbf{B}$, let $\mathbf{A} \ominus \mathbf{B}$ denote a matrix operation analogous to the Kronecker product but with subtraction in place of multiplication. In addition, define $\ch:\R\to\R$ as $\ch(a) \defeq c^a + c^{-a}$ and its entry-wise counterpart for matrices $\ch:\R^{p\times q}\to\R^{p\times q}$, so that we are able to compactly write
    \begin{align*}
    ...
    We are now ready to provide an upper bound on the eigenvalues' total magnitude. Using the symmetry of $\HH$,
    \begin{align*}
