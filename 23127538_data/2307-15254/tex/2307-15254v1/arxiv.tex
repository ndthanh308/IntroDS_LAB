...
% \title{Hard Instance Mining based Multiple Instance Learning Framework for Whole Slide Image Classification}
% \title{MHIM-MIL: Masked Multiple Instance Learning Framework with Hard Instance Mining for Whole Slide Image Classification}
% \title{Masked Hard Instance Mining Multiple Instance Learning for Whole Slide Image Classification}
% \title{Mining Hard Instances with Masked Multiple Instance Learning for boosting Whole Slide Image Classification}
% \title{MHIM-MIL: Hard Instance Mining Multiple Instance Learning Framework for Whole Slide Image Classification}

\author{Wenhao Tang$^1$ ~~ Sheng Huang$^1$$\thanks{Corresponding Author.}$ ~~ Xiaoxian Zhang$^1$ ~~ Fengtao Zhou$^2$ ~~ Yi Zhang$^1$ ~~ Bo Liu$^3$\\
{ $^1$ Chongqing University ~~ $^2$ The Hong Kong University of Science and Technology ~~ $^3$ Walmart}\\
{\tt\small \{whtang, huangsheng, zhangxiaoxian, zhangyii\}@cqu.edu.cn, fzhouaf@connect.ust.hk, kfliubo@gmail.com}
}
...
\label{sec:intro}
Histopathological image analysis plays a crucial role in modern medicine, particularly in the treatment of cancer, where it serves as the gold standard for diagnosis~\cite{pinckaers2020intro_pami,tellez2019intro_pami_2,li2021dt,zhao2022setmil,lu2021nature}.
...
Multiple Instance Learning (MIL)~\cite{dietterich1997solving} has been widely used in WSI analysis with its unique learning paradigm in recent years~\cite{manivannan2017subcategory,xu2014weakly,xu2019camel,li2021dual,shao2021transmil,tong2014multiple}.
...
We define an $N$-dimensional binary vector $M=[m_1,\dots,m_i,\dots,m_N]$ for encoding the mask flags of instances where $m_i\in\{0,1\}$. If $m_i=1$, the $i$-th instance is unmasked, otherwise, it is masked.
...
we adopt 3-times three-fold cross-validation on this dataset to ensure that each slide is used in training and testing, which can alleviate the impact of data split and random seed on the model evaluation.
Each fold has approximately 133 slides. We report the mean and standard
deviation of performance metrics over three runs.
...
Following the previous work~\cite{shao2021transmil, clam} we leverage Accuracy, Area Under Curve (AUC), to evaluate model performance.
Besides, F1-score is also considered.
AUC is the primary performance metric in the binary classification task, and we only report AUC in ablation experiments. Please refer to the Supplementary for the details of these two datasets.
...
For details on network architectures and training are described in Supplementary Material.
% ----------------------------------------------------------
...
AB-MIL & 94.00 & 93.17 &\textbf{657K} & \textbf{4.0s} & 2.4G \\
...
We provide more details and an in-depth analysis of this patch visualization in Supplementary Material.
...
\begin{algorithm*}[t]
  ...
  \Indp   % start indent
    ...
    \PyCode{logits\underline{~}t,attn\underline{~}t = f\underline{~}t.forward(x)}\\
    \PyComment{stop gradient of teacher network}\\
    \PyCode{logits\underline{~}t = logits\underline{~}t.detach()}\\
    ...
    ~\\
    \PyCode{logits\underline{~}s,\underline{~} = f\underline{~}s.forward(x\underline{~}hard)}\\
    ~\\
    \PyComment{consistency loss}\\
    \PyCode{loss\underline{~}con = -softmax(logits\underline{~}t / tp) * log\underline{~}softmax(logits\underline{~}s)}\\
