...
\item \textbf{Effectiveness}: Successful ownership verification of stolen models with high confidence.
...
The first part of Equation~\ref{eq:lossfunction} maximizes the categorical cross-entropy loss between $\pi_{\mathcal{V}}$'s predictions for clean and adversarial states. The second part minimizes the categorical cross-entropy loss of independent policies in the same clean and adversarial states only if the predicted action for $\vect{s}_t$ is the same for both $\pi_{\mathcal{V}}$ and $\pi_{i}$. Multiple independently trained models are used to calculate the second part of Equation~\ref{eq:lossfunction} for each $i \in \mathcal{I}$ by taking the average of individual losses. A universal adversarial mask should also achieve a high fooling rate $\delta_{\vect{r}}$ that was presented in Equation~\ref{eq:foolingrate}. 
...
The final verdict (stolen vs. independent) is made based on the \emph{majority vote}. \fingerprint also returns $AA$ averaged on all fingerprints to quantify the confidence in the final decision. The verification procedure is summarized by Algorithm~\ref{alg:fingerprint_verification}.
