\documentclass[conference]{IEEEtran}
\IEEEoverridecommandlockouts
% The preceding line is only needed to identify funding in the first footnote. If that is unneeded, please comment it out.
\usepackage{cite}
\usepackage{amsmath,amssymb,amsfonts}
\usepackage{bbm}
\usepackage{algorithmic}
\usepackage{graphicx}
\usepackage{textcomp}
\usepackage{xcolor}
% \usepackage{subfigure}
\usepackage{booktabs}
\usepackage{makecell}
\usepackage{enumerate}
\usepackage{amsthm}
\usepackage{algorithm}
\usepackage{mathtools}
% \usepackage{appendix}
\usepackage{etoolbox}
\usepackage{apptools}
\usepackage{float}
\usepackage{caption}
\usepackage{subcaption}
\usepackage{multirow}
\usepackage{tabularray}
\usepackage[T1]{fontenc}
\usepackage{balance}
\usepackage{xspace}
\usepackage{pifont}
% \AtAppendix{\counterwithin{lemma}{section}}  % recount lemma as A.1 ...
\BeforeBeginEnvironment{appendices}{\clearpage}
\newtheorem{assumption}{\textbf{Assumption}}
\newtheorem{lemma}{\textbf{Lemma}}
\newtheorem{theorem}{\textbf{Theorem}}
\theoremstyle{definition}
\newtheorem{remark}{\textbf{Remark}}

\newcommand{\rednote}[1]{\textcolor{red}{#1}}
\newcommand{\bluenote}[1]{\textcolor{black}{#1}}
\newcommand{\redcomment}[1]{$\langle$\textcolor{red}{\textbf{#1}}$\rangle$}
\newcommand{\bluecomment}[1]{$\langle$\textcolor{blue}{\textbf{#1}}$\rangle$}

\def\ie{\textit{i.e.}\xspace}
\def\etal{\textit{et al.}\xspace}
\def\etc{\textit{etc.}\xspace}
\def\whp{\textit{w.h.p.}\xspace}
\def\eg{\textit{e.g.}\xspace}
\def\iid{\textit{i.i.d.}\xspace}
\def\st{\xspace\textbf{s.t.}\xspace}
\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\argmin}{arg\,min}

\def\BibTeX{{\rm B\kern-.05em{\sc i\kern-.025em b}\kern-.08em
    T\kern-.1667em\lower.7ex\hbox{E}\kern-.125emX}}

\def\InvReg{global updating\xspace}
\def\Reg{local updating\xspace}
\def\locReg{split forward and backward propagation\xspace}
\def\srvUp{supervised training\xspace}
\def\srvmodh{\boldsymbol{w}^{h - \frac{1}{2}}}

\begin{document}

% \title{Efficient Semi-Supervised Federated Learning in Heterogeneous Networks*\\
% {\footnotesize \textsuperscript{*}Note: Sub-titles are not captured in Xplore and
% should not be used}
\title{Efficient Semi-Supervised Federated Learning for Heterogeneous Participants
%\thanks{Identify applicable funding agency here. If none, delete this.}
}

\author{
    \IEEEauthorblockN{
		Zhipeng Sun,
		Yang Xu,~\IEEEmembership{Member,~IEEE,}
		Hongli Xu,~\IEEEmembership{Member,~IEEE,}
        Zhiyuan Wang
    }
    \IEEEauthorblockA{
		School of Computer Science and Technology, University of Science and Technology of China, Hefei, China\\
		Suzhou Institute for Advanced Research, University of Science and Technology of China, Suzhou, China\\
		rodman@mail.ustc.edu.cn, xuyangcs@ustc.edu.cn, xuhongli@ustc.edu.cn, cswangzy@mail.ustc.edu.cn
    }
}

\maketitle

\begin{abstract}
\input{contents/abstract.tex}
\end{abstract}

\begin{IEEEkeywords}
Federated Learning, Split Learning, Semi-Supervised Learning
\end{IEEEkeywords}

\section{Introduction}\label{intro}
\input{contents/intro.tex}

\section{Background}\label{bg}
\input{contents/background.tex}

\section{System Description}\label{workflow-section}
\input{contents/system.tex}

\section{Algorithm Design}
\input{contents/algorithm.tex}

\section{Experiments and Evaluation}
\input{contents/evaluation.tex}

\section{Related Works}
\input{contents/relworks.tex}

\section{Conclusions}
\input{contents/conclusions.tex}

% \section*{Acknowledgment}

% The preferred spelling of the word ``acknowledgment'' in America is without 
% an ``e'' after the ``g''. Avoid the stilted expression ``one of us (R. B. 
% G.) thanks $\ldots$''. Instead, try ``R. B. G. thanks$\ldots$''. Put sponsor 
% acknowledgments in the unnumbered footnote on the first page.

% \section*{References}
\balance
\bibliographystyle{IEEEtran}
\bibliography{reference}

% APPENDIX
\input{contents/appendix.tex}

\end{document}
