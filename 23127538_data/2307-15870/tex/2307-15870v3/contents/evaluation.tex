...
\bluenote{The STL-10 dataset contains images from 10 different classes.
...
\textbf{Models:} Three models with different types and structures are adopted on the above three real-world datasets for performance evaluation: (i) CNN on SVHN, (ii) AlexNet \cite{krizhevsky2017imagenet} on CIFAR-10, (iii) VGG13 \cite{simonyan2014very} on STL-10, (iv) VGG16 \cite{simonyan2014very} on IMAGE-100. 
...
% \bluenote{
We evaluate the performance of \method on an EC hardware prototype system, which consists of an AMAX deep learning workstation equipped with an Intel(R) Xeon(R) Platinum 8358P CPU, 8 NVIDIA GeForce RTX A6000 GPUs, and 512 GB RAM, along with 80 Nvidia Jetson devices.
...
Due to random channel noise and varying distances between workers and the router, the outbound bandwidth fluctuates between 0.8 Mbps and 8 Mbps, and the inbound bandwidth varies between 10 Mbps and 20 Mbps.
...
\method also improves accuracy from 2.5\% to 37.0\% compared to FedMatch across the four datasets, highlighting the effectiveness of our proposed clustering regularization in \method.
...
\bluenote{FedMatch is excluded from the comparison plots on IMAGE-100 and from some bars in the STL-10 plots because it fails to achieve the target accuracy.}
...
For instance, to achieve 80\% accuracy on CIFAR-10, \method requires 86.5 min on average to train an AlexNet, compared to 113.6 min for FedSwitch-SL and 279.9-702.4 min for other FL baselines, indicating speed-ups of 1.3-8.1$\times$ over the baselines. For a higher target accuracy of 85\%, \method takes 322.8 min, while FedSwitch-SL requires 330.9 min, and other Semi-FL baselines span from 591.9 to 1211.9 min. 
In contrast, \method only takes 322.8 min.
\bluenote{Additionally, \method speeds up training by 3.6-19.5$\times$ to reach 85\% accuracy with VGG13 on STL-10 and by 3.8-7.0$\times$ to reach 60\% accuracy with VGG16 on IMAGE-100 compared to FedSwitch-SL and other Semi-FL baselines.}
...
\bluenote{Similarly, Fig. \ref{fig-commcost}(c) shows a 72.9\%-92.9\% reduction in communication costs for \method to reach 85\% accuracy on STL-10, and Fig. \ref{fig-commcost}(d) shows a 73.6\%-83.5\% reduction to reach 60\% accuracy on IMAGE-100.}
...
We compare \method with baselines dealing with data non-IIDness. This is implemented through sampling data from a Dirichlet distribution $Dir(\alpha)$ \cite{hsu2019measuring} for each client.
...
It demonstrates the effectiveness of \method in mitigating data non-IIDness, which is getting more significant on datasets with a wider variety of categories.
...
Notably, the performance of FedMatch fluctuates dramatically for around 200 minutes since it uses decomposed model parameters for supervised training and semi-supervised training, highlighting the detrimental effect of such disjoint learning on model convergence.
...
\bluenote{Notably, \method with client labels achieves significant improvement in accuracy, up to 7.6\% compared to the models that do not leverage labeled data.}
...
These results demonstrate that the adaptive algorithm substantially enhances training efficiency and model performance environments with label scarcity by intelligently adjusting updating frequencies. 
...
As shown in Fig. \ref{fig-abs}(a), settings between 0.9 and 0.99 yield the best performance, while the accuracy declines at 0.999 by delaying the integration of new information from unlabeled data since the teacher model is only updated on the PS. 
