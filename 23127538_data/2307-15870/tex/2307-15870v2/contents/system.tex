...
% In this section, we delve into the detailed workflow of our system. 
In our system, \textit{cross-entity semi-supervised training} always follows the \textit{server-side \srvUp}.
Such alternate training procedure will be conducted for $H$ rounds. 
Fig. \ref{workflow-fig} illustrates the workflow of our system. 
% (teacher top models are omitted for simplicity). 
In round $h$, our system workflow consists of the following five processes:
...
Considering the feed-forward process, any data sample $x$ in a mini-batch $\mathcal{B}_{u, i} \subset \mathcal{D}_{u,i}$, $|\mathcal{B}_{u, i}| = d_i^{h + \frac{1}{2},k}$
\bluenote{
first undergoes weak augmentation $a_w(\cdot)$, including random horizontal flipping and cropping, and strong augmentation $a_s(\cdot)$, exemplified by RandAugment \cite{cubuk2020randaugment} in our system. 
