...
\end{proof}
Theorem~\ref{theorGraphCircuits} has a simple interpretation: to check that in a picture all rows and columns are Dyck words of respectively $D^{Row}_k$ and $D^{Col}_k$, we can proceed along each linear path. The process moves from an open parenthesis (say $a$) to the closed matching letter ($b$) on the same row, then to the closed matching letter ($d$) on the column of $b$, and so on, until the circuit is closed, or interrupted causing rejection.
...
\psset{arrows=-,labelsep=15pt,colsep=10pt,rowsep=15pt,nodealign=true}         
		\scalebox{0.9}{
		\begin{psmatrix}                                   
\pnode{p11}{$a$}&\pnode{p12}{$a$}&\pnode{p13}{$b$}&\pnode{p14}{$a$}&\pnode{p15}{$b$}&\pnode{p16}{$b$}                          
