...
\maketitle
\begin{abstract}
The Navier-Stokes Equations on the Euclidean space $\mathbb{R}^3$ may be written as $\partial_t u -\Delta u +\mathcal{E}(u,u) =0,$
where the bilinear operator $\mathcal{E}$ formally allows the dissipation of energy via the "cancellation identity" $\langle \mathcal{E}(u,u), u\rangle=0$. We find an averaged version of $\mathcal{E}$, call it $C$, satisfying the cancellation identity and such that 
$\partial_t u +(-\Delta)^\alpha u +C(u,u)=0$
admits a solution that blows up in finite time for all $\alpha \in (1/2,5/4)$. Additionally, we find a class $\mathfrak{B}$ of bilinear operators whose corresponding equations admit the following partial regularity theorem. Let $B\in \mathfrak{B}$, $n\geq2$, and $\alpha \in (1/2, (n+2)/4)$, and assume that a solution of the pseudodifferential equation 
$\partial_t u +(-\Delta)^\alpha u +B(u,u)=0$ is smooth on $\mathbb{R}^n\times [0,T)$. Then the solution is also smooth at the blowup time $T$ away from a closed set of Hausdorff dimension at most $n+2-4\alpha$. Lastly, we show that the operator $C\in \mathfrak{B}$, which connects our two results and strongly suggests that obtaining a positive answer to the Navier-Stokes global regularity problem requires more than a refinement of partial regularity theory.
\end{abstract}
\tableofcontents
...
\end{gathered}
\end{equation}
The $\alpha$-dissipative Navier-Stokes system is typically called hyperdissipative if $\alpha>1$ and hypodissipative if $\alpha<1$. The case when $\alpha=1$ coincides with the classical Navier-Stokes equations. For $\alpha\geq 5/4$ and for any smooth function $u_0$ which decays sufficiently fast at infinity, it is known that the system (\ref{FRACNS}) has a classical global-in-time solution. In the case of Equation (\ref{FRACNS}) on $\mathbb{R}^n$, classical global-in-time solutions are guaranteed whenever $\alpha >(n+2)/4$ and the initial data is smooth. We offer a very short proof of the latter fact in Section 5.6. Global regularity in $\mathbb{R}^3$ is also known for an open set of initial data if $\alpha$ is slightly less than $5/4$, see \cite{CH}. Generally, when $\alpha<5/4$, the global existence of classical solutions is still an open question. As one of the Millennium problems, the question of global regularity is stated in \cite{F} as:
\begin{conjecture}
...
where $1/p = 1/q+1/r$ and $1< p,q,r< \infty$, and whose associated partial differential equation admits a finite-time singularity.
...
There exists a symmetric bilinear operator $C$, which is an averaged version of $\mathcal{E}$, obeys the cancellation identity
...
\end{enumerate}
\end{definition}
The class $\mathfrak{B}$ is the collection of all amenable operators. Throughout the rest of our work, we may alternately say that $B\in \mathfrak{B}$ or that $B$ is amenable. Our main partial regularity theorem is:
\begin{theorem}
\label{PRMAINMAIN}
Let $n\geq 2$ and $1/2<\alpha<(n+2)/4$. Let $B$ be an amenable bilinear operator.
...
Our theorem is a partial regularity result of wide generality. For instance the $\alpha$-dissipative Navier-Stokes equations in three-dimensions were previously known to admit partial regularity whenever $\alpha>3/4$. The authors of \cite{TY} dealt with the hypodissipative case $3/4<\alpha <1$ and the work of \cite{CDM} dealt definitively with the hyperdissipative case. Additionally, the only partial regularity result that we know of for non-stationary higher-dimensional Navier-Stokes equations is found in \cite{DG}, whose authors prove a result analogous to the one in \cite{CKN} in four dimensions. On the other hand, an immediate corollary of Theorem \ref{PRMAINMAIN} is a partial regularity result for the $\alpha$-dissipative Navier-Stokes equations whenever $\alpha>1/2$ and for any dimension $n\geq 2$, which, since it is an improvement on what is already known, we state as a separate corollary:
\begin{corollary}
Let $n\geq 2$ and $1/2<\alpha<(n+2)/4$.
Suppose $u(t)$ is a smooth vector field on $\mathbb{R}^n\times [0,T) \to \mathbb{R}^n$ and suppose there exists a smooth function $p$ on $\mathbb{R}^n\times [0,T)$ such that:
$$
\begin{gathered}
\partial_t u +(-\Delta )^\alpha u+(u\cdot\nabla)u+\nabla p=0\\
...
If $T$ is the time of first blowup, then the closed set $S_T$ has Hausdorff dimension at most $n+2-4\alpha$.
\end{corollary}
Our corollary for the $\alpha$-dissipative Navier-Stokes equations extends the range of partial regularity to the farthest endpoints expected by scaling. In particular, when $\alpha\geq (n+2)/4$, we know that solutions are globally regular, and when $\alpha \leq 1/2$, we see that the scaling of the equation predicts no useful partial regularity result. We should also mention that the authors of \cite{CDR} prove an ill-posedness result for weak (but not Leray-Hopf weak) solutions of (\ref{FRACNS}) when $\alpha<1/2$, further evidence that it is a "natural" and non-dimensional endpoint for fractionally dissipative Navier-Stokes.

 We find it important to note that while previous results like those of \cite{CDM}, \cite{TY}, \cite{CKN}, and \cite{DG} prove partial regularity of the solution in space-time, our theorem only concludes that the solution is partially regular in space at the blowup time. Thus, although our technique does not require a priori $L^3$ spacetime integrability to estimate the nonlinear term (and this allows us to reach $\alpha>1/2$), our theorem has a weaker conclusion and does not prove a space-time partial regularity result.

We believe it beneficial to the reader to now succinctly outline the rest of our paper. In Section 2 we prove Theorem \ref{PRMAINMAIN}. In particular, given the vector field solution $u$, we construct a set $M$ of Hausdorff dimension $n+2-4\alpha$ and prove that it contains the singular set $S_T$. In Section 3, we test the partial differential equation in the statement of Theorem \ref{PRMAINMAIN} with localizations of the solution in both frequency and physical space. We then prove estimates on the size of these wavelet coefficients that are used in the proofs of Section 2. In Section 4, we show that Tao's method is flexible enough to construct a blowup solution to the pseudodifferential equation associated to some amenable bilinear operator, which, together with Theorem \ref{PRMAINMAIN}, proves Theorem \ref{MAINMAIN}. In Section 5, we prove the lemmas that form our "technical toolbox" and that are used throughout the paper. 

We would like to thank our advisor Camillo de Lellis for suggesting the topic of this paper, for many helpful discussions, and for reviewing several drafts of our work. This paper would never have come to fruition without his support. We would also like to thank Vikram Giri for constructive conversations about mathematical fluid dynamics in general and about the paper \cite{KP} in particular. Lastly, we would like to acknowledge the support of the National Science Foundation in the form of an NSF Graduate Research Fellowship and under Grant No. DMS-1946175.
...
we have the usual $\alpha$-dissipative Navier-Stokes equations in Equation (\ref{PEQN}). Thus, our method here improves the range of $\alpha$ from results in the previously mentioned works of \cite{S}, \cite{KP}, \cite{CDM}, and \cite{TY}.
...
Throughout the rest of our paper, the symbol $K$ will denote a constant appropriately chosen so that every inequality where it appears is satisfied. The choice of $K$ will be obvious at each instance of its use, and it will only depend on the $L^2$ norm of the initial data, the blowup time $T$, the dimension $n\geq 2$, and the fixed constants $\epsilon, \alpha$. Also, we introduce several other parameters that depend on $\alpha$ and the dimension $n\geq 2$: $\mu_0>3000$ that will appear in a forthcoming definition, $p_0>2$ that will be the exponent in an application of the H\"{o}lder Inequality, and the parameters $\gamma, \delta$, which will be used in Section 3 when we estimate the nonlinear term. Given $\alpha>1/2$ and $n\geq 2$, we shall carefully choose the parameters $\delta, \mu_0, p_0$, and $\gamma$ so that our argument closes, in a way that we describe below. 
...
$$2^{\mu_0j}\int_{T-2^{-\mu_0j}}^T u_{\mathcal{N}^{1000/\epsilon}(Q)}^2dt+\int_0^T \int \sum_{k\geq j} 2^{2\alpha k}\abs{\phi_{Q,j}P_{k} u}^2 dxdt \geq K 2^{-(n+2-4\alpha)j-100\epsilon j-\gamma j}.$$
...
$$\leq K \sum_{Q'} \bigg(\sum_{k\geq j}2^{2\alpha k} \int_0^T\int \abs{\phi_{Q',j}P_k u}^2 + 2^{\mu_0j}\int_{T-2^{-\mu_0j}}^T u^2_{\mathcal{N}^{1000/\epsilon}(Q')}dt\bigg) \leq$$
$$\leq K \bigg(  \sum_{k\geq j}2^{2\alpha k}\int_0^T \int \sum_{Q'}\abs{\phi_{Q',j}P_{k} u}^2 + 2^{\mu_0j}\int_{T-2^{-\mu_0j}}^T \sum_{Q'}u^2_{\mathcal{N}^{1000/\epsilon}(Q')}dt\bigg) \leq K,$$
...
\begin{equation}\label{GOODYTWOSHOES} T-2^{-\mu_0j}< t_n\end{equation}
...
$$\abs{ \langle-\hat{B}(u,u),P_j \phi_{Q',j}^2P_j u\rangle} \leq K2^{j(1+\frac{n\delta}{2}+\frac{n\gamma}{3})}u_{\mathcal{N}^1(Q')}u_{Q'}+K\sum_{k=\delta j}^{j-1000/\epsilon} 2^{\frac{nk}{2}+j +\frac{n\gamma j}{3}}u_{Q'_k}u_{\mathcal{N}^1(Q')}u_{Q'}+$$
$$+ K \sum_{k>j+1000/\epsilon} 2^{nj(\frac{1}{2}-\frac{2\gamma}{3}) +k(n\gamma +1)}u_{Q'}\|\phi_{Q'_j,j}P_k u\|_2^2 +K2^{\frac{(n+2)j}{2}+\frac{n\gamma j}{3}}u_{Q'}u^2_{\mathcal{N}^{1000/\epsilon}(Q')}+K2^{-150j}.$$

We shall use the definition of mildly bad cubes to appropriately bound the terms above and achieve a contradiction.
For the rest of the proof we denote $Q_1$ to be an arbitrary member of the collection $\mathcal{N}^1(Q')$. We first aim to bound the contribution from the "high-low" terms. We observe that since $\alpha>1/2$, we may choose $\delta>0$ and $\gamma>0$ small enough so that $2^{j(1+\frac{n\delta}{2} +\frac{n\gamma}{3})}u_{Q_1}\leq K2^{2\alpha j}u_{\mathcal{N}^1(Q')}$. It follows that 
\begin{equation}\label{1bad1}\int_{t_n}^TK2^{j(1+\frac{n\delta}{2}+\frac{n\gamma}{3})}u_{\mathcal{N}^1(Q')}u_{Q'}dt\leq \int_{t_n}^TK2^{2\alpha j}u_{\mathcal{N}^1(Q')}^2dt.\end{equation}
...
$$\int_{t_n}^T 2^{\frac{nk}{2}+j+\frac{n\gamma j}{3}}u_{Q_1}u_{Q_2}dt.$$
Since $Q_1$ and $Q_2$ are both not mildly bad, we have:
$$\int_{t_n}^T 2^{nk}u_{Q_2}^2 dt \leq K 2^{(2\alpha-2)k-100\epsilon k -\gamma k }$$
$$\int_{t_n}^T 2^{2j+\frac{2n\gamma j}{3}}u_{Q_1}^2 \leq K 2^{(4\alpha -n)j+\frac{\gamma(2n-3)j}{3} -100\epsilon j}(T-t_n).$$
Thus, by the Cauchy-Schwartz inequality we have:
$$\int_{t_n}^T 2^{\frac{nk}{2}+j+\frac{n\gamma j}{3}}u_{Q_1}u_{Q_2}dt \leq K2^{(1-\alpha+50\epsilon +\frac{\gamma}{2})(j-k)+j(3\alpha-\frac{n+2}{2}+\frac{\gamma(2n-6)}{6}-100\epsilon)}(T-t_n).$$
We use that $1000/\epsilon<j-k\leq (1-\delta)j$ and let $\gamma, \epsilon$ sufficiently small and $\mu_0$ sufficiently large to conclude that for some constant $K$ depending on $\epsilon$:
$$\int_{t_n}^T 2^{\frac{nk}{2}+j+\frac{n\gamma j}{3}}u_{Q_1}u_{Q_2}dt \leq K 2^{-140j}.$$
...
\sum_{k=\delta j}^{j-1000/\epsilon}\int_{t_n}^T 2^{\frac{nk}{2}+j+\frac{n\gamma j}{3}}u_{\mathcal{N}^1(Q')}u_{Q_k'}u_{Q'}dt \leq K 2^{-140j}
\end{equation}
For the "local" frequencies, we use $u_{Q'}\leq K$, the definition of the mildly bad cubes, and a choice of $\mu_0$ sufficiently large depending on $n$ to conclude:
$$
\int_{t_n}^T 2^{\frac{(n+2)j}{2}+\frac{n\gamma j}{3}}u_{Q'}u^2_{\mathcal{N}^{1000/\epsilon}(Q')}dt \leq$$ \begin{equation} \label{1bad3}\leq \int_{t_n}^T2^{\frac{(n+2)j}{2}+\frac{n\gamma j}{3}}u^2_{\mathcal{N}^{1000/\epsilon}(Q')}dt\leq K (T-t_n)2^{-(n+2-4\alpha)j+(\frac{n+2}{2}+\frac{n\gamma}{3})j}\leq  K 2^{-140j}.   
\end{equation}
...
$$2^{nj(\frac{1}{2}-\frac{2\gamma}{3})} \int_{t_n}^T u_{Q'}\bigg( \sum_{k>j+1000/\epsilon}2^{k(n\gamma+1)}\|\phi_{Q'_j,j}P_k u\|_2^2\bigg)dt \leq$$ $$\leq 2^{nj(\frac{1}{2}-\frac{2\gamma}{3})}\sum_{k\geq j+1000/\epsilon} 2^{k(\gamma n+1)} \bigg(\int_{t_n}^T u_{Q'}^{p_0} dt\bigg)^{\frac{1}{p_0}}\bigg(\int_{t_n}^T\|\phi_{Q'_j,j}P_k u\|_2^{\frac{2p_0}{p_0-1}} dt\bigg)^{\frac{p_0-1}{p_0}}.$$
Using the energy dissipation law and our assumption that $p_0>2$, we can perform some crude estimates to get:
$$2^{nj(\frac{1}{2}-\frac{2\gamma}{3})}\sum_{k\geq j+1000/\epsilon} 2^{k(\gamma n+1)} \bigg(\int_{t_n}^T u_{Q'}^{p_0} dt\bigg)^{\frac{1}{p_0}}\bigg(\int_{t_n}^T\|\phi_{Q'_j,j}P_k u\|_2^{\frac{2p_0}{p_0-1}} dt\bigg)^{\frac{p_0-1}{p_0}}\leq $$
$$\leq K2^{nj(\frac{1}{2}-\frac{2\gamma}{3})}\sum_{k\geq j+1000/\epsilon} 2^{k(\gamma n+1)} \bigg(\int_{t_n}^T u_{Q'}^{2} dt\bigg)^{\frac{1}{p_0}}\bigg(\int_{t_n}^T\|\phi_{Q'_j,j}P_k u\|_2^{2} dt\bigg)^{\frac{p_0-1}{p_0}}.$$
Since $Q'$ is not mildly bad, we get:
$$2^{nj(\frac{1}{2}-\frac{2\gamma}{3})} \int_{t_n}^T u_{Q'}\bigg( \sum_{k>j+1000/\epsilon}2^{k(\gamma n+1)}\|\phi_{Q'_j,j}P_k u\|_2^2\bigg)dt \leq$$
...
\begin{equation} \label{1bad4} 2^{nj(\frac{1}{2}-\frac{2\gamma}{3})} \int_{t_n}^T u_{Q'}\bigg( \sum_{k>j+1000/\epsilon}2^{k(\gamma n+1)}\|\phi_{Q'_j,j}P_k u\|_2^2\bigg)dt \leq 2^{-140j}.\end{equation}
...
$$2^{-100j}+K\int_{t_n}^T 2^{2\alpha j}u_{\mathcal{N}^l(Q)}^2 dt \leq \int_{t_n}^T 2^{2\alpha j}u_{\mathcal{N}^l(Q)}^2 dt  + K2^{-140j}$$
...
\begin{equation}\label{est1}\|\phi_{Q_k,k}P_k u\|_\infty \leq K 2^{nk/2}u_{Q_k} + K2^{-250k}\end{equation}
...
$$\abs{\langle-\phi_{Q,j}P_j \hat{B}(P_k u,\tilde{P}_j u), \phi_{Q,j}P_j u \rangle } \leq $$
$$\leq2^j u_Q \|P_k u\|_\infty \|\phi_{Q,j}\tilde{P}_ju\|_q+K2^{-150j}\leq K2^{j+n\delta j/2+\frac{n\gamma j}{3}}u_Q u_{\mathcal{N}^1(Q)}+K2^{-150j},$$
...
Henceforth, we can always have $0<\gamma<1/2$. Assuming this, we can pick $p_1,p_2,r$ so that
...
In this section we deal only with functions and vector fields defined on $\mathbb{R}^3$. For this reason, the parameter $n$ does not denote the dimension of space but usually and index of summation.
...
We determine the critical exponent of hyperdissipation that guarantees global regularity for solutions of Equation (\ref{FRACNS}). A proof along the same lines for the three-dimensional Navier-Stokes equations may be found in \cite{KP}.

\begin{proposition}
Let $\alpha>(n+2)/4$ and let $u(t):\mathbb{R}^n\times [0,\epsilon)\to \mathbb{R}^n$ be a smooth solution of Equation (\ref{FRACNS}) for some small $\epsilon>0$. Then, $u(t)$ is smooth for all times $t\geq 0$. 
\end{proposition}
\begin{proof}
...
\begin{equation}\label{UH1} \frac{d}{dt}(\|u\|_{\beta}^2)+\|u\|_{\alpha+\beta}^2 +\langle (-\Delta)^{\frac{\beta}{2}}\textrm{div}(u\otimes u), (-\Delta)^{\frac{\beta}{2}} u\rangle =0.\end{equation}
...
$$\frac{d}{dt}(\|u\|_{\beta}^2) \leq \frac{1}{2}\|u\|_\alpha^2\|u\|_\beta^2$$
It easily follows that
$$\|u(t)\|_\beta^2 \leq \|u(0)\|_\beta^2 \exp\big(\frac{1}{2}\int_0^t \|u(s)\|_\alpha^2ds\big).$$
However, by the Energy Dissipation Law, we know
$$\int_0^t \|u(s)\|_\alpha^2 ds\leq \|u(0)\|_{L^2}$$
for all times $t$ whenever $u(t)$ is smooth. Since $\beta$ was arbitrary and $u(t)$ was smooth for small times $t\in [0,\epsilon)$, we can apply a standard bootstrap argument and get regularity for all times $t\geq 0$.
\end{proof}
...
The positive function $\epsilon(p_0)\to 0$ as $p_0\to \infty$. We then use this expression to simplify the Inequality in (\ref{ALG2}) as:
