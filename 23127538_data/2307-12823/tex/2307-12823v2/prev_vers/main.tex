...
\begin{abstract}
	...
	We benchmark our approach for a number of quantum tomography protocols using both simulation and demonstration withe the use of a cloud-accessible quantum processor.
...
This problem becomes even more significant for quantum process tomography (QPT)~\cite{Poyatos1997,Chuang1997,DAriano2001,Kiktenko20202,Kiktenko2021}, which is an important tool for characterizing the quality of operations performed by quantum devices~\cite{White2004,Wineland2008}.  
...
\BLUE{TODO: note IC requirement for $A$.}
...
Indeed,
\begin{multline*}
	\|A^+\Delta\boldsymbol{ p}\|_2^2 = \sum_i\left(\sum_j A^+_{ij}\Delta p_j\right)^2 \\ = \sum_i \left(\sum_j\sum_k A^+_{ij}A^+_{ik}\Delta p_j\Delta p_k\right) \\ 
...
For this purpose, we process the data obtained from the superconducting quantum processor ${\sf ibmqx2}$ (${\sf IBMQ~5~Yorktown}$) provided by IBM (for more details see~\cite{IBMQ5}).
...
Then we repeat this procedure $D=10^4$ times. 
In the ideal case, if the confidence interval with a confidence level ${\rm CL}$ is valid and the most tight possible, the fraction of measurements $f_{\rm inside}$, in which the reconstructed state (or process) lies inside the interval should be exactly equal to ${\rm CL}$. 
...
including coherent-state quantum tomography~\cite{Lvovsky2008,Lvovsky2011,Anis2012,Fedorov2015, ghalaii2017scheme}.
