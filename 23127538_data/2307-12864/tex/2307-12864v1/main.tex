...
\def\mycopyrightnotice{%
	\begin{minipage}{\textwidth}
		\scriptsize
		\copyright 2023 IEEE.  Personal use of this material is permitted. Permission from
...
\begin{itemize}
	...
	\item An evaluation in trained inter coders to show the effect of bottlenecks in a conditional coder and the superior performance of the conditional residual coder
\end{itemize}
...
In the second part consisting of Sections VII and VIII, we analyze implementations of conditional and conditional residual coders. The coders were designed such that the strength of the bottleneck can be steered. We present a simple method to measure and visualize bottlenecks and demonstrate that the conditional residual coder---different than the conditional coder---does not exhibit bottleneck effects anymore. The paper closes with a conclusion and an outlook in Section IX.
...
\rate_1 \geq H(X_1|X_0)
\end{equation}
Curiously, in this coding scheme, it does not matter if the encoder also has access to the additional information, as long as the decoder does. 
...
\end{equation}
where $\resD$ is the source emitting $\resV=\curV-\predV$. 
...
\end{equation}
The inequality follows again from the data processing theorem.

Note that the severity of bottlenecks highly depends on the implementation details of the coder. Conditional coders in literature feature concepts such as additionally transmitted skip modes~\cite{LadunePH2020_ModeNetModeSelection}, additional enhancement layers~\cite{LiLD2020_SpatialChannelContext}, or adaptive switching between conditional and residual coders~\cite{BrandSK2022_PFrameCoding}. We will show later that effects of the bottleneck can be reduced by strongly increasing the capacity and thereby the complexity of the networks in the conditional path. 

\section{Influence of Bottlenecks on the Conditional Coder}
It is well-known that conditional coders can achieve superior performance in compression efficiency over a residual coder. In~\cite{BrandSK2022_BenefitsChallengesConditional} we showed that the mutual information $I(\predD;\resD)$ quantifies the rate savings of a conditional coder:
\begin{equation}
H(\resD) = H(\curD|\predD) + I(\predD;\resD).
\label{Eq:Main}
\end{equation}

In the following, we compute the effects of bottlenecks in the prediction path. Since we model the bottleneck with a general function \mbox{$\predBNV = f\left(\predV\right)$}, where $\predBNV$ is a degraded version of $\predV$, it is clear from the data processing theorem that
\begin{equation}
...
\label{Eq:Main2}
\end{equation}
Since $I(\curD;\predD|\predBND)$ is non-negative, $H(\resD)\geq H(\curD|\predBND)$ does not necessarily hold. Depending on how much information is lost during the bottleneck $f$, the rate might therefore increase for a conditional coder. When considering bottlenecks, conditional coding might hence no longer be better than residual coding due to losses imposed by the network structure.
...
In the previous section, we took a close look at conditional coding and showed that $H(\resD) \geq H(\curD|\predD)$ hold in general. However, we also showed that when we take the information bottlenecks between the prediction and the reconstructed signal into account, the overall performance may be below the residual coder. In this section, we propose the conditional residual coding paradigm for inter frames, which compresses the residual $\resV$ under the condition of knowing $\predV$. The basic structure is shown in Fig.~\ref{Fig:CondResSchematic}(a). Comparing the coder with the conditional coder in Fig.~\ref{Fig:CondVsResConcept}(b), we see that only a residual structure was added to the coder. This also means that this extension is practically for free since only one addition and one subtraction are required per pixel. We furthermore see that the conditional residual coder includes a bottleneck-free path from $\predV$ to $\recV$, which already suggests larger robustness. In lossless coding, the required rate is given by $H(\resD|\predD)$ or $H(\resD|\predBND)$, without and with considering bottlenecks, respectively.
...
\begin{enumerate}
	...
	\item When taking bottlenecks into account, the conditional residual coder is never worse than the residual coder as long as $H(\resD)<H(\curD)$ holds.
\end{enumerate}
...
\subsection{Ideal Conditional Residual Coder}
At first, we want to show that without considering bottlenecks, transmitting $\curV$ is equivalent to transmitting $\resV$, or that 
\begin{equation}
...
\begin{subequations}
	\begin{align}
	H(\resD|\predD) &= H(\resD|\predD) + \underbrace{H(\resD|\curD,\predD)}_{~~~~~~=0~(\ref{Eq:rentropy})}\\
	&=H(\curD,\resD|\predD)\\
	&=H(\curD|\predD) + \underbrace{H(\curD|\resD,\predD)}_{~~~~~~=0~(\ref{Eq:xentropy})}\\
	&= H(\curD|\predD),
	\end{align}
\end{subequations}
...
\subsection{Considering Bottlenecks}
...
In the second case (\ref{Eq:Case2}) with a tighter bottleneck, the bottleneck first causes $I(\curD;\predD) = \BN$. Thereby we obtain
\begin{subequations}
	\begin{align}
	\begin{split}
...
\end{subequations}
That the last expression is greater or equal to zero follows directly from the definition of the second case.

In the third case (\ref{Eq:Case1}), $I(\curD;\predD) = I(\resD;\predD) = \BN$ holds and thereby
\begin{subequations}
	\begin{align}
	\begin{split}
	...
	\end{align}
\end{subequations}
where the final inequality follows from (\ref{Eq:BFull}) and (\ref{Eq:Assumption1}).

We see that however tight we choose the bottleneck, conditional residual coding always performs at least as good as conditional coding, as long as the basic condition \mbox{$H(\resD)<H(\curD)$} is fulfilled. That way, we proved the third statement and have shown that the conditional residual coder always performs better or equal to both the conditional and the residual coder, thereby combining the advantages of both.

%% Figure environment removed
\section{Model-Based Validation}
...
Since there is no accurate probability model for an entire image, it is not possible to compute the entropy of an image or the conditional entropy of one image given another. Furthermore, there are no established models connecting the prediction frame $\predV$ and the current frame $\curV$. Since we are more interested in temporal than in spatial redundancy here, we limit our model to the transmission of one pixel $\cur$ under temporal prediction with the predictor $\pred$.

To model temporal redundancy, we consider an ideal motion compensation based predictor with the following properties: If there is a match in the reference frame, the predictor will find this match. If there is no match, as is the case e.g. for occlusion, then we assume that the prediction will assume a random and uncorrelated value. We denote the probability for an occlusion with $\probMVE$. We can therefore write the probability for $\pred$ given $\cur$ as:
\begin{equation}
...
\end{equation} 
with the quantization step size $\QuantStep$, which constitutes the second model parameter.
...
We used an MSE rate distortion function $\distMSE(\cdot;\cdot)$ and a rate estimation function $\rate(\cdot)$ to train the different coders on a joint rate-distortion loss:
\begin{equation}
...
For the conditional residual coder, as shown in Fig.~\ref{Fig:ResCodecNetResults}, the picture looks completely different. Even when we test on the entire set, all coders perform better than the residual coder. We still see an increasing performance with wider bottlenecks, but the difference is much less severe than for the conditional coder. Here we have to also keep in mind that the number of parameters increases with $\predChannels$, which just by itself can also lead to an improved performance. 
...
The concepts and results from this work can be used in future work to optimize video coding systems. Since the theoretical findings are unequivocally stating that conditional residual coding is the optimal concept for inter coding, gains can also be expected in the latest compression methods. The knowledge of bottleneck effects and theoretically sound mitigation strategies can contribute to more systematic and targeted improvements of conditional inter coding.
