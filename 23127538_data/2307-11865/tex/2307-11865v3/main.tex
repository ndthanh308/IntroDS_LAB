...
%
\long\def\invis#1{}  % Used to hide a block of text
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% \newcommand\gderror[1]{
%   \typeout{--------------------------------------------------------------------}
%   \typeout{------- #1 ---------}
...
% \ifnum\value{page}>#2
%     \gderror{\Large \bf On page {\thepage} we are past page #2 (too long).   #1 }
% \else\fi
% }
% %
% % Check for the appropriate number of supplementary pages (maxpageSinceLast)
% %
...
\long\def\greg#1{{\bf \color{red} #1 --gd}}  
\long\def\fhogan#1{{\bf \color{blue} #1}}
\long\def\drivkin#1{{\bf \color{purple} #1}}
\long\def\nikhil#1{{\bf \color{orange} #1}}
\long\def\bhb#1{{\bf \color{teal} #1}}

% DON'T DELETE ABOVE, BUT REPLACE WITH EMPTY MACROS BELOW
\long\def\greg#1{}  
\long\def\fhogan#1{}
\long\def\drivkin#1{}
\long\def\nikhil#1{}
...
%An example of such a query could be a 
%
%We speculate, based on observations of existing conversational agents, that many tasks expected of domestic robots will  
%fit the pattern of "go somewhere, then do something." 
...
%\greg{I'm looking for a nice example from literature, but it's hard to %find.  We cannot use this, I think, but here is a description of bringing %bottles to the store:  {\em ``She had said I’m tired of begging God to %overthrow my son, because all this business of living in the presidential %palace is like having the lights on all the time, sir, and she had said %it with the same naturalness with which on one national holiday she had %made her way through the guard of honor with a basket of empty bottles %and reached the presidential limousine that was leading the parade of %celebration in an uproar of ovations and martial music and storms of %flowers and she shoved the basket through the window and shouted to her %son that since you’ll be passing right by take advantage and return these %bottles to the store on the corner, poor mother.''} --GABRIEL GARCIA %MARQUEZ, “AUTUMN OF THE PATRIARCH.”}
...
NLMap \cite{chen2023open} augments Say-Can \cite{brohan2023can} with mapping and navigation capabilities, using an open-vocabulary CLIP-based map similar to VLMaps. An LLM is used to make object proposals based on the user query, then the map is searched for the presence of these proposed objects. If any of the objects are located in the map, they are used in the construction of a planning prompt. FindThis~\cite{findthis} focuses on interactive dialogues which support localizing the specific instances of an object (e.g. ``the cup by the sink"). Similarly to NLMap, objects must first be proposed based on the query before they can be found in the environment. CARTIER differs from both in that it uses an object detector to compile a list of all objects in the scene, thereby providing more information to the planner.
...
% I really think we should cite Ansel, so I removed FH's
% question.

% \subsection{Navigation tasks}
% \drivkin{TODO, or maybe just drop this section altogether}
% A large number of language-driven navigation tasks has been proposed. To create a language-driven navigation task, one must begin with a 3D environment model. Two popular sources of these include AI2Thor \cite{ai2thor}, a simulated environment with the possibility of object interaction, and Matterport3D \cite{Matterport3D}, a large collection of 3D scans of real homes. Both of these come with ground-truth 3D semantic segmentations, at least for some subset of the scenes. In this work we use AI2Thor only, as we were unable to obtain a licence for Matterport3D. 
% Once a set of 3D environments is obtained, tasks can be defined on top of them. These tasks are differentiated primarily by the tasks descriptions and the rules of interacting with the environment. AI2Thor-derived navigation tasks include 

% The text description in these is quite simple compared to ours.
% \begin{itemize}
%     \item ALFRED and some of the million derived tasks
%     \item Matterport3D and habitat
%     \item SQA3D. \cite{sqa3d}
% \end{itemize}
...
\section{Method}
In this Section, we introduce CARTIER, a framework that can interpret complex user language instructions to generate navigation targets. CARTIER is composed of five parts: 1) Environment exploration, 2) Object detection and spatial language index construction, 3) LLM query construction, 4) Querying the LLM,  5) Querying the spatial language index. These are summarized graphically in Figure \ref{fig:pipeline}. The rest of the this section explores each of these parts in more detail.
...
% \begin{scriptsize}
% \begin{verbatim}
% Given a user request, your job is to guess which object in
% a room a robot should navigate to in order to begin 
% executing the users request. The objects which you may 
% choose from will be provided. Answer simply with the 
...
\drivkin{Frank and Greg: does this sound good?}


% presented with two examples of correct questions/answer pairs in the prompt. The Backus–Naur form  (BNF)~\cite{bnf} completely describing the construction of the LLM query is given in Figure \ref{fig:bnf}.
% % Figure environment removed
...
% \drivkin{We either use either OFA captioning with noun extraction or a dedicated object detector, EVA-lseg. We have already implemented both of these, so they should be pretty easy to do. I actually think it might be interesting to see how the choice of object extractor affects things, so we could do both, and it could beef up our results section a bit.}


% \subsection{LLM Query}
% We query the LLM by providing a couple of examples of the behavior we want. \drivkin{TODO: put a figure of an example query, and maybe also one of those grammars we used in ANSEL. Maybe it's time to cite ANSEL here too.}
% \drivkin{It occurs to me that we could probably add personalization to the queries, or meta info, like what time it is. That could be a really interesting extension that we may need to leave for future work, but also might not be that hard to squeeze in here.}

% \subsection{Grounding and mapping.}
% We explore two approaches to grounding. One is to use the approach in VLMaps, creating a 2D floorplan with per-pixel CLIP embeddings. See VLMaps paper for implementation details. \drivkin{Discuss any extra clustering we had to do here.}

% A simpler approach is to just keep track of what frames we saw all of the objects in, as well as the size of their bounding boxes \drivkin{assuming we used an object detector that gave bounding boxes}. We then simply pick the frame that had the largest bounding box for the selected object. This heurisitic is chosen based on the intuition that if an object takes up a large part of the frame, the robot was likely close to the object when the frame was collected. \drivkin{If we don't use an object detector with bounding boxes, come up with a different heuristic}
\section{Evaluation}
...
gpt4 & implicit & \textbf{1.17} & 1.89 & 1.66  & 2.09 & 2.27 \\ \hline
...
Figure \ref{fig:object_matching} also confirms the superiority of CARTIER on the challenging conversational queries. CARTIER's performance using GPT4 on all three query types is comparable, suggesting that the limiting factor is the object detector, rather than the reasoning capabilities of the LLM. NLMap beats CARTIER on explicit queries in terms of the object-matching metric, but loses on euclidean-distance. This suggests that CARTIER is able to exploit object co-location (as described in Sections \ref{sec:query_the_llm}  \ref{sec:metrics}) and/or more robustly localize the objects it does find. For an illustration of this effect, consider ``bed-sheets'' example from Section ~\ref{sec:metrics}. In many scenes, bed and sheets are likely to indeed be co-located, so while the manual object-matching score may consider the answer ``sheets" to be wrong answer, the distance-based metric is likely to make no distinction between the two.
...
CARTIER's approach of providing the LLM decision maker with a detailed description of the scene contents represents an alternative approach to those described in work such as NLMap, where the LLM must first hypothesise which objects are useful given the input query. Our results show that this approach provides more relative benefit as the complexity of the queries increases. One potential downside is that the object detector may ignore certain object types, or fail to distinguish important object attributes, such as whether a plate is dirty. Thus, an interesting avenue for future work may be to synthesise the two approaches by both listing known objects in the prompt and allowing the LLM to ask a VLM targeted questions about the objects. By allowing users to specify ``go-do tasks'' in more natural ways, we hope that the utility and impact of robot navigation agents will be greatly enhanced.



\maxpage[Six page limit before bibliography. ``The page limit is 6 pages for the paper (text, figures, tables, acknowledgement, etc.) + any number of pages for the bibliography/references. Papers exceeding the (6+n) page limit at the time of submission will be returned without review.'']{6}
