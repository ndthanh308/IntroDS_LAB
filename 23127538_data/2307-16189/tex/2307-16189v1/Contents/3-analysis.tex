...
In the field of deep learning, matrices play a significant role in representing and computing large amounts of data and their interconnections. In the context of forward propagation, we introduce the concept of input matrix $\textbf{X}$, where each column delineates a separate training example \cite{Goodfellow2016}. This matrix contains $m$ instances of training examples, each with $n$ features, illustrated as:
\begin{equation*}
...
\end{bmatrix}
\end{equation*}
For the model's weights, we utilize a weight matrix $\textbf{W}$, where each column signifies the weights related to the connections between all input neurons and a specific hidden neuron. Assuming $k$ neurons are present in the hidden layer, the weight matrix takes the form:
\begin{equation*}
...
\end{bmatrix}
\end{equation*}
In addition, a bias vector $\textbf{b}$, a $k \times 1$ vector, is integrated:
\begin{equation*}
...
\end{bmatrix}
\end{equation*}
During the forward propagation, the pre-activation matrix $\textbf{Z}$ is determined through a matrix multiplication operation between the transpose of the weight matrix $\textbf{W}^T$ and the input matrix, with the bias term being added \cite{Goodfellow2016}:
\begin{equation*}
\textbf{Z} = \textbf{W}^T \textbf{X} + \textbf{b}
\end{equation*}
This matrix $\textbf{Z}$ encompasses each weighted sum of the inputs corresponding to the $i$-th training example and the $j$-th hidden neuron, with the bias of the $j$-th hidden neuron integrated. Following this computation, the matrix $\textbf{Z}$ is passed through an activation function $\sigma(\cdot)$, enabling the calculation of the output matrix $\textbf{A}$, which is applied element-wise:
\begin{equation*}
\textbf{A} = \sigma(\textbf{Z})
\end{equation*}
Here, every element $a_{ij}$ within $\textbf{A}$ refers to the activated output of the $j$-th hidden neuron for the $i$-th training example.
