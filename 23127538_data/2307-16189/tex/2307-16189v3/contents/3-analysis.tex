...
\end{bmatrix}
\end{equation*}
During forward propagation, the raw outputs or pre-activations (denoted as $Z$) are computed by multiplying the transposed weight matrix with the input matrix and adding the bias. This step essentially aggregates information from the input, weighed by the network's current understanding (weights and biases).
\begin{equation*}
Z = W^T X + b
\end{equation*}
...
In DNN(Fully Connected Network), each operation in the forward pass involves straightforward mathematical operations like addition and multiplication. There is no division or complex operation that could amplify small numerical errors or lead to potential instability. However, as we progress deeper into networks, especially very deep DNNs, the repeated multiplication during forward propagation can sometimes cause values to vanish (become very small) or explode (become very large). 
...
In the convolution operation of a Convolutional Neural Network (CNN), the input matrix, typically representing an image or another feature map, is processed using a filter or kernel matrix. This operation is used to extract high-level features, such as edges, corners, and textures, from the input. The convolution layer can be detailed by observing the element-wise multiplication and summation \cite{lecun1998gradient}. Specifically, in our 3x3 input matrix \(I\) and 2x2 filter \(F\) example:
\begin{align*}
I &= \begin{bmatrix}
i_{11} & i_{12} & i_{13} \\
i_{21} & i_{22} & i_{23} \\
...
Especially those with a significant number of layers, the iterative multiplications during forward propagation can intermittently lead to either vanishingly small values or notably large increments in the activations. Methods such as batch normalization\cite{batch} have been introduced to regulate and prevent these activations from attaining extreme values, thereby enhancing numerical stability. Numerical instability remains a prevalent challenge during the training of deep learning models, most notably during backpropagation. This vulnerability arises from the complex calculations of gradients of the loss function relative to the model parameters, which can occasionally yield exceptionally large or minuscule numerical values, thereby inducing instability \cite{Goodfellow2016}. In contrast, forward propagation, which involves the transmission of input data through the network to produce an output, is generally more resilient to such instabilities. This comparative robustness can be ascribed to a variety of underlying reasons:
\\
\begin{itemize}
