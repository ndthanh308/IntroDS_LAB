...
% \subsection{Numerical Guarantee Method for Adam Optimizer}

% In addressing the limitations of conventional optimizers when applied to 16-bit neural network training, we introduce the Numerical Guarantee Method (NGM). This method modifies the standard Adam optimizer to reinforce numerical stability, ensuring that the training process is not compromised by the precision limitations inherent in 16-bit floating-point calculations.

% \subsubsection{Preliminaries}
% Let the parameter space be denoted as \( \mathbb{R}^d \), where each parameter vector \( \mathbf{w} \) is associated with two state variables similar to the original Adam optimizer:
% \begin{align*}
%     m(\mathbf{w}) &: \text{Moving average of the gradients with respect to } \mathbf{w}. \\
%     v(\mathbf{w}) &: \text{Moving average of the squared gradients with respect to } \mathbf{w}.
% \end{align*}
% The NGM introduces an adjusted epsilon \( \epsilon' \), larger than the original \( \epsilon \) used in the Adam optimizer, to prevent underflow issues in 16-bit precision environments.

% \subsubsection{State Updates}
% The NGM maintains and updates the moving averages of the gradients and their squares with the following adjustments to accommodate the 16-bit environment:

% \paragraph{Moving Average of Gradients: \( m \)}
% The update rule for the moving average of gradients is adjusted to use \( \epsilon' \) in the bias correction term to enhance stability:
% \begin{equation}
% m_t = \beta_1 \cdot m_{t-1} + (1 - \beta_1) \cdot g_t,
% \end{equation}
% where \( g_t \) is the gradient at time step \( t \), and \( \beta_1 \) is the exponential decay rate for the moving average of the gradients.

% \paragraph{Moving Average of Squared Gradients: \( v \)}
% For the moving average of squared gradients, the update is similarly adjusted:
% \begin{equation}
% v_t = \beta_2 \cdot v_{t-1} + (1 - \beta_2) \cdot g_t^2,
% \end{equation}
% where \( g_t^2 \) is the element-wise square of the gradient and \( \beta_2 \) is the decay rate for the moving average of the squared gradients.

% \subsubsection{Bias Correction}
% Bias correction in NGM is applied as follows to mitigate initialization bias, accounting for the modified \( \epsilon' \):

% \paragraph{Correcting the Bias in \( m \)}
% The bias-corrected estimate for the first moment vector is computed with:
% \begin{equation}
% \hat{m_t} = \frac{m_t}{1 - \beta_1^t},
% \end{equation}
% where \( \beta_1^t \) is \( \beta_1 \) to the power of the time step \( t \).

% \paragraph{Correcting the Bias in \( v \)}
% Similarly, the bias-corrected estimate for the second moment vector uses the modified epsilon:
% \begin{equation}
% \hat{v_t} = \frac{v_t}{1 - \beta_2^t},
% \end{equation}
% This bias correction is crucial for the NGM to function correctly within the 16-bit precision constraints.

% \subsubsection{Parameter Update Rule}
% Incorporating the numerical guarantee into the parameter update rule, we arrive at:
% \begin{equation}
% \mathbf{w}_{t+1} = \mathbf{w}_t - \alpha \cdot \frac{\hat{m_t}}{\sqrt{\hat{v_t}} + \epsilon'},
% \end{equation}
% where \( \alpha \) is the learning rate. This rule ensures that updates are kept within the numerical limits of 16-bit precision, avoiding overflow or underflow issues that could derail the training process.

% By integrating these modifications into the Adam optimizer, the NGM provides a numerically stable approach to training neural networks, particularly in 16-bit environments, while maintaining the adaptive benefits of the original optimizer.
