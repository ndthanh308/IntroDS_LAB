In the field of deep learning, matrices play a significant role in representing and computing large amounts of data and their interconnections. In the context of forward propagation, we introduce the concept of input matrix $X$, where each column delineates a separate training example \cite{Goodfellow2016}. This matrix contains $m$ instances of training examples, each with $n$ features, illustrated as:
\begin{equation*}
X = \begin{bmatrix}
...
\end{bmatrix}
\end{equation*}
For the model's weights, we utilize a weight matrix $W$, where each column signifies the weights related to the connections between all input neurons and a specific hidden neuron. Assuming $k$ neurons are present in the hidden layer, the weight matrix takes the form:
\begin{equation*}
W = \begin{bmatrix}
...
\end{bmatrix}
\end{equation*}
In addition, a bias vector $b$, a $k \times 1$ vector, is integrated:
\begin{equation*}
b = \begin{bmatrix}
b_{1} \\
b_{2} \\
...
\end{bmatrix}
\end{equation*}
During the forward propagation, the pre-activation matrix $Z$ is determined through a matrix multiplication operation between the transpose of the weight matrix $W^T$ and the input matrix, with the bias term being added \cite{Goodfellow2016}:
\begin{equation*}
Z = W^T X + b
\end{equation*}
This matrix $Z$ encompasses each weighted sum of the inputs corresponding to the $i$-th training example and the $j$-th hidden neuron, with the bias of the $j$-th hidden neuron integrated. Following this computation, the matrix $Z$ is passed through an activation function $\sigma(\cdot)$, enabling the calculation of the output matrix $A$, which is applied element-wise:
\begin{equation*}
A = \sigma(Z)
\end{equation*}
Here, every element $a_{ij}$ within $A$ refers to the activated output of the $j$-th hidden neuron for the $i$-th training example.

The issue of numerical instability is a common impediment encountered during the training phase of deep learning models, with the backpropagation phase being particularly susceptible due to the intricate computation of gradients of the loss function concerning model parameters, potentially leading to exceedingly large or small numerical values and subsequent instability \cite{Goodfellow2016}. However, forward propagation, comprising the passing of input data through the network to obtain output, generally demonstrates less susceptibility to this instability. This relative stability is attributed to several factors:
\\
\begin{itemize}
...
\textbf{Original Role of Epsilon}: To understand the role of $\epsilon$ more clearly, consider the function $f(x) = \frac{1}{x}$. As $x$ approaches 0 from the right, $f(x)$ approaches infinity (a vertical asymptote at $x=0$. By adding $\epsilon$ to x (i.e.,  $f(x) = \frac{1}{x + \epsilon}$), the function becomes bounded as $x$ approaches 0, because 
$f(0) = \frac{1}{\epsilon}$, which is a finite number. This change ensures that no division by zero occurs and that the function remains bounded for all $x$ in its domain. In optimization algorithms, this modification ensures that the parameter updates remain within a reasonable range, preventing large oscillations or divergence which can be caused by large gradient updates. This way, $\epsilon$ helps in maintaining numerical stability during the training process. However, when $\epsilon$ with this purpose encounters a special situation called low precision, it rather creates a numerical instability. In such situations, the inclusion of $\epsilon$ can lead to gradient values being dominated by this constant, leading to skewed updates and thus, unstable training dynamics.

This issue is not exclusive to the Adam optimizer but indeed pervades most optimizers that base their operations on it. While one could ostensibly resolve this problem by simply modifying the value of Epsilon, pinpointing the ideal Epsilon value presents a significant challenge. Furthermore, the appropriateness of a given Epsilon value can be uncertain and highly dependent on the specific context. We have devised a strategy to effectively address this issue, and we intend to elaborate on this in the ensuing section. Our proposed solution provides a methodical approach to Epsilon adjustment, enabling more stable and reliable optimization even in the face of numerical instability challenges.