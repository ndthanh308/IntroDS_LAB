...
\author{\IEEEauthorblockN{1\textsuperscript{st} Kaixin Zhang}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
21B903037@stu.hit.edu.cn}
\and
...
wangzh@hit.edu.cn}
\and
\IEEEauthorblockN{3\textsuperscript{rd} Yabin Lu}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
email address or ORCID}
\and
\IEEEauthorblockN{4\textsuperscript{th} Ziqi Li}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
email address or ORCID}
\and
\IEEEauthorblockN{5\textsuperscript{th} Chang Shu}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
email address or ORCID}
\and
\IEEEauthorblockN{6\textsuperscript{th} Yu Yan}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
email address or ORCID}
\and
\IEEEauthorblockN{6\textsuperscript{th} Donghua Yang}
\IEEEauthorblockA{\textit{Massive Data Computing Lab} \\
\textit{Harbin Institute of Technology}\\
Harbin, China \\
email address or ORCID}
...
% \renewcommand{\itemautorefname}{Problem}
...
%Since Duet is based on a data-driven method and can support hybrid training, we first introduce the most representative data-driven and hybrid cardinality estimation method.

% In summary, the query-driven, data-driven, and hybrid cardinality estimation methods have at least one of the problems as follows:\footnote{seems repeated to above, suggest to combine them}
% 这里是想总结一下上面三类方法存在的不同问题，不然看起来太散了
...
%and problem (5) is a common occurrence in machine learning. We believe that it is especially significant for high-dimensional tables with large amounts of columns since Naru and UAE's sampling error accumulates as columns increase. Also, problem (5) is also significant for high-cardinality tables with large amounts of tuples since they have more complex joint distribution to learn.

%%逻辑最好是现在方法问题的总结，根源的分析，从分析的根源引出我们的方法
%那我把这个总结放到上面吧
%and can be alleviated by fine-tuning specifically for long-tailed distributions, which is blocked by the sampling process in data-driven and hybrid methods.

%%此处增加改为一句话，概括技术特点，直接就能看出来可以解决已有工作的问题，和摘要一样
%done
Thus, in this paper, we propose a novel methodology that introduces the predicates information into the autoregressive model by learning from virtual tables and a hybrid approach (\textbf{Duet}) for efficient and scalable cardinality estimation to simultaneously address the five problems by removing the sampling process for autoregressive-based data-driven and hybrid approaches. Specifically, our approach enables accurate and deterministic cardinality estimation for various range query workloads, high cardinality, and high dimensional tables, with much lower time and memory costs since no sampling is needed. Furthermore, since Duet's estimation process is differentiable and considering the temporal locality of queries, users can further improve the long-tailed distribution problem by applying hybrid training on historical queries.

Firstly, to adapt to various query workloads, Duet is a hybrid method modified and based on Naru, which is a data-driven method. Therefore, Duet can adapt to any query workload distribution, rather than being limited to a fixed query workload distribution. Moreover, we addressed the remaining three issues by modifying the problem modeling and removing the sampling process.

Secondly, to reduce the inference cost, we introduce predicates information into the autoregressive model that estimates $P(C_n|c_{<n-1})$, and learn from a virtual table that contains range predicate information by adding predicate operators such as $=, \neq, >, <, \geq, \leq$ into the model's input. This process is not a just simple change of the model's input but completely changes the problem modeling of the autoregressive learning process. %We explain this roughly in the next paragraph and more detailed information can be found in section \autoref{}.
...
Thirdly, by removing the sampling processing, the sampling error accumulation is removed accordingly. Thus, the long-tailed distribution problem caused by the high-dimensional table is alleviated. For the long-tailed distribution problem caused by the high-cardinality table, since range queries' cardinality can be predicted without any non-differentiable process, we introduce the QError as a feedback signal to the loss function to achieve hybrid training. Considering the temporal locality of real-world workloads, users can use historical queries for hybrid training to significantly improve model accuracy.
...
Finally, as for stability, Duet is stable since no randomness exists during estimation. For a given query, Duet will provide a deterministic estimation result that could help the query optimizer work better and help the DBA to find problems easier.

Our contribution can be summarized as follows.
\begin{itemize}
    \item We proposed a novel methodology for modeling data-driven cardinality estimation which can deal with range queries without sampling and any non-differentiable process during inference. Taking a neural network inference as a basic operation, Duet can achieve O(1) time complexity during inference with only a single network forward pass for each estimation. This feature makes Duet can even be inferred on the CPU with less time cost than that of most learned methods on GPU.
    
    \item Duet is a differentiable cardinality estimation method and naturally supports hybrid training. By combining data-driven and query-driven training, Duet can improve the model's accuracy on high-cardinality and high-dimensional tables significantly.
    \item Benefiting from the sampling-free estimation design, Duet is a stable and practical cardinality estimation method that always gives a determinate estimation result. And compared with other hybrid methods, Duet saves a massive amount of GPU memory and calculation during training.
    \item The experimental results demonstrate that Duet can achieve lower estimation error, less time cost of inference, and lower memory cost compared to previous works.
\end{itemize}
...
\textbf{Query.}
A query is a conjunction of several predicates, and each predicate contains three parts: column $C_i$, predicate operator (a function) $pred_i$, and predicate values $v_i$. Let $c \in \{C_1, C_2, ..., C_n\}$, $pred_i \in \{=, \neq, >, <, \geq, \leq\}$. 
...
As the definition above shows, Duet can support both equalization predicate and range predicates including $>, <, >=, <=$. Since Duet shares the framework of Naru, it also supports joins just like NeuroCard~\cite{NeuroCard}, which is also based on Naru and does not need the sample information to deal with the fan-out value during inference. As for disjunction between predicates, its estimation can be performed by converting disjunction into conjunction.

\textbf{Cardinality Estimation.}
Given a query $q$ on table $T$ with columns $\{C_1, C_2, ..., C_n\}$, the selectivity of $q$ can be defined as $$Sel(q)=\sum_{x=(c_1,...,c_n)\in C_1 \times ... \times C_n}{I(x)p(x)}$$, where $p(x)$ is the occurrences frequency of tuple $x$, and $I(x)$ is an indicator function which returns 1 if tuple $x$ is subject to the predicate constraints of query $q$, and returns 0 otherwise. 

\section{Methodology}
The basic idea of our approach is to learn the joint distribution with a virtual table containing predicates conditions rather than the original table, which gives our estimator the ability to deal with range queries without sampling. To achieve this goal, we solve the main challenges of the expensive cost of generating or sampling from the virtual table, and the difficulty of dealing with multiple predicates for a single column. In this section, we first overview our approach and then introduce the techniques to address the challenges in the following parts.  

%how we learn from the virtual table in Section~\autoref{section: Learn From Virtual Table with Predicates}, our sampling method of generating the virtual tuples in \autoref{section: Training on Virtual Table Efficiently}, our hybrid training method in \autoref{section: Hybrid Training with Query Workloads}, and how we support multiple predicates on a single column in \autoref{section: Support Multiple Predicates on A Single Column}.
%把这些合并到overview里面去
...
\textbf{Challenge.} Current hybrid methods are based on data-driven approaches. The cornerstone of data-driven methods is Naru's autoregressive model of the joint distribution of data, which inevitably involves sampling to estimate range queries, leading to problems 2, 3, and 4. One feasible method to address problem 4 is to perform targeted fine-tuning to workloads with higher errors, but the non-differentiable sampling process hinders such training. Therefore, the fundamental challenge in solving these four issues of existing methods is how to move beyond Naru's autoregressive model of the dataset and develop a novel method to learn the conditional probability distribution that includes predicates to avoid sampling during prediction.

\textbf{Core Idea.} To avoid sampling during inference, we can directly estimate the selectivity of a query by utilizing the predicted conditional probability distribution. As shown in \autoref{fig:Overview}, instead of estimating $p(x)$ as Naru, we directly estimate the selectivity $Sel(q)$ under given predicates. Let $(pred_i, v_i)$ denote that column $i$ is subject to the constraint of predicates operator $pred_i$ and the predicates value $v_i$.\footnote{We organize the input in (v\_i, pred\_i) order in our code.} For example, $(>=, 1.23)$ represents the event that $column_i>=1.23$. With the auto-regression decomposition, we can estimate the selectivity with the following formulation:
\begin{align}
Sel(q) &= p((pred_{1}, v_{1}), ..., (pred_n, v_n))\\
&=\prod_{i=1}^{n}{p((pred_{i},v_{i})|(pred_{i-1},v_{i-1}),..., (pred_1,v_1))}\\
&=\prod_{i=1}^{n}{\sum {Pred_i(R_i, v_i)P((C_i|(pred_{i-1},v_{i-1}),..., (pred_1,v_1))}}
\end{align}
, where $Pred_i(\cdot)$ is the predicate operator in the query's predicates, it returns a vector to indicate the distinct values that obey the $i$th predicate's constraint, and $n$ is the column number. We would like to emphasize that the learning target of previous works such as Naru and UAE is $P(C_i|v_{<i})$, which does not contain the predicate operator's information like Duet and causes that they can only estimate equivalent predicate through the model and have to use the sampling method to deal with range queries.
...
%%这部分似乎过于强调Naru了，能否设计一下，以介绍我们的方法和动机为主？
% done
...
\begin{algorithm}[]
    ...
    \Fn{\FMain{tuples\_i}}{
        ...
        \ForEach{$s, e$ in $slices$}{
            ...
            \uElse{
                Calculate $lower\_bound$ and $upper\_bound$;\\
    ...
    \For{$i\in [0, ncol)$}{
        Launch thread to run $SampleColumn(tuples[...,i]);$\\
    }
...
\begin{algorithm}[]
    ...
    \LinesNumbered %要求显示行号
    \KwIn{dataset $\mathcal{D}$, training workload $\mathcal{Q}$, batch size $bs$, expand coefficient $\mu$, trade-off coefficient $\gamma$}%输入参数
    ...
    \ForEach{$batch$ in $\mathcal{D}$}{
        ...
        Perform SGD via backpropagation for $\mathcal{L}=\mathcal{L}_{data} + \gamma * \mathcal{L}_{query}$ to update $\theta$;\\
    }
    \Return $\theta$;\\
\end{algorithm}
...
% \textcolor{red}{experiment section}%%具体章节
%%done
...
The key problem is how to combine the variable-length predicates' information to fit the input size of the autoregressive network, which is a problem that Naru does not need to deal with since it uses progressive sampling and deals with multiple predicates' information at the batch dimension. Due to the limited number of predicates, we encode the predicate operator with the one-hot encoder and provide binary, one-hot, and embedding strategies for the predicate values just like Naru. To integrate such variable-length information before inputting them into the auto-regression model, we propose three MPSNs to embed the predicates into a fixed-length dummy vector space and evaluate their performance with experiments, as are introduced follows.

\textbf{MLP \& Vector Sum.} Since the number of predicates on a single column may not be too large in most queries, we use a simple MLP network to process each pair of $(pred^{j}_i,v^{j}_i)$ and embed them into vectors, then we sum these vectors up and obtain the final embedding result with a fixed length.
...
\textbf{Recursive Network.} We also evaluate the recursive network's performance for embedding. We build a recursive network whose forward pass formulae is $out = ReLU(W*(E(pred^{j}_i,v^{j}_i) || out)+b)$, where $E$ is the encoding function, and the default value of $out$ takes zero for the single predicate situation. Compared to the RNN network, the recursive network does not have the long-distance information forgetting problem. However, with the increasing number of predicates, the calculation costs increase fast. 
...
We followed previous works~\cite{Naru, AreWeReady} to generate workloads. Specifically, we sample a tuple $(x_1, ..., x_n)$ from the table and then generate predicates based on them. For example, when the sampled tuple is $(3, RED, 2011)$, we uniformly and randomly select predicates for each column and get a query $(>=3, =RED, <=2011)$. This method can generate a uniform distribution of query workload~\cite{AreWeReady} that includes both high selectivity and low selectivity.

\textbf{Training Workloads.} We use a random seed 42 to generate a workload that contains $10^5$ queries as the training workload. For the Duet model that is trained without workloads, we denote it by \textbf{Duet$_{\mathcal{D}}$}
...
\subsubsection{\textbf{Model Architectures}} We follow Naru and use a MADE with hidden units: 512, 256, 512, 128, 1,024 for DMV. For Kddcup98 and Census, we follow UAE and use a 2 layers ResMADE with hidden units 128 as our Auto-regressive models. Since the time efficiency of cardinality estimation in real-world scenarios is critical, our evaluations focus on the MADE's performance. However, it is reasonable to expect that Duet can achieve much higher speed and scalability improvement on Transformer since it has a higher cost for a single forward pass.
...
\begin{enumerate}[label={\arabic*.}]
    \item \textbf{Sampling.} This estimator uniformly samples $p\%$ tuples from all tuples into the memory and applies estimation on those samples for all queries. We use the implementation from \cite{Naru, NaruCode}.
...
\end{enumerate}
\textbf{Data-driven Models.} Data-driven models include the most advanced estimation models. They are not limited to query distribution, and most of them have smaller model sizes. We compare both Duet's procession and the cost with them.
\begin{enumerate}[label={\arabic*.}]
    \setcounter{enumi}{4}
...
\subsubsection{\textbf{Hardware}} We run our experiments on a server with 4 RTX A6000 (48GB memory), Intel(R) Xeon(R) Silver 4210R CPU, and 504GB RAM if there is no special instruction in the corresponding subsection.


%%建议注意实验顺序，可以按照开始的4个挑战的对比组织前四个实验， hyper parameter可以放到最后，加一句话
%% 按问题顺序展示结果
%% 效率放前面
...
%\textbf{Impact of expand coefficient $\mu$.} The expand coefficient $\mu$ affects Duet's accuracy and convergence speed. We explore the values \{1, 2, 4, 8\} for $\mu$ and use the max QError as the metric for accuracy and draw the convergence curve to compare their convergence speed. As shown in \autoref{}, with the increasing of $\mu$, the max QError decreases and the model converges faster. \textcolor{red}{TODO.}
...
As an information lossless method, we add the method that directly concatenates multiple predicates' encoding (denoted by \textit{CAT}) as the baseline to evaluate the performance of each method. Noted that we do not apply the \textit{CAT} method to other experiments since it can only support a fixed number of predicates for each column.

The networks of \textit{MLP} and \textit{REC} used have 2 hidden layers with 64 hidden units, activated by ReLU. The \textit{RNN} includes a network with a 2-layer LSTM followed by a Full-Connected layer(FC layer), both of which have 64 hidden units for each layer. The different predicates' outputs of the LSTM are processed by the FC layer and summed together. Each column has an independent MPSN. The results are shown in \autoref{table:Evaluation Result for Multiple Predicates Support} and \autoref{fig:MultiplePredicatesCmp}. As an information lossless method, \textit{CAT} out-perform other methods with the minimize max QError 31.789. The \textit{RNN} method has the lowest max QError due to the long-short memory mechanism. Besides, \textit{MLP} and t\textit{REC} have the same acceptable accuracy. For method \textit{MLP}, \textit{REC}, and \textit{RNN}, we observe that the \textit{MLP}'s inference time cost and training time cost is much less than others, this is expected since the other two methods involve much more computation and cannot be parallelized like \textit{MLP}. Although \textit{REC} converges a little faster (20 epochs VS 24 epochs), and \textit{RNN} has the best accuracy, we still choose \textit{MLP} as the default method to support multiple predicates situation for efficiency considerations.
...
CAT  & 11.000          & 1.767            & \textbf{18.709}      & 48        \\ \hline
MLP  & \textbf{7.750}  & \textbf{1.642}   & 28.454               & 48          \\
...
\caption{DMV}
\begin{center}
\begin{tabular}{|l|l|l|lllll|lllll|}
\hline
\multirow{2}{*}{Estimator} & \multirow{2}{*}{Size(MB)} & \multirow{2}{*}{Cost(ms)} & \multicolumn{5}{c|}{In-Workload Queries} & \multicolumn{5}{c|}{Random Queries}        \\ \cline{4-13}
                           &                           &                           & mean            & median          & 75th           & 99th           &  max           & mean           & median         & 75th          & 99th         & max      \\ \hline\hline
Sampling                   & 9.8                       & 0.27                      & 39.73           & 1.080           & 1.551          & 828.2          & 2212           & 48.81          & 1.076          & 1.548         & 1099         & 3734          \\ 
Indep                      & -                         & 260                       & 90.49           & 1.266           & 1.597          & 1775.1         & 21891          & 71.14          & 1.276          & 1.624         & 685.8        & 45437   \\
MHist                      & 17.0                      & 221.1                     & 8.905           & 2.181           & 4.142          & 102.8          & 2727.0         & 8.598          & 2.163          & 4.066         & 102.9        & 2961.6   \\ \hline\hline
MSCN                       & 6.12                      & 0.46                      & 5.745           & 1.212           & 2.722          & 77.0           & 717.0          & 6.518          & 1.278          & 3.333         & 103.0        & 650.0    \\ \hline\hline
DeepDB                     & 0.165                     & 4.35                      & 3.099           & 1.099           & 1.252          & 10.01          & 1096.8         & 2.619          & 1.100          & 1.254         & 20.06        & 414.0    \\
Naru                       & 15.5                      & 11.2                      & 1.048           & \textbf{1.013}  & 1.038          & 1.640          & 4.0            & \textbf{1.063} & \textbf{1.015} & \textbf{1.039}& \textbf{2.0} & \textbf{11.0}   \\ \hline\hline
UAE                        & 2.5                       & 7.32                      & 1.092           & 1.030           & 1.074          & 2.0            & 14.0           & 1.108          & 1.031          & 1.079         & 2.484        & \textbf{11.0}    \\ \hline\hline
Duet$_{\mathcal{D}}$       & 16.3                      & 1.55                      & 1.180           & 1.048           & 1.114          & 3.545          & 14.0           & 1.107          & 1.026          & 1.072         & 2.445        & \textbf{11.0}    \\
Duet                       & 16.3                      & 1.55                      & \textbf{1.042}  & \textbf{1.013}  & \textbf{1.032} & \textbf{1.623} & \textbf{2.312} & 1.105          & 1.018          & 1.058         & 2.670        & \textbf{11.0}    \\ \hline

\end{tabular}
\label{table:Multiple_DMV_Accuracy}
\end{center}
\end{table*}
...
\multirow{2}{*}{Estimator} & \multirow{2}{*}{Size(MB)} & \multirow{2}{*}{Cost(ms)} & \multicolumn{5}{c|}{In-Workload Queries} & \multicolumn{5}{c|}{Random Queries}        \\ \cline{4-13}
                           &                           &                           & mean            & median          & 75th           & 99th           & max            & mean           & median         & 75th           & 99th           & max      \\ \hline\hline
Sampling                   & 3.2                       & 0.05                      & 9.534           & 1.212           & 2.324          & 164.0          & 349.0          & 8.206          & 1.195          & 2.130          & 138.0          & 432.0     \\
Indep                      & -                         & 1.90                      & 1.842           & 1.168           & 1.538          & 8.657          & 193.0          & 1.647          & 1.160          & 1.528          & 8.0            & 100.7   \\
MHist                      & 45.5                      & 323.1                     & 19.93           & 15.31           & 76.18          & 151.8          & 2736.0         & 16.40          & 4.180          & 14.06          & 115.5          & 1939.0  \\ \hline\hline
MSCN                       & 7.07                      & 0.46                      & 2.120           & 1.151           & 1.797          & 14.0           & 38.0           & 2.127          & 1.175          & 1.874          & 13.0           & 73.0     \\ \hline\hline
DeepDB                     & 2.7                       & 10.1                      & 1.304           & \textbf{1.086}  & 1.285          & 4.0            & 24.0           & 1.280          & 1.093          & 1.250          & \textbf{2.048} & 16.57   \\
Naru                       & 3.4                       & 12.3                      & 1.610           & 1.222           & 1.556          & 6.443          & 48.0           & 1.428          & 1.189          & 1.463          & 2.378          & 24.16     \\ \hline\hline
UAE                        & 3.4                       & 8.41                      & 1.573           & 1.231           & 1.599          & 6.564          & 29.69          & 1.490          & 1.233          & 1.542          & 5.576          & 17.77      \\ \hline\hline
...
Duet                       & 7.3                       & 3.11                      & \textbf{1.282}  & 1.128           & \textbf{1.285} & \textbf{4.0}   & \textbf{8.0}   & \textbf{1.261} & 1.122          & 1.286          & 3.0            & \textbf{7.0}   \\ \hline

\end{tabular}
\label{table:Multiple_Kddcup98_Accuracy}
\end{center}
\end{table*}
...
\multirow{2}{*}{Estimator} & \multirow{2}{*}{Size(MB)} & \multirow{2}{*}{Cost(ms)} & \multicolumn{5}{c|}{In-Workload Queries} & \multicolumn{5}{c|}{Random Queries}        \\ \cline{4-13}
                           &                           &                           & mean            & median            & 75th           & 99th           &  max           & mean           & median         & 75th           & 99th           & max      \\ \hline\hline
Sampling                   & 0.3                       & 0.05                      & 12.55           & 1.748             & 7.085          & 163.0          & 578.0          & 12.63          & 2.0            & 8.0            & 160.1          & 445.0      \\
Indep                      & -                         & 1.09                      & 4.841           & 2.0               & 4.443          & 41.85          & 247.0          & 5.139          & 2.0            & 4.500          & 49.0           & 275.0         \\
MHist                      & 3.5                       & 15.3                      & 5.576           & 2.186             & 4.574          & 49.0           & 584.0          & 5.141          & 2.101          & 4.375          & 48.0           & 471.0     \\ \hline\hline
MSCN                       & 6.15                      & 0.46                      & 2.667           & 1.595             & 3.0            & 14.0           & 38.33          & 2.774          & 1.667          & 3.0            & 17.01          & 34.0   \\ \hline\hline
DeepDB                     & 0.4                       & 9.60                      & 1.872           & 1.043             & 2.0            & 4.319          & 46.0           & 2.018          & 1.360          & 2.0            & 11.0           & 47.0     \\
Naru                       & 0.5                       & 9.95                      & \textbf{1.273}  & \textbf{1.109}    & \textbf{1.317} & \textbf{3.0}   & \textbf{6.0}   & \textbf{1.275} & \textbf{1.117} & \textbf{1.320} & \textbf{3.0}   & \textbf{5.0} \\\hline\hline
UAE                        & 0.5                       & 6.91                      & 1.312           & 1.125             & 1.373          & 3.500          & 7.0            & 1.311          & 1.137          & 1.385          & \textbf{3.0}   & \textbf{5.0} \\\hline\hline
...
\end{tabular}
\end{center}
\label{table:Multiple_Census_Accuracy}
\end{table*}

We use the exactly same settings of Duet and baselines with \autoref{subsection: Scalability and Inference Cost Evaluation} to evaluate the estimation accuracy and compare it with the baselines. We modify the implementation of the previous works~\cite{Naru, AreWeReady, UAE} to support our workloads and run them with the best setting introduced within their documentation. All data-driven and hybrid methods learn from the same datasets, and all query-driven and hybrid methods use the same training workload introduced in \autoref{subsubsection: Query Workloads}. Also, to run an \textbf{ablation study} of the hybrid training of Duet, we also run Duet$_{\mathcal{D}}$ which is training with data-driven only. We evaluate them on both \textit{Rand-Q} and \textit{In-Q} workloads to compare their accuracy. The \autoref{table:Multiple_DMV_Accuracy}, \autoref{table:Multiple_Kddcup98_Accuracy}, and \autoref{table:Multiple_Census_Accuracy} show the results of all methods' accuracy on different workloads and settings, we can obtain a few conclusions from them. (Noted that we only compare Duet's cost with other data-driven and hybrid methods since the query-driven methods are not practical enough due to the data and workload drifting problem, and all traditional methods have too high errors.

\begin{itemize}
    \item \textbf{Duet outperforms all methods on the high cardinality table with in-workload queries.} We draw this conclusion from \autoref{table:Multiple_DMV_Accuracy}. As we have analyzed, cardinality estimation on high cardinality tables usually causes long-tailed distribution problems. Although other hybrid methods such as UAE can learn from queries to alleviate the problem, its performance cannot be improved in such a situation. The reason is that high cardinality tables require large batch size to finish the training in a limited time, but UAE trains with $batch\ size\ \times$ $\#sample$ samples, which is expensive and limits the value of $\#sample$, and eventually lead to the low accuracy of UAE. Since Duet's hybrid training is more lightweight and without progressive sampling, it achieves the best performance on in-workload queries, and it can achieve better accuracy than other methods in real scenes with temporal locality queries.
    ...
    \item \textbf{Duet outperforms all methods on the high dimensional table with both test workloads.} We draw this conclusion from \autoref{table:Multiple_Kddcup98_Accuracy}. Both Duet and Duet$_{\mathcal{D}}$ outperform all methods which show that we can obtain huge accuracy improvement by removing the progressive sampling.
    ...
    \item \textbf{Overall, Duet's accuracy is about the same as Naru and other hybrid methods represented by UAE in other experiments.} We observe from \autoref{table:Multiple_Census_Accuracy} that Naru and UAE's accuracy is slightly higher than Duet's. This is because the Census table is the smallest, so the shortcomings of their progressive sampling are not significant. Also, since Duet learns from the samples of the virtual table, it is hard for it to be trained on every possible tuple.
...
We use the same setting with \autoref{subsection: Accuracy Comparison} to train Duet and Duet$_{\mathcal{D}}$, and compare its convergence speed, throughput, and GPU memory cost with state-of-the-art methods such as Naru and UAE.

\autoref{fig:ConvergenceSpeedCompare} shows the convergence of the max QError as training progresses when evaluated on random queries. The results indicate that Duet converges significantly faster and better than Naru and UAE on the high dimensional dataset (Kddcup98). Even on the biggest dataset DMV, Duet still takes less or the same epochs to achieve the same max QError compared with Naru. This is because such an auto-regression model can not only learn each data point's probability but also learns the actual semantics of predicates. Therefore, in the iterative training process, since the sampled virtual tuples used in training Duet are generated from all tuples, Duet can still converge at a fast speed. This also proves the effectiveness of our sampling approach~\autoref{algorithm: Virtual Tuple Sampling Method}. Also, we observe that the UAE converges much slower than others on the DMV dataset, The reason is that the DMV dataset has higher cardinality, and since UAE just scales $\mathcal{L}_{query}$ with a single factor, the initial high value of the $\mathcal{L}_{query}$ (shown in \autoref{fig:loss_convergence}) affects its convergence.
...
\begin{table}[]
\caption{Throughput (tuples/s) comparison of data-driven and hybrid methods.}
\begin{center}
\begin{tabular}{llll}
\toprule
