...
\begin{abstract}
Let $\mathcal A_t \subseteq 2^\omega$ denote the set of infinite sequences of effective dimension $t$. Extending the work of Greenberg, Miller, Shen, and Westrick~\cite{GMSW:18}, we determine both how close and how far an infinite sequence of dimension $s$ can be from one of dimension $t$. That is, we find $\inf_{X\in \mathcal A_s} d(X,\mathcal A_t)$ and $\sup_{X\in \mathcal A_s} d(X,\mathcal A_t)$, where distance in $2^\omega$ is measured using the Besicovitch pseudometric. We also identify classes of sequences for which these infima and suprema are realized as minima and maxima. When $t<s$, we find $d(X,\mathcal A_t)$ is minimized when $X$ is a Bernoulli $H^{-1}(s)$-random, and maximized when $X$ belongs to a class of infinite sequences that we call $s$-codewords. When $s<t$, the situation is reversed. \joe{Add something about redundancy}
\end{abstract}
...
%We provide this generalization, and we give more details on where the maximum and minimum values of $d(X,t)$ are obtained. In Section \ref{sec:covering}, we define an \emph{$s$-codeword} to be an infinite sequence of dimension $s$ created in a certain manner to maximize the redundancy of the information that it contains.
...
\joe{I will add a couple sentences about redundancy of information and the case transition}
...
\end{lem}
\begin{proof}
It is easy to see that the two expressions for $S$ are equal using equation~(\ref{eqn:V(n,r)_basic_bound}) from Section~\ref{sec:prelim}.
...
\begin{align*}
K((X\uhr \ell_j) \concat \xi_j) &\leq s\ell_j - (s-t)\ell_j + (t-(1-H(c)))m_j + (1-H(c))m_j + o(\ell_j)\\
& = t(\ell_j+m_j) + o(\ell_j),\end{align*}
...
exactly $s$ and $d(X,1) = H^{-1}(1-s)$.
% $\inf \{d(X,Y) : \dim Y = 1\} = H^{-1}(1-s)$.
Does every 
...
Consider first the lower edge of $\A_t$ (i.e., the case where $s<t$). If $X \in \A_s$ and $d(X,\A_t) = d < \Worst(t,s)$, then no $t$-codeword can be the witness of this distance, simply because the $d$-ball around a $t$-codeword does not intersect $\A_s$. So, at least from the standpoint of these $s$ and $d$, no $t$-codeword is on the lower edge of $\A_t$. \joe{So if $t<1$, then for every $s$, if $d(X,\A_t) = d = \text{Best}(s,t)$, then no $t$-codeword can witness this distance.}

Turning our attention to the upper edge, consider when $s>t$. No Bernoulli $H^{-1}(t)$-random can be on the upper edge of $\A_t$ when $d < H^{-1}(s)-H^{-1}(t)$, similarly because the $d$-ball around a Bernoulli $H^{-1}(t)$-random does not intersect $\A_s$. \joe{So for every $s<1$, if $d(X,\A_t) = d = \text{Best}(s,t)$, then no Bernoulli $H^{-1}(t)$-random can witness this distance.}
...
We define $X_0$ 
as above, and also define $X_1$ as above, but in the definition of $X_1$ we 
additionally choose $B$ to be $2H^{-1}(s)$-Bernoulli random relative to both $Y$ and $s$. \joe{why $s$?}
Define $X = \Mix(X_0,X_1,r)$.
...
same proposition to $B$ and the sequence of $K_j$ (now relativized to $Y$ and $s$ \joe{again, why $s$?}),
we see that $\rho(B\restriction K_j) = 2H^{-1}(s) + o(1)$. So the number of 0's 
in $B\restriction K_j$ is 
\[
\textstyle{\left(1-2H^{-1}(s)+o(1)\right)\Bigl(\frac{1}{2}j + o(j)\Bigr) = \left(\frac{1}{2} - H^{-1}(s)\right)j + o(j).}
\]
Therefore, for each $j$,
\begin{align*}
\Delta(X\restriction n_j, Y \restriction n_j)
&\leq \sum_{\substack{i<j\\ b(r)(i)=0}} H^{-1}(1-s)i + o(i) \\[-20pt]
& \hspace{3cm} + \sum_{\substack{i<j\\ b(r)(i) = 1}} \left(\frac{1}{2} - H^{-1}(s)\right)i + o(i)\\
...
of $X_1\restriction n_j$. As before, but slightly more complicated, this quantity can be expressed as
\[
K(Y\restriction n_j) + K(B\restriction m(n_j)) = K(X\restriction n_j) + K(Y\restriction n_j, B\restriction m(n_j) \cond X\restriction n_j).
\]
\hl{***}
...
with $H(r_j) = (1-s)j + o(j)$ bits, and we can describe $B\restriction K_j$ 
...
\begin{align*}
K(Y\restriction n_j, B\restriction m(n_j) \cond X\restriction n_j) &\leq \sum_{\substack{i<j\\b(r)(i)=0}} \left((1-s) + \frac{1}{2}H(2p)\right)i + o(i) +{} \\
&\qquad \qquad \sum_{\substack{i<j\\ b(r)(i)=1}} H\left(\frac{\frac{1}{2}-p}{1-p}\right)(1-p)i + o(i)\\
 &\textstyle{ = (1-r)\left((1-s) + \frac{1}{2}H(2p)\right)n_j +{} }\\
 &\qquad \qquad \textstyle{rH\left(\frac{\frac{1}{2}-p}{1-p}\right)(1-p)n_j + o(n_j)}\\
 &= \textstyle{\left((1-s) + \frac{1}{2}H(2p)\right)n_j + o(n_j),}
 \end{align*}
the terms related to $r$ having canceled out due to the ternary entropy identity.
...
$$\textstyle{K(X\restriction n_j) \geq n_j\left(1 + \frac{1}{2}H(2p) - \left((1-s) + \frac{1}{2}H(2p)\right)\right) + o(n_j) = sn_j + o(n_j).}$$
\end{proof}
...
To bound the RHS, note that $\lim_j |C_j|/n_j = 1-r > 0$. Since $n_j = |E_j| + |C_j|$ and $\liminf_j H(d(X\restriction C_j,Z\restriction C_j)) < 1-s$ (as derived above), it follows that $\liminf_j K(Z\restriction n_j)/n_j < 1$. Therefore $\dim(Z) < 1$ as desired.
...
To bound the RHS, note that $\lim_j |E_j|/n_j = r > 0$. Since $n_j = |C_j| + |E_j|$ and $\liminf_j H(\rho(Z\restriction E_j)) < 1$ (as derived above), it follows that $\liminf_j K(Z\restriction n_j)/n_j <~1$. Therefore $\dim(Z) < 1$ as desired.
\end{proof}
