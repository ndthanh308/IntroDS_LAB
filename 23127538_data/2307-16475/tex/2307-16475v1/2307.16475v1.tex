...
\begin{document}
 ...
 The quasi-split case over a semilocal base is of special 
 interest and permits to   answer a  question raised by Marc Levine 
 on homogeneous $\SL_n$-spaces. At the end, we provide an application to the local-global principles for embeddings of \'etale algebras with involution into central simple algebras with involution.
...
which is a $\SL_n$-homogeneous space such that the stabilizer
...
We used implicitely there that the Galois cohomology set $H^1(k,S_n)$ classifies isomorphism classes of 
...
by an arbitrary  semilocal ring
...
\noindent{\bf Acknowledgments.} We thank Marc Levine for useful discussions and  the Camille Jordan  Institute for inviting  the second author.
...
which is an $S$--functor in associative commutative and unital algebras
\cite[9.6.2]{EGA1}.
...
category of fppf sheaves in groups, that is
$\bullet(T)= \Hom_{S-sch}(T,S)=h_S(T)$ for each $S$--scheme $T$.

 Let $\uG$ be a fppf $S$-sheaf in groups.
...
(ii) There exists a fppf cover $(S_i)_{i \in I}$ of $S$ and  $S_i$-group subsheaf  $\uH_i \subset \uG \times_S S_i$
...
In particular, if $\uH$ is a fppf $S$--subsheaf in groups of $\uG$, then
...
Hence there is a fppf cover $(S_i)_{i\in I}$ of $S$ such that $\uX(S_i)$ is nonempty and the action map $(\uG{\times_S}\uX)_{S_i}\to (\uX{\times}_S\uX)_{S_i}$ is surjective.
...
the $S$--sheaf in groups $\uA=  \uAut_\uG(\uX)$ which acts on the left of $\uX$.
...
$\phi: \uW \to \uA^{op}$ of $S$-sheaves in groups.

\smallskip

\noindent{\it $\phi$ is a monomorphism.} Let $T$ be a $S$-scheme and let
$w \in \uW(T)$ be an element such that $\phi(w)=1$. Up to localize we can assume
...
$f \in \uA(T)$. We put $x=f([1]) \in \uX(T)$ and up to localize we can assume
...
Consider the $T$-sheaf in groups $\uStab_{\uW}(x)$.
For each $T$-scheme $T'$ and $w\in \uStab_{\uW}(x)(T')$, up to localization we can assume that $w$ comes from an element $n\in \uN(T')$.
Then $[g]= [g]\cdot w=[gn]$, which implies $n\in \uH(T')$ and $w$ is the identity in $\uW(T')$.
Hence $\uW$ acts freely on $\uX$ and $\uX\to\uA\backslash\uX$ is a right torsor of $\uW$.
...
Choose a $fppf$-cover $\{S_i\}$of $S$ which trivializes the $\uA$-torsor $\uF$.
...
The map $\iota_i$ induces an isomorphism between $\uX'_{S_i}$ and $\uX_{S_i}$ which sends $[f, x]$ to $\iota_i(f)(x)$ for $x\in \uX(T_i)$, $f\in\uF(T_i)$ and for arbitrary $S_i$-scheme $T_i.$
...
Then the sheaf of isomorphisms $\uIsom_\uG(X,X')$ is a right $\uA$-torsor and $\uIsom_{\uG}({\uX,\uX'})\land^\uA\uX$ is canonically isomorphic to $\uX'$.
...
There is a natural map $i$ from $\uIsom_\uA(\uA,\uF)$ to $\uF$ which sends $\phi\in \uIsom_\uA(\uA,\uF)(S')$ to $\phi(1_\uA)\in\uF(S')$ for all $S$-schemes $S'$.
...
Therefore the map which sends an $S$-form $\uX'$ to the right $\uA$-torsor $\uIsom_{\uG}(\uX,\uX')$  is a bijection.
\end{proof}
...
Denote by $\uM_S$ (resp.\ $\uR_S$) the constant sheaf of $S$ associated to $M$ (resp.\ $R$).  
...
a twisted root datum. (c.f. \cite{SGA3} Exp. XXI, 1.1).
...
We denote by $W(\Psi)$ the Weyl group of the twisted root datum $\Psi$.
...
In this case, the Weyl group $W(\Psi)$ is a finite group generated by the reflections $s_\alpha$ defined by
...
We denote by $W(T)$  the group generated by $\tilde{s}_\alpha$.
The map sending  $s_{\alpha}$ to $\tilde{s}_\alpha$ defines an isomorphism
from  $W(\Psi)^{op}$ to $W(T)$.
For $w\in W(\Psi)$, we denote by $\tilde{w}$ its image in $W(T).$
In general, $W(\Psi)$ (resp.\ $W(T)$) is a finite \'etale group scheme over $S$, which is regarded as a subgroup scheme of $\uAut(\cM)$ (resp.\ $\uAut(T)$).
...
Let $\uN_G(T)$  the normalizer of $T$ in $G$. Then $\uN_G(T)/T$ is isomorphic to ${W}(T)\simeq W(\Phi(G,T))^{op}$. (cf. \cite{SGA3} Exp. XXII, 3.1-3.4)

\medskip

\subsection{The Orientation}
Let $\Psi_1$, $\Psi_2$ be two twisted root data. Suppose that $\Psi_2$ and $\Psi_1$ are of the same type at each $s\in S$ . 
...
Suppose $\uIsomext(\Psi_1,\Psi_2)(S)$ is nonempty.
For $u\in\uIsomext(S)$, we define $\uIsomint_u(\Psi_1,\Psi_2)$ to be the fiber of $\uIsom(\Psi_1,\Psi_2)\to\uIsomext(\Psi_1,\Psi_2)$ at $u$.


Let $G$ be a reductive group scheme  over $S$. 
The \emph{type} of
$G$ at $s$ is the type of the root datum of $G_{\ol{s}}$ with respect to its maximal torus. (ref.~\cite{SGA3},
Exp. 22, Def. 2.6.1, 2.7).

%We recall the definition of $\uIsomext(G,\Psi)$ and relevant properties.
Suppose that $G$ has a maximal torus $T$. We define $\uIsomext(G,\Psi)$ to be  $\uIsomext(\Phi(G,T),\Psi)$.
For a different choice of maximal tori $T'$, there is a canonical isomorphism between $\uIsomext(\Phi(G,T),\Psi)$
and $\uIsomext(\Phi(G,T'),\Psi)$ (ref. \cite[\S 1.2.1]{L1}). Hence $\uIsomext(G,\Psi)$ is well-defined.

We define $\uIsomext(\Psi,G)$ in a similar way.
As there is a canonical isomorphism between $\uIsom(\Phi(G,T),\Psi)$ and $\uIsom(\Psi,\Phi(G,T))$, which sends $f\in\uIsom(\Phi(G,T),\Psi)(S')$ to $f^{-1}\in\uIsom(\Psi,\Phi(G,T))(S')$, 
...
we have the following map \label{pairing1}:
\begin{equation}
\uIsomext(\Psi_1,\Psi_2)\times\uIsomext(\Psi_2,\Psi_3)\to\uIsomext(\Psi_1,\Psi_3),
\end{equation}
which comes from the composition of isomorphisms 
...
Denote by $G//G$ the adjoint quotient of $G$ (ref \cite{L2}). 
...
Let $T_1$ be the centralizer $\uC_G(t_1)$, which is also a maximal torus.
...
immersion and a group }\\
 \mbox{homomorphism which induces an isomorphism
}\\
...
Suppose  $\uIsomext(G,\Psi)(S)$ is nonempty. Fix an orientation $v \in \uIsomext(G,\Psi)(S)$.
The
...
The oriented embedding functor $\gE(G,\Psi,v)$ is representable by an affine $S$--scheme
which is a left homogeneous $G$-space (\cite[1.6]{L1}).  
...
It admits a right  action of
$W(T)$ and the quotient sheaf $\gE(G,\Psi,v)/W(T)$ identifies with the
$S$-scheme $\cT_G$ of maximal tori of $G$. (See \cite[\S 1.1 and \S1.2]{L1} for mare details.)
...
and $v' \in \Isomext(G,\Psi')(S)$. Let $u \in \uIsomext(\Psi,\Psi')(S)$ be the orientation  $v’ \cdot v$  by the pairing (\ref{pairing}). Under the identification of $W(\Psi)$ and $\uAut_G(\gE(G,\Psi,v))$ in (1), there is a natural isomorphism between
 $\uIsomint_u( \Psi, \Psi')$ and $\uIsom(\gE(G,\Psi,v), \gE(G,\Psi',v'))$ as right $W(\Psi)$--torsors.
...
Define a morphism $\iota:W(\Psi)\to \uAut_G(\gE(G,\Psi,v))$ as the following.
...
Define $\eta:\uIsomint_u(\Psi,\Psi')\to \uIsom(\gE(G,\Psi,v),\gE(G,\Psi',v'))$ by $\eta(f)(x)=x\circ \tilde{f} $,
...
Since $\uIsomint_u(\Psi,\Psi')$ and $\uIsom(\gE(G,\Psi,v),\gE(G,\Psi',v'))$ are both right $W(\Psi)$-torsors, this implies that $\eta$ is an isomorphism.
...
$\gE(G,\Psi',v')$, which is clearly an isomorphism.
...
\noindent We assume that $X$ is a left homogeneous space under $G$
and that for each point $s \in S$, the stabilizers of $X_{\ol s}$ are maximal tori  of $G_{\ol s}$.

\smallskip

(1) There exists a root datum $\Psi= (\cM, \cM^\vee, \cR, \cR^\vee)$
which is admissible for $G$ and an orientation $v \in \uIsomext(G, \Psi)(S)$
such that $X$ is isomorphic to $\gE(G,\Psi,v)$ as left $G$-homogeneous spaces.
...
we can find a fppf-cover ${S_{ij}}$ of $S_i$ such that there is $f\in X^q(S_{ij})$ with $f(T^q_{S{ij}})=H_{i,S_{ij}}$.
As the stabilizer $\uStab_{G_{S_{ij}}}(f)$ is $H_{i,S_{ij}}$, we have $X^q_{s_{ij}}\simeq G_{S_{ij}}/H_{i,S_{ij}}$ as $G_{S_{ij}}$-spaces.
Hence there is an isomorphism between $X^q$ and $X$ over $S_{ij}$ which preserves the $G$-structure.
...
Let $(B,T)$ be a Killing couple of $G$. We identity the Weyl group $W(T)$ with $N_G(T)/T$ and denote by $\Dyn(G)$
...
\section{The case of a semilocal ring}
...
Our goal is to adapt the framework of versal torsors \cite[\S 5]{GMS} from fields to the semilocal
ring setting by using algebraic spaces for which we use 
Olsson's book as reference \cite{O} as well with the 
Stacks project \cite{St}. It is close of 
Reichstein-Tossici's definition of (3)-versality
for algebraic groups \cite[def. 1.3]{RT}. 


Let $S=\Spec(R)$ be a semilocal scheme
and let $G$ be an affine flat $S$--group scheme of finite presentation.

\begin{sdefinition} 
Let $f:E \to X$ be a $G$--torsor where $E$ is a
qcqs $S$--scheme and $X$ is a qcqs $S$-algebraic space.
We say that $E \to X$ is (4)--versal if  
for any open retrocompact subspace $U$ 
schematically $S$--dense in  $X$ then for each semilocal $R$--algebra $B$ whose  residue fields  of maximal ideals are infinite, and for each $G$--torsor $F$ over $B$, there exists $x \in U(B)$ such that $E_x \cong F$ as $G$--torsors.
\end{sdefinition}
...
subspace $X'$ of $X$ which is a scheme \cite[Tag 06NH]{St}. If $X'$ can be chosen furthermore schematically $S$-dense
retrocompact, 
we see then that
...
\noindent (2) Assume that $\VV(\cE)$ admits an open universally dense $S$-subscheme $V$  which is $G$--stable  and such that $G$ acts freely on $V$. Let $Y=V/G$ be the quotient algebraic space.
...
Let $B$ be a semilocal ring whose residue fields of maximal ideals are infinite and $F$ be a $G$-torsor over $B$.
Denote by $\kappa_i$ the residue fields of maximal ideals  $\gM_i$ of $B$.

We first consider the case that  $B$ is indecomposable, i.e. $\Spec B$ is connected.
In this case $\cE\otimes_R B$ becomes a free module over $B$ of rank $n$, and we can regard $\GL(\cE)_B$ as $\GL_{n,B}$.
Consider the exact sequence of pointed sets:
\begin{align*}
1\to G(B)\to \GL_n(B)\to X(B)\overset{\delta}{\to} H^1_{fppf}(B,G)\to H^1_{fppf}(B,\GL_n).
\end{align*}
As $B$ is semilocal, we have  $H^1_{fppf}(B,\GL_n)=1$
since all locally free $B$-modules of rank $r$ are trivial.
Hence the torsor $F$ is a pull-back of $\GL(\cE)\to X$ at some element $x\in X(B)$.
Denote by $\overline{x}_i$ the image of $x$ in $X(\kappa_i)$.


Denote by $X_{\kappa_i}$ (resp.\ $X_B$) the functor obtained from $X$ restricted  to the category of $\kappa_i$-schemes (resp.\ $B$-schemes). 
Then $X_{\kappa_i}$ is an algebraic space over $\Spec \kappa_i$.
Note that we have a natural left $\GL(\cE)$-action on $X$.
Hence $x$ defines a morphism $f_x$ from $\GL_{n,B}$ to $X_B$ as follows:
$$f_x(g)=g\cdot x,$$
for all $B$-algebra $C$ and all $g\in \GL_{n,B}(C)$.
This induces a morphism $f_{\overline{x}_i}: \GL_{n,\kappa_i}\to X_{\kappa_i}$.

Let $U$ be an open retrocompact subspace of $X$  which is
...
by an open retrocompact subscheme $V_x$ of $\GL_{n,B}$ which is $S$-dense according to base change property listed in 
\ref{subsec_dominant}. 
Since $\kappa_i$ is infinite, $V_x(\kappa_i)$ is nonempty. 
Hence for each $i$, there is a point $g_i\in V_x(\kappa_i)$ such that $g_i\cdot x_i$ is in the image of $U(\kappa_i).$
By Chinese remainder theorem, there is an element $\overline{g}\in \GL_{n,B}(B/\prod \gM_i)$ which lifts $g_i$ for all $i$.
As $B$ is semilocal,  we can lift $\overline{g}$ to an element $g\in\GL_{n,B}(B)$.
By our construction $g$ is in $V_x(B)$. Hence $g\cdot x$ is in the image of $U(B)$.
 Since $\delta(x)=\delta(g\cdot x)$, $F$ is a pull back of $\GL(\cE)\to X$ at  $g\cdot x\in U(B)$.

As every semilocal ring  $B$ can be decomposed into a finite product of indecomposable semilocal rings $B_i$,
...
The  Chinese Remainder theorem 
 implies that $V(B) \not = \emptyset$.
We pick  $v\in V(B)$, we can define a right $G$-equivariant morphism $f_v$ from $\GL(\cE)$ to $\VV(\cE)$ by 
...
Let $G$ be a finite locally free $S$-scheme. Let $\cE$ be a locally free $G-R$-module of finite rank.
...
In this section, we assume that $S$ is a  semilocal scheme
which is a finite field or whose residue fields (of maximal ideals) are infinite.
Let $G$ be a quasi-split reductive $S$--group scheme.

Let $(B,T)$ be a Killing couple of $G$. 
We identity the Weyl group $W(T)$ with $N_G(T)/T$ and denote by $\Dyn(G)$
the Dynkin scheme (which is finite \'etale over $S$). We have a canonical isomorphism
$\Dyn(\Phi(G,T)) \simlgr \Dyn(G)$.
...
The following generalizes to the semilocal
setting a result of Gille-Raghunathan \cite{Gi2004,Rg}. 



\begin{stheorem} \label{thm_semi_local} Let $S$ be a  semilocal scheme
which is a finite field or whose residue fields (at maximal ideals) are infinite.
...
\begin{proof} The finite field is obvious since all homogeneous $G$-spaces have a $k$-rational point \cite[III.2.4]{Se}.
We assume that $S=\Spec(R)$ where $R$ is a semilocal ring whose residue fields (of maximal ideals)
$\kappa_1, \dots \, \kappa_c$ are infinite.

Without loss of generality, we can assume that $S$ is connected and $G$ is semisimple simply connected. Furthermore using the 
decomposition \cite[XXIV.5.5]{SGA3} and  the usual Weil restriction 
argument, we can assume
that the geometric fibers of $G$ are almost simple.
...
\noindent
\textbf{Case 1.}  G is without $A_{2n}$-components.
...
Denote by $u'\in\uIsomext(\Phi(G,T),\Phi(G,T'))(S)$ be the image of $(c,c')$ under the pairing in Prop. \ref{pairing}.
...
First note that for type $A$, we have a Cartan involution defined over $S$, hence if we we have an embedding with respect to one orientation, we have an embedding with respect to the other orientation.
Hence it suffices to show that $\gE(G,\Psi)\neq\emptyset$.
The key point is to reduce to the case that $G$ is of type $A_{2n+1}$. 

Let $R/S$ be an \'etale quadratic algebra which splits $G$.
%Let $(V,h)$ be  a free $R$-module of rank $2n+1$ with a Hermitian form $h$ over $R/S$ whose Witt index is $n$.
We can assume that $G$ is the unitary group $U(V, h)$ for some non-degenerate hermitian form $(V,h)$ over $R/S$, where $V$ is a free $R$-module $V$ of rank $2n+1$
and $h$ is of Witt index $n$. (See \cite[Cor. 1.7]{L1}.)

%such that $\sigma\mid_R=\tau_{2n+1}\mid_R$.
%Moreover the embeddings of $\Psi$ in $G$ are in one-to-one correspondence to  embeddings of $S$-algebras with involution $(E,\sigma)$ to $(M_{2n+1},\tau_{2n+1})$.
%To fix an orientation $v \in \uIsomext( G, \Psi)(S)$ is equivalent to fix an $S$-algebra isomorphism $f$ from $R$ to itself.
%An oriented embedding of $\Psi$ in $G$ with respect to $v$ corresponds to an embedding $\eta$ of $S$-algebras with involution such that $\eta \mid_R=f\mid_R$.(ref)
...
$\gS_{2n+1}\times\ZZ /2 \ZZ$ to $\gS_{2n+1}\times \ZZ/ 2\ZZ$ which maps $\ZZ/2\ZZ$ isomorphically to itself.
The map $\iota$ induces a map $\iota_{\ast}: H^1_{\acute{e}t}(S,\gS_{2n+1}\times \ZZ/2\ZZ )\to H^1_{\acute{e}t}(S,\gS_{2n+2}\times\ZZ/ 2\ZZ)$.
Note that $\gS_{2n+1}\times\ZZ /2 \ZZ$ is isomorphic to the Weyl group of the split root datum of $\GL_{2n}$. 
Hence $H^1_{\acute{e}t}(S,\gS_{2n+1}\times \ZZ/2\ZZ)$ classifies those root data which are \'etale locally isomorphic to the split root datum of $\GL_{2n+1,S}$.

Thus $\Psi=(\cM,\cM^\vee,\cR,\cR^\vee)$ corresponds to some   $[\alpha] \in H^1_{\acute{e}t}(S,\gS_{2n+1}\times \ZZ/2\ZZ )$.
We twist the split root datum of $\GL_{2n+2,S}$ by $\iota_\ast([\alpha])$ and denote it by $\Psi'=(\cM',\cM'^\vee,\cR',\cR'^\vee)$.
Let $T$ and $T'$ be the tori determined by $\cM$ and $\cM'$ respectively.
Note that by our construction $\cM'^\vee$ can be written as $\cM^\vee\oplus \cE^\vee$ with $\cE$ is locally free of rank 1 in such a way that  $R^\vee$ is injectively sent to a subset of $R'^\vee$.
Fix such an isomorphism $\cM'^\vee\simeq\cM^\vee\oplus \cE^\vee$. This in turns gives an isomorphism $T'\simeq T\times \bR^{(1)}_{R/S}(\bG_m)$.
We denote by $T_0$ the subtorus $1\times \bR^{(1)}_{R/S}(\bG_m)\subseteq T'$ (under the above isomorphism).
...
Since $G'$ is quasi-split of type $A_{2n+1}$, by Case 1 we have $\gE(G',\Psi',v')(S)\neq\emptyset$.

Choose $f\in \gE(G',\Psi',v')(S)$. 
...
Let $V_0$ be the maximal submodule of $V$ fixed by $f(T_0)$. Namely, $V_0$ is the maximal submodule on which $f(T_0)$ acts trivially.
Note that as $f(T_0)$ is a torus,  $V$ is \'etale locally decomposed into eigenspace of $f(T_0)$. Thus the rank of $V_0$ is $2n+1$, which can be checked \'etale locally.
As $h'$ is a hyperbolic Hermitian form on $V'$, $V_0$ must contains a totally isotropic submodule of rank $n$.
Let $h_0$ be the restriction of $h'$ on $V_0$. 
As  $(V_0,h_0)$ and $(V,h)$ are $R$-module of the same rank and with maximal Witt index,  we have $U(V_0,h_0)\simeq U(V,h)=G$.  
...
Therefore $C/f(T_0)\simeq U(V_0,h_0)$ and $f$ gives an embedding of $T\simeq T'/T_0$ in $G$ with respect to $\Psi$.
\end{proof}


\begin{scorollary} \label{cor_main} Let $S$ be a semilocal scheme 
which is the spectrum of a finite field
or the spectrum of a semilocal rings
whose residue fields at the maximal ideals
are infinite.
Let $G$ be a quasi-split reductive $S$--group scheme.
...
We assume that $X$ is a left homogeneous under $G$
and that for each point $s \in S$, the stabilizers of $X_{\ol s}$ are maximal tori  of $G_{\ol s}$.
...
\bibitem{A}  S. Anantharaman, {\it Schémas en groupes, espaces homogènes et espaces algébriques sur une base de dimension 1},
  Mémoires de la Société Mathématique de France {\bf 33} (1973), 5-79. 
...
\bibitem{DG} M. Demazure et P. Gabriel,
{\it Groupes alg\'ebriques}, Masson (1970).



\bibitem{GMS} S. Garibaldi, A. Merkurjev et J.-P. Serre, {\it
...
\bibitem[O]{O} M. Olsson, {\it Algebraic spaces and stacks}, American Mathematical Society Colloquium Publications, 62. American Mathematical Society, Providence, RI, 2016.
