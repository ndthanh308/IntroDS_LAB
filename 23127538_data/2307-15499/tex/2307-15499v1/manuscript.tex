...
\[\mathcal{L}_c = -\partial_{x}^3+(c-2\phi_c) \partial_{x}-2\partial_x \phi_c = -\partial_{x}^3+c\partial_x -2\partial_x[\phi_c \cdot].\] 
As a linear operator on the space $L^2(\R)$, the operator $\mathcal{L}_c$ has spectrum $i\R$. Contained therein is a double eigenvalue at 0 with %corresponding eigenvector $\partial_x \phi_c$, which arises due to %translational invariance. In fact, the linear operator $\mathcal{L}_c$ has 
...
$L^2_a$ with $0<a<\sqrt{c}$ moves the essential spectrum to the left of the imaginary axis. Its (formal) adjoint $\mathcal{L}^*_c$ on the space $L^2_{-a}$ has a generalized kernel spanned by the soliton $ \phi_c$ and the primitive 
...
De Bouard and Debussche analyzed the stochastic soliton dynamics produced by the KdV equation \eqref{eqn:SKDV}  with multiplicative space-time noise in \cite{debouardsoliton}. Similar to the approach to the approach taken in this work, the authors consider a decomposition of the form \eqref{eqn:simpledecomposition} and formulate modulation equations for the soliton parameters. The authors, moreover, estimate the exit-time of the solution from a neighborhood of the modulated soliton. See also the works \cite{debouardadditive,NLSmodulation}, where the same authors study solitons in the KdV equation with additive noise and a stochastic Gross-Pitaevskii equation.
...
In a more recent work by Cartwright and Gottwald \cite{cartwright}, the authors apply a collective coordinate framework developed in \cite{cartwright2019collective} to \eqref{eqn:SKDV} in the scalar case, where $ W_t^Q$ in \eqref{eqn:SKDV} is a scalar Brownian motion $ \beta_t$. Their approach modulates the amplitude and width of the soliton independently, instead of requiring that the modulated soliton remains in the family \eqref{eqn:soliton}. The authors develop an $O(\sigma)$ modulation system which dictates that the amplitude follows a geometric Brownian motion with zero drift.  The modulation system of \cite{cartwright} furthermore predicts to leading-order that the soliton-width remains constant, which is not captured in numerical simulations of \eqref{eqn:SKDV} that naturally also include higher order effects.
...
We restrict ourselves in this work to formal arguments to support the construction of the soliton-tracking method, which yields a coupled SPDE system that describes the evolution of $v(t), c(t)$ and $\xi(t)$. Although we anticipate that a rigorous meta-stability result based on the methods presented in this work is possible, one needs to control transformations of the perturbation due to the rescaling. It seems that this requires spatial information on the linearized frozen-frame evolution beyond the semigroup bounds obtained in \cite{pegoweinstein}. For the moment, we rely on numerical simulations to validate our findings.
...
In contrast to the stochastic wave position studied in \cite{hamsterscalar} and \cite{hamster}, the evolution of the perturbation $v(t)$ does not decouple from the rescaling process $\alpha(t)$. This significantly complicates our analysis, since $\alpha(t)$ develops significant fluctuations on short timescales. In order to account for this, we first expand the perturbation $v(t)$ in terms of the small parameter $\sigma$, using $\alpha$
as an external input. Expanding the modulation equation for $\alpha(t)$ in terms of $v$ subsequently produces SDE approximations for $\alpha(t)$ that have random coefficients. Solving these SDEs then provides the desired final approximations, which can in principle be computed at any desired order in $\sigma$.
%finally closes the  
%We subsequently 
...
The amplitude growth rate and velocity correction are determined by statistical properties of the perturbation with respect to the modulated soliton. This further motivates  the use of the frozen-frame formulation \eqref{eqn:decomposition}, which keeps the stochastic traveling wave in a fixed reference frame and facilitates analysis of the perturbation shape. 
...
\paragraph{Outline}
This paper is organized as follows. In \S\ref{subsec:frozen}, we derive an SPDE that describes the dynamics of the remainder $v$ defined in \eqref{eqn:decomposition} via It\^o's lemma. We proceed by constructing the It\^o processes $\xi(t)$ and $\alpha(t)$ that track the soliton position and amplitude, respectively, in \S\ref{subsec:modulation}. In \S\ref{sec:solitondynamics}, we formulate an expansion of the SPDE system that governs the dynamics of $\alpha(t)$ and $v(t)$, and from there derive the leading-order behavior of the mean and variance of the effective soliton parameter $c(t)$ and position $\xi(t)$. We verify these findings via numerical simulations throughout \S\ref{sec:solitondynamics}.
...
First, in \S\ref{subsec:setup}, we describe the forms of multiplicative noise that can be treated by our method. In \S\ref{subsec:frozen}, we derive an SPDE that describes the evolution of the perturbation $v$ in the co-moving frame, which we use in \S\ref{subsec:modulation} to prescribe the dynamics of our processes $\xi$ and $\alpha$. Finally, we illustrate the effectiveness of our decomposition in \S\ref{subsec:numerics} and explore the resulting dynamics via numerical simulations.
...
With this assumption in place, we can assign a rigorous meaning to the SPDE \eqref{eqn:skdvgeneral}. Based on the flow $S(t)=e^{-t\partial_x^3}$ generated by the linear equation $u_t=-\partial_x^3 u$, we call an $H^1$-valued process $u$ a mild solution to \eqref{eqn:skdvgeneral} with initial condition $u(0,\cdot)=u_0$ if the mild formula
\begin{align}
    u(t)=S(t)u_0-\int_0^t S(t-s) \partial_x\bigl(u^2(s)\bigr) \ \d s+\sigma\int_0^tS(t-s) M\bigl(u(s)\bigr) [\d W^Q_s]\label{eqn:mild}
\end{align}
holds for all $t>0$. Alternatively, one can also consider weak solutions: an $H^1$-valued process $u$ is a weak solution to \eqref{eqn:skdvgeneral} with initial condition $u(0,\cdot)=u_0$ if the identity 
\begin{align}
...
\end{align}
holds for all $\zeta\in H^2(\R)$ and $t>0$. See for instance \cite{weaksol}, which asserts the existence of weak solutions for \eqref{eqn:skdvgeneral} posed on a bounded domain with scalar noise. This solution type is, however, less common in the stochastic KdV literature.
...
As pointed out in \cite{cartwright}, we remark that \eqref{eqn:scalar} can be transformed into a KdV equation with a random coefficient on the nonlinearity. Indeed, by factoring out the geometric Brownian motion 
\[g(t)=e^{-\frac{\sigma^2}{2}t+\sigma\beta_t}\] 
as $v=g^{-1}u$, we find  
\[\d v=-(\partial_x^3 v+2 g(t)v\partial_x v) \ \d t.\]
This reduces matters of well-posedness to the well-posedness of a KdV equation with varying coefficient on the nonlinearity. We are, however, unaware of results that establish the well-posedness of such an equation with a non-smooth coefficient.
...
so that the correlation length $\zeta$ of the Guassian kernel is effectively rescaled to $\zeta/\alpha$.
...
\end{bmatrix}.
\end{align}
The matrix $K(v)$ is invertible in case $\|v\|_{L^2}<\delta$ for some $\delta>0$, since $K(v)$ is invertible at $v=0$ and the mapping $v \mapsto \det K(v)$ is continuous from $L^2$ to $\R$. We then set
...
\subsubsection{Example I: Scalar noise}\label{subsubsec:scalarnoisemod}
...
\subsubsection{Example I\joinR I\joinR I: Space-time white noise}\label{subsubsec:whitenoisemod}
...
Figure~\ref{fig:pathwisesoliton} shows one realization obtained from a simulation of the KdV equation \eqref{eqn:scalar} with multiplicative scalar noise in both the original frame and co-moving frame. 
...
To develop our approximation procedure, it is relevant to note that the process $\alpha$ exhibits significant fluctuations, while the perturbation $v$ remains relatively small due to the damping of the semigroup. Indeed, we observe that $\alpha$ grows as $O(\sigma \sqrt{t})$, which can be anticipated by noting that $I^\sigma_\alpha$ contains no damping terms. On the other hand, Figure~\ref{fig:perturbationscalar} indicates that $v$ grows at a slower rate, namely $O(\sigma \ln t)$. To construct approximations of $\alpha$ that account for the influence of the perturbation $v$, we introduce SDEs based on an expansion of the $v$-dependent coupling terms. In broad terms, we will expand the $\alpha$ dynamics in terms of $v$, while expanding the $v$ dynamics in terms of $\sigma$, treating $\alpha$ as an external input.


Below, in \S\ref{subsec:expansion}, we describe the approximations to the coupled system \eqref{eqn:integralformalpha} in more detail
and discuss how they lead to 
%We discuss how 
approximations for the soliton amplitude $c$ and phase shift $\Omega$ defined in \eqref{eqn:phaseshift}. %, follow. 
We proceed by evaluating the first few approximations for the cases of Example I (scalar noise) and Example I\joinR I\joinR I (space-time white noise) in \S\ref{subsec:scalarnoiseapprox} and \S\ref{subsec:whitenoiseapprox}, respectively. These explicit approximations are used to compute leading-order statistical properties, which we compare with sample statistics of the numerical observations $c_{\text{fit}}$ and $\Omega_{\text{fit}}$ defined in \eqref{eqn:phasedefinitions} and \eqref{eqn:phaseshift}.
...
\paragraph{The combined system approximation}
...
and introduce for $k\geq 1$ the approximations
    \[v_k(t)={V}_k(\alpha_{k-1},t),\]
to $v$.

\paragraph{Soliton parameters}
...
On the other hand, $\xi$ depends less straightforwardly on $v$ and $\alpha$. In \S\ref{sec:tracking} we have seen that $\xi$ can be recovered from the identity
\begin{align*}
...
\begin{align*}
 \Omega_0(t)=&\ I_\Omega^\sigma(0,\alpha_0,t),
\end{align*}
and for $k\geq1$
\begin{align*}
 \Omega_k(t)=&\ I^\sigma_\Omega(0,\alpha_{k},t)+\sum_{i=1}^k[I^\sigma_\Omega]^{(i)}\bigl(V_{\lfloor \frac{k}{i} \rfloor}(\alpha_k,t);\alpha_{k},t\bigr).
\end{align*}
...
\subsection{Example I: Scalar noise}\label{subsec:scalarnoiseapprox}
...
\subsection{Example I\joinR I\joinR I: Space-time white noise}
