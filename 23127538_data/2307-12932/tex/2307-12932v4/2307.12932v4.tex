...
\partial_{ee}u\leq \partial_{ee}u_0+\int_0^Tc_f(t)\,dt,\ e\in\R^n,\ |e|=1,\ \partial_{ee}f\leq c_f(t)\in L^1(0,T)
\end{equation}
and  
\[
...
\[
\|(u_\eps-u)^+\|_{L^\infty}\leq C\eps,
\]
and we discuss its sharpness in Example \ref{ex2rate}. The best rate from below on the difference $u_\eps-u$ known in the literature is the classical $\mathcal{O}(\sqrt{\eps})$ bound for Lipschitz solutions. We try to enhance this order by investigating (two-side) rates of convergence in weaker $L^p$ norms. First, we prove that for semiconcave solutions the estimate
\[
\|u_\eps-u\|_{L^\infty_t(L^1_x)}\leq C\eps.
\]
This is crucial to achieve the interpolated bound
\[
\|u_\eps-u\|_{L^\infty_t(L^p_x)}\leq C\eps^{\frac12+\frac{1}{2p}},
\]
which appears to our knowledge the best two-side estimate available in the literature for semiconcave solutions. We complement our analysis with $L^p$ convergence of gradients in the vanishing viscosity process, see Remark \ref{rateDu}.\\
...
Another important progress is the treatment, through duality methods, of the following regularization effect for (not necessarily uniformly convex) Hamiltonians $H$ behaving like $|p|^\gamma$, $\gamma>1$, see \eqref{H1} below,
\[
\|Du_\eps(t)\|_\infty\leq \frac{1}{C_H t^\frac1\gamma}\left(\mathrm{osc}_{\R^n\times(0,T)}u_\eps\right)^\frac1\gamma.
\]
...
In perspective, the previous results can  lead to new developments in the context of Mean Field Games, nonlocal problems, fully nonlinear equations, and Porous-Medium equations, that will be the matter of future research.
...
\[
H_1(x,p)=h_1(x)\frac{(|p|^2-1)^2-1}{|p|^2+1}
\]
with $h_1>0$, smooth enough (to ensure \eqref{H2} and \eqref{H3}) and bounded, is nonconvex and satisfies the assumption \eqref{H1} with $\gamma=2$, $C_{H,1}=\inf_{\R^n} h_1$ and $\widetilde{C}_{H,1}=-6\sup_{\R^n} h_1$. It can be written as
\[
H_1(x,p)=h_1(x)(|p|^2+z(p)),\ z(p)=-3+\frac{3}{1+|p|^2}\leq0.
\]
A nonconvex example satisfying \eqref{H4} with $\gamma=2$ is the following
\[
H_2(x,p)=h_2(x)(|p|^2+g(p)),
\]
where $g(p)=g_1(p)+g_2(p)$ with $g_1$ convex and $g_2\in C^2$ such that $\|D^2g_2(p)\|_\infty\leq K$. Here, $C_{H,4}=2\inf_{\R^n} h_2$ and $\widetilde C_{H,4}=-2K\inf_{\R^n} h_2$.
...
\end{ex}
We start by proving the preservation of semiconcavity properties from the data. From now on, we will exploit integral methods to get zero, first- and second-order estimates, so we implicitly assume that $u$ and its derivatives belong to a suitable energy class when deriving the next estimates by duality. This in particular allows to justify integration by parts on the whole space $\R^n$. We thus discuss only how to obtain a priori estimates, without addressing the problem of existence and uniqueness of solutions in parabolic Sobolev spaces: we refer to \cite{CGsima} for more details.\\
...
\begin{multline*}
\int_{\R^n} v(x, \tau) \rho_{\tau}(x)\,dx + \int_{0}^{\tau} \int_{\R^n} Du_{\xi} \cdot D^{2}_{pp}H(x,Du) Du_{\xi} \rho\, dxdt = 
\\
...
%\textcolor{red}{\begin{rem}PROPONGO DI TOGLIERLO
...
\end{equation}
using the same localization argument of Lemma \ref{well}. Hence the proof continues along the same lines of that in Theorem \ref{semiconc1}. Note that by duality one gets a global estimate of $(u_{\xi\xi})^+$ in the space $L^\infty(\R^n)+L^p(\R^n)$, which is embedded into $L^p_{\mathrm{loc}}(\R^n)$.
\end{proof}
...
We start proving a two-side rate of convergence for the vanishing viscosity for Lipschitz solutions. The result is already known from \cite{Evansadjoint}, we only slightly reword the proof and take care of the constants in the estimates to compare it with the corresponding results obtained in \cite{L82book}, see also \cite[p. 207]{FlemingGame}.
\begin{thm}\label{ev1}
...
%\textcolor{red}{POSSIAMO ANCHE NON METTERLO, IN CASO CAMBIARE INTRODUZIONE\begin{rem}\label{newproof}
%We also show another viewpoint for the proof of Theorem \ref{ev1}, suggested by L.C. Evans in \cite{Evansadjoint} via the maximum principle. Since the equation is smooth in the viscosity parameter $\eps$ away from $\eps=0$, we can differentiate the PDE respect to $\eps$. One can formalize the argument proceeding as above with an estimate on the difference of two solutions with viscosity parameters $\eps$ and $\eta$. We proceed by the adjoint-Bernstein method, by finding the equation solved by the function
%\[
%w=\eps^\alpha v+|Du^\eps|^2,\ \alpha>0,\ v=\frac{\partial u}{\partial\eps},
%\]
%$\alpha$ being selected later. We find
%\[
%\partial_t w-\eps\Delta w+2\eps|D^2u^\eps|^2+D_pH(Du)\cdot Dw=\eps^\alpha\Delta u^\eps
%\]
%By duality, we obtain
%\[
%\int_{\R^n}w(\tau)\rho_\tau(x)\,dx+2\eps\iint_{Q_\tau}|D^2u^\eps|^2\rho\,dxdt=\int_{\R^n}w(0)\rho(x,0)\,dx+\iint_{Q_\tau}\eps^\alpha\Delta u\rho\,dxdt.
%\]
%We then use Cauchy-Schwarz and the weighted Young's inequality to find
%\[
%\iint_{Q_\tau}\eps^\alpha\Delta u\rho\,dxdt\leq 2\eps^{2\alpha}\iint_{Q_\tau}|D^2u^\eps|^2\rho+\frac{n}{8}\iint_{Q_\tau}\rho\,dxdt.
%\]
%Recall that
%\[
%\int_{\R^n}\rho\,dx=1
%\]
%and find, choosing $\alpha=\frac12$, the inequality
%\[
%\int_{\R^n}w(\tau)\rho_\tau(x)\,dx\leq \int_{\R^n}w(0)\rho(x,0)\,dx+\frac{n\tau}{8}.
%\]
%This implies, using that $v(0)=0$, the one-side estimate
%\[
%\sqrt{\eps}v\leq \|Du_0\|_{L^\infty(\R^n)}^2+\frac{n\tau}{8}.
%\]
%The previous estimate is useful since $1/\sqrt{\eps}$ is integrable near $\eps=0$. This implies
%\[
%u^{\eps_1}(x,\tau)-u^{\eps_2}(x,\tau)\leq 2\left(\|Du_0\|_{L^\infty(\R^n)}^2+\frac{n\tau}{8}\right)
%\]
%provided $0<\eps_2<\eps_1$. We let $\eps_2\to0$ and conclude the estimate from above. The estimate from below is similar, it is enough to find the equation solved by
%\[
%\widetilde w=\sqrt{\eps}v-|Du^\eps|^2
%\]
%and argue in the same way. The same estimate can be achieved from the differential inequality solved by $w$ (resp. $\widetilde{w}$) via the maximum (minimum) principle.
%\end{rem}}
We now turn to SSH solutions, and propose a new proof by PDE methods of a result obtained by P.-L. Lions in \cite{L82book} through a related probabilistic argument. Differently from \cite{L82book}, we require on $u_\eps$ the weaker condition $L^1_t(L^\infty_x)$-SSH.
...
%It is well-known that $u_\eps$ is also semiconvex for short times independently of the diffusion. This observation combined with Theorem \ref{inftyrate1s} imply a two-side bound of order $\mathcal{O}(\eps)$ in $L^\infty$ norm.
%\end{rem}}
...
Assume that $u_\eps,u$ are $L^1_t(L^\infty_x)$-semiconcave and Lipschitz continuous solutions of \eqref{hjsemi}. Then we have the $L^p$ rate of convergence
\[
...
%\end{proof}
%\textcolor{red}{PROPONGO DI TOGLIERLO We now provide an improvement of the rate for Lipschitz solutions of Hamilton-Jacobi equations with semisuperharmonic initial condition. Such an enhancement is due to the uniform convexity of the Hamiltonian. This extends a result of \cite{TranBook} to the time-dependent setting:
...
%\end{proof}
%\begin{rem}
%A slight modification of the proof of Theorem \ref{new2} gives an alternative proof to Theorem \ref{rateL1}. It is enough to take $\rho_\tau\in L^\infty(\T^n)$ such that $\|\rho_\tau\|_{L^\infty(\T^n)}=1$ and avoid the integration by parts in t\end{rem}}
%he estimate \eqref{D2ppH}.
...
This implies
\[
\|Du_\eps-Du_\eta\|_{L^\infty(0,T;L^2(\T^n))}\leq C\sqrt{\eps-\eta}.
\]
or
\[
\|Du_\eps-Du_\eta\|_{L^\infty(0,T;L^2(\T^n))}\leq C(\eps-\eta)^\frac14.
\]
...
\[
\|Du_\eps-Du_\eta\|_{L^\infty(0,T;L^1(\T^n))}\leq C\sqrt{\eps-\eta}.
\]
...
\begin{proof}
	...
	\[
	u_{\xi\xi}\leq \frac{C(\delta +\|Du\|_{\infty}^2)^\frac{2-\gamma}{2}}{t}=:C_0(t).
	\]
The one-side interpolation inequality, cf. \cite{L82book},
	...
	\[
	\|Du(t)\|_{L^\infty(\R^n)}\leq (2\|u(t)\|_{L^\infty(\R^n)}C_0(t))^\frac12=\sqrt{2C}\|u\|_\infty^\frac12(\delta +\|Du\|_{\infty}^2)^\frac{2-\gamma}{4}t^{-\frac12}.
	\] 
	Rearranging the terms this implies
	\[
	\|Du(t)\|_{L^\infty(\R^n)}\leq 2^\frac1\gamma C^\frac{1}{\gamma}\|u\|_\infty^{\frac{1}{\gamma}}t^{-\frac1\gamma}
	\]
	and concludes the proof.
\end{proof}
%\begin{rem}
%The previous result applies, for instance, to convex Hamiltonians and, in particular, to those having power-like growth. The proof of the one-side interpolation result can be found in P.-L. Lions, see Proposition 13.1 in \cite{L82book}. \textcolor{red}{TOGLIERE?Related results have been obtained in \cite{Lions85aa} and also in \cite{Laurencot} for first-order equations with power-like gradient dependent terms, under the general assumption (corresponding roughly speaking to \eqref{H1})
%\[
...
%\end{rem}
%
%\textcolor{red}{togliere\begin{rem}
...
In this section we discuss regularizing effects for general coercive Hamiltonians as initiated in \cite{Lions85aa,Laurencot}. The main result is the following\begin{thm}
...
\[
\|Du_\eps(\tau)\|_\infty\leq \frac{1}{C_{H,1} \tau^\frac1\gamma}\left(\mathrm{osc}_{\R^n\times(0,T)}u_\eps\right)^\frac1\gamma,\ \tau\in(0,T).
\]
\end{thm}
\begin{proof}
...
\[
\|Du(\tau)\|_\infty\leq \frac{(\gamma')^\frac1\gamma}{\tau^\frac1\gamma}(\mathrm{osc}_{\R^n\times(0,T)}u)^\frac1\gamma.
\]
...
%\end{proof}
%\textcolor{red}{The next is a one-side interpolation result of Gagliardo-Nirenberg type in the endpoint case.
...
Assume $b\in L^\infty_{\mathrm{loc}}(\R^n\times(0,\tau))$. Then there exists a distributional solution $\rho$ verifying $\int_{\R^n}\rho(t)\,dx\leq1$. If in addition $b\in L^2(\R^n\times(0,\tau))$, $|b|\in L^k(\rho\,dxdt)$ for some $k>1$, $\eps>0$, we have a unique weak (energy) solution of \eqref{adjoint} which satisfies $\rho\in L^\infty([0,\tau];L^1(\R^n))$. Moreover, if $\|\rho_\tau\|_1=1$ and $\rho_\tau\geq0$, we have $\|\rho(t)\|_1=1$ for all $t\in[0,\tau)$ and $\rho\geq0$ in $Q$.
\end{lemma}
\begin{proof}
...
%\proof
%
%\textcolor{red}{PROPONGO DI TAGLIARLO E CITARE BOGACHEV Assume that the data of the equation are locally smooth function, then the conclusion follows by an approximation argument from the local integrability. Moreover, in the following we use the summation index notation to simplify the presentation. 
