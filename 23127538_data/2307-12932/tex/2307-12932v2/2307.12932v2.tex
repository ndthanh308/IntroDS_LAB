...
\[
\iint_{Q_\tau}\alpha t^{\alpha-1}u_{\xi\xi}\rho\,dxdt\leq C_{H,4}\iint_{Q_\tau}t^\alpha|D^2u|^2(\delta+|Du|^2)^{\frac{\gamma-2}{2}}\rho\,dxdt+\frac{\alpha^2}{4C_{H,4}}\iint_{Q_\tau}t^{\alpha-2}\rho(\delta+|Du|^2)^{-\frac{\gamma-2}{2}}\,dxdt.
\]
This implies
\[
\tau^\alpha u_{\xi\xi}\leq \frac{\alpha^2}{4C_{H,4}(\alpha-1)}\|(\delta+|Du|^2)^{\frac{2-\gamma}{2}}\|_\infty\tau^{\alpha-1}+\frac{\tau^{\alpha+1}}{\alpha+1},
\]
...
\end{rem}
The next are the main results of the section.
...
\]
We can now proceed again by duality using that $\Delta u_\eta\geq -c_\eta(t)$, and hence testing equation solved by $z$ against the solution of \eqref{adjrate2} having $\rho_\tau\in L^1$. This implies, since $\int_{\T^n}\rho\,dx=1$ and $\rho\geq0$, the inequality
\[
...
\[
\|u_\eps-u_\eta\|_{L^\infty(0,T;L^1(\T^n))}\leq C\eps.
\]
...
\begin{thm}\label{new}
Let $u_\eps,u_\eta$ be semiconcave solutions of \eqref{hjsemi} with viscosity $\eps$ and, respectively, $\eta$. Let also $H$ be uniformly convex. Then
\[
...
\end{proof}
\begin{rem}
Theorem \ref{new} provides a different proof of the $L^1$ rate in Theorem \ref{rateL1} just by taking $\rho(\tau)=\mathrm{sgn}(\tau)$. The estimate from above on $u_\eps-u_\eta$ in Theorem \ref{new} can be improved to $L^\infty$ as shown in Theorem \ref{inftyrate1s}. 
%Moreover, in the term 
%\[
%-\int_{\T^n}(u_\eps)_{x_ix_i}(\tau)\rho_\tau(x)\,dx+\int_{\T^n}(u_\eps)_{x_ix_i}(0)\rho(0)\,dx
%\]
%we can integrate by parts and avoid to impose the semiconcavity to get that the last inequality is less than or equal to
%\[
%2\|(u_{\eps})_{x_i}\|_{L^\infty(Q)}\|\rho_{x_i}\|_{L^1(\T^n)},
%\]
%and get the average estimate
%\[
%\sup_{t\in(0,T)}\left|\int_{\T^n}(u_\eps-u_\eta)(t)\rho_\tau(x)\,dx\right|\leq C(\eps-\eta),\ \rho_\tau\in W^{1,1}(\T^n),
%\]
%where $C$ depends in particular on $\|D\rho_\tau\|_{L^1(\T^n)}$. This has been already studied in Theorem 1.43 of \cite{TranBook} in the elliptic case.
\end{rem}
\begin{rem}
...
\begin{rem}
It is worth remarking that under the assumption of the semi-superharmonic estimate in Theorem \ref{semiconc1}, Lemma \ref{laplL1} combined with the rates $\|u_\eps-u_\eta\|_{L^\infty}\leq C(\eps-\eta)$ or $\|u_\eps-u_\eta\|_{L^\infty}\leq C(\sqrt{\eps}-\sqrt{\eta})$ obtained in Theorems \ref{rateL1} and \ref{ev1} imply the following integral a priori estimates
\[
