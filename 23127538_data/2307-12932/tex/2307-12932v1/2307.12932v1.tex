...
\]
which are motivated by uniqueness and stability properties initiated in \cite{L82book,LinTadmor}. As a byproduct, we provide new insights about the vanishing viscosity process of Hamilton-Jacobi equations. More in detail, we address the following issues, referring to the introductory part within each section for a detailed comparison and the improvements with respect to the literature:
\begin{itemize}
...
\begin{rem}
	Definition \ref{semic}-(i) is equivalent to the existence of a constant $C>0$ such that $u(x,t)-\frac12 C|x|^2$ is concave on $\R^n$, see e.g. Proposition 5.2 of \cite{BardiDragoni} or \cite{CannarsaSinestrari}. Moreover, a result of P.-L. Lions \cite{Lions83} and H. Ishii \cite{IshiiFE} show that it is equivalent to the validity of $D^2u\leq CI_n$ for $t>0$ in the viscosity sense.\\
...
\begin{defn}\label{semicmix}
	 A function $u\in C(Q)$ is said to be $L^q_t(L^p_x)$-semiconcave if there exists a function $k(x,t)$ in  $L^q(0,T;L^p_{\mathrm{loc}}(\R^n))$, for   $1\leq q,p<\infty$,    and  in $L^q(0,T;L^\infty(\R^n))$, for $p=\infty$, such that
	\[
...
%\textcolor{red}{For general $p,q$ we have the following characterization that can be of independent interest:
%	\begin{prop}
%		Let $u\in C(Q)$. Then  $u$ is $L^p_t(L^p_x)$-semi-superharmonic with \textcolor{red}{$p\geq n+1$} if and only if $u$ solves $g-\Delta u\geq 0$ in the $L^p$-viscosity sense.  
%	\end{prop}
%	Da provare che se $u$ soddisfa Def. \ref{SSHp} e' equivalente a essere $L^p$ viscosity supersolution di $\Delta u=g$ nel senso di Caffarelli-Swiech.... seguendo Ishii....\\}
...
\end{defn}
\begin{rem}
	Both the definitions of solutions for \eqref{hjintroinv} satisfy  existence and  uniqueness results, and hold in the presence of the viscosity $\eps>0$. As for Definition \ref{visco} we refer to \cite{BCD,CEL} for general Hamiltonians $H$, while corresponding results for generalized solutions are due to S.N. Kruzhkov \cite{Kruzhkov}, but only in the case  $H\in C^2$ and convex in $p$. \\
	In P.-L. Lions \cite[Theorems 3.1 and 10.1]{L82book},  assumption (iii) in Definition \ref{def;weakae} was relaxed to $L^\infty_{x,t}$-SSH, obtaining existence and  uniqueness results for  SSH solutions to Hamilton-Jacobi equations. When $u$ is only $L^1_t(L^p_x)$-SSH and solves a stationary Hamilton-Jacobi equation with $H$ convex, the stability result was noted in \cite{L82book}. This is however conditional to the validity of unilateral bounds in $L^p$ of $\Delta u$ and cannot be reached, as the author in \cite{L82book} explains, using the sole maximum principle. We provide a way to derive these bounds in Section \ref{sec;semiLp}.
\end{rem}
...
We start reviewing the literature on (one-side) second-order bounds  for solution to Hamilton-Jacobi equations. To the authors' knowledge, there are three main approaches to derive semiconcavity estimates for  Hamilton-Jacobi equations: 
\begin{itemize}
...
\item[(c)] Another method requires to triple the number of variables, as in the doubling of variable argument in the viscosity solutions' theory, which avoids the differentiation of the equation. Indeed, the low regularity of viscosity solutions to first-order problems prevents from the formal differentiation of the problem as in the viscosity regularization described in (a).
\end{itemize}
All the above methods usually require convexity-type assumptions of the Hamiltonian. The parabolic regularization to prove semiconcavity (or SSH) properties was first used by S. N. Kruzhkov \cite{K66I,K67II}, see also P.-L. Lions \cite{L82book}. The method in (c) is due to H. Ishii and P.-L. Lions \cite{IL} for second-order fully nonlinear equations, see also \cite{Barles,Calder,CannarsaSinestrari} for a proof for first-order equations and \cite{Giga} for convexity preserving properties. We do not focus on the approach in (b), referring to \cite{CannarsaSinestrari} for further details, mentioning, however, that it allows weaker differentiability properties on $H$ and, sometimes, it encompasses also some non-convex cases, cf.  \cite[Theorem 5.3.9]{CannarsaSinestrari}. A survey on the previous approaches can be found in Section 5.3 of \cite{CannarsaSinestrari}, while other  recent results based on different  techniques can be found in \cite{Liu}.\\
...
D^2u_\eps\xi\cdot \xi\leq c_0+\int_0^Tc_f(t)\,dt+\widetilde{C}_{H,4}\tau, \quad \forall \xi\in\R^n,|\xi|=1.
\end{equation}
\end{thm}
\begin{rem}
...
\begin{rem}
The same approach of Theorem \ref{semiconc1} also leads to a discrete semiconcavity bound, as studied in Lemma 5.2 of \cite{LinTadmor}. Indeed, we can prove that
\begin{equation*}
...
When $H=H(p)$ is only convex, the equation satisfied by $u_{\xi\xi}$ reads as
\[
...
\begin{rem}
The obtainment of Lipschitz estimates typically requires to impose some mild coercivity assumptions on $H$ with respect to $Du$. This is done for instance in \cite{Barles,L82book} in the theory of viscosity or generalized solutions. Here what we really need is the conservation of mass for the dual problem solved by $\rho$. In the periodic setting this property is automatically satisfied by using the test function identically equal to $1$, and a locally Lipschitz $H$ is enough to run the argument. The whole space $\R^n$ requires more care, and some additional assumptions than the sole local Lipschitz continuity are needed, as for instance \eqref{H1}, cf. Lemma \ref{well}.
\end{rem}
...
Assume that $H$ satisfies \eqref{H4} and $f\equiv0$ (no further hypotheses are assumed on $u_0$). Then, any  solution to \eqref{hjintro} with $Du\in L^\infty_{x,t}$ satisfies for any $\delta\geq0$ the one-side estimate
\[
u_{\xi\xi}\leq\frac{C_1}{t}\|\delta+|Du|^2\|_\infty^{\frac{2-\gamma}{2}}+C_2t
\]
where $C_1,C_2$ depend on $C_{H,4},\widetilde{C}_{H,4}$, and do not depend neither on $\eps$ nor on $\delta$. The constant $C_2=0$ if $\widetilde{C}_{H,4}=0$.
\end{thm}
\begin{proof}
...
\]
Taking a smooth function $\chi:[0,T]\to\R$ to be chosen later  and setting $z=\chi w$ we have
\[
...
\end{cases}
\]
and we choose $\chi(t)=t^\alpha$ on $[0,\tau]$, $\alpha\geq2$, observing that this implies $\int_{\R^n}z(0)\rho(0)\,dx=0$. This choice is crucial to shift the time horizon away from the initial time $t=0$, and avoids to require second-order properties on the initial datum. We test the equation of $z$ by $\rho$ to get
\[
\int_{\R^n}z(\tau)\rho_\tau(x)\,dx+\iint_{Q_\tau}t^\alpha D^2_{pp}H(Du)Du_\xi\cdot Du_\xi\rho\,dxdt=\iint_{Q_\tau}\alpha t^{\alpha-1}u_{\xi\xi}\rho\,dxdt.
\]
On the one hand, we have
\[
\iint_{Q_\tau}t^\alpha D^2_{pp}H(Du)Du_\xi\cdot Du_\xi\rho\,dxdt\geq C_{H,4}\iint_{Q_\tau}t^{\alpha}|D^2u|^2(\delta+|Du|^2)^{\frac{\gamma-2}{2}}\rho\,dxdt-\widetilde{C}_{H,4}\int_0^\tau\int_{\R^n}t^\alpha\rho\,dxdt.
\]
On the other side, by the Young inequality we conclude
\[
\iint_{Q_\tau}\alpha t^{\alpha-1}u_{\xi\xi}\rho\,dxdt\leq C_H\iint_{Q_\tau}t^\alpha|D^2u|^2(\delta+|Du|^2)^{\frac{\gamma-2}{2}}\rho\,dxdt+\frac{\alpha^2}{4C_H}\iint_{Q_\tau}t^{\alpha-2}\rho(\delta+|Du|^2)^{-\frac{\gamma-2}{2}}\,dxdt.
\]
This implies
\[
\tau^\alpha u_{\xi\xi}\leq \frac{\alpha^2}{4C_H(\alpha-1)}\|(\delta+|Du|^2)^{\frac{2-\gamma}{2}}\|_\infty\tau^{\alpha-1}+\frac{\tau^{\alpha+1}}{\alpha+1},
\]
...
\]
by choosing $\alpha=2$ during the proof.
\end{rem}

\begin{rem}
In the subquadratic case, when $\widetilde{C}_{H,4}=0$, we recover by a different method estimate 3.2 in Proposition 3.2 of \cite{BKL}, and extend it to the superquadratic regime.
\end{rem}
...
\end{cases}
\]
with $\rho_\tau\in C_c^\infty(\R^n)$, $\rho_\tau\in L^1(\R^n)\cap L^{p'}(\R^n)$, $\rho_\tau\geq0$ and $\|\rho_\tau\|_{L^{p'}(\R^n)}=1$. This implies that
\[
\int_{\R^n}\rho_\tau(x)\,dx\leq C_1
\] 
where $C_1$ depends on $p'$ and the support of $\rho_\tau$. Moreover, we have
\begin{equation}\label{conserv}
\int_{\R^n}\rho(x,t)\,dxdt\leq C_2.
\end{equation}
...
A similar proof to those in Theorems \ref{semic>0} and \ref{aho} leads to the following (mild) regularization effect:
\begin{thm}\label{semic>0p}
Assume $H=H(p)$ as in Theorem \ref{semic>0} and $f=0$. 
%(no further hypotheses are assumed on $u_0$). 
Then, any solution to \eqref{hjintro} satisfies the one-side estimate
\[
\|(u_{\xi\xi})^+(t)\|_{L^p_{\mathrm{loc}}(\R^n)}\leq\frac{C}{t},
\]
where $C$ depends on $C_{H,4}$.
\end{thm}
...
\]
see \cite[Theorem 3.2]{BCD}. The rate would become slower if the Hamiltonian is  less regular, say only H\"older continuous  with exponent $\beta\in(0,1)$, and the corresponding rate would depend also on this new parameter. This latter point can be seen by a direct inspection of the proof in \cite[Theorem 3.2]{BCD}.\\
...
The previous bound holds  for nonconvex $W^{1,\infty}_{\mathrm{loc}}$ Hamiltonians, but it is conditional to the unilateral bound on $u_\eps$ which  usually requires convexity type assumptions. This improved rate has been first proved in Section 11 of \cite{L82book} using probabilistic methods under the assumption that $H$ is convex and the initial datum is SSH (i.e. $\Delta u_0\leq c_0$), and in \cite{BCD} using techniques from viscosity solutions' theory.\\
 ...
 In this section we provide a unifying method for proving rates of convergence in any $L^p$ norm $1\leq p\leq\infty$ using duality methods and properties of transport equations, extending all the previous results under more general and weaker assumptions on $H$. We further mention that the approach is flexible enough to cover all possible boundary conditions (periodic, Cauchy-Dirichlet, Neumann, whole space,...) as well as stationary problems. We will also give precise results on the size of the estimates taking care of the constants in the bounds.
...
\]
to make it globally bounded and ensure the conservation of mass for Fokker-Planck equations. Lipschitz estimates are in general known under rather general conditions of coercivity, an example being \eqref{H1}, those in Corollary 4.1 p.100 in \cite{L82book}, Section 8 in \cite{Barles} or Chapter 1 in \cite{TranBook}.\\
...
\begin{thm}\label{ev1}
Let $H\in W^{1,\infty}_{\mathrm{loc}}(\R^n)$, $u_0\in W^{1,\infty}(\R^n)$ and $u_\eps$ be a solution to \eqref{hjsemi} with $f=0$. Then, $u_\eps$ converges in $L^\infty(\R^n)$ to the viscosity solution $u\in W^{1,\infty}(Q)$ of the first-order Hamilton-Jacobi equation
\eqref{first}. Moreover, we have the rate 
\[
...
\end{thm}
\begin{proof}
We can assume that $H\in W^{1,\infty}(\R^n)$ by the validity of a global Lipschitz estimate by Lemma \ref{grad}. Indeed, one can consider (since $f=0$), setting $R=\|Du_0\|_\infty$, the truncated Hamiltonian
\[
...
\]
We use the B\"ochner's identity to find, setting $g=|Du_\eps|^2$
\[
...
\begin{align*}
\int_{\R^n}w(\tau)\rho_\tau(x)\,dx&\leq (\eps-\eta)\iint_{Q}\Delta u_\eps\rho\,dxdt\leq (\eps-\eta)\sqrt{n}\iint_{Q}|D^2u_\eps|\rho\,dxdt\\
...
\end{align*}
This gives the estimate on $\|(w^+)\|_{L^\infty}$. The estimate on the negative part is similar.
\end{proof}
\begin{rem}
...
\end{rem}
\begin{rem}
The rate of convergence of order $\mathcal{O}(\sqrt{\eps})$ is valid even when $H$ is only continuous, as shown in the Remark at p. 18 of \cite{CL83tams}, but this weak assumption is outside the scope of our treatment.
\end{rem}
...
\]
As in Theorem \ref{inftyrate1s} we linearize the equation and then introduce the adjoint problem \eqref{adjrate} with terminal datum $\rho_\tau\in C_0^\infty(\R^n)$, $\rho_\tau\geq0$, $\rho_\tau\in L^1(\R^n)\cap L^{p'}(\R^n)$, $\|\rho_\tau\|_{p'}=1$. Note that, arguing as in Theorem \ref{aho}, we have
\begin{equation}\label{L1p}
\int_{\R^n}\rho_\tau(x)\,dx\leq C_1\quad \textrm{and}\quad  \int_{\R^n}\rho(x,t)\,dx\leq \widetilde C_1,\ \forall t\in[0,\tau),
\end{equation} 
where $\widetilde C_1$ depends on $p'$ and the support of $\rho_\tau$. Moreover, $\rho\geq0$ by the maximum principle. By duality, we obtain
\[
...
\begin{rem}
One can remove the bound on $\|(\Delta u_\eps)^+\|_{L^1(0,T;L^\infty(\R^n))}$ in Theorem \ref{prate1}, as in Corollary \ref{rateSSH}, using the one-side bounds in Theorems \ref{inftyrate1s} and \ref{semiconc1} and obtain a more precise estimate.
\end{rem}
...
\[
\partial_t w-\eta\Delta w+D_pH(Du_\eta)\cdot Dw\leq (\eps-\eta)\Delta u_\eps, w(0)=0.
\]
Now, we consider the adjoint problem 
\begin{equation*}
\begin{cases}
\partial_t \rho-\eta\Delta \rho-\mathrm{div}(D_pH(Du_\eta)\rho)=0&\text{ in }\R^n\times(0,\tau)\\
\rho(x,\tau)=\rho_\tau(x)&\text{ in }\R^n.
\end{cases}
\end{equation*}
with terminal datum $\rho_\tau:=|w|^{p-1}\mathrm{sgn}(w)/\|w\|^{p-1}_{L^p}$ that belongs only to $L^{p'}$ having $\|\rho_\tau\|_{p'}\leq1$ (and not on the intersection $L^1\cap L^{p'}$). This yields a (global) bound on $(u_\eps-u_\eta)^+(t)\in L^p(\R^n)$ for all $t\in(0,T)$. However, one has to estimate the term involving $\Delta u_\eps$ on the right-hand side of the equation satisfied by $w$ by the H\"older's inequality as follows:
\[
...
We now address $L^p$ rates of convergence using techniques from transport equations, as in \cite{Bris}, or duality methods, as in \cite{LinTadmor}. In this setting we need to strengthen the requirement both on $u_\eps$ and $u_\eta$, and work in a compact state space as the flat torus, but we are able to get a two side control on the difference $u_\eps-u_\eta$ in any $L^p$ space and for semiconcave or semi-superharmonic solutions. The same estimate could be obtained for boundary-value problems. We premise the following lemma concerning superharmonic and semiconcave functions to study the $L^1$ case.
...
To prove (ii), it is enough to note that $u$ is semiconcave in $\R^n$ if and only if $v(x)=u(x)-C|x|^2$ is concave in $\R^n$, and also that $v$ is concave if and only if it is concave when restricted to any line that intersects its domain, cf. \cite{BardiDragoni}. Using these properties, we can follow an argument of \cite{K65}. It is enough to prove that for a $C^2$ function $v$ on the bounded interval $[a-h,b+h]$, $a,b\in\R$, $h>0$, which satisfies $|v'(x)|\leq K_0$ and $v''(x)\leq K_1$ (recall that a semiconcave function is also Lipschitz continuous, cf. \cite{CannarsaSinestrari}), we have
\[
...
\end{rem}
The next is the main result of the section.
...
\[
\|(u_\eps-u_\eta)(t)\|_{L^p(\T^n)}\leq C(\eps-\eta),\ \eps\geq\eta\geq0,\;\; 1 \leq p \leq \infty,
\]
where $C$ depends on the semiconcavity constant of $u_\eps$.
\end{thm}
\begin{proof}
This in inspired from \cite{K66d,K75}. Consider $w=u_\eps-u_\eta$ satisfying the equation
\[
\partial_t w-\eta\Delta w+H(Du_\eps)-H(Du_\eta)= (\eps-\eta)\Delta u_\eps, w(0)=0,
\]
We thus have
...
We next proceed with a different proof for $p=1$ as in Theorem \ref{pSSHrate}, considering the equation satisfied by the difference $w=u_\eps-u_\eta$ and by using the adjoint method. Here, we consider again the adjoint problem \eqref{adjrate}, but with terminal datum $\rho(\tau)=\mathrm{sgn}(w(\tau))$ on $\T^n$.
...
\begin{multline*}
\int_{\T^n}w(\tau)\rho_\tau(x)\,dx=\int_{\R^n}w(0)\rho(0)+(\eps-\eta)\int_0^T\int_{\T^n}\Delta u_\eps\rho\,dxdt\\
=\|w(0)\|_{L^1(\T^n)}\|\rho(0)\|_{L^\infty(\T^n)}+ (\eps-\eta)\|\Delta u_\eps\|_{L^1(Q_\tau)}\|\rho\|_{L^\infty(Q_\tau)}\leq C\|\rho\|_{L^\infty(Q_\tau)}(\eps-\eta).
\end{multline*}
It remains to estimate $\|\rho\|_{L^\infty(Q_\tau)}$ in terms of the terminal datum of the adjoint problem, since $w(0)=0$. A classical stability estimate for continuity equations that follows from the Gronwall's inequality or the Feynman-Kac formula \cite[Theorem 4.12]{Figalli} leads to
\[
\|\rho(t)\|_{L^\infty(\T^n)}\leq \|\rho(\tau)\|_{L^\infty(\T^n)}e^{\int_0^T\|(-\mathrm{div}(b))^-\|_{L^\infty(\T^n)}},\ t\in[0,\tau).
\]
We can then conclude with the case $1<p<2$ by interpolation, using the result for $p=1$ and those already obtained for $p\geq2$.
\end{proof}

\begin{cor}
Under the assumptions of Theorem \ref{semiconc1}, let $u_\eps$ and $u_\eta$ with viscosity $\eps$ and, respectively, $\eta$. Then
\[
\|u_\eps-u_\eta\|_{L^p(\T^n)}\leq C\eps,\ 1\leq p\leq \infty,
\]
where $C$ depends on $c_f(t)\in L^1(0,T)$, $c_0$ and the constants $C_{H,4}$ appearing in \eqref{H4}.
\end{cor}

\begin{rem}
The estimate in Theorem \ref{rateL1} was stated in Theorem 4 of \cite{K75}, eq. (16), in $L^1$ norms with order $\mathcal{O}(\eps^\nu)$, $\nu\in(0,1)$, and in eq. (17) of  \cite{K75} in $L^p$ norms for $p$ sufficiently large, with a slower order. The method of proof is reminiscent to the one we outlined here.
\end{rem}



\begin{rem}
It is worth remarking that under the assumption of the semi-superharmonic estimate in Theorem \ref{semiconc1}, Lemma \ref{laplL1} combined with the bound $\|u_\eps-u_\eta\|_{L^\infty}\leq C(\eps-\eta)$ obtained in Theorem \ref{rateL1} imply the following integral a priori estimate
\[
\int_0^T\int_{\T^n}|D(u_\eps-u_\eta)|^2\,dxdt\leq C(\eps-\eta).
\]
This slightly improves Remark 6.9 in \cite{L82book}, being valid for possibly nonconvex $H$ (i.e. under \eqref{H1}-\eqref{H4}) and with a better rate ($\mathcal{O}(\sqrt{\epsilon})$ instead of $\mathcal{O}(\epsilon^\frac14)$). Indeed, an integration by parts and Lemma \ref{laplL1} give
\begin{align*}
\int_0^T\int_{\T^n}|D(u_\eps-u_\eta)|^2\,dxdt&=-\int_0^T\int_{\T^n}(u_\eps-u_\eta)\Delta(u_\eps-u_\eta)\,dxdt\\
&\leq \|u_\eps-u_\eta\|_{L^\infty}\|\Delta(u_\eps-u_\eta)\|_{L^1}\leq C(\eps-\eta).
\end{align*}
This implies
\[
\|Du_\eps-Du_\eta\|_{L^2(Q)}\leq C\sqrt{\eps-\eta}.
\]
S.N. Kruzhkov obtained in \cite{K66d} a $L^1$ rate of convergence of the gradient with order $\mathcal{O}(\eps^\frac12)$ using Gagliardo-Nirenberg inequalities and (ii) in Lemma \ref{laplL1}. A direct integration in the one-side interpolation argument of Lemma \ref{onesideinterp} shows
\[
...
\begin{thm}
Assume that $H,f\in W^{1,\infty}_{\mathrm{loc}}(\R^n)$. Then
\[
...
\end{thm}
\begin{rem}
The $L^p$ rate of the previous result can be improved by using the parabolic transformation and exploiting the two-side estimates in Theorem \ref{rateL1}.
\end{rem}
...
To conclude, let us so consider the function $w^{\eps} = u^{\eps} - \bar{u}^{\eps}$ that solves the equation 
	\begin{equation*}
	w^{\eps} + H\left(\frac{x}{\eps}, Du^{\eps}\right) - H\left(\frac{x}{\eps}, D\bar{u}^{\eps}\right) = \eps v\left(\frac{x}{\eps}\right).
	\end{equation*}
	in the viscosity sense. In order to get the estimate we proceed by approximation considering 
	...
	\end{equation*}
	This yields the conclusion since the constant on the right-hand side is independent of $\sigma$ and letting $\sigma \downarrow 0$ we obtain 
	\begin{equation*}
...
\]
equipped with an affine initial condition of the form $u_0(x)=\alpha+p\cdot x,\ \alpha\in\R,\ p\in\R^n$, the solution of the homogenized parabolic problem is given by $u(x,t)=\alpha+p\cdot x-t\overline{H}(p)$, cf. Section I.2 of \cite{LPV}. To do this, we first consider the formal asymptotic expansion given by
\[
\bar u^{\eps}(x,t)=u^\eps(x,t)+\eps v\left(\frac{x}{\eps}\right),
\]
and, as in \cite{LPV}, one finds the PDE
\[
...
Let $u_0$ be bounded and assume the hypotheses on $H$ of Theorem \ref{semic>0} with $\widetilde{C}_{H,4}$. Let $u_\eps$ be any bounded classical solution to \eqref{hjregularizing}. Then, for any $\delta>0$ we have $u_\eps\in W^{1,\infty}(\R^n\times(\delta,T))$.
\end{thm}
\begin{proof}
	We drop the subscript $\eps$ for simplicity of notation. We have for all $\tau\in(0,\infty)$
	\[
...
\end{equation}
when $H$ satisfies (H4) with $\gamma\in(1,2]$.  Our main result is inspired from the Liouville type property appeared in Section 1 of \cite{K67II}, and extends it to more general Hamiltonians. These kind of results are rather unnatural due to the absence of diffusive terms in the equation. We may assume without loss of generality that $H(0)=0$, otherwise replace $u$ with $\bar u$ solving
\[
\bar u=u-tH(0)
\]
...
\end{thm}
\begin{rem}
The previous decay condition is satisfied if $u$ bounded from below or has   sublinear decay at infinity. In such a case $\mu(r)=r^{\alpha-1}$, $r=|x|$, $\alpha\in(0,1)$, satisfies the condition $\mu(r)\to0\text{ as }r\to\infty$ with $\mu(r)\geq0$.
\end{rem}
