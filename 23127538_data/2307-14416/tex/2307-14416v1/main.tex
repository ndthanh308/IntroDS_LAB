...
%Globally, however, each of the two AdS patches forming $\Co$ are strictly contained within the WDW patch covered by the metric \eqref{eq:flrwintro}, since they are restricted to the domain $\rho < \rho_{\max}(t_c)$, where $\rho_{\max}(t_c)$ parametrizes the radial trajectory of the shell in these coordinates (see Fig. \ref{fig:WDWpatch}). This simple cosmological model is thus inhomogeneous due to the presence of the thin shell. In Sec. \ref{sec:2} we show that the size of the spatial slices shrinks to zero at $t_c=\pm t_f$, with $t_f<\pi\ell/2$, corresponding to the endpoints of the shell's collapse. The spacetime curvature is singular at these points. They define the big bang and the big crunch singularities of the cosmological spacetime $D(\Co)$. 
...
\subsection*{Cosmology as an entanglement island}
 ...
 In the present setup, the entropy of the bulk fields is $O(G^0)$ and therefore subleading with respect to any ${O}(G^{-1})$ area term. This implies that all possible genometrically non-empty extremal surfaces are disfavored and that the RT surface is indeed empty, $\gamma_\Le=\emptyset$, for the CFT${}_\Le$.\footnote{An interesting classical extremal surface is given by the initial sphere associated to the position of the shell, at $r=R_*$, in the time reflection-symmetric slice $\Sigma$. This surface has index 1 at least, which means that the surface is maximal in at least one direction of deformation space, as opposed to the candidate RT surfaces, which are always minimal surfaces. In the regime where the quantum-corrected RT prescription is valid, the whole cosmology resembles a python's lunch, encoded in either CFT${}_\Le$ or CFT${}_\Ri$.}  However, due to the presence of the closed cosmology, it is still neccesary to specify the homology hypersurface $\Sigma_{\Le}$, with $\partial \Sigma_{\Le} = \Le$. The two possibilities are:
\begin{itemize}
...
%\footnote{The fact that the cosmology is a closed space is evaded one might be running into problems, given that the $G=0$ EFT of free gravitons and matter is not a good approximation to the semiclassical EFT, and the latter requires the implementation of the gravitational Gauss' law perturbatively.}

%For a specific bulk reference state $\ket{\psi}$ prepared with the Euclidean path integral we consider the cosmological EFT on top of this state as the set of all states attained by applying local unitaries supported on the DOF on the cosmology, $\mathcal{H}_\Co = \lbrace U_\Co\ket{\psi}: U_\Co U_\Co^\dagger = \mathbb{1}, \text{supp}(U_{\Co}) = \Co \rbrace \subset \mathcal{H}_\Sigma$. The formal map to the CFTs
%\be 
%V:\mathcal{H}_\Co\rightarrow \mathcal{H}_{\Le}\otimes \mathcal{H}_\Ri\;,
%\ee 
%defines the cosmology-to-boundary map for us.

%From the Euclidean preparation of the PETS, we can 

%In order to define a bulk-to-boundary map, we first need to specify what are the bulk and boundary theories we are mapping between.
...
%
\be
    L=\ell \log\left[2\tanh\left(\frac{\tau_*}{2}\right)\right],
...
%
\be
\alpha=\left[\frac{\tanh\left(\frac{\beta_\Ri}{4}-\frac{\tau_*^{i,X}}{2}\right)}{\tanh\left(\frac{\tau_*^{i,X}}{2}\right)}\right]^{-m_i\ell}\left[\frac{\tanh\left(\frac{\beta_\Ri}{4}-\frac{\tau_*^{j,X}}{2}\right)}{\tanh\left(\frac{\tau_*^{j,X}}{2}\right)}\right]^{-m_j\ell}.
\ee
%
...
The rank of the $K\times K$ Gram matrices of overlaps \eqref{eq:gram} and \eqref{eq:gramCFT} provide information about the number of linearly independent states on $S$ and on $V_\psi S = \lbrace \ket{\Psi_{I_1}},...,\ket{\Psi_{I_K}}\rbrace $. For the former, there are $K$ independent states by assumption,
\be
...
%The time-ordering of all operator insertions including $\mathcal{O}_\phi$ is thus dependent on the specific state $\ket{\Psi_I}$ we are starting from.
...
S_{\lef}+S_{\ri}  \gtrsim  \dfrac{2(d^2+1)}{d^2-1}\dfrac{1}{1-2\gamma}\,\log N \;,
\ee 
...
%A heuristic argument suggesting why such disconnected configurations are subdominant from the gravity side comes from the entropic dominance of connected shell configurations. Naively, we have two operators, $\Op$ and $\Op^\dagger$, with $n$ local operator insertions on each of them. There are thus roughly $n! \sim \exp(n\log n)$ ways to connect the trajectories of the particles forming the two operators, so that the coarse-grained trajectory is that of a coherent and approximately spherically symmetric thin shell. For the disconnected configurations described above, the local operator contractions are between the $n$ insertions in each one of the two spheres, so that there are roughly $(\frac{n!}{\frac{n}{2}!2^n})^2\sim \exp(n\log n - n\log 2)$ microscopic configurations instead. As we take the number of operator insertions to be parametrically large, $n \sim N^2$, the disconnected configurations, and thus their weight in the semiclassical wavefunction of the PETS, are exponentially suppressed in $N^2$ (by a factor $2^{-n}$), according to this simple argument.
